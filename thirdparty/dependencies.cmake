...
if(JPEG_VERSION AND JPEG_VERSION VERSION_GREATER_EQUAL 80)
    set(JPEG_NAMES ${JPEG_NAMES} jpeg libjpeg)
    find_library(JPEG_LIBRARIES NAMES ${JPEG_NAMES})
    if(JPEG_LIBRARIES)
        set(JPEG_FOUND TRUE)
    endif ()
endif()
if(JPEG_FOUND)
    ...
    message(STATUS "JPEG is found, JPEG_INCLUDE_DIR=${JPEG_INCLUDE_DIR}, JPEG_LIBRARIES=${JPEG_LIBRARIES}, JPEG_VERSION=${JPEG_VERSION}") 
else()
    message(STATUS "JPEG not found in system, using libgif in thirdparty/jpeg") 
    include(thirdparty/jpeg/jpeg.cmake)
endif()
...
if(PNG_INCLUDE_DIR)
    file(STRINGS "${PNG_INCLUDE_DIR}/png.h" png_lib_version REGEX "^#define[ \t]+PNG_LIBPNG_VER_STRING[ \t]+\".+\"")
    string(REGEX REPLACE "^#define[ \t]+PNG_LIBPNG_VER_STRING[ \t]+\"([^\"]+)\".*" "\\1" PNG_VERSION "${png_lib_version}")
    unset(png_lib_version)
endif()
if(PNG_VERSION AND PNG_VERSION VERSION_GREATER_EQUAL 1.6)
...
if(PNG_FOUND)
    message(STATUS "PNG is found, PNG_INCLUDE_DIR=${PNG_INCLUDE_DIR}, PNG_LIBRARIES=${PNG_LIBRARIES}, PNG_VERSION=${PNG_VERSION}") 
else()
    message(STATUS "PNG not found in system, using libgif in thirdparty/png") 
    include(thirdparty/png/png.cmake)
endif()
...
if(GIF_VERSION AND GIF_VERSION VERSION_GREATER_EQUAL 5.1)
    set(GIF_NAMES ${GIF_NAMES} gif libgif)
    find_library(GIF_LIBRARIES NAMES ${GIF_NAMES})
    if(GIF_LIBRARIES)
        set(GIF_FOUND TRUE)
    endif ()
endif()
if(GIF_FOUND)
    message(STATUS "GIF is found, GIF_INCLUDE_DIR=${GIF_INCLUDE_DIR}, GIF_LIBRARIES=${GIF_LIBRARIES}, GIF_VERSION=${GIF_VERSION}") 
else()
    message(STATUS "GIF not found in system, using libgif in thirdparty/gif") 
    include(thirdparty/gif/gif.cmake)
endif()
