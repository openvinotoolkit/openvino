...
struct NpyArray {
  ...
  NpyArray(
    ...
    for (size_t i = 0; i < shape.size(); i++)
      num_vals *= shape[i];
    if (word_size && num_vals > (GetFreeMemorySize() / word_size))
      throw std::length_error(
          "NpyArray of " + std::to_string(num_vals) + "*" +
          std::to_string(word_size) + " elements is too big.");
    try {
...
void npz_save(
  ...
  // write everything
  if (fp) {
    if (fwrite(&local_header[0], sizeof(char), local_header.size(), fp) !=
        local_header.size()) {
      fclose(fp);
      throw std::runtime_error("npz_save: failed fwrite");
    }
    if (fwrite(&npy_header[0], sizeof(char), npy_header.size(), fp) !=
        npy_header.size()) {
      fclose(fp);
      throw std::runtime_error("npz_save: failed fwrite");
    }
    ...
    if (fwrite(&global_header[0], sizeof(char), global_header.size(), fp) !=
        global_header.size()) {
      fclose(fp);
      throw std::runtime_error("npz_save: failed fwrite");
    }
