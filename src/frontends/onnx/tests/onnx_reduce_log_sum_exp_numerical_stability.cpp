...
class ReduceLogSumExpNumericalStabilityTest : public ::testing::Test {
protected:
    void SetUp() override {
        frontend = std::make_shared<ov::frontend::onnx::FrontEnd>();
    }

    std::shared_ptr<ov::frontend::onnx::FrontEnd> frontend;
};

TEST_F(ReduceLogSumExpNumericalStabilityTest, TestNumericalStability) {
    // Test cases that would cause overflow with naive implementation
    // but should work with numerically stable implementation
    ...
    // Test case 1: Large positive values that would overflow exp() in naive implementation
    {
        ...
        // Create the numerically stable ReduceLogSumExp manually to test the implementation
        auto k = std::make_shared<ov::opset13::ReduceMax>(input, axes, true);
        auto input_minus_k = std::make_shared<ov::opset13::Subtract>(input, k);
        auto exp_node = std::make_shared<ov::opset13::Exp>(input_minus_k);
        auto sum_node = std::make_shared<ov::opset13::ReduceSum>(exp_node, axes, false);
        auto log_node = std::make_shared<ov::opset13::Log>(sum_node);
        auto k_squeezed = std::make_shared<ov::opset13::Squeeze>(k, axes);
        auto result = std::make_shared<ov::opset13::Add>(k_squeezed, log_node);
...
TEST_F(ReduceLogSumExpNumericalStabilityTest, CompareNaiveVsStable) {
    // This test shows why the stable implementation is necessary
    
    std::vector<float> test_values = {88.0f, 88.0f};  // Values that cause issues in naive implementation
