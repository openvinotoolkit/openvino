...
TEST_P(FrontEndLoadFromTest, testLoadUsingSimpleGraphIterator) {
    ...
    class SimpleIterator : public ov::frontend::tensorflow_lite::GraphIterator {
    public:
    ...
    {
        auto graph_iter = std::dynamic_pointer_cast<ov::frontend::tensorflow_lite::GraphIterator>(iter);
        ASSERT_NO_THROW(m_frontEnd = m_fem.load_by_framework("tflite"))
            << "Could not create the TFLITE FE using a pointer GraphIterator";
