...
def get_node_model(op_type, *input_data, opset=1, num_outputs=1, **node_attributes):
    ...
    node_output_names = [
        ascii_uppercase[num_inputs + idx] for idx in range(num_outputs)
    ]
    onnx_node = make_node(
        op_type, node_input_names, node_output_names, **node_attributes
    )
...
def all_arrays_equal(first_list, second_list):
    ...
    """
    return all(
        map(lambda pair: np.array_equal(*pair), zip(first_list, second_list))
    )  # noqa: C417
