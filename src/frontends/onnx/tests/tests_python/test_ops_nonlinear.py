...
def assert_onnx_import_equals_callable(onnx_op_type, python_function, data, **kwargs):
    ...
    assert np.allclose(
        import_and_compute(onnx_op_type, data, **kwargs),
...
def test_relu():
    ...
    assert_onnx_import_equals_callable(
        "Relu", relu, [-0.9, -0.8, -0.7, -0.4, -0.3, -0.2, -0.1]
    )
...
def test_leaky_relu():
    ...
    assert_onnx_import_equals_callable(
        "LeakyRelu", leaky_relu, [-2, -1.0, 0.0, 1.0, 2.0], alpha=0.5
    )
    ...
    assert_onnx_import_equals_callable(
        "LeakyRelu", leaky_relu, [[1, 2, 3], [4, 5, 6]], alpha=0.2
    )
    assert_onnx_import_equals_callable(
        "LeakyRelu", leaky_relu, [[-3, -2, -1], [1, 2, 3]]
    )
...
def test_parametric_relu(value, slope):
    ...
    value, slope = np.array(value).astype(np.float32), np.array(slope).astype(
        np.float32
    )
