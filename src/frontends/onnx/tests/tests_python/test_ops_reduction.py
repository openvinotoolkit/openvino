...
from tests.tests_python.utils import import_onnx_model, run_node

from tests import xfail_issue_99962

reduce_data = np.array(
    ...
    graph = onnx.helper.make_graph(
        ...
        [
            onnx.helper.make_tensor_value_info(
                "x", onnx.TensorProto.FLOAT, data_input.shape
            )
        ],
...
def test_reduce_operation_keepdims_none_axes(operation, ref_operation):
    assert np.array_equal(
        import_and_compute(operation, reduce_data, keepdims=True),
        ref_operation(reduce_data, keepdims=True),
    )


@pytest.mark.parametrize(
    ("operation", "ref_operation"), reduce_operation_parameters_as_attr
)
...
def test_reduce_operation_keepdims_with_axes_as_attr(operation, ref_operation, axes):
    assert np.array_equal(
        import_and_compute(operation, reduce_data, axes=axes, keepdims=True),
        ref_operation(reduce_data, keepdims=True, axis=axes),
    )


@pytest.mark.parametrize(
    ("operation", "ref_operation"), reduce_operation_parameters_as_const
)
@pytest.mark.parametrize("axes", reduce_axis_parameters)
def test_reduce_operation_keepdims_with_axes_as_const(operation, ref_operation, axes):
    assert np.array_equal(
        import_and_compute_with_axes_as_const(
            operation, reduce_data, axes, keepdims=True
        ),
        ref_operation(reduce_data, keepdims=True, axis=axes),
    )
...
@pytest.mark.parametrize(
    "axes", [None, (0,), (1,), (2,), (0, 1), (0, 2), (1, 2), (0, 1, 2)]
)
@pytest.mark.parametrize(
    ("operation", "ref_operation"), reduce_operation_parameters_as_attr
)
...
def test_reduce_operation_no_keepdims_axes_as_attr(operation, ref_operation, axes):
    if axes:
        assert np.array_equal(
            import_and_compute(operation, reduce_data, axes=axes, keepdims=False),
            ref_operation(reduce_data, keepdims=False, axis=axes),
        )
    else:
        assert np.array_equal(
            import_and_compute(operation, reduce_data, keepdims=False),
...
def test_reduce_operation_no_keepdims_axes_as_const(operation, ref_operation, axes):
    if axes:
        assert np.array_equal(
            import_and_compute_with_axes_as_const(
                operation, reduce_data, axes, keepdims=False
            ),
            ref_operation(reduce_data, keepdims=False, axis=axes),
        )
    else:
...
def test_reduce_l1(reduction_axes):
    ...
    node = onnx.helper.make_node(
        "ReduceL1", inputs=["x"], outputs=["y"], axes=reduction_axes
    )
    ...
    node = onnx.helper.make_node(
        "ReduceL1", inputs=["x"], outputs=["y"], keepdims=0, axes=reduction_axes
    )
    graph_result = np.array(run_node(node, [input_data]).pop())
    assert np.array_equal(expected.shape, graph_result.shape)
    assert np.allclose(expected, graph_result)
...
def test_reduce_l2(reduction_axes):
    ...
    expected = np.sqrt(
        np.sum(np.square(input_data), keepdims=True, axis=reduction_axes)
    )
    node = onnx.helper.make_node(
        "ReduceL2", inputs=["x"], outputs=["y"], axes=reduction_axes
    )
    ...
    expected = np.sqrt(
        np.sum(np.square(input_data), keepdims=False, axis=reduction_axes)
    )
    node = onnx.helper.make_node(
        "ReduceL2", inputs=["x"], outputs=["y"], keepdims=0, axes=reduction_axes
    )
    graph_result = np.array(run_node(node, [input_data]).pop())
    assert np.array_equal(expected.shape, graph_result.shape)
    assert np.allclose(expected, graph_result)
...
def test_reduce_log_sum(reduction_axes):
    ...
    node = onnx.helper.make_node(
        "ReduceLogSum", inputs=["x"], outputs=["y"], axes=reduction_axes
    )
    ...
    node = onnx.helper.make_node(
        "ReduceLogSum", inputs=["x"], outputs=["y"], keepdims=0, axes=reduction_axes
    )
    graph_result = run_node(node, [input_data]).pop()
    assert np.array_equal(expected.shape, graph_result.shape)
    assert np.allclose(expected, graph_result)
...
def test_reduce_log_sum_default_axes():
    ...
    node = onnx.helper.make_node(
        "ReduceLogSum", inputs=["x"], outputs=["y"], keepdims=0
    )
    graph_result = np.array(run_node(node, [input_data]).pop())
    assert np.array_equal(expected.shape, graph_result.shape)
    assert np.allclose(expected, graph_result)
...
def test_reduce_log_sum_exp():
    ...
    assert np.array_equal(
        import_and_compute("ReduceLogSumExp", data), logsumexp(data, keepdims=True)
    )
    assert np.array_equal(
        import_and_compute("ReduceLogSumExp", data, keepdims=0),
        logsumexp(data, keepdims=False),
    )

    assert np.array_equal(
        import_and_compute("ReduceLogSumExp", data, axes=(1,)),
    ...
    assert np.array_equal(
        import_and_compute("ReduceLogSumExp", data, axes=(0, 2)),
...
def test_reduce_sum_square(reduction_axes):
    ...
    node = onnx.helper.make_node(
        "ReduceSumSquare", inputs=["x"], outputs=["y"], axes=reduction_axes
    )
    graph_result = np.array(run_node(node, [input_data]).pop())
    assert np.array_equal(expected.shape, graph_result.shape)
    assert np.allclose(expected, graph_result)
...
def test_reduce_sum_square_default_axes():
    ...
    node = onnx.helper.make_node(
        "ReduceSumSquare", inputs=["x"], outputs=["y"], keepdims=0
    )
    graph_result = np.array(run_node(node, [input_data]).pop())
    assert np.array_equal(expected.shape, graph_result.shape)
    assert np.allclose(expected, graph_result)
...
def test_reduce_argmin():
    ...
    assert np.array_equal(
        import_and_compute("ArgMin", data, axis=0), argmin(data, keepdims=True, axis=0)
    )
    assert np.array_equal(
        import_and_compute("ArgMin", data, axis=0, keepdims=0),
        argmin(data, keepdims=False, axis=0),
    )
    assert np.array_equal(
        import_and_compute("ArgMin", data, axis=1), argmin(data, keepdims=True, axis=1)
    )
    assert np.array_equal(
        import_and_compute("ArgMin", data, axis=1, keepdims=0),
        argmin(data, keepdims=False, axis=1),
    )
    assert np.array_equal(
        import_and_compute("ArgMin", data, axis=2), argmin(data, keepdims=True, axis=2)
    )
    assert np.array_equal(
        import_and_compute("ArgMin", data, axis=2, keepdims=0),
...
def test_reduce_argmax():
    ...
    assert np.array_equal(
        import_and_compute("ArgMax", data, axis=0), argmax(data, keepdims=True, axis=0)
    )
    assert np.array_equal(
        import_and_compute("ArgMax", data, axis=0, keepdims=0),
        argmax(data, keepdims=False, axis=0),
    )
    assert np.array_equal(
        import_and_compute("ArgMax", data, axis=1), argmax(data, keepdims=True, axis=1)
    )
    assert np.array_equal(
        import_and_compute("ArgMax", data, axis=1, keepdims=0),
        argmax(data, keepdims=False, axis=1),
    )
    assert np.array_equal(
        import_and_compute("ArgMax", data, axis=2), argmax(data, keepdims=True, axis=2)
    )
    assert np.array_equal(
        import_and_compute("ArgMax", data, axis=2, keepdims=0),
