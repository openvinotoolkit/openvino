...
def import_and_compute(
    op_type, input_data_left, input_data_right, opset=7, **node_attributes
):
    inputs = [np.array(input_data_left), np.array(input_data_right)]
    onnx_node = onnx.helper.make_node(
        op_type, inputs=["x", "y"], outputs=["z"], **node_attributes
    )
...
def test_add_opset4():
    assert np.array_equal(
        import_and_compute("Add", 1, 2, opset=4), np.array(3, dtype=np.float32)
    )
    ...
    assert np.array_equal(
        import_and_compute("Add", [1, 2], [3, 4], opset=4),
        np.array([4, 6], dtype=np.float32),
    )

    assert np.array_equal(
        import_and_compute("Add", [1, 2, 3], [4, 5, 6], opset=4),
        np.array([5, 7, 9], dtype=np.float32),
    )

    assert np.array_equal(
        import_and_compute(
            "Add", [[1, 2, 3], [4, 5, 6]], [7, 8, 9], broadcast=1, opset=4
        ),
    ...
    assert np.array_equal(
        import_and_compute("Add", left_operand, 8, broadcast=1, opset=4),
        left_operand + 8,
    )
    ...
    assert np.array_equal(
        import_and_compute(
            "Add", left_operand, right_operand, broadcast=1, axis=1, opset=4
        ),
    ...
    assert np.array_equal(
        import_and_compute(
            "Add", left_operand, right_operand, broadcast=1, axis=0, opset=4
        ),
...
def test_add_opset7(left_shape, right_shape):
    ...
    assert np.array_equal(
        import_and_compute("Add", left_input, right_input), left_input + right_input
    )


def test_sub():
    assert np.array_equal(
        import_and_compute("Sub", 20, 1), np.array(19, dtype=np.float32)
    )
    ...
    assert np.array_equal(
        import_and_compute("Sub", [20, 19], [1, 2]),
        np.array([19, 17], dtype=np.float32),
    )

    assert np.array_equal(
        import_and_compute(
            "Sub", [[1, 2, 3], [4, 5, 6]], [7, 8, 9], opset=6, broadcast=1
        ),
...
def test_mul():
    assert np.array_equal(
        import_and_compute("Mul", 2, 3), np.array(6, dtype=np.float32)
    )
    ...
    assert np.array_equal(
        import_and_compute("Mul", [2, 3], [4, 5]), np.array([8, 15], dtype=np.float32)
    )

    assert np.array_equal(
        import_and_compute(
            "Mul", [[1, 2, 3], [4, 5, 6]], [7, 8, 9], opset=6, broadcast=1
        ),
...
def test_div():
    assert np.array_equal(
        import_and_compute("Div", 6, 3), np.array(2, dtype=np.float32)
    )
    ...
    assert np.array_equal(
        import_and_compute("Div", [6, 8], [3, 2]), np.array([2, 4], dtype=np.float32)
    )

    assert np.array_equal(
        import_and_compute(
            "Div", [[10, 20, 30], [40, 50, 60]], [2, 5, 6], opset=6, broadcast=1
        ),
