...
OPENVINO_TEST(${BACKEND_NAME}, onnx_model_lstm_rank5_squeeze) {
    if (std::string("${BACKEND_NAME}") == std::string("IE_GPU")) {
        GTEST_SKIP() << "GPU backend has known accuracy issues with LSTMSequence Y_h/Y_c outputs. "
                     << "Root cause: fp16/fp32 buffer mismatch in OneDNN integration.";
    }
    ...
    std::vector<float> X = {0.8125258088111877f,   1.3562400341033936f,  -0.07201012223958969f, 1.003532886505127f,
    ...
    std::vector<float> W = {0.49671414494514465f, -0.13826429843902588f, 0.6476885676383972f,  1.5230298042297363f,
    ...
    std::vector<float> R = {-0.013497225008904934f,
                            -1.057710886001587f,
                            0.8225449323654175f,
                            -1.2208436727523804f,
                            0.20886360108852386f,
                            -1.959670066833496f,
                            -1.32818603515625f,
                            0.19686123728752136f,
                            0.7384665608406067f,
                            0.1713682860136032f,
                            -0.1156482845544815f,
                            -0.3011036813259125f,
                            -1.4785219430923462f,
                            -0.7198442220687866f,
                            -0.46063876152038574f,
                            1.0571222305297852f};

    // Bias B shape: [1, 16]
    std::vector<float> B = {0.3436183035373688f,
                            -1.7630401849746704f,
                            0.32408398389816284f,
                            -0.38508227467536926f,
                            -0.6769220232963562f,
                            0.6116762757301331f,
                            1.0309995412826538f,
                            0.9312801361083984f,
    ...
    // Y shape: [3, 1, 2, 2]
    std::vector<float> expected_Y = {0.04541102051734924f,
                                     0.007742831949144602f,
                                     -0.05228624865412712f,
                                     0.24427476525306702f,
                                     0.24919648468494415f,
                                     0.016252165660262108f,
                                     -0.10558786988258362f,
                                     0.337483674287796f,
                                     -0.1856241226196289f,
                                     -0.027182353660464287f,
                                     -0.1143757626414299f,
                                     0.14773108065128326f};

    // Y_h shape: [1, 2, 2]
    std::vector<float> expected_Y_h = {-0.1856241226196289f,
                                       -0.027182353660464287f,
                                       -0.1143757626414299f,
                                       0.14773108065128326f};

    // Y_c shape: [1, 2, 2]
    std::vector<float> expected_Y_c = {-0.3040353059768677f,
                                       -0.4779679775238037f,
                                       -0.31235989928245544f,
                                       0.3412477970123291f};
...
OPENVINO_TEST(${BACKEND_NAME}, onnx_model_lstm_rank4_with_unsqueeze) {
    ...
    std::vector<float> X_input = {
        -0.5138669013977051f, -1.0592135190963745f,  -0.06267909705638885f, 0.9551423192024231f,   -0.9857260584831238f,
        0.5040464997291565f,  -0.5302576422691345f,  -0.7928728461265564f,  -0.10703036189079285f, -1.0352423191070557f,
        -0.5536493062973022f, -1.1978778839111328f,  1.964725136756897f,    0.0352635532617569f,   -0.6997255086898804f,
        0.21397991478443146f, -0.11232805252075195f, -0.22096960246562958f, 0.6141666769981384f,   0.7575076818466187f,
    ...
    std::vector<float> expected_Y_h = {0.3284722566604614f,
                                       0.4379877746105194f,
                                       -0.22600656747817993f,
                                       -0.107466921210289f,
                                       -0.6092900633811951f,
                                       0.4817439317703247f,
                                       -0.06534680724143982f,
                                       -0.039009325206279755f,
                                       0.191391259431839f,
                                       0.07052464038133621f,
                                       -0.33953383564949036f,
                                       0.14264167845249176f,
                                       -0.7233807444572449f,
                                       0.01202191412448883f,
                                       0.009454132989048958f,
                                       -0.23135091364383698f};

    std::vector<float> expected_Y_c = {0.843663215637207f,
                                       0.4875003397464752f,
                                       -0.501198947429657f,
                                       -0.2064143717288971f,
                                       -1.8175270557403564f,
...
/// Test for LSTM with missing num_directions dimension (rank < expected).
/// This tests the Unsqueeze path in normalize_tensor_rank() for non-conformant models
/// where num_directions=1 dimension was omitted by old PyTorch exports.
/// W shape: [8, 4] instead of [1, 8, 4]
/// R shape: [8, 2] instead of [1, 8, 2]
/// B shape: [16] instead of [1, 16]
/// Reference outputs generated by ONNX Runtime with conformant shapes.
OPENVINO_TEST(${BACKEND_NAME}, onnx_model_lstm_missing_num_directions) {
    ...
    // Input X shape: [3, 2, 4] (seq_length=3, batch_size=2, input_size=4)
    ...
    // Weight W shape: [8, 4] - MISSING num_directions dimension
    // Original conformant shape would be [1, 8, 4]
    ...
    // Recurrent weight R shape: [8, 2] - MISSING num_directions dimension
    // Original conformant shape would be [1, 8, 2]
    std::vector<float> R = {-0.8392175436019897f,   -0.3092123866081238f,
                            0.3312634229660034f,    0.9755451083183289f,
                            -0.4791742265224457f,   -0.18565897643566132f,
                            -1.106334924697876f,    -1.1962065696716309f,
                            0.8125258088111877f,    1.3562400341033936f,
                            -0.07201012223958969f,  1.003532886505127f,
                            0.3616360127925873f,    -0.6451197266578674f,
                            0.36139559745788574f,   1.538036584854126f};

    // Bias B shape: [16] - MISSING num_directions dimension
    // Original conformant shape would be [1, 16]
    std::vector<float> B = {-0.03582603856921196f,  1.5646436214447021f,
                            -2.6197450160980225f,   0.8219025135040283f,
                            0.08704707026481628f,   -0.2990073561668396f,
                            0.0917607769370079f,    -1.9875688552856445f,
                            -0.21967189013957977f,  0.3571125566959381f,
                            1.4778940677642822f,    -0.5182701945304871f,
                            -0.8084936141967773f,   -0.501757025718689f,
                            0.9154021143913269f,    0.3287511169910431f};

    // Expected outputs (generated by ONNX Runtime with conformant shapes)
    // Y shape: [3, 1, 2, 2]
    std::vector<float> expected_Y = {0.022259334102272987f,  0.0033883226569741964f,
                                     0.052766889333724976f,  0.11502055823802948f,
                                     0.006480472162365913f,  -0.2669661045074463f,
                                     0.35137200355529785f,   -0.5121785998344421f,
                                     0.13379308581352234f,   -0.42883145809173584f,
                                     0.3751090168952942f,    -0.09079679846763611f};

    // Y_h shape: [1, 2, 2]
    std::vector<float> expected_Y_h = {0.13379308581352234f,  -0.42883145809173584f,
                                       0.3751090168952942f,   -0.09079679846763611f};

    // Y_c shape: [1, 2, 2]
    std::vector<float> expected_Y_c = {0.35507580637931824f,  -0.7553168535232544f,
                                       0.6096105575561523f,   -0.12818993628025055f};

    // Add inputs - note the shapes WITHOUT num_directions dimension
