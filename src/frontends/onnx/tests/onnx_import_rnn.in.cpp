...
// Test for LSTM with high-rank input (rank reduction)
// Tests the reduce_tensor_rank() function in lstm.cpp
OPENVINO_TEST(${BACKEND_NAME}, onnx_model_lstm_rank5_squeeze) {
    auto model = convert_model("lstm_rank5_squeeze.onnx");
    auto test_case = ov::test::TestCase(model, s_device);

    // Input X shape: [1, 1, 3, 2, 4]
    std::vector<float> X = {
    0.8125258088111877f, 1.3562400341033936f, -0.07201012223958969f, 1.003532886505127f, 
    0.3616360127925873f, -0.6451197266578674f, 0.36139559745788574f, 1.538036584854126f, 
    -0.03582603856921196f, 1.5646436214447021f, -2.6197450160980225f, 0.8219025135040283f, 
    0.08704707026481628f, -0.2990073561668396f, 0.0917607769370079f, -1.9875688552856445f, 
    -0.21967189013957977f, 0.3571125566959381f, 1.4778940677642822f, -0.5182701945304871f, 
    -0.8084936141967773f, -0.501757025718689f, 0.9154021143913269f, 0.3287511169910431f
    };

    // Weight W shape: [1, 8, 4]
    std::vector<float> W = {
    0.49671414494514465f, -0.13826429843902588f, 0.6476885676383972f, 1.5230298042297363f, 
    ...
    -0.5443827509880066f, 0.11092258989810944f, -1.1509935855865479f, 0.3756980299949646f, 
    -0.6006386876106262f, -0.2916937470436096f, -0.6017066240310669f, 1.852278232574463f
    };

    // Recurrent weight R shape: [1, 8, 2]
    std::vector<float> R = {
    ...
    -0.8392175436019897f, -0.3092123866081238f, 0.3312634229660034f, 0.9755451083183289f, 
    -0.4791742265224457f, -0.18565897643566132f, -1.106334924697876f, -1.1962065696716309f
    };

    // Expected outputs from ONNX Runtime
    // Y shape: [3, 1, 2, 2]
    std::vector<float> expected_Y = {
    0.04541102051734924f, 0.007742831949144602f, -0.05228624865412712f, 0.24427476525306702f, 
    0.24919648468494415f, 0.016252165660262108f, -0.10558786988258362f, 0.337483674287796f, 
    -0.1856241226196289f, -0.027182353660464287f, -0.1143757626414299f, 0.14773108065128326f
    };
    ...
    -0.3040353059768677f, -0.4779679775238037f, -0.31235989928245544f, 0.3412477970123291f
    };
...
// Test for LSTM with high-rank input (rank reduction)
// Tests the reduce_tensor_rank() function in lstm.cpp
OPENVINO_TEST(${BACKEND_NAME}, onnx_model_lstm_rank5_reshape) {
    auto model = convert_model("lstm_rank5_reshape.onnx");
    auto test_case = ov::test::TestCase(model, s_device);

    // Input X shape: [2, 3, 4, 2, 3]
    std::vector<float> X = {
    -0.2234627902507782f, 0.7140005230903625f, 0.47323763370513916f, -0.07282891124486923f, 
    -0.8467937111854553f, -1.5148472785949707f, -0.44651496410369873f, 0.8563988208770752f, 
    0.21409374475479126f, -1.2457387447357178f, 0.17318092286586761f, 0.3853173851966858f, 
    -0.8838574290275574f, 0.15372510254383087f, 0.05820871889591217f, -1.142970323562622f, 
    0.3577873706817627f, 0.5607845187187195f, 1.0830512046813965f, 1.0538020133972168f, 
    -1.377669334411621f, -0.9378250241279602f, 0.5150352716445923f, 0.5137859582901001f, 
    0.5150476694107056f, 3.852731466293335f, 0.570890486240387f, 1.1355656385421753f, 
    0.954001784324646f, 0.6513912677764893f, -0.31526923179626465f, 0.758969247341156f, 
    -0.6649172306060791f, -0.5597132444381714f, -0.1288355588912964f, -0.762666642665863f, 
    -0.18103575706481934f, -0.08771304786205292f, -0.604515790939331f, -0.6165962219238281f, 
    -0.07080619037151337f, -0.941373884677887f, 0.6053177118301392f, -0.14255407452583313f, 
    -0.6849688291549683f, -0.3811478018760681f, -0.0843273475766182f, -0.8984147310256958f, 
    0.7703230381011963f, -0.2312861680984497f, -0.5846250653266907f, -0.48408243060112f, 
    -0.10606604814529419f, -0.5018020272254944f, -0.4337713420391083f, -0.18315517902374268f, 
    -0.5660727620124817f, -0.5456831455230713f, -0.08497992157936096f, -0.9198301434516907f, 
    0.7035802006721497f, -0.04937483370304108f, -0.9188945293426514f, -0.3845064342021942f, 
    -0.17959071695804596f, -0.7615110874176025f, 0.3007529079914093f, -0.09245641529560089f, 
    -0.9267237782478333f, 0.16126775741577148f, -0.09855153411626816f, -0.8755027055740356f, 
    0.71486896276474f, -0.07205609232187271f, -0.919130802154541f, 0.6385470032691956f, 
    -0.12470933049917221f, -0.5933496952056885f, 0.5531017184257507f, 0.05431796610355377f, 
    -0.9666638970375061f, 0.6817083358764648f, -0.1281207948923111f, -0.683216392993927f, 
    0.7294043898582458f, 0.007884210906922817f, -0.949615478515625f, 0.7697608470916748f, 
    -0.1193147674202919f, -0.7113837599754333f, 0.854475200176239f, -0.032020453363657f, 
    -0.9166643619537354f, 0.8592878580093384f, -0.11676548421382904f, -0.30697572231292725f, 
    0.7922672629356384f, -0.04735323414206505f, -0.9057092070579529f, 0.9064471125602722f, 
    -0.13761933147907257f, -0.07154004275798798f, 0.3420600891113281f, -0.038642872124910355f, 
    -0.9378430247306824f, 0.9190859198570251f, -0.22744931280612946f, -0.08302775770425797f, 
    0.4912205636501312f, -0.04762669652700424f, -0.5844769477844238f, 0.8532794713973999f, 
    -0.30327269434928894f, -0.20391927659511566f, -0.43926548957824707f, -0.06528590619564056f, 
    -0.8576780557632446f, 0.8616430759429932f, -0.1743767112493515f, -0.46792495250701904f, 
    -0.14900918304920197f, -0.07638663798570633f, -0.9411453008651733f, 0.9263080954551697f
    };

    // Y_h shape: [1, 2, 3]
    std::vector<float> expected_Y_h = {
    -0.1743767112493515f, -0.46792495250701904f, -0.14900918304920197f, -0.07638663798570633f, 
    -0.9411453008651733f, 0.9263080954551697f
    };

    // Y_c shape: [1, 2, 3]
    std::vector<float> expected_Y_c = {
    -0.5377441644668579f, -0.6441870927810669f, -0.18324995040893555f, -0.09202337265014648f, 
    -2.6309618949890137f, 1.7692674398422241f
    };

    // Add inputs
    test_case.add_input<float>(Shape{2, 3, 4, 2, 3}, X);
    test_case.add_input<float>(Shape{1, 12, 3}, W);
    test_case.add_input<float>(Shape{1, 12, 3}, R);
    test_case.add_input<float>(Shape{1, 24}, B);

    // Add expected outputs
    test_case.add_expected_output<float>(Shape{24, 1, 2, 3}, expected_Y);
    test_case.add_expected_output<float>(Shape{1, 2, 3}, expected_Y_h);
    test_case.add_expected_output<float>(Shape{1, 2, 3}, expected_Y_c);

    test_case.run_with_tolerance_as_fp(1.0e-4f);
}

// Test for LSTM with Unsqueeze preprocessing (reproduces silero_vad structure)
// Input goes through Unsqueeze(axes=[0]) to get rank-4: [1, seq, batch, input]
// Then LSTM applies rank reduction (Squeeze path) internally
// W, R, B are embedded as initializers, not passed as test inputs
OPENVINO_TEST(${BACKEND_NAME}, onnx_model_lstm_rank4_with_unsqueeze) {
    auto model = convert_model("lstm_rank4_with_unsqueeze.onnx");
    auto test_case = ov::test::TestCase(model, s_device);

    // Input X_input shape: [3, 2, 4]
    // Inside model: Unsqueeze(axes=[0]) -> [1, 3, 2, 4]
    // Then LSTM reduces rank-4 back to rank-3
    std::vector<float> X_input = {
    -0.5138669013977051f, -1.0592135190963745f, -0.06267909705638885f, 0.9551423192024231f,
    -0.9857260584831238f, 0.5040464997291565f, -0.5302576422691345f, -0.7928728461265564f,
    -0.10703036189079285f, -1.0352423191070557f, -0.5536493062973022f, -1.1978778839111328f,
    1.964725136756897f, 0.0352635532617569f, -0.6997255086898804f, 0.21397991478443146f,
    -0.11232805252075195f, -0.22096960246562958f, 0.6141666769981384f, 0.7575076818466187f,
    -0.530501127243042f, -0.5758182406425476f, -0.27505168318748474f, -2.3019211292266846f
    };

    // Expected outputs from ONNX Runtime
    std::vector<float> expected_Y = {
    -0.041506070643663406f, 0.5078101754188538f, -0.1956426054239273f, -0.6245316863059998f,
    -0.46069785952568054f, 0.16945451498031616f, -0.036582063883543015f, -0.14519499242305756f,
    0.03458118066191673f, 0.22937199473381042f, -0.33998003602027893f, -0.24845707416534424f,
    -0.10919304192066193f, 0.041263557970523834f, -0.01278171967715025f, -0.09499483555555344f,
    -0.05660216882824898f, 0.4603622555732727f, -0.6351584196090698f, 0.03479098901152611f,
    -0.88917076587677f, 0.037890415638685226f, 0.006493804045021534f, -0.5771874189376831f,
    0.08443683385848999f, -0.06232306361198425f, -0.8248671293258667f, 0.05733451619744301f,
    -0.454610675573349f, 0.02996906079351902f, 0.0028792552184313536f, -0.045823417603969574f,
    0.3284722566604614f, 0.4379877746105194f, -0.22600656747817993f, -0.107466921210289f,
    -0.6092900633811951f, 0.4817439317703247f, -0.06534680724143982f, -0.039009325206279755f,
    0.191391259431839f, 0.07052464038133621f, -0.33953383564949036f, 0.14264167845249176f,
    -0.7233807444572449f, 0.01202191412448883f, 0.009454132989048958f, -0.23135091364383698f
    };
    ...
    std::vector<float> expected_Y_c = {
    0.843663215637207f, 0.4875003397464752f, -0.501198947429657f, -0.2064143717288971f,
    -1.8175270557403564f, 0.9995222091674805f, -0.0911954939365387f, -1.0586004257202148f,
    0.2041035294532776f, 0.0721229761838913f, -1.948965311050415f, 1.1799299716949463f,
    -2.050872564315796f, 0.5028446316719055f, 0.1787012219429016f, -0.24886609613895416f
    };

    // Add input (only X, weights are embedded)
    test_case.add_input<float>(Shape{3, 2, 4}, X_input);

    // Add expected outputs
    test_case.add_expected_output<float>(Shape{3, 1, 2, 8}, expected_Y);
    test_case.add_expected_output<float>(Shape{1, 2, 8}, expected_Y_h);
    test_case.add_expected_output<float>(Shape{1, 2, 8}, expected_Y_c);
