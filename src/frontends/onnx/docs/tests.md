...
2. Set up Python paths via `source <OV_INSTALL_DIR>/setupvars.sh` for Linux, or `. <path-to-setupvars-folder>/setupvars.ps1` in PowerShell (or `sh <INSTALL_DIR>\setupvars.bat` in Command Prompt) for Windows.
