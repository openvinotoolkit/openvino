...
OpInputMap::OpInputMap(const onnx_import::Node& node, std::size_t gates_count) {
    ...
    m_map[OpInput::X] = ngraph::builder::opset1::reorder_axes(ng_inputs.at(0), {1, 0, 2});
    ...
    if (ng_inputs.size() > 3 && !ov::op::is_null(ng_inputs.at(3))) {
        ...
        auto split_bias = ngraph::builder::opset1::split(bias, 2, 1);
    ...
    if (ng_inputs.size() > 5 && !ov::op::is_null(ng_inputs.at(5))) {
        m_map[OpInput::INIT_H] = ngraph::builder::opset1::reorder_axes(ng_inputs.at(5), {1, 0, 2});
    } else {
