...
ov::Shape get_kernel_shape(const ONNX_Node& node);
...
///             depth).
ov::Strides get_strides(const ONNX_Node& node, const std::size_t kernel_rank = 0UL);
...
///             (height, width, depth).
ov::Strides get_dilations(const ONNX_Node& node, const std::size_t kernel_rank = 0UL);
...
std::pair<ov::CoordinateDiff, ov::CoordinateDiff> get_pads(const ONNX_Node& node, const size_t kernel_rank);
...
/// \return A pair of (padding_above, padding_below), which elements contains number of
///         pixels to pad in respective dimensions (height, width, depth).
std::pair<ov::CoordinateDiff, ov::CoordinateDiff> get_pads(const ONNX_Node& node);
