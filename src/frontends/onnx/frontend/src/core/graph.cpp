    ...
    for (const auto& extension : conversions) {
        if (const auto common_conv_ext = std::dynamic_pointer_cast<ov::frontend::ConversionExtension>(extension)) {
            bridge.overwrite_operator(
                ...
                "",
                [common_conv_ext](const ngraph::onnx_import::ONNX_Node& node) -> ov::OutputVector {
            ...
            bridge.overwrite_operator(onnx_conv_ext->get_op_type(),
                                      ...
                                      [onnx_conv_ext](const ngraph::onnx_import::ONNX_Node& node) -> ov::OutputVector {
...
void Graph::convert_to_ov_nodes() {
    ...
    for (const auto& node_proto : m_model->get_graph().node()) {
        ...
        const ONNX_Node node{node_proto, this};
...
OPENVINO_SUPPRESS_DEPRECATED_START
ov::OutputVector Graph::make_framework_nodes(const ONNX_Node& onnx_node) {
...
ov::OutputVector Graph::make_ov_nodes(const ONNX_Node& onnx_node) {
...
void Graph::set_friendly_names(const ONNX_Node& onnx_node, const ov::OutputVector& ov_subgraph_outputs) const {
