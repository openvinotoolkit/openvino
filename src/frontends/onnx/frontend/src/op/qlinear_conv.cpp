...
OutputVector qlinear_conv(const Node& node) {
    ...
    x = set_13::detail::dequantize_linear(x,
                                          x_scale,
                                          std::make_shared<ov::opset6::Convert>(x_zero_point, element::f32),
                                          1,
                                          node)[0];
    w = set_13::detail::dequantize_linear(w,
                                          w_scale,
                                          std::make_shared<ov::opset6::Convert>(w_zero_point, element::f32),
                                          1,
                                          node)[0];

    if (!ov::op::util::is_null(B)) {
        B = std::make_shared<ov::opset6::Multiply>(std::make_shared<ov::opset6::Convert>(B, x_scale.get_element_type()),
                                                   std::make_shared<ov::opset6::Multiply>(x_scale, w_scale))
                ->output(0);
    }
