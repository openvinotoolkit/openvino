...
std::shared_ptr<v1::StridedSlice> make_slice(std::shared_ptr<ov::Node> node, int64_t start, int64_t end) {
    return std::make_shared<v1::StridedSlice>(
        node,
        v0::Constant::create(element::i64, ov::Shape{1}, std::vector<int64_t>{start}),
        v0::Constant::create(element::i64, ov::Shape{1}, std::vector<int64_t>{end}),
...
OutputVector prior_box(const Node& node) {
    ...
    auto axes = v0::Constant::create(element::i64, ov::Shape{1}, std::vector<int64_t>{0});
