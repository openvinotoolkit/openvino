...
OutputVector softsign(const Node& node) {
    ...
    std::shared_ptr<ngraph::Node> one_node = default_opset::Constant::create(data.get_element_type(), Shape{}, {1});
    auto abs_data = std::make_shared<default_opset::Abs>(data);
    auto data_plus_one_node = std::make_shared<default_opset::Add>(abs_data, one_node);
