...
ov::OutputVector expand(const ov::frontend::onnx::Node& node) {
    ...
    } else if (common::is_constant_empty_node(shape.get_node_shared_ptr())) {
        const auto identity_broadcast = v0::Constant::create(ov::element::i64, ov::Shape{0}, {});
        return {std::make_shared<ov::op::v1::Reshape>(data, identity_broadcast, true)};
    } else {
