...
ov::OutputVector quickgelu(const ov::frontend::onnx::Node& node) {
    ...
    CHECK_VALID_NODE(node,
        element_type == ov::element::f16 || element_type == ov::element::f32 || element_type == ov::element::f64 || element_type == ov::element::bf16,
        "Unsupported input x type, accepted FP16, FP32, FP64, BFP16 but got: ", element_type);

    // Get attribute from node
    const float alpha = node.get_attribute_value<float>("alpha");

    // Numpy broadcasting rule is automatically applied with mismatched shapes according to: https://docs.openvino.ai/2022.3/openvino_docs_ops_arithmetic_Multiply_1.html
    // "Tensor with dimension of size 1 will be implicitly broadcasted to match the size of the second tensor."
    // Convert alpha to tensor with size 1
