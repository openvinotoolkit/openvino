...
OutputVector fused_conv(const Node& node) {
    ...
    if (activation_type == "LeakyRelu") {
        ...
        CHECK_VALID_NODE(node, activation_params.size() == 2, "alpha and beta attributes of HardSigmoid activation function were not provided");
        ...
        CHECK_VALID_NODE(node, activation_params.size() == 2, "min and max attributes of Clip activation function were not provided");
        return {std::make_shared<default_opset::Clamp>(conv_res, activation_params[0], activation_params[1])};
    }
    CHECK_VALID_NODE(node, !activation_type.empty(), "Not supported: ", activation_type, " activation function was used");
