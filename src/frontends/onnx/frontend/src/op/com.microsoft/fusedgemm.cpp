...
OutputVector fusedgemm(const Node& node) {
    ...
    auto gemm_res = std::make_shared<default_opset::Add>(matmul_node, beta_times_input_c);
	...
	std::shared_ptr<ngraph::Node> activation_alpha_node = default_opset::Constant::create(input_c.get_element_type(), Shape{1}, {activation_alpha});
...
OutputVector fusedgemm(const Node& node) {
    ...
    auto gemm_res = std::make_shared<default_opset::Add>(matmul_times_alpha, beta_times_input_c);
