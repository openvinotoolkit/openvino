...
struct LSTMNgInputMap {
    explicit LSTMNgInputMap(const Node& node) {
        ...
        m_input_map[LSTMInput::LSTM_INPUT_X] = ov::utils::reorder_axes(ng_inputs.at(0), {1, 0, 2});
        ...
        if (ng_inputs.size() > 4 && !ngraph::op::is_null(ng_inputs.at(4))) {
            ...
            m_input_map[LSTMInput::LSTM_INPUT_SEQ_LENGTHS] =
                std::make_shared<default_opset::Broadcast>(seq_length_node, batch_size_node);
        }
        ...
        if (ng_inputs.size() > 5 && !ngraph::op::is_null(ng_inputs.at(5))) {
            m_input_map[LSTMInput::LSTM_INPUT_INIT_H] = ov::utils::reorder_axes(ng_inputs.at(5), {1, 0, 2});
        } else {
            auto init_h_shape = std::make_shared<default_opset::Concat>(
                OutputVector{batch_size_node, num_directions_node, hidden_size_node},
                0);
            m_input_map[LSTMInput::LSTM_INPUT_INIT_H] = std::make_shared<default_opset::Broadcast>(
                default_opset::Constant::create(m_input_map[LSTMInput::LSTM_INPUT_X].get_element_type(), Shape{}, {0}),
                init_h_shape);
        }
        ...
        if (ng_inputs.size() > 6 && !ngraph::op::is_null(ng_inputs.at(6))) {
            m_input_map[LSTMInput::LSTM_INPUT_INIT_C] = ov::utils::reorder_axes(ng_inputs.at(6), {1, 0, 2});
        } else {
            auto init_c_shape = std::make_shared<default_opset::Concat>(
                OutputVector{batch_size_node, num_directions_node, hidden_size_node},
                0);
            m_input_map[LSTMInput::LSTM_INPUT_INIT_C] = std::make_shared<default_opset::Broadcast>(
                default_opset::Constant::create(m_input_map[LSTMInput::LSTM_INPUT_X].get_element_type(), Shape{}, {0}),
                init_c_shape);
        }
...
OutputVector lstm(const Node& node) {
    ...
    return {ov::utils::reorder_axes(Y, {2, 1, 0, 3}),
            ov::utils::reorder_axes(Y_h, {1, 0, 2}),
            ov::utils::reorder_axes(Y_c, {1, 0, 2})};
}
}  // namespace set_1
