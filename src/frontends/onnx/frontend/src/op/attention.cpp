...
ov::OutputVector build_manual_attention(const ov::Output<ov::Node>& Q,
    ...
    if (scale_attr != 0.0f) {
        ...
        // Default scale: 1/sqrt(head_size) where head_size is the last dim of Q
        auto q_shape = std::make_shared<v3::ShapeOf>(Q);
        auto head_size = get_dimensions(q_shape, {static_cast<int>(Q.get_partial_shape().rank().get_length()) - 1});
