...
ov::OutputVector random_uniform_like(const ov::frontend::onnx::Node& node) {
    ...
    std::shared_ptr<ov::Node> high_convert =
        target_type == ov::element::f32
            ...
            : std::static_pointer_cast<ov::Node>(std::make_shared<v0::Convert>(high_const, target_type));
    std::shared_ptr<ov::Node> low_convert =
        target_type == ov::element::f32
            ? std::static_pointer_cast<ov::Node>(low_const)
            : std::static_pointer_cast<ov::Node>(std::make_shared<v0::Convert>(low_const, target_type));
