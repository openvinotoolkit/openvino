...
OutputVector non_max_suppression(const Node& node) {
    ...
    Output<ngraph::Node> soft_nms_sigma;
    if (ng_inputs.size() > 5 && !is_null(ng_inputs.at(5))) {
        soft_nms_sigma = ngraph::onnx_import::reshape::interpret_as_scalar(ng_inputs.at(5));
    } else {
        soft_nms_sigma = default_opset::Constant::create(element::f32, Shape{}, {.0f});
    }
