...
// Numerically stable ReduceLogSumExp: computes max(x) + log(sum(exp(x - max(x))))
// instead of log(sum(exp(x))), to avoid FP overflow when any element of x exceeds
// the exp() representable range (~88 for fp32).
std::shared_ptr<ov::Node> onnx_reduce_log_sum_exp(const ov::frontend::onnx::Node& node,
                                                  const std::set<element::Type>& supported_types,
                                                  const bool axes_as_attr = true) {
    const auto input = node.get_ov_inputs().at(0);
    const std::int64_t keepdims = node.get_attribute_value<std::int64_t>("keepdims", 1);
    ...
    CHECK_VALID_NODE(node,
                     supported_types.find(input.get_element_type()) != supported_types.end(),
                     "Unsupported input type ",
    ...
    return std::make_shared<v0::Squeeze>(out_kd, reduction_axes);
}
}  // namespace
...
ov::OutputVector reduce_log_sum_exp(const ov::frontend::onnx::Node& node) {
    return {onnx_reduce_log_sum_exp(node, supported_types_v1)};
}
...
ov::OutputVector reduce_log_sum_exp(const ov::frontend::onnx::Node& node) {
    return {onnx_reduce_log_sum_exp(node, supported_types_v2)};
}
...
ov::OutputVector reduce_log_sum_exp(const ov::frontend::onnx::Node& node) {
    return {onnx_reduce_log_sum_exp(node, supported_types_v3, false)};
}
