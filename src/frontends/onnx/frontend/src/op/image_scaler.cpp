...
OutputVector image_scaler(const Node& node) {
    ...
    OPENVINO_ASSERT(inputs.size() == 1, "ImageScaler 1 input tensor. Got: ", inputs.size());

    const auto data = inputs[0];
    const auto& data_shape = data.get_partial_shape();
    OPENVINO_ASSERT(data_shape.rank().same_scheme({4}),
    ...
    OPENVINO_ASSERT(data_shape[1].same_scheme(bias.size()),
    ...
    const auto scaler =
        std::make_shared<default_opset::Add>(std::make_shared<default_opset::Multiply>(data, scale_const), bias_const);
