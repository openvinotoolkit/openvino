...
public:
    ov::util::Path m_file_name;  // Plugin file name, e.g. "libopenvino_ir_frontend.so"
    ov::util::Path m_file_path;  // Plugin file full path
    ...
    ov::util::Path get_name_from_file() const;
...
void find_plugins(const ov::util::Path& dir_name, std::vector<PluginInfo>& res);
