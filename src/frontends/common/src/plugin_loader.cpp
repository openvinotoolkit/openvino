...
void load_static_plugins(const std::vector<std::string>& plugins_names, std::vector<PluginInfo>& res) {
    for (const auto& frontend : getStaticFrontendsRegistry()) {
        ...
        if (std::find(plugins_names.begin(), plugins_names.end(), factory.m_name) != plugins_names.end()) {
            plugin_info.m_file_name = factory.m_name;
        }
...
void ov::frontend::find_plugins(const std::string& dir_name, const std::vector<std::string>& plugins_files_names, std::vector<PluginInfo>& res) {
#ifdef OPENVINO_STATIC_LIBRARY
    ...
    for (const auto& file_path : list_files(dir_name)) {
        ...
        if (std::find_if(res.begin(), res.end(), [&plugin_info](const PluginInfo& pd) {
            ...
            if (std::find_if(plugins_files_names.begin(), plugins_files_names.end(), [&plugin_info](const std::string& plugin_name) {
