...
class InputModel::InputModelPDPDImpl {
public:
    template <typename T>
    ...
    std::map<paddlepaddle::TensorName, Output<Node>> get_tensor_values() const {
...
private:
    ...
    std::map<paddlepaddle::TensorName, Output<Node>> m_tensor_values;
...
void InputModel::InputModelPDPDImpl::loadPlaces() {
    ...
    for (int block_idx = 0; block_idx < cnt_of_blocks; block_idx++) {
        ...
        for (const auto& op : block.ops()) {
            ...
            if (op.type() == "feed") {
                ...
                const auto& var_place = std::dynamic_pointer_cast<TensorPlace>(place->get_target_tensor_pdpd());
...
std::basic_string<T> get_const_path(const std::basic_string<T>& folder_with_weights, const std::string& name) {
    return folder_with_weights + paddlepaddle::get_path_sep<T>() + name;
}
...
std::basic_string<wchar_t> get_const_path(const std::basic_string<wchar_t>& folder, const std::string& name) {
    ...
    return folder + paddlepaddle::get_path_sep<wchar_t>() + _name;
}
#endif
...
std::basic_string<T> get_model_path(const std::basic_string<T>& path, std::ifstream* weights_stream) {
    ...
    if (paddlepaddle::endsWith(model_file, ext)) {
        ...
        model_file += paddlepaddle::get_path_sep<T>() + "__model__";
    }
    return model_file;
}
...
std::basic_string<wchar_t> get_model_path(const std::basic_string<wchar_t>& path, std::ifstream* weights_stream) {
    ...
    if (paddlepaddle::endsWith(model_file, ext)) {
        ...
        model_file += paddlepaddle::get_path_sep<wchar_t>() + L"__model__";
    }
    return model_file;
}
#endif
...
std::vector<std::shared_ptr<OpPlace>> InputModel::InputModelPDPDImpl::get_op_places() const {
    if (m_graph_changed) {
...
std::vector<std::shared_ptr<OpPlace>> InputModel::InputModelPDPDImpl::determine_cut_nodes() const {
    std::queue<OpPlace*> q;
    ...
    for (const auto& output : getOutputs()) {
        if (!output->is_input()) {
            auto pdpd_output_op = std::dynamic_pointer_cast<OpPlace>(output->get_producing_operation());
        ...
        for (const auto& map_pair : p_op->get_input_ports()) {
            for (const auto& port : map_pair.second) {
                ...
                if (tensor && !tensor->is_input() && !m_tensor_values.count(tensor->get_names()[0])) {
                    std::shared_ptr<OpPlace> pdpd_op =
...
template <typename T>
void InputModel::InputModelPDPDImpl::loadConsts(const std::basic_string<T>& folder_with_weights,
                                                std::istream* weight_stream) {
    for (const auto& item : m_var_places) {
        const auto& var_desc = item.second->get_desc();
        const auto& name = item.first;
        if (paddlepaddle::endsWith(name, std::string{"feed"}) || paddlepaddle::endsWith(name, std::string{"fetch"}))
            continue;
...
template <typename T>
InputModel::InputModelPDPDImpl::InputModelPDPDImpl(const std::basic_string<T>& path,
                                                   const InputModel& input_model,
                                                   const std::shared_ptr<TelemetryExtension>& telemetry)
...
InputModel::InputModelPDPDImpl::InputModelPDPDImpl(const std::vector<std::istream*>& streams,
                                                   const InputModel& input_model,
                                                   const std::shared_ptr<TelemetryExtension>& telemetry)
    : m_fw_ptr{std::make_shared<ProgramDesc>()},
      m_input_model(input_model),
...
std::vector<Place::Ptr> InputModel::InputModelPDPDImpl::getInputs() const {
    return m_inputs;
}

std::vector<Place::Ptr> InputModel::InputModelPDPDImpl::getOutputs() const {
    return m_outputs;
}

Place::Ptr InputModel::InputModelPDPDImpl::getPlaceByTensorName(const std::string& tensorName) const {
...
void InputModel::InputModelPDPDImpl::overrideAllInputs(const std::vector<Place::Ptr>& inputs) {
...
void InputModel::InputModelPDPDImpl::overrideAllOutputs(const std::vector<Place::Ptr>& outputs) {
    m_graph_changed = true;
    m_outputs.clear();
...
void InputModel::InputModelPDPDImpl::extractSubgraph(const std::vector<Place::Ptr>& inputs,
...
void InputModel::InputModelPDPDImpl::setDefaultShape(Place::Ptr place, const ov::Shape& shape) {
    FRONT_END_NOT_IMPLEMENTED("setDefaultShape");
}

void InputModel::InputModelPDPDImpl::setPartialShape(Place::Ptr place, const ov::PartialShape& p_shape) {
    castToTensorPlace(place)->set_partial_shape(p_shape);
}

ov::PartialShape InputModel::InputModelPDPDImpl::getPartialShape(Place::Ptr place) const {
    return castToTensorPlace(place)->get_partial_shape();
}

void InputModel::InputModelPDPDImpl::setElementType(Place::Ptr place, const ov::element::Type& type) {
    castToTensorPlace(place)->set_element_type(type);
}

void InputModel::InputModelPDPDImpl::setTensorValue(Place::Ptr place, const void* value) {
    m_graph_changed = true;
...
std::map<paddlepaddle::TensorName, Output<Node>> InputModel::get_tensor_values() const {
