...
static std::shared_ptr<FakeQuantize> get_quant_common(const int range, const float scale, const ov::Output<ov::Node>& input) {
    ...
    return std::make_shared<FakeQuantize>(clip,
        input_low,
        input_high,
        output_low,
        output_high,
...
static std::shared_ptr<FakeQuantize> get_fakequant_for_data(const std::shared_ptr<const ngraph::op::internal::FakeQuantInternal> fake_internal) {
...
ov::frontend::pass::FuseQuant::FuseQuant() {
    auto fake_quant = ngraph::pattern::wrap_type<ngraph::op::internal::FakeQuantInternal>({ngraph::pattern::any_input(), ngraph::pattern::any_input()});
...
static std::shared_ptr<Node> get_dequant_common(const std::vector<float>& weights, const Shape& weights_shape, const element::Type& data_type,
    ...
    auto mul = std::make_shared<Multiply>(weight_float,
        Constant::create(element::f32, scales_shape, scales));
    return mul;
}

static std::shared_ptr<Node> get_fakequant_for_weight(const std::shared_ptr<const ngraph::op::internal::FakeDequantInternal> fake_internal) {
    const auto range = (1 << (fake_internal->m_bit_length - 1)) - 1;
...
ov::frontend::pass::FuseChannelDequant::FuseChannelDequant() {
    ...
    matcher_pass_callback callback = [fake_dequant](pattern::Matcher & m) -> bool {
        ...
        const auto & op = pattern_to_output.at(fake_dequant).get_node_shared_ptr();
        const auto & fake_dequant_op = std::dynamic_pointer_cast<const ngraph::op::internal::ChannelFakeQuantInternal>(op);
        // deal weight
        auto new_conv = get_fakequant_for_weight(fake_dequant_op);
        replace_node(op, new_conv);
...
static std::shared_ptr<ngraph::Node> get_fakequant_for_data(const std::shared_ptr<const ngraph::op::internal::FakeQuantDequantInternal> fake_internal) {
    const auto range = (1 << (fake_internal->m_bit_length - 1)) - 1;
    ...
    if (fake_internal->m_op_type == "fake_quantize_dequantize_abs_max") {
        ...
        std::transform(neg_scales.begin(), neg_scales.end(), neg_scales.begin(), [](const float v) { return -v; });
