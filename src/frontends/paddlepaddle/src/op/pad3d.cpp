...
NamedOutputs pad3d(const NodeContext& node) {
    ...
    if (node.has_attribute<std::vector<int32_t>>("paddings")) {
        ...
        PADDLEPADDLE_OP_CHECK(node, paddings_vector.size() == 6, "paddings Params size should be 6 in pad3d!");
        ...
        PADDLEPADDLE_OP_CHECK(node, false, "Unsupported paddings attribute!");
    }
    ...
    if (mode == "constant") {
        ...
        PADDLEPADDLE_OP_CHECK(node, false, "Unsupported 3d paddings mode: [" + mode + "]");
    }
    ...
    if (data_format == "NCDHW") {
        ...
        PADDLEPADDLE_OP_CHECK(node, false, "Unsupported 3d paddings data_format: [" + data_format + "]");
    }
