...
NamedOutputs hard_swish(const NodeContext& node) {
    ...
    if (node.has_attribute<float>("threshold")) {
        ...
        PDPD_ASSERT(std::abs(threshold - 6.0) < 0.001, "hard_swish: Only threshold = 6.0 is currently supported");
    }
    if (node.has_attribute<float>("scale")) {
        auto scale = node.get_attribute<float>("scale");
        PDPD_ASSERT(std::abs(scale - 6.0) < 0.001, "hard_swish: Only scale = 6.0 is currently supported");
    }
