...
public:
    ...
    PtFrameworkNode(const std::shared_ptr<Decoder>& decoder, const OutputVector& inputs)
        : ov::op::util::FrameworkNode(inputs, decoder->num_of_outputs(), decoder->get_subgraph_size()),
          m_decoder(decoder) {
    ...
    bool visit_attributes(AttributeVisitor& visitor) override {
        ...
        for (size_t i = 0; i < m_bodies.size(); ++i) {
            ...
            visitor.on_attribute(name, m_bodies[i]);
        }
...
OutputVector convert_node(const std::shared_ptr<Decoder> decoder, TensorMap* tensor_map, ParameterVector* external_parameters) {
        ...
        static const std::map<std::string, std::function<OutputVector()>> converters = {
            ...
            {"aten::conv2d",
             [&]() -> OutputVector {
                 ...
                 if (groups == 1) {
                     conv = make_shared<opset7::Convolution>(context.get_input(0),
                                                             context.get_input(1),
                                                             strides,
                                                             pads_begin,
                                                             pads_end,
            ...
            {"aten::batch_norm",
             [&]() -> OutputVector {
                 ...
                 return {
                     context.mark_node(make_shared<opset7::BatchNormInference>(context.get_input(0),
                                                                               context.get_input(1),
                                                                               context.get_input(2),
                                                                               context.get_input(3),
                                                                               context.get_input(4),
            ...
            {"aten::layer_norm",
             [&]() -> OutputVector {
                 ...
                 auto axes = context.mark_node(
                     opset7::Constant::create(element::i64, Shape{1}, {-1}));  // TODO: support any dimention
                 auto mvn = context.mark_node(
                     make_shared<opset7::MVN>(context.get_input(0), axes, true, eps, op::MVNEpsMode::INSIDE_SQRT));
            ...
            {"aten::div",
             [&]() -> OutputVector {
                 ...
                 return {context.mark_node(
                     make_shared<opset7::Divide>(context.get_input(0), context.get_input(1), pythondiv))};
             }},
            ...
            {"aten::softmax",
             [&]() -> OutputVector {
                 ...
                 return {
                     context.mark_node(make_shared<opset7::Softmax>(context.get_input(0), static_cast<size_t>(axis)))};
             }},
            ...
            {"aten::max_pool2d",
             [&]() -> OutputVector {
                 ...
                 return {context.mark_node(make_shared<opset7::MaxPool>(context.get_input(0),
                                                                        strides,
            ...
            {"aten::avg_pool2d",
             [&]() -> OutputVector {
                 ...
                 return {context.mark_node(make_shared<opset7::AvgPool>(context.get_input(0),
                                                                        strides,
                                                                        pads_begin,
                                                                        pads_end,
                                                                        kernel,
            ...
            {"aten::adaptive_avg_pool2d",
             [&]() -> OutputVector {
                 return {context.mark_node(
                     make_shared<opset8::AdaptiveAvgPool>(context.get_input(0), context.get_input(1)))};
             }},

            {"aten::adaptive_max_pool2d",
             [&]() -> OutputVector {
                 auto adaptive_max_pool = context.mark_node(
                     make_shared<opset8::AdaptiveMaxPool>(context.get_input(0), context.get_input(1)));
            ...
            {"aten::mean",
             [&]() -> OutputVector {
                 ...
                 return {context.mark_node(
                     make_shared<opset8::ReduceMean>(context.get_input(0), context.get_input(1), keep_dims))};
             }},
            ...
            {"aten::embedding",
             [&]() -> OutputVector {
                 ...
                 return {context.mark_node(
                     make_shared<opset7::Gather>(context.get_input(0), context.get_input(1), axis_0))};
             }},
             ...
             [&]() -> OutputVector {
                 ...
                 if (shape_node_fw_node && shape_node_fw_node->get_decoder()->get_op_type() == "prim::ListConstruct") {
                     ...
                     for (auto& input : shape_node->inputs()) {
                         ...
                         auto unsqueeze =
                             context.mark_node(make_shared<opset7::Unsqueeze>(input.get_source_output(), axis_0));
                     ...
                     reshape = context.mark_node(
                         make_shared<opset7::Reshape>(context.get_input(0), context.get_input(1), false));
                 }
                 return {reshape};
             }},
            ...
            {"aten::rsub",
             [&]() -> OutputVector {
                 ...
                 auto alpha_casted = context.mark_node(
                     make_shared<opset8::Convert>(context.get_input(2), context.get_input(0).get_element_type()));
            ...
            {"aten::slice",
             [&]() -> OutputVector {
                 ...
                 if (start.get_partial_shape().rank().is_static() &&
                     start.get_partial_shape().rank().get_length() == 0) {
    ...
    // }
        ...
        std::cout << "Some exception happened during convertion of node of type: " << context.get_op_type()
                  << std::endl;
        // throw;
    }
    ...
    for (size_t i = 0; i < decoder->get_subgraph_size(); ++i) {
        ...
        auto subgraph_decoder = decoder->get_subgraph_decoder(i);
        auto inputs = subgraph_decoder->inputs();
        input_idxs.insert(inputs.begin(), inputs.end());
        auto subgraph = convert_pytorch_model(subgraph_decoder, *tensor_map);
        fw_node->set_function(i, subgraph);
        std::cout << "====Before outputs====" << std::endl;
        for (auto result : subgraph->get_results()) {
            auto name = result->get_input_node_ptr(0)->get_friendly_name();
            size_t out_idx = (size_t)std::stoll(name);
            if (outputs_map.count(out_idx)) {
        ...
        for (auto param : subgraph->get_parameters()) {
            // std::cout << "====Before invariant inputs====" << std::endl;
            ...
            // std::cout << "====After invariant inputs====" << std::endl;
        }
        // fw_node->add_input();
        std::cout << "End converting subgraph\n";
    }
    for (auto output : outputs_map) {
        (*tensor_map)[output.first] = fw_node->set_body_outputs(output.second);
    }
    for (auto input : inputs_map) {
        if (tensor_map->find(input.first) != tensor_map->end()) {
            auto external_output = tensor_map->at(input.first);
            std::cout << "====set_invariant_inputs " << external_output << " size: " << input.second.size() << "====" << std::endl;
            fw_node->set_invariant_inputs(external_output, input.second);
        } else {
            if (!input_idxs.count(input.first)) {
                // nested subgraphs case
                std::cout << "====nested subgraphs case: " << input.first << " ====" << std::endl;
                auto parameter = std::make_shared<opset7::Parameter>(element::dynamic, PartialShape::dynamic());
                parameter->set_friendly_name(std::to_string(input.first));
                (*tensor_map)[input.first] = parameter;
                external_parameters->push_back(parameter);
                fw_node->set_invariant_inputs(parameter, input.second);
            }
        }
    }
...
std::shared_ptr<ov::Model> convert_pytorch_model(std::shared_ptr<Decoder> pytorch_model,
    ...
    std::shared_ptr<ov::Model> resulting_model;  // define here to make a conversion in a nested scope
    {
        ...
        for (int i = 0; i < inputs.size(); ++i) {
            ...
            parameter->set_friendly_name(std::to_string(pytorch_model->input(i)));
            parameter->output(0).add_names({std::to_string(pytorch_model->input(i))});
        ...
        auto node_visitor = [&](std::shared_ptr<Decoder> node) {
            ...
            for (size_t i = 0; i < raw_inputs.size(); ++i) {
                ...
                if (tensor_map.find(input) == tensor_map.end()) {
                    ...
                    parameter->set_friendly_name(std::to_string(input));
                    parameters.push_back(parameter);
                    //std::cout << "External tensor: " << input << " node: " << external_tensor_map.at(input) << std::endl;
                }
            }
            // std::cerr << "Node convert before translator" << std::endl;

            auto converted_outputs = convert_node(node, &tensor_map, &parameters);
        ...
        for (size_t i = 0; i < pytorch_model->num_of_outputs(); ++i) {
            ...
            if (tensor_map.find(id) == tensor_map.end()) {
                ...
                auto parameter = std::make_shared<opset7::Parameter>(element::dynamic, PartialShape::dynamic());
                parameter->set_friendly_name(std::to_string(id));
                parameters.push_back(parameter);
                tensor_map[id] = parameter;
                std::cout << "Added new parameter based on external value " << id << "\n";
            }
            ...
            ov_output.get_node_shared_ptr()->set_friendly_name(std::to_string(id));
            // std::cout << "X\n";
            // std::cout << ov_output << '\n';
        ...
        std::cout << "=====Tensor map end=====" << std::endl;
    ...
    std::cout << "=====Convert model end=====" << std::endl;
