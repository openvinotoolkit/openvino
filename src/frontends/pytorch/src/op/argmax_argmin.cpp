...
OutputVector create_argmax_argmin_op(const NodeContext& context, TopKMode mode) {
    ...
    if (!context.input_is_none(1)) {
        ...
        auto topk = context.mark_node(std::make_shared<v11::TopK>(input, k, axis, mode, TopKSortType::NONE, element::i64));
        ...
        auto topk = context.mark_node(std::make_shared<v11::TopK>(flatten_input, k, axis, mode, TopKSortType::NONE, element::i64));
        indices = topk->output(1);
        if (keep_dims) {
