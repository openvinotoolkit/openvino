...
OutputVector translate_quantize_per_tensor(const NodeContext& context) {
    ...
    const auto quantized_pt_node = context.mark_node(std::make_shared<QuantizedPtNode>(QuantizedPtNodeType::QUANTIZE_PER_TENSOR,
                                             context,
                                             output,
                                             scale,
                                             zero_point,
...
OutputVector translate_quantize_per_channel(const NodeContext& context) {
    ...
    // TODO
    const auto quantized_pt_node = context.mark_node(std::make_shared<QuantizedPtNode>(QuantizedPtNodeType::QUANTIZE_PER_CHANNEL,
                                             context,
                                             input,
                                             scales,
                                             zero_points,
