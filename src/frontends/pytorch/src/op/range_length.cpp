...
OutputVector translate_range_length(const NodeContext& context) {
    ...
    auto step = context.mark_node(std::make_shared<v0::Convert>(context.get_input(2), ov::element::f64));
    auto length = context.mark_node(std::make_shared<v1::Subtract>(hi, lo));
    auto length_f64 = context.mark_node(std::make_shared<v0::Convert>(length, ov::element::f64));
    ...
    auto ceil_int = context.mark_node(std::make_shared<v0::Convert>(ceil, ov::element::i64));
