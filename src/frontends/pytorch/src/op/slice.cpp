...
OutputVector translate_slice(NodeContext& context) {
    ...
    if (!context.input_is_none(start_idx)) {
        ...
        start = context.mark_node(opset8::Constant::create(element::i64, Shape{1}, {0}));
    }
    ...
    if (!context.input_is_none(end_idx)) {
        ...
        end = context.mark_node(opset8::Constant::create(element::i64, Shape{1}, {INT_MAX}));
    }
    ...
    if (!context.input_is_none(step_idx)) {
        ...
        step = context.mark_node(opset8::Constant::create(element::i64, Shape{1}, {1}));
    }
