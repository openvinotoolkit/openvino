...
OutputVector translate_max_pool_base(const NodeContext& context, const Output& input,
    ...
    if (!context.input_is_none(2)) {
        strides = context.const_input<Strides>(2); // Extract strides if it is provided
    }
    const bool use_kernel = context.input_is_none(2) || (strides.size() == 0);
    if (use_kernel) {
        // In case strides are not provided, default will be the kernel
        strides = kernel;
    }
    Shape pads;
    ...
    // We call the max pooling operation with extracted parameters
...
OutputVector translate_max_pool1d(const NodeContext& context) {
    ...
    return translate_max_pool_base(context, context.const_input(0), kernel_shape, {}, {}, {}, RoundingType::FLOOR, 1); // Pooling dimension for 1D
}
