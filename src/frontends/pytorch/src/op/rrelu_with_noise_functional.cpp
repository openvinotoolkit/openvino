...
OutputVector translate_rrelu_with_noise_functional(const NodeContext& context) {
    // 0 tensor, 1 noise, 2 lower, 3 upper, 4 training, 5 generator
    num_inputs_check(context, 1, 4);
    auto x = context.get_input(0);
    auto noise = context.get_input(1);
    ...
    const auto has_lower = (input_size > 2 && !context.input_is_none(2));
    const auto has_upper = (input_size > 3 && !context.input_is_none(3));
    const bool training = (input_size > 4 && !context.input_is_none(4)) ? context.const_input<bool>(4) : false;
    const auto generator = context.get_input(5);
    (void)training;
    (void)generator;
    if (!has_lower && !has_upper) {
        ...
        return {context.mark_node(std::make_shared<v0::PRelu>(x, average)), noise};
    } else if (has_lower && !has_upper) {
        // lower limit is given
        auto lower_limit = context.get_input(2);
