...
OutputVector translate_inverse(const NodeContext& context) {
    ...
    if (input_type == element::bf16 || input_type == element::f16 || input_type == element::f32) {
        ...
        auto inverse = context.mark_node(std::make_shared<ov::op::v14::Inverse>(input_tensor, false));
        result = context.mark_node(std::make_shared<ov::op::v0::Convert>(input_tensor, input_type));
    }
