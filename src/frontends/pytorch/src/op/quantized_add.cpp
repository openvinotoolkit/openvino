...
OutputVector translate_quantized_add(const NodeContext& context) {
    ...
    return {context.mark_node(quantize(context, quantized_add, scale, zero_point, x))};  // take dtype from x
}
...
OutputVector translate_quantized_add_relu(const NodeContext& context) {
    ...
    return {context.mark_node(quantize(context, quantized_add_relu, scale, zero_point, x))};  // take dtype from x
}
