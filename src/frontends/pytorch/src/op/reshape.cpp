    ...
    if (context.get_input_type(1).is<type::List>()) {
        ...
        if (num_dyn_dims >= 2) {
            ...
            if (inp_shape.rank().is_static()) {
                ...
                if (inp_shape.size() >= 3 && inp_shape.size()+1 == shape_vec.size() && shape_vec[0] == 1 && inp_shape[0] == shape_vec[1]) {
                    // [N, ...] -> [1, N, ...] Can be translated to Unsqueeze
                    auto unsqueeze =
                        context.mark_node(std::make_shared<ov::op::v0::Unsqueeze>(context.get_input(0), zero));
                    return {unsqueeze};
                } else if (shape_vec.size() >= 3 && shape_vec.size()+1 == inp_shape.size() && inp_shape[0] == 1 && inp_shape[1] == shape_vec[0]) {
                    // [1, N, ...] -> [N, ...] Can be translated to Squeeze
                    auto squeeze =
                        context.mark_node(std::make_shared<ov::op::v0::Squeeze>(context.get_input(0), zero));
                    return {squeeze};
                    ...
                    FRONT_END_GENERAL_CHECK(false, "Cannot support reshape with multiple dynamic dimensions for unequal ranks");
                }
	        } else {
                FRONT_END_GENERAL_CHECK(false, "Cannot support reshape with multiple dynamic dimensions for dynamic input ranks");
            }
        }
