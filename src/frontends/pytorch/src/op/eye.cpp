...
OutputVector translate_eye(NodeContext& context) {
    ...
    x = context.mark_node(std::make_shared<opset10::Convert>(x, element::i64));
    Output<Node> y;
    size_t dtype_id;
    auto dtype = element::f32;
    // aten::eye support only main diagonal
    auto diagonal = context.mark_node(opset10::Constant::create(element::i64, Shape{}, {0}));
    if (num_inputs == 5) {
        ...
        y = context.mark_node(std::make_shared<opset10::Convert>(y, element::i64));
        dtype_id = 2;
    } else {
    ...
    if (!context.input_is_none(dtype_id)) {
        dtype = convert_dtype(dtype_id);
    }
    auto eye = context.mark_node(std::make_shared<opset10::Eye>(x, y, diagonal, element::i32));
    return {context.mark_node(std::make_shared<opset10::Convert>(eye, dtype))};
};
