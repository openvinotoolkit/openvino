...
namespace {
Output<ov::Node> translate_quantized_linear_base(const NodeContext& context) {
    // "quantized::linear(Tensor X, __torch__.torch.classes.quantized.LinearPackedParamsBase W_prepack, float Y_scale,
    // int Y_zero_point) -> Tensor Y"
    ...
    if (bias.get_node_shared_ptr() != nullptr) {
...
OutputVector translate_quantized_linear(const NodeContext& context) {
    auto linear = translate_quantized_linear_base(context);
    auto scale = context.get_input(2);
    auto zero_point = context.get_input(3);
    return {quantize(context, linear, scale, zero_point, context.get_input(0))};
}
...
OutputVector translate_quantized_linear_relu(const NodeContext& context) {
    ...
    auto scale = context.get_input(2);
    auto zero_point = context.get_input(3);
    return {quantize(context, relu, scale, zero_point, context.get_input(0))};
}
