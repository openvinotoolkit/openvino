...
OutputVector translate_to_copy(const NodeContext& context) {
    num_inputs_check(context, 5, 6);  // Handle 5 or 6 inputs

    Output<Node> input = context.get_input(0);

    // If dtype is None, return input unmodified
    if (context.input_is_none(2)) {
        return {input};
    }

    // Extract dtype from input(2)
    Output<Node> dtype_node = context.get_input(2);
    auto dtype_const = std::dynamic_pointer_cast<v0::Constant>(dtype_node.get_node_shared_ptr());

    PYTORCH_OP_CONVERSION_CHECK(dtype_const, "Expected dtype (input 2) to be a Constant in aten::_to_copy");
