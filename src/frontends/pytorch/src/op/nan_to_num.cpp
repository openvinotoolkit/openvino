...
OutputVector translate_nan_to_num(const NodeContext& context) {
    ...
    auto nan_value = context.input_is_none(1) ? v0::Constant::create(element::f32, Shape{}, {0}) : context.get_input(1);
    auto posinf_value = context.input_is_none(2)
                            ? v0::Constant::create(element::f32, Shape{}, {std::numeric_limits<float>::max()})
                            : context.get_input(2);
    auto neginf_value = context.input_is_none(3)
                            ? v0::Constant::create(element::f32, Shape{}, {std::numeric_limits<float>::lowest()})
                            : context.get_input(3);

    auto nan_replacement = context.mark_node(std::make_shared<v1::ConvertLike>(nan_value, x));
    ...
    auto replaced_neginf =
        context.mark_node(std::make_shared<v1::Select>(is_neginf, neginf_replacement, replaced_posinf));
