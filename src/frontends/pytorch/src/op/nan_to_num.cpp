...
OutputVector translate_nan_to_num(const NodeContext& context) {
    ...
    auto posinf_value = context.input_is_none(2) ? v0::Constant::create(element::f32, Shape{}, {std::numeric_limits<float>::max()}) : context.get_input(2);
    auto neginf_value = context.input_is_none(3) ? v0::Constant::create(element::f32, Shape{}, {std::numeric_limits<float>::lowest()}) : context.get_input(3);
