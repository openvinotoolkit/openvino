...
OutputVector translate_select(NodeContext& context) {
    ...
    auto dim = context.mark_node(std::make_shared<opset8::Reshape>(context.get_input(1), const_1, false));
    auto start = context.mark_node(std::make_shared<opset8::Reshape>(context.get_input(2), const_1, false));
