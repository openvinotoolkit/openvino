...
OutputVector translate_unfold(NodeContext& context) {
    // constants
    auto const_0 = context.mark_node(Constant::create(element::i64, Shape{}, {0}));
    auto const_1 = context.mark_node(Constant::create(element::i64, Shape{}, {1}));
    auto const_0_list = context.mark_node(Constant::create(element::i64, Shape{1}, {0}));
    auto const_1_list = context.mark_node(Constant::create(element::i64, Shape{1}, {1}));
    auto const_neg_1_list = context.mark_node(Constant::create(element::i64, Shape{1}, {-1}));
    ...
    auto step = context.mark_node(std::make_shared<Convert>(context.get_input(3), element::i64));
    ...
    auto start_indices_unscaled =
        context.mark_node(std::make_shared<Range>(const_0, slices_count_scalar, const_1, element::i64));
    ...
    auto shape_perm = context.mark_node(context.mark_node(Constant::create(element::i64, Shape{2}, {1, 0})));
    auto transpose_indices = context.mark_node(std::make_shared<Transpose>(tile, shape_perm));
    auto range_zero_to_size = context.mark_node(std::make_shared<Range>(const_0, size_scalar, const_1, element::i64));
    ...
    auto perm_begin =
        context.mark_node(std::make_shared<Range>(const_0, dimension_plus_1_scalar, const_1, element::i64));
    auto perm_end =
        context.mark_node(std::make_shared<Range>(dimension_plus_2_scalar, rank_plus_1_scalar, const_1, element::i64));
