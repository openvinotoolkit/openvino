...
AppendListUnpackReplacer::AppendListUnpackReplacer() {
    ...
    ov::matcher_pass_callback callback = [](ov::pass::pattern::Matcher& m) {
        ...
        if (getitem_node) {
            ...
            if (index < -num_inputs || index >= num_inputs) {
                add_exception_to_fw_node(list_unpack,
                                         "prim::ListUnpack: aten::__getitem__ index " + std::to_string(index_val[0]) +
                                             " is out of bounds for input list with " + std::to_string(num_inputs) +
                                             " elements.");
                return false;
            }
            if (index < 0) {
