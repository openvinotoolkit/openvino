    ...
    bool run_on_model (const std::shared_ptr<Model>& model) override {
        ...
        for(size_t i = 0; i < parameters.size(); ++i) {
            ...
            if(std::get<0>(custom_types)) {
                ...
                // Keep original parameters in the model, just detach it from the network -- to avoid parameters renumbering for unchanged parameters
                ...
                for(size_t i = 0; i < 5; ++i) {
                    auto new_parameter = make_shared<opset9::Parameter>(element::i32, PartialShape{Dimension::dynamic()});
                ...
                // for tensor elements
                auto new_parameter = make_shared<opset9::Parameter>(element::dynamic, PartialShape{Dimension::dynamic()});
                new_parameters.push_back(new_parameter);
                inputs_for_list_pack.push_back(new_parameter);

                auto list_pack = make_list_pack(inputs_for_list_pack, std::get<1>(custom_types), parameter->get_output_partial_shape(0));
...
public:
    ...
    DecomposeGetItem() {
        ...
        auto list_pack = wrap_type<FrameworkNode>({begins, ends, shape_begins, shape_ends, shape_dims, tensor_elements});
        ...
        ov::matcher_pass_callback callback = [=](Matcher& m) {
            ...
            auto elements = make_shared<opset9::StridedSlice>(
                matches.at(tensor_elements),
            ...
            auto shape = make_shared<opset9::StridedSlice>(
...
public:
    ...
    DecomposeAppend() {
        ...
        ov::matcher_pass_callback callback = [=](Matcher& m) {
            ...
            auto new_shape_begins = make_shared<opset9::Concat>(NodeVector{matches.at(shape_begins), cur_shape_dims_size}, 0);
            ...
            auto new_shape_ends = make_shared<opset9::Concat>(NodeVector{matches.at(shape_ends), new_shape_dims_size}, 0);

            // Appending new elements after flattening to existing elements

            auto item_flatten = make_shared<opset9::Reshape>(matches.at(item), opset9::Constant::create(element::i32, {1}, {-1}), false);
            auto new_begins = make_shared<opset9::Concat>(NodeVector{matches.at(begins), make_shared<opset9::ShapeOf>(matches.at(elements), element::i32)}, 0);
            ...
            auto new_elements =
                (initial_elements_const && shape_size(initial_elements_const->get_output_shape(0)) == 0) ?
                    shared_ptr<Node>(item_flatten) :   // empty initial list -- just take appended elements as a new content for the list; derive type from that tensor
                    shared_ptr<Node>(make_shared<opset9::Concat>(NodeVector{matches.at(elements), item_flatten}, 0));   // existing list, just concat
            
            auto new_ends = make_shared<opset9::Concat>(NodeVector{matches.at(ends), make_shared<opset9::ShapeOf>(new_elements, element::i32)}, 0);

            auto new_list_pack = make_list_pack(
                {new_begins, new_ends, new_shape_begins, new_shape_ends, new_shape_dims, new_elements},
...
class DecomposeListResults : public pass::ModelPass {
    ...
    bool run_on_model (const std::shared_ptr<Model>& model) override {
        ...
        ResultVector results = model->get_results();    // make a copy, leter results in the model are going to be modified
