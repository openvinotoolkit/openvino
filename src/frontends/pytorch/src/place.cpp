...
Place::Place(const ov::frontend::InputModel& input_model, size_t tensor_index)
    ...
    if (in_it != inputs.end()) {
        m_is_input = true;
        const auto& signature_name = decoder->get_input_signature_name(std::distance(inputs.begin(), in_it));
        m_names.push_back(signature_name);
    }
    auto out_it = std::find(outputs.begin(), outputs.end(), tensor_index);
    if (out_it != outputs.end()) {
        m_is_output = true;
        const auto& debug_name = decoder->get_output_debug_name(std::distance(outputs.begin(), out_it));
