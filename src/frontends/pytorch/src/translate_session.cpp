    ...
    std::shared_ptr<Model> resulting_model;  // define here to make a conversion in a nested scope
    {
        ...
        auto node_visitor =
            [&](std::shared_ptr<TorchDecoder> node) {
                // Explore all inputs of node. Node may refer to input value that hasn't been created in the current
                // scope. But this value can be found in the outer scope, for this purpose we create new input for the
                // model to link with external scope on a higher level.
                ...
                // Ops with subgraphs or with mutated inputs may have more outputs after conversion compared to pytorch
                // ones
                ...
                for (size_t i = 0; i < fw_outputs.size(); ++i) {
                    ...
                    if (node->inputs().size() > 0 && node->may_produce_alias(0, i)) {
                        ...
                        if (m_may_be_alias.count(fw_tensor_id)) {
                            ...
                            std::tie(recorded_in_tensor_id, recorded_node, std::ignore) =
                                m_may_be_alias.at(fw_tensor_id);
                ...
                FRONT_END_GENERAL_CHECK(pytorch_model->get_subgraph_size() == 1,
                                        "Model should have exactly 1 subgraph.");
                ...
                // Did a conversion in a nested scope to automatically remove any holders of nodes except those in the
                // graph
            }
