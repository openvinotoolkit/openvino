...
void ParsePreProcess(pugi::xml_node& root,
    ...
    inputName = pugixml_utils::GetStrAttr(ppNode, "reference-layer-name", "");
    ...
    FOREACH_CHILD (chan, ppNode, "channel") {
        auto chanNo = pugixml_utils::GetUInt64Attr(chan, "id", next_channel_id++);
        ...
        if (!meanNode.empty()) {
            ...
            if (meanNode.attribute("value")) {
                mean_scalar_values.insert({chanNo, pugixml_utils::GetFloatAttr(meanNode, "value")});
            }
            if (meanNode.attribute("size") && meanNode.attribute("offset")) {
                auto const_size = pugixml_utils::GetUInt64Attr(meanNode, "size");
                auto const_offset = pugixml_utils::GetUInt64Attr(meanNode, "offset");
...
std::shared_ptr<Function> InputModel::InputModelIRImpl::convert() {
    ...
    // Load default opsets
    size_t version = pugixml_utils::GetUIntAttr(m_root, "version", 0);
