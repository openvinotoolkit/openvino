...
bool op::internal::FakeQuantDequantInternal::visit_attributes(AttributeVisitor& visitor) {
    visitor.on_attribute("op_type", m_op_type);
    visitor.on_attribute("quant_axis", m_quant_axis);
    visitor.on_attribute("bit_length", m_bit_length);
    return true;
}

void op::internal::FakeQuantDequantInternal::validate_and_infer_types() {
