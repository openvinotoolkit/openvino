...
namespace paddle {
namespace op {
NamedOutputs softshrink(const NodeContext& node_context) {
    ...
    //comparison
    auto pos_mask = std::make_shared<Greater>(loss, pos_lam);
    auto neg_mask = std::make_shared<Greater>(neg_lam, loss);
    //select
    ...
    auto mid_result = std::make_shared<Select>(pos_mask, positive, zero);
    auto result = std::make_shared<Select>(neg_mask, negative, mid_result);
