...
NamedOutputs less_than(const NodeContext& node) {
    ...
    if ((axis == -1) || (axis == x_rank - 1) || (x_rank == y_rank)) {
        return node.default_single_output_mapping({std::make_shared<default_opset::Less>(x, y)}, {"Out"});
    } else {
        ...
        return node.default_single_output_mapping({std::make_shared<default_opset::Less>(x, y_node)}, {"Out"});
    }
}
}  // namespace op
}  // namespace paddle
