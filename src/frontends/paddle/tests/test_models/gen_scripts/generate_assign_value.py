...
def paddle_assign_value(name, test_x):
    ...
    with paddle.static.program_guard(main_program, startup_program):
        node_x = paddle.static.data(
            name="x",
            shape=test_x.shape,
            dtype=test_x.dtype if test_x.dtype != bool else np.int32,
        )
...
def compare():
    ...
    test_cases = [
        ...
        {"name": "assign_value_int32", "input": np.ones([1, 1, 4, 4]).astype(np.int32)},
        {"name": "assign_value_int64", "input": np.ones([1, 1, 4, 4]).astype(np.int64)},
        {"name": "assign_value_boolean", "input": np.array([False, True, False])},
    ]
