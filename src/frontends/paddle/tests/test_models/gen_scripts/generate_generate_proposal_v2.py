...
def generate_proposals_v2(name: str, input_data: dict, attr: dict):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        scores = paddle.static.data(
            name="scores", shape=scores_np.shape, dtype="float32"
        )  # [N, A, H, W]
        bbox_deltas = paddle.static.data(
            name="bbox_deltas", shape=bbox_deltas_np.shape, dtype="float32"
        )  # [N, 4 * A, H, W]
        im_shape = paddle.static.data(
            name="im_shape", shape=im_shape_np.shape, dtype="float32"
        )  # [N, 2]
        anchors = paddle.static.data(
            name="anchors", shape=anchors_np.shape, dtype="float32"
        )  # [H, W, A, 4]
        variances = paddle.static.data(
            name="var", shape=variances_np.shape, dtype="float32"
        )  # [H, W, A, 4]
        ...
        saveModel(
            name,
            exe,
            feedkeys=["scores", "bbox_deltas", "im_shape", "anchors", "var"],
...
if __name__ == "__main__":
    ...
    input_data["anchors"] = np.reshape(np.arange(4 * 4 * 3 * 4), [4, 4, 3, 4]).astype(
        "float32"
    )
    ...
    input_data["bbox_deltas"] = np.concatenate(
        (bbox_deltas0, bbox_deltas0, bbox_deltas1), axis=2
    )

    anchors0 = np.reshape(np.arange(1 * 4 * 3 * 4), [1, 4, 3, 4]).astype("float32")
    anchors1 = np.reshape(np.arange(3 * 4 * 3 * 4), [3, 4, 3, 4]).astype("float32")
