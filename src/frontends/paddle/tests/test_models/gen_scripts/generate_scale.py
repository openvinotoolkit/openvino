...
def paddle_scale(name: str, x, scale, bias, attrs: dict, data_type):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        out = paddle.scale(
            x=node_x, scale=scale, bias=bias, bias_after_scale=attrs["bias_after_scale"]
        )
...
def main():
    ...
    paddle_scale_tensor(
        "scale_tensor_bias_after", data, scale, bias, paddle_attrs, "float32"
    )
    ...
    paddle_scale_tensor(
        "scale_tensor_bias_before", data, scale, bias, paddle_attrs, "float32"
    )
