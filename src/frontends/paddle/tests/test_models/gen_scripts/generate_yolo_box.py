...
def yolo_box(name: str, x, img_size, attrs: dict):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        node_img_size = paddle.static.data(
            name="img_size", shape=img_size.shape, dtype=img_size.dtype
        )
        ...
        outs = exe.run(feed={"x": x, "img_size": img_size}, fetch_list=[boxes, scores])
...
def TEST2():
    ...
    x_shape = (
        N,
        num_anchors * (5 + paddle_attrs["class_num"]),
        SPATIAL_HEIGHT,
        SPATIAL_WIDTH,
    )
    imgsize_shape = (N, 2)
