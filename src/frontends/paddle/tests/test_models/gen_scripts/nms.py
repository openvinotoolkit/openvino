...
from save_model import exportModel, print_alike, saveModel
...
def NMS(name: str, bboxes, scores, attrs: dict, rois_num=None, verbose=False):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        node_boxes = paddle.static.data(
            name="bboxes", shape=[-1, -1, 4], dtype=bboxes.dtype, lod_level=1
        )
        node_scores = paddle.static.data(
            name="scores",
        ...
        if rois_num is not None:
            node_rois_num = paddle.static.data(
                name="rois_num", shape=rois_num.shape, dtype=rois_num.dtype, lod_level=1
            )

        if attrs["nms_type"] is "multiclass_nms3":
            nms_outputs = multiclass_nms(
                bboxes=node_boxes,
                scores=node_scores,
        ...
        output_lod = exe.run(feed=feed_dict, fetch_list=fetch_vars, return_numpy=False)
        ...
        nms_rois_num = np.array(output_lod.pop(0)) if output[1] is not None else None
        if paddle.__version__ >= "2.0.0":
            index = (
                np.array(output_lod.pop(0)).astype(
                    data_feeder.convert_dtype(output[2].dtype)
                )
            ...
            index = (
                np.array(output_lod.pop(0)).astype(
                    paddle.fluid.data_feeder.convert_dtype(output[2].dtype)
                )
                if output[2] is not None
                else None
            )
    ...
    if verbose:
        ...
        print("\033[91m" + "nms_rois_num_np: {}".format(nms_rois_num.shape) + "\033[0m")
