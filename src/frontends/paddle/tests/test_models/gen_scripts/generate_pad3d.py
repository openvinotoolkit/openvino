...
def pad3d(name: str, x, in_dtype, pad, data_format, mode, value=0):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        if mode == "constant":
            pad_constant = paddle.nn.Pad3D(
                padding=pad, mode=mode, value=value, data_format=data_format
            )
            out = pad_constant(node_x)
        else:
            pad_other_mode = paddle.nn.Pad3D(
                padding=pad, mode=mode, data_format=data_format
            )
