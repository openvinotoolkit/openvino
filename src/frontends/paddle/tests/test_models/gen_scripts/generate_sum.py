...
def sum_(name: str, input):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        outs = exe.run(feed={"data": input}, fetch_list=[out])
        saveModel(
            name,
            exe,
            feedkeys=["data"],
...
def sum(name: str, inputs):
    paddle.enable_static()
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        data0 = paddle.static.data(
            "data0", shape=inputs[0].shape, dtype=inputs[0].dtype
        )
        data1 = paddle.static.data(
            "data1", shape=inputs[1].shape, dtype=inputs[1].dtype
        )
        data2 = paddle.static.data(
            "data2", shape=inputs[2].shape, dtype=inputs[2].dtype
        )
        ...
        exe.run(paddle.static.default_startup_program())
        outs = exe.run(
            feed={"data0": inputs[0], "data1": inputs[1], "data2": inputs[2]},
            fetch_list=[out],
        )
        saveModel(
            name,
            exe,
            feedkeys=["data0", "data1", "data2"],
            fetchlist=[out],
