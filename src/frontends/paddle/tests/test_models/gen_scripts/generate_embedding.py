...
def ov_embedding(ids, vocab_embeddings, vocab_size, embedding_dim, padding_idx, sparse):
    ...
    node_w = opset8.parameter(
        shape=vocab_embeddings.shape, name="w", dtype=vocab_embeddings.dtype
    )
    ...
    node_gather = opset8.gather(
        data=node_masked_w if padding_idx else node_w,
        indices=node_ids,
        axis=node_axis,
        batch_dims=0,
    )
...
def embedding(
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        node_embedding = paddle.nn.Embedding(
        ...
        infer_results = exe.run(feed=input_dict, fetch_list=output_vars_list)

        saveModel(
            name,
            exe,
            feedkeys=[node_ids],
            fetchlist=output_vars_list,
    ...
    if compare:
        ...
        match = np.all(np.isclose(paddle_result, ng_result, rtol=1e-4, atol=1e-5))
        ...
        if not match:
            ...
            print(
                prefix_color + "paddle_result: {}".format(paddle_result) + "\033[0m\n"
            )
...
if __name__ == "__main__":
    ...
    if paddle.__version__ >= "2.0.0":
        outputs = embedding(
            "embedding_tensorIds_paddings",
            ids,
            vocab_size,
            embedding_dim,
            padding_idx=np.compat.long(padding_idx),
            vocab_embeddings=table,
            compare=False,
        )
    else:
