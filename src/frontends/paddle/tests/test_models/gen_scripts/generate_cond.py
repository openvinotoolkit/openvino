...
def test_model(pred):
    ...
    def true_func():
        return paddle.full(
            shape=[3, 4],
            dtype="float32",  # TODO: FAILED with different dtype
            fill_value=1,
        )

    def false_func():
        return paddle.full(shape=[1, 2], dtype="float32", fill_value=3)
...
def test_model_2outputs(pred):
    # pred: A boolean tensor whose numel should be 1.
    def true_func():
        return paddle.full(shape=[1, 2], dtype="float32", fill_value=1), paddle.full(
            shape=[1, 3],
            dtype="float32",  # TODO: FAILED with different dtype
            fill_value=3,
        )
    ...
    def false_func():
        return paddle.full(shape=[3, 4], dtype="float32", fill_value=3), paddle.full(
            shape=[1, 4], dtype="float32", fill_value=4
        )
...
# 95436: sporadic failure
exportModel(
    "conditional_block_const_2outputs",
    test_model_2outputs,
    [data],
...
# 95436: sporadic failure
exportModel(
    "conditional_block_2inputs_2outputs",
    test_model_2inputs_2outputs,
    [a, b],
    target_dir=sys.argv[1],
)
...
# 95436: sporadic failure
exportModel(
    "conditional_block_2inputs_dyn", test_model_dyn, [a, b], target_dir=sys.argv[1]
)
...
# 95436: sporadic failure
exportModel(
    "conditional_block_2inputs_dyn_2outputs",
    test_model_dyn_2outputs,
    [a, b],
    target_dir=sys.argv[1],
)
...
# 95436: sporadic failure
exportModel(
    "conditional_block_dyn_conditionalblock_only",
    test_model_dyn_conditionalblock_only,
    [a],
    target_dir=sys.argv[1],
)
...
# 95436: sporadic failure
exportModel(
    "conditional_block_dyn_multiple_consumers",
    test_model_dyn_multiple_consumers,
    [a, b],
    target_dir=sys.argv[1],
)
...
# 95436: sporadic failure
exportModel(
    "conditional_block_dyn_multiple_blocks",
    test_model_dyn_multiple_blocks,
    [a, b, c],
    target_dir=sys.argv[1],
)
...
# 95436: sporadic failure
exportModel(
    "conditional_block_dyn_multiple_blocks2",
    test_model_dyn_multiple_blocks2,
    [a, b, c],
    target_dir=sys.argv[1],
)
