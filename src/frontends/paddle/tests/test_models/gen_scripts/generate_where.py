...
def where(name, test_x, test_y, test_cond):
    ...
    with paddle.static.program_guard(main_program, startup_program):
        X_Node = paddle.static.data(name="x", shape=test_x.shape, dtype=test_x.dtype)
        Y_Node = paddle.static.data(name="y", shape=test_y.shape, dtype=test_y.dtype)
        ...
        outs = exe.run(
            feed={"x": test_x, "y": test_y, "cond": test_cond}, fetch_list=[out]
        )
        ...
        saveModel(
            ...
            feedkeys=["x", "y", "cond"],
            fetchlist=[out],
            inputs=[test_x, test_y, test_cond],
            outputs=[outs[0]],
