...
def paddle_rnn_lstm(input_size, hidden_size, layers, direction, seq_len):
    ...
    with paddle.static.program_guard(main_program, startup_program):
        ...
        if seq_len:
            outs = exe.run(
                feed={
                    "x": np.ones([4, 3, input_size]).astype(np.float32),
                    "sl": np.array(seq_len).astype(np.int32),
                },
                fetch_list=[y, h, c],
            ...
            saveModel(
                ...
                + str(len(seq_len)),
                exe,
                feedkeys=[data, seq_lengths],
                fetchlist=[y, h, c],
                inputs=[
                    np.ones([4, 3, input_size]).astype(np.float32),
                    np.array(seq_len).astype(np.int32),
                ],
                outputs=[outs[0], outs[1], outs[2]],
            ...
            saveModel(
                "rnn_lstm_layer_" + str(layers) + "_" + str(direction),
                exe,
                feedkeys=[data],
                fetchlist=[y, h, c],
