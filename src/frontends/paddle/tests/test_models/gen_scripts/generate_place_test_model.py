...
def paddle_rnn_lstm(input_size, hidden_size, layers, direction):
    ...
    with paddle.static.program_guard(main_program, startup_program):
        ...
        prev_h = paddle.ones(
            shape=[layers * num_of_directions, 4, hidden_size],
            dtype=np.float32,
            name="const_1",
        )
        prev_c = paddle.ones(
        ...
        if paddle.__version__ >= "2.0.0":
            ...
            path_prefix = os.path.join(
                sys.argv[1], "place_test_model", "place_test_model"
            )
            ...
            condition = (
                lambda v: not v.persistable
                and v.name != "transpose_1.tmp_1"
                and v.name != "transpose_0.tmp_1"
            )
            vars_ = list(filter(condition, program.list_vars()))
            vars_name = [v.name for v in vars_]
            vars_name_file = os.path.join(
                sys.argv[1], "place_test_model", "vars_name.txt"
            )
            ...
            outputs_name_file = os.path.join(
                sys.argv[1], "place_test_model", "outputs_name.txt"
            )
...
if __name__ == "__main__":
    ...
    for test in testCases:
        paddle_rnn_lstm(
            test["input_size"], test["hidden_size"], test["layers"], test["direction"]
        )
