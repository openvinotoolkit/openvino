...
def top_k_v2(name: str, x, k: int, axis=None, largest=True, sorted=True, k_is_var=True):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        input_k = (
            paddle.static.data(name="k", shape=[1], dtype="int32") if k_is_var else k
        )
        ...
        outs = exe.run(feed=feed_list, fetch_list=[value, indices])
...
def main():
    ...
    top_k_v2("top_k_v2_test_4", data, k=7, axis=None, largest=True, sorted=True)
    top_k_v2("top_k_v2_test_5", data, k=6, axis=2, largest=False, sorted=True)
    top_k_v2(
        "top_k_v2_test_6", data, k=6, axis=2, largest=False, sorted=True, k_is_var=False
    )
