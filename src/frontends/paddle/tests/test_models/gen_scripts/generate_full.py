...
def full(name: str, shape: list, dtype, value):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        if paddle.__version__ >= "2.0.0":
            ...
            x1 = paddle.fluid.layers.fill_constant(
                shape=shape, value=value, dtype=dtype, name="fill_constant"
            )
            x2 = paddle.fluid.layers.fill_constant(
                shape=shape, value=value, dtype=dtype, name="fill_constant"
            )
...
def full_tensor(name: str, shape: list, dtype, value):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        if paddle.__version__ >= "2.5.1":
            ...
            x1 = paddle.full(
                shape=shape, fill_value=node_value, dtype=dtype, name="full1"
            )
            ...
            node_value = paddle.static.data(name="value", shape=[1], dtype=dtype)
            x1 = paddle.fluid.layers.fill_constant(
                shape=shape, value=node_value, dtype=dtype, name="fill_constant1"
            )
...
def full_shape_tensor(name: str, shape, dtype, value):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        if paddle.__version__ >= "2.0.0":
            node_shape = paddle.full(
                shape=[2], fill_value=shape, dtype="int32", name="shape"
            )
            x1 = paddle.full(
                shape=node_shape, fill_value=value, dtype=dtype, name="full"
            )
        else:
            node_shape = paddle.fluid.layers.fill_constant(
                shape=[1], value=shape, dtype="int32", name="shape"
            )
            x1 = paddle.fluid.layers.fill_constant(
                shape=[2, node_shape], value=value, dtype=dtype, name="fill_constant"
            )
        out = paddle.cast(x1, np.float32)
        cpu = paddle.static.cpu_places(1)
        exe = paddle.static.Executor(cpu[0])
...
def full_shape_tensor_list(name: str, shape: list, dtype, value):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        node_shape = paddle.full(
            shape=[1], fill_value=shape, dtype="int32", name="shape"
        )
        x1 = paddle.full(
            shape=[2, node_shape], fill_value=value, dtype=dtype, name="full"
        )
        out = paddle.cast(x1, np.float32)
        cpu = paddle.static.cpu_places(1)
        exe = paddle.static.Executor(cpu[0])
