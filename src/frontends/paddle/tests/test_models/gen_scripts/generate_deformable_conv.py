...
def deformable_conv(
    ...
    with pdpd.static.program_guard(pdpd.static.Program(), pdpd.static.Program()):
        ...
        node_out = pdpd.vision.ops.deform_conv2d(
            node_x,
            node_offset,
            node_weight,
        ...
        outs = exe.run(feed=feed_dict, fetch_list=node_out)
...
def get_output_size(input_size, out_channels, kernel_size, stride, padding, dilation):
    ...
    out_h = int(
        out_size(input_size[2], padding[0], dilation[0], kernel_size[0], stride[0])
    )
    assert out_h > 0

    out_w = int(
        out_size(input_size[3], padding[1], dilation[1], kernel_size[1], stride[1])
    )
    assert out_w > 0
...
def generator(
    ...
    offset_c = 2 * deformable_groups * filter_size[2] * filter_size[3]
    mask_c = deformable_groups * filter_size[2] * filter_size[3]
    offset_size = [input_size[0], offset_c, output_size[2], output_size[3]]  # offset
    mask_size = [input_size[0], mask_c, output_size[2], output_size[3]]  # mask
    ...
    offset = 10 * np.random.uniform(size=offset_size).astype("int").astype(
        dtype
    )  # TODO: negative, fractioned

    mask = 10 * np.random.random(size=mask_size).astype(dtype) if not no_mask else None

    bias = (
        np.random.uniform(-1, 1, size=(filter_size[0],)).astype(dtype)
        if not no_bias
        else None
    )
...
def TestWithPad():
    padding = 1
    data_x, data_weight, data_offset, data_mask, data_bias = generator(
        input_size=[1, 1, 4, 4], kernel_size=[3, 3], out_channels=1, padding=padding
    )
    deformable_conv(
...
def TestWithPadTuple():
    padding = (3, 3)
    data_x, data_weight, data_offset, data_mask, data_bias = generator(padding=padding)
    deformable_conv(
...
def TestWithStride():
    stride = 2
    data_x, data_weight, data_offset, data_mask, data_bias = generator(stride=stride)
    deformable_conv(
        "deformable_conv_with_stride",
        data_x,
        data_weight,
        data_offset,
        data_mask,
...
def TestWithStrideTuple():
    ...
    data_x, data_weight, data_offset, data_mask, data_bias = generator(stride=stride)
    deformable_conv(
        "deformable_conv_with_stride_tuple",
        data_x,
        data_weight,
        data_offset,
        data_mask,
...
def TestWithStrideList():
    ...
    data_x, data_weight, data_offset, data_mask, data_bias = generator(stride=stride)
    deformable_conv(
        "deformable_conv_with_stride_list",
        data_x,
        data_weight,
        data_offset,
        data_mask,
...
def TestWithDilationTuple():
    ...
    deformable_conv(
        "deformable_conv_with_dilation_tuple",
        data_x,
        data_weight,
        data_offset,
        data_mask,
...
def TestWithDilationList():
    ...
    deformable_conv(
        "deformable_conv_with_dilation_list",
        data_x,
        data_weight,
        data_offset,
        data_mask,
...
def TestWithGroup():
    data_x, data_weight, data_offset, data_mask, data_bias = generator(groups=2)
    deformable_conv(
        "deformable_conv_with_groups",
        data_x,
        data_weight,
        data_offset,
        data_mask,
...
def TestWithDeformableGroups():
    ...
    deformable_conv(
        "deformable_conv_with_deformable_groups",
        data_x,
        data_weight,
        data_offset,
        data_mask,
...
def TestWithMask():
    data_x, data_weight, data_offset, data_mask, data_bias = generator(no_mask=False)
    deformable_conv(
        "deformable_conv_with_mask",
        data_x,
        data_weight,
        data_offset,
        data_mask,
...
def TestWithBias():
    data_x, data_weight, data_offset, data_mask, data_bias = generator(no_bias=False)
    deformable_conv(
        "deformable_conv_with_bias",
        data_x,
        data_weight,
        data_offset,
        data_mask,
...
def TestWithMaskBias():
    ...
    deformable_conv(
        "deformable_conv_with_mask_bias",
        data_x,
        data_weight,
        data_offset,
        data_mask,
