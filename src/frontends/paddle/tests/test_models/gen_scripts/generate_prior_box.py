...
def prior_box(name: str, input_data, image_data, attrs: dict):
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        outs = exe.run(
            feed={"Input": input_data, "Image": image_data}, fetch_list=[box, var]
        )
        ...
        saveModel(
            name,
            exe,
            feedkeys=["Input", "Image"],
            fetchlist=[box, var],
            inputs=[input_data, image_data],
            outputs=outs,
...
if __name__ == "__main__":
    ...
    input_data = np.random.random(
        (batch_size, input_channels, layer_w, layer_h)
    ).astype("float32")

    image_data = np.random.random(
        (batch_size, image_channels, image_w, image_h)
    ).astype("float32")
