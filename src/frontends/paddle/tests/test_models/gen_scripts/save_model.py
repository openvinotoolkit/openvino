...
def saveModel(
    ...
    for i, input in enumerate(inputs):
        ...
        np.save(
            os.path.join(model_dir, "input{}.{}.{}".format(i, feedkey, input.dtype)),
            input,
        )
    # print("\n")
...
def exportModel(
    name, dyn_func, input_data: list, target_dir: str, dyn_shapes: list = []
):
    model_dir = os.path.join(target_dir, name)
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
    ...
    for idx, data in enumerate(input_data):
        ...
        input_shape = (
            dyn_shapes[idx]
            if len(dyn_shapes) > 0 and dyn_shapes[idx] is not None
            else data.shape
        )

        input_specs.append(
            paddle.static.InputSpec(
                shape=input_shape, dtype=data.dtype, name=input_name
            )
        )
