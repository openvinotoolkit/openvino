...
def loop():
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        out, dummy = paddle.static.nn.while_loop(
            cond, body, [node_i, node_ten], name="while_loop"
        )

        exe = paddle.static.Executor(place=paddle.CPUPlace())
        res = exe.run(
            paddle.static.default_main_program(), feed={"x": x}, fetch_list=out
        )
...
def loop_t():
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        out_i, out_t = paddle.static.nn.while_loop(
            cond, body, [node_i, node_ten], name="while_loop"
        )
...
def loop_if_loop_complex():
    ...
    return exportModel(
        "loop_if_loop_complex", test_model, [x, y], target_dir=sys.argv[1]
    )
