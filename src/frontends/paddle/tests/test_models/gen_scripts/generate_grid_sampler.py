...
def grid_sampler(
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        if is_dynamic:
            ...
            grid_node = paddle.static.data(
                name="grid", shape=(-1, -1, -1, 2), dtype=grid.dtype
            )
        else:
            x_node = paddle.static.data(name="x", shape=x.shape, dtype=x.dtype)
            grid_node = paddle.static.data(
                name="grid", shape=grid.shape, dtype=grid.dtype
            )
        out = (
            ...
            if not_empty
            else paddle.nn.functional.grid_sample(x_node, grid_node)
        )
