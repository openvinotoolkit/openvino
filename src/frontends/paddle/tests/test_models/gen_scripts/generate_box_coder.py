...
def test_box_coder(
    ...
    with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
        ...
        prior_box_decode = paddle.static.data(
            name="prior_box", shape=prior_box.shape, dtype=dtype
        )
        target_box_decode = paddle.static.data(
            name="target_box", shape=target_box.shape, dtype=dtype
        )
        if is_tensor:
            prior_box_var_decode = paddle.static.data(
                name="prior_box_var", shape=prior_box_var.shape, dtype=dtype
            )
        else:
        ...
        outs = exe.run(feed=feed_dict, fetch_list=[out])

        saveModel(
            name,
            exe,
            feedkeys=[*feed_dict.keys()],
            fetchlist=[out],
            inputs=[*feed_dict.values()],
            outputs=[outs[0]],
...
def box_coder_dygraph():
    ...
    def test_model_1(prior_box, target_box):
        ...
        axis = 1
        out = box_coder(
            prior_box, prior_box_var, target_box, code_type, box_normalized, axis=axis
        )
        return out
    ...
    prior_box_var = paddle.tile(
        paddle.to_tensor([[0.1, 0.2, 0.1, 0.1]], dtype=datatype),
        [prior_box.shape[0], 1],
    )
