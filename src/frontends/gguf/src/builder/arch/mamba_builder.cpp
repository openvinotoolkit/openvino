...
public:
    ...
    std::shared_ptr<GgufGraph> build() override {
        ...
        std::vector<std::pair<GgufValue, GgufValue>> recurrent_states;
        ...
        for (int64_t layer = 0; layer < layers; ++layer) {
            ...
            if (hybrid && !weights.has(prefix + "ssm_in.weight")) {
                ...
                recurrent_states.emplace_back(conv_state, slice(window, -(kernel - 1), kernel - 1));
        ...
        // Keep logits first in the stateless contract; recurrent updates follow.
        for (const auto& state : recurrent_states)
            g.add_recurrent_state(state.first, state.second);
        return g.finish();
    }
