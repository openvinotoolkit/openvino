...
std::string swiglu_gate_up(GraphEmitter& e,
                           ...
                           bool has_bias) {
    e.add_weight(gate_w);
    auto gate = e.add_op("GGML_OP_MUL_MAT", gate_name, {gate_w, ffn_norm}, f32);
    if (has_bias) {
        gate = add_bias(e, gate, bias_weight_name(gate_w), gate_name + "_b");
    }
    e.add_weight(up_w);
    auto up = e.add_op("GGML_OP_MUL_MAT", up_name, {up_w, ffn_norm}, f32);
    if (has_bias) {
        up = add_bias(e, up, bias_weight_name(up_w), up_name + "_b");
    }
    return e.add_op("GGML_GLU_OP_SWIGLU", glu_name, {gate, up}, f32, 0, {{"swapped", false}});
}
...
std::string geglu_ffn(GraphEmitter& e, const DecoderConfig& cfg, const std::string& p, const std::string& ffn_norm) {
    ...
    auto gate = e.add_op("GGML_OP_MUL_MAT", p + "ffn_gate", {p + "ffn_gate.weight", ffn_norm}, f32);
    auto up = e.add_op("GGML_OP_MUL_MAT", p + "ffn_up", {p + "ffn_up.weight", ffn_norm}, f32);
    auto glu = e.add_op("GGML_GLU_OP_GEGLU", p + "ffn_geglu", {gate, up}, f32, 0, {{"swapped", false}});
    return e.add_op("GGML_OP_MUL_MAT", p + "ffn_out", {p + "ffn_down.weight", glu}, f32);
}
...
std::string dense_ffn(GraphEmitter& e, const DecoderConfig& cfg, const std::string& p, const std::string& ffn_norm) {
    ...
    if (fused_ffn) {
        auto up = e.add_op("GGML_OP_MUL_MAT", p + "ffn_up", {p + "ffn_up.weight", ffn_norm}, f32);
        glu = e.add_op("GGML_GLU_OP_SWIGLU", p + "ffn_swiglu", {up}, f32, 0, {{"swapped", false}});
    } else {
    ...
    auto out = e.add_op("GGML_OP_MUL_MAT", p + "ffn_out", {p + "ffn_down.weight", glu}, f32);
    if (has_ffn_bias) {
...
std::string moe_ffn(GraphEmitter& e, const DecoderConfig& cfg, const std::string& p, const std::string& ffn_norm) {
    ...
    auto logits = e.add_op("GGML_OP_MUL_MAT", p + "moe_logits", {p + "ffn_gate_inp.weight", ffn_norm}, f32);
    ...
    if (cfg.moe_sigmoid_gating) {
        probs = e.add_op("GGML_UNARY_OP_SIGMOID", p + "moe_probs", {logits}, f32);
    } else if (!cfg.moe_softmax_weight) {
        probs = e.add_op("GGML_OP_SOFT_MAX", p + "moe_probs", {logits}, f32, 0, {{"softmax_axis", int64_t(-1)}});
    }
    ...
    auto weights = e.add_op("GGML_OP_GET_ROWS", p + "moe_w", {probs, selected}, f32, 10);
    if (cfg.moe_softmax_weight) {
        weights = e.add_op("GGML_OP_SOFT_MAX", p + "moe_w_sm", {weights}, f32, 0, {{"softmax_axis", int64_t(2)}});
    }
    ...
    if (cfg.expert_weights_norm && !cfg.moe_softmax_weight) {
        auto w_tr = e.add_op("GGML_OP_TRANSPOSE", p + "moe_w_tr", {weights}, f32);
        auto w_sum = e.add_op("GGML_OP_SUM_ROWS", p + "moe_w_sum", {w_tr}, f32);
        ...
        weights = e.add_op("GGML_OP_DIV", p + "moe_w_norm", {weights, w_sum}, f32);
    }
    ...
    auto up = e.add_op("GGML_OP_MUL_MAT_ID", p + "moe_up", {p + "ffn_up_exps.weight", ffn_norm, selected}, f32);
    if (eb) {
        e.add_named_weight(p + "ffn_up_exps.bias");
        up = e.add_op("GGML_OP_ADD_ID", p + "moe_up_b", {up, p + "ffn_up_exps.bias", selected}, f32);
    }
    auto gate = e.add_op("GGML_OP_MUL_MAT_ID", p + "moe_gate", {p + "ffn_gate_exps.weight", ffn_norm, selected}, f32);
    if (eb) {
        e.add_named_weight(p + "ffn_gate_exps.bias");
        gate = e.add_op("GGML_OP_ADD_ID", p + "moe_gate_b", {gate, p + "ffn_gate_exps.bias", selected}, f32);
    }
    std::string act;
    if (cfg.moe_swiglu_oai) {
        ...
        act = e.add_op("GGML_GLU_OP_SWIGLU", p + "moe_act", {gate, up}, f32, 0, {{"swapped", false}});
    }
    auto experts = e.add_op("GGML_OP_MUL_MAT_ID", p + "moe_down", {p + "ffn_down_exps.weight", act, selected}, f32);
    if (eb) {
        e.add_named_weight(p + "ffn_down_exps.bias");
        experts = e.add_op("GGML_OP_ADD_ID", p + "moe_down_b", {experts, p + "ffn_down_exps.bias", selected}, f32);
    }
    ...
    if (e.has_weight(p + "ffn_gate_shexp.weight")) {
        ...
        auto s_down = e.add_op("GGML_OP_MUL_MAT", p + "shexp_out", {p + "ffn_down_shexp.weight", s_act}, f32);
        moe_out = e.add_op("GGML_OP_ADD", p + "moe_shared_out", {moe_out, s_down}, f32);
    }
    return moe_out;
}
