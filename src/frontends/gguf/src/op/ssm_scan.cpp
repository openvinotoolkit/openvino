...
OutputVector translate_ssm_scan(const NodeContext& context) {
    ...
    const auto ssm = std::make_shared<internal::SelectiveSSM>(a,
                                                              ...
                                                              context.get_input(5),
                                                              state);
    // ggml exposes one flat tensor containing all token outputs followed by the final states.
    const auto y = std::make_shared<v1::Reshape>(ssm->output(0), flat, false);
    const auto s = std::make_shared<v1::Reshape>(ssm->output(1), flat, false);
    const auto packed = std::make_shared<v0::Concat>(OutputVector{y, s}, 0);
    const auto result =
        std::make_shared<v1::Reshape>(packed, v0::Constant::create(element::i64, {4}, {1, 1, 1, -1}), false);
