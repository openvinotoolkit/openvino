...
OutputVector translate_soft_max(const NodeContext& context) {
    ...
    if (max_bias > 0.0f) {
        ...
        // ggml's head dimension is ne[2], which is axis 1 in the reversed four-dimensional OV
        // layout. Axis 0 is ne[3] (batch/repetition) and must not affect the slope sequence.
        FRONT_END_OP_CONVERSION_CHECK(in_ps.rank().is_static() && in_ps.rank().get_length() == 4 &&
                                          in_ps[1].is_static(),
                                      "SOFT_MAX ALiBi requires a static head-count dimension");
        const uint32_t n_head = static_cast<uint32_t>(in_ps[1].get_length());
