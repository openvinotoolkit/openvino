...
bool is_gpu_supported_fc_type(const ov::element::Type& type) {
    return type == ov::element::f32 || type == ov::element::f16 || type == ov::element::bf16 ||
           type == ov::element::gguf_q4_0 || type == ov::element::gguf_q4_1 || type == ov::element::gguf_q4_k ||
           type == ov::element::gguf_q5_k || type == ov::element::gguf_q6_k || type == ov::element::gguf_q8_0 ||
           type == ov::element::gguf_q3_k || type == ov::element::gguf_iq2_xs || type == ov::element::gguf_iq2_s ||
           type == ov::element::gguf_iq3_xxs || type == ov::element::gguf_iq3_s;
}

// GGUF block types the GPU gather_gguf kernel can decode row-selectively (must stay in sync with
// gather_gguf_supported_type() in impls/ocl_v2/gguf/gather_gguf.hpp). For any other embedding type
// we fall back to dense f16 materialization.
bool is_gpu_supported_embedding_type(const ov::element::Type& type) {
    return type == ov::element::gguf_q4_0 || type == ov::element::gguf_q4_1 || type == ov::element::gguf_q8_0 ||
           type == ov::element::gguf_q4_k || type == ov::element::gguf_q5_k || type == ov::element::gguf_q6_k;
}
...
Output<Node> make_rms_norm(const GGUFReader& reader,
    ...
    std::shared_ptr<ov::Node> mul = std::make_shared<v1::Multiply>(reciprocal, input, ov::op::AutoBroadcastType::NUMPY);
...
std::shared_ptr<v1::Transpose> split_heads(const Output<Node>& x,
    ...
    auto shape =
        std::make_shared<v0::Constant>(ov::element::i64, ov::Shape{4}, std::vector<int64_t>{0, 0, num_h, head_dim});
...
Output<Node> rotate_half(const Output<Node>& x, int64_t head_size, const Output<Node>& axis) {
    auto start_2nd =
        std::make_shared<v0::Constant>(ov::element::i64, ov::Shape{1}, std::vector<int64_t>{head_size / 2});
    auto stop_max =
        std::make_shared<v0::Constant>(ov::element::i64, ov::Shape{1}, std::vector<int64_t>{INT64_MAX_SLICE});
...
std::tuple<Output<Node>, Output<Node>, std::pair<Output<Node>, Output<Node>>> apply_rotary_pos_emb(
    ...
    auto q_rot =
        std::make_shared<v1::Add>(std::make_shared<v1::Multiply>(q, cos_u),
                                  std::make_shared<v1::Multiply>(rotate_half(q, head_size, hidden_dim), sin_u));
    auto k_rot =
        std::make_shared<v1::Add>(std::make_shared<v1::Multiply>(k, cos_u),
...
std::tuple<Output<Node>, ov::SinkVector, std::pair<Output<Node>, Output<Node>>, Output<Node>> multi_head_attention(
    ...
    auto create_cache = [&](const std::string& name, const Output<Node>& init_value) {
        ...
        auto gathered = std::make_shared<v8::Gather>(read_value,
                                                     beam_idx,
                                                     std::make_shared<v0::Constant>(ov::element::i64, ov::Shape{}, 0),
                                                     0);
    ...
    if (num_heads != num_heads_kv) {
        ...
        auto k_unsq = std::make_shared<v0::Unsqueeze>(k_combined,
                                                      std::make_shared<v0::Constant>(ov::element::i64, ov::Shape{}, 2));
        auto v_unsq = std::make_shared<v0::Unsqueeze>(v_combined,
                                                      std::make_shared<v0::Constant>(ov::element::i64, ov::Shape{}, 2));
        auto gqa_shape = [&]() {
            return std::make_shared<v0::Concat>(
                OutputVector{batch_dim,
    ...
    auto attention = std::make_shared<v13::ScaledDotProductAttention>(q_rot, k_reshaped, v_reshaped, final_mask, false);
    auto transpose_order =
        std::make_shared<v0::Constant>(ov::element::i32, ov::Shape{4}, std::vector<int32_t>{0, 2, 1, 3});
...
build_layer(const GGUFReader& reader,
    ...
    if (!output_shape) {
        ...
        auto gathered = std::make_shared<v8::Gather>(shape_of,
                                                     indices,
                                                     std::make_shared<v0::Constant>(ov::element::i64, ov::Shape{}, 0));
...
std::shared_ptr<ov::Model> build_language_model(const GGUFReader& reader, const Qwen3Config& cfg) {
    ...
    // Embedding: keep token_embd.weight in its native GGUF block format. When the GPU can decode it
    // row-selectively (gather_gguf), emit an opaque external gguf_* Constant (mmap-backed, so it is
    // NOT copied into the model .bin) and a GatherCompressed that decodes only the gathered rows on
    // device. Otherwise fall back to dense f16 materialization (the old path, which produces a large
    // .bin). Either way convert to the f32 activation precision used by the rest of the graph.
    const GGUFTensorInfo* embed_info = reader.find_tensor("token_embd.weight");
    OPENVINO_ASSERT(embed_info, "[GGUF Frontend] Missing 'token_embd.weight' tensor.");
    ...
    Output<Node> gathered;
    if (is_gpu_supported_embedding_type(embed_info->type)) {
        auto embed_w = reader.tensor_constant("token_embd.weight");  // opaque gguf_* external constant
        // Dummy f16 scale: GatherCompressed's output precision follows its scale input; gather_gguf
        // decodes the real scales from the block bytes and ignores this value.
        auto scale = std::make_shared<v0::Constant>(ov::element::f16, ov::Shape{1}, 1.0f);
        gathered = std::make_shared<GatherCompressed>(embed_w, ids_i32, gather_axis, 0, scale);
    } else {
        auto embed_f16 = dequantize_to_f16(reader, "token_embd.weight");
        gathered = std::make_shared<v8::Gather>(embed_f16, ids_i32, gather_axis);
    }
    ...
    // The final RMS norm + LM Head consume the full [batch, sequence, hidden] hidden
    // states. Selecting only the last sequence position for the LM Head is handled by
    // OpenVINO.GenAI's apply_slice_before_matmul_transformation (which recognizes the
    // FullyConnectedCompressed output node), matching the NNCF/optimum path. Do NOT
    // pre-slice here: the frontend graph is transformed to PagedAttention at compile
    // time, so a frontend-level slice runs at the wrong stage and corrupts the first
    // dynamic inference.
