...
GGUFReader::GGUFReader(const std::string& path, bool mmap_enable) : m_mmap_enable(mmap_enable) {
    m_source_basename = std::filesystem::path(path).filename().string();
    if (m_mmap_enable) {
        ...
        OPENVINO_ASSERT(stream.read(m_buffer->get_ptr<char>(), size), "[GGUF Frontend] Failed to read '", path, "'.");
...
void GGUFReader::parse() {
    ...
    OPENVINO_ASSERT(magic == kGGUFMagic, "[GGUF Frontend] Not a GGUF file (bad magic 0x", std::hex, magic, ").");
    ...
    for (uint64_t i = 0; i < tensor_count; ++i) {
        ...
        OPENVINO_ASSERT(n_dims <= 4, "[GGUF Frontend] Tensor '", info.name, "' has unsupported rank ", n_dims, ".");
...
std::shared_ptr<ov::op::v0::Constant> GGUFReader::make_constant(const ov::element::Type& type,
    ...
    if (m_mmap_enable) {
        auto shared = std::make_shared<ov::SharedBuffer<std::shared_ptr<ov::MappedMemory>>>(
            ...
            byte_size,
            m_mmap);
        constant = std::make_shared<ov::op::v0::Constant>(type, shape, shared);
    } else {
    ...
    // Tag the constant with an external-weight reference into the original GGUF file so that the
    // IR serializer can emit a `<data source=... offset=... size=...>` reference instead of copying
    // the raw block bytes into the model .bin, and the IR deserializer can mmap them back from the
    // sibling GGUF. The offset is absolute (relative to the GGUF file start).
    if (m_base != nullptr && ptr >= m_base) {
        auto& rt = constant->get_rt_info();
        rt["gguf_ext_source"] = m_source_basename;
        rt["gguf_ext_offset"] = static_cast<uint64_t>(ptr - m_base);
        rt["gguf_ext_size"] = static_cast<uint64_t>(byte_size);
    }
    return constant;
}
