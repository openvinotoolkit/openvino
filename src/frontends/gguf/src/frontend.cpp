...
void populate_rt_info(const std::shared_ptr<ov::Model>& model, const GGUFReader& reader) {
    ...
    // --- qwen35 / qwen35moe (hybrid attention: linear Gated-DeltaNet SSM + periodic full
    // attention) extensions. Only written for the hybrid architectures; dense qwen3 never carries these keys.
    if (arch == "qwen35moe" || arch == "qwen35") {
        // Hybrid-attention interval and per-head key/value length.
        if (reader.has(a(arch_suffix::full_attention_interval)))
            set(reader.get_u64(a(arch_suffix::full_attention_interval)), {"attention", "full_attention_interval"});
        if (reader.has(a(arch_suffix::attention_key_length)))
            set(reader.get_u64(a(arch_suffix::attention_key_length)), {"attention", "key_length"});
        if (reader.has(a(arch_suffix::attention_value_length)))
            set(reader.get_u64(a(arch_suffix::attention_value_length)), {"attention", "value_length"});

        // partial-mrope interleave sections (int32[4]).
        if (reader.has(a(arch_suffix::rope_dimension_sections)))
            set(reader.raw(a(arch_suffix::rope_dimension_sections)), {"rope", "sections"});

        // Gated-DeltaNet SSM hyper-parameters.
        if (reader.has(a(arch_suffix::ssm_conv_kernel)))
            set(reader.get_u64(a(arch_suffix::ssm_conv_kernel)), {"ssm", "conv_kernel"});
        if (reader.has(a(arch_suffix::ssm_state_size)))
            set(reader.get_u64(a(arch_suffix::ssm_state_size)), {"ssm", "state_size"});
        if (reader.has(a(arch_suffix::ssm_group_count)))
            set(reader.get_u64(a(arch_suffix::ssm_group_count)), {"ssm", "group_count"});
        if (reader.has(a(arch_suffix::ssm_time_step_rank)))
            set(reader.get_u64(a(arch_suffix::ssm_time_step_rank)), {"ssm", "time_step_rank"});
        if (reader.has(a(arch_suffix::ssm_inner_size)))
            set(reader.get_u64(a(arch_suffix::ssm_inner_size)), {"ssm", "inner_size"});
    }

    // --- qwen35moe-only: MoE expert block (dense qwen35 has no experts) ---
    if (arch == "qwen35moe") {
        set(reader.get_u64(a(arch_suffix::expert_count)), {"expert", "count"});
        set(reader.get_u64(a(arch_suffix::expert_used_count)), {"expert", "used_count"});
        set(reader.get_u64(a(arch_suffix::expert_feed_forward_length)), {"expert", "feed_forward_length"});
        // GGUF carries no explicit shared-expert count. The qwen35moe graph and its
        // ffn_*_shexp tensors use one shared expert per layer.
        set(static_cast<uint64_t>(1), {"expert", "shared_count"});
        if (reader.has(a(arch_suffix::expert_shared_feed_forward_length)))
            set(reader.get_u64(a(arch_suffix::expert_shared_feed_forward_length)),
                {"expert", "shared_feed_forward_length"});
        set(std::string("softmax"), {"expert", "routing_type"});
        set(true, {"expert", "norm_topk_prob"});
    }
    ...
    if (reader.has(file_keys::tokenizer_padding_id))
        set(static_cast<uint32_t>(reader.get_u64(file_keys::tokenizer_padding_id)),
