...
std::shared_ptr<ov::frontend::tensorflow_lite::DecoderFlatBuffer> ov::frontend::tensorflow_lite::GraphIteratorFlatBuffer::get_decoder() const {
    ...
    for (auto input : *m_nodes[node_index]->inputs()) {
        ...
        int64_t input_idx = !is_input ? -1 : std::find(inputs_vec->begin(), inputs_vec->end(), input) - inputs_vec->begin();
        auto is_output = outputs.find(input) != outputs.end();
        int64_t output_idx = !is_output ? -1 : std::find(outputs_vec->begin(), outputs_vec->end(), input) - outputs_vec->begin();
    ...
    for (auto output: *m_nodes[node_index]->outputs()) {
        ...
        int64_t output_idx = !is_output ? -1 : std::find(outputs_vec->begin(), outputs_vec->end(), output) -
                                               outputs_vec->begin();
    ...
    return std::make_shared<ov::frontend::tensorflow_lite::DecoderFlatBuffer>(m_nodes[node_index], type, std::to_string(node_index),
