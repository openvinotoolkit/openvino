...
OutputVector fully_connected(const ov::frontend::tensorflow_lite::NodeContext& node) {
    ...
    if (!decoder->get_attribute(&FCOptions::keep_num_dims)) {
        auto new_shape = make_shared<opset10::Concat>(OutputVector{
                opset10::Constant::create(element::i64, {1}, {-1}),
                ov::op::util::node_to_get_shape_value_of_indices_from_shape_source(weights, {1})}, 0);
