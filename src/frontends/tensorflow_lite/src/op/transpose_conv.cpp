...
OutputVector transpose_conv(const ov::frontend::tensorflow_lite::NodeContext& node) {
    ...
    const std::map<std::string, ov::Any> attrs{
            {"strides", std::vector<int64_t>{1, decoder->get_attribute(&Opts::stride_h), decoder->get_attribute(&Opts::stride_w), 1}},
    ...
    auto outputs = attribute_helper(node, attrs, translate_conv_2d_backprop_input_op, "Conv2DBackpropInput", true, inputs);
