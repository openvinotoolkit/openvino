...
std::shared_ptr<ov::frontend::tensorflow_lite::DecoderMap> get_pool_decoder_map(
    ...
    const std::map<std::string, ov::Any> attrs{
        {"strides",
         std::vector<int64_t>{1, decoder->get_attribute(&tflite::Pool2DOptions::stride_h), decoder->get_attribute(&tflite::Pool2DOptions::stride_w), 1}},
        {"padding", std::string(EnumNamePadding(decoder->get_attribute(&tflite::Pool2DOptions::padding)))},
        {"ksize",
         std::vector<int64_t>{1, decoder->get_attribute(&tflite::Pool2DOptions::filter_height),
                              decoder->get_attribute(&tflite::Pool2DOptions::filter_width), 1}},
        {"data_format", "NHWC"},
        {"activation", EnumNameActivationFunctionType(decoder->get_attribute(&tflite::Pool2DOptions::fused_activation_function))},
    };
...
OutputVector attribute_helper(const ov::frontend::tensorflow::NodeContext& node,
    ...
    auto decoder = std::make_shared<ov::frontend::tensorflow_lite::DecoderMap>(
            original_decoder, attrs, (new_op_type.empty() ? original_decoder->get_op_type() : new_op_type), empty_name);
