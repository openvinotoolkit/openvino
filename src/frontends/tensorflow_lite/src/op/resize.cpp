...
OutputVector resize_bilinear(const ov::frontend::tensorflow::NodeContext& node) {
    ...
    const std::map<std::string, ov::Any> attrs{
        ...
        {"half_pixel_centers", static_cast<bool>(decoder->get_attribute(&tflite::ResizeBilinearOptions::half_pixel_centers))},
    };
...
OutputVector resize_nearest_neightbor(const ov::frontend::tensorflow::NodeContext& node) {
    ...
    FRONT_END_GENERAL_CHECK(decoder != nullptr,
                            "Unexpected decoder during operation translation. Expected DecoderFlatBuffer");
    const std::map<std::string, ov::Any> attrs{
        {"align_corners", static_cast<bool>(decoder->get_attribute(&tflite::ResizeNearestNeighborOptions::align_corners))},
