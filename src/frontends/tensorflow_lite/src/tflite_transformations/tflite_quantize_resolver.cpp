...
pass::TFLQuantizeConvert::TFLQuantizeConvert() {
    ...
    auto m = std::make_shared<pattern::Matcher>(convert_label, "ov::frontend::tensorflow_lite::pass::TFLQuantizeResolver");
...
void fuse_zp_to_weights(ov::Output<ov::Node>& output, std::vector<int64_t>& zero_point, const ov::Shape& zp_shape) {
    if (std::all_of(zero_point.begin(), zero_point.end(), [](const int64_t& i){ return i == 0; }))
        return;
...
pass::TFLQuantizeReplacer::TFLQuantizeReplacer() {
    ...
    matcher_pass_callback callback = [=](Matcher& m) {
        ...
        FRONT_END_GENERAL_CHECK(quantization != nullptr, "Internal operation TFLQuantized representing quantized tensor doesn't have quantization details");
        ...
        output_low = std::make_shared<opset10::Multiply>(
                std::make_shared<opset10::Negative>(scale_node), zp_node);
        output_high = std::make_shared<opset10::Multiply>(
                scale_node, std::make_shared<opset10::Subtract>(
                        ov::opset10::Constant::create(element::f32, {}, {levels - 1}), zp_node));
        if (in_type != element::f32) {
    ...
    auto m = std::make_shared<pattern::Matcher>(tfl_quantize_label, "ov::frontend::tensorflow_lite::pass::TFLQuantizeReplacer");
    register_matcher(m, callback);
}
