...
def main():
    ...
    with tf.compat.v1.Session() as sess:
        ...
        resource_gather1 = tf.raw_ops.Gather(
            params=table1, indices=cast1, name="embedding_lookup1"
        )
        resource_gather2 = tf.raw_ops.Gather(
            params=table2, indices=cast2, name="embedding_lookup2"
        )
    ...
    tf.io.write_graph(
        tf_net,
        os.path.join(sys.argv[1], "resource_gather_model"),
        "resource_gather_model.pbtxt",
        True,
    )

    with open(
        os.path.join(
            sys.argv[1], "resource_gather_model", "resource_gather_model.pbtxt"
        ),
        mode="r",
    ) as file:
