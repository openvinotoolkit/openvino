...
def main():
    ...
    tf.io.write_graph(
        tf_net,
        os.path.join(sys.argv[1], "relu_unsupported"),
        "relu_unsupported.pb",
        False,
    )

    with open(
        os.path.join(sys.argv[1], "relu_unsupported", "relu_unsupported.pb"), mode="rb"
    ) as file:
    ...
    with open(
        os.path.join(sys.argv[1], "relu_unsupported", "relu_unsupported.pb"), mode="wb"
    ) as file:
