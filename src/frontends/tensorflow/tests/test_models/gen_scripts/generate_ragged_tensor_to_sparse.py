...
def main():
    ...
    def second_func(strings, row_splits):
        sparse_indices, sparse_values, sparse_dense_shape = (
            tf.raw_ops.RaggedTensorToSparse(
                rt_nested_splits=[row_splits], rt_dense_values=strings
            )
        )
    ...
    def first_func(strings, row_splits):
        ...
        concat = tf.raw_ops.Concat(
            concat_dim=0, values=[sparse_indices, sparse_dense_shape]
        )
        return concat

    tf_net = first_func.get_concrete_function(
        tf.constant(["abc", "bcd", "cc"], dtype=tf.string),
        tf.constant([0, 2, 2, 3, 3], dtype=tf.int32),
    ).graph.as_graph_def()
    tf.io.write_graph(
        tf_net,
        os.path.join(sys.argv[1], "ragged_tensor_to_sparse"),
        "ragged_tensor_to_sparse.pb",
        False,
    )
