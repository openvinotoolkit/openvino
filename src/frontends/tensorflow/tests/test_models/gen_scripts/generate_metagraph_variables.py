...
with tf.compat.v1.Session() as sess:
    ...
    z_value = [[2.0, 2.0, 1.0], [1.0, 1.0, 2.0]]
    ...
    restorev2 = tf.raw_ops.RestoreV2(
        prefix=input_name,
        tensor_names=["Variable"],
        shape_and_slices=[""],
        dtypes=[tf.float32],
        name="save/RestoreV2/wPack",
    )
    ...
    assign_var = tf.compat.v1.raw_ops.AssignVariableOp(
        resource=var_handle, value=restorev2[0]
    )
