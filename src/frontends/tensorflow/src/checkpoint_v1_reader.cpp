...
CheckpointV1Reader::CheckpointV1Reader(const std::string& checkpoints) {
    // figure out if the input is a file or a directory of checkpoints
    bool is_directory = false;
    struct stat s;
    if (stat(checkpoints.data(), &s) == 0) {
    ...
    if (is_directory) {
        // the checkpoint directory can contain multiple files with v1 checkpoints
        // read all v1 checkpoints
        checkpoints_paths = list_files_in_dir(checkpoints);
    } else {
    ...
    for (auto checkpoint_path : checkpoints_paths) {
        ...
        FRONT_END_GENERAL_CHECK(shard_stream && shard_stream->is_open(),
                                "[TensorFlow Frontend] incorrect model: checkpoint file does not exist");
        m_shards.push_back(shard_stream);
        std::string value;
        find_entry(shard_stream, saved_tensor_slices_key, value);
...
void CheckpointV1Reader::seek_block(const std::string& target_key,
    ...
    while (true) {
        FRONT_END_GENERAL_CHECK(
            ...
            "[TensorFlow Frontend] incorrect model: no more entries to return, invalid checkpoint file");
        ...
        FRONT_END_GENERAL_CHECK(curr_value_pos && key.size() >= shared,
                                "[TensorFlow Frontend] incorrect model: corruption error in checkpoint file");
...
void CheckpointV1Reader::init_block(const std::shared_ptr<std::ifstream>& shard,
    ...
    FRONT_END_GENERAL_CHECK(shard, "[TensorFlow Frontend] internal error: nullptr pointer to shard file");
    shard->seekg(0, shard->end);
    uint64_t shard_size = static_cast<uint64_t>(shard->tellg());
    FRONT_END_GENERAL_CHECK(
        offset < shard_size,
        "[TensorFlow Frontend] internal error or inconsistent checkpoint file: block offset is out-of-range");
    ...
    FRONT_END_GENERAL_CHECK(size >= sizeof(uint32_t),
                            "[TensorFlow Frontend] internal error: block size must be not less than 4 bytes");
    ...
    FRONT_END_GENERAL_CHECK(num_restarts <= max_restarts_allowed,
                            "[TensorFlow Frontend] internal error: num_restarts is greater than max_restarts_allowed");
...
void CheckpointV1Reader::find_entry(const std::shared_ptr<std::ifstream>& shard,
    ...
    init_block(shard, block_offset, block_size, block, restart_offset);

    // seek entry in the index block
    // this entry contains offset and size of the data block
    seek_block(entry_key, block.data(), static_cast<uint32_t>(restart_offset), entry_value);

    // initialize the data block
    FRONT_END_GENERAL_CHECK(get_varint64(entry_value, &block_offset) && get_varint64(entry_value, &block_size),
                            "[TensorFlow Frontend] incorrect input model: bad block handle");
    init_block(shard, block_offset, block_size, block, restart_offset);
...
void CheckpointV1Reader::read_variable(const std::string& variable_name, ov::Any& data) {
    ...
    std::string raw_data;
    find_entry(shard_ptr, encoded_name, raw_data);
