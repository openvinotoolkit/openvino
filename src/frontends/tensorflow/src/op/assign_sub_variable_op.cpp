...
namespace op {
OutputVector translate_sub_variable_op(const NodeContext& node) {
    default_op_checks(node, 2, {"AssignSubVariableOp"});
    auto ref = as_type_ptr<Variable>(node.get_input_by_reference(0).get_node_shared_ptr());
    TENSORFLOW_OP_VALIDATION(
        node,
        ref,
        "[TensorFlow Frontend] internal error: AssignSubVariableOp operation expects variable by the first input");
