...
std::shared_ptr<InPortPlaceTF> OpPlaceTF::get_input_port_tf(const std::string& inputName, int inputPortIndex) const {
    FRONT_END_GENERAL_CHECK(static_cast<size_t>(inputPortIndex) <= m_input_ports.at(inputName).size(),
...
ov::frontend::Place::Ptr OpPlaceTF::get_input_port(const std::string& name) const {
    FRONT_END_GENERAL_CHECK(m_input_ports.at(name).size() == 1, "Only one input port should exist.");
    return m_input_ports.at(name)[0];
}

ov::frontend::Place::Ptr OpPlaceTF::get_input_port(int outputPortIndex) const {
...
ov::frontend::Place::Ptr OpPlaceTF::get_output_port(int outputPortIndex) const {
...
ov::frontend::Place::Ptr OpPlaceTF::get_output_port() const {
...
std::vector<ov::frontend::Place::Ptr> OpPlaceTF::get_consuming_operations() const {
...
std::vector<ov::frontend::Place::Ptr> OpPlaceTF::get_consuming_operations(int outputPortIndex) const {
    return get_output_port(outputPortIndex)->get_consuming_operations();
}

std::vector<ov::frontend::Place::Ptr> OpPlaceTF::get_consuming_ports() const {
...
ov::frontend::Place::Ptr OpPlaceTF::get_input_port(const std::string& inputName, int inputPortIndex) const {
    FRONT_END_GENERAL_CHECK(inputPortIndex <= m_input_ports.at(inputName).size(), "inputPortIndex is out of bounds.");
    return m_input_ports.at(inputName)[inputPortIndex];
}

ov::frontend::Place::Ptr OpPlaceTF::get_source_tensor() const {
    return get_input_port()->get_source_tensor();
}

ov::frontend::Place::Ptr OpPlaceTF::get_source_tensor(const std::string& inputName) const {
    return get_input_port(inputName)->get_source_tensor();
}

ov::frontend::Place::Ptr OpPlaceTF::get_source_tensor(int inputPortIndex) const {
    return get_input_port(inputPortIndex)->get_source_tensor();
}

ov::frontend::Place::Ptr OpPlaceTF::get_source_tensor(const std::string& inputName, int inputPortIndex) const {
    return get_input_port(inputName, inputPortIndex)->get_source_tensor();
}

ov::frontend::Place::Ptr OpPlaceTF::get_target_tensor() const {
    return get_output_port()->get_target_tensor();
}

ov::frontend::Place::Ptr OpPlaceTF::get_producing_operation(const std::string& inputName) const {
...
ov::frontend::Place::Ptr OpPlaceTF::get_producing_operation() const {
    return get_input_port()->get_producing_operation();
}

ov::frontend::Place::Ptr OpPlaceTF::get_producing_operation(int inputPortIndex) const {
    return get_input_port(inputPortIndex)->get_producing_operation();
}

ov::frontend::Place::Ptr OpPlaceTF::get_target_tensor(int outputPortIndex) const {
...
std::vector<ov::frontend::Place::Ptr> TensorPlaceTF::get_consuming_ports() const {
    std::vector<ov::frontend::Place::Ptr> consuming_ports;
...
ov::frontend::Place::Ptr TensorPlaceTF::get_producing_port() const {
...
std::vector<ov::frontend::Place::Ptr> TensorPlaceTF::get_consuming_operations() const {
    std::vector<ov::frontend::Place::Ptr> consuming_ops;
    for (const auto& consuming_port : m_consuming_ports) {
...
ov::frontend::Place::Ptr TensorPlaceTF::get_producing_operation() const {
...
std::vector<ov::frontend::Place::Ptr> InPortPlaceTF::get_consuming_operations() const {
...
ov::frontend::Place::Ptr InPortPlaceTF::get_source_tensor() const {
    if (const auto& tensor = m_source_tensor.lock()) {
        return tensor;
    }
    FRONT_END_THROW("Source Tensor has expired.");
...
ov::frontend::Place::Ptr InPortPlaceTF::get_producing_port() const {
...
ov::frontend::Place::Ptr InPortPlaceTF::get_producing_operation() const {
    return get_producing_port()->get_producing_operation();
}
...
std::vector<ov::frontend::Place::Ptr> OutPortPlaceTF::get_consuming_operations() const {
...
std::vector<ov::frontend::Place::Ptr> OutPortPlaceTF::get_consuming_ports() const {
    if (auto tensor_ptr = m_target_tensor.lock()) {
...
ov::frontend::Place::Ptr OutPortPlaceTF::get_target_tensor() const {
    if (const auto& target_tensor = m_target_tensor.lock()) {
        return target_tensor;
    }
    FRONT_END_THROW("Target Tensor has expired.");
...
ov::frontend::Place::Ptr OutPortPlaceTF::get_producing_operation() const {
