...
with open(supported_ops_doc, "rt") as f:
    for line in f.readlines():
        ...
        if table_line > 2 and is_table_line:
            ...
            # remove NEW mark
            documented_op = documented_op.replace(
                '<sup><mark style="background-color: #00FF00">NEW</mark></sup>', ""
            )
            is_supported = False
...
if len(undocumented_supported_ops) > 0:
    raise Exception(
        "[TensorFlow Frontend] failed: Undocumented Supported Operations = ",
        undocumented_supported_ops,
    )

if len(incorrectly_documented_ops) > 0:
    raise Exception(
        "[TensorFlow Frontend] failed: Incorrectly Documented Operations = ",
        incorrectly_documented_ops,
    )
