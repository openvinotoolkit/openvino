...
class Variable : public InternalOperation {
public:
    using Ptr = std::shared_ptr<Variable>;
    OPENVINO_OP("Variable", "ov::frontend::tensorflow::util", InternalOperation);

    Variable(const std::string& name,
             const ov::Shape& shape,
             const ov::element::Type& type,
             const std::shared_ptr<DecoderBase>& decoder = std::make_shared<DecoderFake>())
        : InternalOperation(decoder, {}, 1, "Variable internal operation is remained"),
          m_name(name),
          m_shape(shape),
          m_type(type),
