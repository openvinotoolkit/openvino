...
OutputVector translate_matrix_diag_v3_op(const NodeContext& node) {
    ...
    OPENVINO_ASSERT(k_lower_const->cast_vector<int32_t>()[0] == 0 &&
                    k_upper_const->cast_vector<int32_t>()[0] == 0,
    ...
    auto output_rows_unsqueezed = make_shared<v0::Unsqueeze>(
        output_rows_i64,
        make_shared<v0::Constant>(element::i64, Shape{1}, 0)
    );
    auto padding_shape = make_shared<v0::Concat>(
        OutputVector{one_i64, diag_shape, output_rows_unsqueezed},
        0
    );
    ...
    auto batch_shape = make_shared<v1::StridedSlice>(
        unsqueeze_diag_shape,
        make_shared<v0::Constant>(element::i64, Shape{1}, 0),
    ...
    auto output_size = make_shared<v1::Multiply>(output_rows_unsqueezed,
                                                   make_shared<v0::Unsqueeze>(output_cols_i64,
                                                   make_shared<v0::Constant>(element::i64, Shape{1}, 0)));
    auto sliced = make_shared<v8::Slice>(
        reshaped,
        make_shared<v0::Constant>(element::i64, Shape{1}, 0),
        output_size,
        make_shared<v0::Constant>(element::i64, Shape{1}, 1),
    ...
    auto output_cols_unsqueezed = make_shared<v0::Unsqueeze>(
        output_cols_i64,
        make_shared<v0::Constant>(element::i64, Shape{1}, 0)
    );
    auto final_shape = make_shared<v0::Concat>(
        OutputVector{diag_shape, output_rows_unsqueezed, output_cols_unsqueezed},
        0
    );
