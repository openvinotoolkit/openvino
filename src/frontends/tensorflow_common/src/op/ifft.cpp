...
OutputVector translate_ifft_op(const NodeContext& node) {
    ...
    set_node_name(node.get_name(), ifft);

    // insert ComplexTypeMark since IFFT generates output of complex type
    complex_type_mark = make_shared<ComplexTypeMark>(ifft, complex_part_type);
