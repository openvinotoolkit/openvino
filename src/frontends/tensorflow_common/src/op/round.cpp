...
OutputVector translate_round_op(const NodeContext& node) {
  ...
  if (complex_type_mark) {
    ...
    auto gather_index_real =
        make_shared<v0::Constant>(element::i32, Shape{}, 0);
    auto gather_index_imag =
        make_shared<v0::Constant>(element::i32, Shape{}, 1);
    auto minus_one = make_shared<v0::Constant>(element::i32, Shape{1}, -1);
    auto real =
        make_shared<v8::Gather>(input, gather_index_real, minus_one)->output(0);
    auto imag =
        make_shared<v8::Gather>(input, gather_index_imag, minus_one)->output(0);
    ...
    auto complex_round =
        make_shared<ComplexTypeMark>(concat, complex_part_type);
