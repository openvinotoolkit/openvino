...
OutputVector translate_Atan2_op(const NodeContext& node) {
    ...
    auto is_x_negative = make_shared<Less>(x, const_zero);
    auto y_non_negative = make_shared<GreaterEqual>(y, const_zero);
    auto cond1 = make_shared<LogicalAnd>(is_x_negative, y_non_negative);
    auto atan_y_x_plus_pi = make_shared<Add>(atan, const_pi);
    auto result = make_shared<Select>(cond1, atan_y_x_plus_pi, result);

    // handle the third consition : x<0 && y<0
    auto is_y_negative = make_shared<Less>(y, const_zero);
    auto cond2 = make_shared<LogicalAnd>(is_x_negative, is_y_negative);
    auto atan_y_x_minus_pi = make_shared<Subtract>(atan, const_pi);
    auto result = make_shared<Select>(cond2, atan_y_x_minus_pi, result);

    // handle the fourth condition : x=0 && y>0
    auto is_x_zero = make_shared<Equal>(x, const_zero);
    auto is_y_positive = make_shared<Greater>(y, const_zero);
    auto cond3 = make_shared<LogicalAnd>(is_x_zero, is_y_positive);
