...
OutputVector translate_sparse_segment_mean(const NodeContext& node) {
    ...
    // segment_ids are always sorted, so the last index from segment_ids can be used to determine the number of output segments.
    ...
    auto embedding_segments_sum = make_shared<v3::EmbeddingSegmentsSum>(data, indices, segment_ids, std::make_shared<v0::Squeeze>(n_segments));

    // get the sizes of each segment
    auto unique_segment_ids = make_shared<v10::Unique>(segment_ids, true, indices.get_element_type(), indices.get_element_type());
    auto broadcast = make_shared<v3::Broadcast>(const_one, n_segments);
    auto divisors = make_shared<v3::ScatterUpdate>(broadcast, unique_segment_ids->output(0), unique_segment_ids->output(3), const_zero);
