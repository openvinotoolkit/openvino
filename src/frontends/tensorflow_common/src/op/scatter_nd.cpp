...
OutputVector translate_scatter_nd_op(const NodeContext& node) {
    ...
    auto complex_type_inputs = (complex_type_mark_input_indices || complex_type_mark_updates || complex_type_mark_shape) ? true : false;
    ...
    if(complex_type_inputs){
        auto complex_scatter_nd = make_shared<ComplexTypeMark>(scatter_nd,complex_type_mark_updates->get_complex_part_type());
