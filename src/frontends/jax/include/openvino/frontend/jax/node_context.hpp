...
public:
  NodeContext(std::shared_ptr<JaxDecoder> decoder,
      ...
      : frontend::NodeContext(decoder->get_op_type()), m_decoder(decoder),
    ...
    FRONT_END_GENERAL_CHECK(
        m_tensor_map != nullptr && m_external_parameters != nullptr &&
  ...
  template <typename T> T const_input(size_t index) const;

  size_t get_input_size() const override { return m_decoder_inputs.size(); };
  ...
  Output<Node> get_input(int index) const override {
    ...
    FRONT_END_GENERAL_CHECK(!m_decoder->input_is_none(index_),
                            "Input doesn't exist with index: ", index);
    auto input = m_decoder_inputs.at(index);
    FRONT_END_GENERAL_CHECK(m_tensor_map->count(input),
                            "No tensor corresponding input: ", input,
                            " exist.");
    return m_tensor_map->at(input);
  }

  Output<Node> get_input(const std::string &name) const override {
    FRONT_END_GENERAL_CHECK(has_attribute(name), "Input with name ", name,
                            " doesn't exist");
    ...
    FRONT_END_GENERAL_CHECK(
        false, "Input has type which can't be converted to ov::Node.");
  }
  ...
  OutputVector inputs() const {
    ...
    for (auto input : m_decoder_inputs) {
      FRONT_END_GENERAL_CHECK(m_tensor_map->count(input),
                              "No tensor corresponding index: ", input,
                              " exist.");
  ...
  size_t get_output_size() const { return m_decoder_outputs.size(); }

  std::vector<size_t> outputs() const { return m_decoder_outputs; }
  ...
  std::shared_ptr<JaxDecoder> get_decoder() const { return m_decoder; }

  TranslateSession *get_session() const { return m_translate_session; }
...
using CreatorFunction =
    std::function<ov::OutputVector(const ov::frontend::jax::NodeContext &)>;
