...
OutputVector translate_select_n(const NodeContext& context) {
    ...
    auto shape_of = std::make_shared<v3::ShapeOf>(context.get_input(1), element::i64);
    auto broadcast_which = std::make_shared<v1::Broadcast>(
        which,
        shape_of
    );
    Output<Node> result = std::make_shared<v6::GatherElements>(cases, broadcast_which, 0);
    return {result};
};
