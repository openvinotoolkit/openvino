...
OutputVector NodeContext::as_constant() const {
  ...
  if (dtype.is<type::Str>()) {
    ...
    auto fw_node =
        std::make_shared<JaxFrameworkNode>(m_decoder, OutputVector{});
    ...
    FRONT_END_OP_CONVERSION_CHECK(c_outs.size() == 1,
...
std::shared_ptr<Node>
NodeContext::mark_node(std::shared_ptr<Node> ov_node) const {
  ov_node = m_decoder->mark_node(ov_node);
  return ov_node;
}

void NodeContext::add_tensor_to_context(size_t index,
                                        Output<Node> ov_output) const {
  if (m_tensor_map->count(index)) {
    OPENVINO_DEBUG << "[ WARNING ] Current context has tensor " << index
                   << ". Assuming mutated output.\n";
  }
...
Output<Node>
NodeContext::get_tensor_from_model_or_create_input(size_t index) const {
  if (m_tensor_map->find(index) != m_tensor_map->end()) {
    return m_tensor_map->at(index);
  } else {
    // nested subgraphs case
    auto parameter = std::make_shared<v0::Parameter>(element::dynamic,
...
Output<Node> NodeContext::get_input_from_visible_context(size_t index) const {
  FRONT_END_GENERAL_CHECK(index < get_input_size(), "Index ", index,
                          " is lower then number of inputs.");
  ...
  if (std::dynamic_pointer_cast<v0::Parameter>(input_node)) {
    ...
    // this parameter
    size_t tensor_idx =
        m_translate_session->decode_tensor_name(input_node->output(0));
...
namespace {
std::shared_ptr<v0::Constant> get_constant_at_input(const NodeContext &ctx,
                                                    size_t index,
                                                    bool allow_empty = true) {
  FRONT_END_GENERAL_CHECK(!ctx.input_is_none(index),
                          "Input with index: ", index, " is none.");
  ...
  FRONT_END_GENERAL_CHECK(constant, "Input with index ", index,
                          " cannot be interpreted as Constant: ", input_val);
  return constant;
}
} // namespace
...
template <>
std::vector<int64_t>
NodeContext::const_input<std::vector<int64_t>>(size_t index) const {
...
template <> Strides NodeContext::const_input<Strides>(size_t index) const {
  auto c = get_constant_at_input(*this, index);
  if (c)
...
template <> Shape NodeContext::const_input<Shape>(size_t index) const {
  auto c = get_constant_at_input(*this, index);
  if (c)
...
template <> int32_t NodeContext::const_input<int32_t>(size_t index) const {
  return get_constant_at_input(*this, index, false)->cast_vector<int32_t>()[0];
}

template <> int64_t NodeContext::const_input<int64_t>(size_t index) const {
  return get_constant_at_input(*this, index, false)->cast_vector<int64_t>()[0];
}

template <> bool NodeContext::const_input<bool>(size_t index) const {
  return get_constant_at_input(*this, index, false)->cast_vector<bool>()[0];
}

template <> double NodeContext::const_input<double>(size_t index) const {
  return get_constant_at_input(*this, index, false)->cast_vector<double>()[0];
}

template <> float NodeContext::const_input<float>(size_t index) const {
...
Any NodeContext::get_values_from_const_input(int index) const {
  FRONT_END_GENERAL_CHECK(static_cast<size_t>(index) < get_input_size(),
                          "Input with index: ", index, " does not exist.");
  if (input_is_none(index))
    return {};
  auto input_val = get_input_from_visible_context(index);
  if (auto input = std::dynamic_pointer_cast<JaxFrameworkNode>(
          input_val.get_node_shared_ptr())) {
  ...
  if (constant) {
    ...
    default:
      FRONT_END_GENERAL_CHECK(false, "Input with index: ", index,
                              " has unsupported type.");
    }
  }
  FRONT_END_GENERAL_CHECK(false, "Input node with index ", index,
                          " cannot be interpreted as constant", input_val);
