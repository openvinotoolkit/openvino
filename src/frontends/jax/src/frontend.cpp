...
void FrontEnd::normalize(const std::shared_ptr<ov::Model>& model) const {
    ...
    if (model->get_parameters().size() > 0) {
        ...
        if (self->output(0).get_target_inputs().empty()) {
            ...
            OPENVINO_DEBUG << "[ WARNING ] Removing parameter[0] in converted jax model, because it is never used "
                              "and treated as `self`\n";
            model->remove_parameter(self);
        } else {
            OPENVINO_DEBUG << "[ WARNING ] Couldn't remove parameter[0] in converted jax model\n";
        }
    }
}
...
ov::frontend::InputModel::Ptr FrontEnd::load_impl(const std::vector<ov::Any>& variants) const {
    ...
    FRONT_END_GENERAL_CHECK(variants.size() == 1 + extra_variants_num,
                            "Jax Frontend supports exactly one parameter in model representation, got ",
                            std::to_string(variants.size()),
                            " instead.");
    FRONT_END_GENERAL_CHECK(variants[0].is<std::shared_ptr<IDecoder>>(),
                            "Jax Frontend doesn't support provided model type. Please provide supported model "
