...
get_unconverted_types_from_model(const std::shared_ptr<Model> &model) {
  ...
  for (const auto &node : model->get_ordered_ops()) {
    if (const auto &fw_node = ov::as_type_ptr<JaxFrameworkNode>(node)) {
      ...
      FRONT_END_GENERAL_CHECK(
          attrs.find(JaxFrameworkNode::op_type_key) != attrs.end(),
      ...
      if (!unconverted_ops_types.count(
              attrs.at(JaxFrameworkNode::op_type_key))) {
        unconverted_ops_types[attrs.at(JaxFrameworkNode::op_type_key)] =
            exception_msg;
      }
    }
    if (const auto &fw_node =
            ov::as_type_ptr<ov::op::util::MultiSubGraphOp>(node)) {
      for (size_t i = 0; i < fw_node->get_internal_subgraphs_size(); i++) {
        const auto &internal_types =
            get_unconverted_types_from_model(fw_node->get_function(i));
        unconverted_ops_types.insert(internal_types.begin(),
                                     internal_types.end());
      }
    }
  }
...
std::string pack_detailed_failure_report(
    const std::map<std::string, std::string> &unconverted_ops,
...
FrontEnd::convert(const ov::frontend::InputModel::Ptr &model) const {
  FRONT_END_GENERAL_CHECK(std::dynamic_pointer_cast<jax::InputModel>(model),
                          "Invalid input model");
  std::map<std::string, CreatorFunction> supported_ops =
      get_supported_ops(model);
  ...
  const auto &unconverted_ops =
      get_unconverted_types_from_model(converted_model);
  for (auto &&op : unconverted_ops) {
    if (m_telemetry) {
  ...
  bool is_conversion_successful =
      unconverted_ops.size() == 0 && norm_err.empty();
  FRONT_END_OP_CONVERSION_CHECK(
      is_conversion_successful,
      pack_detailed_failure_report(unconverted_ops, norm_err));
...
std::shared_ptr<Model>
FrontEnd::convert_partially(const ov::frontend::InputModel::Ptr &model) const {
  FRONT_END_GENERAL_CHECK(std::dynamic_pointer_cast<jax::InputModel>(model),
                          "Invalid input model");
  std::map<std::string, CreatorFunction> supported_ops =
      get_supported_ops(model);
...
void FrontEnd::add_extension(const std::shared_ptr<ov::Extension> &extension) {
  if (auto conv_ext =
          std::dynamic_pointer_cast<ov::frontend::ConversionExtension>(
              extension)) {
    m_conversion_extensions.push_back(conv_ext);
    m_op_extension_translators[conv_ext->get_op_type()] =
        [=](const NodeContext &context) {
          return conv_ext->get_converter()(context);
        };
  } else if (auto conv_ext = std::dynamic_pointer_cast<
                 ov::frontend::jax::ConversionExtension>(extension)) {
    m_conversion_extensions.push_back(conv_ext);
    m_op_extension_translators[conv_ext->get_op_type()] =
        [=](const NodeContext &context) {
          return conv_ext->get_converter()(context);
        };
  } else if (const auto &so_ext =
                 std::dynamic_pointer_cast<ov::detail::SOExtension>(
                     extension)) {
    add_extension(so_ext->extension());
    m_extensions.push_back(so_ext);
  } else if (const auto &telemetry =
                 std::dynamic_pointer_cast<TelemetryExtension>(extension)) {
    m_telemetry = telemetry;
  } else if (auto op_base_ext =
                 std::dynamic_pointer_cast<ov::BaseOpExtension>(extension)) {
...
std::map<std::string, CreatorFunction>
FrontEnd::get_supported_ops(const ov::frontend::InputModel::Ptr &model) const {
  std::map<std::string, CreatorFunction> supported_ops;
  supported_ops = get_supported_ops_jaxpr();
  for (auto i = m_op_extension_translators.begin();
       i != m_op_extension_translators.end(); i++)
