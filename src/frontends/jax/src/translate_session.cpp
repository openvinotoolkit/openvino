...
TranslateSession::TranslateSession(
    ...
    : m_input_model(input_model), m_translator_map(translator_map),
      m_telemetry(telemetry), m_ov_model(nullptr) {}

TranslateSession::~TranslateSession() {
  if (m_telemetry) {
    // Send statistics
    for (const auto &op : m_op_statistics) {
      m_telemetry->send_event("op_count", "jax_" + op.first,
                              static_cast<int>(op.second));
    }
  }
}
...
std::shared_ptr<ov::Model> TranslateSession::translate_graph(
    const ov::frontend::InputModel::Ptr &input_model) {
...
std::shared_ptr<Model> TranslateSession::convert_jax_model(
    std::shared_ptr<JaxDecoder> jax_model, const TensorMap &external_tensor_map,
    const std::shared_ptr<jax::InputModel> &input_model) {
  std::shared_ptr<Model>
      resulting_model; // define here to make a conversion in a nested scope
  {
    auto parameters = std::make_shared<ParameterVector>();
    auto tensor_map =
        std::make_shared<TensorMap>(); // tensor map of the current context
    ...
    if (input_model) {
      ...
      for (auto &input_p : input_model->m_inputs) {
        ...
        FRONT_END_GENERAL_CHECK(
            jax_place, "Only place produced by Jax Frontend is supported.");
      ...
      for (size_t i = 0; i < inputs.size(); ++i) {
        ...
        encode_tensor_name(parameter->output(0), inputs.at(i),
                           {jax_model->get_input_debug_name(i)});
        parameters->push_back(parameter);
    ...
    auto node_visitor = [&](std::shared_ptr<JaxDecoder> node) {
      ...
      auto context = NodeContext(node, external_tensor_map, tensor_map,
                                 parameters, mutated_tensors, this);
      ...
      FRONT_END_OP_CONVERSION_CHECK(
          fw_outputs.size() <= converted_outputs.size(), "Number of ",
          context.get_op_type(),
          " outputs greater than number of converted outputs, which are",
          fw_outputs.size(), " and ", converted_outputs.size(),
          " respectively.");
      ...
      for (size_t i = 0; i < fw_outputs.size(); ++i) {
        ...
        if (node->inputs().size() > 0) {
          ...
          if (m_may_be_alias.count(fw_tensor_id)) {
            ...
            FRONT_END_GENERAL_CHECK(
                recorded_in_tensor_id == in_tensor_id, "Operation ",
                context.get_op_type(),
                " creates alias to tensor which was already created before by ",
                recorded_node->get_op_type(),
                ", but from different tensor: ", in_tensor_id, " vs ",
                recorded_in_tensor_id);
          }
          m_may_be_alias[fw_tensor_id] = {node->inputs().at(0), node,
                                          converted_outputs[i]};
          OPENVINO_DEBUG << "Registered alias: " << fw_tensor_id
                         << " of tensor: " << node->inputs().at(0)
                         << " of operation: " << context.get_op_type();
        }
        FRONT_END_GENERAL_CHECK(
            tensor_map->find(fw_tensor_id) == tensor_map->end(),
            "Duplicated producer for Jax value with unique ID: ", fw_tensor_id);
        auto out_type = context.get_output_type(i);
        if (out_type.is<element::Type>()) {
          if (!converted_outputs[i].get_element_type().compatible(
                  out_type.as<element::Type>())) {
            OPENVINO_DEBUG << "[WARNING] Produced output type for operation "
                           << context.get_op_type()
                           << " for tensor id: " << fw_tensor_id
        ...
        encode_tensor_name(converted_outputs[i], fw_tensor_id,
                           {node->get_output_debug_name(i)});
      }
    };
    ...
    if (input_model) {
      ...
      for (auto &output_p : input_model->get_outputs()) {
        ...
        FRONT_END_GENERAL_CHECK(
            ov_output.get_names().size() > 0,
            "Tensor doesn't have name, while it should have name: ", tensor_id);
      ...
      for (size_t i = 0; i < jax_model->num_outputs(); ++i) {
        ...
        if (tensor_map->find(id) == tensor_map->end()) {
          ...
          // scope
          auto parameter = std::make_shared<v0::Parameter>(
              element::dynamic, PartialShape::dynamic());
        ...
        FRONT_END_GENERAL_CHECK(
            ov_output.get_names().size() > 0,
            "Tensor doesn't have name, while it should have name: ", id);
        auto result = std::make_shared<v0::Result>(ov_output);
        results.push_back(result);
      }
    }
    ...
    for (const auto &tensor_id : *mutated_tensors) {
      if (param_names.count(tensor_id)) {
        ...
        if (!external_tensor_map.empty()) {
          ...
          results.push_back(
              std::make_shared<v0::Result>(tensor_map->at(tensor_id)));
        }
      } else {
        OPENVINO_DEBUG << "Mutated tensor with id " << tensor_id
                       << " doesn't exist in inputs, skipping.";
      }
    }
    if (!external_tensor_map.empty()) {
...
OutputVector TranslateSession::convert_node(const NodeContext &context) {
  ...
  try {
    ...
    exception = e.what();
    if (m_telemetry) {
      auto cropped_message =
          ov::util::filter_lines_by_prefix(exception, get_jax_prefix());
  ...
  try {
    ...
    exception +=
        " Exception happened while creating FrameworkNode with subgraphs: " +
        std::string(e.what());
  } catch (...) {
...
void TranslateSession::encode_tensor_name(
    Output<Node> output, size_t tensor_idx,
    std::vector<std::string> additional_names) {
  if (!output.get_names().empty()) {
    OPENVINO_DEBUG << "Tensor names already exist: " << output.get_any_name()
                   << ". Will not be rewritten with " << tensor_idx
...
size_t TranslateSession::decode_tensor_name(const Output<Node> &output) {
  ...
  FRONT_END_GENERAL_CHECK(is_number(name),
                          "Tensor name is not a number: ", name);
