...
ov::op::PadType convert_pad(const std::string& pt_pad) {
    FRONT_END_OP_CONVERSION_CHECK(JAX_AUTO_PAD_TO_OV.count(pt_pad), "Unknown pad: ", pt_pad);
    return JAX_AUTO_PAD_TO_OV.at(pt_pad);
};
...
std::shared_ptr<ov::op::util::FrameworkNode> cast_fw_node(std::shared_ptr<Node> node, const std::string& type) {
    ...
    if (attrs.find(JaxFrameworkNode::op_type_key) == attrs.end() || attrs.at(JaxFrameworkNode::op_type_key) != type) {
        return nullptr;
    }
    return fw_node;
}
...
std::shared_ptr<ov::Node> make_list_construct(const ov::OutputVector& inputs) {
    ...
    attrs[JaxFrameworkNode::op_type_key] = "prim::ListConstruct";
...
std::deque<Output<Node>> get_list_as_outputs(const Output<Node>& start) {
    ...
    while (const auto& input_fw_node =
        ...
        if (attrs.find(JaxFrameworkNode::op_type_key) == attrs.end()) {
            break;
        }
        if (attrs.at(JaxFrameworkNode::op_type_key) == "aten::append") {
            res.push_front(input_fw_node->input(1).get_source_output());
        } else if (attrs.at(JaxFrameworkNode::op_type_key) == "aten::add") {
