...
def decompression_pattern(weights):
    ...
    return torch.stack(
        (torch.bitwise_and(weights, mask), torch.bitwise_right_shift(weights, 4)),
        dim=-1,
    )
...
def patched_forward(self, *args, **kwargs):
    ...
    unpacked_weights = (
        decompression_pattern(self._openvino_u4_compression_submodule_qweights())
        .contiguous()
        .view(height, -1, 8)
    )
    unpacked_weights = (
        torch.transpose(unpacked_weights, 1, 2)
        .contiguous()
        .view(-1, self.group_size, self.width)
    )
    unpacked_zp = (
        decompression_pattern(self._openvino_u4_compression_submodule_qzeros())
        .contiguous()
        .view(groups, 1, -1)
    )
...
def patch_model(model):
    for name, m in model.named_modules():
        ...
        if hasattr(m, "QUANT_TYPE"):
            ...
            m.add_module(
                "_openvino_u4_compression_submodule_qzeros", KeepWeight(m.qzeros)
            )
    ...
    for _, m in model.named_modules():
        if hasattr(m, "_openvino_patch_orig_forward"):
            try:
                ...
                print(
                    "[ WARNING ] Exception raised during GPTQ model unpatching. Depending on the exact issue it may lead to broken original model"
                )
                print(error)
...
def detect_gptq_model_raw(model):
    return (
        model
        and getattr(model, "config", None)
        and getattr(model.config, "quantization_config", None)
        and model.config.quantization_config.quant_method == "gptq"
    )


def detect_gptq_model(model):
    return (
        detect_gptq_model_raw(model)
        or getattr(model, "model", None)
        and detect_gptq_model_raw(model.model)
    )
