...
def _is_fastpath_eligible(sampling_metadata) -> bool:
    ...
    if procs is not None:
        ...
        def _is_noop(proc):
            if hasattr(proc, "min_p_count"):
                return proc.min_p_count == 0
            if hasattr(proc, "min_toks"):
                return not proc.min_toks
            if hasattr(proc, "biases"):
                return not proc.biases
            return False
...
def _build_native_sampler(vocab: int, top_k: int):
    ...
    tk = op.topk(scaled, k=op.constant(top_k, Type.i32), axis=-1, mode="max", sort="value")
    tv, ti = tk.output(0), tk.output(1)
    probs = op.softmax(tv, axis=-1)
    shape_of = op.shape_of(tv, Type.i64)
    uniform = op.random_uniform(shape_of,
    ...
    gumbel = op.multiply(op.log(op.add(op.multiply(op.log(op.add(uniform, eps)), neg), eps)), neg)
    score = op.add(op.log(op.add(probs, op.constant(1e-30, Type.f32))), gumbel)
    ...
    model = Model([token_i32.output(0)], [logits_p, temp_p], f"ov_native_sampler_v{vocab}_k{top_k}")
...
def _build_fused_sampler():
    ...
    def _impl(logits, temperature, top_k, top_p, exp_noise):
        ...
        max_k_c = torch.clamp(max_k, max=vocab_size)
...
def install():
    ...
    def _patched_sample(self, logits, sampling_metadata, logprobs_mode_override=None):
        ...
        if _use_native:
            ...
            if _use_native:
                cache_key = (vocab_size, k_val)
                compiled = _OV_NATIVE_COMPILED.get(cache_key)
                if compiled is None:
                    try:
                        compiled = _build_native_sampler(vocab_size, k_val)
                        _OV_NATIVE_COMPILED[cache_key] = compiled
                        logger.info(f"[OV plugin] Native sampler compiled (V={vocab_size}, k={k_val})")
                    except Exception as e:
            ...
            if _use_native:
                ...
                if temperature.dim() == 0:
                    temperature = temperature.unsqueeze(0).expand(batch_size)
        ...
        if sampling_metadata.top_p is not None:
            ...
            top_p = torch.full((batch_size,), 1.0, dtype=torch.float32)
