...
from torch.fx.experimental.proxy_tensor import DecompositionInterpreter, make_fx
...
class Partitioner:
        ...
        for partition in partitions:
            for pnode in partition.nodes:
                for pnode_input in pnode.all_input_nodes:
                    if (
                        pnode_input.op in ["get_attr"]
                        and pnode_input.op not in getattr_to_merge
                    ):
    ...
    def make_partitions(self, graph_module: GraphModule, options) -> GraphModule:
        ...
        partitioner = CapabilityBasedPartitioner(
            graph_module,
            self.supported_ops,
            allows_single_node_partition=allow_single_node_partition,
        )
