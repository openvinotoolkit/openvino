...
def openvino_execute(
    gm: GraphModule, *args, executor_parameters=None, partition_id, options
):
    ...
    if use_cache and (partition_id in compiled_cache):
        ...
        compiled = openvino_compile(
            gm, *args, model_hash_str=model_hash_str, options=options
        )
...
def openvino_execute_partitioned(
    gm: GraphModule, *args, executor_parameters=None, options=None
):
    executor_parameters = executor_parameters or DEFAULT_OPENVINO_PYTHON_CONFIG
