...
def maybe_register_pa_op(support_dict, options):
    """Mark the OV paged_attention custom op supported, so the partitioner
...
def maybe_rewrite_paged_attention(graph_module, options):
    ...
    HOP nodes into torch.ops.openvino.paged_attention.default, so OV can keep
    the call inside its partition.

    Returns the number of rewrites, or 0 on no-op / failure.
    """
    from openvino.frontend.pytorch.torchdynamo.vllm.preset import bool_opt
