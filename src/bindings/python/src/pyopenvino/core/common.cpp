...
py::array as_contiguous(py::array& array, ov::element::Type type) {
    ...
    default:
        OPENVINO_THROW_NORETURN("Tensor cannot be created as contiguous!");
    }
}
...
py::array array_from_tensor(ov::Tensor&& t) {
    ...
    default: {
        OPENVINO_THROW_NORETURN("Numpy array cannot be created from given OV Tensor!");
        break;
    }
    }
}
...
ov::op::v0::Constant create_shared(py::array& array) {
    ...
    OPENVINO_THROW_NORETURN("SHARED MEMORY MODE FOR THIS CONSTANT IS NOT APPLICABLE! Passed numpy array must be C contiguous.");
}
...
ov::Tensor create_shared(py::array& array) {
    ...
    // If passed array is not C-style, throw an error.
    OPENVINO_THROW_NORETURN("SHARED MEMORY MODE FOR THIS TENSOR IS NOT APPLICABLE! Passed numpy array must be C contiguous.");
}
...
uint32_t get_optimal_number_of_requests(const ov::CompiledModel& actual) {
    try {
        ...
        OPENVINO_THROW_NORETURN("Can't load network: ", ex.what(), " Please specify number of infer requests directly!");
    }
}
