...
inline ov::Any py_object_to_any(const pybind11::object& py_obj) {
    ...
    if (pybind11::isinstance<pybind11::str>(py_obj)) {
        ...
        switch (detected_type) {
            case PY_TYPE::STR:
                return _tuple.cast<std::vector<std::string>>();
            case PY_TYPE::FLOAT:
                return _tuple.cast<std::vector<float>>();
            case PY_TYPE::INT:
                return _tuple.cast<std::vector<int64_t>>();
            case PY_TYPE::BOOL:
                return _tuple.cast<std::vector<bool>>();
            default:
                OPENVINO_ASSERT(false, "Unsupported attribute type.");
        }
