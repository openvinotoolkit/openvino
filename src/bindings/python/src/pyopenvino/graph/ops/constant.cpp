...
template <typename T>
py::buffer_info _get_buffer_info(const ov::op::v1::Constant& c) {
...
template <>
py::buffer_info _get_buffer_info<ov::float16>(const ov::op::v1::Constant& c) {
    ov::Shape shape = c.get_shape();
    return py::buffer_info(const_cast<void*>(c.get_data_ptr()),               /* Pointer to buffer */
                           static_cast<ssize_t>(c.get_element_type().size()), /* Size of one scalar */
...
template <typename T>
py::array _cast_vector(const ov::op::v1::Constant& self) {
    auto vec = self.cast_vector<T>();
    return py::array(vec.size(), vec.data());
}

void regclass_graph_op_Constant(py::module m) {
    py::class_<ov::op::v1::Constant, std::shared_ptr<ov::op::v1::Constant>, ov::Node> constant(m,
                                                                                               "Constant",
                                                                                               py::buffer_protocol());
    constant.doc() = "openvino.runtime.op.Constant wraps ov::op::v1::Constant";
    ...
    constant.def("get_value_strings", &ov::op::v1::Constant::get_value_strings);

    constant.def("get_vector", [](const ov::op::v1::Constant& self) {
    ...
    constant.def_buffer([](const ov::op::v1::Constant& self) -> py::buffer_info {
        auto element_type = self.get_element_type();
        if (element_type == ov::element::boolean) {
