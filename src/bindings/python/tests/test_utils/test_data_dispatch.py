...
from openvino import Core, Model, PartialShape, Shape, Tensor, Type
...
def _run_dispatcher_single_input(
    device, input_data, is_shared, input_shape, input_dtype=np.float32
):
...
def _run_dispatcher_multi_input(
    device, input_data, is_shared, input_shape, input_dtype=np.float32
):
...
@pytest.mark.parametrize("input_shape", [[], [1]])
@pytest.mark.parametrize("is_shared", [True, False])
def test_scalars_dispatcher_new_0(
    device, input_data, input_dtype, input_shape, is_shared
):
    expected = Tensor(np.array(input_data, dtype=input_dtype))

    result, _ = _run_dispatcher_single_input(
        device, input_data, is_shared, input_shape, input_dtype
    )
...
def test_scalars_dispatcher_new_1(device, input_data, is_shared, expected, input_shape):
    result, _ = _run_dispatcher_single_input(
        device, input_data, is_shared, input_shape, np.float32
    )
...
def test_tensor_dispatcher(device, input_shape, is_shared):
    ...
    assert (
        np.array_equal(array, result.data)
        if is_shared
        else not np.array_equal(array, result.data)
    )
...
def test_ndarray_shared_dispatcher_casting(device, input_shape):
    ...
    result, infer_request = _run_dispatcher_single_input(
        device, test_data, True, input_shape
    )

    assert isinstance(result, Tensor)
    assert result.get_shape() == Shape(test_data.shape)
    assert (
        result.get_element_type() == infer_request.input_tensors[0].get_element_type()
    )
    assert np.array_equal(result.data, test_data)
...
def test_ndarray_copied_dispatcher(device, input_shape):
    ...
    result, infer_request = _run_dispatcher_single_input(
        device, test_data, False, input_shape
    )
...
def test_array_interface_copied_multi_dispatcher(device, input_shape, input_container):
    ...
    results, infer_request = _run_dispatcher_multi_input(
        device, test_inputs, False, input_shape
    )
    ...
    if is_shared:
        ...
        if test_data.dtype.kind == "U":
            assert np.array_equal(
                infer_request.input_tensors[0].bytes_data, np.char.encode(test_data)
            )
            ...
            assert np.array_equal(
                infer_request.input_tensors[0].str_data, np.char.decode(test_data)
            )
        assert not np.shares_memory(
            infer_request.input_tensors[0].bytes_data, test_data
        )
        ...
        if test_data.dtype.kind == "U":
            ...
            assert not np.array_equal(
                infer_request.input_tensors[0].str_data, test_data
            )
        else:
            assert not np.array_equal(
                infer_request.input_tensors[0].bytes_data, test_data
