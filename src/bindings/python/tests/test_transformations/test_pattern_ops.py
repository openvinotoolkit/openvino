...
def test_wrap_type_pattern_type():
    ...
    expect_exception(
        lambda: WrapType(f"opset{last_opset_number + 1}.Parameter"),
...
def test_wrap_type_ctors():
    ...
    matcher = Matcher(
        WrapType(
            ["opset13.Relu", "opset13.PRelu"], WrapType("opset13.Parameter").output(0)
        ),
        "FindActivation",
    )
    assert matcher.match(relu)
...
def test_or():
    ...
    matcher = Matcher(
        Or([WrapType("opset13.Relu"), WrapType("opset13.PRelu")]), "FindActivation"
    )
    assert matcher.match(relu)
    assert matcher.match(prelu)
...
def test_any_input():
    ...
    matcher = Matcher(
        WrapType("opset13.PRelu", [AnyInput(), AnyInput()]), "FindActivation"
    )
...
def test_any_input_predicate():
    ...
    matcher = Matcher(
        AnyInput(lambda output: len(output.get_shape()) == 4), "FindActivation"
    )
...
def test_optional_one_node():
    ...
    assert Matcher(Optional(["opset13.Parameter"]), "OneNodeTest").match(
        ops.parameter(PartialShape.dynamic())
    )
    assert not Matcher(Optional(["opset13.Relu"]), "OneNodeTest").match(
        ops.parameter(PartialShape.dynamic())
    )
...
def test_optional_predicate():
    ...
    assert Matcher(
        Optional(["opset13.Relu"], lambda x: True), "TestInputPredicate"
    ).match(model_relu)
    assert not Matcher(
        Optional(["opset13.Relu"], lambda x: False), "TestInputPredicate"
    ).match(model_relu)
    assert Matcher(
        Optional(["opset13.Add"], consumers_count(2)), "FindPredicate"
    ).match(model_add)
    assert not Matcher(
        Optional(["opset13.Add"], consumers_count(1)), "FindPredicate"
    ).match(model_add)
    assert Matcher(
        Optional(["opset13.Abs", "opset13.Result"], consumers_count(0)), "FindPredicate"
    ).match(model_abs)
...
def test_optional_with_input():
    ...
    assert Matcher(Optional(["opset13.Relu"], model_add.output(0)), "TestInput").match(
        model_relu
    )
    assert not Matcher(
        Optional(["opset13.Cos"], model_add.output(0)), "TestInput"
    ).match(model_relu)
...
def test_optional_with_input_and_predicate():
    ...
    assert Matcher(
        Optional(["opset13.Relu"], pattern_add.output(0), lambda x: True),
        "TestInputPredicate",
    ).match(model_relu)
    assert not Matcher(
        Optional(["opset13.Relu"], pattern_add.output(0), lambda x: False),
        "TestInputPredicate",
    ).match(model_relu)
...
def test_optional_with_input_node():
    ...
    assert Matcher(Optional(["opset13.Relu"], model_add), "TestInputNode").match(
        model_relu
    )
    assert not Matcher(Optional(["opset13.Cos"], model_add), "TestInputNode").match(
        model_relu
    )
...
def test_optional_with_input_node_and_predicate():
    ...
    assert not Matcher(
        Optional(["opset13.Cos"], model_add, lambda x: True), "TestInputNodePredicate"
    ).match(model_relu)
...
def test_optional_with_multi_input_node():
    ...
    assert Matcher(
        Optional(["opset13.Add"], [model_relu, model_input_1]), "MultiInNode"
    ).match(model_add)
    assert Matcher(
        Optional(["opset13.Add"], [model_relu, model_input_1]), "MultiInNode"
    ).match(model_relu)
    ...
    assert not Matcher(
        Optional(["opset13.Add"], [model_relu, model_input_1], lambda x: False),
        "MultiInNodePredicate",
    ).match(model_add)
    assert Matcher(
        Optional(["opset13.Add"], [model_relu, model_input_1], lambda x: True),
        "MultiInNodePredicate",
    ).match(model_add)
...
def test_all_predicates():
    ...
    assert Matcher(WrapType("opset13.Parameter", consumers_count(0)), "Test").match(
        static_param
    )
    assert not Matcher(WrapType("opset13.Parameter", consumers_count(1)), "Test").match(
        static_param
    )

    assert Matcher(WrapType("opset13.Parameter", has_static_dim(1)), "Test").match(
        static_param
    )
    assert not Matcher(WrapType("opset13.Parameter", has_static_dim(0)), "Test").match(
        dynamic_param
    )

    assert Matcher(
        WrapType("opset13.Parameter", has_static_dims([0, 3])), "Test"
    ).match(static_param)
    assert not Matcher(
        WrapType("opset13.Parameter", has_static_dims([0, 1])), "Test"
    ).match(dynamic_param)

    assert Matcher(WrapType("opset13.Parameter", has_static_shape()), "Test").match(
        static_param
    )
    assert not Matcher(WrapType("opset13.Parameter", has_static_shape()), "Test").match(
        dynamic_param
    )

    assert Matcher(WrapType("opset13.Parameter", has_static_rank()), "Test").match(
        dynamic_param
    )
    assert not Matcher(WrapType("opset13.Parameter", has_static_rank()), "Test").match(
        fully_dynamic_param
    )

    assert Matcher(
        WrapType("opset13.Parameter", type_matches(get_element_type(np.float32))),
        "Test",
    ).match(static_param)
    assert not Matcher(
        WrapType("opset13.Parameter", type_matches(get_element_type(np.float32))),
    ...
    assert Matcher(
        WrapType(
            "opset13.Parameter",  # noqa: ECE001
            type_matches_any(
                [get_element_type(np.float32), get_element_type(np.compat.long)]
            ),
        ),
        "Test",
    ).match(static_param)
