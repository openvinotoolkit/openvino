...
from openvino import Core, Model, PartialShape, save_model, serialize
...
def test_serialize_pass_v2(request, tmp_path, is_path_xml, is_path_bin):
    model, xml_path, bin_path = prepare_test_model_for_serialize(
        request, tmp_path, is_path_xml, is_path_bin
    )
...
def test_save_model(request, tmp_path, is_path_xml):
    model, xml_path, bin_path = prepare_test_model_for_serialize(
        request, tmp_path, is_path_xml, False
    )
...
def test_compress_model_transformation():
    node_constant = ov.opset8.constant(
...
def test_flush_fp32_subnormals_to_zero():
    ...
    weights = ov.opset10.constant(
        np.array(
            [0.0, 1.0, 2.0, 3.0, subnorm_val, subnorm_val, subnorm_val, subnorm_val]
        ),
        dtype=np.float32,
    )
    ...
    apply_moc_transformations(
        model, cf=False, smart_reshape=True
