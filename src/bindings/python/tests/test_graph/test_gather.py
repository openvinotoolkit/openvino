...
@pytest.mark.parametrize(
    ("input_shape", "indices", "axis", "expected_shape", "batch_dims"),
    [
        ((3, 3), (1, 2), [1], [3, 1, 2], []),
        ((3, 3), (1, 2), 1, [3, 1, 2], []),
        ((2, 5), (2, 3), [1], [2, 3], [1]),
...
@pytest.mark.parametrize(
    ("data_str", "input_shape", "indices", "axis", "expected_shape", "batch_dims"),
    [
...
def test_gather_string(
    data_str, input_shape, indices, axis, expected_shape, batch_dims, op_name
):
    ...
    node.evaluate(
        [out_tensor], [Tensor(input_data), Tensor(input_indices), Tensor(input_axis)]
    )
