...
def test_nms_rotated_default_attrs(
    ...
    scores_parameter = ov_opset13.parameter(
...
@pytest.mark.parametrize(
    "op_name",
    ["nms_rotated", "nmsRotated", "nmsRotatedOpset13", "nmsRotatedcustomAttrs"],
)
def test_nms_rotated_custom_attrs(
    boxes_shape,
    scores_shape,
    max_output_boxes,
    iou_threshold,
    ...
    assert (
        node.get_output_element_type(0) == Type.i32
        if output_type == "i32"
        else Type.i64
    )
    assert node.get_output_element_type(1) == Type.f32
    assert (
        node.get_output_element_type(2) == Type.i32
        if output_type == "i32"
        else Type.i64
    )
