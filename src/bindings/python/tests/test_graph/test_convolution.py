...
@pytest.mark.parametrize(
    ("strides", "pads_begin", "pads_end", "dilations", "expected_shape"),
    [
        (
            np.array([1, 1]),
            np.array([1, 1]),
...
def test_convolution_2d(
    strides, pads_begin, pads_end, dilations, expected_shape, op_name
):
    ...
    node = ov.convolution(
        input_x, input_filter, strides, pads_begin, pads_end, dilations, name=op_name
    )
...
def test_convolution_backprop_data(op_name):
    ...
    deconvolution = ov.convolution_backprop_data(
        data_node, filter_node, strides, output_shape_node, name=op_name
    )
