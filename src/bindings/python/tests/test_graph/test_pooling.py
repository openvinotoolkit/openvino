...
@pytest.mark.parametrize("op_name", ["avg_pool", "avgPool", "avgPoolOpset16"])
def test_avg_pool_opset16_with_dilations(op_params, input_shape, expected_output_shape, op_name):
    param = ov.parameter(input_shape, name="A", dtype=np.float32)
    node = ov16.avg_pool(param, *op_params, name=op_name)
    assert node.get_type_name() == "AvgPool"
    assert node.get_friendly_name() == op_name
    assert node.get_output_size() == 1
    assert list(node.get_output_shape(0)) == expected_output_shape
...
def test_avg_pool_opset16_without_dilations():
    param = ov.parameter([1, 1, 4, 4], name="A", dtype=np.float32)
    node = ov16.avg_pool(
        param,
        strides=[2, 2],
        pads_begin=[0, 0],
        ...
        exclude_pad=True,
        rounding_type="floor",
        name="avg_pool_no_dilations"
    )
    assert node.get_type_name() == "AvgPool"
    assert node.get_friendly_name() == "avg_pool_no_dilations"
    assert node.get_output_size() == 1
