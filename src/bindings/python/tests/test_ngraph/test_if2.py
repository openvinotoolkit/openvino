...
def simple_if(condition_val):
    ...
    then_body = Function([then_body_res_1], [X_t, Y_t], "then_body_function")
    then_body_inputs = [InvariantInputDescription(1, 0), InvariantInputDescription(2, 1)]
    then_body_outputs = [BodyOutputDescription(0, 0)]
    # else_body
    X_e = ov.parameter([2], np.float32, "X")
    ...
    else_body = Function([else_body_res_1], [X_e, Y_e], "else_body_function")
    else_body_inputs = [InvariantInputDescription(1, 0), InvariantInputDescription(2, 1)]
    ...
    #if_node.set_output_descriptions(0, then_body_outputs)
    #if_node.set_input_descriptions(1, else_body_inputs)
    ...
    #assert np.allclose(relu, np.array([6, 4], dtype=np.float32dtype))
    return relu
