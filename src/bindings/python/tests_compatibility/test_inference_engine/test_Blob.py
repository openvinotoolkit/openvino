...
def test_incompatible_array_and_td():
    ...
    assert (
        "Number of elements in provided numpy array 24 and "
...
def test_incompatible_input_precision():
    ...
    assert (
        "Data type float64 of provided numpy array "
...
@pytest.mark.skipif(
    os.environ.get("TEST_DEVICE", "CPU") != "CPU", reason="Device dependent test"
)
def test_buffer_values_after_add_outputs(device):
    path_to_repo = os.environ["MODELS_PATH"]
    test_net_xml_fp16 = os.path.join(
        path_to_repo, "models", "test_model", "test_model_fp16.xml"
    )
    test_net_bin_fp16 = os.path.join(
        path_to_repo, "models", "test_model", "test_model_fp16.bin"
    )
    ie_core = IECore()
    if device == "CPU":
    ...
    feed_dict = {"data": np.random.normal(0, 1, (1, 3, 32, 32)).astype(np.float32)}
...
def test_cannot_set_shape_preallocated_memory():
    ...
    with pytest.raises(RuntimeError) as e:
        blob.set_shape([1, 4, 128, 128])
    assert (
        "Cannot call setShape for Blobs created on top of preallocated memory"
        in str(e.value)
    )
