...
def test_set_batch_size(device):
    ...
    assert np.allclose(
        int(round(request.output_blobs["fc_out"].buffer[0][2])), 1
...
def test_resize_algorithm_work(device):
    ...
    request = exec_net_2.requests[0]
    assert (
        request.preprocess_info["data"].resize_algorithm == ie.ResizeAlgorithm.RESIZE_BILINEAR
    )
    request.set_blob("data", img_blob)
    request.infer()
    res_2 = np.sort(request.output_blobs["fc_out"].buffer)
...
@pytest.mark.parametrize(
    "mode", ["set_init_memory_state", "reset_memory_state", "normal"]
)
    ...
    for i in range(1, 10):
        if mode == "set_init_memory_state":
            ...
            init_array = np.full(
                input_shape,
                const_init,
                dtype=format_map[mem_state.state.tensor_desc.precision],
            )
            tensor_desc = TensorDesc(
                mem_state.state.tensor_desc.precision,
                input_shape,
                layout[len(input_shape) - 1],
            )
            blob = Blob(tensor_desc, init_array)
            mem_state.state = blob

            res = exec_net.infer(
                {"input_data": np.full(input_shape, 1, dtype=format_map[data_type])}
            )
            expected_res = np.full(
                input_shape, 1 + const_init, dtype=format_map[data_type]
            )
        ...
        assert np.allclose(
            res["MemoryAdd"], expected_res, atol=1e-6
...
def test_set_blob_with_incorrect_name():
    ...
    assert "Failed to find input or output with name: 'incorrect_name'" in str(e.value)
...
def test_set_blob_with_incorrect_size():
    ...
    assert "Input blob size is not equal network input size" in str(e.value)
    with pytest.raises(RuntimeError) as e:
        exec_net.requests[0].set_blob("out", blob)
    assert "Output blob size is not equal network output size" in str(e.value)
