...
def test_async_infer_many_req_get_idle(device):
    ...
    for id in range(2 * num_requests):
        ...
        request_id = exec_net.get_idle_request_id()
        assert request_id >= 0
        request_handler = exec_net.start_async(
            request_id=request_id, inputs={"data": img}
        )
...
def test_wrong_num_requests(device):
    with pytest.raises(ValueError) as e:
        ...
        assert (
            "Incorrect number of requests specified: -1. Expected positive integer number or zero for auto detection"
            in str(e.value)
        )
        del ie_core
...
@pytest.mark.skipif(
    os.environ.get("TEST_DEVICE", "CPU") != "MYRIAD",
...
@pytest.mark.skipif(
    os.environ.get("TEST_DEVICE", "CPU") != "GNA", reason="Device dependent test"
)
