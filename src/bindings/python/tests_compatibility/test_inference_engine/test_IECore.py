...
@pytest.mark.skipif(
    os.environ.get("TEST_DEVICE", "CPU") != "CPU", reason="Device independent test"
)
def test_load_network_wrong_device():
    ...
    assert 'Device with "BLA" name is not registered in the InferenceEngine' in str(
        e.value
    )
...
def test_query_network(device):
    ...
    assert [
        key for key in query_res.keys() if key not in ops_net_names
...
def test_register_plugin():
    ...
    if ie.get_metric("CPU", "FULL_DEVICE_NAME") == "arm_compute::NEON":
        pytest.skip(
            "Can't run on ARM plugin due-to openvino_intel_cpu_plugin specific test"
        )
...
def test_register_plugins():
    ...
    assert isinstance(exec_net, ExecutableNetwork), (
        "Cannot load the network to the registered plugin with name 'CUSTOM' "
        "registred in the XML file"
    )


@pytest.mark.skip(
    reason="Need to figure out if it's expected behaviour (fails with C++ API as well"
)
...
def test_available_devices(device):
    ...
    assert (
        device in devices
    ), f"Current device '{device}' is not listed in available devices '{', '.join(devices)}'"
...
def test_get_metric_list_of_str():
    ...
    assert all(isinstance(v, str) for v in param), (
        "Not all of the parameter values for 'OPTIMIZATION_CAPABILITIES' "
        "metric are strings!"
    )
...
def test_get_metric_tuple_of_two_ints():
    ...
    assert all(isinstance(v, int) for v in param), (
        "Not all of the parameter values for 'RANGE_FOR_STREAMS' "
        "metric are integers!"
    )
...
@pytest.mark.skipif(
    os.environ.get("TEST_DEVICE", "CPU") != "GPU", reason="Device dependent test"
)
