...
ov_status_e ov_core_compile_model_with_cache_crypto(const ov_core_t* core,
                                                    const ov_model_t* model,
                                                    const char* device_name,
                                                    const size_t property_args_size,
    ...
    try {
        ...
        if (device_name) {
            dev_name = device_name;
            object = core->object->compile_model(model->object, dev_name, property, encrypt_func, decrypt_func);
        } else {
            object = core->object->compile_model(model->object, property, encrypt_func, decrypt_func);
        }
        std::unique_ptr<ov_compiled_model_t> _compiled_model(new ov_compiled_model_t);
        _compiled_model->object = std::make_shared<ov::CompiledModel>(std::move(object));
        *compiled_model = _compiled_model.release();
...
ov_status_e ov_core_compile_model_from_file_with_cache_crypto(
    ...
    try {
        ...
        if (device_name) {
            dev_name = device_name;
            object = core->object->compile_model(model_path, dev_name, property, encrypt_func, decrypt_func);
        } else {
            object = core->object->compile_model(model_path, property, encrypt_func, decrypt_func);
        }
        std::unique_ptr<ov_compiled_model_t> _compiled_model(new ov_compiled_model_t);
        _compiled_model->object = std::make_shared<ov::CompiledModel>(std::move(object));
        *compiled_model = _compiled_model.release();
