...
//#include "../../inference/include/ie/gpu/gpu_ocl_wrapper.hpp"
#include <CL/cl2.hpp>
...
public:
    void get_model_tensor_info(ov_model_t* model, bool input, char** name, ov_shape_t* shape, ov_element_type_e* type) {
...
TEST_P(ov_remote_context_ocl, create_remote_tensor_nv12_from_cl_image2D) {
    ...
    int64_t dims_y[4] = {1, 1, width, height};
    ov_shape_t shape_uv = {0, nullptr};
    int64_t dims_uv[4] = {1, 1, width / 2, height / 2};
...
TEST_P(ov_remote_context_ocl, create_remote_tensor_inference) {
    ...
    get_model_tensor_info(model, true, &in_tensor_name, &shape, &tensor_type);
