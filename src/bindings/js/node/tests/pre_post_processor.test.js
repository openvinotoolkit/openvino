...
describe('PrePostProcess', () => {
  ...
  it('output() ', () => {
    assert.doesNotThrow(() => new ov.PrePostProcessor(model).output());
  });

  it('output(size_t output_index)', () => {
    assert.doesNotThrow(() => new ov.PrePostProcessor(model).output(0));
  });

  it('output(const std::string& tensor_name)', () => {
    assert.doesNotThrow(() => new ov.PrePostProcessor(model).output('fc_out'));
  });
...
describe('OutputInfo', () => {
    it('tensor()', () => {
      assert.doesNotThrow(() => new ov.PrePostProcessor(model).output(0).tensor());
    });
  
    it('tensor(param) throws', () => {
      assert.throws(() => new ov.PrePostProcessor(model).output(0).tensor(0),
        /Function does not take any parameters./);
    });
  
    it('tensor().setElementType()', () => {
      assert.doesNotThrow(() => new ov.PrePostProcessor(model).output(0).tensor().setElementType(ov.element.u8));
    });
  
    it('tensor().setElementType() throws', () => {
      assert.throws(() => new ov.PrePostProcessor(model).output(0).tensor().setElementType(),
        /Wrong number of parameters./);
    });
  
    it('tensor().setElementType() throws', () => {
      assert.throws(() => new ov.PrePostProcessor(model).output(0).tensor().setElementType('invalidType'),
        /Cannot create ov::element::Type/);
    });
  
    it('tensor().setLayout()', () => {
      assert.doesNotThrow(() => new ov.PrePostProcessor(model).output(0).tensor().setLayout('NHWC'));
    });
  
    it('tensor().setLayout() throws', () => {
      assert.throws(() => new ov.PrePostProcessor(model).output(0).tensor().setLayout(),
        /Wrong number of parameters./);
    });
