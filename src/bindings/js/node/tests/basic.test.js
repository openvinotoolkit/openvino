...
const modelLike = [ [ model ], [ compiledModel ] ];
...
describe('Core.compileModelSync()', () => {
  ...
  it('compileModelSync(model) throws if the number of arguments is invalid',
     () => {
       assert.throws(
           () => core.compileModelSync(model), /Invalid number of arguments/);
     });
});
...
describe('Core.compileModel()', () => {
  ...
  it('compileModel(model:Model, deviceName: string, config: {}) ', () => {
    core.compileModel(model, 'CPU', tput)
        .then(cm => { assert.deepStrictEqual(cm.output(0).shape, [ 1, 10 ]); });
  });

  it('compileModel(model:model_path, deviceName: string, config: {}) ', () => {
    core.compileModel(testXml, 'CPU', tput)
        .then(cm => { assert.equal(cm.inputs.length, 1); });
  });

  it('compileModel(model:model_path, deviceName: string) ', () => {
    core.compileModel(testXml, 'CPU')
        .then(cm => { assert.deepStrictEqual(cm.output(0).shape, [ 1, 10 ]); });
  });
  ...
  it('compileModel(model, device, config) throws when config value is not a string',
     () => {
       assert.throws(
           () => core.compileModel(model, 'CPU', {'PERFORMANCE_HINT' : tput})
                     .then(),
           /Cannot convert Napi::Value to ov::Any/);
     });
  ...
  it('compileModel(model) throws if the number of arguments is invalid', () => {
    assert.throws(
        () => core.compileModel(model).then(), /Invalid number of arguments/);
  });
});

describe('Output class', () => {
  it('Output type', () => { assert.ok(model.output() instanceof ov.Output); });
...
describe('Input class for ov::Input<const ov::Node>', () => {
  it('Output type', () => { assert.ok(model.input() instanceof ov.Output); });
  ...
  modelLike.forEach(([ obj ]) => {
    ...
    it('inputs property', () => { assert.strictEqual(obj.inputs.length, 1); });
...
describe('Test exportModel()/importModel()', () => {
  ...
  const epsilon = 0.5;
  const tensor =
      Float32Array.from({length : 3072}, () => (Math.random() + epsilon));
  ...
  it('Test importModel(stream, device, config)', () => {
    const newCompiled =
        core.importModelSync(userStream, 'CPU', {'NUM_STREAMS' : 1});
    const newInferRequest = newCompiled.createInferRequest();
    const res2 = newInferRequest.infer([ tensor ]);
