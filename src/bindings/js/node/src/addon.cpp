...
void init_class(Napi::Env env, Napi::Object exports, std::string class_name,
                Prototype func, Napi::FunctionReference &reference) {
...
Napi::Value save_model_sync(const Napi::CallbackInfo &info) {
  ...
  try {
    if (ov::js::validate<ModelWrap, Napi::String>(info, allowed_signatures)) {
    ...
    } else if (ov::js::validate<ModelWrap, Napi::String, Napi::Boolean>(
      ...
      OPENVINO_THROW("'saveModelSync'", ov::js::get_parameters_error_msg(
                                            info, allowed_signatures));
    }
...
Napi::Object init_module(Napi::Env env, Napi::Object exports) {
  ...
  init_class(env, exports, "CompiledModel", &CompiledModelWrap::get_class,
             addon_data->compiled_model);
  init_class(env, exports, "InferRequest", &InferRequestWrap::get_class,
             addon_data->infer_request);
  init_class(env, exports, "Tensor", &TensorWrap::get_class,
             addon_data->tensor);
  init_class(env, exports, "Output", &Output<ov::Node>::get_class,
             addon_data->output);
  init_class(env, exports, "ConstOutput", &Output<const ov::Node>::get_class,
             addon_data->const_output);
  init_class(env, exports, "PartialShape", &PartialShapeWrap::get_class,
             addon_data->partial_shape);
