...
NodeWrap::NodeWrap(const Napi::CallbackInfo& info, ov::Node* node) : Napi::ObjectWrap<NodeWrap>(info), node_(node) {
...
Napi::Function NodeWrap::get_class(Napi::Env env) {
    if (constructor.IsEmpty()) {
        throw Napi::Error::New(env, "NodeWrap class is not initialized. Call Init first.");
    }
