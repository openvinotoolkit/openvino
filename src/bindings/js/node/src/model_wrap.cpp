...
ModelWrap::ModelWrap(const Napi::CallbackInfo &info)
    : Napi::ObjectWrap<ModelWrap>(info), _model{}, _core{}, _compiled_model{} {}
...
Napi::Function ModelWrap::get_class(Napi::Env env) {
  return DefineClass(
      ...
      {InstanceMethod("getName", &ModelWrap::get_name),
       ...
       InstanceMethod("getOutputElementType",
                      &ModelWrap::get_output_element_type),
...
std::shared_ptr<ov::Model> ModelWrap::get_model() const { return _model; }
...
Napi::Value ModelWrap::get_output_element_type(const Napi::CallbackInfo &info) {
  ...
  try {
    if (ov::js::validate<int>(info, allowed_signatures)) {
      ...
      return cpp_to_js<ov::element::Type_t, Napi::String>(
          info, output.get_element_type());
    } else {
      OPENVINO_THROW("'getOutputElementType'", ov::js::get_parameters_error_msg(
                                                   info, allowed_signatures));
    }
  } catch (const std::exception &e) {
    reportError(info.Env(), e.what());
    return info.Env().Undefined();
...
Napi::Value ModelWrap::clone(const Napi::CallbackInfo &info) {
  ...
  try {
    if (ov::js::validate(info, allowed_signatures)) {
      ...
      OPENVINO_THROW("'clone'", ov::js::get_parameters_error_msg(
                                    info, allowed_signatures));
    }
  } catch (const std::exception &e) {
    reportError(info.Env(), e.what());
