...
//           << vec2str(desc.getStrides()) << " "
//           << vec2str(desc.getBlockDims()) << " "
//           << vec2str(desc.getOrder()) << " "
//           << vec2str(desc.getOffsetPaddingToData()) << " "
//           << desc.getOffsetPadding() << " "
//           << desc.getPaddedElementsCount() << " " << std::endl;



// TEST(MemDescWithZeroDimsPld, createDesc) {
//     auto testBody = [] (dnnl::memory::format_tag fmt, Shape shape) {
//         // DnnlBlockedMemoryDesc desc(shape, dnnl::memory::data_type::f32, fmt);
//         DnnlBlockedMemoryDesc desc(InferenceEngine::Precision::FP32, shape);
//         std::cout << desc.getShape().toString() << " " << vec2str(desc.getStrides()) << " " << vec2str(desc.getBlockDims()) << " " << vec2str(desc.getOrder())
//                   << " " << vec2str(desc.getOffsetPaddingToData()) << " " << desc.getOffsetPadding()
//                   << " " << desc.getPaddedElementsCount() << " " << std::endl;
//         return true;
//     };

//     std::pair<dnnl::memory::format_tag, Shape> testCases[] {
//         { dnnl::memory::format_tag::nchw, Shape(VectorDims{4, 0, 10, 0 }) },
//         // { dnnl::memory::format_tag::nhwc,        {4, 2, 10, 7 } },
//         // { dnnl::memory::format_tag::nChw8c,     {1, 1, 10, 10} },
//         // { dnnl::memory::format_tag::nChw16c,     {1, 1, 10, 10} },
//         // { dnnl::memory::format_tag::NChw16n16c,  {4, 2, 10, 7 } },
//         // { dnnl::memory::format_tag::BAcd16a16b,  {4, 2, 10, 7 } },
//         // { dnnl::memory::format_tag::Acdb16a,     {96, 1, 7, 7 } },
//     };

//     for (const auto &tc : testCases) {
//         ASSERT_TRUE(testBody(tc.first, tc.second));
//     }
