...
public:
    ...
    void updateOPsStats(const std::shared_ptr<ov::Model> &model, const PassRate::Statuses &status, size_t k = 1);
    void updateOPsImplStatus(const std::shared_ptr<ov::Model> &model, const bool implStatus);

    void updateOPsStats(const ov::NodeTypeInfo &op, const PassRate::Statuses &status, size_t k = 1);
