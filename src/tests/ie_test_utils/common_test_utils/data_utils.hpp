...
inline fill_roi_raw_ptr(dataType* data, size_t data_size, const uint32_t range, const int height, const int width, const float omega,
...
fill_data_roi(InferenceEngine::Blob::Ptr &blob, const uint32_t range, const int height, const int width, const float omega,
    ...
    fill_roi_raw_ptr<dataType>(data, blob->size(), range, height, width, omega, is_roi_max_mode, seed);
}
