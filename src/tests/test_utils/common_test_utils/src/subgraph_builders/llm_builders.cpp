...
std::shared_ptr<ov::Node> make_state_initializer(ov::Output<ov::Node> input,
    ...
    for (size_t i = 0; i < kv_cache_size.size(); i++) {
        if (i == static_cast<size_t>(qkv_order[0])) {
            ...
            dims[i] = std::make_shared<ov::op::v0::Constant>(ov::element::i32,
                                                             ov::Shape{1},
...
std::shared_ptr<ov::Node> make_attention_mask(ov::Output<ov::Node> q,
    ...
    auto horizontal_range =
        std::make_shared<ov::op::v4::Range>(zero_i, source_s_len, one_i, ov::element::i32)->output(0);
...
std::shared_ptr<ov::Model> make_llm_kv_cache_pattern(ov::Dimension batch,
    ...
    if (reorder_params) {
        ...
        auto updates = std::make_shared<ov::op::v8::Gather>(concat_input, in_src_idx, axis_const);
        concat_input =
            std::make_shared<ov::op::v3::ScatterElementsUpdate>(concat_input, in_dst_idx, updates, axis_const);
...
std::shared_ptr<ov::Model> make_llm_kv_cache_sdpa_pattern(ov::Dimension batch,
                                                          ov::Dimension n_heads,
                                                          ov::Dimension k_features,
                                                          ov::Dimension v_features,
                                                          ov::element::Type_t element_type,
    ...
    if (with_mask && with_scale) {
        scale =
            ov::op::v0::Constant::create(element_type, ov::Shape{}, {1.0f / std::sqrt(k_features.get_max_length())});
    }
