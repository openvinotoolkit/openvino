...
def path_to_model(model_path: os.path, prefix: str):
    ...
    for item in str(model).split(os.path.sep):
        ...
        if item in precisions:
            ...
            model_prc += item
    return Model(
        model_name=model_name, model_framework=model_framework, model_prc=model_prc
    )
    ...
    with open(accuracy_res_file, newline="") as csv_file:
        ...
        for row in csv_reader:
            if model_name_row_idx == None:
                ...
                continue
            model = Model(
                model_name=row[model_name_row_idx],
                model_framework=row[framework_row_idx],
                model_prc=row[precision_row_idx],
            )
            if target_device in row[device_row_idx]:
                if model in results.keys():
                    old_status = convert_accuracy_res_to_bool(results[model])
                    new_status = convert_accuracy_res_to_bool(
                        row[accuracy_status_row_idx]
                    )
                    if old_status != new_status and (
                        new_status == "passed"
                        or old_status == "not_found"
                        or old_status == ""
                    ):
                    ...
                    results.update(
                        {
                            model: convert_accuracy_res_to_bool(
                                row[accuracy_status_row_idx]
                            )
                        }
                    )
...
def process_conformance(failed_models_path: os.path):
    if not os.path.isfile(failed_models_path):
        raise Exception(
            f"Conformance failed model csv: {failed_models_path} is not file!"
        )
    model_paths = set()
    models = dict()
    ...
    with open("test_results.csv", "w", newline="") as csv_file:
        ...
        for model in model_set:
            ...
            for acc_model in accuracy_res.keys():
                if (
                    model.get_name() == acc_model.get_name()
                    and model.get_framework() == acc_model.get_framework()
                    and model.get_precision() == acc_model.get_precision()
                ):
                    accuracy_status = accuracy_res[acc_model]
                    break
            for conf_model in conformance_res.keys():
                if (
                    model.get_name() == conf_model.get_name()
                    and model.get_framework() == conf_model.get_framework()
                    and model.get_precision() == conf_model.get_precision()
                ):
