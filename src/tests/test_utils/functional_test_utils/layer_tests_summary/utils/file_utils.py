...
from . import conformance_utils, constants
...
def prepare_filelist(input_dir: os.path, patterns: list, is_save_to_file=True):
    ...
    if os.path.isfile(filelist_path):
        conformance_utils.UTILS_LOGGER.info(
            f"{filelist_path} is exists! The script will update it!"
        )
    model_list = list()
    ...
    if is_save_to_file:
        try:
            ...
            conformance_utils.UTILS_LOGGER.warning(
                f"Impossible to update {filelist_path}! Something going is wrong!"
            )
    else:
        return model_list
...
def unzip_archieve(zip_path: os.path, dst_path: os.path):
    ...
    if tarfile.is_tarfile(zip_path):
        ...
        exit(-1)
    conformance_utils.UTILS_LOGGER.info(
        f"Archieve {dst_path} was extacted to {dst_dir}"
    )
    os.remove(dst_path)
...
def find_latest_dir(in_dir: Path, pattern="*"):
    ...
    for entity in entities:
        if entity.is_dir():
            return entity
    conformance_utils.UTILS_LOGGER.error(
        f"{in_dir} does not contain applicable directories to pattern: {pattern}"
    )
    exit(-1)
...
def get_ov_path(script_dir_path: os.path, ov_dir=None, is_bin=False):
    if ov_dir is None or not os.path.isdir(ov_dir):
        ov_dir = os.path.abspath(script_dir_path)[
            : os.path.abspath(script_dir_path).find(constants.OPENVINO_NAME)
