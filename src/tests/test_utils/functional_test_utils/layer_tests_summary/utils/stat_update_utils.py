...
def update_rel_values(xml_node: ET.SubElement):
    ...
    if not "relative_all" in xml_node.attrib:
        test_cnt = (
            int(xml_node.attrib.get("passed"))
            + int(xml_node.attrib.get("failed"))
            + int(xml_node.attrib.get("skipped"))
            + int(xml_node.attrib.get("crashed"))
            + int(xml_node.attrib.get("hanged"))
        )
...
def update_passrates(results: ET.SubElement, rel_weights={}):
    for device in results:
        for op in device:
            ...
            passrate = (
                float(passed_tests * 100 / total_tests) if total_tests != 0 else 0
            )
            rel_all_tests = (
                rel_all_tests_actual
                if rel_all_tests_expected is None
                else rel_all_tests_expected
            )
            k = (
                1
                if rel_all_tests_expected is None
                else round(rel_all_tests_actual / rel_all_tests_expected)
            )
            rel_passrate = (
                float(rel_passed_tests * 100 / (k * rel_all_tests))
                if rel_all_tests != None and rel_all_tests != 0
                else 0
            )
    ...
    for device in results.find("results"):
        for op in device:
            ...
            for attr_name in op.attrib:
                if (
                    attr_name == "passrate"
                    or attr_name == "implemented"
                    or attr_name == "relative_passrate"
                ):
                    continue
    ...
    for device in results.find("results"):
        for op in device:
            if op.tag in incorrect_ops:
                ...
                for attr_name in op.attrib:
                    if (
                        "passrate" in attr_name
                        or attr_name == "implemented"
                        or "relative_" in attr_name
                    ):
                        continue
                    else:
                ...
                if test_cnt != max_test_cnt[op.tag]:
                    ...
                    conformance_utils.UTILS_LOGGER.warning(
                        f"{device.tag}: added {diff} skipped tests for {op.tag}"
                    )
