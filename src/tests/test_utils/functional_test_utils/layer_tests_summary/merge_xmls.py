...
from utils.constants import API_CONFORMANCE, OP_CONFORMANCE
...
def parse_arguments():
    ...
    parser.add_argument(
        "-i", "--input_folders", help=input_folders_help, nargs="*", required=True
    )
    parser.add_argument("-o", "--output_folder", help=output_folders_help, default=".")
    parser.add_argument(
        "-f", "--output_filename", help=output_filename_help, default="report"
    )
    parser.add_argument("-t", "--report_type", help=report_type_help, default="OP")
    parser.add_argument(
        "-m", "--merge_device_id", help=merge_device_id_help, default=False
    )
...
def update_result_node(xml_node: SubElement, aggregated_res: SubElement):
    ...
    for attr_name in xml_node.attrib:
        ...
        if attr_name == "implemented":
            ...
            continue
        xml_value = (
            float(xml_node.attrib.get(attr_name))
            if "relative_" in attr_name
            else int(xml_node.attrib.get(attr_name))
        )
        aggregated_value = (
            float(aggregated_res.attrib.get(attr_name))
            if "relative_" in attr_name
            else int(aggregated_res.attrib.get(attr_name))
        )
        aggregated_res.set(attr_name, str(xml_value + aggregated_value))


def aggregate_test_results(
    aggregated_results: SubElement,
    xml_reports: list,
    report_type: str,
    merge_device_suffix=False,
):
    ...
    for xml in xml_reports:
        ...
        for xml_device_entry in xml_results:
            if merge_device_suffix and "." in xml_device_entry.tag:
                ...
                new_data = (
                    ET.tostring(xml_device_entry)
                    .decode("utf8")
                    .replace(xml_device_entry.tag, device_name)
                )
            ...
            for xml_results_entry in xml_device_entry:
                if (
                    report_type == OP_CONFORMANCE
                    or report_type == OP_CONFORMANCE.lower()
                ):
                    aggregated_results_entry = None
                    if not aggregated_device_results is None:
                        aggregated_results_entry = aggregated_device_results.find(
                            xml_results_entry.tag
                        )
                    if aggregated_results_entry is None:
                        ...
                        if aggregated_device_results is None:
                            ...
                            aggregated_device_results = aggregated_results.find(
                                device_name
                            )
                            break
                        else:
                    ...
                    if aggregated_results_entry:
                        for xml_real_device_entry in xml_results_entry:
                            ...
                            aggregated_real_device_api_report = (
                                aggregated_results_entry.find(xml_real_device_entry.tag)
                            )
                            if aggregated_real_device_api_report is None:
                                ...
                                continue
                            update_result_node(
                                xml_real_device_entry, aggregated_real_device_api_report
                            )
                    else:
                        aggregated_device_results.append(xml_results_entry)
...
def merge_xml(
    ...
    if report_type == OP_CONFORMANCE.lower() or report_type == OP_CONFORMANCE:
        ...
        raise Exception(
            f"Error to create aggregated report. Incorrect report type: {report_type}"
        )
    ...
    for folder_path in input_folder_paths:
        ...
        timestamp = aggregate_test_results(
            results, xml_reports, report_type, merge_device_suffix
        )
        if report_type == "OP":
