...
def parse_arguments():
    ...
    parser.add_argument(
        "--output_filename", help=output_filename_help, default="report"
    )
    ...
    parser.add_argument(
        "--rel_weights", help=rel_weights_help, type=str, required=False
    )
...
def parse_rel_weights(rel_weights_path: os.path):
    ...
    if rel_weights_path:
        ...
        if os.path.isfile(rel_weights_file_path):
            ...
            logger.warning(
                f"Rel weights file does not exist! The expected passrates will be taken from runtime"
            )
    else:
        logger.warning(
            f"Rel weights file is not specified! The expected passrates will be taken from runtime"
        )
...
def merge_xmls(xml_paths: list):
    ...
    for xml_path in xml_paths:
        ...
        if timestamp is None or timestamp < xml_root.attrib["timestamp"]:
            logger.info(
                f'Timestamp is updated from {timestamp} to {xml_root.attrib["timestamp"]}'
            )
                ...
                for op_result in device:
                    ...
                    if current_op_res is not None:
                        ...
                        for attr_name in device_results.find(op_result.tag).attrib:
                            if (
                                "relative_" in attr_name
                                or attr_name == "passrate"
                                or attr_name == "implemented"
                            ):
                                continue
                            total_tests_count_xml += int(
                                op_result.attrib.get(attr_name)
                            )
                            total_tests_count_summary += int(
                                current_op_res.attrib.get(attr_name)
                            )
                        if total_tests_count_xml > total_tests_count_summary:
                            logger.warning(
                                f"Test counter is different in {op_result.tag} for {device.tag}"
                            ...
                            for attr_name in device_results.find(op_result.tag).attrib:
                                ...
                                device_results.find(current_op_res.tag).set(
                                    attr_name, str(xml_value)
                                )
                    else:
...
def collect_statistic(root: Element, is_conformance_mode: bool):
    ...
    for device in root.find("results"):
        ...
        for op in results[device.tag]:
            ...
            op_test_cnt = (
                int(results[device.tag][op]["passed"])
                + int(results[device.tag][op]["failed"])
                + int(results[device.tag][op]["crashed"])
                + int(results[device.tag][op]["skipped"])
            ...
            relative_pass_rate = float(
                "%.2f" % float(results[device.tag][op]["relative_passrate"])
            )
            ...
            general_test_count_rel[device.tag] += float(
                results[device.tag][op]["relative_all"]
            )
            general_passed_tests[device.tag] += int(results[device.tag][op]["passed"])
            general_passed_tests_rel[device.tag] += float(
                results[device.tag][op]["relative_passed"]
            )
            pass_rate_avg[device.tag] += float(results[device.tag][op]["passrate"])
            pass_rate_avg_rel[device.tag] += float(
                results[device.tag][op]["relative_passrate"]
            )
        ...
        pass_rate_avg[device.tag] = (
            0
            if covered_ops[device.tag] == 0
            else pass_rate_avg[device.tag] / covered_ops[device.tag]
        )
        pass_rate_avg[device.tag] = float("%.2f" % float(pass_rate_avg[device.tag]))
        pass_rate_avg_rel[device.tag] = (
            0
            if covered_ops[device.tag] == 0
            else pass_rate_avg_rel[device.tag] / covered_ops[device.tag]
        )
        pass_rate_avg_rel[device.tag] = float(
            "%.2f" % float(pass_rate_avg_rel[device.tag])
        )
        general_pass_rate[device.tag] = (
            0
            if general_test_count[device.tag] == 0
            else (
                general_passed_tests[device.tag] * 100 / general_test_count[device.tag]
            )
        )
        general_pass_rate[device.tag] = float(
            "%.2f" % float(general_pass_rate[device.tag])
        )
        general_pass_rate_rel[device.tag] = (
            0
            if general_test_count_rel[device.tag] == 0
            else (
                general_passed_tests_rel[device.tag]
                * 100
                / general_test_count_rel[device.tag]
            )
        )
        general_pass_rate_rel[device.tag] = float(
            "%.2f" % float(general_pass_rate_rel[device.tag])
        )
        trusted_ops[device.tag] = (
            float(
                "%.2f"
                % (
                    float("%.2f" % (float(trusted_ops[device.tag]) * 100))
                    / covered_ops[device.tag]
                )
            )
            if device.tag in covered_ops and covered_ops[device.tag] != 0
            else 0
        )
...
def create_summary(
    summary_root: Element,
    output_folder: os.path,
    expected_devices: list,
    report_tag: str,
    report_version: str,
    is_conformance_mode: bool,
    is_serialize_to_csv: bool,
    rel_weights_path: str,
    ...
    res_summary = template.render(
        ...
        results=results,
        timestamp=timestamp,
        general_pass_rate=general_pass_rate,
        general_pass_rate_rel=general_pass_rate_rel,
        pass_rate_avg=pass_rate_avg,
        pass_rate_avg_rel=pass_rate_avg_rel,
                ...
                for test_type in xml_root.findall(f"results/{device.tag}/*"):
                    ...
                    for plugin_info in xml_root.findall(
                        ...
                        if str(plugin_info.tag).upper() != str(device.tag).upper() and (
                            plugin_info.tag in sw_plugins
                            and str(plugin_info.tag).upper() != "TEMPLATE"
                        ):
                            continue

                        api_info[test_type.tag].setdefault(
                            device.tag, {"test_amout": 0}
                        )
                        for key in [
                            "passed",
                            "failed",
                            ...
                            api_info[test_type.tag][device.tag]["test_amout"] += (
                                val if "relative" not in key else 0
                            )
                            val += api_info[test_type.tag][device.tag].setdefault(
                                key, 0
                            )
                            api_info[test_type.tag][device.tag][key] = val

                        if api_info[test_type.tag][device.tag]["relative_all"] > 0:
                            api_info[test_type.tag][device.tag]["relative_passrate"] = (
                                round(
                                    api_info[test_type.tag][device.tag][
                                        "relative_passed"
                                    ]
                                    * 100
                                    / api_info[test_type.tag][device.tag][
                        ...
                        if api_info[test_type.tag][device.tag]["test_amout"] > 0:
                            api_info[test_type.tag][device.tag]["passrate"] = round(
                                api_info[test_type.tag][device.tag]["passed"]
                                * 100
                                / api_info[test_type.tag][device.tag]["test_amout"],
                                2,
                            )
