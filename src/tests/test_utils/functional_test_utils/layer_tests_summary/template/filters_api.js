...
$(document).ready(function() {
  ...
  Array.from($("#devices")[0].options)
      .map((element) => allDevices.push(element.value))
  ...
  for (device of allDevices) {
    ...
    $("#report #data td:not(:hidden)").filter(function() {
      if ($(this).attr(DATA_ATTRIBUTES.DEVICE) === device) {
        if ($(this).attr(DATA_ATTRIBUTES.PASSED) &&
            $(this).attr(DATA_ATTRIBUTES.TESTS_AMOUNT) &&
        ...
        if ($(this).attr(DATA_ATTRIBUTES.REL_PASSED) &&
            $(this).attr(DATA_ATTRIBUTES.REL_TESTS_AMOUNT) &&
...
function update_data() {
  ...
  if (scope_toggle == SCOPE_TYPE.OPTIONAL) {
    $('#scope_legend')
        .text(
            'Optinal - includes information and passrate about all apiConformance tests.')
  }
  else {
      $('#scope_legend')
          .text(
              'Mandatory - includes information about tests from mandatory tests.\
  ...
  $("#report #data td")
      .filter(function() {
        if (scope_toggle == SCOPE_TYPE.OPTIONAL &&
            $(this).attr(DATA_ATTRIBUTES.PASSRATE)) {
          $(this).children('span').text(
              $(this).attr(DATA_ATTRIBUTES.PASSRATE) + ' %')

          $($(this).find('.green')[0])
              .text('P:' + $(this).attr(DATA_ATTRIBUTES.PASSED))
          $($(this).find('.red')[0])
              .text('F:' + $(this).attr(DATA_ATTRIBUTES.FAILED))
          $($(this).find('.grey')[0]).show()
          $($(this).find('.grey')[0])
              .text('S:' + $(this).attr(DATA_ATTRIBUTES.SKIPPED))
          $($(this).find('.dark')[0]).show()
          $($(this).find('.dark')[0])
              .text('C:' + $(this).attr(DATA_ATTRIBUTES.CRASHED))
          $($(this).find('.grey-red')[0]).show()
          $($(this).find('.grey-red')[0])
              .text('H:' + $(this).attr(DATA_ATTRIBUTES.HANGED))
        } else if (
            scope_toggle == SCOPE_TYPE.MANDATORY &&
            $(this).attr(DATA_ATTRIBUTES.REL_PASSRATE)) {
          ...
          $($(this).find('.green')[0])
              .text('P:' + parseInt($(this).attr(DATA_ATTRIBUTES.REL_PASSED)))
          $($(this).find('.red')[0])
              .text(
                  'F:' +
                  (parseInt($(this).attr(DATA_ATTRIBUTES.REL_TESTS_AMOUNT)) -
                   parseInt($(this).attr(DATA_ATTRIBUTES.REL_PASSED))))
...
function filter_by_test_type() {
  ...
  $("#report #data td").filter(function() {
    ...
    let data_filter = $(this).attr(DATA_ATTRIBUTES.FILTER) || "";
    if (!testTypeName ||
        test_type.toLowerCase().indexOf(testTypeName.toLowerCase()) > -1) {
...
function change_device(value) {
  ...
  $("#report td").filter(function() {
    ...
    if (!selected_devices || selected_devices.length == 0 || !device ||
        selected_devices.includes(device)) {
