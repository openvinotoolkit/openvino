...
function filterTable() {
  device = $("#devices").val();
  if (device == 0) {
    $("#report td.value, #report td.nr_value, #report td.table-secondary, #report td.table-primary, #report th.table-dark.device")
        .show();
  } else {
    $("#report td.value, #report td.nr_value, #report td.table-secondary, #report td.table-primary, #report th.table-dark.device")
        .filter(function() {
  ...
  if (operationName) {
    $("#report #data tr:not(:hidden)").filter(function() {
      $(this).toggle(
          $(this).find('th').text().toLowerCase().indexOf(
              operationName.toLowerCase()) > -1);
    });
  }
  ...
  if (implementation != 0) {
    if (implementation == 'ni') {
      $("#report #data tr:not(:hidden)").filter(function() {
        $(this).toggle(
            $(this).find('td').hasClass("value " + device + " not_impl"))
      });
    } else if (implementation == 'i') {
      $("#report #data tr:not(:hidden)").filter(function() {
        $(this).toggle(
            $(this).find('td').hasClass("value " + device + " impl"));
      });
    } else {
      $("#report #data tr:not(:hidden)").filter(function() {
  ...
  if (status) {
    ...
    select.forEach(item => {
      if (item == '100p') {
        selector.push(
            '.value:visible[crashed="0"][failed="0"][skipped="0"][hanged="0"][value!="---"]');
      }
      if (item == '100f') {
...
function calculateColumnStatistics(device) {
  ...
  // trusted op
  count_trusted_op =
      $("#report #data tr:not(:hidden) ." + device +
        ".value[value^='100'][crashed='0'][failed='0'][skipped='0']")
          .length;
  all_operations =
      $("#report #data tr:not(:hidden) .value[value!='N/A'][value!='---'][value!='NOT RUN']." +
        device)
          .length;
  if (!all_operations) {
    trusted_op = "---";
  ...
  // $('#statistic .table-primary.' + device +
  // '.test_total').text(all_operations || 0);

  // tested op_counter
  tested_op_count = 0;
  passed_tested_op_count = 0;
  $("#report #data tr:not(:hidden) ." + device + ".value span")
      .each(function() {
  ...
  if (tested_op_count == 0) {
    ...
    general_pass_rate =
        (passed_tested_op_count * 100 / tested_op_count).toFixed(2) + ' %';
    $('#statistic .table-primary.' + device + '.general_pass_rate')
        .text(general_pass_rate);
  }
  $('#statistic .table-primary.' + device + '.tested-ops_count')
      .text(tested_op_count);

  // AVG Pass Rate
  sum_pass_rate = 0;
  $("#report #data tr:not(:hidden) ." + device + ".value").each(function() {
    if ($(this).attr('value') != 'N/A' && $(this).attr('value') != 'NOT RUN' &&
        $(this).attr('value') != '---') {
  ...
  if (all_operations == 0) {
    ...
    $('#statistic .table-primary.' + device + '.rel_pass_rate')
        .text(rel_pass_rate);
  }
}
