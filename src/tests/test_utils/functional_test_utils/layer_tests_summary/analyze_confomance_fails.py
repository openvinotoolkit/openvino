...
class AnalyzerConformanceLog:
    ...
    def setup_output_file(self, output_path, output_file_name):
        if output_path:
            if os.path.exists(output_path):
                ...
                logger.error(
                    f"Output file path {output_path} is not exists, directory {self.output_path} will be used"
                )
            ...
            for job_name in job_list:
                ...
                with zipfile.ZipFile(extended_path, "r") as zipObj:
                    for name in zipObj.namelist():
                        if (
                            API_CONFORMANCE_SUFFIX in name
                            and "logs" in name
                            and self.status_folder_exists(name)
                        ):
    ...
    def collect_tests_result(self, exclude_from_log):
        ...
        for root, dirs, files in os.walk(self.local_log_dir, topdown=False):
            if (
                not API_CONFORMANCE_SUFFIX in root
                or not "logs" in root
                or not self.status_folder_exists(root)
            ):
                continue
            ...
            for file_name in files:
                ...
                try:
                    ...
                    retult_root_match = re.match(
                        f"(\w*)_conformance_(dlb|omz)_{API_CONFORMANCE_SUFFIX}_.*",
                        path_components[-4],
                    )
                    ...
                    for line in lines:
                        ...
                        if (
                            line == ""
                            or constants.MEM_USAGE in line
                            or constants.REF_COEF in line
                            or status == "skipped"
                            or status == "passed"
                            or constants.TEST_STATUS["failed"][0] in line
                        ):
                            continue
                    ...
                    self.logs_info[test_group][test_name][device] = {
                        "name": test_name,
                        "status": status,
                        "err_info": error_msg,
                    }
    ...
    def create_exel(self, expected_devices):
        ...
        fail_fill = openpyxl.styles.PatternFill(
            start_color="FF8B8B", end_color="FF8B8B", fill_type="solid"
        )

        pass_font = openpyxl.styles.Font(color="006100")
        pass_fill = openpyxl.styles.PatternFill(
            start_color="C6E0B4", end_color="C6E0B4", fill_type="solid"
        )

        not_run_font = openpyxl.styles.Font(color="000000")
        not_run_fill = openpyxl.styles.PatternFill(
        ...
        for group_name, group_info in self.logs_info.items():
            ...
            worksheet.merge_cells(
                ...
                start_column=2,
                end_row=1,
                end_column=(len(expected_devices) * 2 + 1),
            )

            column = 2
            for dev in expected_devices:
                worksheet.cell(2, column, dev.upper())
                worksheet.cell(2, column).alignment = openpyxl.styles.Alignment(
                    horizontal="center"
                )
            ...
            for test_name, devices_info in group_info.items():
                ...
                all_pass = all(
                    [
                        dev.lower() not in devices_info
                        or (
                            dev.lower() in devices_info
                            and (
                                devices_info[dev.lower()]["status"] == "passed"
                                or devices_info[dev.lower()]["status"] == "skipped"
                            )
                        )
                        for dev in expected_devices
                    ]
                ...
                for dev in expected_devices:
                    ...
                    if real_dev.lower() in devices_info:
                        if devices_info[real_dev.lower()]["status"] == "passed":
                            ...
                            devices_info[real_dev.lower()]["status"] == "failed"
                            or devices_info[real_dev.lower()]["status"] == "hanged"
                            or devices_info[real_dev.lower()]["status"] == "crashed"
                            or devices_info[real_dev.lower()]["status"] == "interapted"
                        ):
                            status = "Fail"
        ...
        if len(workbook.worksheets) > 0:
            ...
            logger.info(
                f"Excel file hasn`t created because there is no content. This means that no errors were detected or something went wrong during the analysis"
            )
