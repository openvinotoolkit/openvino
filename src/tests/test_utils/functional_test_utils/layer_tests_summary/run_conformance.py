...
from summarize import create_api_summary, create_summary
...
def parse_arguments():
    ...
    entity_help = (
        "Specify validation entity: `Inference`, `ImportExport`, `QueryModel` or `OpImpl` for `OP` or "
    ...
    parser.add_argument(
        "-d", "--device", help=device_help, type=str, required=False, default="CPU"
    )
    parser.add_argument(
        "-t",
        "--type",
        ...
        type=str,
        required=False,
        default=constants.OP_CONFORMANCE,
    )
    parser.add_argument(
        "--gtest_filter",
        help=gtest_filter_helper,
        type=str,
        required=False,
        default="*",
    )
    ...
    parser.add_argument(
        "-ov", "--ov_path", help=ov_help, type=str, required=False, default=""
    )
    parser.add_argument(
        "-j",
        "--workers",
    ...
    parser.add_argument(
        "-c",
        "--ov_config_path",
        help=ov_config_path_helper,
        type=str,
        required=False,
        default="",
    )
    parser.add_argument(
        "-s", "--dump_graph", help=dump_graph_help, type=int, required=False, default=0
    )
    parser.add_argument(
        "-sm",
        "--special_mode",
        help=special_mode_help,
        type=str,
        required=False,
        default="",
    )
    parser.add_argument(
        "-e", "--entity", help=entity_help, type=str, required=False, default=""
    )
    parser.add_argument(
        "-p",
        "--parallel_devices",
    ...
    parser.add_argument(
        "-u",
        "--expected_failures_update",
        help=expected_failures_update_help,
        required=False,
        default=False,
        action="store_true",
    )
    parser.add_argument(
        "--cache_path", help=cache_path_help, type=str, required=False, default=""
    )
    parser.add_argument(
        "-r",
        "--disable_rerun",
        help=disable_rerun_help,
        required=False,
        default=False,
        type=bool,
    )
    parser.add_argument(
        "--timeout", help=timeout_help, required=False, default=-1, type=int
    )
...
class Conformance:
    def __init__(
        ...
        gtest_filter: str,
        working_dir: os.path,
        ov_config_path: os.path,
        special_mode: str,
        entity: str,
        cache_path: str,
        parallel_devices: bool,
        expected_failures_file: str,
        expected_failures_update: bool,
        disable_is_rerun: bool,
        timeout: int,
    ):
        self._device = device
        ...
        if type == constants.OP_CONFORMANCE:
            if (
                entity == "Inference"
                or entity == "QueryModel"
                or entity == "ImportExport"
                or entity == ""
                or entity == "OpImpl"
            ):
                ...
                logger.error(
                    f"Incorrect value to set entity type: {special_mode}. Please check `help` to get possible values"
                )
                exit(-1)
            if (
                special_mode == "static"
                or special_mode == "dynamic"
                or special_mode == ""
            ):
                self._special_mode = special_mode
            else:
                logger.error(
                    f"Incorrect value to set shape mode: {special_mode}. Please check `help` to get possible values"
                )
                exit(-1)
            self._gtest_filter = f"*{self.__entity}*{gtest_filter}*{'' if self.__entity == 'OpImpl' else ':-*OpImpl*'}"
        elif type == constants.API_CONFORMANCE:
            if (
                entity == "ov_compiled_model"
                or entity == "ov_plugin"
                or entity == "ov_infer_request"
                or entity == ""
            ):
                self.__entity = entity
            else:
                logger.error(
                    f"Incorrect value to set shape mode: {special_mode}. Please check to get possible values"
                )
                exit(-1)
            self._special_mode = ""
            if special_mode == "mandatory":
                ...
                logger.error(
                    f"Incorrect value to set API scope: {special_mode}. Please check to get possible values"
                )
                exit(-1)
        else:
            logger.error(
        ...
        if os.path.isfile(expected_failures_file):
            self._expected_failures = self.__get_failed_test_from_csv(
                expected_failures_file
            )
        else:
            logger.warning(
                f"Expected failures testlist `{self._expected_failures_file}` does not exist!"
            )
    ...
    def __download_models(self, url_to_download, path_to_save):
        ...
        try:
            logger.info(
                f"Conformance IRs will be downloaded from {url_to_download} to {download_path}"
            )
            ur.urlretrieve(url_to_download, filename=download_path)
        except Exception as exc:
            logger.error(
                f"Please verify URL: {url_to_download}. It might be incorrect. See below for the full error."
            )
            logger.exception(f"FULL ERROR: {exc}")
            exit(-1)
        logger.info(
            f"Conformance IRs were downloaded from {url_to_download} to {download_path}"
        )
        ...
        if file_utils.is_archieve(download_path):
            logger.info(
                f"The file {download_path} is archived. Should be unzipped to {path_to_save}"
            )
            return file_utils.unzip_archieve(download_path, path_to_save)
        return download_path

    def __dump_subgraph(self):
        subgraph_dumper_path = os.path.join(
            self._ov_path, f"{SUBGRAPH_DUMPER_BIN_NAME}{constants.OS_BIN_FILE_EXT}"
        )
        ...
        self._model_path = file_utils.prepare_filelist(
            self._model_path, constants.SUPPORTED_MODEL_EXTENSION
        )
        ...
        if has_python_api:
            ...
            logger.info(
                f"All conformance IRs in {self._model_path} were renamed based on hash"
            )
        else:
            logger.warning(
    ...
    def __update_expected_failures(self):
        this_failures_file = os.path.join(
            self._working_dir, f"{self._device}_logs", "logs", "fix_priority.csv"
        )
        ...
        if (
            this_run_failures != self._expected_failures
            and self._expected_failures_update
            and not len(self._unexpected_failures)
        ):
            logger.info(
                f"Expected failures file {self._expected_failures_file} will be updated! "
    ...
    def __run_conformance(self):
        ...
        if self._type == constants.OP_CONFORMANCE:
            conformance_path = os.path.join(
                self._ov_path, f"{OP_CONFORMANCE_BIN_NAME}{constants.OS_BIN_FILE_EXT}"
            )
        else:
            conformance_path = os.path.join(
                self._ov_path, f"{API_CONFORMANCE_BIN_NAME}{constants.OS_BIN_FILE_EXT}"
            )
        ...
        command_line_args = [
            ...
            (
                f'--input_folders="{self._model_path}"'
                if self._type == constants.OP_CONFORMANCE
                else ""
            ),
            f"--report_unique_name",
            f'--output_folder="{parallel_report_dir}"',
            f'--gtest_filter="{self._gtest_filter}"',
            f'--config_path="{self._ov_config_path}"',
            f"--shape_mode={self._special_mode}",
        ]
        conformance = TestParallelRunner(
            exec_file_path=f"{conformance_path}",
        ...
        merge_xml(
            [parallel_report_dir], report_dir, final_report_name, self._type, True
        )
    ...
    def __summarize(self, xml_report_path: os.path, report_dir: os.path):
        if self._type == constants.OP_CONFORMANCE:
            ...
            rel_weights_path = os.path.join(
                self._model_path,
                constants.REL_WEIGHTS_FILENAME.replace(
                    constants.REL_WEIGHTS_REPLACE_STR, self._special_mode
                ),
            )
            create_summary(
                summary_root, report_dir, [], "", "", True, True, rel_weights_path
            )
        else:
            create_api_summary([xml_report_path], report_dir, [], "", "")
        copytree(
            os.path.join(SCRIPT_DIR_PATH, "template"),
            os.path.join(report_dir, "template"),
        )
    ...
    def run(self, dump_models: bool):
        ...
        logger.info(
            f"[ARGUMENTS] --parallel_devices = {self._is_parallel_over_devices}"
        )
        ...
        logger.info(
            f"[ARGUMENTS] --expected_failures_update = {self._expected_failures_update}"
        )
        ...
        if self._type == constants.OP_CONFORMANCE:
            if file_utils.is_url(self._model_path):
                self._model_path = self.__download_models(
                    self._model_path, self._working_dir
                )
            if (
                self._model_path == NO_MODEL_CONSTANT
                or os.path.splitext(self._model_path)[1] == ".lst"
            ):
