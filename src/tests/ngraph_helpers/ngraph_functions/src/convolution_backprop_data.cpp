...
std::shared_ptr<Node> makeConvolutionBackpropData(const ov::Output<Node>& in,
                                                  ...
                                                  const std::vector<float>& biasesWeights) {
    auto deconv =
        std::make_shared<opset1::ConvolutionBackpropData>(in, weights, strides, padsBegin, padsEnd, dilations, autoPad);

    if (!outputPadding.empty()) {
        deconv = std::make_shared<opset1::ConvolutionBackpropData>(in,
                                                                   weights,
                                                                   strides,
                                                                   padsBegin,
                                                                   padsEnd,
...
std::shared_ptr<Node> makeConvolutionBackpropData(const ov::Output<Node>& in,
    ...
    auto deconv = std::make_shared<opset1::ConvolutionBackpropData>(in,
                                                                    filterWeightsNode,
                                                                    outputShape,
                                                                    strides,
                                                                    padsBegin,
