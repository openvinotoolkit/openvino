...
std::shared_ptr<Node> makeConvolutionBackpropData(const ov::Output<Node> &in,
                                                  ...
                                                  const std::vector<ptrdiff_t> &outputPadding,
                                                  const std::vector<float> &biasesWeights) {
    auto deconv = std::make_shared<ov::op::v1::ConvolutionBackpropData>(in, weights, strides, padsBegin, padsEnd, dilations, autoPad);

    if (!outputPadding.empty()) {
        deconv = std::make_shared<ov::op::v1::ConvolutionBackpropData>(in, weights, strides, padsBegin, padsEnd, dilations, autoPad, outputPadding);
    }
...
std::shared_ptr<Node> makeConvolutionBackpropData(const ov::Output<Node> &in,
    ...
    auto deconv = std::make_shared<ov::op::v1::ConvolutionBackpropData>(in, filterWeightsNode, outputShape, strides, padsBegin, padsEnd, dilations, autoPad);

    if (!outputPadding.empty()) {
        deconv = std::make_shared<ov::op::v1::ConvolutionBackpropData>(in, filterWeightsNode, outputShape, strides, padsBegin,
