...
ngraph::ParameterVector makeDynamicParams(const std::vector<element::Type>& types,
                                          const std::vector<ov::PartialShape>& shapes);

ngraph::ParameterVector makeParams(const element::Type& type,
                                   const std::vector<std::pair<std::string, std::vector<size_t>>>& inputs);

template <typename T>
std::shared_ptr<Node> makeConstant(const element::Type& type,
...
#define makeNode(TYPE)                                                                                               \
    case TYPE:                                                                                                       \
        weightsNode = std::make_shared<ngraph::opset1::Constant>(                                                    \
            ...
            shape,                                                                                                   \
            random                                                                                                   \
                ? NGraphFunctions::Utils::generateVector<TYPE>(                                                      \
...
std::shared_ptr<ngraph::Node> makeInputLayer(const element::Type& type,
                                             ngraph::helpers::InputLayerType inputType,
                                             const std::vector<size_t>& shape);

std::shared_ptr<ngraph::Node> makeDynamicInputLayer(const element::Type& type,
                                                    ngraph::helpers::InputLayerType inputType,
...
std::shared_ptr<ngraph::Node> makeCTCGreedyDecoder(const ngraph::Output<Node>& inputData, const bool mergeRepeated);

std::shared_ptr<ngraph::Node> makeCTCGreedyDecoderSeqLen(const ngraph::Output<Node>& inputData,
...
std::shared_ptr<ngraph::Node> makeCTCLoss(const ngraph::Output<Node>& logitsNode,
...
std::shared_ptr<ngraph::Node> makeEmbeddingBagOffsetsSum(const element::Type& dataType,
...
std::shared_ptr<ngraph::Node> makeEmbeddingBagPackedSum(const element::Type& dataType,
                                                        const element::Type& indicesType,
                                                        const ngraph::Output<Node>& emb_table_node,
                                                        const std::vector<std::vector<size_t>>& indices,
                                                        bool with_weights);

std::shared_ptr<ngraph::Node> makeEmbeddingSegmentsSum(const element::Type& dataType,
                                                       const element::Type& indicesType,
                                                       const ngraph::Output<Node>& emb_table_node,
                                                       const std::vector<size_t>& indices,
...
std::shared_ptr<Node> makeShuffleChannels(const ngraph::Output<Node>& in, int axis, int group);
...
std::shared_ptr<ngraph::Node> makeBatchNormInference(const ngraph::Output<Node>& data, double epsilon);

std::shared_ptr<ngraph::Node> makeLSTM(
...
std::shared_ptr<ngraph::Node> makeGRU(
    const OutputVector& in,
    const std::vector<ngraph::Shape>& constants,
    std::size_t hidden_size,
...
std::shared_ptr<ngraph::Node> makeRNN(
    const OutputVector& in,
    const std::vector<ngraph::Shape>& constants,
    std::size_t hidden_size,
...
std::shared_ptr<ngraph::Node> makeGatherElements(const ngraph::Output<Node>& dataNode,
                                                 const ngraph::Shape& indicesShape,
                                                 const element::Type& indicesType,
                                                 const int axis);

std::shared_ptr<ngraph::Node> makeGatherND(const ngraph::Output<Node>& dataNode,
                                           const ngraph::Shape& indicesShape,
                                           const element::Type& indicesType,
                                           const std::size_t batchDims);

std::shared_ptr<ngraph::Node> makeGatherND8(const ngraph::Output<Node>& dataNode,
                                            const ngraph::Shape& indicesShape,
                                            const element::Type& indicesType,
                                            const std::size_t batchDims);

std::shared_ptr<ngraph::Node> makeTile(const ngraph::Output<Node>& in, const std::vector<int64_t>& repeats);
