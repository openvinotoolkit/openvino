 ...
 * (declared in subgraph_simple.hpp). Note that the corresponding SnippetsFunctionBase child should use virtual
 * inheritance from SnippetsFunctionBase (typically "virtual public") to avoid creation of two internal copies of
 * SnippetsFunctionBase.
 */
...
public:
    explicit AddFunctionLoweredBroadcast(const std::vector<PartialShape>& inputShapes,
                                         const std::vector<Shape>& broadcastShapes)
        : AddFunction(inputShapes),
...
public:
    explicit EltwiseThreeInputsLoweredFunction(const std::vector<PartialShape>& inputShapes,
                                               const std::vector<Shape>& broadcastShapes)
        : EltwiseThreeInputsFunction(inputShapes),
          broadcast_shapes{broadcastShapes} {
        OPENVINO_ASSERT(input_shapes.size() == broadcast_shapes.size(),
                        "Broadcast shapes should have the same size as input_shapes");
...
public:
    explicit Transpose0213MatMulLoweredFunction(const std::vector<PartialShape>& inputShapes, size_t position = 0)
        : Transpose0213MatMulFunction(inputShapes,
