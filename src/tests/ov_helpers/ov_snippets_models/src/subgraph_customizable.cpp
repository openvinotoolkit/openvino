...
std::shared_ptr<ov::Model> ConvBiasActivationFunction::initOriginal() const {
    ...
    auto add_param = std::make_shared<op::v0::Parameter>(precision, input_shapes[1]);

    auto add = custom_ops[0]->clone_with_new_inputs({conv->output(0), add_param->output(0)});
    auto unary = custom_ops[1]->clone_with_new_inputs({add->output(0)});
    add->set_arguments({conv->output(0), add_param->output(0)});
    unary->set_arguments({add->output(0)});

    return std::make_shared<ov::Model>(NodeVector{unary}, ParameterVector{conv_param, add_param});
}
std::shared_ptr<ov::Model> ConvBiasTwoActivationFunction::initOriginal() const {
    ...
    return std::make_shared<ov::Model>(NodeVector{unary_2}, ParameterVector{conv_param, add_param});
}
std::shared_ptr<ov::Model> ConvBiasTwoActivationFunction::initReference() const {
    ...
    return std::make_shared<ov::Model>(NodeVector{subgraph}, ParameterVector{conv_param, add_param});
}
std::shared_ptr<ov::Model> MatMulTwoActivationFunction::initOriginal() const {
    ...
    auto add_param = std::make_shared<op::v0::Parameter>(precision, input_shapes[2]);

    auto add = std::make_shared<op::v1::Add>(matmul, add_param);
    auto unary_1 = custom_ops[1]->clone_with_new_inputs({add->output(0)});
    auto unary_2 = custom_ops[2]->clone_with_new_inputs({unary_1->output(0)});
    add->set_arguments({matmul->output(0), add_param->output(0)});
    unary_1->set_arguments({add->output(0)});
    unary_2->set_arguments({unary_1->output(0)});

    return std::make_shared<ov::Model>(NodeVector{unary_2}, ParameterVector{matmul_param0, matmul_param1, add_param});
}
std::shared_ptr<ov::Model> MatMulBiasActivationBinaryFunction::initOriginal() const {
    ...
    auto binary_param = std::make_shared<op::v0::Parameter>(precision, input_shapes[3]);
    ...
    return std::make_shared<ov::Model>(NodeVector{binary}, ParameterVector{matmul_param0, matmul_param1, add_param, binary_param});
}
std::shared_ptr<ov::Model> MatMulBiasActivationBinaryFunction::initReference() const {
    ...
    return std::make_shared<ov::Model>(NodeVector{subgraph}, ParameterVector{matmul_param0, matmul_param1, add_param, binary_param});
}
}  // namespace snippets
}  // namespace test
}  // namespace ov