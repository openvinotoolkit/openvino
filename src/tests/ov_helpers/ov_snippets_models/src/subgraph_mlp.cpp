...
std::ostream& operator<<(std::ostream& os, MLPFunction::WeightFormat type) {
    switch (type) {
    case MLPFunction::WeightFormat::FP32:
        os << "FP32";
        break;
    case MLPFunction::WeightFormat::FP16:
        os << "FP16";
        break;
    default:
...
std::shared_ptr<ov::Node> MLPFunction::makeWeights(const Shape& shape) const {
    switch (m_wei_format) {
...
std::shared_ptr<ov::Node> MLPFunction::makeFC(const ov::Output<ov::Node>& output, const ov::Output<ov::Node>& weights) const {
    return std::make_shared<ov::op::v0::MatMul>(output, weights, false, true);
}

std::shared_ptr<ov::Node> MLPFunction::makeFC(const ov::Output<ov::Node>& output, const Shape& shape) const {
...
std::shared_ptr<ov::Model> MLPFunction::initReference() const {
