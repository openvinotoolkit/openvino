...
std::shared_ptr<ov::Model> MatMulFunction::initReference() const {
    ...
    const auto subgraph = std::make_shared<ov::snippets::op::Subgraph>(
        ...
        std::make_shared<ov::Model>(NodeVector{matmul}, ParameterVector{indata0, indata1}));
...
std::shared_ptr<ov::Model> FQMatMulFunction::initOriginal() const {
    ...
    auto constant = ov::test::utils::deprecated::make_constant(ov::element::i8,
                                                               const_shape.get_shape(),
                                                               std::vector<int8_t>{},
                                                               true);
    auto convert = std::make_shared<op::v0::Convert>(constant, ov::element::f32);
    auto deq_mul =
        std::make_shared<op::v0::Constant>(ov::element::f32, ov::Shape{1}, std::vector<float>{0.00499185826});
...
std::shared_ptr<ov::Model> MatMulBiasQuantizedFunction::initOriginal() const {
    ...
    auto matmul = std::make_shared<op::TypeRelaxed<op::v0::MatMul>>(
        ...
        ov::op::TemporaryReplaceOutputType(data0, element::f32).get(),
        ov::op::TemporaryReplaceOutputType(data1, element::f32).get());
    auto fq2 = ov::test::utils::make_fake_quantize(matmul,
                                                   ov::element::f32,
                                                   256,
                                                   {1},
                                                   {-35.0172004},
...
std::shared_ptr<ov::Model> MatMulsQuantizedFunction::initOriginal() const {
    ...
    auto matmul0 = std::make_shared<op::TypeRelaxed<op::v0::MatMul>>(
        ...
        ov::op::TemporaryReplaceOutputType(data1, element::f32).get());
    auto fq0 =
        ov::test::utils::make_fake_quantize(matmul0, ov::element::f32, 256, {1}, {0}, {0.820726}, {0}, {0.820726});
    auto fq2 = ov::test::utils::make_fake_quantize(data2,
                                                   ov::element::f32,
                                                   256,
                                                   {1},
                                                   {-35.0172004},
                                                   ...
                                                   {-35.0172004},
                                                   {34.7436294});
    auto new_shape = std::make_shared<ov::op::v0::Constant>(
        ov::element::u64,
        ov::Shape{4},
...
std::shared_ptr<ov::Model> MatMulsQuantizedSoftmaxFunction::initOriginal() const {
    ...
    auto fq0 =
        ov::test::utils::make_fake_quantize(softmax, ov::element::f32, 256, {1}, {0}, {0.820726}, {0}, {0.820726});
    auto fq2 = ov::test::utils::make_fake_quantize(data2,
                                                   ov::element::f32,
                                                   256,
                                                   {1},
