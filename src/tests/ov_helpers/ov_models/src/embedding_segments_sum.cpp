...
std::shared_ptr<Node> makeEmbeddingSegmentsSum(
    ...
    auto indicesNode = std::make_shared<ov::opset1::Constant>(indicesType, i_shape, indices);
    std::vector<size_t> o_shape = {segment_ids.size()};
    auto segmentIdNode = std::make_shared<ov::opset1::Constant>(indicesType, o_shape, segment_ids);
    std::vector<size_t> shape_0 = {};
    auto segmentNumNode = std::make_shared<ov::opset1::Constant>(indicesType, shape_0, num_segments);

    std::shared_ptr<Node> embBag;
    if (with_default_index) {
        auto defIdxNode = std::make_shared<ov::opset1::Constant>(indicesType, shape_0, default_index);
        if (with_weights) {
