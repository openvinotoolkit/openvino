...
std::shared_ptr<Node> makeEmbeddingBagOffsetsSum(
    ...
    auto offsetsNode = std::make_shared<ov::opset1::Constant>(indicesType, o_shape, offsets);

    std::shared_ptr<Node> embBag;
    if (with_default_index) {
        std::vector<size_t> d_shape = {};
        auto defIdxNode = std::make_shared<ov::opset1::Constant>(indicesType, d_shape, default_index);
        if (with_weights) {
