...
std::vector<std::pair<ov::element::Type, std::vector<std::uint8_t>>>
        interpreterFunction(const std::shared_ptr<Model> &function,
...
std::vector<ov::Tensor> interpretFunction(const std::shared_ptr<Model> &function,
...
std::shared_ptr<Model> foldFunction(const std::shared_ptr<Model> &function,
                                       const std::vector<std::vector<std::uint8_t>> &inputs,
                                       const std::vector<ov::element::Type> &inputTypes) {
    const auto &parameters = function->get_parameters();
    const auto &parametersNumber = parameters.size();
    ...
    for (const auto &op : foldedFunc->get_ops()) {
        OPENVINO_ASSERT(op::util::is_constant(op) || op::util::is_output(op) || op::util::is_parameter(op),
...
std::vector<std::pair<ov::element::Type, std::vector<std::uint8_t>>> getConstData(const std::shared_ptr<Model> &function) {
    ...
    for (size_t i = 0; i < numOutputs; i++) {
        ...
        OPENVINO_ASSERT(op::util::is_constant(parrentNode), "Function was not fully folded to constant state!\n",
...
void CompareShapes(const PartialShape& actual, const PartialShape& expected) {
    OPENVINO_ASSERT(actual.relaxes(expected) && actual.refines(expected), "Functions compare: Different shape detected ", actual, " and ", expected);
}
...
void CompareNodes(const Node& actual, const Node& expected) {
    ...
    OPENVINO_ASSERT(actualType == expectedType, "Functions compare: data types must be equal ", toString(actualType), " != ", toString(expectedType));

    const auto& numActualInputs = actual.inputs().size();
    const auto& numExpectedInputs = expected.inputs().size();
    OPENVINO_ASSERT(numActualInputs == numExpectedInputs, "Functions compare: numbers of inputs are different: ", numActualInputs, " and ", numExpectedInputs);

    const auto& numActualOutputs = actual.outputs().size();
    const auto& numExpectedOutputs = expected.outputs().size();
    OPENVINO_ASSERT(numActualOutputs == numExpectedOutputs, "Functions compare: numbers of outputs are different: ",
                 numActualOutputs, " and ", numExpectedOutputs);
}

}  // namespace

void CompareFunctions(const Model& actual, const Model& expected) {
...
std::shared_ptr<ov::Node> getNodeSharedPtr(const ov::NodeTypeInfo &type_info, const ov::OutputVector &outputVector) {
    for (const auto& opset : {ov::get_opset5(), ov::get_opset4(), ov::get_opset3(), ov::get_opset2(), ov::get_opset1()})
        if (opset.contains_type(type_info)) {
            const auto ngraphNode = std::shared_ptr<ov::Node>(opset.create(type_info.name));
...
bool is_tensor_iterator_exist(const std::shared_ptr<ov::Model> & func) {
    ...
    for (const auto& node : ops) {
        const auto& ti = std::dynamic_pointer_cast<ov::opset5::TensorIterator>(node);
        if (ti) {
            return true;
        }
    }
...
std::ostream& operator<<(std::ostream & os, const ov::helpers::EltwiseTypes type) {
    switch (type) {
...
std::ostream& operator<<(std::ostream & os, ov::helpers::SqueezeOpType type) {
    switch (type) {
...
std::ostream& operator<<(std::ostream& os, ov::helpers::InputLayerType type) {
    switch (type) {
...
std::ostream& operator<<(std::ostream & os, ov::helpers::ComparisonTypes type) {
    switch (type) {
...
std::ostream& operator<<(std::ostream & os, ov::helpers::LogicalTypes type) {
    switch (type) {
