...
std::shared_ptr<Node> makeFakeQuantize(const ov::Output<Node> &in,
    ...
    auto fq = std::make_shared<ov::opset1::FakeQuantize>(in, inputLowNode, inputHighNode, outputLowNode, outputHighNode, levels);
...
std::shared_ptr<ov::Node> makeFakeQuantize(const ov::Output<ov::Node> &in,
    ...
    auto inputLowNode = ov::builder::makeConstant(type, constShapes, inputLowData, inputLowData.empty(), 10.f, 1.f, seed);
    auto inputHighNode = ov::builder::makeConstant(type, constShapes, inputHighData, inputHighData.empty(), 10.f, 1.f, seed);
    auto outputLowNode = ov::builder::makeConstant(type, constShapes, outputLowData, outputLowData.empty(), 10.f, 1.f, seed);
    auto outputHighNode = ov::builder::makeConstant(type, constShapes, outputHighData, outputHighData.empty(), 10.f, 1.f, seed);
