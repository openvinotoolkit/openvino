...
std::shared_ptr<Node> makeFullyConnected(const ov::Output<Node>& in,
    ...
    auto weightsNode = ov::test::utils::make_constant(type, shape, weights, randomWeights);
    ...
    if (addBias) {
        ...
        auto biasWeightsNode = ov::test::utils::make_constant(type, {}, biasWeights, randomBiasWeights);
