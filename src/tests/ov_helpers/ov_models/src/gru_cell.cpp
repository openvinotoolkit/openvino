...
std::shared_ptr<ov::Node> makeGRU(const OutputVector& in,
    ...
    auto W = ov::test::utils::make_constant(in[0].get_element_type(), constants[0], empty, true);
    auto R = ov::test::utils::make_constant(in[0].get_element_type(), constants[1], empty, true);
    auto B = ov::test::utils::make_constant(in[0].get_element_type(), constants[2], empty, true);
    if (!make_sequence) {
        ...
        if (in.size() > 2 && in[2].get_partial_shape().is_dynamic()) {
            ...
            case ov::test::utils::SequenceTestsMode::CONVERT_TO_TI_MAX_SEQ_LEN_CONST: {
                ...
                seq_lengths = ov::test::utils::make_constant(element::i64, constants[3], lengths, false);
                break;
            }
            ...
            case ov::test::utils::SequenceTestsMode::PURE_SEQ_RAND_SEQ_LEN_CONST: {
                for (size_t i = 0; i <= in[0].get_shape().at(0); ++i) {
                    ...
                    seq_lengths = ov::test::utils::make_constant(element::i64,
                                                                 constants[3],
                                                                 lengths,
                                                                 true,
