...
std::shared_ptr<ov::Node> makeDetectionOutput(const ov::OutputVector &inputs,
    ...
    if (inputs.size() == 3)
        return std::make_shared<ov::opset3::DetectionOutput>(inputs[0], inputs[1], inputs[2], attrs);
    else if (inputs.size() == 5)
        return std::make_shared<ov::opset3::DetectionOutput>(inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], attrs);
    else
