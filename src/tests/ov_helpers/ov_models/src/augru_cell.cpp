...
std::shared_ptr<ov::Node> makeAUGRU(const OutputVector& in,
    ...
    auto W = ov::builder::makeConstant(in[0].get_element_type(), constants[0], empty, true);
    W->set_friendly_name("augru_w");
    auto R = ov::builder::makeConstant(in[0].get_element_type(), constants[1], empty, true);
    R->set_friendly_name("augru_r");
    auto B = ov::builder::makeConstant(in[0].get_element_type(), constants[2], empty, true);
    ...
    if (!make_sequence) {
        ...
        if (in.size() > 3 && in[3].get_partial_shape().is_dynamic()) {
            ...
            switch (mode) {
                case ov::helpers::SequenceTestsMode::PURE_SEQ:
                case ov::helpers::SequenceTestsMode::CONVERT_TO_TI_MAX_SEQ_LEN_CONST: {
                    std::vector<float> lengths(in[0].get_partial_shape()[0].get_min_length(), in[0].get_partial_shape()[1].get_min_length());
                    seq_lengths = ov::builder::makeConstant(element::i64, constants[3], lengths, false);
                    break;
                }
                case ov::helpers::SequenceTestsMode::CONVERT_TO_TI_RAND_SEQ_LEN_CONST:
                case ov::helpers::SequenceTestsMode::PURE_SEQ_RAND_SEQ_LEN_CONST: {
                    for (size_t i = 0; i <= in[0].get_shape().at(0); ++i) {
                        std::vector<float> lengths;
                        seq_lengths = ov::builder::makeConstant(element::i64, constants[3], lengths, true,
                                                                    static_cast<float>(in[0].get_shape()[1]), 0.f);
                    }
                    break;
                }
                case ov::helpers::SequenceTestsMode::CONVERT_TO_TI_RAND_SEQ_LEN_PARAM:
                case ov::helpers::SequenceTestsMode::CONVERT_TO_TI_MAX_SEQ_LEN_PARAM:
                case ov::helpers::SequenceTestsMode::PURE_SEQ_RAND_SEQ_LEN_PARAM: {
