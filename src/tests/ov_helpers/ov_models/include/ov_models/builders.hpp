...
#define makeNode(TYPE) \
        case TYPE: \
            weightsNode = std::make_shared<ov::opset1::Constant>( \
                    type, shape, \
                    random ? NGraphFunctions::Utils::generateVector<TYPE>( \
                                ov::shape_size(shape), \
                                ov::helpers::nGraphTypesTrait<TYPE>::value_type(upTo), \
                                ov::helpers::nGraphTypesTrait<TYPE>::value_type(startFrom), \
                                seed) \
                           : NGraphFunctions::Utils::castVector<T, ov::helpers::nGraphTypesTrait<TYPE>::value_type >(data)); \
            break;
    switch (type) {
...
std::shared_ptr<ov::Node> makeInputLayer(const element::Type& type, ov::helpers::InputLayerType inputType,
                                             const std::vector<size_t>& shape);

std::shared_ptr<ov::Node> makeDynamicInputLayer(const element::Type& type, ov::helpers::InputLayerType inputType,
...
std::shared_ptr<ov::Node> makeCTCGreedyDecoder(
        const ov::Output<Node>& inputData,
        const bool mergeRepeated);

std::shared_ptr<ov::Node> makeCTCGreedyDecoderSeqLen(
        const ov::Output<Node>& inputData,
...
std::shared_ptr<ov::Node> makeCTCLoss(
        const ov::Output<Node>& logitsNode,
...
std::shared_ptr<ov::Node> makeEmbeddingBagOffsetsSum(
...
std::shared_ptr<ov::Node> makeEmbeddingBagPackedSum(
        const element::Type &dataType,
        const element::Type &indicesType,
        const ov::Output<Node> &emb_table_node,
...
std::shared_ptr<ov::Node> makeEmbeddingSegmentsSum(
        const element::Type &dataType,
        const element::Type &indicesType,
        const ov::Output<Node> &emb_table_node,
        const std::vector<size_t> &indices,
...
std::shared_ptr<ov::Node> makeDepthToSpace(const ov::Output<Node> &in,
                                               ov::opset3::DepthToSpace::DepthToSpaceMode mode,
                                               size_t blockSize);
...
std::shared_ptr<ov::Node> makeConcat(const std::vector<ov::Output<Node>>& in,
                                         const int& axis);
...
std::shared_ptr<ov::Node> makeBatchNormInference(const ov::Output<Node>& data,
                                                     double epsilon);

std::shared_ptr<ov::Node> makeLSTM(const OutputVector& in,
                                           ...
                                           ov::helpers::SequenceTestsMode mode = ov::helpers::SequenceTestsMode::PURE_SEQ,
                                           float WRB_range = 0.f);
...
std::shared_ptr<ov::Node> makeGatherElements(
                                      const ov::Output<Node>& dataNode,
                                      const ov::Shape& indicesShape,
                                      const element::Type& indicesType,
                                      const int axis);

std::shared_ptr<ov::Node> makeGatherND(
                                      const ov::Output<Node>& dataNode,
                                      const ov::Shape& indicesShape,
                                      const element::Type& indicesType,
                                      const std::size_t batchDims);

std::shared_ptr<ov::Node> makeGatherND8(
                                      const ov::Output<Node>& dataNode,
                                      const ov::Shape& indicesShape,
                                      const element::Type& indicesType,
                                      const std::size_t batchDims);
