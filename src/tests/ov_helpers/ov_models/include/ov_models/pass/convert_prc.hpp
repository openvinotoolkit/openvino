...
public:
    ConvertConstantsPrecision() {
        auto constant =
            std::make_shared<ov::op::v0::Constant>(element::f32, Shape{1}, std::vector<float>{0});
        ...
        ov::matcher_pass_callback callback = [](ov::pass::pattern::Matcher &m) {
            ...
            if (constant->get_element_type() == ov::element::Type(from)) {
                auto data = constant->cast_vector<typename ov::helpers::nGraphTypesTrait<to>::value_type>();
...
public:
    ConvertConvertLayerOutputPrecision() {
        auto convert = ov::pass::pattern::wrap_type<opset1::Convert>();
        ov::matcher_pass_callback callback = [](ov::pass::pattern::Matcher &m) {
