...
std::shared_ptr<Model> ConvolutionBackpropDataFunction::get(
    ...
    ov::ResultVector results{ std::make_shared<opset1::Result>(convolutionBackpropData) };
...
std::shared_ptr<Model> ConvolutionBackpropDataFunction::getOriginal(
...
std::shared_ptr<Model>  ConvolutionBackpropDataFunction::getReference(
    const element::Type precision,
    const element::Type netPrecision,
    const PartialShape& inputShape,
    const Shape& outputShape,
    ...
    ov::ResultVector results{ std::make_shared<opset1::Result>(result) };
    return std::make_shared<ov::Model>(results, ParameterVector{ input }, "ConvolutionBackpropDataTransformation");
}
