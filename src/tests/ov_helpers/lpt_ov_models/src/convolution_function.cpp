...
std::shared_ptr<ov::Model> ConvolutionFunction::getOriginal(
    ...
    if (weights->cast_vector<float>().size() == 1ul) {
        ...
        weights = ov::as_type_ptr<ov::opset1::Constant>(fold<ov::opset1::Broadcast>(
            weights, op::v0::Constant::create(ov::element::i64, Shape{ targetShape.size() }, targetShape)));
    }
...
std::shared_ptr<ov::Model> ConvolutionFunction::get(
    ...
    const std::shared_ptr<ov::opset1::FakeQuantize> fqOnData = ov::as_type_ptr<ov::opset1::FakeQuantize>(ov::builder::makeFakeQuantize(
        input,
        precision,
