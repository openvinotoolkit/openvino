...
std::shared_ptr<ov::opset1::FakeQuantize> makeFakeQuantize(
    ...
    return ov::as_type_ptr<ov::opset1::FakeQuantize>(ov::builder::makeFakeQuantize(
        output,
        constantType,
...
std::shared_ptr<ov::opset1::FakeQuantize> makeFakeQuantize(
    ...
    if (subgraphOnConstantPath) {
        const auto topConstant = ov::builder::makeConstant(constantPrecision, ov::Shape{1}, std::vector<float>(1, 0.f), false);
        ...
        const auto subtractMin = std::make_shared<opset1::Subtract>(
            std::make_shared<opset1::Constant>(constantPrecision, ov::Shape{ 1 }, std::vector<float>{fqOnData.outputLowValues[0]}),
            convert);
        const auto subtractMax = std::make_shared<opset1::Subtract>(
            std::make_shared<opset1::Constant>(constantPrecision, ov::Shape{ 1 }, std::vector<float>{fqOnData.outputHighValues[0]}),
            convert);
        ...
        inputLowNode = ov::builder::makeConstant(
            constantPrecision,
            fqOnData.constantShapes.empty() ? ov::Shape{} : fqOnData.constantShapes[0],
            fqOnData.inputLowValues,
            fqOnData.inputLowValues.empty());
        if (fqOnData.addConverts) {
            inputLowNode = ov::builder::makeConversion(inputLowNode, ov::element::f32, ov::helpers::ConversionTypes::CONVERT);
        }

        inputHighNode = ov::builder::makeConstant(
            constantPrecision,
        ...
        if (fqOnData.addConverts) {
            inputHighNode = ov::builder::makeConversion(inputHighNode, ov::element::f32, ov::helpers::ConversionTypes::CONVERT);
        }
    }

    auto outputLowNode = ov::builder::makeConstant(
        constantPrecision,
        fqOnData.constantShapes.empty() ?
            ov::Shape{} :
    ...
    if (fqOnData.addConverts) {
        outputLowNode = ov::builder::makeConversion(outputLowNode, ov::element::f32, ov::helpers::ConversionTypes::CONVERT);
    }

    auto outputHighNode = ov::builder::makeConstant(
        constantPrecision,
        fqOnData.constantShapes.empty() ?
            ov::Shape{} :
    ...
    if (fqOnData.addConverts) {
        outputHighNode = ov::builder::makeConversion(outputHighNode, ov::element::f32, ov::helpers::ConversionTypes::CONVERT);
    }
...
std::shared_ptr<Node> makeConvolution(
    ...
    if (weightsWithoutFQ) {
        weights = std::make_shared<opset1::Constant>(weightsprecision, shape, std::vector<int>(ov::shape_size(shape), 100));
    } else {
        weights = ov::builder::makeFakeQuantize(
            std::make_shared<opset1::Constant>(precision, shape, std::vector<float>(ov::shape_size(shape), 1.f)),
            precision,
            255,
