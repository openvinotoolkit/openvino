...
std::shared_ptr<ov::Model> TransposeSoftmaxFunction::initOriginal() const {
    ...
    const auto transpose0Const = ov::builder::makeConstant(ov::element::i64, ov::Shape{m_order.size()}, m_order);
    const auto transpose2 = std::make_shared<ov::op::v1::Transpose>(transpose0Param, transpose0Const);
    const auto softMax = std::make_shared<ov::opset8::Softmax>(transpose2, m_axis);
...
std::shared_ptr<ov::Model> TransposeSoftmaxEltwiseFunction::initOriginal() const {
    const auto transpose0Param = std::make_shared<ov::opset1::Parameter>(precision, input_shapes[0]);
    const auto transpose0Const = ov::builder::makeConstant(ov::element::i64, ov::Shape{m_order.size()},
                                                               m_order);
    const auto transpose2 = std::make_shared<ov::op::v1::Transpose>(transpose0Param, transpose0Const);
    const auto mulConst = ov::builder::makeConstant(ov::element::f32, transpose2->get_shape(),
                                                        std::vector<float>{}, true);
    const auto mul = std::make_shared<ov::opset1::Multiply>(transpose2, mulConst);
    const auto softMax = std::make_shared<ov::opset8::Softmax>(mul, m_axis);
    const auto hswish = std::make_shared<ov::opset6::HSwish>(softMax);
