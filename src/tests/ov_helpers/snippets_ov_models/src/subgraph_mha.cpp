...
std::shared_ptr<ov::Model> MHAFunction::initOriginal() const {
    ...
    auto transpose0Const = ov::builder::makeConstant(ov::element::i64, constantShapes[0], std::vector<int64_t>{0, 2, 1, 3});
    auto transpose1Const = ov::builder::makeConstant(ov::element::i64, constantShapes[1], std::vector<int64_t>{0, 2, 3, 1});
    auto transpose2Const = ov::builder::makeConstant(ov::element::i64, constantShapes[5], std::vector<int64_t>{0, 2, 1, 3});
    auto transpose3Const = ov::builder::makeConstant(ov::element::i64, constantShapes[6], std::vector<int64_t>{0, 2, 1, 3});
    ...
    std::vector<int64_t> reshape0ConstData = {static_cast<int64_t>(input_shapes[0].get_shape()[0] *
                                              ...
                                              -1};
    auto reshape0Const = ov::builder::makeConstant(ov::element::i64, constantShapes[3], reshape0ConstData);
    ...
    std::vector<int64_t> reshape1ConstData = {static_cast<int64_t>(input_shapes[0].get_shape()[0]),
                                              ...
                                              static_cast<int64_t>(input_shapes[0].get_shape()[1])};
    auto reshape1Const = ov::builder::makeConstant(ov::element::i64, constantShapes[4], reshape1ConstData);
    ...
    if (with_mul) {
        ...
        auto mulConst = ov::builder::makeConstant(precisions[1], constantShapes[2], mulConstData, true);
        matmul_parent1 = std::make_shared<ov::opset3::Multiply>(transpose1, mulConst);
    }
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(transpose0, matmul_parent1, transA, transB);
    const auto add = std::make_shared<ov::opset3::Add>(matMul0, addParam);
    ...
    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(reshape1, transpose2, transA, transB);
...
std::shared_ptr<ov::Model> MHAFunction::initReference() const {
    ...
    if (with_mul) {
        std::vector<float> mulConstData(ov::shape_size(constantShapes[2]));
        auto mulConst = ov::builder::makeConstant(precisions[1], constantShapes[2], mulConstData, true);
        auto mulParam = std::make_shared<ov::opset1::Parameter>(precisions[1], mulConst->get_shape());
        matmul_parent1 = std::make_shared<ov::opset3::Multiply>(transpose1, mulParam);
...
std::shared_ptr<ov::Model> MHAMatMul0TransposeFunction::initOriginal() const {
    ...
    auto transpose0Const = ov::builder::makeConstant(ov::element::i64, constantShapes[0], order);
    auto transpose1Const = ov::builder::makeConstant(ov::element::i64, constantShapes[1], order);
    auto transpose2Const = ov::builder::makeConstant(ov::element::i64, constantShapes[5], order);
    auto transpose3Const = ov::builder::makeConstant(ov::element::i64, constantShapes[6], order);

    std::vector<float> mulConstData(1);
    auto mulConst = ov::builder::makeConstant(precisions[1], ov::Shape{1}, mulConstData, true);
    ...
    const auto transpose0 = std::make_shared<ov::op::v1::Transpose>(transpose0Param, transpose0Const);
    const auto transpose1 = std::make_shared<ov::op::v1::Transpose>(transpose1Param, transpose1Const);
    const auto mul = std::make_shared<ov::opset3::Multiply>(transpose1, mulConst);
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(transpose0, mul, transA, true);
    const auto add = std::make_shared<ov::opset3::Add>(matMul0, addParam);
    const auto reshape0 = std::make_shared<ov::opset1::Reshape>(add, reshape0Const, true);
    const auto softMax = std::make_shared<ov::opset1::Softmax>(reshape0, 1);
    const auto reshape1 = std::make_shared<ov::opset1::Reshape>(softMax, reshape1Const, true);
...
std::shared_ptr<ov::Model> MHAMatMul0TransposeFunction::initReference() const {
    ...
    const auto transpose1 = std::make_shared<ov::op::v1::Transpose>(transpose1Param, transpose1Const);
    const auto mul = std::make_shared<ov::opset3::Multiply>(transpose1, mulConst);
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(transpose0, mul, transA, transB);
    const auto add = std::make_shared<ov::opset3::Add>(matMul0, addParam);
    const auto reshape0 = std::make_shared<ov::opset1::Reshape>(add, reshape0Const, true);
    const auto softMax = std::make_shared<ov::opset1::Softmax>(reshape0, 1);
    const auto reshape1 = std::make_shared<ov::opset1::Reshape>(softMax, reshape1Const, true);
...
std::shared_ptr<ov::Model> MHASelectFunction::initOriginal() const {
    ...
    auto transpose0Const = ov::builder::makeConstant(ov::element::i64, constantShapes[0],
                                                         std::vector<int64_t>{0, 2, 1, 3});
    auto transpose1Const = ov::builder::makeConstant(ov::element::i64, constantShapes[1],
                                                         std::vector<int64_t>{0, 2, 3, 1});
    auto transpose2Const = ov::builder::makeConstant(ov::element::i64, constantShapes[5],
                                                         std::vector<int64_t>{0, 2, 1, 3});
    auto transpose3Const = ov::builder::makeConstant(ov::element::i64, constantShapes[6],
                                                         std::vector<int64_t>{0, 2, 1, 3});
    ...
    auto selectConst = ov::builder::makeConstant(precisions[2], ov::Shape{1}, std::vector<float>{1});

    float transA = false;
    float transB = false;
    const auto transpose0 = std::make_shared<ov::op::v1::Transpose>(transpose0Param, transpose0Const);
    const auto transpose1 = std::make_shared<ov::op::v1::Transpose>(transpose1Param, transpose1Const);
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(transpose0, transpose1, transA, transB);
    const auto add = std::make_shared<ov::opset3::Add>(matMul0, addParam);
    const auto less = std::make_shared<ov::opset3::Less>(less0Param, less1Param);
    std::shared_ptr<ov::Node> selectCond = less;
    if (add->get_output_partial_shape(0) != input_shapes[3]) {
        const auto broadcast_shape = ov::builder::makeConstant(ov::element::i64, constantShapes[5],
                                                                   add->get_output_shape(0));
        const auto broadcast = ov::builder::makeBroadcast(selectCond, broadcast_shape,
...
std::shared_ptr<ov::Model> MHASelectSplitMFunction::initOriginal() const {
    ...
    // Value is equal to '1' - to avoid situation e^(-1000) / (sum(e^(-1000)) = 0/0 = NAN
    auto selectConst = ov::builder::makeConstant(precision, ov::Shape{1}, std::vector<float>{1});

    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(transpose0Param, transpose1Param);
    const auto add = std::make_shared<ov::opset3::Add>(matMul0, addParam);
    ...
    if (add->get_output_partial_shape(0) != selectParam->get_output_partial_shape(0)) {
        const auto broadcast_shape =
                ov::builder::makeConstant(ov::element::i64, ov::Shape{add->get_output_shape(0).size()}, add->get_output_shape(0));
    ...
    auto reshape0Const = ov::builder::makeConstant(ov::element::i64, ov::Shape{reshape0ConstData.size()}, reshape0ConstData);

    std::vector<int64_t> reshape1ConstData;
    for (const auto& dim : interm_shape)
        reshape1ConstData.push_back(static_cast<int64_t>(dim));
    auto reshape1Const = ov::builder::makeConstant(ov::element::i64, ov::Shape{reshape1ConstData.size()}, reshape1ConstData);

    const auto reshape0 = std::make_shared<ov::opset1::Reshape>(select, reshape0Const, true);
    const auto softMax = std::make_shared<ov::opset1::Softmax>(reshape0, 1);
    const auto reshape1 = std::make_shared<ov::opset1::Reshape>(softMax, reshape1Const, true);
    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(reshape1, transpose2Param);
...
std::shared_ptr<ov::Model> MHASelectSplitMFunction::initReference() const {
    ...
    auto make_reshape = [](const std::shared_ptr<ov::Node>& node, const ov::Shape& new_shape) {
        auto shape_const = ov::builder::makeConstant(ov::element::i32, {new_shape.size()}, new_shape);
    ...
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(data0, data1);
    const auto add = std::make_shared<ov::opset3::Add>(matMul0, dataAdd);
    ...
    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(softMax, data2);
...
std::shared_ptr<ov::Model> MHAWOTransposeOnInputsFunction::initOriginal() const {
    ...
    auto transpose3Const = ov::builder::makeConstant(ov::element::i64, ov::Shape({4}), std::vector<int64_t>{0, 2, 1, 3});

    float transA = false;
    float transB = false;
    const auto mulConst = ov::builder::makeConstant(precision, ov::Shape({1}), std::vector<float>{1}, true);
    const auto mul = std::make_shared<ov::opset3::Multiply>(param1, mulConst);
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(param0, mul, transA, transB);
    const auto softmax = std::make_shared<ov::opset8::Softmax>(matMul0, -1);
    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(softmax, param2, transA, transB);
    const auto transpose3 = std::make_shared<ov::op::v1::Transpose>(matMul1, transpose3Const);
...
std::shared_ptr<ov::Model> MHAWOTransposeFunction::initOriginal() const {
    ...
    float transA = false;
    float transB = false;
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(param0, param1, transA, transB);
    const auto softmax = std::make_shared<ov::opset8::Softmax>(matMul0, -1);
    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(softmax, param2, transA, transB);
...
std::shared_ptr<ov::Model> MHAFQAfterMatMulFunction::initOriginal() const {
    ...
    auto transpose0Const = ov::builder::makeConstant(ov::element::i64, {shape_rank}, std::vector<int64_t>{0, 2, 1, 3});
    auto transpose1Const = ov::builder::makeConstant(ov::element::i64, {shape_rank}, std::vector<int64_t>{0, 2, 3, 1});
    auto transpose2Const = ov::builder::makeConstant(ov::element::i64, {shape_rank}, std::vector<int64_t>{0, 2, 1, 3});
    auto transpose3Const = ov::builder::makeConstant(ov::element::i64, {shape_rank}, std::vector<int64_t>{0, 2, 1, 3});

    std::vector<int64_t> reshape0ConstData = {static_cast<int64_t>(input_shapes[0].get_shape()[0] *
                                                                   input_shapes[0].get_shape()[1] * input_shapes[0].get_shape()[2]),
                                              -1};
    auto reshape0Const = ov::builder::makeConstant(ov::element::i64, {reshape0ConstData.size()}, reshape0ConstData);

    std::vector<int64_t> reshape1ConstData = {static_cast<int64_t>(input_shapes[0].get_shape()[0]),
                                              static_cast<int64_t>(input_shapes[0].get_shape()[2]),
                                              static_cast<int64_t>(input_shapes[0].get_shape()[1]),
                                              static_cast<int64_t>(input_shapes[0].get_shape()[1])};
    auto reshape1Const = ov::builder::makeConstant(ov::element::i64, {reshape1ConstData.size()}, reshape1ConstData);
    ...
    const auto transpose1 = std::make_shared<ov::op::v1::Transpose>(transpose1Param, transpose1Const);
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(transpose0, transpose1, transA, transB);
    auto fq0 = ov::builder::makeFakeQuantize(matMul0, ov::element::f32, 256, {1},
                                                 {-35.0172004}, {34.7436294}, {-35.0172004}, {34.7436294});
    const auto add = std::make_shared<ov::opset3::Add>(fq0, addParam);
    const auto reshape0 = std::make_shared<ov::opset1::Reshape>(add, reshape0Const, true);
    const auto softMax = std::make_shared<ov::opset1::Softmax>(reshape0, 1);
    const auto reshape1 = std::make_shared<ov::opset1::Reshape>(softMax, reshape1Const, true);
    const auto transpose2 = std::make_shared<ov::op::v1::Transpose>(transpose2Param, transpose2Const);
    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(reshape1, transpose2, transA, transB);
    auto fq1 = ov::builder::makeFakeQuantize(matMul1, ov::element::f32, 256, {1},
                                                 {-35.0172004}, {34.7436294}, {-35.0172004}, {34.7436294});
...
std::shared_ptr<ov::Model> MHAINT8MatMulFunction::initOriginal() const {
    ...
    auto fq0 = ov::builder::makeFakeQuantize(transpose0Param, ov::element::f32, 256, {1},
                                                 {-35.0172004}, {34.7436294}, {-35.0172004}, {34.7436294});
    auto fq1 = ov::builder::makeFakeQuantize(transpose1Param, ov::element::f32, 256, {1},
                                                 {-35.0172004}, {34.7436294}, {-35.0172004}, {34.7436294});
    auto fq2 = ov::builder::makeFakeQuantize(transpose2Param, ov::element::f32, 256, {1},
                                                 {-35.0172004}, {34.7436294}, {-35.0172004}, {34.7436294});
    float transA = false;
    float transB = false;
    ...
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(transpose0, transpose1, transA, transB);
    auto fq3 = ov::builder::makeFakeQuantize(matMul0, ov::element::f32, 256, {1},
                                                 {-35.0172004}, {34.7436294}, {-35.0172004}, {34.7436294});
    const auto add = std::make_shared<ov::opset3::Add>(fq3, addParam);
    const auto reshape0 = std::make_shared<ov::opset1::Reshape>(add, reshape0Const, true);
    const auto softMax = std::make_shared<ov::opset1::Softmax>(reshape0, 1);
    const auto reshape1 = std::make_shared<ov::opset1::Reshape>(softMax, reshape1Const, true);
    auto fq4 = ov::builder::makeFakeQuantize(reshape1, ov::element::f32, 256, {1},
                                                 {0}, {0.820726}, {0}, {0.820726});
    const auto transpose2 = std::make_shared<ov::op::v1::Transpose>(fq2, transpose2Const);
    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(fq4, transpose2, transA, transB);
    auto fq5 = ov::builder::makeFakeQuantize(matMul1, ov::element::f32, 256, {1},
                                                 {-35.0172004}, {34.7436294}, {-35.0172004}, {34.7436294});
...
std::shared_ptr<ov::Model> MHAQuantMatMul0Function::initOriginal() const {
    ...
    auto reshape0Const = ov::builder::makeConstant(ov::element::i64, {new_shape.size()}, new_shape);
    auto reshape1Const = ov::builder::makeConstant(ov::element::i64, {new_shape.size()}, new_shape);
    auto reshape2Const = ov::builder::makeConstant(ov::element::i64, {new_shape.size()}, new_shape);
    auto reshape3Const = ov::builder::makeConstant(ov::element::i64, {input_shapes[0].size()}, std::vector<int64_t>{0, 0, -1});

    auto transpose0Const = ov::builder::makeConstant(ov::element::i64, {4}, std::vector<int64_t>{0, 2, 1, 3});
    auto transpose1Const = ov::builder::makeConstant(ov::element::i64, {4}, std::vector<int64_t>{0, 2, 3, 1});
    auto transpose2Const = ov::builder::makeConstant(ov::element::i64, {4}, std::vector<int64_t>{0, 2, 1, 3});
    auto transpose3Const = ov::builder::makeConstant(ov::element::i64, {4}, std::vector<int64_t>{0, 2, 1, 3});
    ...
    auto fq1 = ov::builder::makeFakeQuantize(transpose1, ov::element::f32, 256, {1},
    ...
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(transpose0, fq1);
    const auto add = std::make_shared<ov::opset3::Add>(matMul0, addParam);
    const auto softMax = std::make_shared<ov::opset8::Softmax>(add, -1);

    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(softMax, transpose2);
    auto fq2 = ov::builder::makeFakeQuantize(matMul1, ov::element::f32, 256, {1},
...
std::shared_ptr<ov::Model> MHAFQFunction::initOriginal() const {
    ...
    const auto fq0 = ov::builder::makeFakeQuantize(transpose0Param, ov::element::f32, 256, {1},
                                                       {-5.217694}, {6.661877}, {-5.217694}, {6.661877});
    const auto fq1 = ov::builder::makeFakeQuantize(transpose1Param, ov::element::f32, 256, {1},
                                                       {-6.40245}, {6.45286}, {-6.40245}, {6.45286});
    const auto fq_add = ov::builder::makeFakeQuantize(addParam, ov::element::f32, 256, {1},
                                                          {-1000}, {0}, {-1000}, {0});
    ...
    const auto transpose1 = std::make_shared<ov::op::v1::Transpose>(fq1, transpose1Const);
    const auto transpose2 = std::make_shared<ov::op::v1::Transpose>(transpose2Param, transpose2Const);
    const auto mul_const = ov::builder::makeConstant(ov::element::i8, ov::Shape{1}, std::vector<int8_t>{127});
    const auto convert = std::make_shared<ov::opset1::Convert>(mul_const, ov::element::f32);
    const auto mul_deq_const = ov::builder::makeConstant(ov::element::f32, ov::Shape{1}, std::vector<float>{0.00098425});
    ...
    auto fq1_1 = ov::builder::makeFakeQuantize(mul, ov::element::f32, 256, {1},
                                                   {-0.8003067}, {0.8066083}, {-0.8003067}, {0.8066083});
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(transpose0, fq1_1, transA, transB);
    auto fq2 = ov::builder::makeFakeQuantize(matMul0, ov::element::f32, 256, {1},
    ...
    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(softMax, transpose2, transA, transB);
    auto fq3 = ov::builder::makeFakeQuantize(matMul1, ov::element::f32, 256, {1},
...
std::shared_ptr<ov::Model> MHAINT8MatMulTypeRelaxedFunction::initOriginal() const {
    ...
    const auto add = std::make_shared<op::TypeRelaxed<ov::opset3::Add>>(
            std::vector<element::Type>{ element::f32, element::f32 },
            std::vector<element::Type>{ element::f32 },
    ...
    const auto deq_mul = std::make_shared<op::TypeRelaxed<ov::opset3::Multiply>>(
            std::vector<element::Type>{ element::f32, element::f32 },
            std::vector<element::Type>{ element::f32 },
            ov::op::TemporaryReplaceOutputType(add, element::f32).get(),
...
std::shared_ptr<ov::Model> MHAMulAddFunction::initOriginal() const {
    ...
    auto transpose0Const = ov::builder::makeConstant(ov::element::i64, ov::Shape{input_shapes[0].size()}, std::vector<int64_t>{0, 2, 1, 3});
    auto transpose1Const = ov::builder::makeConstant(ov::element::i64, ov::Shape{input_shapes[1].size()}, std::vector<int64_t>{0, 2, 3, 1});
    auto transpose2Const = ov::builder::makeConstant(ov::element::i64, ov::Shape{input_shapes[2].size()}, std::vector<int64_t>{0, 2, 1, 3});
    auto transpose3Const = ov::builder::makeConstant(ov::element::i64, ov::Shape{input_shapes[2].size()}, std::vector<int64_t>{0, 2, 1, 3});
    ...
    const auto transpose1 = std::make_shared<ov::op::v1::Transpose>(transpose1Param, transpose1Const);
    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(transpose0, transpose1, transA, transB);
    auto mulConst = ov::builder::makeConstant(ov::element::f32, matMul0->get_shape(), std::vector<float>{}, true);
    auto addConst = ov::builder::makeConstant(ov::element::f32, matMul0->get_shape(), std::vector<float>{}, true);
    const auto mul = std::make_shared<ov::opset3::Multiply>(matMul0, mulConst);
    const auto add = std::make_shared<ov::opset3::Add>(mul, addConst);
...
std::shared_ptr<ov::Model> MHATransposedInputFunction::initOriginal() const {
    ...
    if (!m_order.empty()) {
        const auto transposeConst = ov::builder::makeConstant(ov::element::i64, ov::Shape{m_order.size()}, m_order);
        matmul0_in1 = std::make_shared<ov::op::v1::Transpose>(param1, transposeConst);
    }

    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(param0, matmul0_in1, false, m_transposed_b);
    const auto softmax = std::make_shared<ov::opset8::Softmax>(matMul0, -1);
    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(softmax, param2);
...
std::shared_ptr<ov::Model> MHATransposedInputFunction::initReference() const {
    ...
    if (m_transposed_b) {
        if (m_order != std::vector<int64_t>{0, 2, 1, 3}) {
            ...
            const auto transposeConst = ov::builder::makeConstant(ov::element::i32, ov::Shape{transpose_order.size()}, transpose_order);
            in1 = std::make_shared<ov::op::v1::Transpose>(in1, transposeConst);
        }
    }
    ...
    std::shared_ptr<ov::Node> matmul0_in1 = param1;
    if (!m_order.empty() && is_supported) {
        const auto transposeConst = ov::builder::makeConstant(ov::element::i32, ov::Shape{m_order.size()}, m_order);
        matmul0_in1 = std::make_shared<ov::op::v1::Transpose>(param1, transposeConst);
    }

    const auto matMul0 = std::make_shared<ov::opset3::MatMul>(param0, matmul0_in1);
    const auto softmax = std::make_shared<ov::opset8::Softmax>(matMul0, -1);
    const auto matMul1 = std::make_shared<ov::opset3::MatMul>(softmax, param2);
...
std::shared_ptr<ov::Model> MHAWithExtractedReshapeFunction::initReference() const {
    ...
    const auto matmul_0 = std::make_shared<ov::opset3::MatMul>(param_0, param_1);
    ...
    const auto matmul_1 = std::make_shared<ov::opset3::MatMul>(softmax, param_3);
