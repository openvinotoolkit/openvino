...
DequantizationOperations::Convert::Convert() : isEmpty(true), outPrecision(ov::element::undefined) {}
...
bool DequantizationOperations::Convert::equal(const DequantizationOperations::Convert& value) const noexcept {
    return (this->outPrecision == value.outPrecision) &&
           (this->addDequantizationAttribute == value.addDequantizationAttribute);
}
...
DequantizationOperations::Subtract::Subtract(const std::vector<float>& values, const ov::element::Type outPrecision)
    : isEmpty(false),
      values(values),
      outPrecision(outPrecision),
      constantShapeIsDefined(false) {}
...
bool DequantizationOperations::Subtract::equal(const DequantizationOperations::Subtract& value) const noexcept {
    return (values == value.values) && (outPrecision == value.outPrecision) && (constantShape == value.constantShape) &&
           (constantShapeIsDefined == value.constantShapeIsDefined) && (constantIndex == value.constantIndex);
}

DequantizationOperations::Subtract& DequantizationOperations::Subtract::setConstantPrecision(
...
bool DequantizationOperations::Multiply::equal(const DequantizationOperations::Multiply& value) const noexcept {
    return (values == value.values) && (outPrecision == value.outPrecision) && (constantShape == value.constantShape) &&
           (constantIndex == value.constantIndex) && (constantPrecision == value.constantPrecision) &&
...
bool DequantizationOperations::equal(const DequantizationOperations& value) const noexcept {
    return (empty() == value.empty()) && (convert == value.convert) && (subtract == value.subtract) &&
           (multiply == value.multiply);
}

DequantizationOperations::Multiply& DequantizationOperations::Multiply::setConstantPrecision(
    const ov::element::Type& precision) {
    constantPrecision = precision;
    return *this;
}
...
DequantizationOperations::DequantizationOperations(const Convert& convert,
