...
Constant::Constant() : isEmpty(true), outPrecision(ov::element::undefined), shapeIsDefined(false) {}
...
Constant::Constant(const std::vector<float>& values, const ov::element::Type outPrecision, const ov::Shape& shape)
    : isEmpty(false),
      values(values),
      outPrecision(outPrecision),
      shape(shape),
