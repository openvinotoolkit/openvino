...
std::shared_ptr<Node> makeReshape(const Output<Node>& data, const Reshape& reshape) {
    auto constant = ov::test::utils::make_constant(ov::element::i64, Shape({ reshape.values.size() }), reshape.values);
...
std::shared_ptr<Node> makeTranspose(const Output<Node>& data, const Transpose& transpose) {
    auto constant = ov::test::utils::make_constant(ov::element::i64, Shape({ transpose.values.size() }), transpose.values);
...
std::shared_ptr<ov::opset1::FakeQuantize> makeFakeQuantize(
    ...
    if (subgraphOnConstantPath) {
        const auto topConstant = ov::test::utils::make_constant(constantPrecision, ov::Shape{1}, std::vector<float>(1, 0.f), false);
