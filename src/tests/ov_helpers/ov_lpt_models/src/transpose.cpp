...
std::shared_ptr<ov::Model> TransposeFunction::getOriginal(const ov::PartialShape& inputShape,
                                                          const std::vector<int>& transposeConstValues,
...
std::shared_ptr<ov::Model> TransposeFunction::getReference(
    ...
    const std::shared_ptr<ov::opset1::Constant> transposeConstant =
        std::make_shared<ov::opset1::Constant>(ov::element::i64,
                                               ov::Shape{transposeConstValues.size()},
                                               transposeConstValues);
    const std::shared_ptr<ov::opset1::Transpose> transpose =
        std::make_shared<ov::opset1::Transpose>(quantizationOpBefore, transposeConstant);
    ...
    if (transpose->get_output_element_type(0) != precisionAfterOperation) {
        THROW_IE_LPT_EXCEPTION(*transpose)
            << "unexpected precision '" << precisionAfterOperation << "' after operation";
    }
