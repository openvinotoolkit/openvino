...
std::shared_ptr<ov::Model> FakeQuantizePrecisionSelectionFunction::getOriginal(const ov::element::Type precision,
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{branch1Last->output(0), branch2Last->output(0)}, 1);
...
std::shared_ptr<ov::Model> FakeQuantizePrecisionSelectionFunction::getReference(const ov::element::Type precision,
    ...
    const auto fakeQuantize =
        ov::builder::subgraph::makeFakeQuantizeTypeRelaxed(input, precision, values.fakeQuantizeOnData);
    fakeQuantize->set_friendly_name("fakeQuantize");
    ...
    if (values.fakeQuantizeOnDataOutPrecision != precision) {
        ov::pass::low_precision::NetworkHelper::setOutDataPrecision(fakeQuantize,
                                                                    values.fakeQuantizeOnDataOutPrecision);
        ...
        if (values.operationBeforeLimitedOperationIsPrecisionTransparent) {
            ...
            if (intermediateOpTr != nullptr) {
                ...
                // openvino\inference-engine\tests\ov_models\src\low_precision_transformations\concat_function.cpp, line
                // #496
                branch1Pooling->set_output_type(0,
                                                values.fakeQuantizeOnDataOutPrecision,
                                                branch1Pooling->get_output_partial_shape(0));
            }
        }

        if (values.fakeQuantizeOnWeights.empty()) {
            replace_node(weights, ov::pass::low_precision::fold<ov::opset1::Convert>(weights, ov::element::i8));
        }
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{branch1Multiply->output(0), branch2Multiply->output(0)},
                                             1);
    concat->set_friendly_name("output");
