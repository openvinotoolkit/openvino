...
std::shared_ptr<ov::Model> FuseConvertFunction::get(
    ...
    if (constInput) {
        parent =
            std::make_shared<ov::opset1::Constant>(inputPrecision, inputShape.to_shape(), std::vector<float>{128.f});
    } else {
        input = std::make_shared<ov::opset1::Parameter>(inputPrecision, inputShape);
        parent = input;
    }
    ...
    auto parameters = constInput ? ov::ParameterVector{} : ov::ParameterVector{input};
...
std::shared_ptr<ov::Model> FuseConvertFunction::getWithFQ(
    ...
    if (constInput) {
        ...
        input1 = std::make_shared<ov::opset1::Parameter>(inputPrecision, inputShape);
        parent = input1;
    }
    ...
    const auto fakeQuantizeOnActivations =
        ov::test::utils::make_fake_quantize(input2, inputPrecision, 256ul, {1ul}, {0.f}, {255.f}, {0.f}, {255.f});
    ...
    const auto add = std::make_shared<ov::opset1::Add>(dequantizationOp, power);

    add->set_friendly_name("output");

    auto parameters = constInput ? ov::ParameterVector{input2} : ov::ParameterVector{input1, input2};
