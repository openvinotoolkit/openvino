...
std::shared_ptr<ov::Model> FuseFakeQuantizeFunction::getOriginal(const ov::PartialShape& inputShape,
    ...
    const auto input =
        std::make_shared<ov::opset1::Parameter>(add.empty() ? precisionBeforeDequantization : precisionBeforeAdd,
                                                inputShape);
    ...
    const std::shared_ptr<Node> fakeQuantize =
        precisionAfterDequantization == precisionFqOnData
...
namespace {
std::shared_ptr<ov::opset1::Convolution> make_convolution(const ov::PartialShape& inputShape,
    ...
    auto weightsConstant =
        std::make_shared<ov::op::v0::Constant>(ov::element::f32, weightsShape, std::vector<float>(9, 1.f));
    auto weights = makeFakeQuantize(weightsConstant,
                                    precisionBefore,
                                    FakeQuantizeOnData(255,
    ...
    auto convolution = std::make_shared<ov::opset1::Convolution>(parent,
                                                                 weights,
                                                                 ov::Strides{1, 1},
...
std::shared_ptr<ov::Model> FuseFakeQuantizeFunction::getReference(const ov::PartialShape& inputShape,
                                                                  const ov::element::Type precisionBeforeAdd,
                                                                  const Add& add,
                                                                  const ov::element::Type precisionBeforeDequantization,
                                                                  const DequantizationOperations& dequantization,
...
std::shared_ptr<ov::Model> FuseFakeQuantizeFunction::get(const ov::PartialShape& inputShape,
                                                         const ov::element::Type precisionBefore,
    ...
    parent = std::make_shared<ov::opset1::MaxPool>(parent, stride, padBegin, padEnd, kernel, roundingType, padType);
...
std::shared_ptr<ov::Model> FuseFakeQuantizeFunction::get(const ov::PartialShape& inputShape,
    ...
    std::shared_ptr<ov::opset1::Multiply> multiply =
        std::make_shared<ov::opset1::Multiply>(lastDequantizations[0], lastDequantizations[1]);
