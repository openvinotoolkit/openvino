...
std::shared_ptr<ov::Model> ClampFunction::getOriginal(const ov::element::Type precision,
                                                      const ov::PartialShape& inputShape,
    ...
    const std::shared_ptr<Node> fq = fakeQuantize.empty()
                                         ? nullptr
                                         : ov::test::utils::make_fake_quantize(input,
                                                                               precision,
    ...
    const std::shared_ptr<ov::opset1::Clamp> clamp =
        std::make_shared<ov::opset1::Clamp>(fakeQuantize.empty() ? input : fq, clampLowConst, clampHighConst);

    ov::ResultVector results{std::make_shared<ov::opset1::Result>(clamp)};
    return std::make_shared<ov::Model>(results, ov::ParameterVector{input}, "ClampFunction");
}

std::shared_ptr<ov::Model> ClampFunction::getWithNonDequantizationMultiply(const ov::PartialShape& inputShape,
...
std::shared_ptr<ov::Model> ClampFunction::getReference(
    ...
    std::shared_ptr<ov::opset1::Clamp> clamp =
        std::make_shared<ov::op::TypeRelaxed<ov::opset1::Clamp>>(quantizationOpBefore, 0, 10);
