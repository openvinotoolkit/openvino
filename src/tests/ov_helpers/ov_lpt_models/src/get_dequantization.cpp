...
std::shared_ptr<ov::Model> GetDequantizationFunction::get(
    ...
    const std::shared_ptr<ov::Node> input = std::make_shared<ov::opset1::Parameter>(ov::element::f32, shape);
    ...
    return std::make_shared<ov::Model>(ov::ResultVector{std::make_shared<ov::opset1::Result>(parent)},
...
std::shared_ptr<ov::Model> GetDequantizationFunction::get(
    ...
    if (dequantization.subtract != nullptr) {
        ...
        parent = dequantization.subtract->clone_with_new_inputs(index == 1ul ? OutputVector{parent, parent2}
                                                                             : OutputVector{parent2, parent});
        parent->set_friendly_name(dequantization.subtract->get_friendly_name());
    }

    if (dequantization.multiply != nullptr) {
        const auto index = ov::pass::low_precision::NetworkHelper::getChildInputIndex(dequantization.multiplyConstant,
                                                                                      dequantization.multiply);
        parent = dequantization.multiply->clone_with_new_inputs(
            index == 1ul ? OutputVector{parent, dequantization.multiplyConstant}
...
std::shared_ptr<ov::Model> GetDequantizationFunction::getOriginal(bool isConvert,
                                                                  bool isSubtract,
                                                                  size_t subDataInput,
                                                                  size_t mulDataInput) {
    const std::shared_ptr<ov::Node> input =
        std::make_shared<ov::opset1::Parameter>(ov::element::f32, ov::Shape{1, 3, 10, 10});
    ...
    return std::make_shared<ov::Model>(ov::ResultVector{std::make_shared<ov::opset1::Result>(multiply)},
                                       ov::ParameterVector{ov::as_type_ptr<op::v0::Parameter>(input)},
                                       "Dequantization");
}
