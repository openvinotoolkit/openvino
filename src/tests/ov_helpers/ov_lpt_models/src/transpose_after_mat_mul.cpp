...
namespace builder {
namespace subgraph {
std::shared_ptr<ov::Model> TransposeAfterMatMulFunction::getOriginal(const ov::element::Type precision,
    ...
    const float k = 50.f;
    const auto fakeQuantize1 =
        ov::test::utils::make_fake_quantize(input1, precision, 256ul, {1ul}, {0.f}, {255.f / k}, {0.f}, {255.f / k});
    input2->set_friendly_name("fakeQuantize1");
    const auto fakeQuantize2 =
        ov::test::utils::make_fake_quantize(input2, precision, 256ul, {1ul}, {0.f}, {255.f / k}, {0.f}, {255.f / k});
    ...
    std::shared_ptr<ov::Model> function =
        std::make_shared<ov::Model>(results, ov::ParameterVector{input1, input2}, "TransposeAfterMatMulFunction");
