...
std::shared_ptr<ov::opset1::Convolution> createConvolution(const ov::element::Type precision,
...
std::shared_ptr<ov::Model> FakeQuantizeAndTwoOutputBranchesWithConvolutionFunction::getOriginal(
    ...
    const std::shared_ptr<ov::opset1::Convolution> convolution1 =
        createConvolution(precision, inputShape, fakeQuantizeOnActivations, fqOnWeights1, false);

    const std::shared_ptr<ov::opset1::Convolution> convolution2 =
        createConvolution(precision, inputShape, fakeQuantizeOnActivations, fqOnWeights2, false);

    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(NodeVector{convolution1, convolution2}, 1ul);
...
std::shared_ptr<ov::Model> FakeQuantizeAndTwoOutputBranchesWithConvolutionFunction::getReference(
    ...
    const auto fakeQuantizeOnActivations =
        fqOnData.empty() ? nullptr : makeFakeQuantizeTypeRelaxed(input, precision, fqOnData);
    ov::pass::low_precision::NetworkHelper::setOutDataPrecisionForTypeRelaxed(fakeQuantizeOnActivations,
                                                                              precisionBeforeOp);
    const auto deqBefore = makeDequantization(fakeQuantizeOnActivations, dequantizationBefore);

    const std::shared_ptr<ov::opset1::Convolution> convolution1 =
        createConvolution(precision, inputShape, deqBefore, FakeQuantizeOnWeights(), true);
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(NodeVector{deqAfter1, deqAfter2}, 1ul);
    ...
    ov::ResultVector results{std::make_shared<ov::opset1::Result>(concat)};
    auto function = std::make_shared<ov::Model>(results,
                                                ov::ParameterVector{input},
                                                "FakeQuantizeAndTwoOutputBranchesWithConvolutionFunction");
