...
std::shared_ptr<ov::Model> MultiplyPartialFunction::get(const ov::element::Type precision,
    ...
    const auto fakeQuantize1 = fq1.empty() ? nullptr
                                           : ov::test::utils::make_fake_quantize(input1,
                                                                                 precision,
                                                                                 fq1.quantizationLevel,
                                                                                 fq1.constantShape,
                                                                                 fq1.inputLowValues,
                                                                                 fq1.inputHighValues,
                                                                                 fq1.outputLowValues,
                                                                                 fq1.outputHighValues);
    ...
    const auto fakeQuantize2 = fq2.empty() ? nullptr
                                           : ov::test::utils::make_fake_quantize(input2,
                                                                                 precision,
                                                                                 fq2.quantizationLevel,
                                                                                 fq2.constantShape,
                                                                                 fq2.inputLowValues,
                                                                                 fq2.inputHighValues,
                                                                                 fq2.outputLowValues,
                                                                                 fq2.outputHighValues);
    if (fakeQuantize2 != nullptr) {
        fakeQuantize2->set_friendly_name("fakeQuantize2");
    }

    const auto multiply = std::make_shared<ov::opset1::Multiply>(fq1.empty() ? input1 : fakeQuantize1,
                                                                 fq2.empty() ? input2 : fakeQuantize2);
    multiply->set_friendly_name("multiply");

    auto const fakeQuantizeAfter = fqAfter.empty() ? nullptr : makeFakeQuantize(multiply, precision, fqAfter);
    if (fakeQuantizeAfter != nullptr) {
        fakeQuantizeAfter->set_friendly_name("fakeQuantizeAfter");
    }

    const std::shared_ptr<Node> result =
        fakeQuantizeAfter == nullptr ? std::dynamic_pointer_cast<Node>(multiply) : fakeQuantizeAfter;
    ov::ResultVector results{std::make_shared<ov::opset1::Result>(result)};
    std::shared_ptr<ov::Model> function = std::make_shared<ov::Model>(
        results,
        secondInputIsConstant ? ov::ParameterVector{input1}
