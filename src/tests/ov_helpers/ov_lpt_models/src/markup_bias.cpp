...
std::shared_ptr<ov::Model> MarkupBiasFunction::get(const ov::element::Type& precision,
    ...
    if (layer_type == "Convolution") {
        ...
        auto weights = ov::test::utils::make_constant<int8_t>(ov::element::i8, Shape{out_channels, in_channels, 1, 1}, {}, true);
        auto convert = std::make_shared<ov::opset1::Convert>(weights, precision);
        auto mul_const = ov::test::utils::make_constant<float>(precision, Shape{1, 1, 1, 1}, {}, true);
        ...
        const size_t in_channels = input_params[0]->get_partial_shape()[1].get_length();
        auto weights = ov::test::utils::make_constant<int8_t>(ov::element::i8, Shape{in_channels, 1, 1, 1}, {}, true);
        auto convert = std::make_shared<ov::opset1::Convert>(weights, precision);
        auto mul_const = ov::test::utils::make_constant<float>(precision, Shape{1, 1, 1, 1}, {}, true);
        auto mul = std::make_shared<ov::opset1::Multiply>(convert, mul_const);
        ...
        const size_t in_channels = input_params[0]->get_partial_shape()[1].get_length();
        auto weights = ov::test::utils::make_constant<int8_t>(ov::element::i8, Shape{in_channels, out_channels, 1, 1}, {}, true);
        auto convert = std::make_shared<ov::opset1::Convert>(weights, precision);
        auto mul_const = ov::test::utils::make_constant<float>(precision, Shape{1, 1, 1, 1}, {}, true);
        auto mul = std::make_shared<ov::opset1::Multiply>(convert, mul_const);
        ...
        const size_t in_channels = input_params[0]->get_partial_shape()[1].get_length();
        auto weights = ov::test::utils::make_constant<int8_t>(ov::element::i8, Shape{out_channels, in_channels}, {}, true);
        auto convert = std::make_shared<ov::opset1::Convert>(weights, precision);
        auto mul_const = ov::test::utils::make_constant<float>(precision, Shape{out_channels, 1}, {}, true);
        auto mul = std::make_shared<ov::opset1::Multiply>(convert, mul_const);
    ...
    if (extra_multipy) {
        ...
        auto mul_input = ov::test::utils::make_constant<float>(precision, mul_shape, {}, true);
    ...
    if (add_shape.is_static() && add_shape.size() == 0) {
        ...
        add_input1 = ov::test::utils::make_constant<float>(precision, bias_shape, {}, true);
    } else {
        if (add_shape.is_static()) {
            add_input1 = ov::test::utils::make_constant<float>(precision, add_shape.to_shape(), {}, true);
        } else {
            auto new_param = std::make_shared<ov::opset1::Parameter>(precision, input_shape);
            input_params.push_back(new_param);
