...
std::shared_ptr<Node> configure_postops(const std::shared_ptr<Node>& parent,
    ...
    return ov::builder::subgraph::makeFakeQuantizeTypeRelaxed(res,
                                                              precision,
...
std::shared_ptr<ov::Model> AddFunction::getOriginal(
    ...
    if (constInput == 0) {
        parent1 = std::make_shared<ov::opset1::Constant>(precision, inputShape1.to_shape(), constValues);
    } else {
        ...
        if (additionalLayer != "") {
            parent1 =
                ov::builder::subgraph::makeFakeQuantizeTypeRelaxed(input1,
                                                                   precision,
    ...
    const auto dequantizationOp1 =
        dequantization1.empty() ? parent1 : makeDequantization(parent1, dequantizationStructure1);

    std::shared_ptr<ov::Node> input2;
    if (constInput == 1) {
        input2 = std::make_shared<ov::opset1::Constant>(precision, inputShape2.to_shape(), constValues);
    } else {
        input2 = std::make_shared<ov::opset1::Parameter>(precision2.is_real() ? precision : precision2, inputShape2);
    }
    auto parent = input2;
    ...
    const auto dequantizationOp2 =
        dequantization2.empty() ? parent : makeDequantization(parent, dequantizationStructure2);
...
std::shared_ptr<ov::Model> AddFunction::getOriginal(const ov::element::Type precision,
    ...
    const auto add =
        std::make_shared<ov::opset1::Add>(fq1.empty() ? input1 : fakeQuantize1, fq2.empty() ? input2 : fakeQuantize2);
...
std::shared_ptr<ov::Model> AddFunction::getReference(
    ...
    const auto dequantizationOp1 =
        ov::is_type<ov::opset1::Constant>(parent1) ? parent1 : makeDequantization(parent1, dequantizationStructure1);
    ...
    const auto dequantizationOp2 =
        ov::is_type<ov::opset1::Constant>(parent) ? parent : makeDequantization(parent, dequantizationStructure2);
