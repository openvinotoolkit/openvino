...
std::shared_ptr<ov::Model> NormalizeL2Function::getOriginal(const ov::element::Type precision,
                                                            ...
                                                            const bool shift) {
    const float low =
        precisionOnActivation == ov::element::u8 ? (0.f + (shift ? 10.f : 0.f)) : (-128.f + (shift ? 10.f : 0.f));
    ...
    const auto fakeQuantize = ov::test::utils::make_fake_quantize(paramNode->output(0),
                                                                  precision,
                                                                  256,
                                                                  shapes.second,
                                                                  {low / inputScale},
    ...
    const auto normalizeL2 =
        std::make_shared<ov::opset1::NormalizeL2>(fakeQuantize->output(0), axesNode, 1e-6, ov::op::EpsMode::ADD);
    ...
    if (fuseMultiply) {
        ...
        const auto multiplyConst =
            std::make_shared<ov::op::v0::Constant>(precision, constantShape, std::vector<float>{2.f});
    ...
    const auto function =
        std::make_shared<ov::Model>(results, ov::ParameterVector{paramNode}, "NormalizeL2Transformation");
    return function;
}
