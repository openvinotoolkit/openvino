...
std::shared_ptr<ov::Model> ConcatFunction::get(const ov::element::Type inputPrecision,
    ...
    if (precisionAfter != ov::element::undefined &&
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginal(const ov::element::Type precision,
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{parent1->output(0), parent2->output(0)}, 1);
    ...
    std::shared_ptr<ov::Model> function = std::make_shared<ov::Model>(results, inputs, "ConcatTransformation");
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginal(const ov::element::Type precision,
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{fakeQuantize1->output(0), fakeQuantize2->output(0)}, 1);
    concat->set_friendly_name("output");
    ...
    std::shared_ptr<ov::Model> function =
        std::make_shared<ov::Model>(results, ov::ParameterVector{input1, input2}, "ConcatTransformation");

    return function;
}

std::shared_ptr<ov::Model> ConcatFunction::getOriginalWithChildAndOutput(const ov::element::Type precision,
                                                                         const ov::PartialShape& inputShape,
                                                                         const FakeQuantizeOnData& fqOnData1,
                                                                         const FakeQuantizeOnData& fqOnData2) {
    const auto input1 = std::make_shared<ov::opset1::Parameter>(precision, inputShape);
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginalWithNeighbors(const ov::element::Type precision,
                                                                    const ov::PartialShape& inputShape,
                                                                    const FakeQuantizeOnData& fqOnData1,
                                                                    const FakeQuantizeOnData& fqOnData2,
    ...
    const auto concat1 =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{fakeQuantize1->output(0), fakeQuantize2->output(0)},
                                             1ull);
    ...
    if (neighborType == "concat") {
        ...
        const auto concat2 =
            std::make_shared<ov::opset1::Concat>(ov::OutputVector{fakeQuantize2->output(0), fakeQuantize3->output(0)},
                                                 1ull);
    ...
    std::shared_ptr<ov::Model> function =
        std::make_shared<ov::Model>(results, inputs, "ConcatWithNeighborsTransformation");

    return function;
}

std::shared_ptr<ov::Model> ConcatFunction::getOriginalWithIntermediate(const ov::element::Type precision,
                                                                       const ov::PartialShape& inputShape,
    ...
    if (transparentIntermediate) {
        ...
        auto weights = ov::opset1::Constant::create(precision,
                                                    ov::Shape{channels, channels, 1, 1},
                                                    std::vector<float>(channels * channels, 1));

        intermediateOp = std::make_shared<ov::opset1::Convolution>(fakeQuantize2->output(0),
                                                                   weights,
                                                                   ov::Strides{1, 1},
                                                                   ov::CoordinateDiff{0, 0},
                                                                   ov::CoordinateDiff{0, 0},
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{fakeQuantize1->output(0), intermediateOp->output(0)}, 1);
    ...
    auto convolution = std::make_shared<ov::opset1::Convolution>(intermediateOp,
                                                                 weights,
                                                                 ov::Strides{1, 1},
                                                                 ov::CoordinateDiff{0, 0},
                                                                 ov::CoordinateDiff{0, 0},
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginalWithIntermediateAvgPool(const ov::element::Type precision,
                                                                              const ov::PartialShape& inputShape,
                                                                              const FakeQuantizeOnData& fqOnData1,
                                                                              const FakeQuantizeOnData& fqOnData2) {
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginalWithSplitedIntermediate(const ov::element::Type precision,
                                                                              const ov::PartialShape& inputShape,
                                                                              const FakeQuantizeOnData& fqOnData1,
                                                                              const FakeQuantizeOnData& fqOnData2,
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{fakeQuantize1->output(0), intermediateOp->output(0)},
                                             splitedAxis);
        ...
        auto weights =
            ov::opset1::Constant::create(precision,
                                         ov::Shape{static_cast<size_t>(inputShape[1].get_length() / numSplit),
                                                   static_cast<size_t>(inputShape[1].get_length() / numSplit),
                                                   1,
                                                   1},
                                         {1});
        auto convolution = std::make_shared<ov::opset1::Convolution>(intermediateOp->output(1),
                                                                     weights,
                                                                     ov::Strides{1, 1},
                                                                     ov::CoordinateDiff{0, 0},
                                                                     ov::CoordinateDiff{0, 0},
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginalSelectionWithIntermediate(const ov::element::Type precision,
                                                                                const ov::Shape& inputShape,
                                                                                const bool transparentIntermediate,
                                                                                const FakeQuantizeOnData& fqOnData1,
                                                                                const FakeQuantizeOnData& fqOnData2) {
    const std::vector<size_t> inputShape1 = {inputShape[0],
                                             inputShape[1],
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginalWithStridedSlice(const ov::element::Type precision,
                                                                       const ov::PartialShape inputShape,
    ...
    if (ssBeforeConcat) {
        const auto beginParam =
            ov::op::v0::Constant::create(ov::element::i64, ov::Shape{rank}, std::vector<int64_t>(rank, 0));
    ...
    if (ssAfterConcat) {
        ...
        const auto stridedSlice =
            std::make_shared<ov::opset1::StridedSlice>(concat, beginParam, endParam, beginMask, endMask);
    ...
    const auto maxPool =
        std::make_shared<ov::opset1::MaxPool>(concat, stride, padBegin, padEnd, kernel, roundingType, padType);
    ...
    std::shared_ptr<ov::Model> function =
        std::make_shared<ov::Model>(results, ov::ParameterVector{input}, "ConcatWithDifferentChildrenTransformation");
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginalWithDifferentPrecisionOnChildren(
    ...
    const auto concat =
        std::make_shared<ov::opset1::Concat>(OutputVector{fakeQuantize1->output(0), fakeQuantize2->output(0)}, axis);
    ...
    const auto avgPool = std::make_shared<ov::opset1::AvgPool>(concat->output(0),
                                                               stride,
                                                               padBegin,
                                                               padEnd,
                                                               kernel,
    ...
    const auto maxPool = std::make_shared<ov::opset1::MaxPool>(concat->output(0),
                                                               stride,
                                                               padBegin,
                                                               padEnd,
                                                               kernel,
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginalWithIntermediateWithConstant(
    ...
    if (transparentIntermediate) {
        ...
        const auto outputShape = ov::opset1::Constant::create(
            ov::element::i64,
            ov::Shape{2},
            ov::Shape{inputShape[2].is_dynamic() ? 9ul : static_cast<size_t>(inputShape[2].get_length()),
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{fakeQuantize2->output(0), intermediateOp->output(0)}, 1);
    concat->set_friendly_name("concat");
    ...
    const auto avgPool =
        std::make_shared<ov::opset1::AvgPool>(concat, stride, padBegin, padEnd, kernel, true, roundingType, padType);
    avgPool->set_friendly_name("avgPool");
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginalWithReshapeAtTheEndTransformation(
    ...
    const std::shared_ptr<ov::opset1::Concat> concat1 =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{fakeQuantize1->output(0), fakeQuantize2->output(0)}, 1);
    concat1->set_friendly_name("concat1");
    ...
    const std::shared_ptr<ov::opset1::Concat> concat2 =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{fakeQuantize3, intermediate}, 1);
    concat2->set_friendly_name("concat2");
...
std::shared_ptr<ov::Model> ConcatFunction::getOriginalWithIntermediateReshape(const ov::element::Type precision,
                                                                              const ov::Shape& inputShape,
    ...
    const auto reshape2 = std::make_shared<ov::opset1::Reshape>(
        ...
        true);
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{reshape1->output(0), reshape2->output(0)}, 1);
    concat->set_friendly_name("output");
    auto& rtInfo = concat->get_rt_info();
    rtInfo["Variant::std::string"] = "concat";
...
std::shared_ptr<ov::Model> ConcatFunction::getReference(const ov::element::Type precision,
                                                        const ov::Shape& inputShape,
                                                        const FakeQuantizeOnData& fqOnData1,
                                                        const FakeQuantizeOnData& fqOnData2,
...
std::shared_ptr<ov::Model> ConcatFunction::get(const ov::element::Type inputPrecision,
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{parent1, parent2}, axis);
    concat->set_friendly_name("concat");
...
std::shared_ptr<ov::Model> ConcatFunction::getReferenceWithIntermediate(
    ...
    if (transparentIntermediate) {
        ...
        const auto weights = ov::opset1::Constant::create(precision,
                                                          ov::Shape{channels, channels, 1, 1},
                                                          std::vector<float>(channels * channels, 1));

        intermediateOp = std::make_shared<ov::opset1::Convolution>(deqBefore2,
                                                                   weights,
                                                                   ov::Strides{1, 1},
                                                                   ov::CoordinateDiff{0, 0},
                                                                   ov::CoordinateDiff{0, 0},
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{deqBefore1, intermediateOp}, 1);
    concat->set_friendly_name("concat");
    ...
    auto weights = ov::opset1::Constant::create(precision, ov::Shape{channels, channels, 1, 1}, {1});
    auto convolution = std::make_shared<ov::opset1::Convolution>(lastDequantization2,
                                                                 weights,
                                                                 ov::Strides{1, 1},
                                                                 ov::CoordinateDiff{0, 0},
                                                                 ov::CoordinateDiff{0, 0},
    ...
    ov::ResultVector results{std::make_shared<ov::opset1::Result>(lastDequantization1),
                             std::make_shared<ov::opset1::Result>(convolution)};
...
std::shared_ptr<ov::Model> ConcatFunction::getReferenceWithIntermediateAvgPool(
    ...
    ov::ResultVector results{std::make_shared<ov::opset1::Result>(parent1),
                             std::make_shared<ov::opset1::Result>(parent2)};
...
std::shared_ptr<ov::Model> ConcatFunction::getReferenceWithSplitedIntermediate(
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{deqBefore1, intermediateOp->output(0)}, splitedAxis);
    concat->set_friendly_name("concat");
...
std::shared_ptr<ov::Model> ConcatFunction::getReferenceSelectionWithIntermediate(
    ...
    const std::shared_ptr<ov::opset1::Concat> concat =
        std::make_shared<ov::opset1::Concat>(ov::OutputVector{deqBefore1, intermediateOp->output(0)}, 1);
    concat->set_friendly_name("concat");
    ov::pass::low_precision::NetworkHelper::setOutDataPrecision(concat, precisionAfterOperation);
    ...
    const std::shared_ptr<ov::Node> lastDequantization1 =
        dequantizationOperations1.empty() ? concat : makeDequantization(concat, dequantizationOperations1);
    lastDequantization1->set_friendly_name("concat");

    const std::shared_ptr<ov::Node> lastDequantization2 =
        dequantizationOperations2.empty() ? nullptr : makeDequantization(intermediateOp, dequantizationOperations2);

    auto weights = ov::opset1::Constant::create(precision, ov::Shape{inputShape[1], inputShape[1], 1, 1}, {1});
    auto convolution =
        std::make_shared<ov::opset1::Convolution>(lastDequantization2 == nullptr ? intermediateOp : lastDequantization2,
                                                  weights,
                                                  ov::Strides{1, 1},
                                                  ov::CoordinateDiff{0, 0},
                                                  ov::CoordinateDiff{0, 0},
...
std::shared_ptr<ov::Model> ConcatFunction::getReferenceWithStridedSlice(const ov::element::Type inputPrecision,
                                                                        const ov::PartialShape inputShape,
                                                                        const FakeQuantizeOnData& fq1,
                                                                        const FakeQuantizeOnData& fq2,
...
std::shared_ptr<ov::Model> ConcatFunction::getReferenceWithDifferentPrecisionOnChildren(
    ...
    const auto avgPool = std::make_shared<ov::opset1::AvgPool>(lastDequantization1,
                                                               stride,
                                                               padBegin,
                                                               padEnd,
                                                               kernel,
    ...
    const std::shared_ptr<ov::opset1::MaxPool> maxPool = std::make_shared<ov::opset1::MaxPool>(concat->output(0),
                                                                                               stride,
                                                                                               padBegin,
                                                                                               padEnd,
                                                                                               kernel,
...
std::shared_ptr<ov::Model> ConcatFunction::getReferenceWithIntermediateWithConstant(
    ...
    const auto avgPool =
        std::make_shared<ov::opset1::AvgPool>(deqAfter, stride, padBegin, padEnd, kernel, true, roundingType, padType);
    avgPool->set_friendly_name("avgPool");
