...
std::shared_ptr<ov::Model> ElementwiseWithMultiParentDequantizationFunction::get(
    ...
    const auto input1_2 =
        std::make_shared<ov::opset1::Parameter>(precision1, ov::Shape({inputShape[0], inputShape[1], 1, 1}));
    ...
    const std::shared_ptr<ov::Node> parent1 =
        dequantization1.empty() ? multiply1 : makeDequantization(multiply1, dequantization1);
    ...
    const std::shared_ptr<ov::Node> parent2 =
        dequantization2.empty() ? multiply2 : makeDequantization(multiply2, dequantization2);
