...
std::shared_ptr<ov::Model> FakeQuantizeAndConvolutionFunction::get(const ov::element::Type precision,
                                                                   const ov::PartialShape& inputShape,
                                                                   const FakeQuantizeOnData& fqOnData,
                                                                   const FakeQuantizeOnWeights& fqOnWeights) {
    const auto rankLength = inputShape.rank().is_dynamic() ? 4 : inputShape.rank().get_length();
    OPENVINO_ASSERT(rankLength == 3ul || rankLength == 4ul || rankLength == 5ul,
    ...
    const auto weights =
        ov::opset1::Constant::create(precision,
                                     rankLength == 3ul ? (ov::Shape{outputChannelsCount, inputChannelsCount, 1})
...
std::shared_ptr<ov::Model> FakeQuantizeAndConvolutionFunction::get(
    ...
    return FakeQuantizeAndConvolutionFunction::get(precision,
                                                   inputShape,
                                                   fqOnData,
                                                   convertOnData,
...
std::shared_ptr<ov::Model> FakeQuantizeAndConvolutionFunction::get(
    ...
    std::shared_ptr<Node> parentOnWeights;
    {
        ...
        const Shape shape = constantOnWeights.shapeIsDefined ? constantOnWeights.shape
                                                             : ov::Shape{outputChannelsCount, inputChannelsCount, 1, 1};
        parentOnWeights =
            ov::opset1::Constant::create(constantOnWeights.outPrecision,
                                         shape,
        ...
        if (!fqOnWeights.empty()) {
            parentOnWeights =
                fqOnWeights.outputPrecision == element::undefined
