...
std::shared_ptr<ov::Model> SplitFunction::getOriginal(const ov::element::Type originalFunctionPrecision,
                                                      const ov::PartialShape& inputShape,
...
std::shared_ptr<ov::Model> SplitFunction::getReference(
    ...
    for (size_t i = 0; i < numSplit; ++i) {
        if (!dequantizationAfter.empty()) {
            ...
            results.push_back(
                std::make_shared<ov::opset1::Result>(makeDequantization(split->output(i), dequantizationAfter[i])));
        } else {
            results.push_back(std::make_shared<ov::opset1::Result>(split->output(i)));
        }
    }
