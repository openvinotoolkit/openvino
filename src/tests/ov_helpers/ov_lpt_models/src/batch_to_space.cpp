...
std::shared_ptr<ov::Model> BatchToSpaceFunction::get(const ov::PartialShape& input_shape,
    ...
    std::shared_ptr<Node> parent = fq_on_data.empty() ? std::dynamic_pointer_cast<ov::Node>(input)
...
std::shared_ptr<ov::Model> BatchToSpaceFunction::get(
    ...
    std::shared_ptr<Node> parent = dequantization_before.empty() ? std::dynamic_pointer_cast<ov::Node>(input)
