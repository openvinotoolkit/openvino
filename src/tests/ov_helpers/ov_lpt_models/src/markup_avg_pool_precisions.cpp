...
std::shared_ptr<Node> createConvolution(const ov::element::Type precision,
...
std::shared_ptr<ov::Model> MarkupAvgPoolPrecisionsFunction::getOriginal(
    ...
    {
        auto createBranch = [](const ov::element::Type precision,
                               const std::string& additionalLayer,
            ...
            auto newParent =
                ov::test::utils::make_fake_quantize(parent, precision, 256, {}, {-1.28}, {1.27}, {-1.28}, {1.27});
    ...
    if (fakeQuantizeBranch != -1) {
        if (fakeQuantizeBranch != 1) {
            parent1 =
                ov::test::utils::make_fake_quantize(parent1, precision, 256, {}, {-1.28}, {1.27}, {-1.28}, {1.27});
            parent1->set_friendly_name("fakeQuantize1");
        }
        if (fakeQuantizeBranch != 0) {
            parent2 =
                ov::test::utils::make_fake_quantize(parent2, precision, 256, {}, {-1.28}, {1.27}, {-1.28}, {1.27});
