...
namespace LayerTestsDefinitions {
    ...
    void GRUSequenceTest::SetUp() {
        ...
        if (m_mode == SequenceTestsMode::CONVERT_TO_TI_MAX_SEQ_LEN_PARAM ||
            ...
            seq_lengths_node = ov::test::utils::make_constant<int64_t>(ov::element::i64, inputShapes[2], {}, true,
                                                             static_cast<int64_t>(seq_lengths), 0.f);
        } else {
            std::vector<int64_t> lengths(batch, seq_lengths);
            seq_lengths_node = ov::test::utils::make_constant(ov::element::i64, inputShapes[2], lengths, false);
        }
