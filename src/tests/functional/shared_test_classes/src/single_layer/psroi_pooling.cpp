...
void PSROIPoolingLayerTest::SetUp() {
    ...
    std::shared_ptr<ov::Node> psroiPooling = std::make_shared<ov::op::v0::PSROIPooling>(params[0],
                                                                                                params[1],
                                                                                                outputDim,
                                                                                                groupSize_,
                                                                                                spatialScale_,
                                                                                                ...
                                                                                                spatialBinsY_,
                                                                                                mode_);
    ov::ResultVector results{std::make_shared<ov::op::v0::Result>(psroiPooling)};
    function = std::make_shared<ov::Model>(results, params, "psroi_pooling");
}
}  // namespace LayerTestsDefinitions
