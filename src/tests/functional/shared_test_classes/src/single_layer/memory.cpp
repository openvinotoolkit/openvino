...
void MemoryTest::CreateTIFunc() {
    ...
    auto body = std::make_shared<Function>(ov::OutputVector{res, Iter_res}, ParameterVector{X, Y, Iter});
    ...
    function =
        std::make_shared<Function>(ov::OutputVector{output, output_iter}, ParameterVector{param, iter_count}, "PureTI");
}
