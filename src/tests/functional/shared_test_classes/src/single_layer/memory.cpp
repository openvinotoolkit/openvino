...
namespace LayerTestsDefinitions {
    ...
    void MemoryTest::CreateTIFunc() {
        auto param = ov::test::utils::builder::make_params(ngPrc, {inputShape}).at(0);
        std::vector<std::vector<size_t>> shape = {{static_cast<size_t>(iteration_count), 1}};
        auto iter_count = ov::test::utils::builder::make_params(ngPrc, shape).at(0);

        // Body
        auto X = ov::test::utils::builder::make_params(ngPrc, {inputShape}).at(0);
        auto Y = ov::test::utils::builder::make_params(ngPrc, {inputShape}).at(0);
        auto Iter = ov::test::utils::builder::make_params(ngPrc, {Shape{1, 1}}).at(0);
