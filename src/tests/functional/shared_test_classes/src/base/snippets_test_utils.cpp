...
void SnippetsTestsCommon::validateNumSubgraphs() {
    ...
    for (const auto& op : compiled_model->get_ops()) {
        ...
        if (layer_type == "Const" || layer_type == "Input" || layer_type == "Output")
            continue;
        auto& rt = op->get_rt_info();
        const auto rinfo = rt.find("layerType");
        ASSERT_NE(rinfo, rt.end()) << "Failed to find layerType in " << op->get_friendly_name() << "rt_info.";
...
void SnippetsTestsCommon::validateOriginalLayersNamesByType(const std::string& layerType,
