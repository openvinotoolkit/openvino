...
inline std::pair<double, double> calculate_thresholds_by_whole_model(const std::shared_ptr<ov::Model>& model) {
...
inline std::pair<double, double> calculate_thresholds_by_model_results(const std::shared_ptr<ov::Model>& model,
    ...
    if (results.size() != ref_results.size()) {
        throw std::runtime_error(
            "Model and ref_model should have the same output number! Impossible to calculate default threshold!");
    }
    for (size_t out_idx = 0; out_idx < results.size(); ++out_idx) {
        ...
        const auto abs_value =
            ov::test::utils::tensor_comparation::calculate_default_abs_threshold(elem_type,
                                                                                 ref_elem_type,
                                                                                 inference_precision);
        if (abs_value > max_abs_threshold) {
            max_abs_threshold = abs_value;
        }
        const auto rel_value =
            ov::test::utils::tensor_comparation::calculate_default_rel_threshold(elem_type,
                                                                                 ref_elem_type,
                                                                                 inference_precision);
        if (rel_value > max_rel_threshold) {
            max_rel_threshold = rel_value;
...
std::pair<double, double> calculate_thresholds_by_model(const std::shared_ptr<ov::Model>& model,
                                                        const std::shared_ptr<ov::Model>& ref_model,
                                                        const ov::element::Type& inference_precision) {
    ...
    std::tie(model_max_abs_threshold, model_max_rel_threshold) =
        ov::test::utils::calculate_thresholds_by_whole_model(model);
    if (ref_model) {
