...
ov::runtime::Tensor generate(const std::shared_ptr<ov::Node>& node,
    ...
    if (elemType.is_real() && in_gen_data.resolution == 1) {
        in_gen_data.range = 10;
        in_gen_data.resolution = 256;
    }
    auto it = inputRanges.find(node->get_type_info());
    if (it != inputRanges.end()) {
        const std::vector<std::vector<InputGenerateData>>& ranges = it->second;
        if (ranges.size() != 2) {
            throw std::runtime_error("Incorrect size of ranges. It should be 2 (real and int cases)");
        }
        const std::vector<InputGenerateData>& range = ranges.at(elemType.is_real());
        const InputGenerateData& in_gen_data_ref = range.size() < inNodeCnt ? range.front() : range.at(port);
        in_gen_data = in_gen_data_ref;
    }
...
//                              const ov::element::Type& elemType,
//                              const ov::Shape& targetShape,
//                              InputGenerateData in_gen_data = InputGenerateData()) {
...
// // ov::runtime::Tensor generate(const std::shared_ptr<ov::op::v1::Reshape>& node,
// //                              size_t port,
// //                              const ov::element::Type& elemType,
// //                              const ov::Shape& targetShape,
//                                   InputGenerateData in_gen_data = InputGenerateData()) {
// //     const auto &shape_size = ov::shape_size(node->get_input_shape(0));
// //     if (port == 1) {
...
//         return tensor;
//     }
//     return generate(std::dynamic_pointer_cast<ov::Node>(node), port, elemType, targetShape, in_gen_data);
// }
...
//                              size_t port,
//                              const ov::element::Typeco& elemType,
//                              const ov::Shape& targetShape,
//                                   InputGenerateData in_gen_data = InputGenerateData()) {
//     return Activation::generate(elemType, targetShape);
// }
...
//             return ov::test::utils::create_and_fill_tensor(elemType,
// targetShape, inGenData.range, inGenData.start_from, inGenData.resolution, inGenData.seed);
//         }
//         default:
//             return generate(std::dynamic_pointer_cast<ov::Node>(node), port, elemType, targetShape);
//     }
...
ov::runtime::Tensor generateInput(const std::shared_ptr<ov::Node>& node,
                                  ...
                                  const ov::Shape& targetShape,
                                  InputGenerateData in_gen_data) {
    return generate(ngraph::as_type_ptr<T>(node), port, elemType, targetShape, in_gen_data);
}
} // namespace
