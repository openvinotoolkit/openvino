...
namespace ov {
namespace test {
std::string ScatterElementsUpdateLayerTest::getTestCaseName(
    const testing::TestParamInfo<scatterElementsUpdateParamsTuple>& obj) {
...
void ScatterElementsUpdateLayerTest::SetUp() {
    ...
    auto indices_const =
        std::make_shared<ov::op::v0::Constant>(indices_type, targetStaticShapes.at(0).at(1), indices_value);
    auto axis_const = std::make_shared<ov::op::v0::Constant>(ov::element::i32, ov::Shape{}, std::vector<int>{axis});
    auto scatter_elements_update =
        std::make_shared<ov::op::v3::ScatterElementsUpdate>(param, indices_const, update_param, axis_const);
    function = std::make_shared<ov::Model>(scatter_elements_update->outputs(),
                                           ov::ParameterVector{param, update_param},
                                           "ScatterElementsUpdate");
}

std::string ScatterElementsUpdate12LayerTest::getTestCaseName(
    const testing::TestParamInfo<scatterElementsUpdate12ParamsTuple>& obj) {
    auto shapes_ss = [](const InputShape& shape) {
        std::stringstream ss;
        ss << "_IS=(" << ov::test::utils::partialShape2str({shape.first}) << ")_TS=";
        for (size_t j = 0lu; j < shape.second.size(); j++)
...
void ScatterElementsUpdate12LayerTest::SetUp() {
    ...
    ov::element::Type model_type, indices_type;
    std::string target_device;
    std::tie(shapes_desc, indices_value, reduceMode, useInitVal, model_type, indices_type, targetDevice) =
        this->GetParam();
    std::tie(input_shapes, axis) = shapes_desc;
    ...
    auto indices_const =
        std::make_shared<ov::op::v0::Constant>(indices_type, targetStaticShapes.at(0).at(1), indices_value);
    auto axis_const = std::make_shared<ov::op::v0::Constant>(ov::element::i32, ov::Shape{}, std::vector<int>{axis});
    auto scatter_elements_update = std::make_shared<ov::op::v12::ScatterElementsUpdate>(param,
                                                                                        indices_const,
                                                                                        update_param,
                                                                                        axis_const,
                                                                                        reduceMode,
