...
std::string ComparisonLayerTest::getTestCaseName(const testing::TestParamInfo<ComparisonTestParams>& obj) {
    ...
    std::tie(shapes, comparison_op_type, second_input_type, model_type, device_name, additional_config) = obj.param;
...
void ComparisonLayerTest::SetUp() {
    ...
    std::tie(shapes, comparison_op_type, second_input_type, model_type, targetDevice, additional_config) =
        this->GetParam();
