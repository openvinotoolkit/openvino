...
void ConvertColorI420AccuracyTest::generate_inputs(const std::vector<ov::Shape>& target_input_static_shapes) {
    inputs.clear();
...
void ConvertColorI420AccuracyTest::compare(const std::vector<ov::Tensor> &expected, const std::vector<ov::Tensor> &actual) {
    ConvertColorI420LayerTest::compare(expected, actual);
    ...
    for (int i = 0; i < expected.size(); i++) {
        ValidateColors(static_cast<float*>(expected[i].data()), static_cast<float*>(actual[i].data()), expected.size(), 0.02);
    }
}
} // namespace test
} // namespace ov
