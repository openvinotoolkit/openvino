...
void MemoryLSTMCellTest::SetUp() {
    ...
    auto input_add_const = ov::test::utils::make_constant(element_type, input_dims, input_bias);
    auto add = ov::test::utils::make_eltwise(input_parameter[0], input_add_const, ov::test::utils::EltwiseTypes::ADD);

    auto input_mul_const = ov::test::utils::make_constant(element_type, input_dims, input_weights);
    ...
    auto cell_memory_constant = ov::test::utils::make_constant<float>(element_type, cell_memory_dims, cell_memory_init);
    auto var_cell =
    ...
    auto hidden_memory_constant =
        ov::test::utils::make_constant<float>(element_type, hidden_memory_dims, hidden_memory_init);
    ...
    auto weightsNode = ov::test::utils::make_constant<float>(element_type, {4 * hiddenSize, inputSize}, weights_vals);
    auto reccurrenceWeightsNode =
        ov::test::utils::make_constant<float>(element_type, {4 * hiddenSize, hiddenSize}, reccurrenceWeights_vals);
    auto biasNode = ov::test::utils::make_constant<float>(element_type, {4 * hiddenSize}, bias_vals);
...
>>>>>>> df24b0b1f6 (Rework make_constant node builder)
