...
std::string QuantizedConvolutionBatchNorm::getTestCaseName(
    const testing::TestParamInfo<QuantizedConvolutionBatchNormParams>& obj) {
...
void QuantizedConvolutionBatchNorm::SetUp() {
    ...
    if (transpose_on_weights) {
        ...
        if (weights_intervals_shape.size() == 4) {
            weights_intervals_shape = {weights_intervals_shape[2],
                                       weights_intervals_shape[0],
                                       weights_intervals_shape[3],
                                       weights_intervals_shape[1]};
        }
    }
    ...
    std::shared_ptr<Node> weights =
        ov::test::utils::deprecated::make_constant(element::f32, weights_shape, {}, true, 0.5f, -0.5f);
    if (quantize_type == QuantizeType::FAKE_QUANTIZE) {
        ...
        weights = std::make_shared<ov::op::v0::FakeQuantize>(weights,
                                                             low_weights,
                                                             high_weights,
                                                             low_weights,
                                                             high_weights,
        ...
        activations = std::make_shared<ov::op::v0::FakeQuantize>(parameter,
                                                                 low_act,
                                                                 high_act,
                                                                 output_low_act,
                                                                 output_high_act,
    ...
    if (transpose_on_weights) {
        weights =
            std::make_shared<ov::op::v1::Transpose>(weights,
                                                    ov::op::v0::Constant::create(element::i32, Shape{4}, {1, 3, 0, 2}));
    }
    std::shared_ptr<Node> conv;
    if (conv_type == ConvType::CONVOLUTION) {
        conv = std::make_shared<ov::op::v1::Convolution>(activations,
                                                         weights,
                                                         Strides{1, 1},
                                                         CoordinateDiff{0, 0},
                                                         CoordinateDiff{0, 0},
        ...
        conv = std::make_shared<ov::op::v1::ConvolutionBackpropData>(activations,
                                                                     weights,
                                                                     Strides{1, 1},
                                                                     CoordinateDiff{0, 0},
                                                                     CoordinateDiff{0, 0},
