...
void TransposeConvTest::SetUp() {
    ...
    auto reshape_pattern = std::make_shared<ov::op::v0::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{conv_input_shape.size()}, conv_input_shape);
    auto reshape = std::make_shared<ov::op::v1::Reshape>(params[0], reshape_pattern, false);

    const auto input_order1 = std::make_shared<ov::op::v0::Constant>(ngraph::element::i64,
    ...
    auto filter_weights_node = ov::test::utils::make_constant<float>(ng_prc, {output_channels, input_channels, kernel_shape[0], kernel_shape[1]},
                                                                  { weight_val });
    ...
    const auto input_order2 = std::make_shared<ov::op::v0::Constant>(ngraph::element::i64,
                                                                         ngraph::Shape({conv_input_shape.size()}),
                                                                         nhwc_order);
    auto transpose2 = std::make_shared<ov::op::v1::Transpose>(conv, input_order2);

    auto constant_squeeze = std::make_shared<ov::op::v0::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{1}, std::vector<size_t>{0});
    auto squeeze = std::make_shared<ov::op::v0::Squeeze>(transpose2, constant_squeeze);
