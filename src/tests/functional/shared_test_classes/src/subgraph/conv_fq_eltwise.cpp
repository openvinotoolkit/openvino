...
void ConvFqEltwiseTest::SetUp() {
    ...
    float weightVal = 0.2;
    auto filterWeightsNode = ov::builder::makeConstant<float>(ngPrc, {outputChannels, inputChannels, kernelShape[0], kernelShape[1]},
                                                                  { weightVal });
    auto convLowNode =
        ov::builder::makeConstant(ngraph::element::f32, std::vector<size_t>{1}, std::vector<float>{-convFQValue});
    auto convHighNode =
        ov::builder::makeConstant(ngraph::element::f32, std::vector<size_t>{1}, std::vector<float>{convFQValue});
    ...
    auto biasesWeightsNode = ov::builder::makeConstant(ngPrc, {}, std::vector<float>{ 0.0f });
    ...
    auto lowNode = ov::builder::makeConstant(ngraph::element::f32, std::vector<size_t>{ 1 },
        std::vector<float>{inputDataMin * weightVal * kernelShape[1] * 1.5f});
    auto highNode = ov::builder::makeConstant(ngraph::element::f32, std::vector<size_t>{ 1 },
    ...
    auto constNode = ov::builder::makeConstant(ngPrc, {}, std::vector<float>{ 0.5f });
