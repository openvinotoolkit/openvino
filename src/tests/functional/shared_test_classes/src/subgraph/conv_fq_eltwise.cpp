...
void ConvFqEltwiseTest::SetUp() {
    ...
    float weightVal = 0.2;
    auto filterWeightsNode = ov::test::utils::make_constant<float>(ngPrc, {outputChannels, inputChannels, kernelShape[0], kernelShape[1]},
                                                                  { weightVal });
    auto convLowNode =
        ov::test::utils::make_constant(ngraph::element::f32, std::vector<size_t>{1}, std::vector<float>{-convFQValue});
    auto convHighNode =
        ov::test::utils::make_constant(ngraph::element::f32, std::vector<size_t>{1}, std::vector<float>{convFQValue});
    ...
    auto conv = std::make_shared<ov::op::v1::Convolution>(reshape1, convWeightsFQ, strides, std::vector<ptrdiff_t>{ 0, 0 },
                                                              ...
                                                              ov::op::PadType::VALID);
    auto biasesWeightsNode = ov::test::utils::make_constant(ngPrc, {}, std::vector<float>{ 0.0f });
    ...
    auto lowNode = ov::test::utils::make_constant(ngraph::element::f32, std::vector<size_t>{ 1 },
        std::vector<float>{inputDataMin * weightVal * kernelShape[1] * 1.5f});
    auto highNode = ov::test::utils::make_constant(ngraph::element::f32, std::vector<size_t>{ 1 },
    ...
    auto constNode = ov::test::utils::make_constant(ngPrc, {}, std::vector<float>{ 0.5f });
    ...
    auto reshape2 = std::make_shared<ov::op::v1::Reshape>(add_2, reshapePattern2, false);
