...
void MVNMultiplyAdd::SetUp() {
    ...
    auto axesNode = ov::test::utils::make_constant(axesType, ov::Shape{axes.size()}, axes);
    ...
    auto gamma = ov::test::utils::make_constant<float>(dataType, constantShapes, {}, true);
    auto mul = std::make_shared<ov::op::v1::Multiply>(mvn, gamma);
    auto beta = ov::test::utils::make_constant<float>(dataType, constantShapes, {}, true);
