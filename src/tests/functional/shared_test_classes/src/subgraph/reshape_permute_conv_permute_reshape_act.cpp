...
void ConvReshapeAct::SetUp() {
    ...
    std::tie(model_type, targetDevice, input_shape, kernel_shape, output_channels, additional_config) =
        this->GetParam();
    ...
    std::vector<size_t> reshape_out_dims = {
        1,
        input_shape[0] * input_shape[1] * (input_shape[2] - kernel_shape[1] + 1) * output_channels};
    ...
    auto conv = ov::test::utils::make_convolution(permute_in,
                                                  model_type,
    ...
    auto reshape_out_pattern =
        std::make_shared<ov::op::v0::Constant>(ov::element::i64, ov::Shape{2}, std::vector<size_t>{reshape_out_dims});
