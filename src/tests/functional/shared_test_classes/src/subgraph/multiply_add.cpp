...
void MultiplyAddLayerTest::SetUp() {
    ...
    auto const_mul = ov::test::utils::make_constant<float>(element_type, constShape, {}, true);
    auto mul = std::make_shared<ov::op::v1::Multiply>(params[0], const_mul);
    auto const_add = ov::test::utils::make_constant<float>(element_type, constShape, {}, true);
