...
void FqConvFqAffineTest::SetUp() {
    ...
    auto convLowNode = ov::test::utils::make_constant(ngraph::element::f32, std::vector<size_t>{ 1 }, std::vector<float>{inputDataMin});
    auto convHighNode = ov::test::utils::make_constant(ngraph::element::f32, std::vector<size_t>{ 1 }, std::vector<float>{inputDataMax});
    ...
    if (permute) {
        auto permuteOrder = std::make_shared<ov::op::v0::Constant>(ngraph::element::i64,
                                                                       ngraph::Shape{4},
                                                                       ngraph::Shape{{0, 3, 2, 1}});
        auto transpose = std::make_shared<ov::op::v1::Transpose>(add, permuteOrder);
    ...
    auto matMulWeightsNode = ov::test::utils::make_constant<float>(ngPrc, {outFormShapes[1], outFormShapes[1]}, { 1.0f });
    auto matMulLowNode = ov::test::utils::make_constant(ngraph::element::f32, std::vector<size_t>{ 1 }, std::vector<float>{inputDataMin});
    auto matMulHighNode = ov::test::utils::make_constant(ngraph::element::f32, std::vector<size_t>{ 1 }, std::vector<float>{inputDataMax});
    ...
    auto matmul = std::make_shared<ov::op::v0::MatMul>(reshape2, matMulWeightsFQ, false, true);
