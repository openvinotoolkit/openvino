...
std::shared_ptr<ov::Model> create_if_model(bool condition) {
    ...
    auto cond = std::make_shared<ngraph::opset1::Constant>(ngraph::element::boolean, ngraph::Shape{ 1 }, condition);
    ...
    return std::make_shared<ngraph::Function>(ngraph::NodeVector{ if_result }, ngraph::ParameterVector{ X, Y });
}
