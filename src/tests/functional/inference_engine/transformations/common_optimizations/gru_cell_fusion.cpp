...
namespace {
ov::Output<ov::Node> create_activation_by_name(const string& activation_name, const ov::Output<ov::Node>& input) {
...
shared_ptr<ov::Model> gen_model(const string& activation_1, const string& activation_2, size_t batch, size_t hidden_size, size_t input_size,
    ...
    auto X = make_shared<Parameter>(f32, ov::Shape{batch, input_size});
    if (use_dyn_shapes) {
        X = make_shared<Parameter>(f32, ov::PartialShape{static_cast<int64_t>(batch), ov::Dimension::dynamic()});
    }
    auto H = make_shared<Parameter>(f32, ov::Shape{batch, hidden_size});
    auto WRzr = make_shared<Parameter>(f32, ov::Shape{2 * hidden_size, input_size + hidden_size});
    auto Bzr = make_shared<Parameter>(f32, ov::Shape{1, 2 * hidden_size});
    auto WRh = make_shared<Parameter>(f32, ov::Shape{hidden_size, input_size + hidden_size});
    auto Bh = make_shared<Parameter>(f32, ov::Shape{1, hidden_size});
    auto concat_1 = make_shared<Concat>(ov::OutputVector{X, H}, 1);
    auto matmul_1 = make_shared<MatMul>(concat_1, WRzr, false, true);
    ov::Output<ov::Node> in_to_activation_1 = matmul_1;
    if (use_bias_add_1) {
        in_to_activation_1 = make_shared<Add>(matmul_1, Bzr);
    }

    auto act_1 = create_activation_by_name(activation_1, in_to_activation_1);
    auto axis_1 = make_shared<Constant>(i64, ov::Shape{}, 1);
    auto split = make_shared<Split>(act_1, axis_1, 2);

    auto multiply_1 = make_shared<Multiply>(split, H);
    auto concat_2 = make_shared<Concat>(ov::OutputVector{X, multiply_1}, 1);
    auto matmul_2 = make_shared<MatMul>(concat_2, WRh, false, true);
    ov::Output<ov::Node> in_to_activation_2 = matmul_2;
    if (use_bias_add_2) {
        in_to_activation_2 = make_shared<Add>(matmul_2, Bh);
    }

    auto act_2 = create_activation_by_name(activation_2, in_to_activation_2);
    auto one = make_shared<Constant>(f32, ov::Shape{1}, 1);
    ...
    return make_shared<ov::Model>(ov::OutputVector{add}, ov::ParameterVector{X, H, WRzr, WRh, Bzr, Bh});
}

shared_ptr<ov::Model> gen_reference(const string& activation_1, const string& activation_2, size_t batch, size_t hidden_size,
                                    size_t input_size, bool use_bias_add_1, bool use_bias_add_2) {
    auto X = make_shared<Parameter>(f32, ov::Shape{batch, input_size});
    auto H = make_shared<Parameter>(f32, ov::Shape{batch, hidden_size});
    auto WRzr = make_shared<Parameter>(f32, ov::Shape{2 * hidden_size, input_size + hidden_size});
    auto WRh = make_shared<Parameter>(f32, ov::Shape{hidden_size, input_size + hidden_size});
    ov::ParameterVector params = {X, H, WRzr, WRh};

    shared_ptr<ov::Node> Bzr = make_shared<Constant>(f32, ov::Shape{1, 2 * hidden_size}, 0);
    if (use_bias_add_1) {
        Bzr = make_shared<Parameter>(f32, ov::Shape{1, 2 * hidden_size});
        params.push_back(std::dynamic_pointer_cast<Parameter>(Bzr));
    }
    shared_ptr<ov::Node> Bh = make_shared<Constant>(f32, ov::Shape{1, hidden_size}, 0);
    if (use_bias_add_2) {
        Bh = make_shared<Parameter>(f32, ov::Shape{1, hidden_size});
        params.push_back(std::dynamic_pointer_cast<Parameter>(Bh));
    }

    auto axis_0 = make_shared<Constant>(i64, ov::Shape{}, 0);
    auto axis_1 = make_shared<Constant>(i64, ov::Shape{}, 1);
    auto split_lenghts = make_shared<Constant>(i64, ov::Shape{2}, vector<size_t>{input_size, hidden_size});
    ...
    auto B = make_shared<Concat>(ov::OutputVector{Bzr, Bh}, 1);
    ...
    return std::make_shared<ngraph::Function>(ov::OutputVector {cell}, params);
}
} // namespace
...
class GRUFusionTest
        : public testing::WithParamInterface<GRUFusionParams>,
...
class GRUFusionTestDyn
        : public testing::WithParamInterface<GRUFusionParams>, public TransformationTestsF {
};
...
static std::vector<GRUFusionParams> params = {
...
INSTANTIATE_TEST_SUITE_P(GRUFusionTest, GRUFusionTest, ::testing::ValuesIn(params));
INSTANTIATE_TEST_SUITE_P(GRUFusionTestDyn, GRUFusionTestDyn, ::testing::ValuesIn(params));