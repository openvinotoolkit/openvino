...
TEST(TransformationTests, DoNotStartSubgraphAfterInputs) {
    ...
    {
        auto data0 = std::make_shared<op::v1::Parameter>(element::f32, Shape{2, 3});
        auto data1 = std::make_shared<op::v1::Parameter>(element::f32, Shape{1, 3});
        const std::vector<float> const_values{3, 2, 10};
        auto const_data = std::make_shared<op::v1::Constant>(element::f32, Shape{1, 3}, const_values);
...
TEST(TransformationTests, StartSubgraphMultipleOutputs) {
    ...
    std::shared_ptr<Model> f(nullptr), f_ref(nullptr);
    {
        auto data0 = std::make_shared<op::v1::Parameter>(element::i32, Shape{2, 3});
        auto data1 = std::make_shared<op::v1::Parameter>(element::i32, Shape{1, 3});
        auto convert0 = std::make_shared<op::v1::Convert>(data0, element::f32);
        auto convert1 = std::make_shared<op::v1::Convert>(data1, element::f32);
        const std::vector<float> const_values{3, 2, 10};
        auto const_data = std::make_shared<op::v1::Constant>(element::f32, Shape{1, 3}, const_values);
    ...
    }
    {
        ...
        const std::vector<float> const_values{3, 2, 10};
        auto const_data = std::make_shared<op::v1::Constant>(element::f32, Shape{1, 3}, const_values);
        auto indata0 = std::make_shared<op::v1::Parameter>(element::f32, Shape{2, 3});
        auto indata1 = std::make_shared<op::v1::Parameter>(element::f32, Shape{1, 3});
        auto indata2 = std::make_shared<op::v1::Parameter>(element::f32, Shape{1, 3});
...
TEST(TransformationTests, TokenizeMulAddSubgraph) {
    ...
    std::shared_ptr<Model> f(nullptr), f_ref(nullptr);
    {
        auto data_1 = std::make_shared<op::v1::Parameter>(element::f32, Shape{ 1, 3, 4, 4 });
        auto data_2 = std::make_shared<op::v1::Parameter>(element::f32, Shape{ 1, 3, 4, 4 });
        auto non_snippet_op = std::make_shared<op::v1::MatMul>(data_1, data_2);

        auto mul_const_1 = op::v1::Constant::create(element::f32, { 1, 3, 1, 1 }, { 4.f });
        auto mul_1 = std::make_shared<op::v1::Multiply>(non_snippet_op, mul_const_1);
        auto add_const_1 = op::v1::Constant::create(element::f32, { 1, 3, 1, 1 }, { 16.4f });
        auto add_1 = std::make_shared<op::v1::Add>(mul_1, add_const_1);
        auto elu = std::make_shared<op::v1::Elu>(add_1, 0.01);

        auto mul_const_2 = op::v1::Constant::create(element::f32, { 1, 3, 1, 1 }, { 4.f });
        auto mul_2 = std::make_shared<op::v1::Multiply>(non_snippet_op, mul_const_2);
        auto add_const_2 = op::v1::Constant::create(element::f32, { 1, 3, 1, 1 }, { 16.4f });
        auto add_2 = std::make_shared<op::v1::Add>(mul_2, add_const_2);
        auto relu = std::make_shared<op::v1::Relu>(add_2);

        auto add = std::make_shared<op::v1::Add>(elu, relu);
        auto result = std::make_shared<op::v1::Result>(add);
    ...
    }
    {
        ...
        // snippet function
        auto snippet_input = std::make_shared<op::v1::Parameter>(element::f32, Shape{ 1, 3, 4, 4 });

        auto sn_mul_const_1 = std::make_shared<op::v1::Parameter>(element::f32, Shape{ 1, 3, 1, 1 });
        auto mul_1 = std::make_shared<op::v1::Multiply>(snippet_input, sn_mul_const_1);
        auto sn_add_const_1 = std::make_shared<op::v1::Parameter>(element::f32, Shape{ 1, 3, 1, 1 });
        auto add_1 = std::make_shared<op::v1::Add>(mul_1, sn_add_const_1);
        auto elu = std::make_shared<op::v1::Elu>(add_1, 0.01);

        auto sn_mul_const_2 = std::make_shared<op::v1::Parameter>(element::f32, Shape{ 1, 3, 1, 1 });
        auto mul_2 = std::make_shared<op::v1::Multiply>(snippet_input, sn_mul_const_2);
        auto sn_add_const_2 = std::make_shared<op::v1::Parameter>(element::f32, Shape{ 1, 3, 1, 1 });
        ...
        auto result = std::make_shared<op::v1::Result>(snippet);
...
TEST(TransformationTests, AttachToSubgraph) {
    ...
    std::shared_ptr<Model> f(nullptr), f_ref(nullptr);
    {
        ...
        auto neg = std::make_shared<op::v1::Negative>(add);
        auto concat = std::make_shared<op::v1::Concat>(NodeVector{add, neg}, 0);
...

    {
        ...
        auto add = std::make_shared<Subgraph>(NodeVector{data0, data1},
            std::make_shared<Model>(NodeVector{std::make_shared<op::v1::Negative>(inner), inner}, ParameterVector{indata0, indata1}));
        auto concat = std::make_shared<op::v1::Concat>(OutputVector{add->output(0), add->output(1)}, 0);
...
TEST(TransformationTests, DontAttachToSubgraphIfLoop) {
    ...
    std::shared_ptr<Model> f(nullptr), f_ref(nullptr);
    {
        ...
        auto add = std::make_shared<Subgraph>(NodeVector{data0, data1},
            std::make_shared<Model>(NodeVector{std::make_shared<op::v1::Add>(indata0, indata1)}, ParameterVector{indata0, indata1}));
        auto log = std::make_shared<op::v1::Log>(add);
...

    {
        ...
        */
        auto add_param = std::make_shared<op::v1::Parameter>(element::f32, add->get_output_shape(0));
        auto log_param = std::make_shared<op::v1::Parameter>(element::f32, log->get_output_shape(0));
