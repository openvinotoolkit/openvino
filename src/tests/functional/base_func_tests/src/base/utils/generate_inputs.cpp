...
void fillCoordTensor(std::vector<float>& coords, int height, int width,
...
void fillIdxTensor(std::vector<int>& idx, int batch_size) {
    int batch_id = 0;
...
ov::Tensor generate(const std::shared_ptr<ov::op::v3::ROIAlign>& node,
    ...
    switch (port) {
        ...
        case 2: {
            ...
            fillIdxTensor(roiIdxVector, node->get_shape()[0]);
