...
std::vector<std::string> disabledTestPatterns() {
    return {
            ...
            // unsupported metrics
            R"(.*nightly_MultiHeteroAutoBatchOVGetMetricPropsTest.*OVGetMetricPropsTest.*(AVAILABLE_DEVICES|OPTIMIZATION_CAPABILITIES|MAX_BATCH_SIZE|RANGE_FOR_ASYNC_INFER_REQUESTS|RANGE_FOR_STREAMS).*)",
    };
}
