...
struct ModelInfo {
    ...
    ModelInfo(const std::string& model_path = "",
        ...
        : total_op_cnt(total_ops_in_model),
          this_op_cnt(this_ops_in_model),
    ...
    bool operator==(const ModelInfo& model_info_ref) const {
        if (this->model_priority != model_info_ref.model_priority || this->this_op_cnt != model_info_ref.this_op_cnt ||
            this->total_op_cnt != model_info_ref.total_op_cnt ||
            this->model_paths.size() != model_info_ref.model_paths.size()) {
            return false;
        }
        for (const auto& model_path : this->model_paths) {
            if (std::find(model_info_ref.model_paths.begin(), model_info_ref.model_paths.end(), model_path) ==
                model_info_ref.model_paths.end()) {
                return false;
            }
        }
        return true;
