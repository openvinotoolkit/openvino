...
inline ModelComparator::IsSubgraphTuple prepare_is_subgraph_result(
    bool is_subgraph,
...
ModelComparator::IsSubgraphTuple ModelComparator::is_subgraph(const std::shared_ptr<ov::Model>& model,
    ...
    size_t ordered_ops_cnt = model->get_ordered_ops().size() - in_info.size() - model->get_results().size(),
           ref_ordered_ops_cnt =
               ref_model->get_ordered_ops().size() - in_info_ref.size() - ref_model->get_results().size();
    ...
    return prepare_is_subgraph_result(matched_op_names.size() == subgraph_to_check_ops_cnt,
                                      subgraph,
                                      graph,
                                      matched_op_names);
}

bool ModelComparator::match(const std::shared_ptr<ov::Node>& node, const std::shared_ptr<ov::Node>& ref_node) const {
    return m_manager.match(node, ref_node);
}

bool ModelComparator::match(const std::shared_ptr<ov::Model>& model,
                            const std::shared_ptr<ov::Model>& ref_model) const {
...
ModelComparator::ExtractedSubgraphTuple ModelComparator::is_subgraph(
    ...
    if (std::get<0>(extractor_res)) {
        ...
        try {
            auto subgraph_in_info_new = ov::util::align_input_info(subgraph,
                                                                   graph,
                                                                   subgraph_in_info,
                                                                   graph_in_info,
...
std::unordered_map<std::string, std::string> ModelComparator::get_matched_ops_in_graphs(
    const std::shared_ptr<ov::Model>& subgraph,
    const std::shared_ptr<ov::Model>& graph,
    ...
    for (const auto& subgraph_op : subgraph_to_check_ops) {
        for (const auto& graph_op : graph_to_check_ops) {
            if (ov::util::is_node_to_skip(subgraph_op) || ov::util::is_node_to_skip(graph_op)) {
                continue;
            }
                    ...
                    for (size_t idx = 0; idx < graph_op->inputs().size(); ++idx) {
                        ...
                        if (ov::util::is_node_to_skip(graph_in) && ov::util::is_node_to_skip(subgraph_in)) {
                            if (match(subgraph_in, graph_in)) {
                                matched_op_names.insert(
                                    {subgraph_in->get_friendly_name(), graph_in->get_friendly_name()});
...
std::pair<bool, std::map<std::string, ov::conformance::InputInfo>> ModelComparator::match(
    const std::shared_ptr<ov::Model>& model,
    ...
    try {
        if (match(model, model_ref)) {
            auto new_input_info = ov::util::align_input_info(model,
                                                             model_ref,
                                                             in_info,
                                                             in_info_ref,
...
std::vector<std::vector<size_t>> ModelComparator::get_matched_op_patterns(const ov::NodeVector& ordered_ops) {
