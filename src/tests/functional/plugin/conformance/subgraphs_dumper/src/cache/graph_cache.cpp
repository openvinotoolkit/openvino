...
void GraphCache::update_cache(const std::shared_ptr<ov::Model>& model,
    ...
    if (from_cache) {
        ...
        if (m_graph_cache_bytesize + 2 * model_bytesize >= mem_size) {
            std::cout << "[ GRAPH CACHE ][ WARNING ] There are not enought RAM memory! Serialize graph cache"
                      << std::endl;
            serialize_cache();
        ...
        if (is_large_model) {
            ...
            std::cout << "[ GRAPH CACHE ][ WARNING ] Model  bytesize is " << model_bytesize_gb
                      << "GB. It is larger than 25% RAM size or >= 8GB: " << mem_size_gb
                      << ". Constants won't be copied!" << std::endl;
        }
...
void GraphCache::update_cache(const std::shared_ptr<ov::Model>& extracted_model,
    ...
    auto this_op_cnt = extracted_model->get_ops().size() - extracted_model->get_parameters().size() -
                       extracted_model->get_results().size();
    std::map<std::string, ov::conformance::InputInfo> updated_input_info;
    if (!m_graph_cache.empty() && model_to_update != nullptr) {
        auto comparator_res = m_model_comparator->match(extracted_model,
                                                        model_to_update,
                                                        input_info,
                                                        m_graph_cache.at(model_to_update).get_input_info());
    ...
    if (model_to_update == nullptr) {
        ...
        for (const auto& extractor : m_manager.get_extractors()) {
            auto tmp_serialized_model_path =
                ov::util::path_join({m_serialization_dir, m_cache_subdir, extractor.first, graph_name + ".xml"});
        ...
        if (!serialized_model_path.empty()) {
            ...
            auto comparator_res =
                m_model_comparator->match(extracted_model, cached_model, input_info, cached_meta.get_input_info());
            if (comparator_res.first) {
            ...
            for (const auto& cached_model : m_graph_cache) {
                auto comparator_res = m_model_comparator->match(extracted_model,
                                                                cached_model.first,
                                                                input_info,
                                                                cached_model.second.get_input_info());
                if (comparator_res.first) {
                    model_to_update = cached_model.first;
                    updated_input_info = comparator_res.second;
                    break;
                } else {
                    auto is_subgraph = m_model_comparator->is_subgraph(extracted_model,
                                                                       cached_model.first,
                                                                       input_info,
                                                                       cached_model.second.get_input_info());
                    // in case if one model is subgraph of other to update model meta info and remove subgraph from
                    // cache
                    if (std::get<0>(is_subgraph)) {
                        ...
                        auto matched_ops =
                            std::get<3>(m_model_comparator->is_subgraph(extracted_model, cached_model.first));
                        auto cached_model_op_cnt = cached_model.first->get_ops().size() -
                                                   cached_model.second.get_input_info().size() -
                                                   cached_model.first->get_results().size();
                        auto extracted_model_op_cnt = extracted_model->get_ops().size() - input_info.size() -
                                                      extracted_model->get_results().size();

                        bool is_same_paired_op_cnt =
                            ov::util::is_same_paired_op_cnt(extracted_model, cached_model.first);
                        if (matched_ops.size() > 0.75 * extracted_model_op_cnt && is_same_paired_op_cnt) {
                            if (cached_model_op_cnt > extracted_model_op_cnt) {
                                return;
                            }
                            m_graph_cache_bytesize +=
                                (extracted_model->get_graph_size() - cached_model.first->get_graph_size());
                            m_graph_cache.erase(cached_model.first);
                            ov::conformance::MetaInfo meta(model_path,
                                                           input_info,
                                                           model_op_cnt,
                                                           this_op_cnt,
                                                           extractor_name);
    ...
    if (pattern_model_size < cached_model_size) {
        ...
        auto new_in_info = ov::util::align_input_info(model_to_update,
                                                      extracted_model,
                                                      m_graph_cache.at(model_to_update).get_input_info(),
                                                      input_info,
                                                      matched_ops);
...
void GraphCache::serialize_cache() {
    while (!m_graph_cache.empty()) {
        ...
        auto rel_dir = ov::util::path_join(
            {m_cache_subdir, ov::util::get_model_type(cache_item->first), cache_item->second.get_any_extractor()});
