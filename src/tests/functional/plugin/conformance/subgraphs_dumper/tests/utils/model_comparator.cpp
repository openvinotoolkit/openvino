...
protected:
    void SetUp() override {
        ...
        }
        {
            std::shared_ptr<ov::op::v0::Parameter> test_parameter =
                std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::Shape{2, 5});
            std::shared_ptr<ov::op::v0::Relu> test_abs = std::make_shared<ov::op::v0::Relu>(test_parameter);
            std::shared_ptr<ov::op::v0::Result> test_res = std::make_shared<ov::op::v0::Result>(test_abs);
            test_model_1 = std::make_shared<ov::Model>(ov::ResultVector{test_res}, ov::ParameterVector{test_parameter});
        }
    }
...
TEST_F(ModelComparatorTest, match_with_in_info) {
    ov::util::ModelComparator::Ptr model_comparator = ov::util::ModelComparator::get();
    std::map<std::string, ov::conformance::InputInfo> test_in_info(
        {{"test_parameter_0", ov::conformance::InputInfo(ov::Shape{1, 2})}}),
