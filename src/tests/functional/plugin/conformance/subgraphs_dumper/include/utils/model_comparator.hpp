...
public:
    ...
    bool match(const std::shared_ptr<ov::Node>& node, const std::shared_ptr<ov::Node>& ref_node) const;
    bool match(const std::shared_ptr<ov::Model>& model, const std::shared_ptr<ov::Model>& ref_model) const;

    std::pair<bool, std::map<std::string, InputInfo>> match(const std::shared_ptr<ov::Model>& model,
    ...
    // {{matched_node_id}}
    std::vector<std::vector<size_t>> get_matched_op_patterns(const ov::NodeVector& ordered_nodes);

    // { op_name_subgraph, op_name_graph}
    std::unordered_map<std::string, std::string> get_matched_ops_in_graphs(const std::shared_ptr<ov::Model>& subgraph,
                                                                           const std::shared_ptr<ov::Model>& graph,
                                                                           bool is_check_inputs = false) const;

    void set_match_coefficient(float _match_coefficient);
    float get_match_coefficient() {
...
protected:
    ...
    ModelComparator() {
        ov::tools::subgraph_dumper::MatchersManager::MatchersMap matchers = {
            {"generic_single_op",
             ov::tools::subgraph_dumper::SingleOpMatcher::Ptr(new ov::tools::subgraph_dumper::SingleOpMatcher)},
            {"convolutions",
             ov::tools::subgraph_dumper::ConvolutionsMatcher::Ptr(new ov::tools::subgraph_dumper::ConvolutionsMatcher)},
        };
