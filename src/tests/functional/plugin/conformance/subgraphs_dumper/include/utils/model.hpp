...
generate_model(ov::NodeVector& nodes,
                    ...
                    for (const auto& orig_node_to_check : orig_in_node->output(out_idx).get_target_inputs()) {
                        if (orig_node_to_check.get_node()->shared_from_this() == node) {
                            ...
                            if (cloned_node_map.count(orig_in_node_name)) {
                                ...
                                ov::replace_output_update_name(cloned_in_node->output(0), orig_in_node->output(out_idx));
                                if (ov::op::util::is_parameter(orig_in_node)) {
                                    ...
                                    model_sniks.push_back(std::dynamic_pointer_cast<ov::op::Sink>(cloned_node->shared_from_this()));
                                }
                                filled_input_idx++;
    ...
    auto model = std::make_shared<ov::Model>(model_results, model_sniks, model_parameters);
