...
public:
    ...
    bool match(const std::shared_ptr<ov::Node>& node, const std::shared_ptr<ov::Node>& ref) const;

    void set_matchers(const MatchersMap& matchers = {}) {
        m_matchers = matchers;
    }
    ...
    const MatchersMap& get_matchers() {
        return m_matchers;
    }
