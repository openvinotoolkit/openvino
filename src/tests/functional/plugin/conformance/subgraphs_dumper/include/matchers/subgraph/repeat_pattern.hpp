...
public:
    ...
    std::vector<std::vector<PatternBorders>> get_repeat_pattern_borders(const std::shared_ptr<ov::Model>& model);
    std::vector<std::vector<ov::NodeVector>> get_repeat_node_vectors(const std::shared_ptr<ov::Model>& model);
...
protected:
    ...
    using ExtractedRepeatPattern =
        std::tuple<std::shared_ptr<ov::Model>, ov::NodeVector, std::map<std::string, ov::conformance::InputInfo>>;
    size_t min_graph_size = 2;
    bool is_split_by_matched_nodes = false, is_recursive_extraction = false;

    // find repeat patterns in model
    std::list<std::vector<ExtractedRepeatPattern>> find_repeat_patterns(const std::shared_ptr<ov::Model>& model,
    ...
    std::vector<std::vector<ov::NodeVector>> get_patterns_by_nodes(const std::vector<size_t>& start_op_vec,
