...
std::shared_ptr<ov::Model> generate(const std::shared_ptr<ov::op::v16::SegmentMax>& node) {
    ...
    const auto SegmentMaxNode =
        std::make_shared<ov::op::v16::SegmentMax>(data, segment_ids, num_segments, ov::op::FillMode::ZERO);
...
std::shared_ptr<ov::Model> generate(const std::shared_ptr<ov::op::v16::SparseFillEmptyRows>& node) {
    ...
    const auto sparseFillEmptyRowsNode =
        std::make_shared<ov::op::v16::SparseFillEmptyRows>(values, dense_shape, indices, default_value);
    ov::ResultVector results{std::make_shared<ov::op::v0::Result>(sparseFillEmptyRowsNode->output(0)),
    ...
    return std::make_shared<ov::Model>(results,
                                       ov::ParameterVector{values, indices, default_value},
                                       "SparseFillEmptyRowsGraph");
}
...
std::shared_ptr<ov::Model> generateMaxPoolBase(const std::shared_ptr<ov::op::Op>& node) {
    ...
    if (ov::is_type<ov::op::v1::MaxPool>(node)) {
        ...
        maxPoolNode =
            std::make_shared<ov::op::v14::MaxPool>(data, strides, dilations, pads_begin, pads_end, kernel_shape);
    } else {
        return nullptr;
    }
...
std::shared_ptr<ov::Model> generateScatterNDBase(const std::shared_ptr<ov::op::Op>& node) {
    ...
    if (ov::is_type<ov::op::v3::ScatterNDUpdate>(node)) {
        ...
        scatterNode = std::make_shared<ov::op::v15::ScatterNDUpdate>(data,
                                                                     indices,
                                                                     updates,
...
std::shared_ptr<ov::Model> generateEmbeddingBagOffsetsBase(const std::shared_ptr<ov::op::Op>& node) {
    ...
    if (ov::is_type<ov::op::v3::EmbeddingBagOffsetsSum>(node)) {
        ...
        EmbeddingBagOffsetsNode = std::make_shared<ov::op::v15::EmbeddingBagOffsets>(params.at(0),
                                                                                     indices,
                                                                                     offsets,
                                                                                     default_index,
                                                                                     reduction);
