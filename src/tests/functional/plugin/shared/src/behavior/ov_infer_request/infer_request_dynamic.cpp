...
TEST_P(OVInferRequestDynamicTests, InferDynamicNetwork) {
    ...
    for (auto& shape : vectorShapes) {
        ov::runtime::Tensor inTensor = ov::test::utils::create_and_fill_tensor(element::f32, shape, ov::test::utils::InputGenerateData(-50, 100));
...
TEST_P(OVInferRequestDynamicTests, InferDynamicNetworkSetUnexpectedOutputTensorBeforeInfer) {
    ...
    const std::string outputname = function->outputs().back().get_any_name();
    OV_ASSERT_NO_THROW(req = execNet.create_infer_request());
    tensor = ov::test::utils::create_and_fill_tensor(element::f32, refShape, ov::test::utils::InputGenerateData(-50, 100));
    OV_ASSERT_NO_THROW(req.set_tensor("input_tensor", tensor));
    auto outShape = refOutShape;
    outShape[0] += 1;
    otensor = ov::test::utils::create_and_fill_tensor(element::f32, outShape, ov::test::utils::InputGenerateData(50, 100));
