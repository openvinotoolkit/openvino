...
std::shared_ptr<ov::Model> OVIterationChaining::getIterativeFunction() {
    ...
    auto concat_const = ov::test::utils::make_constant(element::Type_t::f32, {1, 16}, std::vector<float>{}, true);
    auto concat = std::make_shared<ov::op::v0::Concat>(ov::NodeVector{params, concat_const}, 0 /*axis*/);
    auto eltwise_const = ov::test::utils::make_constant(element::Type_t::f32, {1, 16}, std::vector<float>{}, true);
