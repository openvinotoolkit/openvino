        ...
        for (int32_t h = 0; h < num_heads; h++) {
            ...
            for (int32_t p = 0; p < head_dim; p++) {
                for (int32_t n = 0; n < state_size; n++) {
                    state[p * state_size + n] = static_cast<float>(recurrent_state_table[state_off(first_block, h, p, n)]);
                }
            }
            ...
            for (int32_t token = token_begin; token < token_end; token++) {
                ...
                for (int32_t p = 0; p < head_dim; p++) {
                    ...
                    for (int32_t n = 0; n < state_size; n++) {
                        ...
                        s = s * dA + x_value * dt_value * static_cast<float>(B[(token * num_groups + g) * state_size + n]);
                ...
                if (reached_interval_boundary || reached_sequence_end) {
                    ...
                    if (slot < seq_blocks) {
                        ...
                        for (int32_t p = 0; p < head_dim; p++) {
                            for (int32_t n = 0; n < state_size; n++) {
                                recurrent_state_table[state_off(block_id, h, p, n)] = static_cast<T>(state[p * state_size + n]);
                            }
                        }
                    }
                }
...
std::string PagedSelectiveSSMLayerTest::getTestCaseName(const testing::TestParamInfo<PagedSelectiveSSMLayerParams>& obj) {
    const auto& [num_heads, num_groups, head_dim, state_size, seq_lengths, cache_intervals, element_type, target_device] = obj.param;
...
void PagedSelectiveSSMLayerTest::SetUp() {
    const auto& [num_heads, num_groups, head_dim, state_size, seq_lengths, cache_intervals, data_type, device] = GetParam();
    ...
    const ov::Shape BC_shape{static_cast<size_t>(tokens), static_cast<size_t>(num_groups), static_cast<size_t>(state_size)};
    const ov::Shape x_shape{static_cast<size_t>(tokens), static_cast<size_t>(num_heads), static_cast<size_t>(head_dim)};
    const ov::Shape state_shape{static_cast<size_t>(num_blocks), static_cast<size_t>(num_heads), static_cast<size_t>(head_dim), static_cast<size_t>(state_size)};
    ...
    auto p_state = std::make_shared<ov::op::v0::Parameter>(data_type, ov::PartialShape{-1, num_heads, head_dim, state_size});
...
void PagedSelectiveSSMLayerTest::generate_inputs(const std::vector<ov::Shape>& targetInputStaticShapes) {
    ...
    const auto& [num_heads, num_groups, head_dim, state_size, seq_lengths, cache_intervals, element_type, device] = GetParam();
...
std::vector<ov::Tensor> PagedSelectiveSSMLayerTest::calculate_refs() {
    ...
    if (element_type == ov::element::f16) {
        return calculate_typed_refs<ov::float16>(host_inputs, function, num_heads, num_groups, head_dim, state_size, element_type);
    }
    if (element_type == ov::element::bf16) {
        return calculate_typed_refs<ov::bfloat16>(host_inputs, function, num_heads, num_groups, head_dim, state_size, element_type);
    }
    return calculate_typed_refs<float>(host_inputs, function, num_heads, num_groups, head_dim, state_size, element_type);
}
...
void PagedSelectiveSSMLayerTest::compare(const std::vector<ov::Tensor>& expected, const std::vector<ov::Tensor>& actual) {
