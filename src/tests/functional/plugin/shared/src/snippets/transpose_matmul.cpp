...
std::string TransposeMatMul::getTestCaseName(testing::TestParamInfo<ov::test::snippets::TransposeMatMulParams> obj) {
    ...
    std::tie(input_shapes, order, transpose_position, elem_types, num_nodes, num_subgraphs, targetDevice) = obj.param;
...
void TransposeMatMul::SetUp() {
    ...
    size_t transpose_position;
    std::vector<ov::element::Type> elem_types;
    std::tie(input_shapes, order, transpose_position, elem_types, ref_num_nodes, ref_num_subgraphs, targetDevice) = this->GetParam();
    init_input_shapes(static_partial_shapes_to_test_representation(input_shapes));

    auto f = ov::test::snippets::TransposeMatMulFunction(input_shapes, elem_types, order, transpose_position);
...
void TransposeMatMulFQ::SetUp() {
    ...
    auto f = ov::test::snippets::FQMatMulFunction(input_shapes, order, transpose_position);
    function = f.getOriginal();
    if (!configuration.count(InferenceEngine::PluginConfigInternalParams::KEY_SNIPPETS_MODE)) {
        configuration.insert({InferenceEngine::PluginConfigInternalParams::KEY_SNIPPETS_MODE,
                              InferenceEngine::PluginConfigInternalParams::IGNORE_CALLBACK});
...
void TransposeMatMulBias::SetUp() {
    std::vector<ov::PartialShape> input_shapes;
    std::vector<int> order;
    size_t transpose_position;
    std::vector<ov::element::Type> elem_types;
...
TEST_P(TransposeMatMulBias, CompareWithRefImpl) {
    SKIP_IF_CURRENT_TEST_IS_DISABLED()
    run();
    validateNumSubgraphs();
}
