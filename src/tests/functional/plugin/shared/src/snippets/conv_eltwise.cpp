...
std::string ConvEltwise::getTestCaseName(testing::TestParamInfo<ov::test::snippets::ConvEltwiseParams> obj) {
    ...
    std::tie(inputShape0, inputShape1, binaryEltwise, num_nodes, num_subgraphs, targetDevice) = obj.param;
...
void ConvEltwise::SetUp() {
    ov::Shape inputShape0, inputShape1;
    std::shared_ptr<ov::Node> binaryEltwise;
    std::tie(inputShape0, inputShape1, binaryEltwise, ref_num_nodes, ref_num_subgraphs, targetDevice) =
        this->GetParam();
