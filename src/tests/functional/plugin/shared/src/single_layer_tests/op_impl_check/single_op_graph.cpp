...
std::shared_ptr<ov::Model> generate(const std::shared_ptr<ov::op::v12::Pad> &node) {
    const auto params = ngraph::builder::makeDynamicParams(ov::element::f32, {{6, 10}});
    const auto pad_begin = ngraph::builder::makeConstant<int64_t>(ov::element::i64, {2}, {4, -2});
    const auto pad_end = ngraph::builder::makeConstant<int64_t>(ov::element::i64, {2}, {5, -1});
...
std::shared_ptr<ov::Model> generate(const std::shared_ptr<ov::op::v0::PriorBoxClustered> &node) {
    ...
    attrs.heights = {3.0f};
    attrs.clip = true;
    const auto params = ngraph::builder::makeDynamicParams(ov::element::f32, {{4, 4}, {50, 50}});
    auto shape_of_1 = std::make_shared<ov::op::v3::ShapeOf>(params.at(0));
    auto shape_of_2 = std::make_shared<ov::op::v3::ShapeOf>(params.at(1));
