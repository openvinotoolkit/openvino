...
std::string ExecGrapDecomposeNormalizeL2::getTestCaseName(testing::TestParamInfo<std::string> obj) {
...
TEST_P(ExecGrapDecomposeNormalizeL2, CheckIfDecomposeAppliedForNonContiguousAxes) {
    ...
    const auto normalize_l2 =
        std::make_shared<ov::opset9::NormalizeL2>(input, axes_const, eps_value, ov::op::EpsMode::MAX);
    ...
    ASSERT_TRUE(model->get_ops().size() <
                compiled_model.get_runtime_model()->get_ops().size());  // decomposition applied
}
...
TEST_P(ExecGrapDecomposeNormalizeL2, CheckIfDecomposeNotAppliedForNotSorted) {
    ...
    const auto normalize_l2 =
        std::make_shared<ov::opset9::NormalizeL2>(input, axes_const, eps_value, ov::op::EpsMode::ADD);
    ...
    ASSERT_TRUE(model->get_ops().size() >=
                compiled_model.get_runtime_model()->get_ops().size());  // decomposition not applied
}
