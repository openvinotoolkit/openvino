...
std::string ExecGraphDisableLoweringPrecision::getTestCaseName(
    testing::TestParamInfo<ExecGraphDisableLoweringPrecisionSpecificParams> obj) {
...
void ExecGraphDisableLoweringPrecision::create_model() {
    ...
    auto weightConst =
        ov::op::v0::Constant::create(ov::element::i64, weiShape, std::vector<int64_t>(ov::shape_size(weiShape), 1));
...
void ExecGraphDisableLoweringPrecision::checkInferPrecision() {
    ...
    compiledModel = core->compile_model(funcPtr, targetDevice, ov::hint::inference_precision(loweringPrecision));
