...
std::shared_ptr<ngraph::Function> makeEltwiseFunction(const std::vector<InferenceEngine::Precision>& inputPrecisions) {
    ...
    auto inputs = ngraph::builder::makeParams(ov::test::utils::convertIe2OvPrc(inputPrecisions[0]), {{1, 16, 5, 4}});
    auto secondaryInput = ngraph::builder::makeInputLayer(ov::test::utils::convertIe2OvPrc(inputPrecisions[1]),
