...
std::string GroupConvolutionTransformation::getTestCaseName(
    const testing::TestParamInfo<GroupConvolutionTransformationParams>& obj) {
    ...
    result << get_test_case_name_by_params(netPrecision, inputShapes.first, targetDevice, params) << "_"
           << inputShapes.first.rank().get_length() << "D_" << inputShapes.first << "_" << inputShapes.second << "_"
           << param.group << "_" << param.groupCalculationDimention << "_" << param.fakeQuantizeOnData << "_"
           << (param.addReshape ? "reshape_on_weights_" : "wo_reshape_") << (addPrecisionPreserved ? "max_pool_" : "")
...
void GroupConvolutionTransformation::SetUp() {
    ...
    function = ov::builder::subgraph::GroupConvolutionFunction::getOriginal(netPrecision,
                                                                            inputShapes.first,
                                                                            inputShapes.second,
                                                                            param.group,
