...
std::string FuseConvertTransformation::getTestCaseName(
    const testing::TestParamInfo<FuseConvertTransformationParams>& obj) {
    ...
    result << get_test_case_name_by_params(precision, shape, targetDevice, params) << "_" << deqOperations << "_"
           << constInput;
    return result.str();
}
...
void FuseConvertTransformation::SetUp() {
    ...
    function = ov::builder::subgraph::FuseConvertFunction::getWithFQ(shape, precision, deqOperations, constInput);
}
