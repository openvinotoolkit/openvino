...
std::string ReduceMeanTransformation::getTestCaseName(
    const testing::TestParamInfo<ReduceMeanTransformationParams>& obj) {
    ...
    result << get_test_case_name_by_params(netPrecision, inputShape, targetDevice, params) << "_" << param.fakeQuantize
           << param.convert << param.dequantizationBefore << (param.reduceMean.keepDims ? "_keepDims_" : "");
...
void ReduceMeanTransformation::SetUp() {
    ...
    function = ov::builder::subgraph::ReduceFunction::get<ov::op::v1::ReduceMean>(netPrecision,
                                                                                  inputShape,
                                                                                  param.fakeQuantize,
                                                                                  param.convert,
