...
using RemoteTensorParams =
    std::tuple<element::Type,                                     // element type
               std::string,                                       // target device
               runtime::ConfigMap,                                // config
               std::pair<runtime::ParamMap, runtime::ParamMap>>;  // remote context and tensor parameters

class OVRemoteTest : public testing::WithParamInterface<RemoteTensorParams>, public CommonTestUtils::TestsCommon {
public:
...
protected:
    ...
    std::shared_ptr<op::v1::Parameter> input;
};
}  // namespace test
}  // namespace ov
