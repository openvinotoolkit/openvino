...
TEST_P(IEClassSpecificDeviceTestSetConfig, SetConfigSpecificDeviceNoThrow) {
    ...
    std::vector<std::string> deviceIDs = ie.GetMetric(cleartarget_device, METRIC_KEY(AVAILABLE_DEVICES))->as<std::vector<std::string>>();
...
TEST(IEClassBasicTest, smoke_GetMetricSupportedMetricsHeteroNoThrow) {
    ...
    std::vector<std::string> t = p->as<std::vector<std::string>>();
...
TEST_P(IEClassGetMetricTest_DEVICE_GOPS, GetMetricAndPrintNoThrow) {
    ...
    std::map<InferenceEngine::Precision, float> t = p->as<std::map<InferenceEngine::Precision, float>>();
...
TEST_P(IEClassGetMetricTest_DEVICE_TYPE, GetMetricAndPrintNoThrow) {
    ...
    InferenceEngine::Metrics::DeviceType t = p->as<InferenceEngine::Metrics::DeviceType>();
...
TEST_P(IEClassGetMetricTest_NUMBER_OF_WAITING_INFER_REQUESTS, GetMetricAndPrintNoThrow) {
    ...
    unsigned int t = p->as<unsigned int>();
...
TEST_P(IEClassGetMetricTest_RANGE_FOR_ASYNC_INFER_REQUESTS, GetMetricAndPrintNoThrow) {
    ...
    std::tuple<unsigned int, unsigned int, unsigned int> t = p->as<std::tuple<unsigned int, unsigned int, unsigned int>>();
...
TEST_P(IEClassGetMetricTest_RANGE_FOR_STREAMS, GetMetricAndPrintNoThrow) {
    ...
    std::tuple<unsigned int, unsigned int> t = p->as<std::tuple<unsigned int, unsigned int>>();
...
TEST_P(IEClassSetDefaultDeviceIDTest, SetDefaultDeviceIDNoThrow) {
    ...
    std::vector<std::string> deviceIDs = ie.GetMetric(target_device, METRIC_KEY(AVAILABLE_DEVICES))->as<std::vector<std::string>>();
    if (std::find(deviceIDs.begin(), deviceIDs.end(), deviceID) == deviceIDs.end()) {
        GTEST_FAIL() << "Incorrect DeviceID" << std::endl;
    }
    std::string value;
