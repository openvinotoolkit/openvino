...
TEST_P(OVCompiledGraphImportExportTest, importExportedIENetworkParameterResultOnly) {
    ...
    ov::CompiledModel importedCompiledModel;
    for (auto i = 0; i < 2; i++) {
        std::string file_name = "importExportedIENetworkParameterResultOnly_" + target_device + "_mmap.blob";
        if (i == 0) {
            std::stringstream strm;
...
TEST_P(OVCompiledGraphImportExportTest, importExportedIENetworkConstantResultOnly) {
    ...
    ov::CompiledModel importedCompiledModel;
    for (auto i = 0; i < 2; i++) {
        std::string file_name = "importExportedIENetworkConstantResultOnly_" + target_device + "_mmap.blob";
        if (i == 0) {
            std::stringstream strm;
            execNet.Export(strm);
            importedCompiledModel = core->import_model(strm, target_device, configuration);
...
TEST_P(OVCompiledGraphImportExportTest, ovImportExportedFunction) {
    ...
    InferenceEngine::ExecutableNetwork importedExecNet =
        ie->ImportNetwork(strm, target_device, any_copy(configuration));
