...
// device.
using OVInferConsistencyTestParamsTuple =
    typename std::tuple<unsigned int,                                    // inferRequst nums per model
...
class OVInferConsistencyTest : public testing::WithParamInterface<OVInferConsistencyTestParamsTuple>,
...
protected:
    ...
    std::vector<ov::Tensor> GetAllOutputs(ov::CompiledModel& model, ov::InferRequest& infer);
    bool IsEqual(std::vector<ov::Tensor>& a, std::vector<ov::Tensor>& b);
