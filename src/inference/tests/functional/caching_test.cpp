...
private:
    ...
    void setupMock(MockCachingIPlugin& plugin) {
        ...
        ON_CALL(plugin,
                import_model(A<std::istream&>(), A<const ov::SoPtr<ov::IRemoteContext>&>(), A<const ov::AnyMap&>()))
            .WillByDefault(
        ...
        ON_CALL(plugin,
                import_model(A<std::istream&>(), A<std::shared_ptr<ov::AlignedBuffer>>(), A<const ov::AnyMap&>()))
            .WillByDefault(
