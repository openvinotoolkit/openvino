...
private:
    ...
    void setupMock(MockCachingIPlugin& plugin) {
        ...
        ON_CALL(plugin, import_model(_, _, _, _))
            .WillByDefault(Invoke([&](std::istream& istr,
        ...
        ON_CALL(plugin, compile_model(_, _, _, _))
        ...
        ON_CALL(plugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _, _))
            .WillByDefault(Invoke([&](const std::shared_ptr<const ov::Model>& model,
                                      const ov::AnyMap& config,
                                      const std::function<std::string(const std::string&)>& encrypt) {
                if (m_checkConfigCb) {
...
TEST_P(CachingTest, TestLoad) {
    ...
    {
        EXPECT_CALL(*mockPlugin, compile_model(_, _, _, _)).Times(m_remoteContext ? 1 : 0);
    ...
    {
        EXPECT_CALL(*mockPlugin, compile_model(_, _, _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _, _)).Times(m_remoteContext ? 1 : 0);
...
TEST_P(CachingTest, TestLoadCustomImportExport) {
    ...
    ON_CALL(*mockPlugin, import_model(_, _, _, _))
        .WillByDefault(Invoke([&](std::istream& s,
                                  const ov::SoPtr<ov::IRemoteContext>&,
                                  const ov::AnyMap&,
                                  const std::function<std::string(const std::string&)>& decrypt) {
...
TEST_P(CachingTest, TestLoadChangeCacheDirOneCore) {
    ...
    {
        EXPECT_CALL(*mockPlugin, compile_model(_, _, _, _)).Times(m_remoteContext ? 2 : 0);
        EXPECT_CALL(*mockPlugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _, _))
...
TEST_P(CachingTest, TestThrowOnImport) {
    ...
    m_post_mock_net_callbacks.pop_back();
    {
        ...
        EXPECT_CALL(*mockPlugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _, _))
            .Times(!m_remoteContext ? 1 : 0);
        if (m_remoteContext) {
            EXPECT_CALL(*mockPlugin, import_model(_, _, _, _)).Times(1).WillOnce(Throw(1));
            EXPECT_CALL(*mockPlugin, import_model(_, _, _)).Times(0);
        } else {
            EXPECT_CALL(*mockPlugin, import_model(_, _, _, _)).Times(0);
...
TEST_P(CachingTest, LoadHetero_NoCacheMetric) {
    ...
    for (int i = 0; i < 2; i++) {
        EXPECT_CALL(*mockPlugin, compile_model(_, _, _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _, _)).Times(1);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _)).Times(0);
        m_post_mock_net_callbacks.emplace_back([&](MockICompiledModelImpl& net) {
            EXPECT_CALL(net, export_model(_)).Times(0);
...
TEST_P(CachingTest, LoadHetero_OneDevice) {
    ...
    m_post_mock_net_callbacks.pop_back();
    {
        ...
        EXPECT_CALL(*mockPlugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _)).Times(1);
        for (auto& net : comp_models) {
            EXPECT_CALL(*net, export_model(_)).Times(0);
        }
        testLoad([&](ov::Core& core) {
...
TEST_P(CachingTest, LoadHetero_MultiArchs) {
    ...
    deviceToLoad = ov::test::utils::DEVICE_HETERO + std::string(":mock.2,mock.52");
    {
        ...
        EXPECT_CALL(*mockPlugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _)).Times(AtLeast(2));  // for .2 and for .52
        for (auto& net : comp_models) {
            EXPECT_CALL(*net, export_model(_)).Times(0);
        }
        testLoad([&](ov::Core& core) {
    ...
    {
        EXPECT_CALL(*mockPlugin, compile_model(_, _, _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _, _)).Times(AtLeast(1));
        EXPECT_CALL(*mockPlugin, import_model(_, _, _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _)).Times(0);
        m_post_mock_net_callbacks.emplace_back([&](MockICompiledModelImpl& net) {
            EXPECT_CALL(net, export_model(_)).Times(AtLeast(1));
...
TEST_P(CachingTest, LoadAUTO_OneDevice) {
    ...
    for (int index = 0; index < TEST_COUNT; index++) {
        ...
        EXPECT_CALL(*mockPlugin, import_model(_, _, _)).Times(index);
        ASSERT_NO_THROW(testLoad([&](ov::Core& core) {
...
TEST_P(CachingTest, LoadMulti_race) {
    ...
    do {
        ...
        EXPECT_CALL(*mockPlugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _, _)).Times(1);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _)).Times(devCount - 1);
        testLoad([&](ov::Core& core) {
            core.set_property(ov::cache_dir(cacheDir));
...
TEST_P(CachingTest, Load_threads) {
    ...
    do {
        ...
        EXPECT_CALL(*mockPlugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _, _)).Times(1);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(_, _, _)).Times(THREADS_COUNT - 1);
        testLoad([&](ov::Core& core) {
