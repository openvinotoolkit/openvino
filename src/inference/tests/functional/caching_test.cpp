...
private:
    ...
    void setupMock(MockCachingIPlugin& plugin) {
        ...
        ON_CALL(plugin, import_model(A<ov::Tensor&>(), _, _))
            .WillByDefault(
                Invoke([&](ov::Tensor& tensor, const ov::SoPtr<ov::IRemoteContext>&, const ov::AnyMap& config) {
                    if (m_checkConfigCb) {
                        m_checkConfigCb(config);
                    }
                    std::string_view str_view(reinterpret_cast<char*>(tensor.data()), tensor.get_byte_size());
                    std::string name(str_view.data(), str_view.find(' '));
                    std::lock_guard<std::mutex> lock(mock_creation_mutex);
                    return create_mock_compiled_model(m_models[name], mockPlugin);
                }));
        ...
        ON_CALL(plugin, import_model(A<ov::Tensor&>(), _))
            .WillByDefault(Invoke([&](ov::Tensor& tensor, const ov::AnyMap& config) {
                if (m_checkConfigCb) {
                    m_checkConfigCb(config);
                }
                std::string_view str_view(reinterpret_cast<char*>(tensor.data()), tensor.get_byte_size());
...
TEST_P(CachingTest, TestLoadCustomImportExport) {
    ...
    ON_CALL(*mockPlugin, import_model(A<ov::Tensor&>(), _, _))
        .WillByDefault(Invoke([&](ov::Tensor& tensor, const ov::SoPtr<ov::IRemoteContext>&, const ov::AnyMap&) {
            char a[sizeof(customData)];
            std::memcpy(a, reinterpret_cast<const char*>(tensor.data()), sizeof(customData));
            EXPECT_EQ(memcmp(a, customData, sizeof(customData)), 0);
            std::string name(reinterpret_cast<const char*>(tensor.data()) + sizeof(customData),
                             tensor.get_byte_size() - sizeof(customData));
            std::lock_guard<std::mutex> lock(mock_creation_mutex);
            return create_mock_compiled_model(m_models[name], mockPlugin);
        }));

    ON_CALL(*mockPlugin, import_model(A<ov::Tensor&>(), _))
        .WillByDefault(Invoke([&](ov::Tensor& tensor, const ov::AnyMap&) {
            char a[sizeof(customData)];
            std::memcpy(a, reinterpret_cast<const char*>(tensor.data()), sizeof(customData));
            EXPECT_EQ(memcmp(a, customData, sizeof(customData)), 0);
            std::string name(reinterpret_cast<const char*>(tensor.data()) + sizeof(customData),
...
TEST_P(CachingTest, LoadHetero_MultiArchs) {
    ...
    deviceToLoad = ov::test::utils::DEVICE_HETERO + std::string(":mock.2,mock.52");
    {
        ...
        EXPECT_CALL(*mockPlugin, import_model(A<ov::Tensor&>(), _, _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(A<std::istream&>(), _)).Times(0);
        EXPECT_CALL(*mockPlugin, import_model(A<ov::Tensor&>(), _)).Times(AtLeast(2));  // for .2 and for .52
        for (auto& net : comp_models) {
            EXPECT_CALL(*net, export_model(_)).Times(0);
        }
        testLoad([&](ov::Core& core) {
...
TEST_P(CachingTest, LoadMulti_Archs) {
...

    {
        ...
        EXPECT_CALL(*mockPlugin, import_model(A<ov::Tensor&>(), _))
            .Times(TEST_DEVICE_MAX_COUNT / 2)
            .WillRepeatedly(Invoke([&](ov::Tensor& tensor, const ov::AnyMap&) {
                std::string_view str_view(reinterpret_cast<const char*>(tensor.data()), tensor.get_byte_size());
                std::string name(str_view.data(), str_view.find(' '));
                std::lock_guard<std::mutex> lock(mock_creation_mutex);
                return create_mock_compiled_model(m_models[name], mockPlugin);
            }));
