...
#ifdef __linux__
TEST(CPUMapParserTest, LinuxMapping) {
    struct linux_system_data_str {
        int _processors;
        int _sockets;
        int _cores;
    ...
    std::vector<linux_system_data_str> simulation_dataset = {
        {
            ._processors = 208,
            ._sockets = 2,
            ._cores = 104,
            ._proc_type_table = {{208}, {104}, {104}, {0}},
            ._cpu_mapping_table =
                {
    ...
    for (auto& test_data : simulation_dataset) {
        int test_sockets = 0;
        int test_cores = 0;
        ...
        ASSERT_EQ(test_data._sockets, test_sockets) << "Linux CPU map test fails on dataset " << i << ".";
        ASSERT_EQ(test_data._cores, test_cores) << "Linux CPU map test fails on dataset " << i << ".";
        ASSERT_EQ(test_data._proc_type_table, test_proc_type_table)
            << "Linux CPU map test fails on dataset " << i << ".";
