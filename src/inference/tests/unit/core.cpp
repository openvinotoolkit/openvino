...
TEST(CoreTests, ThrowOnRegisterPluginTwice) {
    ...
    OV_EXPECT_THROW(core.register_plugin("test_plugin", "TEST_DEVICE"), ov::Exception, ::testing::HasSubstr("Device with \"TEST_DEVICE\"  is already registered in the OpenVINO Runtime"));
}
...
TEST(CoreTests, ThrowOnRegisterPluginsTwice) {
    ...
    OV_EXPECT_THROW(core.register_plugins(getPluginXml()), ov::Exception, ::testing::HasSubstr("Device with \"TEST_DEVICE\"  is already registered in the OpenVINO Runtime"));
}