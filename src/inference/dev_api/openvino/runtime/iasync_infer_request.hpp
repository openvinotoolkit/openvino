...
protected:
    using Stage = std::pair<std::shared_ptr<ov::ITaskExecutor>, ov::Task>;
    /**
...
private:
    ...
    ov::Task make_next_stage_task(const Pipeline::iterator itStage,
                                  const Pipeline::iterator itEndStage,
                                  const std::shared_ptr<ov::ITaskExecutor> callbackExecutor);
    ...
    std::shared_ptr<ov::ITaskExecutor> m_request_executor;  //!< Used to run inference CPU tasks.
    std::shared_ptr<ov::ITaskExecutor>
