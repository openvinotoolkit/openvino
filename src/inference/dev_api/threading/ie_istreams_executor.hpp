...
public:
    ...
    struct INFERENCE_ENGINE_API_CLASS(Config) {
        ...
        static void UpdateHybridCustomThreads(Config& config, const int num_big_cores_phys, const int num_small_cores);
        static int LimitHybridStreams(const int num_streams, const int num_threads);
