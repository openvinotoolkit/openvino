 ...
 *  - ALL_CORE: all processors can be used. If hyper threading is enabled, both processors ofoneperformance-corewill be used.
 *  - PHY_CORE_ONLY: only one processor can be used per CPU core even with hyper threading enabled.
 *  - P_CORE_ONLY: only processors of performance-cores can be used. If hyper threading is enabled, both processors of one performance-core will be used.
