...
public:
    ...
    /**
     * @brief Imports a compiled model from the previously exported model file.
     * @param model_path std::string input model file containing a model previously exported using the
     * ov::CompiledModel::export_model method.
     * @param device_name Name of a device to import a compiled model for. Note, if @p device_name device was not used
     * to compile the original mode, an exception is thrown.
     * @param properties Optional map of pairs: (property name, property value) relevant only for this load
     ...
     * @return A compiled model.
     */
    CompiledModel import_model(const std::string& model_path,
                               const std::string& device_name,
                               const AnyMap& properties = {});

#ifdef OPENVINO_ENABLE_UNICODE_PATH_SUPPORT
    CompiledModel import_model(const std::wstring& model_path,
                               const std::string& device_name,
                               const AnyMap& properties = {});
#endif
    /**
     ...
     */
    template <typename... Properties>
    util::EnableIfAllStringAny<CompiledModel, Properties...> import_model(const std::string& model_path,
                                                                          const std::string& device_name,
                                                                          Properties&&... properties) {
        return import_model(model_path, device_name, AnyMap{std::forward<Properties>(properties)...});
    }

#ifdef OPENVINO_ENABLE_UNICODE_PATH_SUPPORT
    template <typename... Properties>
    util::EnableIfAllStringAny<CompiledModel, Properties...> import_model(const std::wstring& model_path,
                                                                          const std::string& device_name,
                                                                          Properties&&... properties) {
        return import_model(model_path, device_name, AnyMap{std::forward<Properties>(properties)...});
    }
    ...
    util::EnableIfAllStringAny<CompiledModel, Properties...> import_model(const std::string& model_path,
                                                                          const RemoteContext& context,
                                                                          Properties&&... properties) {
        return import_model(model_path, context, AnyMap{std::forward<Properties>(properties)...});
    }
