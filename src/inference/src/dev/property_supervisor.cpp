...
namespace ov {
struct PropertySupervisor::SubAccess : public Access {
    SubAccess(PropertySupervisor property_access_, const std::shared_ptr<void>& so_)
    ...
    PropertySupervisor property_access;
...
PropertySupervisor& PropertySupervisor::add(PropertySupervisor sub_accesses) {
...
PropertySupervisor& PropertySupervisor::add(const std::string& name,
...
PropertySupervisor& PropertySupervisor::add(const NamedProperties& named_properties,
...
PropertySupervisor& PropertySupervisor::remove(const std::string& name) {
...
PropertySupervisor& PropertySupervisor::ro() {
...
PropertySupervisor& PropertySupervisor::ro(const std::string& name) {
...
bool PropertySupervisor::has(const std::string name) const {
    return find_property(util::split(name, '.')).size() == 1;
}

std::vector<std::vector<std::string>> PropertySupervisor::get_all_pathes() const {
...
PropertySupervisor::Access::Ptr& PropertySupervisor::find_or_create(const std::string& name) {
...
PropertySupervisor* PropertySupervisor::find_property_access(const std::vector<std::string>& in_path) {
    std::vector<std::string> path;
...
const void* PropertySupervisor::find_access(const std::vector<std::string>& in_path) const {
    std::vector<std::string> path;
...
void* PropertySupervisor::find_access(const std::vector<std::string>& path) {
    return const_cast<void*>(const_cast<const PropertySupervisor*>(this)->find_access(path));
}

std::vector<std::vector<std::string>> PropertySupervisor::find_property(const std::vector<std::string>& rout) const {
...
std::vector<PropertyName> PropertySupervisor::get_supported() const {
...
AnyMap PropertySupervisor::get(const PropertyMutability mutability, bool intially_mutable) const {
    AnyMap result;
    for (auto&& access : accesses) {
        if (access.second->is_sub_access()) {
...
Any PropertySupervisor::get(const std::string& name, const AnyMap& args) const {
    auto names = util::split(name, '.');
...
PropertySupervisor& PropertySupervisor::set(const std::string& name, const Any& property, bool skip_unsupported) {
    auto names = util::split(name, '.');
...
PropertySupervisor& PropertySupervisor::set(const AnyMap& properties, bool skip_unsupported) {
...
bool PropertySupervisor::empty() const {
