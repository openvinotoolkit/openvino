...
ov::SoPtr<InferenceEngine::IExecutableNetworkInternal> ov::CoreImpl::compile_model_impl(
    ...
    execNetwork =
        context ? plugin.compile_model(network, context, _parsedConfig) : plugin.compile_model(network, _parsedConfig);
...
bool ov::CoreImpl::CoreConfig::plugin_config_is_supported(const ov::InferencePlugin& plugin,
    ...
    if (config_name == ov::auto_batch_timeout.name()) {
        supported = device_name.find("BATCH") != std::string::npos || device_name.find("AUTO") != std::string::npos ||
                    device_name.find("MULTI") != std::string::npos;
