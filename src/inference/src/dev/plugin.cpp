...
ov::SoPtr<ov::ICompiledModel> ov::Plugin::import_model(std::shared_ptr<ngraph::runtime::AlignedBuffer>& model_buffer, const ov::AnyMap& properties) const {
    OV_PLUGIN_CALL_STATEMENT(return {m_ptr->import_model(model_buffer, properties), m_so});
}

ov::SoPtr<ov::ICompiledModel> ov::Plugin::import_model(std::shared_ptr<ngraph::runtime::AlignedBuffer>& model_buffer,
                                                       const ov::RemoteContext& context,
                                                       const ov::AnyMap& config) const {
