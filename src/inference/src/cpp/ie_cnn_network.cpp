...
CNNNetwork::CNNNetwork(const std::shared_ptr<ov::Model>& graph) {
    ...
    network = std::make_shared<details::CNNNetworkNGraphImpl>(graph);
    actual = network.get();
    if (actual == nullptr) {
        IE_THROW() << "CNNNetwork was not initialized.";
    }
...
std::shared_ptr<ov::Model> CNNNetwork::getFunction() {
    if (actual == nullptr)
        IE_THROW() << "CNNNetwork was not initialized.";
    return actual->getFunction();
}

std::shared_ptr<const ov::Model> CNNNetwork::getFunction() const {
    if (actual == nullptr)
        IE_THROW() << "CNNNetwork was not initialized.";
    return actual->getFunction();
}
