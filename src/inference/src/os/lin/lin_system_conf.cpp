...
CPU::CPU() {
    ...
    auto check_valid_cpu = [&]() {
        ...
        for (int i = 0; i < _processors; i++) {
            if (CPU_ISSET(i, &mask)) {
                ...
                if (_sockets > 1) {
                    ...
                    if (std::find(numa_node_list.begin(),
                                  numa_node_list.end(),
                                  _cpu_mapping_table[i][CPU_MAP_NUMA_NODE_ID]) == numa_node_list.end()) {
                        numa_node_list.push_back(_cpu_mapping_table[i][CPU_MAP_NUMA_NODE_ID]);
                    }
                }
            }
        }
        if (_sockets > 1) {
            ...
            std::sort(numa_node_list.begin(), numa_node_list.end());
            std::map<int, int> sockets_map;
            std::map<int, int> numa_node_map;
            for (int i = 0; i < socket_list.size(); i++) {
                sockets_map.insert(std::pair<int, int>(socket_list[i], i));
            }
            for (int i = 0; i < numa_node_list.size(); i++) {
                numa_node_map.insert(std::pair<int, int>(numa_node_list[i], i));
            }
