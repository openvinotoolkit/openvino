...
class SelfAttention(torch.nn.Module):
    def __init__(self, hidden_size, num_attention_heads,
                 ...
                 params_dtype=torch.float, position_encoding_2d=True, empty_init=True, max_sequence_length=2048):
        if empty_init:
    ...
    def forward(
        ...
        if self.layer_past_key_padded is None:
            shape = (hidden_states.size(0), self.num_attention_heads, self.max_sequence_length, self.head_size_aligned)
            self.layer_past_key_padded = torch.zeros(shape, dtype=torch.bfloat16)
