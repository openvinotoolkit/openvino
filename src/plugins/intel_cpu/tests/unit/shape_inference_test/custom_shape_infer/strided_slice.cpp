...
TEST_P(StridedSliceCpuShapeInferenceTest , shape_inference_in_const_map) {
    ...
    const auto begin_tensor = std::make_shared<ngraph::runtime::HostTensor>(begin_const);
    const auto end_tensor = std::make_shared<ngraph::runtime::HostTensor>(end_const);
    const auto stride_tensor = std::make_shared<ngraph::runtime::HostTensor>(stride_const);
    const std::map<size_t, std::shared_ptr<ngraph::runtime::HostTensor>>& constant_data = {{1, begin_tensor},
                                                                                           {2, end_tensor},
                                                                                           {3, stride_tensor}};
    // implementation depends on some output information of the op
    op->set_output_type(0, element::i32, {-1, -1, -1});
