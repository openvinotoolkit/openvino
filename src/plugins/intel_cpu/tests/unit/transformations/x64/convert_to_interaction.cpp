...
static std::shared_ptr<ov::Model> makeInteraction(const ov::PartialShape& inputShape,
    ...
    auto gather_batch_indices =
        std::make_shared<ov::op::v0::Constant>(element::i32, ov::Shape{1}, std::vector<int32_t>{0});
    ...
    auto gather_feature_indices =
        std::make_shared<ov::op::v0::Constant>(element::i32, ov::Shape{1}, std::vector<int32_t>{1});
    ...
    auto reshape_shape =
        std::make_shared<ov::op::v0::Concat>(NodeVector{gather_batch, reshape_dim2, gather_feature}, 0);
    ...
    if (postFQ) {
        auto input_low =
            std::make_shared<ov::op::v0::Constant>(element::f32, ov::Shape{1}, std::vector<float>{-5.12978f});
        auto input_high =
            std::make_shared<ov::op::v0::Constant>(element::f32, ov::Shape{1}, std::vector<float>{5.08965f});
    ...
    std::shared_ptr<ov::Model> f(nullptr), f_ref(nullptr);
    {
        ...
        // construct ref interaction
        {
            ...
            auto interaction = std::make_shared<ov::op::TypeRelaxed<ov::intel_cpu::InteractionNode>>(
                ov::intel_cpu::InteractionNode(features),
                element::i8);
            f_ref = std::make_shared<ov::Model>(interaction, inputsParams, "interaction");
        }
        auto res = compare_functions(f, f_ref);
