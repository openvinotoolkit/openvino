...
void EltwiseChainTest::generate_inputs(const std::vector<ov::Shape>& targetInputStaticShapes) {
    ...
    for (size_t i = 0; i < funcInputs.size(); ++i) {
        ...
        inputs.insert({funcInput.get_node_shared_ptr(),
                       generate_eltwise_input(funcInput.get_element_type(), targetInputStaticShapes[i])});
    }
}
...
void EltwiseChainTest::SetUp() {
    ...
    if (secondaryInputType == utils::InputLayerType::PARAMETER) {
        ...
        paramVec = ov::ParameterVector{
            std::make_shared<ov::op::v0::Parameter>(inputPrecisions[0], inputDynamicShapes.front())};
        inputNodes1.push_back(conversion == ov::element::undefined
                                  ? paramVec.front()
    ...
    if (withQuantization) {
        ...
        eltwiseOps.push_back(
            make_eltwise(fq, inputNodes2[eltwiseOpTypes.size() - 1], eltwiseOpTypes[eltwiseOpTypes.size() - 1]));
...
std::vector<std::vector<ov::Shape>> inputShapes() {
    return {{{1, 1, 2, 3}, {1, 1, 2, 3}, {1, 1, 2, 3}, {1, 1, 2, 3}},
