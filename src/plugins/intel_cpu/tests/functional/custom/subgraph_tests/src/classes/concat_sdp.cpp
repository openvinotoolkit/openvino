...
void ConcatSDPTest::SetUp() {
    ...
    // For stateful-decode causal=true case, we constructed a new reference because the reference needs an explicit
    // attention_mask based on the
    //  accumulated history. Reusing the old path would make the reference rely on the implicit causal mask
    // generated by ScaledDotProductAttentionDecomposition which is standard top-left form and does not capture
    //  the required history offset.
    if (m_isCausal) {
