...
public:
    static std::string get_test_case_name(const ::testing::TestParamInfo<testParams>& obj) {
        const auto& use_compile_model_api = std::get<0>(obj.param);
        const auto& do_encryption         = std::get<1>(obj.param);
        const auto& inference_mode        = std::get<2>(obj.param);
        const auto& model_dtype           = std::get<3>(obj.param);

        std::ostringstream result;
        result << "use_compile_model_api=" << use_compile_model_api;
        result << "_do_encryption=" << do_encryption;
        result << "_inference_mode=" << inference_mode;
        result << "_model_dtype=" << model_dtype;
        return result.str();
    }
...
void CheckWeightlessCacheAccuracy::run() {
    ...
    compiled_model = core->compile_model(xml_path, ov::test::utils::DEVICE_CPU, config);
    ...
    if (use_compile_model_api) {
        imported_model = core->compile_model(xml_path, ov::test::utils::DEVICE_CPU, config);
    } else {
        auto ifstr = std::ifstream(cache_path, std::ifstream::binary);
        imported_model = core->import_model(ifstr, ov::test::utils::DEVICE_CPU, config_with_weights_path);
        ifstr.close();
    }
