...
> ConvSwishFQTestParams;

class ConvSwishFQ : public testing::WithParamInterface<ConvSwishFQTestParams>,
                  virtual public SubgraphBaseTest, public CPUTestsBase {
public:
    static std::string getTestCaseName(const testing::TestParamInfo<ConvSwishFQTestParams>& obj) {
...
protected:
    void SetUp() override {
        ...
        auto post_scale = op::v0::Constant::create(element::f32, {1, 4, 1, 1}, {0.02f, 0.025f, 0.03f, 0.035f});
        auto mul_after  = std::make_shared<op::v1::Multiply>(conv, post_scale);

        std::vector<float> bias_vals(4, withBias ? 0.1f : 0.0f);
        auto bias_const = op::v0::Constant::create(element::f32, {1, 4, 1, 1}, bias_vals);
        auto add_after  = std::make_shared<op::v1::Add>(mul_after, bias_const);

        //Swish
        auto fqInput = std::make_shared<op::v4::Swish>(add_after);
        ...
        function = create_ov_model(inputPrecision, input_params, matMul, "ConvSwishFQ");
    }
...
INSTANTIATE_TEST_SUITE_P(smoke_ConvSwishFQ_CPU,
                         ConvSwishFQ,
