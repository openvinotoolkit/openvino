...
std::vector<ov::AnyMap> generate_additional_config() {
    std::vector<ov::AnyMap> additional_config = {{{ov::hint::dynamic_quantization_group_size.name(), std::numeric_limits<uint64_t>::max()}}};
...
const std::vector<ov::test::ElementType> weights_precisions = {ov::element::i8, ov::element::i4};
...
INSTANTIATE_TEST_SUITE_P(smoke_MoeCompressedWeights,
                         ...
                         ::testing::Combine(::testing::ValuesIn(moe_params_smoke),
                                            ...
                                            ::testing::Values(-1),     // decompression group size
                                            ::testing::ValuesIn(generate_additional_config()),
