...
protected:
    ...
    void SetUp() override {
        constexpr size_t alignment = 64;  // bytes cache line size, to avoid denormals zeroing due to memory
                                          // reallocation in the input node implementation
...
TEST_F(DenormalNullifyCheck, smoke_CPU_Denormal_Check) {
    ...
    const indexInterval intervals[] = {{0, elemsCount / 2}, {elemsCount / 2, elemsCount}, {0, elemsCount}};
    ...
    for (auto& interval : intervals) {
        auto randomIndices =
            ov::test::utils::generateVector<ov::element::u32>(denormalsCount, interval.second, interval.first);
        std::unordered_set<decltype(randomIndices)::value_type> randomIndexSet(randomIndices.begin(),
                                                                               randomIndices.end());
