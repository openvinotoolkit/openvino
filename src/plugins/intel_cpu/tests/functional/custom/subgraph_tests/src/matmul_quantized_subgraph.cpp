...
//   (u8/s8 + s8)->f32
//   (u8/s8 + s8)->u8/s8
class MatmulBrgemmInt8Test : public testing::WithParamInterface<MatmulBrgemmInt8TestParams>,
...
protected:
    ...
    void SetUp() override {
        ...
        if (inType == ElementType::u8)
            fq1 =
                ov::test::utils::make_fake_quantize(inputParams[0], ngPrec, 256, {}, {0.0f}, {2.55f}, {0.0f}, {2.55f});
        else
            fq1 = ov::test::utils::make_fake_quantize(inputParams[0],
                                                      ngPrec,
                                                      256,
                                                      {},
                                                      {-1.28f},
        ...
        if (isFC) {
            ...
            auto weightsNode =
                ov::test::utils::deprecated::make_constant(ngPrec, weightShape, std::vector<float>{0.0f}, true);
            auto fq2 =
            ...
            auto biasWeightsNode =
                ov::test::utils::deprecated::make_constant(ngPrec, {}, std::vector<float>{0.0f}, true);
            matMul = std::make_shared<ov::op::v1::Add>(fc, biasWeightsNode);
        } else {
            auto fq2 = ov::test::utils::make_fake_quantize(inputParams[0],
                                                           ngPrec,
                                                           256,
                                                           {},
                                                           {-1.28f},
        ...
        if (outType == ElementType::u8)
            nodeBeforeConv =
                ov::test::utils::make_fake_quantize(matMul, ngPrec, 256, {}, {0.0f}, {2.55f}, {0.0f}, {2.55f});
        else if (outType == ElementType::i8)
            nodeBeforeConv =
                ov::test::utils::make_fake_quantize(matMul, ngPrec, 256, {}, {-1.28f}, {1.27f}, {-1.28f}, {1.27f});
        else
            nodeBeforeConv = matMul;

        // matmul->fq->matmul can cover x8*s8->x8 case
        auto filterWeightsShape = matMul->get_output_shape(0);
        auto filterWeightsNode = ov::test::utils::deprecated::make_constant(ov::element::f32,
                                                                            filterWeightsShape,
                                                                            std::vector<float>{},
                                                                            true);
        auto fq3 = ov::test::utils::make_fake_quantize(filterWeightsNode,
                                                       ngPrec,
                                                       256,
                                                       {},
                                                       {-1.28f},
...
const std::vector<CPUSpecificParams> matmulSpecificFilterParams = {{{}, {}, {"brgemm_avx2"}, "brgemm_avx2"},
                                                                   {{}, {}, {"jit_gemm"}, "jit_gemm"}};
