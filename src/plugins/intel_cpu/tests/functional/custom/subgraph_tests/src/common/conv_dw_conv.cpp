...
protected:
    void SetUp() override {
        ...
        auto conv_weights = ov::test::utils::make_constant(precision, ov::inplace_vector<size_t>{32, 32, 1, 1});
        auto conv = ov::test::utils::make_convolution(params[0],
                                                      conv_weights,
                                                      precision,
                                                      ov::inplace_vector<size_t>{1, 1},
                                                      ov::inplace_vector<size_t>{1, 1},
        ...
        auto dw_conv_weights = ov::test::utils::make_constant(precision, ov::inplace_vector<size_t>{32, 1, 1, 3, 3});
