...
ov::Tensor EltwiseLayerCPUTest::generate_eltwise_input(const ov::element::Type& type,
                                                       const ov::Shape& shape,
                                                       const bool adopt_intervals) {
    struct gen_params {
        uint32_t range;
        int32_t start_from;
        int32_t resolution;
...
void EltwiseLayerCPUTest::generate_inputs(const std::vector<ov::Shape>& targetInputStaticShapes) {
    ...
    for (size_t i = 0; i < funcInputs.size(); ++i) {
        ...
        inputs.insert({funcInput.get_node_shared_ptr(),
                       generate_eltwise_input(funcInput.get_element_type(),
                                              targetInputStaticShapes[i],
                                              (funcInput.get_element_type() == element::i32) ||
                                                  (funcInput.get_element_type() == element::u32) ||
                                                  (funcInput.get_element_type() == element::i8) ||
                                                  (funcInput.get_element_type() == element::u8))});
    }
}
...
void EltwiseLayerCPUTest::SetUp() {
    ...
    updateSelectedType(getPrimitiveType(eltwiseType, netType, shapes), netType, configuration);
    ...
    if (eltwiseType != utils::EltwiseTypes::BITWISE_NOT) {
        ...
        case utils::InputLayerType::CONSTANT: {
            ...
            auto data_tensor = generate_eltwise_input(netType,
                                                      shape,
...
std::string EltwiseLayerCPUTest::getPrimitiveType(
...
#    if defined(OPENVINO_ARCH_ARM64)
    if ((eltwise_type == utils::EltwiseTypes::ADD) || (eltwise_type == utils::EltwiseTypes::MULTIPLY) ||
        (eltwise_type == utils::EltwiseTypes::SUBTRACT) || (eltwise_type == utils::EltwiseTypes::DIVIDE)) {
        return "jit";
    }
#    endif
    return "acl";
...
const std::vector<ElementType>& netType() {
    static const std::vector<ElementType> netType = {ElementType::f32};
    return netType;
}
...
const std::vector<utils::EltwiseTypes>& eltwiseOpTypesDiffInp() {
    static const std::vector<utils::EltwiseTypes> eltwiseOpTypesDiffInp = {
...
const std::vector<CPUSpecificParams>& cpuParams_4D() {
    static const std::vector<CPUSpecificParams> cpuParams_4D = {CPUSpecificParams({nhwc, nhwc}, {nhwc}, {}, {}),
                                                                CPUSpecificParams({nchw, nchw}, {nchw}, {}, {})};
    return cpuParams_4D;
}

const std::vector<CPUSpecificParams>& cpuParams_5D() {
    static const std::vector<CPUSpecificParams> cpuParams_5D = {CPUSpecificParams({ndhwc, ndhwc}, {ndhwc}, {}, {}),
...
const std::vector<InputShape>& inShapes_4D_dyn_param() {
    static const std::vector<InputShape> inShapes_4D_dyn_param = {{// dynamic
                                                                   {-1, {2, 15}, -1, -1},
                                                                   // target
                                                                   {
                                                                       {3, 2, 1, 1},
...
const std::vector<InputShape>& inShapes_5D_dyn_param() {
    static const std::vector<InputShape> inShapes_5D_dyn_param = {
        ...
        {// dynamic
         {-1, {2, 25}, -1, -1, -1},
         // target
         {{1, 2, 5, 1, 5}, {3, 7, 1, 10, 1}, {3, 3, 4, 11, 6}}}};
