...
namespace ov {
namespace test {
using LRNParams = std::tuple<ElementType,            // data precision
...
class LRNLayerCPUTest : public testing::WithParamInterface<LRNParams>,
...
public:
    static std::string getTestCaseName(testing::TestParamInfo<LRNParams> obj) {
        ...
        result << inputPrecision << "_" << "IS=" << ov::test::utils::partialShape2str({inputShapes.first}) << "_"
               << "TS=(";
        for (const auto& shape : inputShapes.second) {
            result << ov::test::utils::vec2str(shape) << "_";
        }

        result << ")_alpha=" << alpha << "_beta=" << beta << "_bias=" << bias << "_size=" << size
               << "_axes=" << ov::test::utils::vec2str(axes);
        return result.str();
    }
...
const std::vector<std::vector<std::int64_t>> axes = {{1}, {2, 3}, {3, 2}, {1, 2, 3}};
...
const auto testCases = ::testing::Combine(::testing::ValuesIn(inputPrecisions),
