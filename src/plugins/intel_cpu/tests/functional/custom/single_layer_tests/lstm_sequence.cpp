...
public:
    static std::string getTestCaseName(const testing::TestParamInfo<LSTMSequenceCpuSpecificParams>& obj) {
        ...
        std::tie(inputShapes, seqMode, activations, clip, direction, netPrecision, cpuParams, additionalConfig) =
            obj.param;
...
protected:
    void SetUp() override {
        ...
        const size_t batchSize = inputDynamicShapes[0][0].is_static()   ? inputDynamicShapes[0][0].get_length()
                                 ...
                                 : inputDynamicShapes.size() > 3 && inputDynamicShapes[3][0].is_static()
                                     ? inputDynamicShapes[3][0].get_length()
                                     : 1lu;
        ...
        std::vector<ov::Shape> WRB = {{numDirections, 4 * hiddenSize, inputSize},
                                      {numDirections, 4 * hiddenSize, hiddenSize},
                                      {numDirections, 4 * hiddenSize},
                                      {batchSize}};
...
INSTANTIATE_TEST_SUITE_P(nightly_static_bf16_BatchSizeOne,
                         LSTMSequenceCPUTest,
...
INSTANTIATE_TEST_SUITE_P(smoke_dynamic_batch,
                         LSTMSequenceCPUTest,
...
INSTANTIATE_TEST_SUITE_P(nightly_dynamic_bf16_BatchSizeOne,
                         LSTMSequenceCPUTest,
                         ::testing::Combine(::testing::ValuesIn({dynamicShapes[4]}),
                                            ::testing::ValuesIn(mode),
                                            ::testing::ValuesIn(activations),
...
INSTANTIATE_TEST_SUITE_P(smoke_dynamic_mixedDynamicStaticBatch,
                         LSTMSequenceCPUTest,
