...
class NmsLayerCPUTest : public testing::WithParamInterface<NmsParams>,
...
public:
    static std::string getTestCaseName(const testing::TestParamInfo<NmsParams>& obj) {
        ...
        if (!bounds.empty()) {
            ...
            result << "BatchesBounds=" << bounds[BATCHES] << "_BoxesBounds=" << bounds[BOXES]
                   << "_ClassesBounds=" << bounds[CLASSES] << "_";
        }
        ...
        result << "boxEncoding=" << boxEncoding << "_sortResDescend=" << sortResDescend << "_outType=" << outType
               << "_";
...
protected:
    void SetUp() override {
        ...
        if (!bounds.empty()) {
            inputDynamicShapes = std::vector<ov::PartialShape>{{bounds[BATCHES], bounds[BOXES], 4},
                                                               {bounds[BATCHES], bounds[CLASSES], bounds[BOXES]}};
        } else {
            ...
            inputDynamicShapes =
                std::vector<ov::PartialShape>{{numBatches, numBoxes, 4}, {numBatches, numClasses, numBoxes}};
        }

        for (const auto& ts : targetInDims) {
            size_t numBatches, numBoxes, numClasses;
            std::tie(numBatches, numBoxes, numClasses) = ts;
            targetStaticShapes.push_back(
                std::vector<ov::Shape>{{numBatches, numBoxes, 4}, {numBatches, numClasses, numBoxes}});
        ...
        if (maxOutBoxesType == ov::test::utils::InputLayerType::PARAMETER) {
            ...
            maxOutBoxesPerClassNode =
                ov::test::utils::deprecated::make_constant(maxBoxPrec,
                                                           ov::Shape{},
                                                           std::vector<int32_t>{maxOutBoxesPerClass});
        }
        ...
        auto softNmsSigmaNode =
            ov::test::utils::deprecated::make_constant(thrPrec, ov::Shape{}, std::vector<float>{softNmsSigma})
                ->output(0);
...
const std::vector<InputShapeParams> inShapeParams = {
    InputShapeParams{std::vector<ov::Dimension>{-1, -1, -1},
                     std::vector<TargetShapeParams>{TargetShapeParams{2, 50, 50},
                                                    TargetShapeParams{3, 100, 5},
                                                    TargetShapeParams{1, 10, 50}}},
    InputShapeParams{std::vector<ov::Dimension>{{1, 5}, {1, 100}, {10, 75}},
                     std::vector<TargetShapeParams>{TargetShapeParams{4, 15, 10},
...
const std::vector<ov::test::utils::InputLayerType> maxBoxInputTypes = {ov::test::utils::InputLayerType::PARAMETER,
