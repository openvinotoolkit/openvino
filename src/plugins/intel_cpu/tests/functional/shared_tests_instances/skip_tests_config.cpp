...
std::vector<std::string> disabledTestPatterns() {
    std::vector<std::string> retVector{
        ...
        R"(.*smoke_CompareWithRefs_4D_Blocked_Blocked_Fusing/EltwiseLayerCPUTest\.CompareWithRefs/IS=\(\[]_\)_TS=\(\(2\.4\.4\.1\)_\)_eltwise_op_type=Sum_secondary_input_type=PARAMETER_opType=VECTOR_model_type=f32_InType=dynamic_OutType=dynamic_trgDev=CPU_config_item=INFERENCE_PRECISION_HINT=f32_inFmts=nChw16c\.nChw16c_outFmts=nChw16c_Fused=FakeQuantize\(PerChannel\)\.Sigmoid\.FakeQuantize\(PerTensor\)_enforceSnippets=0.*)",
