...
static std::map<std::string, std::string> get_split_dim_m_config() {
...
INSTANTIATE_TEST_SUITE_P(smoke_Snippets_MHA_4D_SplitDimensionM, MHA,
                         ::testing::Combine(
                                 ...
                                 ::testing::Values(1),
                                 ::testing::Values(ov::test::utils::DEVICE_CPU),
                                 ::testing::Values(get_split_dim_m_config())),
                         MHA::getTestCaseName);
...
INSTANTIATE_TEST_SUITE_P(smoke_Snippets_MHAWOTranspose_3D_SplitDimensionM, MHAWOTranspose,
                         ::testing::Combine(
                                 ::testing::Values(std::vector<ov::PartialShape>{{4, 512, 39}, {4, 39, 512}, {4, 512, 39}}),
                                 ::testing::ValuesIn(precision_f32(3)),
                                 ::testing::Values(ov::element::f32),
                                 ::testing::ValuesIn({true}),  // Need to support False for graph builder in tests
                                 ::testing::Values(8),
...
INSTANTIATE_TEST_SUITE_P(smoke_Snippets_MHAINT8MatMul_SplitDimensionM, MHAINT8MatMul,
                         ::testing::Combine(
                                 ::testing::Values(std::vector<ov::PartialShape>{{1, 128, 8, 64}, {1, 128, 8, 64}, {1, 8, 1, 1}, {1, 128, 8, 64}}),
                                 ::testing::Values(std::vector<element::Type>{}),
                                 ::testing::Values(ov::element::f32),
                                 ::testing::Values(false), // The graph doesn't contain Multiply
                                 ::testing::Values(18),
                                 ::testing::Values(11),     // FQx3 on inputs + MHA + Transpose on output + Deq Mul + 5 Reshapes
                                 ::testing::Values(5),     // FQx3 on inputs + MHA + Deq Mul
                                 ::testing::Values(ov::test::utils::DEVICE_CPU),
                                 ::testing::Values(get_split_dim_m_config())),
                         MHA::getTestCaseName);
