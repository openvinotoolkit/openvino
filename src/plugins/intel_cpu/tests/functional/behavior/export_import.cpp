...
std::shared_ptr<ov::Model> MakeMatMulModel() {
    ...
    auto matmul_const = ov::test::utils::make_constant(precision, {4096, 1024}, std::vector<float>{}, true);
    auto matmul = std::make_shared<ov::op::v0::MatMul>(params[0], matmul_const);

    auto add_const = ov::test::utils::make_constant(precision, {1, 1024}, std::vector<float>{}, true);
