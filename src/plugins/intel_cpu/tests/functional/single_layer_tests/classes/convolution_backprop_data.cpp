...
void DeconvolutionLayerCPUTest::init_ref_function(std::shared_ptr<ov::Model>& funcRef,
...
std::shared_ptr<ov::Model> DeconvolutionLayerCPUTest::createGraph(const std::vector<ov::PartialShape>& inShapes,
    ...
    if (!outShapeData.empty()) {
        if (outShapeType == ngraph::helpers::InputLayerType::PARAMETER) {
            ...
            auto outShapeParam =
                std::make_shared<ngraph::opset8::Parameter>(ngraph::element::i32, inputDynamicShapes.back());
            params.push_back(outShapeParam);
            outShapeNode = outShapeParam;
        } else {
            outShapeNode = ngraph::opset8::Constant::create(ngraph::element::i32,
    ...
    if (!outShapeData.empty()) {
        ...
        deconv = ngraph::builder::makeConvolutionBackpropData(params[0],
                                                              outShapeNode,
                                                              prec,
                                                              kernel,
                                                              stride,
