...
void RandomUniformLayerTestCPU::SetUp() {
    ...
    if (!const_in_1) {
        ...
        inputs.push_back(ov::test::utils::make_constant(shape_prc, {m_output_shape.size()}, m_output_shape));
    }
    if (!const_in_2) {
        ...
        inputs.push_back(ov::test::utils::make_constant(output_prc, {1}, std::vector<double>{m_min_val}));
    }
    if (!const_in_3) {
        in_shapes.push_back({{}, {{1}}});
        in_params.push_back(std::make_shared<ov::op::v0::Parameter>(output_prc, ov::PartialShape{1}));
        ...
        inputs.push_back(ov::test::utils::make_constant(output_prc, {1}, std::vector<double>{m_max_val}));
    }
