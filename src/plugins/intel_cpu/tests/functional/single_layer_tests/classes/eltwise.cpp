    ...
    if (type.is_real()) {
        ...
        switch (eltwiseType) {
            case ngraph::helpers::EltwiseTypes::MULTIPLY:
                CommonTestUtils::fill_data_random(tensor.data<int64_t>(), tensor.get_size(), int64_t(100), int64_t(-50));
                break;
            default:
                CommonTestUtils::fill_data_random(tensor.data<int64_t>(), tensor.get_size(), int64_t(INT64_MAX), int64_t(INT64_MIN / 2));
                break;
        }
        return tensor;
    } else {
