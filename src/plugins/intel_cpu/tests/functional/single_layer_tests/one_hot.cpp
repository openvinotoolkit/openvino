...
public:
    static std::string getTestCaseName(const testing::TestParamInfo<oneHotCPUTestParams>& obj) {
        ...
        std::tie(inputShape, axis, inputType, depth, onValue, offValue, inPrc, outPrc, config, cpuParams) = obj.param;
...
protected:
    void SetUp() override {
        ...
        selectedType = makeSelectedTypeStr(selectedType, inType, configuration);
