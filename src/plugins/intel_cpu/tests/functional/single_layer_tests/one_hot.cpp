...
public:
    static std::string getTestCaseName(const testing::TestParamInfo<oneHotCPUTestParams>& obj) {
        ...
        std::tie(inputShape, axis, inputType, depth, onValue, offValue, inPrc, outPrc, cpuParams) = obj.param;
...
protected:
    void SetUp() override {
        ...
        std::pair<ngraph::helpers::InputLayerType, bool> inputType;
        CPUSpecificParams cpuParams;
        std::tie(inputShape, Axis, inputType, Depth, OnValue, OffValue, inType, outType, cpuParams) = this->GetParam();
    ...
    std::shared_ptr<ov::Model> createFunction(bool depthConst) {
        ...
        if (depthConst) {
            depth = ov::opset1::Constant::create(inType, ov::Shape{ }, {Depth});
        } else {
            auto depthParam = std::make_shared<ov::opset1::Parameter>(inType, ov::Shape{ });
            depthParam->set_friendly_name("ParamDepth");
            params.push_back(depthParam);
            depth = depthParam;
        }
        auto paramOuts = ngraph::helpers::convert2OutputVector(ngraph::helpers::castOps2Nodes<ov::opset1::Parameter>(params));
        auto on_value_const = std::make_shared<ov::opset1::Constant>(outType, ov::Shape{ }, OnValue);
        auto off_value_const = std::make_shared<ov::opset1::Constant>(outType, ov::Shape{ }, OffValue);
        auto oneHot = std::make_shared<ov::opset1::OneHot>(paramOuts[0], depth, on_value_const, off_value_const, Axis);
