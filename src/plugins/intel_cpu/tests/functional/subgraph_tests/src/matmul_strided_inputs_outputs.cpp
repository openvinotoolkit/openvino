...
protected:
    void SetUp() override {
        ...
        const auto splitOutputNodes = ov::helpers::convert2OutputVector(ov::helpers::castOps2Nodes<op::Parameter>(splitInputParams));
        const auto split = ov::builder::makeSplit(splitOutputNodes[0], ngPrec, 2 /* splits */, 1 /* 2nd axis */);
        ...
        const auto concatOutputNodes = ov::helpers::convert2OutputVector(ov::helpers::castOps2Nodes<op::Parameter>(concatInputParams));
        const auto concat = ov::builder::makeConcat(concatOutputNodes, 2);

        const auto matMul1 = ov::builder::makeMatMul(split->output(0), concat, false, false);
        ...
        const auto matmulOutputNodes = ov::helpers::convert2OutputVector(ov::helpers::castOps2Nodes<op::Parameter>(matmulInputParams));

        const auto matMul2 = ov::builder::makeMatMul(split->output(1), matmulOutputNodes[0], false, false);

        const auto concatMatMuls = ov::builder::makeConcat({matMul1, matMul2}, 2 /* 3rd axis */);
