...
protected:
    void SetUp() override {
        ...
        auto params = ov::test::utils::builder::makeParams(ngPrec, {splitShape});
        ...
        auto fc1secondInput = std::make_shared<ov::op::v0::Constant>(ngPrec, fcWeightsShape);
        const auto fc1 = builder::makeMatMul(split->output(0), fc1secondInput, false, false);

        auto fc2secondInputB = std::make_shared<ov::op::v0::Constant>(ngPrec, fcWeightsShape);
