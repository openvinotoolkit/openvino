...
protected:
    void SetUp() override {
        ...
        auto FQ = ov::builder::makeFakeQuantize(paramOuts[1], ngPrc, 256, {}, {-2.8215785026550293}, {2.799535036087036},
                                                      {-2.8215785026550293}, {2.799535036087036});
        auto FQ_0 = ov::builder::makeFakeQuantize(paramOuts[1], ngPrc, 256, {}, {-5.031249523162842}, {4.991942882537842},
                                                    {-5.031249523162842}, {4.991942882537842});

        auto Add_0 = ov::builder::makeEltwise(FQ_0, FQ, EltwiseTypes::ADD);

        auto FQ_1 = ov::builder::makeFakeQuantize(paramOuts[0], ngPrc, 256, {}, {-2.122633457183838}, {2.106050491333008},
                                                      {-2.122633457183838}, {2.106050491333008});

        auto Const = ov::builder::makeConstant(ngPrc, {128, 512, 1}, std::vector<float>{-0.0512377955019474}, false);
        auto FQ_2 = ov::builder::makeFakeQuantize(Const, ngPrc, 255, {128, 1, 1}, {-0.56387859582901}, {0.56387859582901},
        ...
        auto Add = ov::builder::makeEltwise(Add_0, Conv, EltwiseTypes::ADD);

        auto FQ_11 = ov::builder::makeFakeQuantize(paramOuts[0], ngPrc, 256, {}, {-3.2050728797912598}, {3.1800332069396973},
                                                      {-3.2050728797912598}, {3.1800332069396973});

        auto Const_ = ov::builder::makeConstant(ngPrc, {128, 512, 1}, std::vector<float>{-0.001183388871140778}, false);
        auto FQ_22 = ov::builder::makeFakeQuantize(Const_, ngPrc, 255, {128, 1, 1}, {-0.325547456741333}, {0.325547456741333},
        ...
        auto Add2 = ov::builder::makeEltwise(Add, Conv2, EltwiseTypes::ADD);
        auto relu3 = ov::builder::makeActivation(Add2, ngPrc, ov::helpers::ActivationTypes::Relu);
