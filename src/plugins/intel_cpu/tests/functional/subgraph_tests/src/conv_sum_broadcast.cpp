...
public:
    ...
    void SetUp() override {
        ...
        if (bias) {
            auto biasNode = ov::test::utils::make_constant<float>(ov::element::Type_t::f32, ov::Shape({1, _convOutChannels, 1, 1}), {}, true);
...
public:
    ...
    std::shared_ptr<ov::Node> makeConv(const ov::ParameterVector& inputParams) override {
        ...
        auto filterWeightsNode = ov::test::utils::make_constant<int8_t>(ov::element::i8, filterShape, {}, true);
