...
public:
    ...
    virtual ngraph::ParameterVector makeParams() {
        return ov::test::utils::builder::make_dynamic_params(ngraph::element::f32, inputDynamicShapes);
    }
...
public:
    ...
    std::shared_ptr<ngraph::Node> makeConv(const ngraph::ParameterVector& inputParams) override {
        ...
        auto inputParamsFP32 = ov::test::utils::builder::make_dynamic_params(element::f32, { inputParams.front()->get_partial_shape() });
