...
protected:
    void SetUp() override {
        ...
        const auto mmOutputNodes = ov::helpers::convert2OutputVector(ov::helpers::castOps2Nodes<op::Parameter>(mmParams));

        const auto mm = ov::builder::makeMatMul(mmOutputNodes[0], mmConst, false, false);
        auto sum = ov::builder::makeEltwise(constShift, mm, ov::helpers::EltwiseTypes::ADD);
        auto fq = ov::builder::makeFakeQuantize(sum, ngraph::element::f32, 256, {}, {-8.0f}, {7.0f}, {-8.0f}, {7.0f});
