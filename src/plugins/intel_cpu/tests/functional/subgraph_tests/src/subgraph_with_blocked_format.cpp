...
protected:
    void SetUp() override {
        ...
        auto weights = ov::test::utils::make_constant(type, Shape{32, 32, 1, 1}, std::vector<float>{}, true);
        ...
        auto mul = std::make_shared<ov::opset8::Multiply>(
            reshape_after,
            ov::test::utils::make_constant(type, Shape{32, 1, 1}, std::vector<float>{}, true));
