...
protected:
    void SetUp() override {
        ...
        auto conv = std::make_shared<ov::op::v1::Convolution>(param, weights, Strides{1, 1}, CoordinateDiff{0, 0}, CoordinateDiff{0, 0}, Strides{1, 1});
        auto mean = std::make_shared<ov::op::v1::ReduceMean>(conv, opset8::Constant::create(element::i32, Shape{2}, {2, 3}), true);
        auto reshape_before = std::make_shared<ov::op::v1::Reshape>(mean, opset8::Constant::create(element::i32, Shape{3}, {0, 16, -1}), true);
        auto mvn = std::make_shared<ov::op::v6::MVN>(reshape_before, opset8::Constant::create(element::i32, Shape{1}, {2}),
        ...
        auto mul = std::make_shared<ov::op::v1::Multiply>(reshape_after, builder::makeConstant(type, Shape{32, 1, 1}, std::vector<float>{}, true));
        auto add = std::make_shared<ov::op::v1::Add>(mul, builder::makeConstant(type, Shape{32, 1, 1}, std::vector<float>{}, true));
