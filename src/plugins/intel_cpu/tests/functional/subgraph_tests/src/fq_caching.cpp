...
protected:
    void SetUp() override {
        ...
        auto makeFQ = [&](int i) {
            ...
            auto il = ov::test::utils::make_constant(ngInPrec, ranges[0], extendData(rangesBounds[0],
                std::accumulate(ranges[0].begin(), ranges[0].end(), 1, std::multiplies<size_t>())));
            auto ih = ov::test::utils::make_constant(ngInPrec, ranges[1], extendData(rangesBounds[1],
                std::accumulate(ranges[1].begin(), ranges[1].end(), 1, std::multiplies<size_t>())));
            auto ol = ov::test::utils::make_constant(ngInPrec, ranges[2], extendData(rangesBounds[2],
                std::accumulate(ranges[2].begin(), ranges[2].end(), 1, std::multiplies<size_t>())));
            auto oh = ov::test::utils::make_constant(ngInPrec, ranges[3], extendData(rangesBounds[3],
        ...
        if (!reshapeShape.empty()) {
            auto reshapeConstNode =
                ov::test::utils::make_constant(ov::element::i32, {reshapeShape.size()}, reshapeShape);
