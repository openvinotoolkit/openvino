...
protected:
    void SetUp() override {
        ...
        if (secondaryInputType == ngraph::helpers::InputLayerType::PARAMETER) {
            ...
            ngraphParam = ov::test::utils::builder::makeDynamicParams(inputPrecisions[0], {inputDynamicShapes.front()});
            for (size_t i = 1; i < inputPrecisions.size(); i++) {
                std::vector<float> ngraphInput1Data(ngraph::shape_size(targetStaticShapes[0][i]));
                ngraphInputs.push_back(ov::test::utils::builder::makeConstant(inputPrecisions[i], targetStaticShapes[0][i],
