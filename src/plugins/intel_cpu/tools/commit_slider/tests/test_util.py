...
from test_data import TestData, TestError
...
def getVersionList(td: TestData):
    ...
    for id, patch in enumerate(patchList):
        ...
        newVersion = {"content": newData, "state": state, "comment": patch["comment"]}
...
def makeRepoContent(td: TestData):
    ...
    td.repoStructure["files"] = formatJSON(
        ...
        lambda content: content.format(repoName=td.repoName, mainFile=td.mainFile),
    )
...
def getBordersByTestData(td: TestData):
    return getMeaningfullCommitTail(td.start), getMeaningfullCommitTail(td.end)
...
def getExpectedCommit(td: TestData):
    ...
    breakList = [
        i
        for i in range(len(markedVersionList))
...
def getActualCommit(td: TestData):
    ...
    # clear temp data
    [shutil.rmtree(dir) for dir in [td.repoName, td.cachePath, td.logPath]]
    os.remove(testCfg)
...
def parseSliderOutput(sliderOutput: str):
    ...
    matcher = re.search(pattern, sliderOutput, flags=re.MULTILINE)
    ...
    if matcher is not None:
        ...
        raise TestError("Unexpected output: {}".format(sliderOutput))


def requireBinarySearchData(td: TestData, rsc: map):
    td.requireTestData(lambda td, rsc: setattr(td, "commonRsc", rsc["binarySearchRes"]))
    td.requireTestData(
        lambda td, rsc: [
            setattr(td, key, rsc[td.getTestName()][key])
            for key in ["testCfg", "patchedFile", "repoName"]
        ]
    )

    [
        setattr(td, key, td.commonRsc[key])
        for key in ["repoStructure", "cachePath", "logPath", "mainFile", "repoPath"]
    ]
    td.patternPrefix = td.commonRsc["patchGeneratorPrefix"]
    td.patternPostfix = td.commonRsc["patchGeneratorPostfix"]
    td.pattern = "{pre}(.+?){post}".format(pre=td.patternPrefix, post=td.patternPostfix)
