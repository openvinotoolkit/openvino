...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--filter",
        type=list,
        default=["xbyak.h", "jit_generator.hpp", "primitive.hpp", "xbyak_mnemonic.h"],
    )
    parser.add_argument(
        "--maxcnt",
        type=int,
        help="Maximum number of entries from call stack to show",
        default=3,
    )
    ...
    print("objdump...")
    disassemble = subprocess.check_output(
        f"objdump -D -b binary -mi386:x86-64 -M intel {args.bin}",
        shell=True,
        encoding="utf-8",
    )
