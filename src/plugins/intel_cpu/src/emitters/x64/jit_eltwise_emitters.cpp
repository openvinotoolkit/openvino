...
void jit_add_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_add_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_add_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    switch (exec_prc_) {
        ...
        default: OPENVINO_THROW("jit_add_emitter doesn't support precision '", exec_prc_, "'");
    }
}
...
void jit_mul_add_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_mul_add_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_mul_add_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    auto uni_madd_xmm = [this](const Xmm &vmm_dst, const Xmm &vmm_src_0, const Xmm &vmm_src_1, const Xmm &vmm_src_2) {
        switch (exec_prc_) {
            ...
            default: OPENVINO_THROW("jit_mul_add_emitter doesn't support precision '", exec_prc_, "'");
        }
    };
...
void jit_subtract_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_subtract_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_subtract_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    switch (exec_prc_) {
        ...
        default: OPENVINO_THROW("jit_subtract_emitter doesn't support precision '", exec_prc_, "'");
    }
}
...
void jit_multiply_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_multiply_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_multiply_emitter::emit_isa(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    ...
    if (in_vec_idxs.size() > 1) {
        ...
        OPENVINO_THROW("jit_multiply_emitter has invalid inputs number.");
    }
    Vmm vmm_dst = Vmm(out_vec_idxs[0]);
    ...
    switch (exec_prc_) {
        ...
        case Precision::I64: {
            if (isa == x64::avx512_core) {
                ...
                if (aux_vec_idxs.size() < 2) {
                    OPENVINO_THROW("jit_multiply_emitter has invalid number of aux vectors.");
                }
        ...
        default: OPENVINO_THROW("jit_multiply_emitter doesn't support precision '", exec_prc_, "'");
    }
}
...
void jit_divide_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_divide_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_divide_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    using Vmm = typename conditional3<isa == x64::sse41, Xmm, isa == x64::avx2, Ymm, Zmm>::type;
    if (in_vec_idxs.size() < 2 || out_vec_idxs.size() < 1) {
        OPENVINO_THROW("jit_divide_emitter has invalid number of input/output indices.");
    }
    Vmm vmm_src_0 = Vmm(in_vec_idxs[0]);
    Vmm vmm_src_1 = Vmm(in_vec_idxs[1]);
    Vmm vmm_dst = Vmm(out_vec_idxs[0]);
    ...
    // As WA we emulate its behavior via fp divide followed by rounding to round_type
    switch (exec_prc_) {
        case Precision::FP32: {
        ...
        case Precision::I32: {
            if (aux_vec_idxs.size() < 1) {
                OPENVINO_THROW("jit_divide_emitter has invalid number of auxiliary indices: ", aux_vec_idxs.size());
            }
            Vmm vmm_aux_0 = Vmm(aux_vec_idxs[0]);
            ...
            if (round_type == RoundType::truncation) {
                h->uni_vcvttps2dq(vmm_dst, vmm_dst);
            } else {
                h->uni_vroundps(vmm_dst, vmm_dst, round_type);
        ...
        case Precision::I64: {
            if (isa == x64::avx512_core) {
                ...
                if (round_type == RoundType::truncation) {
                    h->vcvttpd2qq(vmm_dst, vmm_dst);
                } else {
                    h->uni_vroundpd(vmm_dst, vmm_dst, round_type);
                ...
                // Convert vmm_src_0 i64->f64
                h->uni_vblendps(vmm_aux_0, vmm_src_0, table_val_64("i64_f64_cvt_mask_0"), 0b10101010);
                h->uni_vpsrlq(vmm_dst, vmm_src_0, 32);
                h->uni_vpxor(vmm_dst, vmm_dst, table_val_64("i64_f64_cvt_mask_1"));
                h->uni_vsubpd(vmm_dst, vmm_dst, table_val_64("i64_f64_cvt_mask_2"));
                h->uni_vaddpd(vmm_dst, vmm_dst, vmm_aux_0);

                if (second_is_float) {
                    h->uni_vdivpd(vmm_dst, vmm_dst, vmm_src_1);
                } else {
                    // Convert vmm_src_1 i64->f64
                    h->uni_vblendps(vmm_aux_0, vmm_src_1, table_val_64("i64_f64_cvt_mask_0"), 0b10101010);
                    h->uni_vpsrlq(vmm_aux_1, vmm_src_1, 32);
                    h->uni_vpxor(vmm_aux_1, vmm_aux_1, table_val_64("i64_f64_cvt_mask_1"));
                    h->uni_vsubpd(vmm_aux_1, vmm_aux_1, table_val_64("i64_f64_cvt_mask_2"));
                    h->uni_vaddpd(vmm_aux_1, vmm_aux_1, vmm_aux_0);

                    h->uni_vdivpd(vmm_dst, vmm_dst, vmm_aux_1);
                }
                h->uni_vroundpd(vmm_dst, vmm_dst, round_type);

                // Convert vmm_dst f64->i64
                h->uni_vaddpd(vmm_dst, vmm_dst, table_val_64("f64_i64_cvt_mask"));
                h->uni_vpsubq(vmm_dst, vmm_dst, table_val_64("f64_i64_cvt_mask"));
            } break;
        }
        default: OPENVINO_THROW("jit_divide_emitter doesn't support precision '", exec_prc_, "'");
    }
}
...
void jit_divide_emitter::register_table_entries() {
   if (host_isa_ != x64::avx512_core && exec_prc_ == Precision::I64) {
        push_arg_entry_of_64("i64_f64_cvt_mask_0", 0x4330000000000000, true);
        push_arg_entry_of_64("i64_f64_cvt_mask_1", 0x4530000080000000, true);
        push_arg_entry_of_64("i64_f64_cvt_mask_2", 0x4530000080100000, true);

        push_arg_entry_of_64("f64_i64_cvt_mask", 0x433800002150d000, true);
   }
}
...
std::set<std::vector<element::Type>> jit_floor_emitter::get_supported_precisions(const std::shared_ptr<ov::Node>& node) {
    return {{element::f32}, {element::i32}, {element::i64}};
}
...
void jit_floor_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_floor_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_floor_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    using Vmm = typename conditional3<isa == x64::sse41, Xmm, isa == x64::avx2, Ymm, Zmm>::type;
    if (in_vec_idxs.size() < 1 || out_vec_idxs.size() < 1) {
        OPENVINO_THROW("jit_floor_emitter has invalid number of input/output indices.");
    }

    Vmm vmm_src = Vmm(in_vec_idxs[0]);
    Vmm vmm_dst = Vmm(out_vec_idxs[0]);
    switch (exec_prc_) {
        case Precision::FP32: h->uni_vroundps(vmm_dst, vmm_src, RoundType::floor); break;
        case Precision::I32:
        case Precision::I64:
        ...
        default: OPENVINO_THROW("jit_floor_emitter doesn't support precision '", exec_prc_, "'");
    }
}
...
void jit_ceiling_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs,
    ...
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_ceiling_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_ceiling_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    using Vmm = typename conditional3<isa == x64::sse41, Xmm, isa == x64::avx2, Ymm, Zmm>::type;
    if (in_vec_idxs.size() < 1 || out_vec_idxs.size() < 1) {
        OPENVINO_THROW("jit_ceiling_emitter has invalid number of input/output indices.");
    }

    Vmm vmm_src = Vmm(in_vec_idxs[0]);
    Vmm vmm_dst = Vmm(out_vec_idxs[0]);
    switch (exec_prc_) {
        case Precision::FP32: h->uni_vroundps(vmm_dst, vmm_src, RoundType::ceil); break;
        case Precision::I32:
        case Precision::I64:
            if (vmm_dst.getIdx() != vmm_src.getIdx()) {
                h->uni_vmovups(vmm_dst, vmm_src);
...
void jit_floor_mod_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_floor_mod_emitter doesn't support ISA '", host_isa_, "'");
    }
}
    ...
    switch (exec_prc_) {
        case Precision::FP32: {
            if (isa == x64::sse41) {
                ...
                h->uni_vroundps(vmm_aux_0, vmm_aux_0, RoundType::floor);
                h->uni_vmulps(vmm_aux_0, vmm_aux_0, vmm_src_1);
                h->uni_vsubps(vmm_dst, vmm_dst, vmm_aux_0);
            } else {
                if (vmm_dst.getIdx() != vmm_src_0.getIdx()) {
                    h->uni_vdivps(vmm_dst, vmm_src_0, vmm_src_1);
                    h->uni_vroundps(vmm_dst, vmm_dst, RoundType::floor);
        ...
        case Precision::I64: {
            ...
            if (isa == x64::avx512_core) {
                ...
                h->uni_vroundpd(vmm_dst, vmm_dst, RoundType::floor);
                h->vfnmadd132pd(vmm_dst, vmm_aux_0, vmm_aux_1);

                h->vcvtpd2qq(vmm_dst, vmm_dst);
            } else {
                Vmm vmm_aux2 = Vmm(aux_vec_idxs[2]);
                h->uni_vmovups(vmm_aux2, table_val_64("dbl_cvt_mask"));
        ...
        default: OPENVINO_THROW("jit_floor_mod_emitter doesn't support precision '", exec_prc_, "'");
    }
}
...
void jit_floor_mod_emitter::register_table_entries() {
    if (host_isa_ != x64::avx512_core && exec_prc_ == Precision::I64) {
        push_arg_entry_of_64("dbl_cvt_mask",  0x433800002150d000, true);
    }
}
...
void jit_mod_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_mod_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_mod_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    using Vmm = typename conditional3<isa == x64::sse41, Xmm, isa == x64::avx2, Ymm, Zmm>::type;
    if (in_vec_idxs.size() < 2 || out_vec_idxs.size() < 1 || aux_vec_idxs.size() < aux_vecs_count()) {
        OPENVINO_THROW("jit_mod_emitter has invalid number of input/output indices.");
    }
    ...
    if (isa == x64::sse41) {
        ...
        h->uni_vroundps(vmm_aux_0, vmm_aux_0, RoundType::truncation);
        h->uni_vmulps(vmm_aux_0, vmm_aux_0, vmm_src_1);
        h->uni_vsubps(vmm_dst, vmm_dst, vmm_aux_0);
    } else {
        if (vmm_dst.getIdx() != vmm_src_0.getIdx())
...
void jit_maximum_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_maximum_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_maximum_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    if (in_vec_idxs.size() > 1) {
        ...
        OPENVINO_THROW("jit_maximum_emitter has invalid inputs number.");
    }
    Vmm vmm_dst = Vmm(out_vec_idxs[0]);
    ...
    switch (exec_prc_) {
        ...
        case Precision::I64: {
            if (isa == x64::avx512_core) {
                ...
                if (aux_vec_idxs.size() < 1) {
                    OPENVINO_THROW("jit_maximum_emitter has invalid number of aux vectors.");
                }
        ...
        default: OPENVINO_THROW("jit_maximum_emitter doesn't support precision '", exec_prc_, "'");
    }
}
...
void jit_minimum_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_minimum_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_minimum_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    if (in_vec_idxs.size() > 1) {
        ...
        OPENVINO_THROW("jit_minimum_emitter has invalid inputs number.");
    }
    Vmm vmm_dst = Vmm(out_vec_idxs[0]);
    ...
    switch (exec_prc_) {
        ...
        case Precision::I64: {
            if (isa == x64::avx512_core) {
                ...
                if (aux_vec_idxs.size() < 1) {
                    OPENVINO_THROW("jit_minimum_emitter has invalid number of aux vectors.");
                }
                auto vmm_aux = Vmm(aux_vec_idxs[0]);
                if (isa == x64::avx2) {
                    h->vpcmpgtq(vmm_aux, vmm_src_0, op_src_1);
        ...
        default: OPENVINO_THROW("jit_minimum_emitter doesn't support precision '", exec_prc_, "'");
    }
}
...
void jit_squared_difference_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_squared_difference_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_squared_difference_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    if (vmm_src_0.getIdx() == vmm_src_1.getIdx()) {
    ...
    switch (exec_prc_) {
        ...
        case Precision::I64: {
            if (isa == x64::avx512_core) {
                ...
                Vmm vmm_aux_0 = Vmm(aux_vec_idxs[0]);
                // There is no multiply int64 instruction on AVX2 and SSE41, thus the WA is used.
                // Represent inputs as vmm_src_0 -> AB and op_src_1 -> CD (== AB)
        ...
        default: OPENVINO_THROW("jit_squared_difference_emitter doesn't support precision '", exec_prc_, "'");
    }
}
...
void jit_power_dynamic_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_power_dynamic_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_equal_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_equal_emitter doesn't support ISA '", host_isa_, "'");
    }
}
    ...
    if (isa == x64::sse41 || isa == x64::avx2) {
        ...
        switch (exec_prc_) {
            ...
            case Precision::I64:
                ...
                break;
            default: OPENVINO_THROW("jit_equal_emitter doesn't support precision '", exec_prc_, "'");
        }
    } else {
        switch (exec_prc_) {
            case Precision::FP32:
...
void jit_not_equal_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_not_equal_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_greater_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_greater_emitter doesn't support ISA '", host_isa_, "'");
    }
}
    ...
    if (isa == x64::sse41 || isa == x64::avx2) {
        ...
        switch (exec_prc_) {
            ...
            case Precision::I64:
                ...
                h->uni_vpsrlq(vmm_dst, vmm_dst, 63);
                break;
            default: OPENVINO_THROW("jit_greater_emitter doesn't support precision '", exec_prc_, "'");
        }
    } else {
        switch (exec_prc_) {
            case Precision::FP32:
...
void jit_greater_equal_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_greater_equal_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_less_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_less_emitter doesn't support ISA '", host_isa_, "'");
    }
}
    ...
    if (isa == x64::sse41 || isa == x64::avx2) {
        ...
        switch (exec_prc_) {
            ...
            case Precision::I64:
                ...
                h->uni_vpsrlq(vmm_dst, vmm_dst, 63);
                break;
            default: OPENVINO_THROW("jit_less_emitter doesn't support precision '", exec_prc_, "'");
        }
    } else {
        switch (exec_prc_) {
            case Precision::FP32:
...
void jit_less_equal_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_less_equal_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_logical_and_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_logical_and_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_logical_or_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_logical_or_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_logical_xor_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_logical_xor_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_logical_not_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_logical_not_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_power_static_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_power_static_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_prelu_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_prelu_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_sqrt_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_sqrt_emitter doesn't support ISA '", host_isa_, "'");
    }
}
    ...
    switch (exec_prc_) {
        ...
        case Precision::I64: {
            if (isa == x64::avx512_core) {
                ...
                Vmm vmm_aux_0 = Vmm(aux_vec_idxs[0]);
                h->uni_vmovups(vmm_aux_0, table_val_64("dbl_cvt_mask"));
        ...
        default: OPENVINO_THROW("jit_sqrt_emitter doesn't support precision '", exec_prc_, "'");
    }
}

size_t jit_sqrt_emitter::aux_vecs_count() const {
    return (exec_prc_ == Precision::I64 && host_isa_ != x64::avx512_core) ? 1 : 0;
}
...
void jit_negative_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_negative_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_erf_emitter::emit_impl(
    ...
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_erf_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_erf_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    auto exp_compute_vector_fwd = [&](const Vmm &vmm_src) {
        ...
        // tmp = floorf(fx)
        h->uni_vroundps(vmm_aux2, vmm_src, RoundType::floor);
...
void jit_soft_sign_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_soft_sign_emitter doesn't support ISA '", host_isa_, "'");
    }
}
...
void jit_select_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    if (host_isa_ == x64::sse41) {
        ...
        OPENVINO_THROW("jit_select_emitter doesn't support ISA '", host_isa_, "'");
    }
}
