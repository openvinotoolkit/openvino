...
void jit_is_finite_emitter::emit_isa(const std::vector<size_t> &inVecIdxs, const std::vector<size_t> &outVecIdxs) const {
    ...
    auto vmm_aux = Vmm(aux_vec_idxs[1]);
    auto xmm_aux = Xmm(vmm_aux.getIdx());
    auto reg32_aux = Reg32(aux_gpr_idxs[0]);

    h->mov(reg32_aux, INF_MASK);
    h->uni_vpinsrd(xmm_aux, xmm_aux, reg32_aux, 0x0);
    h->uni_vpbroadcastd(vmm_aux, xmm_aux);
    if (isa == x64::avx2) {
        h->uni_vandps(vmm_dst, vmm_src, vmm_aux);
    } else {
        h->uni_vmovups(vmm_dst, vmm_src);
        h->uni_vandps(vmm_dst, vmm_dst, vmm_aux);
    }
    h->uni_vcmpps(vmm_dst, vmm_dst, vmm_aux, 0B00000100); // NEq

    h->mov(reg32_aux, NAN_MASK);
    h->uni_vpinsrd(xmm_aux, xmm_aux, reg32_aux, 0x0);
    h->uni_vpbroadcastd(vmm_aux, xmm_aux);
    h->uni_vandps(vmm_src, vmm_src, vmm_aux);
    h->uni_vcmpps(vmm_src, vmm_src, vmm_aux, 0B00000100); // NEq
    h->uni_vandps(vmm_dst, vmm_dst, vmm_src);

    auto xmm_src = Xmm(vmm_src.getIdx());
    h->mov(reg32_aux, CONST_1_F);
    h->uni_vpinsrd(xmm_src, xmm_src, reg32_aux, 0x0);
    h->uni_vpbroadcastd(vmm_src, xmm_src);
    if (equalInOut) {
...
void jit_is_inf_emitter::emit_isa(const std::vector<size_t> &inVecIdxs, const std::vector<size_t> &outVecIdxs) const {
    ...
    if (detect_negative || detect_positive) {
        ...
        if (detect_positive) {
            ...
            h->uni_vpcmpeqd(vmm_dst, vmm_dst, vmm_aux);
            ...
            if (!detect_negative) {
                ...
                h->uni_vpbroadcastd(vmm_aux, xmm_aux);
                h->uni_vandps(vmm_src, vmm_src, vmm_aux);
                h->uni_vpcmpeqd(vmm_src, vmm_src, vmm_aux);
                if (isa == x64::avx2) {
                    h->vandnps(vmm_dst, vmm_src, vmm_dst);
                } else {
                    h->andnps(vmm_src, vmm_dst);
                    h->uni_vmovups(vmm_dst, vmm_src);
                }
            }
            ...
            h->uni_vandps(vmm_src, vmm_src, vmm_dst);
            h->uni_vpcmpeqd(vmm_dst, vmm_dst, vmm_src);
        }
...
void jit_is_nan_emitter::emit_isa(const std::vector<size_t> &inVecIdxs, const std::vector<size_t> &outVecIdxs) const {
    ...
    h->uni_vpbroadcastd(vmm_dst, xmm_dst);
    h->uni_vandps(vmm_src, vmm_src, vmm_dst);
    h->uni_vpcmpeqd(vmm_src, vmm_src, vmm_dst);
