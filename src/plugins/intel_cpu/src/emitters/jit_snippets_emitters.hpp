...
public:
    MemoryEmitter(dnnl::impl::cpu::x64::jit_generator* h, dnnl::impl::cpu::x64::cpu_isa_t isa,
                  const std::shared_ptr<ov::Node>& n, emitter_in_out_map in_out_type)
...
public:
    ...
    : MemoryEmitter(h, isa, n, emitter_in_out_map::vec_to_gpr) {
        if (src_prc != dst_prc)
...
public:
    ...
    : MemoryEmitter(h, isa, n, emitter_in_out_map::gpr_to_vec), shouldPostIncrement(*n->get_input_shape(0).rbegin() != 1) {
        if (src_prc != dst_prc)
...
public:
    ...
    : MemoryEmitter(h, isa, n, emitter_in_out_map::gpr_to_vec) {
        if (src_prc != dst_prc)
