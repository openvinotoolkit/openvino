...
void jit_add_emitter::emit_isa(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    ...
    auto vmm_src0 = Vmm(static_cast<int>(in_vec_idxs[0]));
    auto vmm_src1 = Vmm(static_cast<int>(in_vec_idxs[1]));
...
void jit_mul_add_emitter::emit_isa(const std::vector<size_t>& in_vec_idxs,
    ...
    auto vmm_src0 = Vmm(static_cast<int>(in_vec_idxs[0]));
    auto vmm_src1 = Vmm(static_cast<int>(in_vec_idxs[1]));
    auto vmm_src2 = Vmm(static_cast<int>(in_vec_idxs[2]));
    auto vmm_aux0 = Vmm(static_cast<int>(aux_vec_idxs[0]));
    auto vmm_dst = Vmm(static_cast<int>(out_vec_idxs[0]));
...
void jit_power_dynamic_emitter::emit_isa(const std::vector<size_t>& in_vec_idxs,
    ...
    for (size_t i = 2; i < get_max_vecs_count() + 2; ++i) {
        h->uni_vmovups(h->ptr[h->rsp + i * get_vec_length()], Vmm(static_cast<int>(i - 2)));
    }
    ...
    for (size_t i = get_max_vecs_count() + 1; i >= 2; --i) {
        h->uni_vmovups(Vmm(static_cast<int>(i - 2)), h->ptr[h->rsp + i * get_vec_length()]);
    }
...
void jit_equal_emitter::emit_isa(const std::vector<size_t>& in_vec_idxs,
    ...
    auto vmm_dst = Vmm(static_cast<int>(out_vec_idxs[0]));
    auto vmm_aux0 = Vmm(static_cast<int>(aux_vec_idxs[0]));
    auto vmm_aux1 = Vmm(static_cast<int>(aux_vec_idxs[1]));
...
void jit_logical_and_emitter::emit_isa(const std::vector<size_t>& in_vec_idxs,
    ...
    auto vmm_aux0 = Vmm(static_cast<int>(aux_vec_idxs[0]));
    auto vmm_aux1 = Vmm(static_cast<int>(aux_vec_idxs[1]));
    auto vmm_aux2 = Vmm(static_cast<int>(aux_vec_idxs[2]));
...
void jit_exp_emitter::emit_isa(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    ...
    Vmm vmm_mask = need_vmm_mask() ? Vmm(static_cast<int>(aux_vec_idxs[0])) : Vmm();
    auto vmm_aux0 = Vmm(static_cast<int>(aux_vec_idxs[0 + static_cast<size_t>(need_vmm_mask())]));
    auto vmm_aux1 = Vmm(static_cast<int>(aux_vec_idxs[1 + static_cast<size_t>(need_vmm_mask())]));
...
void jit_erf_emitter::emit_isa(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    ...
    auto vmm_aux1 = Vmm(static_cast<int>(aux_vec_idxs[1]));
    auto vmm_aux2 = Vmm(static_cast<int>(aux_vec_idxs[2]));
    auto vmm_aux3 = Vmm(static_cast<int>(aux_vec_idxs[3]));
...
void jit_is_finite_emitter::emit_isa<x64::avx512_core>(const std::vector<size_t>& in_vec_idxs,
                                                       const std::vector<size_t>& out_vec_idxs) const {
    auto vmm_src = Zmm(static_cast<int>(in_vec_idxs[0]));
    auto vmm_dst = Zmm(static_cast<int>(out_vec_idxs[0]));
    const auto& ones_mask = h->k1;
    auto reg32_one = Reg32(static_cast<int>(aux_gpr_idxs[0]));
...
void jit_select_emitter::emit_isa(const std::vector<size_t>& in_vec_idxs,
                                  const std::vector<size_t>& out_vec_idxs) const {
    using Vmm = typename conditional3<isa == x64::sse41, Xmm, isa == x64::avx2, Ymm, Zmm>::type;
    auto vmm_cond = Vmm(static_cast<int>(in_vec_idxs[0]));
    auto vmm_src0 = Vmm(static_cast<int>(in_vec_idxs[1]));
    auto vmm_src1 = Vmm(static_cast<int>(in_vec_idxs[2]));
    auto vmm_dst = Vmm(static_cast<int>(out_vec_idxs[0]));

    if (isa == x64::sse41) {
        auto vmm_mask = Vmm(static_cast<int>(aux_vec_idxs[0]));
        auto vmm_zero = Vmm(static_cast<int>(aux_vec_idxs[1]));
