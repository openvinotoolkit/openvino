...
void
jit_elu_emitter::emit_impl(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    if (host_isa_ == ov::intel_cpu::riscv64::cpu_isa_t::gv) {
        emit_isa<ov::intel_cpu::riscv64::cpu_isa_t::gv>(in_vec_idxs, out_vec_idxs);
    } else {
        OPENVINO_THROW("Can't create jit eltwise kernel");
...
template<ov::intel_cpu::riscv64::cpu_isa_t isa>
void
jit_elu_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    auto src = VReg(in_vec_idxs[0]);
    auto dst = VReg(out_vec_idxs[0]);
    ...
    exp_emitter->emit_code({static_cast<size_t>(src.getIdx())}, {static_cast<size_t>(dst.getIdx())},
    ...
    h->vfsub_vv(dst, dst, dst, VM::masked);
    h->vfadd_vv(dst, dst, aux1, VM::masked);
}

std::set<std::vector<element::Type>>
jit_elu_emitter::get_supported_precisions(const std::shared_ptr<ov::Node> &node) {
    return {{element::f32}};
}
