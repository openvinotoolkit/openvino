...
// fp16 code support emission
//#define OV_CPU_JIT_EMITTER_FP16

#ifdef OV_CPU_JIT_EMITTER_FP16

#define OV_CPU_JIT_EMITTER_ASSERT_SUPPORTED_PRECISIONS(exec_prc_)             \
    OV_CPU_JIT_EMITTER_ASSERT(                                                \
        ((exec_prc_ == ov::element::f16) || (exec_prc_ == ov::element::f32)), \
        "unsupported precision: " + exec_prc_.to_string());                   \

#define OV_CPU_JIT_EMITTER_SUPPORTED_PRECISIONS { {element::f16}, {element::f32} }
#define OV_CPU_JIT_EMITTER_SUPPORTED_PRECISIONS2 { {element::f16, element::f16}, {element::f32, element::f32} }
#define OV_CPU_JIT_EMITTER_SUPPORTED_PRECISIONS3 { {element::f16, element::f16, element::f16}, {element::f32, element::f32, element::f32} }

#else

#define OV_CPU_JIT_EMITTER_ASSERT_SUPPORTED_PRECISIONS(exec_prc_) \
    OV_CPU_JIT_EMITTER_ASSERT(                                    \
        (exec_prc_ == ov::element::f32),                          \
        "unsupported precision: " + exec_prc_.to_string());       \

#define OV_CPU_JIT_EMITTER_SUPPORTED_PRECISIONS { {element::f32} }
#define OV_CPU_JIT_EMITTER_SUPPORTED_PRECISIONS2 { {element::f32, element::f32} }
#define OV_CPU_JIT_EMITTER_SUPPORTED_PRECISIONS3 { {element::f32, element::f32, element::f32} }
...
void jit_abs_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    OV_CPU_JIT_EMITTER_ASSERT_SUPPORTED_PRECISIONS(exec_prc_)
...
std::set<std::vector<element::Type>> jit_abs_emitter::get_supported_precisions(const std::shared_ptr<ov::Node>& node) {
    return OV_CPU_JIT_EMITTER_SUPPORTED_PRECISIONS;
}
...
std::set<std::vector<element::Type>> jit_add_emitter::get_supported_precisions(const std::shared_ptr<ov::Node>& node) {
    return OV_CPU_JIT_EMITTER_SUPPORTED_PRECISIONS2;
}
...
std::set<std::vector<element::Type>> jit_equal_emitter::get_supported_precisions(const std::shared_ptr<ov::Node>& node) {
    // TensorFlow test fail for fp16 Equal for NaN/Inf (?) values
...
void jit_gelu_tanh_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    h->fadd(vmm_aux0, vmm_aux1, vmm_aux2);
    h->ld1r(vmm_aux1, table_val2("half"));
    h->fmul(vmm_aux0, vmm_aux1, vmm_aux0);
    h->fmul(vmm_dst, vmm_src, vmm_aux0);
}
...
std::set<std::vector<element::Type>> jit_mish_emitter::get_supported_precisions(const std::shared_ptr<ov::Node>& node) {
    // TODO: use fp32 only: current approximation implementation accuracy drop issue
    return {{element::f32}};
}
...
void jit_mod_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    h->fdiv(aux, dividend, divisor);
    h->frintz(aux, aux);
    h->fmul(aux, aux, divisor);
    h->fsub(r, dividend, aux);
}
...
std::set<std::vector<element::Type>> jit_mul_add_emitter::get_supported_precisions(const std::shared_ptr<ov::Node>& node) {
    return OV_CPU_JIT_EMITTER_SUPPORTED_PRECISIONS3;
}
