...
void jit_convert_truncation_emitter::dword2int8(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    ...
    if (isa == dnnl::impl::cpu::x64::avx512_core) {
        ...
        h->vpand(vmm_src, vmm_src, table_val("mask_byte"));  // to avoid saturation
    ...
    switch (output_type) {
        ...
        case ov::element::i8:
            if (input_type == ov::element::i32) {
                ...
                dword2sint8<isa>({static_cast<size_t>(vmm_dst.getIdx())}, {static_cast<size_t>(vmm_dst.getIdx())});
            }
            break;
        case ov::element::u8:
            if (input_type == ov::element::i32) {
                ...
                dword2uint8<isa>({static_cast<size_t>(vmm_dst.getIdx())}, {static_cast<size_t>(vmm_dst.getIdx())});
            }
            break;
        default:
            assert(!"unsupported output data type");
...
template <dnnl::impl::cpu::x64::cpu_isa_t isa>
void jit_convert_saturation_emitter::dword2sint8(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    using Vmm = typename conditional3<isa == cpu::x64::sse41, Xmm, isa == cpu::x64::avx2, Ymm, Zmm>::type;
    Vmm vmm_src = Vmm(in_vec_idxs[0]);
    Xmm xmm_src = Xmm(in_vec_idxs[0]);
    Ymm ymm_src = Ymm(in_vec_idxs[0]);
...
template <dnnl::impl::cpu::x64::cpu_isa_t isa>
void jit_convert_saturation_emitter::dword2uint8(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    using Vmm = typename conditional3<isa == cpu::x64::sse41, Xmm, isa == cpu::x64::avx2, Ymm, Zmm>::type;
    Vmm vmm_src = Vmm(in_vec_idxs[0]);
    Xmm xmm_src = Xmm(in_vec_idxs[0]);
    Ymm ymm_src = Ymm(in_vec_idxs[0]);
