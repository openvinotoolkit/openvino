...
BinaryEltwiseTppEmitter::BinaryEltwiseTppEmitter(jit_generator* h, cpu_isa_t isa, const ExpressionPtr& expr)
    ...
    OV_CPU_JIT_EMITTER_ASSERT(std::min(N_in0, N_in1) == N || std::min(N_in0, N_in1) == 1,
                              "Invalid subtensor broadcasting: N");
    OV_CPU_JIT_EMITTER_ASSERT(std::min(M_in0, M_in1) == M || std::min(M_in0, M_in1) == 1,
                              "Invalid subtensor broadcasting: M");
    ...
    m_shape = libxsmm_create_meltw_binary_shape(N,
                                                M,
                                                io_strides[0],
                                                io_strides[1],
                                                io_strides[2],
...
const uintptr_t BinaryEltwiseTppEmitter::get_compiled_kernel_ptr() const {
    // Note: libxsmm hides memory management from the user, so we don't have to store pointer to compiled kernel to keep
    // it alive. libxsmm will keep the pointer alive until the end of program execution (it doesn't matter whether we
    // save the pointer in the emitter or not)
