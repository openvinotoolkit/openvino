...
protected:
    ...
    static libxsmm_blasint get_broadcasted_dim(libxsmm_blasint dim0,
                                               libxsmm_blasint dim1,
                                               std::pair<bool, bool>& bcast_flags);
};
...
public:
    ...
    const uintptr_t get_compiled_kernel_ptr() const override {
        return COMPILE_TPP_KERNEL(libxsmm_dispatch_meltw_unary(m_op_type, m_shape, m_compile_flags));
    }
    const uintptr_t get_execute_function_ptr() const override {
        return reinterpret_cast<const uintptr_t>(execute_kernel);
    }
