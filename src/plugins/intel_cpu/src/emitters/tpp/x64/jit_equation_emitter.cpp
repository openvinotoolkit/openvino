...
EquationTppEmitter::EquationTppEmitter(jit_generator* h, cpu_isa_t isa, const ExpressionPtr& expr)
    ...
    for (const auto& op_desc : eq_tpp->get_op_descs()) {
        ...
        case tpp::op::OpDescTPP::ARITY::ZERO: {
            ...
            auto shape = libxsmm_create_meqn_arg_shape(N,
                                                       M,
    ...
    m_out_shape =
        libxsmm_create_meqn_arg_shape(N, M, static_cast<libxsmm_blasint>(io_strides.back()), io_dtypes.back());
}
...
void EquationTppEmitter::validate_arguments(const std::vector<size_t>& in, const std::vector<size_t>& out) const {
    OV_CPU_JIT_EMITTER_ASSERT(
        in.size() == m_num_inputs,
        "Expects " + std::to_string(m_num_inputs) + " input registers, got " + std::to_string(in.size()));
    const auto num_outputs = num_kernel_args - m_num_inputs;
    OV_CPU_JIT_EMITTER_ASSERT(
        out.size() == num_outputs,
        "Expects " + std::to_string(num_outputs) + " output register, got " + std::to_string(out.size()));
}
