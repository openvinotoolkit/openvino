...
size_t jit_debug_emitter::aux_fp_gprs_count() const {
    return m_target_emitter->aux_fp_gprs_count();
}
...
void jit_debug_emitter::emitter_preamble(const std::vector<size_t>& in_idxs,
    ...
    m_target_emitter->emitter_preamble(in_idxs, out_idxs, pool_vec_idxs, pool_gpr_idxs, pool_fp_gpr_idxs);
}
...
void jit_debug_emitter::emit_code_impl(const std::vector<size_t>& in_idxs,
    ...
    if (any_of(m_decorator_emit_loc, EmissionLocation::preamble, EmissionLocation::both)) {
        m_decorator_emitter->emit_code(in_idxs, out_idxs, pool_vec_idxs, pool_gpr_idxs, pool_fp_gpr_idxs);
    }

    m_target_emitter->emit_code(in_idxs, out_idxs, pool_vec_idxs, pool_gpr_idxs, pool_fp_gpr_idxs);
