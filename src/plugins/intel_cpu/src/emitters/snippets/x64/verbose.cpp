...
std::string init_info_jit_memory_emitter(const jit_memory_emitter* emitter) {
    ...
    ss << " src_precision:" << emitter->src_prc << " dst_precision:" << emitter->dst_prc
       << " load/store_element_number:" << emitter->count << " byte_offset:" << emitter->byte_offset;
    return ss.str();
}
...
static std::string init_info_jit_load_convert_emitter(const jit_load_convert_emitter* emitter) {
    std::stringstream ss;
    std::string memory_emitter_info = init_info_jit_memory_emitter(emitter);
    ss << "Emitter_type_name:jit_load_convert_emitter" << memory_emitter_info;
    return ss.str();
}
...
static std::string init_info_jit_store_convert_emitter(const jit_store_convert_emitter* emitter) {
    std::stringstream ss;
    std::string memory_emitter_info = init_info_jit_memory_emitter(emitter);
    ss << "Emitter_type_name:jit_store_convert_emitter" << memory_emitter_info;
    return ss.str();
}

std::string init_info_jit_brgemm_emitter(const jit_brgemm_emitter* emitter) {
    std::stringstream ss;
    ss << "Emitter_type_name:jit_brgemm_emitter" << emitter->m_kernel_executor->config_to_string()
       << " m_load_offset_a:" << emitter->m_load_offset_a << " m_load_offset_b:" << emitter->m_load_offset_b
       << " m_load_offset_scratch:" << emitter->m_load_offset_scratch
       << " m_store_offset_c:" << emitter->m_store_offset_c << " m_with_scratch:" << emitter->m_with_scratch;
...
std::string init_info_jit_brgemm_copy_b_emitter(const jit_brgemm_copy_b_emitter* emitter) {
    std::stringstream ss;
    ss << "Emitter_type_name:jit_brgemm_copy_b_emitter" << " m_brg_weight_etype:" << emitter->m_brg_weight_etype
       << " m_N_blk:" << emitter->m_N_blk << " m_inner_N_block:" << emitter->m_inner_N_block
       << " m_inner_N_tail:" << emitter->m_inner_N_tail << " m_K_blk:" << emitter->m_K_blk
       << " m_brgemmVNNIFactor:" << emitter->m_brgemmVNNIFactor << " m_in_offset:" << emitter->m_in_offset
       << " m_out_offset:" << emitter->m_out_offset << " m_comp_offset:" << emitter->m_comp_offset
