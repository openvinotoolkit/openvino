...
void jit_horizon_emitter::emit_isa(const std::vector<size_t>& in, const std::vector<size_t>& out) const {
    ...
    auto aux_vmm = Vmm(static_cast<int>(aux_vec_idxs[0]));

    if (in[0] != out[0]) {
        h->uni_vmovups(dst_vmm, src_vmm);
    }
    if (isa == dnnl::impl::cpu::x64::avx512_core) {
        auto dst_zmm = Xbyak::Zmm(static_cast<int>(out[0]));
        auto aux_zmm = Xbyak::Zmm(static_cast<int>(aux_vec_idxs[0]));
        ...
        auto dst_ymm = Xbyak::Ymm(static_cast<int>(out[0]));
        auto aux_ymm = Xbyak::Ymm(static_cast<int>(aux_vec_idxs[0]));
