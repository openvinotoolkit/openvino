...
BrgemmKernelConfig::BrgemmKernelConfig(const element::Type& in0_dtype,
    ...
    : dt_in0(DTYPE_CAST(in0_dtype)),
      ...
      LDC(DIM_CAST(LDC)) {
    bool is_int8 =
        utils::one_of(dt_in0, data_type::u8, data_type::s8) && utils::one_of(dt_in1, data_type::u8, data_type::s8);
    isa = is_with_amx                             ? cpu::x64::avx512_core_amx
          : dt_in0 == dnnl_data_type_t::dnnl_bf16 ? cpu::x64::avx512_core_bf16
          : is_int8                               ? cpu::x64::avx512_core_vnni
                                                  : cpu::x64::avx512_core;
}
...
#define EQUAL(X) X == rhs.X
    return EQUAL(dt_in0) && EQUAL(dt_in1) && EQUAL(is_with_amx) && EQUAL(is_with_comp) && EQUAL(beta) && EQUAL(isa) &&
           EQUAL(M) && EQUAL(N) && EQUAL(K) && EQUAL(LDA) && EQUAL(LDB) && EQUAL(LDC);
#undef EQUAL
}
bool BrgemmKernelConfig::operator!=(const BrgemmKernelConfig& rhs) const {
    return !(*this == rhs);
...
std::shared_ptr<brgemm_kernel_t> BrgemmKernelExecutor::compile_kernel(
    const std::shared_ptr<BrgemmKernelConfig>& config) const {
    OV_CPU_JIT_EMITTER_ASSERT(config, "Invalid config provided for BrgemmKernelDesc::compile_kernel");
    cpu::x64::brgemm_t desc;
    auto status = brgemm_desc_init(&desc,
                                   config->isa,
                                   cpu::x64::brgemm_strd,
                                   config->dt_in0,
                                   config->dt_in1,
