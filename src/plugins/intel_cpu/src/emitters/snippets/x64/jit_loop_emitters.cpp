...
std::shared_ptr<ov::snippets::op::LoopEnd> jit_loop_begin_emitter::get_loop_end(
    const ov::snippets::lowered::ExpressionPtr& expr) {
    OV_CPU_JIT_EMITTER_ASSERT(expr->get_output_port_connectors().size() == 1,
                              "has invalid LoopBegin expression configuration");
...
jit_loop_begin_static_emitter::jit_loop_begin_static_emitter(dnnl::impl::cpu::x64::jit_generator* h,
                                                             dnnl::impl::cpu::x64::cpu_isa_t isa,
                                                             const ov::snippets::lowered::ExpressionPtr& expr)
...
void jit_loop_begin_static_emitter::validate_arguments(const std::vector<size_t>& in,
...
void jit_loop_begin_static_emitter::emit_code(const std::vector<size_t>& in,
...
jit_loop_begin_dynamic_emitter::jit_loop_begin_dynamic_emitter(dnnl::impl::cpu::x64::jit_generator* h,
                                                               dnnl::impl::cpu::x64::cpu_isa_t isa,
                                                               const ov::snippets::lowered::ExpressionPtr& expr)
    : jit_loop_begin_emitter(h, isa, expr),
      loop_end_label(nullptr) {
    OV_CPU_JIT_EMITTER_ASSERT(ov::is_type<snippets::op::LoopBeginDynamic>(expr->get_node()),
                              "expects LoopBeginDynamic expression");
    const auto loop_end = get_loop_end(expr);
    wa_increment = loop_end->get_increment();
    loop_id = loop_end->get_id();
}

void jit_loop_begin_dynamic_emitter::validate_arguments(const std::vector<size_t>& in,
                                                        const std::vector<size_t>& out) const {
...
void jit_loop_begin_dynamic_emitter::emit_code(const std::vector<size_t>& in,
                                               const std::vector<size_t>& out,
                                               const std::vector<size_t>& pool_vec_idxs,
                                               const std::vector<size_t>& pool_gpr_idxs) const {
    validate_arguments(in, out);
...
jit_loop_end_static_emitter::jit_loop_end_static_emitter(dnnl::impl::cpu::x64::jit_generator* h,
                                                         dnnl::impl::cpu::x64::cpu_isa_t isa,
                                                         const ov::snippets::lowered::ExpressionPtr& expr)
...
void jit_loop_end_static_emitter::validate_arguments(const std::vector<size_t>& in,
                                                     const std::vector<size_t>& out) const {
...
void jit_loop_end_static_emitter::emit_code(const std::vector<size_t>& in,
                                            const std::vector<size_t>& out,
                                            const std::vector<size_t>& pool_vec_idxs,
                                            const std::vector<size_t>& pool_gpr_idxs) const {
    validate_arguments(in, out);
...
jit_loop_end_dynamic_emitter::jit_loop_end_dynamic_emitter(dnnl::impl::cpu::x64::jit_generator* h,
                                                           dnnl::impl::cpu::x64::cpu_isa_t isa,
                                                           const ov::snippets::lowered::ExpressionPtr& expr)
    : jit_loop_end_emitter(h, isa, expr),
    ...
    const auto& loop_begin_emitter =
        std::dynamic_pointer_cast<jit_loop_begin_dynamic_emitter>(begin_expr->get_emitter());
...
void jit_loop_end_dynamic_emitter::validate_arguments(const std::vector<size_t>& in,
                                                      const std::vector<size_t>& out) const {
    OV_CPU_JIT_EMITTER_ASSERT(loop_end_label != nullptr && loop_begin_label != nullptr, "has not inited labels!");
...
void jit_loop_end_dynamic_emitter::emit_code(const std::vector<size_t>& in,
                                             const std::vector<size_t>& out,
                                             const std::vector<size_t>& pool_vec_idxs,
                                             const std::vector<size_t>& pool_gpr_idxs) const {
    validate_arguments(in, out);
...
void jit_loop_end_dynamic_emitter::emit_impl(const std::vector<size_t>& in, const std::vector<size_t>& out) const {
    ...
    // todo: Note that we can pre-save reg_loop_args_ptr in jit_loop_begin_dynamic_emitter and pass it here like
    // work_amount_reg
