...
ov::snippets::lowered::ExpressionPtr jit_loop_begin_emitter::get_loop_end_expr(const ov::snippets::lowered::ExpressionPtr& expr) {
    ...
    OPENVINO_ASSERT(ov::is_type<snippets::op::LoopEnd>(loop_end_expr->get_node()), "jit_loop_begin_emitter has invalid LoopBegin expression configuration");
...
std::shared_ptr<ov::snippets::op::LoopEnd> jit_loop_begin_emitter::get_loop_end(const ov::snippets::lowered::ExpressionPtr& expr) {
    return ov::as_type_ptr<snippets::op::LoopEnd>(get_loop_end_expr(expr)->get_node());
}
...
jit_loop_begin_static_emitter::jit_loop_begin_static_emitter(dnnl::impl::cpu::x64::jit_generator* h, dnnl::impl::cpu::x64::cpu_isa_t isa,
    ...
    OPENVINO_ASSERT(ov::is_type<snippets::op::LoopBeginStatic>(expr->get_node()), "jit_loop_begin_static_emitter expects LoopBeginStatic expression");
...
void jit_loop_begin_static_emitter::validate_arguments(const std::vector<size_t> &in, const std::vector<size_t> &out) const {
    OPENVINO_ASSERT(in.empty(), "Invalid inputs size: expected 0 got " + std::to_string(in.size()));
...
jit_loop_begin_dynamic_emitter::jit_loop_begin_dynamic_emitter(dnnl::impl::cpu::x64::jit_generator* h, dnnl::impl::cpu::x64::cpu_isa_t isa,
    ...
    OPENVINO_ASSERT(ov::is_type<snippets::op::LoopBeginDynamic>(expr->get_node()), "jit_loop_begin_dynamic_emitter expects LoopBeginDynamic expression");
...
void jit_loop_begin_dynamic_emitter::validate_arguments(const std::vector<size_t> &in, const std::vector<size_t> &out) const {
    ...
    // Note: the only expected output is work amount register (communicated to jit_loop_end_emitter)
    OPENVINO_ASSERT(out.size() == 1, "Invalid outputs size: expected 1 got " + std::to_string(out.size()));
    OPENVINO_ASSERT(loop_end_label != nullptr && loop_begin_label != nullptr, "jit_loop_begin_dynamic_emitter has not inited labels!");
}
...
jit_loop_end_emitter::jit_loop_end_emitter(dnnl::impl::cpu::x64::jit_generator* h, dnnl::impl::cpu::x64::cpu_isa_t isa,
    ...
    if (!loop_end)
        OPENVINO_THROW("jit_loop_end_emitter expected LoopEnd expr");
    ...
    OPENVINO_ASSERT(loop_begin_emitter, "LoopBegin expected jit_loop_begin_emitter");
...
jit_loop_end_static_emitter::jit_loop_end_static_emitter(dnnl::impl::cpu::x64::jit_generator* h, dnnl::impl::cpu::x64::cpu_isa_t isa,
    ...
    if (!loop_end)
        OPENVINO_THROW("jit_loop_end_static_emitter expected LoopEndStatic expr");
...
void jit_loop_end_static_emitter::validate_arguments(const std::vector<size_t> &in, const std::vector<size_t> &out) const {
    ...
    OPENVINO_ASSERT(out.size() == 0, "Invalid number of out arguments: expected ", 0, " got ", out.size());
    OPENVINO_ASSERT(in.size() == io_size + 1, "Invalid number of in arguments: expected ", io_size + 1, " got ", in.size());
    ...
    OPENVINO_ASSERT(data_sizes.size() == io_size, "Invalid data_sizes size: expected: ", io_size, " got ", data_sizes.size());
}
...
void jit_loop_end_static_emitter::emit_impl(const std::vector<size_t>& in, const std::vector<size_t>& out) const {
    ...
    Xbyak::Reg64 reg_work_amount = Xbyak::Reg64(in.back());
    if (!evaluate_once) {
...
jit_loop_end_dynamic_emitter::jit_loop_end_dynamic_emitter(dnnl::impl::cpu::x64::jit_generator* h, dnnl::impl::cpu::x64::cpu_isa_t isa,
    ...
    OPENVINO_ASSERT(loop_end != nullptr, "jit_loop_end_dynamic_emitter expected LoopEndDynamic expr");
    loop_id = loop_end->get_id();

    const auto begin_expr = get_loop_begin_expr(expr);
    const auto& loop_begin_emitter = std::dynamic_pointer_cast<jit_loop_begin_dynamic_emitter>(begin_expr->get_emitter());
    OPENVINO_ASSERT(loop_begin_emitter, "LoopBeginDynamic expected jit_loop_begin_dynamic_emitter");
...
void jit_loop_end_dynamic_emitter::validate_arguments(const std::vector<size_t> &in, const std::vector<size_t> &out) const {
    ...
    // Note: there must be additional input argument for runtime parameters
     const auto io_size = num_inputs + num_outputs;
    OPENVINO_ASSERT(in.size() == io_size + 2, "Invalid number of in arguments: expected ", io_size + 2, " got ", in.size());
    OPENVINO_ASSERT(out.size() == 0, "Invalid number of out arguments: expected ", 0, " got ", out.size());
}
