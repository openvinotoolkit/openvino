...
std::string init_info_jit_gemm_emitter(const jit_gemm_emitter* emitter) {
    ...
    ss << "Emitter_type_name:jit_gemm_emitter"
       << " is_f16:" << emitter->m_is_f16 << " m_memory_offset:" << vector_to_string(emitter->m_memory_offsets)
...
void jit_emitter_info_t::init(const jit_emitter* emitter) {
    ...
    } else if (const auto* e_type = dynamic_cast<const jit_gemm_emitter*>(emitter)) {
        str_ = init_info_jit_gemm_emitter(e_type);
    } else if (const auto* e_type = dynamic_cast<const jit_gemm_copy_b_emitter*>(emitter)) {
