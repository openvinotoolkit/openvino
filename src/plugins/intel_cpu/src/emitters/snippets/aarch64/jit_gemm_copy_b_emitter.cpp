...
jit_gemm_copy_b_emitter::jit_gemm_copy_b_emitter(jit_generator* h,
    ...
    const auto brgemm_repack = ov::as_type_ptr<GemmCopyB>(expr->get_node());
    OV_CPU_JIT_EMITTER_ASSERT(brgemm_repack, "expects GemmCopyB node");
    ...
    m_kernel_executor =
        kernel_table->register_kernel<GemmCopyBKaiKernelExecutor>(expr, kernel_config);
}
...
void jit_gemm_copy_b_emitter::emit_code_impl(const std::vector<size_t>& in,
...
void jit_gemm_copy_b_emitter::emit_impl(const std::vector<size_t>& in, const std::vector<size_t>& out) const {
    ...
    h->mov(x2, Xbyak_aarch64::XReg(out[0]));
    h->blr(func_reg);
