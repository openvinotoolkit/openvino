...
void GemmCopyBKaiKernelExecutor::execute(const GemmCopyBKaiKernelExecutor* executor, const call_args* args) {
    OV_CPU_JIT_EMITTER_ASSERT(executor, "has nullptr executor");
    OV_CPU_JIT_EMITTER_ASSERT(executor->get_kernel(), "has nullptr kernel in executor");
    OV_CPU_JIT_EMITTER_ASSERT(args, "has nullptr args");
    OV_CPU_JIT_EMITTER_ASSERT(ov::intel_cpu::none_of(nullptr, args->in, args->bias, args->out),
                              "has nullptr pointers in args");
    ...
    // Fill bias by zero
    // TODO: Add support of non-zero bias
    auto bias_fptr = static_cast<float*>(args->bias);
    std::fill(bias_fptr, bias_fptr + N, 0);
