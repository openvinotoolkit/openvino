...
jit_loop_begin_base_emitter::jit_loop_begin_base_emitter(jit_generator_t* h,
                                                         ...
                                                         bool is_parallel)
    : jit_emitter(h, isa),
      m_loop_begin_label(std::make_shared<Xbyak_aarch64::Label>()) {
...
void jit_loop_begin_base_emitter::emit_loop_begin_work_amount_check(const std::vector<size_t>& out,
    ...
    const auto reg_work_amount = XReg(static_cast<int>(out.back()));
    if (is_work_amount_dynamic) {
        const auto reg_runtime_params = XReg(Operand::X0);
        const auto reg_loop_args = h->X_TMP_1;
        h->ldr(reg_loop_args, ptr(reg_runtime_params, static_cast<int32_t>(GET_OFF(loop_args))));
        h->ldr(reg_work_amount,
               ptr(reg_loop_args, static_cast<int32_t>(m_loop_id_offset + GET_OFF_LOOP_ARGS(m_work_amount))));
    } else {
        h->mov(reg_work_amount, work_amount_static);
    }

    const auto increment =
        m_evaluate_once && ov::snippets::utils::is_dynamic_value(m_wa_increment) ? 1 : m_wa_increment;
    h->cmp(reg_work_amount, increment);
    h->b(LT, *m_loop_end_label);
}
...
jit_loop_end_base_emitter::jit_loop_end_base_emitter(jit_generator_t* h,
                                                     ...
                                                     bool is_parallel)
    : jit_emitter(h, isa),
      m_loop_end_label(std::make_shared<Xbyak_aarch64::Label>()) {
    in_out_type_ = emitter_in_out_map::gpr_to_gpr;
...
ov::snippets::lowered::ExpressionPtr jit_loop_end_base_emitter::get_loop_begin_expr(
    const ov::snippets::lowered::ExpressionPtr& expr) {
    const auto begin_expr = expr->get_input_expr_ptr(expr->get_input_count() - 1);
...
void jit_loop_end_base_emitter::apply_increments_to_ptrs(const std::vector<size_t>& data_ptr_reg_idxs,
    ...
    const auto reg_aux = h->X_TMP_1;
    const auto reg_increments = h->X_TMP_0;
    if (use_runtime_args) {
        const auto reg_runtime_params = XReg(Operand::X0);
        h->ldr(reg_increments, ptr(reg_runtime_params, static_cast<int32_t>(GET_OFF(loop_args))));
        h->ldr(reg_increments, ptr(reg_increments, static_cast<int32_t>(m_loop_id_offset + field_offset)));
    }
    ...
    for (size_t idx = 0; idx < data_ptr_reg_idxs.size(); ++idx) {
        ...
        const auto data_reg = XReg(static_cast<int>(data_ptr_reg_idxs[idx]));
        if (ov::snippets::utils::is_dynamic_value(increment)) {
            OV_CPU_JIT_EMITTER_ASSERT(use_runtime_args, "Loop argument structure cannot be loaded to aux GPR");
...
void jit_loop_end_base_emitter::emit_loop_end_impl(const std::vector<size_t>& in,
    ...
    if (!m_evaluate_once) {
        ...
        const auto reg_work_amount = XReg(static_cast<int>(in.back()));
        const auto reg_aux = h->X_TMP_1;
        h->sub_imm(reg_work_amount, reg_work_amount, m_wa_increment, reg_aux);
        h->cmp(reg_work_amount, m_wa_increment);
        h->b(GE, *m_loop_begin_label);
    }
