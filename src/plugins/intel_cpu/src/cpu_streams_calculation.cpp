...
int get_model_prefer_threads(const int num_streams,
    ...
    if (-1 == config.modelPreferThreads) {
        ...
        config.modelPreferThreads = IStreamsExecutor::Config::StreamMode::DEFAULT;
    ...
    if (num_streams <= sockets && num_streams > 0) {
        if (config.streamExecutorConfig._threadBindingType == IStreamsExecutor::ThreadBindingType::HYBRID_AWARE) {
