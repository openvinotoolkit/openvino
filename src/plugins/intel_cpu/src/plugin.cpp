...
void Plugin::get_performance_streams(Config& config, const std::shared_ptr<ov::Model>& model, int& modelPreferThreads) const {
    ...
    if (!((0 == streams_set) && config.is_set_by_user(ov::num_streams))) {
        get_num_streams(streams, model, config, modelPreferThreads);
    } else {
...
void Plugin::calculate_streams(Config& conf, const std::shared_ptr<ov::Model>& model, bool imported) const {
    ...
    if (imported && model->has_rt_info("intel_cpu_hints_config")) {
        ...
        if (it_model_prefer != hints_config.end()) {
            ...
            modelPreferThreads = cache_model_prefer;
        }
    }
    get_performance_streams(conf, model, modelPreferThreads);
    // save model_prefer_threads to model rt_info when loading network
    if (!imported) {
        ov::AnyMap hints_props;
        hints_props.insert({model_prefer_name, std::to_string(modelPreferThreads)});
