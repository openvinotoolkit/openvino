...
void Graph::Allocate(const std::vector<MemoryPtr>& inputMemory,
    ...
    Allocate(syncNodesInds, inputMemory, outputMemory);
...
void Graph::Allocate(const std::vector<size_t>& syncNodesInds,
    ...
    for (size_t i = 0; i < inputMemory.size(); i++) {
        const auto& node = inputNodesMap[i];

        auto childEdges = node->getChildEdgesAtPort(0);
        for (const auto& childEdge : childEdges) {
            if (childEdge->getStatus() != Edge::Status::Uninitialized)
                continue;

            if (childEdge->inPlace(Edge::LOOK_UP))
                node->resolveInPlaceEdgesNew(Edge::LOOK_UP, inputMemory[i]);
        }
    }

    for (size_t i = 0; i < outputMemory.size(); i++) {
        const auto& node = outputNodesMap[i];
        const auto& parentEdge = node->getParentEdgeAt(0);

        if (parentEdge->getStatus() != Edge::Status::Uninitialized)
            continue;

        if (parentEdge->inPlace(Edge::LOOK_DOWN))
            node->resolveInPlaceEdgesNew(Edge::LOOK_DOWN, outputMemory[i]);
    }
...
void Graph::InferDynamicWithAsync(SyncInferRequest* request) {
    ...
    for (const auto& node : m_executableGraphNodes) {
        ...
        // std::cout << "Execute node: " << node->getName()
        //           << " substreamid: " << subStreamId
        //           << " numaid: " << m_context->getCPUStreamExecutor()->get_numa_node_id() << "\n";
...
static bool moveToNuma() {
    static bool result = std::getenv("MOVE_TO_NUMA") ? true : false;
    return result;
}
...
inline void Graph::ExecuteNode(const NodePtr& node, const dnnl::stream& stream) const {
    ...
    if (moveToNuma()) {
        auto numaId = getGraphContext()->getCPUStreamExecutor()->get_numa_node_id();
        if (node->isDynamicNode()) {
