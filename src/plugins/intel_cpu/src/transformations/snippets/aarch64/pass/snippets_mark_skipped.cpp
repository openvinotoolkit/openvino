...
#if defined(OPENVINO_ARCH_ARM64)
void markACLInt8ConvFQChain(const std::shared_ptr<Node>& node) {
    ...
    const auto add = ov::as_type_ptr<ov::op::v1::Add>(mul->get_input_node_shared_ptr(0));
    if (add) {
