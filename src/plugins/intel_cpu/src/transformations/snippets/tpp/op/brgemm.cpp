...
void BrgemmTPP::custom_constructor_validate_and_infer_types(std::vector<size_t> layout_a, std::vector<size_t> layout_b, std::vector<size_t> layout_c) {
    INTERNAL_OP_SCOPE(BrgemmTPP_constructor_validate_and_infer_types);
    validate_inputs();

    // During ctor call, BrgemmTPP doesn't know his port descriptors.
...
void BrgemmTPP::validate_and_infer_types() {
    INTERNAL_OP_SCOPE(BrgemmTPP_validate_and_infer_types);
    validate_inputs();
...
void BrgemmTPP::validate_with_scratchpad(const ov::Shape& shape_b) const {
    // Additional check for 3rd input
...
void BrgemmTPP::validate_inputs() const {
    ...
    NODE_VALIDATION_CHECK(this, get_input_partial_shape(0).is_static() && get_input_partial_shape(1).is_static(),
                          "BrgemmTPP currently supports only static shapes.");
    OPENVINO_ASSERT(implication(one_of(m_type, Type::Floating, Type::WithDataRepacking), get_input_size() == 2),
                    "BrgemmTPP expects 2 inputs in cases, when input precisions are f32|f32, u8|i8 or bf16|bf16 (non-AMX system)");
    OPENVINO_ASSERT(implication(one_of(m_type, Type::WithCompensations, Type::AMX), get_input_size() == 3),
                    "BrgemmTPP expects 3 inputs with input precisions i8|i8 and bf16|bf16 on AMX system");
}
...
std::shared_ptr<Node> BrgemmTPP::clone_with_new_inputs(const OutputVector& new_args) const {
    ...
    if (!is_with_scratchpad()) {
        return std::make_shared<BrgemmTPP>(new_args.at(0), new_args.at(1), m_type,
