...
pass::FuseScaleShift::FuseScaleShift() {
    ...
    auto callback = [=](Matcher& m) {
        ...
        const autp alpha = ov::as_type_ptr<Scalar>(pattern_map.at(m_alpha).get_node_shared_ptr())->get_value<float>();
        const autp beta = ov::as_type_ptr<Scalar>(pattern_map.at(m_beta).get_node_shared_ptr())->get_value<float>();
