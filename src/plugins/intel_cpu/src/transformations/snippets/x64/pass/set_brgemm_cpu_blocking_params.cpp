...
pass::SetBrgemmCPUBlockingParams::SetBrgemmCPUBlockingParams() {
    ...
    auto callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        const auto is_f32 = input_1_precision == ov::element::f32;
        constexpr auto dyn_value = snippets::utils::get_dynamic_value<size_t>();
        // Ticket: 113745
        ...
        auto get_block_size_k = [&](const ov::Dimension& K_dim) -> size_t {
            if (K_dim.is_dynamic())
                return is_f32 ? 512 : dyn_value;

            const auto K = K_dim.get_length();
            return is_f32 ? (K > 1024 ? 1024 : K > 512 ? 512 : K) : K;
        };
        auto get_block_size_n = [&](const ov::Dimension& N_dim) -> size_t {
            return is_f32 ? 64 :
                   N_dim.is_dynamic() ? dyn_value : N_dim.get_length();
        };
