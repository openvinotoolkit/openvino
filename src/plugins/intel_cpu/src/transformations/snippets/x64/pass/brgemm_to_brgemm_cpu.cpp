...
pass::BrgemmToBrgemmCPU::BrgemmToBrgemmCPU() {
    ...
    auto callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        const bool with_amx = isAMXSupported && element_type_a != ov::element::f32 && (K % brgemmVNNIFactor == 0) &&
                              (N % brgemmVNNIFactor == 0);
        ...
        if (element_type_a == ov::element::f32) {
            ...
            brgemm_repacking = std::make_shared<BrgemmCopyB>(brgemm->input_value(1),
                                                             ...
                                                             0,
                                                             brgemm_in1_desc->get_layout());
            set_port_desc(brgemm_repacking->input(0),
            ...
            if (with_amx) {
                const auto scratch =
                    std::make_shared<snippets::op::NewMemoryBuffer>(ov::Shape{BrgemmCPU::SCRATCH_BYTE_SIZE});
                brgemm_cpu = std::make_shared<BrgemmCPU>(brgemm->input_value(0),
                ...
                brgemm_cpu = std::make_shared<BrgemmCPU>(brgemm->input_value(0),
                                                         ...
                                                         BrgemmCPU::Type::WithCompensations,
                                                         offset_a,
                                                         offset_b,
                                                         0,
                                                         offset_c,
                ...
                brgemm_cpu = std::make_shared<BrgemmCPU>(brgemm->input_value(0),
                                                         brgemm_repacking->output(0),
                                                         BrgemmCPU::Type::WithDataRepacking,
                                                         offset_a,
                                                         offset_b,
                                                         offset_c,
                                                         brgemm_in0_desc->get_layout(),
        ...
        // Transfer ports
        set_port_desc(brgemm_cpu->input(0),
        ...
        if (brgemm_repacking) {
            ...
            set_port_desc(brgemm_cpu->input(1),
                          brgemm_in1_desc->get_shape(),
                          brgemm_in1_desc->get_subtensor(),
                          brgemm_in1_desc->get_layout());
        }
        set_port_desc(brgemm_cpu->output(0),
