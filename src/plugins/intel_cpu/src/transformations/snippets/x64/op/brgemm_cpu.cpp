...
void BrgemmCPU::custom_constructor_validate_and_infer_types(std::vector<size_t> layout_a,
    ...
    const auto planar_input_shapes = std::vector<ov::PartialShape>{
...
void BrgemmCPU::validate_and_infer_types() {
    ...
    const auto planar_input_shapes =
        get_planar_input_shapes({input(0), brgemm_copy ? brgemm_copy->input(0) : input(1)});
    auto output_shape = get_output_partial_shape(planar_input_shapes);
...
void BrgemmCPU::validate_inputs() const {
    ...
    OPENVINO_ASSERT(
        implication(one_of(m_type, Type::Floating, Type::WithDataRepacking), get_input_size() == 2),
...
std::shared_ptr<BrgemmCopyB> BrgemmCPU::get_brgemm_copy() const {
    OPENVINO_ASSERT(one_of(m_type, Type::WithDataRepacking, Type::WithCompensations, Type::AMX),
                    "Brgemm doesn't need BrgemmCopyB");
...
size_t BrgemmCPU::get_offset_scratch() const {
    OPENVINO_ASSERT(is_with_scratchpad() && get_input_size() == 3,
