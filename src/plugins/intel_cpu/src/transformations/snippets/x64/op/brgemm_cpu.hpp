...
public:
    ...
    enum Type {
        Floating,           // f32|f32
        WithDataRepacking,  // u8|i8 or bf16|bf16 (non-AMX system) - needs BrgemmCopyB on second input for data
                            // repacking
    ...
    bool is_with_data_repacking() const {
        return m_type != Type::Floating;
    }
    bool is_amx() const {
        return m_type == Type::AMX;
    }
    bool is_with_scratchpad() const {
        return is_with_compensations() || is_amx();
    }
