...
public:
    ...
    enum Type {
        ...
        WithCompensations,  // Repack data and caclulate compensations - 2 outputs (is needed for BrgemmCPU with
                            // compensations)
    };

    BrgemmCopyB(const Output<Node>& x,
                const element::Type src_type,
                const Type type = Type::OnlyRepacking,
                const size_t offset_in = 0lu,
                const size_t offset_out0 = 0lu,
                const size_t offset_out1 = 0lu,
                std::vector<size_t> layout_input = {},
                const size_t blk_size_k = 0,
                const size_t blk_size_n = 0);
    BrgemmCopyB(const Output<Node>& x,
                const element::Type src_type,
                const Type type = Type::OnlyRepacking,
    ...
    size_t get_k_block_size() const {
        return m_K_blk;
    }
    size_t get_n_block_size() const {
        return m_N_blk;
    }
    size_t get_n_inner_block_size() const {
        return m_inner_n_block;
    }
    void set_k_block_size(size_t block_size) {
        m_K_blk = block_size;
    }
    void set_n_block_size(size_t block_size) {
        m_N_blk = block_size;
    }
    ...
    size_t get_brgemm_vnni_factor() const {
        return m_brgemmVNNIFactor;
    }
    element::Type get_src_element_type() const {
        return m_src_type;
    }
    bool is_with_compensations() const {
        return m_type == Type::WithCompensations;
    }
...
private:
    ...
    size_t m_K_blk = 0;
    size_t m_N_blk = 0;
    // OneDNN implementation requirement: BrgemmCopyB oneDNN implementation repacks data by m_brgemmVNNIFactor *
    // m_inner_n_block blocks. Consequently, in snippets emitter, we need to invoke the oneDNN kernel iterating
    // accordingly to this block
