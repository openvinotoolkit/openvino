...
void Transformations::UpToLpt() {
    ...
    const bool useLpt = config.get_lp_transforms_mode() &&
...
void Transformations::PreLpt(const std::vector<ov::element::Type>& defaultPrecisions) {
    ...
    auto get_convert_precisions = [&]() {
        ...
        if (!one_of(config.get_inference_precision(), element::f16, element::undefined) || !hasHardwareSupport(element::f16)) {
    ...
    if (!useLpt) {
        CPU_SET_CALLBACK_X64(
            ...
            [this](const_node_ptr& node) -> bool {
                ...
                if (node->is_dynamic() || !one_of(config.get_inference_precision(), element::f32, element::undefined) ||
                ...
                if (config.get_snippets_mode() != SnippetsMode::IGNORE_CALLBACK) {
                    ...
                    if (!group_norm || !implication(config.get_inference_precision() == element::undefined,
...
void Transformations::MainSnippets(void) {
    ...
    bool mha_token_enable_transpose_on_output = one_of(config.get_inference_precision(), element::f32, element::undefined);
    size_t concurrency = config.streamExecutorConfig.get_threads_per_stream();
    if (concurrency == 0)
        concurrency = parallel_get_max_threads();
    ...
    const bool isMHASupported =
        ...
        (dnnl::impl::cpu::x64::mayiuse(dnnl::impl::cpu::x64::avx2) &&
         one_of(config.get_inference_precision(), ov::element::f32, element::undefined)) ||
        (dnnl::impl::cpu::x64::mayiuse(dnnl::impl::cpu::x64::avx512_core) &&
         one_of(config.get_inference_precision(), ov::element::bf16, ov::element::f32, element::undefined)) ||
    ...
    auto is_supported_matmul = [this](const std::shared_ptr<const ov::Node>& n) {
        ...
        const auto is_fp32 = (in_type0 == ov::element::f32 && in_type1 == ov::element::f32 &&
                              one_of(config.get_inference_precision(), element::f32, element::undefined));
        const auto is_fp16 =
    ...
    auto mm_supports_transpose_b = [this, ignoreCallback](const std::shared_ptr<const ov::Node>& n) {
        ...
        auto push_precision = [&](const ov::element::Type& precision) {
            if (config.get_inference_precision() == ov::element::bf16 && precision == ov::element::f32)
