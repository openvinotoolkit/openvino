...
#include "transformations/cpu_opset/arm/pass/replace_power_by_mul.hpp"
...
void Transformations::PreLpt(const std::vector<ov::element::Type>& defaultPrecisions, const bool isLegacyApi) {
    ...
    // For MVN6 decomposition x^2 is calculated with bad accuracy
    // For x^1, remove power operation, for x^2 - op uses multiply instead power
    CPU_REGISTER_PASS_ARM(manager, ReplacePowerByMul);
