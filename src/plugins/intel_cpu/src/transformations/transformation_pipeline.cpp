...
void Transformations::MainSnippets(void) {
    ...
    if (snippetsMode != Config::SnippetsMode::IgnoreCallback) {
        auto mha_callback = [&](const std::shared_ptr<const ov::Node>& n) -> bool {
        ...
        CPU_SET_CALLBACK_X64(snippetsManager, mha_callback, snippets::pass::TokenizeMHASnippets);
        CPU_SET_CALLBACK_X64(snippetsManager, mha_callback, snippets::pass::ExtractReshapesFromMHA);
