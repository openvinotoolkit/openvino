...
ov::intel_cpu::PermuteNCHWAsNHWCInterpolate::PermuteNCHWAsNHWCInterpolate() {
    ...
    matcher_pass_callback callback = [=](pass::pattern::Matcher& m) {
        ...
        auto axes = (ov::as_type_ptr<ov::opset1::Constant>(interpolate->input_value(2).get_node_shared_ptr()))->cast_vector<int64_t>();
        ...
        auto input_transpose = std::make_shared<ov::opset1::Transpose>(interpolate->input_value(0), input_transpose_sizes);

        std::vector<int64_t> axes_shapes = {2, 3};
        auto axes_shapes_const = std::make_shared<ov::opset1::Constant>(ov::element::i64, ov::Shape{axes_shapes.size()}, axes_shapes);
        auto new_interpolate = interpolate->clone_with_new_inputs({input_transpose, interpolate->input_value(1), axes_shapes_const});
        ...
        auto new_transpose = std::make_shared<ov::opset1::Transpose>(new_interpolate, new_transpose_sizes);

        ov::replace_node(new_interpolate, new_transpose);
        new_transpose->set_friendly_name(new_interpolate->get_friendly_name());
        ov::copy_runtime_info(new_interpolate, new_transpose);
        return true;
    };
