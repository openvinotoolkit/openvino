...
#include "convert_fc_to_compressed_fc.hpp"
#include <memory>
...
ConvertFullyConnectedToFullyConnectedCompressed::ConvertFullyConnectedToFullyConnectedCompressed(bool convert_u4zp_to_u8) {
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        auto fc = std::dynamic_pointer_cast<ov::intel_cpu::FullyConnectedNode>(pattern_map.at(fully_connected_m).get_node_shared_ptr());
        ...
        if (with_zero_point) {
            new_fc = std::make_shared<ov::intel_cpu::FullyConnectedNode>(fc_input_a,
                                                                         fc_input_b,
                                                                         // fc_input_bias,
                                                                         fc->get_output_rank(),
                                                                         fc->get_output_type(),
