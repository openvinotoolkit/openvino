...
CausalMaskPreprocess::CausalMaskPreprocess() {
    ...
    auto constant0 =
        pattern::wrap_type<v0::Constant>(pattern::type_matches(element::f32) && pattern::shape_matches("[1, 1, 1, 1]") &&
    ...
    auto strided_slice0 = NewGenStridedSlice(reshape1, {0, 0, 0, 0}, scatter_update0, {1, 1, 1, 1}, 3);
    ...
    auto causal_mask_boolean_strided_slice = NewGenStridedSlice(multiply0, {0, 0, 0, 0}, scatter_update0, {1, 1, 1, 1}, 3);
    auto constant1 = pattern::wrap_type<v0::Constant>(
                                        pattern::type_matches(ov::element::f32) && pattern::shape_matches("[1, 1, 1, 1]") &&
                                        pattern::value_matches(std::to_string(0.0F)));
    auto equal0 = pattern::wrap_type<v1::Equal>({causal_mask_boolean_slice | causal_mask_boolean_strided_slice, constant1},
    ...
    auto constant2 = pattern::wrap_type<v0::Constant>(
                                        pattern::type_matches(ov::element::f32) && pattern::shape_matches("[1, 1, 1, 1]") &&
                                        pattern::value_matches(std::to_string(0.0F)));
    auto equal1 = pattern::wrap_type<v1::Equal>({convert1, constant2}, {{"auto_broadcast", "numpy"}});
    auto and0 =
        pattern::wrap_type<v1::LogicalAnd>({equal0, equal1}, {{"auto_broadcast", "numpy"}});
    auto masked_fill_Select = pattern::wrap_type<v1::Select>({and0, -FLT_MAX, causal_mask_boolean_slice | causal_mask_boolean_strided_slice},
                                                             {{"auto_broadcast", "numpy"}});
    auto shape_of2 = pattern::wrap_type<v0::ShapeOf>({causal_mask_boolean_slice | causal_mask_boolean_strided_slice});
    auto constant3 = pattern::wrap_type<v0::Constant>(
        pattern::type_matches(element::u8) && pattern::shape_matches("[]") && pattern::value_matches("0"));
    auto broadcast0 = pattern::wrap_type<v1::Broadcast>({masked_fill_Select, shape_of2, constant3}, {{"mode", "numpy"}});
    auto reshape3 =
        pattern::wrap_type<v1::Reshape>({broadcast0, {-1}}, {{"special_zero", false}});
    auto scatternd_update0 = pattern::wrap_type<v3::ScatterNDUpdate>({reshape0, reshape2, reshape3});
    auto reshape4 =
        pattern::wrap_type<v1::Reshape>({scatternd_update0, {"-1", "1", "max_seq_len", "max_seq_len"}},
                                        {{"special_zero", true}});
    auto scatternd_update1 = pattern::wrap_type<v3::ScatterUpdate>({{0, 0, 0, 0}, {3}, kvLen, {0}});
    auto strided_slice1 = NewGenStridedSlice(reshape4, {0, 0, 0, 0}, scatternd_update1, {1, 1, 1, 1}, 3);
    auto slice1 = pattern::wrap_type<v8::Slice>({reshape4, {0}, kvLen, {1}, {3}});
    auto index_Gather = pattern::wrap_type<v8::Gather>({slice1 | strided_slice1, cache_positions, 2}, {{"batch_dims", 0}});
