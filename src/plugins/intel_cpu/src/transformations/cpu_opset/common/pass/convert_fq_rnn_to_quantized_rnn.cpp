...
ov::intel_cpu::ConvertFqRnnToQuantizedRnn::ConvertFqRnnToQuantizedRnn() {
    ...
    auto X_m = any_input();
    auto convert_X = wrap_type<ov::op::v0::Convert>({X_m});
    auto input_shift_X = wrap_type<ov::op::v0::Constant>();
    auto subtract_X = wrap_type<ov::op::v1::Subtract>({convert_X, input_shift_X});
    auto input_scale_X = wrap_type<ov::op::v0::Constant>();

    auto deq_X = wrap_type<ov::op::v1::Multiply>({convert_X, input_scale_X}) |
                 wrap_type<ov::op::v1::Multiply>({subtract_X, input_scale_X});

    auto H_m = any_input();
    auto convert_H = wrap_type<ov::op::v0::Convert>({H_m});
    auto input_shift_H = wrap_type<ov::op::v0::Constant>();
    auto subtract_H = wrap_type<ov::op::v1::Subtract>({convert_H, input_shift_H});
    auto input_scale_H = wrap_type<ov::op::v0::Constant>();

    auto deq_H = wrap_type<ov::op::v1::Multiply>({convert_H, input_scale_H}) |
                 wrap_type<ov::op::v1::Multiply>({subtract_H, input_scale_H});

    auto H_as_const = wrap_type<ov::op::v0::Constant>();
    auto H_in = deq_H | H_as_const;

    auto cell_state_m = any_input();       // for LSTM
    auto sequence_length_m = any_input();  // for Sequences

    auto W_m = wrap_type<ov::op::v0::Constant>();
    auto convert_W = wrap_type<ov::op::v0::Convert>({W_m});
    auto weights_scale_W = wrap_type<ov::op::v0::Constant>();
    auto deq_W = wrap_type<ov::op::v1::Multiply>({convert_W, weights_scale_W});

    auto R_m = wrap_type<ov::op::v0::Constant>();
    auto convert_R = wrap_type<ov::op::v0::Convert>({R_m});
    auto weights_scale_R = wrap_type<ov::op::v0::Constant>();
    auto deq_R = wrap_type<ov::op::v1::Multiply>({convert_R, weights_scale_R});

    const auto B_m = wrap_type<ov::op::v0::Constant>();

    auto lstm_seq_m =
        wrap_type<ov::op::v5::LSTMSequence>({deq_X, H_in, cell_state_m, sequence_length_m, deq_W, deq_R, B_m});
    auto gru_seq_m = wrap_type<ov::op::v5::GRUSequence>({deq_X, H_in, sequence_length_m, deq_W, deq_R, B_m});

    auto rnn_pattern = lstm_seq_m | gru_seq_m;
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](Matcher& m) {
        ...
        if (const auto lstm_seq = ov::as_type_ptr<ov::op::v5::LSTMSequence>(rnn)) {
            ...
            auto rnn_quantized_tr = std::make_shared<ov::op::TypeRelaxed<ov::op::v5::LSTMSequence>>(
                ...
                ov::op::TemporaryReplaceOutputType(activation, element::f32).get(),
                ov::op::TemporaryReplaceOutputType(hidden_state, element::f32).get(),
                ov::op::TemporaryReplaceOutputType(cell_state, element::f32).get(),
                ov::op::TemporaryReplaceOutputType(sequence_length, element::f32).get(),
                ov::op::TemporaryReplaceOutputType(weights, element::f32).get(),
                ov::op::TemporaryReplaceOutputType(r_weights, element::f32).get(),
                ov::op::TemporaryReplaceOutputType(bias, element::f32).get(),
        ...
        } else if (const auto gru_seq = ov::as_type_ptr<ov::op::v5::GRUSequence>(rnn)) {
            const auto& sequence_length = pattern_map.at(sequence_length_m);

            auto rnn_quantized_tr = std::make_shared<ov::op::TypeRelaxed<ov::op::v5::GRUSequence>>(
        ...
        const auto input_scale_constant =
            ov::as_type_ptr<ov::op::v0::Constant>(input_scale_output.get_node_shared_ptr());
        const auto weights_scale_constant =
            ov::as_type_ptr<ov::op::v0::Constant>(weights_scale_output.get_node_shared_ptr());
        ...
        if (input_shift_it != pattern_map.end()) {
            const auto input_shift_constant =
                ov::as_type_ptr<ov::op::v0::Constant>(input_shift_it->second.get_node_shared_ptr());
        ...
        if (hidden_state_it != pattern_map.end()) {
            ...
            if (subtract_it != pattern_map.end()) {
                const auto subtract = ov::as_type_ptr<ov::op::v1::Subtract>(subtract_it->second.get_node_shared_ptr());
    ...
    auto m = std::make_shared<Matcher>(rnn_pattern, matcher_name);
