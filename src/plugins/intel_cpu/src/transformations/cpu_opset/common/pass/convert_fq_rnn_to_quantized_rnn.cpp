...
ov::intel_cpu::ConvertFqRnnToQuantizedRnn::ConvertFqRnnToQuantizedRnn() {
    ...
    auto deq_X = std::make_shared<ov::pass::pattern::op::Or>(OutputVector{
    ...
    auto deq_H = std::make_shared<ov::pass::pattern::op::Or>(OutputVector{
    ...
    auto lstm_seq_m = ov::pass::pattern::wrap_type<ov::opset9::LSTMSequence>(
        {deq_X, H_in, cell_state_m, sequence_length_m, deq_W, deq_R, B_m});
    auto gru_seq_m =
        ov::pass::pattern::wrap_type<ov::opset9::GRUSequence>({deq_X, H_in, sequence_length_m, deq_W, deq_R, B_m});
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        const auto input_scale_constant =
            std::dynamic_pointer_cast<ov::opset9::Constant>(input_scale_output.get_node_shared_ptr());
        const auto weights_scale_constant =
            std::dynamic_pointer_cast<ov::opset9::Constant>(weights_scale_output.get_node_shared_ptr());
        ...
        if (input_shift_it != pattern_map.end()) {
            const auto input_shift_constant =
                std::dynamic_pointer_cast<ov::opset9::Constant>(input_shift_it->second.get_node_shared_ptr());
        ...
        if (hidden_state_it != pattern_map.end()) {
            ...
            if (subtract_it != pattern_map.end()) {
                const auto subtract =
                    std::dynamic_pointer_cast<ov::opset9::Subtract>(subtract_it->second.get_node_shared_ptr());
