...
ov::intel_cpu::ConvertMatMulToFC::ConvertMatMulToFC() {
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        if (rank_b != 2) {
            ...
            auto reshape_shape_values =
                matmul->get_transpose_b() ? std::vector<int64_t>{-1, k_len} : std::vector<int64_t>{k_len, -1};
        ...
        auto fc = std::make_shared<ov::intel_cpu::FullyConnectedNode>(fc_input_a,
                                                                      fc_input_b,
                                                                      output_rank,
