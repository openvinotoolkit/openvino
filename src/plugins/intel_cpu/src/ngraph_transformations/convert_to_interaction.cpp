...
ov::intel_cpu::ConvertInteractionInt8::ConvertInteractionInt8() {
    ...
    OutputVector features_output;
    auto dense_fq_m = wrap_type<ov::opset8::FakeQuantize>({any_input(has_static_shape()), wrap_type<ov::opset1::Constant>(),
        wrap_type<ov::opset1::Constant>(),
        wrap_type<ov::opset1::Constant>(),
        wrap_type<ov::opset1::Constant>()});
    ...
    auto reshape_m = wrap_type<ov::opset8::Reshape>({concat_m->output(0), any_input()->output(0)});
    auto matmul_m = wrap_type<ov::opset1::MatMul>({reshape_m, reshape_m});
    auto sparse_fq = wrap_type<ov::opset8::FakeQuantize>({matmul_m->output(0), wrap_type<ov::opset1::Constant>(),
        wrap_type<ov::opset1::Constant>(),
        wrap_type<ov::opset1::Constant>(),
        wrap_type<ov::opset1::Constant>()});
    ...
    matcher_pass_callback callback = [=](Matcher& m) {
        ...
        for (size_t i = 1; i < dense_fq_node->get_input_size(); i++) {
            auto dense_const = ov::as_type_ptr<ov::opset8::Constant>(dense_fq_node->get_input_node_shared_ptr(i))->cast_vector<float>();
            auto sparse_const = ov::as_type_ptr<ov::opset8::Constant>(sparse_fq_node->get_input_node_shared_ptr(i))->cast_vector<float>();
