...
ov::intel_cpu::MHAFloatFusion::MHAFloatFusion() {
    ...
    auto in6 = ngraph::pattern::any_input(ngraph::pattern::has_static_shape());
    auto in7 = ngraph::pattern::wrap_type<ngraph::opset4::Constant>();
    auto in8 = ngraph::pattern::wrap_type<ngraph::opset4::Constant>();
    ...
    auto transpose2 = std::make_shared<ngraph::opset3::Transpose>(in6, in7);
    auto matmul1 = std::make_shared<ngraph::opset3::MatMul>(softmax, transpose2);
    auto transpose3 = std::make_shared<ngraph::opset3::Transpose>(matmul1, in8);
    ...
    ngraph::matcher_pass_callback callback = [=](ngraph::pattern::Matcher& m) {
        ...
        auto transpose2_in = pattern_to_output.at(in6);
        ...
        if (!valid_transpose_order(pattern_to_output.at(in7).get_node_shared_ptr(), {0, 2, 1, 3})) return false;
        if (!valid_transpose_order(pattern_to_output.at(in8).get_node_shared_ptr(), {0, 2, 1, 3})) return false;
        ...
        if (!softmax_node)
            return false;
        if (softmax_node->get_axis() != softmax_node->get_input_partial_shape(0).rank().get_length() - 1)
            return false;
...
ov::intel_cpu::MHAQuantFusion::MHAQuantFusion() {
    ...
    auto softmax = std::make_shared<ngraph::opset1::Softmax>(mul);
    auto fakeQuantize1 = ngraph::pattern::wrap_type<ngraph::opset1::FakeQuantize>({softmax,
                                                                                   ngraph::pattern::wrap_type<ngraph::opset4::Constant>(),
                                                                                   ngraph::pattern::wrap_type<ngraph::opset4::Constant>(),
                                                                                   ngraph::pattern::wrap_type<ngraph::opset4::Constant>(),
                                                                                   ngraph::pattern::wrap_type<ngraph::opset4::Constant>()});
    ...
    auto fakeQuantize2 = ngraph::pattern::wrap_type<ngraph::opset1::FakeQuantize>({matmul1,
                                                                                   ...
                                                                                   ngraph::pattern::wrap_type<ngraph::opset4::Constant>(),
                                                                                   ngraph::pattern::wrap_type<ngraph::opset4::Constant>()});
    auto transpose3 = std::make_shared<ngraph::opset3::Transpose>(fakeQuantize2, in8);
