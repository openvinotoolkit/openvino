...
ov::intel_cpu::MHAFusion::MHAFusion() {
    ...
    ngraph::matcher_pass_callback callback = [=](ngraph::pattern::Matcher& m) {
        ...
        auto mul_node = std::dynamic_pointer_cast<ngraph::opset3::Multiply>(pattern_to_output.at(mul).get_node_shared_ptr());
        if (!mul_node)
            return false;
        ...
        auto mha = std::make_shared<ov::intel_cpu::MHANode>(transpose0_in, transpose1_in, mul_in1, add_in1, transpose2_in, add->output(0).get_element_type());
        ...
        MATCHER_SCOPE_ENABLE(MHAFusion);
        return true;
    };
...
ov::intel_cpu::MHAQuantFusion::MHAQuantFusion() {
    ...
    ngraph::matcher_pass_callback callback = [=](ngraph::pattern::Matcher& m) {
        ...
        auto mha = std::make_shared<ov::intel_cpu::MHANode>(transpose0_in, transpose1_in, mul_in1, add_in1, transpose2_in,
                                                            fq0_scale, fq1_scale, fq2_scale, add->output(0).get_element_type());
        mha->set_friendly_name(m.get_match_root()->get_friendly_name());
        ngraph::copy_runtime_info({pattern_to_output.at(transpose0).get_node_shared_ptr(),
                                   pattern_to_output.at(transpose1).get_node_shared_ptr(),
                                   pattern_to_output.at(matmul0).get_node_shared_ptr(),
        ...
        ngraph::replace_node(m.get_match_root(), mha);
        MATCHER_SCOPE_ENABLE(MHAQuantFusion);
        return true;
    };
