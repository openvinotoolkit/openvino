...
ov::intel_cpu::ConvertFqRnnToQuantizedRnn::ConvertFqRnnToQuantizedRnn() {
    ...
    auto convert_X = ngraph::pattern::wrap_type<ngraph::op::Convert>({X_m});
    auto input_shift_X = ngraph::pattern::wrap_type<ngraph::op::Constant>();
    auto subtract_X = ngraph::pattern::wrap_type<ngraph::op::v1::Subtract>({convert_X, input_shift_X});
    auto input_scale_X = ngraph::pattern::wrap_type<ngraph::op::Constant>();

    auto deq_X = std::make_shared<ngraph::pattern::op::Or>(
        OutputVector{
            ngraph::pattern::wrap_type<ngraph::op::v1::Multiply>({convert_X, input_scale_X}),
            ngraph::pattern::wrap_type<ngraph::op::v1::Multiply>({subtract_X, input_scale_X}),
        });

    auto H_m = ngraph::pattern::any_input();
    auto convert_H = ngraph::pattern::wrap_type<ngraph::op::Convert>({H_m});
    auto input_shift_H = ngraph::pattern::wrap_type<ngraph::op::Constant>();
    auto subtract_H = ngraph::pattern::wrap_type<ngraph::op::v1::Subtract>({convert_H, input_shift_H});
    auto input_scale_H = ngraph::pattern::wrap_type<ngraph::op::Constant>();

    auto deq_H = std::make_shared<ngraph::pattern::op::Or>(
        OutputVector{
            ngraph::pattern::wrap_type<ngraph::op::v1::Multiply>({convert_H, input_scale_H}),
            ngraph::pattern::wrap_type<ngraph::op::v1::Multiply>({subtract_H, input_scale_H}),
        });

    auto H_as_const = ngraph::pattern::wrap_type<ngraph::op::Constant>();
    ...
    auto weights_scale_W = ngraph::pattern::wrap_type<ngraph::op::Constant>();
    auto deq_W = ngraph::pattern::wrap_type<ngraph::op::v1::Multiply>({convert_W, weights_scale_W});

    auto R_m = ngraph::pattern::wrap_type<ngraph::op::Constant>();
    auto convert_R = ngraph::pattern::wrap_type<ngraph::op::Convert>({R_m});
    auto weights_scale_R = ngraph::pattern::wrap_type<ngraph::op::Constant>();
    auto deq_R = ngraph::pattern::wrap_type<ngraph::op::v1::Multiply>({convert_R, weights_scale_R});
    ...
    ngraph::matcher_pass_callback callback = [=](ngraph::pattern::Matcher& m) {
        ...
        if (const auto lstm_seq = ngraph::as_type_ptr<ngraph::opset8::LSTMSequence>(rnn)) {
            ...
            auto rnn_quantized_tr = std::make_shared<ngraph::op::TypeRelaxed<ngraph::opset8::LSTMSequence>>(
        ...
        } else if (const auto gru_seq = ngraph::as_type_ptr<ngraph::opset8::GRUSequence>(rnn)) {
            const auto& sequence_length = pattern_map.at(sequence_length_m);

            auto rnn_quantized_tr = std::make_shared<ngraph::op::TypeRelaxed<ngraph::opset8::GRUSequence>>(
        ...
        const auto input_scale_constant   = std::dynamic_pointer_cast<ngraph::op::Constant>(input_scale_output.get_node_shared_ptr());
        const auto weights_scale_constant = std::dynamic_pointer_cast<ngraph::op::Constant>(weights_scale_output.get_node_shared_ptr());
        ...
        if (input_shift_it != pattern_map.end()) {
            const auto  input_shift_constant = std::dynamic_pointer_cast<ngraph::op::Constant>(input_shift_it->second.get_node_shared_ptr());
        ...
        if (hidden_state_it != pattern_map.end()) {
            ...
            if (subtract_it != pattern_map.end()) {
                const auto subtract = std::dynamic_pointer_cast<ngraph::op::v1::Subtract>(subtract_it->second.get_node_shared_ptr());
