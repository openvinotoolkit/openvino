...
std::vector<std::vector<int>> apply_hyper_threading(bool& input_ht_hint,
    ...
    if (proc_type_table[0][HYPER_THREADING_PROC] > 0) {
        if (((!input_ht_hint) && input_ht_changed) ||
            ((!input_ht_changed) && (input_pm_hint == ov::hint::PerformanceMode::LATENCY)) ||
            ((!input_ht_changed) && (input_pm_hint == ov::hint::PerformanceMode::THROUGHPUT) &&
