...
void GraphOptimizer::FuseConvMatmulFCDeconvAndDQScales(Graph& graph) {
    ...
    auto isDQScaleGraphPattern = [](NodePtr node) {
        ...
        if (!(parentNode->getType() == Type::Convolution || parentNode->getType() == Type::MatMul ||
              parentNode->getType() == Type::Deconvolution || parentNode->getType() == Type::FullyConnected))
            return false;
...
void GraphOptimizer::FuseFCAndWeightsDecompression(Graph& graph) {
    ...
    for (size_t i = 0; i < graphNodes.size(); i++) {
        ...
        if (supportedDataPrecisions.find(fcNode->getOriginalInputPrecisionAtPort(0)) == supportedDataPrecisions.end())
            continue;
        if (supportedWeightsPrecisions.find(weightsNode->getOriginalOutputPrecisionAtPort(0)) ==
            supportedWeightsPrecisions.end())
            continue;
        if (withSubtract && !one_of(subtractConstNode->getOriginalOutputPrecisionAtPort(0),
                                    weightsNode->getOriginalOutputPrecisionAtPort(0),
                                    ov::element::f32))
            continue;
        ...
        if (weightsShape != multiplyNode->getOutputShapeAtPort(0))
            continue;
        if (reshapeNode &&
            (reshapeNode->getInputShapeAtPort(0).getRank() != 3 || reshapeNode->getOutputShapeAtPort(0).getRank() != 2))
            continue;
        ...
        if (fcInputWeightsShape.getRank() == weightsShape.getRank()) {
            ...
            // Group decompression case: last 3 dimension (there could be also prepending '1's in the beginning) of
            // weights shape must be: [N, G, O], if transpose = true [O, N, G], otherwise. O - output channels N -
            // number of groups G - group size
        ...
        auto check_decompression_shape = [&decompressionConstShape](const VectorDims& shape_to_check) {
            ...
            return std::equal(shape_to_check.begin(),
                              shape_to_check.end(),
                              decompressionConstShape.begin() + comparison_start_pos);
        };
        ...
        if (impl::cpu::x64::mayiuse(impl::cpu::x64::avx512_core_amx) &&
            ...
            // OneDNN AMX IP implementation has limited shapes support due to performance considerations. As a current
            // solution conditions below are copied from OneDNN to make sure correct IP impl will be used since fallback
            // one doesn't support weights decompression feature.
...
void GraphOptimizer::FuseFCAndConvertOnWeights(Graph& graph) {
    ...
    auto isSuitablePattern = [](NodePtr parent) {
        bool res = true && parent->getType() == Type::Convert && parent->getChildEdges().size() == 1 &&
...
void GraphOptimizer::FuseFCAndTransposeOnWeights(Graph& graph) {
    ...
    auto isSuitablePattern = [](NodePtr parent) {
        bool res = true && parent->getType() == Type::Transpose && parent->getChildEdges().size() == 1 &&
                   parent->getChildEdgeAt(0)->getOutputNum() == 1 &&
                   parent->getChildEdgeAt(0)->getChild()->getType() == Type::FullyConnected &&
                   parent->getOutputShapeAtPort(0).getRank() == 2 && parent->isConstant();
        return res;
    };
