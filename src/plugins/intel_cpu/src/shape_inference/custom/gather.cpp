...
ShapeInferPtr GatherShapeInferFactory::makeShapeInfer() const {
    ...
    const bool isAxisInputConst = ov::is_type<ov::op::v0::Constant>(m_op->get_input_node_ptr(GATHER_AXIS));
    ...
    const bool isIndicesScalar = indicesShape.rank().get_length() == 0;
    const int axis =
        isAxisInputConst
            ? ov::as_type<ov::op::v0::Constant>(m_op->get_input_node_ptr(GATHER_AXIS))->cast_vector<int>()[0]
            : 0;
    const int batchDims = ov::is_type<ov::op::v8::Gather>(m_op)
