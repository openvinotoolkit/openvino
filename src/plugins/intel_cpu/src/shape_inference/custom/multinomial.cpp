...
Result MultinomialShapeInfer::infer(const std::vector<std::reference_wrapper<const VectorDims>>& input_shapes,
    ...
    // const auto output_partial_shape = shape_infer(m_op.get(), input_shape_vec, make_tensor_accessor(data_dependency))[0];
    // VectorDims dims;
    // std::transform(output_partial_shape.begin(), output_partial_shape.end(), std::back_inserter(dims), [](ov::Dimension d) { return d.get_max_length();} );
    // return {{dims}, ShapeInferStatus::success};
}
