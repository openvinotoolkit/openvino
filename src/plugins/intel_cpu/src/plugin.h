...
public:
    ...
    std::shared_ptr<RemoteContextImpl> get_default_context() const {
        return m_remote_context;
    }

private:
    void get_performance_streams(Config& config, const std::shared_ptr<ov::Model>& model, int& modelPreferThreads) const;
    ...
    std::shared_ptr<RemoteContextImpl> m_remote_context;
};
