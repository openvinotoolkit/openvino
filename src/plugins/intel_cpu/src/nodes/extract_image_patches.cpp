...
void ExtractImagePatches::ExtractImagePatchesRefExecutor::executeReference(void* src,
    ...
    parallel_for4d(OB, jpp.KH, jpp.KW, IC, [&](const size_t ob, const size_t kh, const size_t kw, const size_t ic) {
        const auto iw_start = static_cast<int64_t>(kw * RW) - PL;
        const auto ih_start = static_cast<int64_t>(kh * RH) - PT;
        const auto ih_lpad = ih_start >= 0 ? 0 : static_cast<size_t>(std::ceil(-1.F * static_cast<float>(ih_start) / static_cast<float>(jpp.SH)));
        const auto iw_lpad = iw_start >= 0 ? 0 : static_cast<size_t>(std::ceil(-1.F * static_cast<float>(iw_start) / static_cast<float>(jpp.SW)));

        const auto ih_hpad = static_cast<size_t>(
            std::ceil((static_cast<float>(IH) - 1.F * static_cast<float>(ih_start)) / static_cast<float>(jpp.SH)) > static_cast<float>(jpp.OH) ? static_cast<float>(jpp.OH) : std::ceil((static_cast<float>(IH) + -1.F * static_cast<float>(ih_start)) / static_cast<float>(jpp.SH)));
        const auto iw_hpad = static_cast<size_t>(
            std::ceil((static_cast<float>(jpp.IW) - 1.F * static_cast<float>(iw_start)) / static_cast<float>(jpp.SW)) > static_cast<float>(jpp.OW)
                                   ? static_cast<float>(jpp.OW)
                                   : std::ceil((static_cast<float>(jpp.IW) - 1.F * static_cast<float>(iw_start)) / static_cast<float>(jpp.SW)));
...
jit_extract_image_patches_params ExtractImagePatches::ExtractImagePatchesExecutor::fillJpp(
    ...
    if (padType != ExtImgPatcherPadType::VALID) {
        const auto ihStep = static_cast<int64_t>(kSizes[0]) + (static_cast<int64_t>(rates[0]) - 1) * (static_cast<int64_t>(kSizes[0]) - 1);
        const auto iwStep = static_cast<int64_t>(kSizes[1]) + (static_cast<int64_t>(rates[1]) - 1) * (static_cast<int64_t>(kSizes[1]) - 1);

        auto PW = static_cast<int64_t>((std::ceil(1.F * static_cast<float>(jpp.IW) / static_cast<float>(strides[1])) - 1) * static_cast<float>(strides[1]) + static_cast<float>(iwStep) - static_cast<float>(jpp.IW));
        auto PH = static_cast<int64_t>((std::ceil(1.F * static_cast<float>(IH) / static_cast<float>(strides[0])) - 1) * static_cast<float>(strides[0]) + static_cast<float>(ihStep) - static_cast<float>(IH));
