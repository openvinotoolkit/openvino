...
void ExtractImagePatches::ExtractImagePatchesRefExecutor::executeReference(void* src,
    ...
    parallel_for4d(OB, jpp.KH, jpp.KW, IC, [&](const size_t ob, const size_t kh, const size_t kw, const size_t ic) {
        ...
        const size_t ih_lpad = ih_start >= 0 ? 0 : static_cast<size_t>(std::ceil(-1.F * static_cast<float>(ih_start) / jpp.SH));
        const size_t iw_lpad = iw_start >= 0 ? 0 : static_cast<size_t>(std::ceil(-1.F * static_cast<float>(iw_start) / jpp.SW));

        const size_t ih_hpad = std::ceil((IH - 1.F * static_cast<float>(ih_start)) / jpp.SH) > jpp.OH
                                   ? jpp.OH
                                   : static_cast<size_t>(std::ceil((IH + -1.F * static_cast<float>(ih_start)) / jpp.SH));
        const size_t iw_hpad = std::ceil((jpp.IW - 1.F * static_cast<float>(iw_start)) / jpp.SW) > jpp.OW
                                   ? jpp.OW
                                   : static_cast<size_t>(std::ceil((jpp.IW - 1.F * static_cast<float>(iw_start)) / jpp.SW));
...
jit_extract_image_patches_params ExtractImagePatches::ExtractImagePatchesExecutor::fillJpp(
    ...
    if (padType != ExtImgPatcherPadType::VALID) {
        ...
        int64_t PW = (static_cast<int64_t>(std::ceil(1.F * jpp.IW / strides[1])) - 1) * strides[1] + iwStep - static_cast<int64_t>(jpp.IW);
        int64_t PH = (static_cast<int64_t>(std::ceil(1.F * IH / strides[0])) - 1) * strides[0] + ihStep - static_cast<int64_t>(IH);
