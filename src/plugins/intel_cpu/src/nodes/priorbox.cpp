    ...
    for (int64_t h = 0; h < H; ++h) {
        for (int64_t w = 0; w < W; ++w) {
            ...
            for (size_t s = 0; s < fixed_size.size(); ++s) {
                ...
                box_width = box_height = static_cast<float>(fixed_size_) * 0.5F;
                    ...
                    for (float ar : fixed_ratio) {
                        ...
                        for (int64_t r = 0; r < density_; ++r) {
                            for (int64_t c = 0; c < density_; ++c) {
                                float center_x_temp = center_x - static_cast<float>(fixed_size_) / 2.F + static_cast<float>(shift) / 2.F + static_cast<float>(c) * static_cast<float>(shift);
                                float center_y_temp = center_y - static_cast<float>(fixed_size_) / 2.F + static_cast<float>(shift) / 2.F + static_cast<float>(r) * static_cast<float>(shift);
