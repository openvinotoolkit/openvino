...
void istft_impl(const float* in_data,
    ...
    const auto num_frames = static_cast<size_t>(data_shape[static_cast<std::ptrdiff_t>(frames_axis)]);
    ...
    const auto fft_results_dim = static_cast<size_t>(data_shape[static_cast<std::ptrdiff_t>(data_shape.size() - 3)]);
    ...
    std::copy(window, window + window_shape[0], pad_window.begin() + static_cast<std::vector<float>::difference_type>((frame_size_dim - window_length) / 2));
