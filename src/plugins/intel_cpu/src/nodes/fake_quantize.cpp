...
void FakeQuantize::executeQuantization(const std::unique_ptr<jit_uni_quantize_kernel>& pKernel) const {
    ...
    if (srcDesc.hasLayoutType(LayoutType::ncsp) && srcDesc.getShape().getRank() == 3) {
        ...
        parallel_nd(N, CB, D, B, [&](dim_t n, dim_t cb, dim_t d, dim_t b) {
            ...
            const size_t data_off = any_of(srcDims.size(), 3, 4)
        ...
        parallel_nd_legacy(N, CB, D, H, [&](dim_t n, dim_t cb, dim_t d, dim_t h) {
            ...
            } else if (any_of(srcDims.size(), 3, 4)) {
...
void FakeQuantize::updateOptimizedFormula(bool do_rounding) {
    ...
    CPU_NODE_ASSERT(any_of(inputScale.size(), 1, OC), "inputScale.size() == ", inputScale.size());
    CPU_NODE_ASSERT(any_of(inputShift.size(), 1, OC), "inputShift.size() == ", inputShift.size());
    CPU_NODE_ASSERT(any_of(cropLow.size(), 1, OC), "cropLow.size() == ", cropLow.size());
    CPU_NODE_ASSERT(any_of(cropHigh.size(), 1, OC), "cropHigh.size() == ", cropHigh.size());
    CPU_NODE_ASSERT(any_of(outputScale.size(), 1, OC), "outputScale.size() == ", outputScale.size());
    CPU_NODE_ASSERT(any_of(outputShift.size(), 1, OC), "outputShift.size() == ", outputShift.size());
    ...
    if (all_of(1, f.ish.size(), f.clo.size(), f.chi.size())) {
