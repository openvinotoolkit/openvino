...
template <cpu_isa_t isa>
void jit_fft_kernel_f32<isa>::generate() {
    using namespace Xbyak::util;
    using Xbyak::Label;
    using Xbyak::Address;
    using Xbyak::Xmm;
...
template <cpu_isa_t isa>
void jit_fft_kernel_f32<isa>::uni_vpermilps(const Xbyak::Xmm& x, const Xbyak::Operand& op, int8_t control) {
    movsd(x, op);
    shufps(x, x, control);
}

template <cpu_isa_t isa>
void jit_fft_kernel_f32<isa>::uni_vpermilps(const Xbyak::Ymm& x, const Xbyak::Operand& op, int8_t control) {
    uni_vpermilps(x, op, control);
}

template struct jit_fft_kernel_f32<cpu::x64::avx2>;
template struct jit_fft_kernel_f32<cpu::x64::avx512_core>;
