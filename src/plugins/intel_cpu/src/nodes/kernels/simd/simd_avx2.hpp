...
inline void store(vec<float, isa::avx2> v, ov::bfloat16* p) {
    // f32 → bf16: round-to-nearest-even then truncate to upper 16 bits.
    auto i32 = _mm256_castps_si256(v.v);
    auto rounded = _mm256_srli_epi32(_mm256_add_epi32(i32, _mm256_set1_epi32(0x00008000)), 16);
    // Pack 8x32→8x16 via _mm256_packs_epi32 (signed), then extract lower 128 bits.
    auto packed = _mm256_packs_epi32(rounded, rounded);
    auto lo = _mm256_castsi256_si128(packed);
    auto hi = _mm256_extracti128_si256(packed, 1);
    auto result = _mm_unpacklo_epi32(lo, hi);
