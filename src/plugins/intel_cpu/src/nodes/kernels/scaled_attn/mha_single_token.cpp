...
template <typename TA, typename TB>
static float dot_product(TA* a, TB* b, size_t n, float* scale, float* zp, float* head_sum, [[maybe_unused]] size_t group_size) {
    size_t i = 0;
    float sum = 0.0f;
#if defined(HAVE_AVX512F)
...
static float dot_product_by_channel(TA* a, uint8_t* b, size_t n, float* scale, float* zp, [[maybe_unused]] size_t group_size) {
    float sum = 0.0f;
    size_t i = 0;
#if defined(HAVE_AVX512F)
