...
template <typename T>
static void reduce_head(int dim, T* dst, float* src, int nthr, size_t stride, bool apply_inv_rotation) {
...
static void mha_reduce_typed(const PlainTensor& attn_score,
    ...
    auto reduce_body = [&](size_t b, size_t h, size_t m) {
        ...
        reduce_head(static_cast<int>(S), dst, src0, nthr, thread_stride, apply_inv_rotation);
    };
    if (q_len == 1) {
