...
repackB(Tdst* dst, ov::float16* src, int N_stride, int N, int K) {
    ...
    OPENVINO_DEBUG_ASSERT(K <= 32, "K must be less than or equal to 32: K=", K, ", max_allowed=32");
    OPENVINO_DEBUG_ASSERT(N <= 16, "N must be less than or equal to 16: N=", N, ", max_allowed=16");
    int k = 0;
    Tdst zero(0.0F);
    for (; k < 32; k += 2) {
        int n = 0;
...
static void repackB(int8_t* dst, int8_t* src, int N_stride, int N, int K) {
    ...
    OPENVINO_DEBUG_ASSERT(K <= 64, "K must be less than or equal to 64: K=", K, ", max_allowed=64");
    OPENVINO_DEBUG_ASSERT(N <= 16, "N must be less than or equal to 16: N=", N, ", max_allowed=16");
...
void MatrixDynQuantPerRow::quantize(size_t BM, ov::bfloat16* psrc, int src_stride) const {
    OPENVINO_DEBUG_ASSERT(static_cast<int64_t>(BM) <= M, "BM exceeds M: BM=", BM, ", M=", M, ", type=bfloat16");
...
void MatrixDynQuantPerRow::quantize(size_t BM, ov::float16* psrc, int src_stride) const {
    OPENVINO_DEBUG_ASSERT(static_cast<int64_t>(BM) <= M, "BM exceeds M: BM=", BM, ", M=", M, ", type=float16");
    parallel_nt_static(0, [&](const size_t ithr, const size_t nthr) {
        size_t start{0};
        size_t end{0};
        splitter(BM, nthr, ithr, start, end);
