...
jit_int8_brgemm_kernel_2x8_udot_packed::jit_int8_brgemm_kernel_2x8_udot_packed() : jit_generator() {}

void jit_int8_brgemm_kernel_2x8_udot_packed::create_ker() {
    jit_generator::create_kernel();
    ker_ = ov::intel_cpu::jit_kernel_cast<ker_t>(jit_ker());
}

void jit_int8_brgemm_kernel_2x8_udot_packed::generate() {
    preamble();
...
jit_int8_brgemm_kernel_2x8_udot_packed_strided::jit_int8_brgemm_kernel_2x8_udot_packed_strided() : jit_generator() {}

void jit_int8_brgemm_kernel_2x8_udot_packed_strided::create_ker() {
    jit_generator::create_kernel();
    ker_ = ov::intel_cpu::jit_kernel_cast<ker_t>(jit_ker());
}

void jit_int8_brgemm_kernel_2x8_udot_packed_strided::generate() {
    preamble();
    ...
    add(reg_src0, reg_src0, 16);
    add(reg_src1, reg_src1, 16);
    udot(v_acc00, v_src0, v_w0);
    udot(v_acc01, v_src0, v_w1);
    udot(v_acc02, v_src0, v_w2);
    udot(v_acc03, v_src0, v_w3);
    udot(v_acc04, v_src0, v_w4);
    udot(v_acc05, v_src0, v_w5);
    udot(v_acc06, v_src0, v_w6);
    udot(v_acc07, v_src0, v_w7);
    udot(v_acc10, v_src1, v_w0);
    udot(v_acc11, v_src1, v_w1);
    udot(v_acc12, v_src1, v_w2);
    udot(v_acc13, v_src1, v_w3);
    udot(v_acc14, v_src1, v_w4);
    udot(v_acc15, v_src1, v_w5);
    udot(v_acc16, v_src1, v_w6);
    udot(v_acc17, v_src1, v_w7);
...
void jit_int8_brgemm_kernel_2x8_udot_packed_strided_interleaved4::create_ker() {
    jit_generator::create_kernel();
    ker_ = ov::intel_cpu::jit_kernel_cast<ker_t>(jit_ker());
}

void jit_int8_brgemm_kernel_2x8_udot_packed_strided_interleaved4::generate() {
    preamble();
    ...
    const VReg16B v_w0 = VReg16B(2);
    const VReg16B v_w1 = VReg16B(3);
    const VReg16B v_mask = VReg16B(4);
    ...
    eor(v_src0b, v_src0b, v_mask);
    eor(v_src1b, v_src1b, v_mask);
...
void jit_int8_brgemm_kernel_2x16_udot_packed_strided_interleaved4::create_ker() {
    jit_generator::create_kernel();
    ker_ = ov::intel_cpu::jit_kernel_cast<ker_t>(jit_ker());
}

void jit_int8_brgemm_kernel_2x16_udot_packed_strided_interleaved4::generate() {
    preamble();
    ...
    const VReg16B v_w2 = VReg16B(4);
    const VReg16B v_w3 = VReg16B(5);
    const VReg16B v_mask = VReg16B(6);
...
void jit_int8_brgemm_kernel_2x32_udot_packed_strided_interleaved4::create_ker() {
    jit_generator::create_kernel();
    ker_ = ov::intel_cpu::jit_kernel_cast<ker_t>(jit_ker());
}

void jit_int8_brgemm_kernel_2x32_udot_packed_strided_interleaved4::generate() {
    preamble();
    ...
    const XReg reg_src_stride = abi_param5;
    const XReg reg_ldc = abi_param6;
    const XReg reg_bias = abi_param7;
    const XReg reg_accum = abi_param8;
    ...
    const XReg reg_c1 = x13;
    const XReg reg_kb = x14;
    const XReg reg_kb_tail = x16;

    const WReg reg_mask = w15;
    ...
    const VReg4S v_tmp0 = VReg4S(11);
    const VReg4S v_tmp1 = VReg4S(12);
    const QReg q_acc00(v_acc00.getIdx());
    const QReg q_acc01(v_acc01.getIdx());
    const QReg q_acc02(v_acc02.getIdx());
    const QReg q_acc03(v_acc03.getIdx());
    ...
    ldr(q_tmp0, ptr(reg_bias));
    add(v_acc00, v_acc00, v_tmp0);
    str(q_acc00, ptr(reg_c0));
    ldr(q_tmp1, ptr(reg_bias, 16));
    add(v_acc01, v_acc01, v_tmp1);
    str(q_acc01, ptr(reg_c0, 16));
    ldr(q_tmp0, ptr(reg_bias, 32));
    add(v_acc02, v_acc02, v_tmp0);
    str(q_acc02, ptr(reg_c0, 32));
    ldr(q_tmp1, ptr(reg_bias, 48));
    add(v_acc03, v_acc03, v_tmp1);
    str(q_acc03, ptr(reg_c0, 48));
    ldr(q_tmp0, ptr(reg_bias, 64));
    add(v_acc04, v_acc04, v_tmp0);
    str(q_acc04, ptr(reg_c0, 64));
    ldr(q_tmp1, ptr(reg_bias, 80));
    add(v_acc05, v_acc05, v_tmp1);
    str(q_acc05, ptr(reg_c0, 80));
    ldr(q_tmp0, ptr(reg_bias, 96));
    add(v_acc06, v_acc06, v_tmp0);
    str(q_acc06, ptr(reg_c0, 96));
    ldr(q_tmp1, ptr(reg_bias, 112));
    add(v_acc07, v_acc07, v_tmp1);
    str(q_acc07, ptr(reg_c0, 112));
    ldr(q_tmp0, ptr(reg_bias));
    add(v_acc10, v_acc10, v_tmp0);
    str(q_acc10, ptr(reg_c1));
    ...
    str(q_acc13, ptr(reg_c1, 48));
    ldr(q_tmp0, ptr(reg_bias, 64));
    add(v_acc14, v_acc14, v_tmp0);
    str(q_acc14, ptr(reg_c1, 64));
    ldr(q_tmp1, ptr(reg_bias, 80));
    add(v_acc15, v_acc15, v_tmp1);
    str(q_acc15, ptr(reg_c1, 80));
    ldr(q_tmp0, ptr(reg_bias, 96));
    add(v_acc16, v_acc16, v_tmp0);
    str(q_acc16, ptr(reg_c1, 96));
    ldr(q_tmp1, ptr(reg_bias, 112));
    add(v_acc17, v_acc17, v_tmp1);
    str(q_acc17, ptr(reg_c1, 112));
    b(done);

    L(store_acc);
    cbnz(reg_bias, store_acc_bias);
    ldr(q_tmp0, ptr(reg_c0));
    add(v_acc00, v_acc00, v_tmp0);
    str(q_acc00, ptr(reg_c0));
    ldr(q_tmp1, ptr(reg_c0, 16));
    ...
    L(store_acc_bias);
    ldr(q_tmp0, ptr(reg_c0));
    ldr(q_tmp1, ptr(reg_bias));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc00, v_acc00, v_tmp0);
    str(q_acc00, ptr(reg_c0));
    ldr(q_tmp0, ptr(reg_c0, 16));
    ldr(q_tmp1, ptr(reg_bias, 16));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc01, v_acc01, v_tmp0);
    str(q_acc01, ptr(reg_c0, 16));
    ldr(q_tmp0, ptr(reg_c0, 32));
    ldr(q_tmp1, ptr(reg_bias, 32));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc02, v_acc02, v_tmp0);
    str(q_acc02, ptr(reg_c0, 32));
    ldr(q_tmp0, ptr(reg_c0, 48));
    ldr(q_tmp1, ptr(reg_bias, 48));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc03, v_acc03, v_tmp0);
    str(q_acc03, ptr(reg_c0, 48));
    ldr(q_tmp0, ptr(reg_c0, 64));
    ldr(q_tmp1, ptr(reg_bias, 64));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc04, v_acc04, v_tmp0);
    str(q_acc04, ptr(reg_c0, 64));
    ldr(q_tmp0, ptr(reg_c0, 80));
    ldr(q_tmp1, ptr(reg_bias, 80));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc05, v_acc05, v_tmp0);
    str(q_acc05, ptr(reg_c0, 80));
    ldr(q_tmp0, ptr(reg_c0, 96));
    ldr(q_tmp1, ptr(reg_bias, 96));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc06, v_acc06, v_tmp0);
    str(q_acc06, ptr(reg_c0, 96));
    ldr(q_tmp0, ptr(reg_c0, 112));
    ldr(q_tmp1, ptr(reg_bias, 112));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc07, v_acc07, v_tmp0);
    str(q_acc07, ptr(reg_c0, 112));
    ...
    add(v_acc10, v_acc10, v_tmp0);
    str(q_acc10, ptr(reg_c1));
    ldr(q_tmp0, ptr(reg_c1, 16));
    ldr(q_tmp1, ptr(reg_bias, 16));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc11, v_acc11, v_tmp0);
    str(q_acc11, ptr(reg_c1, 16));
    ldr(q_tmp0, ptr(reg_c1, 32));
    ldr(q_tmp1, ptr(reg_bias, 32));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc12, v_acc12, v_tmp0);
    str(q_acc12, ptr(reg_c1, 32));
    ldr(q_tmp0, ptr(reg_c1, 48));
    ldr(q_tmp1, ptr(reg_bias, 48));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc13, v_acc13, v_tmp0);
    str(q_acc13, ptr(reg_c1, 48));
    ldr(q_tmp0, ptr(reg_c1, 64));
    ldr(q_tmp1, ptr(reg_bias, 64));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc14, v_acc14, v_tmp0);
    str(q_acc14, ptr(reg_c1, 64));
    ldr(q_tmp0, ptr(reg_c1, 80));
    ldr(q_tmp1, ptr(reg_bias, 80));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc15, v_acc15, v_tmp0);
    str(q_acc15, ptr(reg_c1, 80));
    ldr(q_tmp0, ptr(reg_c1, 96));
    ldr(q_tmp1, ptr(reg_bias, 96));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc16, v_acc16, v_tmp0);
    str(q_acc16, ptr(reg_c1, 96));
    ldr(q_tmp0, ptr(reg_c1, 112));
    ldr(q_tmp1, ptr(reg_bias, 112));
    add(v_tmp0, v_tmp0, v_tmp1);
    add(v_acc17, v_acc17, v_tmp0);
    str(q_acc17, ptr(reg_c1, 112));
...
void jit_int8_brgemm_kernel_4x16_udot_packed_strided_interleaved4::create_ker() {
    jit_generator::create_kernel();
    ker_ = ov::intel_cpu::jit_kernel_cast<ker_t>(jit_ker());
}

void jit_int8_brgemm_kernel_4x16_udot_packed_strided_interleaved4::generate() {
    preamble();
    ...
    const XReg reg_kb4 = x24;
    const XReg reg_kb_tail = x25;

    const WReg reg_mask = w23;
    ...
    const VReg4S v_tmp0 = VReg4S(9);
    const VReg4S v_tmp1 = VReg4S(10);
    const QReg q_acc00(v_acc00.getIdx());
    const QReg q_acc01(v_acc01.getIdx());
    const QReg q_acc02(v_acc02.getIdx());
    const QReg q_acc03(v_acc03.getIdx());
    ...
    for (int unroll = 0; unroll < 4; ++unroll) {
        ...
        eor(v_src0b, v_src0b, v_mask);
        eor(v_src1b, v_src1b, v_mask);
        eor(v_src2b, v_src2b, v_mask);
        eor(v_src3b, v_src3b, v_mask);
...
void jit_int8_brgemm_kernel_4x16_udot_packed_lhs_strided_interleaved4::create_ker() {
    jit_generator::create_kernel();
    ker_ = ov::intel_cpu::jit_kernel_cast<ker_t>(jit_ker());
}

void jit_int8_brgemm_kernel_4x16_udot_packed_lhs_strided_interleaved4::generate() {
    preamble();
    ...
    for (int unroll = 0; unroll < 4; ++unroll) {
        ...
        ld1(VReg16BList(v_w0, v_w1), post_ptr(reg_wei, 32));
        ld1(VReg16BList(v_w2, v_w3), post_ptr(reg_wei, 32));
        udot(v_acc00, v_src0b, v_w0);
        udot(v_acc01, v_src0b, v_w1);
        udot(v_acc02, v_src0b, v_w2);
        udot(v_acc03, v_src0b, v_w3);
        udot(v_acc10, v_src1b, v_w0);
        udot(v_acc11, v_src1b, v_w1);
        udot(v_acc12, v_src1b, v_w2);
        udot(v_acc13, v_src1b, v_w3);
        udot(v_acc20, v_src2b, v_w0);
        udot(v_acc21, v_src2b, v_w1);
        udot(v_acc22, v_src2b, v_w2);
        udot(v_acc23, v_src2b, v_w3);
        udot(v_acc30, v_src3b, v_w0);
        udot(v_acc31, v_src3b, v_w1);
        udot(v_acc32, v_src3b, v_w2);
        udot(v_acc33, v_src3b, v_w3);
    }
    subs(reg_kb4, reg_kb4, 1);
    b(NE, loop4);
