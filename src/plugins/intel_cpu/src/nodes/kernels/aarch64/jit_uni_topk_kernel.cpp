        ...
        if (!jcp->topk_innermost) {
                ...
                if (dnnl::impl::cpu::aarch64::mayiuse(dnnl::impl::cpu::aarch64::sve_128)) {
                    ...
                    auto process_vec = [&](svbool_t pg, size_t lane_base) {
                        ...
                        for (size_t i = 1; i < axis_dim; ++i) {
                            const auto* ptr = reinterpret_cast<const float*>(src + i * args->sort_stride + lane_base);
                            svfloat32_t v = svld1(pg, ptr);
            ...
            if constexpr (std::is_same_v<T, float>) {
                ...
                for (; lane + V <= work_amount; lane += V) {
                    ...
                    for (size_t i = 1; i < axis_dim; ++i) {
                        const auto* ptr = src + i * args->sort_stride + lane;
...
#    if defined(HAVE_SVE)
                if (dnnl::impl::cpu::aarch64::mayiuse(dnnl::impl::cpu::aarch64::sve_128)) {
                    ...
                    for (; lane + vlen_h <= work_amount; lane += vlen_h) {
                        ...
                        svfloat16_t best_low_f16 = svzip1_f16(best_f16, zero);
                        svfloat16_t best_high_f16 = svzip2_f16(best_f16, zero);
                        svfloat32_t best_low = svcvt_f32_f16_x(pg_f32, best_low_f16);
                        svfloat32_t best_high = svcvt_f32_f16_x(pg_f32, best_high_f16);
                        svint32_t best_idx_low = svdup_s32(0);
                        svint32_t best_idx_high = svdup_s32(0);

                        for (size_t i = 1; i < axis_dim; ++i) {
                            const auto* ptr =
                                reinterpret_cast<const float16_t*>(src + i * args->sort_stride + lane);
                            svfloat16_t v_f16 = svld1(pg_f16, ptr);
                            svfloat16_t v_low_f16 = svzip1_f16(v_f16, zero);
                            svfloat16_t v_high_f16 = svzip2_f16(v_f16, zero);
                            svfloat32_t v_low = svcvt_f32_f16_x(pg_f32, v_low_f16);
                            svfloat32_t v_high = svcvt_f32_f16_x(pg_f32, v_high_f16);
                            ...
                            const svint32_t idx_vec = svdup_s32(static_cast<int32_t>(i));
                        ...
                        auto* out_idx = dst_idx + lane;
                        svfloat16_t out_low = svcvt_f16_f32_x(pg_f32, best_low);
                        svfloat16_t out_high = svcvt_f16_f32_x(pg_f32, best_high);
                        svfloat16_t out_f16 = svuzp1(out_low, out_high);
...
#    endif  // HAVE_SVE
                ...
                for (; lane + V <= work_amount; lane += V) {
                    const auto* base = src + lane;
                    float16x8_t best = vld1q_f16(reinterpret_cast<const float16_t*>(base));
                    int32x4_t best_idx0 = vdupq_n_s32(0);
                    int32x4_t best_idx1 = vdupq_n_s32(0);
                    for (size_t i = 1; i < axis_dim; ++i) {
                        const auto* ptr = src + i * args->sort_stride + lane;
                        float16x8_t v = vld1q_f16(reinterpret_cast<const float16_t*>(ptr));
                        const uint16x8_t mask =
                            mode_max ? vcgtq_f16(v, best) : vcltq_f16(v, best);
                        best = vbslq_f16(mask, v, best);

                        const int32x4_t idx_vec = vdupq_n_s32(static_cast<int32_t>(i));
                        uint32x4_t mask0 = vmovl_u16(vget_low_u16(mask));
                        uint32x4_t mask1 = vmovl_u16(vget_high_u16(mask));
                        mask0 = vorrq_u32(mask0, vshlq_n_u32(mask0, 16));
                        mask1 = vorrq_u32(mask1, vshlq_n_u32(mask1, 16));
                        best_idx0 = vbslq_s32(mask0, idx_vec, best_idx0);
                        best_idx1 = vbslq_s32(mask1, idx_vec, best_idx1);
                    }
                    auto* out = dst + lane;
                    auto* out_idx = dst_idx + lane;
                    vst1q_f16(reinterpret_cast<float16_t*>(out), best);
                ...
                if (dnnl::impl::cpu::aarch64::mayiuse(dnnl::impl::cpu::aarch64::sve_128)) {
                    ...
                    auto process_vec = [&](size_t lane_base, svbool_t pg_b8) {
                        ...
                        for (size_t i = 1; i < axis_dim; ++i) {
                            for (size_t s = 0; s < 4; ++s) {
                                const size_t lane_s = lane_base + s * vlen_w;
                                const auto* base = src + i * args->sort_stride + lane_s;
                                if constexpr (std::is_same_v<T, int8_t>) {
                                    svint32_t v = svld1sb_s32(pg_w[s], reinterpret_cast<const int8_t*>(base));
                                    const svbool_t mask =
                                        mode_max ? svcmpgt_s32(pg_w[s], v, v_s32[s])
                                                 : svcmplt_s32(pg_w[s], v, v_s32[s]);
                                    v_s32[s] = svsel(mask, v, v_s32[s]);
                                    const svint32_t idx_vec = svdup_s32(static_cast<int32_t>(i));
                                    best_idx[s] = svsel(mask, idx_vec, best_idx[s]);
                                } else {
                                    svuint32_t v = svld1ub_u32(pg_w[s], reinterpret_cast<const uint8_t*>(base));
                                    const svbool_t mask =
                ...
                for (; lane + V <= work_amount; lane += V) {
                    ...
                    for (size_t i = 1; i < axis_dim; ++i) {
                        ...
                        uint32x4_t m32_0 = vmovl_u16(vget_low_u16(m16_low));
                        uint32x4_t m32_1 = vmovl_u16(vget_high_u16(m16_low));
                        uint32x4_t m32_2 = vmovl_u16(vget_low_u16(m16_high));
                        uint32x4_t m32_3 = vmovl_u16(vget_high_u16(m16_high));
        ...
        if constexpr (std::is_same_v<T, float>) {
                ...
                if (top_k <= kVecMax) {
                    ...
                    auto process_vec = [&](svbool_t pg, size_t lane_base) {
                        ...
                        for (size_t i = top_k; i < axis_dim; ++i) {
                            const auto* base =
                                reinterpret_cast<const float*>(src + i * args->sort_stride + lane_base);
                            svfloat32_t v = svld1(pg, base);
                            svint32_t v_idx = svdup_s32(static_cast<int32_t>(i));
                            for (size_t k = 0; k < top_k; ++k) {
                                const svbool_t mask =
            ...
            if (top_k <= kVecMax && work_amount >= V) {
                ...
                for (; lane + V <= work_amount; lane += V) {
                    ...
                    for (size_t i = top_k; i < axis_dim; ++i) {
                        float32x4_t v = vld1q_f32(reinterpret_cast<const float*>(src + i * args->sort_stride + lane));
                        int32x4_t v_idx = vdupq_n_s32(static_cast<int32_t>(i));
                        for (size_t k = 0; k < top_k; ++k) {
                            const uint32x4_t mask =
                ...
                if (top_k <= kVecMax) {
                    ...
                    for (; lane + vlen_h <= work_amount; lane += vlen_h) {
                        ...
                        for (size_t k = 0; k < top_k; ++k) {
                            ...
                            svfloat16_t v_low_f16 = svzip1_f16(v_f16, zero);
                            svfloat16_t v_high_f16 = svzip2_f16(v_f16, zero);
                            vals_low[k] = svcvt_f32_f16_x(pg_f32, v_low_f16);
                            vals_high[k] = svcvt_f32_f16_x(pg_f32, v_high_f16);
                        ...
                        for (size_t k = 0; k < top_k; ++k) {
                            ...
                            auto* out_idx = dst_idx + k * args->sort_stride + lane;
                            svfloat16_t out_low = svcvt_f16_f32_x(pg_f32, vals_low[k]);
                            svfloat16_t out_high = svcvt_f16_f32_x(pg_f32, vals_high[k]);
                            svfloat16_t out_f16 = svuzp1(out_low, out_high);
                            svst1(pg_f16, out, out_f16);
...
#    endif  // HAVE_SVE
            ...
            if (top_k <= kVecMax && work_amount >= V) {
                for (; lane + V <= work_amount; lane += V) {
                    float16x8_t vals[kVecMax];
                    int32x4_t idx0[kVecMax];
                    int32x4_t idx1[kVecMax];
                    for (size_t k = 0; k < top_k; ++k) {
                        vals[k] = vld1q_f16(reinterpret_cast<const float16_t*>(src + k * args->sort_stride + lane));
                    ...
                    for (size_t i = 0; i + 1 < top_k; ++i) {
                        for (size_t j = top_k - 1; j > i; --j) {
                            const uint16x8_t mask =
                                mode_max ? vcgtq_f16(vals[j], vals[j - 1]) : vcltq_f16(vals[j], vals[j - 1]);
                            const float16x8_t v_hi = vbslq_f16(mask, vals[j], vals[j - 1]);
                            const float16x8_t v_lo = vbslq_f16(mask, vals[j - 1], vals[j]);
                            vals[j - 1] = v_hi;
                            vals[j] = v_lo;
                    ...
                    for (size_t i = top_k; i < axis_dim; ++i) {
                        float16x8_t v =
                            vld1q_f16(reinterpret_cast<const float16_t*>(src + i * args->sort_stride + lane));
                        int32x4_t v_idx0 = vdupq_n_s32(static_cast<int32_t>(i));
                        int32x4_t v_idx1 = vdupq_n_s32(static_cast<int32_t>(i));
                        for (size_t k = 0; k < top_k; ++k) {
                            const uint16x8_t mask = mode_max ? vcgtq_f16(v, vals[k]) : vcltq_f16(v, vals[k]);
                            const float16x8_t v_new = vbslq_f16(mask, vals[k], v);
                            const float16x8_t slot_new = vbslq_f16(mask, v, vals[k]);
                            v = v_new;
                            vals[k] = slot_new;
                    ...
                    for (size_t k = 0; k < top_k; ++k) {
                        auto* out = dst + k * args->sort_stride + lane;
                        auto* out_idx = dst_idx + k * args->sort_stride + lane;
                        vst1q_f16(reinterpret_cast<float16_t*>(out), vals[k]);
            ...
            if (top_k <= kVecMax && work_amount >= V) {
                ...
                for (; lane + V <= work_amount; lane += V) {
                    ...
                    for (size_t i = top_k; i < axis_dim; ++i) {
                        uint8x16_t v;
                        const auto* base = src + i * args->sort_stride + lane;
                        if constexpr (std::is_same_v<T, int8_t>) {
                        ...
                        int32x4_t v_idx0 = vdupq_n_s32(static_cast<int32_t>(i));
                        int32x4_t v_idx1 = vdupq_n_s32(static_cast<int32_t>(i));
                        int32x4_t v_idx2 = vdupq_n_s32(static_cast<int32_t>(i));
                        int32x4_t v_idx3 = vdupq_n_s32(static_cast<int32_t>(i));
