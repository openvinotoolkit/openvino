...
private:
    ...
    //inline Xbyak_riscv::VReg aux_vec(const int idx) {
    //    OPENVINO_ASSERT(idx >= 0 && (idx + 8 < vec_count), "aux vector reg " + std::to_string(idx) + " is not supported");
    //    return Xbyak_riscv::VReg(idx + 8);
    //}
    ...
    void compute_eltwise_op(const Xbyak_riscv::LMUL lmul);
    void apply_post_ops(const Xbyak_riscv::LMUL lmul);
    void emit_data() const;

    int lmul2int(const Xbyak_riscv::LMUL lmul) const;
