...
ExperimentalDetectronPriorGridGenerator::ExperimentalDetectronPriorGridGenerator(const std::shared_ptr<ov::Node>& op,
    ...
    grid_w_ = static_cast<int>(attr.w);
    grid_h_ = static_cast<int>(attr.h);
...
void ExperimentalDetectronPriorGridGenerator::execute([[maybe_unused]] const dnnl::stream& strm) {
    const auto num_priors_ = static_cast<int>(getParentEdgeAt(INPUT_PRIORS)->getMemory().getStaticDims()[0]);
    assert(getParentEdgeAt(INPUT_PRIORS)->getMemory().getStaticDims()[1] == 4);

    // Execute
    const auto layer_width = grid_w_ ? grid_w_ : static_cast<int>(getParentEdgeAt(INPUT_FEATUREMAP)->getMemory().getStaticDims()[3]);
    const auto layer_height = grid_h_ ? grid_h_ : static_cast<int>(getParentEdgeAt(INPUT_FEATUREMAP)->getMemory().getStaticDims()[2]);
    const float step_w =
        (stride_w_ != 0.0F)
            ? stride_w_
            : static_cast<float>(getParentEdgeAt(INPUT_IMAGE)->getMemory().getStaticDims()[3]) / static_cast<float>(layer_width);
    const float step_h =
        (stride_h_ != 0.0F)
            ? stride_h_
            : static_cast<float>(getParentEdgeAt(INPUT_IMAGE)->getMemory().getStaticDims()[2]) / static_cast<float>(layer_height);
