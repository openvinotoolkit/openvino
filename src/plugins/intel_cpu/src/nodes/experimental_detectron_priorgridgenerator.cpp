...
void ExperimentalDetectronPriorGridGenerator::execute(const dnnl::stream& strm) {
    ...
    const int layer_width =
        (grid_w_ != 0) ? grid_w_ : getParentEdgeAt(INPUT_FEATUREMAP)->getMemory().getStaticDims()[3];
    const int layer_height =
        (grid_h_ != 0) ? grid_h_ : getParentEdgeAt(INPUT_FEATUREMAP)->getMemory().getStaticDims()[2];
    const float step_w =
        (stride_w_ != 0.0f)
            ? stride_w_
