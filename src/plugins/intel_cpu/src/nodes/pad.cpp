...
void Pad::PadExecutor::paramsInitialization(const PadAttrs& attrs,
    ...
    if (blockSize > 1) {
        params.attrs.padsBegin[1] /= static_cast<VectorIdxs::value_type>(blockSize);
        params.attrs.padsEnd[1] /= static_cast<VectorIdxs::value_type>(blockSize);
    ...
    for (int i = static_cast<int>(params.attrs.padsBegin.size()) - 1; i >= 0; --i) {
        if (params.attrs.padsBegin[i] != 0 || params.attrs.padsEnd[i] != 0) {
