...
EmbeddingBagSum::EmbeddingBagSum(const std::shared_ptr<ov::Node>& op,
...
template <typename T>
void EmbeddingBagSum::processData(const T* srcData,
                                  const T* weightsData,
...
void EmbeddingBagSum::execute(const uint8_t* srcData,
