...
general background for anyone working on CPU nodes, independent of the op-specific
...
2. **String ↔ `Type` mapping** in [cpu_types.cpp](../cpu_types.cpp): add
   ...
   `NameFromType()` switch. The string key must match the core op's
   `get_type_name()` (the `OPENVINO_OP("...")` macro) exactly.
3. **Factory entry** in [nodes_factory.cpp](../nodes_factory.cpp): include the node
   header and add `INTEL_CPU_NODE(OpName, Type::OpName);`. For nodes that do not
   build on every architecture (e.g. JIT-only), guard both the include and the
   entry with the condition matching the node's actual support — both
   `#if defined(OPENVINO_ARCH_X86_64)` and
   `#if defined(OPENVINO_ARCH_X86) || defined(OPENVINO_ARCH_X86_64)` are in use.

## Shape inference factories

The `Node` constructor takes a shape-inference factory:

- **`NgraphShapeInferFactory(op)`** — the default. Delegates to a per-opset static
  shape-inference implementation registered in `shape_inference.cpp`, falling back
  to the op's generic `validate_and_infer_types()`-based path when no specialized
  implementation is registered. Use it whenever the core/generic shape inference is
  sufficient.
