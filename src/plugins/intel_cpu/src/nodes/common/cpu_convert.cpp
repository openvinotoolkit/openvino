...
const std::tuple<U, U>& Range<T, U>::fit(const ov::element::Type& prec) {
    if (prec.is_real()) {
        ...
        using ltype = std::conditional_t<std::is_floating_point_v<U>, double, int64_t>;
        using utype = std::conditional_t<std::is_floating_point_v<U>, double, uint64_t>;
        std::get<0>(_range) =
    ...
    void operator()(ConvertContext& ctx) {
        ...
        if (ov::intel_cpu::any_of_v<src_t, ov::float8_e4m3, ov::float8_e5m2> ||
            ...
            parallel_for(ctx.size, [&](size_t i) {
                using direct_cast_input_t = std::conditional_t<
                    ov::intel_cpu::
        ...
        if (std::is_integral_v<src_t> || ctx.interimPrc.is_real() || std::is_integral_v<dst_t>) {
            parallel_for(ctx.size, [&, lbound = lbound, ubound = ubound](size_t i) {
                using cast_input_t = std::
                    conditional_t<ov::intel_cpu::any_of_v<dst_t, ov::float16, ov::intel_cpu::bfloat16_t>, float, src_t>;
                dst[i] = static_cast<dst_t>(static_cast<cast_input_t>(std::max(std::min(src[i], ubound), lbound)));
            });
        } else {
            parallel_for(ctx.size, [&, lbound = lbound, ubound = ubound](size_t i) {
                using trunc_result_t =
                    std::conditional_t<ov::intel_cpu::any_of_v<dst_t, ov::float16, ov::intel_cpu::bfloat16_t>,
                                       float,
                                       double>;
                dst[i] = static_cast<dst_t>(static_cast<trunc_result_t>(
                    std::trunc(static_cast<double>(std::max(std::min(src[i], ubound), lbound)))));
            });
        }
    ...
    void operator()(ConvertFromBinContext& ctx) {
        ...
        parallel_for(nBytes, [&](size_t byteIndex) {
            ...
            for (size_t bitIndex = 0; bitIndex < currentBitNum; ++bitIndex) {
                using bin_cast_input_t = std::conditional_t<
                    ov::intel_cpu::
                        any_of_v<dst_t, ov::float8_e4m3, ov::float8_e5m2, ov::float16, ov::intel_cpu::bfloat16_t>,
                    float,
                    uint8_t>;
