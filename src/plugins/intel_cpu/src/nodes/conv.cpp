...
void Convolution::getSupportedDescriptors() {
    ...
    if (canBeExecutedInInt8()) {
        DEBUG_LOG("Creating I8 descriptor");
    ...
    for (int i = 0; i < fusedWith.size(); ++i) {
        ...
        if (auto* eltwiseNode = dynamic_cast<Eltwise*>(node.get())) {
            if (eltwiseNode->isSpecialConvolutionAddFusing()) {
                ...
                DEBUG_LOG("Append ", node->getName(), " as sum post op");
                ...
                if (useLegacyPostOps) {
                    ...
                    //     DEBUG_LOG("Append ", node->getName(), " as original post op without binary");
                    //     continue;
                    // }
                    DEBUG_LOG("Append ", node->getName(), " as legacy post op");
                    eltwiseNode->appendPostOps(ops, dims, args);
                } else {
                    DEBUG_LOG("Append ", node->getName(), " as original post op with binary");
        ...
        if (convolutionNode) {
            if (initWeights) {
                ...
                DEBUG_LOG("Append ", node->getName(), " as DW convolution");
...
createDescriptorInternal(const dnnl::engine& engine,
    ...
    std::shared_ptr<dnnl::convolution_forward::primitive_desc> conv_desc;

    try {
        if (withBiases) {
            conv_desc = std::make_shared<dnnl::convolution_forward::primitive_desc>(
                engine,
...
void Convolution::SetPostOpsAndZeroPoints(std::vector<dnnl::primitive_attr> &attrs) {
    ...
    DEBUG_LOG("setPostOps, attr 0, useLegacyPostOps=true");
    ...
    if (inputZeroPointType == zpType::PerChannel) {
        DEBUG_LOG("Per channel zero point can only supported on attr[0].Avoid extra useless attribute.");
        return;
    }
    if (!shouldTryBrgconv) {
    ...
    if (inputZeroPointType == zpType::PerTensor && dnnl::impl::cpu::x64::mayiuse(dnnl::impl::cpu::x64::avx512_core_amx)) {
        ...
        DEBUG_LOG("setPostOps, attr 1, useLegacyPostOps=true");
        setPostOps(attrs[1], outputShape, true);
    } else {
        DEBUG_LOG("setPostOps, attr 1, useLegacyPostOps=false");
...
void Convolution::initDescriptor(const NodeConfig& config) {
    ...
    int n = attrs.size() == 1 ? 0 :
        descIdx[selectedPrimitiveDescriptorIndex] % 2 == 0 ? 0 : 1;
...
//         primitive_desc_iterator itpd = *descs[i];
//         if (containJitImpl && isPossibleToSkipInitConfig(itpd))
//             continue;
...
//     // std::cout << "InitDescriptor: " << "Choosing desc: " << itpd.get()->info() << "\n";
...
bool Convolution::isPossibleToSkipInitConfig(dnnl::primitive_desc &desc) const {
...
void Convolution::prepareParams() {
    ...
    auto builder = [&engine](const ConvKey& key) -> executorPtr {
        auto createDnnlConvDesc = [](const dnnl::engine engine,
                                     ...
                                     const std::vector<ptrdiff_t>& paddingR,
                                     dnnl::algorithm alg,
                                     const dnnl::primitive_attr attr) -> std::shared_ptr<dnnl::primitive_desc> {
        ...
        std::shared_ptr<dnnl::primitive_desc> desc = createDnnlConvDesc(engine,
        ...
        if (!execPtr) {
            ...
            auto reordItpd = *reorderConvDesc;
    ...
    if (execPtr) {
        ...
        // auto& prim = primArgs[DNNL_ARG_ATTR_SCALES | DNNL_ARG_DST];
