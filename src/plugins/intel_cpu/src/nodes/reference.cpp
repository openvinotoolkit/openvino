...
Reference::Reference(const std::shared_ptr<ngraph::Node>& op, RuntimeEnv::Ptr rtEnv,
                                         const std::string& errorMessage) :
        Node(op, rtEnv, NgraphShapeInferFactory(op, FULL_PORT_MASK)), ngraphOp(op), additionalErrorMessage(errorMessage) {
    if (!op->has_evaluate()) {
