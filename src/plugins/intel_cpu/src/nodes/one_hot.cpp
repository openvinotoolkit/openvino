...
public:
    ...
    Result infer(const std::vector<std::reference_wrapper<const VectorDims>>& input_shapes,
        ...
        int64_t depth;
        auto memPtr = data_dependency.at(1);
        if (memPtr->getDesc().getPrecision() == Precision::I64) {
            depth = reinterpret_cast<int64_t *>(memPtr->getData())[0];
        } else {
            depth = reinterpret_cast<int32_t *>(memPtr->getData())[0];
        }
...
public:
    ...
    ShapeInferPtr makeShapeInfer() const override {
        auto oneHot = ov::as_type_ptr<const op::v1::OneHot>(m_op);
        if (!oneHot) {
...
bool OneHot::isSupportedOperation(const std::shared_ptr<const ov::Node>& op, std::string& errorMessage) noexcept {
    try {
        if (op->get_type_info() != op::v1::OneHot::get_type_info_static()) {
            errorMessage = "Only opset1 OneHot operation is supported";
            return false;
        }
        if (op->get_input_node_shared_ptr(ON_VALUE_ID)->get_type_info() != op::v0::Constant::get_type_info_static()) {
            errorMessage = "Only const 'on_value' input is supported";
            return false;
        }
        if (op->get_input_node_shared_ptr(OFF_VALUEAXES_ID)->get_type_info() != op::v0::Constant::get_type_info_static()) {
...
OneHot::OneHot(const std::shared_ptr<ov::Node>& op, const GraphContext::CPtr& context)
    ...
    const auto oneHot = ov::as_type_ptr<const op::v1::OneHot>(op);
    const auto depthNode = ov::as_type_ptr<const op::v0::Constant>(oneHot->get_input_node_shared_ptr(DEPTH_ID));
    if (depthNode) {
        depth = depthNode->cast_vector<Dim>()[0];
    }
...
void OneHot::one_hot(size_t prefix_size, size_t suffix_size) {
    ...
    if (inputPrecision == Precision::I64) { // TODO template?
