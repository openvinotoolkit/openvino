...
void EltwiseRefBaseExecutor<T>::offset_out_calc(VectorDims& offset, const VectorDims& dims) {
    size_t k = 1;
    for (auto i = offset.size() - 1; i >= 0; i--) {
        offset[i] = k;
        k *= dims[i];
    }
}
...
void EltwiseRefExecutor<T, Enable>::exec(const jit_eltwise_call_args_ptrs& args_ptrs,
    ...
    if (this->m_opData.algo == Algorithm::EltwisePowerStatic) {
        ...
        if (this->m_opData.alpha == 2) {
            cpu_parallel->parallel_for(this->m_fullWorkAmount, [&](size_t i) {
                const float v = static_cast<float>(this->m_opData.beta * static_cast<double>(src_ptr_f[i]) + this->m_opData.gamma);
                dst_ptr_f[i] = static_cast<T>(v * v);
            });
        } else {
            cpu_parallel->parallel_for(this->m_fullWorkAmount, [&](size_t i) {
                dst_ptr_f[i] = powf(static_cast<float>(this->m_opData.beta * src_ptr_f[i] + this->m_opData.gamma), static_cast<float>(this->m_opData.alpha));
            });
        }
        return;
    }
