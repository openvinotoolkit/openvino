...
public:
    ...
    void exec(const jit_eltwise_call_args_ptrs& args_ptrs, const VectorDims& dims_out) override {
        ...
        parallel_nt(0, [&](const int ithr, const int nthr) {
            ...
            for (size_t iwork = start; iwork < end; ++iwork) {
                ...
                // Execute the operation based on algorithm
                for (size_t iwork = start; iwork < end; ++iwork) {
                    std::vector<T> src_f(this->m_inputNum);
                    T* dst_ptr_f = nullptr;
                    this->init_ptr(args_ptrs, dims_out, counters, iwork, src_f, dst_ptr_f);
    ...
    static bool isSupportedConfiguration([[maybe_unused]] const EltwiseConfig& config) {
        return true;
    }
...
template <typename T>
static EltwiseExecutorPtr createRefExecutor(const EltwiseRefKey& key) {
    if constexpr (std::is_same_v<T, float> || std::is_same_v<T, dnnl::impl::float16_t>) {
        return std::make_shared<EltwiseRefExecutor<T>>(key);
    } else {
        return std::make_shared<BitwiseRefExecutor<T>>(key);
    }
}
...
static EltwiseExecutorPtr createRefExecutorByPrecision(const EltwiseRefKey& key) {
    switch (key.outPrc) {
    case ov::element::f16:
        return createRefExecutor<dnnl::impl::float16_t>(key);
    case ov::element::i8:
        return createRefExecutor<int8_t>(key);
    case ov::element::u8:
        return createRefExecutor<uint8_t>(key);
    case ov::element::i16:
        return createRefExecutor<int16_t>(key);
    case ov::element::u16:
        return createRefExecutor<uint16_t>(key);
    case ov::element::i32:
        return createRefExecutor<int32_t>(key);
    default:
        // Use float reference executor for any other precision
        return createRefExecutor<float>(key);
    }
}

inline EltwiseExecutorPtr create(const std::vector<VectorDims>& inDims,
