    ...
    parallel_for3d(B, OH, OW, [&](size_t b, size_t h, size_t w) {
        ...
        for (int y = iy - 1, i = 0; y <= iy + 2; y++, i++) {
            ...
            for (int x = ix - 1, j = 0; x <= ix + 2; x++, j++) {
                ...
                kernelIndex[i * baseInterpolateAttrs.CUBIC_GRID_LEN + j] = xInRange;
            }
        }
        auto arg = jit_interpolate_call_args();
        arg.dst = out_ptr_nhw;
        ...
        arg.weight_ptr[0] = static_cast<float*>(&xFactor[w * baseInterpolateAttrs.CUBIC_GRID_LEN]);
        arg.weight_ptr[1] = static_cast<float*>(&yFactor[h * baseInterpolateAttrs.CUBIC_GRID_LEN]);
