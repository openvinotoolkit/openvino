...
jit_permute_config_params TransposeExecutor::prepareParams(const PermuteParams& params) {
    ...
    for (auto i = params.src_block_dims.size() - 2; i >= 0; i--) {
        src_block_strides[i] = src_block_strides[i + 1] * params.src_block_dims[i + 1];
    }
    for (auto i = params.dst_block_dims.size() - 2; i >= 0; i--) {
    ...
    for (auto i = tmp_order.size() - 1; i >= 0; i--) {
    ...
    auto batch_ord = std::distance(params.order.begin(), std::find(params.order.begin(), params.order.end(), 0));
    int batch_count = 0;
    size_t batch_pos = 0;
    for (auto i = 0; i < new_dst_block_order.size(); i++) {
    ...
    jcp.n = std::min(n, n2);
    jcp.ndims = static_cast<uint32_t>(sorted_order.size());
