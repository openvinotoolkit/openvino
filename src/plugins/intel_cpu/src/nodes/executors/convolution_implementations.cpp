...
const std::vector<ExecutorImplementation<ConvAttrs>>& getImplementations() {
    static const std::vector<ExecutorImplementation<ConvAttrs>> convolutionImplementations {
        ...
        // OV_CPU_INSTANCE_DNNL_X64(
        //     "convolution_fallback_graph", ExecutorType::Dnnl, OperationType::Convolution,  ShapeTolerance::Agnostic,
        //     // supports
        //     SupportsAnyConfig<ConvAttrs>{},
        //     RequiresFallbackDefault<ConvAttrs>{{LayoutType::nspc, LayoutType::ncsp, LayoutType::nspc, LayoutType::nspc}},
        //     AcceptsAnyShape{},
        ...
        //         auto memoryDescs = GraphEmitter<ConvAttrs>::memoryDescsFromMemory(memory);
        //         auto postOpsWithoutSum = postOps;
        //         postOpsWithoutSum.erase(std::remove_if(postOpsWithoutSum.begin(), postOpsWithoutSum.end(),
        //                                                [](const std::shared_ptr<PostOp>& po) {
        //                                                    return static_cast<bool>(std::dynamic_pointer_cast<SumPostOp>(po));
        //                                                }), postOpsWithoutSum.end());
        //         auto originalConfig = executor::Config<ConvAttrs>{memoryDescs, attrs, postOps};
        //         auto fallbackConfig = executor::Config<ConvAttrs>{memoryDescs, attrs, postOpsWithoutSum};
        //         return GraphEmitter<ConvAttrs>::fallback(originalConfig, fallbackConfig, memory, "convolution_fallback_graph", context->getGraphContext());
        //     }
        //     )
    };
