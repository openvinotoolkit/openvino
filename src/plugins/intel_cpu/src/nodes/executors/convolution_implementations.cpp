...
static const TypeMapping dnnlConvTypeMapping {
    ...
    {{_u8 | _i8, _i8, _any, _quant | _hw_float | _i32},        pt(bypass(), bypass(), use<3>(),  bypass())},
...
static const LayoutConfig dnnlNcsp8cLayoutConfig{LayoutType::nCsp8c, LayoutType::nCsp8c, LayoutType::nCsp8c, LayoutType::nCsp8c};
static const LayoutConfig dnnlNcsp16cLayoutConfig{LayoutType::nCsp16c, LayoutType::nCsp16c, LayoutType::nCsp16c, LayoutType::nCsp16c};
...
struct AcceptsAnyShape {
    bool operator()(const MemoryArgs&) const {
        return true;
    }
};
...
struct CreateDefault {
    ExecutorPtr operator()(const ConvAttrs& attrs,
        ...
        return std::make_shared<DnnlFCExecutor<DnnlConvolutionPrimitive, ConvAttrs, DnnlShapeAgnosticData>>
            (attrs, postOps, memory, context, false);
    }
};

template<typename Attrs>
struct RequiresFallbackDefault {
    ov::optional<executor::Config<Attrs>> operator()(const executor::Config<Attrs>& config) const {
...
template<typename Attrs>
bool MatchesMemoryFormatFilter(const executor::Config<Attrs>& config, const LayoutConfig& layoutConfig, const MemoryFormatFilter& filter) {
    ...
    for (size_t i = 0; i < filter.input.size(); i++) {
        ...
        const auto dnnlDesc = DnnlBlockedMemoryDesc(config.descs.at(notation[i])->getShape(), dnnl::memory::data_type::f32, filter.input[i]);
    ...
    const auto desc = DnnlBlockedMemoryDesc(config.descs.at(ARG_DST)->getShape(), dnnl::memory::data_type::f32, filter.output.front());
