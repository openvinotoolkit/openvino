...
static primitive_desc createPrimitiveDesc(const dnnl::memory::desc& inputDesc,
    ...
    return withDnnlDescriptorConcurrency([&]() -> primitive_desc {
        auto createAnyDescriptor = [&]() -> dnnl::convolution_forward::primitive_desc {
        ...
        auto createSelectedDescriptor = [&]() -> dnnl::convolution_forward::primitive_desc {
            if (fcSemantic) {
                return createInnerProductDescriptor(inputDesc,
                                                    weightDesc,
                                                    biasDesc,
                                                    outputDesc,
                                                    stride,
        ...
        auto prim_desc = createSelectedDescriptor();
        auto first_desc = prim_desc;
        if (defaultImplType == impl_desc_type::undef) {
            if (!prim_desc) {
                return createAnyDescriptor();
            }
            ...
            for (auto preferredImplType : implPriorities) {
                ...
                prim_desc = createSelectedDescriptor();
            }
