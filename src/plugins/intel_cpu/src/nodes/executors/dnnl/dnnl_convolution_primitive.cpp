...
static primitive_desc createPrimitiveDesc(const dnnl::engine& engine,
    ...
    auto acceptableImplType = [](const dnnl::primitive_desc& pd,
                                 const std::vector<impl_desc_type>& implPriorities) -> bool {
        constexpr auto slow_impl_types = impl_desc_type::gemm | impl_desc_type::ref;
        const auto current_impl_type = parse_impl_name(pd.impl_info_str());
        const auto priority_impl_type = implPriorities.empty() ? impl_desc_type::undef : implPriorities.front();
        const bool current_contains_slow_impl_types = current_impl_type & slow_impl_types;
        const bool priority_contains_slow_impl_types = priority_impl_type & slow_impl_types;

        if (current_contains_slow_impl_types && !priority_contains_slow_impl_types) {
            return false;
        }
    ...
    for (const auto preferredImplType : implPriorities) {
        // primitive descriptor must be copied, since it mutates in scope of the iteration
        auto a_desc = prim_desc;

        const bool found = DnnlExtensionUtils::find_implementation(a_desc, preferredImplType);

        if (found && acceptableImplType(a_desc, implPriorities)) {
            return std::move(a_desc);
        }
    }
...
static DnnlPrimitiveAttrs createPrimitiveAttrs(const ConvAttrs& attrs,
    ...
    // @todo avoid extra step of creating config
...
DnnlShapeAgnosticDataPtr DnnlConvolutionPrimitive::createShapeAgnosticData(const ConvAttrs& attrs,
    ...
    auto outputsDims =
        node::convolution_auto_pad_shape_infer(std::vector<VectorDims>{dummyInputDims, weiShape.getStaticDims()},
                                               strides,
                                               ovDilation,
                                               paddingL,
                                               paddingR,
    ...
    assert(outputsDims.size() == 1);  // only one output shape is expected
    const auto& dummyOutputDims = outputsDims.front();
...
void DnnlConvolutionPrimitive::execute(dnnl_primitive_args& primArgs) {
    for (const auto& [idx, reorder] : m_intermediateReorders.m_inputReorder) {
        if (auto primArg = primArgs.find(idx); primArg != primArgs.end()) {
            auto& [idx, memory] = *primArg;
            reorder.execute(m_stream, memory, m_intermediateReorders.m_inputMemory.at(idx));
            memory = m_intermediateReorders.m_inputMemory.at(idx);
        }
    }

    auto outputMem = primArgs.at(DNNL_ARG_DST);

    if (const auto& outputReorder = m_intermediateReorders.m_outputReorder.find(DNNL_ARG_DST);
        outputReorder != m_intermediateReorders.m_outputReorder.end()) {
        primArgs[DNNL_ARG_DST] = m_intermediateReorders.m_outputMemory.at(DNNL_ARG_DST);
    }
    ...
    if (const auto& outputReorder = m_intermediateReorders.m_outputReorder.find(DNNL_ARG_DST);
        outputReorder != m_intermediateReorders.m_outputReorder.end()) {
        outputReorder->second.execute(m_stream, primArgs[DNNL_ARG_DST], outputMem);
        primArgs[DNNL_ARG_DST] = outputMem;
    }
}
...
bool DnnlConvolutionPrimitive::isNspcAvailable(const ConvConfig& config) {
    ...
    if (!mayiuse(impl::cpu::x64::avx)) {
        // SSE41 nspc convolutions do not support ic and oc tails yet and the blocked implementation will be much better
        // than gemm
        if ((IC % 8) || (OC % 8)) {
            return false;
        }
