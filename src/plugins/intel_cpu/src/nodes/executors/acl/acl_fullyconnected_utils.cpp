    ...
    if (reorderWithoutConvert &&
        ...
        if (reorderWithoutConvert) {
            dnnl::stream loc_stream =
                dnnl::threadpool_interop::make_stream(output->getPrimitive().get_engine(), getThreadPool());
...
MemoryPtr acl_fc_executor::reorderData(const DnnlMemoryDescPtr& srcWeightDesc,
    ...
    if (directReorder) {
        dnnl::stream loc_stream = dnnl::threadpool_interop::make_stream(engine, context->getThreadPool());
