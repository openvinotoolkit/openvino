...
void mm_exec(shared_mem src, shared_mem wei, shared_mem bias, shared_mem scales, shared_mem subtracts, shared_mem dst, bool weightsNonTransposed) {
    ...
    std::vector<arm_compute::Tensor> tensorsA;
    std::vector<arm_compute::TensorInfo> tensorInfosA;
    tensorsA.resize(num_blocks_N);
    tensorInfosA.resize(num_blocks_N);
    std::vector<arm_compute::Tensor> tensorsB;
    std::vector<arm_compute::TensorInfo> tensorInfosB;
    tensorsB.resize(num_blocks_N);
    tensorInfosB.resize(num_blocks_N);
    std::vector<arm_compute::Tensor> tensorsC;
    std::vector<arm_compute::TensorInfo> tensorInfosC;
    tensorsC.resize(num_blocks_N);
    tensorInfosC.resize(num_blocks_N);
    for (size_t n_block = 0; n_block < num_blocks_N; ++n_block) {
        auto aInfo = arm_compute::TensorInfo();
        aInfo.init(arm_compute::TensorShape(BLOCK_SIZE, M),
                   arm_compute::Format::F32,
                   arm_compute::Strides{sizeof(float), K * sizeof(float)},
                   size_t(0),
                   size_t(M * K * sizeof(float)));
        auto bInfo = arm_compute::TensorInfo(arm_compute::TensorShape(BLOCK_SIZE, BLOCK_SIZE),
                                             1,
                                             arm_compute::DataType::F32);
        auto cInfo = arm_compute::TensorInfo();
        cInfo.init(arm_compute::TensorShape(BLOCK_SIZE, M),
                   arm_compute::Format::F32,
                   arm_compute::Strides{sizeof(float), N * sizeof(float)},
                   size_t(0),
                   size_t(N * M * sizeof(float)));
        tensorsA[n_block].allocator()->init(aInfo);
        tensorsB[n_block].allocator()->init(bInfo);
        tensorsC[n_block].allocator()->init(cInfo);
        tensorsB[n_block].allocator()->allocate();
        tensorInfosA[n_block] = aInfo;
        tensorInfosB[n_block] = aInfo;
        tensorInfosC[n_block] = aInfo;
    }
    ...
    parallel_for(num_blocks_N, [&](size_t n_block) {
        for (size_t k_block = 0; k_block < num_blocks_K; ++k_block) {
            ...
            auto _b = reinterpret_cast<float*>(tensorsB[n_block].buffer());
            ...
            std::cout << "dequantize " << ms_int.count() << " ns" << std::endl;

            t1 = std::chrono::high_resolution_clock::now();
            auto _a = reinterpret_cast<void*>(psrc + start_k);
            auto _c = reinterpret_cast<void*>(pdst + start_n);
            tensorsA[n_block].allocator()->import_memory(_a);
            tensorsC[n_block].allocator()->import_memory(_c);
            ...
            gemm.configure(&tensorsA[n_block], &tensorsB[n_block], nullptr, &tensorsC[n_block], 1.0f, 0.0f, gemmInfo);
            gemm.run();
            t2 = std::chrono::high_resolution_clock::now();
            ms_int = duration_cast<std::chrono::nanoseconds>(t2 - t1);
            std::cout << "gemm " << ms_int.count() << " ns" << std::endl;
        }
    });
//    if (bias != nullptr) {
