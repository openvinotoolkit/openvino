...
JitConv3DExecutor::JitConv3DExecutor(const ConvAttrs& attrs,
    ...
    // Initialize minimal inner-product kernel (scaffold)
...
bool JitConv3DExecutor::supports(const ConvConfig& cfg) {
    // Require 5D NCDHW, FP16 src/wei/dst, group=1, no dilation, stride 1 or 2
    ...
    if (!(sp == ov::element::f16 && wp == ov::element::f16 && dp == ov::element::f16)) return false;
