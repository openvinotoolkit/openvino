...
static MemoryPtr prepareWeightMemory(const MemoryPtr weightsMemory,
    ...
    auto packedBsize = mlas_sgemm_pack_get_size(static_cast<int>(N), static_cast<int>(K));
    ...
    auto create = [&]() {
        ...
        mlas_sgemm_pack(weightsTransposed ? "T" : "F", static_cast<int>(N), static_cast<int>(K), static_cast<int>(ldb), weightPtr, prepackedDst);
        return _ptr;
    };
