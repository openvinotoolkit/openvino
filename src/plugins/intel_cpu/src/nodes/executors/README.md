...
This is the reference for the framework itself; for the node side (lifecycle,
registration) see [../README.md](../README.md).
...
| `OV_CPU_INSTANCE_X64(...)` | x86-64 (`OPENVINO_ARCH_X86_64`) |
| `OV_CPU_INSTANCE_ARM64(...)` | AArch64 (`OPENVINO_ARCH_ARM64`) |
| `OV_CPU_INSTANCE_ACL(...)` | ARM Compute Library, AArch32 or AArch64 |
| `OV_CPU_INSTANCE_ACL32(...)` | ARM Compute Library, AArch32 only |
| `OV_CPU_INSTANCE_ACL64(...)` | ARM Compute Library, AArch64 only |
| `OV_CPU_INSTANCE_DNNL(...)` | oneDNN available, any architecture |
| `OV_CPU_INSTANCE_DNNL_X64(...)` | oneDNN available on x86-64 |
| `OV_CPU_INSTANCE_DNNL_ARM64(...)` | oneDNN available on AArch64 |
| `OV_CPU_INSTANCE_MLAS_X64(...)` | MLAS available on x86-64 |
| `OV_CPU_INSTANCE_MLAS_ARM64(...)` | MLAS available on AArch64 |
| `OV_CPU_INSTANCE_RISCV64(...)` | RISC-V 64 (`OPENVINO_ARCH_RISCV64`) |
| `OV_CPU_INSTANCE_KLEIDIAI(...)` | KleidiAI available |
...
class Executor {
public:
    virtual bool update(const MemoryArgs& memory);          // reshape / re-init
    virtual void execute(const MemoryArgs& memory);          // run computation
    [[nodiscard]] virtual impl_desc_type implType() const = 0;  // report impl type
    virtual ~Executor() = default;
};
```

Only `implType()` is pure virtual; `update()`/`execute()` have default bodies that
throw `OPENVINO_THROW_NOT_IMPLEMENTED` so a concrete executor only needs to
override the overload(s) it actually uses.

A reference executor returns `impl_desc_type::ref`; a JIT executor wraps a kernel
(see [Writing a JIT kernel](#writing-a-jit-kernel) below and the
...
|-----|-------------|--------------|
| AVX2 | 256-bit (8×f32) | `ymm` registers, FMA; the baseline most new JIT executors gate on |
...
> SSE4.1 is still used as the last-resort ISA fallback in some existing JIT
> kernels (e.g. the generic eltwise JIT executor in `jit/eltwise.cpp`), but new
