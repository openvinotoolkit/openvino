...
bool MlasTransposeExecutor::init(const TransposeParams &transposeParams,
    ...
    bool moving_single_axis = IsTransposeMovingSingleAxis(transposeParams.permuteParams.order, from, to);
    if (moving_single_axis && srcDescs[0]->hasLayoutType(LayoutType::ncsp)) {
            return true;
    }
    return false;
}
