...
void NewRefInterpolateExecutor::buildIndexWeightTables() {
    ...
    // Make sure we have dimensions and scales
    if (srcDimsPadded.empty() || dstDims_.empty()) {
        return;
    }
    
    // Create old reference executor with padded dimensions
    ...
    try {
        ...
        if (!oldRefExecutor_) {
            ...
            std::cerr << "Failed to create old reference executor - nullptr" << std::endl;
            return;
        }
    } catch (const std::exception& e) {
        // Failed to create reference executor
        std::cerr << "Failed to create old reference executor: " << e.what() << std::endl;
...
void NewRefInterpolateExecutor::exec(const std::vector<MemoryCPtr>& src,
    ...
    if (!oldRefExecutor_) {
        // Fall back to creating the executor here if not already created
        std::cerr << "Old ref executor is null, trying to build tables" << std::endl;
        buildIndexWeightTables();
    }
    
    if (!oldRefExecutor_ || src.empty() || dst.empty()) {
        std::cerr << "Cannot execute: oldRefExecutor=" << (oldRefExecutor_ ? "valid" : "null") 
