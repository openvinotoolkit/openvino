...
namespace ov::intel_cpu {
    namespace kai_common {
        enum class KAIKernelTag {
            F32_NEON_MLA,
            I8_NEON_DOTPROD,
            I8_NEON_IMM,
            I4_NEON_DOTPROD,
        ...
        using KernelInterface = std::variant<
        ...
        class uKernelBase {
            ...
            public:
                ...
                /// @param dstMem 
                /// @param srcMem 
                virtual void execute(const ov::intel_cpu::CpuParallelPtr& cpu_parallel, const ov::intel_cpu::Dim M, ov::intel_cpu::MemoryPtr dstMem, ov::intel_cpu::MemoryPtr srcMem) = 0;
                
                /// @brief Returns the size required for RHS packing
                /// @return 
                virtual const size_t get_rhsPackedSize() {
                    return 0;
                }
                ...
                virtual void packData(bool isTransposed, MemoryCPtr weightsMemory, MemoryPtr biasMem, bool hasBias, float* rhs_scales, MemoryPtr rhsPackedMemory) = 0;
                
                /// @brief Returns the size of LHS packing
                /// @param m 
                /// @return 
                virtual size_t getLHSPackedSize(size_t m) {
                    return 0;
                }
        };
        ...
        class uKernel<KAIKernelTag::F32_NEON_MLA> : public uKernelBase {
            ...
            public:
                ...
                const size_t get_rhsPackedSize() override {
                    return kai_get_rhs_packed_size_rhs_pack_kxn_f32p8x1biasf32_f32_f32_neon(N, K);
                }

                void packData(bool isTransposed, MemoryCPtr packedWeights, MemoryPtr biasMem, bool hasBias, float* rhs_scales, MemoryPtr rhsPackedMemory) override {
                    ...
                    kai_run_rhs_pack_kxn_f32p8x1biasf32_f32_f32_neon(1, N, K, nr, kr, sr, rhs_stride, rhs, bias, nullptr, rhs_packed, 0, nullptr);
                }
                ...
                void execute(const ov::intel_cpu::CpuParallelPtr& cpu_parallel, const ov::intel_cpu::Dim M, ov::intel_cpu::MemoryPtr dstMem, ov::intel_cpu::MemoryPtr srcMem) override {
                    ...
                    cpu_parallel->parallel_for(n_blocks, [&](size_t n_block) {
                        ...
                        _ukernel.run_matmul(M, n_block_size, K, lhs, lhs_stride, rhs_ptr, dst_ptr, dst_stride_row, dst_stride_col, FLOAT_MIN, FLOAT_MAX);
                    });
                }   // end of execute()...
        };
        ...
        class uKernel<KAIKernelTag::I8_NEON_DOTPROD> : public uKernelBase {
            ...
            public:
                ...
                void packData(bool isTransposed, MemoryCPtr weightsMemory, MemoryPtr biasMem, bool hasBias, float* rhs_scales, MemoryPtr rhsPackedMemory) override {
                    rhsPackedMem = rhsPackedMemory;
                    ...
                    kai_run_rhs_pack_kxn_qsi8cxp_qsi8cx_neon(1, N, K, nr, kr, sr, rhs_native_qs8cx, bias, rhs_scales, rhs_packed_qs8cx, 0, &params);
                }
        ...
        class uKernel<KAIKernelTag::I4_NEON_DOTPROD> : public uKernelBase {
            ...
            public:
                ...
                void packData(bool isTransposed, MemoryCPtr weightsMemory, MemoryPtr biasMem, bool hasBias, float* rhs_scales, MemoryPtr rhsPackedMemory) override {
                    ...
                    if (isTransposed)
                        kai_run_rhs_pack_kxn_qsi4cxp_qs4cxs1s0(1, N, K, nr, kr, sr, rhs_native_qs4cx, bias_ptr, rhs_scales, rhs_packed_qs4cx, 0, &params);
                    else
                        kai_run_rhs_pack_nxk_qsi4cxp_qs4cxs1s0(1, N, K, nr, kr, sr, rhs_native_qs4cx, bias_ptr, rhs_scales, rhs_packed_qs4cx, 0, &params);
                }
                ...
                void execute(const ov::intel_cpu::CpuParallelPtr& cpu_parallel, const ov::intel_cpu::Dim M, ov::intel_cpu::MemoryPtr dstMem, ov::intel_cpu::MemoryPtr srcMem) override {
                    kai_matmul_clamp_f32_qai8dxp_qsi4cxp_ukernel _ukernel = std::get<kai_matmul_clamp_f32_qai8dxp_qsi4cxp_ukernel>(getuKernelInterface());
                    auto *lhs_packed_lowp = lhsPackedMem->getDataAs<int8_t>();
                    auto *rhs_packed_lowp = rhsPackedMem->getDataAs<int8_t>();
                    ...
                    cpu_parallel->parallel_for(M_BLOCKS, [&](size_t m_blk) {
                        const size_t M_iter = std::min(M - m_blk * m_step, m_step);
                        auto* lhs_packed_block = lhs_packed_lowp + m_blk * packedlhs_block_in_bytes;
                        kai_run_lhs_quant_pack_qai8dxp_f32(M_iter, K, mr, kr, sr, 0,
                            lhs + m_blk * m_step * K,  // LHS (F32)
                            lhs_stride, lhs_packed_block  // lhs packed output
                        );
                        cpu_parallel->parallel_for(N_BLOCKS, [&](size_t n_blk) {
                            //  matmul exec
                            const size_t rhs_packed_offset = _ukernel.get_rhs_packed_offset(n_blk * n_step, K);
                            ...
                            float* dst_ptr = (dst + dst_offset / sizeof(float));
                            const size_t N_iter = std::min(N - n_blk * n_step, n_step);
                            _ukernel.run_matmul(M_iter, N_iter, K, lhs_ptr, rhs_ptr, dst_ptr, dst_stride_row, dst_stride_col, FLOAT_MIN, FLOAT_MAX);
                        });
                    });
            ...
            public:
                ...
                void packData(bool isTransposed, MemoryCPtr weightsMemory, MemoryPtr biasMem, bool hasBias, float* rhs_scales, MemoryPtr rhsPackedMemory) override {
                    ...
                    if (isTransposed) {
                        ...
                        kai_run_rhs_pack_kxn_qsi4c32p_qsu4c32s1s0(1, N, K, nr, kr, sr, group_size , convertedRhs.data(), N/2, bias_ptr, scalesBf16.data(), scale_stride, rhs_packed, 0, &params_kxn);
                    } else {
                        ...
                        kai_run_rhs_pack_nxk_qsi4c32p_qsu4c32s1s0(1, N, K, nr, kr, sr, group_size , convertedRhs.data(), K/2, bias_ptr, scalesBf16.data(), scale_stride, rhs_packed, 0, &params_nxk);
                    }
                }
                ...
                void execute(const ov::intel_cpu::CpuParallelPtr& cpu_parallel, const ov::intel_cpu::Dim M, ov::intel_cpu::MemoryPtr dstMem, ov::intel_cpu::MemoryPtr srcMem) override {
                    kai_matmul_clamp_f32_qai8dxp_qsi4c32p_ukernel _ukernel = std::get<kai_matmul_clamp_f32_qai8dxp_qsi4c32p_ukernel>(getuKernelInterface());
                    ...
                    cpu_parallel->parallel_for(M_BLOCKS, [&](size_t m_blk) {
                        ...
                        cpu_parallel->parallel_for(N_BLOCKS, [&](size_t n_blk) {
                            ...
                            _ukernel.run_matmul(M_iter, N_iter, K, group_size, lhs_ptr, rhs_ptr, dst_ptr, dst_stride_row, dst_stride_col, FLOAT_MIN, FLOAT_MAX);
                        });
                    });
                }   // end of execute()...
        };

    }   // end of ov:intel_cpu::kai_common ...
}   // end of ov::intel_cpu ...