...
public:
    EltwiseStateFulExecutor(EltwiseAttrs attrs, const MemoryArgs& memory, ExecutorContext::CPtr context)
        ...
        std::vector<ov::element::Type> output_precisions{
            memory.at(ARG_DST)->getPrecision()};  // -1 for output precision
        ...
        bool canUseOptimizedImpl =
            EltwiseJitExecutor::supports(m_attrs,
        ...
        // std::cout << m_shapeAgnosticData.eltwise_data << "\n";
    }
    ...
    bool update(const MemoryArgs& memory) override {
        // memory.at(ARG_DST)->getDescWithType<BlockedMemoryDesc>()
        // const bool inputBlkDimsChanged = std::any_of(memory.begin(), memory.end(), [&](const std::pair<int,
        // MemoryPtr>& entry) {
        //     const auto& [argId, mem] = entry;
        //     if (argId == ARG_DST) {
        //         return false;
        //     }

        //     return mem->getDescWithType<BlockedMemoryDesc>()->getBlockDims() != m_currentInBlkDims[argId - ARG_SRC];
        // });

        // if (!inputBlkDimsChanged) {
        //     return true;
        // }
    ...
    void execute(const MemoryArgs& memory) override {
        ...
        const VectorDims& outDims = eltwiseImplType == EltwiseImplType::optimizedShapeAgnostic
                                        ? m_execParams.outDims
                                        : m_executor->getOutDims();
        // size_t inputIndex = 0;
        for (const auto& [argId, mem] : memory) {
            if (argId == ARG_DST) {
        ...
        // print_dnnl_memory(memory.at(ARG_SRC)->getPrimitive(), 10, ARG_SRC);
        // print_dnnl_memory(memory.at(ARG_SRC_1)->getPrimitive(), 10, ARG_SRC_1);

        m_executor->exec(args_ptrs, outDims);

        // print_dnnl_memory(memory.at(ARG_DST)->getPrimitive(), 10, ARG_SRC_2);
    }
