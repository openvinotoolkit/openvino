...
bool EltwiseJitExecutor::isSupportedConfiguration(
    const EltwiseAttrs& attrs,
    const size_t rank,
    ...
    if (!postOps.empty()) {
        ...
        if (const auto eltwisePostOp = std::any_cast<ScaleShiftPostOp>(&postOps.back())) {
            if (eltwisePostOp->type() == ScaleShiftPostOp::Type::divide) {
                supported_input_precisions = std::set<ov::element::Type>{ov::element::f16, ov::element::f32};
            }
        } else if (const auto eltwisePostOp = std::any_cast<ActivationPostOp>(&postOps.back())) {
            if (eltwisePostOp->type() == ActivationPostOp::Type::floor) {
                supported_input_precisions = std::set<ov::element::Type>{ov::element::f16, ov::element::f32};
            }
        }
    } else {
...
bool EltwiseJitExecutor::isSupportedConfiguration(const EltwiseConfig& config) {
    ...
    return isSupportedConfiguration(config.attrs, srcRank(config));
}
...
// void EltwiseJitExecutor::execute(const MemoryArgs& memory) {
//     auto dstMemDesc = memory.at(ARG_DST);
//     VectorDims dims_out = dstMemDesc->getShape().getStaticDims();

//     exec(args_ptrs, dims_out);
// }

// bool EltwiseJitExecutor::update(const MemoryArgs& /*memory*/) {
//     // For shape agnostic kernels, update runtime parameters
//     if (m_useRuntimePtrs) {
//         // Update offsets and work amount for new shapes
//         // @todo updateRuntimeParams(memory);
//     }
//     return true;
// }
...
//     // Check if input shape rank is within limits
//     auto srcDesc = config.descs.at(ARG_SRC);
//     if (srcDesc->getShape().getRank() > MAX_ELTWISE_DIM_RANK) {
//         return false;
//     }
