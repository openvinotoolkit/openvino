...
void StridedSlice::initSupportedPrimitiveDescriptors() {
    ...
    if (nDims > 2 && attrs.equalDims) {
        ...
        if (hasConstAttrInputs) {
            ...
            addHiddenDims(tmpAttrs,
                          getInputShapeAtPort(DATA_ID).getRank(),
    ...
    for (auto itr = range.first; itr != range.second; ++itr) {
        ...
        config.inConfs[BEGIN_ID].setMemDesc(
            creators.at(LayoutType::ncsp)->createSharedDesc(iPrecision, getInputShapeAtPort(BEGIN_ID)));
        config.inConfs[END_ID].setMemDesc(
            creators.at(LayoutType::ncsp)->createSharedDesc(iPrecision, getInputShapeAtPort(END_ID)));
        if (isStrideSpecified)
            config.inConfs[STRIDE_ID].setMemDesc(
                creators.at(LayoutType::ncsp)->createSharedDesc(iPrecision, getInputShapeAtPort(STRIDE_ID)));
        if (isAxesSpecified)
            config.inConfs[AXES_ID].setMemDesc(
                creators.at(LayoutType::ncsp)->createSharedDesc(iPrecision, getInputShapeAtPort(AXES_ID)));
