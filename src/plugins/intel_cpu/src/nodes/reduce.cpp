...
inline void Reduce::create_hybrid_working_memory() {
    ...
    if (layout == ReduceLayoutType::reduce_nspc) {
        format = (rank == 4) ? memory::format_tag::nhwc : memory::format_tag::ndhwc;
    } else if (rank == 4) {
        format = mayiuse(cpu::x64::avx512_core) ? memory::format_tag::nChw16c : memory::format_tag::nChw8c;
    } else {
        format = mayiuse(cpu::x64::avx512_core) ? memory::format_tag::nCdhw16c : memory::format_tag::nCdhw8c;
    }
