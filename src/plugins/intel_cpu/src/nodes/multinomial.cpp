...
Multinomial::Multinomial(const std::shared_ptr<ov::Node>& op, const GraphContext::CPtr& context)
    ...
    const auto& probs_dims = getInputShapeAtPort(PROBS_PORT).getDims();
    const auto& num_samples_dims = getInputShapeAtPort(NUM_SAMPLES_PORT).getDims();
    auto probs_ps = PartialShape(probs_dims);
    auto num_samples_ps = PartialShape(num_samples_dims);

    m_errorPrefix = "Multinomial node with name '" + getName() + "' ";
    if (probs_dims.size() != 1 && probs_dims.size() != 2)
        IE_THROW() << m_errorPrefix << "has unsupported 'probs' input rank: " << boxes_dims.size();
    if (num_samples_dims.size() != 0 && num_samples_dims.size() != 1)
        IE_THROW() << m_errorPrefix << "has unsupported 'num_samples' input rank: " << boxes_dims.size();
}
...
void Multinomial::createPrimitive() {
    ...
    if (m_algo == PHILOX) {
        ...
        kernel::MultinomialCompileParams jcp;

        jcp.out_data_type = m_output_prc;

        m_jit_kernel = MultinomialJitKernel::createInstance<kernel::Multinomial>(jcp);
...
float uint32ToFloat(uint32_t x) {
    Multinomial::OutputType out_val;
...
std::pair<uint64_t, uint64_t> Multinomial::computePhilox(void* out, size_t out_el_num, const std::pair<uint64_t, uint64_t>& prev_state) {
    // When both seed values are equal to zero Multinomial should generate non-deterministic sequence.
    ...
    if (m_jit_kernel) {
        parallel_nt(m_threads_num, [&](const int ithr, const int nthr) {
                ...
                kernel::MultinomialCallArgs args;
        ...
        auto threadBody = [&](const int ithr, const int nthr) {
            ...
            for (; work_rest > 0; work_rest -= p.step_b, out_cur += p.step_b) {
                ...
                switch (m_output_prc) {
                    ...
                    default: THROW_CPU_NODE_ERR("Unsupported type of Multinomial: ", m_output_prc.to_string());
                }
    ...
    // Calculate counter values for next Multinomial run.
...
////////////// STL algo ///////////////
void Multinomial::computeStl(void* out, size_t work_amount) {
    switch (m_output_prc) {
        case element::f32: {
...
void Multinomial::generateData(DISTR_TYPE distribution, void* out, size_t work_amount) {
...
void Multinomial::initEdgeValues(OutputType& dst, const void* src, const element::Type& output_type) {
#define EL_CASE(E) \
    case element::E: \
