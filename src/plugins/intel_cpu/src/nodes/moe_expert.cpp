...
#include "/mnt/tingqian/aboutSHW/include/linux_perf.hpp"
...
static int MOE_DEBUG = std::getenv("MOE_DEBUG") ? std::atoi(std::getenv("MOE_DEBUG")) : 0;
...
struct onednn_matmul {
    ...
    onednn_matmul& w_scale(int k_group_size) {
        if (m_use_ip) {
            ...
            // shape & layout has to be as following:
    ...
    void exec(const engine &aengine, const stream &astream,
              void * psrc, //memory& src_mem,
              void * pdst, //memory& dst_mem,
              memory& weight,
              memory& scale,
              memory& zp,
              memory& bin_mem) {
...
struct MOEExpert::Executor : MOEExpert::ExecutorBase {
    int scratch_batch_size;
    memory scratch_src;
    memory scratch_up;
    memory scratch_gate;
    memory scratch_down;
    ...
    int normalize_batch_size(int batch_size) {
        if (batch_size < 16) {
    ...
    nkernels& get_nkernels (const dnnl::engine& engine, int batch_size) {
        ...
        std::cout << ">>>>>>>>>> get_nkernels batch_size=" << batch_size << std::endl;

        nkernels& nks = nkernels_cache[batch_size];

        nks.up.init(m_act_dtype, m_weight_dtype, batch_size, hidden_size, intermediate_size, ic_group_size);
        nks.up.create(engine, linear_up.m_wei_md);
        
        nks.gate.init(m_act_dtype, m_weight_dtype, batch_size, hidden_size, intermediate_size, ic_group_size);
        nks.gate.post_op_silu().post_op_bin_mul(true).create(engine, linear_gate.m_wei_md);
        
        nks.down.init(m_act_dtype, m_weight_dtype, batch_size, intermediate_size, hidden_size, ic_group_size);
    ...
    Executor(const dnnl::engine& engine,
             ...
             int topk,
             int expert_num
             ) : m_act_dtype(act_dtype), m_weight_dtype(weight_dtype), hidden_size(hidden_size),
                 intermediate_size(intermediate_size), ic_group_size(ic_group_size),
                 expert_num(expert_num) {
        // specialized 2nd-token kernel with more fused ops
        linear_up.init(m_act_dtype, m_weight_dtype, 1, hidden_size, intermediate_size, ic_group_size);
        linear_gate.init(m_act_dtype, m_weight_dtype, 1, hidden_size, intermediate_size, ic_group_size);
        linear_down.init(m_act_dtype, m_weight_dtype, 1, intermediate_size, hidden_size, ic_group_size);
    ...
    void reorder_weights(const dnnl::engine& engine, ExpertWeights * pweight) {
        stream s(engine);
        auto do_reorder = [&](Weight* pw, onednn_matmul& mm) {
            int oc = pw->oc;
            int ic = pw->ic;
            int n_groups = pw->ic / pweight->up.qg_size;
            ...
            auto ic_qg = pw->ic/pw->qg_size;
            memory new_src({{pw->oc, ic_qg}, memory::data_type::u8, memory::format_tag::ab}, engine);
            int cnt = ic_qg * pw->oc;
            auto* psrc0 = pw->zp;
            auto* psrc1 = (int8_t*)new_src.get_data_handle();
            for(int i = 0; i < cnt; i+=2) {
                auto v = *psrc0++;
                *psrc1++ = v & 0xF;
                *psrc1++ = v >> 4;
            }
            //dnnl::reorder(src, new_src).execute(s, src, new_src);
    ...
    void execute(const dnnl::stream& astream, MOEExpert* pnode) override {
        auto prof = LinuxPerf::Profile(pnode->getTypeStr(), "execute");
        ...
        if (0) {
            std::cout << ">>>>> execute " << pnode->m_moe_op->get_friendly_name() << std::endl;
            std::cout << dims2str(dims_0) << input_precisions[0] << " " << pnode->getSrcDataAtPortAs<const float>(0) << std::endl;
            std::cout << dims2str(dims_1) << input_precisions[1] << " " <<pnode->getSrcDataAtPortAs<const float>(1) <<std::endl;
            std::cout << dims2str(dims_out) << output_precisions[0] << " " <<pnode->getDstDataAtPortAs<const float>(0) <<std::endl;
            //if (dims_0[0] == 1) abort();
        }
        ...
        //if (pnode->m_moe_op->get_friendly_name() == "moe_router_47") {
        //    std::cout << " moe_router_47 : " << dims2str(dims_0) << "," << dims2str(dims_1) << std::endl;
        //}

        memset(final_hidden_states, 0, n_states * n_tokens * output_precisions[0].bitwidth()/8);

        auto cmp_less = [](const std::pair<int, float>& a, const std::pair<int, float>& b) {
        ...
        // topk + softmax
        for (auto& v : expert_ntoks) v->store(0);
        ...
        {
            auto prof = LinuxPerf::Profile("topk");
            parallel_for(n_tokens, [&](int i) {
                ...
                for(int k = 0; k < pnode->m_config.expert_num; k++) {
                    max_heap[k].first = k;
                    max_heap[k].second = p_router_weights[k];
                }
                std::nth_element(max_heap.begin(), max_heap.begin()+pnode->m_config.topk-1, max_heap.end(), cmp_greater);

                // softmax
                float denorm = 0;
                float vmax = max_heap[0].second;
                for(int k = 0; k < pnode->m_config.topk; k++) {
                    auto v = std::expf(max_heap[k].second - vmax);
                    denorm += v;
                    max_heap[k].second = v;
                }
        ...
        if (n_tokens > 1) {
            ...
            for(int i = 0; i < expert_num; i++) {
                ...
                if (max_num_toks < num_tokens)
                    max_num_toks = num_tokens;
            }
            ...
            for(int expert_id = 0; expert_id < expert_num; expert_id++) {
                ...
                if (num_tokens == 0)
                    continue;
                prof = LinuxPerf::Profile("gather", expert_id);
                ...
                parallel_for(n_tokens, [&](int i) {
                    ...
                    for(int k = 0; k < pnode->m_config.topk; k++) {
                        if (p_expert_ids[k] == expert_id) {
                            ...
                            auto* dst = reinterpret_cast<int8_t*>(scratch_src.get_data_handle()) + seq * n_states * act_dtype_size;
                ...
                //for(int seq = 0; seq < num_tokens; seq++)
                //    std::cout << ">>>>>> token,expert,weight = " << token_ids[seq] << "," << expert_id << "," << token_weights[seq] << std::endl;
                ...
                nks.up.exec(engine, astream, scratch_src.get_data_handle(), scratch_up.get_data_handle(), up.mem_data, up.mem_scale, up.mem_zp, empty_mem);
                nks.gate.exec(engine, astream, scratch_src.get_data_handle(), scratch_gate.get_data_handle(), gate.mem_data, gate.mem_scale, gate.mem_zp, scratch_up);
                nks.down.exec(engine, astream, scratch_gate.get_data_handle(), scratch_down.get_data_handle(), down.mem_data, down.mem_scale, down.mem_zp, empty_mem);
                ...
                //std::cout << "\t" << to_string((float*)final_hidden_states, n_states, n_tokens, 16);
            }
        } else {
            for(int i = 0; i < n_tokens; i++) {
                ...
                for(int k = 0; k < pnode->m_config.topk; k++) {
                    ...
                    auto prof = LinuxPerf::Profile("expert", expert_id);
                    if (router_w) {
                        switch(m_act_dtype) {
                            case memory::data_type::bf16:
                            ...
                            case memory::data_type::f16:
                                ...
                                break;
                            case memory::data_type::f32:
                                (reinterpret_cast<float*>(router_w))[0] = expert_weight;
                                break;
                        }
                    }
                    auto& up = pnode->m_weights[expert_id].up;
                    ...
                    auto* pdata_up = (float*)scratch_up.get_data_handle();
                    auto* pdata_gate = (float*)scratch_gate.get_data_handle();

                    linear_up.exec(engine, astream, hidden_states, scratch_up.get_data_handle(), up.mem_data, up.mem_scale, up.mem_zp, empty_mem);
                    linear_gate.exec(engine, astream, hidden_states, scratch_gate.get_data_handle(), gate.mem_data, gate.mem_scale, gate.mem_zp, scratch_up);
                    linear_down.exec(engine, astream, scratch_gate.get_data_handle(), final_hidden_states, down.mem_data, down.mem_scale, down.mem_zp, scratch_router);
                }
        ...
        static int layers = 0;
        //std::cout << "final_hidden_states: " << layers << " " << to_string((float*)pnode->getDstDataAtPortAs<float>(0), n_states, n_tokens, 8, -8);
        layers++;
        //if (layers >= 1) abort();
        if (MOE_DEBUG) abort();
    };
...
void MOEExpert::extractConsts() {
    const auto& engine = getEngine();
    const auto& consts = m_moe_op->get_consts();
    OPENVINO_ASSERT(m_config.expert_num == consts.size());
    m_weights.resize(consts.size());

    auto extract_consts = [](std::shared_ptr<ov::op::v0::Constant> op, std::shared_ptr<ov::op::v0::Constant> sc, std::shared_ptr<ov::op::v0::Constant> zp, Weight& wei) {
...
void MOEExpert::initSupportedPrimitiveDescriptors() {
    if (!supportedPrimitiveDescriptors.empty()) {
        return;
    }

    auto input_precision0 = getOriginalInputPrecisionAtPort(0);
    auto input_precision1 = getOriginalInputPrecisionAtPort(1);
    ...
    if (1) {
        std::cout << "MOEExpert::initSupportedPrimitiveDescriptors(" << input_precision0 << ","
                  << input_precision1 << ")=>"
                  << output_precision << std::endl;
    }
    inPortConfigs.emplace_back(LayoutType::ncsp, input_precision0, getInputShapeAtPort(0), false, -1);  // hidden_states
    inPortConfigs.emplace_back(LayoutType::ncsp, ov::element::f32, getInputShapeAtPort(1), false, -1);  // router-weights
    outPortConfigs.emplace_back(LayoutType::ncsp, output_precision, getOutputShapeAtPort(0), false, -1); // final_hidden_states
...
void MOEExpert::prepareParams() {
    ...
    //std::cout << input_precisions[0] << input_precisions[1] << input_precisions[2] << std::endl;
    MoEKey key = {.act_dtype = DnnlExtensionUtils::ElementTypeToDataType(input_precisions[0], DnnlExtensionUtils::throw_tag()),
                 .weight_dtype = memory::data_type::u4,
                 .hidden_size = m_config.hidden_size,
                 .intermediate_size = m_weights[0].gate.oc,
                 .ic_group_size = m_weights[0].gate.qg_size,
                 .topk = m_config.topk,
                 .expert_num = m_config.expert_num};
...
void MOEExpert::execute(const dnnl::stream& strm) {
