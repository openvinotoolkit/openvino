...
void Eltwise::initSupportedPrimitiveDescriptors() {
    ...
    // std::cout << getName() << ":" << getTypeStr() << " Node descriptors list size: " << nodeDescriptorsList.size() <<
    // "\n";
    ...
    for (const auto& nodeDescriptors : nodeDescriptorsList) {
        ...
        // std::cout << "Create node config: " << nodeConfig << "\n";
...
bool Eltwise::canFuse(const NodePtr& node) const {
    ...
    if ((eltwise == nullptr) || (!EltwiseJitExecutor::supports(m_attrs, inputShapes.front().getRank()))) {
        return false;
    }
#endif
...
bool Eltwise::canFuseParent(const NodePtr& parentNode) const {
    ...
    if (!EltwiseJitExecutor::supports(m_attrs, inputShapes.front().getRank(), input_precisions)) {
        return false;
    }
#else
