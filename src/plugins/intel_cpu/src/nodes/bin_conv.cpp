...
void BinaryConvolution::createPrimitive() {
    ...
    jcp.ngroups = static_cast<int>(group);
    ...
    jcp.typesize_in = srcPrecision == ov::element::u1 ? 1 : static_cast<int>(srcPrecision.size());
    jcp.typesize_out = dstPrecision == ov::element::u1 ? 1 : static_cast<int>(dstPrecision.size());
