...
private:
    ...
    void roi_pool_max(int c_blocks) {
        ...
        for (int i = 0; i < c_blocks; i++) {
            ...
            store_emitter->emit_code({static_cast<size_t>(vmm_dst.getIdx()), static_cast<size_t>(i * dst_c_off)},
    ...
    void roi_pool_bilinear(int c_blocks) {
        ...
        for (int i = 0; i < c_blocks; i++) {
            ...
            store_emitter->emit_code({static_cast<size_t>(vmm_src11.getIdx()), static_cast<size_t>(dst_c_off)},
                                     {static_cast<size_t>(reg_output.getIdx())},
    ...
    void empty_roi(int c_blocks) {
        ...
        for (int i = 0; i < c_blocks; i++) {
            store_empty_roi_emitter->emit_code(
                {static_cast<size_t>(vmm_zero.getIdx()), static_cast<size_t>(i * dst_c_off)},
                {static_cast<size_t>(reg_output.getIdx())},
                store_pool_vec_idxs,
                store_pool_gpr_idxs);
        }
