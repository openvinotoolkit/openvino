...
ROIPooling::ROIPooling(const std::shared_ptr<ov::Node>& op, const GraphContext::CPtr& context)
    ...
    auto roiPooling = ov::as_type_ptr<const ov::op::v0::ROIPooling>(op);
    refParams.pooled_h = static_cast<int>(roiPooling->get_output_roi()[0]);
    refParams.pooled_w = static_cast<int>(roiPooling->get_output_roi()[1]);
...
void ROIPooling::prepareParams() {
    ...
    refParams.mb = static_cast<int>(outDims[0]);
    refParams.c = static_cast<int>(rnd_up(inDims[1], refParams.c_block));
    refParams.nb_c = refParams.c / refParams.c_block;
    refParams.ih = static_cast<int>(inDims[2]);
    refParams.iw = static_cast<int>(inDims[3]);
    refParams.oh = static_cast<int>(outDims[2]);
    refParams.ow = static_cast<int>(outDims[3]);
                    ...
                    if (in_y < 0 || in_y > jpp.ih - 1 || in_x < 0 || in_x > jpp.iw - 1) {
                        ...
                        for (int cbb_cur = 0; cbb_cur < cb_num; cbb_cur++) {
                            ...
                            for (int c = 0; c < c_block; c++) {
                                ...
                                const float top = top_left + (top_right - top_left) * (in_x - static_cast<float>(left_x_index));
                                const float bottom = bottom_left + (bottom_right - bottom_left) * (in_x - static_cast<float>(left_x_index));

                                dst[n * dst_strides[0] + ch_blk_cur * dst_strides[1] + oh * dst_strides[2] +
                                    ow * dst_strides[3] + c] = top + (bottom - top) * (in_y - static_cast<float>(top_y_index));
                            }
                        }
                    }
                }
...
std::pair<float, float> ROIPooling::ROIPoolingExecutor::getXYForBilinearMode(const float roi_start_h,
                                                                             ...
                                                                             const int pooled_h,
                                                                             const int pooled_w) {
    float height_scale = (pooled_h > 1 ? ((roi_end_h - roi_start_h) * static_cast<float>(ih - 1)) / static_cast<float>(pooled_h - 1) : 0.0f);
    float width_scale = (pooled_w > 1 ? ((roi_end_w - roi_start_w) * static_cast<float>(iw - 1)) / static_cast<float>(pooled_w - 1) : 0.0f);
    ...
    if (pooled_h > 1) {
        in_y = (oh == pooled_h - 1 ? roi_end_h * static_cast<float>(ih - 1) : (static_cast<float>(oh) * height_scale + roi_start_h * static_cast<float>(ih - 1)));
    } else {
        in_y = 0.5f * (roi_start_h + roi_end_h) * static_cast<float>(ih - 1);
    }
    if (pooled_w > 1) {
        in_x = (ow == pooled_w - 1 ? roi_end_w * static_cast<float>(iw - 1) : (static_cast<float>(ow) * width_scale + roi_start_w * static_cast<float>(iw - 1)));
    } else {
