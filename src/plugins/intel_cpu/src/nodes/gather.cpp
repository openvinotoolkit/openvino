        ...
        if (jitKernel) {
            ...
            if (!isDynamicNode()) {
                ...
                parallel_nt(m_threads_num, [&](const int ithr, [[maybe_unused]] const int nthr) {
                    ...
                    p.betweenBatchAndAxisIter =
                        static_cast<int>((dstStart / specIndicesSize) % betweenBatchAndAxisSize);
                    for (uint64_t j = 0LU; j < dataElPerVec; j++) {
                        p.specIdxInBytes[j] =
                            static_cast<int>((((dstStart + j) / afterAxisSize) % specIndicesSize) * idxTypeSize);
...
void Gather::initShortParams(threadExecParams& p, const uint64_t start) {
    ...
    if (afterAxisSize == 1) {  // Elementwise gather.
        ...
        auto secondStart = static_cast<int>(start + idxElPerVec);
        for (uint64_t i = 0; i < idxElPerVec; i++) {
            p.afterAxIdxInBytes[i] = static_cast<int>((start + i) % afterAxisSize);
            p.specIdxDiff[i] = static_cast<int>((((secondStart + i) / afterAxisSize) % specIndicesSize) * idxTypeSize) -
                               p.specIdxInBytes[i];
            if (p.specIdxDiff[i] < 0) {
                p.specIdxDiff[i] += static_cast<int>(specIndicesSize * idxTypeSize);
            }
            p.srcBeforeAxisDiff[i] =
                static_cast<int>(((start + i + idxElPerVec) / (specIndicesSize * afterAxisSize)) *
                                 axisAndAfterAxisSizeInBytes) -
                static_cast<int>(((start + i) / (specIndicesSize * afterAxisSize)) * axisAndAfterAxisSizeInBytes);

            p.afterAxIdxInBytes[i] *= static_cast<int>(dataTypeSize);
