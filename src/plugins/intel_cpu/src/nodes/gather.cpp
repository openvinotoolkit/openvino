            ...
            if (!isDynamicNode()) {
                ...
                parallel_nt(m_threads_num, [&](const int ithr, [[maybe_unused]] const int nthr) {
                    ...
                    p.betweenBatchAndAxisIter =
                        static_cast<int32_t>((dstStart / specIndicesSize) % betweenBatchAndAxisSize);
                    for (uint64_t j = 0LU; j < dataElPerVec; j++) {
                        p.specIdxInBytes[j] =
                            static_cast<int32_t>((((dstStart + j) / afterAxisSize) % specIndicesSize) * idxTypeSize);
...
void Gather::initShortParams(threadExecParams& p, const uint64_t start) {
    ...
    if (afterAxisSize == 1) {  // Elementwise gather.
        ...
        for (uint64_t i = 0; i < idxElPerVec; i++) {
            ...
            p.specIdxDiff[i] =
                static_cast<int32_t>((((secondStart + i) / afterAxisSize) % specIndicesSize) * idxTypeSize) -
                p.specIdxInBytes[i];
