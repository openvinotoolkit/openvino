...
void SelectiveSSM::initSupportedPrimitiveDescriptors() {
    ...
    for (const auto& [argId, portId] : m_atoi) {
        descs[argId] = creatorsMap.at(LayoutType::ncsp)->createSharedDesc(getOriginalInputPrecisionAtPort(portId),
                                                                          getInputShapeAtPort(portId));
    }
    descs[ARG_SSM_OUT] =
        creatorsMap.at(LayoutType::ncsp)->createSharedDesc(getOriginalOutputPrecisionAtPort(0), getOutputShapeAtPort(0));
    descs[ARG_SSM_OUT_STATE] =
        creatorsMap.at(LayoutType::ncsp)->createSharedDesc(getOriginalOutputPrecisionAtPort(1), getOutputShapeAtPort(1));
