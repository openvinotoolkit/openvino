...
void AdaptivePooling::execute([[maybe_unused]] const dnnl::stream& strm) {
    ...
    const int chPadding = blockSize * (isBlkFmt ? static_cast<int>(srcBlockDesc->getBlockDims()[1])
                                                : static_cast<int>(srcMemory0.getShape().getStaticDims()[1]));
    ...
    auto poolMax = [&](const float* srcData, float* dstData, int od, int oh, int ow, size_t spatIndOff) {
        ...
        int resIndex = static_cast<int>(dStart) * iHW + static_cast<int>(hStart) * IW +
                       static_cast<int>(wStart);  // initial max index
        for (size_t pixD = dStart; pixD < dEnd; pixD++) {
            for (size_t pixH = hStart; pixH < hEnd; pixH++) {
                for (size_t pixW = wStart; pixW < wEnd; pixW++) {
                    ...
                    resIndex = (res < curr ? static_cast<int>(pixD) * iHW + static_cast<int>(pixH) * IW +
                                                 static_cast<int>(pixW)
                                           : resIndex);
