...
If::If(const std::shared_ptr<ov::Node>& op, RuntimeEnv::Ptr rtEnv) :
        Node(op, rtEnv, InternalDynShapeInferFactory()), ovOp(op) {
...
void If::getSupportedDescriptors() {
    ...
    subGraphThen.CreateGraph(thenBody, rtEnv);
    subGraphElse.CreateGraph(elseBody, rtEnv);
