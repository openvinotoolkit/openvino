...
void Concat::initSupportedPrimitiveDescriptors() {
    ...
    for (auto itr = itrRange.first; itr != itrRange.second; ++itr) {
        ...
        for (size_t i = 0; i < getParentEdges().size(); ++i) {
            ...
            if (isDynamicNode()) {
                ...
                config.inConfs[i].setMemDesc(desc, BlockedMemoryDesc::BLOCKED_DESC_EMPTY_MASK);
            }
        }
    ...
    for (auto refPdIndex : pdIndexesToReuse) {
        ...
        BlockedMemoryDesc::CmpMask mask = BlockedMemoryDesc::BLOCKED_DESC_SKIP_OFFSET_MASK; // any offset
