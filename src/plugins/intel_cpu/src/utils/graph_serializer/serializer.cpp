...
public:
    ...
    WeightlessWriter::FilePosition write([[maybe_unused]] const char* ptr,
        ...
        if (m_skip_weights) {
            ...
            return util::ConstantWriter::write(ptr, size, new_size, compress_to_fp16, src_type, ptr_is_temporary);
        }
    }
...
private:
    bool append_rt_attribute(pugi::xml_node& node, const ov::RuntimeAttribute& attribute) override {
        if (auto wl_attr = ov::as_type<const ov::WeightlessCacheAttribute>(&attribute)) {
            ...
            node.append_attribute("type").set_value(util::get_ir_precision_name(wl_attr->original_dtype));
            node.append_attribute("offset").set_value(wl_attr->bin_offset);
            node.append_attribute("size").set_value(wl_attr->original_size);

            return true;
        } else {
            return util::XmlSerializer::append_rt_attribute(node, attribute);
        }
    }

    bool append_node_attributes(ov::Node& node) override {
        m_weightless_const_writer.skip_weights(m_weightless_mode && node.get_rt_info().count(ov::WeightlessCacheAttribute::get_type_info_static()) != 0);
