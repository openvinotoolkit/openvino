...
void init_tsc() {
    ...
    std::call_once(flag, []() {
        ...
        std::cout << "[CPU_PROFILE] tsc_ticks_per_second = " << tsc_ticks_per_second << std::endl;
...
struct ProfilerManagerFinalizer {
    ...
    const char* dump_file_name = "profile.json";
    ...
    void finalize() {
        ...
        for (auto& pthis : all_managers) {
            ...
            std::cout << "[CPU_PROFILE] #" << pthis->serial << "(" << pthis << ") finalize: dumpped "
                      << data_size << std::endl;
        }
        all_managers.clear();
        ...
        dump_file_over = true;
        not_finalized = false;
        std::cout << "[CPU_PROFILE] Dumpped " << total_size / (1024 * 1024) << " (MB) to " << dump_file_name
                  << std::endl;
    }
    ...
    void register_manager(ProfilerManager* pthis) {
        std::lock_guard<std::mutex> guard(g_mutex);
        std::stringstream ss;
        ss << "[CPU_PROFILE] #" << pthis->serial << "(" << this << ") : is registed." << std::endl;
        std::cout << ss.str();
...
ProfilerManager::ProfilerManager() {
    const char* str_enable = std::getenv("CPU_PROFILE");
    if (!str_enable)
        str_enable = "0";
