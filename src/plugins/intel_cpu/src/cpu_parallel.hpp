...
private:
    ...
    void cpu_parallel_for(const T0& D0, const F& func) const {
#if OV_THREAD == OV_THREAD_TBB_ADAPTIVE
        auto work_amount = static_cast<int32_t>(D0);
        const int32_t nthr = parallel_get_max_threads();
        int32_t virtual_threads = nthr;
        if (m_partitioner == ov::intel_cpu::TbbPartitioner::AUTO) {
        ...
        if (virtual_threads == 1) {
            ...
            if (m_partitioner == ov::intel_cpu::TbbPartitioner::AUTO) {
                tbb::parallel_for(0, virtual_threads, [&](int32_t ithr) {
...
#if OV_THREAD == OV_THREAD_TBB_ADAPTIVE
        auto work_amount = static_cast<int32_t>(D0 * D1);
        const int32_t nthr = parallel_get_max_threads();
        int32_t virtual_threads = nthr;
        if (m_partitioner == ov::intel_cpu::TbbPartitioner::AUTO) {
            virtual_threads = 1 == nthr ? 1 : nthr * m_multiplier;
...
#if OV_THREAD == OV_THREAD_TBB_ADAPTIVE
        auto work_amount = static_cast<int32_t>(D0 * D1 * D2);
        const int32_t nthr = parallel_get_max_threads();
        int32_t virtual_threads = nthr;
        if (m_partitioner == ov::intel_cpu::TbbPartitioner::AUTO) {
            virtual_threads = 1 == nthr ? 1 : nthr * m_multiplier;
...
#if OV_THREAD == OV_THREAD_TBB_ADAPTIVE
        auto work_amount = static_cast<int32_t>(D0 * D1 * D2 * D3);
        const int32_t nthr = parallel_get_max_threads();
        int32_t virtual_threads = nthr;
        if (m_partitioner == ov::intel_cpu::TbbPartitioner::AUTO) {
            virtual_threads = 1 == nthr ? 1 : nthr * m_multiplier;
...
#if OV_THREAD == OV_THREAD_TBB_ADAPTIVE
        auto work_amount = static_cast<int32_t>(D0 * D1 * D2 * D3 * D4);
        const int32_t nthr = parallel_get_max_threads();
        int32_t virtual_threads = nthr;
        if (m_partitioner == ov::intel_cpu::TbbPartitioner::AUTO) {
            virtual_threads = 1 == nthr ? 1 : nthr * m_multiplier;
    ...
    void cpu_parallel_for6d(const T0& D0,
                            ...
                            const F& func) const {
#if OV_THREAD == OV_THREAD_TBB_ADAPTIVE
        auto work_amount = static_cast<int32_t>(D0 * D1 * D2 * D3 * D4 * D5);
        const int32_t nthr = parallel_get_max_threads();
        int32_t virtual_threads = nthr;
        if (m_partitioner == ov::intel_cpu::TbbPartitioner::AUTO) {
            virtual_threads = 1 == nthr ? 1 : nthr * m_multiplier;
    ...
    int32_t m_multiplier = default_multiplier;
