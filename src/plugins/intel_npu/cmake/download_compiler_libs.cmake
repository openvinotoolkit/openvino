...
function(download_and_extract url zip_file extracted_dir)
    ...
    if(NOT EXISTS "${extracted_dir}")
        ...
        if(NOT "${url}" STREQUAL "")
            message(VERBOSE "${url} is not empty")
            ...
            if(NOT download_result EQUAL 0)
                ...
                message(VERBOSE "Download completed: ${zip_file}")
            endif()
        endif()

        message(VERBOSE "Unzipping prebuilt Plugin compiler libraries to ${extracted_dir}")
        ...
        elseif("${zip_file}" MATCHES "\\.tar.gz$")
            if(NOT EXISTS "${extracted_dir}")
                ...
                message(VERBOSE "Directory ${extracted_dir} created to unzip.")
            endif()
        ...
        elseif("${zip_file}" MATCHES "\\.exe$")
            ...
            if(WINRAR_FOUND)
                message(VERBOSE "WinRAR found at: ${WINRAR_EXECUTABLE} and extract ${zip_file} to ${extracted_dir}")
                file(MAKE_DIRECTORY "${extracted_dir}")
                execute_process(
                ...
                if(result EQUAL 0)
                    message(VERBOSE "Extraction successful: ${output}")
                else()
                    #file(REMOVE_RECURSE "${extracted_dir}")
                    message(FATAL_ERROR "Extraction failed: ${error}")
                endif()
            else()
        ...
        if(UNIX AND NOT APPLE)
            ...
            if(OS_NAME STREQUAL "Ubuntu")
                ...
                elseif(OS_VERSION STREQUAL "24.04")
                    message(VERBOSE "This is Ubuntu 24.04")
            ...
            message(WARNING "This is a different Linux distribution: ${OS_NAME}, skip downloading prebuilt Plugin compiler libraries. Can not use plugin compiler libraries!")
