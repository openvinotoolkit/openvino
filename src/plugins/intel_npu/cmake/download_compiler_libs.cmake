...
if(ENABLE_INTEL_NPU_COMPILER)
    ...
    if(NPU_PLUGIN_COMPILER)
        ...
        # The archive may ship the VM runtime already renamed (openvino_intel_npu_vm_runtime)
        # or under the legacy name (npu_interpreter_runtime); pick whichever is present.
        if(EXISTS "${PLUGIN_COMPILER_LIB_PATH}/${PLUGIN_COMPILER_VM_RT_RENAMED_LIB_NAME}")
            set(PLUGIN_COMPILER_VM_RT_LIB "${PLUGIN_COMPILER_LIB_PATH}/${PLUGIN_COMPILER_VM_RT_RENAMED_LIB_NAME}")
        elseif(EXISTS "${PLUGIN_COMPILER_LIB_PATH}/${PLUGIN_COMPILER_VM_RT_LIB_NAME}")
            set(PLUGIN_COMPILER_VM_RT_LIB "${PLUGIN_COMPILER_LIB_PATH}/${PLUGIN_COMPILER_VM_RT_LIB_NAME}")
        else()
            message(FATAL_ERROR "VM runtime library not found in ${PLUGIN_COMPILER_LIB_PATH} (looked for ${PLUGIN_COMPILER_VM_RT_RENAMED_LIB_NAME} and ${PLUGIN_COMPILER_VM_RT_LIB_NAME})")
        endif()
        ...
        if(WIN32)
            ...
            # Match the PDB to whichever VM runtime name the archive shipped.
            if(EXISTS "${PLUGIN_COMPILER_PDB_PATH}/${PLUGIN_COMPILER_VM_RT_RENAMED_PDB_NAME}")
                set(PLUGIN_COMPILER_VM_RT_PDB "${PLUGIN_COMPILER_PDB_PATH}/${PLUGIN_COMPILER_VM_RT_RENAMED_PDB_NAME}")
            elseif(EXISTS "${PLUGIN_COMPILER_PDB_PATH}/${PLUGIN_COMPILER_VM_RT_PDB_NAME}")
                set(PLUGIN_COMPILER_VM_RT_PDB "${PLUGIN_COMPILER_PDB_PATH}/${PLUGIN_COMPILER_VM_RT_PDB_NAME}")
           else()
                 message(FATAL_ERROR "VM runtime PDB not found in ${PLUGIN_COMPILER_PDB_PATH} (looked for ${PLUGIN_COMPILER_VM_RT_RENAMED_PDB_NAME} and ${PLUGIN_COMPILER_VM_RT_PDB_NAME})")
            endif()
