...
void intel_npu::registerNPUWOptions(OptionsDesc& desc) {
    for_each_npuw_option<NPUWOptionGroup::ROOT>([&](auto tag) {
...
void intel_npu::registerNPUWLLMOptions(OptionsDesc& desc) {
    for_each_npuw_option<NPUWOptionGroup::LLM>([&](auto tag) {
        using Opt = typename decltype(tag)::type;
        desc.add<Opt>();
    });
}
...
void intel_npu::registerNPUWKokoroOptions(OptionsDesc& desc) {
    for_each_npuw_option<NPUWOptionGroup::KOKORO>([&](auto tag) {
        using Opt = typename decltype(tag)::type;
        desc.add<Opt>();
    });
}
