...
public:
    DiscretePipeline(const Config& config,
        ...
        for (const auto& desc : executor->inputs_desc_map()) {
            _deviceInputs.appendArgument(legacyNameToCurrentName.at(desc.first), desc.second.info);
        }
        _deviceInputs.allocate(device_handle, context);

        for (const auto& desc : executor->inputs_desc_map()) {
            const std::shared_ptr<ov::ITensor>& inputTensor = tensors.at(legacyNameToCurrentName.at(desc.first));
            ...
            _command_list[stage::UPLOAD].appendMemoryCopy(_deviceInputs.getDevicePtr(legacyNameToCurrentName.at(desc.first)), tensorBuffer, size);

            executor->setArgumentValue(desc.second.idx, _deviceInputs.getDevicePtr(legacyNameToCurrentName.at(desc.first)));
        }
        ...
        for (const auto& desc : executor->outputs_desc_map()) {
            _deviceOutputs.appendArgument(legacyNameToCurrentName.at(desc.first), desc.second.info);
        }
        _deviceOutputs.allocate(device_handle, context);

        for (const auto& desc : executor->outputs_desc_map()) {
            const std::shared_ptr<ov::ITensor>& outputTensor = tensors.at(legacyNameToCurrentName.at(desc.first));
            ...
            _command_list[stage::READBACK].appendMemoryCopy(tensorBuffer,
                                                            _deviceOutputs.getDevicePtr(legacyNameToCurrentName.at(desc.first)),
                                                            size);

            executor->setArgumentValue(desc.second.idx, _deviceOutputs.getDevicePtr(legacyNameToCurrentName.at(desc.first)));
        }
