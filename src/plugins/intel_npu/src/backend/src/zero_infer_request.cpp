...
void ZeroInferRequest::prepare_inputs() {
    OV_ITT_TASK_CHAIN(ZERO_INFER, itt::domains::LevelZeroBackend, "infer_async", "prepare_inputs");
    {
        ...
        if (!_pipelineIsCreated || _dynamicBatchValueChanged) {
            ...
            const bool log_profiling = _logger.isActive(ov::log::Level::INFO);
            const auto pipeline_start =
                log_profiling ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point{};
            ...
            if (log_profiling) {
                ...
                _logger.info("create_pipeline() took %.3f ms (first_creation=%d, dynamic_batch_changed=%d)",
