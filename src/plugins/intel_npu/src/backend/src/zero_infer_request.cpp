...
void ZeroInferRequest::create_pipeline() {
    ...
    for (size_t outputIndex = 0; outputIndex < _metadata.outputs.size(); ++outputIndex) {
        ...
        _levelZeroOutputTensors.at(outputIndex) =
            allocate_tensor_for_pipeline(_metadata.outputs.at(outputIndex), outputIndex, OUTPUT, *_outputAllocator, batchSize);
        ...
        for (size_t i = 0; i < tensors.size(); i++) {
            ...
            if (remoteTensor == nullptr) {
                ...
                if (!levelZeroTensorCreated) {
                    ...
                    get_level_zero_input(foundPort.idx, i) = allocate_tensor_for_pipeline(_metadata.inputs.at(foundPort.idx),
                                                                             foundPort.idx,
                                                                             true,
                                                                             *_inputAllocator,
                                                                             batchSize);
