...
ZeroInferRequest::ZeroInferRequest(const std::shared_ptr<ZeroInitStructsHolder>& initStructs,
    ...
    _logger.debug("ZeroInferRequest::ZeroInferRequest");
    ...
    _logger.debug("ZeroInferRequest::ZeroInferRequest completed");
}
...
void ZeroInferRequest::create_pipeline() {
    ...
    _logger.debug("ZeroInferRequest::create_pipeline - completed");
}
...
void ZeroInferRequest::set_tensor(const ov::Output<const ov::Node>& port, const ov::SoPtr<ov::ITensor>& tensor) {
    ...
    if (_initStructs->getMutableCommandListExtVersion() >= ZE_MAKE_VERSION(1, 0)) {
        ...
        std::string portName = port.get_node()->get_friendly_name();
        std::string modelName = std::dynamic_pointer_cast<const CompiledModel>(this->get_compiled_model())
                                    ->get_graph()
                                    ->get_metadata()
                                    .name;
        if (remoteTensor == nullptr) {
            _logger.debug("ZeroInferRequest::set_tensor - set new tensor with address %p and size %d for port %s with "
                          "shape %s = %d of model %s",
                          tensor->data(),
                          tensor->get_byte_size(),
                          portName.c_str(),
                          strShape.c_str(),
                          ov::shape_size(port.get_shape()) * port.get_element_type().size(),
                          modelName.c_str());
            set_tensor_data(tensor._ptr, foundPort.idx, foundPort.is_input());
        } else {
            _logger.debug("ZeroInferRequest::set_tensor - set new remote tensor with size %d for port %s with shape %s "
                          "= %d of model %s",
                          tensor->get_byte_size(),
                          portName.c_str(),
                          strShape.c_str(),
...
void ZeroInferRequest::infer_async() {
    ...
    // Sanity check - all tensors still in L0 context

    _logger.debug("Before _pipeline->push(), input tensors are:");
    for (const auto& batchedInputTensors : _levelZeroInputTensors) {
        for (const auto& inputTensor : batchedInputTensors) {
            if (!is_remote_tensor(inputTensor)) {
                OPENVINO_ASSERT(zeroUtils::memory_was_allocated_in_the_same_l0_context(_initStructs->getContext(),
                                                                                       inputTensor->data()),
                                "Input tensor ",
                                inputTensor->data(),
                                " was deallocated from L0 context!");
                ze_memory_allocation_properties_t desc = {};
                desc.stype = ZE_STRUCTURE_TYPE_MEMORY_ALLOCATION_PROPERTIES;
                auto res =
                    intel_npu::zeMemGetAllocProperties(_initStructs->getContext(), inputTensor->data(), &desc, nullptr);
                if (res == ZE_RESULT_SUCCESS) {
                    std::stringstream ss;
                    ss << inputTensor->get_shape();
                    if (desc.id) {
                        if (desc.type == ZE_MEMORY_TYPE_HOST) {
                            _logger.debug(
                                "\tAddress = %p, Shape = %s, Byte size = %d, id = %ld, type = ZE_MEMORY_TYPE_HOST",
                                inputTensor->data(),
                                ss.str().c_str(),
                                inputTensor->get_byte_size(),
                                desc.id);
                        } else if (desc.type == ZE_MEMORY_TYPE_DEVICE) {
                            _logger.debug(
                                "\tAddress = %p, Shape = %s, Byte size = %d, id = %ld, type = ZE_MEMORY_TYPE_DEVICE",
                                inputTensor->data(),
                                ss.str().c_str(),
                                inputTensor->get_byte_size(),
                                desc.id);
                        } else if (desc.type == ZE_MEMORY_TYPE_SHARED) {
                            _logger.debug(
                                "\tAddress = %p, Shape = %s, Byte size = %d, id = %ld, type = ZE_MEMORY_TYPE_SHARED",
                                inputTensor->data(),
                                ss.str().c_str(),
                                inputTensor->get_byte_size(),
                                desc.id);
                            ...
                            _logger.debug(
                                "\tAddress = %p, Shape = %s, Byte size = %d, id = %ld, type = ZE_MEMORY_TYPE_UNKNOWN",
                                inputTensor->data(),
                                ss.str().c_str(),
                                inputTensor->get_byte_size(),
                                desc.id);
                        ...
                        _logger.debug("\tTensor without ID: Address = %p, Shape = %s, Byte size = %d",
                                      inputTensor->data(),
                                      ss.str().c_str(),
                                      inputTensor->get_byte_size());
                    }
                ...
                size_t driverSize = 0;
                void* gpuVA = nullptr;
                res = intel_npu::zeMemGetAddressRange(_initStructs->getContext(),
                                                      inputTensor->data(),
                                                      &gpuVA,
                                                      &driverSize);
                if (res == ZE_RESULT_SUCCESS) {
                    _logger.debug("\tDriver size = %ld, GPU_VA = %p", driverSize, gpuVA);
                    if (inputTensor->get_byte_size() != driverSize) {
                        char op = inputTensor->get_byte_size() > driverSize ? '>' : '<';
                        _logger.debug("\tDriver size differs from actual tensor size: Tensor Size %c Driver Size", op);
                    }
                }
            }
        }
    ...
    for (const auto& outputTensor : _levelZeroOutputTensors) {
        if (!is_remote_tensor(outputTensor)) {
            OPENVINO_ASSERT(zeroUtils::memory_was_allocated_in_the_same_l0_context(_initStructs->getContext(),
                                                                                   outputTensor->data()),
                            "Output tensor ",
                            outputTensor->data(),
...
std::shared_ptr<ZeroTensor> ZeroInferRequest::create_tensor(ov::element::Type type,
    ...
    OPENVINO_ASSERT(allocator, "Allocator must be provided when creating a zero tensor!");
