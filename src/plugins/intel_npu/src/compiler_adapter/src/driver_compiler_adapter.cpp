...
/**
 * @brief Applies the common OV passes previously found in the compiler.
 *
 * @param model The target model.
 * @return A clone of the original model on which the passes have been applied.
 */
std::shared_ptr<ov::Model> applyCommonPasses(const std::shared_ptr<const ov::Model>& model) {
...
std::shared_ptr<IGraph> DriverCompilerAdapter::compile(const std::shared_ptr<const ov::Model>& model,
    ...
    auto serializedIR = serializeIR(modelAfterPasses, compilerVersion, maxOpsetVersion, applyCommonPassesFlag);
...
ov::SupportedOpsMap DriverCompilerAdapter::query(const std::shared_ptr<const ov::Model>& model,
    ...
    if (applyCommonPassesFlag) {
        ...
        _logger.info("Common OV passes have been applied inside the plugin");
    } else {
        _logger.info("No common OV passes have been applied inside the plugin");
    }
...
SerializedIR DriverCompilerAdapter::serializeIR(const std::shared_ptr<const ov::Model>& model,
                                                ...
                                                const bool commonPassesApplied) const {
    driver_compiler_utils::IRSerializer irSerializer(model, supportedOpsetVersion, commonPassesApplied);
