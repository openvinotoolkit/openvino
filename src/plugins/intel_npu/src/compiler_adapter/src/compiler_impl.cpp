...
struct vcl_allocator : vcl_allocator2_t {
    ...
    static uint8_t* allocate(vcl_allocator2_t* allocator, size_t size) {
        ...
        auto allocatedPtr = static_cast<uint8_t*>(aligned_alloc_util(intel_npu::utils::STANDARD_PAGE_SIZE, size));
        if (allocatedPtr == nullptr) {
            OPENVINO_THROW("Failed to allocate aligned memory for allocator");
        }
        m_allocated = allocatedPtr;
        m_size = alignedSize;
        return allocatedPtr;
    }
    ...
    static uint8_t* m_allocated;
    static size_t m_size;
};
size_t vcl_allocator::m_size = 0;
uint8_t* vcl_allocator::m_allocated = nullptr;
...
struct vcl_allocator_2 : vcl_allocator2_t {
    ...
    static uint8_t* allocate(vcl_allocator2_t* allocator, size_t size) {
        ...
        m_info.emplace_back(std::make_pair(allocatedPtr, alignedSize));
        return allocatedPtr;
    }
    ...
    static std::vector<std::pair<uint8_t*, size_t>> m_info;
};
std::vector<std::pair<uint8_t*, size_t>> vcl_allocator_2::m_info;
