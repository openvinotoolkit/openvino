...
void LevelZeroCompilerInDriver<TableExtension>::getNativeBinary(ze_graph_dditable_ext_curr_t& graphDdiTableExt,
    ...
    if (ZE_RESULT_SUCCESS != result) {
        _logger.warning("Failed to compile network. ");
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnGetNativeBinary get blob size", result, _graphDdiTableExt);
    }
    ...
    if (ZE_RESULT_SUCCESS != result) {
        _logger.warning("Failed to compile network. ");
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnGetNativeBinary get blob data", result, _graphDdiTableExt);
    }
...
ze_result_t LevelZeroCompilerInDriver<TableExtension>::seriazlideIRModelAndQueryNetworkCreateV1(
    ...
    if (ZE_RESULT_SUCCESS != result) {
        THROW_ON_FAIL_FOR_GRAPH_EXT("seriazlideIRModelAndQueryNetworkCreateV1", result, _graphDdiTableExt);
    }
    return result;
}
...
std::unordered_set<std::string> LevelZeroCompilerInDriver<TableExtension>::queryImpl(
    ...
    if (ZE_RESULT_SUCCESS != result) {
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnDeviceGetGraphProperties", result, _graphDdiTableExt);
    }
...
ze_result_t LevelZeroCompilerInDriver<TableExtension>::seriazlideIRModelAndQueryNetworkCreateV2(
    ...
    if (ZE_RESULT_SUCCESS != result) {
        THROW_ON_FAIL_FOR_GRAPH_EXT("seriazlideIRModelAndQueryNetworkCreateV2", result, _graphDdiTableExt);
    }
    return result;
}
...
std::unordered_set<std::string> LevelZeroCompilerInDriver<TableExtension>::getQueryResultFromSupportedLayers(
    ...
    if (ZE_RESULT_SUCCESS != result) {
        ...
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnQueryNetworkGetSupportedLayers get size of query result",
                                    result,
                                    _graphDdiTableExt);
    }
    ...
    if (ZE_RESULT_SUCCESS != result) {
        _graphDdiTableExt.pfnQueryNetworkDestroy(hGraphQueryNetwork);
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnQueryNetworkGetSupportedLayers get result data of query",
                                    result,
                                    _graphDdiTableExt);
    }
    ...
    if (ZE_RESULT_SUCCESS != result) {
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnQueryNetworkDestroy", result, _graphDdiTableExt);
    }
...
ze_result_t LevelZeroCompilerInDriver<TableExtension>::createGraph(const ze_graph_format_t& format,
    ...
    if (ZE_RESULT_SUCCESS != result) {
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnCreate", result, _graphDdiTableExt);
    }
    return result;
}
...
ze_result_t LevelZeroCompilerInDriver<TableExtension>::createGraph(const ze_graph_format_t& format,
    ...
    if (ZE_RESULT_SUCCESS != result) {
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnCreate2", result, _graphDdiTableExt);
    }
    return result;
}
template <typename TableExtension>
...
NetworkDescription LevelZeroCompilerInDriver<TableExtension>::compile(const std::shared_ptr<const ov::Model>& model,
    ...
    auto result = _graphDdiTableExt.pfnDeviceGetGraphProperties(_deviceHandle, &deviceGraphProperties);
    if (ZE_RESULT_SUCCESS != result) {
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnDeviceGetGraphProperties, Failed to compile network.",
                                    result,
                                    _graphDdiTableExt);
    }
...
void LevelZeroCompilerInDriver<TableExtension>::getMetadata(ze_graph_dditable_ext_curr_t& graphDdiTableExt,
    ...
    if (ZE_RESULT_SUCCESS != result) {
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnGetArgumentProperties3", result, _graphDdiTableExt);
    }
    switch (arg.type) {
...
void LevelZeroCompilerInDriver<TableExtension>::getMetadata(ze_graph_dditable_ext_curr_t& graphDdiTableExt,
    ...
    if (!isStateInputName(arg.name) && !isStateOutputName(arg.name) && !isShapeTensorName(arg.name)) {
        ...
        if (ZE_RESULT_SUCCESS != result) {
            THROW_ON_FAIL_FOR_GRAPH_EXT("pfnGraphGetArgumentMetadata", result, _graphDdiTableExt);
        }
...
NetworkMetadata LevelZeroCompilerInDriver<TableExtension>::getNetworkMeta(ze_graph_handle_t graphHandle) const {
    ...
    if (ZE_RESULT_SUCCESS != result) {
        THROW_ON_FAIL_FOR_GRAPH_EXT("pfnGetProperties", result, _graphDdiTableExt);
    }
