...
public:
    ...
    std::shared_ptr<BlobWriter> create_blob_writer() override {
        ...
        auto elfMainScheduleSection = std::dynamic_pointer_cast<ELFMainScheduleSection>(
            m_blob_reader.retrieve_first_section(KnownSectionType::ELF_MAIN_SCHEDULE));

        if (elfMainScheduleSection) {
            auto initSchedulesSection = std::dynamic_pointer_cast<ELFInitSchedulesSection>(
                m_blob_reader.retrieve_first_section(KnownSectionType::ELF_INIT_SCHEDULES));
            ...
            auto dynamicScheduleSection = std::dynamic_pointer_cast<DynamicScheduleSection>(
                m_blob_reader.retrieve_first_section(KnownSectionType::DYNAMIC_SCHEDULE));
...
private:
    ...
    void register_known_sections_and_evaluators(const std::shared_ptr<CompilerOptionSupportHelper>& option_helper) {
        ...
        m_blob_reader.register_reader(KnownSectionType::ELF_MAIN_SCHEDULE, ELFMainScheduleSection::read);
        m_blob_reader.register_reader(KnownSectionType::ELF_INIT_SCHEDULES, ELFInitSchedulesSection::read);
        m_blob_reader.register_reader(KnownSectionType::DYNAMIC_SCHEDULE, ELFInitSchedulesSection::read);
        m_blob_reader.register_reader(KnownSectionType::BATCH_SIZE, BatchSizeSection::read);
        m_blob_reader.register_reader(KnownSectionType::IO_LAYOUTS, IOLayoutsSection::read);
        m_blob_reader.register_reader(KnownSectionType::ENCRYPTED_SCHEDULES_FLAG, IOLayoutsSection::read);
        m_blob_reader.register_reader(KnownSectionType::COMPILER_VERSION, IOLayoutsSection::read);
        ...
        m_blob_reader.register_section_instance_evaluator(KnownSectionType::ELF_MAIN_SCHEDULE,
                                                          compiler_schedules_instance_evaluator);
        m_blob_reader.register_section_instance_evaluator(KnownSectionType::DYNAMIC_SCHEDULE,
                                                          compiler_schedules_instance_evaluator);
    }
    ...
    void verify_valid_sections() {
        const bool has_elf_main_schedule = m_blob_reader.has_section_of_type(KnownSectionType::ELF_MAIN_SCHEDULE);
        const bool has_init_schedules = m_blob_reader.has_section_of_type(KnownSectionType::ELF_INIT_SCHEDULES);
        const bool has_dynamic_schedule = m_blob_reader.has_section_of_type(KnownSectionType::DYNAMIC_SCHEDULE);
    ...
    void decrypt_schedules() override {
        const auto encrypted_schedules_flag_section = std::dynamic_pointer_cast<EncryptedSchedulesFlagSection>(
            m_blob_reader.retrieve_first_section(KnownSectionType::ENCRYPTED_SCHEDULES_FLAG));
    ...
    std::optional<int> extract_batch_size() const override {
        const auto batch_size_section = std::dynamic_pointer_cast<BatchSizeSection>(
            m_blob_reader.retrieve_first_section(KnownSectionType::BATCH_SIZE));
    ...
    std::optional<std::pair<std::vector<ov::Layout>, std::vector<ov::Layout>>> extract_layouts() const override {
        const auto io_layouts_section = std::dynamic_pointer_cast<IOLayoutsSection>(
            m_blob_reader.retrieve_first_section(KnownSectionType::IO_LAYOUTS));
    ...
    std::optional<uint32_t> extract_compiler_version() const override {
        const auto compiler_version_section = std::dynamic_pointer_cast<CompilerVersionSection>(
            m_blob_reader.retrieve_first_section(KnownSectionType::COMPILER_VERSION));
    ...
    std::optional<BlobType> extract_blob_type() const override {
        if (m_blob_reader.has_section_of_type(KnownSectionType::ELF_MAIN_SCHEDULE)) {
            return BlobType::ELF;
        }
