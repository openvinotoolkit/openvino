...
inline void try_register_simple_property(const ConfigLike& config,
 ...
 */
template <typename OptionType, typename ConfigLike>
inline void try_register_explicit_visibility_property(const ConfigLike& config,
                                                      PropertyMap& properties,
                                                      const std::string& propertyName,
                                                      bool isPublic) {
    if (!config.isAvailable(propertyName)) {
 ...
 *
 * Use this when a custom getter is required. Visibility and mutability are taken from the option descriptor.
 */
...
/**
 * @brief Conditionally register a metric property with a standard (Config-only) getter.
 *
...
/**
 * @brief Conditionally register a metric property whose getter requires additional AnyMap arguments.
 *
 * Registers the property only when @p shouldRegister is true.
