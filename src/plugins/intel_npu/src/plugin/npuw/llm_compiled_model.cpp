...
std::optional<NPUDesc> extract_npu_descriptor(const std::shared_ptr<const ov::IPlugin>& plugin,
    ...
    OPENVINO_ASSERT(core, "Plugin's core is nullptr!");
    const auto all_devices = core->get_property("NPU", ov::available_devices);
...
ov::npuw::LLMCompiledModel::LLMCompiledModel(const std::shared_ptr<ov::Model>& model,
    ...
    // Compile multiple generate model variants with different sizes
    compile_generate_model_variants(generate_model_variants, factory, plugin, generate_config);

    m_prefill_compiled =
        std::dynamic_pointer_cast<ov::npuw::ICompiledModel>(factory->create(prefill_model, plugin, prefill_config));
    ...
    if (lm_head_model) {
        ...
        m_lm_head_compiled =
            std::dynamic_pointer_cast<ov::npuw::ICompiledModel>(factory->create(lm_head_model, plugin, lm_head_config));
...
std::shared_ptr<ov::npuw::LLMCompiledModel> ov::npuw::LLMCompiledModel::import_model(
    ...
    if (!encrypted) {
        CompiledContext ctx(false, nullptr, nullptr);
        auto compiled_model = ov::npuw::LLMCompiledModel::deserialize(stream, factory, plugin, properties, ctx);
    ...
    if (is_weightless) {
        ...
        CompiledContext ctx(false, nullptr, nullptr);
        compiled_model = ov::npuw::LLMCompiledModel::deserialize(decrypted_stream, factory, plugin, properties, ctx);
    } else {
        CompiledContext ctx(true, nullptr, enc_callbacks.decrypt);
        compiled_model = ov::npuw::LLMCompiledModel::deserialize(stream, factory, plugin, properties, ctx);
    }
...
std::shared_ptr<ov::npuw::LLMCompiledModel> ov::npuw::LLMCompiledModel::deserialize(
    ...
    auto read_model_meta = [&](std::istream& model_stream) {
        ...
        if (is_shared_lm_head) {
            compiled->m_lm_head_compiled = factory->deserialize(model_stream, plugin, properties, enc_ctx);
        }
