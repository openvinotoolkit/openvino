    ...
    for (size_t i = 0; i < m_kvcache_sizes.size(); ++i) {
        ...
        if (m_swa_layout.enabled()) {
            LOG_DEBUG("[SWA] Applying sliding-window KV-cache reduction to generate variant (kv_size=" << kv_size
                                                                                                       << ").");
            ov::npuw::PatchSlidingWindowKVCache(m_swa_layout, kv_size, max_generation_token_len, axes)
                .run_on_model(generate_variant);
        }
...
ov::npuw::LLMCompiledModel::LLMCompiledModel(const std::shared_ptr<ov::Model>& model,
    ...
    if (m_swa_layout.enabled()) {
        ...
        ov::npuw::PatchSlidingWindowKVCache(m_swa_layout, m_kvcache_desc.max_prompt_size, prefill_input_size, axes)
            .run_on_model(prefill_model);
    }
