...
public:
    ...
    ScaledDotProductAttentionDecomposition() {
        auto pattern_node = opp::wrap_type<ov::op::v13::ScaledDotProductAttention>();
        ...
        auto m = std::make_shared<opp::Matcher>(pattern_node, "ScaledDotProductAttentionDecomposition");
...
void reshape_to_static(std::shared_ptr<ov::Model> model,
    ...
    for (const auto& input : model->inputs()) {
        ...
        if (input_name.find("input_ids") != std::string::npos) {
            ...
            new_shape[1] = lhs_seq_size; // sequence size
...
ov::npuw::LLMCompiledModel::LLMCompiledModel(const std::shared_ptr<ov::Model>& model,
    ...
    const uint32_t whisper_lhs_seq_size = m_cfg.get<::intel_npu::NPUW_WHISPER_LHS_SEQ_SIZE>();
    if (is_whisper) {
