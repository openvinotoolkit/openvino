...
class SharedVocabDetectingMatcher final : public ov::pass::MatcherPass {
public:
    OPENVINO_MATCHER_PASS_RTTI("ov::npuw::SharedVocabDetectingMatcher");

    SharedVocabDetectingMatcher() {
        const auto qweight = opp::wrap_type<ov::op::v0::Constant>();
        const auto qzerop = opp::wrap_type<ov::op::v0::Constant>();
        const auto qcoeff = opp::wrap_type<ov::op::v0::Constant>();
        const auto qweight_convert = opp::wrap_type<ov::op::v0::Convert>({qweight});
        ...
        const auto qscale = opp::wrap_type<ov::op::v1::Multiply>({qsub, qcoeff});
        const auto qconvert = opp::optional<ov::op::v0::Convert>({qscale});
        const auto gather = opp::wrap_type<ov::op::v8::Gather>({qconvert, opp::any_input(), opp::any_input()});

        auto callback = [=](opp::Matcher& matcher) {
            const auto& values = matcher.get_pattern_value_map();
            const auto weight = std::static_pointer_cast<ov::op::v0::Constant>(values.at(qweight).get_node_shared_ptr());
            const auto zerop = std::static_pointer_cast<ov::op::v0::Constant>(values.at(qzerop).get_node_shared_ptr());
            const auto scale = std::static_pointer_cast<ov::op::v0::Constant>(values.at(qcoeff).get_node_shared_ptr());
            const auto gather_shape = values.at(gather).get_shape();

            if (weight->get_element_type() != ov::element::u8 || zerop->get_element_type() != ov::element::u8 ||
                weight->get_shape().size() != 2 || zerop->get_shape().size() != 2 || scale->get_shape().size() != 2 ||
                scale->get_shape()[1] != 1 || weight->get_shape()[0] < weight->get_shape()[1] ||
                (gather_shape.size() != 3 && gather_shape.size() != 4)) {
                return false;
            }
        ...
        register_matcher(std::make_shared<opp::Matcher>(gather, "SharedVocabDetecting"), std::move(callback));
    }
};
...
ov::npuw::InsertVocabSub128::InsertVocabSub128() {
    ...
    add_matcher<SharedVocabDetectingMatcher>();
}

namespace {

struct QuantizedVocab {
    std::shared_ptr<ov::op::v0::Constant> weight;
    std::shared_ptr<ov::op::v0::Constant> zerop;
    std::shared_ptr<ov::op::v0::Constant> scale;
};

std::optional<QuantizedVocab> get_quantized_vocab(const ov::Output<ov::Node>& output) {
    ...
    auto subtract = ov::as_type_ptr<ov::op::v1::Subtract>(dequantized);
    if (!subtract) {
        return std::nullopt;
    }
    ...
    if (!weight || !zerop || weight->get_element_type() != ov::element::u8 ||
        ...
        zerop->get_shape().size() != 2 || scale->get_shape().size() != 2 || scale->get_shape()[1] != 1 ||
        zerop->get_shape()[0] != weight->get_shape()[0] || scale->get_shape()[0] != weight->get_shape()[0]) {
        return std::nullopt;
    }

    return QuantizedVocab{weight, zerop, scale};
}

bool same_storage(const std::shared_ptr<ov::op::v0::Constant>& lhs,
...
void log_vocab_names(const char* label, const QuantizedVocab& vocab) {
    LOG_WARN(label << " weight='" << vocab.weight->get_friendly_name() << "', zero_point='"
                  << vocab.zerop->get_friendly_name() << "', scale='" << vocab.scale->get_friendly_name() << "'");
}
...
bool ov::npuw::DetectVocabSharing::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    std::vector<QuantizedVocab> embedding_vocabs;
    std::vector<QuantizedVocab> lm_head_vocabs;
    for (const auto& node : model->get_ordered_ops()) {
        if (ov::is_type<ov::op::v8::Gather>(node)) {
            if (auto vocab = get_quantized_vocab(node->input_value(0))) {
            ...
            if (auto vocab = get_quantized_vocab(matmul->input_value(1))) {
    ...
    if (embedding_vocabs.empty() && lm_head_vocabs.empty()) {
        LOG_WARN("NPUW_LLM_VOCAB_ASYM_SHARED is enabled, but no quantized embedding Gather or LM-head MatMul "
                 "vocabulary was detected. Sub128 graph transformations are skipped.");
    } else if (embedding_vocabs.empty()) {
        LOG_WARN("NPUW_LLM_VOCAB_ASYM_SHARED is enabled, but no quantized embedding Gather vocabulary was detected. "
        ...
        LOG_WARN("NPUW_LLM_VOCAB_ASYM_SHARED is enabled, but no quantized LM-head MatMul vocabulary was detected. "
                 "Sub128 graph transformations are skipped.");
        log_vocab_names("Embedding candidate:", embedding_vocabs.front());
    } else {
        LOG_WARN("NPUW_LLM_VOCAB_ASYM_SHARED is enabled, but embedding and LM-head quantized vocabularies are "
