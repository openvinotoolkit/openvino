...
std::optional<PyramidValidationResult> validate_and_setup_pyramid_attention(const std::shared_ptr<ov::Model>& model) {
    ...
    auto extract_sequence_dims = [&](const std::shared_ptr<ov::Node>& concat_node,
        ...
        for (const auto& param : original_params) {
            const std::string param_name = param->get_friendly_name();
            const bool is_target_param =
                is_key ? static_cast<bool>(ov::npuw::util::isPastKeyValuesKey(param_name))
                       : static_cast<bool>(ov::npuw::util::isPastKeyValuesValue(param_name));
...
std::vector<SDPAPatternNodes> find_sdpa_pattern_nodes_internal(const std::shared_ptr<ov::Model>& model, bool findAll) {
    auto extract_past_kv_index = [](const std::string& name, bool key) -> std::optional<size_t> {
        const auto pattern = key ? std::regex(R"(past_key_values\.(\d+)\.key)")
                                 : std::regex(R"(past_key_values\.(\d+)\.value)");
        std::smatch match;
    ...
    for (auto input : model->inputs()) {
        ...
        auto past_k = extract_past_kv_index(input_name, true);
        auto past_v = extract_past_kv_index(input_name, false);
        if (past_k) {
