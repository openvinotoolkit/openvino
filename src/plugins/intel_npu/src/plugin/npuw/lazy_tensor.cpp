...
ov::Tensor Const::eval() const {
    ...
    if (!m_weights_path.empty() || m_handle_getter) {
        ...
        // Use handle_getter if available, otherwise use default mmap
        if (m_handle_getter) {
            ov::FileHandle handle = m_handle_getter();
...
void Const::read_weight(const ov::npuw::s11n::WeightsContext& ctx) {
    ...
    if (ctx.weights) {
        if (ctx.bf16_consts.find({m_offset, m_byte_size}) != ctx.bf16_consts.end()) {
            ...
            NPUW_ASSERT(!ctx.weights_path.empty() || ctx.handle_getter);
            // Just save weights_path for the eval() to call the actual mmap.
            m_weights_path = ctx.weights_path;
            // Also save handle_getter if available
            m_handle_getter = ctx.handle_getter;
        }
    } else {
