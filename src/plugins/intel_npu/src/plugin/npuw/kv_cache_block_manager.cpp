...
uint32_t KVCacheBlockManager::allocate_block_with_tensor(ov::SoPtr<ov::ITensor> tensor, uint32_t num_tokens) {
    OPENVINO_ASSERT(!free_block_ids_.empty(),
                    "KVCacheBlockManager: No free blocks available for prefix block injection");
    OPENVINO_ASSERT(num_tokens == block_size_,
                    "KVCacheBlockManager: prefix injection requires a full block, but num_tokens (",
                    num_tokens,
                    ") != block_size (",
                    block_size_,
                    ")");
    ...
    LOG_VERB("KVCacheBlockManager: Injected cached prefix tensor into block " << block_id << " (" << num_tokens
                                                                              << " tokens)");
    return block_id;
}
