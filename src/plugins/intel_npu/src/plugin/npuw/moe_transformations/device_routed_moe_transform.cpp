...
void collect_tiles(const std::shared_ptr<ov::Model>& model,
...
void collect_reshapes(const std::shared_ptr<ov::Model>& model, const std::string& layer_id, LayerNodes& nodes) {
    for (const auto& n : model->get_ordered_ops()) {
        ...
        if (nodes.num_experts > 0 && !shape_data.empty() && shape_data[0] == static_cast<int64_t>(nodes.num_experts)) {
            nodes.constant_reshapes.push_back(reshape);
        }
    }
}

void collect_matmuls(const std::shared_ptr<ov::Model>& model, const std::string& layer_id, LayerNodes& nodes) {
    for (const auto& n : model->get_ordered_ops()) {
...
void collect_adds(const std::shared_ptr<ov::Model>& model, const std::string& layer_id, LayerNodes& nodes) {
    for (const auto& n : model->get_ordered_ops()) {
...
void collect_transpose(const std::shared_ptr<ov::Model>& model, const std::string& layer_id, LayerNodes& nodes) {
    for (const auto& n : model->get_ordered_ops()) {
...
LayerNodes collect_layer_nodes(const std::shared_ptr<ov::Model>& model, const RouterInfo& router) {
    ...
    collect_tiles(model, router.layer_id, router.k_value, nodes);
    collect_reshapes(model, router.layer_id, nodes);
    collect_matmuls(model, router.layer_id, nodes);
    collect_adds(model, router.layer_id, nodes);
    collect_transpose(model, router.layer_id, nodes);
