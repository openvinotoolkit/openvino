...
private:
    ...
    // Host-side LongRoPE full-range cos/sin LUT data (see LongRopeHostLut,
    // pre_compute.hpp) used to populate the npuw_lr_full_cos/npuw_lr_full_sin
    // Parameters at runtime (see process_longrope() in llm_infer_request.cpp) -
    // only valid (is_valid() == true) when NPUW_LONGROPE_UNROTATED_KV is set and
    // the model matched LongRopePatternPhi_v5. One entry for prefill, one per
    // generate variant (parallel to m_kvcache_sizes - sizes can differ).
