...
void Partitioner::decompressionCutOff(const std::string& func_name) {
    ...
    LOG_DEBUG("Running the graph transformations...");
    {
        ...
        rewr.add_matcher<ov::npuw::patterns::SymmNoZP::DCOFFPassMatMul>(dcoff_mode,
                                                                        dcoff_type,
                                                                        enable_transpose,
                                                                        std::ref(params_to))
            ->build();
        rewr.add_matcher<ov::npuw::patterns::SymmNoZP::DCOFFPassGather>(dcoff_mode,
                                                                        dcoff_type,
                                                                        enable_transpose,
                                                                        std::ref(params_to))
            ->build();
        ...
        // Phi-3 4SymW16A/GPTQ
        rewr.add_matcher<ov::npuw::patterns::SymmZP::DCOFFPassReshape3>(dcoff_mode,
                                                                        dcoff_type,
                                                                        enable_transpose,
                                                                        std::ref(params_to));

        // Asymmetric zeropoints
        rewr.add_matcher<ov::npuw::patterns::AsymmZP::DCOFFPassReshape>(dcoff_mode,
                                                                        dcoff_type,
                                                                        enable_transpose,
                                                                        std::ref(params_to));
