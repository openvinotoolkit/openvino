...
std::optional<ov::npuw::online::Avoid> ov::npuw::online::util::parseAvoid(const std::string& s) {
    ...
    if (pos_col == std::string::npos || pos_sl == std::string::npos) {
        LOG_WARN("Incorect pattern in OPENVINO_NPUW_AVOID: " << s
                << ". Please, separate a device with / and pattern type with :, e.g. Op:Select/NPU,P:RMSNorm/NPU."
    ...
    if (type != "Op" && type != "P") {
        LOG_WARN("Incorect pattern type in OPENVINO_NPUW_AVOID: " << type << ". Please, use either Op for operation or P for pattern."
