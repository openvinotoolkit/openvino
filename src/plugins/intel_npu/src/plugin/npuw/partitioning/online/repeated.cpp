...
bool Interconnect::operator==(const Interconnect& other) const {
    return input_port == other.input_port &&
            output_port == other.output_port &&
            input_node == other.input_node &&
            output_node == other.output_node;
};

bool MetaInterconnect::operator==(const MetaInterconnect& other) const {
    return other.input_meta == input_meta &&
           other.output_meta == output_meta &&
           other.input_port == input_port &&
           other.output_port == output_port &&
           other.input_reptrack == input_reptrack &&
           other.output_reptrack == output_reptrack;
};

bool MetaInterconnect::operator<(const MetaInterconnect& other) const {
    return std::make_tuple(input_meta, input_port, input_reptrack,
                           output_port, output_meta, output_reptrack)
           < std::make_tuple(other.input_meta, other.input_port, other.input_reptrack,
                             other.output_port, other.output_meta, other.output_reptrack);
};
