...
ov::npuw::JustInferRequest::JustInferRequest(const std::shared_ptr<ov::npuw::CompiledModel>& compiled_model)
    ...
    // Fill weights bank if applicable
    if (!m_npuw_model->m_weights_bank_name.empty()) {
        // FIXME: mostly copied from unpack_closure()
        for (size_t i = 0; i < m_num_submodels; i++) {
            ...
            auto weights_bank = ov::npuw::weights::bank(m_npuw_model->m_weights_bank_name, *comp_model_desc.device_it);
            ...
            for (std::size_t cidx = 0u; cidx < comp_model_desc.closure.size(); cidx++) {
                ...
                if (closure.get_element_type() == clparam->get_element_type()) {
                    ...
                    comp_model_desc.unpack_required[cidx] = false;
                    closure = weights_bank->get(closure);
                } else {
                    // Utilize unpack
                    comp_model_desc.unpack_required[cidx] = true;
                }
            }  // for(closure)
            LOG_INFO("DONE");
        }
...
void ov::npuw::JustInferRequest::unpack_closure(std::size_t idx, RqPtr request) {
    ...
    for (std::size_t cidx = 0u; cidx < comp_model_desc.closure.size(); cidx++) {
        ...
        auto clparam = request->get_tensor(iport);
        if (comp_model_desc.unpack_required[cidx] && closure.get_element_type() != clparam->get_element_type()) {
