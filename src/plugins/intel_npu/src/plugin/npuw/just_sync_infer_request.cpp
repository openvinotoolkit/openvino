...
ov::npuw::JustInferRequest::JustInferRequest(const std::shared_ptr<ov::npuw::CompiledModel>& compiled_model)
        ...
        for (std::size_t i = 0; i < m_num_submodels; i++) {
            ...
            if (comp_model_desc.replaced_by) {                     // a function call..
                ...
                if (!prevs[real_id]) {                             // ..met for a first time
                    ...
                    LOG_INFO("Mark subgraph[" << i << "] as a successor of subraph["
                             << prev_id << "] in the function pipeline");
    ...
    for (size_t i = 0; i < m_npuw_model->inputs().size(); i++) {
        ...
        if (to_submodel != ov::npuw::CompiledModel::NO_LINK) {
            ...
            m_port_orig_to_sub[port] = get_real_subrequest(to_submodel.first)
                ->get_compiled_model()
                ->inputs()[to_submodel.second];
        } else {
            // Quick hack to support models with unused Parameters...
            m_port_to_subrequest_idx[port] = size_t{-1};
        }
    }
    ...
    for (auto &&it : m_npuw_model->m_param_subscribers) {
        ...
        for (auto &&to_submodel : it.second) {
            ...
            m_port_orig_to_sub[prim_port] = get_real_subrequest(to_submodel.first)
                ->get_compiled_model()
                ->inputs()[to_submodel.second];
        }
    }
    ...
    for (size_t i = 0; i < m_npuw_model->outputs().size(); i++) {
        ...
        LOG_INFO("Output " << i << " of " << m_npuw_model->outputs().size()
                 << ": " << port);
        ...
        m_port_to_subrequest_idx[port] = submodel_idx;
        m_port_orig_to_sub[port] = get_real_subrequest(from_submodel.first)
            ->get_compiled_model()
            ->outputs()[from_submodel.second];
...
void ov::npuw::JustInferRequest::connect_subrequests() {
    ...
    for (const auto& kvp : m_npuw_model->m_submodels_input_to_prev_output) {
        ...
        if (subm[subm_idx_from].replaced_by && subm[subm_idx_to].replaced_by) {
            ...
            const auto &iport  = m_subrequests[subm_idx_to]
                ->get_compiled_model()
                ->inputs()[port_idx_to];
            ...
            // stages.
            OPENVINO_THROW("FATAL: \"Prev. Output\" Request ", subm_idx_from,
                           " in input_to_prev_output mapping was optimized out,"
                           " but it consumer request ", subm_idx_to, " wasn't!");
            ...
            LOG_DEBUG(   "Set Subgraph[" << subm_idx_to   << "]/" << iport
...
void ov::npuw::JustInferRequest::bind_params_results() {
    ...
    for (size_t i = 0; i < m_npuw_model->inputs().size(); i++) {
        ...
        if (m_npuw_model->m_compiled_submodels[ptidx].replaced_by) {
            ...
            LOG_DEBUG("Subrequest[" << ptidx << "]: set parameter[" << port
                     << "] to global parameter [" << i << "] (direct)");
            subr->set_tensor(m_port_orig_to_sub.at(port),
                             m_port_to_tensor.at(port).tensor);
        }
    } // for(inputs)
    ...
    for (auto &&it : m_npuw_model->m_param_subscribers) {
        ...
        for (auto &&sit : it.second) {
            ...
            if (m_npuw_model->m_compiled_submodels[sub_idx].replaced_by) {
                ...
                const auto &sub_port     = m_subrequests[sub_idx]->
                    get_compiled_model()->inputs()[sub_param_idx];

                LOG_DEBUG("Subrequest[" << sub_idx << "]: set parameter[" << sub_param_idx
                         << "] to global parameter [" << param_idx << "] (subscriber)");
...
ov::npuw::IBaseInferRequest::RqPtr
ov::npuw::JustInferRequest::get_real_subrequest(std::size_t idx) {
    ...
    for (size_t i = 0; i < func_desc.param_base; i++) {
        ...
        if (link_iter != m_npuw_model->m_submodels_input_to_prev_output.end()) {
            ...
            if (!m_npuw_model->m_compiled_submodels[prod_idx].replaced_by) {
                ...
                const auto &oport = m_npuw_model->m_compiled_submodels[prod_idx]
                    .compiled_model
                    ->outputs()[prod_port];
    ...
    //    ov::parallel_for(closure_unpack_required.size(), [&](std::size_t j) {
        for (std::size_t j = 0; j != closure_unpack_required.size(); j++) {
            ...
            if (!comp_model_desc.scales.empty() && comp_model_desc.scales[cidx] && comp_model_desc.zerops[cidx]) {
                ...
                ov::npuw::util::unpack(ov::get_tensor_impl(closure), ov::get_tensor_impl(comp_model_desc.zerops[cidx]),
                                       ov::get_tensor_impl(comp_model_desc.scales[cidx]), clparam);
            } else if (!comp_model_desc.scales.empty() && comp_model_desc.scales[cidx]) {
                // Unpacking this weight requires scaling
                ov::npuw::util::unpack(ov::get_tensor_impl(closure), ov::get_tensor_impl(comp_model_desc.scales[cidx]),
                                       clparam);
            } else {
