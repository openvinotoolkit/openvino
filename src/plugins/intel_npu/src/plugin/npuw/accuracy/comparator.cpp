...
ov::npuw::metrics::Result
ov::npuw::metrics::NRMSE::operator()(const ov::SoPtr<ov::ITensor>& actual,
    ...
    if (rmse < 0.0) {
        LOG_WARN("Calculated RMSE metric is < 0.0, what is unexpected. Please treat tensors as "
                 "unequal.");
        result.metric = rmse;
        return result;
    }
