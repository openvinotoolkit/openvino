...
public:
    ...
    DecomposeDynamicQuantize() {
        ...
        auto callback = [=](opp::Matcher& m) {
            auto& node_to_output = m.get_pattern_value_map();
            auto dq_ptr = ov::as_type_ptr<ov::op::internal::DynamicQuantize>(
                node_to_output.at(dynamic_quantize).get_node_shared_ptr());
            if (!dq_ptr) {
                return false;
            }
            ...
            const auto& attrs = dq_ptr->get_attrs();
            const auto has_zp = dq_ptr->outputs().size() == 3;

            auto set_friendly_name = [&dq_ptr](const std::shared_ptr<ov::Node>& node, const std::string& name) {
                node->set_friendly_name(dq_ptr->get_friendly_name() + "/" + name);
            };

            auto dq_input = dq_ptr->input_value(0);
            if (dq_input.get_element_type() != ov::element::f32) {
                auto fp32_input = std::make_shared<ov::op::v0::Convert>(dq_input, ov::element::f32);
                set_friendly_name(fp32_input, "InputConvert");
                dq_input = fp32_input;
            }

            const auto rank = dq_input.get_partial_shape().rank();
            OPENVINO_ASSERT(rank.is_static(),
                            "DynamicQuantize decomposition expects static rank input for ",
                            dq_ptr->get_friendly_name());

            std::vector<int64_t> axes(rank.get_length());
            std::iota(axes.begin(), axes.end(), 0);
            auto reduce_axes = ov::op::v0::Constant::create(ov::element::i64, ov::Shape{axes.size()}, axes);

            const auto quant_range_min = ov::op::v0::Constant::create(ov::element::f32, ov::Shape{}, {0.0f});
            const auto quant_range_max = ov::op::v0::Constant::create(ov::element::f32, ov::Shape{}, {255.0f});
            const auto quant_range_span = std::make_shared<ov::op::v1::Subtract>(quant_range_max, quant_range_min);
            set_friendly_name(quant_range_span, "QuantRangeSpan");

            auto reduce_min = std::make_shared<ov::op::v1::ReduceMin>(dq_input, reduce_axes, false);
            auto reduce_max = std::make_shared<ov::op::v1::ReduceMax>(dq_input, reduce_axes, false);
            set_friendly_name(reduce_min, "ReduceMin");
            set_friendly_name(reduce_max, "ReduceMax");

            const auto zero_node = ov::op::v0::Constant::create(ov::element::f32, ov::Shape{}, {0.0f});
            auto x_min = std::make_shared<ov::op::v1::Minimum>(zero_node, reduce_min);
            auto x_max = std::make_shared<ov::op::v1::Maximum>(zero_node, reduce_max);
            set_friendly_name(x_min, "Minimum");
            set_friendly_name(x_max, "Maximum");

            auto x_span = std::make_shared<ov::op::v1::Subtract>(x_max, x_min);
            auto y_scale = std::make_shared<ov::op::v1::Divide>(x_span, quant_range_max);
            auto x_min_shifted = std::make_shared<ov::op::v1::Subtract>(quant_range_min, x_min);
            set_friendly_name(x_span, "SubtractSpan");
            set_friendly_name(y_scale, "Scale");
            set_friendly_name(x_min_shifted, "MinShifted");

            ov::Output<ov::Node> output_scale = y_scale;
            if (attrs.scale_dt.is_static() && attrs.scale_dt != ov::element::f32) {
                auto converted_scale = std::make_shared<ov::op::v0::Convert>(y_scale, attrs.scale_dt);
                set_friendly_name(converted_scale, "ScaleConvert");
                output_scale = converted_scale;
            }

            auto intermediate_zero_point = std::make_shared<ov::op::v5::Round>(
                std::make_shared<ov::op::v1::Divide>(x_min_shifted, y_scale),
                ov::op::v5::Round::RoundMode::HALF_TO_EVEN);
            auto clamped_zero_point = std::make_shared<ov::op::v0::Clamp>(intermediate_zero_point, 0.0f, 255.0f);
            auto zero_point_u8 = std::make_shared<ov::op::v0::Convert>(clamped_zero_point, ov::element::u8);
            set_friendly_name(intermediate_zero_point, "ZeroPointRound");
            set_friendly_name(clamped_zero_point, "ZeroPointClamp");
            set_friendly_name(zero_point_u8, "ZeroPointU8");

            auto scaled_input = std::make_shared<ov::op::v1::Divide>(
                std::make_shared<ov::op::v1::Multiply>(dq_input, quant_range_span),
                x_span);
            auto rounded_input = std::make_shared<ov::op::v5::Round>(scaled_input, ov::op::v5::Round::RoundMode::HALF_TO_EVEN);
            auto shifted_input = std::make_shared<ov::op::v1::Add>(rounded_input, clamped_zero_point);
            auto clamped_output = std::make_shared<ov::op::v0::Clamp>(shifted_input, 0.0f, 255.0f);
            auto quantized_output_u8 = std::make_shared<ov::op::v0::Convert>(clamped_output, ov::element::u8);
            set_friendly_name(scaled_input, "ScaledInput");
            set_friendly_name(rounded_input, "RoundedInput");
            set_friendly_name(shifted_input, "ShiftedInput");
            set_friendly_name(clamped_output, "OutputClamp");
            set_friendly_name(quantized_output_u8, "OutputU8");

            ov::Output<ov::Node> output_quant = quantized_output_u8;
            if (attrs.quantization_dt.is_static() && attrs.quantization_dt != ov::element::u8) {
                auto converted_output = std::make_shared<ov::op::v0::Convert>(quantized_output_u8, attrs.quantization_dt);
                set_friendly_name(converted_output, "OutputConvert");
                output_quant = converted_output;
            }

            dq_ptr->output(0).replace(output_quant);
            dq_ptr->output(1).replace(output_scale);

            if (has_zp) {
                ov::Output<ov::Node> output_zp = zero_point_u8;
                if (attrs.zp_dt.is_static() && attrs.zp_dt != ov::element::u8) {
                    auto converted_zp = std::make_shared<ov::op::v0::Convert>(zero_point_u8, attrs.zp_dt);
                    set_friendly_name(converted_zp, "ZeroPointConvert");
                    output_zp = converted_zp;
                }
                dq_ptr->output(2).replace(output_zp);
            }
...
void ov::npuw::run_kv_cache_dynamic_qantization_passes(const std::shared_ptr<ov::Model>& model, ov::element::Type kv_cache_precision_hint) {
    ...
    if (kv_cache_precision_hint != ov::element::i8 && kv_cache_precision_hint != ov::element::u8) {
        LOG_WARN("KV-cache compression is currently implemented for i8/u8 host tensors, got: "
                 << kv_cache_precision_hint);
    }

    const ov::PartialShape scalar_shape{};
    ...
    auto create_dequant_nodes = [&create_parameter_with_name,
                     &make_name,
                     &qt_type,
                     &scalar_shape,
                     &kv_cache_precision_hint]
            ...
            std::shared_ptr<ov::Node> fp_input = start_node;
            if (start_node->get_output_element_type(0) != ov::element::f32) {
                fp_input = std::make_shared<ov::op::v0::Convert>(start_node, ov::element::f32);
                fp_input->set_friendly_name(make_dq_name("input_convert"));
            }

            std::shared_ptr<ov::Node> fp_subtracted_zp = fp_input;
            if (qt_type == ov::op::internal::DynamicQuantize::QuantizationType::Asymmetric) {
                ...
                auto zp = create_parameter_with_name(kv_cache_precision_hint, scalar_shape, make_dq_param_name("zp"));
                ...
                fp_subtracted_zp = std::make_shared<ov::op::v1::Subtract>(fp_input, converted_zp);
                fp_subtracted_zp->set_friendly_name(make_dq_name("zp_sub"));
            }

            // Multiply by scale
            auto fp_scale = create_parameter_with_name(ov::element::f32, scalar_shape, make_dq_param_name("scale"));
    ...
    for (const auto &result : original_results) {
       ...
       std::vector<uint64_t> shape_group_size(rank, std::numeric_limits<uint64_t>::max());

        ov::op::internal::DynamicQuantize::Attributes config;
       config.quantization_dt = kv_cache_precision_hint;
        ...
        if (qt_type == ov::op::internal::DynamicQuantize::QuantizationType::Asymmetric) {
           ...
           config.zp_dt = kv_cache_precision_hint;
         }
