...
ov::npuw::LLMInferRequest::LLMInferRequest(const std::shared_ptr<ov::npuw::LLMCompiledModel>& compiled_model)
    ...
    auto input_ids_port = ov::npuw::util::find_port_by_name(compiled_model->m_prefill_compiled->inputs(), "input_ids");
    if (input_ids_port.has_value()) {
        m_input_ids_name = "input_ids";
    } else {
        OPENVINO_ASSERT(ov::npuw::util::find_port_by_name(compiled_model->m_prefill_compiled->inputs(), "inputs_embeds").has_value());
...
void ov::npuw::LLMInferRequest::prepare_for_new_conversation() {
    ...
    ov::npuw::util::fill_tensor<int64_t>(m_prefill_request->get_tensor(m_prefill_in_ports.at("position_ids")), 0);
...
void ov::npuw::LLMInferRequest::infer_prefill_in_chunk(ov::SoPtr<ov::ITensor> input_ids,
    ...
    while (remaining_prompts > 0) {
        ...
        for (std::size_t i = kStartOutputKVCacheLayers; i < prefill_compiled->outputs().size(); ++i) {
            ...
            auto chunk_prefill_past_kv_in_tensor = m_prefill_request->get_tensor(m_prefill_in_ports.at(input_name));
            const auto& kv_dim = (output_name.find("value") != std::string::npos && kvcache_desc.v_tensors_transposed)
                                     ? 3u
                                     : kvcache_desc.dim;
            auto chunk_prefill_past_kv_in_slice =
                ov::npuw::util::make_tensor_slice(chunk_prefill_past_kv_in_tensor,
                                                  kv_dim,
                                                  kvcache_desc.num_stored_tokens - static_cast<uint32_t>(current_prompts_len),
                                                  kvcache_desc.num_stored_tokens);
            auto chunk_prefill_present_kv_out_tensor =
                m_prefill_request->get_tensor(m_prefill_out_ports.at(output_name));
            ov::npuw::util::update_tensor_by_dim(chunk_prefill_present_kv_out_tensor, chunk_prefill_past_kv_in_slice, kv_dim);
        }
...
void ov::npuw::LLMInferRequest::infer_generate(ov::SoPtr<ov::ITensor> input_ids,
        ...
        for (std::size_t i = kStartOutputKVCacheLayers; i < kvcache_compiled->outputs().size(); ++i) {
            ...
            if (use_chunk_prefill) {
                ...
                if (tokens_in_past_chunks > 0) {
                    ...
                    ov::npuw::util::update_tensor_by_dim(prefill_past_kv_chunks, kvcache_past_kv_chunks, kv_dim);
                }
                ...
                ov::npuw::util::update_tensor_by_dim(prefill_present_kv_chunk, kvcache_last_kv_chunk, kv_dim);
            } else {
                ...
                ov::npuw::util::update_tensor_by_dim(prefill_out_slice, kvcache_in_slice, kv_dim);
            }
        }
    ...
    for (std::size_t i = kStartOutputKVCacheLayers; i < kvcache_compiled->outputs().size(); ++i) {
        ...
        if (kv_dim == 3u) {
            ...
            ov::npuw::util::copy_by_planes(kvcache_out_tensor, kvcache_in_slice);
        } else {
...
void ov::npuw::LLMInferRequest::infer() {
    ...
    auto attention_mask = get_tensor(ov::npuw::util::find_port_by_name(inputs, "attention_mask").value());
    auto position_ids = get_tensor(ov::npuw::util::find_port_by_name(inputs, "position_ids").value());
