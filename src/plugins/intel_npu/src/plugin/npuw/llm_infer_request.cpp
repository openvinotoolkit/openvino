    ...
    for (auto state : m_variableStates) {
        ...
        if (state_tensor->get_size() == 0) {
            ...
            auto fill_lora_in_tensor = [low_rank_dim, state_tensor_rank](ov::SoPtr<ov::ITensor> state_tensor,
                ...
                auto new_tensor_slice = util::make_tensor_slice(infer_tensor, low_rank_dim, 0u, state_tensor_rank);
                if (low_rank_dim == 1) {
    ...
    ov::parallel_for(kvcache_compiled->outputs().size() - layer_ids::kStartOutputKVCacheLayers, [&](size_t out_idx) {
        ...
        if (use_chunk_prefill) {
            ...
            if (tokens_in_past_chunks > 0) {
                ...
                auto prefill_past_kv_chunks = util::make_tensor_slice(prefill_past_kv,
                                                                      pre_kv_dim,
                                                                      0u,
                                                                      static_cast<uint32_t>(tokens_in_past_chunks));

                auto kvcache_past_kv_chunks = util::make_tensor_slice(kvcache_in_tensor,
                                                                      gen_kv_dim,
                                                                      0u,
                                                                      static_cast<uint32_t>(tokens_in_past_chunks));

                util::copy_tensor_by_dim(prefill_past_kv_chunks, kvcache_past_kv_chunks, pre_kv_dim, gen_kv_dim);
            }

            // Copy part 2 KV results
            auto prefill_present_kv_chunk =
                util::make_tensor_slice(prefill_out_tensor,
                                        pre_kv_dim,
                                        static_cast<uint32_t>(prefill_chunk_size - m_tokens_in_present_chunk),
                                        static_cast<uint32_t>(prefill_chunk_size));

            auto kvcache_last_kv_chunk = util::make_tensor_slice(kvcache_in_tensor,
                                                                 gen_kv_dim,
                                                                 static_cast<uint32_t>(tokens_in_past_chunks),
                                                                 kvcache_desc.num_stored_tokens);

            util::copy_tensor_by_dim(prefill_present_kv_chunk, kvcache_last_kv_chunk, pre_kv_dim, gen_kv_dim);
        } else {
            ...
            auto kvcache_in_slice =
                util::make_tensor_slice(kvcache_in_tensor, gen_kv_dim, 0u, kvcache_desc.num_stored_tokens);

            util::copy_tensor_by_dim(prefill_out_slice, kvcache_in_slice, pre_kv_dim, gen_kv_dim);
        }
    });
    LOG_DEBUG("Done.");
}
...
void ov::npuw::LLMInferRequest::update_kvcache_for(
    ...
    for (std::size_t i = layer_ids::kStartOutputKVCacheLayers; i < compiled->outputs().size(); ++i) {
        ...
        auto dst_slice = util::make_tensor_slice(dst_tensor,
                                                 kv_dim,
        ...
        if (src_seq_len > num_tokens) {
            auto src_slice = util::make_tensor_slice(src_tensor, kv_dim, src_seq_len - num_tokens, src_seq_len);
            util::copy_tensor_by_dim(src_slice, dst_slice, kv_dim, kv_dim);
        } else {
            util::copy_tensor_by_dim(src_tensor, dst_slice, kv_dim, kv_dim);
        }
    }
    LOG_DEBUG("Done.");
}
...
void ov::npuw::LLMInferRequest::infer_chunked_prefill(ov::SoPtr<ov::ITensor> input_ids,
    ...
    while (remaining_prompts > 0) {
        ...
        auto actual_position_ids_slice =
            util::make_tensor_slice(position_ids,
