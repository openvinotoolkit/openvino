...
void ov::npuw::LLMInferRequest::apply_lora() {
    ...
    for (auto state : m_variableStates) {
        ...
        if (state_tensor->get_size() == 0) {
            ...
            ov::npuw::util::fill_tensor<float>(new_tensor_for_infer, 0.0f);
            ...
            if (state_tensor_rank == target_lora_rank) {
                ...
                ov::npuw::util::fill_tensor<float>(new_tensor_for_infer, 0.0f);
                auto new_tensor_slice = ov::npuw::util::make_tensor_slice(new_tensor_for_infer, low_rank_dim, 0u, state_tensor_rank);
                if (low_rank_dim == 1) {
