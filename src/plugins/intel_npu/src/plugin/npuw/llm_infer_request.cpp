    ...
    for (size_t i = 0; i < variants.size(); ++i) {
        ...
        for (const auto& input_port : async_req->get_compiled_model()->inputs()) {
            for (const auto& name : input_port.get_names()) {
                auto res = variant_in_ports.emplace(name, input_port);
        ...
        for (const auto& output_port : async_req->get_compiled_model()->outputs()) {
            for (const auto& name : output_port.get_names()) {
                auto res = variant_out_ports.emplace(name, output_port);
    ...
    while (remaining_prompts > 0) {
        ...
        if (prefill_pyramid_active && is_tail_chunk && current_prompts_len < base_chunk_len) {
            ...
            if (tail_req != m_prefill_requests.back()) {
                ...
                process_longrope_tables(tail_req,
                                        m_prefill_variant_in_ports.at(tail_req),
