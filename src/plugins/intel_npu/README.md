...
| `ov::intel_npu::max_tiles`/</br>`NPU_MAX_TILES` | RO | Maximum number of tiles supported by the device we compile for. It will be populated by driver, if present. Will not be listed as supported for offline compilation.| `[0-]` | `[1-6] depends on npu platform` |
...
`ov::intel_npu::max_tiles` is a read-only property, for offline compilation users will still be able to set ``ov::intel_npu::tiles``.  
