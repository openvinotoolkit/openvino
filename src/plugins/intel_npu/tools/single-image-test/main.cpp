...
using TensorMap = std::map<size_t, ov::Tensor>;
...
using TensorDescriptorMap = std::unordered_map<size_t, TensorDescriptor>;
using LayoutMap = std::unordered_map<size_t, ov::Layout>;
...
std::pair<TensorMap, ProfVec> runInfer(ov::InferRequest& inferRequest,
    ...
    for (const auto& [tensorName, tensor] : inputs) {
        inferRequest.set_input_tensor(tensorName, tensor);
    }
    ...
    for (const auto& outputInfo : compiledModel.outputs()) {
        out.insert({outputInfo.get_index(), inferRequest.get_tensor(outputInfo)});
    }
    ...
    try {
        ...
        for (size_t numberOfTestCase = 0; numberOfTestCase < inputFilesPerCase.size(); ++numberOfTestCase) {
            ...
            for (const auto& inputInfo : inputsInfo) {
                ...
                inputDescriptors.emplace(inputInfo.get_index(),
                ...
                inTensors.emplace(inputInfo.get_index(), std::move(tensor));
            }
            ...
            if (FLAGS_run_test) {
                ...
                for (const auto& out : compiledModel.outputs()) {
                    const auto& tensorName = out.get_index();
                ...
                for (const auto& out : compiledModel.outputs()) {
                    const auto& tensor = outputTensors.at(out.get_index());
                    std::ostringstream ostr;
