    ...
    for model_input in model.graph.input:
        ...
        if model_input_name in preprocess_model_data.preproc_per_io.keys():
            ...
            if "element_type" in preprocess_model_data.preproc_per_io[model_input_name].keys():
                tensor_type = model_input.type.tensor_type
                tensor_type.elem_type = onnx.helper.np_dtype_to_tensor_dtype(
                    np.dtype(getattr(np, preprocess_model_data.preproc_per_io[model_input_name]["element_type"]))
                )
                ...
                print(
                    f"setting 'element_type': {tensor_type.elem_type} is unsupported for ONNX models (probably we should traverse entire gpraph and overwrite 'elem_type' in each node)"
                )
    return model
...
class CPUExecutionProvider(Provider):
    ...
    def create_model(self, preprocessing_request_data, provider_config: Config) -> Provider:
        ...
        external_provider_dict_options = OnnxContextBase.get_provider_config(CPUExecutionProvider.name(), provider_config.cfg_dict)
    ...
    for model_input in model.graph.input:
        ...
        if model_input_name in preprocess_model_data.preproc_per_io.keys():
            ...
            if "element_type" in preprocess_model_data.preproc_per_io[model_input_name].keys():
                ...
                # raise RuntimeError("setting 'element_type' is unsupported for ONNX models (probably we should traverse entire gpraph and overwrite 'elem_type' in each node)")
                print(
                    "setting 'element_type' is unsupported for ONNX models (probably we should traverse entire gpraph and overwrite 'elem_type' in each node)"
                )
...
class OVEPCPU(CPUExecutionProvider):
    ...
    def create_model(self, preprocessing_request_data, provider_config: Config) -> Provider:
        ...
        external_provider_dict_options = OnnxContextBase.get_provider_config(OVEPCPU.name(), provider_config.cfg_dict)
...
class OVEPNPU(CPUExecutionProvider):
    ...
    def create_model(self, preprocessing_request_data, provider_config: Config) -> Provider:
        ...
        external_provider_dict_options = OnnxContextBase.get_provider_config(OVEPNPU.name(), provider_config.cfg_dict)
        if "device_type" in external_provider_dict_options.keys():
            raise RuntimeError(f'ERROR: Provider options conflict: "device_type" must not be specified in options: {external_provider_dict_options}')
...
class ONNXContext(Context):
    ...
    def __init__(self, provider_name):
        ...
        if prefix != "onnx":
            raise RuntimeError(f'Incorrect prefix: {prefix} of the provider name: {provider_name} - "onnx" expected.')

        self.provider_fabric, device = ONNXContext.registered_providers.get_provider_by_name(provider_name_specific)
