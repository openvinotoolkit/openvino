        ...
        for field_to_check in ["shape", "element_type", "files"]:
            if field_to_check not in output_data.keys():
                provider_result["status"].append(
                    f'Deserialized tensors info of provider: "{provider_name}" doesn\'t contain important field "{field_to_check}": {output_data}. Result is malformed'
                )
...
def get_comparison_profiver_output_files_result_data(ref_provider_output_blob_file_data, provider_to_cmp_output_blob_file_data, datatype):
    provider_to_cmp_output_blob_file_data["std_correlation"] = bin_diff.compare_blobs(
        ref_provider_output_blob_file_data["path"], provider_to_cmp_output_blob_file_data["path"], datatype
    )
...
def get_comparison_provider_output_result_data(ref_provider_name, ref_provider_output_data, provider_to_cmp_name, provider_to_cmp_output_data):
    for field_to_compare in ["shape", "element_type"]:
        if ref_provider_output_data[field_to_compare] != provider_to_cmp_output_data[field_to_compare]:
            provider_to_cmp_output_data["status"].append(
                f'Incorrect "{field_to_compare}": {provider_to_cmp_output_data[field_to_compare]}, expected: {ref_provider_output_data[field_to_compare]}'
            )
            ref_provider_output_data["status"].append(
                f'Incorrect "{field_to_compare}": {ref_provider_output_data[field_to_compare]}, expected: {provider_to_cmp_output_data[field_to_compare]}'
            )
    ...
    for f in ref_provider_output_data["files"]:
        if f in provider_to_cmp_output_data["data"].keys():
            try:
                provider_to_cmp_output_data["data"][f] = get_comparison_profiver_output_files_result_data(
                    ref_provider_output_data["data"][f], provider_to_cmp_output_data["data"][f], ref_provider_output_data["element_type"]
                )
...
def get_comparison_provider_result_data(ref_provider_name, ref_provider_result_data, provider_to_cmp_name, provider_to_cmp_result_data):
    for ref_output_name in ref_provider_result_data["outputs"]:
        ...
        if ref_output_name in provider_to_cmp_result_data["data"].keys():
            provider_to_cmp_result_data["data"][ref_output_name] = get_comparison_provider_output_result_data(
                ref_provider_name, ref_provider_result_data["data"][ref_output_name], provider_to_cmp_name, provider_to_cmp_result_data["data"][ref_output_name]
            )
        else:
...
As necessary input arguments, it requires both a name of a reference provider and names of providers to compare to that reference.""",
    ...
    parser.add_argument("providers_to_compare", nargs="+", help="An inference provider names to compare to the reference provider", default=None)
