...
def calculate_nrmse(lhs_normalized_bin_array, rhs_normalized_bin_array):
    ...
    return float(1 - np.sqrt(np.sum(diff_img * diff_img) / (len(lhs_normalized_bin_array.flatten()))) / (max(0.001, max(0, (np.max(lhs_normalized_bin_array))) - min(0, np.min(lhs_normalized_bin_array)), max(0, (np.max(rhs_normalized_bin_array))) - min(0, np.min(rhs_normalized_bin_array)))))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="nrmse-calculator",
                                     description='''
