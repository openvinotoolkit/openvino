...
class ArchTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.evaluable = get_evaluable_architecture(
            project_dir, project_dir, exclusions=["*.git*", "*__pycache__*", "*venv", "*.venv*", "*test*"], exclude_external_libraries=False
        )
    ...
    def test_no_openvino_package_in_onnx_provider(self):
        rule = Rule().modules_that().are_sub_modules_of(f"{self.project_name}.providers.onnx").should_not().import_modules_that().are_named(["openvino"])
    ...
    def test_no_onnx_package_in_ov_provider(self):
        rule = Rule().modules_that().are_sub_modules_of(f"{self.project_name}.providers.ov").should_not().import_modules_that().have_name_matching(r".*onnx.*")
        rule.assert_applies(self.evaluable)
    ...
    def test_low_level_do_not_depend_on_high_level(self):
        rule = (
            ...
            .have_name_matching([f"{self.project_name}.providers.interfaces", self.project_name + r"\.common\..*"])
        )
        rule.assert_applies(self.evaluable)
