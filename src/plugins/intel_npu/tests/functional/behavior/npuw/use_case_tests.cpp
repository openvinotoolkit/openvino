...
std::shared_ptr<ov::Model> NPUWUseCaseTests::create_example_model() {
...
TEST_F(NPUWUseCaseTests, TestInfrastructureIsCorrect) {
...
TEST_F(NPUWUseCaseTests, CompilationIsSuccessful) {
    // Set expectations first:
...
TEST_F(NPUWUseCaseTests, CompilationIsFailSafe) {
    // Set expectations first:
    {
        InSequence s;
...
TEST_F(NPUWUseCaseTests, CompilationIsFailed) {
    // Set expectations first:
    EXPECT_CALL(*npu_plugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _))
            .Times(1)
            .WillOnce(Throw(std::runtime_error("Compilation on MockNPU is failed")));
...
TEST_F(NPUWUseCaseTests, InferRequestCreationIsSuccessful) {
    // Set expectations first:
...
TEST_F(NPUWUseCaseTests, InferRequestCreationIsFailSafe) {
    // Set expectations first:
    {
        InSequence s;
        EXPECT_CALL(*npu_plugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _)).Times(1);
...
TEST_F(NPUWUseCaseTests, InferRequestCreationIsFailed) {
    // Set expectations first:
    EXPECT_CALL(*npu_plugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _)).Times(1);
    EXPECT_CALL(*cpu_plugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _)).Times(0);
...
TEST_F(NPUWUseCaseTests, InferIsSuccessful) {
    // Set expectations first:
    EXPECT_CALL(*npu_plugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _)).Times(1);
    EXPECT_CALL(*cpu_plugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _)).Times(0);
    npu_plugin->set_expectations_to_comp_models(0, [](MockCompiledModel& model) {
...
TEST_F(NPUWUseCaseTests, InferIsFailSafe) {
    // Set expectations first:
    {
        InSequence seq;
        EXPECT_CALL(*npu_plugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _)).Times(1);
...
TEST_F(NPUWUseCaseTests, InferIsFailed) {
    // Set expectations first:
    {
        InSequence seq;
        EXPECT_CALL(*npu_plugin, compile_model(A<const std::shared_ptr<const ov::Model>&>(), _)).Times(1);
...
using NPUWUseCaseOnlinePartTests = NPUWUseCaseTests;
