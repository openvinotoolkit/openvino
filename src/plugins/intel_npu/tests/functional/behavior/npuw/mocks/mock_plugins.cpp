    ...
    ON_CALL(*this, get_property).WillByDefault([this](const std::string& name) -> ov::Any {
        if (name == ov::supported_properties) {
            ...
            if (this->m_config.count(ov::internal::exclusive_async_requests.name())) {
                auto exclusive_async_requests =
                        m_config.at(ov::internal::exclusive_async_requests.name()).as<bool>();
...
void MockPluginBase<DeviceType>::create_implementation() {
    ...
    ON_CALL(*this, compile_model(testing::A<const std::string&>(), testing::_))
            .WillByDefault([](const std::string& model_path,
    ...
    ON_CALL(*this, compile_model(testing::A<const std::shared_ptr<const ov::Model>&>(), testing::_, testing::_))
            .WillByDefault([](const std::shared_ptr<const ov::Model>& model, const ov::AnyMap& properties,
    ...
    ON_CALL(*this, get_property).WillByDefault([this](const std::string& name, const ov::AnyMap& arguments) -> ov::Any {
        ...
        const static std::vector<ov::PropertyName> roProperties{
                RO_property(ov::supported_properties.name()), RO_property(ov::available_devices.name()),
                RO_property(ov::device::uuid.name()), RO_property(ov::device::capabilities.name())
        };
    ...
    ON_CALL(*this, import_model(testing::_, testing::_, testing::_))
            .WillByDefault([](std::istream& model, const ov::SoPtr<ov::IRemoteContext>& context,
                                  const ov::AnyMap& properties) -> std::shared_ptr<ov::ICompiledModel> {
                OPENVINO_NOT_IMPLEMENTED;
            });
...
template <typename DeviceType>
void MockPluginBase<DeviceType>::set_expectations_to_comp_models(int model_idx, std::function<void(MockCompiledModel&)> expectations) {
