...
TEST_P(SetShapeInferRunTests, remoteCheckResultsAfterIOBlobReallocation0) {
    // Skip test according to plugin specific disabledTestPatterns() (if any)
    SKIP_IF_CURRENT_TEST_IS_DISABLED()
    ...
    auto l0_host_input_tensor =
        context.create_l0_host_tensor(ov::element::f32, original_shape, ov::intel_npu::TensorType::INPUT);
    auto l0_host_output_tensor = context.create_l0_host_tensor(ov::element::f32, original_shape);

    auto* host_tensor = l0_host_input_tensor.get();
    auto* input_data = reinterpret_cast<float*>(host_tensor);
    for (size_t i = 0; i < shape_size; ++i) {
        input_data[i] = 5.f;
    }
    ...
    auto* actual_host_tensor = l0_host_output_tensor.get();
    auto* actual = reinterpret_cast<float*>(actual_host_tensor);
    for (size_t i = 0; i < shape_size; ++i) {
        EXPECT_NEAR(actual[i], 6.f, 1e-5) << "Expected=6, actual=" << actual[i] << " for index " << i;
    }

    // imitates blob reallocation
    OV_ASSERT_NO_THROW(l0_host_input_tensor.set_shape({1, 50, 20, 20}));
    OV_ASSERT_NO_THROW(l0_host_input_tensor.set_shape(in_shape));

    OV_ASSERT_NO_THROW(l0_host_output_tensor.set_shape({1, 20, 20, 20}));
    OV_ASSERT_NO_THROW(l0_host_output_tensor.set_shape(out_shape));

    host_tensor = l0_host_input_tensor.get();
    input_data = reinterpret_cast<float*>(host_tensor);
    for (size_t i = 0; i < shape_size; ++i) {
        input_data[i] = 9.f;
    }
    ...
    actual_host_tensor = l0_host_output_tensor.get();
    actual = reinterpret_cast<float*>(actual_host_tensor);
    for (size_t i = 0; i < shape_size; ++i) {
        EXPECT_NEAR(actual[i], 10.f, 1e-5) << "Expected=10, actual=" << actual[i] << " for index " << i;
    }
...
TEST_P(SetShapeInferRunTests, remoteCheckResultsAfterIOBlobReallocation1) {
    // Skip test according to plugin specific disabledTestPatterns() (if any)
    SKIP_IF_CURRENT_TEST_IS_DISABLED()
    ...
    OV_ASSERT_NO_THROW(l0_host_input_tensor.set_shape({1, 70, 30, 30}));
    OV_ASSERT_NO_THROW(l0_host_input_tensor.set_shape(in_shape));

    OV_ASSERT_NO_THROW(l0_host_output_tensor.set_shape({1, 30, 30, 30}));
    OV_ASSERT_NO_THROW(l0_host_output_tensor.set_shape(out_shape));
    ...
    actual = reinterpret_cast<float*>(actual_host_tensor);
    for (size_t i = 0; i < shape_size; ++i) {
        EXPECT_NEAR(actual[i], 16.f, 1e-5) << "Expected=16, actual=" << actual[i] << " for index " << i;
    }

    auto host_input_tensor = context.create_host_tensor(ov::element::f32, original_shape);
    auto host_output_tensor = context.create_host_tensor(ov::element::f32, original_shape);

    inference_request.set_input_tensor(host_input_tensor);
    inference_request.set_output_tensor(host_output_tensor);

    OV_ASSERT_NO_THROW(host_input_tensor.set_shape({1, 70, 30, 30}));
    OV_ASSERT_NO_THROW(host_input_tensor.set_shape(in_shape));

    OV_ASSERT_NO_THROW(host_output_tensor.set_shape({1, 30, 30, 30}));
    OV_ASSERT_NO_THROW(host_output_tensor.set_shape(out_shape));

    auto* input_data_buffer = reinterpret_cast<float*>(host_input_tensor.data());
    for (size_t i = 0; i < shape_size; ++i) {
        input_data_buffer[i] = 99.f;
    }

    OV_ASSERT_NO_THROW(inference_request.infer());

    auto* output_data_buffer = reinterpret_cast<float*>(host_output_tensor.data());
    for (size_t i = 0; i < shape_size; ++i) {
        EXPECT_NEAR(output_data_buffer[i], 100.f, 1e-5)
            << "Expected=100, actual=" << output_data_buffer[i] << " for index " << i;
    }
}
