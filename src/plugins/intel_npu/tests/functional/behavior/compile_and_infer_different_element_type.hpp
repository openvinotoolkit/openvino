...
#include <cstdlib> // add for getenv
...
// Test whether the serialization and inference results of the dynamic type model and the undefined type model are the same
TEST_P(InferRequestElementTypeTests, CompareDynamicAndUndefinedTypeNetwork2) {
    // Skip test according to plugin specific disabledTestPatterns() (if any)
    SKIP_IF_CURRENT_TEST_IS_DISABLED()
...
<net name="custom_model" version="11">
    ...
    const char* value = std::getenv("RUN_WITH_OV_ASSERT_NO_THROW");
    if (value) {
        std::cout << "2.1un with OV_ASSERT_NO_THROW -----" << std::endl;
        reqUndefined = execNetUndefined.create_infer_request();
    } else {
        std::cout << "2.2run with OV_ASSERT_NO_THROW -----" << std::endl;
        OV_ASSERT_NO_THROW(reqUndefined = execNetUndefined.create_infer_request());
    }
