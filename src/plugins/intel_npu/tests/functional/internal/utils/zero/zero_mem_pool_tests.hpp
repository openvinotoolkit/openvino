...
TEST_P(ZeroMemPoolTests, GetUnallocatedInZeroMemoryData) {
    SKIP_IF_CURRENT_TEST_IS_DISABLED()

    void* data = ::operator new(4096, std::align_val_t(4096));

    auto get_zero_mem = ::intel_npu::ZeroMemPool::get_instance().get_zero_memory(init_struct, data, 4096);
    EXPECT_EQ(get_zero_mem, nullptr);
    ...
    if (init_struct->isExternalMemoryStandardAllocationSupported()) {
        ...
        for (int i = 0; i < threads_no; ++i) {
            threads[i] = std::thread([this, &zero_mem, i]() -> void {
                for (int j = 0; j < 256; j++) {
                    auto get_zero_mem =
                        ::intel_npu::ZeroMemPool::get_instance().get_zero_memory(init_struct,
                                                                                 zero_mem[i % 5]->data(),
                                                                                 4096);
                    sleep_ms(0);
                    get_zero_mem = {};
...
TEST_P(ZeroMemPoolTests, MultiThreading1) {
    ...
    for (int i = 0; i < threads_no; ++i) {
        threads[i] = std::thread([this, &data]() -> void {
            for (int j = 0; j < 256; j++) {
                std::shared_ptr<::intel_npu::ZeroMem> zero_mem;
                try {
                    zero_mem = ::intel_npu::ZeroMemPool::get_instance().get_zero_memory(init_struct, data[j % 5], 4096);

                    if (zero_mem == nullptr) {
                        zero_mem =
                            ::intel_npu::ZeroMemPool::get_instance().import_standard_allocation_memory(init_struct,
                                                                                                       data[j % 5],
                                                                                                       4096);
                    }
                } catch (...) {
