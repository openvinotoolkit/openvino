...
static std::shared_ptr<ov::Model> buildROPE_Llama2(const size_t seq_length,
                                                   const size_t max_position_embeddings,
                                                   const size_t ndims,
                                                   bool sin_cos_preprocessing) {
    
    auto position_ids = std::make_shared<ov::opset1::Parameter>(ov::element::f32, ov::Shape{1, 1, max_position_embeddings});
