...
static std::shared_ptr<Model> build_attention_broadcast4_static_model() {
    ...
    auto kv_param = ov::test::utils::make_param(element::f32, Shape{1, 2, 4, 8}, "kv_param");
    ...
    // gather is input 3
    auto concat_gather = std::make_shared<op::v0::Concat>(OutputVector{dim_a, dim_c, dim_b, gather}, /*axis=*/0);
...
static std::shared_ptr<Model> build_attention_broadcast4_dynamic_model() {
    ...
    auto kv_param = ov::test::utils::make_param(element::f32, PartialShape{1, 2, Dimension::dynamic(), 8}, "kv_param");
...
TEST(AttentionBroadcast4Test, FoldsShapeOfChainIntoConstant) {
    ...
    EXPECT_EQ(count_ops<op::v3::ShapeOf>(model), 0u) << "ShapeOf must be eliminated when its input has a static shape";
}
...
TEST(AttentionBroadcast4Test, DoesNotFoldWhenShapeIsDynamic) {
    ...
    EXPECT_EQ(count_ops<op::v3::ShapeOf>(model), 1u) << "ShapeOf must be preserved when the input shape is dynamic";
}
...
static std::shared_ptr<Model> build_chunk_prefill_model(bool sequence_only = false) {
    ...
    auto merged_heads = op::v0::Constant::create(element::i64, Shape{1}, {8});
...
static std::shared_ptr<Model> build_early_broadcast_reshape_model(bool three_input_shape, bool shared_gather) {
    ...
    auto kv_concat = std::make_shared<op::v0::Concat>(OutputVector{past, current}, /*axis=*/0);
    auto shape_of = std::make_shared<op::v3::ShapeOf>(kv_concat, element::i64);
    auto sequence_index = op::v0::Constant::create(element::i64, Shape{1}, {0});
    auto gather_axis = op::v0::Constant::create(element::i64, Shape{}, {0});
    auto sequence = std::make_shared<op::v8::Gather>(shape_of, sequence_index, gather_axis);

    auto two = op::v0::Constant::create(element::i64, Shape{1}, {2});
    auto four = op::v0::Constant::create(element::i64, Shape{1}, {4});
    auto eight = op::v0::Constant::create(element::i64, Shape{1}, {8});
    auto two_four = op::v0::Constant::create(element::i64, Shape{2}, {2, 4});
    auto broadcast_shape = std::make_shared<op::v0::Concat>(
        three_input_shape ? OutputVector{sequence, two_four, eight} : OutputVector{sequence, two, four, eight},
        /*axis=*/0);
    ...
    auto reshape_shape = shared_gather
                             ? std::make_shared<op::v0::Concat>(OutputVector{sequence, eight, eight}, /*axis=*/0)
                             : broadcast_shape;
    auto reshape = std::make_shared<op::v1::Reshape>(broadcast, reshape_shape, false);
    auto model = std::make_shared<Model>(ResultVector{std::make_shared<op::v0::Result>(reshape)},
                                         ParameterVector{past, current});
    model->validate_nodes_and_infer_types();
    ov::util::evaluate_both_bounds(broadcast_shape->output(0));
    ov::util::evaluate_both_bounds(sequence->output(0));
    return model;
}
...
TEST(ShapeOfConcatTest, DoesNotFoldWhenConcatShapeIsDynamic) {
    auto model = build_shape_of_concat_dynamic_model();
    ASSERT_EQ(count_ops<op::v3::ShapeOf>(model), 1u);
...
TEST(ShapeOfConcatTest, ChunkPrefillKeepsSequenceAndFoldsBatch) {
    ...
    Tensor past(element::f32, Shape{1, 2, 64, 256});
    Tensor current(element::f32, Shape{1, 2, 64, 256});
    std::fill_n(past.data<float>(), past.get_size(), 1.0f);
...
TEST(ShapeOfConcatTest, EarlierAttentionBroadcastStillFoldsWithoutChunkPreservation) {
    ...
    ov::npuw::patterns::regularize::RegularizeSDPA(/*run_broadcast_pattern=*/true).run_on_model(model);

    const auto reshape = model->get_result()->input_value(0).get_node_shared_ptr();
    const auto broadcast = reshape->input_value(0).get_node_shared_ptr();
...
static SharedVCacheModel build_shared_vcache_model() {
    ...
    // --- K-path ---
    auto past_k = ov::test::utils::make_param(element::f16, kv_past, "past_k");
    auto new_k = ov::test::utils::make_param(element::f16, kv_new, "new_k");
    ...
    // --- Q×K^T matmul ---
    auto query_k = ov::test::utils::make_param(element::f32, query_sh, "query_k");
    ...
    auto mask_global = ov::test::utils::make_param(element::f32, mask_sh, "attention_mask_global");
    ...
    auto past_v = ov::test::utils::make_param(element::f16, kv_past, "past_v");
    auto new_v = ov::test::utils::make_param(element::f16, kv_new, "new_v");
    ...
    auto extra_q = ov::test::utils::make_param(element::f32, attn_sh, "extra_q");
...
static SharedKCacheModel build_shared_kcache_model() {
    ...
    auto extra_q = ov::test::utils::make_param(element::f32, query_sh, "extra_q");
...
static QuantizedSDPAWithGlobalMaskModel build_sdpa_decomposed1_model() {
    ...
    // Q×K^T
    auto query = ov::test::utils::make_param(element::f32, query_sh, "query");
...
TEST(QuantizedSDPAWithGlobalMaskTest, IsolatesAllPatternNodes) {
    ...
    // No nodes should be tagged before the pass runs.
    {
        ...
        for (const auto& [node, gptr] : *gptr_map) {
            ASSERT_TRUE(gptr->isolatedTag().empty()) << "No node must be tagged before the pattern pass runs";
        }
    }
    ...
    EXPECT_GE(tagged, kPatternNodes) << "at least " << kPatternNodes << " pattern nodes must be tagged 'attn'";
}
...
TEST(QuantizedSDPAWithGlobalMaskTest, RenamesNewKVInputsToKVCacheNames) {
    ...
    EXPECT_EQ(new_k->get_friendly_name(), "past_key_values.0.key") << "new_k must be renamed to the KV-cache key name";
