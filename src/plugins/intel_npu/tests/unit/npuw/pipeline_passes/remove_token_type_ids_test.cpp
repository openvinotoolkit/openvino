...
void add_vision_chain(const std::shared_ptr<v0::Parameter>& tti, ParameterVector& params, ResultVector& results) {
    ...
    auto range_input = ov::test::utils::make_param(element::i64, Shape{1, 1, 1, SEQ}, "range_input");
    ...
    for (const char* branch : {"global", "local"}) {
        ...
        auto select_input = ov::test::utils::make_param(element::i64, Shape{1, 1, 1, SEQ}, "select_input" + suffix);
        auto branch_select = std::make_shared<v1::Select>(subg1_less, select_input, zeros);
        auto image_group_ids = ov::test::utils::make_param(element::i64, Shape{1, 1, SEQ, 1}, "image_group_ids" + suffix);
        ...
        auto image_token_mask = ov::test::utils::make_param(element::boolean, Shape{1, 1, 1, SEQ}, "image_token_mask" + suffix);
        ...
        auto causal_mask = ov::test::utils::make_param(element::boolean, Shape{1, 1, SEQ, SEQ}, "causal_mask" + suffix);
...
void add_shapeof_chain(const std::shared_ptr<v0::Parameter>& tti, ParameterVector& params, ResultVector& results) {
    ...
    auto tti_gather = std::make_shared<v8::Gather>(tti_shape_of,
                                                   v0::Constant::create(element::i64, Shape{}, {1}),
                                                   v0::Constant::create(element::i32, Shape{}, {0}));
    auto tti_range = ov::test::utils::make_param(element::i64, Shape{1, SEQ}, "tti_range");
    auto tti_less = std::make_shared<v1::Less>(tti_range, tti_gather);
    auto tti_pos_data = ov::test::utils::make_param(element::i64, Shape{1, SEQ}, "tti_pos_data");
    auto tti_select =
    ...
    auto attention_mask = ov::test::utils::make_param(element::i64, Shape{1, SEQ}, "attention_mask");
    ...
    auto attn_idx_src = ov::test::utils::make_param(element::i32, Shape{1, SEQ}, "attn_idx_src");
    ...
    auto preceding_mask = ov::test::utils::make_param(element::boolean, Shape{1, 1, SEQ, SEQ}, "preceding_mask");
...
std::shared_ptr<ov::Model> make_vision_model() {
    auto tti = ov::test::utils::make_param(element::i64, Shape{1, SEQ}, "token_type_ids");
...
TEST(RemoveTokenTypeIdsTest, NoOpWhenTokenTypeIdsAbsent) {
    auto input = ov::test::utils::make_param(element::f32, Shape{1, SEQ}, "input_ids");
