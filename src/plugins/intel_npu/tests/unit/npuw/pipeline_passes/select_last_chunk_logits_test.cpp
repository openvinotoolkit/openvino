...
void expect_selected_logits(const std::shared_ptr<ov::Model>& model,
                            uint32_t batch_dim,
                            int64_t valid_token_count,
                            int64_t expected_token_index) {
    ...
    const size_t sequence_dim = 1u - batch_dim;
    for (int64_t token_idx = 0; token_idx < kChunkSize; ++token_idx) {
        size_t offset = 0;
        if (sequence_dim == 0u) {
            offset = static_cast<size_t>(token_idx) * logits_shape[1] * kVocabSize;
        } else {
            offset = static_cast<size_t>(token_idx) * kVocabSize;
        }
        logits_data[offset] = static_cast<float>(100 + token_idx);
        logits_data[offset + 1] = static_cast<float>(200 + token_idx);
    }
    ...
    std::fill(attention_mask.data<int64_t>(), attention_mask.data<int64_t>() + valid_token_count, 1);
    ...
    EXPECT_FLOAT_EQ(output.data<float>()[0], static_cast<float>(100 + expected_token_index));
    EXPECT_FLOAT_EQ(output.data<float>()[1], static_cast<float>(200 + expected_token_index));
}

class SelectLastChunkLogitsTest : public ::testing::TestWithParam<uint32_t> {};

TEST_P(SelectLastChunkLogitsTest, SelectsExactBoundaryAndShortTailTokens) {
    ...
    expect_selected_logits(model, batch_dim, 64, 31);
    expect_selected_logits(model, batch_dim, 65, 0);
}
