...
std::string make_test_name(const ResolveStorageTypesParams& p) {
    ...
    os << mode_to_token(p.mode) << "_" << (p.is_symmetric ? "Sym" : "Asym") << "_Q" << type_to_token(p.quant_dt) << "_S"
       << type_to_token(p.scale_dt) << "_RQ" << type_to_token(p.expected_quantized_data_type) << "_RZ"
