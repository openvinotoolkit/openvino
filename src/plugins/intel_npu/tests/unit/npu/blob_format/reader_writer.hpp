...
void reader_register_sections(const std::shared_ptr<BlobReader>& blobReader,
    ...
    blobReader->register_reader(KnownSectionType::BATCH_SIZE, BatchSizeSection::read);
    blobReader->register_reader(KnownSectionType::IO_LAYOUTS, IOLayoutsSection::read);
}
...
std::string make_simple_blob(int64_t batch_size, BlobReader& reader) {
    ...
    reader.register_section_type_evaluator(
        std::make_shared<SupportedSectionTypeEvaluator>(KnownSectionType::BATCH_SIZE));
    return stream.str();
}
}  // namespace
...
TEST_P(AllSections, WriteRead) {
    ...
    auto read_batch =
        std::dynamic_pointer_cast<BatchSizeSection>(reader->retrieve_first_section(KnownSectionType::BATCH_SIZE));
    ASSERT_TRUE(read_batch);
    EXPECT_EQ(read_batch->get_batch_size(), batch_section->get_batch_size());

    auto read_io =
        std::dynamic_pointer_cast<IOLayoutsSection>(reader->retrieve_first_section(KnownSectionType::IO_LAYOUTS));
    ASSERT_TRUE(read_io);
...
TEST_F(WriterReaderEdgeCases, CorruptedMagicByte) {
    ...
    reader.register_reader(KnownSectionType::BATCH_SIZE, BatchSizeSection::read);
...
TEST_F(WriterReaderEdgeCases, ReExportRoundTrip) {
    ...
    reader_1->register_reader(KnownSectionType::BATCH_SIZE, BatchSizeSection::read);
    reader_1->register_reader(KnownSectionType::IO_LAYOUTS, IOLayoutsSection::read);
    ...
    reader_2->register_reader(KnownSectionType::BATCH_SIZE, BatchSizeSection::read);
    reader_2->register_reader(KnownSectionType::IO_LAYOUTS, IOLayoutsSection::read);
    ...
    auto read_batch =
        std::dynamic_pointer_cast<BatchSizeSection>(reader_2->retrieve_first_section(KnownSectionType::BATCH_SIZE));
    ASSERT_TRUE(read_batch);
    EXPECT_EQ(read_batch->get_batch_size(), BATCH);

    auto read_io =
        std::dynamic_pointer_cast<IOLayoutsSection>(reader_2->retrieve_first_section(KnownSectionType::IO_LAYOUTS));
    ASSERT_TRUE(read_io);
...
TEST_F(WriterReaderEdgeCases, MultipleSectionsSameType) {
    ...
    auto sections = reader.retrieve_sections_same_type(KnownSectionType::BATCH_SIZE);
...
TEST_F(WriterReaderEdgeCases, UnknownSectionSkipped) {
    ...
    ASSERT_NO_THROW(reader.read(tensor));
    EXPECT_EQ(reader.retrieve_first_section(KnownSectionType::BATCH_SIZE), nullptr);
}
...
TEST_F(WriterReaderEdgeCases, RetrieveSectionByExplicitID) {
    ...
    auto section = reader.retrieve_section(SectionID(KnownSectionType::BATCH_SIZE, 0));
    ASSERT_NE(section, nullptr);
    EXPECT_EQ(std::dynamic_pointer_cast<BatchSizeSection>(section)->get_batch_size(), BATCH);

    // there is no second instance
    EXPECT_EQ(reader.retrieve_section(SectionID(KnownSectionType::BATCH_SIZE, 1)), nullptr);
}
...
TEST_F(WriterReaderEdgeCases, RetrieveSectionsAbsentType) {
    ...
    auto absent = reader.retrieve_sections_same_type(KnownSectionType::IO_LAYOUTS);
...
TEST_F(WriterReaderEdgeCases, RegisterSectionInstanceIDs) {
    ...
    section = reader.retrieve_section(SectionID(KnownSectionType::BATCH_SIZE, 1));
    ASSERT_NE(section, nullptr);
    EXPECT_EQ(std::dynamic_pointer_cast<BatchSizeSection>(section)->get_batch_size(), BATCH_B);

    section = reader.retrieve_section(SectionID(KnownSectionType::BATCH_SIZE, 2));
    ASSERT_NE(section, nullptr);
