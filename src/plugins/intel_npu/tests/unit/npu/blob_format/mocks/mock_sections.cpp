...
std::shared_ptr<ISection> MockSection_2::read(BlobReaderInterface& blob_reader) {
    uint64_t size;
    blob_reader.read_into_buffer(reinterpret_cast<char*>(&size), sizeof(size));
...
std::shared_ptr<ISection> MockSectionWithTable::read(BlobReaderInterface& blob_reader) {
    ...
    blob_reader.read_into_buffer(reinterpret_cast<char*>(&table_location), sizeof(table_location));
    ...
    blob_reader.read_into_buffer(reinterpret_cast<char*>(&number_of_entries), sizeof(number_of_entries));
    ...
    for (uint64_t i = 0; i < number_of_entries; i++) {
        ...
        blob_reader.read_into_buffer(reinterpret_cast<char*>(&type), sizeof(type));
        blob_reader.read_into_buffer(reinterpret_cast<char*>(&instance), sizeof(instance));
        blob_reader.read_into_buffer(reinterpret_cast<char*>(&offset), sizeof(offset));
        blob_reader.read_into_buffer(reinterpret_cast<char*>(&length), sizeof(length));
