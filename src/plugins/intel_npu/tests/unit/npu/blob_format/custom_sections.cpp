...
TEST(MockSection1, WriteRead) {
    ...
    reader.register_section_type_evaluator(
        std::make_shared<SupportedSectionTypeEvaluator>(ValidSectionTypeCode::RUNTIME_REQUIREMENTS));
    reader.read(tensor);
...
TEST(MockSections, GetROITensors) {
    ...
    auto manifest_section =
        std::dynamic_pointer_cast<ManifestSection>(reader.retrieve_first_section(ValidSectionTypeCode::MANIFEST));
