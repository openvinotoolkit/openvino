...
public:
    ...
    static const primitive_db& get_db() {
        return db;
    }
...
protected:
    ...
    static void CheckDispatchData(const std::string& kernelName,
    ...
    virtual JitConstants MakeFusedOpsJitConstants(const base_params& params,
                                                  const std::vector<FusedOpsConfiguration>& conf) const;
    virtual JitConstants MakeFusedOpsDeclsJitConstants(const base_params& params,
                                                       const std::vector<FusedOpsConfiguration>& conf) const;

    // Basic check for extensions requirements
    // If params has at least 1 tensor of fp32/fp16/(u)int8 type, then it sets corresponding subgroup extension as
    // required Should be used carefully, as in some cases tensor types may not correspond to actually used extension
