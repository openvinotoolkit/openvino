...
DataTensor DataTensor::FlattenFeatureAndSpatials() const {
    ...
    case Tensor::byxf:
        ...
        throw std::runtime_error("Unsupported - cannot flatten yxf to f if f/yx != 1");
    case Tensor::bzyxf:
        if ((x.pitch == f.pitch && y.pitch == x.v * x.pitch &&
             z.pitch == y.v * y.pitch) ||  // YX - no Features (val/pitch)
            (z.v == 1 && y.v == 1 && x.v == 1 && x.pitch == f.pitch && y.pitch == f.pitch &&
             z.pitch == f.pitch) ||  // Feature only
            (f.v * f.pitch == x.pitch && f.v * f.pitch == y.pitch && f.v * f.pitch == z.pitch && z.v == 1 && y.v == 1 &&
             x.v == 1)) {  // Feature only
            l = targetLayout;
            break;
        }
...
NDims WeightsTensor::GetSimpleDims(const std::vector<size_t>& d, WeightsLayout l) {
    ...
    } else if (l == iy_xs_os_xsv2_osv16__ao32 || l == iy_xs_os_xsv2_osv8__ao32) {
    ...
    } else if (l == giy_xs_os_xsv2_osv16__ao32 || l == giy_xs_os_xsv2_osv8__ao32) {
        ret[2].pitch = RoundUp(ret[1].v, 2) * ret[1].pitch;
        ret[1].pad.after = newDims[1] - ret[1].v;
