...
FullyConnectedKernelIMAD::Parent::DispatchData FullyConnectedKernelIMAD::SetDefault(const fully_connected_params& params, int) const {
    ...
    if (params.output.GetLayout() == DataLayout::bfyx) {
        dispatchData.gws[0] = RoundUp(params.output.Y().v, tuning_data.sub_group_size) / tuning_data.tile_ofm * tuning_data.slm_div_factor;
        dispatchData.gws[1] = params.output.Batch().v;
        dispatchData.gws[2] = params.output.Feature().v / tuning_data.tile_batch;
    } else {
        dispatchData.gws[0] = RoundUp(params.output.Feature().v, tuning_data.sub_group_size) / tuning_data.tile_ofm * tuning_data.slm_div_factor;
...
bool FullyConnectedKernelIMAD::Validate(const Params& params, const optional_params& options) const {
    ...
    if (out_l == DataLayout::bfyx) {
        ...
        if (fc_params.output.Feature().v % tuning_data.tile_batch != 0)
            return false;
...
FullyConnectedKernelIMAD::FullyConnectedTuningData FullyConnectedKernelIMAD::GetTuningParams(const fully_connected_params& params) const {
    ...
    auto has_output_leftovers_ofm2 = of_num % (2 * tuning_data.sub_group_size) != 0;
    auto has_output_leftovers_ofm1 = of_num % (1 * tuning_data.sub_group_size) != 0;

    tuning_data.tile_ofm = !has_output_leftovers_ofm2 ? 2 : 1;

    // Use batched tile only if there are no leftovers for max ofm tile size (2)
    if (!has_output_leftovers_ofm2) {
        tuning_data.tile_batch = tuning_data.sub_group_size == 8 ? 16 : 8;
        while (ob_num % tuning_data.tile_batch != 0)
    ...
    // Try SLM splitting for some cases with batch = 1, low occupancy and without leftovers (of % simd == 0)
    if (!has_output_leftovers_ofm1 && tuning_data.tile_batch == 1 && tuning_data.in_f_blocks_number != 0) {
        size_t max_work_group_size = params.engineInfo.maxWorkGroupSize;

        // Decrease tile_ofm to improve occupancy
        if (tuning_data.in_f_blocks_number % tuning_data.slm_div_factor == 0 &&
            tuning_data.slm_div_factor <= max_work_group_size / tuning_data.sub_group_size &&
            tuning_data.tile_ofm == 2) {
            if (EstimateOccupancy(params, tuning_data.tile_ofm, tuning_data.tile_batch, tuning_data.slm_div_factor) < 1.0) {
                tuning_data.tile_ofm = 1;
            }
        }

        // SLM splitting via ifm
        while (tuning_data.in_f_blocks_number % (tuning_data.slm_div_factor * 2) == 0 &&
            (tuning_data.slm_div_factor * 2 <= max_work_group_size / tuning_data.sub_group_size) &&
            EstimateOccupancy(params, tuning_data.tile_ofm, tuning_data.tile_batch, tuning_data.slm_div_factor * 2) < 1.0) {
...
JitConstants FullyConnectedKernelIMAD::GetJitConstants(const fully_connected_params& params, const DispatchData& dispatchData) const {
    ...
    auto has_output_leftovers = (of_num % (tuning_data.tile_ofm * tuning_data.sub_group_size)) != 0;

    jit.AddConstant(MakeJitConstant("IN_F_BLOCKS_NUMBER", tuning_data.in_f_blocks_number));
    ...
    jit.AddConstant(MakeJitConstant("HAS_OUTPUT_LEFTOVERS", has_output_leftovers));
