...
)
{
    ...
    if (workers_per_data > SUB_GROUP_SIZE)
    {
        ...
        {
            ACC_TYPE vec_tmp = RMS_CLAMP(TO_ACC_TYPE(BLOCK_READ(input, input_data_offset + subgroup_offset + i * get_sub_group_size())));
#if SUBGROUP_BLOCK_SIZE == 1
    ...
    for (; i < items_num; i++)
    {
        ACCUMULATOR_TYPE tmp = RMS_CLAMP(TO_ACCUMULATOR_TYPE(input[input_data_offset + subgroup_offset + get_sub_group_local_id() + i * get_sub_group_size()]));
        rms += native_powr(tmp, 2);
        data[i] = tmp;
    }
    ...
    {
        ACCUMULATOR_TYPE tmp = RMS_CLAMP(TO_ACCUMULATOR_TYPE(input[input_data_offset + workers_per_data * items_num + in_data_idx]));
        rms += native_powr(tmp, 2);
        data[items_num] = tmp;
    }
