...
)
{
    ...
    unroll_for(uint i=0;i<real_seq_length;++i){
        ...
        unroll_for(uint l=0;l<NUM_HIDDEN_TO_DO;++l) { //kernel responsible for HIDDEN_SIZE
            ...
            #ifdef SEQUENCE
                #if DIRECTION == 2
                    hidden_state[OUTPUT1_GET_INDEX_SAFE(b, dir, hidden_idx, 0)] = TO_OUTPUT1_TYPE(hidden_value);
                #else
                    hidden_state[OUTPUT1_GET_INDEX_SAFE(b, 0, hidden_idx, 0)] = TO_OUTPUT1_TYPE(hidden_value);
                #endif
            #else
                hidden_state[OUTPUT_GET_INDEX_SAFE(b, hidden_idx, 0, 0)] = TO_OUTPUT_TYPE(hidden_value);
            #endif
            #ifdef SEQUENCE
                #if DIRECTION == 2
                    hidden_history[OUTPUT_GET_INDEX_SAFE(b, dir, cur_history_idx, hidden_idx)] = TO_OUTPUT_TYPE(hidden_value);
                #else // DIRECTION == 2
                    hidden_history[OUTPUT_GET_INDEX_SAFE(b, 0, cur_history_idx, hidden_idx)] = TO_OUTPUT_TYPE(hidden_value);
                #endif
            #endif
            if(i==real_seq_length-1){
                #ifdef SEQUENCE
                    #if DIRECTION == 2
                            cell_state[OUTPUT2_GET_INDEX_SAFE(b, dir, hidden_idx, 0)] = TO_OUTPUT2_TYPE(temp_cell_state[l]);
                    #else
                            cell_state[OUTPUT2_GET_INDEX_SAFE(b, 0, hidden_idx, 0)] = TO_OUTPUT2_TYPE(temp_cell_state[l]);
                    #endif
                #else
                        cell_state[OUTPUT1_GET_INDEX_SAFE(b, hidden_idx, 0, 0)] = TO_OUTPUT1_TYPE(temp_cell_state[l]);
                #endif
            }
        }
    }   
