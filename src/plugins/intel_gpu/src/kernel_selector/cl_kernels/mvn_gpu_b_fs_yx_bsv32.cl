...
KERNEL(mvn_final_bsv32)(
    ...
    // Decode each input element via DECODE_INPUT0_COMPUTE_TYPE: for bf16 this is
    // _convert_as_bfloat16_float (bf16->float); for f16/f32/i8/u8 it is identity, and the
    // subtraction with the float mean_vals promotes it to float.
    // (ACTIVATION_TYPE for bf16 is ushort, so fma/arithmetic on it would be integer math.)
