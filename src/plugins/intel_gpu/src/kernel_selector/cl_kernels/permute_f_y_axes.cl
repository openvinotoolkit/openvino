    ...
    )
{
    ...
    for (int j = 0; j < J_TIMES; ++j) {
        ...
        __attribute__((opencl_unroll_hint(VEC_SIZE)))
        for (int k = 0; k < VEC_SIZE; ++k) {
            transpose_buf[j_vec + k][bf_local] = TO_ACCUMULATOR_TYPE(ACTIVATION(DECODE_INPUT0_COMPUTE_TYPE(res[k]), ACTIVATION_PARAMS));
        }
#endif
    }
#if HAS_FUSED_OPS
    ...
    for (int j = 0; j < J_TIMES; ++j) {
        ...
        WRITE_VEC(TO_OUTPUT_VECTOR_TYPE(DECODE_INPUT0_COMPUTE_VECTOR_TYPE(READ_VEC(0, &transpose_buf[bf_local][j_vec]),VEC_SIZE), VEC_SIZE), 0, &output[output_idx]);
    }
#endif
