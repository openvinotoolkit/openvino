...
#define DQ_VEC_TYPE                         MAKE_VECTOR_TYPE(INT8_TYPE, TILE_IFM)
#define INT8_SLM_FILTER_VEC                 MAKE_VECTOR_TYPE(INT8_TYPE, TILE_OFM)
#define INT8_SLM_FILTER_PACKED_VEC          MAKE_VECTOR_TYPE(INT8_TYPE, FILTER_LOAD_BLOCK_SIZE)
#define INT8_SLM_FILTER_UNPACKED_VEC        MAKE_VECTOR_TYPE(INT8_TYPE, FILTER_ELEMENTS_PER_LOAD)
#define DQ_FILTER_VEC_TYPE                  MAKE_VECTOR_TYPE(INT8_TYPE, TILE_K_OFM)

#define TO_INT8_TYPE(x)                     CAT(CAT(convert_, INT8_TYPE),_sat)(x)
#define TO_DQ_VEC_TYPE(x)                   CAT(convert_, DQ_VEC_TYPE)(x)
#define TO_INT8_SLM_FILTER_UNPACKED_VEC(x)  CAT(convert_, INT8_SLM_FILTER_UNPACKED_VEC)(x)
...
inline void FUNC(fc_bf_tiled_kernel_default)(
...
#if USE_SLM
    , __local INT8_TYPE* dq_wei_local_mem
#endif
#if BIAS_TERM
    ...
    for (uint ni = 0; ni < iterations; ++ni) {
        ...
        // USE_SLM == 1 && COMPRESSED_WEIGHTS_INT4 == 1
            ...
            // Check scale
            MAKE_VECTOR_TYPE(INPUT0_TYPE, TILE_B) scale_value_b = 1;
            MAKE_VECTOR_TYPE(INPUT0_TYPE, TILE_B) r_scale_value_b = 1;
            MAKE_VECTOR_TYPE(INPUT0_TYPE, TILE_B) max_input;
            unroll_for (uint bi = 0; bi < TILE_B; ++bi) {
                max_input[bi] = sub_group_reduce_max(fmax(fabs(in_0[bi][0]), fabs(in_0[bi][1])));
            }
            scale_value_b = max_input / 128;
            r_scale_value_b = 1 / scale_value_b;

            unroll_for (uint bi = 0; bi < TILE_B; ++bi) {
                // slm_input_buffer[bi * 32 + sglid] = TO_INT8_TYPE(in_0[bi][0] * r_scale_value_b[bi]);
                // slm_input_buffer[bi * 32 + sglid + 16] = TO_INT8_TYPE(in_0[bi][1] * r_scale_value_b[bi]);
                dq_in_0[bi][0] = TO_INT8_TYPE(in_0[bi][0] * r_scale_value_b[bi]);
                dq_in_0[bi][1] = TO_INT8_TYPE(in_0[bi][1] * r_scale_value_b[bi]);
            }

            __local INT8_SLM_FILTER_VEC* int8_slm_wei_vec = (__local INT8_SLM_FILTER_VEC*)dq_wei_local_mem;
            ...
            unroll_for(uint load_iter = 0; load_iter < FILTER_LOAD_ITERS; ++load_iter) {
                ...
                #if DECOMPRESSION_SCALE_POST_OP
                    INT8_SLM_FILTER_UNPACKED_VEC dq_wei_unpacked = UNPACK_MIXED_INT4x2(INT8_TYPE, *((INT4_PACKED_TYPE_PRELOAD*)&wei_packed));
                    INT8_TYPE* dq_w = (INT8_TYPE*)(&dq_wei_unpacked);
                #else
                ...
                unroll_for(uint fi = 0; fi < TILE_OFM; ++fi) {
                    unroll_for(uint kii = 0; kii < FILTER_LOAD_BLOCK_SIZE; ++kii) {
                        ...
                        #if DECOMPRESSION_SCALE_POST_OP
                            dq_w[w_idx] = (dq_w[w_idx]- TO_INT8_TYPE(dzp));
                        #else
        ...
        unroll_for(uint ki = 0; ki < (TILE_IFM * SIMD) / TILE_K ; ++ki) {
            // Load compressed weight
                    ...
                    #if DECOMPRESSION_SCALE_POST_OP
                        wei = UNPACK_INT4x2(INT8_TYPE, *((INT4_PACKED_TYPE*)&wei_packed));
                    #else
...
#if DECOMPRESSION_SCALE_POST_OP
            #if USE_SLM
                const uint total_k = ki * TILE_K;
                uint tile_num = ki * TILE_K % SIMD;
                unroll_for (uint bi = 0; bi < TILE_B; ++bi) {
                    unroll_for (uint fi = 0; fi < TILE_OFM; ++fi) {
                        MAKE_VECTOR_TYPE(INT8_TYPE, 4) dq_in_val = (char4)(sub_group_broadcast(((INT8_TYPE*)(&dq_in_0[bi]))[total_k / SIMD], tile_num),
                                                                            sub_group_broadcast(((INT8_TYPE*)(&dq_in_0[bi]))[total_k / SIMD], tile_num+1),
                                                                            sub_group_broadcast(((INT8_TYPE*)(&dq_in_0[bi]))[total_k / SIMD], tile_num+2),
                                                                            sub_group_broadcast(((INT8_TYPE*)(&dq_in_0[bi]))[total_k / SIMD], tile_num+3));
                        ((int*)(&acc_tmp[bi]))[fi] = IMAD(((int*)(&acc_tmp[bi]))[fi],
                                                        dq_in_val,
                                                        *((MAKE_VECTOR_TYPE(INT8_TYPE, 4) *)(&((INT8_TYPE*)(&wei))[fi * TILE_K])));
                    }
                }
            #else
                unroll_for (uint kii = 0; kii < TILE_K; ++kii) {
                    ...
                    unroll_for (uint bi = 0; bi < TILE_B; ++bi) {
                        ...
                        const uint index_slm = ki * (TILE_B * TILE_K) + kii * TILE_B + bi;
                        INT8_TYPE dq_input = slm_input_buffer[index_slm];

                        unroll_for (uint fi = 0; fi < TILE_OFM; ++fi) {
                                int dq_temp = dq_input * ((INT8_TYPE*)(&wei))[kii * TILE_OFM + fi];
                                ((int*)(&acc_tmp[bi]))[fi] += (int)(dq_temp);
                        }
                    }
                }
            #endif
            ...
            unroll_for (uint bi = 0; bi < TILE_B; ++bi) {
                unroll_for(uint fi = 0; fi < TILE_OFM; ++fi) {
                    ...
                    ((ACCUMULATOR_TYPE*)(&acc[bi]))[fi] += TO_ACCUMULATOR_TYPE(((int*)(&acc_tmp[bi]))[fi]) * ds * scale_value_b[bi];
                    acc_tmp[bi][fi] = 0;
                }
            }
#endif
...
#if USE_SLM
    __local ACCUMULATOR_TYPE dq_wei_local_mem[TILE_IFM * SIMD * TILE_OFM * SIMD];
#endif
