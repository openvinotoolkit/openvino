...
(const __global INPUT0_TYPE* output_size, const __global INPUT1_TYPE* image_size, __global OUTPUT_TYPE* output) {
    ...
    const uint channel_size = OUTPUT_LENGTH / 2;
        ...
        // Each prior box produces exactly 4 coordinates (xmin, ymin, xmax, ymax),
        // so (out_index - start_out_index) is always a multiple of 4.
        for (uint i = start_out_index; i < out_index; i += 4) {
            for (uint j = 0; j < 4 && (i + j) < out_index; ++j) {
