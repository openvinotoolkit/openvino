...
#else
#error reorder_weights_bf16.cl: input format - not supported
#endif
}
...
#else
#error reorder_weights_bf16.cl: output format - not supported
#endif
}

#if OUTPUT_LAYOUT_IMAGE_2D_WEIGHTS_C1_B_FYX
KERNEL (reorder_weights_bf16)(const __global INPUT0_TYPE* input, write_only image2d_t output)
{
    ...
    input_val.s0 = TO_OUTPUT_TYPE(_convert_as_bfloat16_float(input[FUNC_CALL(get_input_index)(ir.s0,ir.s1,ir.s2,ir.s4,ir.s5,ir.s6)]));
    IMAGE_WRITE(output, coord, input_val);
}
#else
KERNEL (reorder_weights_bf16)(const __global INPUT0_TYPE* input, __global OUTPUT_TYPE* output)
{
