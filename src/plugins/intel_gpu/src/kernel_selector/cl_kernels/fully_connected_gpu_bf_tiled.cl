...
#if USE_SLM && SUPPORT_DYNAMIC_QUANTIZE && COMPRESSED_WEIGHTS_INT4
    if (get_group_id(0) == 0 && get_group_id(2) == 0 && get_local_id(2) == 0 && get_sub_group_local_id() == 0)
        printf("!!!!!!!!!!!!!!!! USE_SUM in bf_tiled : SCALE_GROUP_SIZE(%d) DECOMPRESSION_ZP_GROUPS_NUM(%d)\n",
                        (int)SCALE_GROUP_SIZE, (int)DECOMPRESSION_ZP_GROUPS_NUM);
#endif
...
inline void FUNC(fc_bf_tiled_kernel_dynamic_quantized)(
    ...
    // 16 sub group handle 2 batch
    const uint TILE_B_CNT = TILE_B / 2;

    ACCUMULATOR_VEC_TYPE    acc[TILE_B] = { };              // MAKE_VECTOR_TYPE(half, 2) acc[8] = { };

    // For pakcing to one integer buffer, load continuous inputs (INPUT_LOAD_SIZE elements)
    MAKE_VECTOR_TYPE(INPUT0_TYPE, INPUT_LOAD_SIZE)  tiled_input_0[INPUT_LOAD_SIZE] = { };
    PACKED_TYPE                                     packed_in_0[INPUT_LOAD_SIZE] = { };
    INPUT0_TYPE                                     max[TILE_B_CNT] = { };
...
// Temporal comment out
    ...
    for (uint ni = 0; ni < iterations; ++ni) {
        ...
        // Load input.
        for (uint bi = 0; bi < TILE_B_CNT; ++bi) {
        ...
        // Dynamic quantizing
            ...
            unroll_for (uint bi = 0; bi < TILE_B_CNT; ++bi) {
...
KERNEL(fc)(
...
#if USE_SLM
    #if SUPPORT_DYNAMIC_QUANTIZE && COMPRESSED_WEIGHTS_INT4
    ...
    if (batch_size == 1) {
        ...
        #if USE_SLM && SUPPORT_DYNAMIC_QUANTIZE && COMPRESSED_WEIGHTS_INT4
            // Validated dynamic quantizing optimization with SLM and 4bit weight
