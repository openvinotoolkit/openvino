...
KERNEL (mvn_gpu_bfyx_opt)(
    ...
    // and normalize loops share a single pass over global memory. What is cached is the decoded
    // value, and every supported input type (f16, bf16, f32, i8, u8) is exactly representable in
    // float, so the cached values are bit-exact.
...
#   pragma unroll
    for (uint i = 0; i < MVN_CACHE_ITERS; ++i) {
        if (i >= iters_num)
            continue;
        cached[i] = DECODE_INPUT0_COMPUTE_TYPE(input[my_data_offset + i * workers_per_data_set]);
    }
#   pragma unroll
    for (uint i = 0; i < MVN_CACHE_ITERS; ++i) {
        if (i >= iters_num)
...
#else
#   define MVN_LOAD(i) DECODE_INPUT0_COMPUTE_TYPE(input[my_data_offset + i * workers_per_data_set])
