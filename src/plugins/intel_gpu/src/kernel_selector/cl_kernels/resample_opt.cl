...
)
{
...
#elif defined(SAMPLE_TYPE_LINEAR_ONNX)
    // ONNX-compatible bilinear interpolation with configurable coordinate transformation
    // (half_pixel, pytorch_half_pixel, align_corners, etc.) via get_original_coordinate().
    // This enables resample_opt (priority 3) to handle LINEAR_ONNX instead of the slower
    // resample_onnx (priority 4) for 4D inputs.
    ...
    float in_y = fmax(0.0f, fmin(iy, (float)(PADDED_Y - 1)));
    ...
    unroll_for (uint out_x = 0; out_x < OUTPUT_X_BLOCK_SIZE; out_x++) {
        ...
        float in_x = fmax(0.0f, fmin(ix, (float)(PADDED_X - 1)));
        ...
        const acc_vec_t top_left     = TO_ACC_VEC_TYPE(DECODE_IN_VEC_TYPE(READ_FUNC(input, INPUT0_GET_INDEX(b, feature_block, in_y1, in_x1))));
        const acc_vec_t top_right    = TO_ACC_VEC_TYPE(DECODE_IN_VEC_TYPE(READ_FUNC(input, INPUT0_GET_INDEX(b, feature_block, in_y1, in_x2))));
        const acc_vec_t bottom_left  = TO_ACC_VEC_TYPE(DECODE_IN_VEC_TYPE(READ_FUNC(input, INPUT0_GET_INDEX(b, feature_block, in_y2, in_x1))));
        const acc_vec_t bottom_right = TO_ACC_VEC_TYPE(DECODE_IN_VEC_TYPE(READ_FUNC(input, INPUT0_GET_INDEX(b, feature_block, in_y2, in_x2))));
