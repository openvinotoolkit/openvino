...
KERNEL(calc_mean_per_group)(
    ...
    if (lid == 0) {
        const uint offset = get_global_id(0) + get_global_id(1) * get_global_size(0);
        const uint group_size = get_local_size(0);

        ACCUMULATOR_TYPE mean = ACCUMULATOR_VAL_ZERO;
        for (uint i = 0; i < group_size; ++i) {
            mean += internal_mean[offset + i];
        }
        mean /= TO_ACCUMULATOR_TYPE(group_size);

        for (uint i = 0; i < group_size; ++i) {
            internal_mean[offset + i] = mean;
        }
    }
}
...
KERNEL(group_normalization_b_fs_yx_fsv16)(
    ...
    const uint my_data_offset = data_set_offset + in_data_set_idx;
    uint bf = bfs * FSV + in_data_set_idx;
    uint channel_index = bf % OUTPUT_FEATURE_NUM;

    ACCUMULATOR_TYPE mean = internal_mean[bf];
    ACCUMULATOR_TYPE variance = internal_variance[bf];

    for (uint i = 0; i < (INPUT0_SIZE_X * INPUT0_SIZE_Y); ++i) {
        uint output_index = my_data_offset + i * SIMD;

        ACTIVATION_TYPE result = (TO_ACTIVATION_TYPE(input[output_index]) - TO_ACTIVATION_TYPE(mean)) * TO_ACTIVATION_TYPE(variance);
