...
// JIT constants make this a compile-time decision. If every dispatched X block
// has its complete input line inside the logical tensor, boundary bookkeeping
// and the slow path are removed from generated code entirely.
#define CONV_FSV16_SKIP_X_CLAMP (PADDING_SIZE_X == 0 && \
    (((X_BLOCKS - 1) * OUTPUT_X_BLOCK_SIZE * STRIDE_SIZE_X + INPUT_LINE_SIZE) <= INPUT0_SIZE_X))
...
#   define NEEDS_X_CLAMP needs_x_clamp
#endif
    ...
    const uint input_offset = b * input_b_pitch + input_fs_pad_before * input_fs_pitch +
    ...
    const uint output_offset = b * output_b_pitch + feature_block * (output_fs_pitch * FEATURE_SLICE_SIZE) +
                               (sglid + OUTPUT_PAD_BEFORE_FEATURE_NUM) * output_fs_pitch +
                               (y + OUTPUT_PAD_BEFORE_SIZE_Y) * output_y_pitch + x + OUTPUT_PAD_BEFORE_SIZE_X;
#else
    ...
    const uint output_offset = b * output_b_pitch + (feature_block + output_fs_pad_before) * output_fs_pitch +
            ...
            for (int kh = 0; kh < FILTER_SIZE_Y; kh++) {
                ...
                if (!NEEDS_X_CLAMP) {
                    ...
                    for (; xb + 8 <= INPUT_LINE_SIZE; xb += 8) {
                        INPUT_TYPE8 vv = DT_INPUT_BLOCK_READ8(input, grouped_input_offset + icb * input_fs_pitch +
                                                                  kh * DILATION_SIZE_Y * input_y_pitch + xb * input_x_pitch);
                        line_cache[xb + 0] = vv[0]; line_cache[xb + 1] = vv[1];
                        line_cache[xb + 2] = vv[2]; line_cache[xb + 3] = vv[3];
                        line_cache[xb + 4] = vv[4]; line_cache[xb + 5] = vv[5];
                        line_cache[xb + 6] = vv[6]; line_cache[xb + 7] = vv[7];
                    }
                    for (; xb + 4 <= INPUT_LINE_SIZE; xb += 4) {
                        INPUT_TYPE4 vv = DT_INPUT_BLOCK_READ4(input, grouped_input_offset + icb * input_fs_pitch +
                                                                  kh * DILATION_SIZE_Y * input_y_pitch + xb * input_x_pitch);
                        line_cache[xb + 0] = vv[0]; line_cache[xb + 1] = vv[1];
                        line_cache[xb + 2] = vv[2]; line_cache[xb + 3] = vv[3];
                    }
                ...
                for (int kw = 0; kw < FILTER_SIZE_X; kw++) {
                    ...
                    for (int ifm = 0; ifm < FILTER_IFM_NUM; ifm++)
                        wei0[ifm] = weights[grouped_filter_offset + ofm_in_group + ifm * filter_isv_pitch +
                    ...
                    dst = mad(wei0.s0, src0, dst); dst = mad(wei0.s1, src1, dst);
                    ...
                    dst = mad(wei0.s0, src0, dst); dst = mad(wei0.s1, src1, dst);
                    dst = mad(wei0.s2, src2, dst); dst = mad(wei0.s3, src3, dst);
                    ...
                    dst = mad(wei0.s0, src0, dst); dst = mad(wei0.s1, src1, dst);
                    dst = mad(wei0.s2, src2, dst); dst = mad(wei0.s3, src3, dst);
                    dst = mad(wei0.s4, src4, dst); dst = mad(wei0.s5, src5, dst);
                    dst = mad(wei0.s6, src6, dst); dst = mad(wei0.s7, src7, dst);
#else
# error unsupported input feature size
#endif
#else
                    FILTER_TYPE8 wei0 = DT_FILTER_BLOCK_READ8(weights, grouped_filter_offset + icb * filter_is_pitch +
                                                                       kh * filter_y_pitch + kw * filter_x_pitch);
                    FILTER_TYPE8 wei1 = DT_FILTER_BLOCK_READ8(weights, grouped_filter_offset + icb * filter_is_pitch +
                                                                       kh * filter_y_pitch + kw * filter_x_pitch + 8 * filter_isv_pitch);
#if GROUPED
                    if (groups_per_sub_group > 1) {
                        ...
                        wei0 = AS_FILTER_TYPE8(w0); wei1 = AS_FILTER_TYPE8(w1);
#else
                        wei0 = _sub_group_shuffle(wei0, correct_lane);
                        wei1 = _sub_group_shuffle(wei1, correct_lane);
#endif
                        if (g != my_group) { wei0 = (FILTER_TYPE8)(0); wei1 = (FILTER_TYPE8)(0); }
                    }
#endif
                    const vec_t src0 = GET_SRC(src, 0); const vec_t src1 = GET_SRC(src, 1);
                    const vec_t src2 = GET_SRC(src, 2); const vec_t src3 = GET_SRC(src, 3);
                    const vec_t src4 = GET_SRC(src, 4); const vec_t src5 = GET_SRC(src, 5);
                    const vec_t src6 = GET_SRC(src, 6); const vec_t src7 = GET_SRC(src, 7);
                    const vec_t src8 = GET_SRC(src, 8); const vec_t src9 = GET_SRC(src, 9);
                    const vec_t src10 = GET_SRC(src, 10); const vec_t src11 = GET_SRC(src, 11);
                    const vec_t src12 = GET_SRC(src, 12); const vec_t src13 = GET_SRC(src, 13);
                    const vec_t src14 = GET_SRC(src, 14); const vec_t src15 = GET_SRC(src, 15);
                    dst = mad(wei0.s0, src0, dst); dst = mad(wei0.s1, src1, dst);
                    dst = mad(wei0.s2, src2, dst); dst = mad(wei0.s3, src3, dst);
                    dst = mad(wei0.s4, src4, dst); dst = mad(wei0.s5, src5, dst);
                    dst = mad(wei0.s6, src6, dst); dst = mad(wei0.s7, src7, dst);
                    dst = mad(wei1.s0, src8, dst); dst = mad(wei1.s1, src9, dst);
                    dst = mad(wei1.s2, src10, dst); dst = mad(wei1.s3, src11, dst);
                    dst = mad(wei1.s4, src12, dst); dst = mad(wei1.s5, src13, dst);
                    dst = mad(wei1.s6, src14, dst); dst = mad(wei1.s7, src15, dst);
#endif
                }
            }
        }
...
#else
# error unsupported output x block size
#endif
#else
