...
)
{
...
#if HAS_FUSED_OPS
    FUSED_OPS;
    OUTPUT_TYPE_BLOCK out = TO_TYPE(OUTPUT_TYPE_BLOCK, FUSED_OPS_RESULT);
#else
    #if ACTIVATION_IN_ACCUMULATOR_TYPE
        OUTPUT_TYPE_BLOCK out = TO_OUTPUT_VECTOR_TYPE(ACTIVATION_TYPED(res, ACTIVATION_PARAMS_TYPED), VEC_SIZE);
    #else
        OUTPUT_TYPE_BLOCK out = ACTIVATION_TYPED(TO_OUTPUT_VECTOR_TYPE(res, VEC_SIZE), ACTIVATION_PARAMS_TYPED);
    #endif
#endif
    ...
    if ((f_block*VEC_SIZE + VEC_SIZE) > OUTPUT_FEATURE_NUM) {
        for (uint fp = OUTPUT_FEATURE_NUM % VEC_SIZE; fp < VEC_SIZE; fp++) {
            out[fp] = TO_OUTPUT_TYPE(OUTPUT_VAL_ZERO);
        }
    }
#endif
