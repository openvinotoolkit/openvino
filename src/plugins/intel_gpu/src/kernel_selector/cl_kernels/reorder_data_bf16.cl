    ...
    )
{
    ...
    INPUT_TYPE4 colorRGBA = { TO_INPUT_REORDER_TYPE(_convert_as_bfloat16_float(input[input_idx_R])), TO_INPUT_REORDER_TYPE(_convert_as_bfloat16_float(input[input_idx_G])), TO_INPUT_REORDER_TYPE(_convert_as_bfloat16_float(input[input_idx_B])), TO_INPUT_REORDER_TYPE(0.f) };
#else
    const uint input_idx_A  = FUNC_CALL(get_input_index)(OPTIONAL_SHAPE_INFO_TENSOR b, 3, v, u, w, z, y, x);
    INPUT_TYPE4 colorRGBA = { TO_INPUT_REORDER_TYPE(_convert_as_bfloat16_float(input[input_idx_R])), TO_INPUT_REORDER_TYPE(_convert_as_bfloat16_float(input[input_idx_G])), TO_INPUT_REORDER_TYPE(_convert_as_bfloat16_float(input[input_idx_B])), TO_INPUT_REORDER_TYPE(_convert_as_bfloat16_float(input[input_idx_A])) };
#endif
#else
    uint8 ov = RESHAPE_DIMS(INPUT0, OUTPUT, b, f, v, u, w, z, y, x);
    ...
    float res = TO_MEAN_TYPE(_convert_as_bfloat16_float(input[input_idx]));
    ...
    MEAN_SUBTRACT_TYPE res = TO_MEAN_TYPE(_convert_as_bfloat16_float(input[input_idx]));
