...
#if GATE_IDX == 0
    ACCUMULATOR_TYPE gate = TO_ACCUMULATOR_TYPE(input[y]);
    #if GLU_STRIDE == 2
        ACCUMULATOR_TYPE value = TO_ACCUMULATOR_TYPE(input[y + 1]);
    #else
        ACCUMULATOR_TYPE value = TO_ACCUMULATOR_TYPE(input[y + GLU_STRIDE]);
    #endif
#else
    #if GLU_STRIDE == 2
        // alternating mode: pair is at positions y, y+1 — gate is the odd neighbor
        ACCUMULATOR_TYPE gate = TO_ACCUMULATOR_TYPE(input[y + 1]);
    #else
        ACCUMULATOR_TYPE gate = TO_ACCUMULATOR_TYPE(input[y + GLU_STRIDE]);
    #endif
    ACCUMULATOR_TYPE value = TO_ACCUMULATOR_TYPE(input[y]);
#endif
#ifdef SCALE_FACTOR
