...
std::string toCodeString(const Tensor::Dim& dim, size_t offset, bool shape_info_as_args, bool padded = false);
std::string toShapeInfoString(size_t arg_idx, size_t data_idx_at_6d, bool is_output = false, size_t num_of_inputs = 0, bool shape_info_as_args = true);
...
std::shared_ptr<JitConstant> MakeJitConstant(const std::string& name, const struct Tensor::DataTensor& value,
                                             size_t dyn_tensor_index = 0, bool shape_info_as_args = true);
...
public:
    ...
    JitConstants MakeFusedTensorJitConstants(const FusedOpsConfiguration& conf, size_t dynamic_in_out_tensors_count, bool shape_info_as_args) const;
