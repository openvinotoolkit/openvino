...
class OpenCL2CHeaders(object):
    ...
    def __init__(
        ...
        kernels_folder,
        out_path,
        out_file_name_prim_db,
        out_file_name_batch_headers,
    ):
    ...
    def find_and_set_batch_headers(self):
        batch_headers_list = [
            ntpath.basename(h)
            for h in glob.glob(
                os.path.join(self.kernels_folder, "include/batch_headers/*.cl")
            )
        ]
        deps = {}
        for h in batch_headers_list:
            header_file = os.path.abspath(
                os.path.join(self.kernels_folder, "include/batch_headers", h)
            )
            f = open(header_file)
            content = f.readlines()
            deps[h] = {h}
    ...
    def convert(self):
        ...
        out_file_name_batch_headers = os.path.join(
            self.out_path, self.out_file_name_batch_headers
        )
            ...
            if line.find("CAT") >= 0:
                ...
                while iter_w < len(words):
                    if words[iter_w] != "CAT":
                        iter_w += 1
                        continue
                    user_exist, concat_len = self.found_concat_user(
                        words, iter_w, macro
                    )
                    if user_exist:
                        return True
        ...
        for line in content:
            ...
            if line.startswith("#include"):
                ...
                if ntpath.basename(include_file_name) in self.batch_headers:
                    continue
                full_path_include = os.path.abspath(
                    os.path.join(os.path.dirname(filename), include_file_name)
                )
                if (
                    full_path_include not in self.include_files[origin_file]
                    or not optimize_includes
                ):
    ...
    def batch_headers_to_str(self):
        ...
        for h in self.batch_headers:
            ...
            header_file = os.path.abspath(
                os.path.join(
                    os.path.dirname(self.kernels_folder + "/include/batch_headers"),
                    "batch_headers/" + h,
                )
            )
            content = []
            ...
            for i, line in enumerate(content):
                if line.startswith("#include"):
                    continue
                if (i + 1) % max_lines == 0 or characters + len(
                    line
                ) + 1 > max_characters:
                    res += ')",' + ' (std::string) R"('
                    characters = 0
...
def main():
    ...
    ap.add_argument(
        ...
        required=True,
        metavar="PATH",
        help="The absolute path to OpenCL kernels folder",
    )
    ap.add_argument(
        "-out_path",
        required=True,
        metavar="PATH",
        help="The absolute path to dump file",
    )
    ap.add_argument(
        "-out_file_name_prim_db", required=True, metavar="PATH", help="dump file name"
    )
    ap.add_argument(
        "-out_file_name_batch_headers",
        required=True,
    ...
    converter = OpenCL2CHeaders(
        args.kernels,
        args.out_path,
        args.out_file_name_prim_db,
        args.out_file_name_batch_headers,
    )
    converter.convert()
