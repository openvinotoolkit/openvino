...
JitConstants DeconvolutionKernelBase::GetJitConstants(const deconvolution_params& dp) const {
    ...
    int64_t input_offset_with_padding =
        (int64_t)input.GetFirstElementOffset() - (dp.filterSize.x - 1 + padding.x) * input.X().pitch - (dp.filterSize.y - 1 + padding.y) * input.Y().pitch;
...
KernelsData DeconvolutionKernelBase::GetKernelsData(const Params& params) const {
    ...
    bool succeed = UpdateWeightsParams(newParams, GetPreferredWeightsLayout(newParams), kd.weightsReorderParams, GetSupportedKey(), newParams.groups);
...
Datatype DeconvolutionKernelBase::GetAccumulatorType(const deconvolution_params& params) const {
    ...
    if (params.inputs[0].GetDType() == Datatype::F16 && (params.dilation.x > 1 || params.dilation.y > 1 || params.dilation.z > 1)) {
        return Datatype::F32;
    }
