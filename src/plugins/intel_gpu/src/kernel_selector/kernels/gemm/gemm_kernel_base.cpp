...
size_t GemmKernelBase::GetOuputSize(const std::vector<int64_t>& output_order,
...
std::vector<std::string> GemmKernelBase::GetTransposedDims(const std::vector<int64_t>& order_idx,
                                                           bool is_tiled_opt) const {
...
std::string GemmKernelBase::GetDimsOrder(const std::vector<int64_t>& order_idx) const {
    ...
    if (order_idx.size() == 2) {
        ...
        dims_order = "b," + dims3[get_order_idx(order_idx, 0)] + ",w,z," + dims3[get_order_idx(order_idx, 1)] + "," +
                     dims3[get_order_idx(order_idx, 2)];
    } else if (order_idx.size() == 4) {
...
JitConstants GemmKernelBase::GetJitConstants(const gemm_params& params) const {
    ...
    if (params.inputs.size() == 4 ||
        (!params.indirect_input0 && !params.indirect_input1 && params.inputs.size() == 3)) {
    ...
    if (params.input0_broadcast_val != 0) {
        jit.AddConstants({
            ...
            MakeJitConstant("DO_BROADCAST_INPUT0",
                            get_broadcast_input_str(params.inputs[0].GetDims().size(),
    ...
    if (params.input1_broadcast_val != 0) {
        jit.AddConstants({
            ...
            MakeJitConstant("DO_BROADCAST_INPUT1",
                            get_broadcast_input_str(params.inputs[1].GetDims().size(),
