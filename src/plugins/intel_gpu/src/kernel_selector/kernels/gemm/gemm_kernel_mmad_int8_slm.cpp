...
GemmKernelMMADslmInt8::GemmTuningData GemmKernelMMADslmInt8::SetTuningParams(const gemm_params& params) const {
    ...
    tuning_data.slm_decimation_factor =
        tuning_data.size_k <= tuning_data.max_slm_preloading_size ? tuning_data.size_k / tuning_data.slm_tile_size : 2;
    return tuning_data;
}
...
KernelsPriority GemmKernelMMADslmInt8::GetKernelsPriority(const Params& params) const {
    ...
    if ((mmad_operations_number >= 1024 * 1024 * 1024) ||
        (tuning_data.size_m == 384 && tuning_data.size_k == 384 && tuning_data.size_n == 64))
