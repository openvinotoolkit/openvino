...
JitConstants GroupNormalizationKernelRef::GetJitConstants(KernelId kernelId,
                                                          const group_normalization_params &params,
                                                          const int sub_kernel_num) const {
    ...
    case eCalcMeanKernel:
        ...
        jit.AddConstant(MakeJitConstant("TARGET_FEATURE", sub_kernel_num));
        break;
    case eCalcPow:
    ...
    case eCalcStandardDeviationKernel:
        ...
        jit.AddConstant(MakeJitConstant("TARGET_SD_FEATURE", sub_kernel_num));
        break;
...
KernelsData GroupNormalizationKernelRef::GetKernelsData(const Params &params, const optional_params &options) const {
    const auto& params_ = dynamic_cast<const group_normalization_params&>(params);

    size_t NUM_CHANNELS_IN_GROUP = (params_.inputs[0].Feature().v / params_.num_groups);
    size_t total_kernel_num = eKernelsNum - 2 + NUM_CHANNELS_IN_GROUP * 2;

    KernelData kd = KernelData::Default<group_normalization_params>(params, total_kernel_num);
    ...
    KernelId id = eCalcMeanKernel;
    for (size_t i = 0 ; i < NUM_CHANNELS_IN_GROUP; i++)
    {
        auto& kernel = kd.kernels[id + i];
        const auto entryPoint = GetEntryPoint(kernelName, parameters.layerID, params, options, id + i);
        auto jitConstants = GetJitConstants(id, parameters, i);
        const auto jit = CreateJit(kernelName, jitConstants, entryPoint);
        const auto dispatchData = SetDefault(id, parameters);
    ...
    id = eCalcPow;
    {
        auto& kernel = kd.kernels[id + NUM_CHANNELS_IN_GROUP - 1];
        const auto entryPoint = GetEntryPoint(kernelName, parameters.layerID, params, options, id + NUM_CHANNELS_IN_GROUP - 1);
        auto jitConstants = GetJitConstants(id, parameters, 0);
        const auto jit = CreateJit(kernelName, jitConstants, entryPoint);
        const auto dispatchData = SetDefault(id, parameters);
    ...
    id = eCalcStandardDeviationKernel;
    for (size_t i = 0 ; i < NUM_CHANNELS_IN_GROUP; i++)
    {
        auto& kernel = kd.kernels[id + NUM_CHANNELS_IN_GROUP - 1 + i];
        const auto entryPoint = GetEntryPoint(kernelName, parameters.layerID, params, options, id + NUM_CHANNELS_IN_GROUP - 1 + i);
        auto jitConstants = GetJitConstants(id, parameters, 0);
        const auto jit = CreateJit(kernelName, jitConstants, entryPoint);
        const auto dispatchData = SetDefault(id, parameters);
    ...
    id = eNormalize;
    {
        auto& kernel = kd.kernels[id + NUM_CHANNELS_IN_GROUP*2 - 2];
        const auto entryPoint = GetEntryPoint(kernelName, parameters.layerID, params, options, id + NUM_CHANNELS_IN_GROUP*2 - 2);
        auto jitConstants = GetJitConstants(id, parameters, 0);
        const auto jit = CreateJit(kernelName, jitConstants, entryPoint);
        const auto dispatchData = SetDefault(id, parameters);
