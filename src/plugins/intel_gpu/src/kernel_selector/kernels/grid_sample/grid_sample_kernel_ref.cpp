...
KernelsData GridSampleKernelRef::GetKernelsData(const Params& params, const optional_params& options) const {
...
bool GridSampleKernelRef::Validate(const Params& params, const optional_params& options) const {
    if (params.GetType() != KernelType::GRID_SAMPLE || options.GetType() != KernelType::GRID_SAMPLE) {
        return false;
    }
