                        ...
                        for (unsigned dispatch_fsv = 1; dispatch_fsv <= 16; ++dispatch_fsv)
                            for (auto exec : Parent::autoTuneOptions) {
                                ...
                                if (tile_k * tile_ofm > 8)
                                    continue;
                                // For bsv == 1 dispatch order reduces to b_fsv, so anything other than fsv == 1 is
                                // redundant.
                                if (dispatch_bsv == 1 && dispatch_fsv != 1)
                                    continue;

                                auto_tune_params
                                    .emplace_back(tile_b, tile_ofm, tile_ifm, tile_k, dispatch_bsv, dispatch_fsv, exec);
                            }
}
...
bool FullyConnected_bf_tiled::Validate(const Params& params) const {
    ...
    if ((wt == WeightsType::UINT4 || wt == WeightsType::INT4) &&
        (weights.IFM().v % 2 != 0 || weights.OFM().v % 2 != 0)) {
        return false;
    }
...
bool TuneParamsSelector::VerifyTuneParams(const fully_connected_params& params, const tune_params& tparams) {
    ...
    if (tparams.kernel_type == FullyConnected_bf_tiled::KernelType::SLM) {
        ...
        const auto required_slm_size =
            tparams.tile_ofm * simd * tparams.tile_ifm * simd * 2;  // 2 bytes per value (FP16 data type)
    ...
    unsigned acc_register_bytes =
        tparams.tile_b * tparams.tile_ofm * simd * BytesPerElement(params.inputs[0].GetDType());
    unsigned in_register_bytes =
        tparams.tile_b * tparams.tile_ifm * simd * BytesPerElement(params.inputs[0].GetDType());
    ...
    if (params.weights.GetDType() == WeightsType::UINT4 || params.weights.GetDType() == WeightsType::INT4) {
        if (!params.is_shape_agnostic && batch == 1) {
            ...
            if (output_f / 2 < ideal_num_threads * 0.8 &&
                params.weights.GetLayout() == WeightsLayout::os_is_yx_osv32_isv2) {
...
FullyConnected_bf_tiled::DispatchData FullyConnected_bf_tiled::SetDefault(const fully_connected_params& params,
                                                                          int autoTuneIndex,
    ...
    const size_t aligned_batch =
        Align(batch_threads, lws_batches);  // Each WG calculates 8x8 batches (TILE_B x LWS[2] size)
...
JitConstants FullyConnected_bf_tiled::GetJitConstants(const fully_connected_params& params,
    ...
    if (dispatchData.use_slm) {
        ...
        OPENVINO_ASSERT(weights_dt == WeightsType::INT4 || weights_dt == WeightsType::UINT4,
                        "[GPU] Unsupported FC weights type for SLM kernel configuration");

        auto lws_batches = dispatchData.lws[2];
        auto total_weights_elements =
            simd * dispatchData.tile_n * simd * dispatchData.tile_mk;  // SIMD * TILE_OFM * SIMD * TILE_IFM
        ...
        jit.Merge(make_int4_packed_type_jit_constant("INT4_PACKED_TYPE_PRELOAD",
                                                     params.weights.GetDType(),
                                                     weights_elements_per_load));
    }
    ...
    bool realign_fp16_offset =
        params.inputs[0].GetDType() == Datatype::F16 && params.inputs[0].GetFirstElementOffset() % 2 != 0;
...
void FullyConnected_bf_tiled::GetUpdateDispatchDataFunc(KernelData& kd) const {
    if (kd.kernels.size() == 1) {
        ...
        kd.update_dispatch_data_func = [this](const Params& params, KernelData& kd) {
            ...
            OPENVINO_ASSERT(kd.kernels.size() == 2,
                            "[GPU] Invalid kernels size for update dispatch data func, expected 2, got ",
                            kd.kernels.size());
            ...
            // We can use SLM version if `output_batch + default_alignment > 256` because memory and batch are aligned
            // (whether 16 or 64 elements)
            ...
            GPU_DEBUG_TRACE_DETAIL << "FC bf tiled: " << (execute_kernel_idx == 1 ? "SLM" : "Default")
