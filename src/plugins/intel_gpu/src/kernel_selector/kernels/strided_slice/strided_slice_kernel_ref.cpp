...
bool StridedSliceKernelRef::Validate(const Params& p) const {
    ...
    bool shrink_mode =
        std::find(params.shrink_axis_mask.begin(), params.shrink_axis_mask.end(), 1) != params.shrink_axis_mask.end();
    if (shrink_mode) {
        size_t shrinked_axes =
            std::count_if(params.shrink_axis_mask.begin(), params.shrink_axis_mask.end(), [](const uint8_t& v) {
                return v == 1;
            });
        size_t used_out_dims = GetUsedOutDimsCount(params);

        // Count of actual output dims + count of shrinked axes shouldn't exceed 5 to be able to find input index
        // correctly
...
JitConstants StridedSliceKernelRef::GetJitConstants(const strided_slice_params& params) const {
    ...
    bool ellipsis_mode =
        std::find(params.ellipsis_mask.begin(), params.ellipsis_mask.end(), 1) != params.ellipsis_mask.end();
    if (ellipsis_mode) {
        ...
        OPENVINO_ASSERT(dims_indexes.size() == output_rank,
                        "[GPU] Number of indexes is expected to match with output rank");
    } else {
    ...
    if (shrink_mode) {
        ...
        auto get_input_idx_order = [&](std::vector<std::string> bfwzyx_in_order) -> std::string {
            std::string order = bfwzyx_in_order[0] + "," + bfwzyx_in_order[1] + "," + bfwzyx_in_order[2] + "," +
                                bfwzyx_in_order[3] + "," + bfwzyx_in_order[4];
