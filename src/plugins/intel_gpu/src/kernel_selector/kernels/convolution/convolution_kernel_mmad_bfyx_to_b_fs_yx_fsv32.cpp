...
DeviceFeaturesKey ConvolutionKernel_mmad_bfyx_to_b_fs_yx_fsv32::get_required_device_features_key(
...
ConvolutionKernel_mmad_bfyx_to_b_fs_yx_fsv32::AutoTuneOption
ConvolutionKernel_mmad_bfyx_to_b_fs_yx_fsv32::GetAutoTuneOptions(const Params& p, int autoTuneIndex) const {
...
static size_t get_slm_byte_size(const convolution_params& cp, size_t lws, size_t block_size_x, size_t block_size_y) {
    ...
    return (cp.stride.x * (lws * block_size_x - 1) + (cp.weights.X().v - 1) * cp.dilation.x + 1) * input_y_height *
           cp.weights.Z().v * sizeof(int32_t);
}

static size_t get_lws(const convolution_params& cp,
                      size_t blocks_count,
                      size_t block_size_x,
                      size_t block_size_y,
                      size_t max_lws) {
...
ConvolutionKernelBase::DispatchData ConvolutionKernel_mmad_bfyx_to_b_fs_yx_fsv32::SetDefault(
    ...
    dispatchData.gws[2] = cp.outputs[0].Batch().v * CeilDiv(cp.outputs[0].Y().v, dispatchData.cldnnStyle.blockHeight) *
                          cp.outputs[0].Z().v;
...
JitConstants ConvolutionKernel_mmad_bfyx_to_b_fs_yx_fsv32::GetJitConstants(const convolution_params& params,
    ...
    size_t slm_line_size =
        params.stride.x * (dispatchData.lws[1] * blockWidth - 1) + (params.weights.X().v - 1) * params.dilation.x + 1;
    ...
    size_t input_y_height =
        blockHeight != 1 ? params.stride.y * (blockHeight - 1) + (params.weights.Y().v - 1) * params.dilation.y + 1
                         : params.weights.Y().v;
    ...
    if (!params.fused_ops.empty()) {
        ...
        if (WeightsTensor::ChannelsCount(GetPreferredWeightsLayout(params)) == 5) {
            ...
            FusedOpsConfiguration conf1 = {"_1",
                                           {"b", "(fg*32 + lid + 16)", "z", "(y+j)", "(x+i)"},
                                           "res1",
                                           input_dt,
                                           1};
            ...
            if (GetPreferredWeightsLayout(params) == WeightsLayout::os_is_yx_osv32_isv4) {
                ...
                FusedOpsConfiguration conf1 = {"_1",
                                               {"b", "(fg*32 + lid + 16)", "(y+j)", "(x+i)"},
                                               "res1",
                                               input_dt,
                                               1};
                jit.Merge(MakeFusedOpsJitConstants(params, {conf0, conf1}));
            } else {
                FusedOpsConfiguration conf0 = {"_0",
                                               {"b", "(fg*32 + 2*lid + 0)", "(y+j)", "(x+i)"},
                                               "res0",
                                               input_dt,
                                               1};
                FusedOpsConfiguration conf1 = {"_1",
                                               {"b", "(fg*32 + 2*lid + 1)", "(y+j)", "(x+i)"},
                                               "res1",
                                               input_dt,
                                               1};
                jit.Merge(MakeFusedOpsJitConstants(params, {conf0, conf1}));
