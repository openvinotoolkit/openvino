...
ConvolutionKernel_b_fs_yx_fsv16::AutoTuneOption ConvolutionKernel_b_fs_yx_fsv16::GetAutoTuneOptions(
    const Params& params,
...
float ConvolutionKernel_b_fs_yx_fsv16::EstimateOccupancy(const convolution_params& params,
    ...
    auto threads =
        CeilDiv(x, blockWidth) * y * CeilDiv(f, tuning_data.feature_block_size) * tuning_data.slm_div_factor * b;

    return static_cast<float>(threads) / static_cast<float>(params.engineInfo.maxThreadsPerDevice);
}

ConvolutionKernel_b_fs_yx_fsv16::ConvolutionTuningData ConvolutionKernel_b_fs_yx_fsv16::GetTuningParams(
    ...
    bool slm_exception = params.outputs[0].X().v == 3 && params.outputs[0].Y().v == 3 &&
                         params.outputs[0].ElementSize() == 4 && params.outputs[0].Feature().v <= 512;
...
bool ConvolutionKernel_b_fs_yx_fsv16::Validate(const Params& p) const {
    ...
    if (params.groups > 1) {
        ...
        auto multipleGroupsInputPreload = (tuning_data.feature_block_size % outFeaturesPerGroup == 0) &&
                                          ...
                                          (outFeaturesPerGroup != 1) && (inFeaturesPerGroup != 1);
        auto grouped = inFeaturesPerGroup % tuning_data.sub_group_size == 0 &&
                       (outFeaturesPerGroup % tuning_data.sub_group_size == 0 ||
                        tuning_data.sub_group_size % outFeaturesPerGroup == 0);
    ...
    if (input.Feature().pad.before % tuning_data.feature_block_size != 0 ||
...
JitConstants ConvolutionKernel_b_fs_yx_fsv16::GetJitConstants(const convolution_params& params,
    ...
    if (post_reorder_fused(params) && input.GetLayout() == DataLayout::b_fs_yx_fsv16 &&
