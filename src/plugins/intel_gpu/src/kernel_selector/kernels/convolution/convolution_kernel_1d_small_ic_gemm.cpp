...
//
// n_per_lane is 2, not 4: 4 forces TILE_N to 64, and the target layer's
// GEMM_N = 6368 = 2^5 * 199 is not a multiple of it, which would drop the layer
// off the B_IN_BOUNDS fast path (see GetJitConstants) for far more than the saved
// traffic. It also halves the accumulator to 32 registers per lane.
...
JitConstants ConvolutionKernel_1d_small_ic_gemm::GetJitConstants(const convolution_params& params,
    ...
    // fp32 accumulation for every dtype, including int8: the values fit exactly
    // and it keeps one code path. The epilogue stays fp32 too, so bias and
    // activation add no rounding and the only rounding is the final store.
...
ConvolutionKernel_1d_small_ic_gemm::Parent::DispatchData ConvolutionKernel_1d_small_ic_gemm::SetDefault(
    ...
    // One work-group per output tile, one sub-group wide: the tile is
    // TILE_M x TILE_N but only SIMD work items, since each lane holds N_PER_LANE
    // columns x TILE_M rows in its accumulator registers.
