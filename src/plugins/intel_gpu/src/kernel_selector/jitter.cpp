    ...
    JitDefinitions GetDefinitions(const Tensor::TensorBaseT<DType, Layout>& t) const {
        ...
        if (!t.is_dynamic()) {
            ...
            definitions.push_back(
                {_name + "_SIZES_DATA",
                 toVectorString(t.GetDims(), "", KERNEL_SELECTOR_TENSOR_DIM_MAX, 1, [](const Tensor::Dim& d) {
                     return d.v;
                 })});
            definitions.push_back(
                {_name + "_PITCHES",
                 toVectorString(t.GetDims(), "size_t", KERNEL_SELECTOR_TENSOR_DIM_MAX, 1, [](const Tensor::Dim& d) {
                     return d.pitch;
                 })});
        } else {
...
JitDefinitions DataTensorJitConstant::GetDefinitions() const {
    ...
    if (_tensor.is_dynamic()) {
        if (_tensor.GetLayout() == DataLayout::bf || _tensor.GetLayout() == DataLayout::bfyx ||
            ...
            definitions.push_back(
                {_name + "_U_PITCH",
                 toVectorMulString({dims_padded.x(), dims_padded.y(), dims_padded.z(), dims_padded.w()})});
            definitions.push_back(
                {_name + "_V_PITCH",
                 toVectorMulString(
                     {dims_padded.x(), dims_padded.y(), dims_padded.z(), dims_padded.w(), dims_padded.u()})});
    ...
    if (DataTensor::ChannelsCount(layout) <= 4) {
        ...
        if (is_common_4d_layout) {
            ...
            if (_tensor.SimpleLayout()) {
            ...
            } else if (layout == DataLayout::b_fs_yx_fsv16 || layout == DataLayout::b_fs_yx_fsv32 ||
                       layout == DataLayout::b_fs_yx_fsv2 || layout == DataLayout::b_fs_yx_fsv4 ||
                       layout == DataLayout::b_fs_yx_fsv8 || layout == DataLayout::fs_b_yx_fsv32 ||
                       layout == DataLayout::bs_fs_yx_bsv16_fsv16 || layout == DataLayout::bs_fs_yx_bsv16_fsv32 ||
                       layout == DataLayout::bs_fs_yx_bsv4_fsv4 || layout == DataLayout::bs_fs_yx_bsv16_fsv8 ||
                       layout == DataLayout::bs_fs_yx_bsv16_fsv4 || layout == DataLayout::bs_fs_yx_bsv16_fsv2 ||
                       layout == DataLayout::bs_fs_yx_bsv8_fsv4 || layout == DataLayout::bs_fs_yx_bsv8_fsv2 ||
                       layout == DataLayout::bs_fs_yx_bsv4_fsv2 || layout == DataLayout::bs_fs_yx_bsv32_fsv16 ||
                ...
                index_func_val =
                    "GET_DATA_BS_FYX_BSV8_INDEX(" + _name + ", b, f, y, x," + toCodeString(sub_group_size) + ")";
                safe_index_func_val =
                    "GET_DATA_BS_FYX_BSV8_INDEX(" + _name + ", b, f, y, x," + toCodeString(sub_group_size) + ")";
                raw_index_func_val =
                    "GET_DATA_BS_FYX_BSV8_INDEX(" + _name + ", b, f, y, x," + toCodeString(sub_group_size) + ")";
    ...
    if (_tensor.LogicalSize() == 1 && !_tensor.is_dynamic()) {
        ...
        if (_tensor.PitchesDifferFromLogicalDims() && _tensor.SimpleLayout()) {
            ...
            definitions.push_back(
                {safe_index_func_name, "(" + offset + " + ((f) % " + f_size + ")  * " + f_pitch + ")"});
...
class WeightTensorJitConstant : public TensorBaseTJitConstant<WeightsType, WeightsLayout> {
    ...
    struct WeightIndexFuncDesc {
        ...
        WeightIndexFuncDesc(std::string tensor_name, const WeightsLayout l) {
            ...
            if (l == WeightsLayout::oiyx || l == WeightsLayout::ioyx || l == WeightsLayout::oyxi ||
                l == WeightsLayout::oyix || l == WeightsLayout::oxiy || l == WeightsLayout::iyxo ||
                l == WeightsLayout::yxio || l == WeightsLayout::iozyx || l == WeightsLayout::oizyx ||
                l == WeightsLayout::goiyx || l == WeightsLayout::gioyx || l == WeightsLayout::giozyx ||
    ...
    (sub_group_size)*( \
        ...
        ((o) / (sub_group_size))*CAT(prefix, _OFM_PITCH) \
                       ...
                       l == WeightsLayout::os_iyx_osv32__ai32 || l == WeightsLayout::g_os_iyx_osv8 ||
                       l == WeightsLayout::g_os_iyx_osv16 || l == WeightsLayout::g_os_iyx_osv32) {
    ...
    (sub_group_size)*( \
        ...
        ((i) / (sub_group_size))*CAT(prefix, _IFM_PITCH) \
                ...
                args funcArgs =
                    {"o", "i", "z", "y", "x", "x_size", "y_size", "z_size", "i_size", "o_size", "osv_size", "isv_size"};
    ...
    return idx; \
                ...
                if (l == WeightsLayout::os_is_yx_osv16_isv4)
                    this->macroBody =
                        FuncCall(layout_name,
                                 {"o", "i", "y", "x", Cat("_IFM_PITCH"), Cat("_OFM_PITCH"), Cat("_SIZE_X"), "16"});
                else if (l == WeightsLayout::os_is_yx_osv32_isv4)
                    this->macroBody =
                        FuncCall(layout_name,
                                 {"o", "i", "y", "x", Cat("_IFM_PITCH"), Cat("_OFM_PITCH"), Cat("_SIZE_X"), "32"});
            } else {
                // throw error?
            }
        }
        ...
        static const std::string MacroName(std::string tensor_name,
        ...
        static const std::string FuncBody(std::string name,
                                          std::initializer_list<std::string> args = {},
                                          std::string body = "return 0;") {
            std::string args_str = "";
            size_t counter = 0;
            for (auto& arg : args)
                args_str += (++counter == args.size()) ? (arg) : (arg + ", ");
...
public:
    WeightTensorJitConstant(const std::string& name, const WeightsTensor& t)
    ...
    if (WeightsTensor::DoesGroupDimExist(layout)) {
        ...
        if (WeightsTensor::ChannelsCount(layout) <= 4) {
            ...
            if (is_common_4d_layout) {
                ...
                if (layout == WeightsLayout::oiyx || layout == WeightsLayout::ioyx || layout == WeightsLayout::oyxi ||
                    layout == WeightsLayout::oyix || layout == WeightsLayout::oxiy || layout == WeightsLayout::iyxo ||
            ...
            if (is_common_5d_layout) {
                ...
                if (layout == WeightsLayout::oizyx || layout == WeightsLayout::iozyx)
                    ...
                    index_func_val = called_func_name + "(" + _name + ", 0, o, i, z, y, x, 16)";
                else if (layout == WeightsLayout::os_is_zyx_osv32_isv16 ||
                         layout == WeightsLayout::os_is_zyx_osv64_isv16)
...
JitConstants MakeActivationJitConstants(ActivationFunction activation_function,
    ...
    auto type_handler = [use_type_parameter, name](const std::string& prefix,
    ...
    case ActivationFunction::ELU: {
        ...
        jitConstants.AddConstant(
            MakeJitConstant(macro_def, (max_func(input, zero) + (alpha * (exp(min_func(input, zero)) - one))).str()));
        break;
    }
    ...
    case ActivationFunction::HARD_SIGMOID: {
        ...
        jitConstants.AddConstant(
            MakeJitConstant(macro_def, max_func(zero, min_func(one, (JitTerm)((alpha * input + beta).str()))).str()));
        break;
    }
    case ActivationFunction::HSIGMOID: {
        ...
        jitConstants.AddConstant(
            MakeJitConstant(macro_def, (min_func(max_func(zero, input + three), six) / six).str()));
        break;
    }
    case ActivationFunction::SIGN:
        jitConstants.AddConstant(
            MakeJitConstant(macro_def, ternary(input.gt(zero), one, ternary(input.eq(zero), zero, neg(one))).str()));
        break;
    ...
    case ActivationFunction::HSWISH: {
        ...
        const JitTerm six("6." + type_suffix);
        jitConstants.AddConstant(
            MakeJitConstant(macro_def, (input * min_func(max_func(zero, input + three), six) / six).str()));
        break;
    }
    case ActivationFunction::MISH: {
        ...
        jitConstants.AddConstant(
            MakeJitConstant(macro_def, ternary(input.ge(bound), input, common_mish_formula).str()));
        break;
    }
    ...
    case ActivationFunction::NOT:
        jitConstants.AddConstant(MakeJitConstant(
            macro_def,
            ternary(input.eq(zero), one, zero).str()));  // the workaround for OpenCL's vector type result (!input)
        break;
...
JitConstants MakeActivationJitConstants(const base_activation_params& params,
                                        ...
                                        bool disable_type_conversion) {
    auto jitConstants =
        JitConstants{MakeJitConstant("NL_M" + suffix, params.m), MakeJitConstant("NL_N" + suffix, params.n)};
    jitConstants.Merge(
        MakeActivationJitConstants(params.function, out_dt, suffix, use_type_parameter, disable_type_conversion));
    return jitConstants;
}
...
JitConstants MakeActivationJitConstants(std::vector<kernel_selector::base_activation_params> params,
    ...
    JitConstants res = {};
    if (params.empty()) {
        return MakeActivationJitConstants({ActivationFunction::NONE, 0.f, 0.f},
                                          out_dt,
                                          suffix,
                                          use_type_parameter,
                                          disable_type_conversion);
    }
    ...
    for (size_t i = 0; i < params.size(); i++) {
        ...
        jitConstants.Merge(MakeActivationJitConstants(params[i].function,
                                                      out_dt,
                                                      activation_suffix,
                                                      use_type_parameter,
                                                      disable_type_conversion));
        ...
        if (i == 0) {
            ...
            if (convert_input_to_output_dt) {
                // Convert the input to the output data type to fix that cl kernel build failed for an ambiguous issue
                // of the fmax/fmin functions occurring by the different data types between input and output.
                res_activation = "ACTIVATION_FUNC" + activation_suffix + "(" +
                                 (use_type_parameter ? "jit_type, " : "") + "convert_" + toCLType(out_dt) +
                                 "(input), params)";
            } else {
...
JitConstants MakeLoopUnrollParamsJitConstants(uint32_t loopCount) {
    ...
    for (uint32_t i = 2; i <= loopCount + 1; i++) {
        jit.AddConstant({
            MakeJitConstant("LOOP" + toCodeString(i) + "(VAR, STMT)",
                            "LOOP" + toCodeString(i - 1) + "(VAR, STMT); (STMT); (VAR)++;"),
        });
    }
...
JitConstants FusedOpsCodeGenerator::MakeInputDeclsJitConstants(const FusedOpsConfiguration& /*conf*/) const {
    ...
    for (size_t op_input_id = 0; op_input_id < desc.tensors.size(); op_input_id++) {
        ...
        input_decls += "\\\n\tconst __global " + toCLType(desc.tensors[op_input_id].GetDType()) + "* " + ptr_name +
                       (op_input_id == desc.tensors.size() - 1 ? "" : ",");
...
JitConstants FusedOpsCodeGenerator::MakeLoadJitConstants(const FusedOpsConfiguration& conf,
                                                         const DataTensor prim_output) const {
    JitConstants jit = {};
    ...
    // TODO: add some generic way to support shuffled feature, lets say possibility to add separate config for each
    // fused op
    if (desc.GetType() == KernelType::ELTWISE && conf.load_type == FusedOpsConfiguration::LoadType::FEATURE_SHUFFLE) {
        ...
        if (found_sub != std::string::npos)
            fused_op_config.bfzyx_idx_order[1].replace(found_sub,
                                                       sub_group_local_id_str.length(),
                                                       fused_op_config.shuffle_var_name);
    }
...
JitConstants FusedOpsCodeGenerator::MakeOpJitConstants(const FusedOpsConfiguration& conf,
    ...
    auto get_acc_t = [&]() -> Datatype {
        ...
        if (floor_integer_div) {
            if (std::all_of(input_types.begin(), input_types.end(), [=](const Datatype& t) -> bool {
                    return (t != Datatype::F32 && t != Datatype::F16);
                })) {
                types_prioritized = {Datatype::INT64,
                                     Datatype::INT32,
                                     Datatype::UINT32,
                                     Datatype::INT16,
                                     Datatype::UINT16,
    ...
    auto get_input = [&](size_t index) -> std::string {
        ...
        auto input_name = (dep.dep_type == kernel_selector::DepType::ORIGINAL) ? in_var
                          : (dep.dep_type == kernel_selector::DepType::INTERNAL)
                              ? GetOutputVarName(in_var, dep.op_id)
    ...
    case KernelType::ELTWISE: {
        ...
        if (floor_integer_div) {
            ...
            op_decls += "\\\n\t" + tmp_var + " -= " + "((" + tmp_var_rem + " != 0 && (" + input_vars[0] + " < 0) != (" +
                        input_vars[1] + " < 0)) ? 1 : 0);";
        }
        op_decls +=
    ...
    case KernelType::QUANTIZE: {
        ...
        std::string in_converted =
            (first_fused_ops_idx < 0) ? in_var : GetOutputVarName(in_var, dep_data[first_fused_ops_idx].op_id);
        ...
        if (p->per_tensor_output_range && p->out_lo < p->out_hi) {
            ...
            // Round operation isn't needed if output type is int8/uint8 and scale coefficient in all output channels is
            // equal to 1.0
            bool output_type_is_int8 =
            ...
            op_decls += "\\\n\t" + GetOutputType(vec_size) + " " + out_var + " = " +
                        ConvertToOutputTypeSat(tmp_var, vec_size) + ";";
            break;
        } else {
            // Input range
            auto in_lo =
                p->per_tensor_input_range
            ...
            if (p->has_clamp) {
                if (p->has_min_clamp && p->has_max_clamp)
                    op_decls += "\\\n\t" + tmp_type_str + " " + tmp_var + " = clamp(" + in_converted + ", " + in_lo +
                                ", " + in_hi + ");";
                else if (p->has_min_clamp)
                    op_decls +=
    ...
    case KernelType::ACTIVATION: {
        ...
        std::string new_in_var =
            (first_fused_ops_idx < 0) ? in_var : GetOutputVarName(in_var, dep_data[first_fused_ops_idx].op_id);
        op_decls += "\\\n\t" + GetOutputType(vec_size) + " " + out_var + " = " +
                    ConvertToOutputType(new_in_var, vec_size) + ";";
        if (activation_p.function != ActivationFunction::NONE) {
            ...
            // Disable type casts in activation, since current jit generator for activation don't respect vector size of
            // parameters. So conversion is explicitly done in params declaration
...
std::string FusedOpsCodeGenerator::GetIdx(size_t input_id, idx_desc idx, bool should_be_safe) const {
    ...
    if (DataTensor::ChannelsCount(desc.tensors[input_id].GetLayout()) <= 4) {
        ...
        idx_order =
            idx.b + "," + idx.f + "," + idx.v + "," + idx.u + "," + idx.w + "," + idx.z + "," + idx.y + "," + idx.x;
    }
...
std::string FusedOpsCodeGenerator::GetJitLoad(const FusedOpsConfiguration& conf,
    ...
    bool valid_broadcast_case =
        input_tensor.LogicalSize() == prim_output.Feature().v || input_tensor.LogicalSize() == 1;
    ...
    // Fsv16 Eltwise whcih requires f axis broadcast such as input[1,1,z,1,1], output[b,f,z,y,x] need to use LT unligned
    // read. In this case, intel_sub_group_block_read() introduces increasing index in feature block.
    ...
    if (desc.GetType() == KernelType::ELTWISE && conf.load_type == FusedOpsConfiguration::LoadType::LT_ALIGNED_READ &&
        ...
        std::string sub_group_local_id_str = "get_sub_group_local_id";
        size_t found_sub = conf.bfzyx_idx_order[1].rfind(sub_group_local_id_str);
        OPENVINO_ASSERT(found_sub == std::string::npos,
                        "[GPU] LT_ALIGNED_READ LoadType is used with get_sub_group_local_id.");
        ...
        if (vec_size > 1) {
            ...
            OPENVINO_ASSERT(vec_axis_idx != -1,
                            "[GPU] Incorrect vec_axis value ",
                            static_cast<int>(conf.vec_axis),
        ...
        if (vec_size > 1) {
            std::string load_str =
                "0;";  // Assign zero to initial variable (GetInputVarName(input_id)) and modify it in the loop below
            load_str += "for (uint loop_var = 0; loop_var < " + std::to_string(vec_size) + "; loop_var++) { ";
            load_str += GetInputVarName(input_id) + "[loop_var] = " + GetInputPtrName(input_id) + "[" +
                        new_index_func_call + "]; }";
            return load_str;
        } else {
    ...
    std::string index_func_call_vec =
        reuse_index ? reused_idx : GetIdx(input_id, idx_desc{idx, desc.tensors[input_id]}, safe_load);
    std::string index_func_call =
        reuse_index ? reused_idx : GetIdx(input_id, idx_desc{idx, desc.tensors[input_id]}, safe_load);
    if (conf.index_type == FusedOpsConfiguration::IndexType::LINEAR_OFFSET) {
        ...
        if (conf.load_type == FusedOpsConfiguration::LoadType::LT_ALIGNED_READ) {
            ...
            if (input_dt == Datatype::F32 || input_dt == Datatype::INT32 || input_dt == Datatype::UINT32) {
                block_read = CastToType(" _sub_group_block_read" + vs + "(" + "(const __global uint*)(" +
                                            GetInputPtrName(input_id) + " + " + index_func_call_vec + "))",
                                        input_dt,
                                        vec_size);
            } else if (input_dt == Datatype::F16) {
                block_read = CastToType(" _sub_group_block_read_us" + vs + "(" + "(const __global ushort*)(" +
                                            GetInputPtrName(input_id) + " + " + index_func_call_vec + "))",
                                        input_dt,
                                        vec_size);
            } else if (input_dt == Datatype::UINT8 || input_dt == Datatype::INT8) {
                block_read = CastToType(" _sub_group_block_read_uc" + vs + "(" + "(const __global uchar*)(" +
                                            GetInputPtrName(input_id) + " + " + index_func_call_vec + "))",
                                        input_dt,
                                        vec_size);
            } else {
...
std::string FusedOpsCodeGenerator::GetInputVarName(size_t input_id, bool is_shuffled, std::string shuffle_var) const {
    if (is_shuffled)
        return "_sub_group_shuffle(" + GetTypeStr() + toCodeString(desc.op_id) + "_data" + toCodeString(input_id) +
               ", " + shuffle_var + ")";
