...
USMHostTensor::USMHostTensor(std::shared_ptr<RemoteContextImpl> context,
                             const element::Type element_type,
                             const Shape& shape)
    : m_impl(std::make_shared<RemoteTensorImpl>(context, shape, element_type, TensorType::BT_USM_HOST_INTERNAL)) {}

USMHostTensor::USMHostTensor(std::shared_ptr<RemoteTensorImpl> tensor) : m_impl(tensor) {}
...
void USMHostTensor::set_memory(std::shared_ptr<RemoteTensorImpl> tensor) {
    OPENVINO_ASSERT(tensor->get_original_memory()->get_allocation_type() == cldnn::allocation_type::usm_host,
                    "[GPU] Unexpected allocation type");
    m_impl = tensor;
}
