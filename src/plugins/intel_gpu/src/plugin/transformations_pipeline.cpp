...

    {
        ...
        auto isCellPrimitiveSupported = [](const_node_ptr &node) -> bool {
            if (ov::as_type_ptr<const ov::op::v0::RNNCell>(node)) {
                ...
                const float clip = lstm_cell_v1->get_clip();
                return (clip == 0.0f || std::isinf(clip)) && lstm_cell_v1->get_activations() == std::vector<std::string>{"sigmoid", "tanh", "tanh"};
            }
            return false;
        };
        ...
        auto isSequencePrimitiveSupported = [](const_node_ptr &node) -> bool {
            ...
            if (ov::as_type_ptr<const ov::op::v5::RNNSequence>(node)) {
                ...
                // clip == 0 and clip == inf both mean "no clipping" (see RNNCellBase::clip), so treat inf as no-clip
                const float clip = gru_seq->get_clip();
                return (clip == 0.0f || std::isinf(clip)) && gru_seq->get_activations() == std::vector<std::string>{"sigmoid", "tanh"} && max_seq_len != 1 &&
                ...
                // clip == 0 and clip == inf both mean "no clipping" (see RNNCellBase::clip), so treat inf as no-clip
                const float clip = lstm_seq->get_clip();
                return ((clip == 0.0f || std::isinf(clip)) && lstm_seq->get_activations() == std::vector<std::string>{"sigmoid", "tanh", "tanh"} &&
