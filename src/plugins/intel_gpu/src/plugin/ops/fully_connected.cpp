...
static void CreateFullyConnectedCompressedOp(ProgramBuilder& p,
                                             const std::shared_ptr<op::FullyConnectedCompressed>& op) {
    ...
    if (op->get_input_size() == INPUT_CNT_WITH_ZP) {
        auto zp_const =
            std::dynamic_pointer_cast<ov::op::v0::Constant>(op->get_input_node_shared_ptr(INPUT_CNT_WITH_ZP - 1));
