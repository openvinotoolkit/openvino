...
static void CreateRangeOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v4::Range>& op) {
    ...
    if (p.use_new_shape_infer()) {
        range_prim = std::make_shared<cldnn::range>(layer_type_name_ID(op), p.GetInputInfo(op), output_dtype);
    } else {
        auto outLayout = cldnn::layout{output_pshape, output_dtype, cldnn::format::bfyx};
        range_prim = std::make_shared<cldnn::range>(layer_type_name_ID(op), p.GetInputInfo(op), outLayout);
    }
