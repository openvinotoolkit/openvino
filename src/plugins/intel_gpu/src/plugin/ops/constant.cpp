...
size_t get_max_experts(const ExecutionConfig& config) {
...
partial_upload_log_state& get_partial_upload_log_state() {
    static partial_upload_log_state state;
    return state;
}

partial_upload_plan prepare_partial_upload_plan(ProgramBuilder& p,
...
void initialize_constant_memory(char* dst,
                                const std::shared_ptr<ov::op::v0::Constant>& op,
                                cldnn::data_types out_dtype,
                                size_t upload_bytes,
...
static void create_data(ProgramBuilder& p, const ov::Shape& const_shape, const std::shared_ptr<ov::op::v0::Constant>& op, const ConstProperties& props) {
    ...
    if (bufIter != p.blobMemCache.end()) {
        ...
        auto upload_plan = prepare_partial_upload_plan(p, op, const_shape, out_dtype, constFormat, constLayout);
        auto mem = upload_plan.memory;
        ...
        auto buf = lock.data();
        initialize_constant_memory(buf,
                                   op,
                                   out_dtype,
                                   upload_plan.upload_bytes,
                                   upload_plan.upload_shape,
                                   upload_plan.skip_initial_copy);
