...
#include "moe_offload_constant_utils.hpp"
...
static void create_data(ProgramBuilder& p, const ov::Shape& const_shape, const std::shared_ptr<ov::op::v0::Constant>& op, const ConstProperties& props) {
    ...
    if (bufIter != p.blobMemCache.end()) {
        ...
        auto upload_plan = moe_offload::prepare_partial_upload_plan(p, op, const_shape, out_dtype, constFormat, constLayout);
        ...
        auto buf = lock.data();
        moe_offload::initialize_constant_memory(buf,
                                                op,
                                                out_dtype,
