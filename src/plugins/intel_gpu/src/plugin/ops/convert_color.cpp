...
static void CreateCommonConvertColorOp(ProgramBuilder& p,
    ...
    if (op->get_input_node_ptr(0)->output(0).get_rt_info().count(
            ov::preprocess::TensorInfoMemoryType::get_type_info_static())) {
        std::string mem_type = op->get_input_node_ptr(0)
                                   ->output(0)
                                   .get_rt_info()
    ...
    if (batch.is_static() && batch.get_length() > 1 && memory_type == cldnn::convert_color::memory_type::image) {
        ...
        for (int64_t b = 0; b < batch.get_length(); ++b) {
            ...
            p.add_primitive(*op,
                            cldnn::convert_color(batched_prim_id, batched_inputs, from_color, to_color, memory_type));
        }
        p.add_primitive(*op, cldnn::concatenation(layerName, convert_color_names, 0));
    } else {
        p.add_primitive(*op, cldnn::convert_color(layerName, inputs, from_color, to_color, memory_type));
    }
}

static void CreateNV12toRGBOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v8::NV12toRGB>& op) {
    validate_inputs_count(op, {1, 2});
    CreateCommonConvertColorOp(p,
                               op,
