...
static void CreateScatterElementsUpdateOp(ProgramBuilder& p,
                                          const std::shared_ptr<op::util::ScatterElementsUpdateBase>& op) {
    ...
    int64_t axis = ov::util::normalize_axis(op.get(),
                                            axes_constant->cast_vector<int64_t>()[0],
    ...
    auto primitive =
        cldnn::scatter_elements_update(layerName, inputs[0], inputs[1], inputs[2], axis, mode, use_init_val);
