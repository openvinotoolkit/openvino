...
using PagedAttentionWithQQBias = ov::op::paged_attention_with_qq_bias;
}  // namespace internal
}  // namespace op
}  // namespace ov
...
static void CreatePagedAttentionWithQQBiasOp(ProgramBuilder& p, const std::shared_ptr<ov::op::paged_attention_with_qq_bias>& op) {
    validate_inputs_count(op, {22});
...
REGISTER_FACTORY_IMPL(internal, PagedAttentionWithQQBias);
}  // namespace ov::intel_gpu
