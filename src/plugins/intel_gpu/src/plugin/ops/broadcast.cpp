...
static void CreateCommonBroadcastOp(ProgramBuilder& p,
                                    const std::shared_ptr<ov::Node>& op,
                                    const ov::AxisSet axis_mapping) {
    ...
    if (input_rank != output_rank && input_pshape.is_static() && output_pshape.is_static() &&
    ...
    if (auto broadcast_v3 = std::dynamic_pointer_cast<ov::op::v3::Broadcast>(op)) {
        ...
        case ov::op::AutoBroadcastType::NONE:
            mode = ov::op::BroadcastType::NONE;
            break;
        case ov::op::AutoBroadcastType::NUMPY:
            mode = ov::op::BroadcastType::NUMPY;
            break;
        case ov::op::AutoBroadcastType::PDPD:
            mode = ov::op::BroadcastType::PDPD;
            break;
        default:
    ...
    if (output_pshape.is_static()) {
        broadcast_prim =
            std::make_shared<cldnn::broadcast>(layerName, input, output_pshape.to_shape(), axis_mapping, mode);
    } else {
        broadcast_prim = std::make_shared<cldnn::broadcast>(layerName, input, inputs[1], axis_mapping, mode);
    }
...
static void CreateBroadcastOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v1::Broadcast>& op) {
    ...
    if (op->get_broadcast_spec().m_type == ov::op::AutoBroadcastType::NONE && op->get_input_size() == 3) {
        ...
        OPENVINO_ASSERT(axis_mapping_node != nullptr,
