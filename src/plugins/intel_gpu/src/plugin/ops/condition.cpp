...
static cldnn::condition::branch gen_branch(ProgramBuilder& p, const std::shared_ptr<ov::op::v8::If>& op, size_t idx) {
    ...
    //GPU_DEBUG_LOG(config) << "Generate inner program for " << "op::v"
    ...
    //GPU_DEBUG_LOG(config) << op->get_friendly_name() << " branch_info[" << internal_body->get_friendly_name() << "] : " << branch << std::endl;
    return branch;
}
