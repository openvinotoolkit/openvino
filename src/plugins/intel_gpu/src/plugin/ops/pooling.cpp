...
static void CreateAvgPoolOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v1::AvgPool>& op) {
    ...
    if (p.use_new_shape_infer()) {
        pooling_prim = std::make_shared<cldnn::pooling>(
            layerName,
            inputs[0],
...
static void CreateMaxPoolOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v1::MaxPool>& op) {
    ...
    if (p.use_new_shape_infer()) {
        ...
        pooling_prim =
            std::make_shared<cldnn::pooling>(layerName,
                                             inputs[0],
                                             cldnn::pooling_mode::max,
                                             op->get_kernel(),
                                             op->get_strides(),
...
static void CreateMaxPoolOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v8::MaxPool>& op) {
    ...
    auto indices_mutable_prim = cldnn::mutable_data(maxpool_mutable_id_w, shared_memory);
    ...
    auto indices_mutable_id_r =
        cldnn::mutable_data(maxpool_mutable_id_r, {cldnn::input_info(layerName)}, shared_memory);
