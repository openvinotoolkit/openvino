...
cldnn::data CreateScalarDataPrimitive(ProgramBuilder& p, const cldnn::primitive_id& name, T value) {
    auto mem =
        p.get_engine().allocate_memory(cldnn::layout{element::from<T>(), cldnn::format::bfyx, {1, 1, 1, 1}}, false);
...
cldnn::data CreateShapeDataPrimitive(ProgramBuilder& p, const cldnn::primitive_id& name, Shape& value) {
    auto mem = p.get_engine().allocate_memory(
        cldnn::layout{element::Type_t::i64, cldnn::format::bfyx, {1, 1, 1, static_cast<int>(value.size())}},
        false);
    cldnn::mem_lock<int8_t> host_mem{mem, p.get_engine().get_service_stream()};
...
static void CreateMultinomialOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v13::Multinomial>& op) {
    ...
    cldnn::slice slice_prim{slice, {cumsum, slice_start, slice_stop, slice_step, slice_axis}};
    ...
    cldnn::random_uniform random_prim{random,
    ...
    OPENVINO_ASSERT(const_num_samples != nullptr,
                    "[GPU] Unsupported num_samples node type in ",
    ...
    cldnn::multinomial multinomial_prim{layer_type_name_ID(op),
                                        cdf_reshaped,
                                        random,
                                        op->get_convert_type(),
