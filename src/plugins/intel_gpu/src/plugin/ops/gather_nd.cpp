...
static void CreateGatherNDOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v5::GatherND>& op) {
    ...
    auto primitive = cldnn::gather_nd(layerName, inputs[0], inputs[1], input_rank, indices_rank, batch_dims);
...
static void CreateGatherNDOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v8::GatherND>& op) {
    ...
    auto primitive = cldnn::gather_nd(layerName, inputs[0], inputs[1], input_rank, indices_rank, batch_dims, false);
