...
template <typename T>
void CreateGatherOpBase(ProgramBuilder& p, const std::shared_ptr<T>& op, const int64_t batch_dim = 0, bool support_neg_ind = false, bool weights_compressed = false) {
    ...
    if (is_static && axis == 0 && input_rank > 1 && indices.get_partial_shape().rank().get_length() == 0 &&
        ...
        if (!weights_compressed) {
            ...
            auto gatherPrim = cldnn::gather(layerName,
                                            ...
                                            reordered_inputs[1],
                                            axis,
                                            reordered_inputs[3].pid,
                                            has_scalar_zp ? "" : reordered_inputs[4].pid,
