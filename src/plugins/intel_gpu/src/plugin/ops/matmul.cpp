...
static void CreateMatMulOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v0::MatMul>& op) {
    ...
    std::array<ov::PartialShape, 2> inputShapes{op->get_input_partial_shape(0), op->get_input_partial_shape(1)};

    auto canTransposeInputs =
        [&](const std::array<ov::PartialShape, 2>& shapes, bool transA, bool transB, ov::element::Type type) -> bool {
        if (!transA && !transB)
            return false;

        // dynamic shapes and 1D tensors are not transposed
        if (shapes[0].is_dynamic() || shapes[1].is_dynamic()) {
            // Currently, cldnn optimized gemm kernel (gemm_tiled_opt) does not support transposed input with shape
            // unaligned for 16. If the shape is not aligned for 16, gemm_ref_kernel will be selected, but the perf is
            // worse than permute + gemm_tiled_opt. So we'll use this permute + gemm_tiled_opt strategy as a temporal
            // solution, until we have an essential solution, i.e., fixing the gemm_tiled_opt kernel to support
            // unaligned shape.
        ...
        bool inputsAligned = std::all_of(shapes[0].rbegin(),
                                         shapes[0].rbegin() + 2,
                                         [](const ov::Dimension& dim) {
                                             return dim.is_static() && dim.get_length() % 16 == 0;
                                         }) &&
                             std::all_of(shapes[1].rbegin(), shapes[1].rbegin() + 2, [](const ov::Dimension& dim) {
                                 return dim.is_static() && dim.get_length() % 16 == 0;
                             });
        if (inputsAligned)
            return false;
        ...
        bool in0_large = std::all_of(shapes[0].rbegin(), shapes[0].rbegin() + 2, [](const ov::Dimension& dim) {
            return dim.is_static() && dim.get_length() >= 64;
        });
        bool in1_large = std::all_of(shapes[1].rbegin(), shapes[1].rbegin() + 2, [](const ov::Dimension& dim) {
            return dim.is_static() && dim.get_length() >= 64;
        });
        // Optimized for clDNN
        ...
        bool needs_to_transpose_inputs =
            (in0_very_large || in1_very_large) && !is_u8_i8 && !p.get_engine().get_device_info().supports_immad;

        return (in0_large && in1_large) || needs_to_transpose_inputs;
    };

    auto transposeInput = [](ProgramBuilder& p,
                             const std::shared_ptr<ov::Node>& op,
                             const ov::PartialShape& shape,
                             const std::string& suffix,
                             const cldnn::primitive_id& primitiveId) -> cldnn::input_info {
        ...
        auto permutePrim = cldnn::permute(permuteName, cldnn::input_info(primitiveId), transposeOrder);
...
static void CreateGemmOp(ProgramBuilder& p, const std::shared_ptr<ov::op::internal::Gemm>& op) {
    ...
    OPENVINO_ASSERT(rank_a == op->get_input0_transpose_order().size(),
                    "[GPU] Length of input0_transpose_order is not same as rank of input0");
    OPENVINO_ASSERT(rank_b == op->get_input1_transpose_order().size(),
                    "[GPU] Length of input1_transpose_order is not same as rank of input1");
    OPENVINO_ASSERT(output_rank == op->get_output_transpose_order().size(),
                    "[GPU] Length of output_transpose_order is not same as rank of output");
