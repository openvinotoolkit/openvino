...
static void CreateLSTMCellOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v4::LSTMCell>& op) {
    ...
    if (op->get_input_partial_shape(0).is_static() && op->get_input_partial_shape(1).is_static() && op->get_input_partial_shape(2).is_static() && \
        op->get_input_partial_shape(3).is_static() && op->get_input_partial_shape(4).is_static()) {
        if (op->get_input_shape(0).size() != 2 || op->get_input_shape(1).size() != 2 || op->get_input_shape(2).size() != 2 || \
            op->get_input_shape(3).size() != 2 || op->get_input_shape(4).size() != 2) {
    ...
    if (p.use_new_shape_infer()) {
        auto prim =  cldnn::lstm_cell(layerName+".out0", inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], inputs[5], \
         cldnn::input_info(), "", "", clip, false, activations, \
    ...
    cldnn::input_info(), layerName + "_md_write1", "", clip, false, activations, \
                                        activation_params, cldnn::lstm_weights_order::fizo));
