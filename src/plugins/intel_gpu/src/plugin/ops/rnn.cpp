...
static void CreateLSTMCellOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v4::LSTMCell>& op) {
    ...
    if (p.use_new_shape_infer()) {
        ...
        p.add_primitive(*op,
                        cldnn::fully_connected(lstm_fc_id, cldnn::input_info(input_concatID), wr_concat_id, bias.pid));
        p.add_primitive(*op,
                        cldnn::lstm_elt(lstm_elt_id,
                                        cldnn::input_info(lstm_fc_id),
                                        inputs[2].pid,
                                        clip,
                                        0,
                                        activations,
        ...
        p.add_primitive(*op,
                        cldnn::crop(outputHiddenCropID,
                                    ...
                                    num_splits));
        p.add_primitive(*op,
                        cldnn::reshape(outputHiddenID,
                                       cldnn::input_info(outputHiddenCropID),
                                       false,
                                       outSzPt,
        ...
        p.add_primitive(*op,
                        cldnn::crop(outputCellCropID,
                                    ...
                                    num_splits));
        p.add_primitive(*op,
                        cldnn::reshape(outputCellID,
                                       cldnn::input_info(outputCellCropID),
                                       false,
                                       outSzPt,
                                       op->get_output_partial_shape(1)));
    } else {
        ...
        p.add_primitive(*op, cldnn::concatenation(input_concatID, {permuteID, hiddenInStr}, 3));
        ...
        p.add_primitive(*op,
                        cldnn::fully_connected(lstm_fc_id, cldnn::input_info(FCInputReshapeID), WRconcatID, bias.pid));
        ...
        p.add_primitive(*op,
                        cldnn::lstm_elt(lstm_elt_id,
                                        cldnn::input_info(gemmReorderID),
                                        cellInStr,
                                        clip,
                                        0,
                                        activations,
        ...
        p.add_primitive(
            *op,
            cldnn::crop(outputHiddenCropID, cldnn::input_info(lstm_elt_id), hiddenSz, cldnn::tensor{0, 0, 0, 0}));
...
static void CreateLSTMSequenceOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v5::LSTMSequence>& op) {
...

    {
        ...
        if (in_dims0.size() != 3 || op->get_input_shape(1).size() != 3 || op->get_input_shape(2).size() != 3)
    ...
    for (int i = 0; i < lstm_sequence_len; ++i) {
        ...
        p.add_primitive(
            *op,
            cldnn::concatenation(concatID, {cldnn::input_info(inputCrop_id), cldnn::input_info(hiddenStr)}, 3));

        cldnn::tensor fc_input_resh_tensor = {crop_tensor.batch[0],
                                              crop_tensor.spatial[0] + inStateShape.spatial[0],
                                              crop_tensor.feature[0],
                                              crop_tensor.spatial[1]};
        ...
        p.add_primitive(*op,
                        cldnn::lstm_elt(lstm_elt_id,
                                        cldnn::input_info(lstm_fc_reor_id),
                                        cellStr,
                                        clip,
                                        0,
                                        activations,
        ...
        p.add_primitive(*op,
                        cldnn::crop(hiddenStr, cldnn::input_info(lstm_elt_id), hiddenSz, cldnn::tensor{0, 0, 0, 0}));
    ...
    if (!isForward)
        std::reverse(output_ids_offsets.begin(), output_ids_offsets.end());
    ...
    p.add_primitive(*op,
                    cldnn::reshape(layerName + ".out0", concatStr, tensor_from_dims(op->get_output_shape(0))),
                    {layerName});
