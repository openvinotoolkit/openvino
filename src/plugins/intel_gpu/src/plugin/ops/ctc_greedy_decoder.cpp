...
static void CreateCommonCTCGreedyDecoderOp(ProgramBuilder& p,
    ...
    if (p.use_new_shape_infer()) {
        ...
        if (reordered_inputs.size() == 3) {
            ...
            if (!blank_index_node) {
                OPENVINO_THROW("Unsupported blank_index node type in ",
            ...
            float val;
            if (ov::shape_size(blank_index_node->get_output_shape(0)) != 1 ||
                !ov::op::util::get_single_value(blank_index_node, val)) {
        ...
        auto primitive = cldnn::ctc_greedy_decoder(layer_type_name_ID(op),
                                                   reordered_inputs,
                                                   blank_index,
                                                   ctc_merge_repeated,
        ...
        auto primitive = cldnn::ctc_greedy_decoder(CTCGreedyDecoderLayerName,
                                                   reordered_inputs,
                                                   blank_index,
                                                   ctc_merge_repeated,
...
static void CreateCTCGreedyDecoderSeqLenOp(ProgramBuilder& p,
                                           const std::shared_ptr<ov::op::v6::CTCGreedyDecoderSeqLen>& op) {
