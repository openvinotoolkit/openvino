...
std::shared_ptr<ov::Model> Graph::get_runtime_model(std::vector<cldnn::primitive_info>& primitives_info, bool filter_const_primitives) {
    ...
    auto create_ov_node = [&](const cldnn::primitive_info& prim_info) {
        ...
        if (prim_info.type_id == "grouped_matmul") {
            auto& node = get_network()->get_primitive(prim_info.original_id)->get_node();
            ...
            if (gm_prim->compressed_weights) {
                ...
                if (gm_prim->decompression_zero_point.is_valid()) {
                    // zp follows scale in dependencies: base_inputs(3) + scale(1) = idx 4
        ...
        if (prim_info.type_id == "fully_connected") {
            auto& node = get_network()->get_primitive(prim_info.original_id)->get_node();
            ...
            if (fc_prim->decompression_scale.is_valid()) {
                ...
                if (fc_prim->decompression_zero_point.is_valid()) {
                    // deps order: input(1) + weights + [bias] + scale + zp
