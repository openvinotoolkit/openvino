...
Parameter Plugin::GetMetric(const std::string& name, const std::map<std::string, Parameter>& options) const {
    ...
    if (name == ov::supported_properties) {
        ...
        if (model_param == options.end()) {
            GPU_DEBUG_LOG(1) << "[OPTIMAL_BATCH_SIZE] ov::hint::model is not set: return 1" << std::endl;
        ...
        GPU_DEBUG_LOG(1) << memPressure.max_mem_tolerance << std::endl;
        GPU_DEBUG_LOG(1) << "MAX_BATCH: " << max_batch_size << std::endl;
        GPU_DEBUG_LOG(1) << "ACTUAL OPTIMAL BATCH: " << batch << std::endl;
        ...
        GPU_DEBUG_LOG(2) << "[GPU_MAX_BATCH_SIZE] available memory is " << available_device_mem
                         << " (occupied: " << occupied_device_mem << ")" << std::endl;

        int64_t max_batch_size = 1;

        if (options.find(ov::hint::model.name()) == options.end()) {
            GPU_DEBUG_LOG(1) << "[GPU_MAX_BATCH_SIZE] MODELS_PTR is not set: return 1" << std::endl;
        ...
        GPU_DEBUG_LOG(1) << "[GPU_MAX_BATCH_SIZE] n_streams : " << n_streams << std::endl;
        ...
        if (available_device_mem_it != options.end()) {
            if (available_device_mem_it->second.is<int64_t>()) {
                ...
                GPU_DEBUG_LOG(2) << "[GPU_MAX_BATCH_SIZE] available memory is reset by user " << available_device_mem << std::endl;
            } else {
        ...
        try {
            ...
            for (size_t input_id = 0; input_id < params.size(); input_id++) {
                ...
                if (shape.is_dynamic()) {
                    GPU_DEBUG_LOG(2) << "[MAX_BATCH_SIZE] does not support dynamic networks" << std::endl;
                    return decltype(ov::max_batch_size)::value_type {static_cast<uint32_t>(max_batch_size)};
                }
                ...
                if (shape.size()) {
                    for (size_t s = 0; s < shape.size(); s++) {
                        if (ov::DimensionTracker::get_label(shape[s])) {
                            ...
                            GPU_DEBUG_LOG(2) << "[MAX_BATCH_SIZE] detected batched input " << batched_input_id
                                             << "[" << s << "]" << std::endl;
            ...
            if (!batched_inputs.size()) {
                GPU_DEBUG_LOG(2) << "[MAX_BATCH_SIZE] MAX_BATCH_SIZE supports only networks with inputs/outputs featuring batched dim." << std::endl;
                return decltype(ov::max_batch_size)::value_type {static_cast<uint32_t>(max_batch_size)};
            }
            ...
            try {
                ...
                GPU_DEBUG_LOG(1) << "[MAX_BATCH_SIZE] Error at reshape to " << base_batch_size << std::endl;
                return decltype(ov::max_batch_size)::value_type {static_cast<uint32_t>(max_batch_size)};
            }
            ...
            GPU_DEBUG_LOG(1) << "[GPU_MAX_BATCH_SIZE] Base batch size: " << base_batch_size  << std::endl;
            GPU_DEBUG_LOG(1) << "[GPU_MAX_BATCH_SIZE] Const mem usage: " << device_memory_usage.first  << std::endl;
            GPU_DEBUG_LOG(1) << "[GPU_MAX_BATCH_SIZE] General mem usage: " << device_memory_usage.second  << std::endl;
        } catch (std::exception& e) {
            GPU_DEBUG_LOG(1) << "[GPU_MAX_BATCH_SIZE] Failed in reshape or build program " << e.what() << std::endl;
        }
        return decltype(ov::max_batch_size)::value_type {static_cast<uint32_t>(max_batch_size)};
