...
void Plugin::transform_model(std::shared_ptr<ov::Model>& model,
    ...
    GPU_DEBUG_LOG << "Transformations time: " << std::chrono::duration_cast<ms>(Time::now() - start).count() << " ms"
                  << std::endl;
}
...
std::map<std::string, RemoteContextImpl::Ptr> Plugin::get_default_contexts() const {
    std::call_once(m_default_contexts_once, [this]() {
        ...
        for (auto& device : m_device_map) {
            auto ctx = std::make_shared<RemoteContextImpl>(get_device_name() + "." + device.first,
                                                           std::vector<cldnn::device::ptr>{device.second});
...
std::shared_ptr<ov::ICompiledModel> Plugin::compile_model(const std::shared_ptr<const ov::Model>& model,
    ...
    OPENVINO_ASSERT(m_configs_map.find(device_id) != m_configs_map.end(),
                    "[GPU] compile_model: Couldn't find config for GPU with id ",
                    device_id);
...
std::shared_ptr<ov::ICompiledModel> Plugin::compile_model(const std::shared_ptr<const ov::Model>& model,
    ...
    OPENVINO_ASSERT(m_configs_map.find(device_id) != m_configs_map.end(),
                    "[GPU] LoadExeNetworkImpl: Couldn't find config for GPU with id ",
                    device_id);
...
ov::Any Plugin::get_property(const std::string& name, const ov::AnyMap& options) const {
    ...
    OPENVINO_ASSERT(!m_device_map.empty(),
                    "[GPU] Can't get ",
                    name,
                    " property as no supported devices found or an error happened during devices query.\n"
...
ov::Any Plugin::get_metric(const std::string& name, const ov::AnyMap& options) const {
    ...
    if (name == ov::intel_gpu::device_total_mem_size) {
        ...
        auto dev_type = device_info.dev_type == cldnn::device_type::discrete_gpu ? ov::device::Type::DISCRETE
                                                                                 : ov::device::Type::INTEGRATED;
        ...
        return static_cast<decltype(ov::intel_gpu::execution_units_count)::value_type>(
        ...
        if (device_info.gfx_ver.major == 0 && device_info.gfx_ver.minor == 0 && device_info.gfx_ver.revision == 0) {
            ...
            s << static_cast<int>(device_info.gfx_ver.major) << "." << static_cast<int>(device_info.gfx_ver.minor)
        ...
        deviceName +=
            std::string(" (") + (device_info.dev_type == cldnn::device_type::discrete_gpu ? "dGPU" : "iGPU") + ")";
        ...
        std::tuple<unsigned int, unsigned int> range =
            std::make_tuple(1, device_info.num_ccs == 1 ? 2 : device_info.num_ccs);
        ...
        if (device_info.gfx_ver.major == 0 && device_info.gfx_ver.minor == 0) {
            ...
            s << "v" << static_cast<int>(device_info.gfx_ver.major) << "."
              << static_cast<int>(device_info.gfx_ver.minor) << "." << static_cast<int>(device_info.gfx_ver.revision);
        }
...
uint32_t Plugin::get_max_batch_size(const ov::AnyMap& options) const {
    ...
    auto statistic_result =
        get_metric(ov::intel_gpu::memory_statistics.name(), options).as<std::map<std::string, uint64_t>>();
    ...
    if (available_device_mem_it != options.end()) {
        if (available_device_mem_it->second.is<int64_t>()) {
            available_device_mem =
                std::min(static_cast<int64_t>(available_device_mem), available_device_mem_it->second.as<int64_t>());
            GPU_DEBUG_LOG << "[GPU_MAX_BATCH_SIZE] available memory is reset by user " << available_device_mem
                          << std::endl;
        } else {
            OPENVINO_THROW(
                "[GPU_MAX_BATCH_SIZE] bad casting: ov::intel_gpu::hint::available_device_mem should be int64_t type");
        }
        if (available_device_mem < 0) {
            OPENVINO_THROW("[GPU_MAX_BATCH_SIZE] ov::intel_gpu::hint::available_device_mem value should be greater "
                           "than 0 for max batch size calculation");
        }
    }
    ...
    GPU_DEBUG_IF(debug_config->base_batch_for_memory_estimation > 0) {
        ...
        base_batch_size =
            (user_specified_base_batch_size != base_batch_size) ? user_specified_base_batch_size : base_batch_size;
    }
    ...
    try {
        ...
        if (!batched_inputs.size()) {
            GPU_DEBUG_LOG
                << "[MAX_BATCH_SIZE] MAX_BATCH_SIZE supports only networks with inputs/outputs featuring batched dim."
                << std::endl;
            return static_cast<uint32_t>(max_batch_size);
        }
        ...
        std::pair<int64_t, int64_t> device_memory_usage =
            program->get_compiled_program()->get_estimated_device_mem_usage();
        if (device_memory_usage.first == static_cast<int64_t>(-1L) &&
            device_memory_usage.second == static_cast<int64_t>(-1L)) {
            return static_cast<uint32_t>(max_batch_size);
        }
        int64_t mem_for_general = std::max<int64_t>(1, available_device_mem - device_memory_usage.first);
        int64_t mem_per_batch =
            std::max<int64_t>(1, device_memory_usage.second / static_cast<int64_t>(base_batch_size));
...
uint32_t Plugin::get_optimal_batch_size(const ov::AnyMap& options) const {
    ...
    GPU_DEBUG_INFO << "DEVICE_INFO:"
                   << "gfx_version.major, " << device_info.gfx_ver.major << "gfx_version.minor "
                   << std::to_string(device_info.gfx_ver.minor) << std::endl;
    ...
    size_t L3_cache_size =
        device_info.gfx_ver.major && (device_info.gfx_ver.major <= 9)
            ? 768 * 1024       // Gen9
    ...
    if (gen_kbytes_per_bank.end() != val) {
        ...
        GPU_DEBUG_INFO << "DEVICE_INFO:"
                       << "num_slices " << device_info.num_slices << ", num_sub_slices_per_slice "
                       << device_info.num_sub_slices_per_slice << ", num_banks_per_slice " << num_banks_per_slice
