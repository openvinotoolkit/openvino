            ...
            for (size_t ic = 0; ic < input_channels; ic++) {
                for (size_t k = 0; k < total_spatial_size; k++) {
                    ...
                    const auto w_offset = g * output_channels * input_channels * total_spatial_size +
                                          oc * input_channels * total_spatial_size + ic * total_spatial_size + k;
...
ov::Tensor get_compensation(std::shared_ptr<ov::Node> w,
                            std::shared_ptr<ov::Node> azp,
                            std::shared_ptr<ov::Node> wzp,
                            int64_t groups) {
    ...
    ov::Tensor azp_tensor(azp_const->get_element_type(),
                          azp_const->get_shape(),
                          const_cast<void*>(azp_const->get_data_ptr()));
    ov::Tensor wzp_tensor;
    if (wzp_const) {
        wzp_tensor = ov::Tensor(wzp_const->get_element_type(),
                                wzp_const->get_shape(),
                                const_cast<void*>(wzp_const->get_data_ptr()));
    }
...
AsymmetricConvolutionMatcher::AsymmetricConvolutionMatcher() {
    ...
    auto azp_const_m = wrap_type<ov::op::v0::Constant>(
        all_of({consumers_count(1), type_matches_any({ov::element::u8, ov::element::i8})}));
    ...
    auto wzp_const_m = wrap_type<ov::op::v0::Constant>(
        all_of({consumers_count(1), type_matches_any({ov::element::u8, ov::element::i8})}));
    ...
    auto convolution_m =
        wrap_type<ov::op::v1::Convolution, ov::op::v1::GroupConvolution>({conv_activations_m, conv_weights_m});
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](Matcher& m) {
        ...
        auto conv_node = std::dynamic_pointer_cast<ov::op::util::ConvolutionFwdPropBase>(
            pattern_map.at(convolution_m).get_node_shared_ptr());
