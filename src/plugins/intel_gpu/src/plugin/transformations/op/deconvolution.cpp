...
Deconvolution::Deconvolution(const ov::Output<Node>& data_batch,
    ...
    : ov::op::util::ConvolutionBackPropBase({data_batch, filters}, strides, pads_begin, pads_end, dilations, auto_pad, output_padding)
    , m_groups(groups)
    , m_asymmetric(false)
...
Deconvolution::Deconvolution(const ov::Output<Node>& data_batch,
                         ...
                         const ov::element::Type& output_type,
                         const ov::CoordinateDiff& output_padding)
    : ov::op::util::ConvolutionBackPropBase({data_batch, filters, output_shape}, strides, pads_begin, pads_end, dilations, auto_pad, output_padding)
    , m_groups(groups)
    , m_asymmetric(false)
    , m_output_type(output_type) {
    validate_and_infer_types();
