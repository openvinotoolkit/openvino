...
GatedMLP::GatedMLP(const ov::Output<Node>& src,
    ...
    : Op({src, w_gate, w_up, w_down, scale_gate, scale_up, scale_down,
      ...
      m_output_type(output_type),
      m_compressed_weights(true),
      m_dynamic_quantized_activation(true) {
    validate_and_infer_types();
}
...
bool GatedMLP::visit_attributes(ov::AttributeVisitor& visitor) {
    ...
    visitor.on_attribute("dynamic_quantized_activation", m_dynamic_quantized_activation);
    return true;
}
...
void GatedMLP::validate_and_infer_types() {
    ...
    NODE_VALIDATION_CHECK(this,
                          input_size == 4 || input_size == 10 || input_size == 13,
                          "GatedMLP expects 4, 10 or 13 inputs, got ",
                          input_size);
    ...
    NODE_VALIDATION_CHECK(this,
                          m_dynamic_quantized_activation == (input_size == 13),
                          "GatedMLP dynamic quantized activation flag doesn't match input count.");
...
std::shared_ptr<Node> GatedMLP::clone_with_new_inputs(const ov::OutputVector& new_args) const {
    ...
    NODE_VALIDATION_CHECK(this, new_args.size() == 13, "Unexpected number of new args: ", new_args.size());
    return std::make_shared<GatedMLP>(new_args.at(0),
                                      new_args.at(1),
                                      new_args.at(2),
                                      new_args.at(3),
