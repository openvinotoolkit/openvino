...
SwiGLU::SwiGLU(const Output<Node>& data, int64_t axis, int64_t split_lengths, const ov::element::Type output_type)
    : Op({data}),
      m_axis(axis),
...
std::shared_ptr<Node> SwiGLU::clone_with_new_inputs(const ov::OutputVector& new_args) const {
    ...
    return std::make_shared<SwiGLU>(new_args.at(0), m_axis, m_split_lengths, m_output_type);
}
