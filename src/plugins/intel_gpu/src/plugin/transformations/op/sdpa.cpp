...
void SDPA::validate_and_infer_types() {
    ...
    if (m_split_kv) {
        // split_kv layout: [Q, K_cache, V_cache, mask, K_new, V_new, kv_len] -- 7 inputs. mask is
        // always present (the fusion pattern that builds split_kv nodes requires one); there is no
        // scale INPUT (scale is the scale_val attribute). KV-cache compression / indirect / sink are
        // not supported together with split_kv.
...
std::vector<ov::PartialShape> shape_infer(const SDPA* op,
    ...
    // [Q, K_cache, V_cache, mask, K_new, V_new, kv_len] (K_new/V_new are the 3rd/2nd from last, see
    // below); the new chunks reuse K/V's transpose order. The op attends over the logical sequence
