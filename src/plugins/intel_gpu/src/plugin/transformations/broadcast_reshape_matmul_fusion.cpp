...
class UnsqueezeBroadcastReshapeMatmulMatcher : public ov::pass::MatcherPass {
public:
    OPENVINO_RTTI("UnsqueezeBroadcastReshapeMatmulMatcher", "0");
    UnsqueezeBroadcastReshapeMatmulMatcher();
};

class BroadcastReshapeMatmulMatcher : public ov::pass::MatcherPass {
public:
    OPENVINO_RTTI("BroadcastReshapeMatmulMatcher", "0");
    BroadcastReshapeMatmulMatcher();
};

BroadcastReshapeMatmulFusion::BroadcastReshapeMatmulFusion() {
    add_matcher<UnsqueezeBroadcastReshapeMatmulMatcher>();
    add_matcher<BroadcastReshapeMatmulMatcher>();
}

UnsqueezeBroadcastReshapeMatmulMatcher::UnsqueezeBroadcastReshapeMatmulMatcher() {
    ...
    ov::matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        auto axes_a = std::vector<int64_t>();
        auto axes_b = std::vector<int64_t>();
        ...
        if (pattern_map.count(unsqueeze_a_m) > 0) {
            ...
            if (!unsqueeze_a)
                return false;
            auto unsqueeze_a_axes = std::dynamic_pointer_cast<ov::op::v0::Constant>(pattern_map.at(unsqueeze_a_axes_m).get_node_shared_ptr());
            axes_a = unsqueeze_a_axes->cast_vector<int64_t>();
        ...
        if (pattern_map.count(unsqueeze_b_m) > 0) {
            ...
            if (!unsqueeze_b)
                return false;
            auto unsqueeze_b_axes = std::dynamic_pointer_cast<ov::op::v0::Constant>(pattern_map.at(unsqueeze_b_axes_m).get_node_shared_ptr());
            axes_b = unsqueeze_b_axes->cast_vector<int64_t>();
    ...
    auto m = std::make_shared<ov::pass::pattern::Matcher>(matmul_m, "UnsqueezeBroadcastReshapeMatmulMatcher");
    this->register_matcher(m, callback);
}

BroadcastReshapeMatmulMatcher::BroadcastReshapeMatmulMatcher() {
    using namespace ov::pass::pattern;
    ...
    auto m = std::make_shared<ov::pass::pattern::Matcher>(matmul_m, "BroadcastReshapeMatmulMatcher");
    this->register_matcher(m, callback);
}
