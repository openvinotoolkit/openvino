...
ConvertGGUFFullyConnectedCompressed::ConvertGGUFFullyConnectedCompressed() {
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        auto is_empty_optional = [](const ov::Output<ov::Node>& v) {
            ...
            return c && (v.get_element_type() == ov::element::dynamic || (v.get_partial_shape().is_static() && ov::shape_size(v.get_shape()) == 0));
        };
        ...
        ov::Output<ov::Node> w_scale = std::make_shared<ov::op::v0::Constant>(ov::element::f16, ov::Shape{1}, std::vector<float>{1.0f});
        ov::Output<ov::Node> w_zp = std::make_shared<ov::intel_gpu::op::Placeholder>();

        auto gpu_fc = std::make_shared<ov::intel_gpu::op::FullyConnectedCompressed>(X, W, bias, w_scale, w_zp, fc->get_output_element_type(0));
