...
SubgraphTracker::SubgraphTracker(Finalizer finalizer) : m_finalizer(std::move(finalizer)) {}

void SubgraphTracker::add_node(const NodePtr& node, const std::string& name) {
    ...
    if (belongs) {
        ...
        for (const auto& id : input_subgraphs) {
        ...
        if (input_subgraphs.empty() || intersected(input_subgraphs, input_dependencies)) {  // no input subgraphs || cannot merge all due to cycles
            ...
            auto merged_subgraph_id =
                std::accumulate(input_subgraphs.begin(), input_subgraphs.end(), *input_subgraphs.begin(), [this](SubgraphID a, SubgraphID b) {
                    merge_subgraphs(a, std::move(b));
                    return a;
                });
...
SubgraphPtr SubgraphTracker::get_current_subgraph(const NodePtr& node) {
    auto id = get_subgraph_id(node);
    return id ? get_subgraph(id) : nullptr;
}

void SubgraphTracker::finalize() {
    for (const auto& subgraph_record : m_subgraphs) {
...
void SubgraphTracker::add_node_to_subgraph(const NodePtr& node, const SubgraphID& id) {
...
SubgraphPtr SubgraphTracker::get_subgraph(const SubgraphID& id) {
    return m_subgraphs.at(ov::symbol::ancestor_of(id));
}

// set/get all subgraph ids that contribute to a given node

const SubgraphTracker::Dependencies& SubgraphTracker::get_dependencies(const NodePtr& node) {
    return node->get_rt_info().at("__subgraph_dependencies").as<Dependencies>();
}
void SubgraphTracker::set_dependencies(const NodePtr& node, const Dependencies& dependencies) {
    node->get_rt_info()["__subgraph_dependencies"] = dependencies;
}

// set/get subgraph id that a give node belongs to

SubgraphID SubgraphTracker::get_subgraph_id(const NodePtr& node) {
    const auto& rti = node->get_rt_info();
    if (rti.count("__subgraph_id") == 0U) {
        return nullptr;
    }
...
void SubgraphTracker::set_subgraph_id(const NodePtr& node, const SubgraphID& id) {
    node->get_rt_info()["__subgraph_id"] = id;
}

bool SubgraphTracker::intersected(const Dependencies& a, const Dependencies& b) {
    return std::any_of(a.begin(), a.end(), [&b](const SubgraphID& id) {
        return b.count(id) != 0;
    });
}
...
void SubgraphTracker::try_terminate_subgraphs(const Dependencies& subgraphs, const NodePtr& terminator) {
