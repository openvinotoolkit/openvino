// Copyright (C) 2018-2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0
//

#pragma once

#include <functional>

#include "common/typedefs.hpp"

namespace ov::intel_gpu::mlir {

struct Subgraph {
    ov::NodeVector nodes;
    ov::OutputVector inputs;
    ov::OutputVector outputs;
    std::vector<InputVector> output_consumers;
    std::string function_name;

    // Consumes other subgraph
    void merge(Subgraph& other);
};

using SubgraphPtr = std::shared_ptr<Subgraph>;
using SubgraphID = SymbolPtr;

class SubgraphTracker {
public:
    // callback to finalize the subgraph when it is terminated
    using Finalizer = std::function<void(SubgraphPtr)>;

    SubgraphTracker(Finalizer finalizer);
    // name == "" means the node does not belong to any subgraph. Nodes that belong are merged only with
    // connected input subgraphs sharing the same name; a different name starts a separate (named) subgraph.
    void add_node(const NodePtr& node, const std::string& name);
    void finalize();
    SubgraphPtr get_current_subgraph(const NodePtr& node);

private:
    std::unordered_map<SubgraphID, SubgraphPtr> m_subgraphs;
    using Dependencies = std::unordered_set<SubgraphID>;
    Finalizer m_finalizer;

    SubgraphID new_subgraph();
    void add_node_to_subgraph(const NodePtr& node, const SubgraphID& id);
    void merge_subgraphs(SubgraphID id1, SubgraphID id2);
    SubgraphPtr get_subgraph(const SubgraphID& id);

    // set/get all subgraph ids that contribute to a given node
    static const Dependencies& get_dependencies(const NodePtr& node);
    static void set_dependencies(const NodePtr& node, const Dependencies& dependencies);

    // set/get subgraph id that a give node belongs to
    static SubgraphID get_subgraph_id(const NodePtr& node);
    static void set_subgraph_id(const NodePtr& node, const SubgraphID& id);

    static bool intersected(const Dependencies& a, const Dependencies& b);
    void terminate_subgraph(SubgraphID id);
    void try_terminate_subgraphs(const Dependencies& subgraphs, const NodePtr& terminator);
};

}  // namespace ov::intel_gpu::mlir
