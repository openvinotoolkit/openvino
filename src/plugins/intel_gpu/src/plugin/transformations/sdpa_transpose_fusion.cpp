...
SDPATransposeFusion::SDPATransposeFusion() {
    ...
    //    moves the head_size dim (e.g. K preceded by Transpose({0,1,3,2}).
    //    In that case the SDPA stays a framework op; convert it here with
    ...
    ov::matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        if (pattern_map.count(sdpa_m) > 0) {
            ...
            if (sdpa->get_kv_compressed()) {
                ...
                new_sdpa->set_sliding_window_size(sdpa->get_sliding_window_size());
            } else {
                new_sdpa = std::make_shared<ov::intel_gpu::op::SDPA>(
                    sdpa->input_values(),
                    sdpa->get_causal(),
