...
FullyConnectedHorizontalFusion::FullyConnectedHorizontalFusion() {
    ...
    ov::matcher_pass_callback callback = [=](Matcher& m) {
        ...
        if (fused_zps)
            new_fc = std::make_shared<op::FullyConnectedCompressed>(input_node, fused_weight, fused_bias, fused_scale, fused_zps, fc_nodes[0]->get_output_type());
        else
            new_fc = std::make_shared<op::FullyConnectedCompressed>(input_node, fused_weight, fused_bias, fused_scale, fc_nodes[0]->get_output_type());
