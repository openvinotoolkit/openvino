...
MultiTensorState::MultiTensorState(const std::vector<VariableStateInfo>& infos,
    ...
    : ov::intel_gpu::VariableStateBase(infos[0].m_id, context) {
    for (auto& info : infos) {
...
VariableStateIndirectKVCache::VariableStateIndirectKVCache(const VariableStateInfo& info,
    ...
    cldnn::layout beam_table_layout(get_beam_table_shape(info.m_layout.get_partial_shape()),
                                    ov::element::i32,
                                    cldnn::format::bfyx);
    ...
    OPENVINO_ASSERT(m_hidden_states.size() == 2,
                    "[GPU] VariableStateIndirectKVCache expects 2 internal states to be initialized");
}
...
void VariableStateIndirectKVCache::set_state(const ov::SoPtr<ov::ITensor>& state) {
    OPENVINO_ASSERT(m_hidden_states.size() == 2,
                    "[GPU] Corrupted VariableStateIndirectKVCache. Expected 2 internal states. Got: ",
                    m_hidden_states.size());
...
static void rearrange_cache(cldnn::memory::ptr kv_in_mem,
                            cldnn::memory::ptr bt_mem,
                            cldnn::memory::ptr kv_out_mem,
                            cldnn::stream& stream,
                            size_t concat_axis) {
        ...
        for (size_t f = 0; f < kv_shape[1]; f++) {
            for (size_t y = 0; y < kv_shape[2]; y++) {
                for (size_t x = 0; x < kv_shape[3]; x++) {
                    ...
                    auto in_idx = std::vector<int>{static_cast<int>(b_kv),
                                                   static_cast<int>(f),
                                                   static_cast<int>(y),
                                                   static_cast<int>(x)};
                    auto out_idx = std::vector<int>{static_cast<int>(b),
                                                    static_cast<int>(f),
                                                    static_cast<int>(y),
                                                    static_cast<int>(x)};
...
ov::SoPtr<ov::ITensor> VariableStateIndirectKVCache::get_state() const {
    ...
    if (kv_layout.get_partial_shape()[m_beam_axis].get_length() > 1 && bt_mem) {
        ...
        auto tensor =
            m_context->create_host_tensor(m_hidden_states[0]->get_user_specified_type(), kv_layout.get_shape());
