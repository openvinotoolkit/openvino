...
RemoteContextImpl::RemoteContextImpl(const std::string& device_name, std::vector<cldnn::device::ptr> devices)
    ...
    GPU_DEBUG_LOG << "Initialize RemoteContext for " << m_device_name << " (" << m_engine->get_device_info().dev_name
                  << ")" << std::endl;
    init_properties();
}

RemoteContextImpl::RemoteContextImpl(const std::map<std::string, RemoteContextImpl::Ptr>& known_contexts,
                                     const AnyMap& params) {
    ...
    if (params.size()) {
        ...
        if (ctx_type == ov::intel_gpu::ContextType::OCL) {
            ...
            OPENVINO_ASSERT(context_id != nullptr,
                            "[GPU] Can't create shared OCL context as user handle is nullptr! Params:\n",
                            params);
            ...
            OPENVINO_ASSERT(m_va_display != nullptr,
                            "[GPU] Can't create shared VA/DX context as user handle is nullptr! Params:\n",
                            params);
    ...
    cldnn::device_query device_query(engine_type,
                                     runtime_type,
                                     context_id,
                                     m_va_display,
                                     ctx_device_id,
    ...
    OPENVINO_ASSERT(device_map.size() == 1,
                    "[GPU] Exactly one device expected in case of context sharing, but ",
                    device_map.size(),
                    " found");
...
ov::SoPtr<ov::IRemoteTensor> RemoteContextImpl::create_tensor(const ov::element::Type& type,
    ...
    if (params.empty()) {
        ...
        OPENVINO_ASSERT(
            !is_usm || m_engine->use_unified_shared_memory(),
...
std::shared_ptr<ov::IRemoteTensor> RemoteContextImpl::reuse_surface(const ov::element::Type type,
                                                                    const ov::Shape& shape,
                                                                    const ov::AnyMap& params) {
...
std::shared_ptr<ov::IRemoteTensor> RemoteContextImpl::create_buffer(const ov::element::Type type,
                                                                    const ov::Shape& shape) {
    return std::make_shared<RemoteTensorImpl>(get_this_shared_ptr(), shape, type, TensorType::BT_BUF_INTERNAL);
}

std::shared_ptr<ov::IRemoteTensor> RemoteContextImpl::create_usm(const ov::element::Type type,
                                                                 const ov::Shape& shape,
...
void RemoteContextImpl::check_if_shared() const {
    OPENVINO_ASSERT(m_type == ContextType::VA_SHARED,
                    "[GPU] Shared context is required to to share this type of memory");
}
