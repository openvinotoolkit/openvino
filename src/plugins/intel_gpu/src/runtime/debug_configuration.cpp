...
bool debug_configuration::is_layer_for_loading(const std::string& layer_name) const {
#ifdef GPU_DEBUG_CONFIG
    ...
    auto iter = std::find_if(load_layers_raw_dump.begin(), load_layers_raw_dump.end(), [&](const std::string& dl){
        size_t file = dl.rfind(":");
        auto loading_layer = dl.substr(0, file);
        return is_match(layer_name, loading_layer);
    });

    return (iter != load_layers_raw_dump.end());
#else
    return false;
#endif
}

std::vector<std::string> debug_configuration::get_loading_filenames(const std::string& id) const {
