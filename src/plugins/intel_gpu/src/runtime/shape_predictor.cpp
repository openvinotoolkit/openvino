...
std::pair<bool, ov::Shape> ShapePredictor::predict_preallocation_shape(const std::string& orig_id,
    ...
    size_t next_iters_prealloc_count =
        custom_next_iters_prealloc_count > 0 ? static_cast<size_t>(custom_next_iters_prealloc_count) : _settings.next_iters_preallocation_count;
