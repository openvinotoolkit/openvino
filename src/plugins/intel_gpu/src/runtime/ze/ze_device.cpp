...
device_info init_device_info(ze_driver_handle_t driver, ze_device_handle_t device) {
    ...
    uint32_t num_ext = 0;
    OV_ZE_EXPECT(ze_api->zeDriverGetExtensionProperties(driver, &num_ext, nullptr));

    std::vector<ze_driver_extension_properties_t> extensions(num_ext);
    OV_ZE_EXPECT(ze_api->zeDriverGetExtensionProperties(driver, &num_ext, &extensions[0]));

    ze_driver_properties_t driver_properties{ZE_STRUCTURE_TYPE_DRIVER_PROPERTIES};
    OV_ZE_EXPECT(ze_api->zeDriverGetProperties(driver, &driver_properties));
    ...
    OV_ZE_EXPECT(ze_api->zeDeviceGetProperties(device, &device_properties));

    ze_device_compute_properties_t device_compute_properties{ZE_STRUCTURE_TYPE_DEVICE_COMPUTE_PROPERTIES};
    OV_ZE_EXPECT(ze_api->zeDeviceGetComputeProperties(device, &device_compute_properties));

    uint32_t queue_properties_count = 0;
    OV_ZE_EXPECT(ze_api->zeDeviceGetCommandQueueGroupProperties(device, &queue_properties_count, nullptr));
    ...
    OV_ZE_EXPECT(ze_api->zeDeviceGetCommandQueueGroupProperties(device, &queue_properties_count, &queue_properties[0]));
    ...
    OV_ZE_EXPECT(ze_api->zeDeviceGetMemoryProperties(device, &memory_properties_count, nullptr));
    ...
    OV_ZE_EXPECT(ze_api->zeDeviceGetMemoryProperties(device, &memory_properties_count, &device_memory_properties[0]));

    ze_device_memory_access_properties_t device_memory_access_properties{ZE_STRUCTURE_TYPE_DEVICE_MEMORY_ACCESS_PROPERTIES};
    OV_ZE_EXPECT(ze_api->zeDeviceGetMemoryAccessProperties(device, &device_memory_access_properties));
    ...
    OV_ZE_EXPECT(ze_api->zeDeviceGetModuleProperties(device, &device_module_properties));

    ze_device_image_properties_t device_image_properties{ZE_STRUCTURE_TYPE_DEVICE_IMAGE_PROPERTIES};
    OV_ZE_EXPECT(ze_api->zeDeviceGetImageProperties(device, &device_image_properties));
    ...
    OV_ZE_EXPECT(ze_api->zeDeviceGetCacheProperties(device, &cache_properties_count, nullptr));
    ...
    if (cache_properties_count > 0) {
        ...
        OV_ZE_EXPECT(ze_api->zeDeviceGetCacheProperties(device, &cache_properties_count, cache_properties.data()));
    ...
    if (supports_luid) {
        ...
        if (ze_api->zeDeviceGetProperties(device, &device_properties) == ZE_RESULT_SUCCESS)
    ...
    if (supports_pci_properties) {
        ...
        if (ze_api->zeDevicePciGetPropertiesExt(device, &pci_properties) == ZE_RESULT_SUCCESS) {
    ...
    OV_ZE_EXPECT(ze_api->zeContextCreate(driver, &context_desc, &context));
    ngen::Product product = ngen::LevelZeroCodeGenerator<ngen::HW::Unknown>::detectHWInfo(context, device);
    OV_ZE_WARN(ze_api->zeContextDestroy(context));
...
void ze_device::initialize() {
    ...
    ze_context_desc_t context_desc = { ZE_STRUCTURE_TYPE_CONTEXT_DESC, nullptr, 0 };
    OV_ZE_EXPECT(ze_api->zeContextCreate(_driver, &context_desc, &_context));
...
ze_device::~ze_device() {
    if (_is_initialized)
        OV_ZE_WARN(ze_api->zeContextDestroy(_context));
}
