...
public:
    static void create_kernels_from_module(std::shared_ptr<ze_module_holder> module, std::vector<kernel::ptr> &out) {
        ...
        OV_ZE_EXPECT(ze_api->zeModuleGetKernelNames(module_handle, &kernel_count, nullptr));
        ...
        OV_ZE_EXPECT(ze_api->zeModuleGetKernelNames(module_handle, &kernel_count, kernel_names.data()));
        ...
        for (auto name_cstr : kernel_names) {
            ...
            OV_ZE_EXPECT(ze_api->zeKernelCreate(module_handle, &kernel_desc, &kernel_handle));
    ...
    std::shared_ptr<kernel> clone(bool reuse_kernel_handle = false) const override {
        if (reuse_kernel_handle) {
            ...
            OV_ZE_EXPECT(ze_api->zeKernelCreate(module_handle, &descriptor, &cloned_handle));
    ...
    std::vector<uint8_t> get_binary() const override {
        ...
        ze_module_handle_t module_handle = get_module_handle();
        OV_ZE_EXPECT(ze_api->zeModuleGetNativeBinary(module_handle, &binary_size, nullptr));

        std::vector<uint8_t> binary(binary_size);
        OV_ZE_EXPECT(ze_api->zeModuleGetNativeBinary(module_handle, &binary_size, binary.data()));
    ...
    std::string get_build_log() const override {
        ...
        size_t log_size = 0;
        OV_ZE_EXPECT(ze_api->zeModuleBuildLogGetString(build_log_handle, &log_size, nullptr));

        std::string log(log_size, ' ');
        OV_ZE_EXPECT(ze_api->zeModuleBuildLogGetString(build_log_handle, &log_size, log.data()));
        return log;
    }
