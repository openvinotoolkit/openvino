...
allocation_type gpu_usm::detect_allocation_type(const ze_engine* engine, const void* mem_ptr) {
    ...
    OV_ZE_EXPECT(ze_api->zeMemGetAllocProperties(engine->get_context(), mem_ptr, &props, &device));
...
void* gpu_usm::lock(const stream& stream, mem_lock_type type) {
    ...
    if (0 == _lock_count) {
        ...
        if (get_allocation_type() == allocation_type::usm_device) {
            ...
            OV_ZE_EXPECT(ze_api->zeCommandListAppendMemoryCopy(_ze_stream.get_queue(),
                                    _host_buffer.get(),
                                    _buffer.get(),
                                    _bytes_count,
                                    nullptr,
                                    0,
                                    nullptr));
            OV_ZE_EXPECT(ze_api->zeCommandListHostSynchronize(_ze_stream.get_queue(), endless_wait));
...
event::ptr gpu_usm::fill(stream& stream, unsigned char pattern, const std::vector<event::ptr>& dep_events, bool blocking) {
    ...
    OV_ZE_EXPECT(ze_api->zeCommandListAppendMemoryFill(_ze_stream.get_queue(),
        _buffer.get(),
        &pattern,
