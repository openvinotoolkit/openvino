...
static std::vector<ze_device_handle_t> get_sub_devices(ze_device_handle_t root_device) {
    ...
    OV_ZE_EXPECT(ze_api->zeDeviceGetSubDevices(root_device, &n_subdevices, nullptr));
    if (n_subdevices == 0)
        return {};

    std::vector<ze_device_handle_t> subdevices(n_subdevices);

    OV_ZE_EXPECT(ze_api->zeDeviceGetSubDevices(root_device, &n_subdevices, &subdevices[0]));
...
std::vector<device::ptr> ze_device_detector::create_device_list(bool initialize_devices) const {
    ...
    OV_ZE_EXPECT(ze_api->zeInit(ZE_INIT_FLAG_GPU_ONLY));

    uint32_t driver_count = 0;
    OV_ZE_EXPECT(ze_api->zeDriverGet(&driver_count, nullptr));

    std::vector<ze_driver_handle_t> all_drivers(driver_count);
    OV_ZE_EXPECT(ze_api->zeDriverGet(&driver_count, &all_drivers[0]));

    for (uint32_t i = 0; i < driver_count; ++i) {
        uint32_t device_count = 0;
        OV_ZE_EXPECT(ze_api->zeDeviceGet(all_drivers[i], &device_count, nullptr));

        std::vector<ze_device_handle_t> all_devices(device_count);
        OV_ZE_EXPECT(ze_api->zeDeviceGet(all_drivers[i], &device_count, &all_devices[0]));

        for (uint32_t d = 0; d < device_count; ++d) {
            try {
                ze_device_properties_t device_properties{ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES};
                OV_ZE_EXPECT(ze_api->zeDeviceGetProperties(all_devices[d], &device_properties));
