...
public:
    ...
    explicit ze_owner(handle_t handle) : _handle(handle) {
        OPENVINO_ASSERT(_handle != nullptr, "[GPU] Can not create ze_owner with nullptr handle");
    }
    ze_owner(const ze_owner<resource_type>& other) = delete;
    ze_owner& operator=(const ze_owner<resource_type>& other) = delete;
    ~ze_owner() {
        deleter_t{}(_handle);
    }
...
template <>
struct ze_owner<ze_resource_type::usm_memory> {
public:
    ...
    explicit ze_owner(ze_context_handle_t context, handle_t handle) : _context(context), _handle(handle) {
        OPENVINO_ASSERT(_context != nullptr, "[GPU] Can not create ze_owner with nullptr context");
        OPENVINO_ASSERT(_handle != nullptr, "[GPU] Can not create ze_owner with nullptr handle");
    }
    ze_owner(const ze_owner<resource_type>& other) = delete;
    ze_owner& operator=(const ze_owner<resource_type>& other) = delete;
...
public:
    ...
    explicit ze_holder(handle_t handle, parent_holder_t parent) {
        ...
        std::shared_ptr<ze_owner<resource_type>> resource;
        if constexpr (resource_type == ze_resource_type::usm_memory) {
            ...
            resource = std::make_shared<ze_owner<resource_type>>(parent.get_handle(), handle);
        } else {
            resource = std::make_shared<ze_owner<resource_type>>(handle);
        }
...
private:
    std::shared_ptr<ze_owner<resource_type>> _holder;
...
public:
    ...
    ze_holder& operator=(const ze_holder&) = default;
    ze_holder& operator=(ze_holder&&) = default;
    explicit ze_holder(handle_t handle)
        : _holder(std::make_shared<ze_owner<resource_type>>(handle)) {}
    ~ze_holder() {
        drop();
    }
