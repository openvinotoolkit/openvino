...
memory_ptr ocl_engine::create_hostbuffer(const void* mmapped_address, size_t data_size, allocation_type _allocation_type, const layout output_layout) {
    auto tracker = std::make_shared<MemoryTracker>(this,
                                                   const_cast<void*>(mmapped_address),  // Point directly to mmap'd memory
                                                   data_size,
                                                   _allocation_type);
    std::uintptr_t mmap_address = reinterpret_cast<std::uintptr_t>(mmapped_address);
    constexpr std::uintptr_t minimal_alignment_no_copy = 16;  // minimal alignment for any pointer
    std::uintptr_t aligned_addr = mmap_address & ~(static_cast<std::uintptr_t>(minimal_alignment_no_copy) - 1);
    void* mmap_aligned_address = reinterpret_cast<void*>(aligned_addr);
