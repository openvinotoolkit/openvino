...
ocl_error::ocl_error(const cl::Error& err) : ov::Exception("[GPU] " + std::string(err.what()) + std::string(", error code: ") + std::to_string(err.err())) {}
...
cl_platform_id get_platform_id_for_device(const cl::Device& device) {
    ...
    OPENVINO_ASSERT(err == CL_SUCCESS && platform != nullptr, "[GPU] Failed to retrieve CL_DEVICE_PLATFORM, error: ", err);
    return platform;
}
}  // namespace

ocl_engine::ocl_engine(const device::ptr dev, runtime_types runtime_type) : engine(dev) {
...
allocation_type ocl_engine::detect_usm_allocation_type(const void* memory) const {
    return use_unified_shared_memory() ? ocl::gpu_usm::detect_allocation_type(this, memory) : allocation_type::unknown;
}
...
memory::ptr ocl_engine::create_subbuffer(const memory& memory, const layout& new_layout, size_t byte_offset) {
    ...
    try {
        if (new_layout.format.is_image_2d()) {
            ...
            return std::make_shared<ocl::gpu_usm>(this, new_layout, sub_buffer, memory.get_allocation_type(), memory.get_mem_tracker());
        } else {
            ...
            return std::make_shared<ocl::gpu_buffer>(this, new_layout, sub_buffer, memory.get_mem_tracker());
        }
...
memory_ptr ocl_engine::create_hostbuffer(void* cpu_address, size_t data_size, allocation_type _allocation_type, const layout output_layout) {
...
memory::ptr ocl_engine::reinterpret_buffer(const memory& memory, const layout& new_layout) {
    ...
    OPENVINO_ASSERT(new_layout.format.is_image() == memory.get_layout().format.is_image(),
                    ...
                    memory.get_layout().format.to_string(),
                    " Target: ",
    ...
    try {
        ...
        if (new_layout.format.is_image_2d()) {
            reinterpret_memory =
                std::make_shared<ocl::gpu_image2d>(this, new_layout, reinterpret_cast<const ocl::gpu_image2d&>(memory).get_buffer(), memory.get_mem_tracker());
        } else if (memory_capabilities::is_usm_type(memory.get_allocation_type())) {
            ...
            reinterpret_memory =
                std::make_shared<ocl::gpu_buffer>(this, new_layout, reinterpret_cast<const ocl::gpu_buffer&>(memory).get_buffer(), memory.get_mem_tracker());
        }
    ...
    try {
        ...
        if (params.mem_type == shared_mem_type::shared_mem_buffer) {
            ...
            OPENVINO_ASSERT(actual_mem_size >= requested_mem_size,
                            "[GPU] shared buffer has smaller size (",
                            actual_mem_size,
                            ") than specified layout (",
                            requested_mem_size,
                            ")");
        ...
        if (params.mem_type == shared_mem_type::shared_mem_usm) {
            ...
            auto requested_mem_size = new_layout.bytes_count();
            OPENVINO_ASSERT(actual_mem_size >= requested_mem_size,
                            "[GPU] shared USM buffer has smaller size (",
                            actual_mem_size,
                            ") than specified layout (",
                            requested_mem_size,
                            ")");
...
bool ocl_engine::is_the_same_buffer(const memory& mem1, const memory& mem2) {
    ...
    if (!memory_capabilities::is_usm_type(mem1.get_allocation_type())) {
        return (reinterpret_cast<const ocl::gpu_buffer&>(mem1).get_buffer() == reinterpret_cast<const ocl::gpu_buffer&>(mem2).get_buffer());
    }
...
void* ocl_engine::get_user_context(runtime_types rt_type) const {
    OPENVINO_ASSERT(rt_type == runtime_types::ocl, "[GPU] OCL engine can only provide OCL context but requested context for ", rt_type);
...
memory_ptr ocl_engine::create_hostbuffer_impl(void* cpu_address,
                                              size_t data_size,
    ...
    OPENVINO_ASSERT((reinterpret_cast<std::uintptr_t>(cpu_address) % minimal_alignment) == 0,
                    ...
                    minimal_alignment,
                    "-byte aligned");
    OPENVINO_ASSERT((data_size % minimal_alignment) == 0, "[GPU] shared buffer size must be a multiple of ", minimal_alignment, " bytes");
...
#ifdef CL_MEM_FORCE_HOST_MEMORY_INTEL
    tracker = std::make_shared<MemoryTracker>(nullptr, cpu_address, data_size, allocation);
#else
    tracker = std::make_shared<MemoryTracker>(this, cpu_address, data_size, allocation);
#endif
