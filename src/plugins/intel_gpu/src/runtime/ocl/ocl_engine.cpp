    ...
    try {
        ...
        switch (clErr.err()) {
            ...
            case CL_INVALID_BUFFER_SIZE:
                OPENVINO_UNREACHABLE("[GPU] out of GPU resources");
            default:
                OPENVINO_UNREACHABLE("[GPU] buffer allocation failed");
        }
    }
}
...
memory::ptr ocl_engine::reinterpret_handle(const layout& new_layout, shared_mem_params params) {
   try {
        if (new_layout.format.is_image_2d() && params.mem_type == shared_mem_type::shared_mem_image) {
            ...
            OPENVINO_UNREACHABLE("[GPU] unknown shared object fromat or type");
        }
    }
    catch (const cl::Error& clErr) {
        switch (clErr.err()) {
