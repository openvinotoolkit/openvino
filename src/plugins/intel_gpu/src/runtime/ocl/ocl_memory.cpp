...
gpu_buffer::gpu_buffer(ocl_engine* engine, const layout& layout)
    : lockable_gpu_mem(),
...
shared_mem_params gpu_buffer::get_internal_params() const {
    ...
    return {shared_mem_type::shared_mem_buffer,
            static_cast<shared_handle>(cl_engine->get_cl_context().get()),
            nullptr,
    ...
    case allocation_type::cl_mem: {
        ...
        try {
            cl_stream.get_cl_queue()
                .enqueueCopyBuffer(mem_inst.get_buffer(), get_buffer(), 0, 0, other.size(), nullptr, &ev_ocl);
        } catch (cl::Error const& err) {
            OPENVINO_THROW(OCL_ERR_MSG_FMT(err));
        }
        if (blocking)
...
event::ptr gpu_buffer::copy_from(stream& stream,
                                 const void* host_ptr,
                                 bool blocking,
                                 size_t dst_offset,
                                 size_t data_size) {
    ...
    try {
        cl_stream.get_cl_queue()
            .enqueueWriteBuffer(_buffer, blocking, dst_offset, data_size, host_ptr, nullptr, ev_ocl);
    } catch (cl::Error const& err) {
        OPENVINO_THROW(OCL_ERR_MSG_FMT(err));
    }
...
gpu_image2d::gpu_image2d(ocl_engine* engine, const layout& layout)
    ...
    case format::image_2d_rgba:
        ...
        if (layout.feature() != 3 && layout.feature() != 4) {
            CLDNN_ERROR_MESSAGE("2D image allocation",
                                "invalid number of channels in image_2d_rgba input image (should be 3 or 4)!");
        }
        type = CL_UNORM_INT8;
        break;
    case format::nv12: {
...
void* gpu_image2d::lock(const stream& stream, mem_lock_type type) {
    ...
    if (0 == _lock_count) {
        try {
            _mapped_ptr = cl_stream.get_cl_queue().enqueueMapImage(_buffer,
                                                                   CL_TRUE,
...
shared_mem_params gpu_image2d::get_internal_params() const {
    auto cl_engine = downcast<const ocl_engine>(_engine);
    return {shared_mem_type::shared_mem_image,
            static_cast<shared_handle>(cl_engine->get_cl_context().get()),
            nullptr,
            static_cast<shared_handle>(_buffer.get()),
#ifdef _WIN32
...
event::ptr gpu_image2d::copy_from(stream& stream, const memory& other, bool blocking) {
    ...
    cl::Event* ev_ocl = &downcast<ocl_event>(ev.get())->get();
    try {
        cl_stream.get_cl_queue().enqueueCopyImage(casted.get_buffer(),
                                                  get_buffer(),
                                                  {0, 0, 0},
                                                  {0, 0, 0},
                                                  {_width, _height, 1},
...
event::ptr gpu_image2d::copy_from(stream& stream,
                                  const void* host_ptr,
                                  bool blocking,
                                  size_t dst_offset,
                                  size_t data_size) {
    ...
    cl::Event* ev_ocl = blocking ? nullptr : &downcast<ocl_event>(ev.get())->get();
    try {
        cl_stream.get_cl_queue().enqueueWriteImage(_buffer,
                                                   blocking,
                                                   {0, 0, 0},
                                                   {_width, _height, 1},
                                                   _row_pitch,
...
event::ptr gpu_image2d::copy_to(stream& stream, memory& other, bool blocking) {
    ...
    cl::Event* ev_ocl = &downcast<ocl_event>(ev.get())->get();
    try {
        cl_stream.get_cl_queue().enqueueCopyImage(get_buffer(),
                                                  casted.get_buffer(),
                                                  {0, 0, 0},
                                                  {0, 0, 0},
                                                  {_width, _height, 1},
...
event::ptr gpu_image2d::copy_to(stream& stream, void* host_ptr, bool blocking) {
    ...
    cl::Event* ev_ocl = blocking ? nullptr : &downcast<ocl_event>(ev.get())->get();
    try {
        cl_stream.get_cl_queue().enqueueReadImage(_buffer,
                                                  blocking,
                                                  {0, 0, 0},
                                                  {_width, _height, 1},
                                                  _row_pitch,
...
gpu_media_buffer::gpu_media_buffer(ocl_engine* engine, const layout& new_layout, shared_mem_params params)
    : gpu_image2d(engine,
                  new_layout,
                  cl::ImageVA(engine->get_cl_context(), CL_MEM_READ_WRITE, params.surface, params.plane),
                  nullptr),
...
shared_mem_params gpu_media_buffer::get_internal_params() const {
    auto cl_engine = downcast<const ocl_engine>(_engine);
    return {shared_mem_type::shared_mem_vasurface,
            static_cast<shared_handle>(cl_engine->get_cl_context().get()),
            device,
            static_cast<shared_handle>(_buffer.get()),
            surface,
...
#ifdef _WIN32
gpu_dx_buffer::gpu_dx_buffer(ocl_engine* engine, const layout& new_layout, shared_mem_params params)
    : gpu_buffer(engine, new_layout, cl::BufferDX(engine->get_cl_context(), CL_MEM_READ_WRITE, params.mem), nullptr),
      device(params.user_device),
      resource(params.mem) {}

shared_mem_params gpu_dx_buffer::get_internal_params() const {
    auto cl_engine = downcast<const ocl_engine>(_engine);
    return {shared_mem_type::shared_mem_dxbuffer,
            static_cast<shared_handle>(cl_engine->get_cl_context().get()),
            device,
            static_cast<shared_handle>(_buffer.get()),
            resource,
...
gpu_usm::gpu_usm(ocl_engine* engine, const layout& layout, allocation_type type)
    ...
    case allocation_type::usm_device:
        ...
        break;
    default:
        CLDNN_ERROR_MESSAGE("gpu_usm allocation type", "Unknown unified shared memory type!");
    }
...
event::ptr gpu_usm::fill(stream& stream, unsigned char pattern) {
    ...
    auto ev = stream.create_base_event();
    cl::Event& ev_ocl = downcast<ocl_event>(ev.get())->get();
    // enqueueFillUsm call will never finish. Driver bug? Uncomment when fixed. Some older drivers doesn't support
    // enqueueFillUsm call at all. cl_stream.get_usm_helper().enqueue_fill_mem<unsigned char>(cl_stream.get_cl_queue(),
    // _buffer.get(), pattern, _bytes_count, nullptr, &ev_ocl) Workarounded with enqeue_memcopy. ToDo: Remove below
    // code. Uncomment above.
    std::vector<unsigned char> temp_buffer(_bytes_count, pattern);
    // TODO: Do we really need blocking call here? Non-blocking one causes accuracy issues right now, but hopefully it
    // can be fixed in more performant way.
...
event::ptr gpu_usm::copy_from(stream& stream, const memory& other, bool blocking) {
    ...
    if (other.get_allocation_type() == allocation_type::cl_mem) {
        ...
        try {
            cl_stream.get_cl_queue()
                .enqueueReadBuffer(mem_inst.get_buffer(), blocking, 0, size(), this->buffer_ptr(), nullptr, ev_ocl);
        } catch (cl::Error const& err) {
            OPENVINO_THROW(OCL_ERR_MSG_FMT(err));
        }
    } else {
        ...
        try {
            cl_stream.get_usm_helper()
                .enqueue_memcpy(cl_stream.get_cl_queue(), dst_ptr, src_ptr, _bytes_count, blocking, nullptr, ev_ocl);
        } catch (cl::Error const& err) {
            OPENVINO_THROW(OCL_ERR_MSG_FMT(err));
        }
    }
...
event::ptr gpu_usm::copy_from(stream& stream,
                              const void* host_ptr,
                              bool blocking,
                              size_t dst_offset,
                              size_t data_size) {
    ...
    try {
        cl_stream.get_usm_helper()
            .enqueue_memcpy(cl_stream.get_cl_queue(), dst_ptr, host_ptr, data_size, blocking, nullptr, ev_ocl);
    } catch (cl::Error const& err) {
        OPENVINO_THROW(OCL_ERR_MSG_FMT(err));
    }
    return ev;
...
event::ptr gpu_usm::copy_to(stream& stream, void* host_ptr, bool blocking) {
    ...
    try {
        cl_stream.get_usm_helper()
            .enqueue_memcpy(cl_stream.get_cl_queue(), host_ptr, src_ptr, _bytes_count, blocking, nullptr, ev_ocl);
    } catch (cl::Error const& err) {
        OPENVINO_THROW(OCL_ERR_MSG_FMT(err));
    }
    return ev;
...
dnnl::memory gpu_usm::get_onednn_memory(dnnl::memory::desc desc, int64_t offset) const {
    auto onednn_engine = _engine->get_onednn_engine();
    dnnl::memory dnnl_mem = dnnl::ocl_interop::make_memory(desc,
                                                           onednn_engine,
...
allocation_type gpu_usm::detect_allocation_type(const ocl_engine* engine, const void* mem_ptr) {
    ...
    case CL_MEM_TYPE_DEVICE_INTEL:
        res = allocation_type::usm_device;
        break;
    case CL_MEM_TYPE_HOST_INTEL:
        res = allocation_type::usm_host;
        break;
    case CL_MEM_TYPE_SHARED_INTEL:
        res = allocation_type::usm_shared;
        break;
    default:
        res = allocation_type::unknown;
    }
...
allocation_type gpu_usm::detect_allocation_type(const ocl_engine* engine, const cl::UsmMemory& buffer) {
    ...
    OPENVINO_ASSERT(alloc_type == allocation_type::usm_device || alloc_type == allocation_type::usm_host ||
                        alloc_type == allocation_type::usm_shared,
                    "[GPU] Unsupported USM alloc type: " + to_string(alloc_type));
    return alloc_type;
}
