...
sync_methods get_expected_sync_method(const ExecutionConfig& config) {
    ...
    return profiling                                ? sync_methods::events
           : queue_type == QueueTypes::out_of_order ? sync_methods::barriers
                                                    : sync_methods::none;
}
...
ocl_stream::ocl_stream(const ocl_engine& engine, const ExecutionConfig& config)
    ...
    bool priorty_extensions = engine.extension_supported("cl_khr_priority_hints") &&
                              engine.extension_supported("cl_khr_create_command_queue");
    queue_builder.set_priority_mode(config.get_property(ov::intel_gpu::hint::queue_priority), priorty_extensions);

    bool throttle_extensions = engine.extension_supported("cl_khr_throttle_hints") &&
                               engine.extension_supported("cl_khr_create_command_queue");
...
dnnl::stream& ocl_stream::get_onednn_stream() {
    OPENVINO_ASSERT(queue_type == QueueTypes::in_order,
    ...
    if (!_onednn_stream) {
        _onednn_stream = std::make_shared<dnnl::stream>(
            dnnl::ocl_interop::make_stream(_engine.get_onednn_engine(), _command_queue.get()));
    }
...
QueueTypes ocl_stream::detect_queue_type(void* queue_handle) {
    ...
    auto status =
        clGetCommandQueueInfo(queue, CL_QUEUE_PROPERTIES, sizeof(cl_command_queue_properties), &properties, nullptr);
    if (status != CL_SUCCESS) {
...
event::ptr ocl_stream::enqueue_kernel(kernel& kernel,
    ...
    try {
        _command_queue.enqueueNDRangeKernel(kern,
                                            cl::NullRange,
                                            global,
                                            local,
                                            dep_events_ptr,
