...
typedef cl_int(CL_API_CALL* clSetKernelArgMemPointerINTEL_fn)(cl_kernel kernel,
                                                              cl_uint arg_index,
...
typedef CL_API_ENTRY cl_int(CL_API_CALL* PFN_clEnqueueAcquireMediaSurfacesINTEL)(cl_command_queue /* command_queue */,
                                                                                 cl_uint /* num_objects */,
...
typedef CL_API_ENTRY cl_int(CL_API_CALL* PFN_clEnqueueReleaseMediaSurfacesINTEL)(cl_command_queue /* command_queue */,
                                                                                 cl_uint /* num_objects */,
                                                                                 const cl_mem* /* mem_objects */,
                                                                                 cl_uint /* num_events_in_wait_list */,
                                                                                 const cl_event* /* event_wait_list */,
...
typedef CL_API_ENTRY cl_mem(CL_API_CALL* PFN_clCreateFromMediaSurfaceINTEL)(cl_context /* context */,
                                                                            cl_mem_flags /* flags */,
                                                                            void* /* surface */,
                                                                            cl_uint /* plane */,
                                                                            cl_int* /* errcode_ret */);
...
#ifdef WIN32
typedef CL_API_ENTRY cl_mem(CL_API_CALL* PFN_clCreateFromD3D11Buffer)(cl_context context,
                                                                      cl_mem_flags flags,
                                                                      void* resource,
                                                                      cl_int* errcode_ret);
#endif
...
public:
    ...
    SharedSurfLock(cl_command_queue queue, std::vector<cl_mem>& surfaces, cl_int* err = NULL)
        : m_queue(queue),
          m_surfaces(surfaces),
          m_errPtr(err) {
        if (pfn_acquire != NULL && m_surfaces.size()) {
            cl_int error =
                pfn_acquire(m_queue, static_cast<cl_uint>(m_surfaces.size()), m_surfaces.data(), 0, NULL, NULL);
    ...
    ~SharedSurfLock() {
        if (pfn_release != NULL && m_surfaces.size()) {
            cl_int error =
                pfn_release(m_queue, static_cast<cl_uint>(m_surfaces.size()), m_surfaces.data(), 0, NULL, NULL);
            if (error != CL_SUCCESS && m_errPtr != NULL) {
                *m_errPtr = error;
            }
        }
...
public:
    ...
    static void Init(cl_platform_id platform) {
        ...
        if (!pfn_clCreateFromMediaSurfaceINTEL) {
            pfn_clCreateFromMediaSurfaceINTEL =
                try_load_entrypoint<PFN_clCreateFromMediaSurfaceINTEL>((cl_platform_id)platform, fname);
        }
    }
     ...
     * Required for MSVC.
     */
    ImageVA(const ImageVA& img) : Image2D(img) {}
...
public:
    ...
    static void Init(cl_platform_id platform) {
        ...
        if (!pfn_clCreateFromD3D11Buffer) {
            pfn_clCreateFromD3D11Buffer =
                try_load_entrypoint<PFN_clCreateFromD3D11Buffer>((cl_platform_id)platform, fname);
        }
    }
...
public:
    ...
    cl_int getDevices(cl_device_source_intel media_adapter_type,
                      void* media_adapter,
        ...
        cl_uint n = 0;
        cl_int err = pfn_clGetDeviceIDsFromMediaAdapterINTEL(object_,
                                                             media_adapter_type,
                                                             media_adapter,
                                                             media_adapter_set,
                                                             0,
        ...
        if (err != CL_DEVICE_NOT_FOUND) {
            ...
            err = pfn_clGetDeviceIDsFromMediaAdapterINTEL(object_,
                                                          media_adapter_type,
                                                          media_adapter,
                                                          media_adapter_set,
                                                          n,
...
public:
    explicit UsmHelper(const cl::Context& ctx, const cl::Device device, bool use_usm) : _ctx(ctx), _device(device) {
        if (use_usm) {
            ...
            _set_kernel_arg_mem_pointer_fn =
                try_load_entrypoint<clSetKernelArgMemPointerINTEL_fn>(_ctx.get(), "clSetKernelArgMemPointerINTEL");
            ...
            _get_mem_alloc_info_fn =
                try_load_entrypoint<clGetMemAllocInfoINTEL_fn>(_ctx.get(), "clGetMemAllocInfoINTEL");
        }
    }
    ...
    cl_int enqueue_set_mem(const cl::CommandQueue& cpp_queue,
        ...
        cl_event tmp;
        cl_int err =
            _enqueue_memset_fn(cpp_queue.get(),
                               dst_ptr,
                               value,
                               bytes_count,
                               wait_list == nullptr ? 0 : static_cast<cl_uint>(wait_list->size()),
    ...
    cl_unified_shared_memory_type_intel get_usm_allocation_type(const void* usm_ptr) const {
        ...
        size_t ret_val_size;
        _get_mem_alloc_info_fn(_ctx.get(),
                               usm_ptr,
...
public:
    ...
    cl_int setArgUsm(cl_uint index, const UsmMemory& mem) {
        return detail::errHandler(_usmHelper.set_kernel_arg_mem_pointer(*this, index, mem.get()),
