...
bool does_device_match_config(const cl::Device& device) {
    ...
    int32_t ocl_major = -1;
    int32_t ocl_minor = -1;
    // Spec says that the format of this string is OpenCL<space><major_version.minor_version><space><vendor-specific
    // information>
...
std::vector<device::ptr> ocl_device_detector::create_device_list() const {
    ...
    for (auto& id : platform_ids) {
        ...
        try {
            ...
            GPU_DEBUG_LOG << "Devices query/creation failed for " << platform.getInfo<CL_PLATFORM_NAME>() << ": "
                          << ex.what() << std::endl;
...
std::vector<device::ptr> ocl_device_detector::create_device_list_from_user_context(void* user_context,
    ...
    for (size_t i = 0; i < all_devices.size(); i++) {
        ...
        supported_devices.emplace_back(
            std::make_shared<ocl_device>(device, ctx, cl::Platform(device.getInfo<CL_DEVICE_PLATFORM>())));
    }
