...
vulkan_kernel_interface vulkan_kernel_interface::reflect(const std::vector<uint8_t>& spirv, const std::string& entry_point) {
    ...
    for (const auto& variable : state.variables) {
        ...
        OPENVINO_ASSERT((!decoration.binding.has_value() && !decoration.descriptor_set.has_value()) || storage_class == spirv_storage_class::storage_buffer ||
                            storage_class == spirv_storage_class::uniform,
