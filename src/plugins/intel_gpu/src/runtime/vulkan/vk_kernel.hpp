...
public:
    ...
    static kernel::ptr create_kernel(VkDevice device, const std::vector<uint32_t>& spirv, const vk_kernel_reflection& reflection);
    ...
    std::shared_ptr<kernel> clone(bool reuse_kernel_handle = false) const override {
        if (reuse_kernel_handle)
            return std::make_shared<vk_kernel>(_device, _pipeline, _layout, _ds_layout, _spirv, _reflection);

        OPENVINO_THROW("Vulkan kernel cloning with reuse_kernel_handle=false is not supported.");
    }

    bool is_same(const kernel& other) const override {
        auto other_ptr = dynamic_cast<const vk_kernel*>(&other);
        if (other_ptr == nullptr) {
            return false;
        }
        return _pipeline == other_ptr->_pipeline;
    }
...
inline kernel::ptr vk_kernel::create_kernel(VkDevice device, const std::vector<uint32_t>& spirv, const vk_kernel_reflection& reflection) {
    ...
    VK_CALL(vkCreateComputePipelines(device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &pipeline), "vkCreateComputePipelines");
