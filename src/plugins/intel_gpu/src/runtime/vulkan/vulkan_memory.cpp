...
vulkan_buffer_allocation::ptr allocate_buffer(vulkan_engine& engine, size_t requested_size, vulkan_buffer_memory_usage usage) {
    ...
    detail::check_vulkan_memory_result(vkCreateBuffer(engine.get_device_handle(), &buffer_info, nullptr, &buffer), "vkCreateBuffer");
    ...
    if (allocation_result != VK_SUCCESS) {
        ...
        detail::check_vulkan_memory_result(allocation_result, "vkAllocateMemory");
    }
    ...
    if (bind_result != VK_SUCCESS) {
        ...
        detail::check_vulkan_memory_result(bind_result, "vkBindBufferMemory");
    }
    ...
    if ((memory_type.flags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) != 0) {
        ...
        if (map_result != VK_SUCCESS) {
            vkDestroyBuffer(engine.get_device_handle(), buffer, nullptr);
            vkFreeMemory(engine.get_device_handle(), device_memory, nullptr);
            detail::check_vulkan_memory_result(map_result, "vkMapMemory");
        }
    }
