...
uint32_t get_compute_queue_family(VkPhysicalDevice physical_device) {
    ...
    throw vulkan_error("vk_device: no compute-capable queue family found");
}
...
memory_capabilities init_memory_caps() {
    // Vulkan buffers cover both host-visible (usm_host/usm_shared) and device-local (usm_device) allocations
    return memory_capabilities({allocation_type::usm_host,
                                allocation_type::usm_shared,
...
vk_device::vk_device(const instance_ptr& instance, VkPhysicalDevice physical_device)
    ...
    , _mem_caps(init_memory_caps())
    , _is_initialized(true) {
    _queue_family_index = get_compute_queue_family(physical_device);
...
bool vk_device::is_same(const device::ptr other) {
    auto casted = downcast<vk_device>(other.get());
    if (!casted)
        return false;

    return _instance.get() == casted->get_instance() && _physical_device == casted->get_physical_device();
}

void vk_device::set_mem_caps(const memory_capabilities& memory_capabilities) {
