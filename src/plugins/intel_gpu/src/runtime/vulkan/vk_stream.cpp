...
vk_stream::vk_stream(VkDevice device, VkQueue queue, uint32_t queue_family, const ExecutionConfig& config)
...
void vk_stream::set_arguments(kernel&, const kernel_arguments_desc&, const kernel_arguments_data&) {
    OPENVINO_THROW("[GPU] vk_stream::set_arguments is not implemented");
}

event::ptr vk_stream::enqueue_kernel(kernel& kern,
                                     const kernel_arguments_desc& args_desc,
                                     const kernel_arguments_data& args,
                                     std::vector<event::ptr> const& deps,
                                     bool is_output) {
    auto* vk_kern = dynamic_cast<vk_kernel*>(&kern);
    if (vk_kern == nullptr) {
        OPENVINO_THROW("[GPU] vk_stream::enqueue_kernel: kernel is not a vk_kernel");
    }
    ...
    vkCmdBindPipeline(_command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, vk_kern->get_handle());

    std::vector<VkDescriptorBufferInfo> buffer_infos;
    auto add_memory = [&buffer_infos](const memory::cptr& mem) {
        if (!mem)
            return;
        auto params = mem->get_internal_params();
        if (params.mem == nullptr)
            return;
        buffer_infos.push_back({ reinterpret_cast<VkBuffer>(params.mem), 0, VK_WHOLE_SIZE });
    };
    ...
    uint32_t local[3] = {wgs.local[0], wgs.local[1], wgs.local[2]};
    ...
    VkFence fence = std::static_pointer_cast<vk_event>(ev)->get_handle();
...
event::ptr vk_stream::enqueue_marker(std::vector<event::ptr> const& deps, bool is_output) {
    return create_user_event(true);
}

event::ptr vk_stream::group_events(std::vector<event::ptr> const& deps) {
    if (deps.size() == 1)
        return deps[0];
    return create_user_event(true);
}
...
void vk_stream::wait_for_events(const std::vector<event::ptr>& events) {
}

event::ptr vk_stream::create_user_event(bool set) {
    OPENVINO_ASSERT(set, "[GPU] create user event with set=false is not supported in Vulkan runtime");
    return std::make_shared<vk_event>(_device, VK_NULL_HANDLE);
}

event::ptr vk_stream::create_base_event() {
    return vk_event::create(_device);
}

std::unique_ptr<surfaces_lock> vk_stream::create_surfaces_lock(const std::vector<memory::ptr>&) const {
    return nullptr;
}
