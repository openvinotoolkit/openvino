...
memory_ptr sycl_engine::create_hostbuffer(void* cpu_address, size_t data_size, allocation_type _allocation_type, const layout output_layout) {
    OPENVINO_NOT_IMPLEMENTED;
}
...
void* sycl_engine::get_user_context(runtime_types rt_type) const {
    OPENVINO_ASSERT(rt_type == runtime_types::sycl,
                    "[GPU] SYCL engine can only provide SYCL context but requested context for ",
                    rt_type);
