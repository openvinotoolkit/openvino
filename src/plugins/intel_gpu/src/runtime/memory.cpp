...
MemoryTracker::MemoryTracker(engine* engine, void* buffer_ptr, size_t buffer_size, allocation_type alloc_type)
    ...
    if (m_engine) {
        ...
        GPU_DEBUG_LOG << "Allocate " << m_buffer_size << " bytes of " << m_alloc_type
                      << " allocation type ptr = " << m_buffer_ptr
...
MemoryTracker::~MemoryTracker() {
    if (m_engine) {
        ...
        GPU_DEBUG_LOG << "Free " << m_buffer_size << " bytes of " << m_alloc_type
                      << " allocation type ptr = " << m_buffer_ptr
                      << " (current=" << m_engine->get_used_device_memory(m_alloc_type) << ";"
                      << " max=" << m_engine->get_max_used_device_memory(m_alloc_type) << ")" << std::endl;
    }
...
memory::memory(engine* engine, const layout& layout, allocation_type type, std::shared_ptr<MemoryTracker> mem_tracker)
    : _engine(engine),
      _layout(layout),
      _bytes_count(_layout.bytes_count()),
      m_mem_tracker(mem_tracker),
      _type(type) {}

std::unique_ptr<surfaces_lock> surfaces_lock::create(engine_types engine_type,
    ...
    default:
        throw std::runtime_error("Unsupported engine type in surfaces_lock::create");
    }
}
