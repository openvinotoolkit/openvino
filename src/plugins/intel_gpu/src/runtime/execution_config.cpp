...
void ExecutionConfig::apply_rt_info(const IRemoteContext* context, const ov::RTMap& rt_info, bool is_llm, bool is_paged_attention_model, bool has_lora) {
    ...
    apply_rt_info_property(ov::hint::attn_mode, rt_info);
...
void ExecutionConfig::apply_model_specific_options(const IRemoteContext* context, const ov::Model& model) {
    ...
    const auto& attn_modes = get_attn_mode();
    ...
    if (is_paged_attention_model && ov::element::Type(get_kv_cache_precision()).bitwidth() == 4) {
        OPENVINO_ASSERT(std::find(attn_modes.begin(), attn_modes.end(), ov::hint::AttnMode::PA_CM) == attn_modes.end(),
                        "[GPU] 4-bit KV cache (u4/i4) is not supported with PA_CM attention kernel mode. "
                        "Please use i8 KV cache precision or remove PA_CM from attention mode.");
