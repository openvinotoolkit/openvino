...
static const std::map<format::type, format_traits> format_traits_map{
    ...
    FMT_TRAITS(os_is_yx_osa4_isa8_osv8_isv4_swizzled_by_4,
               1,
               1,
               2,
               0,
               ...
               "oixy",
               {{0, 32}, {1, 32}}),
    FMT_TRAITS(os_is_zyx_osa4_isa8_osv8_isv4_swizzled_by_4,
               1,
               1,
               3,
               0,
    ...
    FMT_TRAITS(g_os_is_zyx_isv8_osv16_isv2,
               ...
               "oixyz???g",
               {{1, 8}, {0, 16}, {1, 2}}),
    FMT_TRAITS(g_os_is_yx_isv8_osv16_isv2,
               1,
               1,
               2,
               1,
...
const format_traits& format::traits(type fmt) {
    OPENVINO_ASSERT(format_traits_map.find(fmt) != format_traits_map.end(),
...
const std::vector<std::pair<size_t, int>> format::per_axis_block_size(format fmt) {
    ...
    for (const auto& block : fmt.block_sizes()) {
        auto it =
            std::find_if(sizes_for_dims.begin(), sizes_for_dims.end(), [&block](const std::pair<size_t, int>& ele) {
...
format format::find_format(const std::vector<uint64_t>& order,
    ...
    auto is_suitable_traits = [&](const std::pair<format::type, format_traits>& traits) -> bool {
        return traits.second._order == order && traits.second.block_sizes == block_sizes &&
