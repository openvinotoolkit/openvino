...
std::vector<layout> kv_cache_inst::calc_output_layouts(kv_cache_node const& /*node*/,
...
template std::vector<layout> kv_cache_inst::calc_output_layouts<ov::PartialShape>(kv_cache_node const& node,
...
void kv_cache_inst::update_shape_info_tensor(const kernel_impl_params& params) {
    ...
    if (params.typed_desc<kv_cache>()->indirect) {
        auto& var =
            dynamic_cast<ov::intel_gpu::VariableStateIndirectKVCache&>(get_network().get_variable(variable_id()));
