...
void propagate_constants::run(program& p) {
    ...
    for (auto& cout : to_replace) {
        ...
        auto const_data =
            std::make_shared<data>("_cldnn_const_prop_" + id_to_replace, std::vector<memory::ptr>{mem_impl} /* <<< REMOVE ME WHEN POSSIBLE */);
...
void propagate_constants::add_deps_to_tpl(program& prog, const std::vector<std::pair<program_node*, int>>& deps) {
    /*
