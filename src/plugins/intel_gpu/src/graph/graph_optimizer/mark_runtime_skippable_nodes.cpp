...
void mark_runtime_skippable_nodes::run(program& p) {
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        if (all_users_are_shape_of(*node) &&
            ...
            !node->is_type<gather_nonzero>() && !node->is_type<unique_gather>() && !node->is_type<non_max_suppression_gather>()) {
        ...
        program_helpers::do_for_types<gather>(*node, [](gather_node& node) {
            ...
            if (node.has_fused_primitives() || (impl_params->get_input_layout(0).data_type != impl_params->get_output_layout().data_type) ||
            ...
            if (node.is_dynamic()) {
                ...
                if (impl_params->get_input_layout(0).get_partial_shape()[axis] == -1 || impl_params->get_input_layout(1).get_partial_shape()[0] == -1 ||
        ...
        program_helpers::do_for_types<permute>(*node, [](permute_node& node) {
            ...
            auto impl_params = node.get_kernel_impl_params();
            if (node.is_output() || node.has_fused_primitives() || (impl_params->get_input_layout(0).data_type != impl_params->get_output_layout().data_type) ||
        ...
        program_helpers::do_for_types<strided_slice>(*node, [](strided_slice_node& node) {
            auto impl_params = node.get_kernel_impl_params();
            if (node.is_output() || node.has_fused_primitives() || (impl_params->get_input_layout(0).format != impl_params->get_output_layout().format) ||
        ...
        program_helpers::do_for_types<reorder>(*node, [](reorder_node& node) {
            ...
            if ((node.is_output() && node.get_dependency(0).is_input()) || node.has_fused_primitives() ||
