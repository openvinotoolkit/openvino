...
void remove_redundant_reorders::run(program& p) {
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        // Prevent optimizing out reorder when a sum post-op is used,
        // as it relies on replacing the original primitive's output buffer
        // with a dependency input. If the output reorder is removed,
        // it may result in reading from an incorrect memory buffer
        // during infer request output processing.
