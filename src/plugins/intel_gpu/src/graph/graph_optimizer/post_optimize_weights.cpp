        ...
        if (weights_reorder_params != nullptr) {
            ...
            if (can_be_fused) {
                ...
                if (node.type() == lstm_seq::type_id()) {
                    program_node& prev_node = node.get_dependency(i);
                    if (i == 5) {
                        add_lstm_bias_reorder(prev_node.id() + "_" + node.id(), weights_reorder_params, p, prev_node, node);
                    } else {
                        add_lstm_weights_reorder(prev_node.id() + "_" + node.id(), weights_reorder_params, p, prev_node, node, i);
                    }
