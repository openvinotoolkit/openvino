...
void pre_replace_deconv::run(program& p) {
    ...
    while (itr != p.nodes_map.end()) {
        ...
        if (node->is_type<deconvolution>()) {
            ...
            auto filter_layout =
                weights_node.get_output_layout().convert_to_weights_layout(deconv_prim->grouped_weights_shape);
            ...
            if (unit_stride) {
                ...
                perform_opt |=
                    cldnn::format::dimension(input_layout.format) == 4 &&
                    (input_layout.data_type == data_types::f32 || input_layout.data_type == data_types::f16) &&
                    !((_lo.get_optimization_attributes().b_fs_yx_fsv16_network ||
                ...
                // remove deconvolution node and its connections to weights and biases, rename it and move to the
                // optimized list
            ...
            } else if (deconv_node.is_output() == false && deconv_node.get_output_layout().feature() == 1 &&
                       deconv_prim->stride[deconv_prim->stride.size() - 1] == 2 &&
                       deconv_prim->stride[deconv_prim->stride.size() - 2] == 2 && filter_layout.spatial(0) == 9 &&
                       filter_layout.spatial(1) == 9 && deconv_prim->pad[deconv_prim->pad.size() - 1] == 4 &&
                       deconv_prim->pad[deconv_prim->pad.size() - 2] == 4 && weights_nodes_id.size() == 1 &&
                       biases_nodes_id.size() == 1 && input_node.get_output_layout().format == format::bfyx) {
                ...
                if (weights_data_type != data_types::f16 && weights_data_type != data_types::f32 &&
                    bias_data_type != data_types::f16 && bias_data_type != data_types::f32)
                    continue;
                ...
                auto target_weights_layout =
                    layout{weights_layout.data_type, weights_layout.format, target_weights_size};
                ...
                const primitive_id weight_replace_node_id = weight_node_id + "_conv_rpl";
                {
                    ...
                    if (weights_data_type == data_types::f16) {
                        mem_lock<ov::float16, mem_lock_type::read> src{
                            weights_node_ptr->as<data>().get_attached_memory_ptr(),
                            stream};
                        for (uint32_t i = 0; i < weights_layout.count(); i++)
                            weights_vec_float.push_back(static_cast<float>(src.data()[i]));
                    } else {
                        mem_lock<float, mem_lock_type::read> src{weights_node_ptr->as<data>().get_attached_memory_ptr(),
                                                                 stream};
                        for (uint32_t i = 0; i < weights_layout.count(); i++)
                ...
                if (bias_data_type == data_types::f16) {
                    mem_lock<ov::float16, mem_lock_type::read> src{
                        bias_id_node_ptr->as<data>().get_attached_memory_ptr(),
                        stream};
                    bias = static_cast<float>(src.data()[0]);
                } else {
                    mem_lock<float, mem_lock_type::read> src{bias_id_node_ptr->as<data>().get_attached_memory_ptr(),
                                                             stream};
                    bias = src.data()[0];
                }
                auto pixel_shuffle_prim = std::make_shared<depth_to_space>(deconv_node_id,
                                                                           deconv_id_conv,
                                                                           2,
                                                                           depth_to_space_mode::blocks_first);
