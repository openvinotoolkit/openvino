    ...
    while (node_itr != p.get_processing_order().end()) {
        ...
        if (node->is_type<strided_slice>() && node->get_output_layout().is_static()) {
            ...
            auto reshape_prim = std::make_shared<reshape>(
                ...
                ov::Shape({static_cast<size_t>(output_dims_sizes[0]),
                           static_cast<size_t>(output_dims_sizes[1]), static_cast<size_t>(output_dims_sizes[3]), static_cast<size_t>(output_dims_sizes[2])}));
