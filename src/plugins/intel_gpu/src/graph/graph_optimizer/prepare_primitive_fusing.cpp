...
void prepare_primitive_fusing::fuse_reorders(program& p) {
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        program_helpers::do_for_types<reorder>(*node, [&p](reorder_node& node) {
            ...
            // - input was optimized
            if (node.has_padded_dependency() || input.is_output() || node.get_dependencies().size() != 1 ||
            ...
            if (input.get_users().size() != 1 || !input.is_type<reorder>() ||
                ...
                !node.get_primitive()->subtract_per_feature.empty() || node.get_primitive()->has_surface_input())
                return;
...
void prepare_primitive_fusing::fuse_bias(program& p) {
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        bool is_bias_add = eltw_node.get_primitive()->mode == eltwise_mode::sum &&
                           eltw_node.get_dependencies().size() == 2 && const_dep_idx >= 0 && const_dep_idx < 2;
        ...
        if (node->get_output_layout().is_dynamic()) {
            ...
            if (broadcast_type != ov::op::AutoBroadcastType::NUMPY && broadcast_type != ov::op::AutoBroadcastType::NONE)
                continue;
            // Numpy broadcast rule requires the dimension size which is not one to be same as the corresponding
            // dimension of the other operand. So we can ensure that the feature size is same for this broadcasting
            // rule, thereby being considered as bias.
        ...
        auto fuse_bias_f =
            [&p](program_node& prev_node, program_node& new_node, program_node& bias_node, program_node& eltw_node) {
                auto eltw_id = eltw_node.id();
                p.replace(prev_node, new_node);
                // Insert bias_node into 3-rd position in dependencies vector to get correct order in case of asymmetric
                // quantization which means that node can have > 2 dependencies even without bias
        ...
        auto recalculate_biases = [&](data_node& original_node, data_node& second_node) -> bool {
            ...
            if (original_mem->count() != second_mem->count() ||
                original_mem->get_layout().data_type != second_mem->get_layout().data_type)
                return false;
        ...
        if (replace_candidate.is_type<convolution>()) {
            ...
            // Copy transposed flag to new prim as convolution node might be produced by deconv -> conv replacement
            // before this pass
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        auto conv_supports_fusings = [&](convolution_node& node) -> bool {
            if (_lo.get_optimization_attributes().use_onednn_impls == 1 &&
                _lo.get_preferred_impl_type(node, format::byxf /*dummy value to disable format checking*/) ==
                    impl_types::onednn) {
                return true;
            }
            ...
            if ((node.get_output_layout().format == format::bfzyx &&
                 (!_lo.get_optimization_attributes().b_fs_zyx_fsv16_network ||
            ...
            if ((node.get_output_layout().format == format::bs_fs_yx_bsv16_fsv16 ||
                 (_lo.is_format_optimized(node, format::bs_fs_yx_bsv16_fsv16) &&
                  _lo.get_optimization_attributes().bs_fs_yx_bsv16_fsv16_network)) &&
                node.get_primitive()->groups == 1)
                return true;

            if (node.get_output_layout().format == format::bs_fs_yx_bsv32_fsv32 ||
                _lo.is_format_optimized(node, format::bs_fs_yx_bsv32_fsv32))
                return true;

            if (node.get_output_layout().format == format::bs_fs_yx_bsv32_fsv16 ||
                _lo.is_format_optimized(node, format::bs_fs_yx_bsv32_fsv16))
                return true;
        ...
        auto gemm_supports_fusings = [](gemm_node& node) -> bool {
            ...
            if (data_type_traits::is_floating_point(in0_dt) && data_type_traits::is_floating_point(in1_dt))
                does_support_fusings = true;

            if (data_type_traits::is_i8_u8(in0_dt) && in0_fmt == format::bfyx && data_type_traits::is_i8_u8(in1_dt) &&
                in1_fmt == format::bfyx) {
        ...
        auto dts_supports_fusings = [](depth_to_space_node& node) -> bool {
            ...
            if (input_conv && out_eltw) {
                ...
                auto conv_size =
                    conv.get_input_layout(0).spatial(0) % 128 == 0 && conv.get_input_layout(0).spatial(1) % 2 == 0;
        ...
        auto get_users_from_fusing_history = [&](const primitive_id& id) {
            ...
            for (auto fusing_info : fusing_history) {
                ...
                auto iter = std::find_if(dep_info_vec.begin(),
                                         dep_info_vec.end(),
                                         [&](std::pair<primitive_id, size_t>& dep_info) {
                ...
                if (!supports_immad && input_data.has_fused_primitives()) {
                    ...
                    if (iter != fusing_history.end()) {
                        ...
                        for (auto& prim_id : iter->second) {
                            ...
                            auto origin_input_iter = std::find_if(fused_descs.begin(),
                                                                  fused_descs.end(),
        ...
        auto fuse_activation_f = [&](activation_node& activation_node) {
            ...
            if (_lo.get_optimization_attributes().use_onednn_impls) {
                ...
                if (activation_func == cldnn::activation_func::relu_negative_slope &&
                    !additional_params_input.empty() && (input.is_type<fully_connected>() || input.is_type<gemm>())) {
            ...
            should_fuse |=
                input.is_type<normalize>() && data_type_traits::is_i8_u8(input.get_input_layout(0).data_type);
            ...
            bool legacy_fusion =
                activation_node.get_dependencies().size() == 1 && !input.can_be_optimized() &&
                !activation_node.is_constant() && !activation_node.has_fused_primitives() &&
                (input.is_type<concatenation>() || input.is_type<convolution>() || input.is_type<crop>() ||
                 input.is_type<eltwise>() || input.is_type<fully_connected>() || input.is_type<normalize>() ||
                 input.is_type<reorder>() || (input.is_type<reshape>() && !input.is_dynamic()) ||
                 input.is_type<roi_pooling>() || input.is_type<softmax>() || input.is_type<depth_to_space>() ||
                 input.is_type<shuffle_channels>() || input.is_type<strided_slice>() || input.is_type<cum_sum>() ||
                 input.is_type<reverse_sequence>() || input.is_type<embedding_bag>() ||
                 input.is_type<extract_image_patches>());

            if (!should_fuse && legacy_fusion) {
                GPU_DEBUG_LOG
                    << activation_node.id()
                    << " is fused by legacy conditions! Consider adding selected kernel with fused ops support\n";
            }
        ...
        auto fuse_quantize_f = [&](quantize_node& quantize_node) {
            ...
            bool per_tensor_values =
                ...
                quantize_node.get_per_tensor_input_shift() && quantize_node.get_per_tensor_input_range() &&
                quantize_node.get_per_tensor_output_scale() && quantize_node.get_per_tensor_output_shift() &&
                quantize_node.get_per_tensor_output_range();

            bool should_fuse =
                input_data.is_type<convolution>() && conv_supports_fusings(input_data.as<convolution>()) &&
                quantize_node.get_scale_shift_opt() &&
                ((out_dt == data_types::f32 || out_dt == data_types::f16) ||
                 in_layout.format == format::b_fs_yx_fsv16 || in_layout.format == format::bs_fs_yx_bsv32_fsv16 ||
                 (_lo.should_select_b_fs_yx_fsv16_layout(input_data.as<convolution>(),
                                                         input_data.get_input_layout(1)) &&
                  !is_grouped_conv(input_data.as<convolution>())) ||
                 // Avoid fusing to b_fs_yx_fsv16 (and similar) kernels
                 _lo.get_optimization_attributes().use_onednn_impls || (in_dt_is_i8_u8 && out_dt_is_i8_u8));
            ...
            should_fuse |= input_data.is_type<resample>() && quantize_node.get_scale_shift_opt() && out_dt_is_i8_u8;
            ...
            should_fuse |= input_data.is_type<normalize>() && quantize_node.get_scale_shift_opt() && in_dt_is_i8_u8;
            ...
            should_fuse |= input_data.is_type<softmax>() && input_data.as<softmax>().get_primitive()->dimension == 1 &&
                           per_tensor_values;
        ...
        auto fuse_eltwise_f = [&](eltwise_node& node) {
            ...
            const std::vector<eltwise_mode> supported_modes = {eltwise_mode::sum,
                                                               eltwise_mode::prod,
                                                               eltwise_mode::sub,
                                                               eltwise_mode::div};
            ...
            for (size_t i = 0; i < parents.size(); i++) {
                can_fuse_parents[i] =
                    (parents[i].first->is_type<convolution>() &&
                     conv_supports_fusings(parents[i].first->as<convolution>())) ||
                    (parents[i].first->is_type<mvn>() && mvn_supports_fusings(parents[i].first->as<mvn>(), true)) ||
                    (parents[i].first->is_type<deconvolution>()) || (parents[i].first->is_type<permute>()) ||
                    (parents[i].first->is_type<resample>()) || (parents[i].first->is_type<space_to_depth>()) ||
                    (parents[i].first->is_type<fully_connected>() &&
                     fc_supports_fusings(parents[i].first->as<fully_connected>())) ||
                    (parents[i].first->is_type<gemm>() && gemm_supports_fusings(parents[i].first->as<gemm>())) ||
                    (parents[i].first->is_type<batch_to_space>()) || (parents[i].first->is_type<space_to_batch>()) ||
                    (parents[i].first->is_type<eltwise>() &&
                     eltwise_supports_fusings(parents[i].first->as<eltwise>())) ||
                    (parents[i].first->is_type<gather_nd>()) || (parents[i].first->is_type<gather_elements>()) ||
                    (parents[i].first->is_type<scatter_nd_update>()) ||
                    (parents[i].first->is_type<scatter_elements_update>()) || (parents[i].first->is_type<pooling>()) ||
                    ...
                    (parents[i].first->is_type<reduce>() && reduce_supports_fusings(parents[i].first->as<reduce>())) ||
                    (parents[i].first->is_type<lrn>());
            }

            // Disable fusion to a node on constant path when second input is in data flow
            for (size_t i = 0; i < parents.size(); i++) {
                can_fuse_parents[i] = can_fuse_parents[i] && (!parents[i].first->is_constant() ||
                                                              parents[parents.size() - 1 - i].first->is_constant());
            }

            if (node.in_shape_of_subgraph || parents[0].first->in_shape_of_subgraph ||
                parents[1].first->in_shape_of_subgraph)
                return;
            ...
            if (parent1.first->get_output_layout().is_static() && parent2.first->get_output_layout().is_static()) {
                ...
                // This is required to avoid an issue when shape is partially defined and incorrectly propagated to
                // further nodes which may ruin shape inference E.g. parent1 [?,?,768], parent2 [?,?,1] expected eltw
                // out shape: [?,?,768] but w/o this check we can fuse eltwise to parent2 and return [?,?,1] as output
            ...
            if (!supports_immad && fused_node->is_type<convolution>() && fused_node->get_users().size() > 1) {
                ...
                // 2. All intermediate fusing nodes except last fusing node(i.e. Elt4) should have only eltwise or
                // activation node as user.
                ...
                // 4. When node_queue has only one node, the while loop is ended and this node is fused to fused
                // node(Conv1)
                //      node_queue having one node means all user nodes from fused node(Conv1) converge at that node.
                // 5. if node_queue has multiple nodes even if the level of current_node is max_levels, it cannot be
                // fused.
                std::deque<std::pair<cldnn::program_node*, size_t>>
                    node_queue;  // std::pair<cldnn::program_node*, layer level>
                ...
                do {
                    ...
                    auto push_node_queue = [&](cldnn::program_node* in_node, size_t level) {
                        auto iter = std::find_if(node_queue.begin(),
                                                 node_queue.end(),
                                                 [&](std::pair<cldnn::program_node*, size_t> element) {
                    ...
                    auto invalid_user_iter =
                        std::find_if(curr_users.begin(), curr_users.end(), [&](cldnn::program_node* user) {
                            return (
                                user->is_output() ||
                                (!(user->is_type<eltwise>() && user->get_primitive()->input.size() == 2 &&
                                   (std::find(supported_modes.begin(),
                                              supported_modes.end(),
                    ...
                    if (invalid_user_iter != curr_users.end()) {
                        // If fused_node(i.e. Conv1) have invalid user node(that is not activation and eltwise ndoe), it
                        // cannot be fused
...
void prepare_primitive_fusing::fuse_constant_transposes(program& p) {
    std::function<const program_node*(const program_node*)> get_weightable_node =
        ...
        if (next_node->is_type<fully_connected>() || next_node->is_type<deconvolution>() ||
            next_node->is_type<convolution>() || next_node->is_type<deformable_conv>()) {
    ...
    while (itr != proc_order.end()) {
        ...
        if (permute_order.size() != 2 && weightable_node->is_type<fully_connected>()) {
            if (permute_order == std::vector<uint16_t>{0, 2, 1} || permute_order == std::vector<uint16_t>{0, 1, 3, 2}) {
                permute_order = {1, 0};
            } else {
                continue;
            }
        ...
        if (_lo.get_preferred_impl_type(const_cast<program_node&>(*weightable_node), format::any /*dummy*/) ==
            impl_types::onednn) {
            auto next_node = new_const_node.get_users().front();
            bool can_be_fused = next_node->is_type<reorder>() && next_node->as<reorder>().is_simple_reorder() &&
            ...
            if (can_be_fused) {
                ...
                auto new_reorder =
                    std::make_shared<reorder>(next_node->id() + "_reorder_fmt", new_const_node.id(), reorder_layout);
                ...
                auto new_reorder = std::make_shared<reorder>(new_const_node.id() + "_reorder_fmt",
                                                             new_const_node.id(),
                                                             reorder_layout);
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        while (fp_itr != fused_prims.end()) {
            ...
            if (fp.is_type<activation>() && fp_next.is_type<quantize>()) {
                ...
                can_skip &= ((act_prim->activation_function == activation_func::relu) &&
                             (act_prim->additional_params.a == 0.0f));
