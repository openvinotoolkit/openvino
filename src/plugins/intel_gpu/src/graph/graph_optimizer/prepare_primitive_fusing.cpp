...
void prepare_primitive_fusing::fuse_bias(program &p) {
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        if (is_3d_fully_connected(*node->get_dependency(0).first))
            out_features = static_cast<size_t>(node->get_dependency(0).first->get_output_layout().size.spatial[1]);
        else if (is_3d_fully_connected(*node->get_dependency(1).first))
            out_features = static_cast<size_t>(node->get_dependency(1).first->get_output_layout().size.spatial[1]);

        int bias_idx = -1;
        for (size_t i = 0; i < eltw_node.get_dependencies().size(); i++) {
            auto& dep = *eltw_node.get_dependency(i).first;
        ...
        auto& bias_node = *eltw_node.get_dependency(bias_idx).first;
        primitive_id bias_name = bias_node.id();
        auto& replace_candidate = bias_idx == 0 ? *eltw_node.get_dependency(1).first : *eltw_node.get_dependency(0).first;
        ...
        auto fuse_bias_f = [&p](program_node& prev_node, program_node& new_node, program_node& bias_node, program_node& eltw_node) {
            ...
            new_node.recalc_output_layouts();
        };
...
void prepare_primitive_fusing::fuse_simple_primitives(program &p) {
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        auto is_grouped_conv = [](convolution_node& node) -> bool {
            auto in_size = node.get_dependency(0).first->get_output_layout().size;
        ...
        auto conv_supports_fusings = [&](convolution_node& node) -> bool {
            ...
            const size_t in_feature = node.get_dependency(0).first->get_output_layout().size.feature[0];
        ...
        auto dts_supports_fusings = [](depth_to_space_node& node) -> bool {
            ...
            if (input_conv && out_eltw) {
                ...
                auto conv_size = conv.get_dependency(0).first->get_output_layout().size.spatial[0] % 128 == 0 &&
                                 conv.get_dependency(0).first->get_output_layout().size.spatial[1] % 2 == 0;
        ...
        auto fuse_scale_f = [&](scale_node& scale_node) {
            ...
            auto& input_data = *scale_node.get_dependency(0).first;
        ...
        auto fuse_eltwise_f = [&](eltwise_node& node) {
            ...
            std::vector<std::pair<program_node*, int>> parents = node.get_dependencies();
            std::list<cldnn::program_node*> users = node.get_users();

            std::vector<bool> can_fuse_parents = { false, false };

            for (size_t i = 0; i < parents.size(); i++) {
                can_fuse_parents[i] = (parents[i].first->is_type<convolution>() && conv_supports_fusings(parents[i].first->as<convolution>())) ||
                                      ...
                                      (parents[i].first->is_type<pooling>() && pooling_supports_fusings(parents[i].first->as<pooling>())) ||
            ...
            auto p1_raw_size = parent1.first->get_output_layout().size.sizes();
            auto p2_raw_size = parent2.first->get_output_layout().size.sizes();
