                ...
                for (size_t idx = 0; idx < usr->get_dependencies().size(); idx++) {
                    ...
                    if (static_cast<bool>(in_padding)) {
                        ...
                        for (size_t i = 0; i < in_layout.get_spatial_rank(); ++i) {
                            spatial_padding |= (in_padding.lower_size()[2 + i] != 0);
                        }
                        for (size_t i = 0; i < in_layout.get_spatial_rank(); ++i) {
                            spatial_padding |= (in_padding.upper_size()[2 + i] != 0);
                        }

                        bool feature_padding = false;
                        feature_padding |= (in_padding.lower_size()[1] != 0);
                        feature_padding |= (in_padding.upper_size()[1] != 0);

                        bool batch_padding = false;
                        batch_padding |= (in_padding.lower_size()[0] != 0);
                        batch_padding |= (in_padding.upper_size()[0] != 0);
