...
void prepare_quantization::prepare_scale_shift_opt(program &p, quantize_node& quantize_node) {
    ...
    for (size_t i = 0; i < scales_layout.count(); i++) {
      ...
      if (in_lo_val !=
              get_data_input_low(i % mem_input_low->get_layout().count()) ||
          in_hi_val !=
              get_data_input_high(i % mem_input_high->get_layout().count())) {
        per_tensor_in_range = false;
      }
      if (out_lo_val !=
              get_data_output_low(i % mem_output_low->get_layout().count()) ||
          out_hi_val !=
              get_data_output_high(i % mem_output_high->get_layout().count())) {
    ...
    if (per_tensor_out_range && !out_is_fp && levels != 256) {
      if ((out_is_int8 && out_lo_val == -128.f) ||
          (out_is_uint8 && out_lo_val == 0.f)) {
        need_min_clamp = false;
      }
      if ((out_is_int8 && out_hi_val == 127.f) ||
          (out_is_uint8 && out_hi_val == 255.f)) {
...
void prepare_quantization::prepare_dequantize_merge(program& p, eltwise_node& eltwise_node) {
    ...
    for (const auto& user : input.get_users()) {
      ...
      if (!user->is_type<eltwise>() ||
          user->get_dependencies().size() !=
              eltwise_node.get_dependencies().size()) {
        continue;
      }

        auto& eltwise_dep = user->as<eltwise>();
        if (eltwise_dep.get_primitive()->mode !=
            eltwise_node.get_primitive()->mode) {
          continue;
        }
...
void prepare_quantization::remove_fake_reorders(program& p, reorder_node& reorder_node) {
    ...
    if (!usr->is_type<convolution>() ||
        usr->get_input_layout(1).data_type != data_types::i8 ||
        !dep.is_input() ||
        ...
        dep.get_output_layout().format !=
            reorder_node.get_output_layout().format ||
        dep.get_output_layout().get_tensor() !=
            reorder_node.get_output_layout().get_tensor()) {
      return;
    }
...
bool prepare_quantization::optimize_quantize(program &p, quantize_node& quantize_node) {
    ...
    for (const auto& usr : input.get_users()) {
      if (!usr->is_type<quantize>() || usr == &quantize_node) {
        ...
        if (!input_low_node_second.is_type<data>() ||
            !input_high_node_second.is_type<data>() ||
            !output_low_node_second.is_type<data>() ||
            !output_high_node_second.is_type<data>()) {
          continue;
        }
        ...
        if (memcmp(mem_input_low_lock_first.data(),
                   mem_input_low_lock_second.data(),
                   mem_input_low_first->size()) != 0 ||
            memcmp(mem_input_high_lock_first.data(),
                   mem_input_high_lock_second.data(),
                   mem_input_high_first->size()) != 0 ||
            memcmp(mem_output_low_lock_first.data(),
                   mem_output_low_lock_second.data(),
                   mem_output_low_first->size()) != 0 ||
        ...
        if (quantize_prim_first->output_data_types[0] !=
                quantize_prim_second->output_data_types[0] ||
...
static void reorder_decompression_param_to_byfx(program_node& node, size_t dep_id, program& p) {
  if (dep_id >= node.get_dependencies().size()) {
    ...
    if (rank == 4 &&
        (dep_pshape[3].is_dynamic() || dep_pshape[3].get_length() != 1)) {
      return;
    }
