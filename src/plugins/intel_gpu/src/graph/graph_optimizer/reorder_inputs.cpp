...
std::map<program_node*, format::type> get_preferred_formats(program& p, layout_optimizer& lo) {
    ...
    if (onednn_impls_counter < 1 && lo.get_optimization_attributes().use_onednn_impls) {
        ...
        GPU_DEBUG_LOG(2) << "Disable oneDNN implementations globally" << std::endl;
    }
...
void propagate_formats_rec(std::map<program_node*, format::type>& fmt_map,
    ...
    fmt_map.at(node) = fmt;
    GPU_DEBUG_LOG(2) << "[reorder_inputs] propagate_formats_rec: " << node->id() << " - " << fmt_to_str(fmt) << std::endl;
...
void insert_reorders_in_dir(program& p, const std::map<program_node*, format::type>& fmt_map, reorder_factory& rf, layout_optimizer& lo, program_node* node) {
    ...
    for (auto next : next_cpy) {
        ...
        GPU_DEBUG_LOG(2) << __func__ << ":" << __LINE__ << ":" << dir_msg(dir) << "  " << node->id() << " --> " << next->id() << " ## "
        ...
        if (reorder && (in_layout.format != format::any && out_layout.format != format::any)) {
            ...
            GPU_DEBUG_LOG(2) << __func__ << ":" << __LINE__ << ":" << dir_msg(dir) << "  " << reorder_node.id()
...
void insert_reorders_in_dir<direction_e::backwards>(program& p, const std::map<program_node*, format::type>& fmt_map,
    ...
    for (auto next : next_cpy) {
        ...
        GPU_DEBUG_LOG(2) << __func__ << ":" << __LINE__ << ":" << dir_msg(direction_e::backwards)
        ...
        if (reorder && (in_layout.format != format::any && out_layout.format != format::any)) {
            auto& reorder_node = p.get_or_create(reorder);
            GPU_DEBUG_LOG(2) << __func__ << ":" << __LINE__ << ":" << dir_msg(direction_e::backwards) << "  " << reorder_node.id()
                             << "  Reorder is added" << std::endl;
            p.add_intermediate(reorder_node,
...
void reorder_inputs::run(program& p, layout_optimizer& lo, reorder_factory& rf) {
    ...
    GPU_DEBUG_LOG(2) << "[reorder_inputs] Preferred formats:" << std::endl;
    for (auto& node_fmt : fmt_map) {
        if (node_fmt.second != format::any) {
            GPU_DEBUG_LOG(2) << "[reorder_inputs]   " << node_fmt.first->id() << " " << fmt_to_str(node_fmt.second) << std::endl;
        }
    }
    ...
    GPU_DEBUG_LOG(2) << "[reorder_inputs] Selected formats:" << std::endl;
    for (auto node_ptr : p.get_processing_order()) {
        if (fmt_map.count(node_ptr) == 0)
            continue;

        auto fmt = fmt_map.at(node_ptr);
        GPU_DEBUG_LOG(2) << "[reorder_inputs]   " << node_ptr->id() << " " << fmt_to_str(fmt) << std::endl;
    }
    ...
    GPU_DEBUG_IF(debug_config->verbose >= 2) {
        ...
        GPU_DEBUG_LOG(2) << "[reorder_inputs] Total number of reorders: " << total_reorder_count.number / 2 << std::endl;
        GPU_DEBUG_LOG(2) << "[reorder_inputs] Total elements count of all reorders: " << total_reorder_count.total_sizes / 2 << std::endl;
        ...
        GPU_DEBUG_LOG(2) << "[reorder_inputs] Number of nodes with fused reorders: " << nodes_with_fusing << std::endl;
        GPU_DEBUG_LOG(2) << "----------------------------------------------" << std::endl;
    }
