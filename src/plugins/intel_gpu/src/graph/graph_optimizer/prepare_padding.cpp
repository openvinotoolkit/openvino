...
cldnn::padding prepare_padding::get_convolution_required_padding(const convolution_node& node) {
    ...
    auto no_required_padding_format = (conv_layout.format != cldnn::format::bfyx &&
    ...
    auto no_required_padding_3d = (prev_prim_output_layout.format == cldnn::format::b_fs_zyx_fsv16 &&
    ...
    auto no_required_padding_input = (conv_input_node.is_output() || conv_input_node.is_type<data>());

    // Padded offsets aren't supported by onednn kernels
    auto is_onednn_impl_type = (conv_input_node.get_preferred_impl_type() == impl_types::onednn ||
                                node.get_preferred_impl_type() == impl_types::onednn);

    if ((node.is_dynamic() && !node.use_explicit_padding()) ||
        no_required_padding_format ||
        no_required_padding_3d ||
        no_required_padding_input ||
        is_onednn_impl_type) {
        return cldnn::padding();
    }

    // Calculating input padding needed for convolution
    auto& filter_node = node.weights();
    ...
    auto needed_padding = cldnn::padding({0, 0, padding_begin_x, padding_begin_y, padding_begin_z}, {0, 0, padding_end_x, padding_end_y, padding_end_z}, 0);
...
void prepare_padding::run(program& p) {
    ...
    for (auto& pair : p.nodes_map) {
        ...
        auto needed_padding = get_convolution_required_padding(node);
        if (!needed_padding)
            continue;
