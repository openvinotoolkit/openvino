...
layout space_to_batch_inst::calc_output_layout(space_to_batch_node const& node, kernel_impl_params const& impl_param) {
    ...
    if (pads_begin.batch[0] != 0)
        CLDNN_ERROR_MESSAGE(
            desc->id,
            "pads_begin[0] is expected to be 0. Actual pads_begin[0] is " + std::to_string(pads_begin.batch[0]));

    if (pads_end.batch[0] != 0)
        CLDNN_ERROR_MESSAGE(
            desc->id,
            "pads_end[0] is expected to be 0. Actual pads_end[0] is " + std::to_string(pads_end.batch[0]));

    if ((input_layout.feature() + pads_begin.feature[0] + pads_end.feature[0]) % block_shape.feature[0] != 0)
        CLDNN_ERROR_MESSAGE(desc->id, "Input feature shape after padding must be divisible by block_shape");

    for (size_t i = 0; i < spatial_num; ++i)
        if ((input_layout.spatial(i) + pads_begin.spatial[i] + pads_end.spatial[i]) % block_shape.spatial[i] != 0)
            CLDNN_ERROR_MESSAGE(desc->id, "Input spatial shapes after padding must be divisible by block_shape");
...
std::vector<layout> space_to_batch_inst::calc_output_layouts(space_to_batch_node const& /*node*/,
...
template std::vector<layout> space_to_batch_inst::calc_output_layouts<ov::PartialShape>(
...
space_to_batch_inst::typed_primitive_inst(network& network, space_to_batch_node const& node) : parent(network, node) {}
