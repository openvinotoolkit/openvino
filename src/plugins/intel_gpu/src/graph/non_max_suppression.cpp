...
std::vector<layout> non_max_suppression_inst::calc_output_layouts(non_max_suppression_node const& /*node*/,
    ...
    if (memory_deps.count(2)) {
        ...
        auto max_output_boxes_per_class_tensor =
            make_tensor(max_output_boxes_per_class_mem->get_layout(), max_output_boxes_per_class_lock.data());
        ...
        // Output tensor has the following shape: [min(num_boxes, max_output_boxes_per_class) * num_batches *
        // num_classes, 3] The first dimension is an upper bound for the number of possible selected boxes
...
template std::vector<layout> non_max_suppression_inst::calc_output_layouts<ov::PartialShape>(
    non_max_suppression_node const& node,
    const kernel_impl_params& impl_param);
