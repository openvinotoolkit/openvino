...
experimental_detectron_topk_rois_inst::typed_primitive_inst(network& network,
                                                            experimental_detectron_topk_rois_node const& node)
    : parent(network, node) {}
...
layout experimental_detectron_topk_rois_inst::calc_output_layout(experimental_detectron_topk_rois_node const& node,
    ...
    return {input_layout.data_type, input_layout.format, {roi_num, input_layout.get_tensor().sizes()[1], 1, 1}};
}
