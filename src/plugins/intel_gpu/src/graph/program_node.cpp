                ...
                // 1. pre-scale & pre-shift
                {
                    if (q_param->_per_tensor_input_scale && q_param->_per_tensor_input_shift) {
                        ...
                        if (q_param->_per_tensor_input_scale) {
                            ...
                            dnnl::memory::desc in_scale_desc = onednn::layout_to_memory_desc(in_scale, onednn::get_default_data_format(in_scale), false);
                        ...
                        if (q_param->_need_pre_shift) {
                            if (q_param->_per_tensor_input_shift) {
                                ...
                                dnnl::memory::desc in_shift_desc = onednn::layout_to_memory_desc(in_shift, onednn::get_default_data_format(in_shift), false);
                    ...
                    if (q_param->_need_post_scale && q_param->_need_post_shift &&
                        ...
                        if (q_param->_need_post_scale) {
                            if (q_param->_per_tensor_output_scale) {
                                ...
                                dnnl::memory::desc out_scale_desc = onednn::layout_to_memory_desc(out_scale, onednn::get_default_data_format(out_scale), false);
                        ...
                        if (q_param->_need_post_shift) {
                            if (q_param->_per_tensor_output_shift) {
                                ...
                                dnnl::memory::desc out_shift_desc = onednn::layout_to_memory_desc(out_shift, onednn::get_default_data_format(out_shift), false);
            ...
            // ********************************* Rare case with input range usage ********************************* //
                ...
                // 1. clamp
                {
                    if (q_param->_need_clamp) {
                        ...
                        dnnl::memory::desc in_lo_desc = onednn::layout_to_memory_desc(in_lo, onednn::get_default_data_format(in_lo), false);
                        dnnl::memory::desc in_hi_desc = onednn::layout_to_memory_desc(in_hi, onednn::get_default_data_format(in_hi), false);
