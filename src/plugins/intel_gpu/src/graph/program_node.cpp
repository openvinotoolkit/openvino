                ...
                // 1. pre-scale & pre-shift
                {
                    if (q_param->_per_tensor_input_scale && q_param->_per_tensor_input_shift) {
                        ...
                        if (q_param->_per_tensor_input_scale) {
                            ...
                            dnnl::memory::desc in_scale_desc = onednn::layout_to_memory_desc(in_scale,
                                (this->is_type<gemm>() || this->is_type<fully_connected>()),
                                cldnn::format::is_blocked(this->get_output_layout().format));
                        ...
                        if (q_param->_need_pre_shift) {
                            if (q_param->_per_tensor_input_shift) {
                                ...
                                dnnl::memory::desc in_shift_desc = onednn::layout_to_memory_desc(in_shift,
                                    (this->is_type<gemm>() || this->is_type<fully_connected>()),
                                    cldnn::format::is_blocked(this->get_output_layout().format));
