...
void program_node::replace_dependency(program_node const& old_dep,
...
std::set<size_t> program_node::get_memory_dependencies() const {
...
bool program_node::is_padded_spatial(size_t idx) const {
    ...
    return std::any_of(lower_size.spatial.begin(),
                       lower_size.spatial.end(),
                       [](const tensor::value_type& el) {
                           return el != 0;
                       }) ||
           std::any_of(upper_size.spatial.begin(), upper_size.spatial.end(), [](const tensor::value_type& el) {
               return el != 0;
           });
}
        ...
        if (desc.is_type<activation>()) {
            ...
            if (fused_desc->activation_function == cldnn::activation_func::relu_negative_slope &&
                ...
                if (alg == dnnl::algorithm::undef)
                    OPENVINO_THROW(
                        "Activations that are undef algorithms must be converted to other activations before "
                        "pushing to post-op.");
            ...
            auto set_binary_op = [&](dnnl::algorithm alg, onednn_post_op_type op_type) {
                if (is_type<fully_connected>()) {
                    ...
                    update_onednn_post_op_list(op_type,
                                               dep_idx,
                                               dnnl::memory::format_tag::ab,
                                               false,
                                               mem_desc.get_dims(),
                                               mem_desc.get_data_type());
                    ...
                    update_onednn_post_op_list(op_type,
                                               dep_idx,
                                               onednn::convert_data_format(in.format),
                                               false,
                                               mem_desc.get_dims(),
                                               mem_desc.get_data_type());
                }
