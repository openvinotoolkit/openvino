...
template std::vector<layout> selective_ssm_inst::calc_output_layouts<ov::PartialShape>(const selective_ssm_node& node,
...
std::string selective_ssm_inst::to_string(const selective_ssm_node& node) {
    ...
    ssm_info.add("num_heads", desc->num_heads);
    ssm_info.add("num_groups", desc->num_groups);
    ssm_info.add("head_dim", desc->head_dim);
    ssm_info.add("state_size", desc->state_size);
