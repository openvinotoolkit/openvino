...
std::string paged_selective_ssm_inst::to_string(const paged_selective_ssm_node& node) {
    ...
    pssm_info.add("num_heads", desc->num_heads);
    pssm_info.add("num_groups", desc->num_groups);
    pssm_info.add("head_dim", desc->head_dim);
    pssm_info.add("state_size", desc->state_size);
