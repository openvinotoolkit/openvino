...
std::pair<std::shared_ptr<reorder>, bool> reorder_factory::get_reorder(primitive_id src_id, const layout& in_layout, const layout& out_layout) {
    return get_reorder(src_id, 0, in_layout, out_layout);
}

std::pair<std::shared_ptr<primitive>, bool> reorder_factory::get_weights_reorder(primitive_id input_id, std::shared_ptr<WeightsReorderParams> reorder_params) {
...
bool layout_optimizer::is_format_supported(program_node& node, format::type fmt) {
    ...
    return test_format<bool>(node, fmt, [](program_node& n) {
        return n.type()->has_impl_for(n);
    });
}
...
bool layout_optimizer::can_fuse_reorder(program_node& prev, program_node& next, format fmt_prev, format fmt_next) {
    ...
    if (next.is_type<region_yolo>() || (next.is_type<strided_slice>() && next.get_preferred_impl_type() != cldnn::impl_types::cpu)) {
        return true;
    }
    ...
    if (reorder_layout.format == next.get_preferred_input_fmt(next.get_dependency_index(reorder_node)) && !reorder_layout.data_padding) {
        return false;
    }

    // resample_opt kernel can work cross-layout between fsv16 and fsv32
    if (next.is_type<resample>() &&
        (fmt_prev == format::b_fs_yx_fsv16 || fmt_prev == format::b_fs_yx_fsv32 || fmt_prev == format::bs_fs_yx_bsv16_fsv16 ||
         fmt_prev == format::bs_fs_yx_bsv16_fsv32 || fmt_prev == format::bs_fs_yx_bsv32_fsv16 || fmt_prev == format::bs_fs_yx_bsv32_fsv32) &&
        (fmt_next == format::b_fs_yx_fsv16 || fmt_next == format::b_fs_yx_fsv32 || fmt_next == format::bs_fs_yx_bsv16_fsv16 ||
         fmt_next == format::bs_fs_yx_bsv16_fsv32 || fmt_next == format::bs_fs_yx_bsv32_fsv16 || fmt_next == format::bs_fs_yx_bsv32_fsv32)) {
        return true;
    }
    ...
    if (next.is_type<mvn>() &&
        (fmt_prev == format::b_fs_yx_fsv16 || fmt_prev == format::b_fs_yx_fsv32 || fmt_prev == format::b_fs_zyx_fsv16 || fmt_prev == format::b_fs_zyx_fsv32) &&
        (fmt_next == format::b_fs_yx_fsv16 || fmt_next == format::b_fs_yx_fsv32 || fmt_next == format::b_fs_zyx_fsv16 || fmt_next == format::b_fs_zyx_fsv32)) {
        return true;
    }

    if (next.is_type<pooling>() && (((prev_simple && next_simple) && (prev_dt == next_dt)) || ((fmt_prev == format::b_fs_yx_fsv4 && fmt_next == format::bfyx) &&
    ...
    if (next.is_type<eltwise>()) {
        ...
        if (use_onednn_impls && (fmt_prev == format::bfyx) && (fmt_next == format::bs_fs_yx_bsv4_fsv2) && prev.is_input() &&
            (prev_dt == data_types::u8 || prev_dt == data_types::i8)) {
            return true;
        }
    }
    ...
    if (next.is_type<fully_connected>() &&
        (fmt_prev == format::bfyx || fmt_prev == format::yxfb || fmt_prev == format::b_fs_yx_fsv16 || fmt_prev == format::fs_b_yx_fsv32 ||
         fmt_prev == format::b_fs_yx_fsv32 ||
         (fmt_prev == format::b_fs_yx_fsv4 && prev_output_layout.feature() % 32 == 0 && prev_output_layout.spatial(0) == 1 &&
    ...
    if (next.is_type<quantize>()) {
        if ((fmt_prev == format::bfyx || fmt_prev == format::bfzyx)) {
            ...
            if (fmt_next == format::b_fs_yx_fsv16 || fmt_next == format::b_fs_zyx_fsv16 || fmt_next == format::bs_fs_yx_bsv16_fsv16 ||
    ...
    if (next.is_type<convolution>()) {
        ...
        if (!use_onednn_impls || next.get_preferred_impl_type() == impl_types::ocl) {
            ...
            if (fmt_prev == format::bfyx && (fmt_next == format::b_fs_yx_fsv16 || fmt_next == format::bs_fs_yx_bsv32_fsv16) && out_channel_count >= 16 &&
                in_channel_count <= 4 && !conv_node.get_primitive()->activations_zero_points.is_valid() &&
            ...
            if ((fmt_prev == format::b_fs_yx_fsv4 || fmt_prev == format::bs_fs_yx_bsv4_fsv4 || fmt_prev == format::bs_fs_yx_bsv8_fsv4) &&
                ((fmt_next == format::b_fs_yx_fsv32 && (in_channel_count == 3 || in_channel_count == 4)) ||
                 ...
                 (fmt_next == format::b_fs_yx_fsv16 && out_channel_count >= 16 &&
                  (in_channel_count == 3 || (in_channel_count == 4 && (prev_dt == data_types::u8 || prev_dt == data_types::i8)))))) {
                return true;
            }
        }
        ...
        if ((!prev.is_type<quantize>() || (prev_dt != data_types::i8 && prev_dt != data_types::u8)) &&
            ...
            (fmt_next == format::b_fs_yx_fsv4 || fmt_next == format::bs_fs_yx_bsv16_fsv16)) {
            return true;
        }

        if (fmt_prev == format::bfyx &&
            ((fmt_next == format::b_fs_yx_fsv16 || fmt_next == format::bs_fs_yx_bsv16_fsv16) && out_channel_count >= 16 && in_channel_count == 3)) {
            return true;
        }

        if (fmt_prev == format::bfzyx &&
            ((fmt_next == format::b_fs_zyx_fsv16 || fmt_next == format::bs_fs_zyx_bsv16_fsv16) && out_channel_count >= 16 && in_channel_count == 3)) {
            return true;
        }
...
bool layout_optimizer::can_fuse_reorder_to_prev(program_node& prev, reorder_node& node, format fmt_prev, format fmt_next) {
    ...
    if ((format::is_simple_data_format(fmt_prev) && format::is_simple_data_format(fmt_next)) &&
        ...
        if ((prev.is_type<mvn>() || prev.is_type<concatenation>() || prev.is_type<gather>() || prev.is_type<broadcast>() || prev.is_type<select>() ||
             prev.is_type<eltwise>() || prev.is_type<rms>()) &&
        ...
        if (prev.is_type<fully_connected>() && (prev.get_preferred_impl_type() == cldnn::impl_types::onednn) && node.only_precision_changed() &&
            node.is_simple_reorder() && !output_layout.data_padding && !data_type_traits::is_i8_u8(prev_layout.data_type) &&
            !data_type_traits::is_i8_u8(output_layout.data_type)) {
            return true;
        }
    }
    ...
    if (prev.is_dynamic() || (!node.get_users().empty() && node.get_users().front()->is_dynamic())) {
        if (!prev.is_type<permute>() && !prev.is_type<group_normalization>() && !prev.is_type<mvn>()) {
            return false;
        }
        is_dynamic = true;
    }
    ...
    if (prev.is_type<region_yolo>() || (prev.is_type<detection_output>() && prev.get_preferred_impl_type() != cldnn::impl_types::cpu)) {
        return true;
    }
    ...
    if (prev.is_type<group_normalization>() && is_dynamic && !prev.has_fused_primitives() && fmt_prev == format::bfyx && fmt_next == format::b_fs_yx_fsv16 &&
    ...
    if (prev.is_type<one_hot>() && !data_type_traits::is_floating_point(dt_prev) && data_type_traits::is_floating_point(dt_next) && fmt_prev == fmt_next) {
        return true;
    }

    if (prev.is_type<quantize>() && (fmt_next == format::b_fs_yx_fsv4 || fmt_next == format::b_fs_zyx_fsv32 || fmt_next == format::b_fs_yx_fsv32 ||
    ...
    if (prev.is_type<permute>()) {
        if (is_dynamic) {
            return !prev.has_fused_primitives() && fmt_prev == format::bfyx && fmt_next == format::b_fs_yx_fsv16;
        }
    ...
    if (use_onednn_impls) {
        ...
        if (reorder_layout.format == prev.get_preferred_output_fmt() && reorder_layout.data_padding == prev.get_output_layout().data_padding) {
            return true;
        }

        if (prev.is_type<eltwise>() && is_mixed_layout(prev, *next, false, {{format::bs_fs_zyx_bsv32_fsv32, format::bs_fs_zyx_bsv32_fsv16}})) {
            return true;
        }
    }
...
namespace {
bool should_use_winograd_2x3_s1(const convolution_node& node, const layout& input_layout, const layout& weights_layout, bool output_size_handling_enabled) {
    ...
    return input_layout.data_type == data_types::f16 &&
           ...
           && (input_layout.spatial(0) >= 8 || input_layout.spatial(1) >= 8)  // disable winograd for small spatials as perf is poor
...
bool layout_optimizer::convolution_bfyx_opt(const layout& output_layout, const layout& weights_layout, std::shared_ptr<const convolution> conv) {
    // A set of rules that define when bfyx mem format has better performance than yxfb
    return output_layout.batch() == 16 || output_layout.batch() % 16 != 0 || output_layout.data_type != data_types::f16 || weights_layout.batch() % 16 != 0 ||
           ((weights_layout.spatial(0) != 1 || weights_layout.spatial(1) != 1) && (weights_layout.spatial(0) < 5 || weights_layout.spatial(1) < 5) &&
            (conv->stride[0] <= 1 || conv->stride[1] <= 1) &&
            (weights_layout.feature() > 32 || output_layout.spatial(0) >= 224 || output_layout.spatial(1) >= 224) &&
            (weights_layout.feature() > 64 || output_layout.spatial(0) >= 112 || output_layout.spatial(1) >= 112) &&
            (weights_layout.feature() > 128 || output_layout.spatial(0) >= 56 || output_layout.spatial(1) >= 56) &&
            (weights_layout.feature() > 256 || output_layout.spatial(0) >= 28 || output_layout.spatial(1) >= 28) &&
            (weights_layout.feature() > 512 || output_layout.spatial(0) >= 14 || output_layout.spatial(1) >= 14) &&
            (weights_layout.feature() > 1024 || output_layout.spatial(0) > 7 || output_layout.spatial(1) > 7)) ||
           ...
           (output_layout.spatial(0) == 82 && output_layout.spatial(1) == 82) || (output_layout.batch() >= 128) ||
...
bool layout_optimizer::convolution_byxf_opt(const layout& input_layout,
    ...
    return (output_layout.data_type == data_types::f16 && weights_layout.spatial(0) == 1 && all_ones(conv->dilation) && !node.get_transposed() &&
            node.get_groups() == 1 && (input_layout.is_static() && input_layout.feature() % 32 == 0) && weights_layout.spatial(1) == 1 &&
            output_layout.feature() % 64 == 0 && weights_layout.batch() % 64 == 0 && all_ones(conv->stride) && all_zeroes(conv->padding_begin) &&
...
bool layout_optimizer::convolution_b_fs_yx_fsv16_opt(const layout& input_layout,
    ...
    bool i8_dt_case = (input_layout.data_type == data_types::u8 || input_layout.data_type == data_types::i8) && weights_layout.data_type == data_types::i8;
    ...
    if (i8_dt_case) {
        ...
        if (input_layout.format.dimension() == 4 &&
            ((ks_x == 7 && ks_y == 7) || (ks_x == 3 && ks_y == 3) || (ks_x == 1 && ks_y == 1) || (ks_x == 5 && ks_y == 5)) && output_layout.feature() >= 16 &&
            ((conv->groups == 1) || conv->groups == static_cast<uint32_t>(input_layout.feature()))) {
            return true;
        }
        // Check for grouped convolution
        if (input_layout.format.dimension() == 4 && input_layout.batch() < 16 && out_features_per_group >= 16 &&
            // Need to extend imad fsv4 kernel to handle e.g. 3 input features
            // per group
            (in_features_per_group % 4 == 0) && ((conv->dilation[conv->dilation.size() - 1] + 1) * (ks_x - 1)) <= 16) {
            return true;
        }
        // Check for fsv16 imad kernel
        return (input_layout.format.dimension() == 4) && ((in_features_per_group > 8) || (out_features_per_group >= 4));
    }
    // A set of rules that define when b_fs_yx_fsv16 mem format can be used for fp16/fp32 case
    int32_t feature_block_size = 16;
    bool correct_data_type =
        (input_layout.data_type == data_types::f16 || input_layout.data_type == data_types::f32) && (weights_layout.data_type == input_layout.data_type);
    ...
    bool correct_in_feature = (input_layout.feature() >= required_feature_num && output_layout.feature() >= required_feature_num);
    ...
    bool grouped = ((feature_block_size % out_features_per_group == 0) && (feature_block_size % in_features_per_group == 0) &&
                    (feature_block_size / out_features_per_group > 1) && (feature_block_size / in_features_per_group > 1) && (out_features_per_group != 1) &&
    ...
    return correct_data_type && correct_batch && correct_spatial_dims && correct_in_feature && (conv->groups == 1 || depthwise || grouped);
}
...
bool layout_optimizer::should_select_b_fs_yx_fsv16_layout(const convolution_node& node, const layout& weights_layout) {
    auto prim = node.get_primitive();
    auto input_layout = node.get_input_layout(0);
    const auto cond_denom = _total_conv > 0 ? 1.0f / static_cast<float>(_total_conv) : 1.0f;
    ...
    auto is_prev_conv_node_supports_layout =
        node.get_dependency(0).is_type<convolution>() && is_format_optimized(node.get_dependency(0).as<convolution>(), format::b_fs_yx_fsv16);
    ...
    return ((((_optimization_attributes.b_fs_yx_fsv16_network) != 0) &&
           ...
           !has_reorder_before_mvn(reinterpret_cast<const program_node&>(node), 0, 3, 8300000);
...
bool layout_optimizer::convolution_b_fs_zyx_fsv16_opt(const layout& input_layout,
    ...
    // Check for fsv16 imad kernel
    if ((input_layout.format.dimension() == 5) && (input_layout.data_type == data_types::i8 || input_layout.data_type == data_types::u8) &&
    ...
    bool format_ver =
        (input_layout.format == format::bfzyx || input_layout.format == format::b_fs_zyx_fsv16 || input_layout.format == format::bs_fs_zyx_bsv16_fsv16);
    ...
    bool groups_ver = conv->groups == 1 || out_features_per_group % 16 == 0 || (conv->groups > 1 && out_features_per_group == 8);
...
bool layout_optimizer::convolution_bs_fs_yx_bsv16_fsv16_opt(const layout& input_layout,
    ...
    int8_sup &= (input_layout.batch() % 16 == 0 && weights_layout.data_type == data_types::i8 && !conv->activations_zero_points.is_valid() &&
                 !conv->weights_zero_points.is_valid());
    auto ks_x = weights_layout.spatial(0);
    auto ks_y = weights_layout.spatial(1);
...
bool layout_optimizer::convolution_fs_b_yx_fsv32_opt(const layout& input_layout,
    ...
    return (input_layout.format == format::fs_b_yx_fsv32) || (correct_out_feature && correct_in_feature && correct_batch && (dw_conv || conv->groups == 1));
}

bool layout_optimizer::deconvolution_b_fs_zyx_fsv16_opt(const layout& input_layout,
                                                        const layout& /*weights_layout*/,
                                                        std::shared_ptr<const deconvolution> deconv) {
    // A set of rules that define when b_fs_zyx_fsv16 mem format can be used
    if ((input_layout.format == format::bfzyx || input_layout.format == format::b_fs_zyx_fsv16 || input_layout.format == format::bs_fs_zyx_bsv16_fsv16) &&
        (input_layout.data_type == data_types::f32 || input_layout.data_type == data_types::f16)) {
        return true;
    }

    return input_layout.format.dimension() == 5 && (input_layout.data_type == data_types::i8 || input_layout.data_type == data_types::u8);
}

bool layout_optimizer::deconvolution_b_fs_yx_fsv16_opt(const layout& input_layout, const layout& weights_layout, std::shared_ptr<const deconvolution> deconv) {
    ...
    return input_layout.format.dimension() == 4 && (input_layout.data_type == data_types::i8 || input_layout.data_type == data_types::u8);
}
...
bool layout_optimizer::users_for_convolution_byxf_opt(const program_node& node, uint32_t depth) {
    ...
    for (const auto& user : node.get_users()) {
        ...
        if ((user->type() == cldnn::eltwise::type_id() && !is_scale_shift(user->as<eltwise>())) || user->type() == cldnn::pooling::type_id()) {
            ...
            if (convolution_byxf_opt(node.get_output_layout(), user->calc_output_layout(), user->get_input_layout(1), user->as<convolution>())) {
                if (!users_for_convolution_byxf_opt(*user, depth - 1)) {
                    return false;
                }
            } else {
...
bool layout_optimizer::deps_for_convolution_byxf_opt(const program_node& node, uint32_t depth) {
    ...
    for (const auto& dep : node.get_dependencies()) {
        ...
        if (dep.first->is_type<convolution>()) {
            ...
            if (!convolution_byxf_opt(conv_dep.get_input_layout(), conv_dep.get_output_layout(), conv_dep.weights().get_output_layout(), conv_dep)) {
                return false;
            }
...
format layout_optimizer::imad_case(const convolution_node& node) const {
...
bool layout_optimizer::is_mixed_layout(program_node& prev, program_node& next, bool check_data_type, std::vector<std::pair<format, format>> custom_list) const {
    auto prev_layout = prev.get_output_layout();
    ...
    for (auto& pair : check_list) {
        if ((prev_fmt == pair.first && next_fmt == pair.second) &&
            ...
            if ((next_fmt == format::bs_fs_yx_bsv32_fsv16 || next_fmt == format::bs_fs_zyx_bsv32_fsv16) && (next_dt == data_types::f32))
                return false;
        ...
        if ((next_fmt == pair.first && prev_fmt == pair.second) &&
            ...
            if ((prev_fmt == format::bs_fs_yx_bsv32_fsv16 || prev_fmt == format::bs_fs_zyx_bsv32_fsv16) && (prev_dt == data_types::f32))
                return false;
...
format layout_optimizer::get_expected_format(const convolution_node& node) {
    auto prim = node.get_primitive();
    auto input_layout = node.get_input_layout(0);
    ...
    const bool activation_only_fusion = fused_prims.empty() || (fused_prims.size() == 1 && fused_prims[0].is_type<activation>() && fused_prims[0].deps.empty());
    ...
    if (use_onednn_impls && onednn_valid_post_ops && node.get_preferred_output_fmt() != format::any) {
        ...
        if (use_onednn_impls && i8_u8_input) {
        ...
        } else if (input_layout.get_rank() == 4 && input_layout.feature() == 1 && input_layout.spatial(0) == 1 && weights_layout.spatial(0) == 1 &&
            ...
            if (((_optimization_attributes.b_fs_yx_fsv16_network != 0) && convolution_b_fs_yx_fsv16_opt(input_layout, output_layout, weights_layout, prim))) {
            ...
            expected_format = cldnn::format::bs_fs_yx_bsv16_fsv16;
                   ...
                   (!has_reorder_before_mvn(reinterpret_cast<const program_node&>(*node.get_users().front()), 0, 3, 1000000) ||
            ...
            expected_format = cldnn::format::b_fs_yx_fsv16;
        } else if (output_layout.data_type == data_types::f16 && layout_optimizer::convolution_byxf_opt(input_layout, output_layout, weights_layout, node) &&
        ...
        } else if (output_layout.format == format::b_fs_yx_fsv4 || output_layout.format == format::os_is_yx_osv16_isv4) {
            // imad case
            // nothing to do, just go out from here.
        } else if (output_layout.batch() % 32 == 0 && output_layout.data_type == data_types::f32 && input_layout.spatial(0) == 1 &&
                   input_layout.spatial(1) > 1 && input_layout.get_rank() <= 4) {
            expected_format = cldnn::format::bs_fs_yx_bsv16_fsv16;
...
format layout_optimizer::get_expected_format(const deconvolution_node& node) {
    auto prim = node.get_primitive();
    auto input_layout = node.get_input_layout(0);
    auto output_layout = node.get_output_layout(0);
    ...
    if (use_onednn_impls && available.count(impl_types::onednn) > 0) {
        ...
        expected_format = node.get_preferred_output_fmt();
    } else if ((_optimization_attributes.b_fs_zyx_fsv16_network != 0) && deconvolution_b_fs_zyx_fsv16_opt(output_layout, weights_layout, prim)) {
    ...
    } else if (((_optimization_attributes.b_fs_yx_fsv16_network) != 0) && deconvolution_b_fs_yx_fsv16_opt(output_layout, weights_layout, prim)) {
...
format layout_optimizer::get_expected_format(const quantize_node& node) {
    ...
    } else if (layout.is_static() && layout.format.spatial_num() == 2 && (layout.data_type == data_types::i8 || layout.data_type == data_types::u8) &&
               layout.batch() % 16 == 0) {
        if (layout.feature() > 8) {
    ...
    if (layout.is_static() && expected == format::any && (node.get_users().size() == 1 && node.get_users().front()->is_type<concatenation>()) &&
...
bool layout_optimizer::is_primitive_implemented_for_onednn(program_node& node) {
    return node.is_type<fully_connected>() || node.is_type<gemm>() || node.is_type<pooling>() || node.is_type<convolution>() || node.is_type<deconvolution>() ||
...
impl_types layout_optimizer::get_preferred_impl_type(program_node& node, format preferred_format) {
    ...
    auto impl = test_format<std::shared_ptr<ImplementationManager>>(node, preferred_format, [&shape_type](program_node& n) {
...
format layout_optimizer::get_preferred_format(program_node& node) {
    ...
    if (!_forcing_map.empty() && _forcing_map.count(node.id()) != 0) {
        ...
        if (node.as<dft>().get_primitive()->mode == dft_mode::real && node.as<dft>().get_primitive()->direction == dft_direction::forward) {
...
bool layout_optimizer::has_all_enabled_onednn_impls_optimization_attribute() {
    return is_enabled_onednn_for<concatenation>() && is_enabled_onednn_for<convolution>() && is_enabled_onednn_for<deconvolution>() &&
           ...
           is_enabled_onednn_for<gru_seq>() && is_enabled_onednn_for<lstm_seq>() && is_enabled_onednn_for<pooling>() && is_enabled_onednn_for<reduce>() &&
           is_enabled_onednn_for<reorder>();
}
...
const std::vector<std::pair<format::type, bool>> layout_optimizer::optimized_formats = {{format::b_fs_yx_fsv16, true},
