...
bool layout_optimizer::is_depthwise(const convolution_node& node) const {
        const int32_t output_channels = static_cast<int32_t>(node.get_output_layout(0).feature());
        const int32_t input_channels = static_cast<int32_t>(node.get_input_layout(0).feature());
...
bool layout_optimizer::convolution_b_fs_yx_fsv16_opt(const layout& input_layout,
    ...
    int32_t in_features_per_group = static_cast<int32_t>(input_layout.feature() / conv->groups);
    int32_t out_features_per_group = static_cast<int32_t>(output_layout.feature() / conv->groups);
