...
static bool is_reduce_blocked_axes(reduce_node const& node) {
    ...
    if (input_layout.is_static() && (count(reduce_axes.begin(), reduce_axes.end(), 1) > 0 ||
...
bool layout_optimizer::onednn_check_data_types_for_convolution(data_types in_dt, data_types wei_dt, data_types out_dt) {
    if ((in_dt == data_types::f16 && wei_dt == data_types::f16) &&
        (out_dt == data_types::f16 || out_dt == data_types::f32 || out_dt == data_types::i8 ||
         out_dt == data_types::u8))
        return true;
    if ((in_dt == data_types::i8 || in_dt == data_types::u8) && wei_dt == data_types::i8 &&
        (out_dt == data_types::f32 || out_dt == data_types::i32 || out_dt == data_types::f16 ||
...
bool layout_optimizer::onednn_check_data_types_for_deconvolution(data_types in_dt,
                                                                 data_types wei_dt,
                                                                 data_types out_dt) {
    if ((in_dt == data_types::f16 && wei_dt == data_types::f16) &&
...
bool layout_optimizer::onednn_check_data_types_for_fc_gemm(data_types in_dt, data_types wei_dt, data_types out_dt) {
    ...
    if (in_dt == data_types::f32 && wei_dt == data_types::f32)
        return true;
    if ((in_dt == data_types::i8 || in_dt == data_types::u8) && (wei_dt == data_types::i8) &&
        (out_dt == data_types::i8 || out_dt == data_types::u8 || out_dt == data_types::i32 ||
         out_dt == data_types::f16 || out_dt == data_types::f32))
        return true;
    return false;
}
...
std::pair<std::shared_ptr<primitive>, bool> reorder_factory::get_weights_reorder(
...
bool layout_optimizer::is_format_supported(program_node& node, format::type fmt) {
    ...
    if (node.is_type<mvn>() && fmt == format::b_fs_yx_fsv16 && node.get_input_layout(0).data_type != data_types::i8 &&
...
bool layout_optimizer::can_fuse_reorder(program_node& prev, program_node& next, format fmt_prev, format fmt_next) {
    ...
    auto is_input_reorder = [](program_node& prev, program_node& next) {
        auto found_reorder = std::find_if(next.get_dependencies().begin(),
                                          next.get_dependencies().end(),
    ...
    if (next.is_type<pooling>() && (((prev_simple && next_simple) && (prev_dt == next_dt)) ||
    ...
    if (next.is_type<fully_connected>() &&
        (fmt_prev == format::bfyx || fmt_prev == format::yxfb || fmt_prev == format::b_fs_yx_fsv16 ||
         fmt_prev == format::fs_b_yx_fsv32 || fmt_prev == format::b_fs_yx_fsv32 ||
         (fmt_prev == format::b_fs_yx_fsv4 && prev_output_layout.feature() % 32 == 0 &&
          prev_output_layout.spatial(0) == 1 && prev_output_layout.spatial(1) == 1)) &&
        is_input_reorder(prev, next))
        return true;

    if (next.is_type<convolution>() && fmt_prev == format::b_fs_yx_fsv16 && fmt_next == format::b_fs_yx_fsv4 &&
        is_input_idx(0))
        return true;

    if (next.is_type<quantize>() && (fmt_prev == format::bfyx || fmt_prev == format::bfzyx) && prev.is_input() &&
        (prev_dt == data_types::u8 || prev_dt == data_types::i8))
        return true;

    if (!use_onednn_impls || next.get_preferred_impl_type() == impl_types::ocl) {
        if (next.is_type<convolution>() && (fmt_prev == format::bfyx || fmt_prev == format::bs_fs_yx_bsv4_fsv2) &&
            ((fmt_next == format::fs_b_yx_fsv32 && next.as<convolution>().get_primitive()->groups == 1) ||
             (fmt_next == format::b_fs_yx_fsv32 &&
              (prev_output_layout.feature() == 3 || prev_output_layout.feature() == 4)) ||
             (fmt_next == format::bs_fs_yx_bsv32_fsv32 &&
              (prev_output_layout.feature() == 3 || prev_output_layout.feature() == 4)) ||
             (fmt_next == format::bs_fs_yx_bsv32_fsv16 &&
              (prev_output_layout.feature() == 3 || prev_output_layout.feature() == 4)) ||
             (fmt_next == format::bs_fs_yx_bsv16_fsv16 && next_output_layout.feature() % 16 == 0 &&
              prev_output_layout.feature() == 3) ||
             (fmt_next == format::bs_fs_yx_bsv16_fsv16 && next_output_layout.feature() >= 16 &&
              prev_output_layout.feature() == 3 &&
              (next_output_layout.data_type != data_types::i8 && next_output_layout.data_type != data_types::u8))))
            return true;

        if (next.is_type<convolution>() && fmt_prev == format::bfyx &&
        ...
        if (next.is_type<convolution>() &&
            (fmt_prev == format::b_fs_yx_fsv4 || fmt_prev == format::bs_fs_yx_bsv4_fsv4 ||
             fmt_prev == format::bs_fs_yx_bsv8_fsv4) &&
            ((fmt_next == format::b_fs_yx_fsv32 &&
              (prev_output_layout.feature() == 3 || prev_output_layout.feature() == 4)) ||
             (fmt_next == format::bs_fs_yx_bsv32_fsv32 &&
              (prev_output_layout.feature() == 3 || prev_output_layout.feature() == 4)) ||
             (fmt_next == format::bs_fs_yx_bsv4_fsv4 &&
              (prev_output_layout.feature() == 3 || prev_output_layout.feature() == 4)) ||
             (fmt_next == format::bs_fs_yx_bsv8_fsv4 &&
              (prev_output_layout.feature() == 3 || prev_output_layout.feature() == 4)) ||
             (fmt_next == format::b_fs_yx_fsv16 && next_output_layout.feature() >= 16 &&
              (prev_output_layout.feature() == 3 ||
               (prev_output_layout.feature() == 4 && (prev_dt == data_types::u8 || prev_dt == data_types::i8))))))
            return true;
    }
    ...
    if (next.is_type<convolution>() && fmt_prev == format::bfzyx &&
    ...
    if (use_onednn_impls) {
        ...
        if (next.is_type<convolution>() && (fmt_prev == format::bfyx && fmt_next == format::bs_fs_yx_bsv32_fsv32) &&
...
bool layout_optimizer::can_fuse_reorder_to_prev(program_node& prev,
                                                reorder_node& node,
                                                format fmt_prev,
                                                format fmt_next) {
    ...
    if ((prev.is_type<mvn>() || prev.is_type<concatenation>()) && !prev.is_in_shape_of_subgraph() &&
    ...
    // Ref kernels are the main for depth_to_space, region_yolo and detection_output. It can do anything. Should not see
    // next.
    ...
    if (prev.is_type<one_hot>() && !data_type_traits::is_floating_point(dt_prev) &&
        data_type_traits::is_floating_point(dt_next) && fmt_prev == fmt_next)
        return true;

    if (prev.is_type<quantize>() && (fmt_next == format::b_fs_yx_fsv4 || fmt_next == format::b_fs_zyx_fsv32 ||
                                     fmt_next == format::b_fs_yx_fsv32 || fmt_next == format::b_fs_yx_fsv16 ||
                                     fmt_next == format::b_fs_zyx_fsv16 || fmt_next == format::bs_fs_yx_bsv16_fsv16))
        return true;
    ...
    if (prev.is_type<permute>()) {
        ...
        if ((fmt_prev == format::b_fs_yx_fsv4 || fmt_prev == format::b_fs_yx_fsv32 ||
             fmt_prev == format::b_fs_zyx_fsv32 || fmt_prev == format::b_fs_yx_fsv16 ||
...
bool should_use_winograd_2x3_s1(const convolution_node& node,
    ...
    bool disable_winograd_conv =
        node.get_program().get_config().get_property(ov::intel_gpu::disable_winograd_convolution);
    ...
    if (input_layout.data_type != data_types::f16 ||
        ...
        || output_size_handling_enabled      // This condition is weird. Need to revise it and replace with something
                                             // meaningful
...
bool layout_optimizer::convolution_bfyx_opt(layout const& output_layout,
    ...
    if (output_layout.batch() == 16 || output_layout.batch() % 16 != 0 || output_layout.data_type != data_types::f16 ||
        weights_layout.batch() % 16 != 0 ||
...
bool layout_optimizer::convolution_byxf_opt(const layout& input_layout,
    ...
    if ((output_layout.data_type == data_types::f16 && weights_layout.spatial(0) == 1 && all_ones(conv->dilation) &&
         !node.get_transposed() && node.get_groups() == 1 && input_layout.feature() % 32 == 0 &&
         weights_layout.spatial(1) == 1 && output_layout.feature() % 64 == 0 && weights_layout.batch() % 64 == 0 &&
         all_ones(conv->stride) && all_zeroes(conv->padding_begin) && all_zeroes(conv->padding_end)) ||
        // Winograd
...
bool layout_optimizer::convolution_b_fs_yx_fsv16_opt(const layout& input_layout,
    ...
    if (i8_dt_case) {
        ...
        if (input_layout.format.dimension() == 4 &&
            ((ks_x == 7 && ks_y == 7) || (ks_x == 3 && ks_y == 3) || (ks_x == 1 && ks_y == 1) ||
             (ks_x == 5 && ks_y == 5)) &&
            output_layout.feature() >= 16 &&
            ((conv->groups == 1) || conv->groups == static_cast<uint32_t>(input_layout.feature())))
            return true;
        // Check for grouped convolution
        else if (input_layout.format.dimension() == 4 && input_layout.batch() < 16 && out_features_per_group >= 16 &&
    ...
    bool correct_batch =
        (input_layout.batch() == 1) || (input_layout.batch() > 1 && input_layout.data_type == data_types::f32);
    ...
    bool correct_in_feature =
        (input_layout.feature() >= required_feature_num && output_layout.feature() >= required_feature_num);
    ...
    bool grouped =
        ((feature_block_size % out_features_per_group == 0) && (feature_block_size % in_features_per_group == 0) &&
         (feature_block_size / out_features_per_group > 1) && (feature_block_size / in_features_per_group > 1) &&
         (out_features_per_group != 1) && (in_features_per_group != 1)) ||
    ...
    if (correct_data_type && correct_batch && correct_spatial_dims && correct_in_feature &&
        (conv->groups == 1 || depthwise || grouped))
        return true;
    return false;
}

static bool has_reorder_before_mvn(const program_node& node,
                                   size_t cur_depth,
                                   size_t max_depth,
                                   uint64_t reorder_size_threshold = 0) {
    // MVN with rank size 3 always requires Reorder and Reshape. Due to this pattern, too many Reorder may occur when
    // used with Convolution, which may cause performance degradation. It stands out in Stable-Diffusion Unet and
    // Decoder.
            ...
            if (reorder_first_user->is_type<reshape>()) {
                for (auto& reshape_user : reorder_first_user->get_users()) {
                    if (reshape_user->is_type<mvn>() &&
                        ...
                        GPU_DEBUG_LOG << node.id() << ": " << node.get_output_layout().to_short_string()
                                      << " : heavy reorder" << std::endl;
                        return true;
                    }
                }
            }
...
bool layout_optimizer::should_select_b_fs_yx_fsv16_layout(convolution_node const& node, layout const& weights_layout) {
    ...
    auto current_conv_supports_layout =
    ...
    auto current_conv_partially_supports_layout =
        convolution_b_fs_yx_fsv16_opt(input_layout, output_layout, weights_layout, prim, true);
...
bool layout_optimizer::convolution_b_fs_zyx_fsv16_opt(const layout& input_layout,
    ...
    bool groups_ver =
        conv->groups == 1 || out_features_per_group % 16 == 0 || (conv->groups > 1 && out_features_per_group == 8);
...
bool layout_optimizer::convolution_bs_fs_yx_bsv16_fsv16_opt(const layout& input_layout,
    ...
    bool correct_feature =
        (input_layout.feature() % 16 == 0 || input_layout.feature() == 3) && output_layout.feature() % 16 == 0;
    ...
    auto ks_x = weights_layout.spatial(0);
    auto ks_y = weights_layout.spatial(1);
    int8_sup &= (input_layout.spatial(2) == 1 &&
                 ((ks_x == 1 && ks_y == 1) || (ks_x == 3 && ks_y == 3) || (ks_x == 7 && ks_y == 7)) &&
...
bool layout_optimizer::convolution_fs_b_yx_fsv32_opt(const layout& input_layout,
    ...
    if (!correct_in_feature && input_layout.feature() == 3 &&
        conv->groups == 1) {  // bfyx with 3 feature -> fs_b_yx_fsv32 case
        correct_in_feature = true;
    }
    ...
    if ((input_layout.format == format::fs_b_yx_fsv32) ||
        (correct_out_feature && correct_in_feature && correct_batch && (dw_conv || conv->groups == 1))) {
        return true;
    }
    return false;
}
...
static bool is_node_for_onednn(reduce_node const& node, format preferred_format) {
    ...
    // oneDNN reduction selects ref kernel for simple formats(bfyx..) which has perf regression with a decent tensor
    // size.
    if (format::is_simple_data_format(preferred_format))
        return false;

    // Onednn reduction does NOT support reordering of unreduced-axes.
    // Currently, an Onednn reduce layer which contains reduction of blocked axes(b-f) is expected to select planar
    // format.
...
static bool is_node_for_onednn(deconvolution_node const& node) {
    ...
    bool onednn_valid_params =
        onednn_valid_dt && input_layout.feature() >= 16 && prim->groups == 1 && get_post_ops_count(node) <= 32;
...
bool layout_optimizer::users_for_convolution_byxf_opt(program_node const& node, uint32_t depth) {
    ...
    for (auto& user : node.get_users()) {
        ...
        if ((user->type() == cldnn::eltwise::type_id() && !is_scale_shift(user->as<eltwise>())) ||
            user->type() == cldnn::pooling::type_id()) {
...
bool layout_optimizer::is_mixed_layout(program_node& prev,
                                       program_node& next,
    ...
    for (auto& pair : check_list) {
        if ((prev_fmt == pair.first && next_fmt == pair.second) &&
            ...
            if ((next_fmt == format::bs_fs_yx_bsv32_fsv16 || next_fmt == format::bs_fs_zyx_bsv32_fsv16) &&
                (next_dt == data_types::f32))
                return false;
            if ((next_fmt == format::bs_fs_yx_bsv16_fsv16 || next_fmt == format::bs_fs_zyx_bsv16_fsv16) &&
                (next_dt == data_types::f16))
                return false;
            return true;
        }
        if ((next_fmt == pair.first && prev_fmt == pair.second) &&
            (!check_data_type ||
             (data_type_traits::is_i8_u8(next_dt) && data_type_traits::is_floating_point(prev_dt)))) {
            if ((prev_fmt == format::bs_fs_yx_bsv32_fsv16 || prev_fmt == format::bs_fs_zyx_bsv32_fsv16) &&
                (prev_dt == data_types::f32))
                return false;
            if ((prev_fmt == format::bs_fs_yx_bsv16_fsv16 || prev_fmt == format::bs_fs_zyx_bsv16_fsv16) &&
                (prev_dt == data_types::f16))
                return false;
            return true;
        }
    }
...
format layout_optimizer::get_expected_format(convolution_node const& node) {
    ...
    if (node.is_dynamic() && output_layout.get_partial_shape().size() == 4 && node.use_explicit_padding() &&
        !i8_u8_input && !(use_onednn_impls && onednn_valid_post_ops && !node.has_padded_dependency())) {
        return format::bfyx;
    }
    ...
    if (use_onednn_impls && onednn_valid_post_ops && node.get_preferred_output_fmt() != format::any) {
        ...
        if (use_onednn_impls && i8_u8_input) {
            ...
            expected_format = cldnn::format::bs_fs_yx_bsv16_fsv16;
                   ...
                   !(has_reorder_before_mvn(reinterpret_cast<program_node const&>(*node.get_users().front()),
                                            0,
                                            3,
                                            1000000) &&
                     !static_cast<bool>(
                         prepare_padding::get_needed_padding_for_convolution(const_cast<convolution_node&>(node))))) {
            // Chose fs_b_yx_fsv32 layout in two cases: 1-st: the current conv primitive totally supports fs_b_yx_fsv32
            // layout
            //                                          2-nd: the previous conv primitive supports fs_b_yx_fsv32 layout
            //                                          and
            //                                                current conv primitives supports this one with weak
            //                                                restrictions - that should be cheaper than reordering data
            //                                                to another layout
        ...
        } else if (layout_optimizer::convolution_bfyx_opt(output_layout, weights_layout, prim) ||
                   _output_size_handling_enabled || node.get_transposed()) {
            {
                if (output_layout.format == format::b_fs_zyx_fsv16 ||
                    output_layout.format == format::bs_fs_zyx_bsv16_fsv16)
                    expected_format = cldnn::format::bfzyx;
                else
...
format layout_optimizer::get_expected_format(deconvolution_node const& node) {
    ...
    auto output_layout = node.get_output_layout(0);
    auto weights_layout =
        node.weights().get_output_layout().convert_to_weights_layout(node.get_primitive()->grouped_weights_shape);
    auto expected_format = output_layout.format;
    ...
    if (use_onednn_impls && is_node_for_onednn(node)) {
        ...
        float f_cost = static_cast<float>(input_features * output_features) /
                       (align_to(input_features, 16) * align_to(output_features, 16));
...
format layout_optimizer::get_expected_format(quantize_node const& node) {
    ...
    if (use_onednn_impls) {
        ...
        // TODO: check other types for first conv
               ...
               (layout.data_type == data_types::i8 || layout.data_type == data_types::u8) && layout.batch() % 16 == 0) {
        if (layout.feature() > 8) {
            expected = format::b_fs_yx_fsv16;
        } else {
            expected = format::b_fs_yx_fsv4;
        }
    } else if (layout.format.spatial_num() == 3 &&
...
bool layout_optimizer::is_primitive_implemented_for_onednn(program_node& node) {
    if (node.is_type<fully_connected>() || node.is_type<gemm>() || node.is_type<pooling>() ||
        ...
        node.is_type<reorder>() || node.is_type<concatenation>()) {
        return true;
    }
    ...
    if (!_forcing_map.empty() && _forcing_map.count(node.id()) != 0) {
        ...
        if (confidence_layout.is_dynamic()) {
            ...
            auto batch_size_limitations = (device_info.supports_immad && device_info.execution_units_count >= 256)
                                              ? true
                                              : confidence_layout.batch() >= 4;
            auto can_use_ocl_impl = confidence_layout.batch() <= lws_max && batch_size_limitations &&
                                    prim->confidence_threshold >= 0.1 && prim->top_k <= 400 &&
                                    prim->num_classes >= 16 && confidence_layout.feature() > 10000;
        ...
        if (std::find(onednn_optimized_fmt.begin(), onednn_optimized_fmt.end(), input_fmt) ==
                onednn_optimized_fmt.end() ||
            std::find(onednn_optimized_fmt.begin(), onednn_optimized_fmt.end(), output_fmt) ==
                onednn_optimized_fmt.end()) {
            preferred_impl = impl_types::ocl;
        }
        ...
        // Unexpected layout
        if (std::find(onednn_optimized_formats.begin(), onednn_optimized_formats.end(), preferred_format) ==
            onednn_optimized_formats.end()) {
...
format layout_optimizer::get_preferred_format(program_node& node) {
    ...
    if (!_forcing_map.empty() && _forcing_map.count(node.id()) != 0) {
        ...
        // Skip all user format check when node is dynamic. It could cause endless recursive call in
        // get_preferred_foramt()
        ...
        if (node.is_type<fully_connected>()) {
            if (allow_new_shape_infer) {
                ...
                // The condition will be relaxed once more shape agnostic kernels for other formats are enabled (e.g.,
                // fsv->bfyx FC optimized kernel(i8)))
        ...
        for (size_t idx = 0; idx < node.get_dependencies().size(); idx++) {
            ...
            if (node.is_type<convolution>() && node.get_input_layouts()[0].feature() == 3) {
                ...
                if (node.get_output_layout().get_rank() == 4 && node.get_dependency(0).is_type<permute>()) {
                    ...
                    can_optimize_permute = pnode.get_users().size() == 1 &&
                                           ...
                                           !pnode.has_fused_primitives() && !pnode.is_output() &&
                                           pnode.get_input_layout(0).is_static() &&
            ...
            if (node.is_type<convolution>() && src_fmt == format::bfyx) {
                ...
                if (conv.get_input_layouts()[0].feature() <= 8 && conv.activations_zero_points_term() &&
                    conv.get_input_layouts()[0].data_type == data_types::u8 &&
                    conv.get_output_layout().data_type == data_types::u8) {
            ...
            // In this case, it can be handled by changing only the shape of permute without the kernel execution.
            if (node.get_output_layout().get_rank() == 4 && node.get_users().size() == 1 &&
                node.get_users().front()->is_type<permute>()) {
                ...
                auto can_optimize_permute = pnode.get_output_layout().data_type == node.get_output_layout().data_type &&
                                            !pnode.has_fused_primitives() && !pnode.is_output() &&
                                            pnode.get_input_layout(0).is_static() && pnode.is_rotating_except_batch();
                if (can_optimize_permute) {
                    dst_fmt = format::byxf;
            ...
            GPU_DEBUG_LOG << "select_preferred_formats:" << node.id() << ": " << fmt_to_str(src_fmt) << " --> "
                          << fmt_to_str(dst_fmt) << " For index : " << idx << std::endl;
        }
    } else if (node.is_type<fully_connected>() || node.is_type<gemm>()) {
        for (size_t idx = 0; idx < node.get_dependencies().size(); idx++) {
            if (node.get_dependency(idx).is_constant())
            ...
            GPU_DEBUG_LOG << "select_preferred_formats:" << node.id() << ": " << fmt_to_str(target_format) << " --> "
                          << fmt_to_str(target_format) << " For index : " << idx << std::endl;
        }
...
bool layout_optimizer::all_users_simple_format_until_output(program_node& origin_node,
...
bool layout_optimizer::is_format_optimized(const convolution_node& node,
                                           const format& format,
                                           bool use_weak_restrictions) {
    auto input_layout = node.get_input_layout();
    ...
    case format::b_fs_yx_fsv16:
        return convolution_b_fs_yx_fsv16_opt(input_layout,
                                             output_layout,
                                             weights_layout,
                                             prim,
                                             use_weak_restrictions) &&
