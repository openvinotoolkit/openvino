...
// Layout support rules for the OCL MVN impl. Kept here (impl level) rather than on the primitive,
// since which layouts are usable depends on the available kernels, not on the operation parameters.
bool mvn_supports_input_layout(const program_node& node) {
...
const std::vector<std::shared_ptr<cldnn::ImplementationManager>>& Registry<mvn>::get_implementations() {
    static const std::vector<std::shared_ptr<ImplementationManager>> impls = {
        OV_GPU_GET_INSTANCE_OCL(mvn, shape_types::static_shape, mvn_supports_input_layout)
        OV_GPU_GET_INSTANCE_OCL(mvn, shape_types::dynamic_shape, mvn_supports_input_layout)
    };
