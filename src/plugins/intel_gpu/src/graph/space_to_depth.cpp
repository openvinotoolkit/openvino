...
std::vector<layout> space_to_depth_inst::calc_output_layouts(space_to_depth_node const& node,
...
template std::vector<layout> space_to_depth_inst::calc_output_layouts<ov::PartialShape>(
    space_to_depth_node const& node,
...
layout space_to_depth_inst::calc_output_layout(space_to_depth_node const& node, kernel_impl_params const& impl_param) {
    ...
    if (depth_mode != SpaceToDepth::SpaceToDepthMode::DEPTH_FIRST &&
        depth_mode != SpaceToDepth::SpaceToDepthMode::BLOCKS_FIRST)
    ...
    if (input_layout.spatial(0) % block_size != 0 || input_layout.spatial(1) % block_size != 0)
        CLDNN_ERROR_MESSAGE(desc->id,
                            "Sizes of spatials x, y must be divisible by block size. Actual spatial sizes are " +
                                std::to_string(input_layout.spatial(0)) + ", " +
                                std::to_string(input_layout.spatial(1)) + " (x, y). Actual block size is " +
                                std::to_string(block_size));
    ...
    if (input_layout.format.dimension() == 5) {
        if (input_layout.spatial(2) % block_size != 0)
            CLDNN_ERROR_MESSAGE(desc->id,
                                "Sizes of spatials z must be divisible by block size. Actual spatial sizes are " +
                                    std::to_string(input_layout.spatial(2)) + " (z). Block size is " +
                                    std::to_string(block_size));
        ...
        return layout{output_type,
                      input_format,
                      tensor(TensorValue(input_layout.batch()),
                             TensorValue(feature),
                             TensorValue(x),
                             TensorValue(y),
                             TensorValue(z))};
...
std::string space_to_depth_inst::to_string(space_to_depth_node const& node) {
    ...
    std::string depth_mode =
        (desc->mode == SpaceToDepth::SpaceToDepthMode::BLOCKS_FIRST) ? "blocks_first" : "depth_first";
...
space_to_depth_inst::typed_primitive_inst(network& network, space_to_depth_node const& node) : parent(network, node) {}
