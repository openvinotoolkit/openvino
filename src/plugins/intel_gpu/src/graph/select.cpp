...
select_inst::typed_primitive_inst(network& network, select_node const& node) : parent(network, node) {
    ...
    if (deps[1].first->get_output_layout().size != cldnn::tensor(1))
    ...
    if (deps[2].first->get_output_layout().size != cldnn::tensor(1))
        CLDNN_ERROR_NOT_EQUAL(node.id(),
                              "Mask format",
                              deps[0].first->get_output_layout().format,
                              "Positive input format",
    ...
    if (node.get_primitive()->broadcast_type == "none") {
        ...
        CLDNN_ERROR_NOT_EQUAL(node.id(),
                                "Mask size",
                                deps[0].first->get_output_layout().size,
                                "Positive input format",
                                deps[1].first->get_output_layout().size,
                                "");
    } else if (node.get_primitive()->broadcast_type == "numpy") {
        if (deps[1].first->get_output_layout().size != cldnn::tensor(1) && deps[2].first->get_output_layout().size != cldnn::tensor(1))
            CLDNN_ERROR_NOT_EQUAL(node.id(),
                                  "Positive input format",
                                  deps[1].first->get_output_layout().format,
        ...
        auto dep1_size = deps[1].first->get_output_layout().size;
        auto dep2_size = deps[2].first->get_output_layout().size;
        ...
        for (size_t i = 0; i < deps.size(); i++) {
            for (size_t d = 0; d < max_dim_count; d++) {
                auto current_dim = deps[i].first->get_output_layout().size.raw[d];
