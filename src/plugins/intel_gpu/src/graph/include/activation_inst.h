...
public:
    ...
    program_node& slope_input() const {
    ...
    bool is_parameterized() const {
        return !typed_desc()->additional_params_input.empty();
    }
...
public:
    ...
    static std::vector<layout> calc_output_layouts(activation_node const& /*node*/,
    ...
    memory::ptr slope_memory() const {
        return dep_memory_ptr(1);
    }

    bool is_parameterized() const {
        return !get_typed_desc<activation>()->additional_params_input.empty();
    }
};
