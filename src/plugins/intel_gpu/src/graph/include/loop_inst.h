...
public:
    ...
    program::ptr get_body_program() const {
        return get_primitive()->body_program;
    }

    const primitive_id& get_trip_count_id() const {
        return get_primitive()->trip_count_id;
    }
    const primitive_id& get_initial_execution_id() const {
        return get_primitive()->first_execution_condition_id;
    }
    const primitive_id& get_current_iteration_id() const {
        return get_primitive()->body_current_iteration_id;
    }
    const primitive_id& get_execution_condition_id() const {
        return get_primitive()->body_execution_condition_id;
    }
    const primitive_id& get_num_iterations_id() const {
        return get_primitive()->num_iteration_id;
    }
    const int32_t get_max_num_iteration() const {
        return get_primitive()->max_num_iterations;
    }

    const std::vector<loop::io_primitive_map>& get_input_primitive_maps() const {
        return input_primitive_maps;
    }
    const std::vector<loop::io_primitive_map>& get_output_primitive_maps() const {
        return output_primitive_maps;
    }
    const std::vector<loop::backedge_mapping>& get_back_edges() const {
        return back_edges;
    }
...
public:
    struct concatenated_memory_mapping {
        ...
        memory::ptr get_sliced_mem(int64_t iteration) const {
            OPENVINO_ASSERT(static_cast<size_t>(iteration) < sliced_mems.size(),
                            "invalid itertion(",
                            iteration,
                            ") for sliced_mes(",
                            sliced_mems.size(),
        ...
        std::vector<memory::ptr>& get_sliced_mems() const {
            return sliced_mems;
        }

        void reset_data_for_shape_changed() {
            if (concatenated_mem)
                concatenated_mem = nullptr;
            sliced_mems.clear();
        }
        ...
        std::string to_string() const {
            ...
            ss << "* concat_data_prim               : "
               << ((concat_data_prim != nullptr) ? concat_data_prim->id() : "nullptr") << std::endl;
            ss << "* sliced_data_prim               : "
               << ((sliced_data_prim != nullptr) ? sliced_data_prim->id() : "nullptr") << std::endl;
            ss << "* concatenated_mem               : "
               << ((concatenated_mem != nullptr) ? concatenated_mem->get_layout().to_short_string() : "nullptr")
               << std::endl;
            ss << "* sliced_mems                    :{ ";
    ...
    struct backedge_memory_mapping {
        ...
        backedge_memory_mapping(std::shared_ptr<primitive_inst> _from_primitive,
                                std::shared_ptr<primitive_inst> _to_primitive,
                                std::shared_ptr<concatenated_memory_mapping> _concat_mem_mapping,
                                memory::ptr _initial_mem,
                                cldnn::stream& _stream,
                                backedge_type _type = CONCAT_OUTPUT)
    ...
    private:
        void validate_backedge_memory() {
            ...
            if (!is_dynamic && from_mem) {
                ...
                OPENVINO_ASSERT((from_mem_bytes == total_bytes),
                                "Invalid backedge memory layout: size(",
                                from_mem_bytes,
                                ",",
                                from_mem->get_layout().to_short_string(),
                                ") not matched with that of initial_mem(",
                                total_bytes,
                                ",",
...
public:
    ...
    network::ptr get_body_network() const {
        return body_network;
    }
