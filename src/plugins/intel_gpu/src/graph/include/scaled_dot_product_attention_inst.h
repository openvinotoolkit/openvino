...
namespace cldnn {
template <>
struct typed_program_node<scaled_dot_product_attention> :
    public typed_program_node_base<scaled_dot_product_attention> {
...
public:
    ...
    std::shared_ptr<NodeFuseParams> get_fuse_params() const override {
        return std::make_shared<NodeFuseParams>(scaled_dot_product_attention::type_id());
    }
...
template <>
class typed_primitive_inst<scaled_dot_product_attention> :
    public typed_primitive_inst_base<scaled_dot_product_attention> {
    using parent = typed_primitive_inst_base<scaled_dot_product_attention>;
    using parent::parent;

public:
    template<typename ShapeType>
    static std::vector<layout> calc_output_layouts(scaled_dot_product_attention_node const& node, kernel_impl_params const& impl_param);
