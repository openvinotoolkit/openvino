...
public:
    ...
    static std::vector<layout> calc_output_layouts(const grouped_matmul_node& node, const kernel_impl_params& impl_param);
    static layout calc_output_layout(const grouped_matmul_node& node, const kernel_impl_params& impl_param);
    static std::string to_string(const grouped_matmul_node& node);

    typed_primitive_inst(network& network, const grouped_matmul_node& node);
};
