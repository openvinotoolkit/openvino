...
public:
    ...
    program_node& output_roi_scores_node() const {
        return get_dependency(4);
    }
};

using experimental_detectron_generate_proposals_single_image_node =
    typed_program_node<experimental_detectron_generate_proposals_single_image>;
...
public:
    ...
    memory::ptr output_roi_scores_memory() const {
        return dep_memory_ptr(4);
    }
};

using experimental_detectron_generate_proposals_single_image_inst =
    typed_primitive_inst<experimental_detectron_generate_proposals_single_image>;
