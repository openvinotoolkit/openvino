...
namespace cldnn {
template <>
struct typed_program_node<generate_proposals> : public typed_program_node_base<generate_proposals> {
...
public:
    ...
    program_node& output_rois_scores_node() const {
        return get_dependency(4);
    }
    program_node& output_rois_nums_node() const {
...
template <>
class typed_primitive_inst<generate_proposals> : public typed_primitive_inst_base<generate_proposals> {
...
public:
    ...
    typed_primitive_inst(network& network, const generate_proposals_node& node) : parent(network, node) {}

    memory::ptr output_rois_scores_memory() const {
        return dep_memory_ptr(4);
    }
    memory::ptr output_rois_nums_memory() const {
