...
JitConstants make_activation_jit_constants(activation_func activation_function,
    ...
    case activation_func::elu: {
        ...
        jit.add(make_jit_constant(macro_def, (max_func(input, zero) + (alpha * (exp(min_func(input, zero)) - one))).str()));
        break;
    }
    case activation_func::clamp: {
        ...
        jit.add(make_jit_constant(macro_def, max_func(m, min_func(n, input)).str()));
        break;
    }
    ...
    case activation_func::hard_sigmoid: {
        ...
        jit.add(make_jit_constant(macro_def, max_func(zero, min_func(one, alpha * input + beta)).str()));
        break;
    }
    case activation_func::hsigmoid: {
        ...
        jit.add(make_jit_constant(macro_def, (min_func(max_func(zero, input + three), six) / six).str()));
        break;
    }
    case activation_func::sign:
        jit.add(make_jit_constant(macro_def, ternary(input.gt(zero), one, ternary(input.eq(zero), zero, neg(one))).str()));
        break;
    ...
    case activation_func::selu: {
        ...
        jit.add(make_jit_constant(macro_def, ternary(input.le(zero), gamma * (alpha * exp(input) - alpha), gamma * input).str()));
        break;
    }
    case activation_func::softplus: {
        jit.add(make_jit_constant(macro_def, log(exp(input) + one).str()));
        break;
    }
    case activation_func::softsign: {
        jit.add(make_jit_constant(macro_def, (input / (one + abs_func(input))).str()));
        break;
    }
    case activation_func::swish: {
        auto beta = disable_type_conversion ? "m"_jit : to_type("m"_jit);
        jit.add(make_jit_constant(macro_def, (input / (one + exp(neg(beta * input)))).str()));
        break;
    }
    case activation_func::hswish: {
        ...
        const JitTerm three("3." + type_suffix);
        const JitTerm six("6." + type_suffix);
        jit.add(make_jit_constant(macro_def, (input * min_func(max_func(zero, input + three), six) / six).str()));
        break;
    }
    case activation_func::mish: {
        ...
        jit.add(make_jit_constant(macro_def, ternary(input.ge(bound), input, common_mish_formula).str()));
        break;
    }
    case activation_func::gelu: {
        ...
        jit.add(make_jit_constant(macro_def, (half * input * (one + erf((input * mult)))).str()));
        break;
    }
    case activation_func::gelu_tanh: {
        ...
        jit.add(make_jit_constant(macro_def, (half * input * (one + tanh(sqrt_2_over_pi * input * (one + mult * input * input)))).str()));
        break;
    }
    case activation_func::negation:
        jit.add(make_jit_constant(macro_def, ternary(input.eq(zero), one, zero).str()));  // the workaround for OpenCL's vector type result (!input)
        break;
