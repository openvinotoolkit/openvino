...
class XetlaLSTMLoopGenerator : public SingleKernelGenerator {
public:
    XetlaLSTMLoopGenerator() : SingleKernelGenerator("xetla_lstm_loop") {}

protected:
    std::string get_build_options(const kernel_impl_params& params) const override {
        return SingleKernelGenerator::get_build_options(params) + get_lstm_build_options();
    }
...
class XetlaLSTMGemmGenerator : public SingleKernelGenerator {
public:
    XetlaLSTMGemmGenerator() : SingleKernelGenerator("xetla_lstm_gemm") {}
...
public:
    ...
    std::vector<layout> get_internal_buffer_layouts(const kernel_impl_params& params) const override {
        auto desc = params.typed_desc<lstm_seq>();
        ...
        auto buf = layout{ov::PartialShape{static_cast<int64_t>(buf_size)}, ov::element::f32, format::bfyx};
        return { buf };
    }
};
