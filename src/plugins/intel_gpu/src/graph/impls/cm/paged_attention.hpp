...
struct PagedAttentionImplementationManager : public ImplementationManager {
    ...
    [[nodiscard]] bool validate_impl(const program_node& node) const override {
        ...
        if (!desc->has_xattention) {
            //GPU_DEBUG_TRACE_DETAIL(config) << "validate_impl() - false because we enable CM PA when XAttention is enabled. " << std::endl;
            return false;
        }
        ...
        if (desc->heads_num / desc->kv_heads_num > 8) {
            //GPU_DEBUG_TRACE_DETAIL(config) << "validate_impl() - false because heads_num / kv_heads_num > 8. " << std::endl;
            return false;
        }
        ...
        if (!check_cm_jit_support(engine, config) || !info.supports_immad || !config.get_use_cm()) {
            //GPU_DEBUG_TRACE_DETAIL(config) << "validate_impl() - false due to unsupported GPU architecture. " << std::endl;
            return false;
        }
        ...
        if (!everyone_is(format::bfyx, q_layout.format, k_layout.format, v_layout.format, out_layout.format)) {
            //GPU_DEBUG_TRACE_DETAIL(config) << "validate_impl() - false due to unsupported qkv layout. " << std::endl;
            return false;
        }

        if (!one_of(k_layout.data_type, supported_q_types) || !one_of(v_layout.data_type, supported_q_types)) {
            //GPU_DEBUG_TRACE_DETAIL(config) << "validate_impl() - false due to unsupported kv data type. " << std::endl;
            return false;
        }

        if (!one_of(q_layout.data_type, supported_q_types) || !one_of(out_layout.data_type, supported_q_types)) {
            //GPU_DEBUG_TRACE_DETAIL(config) << "validate_impl() - false due to unsupported q/out data type. " << std::endl;
            return false;
        }
        ...
        if (!one_of(kcache_layout.data_type, supported_kv_types) || !one_of(vcache_layout.data_type, supported_kv_types)) {
            //GPU_DEBUG_TRACE_DETAIL(config) << "validate_impl() - false due to unsupported kv cache data type. " << std::endl;
            return false;
        }

        //GPU_DEBUG_TRACE_DETAIL(config) << "validate_impl() - true" << std::endl;
        return true;
    }
};
}  // namespace ov::intel_gpu::cm
