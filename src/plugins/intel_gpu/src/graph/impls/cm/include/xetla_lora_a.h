...
template <typename tile_op_t_, typename tile_shape_, typename mem_desc_c_t_,
        gpu_arch arch_tag_, msg_type msg_type_c_, bool store_result = true>
class epilogue_lora_gemm_a_t {
public:
...
template <int num_local_kslicing_, typename gemm_t_, typename epilogue_t_,
...
class gemm_lora_a_t {
    ...
    static constexpr uint32_t num_global_kslicing = 1;
...
public:
    ...
    inline void operator()(sycl::nd_item<3> &item,
            const arguments_t &args, matAcc_t &matAcc, uint32_t slm_base = 0,
            uint32_t nbarrier_base = 0) const {
        // set up workgroup level coordinates and boundaries
#if LORA_TEMP_IN_REG == 1
        work_group_t g((item.get_local_linear_id() / item.get_local_range(2))
                * wg_size_x);
#else
        ...
        int start_n
                = 0; //group_swizzle.template get_tile_idx<2>(item) * wg_tile_n;
        int start_k = 0;
...
template <typename dtype_a, typename dtype_b, typename dtype_c,
        ...
        mem_space mem_space_c, uint32_t local_kslicing, mma_engine engine,
        uint32_t periodic_sync_interval, uint32_t prefetch_distance,
        gpu_arch arch_tag, bool unaligned = false, bool out_in_reg = false>
struct gemm_lora_a {
    using tile_shape = group::tile_shape_t<wg_n, wg_m, sg_n, sg_m>;

    static_assert(!((local_kslicing > 1) && (out_in_reg)),
            "when using local kslicing, store_result should be true!");
    ...
    using epilogue = epilogue_lora_gemm_a_t<tile_op_t, tile_shape, mem_desc_c,
            arch_tag, unaligned ? msg_type::unaligned_2d : msg_type::block_2d,
            !out_in_reg>;
    ...
    using gemm_op_t
            = gemm_lora_a_t<local_kslicing, gemm, epilogue, group_swizzle_t>;

    using matAcc_t = typename gemm::matAcc_t;
    ...
    inline static void run(sycl::nd_item<3> &item, uint32_t mat_m,
            uint32_t mat_k, uint32_t mat_n, dtype_a *a, dtype_b *b,
            typename epilogue::mem_desc_c_t::base_t c, dtype_b *scale_input,
            matAcc_t &matAcc) {
        gemm_op_t gemm_op;
