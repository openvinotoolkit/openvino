...
std::string SingleKernelGenerator::build_code(std::string_view template_name, const JitConstants& jit_constants, const std::string& kernel_id) const {
    ...
    try {
        code.add_line(std::string(CMSourcesDB::get_kernel_template(template_name)));
    } catch (std::exception&) {
        OPENVINO_THROW("[GPU] Couldn't find CM kernel template: ", template_name);
    }
