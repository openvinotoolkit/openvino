...
public:
    explicit wait_for_events_impl(const program_node& /*node*/) : primitive_impl("wait_for_events") {}
    ...
    event::ptr execute(const std::vector<event::ptr>& events, primitive_inst& instance) override {
        ...
        return events.empty() ? stream.create_user_event(true) : stream.enqueue_marker(events);
    }
...
attach_input_layout_common::attach_input_layout_common() {
    implementation_map<input_layout>::add(impl_types::common,
                                          shape_types::any,
                                          wait_for_events_impl::create_input_layout,
                                          {});
}
