...
micro::Type convert_type(ov::element::Type t) {
    switch (t) {
        case ov::element::f32: return micro::Type::f32;
        case ov::element::f16: return micro::Type::f16;
        case ov::element::i8: return micro::Type::s8;
        case ov::element::u8: return micro::Type::u8;
        default: break;
    }
...
JitConstants unit_parameters(std::string prefix) {
...
public:
    SDPAMicroGenerator(bool prefill) : SDPABase("sdpa_micro", false), m_is_prefill(prefill) {
        m_stage_suffix = m_is_prefill ? "_prefill" : "_generate";
    }

    std::string get_build_options(const kernel_impl_params& params) const override {
        auto base_options = SingleKernelGenerator::get_build_options(params);
...
protected:
    ...
    JitConstants get_jit_constants(const kernel_impl_params& params, const micro::Package& gemm_kq, const micro::Package& gemm_vs) const {
        ...
        jit.merge(convert_strides("QRY", "INPUT0", desc->input_q_transpose_order));
        jit.merge(convert_strides("KEY", "INPUT1", desc->input_k_transpose_order));
        jit.merge(convert_strides("VAL", "INPUT2", desc->input_v_transpose_order));
        jit.merge(convert_strides("DST", "OUTPUT", desc->output_transpose_order));
    ...
    void init_microkernels(const kernel_impl_params& params, micro::Package& gemm_kq, micro::Package& gemm_vs, bool is_prefill) const {
        ...
        bool is_quantized = (K.data_type == ov::element::u8 || K.data_type == ov::element::i8) ||
