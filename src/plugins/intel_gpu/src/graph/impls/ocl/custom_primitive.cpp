...
struct custom_gpu_primitive_impl : typed_primitive_impl<custom_gpu_primitive> {
    ...
    custom_gpu_primitive_impl() : _kernels() {}

    custom_gpu_primitive_impl(const custom_gpu_primitive_impl& other) : cl_kernel(other.cl_kernel), _kernels({}) {
    ...
    event::ptr execute_impl(const std::vector<event::ptr>& events, custom_gpu_primitive_inst& instance) override {
        auto& stream = instance.get_network().get_stream();
        kernel_arguments_data args;
        for (auto& dep : instance.dependencies()) {
            args.inputs.push_back(dep.first->output_memory_ptr());
...
static void add_layout_to_jit(kernel_selector::jit_constants& mem_consts, const std::string& name, const layout& l) {
    ...
    OPENVINO_ASSERT(dataTypeToIndex.find(l.data_type) != dataTypeToIndex.end(),
                    "[GPU] Add layout to jit error: unhandled data type in layout");
