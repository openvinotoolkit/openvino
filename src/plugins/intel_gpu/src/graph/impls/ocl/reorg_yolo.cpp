...
struct reorg_yolo_impl : typed_primitive_impl_ocl<reorg_yolo> {
    ...
    static std::unique_ptr<primitive_impl> create(const reorg_yolo_node& arg) {
        ...
        ry_params.stride = arg.get_primitive()->stride;

        const auto& kernel_selector = kernel_selector::reorg_yolo_kernel_selector::Instance();
        ...
        return make_unique<reorg_yolo_impl>(arg, best_kernels.front());
    }
};
