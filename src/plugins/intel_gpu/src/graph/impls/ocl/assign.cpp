...
struct assign_impl : public typed_primitive_impl_ocl<assign> {
    ...
    DECLARE_OBJECT_TYPE_SERIALIZATION(cldnn::ocl::assign_impl)
    ...
    event::ptr execute_impl(const std::vector<event::ptr>& events, assign_inst& instance) override {
        ...
        const auto ev_set_memory = variable.get_memory()->copy_from(stream, instance.input_memory(), 0, 0, variable.get_layout().bytes_count(), false);
        variable.set();
...
attach_assign_impl::attach_assign_impl() {
    implementation_map<assign>::add(impl_types::ocl, shape_types::dynamic_shape, assign_impl::create, {});
    implementation_map<assign>::add(impl_types::ocl, shape_types::static_shape, assign_impl::create, {});
}
...
BIND_BINARY_BUFFER_WITH_TYPE(cldnn::ocl::assign_impl)
