...
bool query_local_block_io_supported(engine& e, const ExecutionConfig& config) {
    ...
    // We assume that new uarch which don't have simd8 support are not affected by driver bug and we can safely return
    // flag value
    ...
    std::string kernel_code = "__attribute__((intel_reqd_sub_group_size(8)))"
...
data_types from_data_type(kernel_selector::data_type dt) {
    ...
    default:
        OPENVINO_THROW("[GPU] Unable to convert kernel_selector data type ",
                       kernel_selector::toString(dt),
                       " to cldnn data type");
    }
}
...
data_types from_weights_type(kernel_selector::weights_type dt) {
    ...
    default:
        OPENVINO_THROW("[GPU] Unable to convert kernel_selector weights type ",
                       kernel_selector::toString(dt),
                       " to cldnn data type");
    }
}
...
void convert_fused_ops_to_legacy_activations(const kernel_impl_params& param_info,
                                             std::vector<kernel_selector::base_activation_params>& activations) {
...
bool use_legacy_fused_ops(const kernel_impl_params& param_info) {
    ...
    if (std::find(legacy_fusion_list.begin(), legacy_fusion_list.end(), param_info.desc->type) ==
        legacy_fusion_list.end()) {
        return false;
    }
...
void set_params(const kernel_impl_params& param_info, kernel_selector::params& params) {
    ...
    params.engineInfo.maxThreadsPerExecutionUnit =
        device_info.num_threads_per_eu > 0 ? device_info.num_threads_per_eu : 7;
    params.engineInfo.maxThreadsPerDevice =
        params.engineInfo.maxThreadsPerExecutionUnit * device_info.execution_units_count;
    ...
    params.allowStaticInputReordering = config.get_property(ov::intel_gpu::optimize_data) ||
                                        config.get_property(ov::intel_gpu::allow_static_input_reorder);
...
void set_default_params(const kernel_impl_params& param_info,
                        kernel_selector::base_params& params,
    ...
    if (use_legacy_fused_ops(param_info)) {
        ...
        for (auto& fused_prim : param_info.fused_desc) {
            ...
            OPENVINO_ASSERT(desc.op_params != nullptr,
                            "[GPU] Invalid fused operation (",
                            param_info.desc->id,
                            ") of type ",
...
void set_weight_bias_zero_point_default_params(const kernel_impl_params& param_info,
    ...
    if (param_info.weights_zero_points_layout) {
        params.weights_zero_points.push_back(
            convert_data_tensor(*param_info.weights_zero_points_layout).FlattenFeatureAndSpatials());
    }

    if (param_info.activations_zero_points_layout) {
        params.activations_zero_points.push_back(
            convert_data_tensor(*param_info.activations_zero_points_layout).FlattenFeatureAndSpatials());
    }
