...
struct extract_image_patches_impl : typed_primitive_impl_ocl<extract_image_patches> {
    ...
    static std::unique_ptr<primitive_impl> create(const extract_image_patches_node& arg) {
        ...
        const auto& kernel_selector = kernel_selector::extract_image_patches_kernel_selector::Instance();
        ...
        return make_unique<extract_image_patches_impl>(arg, best_kernels.front());
    }
};
