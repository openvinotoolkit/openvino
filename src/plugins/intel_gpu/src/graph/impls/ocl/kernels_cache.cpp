    ...
    for (auto& c : program_buckets) {
        ...
        for (auto& b : batches) {
...
#ifdef ENABLE_CM_FOR_GPU
                                prog.language == kernel_language::OCLC_V2 ? ov::intel_gpu::ocl::SourcesDB::get_kernel_header(header)
            ...
            if (!dump_sources_dir.empty() && b.language != kernel_language::CM) {
                ...
                current_dump_file_name += "clDNN_program_" + std::to_string(_prog_id) + "_bucket_" + std::to_string(b.bucket_id) + "_part_" +
                                          std::to_string(b.batch_id) + "_" + std::to_string(b.hash_value) + ".cl";
