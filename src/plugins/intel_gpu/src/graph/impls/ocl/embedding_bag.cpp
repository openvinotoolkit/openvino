...
struct embedding_bag_impl : typed_primitive_impl_ocl<embedding_bag> {
    ...
    static std::unique_ptr<primitive_impl> create(const embedding_bag_node& arg) {
        ...
        const auto& kernel_selector = kernel_selector::embedding_bag_kernel_selector::Instance();
        const auto best_kernels = kernel_selector.GetBestKernels(embedding_bag_params, embedding_bag_optional_params);
        ...
        return make_unique<embedding_bag_impl>(arg, best_kernels.front());
    }
};
