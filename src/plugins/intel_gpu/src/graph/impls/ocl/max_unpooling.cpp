...
struct max_unpooling_impl : typed_primitive_impl_ocl<max_unpooling> {
    ...
    static std::unique_ptr<primitive_impl> create(const max_unpooling_node& arg) {
        ...
        const auto& kernel_selector = kernel_selector::max_unpooling_kernel_selector::Instance();
        const auto best_kernels = kernel_selector.GetBestKernels(max_unpooling_params, max_unpooling_optional_params);
        ...
        return make_unique<max_unpooling_impl>(arg, best_kernels.front());
    }
