...
public:
    static kernel_params_t get_kernel_params(const kernel_impl_params& impl_param) {
        ...
        uint32_t stride_z = stride.size() >= 3 ? (uint32_t)stride[stride.size() - 3] : 1;
        uint32_t stride_y = stride.size() >= 2 ? (uint32_t)stride[stride.size() - 2] : 1;
        uint32_t stride_x = stride.size() >= 1 ? (uint32_t)stride[stride.size() - 1] : 1;
        params.stride = {stride_x, stride_y, stride_z};

        uint32_t dilation_z = dilation.size() >= 3 ? (uint32_t)dilation[dilation.size() - 3] : 1;
        uint32_t dilation_y = dilation.size() >= 2 ? (uint32_t)dilation[dilation.size() - 2] : 1;
        uint32_t dilation_x = dilation.size() >= 1 ? (uint32_t)dilation[dilation.size() - 1] : 1;
