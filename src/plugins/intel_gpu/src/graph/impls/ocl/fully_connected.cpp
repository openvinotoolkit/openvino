...
public:
    static kernel_params_t get_kernel_params(const kernel_impl_params& impl_param) {
        ...
        if (!primitive->new_shape_infer) {
            auto get_fc_input_layouts = [primitive](const std::vector<layout>& input_layouts) {
                auto reshape_to_2d = [](const ov::PartialShape& shape, int64_t feature) {
                    ...
                    size_t total = std::accumulate(staticShape.begin(), staticShape.end(), 1,
                ...
                int64_t feature = input0_pshape[std::min(primitive->input_size, static_cast<size_t>(4)) -
                                                1ul].get_length();
        ...
        params.new_shape_infer = primitive->new_shape_infer;
