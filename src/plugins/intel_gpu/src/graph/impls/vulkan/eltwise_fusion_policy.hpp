...
public:
    ...
    bool controls(fusion_kind kind) const noexcept override {
        return kind == fusion_kind::activation || kind == fusion_kind::quantize || kind == fusion_kind::eltwise ||
               kind == fusion_kind::terminal_reorder;
    }

    bool can_consider_output(const program_node& node) const override {
        return node.is_type<eltwise>() || node.is_type<activation>() || node.is_type<quantize>() ||
    ...
    fusion_decision evaluate(const fusion_query& query) const override {
        ...
        case fusion_kind::eltwise:
            return query.peer == nullptr ? fusion_decision::reject
                                         : evaluate_eltwise(query.producer, *query.peer, query.consumer);
...
private:
    ...
    static bool has_same_dense_geometry(const layout& input_layout, const layout& output_layout) {
        return has_dense_storage(input_layout) && has_dense_storage(output_layout) &&
    ...
    static bool has_enough_work(const program_node& node, const layout& output_layout) {
        ...
        return eltwise_shader_abi::post_op_fusion_has_enough_work(output_layout.count(),
    ...
    static bool supports_terminal_post_op(const program_node& producer, const layout& post_output_layout) {
        if (!producer.is_type<eltwise>() || producer.is_output() || producer.get_users().size() != 1 ||
            producer.has_fused_primitives() || producer.get_dependencies().empty() ||
            producer.get_inputs_count() != 2) {
            return false;
        }
        ...
        if ((base_output_layout.data_type != data_types::f16 && base_output_layout.data_type != data_types::f32) ||
            !has_same_dense_geometry(base_output_layout, post_output_layout) ||
            !has_enough_work(producer, post_output_layout)) {
            return false;
        }
        return producer.get_input_layout(0).identical(base_output_layout) &&
               producer.get_input_layout(1).identical(base_output_layout);
    }
    ...
    static fusion_decision evaluate_activation(const program_node& producer, const program_node& consumer) {
        ...
        const bool supported =
            activation_node.get_dependencies().size() == 1 && !desc->additional_params_input.is_valid() &&
    ...
    static fusion_decision evaluate_quantize(const program_node& producer, const program_node& consumer) {
        ...
        const bool supported_output_type = output_type == data_types::f16 || output_type == data_types::f32 ||
                                           output_type == data_types::i8 || output_type == data_types::u8;
    ...
    static bool supports_packed_output(const layout& output_layout) {
        ...
        return scalar_size >= sizeof(uint32_t) ||
               (output_layout.count() % packed_width == 0 && output_layout.get_linear_offset() % packed_width == 0);
    }

    static fusion_decision evaluate_eltwise(const program_node& producer,
                                            const program_node& peer,
        ...
        const bool layouts_are_static =
            !producer_layout.is_dynamic() && !peer_layout.is_dynamic() && !output_layout.is_dynamic();
        const bool peer_is_broadcast = layouts_are_static && !peer_layout.identical(output_layout);
        const bool peer_layout_supported =
            layouts_are_static &&
            (!peer_is_broadcast ||
             (consumer_primitive->broadcast_spec.m_type == ov::op::AutoBroadcastType::NUMPY &&
              data_type_traits::size_of(output_layout.data_type) >= sizeof(uint32_t) &&
              producer.get_fused_primitives().empty() && is_numpy_broadcast_compatible(peer_layout, output_layout)));
        const bool supported = !producer.is_output() && producer.is_type<eltwise>() &&
                               producer.get_fused_primitives().size() <
                                   eltwise_shader_abi::value(eltwise_shader_abi::limit::max_fused_chain_length) &&
                               producer.get_inputs_count() == 2 && layouts_are_static &&
                               has_dense_storage(producer_layout) && has_dense_storage(peer_layout) &&
                               has_dense_storage(output_layout) && supports_packed_output(output_layout) &&
                               peer_layout_supported && producer.get_input_layout(0).identical(output_layout) &&
                               producer.get_input_layout(1).identical(output_layout) &&
    ...
    static fusion_decision evaluate_reorder_elimination(const program_node& producer, const program_node& consumer) {
        ...
        return has_enough_work(producer, consumer.get_output_layout()) ? fusion_decision::accept
