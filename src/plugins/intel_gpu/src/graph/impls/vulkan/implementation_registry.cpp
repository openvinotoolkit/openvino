...
bool supports_implementation_fusions(cldnn::runtime_types runtime) {
    // CLSPV reference adapters accept unfused primitives. Common graph cleanup
    // still runs; implementation-specific fusions belong to their own backend.
    return runtime != cldnn::runtime_types::vulkan;
}
