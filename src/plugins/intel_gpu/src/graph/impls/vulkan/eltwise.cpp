...
kernel_selector::KernelsData get_reference_kernels_data(const kernel_impl_params& params) {
    ...
    for (auto& candidate : candidates) {
        ...
        // Shared dispatch callbacks capture their selector. Keep the persistent
        // reference selector, not the temporary, JIT-only type adapter.
        get_reference_kernel().GetUpdateDispatchDataFunc(candidate);
    }
    return candidates;
}
...
public:
    ...
    eltwise_impl(kernel_selector::KernelData kernel_data, bool is_dynamic) : parent(std::move(kernel_data), is_dynamic) {
        OPENVINO_ASSERT(_kernel_data.kernels.size() == 1, "[GPU][Vulkan] Expected one reference Eltwise dispatch");
    }
