...
static void reorder_unreduced_axis_no_fusion(const cldnn::layout& input_layout, cldnn::layout& output_layout, std::vector<int64_t> axes) {
    // oneDNN reduction requires src/dst to share the same rank, with reduced axes set to 1 in dst.
    // oneDNN derives that rank from format::dimension(), so normalize the output to the input's
    // format rank instead of assuming output_layout already matches it. validate_impl() guarantees
    // both layouts have partial shape rank <= format dimension, so in_dims below is built at
    // num_dims and the axis remapping stays consistent with it.
    ...
    if (output_layout.format.dimension() != num_dims)
        output_layout.format = format::adjust_to_rank(output_layout.format, num_dims);
    output_layout.set_tensor(tensor(in_dims));
}
