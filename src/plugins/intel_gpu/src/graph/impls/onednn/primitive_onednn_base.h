...
protected:
    ...
    virtual std::unordered_map<int, dnnl::memory> get_arguments(typed_primitive_inst<PType>& instance) const {
...

        {
            ...
            auto offset = onednn::get_f_offset(instance.get_node_input_layout(), _pd.dnnl::primitive_desc_base::src_desc(0));
            args.insert({DNNL_ARG_SRC, input.get_onednn_memory(_pd.dnnl::primitive_desc_base::src_desc(0), offset)});
        }

        {
            auto& output = instance.output_memory();
            auto offset = onednn::get_f_offset(instance.get_node_output_layout(), _pd.dnnl::primitive_desc_base::dst_desc(0));
