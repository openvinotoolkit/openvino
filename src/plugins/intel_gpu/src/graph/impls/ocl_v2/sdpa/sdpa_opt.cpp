...
bool SDPAOpt::supports_micro_sdpa(const RuntimeParams& params) {
    ...
    // Same predicate as micro_transpose_v: the V*S leading dimension becomes the key count,
    // so both K and V must be static for the generator to take this layout.
    const bool value_transposed = extended_input_v_transpose_order == std::vector<int64_t>{0, 1, 3, 2} && !k_layout.is_dynamic() && !v_layout.is_dynamic();
