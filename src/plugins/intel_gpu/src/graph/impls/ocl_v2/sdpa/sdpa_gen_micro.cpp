...
const char* sdpa_debug_env(const char* name) {
...
JitConstants SDPAMicroGenerator::get_jit_constants(const kernel_impl_params& params, const micro::Package& gemm_kq, const micro::Package& gemm_vs) const {
    ...
    jit.make("I8_KQ", K.data_type == ov::element::i8 ? 1 : 0);


    const bool i8_vs = V.data_type == ov::element::i8;
    ...
    if (i8_vs) {
        ...
        const char* deq = sdpa_debug_env("OV_SDPA_VS_DEQ");
        jit.make("I8_VS_DEQ", deq ? deq : "1.0f");
        if (sdpa_debug_env("OV_SDPA_I8VS_FOLD")) {
        ...
        if (sdpa_debug_env("OV_SDPA_I8VS_MAGIC"))
            // RTE via the 2^23 exponent bias; see tile_quantize_to_char4.
            jit.make("I8_VS_QUANT_MAGIC", 1);
    }

    // when the trailing per-tensor quantizer is fused into micro-SDPA
    ...
    if (d_full) {
        ...
        // the block2d store is defined on the half tile only; keep it off when the
...
void SDPAMicroGenerator::init_microkernels(const kernel_impl_params& params,
    ...
    {  // debug override must apply in prefill too, so the tile config is tunable for short-sequence SDPA.
