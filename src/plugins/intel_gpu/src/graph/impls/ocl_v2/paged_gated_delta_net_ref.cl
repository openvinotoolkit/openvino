 ...
 int v_head_stride) {
    ...
    for (int v_idx = 0; v_idx < V_BLOCK_SIZE; v_idx++) {
...

        {
            ...
            int base = (block_id * V_HEAD_NUM + h) * (K_HEAD_DIM * V_HEAD_DIM) + curr_iv * K_HEAD_DIM;
#if KV_OPT_VEC_PATH
            for (int kc = 0; kc < K_VEC_COUNT; kc++) {
                ...
                for (int ke = 0; ke < K_VEC_SIZE; ke++) {
                    ...
                    tmp[ke] = convert_float(recurrent_state_table[base + k_idx]);
                }
        ...
        if (interval > 0) {
            ...
            for (int v_idx = 0; v_idx < V_BLOCK_SIZE; v_idx++) {
                ...
                for (int kc = 0; kc < K_VEC_COUNT; kc++) {
                    ...
                    for (int ke = 0; ke < K_VEC_SIZE; ke++) {
                        const int k_idx = lid * K_LANE_ELEMS + kc * K_VEC_SIZE + ke;
                        recurrent_state_table[base + k_idx] = (INPUT3_TYPE)(tmp[ke]);
                    }
                }
#else
                for (int ks = 0; ks < K_SLICE_SIZE; ks++) {
