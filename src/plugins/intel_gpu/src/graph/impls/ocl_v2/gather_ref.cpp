...
inline layout extend_shape_to_rank_from_end(const layout& l, size_t rank = 4) {
    auto pshape = l.get_partial_shape();
    if (pshape.size() >= rank) {
        return l;
    }
    pshape.insert(pshape.end(), rank - pshape.size(), ov::Dimension(1));
    return {pshape, l.data_type, l.format, l.data_padding};
}
...
layout get_extended_out_layout(const RuntimeParams& params) {
    ...
    return extend_shape_to_rank_from_end(out_l);
}
...
// TODO: simplify these functions once we migrate to new shape infer - at least we don't need this non_empty_dims stuff as
// we know actual ranks from partial shape
std::string get_indices_idx_order(const layout& idx_l, const layout& out_l, size_t axis, int64_t batch_dim) {
    ...
    if ((axis == static_cast<size_t>(batch_dim)) && (axis > 1) && (idx_l.get_rank() > 4)) {
        ...
        std::cerr << "init: " << get_order_string(idx_order) << std::endl;
        std::cerr << "batch_dim: " << batch_dim << std::endl;
...
protected:
    [[nodiscard]] JitConstants get_jit_constants(const RuntimeParams& params) const override {
        ...
        const auto& data_l = extend_shape_to_rank_from_end(params.input_layouts[0]);
        const auto& idx_l = extend_shape_to_rank_from_end(params.input_layouts[1]);
        ...
        jit.add(make_layout_jit_constants("INPUT0", data_l, in_offsets_map.at(0)));
        jit.add(make_layout_jit_constants("INPUT1", idx_l, in_offsets_map.at(1)));
        jit.add(make_layout_jit_constants("OUTPUT", out_l, out_offsets_map.at(0)));
