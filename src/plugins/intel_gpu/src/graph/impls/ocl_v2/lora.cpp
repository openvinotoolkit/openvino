...
protected:
    ...
    [[nodiscard]] DispatchDataFunc get_dispatch_data_func() const override {
        return DispatchDataFunc{[](const RuntimeParams& params, KernelData& kd, ImplRuntimeParams* rt_params) {
            ...
            wgs.local = {1, gemma_sgK, lora_rank * lora_count};
        }};
    }
};
...
std::vector<size_t> get_stages_execution_order_fused_lora(const cldnn::primitive_inst& instance) {
    const auto& params = *instance.get_impl_params();
    std::vector<size_t> stages_order;
...
public:
    ...
    cldnn::event::ptr execute(const std::vector<cldnn::event::ptr>& events, cldnn::primitive_inst& instance) override {
        ...
        const auto& exec_stages = is_simple_lora ? get_stages_execution_order_single_lora(instance)
                                                 : get_stages_execution_order_fused_lora(instance);
