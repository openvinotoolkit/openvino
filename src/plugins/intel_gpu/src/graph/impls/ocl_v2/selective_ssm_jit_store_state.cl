...
// SSM_JIT_STORE_STATE(p, state_element, value) for its state-buffer layout.
#ifndef SSM_JIT_STORE_STATE
#    error "SelectiveSSM state store expression is not defined"
#endif
...
#pragma unroll
    for (uint step = 0; step < SSM_STATE_ITERATIONS; ++step) {
        ...
        if (p < SSM_HEAD_DIM && state_element < SSM_STATE_SIZE)
            SSM_JIT_STORE_STATE(p, state_element, SSM_STATE_AT(p_offset, step, state_element));
    }
}
