...
class PagedAttentionGeneratorBase : public SingleKernelGenerator {
public:
    PagedAttentionGeneratorBase() : SingleKernelGenerator("paged_attention_opt") {}
    ...
    void add_intermediate_inputs(Arguments& args, bool has_scores_output, bool is_multi_token_kernel = false) const {
...
protected:
    ...
    DispatchDataFunc get_dispatch_data_func() const override {
        static auto f = DISPATCH_DATA_FUNC(params, kd, rt_params) {
            if (params.output_layouts[0].is_static() && params.output_layouts[1].is_static()) {
                auto& wgs = kd.params.workGroups;
    ...
    DispatchDataFunc get_dispatch_data_func() const override {
        static auto f = DISPATCH_DATA_FUNC(params, kd, rt_params) {
            if (!params.is_dynamic()) {
                ...
                wgs.global = { heads_num,
                               ceil_div(rtp->paged_attention_aligned_seq_len, get_target_seq_len_block_size()),
    ...
    std::vector<layout> get_internal_buffer_layouts(const kernel_impl_params& params) const override {
        ...
        */
        ...
        if (has_scores_output) {
            ...
            layouts.push_back(layout{ov::PartialShape{subsequences_number}, indexes_dt, format::bfyx});

            if (stage == PagedAttentionStage::PREFILL) {
                // Recalculate buf_size as in case of PREFILL stage it's not needed to allocate buffer per each input token
                buf_elements_count = subsequences_number * desc->heads_num * num_of_partitions;
        ...
        layout buf_layout{ov::PartialShape{buf_elements_count}, softmax_accumulator_type, format::bfyx};

        layouts.push_back(buf_layout); // softmax exp_sums
        layouts.push_back(buf_layout); // softmax max_logits
        layouts.push_back({ov::PartialShape{tmp_out_elements_count}, softmax_accumulator_type, format::bfyx});  // intermediate output

        const auto multi_tokens_mode = stage == PagedAttentionStage::MIXED;
        if (multi_tokens_mode) {
            layouts.push_back({ov::PartialShape{total_tokens}, softmax_accumulator_type, format::bfyx});
        }
