...
// NOTE: `unroll_for` is intentionally NOT (re)defined in this file.
// It is already provided once per clBuildProgram batch by the shared
// "batch header" cl_kernels/include/batch_headers/common.cl (see
// Kernels2CHeaders.find_and_set_batch_headers()/append_undefs() in
// primitive_db_gen.py: batch headers are injected ONCE at the start of the
// whole batched translation unit, not per kernel/file). If this file were to
// redefine `unroll_for` itself, primitive_db_gen's blind per-file `#undef`
// generation (it #undefs every macro #define'd anywhere in a kernel .cl file
// at the end of that file's copy in the batch) would strip out the *shared*
// batch-header definition too -- silently breaking `unroll_for` for every
// OTHER kernel file compiled later in the same clBuildProgram call that
// relies on the batch-header definition without redefining it itself (e.g.
// moe_3gemm_swiglu_mlp.cl), causing "implicit declaration of function
// 'unroll_for'" errors that only reproduce when kernels get batched together.
