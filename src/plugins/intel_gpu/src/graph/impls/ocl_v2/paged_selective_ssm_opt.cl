...
KERNEL(paged_selective_ssm_opt)(
    ...
    if (token_begin < 0 || token_end < token_begin || (size_t)token_end > tokens ||
        block_begin < 0 || block_end <= block_begin || (size_t)block_end > block_indices_count)
        return;
    ...
    const long previous_in_interval = interval > 0 ? processed % interval : 0;
    ...
    for (long token = token_begin; token < token_end; ++token) {
        ...
        for (size_t n = lane; n < state_size; n += lws) {
            ...
            local_state[n] = fma(local_state[n], dA,
                                 x_value * dt_value * SSM_TO_FLOAT(B[b_idx]));
            partial = fma(local_state[n], SSM_TO_FLOAT(C[c_idx]), partial);
        }
        ...
        if (interval > 0) {
            const long current_tokens = token - token_begin + 1;
            const long cached_tokens = previous_in_interval + current_tokens;
            const bool at_boundary = cached_tokens % interval == 0;
            const bool at_sequence_end = token + 1 == token_end;
            if (at_boundary || at_sequence_end) {
                const long slot = 1 + (cached_tokens - 1) / interval;
                const long block_position = block_begin + slot;
                if (slot < sequence_blocks && block_position >= 0 && (size_t)block_position < block_indices_count) {
