...
gathermatmul_config GatherMatmulMicroGenerator::get_config(const kernel_impl_params& params) {
    ...
    if (std::any_of(quantized_types.begin(), quantized_types.end(), [&](const cldnn::data_types& t) -> bool {
        ...
        // All 6 inputs always present; scale/zp at fixed positions even when has_bias=false.
        ...
        OPENVINO_ASSERT(k % num_scale_groups == 0, "GatherMatmul: K (", k, ") must be divisible by num_scale_groups (", num_scale_groups, ")");
        cfg.weight_group_size = static_cast<int32_t>(k / num_scale_groups);
        // ZP input is always at weight_zp_idx, but is a placeholder when has_zp=false.
...
JitConstants GatherMatmulMicroGenerator::get_jit_constants(const kernel_impl_params& params,
    ...
    // Input order: INPUT, WEIGHT, INDICES, BIAS, SCALE, ZP.
...
void GatherMatmulMicroGenerator::init_microkernels(const kernel_impl_params& params, micro::Package& gemm_bgm, bool is_prefill) {
    ...
    if (bgm_cfg.is_weight_quantized) {
        ...
        problem.A.setAlignment(micro::alignment_for_ld(static_cast<int>(k * problem.Ta_ext)));
        ...
        problem.A.setAlignment(micro::alignment_for_ld(static_cast<int>(k * problem.Ta)));
    }
    ...
    problem.B.setAlignment(micro::alignment_for_ld(static_cast<int>(k * problem.Tb)));
...
DispatchDataFunc GatherMatmulMicroGenerator::get_dispatch_data_func() const {
    return DispatchDataFunc{[](const RuntimeParams& params, KernelData& kd, ImplRuntimeParams* rt_params) {
        ...
        // One WG per (token, expert_slot); z = n_tokens * top_k.
