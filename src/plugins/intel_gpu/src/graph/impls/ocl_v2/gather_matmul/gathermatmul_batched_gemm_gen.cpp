...
JitConstants GatherMatmulBatchedGemmGenerator::get_jit_constants(const kernel_impl_params& params,
    ...
    // expert_gemm_compute.cl references INPUT0_TYPE in its signature.
    const auto& in_offsets_map = params.in_port_to_shape_info_offset;
    const auto& out_offsets_map = params.out_port_to_shape_info_offset;
    jit.add(make_layout_jit_constants("INPUT0", params.input_layouts[gather_matmul::BGMInputIdx::INPUT], in_offsets_map.at(gather_matmul::BGMInputIdx::INPUT)));
    jit.add(
...
DispatchDataFunc GatherMatmulBatchedGemmGenerator::get_dispatch_data_func() const {
    return DispatchDataFunc{[](const RuntimeParams& params, KernelData& kd, ImplRuntimeParams* rt_params) {
        ...
        // x = M tiles, y = token tiles, z = max_groups.
