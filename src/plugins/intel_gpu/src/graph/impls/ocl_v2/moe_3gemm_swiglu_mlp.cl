...
// Only included here (not at top level) to avoid duplicate definitions when
// gate_up / down / reduce variants are batch-compiled into a single CL unit.
#include "moe_gate_activation.cl"
...
#    endif
        ...
        if (id_local == 0) {
            if (silu) {
                y[n] *= moe_gate_activation(sum_all0);
                y[n + 1] *= moe_gate_activation(sum_all1);
            } else {
                y[n] = sum_all0;
                y[n + 1] = sum_all1;
            }
