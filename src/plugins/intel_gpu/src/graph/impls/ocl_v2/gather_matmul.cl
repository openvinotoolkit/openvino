...
// BatchGatherMatmul, per-token dispatch.
// gid(0)=M tile, gid(2)=flat (token_idx, expert_slot); A is broadcast (n_act=1) or per-slot (n_act=top_k).
...
#    endif
    ...
    // Decode path: 1 token/WG. Prefill uses gather_matmul_batched.cl.
    int cur_n_tokens = 1;
