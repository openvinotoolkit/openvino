...
std::vector<uint32_t> ResidentExpertWeightProvider::acquire(size_t /*layer*/,
...
std::vector<uint32_t> OffloadExpertWeightProvider::acquire(size_t layer,
                                                           const std::vector<uint32_t>& experts,
                                                           cldnn::stream& stream) {
    ...
    for (size_t i = 0; i < experts.size(); i++) {
        ...
        const auto item = _cache->get_lru_item(layer, expert);
        ...
        if (item.second) {
            ...
            moe_otd::fill_weights_memory(stream, _config, _weight_bin_offsets, _weights_path, *_resident, {expert}, {slot}, layer);
