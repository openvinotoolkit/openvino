...
JitConstants MoE3GemmMicroGenerator::get_jit_constants(const kernel_impl_params& params, const micro::Package& moe_gemm, const moe_3gemm_config& cfg) const {
    ...
    jit.make("INPUT1_TYPE", to_ocl_type(data_types::u8));       // weight: u4
    ...
    jit.make("WEIGHT_SCALE_DT", to_ocl_type(data_types::f16));  // scale: f16
    jit.make("WEIGHT_ZP_DT", to_ocl_type(data_types::u8));      // zp: u4
    ...
    if (cfg.weight_group_size > 0) {
        ...
        GPU_DEBUG_TRACE_DETAIL << "\t NUM_GROUPS: " << params.input_layouts[m_scale_idx].get_shape()[1] << std::endl;
    } else {
...
KernelData MoE3GemmMicroGenerator::get_kernel_data(const kernel_impl_params& params) const {
    ...
    try {
        ...
        GPU_DEBUG_TRACE_DETAIL << "MoE3GemmMicroGenerator::get_kernel_data() - can't build code: " << ex.what() << std::endl;
