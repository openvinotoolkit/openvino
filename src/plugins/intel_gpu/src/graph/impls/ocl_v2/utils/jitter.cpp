...
JitConstants make_type_jit_constants(const std::string& name, const ov::element::Type& value) {
    ...
    default:
        ...
        if (value == element::dynamic) {
            // element::dynamic marks an unused/empty placeholder input (e.g. the symmetric MoE
            // zero-point, which carries no data). Expose it to the kernel as a byte type so the
            // auto-generated *_TYPE constants stay valid; the kernel never reads it (guarded by
            // HAS_ZP / GGUF decode paths).
            type = "uchar";
            max_val = "UCHAR_MAX";
            min_val = "0";
            val_one = "(uchar) 1";
