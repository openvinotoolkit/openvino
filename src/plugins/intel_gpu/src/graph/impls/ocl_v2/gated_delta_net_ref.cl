...
(__global INPUT0_TYPE* q,
 ...
 int value_offset) {
    ...
    const int QK_T_STRIDE = HK_len * K_len;
    ...
    __global INPUT3_TYPE* state_out = initial_state;
    #if OUTPUT_STATE
        state_out = (__global INPUT3_TYPE*)output_state;
    #endif
#if (K_HEAD_DIM == 128)
...
#    pragma unroll
    for (int row_chunk = 0; row_chunk < (K_HEAD_DIM / 16); row_chunk++) {
...
#    pragma unroll
    ...
    for (int t = 0; t < T_len; t++) {
        ...
        int qk_offset = b * QK_B_STRIDE + t * QK_T_STRIDE + hk * K_len;
...
//             b_output = sub_group_broadcast(b_output, 0);
            ...
            // output[v_offset + start_iv + v_idx] = TO_OUTPUT_TYPE(b_output);
        }
#else
        if (lid == 0) {
            int g_idx = (b * T_len + t) * H_len + h;
...
#    pragma unroll
        for (int v_idx = 0; v_idx < V_BLOCK_SIZE; v_idx++) {
            int curr_iv = start_iv + v_idx;
            state_out[STATE_BASE + row_idx * V_len + curr_iv] = TO_OUTPUT_TYPE(h_state[v_idx][row_chunk]);
        }
    }
#else
    if (lid == 0) {
        for (int k_idx = 0; k_idx < K_len; k_idx++) {
            for (int v_idx = 0; v_idx < V_BLOCK_SIZE; v_idx++) {
                int curr_iv = start_iv + v_idx;
                state_out[STATE_BASE + k_idx * V_len + curr_iv] = TO_OUTPUT_TYPE(h_state_f[v_idx][k_idx]);
            }
        }
    }
#endif
