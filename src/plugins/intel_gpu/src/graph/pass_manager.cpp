...
void pass_manager::run(program& p, base_pass& pass) {
    ...
    //GPU_DEBUG_LOG(config) << "Run pass " << pass.get_name() << " (program_id=" << p.get_id() << ")" << std::endl;
    ...
    //GPU_DEBUG_LOG(config) << "Pass " << pass.get_name() << " execution time: " << opt_pass_time.count() << " ms" << std::endl;
