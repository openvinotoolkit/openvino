...
static cldnn::layout get_eltwise_output_layout(const layout& input_layout, const kernel_impl_params& impl_param) {
...
layout eltwise_inst::calc_output_layout(const eltwise_node& node, const kernel_impl_params& impl_param) {
    ...
    if (input_node_layout.data_type == data_types::i8 || input_node_layout.data_type == data_types::u8 || input_node_layout.data_type == data_types::i32 ||
        input_node_layout.data_type == data_types::i64) {
        std::vector<eltwise_mode> eltwise_int_modes = {
            eltwise_mode::sum,         eltwise_mode::sub,        eltwise_mode::prod,       eltwise_mode::div,          eltwise_mode::min,
            eltwise_mode::max,         eltwise_mode::mod,        eltwise_mode::eq,         eltwise_mode::ne,           eltwise_mode::lt,
            eltwise_mode::le,          eltwise_mode::gt,         eltwise_mode::ge,         eltwise_mode::squared_diff, eltwise_mode::floor_mod,
            eltwise_mode::logic_and,   eltwise_mode::logic_or,   eltwise_mode::logic_xor,  eltwise_mode::right_shift,  eltwise_mode::left_shift,
            eltwise_mode::bitwise_and, eltwise_mode::bitwise_or, eltwise_mode::bitwise_xor};
...
template <typename ShapeType>
std::vector<layout> eltwise_inst::calc_output_layouts(const eltwise_node& /*node*/, const kernel_impl_params& impl_param) {
    auto desc = impl_param.typed_desc<eltwise>();
    ...
    auto mode = desc->mode;
    // list of operations supported for integer types
    if (input_layout.data_type == data_types::i8 || input_layout.data_type == data_types::u8 || input_layout.data_type == data_types::i32 ||
        input_layout.data_type == data_types::i64) {
        std::vector<eltwise_mode> eltwise_int_modes = {
            eltwise_mode::sum,         eltwise_mode::sub,        eltwise_mode::prod,       eltwise_mode::div,          eltwise_mode::min,
            eltwise_mode::max,         eltwise_mode::mod,        eltwise_mode::eq,         eltwise_mode::ne,           eltwise_mode::lt,
            eltwise_mode::le,          eltwise_mode::gt,         eltwise_mode::ge,         eltwise_mode::squared_diff, eltwise_mode::floor_mod,
...
template std::vector<layout> eltwise_inst::calc_output_layouts<ov::PartialShape>(const eltwise_node& node, const kernel_impl_params& impl_param);
...
std::string eltwise_inst::to_string(const eltwise_node& node) {
...
eltwise_inst::typed_primitive_inst(network& network, const eltwise_node& node) : parent(network, node) {
    ...
    if (!prim->stride.empty()) {
        ...
        CLDNN_ERROR_NOT_EQUAL(node.id(), "Eltwise inputs count", inputs_count, "Eltwise strides count", prim->stride.size(), "");
        ...
        for (size_t i = 0; i < inputs_count; i++) {
            ...
            if (prim->broadcast_spec == ov::op::AutoBroadcastType::NUMPY && inputs_count >= 2) {
                ...
                for (size_t i = 0; i < inputs_count; ++i) {
                    input_shapes.push_back(node.get_input_pshape(i));
                }

                // Use OpenVINO core shape inference
                ov::op::v1::Add dummy_op;
                dummy_op.set_autob(ov::op::AutoBroadcastType::NUMPY);

                try {
                    ov::op::eltwise_shape_infer(&dummy_op, input_shapes);
                } catch (const std::exception&) {
                    CLDNN_ERROR_MESSAGE(node.id(), "Invalid input shapes");
                }
            }
        }
    }
...
void eltwise_inst::check_inputs_count(const eltwise_node& node) {
    ...
    case eltwise_mode::logic_or:
    case eltwise_mode::logic_xor:
        OPENVINO_ASSERT(inputs_number >= 2, "Node id: ", node.id(), ". Invalid eltwise inputs number (should be equal at least to 2). Actual: ", inputs_number);
        break;
    case eltwise_mode::eq:
    case eltwise_mode::ne:
    case eltwise_mode::lt:
    ...
    case eltwise_mode::bitwise_or:
    case eltwise_mode::bitwise_xor:
        OPENVINO_ASSERT(inputs_number == 2, "Node id: ", node.id(), ". Invalid eltwise inputs number (should be equal to 2). Actual: ", inputs_number);
        break;
    case eltwise_mode::is_finite:
    case eltwise_mode::is_inf:
    case eltwise_mode::is_nan:
        OPENVINO_ASSERT(inputs_number == 1, "Node id: ", node.id(), ". Invalid eltwise inputs number (should be equal to 1). Actual: ", inputs_number);
        break;
    }
}

bool eltwise_node::need_align_for_numpy_broadcast(const layout& input) const {
    if (is_valid_output_layout() && (format::is_default_format(get_output_layout().format) || input.format == get_output_layout().format))
        return false;
    ...
    if (pshape_a_rank != pshape_b_rank && pshape_a_rank > 1 && pshape_b_rank > 1 && input.get_partial_shape().rank() == small_pshape_rank)
        return true;
