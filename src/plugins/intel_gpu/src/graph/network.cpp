...
network::network(cldnn::BinaryInputBuffer& ib, stream::ptr stream, engine& engine, uint16_t stream_id)
    ...
    for (int i = 0; i < num_data_nodes; ++i) {
        ...
        std::shared_ptr<cldnn::primitive_inst> new_primitive_inst = cldnn::data::type_id()->create_instance(*this);
    ...
    std::cout << "[network load 2-2-2]: " <<
                 (std::chrono::duration_cast<std::chrono::nanoseconds>(startTime - endTime).count() * 0.000001)
                 << std::endl;
    ...
    for (const auto& p_inst : _exec_order) {
        ib >> *p_inst;
        _primitives[p_inst->get_id()] = p_inst;
        if (p_inst->is_input())
        ...
        if (p_inst->type() == cldnn::concatenation::type_id() && p_inst->can_be_optimized()) {
            ...
            while (!stack.empty()) {
                ...
                for (auto processed_node : *nodes_list) {
                    ...
                    dep_node->set_output_memory(p_inst->output_memory_ptr());
