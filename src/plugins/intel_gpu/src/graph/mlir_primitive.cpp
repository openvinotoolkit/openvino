...
layout mlir_primitive_inst::calc_output_layout(const mlir_primitive_node& node, const kernel_impl_params& impl_param) {
    return calc_output_layouts<ov::PartialShape>(node, impl_param)[0];
}

template <typename ShapeType>
std::vector<layout> mlir_primitive_inst::calc_output_layouts(const mlir_primitive_node& /*node*/, const kernel_impl_params& impl_param) {
    ...
    OPENVINO_ASSERT(prim->shape_infer_f,
                    "[GPU] Shape inference function is not set for mlir_primitive '",
                    prim->id,
    ...
    for (size_t i = 0; i < output_shapes.size(); ++i) {
        const auto out_dt = prim->get_output_data_type(i);
        if (!out_dt.has_value()) {
            OPENVINO_THROW("[GPU] Output data type is not set for mlir_primitive '", prim->id, "', output ", i);
        }
        out_layouts.emplace_back(output_shapes[i], *out_dt, format::get_default_format(output_shapes[i].size()));
    }
    return out_layouts;
}

template std::vector<layout> mlir_primitive_inst::calc_output_layouts<ov::PartialShape>(const mlir_primitive_node&, const kernel_impl_params&);

std::string mlir_primitive_inst::to_string(const mlir_primitive_node& node) {
