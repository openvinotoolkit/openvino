...
namespace cldnn {
GPU_DEFINE_PRIMITIVE_TYPE_ID(fc_shape_of)
...
data_types get_output_data_type(const kernel_impl_params& impl_param) {
    ...
    const auto prim = impl_param.typed_desc<fc_shape_of>();
    return prim->output_data_types[0].value_or(data_types::i32);
}
}  // namespace

layout fc_shape_of_inst::calc_output_layout(fc_shape_of_node const& node, kernel_impl_params const& impl_param) {
    const auto prim = impl_param.typed_desc<fc_shape_of>();
...
std::vector<layout> fc_shape_of_inst::calc_output_layouts(fc_shape_of_node const& /*node*/, const kernel_impl_params& impl_param) {
    const auto dt = get_output_data_type(impl_param);
...
template std::vector<layout> fc_shape_of_inst::calc_output_layouts<ov::PartialShape>(fc_shape_of_node const& node, const kernel_impl_params& impl_param);

std::string fc_shape_of_inst::to_string(fc_shape_of_node const& node) {
    ...
    node_info->add("fc_shape_of info", shape_of_info);
    node_info->dump(primitive_description);

    return primitive_description.str();
}

fc_shape_of_inst::typed_primitive_inst(network& network, fc_shape_of_node const& node) : parent(network, node, true) { }
}  // namespace cldnn
