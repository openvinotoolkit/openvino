...
template <typename T>
static std::vector<layout> get_output_layouts(kernel_impl_params const& impl_param,
                                              std::vector<T> body_outputs,
                                              const int64_t num_iterations = -1) {
    auto prim = impl_param.typed_desc<loop>();
...
std::vector<layout> loop_inst::calc_output_layouts(loop_node const& /*node*/, kernel_impl_params const& impl_param) {
    ...
    if (impl_param.inner_nets.empty()) {
        ...
        OPENVINO_ASSERT(memory_deps.count(current_iteration_idx) > 0,
                        "The count of memory deps(current_iteration) should not be zero");
        cldnn::mem_lock<int64_t, mem_lock_type::read> current_iterations_lock(memory_deps.at(current_iteration_idx),
                                                                              impl_param.get_stream());
        int64_t current_iteration = static_cast<int64_t>(*current_iterations_lock.data());
        GPU_DEBUG_LOG << "* current_iteration(" << memory_deps.at(current_iteration_idx) << ")  : " << current_iteration
                      << std::endl;
        ...
        output_layouts =
            get_output_layouts<std::shared_ptr<primitive_inst>>(impl_param, body_outputs, current_iteration);
    }
    return output_layouts;
}

template std::vector<layout> loop_inst::calc_output_layouts<ov::PartialShape>(loop_node const& node,
...
static void validate_mappings(loop_node const& node) {
    ...
    for (const auto& id : outer_inputs) {
        if (id == node.get_trip_count_id() || id == node.get_initial_execution_id() ||
            id == node.get_num_iterations_id()) {
            continue;
        }
        const auto results =
            find_io_primitive_maps(node.get_input_primitive_maps(), node.get_output_primitive_maps(), id, true);
    ...
    for (const auto& pm : input_primitive_maps) {
        ...
        OPENVINO_ASSERT(found != outer_inputs.end(),
                        node.id(),
                        " : external id '",
                        pm.external_id.pid,
                        "' in primitive map cannot be found loop inputs");
    }
    ...
    for (const auto& pm : input_primitive_maps) {
        ...
        OPENVINO_ASSERT(found != nodes.end(),
                        node.id(),
                        " : internal id '",
                        pm.internal_id.pid,
                        "' in primitive map cannot be found loop body");
    }
    for (const auto& pm : output_primitive_maps) {
        ...
        OPENVINO_ASSERT(found != nodes.end(),
                        ...
                        " : internal id '",
                        pm.internal_id.pid,
                        "' in primitive map cannot be found body body");
    }
}
...
void loop_inst::update_input_mapped_memory() {
    for (size_t memory_num = 0; memory_num < inputs_memory_count(); memory_num++) {
        ...
        auto input_map_ptrs =
            find_io_primitive_maps(_input_primitive_maps, _output_primitive_maps, input_external_id, true);
        if (input_map_ptrs.empty()) {
            if (input_external_id == _trip_count_id || input_external_id == _initial_execution_id) {
                continue;
            }
        }
...
void loop_inst::update_backedge_mapped_memory() {
    ...
    for (const auto& back_edge : _back_edges) {
        ...
        const auto input_map_ptrs =
            find_io_primitive_maps(_input_primitive_maps, _output_primitive_maps, back_edge.to, false);
            ...
            if (backedge_mapping.from_primitive->id() == backedge_from_prim->id() &&
                ...
                if (backedged_sliced_output == nullptr) {
                    ...
                    const auto output_mapping =
                        find_io_primitive_maps(_input_primitive_maps, _output_primitive_maps, back_edge.from, false);
...
loop_inst::concatenated_memory_mapping::ptr loop_inst::create_concat_memory_map(
    ...
    if (extern_mem_ptr != nullptr) {
        ...
        if (inter_mem_ptr == nullptr || shape_changed()) {
            ...
            GPU_DEBUG_LOG << "output pshape for [" << intern_prim->id() << "] is changed from "
                          << sliced_layout.get_partial_shape().to_string() << " to "
                          << updated_sliced_layout.to_string() << std::endl;
        ...
        // When num_iterations is -1, allocate first sliced_mem and allocate sliced memory if additional sliced mem is
        // required
        if (num_iterations < 0) {
    ...
    return std::make_shared<concatenated_memory_mapping>(extern_mem_ptr,
                                                         sliced_mems,
                                                         stream,
                                                         engine,
                                                         concat_data_prim,
...
void loop_inst::preprocess_output_memory(const int64_t num_iterations) {
    ...
    for (size_t i = 0; i < _output_primitive_maps.size(); ++i) {
        ...
        if (output_mapping.axis < 0) {
            // In dynamic model, Don't get output memory of loop node because body network's output layouts are not
            // calculated
            if (memory != nullptr) {
            ...
            if (iter == concatenated_output_mem_mappings.end()) {
                ...
                GPU_DEBUG_LOG << i << ") generate concat output memory mapping: " << memory_mapping_info->to_string()
                              << std::endl;
            }
            GPU_DEBUG_IF(iter != concatenated_output_mem_mappings.end()) {
                GPU_DEBUG_LOG << i << ") memory_mapping_info is already existed : " << (*iter)->to_string()
                              << std::endl;
            }
        }
    }
...
void loop_inst::preprocess_input_memory(const int64_t num_iterations) {
    for (size_t memory_num = 0; memory_num < inputs_memory_count(); memory_num++) {
        ...
        if (input_map_ptrs.size() == 0) {
            OPENVINO_ASSERT((input_external_id == _trip_count_id || input_external_id == _num_iterations_id ||
                             input_external_id == _initial_execution_id),
                            id(),
                            "loop primitive_map is incomplete "
                            "input_external_id(",
                            input_external_id,
                            ") != _trip_count_id(",
                            _trip_count_id,
            ...
            if (input_map->axis < 0) {
                ...
                if (memory->get_layout() != input_inst->get_output_layout()) {
                    ...
                    GPU_DEBUG_LOG << input_inst->id() << " is changed memory because layout is changed from "
                                  << input_inst->get_output_layout().to_short_string() << " to "
                                  << memory->get_layout().to_short_string() << std::endl;
                }

                auto internal_input_memory = memory;
                auto iter =
                    std::find_if(_back_edges.begin(), _back_edges.end(), [&](cldnn::loop::backedge_mapping& mapping) {
                ...
                if (iter != _back_edges.end()) {
                    ...
                    GPU_DEBUG_LOG << "Input memory of internal node(" << internal_id.to_string()
                                  << ") is set to new memory(" << internal_input_memory << ", "
                                  << internal_input_memory->get_layout().to_short_string()
                                  << ") instead of external node(" << external_id.to_string() << ")'s memory(" << memory
                                  << "," << memory->get_layout().to_short_string() << ")" << std::endl;
                }

                body_network->set_input_data(internal_id.pid, internal_input_memory);
            } else {
                OPENVINO_ASSERT(memory != nullptr,
                                "In preprocessing concat input mapping, concat memory should be allocated");
                ...
                GPU_DEBUG_LOG << i << ") generate concat input memory mapping: " << memory_mapping_info->to_string()
                              << std::endl;
            }
        }
    }
...
void loop_inst::preprocess_backedge_memory() {
    ...
    for (size_t idx = 0; idx < _back_edges.size(); idx++) {
        ...
        if (backedged_sliced_output != nullptr) {
            ...
            backedge_memory_mappings.emplace_back(backedge_from_prim,
                                                  backedge_to_prim,
                                                  backedged_sliced_output,
                                                  initial_mem,
                                                  body_network->get_stream());
        ...
        } else if ((output_mapping.empty() &&
                    backedge_to_prim.get() == backedge_from_prim->dependencies().front().first) ||
            ...
            backedge_memory_mappings.emplace_back(backedge_from_prim,
                                                  ...
                                                  initial_mem,
                                                  body_network->get_stream());
            GPU_DEBUG_LOG << idx << ") add back_edge mapping with SINGLE type, initial_mem(" << initial_mem << ")"
                          << std::endl;
        } else {
            // SINGLE_SHARED mode
            memory::ptr backedge_mem;
            ...
            if (is_dynamic()) {
                if (output_prim->outputs_allocated()) {
                    ...
                    if (internal_output_prim_mem->get_layout() == initial_mem->get_layout()) {
                        ...
                        body_network->set_input_data(back_edge.to, backedge_mem);
                        GPU_DEBUG_LOG << idx << ") Get backedge_mem(" << backedge_mem << ") from back_edge.from("
                                      << back_edge.from << ")" << std::endl;
                    } else {
                        ...
                        GPU_DEBUG_LOG << idx << ") Just set input data using initial_mem because back_edge.from("
                                      << back_edge.from << ") layout is changed or backedge_mem is nullptr"
                                      << std::endl;
                    }
                } else {
                    body_network->set_input_data(back_edge.to, initial_mem);
                ...
                if (output_mapping.empty()) {
                    ...
                    GPU_DEBUG_LOG << idx << ") Get backedge_mem(" << backedge_mem
                                  << ") from output_mapping_external_id.pid(" << out_mapping_ext_id.pid << ")"
                                  << std::endl;
            ...
            backedge_memory_mappings.emplace_back(backedge_from_prim,
                                                  ...
                                                  initial_mem,
                                                  body_network->get_stream());
            GPU_DEBUG_LOG << idx << ") add back_edge mapping with SINGLE_SHARED type, backedge_mem(" << backedge_mem
                          << "), initial_mem(" << initial_mem << ")" << std::endl;
        }
    }
}

std::shared_ptr<loop_inst::concatenated_memory_mapping> loop_inst::get_sliced_mem(
    const primitive_id& internal_id) const {
    ...
    for (const auto& back_edge : back_edges) {
        for (const auto& mapping : input_primitive_maps) {
            OPENVINO_ASSERT((mapping.internal_id.pid != back_edge.to || mapping.axis < 0),
                            node.id(),
                            ": input with iteration axis should not have backedges external_id: ",
                            mapping.external_id.to_string(),
                            ", internal_id: ",
                            mapping.internal_id.to_string(),
                            ", back_edge.to: ",
                            back_edge.to,
                            ", back_edge.from ",
                            back_edge.from,
                            ", mapping.axis: ",
                            std::to_string(mapping.axis));
        }
    }
}
...
loop_inst::typed_primitive_inst(network& network, loop_node const& node)
    ...
    OPENVINO_ASSERT(node.get_program().get_node(num_iterations_id).is_type<mutable_data>(),
                    node.id(),
                    ": num_iterations is not mutable_data");
    ...
    if (is_dynamic) {
        ...
        for (size_t i = 0; i < _output_primitive_maps.size(); ++i) {
            ...
            bool output_allocated =
                (static_cast<size_t>(external_id.idx) < _outputs.size() && _outputs[external_id.idx] != nullptr);
            if (output_mapping.axis < 0) {
                ...
                if (!output_allocated) {
                    ...
                    GPU_DEBUG_LOG << "[Internal: " << internal_id.to_string()
                                  << ", External: " << external_id.to_string() << " ] " << "Set internal memory("
                                  << internal_mem << ") to external output because external output memory is nullptr."
                                  << std::endl;
                } else {
                    ...
                    if (external_mem != internal_mem) {
                        if (external_mem->get_layout() != internal_mem->get_layout()) {
                            external_outputs[external_id.idx] = internal_mem;
                            GPU_DEBUG_LOG
                                << "[Internal: " << internal_id.to_string() << ", External: " << external_id.to_string()
                                << " ] " << "Set internal memory(" << internal_mem
                                << ") to external output for different layout between external_mem and internal_mem."
                                << std::endl;
                        } else {
                            external_mem->copy_from(get_network().get_stream(), *internal_mem);
                            external_outputs[external_id.idx] = external_mem;
                            GPU_DEBUG_LOG << "[Internal: " << internal_id.to_string()
                                          << ", External: " << external_id.to_string() << " ] "
                ...
                if (!output_allocated || shape_changed()) {
                    ...
                    auto iter =
                        std::find_if(concatenated_output_mem_mappings.begin(),
                                     concatenated_output_mem_mappings.end(),
                    ...
                    if (iter != concatenated_output_mem_mappings.end()) {
                        ...
                        GPU_DEBUG_LOG << "[Internal: " << internal_id.to_string()
                                      << ", External: " << external_id.to_string() << " ]" << " Update concat_mem"
                                      << std::endl;
                    }
                    GPU_DEBUG_IF(iter == concatenated_output_mem_mappings.end()) {
                        GPU_DEBUG_LOG << "[Internal: " << internal_id.to_string()
                    ...
                    GPU_DEBUG_LOG << "[Internal: " << internal_id.to_string()
                                  << ", External: " << external_id.to_string() << " ]" << " No update concat_mem"
                                  << std::endl;
                }
            }
        }
    ...
    if (new_layouts.empty()) {
        ...
        for (size_t i = 0; i < new_layouts.size(); ++i) {
            ...
            new_layout.data_padding =
                padding::max(_node->get_primitive()->get_output_padding(i), new_layout.data_padding);
...
void loop_inst::concatenated_memory_mapping::slice_mem(const int64_t num_iterations) const {
    ...
    OPENVINO_ASSERT(num_iters > 0 && num_iters == sliced_mems.size(),
                    "num_iterations(",
                    num_iters,
                    ") should be same with sliced_mems.size(",
                    sliced_mems.size(),
                    ")");
    ...
    OPENVINO_ASSERT((static_cast<size_t>(step) == sliced_mem_shape[axis]) &&
                    ...
                    "slice_mem: concat_mem_shape[axis(",
                    axis,
                    "),step(",
                    step,
                    ")](",
                    concat_mem_shape.to_string(),
                    ") != num_iterations(",
                    num_iterations,
                    ") * sliced_mem_shape[axis](",
                    sliced_mem_shape.to_string(),
                    ")");
    ...
    GPU_DEBUG_LOG << "slice memory [" << io_prim_map.to_short_string() << "] from concat_mem["
                  << concatenated_mem->get_layout().to_short_string() << "], current_iteration: " << num_iterations
                  << ", stride: " << stride << " to sliced_mems[" << sliced_mems.front()->get_layout().to_short_string()
                  << "]" << std::endl;
}
...
void loop_inst::concatenated_memory_mapping::concat_mem(const int64_t curent_iterations) const {
    ...
    OPENVINO_ASSERT(sliced_mems.size() >= curr_iters,
                    "curent_iterations(",
                    curr_iters,
                    ") should be less than the number of sliced_mems(",
                    sliced_mems.size(),
                    ")");
    OPENVINO_ASSERT(concatenated_mem != nullptr, "concatenated_mem should not be nullptr");
    ...
    OPENVINO_ASSERT((static_cast<size_t>(step) == sliced_mem_shape[axis]) &&
                    ...
                    "concat_mem: concat_mem_shape[axis(",
                    axis,
                    "),step(",
                    step,
                    ")](",
    ...
    GPU_DEBUG_LOG << "concatenate memory [" << io_prim_map.to_short_string() << "] from sliced_mems["
                  << sliced_mems.front()->get_layout().to_short_string()
                  << "], current_iteration: " << curent_iterations << ", stride: " << stride << " to concat_mem["
...
int64_t loop_inst::get_num_iterations() {
    ...
    for (auto& input_map : _input_primitive_maps) {
        ...
        OPENVINO_ASSERT(num_iterations == current_num_iterations,
                        "iteration num shuld be same between ",
                        num_iterations,
                        " and ",
                        current_num_iterations);
    }
...
void loop_inst::set_memory_in_body_network(cldnn::network::ptr body_network,
                                           const std::shared_ptr<cldnn::primitive_inst>& inst,
                                           memory::ptr mem) {
    if (inst->is_input()) {
...
std::vector<event::ptr> loop_inst::handle_buffers_for_next_iteration(const loop_inst::backedge_memory_mapping& mapping,
    ...
    if (mapping.type == loop_inst::backedge_memory_mapping::CONCAT_OUTPUT) {
        if (iter == 0) {
            ...
            GPU_DEBUG_LOG << iter << ") [CONCAT_OUTPUT] Copy data from inintal_mem(" << mapping.initial_mem << ") to "
                          << mapping.to_primitive->id() << std::endl;
        } else if (iter > 0) {
            if (is_dynamic()) {
                ...
                if (ev)
                    event_vec = {ev};
                GPU_DEBUG_LOG << iter << ") [CONCAT_OUTPUT] Copy data from [" << mapping.from_primitive->id() << "("
                              << from_mem << ")] to [" << mapping.to_primitive->id() << "(" << to_mem << ")]"
                              << std::endl;
            } else {
        ...
        if (iter == 0) {
            if (mapping.from_mem != nullptr) {
                ...
                if (ev)
                    event_vec = {ev};
                GPU_DEBUG_LOG << iter << ") [SINGLE_SHARED] Copy data from inintal_mem(" << mapping.initial_mem << ")"
                              << std::endl;
            }
        } else {
            // In dynamic model, output memory is not defined before execution.
            // After body network execution, replace input memory from initial_mem(external input memory) to output
            // memory.
            if (mapping.from_mem == nullptr) {
                ...
                GPU_DEBUG_LOG << iter << ") [SINGLE_SHARED] Set memory from from_mem(" << mapping.from_mem << ") to "
                              << mapping.to_primitive->id() << ")" << std::endl;
            }
        }
            ...
            if (iter == 0) {
                ...
                if (!mapping.initial_mem->get_layout().identical(to_mem->get_layout())) {
                    ...
                    GPU_DEBUG_LOG << iter << ") [SINGLE] Backedge_to node(" << to_id << ") is set to new memory("
                                  ...
                                  << ") because of shape update from initial memory(" << mapping.initial_mem << ","
                                  << mapping.initial_mem->get_layout().to_short_string() << ")" << std::endl;
                } else {
                    ev = to_mem->copy_from(body_network->get_stream(), *(mapping.initial_mem));
                    GPU_DEBUG_LOG << iter << ") [SINGLE] Copy data from inintal_mem(" << mapping.initial_mem << ")"
                                  << std::endl;
                }
            } else {
                auto from_id = mapping.from_primitive->id();
                ...
                if (!from_mem->get_layout().identical(to_mem->get_layout())) {
                    ...
                    GPU_DEBUG_LOG << iter << ") [SINGLE] Backedge_to node(" << to_id << ") is set to new memory("
                                  << to_mem << ", " << to_mem->get_layout().to_short_string()
                                  << ") because of shape update from backedge_from()" << from_id << ")'s memory("
                                  << from_mem << "," << from_mem->get_layout().to_short_string() << ")" << std::endl;
                    body_network->set_input_data(to_id, to_mem);
...
std::vector<event::ptr> loop_inst::preprocess_memory_for_body_network(int64_t current_iteration_idx) {
    ...
    for (auto& backedge_memory_mapping : backedge_memory_mappings) {
        auto event_vec =
            handle_buffers_for_next_iteration(backedge_memory_mapping, body_network, current_iteration_idx);
...
std::vector<event::ptr> loop_inst::postprocess_memory_for_body_network(int64_t current_iteration_idx) {
    ...
    for (const auto& concat_output_mem_mapping : concatenated_output_mem_mappings) {
        ...
        memory::ptr new_sliced_mem =
            concat_output_mem_mapping->get_or_create_sliced_mem(current_iteration_idx, output_mem_ptr->get_layout());
