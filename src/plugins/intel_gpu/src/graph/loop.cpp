...
static bool check_if_axis_is_set_properly(loop_node const & node) {
    ...
    const std::vector<std::pair<program_node*, int>>& dependencies = node.get_dependencies();
    int32_t iteration_size = -1;
    for (const auto& pm : input_with_axis_iteration) {
        auto found = std::find_if(dependencies.begin(), dependencies.end(), [&pm](const std::pair<program_node*, int>& dep){
            return dep.first->id() == pm.get().external_id;
        });
