...
void detection_output_inst::save(cldnn::BinaryOutputBuffer& buffer) const {
    parent::save(buffer);

    // argument (struct detection_output)
    buffer << argument->id;
    buffer << argument->input[0];
    buffer << argument->input[1];
    buffer << argument->input[2];
    // buffer << argument->ext_prim_id;
    buffer << cldnn::make_data(&argument->output_padding, sizeof(argument->output_padding));
    buffer << argument->num_classes;
    buffer << argument->keep_top_k;
    buffer << argument->share_location;
    buffer << argument->background_label_id;
    buffer << argument->nms_threshold;
    buffer << argument->top_k;
    buffer << argument->eta;
    buffer << cldnn::make_data(&argument->code_type, sizeof(argument->code_type));
    buffer << argument->variance_encoded_in_target;
    buffer << argument->confidence_threshold;
    buffer << argument->prior_info_size;
    buffer << argument->prior_coordinates_offset;
    buffer << argument->prior_is_normalized;
    buffer << argument->input_width;
    buffer << argument->input_height;
    buffer << argument->decrease_label_id;
    buffer << argument->clip_before_nms;
    buffer << argument->clip_after_nms;
}

void detection_output_inst::load(cldnn::BinaryInputBuffer& buffer) {
    parent::load(buffer);

    {
        ...
        buffer >> id;
        buffer >> input_location;
        buffer >> input_confidence;
        buffer >> input_prior_box;
        // buffer >> ext_prim_id;
        buffer >> cldnn::make_data(&output_padding, sizeof(output_padding));
        buffer >> num_classes;
        buffer >> keep_top_k;
        buffer >> share_location;
        buffer >> background_label_id;
        buffer >> nms_threshold;
        buffer >> top_k;
        buffer >> eta;
        buffer >> cldnn::make_data(&code_type, sizeof(code_type));
        buffer >> variance_encoded_in_target;
        buffer >> confidence_threshold;
        buffer >> prior_info_size;
        buffer >> prior_coordinates_offset;
        buffer >> prior_is_normalized;
        buffer >> input_width;
        buffer >> input_height;
        buffer >> decrease_label_id;
        buffer >> clip_before_nms;
        buffer >> clip_after_nms;
