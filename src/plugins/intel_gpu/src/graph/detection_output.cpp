...
layout detection_output_inst::calc_output_layout(detection_output_node const& node,
    ...
    if (desc->share_location) {
        num_classes = (desc->background_label_id == 0) ? desc->num_classes - 1 : desc->num_classes;
...
std::vector<layout> detection_output_inst::calc_output_layouts(detection_output_node const& node,
                                                               kernel_impl_params const& impl_param) {
    auto desc = impl_param.typed_desc<detection_output>();
    ...
    std::vector<ShapeType> input_shapes = {box_logits_shape, class_preds_shape, proposals_shape};
...
template std::vector<layout> detection_output_inst::calc_output_layouts<ov::PartialShape>(
