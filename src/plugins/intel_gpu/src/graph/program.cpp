...
void program::init_program() {
    ...
    if (_engine.runtime_type() != runtime_types::sycl) {
        _kernels_cache = std::unique_ptr<kernels_cache>(
            new kernels_cache(_engine, _config, prog_id, _task_executor, kernel_selector::KernelBase::get_db().get_batch_headers()));
...
void program::post_optimize_graph(bool is_internal) {
    ...
    if (optimize_data) {
        apply_opt_pass<remove_redundant_reorders>(false, true, true);  // pass to remove output reorders while all others
...
void program::rename(program_node& node, const primitive_id& new_id) {
    ...
    if (node.is_output()) {
        throw std::invalid_argument("Trying to rename an output node. If you intend to do that, please "
...
void program::replace(program_node& old_node, program_node& new_node) {
    ...
    if (new_node.is_output()) {
        throw std::invalid_argument("Replacement node shouldn't be marked as an output since it's "
...
void program::set_layout_optimizer_attributes(layout_optimizer& lo) {
    ...
    for (const auto& node : get_processing_order()) {
        ...
        if (prim.is_in_data_flow() && prim.type() != cldnn::convolution::type_id() && prim.type() != cldnn::deconvolution::type_id() &&
            ...
            prim.type() != cldnn::experimental_detectron_generate_proposals_single_image::type_id() && prim.type() != cldnn::rms::type_id() &&
