...
TEST_P(InferencePrecisionTests, smoke_canSetInferencePrecisionAndInfer) {
    ...
    OV_ASSERT_NO_THROW(compiled_model = core->compile_model(function,
                                                            ov::test::utils::DEVICE_GPU,
                                                            ov::hint::inference_precision(inference_precision)));
...
INSTANTIATE_TEST_SUITE_P(smoke_GPU_BehaviorTests,
                         InferencePrecisionTests,
                         ::testing::ValuesIn(test_params),
                         InferencePrecisionTests::getTestCaseName);
...
TEST(ExecutionModeTest, SetCompileGetInferPrecisionAndExecMode) {
    ...
    {
        auto compiled_model =
            core.compile_model(model, ov::test::utils::DEVICE_GPU, ov::hint::inference_precision(ov::element::f32));
