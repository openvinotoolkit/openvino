...
public:
    ...
    virtual std::shared_ptr<ov::Model> get_function() {
        return _func;
    }
    virtual std::shared_ptr<ov::op::v0::Parameter> get_input() {
        return _param;
    }
    virtual std::shared_ptr<ov::op::v0::Result> get_result() {
        return _result;
    }
...
protected:
    std::shared_ptr<ov::Model> generate(ov::PartialShape& input_shape, ov::element::Type prc) override {
        ...
        auto body =
            std::make_shared<ov::Model>(ov::OutputVector{result}, ov::ParameterVector{data}, "eltwise_mul_pooling");
        return body;
    }
...
protected:
    std::shared_ptr<ov::Model> generate(ov::PartialShape& input_shape, ov::element::Type prc) override {
        ...
        std::shared_ptr<ov::Node> reductionAxesNode =
            std::make_shared<ov::op::v0::Constant>(ov::element::i64, ov::Shape(shapeAxes), axes);

        const auto reduce =
            ov::test::utils::make_reduce(add, reductionAxesNode, false, ov::test::utils::ReductionType::Min);
...
protected:
    std::shared_ptr<ov::Model> generate(ov::PartialShape& input_shape, ov::element::Type prc) override {
        ...
        auto body =
            std::make_shared<ov::Model>(ov::OutputVector{result}, ov::ParameterVector{data}, "parameter_to_result");
        return body;
    }
};
...
public:
    enum PredicateTypes { PARAM, NODE };
...
public:
    TestModelGenerator(InnerBodyGenerator::InnerBodyType then_body_type,
        ...
        if (then_body_type == InnerBodyGenerator::InnerBodyType::Type06 ||
            else_body_type == InnerBodyGenerator::InnerBodyType::Type06) {
    ...
    std::shared_ptr<ov::Model> get_function() {
        return function;
    }
...
using ConditionParams = typename std::tuple<ov::Shape,                           // Shape
...
std::vector<ov::element::Type> netPrecisions_static = {ov::element::f32, ov::element::f16, ov::element::i8};

std::vector<ov::Shape> inputs_shape = {{3, 6}};

std::vector<TestModelGenerator::PredicateTypes> if_cond_types = {TestModelGenerator::PredicateTypes::PARAM};

INSTANTIATE_TEST_SUITE_P(smoke_ConditionGPUTest_static,
                         StaticConditionLayerGPUTest,
                         testing::Combine(testing::ValuesIn(inputs_shape),
...
std::vector<ov::element::Type> model_types_static_single = {ov::element::f32, ov::element::f16, ov::element::i8};

std::vector<ov::Shape> inputs_shape_single = {{64}};

INSTANTIATE_TEST_SUITE_P(smoke_ConditionGPUTest_static,
                         StaticConditionSingleLayerGPUTest,
                         testing::Combine(testing::ValuesIn(inputs_shape_single),
...
using ConditionGPUParams = typename std::tuple<InputShape,                          // Input Shapes
    ...
    void generate_inputs(const std::vector<ov::Shape>& targetInputStaticShapes) override {
        ...
        for (const auto& param : function->get_parameters()) {
            if (param->get_output_element_type(0) == ov::element::boolean) {
                ...
                inGenData.seed = 1;
                auto tensor =
                    ov::test::utils::create_and_fill_tensor(param->get_element_type(), input_shape, inGenData);
                inputs.insert({param, tensor});
            }
        }
    }
...
const std::vector<ov::test::InputShape> dynamicInputShapes_f32 = {
    ov::test::InputShape(ov::PartialShape({-1, -1, -1, -1, -1}),
                         {{4, 1, 1, 64, 32}, {6, 1, 1, 8, 4}, {8, 1, 1, 24, 16}}),
...
const std::vector<InnerBodyTypeParams> innerBodyTypes_f32 = {
    {InnerBodyGenerator::InnerBodyType::Type01, InnerBodyGenerator::InnerBodyType::Type02},
    {InnerBodyGenerator::InnerBodyType::Type02, InnerBodyGenerator::InnerBodyType::Type03}};

const std::vector<InnerBodyTypeParams> innerBodyTypes_f16 = {
    {InnerBodyGenerator::InnerBodyType::Type04, InnerBodyGenerator::InnerBodyType::Type03},
    {InnerBodyGenerator::InnerBodyType::Type02, InnerBodyGenerator::InnerBodyType::Type05}};

const std::vector<InnerBodyTypeParams> innerBodyTypes_zero_dims = {
    {InnerBodyGenerator::InnerBodyType::Type02, InnerBodyGenerator::InnerBodyType::Type06},
};

const std::vector<TestModelGenerator::PredicateTypes> condTypes = {TestModelGenerator::PredicateTypes::PARAM,
                                                                   TestModelGenerator::PredicateTypes::NODE};

const std::vector<TestModelGenerator::PredicateTypes> condTypes_zero_dims = {TestModelGenerator::PredicateTypes::PARAM};

INSTANTIATE_TEST_SUITE_P(smoke_ConditionGPUTest_dynamic_f32,
                         DynamicConditionLayerGPUTest,
                         testing::Combine(testing::ValuesIn(dynamicInputShapes_f32),  // input shapes
...
INSTANTIATE_TEST_SUITE_P(smoke_ConditionGPUTest_dynamic_f16,
                         DynamicConditionLayerGPUTest,
                         testing::Combine(testing::ValuesIn(dynamicInputShapes_f16),  // input shapes
...
INSTANTIATE_TEST_SUITE_P(smoke_ConditionGPUTest_zero_dims,
                         DynamicConditionLayerGPUTest,
                         testing::Combine(testing::ValuesIn(dynamicInputShapes_zero_dims),  // input shapes
