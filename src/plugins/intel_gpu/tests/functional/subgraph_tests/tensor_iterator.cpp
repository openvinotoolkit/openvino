 ...
 */
static std::shared_ptr<ov::Model> makeTIwithLSTMcell(ov::element::Type_t model_type,
                                                     ov::PartialShape initShape,
                                                     size_t N,
                                                     size_t I,
                                                     size_t H,
 ...
 *   @param H hidden layer
 */
static std::shared_ptr<ov::Model> makeLSTMSequence(ov::element::Type_t model_type,
                                                   ov::PartialShape initShape,
                                                   size_t N,
                                                   size_t I,
                                                   size_t H,
    ...
    auto rnn_sequence = std::make_shared<ov::op::v5::LSTMSequence>(X, Y, Z, seq_lengths, W, R, B, 128, seq_direction);
...
using DynamicTensorIteratorParams =
...
protected:
    void SetUp() override {
        ...
        if (type == LSTMType::LSTMCell)
            function = makeTIwithLSTMcell(model_type,
                                          init_shape,
                                          batch_size,
                                          input_size,
                                          hidden_size,
            ...
            function = makeLSTMSequence(model_type,
                                        init_shape,
                                        batch_size,
                                        input_size,
                                        hidden_size,
                    ...
                    for (size_t port = 0; port < nodePtr->get_input_size(); ++port) {
                        if (itTargetShape != targetInputStaticShapes.end()) {
                            if (nodePtr->get_input_node_ptr(port)->shared_from_this() ==
                                ...
                                inputs.insert(
                                    {param, it->second(nodePtr, port, param->get_element_type(), *itTargetShape)});
                                break;
                            }
                        } else {
...
std::vector<LSTMType> lstm_types = {LSTMType::LSTMCell, LSTMType::LSTMSequence};
...
INSTANTIATE_TEST_SUITE_P(smoke_DynamicTensorIterator_LSTMCell,
                         DynamicTensorIteratorTest,
                         testing::Combine(
...
INSTANTIATE_TEST_SUITE_P(smoke_DynamicTensorIterator_LSTMSequence,
                         DynamicTensorIteratorTest,
                         testing::Combine(
