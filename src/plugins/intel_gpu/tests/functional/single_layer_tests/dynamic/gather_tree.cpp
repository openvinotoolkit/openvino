...
typedef std::tuple<InputShape,                       // Input tensors shape
...
protected:
    void SetUp() override {
        ...
        if (ov::test::utils::InputLayerType::PARAMETER == secondaryInputType) {
            ...
            auto inp2_tensor =
                ov::test::utils::create_and_fill_tensor(model_type, inputShape.second.front(), maxBeamIndex);
            inp2 = std::make_shared<ov::op::v0::Constant>(inp2_tensor);
            auto inp3_tensor = ov::test::utils::create_and_fill_tensor(model_type,
                                                                       ov::Shape{inputShape.second.front().at(1)},
                                                                       maxBeamIndex);
...
INSTANTIATE_TEST_SUITE_P(smoke_gathertree_parameter_compareWithRefs_dynamic,
                         GatherTreeLayerGPUTest,
...
INSTANTIATE_TEST_SUITE_P(smoke_gathertree_constant_compareWithRefs_dynamic,
                         GatherTreeLayerGPUTest,
