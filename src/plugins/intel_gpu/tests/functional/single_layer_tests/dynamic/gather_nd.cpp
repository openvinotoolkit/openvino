...
typedef std::tuple<GatherNDShapeParams,
...
protected:
    void SetUp() override {
        ...
        if (isIndicesConstant) {
            ...
            auto indices_tensor = ov::test::utils::create_and_fill_tensor(ov::element::i64,
        ...
        gather_ndNode =
            std::make_shared<ov::op::v8::GatherND>(params[0], isIndicesConstant ? indicesNode : params[1], batchDims);
...
INSTANTIATE_TEST_SUITE_P(smoke_dynamic_input_shapes_const_target_shapes,
                         GatherNDGPUTest,
                         ::testing::Combine(::testing::ValuesIn(dynamicInputShapeConstTargetShape),  // input shapes
