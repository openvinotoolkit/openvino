...
using basicGPUMvnParams = std::tuple<InputShape,         // Input shapes
                                     ov::element::Type,  // Input precision
                                     std::vector<int>,   // Reduction axes
                                     bool,               // Normalize variance
                                     double>;            // Epsilon

using MvnLayerGPUTestParamSet = std::tuple<basicGPUMvnParams,
...
const std::vector<InputShape> inputShapes_1D = {{// dynamic
                                                 {{1, 20}},
                                                 // target
                                                 {{1}, {16}, {4}, {16}}}};

const std::vector<InputShape> inputShapes_3D = {{// dynamic
                                                 {-1, -1, -1},
                                                 // target
...
const std::vector<InputShape> inputShapes_4D = {{// dynamic
                                                 {-1, -1, -1, -1},
                                                 // target
...
const auto Mvn4D = ::testing::Combine(
    ::testing::Combine(
        ...
        ::testing::Values(ov::element::f32),
        ::testing::ValuesIn(
            {reduction_axes_2, reduction_axes_3, reduction_axes_12, reduction_axes_23, reduction_axes_123}),
        ::testing::ValuesIn(normalizeVariance),
        ::testing::ValuesIn(epsilon)),
    ::testing::ValuesIn(inpPrc));
