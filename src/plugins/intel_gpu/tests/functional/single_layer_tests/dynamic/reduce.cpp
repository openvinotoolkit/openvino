...
typedef std::tuple<ReduceInput,                     // input data (data shape, axes shape, axes values)
...
const ReduceInput dyn1d = {{{{-1}, {{4}, {5}}}}, {0}};

const ReduceInput dyn2d = {{{{-1, -1}, {{100, 3}, {5, 6}}}}, {1}};

const ReduceInput dyn3d = {{{{-1, -1, -1}, {{4, 5, 6}, {5, 1, 6}}}}, {0, -1}};

const ReduceInput dyn4d = {{{{-1, -1, -1, -1}, {{2, 3, 4, 5}, {5, 4, 3, 1}}}}, {1, -2}};

const ReduceInput dyn5d = {{{{-1, -1, -1, -1, -1}, {{2, 3, 4, 5, 6}, {5, 6, 3, 1, 2}}}}, {-3, 3}};

const ReduceInput dyn6d = {{{{-1, -1, -1, -1, -1, -1}, {{2, 3, 4, 5, 6, 7}, {5, 4, 3, 1, 2, 6}}}}, {1}};

// ================== Reduction int32/float types (Sum, Min, Max, L1) ==================
const auto reduceSum = ::testing::Combine(::testing::ValuesIn({dyn1d, dyn5d}),
                                          ::testing::ValuesIn(float_int_types),
                                          ::testing::Values(ov::test::utils::ReductionType::Sum),
                                          ::testing::ValuesIn(keepDims),
                                          ::testing::Values(ov::test::utils::DEVICE_GPU));
INSTANTIATE_TEST_SUITE_P(smoke_reduce_sum_compareWithRefs_dynamic,
                         ReduceLayerGPUTest,
                         reduceSum,
                         ReduceLayerGPUTest::getTestCaseName);

const auto reduceMin = ::testing::Combine(::testing::ValuesIn({dyn2d, dyn6d}),
                                          ::testing::ValuesIn(float_int_types),
                                          ::testing::Values(ov::test::utils::ReductionType::Min),
                                          ::testing::ValuesIn(keepDims),
                                          ::testing::Values(ov::test::utils::DEVICE_GPU));
INSTANTIATE_TEST_SUITE_P(smoke_reduce_min_compareWithRefs_dynamic,
                         ReduceLayerGPUTest,
                         reduceMin,
                         ReduceLayerGPUTest::getTestCaseName);

const auto reduceMax = ::testing::Combine(::testing::ValuesIn({dyn3d, dyn5d}),
                                          ::testing::ValuesIn(float_int_types),
                                          ::testing::Values(ov::test::utils::ReductionType::Max),
                                          ::testing::ValuesIn(keepDims),
                                          ::testing::Values(ov::test::utils::DEVICE_GPU));
INSTANTIATE_TEST_SUITE_P(smoke_reduce_max_compareWithRefs_dynamic,
                         ReduceLayerGPUTest,
                         reduceMax,
                         ReduceLayerGPUTest::getTestCaseName);

const auto reduceL1 = ::testing::Combine(::testing::ValuesIn({dyn4d, dyn6d}),
                                         ::testing::ValuesIn(float_int_types),
                                         ::testing::Values(ov::test::utils::ReductionType::L1),
                                         ::testing::ValuesIn(keepDims),
                                         ::testing::Values(ov::test::utils::DEVICE_GPU));
INSTANTIATE_TEST_SUITE_P(smoke_reduce_l1_compareWithRefs_dynamic,
                         ReduceLayerGPUTest,
                         reduceL1,
                         ReduceLayerGPUTest::getTestCaseName);

// ================== Reduction float types (Mean, Prod, L2) ==================
const auto reduceMean = ::testing::Combine(::testing::ValuesIn({dyn1d, dyn6d}),
                                           ::testing::ValuesIn(float_types),
                                           ::testing::Values(ov::test::utils::ReductionType::Mean),
                                           ::testing::ValuesIn(keepDims),
                                           ::testing::Values(ov::test::utils::DEVICE_GPU));
INSTANTIATE_TEST_SUITE_P(smoke_reduce_mean_compareWithRefs_dynamic,
                         ReduceLayerGPUTest,
                         reduceMean,
                         ReduceLayerGPUTest::getTestCaseName);

const auto reduceProd = ::testing::Combine(::testing::ValuesIn({dyn2d, dyn4d}),
...
const auto reduceL2 = ::testing::Combine(::testing::ValuesIn({dyn4d, dyn5d}),
                                         ::testing::ValuesIn(float_types),
                                         ::testing::Values(ov::test::utils::ReductionType::L2),
                                         ::testing::ValuesIn(keepDims),
                                         ::testing::Values(ov::test::utils::DEVICE_GPU));
INSTANTIATE_TEST_SUITE_P(smoke_reduce_l2_compareWithRefs_dynamic,
                         ReduceLayerGPUTest,
                         reduceL2,
                         ReduceLayerGPUTest::getTestCaseName);

// ================== Reduction logical types (LogicalOr, LogicalAnd) ==================
const auto reduceLogicalOr = ::testing::Combine(::testing::ValuesIn({dyn1d, dyn6d}),
                                                ::testing::Values(ov::element::boolean),
                                                ::testing::Values(ov::test::utils::ReductionType::LogicalOr),
                                                ::testing::ValuesIn(keepDims),
                                                ::testing::Values(ov::test::utils::DEVICE_GPU));
INSTANTIATE_TEST_SUITE_P(smoke_reduce_logicalor_compareWithRefs_dynamic,
                         ReduceLayerGPUTest,
                         reduceLogicalOr,
                         ReduceLayerGPUTest::getTestCaseName);

const auto reduceLogicalAnd = ::testing::Combine(::testing::ValuesIn({dyn3d, dyn5d}),
                                                 ::testing::Values(ov::element::boolean),
                                                 ::testing::Values(ov::test::utils::ReductionType::LogicalAnd),
                                                 ::testing::ValuesIn(keepDims),
                                                 ::testing::Values(ov::test::utils::DEVICE_GPU));
INSTANTIATE_TEST_SUITE_P(smoke_reduce_logicaland_compareWithRefs_dynamic,
                         ReduceLayerGPUTest,
                         reduceLogicalAnd,
                         ReduceLayerGPUTest::getTestCaseName);
...
const std::vector<ReduceInput> dynVariousAxisInputs = {
    // 4D
    {{{{-1, -1, -1, -1}, {{2, 3, 4, 5}, {5, 4, 3, 1}}}}, {0}},
    {{{{-1, -1, -1, -1}, {{2, 3, 4, 5}, {5, 4, 3, 1}}}}, {1, -1}},
    {{{{-1, -1, -1, -1}, {{2, 3, 4, 5}, {5, 3, 7, 1}}}}, {2, 3}},
    {{{{-1, -1, -1, -1}, {{2, 3, 4, 5}, {1, 2, 3, 1}}}}, {0, 2, -1}},
    // 5D
    {{{{-1, -1, -1, -1, -1}, {{2, 4, 3, 4, 5}, {5, 3, 2, 1, 2}}}}, {1}},
    {{{{-1, -1, -1, -1, -1}, {{4, 3, 2, 5, 6}, {5, 3, 2, 1, 4}}}}, {0, -3}},
    {{{{-1, -1, -1, -1, -1}, {{3, 4, 2, 6, 5}, {3, 5, 7, 1, 5}}}}, {2, -2, 4}},
    {{{{-1, -1, -1, -1, -1}, {{4, 2, 5, 1, 9}, {5, 3, 7, 1, 2}}}}, {0, 1, -2, 4}},
    // 6D
    {{{{-1, -1, -1, -1, -1, -1}, {{2, 3, 4, 5, 6, 7}, {5, 3, 4, 1, 7, 5}}}}, {0}},
    {{{{-1, -1, -1, -1, -1, -1}, {{2, 3, 4, 5, 6, 7}, {5, 3, 5, 1, 2, 5}}}}, {0, -3}},
    {{{{-1, -1, -1, -1, -1, -1}, {{2, 3, 4, 5, 6, 7}, {2, 5, 4, 1, 5, 3}}}}, {2, 3, -2, 5}},
    {{{{-1, -1, -1, -1, -1, -1}, {{2, 3, 4, 5, 6, 7}, {3, 5, 4, 1, 8, 5}}}}, {0, 2, -3, 4, 5}},
    {{{{-1, -1, -1, -1, -1, -1}, {{2, 3, 4, 5, 6, 7}, {7, 5, 3, 1, 6, 9}}}}, {4}},
};

const auto reduceMaxWithVariousAxis = ::testing::Combine(::testing::ValuesIn(dynVariousAxisInputs),
