...
enum class priorbox_type { V0, V8, Clustered };
typedef std::tuple<InputShape, InputShape, ov::element::Type, std::vector<float>, priorbox_type>
...
protected:
    void SetUp() override {
        ...
        case priorbox_type::Clustered: {
            ...
            auto priorBoxOp =
                std::make_shared<ov::op::v0::PriorBoxClustered>(stridedSliceOp1, stridedSliceOp2, attributes_clustered);
...
std::vector<ov::test::InputShape> inShapesDynamic = {
    {{1, 3, -1, -1}, {{1, 3, 30, 30}, {1, 3, 20, 20}, {1, 3, 40, 40}}},
};
std::vector<ov::test::InputShape> imgShapesDynamic = {
    {{1, 3, -1, -1}, {{1, 3, 224, 224}, {1, 3, 300, 300}, {1, 3, 200, 200}}},
};

std::vector<std::vector<float>> max_size = {{}, {300}};
