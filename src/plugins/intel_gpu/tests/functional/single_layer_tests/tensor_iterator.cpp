...
namespace {
using TensorIteratorWithConfigParams = typename std::tuple<size_t,  // seq_lengths
                                                           size_t,  // batch
                                                           size_t,  // hidden size
                                                           // todo: fix. input size hardcoded to 10 due to limitation
                                                           // (10 args) of gtests Combine() func. size_t, // input size
...
public:
    static std::string getTestCaseName(const testing::TestParamInfo<TensorIteratorWithConfigParams>& obj) {
        ...
        std::tie(seq_lengths, batch, hidden_size, sequence_axis, clip, ti_body, direction, model_type, target_device) =
            obj.param;
...
protected:
    void SetUp() override {
        ...
        ov::op::RecurrentSequenceDirection direction;
        ov::element::Type model_type;
        std::tie(seq_lengths, batch, hidden_size, sequence_axis, clip, ti_body, direction, model_type, targetDevice) =
            this->GetParam();
        ...
        case ov::test::utils::TensorIteratorBody::LSTM: {
            ...
            ov::ParameterVector outer_params{
                std::make_shared<ov::op::v0::Parameter>(model_type, ov::Shape(inputShapes[0])),
...
INSTANTIATE_TEST_SUITE_P(smoke_TensorIteratorCommon,
                         TensorIteratorWithConfigTest,
                         ::testing::Combine(::testing::ValuesIn(std::vector<size_t>{2, 4}),  // seq lengths
