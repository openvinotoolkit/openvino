...
protected:
    ...
    void SetUp() override {
        ...
        HRESULT err = CreateDXGIFactory(__uuidof(IDXGIFactory), reinterpret_cast<void**>(&out_factory));
        if (FAILED(err)) {
    ...
    std::tuple<CComPtr<ID3D11Device>, CComPtr<ID3D11DeviceContext>> create_device_with_ctx(
        ...
        HRESULT err = D3D11CreateDevice(adapter,
                                        ...
                                        &ret_ctx_ptr);
        if (FAILED(err)) {
            throw std::runtime_error("Cannot create D3D11CreateDevice, error: " + std::to_string(HRESULT_CODE(err)));
        }
...
struct DX11CachedTexture_Test : DX11RemoteCtx_Test {
    ...
    void SetUp() override {
        ...
        ASSERT_NO_THROW(std::tie(device_ptr, ctx_ptr) = create_device_with_ctx(*intel_adapters.begin()));
...
#        endif
        ...
        auto fn_ptr_remote =
            ov::test::utils::make_conv_pool_relu({1, 3, texture_description.Height, texture_description.Width});
        ov::Core core;
        ...
        for (size_t i = 0; i < iteration_count; i++) {
            auto tensor =
                context.create_tensor_nv12(texture_description.Height, texture_description.Width, dx11_textures[0]);
...
#        endif
    ...
    ASSERT_NO_THROW(std::tie(device_ptr, ctx_ptr) = create_device_with_ctx(intel_adapters[0]));
    ...
    for (auto adapter : other_adapters) {
        ...
        ASSERT_NO_THROW(std::tie(device_ptr, ctx_ptr) = create_device_with_ctx(adapter));
        ASSERT_THROW(ov::intel_gpu::ocl::D3DContext gpu_context(core, device_ptr), std::runtime_error);
    }
}
...
#        endif
    ...
    for (size_t i = 0; i < total_run_number; i++) {
        for (const auto& t : dx11_textures) {
            ASSERT_NO_THROW(auto tensor =
                                context.create_tensor_nv12(texture_description.Height, texture_description.Width, t));
        }
    }
}
