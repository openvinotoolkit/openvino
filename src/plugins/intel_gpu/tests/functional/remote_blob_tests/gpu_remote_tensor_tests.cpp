...
public:
    ...
    void run_smoke_canInferOnUserQueue_out_of_order(bool is_caching_test) {
        ...
        // compare results
        {
            ...
            auto thr = ov::test::utils::GetComparisonThreshold(InferenceEngine::Precision::FP32);
            ASSERT_NO_THROW(output_tensor_regular.data());
            ov::test::utils::compare_tensor(output_tensor_regular, out_tensor, thr);
        }
...
TEST_F(OVRemoteTensor_Test, NV12toGray) {
    ...
    ASSERT_NO_THROW(output_tensor_shared.data());
    float thr = 0.1f;
    ov::test::utils::compare_tensor(output_tensor_shared, output_tensor_regular, thr);
}
...
TEST_F(OVRemoteTensor_Test, NV12toBGR_buffer) {
    ...
    float thr = 0.1f;
    ov::test::utils::compare_tensor(out_tensor, output_tensor_regular, thr);
}
...
TEST_P(OVRemoteTensorBatched_Test, NV12toBGR_image_single_plane) {
    ...
    for (size_t i = 0; i < num_batch; i++) {
        fake_image_data_yuv.push_back(
            ov::test::utils::create_and_fill_tensor(ov::element::u8, {1, 1, height * 3 / 2, width}, 50, 0, 1, static_cast<int32_t>(i)));
    }
    ...
    for (size_t i = 0; i < num_batch; ++i) {
        ...
        ov::test::utils::compareRawBuffers<float>(static_cast<float*>(output_tensor_shared.data()) + i * output_tensor_regular.get_size(),
...
TEST_P(OVRemoteTensorBatched_Test, NV12toBGR_image_two_planes) {
    ...
    for (size_t i = 0; i < num_batch; i++) {
        ...
        fake_image_data_uv.push_back(
            ov::test::utils::create_and_fill_tensor(ov::element::u8, {1, 2, height / 2, width / 2}, 256, 0, 1, static_cast<int32_t>(i)));
    }
...
TEST_P(OVRemoteTensorBatched_Test, NV12toBGR_buffer) {
    ...
    for (size_t i = 0; i < num_batch * 2; ++i) {
        fake_image_data_y.push_back(ov::test::utils::create_and_fill_tensor(ov::element::u8, {1, 1, height, width}, 50, 0, 1, static_cast<int32_t>(i)));
        fake_image_data_uv.push_back(ov::test::utils::create_and_fill_tensor(ov::element::u8, {1, 2, height / 2, width / 2}, 256, 0, 1, static_cast<int32_t>(i)));
    }
    ...
    for (size_t i = 0; i < num_batch; ++i) {
        ...
        ov::test::utils::compareRawBuffers<float>(static_cast<float*>(out_tensor_new.data()) + i * output_tensor_regular.get_size(),
                                                static_cast<float*>(output_tensor_regular.data()),
                                                output_tensor_regular.get_size(), output_tensor_regular.get_size(), thr);
    }
}
