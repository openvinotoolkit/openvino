...
void set_device_suffix(const std::string& suffix) {
    ...
    new_gpu_name =
        std::string(ov::test::utils::DEVICE_GPU) + std::string(1, ov::test::utils::DEVICE_SUFFIX_SEPARATOR) + suffix;
    ...
    if (std::find(available_devices.begin(), available_devices.end(), new_gpu_name) == available_devices.end()) {
        throw std::runtime_error("The device " + new_gpu_name +
                                 " is not in the available devices! Please use other on!");
    }
