...
std::string NmsRotatedLayerTestGPU::getTestCaseName(const testing::TestParamInfo<NmsRotatedParamsGPU>& obj) {
    ...
    result << "_ConstIn={" << utils::bool2str(std::get<10>(obj.param)) << ","
           << utils::bool2str(std::get<11>(obj.param)) << "," << utils::bool2str(std::get<12>(obj.param)) << ","
           << utils::bool2str(std::get<13>(obj.param)) << "," << utils::bool2str(std::get<14>(obj.param)) << "}";
...
#define CONST_CASE(P, S, H, L)                                                                                \
    case P:                                                                                                   \
        inputs.push_back(                                                                                     \
            ov::test::utils::deprecated::make_constant(P,                                                     \
                                                       S,                                                     \
                                                       std::vector<ov::element_type_traits<P>::value_type>{}, \
                                                       true,                                                  \
                                                       ov::element_type_traits<P>::value_type(H),             \
                                                       ov::element_type_traits<P>::value_type(L)));           \
        break;
...
#define CREATE_INPUT(C, P, S, N, H, L)                                                              \
    if (C) {                                                                                        \
        ...
        default:                                                                                    \
            OPENVINO_THROW("NmsRotated does not support precision ", P, " for the ", N, " input."); \
        }                                                                                           \
    } else {                                                                                        \
    ...
    CREATE_INPUT(is_2_in_const,
                 max_boxes_prc,
                 in_shape_1d,
                 "MaxOutputBoxesPerClass",
                 m_max_out_boxes_per_class,
                 m_max_out_boxes_per_class)
...
#undef CREATE_INPUT
    ...
    const auto nms_op = std::make_shared<ov::op::v13::NMSRotated>(inputs[0],
                                                                  inputs[1],
                                                                  inputs[2],
                                                                  inputs[3],
                                                                  inputs[4],
...
void NmsRotatedLayerTestGPU::compare(const std::vector<ov::Tensor>& expectedOutputs,
...
#define GEN_DATA(P, R, S, K)                                                           \
    case P:                                                                            \
        utils::fill_data_random(tensor.data<ov::element_type_traits<P>::value_type>(), \
                                shape_size(targetInputStaticShapes[i]),                \
                                R,                                                     \
                                S,                                                     \
                                K);                                                    \
        break;
        ...
        if (name == "Boxes") {
            ...
            default:
                OPENVINO_THROW("NmsRotated does not support precision ",
                               in_prc,
                               " for the MaxOutputBoxesPerClass input.");
            }
...
namespace {
const std::vector<std::vector<ov::test::InputShape>> inShapeParams = {{{{}, {{2, 50, 5}}}, {{}, {{2, 50, 50}}}},
                                                                      {{{}, {{9, 10, 5}}}, {{}, {{9, 10, 10}}}}};
