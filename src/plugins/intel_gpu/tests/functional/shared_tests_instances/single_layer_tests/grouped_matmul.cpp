...
const std::string GPU = ov::test::utils::DEVICE_GPU;
...
const std::vector<GroupedMatMulShapeParams> shapes_2d_3d = {
    ...
    {{ov::PartialShape{-1, 32}, {{12, 32}, {20, 32}}},
     {4, 16, 32},
     TokensPerExpert{{4, 5, 3, 0}, {0, 7, 6, 7}}},
    {{ov::PartialShape{-1, 64}, {{16, 64}, {32, 64}}},
     {4, 32, 64},
     TokensPerExpert{{7, 3, 5, 1}, {2, 10, 8, 12}}},
};
...
// ---- Plain-weights instantiations ----

INSTANTIATE_TEST_SUITE_P(smoke_GroupedMatMul_3Dx3D, GroupedMatMulLayerTest,
    ::testing::Combine(
        ::testing::ValuesIn(shapes_3d_3d),
        ::testing::Values(ov::element::f16),
        ::testing::Values(GPU)),
    GroupedMatMulLayerTest::getTestCaseName);

INSTANTIATE_TEST_SUITE_P(smoke_GroupedMatMul_2Dx3D, GroupedMatMulLayerTest,
    ::testing::Combine(
        ::testing::ValuesIn(shapes_2d_3d),
        ::testing::Values(ov::element::f16),
        ::testing::Values(GPU)),
    GroupedMatMulLayerTest::getTestCaseName);

// ---- Compressed-weights instantiations ----
// After decompression B is f16 → GroupedMatMul sees f16 weights (GPU requirement).

INSTANTIATE_TEST_SUITE_P(smoke_GroupedMatMul_Compressed_3Dx3D, GroupedMatMulCompressedLayerTest,
    ::testing::Combine(
        ::testing::ValuesIn(shapes_3d_3d),
...
INSTANTIATE_TEST_SUITE_P(smoke_GroupedMatMul_Compressed_2Dx3D, GroupedMatMulCompressedLayerTest,
    ::testing::Combine(
        ::testing::ValuesIn(shapes_2d_3d),
        ::testing::Values(ov::element::f16),
