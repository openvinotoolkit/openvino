...
INSTANTIATE_TEST_SUITE_P(
    smoke_TestsROIAlign_average,
    ROIAlignLayerTest,
    ::testing::Combine(::testing::ValuesIn(ov::test::static_shapes_to_test_representation(
                           std::vector<std::vector<ov::Shape>>{{{3, 8, 16, 16}}, {{2, 1, 16, 16}}, {{2, 1, 8, 16}}})),
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ROIAlignLayerTest,
    ::testing::Combine(::testing::ValuesIn(ov::test::static_shapes_to_test_representation(
                           std::vector<std::vector<ov::Shape>>{{{2, 8, 20, 20}}, {{2, 1, 20, 20}}, {{2, 1, 10, 20}}})),
                       ::testing::Values(std::vector<size_t>{2, 4}),
                       ::testing::Values(2),
                       ::testing::Values(2),
                       ::testing::ValuesIn(std::vector<float>{1, 0.625}),
...
INSTANTIATE_TEST_SUITE_P(
    smoke_TestsROIAlign_avg_asym,
    ROIAlignV9LayerTest,
    ::testing::Combine(
        ::testing::ValuesIn(ov::test::static_shapes_to_test_representation(
            std::vector<std::vector<ov::Shape>>{{{2, 1, 8, 8}}, {{2, 8, 20, 20}}, {{2, 1, 20, 20}}, {{2, 1, 10, 20}}})),
