...
const std::vector<ov::element::Type> netPrecisions = {ov::element::f32, ov::element::f16, ov::element::i64};
...
static std::vector<reshapeParams> generate_tests() {
    ...
    for (auto& p : params) {
        reshapeParams test_case =
            std::make_tuple(false, ov::element::f16, p.first, p.second, ov::test::utils::DEVICE_GPU);
...
INSTANTIATE_TEST_SUITE_P(smoke_ReshapeTests,
                         ReshapeLayerTest,
