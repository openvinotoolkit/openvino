...
#define CASE(X, Y, _expected_offset, _actual_offset, _size, _threshold)                              \
    case X:                                                                                          \
        ov::test::utils::compare_raw_data(                                                           \
            ...
            reinterpret_cast<const ov::fundamental_type_for<Y>*>(actualBuffer) + _actual_offset,     \
            _size,                                                                                   \
            _threshold);                                                                             \
        break;
...
void MatrixNmsLayerTestGPU::SetUp() {
    ...
    std::tie(shapes,
             model_type,
             m_attrs.sort_result_type,
             m_attrs.output_type,
             top_k_params,
             threshold_params,
             m_attrs.background_class,
             m_attrs.normalized,
             m_attrs.decay_function,
...
const std::vector<ov::op::v8::MatrixNms::SortResultType> sortResultType = {
