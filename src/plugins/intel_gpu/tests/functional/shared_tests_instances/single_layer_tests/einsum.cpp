...
const std::vector<ov::test::EinsumEquationWithInput> equationsWithInput = {
    ...
    {"ab,bcd,bc->ca",
     ov::test::static_shapes_to_test_representation({{2, 4}, {4, 3, 1}, {4, 3}})},  // multiple multiplications
    {"kii->ki", ov::test::static_shapes_to_test_representation({{1, 3, 3}})},       // diagonal
    {"abbac,bad->ad",
     ov::test::static_shapes_to_test_representation(
         {{2, 3, 3, 2, 4}, {3, 2, 1}})},  // diagonal and multiplication with repeated labels
    ...
    {"a...j,j...->a...",
     ov::test::static_shapes_to_test_representation({{1, 1, 4, 3}, {3, 4, 2, 1}})}  // complex multiplication
};
