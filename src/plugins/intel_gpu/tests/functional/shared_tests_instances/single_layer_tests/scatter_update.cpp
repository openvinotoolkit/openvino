...
const std::vector<std::vector<int64_t>> idxValue = {{0, 2, 4, 6, 1, 3, 5, 7}};
    ...
    for (auto& input_shape : input_shapes) {
        ...
        for (auto& item : input_shape.second) {
            ...
            for (auto& axis : item.second) {
                ...
                res_vec.push_back(
                    ov::test::axisUpdateShapeInShape{ov::test::static_shapes_to_test_representation(in_shapes),
...
INSTANTIATE_TEST_SUITE_P(smoke_ScatterUpdate,
