...
TEST_F(U2_IR_Serialization, RoundTrip) {
    // Model structure: Input [1,1,256] -> MatMul with u2 decompressed weights [256,128] -> Output [1,1,128]
    const size_t M = 256;
    const size_t N = 128;
    ...
    // Create u2 decompression subgraph using canonical OpenVINO infrastructure
    // This matches exactly what production models use (NNCF, Model Optimizer, etc.)
    const auto weights_subgraph = ov::test::utils::initMatMulDecompressionSubgraph(
        Shape{M, N},                           // weights_shape
        -1,                                    // group_size (-1 = no group quantization)
        element::f32,                          // data_precision
        element::u2,                           // weights_precision
        element::f32,                          // decompression_precision
        element::dynamic,                      // scale_precision
        false,                                 // transpose_weights
        ov::test::utils::DecompressionType::full,     // decompression_multiply_type
        ov::test::utils::DecompressionType::empty,    // decompression_subtract_type (no zero-point for simplicity)
        false                                  // reshape_on_decompression_constant
    );

    // Create MatMul with decompressed weights
    ...
    for (size_t i = 0; i < M; i++) {
        input_data[i] = static_cast<float>(i % 10) / 10.0f;  // 0.0, 0.1, ..., 0.9, repeat
    }
    ...
    // === BINARY SIZE VALIDATION (Three-level check) ===
    size_t theoretical_payload = (num_weights * 2 + 7) / 8;  // Bits to bytes, rounded up (u2 weights only)
    
    // Account for decompression constants:
    // - Scale: f32, shape [M, N] = 256*128*4 bytes = 131072 bytes
    // However, scales are typically per-channel or per-tensor, not per-weight
    // For full decompression: scales shape is [OC, 1] = [128, 1] = 128 * 4 = 512 bytes
    size_t scale_size = N * sizeof(float);  // Per-output-channel scales
    ...
    // === RUNTIME INSPECTION (Diagnostic only, non-asserting) ===
    try {
        ...
        // Log operation types (informational)
