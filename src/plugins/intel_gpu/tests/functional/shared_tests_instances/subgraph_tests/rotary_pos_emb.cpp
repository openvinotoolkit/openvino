...
class RoPEGPUTestLlama2 : public SubgraphBaseTest {
public:
...
class RoPEGPUTestChatGLM : public SubgraphBaseTest {
public:
    ov::Tensor create_i32_tensor(const ov::Shape& shape, int start, int step = 1) {
        auto tensor = ov::Tensor(ov::element::i32, shape);
        auto* ptr = static_cast<int32_t*>(tensor.data());
...
class RoPEGPUTestQwen7b : public SubgraphBaseTest {
public:
    void generate_inputs(const std::vector<ov::Shape>& targetInputStaticShapes) override {
        const auto& funcInputs = function->inputs();
...
class RoPEGPUTestGPTJ : public SubgraphBaseTest, public testing::WithParamInterface<bool> {
public:
...
INSTANTIATE_TEST_SUITE_P(smoke_RoPEGPUTestGPTJ,
                         RoPEGPUTestGPTJ,
                         ::testing::Values(true, false),
                         RoPEGPUTestGPTJ::getTestCaseName);
