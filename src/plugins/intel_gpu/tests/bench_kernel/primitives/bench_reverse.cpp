...
class bench_reverse : public kernel_base {
public:
    std::string name() const override { return "reverse"; }

    void run_single(cldnn::engine& engine, const bench_config& config) override {
        auto shapes = parse_shapes(config.shapes_str);
        if (shapes.empty()) throw std::runtime_error("reverse requires at least 1 shape. Got: " + config.shapes_str);

        auto dt = config.data_types.empty() ? cldnn::data_types::f16 : config.data_types[0];
        auto mode = config.reverse_mode == 0 ? cldnn::reverse_mode::index : cldnn::reverse_mode::mask;

        // Parse axes from reduce_axes config (reuse)
        std::vector<int32_t> axes;
        if (!config.reduce_axes.empty()) {
            auto av = parse_colon_vec(config.reduce_axes);
            for (auto v : av) axes.push_back(static_cast<int32_t>(v));
        }
        if (axes.empty()) axes = {0};  // default: reverse first axis
        ...
        // Create axes as data input
        ov::PartialShape axes_ps({static_cast<int64_t>(axes.size())});
        cldnn::layout axes_layout(axes_ps, cldnn::data_types::i32, cldnn::format::bfyx);
        auto axes_mem = engine.allocate_memory(axes_layout);
        {
            cldnn::mem_lock<int32_t> lock(axes_mem, *stream);
            for (size_t i = 0; i < axes.size(); ++i) lock[i] = axes[i];
        }

        cldnn::topology topology;
        topology.add(cldnn::input_layout("input", input_layout));
        topology.add(cldnn::data("axes", axes_mem));
        topology.add(cldnn::reverse("prim", cldnn::input_info("input"), cldnn::input_info("axes"), mode));
        ...
        if (config.is_acc()) {
            ...
            auto ref_out = ref::reverse_data(input_f32, shapes[0], axes);
            float atol, rtol;
