...
class bench_multiclass_nms : public kernel_base {
public:
    std::string name() const override { return "multiclass_nms"; }

    void run_single(cldnn::engine& engine, const bench_config& config) override {
        // shapes: boxes[N,M,4] : scores[N,C,M]
        auto shapes = parse_shapes(config.shapes_str);
        if (shapes.size() < 2) throw std::runtime_error("multiclass_nms requires 2 shapes (boxes:scores). Got: " + config.shapes_str);
        ...
        attrs.iou_threshold = config.nms_iou_threshold;
        ...
        topology.add(cldnn::multiclass_nms("prim", inputs, attrs));
...
REGISTER_KERNEL(bench_multiclass_nms)
