...
class bench_search_sorted : public kernel_base {
public:
    std::string name() const override { return "search_sorted"; }

    void run_single(cldnn::engine& engine, const bench_config& config) override {
        auto shapes = parse_shapes(config.shapes_str);
        if (shapes.size() < 2) throw std::runtime_error("search_sorted requires 2 shapes (sorted:values). Got: " + config.shapes_str);
        ...
        auto sorted_mem = engine.allocate_memory(lay0);
        auto values_mem = engine.allocate_memory(lay1);
        fill_memory_random(sorted_mem, *stream, dt);
        fill_memory_random(values_mem, *stream, dt);

        cldnn::topology topology;
        topology.add(cldnn::input_layout("sorted", lay0));
        topology.add(cldnn::input_layout("values", lay1));
        topology.add(cldnn::search_sorted("prim",
            cldnn::input_info("sorted"), cldnn::input_info("values"), false));

        cldnn::network network(engine, topology, exec_config);
        network.set_input_data("sorted", sorted_mem);
        ...
        if (config.is_acc()) {
            ...
            auto sorted_f32 = read_memory_to_f32(sorted_mem, *stream);
            auto values_f32 = read_memory_to_f32(values_mem, *stream);
            auto ref_out = ref::search_sorted(sorted_f32, values_f32, shapes[0], shapes[1], false);
...
REGISTER_KERNEL(bench_search_sorted)
