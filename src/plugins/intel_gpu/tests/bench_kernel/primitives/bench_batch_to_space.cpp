...
class bench_batch_to_space : public kernel_base {
public:
    std::string name() const override { return "batch_to_space"; }

    void run_single(cldnn::engine& engine, const bench_config& config) override {
        // shapes: input:block_shape:crops_begin:crops_end
        auto shapes = parse_shapes(config.shapes_str);
        if (shapes.size() < 4) throw std::runtime_error("batch_to_space requires 4 shapes (input:block:crops_begin:crops_end). Got: " + config.shapes_str);
        ...
        // Convert block/crops to tensors
        ...
        cldnn::tensor crops_begin_t = to_tensor(shapes[2]);
        cldnn::tensor crops_end_t = to_tensor(shapes[3]);
        ...
        for (size_t i = 0; i < in_s.size(); ++i) {
            if (i == 0) out_s[i] = in_s[i] / (i < block_s.size() ? block_s[i] : 1);
            else out_s[i] = in_s[i] * (i < block_s.size() ? block_s[i] : 1) - (i < cb.size() ? cb[i] : 0) - (i < ce.size() ? ce[i] : 0);
        }
        cldnn::tensor out_t = to_tensor(out_s);

        cldnn::topology topology;
        topology.add(cldnn::input_layout("input", input_layout));
        topology.add(cldnn::batch_to_space("prim", cldnn::input_info("input"),
                     block_t, crops_begin_t, crops_end_t, out_t));
        ...
        if (config.is_acc()) {
            ...
            auto ref_out = ref::batch_to_space(input_f32, shapes[0], shapes[1], shapes[2], shapes[3]);
            float atol, rtol;
...
REGISTER_KERNEL(bench_batch_to_space)
