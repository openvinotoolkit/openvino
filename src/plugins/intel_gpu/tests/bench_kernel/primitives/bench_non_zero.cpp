...
class bench_non_zero : public kernel_base {
public:
    std::string name() const override { return "non_zero"; }

    void run_single(cldnn::engine& engine, const bench_config& config) override {
        auto shapes = parse_shapes(config.shapes_str);
        if (shapes.empty()) throw std::runtime_error("non_zero requires at least 1 shape. Got: " + config.shapes_str);
        ...
        topology.add(cldnn::count_nonzero("count", cldnn::input_info("input")));
        topology.add(cldnn::gather_nonzero("prim", cldnn::input_info("input"), cldnn::input_info("count")));
        ...
        if (config.is_acc()) {
            ...
            if (gpu_out.empty()) {
                ...
                auto ref_out = ref::non_zero(input_f32, shapes[0]);
...
REGISTER_KERNEL(bench_non_zero)
