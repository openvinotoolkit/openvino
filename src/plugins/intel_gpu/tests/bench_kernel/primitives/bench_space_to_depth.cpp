...
class bench_space_to_depth : public kernel_base {
public:
    std::string name() const override { return "space_to_depth"; }

    void run_single(cldnn::engine& engine, const bench_config& config) override {
        auto shapes = parse_shapes(config.shapes_str);
        if (shapes.empty()) throw std::runtime_error("space_to_depth requires at least 1 shape. Got: " + config.shapes_str);
        ...
        topology.add(cldnn::space_to_depth("prim", cldnn::input_info("input"), mode, block_size));
        ...
        if (config.is_acc()) {
            ...
            auto ref_out = ref::space_to_depth(input_f32, shapes[0], static_cast<int64_t>(block_size), blocks_first);
            float atol, rtol;
...
REGISTER_KERNEL(bench_space_to_depth)
