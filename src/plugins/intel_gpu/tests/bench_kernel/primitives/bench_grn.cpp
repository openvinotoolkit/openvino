...
class bench_grn : public kernel_base {
public:
    std::string name() const override { return "grn"; }

    void run_single(cldnn::engine& engine, const bench_config& config) override {
        auto shapes = parse_shapes(config.shapes_str);
        if (shapes.empty()) throw std::runtime_error("grn requires at least 1 shape. Got: " + config.shapes_str);

        auto dt = config.data_types.empty() ? cldnn::data_types::f16 : config.data_types[0];
        float bias = config.grn_bias;
        ...
        topology.add(cldnn::grn("prim", cldnn::input_info("input"), bias, dt));
        ...
        if (config.is_acc()) {
            ...
            auto ref_out = ref::grn(input_f32, shapes[0], bias);
            float atol, rtol;
