...
class bench_shuffle_channels : public kernel_base {
public:
    std::string name() const override { return "shuffle_channels"; }

    void run_single(cldnn::engine& engine, const bench_config& config) override {
        auto shapes = parse_shapes(config.shapes_str);
        if (shapes.empty()) throw std::runtime_error("shuffle_channels requires at least 1 shape. Got: " + config.shapes_str);

        auto dt = config.data_types.empty() ? cldnn::data_types::f16 : config.data_types[0];
        int32_t group = config.shuffle_group > 0 ? config.shuffle_group : 2;
        int32_t ax = config.axis == -1 ? 1 : config.axis;
        ...
        topology.add(cldnn::shuffle_channels("prim", cldnn::input_info("input"), group, ax));
        ...
        if (config.is_acc()) {
            ...
            auto ref_out = ref::shuffle_channels(input_f32, shapes[0], group, ax);
            float atol, rtol;
...
REGISTER_KERNEL(bench_shuffle_channels)
