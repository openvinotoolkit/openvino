...
class bench_reorg_yolo : public kernel_base {
public:
    std::string name() const override { return "reorg_yolo"; }

    void run_single(cldnn::engine& engine, const bench_config& config) override {
        auto shapes = parse_shapes(config.shapes_str);
        if (shapes.empty()) throw std::runtime_error("reorg_yolo requires at least 1 shape. Got: " + config.shapes_str);

        auto dt = config.data_types.empty() ? cldnn::data_types::f16 : config.data_types[0];
        uint32_t stride = config.block_size > 0 ? config.block_size : 2;
        ...
        topology.add(cldnn::reorg_yolo("prim", cldnn::input_info("input"), stride));
        ...
        if (config.is_acc()) {
            ...
            auto ref_out = ref::reorg_yolo(input_f32, shapes[0], static_cast<int64_t>(stride));
            float atol, rtol;
...
REGISTER_KERNEL(bench_reorg_yolo)
