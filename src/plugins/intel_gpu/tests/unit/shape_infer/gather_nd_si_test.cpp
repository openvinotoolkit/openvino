...
TEST_P(gather_nd_test, shape_infer) {
    ...
    auto gather_nd_prim = std::make_shared<gather_nd>("output",
                                                      input_info("input0"),
                                                      input_info("input1"),
                                                      input_rank,
                                                      indices_rank,
                                                      p.batch_dim,
                                                      p.batch_merged_output);
    ...
    auto res =
        gather_nd_inst::calc_output_layouts<ov::PartialShape>(gather_nd_node, *gather_nd_node.get_kernel_impl_params());
