...
TEST_P(bucketize_test, shape_infer) {
    ...
    auto input1_layout_prim =
        std::make_shared<input_layout>("input1", layout({3}, p.input_layout.data_type, format::bfyx));
    ...
    auto res =
        bucketize_inst::calc_output_layouts<ov::PartialShape>(bucketize_node, *bucketize_node.get_kernel_impl_params());
