...
TEST(dyn_shape_bounded_shape_mem, reorder) {
    ...
    auto input_lay =
        layout{ov::PartialShape{ov::Dimension(1, 10), ov::Dimension(1, 20)}, data_types::f32, format::bfyx};
...
TEST(dyn_shape_bounded_shape_mem, permute) {
    auto& engine = get_test_engine();
    auto input_lay =
        layout{ov::PartialShape{ov::Dimension(1, 10), ov::Dimension(1, 15), ov::Dimension(1, 5), ov::Dimension(1, 20)},
               data_types::f32,
               format::bfyx};
    topology topology(input_layout("input", input_lay), permute("permute", input_info("input"), {0, 2, 3, 1}));
    ExecutionConfig config = get_test_default_config(engine);
    config.set_property(ov::intel_gpu::allow_new_shape_infer(true));
    network network(engine, topology, config);
