...
TEST(memory_reuse_realloc_reset_test, basic_conv_with_padding) {
    ...
    set_values(input_mem_2, {11.0f, 11.0f, 11.0f, 11.0f, 11.0f, 11.0f, 11.0f, 11.0f, 11.0f, 11.0f, 11.0f, 11.0f});
...
TEST(softmax_gpu_dynamic_f32_test_upper_bound, input_same_values) {
    static const int32_t output_x_1 = 10, output_b_1 = 8, input_x_1 = 10, input_b_1 = 8,
                         out_size_1 = output_x_1 * output_b_1, output_x_2 = 10, output_b_2 = 4, input_x_2 = 10,
                         input_b_2 = 4, out_size_2 = output_x_2 * output_b_2, output_x_3 = 10, output_b_3 = 16,
                         input_x_3 = 10, input_b_3 = 16, out_size_3 = output_x_3 * output_b_3;
    ...
    network network(engine,
                    topology(input_layout("input", in_layout),
...
TEST(dyn_shape_mem_test, igpu_shape_infer_dep_mem_type) {
    ...
    topology topology(input_layout("input1", input_lay_1),
                      ...
                      eltwise("eltwise",
                              {input_info("pattern1"), input_info("pattern2")},
                              eltwise_mode::sum,
    ...
    if (engine.get_device_info().dev_type == device_type::integrated_gpu) {
        // for iGPU, allocating shape infer dep mem to usm_host improves shape_infer performance by preventing memcpy
        // b/w device to host mem
        ASSERT_EQ(pattern_mem.get_allocation_type(), allocation_type::usm_host);
    } else {
        // if allocate shape infer dep mem to host && write result from device && read from host, cache coherence issue
        // occurs
