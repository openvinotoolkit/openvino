...
INSTANTIATE_TEST_SUITE_P(smoke,
                         ...
                         testing::ValuesIn(std::vector<desc_stride_test_params>{
                             ...
                             {{{6, 1, 1, 8}, dnnl::memory::data_type::f16, {96, 16, 16, 1}},
                              {{0, 1, 2, 3}, {0, 2, 1, 3}}},
                             {{{1, 1, 1, 16}, dnnl::memory::data_type::f16, {32, 32, 32, 1}},
                              {{0, 1, 2, 3}, {0, 2, 1, 3}, {1, 0, 2, 3}, {2, 1, 0, 3}, {1, 2, 0, 3}, {2, 0, 1, 3}}}}),
...
public:
    static std::string PrintToString(testing::TestParamInfo<format_matching_test_params> param_info) {
        ...
        std::string res = " data format (dnnl::memory::format_tag) : " +
                          std::string(dnnl_fmt_tag2str((dnnl_format_tag_t)dnnl_format));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::ValuesIn(std::vector<format_matching_test_params>{
        ...
        {{1, 3, 16, 16, 16},
         ...
         cldnn::format::b_fs_zyx_fsv4},
        {{12, 12, 16, 16},
         dnnl::memory::data_type::f16,
         dnnl::memory::format_tag::NChw16n16c,
         cldnn::format::bs_fs_yx_bsv16_fsv16},
        {{32, 32, 16, 16},
         dnnl::memory::data_type::u8,
         dnnl::memory::format_tag::NChw32n32c,
         cldnn::format::bs_fs_yx_bsv32_fsv32},
    }),
...
INSTANTIATE_TEST_SUITE_P(
    smoke,
    data_format_test_not_match_dnnl,
    testing::ValuesIn(std::vector<format_matching_test_params>{
...
public:
    static std::string PrintToString(testing::TestParamInfo<format_matching_test_params> param_info) {
        auto dnnl_format = param_info.param.dnnl_format;
        std::string res = " weight format (dnnl::memory::format_tag) : " +
                          std::string(dnnl_fmt_tag2str((dnnl_format_tag_t)dnnl_format));
        res += " > " + format::traits(param_info.param.cldnn_format).str;
...
INSTANTIATE_TEST_SUITE_P(
    smoke,
    memory_desc_to_fmt_conversion_test,
    testing::ValuesIn(std::vector<memory_desc_to_fmt_conversion_params>{
        {{1, 3, 8, 8},
         dnnl::memory::data_type::f16,
         dnnl::memory::format_tag::abcd,
         false,
         format_traits{"custom", 1, 1, 2, 0, {0, 1, 2, 3}, "oiyx", "oixy?", {}}},
        {{16, 16, 8, 8},
         dnnl::memory::data_type::u8,
         dnnl::memory::format_tag::ABcd16b16a,
         false,
        ...
        {{8, 4, 16, 16, 16},
         ...
         false,
         format_traits{"custom", 1, 1, 3, 0, {0, 1, 2, 3, 4}, "oizyx", "oixyz", {{0, 8}, {1, 4}}}},
        {{16, 16, 8, 8},
         dnnl::memory::data_type::f16,
        ...
        {{64, 64, 8, 8},
         ...
         false,
         format_traits{"custom", 1, 1, 2, 0, {1, 0, 2, 3}, "ioyx", "oixy?", {{1, 4}, {0, 8}, {1, 8}, {0, 4}}}},
        {{32, 32, 8, 8},
         dnnl::memory::data_type::u8,
         dnnl::memory::format_tag::BAcd4b8a8b4a,
         false,  // same format as above but different sizes
         format_traits{
             "custom",
             1,
             1,
             ...
             "oiyx",
             "oixy?",
             {{1, 4}, {0, 8}, {1, 8}, {0, 4}}  // Order is different due to A=B=1 (strides are equal)
         }},
        {{65, 32, 8, 8},
         dnnl::memory::data_type::u8,
         dnnl::memory::format_tag::BAcd4b8a8b4a,
        ...
        {{10, 20, 30, 40, 50},
         ...
         dnnl::memory::format_tag::AcdeB24a2b,
         true,
         format_traits{"custom", 1, 1, 2, 1, {0, 2, 3, 4, 1}, "giyxo", "oixy????g", {{0, 24}, {1, 2}}}},
        {{10, 60, 70, 40, 50},
         dnnl::memory::data_type::f32,
         dnnl::memory::format_tag::aCBde4c8b8c4b,
         true,
         format_traits{"custom", 1, 1, 2, 1, {0, 2, 1, 3, 4}, "gioyx", "oixy????g", {{2, 4}, {1, 8}, {2, 8}, {1, 4}}}},
    }),
