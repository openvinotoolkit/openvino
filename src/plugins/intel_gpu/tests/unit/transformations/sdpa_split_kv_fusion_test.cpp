...
// op::SDPA. In the real pipeline, ov::pass::SDPASplitAttentionFusionMatcher (registered immediately
// before this pass in the GPU pipeline) already rewrites the raw MatMul/Concat/Softmax/VariadicSplit
// sub-graph into v13::SDPA(Q, Concat(K_cache,K_new), Concat(V_cache,V_new), mask) before this pass
// runs, so these tests build the raw sub-graph and run both passes in that order. Covers the
// plain-decode and GQA-folded positive cases, plus the multi-token and dynamic-shape cases that must
// be left unfused.
...
// Same sub-graph as build_split_kv_decode_model, but with a rank-2 mask [S_q, S_cache + S_new]
// broadcastable to the QK scores (legal per the SDPA spec) instead of the canonical rank-4
// [B, 1, S_q, S_cache + S_new]. The kv_len derivation only knows how to slice a rank-4 mask, so this
// must be left unfused rather than build an out-of-range Slice.
std::shared_ptr<ov::Model> build_split_kv_decode_model_with_rank2_mask(const SplitKVDims& d) {
    const auto et = ov::element::f32;

    auto Q = std::make_shared<v0::Parameter>(et, ov::PartialShape{d.B, d.H, d.S_q, d.D});
    auto Kc = std::make_shared<v0::Parameter>(et, ov::PartialShape{d.B, d.H, d.S_cache, d.D});
    auto Kn = std::make_shared<v0::Parameter>(et, ov::PartialShape{d.B, d.H, d.S_new, d.D});
    auto Vc = std::make_shared<v0::Parameter>(et, ov::PartialShape{d.B, d.H, d.S_cache, d.D});
    auto Vn = std::make_shared<v0::Parameter>(et, ov::PartialShape{d.B, d.H, d.S_new, d.D});
    auto M = std::make_shared<v0::Parameter>(et, ov::PartialShape{d.S_q, d.S_cache + d.S_new});
...
std::shared_ptr<op::SDPA> run_fusion(const std::shared_ptr<ov::Model>& model) {
    ...
    // Mirrors the real pipeline order: SDPASplitAttentionFusionMatcher fuses the raw sub-graph into
    // v13::SDPA + Concat(K)/Concat(V) first, then the GPU pass un-does the Concat for split-KV.
...
// A rank-2 mask broadcastable to the QK scores is legal per the SDPA spec, but the kv_len
// derivation only knows how to slice a rank-4 mask. The fusion must bail (not throw building an
// out-of-range Slice) and leave the sub-graph unfused.
TEST(SDPASplitKVFusionTest, NonRank4MaskNoFusion) {
    auto model = build_split_kv_decode_model_with_rank2_mask(SplitKVDims{});

    EXPECT_EQ(run_fusion(model), nullptr) << "non-rank-4 mask must not fuse into split-KV SDPA";
}
