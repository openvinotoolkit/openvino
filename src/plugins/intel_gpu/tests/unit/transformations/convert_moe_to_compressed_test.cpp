...
TEST_F(TransformationTestsF, ConvertMOEToMOE3GemmSharedExpertCompressedTest) {
    disable_rt_info_check();
    {
        ...
        auto convert_shared_gate = convert_gate->clone_with_new_inputs({reshape_gate});
        auto convert_shared_up = convert_up->clone_with_new_inputs({reshape_up});
        auto convert_shared_down = convert_down->clone_with_new_inputs({reshape_down});
        auto shared_gate_m = std::make_shared<ov::op::v0::MatMul>(hidden_states_f32, convert_shared_gate, false, true);
        auto shared_swish_m = std::make_shared<ov::op::v4::Swish>(shared_gate_m);
        auto shared_up_m = std::make_shared<ov::op::v0::MatMul>(hidden_states_f32, convert_shared_up, false, true);
        auto shared_mul_m = std::make_shared<ov::op::v1::Multiply>(shared_swish_m, shared_up_m);
        auto shared_down_m = std::make_shared<ov::op::v0::MatMul>(shared_mul_m, convert_shared_down, false, true);
        ...
        auto f16_shared_expert_gated_m = std::make_shared<ov::op::v0::Convert>(shared_expert_gated_m, element::f16);
