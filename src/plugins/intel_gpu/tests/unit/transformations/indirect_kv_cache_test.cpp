...
TEST_F(TransformationTestsF, IndirectKVCache1) {
    ...
    std::vector<int64_t> out_order = {0, 1, 2, 3};
    {
        ...
        auto new_token_param =
            std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::PartialShape{1, 32, -1, 80});
        ...
        model = std::make_shared<ov::Model>(ov::ResultVector{result},
                                            ov::ParameterVector{new_token_param, beam_idx, gemm_in});
    ...
    }
    {
        ...
        auto past = std::make_shared<ov::intel_gpu::op::ReadValue>(variable);
        auto kv_cache =
            std::make_shared<ov::intel_gpu::op::KVCache>(past, parameter, beam_idx, variable, 2, 0, ov::element::f32);
        auto gemm_in = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::PartialShape{1, 32, -1, -1});
        auto gemm = std::make_shared<ov::intel_gpu::op::IndirectGemm>(gemm_in,
                                                                      kv_cache->output(0),
                                                                      kv_cache->output(1),
                                                                      false,
                                                                      true,
        ...
        model_ref =
            std::make_shared<ov::Model>(ov::ResultVector{result}, ov::ParameterVector{parameter, beam_idx, gemm_in});
...
TEST_F(TransformationTestsF, IndirectKVCache2) {
    ...
    }
    {
        ...
        auto kv_cache =
            std::make_shared<ov::intel_gpu::op::KVCache>(past, parameter, beam_idx, variable, 2, 0, ov::element::f32);
        auto gemm_in = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::PartialShape{1, 32, -1, -1});
        auto gemm = std::make_shared<ov::intel_gpu::op::IndirectGemm>(kv_cache->output(0),
                                                                      gemm_in,
                                                                      kv_cache->output(1),
                                                                      true,
                                                                      false,
