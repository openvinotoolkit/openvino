...
std::shared_ptr<ov::Model> build_model_with_transpose_split(AttentionType attn_type) {
    ...
    // RoPE for q and k: use the real internal RoPE op, since RoPEFusion runs before
    // TransposeFusion in the GPU pipeline (see transformations_pipeline.cpp) and
    // TransposeSplitMatcher's VLSDPA-reachability check (CVS-192936) specifically
    // looks for ov::op::internal::RoPE, not a generic elementwise stand-in.
    auto cos = Constant::create(element::f16, Shape{1, num_head, head_size}, {1.0f});
    auto sin = Constant::create(element::f16, Shape{1, num_head, head_size}, {0.0f});

    auto rope_q = std::make_shared<ov::op::internal::RoPE>(OutputVector{reshape_q, cos, sin}, ov::op::internal::RoPE::Config());
    rope_q->set_friendly_name("rope_q");

    auto rope_k = std::make_shared<ov::op::internal::RoPE>(OutputVector{reshape_k, cos, sin}, ov::op::internal::RoPE::Config());
...
// Only reachable for VLSDPA: CVS-192936 locks the axis-1 rewrite down to patterns
// that provably terminate in VLSDPA (see TransposeSplitMatcher's leads_to_vlsdpa
// check), so this builder is only used for AttentionType::VLSDPA.
...
// CVS-192936: for a generic (non-VL) SDPA consumer, TransposeSplitMatcher must NOT
// rewrite the split axis (its GPU kernels don't honor the padding that rewrite
// introduces). The Transpose+Split(axis=0) structure is expected to stay untouched;
// only the independent TransposeSDPAMatcher still converts ScaledDotProductAttention
// into the GPU-internal op::SDPA.
std::shared_ptr<ov::Model> build_target_model_with_unrewritten_split_sdpa() {
    const int64_t num_head = 8;
    const int64_t head_size = 32;
    ...
    // Create GPU-specific SDPA (TransposeSDPAMatcher converts to this independently
    // of TransposeSplitMatcher)
...
TEST_P(TransposeSplitFusionTest, TransposeSplitWithAttention) {
    ...
    {
        model_ref = attn_type == AttentionType::VLSDPA ? build_target_model_with_optimized_split()
                                                        : build_target_model_with_unrewritten_split_sdpa();
    }
}
