...
template <typename T>
void test_moe_gather(bool is_caching_test) {
    ...
    network network(engine, topology, get_test_default_config(engine), is_caching_test);
...
TEST(moe_unit, moe_gather_test) {
    test_moe_gather<float>(false);
}

TEST(moe_unit, moe_gather_test_cached) {
    test_moe_gather<float>(true);
}
