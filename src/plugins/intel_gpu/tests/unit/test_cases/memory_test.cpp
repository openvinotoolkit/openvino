...
public:
    ...
    void test_shared_mem_pool_same_topology_twice() {
        ...
        auto input = engine->allocate_memory(
            {data_types::f32,
             format::bfyx,
             {tensor(spatial(inp_x_size, inp_y_size), feature(feature_num), batch(batch_num))}});
    ...
    void test_shared_mem_pool_same_topology_twice_weights() {
        ...
        std::vector<float> dummy_input_data_1 = {
            /*f0 xy*/ 0.8f,  0.65f, 0.1f,  1.0f,  1.0f,  0.5f,  0.11f, 0.33f,
            0.66f,           0.11f, 0.22f, 0.33f, 0.99f, 0.8f,  0.7f,  0.5f,
            /*f1 xy*/ 0.48f, 0.05f, 0.35f, 1.0f,  1.0f,  0.51f, 0.51f, 0.13f,
            0.86f,           0.10f, 0.29f, 0.53f, 0.99f, 0.4f,  0.3f,  0.1f,
            /*f2 xy*/ 0.98f, 0.35f, 0.3f,  0.01f, 0.9f,  0.55f, 0.15f, 0.39f,
            0.36f,           0.01f, 0.32f, 0.4f,  0.3f,  0.2f,  0.1f,  0.5f,
        };
        ...
        uint64_t usm_result = 1208;  // USM have a higher peak, since transfering memory to device adds temporay memory
                                     // bytes allocated. Old memory is deallocated quickly, but max peak is higher.
        auto is_correct =
            engine->get_max_used_device_memory() == cl_mem_result || engine->get_max_used_device_memory() == usm_result;
        ...
        cl_mem_result = 1224;
        usm_result = 1992;  // USM have a higher peak, since transfering memory to device adds temporay memory bytes
                            // allocated. Old memory is deallocated quickly, but max peak is higher.
        is_correct =
            engine->get_max_used_device_memory() == cl_mem_result || engine->get_max_used_device_memory() == usm_result;
        ASSERT_TRUE(is_correct) << "Memory max peak is not correct";
        ASSERT_EQ(output_layout_first, output_layout_second);
    ...
    void test_shared_mem_pool_diff_batches(bool is_caching_test) {
        ...
        std::vector<float> dummy_input_data_1 =
            rg.generate_random_1d<float>(batch_1 * feature_num * inp_x_size * inp_y_size, 0, 1);
        std::vector<float> dummy_input_data_8 =
            rg.generate_random_1d<float>(batch_8 * feature_num * inp_x_size * inp_y_size, 0, 1);
    ...
    void test_non_opt_intermidate_opt_after(bool is_caching_test) {
        ...
        auto topology = cldnn::topology(input, input2, concat, reshape, crop1, crop2, eltwise1, eltwise2, data_memory);
    ...
    void test_add_mem_dep(bool is_caching_test) {
        ...
        set_values(input_memory1, {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f});
        set_values(scale_memory, {1.0f});
        ...
        auto topology =
            cldnn::topology(input, crop1, crop2, actv1, actv2, eltwise1, eltwise2, data_memory, actv3, actv4);
    ...
    void test_dynamic_mem_reuse() {
        ...
        auto fc_weights_data =
            engine.allocate_memory({ov::PartialShape{weight_b, input_f}, data_types::f32, format::bfyx});

        set_values(input_data1, {-0.5f, 2.0f, 0.5f});
        set_values(input_data2, {0.5f, -2.0f, -0.5f, -0.5f, 2.0f, 0.5f});
        set_values(fc_weights_data, {1.5f, 1.0f, 0.5f, -1.0f, 0.0f, 0.5f, 0.5f, -0.5f, -2.0f, -0.5f, 1.0f, 1.5f});
...

        {
            ...
            ASSERT_EQ(std::static_pointer_cast<fully_connected_inst>(network.get_primitive("relu1"))
                          ->output_memory_ptr()
                          ->buffer_ptr(),
                      std::static_pointer_cast<fully_connected_inst>(network.get_primitive("relu2"))
                          ->output_memory_ptr()
                          ->buffer_ptr());
        }
    ...
    void test_dynamic_mem_reuse_for_null_sel_impl() {
        ...
        auto fc_weights_data =
            engine.allocate_memory({ov::PartialShape{2, input_f, 1, 1}, data_types::f32, format::bfyx});

        set_values(input_data1, {0.5f, -2.0f, -0.5f, -0.5f, 2.0f, 0.5f, -0.5f, 2.0f, 0.5f, -0.5f, 2.0f, 0.5f});
        set_values(fc_weights_data, {1.5f, 1.0f, 0.5f, -1.0f, 0.0f, 0.5f});

        GridSampleOp::Attributes attributes(false,
                                            GridSampleOp::InterpolationMode::NEAREST,
                                            GridSampleOp::PaddingMode::ZEROS);

        cldnn::topology topology{input_layout("input", input_static_layout),
                                 input_layout("input_dyn", input_dyn_layout),
                                 activation("relu1", input_info("input"), activation_func::relu),
                                 eltwise("elt1", {input_info("input"), input_info("relu1")}, eltwise_mode::prod),
                                 activation("relu2", input_info("elt1"), activation_func::sqrt),
                                 // The 'grid_sample' layer should be dynamic, not 'shape agnostic kernel' and user of
                                 // 'elt1'. This is a key condition of this test.
...

        {
            ...
            ASSERT_NE(std::static_pointer_cast<fully_connected_inst>(network.get_primitive("relu1"))
                          ->output_memory_ptr()
                          ->buffer_ptr(),
                      std::static_pointer_cast<fully_connected_inst>(network.get_primitive("relu2"))
                          ->output_memory_ptr()
