...
TEST(depth_to_space_fp16_gpu, d1411_bs2) {
    ...
    set_values(input1, {ov::float16(0.0f), ov::float16(1.0f), ov::float16(2.0f), ov::float16(3.0f)});

    topology topology;
    topology.add(input_layout("Input0", input1->get_layout()));
    topology.add(depth_to_space("depth_to_space", input_info("Input0"), block_size, depth_to_space_mode::blocks_first));
    ...
    std::vector<float> expected_results = {0.f, 1.f, 2.f, 3.f};
...
TEST(depth_to_space_fp16_gpu, d1421_bs2) {
    ...
    std::vector<float> expected_results = {0.0f, 2.0f, 4.0f, 6.0f, 1.0f, 3.0f, 5.0f, 7.0f};
...
TEST(depth_to_space_fp16_gpu, d1933_bs3) {
    ...
    std::vector<float> expected_results = {
        0.0f,  9.0f,  18.0f, 1.0f,  10.0f, 19.0f, 2.0f,  11.0f, 20.0f, 27.0f, 36.0f, 45.0f, 28.0f, 37.0f,
        46.0f, 29.0f, 38.0f, 47.0f, 54.0f, 63.0f, 72.0f, 55.0f, 64.0f, 73.0f, 56.0f, 65.0f, 74.0f, 3.0f,
        12.0f, 21.0f, 4.0f,  13.0f, 22.0f, 5.0f,  14.0f, 23.0f, 30.0f, 39.0f, 48.0f, 31.0f, 40.0f, 49.0f,
        32.0f, 41.0f, 50.0f, 57.0f, 66.0f, 75.0f, 58.0f, 67.0f, 76.0f, 59.0f, 68.0f, 77.0f, 6.0f,  15.0f,
        24.0f, 7.0f,  16.0f, 25.0f, 8.0f,  17.0f, 26.0f, 33.0f, 42.0f, 51.0f, 34.0f, 43.0f, 52.0f, 35.0f,
        44.0f, 53.0f, 60.0f, 69.0f, 78.0f, 61.0f, 70.0f, 79.0f, 62.0f, 71.0f, 80.0f};
...
TEST(depth_to_space_fp32_gpu, d1411_bs2) {
    ...
    set_values(input1, {0.f, 1.f, 2.f, 3.f});
...
TEST(depth_to_space_fp32_gpu, d112960540_bs2) {
    ...
    topology_ref.add(
        reorder("reorder1",
                input_info("Input0"),
                {data_types::f16, format::bfwzyx, tensor{batch(1), feature(12), spatial(1, 1, 960, 540)}}));
    topology_ref.add(reshape("reshape", input_info("reorder1"), tensor{batch(1), feature(2), spatial(960, 540, 3, 2)}));
    topology_ref.add(permute("perm", input_info("reshape"), perm));
    topology_ref.add(reshape("reshape2", input_info("perm"), tensor(1, 3, 2 * 960, 2 * 540)));
...
TEST(depth_to_space_fp32_gpu, d1933_bs3) {
    ...
    set_values(input1,
               {0.0f,  1.0f,  2.0f,  3.0f,  4.0f,  5.0f,  6.0f,  7.0f,  8.0f,  9.0f,  10.0f, 11.0f, 12.0f, 13.0f,
                ...
                70.0f, 71.0f, 72.0f, 73.0f, 74.0f, 75.0f, 76.0f, 77.0f, 78.0f, 79.0f, 80.0f});
...
TEST(depth_to_space_fp32_gpu, d1822_bs2_blocks_first) {
    ...
    set_values(input1, {0.0f,  1.0f,  2.0f,  3.0f,  4.0f,  5.0f,  6.0f,  7.0f,  8.0f,  9.0f,  10.0f,
    ...
    std::vector<float> expected_results = {0.0f,  8.0f,  1.0f,  9.0f,  16.0f, 24.0f, 17.0f, 25.0f, 2.0f,  10.0f, 3.0f,
                                           11.0f, 18.0f, 26.0f, 19.0f, 27.0f, 4.0f,  12.0f, 5.0f,  13.0f, 20.0f, 28.0f,
                                           21.0f, 29.0f, 6.0f,  14.0f, 7.0f,  15.0f, 22.0f, 30.0f, 23.0f, 31.0f};
...
void test_depth_to_space_fp32_gpu_d1822_bs2_depth_first(bool is_caching_test) {
    ...
    topology topology;
    topology.add(input_layout("Input0", input1->get_layout()));
    topology.add(depth_to_space("depth_to_space", input_info("Input0"), block_size, depth_to_space_mode::depth_first));
    ...
    std::vector<T> expected_results = {0.0f,  4.0f,  1.0f,  5.0f,  8.0f,  12.0f, 9.0f,  13.0f, 2.0f,  6.0f,  3.0f,
                                       7.0f,  10.0f, 14.0f, 11.0f, 15.0f, 16.0f, 20.0f, 17.0f, 21.0f, 24.0f, 28.0f,
                                       25.0f, 29.0f, 18.0f, 22.0f, 19.0f, 23.0f, 26.0f, 30.0f, 27.0f, 31.0f};
...
static void test_depth_to_space_fp16_input_fp32_output(bool is_caching_test) {
    ...
    topology.add(cldnn::reorder("reorder_input",
                                input_info("input"),
                                cldnn::layout(data_types::f16, format::byxf, {1, 1, 4, 5})));
    topology.add(cldnn::convolution("conv",
                                    ...
                                    false));
    topology.add(
        cldnn::depth_to_space("depth_to_space", input_info("conv"), block_size, depth_to_space_mode::depth_first));
    topology.add(cldnn::activation("activate",
                                   input_info("depth_to_space"),
                                   cldnn::activation_func::relu_negative_slope,
                                   {0.25f, 0.f}));
    topology.add(cldnn::reorder("convert:output",
                                input_info("activate"),
                                format::any,
                                data_types::f32,
                                {},
                                ...
                                padding(),
                                true));
    topology.add(cldnn::reorder("result:output/sink_port_0",
                                input_info("convert:output"),
                                format::bfyx,
                                data_types::f32,
                                {},
                                reorder_mean_mode::subtract,
    ...
    std::vector<float> expected_results = {24.0f, 24.0f, 32.0f, 28.0f};
