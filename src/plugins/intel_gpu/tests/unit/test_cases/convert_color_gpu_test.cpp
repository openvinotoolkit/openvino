...
void createReferenceDataNV12(const T* arg_y,
                             const T* arg_uv,
                             U* out_ptr,
                             size_t batch_size,
                             size_t image_h,
...
TEST(convert_color, nv12_to_rgb_two_planes_buffer_fp32) {
    ...
    createReferenceDataNV12<float, float>(input_y_data.data(),
                                          input_uv_data.data(),
                                          ref_res.data(),
                                          1,
                                          height,
...
TEST(convert_color, nv12_to_bgr_two_planes_buffer_fp32) {
    ...
    std::vector<float> ref_res(width * height * 3);
    createReferenceDataNV12<float>(input_y_data.data(),
                                   input_uv_data.data(),
                                   ref_res.data(),
                                   1,
                                   height,
...
TEST(convert_color, nv12_to_rgb_two_planes_buffer_u8) {
    ...
    std::vector<float> ref_res(width * height * 3);
    createReferenceDataNV12<uint8_t, float>(input_y_data.data(),
                                            input_uv_data.data(),
                                            ref_res.data(),
                                            1,
                                            height,
...
TEST(convert_color, nv12_to_rgb_two_planes_buffer_fp16) {
    ...
    std::vector<float> ref_res(width * height * 3);
    createReferenceDataNV12<ov::float16, float>(input_y_data.data(),
                                                input_uv_data.data(),
                                                ref_res.data(),
                                                1,
                                                height,
...
TEST(convert_color, nv12_to_rgb_single_plane_buffer_fp32) {
    ...
    std::vector<float> ref_res(width * height * 3);
    createReferenceDataNV12<float, float>(input_data.data(),
                                          input_data.data() + height * width,
                                          ref_res.data(),
                                          1,
                                          height,
                                          width,
...
TEST(convert_color, nv12_to_rgb_single_plane_buffer_u8) {
    ...
    std::vector<float> ref_res(width * height * 3);
    createReferenceDataNV12<uint8_t, float>(input_data.data(),
                                            input_data.data() + height * width,
                                            ref_res.data(),
                                            1,
                                            height,
...
TEST(convert_color, nv12_to_rgb_two_planes_surface_u8) {
    ...
    cl_image_desc image_desc = {CL_MEM_OBJECT_IMAGE2D, (size_t)width, (size_t)height, 0, 0, 0, 0, 0, 0, {nullptr}};

    cl_mem nv12_image_plane_y =
        clCreateImage(ocl_instance->_context.get(), CL_MEM_READ_WRITE, &image_format, &image_desc, nullptr, &err);
    ...
    createReferenceDataNV12<uint8_t, uint8_t>(data.data(),
                                              data.data() + height * width,
                                              reference_results.data(),
                                              1,
                                              height,
                                              width,
                                              height * width,
...
TEST(convert_color, nv12_to_rgb_single_plane_surface_u8) {
    ...
    cl_image_desc image_desc =
        {CL_MEM_OBJECT_IMAGE2D, (size_t)width, (size_t)input_height, 0, 0, 0, 0, 0, 0, {nullptr}};
    ...
    std::vector<uint8_t> reference_results(width * height * 3);
    createReferenceDataNV12<uint8_t, uint8_t>(input_data.data(),
                                              input_data.data() + height * width,
                                              reference_results.data(),
                                              1,
                                              height,
...
template <typename T, typename U>
void createReferenceDataI420(const T* arg_y,
                             const T* arg_u,
                             const T* arg_v,
                             U* out_ptr,
                             size_t batch_size,
...
TEST(convert_color, i420_to_rgb_three_planes_buffer_fp32) {
    ...
    std::vector<float> ref_res(width * height * 3);
    createReferenceDataI420<float, float>(input_y_data.data(),
                                          input_u_data.data(),
                                          input_v_data.data(),
                                          ref_res.data(),
                                          1,
...
void test_convert_color_i420_to_rgb_three_planes_surface_u8(bool is_caching_test) {
    ...
    cl_mem i420_image_plane_u =
        clCreateImage(ocl_instance->_context.get(), CL_MEM_READ_WRITE, &image_format, &image_desc, nullptr, &err);
    checkStatus(err, "Creating i420 image plane_u failed");

    cl_mem i420_image_plane_v =
        clCreateImage(ocl_instance->_context.get(), CL_MEM_READ_WRITE, &image_format, &image_desc, nullptr, &err);
    ...
    err = clEnqueueWriteImage(ocl_instance->_queue.get(),
                              ...
                              0,
                              0,
                              &data[width * (height + height / 4)],
                              0,
                              nullptr,
                              nullptr);
    checkStatus(err, "Writing i420 image plane_v failed");

    auto input = input_layout("input",
                              {
                                  {1, height, width, 1},
                                  data_types::u8,
                                  format::nv12,
    ...
    std::vector<uint8_t> reference_results(width * height * 3);
    createReferenceDataI420<T, uint8_t>(data.data(),
                                        data.data() + height * width,
                                        data.data() + width * (height + height / 4),
                                        reference_results.data(),
                                        1,
                                        height,
                                        width,
