...
bool is_output_equal(const cldnn::memory::ptr mem, const std::vector<T>& ref) {
    ...
    for (size_t i = 0; i < mem->get_layout().count(); i++) {
        if (!are_equal(ptr[i], ref[i]))
            return false;
    }
    return true;
}

topology generate_simple_branch(bool branch_true_false,
                                const primitive_id& id,
                                const primitive_id& input_id,
                                const data_types dt = data_types::f32) {
    topology branch;
    if (branch_true_false) {
        branch.add(input_layout(input_id, {dt, format::bfyx, {1, 1, 4, 1}}),
...
public:
    ...
    void run_test(bool is_caching_test = false) {
        ...
        condition::branch branch_true;
        {
            ...
            branch_true.inner_program =
                program::build_program(engine, branch_true_topology, config, false, false, true);
        ...
        condition::branch branch_false;
        {
            ...
            branch_false.inner_program =
                program::build_program(engine, branch_false_topology, config, false, false, true);
        ...
        topology.add(input_layout(pred_id, predicate->get_layout()));
        topology.add(input_layout(scale_data_id, scale_mem->get_layout()));
        topology.add(condition(cond_id, {input_info(pred_id), input_info(input_id)}, branch_true, branch_false));
        topology.add(eltwise(output_id, {input_info(cond_id), input_info(scale_data_id)}, eltwise_mode::prod));
...
using test_data_types = testing::Types<condition_data_types<ov::float16>, condition_data_types<float>>;
...
public:
    void test_basic_range_equal_comp(bool is_caching_test) {
        ...
        topology.add(input_layout("predicate", predicate->get_layout()));
        topology.add(concatenation("concat", {input_info("input0"), input_info("input1")}, 3));
        ...
        topology.add(condition("condi", {input_info("predicate"), input_info("concat")}, branch_true, branch_false));

        std::vector<float> input0_data = {1, 2};
        std::vector<float> input1_data = {3, 4};
        std::vector<uint8_t> predicate_data_true = {1};
        std::vector<float> pooling_when_true_data = {2, 4};
        std::vector<uint8_t> predicate_data_false = {0};
        std::vector<float> pooling_when_false_data = {1.5, 3.5};
    ...
    void test_basic_stacked_ifs(bool is_caching_test) {
        ...
        */
        ...
        condi_2_true.add(input_layout(branch_input_id, {data_types::f32, format::bfyx, {1, 1, 2, 1}}),
                         activation("activ_when_true", input_info(branch_input_id), activation_func::log2));
        topology condi_2_false;
        condi_2_false.add(input_layout(branch_input_id, {data_types::f32, format::bfyx, {1, 1, 2, 1}}),
        ...
        topology.add(input_layout(predicate2_id, predicate2->get_layout()));
        topology.add(condition(cond2_id,
                               {input_info(predicate2_id), input_info(cond_id)},
                               branch_condi_2_true,
                               branch_condi_2_false));

        std::vector<float> input_data = {1, 2, 3, 4};
        std::vector<uint8_t> predicate_data = {1};
        std::vector<uint8_t> predicate_2_data = {0};
        ...
        std::vector<float> ref_data = {2.0f, 4.0f};
    ...
    void test_basic_nested_ifs(bool is_caching_test) {
        ...
        */
        ...
        condition::branch nested_true;
        {
            ...
            nested_true.inner_program =
                program::build_program(engine, nested_true_topology, config, false, false, true);
        ...
        condition::branch nested_false;
        {
            ...
            nested_false.inner_program =
                program::build_program(engine, nested_false_topology, config, false, false, true);
        ...
        condition::branch branch_true;
        {
            ...
            branch_true_topology.add(
                ...
                condition("condi_nested",
                          {input_info("predicate2"), input_info("pooling_when_true")},
                          nested_true,
                          nested_false));
            branch_true.inner_program =
                program::build_program(engine, branch_true_topology, config, false, false, true);
        ...
        condition::branch branch_false;
        {
            ...
            branch_false_topology.add(input_layout("branch_input4", {data_types::f32, format::bfyx, {1, 1, 4, 1}}),
                                      pooling("pooling_when_false",
                                              input_info("branch_input4"),
        ...
        topology.add(condition("condi", {input_info("predicate"), input_info("input")}, branch_true, branch_false));

        std::vector<float> input_data = {1.0f, 2.0f, 3.0f, 4.0f};
        std::vector<float> predicate_data = {1.0f};
        std::vector<float> predicate_2_data = {2.0f, 4.0f};
        set_values(input, input_data);
        set_values(predicate, predicate_data);
        set_values(predicate2, predicate_2_data);
    ...
    void test_negative_predicate_wrong_layout(bool is_caching_test) {
        ...
        {
            cldnn::topology branch_true_topology =
                generate_simple_branch(true, cond_id, branch_input_id, data_types::f32);
            branch_true.inner_program =
                program::build_program(engine, branch_true_topology, config, false, false, true);
            branch_true.input_map.insert({input_id, branch_input_id});
            branch_true.output_map.insert({0, "condi_when_true"});
        ...
        {
            cldnn::topology branch_false_topology =
                generate_simple_branch(false, cond_id, branch_input_id, data_types::f32);
            branch_false.inner_program =
                program::build_program(engine, branch_false_topology, config, false, false, true);
            branch_false.input_map.insert({input_id, branch_input_id});
            branch_false.output_map.insert({0, "condi_when_false"});
        ...
        EXPECT_ANY_THROW(network::ptr net =
    ...
    void test_negative_same_names_within_different_networks(bool is_caching_test) {
        ...
        topology.add(input_layout(pred_id, predicate->get_layout()));
        topology.add(condition(cond_id, {input_info(pred_id), input_info(input_id)}, branch_true, branch_false));
        topology.add(pooling(duplicated_id, input_info(cond_id), cldnn::pooling_mode::max, {2, 1}, {2, 1}));
    ...
    void test_empty_body(bool is_caching_test) {
        ...
        topology.add(condition(cond_id,
                               {input_info(pred_id), input_info(input_id1), input_info(input_id2)},
                               branch_true,
                               branch_false));

        network::ptr net = get_network(engine, topology, config, get_test_stream_ptr(), is_caching_test);
        ASSERT_TRUE(net->get_primitive(cond_id)
                        ->get_node()
                        .as<condition>()
                        .get_branch_false()
                        .inner_program->can_be_optimized());
        ASSERT_FALSE(net->get_primitive(cond_id)
                         ->get_node()
                         .as<condition>()
                         .get_branch_true()
                         .inner_program->can_be_optimized());
...
TEST(condition_gpu, empty_body_with_different_shapes) {
    ...
    {
        topology branch_false_topology;
        branch_false_topology.add(input_layout(branch_input_id3, {const_pshape, data_types::f32, format::bfyx}),
    ...
    topology.add(input_layout(pred_id, predicate_mem->get_layout()));
    topology.add(condition(cond_id,
                           {input_info(pred_id), input_info(input_id1), input_info(input_id2), input_info(input_id3)},
                           branch_true,
                           branch_false));

    network net(engine, topology, config);
    ASSERT_FALSE(
        net.get_primitive(cond_id)->get_node().as<condition>().get_branch_true().inner_program->can_be_optimized());
    ASSERT_TRUE(
        net.get_primitive(cond_id)->get_node().as<condition>().get_branch_false().inner_program->can_be_optimized());
...
TEST(condition_gpu, set_empty_tensor) {
    ...
    {
        topology branch_false_topology;
        branch_false_topology.add(input_layout(branch_input_id2, {{1, 1, 4, 1}, data_types::f32, format::bfyx}),
    ...
    topology.add(condition(cond_id,
                           {input_info(pred_id), input_info(reorder_id), input_info(input_id)},
                           branch_true,
                           branch_false));
