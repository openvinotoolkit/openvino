...
std::vector<Params<T>> generateParams() {
    static const std::vector<Params<T>> result = {
        ...
        {
            tensor(5, 1, 1, 10),
            std::vector<T>{1, 4, 9, 7, 9, 1, 2, 3, 9, 2, 3, 1, 4, 2, 4, 4, 7, 4, 9, 5, 8, 4, 3, 7, 5,
                           2, 4, 8, 3, 1, 5, 7, 9, 4, 5, 6, 4, 2, 9, 2, 8, 8, 7, 9, 8, 3, 1, 7, 5, 9},
            tensor(5, 1, 1, 10),
            std::vector<T>{1, 4, 9, 7, 9, 1, 2, 3, 9, 2, 3, 1, 4, 2, 4, 4, 7, 4, 9, 5, 8, 4, 3, 7, 5,
                           2, 4, 8, 3, 1, 5, 7, 9, 4, 5, 6, 4, 2, 9, 2, 8, 8, 7, 9, 8, 3, 1, 7, 5, 9},
            tensor(1, 1, 1, 1),
            ...
            std::vector<T>{9},
            tensor(5, 1, 1, 10),
            std::vector<T>{4, 4, 9, 9, 4, 9, 2, 9, 9, 9, 1, 1, 9, 9, 1, 9, 9, 9, 9, 9, 1, 1, 9, 9, 1,
...
struct PrintToStringParamName {
    ...
    std::string operator()(const testing::TestParamInfo<ParamsWithLayout<T>>& param) {
        ...
        buf << " test case " << p.testcase_name << " plain layout " << plain_layout << " target layout "
            << target_layout << " is_caching_test " << is_caching_test;
        return buf.str();
    }
};
};  // namespace
...
template <typename T>
struct gather_tree_test : public ::testing::TestWithParam<ParamsWithLayout<T>> {
public:
    void test() {
        ...
        topology.add(gather_tree(result_id,
                                 input_info(reorder_step_id),
                                 input_info(reorder_parent_id),
                                 input_info(reorder_max_seq_len_id),
                                 input_info(reorder_end_token_id)));
...
INSTANTIATE_TEST_SUITE_P(gather_tree,
                         ...
                         ::testing::Combine(::testing::ValuesIn(generateParams<float>()),
...
INSTANTIATE_TEST_SUITE_P(gather_tree,
                         ...
                         ::testing::Combine(::testing::ValuesIn(generateParams<int32_t>()),
                                            ::testing::Values(format::bfyx),
                                            ::testing::ValuesIn(layouts),
                                            ::testing::Values(false)),
                         PrintToStringParamName());
...
INSTANTIATE_TEST_SUITE_P(export_import,
                         gather_tree_test_int32,
                         ::testing::Combine(::testing::Values(generateParams<int32_t>()[0]),
                                            ::testing::Values(format::bfyx),
