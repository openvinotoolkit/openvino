...
public:
    ...
    // Two dynamic_quantize primitives whose input layouts are identical (innermost dimension dynamic)
    // but whose actual innermost length differs. The layouts alone cannot tell them apart, so unless
    // the resolved length is part of the primitive both nodes share a single shape agnostic kernel,
    // whose group count is baked in at compile time for whichever node was compiled first.
    void test_dynamic_quantization_dynamic_innermost_dim(size_t innermost_size_0, size_t innermost_size_1, uint64_t group_size) {
        tests::random_generator rg(GET_SUITE_NAME);
        auto& engine = get_test_engine();

        const size_t batch = 32;
        const ov::PartialShape dyn_ps{-1, 1, -1};
        ...
        const std::vector<size_t> innermost_sizes = {innermost_size_0, innermost_size_1};
        std::vector<memory::ptr> input_mems;
        for (auto innermost_size : innermost_sizes) {
            const ov::Shape static_shape{batch, 1, innermost_size};
            auto input_mem = engine.allocate_memory({static_shape, data_types::f32, format::bfyx});
            set_values(input_mem, rg.generate_random_1d<float>(ov::shape_size(static_shape), -16.0f, 20.0f));
            input_mems.push_back(input_mem);
        }

        auto build_topology = [&]() {
            topology topology;
            for (size_t i = 0; i < innermost_sizes.size(); i++) {
                const auto idx = std::to_string(i);
                const ov::Shape static_shape{batch, 1, innermost_sizes[i]};
                topology.add(input_layout("input" + idx, layout{dyn_ps, data_types::f32, format::bfyx}));
                topology.add(reorder("reorder_in" + idx, input_info("input" + idx), layout{dyn_ps, data_types::f16, format::bfyx}));
                topology.add(dynamic_quantize("dyn_quan_prim" + idx, input_info("reorder_in" + idx), dq_config, dyn_ps.size(), innermost_sizes[i]));
                topology.add(reorder("reorder_data" + idx,
                                     input_info("dyn_quan_prim" + idx, 0),
                                     layout{static_shape, data_types::f16, format::bfyx}));
                topology.add(reorder("reorder_scale" + idx,
                                     input_info("dyn_quan_prim" + idx, 1),
                                     layout{ov::PartialShape::dynamic(dyn_ps.size()), data_types::f16, format::bfyx}));
            }
            return topology;
        };

        bool shares_impl_params = false;
        auto run = [&](bool force_ref) {
            auto config = get_test_default_config(engine);
            config.set_property(ov::intel_gpu::allow_new_shape_infer(true));
            config.set_property(ov::intel_gpu::optimize_data(true));

            if (force_ref) {
                ov::intel_gpu::ImplementationDesc ref_desc = {format::bfyx, "dynamic_quantize_gpu_ref", impl_types::ocl};
                config.set_property(ov::intel_gpu::force_implementations(
                    ov::intel_gpu::ImplForcingMap{{"dyn_quan_prim0", ref_desc}, {"dyn_quan_prim1", ref_desc}}));
            }
            ...
            // Both nodes see the very same layouts, so their impl params may only be told apart by the
            // innermost dimension carried in the primitive. Equal params make them share one kernel.
            const auto& prog = *network.get_program();
            shares_impl_params |= *prog.get_node("dyn_quan_prim0").get_kernel_impl_params() ==
                                  *prog.get_node("dyn_quan_prim1").get_kernel_impl_params();

            std::map<std::string, memory::ptr> results;
            for (const auto& output : outputs) {
                auto output_mem = output.second.get_memory();
                results.emplace(output.first, engine.reinterpret_buffer(*output_mem, output.second.get_layout()));
            }
            return results;
        };

        auto ref_results = run(true);
        auto opt_results = run(false);

        ASSERT_FALSE(shares_impl_params) << "both dynamic_quantize nodes map to the same impl params, "
                                            "so they will share a kernel built for a single innermost size";
        ASSERT_EQ(ref_results.size(), opt_results.size());
        ASSERT_FALSE(ref_results.empty());

        for (const auto& ref : ref_results) {
            ASSERT_TRUE(opt_results.count(ref.first)) << "missing output " << ref.first;

            cldnn::mem_lock<ov::float16, mem_lock_type::read> ref_ptr(ref.second, get_test_stream());
            cldnn::mem_lock<ov::float16, mem_lock_type::read> opt_ptr(opt_results.at(ref.first), get_test_stream());
            ASSERT_EQ(ref_ptr.size(), opt_ptr.size()) << "size mismatch for output " << ref.first;

            const int abs_error_threshold = 2;
            for (size_t i = 0; i < ref_ptr.size(); i++) {
                ASSERT_NEAR(ref_ptr[i], opt_ptr[i], abs_error_threshold)
                    << "output " << ref.first << " differs at " << i;
            }
        }
    }
};

// Reproduces a GPU accuracy issue seen on Gemma4 VLMs, where the o_proj activations keep their innermost
// dimension dynamic and the sliding/full attention layers feed FullyConnected primitives with different
// IFM. In per token mode the group count is a compile time constant, so sharing one kernel between the
// two IFM values quantizes over the wrong number of elements for one of them.
TEST_F(dynamic_quantization_gpu_tests, dynamic_innermost_dim_per_token_multiple_ifm) {
    this->test_dynamic_quantization_dynamic_innermost_dim(4096, 8192, UINT64_MAX);
}

// Grouped quantization uses the group size instead of the innermost dimension for the group count, so
// this configuration was never broken. Kept as a regression guard for the sharing logic.
TEST_F(dynamic_quantization_gpu_tests, dynamic_innermost_dim_grouped_multiple_ifm) {
    this->test_dynamic_quantization_dynamic_innermost_dim(4096, 8192, 64);
}
