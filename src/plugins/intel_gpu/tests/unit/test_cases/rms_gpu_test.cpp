...
TEST(rms_gpu_test, rms_test_bfyx_opt_dyn) {
    ...
    auto gamma_layout_dynamic = layout{ov::PartialShape{ov::Dimension::dynamic(), ov::Dimension::dynamic(), 4096},
                                       data_types::f32, format::bfyx};
    auto input = engine.allocate_memory({ov::PartialShape{1, 1, 4096}, data_types::f32, format::bfyx});
    auto gamma = engine.allocate_memory({ov::PartialShape{1, 1, 4096}, data_types::f32, format::bfyx});
    auto output_ref = engine.allocate_memory({ov::PartialShape{1, 1, 4096}, data_types::f32, format::bfyx});
    ...
    topology.add(input_layout("gamma", gamma_layout_dynamic));
    topology.add(rms("rms", input_info("input"), input_info("gamma"), 1e-5f));
