...
struct RangeArg {
    ...
    RangeArg(data_types dt, const char name[])
        : name{name},
          p{tests::get_test_engine().allocate_memory({ov::PartialShape{}, dt, format::bfyx})} {}
...
struct RangeArgs {
    ...
    memory::ptr run(int outLen, bool use_new_shape_infer) const {
        ...
        stop.addTo(topology);
        step.addTo(topology);
        topology.add(range{"range",
                           {input_info(start.name), input_info(stop.name), input_info(step.name)},
                           {dt, format::bfyx, tensor{batch(outLen)}}});
...
struct range_test_param_generator : std::vector<range_test_params> {
    ...
    range_test_param_generator& simple_params(std::vector<data_types>& data_types_list,
                                              double start,
                                              double stop,
                                              double step) {
...
std::vector<data_types> general_types = {data_types::u8,
                                         data_types::i32,
                                         data_types::i32,
                                         data_types::f16,
                                         data_types::f32};
...
INSTANTIATE_TEST_SUITE_P(range_gpu_test,
                         smoke_range_test,
                         testing::ValuesIn(range_test_param_generator()
...
TEST(range_gpu_test, range_with_select) {
    ...
    topology.add(
        cldnn::select("select", input_info("select_input1"), input_info("select_input2"), input_info("select_mask")));
...
TEST(range_cpu_impl_test, dynamic_all) {
    ...
    config.set_property(ov::intel_gpu::force_implementations(
        ov::intel_gpu::ImplForcingMap{{"range", {format::bfyx, "", impl_types::cpu}}}));
