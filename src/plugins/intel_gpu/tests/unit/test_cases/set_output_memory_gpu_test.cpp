...
void test_basic(bool is_caching_test) {
    ...
    topology.add(reorder("reorder", input_info("Input"), input_data->get_layout()));
...
TEST(set_output_memory_gpu, basic_const) {
    ...
    topology.add(reorder("reorder_dyn", input_info("Input"), input_data->get_layout()),
...
TEST(set_output_memory_gpu, top_k1) {
    ...
    auto output_mem = engine.allocate_memory({data_types::f32, format::bfyx, {top_k, feature_num, x_size, y_size}});
    topology topology;
...
TEST(set_output_memory_gpu, basic_opt) {
    ...
    std::vector<float> input_vec1 = {// y0x0 y0x1 y1x0 y1x1
                                     /*b0f0*/ 0.1f,
                                     -2.1f,
                                     -3.1f,
                                     -4.1f,
    ...
    std::vector<float> input_vec2 = {// y0x0 y0x1 y1x0 y1x1
                                     /*b1f0*/ 0.2f,
                                     -2.2f,
                                     -3.2f,
                                     -4.2f,
    ...
    std::vector<float> output_vec = {// y0x0 y0x1 y1x0 y1x1
...
TEST(set_output_memory_gpu, mutable_output_data) {
    ...
    topology.add(arg_max_min("arg_max",
                             {input_info("Add_1396"), input_info("second_input"), input_info("12220_md_write")},
                             ov::op::TopKMode::MIN,
                             top_k,
                             0));
