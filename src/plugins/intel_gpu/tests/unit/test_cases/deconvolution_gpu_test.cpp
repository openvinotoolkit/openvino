...
TEST(deconvolution_gpu, input_b_fs_yx_fsv16_output_bfyx_nopad) {
    auto& engine = get_test_engine();
    auto input = engine.allocate_memory({ data_types::f32, format::b_fs_yx_fsv16, { 1, 16, 4, 4 } });
    auto weights = engine.allocate_memory({ data_types::f32, format::bfyx, { 8, 16, 3, 3 } });
    auto biases = engine.allocate_memory({ data_types::f32, format::bfyx, { 1, 8, 1, 1 } });
    ...
    auto create_topology =[&]() {
        ...
        topology.add(data("weights", weights));
        topology.add(data("biases", biases));
        topology.add(deconvolution("deconv", input_info("input"), { "weights" }, { "biases" }, { 2, 2 }, { 0, 0 }));
        topology.add(activation("activation", input_info("deconv"), activation_func::relu_negative_slope, { 0.1f, 0.0f }));
        topology.add(reorder("output", input_info("activation"), format::bfyx, data_types::f32));
        return topology;
    };
    ...
    ov::intel_gpu::ImplementationDesc impl_test = { format::b_fs_yx_fsv16, std::string(""), impl_types::ocl };
    ov::intel_gpu::ImplementationDesc impl_ref = { format::bfyx, std::string(""), impl_types::ocl };
