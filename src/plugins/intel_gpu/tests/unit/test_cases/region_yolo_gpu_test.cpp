...
template <typename T>
inline void softmax_generic(const T* src_data,
                            T* dst_data,
                            uint32_t batches,
                            uint32_t channels,
                            uint32_t height,
...
void region_yolo_ref(const T* input,
    ...
    for (unsigned int batch_idx = 0; batch_idx < batches; batch_idx++) {
        for (unsigned int n = 0; n < num_regions; n++) {
            ...
            std::transform(input + index, input + index + 2 * width * height, output + index, [](T elem) {
                return sigmoid<T>(elem);
            });

            index = entry_index(width, height, coords, classes, inputs_size, batch_idx, n * width * height, coords);
            std::transform(input + index, input + index + end_index, output + index, [](T elem) {
                return sigmoid<T>(elem);
            });
        }
    }
...
void runRegionTest(region_yolo_test_params& params, bool is_caching_test = false) {
    ...
    auto inputData =
        rg.generate_random_1d<T>(params.tensor[0] * params.tensor[1] * params.tensor[2] * params.tensor[3], -1, 1);
    ...
    topology.add(region_yolo("region_yolo",
                             input_info("reorder_pre"),
                             params.coords,
                             params.classes,
                             params.regionNum,
                             params.mask,
                             static_cast<uint32_t>(params.mask.size()),
                             params.axis,
                             params.end_axis,
                             params.softMax));
    ...
    region_yolo_ref<T>(inputData.data(),
                       refOutputData.data(),
                       params.tensor,
                       params.coords,
                       params.classes,
