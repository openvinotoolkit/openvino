...
using AdaptiveAvgPoolingParamsWithLayout = std::tuple<AdaptiveAvgPoolingParams,
...
template <typename T>
void generateTestData(const AdaptiveAvgPoolingParams& p,
                      const format fmt,
                      const std::vector<float>& random_inputs,
                      std::vector<T>& inputs,
...
struct PrintToStringParamName {
    std::string operator()(const testing::TestParamInfo<AdaptiveAvgPoolingParamsWithLayout>& param) {
        ...
        buf << " input tensor " << p.inputTensor.to_string() << " output tensor " << p.outputTensor.to_string()
            << " plain layout " << plain_layout << " target layout " << target_layout << " is_caching_test "
            << is_caching_test;
        return buf.str();
    }
};
...
template <typename T>
struct adaptive_avg_pooling_test : public ::testing::TestWithParam<AdaptiveAvgPoolingParamsWithLayout> {
...
public:
    void test() {
        ...
        const std::vector<float> random_input_data =
            rg.generate_random_1d<float>(params.inputTensor.count(), -127, 127, 8);
        ...
        topology.add(
            adaptive_pooling("adaptive_avg_pooling_blocked", input_info("input_reordered"), params.outputTensor));
        topology.add(
            reorder("adaptive_avg_pooling", input_info("adaptive_avg_pooling_blocked"), plain_layout, data_type));
...
INSTANTIATE_TEST_SUITE_P(smoke_adaptive_avg_pooling_test_f32_2d,
                         ...
                         ::testing::Combine(::testing::ValuesIn(std::vector<AdaptiveAvgPoolingParams>{
