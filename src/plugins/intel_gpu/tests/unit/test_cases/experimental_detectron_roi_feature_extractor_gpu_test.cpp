...
void test_experimental_detectron_roi_feature_extractor_gpu_fp32_one_level(bool is_caching_test) {
    ...
    auto roi_input =
        engine.allocate_memory({data_types::f32, format::bfyx, tensor(batch(rois_num), feature(rois_feature_dim))});
    ...
    topology.add(experimental_detectron_roi_feature_extractor(
    ...
    std::vector<T> expected_first_output{
        1.416667f, 1.75f, 2.083333f, 2.416667f, 2.75f, 3.083333f, 3.166667f, 3.5f,  3.833333f,
...
TEST(experimental_detectron_roi_feature_extractor_gpu_fp32, two_levels) {
    ...
    std::vector<float> expected_first_output{
        7.41662f,   7.7499523f, 8.0832853f, 8.41662f,   8.74995f,   9.0832853f, 9.16664f,   9.49998f,   9.83331f,
...
TEST(experimental_detectron_roi_feature_extractor_gpu_fp32,
     multiple_feature_extractor_op_with_different_number_of_inputs) {
    auto& engine = get_test_engine();
    ...
    const int sampling_ratio = 2;
    const bool aligned = false;
    auto roi_input_first_instance =
        engine.allocate_memory({data_types::f32, format::bfyx, tensor(batch(rois_num), feature(rois_feature_dim))});
    auto roi_input_second_instance =
        engine.allocate_memory({data_types::f32, format::bfyx, tensor(batch(rois_num), feature(rois_feature_dim))});
    auto level_1 = engine.allocate_memory({data_types::f32, format::bfyx, {1, 2, 3, 2}});
    auto level_2 = engine.allocate_memory({data_types::f32, format::bfyx, {1, 2, 3, 2}});
    auto second_output_first_instance =
        engine.allocate_memory({data_types::f32, format::bfyx, tensor(batch(rois_num), feature(rois_feature_dim))});
    auto second_output_second_instance =
        engine.allocate_memory({data_types::f32, format::bfyx, tensor(batch(rois_num), feature(rois_feature_dim))});
    ...
    topology.add(experimental_detectron_roi_feature_extractor(feature_extractor_first_instance_id,
                                                              {input_info(input_rois_first_instance_id),
                                                               input_info(input_level_1_first_instance_id),
                                                               input_info(second_output_w_first_instance_id)},
                                                              output_dim,
                                                              pyramid_scales_first_instance,
                                                              sampling_ratio,
                                                              aligned));
    topology.add(
        activation(activation_abs_first_instance_id, feature_extractor_first_instance_id, activation_func::abs));
    topology.add(mutable_data(second_output_r_first_instance_id,
                              {feature_extractor_first_instance_id},
                              second_output_first_instance));
    ...
    topology.add(experimental_detectron_roi_feature_extractor(feature_extractor_second_instance_id,
                                                              {input_info(input_rois_second_instance_id),
                                                               input_info(input_level_1_second_instance_id),
                                                               input_info(input_level_2_second_instance_id),
                                                               input_info(second_output_w_second_instance_id)},
                                                              output_dim,
                                                              pyramid_scales_second_instance,
                                                              sampling_ratio,
                                                              aligned));
    topology.add(activation(activation_abs_second_instance_id,
                            input_info(feature_extractor_second_instance_id),
                            activation_func::abs));
    topology.add(mutable_data(second_output_r_second_instance_id,
                              {input_info(feature_extractor_second_instance_id)},
                              second_output_second_instance));
    ...
    std::vector<float> expected_first_output_first_instance{
        1.416667f, 1.75f, 2.083333f, 2.416667f, 2.75f, 3.083333f, 3.166667f, 3.5f,  3.833333f,
        7.416667f, 7.75f, 8.083333f, 8.416667f, 8.75f, 9.083334f, 9.166666f, 9.5f,  9.833334f,
        4.166667f, 4.5f,  4.833333f, 4.166667f, 4.5f,  4.833333f, 2.083333f, 2.25f, 2.416667f,
        10.16667f, 10.5f, 10.83333f, 10.16667f, 10.5f, 10.83333f, 5.083333f, 5.25f, 5.416667f};
    ...
    std::vector<float> expected_first_output_second_instance{
        7.41662f,   7.7499523f, 8.0832853f, 8.41662f,   8.74995f,   9.0832853f, 9.16664f,   9.49998f,   9.83331f,
        1.4166187f, 1.7499521f, 2.0832853f, 2.4166186f, 2.7499518f, 3.0832853f, 3.1666427f, 3.4999762f, 3.83331f,
        4.166667f,  4.5f,       4.833333f,  4.166667f,  4.5f,       4.833333f,  2.083333f,  2.25f,      2.416667f,
        10.16667f,  10.5f,      10.83333f,  10.16667f,  10.5f,      10.83333f,  5.083333f,  5.25f,      5.416667f};
