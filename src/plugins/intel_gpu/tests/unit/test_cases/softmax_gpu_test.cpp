...
public:
    static const int32_t output_x = 10, output_b = 2,  // size of whole output buffer
        input_x = 10, input_b = 2,                     // size of whole input buffer
        in_size = input_x * input_b, out_size = output_x * output_b;
    ...
    void compare_out_buffer_with_expected_batch_wise() {
        for (size_t b = 0; b < output_b; ++b) {
            ...
            ASSERT_TRUE(are_equal(batch_wise_sum, 1.0f)) << "Expected : " << 1.0f << " actual : " << batch_wise_sum;
        }
    }
    ...
    void test_values_batch_wise(bool is_caching_test) {
        ...
        float exp_buf[out_size] = {0.02569957f,  0.02569957f,  0.02569957f,  0.02569957f,  0.02569957f,  0.02569957f,
                                   0.069858674f, 0.069858674f, 0.516189665f, 0.516189665f, 0.189895565f, 0.189895565f,
                                   0.069858674f, 0.069858674f, 0.02569957f,  0.02569957f,  0.02569957f,  0.02569957f,
...
TEST(softmax_gpu_bfyx_f32, normalize_y) {
    ...
    static const int32_t x_size = 2, y_size = 2, feature_num = 3, batch_num = 2,
                         buf_size = x_size * y_size * batch_num * feature_num;
    ...
    vector<float> input_vec = {// y0x0  y0x1   y1x0    y1x1
    ...
    for (uint32_t i = 0; i < batch_num;
         i++) {  // this for loops will sum results in a batch per feature, we expect that: sum = 1.0f
        for (uint32_t l = 0; l < feature_num; l++) {
            for (uint32_t k = 0; k < x_size; k++) {
                float sum = 0.0f;
                for (uint32_t j = 0; j < y_size; j++) {
                    int index = i * feature_num * x_size * y_size + l * x_size * y_size + j * x_size + k;
...
TEST(softmax_gpu_bfzyx_f32, normalize_z) {
    //  Input  : 2x3x2x2x2
    static const int32_t x_size = 2, y_size = 2, z_size = 2, feature_num = 3, batch_num = 2,
                         buf_size = x_size * y_size * z_size * batch_num * feature_num;
    auto& engine = get_test_engine();
    ...
    vector<float> input_vec = {//    z0y0x0 z0y0x1 z0y1x0 z0y1x1 z1y0x0 z1y0x1 z1y1x0 z1y1x1
    ...
    float expected_max_values[24] = {
        0.524979f, 0.524979f, 0.5f,      0.731059f, 0.524979f, 0.524979f, 0.731059f, 0.731059f,
        0.524979f, 0.524979f, 0.731059f, 0.731059f, 0.880797f, 0.598688f, 0.731059f, 0.999955f,
        0.858149f, 0.549834f, 0.880797f, 0.952574f, 0.731059f, 0.524979f, 0.880797f, 0.952574f,
    };
            ...
            for (uint32_t j = 0; j < y_size; j++) {
                for (uint32_t k = 0; k < x_size; k++) {
                    ...
                    for (uint32_t m = 0; m < z_size; m++) {
                        int index = i * feature_num * x_size * y_size * z_size + l * x_size * y_size * z_size +
                                    m * x_size * y_size + j * x_size + k;
                        if (out_buffer[index] >= temp_max) {
                            temp_max = out_buffer[index];
                        }
                        sum += out_buffer[index];
...
TEST(softmax_gpu_bfyx_f32, normalize_b) {
    //  Input  : 3x2x2x2
    static const int32_t x_size = 2, y_size = 2, feature_num = 2, batch_num = 3,
                         buf_size = x_size * y_size * batch_num * feature_num;
    auto& engine = get_test_engine();
    ...
    float expected_sum = 1.0f;
    int max_value_buffer_index = 0;
    for (uint32_t i = 0; i < feature_num;
         i++) {  // this for loops will sum results in a batch per feature, we expect that: sum = 1.0f
        for (uint32_t j = 0; j < y_size; j++) {
            for (uint32_t k = 0; k < x_size; k++) {
                float sum = 0.0f;
                for (uint32_t l = 0; l < batch_num; l++) {
                    int index = l * feature_num * x_size * y_size + i * x_size * y_size + j * x_size + k;
                    if (out_buffer[index] >= temp_max) {
                        temp_max = out_buffer[index];
                    }
                    sum += out_buffer[index];
...
public:
    ...
    bool is_format_supported(cldnn::format format) override {
        return format == cldnn::format::yxfb || format == cldnn::format::bfyx;
    }
    ...
    memory::ptr generate_reference_typed(const std::vector<memory::ptr>& inputs) {
        ...
        // Output is bfyx
        auto output = engine.allocate_memory(
            cldnn::layout(input->get_layout().data_type, input->get_layout().format, input->get_layout().get_tensor()));
    ...
    static std::string custom_param_name(
        const ::testing::TestParamInfo<
            std::tuple<std::shared_ptr<tests::test_params>, std::shared_ptr<cldnn::primitive>>>& info) {
        std::stringstream res;

        const auto& p = std::get<0>(info.param);

        assert(p->data_type == data_types::f32 || p->data_type == data_types::f16);

        res << info.index << "_" << (p->data_type == data_types::f32 ? "f32" : "f16");
...
INSTANTIATE_TEST_SUITE_P(DISABLED_SOFTMAX,
                         softmax_test,
                         ::testing::Combine(::testing::ValuesIn(softmax_test::generate_generic_test_params()),
                                            ::testing::ValuesIn(softmax_test::generate_specific_test_params())),
...
template <typename T>
using SoftmaxParamsWithFormat = std::tuple<SoftmaxParams<T>,
                                           format::type,  // source (plain) layout
                                           format::type   // target (blocked) layout
                                           >;

const std::vector<format::type> formats2D{format::bfyx,
...
std::vector<SoftmaxParams<T>> generateSoftmaxParams2D() {
    const std::vector<SoftmaxParams<T>> result = {
        {0,
         tensor(3, 2, 2, 2),
         getValues<T>({0.1f, -0.1f, 0.9f, 1.5f, 3.f,  0.5f, 7.f,   12.f, 0.2f, 0.2f, -10.f, 5.2f,
                       4.f,  0.5f,  8.f,  8.2f, 0.2f, 0.2f, -10.f, 5.2f, 0.2f, 0.2f, -10.f, 5.2f}),
         getValues<T>({0.311493f,    0.270291f, 0.999963f,  0.0122108f, 0.264614f,    0.364855f,
                       0.268941f,    0.977054f, 0.344253f,  0.364855f,  1.84576e-05f, 0.493895f,
                       0.719295f,    0.364855f, 0.731059f,  0.0218575f, 0.344253f,    0.364855f,
                       1.84576e-05f, 0.493895f, 0.0160912f, 0.270291f,  1.1134e-08f,  0.00108822f})},
        {1,
         tensor(2, 3, 2, 2),
         getValues<T>({0.1f, -0.1f, 0.9f, 1.5f, 0.2f, 0.2f, -10.f, 5.2f, 0.2f, 0.2f, -10.f, 5.2f,
                       3.f,  0.5f,  7.f,  12.f, 4.f,  0.5f, 8.f,   8.2f, 0.2f, 0.2f, -10.f, 5.2f}),
         getValues<T>({0.311493f,    0.270291f,  0.999963f,  0.0122108f, 0.344253f,    0.364855f,
                       1.84576e-05f, 0.493895f,  0.344253f,  0.364855f,  1.84576e-05f, 0.493895f,
                       0.264614f,    0.364855f,  0.268941f,  0.977054f,  0.719295f,    0.364855f,
                       0.731059f,    0.0218575f, 0.0160912f, 0.270291f,  1.1134e-08f,  0.00108822f})},
        {2,
         tensor(2, 3, 2, 2),
         getValues<T>({0.1f, -0.1f, 0.9f, 1.5f, 0.2f, 0.2f, -10.f, 5.2f, 0.2f, 0.2f, -10.f, 5.2f,
                       3.f,  0.5f,  7.f,  12.f, 4.f,  0.5f, 8.f,   8.2f, 0.2f, 0.2f, -10.f, 5.2f}),
         getValues<T>({0.310026f,    0.167982f,  0.689974f, 0.832018f,   0.999963f,    0.00669285f,
                       3.71689e-05f, 0.993307f,  0.999963f, 0.00669285f, 3.71689e-05f, 0.993307f,
                       0.0179862f,   1.013e-05f, 0.982014f, 0.99999f,    0.0179862f,   0.000452622f,
                       0.982014f,    0.999547f,  0.999963f, 0.00669285f, 3.71689e-05f, 0.993307f})},
        {3,
         tensor(2, 3, 2, 2),
         getValues<T>({0.1f, -0.1f, 0.9f, 1.5f, 0.2f, 0.2f, -10.f, 5.2f, 0.2f, 0.2f, -10.f, 5.2f,
                       3.f,  0.5f,  7.f,  12.f, 4.f,  0.5f, 8.f,   8.2f, 0.2f, 0.2f, -10.f, 5.2f}),
         getValues<T>({0.549834f, 0.450166f,  0.354344f,    0.645656f, 0.5f,      0.5f,       2.50452e-07f, 1.0f,
                       0.5f,      0.5f,       2.50452e-07f, 1.0f,      0.924142f, 0.0758582f, 0.00669285f,  0.993307f,
                       0.970688f, 0.0293122f, 0.450166f,    0.549834f, 0.5f,      0.5f,       2.50452e-07f, 1.0f})},
...
std::vector<SoftmaxParams<T>> generateSoftmaxParams3D() {
    const std::vector<SoftmaxParams<T>> result = {
        {0,
         tensor(2, 3, 2, 2, 2),
         getValues<T>({0.1f, -0.1f, 0.9f,  1.5f, 0.2f, -0.2f, 0.9f,  2.5f, 0.2f, 0.2f, -10.f, 5.2f,
                       0.3f, 0.1f,  -11.f, 6.2f, 0.2f, 0.2f,  -10.f, 5.2f, 0.1f, 0.3f, -9.f,  4.2f,
                       3.f,  0.5f,  7.f,   12.f, 5.f,  0.1f,  6.f,   22.f, 4.f,  0.5f, 8.f,   8.2f,
                       2.2f, 0.3f,  6.f,   5.2f, 0.2f, 0.2f,  -10.f, 5.2f, 1.2f, 0.3f, -12.f, 2.2f}),
         getValues<T>({0.0521536f,   0.354344f,  0.00223785f, 2.75357e-05f, 0.00816257f, 0.425557f, 0.0060598f,
                       3.39827e-09f, 0.0218813f, 0.425557f,   1.523e-08f,   0.0474259f,  0.130108f, 0.450166f,
                       4.13994e-08f, 0.731059f,  0.5f,        0.5f,         0.5f,        0.5f,      0.24974f,
                       0.5f,         0.952574f,  0.880797f,   0.947846f,    0.645656f,   0.997762f, 0.999972f,
                       0.991837f,    0.574443f,  0.99394f,    1.0f,         0.978119f,   0.574443f, 1.0f,
                       0.952574f,    0.869892f,  0.549834f,   1.0f,         0.268941f,   0.5f,      0.5f,
                       0.5f,         0.5f,       0.75026f,    0.5f,         0.0474259f,  0.119203f})},
        {1,
         tensor(2, 3, 2, 2, 2),
         getValues<T>({0.1f, -0.1f, 0.9f,  1.5f, 0.2f, -0.2f, 0.9f,  2.5f, 0.2f, 0.2f, -10.f, 5.2f,
                       0.3f, 0.1f,  -11.f, 6.2f, 0.2f, 0.2f,  -10.f, 5.2f, 0.1f, 0.3f, -9.f,  4.2f,
                       3.f,  0.5f,  7.f,   12.f, 5.f,  0.1f,  6.f,   22.f, 4.f,  0.5f, 8.f,   8.2f,
                       2.2f, 0.3f,  6.f,   5.2f, 0.2f, 0.2f,  -10.f, 5.2f, 1.2f, 0.3f, -12.f, 2.2f}),
         getValues<T>({0.311493f,    0.270291f,    0.999963f,  0.0122108f,   0.332225f,    0.250089f,  0.999943f,
                       0.0213123f,   0.344253f,    0.364855f,  1.84576e-05f, 0.493895f,    0.367165f,  0.337585f,
                       6.79002e-06f, 0.862025f,    0.344253f,  0.364855f,    1.84576e-05f, 0.493895f,  0.30061f,
                       0.412327f,    5.01718e-05f, 0.116662f,  0.264614f,    0.364855f,    0.268941f,  0.977054f,
                       0.923207f,    0.290461f,    0.5f,       1.0f,         0.719295f,    0.364855f,  0.731059f,
                       0.0218575f,   0.0561403f,   0.35477f,   0.5f,         5.05653e-08f, 0.0160912f, 0.270291f,
                       1.1134e-08f,  0.00108822f,  0.0206528f, 0.35477f,     7.615e-09f,   2.5175e-09f})},
        {2,
         tensor(2, 3, 2, 2, 2),
         getValues<T>({0.1f, -0.1f, 0.9f,  1.5f, 0.2f, -0.2f, 0.9f,  2.5f, 0.2f, 0.2f, -10.f, 5.2f,
                       0.3f, 0.1f,  -11.f, 6.2f, 0.2f, 0.2f,  -10.f, 5.2f, 0.1f, 0.3f, -9.f,  4.2f,
                       3.f,  0.5f,  7.f,   12.f, 5.f,  0.1f,  6.f,   22.f, 4.f,  0.5f, 8.f,   8.2f,
                       2.2f, 0.3f,  6.f,   5.2f, 0.2f, 0.2f,  -10.f, 5.2f, 1.2f, 0.3f, -12.f, 2.2f}),
         getValues<T>({0.475021f, 0.524979f, 0.5f,      0.268941f,    0.524979f, 0.475021f, 0.5f,      0.731059f,
        ...
        {3,
         ...
         getValues<T>({0.1f, -0.1f, 0.9f,  1.5f, 0.2f, -0.2f, 0.9f,  2.5f, 0.2f, 0.2f, -10.f, 5.2f,
                       ...
                       3.f,  0.5f,  7.f,   12.f, 5.f,  0.1f,  6.f,   22.f, 4.f,  0.5f, 8.f,   8.2f,
                       2.2f, 0.3f,  6.f,   5.2f, 0.2f, 0.2f,  -10.f, 5.2f, 1.2f, 0.3f, -12.f, 2.2f}),
         getValues<T>({0.310026f,    0.167982f,    0.689974f,   0.832018f,    0.331812f,    0.0629734f,   0.668188f,
                       0.937027f,    0.999963f,    0.00669285f, 3.71689e-05f, 0.993307f,    0.999988f,    0.00223785f,
                       1.23728e-05f, 0.997762f,    0.999963f,   0.00669285f,  3.71689e-05f, 0.993307f,    0.999888f,
                       0.0198403f,   0.000111653f, 0.98016f,    0.0179862f,   1.013e-05f,   0.982014f,    0.99999f,
                       0.268941f,    3.08284e-10f, 0.731059f,   1.0f,         0.0179862f,   0.000452622f, 0.982014f,
                       0.999547f,    0.0218813f,   0.00739154f, 0.978119f,    0.992609f,    0.999963f,    0.00669285f,
                       3.71689e-05f, 0.993307f,    0.999998f,   0.130108f,    1.8506e-06f,  0.869892f})},
        {4,
         tensor(2, 3, 2, 2, 2),
         getValues<T>({0.1f, -0.1f, 0.9f,  1.5f, 0.2f, -0.2f, 0.9f,  2.5f, 0.2f, 0.2f, -10.f, 5.2f,
                       0.3f, 0.1f,  -11.f, 6.2f, 0.2f, 0.2f,  -10.f, 5.2f, 0.1f, 0.3f, -9.f,  4.2f,
                       3.f,  0.5f,  7.f,   12.f, 5.f,  0.1f,  6.f,   22.f, 4.f,  0.5f, 8.f,   8.2f,
                       2.2f, 0.3f,  6.f,   5.2f, 0.2f, 0.2f,  -10.f, 5.2f, 1.2f, 0.3f, -12.f, 2.2f}),
         getValues<T>({0.549834f,    0.450166f,   0.354344f,    0.645656f,    0.598688f,    0.401312f,   0.167982f,
                       0.832018f,    0.5f,        0.5f,         2.50452e-07f, 1.0f,         0.549834f,   0.450166f,
                       3.38949e-08f, 1.0f,        0.5f,         0.5f,         2.50452e-07f, 1.0f,        0.450166f,
                       0.549834f,    1.8506e-06f, 0.999998f,    0.924142f,    0.0758582f,   0.00669285f, 0.993307f,
                       0.992609f,    0.00739154f, 1.12535e-07f, 1.0f,         0.970688f,    0.0293122f,  0.450166f,
                       0.549834f,    0.869892f,   0.130108f,    0.689974f,    0.310025f,    0.5f,        0.5f,
                       2.50452e-07f, 1.0f,        0.710949f,    0.28905f,     6.80798e-07f, 0.999999f})}};
    return result;
}
...
struct PrintToStringParamName {
    ...
    std::string operator()(const testing::TestParamInfo<SoftmaxParamsWithFormat<T>>& param) {
        ...
        buf << "_inputTensor=" << p.input_tensor.to_string() << "_axis=" << p.axis
            << "_plainFormat=" << fmt_to_str(plain_format) << "_targetFormat=" << fmt_to_str(target_format);
        return buf.str();
    }
};
};  // namespace
...
template <typename T>
struct softmax_gpu_formats_test : public ::testing::TestWithParam<SoftmaxParamsWithFormat<T>> {
public:
    void test(bool is_caching_test) {
        ...
        for (uint32_t i = 0; i < output_ptr.size(); i++) {
            ASSERT_NEAR(output_ptr[i], params.expected[i], getError<T>())
                << "target_format=" << target_format << ", i=" << i;
        }
    }
};
...
INSTANTIATE_TEST_SUITE_P(softmax_gpu_formats_test_f32_2d,
                         ...
                         ::testing::Combine(::testing::ValuesIn(generateSoftmaxParams2D<float>()),
...
INSTANTIATE_TEST_SUITE_P(softmax_gpu_formats_test_f16_2d,
                         ...
                         ::testing::Combine(::testing::ValuesIn(generateSoftmaxParams2D<ov::float16>()),
                                            ::testing::Values(format::bfyx),
                                            ::testing::ValuesIn(formats2D)),
                         PrintToStringParamName());
...
INSTANTIATE_TEST_SUITE_P(softmax_gpu_formats_test_f32_3d,
                         softmax_gpu_formats_test_f32,
                         ::testing::Combine(::testing::ValuesIn(generateSoftmaxParams3D<float>()),
...
INSTANTIATE_TEST_SUITE_P(softmax_gpu_formats_test_f16_3d,
                         softmax_gpu_formats_test_f16,
                         ::testing::Combine(::testing::ValuesIn(generateSoftmaxParams3D<ov::float16>()),
                                            ::testing::Values(format::bfzyx),
                                            ::testing::ValuesIn(formats3D)),
                         PrintToStringParamName());
...
TEST(softmax_gpu_bfyx_f32, normalize_f_dynamic) {
    ...
    float expected_sum = 1.0f;
    int max_value_buffer_index = 0;
    for (uint32_t i = 0; i < b;
         i++) {  // this for loops will sum results in a batch per feature, we expect that: sum = 1.0f
        for (uint32_t j = 0; j < y; j++) {
            for (uint32_t k = 0; k < x; k++) {
                float sum = 0.0f;
                for (uint32_t l = 0; l < f; l++) {
                    int index = i * f * x * y + l * x * y + j * x + k;
                    if (out_buffer[index] >= temp_max) {
                        temp_max = out_buffer[index];
                    }
                    sum += out_buffer[index];
...
static void run_softmax_bfyx_opt(const int64_t b,
                                 const int64_t f,
                                 const int64_t y,
                                 const int64_t x,
                                 const uint64_t axis) {
    ...
    config.set_property(
        ov::intel_gpu::force_implementations(ov::intel_gpu::ImplForcingMap{{"softmax", softmax_bf_kernel}}));

    const int64_t buf_size = b * f * y * x;
    auto input_layout_dynamic =
        layout{ov::PartialShape{ov::Dimension::dynamic(), f, ov::Dimension::dynamic(), ov::Dimension::dynamic()},
               data_types::f16,
               format::bfyx};
    ...
    ov::reference::softmax<ov::float16>(input_data.data(),
                                        output_ref.data(),
                                        input_layout_static.get_shape(),
                                        ov::AxisSet{axis});
    ASSERT_NE(output, nullptr);
    const float threshold_fp16 = 1e-1;
    cldnn::mem_lock<ov::float16> output_ptr(output, get_test_stream());
    for (size_t idx = 0; idx < static_cast<size_t>(buf_size); idx++) {
        ASSERT_NEAR(float(output_ptr[idx]), float(output_ref[idx]), threshold_fp16)
            << idx << ", " << std::fixed << setprecision(8) << output_ptr[idx] << " vs " << output_ref[idx];
    }
}
