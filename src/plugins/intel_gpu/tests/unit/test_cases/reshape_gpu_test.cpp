...
void generic_reshape_test(format fmt, tensor const& input_size, tensor const& reshape_size,
    ...
    ASSERT_TRUE(tensor(default_fmt, output->get_layout().get_padded_dims()) ==
        reshape_size.add(tensor(default_fmt, output_padd.lower_size(output_fmt.dimension()), 0)).add(tensor(default_fmt, output_padd.upper_size(output_fmt.dimension()), 0)));

    {
        ...
        auto lower = tensor(default_fmt, output_padd.lower_size(output_fmt.dimension()), 0).sizes(fmt);
        auto upper = tensor(default_fmt, output_padd.upper_size(output_fmt.dimension()), 0).sizes(fmt);
