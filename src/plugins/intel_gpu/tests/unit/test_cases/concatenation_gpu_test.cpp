...
TEST(concat_gpu, mixed_input_types) {
    ...
    VF<float> output_vec = {1.0f,  2.0f,  3.0f,  4.0f,  2.0f,  2.0f,  3.0f,  4.0f,  3.0f,  3.0f,  3.0f,  5.0f,
...
void start_concat_test_dynamic(impl_types impl_type) {
    ...
    if (impl_type != impl_types::any) {
        ...
        config.set_property(
            ov::intel_gpu::force_implementations(ov::intel_gpu::ImplForcingMap{{primitive_id("concat"), force_impl}}));
    }
    ...
    auto run_on_shapes = [&](layout layout0, layout layout1, layout layout2, layout layout3) {
        ...
        expected_shape[1] = layout0.get_partial_shape()[1] + layout1.get_partial_shape()[1] +
                            layout2.get_partial_shape()[1] + layout3.get_partial_shape()[1];
...
TEST(concat_gpu, pooling_dynamic_input_no_exception) {
    ...
    set_values(input_grid,
               {13, 13, 13, 13, 15, 15, 16, 15, 16, 14, 13, 14, 13, 14, 13, 18, 16, 18, 16, 15, 16, 15, 18, 14});

    set_values<float>(
        input0,
        {
            11, 12, 13, 14, 12, 12, 13, -14, 13, 13, -13, 15, 16, -16, -13, -14, 12, 11, 16, -14, -13, 18, -13, -15,
        });
    set_values<float>(input1,
                      {11, 12, 13, 15, 12, 12, 13, 14, 12, 13, 13, 15, 12, 14, 13, 14, 17, 18, 13, 14, 11, 13, 13, 15});

    GridSampleOp::Attributes attributes(false,
...
TEST(concat_gpu, i8_optimization_with_pool) {
    ...
    set_values<int8_t>(input1, {11, 12, 13, 15, 12, 12, 13, 14, 12, 13, 13, 15,
                                12, 14, 13, 14, 17, 18, 13, 14, 11, 13, 13, 15});

    VF<int8_t> output_vec = {13, 13, 13, 13, 15, 15, 16, 15, 16, 14, 13, 14, 13,  14,
                             13, 18, 16, 18, 16, 15, 16, 15, 18, 14, 18, 14, -13, 15};
...
TEST(concat_gpu, i8_optimization_with_conv) {
    ...
    set_values<int8_t>(weights, {1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2});

    set_values<int8_t>(input0, {1, 2, 3, -4, 5, 2, 2, 3, 4, -6, -3, 3, 3, 5, 1, -1, 1, 1, 1, -1});
    set_values<int8_t>(input1, {5, 5, 3, -4, 5, 2, -2, 5, 4, 6, 6, 1, 3, 5, 1, 1, 2, -3, -4, 5});
    set_values<int8_t>(input2, {-2, 1, 3, 2, -5, 1, 2, -2, 4, 2, 3, 5, 3, -3, 1, 5, 4, 3, 2, 1});
...
TEST(concat_gpu, i8_optimization_with_pool_conv) {
    ...
    set_values<int8_t>(weights, {-1, 2, -2, 2,  -2, 1, 1, 0,  -1, 1, 2, -2, 2,  1,  -2, 0,  0,  -2, -2, -2, -2, -1,
                                 2,  1, 2,  -1, -1, 0, 2, -2, -2, 1, 0, -2, 0,  1,  -2, -1, -2, 0,  -1, -1, -2, 1,
                                 -2, 0, 1,  2,  2,  2, 2, -2, 0,  2, 1, -2, -1, -1, 0,  -2, 2,  -1, 2,  -1});

    set_values<int8_t>(input0,
                       {-3, 6,  0, 2, -1, -1, 6,  0,  5, 4, 1,  6,  2, 4,  0,  5, -2, -1, 1,  0,  2, 3,  3,  3,
                        6,  2,  4, 7, 3,  6,  7,  -1, 7, 7, 5,  -3, 1, -1, 5,  4, 0,  3,  -2, 6,  2, 5,  2,  4,
                        5,  -1, 3, 6, 2,  0,  -3, -1, 0, 3, 0,  -1, 1, 6,  1,  6, 5,  -2, 2,  -1, 5, 6,  3,  4,
                        1,  0,  6, 6, 7,  2,  6,  3,  6, 7, -1, 5,  5, 6,  -1, 0, -1, 5,  5,  2,  3, -1, -3, 4});

    set_values<int8_t>(input1,
                       {4,  -2, 0,  0,  6, 2,  0,  4,  6, 4, 4,  4,  -3, -1, 4,  -3, 1,  0,  -1, 5,  -1, 1,  4,  2,
                        7,  7,  0,  2,  3, 4,  -1, 3,  7, 7, 2,  -3, -1, 5,  -2, 2,  6,  -3, 0,  7,  0,  3,  3,  3,
                        -1, 0,  -2, -2, 7, -3, -3, -1, 5, 0, 3,  4,  0,  -1, 2,  5,  2,  -1, 2,  -3, 0,  -3, -3, 2,
                        4,  3,  3,  5,  5, 7,  5,  1,  2, 2, -3, 6,  6,  7,  1,  -1, -2, 5,  1,  -1, 4,  5,  -3, -2});
...
TEST(concat_gpu, no_exception_in_input_order_opt_b_fs_yx_fsv16_with_conv_port2) {
    ...
    topology topology(
        input_layout("concat_input0", concat_input0->get_layout()),
        ...
        concatenation("concat",
                      {input_info("concat_input0"),
                       input_info("concat_input1"),
                       input_info("concat_input2"),
                       input_info("concat_input3")},
                      1,
                      data_types::f32,
                      padding{{0, 0, 0, 0}, 0}),
...
struct concat_gpu : public ::testing::TestWithParam<TestParamType_concat> {
    ...
    static std::string PrintToStringParamName(testing::TestParamInfo<TestParamType_concat> param_info) {
        std::string in;
        ...
        return "in" + std::to_string(testing::get<0>(param_info.param)) + "x" + in + "x" +
               std::to_string(testing::get<2>(param_info.param)) + 'x' +
               std::to_string(testing::get<3>(param_info.param)) + "is_caching_test" +
               std::to_string(testing::get<4>(param_info.param));
    }
};
...
struct concat_axis3_gpu : public ::testing::TestWithParam<TestParamType_concat_axis3> {
    ...
    static std::string PrintToStringParamName(testing::TestParamInfo<TestParamType_concat_axis3> param_info) {
        std::string in;
        ...
        return "in" + std::to_string(testing::get<0>(param_info.param)) + "x" + in + "x" +
               ...
               std::to_string(testing::get<2>(param_info.param));
    }
};
                ...
                for (size_t fi = 0; fi < in_features[in_i]; fi++) {
                    for (size_t yi = 0; yi < input_y; yi++) {
                        for (size_t xi = 0; xi < input_x; xi++) {
                            ...
                            ASSERT_EQ(ref_val, actual_val) << " b=" << bi << ", f=" << f_sum + fi << "(input " << in_i
                                                           << "), y=" << yi << ", x=" << xi;
                        }
                    }
                }
                ...
                for (size_t yi = 0; yi < input_y; yi++) {
                    ...
                    for (size_t in_i = 0; in_i < input_x.size(); in_i++) {
                        for (size_t xi = 0; xi < input_x[in_i]; xi++) {
                            ...
                            auto ref_val = in_data[in_i][bi][fi][yi][xi];
                            auto actual_val = out_ptr[output_offset];
                            ASSERT_EQ(ref_val, actual_val) << " b=" << bi << ", f=" << fi << ", y=" << yi
                                                           << ", x=" << x_sum + xi << "(input " << in_i << ")";
                        }
                        x_sum += input_x[in_i];
                    }
                }
...
INSTANTIATE_TEST_SUITE_P(smoke, concat_gpu_4d_f16, concat_gpu_all_params, concat_gpu::PrintToStringParamName);
...
INSTANTIATE_TEST_SUITE_P(smoke,
                         ...
                         ::testing::Values(TestParamType_concat_axis3(2, 16, 2, {2, 3}),
...
public:
    void test(format::type fmt) {
        ...
        topology.add(
            convolution("conv", input_info("concat"), "weights", "", 1, {1, 1}, {1, 1}, {0, 0}, {0, 0}, false));

        ExecutionConfig config = get_test_default_config(engine);
        config.set_property(ov::intel_gpu::optimize_data(true));
        auto conv_forcing = ov::intel_gpu::ImplementationDesc{fmt, std::string()};
        config.set_property(
            ov::intel_gpu::force_implementations(ov::intel_gpu::ImplForcingMap{{primitive_id("conv"), conv_forcing}}));
        network network(engine, topology, config);
...
INSTANTIATE_TEST_SUITE_P(smoke_low_precision,
                         ...
                         ::testing::Values(TestParamType_concat(2, {2, 32}, 2, 1, false),
...
public:
    cldnn::memory::ptr run_concat_network(std::vector<std::vector<std::vector<std::vector<std::vector<Type>>>>> input,
        ...
        for (size_t i = 0; i < in_features.size(); i++) {
            ...
            topology.add(input_layout("input" + std::to_string(i), in_lay));
            topology.add(pooling("pool" + std::to_string(i),
                                 input_info("input" + std::to_string(i)),
                                 pooling_mode::max,
                                 {1, 1},
                                 {1, 1}));
        ...
        topology.add(reorder("reorder",
                             input_info("pool_final"),
                             layout(data_type,
                                    format::bfyx,
                                    {(int32_t)batch_num, (int32_t)output_f, (int32_t)input_y, (int32_t)input_x})));

        cldnn::network::ptr concat_network =
            get_network(engine, topology, config, get_test_stream_ptr(), is_caching_test);
        ...
        bool concat_opt_result =
            std::static_pointer_cast<concatenation_inst>(concat_network->get_primitive("concat"))->can_be_optimized();
...
INSTANTIATE_TEST_SUITE_P(smoke,
                         ...
                         ::testing::Values(TestParamType_concat(1, {16, 16}, 2, 2, false),
...
INSTANTIATE_TEST_SUITE_P(export_import,
                         concat_implicit_gpu_4d_f16,
                         ::testing::Values(TestParamType_concat(1, {8, 16}, 2, 2, true)),
                         concat_gpu::PrintToStringParamName);
...
public:
    cldnn::memory::ptr run_concat_network(std::vector<std::vector<std::vector<std::vector<std::vector<Type>>>>> input,
        ...
        bool concat_opt_enabled = config.get_property(ov::intel_gpu::optimize_data);
        bool concat_opt_result = std::static_pointer_cast<concatenation_inst>(concat_network.get_primitive("concat"))
...
INSTANTIATE_TEST_SUITE_P(smoke,
                         ...
                         ::testing::Values(TestParamType_concat(1, {32, 32}, 2, 2, false),
...
public:
    cldnn::memory::ptr run_concat_network(std::vector<std::vector<std::vector<std::vector<std::vector<Type>>>>> input,
        ...
        topology.add(data("weights", weights_mem));
        topology.add(
            convolution("conv", input_info("eltwise2"), "weights", "", 1, {1, 1}, {1, 1}, {0, 0}, {0, 0}, false));
        ...
        topology.add(
            reorder("reorder",
                    ...
                    layout(data_types::f32,
                           format::bfyx,
                           {(int32_t)batch_num, (int32_t)(output_f * 2), (int32_t)input_y, (int32_t)input_x})));
        ...
        if (concat_opt_enabled && batch_num > 1)
            concat_opt_result = !concat_opt_result;
        EXPECT_EQ(concat_opt_enabled, concat_opt_result);
...
INSTANTIATE_TEST_SUITE_P(smoke,
                         ...
                         ::testing::Values(TestParamType_concat(1, {16}, 2, 2, false),
