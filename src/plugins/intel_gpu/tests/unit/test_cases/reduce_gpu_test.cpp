    ...
    void remap(std::vector<size_t>& out_dims,
        ...
        for (int index = 0; index < static_cast<int>(out_dims.size()); index++) {
            ...
            auto index_to_remove =
                std::find(axis_to_remove.begin(), axis_to_remove.end(), axes_map.find(index)->second) !=
                axis_to_remove.end();
        ...
        for (auto& coord : coords) {
            ...
            remap_coords[coord] =
                static_cast<int>(std::distance(coords.begin(), std::find(coords.begin(), coords.end(), temp_coords)));
            ++j;
        }
...
template <typename InputT,
          typename AccT = typename accumulator_type<InputT>::type,
          typename OutputT = typename output_type<InputT>::type>
VVVVVVF<OutputT> reference_reduce(VVVVVVF<InputT>& input,
    ...
    VVVVVVF<AccT> previous(
        input_dims[0],
        VVVVVF<AccT>(input_dims[1],
                     VVVVF<AccT>(input_dims[2],
                                 VVVF<AccT>(input_dims[3], VVF<AccT>(input_dims[4], VF<AccT>(input_dims[5], 0))))));
                        ...
                        for (size_t yi = 0; yi < temp_dims[4]; ++yi) {
                            for (size_t xi = 0; xi < temp_dims[5]; ++xi) {
                                ...
                                AccT acc = static_cast<AccT>(
                                    temp_output[bi % out_dims[0]][fi % out_dims[1]][wi % out_dims[2]][zi % out_dims[3]]
                                               [yi % out_dims[4]][xi % out_dims[5]]);

                                temp_output[bi % out_dims[0]][fi % out_dims[1]][wi % out_dims[2]][zi % out_dims[3]]
                                           [yi % out_dims[4]][xi % out_dims[5]] =
                                               reduce.accumulate(acc,
                                                                 input_val,
                                                                 reduce_mode,
                                                                 &axis != &reduce_axis.front());
                            }
                        }
    ...
    if (keepDims) {
        ...
        VVVVVVF<AccT> actual_output(
            actual_dims[0],
            VVVVVF<AccT>(
                actual_dims[1],
                VVVVF<AccT>(actual_dims[2],
                            VVVF<AccT>(actual_dims[3], VVF<AccT>(actual_dims[4], VF<AccT>(actual_dims[5], 0))))));
                        ...
                        for (size_t yi = 0; yi < previous[0][0][0][0].size(); ++yi)
                            for (size_t xi = 0; xi < previous[0][0][0][0][0].size(); ++xi) {
                                ...
                                actual_output[coords.at(remap_coords['b'])][coords.at(remap_coords['f'])]
                                             ...
                                             [coords.at(remap_coords['y'])][coords.at(remap_coords['x'])] =
                                                 previous[bi][fi][wi][zi][yi][xi];
                            }

        output = std::move(actual_output);
    }

    VVVVVVF<OutputT> final_output(
        output.size(),
...
using TestParamType_general_reduce_gpu = ::testing::tuple<int,
                                                          int,
                                                          int,  // 0, 1, 2  -  b, f, w
                                                          int,
                                                          int,
...
struct general_reduce_gpu : public ::testing::TestWithParam<TestParamType_general_reduce_gpu> {
    static std::string PrintToStringParamName(testing::TestParamInfo<TestParamType_general_reduce_gpu> param_info) {
        ...
        for (auto& axis : reduce_axes)
            string_axes += std::to_string(axis) + "_";

        // Readable name
        return "in_b_" + std::to_string(testing::get<0>(param_info.param)) + "_f_" +
               std::to_string(testing::get<1>(param_info.param)) + "_w_" +
               std::to_string(testing::get<2>(param_info.param)) + "_z_" +
               std::to_string(testing::get<3>(param_info.param)) + "_y_" +
               std::to_string(testing::get<4>(param_info.param)) + "_x_" +
               std::to_string(testing::get<5>(param_info.param)) + "_format_" +
               std::to_string(testing::get<6>(param_info.param)) + "_reduce_mode_" +
               std::to_string(
                   static_cast<std::underlying_type<cldnn::reduce_mode>::type>(testing::get<7>(param_info.param))) +
               "_axes_" + string_axes + "_kernel_name_" + testing::get<9>(param_info.param) + "_keepDims_" +
               std::to_string(testing::get<10>(param_info.param));
    }
};
...
public:
    void execute(bool is_caching_test) {
        ...
        auto reference_result = reference_reduce(input_data,
                                                 reduce_mode,
                                                 reduce_axis,
                                                 batch_num,
                                                 input_f,
        ...
        config.set_property(
            ov::intel_gpu::force_implementations(ov::intel_gpu::ImplForcingMap{{"reduce", reduce_impl}}));
...
INSTANTIATE_TEST_SUITE_P(reduce_gpu_b_fs_yx_fsv16_i8_i8,
                         ...
                         ::testing::Values(TestParamType_general_reduce_gpu(2,
                                                                            ...
                                                                            {2, 3},
                                                                            "reduce_gpu_b_fs_yx_fsv16",
                                                                            false,
                                                                            data_types::i8,
                                                                            true,
                                                                            data_types::i8),
                                           TestParamType_general_reduce_gpu(2,
                                                                            3,
                                                                            1,
                                                                            1,
                                                                            8,
                                                                            ...
                                                                            true,
                                                                            data_types::i8),
                                           TestParamType_general_reduce_gpu(3,
                                                                            3,
                                                                            1,
                                                                            1,
                                                                            3,
                                           ...
                                           TestParamType_general_reduce_gpu(3,
                                                                            ...
                                                                            true,
                                                                            data_types::i8),
                                           TestParamType_general_reduce_gpu(1,
                                                                            3,
                                                                            1,
                                                                            1,
                                                                            6,
...
INSTANTIATE_TEST_SUITE_P(reduce_gpu_b_fs_yx_fsv16_i8_f32,
                         ...
                         ::testing::Values(TestParamType_general_reduce_gpu(3,
                                                                            3,
                                                                            1,
                                                                            1,
                                                                            3,
                                           ...
                                           TestParamType_general_reduce_gpu(3,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(7,
                                                                            3,
                                                                            1,
                                                                            1,
                                                                            13,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(26,
                                                                            12,
                                                                            1,
                                                                            1,
                                                                            16,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(17,
                                                                            12,
                                                                            1,
                                                                            1,
                                                                            13,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(16,
                                                                            4,
                                                                            1,
                                                                            1,
                                                                            16,
...
INSTANTIATE_TEST_SUITE_P(reduce_gpu_b_fs_yx_fsv16_f32_f32,
                         ...
                         ::testing::Values(TestParamType_general_reduce_gpu(7,
                                                                            3,
                                                                            1,
                                                                            1,
                                                                            13,
...
INSTANTIATE_TEST_SUITE_P(reduce_gpu_ref_f32_f32,
                         ...
                         ::testing::Values(TestParamType_general_reduce_gpu(2,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(6,
                                                                            16,
                                                                            6,
                                                                            3,
                                                                            8,
                                           ...
                                           TestParamType_general_reduce_gpu(3,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(8,
                                                                            4,
                                                                            1,
                                                                            7,
                                                                            2,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(4,
                                                                            5,
                                                                            1,
                                                                            7,
                                                                            12,
                                           ...
                                           TestParamType_general_reduce_gpu(6,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(5,
                                                                            5,
                                                                            1,
                                                                            1,
                                                                            17,
...
INSTANTIATE_TEST_SUITE_P(DISABLED_reduce_gpu_ref_f32_f32,
                         general_reduce_gpu_f32_f32,
                         ::testing::Values(TestParamType_general_reduce_gpu(1,
                                                                            7,
                                                                            1,
                                                                            1,
                                                                            4,
                                           ...
                                           TestParamType_general_reduce_gpu(1,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(12,
                                                                            2,
                                                                            1,
                                                                            1,
                                                                            4,
                                           ...
                                           TestParamType_general_reduce_gpu(7,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(9,
                                                                            7,
                                                                            1,
                                                                            1,
                                                                            4,
                                           ...
                                           TestParamType_general_reduce_gpu(9,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(11,
                                                                            10,
                                                                            1,
                                                                            1,
                                                                            4,
                                           ...
                                           TestParamType_general_reduce_gpu(12,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(13,
                                                                            7,
                                                                            1,
                                                                            1,
                                                                            4,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(14,
                                                                            7,
                                                                            1,
                                                                            1,
                                                                            4,
                                                                            ...
                                                                            false,
                                                                            data_types::f32),
                                           TestParamType_general_reduce_gpu(18,
                                                                            7,
                                                                            1,
                                                                            1,
                                                                            4,
...
TEST(reduce_gpu, b_fs_yx_fsv16_min_dynamic) {
    ...
    set_values(input, {1.0f, -1.0f, 2.0f, -2.0f, 3.0f, -3.0f, 4.0f, -4.0f, 5.0f, -5.0f, 6.0f, -6.0f,
                       7.0f, -7.0f, 8.0f, -8.0f, 9.0f, -9.0f, 8.0f, -8.0f, 7.0f, -7.0f, 6.0f, -6.0f,
                       5.0f, -5.0f, 4.0f, -4.0f, 3.0f, -3.0f, 2.0f, -2.0f, 1.0f, -1.0f});
...
public:
    void execute(bool is_caching_test) {
        ...
        auto input_size = tensor(batch(batch_num), feature(input_f), spatial(input_x, input_y, input_z, input_w));
        auto input_data =
            rg.generate_random_6d<input_t>(batch_num, input_f, input_x, input_y, input_z, input_w, 1, 5, 9);
        auto input_lay = layout(input_dt, layout_format, input_size);
        auto input_mem = engine.allocate_memory(input_lay);
        ...
        std::vector<cldnn::reduce_mode> modes{cldnn::reduce_mode::max,
                        ...
                        for (size_t zi = 0; zi < reference_result[0][0][0].size(); zi++)
                            for (size_t yi = 0; yi < reference_result[0][0][0][0].size(); yi++) {
                                for (size_t xi = 0; xi < reference_result[0][0][0][0][0].size(); xi++) {
                                    ...
                                    auto val = out_ptr[offset];
                                    auto val_ref = static_cast<output_t>(reference_result[bi][fi][wi][zi][yi][xi]);
                                    bool equal = need_adjust_threshold ? are_equal(val_ref, val, 1e-1f, 1.0f, 10.0f)
                                                                       : are_equal(val_ref, val, 1e-1f);
                    ...
                    for (size_t zi = 0; zi < reference_result[0][0][0].size(); zi++)
                        for (size_t yi = 0; yi < reference_result[0][0][0][0].size(); yi++) {
                            for (size_t xi = 0; xi < reference_result[0][0][0][0][0].size(); xi++) {
                                ...
                                if (!equal)
                                    std::cout << "Reference value at batch: " << bi << " output_f: " << fi
                                              << " y: " << yi << " x: " << xi << " = " << static_cast<float>(val_ref)
                                              << " Val = " << static_cast<float>(val) << std::endl;
...
TEST_P(general_reduce_gpu_i8_i8, base_cached) {
    execute(true);
}

TEST_P(general_reduce_gpu_i8_f32, base_cached) {
    execute(true);
}

TEST_P(general_reduce_gpu_f32_f32, base_cached) {
    execute(true);
}

TEST_P(general_reduce_gpu_xy_f32, base_cached) {
    execute(true);
}

TEST_P(general_reduce_gpu_xy_i8, base_cached) {
    execute(true);
}
