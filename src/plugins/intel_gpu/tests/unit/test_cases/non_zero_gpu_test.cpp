...
void test_count_non_zero(layout in_layout, std::vector<T> in_data) {
    ...
    auto count_non_zero = ngraph::reference::non_zero_get_count<T>(in_data.data(), in_layout.get_shape());
...
TEST(test_count_non_zero, dynamic_2d_f32_bfyx) {
    ...
    for (size_t& input_length : input_shapes) {
        ...
        auto count_non_zero = ngraph::reference::non_zero_get_count<float>(in_data.data(), in_layout.get_shape());
        ASSERT_EQ(count_non_zero, output_ptr[0]);
    }
}
...
void test_gather_non_zero(layout in_layout, std::vector<T> in_data) {
    ...
    ngraph::reference::non_zero<T, int32_t>(in_data.data(), expected_results.data(), in_layout.get_shape());
...
TEST(test_gather_non_zero, not_use_local_mem) {
    ...
    ngraph::reference::non_zero<float, int32_t>(in_data.data(), expected_results.data(), in_layout.get_shape());
