...
public:
    ...
    void FillWithBasicBfyxPositiveStepAxesLessThanRankData(SliceTestParams& params) {
        ...
        params.input = this->template AllocateTensor<TypeParam>(input_shape,
                                                                format::bfyx,
                                                                helpers::GenInput<TypeParam>(input_shape));
        params.start = this->template AllocateTensor<int64_t>(ov::PartialShape{3}, format::bfyx, {1, 1, 0});
        params.stop = this->template AllocateTensor<int64_t>(ov::PartialShape{3}, format::bfyx, {2, 120, 5});
        params.step = this->template AllocateTensor<int64_t>(ov::PartialShape{3}, format::bfyx, {1, 10, 1});
        params.axes = this->template AllocateTensor<int64_t>(ov::PartialShape{3}, format::bfyx, {1, 3, 2});
        params.wanted_output = this->template AllocateTensor<TypeParam>(
            ov::PartialShape{1, 1, 5, 10},
            format::bfyx,
            {
                1201, 1211, 1221, 1231, 1241, 1251, 1261, 1271, 1281, 1291, 1301, 1311, 1321, 1331, 1341, 1351, 1361,
                1371, 1381, 1391, 1401, 1411, 1421, 1431, 1441, 1451, 1461, 1471, 1481, 1491, 1501, 1511, 1521, 1531,
                1541, 1551, 1561, 1571, 1581, 1591, 1601, 1611, 1621, 1631, 1641, 1651, 1661, 1671, 1681, 1691,
            });
    }
...
TYPED_TEST(SliceTest, bfyx_negative_step) {
    ...
    const ov::PartialShape input_shape{1, 2, 12, 100};
    params.input =
        this->template AllocateTensor<TypeParam>(input_shape, format::bfyx, helpers::GenInput<TypeParam>(input_shape));
    params.start = this->template AllocateTensor<int64_t>(ov::PartialShape{4}, format::bfyx, {0, 1, 5, 90});
    params.stop = this->template AllocateTensor<int64_t>(ov::PartialShape{4}, format::bfyx, {1, 0, 0, 10});
    params.step = this->template AllocateTensor<int64_t>(ov::PartialShape{4}, format::bfyx, {1, -1, -1, -10});
    params.wanted_output = this->template AllocateTensor<TypeParam>(
        ov::PartialShape{1, 1, 5, 8},
        format::bfyx,
        {1789, 1779, 1769, 1759, 1749, 1739, 1729, 1719, 1689, 1679, 1669, 1659, 1649, 1639,
         1629, 1619, 1589, 1579, 1569, 1559, 1549, 1539, 1529, 1519, 1489, 1479, 1469, 1459,
         1449, 1439, 1429, 1419, 1389, 1379, 1369, 1359, 1349, 1339, 1329, 1319});
...
TYPED_TEST(SliceTest, bfzyx) {
    ...
    params.input =
        this->template AllocateTensor<TypeParam>(input_shape, format::bfzyx, helpers::GenInput<TypeParam>(input_shape));
    params.start = this->template AllocateTensor<int64_t>(ov::PartialShape{5}, format::bfzyx, {0, 0, 0, 0, 0});
    params.stop = this->template AllocateTensor<int64_t>(ov::PartialShape{5}, format::bfzyx, {1, 2, 2, 2, 2});
    params.step = this->template AllocateTensor<int64_t>(ov::PartialShape{5}, format::bfzyx, {1, 1, 1, 1, 1});
    params.wanted_output =
        this->template AllocateTensor<TypeParam>(ov::PartialShape{1, 2, 2, 2, 2},
                                                 format::bfzyx,
                                                 {0, 1, 5, 6, 60, 61, 65, 66, 600, 601, 605, 606, 660, 661, 665, 666});
