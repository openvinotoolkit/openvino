...
std::vector<GenerateProposalsParams<T>> getGenerateProposalsParams() {
    std::vector<GenerateProposalsParams<T>> params = {
        {1.0f,
         ...
         true,
         1.0,
         getValues<T>({4.49132, 4.30537, 8.75027, 8.8035,  0,       1.01395, 4.66909, 5.14337,
                       135.501, 137.467, 139.81,  141.726, 4.49132, 4.30537, 8.75027, 8.8035,
                       0,       1.01395, 4.66909, 5.14337, 135.501, 137.467, 139.81,  141.726}),
         getValues<T>({0.826641, 0.566372, 0.559075, 0.826641, 0.566372, 0.559075}),
         {3, 3}},
        {1.0f,
         0.7f,
         1000,
         ...
         true,
         1.0,
         getValues<T>({4.49132, 4.30537, 8.75027, 8.8035,  0,       1.01395, 4.66909, 5.14337, 135.501, 137.467,
                       139.81,  141.726, 47.2348, 47.8342, 52.5503, 52.3864, 126.483, 128.3,   131.625, 133.707,
                       4.49132, 4.30537, 8.75027, 8.8035,  0,       1.01395, 4.66909, 5.14337, 135.501, 137.467,
                       139.81,  141.726, 47.2348, 47.8342, 52.5503, 52.3864, 126.483, 128.3,   131.625, 133.707}),
         getValues<T>(
             {0.826641, 0.566372, 0.559075, 0.479851, 0.267999, 0.826641, 0.566372, 0.559075, 0.479851, 0.267999}),
         {5, 5}},
        {0.0f,
         0.7f,
         14,
         ...
         true,
         1.0,
         getValues<T>({108.129, 109.37,  111.623, 111.468, 12.9725, 11.6102, 16.4918, 16.9624, 112.883, 113.124,
                       115.17,  118.213, 4.49132, 4.30537, 8.75027, 8.8035,  24.9778, 25.0318, 27.2283, 28.495,
                       100.126, 101.409, 102.354, 106.125, 108.129, 109.37,  111.623, 111.468, 12.9725, 11.6102,
                       16.4918, 16.9624, 112.883, 113.124, 115.17,  118.213, 4.49132, 4.30537, 8.75027, 8.8035,
                       24.9778, 25.0318, 27.2283, 28.495,  100.126, 101.409, 102.354, 106.125}),
         getValues<T>({0.922952,
                       0.90457,
                       0.886462,
                       0.826641,
                       0.698273,
        ...
        {1.0f,
         ...
         false,
         1.0,
         getValues<T>({13.4588, 10.9153, 17.7377, 17.9436, 4.73698, 3.95806, 10.1254, 9.70525, 89.5773, 90.0053,
                       92.9476, 95.3396, 0,       1.02093, 6.00364, 6.21505, 92.3608, 94.306,  96.3198, 98.4288,
                       135.252, 137.7,   140.716, 143.09,  13.4588, 10.9153, 17.7377, 17.9436, 4.73698, 3.95806,
                       10.1254, 9.70525, 89.5773, 90.0053, 92.9476, 95.3396, 0,       1.02093, 6.00364, 6.21505,
                       92.3608, 94.306,  96.3198, 98.4288, 135.252, 137.7,   140.716, 143.09}),
         getValues<T>({0.90457,
                       0.826641,
                       0.657244,
                       0.566372,
        ...
        {0.0f,
         ...
         false,
         1.0,
         getValues<T>({108.194, 109.556, 112.435, 111.701, 13.4588, 10.9153, 17.7377, 17.9436, 113.324, 113.186,
                       115.755, 119.82,  4.73698, 3.95806, 10.1254, 9.70525, 25.4666, 25.0477, 27.8424, 29.2425,
                       100.188, 101.614, 102.532, 107.687, 108.194, 109.556, 112.435, 111.701, 13.4588, 10.9153,
                       17.7377, 17.9436, 113.324, 113.186, 115.755, 119.82,  4.73698, 3.95806, 10.1254, 9.70525,
                       25.4666, 25.0477, 27.8424, 29.2425, 100.188, 101.614, 102.532, 107.687}),
         getValues<T>({0.922952,
                       0.90457,
                       0.886462,
                       0.826641,
...
struct generate_proposals_test : public ::testing::TestWithParam<GenerateProposalsParamsWithLayout<T>> {
public:
    void test() {
        ...
        const auto input_im_info =
            engine.allocate_memory({data_type, format::bfyx, tensor{batch(num_batches), feature(3)}});
        ...
        const cldnn::mem_lock<T> roi_scores_ptr(get_plane_data(output_roi_scores, data_type, rois_scores_layout),
                                                *stream);
        ASSERT_EQ(roi_scores_ptr.size(), num_batches * param.post_nms_count);

        const cldnn::mem_lock<ROIS_NUM_T> rois_num_ptr(get_plane_data(output_rois_num, rois_num_type, rois_num_layout),
                                                       *stream);
        ...
        for (auto i = 0; i < param.post_nms_count; ++i) {
            ...
            if (static_cast<float>(expected_roi_scores[i]) != 0.0f) {
                for (size_t coord = 0; coord < 4; ++coord) {
                    ...
                    ASSERT_NEAR(expected_rois[roi_idx], rois_ptr[roi_idx], getError<T>())
                        << "i=" << i << ", coord=" << coord;
                }
            }
        }
...
INSTANTIATE_TEST_SUITE_P(generate_proposals_gpu_test,
                         f32_i32,
                         ::testing::Combine(::testing::ValuesIn(getGenerateProposalsParams<float>()),
...
INSTANTIATE_TEST_SUITE_P(generate_proposals_gpu_test,
                         f16_i32,
                         ::testing::Combine(::testing::ValuesIn(getGenerateProposalsParams<ov::float16>()),
                                            ::testing::ValuesIn(layouts),
                                            ::testing::Values(false)));
...
INSTANTIATE_TEST_SUITE_P(export_import_generate_proposals_gpu_test,
                         f16_i64,
                         ::testing::Combine(::testing::Values(getGenerateProposalsParams<ov::float16>()[0]),
