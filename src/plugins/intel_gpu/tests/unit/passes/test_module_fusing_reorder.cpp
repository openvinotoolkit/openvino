...
TEST(test_can_fuse_reorder, reorder_for_mixed_type_convolution_fsv32_onednn) {
    ...
    topology.add(cldnn::convolution("conv",
    ...
    while (itr != prog->get_processing_order().end()) {
        ...
        for (auto usr : node_ptr->get_users()) {
            ASSERT_EQ(
                false,
                lo.can_fuse_reorder(input, *usr, node.get_input_layout().format, usr->get_output_layout().format));
        }
    }
}
...
TEST_P(test_fused_reorder_deep_depth, no_removal_for_deep_depth_conv) {
    ...
    layout conv_layout(p.input_data_type,
                       p.output_format,
                       p.out_shape,
                       padding(
                           {
                           ...
                           0));
    layout reorder_layout(p.output_data_type,
                          p.output_format,
                          p.out_shape,
                          padding(
                              {
...
INSTANTIATE_TEST_SUITE_P(testing_deep_depth_conv,
                         test_fused_reorder_deep_depth,
                         ::testing::ValuesIn(std::vector<reorder_test_param>{
                             reorder_test_param{format::bfyx,
                                                format::b_fs_yx_fsv32,
                                                data_types::u8,
                                                data_types::u8,
                                                {1, 32, 8, 8},
...
// To test removal of reorder for first convolution optimizing in cldnn kernel (shallow input depth to deep output
// depth)
...
TEST_P(test_can_fuse_reorder_cldnn, reorder_for_firstconv_cldnn) {
    ...
    topology.add(data("bias", bias));
    topology.add(reorder("reorder_input", input_info("input"), p.output_format, p.input_data_type));
    topology.add(cldnn::convolution("conv2",
                                    input_info("reorder_input"),
                                    "weights",
                                    "bias",
                                    1,
...
INSTANTIATE_TEST_SUITE_P(testing_can_fuse_reorder_first_conv,
                         test_can_fuse_reorder_cldnn,
                         ::testing::ValuesIn(std::vector<reorder_test_param>{
                             reorder_test_param{format::bfyx,
                                                format::b_fs_yx_fsv32,
                                                data_types::u8,
...
// To test removal of reorder for first convolution optimizing in onednn kernel (shallow input depth to deep output
// depth)
...
INSTANTIATE_TEST_SUITE_P(testing_can_fuse_reorder_first_conv,
                         test_can_fuse_reorder_onednn,
                         ::testing::ValuesIn(std::vector<reorder_test_param>{
                             reorder_test_param{format::bs_fs_yx_bsv8_fsv4,
                                                format::b_fs_yx_fsv32,
                                                data_types::f32,
                                                data_types::u8,
                                                {1, 3, 8, 8},
                                                ...
                                                format::goiyx,
                                                true},
                             reorder_test_param{format::bs_fs_yx_bsv8_fsv2,
                                                format::b_fs_yx_fsv16,
                                                data_types::f32,
                                                data_types::f16,
                                                {1, 3, 8, 8},
...
TEST_P(can_fuse_reorder, surface_input_reorder) {
    ...
    auto conv_prim =
        cldnn::convolution("conv", input_info("permute"), "weights", "", 1, {1, 1}, {1, 1}, {0, 0}, {0, 0}, false);

    topology.add(input_layout_prim,
                 weights_data_prim,
                 surface_input_reorder_prim,
                 conv_input_reorder_prim,
                 transpose,
                 conv_prim);
...
TEST_P(can_fuse_reorder, surface_input_reorder_batched) {
    ...
    topology.add(input_layout_prim1,
                 input_layout_prim2,
                 weights_data_prim,
                 surface_input_reorder_prim1,
                 surface_input_reorder_prim2,
                 transpose,
                 conv_input_reorder_prim,
                 concat,
                 conv_prim);
...
TEST_P(test_can_fuse_reorder_onednn_errata, errata_case_for_conv) {
    ...
    topology.add(
        reorder("reorder_conv", input_info("reorder_input"), p.reorder_layout.format, p.reorder_layout.data_type));
    topology.add(
        convolution("conv", input_info("reorder_conv"), "weights", "", 1, {1, 1}, {1, 1}, {0, 0}, {0, 0}, false));
    ...
    while (itr != prog->get_processing_order().end()) {
        ...
        for (auto next : node_ptr->get_users()) {
            ASSERT_EQ(
                p.expected_result,
                lo.can_fuse_reorder(prev, *next, prev.get_output_layout().format, next->get_output_layout().format));
        }
    }
}

INSTANTIATE_TEST_SUITE_P(testing_can_fuse_reorder_errata_case_for_conv,
                         test_can_fuse_reorder_onednn_errata,
                         ::testing::ValuesIn(std::vector<onednn_layout_errata_test_param>{
                             onednn_layout_errata_test_param{
                                 ...
                                 true},
                             onednn_layout_errata_test_param{layout(data_types::f16, format::bfyx, {1, 8, 8, 8}),
                                                             layout(data_types::f16, format::byxf, {1, 8, 8, 8}),
