...
TEST(propagate_constants, select_impl_for_static_node_without_compile_graph) {
    auto& engine = get_test_engine();

    auto input_mem = engine.allocate_memory({{1, 3, 4, 4}, data_types::f32, format::bfyx});
    ...
    ASSERT_EQ(reorder_node.get_selected_impl(), nullptr);
