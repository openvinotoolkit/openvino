...
TEST(clamp_fp16_output_test, test_gemm_softmax_simple) {
    ...
    std::vector<float> input2_data = {2.f, 5.f, -4.f, -7.f};
    set_values(input1, input1_data);
    set_values(input2, input2_data);

    std::vector<float> out_data = {0.f, 0.8803f, 0.1192f};
    ...
    topology.add(input_layout("input1", in1_layout),
                 ...
                 gemm("gemm",
                      {input_info("input1_fp16"), input_info("input2_fp16")},
                      data_types::f16,
                      false,
                      true,
                      1.0f,
...
TEST(clamp_fp16_output_test, test_gemm_softmax_mult_fused) {
    ...
    std::vector<float> elt_data = {10.f, -5.5f, -0.05f};

    set_values(input1, input1_data);
    set_values(input2, input2_data);
    set_values(input_elt, elt_data);

    std::vector<float> out_data = {0.1209f, 0.0269f, 0.8520f};
