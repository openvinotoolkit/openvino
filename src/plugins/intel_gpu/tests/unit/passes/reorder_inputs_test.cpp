...
TEST(reorder_inputs, set_preferred_format_of_strided_slice_from_default_format) {
    tests::random_generator rg(GET_SUITE_NAME);
    auto& engine = get_test_engine();
    ...
    auto& strided_slice = program->get_node("strided_slice2");
    ASSERT_EQ(lo.get_preferred_format(strided_slice), format::bfyx);
