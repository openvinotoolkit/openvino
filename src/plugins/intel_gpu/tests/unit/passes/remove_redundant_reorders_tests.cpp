...
TEST(remove_redundant_reorders, remove_dep_dynamic) {
    ...
    // In reorders, output format of dependency reorder should be saved as output_format of orginial reorder
...
TEST(remove_redundant_reorders, skip_reorder_fusing_when_sibling_not_support_padding) {
    // Reorder fusing with padding in remove_redundant_reorders pass should check all sibiling nodes whether they
    // support padding or not. This test case has two reorders after convolution and one has padding. This reorder
    // shouldn't be fused in the pass. Reference model : Enhance3-lite
    ...
    topology.add(input_layout("input", input->get_layout()));
    topology.add(convolution("convolution",
                             input_info("input"),
                             "weights",
                             "",
                             1,
    ...
    topology.add(reorder("reorder_reshape_1",
                         input_info("convolution"),
                         {data_types::f16, format::bfwzyx, {2, 16, 1, 1, 480, 270}}));
    topology.add(permute("transpose_1", input_info("reorder_reshape_1"), {0, 1, 2, 3, 5, 4}));
    topology.add(reorder("convolution_reorder_1",
                         input_info("convolution"),
                         {data_types::f16, format::fs_b_yx_fsv32, {2, 16, 480, 270}, padding({0, 0, 1, 1}, 0)}));
    topology.add(convolution("convolution_2",
                             input_info("convolution_reorder_1"),
                             "weights_2",
                             "",
                             1,
                             {1, 1},
...
TEST(remove_redundant_reorders, fuse_reorder_to_prev_mvn_dyn) {
    ...
    auto in_layout = layout{ov::PartialShape{ov::Dimension::dynamic(), ov::Dimension::dynamic(), 256},
                            data_types::f32,
                            format::bfyx};
...
TEST(remove_redundant_reorders, fuse_reorder_to_prev_concat_dyn) {
    auto& engine = get_test_engine();
    auto in_layout1 = layout{ov::PartialShape{ov::Dimension::dynamic(), 32, ov::Dimension::dynamic(), 80},
                             data_types::f16,
                             format::bfyx};
    auto in_layout2 = layout{ov::PartialShape{ov::Dimension::dynamic(), 32, ov::Dimension::dynamic(), 80},
                             data_types::f16,
                             format::bfyx};
...
TEST(remove_redundant_reorders, not_to_fuse_concat_with_reorder_inside_shape_of_subgraph) {
    ...
    topology.add(
        reshape("reshape0", input_info("eltwise0"), false, {}, ov::PartialShape{1}, reshape::reshape_mode::unsqueeze));
    ...
    topology.add(
        reshape("reshape1", input_info("eltwise1"), false, {}, ov::PartialShape{1}, reshape::reshape_mode::unsqueeze));
    topology.add(concatenation("concat0", {input_info("reshape0"), input_info("reshape1")}, 0, data_types::f32));
    topology.add(reorder("reorder3",
                         input_info("concat0"),
                         format::any,
                         data_types::i32,
                         std::vector<float>(),
