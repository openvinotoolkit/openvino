...
TEST(add_required_reorders, input_reorder_inside_shape_of_subgraph) {
    ...
    topology.add(
        eltwise("eltwise0", {input_info("gather0"), input_info("data_1")}, eltwise_mode::prod, data_types::f32));
    topology.add(
    ...
    topology.add(
        eltwise("eltwise1", {input_info("gather1"), input_info("data_1")}, eltwise_mode::prod, data_types::f32));
    topology.add(
...
TEST(add_required_reorders, prevent_input_dt_changing_for_convs) {
    ...
    auto input_mem =
        engine.allocate_memory({{input_b, input_f, input_y, input_x}, data_types::u8, format::bs_fs_yx_bsv16_fsv32});
    auto input2_mem =
        engine.allocate_memory({{input_b, input_f, input_y, input_x}, data_types::u8, format::bs_fs_yx_bsv16_fsv32});
    ...
    auto output_reorder = reorder("reorder",
                                  input_info("conv1"),
                                  {data_types::f32, format::bfyx, {output_b, output_f, output_y, output_x}});
    ...
    config_test.set_property(
        ov::intel_gpu::force_implementations(ov::intel_gpu::ImplForcingMap{{"conv1", conv1_impl_test}}));
...
TEST(add_required_reorders, prevent_users_invalidation) {
    ...
    auto input_mem_padded =
        engine.allocate_memory({{1, 16, 8, 8}, data_types::f16, format::bfyx, padding{{0, 0, 2, 2}, 0}});
    ...
    auto prog =
        program::build_program(engine, topology(input, weights, conv), get_test_default_config(engine), false, true);
...
TEST(add_required_reorders, skip_adding_reorder_batch_axis_padding) {
    ...
    topology.add(concatenation("concat",
                               {input_info("reshape1"), input_info("reshape2")},
                               1,
                               data_types::f32,
                               padding{{0, 0, 0, 0}, 0}));
