...
TEST(handle_reshape, dont_remove_reshape_that_changes_rank_chain) {
    ...
    topology.add(reshape("reshape2",
                         ...
                         true,
                         {1, 2, 0},
                         {1, 2, 0}));  // swap 0 and 1 axis and keep 2nd axis as is
...
TEST(handle_reshape, skip_reorder_node_to_split_when_onndnn_not_support) {
    // Onednn FC does not support fp32 input, fp16 weight. In such case, we need to ignore reorder_split from
    // handle_reshape pass
    auto& engine = get_test_engine();
    ...
    topology.add(
        reorder("convert_to_f32_matmul", input_info("matmul"), {data_types::f32, format::bfyx, {9, 1, 1, 1024}}));
...
TEST(handle_reshape, reshape_input_reorder) {
    ...
    auto in0_memory =
        engine.allocate_memory(layout{ov::PartialShape{1, 2, 16, 64, 64}, data_types::f16, format::bfzyx});
    ...
    // [Updated] get_preferred_format() updated to use 'in_lay_rank' instead of 'out_lay_rank' for preferred input
    // format
...
TEST(handle_reshape, reshape_opt_out_layout_update) {
    ...
    topology.add(input_layout("input", input->get_layout()));
    topology.add(convolution("conv1",
                             input_info("input"),
                             "weights1",
                             "",
                             1,
    ...
    topology.add(convolution("conv2",
                             input_info("pool"),
                             "weights2",
                             "",
                             1,
