...
static bool check_subgraph(const program_node& node,
                           const program_node& last_node,
                           std::map<std::string, size_t> custom_dependant_nodes_count = {}) {
...
TEST(mark_shape_of_subgraphs, simple_chain) {
    ...
    auto input_layout_dynamic =
        layout{ov::PartialShape{ov::Dimension::dynamic(), ov::Dimension::dynamic()}, data_types::f32, format::bfyx};
    ...
    topology.add(
        broadcast("broadcast", input_info("input"), input_info("concat"), {}, ov::op::BroadcastType::BIDIRECTIONAL));
...
TEST(mark_shape_of_subgraphs, simple_chain_w_reshape_inside_subgraph) {
    auto& engine = get_test_engine();
    auto input_layout_dynamic =
        layout{ov::PartialShape{ov::Dimension::dynamic(), ov::Dimension::dynamic()}, data_types::f16, format::bfyx};
    auto data_0 = engine.allocate_memory({ov::PartialShape{1}, data_types::i64, format::bfyx});
    ...
    topology.add(
        broadcast("broadcast", input_info("input"), input_info("reshape"), {}, ov::op::BroadcastType::BIDIRECTIONAL));
...
TEST(mark_shape_of_subgraphs, parallel_shape_of_subgraphs_cascade) {
    ...
    topology.add(
        scatter_update("scatter_update_0", input_info("gather_0"), input_info("data_0"), input_info("data_0"), 0));
    topology.add(
        scatter_update("scatter_update_1", input_info("gather_1"), input_info("data_0"), input_info("data_0"), 0));
    ...
    topology.add(
        scatter_update("scatter_update_2", input_info("gather_2"), input_info("data_0"), input_info("data_0"), 0));
    ...
    std::map<std::string, size_t> custom_dependant_nodes_count = {{"strided_slice_1", 2},
                                                                  {"strided_slice_2", 3},
                                                                  {"reshape", 3}};
...
TEST(mark_shape_of_subgraphs, concat_with_empty_tensor_inputs) {
    ...
    topology.add(
        concatenation("concat", {input_info("gather01"), input_info("shape_of_02"), input_info("shape_of_03")}, 0));
...
TEST(mark_shape_of_subgraphs, gather_compressed_no_mark) {
    ...
    topology.add(gather("gather_compressed",
                        input_info("shape_of"),
                        input_info("data_0"),
                        0,
