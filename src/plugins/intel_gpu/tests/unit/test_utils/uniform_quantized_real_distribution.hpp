...
template <typename DataType,
          bool IsFloatingPoint =
              (std::is_floating_point<DataType>::value && std::numeric_limits<DataType>::is_iec559 &&
               std::numeric_limits<DataType>::radix == 2 && std::numeric_limits<DataType>::digits > 0),
          bool IsIntegral = (std::is_integral<DataType>::value && std::numeric_limits<DataType>::is_integer &&
                             std::numeric_limits<DataType>::radix == 2 && std::numeric_limits<DataType>::digits > 0)>
...
private:
    ...
    static constexpr std::size_t exponent_vals_cnt =
        (std::numeric_limits<DataType>::max_exponent - std::numeric_limits<DataType>::min_exponent + 1U) +
...
public:
    ...
    static constexpr std::size_t value =
        round_up_to_multiply(static_cast<std::size_t>(std::numeric_limits<DataType>::is_signed) +
                                 log2_floor(exponent_vals_cnt) + (std::numeric_limits<DataType>::digits - 1U),
                             CHAR_BIT);
};
...
struct estimate_repr_size_helper2_<DataType, false, true> {
    ...
    static constexpr std::size_t value = round_up_to_multiply(
        static_cast<std::size_t>(std::numeric_limits<DataType>::is_signed) + std::numeric_limits<DataType>::digits,
        CHAR_BIT);
};

template <typename DataType, bool IsArithmetic = std::is_arithmetic<DataType>::value>
...
template <typename DataType>
struct estimate_repr_size_helper1_<DataType, true> : estimate_repr_size_helper2_<DataType> {};
...
public:
    ...
    explicit ios_format_guard(std::basic_istream<CharType, CharTraits>& stream) : ios_format_guard(stream, 0) {}
    ...
    // ReSharper disable once CppPossiblyUninitializedMember
    explicit ios_format_guard(std::basic_ostream<CharType, CharTraits>& stream) : ios_format_guard(stream, 0) {}
...
private:
    ...
    static constexpr part_type last_part_mask =
        little_endian ? ~(part_ones << last_part_bit_size) : ~(part_ones >> last_part_bit_size);
    ...
    struct data_parts {
        ...
        constexpr data_parts(data_parts_zero_mark) : parts{} {}
...
private:
    ...
    constexpr explicit data_type_mask(const data_parts& mask) : _mask(mask) {}

public:
    /// @brief Creates mask with all bits cleared (set to @c 0).
    constexpr data_type_mask() noexcept : _mask(data_parts_zero) {}
    ...
    /// @param from Index (@c 0-based) indicating first bit to set.
    // ReSharper disable once CppDoxygenUndocumentedParameter
    data_type_mask(const std::size_t from, create_by_pos_mark) noexcept : _mask(data_parts_zero) {
    ...
    data_type_mask(const std::size_t from,
    ...
    template <typename CharType, typename CharTraits>
    friend std::basic_istream<CharType, CharTraits>& operator>>(std::basic_istream<CharType, CharTraits>& is,
                                                                data_type_mask& rhs) {
        ...
        if (little_endian) {
            ...
            if (last_part_bit_size > 0U) {
                ...
                mask.part_hilo_ord(full_parts_count) <<=
                    ((data_part_bit_size - last_part_bit_size) % data_part_bit_size);
...
public:
    ...
    static constexpr unsigned significand_max_bits =
        ((std::numeric_limits<result_type>::digits - 1) > 0) ? std::numeric_limits<result_type>::digits - 1 : 0;
    ...
    struct param_type {
        ...
        explicit param_type(const result_type a = result_zero,
            ...
            : _a(a),
              _b(b),
              _significand_rand_bits(std::min(significand_rand_bits, +significand_max_bits)) {
            if (!std::isfinite(a))
        ...
        friend bool operator==(const param_type& lhs, const param_type& rhs) {
            return lhs._a == rhs._a && lhs._b == rhs._b && lhs._significand_rand_bits == rhs._significand_rand_bits;
        }
...
private:
    ...
    struct param_swap_guard {
        ...
        param_swap_guard(distribution_type& distribution, const param_type& new_param)
            : _distibution(distribution),
              _old_param(distribution.param()) {
    ...
    template <typename Distribution>
    param_swap_guard<Distribution> temporary_swap_param(Distribution& distribution,
...
public:
    ...
    /// @throw std::invalid_argument Upper bound (@p b) is not finite number (infinite or @c NaN).
    /// @throw std::invalid_argument Lower bound (@p a) is greater than upper bound (@p b).
    explicit uniform_quantized_real_distribution(const result_type a = result_zero,
                                                 const result_type b = result_one,
                                                 const unsigned significand_rand_bits = significand_max_bits)
    ...
    /// @param significand_rand_bits Number of high bits to randomize in significand part of floating-point
    ///                              value. The value is clamped from above by significand_max_bits.
    explicit uniform_quantized_real_distribution(const unsigned significand_rand_bits)
    ...
    // ReSharper disable once CppNonExplicitConvertingConstructor
    uniform_quantized_real_distribution(const param_type& param)
        : _param(param),
    ...
    friend std::basic_ostream<CharType, CharTraits>& operator<<(std::basic_ostream<CharType, CharTraits>& os,
        ...
        stream_type ss;
        ss << hdr_a << mask_type(rhs._param.a()) << hdr_b << mask_type(rhs._param.b()) << hdr_srb
           << rhs._param.significand_rand_bits() << hdr_m << rhs._mask << hdr_d << rhs._base_distribution << ftr;
...
private:
    ...
    /// @return   Input stream. The same as @c is.
    template <typename CharType, typename CharTraits>
    static std::basic_istream<CharType, CharTraits>& skip_to_char(std::basic_istream<CharType, CharTraits>& is,
                                                                  char c) {
...
public:
    ...
    friend bool operator==(const uniform_quantized_real_distribution& lhs,
                           const uniform_quantized_real_distribution& rhs) {
        return lhs._param == rhs._param && lhs._mask == rhs._mask && lhs._base_distribution == rhs._base_distribution;
    }
