...
template <class T>
void set_value(T* ptr, uint32_t index, T value) {
    ptr[index] = value;
}
template <class T>
T get_value(T* ptr, uint32_t index) {
    return ptr[index];
}
        ...
        for (cldnn::tensor::value_type f = 0; f < l.feature(); ++f) {
            for (cldnn::tensor::value_type y = 0; y < l.spatial(1); ++y) {
                for (cldnn::tensor::value_type x = 0; x < l.spatial(0); ++x) {
                    unsigned int input_it =
                        b * pitches.batch[0] + f * pitches.feature[0] + y * pitches.spatial[1] + x * pitches.spatial[0];
...
template <
    typename T,
    typename std::enable_if<std::is_floating_point<T>::value || std::is_same<T, ov::float16>::value>::type* = nullptr>
...
inline bool are_equal(const float ref_item,
                      const float item,
    ...
    } else if (fabs(item - ref_item) / fabs(ref_item) > relative_error_threshold) {
        std::cout << "Ref val: " << ref_item << "\tSecond val: " << item << std::endl;
        return false;
    }
...
public:
    ...
    test_params(cldnn::data_types dt,
                cldnn::format input_format,
                int32_t batch_size,
                int32_t feature_size,
                cldnn::tensor input_size,
...
class generic_test
    : public ::testing::TestWithParam<std::tuple<std::shared_ptr<test_params>, std::shared_ptr<cldnn::primitive>>> {
public:
    generic_test();
    ...
    static size_t get_linear_index(const cldnn::layout& layout,
                                   size_t b,
                                   size_t f,
                                   size_t y,
                                   size_t x,
    ...
    static size_t get_linear_index(const cldnn::layout& layout,
                                   ...
                                   size_t x,
                                   const memory_desc& desc);
    static size_t get_linear_index_with_broadcast(const cldnn::layout& in_layout,
                                                  size_t b,
                                                  size_t f,
                                                  size_t y,
                                                  size_t x,
    ...
    static std::vector<std::shared_ptr<test_params>> generate_generic_test_params(
    ...
    struct custom_param_name_functor {
        std::string operator()(
            const ::testing::TestParamInfo<std::tuple<std::shared_ptr<test_params>, std::shared_ptr<cldnn::primitive>>>&
                info) {
...
protected:
    ...
    int max_ulps_diff_allowed;  // Max number of ulps allowed between 2 values when comparing the output buffer and the
                                // reference buffer.
...
inline void PrintTupleTo(const std::tuple<std::shared_ptr<test_params>, std::shared_ptr<cldnn::primitive>>& t,
                         ::std::ostream* os) {
    ...
    if (primitive->type == cldnn::concatenation::type_id()) {
        ...
        std::string norm_region =
            (lrn->norm_region == cldnn::lrn_norm_region_across_channel) ? "across channel" : "within channel";
        str << "Norm region: " << norm_region << " Size: " << lrn->size << " Alpha: " << lrn->alpha
            << " Beta: " << lrn->beta << " K: " << lrn->k;
        ...
        str << "Pooling mode: " << (p->mode == cldnn::pooling_mode::max ? "MAX" : "AVG")
            << " Pooled width: " << p->pooled_width << " Pooled height: " << p->pooled_height
            << " Spatial scale: " << p->spatial_scale << " Spatial bins x: " << p->spatial_bins_x
            << " Spatial bins y: " << p->spatial_bins_y << " Output dim: " << p->output_dim;
        ...
        str << "Output data type: " << ov::element::Type(*reorder->output_data_types[0]) << " Mean: " << reorder->mean
            << "Subtract per feature: " << "TODO" /*std::vector<float> subtract_per_feature*/;
        ...
        str << "Norm region: " << norm_region << " Epsilon: " << normalize->epsilon
            << " Scale input id: " << normalize->scale_input;
        ...
        str << "Negative slope: " << activation->additional_params.a
            << " Negative slope input id: " << activation->additional_params_input;
        ...
        str << "Pooling mode: " << pooling_mode << " Pads_begin x: " << pooling->pads_begin[1]
            << " Pads_begin y: " << pooling->pads_begin[0] << " Pads_end x: " << pooling->pads_end[1]
            << " Pads_end y: " << pooling->pads_end[0] << " Stride x: " << pooling->stride[1]
            << " Stride y: " << pooling->stride[0] << " Size x: " << pooling->size[1]
            << " Size y: " << pooling->size[0];
    } else {
...
template <class T>
std::vector<float> get_output_values_to_float(cldnn::network& net,
                                              const cldnn::network_output& output,
    ...
    if (ptr->get_layout().data_type != ov::element::from<T>())
        OPENVINO_THROW("target type ",
                       ov::element::from<T>().get_type_name(),
                       " mismatched with actual type ",
                       ov::element::Type(ptr->get_layout().data_type).get_type_name());
    for (size_t i = 0; i < std::min(max_cnt, ptr->get_layout().count()); i++)
        ret.push_back(mem[i]);
    return ret;
}

inline std::vector<float> get_output_values_to_float(cldnn::network& net,
                                                     const cldnn::network_output& output,
                                                     size_t max_cnt = std::numeric_limits<size_t>::max()) {
