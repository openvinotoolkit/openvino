...
struct number_caps {
    static_assert(meta::always_false<NumberTy>::value,
                  "Capabilities were not defined for selected number type. Please add specialization.");
};
...
struct number_caps<float> {
    ...
    static constexpr calc_type inv_exp2(const unsigned magnitude) {
        return magnitude == 0U ? 1.0f
                               : (magnitude & 1U ? 0.5f : 1.0f) * inv_exp2(magnitude >> 1) * inv_exp2(magnitude >> 1);
    }
...
struct number_caps<double> {
    ...
    static constexpr calc_type inv_exp2(const unsigned magnitude) {
        return magnitude == 0U ? 1.0
                               : (magnitude & 1U ? 0.5 : 1.0) * inv_exp2(magnitude >> 1) * inv_exp2(magnitude >> 1);
    }
...
auto gen_number(RndEngineTy& rnd_engine,
                ...
                const unsigned scale = 1) -> typename number_caps<NumberTy>::output_type {
