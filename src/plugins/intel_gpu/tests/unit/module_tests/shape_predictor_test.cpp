...
TEST_P(shape_predictor_tests, prediction) {
    ...
    for (auto& shape : in_shapes)
        result = sp.predict_preallocation_shape(
            "dummy_name",
            cldnn::layout(shape, ov::element::f32, cldnn::format::get_default_format(shape.size())),
            p.can_reuse_buffer);
...
TEST_P(shape_predictor_tests_b_fs_yx_fsv16, prediction) {
    ...
    for (auto& shape : in_shapes)
        result = sp.predict_preallocation_shape("dummy_name",
                                                cldnn::layout(shape, ov::element::f32, cldnn::format::b_fs_yx_fsv16),
                                                p.can_reuse_buffer);
...
INSTANTIATE_TEST_SUITE_P(smoke,
                         shape_predictor_tests_b_fs_yx_fsv16,
                         testing::ValuesIn(std::vector<shape_predictor_test_params>{
                             // Preallocation for next N iterations tests
                             {{{1, 1}, {1, 1}, {1, 1}}, {}, 1.0f, false},
                             {{{1, 1}, {1, 21}, {1, 31}}, {}, 1.0f, false},
