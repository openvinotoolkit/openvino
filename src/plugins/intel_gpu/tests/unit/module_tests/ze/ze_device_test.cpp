...
TEST(ze_devices_test, on_demand_initialization) {
	...
	for (const auto& device : devices) {
		...
		ASSERT_TRUE(ze_device->get_device().get_ze_handle() != nullptr);
		ASSERT_TRUE(ze_device->get_driver().get_ze_handle() != nullptr);
	...
	for (const auto& device : devices) {
		...
		ASSERT_TRUE(ze_device->get_device().get_ze_handle() != nullptr);
		ASSERT_TRUE(ze_device->get_context().get_ze_handle() != nullptr);
	}
}
...
TEST(ze_devices_test, user_context_initialization_not_implemented) {
	...
	ASSERT_TRUE(user_context.get_ze_handle() != nullptr);

	ASSERT_ANY_THROW(device_detector.get_available_devices(user_context.get_ze_handle(), nullptr, 0, std::numeric_limits<int>::max() /* ignore sub-devices */));
}
...
TEST(ze_devices_test, user_device_initialization_not_implemented) {
	...
	ASSERT_TRUE(user_device.get_ze_handle() != nullptr);

	ASSERT_ANY_THROW(device_detector.get_available_devices(nullptr, user_device.get_ze_handle(), 0, std::numeric_limits<int>::max() /* ignore sub-devices */));
}
