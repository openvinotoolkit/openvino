...
TYPED_TEST(uniform_quantized_real_distribution_test, param_construct_a_b_srb) {
    ...
    ASSERT_THROW({ uqr_dist_param dist_param_instance6(expected_a, expected_b, test_srb); }, std::invalid_argument);
    ...
    ASSERT_THROW({ uqr_dist_param dist_param_instance7(expected_a, expected_b, test_srb); }, std::invalid_argument);
    ...
    ASSERT_THROW({ uqr_dist_param dist_param_instance8(expected_a, expected_b, test_srb); }, std::invalid_argument);
}
...
TYPED_TEST(uniform_quantized_real_distribution_test, construct_a_b_srb) {
    ...
    ASSERT_THROW({ uqr_dist dist_instance6(expected_a, expected_b, test_srb); }, std::invalid_argument);
    ...
    ASSERT_THROW({ uqr_dist dist_instance7(expected_a, expected_b, test_srb); }, std::invalid_argument);
    ...
    ASSERT_THROW({ uqr_dist dist_instance8(expected_a, expected_b, test_srb); }, std::invalid_argument);
}
...
TYPED_TEST(uniform_quantized_real_distribution_test, serialize) {
    ...
    ss3 << std::dec << std::setprecision(5) << std::right << std::setw(400) << std::skipws << std::noboolalpha
        << std::setfill(ss3.widen('*'));
...
TYPED_TEST(uniform_quantized_real_distribution_test, DISABLED_generate_random_degen_srb) {
    ...
    for (std::size_t ii = 0; ii < TestFixture::rnd_iter_num; ++ii) {
        ...
        if (rnd_val == expected_a) {
            ++count_a;
        }
        if (rnd_val == expected_b) {
            ++count_b;
        }
