...
TEST(DISABLED_oooq_test, simple)
{
    ...
    auto input_mem = eng->allocate_memory(layout{ data_types::f32, format::bfyx, tensor{ 1, 1, 1, 1 } });
    ...
    */
    ...
    auto t = concat_lay.get_tensor();
    t.spatial[0] *= 2;

    tpl.add(reorder("r7", "c6", layout{concat_lay.data_type, concat_lay.format, t}, std::vector<float>{ 7 }));
    tpl.add(reorder("r8", "c6", layout{concat_lay.data_type, concat_lay.format, t}, std::vector<float>{ 8 }));
