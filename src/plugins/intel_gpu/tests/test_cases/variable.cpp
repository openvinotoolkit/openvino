...
struct variable_test : public ::testing::TestWithParam<VariableParams<T>> {
    void test() {
        ...
        const auto variable_memory = engine.allocate_memory(variable_layout);

        topology topology;
        topology.add(input_layout("input", input_data->get_layout()));
        topology.add(read_value{"read_value", {"input"}, "v0", variable_memory, variable_layout});
...
TEST(variable_test_common, variables_are_preserved_across_inferences) {
    ...
    const auto variable_1_memory = engine.allocate_memory(variable_layout);
    const auto variable_2_memory = engine.allocate_memory(variable_layout);
    const auto variable_result_memory = engine.allocate_memory(variable_layout);

    topology topology;
    topology.add(input_layout("input_1", input_1->get_layout()));
    topology.add(assign{"assign_1", {"input_1"}, "v1", variable_1_memory, variable_layout});

    topology.add(input_layout("input_2", input_2->get_layout()));
    topology.add(assign{"assign_2", {"input_2"}, "v2", variable_2_memory, variable_layout});

    topology.add(data("dummy1", dummy1));
    topology.add(read_value{"read_value_1", {"dummy1"}, "v1", variable_1_memory, variable_layout});
    topology.add(read_value{"read_value_2", {"dummy1"}, "v2", variable_2_memory, variable_layout});

    topology.add(eltwise{"sum", {"read_value_1", "read_value_2"}, eltwise_mode::sum, {}, variable_layout.data_type});
    topology.add(assign{"assign_result", {"sum"}, "v_result", variable_result_memory, variable_layout});
