...
public:
    ...
    // Builds the legacy v1 GPU compatibility descriptor.
    // Format: meta=<ver>;ov=<ov>;desc=[<driver/hw features>]
    static std::string build_runtime_requirements(const cldnn::device_info& info);

    // Builds the current backend-tagged compatibility descriptor.
    static std::string build_runtime_requirements(const cldnn::device& device);
    ...
    static bool is_runtime_requirements_compatible(const std::string& requirements, const cldnn::device& device);
    static bool is_runtime_requirements_compatible(const std::string& requirements, const cldnn::device& device, uint32_t descriptor_version);
    ...
    static constexpr uint32_t legacy_runtime_requirements_version = 1;
...
private:
    ...
    uint32_t m_runtime_requirements_version = runtime_requirements_version;
