...
/// neither data type nor format of the input buffer and total number of elements in input and output (excluding
/// paddings) must match. Please note that there is no guarantee that underlying data will be in proper format if
/// primitive was explicitly added to output list.
struct reshape : public primitive_base<reshape> {
    CLDNN_DECLARE_PRIMITIVE(reshape)

    reshape() : primitive_base("", {}) {}

    enum reshape_mode : uint32_t { base, squeeze, unsqueeze };
    ...
    /// At most one dimension of the new shape can be -1. In this case, the value is inferred from the size of the
    /// tensor and the remaining dimensions.
    ...
    bool operator==(const primitive& rhs) const override {
        ...
        return special_zero == rhs_casted.special_zero && mode == rhs_casted.mode;
    }
