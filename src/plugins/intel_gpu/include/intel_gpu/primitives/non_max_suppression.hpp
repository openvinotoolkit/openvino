...
#define cmp_fields(name) name == rhs_casted.name
        return cmp_fields(selected_indices_num) && cmp_fields(center_point_box) && cmp_fields(sort_result_descending) &&
               cmp_fields(num_select_per_class.empty()) && cmp_fields(iou_threshold.empty()) &&
               cmp_fields(score_threshold.empty()) && cmp_fields(soft_nms_sigma.empty()) &&
               cmp_fields(second_output.empty()) && cmp_fields(third_output.empty()) && cmp_fields(rotation);
#undef cmp_fields
    }
