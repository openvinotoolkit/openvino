...
public:
    ...
    virtual size_t ops_count() const {
        return 0;
    }
...
struct fused_primitive_desc {
    ...
    bool is_type() const {
        static_assert(meta::is_primitive<PType>::value,
                      "Type argument fused_primitive_desc::is_type should be a non-const, non-volatile type derived "
                      "from primitive");
    ...
    bool has_outer_dep() const {
        return outer_dep_start_idx >= 0;
    }
    ...
    // TODO:
    // Currently, it assumes very simple case where dep 0 is the fused node and no input sharing b/w fused node and peer
    // node To cover such cases where some of the peer node uses input of fused node, we need to maintain actual indexes
    // of the dependencies not only the "starting index".
