...
struct network_output {
    ...
    memory::ptr get_memory(bool do_sync = true) const {
        // TODO: in_order queue doesn't create proper output event in some cases which leads to syncronization issues
        // with user app So call finish for associated stream to enusre that the output data is ready.
        if (do_sync) {
    ...
    layout get_layout()
        const {  // Last tensor memory might be null (e.g., {N, 0} shape) but we should be able to get the layout
        return _layout;
    }
...
public:
    ...
    std::vector<std::shared_ptr<primitive_inst>> const& get_outputs() {
        return _outputs;
    }
    ...
    bool has_event(const primitive_id& id) const {
        return _events.count(id);
    }
    std::vector<primitive_inst*> get_primitives(const std::vector<primitive_id>& ids);
    std::vector<std::pair<primitive_inst*, int>> get_primitives(
        const std::vector<std::pair<program_node*, int>>& nodes);
    void execute_primitive(const std::shared_ptr<primitive_inst>& primitive, const std::vector<event::ptr>& events);
    ...
    bool is_primary_stream() const {
        return _is_primary_stream;
    }
    ...
    std::vector<primitive_id> get_kv_cache_ids() const {
        return kv_cache_ids;
    }
    ...
    std::shared_ptr<ShapePredictor> get_shape_predictor() {
        return _shape_predictor;
    }
    void set_shape_predictor(std::shared_ptr<ShapePredictor> shape_predictor) {
        _shape_predictor = shape_predictor;
    }

#ifdef GPU_DEBUG_CONFIG
    int64_t get_current_iteration_num() {
        return iteration;
    }
#endif
