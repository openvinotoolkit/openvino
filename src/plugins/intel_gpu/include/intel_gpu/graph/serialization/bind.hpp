...
#define DECLARE_OBJECT_TYPE_SERIALIZATION(cls_name)     \
    ...
    const std::string& get_type_info() const override { \
        return get_type_info_s();                       \
    }
...
template <typename BufferType>
using dif =
    loader_storage<BufferType, std::function<void(BufferType&, std::unique_ptr<void, void_deleter<void>>&, engine&)>>;

template <typename BufferType, typename T, typename Enable = void>
class buffer_binder;

template <typename BufferType, typename T>
class buffer_binder<BufferType,
                    T,
...
private:
    buffer_binder() {
        dif<BufferType>::instance().set_load_function(
            {T::get_type_info_s(),
             [](BufferType& buffer, std::unique_ptr<void, void_deleter<void>>& result_ptr, engine& engine) {
