...
OV_CONFIG_RELEASE_OPTION(ov,
                         compilation_num_threads,
                         std::max(1, static_cast<int>(std::thread::hardware_concurrency())),
                         "Max number of CPU threads used for model compilation for the stages that supports parallelism")
OV_CONFIG_RELEASE_OPTION(ov::hint,
                         inference_precision,
                         ov::element::f16,
                         "Model floating-point inference precision. Supported values: { f16, f32, dynamic }",
                         [](ov::element::Type t) {
                             return t == ov::element::f16 || t == ov::element::f32 || t == ov::element::dynamic;
                         })
OV_CONFIG_RELEASE_OPTION(ov::hint,
                         model_priority,
                         ov::hint::Priority::MEDIUM,
                         "High-level hint that defines the priority of the model. It may impact number of threads used for model compilation and inference as "
                         "well as device queue settings")
OV_CONFIG_RELEASE_OPTION(ov::hint,
                         performance_mode,
...
OV_CONFIG_RELEASE_OPTION(ov::intel_gpu::hint,
                         queue_throttle,
                         ov::intel_gpu::hint::ThrottleLevel::MEDIUM,
                         "Low-level hint that controls the queue throttle level")
...
OV_CONFIG_RELEASE_OPTION(ov::intel_gpu::hint,
                         ...
                         true,
                         "Enable/Disable LoRA operation. The separate operation is less versatile, but has better performance")
OV_CONFIG_RELEASE_OPTION(ov::intel_gpu::hint,
                         enable_large_allocations,
                         false,
                         "Enable/Disable large buffer allocations (>4gb). Enabling this option may lead to performance degradation")
...
OV_CONFIG_RELEASE_OPTION(ov,
                         cache_mode,
                         ov::CacheMode::OPTIMIZE_SPEED,
                         "Cache mode defines the trade-off between the model compilation time and the disk space required for the cache")
...
OV_CONFIG_RELEASE_OPTION(
    ...
    UINT64_MAX,
    "Maximum dynamic quantization group size. When group_size is set as a higher value than this number, dynamic quantization will be turned off")
...
OV_CONFIG_RELEASE_OPTION(ov::hint,
                         ...
                         -1.0f,
                         "Scalar floating point value that is used for runtime activation tensor scaling with fp16 inference precision")
...
OV_CONFIG_RELEASE_INTERNAL_OPTION(ov::intel_gpu,
                                  queue_type,
                                  QueueTypes::out_of_order,
                                  "Type of the queue that must be used for model execution. May be in-order or out-of-order")
...
OV_CONFIG_RELEASE_INTERNAL_OPTION(ov::intel_gpu,
                                  ...
                                  false,
                                  "Controls if weights tensors can be reordered during model compilation to more friendly layout for specific kernel")
...
OV_CONFIG_RELEASE_INTERNAL_OPTION(ov::intel_gpu,
                                  ...
                                  false,
                                  "Early exit from model compilation process which allows faster execution graph dumping")
...
OV_CONFIG_RELEASE_INTERNAL_OPTION(ov::intel_gpu,
                                  impls_cache_capacity,
                                  300,
                                  "Controls capacity of LRU implementations cache that is created for each program object for dynamic models")
...
OV_CONFIG_RELEASE_INTERNAL_OPTION(ov::intel_gpu,
                                  could_use_flashattn_v2,
                                  true,
                                  "Enable/Disable SDPA primitive executing with FlashAttenV2 online softmax tricks.")
OV_CONFIG_RELEASE_INTERNAL_OPTION(ov::intel_gpu,
                                  dynamic_quantization_threshold,
                                  64,
                                  "Apply dynamic quantization only when batch size is larger than this value in OneDNN")
OV_CONFIG_RELEASE_INTERNAL_OPTION(ov::intel_gpu,
                                  dynamic_quantization_precomputed_reduction,
                                  true,
                                  "Precompute reduction of activation for faster dynamic quantization in case of asymmetric weight")
...
OV_CONFIG_RELEASE_INTERNAL_OPTION(
    ...
    weightless_attr,
    nullptr,
    "Used to configure ov::WeightlessCacheAttribute for constants that are not loaded from a .bin file. This typically applies to non-IR inputs (e.g., ORT)")

OV_CONFIG_DEBUG_GLOBAL_OPTION(ov::intel_gpu, help, false, "Print help message for all config options")
OV_CONFIG_DEBUG_GLOBAL_OPTION(ov::intel_gpu,
                              verbose,
                              0,
                              "Enable logging for debugging purposes. The higher value the more verbose output. 0 - Disabled, 4 - Maximum verbosity")
...
OV_CONFIG_DEBUG_OPTION(ov::intel_gpu,
                       dump_tensors_format,
                       ov::intel_gpu::DumpFormat::text,
                       "Format of the tensors dump. Supported values: binary, text, text_raw")
...
OV_CONFIG_DEBUG_OPTION(ov::intel_gpu,
                       dump_src_after_exec,
                       false,
                       "Enable source data dump after layer execution. Useful for capturing updated states in stateful models.")
OV_CONFIG_DEBUG_OPTION(ov::intel_gpu,
                       host_time_profiling,
                       0,
                       "Measure and print host time spent from the beginning of the infer until all host work is done and plugin is ready to block thread on "
                       "the final clFinish() call")
OV_CONFIG_DEBUG_OPTION(ov::intel_gpu,
                       disable_async_compilation,
                       false,
                       "Disable feature that allows to asynchronously prepare static-shaped implementations for the primitives with shape-agnostic kernels "
...
OV_CONFIG_DEBUG_OPTION(ov::intel_gpu,
                       ...
                       0,
                       "Disable fusions of operations as post-ops/fused-ops. Detailed debugging is possible by entering specific numbers. 1 specifies to "
                       "disable all fusions of post-ops. 2-8 specifies to enable only single fusion sub-module from fuse_reorder() to optimize_fused_opt(). "
                       "11-13 specifies to enable only single fusion sub-module in fuse_simple_primitives.")
...
OV_CONFIG_DEBUG_OPTION(ov::intel_gpu,
                       ...
                       false,
                       "Disable fake alignment feature which tries to keep gpu friendly memory alignment for arbitrary tensor shapes")
...
OV_CONFIG_DEBUG_OPTION(ov::intel_gpu,
                       ...
                       false,
                       "Validate output buffers of all layers which have fp16 data-type to find 'inf' and 'nan' value.")
