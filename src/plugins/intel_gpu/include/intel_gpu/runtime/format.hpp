...
struct format_traits {
    ...
    static const char* group_chars() {
        return "g";
    }
    /// @brief Checks if @p c represents batch dimension.
    static bool is_batch_char(char c) {
        return std::string(batch_chars()).find_first_of(c) != std::string::npos;
    }
    /// @brief Checks if @p c represents feature map/channel dimension.
    static bool is_feature_char(char c) {
        return std::string(feature_chars()).find_first_of(c) != std::string::npos;
    }
    /// @brief Checks if @p c represents spatial dimension.
    static bool is_spatial_char(char c) {
        return std::string(spatial_chars()).find_first_of(c) != std::string::npos;
    }
    /// @brief Checks if @p c represents group dimensions.
    static bool is_group_char(char c) {
        return std::string(group_chars()).find_first_of(c) != std::string::npos;
    }

    /// @brief Checks if order has @p c dimension.
    bool has_dimension(char c) const {
        return order.find_first_of(c) != std::string::npos;
    }
};
...
/// - f - number of feature maps, features or channels. For weights formats: input features - conv, inputs, inner
/// product
/// - x - spatial, width
/// - y - spatial, height
/// /n
...
struct format {
    enum type : int32_t {
        ...
        bzyxf,
        byfx,  ///< To be used when onednn gemm allows permute fusing in transformer network. Not for normal use from
               ///< cldnn.
        bxfy,  ///< To be used when onednn gemm allows permute fusing in transformer network. Not for normal use from
               ///< cldnn. for user provided formats.
        b_fs_yx_fsv2,
        b_fs_zyx_fsv2,
        ...
        image_2d_rgba,  ///< format for image2d RGBA, always allocates memory for 4 feature maps (even when only 3 are
                        ///< used)
        ...
        custom,      ///< means that this format is created based on custom format traits and may have no corresponding
                     ///< label
        any = -1
    };
    ...
    static size_t batch_num(const format& fmt) {
        return fmt.traits().batch_num;
    }
    /// @brief Returns number of feature dimensions for a @p format.
    static size_t feature_num(const format& fmt) {
        return fmt.traits().feature_num;
    }
    /// @brief Returns number of spatial dimensions for a @p format.
    static size_t spatial_num(const format& fmt) {
        return fmt.traits().spatial_num;
    }
    /// @brief Returns number of group dimensions for a @p format.
    static size_t group_num(const format& fmt) {
        return fmt.traits().group_num;
    }
    /// @brief Returns an order of dimensions for a @ format.
    static const std::string& order(const format& fmt) {
        return fmt.traits().order;
    }
    /// @brief Returns an internal orders of dimensions for a @p format.
    static const std::string& internal_order(const format& fmt) {
        return fmt.traits().internal_order;
    }
    /// @brief Returns block sizes for @p format.
    static const std::vector<std::pair<size_t, int>>& block_sizes(const format& fmt) {
        return fmt.traits().block_sizes;
    }
    /// @brief Returns number of dimensions contained within a @p format
    static size_t dimension(const format& fmt) {
        return order(fmt).size();
    }
    /// @brief Checks if @p format is a winograd format
    static bool is_winograd(const format& fmt) {
        return (fmt == winograd_2x3_s1_data || fmt == winograd_2x3_s1_weights || fmt == winograd_2x3_s1_fused_weights ||
                fmt == winograd_6x3_s1_fused_weights || fmt == image_2d_weights_winograd_6x3_s1_fbxyb ||
    ...
    static bool is_image_2d(const format& fmt) {
        return (fmt == image_2d_weights_c4_fyx_b || fmt == image_2d_weights_c1_b_fyx ||
                fmt == image_2d_weights_winograd_6x3_s1_fbxyb || fmt == image_2d_weights_winograd_6x3_s1_xfbyb ||
                fmt == nv12 || fmt == image_2d_rgba);
    }
    ...
    static bool is_simple_data_format(const format& fmt) {
        return (fmt == yxfb || fmt == byxf || fmt == byfx || fmt == bxfy || fmt == bfyx || fmt == fyxb || fmt == fbyx ||
                fmt == bfzyx || fmt == bfwzyx || fmt == bfuwzyx || fmt == bfvuwzyx);
    }
