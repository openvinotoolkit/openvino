...
enum class device_type { integrated_gpu = 0, discrete_gpu = 1 };
...
struct gfx_version {
    ...
    friend bool operator<(const gfx_version& l, const gfx_version& r) {
        return std::tie(l.major, l.minor, l.revision) < std::tie(r.major, r.minor, r.revision);  // same order
    }
};
...
struct device_info {
    ...
    uint64_t max_work_group_size;  ///< Maximum number of work-items in a work-group executing a kernel using the data
