...
enum class LogLevel : int8_t { DISABLED = 0, INFO = 1, LOG = 2, TRACE = 3, TRACE_DETAIL = 4 };
...
#    define GPU_DEBUG_DEFINE_MEM_LOGGER(stage)                     \
        cldnn::instrumentation::mem_usage_logger mem_logger{stage, \
                                                            cldnn::debug_configuration::get_instance()->verbose >= 2};
#    define GPU_DEBUG_PROFILED_STAGE(stage)                                           \
        auto stage_prof = cldnn::instrumentation::profiled_stage<primitive_inst>(     \
            !cldnn::debug_configuration::get_instance()->dump_profiling_data.empty(), \
            *this,                                                                    \
            stage)
...
#    define GPU_DEBUG_LOG_RAW_INT(min_verbose_level)                                             \
        if (cldnn::debug_configuration::get_instance()->verbose >= min_verbose_level)            \
        ((cldnn::debug_configuration::get_instance()->verbose_color == 0) ? GPU_DEBUG_LOG_PREFIX \
                                                                          : GPU_DEBUG_LOG_COLOR_PREFIX)
...
#    define GPU_DEBUG_LOG_PREFIX \
        std::cout << cldnn::debug_configuration::prefix << __FILENAME__ << ":" << __LINE__ << ":" << __func__ << ": "
#    define GPU_DEBUG_LOG_COLOR_PREFIX                                                                        \
        std::cout << DARK_GRAY << cldnn::debug_configuration::prefix << BLUE << __FILENAME__ << ":" << PURPLE \
                  << __LINE__ << ":" << CYAN << __func__ << ": " << RESET
...
#define GPU_DEBUG_GET_INSTANCE(name)                        \
    auto name = cldnn::debug_configuration::get_instance(); \
    (void)(name);
...
public:
    ...
    int dump_profiling_data_per_iter;      // Enables dump of extended performance profiling to specified dir for each
                                           // iteration
