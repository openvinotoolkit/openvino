...
ISyncInferPtr Schedule::create_sync_infer_request() {
    auto compiled_model = std::dynamic_pointer_cast<CompiledModel>(m_context->m_compiled_model.lock());
    ...
    if (m_context->m_bind_buffer) {
        ...
        for (const auto& device : m_context->m_device_priorities_initial) {
            ...
            if ((request_id - sum) < dev_requests.size()) {
                ...
                INFO_RUN([&]() {
                    ...
                    m_dev_infer.insert(
                        std::make_pair(request_to_share_tensors_with._ptr, &dev_requests.at(request_id - sum)));
                });
                break;
            }
        ...
        if (!request_to_share_tensors_with) {
            OPENVINO_THROW(
                "binder mode does not allow oversubsciption of infer requests, please use optimal infer request");
        }
    } else if (m_passthrough_compiled_model) {
        request_to_share_tensors_with = {m_passthrough_compiled_model->create_infer_request(),
                                         m_passthrough_compiled_model._so};
    }
    return std::make_shared<InferRequest>(std::static_pointer_cast<const CompiledModel>(compiled_model),
                                          request_to_share_tensors_with);
}
...
void Schedule::generate_workers(const std::string& device, const SoCompiledModel& compiled_model) {
    ...
    auto it_numrequests =
        deviceChecker().check_and_return_if_device_in_list<DeviceInformation>(real_devicename,
    ...
    try {
        ...
        OPENVINO_THROW("Every device used with AUTO should support query optimal_number_of_infer_requests property "
                       "from compiled model ",
                       iie.what());
    }
    ...
    for (auto&& worker_request : worker_requests) {
        ...
        worker_request.m_inferrequest->set_callback([worker_request_ptr, this, device, idle_workerrequests_ptr](
...
Pipeline Schedule::get_async_pipeline(const ISyncInferPtr& infer_request, WorkerInferRequest** worker_infer_request) {
    ...
    if (m_passthrough_compiled_model || std::static_pointer_cast<InferRequest>(infer_request)->get_shared_request()) {
        struct RequestExecutor : ov::threading::ITaskExecutor {
            explicit RequestExecutor(const SoAsyncInferRequest& infer_request, WorkerInferRequest* worker)
        ...
        auto requestExecutor = std::make_shared<RequestExecutor>(shared_request, worker);
        ...
        pipeline = {
            // if the request is coming with device-specific remote tensors make sure it is scheduled to the specific
            // device only:
            Stage{/*TaskExecutor*/ first_executor,
                  /*task*/
                  [this, &infer_request]() {
                      ...
                      for (const auto& it : compiled_model->inputs()) {
                          ...
                          if (auto remote_tensor = std::dynamic_pointer_cast<ov::IRemoteTensor>(tensor._ptr)) {
                              ...
                              const auto res =
                                  std::find_if(m_context->m_device_priorities_initial.cbegin(),
                                               m_context->m_device_priorities_initial.cend(),
                                               [&name](const DeviceInformation& d) {
                                                   return (d.default_device_id.empty()
                                                               ? d.device_name
                                                               : (d.device_name + "." + d.default_device_id)) == name;
                                               });
                              if (m_context->m_device_priorities_initial.cend() == res) {
                                  OPENVINO_THROW(
                                      "None of the devices supports a remote tensor created on the device named ",
                                      name);
                              } else {
                                  // it is ok to take the c_str() here (as pointed in the schedule.hpp we need to use
                                  // const char*) as the original strings are from the "persistent" vector (with the
                                  // right lifetime)
            ...
            // as the scheduling algo may select any device, this stage accepts the scheduling decision (actual
            // workerRequest)
            // then sets the device-agnostic tensors to the actual (device-specific) request
            Stage{/*TaskExecutor*/ std::dynamic_pointer_cast<ov::threading::ITaskExecutor>(shared_from_this()),
                  /*task*/
                  [&infer_request, worker_infer_request]() {
            ...
            Stage{/*TaskExecutor*/ std::make_shared<ThisRequestExecutor>(worker_infer_request, first_executor), /*task*/
        ...
        for (auto&& worker_request : m_worker_requests) {
            ...
            req_all_end_times.sort(std::less<Time>());
            {
                ...
                if (n >= 1) {
                    ...
                    std::chrono::duration<double, std::milli> durtation = req_all_end_times.back() - time;
                    LOG_INFO_TAG("%s:fps:%lf", worker_request.first.c_str(), n * 1000 / durtation.count());
                }
            }
        }
    });
