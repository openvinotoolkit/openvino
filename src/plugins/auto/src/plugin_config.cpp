...
void PluginConfig::register_property_impl(const ov::AnyMap::value_type& property,
...
void PluginConfig::set_property(const ov::AnyMap& properties) {
    for (auto& kv : properties) {
        ...
        if (is_supported(kv.first)) {
            ...
            // when user call set_property to set some config to plugin, we also respect this and pass through the
            // config in this case
