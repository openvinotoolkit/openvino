...
std::shared_ptr<ov::ICompiledModel> Plugin::compile_model_impl(const std::string& model_path,
    ...
    if (!is_cumulative) {
        devices_with_priority = get_valid_device(support_devices, model_precision, 80);
    }
...
std::map<std::string, double> Plugin::get_device_utilization(const std::string& device_luid) const {
    ...
    return devices_monitor.getUtilization(device_luid);
}
        ...
        if (utilization_threshold > 0 && utilization_threshold <= 100) {
            ...
            try {
                ...
                if (device_utilization.empty() || (device_luid.empty() && device_utilization.count("Total") == 0) ||
                    device_utilization.count(device_luid) == 0) {
