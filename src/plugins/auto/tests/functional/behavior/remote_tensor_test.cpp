...
TEST_F(AutoFuncTests, can_create_remotetensor_then_infer_with_affinity) {
    ...
    ASSERT_NO_THROW(compiled_model = core.compile_model(model_can_batch, "MULTI", {ov::device::priorities("MOCK_1")}));
    auto input = model_can_batch->get_parameters().at(0);
    auto output = model_can_batch->get_results().at(0);
    ...
    auto cldnn_context = core.get_default_context("MOCK_1");
...
TEST_F(AutoFuncTests, cannot_infer_remote_if_not_initialized_for_device) {
    ...
    auto input = model_can_batch->get_parameters().at(0);
    auto remote_tensor =  cldnn_context.create_tensor(input->get_element_type(), input->get_shape());
    ASSERT_NO_THROW(compiled_model = core.compile_model(model_can_batch, "MULTI", {ov::device::priorities("MOCK_3")}));
    auto infer_req_remote = compiled_model.create_infer_request();
    infer_req_remote.set_tensor(input, remote_tensor);
    ASSERT_NO_THROW(infer_req_remote.start_async());
...
TEST_F(AutoFuncTests, can_create_remotetensor_then_infer_with_affinity_2_devices) {
    ...
    ov::CompiledModel compiled_model;
    auto input = model_can_batch->get_parameters().at(0);
    ASSERT_NO_THROW(compiled_model = core.compile_model(model_can_batch, "MULTI", {ov::device::priorities("MOCK_1", "MOCK_3")}));
    std::vector<ov::InferRequest> inf_req_shared = {};
    auto cldnn_context = core.get_default_context("MOCK_1");
    auto remote_tensor =  cldnn_context.create_tensor(input->get_element_type(), input->get_shape());
    ASSERT_EQ(remote_tensor.get_device_name(), "MOCK_1");
...
TEST_F(AutoFuncTests, can_create_remotetensor_then_infer_with_affinity_2_devices_device_id) {
    ov::CompiledModel compiled_model;
    auto input = model_can_batch->get_parameters().at(0);
    ASSERT_NO_THROW(compiled_model = core.compile_model(model_can_batch, "MULTI", {ov::device::priorities("MOCK_1.1", "MOCK_2")}));
    auto cldnn_context = core.get_default_context("MOCK_1");
    auto remote_tensor =  cldnn_context.create_tensor(input->get_element_type(), input->get_shape());
    ASSERT_EQ(remote_tensor.get_device_name(), "MOCK_1");
    auto infer_req_remote = compiled_model.create_infer_request();
    ...
    // infer using remote tensor
    ASSERT_NO_THROW(infer_req_remote.start_async());
    ASSERT_THROW_WITH_MESSAGE(infer_req_remote.wait(), ov::Exception,
                                "None of the devices supports a remote tensor created on the device named MOCK_1");
}

TEST_F(AutoFuncTests, can_throw_if_oversubsciption_of_inferrequest) {
    ov::CompiledModel compiled_model;
    ASSERT_NO_THROW(compiled_model = core.compile_model(model_can_batch, "MULTI", {ov::device::priorities("MOCK_1", "MOCK_2"), ov::intel_auto::device_bind_buffer(true)}));
