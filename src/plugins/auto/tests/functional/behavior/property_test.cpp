...
TEST_F(AutoFuncTests, default_perfmode_for_multi) {
    auto compiled_model = core.compile_model(model_cannot_batch, "MULTI", {ov::device::priorities("MOCK_1", "MOCK_2")});
...
TEST_F(AutoFuncTests, respect_secondary_property_for_multi) {
    auto compiled_model = core.compile_model(model_cannot_batch, "MULTI", {ov::device::priorities("MOCK_1", "MOCK_2"),
                                             ov::device::properties("MOCK_1", ov::hint::performance_mode(ov::hint::PerformanceMode::THROUGHPUT)),
                                             ov::device::properties("MOCK_2", ov::hint::performance_mode(ov::hint::PerformanceMode::LATENCY))});
    EXPECT_EQ(compiled_model.get_property(ov::hint::performance_mode), ov::hint::PerformanceMode::THROUGHPUT);
...
TEST_F(AutoFuncTests, default_perfmode_for_auto_ctput) {
    auto compiled_model = core.compile_model(model_cannot_batch, {ov::device::priorities("MOCK_1", "MOCK_2"),
