...
public:
    ...
    ov::SoPtr<IExecutableNetworkInternal>  mockExeNetworkVPU;
    ...
    std::shared_ptr<NiceMock<MockIInferRequestInternal>>     inferReqInternalVPU;
    ...
    std::shared_ptr<NiceMock<MockIExecutableNetworkInternal>>     mockIExeNetVPU;
    ...
    std::shared_ptr<mockAsyncInferRequest>     mockInferrequestVPU;
    ...
    std::shared_ptr<ImmediateExecutor>     mockExecutorVPU;
...
public:
    ...
    void SetUp() override {
        ...
        mockIExeNetVPU = std::make_shared<NiceMock<MockIExecutableNetworkInternal>>();
        ...
        std::vector<std::string> availableDevs = {"CPU", "GPU.0", "GPU.1", "VPU"};
        ...
        inferReqInternalVPU = std::make_shared<NiceMock<MockIInferRequestInternal>>();
        mockExecutorVPU = std::make_shared<ImmediateExecutor>();
        ON_CALL(*mockIExeNetVPU.get(), GetMetric(StrEq(METRIC_KEY(OPTIMAL_NUMBER_OF_INFER_REQUESTS))))
           .WillByDefault(Return(optimalNum));
    }
};
...
TEST_P(AutoRuntimeFallback, releaseResource) {
    ...
    for (auto& deviceInfo : targetDevices) {
        ...
        if (deviceName == "CPU") {
            ...
            mockInferrequestVPU = std::make_shared<mockAsyncInferRequest>(
                inferReqInternalVPU, mockExecutorVPU, nullptr, ifThrow);
            ON_CALL(*mockIExeNetVPU.get(), CreateInferRequest()).WillByDefault(InvokeWithoutArgs([this]() {
                        std::this_thread::sleep_for(std::chrono::milliseconds(0));
