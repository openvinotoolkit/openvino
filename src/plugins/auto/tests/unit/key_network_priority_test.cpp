...
TEST_P(KeyNetworkPriorityTest, SelectDevice) {
    ...
    EXPECT_CALL(*plugin, select_device(_, _, _, _, _, _)).Times(sizeOfConfigs);
    EXPECT_CALL(*core, get_property(_, _, _)).Times(AtLeast(sizeOfConfigs * 4));

    for (auto& item : PriorityConfigs) {
        resDevInfo.push_back(plugin->select_device(metaDevices, netPrecision, std::get<0>(item), {}, {}, false));
    }
...
TEST_P(KeyNetworkPriorityTest, MultiThreadsSelectDevice) {
    ...
    EXPECT_CALL(*plugin, select_device(_, _, _, _, _, _)).Times(sizeOfConfigs * 2);
    ...
    for (auto& item : PriorityConfigs) {
        ...
        auto future = std::async(std::launch::async, [this, priority] {
            auto deviceInfo = plugin->select_device(metaDevices, netPrecision, priority, {}, {}, false);
