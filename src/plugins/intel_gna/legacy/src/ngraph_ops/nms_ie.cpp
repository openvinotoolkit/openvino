...
std::shared_ptr<Node> op::NonMaxSuppressionIE::clone_with_new_inputs(const ngraph::OutputVector& new_args) const {
    ...
    return make_shared<NonMaxSuppressionIE>(new_args.at(0),
                                            new_args.at(1),
                                            new_args.at(2),
                                            new_args.at(3),
                                            new_args.at(4),
...
void op::NonMaxSuppressionIE::validate_and_infer_types() {
    ...
    auto nms = std::make_shared<opset3::NonMaxSuppression>(
        input_value(0),
        input_value(1),
        /* second input is used for output calculation and only if it's Constant output shape won't be dynamic */
        max_output_boxes_per_class
            ? opset3::Constant::create(element::i64, Shape{}, max_output_boxes_per_class->cast_vector<int64_t>())
...
std::shared_ptr<Node> op::NonMaxSuppressionIE2::clone_with_new_inputs(const ngraph::OutputVector& new_args) const {
    check_new_args_count(this, new_args);
    return make_shared<NonMaxSuppressionIE2>(new_args.at(0),
                                             new_args.at(1),
                                             new_args.at(2),
                                             new_args.at(3),
                                             new_args.at(4),
...
void op::NonMaxSuppressionIE2::validate_and_infer_types() {
    ...
    auto nms = std::make_shared<opset4::NonMaxSuppression>(
        input_value(0),
        input_value(1),
        /* second input is used for output calculation and only if it's Constant output shape won't be dynamic */
        max_output_boxes_per_class
            ? opset4::Constant::create(element::i64, Shape{}, max_output_boxes_per_class->cast_vector<int64_t>())
            : squeeze_input(input_value(2)),
        squeeze_input(input_value(3)),
        squeeze_input(input_value(4)),
...
std::shared_ptr<Node> op::NonMaxSuppressionIE3::clone_with_new_inputs(const ngraph::OutputVector& new_args) const {
    if (new_args.size() == 6) {
        return make_shared<NonMaxSuppressionIE3>(new_args.at(0),
                                                 new_args.at(1),
                                                 new_args.at(2),
                                                 new_args.at(3),
                                                 new_args.at(4),
