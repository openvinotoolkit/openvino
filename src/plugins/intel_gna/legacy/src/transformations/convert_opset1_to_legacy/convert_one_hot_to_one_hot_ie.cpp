...
ngraph::pass::ConvertOneHotToOneHotIEMatcher::ConvertOneHotToOneHotIEMatcher() {
    ...
    ngraph::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        const auto depth_node =
            std::dynamic_pointer_cast<ngraph::opset1::Constant>(one_hot->input_value(1).get_node_shared_ptr());
        const auto on_value_node =
            std::dynamic_pointer_cast<ngraph::opset1::Constant>(one_hot->input_value(2).get_node_shared_ptr());
        const auto off_value_node =
            std::dynamic_pointer_cast<ngraph::opset1::Constant>(one_hot->input_value(3).get_node_shared_ptr());

        // can be converted iff inputs with depth, on/off values are constants
        if (depth_node == nullptr || on_value_node == nullptr || off_value_node == nullptr)
            return false;
        ...
        auto one_hot_ie = std::make_shared<ngraph::op::OneHotIE>(one_hot->input_value(0),
                                                                 static_cast<int>(one_hot->get_axis()),
                                                                 depth_value,
                                                                 on_value,
                                                                 off_value,
                                                                 m_output_type);
