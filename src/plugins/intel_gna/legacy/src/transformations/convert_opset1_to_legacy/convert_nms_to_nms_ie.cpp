...
ngraph::pass::ConvertNMSToNMSIEMatcher::ConvertNMSToNMSIEMatcher() {
    ...
    ngraph::matcher_pass_callback callback = [](pattern::Matcher& m) {
        ...
        if (max_output_boxes_per_class_rank.is_dynamic() || iou_threshold_rank.is_dynamic() ||
            score_threshold_rank.is_dynamic()) {
            return false;
        }

        if (max_output_boxes_per_class_rank.get_length() == 1 && iou_threshold_rank.get_length() == 1 &&
        ...
        if (max_output_boxes_per_class_rank.get_length() == 0) {
            ...
            if (auto new_max_per_class_const =
                    std::dynamic_pointer_cast<opset1::Constant>(new_max_per_class.get_node_shared_ptr())) {
                new_max_per_class =
                    opset1::Constant::create(element::i64, Shape{1}, new_max_per_class_const->cast_vector<int64_t>());
            } else {
                new_max_per_class =
                    std::make_shared<ngraph::op::v0::Unsqueeze>(nms->input_value(2),
        ...
        if (iou_threshold_rank.get_length() == 0) {
            new_iou_threshold =
                std::make_shared<ngraph::op::v0::Unsqueeze>(nms->input_value(3),
                                                            opset1::Constant::create(element::i64, Shape{1}, {0}));
        ...
        if (score_threshold_rank.get_length() == 0) {
            new_score_threshold =
                std::make_shared<ngraph::op::v0::Unsqueeze>(nms->input_value(4),
                                                            opset1::Constant::create(element::i64, Shape{1}, {0}));
