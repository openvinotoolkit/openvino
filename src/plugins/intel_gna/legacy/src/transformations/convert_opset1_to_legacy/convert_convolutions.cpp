...
ngraph::pass::ConvertGroupConvolution::ConvertGroupConvolution() {
    ...
    ngraph::matcher_pass_callback callback = [](pattern::Matcher& m) {
        ...
        if (std::dynamic_pointer_cast<opset1::Reshape>(w_input) &&
            w_input->input_value(0).get_shape() == reshape_shape) {
...
ngraph::pass::ConvertDeconvolution::ConvertDeconvolution() {
    ...
    ngraph::matcher_pass_callback callback = [](pattern::Matcher& m) {
        ...
        auto deconv_ie = std::make_shared<ngraph::op::DeconvolutionIE>(
            deconv->input_value(0),
            ...
            (deconv->inputs().size() == 3 ? deconv->input_value(2).get_node_shared_ptr() : nullptr));
...
ngraph::pass::ConvertGroupDeconvolution::ConvertGroupDeconvolution() {
    ...
    ngraph::matcher_pass_callback callback = [](pattern::Matcher& m) {
        ...
        auto reshape = std::make_shared<ngraph::opset1::Reshape>(
            ...
            op::Constant::create(element::i64, Shape{reshape_shape.size()}, reshape_shape),
            true);
        auto conv_ie = std::make_shared<ngraph::op::DeconvolutionIE>(
            gconv->input_value(0),
            reshape,
            gconv->get_strides(),
            gconv->get_dilations(),
            ...
            (gconv->inputs().size() == 3 ? gconv->input_value(2).get_node_shared_ptr() : nullptr));
        conv_ie->set_friendly_name(gconv->get_friendly_name());
        ngraph::copy_runtime_info(gconv, conv_ie);
        ngraph::replace_node(gconv, conv_ie);
        return true;
