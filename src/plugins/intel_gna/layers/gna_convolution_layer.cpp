...
bool isMappableFrom2DTo1D(const uint32_t inHeight, const uint32_t inWidth,
    ...
    return ((inHeight > 1 && inWidth > 1) &&
            ((inWidth == kernelWidth && strideWidth == 1) ||
             (inHeight == kernelHeight && strideHeight == 1)));
}
...
double getWeightsReducer(InferenceEngine::ConvolutionLayer& conv) {
    ...
    if (isConv2D(inHeight, inWidth, inDepth, conv._kernel_y, conv._kernel_x) &&
         !isMappableFrom2DTo1D(inHeight, inWidth, conv._kernel_y, conv._kernel_x, conv._stride_y, conv._stride_x)) {
