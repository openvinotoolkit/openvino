...
void Config::UpdateFromMap(const std::map<std::string, std::string>& config) {
    for (auto&& item : config) {
        ...
        if (key ==  ov::intel_gna::scale_factors_per_input) {
            inputScaleFactorsPerInput = ov::util::string_to_property(value, ov::intel_gna::scale_factors_per_input);
...
OPENVINO_SUPPRESS_DEPRECATED_END
            ...
            auto target = ov::util::string_to_property(value, ov::intel_gna::execution_target);
            ...
            performance_mode = ov::util::string_to_property(value, ov::hint::performance_mode);
            ...
            gnaFlags.pwl_design_algorithm = ov::util::string_to_property(value, ov::intel_gna::pwl_design_algorithm);
...
OPENVINO_SUPPRESS_DEPRECATED_END
            ...
            if (value == PluginConfigParams::LOG_WARNING || value == PluginConfigParams::LOG_NONE) {
                gnaFlags.log_level = ov::util::string_to_property(value, ov::log::level);
            } else {
...
void Config::AdjustKeyMapValues() {
    ...
    if (!inputScaleFactorsPerInput.empty()) {
        keyConfigMap[ov::intel_gna::scale_factors_per_input.name()] = ov::util::property_to_string(inputScaleFactorsPerInput);
    } else {
    ...
    if (gnaFlags.sw_fp32) {
        device_mode = ov::util::property_to_string(ov::intel_gna::ExecutionMode::SW_FP32);
    } else {
        for (auto&& value : supported_values) {
            if (value.second.first == pluginGna2AccMode &&
                value.second.second == swExactMode) {
                device_mode = ov::util::property_to_string(value.first);
                break;
            }
        }
    }
    ...
    keyConfigMap[ov::hint::performance_mode.name()] = ov::util::property_to_string(performance_mode);
    if (inference_precision != ov::element::undefined) {
        keyConfigMap[ov::hint::inference_precision.name()] = ov::util::property_to_string(inference_precision);
    } else {
...
OPENVINO_SUPPRESS_DEPRECATED_START
    if (gnaFlags.pwl_design_algorithm != ov::intel_gna::PWLDesignAlgorithm::UNDEFINED) {
        keyConfigMap[ov::intel_gna::pwl_design_algorithm.name()] = ov::util::property_to_string(gnaFlags.pwl_design_algorithm);
    } else {
...
OPENVINO_SUPPRESS_DEPRECATED_END
    ...
    keyConfigMap[ov::log::level.name()] = ov::util::property_to_string(gnaFlags.log_level);
}
