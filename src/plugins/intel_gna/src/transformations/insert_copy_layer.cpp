...
static const std::string kResultProprty("result_vector");
...
MatchNonComputationalLayers::MatchNonComputationalLayers() {
    ...
    ngraph::matcher_pass_callback callback = [=](ngraph::pattern::Matcher& m) {
        ...
        if (res_node) {
            ...
            rt_info[kResultProprty] = ngraph::ResultVector{res_node};
        }
        ...
        for (size_t i = 0; i < node->get_input_size(); i++) {
            ...
            if (input_rti.count(kResultProprty) && !input_rti[kResultProprty].as<ngraph::ResultVector>().empty()) {
                for (auto&& res : rt_info[kResultProprty].as<ngraph::ResultVector>()) {
                    input_rti[kResultProprty].as<ngraph::ResultVector>().push_back(res);
                }
            } else {
                input_rti[kResultProprty] = rt_info[kResultProprty];
            }
        }
        ...
        if (std::dynamic_pointer_cast<ngraph::opset8::Parameter>(node)) {
            auto result_vec = rt_info[kResultProprty].as<ngraph::ResultVector>();
...
bool HandleNonFunctionalSubgraphsCleanup::run_on_model(const std::shared_ptr<ov::Model>& m) {
    ...
    std::vector<std::string> properties{kNonCompProperty, kResultProprty};
