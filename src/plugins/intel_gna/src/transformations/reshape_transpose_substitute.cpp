...
AxisVector FindSuitableTransposeOrder(const Shape& input_shape, const Shape& output_shape, const std::vector<AxisVector>& orders) {
...
AxisVector FindSuitableTransposeOrder(const Shape& input_shape, const Shape& output_shape) {
    static std::vector<AxisVector> orders_4d = {
        AxisVector{0, 2, 3, 1},
        AxisVector{0, 3, 1, 2}
    };
    static std::vector<AxisVector> orders_3d = {
        AxisVector{1, 2, 0},
        AxisVector{2, 0, 1}
    };
...
ReshapeTransposeSubstitute::ReshapeTransposeSubstitute() {
    ...
    ov::matcher_pass_callback matcher_pass_callback = [=](Matcher& m) {
        ...
        auto new_transpose_const = std::make_shared<Constant>(element::i64,
