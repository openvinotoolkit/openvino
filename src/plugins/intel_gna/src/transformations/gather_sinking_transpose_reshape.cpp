...
std::vector<std::shared_ptr<ov::Node>> SinkForward(std::shared_ptr<ov::Node> transpose,
    ...
    auto reshape_out = reshape->clone_with_new_inputs({gather, reshape->input_value(1)});

    ov::replace_output_update_name(reshape->output(0), reshape_out->output(0));

    ov::copy_runtime_info({reshape}, {gather, gather_indices, gather_axis, reshape_in});

    return std::vector<std::shared_ptr<ov::Node>>({reshape_in, gather, reshape_out});
}

std::vector<std::shared_ptr<ov::Node>> SinkBackward(std::shared_ptr<ov::Node> reshape,
    ...
    ov::replace_node_update_name(transpose, reshape_out);

    ov::copy_runtime_info({transpose}, {gather, gather_indices, gather_axis, reshape_out, reshape_out_const});
...
GatherSinkingTransposeReshapeForward::GatherSinkingTransposeReshapeForward() {
    ...
    ov::matcher_pass_callback matcher_pass_callback = [=](Matcher& m) {
        ...
        const std::vector<std::shared_ptr<ov::Node>> new_nodes = SinkForward(transpose, reshape);
...
GatherSinkingTransposeReshapeBackward::GatherSinkingTransposeReshapeBackward() {
    ...
    ov::matcher_pass_callback matcher_pass_callback = [=](Matcher& m) {
        ...
        const std::vector<std::shared_ptr<ov::Node>> new_nodes = SinkBackward(reshape, transpose);
        for (const auto& node : new_nodes) {
            register_new_node(node);
        }
