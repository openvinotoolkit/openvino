...
bool GNAPluginNS::GNAGraphCompiler::UseOnlyConv2DGnaIface() const {
...
void GNAPluginNS::GNAGraphCompiler::ValidateCnn2D(const std::string& name,
    ...
    if (cnn2dValidator) {
        if (UseOnlyConv2DGnaIface() &&
            GNAPluginNS::GNAConvolutionLayer::isConv1D(inHeight, inWidth, inChannels, kH, kW)) {
            cnn2dValidator->ValidateCnn1D(name, inHeight, inWidth, inChannels, kH, kW, kN, strideH, strideW, dilH, dilW, inPrecision);
        } else {
