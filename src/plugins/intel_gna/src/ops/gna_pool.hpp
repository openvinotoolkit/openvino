...
class GNAPoolBase : public ov::op::Op {
public:
    OPENVINO_OP("GNAPoolBase", "intel_gna", ov::op::Op);

    /// \brief Constructs a batched max pooling operation.
    GNAPoolBase() = default;

    /// \brief Constructs a batched pooling operation.
    ///
    ...
    GNAPoolBase(const ov::Output<ov::Node>& arg,
...
/// \brief Batched max pooling operation.
class GNAMaxPool : public GNAPoolBase {
public:
    ...
    GNAMaxPool(const ov::Output<ov::Node>& arg,
               ...
               const ov::op::RoundingType rounding_type = ov::op::RoundingType::FLOOR,
               const ov::op::PadType auto_pad = ov::op::PadType::EXPLICIT)
        : GNAPoolBase(arg, strides, pads_begin, pads_end, kernel, rounding_type, auto_pad){};
};

/// \brief Batched avg pooling operation.
class GNAAvgPool : public GNAPoolBase {
public:
    OPENVINO_OP("GNAAvgPool", "intel_gna", ov::op::Op);
    /// \brief Constructs a batched max pooling operation.
    GNAAvgPool() = default;
    ...
    /// computing output shape.
    /// \param auto_pad The pad type for automatically computing padding sizes.
    GNAAvgPool(const ov::Output<ov::Node>& arg,
               const ov::Strides& strides,
               const ov::Shape& pads_begin,
               const ov::Shape& pads_end,
               const ov::Shape& kernel,
