...
// #define accurate_identity_scale_factor
        ...
        auto x_min = are_fp_eq(fmod(exponent, 1.0), 0.0) ? input_min_value / quantizedParams->_src_quant.GetScale() : 0.0;
        ...
        if (CNNNetHasPrevLayer(cnnLayer)) {
            ...
            for (auto&& input : CNNNetGetAllInputLayers(cnnLayer)) {
                ...
                if ((!fake_quantized && quantSibling->_dst_quant.IsScaleSet()) ||
                    (fake_quantized && quantSibling->_dst_quant.IsScaleSet() && !are_fp_eq(quantSibling->_dst_quant.GetScale(), 1.0f) &&
