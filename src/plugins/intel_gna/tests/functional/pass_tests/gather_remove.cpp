...
protected:
    void SetUp() override {
        ...
        auto gather_indexes_node =
            ngraph::opset9::Constant::create(ngraph::element::i64, ov::Shape{indexes.size()}, indexes);
        ...
        auto gather_node =
            std::make_shared<ngraph::opset9::Gather>(input_params[0], gather_indexes_node, gather_axis_node);

        auto multiply_input_const_node =
            ngraph::opset9::Constant::create(ngPrc, input_shape, GenerateVector(input_shape_product, 1));

        auto matmul_node = std::make_shared<ngraph::opset9::Multiply>(gather_node, multiply_input_const_node);

        auto add_input_const_node =
            ngraph::opset9::Constant::create(ngPrc, input_shape, GenerateVector(input_shape_product, 1));

        auto add_node = std::make_shared<ngraph::opset9::Add>(matmul_node, add_input_const_node);
...
protected:
    void SetUp() override {
        ...
        const size_t gather_axis = 1;
        auto gather_axis_node = ngraph::opset9::Constant::create(ngraph::element::i64, ngraph::Shape{}, {gather_axis});
        auto gather_node = std::make_shared<ngraph::opset9::Gather>(abs, gather_indexes_node, gather_axis_node);
...
INSTANTIATE_TEST_SUITE_P(smoke_gather_on_cpu,
                         RemoveInputGather,
