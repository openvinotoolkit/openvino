...
using VariadicSplitParameters = std::tuple<
    ov::Shape,              // input size
    uint32_t,               // axis
...
TEST(CheckSplitSupported, CheckVariadicSplitSupported) {
    ...
    for (const auto &item : variadic_split_data) {
        ...
        auto split = std::make_shared<ngraph::opset9::VariadicSplit>(
                ...
                ngraph::opset9::Constant::create(ngraph::element::i64, ngraph::Shape({split_lengths.size()}), split_lengths));
        ASSERT_TRUE(ov::intel_gna::ngraph_util::is_split_supported(split, false) == result);
    }
}

using SplitParameters = std::tuple<
    ov::Shape,              // input size
    uint32_t,               // axis
    uint32_t,               // num_splits
    bool                    // supported
...
TEST(CheckSplitSupported, CheckSplitSupported) {
    ...
    for (const auto &item : split_data) {
        ...
        auto split = std::make_shared<ngraph::opset9::Split>(
                std::make_shared<ngraph::opset9::Parameter>(ngraph::element::f32, input_shape),
                ngraph::opset9::Constant::create(ngraph::element::i64, ngraph::Shape({}), {axis}), num_splits);
        ASSERT_TRUE(ov::intel_gna::ngraph_util::is_split_supported(split, false) == result);
    }
}
} // namespace