...
class TransposeCompressTest : public CommonTestUtils::TestsCommon,
...
public:
    ...
    virtual void set_test_model () {
        ...
        auto transpose_const = std::make_shared<Constant>(ov::element::i32, ov::Shape{m_test_shapes.tr_order_src.size()}, m_test_shapes.tr_order_src);
        ...
        m_model_test = std::make_shared<ov::Model>(ov::ResultVector{result}, ov::ParameterVector{param}, "transpose_compress");
    }

    virtual void set_ref_model () {
        auto param = std::make_shared<Parameter>(m_type, m_test_shapes.shape_src);

        auto shape_in_const = std::make_shared<Constant>(ov::element::i32, ov::Shape{m_test_shapes.shape_ref.size()}, m_test_shapes.shape_ref);
        auto shape_in = std::make_shared<Reshape>(param, shape_in_const, false);

        auto transpose_const = std::make_shared<Constant>(ov::element::i8, ov::Shape{m_test_shapes.tr_order_ref.size()}, m_test_shapes.tr_order_ref);
        ...
        m_model_ref = std::make_shared<ov::Model>(ov::ResultVector{result}, ov::ParameterVector{param}, "transpose_compress");
    }
...
const std::vector<TestData> test_shapes = {
    {{2, 2, 4}, {1, 2, 0}, {2, 8}, {1, 0}},
    {{2, 2, 4, 4}, {2, 3, 0, 1}, {4, 16}, {1, 0}}
};

const std::vector<TestData> test_neg_shapes = {
    {{1, 2, 3}, {1, 2, 0}, {1, 6}, {1, 0}},
    {{1, 2, 3, 4}, {1, 0, 2, 3}, {}, {}}
};
