...
std::shared_ptr<ngraph::Function> CreateTransposeMatmulFunction(const ngraph::Shape& input_shape,
    ...
    auto new_shape_const =
        ov::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{reshape_shape.size()}, reshape_shape);
    auto reshape = std::make_shared<ov::opset7::Reshape>(input_params, new_shape_const, false);

    auto transpose_order = ov::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{2}, {1, 0});
    auto transpose = std::make_shared<ov::opset7::Transpose>(reshape, transpose_order);

    std::vector<size_t> data(ngraph::shape_size(matmul_shape));
    std::iota(std::begin(data), std::end(data), 1);
    auto constant = ov::op::v0::Constant::create(ngraph::element::i64, matmul_shape, data);
    ...
    if (create_reshape_after_transpose) {
        ...
        auto reshape_after_transpose =
            std::make_shared<ov::opset7::Reshape>(transpose, reshape_after_transpose_const, false);
        matmul = std::make_shared<ov::opset7::MatMul>(reshape_after_transpose, constant);
    } else {
        matmul = std::make_shared<ov::opset7::MatMul>(transpose, constant);
    }
...
std::shared_ptr<ngraph::Function> CreateMatmulFunction(const ngraph::Shape& input_shape,
    ...
    std::shared_ptr<ov::opset7::Reshape> reshape;
    auto const_shape =
        ov::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{reshape_shape.size()}, reshape_shape);
    ...
    std::iota(std::begin(data), std::end(data), 1);
    auto constant = ov::op::v0::Constant::create(ngraph::element::i64, matmul_shape, data);
    auto matmul = std::make_shared<ov::opset7::MatMul>(reshape, constant);
...
std::shared_ptr<ngraph::Function> CreateConcatTransposeMatmulFunction(const ngraph::Shape& input1_shape,
    ...
    auto input1_params = std::make_shared<ov::op::v0::Parameter>(ngraph::element::i64, input1_shape);
    std::vector<size_t> data1(ngraph::shape_size(input1_shape));
    std::iota(std::begin(data1), std::end(data1), 1);
    auto concat1_const = ov::op::v0::Constant::create(ngraph::element::i64, input1_shape, data1);
    ngraph::OutputVector concat1_chunks{input1_params, concat1_const};
    auto concat1 = std::make_shared<ov::opset7::Concat>(concat1_chunks, 0);
    auto transpose1 = std::make_shared<ov::opset7::Transpose>(concat1, transpose_order);

    auto input2_params = std::make_shared<ov::op::v0::Parameter>(ngraph::element::i64, input2_shape);
    std::vector<size_t> data2(ngraph::shape_size(input2_shape));
    std::iota(std::begin(data2), std::end(data2), 1);
    auto concat2_const = ov::op::v0::Constant::create(ngraph::element::i64, input2_shape, data2);
    ngraph::OutputVector concat2_chunks{input2_params, concat2_const};
    auto concat2 = std::make_shared<ov::opset7::Concat>(concat2_chunks, 0);
    auto transpose2 = std::make_shared<ov::opset7::Transpose>(concat2, transpose_order);
    ...
    if (create_reshape_after_transpose) {
        ...
        matmul = std::make_shared<ov::opset7::MatMul>(transpose1, transpose2);
    }
...
std::shared_ptr<ngraph::Function> CreateConcatMatmulFunction(const ngraph::Shape& input1_shape,
    ...
    if (create_reshape_instead_of_transpose) {
        ...
        matmul = std::make_shared<ov::opset7::MatMul>(concat1, concat2);
    }
...
std::shared_ptr<ngraph::Function> CreateMatmulTransposeFunction(const ngraph::Shape& input_shape,
    ...
    std::vector<size_t> data(ngraph::shape_size(matmul_shape));
    std::iota(std::begin(data), std::end(data), 1);
    auto matmul_constant = ov::op::v0::Constant::create(ngraph::element::i64, matmul_shape, data);
    std::shared_ptr<ngraph::Node> node = std::make_shared<ov::opset7::MatMul>(input_params, matmul_constant);
    ...
    if (enable_add) {
        auto add_const = ov::op::v0::Constant::create(ngraph::element::i64, matmul_output_shape, {1});
        if (matmul_on_left_side) {
            node = std::make_shared<ov::opset7::Add>(add_const, node);
        } else {
            node = std::make_shared<ov::opset7::Add>(node, add_const);
        }
    ...
    auto transpose_order = ov::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{2}, {1, 0});
    node = std::make_shared<ov::opset7::Transpose>(node, transpose_order);

    if (enable_last_reshape) {
        auto shape_const =
            ov::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{reshape_shape.size()}, reshape_shape);
        node = std::make_shared<ov::opset7::Reshape>(node, shape_const, false);
    }
...
std::shared_ptr<ngraph::Function> CreateMatmulFunction(const ngraph::Shape& input_shape,
    ...
    if (create_reshape_instead_of_transpose) {
        ...
        if (enable_last_reshape) {
            reshape = std::make_shared<ov::opset7::Reshape>(reshape_instead_of_transpose, shape_const, false);
        }
    } else {
        reshape = node;
        if (enable_last_reshape) {
            reshape = std::make_shared<ov::opset7::Reshape>(node, shape_const, false);
        }
    }
