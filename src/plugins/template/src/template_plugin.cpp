...
ov::SupportedOpsMap Plugin::query_model(const std::shared_ptr<const ov::Model>& model,
    ...
    if (model == nullptr) {
        IE_THROW() << "Only OV-based models are supported!";
    }
    ...
    auto supported = InferenceEngine::GetSupportedNodes(
        ...
        [&](std::shared_ptr<ngraph::Node> node) {
            ...
            for (const auto& opset : ov::get_available_opsets()) {
                if (opset.second().contains_type(node->get_type_info()))
                    return true;
            }
            return false;
        });
...
void Plugin::add_extension(const InferenceEngine::IExtensionPtr& /*extension*/) {
...
//         TEMPLATE_METRIC_VALUE(HARDWARE_CONVOLUTION)<]}; IE_SET_METRIC_RETURN(OPTIMIZATION_CAPABILITIES,
//         capabilities);
//     } else if (METRIC_KEY(RANGE_FOR_ASYNC_INFER_REQUESTS) == name) {
//         // TODO: fill with actual values
//         using uint = unsigned int;
