...
static std::vector<double> reduce_to_token_axis(const ov::Tensor& t) {
    ...
    if (t.get_size() == 0) return out;

    // product of preceding dims
    size_t lead = 1;
    for (size_t i = 0; i + 1 < shape.size(); ++i) lead *= shape[i];
...
static void max_pool_last_window(std::vector<double>& per_token,
                                 size_t window_size) {
    if (window_size == 0 || per_token.empty()) return;
    const size_t n = per_token.size();
    const size_t start = n > window_size ? (n - window_size) : 0;
    double maxv = 0.0;
    for (size_t i = start; i < n; ++i) maxv = std::max(maxv, per_token[i]);
    for (size_t i = start; i < n; ++i) per_token[i] = std::max(per_token[i], maxv);
}

// ============ EvictionScoreManager ============

void EvictionScoreManager::register_new_token_scores(
    const AttentionScoresForEachDecoderLayer& per_layer_scores,
    const std::set<size_t>& skipped_logical_block_ids) {

    OPENVINO_ASSERT(per_layer_scores.size() == m_num_decoder_layers,
                    "EvictionScoreManager: mismatch in layer count");
    ...
    for (size_t l = 0; l < m_num_decoder_layers; ++l) {
        ...
        if (m_aggregation_mode == AggregationMode::NORM_SUM) {
            for (size_t i = 0; i < scores.size(); ++i) {
                if (counters[i] > 0) scores[i] /= static_cast<double>(counters[i]);
            }
        }
    }
}
...
void EvictionScoreManager::remove_scores(const std::vector<std::size_t>& evicted_block_indices,
                                         size_t layer_idx) {
    if (layer_idx >= m_num_decoder_layers || m_block_size == 0) return;
...
void EvictionScoreManager::add_with_skips(std::vector<double>& dst,
    ...
    if (dst.empty() || src.empty() || m_block_size == 0) return;
    ...
    for (size_t lb : skipped_logical_block_ids) {
        size_t off = lb * m_block_size;
        for (size_t j = 0; j < m_block_size && (off + j) < skip.size(); ++j) skip[off + j] = 1;
    }
...
std::vector<std::set<std::size_t>> CacheEvictionAlgorithm::evict_logical_blocks() {
    ...
    if (m_num_decoder_layers == 0 || m_block_size == 0) return {};

    const auto L_tokens = m_score_manager.get_current_scores_length_in_tokens(0);
    if (L_tokens == 0) return std::vector<std::set<std::size_t>>(m_num_decoder_layers); // nothing to evict

    // compute how many blocks to evict (based on layer 0 token length)
    const size_t n_evict_blocks = get_num_blocks_to_evict(0);
    if (n_evict_blocks == 0) return std::vector<std::set<std::size_t>>(m_num_decoder_layers);
    ...
    if (lend <= lbeg) return std::vector<std::set<std::size_t>>(m_num_decoder_layers);
    ...
    for (auto& v : avg_scores) v /= static_cast<double>(m_num_decoder_layers);
    ...
    for (auto i : idx_local) evicted_global.push_back(lbeg + i);
...
std::size_t CacheEvictionAlgorithm::get_num_blocks_to_evict(size_t decoder_layer_idx) const {
    ...
    if (L <= m_eviction_config.get_max_cache_size()) return 0;
...
std::size_t CacheEvictionAlgorithm::get_num_evictable_blocks(size_t decoder_layer_idx) const {
    ...
    if (total_blocks <= start_blocks + recent_blocks) return 0;
    return total_blocks - start_blocks - recent_blocks;
}

CacheEvictionAlgorithm::CacheEvictionRange
CacheEvictionAlgorithm::get_evictable_block_range(size_t layer_idx) const {
    const size_t L = m_score_manager.get_current_scores_length_in_tokens(layer_idx);
    if (L == 0) return CacheEvictionRange::invalid();

    const size_t total_blocks = get_num_blocks(L);
    const size_t start_blocks  = get_num_blocks(m_eviction_config.get_start_size());
    const size_t recent_blocks = get_num_blocks(m_eviction_config.get_recent_size());

    if (total_blocks <= start_blocks + recent_blocks) return CacheEvictionRange::invalid();
...
std::vector<double> CacheEvictionAlgorithm::get_scores_for_all_evictable_blocks(size_t decoder_layer_idx) const {
    ...
    if (range == CacheEvictionRange::invalid()) return {};
...
std::vector<std::size_t> CacheEvictionAlgorithm::get_indices_of_blocks_to_evict(
    const std::vector<double>& scores, size_t k) const {
    std::vector<size_t> idx(scores.size());
    std::iota(idx.begin(), idx.end(), 0);
    // Evict the lowest scores
    if (k >= idx.size()) return idx;
