...
private:
    static std::shared_ptr<ov::Model> CreateFunction(const SparseFillEmptyRowsParams& params) {
        ...
        const auto dense_shape = std::make_shared<Parameter>(params.denseShapeTensor.type, params.denseShapeTensor.shape);
        const auto indices = std::make_shared<Parameter>(params.indicesTensor.type, params.indicesTensor.shape);
        const auto default_value = std::make_shared<Parameter>(params.defaultValueTensor.type, params.defaultValueTensor.shape);
        
        const auto sparseFillEmptyRows = std::make_shared<ov::op::v16::SparseFillEmptyRows>(
            values, dense_shape, indices, default_value);
            
        return std::make_shared<ov::Model>(
            ov::OutputVector{sparseFillEmptyRows->output(0), sparseFillEmptyRows->output(1), sparseFillEmptyRows->output(2)},
