...
class ReferenceAvgPoolLayerTestV16 : public testing::TestWithParam<AvgPoolParams>, public CommonReferenceTest {
public:
    void SetUp() override {
        const auto& params = GetParam();
        function = CreateFunction(params.m_input_shape,
    ...
    static std::string getTestCaseName(const testing::TestParamInfo<AvgPoolParams>& obj) {
        ...
        result << "oShape=" << params.m_output_shape << "_";
        result << "oType=" << params.m_output_type << "_";
        result << "dilations=" << params.m_dilations << "_";
        result << "excludePad=" << params.m_exclude_pad << "_";
        result << "roundingType=" << params.m_rounding_type << "_";
        result << "padType=" << params.m_pad_type;
        return result.str();
...
private:
    static std::shared_ptr<Model> CreateFunction(const Shape& input_shape,
                                                 ...
                                                 const op::PadType pad_type) {
        const auto in = std::make_shared<op::v0::Parameter>(input_type, input_shape);
        const auto avgPool = std::make_shared<op::v16::AvgPool>(in,
                                                                strides,
                                                                dilations,
                                                                pads_begin,
                                                                pads_end,
...
TEST_P(ReferenceAvgPoolLayerTestV16, AvgPoolWithHardcodedRefs) {
    Exec();
}

template <element::Type_t IN_ET>
std::vector<AvgPoolParams> generateParamsForAvgPoolV16() {
    using T = typename element_type_traits<IN_ET>::value_type;
    ...
    std::vector<AvgPoolParams> params{
        ...
        AvgPoolParams(ov::Shape{1, 1, 4, 4},
                      ...
                      op::RoundingType::CEIL,
                      op::PadType::NOTSET),
        AvgPoolParams(ov::Shape{1, 1, 6, 6},
                      ov::Shape{1, 1, 2, 2},
                      IN_ET,
                      IN_ET,
                      getContinuousIncreasingValue<T>(1 * 1 * 6 * 6, 1),
                      std::vector<T>{15, 16, 21, 22},
                      Strides{1, 1},
                      Strides{2, 2},
                      Shape{0, 0},
                      Shape{0, 0},
        ...
        AvgPoolParams(ov::Shape{1, 1, 2, 2},
                      ...
                      IN_ET,
                      std::vector<T>{1, 2, 3, 4},
                      std::vector<T>{4, 3, 4, 2, 1, 2, 4, 3, 4},
                      Strides{1, 1},
                      Strides{2, 2},
                      Shape{1, 1},
                      Shape{2, 2},
        ...
        AvgPoolParams(ov::Shape{1, 1, 3, 3},
                      ...
                      IN_ET,
                      std::vector<T>{1, 2, 3, 4, 5, 6, 7, 8, 9},
                      std::vector<T>{1.25, 2.5, 1.25, 2.5, 5, 2.5, 1.25, 2.5, 1.25},
                      Strides{1, 1},
                      Strides{2, 2},
                      Shape{0, 0},
                      Shape{0, 0},
...
std::vector<AvgPoolParams> generateCombinedParamsForAvgPoolV16() {
    const std::vector<std::vector<AvgPoolParams>> allTypeParams{generateParamsForAvgPoolV16<element::Type_t::f32>(),
                                                                generateParamsForAvgPoolV16<element::Type_t::f16>(),
                                                                generateParamsForAvgPoolV16<element::Type_t::bf16>()};
...
INSTANTIATE_TEST_SUITE_P(smoke_AvgPool_With_Hardcoded_Refs,
                         ReferenceAvgPoolLayerTestV16,
                         ::testing::ValuesIn(generateCombinedParamsForAvgPoolV16()),
                         ReferenceAvgPoolLayerTestV16::getTestCaseName);
}  // namespace
