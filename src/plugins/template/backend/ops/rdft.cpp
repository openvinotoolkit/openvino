...
InfoForRFFT9 get_info_for_rfft9_eval(const std::vector<std::shared_ptr<ngraph::HostTensor>>& inputs) {
    ...
    auto canonicalized_axes =
        ngraph::reference::canonicalize_axes(result.axes_data.data(), result.axes_data_shape, input_rank);
...
bool evaluate(const std::shared_ptr<ngraph::op::v9::RDFT>& op,
    ...
    ngraph::reference::rdft(info.input_data,
    ...
    ngraph::reference::fft_postprocessing(outputs, output_type, rfft_result);
    return true;
}
