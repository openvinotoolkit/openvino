...
bool evaluate(const std::shared_ptr<ov::op::v13::ScaledDotProductAttention>& op,
    ...
    const T* mask = inputs.size() == 4 ? inputs[3].data<const T>() : nullptr;
    const T* scale = inputs.size() == 5 ? inputs[4].data<const T>() : nullptr;
    auto mask_shape = inputs.size() == 4 ? inputs[3].get_shape() : ov::Shape{};
    ov::reference::scaled_dot_product_attention<T>(inputs[0].data<const T>(),
...
#define CASE(type)                                                                                        \
    case ov::element::type:                                                                               \
        return evaluate<ov::element::type>(ov::as_type_ptr<ov::op::v13::ScaledDotProductAttention>(node), \
                                           outputs,                                                       \
                                           inputs);
