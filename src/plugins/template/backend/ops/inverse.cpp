...
inline bool evaluate(const std::shared_ptr<ov::op::v13::Multinomial>& op,
                     ov::TensorVector& outputs,
                     const ov::TensorVector& inputs) {
    using T = typename ov::element_type_traits<ET>::value_type;

    const std::vector<ov::PartialShape> input_shapes{op->get_input_shape(0)};
    const auto out_shape = ov::op::v13::shape_infer(op.get(), input_shapes, make_tensor_accessor()).front().to_shape();
