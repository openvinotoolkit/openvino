...
inline void evaluate(const std::shared_ptr<ov::op::v4::CTCLoss>& op,
    ...
    OPENVINO_ASSERT(false, "The data type for logits is expected to be a floating point type. Got:", ov::element::Type(t1));
}
...
bool evaluate(const std::shared_ptr<ov::op::v4::CTCLoss>& op, const ov::HostTensorVector& outputs, const ov::HostTensorVector& inputs) {
