...
InfoForFFT7 get_info_for_fft7_eval(const std::vector<std::shared_ptr<ngraph::HostTensor>>& inputs) {
    ...
    auto canonicalized_axes =
        ngraph::reference::canonicalize_axes(result.axes_data.data(), result.axes_data_shape, complex_data_rank);
...
bool evaluate(const std::shared_ptr<ngraph::op::v7::DFT>& op,
    ...
    ngraph::reference::fft(info.input_data.data(),
                           info.input_data_shape,
                           info.axes_data.data(),
                           info.axes_data_shape,
                           fft_result.data(),
                           info.output_shape,
                           ngraph::reference::FFTKind::Forward);

    const auto output_type = op->get_input_element_type(0);
    ngraph::reference::fft_postprocessing(outputs, output_type, fft_result);
    return true;
}
...
bool evaluate(const std::shared_ptr<ngraph::op::v7::IDFT>& op,
    ...
    ngraph::reference::fft(info.input_data.data(),
                           ...
                           fft_result.data(),
                           info.output_shape,
                           ngraph::reference::FFTKind::Inverse);
