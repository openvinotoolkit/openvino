...
bool evaluate(const std::shared_ptr<ngraph::op::v0::BatchNormInference>& op,
    ...
    ngraph::reference::batch_norm_inference<T>(static_cast<float>(op->get_eps_value()),
...
bool evaluate(const std::shared_ptr<ngraph::op::v5::BatchNormInference>& op,
              ...
              const ngraph::HostTensorVector& inputs) {
    using T = typename ngraph::element_type_traits<ET>::value_type;
    ngraph::reference::batch_norm_inference<T>(static_cast<float>(static_cast<float>(op->get_eps_value())),
