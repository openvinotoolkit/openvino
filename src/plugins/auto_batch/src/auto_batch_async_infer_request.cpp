...
ov::autobatch_plugin::AsyncInferRequest::AsyncInferRequest(
    ...
    : ov::IAsyncInferRequest(request, nullptr, callback_executor),
      ...
      m_inferRequestWithoutBatch(inferRequestWithoutBatch) {
    ...
    struct ThisRequestExecutor : public ov::threading::ITaskExecutor {
        ...
        void run(ov::threading::Task task) override {
            auto workerInferRequest = _this->m_sync_request->_myBatchedRequestWrapper;
    ...
    m_pipeline = {{/*TaskExecutor*/ std::make_shared<ThisRequestExecutor>(this), /*task*/ [this] {
                       if (this->m_sync_request->_exceptionPtr)  // if the exception happened in the batch1 fallback
                       {
                           std::rethrow_exception(this->m_sync_request->_exceptionPtr);
                       }
                       auto batchReq = this->m_sync_request->_myBatchedRequestWrapper;
                       ...
                       if (SyncInferRequest::eExecutionFlavor::BATCH_EXECUTED ==
                           this->m_sync_request->_wasBatchedRequestUsed) {
                           this->m_sync_request->CopyOutputsIfNeeded();
                       }
                   }}};
}
...
std::vector<ov::ProfilingInfo> ov::autobatch_plugin::AsyncInferRequest::get_profiling_info() const {
    check_state();
    if (ov::autobatch_plugin::SyncInferRequest::eExecutionFlavor::BATCH_EXECUTED == m_sync_request->_wasBatchedRequestUsed)
        return m_sync_request->_myBatchedRequestWrapper->_inferRequestBatched->get_profiling_info();
    else
        return m_inferRequestWithoutBatch->get_profiling_info();
}
