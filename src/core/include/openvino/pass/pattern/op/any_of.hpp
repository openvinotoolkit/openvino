...
public:
    ...
    AnyOf(const element::Type& type, const PartialShape& s, Fn pred, const OutputVector& wrapped_values)
    ...
    ///        shape of \sa node.
    template <typename Fn, typename std::enable_if_t<std::is_constructible_v<Predicate, Fn>>* = nullptr>
    AnyOf(const Output<Node>& node, Fn pred, const OutputVector& wrapped_values)
