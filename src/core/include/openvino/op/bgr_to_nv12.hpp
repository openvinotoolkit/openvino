...
///
/// \details Conversion of each pixel from BGR to NV12 (YUV) space is represented by following formulas
/// (after reversing channel order to RGB):
...
public:
    ...
    explicit BGRtoNV12(const Output<Node>& arg);
    /// \brief Constructs a conversion operation from input image in BGR format
    ...
    explicit BGRtoNV12(const Output<Node>& arg, bool single_plane);
