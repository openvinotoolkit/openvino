...
public:
    ...
    /// \param u64_integer_promotion_target  Element type attribute to select promotion result for u64 and signed integers.
    ...
    /// \brief Get bool attribute whether to promote scalar input to type provided by non-scalar input when number format is matching.
    bool get_pytorch_scalar_promotion() const;

    /// \brief Set bool attribute whether to promote scalar input to type provided by non-scalar input when number format is matching.
    void set_pytorch_scalar_promotion(bool pytorch_scalar_promotion);

    /// \brief Get bool attribute whether to allow promotions that might result in bit-widening, precision loss and undefined behaviors.
    bool get_promote_unsafe() const;

    /// \brief Set bool attribute whether to allow promotions that might result in bit-widening, precision loss and undefined behaviors.
