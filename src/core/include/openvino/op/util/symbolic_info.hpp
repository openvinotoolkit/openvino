...
OPENVINO_API void set_up_symbolic_info(const std::shared_ptr<ov::Model>& model,
                                       const std::shared_ptr<ov::LabelTable>& table);
...
OPENVINO_API std::shared_ptr<ov::LabelTable> table_of_equivalence(const std::shared_ptr<ov::Model>& model);
OPENVINO_API std::shared_ptr<ov::LabelTable> table_of_equivalence(const ov::descriptor::Tensor& tensor);
 ...
 * partial values and labels during partial value propagation and keeps shared_ptr to LabelTable.
 */
class OPENVINO_API SymbolicInfo : public RuntimeAttribute {
public:
    OPENVINO_RTTI("SymbolicInfo", "0");
    explicit SymbolicInfo(bool skip_invalidation, const std::shared_ptr<ov::LabelTable>& table)
    ...
    std::shared_ptr<ov::LabelTable> get_table() const {
        return m_table;
    }

private:
    bool m_skip_invalidation;
    std::shared_ptr<ov::LabelTable> m_table;
};
