...
public:
    ...
    /// input data shape. If it is needed to change only input height & width of input image, consider define layout and use `set_spatial_static_shape' or 'set_spatial_dynamic_shape' instead.
    /// This method allows defining any custom input shape and can be useful for custom preprocessing operations
    ///
    /// \note Methods 'set_spatial_dynamic_shape', 'set_spatial_static_shape' are also intended to modify input shape, using those methods together will throw ov::AssertFailure exception
    ///
