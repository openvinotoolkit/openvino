...
std::vector<std::shared_ptr<Node>> topological_sort(T root_nodes) {
    ...
    std::vector<Node*> root_nodes_vector;
    for (auto& node : root_nodes) {
        root_nodes_vector.push_back(node.get());
    }
    std::reverse(root_nodes_vector.begin(), root_nodes_vector.end());
    std::stack<Node*, std::vector<Node*>> nodes_to_do(root_nodes_vector);
