...
template <>
constexpr uint64_t crc64<size_t(-1)>(const char* str) {
    return 0xFFFFFFFFFFFFFFFF;
}

constexpr uint64_t hash_mul(uint64_t v) {
    return v * uint64_t(0x9ddfea08eb382d69);
}
constexpr uint64_t hash_xor_and_shift(uint64_t v, uint64_t shift) {
    return v ^ (v >> shift);
}
constexpr uint64_t combine_hash(uint64_t seed, uint64_t v) {
    // a = (v ^ seed) * mul;
    // a ^= (a >> 47);
    // b = (seed ^ a) * mul;
    // b ^= (b >> 47);
    // seed = b * mul;
    return hash_mul(hash_xor_and_shift(hash_mul(seed ^ hash_xor_and_shift(hash_mul(v ^ seed), 47)), 47));
}
