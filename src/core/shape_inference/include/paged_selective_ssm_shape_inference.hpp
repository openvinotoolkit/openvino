...
std::vector<TRShape> shape_infer(const PagedSelectiveSSM* op, const std::vector<T>& input_shapes) {
    ...
    NODE_SHAPE_INFER_CHECK(
        op,
        input_shapes,
        dt_ps[0].compatible(batch_tokens) && B_ps[0].compatible(batch_tokens) && C_ps[0].compatible(batch_tokens),
        "The token dimension of `dt`, `B`, `x` and `C` should be the same.");
    NODE_SHAPE_INFER_CHECK(
        op,
        input_shapes,
        A_ps[0].compatible(num_heads) && dt_ps[1].compatible(num_heads) && state_ps[1].compatible(num_heads),
    ...
    NODE_SHAPE_INFER_CHECK(op,
                           input_shapes,
                           input_shapes[6][0].compatible(input_shapes[8][0]),
                           "`subsequence_begins` and `la_block_indices_begins` must have the same length.");
    NODE_SHAPE_INFER_CHECK(op,
                           input_shapes,
                           input_shapes[6][0].compatible(input_shapes[9][0] + 1),
                           "`num_processed_tokens` length must equal `subsequence_begins` length minus one.");
    NODE_SHAPE_INFER_CHECK(op,
                           input_shapes,
                           input_shapes[6][0].compatible(input_shapes[10][0] + 1),
                           "`cache_interval` length must equal `subsequence_begins` length minus one.");
