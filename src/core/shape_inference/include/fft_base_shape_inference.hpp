    ...
    if (input_shape.rank().is_static()) {
        ...
        if (axes_shape.is_static()) {
            NODE_VALIDATION_CHECK(op,
                                  ...
                                  "The input rank must be greater than or equal to the number of RFFT op axes. Got "
                                  "input rank: ",
                                  input_rank,
                                  ", number of axes: ",
        ...
        // RFFT operation supports for negative axes to transform. More precisely, according to
        // the RFFT operation specification, axes should be integers from -r to (r - 1)
        ...
        if (axes_shape.rank().is_static() && axes_are_known) {
            ...
            NODE_VALIDATION_CHECK(op, axes.size() == axes_set.size(), "RFFT op axes must be unique.");
        }
    }

    NODE_VALIDATION_CHECK(op, axes_shape.rank().compatible(1), "RFFT op axes input must be 1D tensor.");

    if (input_shapes.size() == 3) {
        const auto& signal_size_shape = input_shapes[2];
        NODE_VALIDATION_CHECK(op,
                              signal_size_shape.rank().compatible(1),
                              "RFFT op signal size input must be 1D tensor. Got signal: ",
                              signal_size_shape);
