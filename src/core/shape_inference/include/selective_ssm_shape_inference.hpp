...
std::vector<TRShape> shape_infer(const SelectiveSSM* op, const std::vector<T>& input_shapes) {
    ...
    NODE_SHAPE_INFER_CHECK(op, input_shapes, A_ps.rank().compatible(1), "`A` input must be rank 1.");
    NODE_SHAPE_INFER_CHECK(op, input_shapes, dt_ps.rank().compatible(3), "`dt` input must be rank 3.");
    NODE_SHAPE_INFER_CHECK(op, input_shapes, B_ps.rank().compatible(4), "`B` input must be rank 4.");
    NODE_SHAPE_INFER_CHECK(op, input_shapes, x_ps.rank().compatible(4), "`x` input must be rank 4.");
    NODE_SHAPE_INFER_CHECK(op, input_shapes, C_ps.rank().compatible(4), "`C` input must be rank 4.");
    NODE_SHAPE_INFER_CHECK(op, input_shapes, state_ps.rank().compatible(4), "`recurrent_state` input must be rank 4.");

    if (A_ps.rank().is_dynamic() || dt_ps.rank().is_dynamic() || B_ps.rank().is_dynamic() || x_ps.rank().is_dynamic() ||
        C_ps.rank().is_dynamic() || state_ps.rank().is_dynamic()) {
        return {x_ps, state_ps};
    }
