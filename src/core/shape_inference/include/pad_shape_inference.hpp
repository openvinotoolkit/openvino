...
std::vector<TShape> shape_infer(const Pad* op,
    ...
    if (arg_shape_rank.is_static()) {
        ...
        if (pads_begin_coord && pads_end_coord) {
            ...
            NODE_VALIDATION_CHECK(op,
                                  pads_begin_rank.is_dynamic() || static_cast<int64_t>(pads_begin_shape[0].get_length()) <= static_cast<int64_t>(arg_rank_len),
                                  "Number of elements of pads_begin must be >= 0 and <= arg rank "
                                  "(pads_begin_shape[0]: ",
                                  pads_begin_shape[0],
                                  ").");
            NODE_VALIDATION_CHECK(op,
                                  pads_begin_rank.is_dynamic() || static_cast<int64_t>(pads_end_shape[0].get_length()) <= static_cast<int64_t>(arg_rank_len),
