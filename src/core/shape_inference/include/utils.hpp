 ...
 */
template <class TShape>
std::shared_ptr<std::vector<std::pair<int64_t, int64_t>>> get_input_bounds(
    const ov::Node* op,
    size_t idx,
    const std::map<size_t, std::shared_ptr<ngraph::runtime::HostTensor>>& constant_data) {
    using BoundsVector = std::vector<std::pair<int64_t, int64_t>>;

    const auto make_bounds_vec = [](const std::vector<int64_t>& lb, const std::vector<int64_t>& ub) {
        BoundsVector out;
        out.reserve(lb.size());
        std::transform(lb.begin(), lb.end(), ub.begin(), std::back_inserter(out), [](int64_t lb, int64_t ub) {
    ...
    if (get_data_as_int64<TShape>(idx, op, lower, constant_data)) {
        return std::make_shared<BoundsVector>(make_bounds_vec(lower, lower));
    } else {
        // if no const data try get input bounds
        auto bounds = ngraph::evaluate_both_bounds(op->get_input_source_output(idx));
        if (bounds.first && bounds.second) {
            lower = std::make_shared<ov::opset1::Constant>(bounds.first)->cast_vector<int64_t>();
            auto&& upper = std::make_shared<ov::opset1::Constant>(bounds.second)->cast_vector<int64_t>();
            return std::make_shared<BoundsVector>(make_bounds_vec(lower, upper));
        }
    }
