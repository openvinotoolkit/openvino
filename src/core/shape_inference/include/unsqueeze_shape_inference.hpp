...
void shape_infer(const Unsqueeze* op,
    ...
    const auto has_axes = get_data_as_int64<T>(Unsqueeze::AXES, op, axes, constant_data);
    ...
    if (has_axes && arg_shape.rank().is_static()) {
        ...
        std::unordered_set<int64_t> tmp(std::make_move_iterator(axes.begin()), std::make_move_iterator(axes.end()));
        ...
        auto normalized_axes = normalize_axes(op->description(), unique_axes, expanded_rank);
        const std::set<int64_t> axes_values(std::make_move_iterator(normalized_axes.begin()),
                                            std::make_move_iterator(normalized_axes.end()));
