...
void shape_infer(const Transpose* op,
    ...
    const auto& order_shape = input_shapes[TransposeIn::ORDER];
    auto& output = output_shapes[TransposeOut::ARG_T];

    std::vector<int64_t> axes;

    if (get_data_as_int64<T>(op::TransposeIn::ORDER, op, axes, constant_data)) {
        const auto out_rank_size =
            input_shape.rank().is_static() ? input_shape.rank().get_length() : order_shape[0].get_length();
