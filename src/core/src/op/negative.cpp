...
BWDCMP_RTTI_DEFINITION(ov::op::v1::Negative);

op::Negative::Negative(const Output<Node>& arg) : UnaryElementwiseArithmetic(arg) {
    constructor_validate_and_infer_types();
}

bool ov::op::v1::Negative::visit_attributes(AttributeVisitor& visitor) {
    NGRAPH_OP_SCOPE(v1_Negative_visit_attributes);
    return true;
}

shared_ptr<Node> op::Negative::clone_with_new_inputs(const OutputVector& new_args) const {
    NGRAPH_OP_SCOPE(v1_Negative_clone_with_new_inputs);
...
bool op::Negative::evaluate(const HostTensorVector& outputs, const HostTensorVector& inputs) const {
    NGRAPH_OP_SCOPE(v1_Negative_evaluate);
...
bool op::Negative::has_evaluate() const {
    NGRAPH_OP_SCOPE(v1_Negative_has_evaluate);
