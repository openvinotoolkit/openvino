...
BWDCMP_RTTI_DEFINITION(ov::op::v1::Exp);

op::Exp::Exp(const Output<Node>& arg) : UnaryElementwiseArithmetic(arg) {
    constructor_validate_and_infer_types();
}

bool ov::op::v1::Exp::visit_attributes(AttributeVisitor& visitor) {
    NGRAPH_OP_SCOPE(v1_Exp_visit_attributes);
    return true;
}

shared_ptr<Node> op::Exp::clone_with_new_inputs(const OutputVector& new_args) const {
    NGRAPH_OP_SCOPE(v1_Exp_clone_with_new_inputs);
