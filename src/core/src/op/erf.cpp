...
BWDCMP_RTTI_DEFINITION(ov::op::v1::Erf);

bool ov::op::v1::Erf::visit_attributes(AttributeVisitor& visitor) {
    NGRAPH_OP_SCOPE(v1_Erf_visit_attributes);
    return true;
}

shared_ptr<Node> op::Erf::clone_with_new_inputs(const OutputVector& new_args) const {
    NGRAPH_OP_SCOPE(v1_Erf_clone_with_new_inputs);
