...
BWDCMP_RTTI_DEFINITION(ov::op::v1::ShuffleChannels);
...
bool ov::op::v1::ShuffleChannels::visit_attributes(AttributeVisitor& visitor) {
    NGRAPH_OP_SCOPE(v1_ShuffleChannels_visit_attributes);
...
void op::ShuffleChannels::validate_and_infer_types() {
    NGRAPH_OP_SCOPE(v1_ShuffleChannels_validate_and_infer_types);
...
shared_ptr<Node> op::ShuffleChannels::clone_with_new_inputs(const OutputVector& new_args) const {
    NGRAPH_OP_SCOPE(v1_ShuffleChannels_clone_with_new_inputs);
    if (new_args.size() != 1) {
...
bool op::ShuffleChannels::evaluate(const HostTensorVector& outputs, const HostTensorVector& inputs) const {
    NGRAPH_OP_SCOPE(v1_ShuffleChannels_evaluate);
    return evaluate_shuffle_channels(outputs, inputs);
}

bool op::ShuffleChannels::has_evaluate() const {
    NGRAPH_OP_SCOPE(v1_ShuffleChannels_has_evaluate);
    return true;
}
