...
bool op::v1::ConvertLike::constant_fold(OutputVector& output_values, const OutputVector& input_values) {
    ...
    if (auto data_const = std::dynamic_pointer_cast<op::v1::Constant>(input_values[0].get_node_shared_ptr())) {
        auto convert = make_shared<ov::op::v1::Convert>(input_values[0], input_values[1].get_element_type());
