...
BWDCMP_RTTI_DEFINITION(ov::op::v1::Tan);

op::Tan::Tan(const Output<Node>& arg) : UnaryElementwiseArithmetic(arg) {
    constructor_validate_and_infer_types();
}

bool ov::op::v1::Tan::visit_attributes(AttributeVisitor& visitor) {
    NGRAPH_OP_SCOPE(v1_Tan_visit_attributes);
    return true;
}

shared_ptr<Node> op::Tan::clone_with_new_inputs(const OutputVector& new_args) const {
    NGRAPH_OP_SCOPE(v1_Tan_clone_with_new_inputs);
