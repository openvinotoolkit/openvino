...
BWDCMP_RTTI_DEFINITION(op::v8::ExperimentalDetectronGenerateProposalsSingleImage);

op::v8::ExperimentalDetectronGenerateProposalsSingleImage::ExperimentalDetectronGenerateProposalsSingleImage(
    const Output<Node>& im_info,
    const Output<Node>& anchors,
    const Output<Node>& deltas,
    const Output<Node>& scores,
...
shared_ptr<Node> op::v8::ExperimentalDetectronGenerateProposalsSingleImage::clone_with_new_inputs(
    const OutputVector& new_args) const {
    NGRAPH_OP_SCOPE(v8_ExperimentalDetectronGenerateProposalsSingleImage_clone_with_new_inputs);
    check_new_args_count(this, new_args);
    return make_shared<op::v8::ExperimentalDetectronGenerateProposalsSingleImage>(new_args.at(0),
                                                                                  new_args.at(1),
                                                                                  new_args.at(2),
                                                                                  new_args.at(3),
                                                                                  m_attrs);
...
bool op::v8::ExperimentalDetectronGenerateProposalsSingleImage::visit_attributes(AttributeVisitor& visitor) {
    NGRAPH_OP_SCOPE(v8_ExperimentalDetectronGenerateProposalsSingleImage_visit_attributes);
    visitor.on_attribute("min_size", m_attrs.min_size);
    visitor.on_attribute("nms_threshold", m_attrs.nms_threshold);
    visitor.on_attribute("post_nms_count", m_attrs.post_nms_count);
    visitor.on_attribute("pre_nms_count", m_attrs.pre_nms_count);
...
void op::v8::ExperimentalDetectronGenerateProposalsSingleImage::validate_and_infer_types() {
    NGRAPH_OP_SCOPE(v8_ExperimentalDetectronGenerateProposalsSingleImage_validate_and_infer_types);
    ...
    set_output_type(0, input_et, output_shapes[0]);
    set_output_type(1, input_et, output_shapes[1]);
    set_output_type(2, element::i32, output_shapes[2]);
}
