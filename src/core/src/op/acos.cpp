...
BWDCMP_RTTI_DEFINITION(ov::op::v1::Acos);

ov::op::v1::Acos::Acos(const Output<Node>& arg) : UnaryElementwiseArithmetic(arg) {
    constructor_validate_and_infer_types();
}

std::shared_ptr<ov::Node> ov::op::v1::Acos::clone_with_new_inputs(const OutputVector& new_args) const {
    NGRAPH_OP_SCOPE(v1_Acos_clone_with_new_inputs);
...
bool ov::op::v1::Acos::evaluate(const HostTensorVector& outputs, const HostTensorVector& inputs) const {
    NGRAPH_OP_SCOPE(v1_Acos_evaluate);
    return acosop::evaluate_acos(inputs[0], outputs[0], shape_size(get_output_shape(0)));
}

bool ov::op::v1::Acos::has_evaluate() const {
    NGRAPH_OP_SCOPE(v1_Acos_has_evaluate);
