...
AvgPool::AvgPool(const Output<Node>& arg,
    ...
    : util::AvgPoolBase(arg, strides, Strides(kernel.size(), 1), pads_begin, pads_end, kernel, exclude_pad, rounding_type, auto_pad) {
...
AvgPool::AvgPool(const Output<Node>& arg,
                 ...
                 RoundingType rounding_type,
                 const PadType& auto_pad)
    : util::AvgPoolBase(arg, strides, dilations, pads_begin, pads_end, kernel, exclude_pad, rounding_type, auto_pad) {
    constructor_validate_and_infer_types();
}
