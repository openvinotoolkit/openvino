...
GroupQueryAttentionExtension::GroupQueryAttentionExtension(const ov::OutputVector& args,
...
// FIXME: Reuse the same function from file attention.cpp, but it requires a bit of adaptation -- I have redesigned part
// of the inputs a bit here and in the helper functions below
...
OutputVector make_split(const Output& value, const std::vector<int64_t>& split_lengths, int64_t axis) {
    ...
    const auto split_lengths_node = Constant::create(ov::element::i64, Shape{split_lengths.size()}, split_lengths);
...
ov::NodeVector split_to_QKV(const Output& node, int64_t num_heads, const std::vector<int64_t>& qkv_hidden_sizes) {
    ...
    if (qkv_hidden_sizes.size() == 0) {
        ...
        auto new_shape =
            std::make_shared<v0::Concat>(ov::OutputVector{batch_size_seq_len, num_heads_node, head_size}, 0);
...
Output rope(const Output& x,
    ...
    Output res_0 =
        make_shared<Subtract>(make_shared<Multiply>(x_split[0], cos), make_shared<Multiply>(x_split[1], sin));

    Output res_1 = make_shared<Add>(make_shared<Multiply>(x_split[0], sin), make_shared<Multiply>(x_split[1], cos));
...
Output broadcast_groups(const Output& cache, const int num_kv_heads, const int num_heads) {
    ...
    auto broadcast_shape =
        make_shared<Concat>(OutputVector{get_elements(shapeof, {0, 1}),
    ...
    auto reshape_shape = make_shared<Concat>(
        OutputVector{Constant::create(element::i32, Shape{3}, {0, num_heads, -1}), get_elements(shapeof, {3})},
        0);
...
// The next methods are variants of cache management. Each return a pair of outputs: {kv_to_be_passed_to_sdpa,
// kv_to_be_returned_from_decomp}

OutputVector concat_cache_generic(const Output& past, const Output& current) {
    const Output concated = make_shared<Concat>(ov::OutputVector{past, current},
                                                2);  // 2 is the dimension index that corresponds to sequence len
...
OutputVector update_cache_generic(const Output& past, const Output& current, const Output& past_len) {
    ...
    Output update_indices = make_shared<Range>(squeeze_1d(past_len),
                                               squeeze_1d(update_end),
                                               Constant::create(element::i32, Shape{}, {1}),
                                               element::i32);
    Output updated = make_shared<ScatterUpdate>(
        past,
        update_indices,
        current,
        Constant::create(element::i32, Shape{1}, {2}));  // 2 is the dimension index that corresponds to sequence len
...
OutputVector update_cache_seu(const Output& past, const Output& current, const Output& past_len) {
    ...
    auto unsqueezed_indices =
        make_shared<Unsqueeze>(update_indices, Constant::create(element::i32, Shape{3}, {0, 1, 3}));
    auto broadcast_indices = make_shared<Broadcast>(unsqueezed_indices, shape_of);
    Output seu = make_shared<ScatterElementsUpdate>(past,
                                                    broadcast_indices,
                                                    current,
                                                    Constant::create(element::i32, Shape{1}, {2}));
    return {seu, seu};
}

Output attn_mask_npuw(const Output& past_seq_len,
    ...
    //    - the last block of 1s has length equal to `current_seq_len` (usually = 1 in case of generate iteration),
    //    usually static
    ...
    past_mask = make_shared<Broadcast>(
        past_mask,
        make_shared<Concat>(OutputVector{current_seq_len, total_seq_len}, 0));  // replicated current_seq_len times

    const Output curr_range = make_shared<Range>(zero, squeeze_1d(current_seq_len), one, element::i32);
    const Output curr_mask =
        make_shared<LessEqual>(curr_range,
                               make_shared<Unsqueeze>(curr_range, Constant::create(element::i32, Shape{}, {1})));

    return make_shared<Concat>(ov::OutputVector{past_mask, curr_mask}, 1);
}

using attn_mask_callback = std::function<Output(
    const Output& past_seq_len,  // occupied part of KV cache, 1D tensor with one or multiple elements depending on the
                                 // current level of support of unaligned sequences in the batch
    const Output& current_seq_len,  // number of currently processed tokens, usually 1 in the normal generate iteration
                                    // (not prefill)
    const Output& total_seq_len,
    const Output& kv_tensors_seq_len  // the total sequence length of KV tensors that come to SDPA to generate a mask of
                                      // the correct shape even if KV is partially filled
    )>;
...
ov::OutputVector group_query_attention_decomposition(
    ...
    attn_mask_callback attn_mask = attn_mask_callback()  // attn_mask generator for SDPA, if not provided, the attn_mask
                                                         // input is not used and causal flag is set
) {
    ...
    // const auto& total_sequence_length = inputs[6];  // use it for checking dimensions only, it should be deduced from
    // other parameters and shapes
    const auto& cos = inputs[7];
    const auto& sin = inputs[8];

    // FIXME: It works only when KV cache is dynamically growing and doesn't have unused space inside. So it is not
    // compatible with statically-shaped KV cache. const auto past_seq_len = detail::get_dimensions(past_K, {0});
    // TODO: GQA spec is not compatible with test model. Spec supposes 1D tensor, in the test model we have 2D tensor,
    // flattening to work in both cases.

    // FIXME: Unaligned elements in KV cache are not supported.
    // We just get the first of the seq lens as a common value for all past sequences ignoring others, under assumption
    // that they are all the same
    const auto& past_seq_len = get_elements(
        std::make_shared<v1::Reshape>(seqlens_k, v0::Constant::create(element::i32, Shape{1}, {-1}), false),
        {0});
    ...
    // If we want to implement not aligned batch of dimensions we have to form not uniform causal mask for attention
    // that adds a significant porition of the code.

    // FIXME: The same tensor at input/output of past/preset K and V are not supported.
    // It requires more complex tensor manipulations that are introduce overhead into pure tensor-value data flow and
    // should be implemented if we really have demand for that. Also inplace KV-cache modification logic is not
    // supported efficiently in any plugins (CPU, GPU and NPU).
...
ov::OutputVector group_query_attention_decomposition(std::shared_ptr<GroupQueryAttentionExtension> gqa) {
    return detail::group_query_attention_decomposition(gqa->input_values(),
                                                       gqa->get_num_heads(),
