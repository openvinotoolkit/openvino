...
BWDCMP_RTTI_DEFINITION(ov::op::v1::DepthToSpace);
...
bool op::DepthToSpace::visit_attributes(AttributeVisitor& visitor) {
    NGRAPH_OP_SCOPE(v1_DepthToSpace_visit_attributes);
...
std::shared_ptr<Node> op::DepthToSpace::clone_with_new_inputs(const OutputVector& new_args) const {
    NGRAPH_OP_SCOPE(v1_DepthToSpace_clone_with_new_inputs);
    if (new_args.size() != 1) {
...
void op::DepthToSpace::validate_and_infer_types() {
    NGRAPH_OP_SCOPE(v1_DepthToSpace_validate_and_infer_types);
...
bool op::DepthToSpace::evaluate(const HostTensorVector& outputs, const HostTensorVector& inputs) const {
    NGRAPH_OP_SCOPE(v1_DepthToSpace_evaluate);
    return evaluate_depth_to_space(outputs, inputs, m_blocksize, m_mode);
}

bool op::DepthToSpace::has_evaluate() const {
    NGRAPH_OP_SCOPE(v1_DepthToSpace_has_evaluate);
    return !get_input_partial_shape(0).is_dynamic();
}

std::ostream& ov::operator<<(std::ostream& s, const ov::op::v1::DepthToSpace::DepthToSpaceMode& type) {
...
BWDCMP_RTTI_DEFINITION(AttributeAdapter<ov::op::v1::DepthToSpace::DepthToSpaceMode>);
}  // namespace ov
