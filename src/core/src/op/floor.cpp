...
BWDCMP_RTTI_DEFINITION(ov::op::v1::Floor);

op::Floor::Floor(const Output<Node>& arg) : UnaryElementwiseArithmetic(arg) {
    constructor_validate_and_infer_types();
}

bool ov::op::v1::Floor::visit_attributes(AttributeVisitor& visitor) {
    NGRAPH_OP_SCOPE(v1_Floor_visit_attributes);
    return true;
}

shared_ptr<Node> op::Floor::clone_with_new_inputs(const OutputVector& new_args) const {
    NGRAPH_OP_SCOPE(v1_Floor_clone_with_new_inputs);
