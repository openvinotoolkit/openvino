    ...
    if (order.get_element_type().is_integral()) {
        ...
        if (arg_type == ov::element::i4 || arg_type == ov::element::u4) {
            ...
            if (arg.get_shape().size() == 2) {
                ...
                OPENVINO_ASSERT(axes_order[0] == 0, "Transpose for i4/u4 dtype is supported only for byx=>bxy case");
                ...
                OPENVINO_ASSERT(false, "Transpose for i4/u4 dtype is supported only for ndims <= 3");
            }
        } else {
