...
bool Transpose::evaluate(TensorVector& outputs, const TensorVector& inputs) const {
    ...
    if (order.get_element_type().is_integral()) {
        ...
        if (arg_type == ov::element::u2) {
            reference::transpose_by_iterator<ov::element::u2>(static_cast<const uint8_t*>(arg.data()),
            ...
            reference::transpose_by_iterator<ov::element::u3>(static_cast<const uint8_t*>(arg.data()),
                                                              static_cast<uint8_t*>(out.data()),
                                                              arg.get_shape(),
                                                              axes_order,
                                                              out_shape);
        } else if (arg_type == ov::element::u6) {
            reference::transpose_by_iterator<ov::element::u6>(static_cast<const uint8_t*>(arg.data()),
                                                              static_cast<uint8_t*>(out.data()),
                                                              arg.get_shape(),
                                                              axes_order,
                                                              out_shape);
