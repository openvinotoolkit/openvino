...
#    define OV_OP_SCOPE(region) OV_SCOPE(ov_op_exec, region)
...
#    define OV_OP_SCOPE(region)                                             \
        if (OV_CC_SCOPE_IS_ENABLED(OV_PP_CAT3(ov_op_exec, _, region)) == 0) \
...
#else
#    define OV_OP_SCOPE(region) OV_ITT_SCOPED_TASK(ov::itt::domains::ov_op_exec, OV_PP_TOSTRING(region))
#    define REGISTER_OP(opset_name, op_name)
#    define INSERT_OP(opset_name, op_name, op_namespace) opset.insert<op_namespace::op_name>()
#endif
...
#define OPENVINO_TYPE_CASE(region, a, ...)                      \
    case ov::element::Type_t::a: {                              \
        OV_SCOPE(ov_op_exec, OV_PP_CAT3(region, _, a)) {        \
...
#define OPENVINO_2_TYPES_CASE(region, a, b, ...)                                \
    case element::Type_t::a: {                                                  \
        OV_SCOPE(ov_op_exec, OV_PP_CAT4(region, _, a, b)) {                     \
