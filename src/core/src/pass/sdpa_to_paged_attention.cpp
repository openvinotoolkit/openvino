...
bool ov::pass::SDPAToPagedAttention::run_on_model(const std::shared_ptr<ov::Model>& model) {
    ...
    auto max_context_len =
        setName(std::make_shared<v0::Parameter>(element::i32, PartialShape{}), "max_context_len");
    ...
    std::shared_ptr<v0::Parameter> input_ids_node = std::dynamic_pointer_cast<v0::Parameter>(model->input("input_ids").get_node_shared_ptr());
    input_ids_node->set_partial_shape(PartialShape{-1});
    auto unsqueezed_input_ids = std::make_shared<v0::Unsqueeze>(input_ids_node, v0::Constant::create(element::i32, Shape{}, {1}));
    replace_node(input_ids_node, unsqueezed_input_ids);

    auto cur_seq_len = std::make_shared<v1::Gather>(std::make_shared<v3::ShapeOf>(unsqueezed_input_ids),
                                                    v0::Constant::create(element::i64, Shape{}, {1}),
                                                    v0::Constant::create(element::i64, Shape{}, {0}));
    auto prev_max_seq_len = std::make_shared<v1::Subtract>(max_context_len, std::make_shared<v0::Convert>(cur_seq_len, element::i32));
    ...
    if (!has_parameter(model, "position_ids")) {
        position_ids =
            setName(std::make_shared<v0::Parameter>(element::i64, PartialShape{-1}), "position_ids");
    ...
    auto unsqueezed_position_ids = std::make_shared<v0::Unsqueeze>(position_ids, v0::Constant::create(element::i32, Shape{}, {1}));
