...
bool ov::pass::MakeStateful::run_on_model(const std::shared_ptr<ov::Model>& f) {
    ...
    for (size_t i = 0; i < m_param_res_pairs.size(); ++i) {
        ...
        auto variable = std::make_shared<ov::op::util::Variable>(
            ov::op::util::VariableInfo{param->get_output_partial_shape(0).to_shape(),
                                       param->get_element_type(),
                                       var_name});
        variables.push_back(variable);

        // Create ReadValue
        auto const_zero = std::make_shared<ov::op::v0::Constant>(param->get_element_type(),
                                                                 param->get_output_partial_shape(0).to_shape(),
                                                                 0);
