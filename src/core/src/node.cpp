...
bool ov::Node::match_value(ov::pass::pattern::Matcher* matcher,
    ...
    if (pattern_value.get_index() != graph_value.get_index() ||
        ...
        OV_LOG_MATCHING(matcher, matcher->level_str, OV_BLOCK_END, "  INDEX, ELEMENT TYPE or PARTIAL SHAPE MISMATCH. EXPECTED IN PATTERN: ",
...
bool ov::Node::match_node(ov::pass::pattern::Matcher* matcher, const Output<Node>& graph_value) {
    ...
    if (graph_value.get_node_shared_ptr()->get_type_info().is_castable(get_type_info())) {
        ...
        if (matcher->match_arguments(this, graph_value.get_node_shared_ptr())) {
            ...
            OV_LOG_MATCHING(matcher, matcher->level_str, OV_BLOCK_END, OV_GREEN, "  ALL ARGUMENTS MATCHED");
            return true;
        }
        OV_LOG_MATCHING(matcher, matcher->level_str, OV_BLOCK_BODY);
        OV_LOG_MATCHING(matcher, matcher->level_str, OV_BLOCK_END, OV_RED, "  ARGUMENTS DIDN'T MATCH");
    } else {
        OV_LOG_MATCHING(matcher, matcher->level_str, OV_BLOCK_END, OV_RED, "  NODES' TYPE DIDN'T MATCH. EXPECTED: ", ov::node_version_type_str(shared_from_this()),
