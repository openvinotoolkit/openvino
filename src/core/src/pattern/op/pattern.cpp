...
const PatternSymbolValue& get_element(const std::vector<PatternSymbolValue>& values, int idx) {
    if (idx < 0)
        idx += values.size();
    OPENVINO_ASSERT(static_cast<size_t>(idx) < values.size(), "Unexpected index");
    return values[idx];
}
