...
# def gather2(name:str, x, y, z):
#     paddle.enable_static()

#     with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
#         data = paddle.static.data(name='x', shape=x.shape, dtype = x.dtype)
#         index = paddle.static.data(name='index', shape=y.shape, dtype = y.dtype)
#         axis = paddle.static.data(name='axis', shape=z.shape, dtype = z.dtype)
...
#         saveModel(name, exe, feedkeys=['x', 'index', 'axis'], fetchlist=[out], inputs=[x, y, z], outputs=[outs[0]], target_dir=sys.argv[1])
...
def main():
    ...
    gather1("gather1_multi_dimension", xnp, index_np)
    ...
    gather1("gather1_one_dimension", xnp, index_np)
    ...
    gather1("gather1_one_dimension2", xnp, index_np)
    ...
    # gather2("gather2_multi_dimension", xnp, index_np, axis_np)
