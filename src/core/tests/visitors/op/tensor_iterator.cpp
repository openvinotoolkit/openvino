...
TEST(attributes, tensor_iterator_lstm) {
    ...
    auto LSTM_cell = make_shared<ov::op::v4::LSTMCell>(ov::utils::reshape(X, Shape{N, I}),
                                                       ov::utils::reshape(H_t, Shape{N, H}),
                                                       ov::utils::reshape(C_t, Shape{N, H}),
                                                       W_body,
                                                       R_body,
                                                       H);
    auto H_o = ov::utils::reshape(LSTM_cell->output(0), Shape{N, 1, H});
    auto C_o = ov::utils::reshape(LSTM_cell->output(1), Shape{N, 1, H});
