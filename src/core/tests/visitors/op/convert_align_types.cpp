...
TEST(attributes, convert_align_types_op_default) {
    NodeBuilder::opset().insert<op::v14::ConvertAlignTypes>();
    ...
    const auto convert = std::make_shared<op::v14::ConvertAlignTypes>(lhs, rhs);
    NodeBuilder builder(convert, {lhs, rhs});

    const auto expected_attr_count = 3;
    EXPECT_EQ(builder.get_value_map_size(), expected_attr_count);

    const auto g_convert = ov::as_type_ptr<op::v14::ConvertAlignTypes>(builder.create());
...
TEST(attributes, convert_align_types_constructor) {
    ...
    const auto convert =
        std::make_shared<op::v14::ConvertAlignTypes>(lhs, rhs, pytorch_scalar_align, promote_unsafe, element::i64);
    NodeBuilder builder(convert, {lhs, rhs});
