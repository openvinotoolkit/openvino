...
TEST(attributes, convert_align_types_op_default) {
    NodeBuilder::opset().insert<ov::op::v14::ConvertAlignTypes>();
    ...
    const auto convert = std::make_shared<ov::op::v14::ConvertAlignTypes>(in0, in1);
    NodeBuilder builder(convert, {in0, in1});

    const auto expected_attr_count = 3;
    EXPECT_EQ(builder.get_value_map_size(), expected_attr_count);

    const auto g_convert = ov::as_type_ptr<ov::op::v14::ConvertAlignTypes>(builder.create());
...
TEST(attributes, convert_align_types_constructor) {
    ...
    const auto convert = std::make_shared<ov::op::v14::ConvertAlignTypes>(in0,
                                                                          in1,
                                                                          pytorch_scalar_align,
                                                                          promote_unsafe,
                                                                          ov::element::i64);
    NodeBuilder builder(convert, {in0, in1});
