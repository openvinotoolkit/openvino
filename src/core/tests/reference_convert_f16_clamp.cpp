...
// JIT/intrinsics and the scalar paths.
...
TEST(reference_convert_f16_clamp, f32_to_f16_with_clamp_clamps_specials) {
...
TEST(reference_convert_f16_clamp, bf16_to_f16_with_clamp_clamps_specials) {
    const float pinf = std::numeric_limits<float>::infinity();
    const float ninf = -std::numeric_limits<float>::infinity();
    const float nan = std::numeric_limits<float>::quiet_NaN();
...
TEST(reference_convert_f16_clamp, f32_to_f16_preserve_specials_and_clamps_finite) {
    const float pinf = std::numeric_limits<float>::infinity();
    const float ninf = -std::numeric_limits<float>::infinity();
    const float nan = std::numeric_limits<float>::quiet_NaN();
    ...
    ov::reference::convert_from_f32_to_f16_with_clamp_preserve_specials(in.data(), out.data(), in.size());
...
TEST(reference_convert_f16_clamp, bf16_to_f16_preserve_specials_and_clamps_finite) {
    const float pinf = std::numeric_limits<float>::infinity();
    const float ninf = -std::numeric_limits<float>::infinity();
    const float nan = std::numeric_limits<float>::quiet_NaN();
    const float big = 1.0e5f;  // finite bf16, exceeds f16::max -> must clamp
    ...
    ov::reference::convert_from_bf16_to_f16_with_clamp_preserve_specials(in.data(), out.data(), in.size());
