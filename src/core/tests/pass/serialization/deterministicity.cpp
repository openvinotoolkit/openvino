...
TEST_F(SerializationDeterministicityTest, BasicModel) {
    const std::string model = CommonTestUtils::getModelFromTestModelZoo(ov::util::path_join({SERIALIZED_ZOO, "ir/add_abc.onnx"}));
...
TEST_F(SerializationDeterministicityTest, ModelWithMultipleLayers) {
    const std::string model = CommonTestUtils::getModelFromTestModelZoo(ov::util::path_join({SERIALIZED_ZOO, "ir/addmul_abc.onnx"}));
...
TEST_F(SerializationDeterministicityTest, ModelWithMultipleOutputs) {
    const std::string model = CommonTestUtils::getModelFromTestModelZoo(ov::util::path_join({SERIALIZED_ZOO, "ir/split_equal_parts_2d.xml"}));
    const std::string weights = CommonTestUtils::getModelFromTestModelZoo(ov::util::path_join({SERIALIZED_ZOO, "ir/split_equal_parts_2d.bin"}));
...
TEST_F(SerializationDeterministicityTest, ModelWithConstants) {
    const std::string model = CommonTestUtils::getModelFromTestModelZoo(ov::util::path_join({SERIALIZED_ZOO, "ir/add_abc_initializers.xml"}));
    const std::string weights = CommonTestUtils::getModelFromTestModelZoo(ov::util::path_join({SERIALIZED_ZOO, "ir/add_abc_initializers.bin"}));
