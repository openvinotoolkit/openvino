...
NGRAPH_TEST(${BACKEND_NAME}, onnx_model_affine) {
    ...
    auto expected_output = ngraph::test::NDArray<float, 2>{{{50.f, 50.5f, 51.f}}}.get_vector();
...
NGRAPH_TEST(${BACKEND_NAME}, onnx_model_crop) {
    ...
    auto expected_output = ngraph::test::NDArray<float, 4>{{{{24.f, 25.f}, {28.f, 29.f}}}}.get_vector();
...
NGRAPH_TEST(${BACKEND_NAME}, onnx_model_crop_with_scale) {
    ...
    auto expected_output = ngraph::test::NDArray<float, 4>{{{{24.f, 25.f, 26.f}, {28.f, 29.f, 30.f}}}}.get_vector();
