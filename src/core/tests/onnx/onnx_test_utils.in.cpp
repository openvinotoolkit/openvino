...
TYPED_TEST_P(ElemTypesTests, onnx_test_add_abc_set_precission) {
    ...
    ov::onnx_editor::ONNXModelEditor editor{ngraph::file_util::path_join(SERIALIZED_ZOO, "onnx/add_abc_3d.onnx")};
...
NGRAPH_TEST(${BACKEND_NAME}, add_abc_from_ir) {
    const auto ir_xml = ngraph::file_util::path_join(SERIALIZED_ZOO, "ir/add_abc.xml");
...
NGRAPH_TEST(${BACKEND_NAME}, add_abc_from_ir_with_bin_path) {
    const auto ir_xml = ngraph::file_util::path_join(SERIALIZED_ZOO, "ir/add_abc.xml");
    const auto ir_bin = ngraph::file_util::path_join(SERIALIZED_ZOO, "ir/weights/add_abc.bin");
