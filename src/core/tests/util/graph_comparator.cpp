...
Comparator::Result Comparator::compare(ngraph::Node* node1, ngraph::Node* node2, std::ostream& err_log) {
    ...
    if (!subgraph_nodes) {
        auto inputs_result = compare_inputs(node1, node2);
        if (!inputs_result.valid)
            return inputs_result;

        auto outputs_result = compare_outputs(node1, node2);
        if (!outputs_result.valid)
...
Comparator::Result Comparator::compare_inputs(ngraph::Node* node1, ngraph::Node* node2) {
    ...
    const auto& err_string = err_log.str();
    if (!err_string.empty()) {
        return Comparator::Result::error(err_string);
    }

    return Comparator::Result::ok();
}

Comparator::Result Comparator::compare_outputs(ngraph::Node* node1, ngraph::Node* node2) {
    std::stringstream err_log;
...
Comparator::Result Comparator::compare_nodes(ngraph::Node* node1, ngraph::Node* node2) {
    ...
    if (should_compare(CmpValues::ATTRIBUTES)) {
        return attributes::compare(node1, node2, m_comparison_flags);
    }
    return Result::ok();
}
