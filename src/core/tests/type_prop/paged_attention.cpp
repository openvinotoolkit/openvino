...
TEST(type_prop, paged_attention_static_eviction_per_block) {
    ...
    const auto qq_bias = std::make_shared<op::v0::Parameter>(element::f32, PartialShape{1, 4, 4});
...
TEST(type_prop, paged_attention_dynamic_ranks_and_types) {
    ...
    const auto qq_bias = std::make_shared<op::v0::Parameter>(element::dynamic, dyn);
