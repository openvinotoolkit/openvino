...
TEST(type_prop, lstm_cell) {
    ...
    const auto lstm_cell = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, hidden_size);
...
TEST(type_prop, lstm_cell_invalid_input) {
    ...
    try {
        const auto lstm_cell = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, B, hidden_size);
        FAIL() << "LSTMCell node was created with invalid data.";
    } catch (const NodeValidationFailure& error) {
...
TEST(type_prop, lstm_cell_dynamic_batch_size) {
    ...
    const auto C_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{batch_size, hidden_size});
...
TEST(type_prop, lstm_cell_dynamic_hidden_size) {
    ...
    const auto X = make_shared<op::v0::Parameter>(element::f32, PartialShape{batch_size, input_size});
    const auto W = make_shared<op::v0::Parameter>(element::f32, PartialShape{hidden_size * gates_count, input_size});
    const auto R = make_shared<op::v0::Parameter>(element::f32, PartialShape{hidden_size * gates_count, hidden_size});
    const auto H_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{batch_size, hidden_size});
    const auto C_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{batch_size, hidden_size});

    const auto lstm_cell = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, 3);
...
TEST(type_prop, lstm_cell_invalid_input_rank0) {
    ...
    auto W = make_shared<op::v0::Parameter>(element::f32, PartialShape{gates_count * hidden_size, input_size});
    ...
    auto C_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{batch_size, hidden_size});

    // Invalid rank0 for W tensor.
    W = make_shared<op::v0::Parameter>(element::f32, PartialShape{});
    ASSERT_THROW(const auto unused = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, hidden_size),
    ...
    C_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{});
    ASSERT_THROW(const auto unused = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, hidden_size),
                 ov::NodeValidationFailure)
        << "LSTMCell node was created with invalid data.";

    // Invalid rank0 for R tensor.
    C_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{batch_size, hidden_size});
    ...
    auto B = make_shared<op::v0::Parameter>(element::f32, PartialShape{});
    ASSERT_THROW(const auto unused = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, B, hidden_size),
                 ov::NodeValidationFailure)
        << "LSTMCell node was created with invalid data.";
}
...
TEST(type_prop, lstm_cell_invalid_input_dynamic_rank) {
    ...
    auto check_dynamic_lstm = [=](const shared_ptr<op::v4::LSTMCell>& lstm) -> bool {
    ...
    auto lstm = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, hidden_size);
    ...
    lstm = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, hidden_size);
    EXPECT_EQ(check_dynamic_lstm(lstm), true);
    ...
    H_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{batch_size, hidden_size});
    C_t = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
    lstm = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, hidden_size);
    EXPECT_EQ(check_dynamic_lstm(lstm), true);
    ...
    R = make_shared<op::v0::Parameter>(element::f32, PartialShape{gates_count * hidden_size, hidden_size});
    auto B = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
    lstm = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, B, hidden_size);
    EXPECT_EQ(check_dynamic_lstm(lstm), true);
}
...
TEST(type_prop, lstm_cell_shape_from_partial) {
    ...
    {
        // from h & w
        auto X = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
        auto W = make_shared<op::v0::Parameter>(element::f32, PartialShape{gates_count * hidden_size, input_size});
        auto R = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
        auto H_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{batch_size, -1});
        auto C_t = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
        auto lstm = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, hidden_size);
        EXPECT_EQ(check_dynamic_lstm(lstm), true);
    }

    {
        ...
        auto W = make_shared<op::v0::Parameter>(element::f32, PartialShape{gates_count * hidden_size, input_size});
        auto R = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
        auto H_t = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
        auto C_t = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
        auto lstm = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, hidden_size);
        EXPECT_EQ(check_dynamic_lstm(lstm), true);
    }

    {
        ...
        auto W = make_shared<op::v0::Parameter>(element::f32, PartialShape{gates_count * hidden_size, input_size});
        auto R = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
        auto H_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{batch_size, input_size});
        auto C_t = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
        auto lstm = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, hidden_size);
        EXPECT_EQ(check_dynamic_lstm(lstm), true);
    }

    {
        ...
        auto R = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
        auto H_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{-1, hidden_size});
        auto C_t = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
        auto lstm = make_shared<op::v4::LSTMCell>(X, H_t, C_t, W, R, hidden_size);
        EXPECT_EQ(check_dynamic_lstm(lstm), true);
    }
