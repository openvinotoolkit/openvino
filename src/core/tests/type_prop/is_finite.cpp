...
TEST(type_prop, isfinite_output_shape) {
    const auto data = std::make_shared<op::v0::Parameter>(ov::element::f16, ov::Shape{4, 2});
    const auto isfinite = std::make_shared<op::v10::IsFinite>(data);

    EXPECT_EQ(isfinite->get_output_partial_shape(0), ov::PartialShape({4, 2}))
        << "The output shape of op::v10::IsFinite is incorrect";
    ASSERT_EQ(isfinite->get_shape(), (ov::Shape{4, 2})) << "The output shape of op::v10::IsFinite is incorrect";
}

TEST(type_prop, isfnite_sample_dynamic_batch) {
    const auto data = std::make_shared<op::v0::Parameter>(ov::element::f16, ov::PartialShape{ov::Dimension::dynamic(), 21, 37});
    const auto isfinite = std::make_shared<op::v10::IsFinite>(data);
...
TEST(type_prop, isfinite_output_type) {
    ...
    EXPECT_EQ(isfinite->get_element_type(), ov::element::boolean)
        << "The output element type of op::v10::IsFinite is not a boolean.";
}

TEST(type_prop, isfinite_sample_interval_dimension) {
    const auto data = std::make_shared<op::v0::Parameter>(ov::element::f16, (ov::PartialShape{ov::Dimension(2, 4), 73, 12}));
    const auto isfinite = std::make_shared<op::v10::IsFinite>(data);
...
TEST(type_prop, isfinite_sample_dynamic_shape) {
    const auto data = std::make_shared<op::v0::Parameter>(ov::element::f16, ov::PartialShape::dynamic(5));
    const auto isfinite = std::make_shared<op::v10::IsFinite>(data);
...
TEST(type_prop, isfinite_incorrect_input_type) {
    auto data = std::make_shared<op::v0::Parameter>(ov::element::i64, ov::Shape{1, 2, 3});
    try {
        auto isfinite = std::make_shared<op::v10::IsFinite>(data);
        FAIL() << "op::v10::IsFinite invalid input type not detected";
