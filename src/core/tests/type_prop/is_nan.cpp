...
TEST(type_prop, isnan_output_shape) {
    const auto data = make_shared<opset9::Parameter>(ov::element::f16, Shape{4, 2});
    ...
    ASSERT_EQ(isnan->get_shape(), (Shape{4, 2}))
        << "The output shape of IsNaN is incorrect";
}

TEST(type_prop, isnan_sample_dynamic_batch) {
    const auto data = make_shared<opset9::Parameter>(ov::element::f16, PartialShape{Dimension::dynamic(), 21, 37});
    const auto isnan = make_shared<opset10::IsNaN>(data);
...
TEST(type_prop, isnan_output_type){
    ...
    EXPECT_EQ(isnan->get_element_type(), element::boolean) 
        << "The output element type of IsNaN is not a boolean.";
}
