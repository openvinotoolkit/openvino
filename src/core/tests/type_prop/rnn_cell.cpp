...
TEST(type_prop, rnn_cell_with_bias) {
    ...
    const auto rnn_cell = make_shared<op::v0::RNNCell>(X, H_t, W, R, B, hidden_size);
    EXPECT_EQ(rnn_cell->get_output_element_type(0), element::f32);
    EXPECT_EQ(rnn_cell->get_output_shape(0), (Shape{batch_size, hidden_size}));
}
...
TEST(type_prop, rnn_cell_dynamic_batch_size) {
    ...
    const auto W = make_shared<op::v0::Parameter>(element::f32, PartialShape{hidden_size, input_size});
    const auto R = make_shared<op::v0::Parameter>(element::f32, PartialShape{hidden_size, hidden_size});
...
TEST(type_prop, rnn_cell_dynamic_hidden_size) {
    ...
    const auto rnn_cell = make_shared<op::v0::RNNCell>(X, H_t, W, R, 3);
    EXPECT_EQ(rnn_cell->get_output_element_type(0), element::f32);
    EXPECT_EQ(rnn_cell->get_output_partial_shape(0), (PartialShape{batch_size, hidden_size}));
}
...
TEST(type_prop, rnn_cell_dynamic_inputs) {
    ...
    const auto rnn_cell = make_shared<op::v0::RNNCell>(X, H_t, W, R, 2);
...
TEST(type_prop, rnn_cell_invalid_input_rank0) {
    ...
    ASSERT_THROW(const auto unused = make_shared<op::v0::RNNCell>(X, H_t, W, R, hidden_size), ov::NodeValidationFailure)
    ...
    X = make_shared<op::v0::Parameter>(element::f32, Shape{batch_size, input_size});
    ...
    ASSERT_THROW(const auto unused = make_shared<op::v0::RNNCell>(X, H_t, W, R, B, hidden_size),
...
TEST(type_prop, rnn_cell_input_dynamic_rank) {
    ...
    auto R = make_shared<op::v0::Parameter>(element::f32, PartialShape{hidden_size_dim, hidden_size_dim});
    auto H_t = make_shared<op::v0::Parameter>(element::f32, PartialShape{batch_size, hidden_size});

    auto check_dynamic_rnn = [=](const shared_ptr<op::v0::RNNCell>& rnn) -> bool {
    ...
    auto rnn_w = make_shared<op::v0::RNNCell>(X, H_t, W, R, hidden_size);
    EXPECT_EQ(check_dynamic_rnn(rnn_w), true);

    // Invalid dynamic rank for X tensor.
    W = make_shared<op::v0::Parameter>(element::f32, PartialShape{hidden_size_dim, input_size});
    X = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
    auto rnn_x = make_shared<op::v0::RNNCell>(X, H_t, W, R, hidden_size);
    ...
    auto rnn_h = make_shared<op::v0::RNNCell>(X, H_t, W, R, hidden_size);
    ...
    auto rnn_r = make_shared<op::v0::RNNCell>(X, H_t, W, R, hidden_size);
    EXPECT_EQ(check_dynamic_rnn(rnn_r), true);

    // Invalid dynamic rank for B tensor.
    R = make_shared<op::v0::Parameter>(element::f32, PartialShape{hidden_size_dim, hidden_size_dim});
    auto B = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(Rank::dynamic()));
    auto rnn_b = make_shared<op::v0::RNNCell>(X, H_t, W, R, B, hidden_size);
