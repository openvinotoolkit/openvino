...
TEST(type_prop, swish) {
    ...
    EXPECT_EQ(swish_func->get_output_partial_shape(0).to_shape(), data->get_output_partial_shape(0).to_shape());
}
...
TEST(type_prop, swish_2_inputs) {
    ...
    EXPECT_EQ(swish_func->get_element_type(), element::f32);
    ASSERT_TRUE(swish_func->get_output_partial_shape(0).same_scheme(data->get_output_partial_shape(0).to_shape()));
    ASSERT_TRUE(swish_func->get_output_partial_shape(0).rank().is_static());
}
