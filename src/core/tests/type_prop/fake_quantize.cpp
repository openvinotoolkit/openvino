...
TEST(type_prop, fake_quantize_invalid_precisions) {
    const auto data = make_shared<ov::op::v0::Parameter>(element::f32, Shape{1, 2, 3, 4});
    const auto input_low = make_shared<ov::op::v0::Parameter>(element::f16, Shape{});
    const auto input_high = make_shared<ov::op::v0::Parameter>(element::f16, Shape{});
    const auto output_low = make_shared<ov::op::v0::Parameter>(element::f16, Shape{});
    const auto output_high = make_shared<ov::op::v0::Parameter>(element::f16, Shape{});
    const size_t levels = 5;
