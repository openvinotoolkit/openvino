...
TEST_F(TypePropSparseFillEmptyRowsUnpackedStringTest, with_const_inputs) {
    const auto begins =
        std::make_shared<op::v0::Constant>(element::i32, Shape{2, 2}, std::vector<int32_t>{0, 5, 10, 15});
    const auto ends =
        std::make_shared<op::v0::Constant>(element::i32, Shape{2, 2}, std::vector<int32_t>{5, 10, 10, 20});
...
TEST_F(TypePropSparseFillEmptyRowsUnpackedStringTest, symbol_propagation) {
    ...
    const auto& out_symbols_symbols = get_shape_symbols(op->get_output_partial_shape(2));
    const auto& out_empty_row_indicator_symbols = get_shape_symbols(op->get_output_partial_shape(3));

    EXPECT_EQ(out_begins_symbols[0], begins_symbols[0]);
    EXPECT_EQ(out_begins_symbols[1], begins_symbols[1]);
    EXPECT_EQ(out_ends_symbols[0], ends_symbols[0]);
    EXPECT_EQ(out_ends_symbols[1], ends_symbols[1]);
    EXPECT_EQ(out_symbols_symbols[0], symbols_symbols[0]);
    EXPECT_EQ(out_empty_row_indicator_symbols[0], begins_symbols[0]);
}
...
TEST_F(TypePropSparseFillEmptyRowsUnpackedStringTest, empty_row_detection) {
    const auto begins =
        std::make_shared<op::v0::Constant>(element::i32,
                                           Shape{3, 2},
