...
void sparse_fill_empty_rows(const std::string* values,
                            const size_t values_size,
                            const T_IDX* dense_shape,
                            const T_IDX* indices,
    ...
    for (size_t i = 0, out_idx = 0; i < total_rows; i++, out_idx += 2) {
        ...
        if (src_idx < values_size) {
            // Use std::string copy constructor for proper deep copy
            output_values[i] = std::string(values[src_idx]);
        } else {
            // Use std::string copy constructor for default value
            output_values[i] = std::string(default_value);
        }
    }
}
