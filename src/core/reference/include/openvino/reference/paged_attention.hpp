...
void paged_attention(
    ...
    const T* scale,                        // attention scale factor
    const int32_t* sliding_window,         // sliding window parameter (not fully used here)
    const T* alibi_slopes,                 // [num_kv_heads]: per-head bias slopes
    const int32_t* max_context_len,        // max context length (for score output indexing)
