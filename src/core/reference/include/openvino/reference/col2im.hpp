...
void col2im(const T* data,
    ...
    const auto height = output_size[0];
    const auto width = output_size[1];
    ...
    const int64_t output_height = (output_size[0] + pads_begin[0] + pads_end[0] - (dilations[0] * (kernel_size[0] - 1) + 1)) / strides[0] + 1;
    const int64_t output_width = (output_size[1] + pads_begin[1] + pads_end[1] - (dilations[1] * (kernel_size[1] - 1) + 1)) / strides[1] + 1;
    ...
    for (int64_t column = 0; column < channels_per_column; ++column) {
        ...
        // 2
        for (int64_t column_height_idx = 0; column_height_idx < output_height; ++column_height_idx) {
            const int64_t image_height_idx =
                column_height_idx * strides[0] - pads_begin[0] - pads_end[0] + height_offset * dilations[0];

            // 3
            for (int64_t column_width_idx = 0; column_width_idx < output_width; ++column_width_idx) {
                ...
                if (image_height_idx >= 0 && image_height_idx < height && image_width_idx >= 0 &&
                    ...
                    int64_t data_idx = output_width * (column * output_height + column_height_idx) + column_width_idx;
