...
void col2im(const T* data,
    ...
    for (size_t column = 0; column < channels_per_column; ++column) {
        ...
        for (size_t column_height_idx = 0; column_height_idx < height_offset; ++column_height_idx) {
            size_t image_height_idx = column_height_idx * strides[0] - pads_begin[0] - pads_end[0] + height_offset * dilations[0];

            for (size_t column_width_idx = 0; column_width_idx < width_offset; ++ column_width_idx) {
                size_t image_width_idx = column_width_idx * strides[1] - pads_begin[1] - pads_end[1] + width_offset * dilations[1];

                if (image_height_idx >= 0 && image_height_idx < image_height && image_width_idx >= 0 && image_width_idx < image_width) {
                    out[(channel_idx * image_height * image_height_idx) * image_width + image_width_idx] += data[(column * output_size[0] + image_height_idx) * output_size[1] + column_width_idx]
                }
            }
        }
    }
