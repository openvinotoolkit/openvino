...
void dump_blocked_attention_scores_bin(const std::string& filename,
                                       const float* data,
                                       size_t num_heads,
                                       size_t num_q_blocks,
                                       size_t num_k_blocks) {
    size_t total_elems = num_heads * num_q_blocks * num_k_blocks;
    std::ofstream ofs(filename, std::ios::binary);
    if (!ofs) {
        std::cerr << "Failed to open file for writing: " << filename << std::endl;
        return;
    }
    ofs.write(reinterpret_cast<const char*>(data), total_elems * sizeof(float));
    ofs.close();

    std::cout << "✅ Dumped blocked_attention_scores to: " << filename
              << " (" << total_elems << " elements, "
              << sizeof(float) * total_elems / 1024.0 << " KB)\n";
}
...
XAttentionRetainedBlockIndicesForAllHeads get_block_indices_to_keep(
    ...
    size_t total_elems = num_heads * num_q_blocks * num_k_blocks;
    std::vector<float> data_f32(total_elems);
    for (size_t i = 0; i < total_elems; i++)
        data_f32[i] = static_cast<float>(blocked_attention_scores_data[i]);
    dump_blocked_attention_scores_bin("blocked_attention_scores.bin",
                                    data_f32.data(), num_heads, num_q_blocks, num_k_blocks);
    for (size_t head_idx = 0; head_idx < num_heads; head_idx++) {
        size_t head_offset = head_idx * num_q_blocks * num_k_blocks;
        ...
        for (size_t q_block_idx = 0; q_block_idx < num_q_blocks; q_block_idx++) {
            ...
            double total_sum = 0.0;
            for (size_t k_block_idx = 0; k_block_idx < num_k_blocks; k_block_idx++) {
                size_t offset = head_offset + q_block_idx * num_k_blocks + k_block_idx;
                T score = *(blocked_attention_scores_data + offset);
                indices_and_scores.push_back({{q_block_idx, k_block_idx}, score});
                total_sum += score;
            }

            double required_sum = m_threshold * total_sum;

            // === 与 Python 一致：按 score 降序排序 ===
            std::sort(indices_and_scores.begin(), indices_and_scores.end(),
                      [](const IndexAndScore& a, const IndexAndScore& b) {
                          return a.score > b.score;
                      });

            // === 模拟 Python 的 cumulative_sum_without_self ===
            // 即：每个元素的累积和是“之前所有元素的和”，自身不计入。
            std::vector<double> shifted_cumsum(num_k_blocks, 0.0);
            for (size_t i = 1; i < num_k_blocks; i++) {
                shifted_cumsum[i] = shifted_cumsum[i - 1] + indices_and_scores[i - 1].score;
            }

            // === 选择 cumulative_sum_without_self < required_sum 的 block ===
            for (size_t i = 0; i < num_k_blocks; i++) {
                if (shifted_cumsum[i] < required_sum) {
                    retval[head_idx].insert(indices_and_scores[i].idx);
                }
            ...
            // ✅ Python 中通常也会强制保留“自身 block”，即 (q_block_idx, q_block_idx)
