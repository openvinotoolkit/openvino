...
using ProposalBox = sProposalBox<float>;
using ProposalBox_v9 = sProposalBox<float>;
...
void unpack_boxes_v9(const float* p_proposals, float* unpacked_boxes, int64_t* is_dead, int64_t pre_nms_topn) {
    for (int64_t i = 0; i < pre_nms_topn; ++i) {
...
void experimental_detectron_proposals_single_image_v9(
    ...
    std::vector<ProposalBox_v9> proposals(num_proposals);
    std::vector<float> unpacked_boxes(5 * pre_nms_topn);
    std::vector<int64_t> is_dead(pre_nms_topn);
    ...
    unpack_boxes_v9(reinterpret_cast<float*>(proposals.data()), unpacked_boxes.data(), is_dead.data(), pre_nms_topn);
