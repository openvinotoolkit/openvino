...
void ov::reference::gather(const char* const data,
    ...
    int64_t inner_size = shape_size(span(data_shape).subspan(axis + 1)) * element_size;

    int64_t batch_data_mul = shape_size(span(data_shape).subspan(batch_dims)) * element_size;
    int64_t batch_out_mul = shape_size(span(out_shape).subspan(batch_dims)) * element_size;

    int64_t axis_size = data_shape[axis];
    int64_t data_offset, out_offset, idx;
    // for out of bound indices is filled with zeros
    std::fill(out, out + shape_size(out_shape) * element_size, char{0});
