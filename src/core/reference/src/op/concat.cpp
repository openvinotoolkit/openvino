...
void concat(const std::vector<const char*>& args,
    ...
    for (size_t step = 0; step < steps; ++step) {
        for (size_t in_index = 0; in_index < args.size(); ++in_index) {
            ...
            // Sub-byte packed types (e.g. u1/u2/u3/u4/i4/u6) store several elements per byte, so the
            // element count must be converted to the actual occupied byte count before it is used to
            // index into `args`/`out` (elem_size is 1 byte for these types). `bitwidth == 0` is the
            // "unknown/dynamic" element type used by some callers that pre-compute `elem_size`
            // themselves (e.g. reference::concat() called with the default element type argument);
            // leave it untouched.
