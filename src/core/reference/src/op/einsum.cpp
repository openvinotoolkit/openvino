...
void reduce_input(HostTensorVector& inputs,
    ...
    reduce_sum(input_ptr->get_data_ptr<T>(), output_ptr->get_data_ptr<T>(), input_shape, reduced_axes);
...
void extract_diagonal(HostTensorVector& inputs, std::vector<std::string>& input_subscripts, size_t input_ind) {
    ...
    reduce_sum(mul_output->get_data_ptr<T>(), result->get_data_ptr<T>(), mul_output->get_shape(), reduced_axes);
