...
public:
    ...
    bool match_value(ov::pass::pattern::Matcher* matcher,
        ...
        if (pattern_value.get_index() != graph_value.get_index()) {
            ...
            OV_LOG_MATCHING(matcher, matcher->level_str, OV_BLOCK_END, OV_RED, "  OUTPUT INDICES DIDN'T MATCH. EXPECTED: ", pattern_value.get_index(),
        ...
        if (!m_vt.predicate(graph_value)) {
            ...
            OV_LOG_MATCHING(matcher, matcher->level_str, OV_BLOCK_END, OV_RED, "  PREDICATE DIDN'T MATCH.");
            return false;
        }
        ...
        if (!m_attrs.empty()) {
            ...
            if (!visitor.matched()) {
                ...
                OV_LOG_MATCHING(matcher, matcher->level_str, OV_BLOCK_END, OV_RED, "  ATTRIBUTES DIDN'T MATCH.");
                return false;
            }
        }
        ...
        if (matcher_verbose_enabled())
            level.push_back('\t');
        OV_LOG_MATCHING(matcher, matcher->level_str, OV_BLOCK_BODY_RIGHT, " TYPE MATCHED. CHECKING PATTERN ARGUMENTS");
        ...
        OV_LOG_MATCHING(matcher, matcher->level_str, OV_BLOCK_END, (ret ? OV_GREEN : OV_RED), (ret ? "  ALL ARGUMENTS MATCHED" : "  ARGUMENTS DIDN'T MATCH"));
        if (matcher_verbose_enabled()) {
            level.pop_back();
