...
TEST(TransformationTests, ConvBiasFusion) {
    ...
    {
        auto input1 = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 3, 64, 64});
        auto weights = op::v0::Constant::create(element::f32, Shape{3, 3, 1, 1}, {1});
        auto bias = op::v0::Constant::create(element::f32, Shape{3, 1, 1}, {1});
        auto conv = std::make_shared<op::v1::Convolution>(input1,
                                                          weights,
                                                          Strides{1, 1},
                                                          CoordinateDiff{0, 0},
                                                          CoordinateDiff{0, 0},
        ...
        auto add = std::make_shared<op::v1::Add>(conv, bias);
    ...
    for (auto& op : f->get_ops()) {
        if (auto conv = ov::as_type_ptr<op::v1::Convolution>(op)) {
