...
TEST(TransformationTests, HasConstantValueHelper) {
    ...
    ASSERT_TRUE(op_util::has_constant_value<float>(float32_scalar, 1.234f));
    ASSERT_TRUE(op_util::has_constant_value<float>(float32_scalar, 1.23f, 0.005f));
    ASSERT_FALSE(op_util::has_constant_value<float>(float32_scalar, 1.23f, 0.003f));
    ...
    ASSERT_TRUE(op_util::has_constant_value<int64_t>(int64_scalar, 12));

    auto bool_scalar = opset4::Constant::create(element::boolean, Shape{}, {true});
    ASSERT_TRUE(op_util::has_constant_value<bool>(bool_scalar, true));

    ASSERT_FALSE(op_util::has_constant_value<int8_t>(nullptr, 0));

    auto float32_2D = opset4::Constant::create(element::f32, Shape{2}, {1.2f, 3.4f});
    ASSERT_FALSE(op_util::has_constant_value<float>(float32_2D, 1.2f));

    float32_scalar = opset4::Constant::create(element::f32, Shape{}, {1.234f});
    ASSERT_FALSE(op_util::has_constant_value<float>(float32_scalar, 1.235f));

    float32_1D = opset4::Constant::create(element::f32, Shape{1}, {1.234f});
    ASSERT_FALSE(op_util::has_constant_value<float>(float32_scalar, 1.235f));

    int64_scalar = opset4::Constant::create(element::i64, Shape{}, {12});
    ASSERT_FALSE(op_util::has_constant_value<int64_t>(int64_scalar, 13));
}
