...
public:
    ...
    void SetUp() override {
        ...
        {
            auto i_weights =
                std::make_shared<ngraph::opset6::Constant>(ngraph::element::i8, ngraph::Shape{weights.size()}, weights);

            auto f_weights = std::make_shared<ngraph::opset6::Convert>(i_weights, float_element_type);

            auto zp = std::make_shared<ngraph::opset6::Constant>(float_element_type,
                                                                 ngraph::Shape{},
                                                                 std::vector<float>{test_case.zp});
            auto subtract_zp = std::make_shared<ngraph::opset6::Subtract>(f_weights, zp);

            auto scale = std::make_shared<ngraph::opset6::Constant>(float_element_type,
                                                                    ngraph::Shape{},
...

        {
            ...
            auto i_low =
                std::make_shared<ngraph::opset6::Constant>(float_element_type,
                                                           ngraph::Shape{},
            ...
            auto o_low = std::make_shared<ngraph::opset6::Constant>(float_element_type,
                                                                    ngraph::Shape{},
                                                                    std::vector<float>{test_case.o_low});
            auto o_high = std::make_shared<ngraph::opset6::Constant>(float_element_type,
                                                                     ngraph::Shape{},
                                                                     std::vector<float>{test_case.o_high});

            auto fq = std::make_shared<ngraph::opset6::FakeQuantize>(f_weights,
                                                                     i_low,
                                                                     i_high,
                                                                     o_low,
                                                                     o_high,
