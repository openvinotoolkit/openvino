...
TEST(nop_elimination, squeeze_reshape_elimination_check_info) {
    std::shared_ptr<ov::Model> f;
    {
        auto arg = std::make_shared<v0::Parameter>(element::f32, PartialShape{8, 16, 1, 3});

        auto relu = std::make_shared<v0::Relu>(arg);
        relu->set_friendly_name("relu");

        auto squeeze_axes = v0::Constant::create(element::i64, Shape{1}, {2});
        auto squeeze = std::make_shared<v0::Squeeze>(relu, squeeze_axes);
        squeeze->set_friendly_name("squeeze");

        auto reshape_shape = v0::Constant::create(element::i64, Shape{4}, {8, 16, 1, 3});
...
TEST(nop_elimination, squeeze_unsqueeze_elimination) {
    std::shared_ptr<ov::Model> f;
    {
        ...
        auto unsqueeze_axes = v0::Constant::create(element::i64, Shape{1}, {2});
...
TEST(nop_elimination, squeeze_unsqueeze_elimination_dynamic_without_squeeze_axis) {
    std::shared_ptr<ov::Model> f;
    {
        auto arg = std::make_shared<v0::Parameter>(element::f32, PartialShape{-1, 16, 1, 3});

        auto squeeze = std::make_shared<v0::Squeeze>(arg);
        squeeze->set_friendly_name("squeeze");
...
TEST(nop_elimination, reshape_arithmetical_reduce_elimination_keep_dims_false_negative) {
    ...
    auto reduce = std::make_shared<op::v1::ReduceMean>(data, axes, false);
...
TEST(nop_elimination, reshape_elimination_v1_check_consumer_count) {
    std::shared_ptr<ov::Model> f;
    {
        ...
        auto reshape_1_shape = v0::Constant::create(element::i64, Shape{2}, {128, 3});
        auto reshape_1 = std::make_shared<v1::Reshape>(arg, reshape_1_shape, false);
        reshape_1->set_friendly_name("reshape_1");

        auto reshape_2_shape = v0::Constant::create(element::i64, Shape{4}, {8, 16, 1, 3});
        auto reshape_2 = std::make_shared<v1::Reshape>(reshape_1, reshape_2_shape, false);
        reshape_2->set_friendly_name("reshape_2");

        auto relu = std::make_shared<v0::Relu>(reshape_1);
        relu->set_friendly_name("relu");
...
TEST(nop_elimination, squeeze_unsqueeze_elimination_negative) {
    auto check_usecase = [](const Shape& shape, const std::vector<int64_t>& indices_val) {
        ...
        auto squeeze = make_shared<v0::Squeeze>(input, indices);
        auto baseline_f = make_shared<ov::Model>(squeeze, ParameterVector{input});
        auto optimized_f = baseline_f->clone();
        pass::Manager manager;
        manager.register_pass<ov::pass::NopElimination>();
        manager.run_passes(optimized_f);
        ASSERT_EQ(count_ops_of_type<v0::Squeeze>(baseline_f), 1);
        ASSERT_EQ(count_ops_of_type<v0::Squeeze>(optimized_f), 1);
    };
...
TEST_P(EliminateEltwiseTests, eliminate_eltwise) {
    ...
    if (type_params.op_type == OpType::SUBTRACT_WITH_CONVERT) {
        constant = std::make_shared<v0::Convert>(constant, type);
    }
    ...
    case OpType::ADD:
        node = make_shared<v1::Add>(A, B);
        break;
    case OpType::SUBTRACT:
    case OpType::SUBTRACT_WITH_CONVERT:
        node = make_shared<v1::Subtract>(A, B);
        break;
    case OpType::MULTIPLY:
        node = make_shared<v1::Multiply>(A, B);
        break;
    case OpType::DIVIDE:
        node = make_shared<v1::Divide>(A, B);
        break;
    default:
        ASSERT_FALSE(true) << "Invalid OpType";
    }
    auto abs = make_shared<v0::Abs>(node);
    model = make_shared<ov::Model>(abs, ParameterVector{parameter});

    manager.register_pass<ov::pass::NopElimination>();

    if (can_fuse) {
        auto abs = make_shared<v0::Abs>(parameter);
...
TEST_F(TransformationTestsF, eliminate_eltwise_dequantization_subgraph) {
    {
        auto constant = v0::Constant::create(element::i8, Shape{}, {2});
        auto convert = make_shared<v0::Convert>(constant, element::f32);
        auto sub = make_shared<v1::Subtract>(convert, v0::Constant::create(element::f32, Shape{}, {0}));
        auto mul = make_shared<v1::Multiply>(sub, v0::Constant::create(element::f32, Shape{}, {1}));
...
TEST_P(SplitConcatElimination, eliminate_split_concat_subgraph) {
    ...
    auto param = make_shared<v0::Parameter>(element::f32, Shape{batch, seq_len, input_size});

    shared_ptr<Node> data = param;
    shared_ptr<Node> sequence;
    auto gate = static_cast<size_t>(p.rnn_type);
    auto axis_const = make_shared<v0::Constant>(element::i64, Shape{}, p.split_axis);
    auto H = make_shared<v0::Parameter>(element::f32, Shape{batch, num_dir, hidden_size});
    auto C = make_shared<v0::Parameter>(element::f32, Shape{batch, num_dir, hidden_size});
    auto seq_lengths = make_shared<v0::Parameter>(element::i64, Shape{batch});
    auto W = make_shared<v0::Parameter>(element::f32, Shape{num_dir, gate * hidden_size, input_size});
    auto R = make_shared<v0::Parameter>(element::f32, Shape{num_dir, gate * hidden_size, hidden_size});
    auto B = make_shared<v0::Parameter>(element::f32, Shape{num_dir, gate * hidden_size});
    auto direction = op::RecurrentSequenceDirection::FORWARD;
    if (p.rnn_type == RNNType::RNN) {
        sequence = make_shared<v5::RNNSequence>(data, H, seq_lengths, W, R, B, hidden_size, direction);
        data = make_shared<v0::Squeeze>(sequence->output(0), axis_const);
        params = {H, seq_lengths, W, R, B};
    } else if (p.rnn_type == RNNType::GRU) {
        sequence = make_shared<v5::GRUSequence>(data, H, seq_lengths, W, R, B, hidden_size, direction);
        data = make_shared<v0::Squeeze>(sequence->output(0), axis_const);
        params = {H, seq_lengths, W, R, B};
    } else if (p.rnn_type == RNNType::LSTM) {
        sequence = make_shared<v5::LSTMSequence>(data, H, C, seq_lengths, W, R, B, hidden_size, direction);
        data = make_shared<v0::Squeeze>(sequence->output(0), axis_const);
    ...
    if (p.split_type == SplitType::Split) {
        split = make_shared<v1::Split>(data->output(0), axis_const, p.seq_len / p.split_len);
    } else if (p.split_type == SplitType::VariadicSplit) {
        auto split_lengths = make_shared<v0::Constant>(element::i64,
                                                       Shape{seq_len / p.split_len},
                                                       std::vector<size_t>(seq_len / p.split_len, p.split_len));
        split = make_shared<v1::VariadicSplit>(data->output(0), axis_const, split_lengths);
    }
    ...
    auto concat = make_shared<v0::Concat>(outputs_to_concat, p.concat_axis);
    auto sigmoid = make_shared<v0::Sigmoid>(concat);
    auto res = make_shared<v0::Result>(sigmoid);
    ...
    EXPECT_EQ(count_ops_of_type<v0::Concat>(model), expect_concat)
        << "SplitConcatElimination transformation has failed. "
           "The number of Concat ops is not " +
               to_string(expect_concat);
    EXPECT_EQ(count_ops_of_type<v1::Split>(model) + count_ops_of_type<v1::VariadicSplit>(model), expect_split)
        << "SplitConcatElimination transformation has failed. "
...
TEST(SplitConcatElimination, split_inputs_not_in_order) {
    ...
    auto param = make_shared<v0::Parameter>(element::f32, Shape{2, 10});
    auto split = make_shared<v1::Split>(param->output(0), axis_const, 10);
    ...
    auto concat = make_shared<v0::Concat>(outputs_to_concat, axis);
    auto sigmoid = make_shared<v0::Sigmoid>(concat);
    auto res = make_shared<v0::Result>(sigmoid);
    ...
    EXPECT_EQ(count_ops_of_type<v0::Concat>(model), 1) << "SplitConcatElimination transformation has failed. "
                                                          "The number of Concat ops is not 1";
    EXPECT_EQ(count_ops_of_type<v1::Split>(model), 1) << "SplitConcatElimination transformation has failed. "
...
TEST(SplitConcatElimination, no_sequence_found) {
    ...
    auto param = make_shared<v0::Parameter>(element::f32, Shape{2, 10});
    auto param_2 = make_shared<v0::Parameter>(element::f32, Shape{2, 1});
    auto split = make_shared<v1::Split>(param->output(0), axis_const, 10);
    OutputVector outputs_to_concat = split->outputs();
...
TEST_F(TransformationTestsF, NopTile) {
    {
        auto data = std::make_shared<v0::Parameter>(element::f32, PartialShape{-1, -1, -1, -1});
        auto repeats = v0::Constant::create(element::i32, Shape{4}, {1, 1, 1, 1});
...
TEST_F(TransformationTestsF, EliminateSliceBeforeGatherElements) {
    {
        ...
        auto stop = v0::Constant::create(element::i32, Shape{1}, {2});
        auto step = v0::Constant::create(element::i32, Shape{1}, {1});
        auto axis = v0::Constant::create(element::i32, Shape{1}, {-1});
        auto slice = std::make_shared<op::v8::Slice>(data, start, stop, step, axis);

        auto indices = std::make_shared<v0::Parameter>(element::i64, PartialShape{-1, -1, -1, -1});
...
TEST_F(TransformationTestsF, EliminateConcatStridedSliceTopKConcat) {
    {
        int64_t axis = 2;
        auto param1 = make_shared<v0::Parameter>(element::f32, Shape{1, 10, 3});
        auto param2 = make_shared<v0::Parameter>(element::f32, Shape{1, 10, 4});
        auto concat = make_shared<v0::Concat>(ov::as_output_vector({param1, param2}), axis);
        ...
        auto strided_slice1 = std::make_shared<v1::StridedSlice>(concat,
                                                                 ...
                                                                 std::vector<int64_t>{1, 1, 0},
                                                                 std::vector<int64_t>{1, 1, 0});
        auto topk = std::make_shared<v1::TopK>(strided_slice1,
        ...
        auto begin_const2 = v0::Constant::create(ov::element::i64, ov::Shape{3}, {0, 0, 3});
        auto end_const2 = v0::Constant::create(ov::element::i64, ov::Shape{3}, {0, 0, 7});
        auto strided_slice2 = std::make_shared<v1::StridedSlice>(concat,
                                                                 begin_const2,
                                                                 end_const2,
                                                                 std::vector<int64_t>{1, 1, 0},
                                                                 std::vector<int64_t>{1, 1, 0});
        auto topk_values = std::make_shared<v0::Result>(topk->output(0));
        auto concat1 = make_shared<v0::Concat>(ov::as_output_vector({topk_values, strided_slice2}), axis);
    ...
    }
    {
        ...
        auto param1 = make_shared<v0::Parameter>(element::f32, Shape{1, 10, 3});
        auto param2 = make_shared<v0::Parameter>(element::f32, Shape{1, 10, 4});
        auto axis_const = std::make_shared<v0::Constant>(ov::element::i64, ov::Shape{}, axis);
        auto topk = std::make_shared<v1::TopK>(param1,
                                               v0::Constant::create(ov::element::i64, ov::Shape{}, {1}),
                                               axis,
                                               op::v1::TopK::Mode::MAX,
                                               op::v1::TopK::SortType::NONE);
        auto topk_values = std::make_shared<v0::Result>(topk->output(0));
        auto concat = make_shared<v0::Concat>(ov::as_output_vector({topk_values, param2}), axis);
        auto result = std::make_shared<op::v0::Result>(concat);
...
TEST_F(TransformationTestsF, EliminateStridedSlice) {
    {
        ...
        auto strided_slice = std::make_shared<v1::StridedSlice>(relu,
                                                                begin_const,
                                                                end_const,
