...
TEST_F(TransformationTestsF, NearestNeighborUpsamplingFusionSpatial2D1) {
    ...
    std::vector<float> mul_const_value{1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f};
    {
        ...
        for (size_t i = 1; i < 2 + 2 * (input_rank - 2); ++i) {
            const auto unsqueezed_const =
                ngraph::opset8::Constant::create(ngraph::element::i64,
                                                 {},
                                                 std::vector<int64_t>{constants_for_concat_1[i]});
            const auto unsqueeze_axis =
                ngraph::opset8::Constant::create(ngraph::element::i64, {}, std::vector<int64_t>{0});
            const auto current_unsqueeze =
                std::make_shared<ngraph::opset8::Unsqueeze>(unsqueezed_const, unsqueeze_axis);
        ...
        for (size_t i = 1; i < input_rank; ++i) {
            const auto unsqueezed_const =
                ngraph::opset8::Constant::create(ngraph::element::i64,
                                                 {},
                                                 std::vector<int64_t>{constants_for_concat_2[i]});
            const auto unsqueeze_axis =
                ngraph::opset8::Constant::create(ngraph::element::i64, {}, std::vector<int64_t>{0});
            const auto current_unsqueeze =
                std::make_shared<ngraph::opset8::Unsqueeze>(unsqueezed_const, unsqueeze_axis);
    ...
    }
    {
        ...
        auto input = std::make_shared<ngraph::opset8::Parameter>(ngraph::element::f32, input_shape);
        auto sizes_node =
            ngraph::opset8::Constant::create(ngraph::element::i64, {new_spatial_shape.size()}, new_spatial_shape);
        auto scales_node =
            ngraph::opset8::Constant::create(ngraph::element::f32, {scales_as_floats.size()}, scales_as_floats);
        auto axes_node = ngraph::opset8::Constant::create(ngraph::element::i64, {2}, std::vector<int64_t>{1, 2});
        auto interpolate =
            std::make_shared<ngraph::opset8::Interpolate>(input, sizes_node, scales_node, axes_node, attrs);
        function_ref =
