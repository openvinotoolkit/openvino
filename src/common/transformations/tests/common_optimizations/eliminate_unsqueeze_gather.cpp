...
public:
    void SetUp() override {
        ...
        ngraph::helpers::CompareFunctions(*transform(inShape, inType, axis), *reference(inShape, inType, axis));
    }

protected:
    std::shared_ptr<const ngraph::Function> transform(const TensorShape& inShape,
        ...
        const auto function = std::make_shared<ngraph::Function>(ngraph::NodeVector{gather},
    ...
    std::shared_ptr<const ngraph::Function> reference(const TensorShape& inShape,
                                                      const TensorType& inType,
                                                      size_t axis) {
        const auto parameter = std::make_shared<ngraph::opset6::Parameter>(inType, inShape);

        return std::make_shared<ngraph::Function>(ngraph::NodeVector{parameter},
                                                  ngraph::ParameterVector{parameter},
                                                  "Reference");
    }
};
...
INSTANTIATE_TEST_SUITE_P(smoke_NGraph,
                         EliminateUnsqueezeGatherTest,
                         testing::Combine(testing::Values(ngraph::element::f16,
                                                          ngraph::element::f32,
                                                          ngraph::element::i32,
                                                          ngraph::element::i64,
                                                          ngraph::element::u8),
