...
TEST_F(TransformationTestsF, TransposeReshapeEliminationForMatMul) {
    ...
    Shape data_shape_2{10, 2, 25};
    {
        ...
        auto const_transpose_before = v0::Constant::create(element::i32, Shape{3}, {1, 2, 0});
        auto transpose_before = std::make_shared<v1::Transpose>(data_2, const_transpose_before);
        auto const_reshape_before = v0::Constant::create(element::i32, Shape{2}, {2, 250});
        auto reshape_before = std::make_shared<v1::Reshape>(transpose_before, const_reshape_before, false);
        auto matmul = std::make_shared<v0::MatMul>(data_1, reshape_before);
        auto const_reshape_after = v0::Constant::create(element::i32, Shape{3}, {10, 10, 25});
        auto reshape_after = std::make_shared<v1::Reshape>(matmul, const_reshape_after, false);
        auto const_tranpose_after = v0::Constant::create(element::i32, Shape{3}, {2, 0, 1});
        auto tranpose_after = std::make_shared<v1::Transpose>(reshape_after, const_tranpose_after);
    ...
    }
    {
        auto data_1 = std::make_shared<v0::Parameter>(element::f32, data_shape_1);
        auto data_2 = std::make_shared<v0::Parameter>(element::f32, data_shape_2);
        auto matmul = std::make_shared<v0::MatMul>(data_1, data_2);
...
TEST_F(TransformationTestsF, TransposeReshapeEliminationForMatMul_TransposedA) {
    ...
    Shape data_shape_2{10, 2, 25};
    {
        ...
        auto const_reshape_before = v0::Constant::create(element::i32, Shape{2}, {2, 250});
        auto reshape_before = std::make_shared<v1::Reshape>(transpose_before, const_reshape_before, false);
        auto matmul = std::make_shared<v0::MatMul>(data_1, reshape_before, true, false);
        auto const_reshape_after = v0::Constant::create(element::i32, Shape{3}, {10, 10, 25});
        auto reshape_after = std::make_shared<v1::Reshape>(matmul, const_reshape_after, false);
        auto const_tranpose_after = v0::Constant::create(element::i32, Shape{3}, {2, 0, 1});
        auto tranpose_after = std::make_shared<v1::Transpose>(reshape_after, const_tranpose_after);
    ...
    }
    {
        auto data_1 = std::make_shared<v0::Parameter>(element::f32, data_shape_1);
        auto data_2 = std::make_shared<v0::Parameter>(element::f32, data_shape_2);
        auto matmul = std::make_shared<v0::MatMul>(data_1, data_2, true, false);
        model_ref = std::make_shared<Model>(OutputVector{matmul}, ParameterVector{data_1, data_2});
    }
}
...
TEST_F(TransformationTestsF, TransposeReshapeEliminationForMatMul_TransposedB) {
    ...
    Shape data_shape_2{10, 2, 25};
    {
        auto data_1 = std::make_shared<v0::Parameter>(element::f32, data_shape_1);
        auto data_2 = std::make_shared<v0::Parameter>(element::f32, data_shape_2);
        auto const_transpose_before = v0::Constant::create(element::i32, Shape{3}, {0, 2, 1});
        auto transpose_before = std::make_shared<v1::Transpose>(data_2, const_transpose_before);
        auto const_reshape_before = v0::Constant::create(element::i32, Shape{2}, {250, 2});
        auto reshape_before = std::make_shared<v1::Reshape>(transpose_before, const_reshape_before, false);
        auto matmul = std::make_shared<v0::MatMul>(data_1, reshape_before, false, true);
        auto const_reshape_after = v0::Constant::create(element::i32, Shape{3}, {10, 10, 25});
        auto reshape_after = std::make_shared<v1::Reshape>(matmul, const_reshape_after, false);
        auto const_tranpose_after = v0::Constant::create(element::i32, Shape{3}, {1, 0, 2});
        auto tranpose_after = std::make_shared<v1::Transpose>(reshape_after, const_tranpose_after);
        model = std::make_shared<Model>(OutputVector{tranpose_after}, ParameterVector{data_1, data_2});
        manager.register_pass<ov::pass::TransposeReshapeEliminationForMatmul>();
    }
...
TEST_F(TransformationTestsF, TransposeReshapeEliminationForMatMul_TransposedAB) {
    ...
    Shape data_shape_2{10, 2, 25};
    {
        ...
        auto const_reshape_before = v0::Constant::create(element::i32, Shape{2}, {250, 2});
        auto reshape_before = std::make_shared<v1::Reshape>(transpose_before, const_reshape_before, false);
        auto matmul = std::make_shared<v0::MatMul>(data_1, reshape_before, true, true);
        auto const_reshape_after = v0::Constant::create(element::i32, Shape{3}, {10, 10, 25});
        auto reshape_after = std::make_shared<v1::Reshape>(matmul, const_reshape_after, false);
        auto const_tranpose_after = v0::Constant::create(element::i32, Shape{3}, {1, 0, 2});
        auto tranpose_after = std::make_shared<v1::Transpose>(reshape_after, const_tranpose_after);
