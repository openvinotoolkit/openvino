...
public:
    void SetUp() override {
        ...
        if (values.fuse_happened == FuseHappened::YES) {
            ...
            auto shuffle_channels =
                std::make_shared<ngraph::opset6::ShuffleChannels>(input0, 1, values.reshape_before_val[1]);
            f_ref = std::make_shared<ngraph::Function>(ngraph::NodeVector{shuffle_channels},
