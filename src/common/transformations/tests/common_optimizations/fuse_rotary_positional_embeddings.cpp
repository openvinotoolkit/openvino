...
TEST_F(TransformationTestsF, ConvertToROPE_chatGML_2d_rope) {
    ...
    const int max_pos_length = 2048;
    {
        ...
        auto ListUnpack_321 = makeOP<ov::opset1::VariadicSplit>({input, -1, {4096, 256, 256}});
        auto view_Reshape = makeOP<ov::opset1::Reshape>({ListUnpack_321->output(0), {0, 0, num_heads, ndims}}, {{"special_zero", true}});
        ...
        auto aten_view_Reshape_1 = makeOP<ov::opset1::Reshape>({ListUnpack_321->output(1), {0, 0, 2, ndims}}, {{"special_zero", true}});
        ...
        auto slice_Slice_369 =
            makeOP<ov::opset1::StridedSlice>({__module_transformer_index_67_Gather, {0, 0}, scatter_update_63441, {1, 1}},
                                        {{"begin_mask", {1, 0}},
                                        {"end_mask", {1, 0}},
                                        {"new_axis_mask", {}},
                                        {"shrink_axis_mask", {}},
                                        {"ellipsis_mask", {}}});
        auto list_construct_concat_1 = makeOP<ov::opset1::Concat>({{-1}, {1}, gather_105252, {{rotary_ndims / 2}}, {2}}, {{"axis", 0}});

        auto reshape_Reshape_373 =
            makeOP<ov::opset1::Reshape>({slice_Slice_357, {0, 32, 0, 32, 2}}, {{"special_zero", true}});
        auto select_Gather_384 = makeOP<ov::opset8::Gather>({reshape_Reshape_373, 0, -1}, {{"batch_dims", 0}});//x_even
        auto select_Gather_381 = makeOP<ov::opset8::Gather>({reshape_Reshape_373, 1, -1}, {{"batch_dims", 0}});//x_odd
        ...
        auto mul_Multiply_386 =
            makeOP<ov::opset1::Multiply>({select_Gather_381, select_Gather_382}, {{"auto_broadcast", "numpy"}});//x_odd_sin
        auto mul_Multiply_383 =
            makeOP<ov::opset1::Multiply>({select_Gather_384, select_Gather_385}, {{"auto_broadcast", "numpy"}});//x_even_cos
        auto Multiply_101315 =
        ...
        auto mul_Multiply_391 =
            makeOP<ov::opset1::Multiply>({select_Gather_381, select_Gather_385}, {{"auto_broadcast", "numpy"}});//x_odd_cos
        auto mul_Multiply_393 =
            makeOP<ov::opset1::Multiply>({select_Gather_384, select_Gather_382}, {{"auto_broadcast", "numpy"}});//x_even_sin
        ...
        auto flatten_Reshape_421 = makeOP<ov::opset1::Reshape>({stack_401, {0, num_heads, 0, rotary_ndims}}, {{"special_zero", true}});
    ...
    manager.register_pass<ov::pass::RoPEFusion>(true);
    {
        ...
        auto cos_sin_cache =
            std::make_shared<ov::opset1::Parameter>(ov::element::f32,
                                                    ov::Shape{max_pos_length, (rotary_ndims / 2), 2});
        auto position_ids = std::make_shared<ov::opset1::Parameter>(ov::element::i32, ov::PartialShape{batch, seq_len});
        auto gather_cos_sin =
            makeOP<ov::opset8::Gather>({cos_sin_cache, position_ids, 0}, {{"batch_dims", 0}});
