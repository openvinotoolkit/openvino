...
TEST_F(TransformationTestsF, ConvertToROPE_chatGLM4_PagedAttention) {
    {
        disable_rt_info_check();
        auto input =
            std::make_shared<ov::opset1::Parameter>(ov::element::f32, ov::PartialShape{DYN, 1, 4608});
        auto input1 =
            std::make_shared<ov::opset1::Parameter>(ov::element::f32, ov::PartialShape{DYN, 1, 32, 2});

        auto aten_slice_Slice_2 = makeOP<ov::opset8::Slice>({input1, {0}, {1}, {1}, {1}});
        auto aten_view_Reshape = makeOP<ov::opset1::Reshape>({aten_slice_Slice_2, {-1, 1, 1, 32, 2}},
                                                             {{"special_zero", false}});
        ...
        auto module_transformer_encoder_layers_0_self_attention_aten_transpose_Transpose =
            makeOP<ov::opset1::Reshape>({module_transformer_encoder_layers_0_self_attention_prim_ListUnpack->output(0), {-1, 32, 1, 128}},
                                        {{"special_zero", false}});
        auto VariadicSplit_41226 =
            makeOP<ov::opset1::VariadicSplit>({module_transformer_encoder_layers_0_self_attention_aten_transpose_Transpose, 3, {64, 64}});
        auto aten_reshape_Reshape =
            makeOP<ov::opset1::Reshape>({VariadicSplit_41226->output(0), {0, 32, 0, 32, 2}},
                                        {{"special_zero", true}});
        ...
        auto aten_select_Gather_2 = makeOP<ov::opset8::Gather>({aten_reshape_Reshape, 1, -1}, {{"batch_dims", 0}});//v
        ...
        auto Unsqueeze_81695 = makeOP<ov::opset1::Reshape>({aten_sub_Subtract, {-1, 32, 1, 32, 1}},
                                                           {{"special_zero", false}});
        ...
        auto Unsqueeze_81696 = makeOP<ov::opset1::Reshape>({aten_add_Add, {-1, 32, 1, 32, 1}},
                                                           {{"special_zero", false}});
        auto aten_stack = makeOP<ov::opset1::Concat>({Unsqueeze_81695, Unsqueeze_81696}, {{"axis", -1}});
        auto aten_flatten_Reshape =
            makeOP<ov::opset1::Reshape>({aten_stack, {0, 32, 0, 64}}, {{"special_zero", true}});
        auto aten_cat_Concat =
            makeOP<ov::opset1::Concat>({aten_flatten_Reshape, VariadicSplit_41226->output(1)}, {{"axis", -1}});

        model =
            std::make_shared<ov::Model>(ov::NodeVector{aten_cat_Concat}, ov::ParameterVector{input, input1});
    }
    manager.register_pass<ov::pass::RoPEFusion>(true);
    {
        auto input =
        ...
        auto rope = makeOP<ov::op::internal::RoPE>(
            {input, input1, input1},
            {{"config.slice_start", 0},
             {"config.slice_stop", 4096},
             {"config.input_trans0213", false},
