...
//     auto param = std::make_shared<ov::op::v0::Parameter>(ov::element::i32, ov::Shape{2, 2});
//     auto constant = ov::op::v0::Constant::create(ov::element::i32, ov::Shape{2}, {1, 4});
//     auto reshape = std::make_shared<ov::op::v1::Reshape>(param, constant, false);
//     std::vector<ov::Output<ov::Node>> concat_input = {};
//     std::cout << "Static: " << (reshape->get_default_output().get_partial_shape().is_static()) << std::endl;
...
//     }
//     auto concat = std::make_shared<ov::op::v0::Concat>(concat_input, 1);
...
//         // return std::dynamic_pointer_cast<ov::op::v0::Tile>(node) != nullptr;
...
// TEST(TransformationTests, TestTransfromationaaa) {
