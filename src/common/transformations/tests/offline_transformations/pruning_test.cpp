...
TEST(TransformationTests, TestInitMasks) {
    ...
    auto conv = std::make_shared<opset5::Convolution>(input,
                                                      weights,
                                                      Strides(2, 1),
                                                      CoordinateDiff(2, 0),
                                                      CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, PropagateMasksBasic) {
    ...
    auto conv2 = std::make_shared<opset5::Convolution>(mul,
                                                       weights2,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
...

    {
        ...
        auto weights =
            opset5::Constant::create(element::f32,
...
TEST(TransformationTests, PropagateMasksDynamicReshape) {
    ...
    auto reshape = std::make_shared<opset5::Reshape>(relu,
                                                     opset5::Constant::create(element::i64, Shape{4}, {-1, 6, 64, 64}),
                                                     true);

    auto weights2 = opset5::Constant::create(element::f32, weights_shape2, {0});
    auto conv2 = std::make_shared<opset5::Convolution>(reshape,
                                                       weights2,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
...
TEST(TransformationTests, PropagateMasksDynamicGroupConvolution) {
    ...
    auto input = std::make_shared<opset5::Parameter>(element::f32, input_shape);
    auto weights = opset5::Constant::create(element::f32, weights_shape, {0});
    auto conv = std::make_shared<opset5::GroupConvolution>(input,
                                                           weights,
                                                           Strides(2, 1),
                                                           CoordinateDiff(2, 0),
                                                           CoordinateDiff(2, 0),
    ...
    auto weights2 = opset5::Constant::create(element::f32, weights_shape2, {0});
    auto conv2 = std::make_shared<opset5::GroupConvolution>(mul,
                                                            weights2,
                                                            Strides(2, 1),
                                                            CoordinateDiff(2, 0),
                                                            CoordinateDiff(2, 0),
...
TEST(TransformationTests, PropagateMasksEmpty) {
    ...
    auto weights2 = opset5::Constant::create(element::f32, weights_shape2, {0});
    auto conv2 = std::make_shared<opset5::Convolution>(add,
                                                       weights2,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, PropagateMaskPassThrough) {
    ...
    auto conv_1 = std::make_shared<opset5::Convolution>(input,
                                                        weights_const_1,
                                                        Strides(2, 1),
                                                        CoordinateDiff(2, 0),
                                                        CoordinateDiff(2, 0),
    ...
    auto max_pool = std::make_shared<opset5::MaxPool>(pad, Strides{1, 1}, Shape{0, 0}, Shape{1, 1}, Shape{4, 4});
    max_pool->set_friendly_name("max_pool");

    auto weights2 = opset5::Constant::create(element::f32, weight_shape2, {0});
    auto conv2 = std::make_shared<opset5::Convolution>(max_pool,
                                                       weights2,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, PropagateMasksHardDependencies) {
    ...
    auto conv1 = std::make_shared<opset5::Convolution>(input1,
                                                       weights1,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
    ...
    auto conv2 = std::make_shared<opset5::Convolution>(input2,
                                                       weights2,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
    ...
    auto reshape =
        std::make_shared<opset5::Reshape>(add1, opset5::Constant::create(element::i64, Shape{2}, {1, 6}), true);
    ...
    auto conv3 = std::make_shared<opset5::Convolution>(add2,
                                                       weights3,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
    ...
    function = std::make_shared<Function>(NodeVector{matmul, conv3}, ParameterVector{input1, input2});
    {
        ...
        auto reshape =
            std::make_shared<opset5::Reshape>(add1, opset5::Constant::create(element::i64, Shape{2}, {1, 5}), true);
        reshape->set_friendly_name("reshape");

        auto matmul =
            std::make_shared<opset5::MatMul>(reshape, opset5::Constant::create(element::f32, Shape{5, 100}, {1.}));
        matmul->set_friendly_name("matmul");
...
TEST_F(TransformationTestsF, PropagateMasksQuantizedGroupConvolution) {
    ...
    auto conv1 = std::make_shared<opset5::Convolution>(input,
                                                       weights1,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
    ...
    auto reshape = std::make_shared<opset5::Reshape>(mul,
                                                     opset5::Constant::create(element::i64, Shape{5}, {8, 1, 1, 3, 3}),
                                                     false);

    auto conv_group = std::make_shared<opset5::GroupConvolution>(conv1,
                                                                 reshape,
                                                                 Strides(2, 1),
                                                                 CoordinateDiff(2, 0),
                                                                 CoordinateDiff(2, 0),
    ...
    function = std::make_shared<Function>(NodeVector{conv2}, ParameterVector{input});
    {
        ...
        auto conv1 = std::make_shared<opset5::Convolution>(input,
                                                           ...
                                                           CoordinateDiff(2, 0),
                                                           Strides(2, 1));
        auto weights_group = opset5::Constant::create(
            element::i8,
        ...
        const auto dims_to_keep =
            opset6::Constant::create(reshape_const->get_element_type(), {dims_to_keep_vec.size()}, dims_to_keep_vec);
        ...
        const auto reshape_concat = std::make_shared<opset6::Concat>(
            NodeVector{opset6::Constant::create(reshape_const->get_element_type(), {2}, {-1, 1}), reshape_gather},
            0);
...
TEST_F(TransformationTestsF, PropagateMasksQuantizedGroupConvolutionWithShapeOf) {
    ...
    function = std::make_shared<Function>(NodeVector{conv2}, ParameterVector{input});
    {
        ...
        auto dims_to_keep_vec = std::vector<size_t>{2, 3, 4};
        const auto dims_to_keep =
            opset6::Constant::create(reshape_concat->get_element_type(), {dims_to_keep_vec.size()}, dims_to_keep_vec);
        ...
        const auto new_reshape_concat = std::make_shared<opset6::Concat>(
            NodeVector{opset6::Constant::create(reshape_concat->get_element_type(), {2}, {-1, 1}), new_reshape_gather},
            0);
...
TEST_F(TransformationTestsF, PropagateMasksFakeQuantizePerTensor) {
    ...
    auto weights_2 = opset5::Constant::create(element::f32, weight_shape2, {0});
    auto conv2 = std::make_shared<opset5::Convolution>(fq,
                                                       weights_2,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, PropagateMasksFakeQuantizePerChannel) {
    ...
    function = std::make_shared<Function>(NodeVector{conv2}, ParameterVector{input});
    {
        auto input = std::make_shared<opset5::Parameter>(element::f32, input_shape);
        auto weights_1 =
            opset5::Constant::create(element::i8,
...
TEST_F(TransformationTestsF, TestConcatMaskPropagation) {
    ...
    auto conv2 = std::make_shared<opset5::Convolution>(input,
                                                       weights_2,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
    ...
    auto conv3 = std::make_shared<opset5::Convolution>(input,
                                                       weights_3,
                                                       Strides(2, 1),
                                                       CoordinateDiff(2, 0),
                                                       CoordinateDiff(2, 0),
    ...
    auto concat =
        std::make_shared<opset5::Concat>(OutputVector{conv1->output(0), conv2->output(0), conv3->output(0)}, 1);

    auto weights_out_conv = create_constant_with_zeros(weight_shape_out_conv, {{}, {}, {}, {}});
    auto conv_out = std::make_shared<opset5::Convolution>(concat,
                                                          weights_out_conv,
                                                          Strides(2, 1),
                                                          CoordinateDiff(2, 0),
                                                          CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, TestConcatMaskPropagationUp) {
    ...
    auto weights_out_conv = create_constant_with_zeros(weight_shape_out_conv, {{}, {}, {}, {}});
    auto conv_out = std::make_shared<opset5::Convolution>(add,
                                                          weights_out_conv,
                                                          Strides(2, 1),
                                                          CoordinateDiff(2, 0),
                                                          CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, PruneConvIsClosingAndInGroup) {
    ...
    auto conv_1 = std::make_shared<opset5::Convolution>(add,
                                                        conv_1_weights,
                                                        Strides(2, 1),
                                                        CoordinateDiff(2, 0),
                                                        CoordinateDiff(2, 0),
    ...
    auto end_conv = std::make_shared<opset5::Convolution>(add_1,
                                                          weights_end_conv,
                                                          Strides(2, 1),
                                                          CoordinateDiff(2, 0),
                                                          CoordinateDiff(2, 0),
...
TEST(TransformationTests, PruneBranchingStopOp) {
    ...
    auto group_conv = std::make_shared<opset5::GroupConvolution>(conv,
                                                                 group_conv_weights,
                                                                 Strides(2, 1),
                                                                 CoordinateDiff(2, 0),
                                                                 CoordinateDiff(2, 0),
    ...
    auto conv_1_weights = create_constant_with_zeros(conv_1_shape, {{1, 2, 3}, {}, {}, {}});
    auto conv_1 = std::make_shared<opset5::Convolution>(group_conv,
                                                        conv_1_weights,
                                                        Strides(2, 1),
                                                        CoordinateDiff(2, 0),
                                                        CoordinateDiff(2, 0),
    ...
    auto end_conv_shape = Shape{weightsShape[1], weightsShape[0], 1, 1};
    auto weights_end_conv = create_constant_with_zeros(end_conv_shape, {{1, 2, 3}, {}, {}, {}});
    auto end_conv = std::make_shared<opset5::Convolution>(mul,
                                                          weights_end_conv,
                                                          Strides(2, 1),
                                                          CoordinateDiff(2, 0),
                                                          CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, PruneReducelayerUp) {
    ...
    auto conv_1_weights = create_constant_with_zeros(conv_1_shape, {{1, 2, 3}, {}, {}, {}});
    auto conv_1 = std::make_shared<opset5::Convolution>(reduce_mean,
                                                        conv_1_weights,
                                                        Strides(2, 1),
                                                        CoordinateDiff(2, 0),
                                                        CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, MaskPropagationReshapeUp) {
    ...
    auto conv_1_weights = create_constant_with_zeros(conv_1_shape, {{1, 2, 3}, {}, {}, {}});
    auto conv_1 = std::make_shared<opset5::Convolution>(reshape,
                                                        conv_1_weights,
                                                        Strides(2, 1),
                                                        CoordinateDiff(2, 0),
                                                        CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, MaskPropagationReshapeUpShapeSubGraph) {
    ...
    const auto concat =
        std::make_shared<opset6::Concat>(NodeVector{gather_1, opset6::Constant::create(element::i64, {1}, {6}), gather},
                                         0);
    ...
    function = std::make_shared<ngraph::Function>(OutputVector{conv_1}, ParameterVector{input});
    {
        ...
        const auto dims_to_keep_1 =
            opset6::Constant::create(element::i64, {dims_to_keep_vec_1.size()}, dims_to_keep_vec_1);
        const auto gather_1 = std::make_shared<opset6::Gather>(shape_of_conv, dims_to_keep_1, axis);
        const auto concat = std::make_shared<opset6::Concat>(
            NodeVector{gather_1, opset6::Constant::create(element::i64, {1}, {6}), gather},
            0);
...
TEST_F(TransformationTestsF, MaskPropagationReshapeExtend) {
    ...
    auto add = std::make_shared<opset5::Add>(reshape_from, add_const);
...
TEST_F(DISABLED_TransformationTestsF, MaskPropagationReshapeDownMul) {
    ...
    auto first_conv = std::make_shared<opset5::Convolution>(input,
                                                            weights,
                                                            Strides(2, 1),
                                                            CoordinateDiff(2, 0),
                                                            CoordinateDiff(2, 0),
    ...
    auto reshape_conv = std::make_shared<opset5::Convolution>(reshape,
    ...
    auto last_conv = std::make_shared<opset5::Convolution>(mul,
                                                           last_conv_weights,
                                                           Strides(2, 1),
                                                           CoordinateDiff(2, 0),
                                                           CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, MaskPropagationReshapeDownAdd) {
    ...
    auto last_conv_weights = create_constant_with_zeros({8, 8, 8, 8}, {{1, 2, 3}, {}, {}, {}});
    auto last_conv = std::make_shared<opset5::Convolution>(add,
                                                           last_conv_weights,
                                                           Strides(2, 1),
                                                           CoordinateDiff(2, 0),
                                                           CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, MaskPropagationReshapeUnsqueezeUp) {
    ...
    auto mul_left_up = std::make_shared<opset5::MatMul>(input, left_up_weights);

    auto mul_left = std::make_shared<opset5::MatMul>(mul_left_up, reshape);
...
TEST(TransformationTests, MaskPropagationWrongDimsElementwise) {
    ...
    auto branch_conv = std::make_shared<opset5::Convolution>(first_conv,
                                                             branch_conv_weights,
                                                             Strides(2, 2),
                                                             CoordinateDiff(2, 0),
                                                             CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, PruneSEBlock) {
    ...
    auto se_conv_0 = std::make_shared<opset5::Convolution>(reshape,
                                                           se_conv_0_weights,
                                                           Strides(2, 1),
                                                           CoordinateDiff(2, 0),
                                                           CoordinateDiff(2, 0),
    ...
    auto se_conv_1 = std::make_shared<opset5::Convolution>(se_conv_0,
                                                           se_conv_1_weights,
                                                           Strides(2, 1),
                                                           CoordinateDiff(2, 0),
                                                           CoordinateDiff(2, 0),
...
TEST(TransformationTests, MaskPropagationMatMulStopEmptyABranch) {
    ...
    auto mul_left = std::make_shared<opset5::MatMul>(left_weights, reshape);
...
TEST(TransformationTests, PruneConvUpShort) {
    ...
    auto conv_1 = std::make_shared<opset5::Convolution>(conv,
                                                        conv_1_const,
                                                        Strides(2, 1),
                                                        CoordinateDiff(2, 0),
                                                        CoordinateDiff(2, 0),
    ...
    auto last_conv = std::make_shared<opset5::Convolution>(bad_add,
                                                           weights_end_conv,
                                                           Strides(2, 1),
                                                           CoordinateDiff(2, 0),
                                                           CoordinateDiff(2, 0),
...
TEST_F(TransformationTestsF, PropagateMasksTransposeComplex) {
    ...
    for (auto& transpose_const_vec : transpose_consts) {
        auto transpose_const =
            opset5::Constant::create(element::i64, {transpose_const_vec.size()}, transpose_const_vec);
...
TEST_F(TransformationTestsF, PropagateMasksBroadcastedEltwise) {
    ...
    auto broadcasted_eltwise_short2 =
        std::make_shared<opset5::Add>(broadcasted_eltwise_weights_short, broadcasted_eltwise_short);
    ...
    auto broadcasted_eltwise_without_weights_mask2 =
        std::make_shared<opset5::Add>(dummy_eltwise, broadcasted_eltwise_without_weights_mask);
...
TEST_P(TransformationTestsBoolParamF, MaskPropagationReshapedPassThroughP) {
    ...
    auto mul_left = std::make_shared<opset5::MatMul>(input, left_weights);

    auto eltwise_mul_const = create_constant_with_zeros(EltwiseShape, {{}, {1}, {}});
    auto eltwise_mul = std::make_shared<opset5::Multiply>(mul_left, eltwise_mul_const);
    ...
    if (add_shape_of)
        reshape_recover_shape = std::make_shared<opset5::ShapeOf>(transpose);
    else
        reshape_recover_shape = opset5::Constant::create(element::i64, {4}, {1, 6, 2, 6});

    auto reshape_recover = std::make_shared<opset5::Reshape>(unary_op, reshape_recover_shape, true);

    auto right_weights = create_constant_with_zeros(weightsShape, {{}, {1, 2, 3, 4, 5}});
    auto mul_right = std::make_shared<opset5::MatMul>(input, right_weights);

    auto eltwise_mul_right_const = create_constant_with_zeros(EltwiseShape, {{}, {1}, {}});
    auto eltwise_mul_right = std::make_shared<opset5::Multiply>(mul_right, eltwise_mul_right_const);

    auto reshape_const_right = opset5::Constant::create(element::i64, {4}, {0, 0, 6, 2});
    auto reshape_right = std::make_shared<opset5::Reshape>(eltwise_mul_right, reshape_const_right, true);

    auto transpose_const_right = opset5::Constant::create(element::i64, {4}, {0, 2, 1, 3});
    auto transpose_right = std::make_shared<opset5::Transpose>(reshape_right, transpose_const_right);

    auto mul_v = std::make_shared<opset5::MatMul>(reshape_recover, transpose_right);

    auto flatten_const = opset5::Constant::create(element::i64, {2}, {1, 24});
    auto flatten = std::make_shared<opset5::Reshape>(mul_v, flatten_const, true);
    ...
    function = std::make_shared<ngraph::Function>(OutputVector{last_mul}, ParameterVector{input});
    {
        ...
        auto rev_flat_const = opset5::Constant::create(element::i64, {2}, {8, 6});
        auto rev_flat = std::make_shared<opset5::Reshape>(transpose, rev_flat_const, true);
        ...
        if (add_shape_of)
            ...
            reshape_recover_shape = opset5::Constant::create(element::i64, {4}, {1, 4, 2, 6});
        ...
        auto reshape_const_right = opset5::Constant::create(element::i64, {4}, {0, 0, 4, 2});
        auto reshape_right = std::make_shared<opset5::Reshape>(eltwise_mul_right, reshape_const_right, true);
        ...
        auto flatten_const = opset5::Constant::create(element::i64, {2}, {1, 16});
        auto flatten = std::make_shared<opset5::Reshape>(mul_v, flatten_const, true);
