    ...
    if (auto convert = ov::as_type_ptr<ov::opset1::Convert>(node)) {
        ...
        if (convert->input(0).get_element_type().is_real() &&
            ...
            if (item != precisions.end()) {
                ...
                CREATE_OP(pre_convert, ov::opset1::Convert, data, item->second);
                data = pre_convert;
            }

            CREATE_OP(abs, ov::opset1::Abs, data);
            CREATE_OP(to_max_value, ov::opset1::Constant, ov::util::make_tensor_of_max_value(to));
            CREATE_OP(to_max_convert, ov::opset1::Convert, to_max_value, abs->get_output_element_type(0));
            CREATE_OP(min, ov::opset1::Minimum, abs, to_max_convert);
            CREATE_OP(ceil, ov::opset1::Ceiling, min);
            CREATE_OP(new_convert, ov::opset1::Convert, ceil, to);
