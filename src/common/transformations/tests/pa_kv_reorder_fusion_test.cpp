...
TEST(PaKVReorderFusionTest, PassInstantiation) {
    auto pass = std::make_shared<pass::PaKVReorderFusion>();
...
TEST(PaKVReorderFusionTest, EmptyModel) {
    ...
    manager.register_pass<pass::PaKVReorderFusion>();
...
TEST(PaKVReorderFusionTest, FusionPattern) {
    ...
    for (const auto& op : model->get_ops()) {
        if (std::dynamic_pointer_cast<op::internal::PaKVReorder>(op)) {
...
TEST_F(TransformationTestsF, PaKVReorderFusion_basic) {
    ...
    {
        auto key_cache = std::make_shared<op::v0::Parameter>(element::f16, PartialShape::dynamic(4));
        key_cache->set_friendly_name("key_cache.0_clone_for_k_update");
        auto value_cache = std::make_shared<op::v0::Parameter>(element::f16, PartialShape::dynamic(4));
        value_cache->set_friendly_name("value_cache.0_clone_for_v_update");

        auto block_indices = std::make_shared<op::v0::Parameter>(element::i32, PartialShape{2});
        block_indices->set_friendly_name("block_indices");
        auto block_indices_begins = std::make_shared<op::v0::Parameter>(element::i32, PartialShape{2});
        block_indices_begins->set_friendly_name("block_indices_begins");

        auto block_update_indices = std::make_shared<op::v0::Parameter>(element::i32, PartialShape{2});
        block_update_indices->set_friendly_name("block_update_indices");
        auto block_update_indices_begins = std::make_shared<op::v0::Parameter>(element::i32, PartialShape{2});
        block_update_indices_begins->set_friendly_name("block_update_indices_begins");

        auto gather_axis = op::v0::Constant::create(element::i64, Shape{}, {0});
        auto scatter_axis = op::v0::Constant::create(element::i64, Shape{}, {0});
...
TEST_F(TransformationTestsF, PaKVReorderFusion_skip_on_mismatched_block_indices) {
    ...
    auto block_indices_k = std::make_shared<op::v0::Parameter>(element::i32, PartialShape{2});
    block_indices_k->set_friendly_name("block_indices_k");
    auto block_indices_v = std::make_shared<op::v0::Parameter>(element::i32, PartialShape{2});
