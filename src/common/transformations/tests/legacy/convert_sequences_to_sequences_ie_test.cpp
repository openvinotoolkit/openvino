...
TEST_F(TransformationTestsF, GRUSequenceConversionTest) {
    ...
    {
        const auto X =
            std::make_shared<ngraph::opset5::Parameter>(ngraph::element::f32, ngraph::Shape{batch_size, 1, input_size});
        const auto W = std::make_shared<ngraph::opset5::Constant>(
            ngraph::element::f32,
            ngraph::Shape{num_directions, gates_count * hidden_size, input_size});
        const auto R = std::make_shared<ngraph::opset5::Constant>(
            ngraph::element::f32,
            ngraph::Shape{num_directions, gates_count * hidden_size, hidden_size});
        const auto H_t =
            std::make_shared<ngraph::opset5::Parameter>(ngraph::element::f32,
        ...
        const auto seq_len =
            std::make_shared<ngraph::opset5::Constant>(ngraph::element::i32, ngraph::Shape{batch_size});
        sequence = std::make_shared<ngraph::opset5::GRUSequence>(X,
                                                                 H_t,
                                                                 seq_len,
                                                                 W,
                                                                 R,
...

    {
        ...
        const auto seq_len =
            std::make_shared<ngraph::opset5::Constant>(ngraph::element::i32, ngraph::Shape{batch_size}, 1);
        ...
        function_ref =
            std::make_shared<ngraph::Function>(ngraph::NodeVector{unsqueeze_1}, ngraph::ParameterVector{X, H_t});
    }
}
...
TEST_F(TransformationTestsF, RNNSequenceConversionTest) {
...

    {
        ...
        auto H = std::make_shared<ngraph::opset5::Parameter>(ngraph::element::f32,
                                                             ngraph::Shape{batch_size, num_directions, 3});
        ...
        sequence = std::make_shared<ngraph::opset5::RNNSequence>(X,
                                                                 H,
                                                                 seq_len,
                                                                 W,
                                                                 R,
...

    {
        ...
        auto unsqueeze_2 = std::make_shared<ngraph::opset5::Unsqueeze>(sequence_ie->output(1), unsqueeze_axis);
        function_ref =
            std::make_shared<ngraph::Function>(ngraph::NodeVector{unsqueeze_1}, ngraph::ParameterVector{X, H});
    }
}
...
TEST_F(TransformationTestsF, LSTMSequenceConversionTest) {
    ...
    std::shared_ptr<ngraph::opset5::LSTMSequence> sequence;
    {
        ...
        const auto seq_len =
            std::make_shared<ngraph::opset4::Constant>(ngraph::element::f32, ngraph::Shape{batch_size});
        ...
        function = std::make_shared<ngraph::Function>(ngraph::OutputVector{sequence->output(0)},
...

    {
        ...
        const auto C_t =
            std::make_shared<ngraph::opset5::Parameter>(ngraph::element::f32,
                                                        ngraph::Shape{batch_size, num_directions, hidden_size});
        const auto seq_lengths =
            std::make_shared<ngraph::opset5::Constant>(ngraph::element::f32, ngraph::Shape{batch_size});
        const auto B =
            std::make_shared<ngraph::opset5::Constant>(ngraph::element::f32,
                                                       ngraph::Shape{num_directions, gates_count * hidden_size});
        ...
        function_ref =
            std::make_shared<ngraph::Function>(ngraph::NodeVector{unsqueeze_1}, ngraph::ParameterVector{X, H_t, C_t});
    }
}
