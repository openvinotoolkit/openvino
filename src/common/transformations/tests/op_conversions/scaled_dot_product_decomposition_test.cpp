...
const std::shared_ptr<ov::Node> scaled_dot_product_attention_decomposition(std::shared_ptr<ov::Node> query,
    ...
    if (sinks) {
        ...
        auto past_seq_len = std::make_shared<ov::op::v8::Gather>(k_shape, minus_two, zero_i);
        scaled_atten = std::make_shared<ov::op::v8::Slice>(scaled_atten, zero_i, past_seq_len, one_i, minus_one);
    } else {
