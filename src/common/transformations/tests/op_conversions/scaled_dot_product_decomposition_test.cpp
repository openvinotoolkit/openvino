...
TEST_F(TransformationTestsF, ScaledDotProductAttentionDecompositionStaticBasic) {
    ...
    const auto scale = std::make_shared<v0::Parameter>(element::f32, scale_shape);
    const auto casual = false;
    const auto gqa_mode = false;
    {
        const auto scaled_dot_product_attention =
            std::make_shared<v13::ScaledDotProductAttention>(query, key, value, attention_mask, scale, gqa_mode, casual);
...
TEST_F(TransformationTestsF, ScaledDotProductAttentionCasualPartiallyDynamic) {
    ...
    {
        const auto scaled_dot_product_attention =
            std::make_shared<v13::ScaledDotProductAttention>(query, key, value, attention_mask, gqa_mode, casual);
...
TEST_F(TransformationTestsF, ScaledDotProductAttentionDecomposition_ScalarScale_MultiplyAfterMatMul) {
    ...
    auto query = std::make_shared<v0::Parameter>(element::f32, query_shape);
    auto key = std::make_shared<v0::Parameter>(element::f32, key_shape);
    auto value = std::make_shared<v0::Parameter>(element::f32, value_shape);
    auto attention_mask = std::make_shared<v0::Parameter>(element::f32, attention_mask_shape);

    const bool casual = false;
    const bool gqa_mode = false;

    {
        auto sdp = std::make_shared<v13::ScaledDotProductAttention>(query, key, value, attention_mask, scale, gqa_mode, casual);
...
TEST_F(TransformationTestsF, ScaledDotProductAttentionDecomposition_DynamicScale) {
    ...
    auto scale = std::make_shared<v0::Parameter>(element::f32, scale_shape);
    auto query = std::make_shared<v0::Parameter>(element::f32, query_shape);
    auto key = std::make_shared<v0::Parameter>(element::f32, key_shape);
    auto value = std::make_shared<v0::Parameter>(element::f32, value_shape);
    auto attention_mask = std::make_shared<v0::Parameter>(element::f32, attention_mask_shape);
...
const std::shared_ptr<ov::Node> scaled_dot_product_attention_decomposition(std::shared_ptr<ov::Node> query,
    ...
    const auto minus_one = v0::Constant::create(element::i32, Shape{}, {-1});
    const auto minus_two = v0::Constant::create(element::i32, Shape{}, {-2});
    const auto zero_i = v0::Constant::create(element::i32, Shape{}, {0});
    const auto one_i = v0::Constant::create(element::i32, Shape{}, {1});
    const auto one_f = std::make_shared<v1::ConvertLike>(one_i, query);
    const auto zero_f = std::make_shared<v1::ConvertLike>(zero_i, query);

    auto extract_dim = [&zero_i](const std::shared_ptr<v3::ShapeOf>& shape_of,
                                 const int64_t idx) -> std::shared_ptr<ov::Node> {
        const auto& shape = shape_of->get_input_partial_shape(0);
        const auto& dim = shape[idx];
        if (dim.is_static()) {
            return v0::Constant::create(element::i32, Shape{}, {dim.get_length()});
        }
        const auto dim_to_extract_const = v0::Constant::create(element::i32, Shape{}, {idx});
        return std::make_shared<v8::Gather>(shape_of, dim_to_extract_const, zero_i);
    };
    ...
    Output<Node> scaled_atten = std::make_shared<v1::Multiply>(atten, scale);
    ...
    if (!casual) {
        ...
        const auto ssl = std::make_shared<v0::Unsqueeze>(source_s_len, zero_i);
    ...
    if (sinks) {
        ...
        auto scaled_attn_sink = std::make_shared<v0::Concat>(OutputVector{scaled_atten, sinks_broadcast}, -1);
        scaled_atten = std::make_shared<v8::Softmax>(scaled_attn_sink, -1);

        auto prev_seq_len = std::make_shared<v8::Gather>(k_shape, minus_two, zero_i);
...
TEST_F(TransformationTestsF, ScaledDotProductAttentionDecomposition_PreScaledQuery) {
    ...
    const PartialShape raw_query_shape{1, 6, 101, 64};
    const PartialShape key_shape{1, 6, 101, 64};
    const PartialShape value_shape{1, 6, 101, 64};
    const PartialShape attention_mask_shape{1, 6, 101, 101};

    auto raw_query = std::make_shared<v0::Parameter>(element::f32, raw_query_shape);
    auto key = std::make_shared<v0::Parameter>(element::f32, key_shape);
    auto value = std::make_shared<v0::Parameter>(element::f32, value_shape);
    auto attention_mask = std::make_shared<v0::Parameter>(element::f32, attention_mask_shape);

    // Q is pre-scaled by a scalar constant (as SDPAFusion leaves it)
    auto q_scale_const = v0::Constant::create(element::f32, Shape{}, {0.353553f});
    auto query_prescaled = std::make_shared<v1::Multiply>(raw_query, q_scale_const);

    // SDPA scale = the K-side scale that was absorbed by SDPAFusion
    auto sdpa_scale = v0::Constant::create(element::f32, Shape{}, {0.353553f});
    ...
    {
        // Expected: scale applied after MatMul(Q, K^T) per SDPA spec
        auto ref =
...
TEST_F(TransformationTestsF, ScaledDotProductAttentionDecomposition_Sinks) {
    ...
    auto scale = std::make_shared<v0::Parameter>(element::f16, scale_shape);
    auto query = std::make_shared<v0::Parameter>(element::f16, query_shape);
    auto key = std::make_shared<v0::Parameter>(element::f16, key_shape);
    auto value = std::make_shared<v0::Parameter>(element::f16, value_shape);
    auto attention_mask = std::make_shared<v0::Parameter>(element::f16, attention_mask_shape);
    auto sinks = std::make_shared<v0::Parameter>(element::f16, sinks_shape);
    ...
    {
        auto sdpa =
            std::make_shared<v13::ScaledDotProductAttention>(query, key, value, attention_mask, scale, sinks, gqa_mode, casual);
