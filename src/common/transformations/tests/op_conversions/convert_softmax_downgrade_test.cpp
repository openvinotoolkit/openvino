...
TEST_F(TransformationTestsF, ConvertSoftMax8ToSoftMax1) {
    {
        ...
        int64_t axis = 1;
        auto softmax_8 = std::make_shared<op::v8::Softmax>(data, axis);
...

    {
        auto data = std::make_shared<op::v0::Parameter>(element::f32, Shape{2, 3});
        size_t axis = 1;
        auto softmax_1 = std::make_shared<op::v1::Softmax>(data, axis);
...
TEST_F(TransformationTestsF, ConvertSoftMax8ToSoftMax1_input_rank_5) {
    {
        auto data = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 3, 5, 5, 5});
        int64_t axis = -2;
        auto softmax_8 = std::make_shared<op::v8::Softmax>(data, axis);
