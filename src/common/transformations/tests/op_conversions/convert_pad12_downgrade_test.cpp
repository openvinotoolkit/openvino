...
std::shared_ptr<ov::Model> create_v12_model(const ov::op::PadMode pad_mode, const int16_t pad_v = -1) {
    const auto input = std::make_shared<ov::op::v0::Parameter>(ov::element::i16, ov::Shape{1, 3, 100, 100});
    ...
    if (pad_v != -1) {
        ...
        pad = std::make_shared<ov::op::v12::Pad>(input, pads_begin, pads_end, pad_value, pad_mode);
    } else {
        pad = std::make_shared<ov::op::v12::Pad>(input, pads_begin, pads_end, pad_mode);
    }
...
std::shared_ptr<ov::Model> create_v1_model(const ov::op::PadMode pad_mode, const int16_t pad_v) {
    ...
    const auto pad = std::make_shared<ov::op::v1::Pad>(input, pads_begin, pads_end, pad_value, pad_mode);
