...
TEST_F(TransformationTestsF, SimplifyCTCGreedyDecoderSeqLenTest) {
    {
        ...
        function = std::make_shared<ngraph::Function>(ngraph::NodeVector{res_1, res_2},
                                                      ngraph::ParameterVector{data, seq_len});
...

    {
        ...
        auto upper_bounds = std::make_shared<ngraph::opset6::Broadcast>(seq_len1, mask_shape->output(0));
        ...
        auto transpose_seq_mask = std::make_shared<ngraph::opset6::Transpose>(
            seq_mask->output(0),
            ngraph::opset6::Constant::create(seq_len_type, Shape({2}), {1, 0}));
        auto transpose_seq_mask_f32 =
            std::make_shared<ngraph::opset6::Convert>(transpose_seq_mask->output(0), data_type);
        auto simplified_decoder =
            std::make_shared<ngraph::opset6::CTCGreedyDecoder>(transpose, transpose_seq_mask_f32->output(0), true);

        auto squeeze2_axis = ngraph::opset6::Constant::create(seq_len_type, Shape({1}), {3});
        auto squeeze2_output_f =
            std::make_shared<ngraph::opset6::Squeeze>(simplified_decoder->output(0), squeeze2_axis);
...
TEST_F(TransformationTestsF, SimplifyCTCGreedyDecoderSeqLenDynamicInputShapeTest) {
    {
        ...
        auto decoder_v6 =
            std::make_shared<ngraph::op::v6::CTCGreedyDecoderSeqLen>(data, seq_len, true, element::i64, element::i32);
        auto res_1 = std::make_shared<opset6::Result>(decoder_v6->output(0));
        auto res_2 = std::make_shared<opset6::Result>(decoder_v6->output(1));
...

    {
        ...
        auto simplified_decoder =
            std::make_shared<ngraph::opset6::CTCGreedyDecoder>(transpose, transpose_seq_mask_f->output(0), true);
...
TEST_F(TransformationTestsF, SimplifyCTCGreedyDecoderSeqLenDynamicBatchTest) {
    {
        auto data = std::make_shared<ngraph::opset6::Parameter>(ngraph::element::f32,
                                                                ngraph::PartialShape{Dimension::dynamic(), 3, 7});
        auto seq_len = std::make_shared<ngraph::opset6::Parameter>(ngraph::element::i32,
                                                                   ngraph::PartialShape{Dimension::dynamic()});

        auto decoder_v6 =
            std::make_shared<ngraph::op::v6::CTCGreedyDecoderSeqLen>(data, seq_len, true, element::i32, element::i64);
        auto res_1 = std::make_shared<opset6::Result>(decoder_v6->output(0));
        auto res_2 = std::make_shared<opset6::Result>(decoder_v6->output(1));
    ...
    {
        auto data1 = std::make_shared<ngraph::opset6::Parameter>(ngraph::element::f32,
                                                                 ngraph::PartialShape{Dimension::dynamic(), 3, 7});
        auto seq_len1 = std::make_shared<ngraph::opset6::Parameter>(ngraph::element::i32,
                                                                    ngraph::PartialShape{Dimension::dynamic()});
...
TEST_F(TransformationTestsF, SimplifyCTCGreedyDecoderSeqLenDynamicSeqLenTest) {
    {
        auto data = std::make_shared<ngraph::opset6::Parameter>(ngraph::element::f32,
                                                                ngraph::PartialShape{2, Dimension::dynamic(), 7});
        auto seq_len = std::make_shared<ngraph::opset6::Parameter>(ngraph::element::i32, ngraph::PartialShape{2});

        auto decoder_v6 = std::make_shared<ngraph::op::v6::CTCGreedyDecoderSeqLen>(data,
                                                                                   seq_len,
                                                                                   true,
                                                                                   ngraph::element::i64,
                                                                                   ngraph::element::i64);
...
TEST_F(TransformationTestsF, SimplifyCTCGreedyDecoderSeqLenWrongBlankIndexTest) {
...

    {
        ...
        auto decoder_v6 = std::make_shared<ngraph::op::v6::CTCGreedyDecoderSeqLen>(data1,
                                                                                   seq_len1,
                                                                                   blank_index1,
                                                                                   true,
                                                                                   ngraph::element::i64,
        ...
        function_ref = std::make_shared<ngraph::Function>(ngraph::NodeVector{res_1, res_2},
                                                          ngraph::ParameterVector{data1, seq_len1});
    }
}
...
TEST_F(TransformationTestsF, SimplifyCTCGreedyDecoderSeqLenDynamicSeqLenParamWithBlankIndexTest) {
    {
        ...
        function = std::make_shared<ngraph::Function>(ngraph::NodeVector{res_1, res_2},
                                                      ngraph::ParameterVector{data, seq_len, blank_index});
...

    {
        ...
        function_ref = std::make_shared<ngraph::Function>(ngraph::NodeVector{res_1, res_2},
                                                          ngraph::ParameterVector{data1, seq_len1, blank_index1});
    }
}
