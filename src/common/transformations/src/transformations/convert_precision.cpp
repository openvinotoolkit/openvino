...
bool fuse_type_to_parameter(const std::shared_ptr<ov::Node>& node,
    ...
    auto it = precisions.find(output_type);
    if (it == precisions.end())
        return false;
    bool changed = false;
    const auto& to = it->second;
    if (auto param = ov::as_type_ptr<ov::op::v0::Parameter>(node)) {
        if (convert_input_precision) {
            ...
            ov::Output<ov::Node> new_out = param;
            if (output_type.is_integral() && to.is_integral() && output_type.size() > to.size()) {
                auto [lbound, ubound] = [](const ov::element::Type& prec) {
                    switch (prec) {
                ...
                new_out = std::make_shared<ov::op::v0::Clamp>(param, lbound, ubound);
            }
            auto convert = std::make_shared<ov::op::v0::Convert>(new_out, to);
            for (auto& input : param_consumers) {
                ...
                if (ov::is_type<ov::op::v0::Result>(consumer) || ov::is_type<ov::op::v0::Convert>(consumer) ||
                    // Param -> Clamp -> Convert pattern is added to limit the input range before Convert
                    // So we should not replace the Convert input to avoid extra Convert operations
                    (ov::is_type<ov::op::v0::Clamp>(consumer) && consumer->output(0).get_target_inputs().size() == 1 &&
                     ov::is_type<ov::op::v0::Convert>(consumer->output(0).get_target_inputs().begin()->get_node())) ||
