...
bool fuse_type_to_nms5(const std::shared_ptr<ov::Node>& node, const precisions_map& precisions) {
    ...
    auto relaxed_op =
        std::make_shared<ov::op::TypeRelaxed<ov::op::v5::NonMaxSuppression>>(*nms, ov::element::TypeVector{}, output_types);
    replace_node(node, relaxed_op);
    return true;
}
...
bool fuse_type_to_nms9(const std::shared_ptr<ov::Node>& node, const precisions_map& precisions) {
    ...
    auto relaxed_op =
        std::make_shared<ov::op::TypeRelaxed<ov::op::v9::NonMaxSuppression>>(*nms, ov::element::TypeVector{}, output_types);
    replace_node(node, relaxed_op);
    return true;
}
