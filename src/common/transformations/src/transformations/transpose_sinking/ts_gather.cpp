...
TSGatherBackward::TSGatherBackward() {
    ...
    ov::matcher_pass_callback matcher_pass_callback = [=](pattern::Matcher& m) {
        ...
        auto out_pshape = transpose->get_output_partial_shape(0);
        ...
        const auto& indices_rank_val = static_cast<size_t>(indices_rank.get_length());
        std::vector<size_t> new_transpose_order(order_val.size() - indices_rank_val + 1);
        ...
        const auto& transpose_const = Constant::create(transpose_order->get_element_type(),
                                                       {new_transpose_order.size()}, new_transpose_order);
        ...
        if (optimization) {
            ...
            bool success = !(new_out_pshape.is_dynamic() || shape == new_shape);
            if (success) {
