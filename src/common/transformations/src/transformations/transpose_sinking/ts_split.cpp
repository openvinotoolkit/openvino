...
OutputTranspose GetOutputTransposes(const NodePtr& node) {
    for (size_t output_idx = 0; output_idx < node->get_output_size(); ++output_idx) {
        for (auto& input : node->get_output_target_inputs(output_idx)) {
            auto transpose_node = ov::as_type<v1::Transpose>(input.get_node());
            if (!transpose_node)
                continue;
            auto constant_node = ov::as_type<v0::Constant>(transpose_node->input_value(1).get_node());
            if (!constant_node)
                continue;
            {
...
bool HasInputSplitAndTransposeSiblings(const Output<Node>& output) {
    NodePtr main_node = FindInputNode<v1::Split>(output.get_node());
    if (!main_node) {
        return false;
    }
...
bool GetSplitAxis(const std::shared_ptr<v0::Constant>& split_axis, const ov::Rank& rank, int64_t& axis) {
...
TSSplitForward::TSSplitForward() {
    ...
    create_pattern<v1::Split, v1::VariadicSplit>({0});

    auto sinking_transformation = [OV_CAPTURE_CPY_AND_THIS](const std::shared_ptr<Node>& main_node,
                                                            const TransposeInputsInfo& transpose_info) -> bool {
        auto split_axis_constant = as_type_ptr<v0::Constant>(main_node->input_value(1).get_node_shared_ptr());
...
TSSplitBackward::TSSplitBackward() {
    ...
    matcher_pass_callback matcher_pass_callback = [OV_CAPTURE_CPY_AND_THIS](pattern::Matcher& m) {
        ...
        NodePtr split = FindInputNode<v1::Split>(transpose_label_node);
        if (!split) {
            split = FindInputNode<v1::VariadicSplit>(transpose_label_node);
        }

        if (!split || transformation_callback(split)) {
            return false;
        }
        auto split_axis_constant = as_type_ptr<v0::Constant>(split->input_value(1).get_node_shared_ptr());
        if (!split_axis_constant) {
            return false;
        }
