...
TSInterpolateForward::TSInterpolateForward() {
    ...
    create_pattern<v4::Interpolate>({0});
    ...
    auto sinking_transformation = [OV_CAPTURE_CPY_AND_THIS](const std::shared_ptr<Node>& main_node,
        ...
        const auto& interpolate = ov::as_type_ptr<v4::Interpolate>(main_node);
        if (interpolate) {
...
TSInterpolateBackward::TSInterpolateBackward() {
    ...
    auto main_node_label = pattern::wrap_type<v4::Interpolate>([](const Output<Node>& output) -> bool {
