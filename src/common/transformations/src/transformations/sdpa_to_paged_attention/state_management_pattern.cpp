    ...
    ov::matcher_pass_callback callback = [=,
        ...
        auto extract_num_kv_heads = [=, &pattern_map](std::shared_ptr<Node> unsqueeze,
            ...
            if (pattern_map.find(unsqueeze) != pattern_map.end()) {
                ...
                unsqueeze = std::move(pattern_map.at(unsqueeze)).get_node_shared_ptr();
        ...
        if (pattern_map.count(qkv_current_split_node)) {
            ...
            auto qkv_split = std::move(pattern_map.at(qkv_current_split_node)).get_node_shared_ptr();
            ...
            auto real_kv_concat_split = std::move(pattern_map.at(kv_concat_split)).get_node_shared_ptr();
        ...
        if (pattern_map.count(scale_input)) {
            auto scale_input_out = pattern_map.at(scale_input);
            scale = scale_input_out.get_node_shared_ptr();
            if (scale_input_out.get_partial_shape().rank() != 0) {
        ...
        if (pattern_map.find(general_alibi) != pattern_map.end()) {
            alibi_slopes = handle_general_alibi(std::move(pattern_map.at(general_alibi)).get_node_shared_ptr());
        ...
        if (pattern_map.count(phi3_offset)) {
            auto offset = std::move(pattern_map.at(phi3_offset)).get_node_shared_ptr();
            ...
            auto gpt_oss_offset_out = pattern_map.at(gpt_oss_offset);
            auto offset = gpt_oss_offset_out.get_node_shared_ptr();
            if (gpt_oss_offset_out.get_partial_shape().rank() != 0) {
        ...
        if (pattern_map.find(v_past_par) != pattern_map.end()) {
            auto param = ov::as_type_ptr<v0::Parameter>(std::move(pattern_map.at(v_past_par)).get_node_shared_ptr());
            if (param) {
                return false;
            }
        ...
        if (pattern_map.find(k_past_par) != pattern_map.end()) {
            auto param = ov::as_type_ptr<v0::Parameter>(std::move(pattern_map.at(k_past_par)).get_node_shared_ptr());
            if (param) {
                return false;
            }
            parameters_to_remove.push_back(param);
