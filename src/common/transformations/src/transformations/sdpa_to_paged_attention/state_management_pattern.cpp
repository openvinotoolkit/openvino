...
ov::pass::StateManagementPattern::StateManagementPattern(ParameterVector& kv_parameters,
    ...
    auto transpose = std::make_shared<v1::Transpose>(pattern::any_input(), pattern::any_input());

    auto sdpa_with_4_inputs =
        pattern::wrap_type<v13::ScaledDotProductAttention>({pattern::any_input(), pattern::any_input(), transpose, pattern::any_input()});
        // pattern::wrap_type<v13::ScaledDotProductAttention>({pattern::any_input(), pattern::any_input(), pattern::any_input(), pattern::any_input()});
    auto sdpa_with_5_inputs =
        pattern::wrap_type<v13::ScaledDotProductAttention>({pattern::any_input(), pattern::any_input(), pattern::any_input(), pattern::any_input(), pattern::any_input()});

    auto sdpa_variants = std::make_shared<pattern::op::Or>(OutputVector{sdpa_with_4_inputs, sdpa_with_5_inputs});

    // transpose->output(0).set_tensor_ptr(std::make_shared<ov::descriptor::Tensor>(element::f16, PartialShape{1, 3}));
