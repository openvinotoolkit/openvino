...
namespace {
bool convertTensorIteratorToSequence(const std::shared_ptr<v0::TensorIterator>& ti,
                                     const std::shared_ptr<op_util::RNNCellBase>& found_cell,
    ...
    std::vector<std::shared_ptr<v0::TensorIterator::InputDescription>> ordered_in_descs(3);
    ...
    for (const auto& input_desc : ti->get_input_descriptions()) {
        ...
        if (param == data.get_node_shared_ptr()) {
            auto slice_input = ov::as_type_ptr<v0::TensorIterator::SliceInputDescription>(input_desc);
            if (!slice_input)
                return false;
    ...
    std::vector<std::shared_ptr<v0::TensorIterator::OutputDescription>> ordered_out_descs(3);

    // Remember the order of cell outputs in the TensorIterator
    for (const auto& output_desc : ti->get_output_descriptions()) {
        std::shared_ptr<v0::Result> res = results[output_desc->m_body_value_index];
        if (res->input_value(0) == unsqueeze_after_cell) {
            auto concat_output = ov::as_type_ptr<v0::TensorIterator::ConcatOutputDescription>(output_desc);
            if (!concat_output)
                return false;
    ...
    if (slice_axis == 0) {
        auto order = v0::Constant::create(ov::element::i64, ov::Shape{3}, {1, 0, 2});
        X = std::make_shared<v1::Transpose>(ti_inputs[ordered_in_descs[0]->m_input_index], order);
    }
    ...
    auto initial_hidden_state = std::make_shared<v0::Unsqueeze>(ti_inputs[ordered_in_descs[1]->m_input_index], axis_1);

    // LSTM case
    std::shared_ptr<ov::Node> initial_cell_state =
        c_pattern.get_node_shared_ptr() == nullptr
            ? nullptr
            : std::make_shared<v0::Unsqueeze>(ti_inputs[ordered_in_descs[2]->m_input_index], axis_1);

    auto shape_of = std::make_shared<v3::ShapeOf>(X);
    auto batch_dimension = std::make_shared<v1::Gather>(shape_of,
                                                        v0::Constant::create(ov::element::i64, {1}, {0}),
                                                        v0::Constant::create(ov::element::i64, {}, {0}));
    auto seq_len_dim = std::make_shared<v1::Gather>(shape_of,
                                                    v0::Constant::create(ov::element::i64, {1}, {1}),
                                                    v0::Constant::create(ov::element::i64, {}, {0}));
    auto seq_lengths = std::make_shared<v3::Broadcast>(seq_len_dim, batch_dimension);
    auto axis_0 = v0::Constant::create(ov::element::i64, ov::Shape{1}, {0});
    auto W = op_util::make_try_fold<v0::Unsqueeze>(w_pattern, axis_0);
    auto R = op_util::make_try_fold<v0::Unsqueeze>(r_pattern, axis_0);
    auto B = op_util::make_try_fold<v0::Unsqueeze>(b_pattern, axis_0);

    std::shared_ptr<ov::Node> sequence;
    if (ov::is_type<v4::LSTMCell>(found_cell) || ov::is_type<v0::LSTMCell>(found_cell)) {
        sequence = std::make_shared<v5::LSTMSequence>(
            X,
            ...
            found_cell->get_clip());
    } else if (ov::is_type<v0::RNNCell>(found_cell)) {
        sequence = std::make_shared<v5::RNNSequence>(
            X,
            initial_hidden_state,
            seq_lengths,
            W,
            ...
            found_cell->get_activations_beta(),
            found_cell->get_clip());
    } else if (ov::is_type<v3::GRUCell>(found_cell)) {
        const auto gru_cell = ov::as_type_ptr<v3::GRUCell>(found_cell);
        sequence = std::make_shared<v5::GRUSequence>(
            X,
            initial_hidden_state,
            seq_lengths,
            W,
    ...
    if (slice_axis == 0) {
        auto order = v0::Constant::create(ov::element::i64, ov::Shape{4}, {2, 1, 0, 3});
        out = std::make_shared<v1::Transpose>(out, order);
    }

    ov::NodeVector outputs;
    // We must remove num_directions dimension that was added before sequence creation
    auto axis_out = v0::Constant::create(ov::element::i64, ov::Shape{1}, {1});
    auto out_0 = std::make_shared<v0::Squeeze>(out, axis_out);
    auto out_1 = std::make_shared<v0::Squeeze>(sequence->output(1), axis_out);
    ...
    if (sequence->get_output_size() == 3) {
        auto out_2 = std::make_shared<v0::Squeeze>(sequence->output(2), axis_out);
...
bool check_condition_increment_pattern(
    ...
    const op_util::MultiSubGraphOp::MultiSubgraphInputDescriptionVector& input_descriptions,
    ...
    auto num_iter_const_label = wrap_type<op::v0::Constant>();
    auto num_iter_param_label = wrap_type<op::v0::Parameter>();
    auto num_iterations_label = std::make_shared<Or>(OutputVector{num_iter_const_label, num_iter_param_label});
    auto counter_label = wrap_type<op::v0::Parameter>();
    auto counter_step_label = wrap_type<op::v0::Constant>();
    auto updated_counter_label = wrap_type<op::v1::Add>({counter_label, counter_step_label});
    auto less_label = wrap_type<op::v1::Less>({updated_counter_label, num_iterations_label});
    auto condition_label = wrap_type<op::v0::Result>({less_label});

    // check a pattern of condition graph and get a number of iterations
    Matcher condition_matcher(condition_label);
    ...
    if (!op_util::get_constant_value(condition_map.at(counter_step_label).get_node_shared_ptr(), counter_step) ||
        counter_step != 1) {
        return false;
    }
    ...
    for (const auto& input_desc : input_descriptions) {
        ...
        auto merged_desc = ov::as_type_ptr<op_util::MultiSubGraphOp::MergedInputDescription>(input_desc);
        if (!merged_desc) {
            return false;
        }
        ...
        // get initial value of counter
        if (!op_util::get_constant_value(loop->input_value(input_idx).get_node_shared_ptr(), initial_counter)) {
            return false;
        }
    ...
    if (condition_map.count(num_iter_param_label)) {
        ...
        if (!op_util::get_constant_value(condition_map.at(num_iter_const_label).get_node_shared_ptr(), num_iters) ||
            num_iters < 1) {
            return false;
        }
        num_iters_output = std::make_shared<v0::Constant>(element::i64, Shape{}, num_iters);
    } else {
        return false;
    }
...
bool check_condition_true_pattern(const std::shared_ptr<op::v0::Result>& cond_result,
                                  ...
                                  Output<Node>& num_iters_output) {
    // return a number of iterations if pattern matches
    auto cond_const_label = wrap_type<op::v0::Constant>();
    auto condition_label = wrap_type<op::v0::Result>({cond_const_label});
    ...
    if (!op_util::get_constant_value(cond_const, cond_value) || !cond_value) {
        return false;
    }
...
void create_mul_by_one_pattern(std::shared_ptr<Node>& param,
    ...
    param = wrap_type<op::v0::Parameter>();
    auto shape_of_input = param;
    if (with_first_squeeze) {
        auto first_squeeze_axes = wrap_type<op::v0::Constant>();
        shape_of_input = wrap_type<op::v0::Squeeze>({param, first_squeeze_axes});
    }
    auto shape_of = wrap_type<op::v3::ShapeOf>({shape_of_input});
    auto concat_value = wrap_type<op::v0::Constant>();
    auto concat = wrap_type<op::v0::Concat>({concat_value, shape_of});
    broadcast_value = wrap_type<op::v0::Constant>();
    auto broadcast = wrap_type<op::v3::Broadcast>({broadcast_value, concat});
    auto squeeze_axes = wrap_type<op::v0::Constant>();
    auto squeeze = wrap_type<op::v0::Squeeze>({broadcast, squeeze_axes});
    mul = wrap_type<op::v1::Multiply>({shape_of_input, squeeze});
}
...
bool check_lstm_cell_pattern(const std::shared_ptr<op::v5::Loop>& loop,
                             const ov::ParameterVector& body_params,
                             const ov::ResultVector& body_results,
                             const op_util::MultiSubGraphOp::MultiSubgraphInputDescriptionVector& input_descriptions,
                             ...
                             std::shared_ptr<v4::LSTMCell>& lstm_cell_node) {
    // check pattern with LSMCell and return key points
    // required for fusion to LSTMSequence
    // pattern for LSTMCell in the body
    auto xi_label = wrap_type<op::v0::Parameter>();
    auto squeeze_axis_label = wrap_type<op::v0::Constant>();
    auto xi_reshape_label = wrap_type<op::v0::Squeeze>({xi_label, squeeze_axis_label});
    auto init_hidden_state_i_label = wrap_type<op::v0::Parameter>();
    auto init_cell_state_i_label = wrap_type<op::v0::Parameter>();
    auto W_label = wrap_type<op::v0::Constant>();
    auto R_label = wrap_type<op::v0::Constant>();
    auto B_label = wrap_type<op::v0::Constant>();
    ...
    auto xi_common_label = std::make_shared<Or>(OutputVector{xi_reshape_label, x_mul});
    auto hidden_common_label = std::make_shared<Or>(OutputVector{init_hidden_state_i_label, h_mul});

    auto lstm_cell_label = wrap_type<op::v4::LSTMCell>(
        {xi_common_label, hidden_common_label, init_cell_state_i_label, W_label, R_label, B_label});
    auto unsqueeze_axis_label = wrap_type<op::v0::Constant>();
    auto unsqueeze_hidden_state_label = wrap_type<op::v0::Unsqueeze>({lstm_cell_label, unsqueeze_axis_label});
    auto result_hidden_state_label = wrap_type<op::v0::Result>({unsqueeze_hidden_state_label});

    // check that body-graph contains a pattern corresponding LSTMCell
    Matcher lstm_cell_matcher(result_hidden_state_label);
    ...
    for (const auto& input_desc : input_descriptions) {
        ...
        if (body_params[param_idx] == lstm_cell_map.at(init_hidden_state_i_label).get_node_shared_ptr()) {
            ...
            auto sliced_desc = ov::as_type_ptr<op_util::MultiSubGraphOp::SliceInputDescription>(input_desc);
...
ov::pass::ConvertTensorIteratorToLSTMSequence::ConvertTensorIteratorToLSTMSequence() {
    ...
    auto tensor_iterator = wrap_type<v0::TensorIterator>();
    ...
    matcher_pass_callback callback = [this](Matcher& m) {
        ...
        auto data = wrap_type<v0::Parameter>(rank_equals(3));
        auto pattern_1 = wrap_type<v0::Constant>(rank_equals(1));
        auto squeeze = wrap_type<v1::Reshape, v0::Squeeze>({data, pattern_1});

        auto input_H_state = wrap_type<v0::Parameter>(rank_equals(2));
        auto input_C_state = wrap_type<v0::Parameter>(rank_equals(2));
        auto input_W = wrap_type<v0::Constant>(rank_equals(2));
        auto input_R = wrap_type<v0::Constant>(rank_equals(2));
        auto input_B = wrap_type<v0::Constant>(rank_equals(1));

        ov::OutputVector cell_inputs{squeeze, input_H_state, input_C_state, input_W, input_R, input_B};
        auto cell = wrap_type<v0::LSTMCell, v4::LSTMCell>(cell_inputs);

        auto pattern_2 = wrap_type<v0::Constant>(rank_equals(1));
        auto unsqueeze = wrap_type<v1::Reshape, v0::Unsqueeze>({cell, pattern_2});
        Matcher matcher(unsqueeze);
        ...
        const auto lstm_cell = ov::as_type_ptr<op_util::RNNCellBase>(found_cell);
    ...
    auto m = std::make_shared<Matcher>(tensor_iterator, matcher_name);
...
ov::pass::ConvertTensorIteratorToRNNSequence::ConvertTensorIteratorToRNNSequence() {
    ...
    matcher_pass_callback callback = [this](Matcher& m) {
        ...
        auto cell = wrap_type<v0::RNNCell>(cell_inputs);
        ...
        const auto& pattern_map = matcher.get_pattern_value_map();
        const auto& rnn_cell = ov::as_type_ptr<v0::RNNCell>(pattern_map.at(cell).get_node_shared_ptr());
        if (rnn_cell == nullptr)
            return false;
...
ov::pass::ConvertTensorIteratorToGRUSequence::ConvertTensorIteratorToGRUSequence() {
    ...
    matcher_pass_callback callback = [this](Matcher& m) {
        ...
        ov::OutputVector cell_inputs{squeeze, input_H_state, input_W, input_R, input_B};
        auto cell = wrap_type<v3::GRUCell>(cell_inputs);
        ...
        const auto& pattern_map = matcher.get_pattern_value_map();
        const auto& gru_cell = ov::as_type_ptr<v3::GRUCell>(pattern_map.at(cell).get_node_shared_ptr());
        if (gru_cell == nullptr)
            return false;
...
ov::pass::ConvertLoopWithScatterUpdateToLSTMSequence::ConvertLoopWithScatterUpdateToLSTMSequence() {
    ...
    auto input_label = any_input(rank_equals(3));
    auto input_transpose_const_label = wrap_type<op::v0::Constant>();
    auto input_transpose_label =
        wrap_type<op::v1::Transpose, op::v1::Reshape>({input_label, input_transpose_const_label}, rank_equals(3));
    auto scatter_indexes_label = wrap_type<op::v0::Constant>();
    auto scatter_update_label = std::make_shared<Or>(OutputVector{input_transpose_label, input_label});
    auto scatter_label = wrap_type<op::v3::ScatterNDUpdate>({any_input(), scatter_indexes_label, scatter_update_label});
    auto trip_count_label = wrap_type<op::v0::Constant>();
    auto cond_label = wrap_type<op::v0::Constant>();
    auto loop_label = wrap_type<op::v5::Loop>(
        {trip_count_label, cond_label, any_input(), any_input(), any_input(), any_input(), any_input(), scatter_label});
    auto output_transpose_const_label = wrap_type<op::v0::Constant>();
    auto output_transpose_label = wrap_type<op::v1::Transpose>({loop_label, output_transpose_const_label});

    // Loop body pattern:
    auto sequence_index_label = any_input(rank_equals(0));
    auto iteration_counter_label = any_input();
    auto iteration_counter_step_label = wrap_type<op::v0::Constant>();
    auto iteration_counter_incremented_label =
        wrap_type<op::v1::Add>({iteration_counter_label, iteration_counter_step_label});
    auto iteration_counter_limit_label = wrap_type<op::v0::Constant>();
    auto iteration_counter_less_than_limit_label =
        wrap_type<op::v1::Less>({iteration_counter_incremented_label, iteration_counter_limit_label});
    auto sequence_index_step_label = wrap_type<op::v0::Constant>();
    auto sequence_index_incremented_label = wrap_type<op::v1::Add>({sequence_index_label, sequence_index_step_label});
    auto sequence_index_limit_label = wrap_type<op::v0::Constant>();
    auto sequence_index_less_than_limit_label =
        wrap_type<op::v1::Less>({sequence_index_incremented_label, sequence_index_limit_label});
    auto and_label =
        wrap_type<op::v1::LogicalAnd>({iteration_counter_less_than_limit_label, sequence_index_less_than_limit_label});
    auto loop_condition_label = wrap_type<op::v0::Result>({and_label});

    auto X_body_label = any_input(rank_equals(3));
    auto C_body_label = any_input(rank_equals(2));
    auto H_body_label = any_input(rank_equals(2));
    auto gather_axis_label = wrap_type<op::v0::Constant>();
    auto sequence_index_new_shape_label = wrap_type<op::v0::Constant>();
    auto sequence_index_reshaped_label =
        wrap_type<op::v1::Reshape>({sequence_index_label, sequence_index_new_shape_label});
    auto sequence_index_or_label =
        std::make_shared<Or>(OutputVector{sequence_index_label, sequence_index_reshaped_label});
    auto gather_body_label =
        wrap_type<ov::op::v8::Gather>({X_body_label, sequence_index_or_label, gather_axis_label}, rank_equals(2));
    auto W_label = any_input();
    auto R_label = any_input();
    auto B_label = wrap_type<op::v0::Constant>();
    auto lstm_cell_label =
        wrap_type<v4::LSTMCell>({gather_body_label, H_body_label, C_body_label, W_label, R_label, B_label});
    auto scatter_index_new_shape_label = wrap_type<op::v0::Constant>();
    auto scatter_index_body_label = wrap_type<op::v1::Reshape>({sequence_index_label, scatter_index_new_shape_label});
    auto updates_label =
        wrap_type<op::v1::Reshape, op::v0::Unsqueeze>({lstm_cell_label, wrap_type<op::v0::Constant>()});
    auto scatter_axis_label = wrap_type<op::v0::Constant>();
    auto scatter_body_label =
        wrap_type<op::v3::ScatterUpdate>({any_input(), scatter_index_body_label, updates_label, scatter_axis_label},
                                         rank_equals(3));
    auto loop_output_label = wrap_type<op::v0::Result>({scatter_body_label});
    ...
    matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](Matcher& m) {
        ...
        if (output_descs.size() != 1)
            return false;
        const auto body_output_desc = ov::as_type_ptr<op_util::MultiSubGraphOp::BodyOutputDescription>(output_descs[0]);
        if (!body_output_desc || body_output_desc->m_iteration != -1)
            return false;

        Matcher loop_condition_matcher(loop_condition_label);
        Matcher loop_output_matcher(loop_output_label);
        ...
        if (!op_util::get_constant_value(loop_condition_map.at(iteration_counter_step_label).get_node_shared_ptr(),
                                         iteration_counter_step) ||
            iteration_counter_step != 1)
            return false;
        int64_t sequence_index_step = -1;
        if (!op_util::get_constant_value(loop_condition_map.at(sequence_index_step_label).get_node_shared_ptr(),
                                         sequence_index_step) ||
            sequence_index_step != 1)
            return false;

        int64_t iteration_counter_limit = -1;
        if (!op_util::get_constant_value(loop_condition_map.at(iteration_counter_limit_label).get_node_shared_ptr(),
                                         iteration_counter_limit))
            return false;
        int64_t sequence_index_limit = -1;
        if (!op_util::get_constant_value(loop_condition_map.at(sequence_index_limit_label).get_node_shared_ptr(),
        ...
        if (!op_util::get_constant_value(loop_output_map.at(gather_axis_label).get_node_shared_ptr(), gather_axis) ||
            gather_axis != 0)
            return false;
        int64_t scatter_axis = -1;
        if (!op_util::get_constant_value(loop_output_map.at(scatter_axis_label).get_node_shared_ptr(), scatter_axis) ||
            scatter_axis != 0)
            return false;
        ...
        for (const auto& desc : input_descs) {
            if (body_parameters[desc->m_body_parameter_index] == X_body) {
                if (!ov::as_type_ptr<op_util::MultiSubGraphOp::InvariantInputDescription>(desc)) {
                    return false;
                }
            ...
            if (body_parameters[desc->m_body_parameter_index] == H_body) {
                auto merged_desc = ov::as_type_ptr<op_util::MultiSubGraphOp::MergedInputDescription>(desc);
                if (!merged_desc) {
                    return false;
                }
        ...
        auto constant_is_zero = [](const Output<Node>& node) -> bool {
            ...
            float value = -1.0f;
            return op_util::get_single_value(constant, value) && value == 0.0f;
        };
        ...
        for (auto&& loop_consumer : loop->output(0).get_target_inputs()) {
            ...
            if (ov::is_type<op_util::ShapeOfBase>(node)) {
    ...
    auto m = std::make_shared<Matcher>(output_transpose_label, matcher_name);
    register_matcher(m, callback);
}
...
ov::pass::ConvertLoopWithSlicedInputConcatOutputToLSTMSequence::ConvertLoopWithSlicedInputConcatOutputToLSTMSequence() {
    ...
    auto loop_label = wrap_type<v5::Loop>();
    ...
    matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](Matcher& m) {
        ...
        for (const auto& output_desc : output_descriptions) {
            ...
            auto concat_desc = ov::as_type_ptr<op_util::MultiSubGraphOp::ConcatOutputDescription>(output_desc);
            if (concat_desc) {
        ...
        auto tr_order = rg.make<v0::Constant>(element::i32, Shape{3}, std::vector<int32_t>{1, 0, 2});
        auto x_prep = rg.make<v1::Transpose>(x, tr_order);
        // initial hidden and cell states are of shape [batch_size, hidden_size]
        // needs to add num_directions size
        auto unsqueeze_axis = rg.make<v0::Constant>(element::i32, Shape{1}, std::vector<int32_t>{1});
        auto init_hidden_state_prep = rg.make<v0::Unsqueeze>(init_hidden_state, unsqueeze_axis);
        auto init_cell_state_prep = rg.make<v0::Unsqueeze>(init_cell_state, unsqueeze_axis);
        ...
        auto ss_stop = rg.make<v0::Constant>(element::i64, Shape{1}, 2);
        ...
        auto batch_size = rg.make<v1::StridedSlice>(x_shape,
                                                    ...
                                                    std::vector<int64_t>{0});
        auto seq_lens_prep = rg.make<v1::Broadcast>(num_iters_output, batch_size);
        // prepare W, R, B to add num_directions dimension
        unsqueeze_axis = rg.make<v0::Constant>(element::i32, Shape{1}, std::vector<int32_t>{0});
        auto W_prep = rg.make<v0::Unsqueeze>(W, unsqueeze_axis);
        auto R_prep = rg.make<v0::Unsqueeze>(R, unsqueeze_axis);
        auto B_prep = rg.make<v0::Unsqueeze>(B, unsqueeze_axis);
        ...
        auto lstm_sequence = rg.make<v5::LSTMSequence>(x_prep,
        ...
        auto squeeze_axis = rg.make<v0::Constant>(element::i32, Shape{1}, std::vector<int32_t>{1});
        ov::Output<Node> all_hidden_states_prep = rg.make<v0::Squeeze>(lstm_sequence->output(0), squeeze_axis);
        all_hidden_states_prep = rg.make<v1::Transpose>(all_hidden_states_prep, tr_order);
        // prepare output with last hidden state
        // LSTMSequence operation outputs it in a format [batch_size, num_directions, hidden_size]
        auto tr_order2 = rg.make<v0::Constant>(element::i32, Shape{3}, std::vector<int32_t>{1, 0, 2});
        ov::Output<ov::Node> last_hidden_state_prep = rg.make<v1::Transpose>(lstm_sequence->output(1), tr_order2);
    ...
    auto m = std::make_shared<Matcher>(loop_label, matcher_name);
    register_matcher(m, callback);
}
...
public:
    ...
    EliminateGatherWithRange() {
        ...
        auto data_label = any_input(rank_equals(3));
        auto shapeof_label = wrap_type<op_util::ShapeOfBase>({data_label});
        auto shapeof_gather_label = wrap_type<op_util::GatherBase>(
            {shapeof_label, wrap_type<op::v0::Constant>(), wrap_type<op::v0::Constant>()});
        auto shapeof_gather2_label = wrap_type<op_util::GatherBase>(
            {shapeof_gather_label, wrap_type<op::v0::Constant>(), wrap_type<op::v0::Constant>()});
        auto reshape_label = wrap_type<op::v1::Reshape>({shapeof_gather2_label, wrap_type<op::v0::Constant>()});
        auto range_label =
            wrap_type<op::v4::Range>({wrap_type<op::v0::Constant>(), reshape_label, wrap_type<op::v0::Constant>()});
        auto match_node = wrap_type<op_util::GatherBase>({data_label, range_label, wrap_type<op::v0::Constant>()});

        matcher_pass_callback callback = [=](Matcher& m) {
            const auto& pattern_map = m.get_pattern_value_map();
            auto gather = ov::as_type_ptr<op_util::GatherBase>(m.get_match_root());
            if (!gather)
                return false;
            ...
            if (!op_util::get_constant_value(shapeof_gather2->get_input_node_shared_ptr(1), shapeof_gather2_index))
                return false;
            if (!op_util::get_constant_value(shapeof_gather2->get_input_node_shared_ptr(2), shapeof_gather2_axis) ||
            ...
            if (!op_util::get_constant_value(range->get_input_node_shared_ptr(0), range_start) || range_start != 0)
                return false;
            if (!op_util::get_constant_value(range->get_input_node_shared_ptr(2), range_step) || range_step != 1)
                return false;

            int64_t gather_axis = -1;
            if (!op_util::get_constant_value(gather->get_input_node_shared_ptr(2), gather_axis) ||
        ...
        auto m = std::make_shared<Matcher>(match_node, "EliminateGatherWithRange");
        register_matcher(m, callback);
    }
};
...
ov::pass::FuseReverseLSTMSequence::FuseReverseLSTMSequence() {
    ...
    auto first_transpose_label =
        wrap_type<op::v1::Transpose, op::v1::Reshape>({data_label, wrap_type<op::v0::Constant>()}, rank_equals(3));
    auto input_to_first_reverse_sequence_label = std::make_shared<Or>(OutputVector{first_transpose_label, data_label});
    auto first_reverse_sequence_label =
        wrap_type<op::v0::ReverseSequence>({input_to_first_reverse_sequence_label, any_input()});
    auto second_transpose_label =
        wrap_type<op::v1::Transpose>({first_reverse_sequence_label, wrap_type<op::v0::Constant>()});
    auto lstm_label = wrap_type<op::v5::LSTMSequence>(
        {second_transpose_label, any_input(), any_input(), any_input(), any_input(), any_input(), any_input()},
        ov::pass::pattern::consumers_count(1));
    auto squeeze_label = wrap_type<op::v0::Squeeze>({lstm_label, wrap_type<op::v0::Constant>()});
    auto second_reverse_sequence_label = wrap_type<op::v0::ReverseSequence>({squeeze_label, any_input()});
    ...
    matcher_pass_callback callback = [=](Matcher& m) {
        ...
        if (!op_util::get_constant_value(squeeze->get_input_node_shared_ptr(1), squeeze_axis) || squeeze_axis != 1)
            return false;
    ...
    auto m = std::make_shared<Matcher>(second_reverse_sequence_label, matcher_name);
    register_matcher(m, callback);
}
...
ov::pass::FuseLSTMSequencesToBidirectionalLSTMSequence::FuseLSTMSequencesToBidirectionalLSTMSequence() {
    ...
    auto data_label = any_input();

    // forward pattern
    auto transpose_forward_label = wrap_type<op::v1::Transpose>({data_label, wrap_type<op::v0::Constant>()});
    auto lstm_sequence_forward_first_input_label =
        std::make_shared<Or>(OutputVector{transpose_forward_label, data_label});
    auto shapeof_forward_label = wrap_type<op_util::ShapeOfBase>({lstm_sequence_forward_first_input_label});
    auto gather_forward_label = wrap_type<op_util::GatherBase>(
        {shapeof_forward_label, wrap_type<op::v0::Constant>(), wrap_type<op::v0::Constant>()});
    auto max_sequence_len_forward_label = wrap_type<op::v0::Constant>();
    auto broadcast_forward_label = wrap_type<op::v3::Broadcast>({max_sequence_len_forward_label, gather_forward_label});
    auto const_sequence_lengths_forward_label = wrap_type<op::v0::Constant>();
    auto sequence_lengths_forward_label =
        std::make_shared<Or>(OutputVector{broadcast_forward_label, const_sequence_lengths_forward_label});
    auto lstm_sequence_forward_label = wrap_type<op::v5::LSTMSequence>({lstm_sequence_forward_first_input_label,
                                                                        any_input(),
                                                                        any_input(),
    ...
    auto squeeze_forward_label =
        wrap_type<op::v0::Squeeze>({lstm_sequence_forward_label, wrap_type<op::v0::Constant>()}, rank_equals(3));

    // backward pattern
    auto transpose_reverse_label =
        wrap_type<op::v1::Transpose, op::v1::Reshape>({data_label, wrap_type<op::v0::Constant>()});
    auto lstm_sequence_reverse_first_input_label =
        std::make_shared<Or>(OutputVector{transpose_reverse_label, data_label});
    auto shapeof_reverse_label = wrap_type<op_util::ShapeOfBase>({lstm_sequence_reverse_first_input_label});
    auto gather_reverse_label = wrap_type<op_util::GatherBase>(
        {shapeof_reverse_label, wrap_type<op::v0::Constant>(), wrap_type<op::v0::Constant>()});
    auto max_sequence_len_reverse_label = wrap_type<op::v0::Constant>();
    auto broadcast_reverse_label = wrap_type<op::v3::Broadcast>({max_sequence_len_reverse_label, gather_reverse_label});
    auto const_sequence_lengths_reverse_label = wrap_type<op::v0::Constant>();
    auto sequence_lengths_reverse_label =
        std::make_shared<Or>(OutputVector{broadcast_reverse_label, const_sequence_lengths_reverse_label});
    auto lstm_sequence_reverse_label = wrap_type<op::v5::LSTMSequence>({lstm_sequence_reverse_first_input_label,
                                                                        any_input(),
                                                                        any_input(),
    ...
    auto squeeze_reverse_label =
        wrap_type<op::v0::Squeeze>({lstm_sequence_reverse_label, wrap_type<op::v0::Constant>()}, rank_equals(3));

    auto concat_label = wrap_type<op::v0::Concat>({squeeze_forward_label, squeeze_reverse_label});
    ...
    matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](Matcher& m) {
        ...
        if (!op_util::get_constant_value(squeeze_forward->get_input_node_shared_ptr(1), squeeze_forward_axis) ||
        ...
        if (!op_util::get_constant_value(squeeze_reverse->get_input_node_shared_ptr(1), squeeze_reverse_axis) ||
        ...
        if (is_forward_sequence_lengths_constant) {
            ...
            int64_t gather_axis = -1;
            if (!op_util::get_constant_value(gather_forward->get_input_node_shared_ptr(1), gather_index) ||
                gather_index != 0)
                return false;
            if (!op_util::get_constant_value(gather_forward->get_input_node_shared_ptr(2), gather_axis) ||
                gather_axis != 0)
                return false;
            ...
            gather_index = -1;
            gather_axis = -1;
            if (!op_util::get_constant_value(gather_reverse->get_input_node_shared_ptr(1), gather_index) ||
                gather_index != 0)
                return false;
            if (!op_util::get_constant_value(gather_reverse->get_input_node_shared_ptr(2), gather_axis) ||
                gather_axis != 0)
                return false;
