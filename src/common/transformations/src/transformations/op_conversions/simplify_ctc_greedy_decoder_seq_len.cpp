...
ov::pass::SimplifyCTCGreedyDecoderSeqLen::SimplifyCTCGreedyDecoderSeqLen() {
    ...
    auto decoder = ov::pass::pattern::wrap_type<v6::CTCGreedyDecoderSeqLen>();

    matcher_pass_callback callback = [=](Matcher& m) {
        auto decoder_seq_len = ov::as_type_ptr<v6::CTCGreedyDecoderSeqLen>(m.get_match_root());
        if (!decoder_seq_len) {
            return false;
        }
        ...
        auto indexT = v0::Constant::create(seq_len_type, Shape{1}, {1});
        auto T = std::make_shared<v1::Gather>(data_shape, indexT, axisT);

        auto axisN = v0::Constant::create(seq_len_type, Shape{}, {0});
        auto indexN = v0::Constant::create(seq_len_type, Shape{1}, {0});
        auto N = std::make_shared<v1::Gather>(data_shape, indexN, axisN);

        auto start = v0::Constant::create(seq_len_type, Shape{}, {1});
        auto step = v0::Constant::create(seq_len_type, Shape{}, {1});
        auto plus1 = v0::Constant::create(element::i64, Shape{1}, {1});
        auto plusT = std::make_shared<v1::Add>(T, plus1);
        auto const_plusT = v0::Constant::create(seq_len_type, Shape{1}, {0});
        auto plusT_scalar = std::make_shared<v0::Squeeze>(plusT, const_plusT);
        ...
        auto mask_val_false = v0::Constant::create(seq_len_type, Shape{}, {0});
        ...
        auto squeeze2_axis = v0::Constant::create(seq_len_type, Shape({1}), {3});
        auto squeeze2_output_f = std::make_shared<v0::Squeeze>(decoder->output(0), squeeze2_axis);
        auto squeeze1_axis = v0::Constant::create(seq_len_type, Shape({1}), {2});
        auto squeeze1_output_f = std::make_shared<v0::Squeeze>(squeeze2_output_f->output(0), squeeze1_axis);
        ...
        auto output_i = std::make_shared<v0::Convert>(squeeze1_output_f->output(0), ci_type);
        auto minus1 = v0::Constant::create(ci_type, Shape{}, {-1});
        // Get to know where equal -1
        auto where_equal_minus1 = std::make_shared<v1::Equal>(output_i, minus1);
        ...
        auto output_seq_len_i = std::make_shared<v0::Convert>(output_seq_len->output(0), sl_type);
    ...
    auto m = std::make_shared<Matcher>(decoder, matcher_name);
