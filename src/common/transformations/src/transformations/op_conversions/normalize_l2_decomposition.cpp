...
ov::pass::NormalizeL2Decomposition::NormalizeL2Decomposition() {
    ...
    auto normalize_l2_pattern = ov::pass::pattern::wrap_type<v0::NormalizeL2>();

    matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](Matcher& m) {
        auto normalize_l2 = ov::as_type_ptr<v0::NormalizeL2>(m.get_match_root());
        ...
        auto eps_const_node =
            v0::Constant::create(normalize_l2->get_input_element_type(0), Shape{}, {normalize_l2->get_eps()});
        switch (normalize_l2->get_eps_mode()) {
        case op::EpsMode::ADD:
            eps_node = std::make_shared<v1::Add>(reduce_sum, eps_const_node);
            break;
        case op::EpsMode::MAX:
            eps_node = std::make_shared<v1::Maximum>(reduce_sum, eps_const_node);
            break;
        default:
            return false;
        }

        auto sqrt = std::make_shared<v0::Sqrt>(eps_node);
        auto div = std::make_shared<v1::Divide>(normalize_l2->input_value(0), sqrt);
    ...
    auto m = std::make_shared<Matcher>(normalize_l2_pattern, matcher_name);
