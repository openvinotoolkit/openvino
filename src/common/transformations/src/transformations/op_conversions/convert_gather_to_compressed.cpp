...
ov::pass::ConvertGatherToGatherCompressed::ConvertGatherToGatherCompressed() {
    ...
    // fp16/bf16 constant + convert(real16 to f32) + gather case
    ...
    auto gather_m_uni = std::make_shared<ov::pass::pattern::op::Or>(ov::OutputVector{gather_m, gather_wo_scale});

    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        const auto& pattern_map = m.get_pattern_value_map();
        if (pattern_map.count(gather_m)) {
            ...
            if (with_zero_point) {
                optional_zero_point = reshape_to_2d
                                          ? reshape_const_to_2d(pattern_map.at(sub_const_m).get_node_shared_ptr())
    ...
    auto m = std::make_shared<ov::pass::pattern::Matcher>(gather_m_uni, "ConvertGatherToGatherCompressed");
