    ...
    const matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](pattern::Matcher& m) {
        ...
        if (!exclude_pad && rounding_type_v14 == ov::op::RoundingType::CEIL_TORCH) {
            ...
            const auto pads_begin_v1 =
                node_registry.make<v0::Concat>(OutputVector{pads_remaining, std::move(pads_begin_node)}, 0);
            const auto pads_end_v1 =
                node_registry.make<v0::Concat>(OutputVector{std::move(pads_remaining), std::move(pads_end_node)}, 0);
            const auto pad_node =
