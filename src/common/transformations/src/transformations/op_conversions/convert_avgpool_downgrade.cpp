...
ov::pass::ConvertAvgPool14ToAvgPool1::ConvertAvgPool14ToAvgPool1() {
    ...
    const matcher_pass_callback callback = [](pattern::Matcher& m) {
        ...
        const auto rounding_type_v1 = rounding_type_v14 == ov::op::RoundingType::CEIL_TORCH
        ...
        if (!exclude_pad) {
            const auto zero = Constant::create(element::f32, Shape{}, {0});
            const auto zero_node = node_registry.make<ConvertLike>(zero, input);
            const auto zero_i64 = Constant::create(element::i64, Shape{}, {0});
            ...
            const auto pads_begin_node = Constant::create(element::i64, Shape{pads_begin_v14.size()}, pads_begin_v14);
            const auto pads_end_v14 = avg_pool_v14->get_pads_end();
            const auto pads_end_node = Constant::create(element::i64, Shape{pads_end_v14.size()}, pads_end_v14);
            const auto pads_len = Constant::create(element::i64, Shape{}, {pads_begin_v14.size()});
        ...
        const auto avg_pool_v1 = node_registry.make<ov::op::v1::AvgPool>(input,
