...
std::shared_ptr<ov::Node> ov::pass::ScaledDotProductAttentionDecomposition::decompose(
    ...
    // before above change, cases either have causal = true with explict build mask or casual = false and direct mask
    // input after above change, cases have mask = null and causual = true
    if ((node->get_causal() || node->get_input_size() > 3) && !(node->get_causal() && node->get_input_size() < 4)) {
        Output<Node> mask;
