...
ov::pass::ConvertMaxPool14ToMaxPool8::ConvertMaxPool14ToMaxPool8() {
    ...
    const matcher_pass_callback callback = [](pattern::Matcher& m) {
        ...
        if (rounding_type_v14 == ov::op::RoundingType::CEIL_TORCH) {
            ...
            const auto padding_begin_node = Constant::create(element::i64, Shape{padding_begin.size()}, padding_begin);
            const auto padding_end = max_pool_v14->get_pads_end();
            const auto padding_end_node = Constant::create(element::i64, Shape{padding_end.size()}, padding_end);
            const auto zero = Constant::create(element::i64, Shape{}, {0});
            const auto one = Constant::create(element::i64, Shape{}, {1});
            const auto two = Constant::create(element::i64, Shape{}, {2});

            const auto pads_size = max_pool_v14->get_pads_begin().size();
            const auto pads_len = Constant::create(element::i64, Shape{}, {pads_size});
            const auto pads_remaining = Constant::create(element::i64, Shape{2}, {0, 0});

            // gather input spatial dims and prepare for compare as values (in_dim + pad)
            const auto end = Constant::create(element::i64, Shape{}, {pads_size + 2});
            ...
            auto minus_inf =
                Constant::create(element::f32, Shape{}, {-std::numeric_limits<float>::infinity()})->output(0);
