...
ov::OutputVector ov::pass::GroupQueryAttentionDecomposition::decompose(
    ...
    const auto zero = register_new_node(ov::op::v0::Constant::create(ov::element::i64, ov::Shape{1}, {0}));
    const auto zero_without_shape = register_new_node(ov::op::v0::Constant::create(ov::element::i64, ov::Shape{}, {0}));
    const auto one = register_new_node(ov::op::v0::Constant::create(ov::element::i64, ov::Shape{1}, {1}));
    const auto one_without_shape = register_new_node(ov::op::v0::Constant::create(ov::element::i64, ov::Shape{}, {1}));
    const auto two = register_new_node(ov::op::v0::Constant::create(ov::element::i64, ov::Shape{1}, {2}));
    const auto seqlens_elemi64 = register_new_node<ov::op::v0::Convert>(seqlens_k, ov::element::i64);
    const auto real_seqlens = register_new_node<ov::op::v1::Add>(seqlens_elemi64, one);

    // Only consider batch is 1
    const auto seqlens_1d = register_new_node<ov::op::v1::Reshape>(real_seqlens, one, false);
    const auto past_seqlen = register_new_node<ov::op::v1::Subtract>(seqlens_1d, current_seqlen);
    ...
    if (do_rotary) {
        ...
        const auto sin = register_new_node<ov::op::v8::Gather>(sin_cache, position_ids, zero);
    ...
    auto construct_kv_cache = [&](const ov::Output<ov::Node>& past, const ov::Output<ov::Node>& current) {
        return register_new_node<ov::op::v0::Concat>(ov::OutputVector{past, current}, 2);
    };
    if (is_static_input) {
    ...
    const auto concat_kv_len_scalar = register_new_node<ov::op::v0::Squeeze>(concat_kv_len);
    ...
    const auto triu = register_new_node<ov::op::v1::Greater>(hori_range, vert_range);
    const auto typed_zero = register_new_node(ov::op::v0::Constant::create(T, ov::Shape{}, {0}));
    ...
    if (scale != 0.0f) {
        auto scale_node = register_new_node(ov::op::v0::Constant::create(T, Shape{}, {scale}));
        qga_output = register_new_node<ov::op::v13::ScaledDotProductAttention>(Q, K, V, mask, scale_node, false);
    } else {
        qga_output = register_new_node<ov::op::v13::ScaledDotProductAttention>(Q, K, V, mask, false);
    }
    ...
    auto perm = register_new_node(ov::op::v0::Constant::create(ov::element::i64, ov::Shape{4}, {0, 2, 1, 3}));
    auto qga_output_transposed = register_new_node<ov::op::v1::Transpose>(qga_output, perm);
    auto dim_merge_shape = register_new_node(ov::op::v0::Constant::create(ov::element::i32, ov::Shape{3}, {0, 0, -1}));
    auto output = register_new_node<ov::op::v1::Reshape>(qga_output_transposed, dim_merge_shape, true)->output(0);
...
std::shared_ptr<ov::Node> ov::pass::GroupQueryAttentionDecomposition::rotaryEmbedding(ov::Output<ov::Node> input,
    ...
    if (interleaved) {
        auto two = ov::op::v0::Constant::create(ov::element::i64, ov::Shape{1}, {2});
        auto cos_last_dim = get_dimensions(cos.get_node_shared_ptr(), {-1});
        auto input_shape = register_new_node<ov::op::v3::ShapeOf>(input);
        auto dim_bns = get_dimensions(input_shape, {0, 1, 2});

        auto negtive_one = ov::op::v0::Constant::create(ov::element::i64, ov::Shape{1}, {-1});
        auto split_input_shape = register_new_node<ov::op::v0::Concat>(ov::NodeVector{dim_bns, cos_last_dim, two}, 0);
        auto reshaped_input = register_new_node<ov::op::v1::Reshape>(input, split_input_shape, false);

        auto in_split = make_split(reshaped_input, 2, -1);
        split_input_shape = register_new_node<ov::op::v0::Concat>(ov::NodeVector{dim_bns, cos_last_dim}, 0);
        auto in_split_0 = register_new_node<ov::op::v1::Reshape>(in_split[0], split_input_shape, false);
        ...
        auto res_1_5d = register_new_node<ov::op::v1::Reshape>(res_1, split_input_shape, false);

        auto concat_ret = register_new_node<ov::op::v0::Concat>(ov::NodeVector{res_0_5d, res_1_5d}, -1);
        return register_new_node<ov::op::v1::Reshape>(concat_ret, input_shape, false);
    } else {
