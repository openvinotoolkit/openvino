...
void ov::pass::ConvertBatchToSpace::convert_batch_to_space() {
    ...
    matcher_pass_callback callback = [this](pattern::Matcher& m) {
        ...
        const auto block_prod = make_shared<ReduceProd>(block, zero);
        const auto batch_div = make_shared<Divide>(batch, block_prod);
        ...
        const auto end = Constant::create(element::i64, Shape{1}, {block_lenght});
        const auto block_tail = make_shared<Slice>(block, one, end, one);
        const auto data_shape_tail = make_shared<Slice>(shape_of_data, one, end, one);
        const auto dispersed_shape = make_shared<Concat>(OutputVector{block_tail, batch_div, data_shape_tail}, 0);
        const bool special_zero = false;
        shared_ptr<Node> flat_node = make_shared<Reshape>(data, dispersed_shape, special_zero);
        ...
        //   * B_{N - 1}])
        const auto squeezed_shape_tail = make_shared<Multiply>(block_tail, data_shape_tail);
        const auto squeezed_shape = make_shared<Concat>(OutputVector{batch_div, squeezed_shape_tail}, 0);
...
void ov::pass::ConvertBatchToSpace::convert_batch_to_space_by_elements() {
    ...
    matcher_pass_callback callback = [this](pattern::Matcher& m) {
        ...
        const auto make_shape_node = [](OutputVector nodes) {
