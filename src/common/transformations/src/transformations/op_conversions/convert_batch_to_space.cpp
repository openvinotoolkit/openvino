...
void ConvertBatchToSpace::convert_batch_to_space() {
    ...
    matcher_pass_callback callback = [this](pattern::Matcher& m) {
        ...
        const auto shape_of_data = rg.make<v3::ShapeOf>(data, block.get_element_type());
        const auto batch = rg.make<v8::Gather>(shape_of_data, zero, zero);
        const auto block_prod = rg.make<v1::ReduceProd>(block, zero);
        const auto batch_div = rg.make<v1::Divide>(batch, block_prod);
        ...
        //   * B_{N - 1}])
        const auto squeezed_shape_tail = rg.make<v1::Multiply>(block_tail, data_shape_tail);
        const auto squeezed_shape = rg.make<v0::Concat>(OutputVector{batch_div, squeezed_shape_tail}, 0);
        ...
        const auto shape_of_flat_node = rg.make<v3::ShapeOf>(flat_node, crops_end.get_element_type());
        const auto upperbounds = rg.make<v1::Subtract>(shape_of_flat_node, crops_end);

        const auto begin_mask = vector<int64_t>(data_shape_rank.get_length(), 0);
        const auto& end_mask = begin_mask;
        flat_node = rg.make<v1::StridedSlice>(flat_node, crops_begin, upperbounds, begin_mask, end_mask);
...
void ConvertBatchToSpace::convert_batch_to_space_by_elements() {
    ...
    matcher_pass_callback callback = [this](pattern::Matcher& m) {
        ...
        const auto zero = rg.make<v0::Constant>(i64, Shape{1}, 0);
        const auto one = rg.make<v0::Constant>(i64, Shape{1}, 1);
        const auto two = rg.make<v0::Constant>(i64, Shape{1}, 2);
        const auto int_max = rg.make<v0::Constant>(i64, Shape{1}, INT_MAX);

        const auto shape_of_data = rg.make<v3::ShapeOf>(data, block.get_element_type());
        const auto et_zero = rg.make<v0::Constant>(block.get_element_type(), Shape{1}, 0);
        shared_ptr<Node> dispersed_shape = rg.make<v0::Concat>(OutputVector{et_zero, shape_of_data}, 0);
        ...
        const auto make_concat = [&](OutputVector nodes) {
            nodes.erase(remove_if(nodes.begin(),
                        ...
                        nodes.end());
            return rg.make<v0::Concat>(nodes, 0);
        };
        ...
        for (int64_t b_idx = 1; b_idx < block_length; ++b_idx) {
            ...
            if (!div) {
                const auto batch = rg.make<v8::Gather>(shape_of_data, zero, zero);
                div = rg.make<v1::Divide>(batch, block_value);
            } else {
                div = rg.make<v1::Divide>(div, block_value);
            }
            auto ds_tail = rg.make<v8::Slice>(dispersed_shape, two, int_max, one);
            ...
            const auto sq_slice = rg.make<v8::Slice>(squeezed_shape, one, block_index, one);
            const auto sq_bidx_dim = rg.make<v8::Gather>(squeezed_shape, block_index, zero);
            const auto sq_mul = rg.make<v1::Multiply>(sq_bidx_dim, block_value);
            const auto sq_shape_tail = rg.make<v8::Slice>(squeezed_shape, block_index_next, int_max, one);
