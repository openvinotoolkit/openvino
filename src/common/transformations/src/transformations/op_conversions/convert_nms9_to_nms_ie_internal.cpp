...
ov::pass::ConvertNMS9ToNMSIEInternal::ConvertNMS9ToNMSIEInternal() {
    ...
    auto nms = ov::pass::pattern::wrap_type<v9::NonMaxSuppression>();

    matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](Matcher& m) {
        auto nms_9 = ov::as_type_ptr<v9::NonMaxSuppression>(m.get_match_root());
        ...
        new_max_per_class = std::make_shared<v1::Reshape>(max_per_class, new_shape_for_max_per_class, true);
        ...
        case v9::NonMaxSuppression::BoxEncodingType::CENTER:
            center_point_box = 1;
            break;
        case v9::NonMaxSuppression::BoxEncodingType::CORNER:
            center_point_box = 0;
            break;
        default:
        ...
        if (nms_9->output(0).get_element_type() != output_0.get_element_type()) {
            output_0 = std::make_shared<v0::Convert>(output_0, nms_9->output(0).get_element_type());
        ...
        if (nms_9->output(2).get_element_type() != output_2.get_element_type()) {
            output_2 = std::make_shared<v0::Convert>(output_2, nms_9->output(2).get_element_type());
