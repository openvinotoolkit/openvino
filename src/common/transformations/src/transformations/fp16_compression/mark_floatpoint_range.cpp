...
MarkFloatingPointRange::MarkFloatingPointRange(const ov::element::Type& target) {
    ...
    ov::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        if (range && range->get_output_type().is_real()) {
            ...
            ov::disable_conversion(node, target);

            // mark inputs as well
            for (const auto& range_input : range->input_values()) {
                ov::disable_conversion(range_input.get_node_shared_ptr(), target);
            }
            is_changed = true;
        } else {
