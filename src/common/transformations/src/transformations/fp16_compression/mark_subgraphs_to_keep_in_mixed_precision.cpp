...
public:
    ...
    PropagateUpMarkToKeepInMixedPrecision() {
        ...
        matcher_pass_callback callback = [=](pattern::Matcher& m) {
            ...
            for (const auto& output : node->outputs()) {
                for (const auto& out_inputs : output.get_target_inputs()) {
                    if (out_inputs.get_element_type().is_real() &&
                        is_compression_disabled(out_inputs.get_node()->shared_from_this(), element::f16)) {
            ...
            disable_compression(node, element::f16);
            return true;
        };
        ...
        matcher_pass_callback callback = [=](pattern::Matcher& m) {
            ...
            for (const auto& in_node : node->input_values()) {
                ...
                if (is_fq_path(in_node.get_node_shared_ptr())) {
                    enable_compression(node, element::f16);
                    return true;
                }

                if (is_compression_disabled(in_node.get_node_shared_ptr(), element::f16)) {
                    disable_compression(node, element::f16);
                    is_changed = true;
                    break;
                }
...
public:
    ...
    MarkDivWithEps() {
        ...
        matcher_pass_callback callback = [=](pattern::Matcher& m) {
            ...
            disable_compression(m.get_match_root(), element::f16);
            return true;
        };
