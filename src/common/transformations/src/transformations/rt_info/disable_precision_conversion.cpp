...
void ov::disable_compression(const std::shared_ptr<Node>& node, const element::Type& to) {
    return disable_compression(node, element::dynamic, to);
}

void ov::disable_compression(const std::shared_ptr<Node>& node,
...
void ov::disable_compression(const std::shared_ptr<Node>& node,
    ...
    for (const auto& from : from_types) {
        for (const auto& to : to_types) {
            disable_compression(node, from, to);
        }
    }
}

void ov::enable_compression(const std::shared_ptr<Node>& node, const element::Type& to) {
    enable_compression(node, element::dynamic, to);
}

void ov::enable_compression(const std::shared_ptr<Node>& node,
                                    const element::Type& from,
                                    const element::Type& to) {
    auto& rt_info = node->get_rt_info();
    if (rt_info.count(DisablePrecisionConversion::get_type_info_static())) {
...
void ov::enable_compression(const std::shared_ptr<Node>& node,
    ...
    for (const auto& from : from_types) {
        for (const auto& to : to_types) {
            enable_compression(node, from, to);
        }
    }
}

bool ov::is_compression_disabled(const std::shared_ptr<const Node>& node, const element::Type& to) {
    return is_compression_disabled(node, element::dynamic, to);
}

bool ov::is_compression_disabled(const std::shared_ptr<const Node>& node,
                                         const element::Type& from,
                                         const element::Type& to) {
    auto& rt_info = node->get_rt_info();
    if (rt_info.count(DisablePrecisionConversion::get_type_info_static())) {
