...
pass::EliminateTranspose::EliminateTranspose() {
    ...
    matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        // don't eliminate Transpose if:
        // - order is an empty tensor - then the dimensions are reverted
        // - order is not [0, 1, 2, ...., rank - 1]
        if (order_values.size() == 0 || order_values != ref_values) {
            return false;
        }
