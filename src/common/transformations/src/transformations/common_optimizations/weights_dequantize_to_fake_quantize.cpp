...
ngraph::pass::WeightsDequantizeToFakeQuantize::WeightsDequantizeToFakeQuantize() {
    ...
    callback = [=](ngraph::pattern::Matcher& m) {
        ...
        auto& zero_point = pattern_map.count(sub_c)
                               ...
                               : opset6::Constant::create(convert_node->get_element_type(), {}, {0});

        const auto& output_low =
            op::util::eltwise_fold<opset6::Multiply>(op::util::eltwise_fold<opset6::Subtract>(input_low, zero_point),
                                                     scale_node);
        const auto& output_high =
            op::util::eltwise_fold<opset6::Multiply>(op::util::eltwise_fold<opset6::Subtract>(input_high, zero_point),
                                                     scale_node);

        auto fq = std::make_shared<opset6::FakeQuantize>(convert_node,
                                                         input_low,
                                                         input_high,
                                                         output_low,
                                                         output_high,
