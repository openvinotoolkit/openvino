...
ov::pass::EliminateLoopInputsOutputs::EliminateLoopInputsOutputs() {
    ...
    matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        auto subgraph = as_type_ptr<ov::op::util::SubGraphOp>(pattern_to_output.at(subgraph_label).get_node_shared_ptr());
        if (!subgraph) {
            return false;
        }
        ...
        for (const auto& input_description : subgraph->get_input_descriptions()) {
            ...
            if (const auto merged_input_desc = as_type_ptr<MergedD>(input_description)) {
                ...
                if (param && param->get_output_target_inputs(0).size() == 1) {
                    process_inputs.emplace_front([=, &body_model]() { body_model->remove_parameter(param);   });
                } else if (param) {
                    process_inputs.emplace_back([=, &new_node]() { new_node->set_invariant_input(body_param, init_value);});
                } else {
                    process_inputs.emplace_back([=, &new_node]() { new_node->set_merged_input(body_param, body_res, init_value); });
                }
        ...
        for (const auto& output_description : subgraph->get_output_descriptions()) {
            ...
            if (const auto& body_param = get_parent_param(body_result)) {
                ...
                process_outputs.emplace_front([=, &body_model, &idx_to_new_output]() {
                 ...
                 * this could only happen if new output description type was added after this transformation
                 * written
                 */
                return false;
            }
        }
        for_each(process_inputs.begin(), process_inputs.end(), [](function<void()>& f){ f(); });
        for_each(process_outputs.begin(), process_outputs.end(), [](function<void()>& f){ f(); });
