...
ReduceType get_reduce_type(const std::shared_ptr<ov::Node>& reduce_node) {
    if (ov::is_type<v1::ReduceMax>(reduce_node)) {
        return ReduceType::MAX;
    } else if (ov::is_type<v1::ReduceMin>(reduce_node)) {
...
PullSqueezeThroughEltwise::PullSqueezeThroughEltwise() {
    ...
    matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](pattern::Matcher& m) {
        ...
        for (size_t input_index = 0; input_index < eltwise_inputs_size; ++input_index) {
            ...
            if (!is_type<v0::Constant>(input_node) && !is_type<v0::Unsqueeze>(input_node) &&
                !is_type<v1::Reshape>(input_node)) {
                return false;
            }
        }
        ...
        for (size_t input_index = 0; input_index < eltwise_inputs_size; ++input_index) {
            ...
            const auto new_input_node = op_util::clone_try_fold(squeeze, {eltwise_input, squeeze->input_value(1)});

            if (!is_type<v0::Constant>(new_input_node))
                register_new_node(as_type_ptr<v0::Squeeze>(new_input_node));

            ov::copy_runtime_info(squeeze, new_input_node);
            eltwise_inputs.push_back(new_input_node);
        }

        const auto new_eltwise = op_util::clone_try_fold(eltwise, eltwise_inputs);
...
ReplaceConcatReduceByMinOrMax::ReplaceConcatReduceByMinOrMax() {
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](pattern::Matcher& m) {
        ...
        switch (reduce_type) {
        case ReduceType::MAX:
            result_node = register_new_node<v1::Maximum>(concat->input_value(0), concat->input_value(1));
            break;
        case ReduceType::MIN:
            result_node = register_new_node<v1::Minimum>(concat->input_value(0), concat->input_value(1));
            break;
        default:
            return false;
        }
