...
std::shared_ptr<ov::pass::pattern::op::Block> mlp3_no_bias_swiglu_block(
    ...
    auto index_add__ScatterElementsUpdate_2 = any_input();
    auto shape_const = wrap_type<v0::Constant>();
    auto expert_id = wrap_type<v0::Constant>();
    auto slice_end_const = wrap_type<v0::Constant>();
    ...
    auto index_add__Convert_1 = wrap_type<v0::Convert>({ListUnpack_Squeeze_0_1}) | ListUnpack_Squeeze_0_1;
    ...
    // Convert is optional - pattern matches both with and without type conversion
    auto index_Convert_4 = wrap_type<v0::Convert>({ListUnpack_Squeeze_1}) | ListUnpack_Squeeze_1;
    auto index_Multiply_1 =
        wrap_type<v1::Multiply>({index_add__Convert_1, index_Split_out_1}, {{"auto_broadcast", "numpy"}});
    auto index_Add_1 = wrap_type<v1::Add>({index_Convert_4, index_Multiply_1}, {{"auto_broadcast", "numpy"}});
...
struct AxisPatterns {
    std::shared_ptr<Node> axis0 = wrap_type<v0::Constant>(ov::pass::pattern::value_matches("0"));
    std::shared_ptr<Node> axis1 = wrap_type<v0::Constant>(ov::pass::pattern::value_matches("1"));
    std::shared_ptr<Node> axis2 = wrap_type<v0::Constant>(ov::pass::pattern::value_matches("2"));
};
...
std::pair<std::shared_ptr<Node>, std::shared_ptr<Node>> create_router_pattern() {
    auto linear_MatMul = any_input();
    auto expert_num = wrap_type<v0::Constant>();
    auto num_topk = wrap_type<v0::Constant>();
    auto one_hot_on = wrap_type<v0::Constant>(ov::pass::pattern::value_matches("1"));
    auto one_hot_off = wrap_type<v0::Constant>(ov::pass::pattern::value_matches("0"));
    auto transpose_perm = wrap_type<v0::Constant>(ov::pass::pattern::value_matches("2, 1, 0"));

    auto softmax = wrap_type<v8::Softmax>({linear_MatMul}, {{"axis", 1}});
    auto topk = wrap_type<ov::op::v11::TopK>(
        {softmax, num_topk},
        {{"axis", -1}, {"mode", "max"}, {"sort", "value"}, {"index_element_type", "i64"}, {"stable", false}});
    topk->set_output_size(2);
    auto one_hot = wrap_type<v1::OneHot>({topk->output(1), expert_num, one_hot_on, one_hot_off}, {{"axis", 2}});
    auto permute = wrap_type<v1::Transpose>({one_hot, transpose_perm});
...
std::shared_ptr<Node> create_expert_indexing_pattern(const std::shared_ptr<Node>& permute_transpose,
    ...
    auto split = wrap_type<v1::Split>({non_zero, axes.axis0}, {{"num_splits", 2}});
    split->set_output_size(2);
    auto squeeze_indices = wrap_type<v0::Squeeze>({split->output(1), axes.axis0});
    // Convert is optional - pattern matches both with and without type conversion
    auto convert = wrap_type<v0::Convert>({squeeze_indices}) | squeeze_indices;
    return convert;
}
...
ov::pass::FuseMOEExperts::FuseMOEExperts() : MultiMatcher("FuseMOEExperts") {
    ...
    auto view_Reshape = any_input();
    auto slice_end = wrap_type<v0::Constant>();  // Used in callback for slice validation

    // Pattern for hidden states preparation
    auto unsqueeze_Unsqueeze = wrap_type<v0::Unsqueeze>({view_Reshape, axes.axis0});
    ...
    auto original_shape = any_input();
    auto last_reshape = wrap_type<v1::Reshape>({expert_scatter, original_shape}, {{"special_zero", false}});
    auto residual_input = any_input();
    auto last_add = wrap_type<v1::Add>({residual_input, last_reshape}, {{"auto_broadcast", "numpy"}});
