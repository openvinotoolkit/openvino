...
ngraph::pass::ConvToBinaryConv::ConvToBinaryConv() {
    ...
    auto fq_pattern = ngraph::pattern::wrap_type<opset5::FakeQuantize>({ngraph::pattern::any_input(),
    ...
    auto conv_pattern =
        ngraph::pattern::wrap_type<opset5::Convolution>({fq_pattern, ngraph::pattern::wrap_type<opset5::Constant>()});
    ...
    ngraph::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        auto output_low_constant =
            std::dynamic_pointer_cast<opset5::Constant>(fq->input_value(3).get_node_shared_ptr());
        ...
        auto output_high_constant =
            std::dynamic_pointer_cast<opset5::Constant>(fq->input_value(4).get_node_shared_ptr());
        ...
        auto bin_weights_constant =
            std::make_shared<opset5::Constant>(element::u1, weights_constant->get_shape(), bin_weights.data());

        if (output_low_is_zero && output_high_is_one) {
            auto new_conv = std::make_shared<opset5::BinaryConvolution>(
                conv->input_value(0),
            ...
            auto weights_reduced = std::make_shared<opset5::ReduceSum>(
                ...
                op::Constant::create(element::i64, Shape{axes.size()}, axes),
                false);
            std::shared_ptr<Node> weights_reduced_reshaped = std::make_shared<opset5::Reshape>(
                weights_reduced,
                op::Constant::create(element::i64, Shape{weights_reduced_shape.size()}, weights_reduced_shape),
                false);
        ...
        auto new_conv =
            std::make_shared<opset5::BinaryConvolution>(conv->input_value(0),
                                                        bin_weights_constant,
                                                        conv->get_strides(),
                                                        conv->get_pads_begin(),
                                                        conv->get_pads_end(),
