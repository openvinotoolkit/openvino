...
EliminateDuplicateTIInputs::EliminateDuplicateTIInputs() {
    ...
    ov::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        std::vector<std::shared_ptr<op_util::SubGraphOp::InputDescription>> should_stay;
        std::map<std::shared_ptr<op_util::SubGraphOp::InputDescription>,
                 std::vector<std::shared_ptr<op_util::SubGraphOp::InputDescription>>>
            need_to_eliminate;
        auto input_descs = ti->get_input_descriptions();
        for (auto& key : input_descs) {
            auto is_equal = [&](const std::shared_ptr<op_util::SubGraphOp::InputDescription>& input) -> bool {
                if (ti->input_value(input->m_input_index) == ti->input_value(key->m_input_index)) {
                    auto invariant_l = ov::as_type_ptr<op_util::SubGraphOp::InvariantInputDescription>(input);
                    auto invariant_r = ov::as_type_ptr<op_util::SubGraphOp::InvariantInputDescription>(key);
                    if (invariant_l && invariant_r) {
                        return true;
                    }

                    auto slice_l = ov::as_type_ptr<op_util::SubGraphOp::SliceInputDescription>(input);
                    auto slice_r = ov::as_type_ptr<op_util::SubGraphOp::SliceInputDescription>(key);
                    ...
                    auto merged_l = ov::as_type_ptr<op_util::SubGraphOp::MergedInputDescription>(input);
                    auto merged_r = ov::as_type_ptr<op_util::SubGraphOp::MergedInputDescription>(key);
        ...
        // Create new TI
        auto new_ti = std::make_shared<v0::TensorIterator>();
        ...
        for (const auto& remain : should_stay) {
            ...
            if (auto invariant = ov::as_type_ptr<op_util::SubGraphOp::InvariantInputDescription>(remain)) {
                new_ti->set_invariant_input(par, in);
            } else if (auto merged = ov::as_type_ptr<op_util::SubGraphOp::MergedInputDescription>(remain)) {
                auto results = body->get_results();
                new_ti->set_merged_input(par, in, results[merged->m_body_value_index]);
            } else if (auto slice = ov::as_type_ptr<op_util::SubGraphOp::SliceInputDescription>(remain)) {
