...
ov::pass::PullTransposeThroughFQUp::PullTransposeThroughFQUp() {
    ...
    ov::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        auto are_inputs_scalars = shape_size(fq->input_value(1).get_partial_shape().to_shape()) == 1 &&
                                  shape_size(fq->input_value(2).get_partial_shape().to_shape()) == 1 &&
                                  shape_size(fq->input_value(3).get_partial_shape().to_shape()) == 1 &&
                                  shape_size(fq->input_value(4).get_partial_shape().to_shape()) == 1;
