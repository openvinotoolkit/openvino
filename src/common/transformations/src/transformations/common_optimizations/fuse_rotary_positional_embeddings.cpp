...
ov::pass::RoPEFusionFlux::RoPEFusionFlux() {
    ...
    auto x1 = makePattern<op::v1::Reshape>({x, x1_target_shape}, {{"special_zero", true}});
    auto split = makePattern<op::v1::Split>({x1, -1}, {{"num_splits", 2}});
    split->set_output_size(2);

    // 3 versions of mulitply by -1 depending on transformations execution prior to this pass
    auto x1_1_neg_1 = makePattern<op::v1::Multiply>({split->output(1), -1.0f}, {{"auto_broadcast", "numpy"}});

    auto squeeze_2 = makePattern<op::v0::Squeeze>({split->output(1), -1});
    auto x1_1_neg_2 = makePattern<op::v1::Multiply>({squeeze_2, -1.0f}, {{"auto_broadcast", "numpy"}});
    auto unsqueeze_2 = makePattern<op::v0::Unsqueeze>({x1_1_neg_2, -1});

    auto x1_1_neg_3 = makePattern<op::v1::Multiply>({split->output(1), -1.0f}, {{"auto_broadcast", "numpy"}});
    auto squeeze_3 = makePattern<op::v0::Squeeze>({x1_1_neg_3, -1});
    auto unsqueeze_3 = makePattern<op::v0::Unsqueeze>({squeeze_3, -1});

    auto x2 = makePattern<op::v0::Concat>({x1_1_neg_1 | unsqueeze_2 | unsqueeze_3, split->output(0)}, {{"axis", -1}});
    auto x3_target_shape = makeConst({0, num_heads, 0, head_size});
    auto x3 = makePattern<op::v1::Reshape>({x2, x3_target_shape}, {{"special_zero", true}});

    auto y1 = makePattern<op::v1::Multiply>({x, t_cos}, {{"auto_broadcast", "numpy"}});
    auto y2 = makePattern<op::v1::Multiply>({x3, t_sin}, {{"auto_broadcast", "numpy"}});

    auto y = makePattern<op::v1::Add>({y1, y2}, {{"auto_broadcast", "numpy"}});
    auto result = y;
...
ov::pass::RoPEFusionGPTNEOX::RoPEFusionGPTNEOX() {
    ...
    auto varsplit = makePattern<op::v1::VariadicSplit>({x, 3, {half_ndims, ov::gen_pattern::Symbol("end")}});
    ...
    auto x2neg = makePattern<op::v1::Multiply>({x2 | varsplit->output(1), -1.0f}, {{"auto_broadcast", "numpy"}});
    auto x1 = GenSlice(x, 0, half_ndims, 1, 3);
    auto x_rotate_half = makePattern<op::v0::Concat>({x2neg, x1 | varsplit->output(0)}, {{"axis", -1}});

    auto mul_cos = makePattern<op::v1::Multiply>({x_or_cos1, x_or_cos2}, {{"auto_broadcast", "numpy"}});
    auto mul_sin = makePattern<op::v1::Multiply>({x_rotate_half, t_sin}, {{"auto_broadcast", "numpy"}});

    // [x1, x2]*cos + [-x2, x1]*sin
    auto result = makePattern<op::v1::Add>({mul_cos, mul_sin}, {{"auto_broadcast", "numpy"}});
...
ov::pass::RoPEFusionCosSinPreprocess::RoPEFusionCosSinPreprocess() {
    ...
    auto cos_const = makePattern<op::v0::Constant>({});  // "f32[1,1,2048,24]"
    auto sin_const = makePattern<op::v0::Constant>({});  // "f32[1,1,2048,24]"
    ...
    auto prepare_cos_sin_gptneox = [&](std::shared_ptr<Node> const_tab) {
        auto slice = makePattern<ov::op::v8::Slice>({const_tab, {0}, node_batch_size, {1}, {0}});
        auto strided_slice = GenStridedSlice(const_tab, {0}, node_batch_size, {1}, 0);
        return makePattern<op::v6::GatherElements>({strided_slice | slice, gather_positions}, {{"axis", 2}});
    };
    ...
    auto prepare_cos_sin_llama = [&](std::shared_ptr<Node> const_tab) {
        auto ScatterUpdate = makePattern<op::v3::ScatterUpdate>({{0, 0, 0}, 2, seq_len, 0});
        auto slice_Slice = makePattern<ov::op::v8::Slice>({const_tab, {0}, seq_len, {1}, {2}});
        auto slice_StridedSlice = GenStridedSlice(const_tab, {0, 0, 0}, ScatterUpdate, {1, 1, 1}, 2);
        auto squeeze = makePattern<op::v1::Reshape>({slice_StridedSlice | slice_Slice, {-1, head_dims}});
        auto index_Gather = makePattern<op::v8::Gather>({squeeze, gather_positions_2d, 0}, {{"batch_dims", 0}});

        // another simplified pattern for gathering at position_ids
        auto slice_Slice2 = makePattern<ov::op::v8::Slice>({const_tab, {0}, seq_len, {1}, {0}});
        auto slice_StridedSlice2 = GenStridedSlice(const_tab, {0}, seq_len, {1}, 0);
        auto index_Gather2 = makePattern<op::v8::Gather>({slice_Slice2 | slice_StridedSlice2, gather_positions_2d, 0},
                                                         {{"batch_dims", 0}});

        auto unsqueeze = makePattern<op::v1::Reshape>({index_Gather | index_Gather2, {1, 1, -1, head_dims}});
        auto unsqueeze2 = makePattern<op::v0::Unsqueeze>({index_Gather2, 1});
...
ov::pass::RoPEFusionIOSlicing::RoPEFusionIOSlicing() {
    ...
    auto varsplit = makePattern<op::v1::VariadicSplit>({data, 3, {ndims, ov::gen_pattern::Symbol("end")}});
    varsplit->set_output_size(2);
    ...
    auto result = makePattern<op::v0::Concat>({x_emb, y | varsplit->output(1)}, {{"axis", -1}});
...
ov::pass::RoPEFusionPreprocess::RoPEFusionPreprocess() {
    ...
    auto x = makePattern<op::v1::Transpose>({input_slice | input_to_trans, {0, 2, 1, 3}});
...
ov::pass::RoPEFusionGPTJ::RoPEFusionGPTJ() {
    ...
    auto varsplit_view_Reshape =
        makePattern<op::v1::VariadicSplit>({view_Reshape, 3, {ndims, ov::gen_pattern::Symbol("end")}});
    varsplit_view_Reshape->set_output_size(2);

    auto gather_sin_cos = makePattern("f32");

    auto varsplit = makePattern<op::v1::VariadicSplit>({gather_sin_cos, -1, {ndims / 2, -1}});
    varsplit->set_output_size(2);
    // Reshape or UnSqueeze should both be support
    auto dim0 = ov::gen_pattern::Symbol("dim0");
    auto dim1 = ov::gen_pattern::Symbol("dim1");
    auto unsqueeze_sin = makePattern<op::v1::Reshape>({varsplit->output(0), {dim0, dim1, 1, 32}}) |
                         makePattern<op::v0::Unsqueeze>({varsplit->output(0), 2});
    auto unsqueeze_cos = makePattern<op::v1::Reshape>({varsplit->output(1), {dim0, dim1, 1, 32}}) |
                         makePattern<op::v0::Unsqueeze>({varsplit->output(1), 2});
    ...
    auto repeat_interleave_sin = makePattern<op::v8::Gather>({unsqueeze_sin, const_idx, 3}, {{"batch_dims", 0}});
    auto repeat_interleave_cos = makePattern<op::v8::Gather>({unsqueeze_cos, const_idx, 3}, {{"batch_dims", 0}});

    // x interleave (-x[:,:,:, 1::2], x[:,:,:, 0::2])
    auto slice_Slice_1174 = GenSlice(slice_Slice_965 | varsplit_view_Reshape->output(0), 1, int32_max, 2, 3);

    auto neg_Multiply_1177 = makePattern<op::v1::Multiply>({slice_Slice_1174, -1.0f}, {{"auto_broadcast", "numpy"}});
    auto Unsqueeze_65524 = makePattern<op::v0::Unsqueeze>({neg_Multiply_1177, -1});
    auto head_num = ov::gen_pattern::Symbol("head_num");
    auto Unsqueeze_28998 =
        makePattern<op::v1::Reshape>({neg_Multiply_1177, {-1, 1, head_num, 32, 1}}, {{"special_zero", false}});

    auto slice_Slice_1168 = GenSlice(slice_Slice_965 | varsplit_view_Reshape->output(0), 0, int32_max, 2, 3);
    auto Unsqueeze_65525 = makePattern<op::v0::Unsqueeze>({slice_Slice_1168, -1});
    auto Unsqueeze_28999 =
        makePattern<op::v1::Reshape>({slice_Slice_1168, {-1, 1, head_num, 32, 1}}, {{"special_zero", false}});
    auto stack_1182 =
        makePattern<op::v0::Concat>({Unsqueeze_28998 | Unsqueeze_65524, Unsqueeze_65525 | Unsqueeze_28999},
                                    {{"axis", -1}});

    auto ShapeOf_169068 = makePattern<op::v0::ShapeOf>({stack_1182});
    auto flatten_Slice_1194 = GenSlice(ShapeOf_169068, 0, 3, 1, 0);
    auto flatten_Concat_1197 = makePattern<op::v0::Concat>({flatten_Slice_1194, {-1}}, {{"axis", 0}});
    // If with special zero, no need to use shapeof to get full shape
    auto flatten_Reshape_1198 = makePattern<op::v1::Reshape>({stack_1182, flatten_Concat_1197});
    auto flatten_Reshape_Zero =
        makePattern<op::v1::Reshape>({stack_1182, ov::pass::pattern::any_input()}, {{"special_zero", true}});

    // x*cos [B,L,H,ndims]
    auto mul_cos =
        makePattern<op::v1::Multiply>({slice_Slice_965 | varsplit_view_Reshape->output(0), repeat_interleave_cos},
                                      {{"auto_broadcast", "numpy"}});
    auto mul_sin = makePattern<op::v1::Multiply>({flatten_Reshape_1198 | flatten_Reshape_Zero, repeat_interleave_sin},
                                                 {{"auto_broadcast", "numpy"}});

    // *cos + *sin
    auto rotary_emb = makePattern<op::v1::Add>({mul_cos, mul_sin}, {{"auto_broadcast", "numpy"}});

    auto slice_Slice_971 = GenSlice(view_Reshape, ndims, int32_max, 1, 3);
    auto cat_Concat_1211 =
        makePattern<op::v0::Concat>({rotary_emb, slice_Slice_971 | varsplit_view_Reshape->output(1)}, {{"axis", -1}});
    ...
    matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        if (rotary_emb_out.get_target_inputs().size() == 2) {
            for (auto& input : rotary_emb_out.get_target_inputs()) {
                if (ov::is_type<op::v0::ShapeOf>(input.get_node())) {
...
ov::pass::RoPEFusionChatGLM::RoPEFusionChatGLM(int split_output_id, const bool support_2d_rope) {
    ...
    auto qkv_proj = makePattern<op::v1::VariadicSplit>({qkv_linear, -1, {total_size_q, total_size_k, total_size_v}});
    qkv_proj->set_output_size(3);
    auto cur_key = makePattern<op::v1::Reshape>({qkv_proj->output(split_output_id), {0, 0, head_cnt, head_size}},
                                                {{"special_zero", true}});
    ...
    if (support_2d_rope) {
        ...
        auto transposed_cur_key =
            makePattern<op::v1::Reshape>({qkv_proj->output(split_output_id), {-1, head_cnt, 1, head_size}},
                                         {{"special_zero", false}});
        // Transpose for SDPA version:
        input_key = makePattern<op::v1::Transpose>({cur_key, {0, 2, 1, 3}}) | transposed_cur_key;
    } else {
    ...
    auto var_split_1 = makePattern<op::v1::VariadicSplit>({input_key, 3, {ndims, ov::gen_pattern::Symbol("end")}});
    ...
    if (support_2d_rope) {
        ...
        reshape_Reshape_453 =
            makePattern<op::v1::Reshape>({slice_Slice_437 | var_split_1->output(0), const_target_shape_1},
                                         {{"special_zero", true}});
    } else {
        auto ListConstruct_452_Concat =
            makePattern<op::v0::Concat>({seq_length, {-1}, {head_cnt}, {ndims / 2}, {2}}, {{"axis", 0}});
        ...
        reshape_Reshape_453 =
            makePattern<op::v1::Reshape>({slice_Slice_437 | var_split_1->output(0),
                                          ListConstruct_452_Concat | const_target_shape_1 | const_target_shape_0});
    }

    auto x_even = makePattern<op::v8::Gather>({reshape_Reshape_453, 0, -1}, {{"batch_dims", 0}});
    auto x_odd = makePattern<op::v8::Gather>({reshape_Reshape_453, 1, -1}, {{"batch_dims", 0}});

    auto var_split_2 = makePattern<op::v1::VariadicSplit>({cos_sin_cache, 0, {0, ov::gen_pattern::Symbol("end")}});
    var_split_2->set_output_size(2);

    std::shared_ptr<ov::Node> view_Reshape_460 = nullptr;
    if (support_2d_rope) {
        auto ListConstruct_379_Concat =
            makePattern<op::v0::Concat>({{-1}, {1}, seq_length, {ndims / 2}, {2}}, {{"axis", 0}});
        auto const_target_shape_2 = makeConst({batch, 1, seq_len, ndims / 2, 2});

        // Slice cos_sin_cache to support 2-dimentional RoPE
        auto ScatterUpdate = makePattern<op::v3::ScatterUpdate>({{0, 0}, {1}, seq_length, {0}}, {});
        auto slice_Slice_449_1d = makePattern<ov::op::v8::Slice>({cos_sin_cache, {0}, seq_length, {1}, {1}});
        auto slice_Slice_449_2d = makePattern<ov::op::v8::Slice>({cos_sin_cache, {0, 0}, ScatterUpdate, {1, 1}, {0}});
        auto ss_stop = makePattern<op::v0::Constant>({}, {});
        ...
        view_Reshape_460 = makePattern<op::v1::Reshape>(
        ...
        auto ListConstruct_379_Concat =
            makePattern<op::v0::Concat>({seq_length, {-1}, {1}, {ndims / 2}, {2}}, {{"axis", 0}});
        auto const_target_shape_0 = makeConst({1, -1, 1, ndims / 2, 2});
        auto const_target_shape_2 = makeConst({seq_len, batch, 1, ndims / 2, 2});

        auto slice_Slice_449 = makePattern<ov::op::v8::Slice>({cos_sin_cache, {0}, seq_length, {1}, {0}});
        auto slice_StridedSlice_449 = GenStridedSlice(cos_sin_cache, {0}, seq_length, {1}, 0);

        // [seq_length, 1, batch, half_rotary_dims, 2]
        view_Reshape_460 =
            makePattern<op::v1::Reshape>({slice_StridedSlice_449 | slice_Slice_449 | var_split_2->output(0),
                                          ListConstruct_379_Concat | const_target_shape_0 | const_target_shape_2});
    }

    auto cos_tab = makePattern<op::v8::Gather>({view_Reshape_460, 0, -1}, {{"batch_dims", 0}});
    auto x_even_cos = makePattern<op::v1::Multiply>({x_even, cos_tab}, {{"auto_broadcast", "numpy"}});

    auto sin_tab = makePattern<op::v8::Gather>({view_Reshape_460, 1, -1}, {{"batch_dims", 0}});
    auto x_odd_sin = makePattern<op::v1::Multiply>({x_odd, sin_tab}, {{"auto_broadcast", "numpy"}});
    auto neg_x_odd_sin = makePattern<op::v1::Multiply>({x_odd_sin, -1.000000f}, {{"auto_broadcast", "numpy"}});
    auto sub_Subtract_469 = makePattern<op::v1::Add>({x_even_cos, neg_x_odd_sin}, {{"auto_broadcast", "numpy"}});
    auto y_even = makePattern<op::v0::Unsqueeze>({sub_Subtract_469, -1}) |
                  makePattern<op::v1::Reshape>({sub_Subtract_469, {A, B, C, ndims / 2, 1}}, {{"special_zero", false}});
    auto x_odd_cos = makePattern<op::v1::Multiply>({x_odd, cos_tab}, {{"auto_broadcast", "numpy"}});
    auto x_even_sin = makePattern<op::v1::Multiply>({x_even, sin_tab}, {{"auto_broadcast", "numpy"}});
    auto add_Add_476 = makePattern<op::v1::Add>({x_odd_cos, x_even_sin}, {{"auto_broadcast", "numpy"}});
    auto y_odd = makePattern<op::v0::Unsqueeze>({add_Add_476, -1}) |
                 makePattern<op::v1::Reshape>({add_Add_476, {A, B, C, ndims / 2, 1}}, {{"special_zero", false}});

    auto stack_481 = makePattern<op::v0::Concat>({y_even, y_odd}, {{"axis", -1}});

    auto ShapeOf_135133 = makePattern<op::v0::ShapeOf>({stack_481});
    auto flatten_Slice_497 = GenSlice(ShapeOf_135133, 0, 3, 1, 0);
    auto flatten_Concat_500 = makePattern<op::v0::Concat>({flatten_Slice_497, {-1}}, {{"axis", 0}});
    ...
    if (support_2d_rope) {
        ...
        flatten_Reshape_501 = makePattern<op::v1::Reshape>({stack_481, flatten_Concat_500 | const_target_shape_3},
                                                           {{"special_zero", true}});
    } else {
        ...
        flatten_Reshape_501 =
            makePattern<op::v1::Reshape>({stack_481, flatten_Concat_500 | const_target_shape_3 | const_target_shape_0},
                                         {{"special_zero", true}});
    }
    auto slice_Slice_443 = GenSlice(input_key, ndims, INT_MAX, 1, 3);

    auto cat_Concat_505 =
        makePattern<op::v0::Concat>({flatten_Reshape_501, slice_Slice_443 | var_split_1->output(1)}, {{"axis", -1}});
    ...
    matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        if (ov::is_type<op::v1::Reshape>(root)) {
...
ov::pass::RoPEFusionQwen::RoPEFusionQwen(int split_output_id) {
    ...
    auto ListUnpack_410_VariadicSplit =
        makePattern<op::v1::VariadicSplit>({qkv_proj, 2, {head_cnt * head_size, head_cnt * head_size, -1}});
    ListUnpack_410_VariadicSplit->set_output_size(3);
    // B,L,H,S
    auto view_Reshape_424 = makePattern<op::v1::Reshape>(
    ...
    auto ShapeOf_485735 = makePattern<op::v0::ShapeOf>({hidden_states}, {});
    auto Multiply_567524 = makePattern<op::v1::Multiply>({ShapeOf_485735, {-1}}, {{"auto_broadcast", "numpy"}});
    auto Gather_377635 = makePattern<op::v8::Gather>({Multiply_567524, {1}, 0}, {{"batch_dims", 0}});

    auto input_ids = makePattern();  // [batch, length]
    auto ShapeOf_409241 = makePattern<ov::op::util::ShapeOfBase>({input_ids}, {});
    auto Gather_311651 = makePattern<op::v8::Gather>({ShapeOf_409241, {1}, 0}, {{"batch_dims", 0}});
    auto neg_Multiply = makePattern<op::v1::Multiply>({Gather_311651, {-1}}, {{"auto_broadcast", "numpy"}});

    auto ScatterUpdate_463814 = makePattern<op::v3::ScatterUpdate>({{0, 0}, {1}, Gather_377635 | neg_Multiply, {0}});
    auto slice_Slice_446 =
        makePattern<ov::op::v8::Slice>({rotary_emb_cos, Gather_377635 | neg_Multiply, {INT_MAX}, {1}, {1}});

    auto gather_cos_by_pos_ids = makePattern<op::v8::Gather>({rotary_emb_cos, position_ids, 1}, {{"batch_dims", 0}});
    auto reshape_cos_to_expected_layout =
        makePattern<op::v1::Reshape>({gather_cos_by_pos_ids, {-1, 1, 1, 128}}, {{"special_zero", false}});
    ...
    auto mul_Multiply_552 = makePattern<op::v1::Multiply>(
    ...
    auto reshape_opt1 = [&](std::shared_ptr<Node> input_BLHS) {
        auto ShapeOf_485814 = makePattern<op::v0::ShapeOf>({input_BLHS}, {});
        auto Gather_377647 = makePattern<op::v8::Gather>({ShapeOf_485814, {1}, 0}, {{"batch_dims", 0}});
        // batch-size, we don't care
        auto Gather_377641 = makePattern("i32[1]");
        auto ListConstruct_581_Concat =
            makePattern<op::v0::Concat>({Gather_377641, Gather_377647, {head_cnt}, {2}, {head_size / 2}},
                                        {{"axis", 0}});
        auto Gather_391791 = makePattern<op::v8::Gather>({ShapeOf_485814, {0, 1}, 0}, {{"batch_dims", 0}});
        auto ListConstruct_522_Concat = makePattern<op::v0::Concat>({Gather_391791, {32}, {2}, {64}}, {{"axis", 0}});

        auto reshape_Reshape_577 =
            makePattern<op::v1::Reshape>({input_BLHS, {-1, 2, head_size / 2}}, {{"special_zero", true}});
        return makePattern<op::v1::Reshape>({reshape_Reshape_577, ListConstruct_581_Concat | ListConstruct_522_Concat},
                                            {{"special_zero", false}});  //  tensor_array<f32[?,?,32,2,64]>
    };

    // If with sepcial_zero, const_shape should be checked later
    auto const_shape = makePattern<op::v0::Constant>({}, {});
    auto reshape_special = makePattern<op::v1::Reshape>({slice_Slice_543, const_shape}, {{"special_zero", true}});

    auto ListUnpack_586_Split =
        makePattern<op::v1::Split>({reshape_opt1(slice_Slice_543) | reshape_special, -2},
                                   {{"num_splits", 2}});  //  tensor_array<f32[?,?,32,1,64] f32[?,?,32,1,64]>
    ListUnpack_586_Split->set_output_size(2);
    auto Multiply_567527 =
        makePattern<op::v1::Multiply>({ListUnpack_586_Split->output(1), -1.000000f},
                                      {{"auto_broadcast", "numpy"}});  //  tensor_array<f32[?,?,32,1,64]>
    auto ListUnpack_586_Squeeze_0 =
        makePattern<op::v0::Squeeze>({Multiply_567527, -2});  //  tensor_array<f32[?,?,32,64]>
    auto ListUnpack_586_Squeeze =
        makePattern<op::v0::Squeeze>({ListUnpack_586_Split->output(0), -2});  //  tensor_array<f32[?,?,32,64]>

    auto ListUnpack_Squeeze_0_1 =
        makePattern<op::v1::Reshape>({Multiply_567527, {-1, 1, 32, 64}}, {{"special_zero", false}});
    auto ListUnpack_Squeeze_1 =
        makePattern<op::v1::Reshape>({ListUnpack_586_Split->output(0), {-1, 1, 32, 64}}, {{"special_zero", false}});

    auto cat_Concat_593 = makePattern<op::v0::Concat>(
    ...
    auto slice_Slice_470 =
        makePattern<op::v8::Slice>({rotary_emb_sin, Gather_377635 | neg_Multiply, {INT_MAX}, {1}, {1}});
    auto gather_sin_by_pos_ids = makePattern<op::v8::Gather>({rotary_emb_sin, position_ids, 1}, {{"batch_dims", 0}});
    auto reshape_sin_to_expected_layout =
        makePattern<op::v1::Reshape>({gather_sin_by_pos_ids, {-1, 1, 1, 128}}, {{"special_zero", false}});
    auto mul_Multiply_594 = makePattern<op::v1::Multiply>(
        {cat_Concat_593, slice_StridedSlice_470 | slice_Slice_470 | reshape_sin_to_expected_layout},
        {{"auto_broadcast", "numpy"}});  //  tensor_array<f32[?,?,32,128]>
    auto add_Add_597 = makePattern<op::v1::Add>({mul_Multiply_552, mul_Multiply_594},
                                                {{"auto_broadcast", "numpy"}});  //  tensor_array<f32[?,?,32,128]>
    ...
    matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        ...
        if (pattern_map.count(reshape_special)) {
            ...
            auto node = ov::as_type_ptr<op::v0::Constant>(reshape_shape.get_node_shared_ptr());
...
ov::pass::RoPEShareCosSin::RoPEShareCosSin() {
    ...
    auto const_inv_freq = makePattern<op::v0::Constant>({}, {});

    auto Constant_58774 = makeConst(element::u8, ov::Shape({}), {0});
    auto Broadcast_58775 = makePattern<op::v1::Broadcast>({{1.000000f}, inputs[0], Constant_58774},
                                                          {{"mode", "numpy"}});  //  tensor_array<f32[?,?,?]>
    auto expand_Broadcast =
        makePattern<op::v1::Multiply>({const_inv_freq, Broadcast_58775},
                                      {{"auto_broadcast", "numpy"}});  //  tensor_array<f32[?,128,?]>
    auto matmul_MatMul =
        makePattern<op::v0::MatMul>({expand_Broadcast, inputs[1]}, {{"transpose_a", false}, {"transpose_b", false}});
    auto transpose_Transpose = makePattern<op::v1::Transpose>({matmul_MatMul, {0, 2, 1}});
    auto cat_Concat = makePattern<op::v0::Concat>({transpose_Transpose, transpose_Transpose}, {{"axis", -1}});
    auto cos_Cos = makePattern<op::v0::Cos>({cat_Concat});
    auto sin_Sin = makePattern<op::v0::Sin>({cat_Concat});
    auto result = makePattern<op::v0::Unsqueeze>({cos_Cos | sin_Sin, 1});
    ...
    matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        auto cur_inv_freq = ov::as_type_ptr<op::v0::Constant>(it->second.get_node_shared_ptr());
        if (!cur_inv_freq) {
            return false;
        }
