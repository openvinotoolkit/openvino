...
ov::pass::RoPEFusionFlux::RoPEFusionFlux() {
    ...
    // y2 = x3 * t_sin
    // y = y1 + y2
    auto x = any_input(rank_equals(4) && ov::pass::pattern::shape_matches("[PRESERVED_DIMS..., head_size]"));
    auto t_cos = any_input(rank_equals(4));
    auto t_sin = any_input(rank_equals(4));

    auto x1 =
        wrap_type<opset1::Reshape>({x, any_input()}, ov::pass::pattern::shape_matches("[PRESERVED_DIMS..., ?, 2]"));
...
ov::pass::RoPEFusionChatGLMHF::RoPEFusionChatGLMHF() {
    ...
    auto const_idx = wrap_type<ov::opset1::Constant>(type_matches(ov::element::i32) && const_idx_predicate);
    auto repeat_interleave_cos = wrap_type<v8::Gather>({cos, const_idx, -1}, {{"batch_dims", 0}});
    auto repeat_interleave_sin = wrap_type<v8::Gather>({sin, const_idx, -1}, {{"batch_dims", 0}});
