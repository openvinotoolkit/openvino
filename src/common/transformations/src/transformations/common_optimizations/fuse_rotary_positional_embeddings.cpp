...
ov::pass::RoPEShareCosSin::RoPEShareCosSin() {
    ...
    matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](pass::pattern::Matcher& m) {
        ...
        auto cur_inv_freq = ov::as_type_ptr<ov::op::v0::Constant>(it->second.get_node_shared_ptr());
        if (!cur_inv_freq) {
            return false;
        }
...
ov::pass::RoPEFusionGPTOSS::RoPEFusionGPTOSS() {
    ...
    auto vsplit_out0 = pattern::wrap_type<v1::VariadicSplit>(
        {x, -1, {"half_ndims", "?"}},
        pattern::output_index_matches(0) && pattern::shape_matches("[?, ?, ?, half_ndims]"));
    auto vsplit_out1 = pattern::wrap_type<v1::VariadicSplit>(
        {x, -1, {"half_ndims", "?"}},
