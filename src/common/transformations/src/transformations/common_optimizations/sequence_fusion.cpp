...
bool is_equal_consts(const shared_ptr<ov::Node>& l, const shared_ptr<ov::Node>& r) {
    auto l_const = ov::as_type_ptr<v0::Constant>(l);
    auto r_const = ov::as_type_ptr<v0::Constant>(r);
    if (l_const && r_const) {
...
bool check_WRB(const shared_ptr<op_util::RNNCellBase>& cell_1, const shared_ptr<op_util::RNNCellBase>& cell_2) {
    ...
    auto lstm_cell_v4_1 = ov::as_type_ptr<v4::LSTMCell>(cell_1);
    auto lstm_cell_v4_2 = ov::as_type_ptr<v4::LSTMCell>(cell_2);
    // 2nd input is Cell State
    if (lstm_cell_v4_1 && lstm_cell_v4_2) {
        increase_indexes();
    }
    auto lstm_cell_v0_1 = ov::as_type_ptr<v0::LSTMCell>(cell_1);
    auto lstm_cell_v0_2 = ov::as_type_ptr<v0::LSTMCell>(cell_2);
...
bool check_lstm_cell(const shared_ptr<op_util::RNNCellBase>& prev_cell,
                     const shared_ptr<op_util::RNNCellBase>& current_cell) {
    // check intermediate C outputs in case of LSTMCell
    // LSTMCell - C -> LSTMCell
    if ((ov::as_type_ptr<v4::LSTMCell>(prev_cell) || ov::as_type_ptr<v0::LSTMCell>(prev_cell))) {
        const auto& target_inputs = prev_cell->get_output_target_inputs(1);
        bool valid = target_inputs.empty() ||
                     (target_inputs.size() == 1 &&
                      ov::as_type<op_util::RNNCellBase>(target_inputs.begin()->get_node()) == current_cell.get() &&
                      target_inputs.begin()->get_index() == 2);

        // if intermediate C output is connected to other node, except v4::LSTMCell,
...
shared_ptr<op_util::RNNCellBase> find_cell_chain(ov::pass::NodeRegistry& cp_from,
    ...
    while (true) {
        ...
        auto prev_cell = ov::as_type_ptr<op_util::RNNCellBase>(prev);

        auto in_X = current->input(0);
        x_to_concat.push_back(cp_to.make<v0::Unsqueeze>(in_X.get_source_output(), axis_1));
        h_outputs_to_redirect[cells_cnt] = current->output(0);

        if (auto augru = ov::as_type_ptr<ov::op::internal::AUGRUCell>(current)) {
            attention_to_concat.push_back(cp_to.make<v0::Unsqueeze>(augru->input_value(5), axis_1));
        }
...
bool create_sequence(ov::pass::NodeRegistry& cp_to,
                     const shared_ptr<op_util::RNNCellBase>& first_cell,
                     const shared_ptr<op_util::RNNCellBase>& last_cell,
    ...
    // 2nd input is Cell State
    bool is_lstm = false;
    if (ov::as_type_ptr<v4::LSTMCell>(last_cell) || ov::as_type_ptr<v0::LSTMCell>(last_cell)) {
        is_lstm = true;
        idx_B++;
        idx_R++;
        idx_W++;
    ...
    const auto X_in = cp_to.make<v0::Concat>(x_to_concat, 1);
    const auto Ht_in = cp_to.make<v0::Unsqueeze>(first_cell->input_value(1), axis_1);
    const auto W_in = cp_to.make<v0::Unsqueeze>(first_cell->input_value(idx_W), axis_0);
    const auto R_in = cp_to.make<v0::Unsqueeze>(first_cell->input_value(idx_R), axis_0);
    const auto B_in = cp_to.make<v0::Unsqueeze>(first_cell->input_value(idx_B), axis_0);

    const auto& shape_node = cp_to.add(op_util::make_try_fold<v3::ShapeOf>(first_cell->input_value(0)));
    ...
    if (ov::as_type_ptr<v4::LSTMCell>(first_cell)) {
        ...
        if (weights_format != ov::op::LSTMWeightsFormat::FICO) {
            W = op_util::convert_lstm_node_format(W_in, convert_lstm_weights_enums(weights_format));
            R = op_util::convert_lstm_node_format(R_in, convert_lstm_weights_enums(weights_format));
            B = op_util::convert_lstm_node_format(B_in, convert_lstm_weights_enums(weights_format));
        }
        const auto Ct_in = cp_to.make<v0::Unsqueeze>(first_cell->input_value(2), axis_1);
        sequence = cp_to.make<v5::LSTMSequence>(X_in,
                                                Ht_in,
    ...
    if (!h_outputs_to_redirect.empty()) {
        auto squeeze_Y = cp_to.make<v0::Squeeze>(sequence->output(0), axis_1);
        ...
        for (auto it : h_outputs_to_redirect) {
            ...
            auto squeeze = cp_to.make<v0::Squeeze>(Hi, axis_1);
    ...
    if (is_lstm) {
        auto squeeze = cp_to.make<v0::Squeeze>(sequence->output(2), axis_1);
...
ov::pass::SequenceFusion::SequenceFusion() {
    ...
    auto cell = ov::pass::pattern::wrap_type<op_util::RNNCellBase>();
    matcher_pass_callback callback = [=](Matcher& m) {
        ov::pass::NodeRegistry copy_from;
        ov::pass::NodeRegistry copy_to;
        auto cell = m.get_match_root();
        shared_ptr<op_util::RNNCellBase> current_cell = ov::as_type_ptr<op_util::RNNCellBase>(cell);
        if (!current_cell) {
            return false;
        }
        ...
        for (const auto& target : cell->get_output_target_inputs(0)) {
            auto cell_1 = ov::as_type_ptr<op_util::RNNCellBase>(target.get_node()->shared_from_this());
        ...
        ov::OutputVector attention_to_concat;
        map<int, ov::Output<ov::Node>> h_outputs_to_redirect;
        auto axis_0 = copy_to.make<v0::Constant>(i64, Shape{}, 0);
        auto axis_1 = copy_to.make<v0::Constant>(i64, Shape{}, 1);
