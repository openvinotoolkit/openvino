...
ov::pass::ConcatFusion::ConcatFusion() {
    ...
    auto has_same_axis_concat_input = [](const Output<Node>& output) {
        ...
        for (auto input : concat->input_values()) {
            const auto inp_concat = ov::as_type_ptr<ov::op::v0::Concat>(input.get_node_shared_ptr());
    ...
    auto concat_pattern = ov::pass::pattern::wrap_type<ov::op::v0::Concat>(has_same_axis_concat_input);

    ov::matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) {
        const auto& pattern_map = m.get_pattern_map();

        const auto& concat = ov::as_type_ptr<ov::op::v0::Concat>(pattern_map.at(concat_pattern));
        const auto axis = concat->get_axis();
        ...
        auto new_concat = std::make_shared<ov::op::v0::Concat>(new_inputs, axis);
