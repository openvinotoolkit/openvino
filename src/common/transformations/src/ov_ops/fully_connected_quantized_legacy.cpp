...
void FullyConnectedQuantizedLegacy::validate_and_infer_types() {
    ...
    // deq_scales is a mandatory input, so it must carry a concrete floating-point element type; a
    ...
    // scaling, so the type is only required to be numeric. String is the only concrete element type
    // that is not numeric, and an absent zero-points input is an empty element::dynamic constant, so
    // string is the only type rejected.
    ...
    NODE_VALIDATION_CHECK(this,
                          ...
                          "deq_zero_points (input 4) must have a numeric element type. Got: ",
                          zp_et);
