...
FullyConnectedQuantizedLegacy::FullyConnectedQuantizedLegacy(const OutputVector& arguments,
...
std::shared_ptr<ov::Node> FullyConnectedQuantizedLegacy::clone_with_new_inputs(const ov::OutputVector& new_args) const {
    ...
    return std::make_shared<FullyConnectedQuantizedLegacy>(new_args, m_output_type);
}

std::shared_ptr<Node> FullyConnectedQuantizedLegacy::fuse_bias(const ov::Output<Node>& bias) const {
    auto inputs = input_values();
    inputs[2] = bias;

    return std::make_shared<FullyConnectedQuantizedLegacy>(inputs, get_output_type());
}
...
bool FullyConnectedQuantizedLegacy::visit_attributes(ov::AttributeVisitor& visitor) {
