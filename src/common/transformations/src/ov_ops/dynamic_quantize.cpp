...
DynamicQuantize::DynamicQuantize(const Output<Node>& data, std::vector<size_t> group_sizes, element::Type dt_scale)
    ...
    , m_dt_scale(dt_scale) {
    OPENVINO_ASSERT(data.get_partial_shape().rank() == group_sizes.size(), "FC input rank should be same as the rank of group_size ",
                                                                                            data.get_tensor_ptr()->get_partial_shape().rank(), " / ", group_sizes.size());
    set_output_size(2);
...
std::vector<ov::PartialShape> DynamicQuantize::shape_infer(const DynamicQuantize* op, std::vector<ov::PartialShape> input_shapes, const std::vector<size_t> group_sizes) {
    std::vector<ov::PartialShape> out_shapes;
    out_shapes.push_back(input_shapes[0]);

    auto scale_shape = input_shapes[0];
    OPENVINO_ASSERT(scale_shape.size() == group_sizes.size(), "Scale_shape and group_size are supposed to have same rank: "
                                                             , scale_shape.size()
                                                             , " / "
                                                             , group_sizes.size());
