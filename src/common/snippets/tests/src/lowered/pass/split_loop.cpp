    ...
    */
    {
        ...
        loop_manager->mark_loop(fill.first, h_max.first, 1024, vector_size, 0,
                                std::vector<LoopPort>{LoopPort((*fill.first)->get_input_port(0), true, 0),
                                                      ...
                                                      // mark reduce_max first and set port, then decompose reduce_max, vector_fill port should not added.
...
TEST_F(SplitLoopTest, SplitLoopFlashAttentionTest) {
    ...
    }
    {
        ...
        // block inner loop for dimension 1.
        // there will be buffers inserted as brgemm1 result and brgemm2 input0.
        // below ports will be connected with the block buffers and load/store inserted based on buffer shape.
        loop_manager->mark_loop(vector_buffer_max.first, brgemm2.first, 32, 1, 1,
                                ...
                                std::vector<LoopPort>{LoopPort((*mul_softmax.first)->get_output_port(0), true, 1),
                                                      // inc 1. one row get one scale. scale buffer(32*1) should be inserted.
                                                      LoopPort((*scale.first)->get_output_port(0), true, 1)});
        ...
        size_t block_nk = loop_manager->mark_loop(brgemm1.first, result.first, 1024, 64, 0,
                                std::vector<LoopPort>{LoopPort((*brgemm1.first)->get_input_port(0), false, 0),
                                                      LoopPort((*brgemm1.first)->get_input_port(1), true, 0), // K matrix inc 64(*1) on 1024 dimension
                                                      // three buffers ports are not increased on N/K block loop.
                                                      ...
                                                      LoopPort((*brgemm2.first)->get_input_port(1), true, 0)},  // V matrix inc 64(*16) on 1024 dimension
                                std::vector<LoopPort>{LoopPort((*brgemm2.first)->get_output_port(0), false, 0)});
        size_t block_m = loop_manager->mark_loop(brgemm1.first, result.first, 512, 32, 1,
                                std::vector<LoopPort>{LoopPort((*brgemm1.first)->get_input_port(0), true, 1),
                                                      LoopPort((*brgemm1.first)->get_input_port(1), false, 1),  // K matrix not inc
                                                      // three buffers ports are increased 32 on M block loop
                                                      LoopPort((*max_new.first)->get_input_port(0), true, 1),
                                                      LoopPort((*sub_scale.first)->get_input_port(0), true, 1),
                                                      LoopPort((*mul_scale.first)->get_input_port(0), true, 1),
    ...
    /*
    // after split_loop, not apply inside fuse_loop
    {
        auto param1 = linear_ir_ref->push_node<ov::opset10::Parameter>(input_precision, input_shape1);
        auto param2 = linear_ir_ref->push_node<ov::opset10::Parameter>(input_precision, input_shape2);
        auto param3 = linear_ir_ref->push_node<ov::opset10::Parameter>(input_precision, input_shape3);
        ...
        // three inner dimension block inside loops(wa:64, inc:16).
        size_t column_loop1 = loop_manager->mark_loop(fill_max.first, h_max.first, 64, vector_size, 0,
                                std::vector<LoopPort>{LoopPort((*fill_max.first)->get_input_port(0), true, 0)},
                                std::vector<LoopPort>{LoopPort((*max.first)->get_output_port(0), true, 0)});
        size_t column_loop2 = loop_manager->mark_loop(max_new.first, h_sum.first, 64, vector_size, 0,
                                std::vector<LoopPort>{LoopPort((*max_new.first)->get_input_port(0), false, 0),
                                                      LoopPort((*max_new.first)->get_input_port(1), false, 0),
                                                      LoopPort((*sub_softmax.first)->get_input_port(0), true, 0)},
                                std::vector<LoopPort>{LoopPort((*exp.first)->get_output_port(0), true, 0),
                                                      LoopPort((*add.first)->get_output_port(0), true, 0)});
        size_t column_loop3 = loop_manager->mark_loop(power_static.first, mul_scale.first, 64, vector_size, 0,
                                std::vector<LoopPort>{LoopPort((*power_static.first)->get_input_port(0), false, 0),
                                                      LoopPort((*mul_softmax.first)->get_input_port(0), true, 0)},
                                std::vector<LoopPort>{LoopPort((*mul_softmax.first)->get_output_port(0), true, 0)});

        // one outer dimension block inside loop(wa:32, inc:1).
        loop_manager->mark_loop(vector_buffer_max.first, brgemm2.first, 32, 1, 1,
                                std::vector<LoopPort>{LoopPort((*fill_max.first)->get_input_port(0), true, 1),
                                                      LoopPort((*sub_softmax.first)->get_input_port(0), true, 1),
                                                      // three buffers inc 1(*64) on each row loop
                                                      LoopPort((*sub_scale.first)->get_input_port(0), true, 1),
                                                      LoopPort((*max_new.first)->get_input_port(0), true, 1),
                                                      LoopPort((*mul_scale.first)->get_input_port(0), true, 1)},
                                                      // inc 1(*64). store to buffer, inc based on buffer shape.
                                std::vector<LoopPort>{LoopPort((*mul_softmax.first)->get_output_port(0), true, 1),
                                                      // inc 1*(1). one row get one scale. scale buffer(32*1) should be inserted.
                                                      LoopPort((*scale.first)->get_output_port(0), true, 1)});

        // three inner dimension block loops(wa:1024, inc:64). these three will be fused, and finally fused with N in brgemm1 and K in brgemm2
        // parent extend to vector_buffer_max. child extend to mul_scale(vector_buffer_sum-loop_end)
        size_t column_loop1 = loop_manager->mark_loop(vector_buffer_max.first, vector_buffer_sum.first, 64, vector_size, 0,
                                std::vector<LoopPort>{LoopPort((*fill_max.first)->get_input_port(0), true, 0),
                                                      LoopPort((*mul_scale.first)->get_input_port(0), false, 0)},  // add
                                std::vector<LoopPort>{LoopPort((*h_max.first)->get_output_port(0), false, 0)});    // update
        // parent extend to vector_buffer_sum. child extend to scale(loop_end is power_static).
        size_t column_loop2 = loop_manager->mark_loop(vector_buffer_sum.first, power_static.first, 64, vector_size, 0,
                                std::vector<LoopPort>{LoopPort((*max_new.first)->get_input_port(0), false, 0),
                                                      LoopPort((*max_new.first)->get_input_port(1), false, 0),
                                                      LoopPort((*sub_softmax.first)->get_input_port(0), true, 0),
                                                      LoopPort((*scale.first)->get_input_port(0), false, 0)},  // add
                                std::vector<LoopPort>{LoopPort((*exp.first)->get_output_port(0), true, 0),
                                                      // update from (*add.first)->get_output_port(0) to h_sum
                                                      LoopPort((*h_sum.first)->get_output_port(0), false, 0),
                                                      LoopPort((*scale.first)->get_output_port(0), false, 0)}); // add
        size_t column_loop3 = loop_manager->mark_loop(power_static.first, mul_scale.first, 64, vector_size, 0,
                                std::vector<LoopPort>{LoopPort((*power_static.first)->get_input_port(0), false, 0),
                                                      LoopPort((*mul_softmax.first)->get_input_port(0), true, 0)},
                                std::vector<LoopPort>{LoopPort((*mul_softmax.first)->get_output_port(0), true, 0)});

        // one outer dimension block loops(wa:512, inc:32). fused to M block in brgemm1 and brgemm2
        loop_manager->mark_loop(vector_buffer_max.first, brgemm2.first, 512, 32, 1,
                                std::vector<LoopPort>{LoopPort((*fill_max.first)->get_input_port(0), true, 1),
                                                      LoopPort((*sub_softmax.first)->get_input_port(0), true, 1),
                                                      // three buffers inc 32(*1)
                                                      LoopPort((*sub_scale.first)->get_input_port(0), true, 1),
                                                      LoopPort((*max_new.first)->get_input_port(0), true, 1),
                                                      LoopPort((*mul_scale.first)->get_input_port(0), true, 1)},
                                                      // 32(*64) buffer inserted and reused
                                std::vector<LoopPort>{LoopPort((*mul_softmax.first)->get_output_port(0), false, 1),
                                                      // 32(*1) buffer inserted and reused
                                                      LoopPort((*scale.first)->get_output_port(0), false, 1)});
