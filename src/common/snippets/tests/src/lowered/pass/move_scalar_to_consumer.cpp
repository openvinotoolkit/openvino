...
TEST_F(LoweredPassTestsF, MoveScalarToConsumer) {
    ...
    const ov::Shape scalar_shape{1};
    {
        ...
        auto result = linear_ir->push_node<ov::snippets::op::Result>(OutputVector{add2.second});
    }
    pipeline.register_pass<MoveScalarToConsumer>();
    {
        ...
        auto result = linear_ir_ref->push_node<ov::snippets::op::Result>(OutputVector{add2.second});
    }
}
}  // namespace snippets
}  // namespace test
