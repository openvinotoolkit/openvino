...
std::string BufferAllocationTest::getTestCaseName(
    testing::TestParamInfo<ov::test::snippets::BufferAllocationParams> obj) {
...
void BufferAllocationTest::ApplyTransformations(
    const std::shared_ptr<ov::snippets::lowered::pass::PassConfig>& pass_config) {
...
std::shared_ptr<ov::Model> EltwiseBufferAllocationTest::GetModel() const {
    ...
    const auto subtensor_buffer =
        std::vector<size_t>{ov::snippets::lowered::PortDescriptor::ServiceDimensions::FULL_DIM,
                            ov::snippets::lowered::PortDescriptor::ServiceDimensions::FULL_DIM};

    const auto parameter0 =
        std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::PartialShape({1, 3, 100, 100}));
    const auto parameter1 =
        std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::PartialShape({1, 3, 100, 100}));
    const auto add = std::make_shared<ov::op::v1::Add>(parameter0, parameter1);
    const auto buffer0 =
        std::make_shared<ov::snippets::op::IntermediateMemoryBuffer>(add,
                                                                     static_cast<int32_t>(subtensor_buffer.size()));
    const auto relu = std::make_shared<ov::op::v0::Relu>(buffer0);
    const auto buffer1 =
        std::make_shared<ov::snippets::op::IntermediateMemoryBuffer>(relu,
                                                                     static_cast<int32_t>(subtensor_buffer.size()));
    const auto exp = std::make_shared<ov::op::v0::Exp>(buffer1);
    const auto body = std::make_shared<ov::Model>(std::make_shared<ov::op::v0::Result>(exp),
...
void MHABufferAllocationTest::MarkBrgemm(const std::shared_ptr<ov::snippets::op::Brgemm>& node,
                                         const std::vector<size_t>& subtensor) {
    const auto subtensor_full = std::vector<size_t>{ov::snippets::lowered::PortDescriptor::ServiceDimensions::FULL_DIM,
                                                    ov::snippets::lowered::PortDescriptor::ServiceDimensions::FULL_DIM};
    ov::snippets::lowered::PortDescriptorUtils::set_port_descriptor_ptr(
        node->input(0),
        std::make_shared<ov::snippets::lowered::PortDescriptor>(node->input(0), subtensor));
    ov::snippets::lowered::PortDescriptorUtils::set_port_descriptor_ptr(
        node->input(1),
        std::make_shared<ov::snippets::lowered::PortDescriptor>(node->input(1), subtensor_full));
    ov::snippets::lowered::PortDescriptorUtils::set_port_descriptor_ptr(
        node->output(0),
        std::make_shared<ov::snippets::lowered::PortDescriptor>(node->output(0), subtensor));
}

std::shared_ptr<ov::Model> MHABufferAllocationTest::GetModel() const {
    const auto subtensor_scalar = std::vector<size_t>{1};
    const auto subtensor_eltwise = std::vector<size_t>{1, m_vector_size};
    const auto subtensor_brgemm =
        std::vector<size_t>{32, ov::snippets::lowered::PortDescriptor::ServiceDimensions::FULL_DIM};
    const auto subtensor_power =
        std::vector<size_t>{1, ov::snippets::lowered::PortDescriptor::ServiceDimensions::FULL_DIM};

    const auto parameter0 =
        std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::PartialShape({1, 12, 128, 64}));
    const auto parameter1 =
        std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::PartialShape({1, 128, 12, 64}));
...
INSTANTIATE_TEST_SUITE_P(smoke_Snippets_BufferAllocation_EltwiseNotOptimized,
                         EltwiseBufferAllocationTest,
...
INSTANTIATE_TEST_SUITE_P(smoke_Snippets_BufferAllocation_EltwiseOptimized,
                         EltwiseBufferAllocationTest,
...
INSTANTIATE_TEST_SUITE_P(
    smoke_Snippets_BufferAllocation_MHAOptimizedWSplit,
    MHABufferAllocationTest,
    ::testing::Combine(::testing::Values(true),
                       ::testing::Values(true),
...
INSTANTIATE_TEST_SUITE_P(
    smoke_Snippets_BufferAllocation_MHAOptimizedWOSplit,
    MHABufferAllocationTest,
    ::testing::Combine(::testing::Values(true),
                       ::testing::Values(false),
