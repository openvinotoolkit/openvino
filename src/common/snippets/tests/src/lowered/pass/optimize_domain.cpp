...
TEST_P(OptimizeDomainTest, DomainOptimization) {
    ...
    auto linear_ir = subgraph->convert_body_to_linear_ir(m_domain_opt_params.min_parallel_work_amount,
                                                         m_domain_opt_params.min_jit_work_amount);
    size_t loop_depth = 1;
    ...
    EXPECT_THAT(master_shape, testing::ContainerEq(m_domain_opt_params.exp_master_shape))
        << "Inconsistent master_shape detected";
}
...
std::vector<OptimizeDomainParams> dopt_params = {
    ...
    // min_parallel_work_amount = 1 is a special case that would cause all dimensions to collapse (up to
    // min_jit_work_amount of course)
    {256, 1, {{4, 1, 6, 7}, {4, 1, 6, 7}}, {1, 1, 1, 168}, 1},
};

INSTANTIATE_TEST_SUITE_P(smoke_Snippets_DomainOptimization,
                         OptimizeDomainTest,
