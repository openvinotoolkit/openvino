...
TEST_F(LoweredPassTestsF, InsertLoadStore) {
    ...
    const ov::PartialShape input_shape{1, 3, 16, 16};
    {
        ...
        auto result = linear_ir->push_node<ov::snippets::op::Result>(OutputVector{convert.second});
    }
    pipeline.register_pass<InsertLoadStore>(vector_size);
    {
        ...
        auto result = linear_ir_ref->push_node<ov::snippets::op::Result>(OutputVector{store.second});
    }
}
}  // namespace snippets
}  // namespace test
