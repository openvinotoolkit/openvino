...
public:
    DummyPrecisionPropagationTargetMachine(const std::set<std::vector<element::Type>>& op1_supported_precisions,
                                           const std::set<std::vector<element::Type>>& op2_supported_precisions)
        : DummyTargetMachine() {
        jitters[DummyAdd::get_type_info_static()] =
            ov::snippets::jitters_value{[](const ov::snippets::lowered::ExpressionPtr& n) {
                                            return std::make_shared<DummyEmitter>();
                                        },
                                        [op1_supported_precisions](const std::shared_ptr<ov::Node>& n) {
                                            return op1_supported_precisions;
                                        }};
        jitters[op::v1::Maximum::get_type_info_static()] =
            ov::snippets::jitters_value{[](const ov::snippets::lowered::ExpressionPtr& n) {
                                            return std::make_shared<DummyEmitter>();
                                        },
                                        [op2_supported_precisions](const std::shared_ptr<ov::Node>& n) {
                                            return op2_supported_precisions;
                                        }};

        auto default_jitter = ov::snippets::jitters_value{[](const ov::snippets::lowered::ExpressionPtr& n) {
                                                              return std::make_shared<DummyEmitter>();
                                                          },
...
std::string PrecisionPropagationTest::getTestCaseName(testing::TestParamInfo<PrecisionPropagationParams> obj) {
    ...
    std::ostringstream result;
    result << "IN0_" << shapes.first << "_" << test_values.input_types[0] << "_" << "IN1_" << shapes.second << "_"
           << test_values.input_types[1] << "_" << "IN2_" << test_values.input_types[2]
           << to_string(test_values.actual.op1_supported_precisions) << "_"
           << to_string(test_values.actual.op2_supported_precisions) << "_"
           << test_values.expected.convertion_before_op1.first << "_"
           << test_values.expected.convertion_before_op1.second << "_" << test_values.expected.convertion_before_op2_1
           << "_" << test_values.expected.convertion_before_op2_2.first << "_"
           << test_values.expected.convertion_before_op2_2.second << "_" << test_values.expected.convertion_after_op2
           << "_";
    return result.str();
}
...
TEST_P(PrecisionPropagationTest, CompareFunctions) {
    ...
    PrecisionPropagationAddFunction function_stub(input_shapes,
    ...
    const auto target_machine =
        std::make_shared<DummyPrecisionPropagationTargetMachine>(test_values.actual.op1_supported_precisions,
