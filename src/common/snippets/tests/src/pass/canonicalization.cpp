...
void normalizeParameter(const std::shared_ptr<ov::opset1::Parameter>& par, size_t num_prepend, size_t num_append) {
    ...
    auto rank_norm = std::make_shared<ov::snippets::op::RankNormalization>(par, num_prepend, num_append, 0);
...
void normalizeResult(const std::shared_ptr<ov::opset1::Result>& res, size_t num_pop) {
    auto rank_norm = std::make_shared<ov::snippets::op::RankNormalization>(res->input_value(0), 0, 0, num_pop);
