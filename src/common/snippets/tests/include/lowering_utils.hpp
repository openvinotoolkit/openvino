...
public:
    ...
    ov::snippets::CompiledSnippetPtr get_snippet() override {
        return std::make_shared<DummyCompiledSnippet>();
    }
    size_t get_lanes() const override {
        return 10;
    }
    std::shared_ptr<TargetMachine> clone() const override {
        return std::make_shared<DummyTargetMachine>();
    }
};
...
public:
    ...
    std::shared_ptr<Generator> clone() const override {
        return std::make_shared<DummyGenerator>(target);
    }

protected:
    ov::snippets::RegType get_op_out_reg_type(const ov::Output<ov::Node>& out) const override {
...
public:
    ...
    static std::shared_ptr<ov::snippets::op::Subgraph> getLoweredSubgraph(
        ...
        size_t min_kernel_work_amount = 256,
