    ...
    return ov::pass::pattern::op::Predicate(
        [=](const ov::Output<ov::Node>& output) -> bool {
            ...
            return ov::is_type<ov::op::v0::MatMul>(node) &&
                   ((!is_down && ov::pass::pattern::consumers_count(1)(output)) || is_down) &&
                   (ov::snippets::op::Brgemm::get_output_type(node->get_input_element_type(0),
                                                              node->get_input_element_type(1)) != ov::element::dynamic);
        },
        "fc_predicate");
}
...
TokenizeGatedMLPSnippets::TokenizeGatedMLPSnippets(const TokenizationConfig& config) {
    ...
    auto m_input = ov::pass::pattern::any_input(ov::pass::pattern::has_static_rank());
    auto m_fc_gate = ov::pass::pattern::wrap_type<ov::op::v0::MatMul>({m_input, make_weights()}, fc_predicate(false));
    auto m_fc_up = ov::pass::pattern::wrap_type<ov::op::v0::MatMul>({m_input, make_weights()}, fc_predicate(false));

    auto m_act_unary = ov::pass::pattern::wrap_type<ov::op::util::UnaryElementwiseArithmetic,
                                                    ov::op::v0::HardSigmoid,
                                                    ov::op::v4::Swish>({m_fc_gate}, act_predicate());
    auto m_act_binary_scalar = ov::pass::pattern::wrap_type<ov::op::v0::Constant>(scalar_predicate());
    auto m_act_binary =
        ov::pass::pattern::wrap_type<ov::op::util::BinaryElementwiseArithmetic, ov::op::v0::PRelu, ov::op::v4::Swish>(
            {m_fc_gate, m_act_binary_scalar},
            act_predicate());
    auto m_act = std::make_shared<ov::pass::pattern::op::Or>(ov::OutputVector{m_act_unary, m_act_binary});

    auto m_mul =
        ov::pass::pattern::wrap_type<ov::op::v1::Multiply>({m_act, m_fc_up}, ov::pass::pattern::consumers_count(1));
    auto m_fc_down = ov::pass::pattern::wrap_type<ov::op::v0::MatMul>({m_mul, make_weights()}, fc_predicate(true));
    ...
    register_matcher(
        ...
        [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
            ...
            const bool is_dynamic =
                std::any_of(ordered_ops.begin(), ordered_ops.end(), [](const std::shared_ptr<ov::Node>& n) {
                    return n->is_dynamic();
                });
