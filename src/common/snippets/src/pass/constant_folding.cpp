...
void constants_to_parameters(
    ...
    //auto convert_to_scalar = [](const std::shared_ptr<opset1::Constant>& constant) {
    //    // ticket #48857
    ...
    //    const auto update_to_scalar = !std::any_of(values.begin(), values.end(), [&](float value) { return values[0] != value; });
    //    if (!update_to_scalar) {
    //        return false;
    //    }

    //    const auto scalar_constant = std::make_shared<opset1::Constant>(constant->get_element_type(), Shape{}, constant->get_data_ptr());
    //    replace_node(constant, scalar_constant);

    //    scalar_constant->set_friendly_name(constant->get_friendly_name());
    //    copy_runtime_info(constant, scalar_constant);
    ...
    for (auto& op : ops) {
        ...
        //if ((constant == nullptr) || convert_to_scalar(constant)) {
