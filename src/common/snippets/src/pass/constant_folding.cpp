...
void constants_to_parameters(
    ...
    for (auto i = 0; i < subgraph->get_input_size(); ++i) {
        auto input = subgraph->get_input_node_shared_ptr(i);
        if (!is_type<opset1::Constant>(input)) {
    ...
    for (auto& op : ops) {
        ...
        if ((constant == nullptr) || op::is_scalar_constant(constant)) {
            continue;
        }
    ...
    replace_node(subgraph, subgraph->clone_with_new_inputs(new_inputs));
}
