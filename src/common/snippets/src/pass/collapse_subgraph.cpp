...
auto is_supported_op(const std::shared_ptr<const Node>& n) -> bool {
    ...
    auto is_supported_binary_eltwise_op = [](const std::shared_ptr<const Node>& n) -> bool {
        return ov::is_type<ov::op::v1::Add>(n) || ov::is_type<ov::op::v1::Divide>(n) ||
               ov::is_type<ov::op::v1::Equal>(n) || ov::is_type<ov::op::v1::FloorMod>(n) ||
               ov::is_type<ov::op::v1::Greater>(n) || ov::is_type<ov::op::v1::GreaterEqual>(n) ||
               ov::is_type<ov::op::v1::Less>(n) || ov::is_type<ov::op::v1::LessEqual>(n) ||
               ov::is_type<ov::op::v1::LogicalAnd>(n) || ov::is_type<ov::op::v1::LogicalOr>(n) ||
               ov::is_type<ov::op::v1::LogicalXor>(n) || ov::is_type<ov::op::v1::Maximum>(n) ||
               ov::is_type<ov::op::v1::Minimum>(n) || ov::is_type<ov::op::v1::Mod>(n) ||
               ov::is_type<ov::op::v1::Multiply>(n) || ov::is_type<ov::op::v1::NotEqual>(n) ||
               ov::is_type<ov::op::v0::PRelu>(n) || ov::is_type<ov::op::v1::Power>(n) ||
               ov::is_type<ov::op::v0::SquaredDifference>(n) || ov::is_type<ov::op::v1::Subtract>(n) ||
               ov::is_type<ov::op::v0::Xor>(n) || ov::is_type<ov::op::v0::Convert>(n);
    };

    auto is_supported_unary_eltwise_op = [](const std::shared_ptr<const Node>& n) -> bool {
        return ov::is_type<ov::op::v0::Abs>(n) || ov::is_type<ov::op::v0::Clamp>(n) ||
               ov::is_type<ov::op::v0::Floor>(n) || ov::is_type<ov::op::v0::Ceiling>(n) ||
               ov::is_type<ov::op::v0::Elu>(n) || ov::is_type<ov::op::v0::Erf>(n) || ov::is_type<ov::op::v0::Exp>(n) ||
               ov::is_type<ov::op::v1::LogicalNot>(n) || ov::is_type<ov::op::v0::Negative>(n) ||
               ov::is_type<ov::op::v0::Relu>(n) || ov::is_type<ov::op::v5::Round>(n) ||
               ov::is_type<ov::op::v0::Sigmoid>(n) || ov::is_type<ov::op::v0::Sqrt>(n) ||
               ov::is_type<ov::op::v0::Tanh>(n) || ov::is_type<ov::op::v0::Gelu>(n) ||
               ov::is_type<ov::op::v7::Gelu>(n) || ov::is_type<ov::op::v4::Swish>(n) ||
...
TokenizeSnippets::TokenizeSnippets() {
    ...
    enum continuation_strategy { reset, abort };

    continuation_strategy strategy = continuation_strategy::reset;
    auto label = std::make_shared<ov::pass::pattern::op::Label>(
        ...
        [](const std::shared_ptr<const Node>& n) {
            ...
            return (GetSnippetsNodeType(n) != SnippetsNodeType::SkippedByPlugin || ov::is_type<ov::op::v0::MatMul>(n) ||
    ...
    ov::graph_rewrite_callback callback = [&, strategy](ov::pass::pattern::Matcher& m) -> bool {
        ...
        auto abort_with_strategy =
            [&](const std::string& message_reset, const std::string& message_abort = "", int priority = 3) {
        ...
        for (const auto& input_value : input_values) {
            ...
            if (ov::is_type<op::Subgraph>(input_node) && !cyclicDependencyIsIntoduced(input_node, currentTopoBounds)) {
                ...
                if (!input_subgraphs.count(input_node)) {
                    ...
                    // Todo:
                    //  Some of the input subgraphs might have common parents, so some of the input_parameters might
                    //  already be in external_inputs and hence in body_parameters. Here we handle this case and remove
                    //  repeated body_parameters. Would it be better to incorporate all inputs first and then remove
                    //  repeated params.
                    for (size_t i = 0; i < input_body_parameters.size(); ++i) {
                        auto found =
                            std::find(external_inputs.begin(), external_inputs.end(), subgraph->input_value(i));
                        if (found != external_inputs.end()) {
                            ...
                            remark(13) << "ternary merge is conducted "
                                       << subgraph->input_value(i).get_node_shared_ptr() << std::endl;
                            ...
                            if (auto to_replace_with =
                                ...
                                // todo: In principle, we can still attach the node to the subgraph if cyclic dependency
                                // is introduced during ternary merge.
                                //  Need to support.
                                if (cyclicDependencyIsIntoduced(to_replace_with, currentTopoBounds))
                                    return abort_with_strategy(
                ...
                // this is there stitching happens, get result of a copy of a body of currently processed input and put
                // it to the new inputs internal output index == external output index
                ...
                // We cannot add new node, that is not Convert, after Convert (that is start node) to avoid arithmetic
                // problems with conversion We can add any new node in Subgraph after Convert (bacause after Input)
                //              Parameter
                //                  |
                //               Convert
                //
                ...
                //               Convert
                //
                // Thus, We can grow subgraph only if Convert is the first node of subgraph and have to abort it's the
                // last one and we want to add not Convert We have this limitation because at the moment we support only
                // one execution precision inside body, so if there is Convert with input and output data types that
                // aren't equal to supported exec type, we can get conversion math errors
                ...
                if (!ov::is_type<ov::op::v0::Convert>(node) && ov::is_type<ov::op::v0::Convert>(output_of_subgraph)) {
                    ...
                    if (!ov::is_type<ov::op::v0::Parameter>(grandparent)) {
                        return abort_with_strategy(
                            "Convert supports only as Input and as Result of subgraph. Aborting");
                    }
                }
        ...
        for (auto subgraph : input_subgraphs) {
            // we should summurize additional needed data count (non-scalar Constants and Buffers) from all input
            // subgraphs because we will collapse them with our node and we should get total count
        ...
        // unique GPR. At the moment, CPU Plugin has limitation for GPR registers: there are 12 available GPRs, and one
        // of them must be reserved for runtime parameters, so only 11 can be used during kernel execution. This
        // limitation will be resolved once generator supports gprs spills [75622].
        ...
        if (body_parameters.size() + body_results.size() + hidden_data_count + unique_buffer_count > 11) {
            const std::string message_reset = "new subgraph is created. Impossible to schedule subgraph with " +
                                              std::to_string(body_parameters.size()) + " inputs, " +
                                              std::to_string(body_results.size()) + " outputs and " +
                                              std::to_string(hidden_data_count) + " non-scalar constants and " +
                                              std::to_string(unique_buffer_count) + "buffers.";
