...
TransposeDecomposition::TransposeDecomposition() {
    ...
    //       This is the reason we match only to Parameter, this limitation could be relaxed if we propagate access
    //       pattern to the appropriate parameter
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        const auto transpose =
            ov::as_type_ptr<ov::opset1::Transpose>(pattern_to_output.at(match_transpose).get_node_shared_ptr());
        ...
        PortDescriptorUtils::set_port_descriptor_ptr(
            load->input(0),
            std::make_shared<PortDescriptor>(load->get_input_shape(0), subtensor, layout));
        PortDescriptorUtils::set_port_descriptor_ptr(
            load->output(0),
            std::make_shared<PortDescriptor>(load->get_output_shape(0), subtensor));
        PortDescriptorUtils::set_port_descriptor_ptr(
            store->input(0),
            std::make_shared<PortDescriptor>(store->get_input_shape(0), subtensor));
        PortDescriptorUtils::set_port_descriptor_ptr(
            store->output(0),
            std::make_shared<PortDescriptor>(store->get_output_shape(0), subtensor));
