...
bool pass::AlignElementTypes::run_on_model(const std::shared_ptr<ov::Model>& m) {
    ...
    bool precisions_match = m_input_precisions.size() == params.size() && m_output_precisions.size() == results.size();
    OPENVINO_ASSERT(precisions_match,
    ...
    for (size_t i = 0; i < m_output_precisions.size(); i++) {
        ...
        if (results[i]->get_input_element_type(0) != needed_out_type) {
            ...
            if (const auto existing_convert =
                    ov::as_type_ptr<ov::snippets::op::ConvertSaturation>(parent_output.get_node_shared_ptr())) {
                ...
                if (ov::snippets::pass::PropagatePrecision::can_be_removed(actual_before,
                                                                           actual_after,
                                                                           required_after) &&
                    parent_output.get_target_inputs().size() == 1) {
    ...
    for (size_t i = 0; i < m_input_precisions.size(); ++i) {
        ...
        if (original_type != needed_in_type) {
            ...
            if (std::any_of(consumer_inputs.cbegin(), consumer_inputs.cend(), [](const ov::Input<ov::Node>& input) {
                    return ov::is_type<ov::op::v1::Transpose>(input.get_node());
                })) {
                OPENVINO_ASSERT(
            ...
            // If there is already Convert[original_type->needed_in_type] and this node is alone consumer, we can remove
            // the Convert, since the sequence new Convert[needed_in_type->original_type] -> existing
            // Convert[original_type->needed_in_type] is redundant
            if (const auto existing_convert = ov::as_type_ptr<ov::snippets::op::ConvertSaturation>(
                    consumer_inputs.cbegin()->get_node()->shared_from_this())) {
