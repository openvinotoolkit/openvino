...
void insert_explicitly_loops(const ov::NodeVector& ops, const ov::PartialShape& master_shape,
    ...
    auto wrapBodyByLoop = [&](const ov::NodeVector& body, const ov::OutputVector& body_parameters, const std::vector<ov::Input<ov::Node>>& body_results) {
        ...
        const auto& inner_loop_end = op::insertLoopEndBeforeInputs(
            body_results, inner_loop_begin, 0lu, inner_work_amount, vector_size,
        ...
        if (outer_work_amount > 1) {
            ...
            op::insertLoopEnd(body_results, outer_loop_begin, 1lu, outer_work_amount, 1lu,
    ...
    for (auto iter = ops.begin(); iter < ops.end(); iter++) {
        ...
        if (loop_begin) {
            ...
            // check for potential parameters of new Loop
            ...
            // check for potential results of new Loop
...
bool InsertLoops::run_on_model(const std::shared_ptr<ov::Model> &model) {
    ...
    if (inner_work_amount > 0) {
        if (m_is_optimized) {
            ...
            const auto& inner_loop_end = insertLoopEnd(commonResults, inner_loop_begin, 0lu, inner_work_amount,
