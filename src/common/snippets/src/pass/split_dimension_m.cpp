...
std::pair<size_t, size_t> ov::snippets::pass::SplitDimensionM::get_splited_dimensions(
    size_t batch_dim,
    size_t m_dim,
...
bool ov::snippets::pass::SplitDimensionM::can_be_optimized(const std::shared_ptr<const ov::Node>& node,
                                                           size_t concurrency) {
...
std::shared_ptr<ov::op::v0::MatMul> ov::snippets::pass::SplitDimensionM::get_matmul(
    const std::shared_ptr<op::Subgraph>& subgraph) {
...
bool ov::snippets::pass::SplitDimensionM::split(const ov::Shape& shape,
                                                size_t optimal_parallelism_work_amount,
                                                size_t& batch_m_dim,
                                                size_t& new_m_dim) {
...
void ov::snippets::pass::SplitDimensionM::reshape_subgraph(const std::shared_ptr<op::Subgraph>& subgraph,
    ...
    auto reshape_transpose = [&](const std::shared_ptr<ov::Node>& transpose, bool is_input) -> size_t {
        ...
        const auto m_index =
            is_input ? order[order.size() - 2] : order.size() - 2;  // Index of M dimension in the previous order
    ...
    for (const auto& op : ops) {
        if (const auto matmul = ov::as_type_ptr<ov::op::v0::MatMul>(op)) {
            ...
            const auto shape_const =
                ov::as_type_ptr<ov::op::v0::Constant>(broadcast->input_value(1).get_node_shared_ptr());
            OPENVINO_ASSERT(shape_const, "SplitDimensionM expects Broadcast with Constant output shape");
            const auto new_shape =
                get_updated_shape(shape_const->cast_vector<size_t>(), broadcast->get_output_shape(0).size() - 2, true);
