...
std::pair<size_t, size_t> SplitDimensionM::split_ideally(size_t batch_dim,
                                                         size_t m_dim,
...
std::pair<size_t, size_t> SplitDimensionM::split_fallback_increase_parallel_wa(size_t batch_dim,
                                                                               size_t m_dim,
                                                                               size_t optimal_parallelism_work_amount) {
...
std::pair<size_t, size_t> SplitDimensionM::split_minimize_kernel_wa(size_t batch_dim,
                                                                    size_t m_dim,
                                                                    size_t optimal_parallelism_work_amount) {
    // This heuristic minimizes 'm_kernel' (=> maximizes 'm_batch') with a limitation that 'm_kernel >= min_kernel_m'.
    // In other words, it tries to find 'm_kernel' bigger than 'min_kernel_m' and at the same time as close as possible
    // to this value.
...
ov::snippets::VectorDims SplitDimensionM::reshape_m_dim(ov::snippets::VectorDims shape,
                                                        size_t m_index,
                                                        size_t batch_m_dim,
                                                        size_t new_m_dim) {
...
std::shared_ptr<ov::op::v0::MatMul> SplitDimensionM::get_matmul(const std::shared_ptr<op::Subgraph>& subgraph) {
    ...
    //           we cannot garantee correct extraction since we don't have correct connections between body I/O and
    //           Subgraph I/O.
    ...
    const auto mm_it = std::find_if(ops.cbegin(), ops.cend(), [](const std::shared_ptr<ov::Node>& node) {
...
bool SplitDimensionM::split(const ov::Shape& shape,
                            size_t optimal_parallelism_work_amount,
                            size_t& batch_m_dim,
                            size_t& new_m_dim) {
    size_t batch_dim = 1;
...
void SplitDimensionM::reshape_subgraph(const std::shared_ptr<op::Subgraph>& subgraph,
    ...
    auto insert_reshape = [&](const std::shared_ptr<ov::op::v0::Parameter>& param, const ov::Shape& new_shape) {
        ...
        const auto shape_const =
            std::make_shared<ov::op::v0::Constant>(ov::element::i32, ov::Shape{new_shape.size()}, new_shape);
    ...
    auto get_updated_shape = [&](const ov::snippets::VectorDims& shape, size_t m_index, bool split_m_dim) {
        ...
        const bool is_shape_valid = !split_m_dim || current_m_dim == 1 || current_m_dim == m_dim;
        OPENVINO_ASSERT(is_shape_valid, "Incorrect shape for splitting!");
    ...
    auto reshape_transpose = [&](const std::shared_ptr<ov::Node>& transpose, bool is_input) -> size_t {
        ...
        const auto m_index =
            is_input ? order[forward_index] : forward_index;  // Index of M dimension in the previous order
        const auto new_order = get_updated_order(order, m_index);
        transpose->set_argument(1,
                                std::make_shared<ov::op::v0::Constant>(order_constant->get_element_type(),
                                                                       ov::Shape{new_order.size()},
                                                                       new_order));
        return m_index;
    };
    ...
    for (const auto& op : ops) {
        if (const auto matmul = ov::as_type_ptr<ov::op::v0::MatMul>(op)) {
            ...
            softmax_v1->set_axis(
                softmax_v1->get_output_partial_shape(0).size());  // since new_shape.size() = old_shape.size() + 1
            ...
            broadcast->set_argument(1,
                                    std::make_shared<ov::op::v0::Constant>(shape_const->get_element_type(),
    ...
    for (size_t i = 0; i < subgraph->get_output_size(); ++i) {
        ...
        const auto shape_const =
            std::make_shared<ov::op::v0::Constant>(ov::element::i32, ov::Shape{output_shape.size()}, output_shape);
...
bool SplitDimensionM::run_on_subgraph(const std::shared_ptr<op::Subgraph>& subgraph) {
    ...
    const auto is_dynamic = [](const std::shared_ptr<ov::Node>& p) {
