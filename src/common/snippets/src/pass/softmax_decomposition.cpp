...
ngraph::snippets::pass::SoftmaxDecomposition::SoftmaxDecomposition(const size_t vector_size) {
    ...
    register_matcher(std::make_shared<ngraph::pattern::Matcher>(
        ...
        if (has_outer_loop) {
            ...
            const auto outer_loop_end = ngraph::snippets::op::insertLoopEndBeforeInputs(
                softmax_results, outer_loop_begin, outer_dim, master_shape[outer_dim], 1, apply_increments);
