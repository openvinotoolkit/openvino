...
PassPosition::PassPosition(Place pass_place, const DiscreteTypeInfo& pass_type_info, size_t pass_instance)
    : m_pass_type_info(pass_type_info),
      ...
      m_place(pass_place) {
    OPENVINO_ASSERT(utils::any_of(m_place, Place::Before, Place::After),
                    "pass_place must be Place::Before or Place::After");
    OPENVINO_ASSERT(m_pass_type_info != DiscreteTypeInfo(), "pass_type_info must be non-empty");
}
