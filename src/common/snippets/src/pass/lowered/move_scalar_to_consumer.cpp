...
bool MoveScalarToConsumer::run(LoweredExprIR& linear_ir) {
   ...
   for (auto expr_it = linear_ir.rbegin(); expr_it != linear_ir.rend(); expr_it++) {
        ...
        if (ov::is_type<op::Scalar>(expr->get_node())) {
            ...
            if (consumers.size() != 1)
                throw ngraph_error("Scalar expression is expected to have a single consumer");
            ...
            if (consumer_expr != *std::next(forward_it)) {
                ...
                while (*consumer_it != consumer_expr)
                    consumer_it++;
                auto next_it = linear_ir.move(forward_it, consumer_it);
                expr_it = std::prev(std::reverse_iterator<LoweredExprIR::exprIt>(next_it));
                modified = true;
            }
        }
    }
