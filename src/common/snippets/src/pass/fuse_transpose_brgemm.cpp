...
bool FuseTransposeBrgemm::is_supported_transpose(const Output<Node>& transpose_port) {
    ...
    return is_type<ngraph::opset1::Parameter>(transpose_node->get_input_node_shared_ptr(0)) &&
...
FuseTransposeBrgemm::FuseTransposeBrgemm() {
    ...
    auto transpose = pattern::wrap_type<opset1::Transpose>({pattern::any_input(), constant}, is_supported_transpose);
    ...
    auto brgemm_out = pattern::wrap_type<op::Brgemm>({pattern::any_input(), pattern::any_input()});
    auto transpose2 = pattern::wrap_type<opset1::Transpose>({brgemm_out, constant});
    ...
    auto callback = [=](pattern::Matcher& m) {
        ...
        if (!brgemm) {
            ...
            const auto& original_port = ngraph::snippets::lowered::PortManager::get_port_descriptor_ptr(brgemm_out);
        ...
        for (size_t i = 0; i < brgemm->get_input_size(); i++) {
            ...
            if (transpose_matcher->match(in_value)) {
                ...
                const auto& original_port = ngraph::snippets::lowered::PortManager::get_port_descriptor_ptr(in);
