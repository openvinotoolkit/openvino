...
bool is_supported_intermediate_op(const std::shared_ptr<ov::Node>& node) {
    const auto is_intermediate_op = [](const std::shared_ptr<ov::Node>& node) {
        return ov::is_type<ov::op::util::UnaryElementwiseArithmetic>(node) ||
               ...
               ov::is_type<ov::op::v0::FakeQuantize>(node) || ov::is_type<ov::op::v1::Select>(node);
    };
...
ov::snippets::pass::TokenizeMHASnippets::TokenizeMHASnippets(const SnippetsTokenization::Config& config) {
    ...
    auto m_matmul0 =
        std::make_shared<ov::opset1::MatMul>(ov::pass::pattern::any_input(ov::pass::pattern::has_static_shape()),
    ...
    register_matcher(
        ...
        [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
            ...
            //     for scalar case. It means that these Buffers cannot be inplace => Each Buffer should have the own
            //     register
            ...
            //  - Between MatMul0 and Transpose1 - At the moment operations after Transpose1 cannot be fused in
            //  Transpose Loop (to avoid performance regressions).
            ...
            //  buffer_count = 1
            //  - Thirdly, MatMul requires unique Buffers on inputs and outputs because blocking implementation
            //  increments input/output pointers during computations
            //    However, all of the Buffers are usually reused by the next MatMul and Softmax.
            //    So on sufficiently large subgraphs we use only one additional unique buffer => buffer_count increments
            //    by 1
            ...
            if (!matmul1 || matmul1->get_output_target_inputs(0).size() != 1 || matmul1->get_transpose_a() ||
                matmul1->get_transpose_b())
                return false;

            const auto matmul1_out_type =
                op::Brgemm::get_output_type(matmul1->get_input_element_type(0), matmul1->get_input_element_type(1));
            if (matmul1_out_type == element::undefined || !is_supported_tensor(matmul1->get_input_tensor(0)) ||
            ...
            // We can support several ops between MatMul0 with transposed_b and Transpose1 with 0213 order (or without
            // this Transpose1) only if these ops have scalar shapes on other inputs. There is transformation
            // ExplicitTransposeMatMulInputs that set supported order and transposed_b(false). We can allow to call this
            // pass only if ops have scalar shapes to avoid shape mismatching
            ...
            tokenize_transpose(transpose1,
                               is_transposed_b_0,
                               get_decomposed_transpose_order(pattern_rank),
                               ordered_ops.begin());
            tokenize_transpose(transpose0,
                               matmul0->get_transpose_a(),
                               get_fusion_transpose_order(pattern_rank),
                               ordered_ops.begin());
            tokenize_transpose(transpose2,
                               matmul1->get_transpose_b(),
                               get_fusion_transpose_order(pattern_rank),
                               ordered_ops.end());
            ...
            while (is_supported_intermediate_op(child)) {
                ...
                if (potential_body_params_count + child->get_output_target_inputs(0).size() +
                        hidden_virtual_ports_count + buffer_count >
                    12) {
                    break;
                }
            ...
            if (!are_ops_after_matmul1) {
                auto transpose3 = config.mha_token_enable_transpose_on_output
            ...
            if (potential_body_params_count + last_node->get_output_size() + hidden_virtual_ports_count + buffer_count >
                11) {
                return false;
            }
            ...
            auto create_body_inputs = [&](const std::shared_ptr<ov::Node>& node) -> void {
                for (size_t i = 0; i < node->get_input_size(); ++i) {
                    ...
                    if (constant &&
                        ...
                        // If Constant has several consumers, we should check that all these consumers are inside
                        // Subgraph body and if all of them are inside body, we can explicitly add Constant to the
                        // body_inputs, otherwise we should make a copy and add copy of Constant to body_inputs For
                        // example, this case is especially valid for Transposes nodes
                        //              (several Transposes have the same order so there can be the common Constant with
                        //              this order)
                        if (constant->get_output_target_inputs(0).size() == 1) {
                            ...
                            bool all_consumers_are_inside = std::all_of(
                                constant_consumers.begin(),
