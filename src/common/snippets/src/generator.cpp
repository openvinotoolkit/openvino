    ...
    for (auto op = ops.begin(); op < ops.end(); op++) {
        ...
        if (loop_begin && loop_begin->get_increment() != 1) {
            ...
            if (need_vector_loop) {
                ...
                vector_evaluate_once = optimize_single_evaluation(vector_loop_end, need_tail);
            }
            ...
            if (need_tail) {
                ...
                const auto tail_work_amount = 1;
                tail_loop_end->set_increment(tail_size);
                tail_loop_end->set_work_amount(tail_work_amount);
                tail_loop_end->has_outer_loop = vector_loop_end->has_outer_loop;
                // tail loop is always executed once
                tail_evaluate_once = optimize_single_evaluation(tail_loop_end);
            }
