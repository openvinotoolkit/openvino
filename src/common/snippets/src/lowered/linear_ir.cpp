...
void update_consumers_and_regs(const ExpressionPtr& new_expr, const std::vector<std::set<ExpressionPort>>& consumers) {
    bool is_consumer_count_valid = consumers.empty() || consumers.size() == new_expr->get_output_count();
    OPENVINO_ASSERT(is_consumer_count_valid,
...
void LinearIR::register_expression(const ExpressionPtr& expr, bool io_allowed, double exec_num) {
    const auto& node = expr->get_node();
    bool is_node_allowed = io_allowed || (!is_type_any_of<ov::op::v0::Result, ov::op::v0::Parameter>(node));
    OPENVINO_ASSERT(is_node_allowed, "LinearIR::insert can't be used to add Parameters or Results to IR");
...
LinearIR::exprIt LinearIR::replace_with_expr(const std::vector<ExpressionPtr>& old_exprs,
    ...
    for (const auto& old_expr : old_exprs) {
        for (const auto& input : old_expr->get_input_port_connectors()) {
            ...
            bool is_source_valid = is_old_expr(source.get_expr()) || is_input_source(source);
            OPENVINO_ASSERT(is_source_valid,
