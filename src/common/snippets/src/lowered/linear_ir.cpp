...
LinearIR::exprIt LinearIR::insert_node(const std::shared_ptr<ov::Node>& new_node, const std::vector<PortConnectorPtr>& new_inputs,
    ...
    get_loop_manager()->reorder_loop_ports(loop_ids);
    return expr_it;
}
template<>
...
LinearIR::exprIt LinearIR::replace_with_expr(const std::vector<ExpressionPtr>& old_exprs, const ExpressionPtr& new_expr, const constExprIt& place) {
    ...
    for (const auto& old_expr : old_exprs) {
        for (size_t i = 0; i < old_expr->get_input_count(); ++i)
            m_loop_manager->replace_loops_ports(loop_ids, LoopInfo::ExpressionPortMap{{old_expr->get_input_port(i), input_ports}});
        for (size_t i = 0; i < old_expr->get_input_count(); ++i)
            m_loop_manager->replace_loops_ports(loop_ids, LoopInfo::ExpressionPortMap{{old_expr->get_output_port(i), output_ports}});
        erase(find(old_expr));
    }
    return new_expr_it;
}
...
double LinearIR::get_inserted_expr_exec_num(constExprIt insertion_pos) const {
    ...
    if (right_pos == cbegin()) {      // On the list begin
        ...
        // heuristic value to compare the current order values:
        // if there is very small difference between them (less 2.22045e-10), we have to renumber expressions
        static const double diff = std::numeric_limits<double>::epsilon() * 10e6;
        if (right_order - left_order <= diff) {
            enumerate_expressions();
            left_order = left_pos->get()->get_exec_num();
            right_order = right_pos->get()->get_exec_num();
        }
