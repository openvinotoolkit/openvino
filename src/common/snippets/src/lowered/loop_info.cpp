...
std::shared_ptr<LoopInfo> UnifiedLoopInfo::clone_with_new_expr(const ExpressionMap& expr_map,
    ...
    if (loop_map.count(this) == 0) {
        ...
        const auto& replacement = std::make_shared<UnifiedLoopInfo>(m_work_amount, m_increment, new_input_ports, new_output_ports,
...
std::shared_ptr<LoopInfo> InnerSplittedUnifiedLoopInfo::clone_with_new_expr(const ExpressionMap& expr_map,
    ...
    if (loop_map.count(this) == 0) {
        ...
        const auto& replacement = std::make_shared<InnerSplittedUnifiedLoopInfo>(m_increment, new_input_ports, new_output_ports,
                                                                                 m_input_port_descs, m_output_port_descs, m_handlers,
...
std::shared_ptr<LoopInfo> ExpandedLoopInfo::clone_with_new_expr(const ExpressionMap& expr_map,
    ...
    if (loop_map.count(this) == 0) {
        ...
        const auto& replacement = std::make_shared<ExpandedLoopInfo>(m_work_amount, m_increment, new_input_ports, new_output_ports,
