...
std::shared_ptr<Expression> ExpressionPort::get_expr() const {
    ...
    OPENVINO_DEBUG_ASSERT(expr_ptr != nullptr, "ExpressionPort has invalid expression pointer");
    return expr_ptr;
}
...
bool operator<(const ExpressionPort& lhs, const ExpressionPort& rhs) {
    ...
    return (lhs.get_index() < rhs.get_index()) ||
           (lhs.get_index() == rhs.get_index() && lhs.get_expr() < rhs.get_expr());
}

void replace_input_port_connectors(const std::set<ExpressionPort>& consumers,
                                   const std::shared_ptr<PortConnector>& to) {
