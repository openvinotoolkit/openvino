...
bool ExtractLoopInvariants::run(LinearIR& linear_ir) {
    ...
    auto extract_from_loop = [&](const size_t& inner_loop_id) {
        bool extracte_complete = false;  // true means all extractable exprs are extracted from this loop
        ...
        while (!extracte_complete) {
            ...
            for (size_t i = 0; i < inner_loop_input_ports.size(); i++) {  // iter loop ports
                ...
                for (size_t i = 0; i < expr_input_ports.size(); ++i) {  // iter expr ports
                    ...
                    bool parent_scalar_with_single_consumer =
                        ov::is_type<snippets::op::Scalar>(parent->get_node()) &&
                ...
                if (extract_applicable) {
                    ...
                    if (outer_loop_ids.size() > 0) {
                        ...
                        for (size_t i = 0; i < port_expr->get_input_count(); ++i) {
                            ...
                            if (std::find(parent_expr_loop_ids.cbegin(), parent_expr_loop_ids.cend(), outer_loop_id) ==
                                parent_expr_loop_ids.cend()) {
                                // parent expr is not in outer loop
                                add_in_ports.push_back(in_port);
                            }
                        }
                        if (!add_in_ports.empty()) {
    ...
    for (auto expr_it = linear_ir.begin(); expr_it != linear_ir.end(); expr_it++) {
        ...
        if (next_loop_ids_size < current_loop_ids_size) {
            ...
            std::vector<size_t> identified_loop_ids(current_loop_ids_size - next_loop_ids_size);
            for (size_t i = 0; i < current_loop_ids_size - next_loop_ids_size; i++) {
                identified_loop_ids[i] = current_loop_ids[current_loop_ids_size - 1 - i];
            }

            for (size_t i = 0; i < identified_loop_ids.size(); i++) {
                extract_from_loop(identified_loop_ids[i]);
            }
        }
    }
