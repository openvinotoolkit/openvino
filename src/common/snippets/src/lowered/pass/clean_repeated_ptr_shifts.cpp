...
bool CleanRepeatedDataPointerShifts::reuse_increments(const LoopManagerPtr& loop_manager,
    ...
    for (size_t i = 0; i < output_count; ++i) {
        ...
        if (buffer_count > 0) {
            OPENVINO_ASSERT(buffer_count == 1, "Loop output must have exactly 1 Buffer");
            OPENVINO_ASSERT(buffer_count + loop_count == consumer_inputs.size(),
                            "Buffer and loop counts must match consumer inputs");
        }
    }
