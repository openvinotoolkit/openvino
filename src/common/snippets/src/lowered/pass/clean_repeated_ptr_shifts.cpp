...
bool CleanRepeatedDataPointerShifts::reuse_increments(const LoopManagerPtr& loop_manager,
    ...
    for (size_t i = 0; i < output_count; ++i) {
        ...
        if (buffer_count > 0) {
            bool is_buffer_constraint_valid =
                (buffer_count == 1) && (buffer_count + loop_count == consumer_inputs.size());
            OPENVINO_ASSERT(is_buffer_constraint_valid, "Loop output must have not more than 1 Buffer");
        }
    }
