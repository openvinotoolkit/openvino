...
bool CleanRepeatedDataPointerShifts::reuse_increments(const LinearIR& linear_ir, const ExpressionPtr& loop_end_expr) {
    ...
    // We have to update LoopEnd and LoopInfo since the both entityies must be valid.
    // Tod avoid the both changes, we can insert Loop ops to LinearIR in the end of pipeline.
    ...
    for (auto idx_to_drop : resetting_data_indexes) {
        ...
        if (idx_to_drop < input_count)
            loop_entries[idx_to_drop].is_incremented = false;
        else
            loop_exits[idx_to_drop - input_count].is_incremented = false;
    }
