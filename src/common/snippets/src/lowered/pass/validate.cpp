...
void validate_result(const ExpressionPtr& expr, [[maybe_unused]] const LinearIR& linear_ir) {
    ...
    bool is_memory_access_valid = ma && ma->is_memory_access_output_port(source.get_index());
    OPENVINO_ASSERT(is_memory_access_valid, "Result expects MemoryAccess parent");
}
...
void validate_loop_end(const ExpressionPtr& expr, const LinearIR& linear_ir) {
    ...
    auto validate_loop_ports = [&](const std::vector<UnifiedLoopInfo::LoopPortInfo>& loop_port_infos,
        ...
        for (size_t i = 0; i < loop_port_infos.size(); ++i) {
            bool are_port_params_compatible = is_incremented[i + shift] == loop_port_infos[i].port.is_incremented() &&
            ...
            OPENVINO_ASSERT(are_port_params_compatible,
