...
bool IdentifyBuffers::can_reuse_id(const ShiftPtrParams& lhs, const ShiftPtrParams& rhs) {
    const auto equal_ptr_params_shifting =
        lhs.ptr_increment == rhs.ptr_increment && lhs.finalization_offset == rhs.finalization_offset;
    const auto equal_element_type_sizes = lhs.data_size == rhs.data_size;
    return equal_ptr_params_shifting &&
           (equal_element_type_sizes || (lhs.ptr_increment == 0 && lhs.finalization_offset == 0));
}
...
bool IdentifyBuffers::are_adjacent(const std::pair<ExpressionPtr, ShiftPtrParams>& lhs,
    ...
    if (equal_loop_ids) {  // Buffers are connected to the same Loop and have the same outer Loops
        ...
        const auto outer_buffer_has_zero_shifts =
            outer_buffer.second.ptr_increment == 0 && outer_buffer.second.finalization_offset == 0;
...
std::vector<bool> IdentifyBuffers::create_adjacency_matrix(LinearIR::constExprIt begin,
