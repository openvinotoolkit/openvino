...
bool InsertBroadcastMove::is_broadcasting_supported(const std::shared_ptr<ov::Node>& n) {
    return !std::dynamic_pointer_cast<modifier::MemoryAccess>(n) &&
           (ov::op::util::supports_auto_broadcast(n) || n->get_autob().m_type == ov::op::AutoBroadcastType::NUMPY ||
...
bool InsertBroadcastMove::is_broadcasting_needed(const std::shared_ptr<ov::Node>& n) {
    ...
    return !utils::is_scalar_constant(n) && !ov::is_type<ov::snippets::op::VectorBuffer>(n) &&
...
std::vector<size_t> InsertBroadcastMove::get_last_dims(const ExpressionPtr& expr) {
    ...
    std::transform(descriptors.begin(),
                   descriptors.end(),
                   last_dims.begin(),
...
bool InsertBroadcastMove::run(LinearIR& linear_ir,
    ...
    for (auto expr_it = begin; expr_it != end; expr_it++) {
        ...
        for (size_t i = 0; i < last_dims.size(); i++) {
            ...
            if (!utils::is_dynamic_value(last_dims[i]) && last_dims[i] != broadcasted_dim &&
                ...
                expr->get_input_port_descriptor(i)->set_shape(
                    broadcast_expr->get_output_port_descriptor(0)->get_shape());
