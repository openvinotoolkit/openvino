...
InsertPerfCount::InsertPerfCount(std::map<std::string, std::string> boundary_op_names)
    : RangedPass(),
      m_boundary_op_names(std::move(boundary_op_names)) {}
...
bool InsertPerfCount::run(LinearIR& linear_ir,
    ...
    for (auto expr_it = begin; expr_it != end; expr_it++) {
        ...
        if (found != m_boundary_op_names.end()) {
            ...
            OPENVINO_ASSERT(perf_count_end_pos != linear_ir.cend(),
                            "Failed to find requested op name to insert PerfCountEnd");
            ...
            linear_ir.insert_node(perf_count_begin,
                                  empty_inputs,
                                  perf_count_begin_pos->get()->get_loop_ids(),
                                  false,
                                  perf_count_begin_pos);
            ...
            const auto& perf_count_end =
                std::make_shared<snippets::op::PerfCountEnd>(perf_count_begin->output(0), dumpers);
            ...
            linear_ir.insert_node(perf_count_end,
                                  empty_inputs,
                                  perf_count_end_pos->get()->get_loop_ids(),
                                  false,
                                  next(perf_count_end_pos));
            seq_number++;
        }
    }
    return true;
