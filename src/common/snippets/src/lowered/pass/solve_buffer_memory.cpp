...
std::pair<SolveBufferMemory::Buffers, SolveBufferMemory::Buffers> SolveBufferMemory::extract_static_and_dynamic_buffers(
    const Buffers& buffer_expressions) {
    ...
    for (const auto& static_buffer : static_buffer_exprs) {
        ...
        auto is_cluster_ids_the_same = [&static_cluster_id](const BufferExpressionPtr& expr) {
            return static_cluster_id == expr->get_cluster_id();
        };
        OPENVINO_ASSERT(
            std::none_of(dynamic_buffer_exprs.cbegin(), dynamic_buffer_exprs.cend(), is_cluster_ids_the_same),
...
std::vector<ov::MemorySolver::Box> SolveBufferMemory::init_boxes(const Buffers& buffer_expressions,
    ...
    auto casted_execution_number = [&](const ExpressionPtr& expr) {
        const auto double_execution_number = expr->get_exec_num();
        OPENVINO_ASSERT(int_execution_numbers.count(double_execution_number) != 0,
                        "Expression execution number has not been found!");
...
void SolveBufferMemory::solve_static_buffer_memory(const Buffers& static_buffer_expressions,
                                                   const LinearIR& linear_ir) {
