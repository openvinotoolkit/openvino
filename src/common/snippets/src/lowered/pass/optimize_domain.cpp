...
size_t OptimizeDomain::optimize(std::vector<VectorDims>& input_shapes,
    ...
    while (jit_work_amount < min_jit_work_amount && (num_dims_collapsed + 1) < input_shapes[min_rank_idx].size() &&
...
inline bool OptimizeDomain::can_increase_jit_work_amount(const VectorDims& master_shape,
    ...
    return master_shape.size() > 2 &&
           master_shape[master_shape.size() - 1] * master_shape[master_shape.size() - 2] * min_parallel_work_amount <=
               total_work_amount;
}
bool OptimizeDomain::run(snippets::lowered::LinearIR& linear_ir) {
    ...
    OPENVINO_ASSERT(config.m_min_parallel_work_amount != 0,
                    "OptimizeDomain: Min parallel work amount can't equal to zero");
    ...
    for (const auto& io_expr : linear_ir.get_IO_ops()) {
        if (io_expr->get_type() == snippets::lowered::IOExpression::io_type::INPUT) {
            ...
            const ExpressionPtr& shape_producing_expr = blocked_input_shapes ? first_consumer : io_expr;
            ...
            OPENVINO_ASSERT(std::none_of(shape.begin(),
                                         shape.end(),
                                         [](size_t d) {
                                             return utils::is_dynamic_value(d);
                                         }),
