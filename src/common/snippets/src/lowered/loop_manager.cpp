...
size_t LoopManager::mark_loop(LinearIR::constExprIt loop_begin_pos,
    ...
    for (auto expr_it = loop_begin_pos; expr_it != loop_end_pos; ++expr_it) {
        insert_loop_ids(*expr_it, {loop_id});
    }
    return loop_id;
}
...
size_t LoopManager::replace_with_new_loop(const LinearIR& linear_ir,
    ...
    OPENVINO_ASSERT(std::all_of(m_map.cbegin(),
                                m_map.cend(),
                                [&loop_info](const std::pair<size_t, LoopInfoPtr>& p) {
                                    return loop_info != p.second;
                                }),
...
void LoopManager::fuse_loop_ports(std::vector<LoopPort>& output_ports,
    ...
    for (const auto& output_port : output_ports) {
        ...
        for (const auto& consumer_input : consumers_inputs) {
            const auto input_port_it =
                std::find_if(input_ports.begin(), input_ports.end(), [&consumer_input](const LoopPort& port) {
                    return *port.get_expr_port() == consumer_input;
                });
...
void LoopManager::expression_replacement(LinearIR::constExprIt new_expr_begin,
    ...
    for (auto it = new_expr_begin; it != new_expr_end; ++it) {
        insert_loop_ids(*it, {loop_id}, true);
    }
...
bool LoopManager::reorder_identifiers(const std::map<size_t, size_t>& loop_id_map) {
    OPENVINO_ASSERT(loop_id_map.size() == m_map.size(),
                    "Failed to sort reorder IDs: count of the current IDs in `loop_id_map` must be equal to count of "
                    "existing loops");
    // If all new IDs are the same as original and they're in the `m_map` - nothing to update
    if (std::all_of(loop_id_map.cbegin(), loop_id_map.cend(), [this](const std::pair<size_t, size_t>& p) {
            return p.first == p.second && m_map.count(p.first) > 0;
        })) {
        return false;
    }
    ...
    for (const auto& p : loop_id_map) {
        ...
        OPENVINO_ASSERT(m_map.count(current_id) > 0,
                        "Failed to reorder loop IDs: the Loop with ID ",
                        current_id,
                        " doesn't exist in the map");
        OPENVINO_ASSERT(ordered_map.count(target_id) == 0,
                        "Failed to reorder loop IDs: there are several Loops with the target ID ",
                        target_id);
...
void LoopManager::insert_loop_ids(const ExpressionPtr& expr,
    ...
    std::unordered_set<size_t> unique_loop_ids(loop_ids.cbegin(), loop_ids.cend());
    unique_loop_ids.insert(new_ids.cbegin(), new_ids.cend());
    OPENVINO_ASSERT(unique_loop_ids.size() == loop_ids.size() + new_ids.size(), "Loop IDs must be unique");
