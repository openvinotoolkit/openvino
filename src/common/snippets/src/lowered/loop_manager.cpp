...
void LoopManager::update_loop_ports(const ExpressionPtr& expr) {
    ...
    for (size_t i = 0; i < expr->get_input_count(); ++i) {
        ...
        const auto loop_ids = std::vector<size_t>{common_outer_loop_ids.cbegin(), common_outer_loop_ids.cbegin() + count_of_common_outer_loops};
        replace_loops_ports(loop_ids, LoopInfo::ExpressionPortMap{{source, output_ports}});
        // Save previous port
        if (count_of_common_outer_loops != common_outer_loop_ids.size()) {
            output_ports.insert(output_ports.begin(), source);
            const auto loop_ids = std::vector<size_t>{common_outer_loop_ids.cbegin() + count_of_common_outer_loops, common_outer_loop_ids.cend()};
            replace_loops_ports(loop_ids, LoopInfo::ExpressionPortMap{{source, output_ports}});
        }
    }
    ...
    for (size_t i = 0; i < expr->get_output_count(); ++i) {
        ...
        for (const auto& consumer : consumers) {
            ...
            replace_loops_ports(common_outer_loop_ids, LoopInfo::ExpressionPortMap{{consumer, input_ports}});
        }
    }
}

void LoopManager::reorder_loop_ports(const std::vector<size_t>& loop_ids) {
    for (auto& loop_id : loop_ids)
        get_loop_info(loop_id)->reorder_ports();
}
...
void LoopManager::expression_replacement(LinearIR::constExprIt new_expr_begin, LinearIR::constExprIt new_expr_end, const ExpressionPtr& decomposed_expr,
    ...
    for (size_t i = 0; i < decomposed_expr->get_input_count(); ++i) {
        replace_loop_ports(loop_id, LoopInfo::ExpressionPortMap{{decomposed_expr->get_input_port(i), new_entries}});
    }
    for (size_t i = 0; i < decomposed_expr->get_output_count(); ++i) {
        replace_loop_ports(loop_id, LoopInfo::ExpressionPortMap{{decomposed_expr->get_output_port(i), new_exits}});
    }
}
