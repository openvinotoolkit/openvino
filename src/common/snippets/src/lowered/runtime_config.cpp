...
RuntimeConfig::LoopDescriptorList::iterator RuntimeConfig::get_loop_desc_it(size_t loop_id, const LoopDescriptor::Type& type) {
    OPENVINO_ASSERT(m_loops.count(loop_id) > 0, "LoopId has not been found!");
    auto& loop_descriptors = m_loops.at(loop_id);
...
RuntimeConfig::LoopDescriptorList::iterator RuntimeConfig::push_new_desc(size_t loop_id, const LoopDescriptor::Type& type) {
    OPENVINO_ASSERT(m_loops.count(loop_id) > 0, "LoopId has not been found!");
    return m_loops[loop_id].insert(m_loops[loop_id].cend(), RuntimeConfig::LoopDescriptor(IShapeInferSnippets::DYNAMIC_DIMENSION, 1, {}, {}, type));
}

size_t RuntimeConfig::get_full_loop_descriptor_count() const {
    return std::accumulate(m_loops.cbegin(), m_loops.cend(), 0,
