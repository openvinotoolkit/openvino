...
bool MemoryAccess::visit_attributes(AttributeVisitor& visitor) {
    ...
    for (const auto& p : m_input_ports) {
        ...
        is_dynamic |= utils::is_dynamic_value(port.count) || utils::is_dynamic_value(port.offset) ||
                      utils::is_dynamic_value(port.stride);
    }
...
void MemoryAccess::set_input_port_descriptor(const PortDescriptor& desc, const size_t i) {
    ...
    OPENVINO_ASSERT(it != m_input_ports.end(),
                    "Index of input port descriptor should be less than count of input ports");
    it->second = desc;
    it->second.index = i;
}
...
void MemoryAccess::set_output_port_descriptor(const PortDescriptor& desc, const size_t i) {
    ...
    OPENVINO_ASSERT(it != m_output_ports.end(),
                    "Index of output port descriptor should be less than count of output ports");
    it->second = desc;
    it->second.index = i;
}
...
MemoryAccess::PortDescriptor& MemoryAccess::get_input_port_descriptor(const size_t i) {
    return const_cast<MemoryAccess::PortDescriptor&>(
        const_cast<const MemoryAccess*>(this)->get_input_port_descriptor(i));
}
...
MemoryAccess::PortDescriptor& MemoryAccess::get_output_port_descriptor(const size_t i) {
    return const_cast<MemoryAccess::PortDescriptor&>(
        const_cast<const MemoryAccess*>(this)->get_output_port_descriptor(i));
}
