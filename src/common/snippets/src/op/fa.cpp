...
ov::PartialShape FA::infer_output_partial_shape(const std::vector<ov::PartialShape>& input_shapes) {
    ...
    const auto& arg_k_shape = input_shapes[1];
    ...
    ov::PartialShape arg_q_shape_tmp(arg_q_shape), arg_k_shape_tmp(arg_k_shape), arg_v_shape_tmp(arg_v_shape);
    ...
    for (size_t i = 0; i < max_rank - 2; ++i) {
        ...
        OPENVINO_ASSERT(DimType::broadcast_merge(output_shape[i], output_shape[i], arg_k_shape_tmp[i]) ||
                            arg_k_shape_tmp[i].is_dynamic(),
                        "Incompatible fa batch dimension");
    }
