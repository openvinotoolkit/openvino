...
RankNormalization::RankNormalization(const Output<Node>& data, size_t num_prepend, size_t num_append) :
    Op({data}), m_num_prepend(num_prepend), m_num_append(num_append), m_num_pop(0) {
    constructor_validate_and_infer_types();
}

RankNormalization::RankNormalization(const Output<Node>& data, size_t num_pop) :
    Op({data}), m_num_prepend(0), m_num_append(0), m_num_pop(num_pop) {
    constructor_validate_and_infer_types();
}
...
std::shared_ptr<ov::Node> RankNormalization::clone_with_new_inputs(const OutputVector& new_args) const {
    ...
    // Used shared_ptr ctor since ctor of RankNormalization with three parameters is private
    return std::shared_ptr<RankNormalization>(new RankNormalization(new_args[0], m_num_prepend, m_num_append, m_num_pop));
}
