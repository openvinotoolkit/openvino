...
ov::PartialShape Brgemm::infer_output_partial_shape(const std::vector<ov::PartialShape>& input_shapes) {
    ...
    bool are_matrix_dimensions_compatible = DimType::merge(merged_dimension, arg0_col_dim, arg1_row_dim) || arg0_col_dim.is_dynamic() ||
                                           arg1_row_dim.is_dynamic();
    OPENVINO_ASSERT(are_matrix_dimensions_compatible,
    ...
    for (size_t i = 0; i < max_rank - 2; ++i) {
        bool are_batch_dimensions_compatible = DimType::broadcast_merge(output_shape[i], arg0_shape_tmp[i], arg1_shape_tmp[i]) ||
                                               arg0_shape_tmp[i].is_dynamic() || arg1_shape_tmp[i].is_dynamic();
        OPENVINO_ASSERT(are_batch_dimensions_compatible,
