...
ov::PartialShape Brgemm::infer_output_partial_shape(const std::vector<ov::PartialShape>& input_shapes) {
    ...
    bool are_matrix_dimensions_compatible = DimType::merge(merged_dimension, arg0_col_dim, arg1_row_dim) ||
    ...
    for (size_t i = 0; i < max_rank - 2; ++i) {
        bool are_batch_dimensions_compatible =
        ...
        OPENVINO_ASSERT(are_batch_dimensions_compatible, "Incompatible Brgemm batch dimension");
    }
