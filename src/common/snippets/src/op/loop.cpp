...
void LoopBegin::validate_and_infer_types() {
    ...
    OPENVINO_ASSERT(last_output_inputs.size() == 1,
                    "LoopBegin must have exactly one input attached to the last output");
...
LoopEnd::LoopEnd(const Output<Node>& loop_begin,
...
LoopEndStatic::LoopEndStatic(const Output<Node>& loop_begin,
                             size_t work_amount,
                             size_t work_amount_increment,
                             std::vector<bool> is_incremented,
...
std::shared_ptr<Node> LoopEndStatic::clone_with_new_inputs(const OutputVector& inputs) const {
    ...
    const auto loop_end = std::make_shared<LoopEndStatic>(inputs.at(0),
                                                          m_work_amount,
...
void LoopEndStatic::update_ptr_increments(int64_t new_increment) {
    std::transform(m_ptr_increments.begin(),
                   m_ptr_increments.end(),
                   m_ptr_increments.begin(),
...
LoopEndDynamic::LoopEndDynamic(const Output<Node>& loop_begin,
                               size_t work_amount_increment,
                               std::vector<bool> is_incremented,
                               std::vector<int64_t> element_type_sizes,
                               size_t input_num,
...
std::shared_ptr<Node> LoopEndDynamic::clone_with_new_inputs(const OutputVector& inputs) const {
    check_new_args_count(this, inputs);
    return std::make_shared<LoopEndDynamic>(inputs.at(0),
                                            m_work_amount_increment,
                                            m_is_incremented,
                                            m_element_type_sizes,
                                            m_input_num,
