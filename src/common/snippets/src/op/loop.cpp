...
std::shared_ptr<Node> LoopEnd::clone_with_new_inputs(const OutputVector& inputs) const {
    ...
    const auto loop_end = std::make_shared<LoopEnd>(inputs.at(0),
                                                    m_work_amount,
...
LoopEndParallel::LoopEndParallel(const Output<Node>& loop_begin, size_t work_amount, size_t work_amount_increment,
    ...
    : LoopEnd(loop_begin, work_amount, work_amount_increment, is_incremented, ptr_increments, finalization_offsets,
...
std::shared_ptr<Node> LoopEndParallel::clone_with_new_inputs(const OutputVector& inputs) const {
    check_new_args_count(this, inputs);
    const auto loop_end = std::make_shared<LoopEndParallel>(inputs.at(0), m_work_amount, m_work_amount_increment, m_is_incremented, m_ptr_increments,
