...
public:
    ...
    static bool canBeParallelismWAOptimized(const std::shared_ptr<const ov::Node>& node, size_t num_threads);
...
private:
    ...
    void SplitDimensionM(const std::shared_ptr<op::Subgraph>& subgraph, size_t num_threads);
};
