...
public:
    ...
    [[nodiscard]] std::vector<Reg> get_kernel_call_regs(const std::shared_ptr<snippets::op::Kernel>& kernel) const {
        ...
        OPENVINO_DEBUG_ASSERT(abi_regs.size() > num_kernel_args,
                              "Too many kernel args requested: ",
                              num_kernel_args,
                              ", available: ",
                              abi_regs.size());
    ...
    [[nodiscard]] const LiveInterval& get_live_range(const Reg& reg) const {
        OPENVINO_DEBUG_ASSERT(m_reg_live_range.count(reg), "Live range for this reg was not set", reg);
