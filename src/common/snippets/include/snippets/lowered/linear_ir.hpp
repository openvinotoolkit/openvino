...
public:
    ...
    // Minimal advised work amount that should be processed during one call of the executable produced by
    // Subgraph::generate Set by a backend, should be large enough to compensate for the kernel call overheads
...
public:
    ...
    const container& get_ops() const {
        return m_expressions;
    }
    const std::vector<ExpressionPtr>& get_parameters() const {
        return m_parameter_expressions;
    }
    const std::vector<ExpressionPtr>& get_results() const {
        return m_result_expressions;
    }
    const std::vector<BufferExpressionPtr>& get_buffers() const {
        return m_buffer_expressions;
    }
    ...
    void set_static_buffer_scratchpad_size(size_t size) {
        m_static_buffer_scratchpad_size = size;
    }
    ...
    container::reference back() noexcept {
        return m_expressions.back();
    }
    container::const_reference back() const noexcept {
        return m_expressions.back();
    }
    container::reference front() noexcept {
        return m_expressions.front();
    }
    container::const_reference front() const noexcept {
        return m_expressions.front();
    }
    ...
    exprReverseIt rbegin() noexcept {
        return m_expressions.rbegin();
    }
    exprReverseIt rend() noexcept {
        return m_expressions.rend();
    }
    constExprReverseIt crbegin() const noexcept {
        return m_expressions.crbegin();
    }
    constExprReverseIt crend() const noexcept {
    ...
    const LoopManagerPtr& get_loop_manager() const {
        return m_loop_manager;
    }

    IShapeInferSnippets::Result shape_infer(const std::vector<VectorDimsRef>& input_shapes);
    const std::shared_ptr<ShapeInferSnippetsNode>& get_shape_infer_instance() const {
        return m_shape_infer;
    }
    ...
    exprIt insert_node(const std::shared_ptr<ov::Node>& new_node,
        ...
        const auto consumers_py_port = consumers.empty() ? std::vector<std::set<ExpressionPort>>{}
                                                         : std::vector<std::set<ExpressionPort>>{consumers};
    ...
    /**
     * @brief Insert new Expression to LinearIR, sets `loops_ids` as loop identifiers and inserts the expression on the
     * `place` in LinearIR. Also connects output ports to `consumers`
     ...
     * @return new expression iterator in LinearIR
     */
    exprIt insert_expr(const ExpressionPtr& new_expr,
                       const std::vector<size_t>& loop_ids,
                       bool update_loop_ports,
                       const constExprIt& place,
                       const std::vector<std::set<ExpressionPort>>& consumers);
     ...
     *           It means that consumers of the expression in seq are expressions from this sequence (except of the last
     * expr) and all sources of the `old_exprs` must be expression from this seq as well or must be source of
     * `new_inputs`
     ...
     * @return new expression iterator in LinearIR
     */
    exprIt replace_with_node(const std::vector<ExpressionPtr>& old_exprs,
                             const std::shared_ptr<ov::Node>& new_node,
                             const std::vector<size_t>& loop_ids,
                             const constExprIt& place);
    /**
     * @brief Replace the several existing expressions with the one new expression that contains `new_node` that are in
     * the same loops. Insert new expression on the place of last `old_exprs`
     * @param old_exprs the sequence of removable expressions
     * @param new_node the target node
     * @return new expression iterator in LinearIR
     */
     ...
     * @return new expression iterator in LinearIR
     */
    exprIt replace_with_expr(const std::vector<ExpressionPtr>& old_exprs,
                             const ExpressionPtr& new_expr,
                             const constExprIt& place);
    /**
...
private:
    ...
    public:
        explicit LIRShapeInfer(const container& body_exprs,
