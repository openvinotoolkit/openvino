...
// Chrono - perf count with chrono call. This is a universal method, and support multi-thread case to output perf count
// data for each thread. BackendSpecific - perf count provided by backend. This is for device specific requirment. For
// example, in sake of more light overhead and more accurate result, x86 CPU specific mode via read RDTSC register is
// implemented, which take ~50ns, while Chrono mode take 260ns for a pair of perf count start and perf count end
// execution, on ICX. This mode only support single thread.
enum PerfCountMode {
    Disabled,
    Chrono,
    BackendSpecific,
...
public:
    ...
    const io_container& get_IO_ops() const {
        return m_io_expressions;
    }
