...
private:
     ...
     * @param output_buffers unordered map [Expression -> set of output ports (one)] which represents output Buffers of
     * Loop
     * @param outer_loop_end_expr_it iterator of Linear IR which refers to the expression with outer LoopEnd
     */
    void parse_nested_loops(const BufferPorts& input_buffers,
                            const BufferPorts& output_buffers,
     ...
     */
    static bool are_buffer_neighbours(const ExpressionPtr& up,
                                      const ExpressionPtr& down,
                                      ExpressionPtr& loop,
                                      size_t& up_idx,
                                      size_t& down_idx);
    /**
     ...
     */
    bool unite_nested_clusters(const AllocateBuffers::BufferClusters::iterator& inner_cluster_it,
                               AllocateBuffers::BufferCluster& outer_cluster,
                               const ExpressionPtr& outer_buffer,
                               bool is_outer_up);
