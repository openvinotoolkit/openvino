 ...
 *         - If MemoryAccess op or Loop can read and write to the same (inplace behavior), the Buffers should be in the
 * one cluster.
...
private:
    ...
    /**
     * @brief Returns True if Buffer is direct source for the target expr (there aren't other loop between the Buffer
     * and target expr)
    ...
    /**
     * @brief Creates new buffer cluster if buffer_exprs is missed in clusters. If buffer_exprs is already in clusters,
     * do nothing
    ...
    /**
     * @brief Returns common ID of cluster if all buffer inside have the same Buffer ID. Otherwise returns the default
     * value SIZE_MAX that means that Buffers in cluster have different IDs.
    ...
    /**
     * @brief Analyzes Loop: if Loop has Buffer ops on inputs and outputs, Loop can read and write from/to the same
     * memory.
    ...
    /**
     * @brief Analyzes full MemoryAccess op: if the op has Buffer ops on I/O, the op can read and write from/to the same
     * memory.
     ...
     */
    static std::pair<BufferMap, BufferMap> get_direct_buffers(const UnifiedLoopInfoPtr& loop_info,
     ...
     */
    void parse_nested_loops(const LoopManagerPtr& loop_manager,
                            const BufferMap& input_buffers,
                            const BufferMap& output_buffers,
     ...
     */
    static bool init_buffer_last_loop_port_info(const LoopManagerPtr& loop_manager,
                                                const BufferExpressionPtr& buffer_expr,
     ...
     */
    bool unite_nested_clusters(const LoopManagerPtr& loop_manager,
                               const BufferClusters::iterator& inner_cluster_it,
                               BufferCluster& outer_cluster,
                               const BufferExpressionPtr& outer_buffer,
                               bool is_outer_up);
