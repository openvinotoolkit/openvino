 ...
 * @param m_cluster_id - number of cluster. The Buffers from the same cluster shares memory between them and will have
 * the same offset.
 * @ingroup snippets
 */
...
public:
    ...
    size_t get_reg_group() const {
        return m_reg_group;
    }
    size_t get_cluster_id() const {
        return m_cluster_id;
    }
    ...
    void set_reg_group(size_t reg_group) {
        m_reg_group = reg_group;
    }
    void set_cluster_id(size_t cluster) {
        m_cluster_id = cluster;
    }
    void set_allocation_size(size_t size) {
    ...
    bool is_independent_memory() const {
