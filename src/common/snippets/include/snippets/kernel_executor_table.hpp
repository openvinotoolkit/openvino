...
public:
    ...
    /**
     * @brief check current config and recompile kernel if necessary. Use kernel caching to avoid redundant
     * recompilations. This method must be called only for complete configs. It's the user responsibility to check
     * is_completed() before calling.
     */
