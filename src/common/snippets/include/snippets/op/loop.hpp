...
public:
    ...
    // flag is needed for performance optimizations, do scalar can reuse work_amount set by vector Loop
    // if set:
    // * LoopEnd doesn't pop reg_work_amount from the stack on exit (to be disabled after assign_registers refactoring)
    // * LoopBegin doesn't set reg_work_amount (it assumes it's already correct)
