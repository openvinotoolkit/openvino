 ...
 *                 each the corresponding MemoryAccess op for Buffer should have offset for common memory pointer of
 * this Buffer
...
public:
    ...
    const ov::Shape& get_allocation_shape() const {
        return m_shape;
    }
    void set_allocation_shape(const ov::Shape& allocation_shape) {
        m_shape = allocation_shape;
    }
