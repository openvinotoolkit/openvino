...
TransposeTransformation::TransposeTransformation(const Params& params) : LayerTransformation(params) {
    ...
    auto matcher = pattern::wrap_type<ov::opset1::Transpose>(
...
void transposeDequantizationConstant(std::shared_ptr<Node>& transpose,
    ...
    if ((subtractShape.empty() || (subtractShape.size() == 1ul)) &&
        (multiplyShape.empty() || (multiplyShape.size() == 1ul))) {
        return;
    }

    auto transposeDeqConstant = [](const std::shared_ptr<ov::opset1::Constant>& dequantizationConstant,
    ...
    if (dequantization.subtract != nullptr) {
        const auto constant = transposeDeqConstant(dequantization.subtractConstant,
    ...
    if (dequantization.multiply != nullptr) {
        const auto constant = transposeDeqConstant(dequantization.multiplyConstant,
                                                   transpose->get_output_partial_shape(0),
                                                   transpose->get_input_node_shared_ptr(1));
...
bool TransposeTransformation::canBeTransformed(const TransformationContext& context, std::shared_ptr<Node> op) const {
    ...
    const std::shared_ptr<ov::opset1::Constant> constant =
        ov::as_type_ptr<ov::opset1::Constant>(op->get_input_node_shared_ptr(1));
    if (constant == nullptr) {
        return false;
    }
    ...
    auto checkShape = [](const std::shared_ptr<ov::opset1::Constant>& dequantizationConstant,
                         const PartialShape& transposeOutputShape) -> bool {
        ...
        if (dequantizationShape.empty() || (dequantizationShape.size() == 1ul) ||
            (dequantizationShape.size() == rankValue)) {
            return true;
        }
    ...
    return !dequantization.empty() &&
           ((dequantization.subtract == nullptr) ||
            checkShape(dequantization.subtractConstant, op->get_output_partial_shape(0))) &&
           ((dequantization.multiply == nullptr) ||
            checkShape(dequantization.multiplyConstant, op->get_output_partial_shape(0)));
}
