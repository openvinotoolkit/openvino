...
//          Multiply
//
ov::pass::low_precision::ConvertSubtractConstant::ConvertSubtractConstant(
    ...
    auto weightsConvertWrapper =
        ov::pass::pattern::wrap_type<opset1::Convert>({weightsConstantWrapper}, pattern::consumers_count(1));
    auto subtractConstantWrapper = ov::pass::pattern::wrap_type<opset1::Constant>(pattern::consumers_count(1));
    auto subtractWrapper =
        ov::pass::pattern::wrap_type<opset1::Subtract>({weightsConvertWrapper, subtractConstantWrapper},
                                                       pattern::consumers_count(1));
    auto multiplyConstantWrapper = ov::pass::pattern::wrap_type<opset1::Constant>(pattern::consumers_count(1));
    auto multiplyWrapper = ov::pass::pattern::wrap_type<opset1::Multiply>({subtractWrapper, multiplyConstantWrapper},
                                                                          pattern::consumers_count(1));
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) -> bool {
        ...
        if (!constantPrecisions.empty()) {
            ...
            if (std::find(constantPrecisions.begin(), constantPrecisions.end(), inputPrecision) ==
                constantPrecisions.end()) {
                return false;
            }
        }
        ...
        if (resultSubtractConstant == nullptr) {
            ...
            const auto newMultiply =
                std::make_shared<opset1::Multiply>(weightsConvert,
                                                   opsMap.at(multiplyConstantWrapper).get_node_shared_ptr());
            ...
            const auto resultConvert =
                std::make_shared<opset1::Convert>(resultSubtractConstant, dequantizationPrecision);
            ...
            const auto newSubtract =
                std::make_shared<opset1::Subtract>(opsMap.at(weightsConvertWrapper).get_node_shared_ptr(),
                                                   resultConvert);
