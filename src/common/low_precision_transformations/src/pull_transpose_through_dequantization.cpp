...
std::shared_ptr<Node> moveThroughElementwise(const std::shared_ptr<Node>& transpose,
                                             const std::shared_ptr<Node>& elementwise) {
    ...
    if (elementwiseValuesShape.size() != shape_size(transposeValuesShape)) {
        ...
        elementwiseValues = ov::pass::low_precision::fold<opset1::Broadcast>(
            elementwiseValues,
            std::make_shared<opset1::Constant>(element::i64,
                                               Shape{shape_size(transposeValuesShape)},
                                               std::vector<size_t>(shape_size(transposeValuesShape), 1ul)));
        assert(ov::is_type<opset1::Constant>(elementwiseValues));
    }

    const std::shared_ptr<opset1::Transpose> newTranspose = ov::as_type_ptr<opset1::Transpose>(
        transpose->clone_with_new_inputs({elementwise->get_input_node_shared_ptr(0ul), transposeValues}));

    const auto newElementwiseValues =
        ov::pass::low_precision::fold<opset1::Transpose>(elementwiseValues, transposeValues);
    ...
    const auto newElementwise = elementwise->clone_with_new_inputs(
        {newTranspose,
         elementwiseValuesConvert == nullptr
             ...
             : std::make_shared<opset1::Convert>(newElementwiseValues,
...
void fuseConstant(const std::shared_ptr<Node>& transpose, const std::shared_ptr<Node>& constant) {
    const auto newConstant = ov::pass::low_precision::fold<opset1::Transpose>(constant, transpose->input_value(1));
    ...
    ov::matcher_pass_callback callback = [=](ov::pass::pattern::Matcher& m) -> bool {
        ...
        while (transpose != nullptr) {
            ...
            if (ov::is_type<opset1::Multiply>(parent) || ov::is_type<opset1::Subtract>(parent)) {
                ...
                pull_transpose_through_dequantization::fuseConstant(transpose,
                                                                    ov::as_type_ptr<opset1::Constant>(parent));
                transpose = nullptr;
            } else {
