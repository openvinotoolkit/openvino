...
bool ShuffleChannelsTransformation::transform(TransformationContext& context, ov::pass::pattern::Matcher& m) {
    ...
    const auto shuffleChannels = ov::as_type_ptr<opset1::ShuffleChannels>(
...
bool ShuffleChannelsTransformation::canBeTransformed(const TransformationContext& context,
    ...
    const FakeQuantizeDequantization dequantization =
        NetworkHelper::getDequantization(shuffleChannels, defaultPrecisions);
    ...
    if (shuffleChannels->get_input_partial_shape(0).rank().is_dynamic() && shuffleChannels->get_axis() < 0) {
        const bool perChannelSub =
            dequantization.subtractConstant ? ov::shape_size(dequantization.subtractConstant->get_shape()) > 0 : false;
        const bool perChannelMul =
            dequantization.multiplyConstant ? ov::shape_size(dequantization.multiplyConstant->get_shape()) > 0 : false;
