...
bool FakeQuantizeTransformation::transform(ov::pass::pattern::Matcher &m) {
    const auto layer = ov::as_type_ptr<op::v0::FakeQuantize>(m.get_match_root());
    if (!layer || !QuantizationDetails::outputLayoutIsSupported(layer)) {
        return false;
    }

    bool wasHandled = false;
    std::shared_ptr<op::v0::FakeQuantize> fakeQuantize = layer;
    do {
...
std::shared_ptr<Node> updateShape(std::shared_ptr<Node> constantOp, const PartialShape& targetShape) {
    ...
    if ((shape.size() > 1ul) && (shape.size() < static_cast<size_t>(targetShape.rank().get_length()))) {
        constantOp = fold<op::v0::Unsqueeze>(
            constantOp,
            std::make_shared<op::v0::Constant>(ov::element::i32, Shape{ 1 }, std::vector<size_t>({ 0ul })));
    }
    return constantOp;
}

std::shared_ptr<Node> getDataNode(const std::shared_ptr<Node>& eltwise) {
    if (!ov::is_type<op::v0::Constant>(eltwise->get_input_node_shared_ptr(0))) {
        return eltwise->get_input_node_shared_ptr(0);
    }

    if (!ov::is_type<op::v0::Constant>(eltwise->get_input_node_shared_ptr(1))) {
...
std::shared_ptr<op::v0::Constant> getConstant(const std::shared_ptr<Node>& eltwise) {
    if (eltwise->get_input_size() != 2) {
        return nullptr;
    }

    std::shared_ptr<op::v0::Constant> constant = ov::as_type_ptr<op::v0::Constant>(eltwise->get_input_node_shared_ptr(1));
    if (constant != nullptr) {
        return constant;
    }

    return ov::as_type_ptr<op::v0::Constant>(eltwise->get_input_node_shared_ptr(0));
}
...
bool FakeQuantizeTransformation::checkElementwise(const std::shared_ptr<Node>& eltwise) {
    const std::shared_ptr<op::v0::Constant> constant = fq::getConstant(eltwise);
    if (constant == nullptr) {
        return false;
    }
...
std::shared_ptr<op::v0::FakeQuantize> FakeQuantizeTransformation::fuseElementwise(
    ...
    std::shared_ptr<op::v0::Constant> constant = fq::getConstant(eltwise);
    if (ov::is_type<op::v1::Multiply>(eltwise) && checkElementwise(eltwise)) {
        const auto value = foldConvert(constant, element::f32);

        const auto valueVec = ov::as_type_ptr<op::v0::Constant>(value)->cast_vector<float>();

        if (std::any_of(valueVec.cbegin(), valueVec.cend(), [](const float value) { return value <= 0.f; })) {
            return nullptr;
        }

        inputLowConst_f32 = fold<op::v1::Divide>(inputLowConst_f32, value);
        inputHighConst_f32 = fold<op::v1::Divide>(inputHighConst_f32, value);
    ...
    } else if (ov::is_type<op::v1::Subtract>(eltwise) && checkElementwise(eltwise)) {
        const auto value = foldConvert(constant, element::f32);

        inputLowConst_f32 = fq::updateShape(fold<op::v1::Add>(inputLowConst_f32, value), fakeQuantize->get_output_partial_shape(0));
        inputHighConst_f32 = fq::updateShape(fold<op::v1::Add>(inputHighConst_f32, value), fakeQuantize->get_output_partial_shape(0));
    } else if (ov::is_type<op::v1::Add>(eltwise) && checkElementwise(eltwise) && !ov::marked_as_bias(eltwise)) {
        const auto value = foldConvert(constant, element::f32);
        inputLowConst_f32 = fq::updateShape(fold<op::v1::Subtract>(inputLowConst_f32, value), fakeQuantize->get_output_partial_shape(0));
        inputHighConst_f32 = fq::updateShape(fold<op::v1::Subtract>(inputHighConst_f32, value), fakeQuantize->get_output_partial_shape(0));
    } else if (ov::is_type<op::v0::Convert>(eltwise)) {
        // issue #40611
    ...
    const auto newFakeQuantize = ov::as_type_ptr<op::v0::FakeQuantize>(fakeQuantize->clone_with_new_inputs({
