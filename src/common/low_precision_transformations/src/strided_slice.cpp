...
std::shared_ptr<ov::op::v0::Constant> stridedSliceDeqConstant(
    const std::shared_ptr<Node> node,
    const std::shared_ptr<Node> dequantizaiton_constant) {
    const auto constant = ov::as_type_ptr<ov::op::v0::Constant>(dequantizaiton_constant);
    ...
    // step #1: align shapes
    std::shared_ptr<ov::op::v0::Constant> new_constant = constant;
    ...
    if (rank != new_constant_shape.size()) {
        ...
        if (original_constant_shape != new_constant_shape) {
            const auto result = fold<ov::op::v1::Broadcast>(
                constant,
                ov::op::v0::Constant::create(ov::element::i32, { new_constant_shape.size() }, new_constant_shape));
            new_constant = ov::as_type_ptr<ov::op::v0::Constant>(result);
        }
    }

    // step #2: update original begin & end & strides
    auto cast_vector = [](const std::shared_ptr<ov::op::v1::StridedSlice>& strided_slice, const size_t i) {
    ...
    const auto strided_slice = ov::as_type_ptr<ov::op::v1::StridedSlice>(node);
    ...
    const auto result = fold<ov::op::v1::StridedSlice>(
        new_constant,
        std::make_shared<ov::op::v0::Constant>(element::i64, Shape{ begin.size() }, begin),
        std::make_shared<ov::op::v0::Constant>(element::i64, Shape{ end.size() }, end),
        std::make_shared<ov::op::v0::Constant>(element::i64, Shape{ strides.size() }, strides),
        begin_mask,
        end_mask,
    ...
    return ov::as_type_ptr<ov::op::v0::Constant>(NetworkHelper::toScalarIfPossible(result));
}
...
StridedSliceTransformation::StridedSliceTransformation(const Params& params) : LayerTransformation(params) {
    ...
    auto matcher = ov::pass::pattern::wrap_type<ov::op::v1::StridedSlice>();
...
bool StridedSliceTransformation::canBeTransformed(const std::shared_ptr<Node>& operation) const {
    if (!ov::is_type<ov::op::v1::StridedSlice>(operation)) {
        return false;
    }
