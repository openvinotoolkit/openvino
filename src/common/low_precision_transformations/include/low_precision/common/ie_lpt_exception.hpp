...
#define THROW_IE_LPT_EXCEPTION(node) \
    throw ::ov::pass::low_precision::InferenceEngineLptException(__FILE__, __LINE__, node)
...
#define THROW_TRANSFORMATION_EXCEPTION \
    throw ::ov::pass::low_precision::Exception() << __FILE__ << ":" << __LINE__ << " "
...
public:
    InferenceEngineLptException(const std::string& filename, const size_t line, const Node& node) {
        *this << filename << ":" << line << " Exception during low precision transformation for " << node
              << " node with type '" << node.get_type_name() << "', name '" << node.get_friendly_name() << "'. ";
    }
