...
public:
    ...
    static std::shared_ptr<Node> setOutDataPrecisionForTypeRelaxed(std::shared_ptr<OperationType> operation,
                                                                   const element::Type& precision);

    template <typename OperationType>
    static std::shared_ptr<Node> setOutDataPrecision(std::shared_ptr<OperationType> operation,
                                                     const element::Type& precision);
    ...
    static std::shared_ptr<Node> swapMultiplyAndAdd(std::shared_ptr<ov::opset1::Add> addAfterMultiply,
                                                    const int multiplyBranch);

    static void copyInfo(const std::vector<std::shared_ptr<Node>>& sources,
    ...
    static void copyInfo(const std::shared_ptr<Node>& source,
                         const std::shared_ptr<Node>& target,
                         bool overrideName = true);
    ...
    static std::shared_ptr<Node> getConstantInput(const std::shared_ptr<const Node>& node,
                                                  const bool convertIsExpected = false);

    static std::vector<size_t> updateReshapeValues(const Shape& elementwiseConstantShape,
    ...
    static std::shared_ptr<ov::opset1::FakeQuantize> composeFakeQuantize(
    ...
    static std::shared_ptr<ov::opset1::FakeQuantize> updateFakeQuantize(std::shared_ptr<ov::opset1::FakeQuantize> fq,
                                                                        element::Type precision,
                                                                        float min,
                                                                        float max,
                                                                        const bool replace = true);

    static FakeQuantizeDequantization makeDequantization(const float dequantizationMul,
    ...
    static std::shared_ptr<ov::Node> makeDequantizationSubtract(const ov::Output<ov::Node>& parent,
    ...
    static std::shared_ptr<ov::opset1::Constant> normalizeDequantizationShape(const std::shared_ptr<Node>& eltwise,
    ...
    public:
        InsertDequantizationResult(const std::shared_ptr<Node>& newOperation,
                                   const std::shared_ptr<Node>& lastDequantization)
    ...
    static InsertDequantizationResult moveDequantizationBefore(const std::shared_ptr<ov::Node>& operation,
                                                               const FakeQuantizeDequantization& dequantization,
                                                               const bool moveSubtract);
    ...
    static bool checkConstantValuePrecision(const element::Type expectedPrecision,
    ...
    static std::shared_ptr<ov::Node> separateInStandaloneBranch(
    ...
    static std::vector<element::Type> precisionIntersection(const std::vector<element::Type>& v1,
                                                            const std::vector<element::Type>& v2) noexcept;

    static bool isPrecisionPreserved(const std::shared_ptr<ov::Node>& node);

    static void insertDequantizationAfter(const std::shared_ptr<Node>& originalNode,
    ...
    static size_t calculateLevels(const float dataPrecisionMin,
...
private:
    static std::shared_ptr<Node> foldFakeQuantize(const std::shared_ptr<ov::opset1::FakeQuantize>& fq,
                                                  const bool roundValues,
...
template <typename OperationType>
std::shared_ptr<Node> NetworkHelper::setOutDataPrecisionForTypeRelaxed(std::shared_ptr<OperationType> layer,
                                                                       const element::Type& precision) {
...
template <typename OperationType>
std::shared_ptr<Node> NetworkHelper::setOutDataPrecision(std::shared_ptr<OperationType> layer,
                                                         const element::Type& precision) {
    // check if it already exteded operation node
    if (auto relaxed_layer = std::dynamic_pointer_cast<ov::op::TypeRelaxedBase>(layer)) {
        relaxed_layer->set_overridden_output_type(precision);
