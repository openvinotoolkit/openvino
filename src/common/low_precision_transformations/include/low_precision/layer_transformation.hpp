...
public:
    ...
    bool empty() const noexcept {
        assert(((precision == element::undefined) && (min == 0.f) && (max == 0.f) && (!hasZeroPoint)) ||
    ...
    static bool isSupported(const element::Type& precision) {
        static const std::set<element::Type_t> lowPrecision =
            {element::i8, element::u8, element::i16, element::u16, element::i32, element::u32};
    ...
    static float getMaxValue(const size_t maxLevelsForPrecision) {
        std::set<size_t> validLevels = {levels::int4,
    ...
    static element::Type getPrecision(const std::vector<float>& outputLowValues,
...
inline bool operator==(const DataPrecision& value1, const DataPrecision& value2) {
    return (value1.precision == value2.precision) && (value1.min == value1.min) && (value1.max == value1.max);
}
...
public:
    class Params {
    public:
        Params(const bool updatePrecisions = true,
    ...
    static PrecisionDetails getPrecisionDetails(const size_t quantizationLevels,
    ...
    // weights specific
    static DataPrecision getDataPrecision(const std::shared_ptr<Node>& layer,
...
protected:
    ...
    static void printDequantizationValues(const std::vector<float>& dequantizationScales,
...
protected:
    std::shared_ptr<ov::Node> moveDequantizationAfter(TransformationContext& context,
                                                      const std::shared_ptr<ov::Node>& operation,
                                                      const FakeQuantizeDequantization& dequantization,
                                                      const bool updateOutputPrecision = true,
                                                      const bool moveSubtract = true) const;

    std::shared_ptr<ov::Node> moveDequantizationBefore(TransformationContext& context,
                                                       const std::shared_ptr<ov::Node>& operation,
                                                       const FakeQuantizeDequantization& dequantization,
                                                       const bool moveSubtract = true) const;

    bool updateOutput(TransformationContext& context,
                      std::shared_ptr<ov::Node> lastNode,
                      std::shared_ptr<ov::Node> originalNode) const;

    void updateOutput(TransformationContext& context,
                      std::shared_ptr<ov::Node> lastNode,
