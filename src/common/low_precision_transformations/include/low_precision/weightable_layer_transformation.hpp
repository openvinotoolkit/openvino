...
public:
    struct LP_TRANSFORMATIONS_API CanBeTransformedParams {
        CanBeTransformedParams(const bool constantWeight = true,
    ...
    bool canConvolutionBeTransformed(const TransformationContext& context,
                                     std::shared_ptr<Node> layer,
                                     const std::vector<ov::element::Type>& defaultPrecisions) const;
    bool isPrecisionPreserved(std::shared_ptr<Node> layer) const noexcept override;

    static bool checkPrecisionOnActivation(const std::shared_ptr<const ov::Node>& node,
