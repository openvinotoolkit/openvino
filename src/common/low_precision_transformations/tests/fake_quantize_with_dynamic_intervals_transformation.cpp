...
typedef std::tuple<ngraph::element::Type, ngraph::Shape, FakeQuantizeWithDynamicIntervalsTransformationTestValues>
...
public:
    void SetUp() override {
        ...
        transform.add<ngraph::pass::low_precision::FakeQuantizeTransformation, ngraph::opset1::FakeQuantize>(
            testValues.params);
...
private:
    std::shared_ptr<ngraph::Function> get(ngraph::element::Type precision,
        ...
        const auto inputLow = inputLowConst
                                  ? std::dynamic_pointer_cast<ngraph::Node>(
                                        std::make_shared<opset1::Constant>(constantPresition, constantShape, low))
                                  : std::make_shared<ngraph::opset1::Parameter>(constantPresition, constantShape);

        const auto inputHigh = inputLowConst
                                   ? std::dynamic_pointer_cast<ngraph::Node>(
                                         std::make_shared<opset1::Constant>(constantPresition, constantShape, high))
                                   : std::make_shared<ngraph::opset1::Parameter>(constantPresition, constantShape);
        ...
        auto fakeQuantize =
            std::make_shared<ngraph::opset1::FakeQuantize>(input, inputLow, inputHigh, outputLow, outputHigh, levels);
