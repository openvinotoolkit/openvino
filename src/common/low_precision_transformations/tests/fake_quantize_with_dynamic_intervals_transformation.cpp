...
inline std::ostream& operator<<(std::ostream& out,
                                const FakeQuantizeWithDynamicIntervalsTransformationTestValues& testValue) {
    return out << "_" << testValue.inputLowConst << "_" << testValue.inpuHighConst << "_" << testValue.outputLowConst
               << "_" << testValue.outputHighConst;
}

typedef std::tuple<ov::element::Type, ov::Shape, FakeQuantizeWithDynamicIntervalsTransformationTestValues>
    FakeQuantizeTransformationParams;

class FakeQuantizeWithDynamicIntervalsTransformation
...
public:
    void SetUp() override {
        ...
        actualFunction = get(precision,
                             shape,
                             testValues.inputLowConst,
                             testValues.inpuHighConst,
                             testValues.outputLowConst,
                             testValues.outputHighConst);

        SimpleLowPrecisionTransformer transform;
        transform.add<ov::pass::low_precision::FakeQuantizeTransformation, ov::op::v0::FakeQuantize>(testValues.params);
        transform.transform(actualFunction);

        referenceFunction = get(precision,
                                shape,
                                testValues.inputLowConst,
                                testValues.inpuHighConst,
                                testValues.outputLowConst,
...
private:
    std::shared_ptr<ov::Model> get(ov::element::Type precision,
                                   ov::Shape inputShape,
        ...
        const auto inputLow = inputLowConst
                                  ? std::dynamic_pointer_cast<ov::Node>(
                                        std::make_shared<ov::op::v0::Constant>(constantPresition, constantShape, low))
                                  : std::make_shared<ov::op::v0::Parameter>(constantPresition, constantShape);

        const auto inputHigh = inputLowConst
                                   ? std::dynamic_pointer_cast<ov::Node>(
                                         std::make_shared<ov::op::v0::Constant>(constantPresition, constantShape, high))
                                   : std::make_shared<ov::op::v0::Parameter>(constantPresition, constantShape);
        ...
        const auto outputHigh =
            outputHighConst ? std::dynamic_pointer_cast<ov::Node>(
                                  std::make_shared<ov::op::v0::Constant>(constantPresition, constantShape, high))
                            : std::make_shared<ov::op::v0::Parameter>(constantPresition, constantShape);
        ...
        auto fakeQuantize =
            std::make_shared<ov::op::v0::FakeQuantize>(input, inputLow, inputHigh, outputLow, outputHigh, levels);
...
const std::vector<ov::element::Type> precisions = {ov::element::f32, ov::element::i32, ov::element::f16};
