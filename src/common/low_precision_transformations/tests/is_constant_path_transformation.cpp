...
TEST(LPT, isConstantPathFQAfterInputTransformation) {
    ...
    const auto fqOnActivations =
        makeFakeQuantize(input, ov::element::f32, FakeQuantizeOnData{256ul, {}, {0.f}, {2.55f}, {0.f}, {2.55f}});
...
TEST(LPT, isConstantPathMatMulParentFQTransformation) {
    ...
    const auto dq2 = makeDequantization(input2, DequantizationOperations{ov::element::f32, {128.f}, {0.1f}});
    const auto matmul = std::make_shared<ov::opset1::MatMul>(dq1, dq2);
    const auto fqAfterMatMul =
        makeFakeQuantize(matmul, ov::element::f32, FakeQuantizeOnWeights{255ul, {}, {0.f}, {254.f}, {-1.27f}, {1.27f}});
...
TEST(LPT, isConstantPathConvParentDqTransformation) {
    ...
    const auto conv = std::make_shared<ov::opset1::Convolution>(input,
                                                                weights,
                                                                ov::Strides{1, 1},
...
TEST(LPT, isConstantPathGroupConvParentDqTransformation) {
    ...
    const auto groupConv = std::make_shared<ov::opset1::GroupConvolution>(input,
                                                                          weights,
                                                                          ov::Strides{1, 1},
                                                                          ov::CoordinateDiff{0, 0},
                                                                          ov::CoordinateDiff{0, 0},
