...
#    define ADD_MATCHER_OBJ_0(obj, nspace, region, ...) \
        std::cout << #obj << " " << #nspace << "::" << #region << " ADD_MATCHER_OBJ_0\n";
...
#    define REGISTER_PASS_1(obj, nspace, region, ...)   \
        obj.register_pass<nspace::region>(__VA_ARGS__); \
        std::cout << #obj << " " << #nspace << "::" << #region << " REGISTER_PASS_1\n";
#    define REGISTER_PASS_0(obj, nspace, region, ...) \
        std::cout << #obj << " " << #nspace << "::" << #region << " REGISTER_PASS_0\n";
#    define REGISTER_PASS_SCOPE(obj, nspace, region, ...)                                 \
        OV_PP_CAT(REGISTER_PASS_, OV_CC_SCOPE_IS_ENABLED(OV_PP_CAT3(ov_pass, _, region))) \
        (obj, nspace, region, __VA_ARGS__)

#    define REGISTER_PASS_WITH_FALSE_1(obj, nspace, region, ...) \
        obj.register_pass<nspace::region, false>(__VA_ARGS__);   \
        std::cout << #obj << " " << #nspace << "::" << #region << " REGISTER_PASS_WITH_FALSE_1\n";
#    define REGISTER_PASS_WITH_FALSE_0(obj, nspace, region, ...) \
        std::cout << #obj << " " << #nspace << "::" << #region << " REGISTER_PASS_WITH_FALSE_0\n";
#    define REGISTER_PASS_SCOPE_WITH_FALSE(obj, nspace, region, ...)                      \
        OV_PP_CAT(REGISTER_PASS_, OV_CC_SCOPE_IS_ENABLED(OV_PP_CAT3(ov_pass, _, region))) \
        (obj, nspace, region, __VA_ARGS__)
