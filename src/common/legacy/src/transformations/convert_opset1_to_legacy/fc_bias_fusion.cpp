...
// TODO refactor, take common code with previous, take FQ under consideration
// TODO fix potential issue with transpose and reshape
ngraph::pass::FullyConnectedBiasWithTransposeAndReshapeFusion::FullyConnectedBiasWithTransposeAndReshapeFusion() {
    auto m_fc = ngraph::pattern::wrap_type<op::FullyConnected>([](Output<Node> output) {
    ...
    auto transpose_before_pattern =
        ngraph::pattern::wrap_type<opset1::Transpose>({m_fc, const_transpose_before_pattern});
    ...
    auto m_bias = pattern::any_input();
    auto m_add = ngraph::pattern::wrap_type<opset1::Add>({m_bias, reshape_before_pattern});
