...
.. note::
   ...
   Note that for the LLM pipeline, the GPU is used only for inferenc, while token selection, tokenization, and 
...
its results `match those generated by Hugging Face <comparing-with-hugging-face-results>`__.
