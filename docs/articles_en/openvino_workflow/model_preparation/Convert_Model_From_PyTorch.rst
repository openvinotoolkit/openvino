...
If your model have ``dict`` as input, for example ``{"x": a, "y": b, "z": c}`` it will be decomposed to inputs of resulting OpenVINO model signature ``(a, b, c)``, where input ``a`` will have name ``x``, ``b`` - ``y``, ``c`` - ``z``.
