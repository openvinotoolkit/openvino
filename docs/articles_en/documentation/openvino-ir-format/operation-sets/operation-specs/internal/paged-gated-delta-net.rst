...
.. meta::
  :description: Learn about PagedGatedDeltaNet - a paged, stateful GatedDeltaNet linear attention operation for memory-efficient LLM inference with grouped-query support.

**Versioned name**: *PagedGatedDeltaNet*

**Category**: *Internal*

**Short description**: *PagedGatedDeltaNet* implements paged, stateful GatedDeltaNet linear
attention with grouped-query support for memory-efficient batched LLM inference.

**Detailed description**:

The *PagedGatedDeltaNet* operation is the paged variant of the stateful GatedDeltaNet linear
...
.. code-block:: py
   :force:
    ...
    # Optional L2 normalization of query and key (when fuse_qk_l2norm is True)
    if fuse_qk_l2norm:
        q[t, h_q] = q[t, h_q] / max(norm(q[t, h_q]), q_l2_norm_eps)
        k[t, h_q] = k[t, h_q] / max(norm(k[t, h_q]), k_l2_norm_eps)
    ...
    # S[h_v]: recurrent state, shape [key_head_dim, value_head_dim]
    r = k[t, h_q] @ S[h_v]                            # shape: [value_head_dim]

    # Gated delta update
    S[h_v] = gate[t, h_v] * S[h_v] \
           + beta[t, h_v] * outer(k[t, h_q], v[t, h_v] - r)

    # Project with query to get output
    output[t, h_v] = q[t, h_q] @ S[h_v]              # shape: [value_head_dim]
...
* *q_l2_norm_eps*
  ...
  * **Description**: Epsilon value used as the minimum denominator when L2-normalizing query
    vectors. Only used when ``fuse_qk_l2norm`` is ``True``.
