...
.. code-block:: py
   ...
   def PagedAttentionExtension(
       ...
       xattention_threshold,                    # input 17  [B_seq] or empty  f16/f32
       ...
       token_type_ids,                                   # input 25  [T] or [T,1] or empty  i32
       qq_bias,                                            # input 26  [Nbias] or empty  u8
       qq_bias_begins,                                     # input 27  [B_seq+1] or empty  i32
   ):
       # ---------------------------------------------------------------
...
.. note::
   ...
   **Input ports are positional and cannot be skipped.**  OpenVINO does not support
   ...
   position N, making all subsequent indices incorrect.  For this reason, all 28
...
* **9**: ``scale`` - scalar tensor of type *T_real*, shape ``[]``.  Attention
...
* **26**: ``qq_bias`` - 1D tensor of type ``u8``, shape ``[Nbias]``, or an empty tensor
  (``size = 0``).  Packed per-sequence query-query attention bias masks.  The raw byte
  buffer encodes triangular or custom causal masks that are applied additively to
  attention logits during prefill, enabling non-standard attention patterns (e.g.
  document-level masking in multi-document prompts).  The per-sequence byte ranges are
  delimited by ``qq_bias_begins``.  Empty = no custom bias applied.
  **Required (always present; empty tensor = disabled).**

* **27**: ``qq_bias_begins`` - 1D tensor of type ``i32``, shape ``[B_seq+1]``, or an
  empty tensor (``size = 0``).  Per-sequence byte offsets into ``qq_bias``.
  ``qq_bias_begins[s]`` to ``qq_bias_begins[s+1]`` delimits the bias bytes for
  sequence ``s``.  Empty = no custom bias.
  **Required (always present; empty tensor = disabled).**
...
* ``Nbias`` - total bytes in the packed ``qq_bias`` buffer.
...
6. **Sinks (input 20) on non-x86_64 platforms**
   ...
   currently guarded by ``orgInput == ID_SINKS + 1`` (i.e. 21 inputs), so for the
   current 28-input format the rejection is inactive and the model will be accepted
...
.. code-block:: xml
   ...
   <!-- Const inputs for ports 5–13 and 18–19, 21 (non-empty scalar / vector values).
        Ports 11, 14–17, 20, 22–27 feed 0-element Const nodes (ALiBi, RoPE trig LUT,
        rotation deltas, xattention threshold, sinks, adaptive-RKV fields,
        token_type_ids, and qq_bias are all disabled for this example). -->
   ...
   <!-- port 26: qq_bias - empty (custom attention bias disabled) -->
   <layer id="26" name="qq_bias/const" type="Const" version="opset1">
       <data element_type="u8" shape="0" offset="76" size="0"/>
       <output><port id="0" precision="U8"/></output>
   </layer>

   <!-- port 27: qq_bias_begins - empty (custom attention bias disabled) -->
   <layer id="27" name="qq_bias_begins/const" type="Const" version="opset1">
       <data element_type="i32" shape="0" offset="76" size="0"/>
       <output><port id="0" precision="I32"/></output>
   </layer>

   <layer id="28" name="paged_attn" type="PagedAttentionExtension" version="extension">
       <input>
           ...
           <port id="26" precision="U8"/>  <!-- qq_bias: empty (custom attention bias disabled) -->
           <port id="27" precision="I32"/> <!-- qq_bias_begins: empty (custom attention bias disabled) -->
       </input>
       <output>
           <port id="28" precision="FP32">  <!-- attention_output [T, Hq*Sv] -->
               <dim>2</dim>
               <dim>512</dim>
           </port>
           <port id="29" precision="FP32">  <!-- score_aggregation [T + sum(past_lens)] = [2+32+63=97] -->
               <dim>97</dim>
           </port>
           <port id="30" precision="FP32">  <!-- diversity_scores: empty (eviction disabled) -->
           </port>
       </output>
   </layer>
