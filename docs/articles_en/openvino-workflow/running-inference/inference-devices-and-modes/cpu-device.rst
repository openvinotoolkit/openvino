...
The default floating-point precision of a CPU primitive is ``f32``. To support the ``f16`` OpenVINO IR on platforms that does not natively support ``float16`` the plugin internally converts
