...
Below guide presents instructions on how to use the OpenVINO™ Security Add-on to create, distribute, and use models that are created with the OpenVINO™ toolkit:

* **Model Developer**: Responsible for interactions with the Independent Software Vendor to control the User access to models. This article shows how to setup hardware and virtual machines to use the OpenVINO™ Security Add-on to define access control to your OpenVINO™ models, and then provide the access controlled models to the users. 
* **Independent Software Vendor**: This guide contains instructions expalining how to use the OpenVINO™ Security Add-on to validate license for access controlled models that are provided to your customers (users). 
* **User**: This guide includes instructions for end users who need to access and run access controlled models through the OpenVINO™ Security Add-on.  

In this release, one person performs duties of both the Model Developer and the Independent Software Vendor. Therefore, this guide provides instructions regarding configuration of one system for these two roles and one system for the User role. This document also provides a way for the same person to play the role of the Model Developer, Independent Software Vendor, and User, to let you see how the OpenVINO™ Security Add-on functions from the User perspective.


## Overview

The OpenVINO™ Security Add-on works with the [OpenVINO™ Model Server](@ref ovms_what_is_openvino_model_server) on Intel® architecture. Together, they provide a way for Model Developers and Independent Software Vendors to use secure packaging and secure model execution to enable access control to the OpenVINO™ models, and for model Users to run inference within assigned limits.
...
- The Model Developer generates an access controlled model from the OpenVINO™ toolkit output. This model uses the model's Intermediate Representation (IR) files to create an access controlled output file archive, distributed to Model Users. The Developer can also put the archive file in long-term storage or back it up without additional security.

- The Model Developer uses the OpenVINO™ Security Add-on Tool (<code>ovsatool</code>) to generate and manage cryptographic keys and related collateral for the access controlled models. Cryptographic material is only available in a virtual machine (VM) environment. The OpenVINO™ Security Add-on key management system allows the Model Developer to get external Certificate Authorities to generate certificates to add to a key-store. 
...
- The Independent Software Vendor hosts the OpenVINO™ Security Add-on License Service, which responds to license validation requests when a user attempts to load an access controlled model in a model server. The licenses are registered with the OpenVINO™ Security Add-on License Service.

- When a user loads the model, the OpenVINO™ Security Add-on Runtime contacts the License Service to confirm that the license is valid and within the parameters that the Model Developer defined with the OpenVINO™ Security Add-on Tool (<code>ovsatool</code>). The user must be able to reach the Independent Software Vendor's License Service over the Internet. 
...
User adds the access controlled model to the OpenVINO™ Model Server config file externally from the OpenVINO™ Security Add-on. The OpenVINO™ Model Server attempts to load the model in memory. Meanwhile, the OpenVINO™ Security Add-on Runtime component validates the user license for the access controlled model against information stored in the License Service provided by the Independent Software Vendor.
...
Each Model Developer, Independent Software Vendor, and User, must prepare one physical hardware machine and one Kernel-based Virtual Machine (KVM). In addition, each person must prepare a Guest Virtual Machine (Guest VM) for each role that person plays. 
...
| Kernel-based Virtual Machine (KVM)    | The OpenVINO™ Security Add-on runs in this virtual machine as it provides an isolated environment for security sensitive operations.        |
...
* The example in this guide uses self-signed certificates.
...
Start this step on the Intel® Core™ or Xeon® processor machine that meets the <a href="#prerequisites">prerequisites</a>.

> **NOTE**: As an alternative to manually following steps 1 - 11, you can run the script `install_host_deps.sh` in the `Scripts/reference directory` under the OpenVINO™ Security Add-on repository. The script stops with an error message if it identifies any issues. If the script stops due to an error, correct the issue that caused it and restart the script. The script runs for several minutes and provides progress information.
...
2. Make sure hardware virtualization support is enabled in the BIOS:
   ...
   If the `kvm-ok` command is not present, install it by running below script:
   ```sh
...
8.  Install the `tpm2-tss` [software tool](https://github.com/tpm2-software/tpm2-tss/releases/download/2.4.4/tpm2-tss-2.4.4.tar.gz).<br>
    Installation details can be found [here](https://github.com/tpm2-software/tpm2-tss/blob/master/INSTALL.md).
9.  Install the `tpm2-abmrd` [software tool](https://github.com/tpm2-software/tpm2-abrmd/releases/download/2.3.3/tpm2-abrmd-2.3.3.tar.gz).<br>
    Installation details can be found [here](https://github.com/tpm2-software/tpm2-abrmd/blob/master/INSTALL.md).
10. Install the `tpm2-tools` [software tool](https://github.com/tpm2-software/tpm2-tools/releases/download/4.3.0/tpm2-tools-4.3.0.tar.gz).<br>
    Installation details can be found [here](https://github.com/tpm2-software/tpm2-tools/blob/master/INSTALL.md).
...
As a result of above steps, following are installed and ready to use:
...
Now everything is ready to configure the Host Machine for networking. 
...
Example in this step uses the following names (Your configuration might use different names):
...
1. Open the network configuration file, from `/etc/netplan` location, for editing. File name should be like `50-cloud-init.yaml`
...
5. Run two commands to activate the updated network configuration file.

> **NOTE**: If you use ssh, you may lose network connectivity when issuing these commands. In that case, reconnect to the network.
...
For each separate role you perform, you must prepare a guest virtual machine (Guest VM). Because in this release, the Model Developer and Independent Software Vendor roles are combined, these instructions will guide you to set up one Guest VM, named `ovsa_isv`.

Begin these steps on the Host Machine. 

Alternatively, you can use `virsh` and the virtual machine manager to create and bring up a Guest VM. For more details regarding this option, see the `libvirtd` documentation.
...
2. Create an empty virtual disk image to serve as the Guest VM for your role as Model Developer and Independent Software Vendor:
   ...
   ```
3. Install Ubuntu 18.04 on the Guest VM, and name it: `ovsa_isv`:
   ```sh
...
5. Follow the prompts on the screen to finish installation of the Guest VM. Name the VM as `ovsa_isv_dev`
...
8. Choose ONE of these options to install additional required software:
   * **Option 1**: Installation via script
   ...
   * **Option 2** : Manual installation
      1. Install the `tpm2-tss` [software tool](https://github.com/tpm2-software/tpm2-tss/releases/download/2.4.4/tpm2-tss-2.4.4.tar.gz). 
      Installation details can be found [here](https://github.com/tpm2-software/tpm2-tss/blob/master/INSTALL.md)
      2. Install the `tpm2-abmrd` [software tool](https://github.com/tpm2-software/tpm2-abrmd/releases/download/2.3.3/tpm2-abrmd-2.3.3.tar.gz). 
      Installation details can be found [here](https://github.com/tpm2-software/tpm2-abrmd/blob/master/INSTALL.md)
      3. Install the `tpm2-tools` [software tool](https://github.com/tpm2-software/tpm2-tools/releases/download/4.3.0/tpm2-tools-4.3.0.tar.gz). 
      Installation details can be found [here](https://github.com/tpm2-software/tpm2-tools/blob/master/INSTALL.md)
...
1. Choose **ONE** of these options to create a Guest VM for the User role:<br>
   **Option 1: Copy and Rename the `ovsa_isv_dev_vm_disk.qcow2` disk image**
   1. Copy the `ovsa_isv_dev_vm_disk.qcow2` disk image to a new image named `ovsa_runtime_vm_disk.qcow2`. The `ovsa_isv_dev_vm_disk.qcow2` disk image was created previously in <a  href="#prerequisites">Step 3</a>.
   ...
   **Option 2: Manually create the Guest VM**
   ...
   ```
   2. Install Ubuntu 18.04 on the Guest VM. As a name use `ovsa_runtime`:
   ```sh
   sudo qemu-system-x86_64 -m 8192 -enable-kvm \
   -cpu host \
   ...
   4. Follow the prompts on the screen to finish installing the Guest VM. As a name us `ovsa_runtime`.
   ...
   7. Choose **ONE** of these options to install additional required software:
      ...
      **Option 2: Manually install additional software**
      1. Install the `tpm2-tss` [software tool](https://github.com/tpm2-software/tpm2-tss/releases/download/2.4.4/tpm2-tss-2.4.4.tar.gz) <br>
      Installation details can be found [here](https://github.com/tpm2-software/tpm2-tss/blob/master/INSTALL.md) <br><br>
      2. Install the `tpm2-abmrd` [software tool](https://github.com/tpm2-software/tpm2-abrmd/releases/download/2.3.3/tpm2-abrmd-2.3.3.tar.gz) <br>
      Installation details can be found [here](https://github.com/tpm2-software/tpm2-abrmd/blob/master/INSTALL.md) <br><br>
      3. Install the `tpm2-tools` [software tool](https://github.com/tpm2-software/tpm2-tools/releases/download/4.3.0/tpm2-tools-4.3.0.tar.gz) <br>
      Installation details can be found [here](https://github.com/tpm2-software/tpm2-tools/blob/master/INSTALL.md) <br><br>
      4. Install the [Docker packages](https://docs.docker.com/engine/install/ubuntu/) 
...
Follow the below steps to build and Install OpenVINO™ Security Add-on on host and other VMs.

### Step 1: Build the OpenVINO™ Model Server image 
Building OpenVINO™ Security Add-on depends on OpenVINO™ Model Server docker containers. To download and build OpenVINO™ Model Server on the host Follow instructions below:
...
In case of using more than one Host Machine repeat Step 3 on each.

### Step 4: Install the OpenVINO™ Security Add-on Model Developer / ISV Components
This step is for the combined role of Model Developer and Independent Software Vendor. The Guest VM is referenced as `ovsa_runtime`.
...
This step is for the User role. The Guest VM is referenced as `ovsa_runtime`.

The Model Hosting components install the OpenVINO™ Security Add-on Runtime Docker container based on OpenVINO™ Model Server NGINX Docker to host an access controlled model. 
...
This guide uses the [face-detection-retail-0004](@ref omz_models_model_face_detection_retail_0044) model as an example. 

The following figure describes the interactions between the Model Developer, Independent Software Vendor, and User.

**Keep in minde** that, the Model Developer/Independent Software Vendor and User roles are related to virtual machine use and one person might fill the tasks required by multiple roles. In this guide the tasks of Model Developer and Independent Software Vendor are combined and use the Guest VM named `ovsa_isv`. It is possible to have all roles set up on the same Host Machine.
...
The Guest VM is referenced as `ovsa_isv_dev`. Log on to the Guest VM as `ovsa` user.
...
This example uses a self-signed certificate for demonstration purposes. In a production environment, use CSR files to request for a CA-signed certificate.
   ...
   ```
   Below files are created along with the keystore file:
...
1. Obtain artefacts from the User who needs access to an access controlled model:
...
2. Create a customer license configuration:
   ...
   ```
   **NOTE**: The parameter `/opt/ovsa/certs/server.crt` contains the certificate used by the License Server. The server certificate will be added to the customer license and validated while used. For more information, refer to [OpenVINO™ Security Add-on License Server Certificate Pinning](https://github.com/openvinotoolkit/security_addon/blob/release_2021_4/docs/ovsa_license_server_cert_pinning.md)
...
The Guest VM is referenced as `ovsa_rumtime`. Log on to the Guest VM as `ovsa` user.
...
1. Communicate your need for a model to the Model Developer. The Developer will ask you to provide the certificate from your key store and other informations. Below example uses the length of time, which the model needs to be available. 
...
3. Copy the artefacts from the Model Developer:
   ...
   ```
4. Rename and edit `sample.json` to include the names of the access controlled model artefacts you received from the Model Developer. The file will look like this:
   ```sh
   {
...
The NGINX Model Server publishes the access controlled model.
   ...
   ```
For information about the NGINX interface, refer to this [document](https://github.com/openvinotoolkit/model_server/blob/main/extras/nginx-mtls-auth/README.md).
...
3. Copy the `face_detection.py` from the example_client in `/opt/ovsa/example_client`:
   ...
   ```
4. Copy the sample images for inferencing. An image directory, that includes a sample image for inferencing, is created.
   ```sh
...
## Summary
By following this guide, You have completed all below tasks:
...
- As the Model Developer or Independent Software Vendor, accessed a controlled model and prepared a license for it.
- As the Model Developer or Independent Software Vendor, prepared and ran a License Server used to verify if User had a valid license to use an access controlled model.
- As the User, provided information to a Model Developer or Independent Software Vendor to get an access controlled model and the license for the model.
- As the User, set up and launched a Host Server on which licensed and access controlled models can be run.
- As the User, loaded a access controlled model, validated the license for the model, and used the model to run inference.
