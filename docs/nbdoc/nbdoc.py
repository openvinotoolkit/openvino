...
from utils import add_content_below, create_content, verify_notebook_name
...
def add_glob_directive(tutorials_file):
    ...
    with open(tutorials_file, "r+", encoding="cp437") as mainfile:
        ...
        if ":glob:" not in readfile:
            add_glob = readfile.replace(":hidden:\n", ":hidden:\n   :glob:\n").replace(
...
class NbTravisDownloader:
    ...
    def download_from_jenkins(
        path: str = notebooks_path, artifact_link: str = artifacts_link
    ):
        ...
        def traverse(
            path: Path, link: str, blacklisted_extensions: list = blacklisted_extensions
        ):
            ...
            for file in files:
                if is_directory(file):
                    ...
                    len(Path(file).suffix) > 0
                    and Path(file).suffix not in blacklisted_extensions
                ):
...
class NbProcessor:
    ...
    def add_binder(
        ...
        buttons_list: list,
        cbuttons_list: list,
        template_with_colab_and_binder: str = binder_colab_template,
        template_without_binder: str = no_binder_template,
    ):
        ...
        if not os.path.exists(openvino_notebooks_ipynb_list):
            ...
            with open(
                openvino_notebooks_ipynb_list, "r+", encoding="cp437"
            ) as ipynb_file:
        ...
        for notebook_file in [
            ...
            nb_path_match = [
                line
                for line in openvino_notebooks_paths_list.split("\n")
            ...
            local_install = (
                ...
                + "/"
                + repo_name
                + "#-installation-guide' target='_blank' title='Install "
                + notebook_item
                + " locally'>local installation</a> \n\n"
            )
            binder_badge = (
                ".. raw:: html\n\n   <a href='"
                + notebooks_binder
                + nb_repo_path
                + "' target='_blank' title='Launch "
                + notebook_item
                + " in Binder'><img src='data:image/svg+xml;base64,"
                + binder_image_base64
                + "' class='notebook-badge' alt='Binder'></a>\n\n"
            )
            colab_badge = (
                ".. raw:: html\n\n   <a href='"
                + notebooks_colab
                ...
                + notebook_item
                + " in Google Colab'><img src='data:image/svg+xml;base64,"
                + colab_image_base64
                + "' class='notebook-badge'alt='Google Colab'></a>\n\n"
            )
            github_badge = (
                ".. raw:: html\n\n   <a href='"
                + notebooks_repo
                ...
                + notebook_item
                + " on Github'><img src='data:image/svg+xml;base64,"
                + github_image_base64
                + "' class='notebook-badge' alt='Github'></a><br><br>\n\n"
            )
...
def main():
    ...
    main_tutorials_file = Path(
        "../../docs/articles_en/learn-openvino/interactive-tutorials-python.rst"
    ).resolve(strict=True)
