...
*   **2**: `selected_scores` -  tensor of type *T_THRESHOLDS* and shape `[min(num_boxes, max_output_boxes_per_class) * num_batches * num_classes, 3]` containing information about scores for each selected box as triplets `[batch_index, class_index.
