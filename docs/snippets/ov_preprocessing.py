...
from openvino.preprocess import ColorFormat, ResizeAlgorithm
from utils import get_advanced_model, get_model, get_path_to_model, get_temp_dir
...
ppp.input(input_name).tensor().set_element_type(Type.u8).set_shape(
    [1, 480, 640, 3]
...
ppp.input(input_name).preprocess().convert_element_type(Type.f32).convert_color(
    ColorFormat.RGB
).resize(ResizeAlgorithm.RESIZE_LINEAR).mean([100.5, 101, 101.5]).scale(
    [50.0, 51.0, 52.0]
)
...
ppp.input().preprocess().scale(50.0)
...
ppp.input(input_name).preprocess().mean([103.94, 116.78, 123.68]).scale(
    [57.21, 57.45, 57.73]
)
# ! [ov:preprocess:mean_scale_array]
...
ppp.input(input_name).preprocess().convert_layout([0, 3, 1, 2])
# 0 -> 0; 3 -> 1; 1 -> 2; 2 -> 3
# ! [ov:preprocess:convert_layout_2]
...
ppp.input(input_name).preprocess().resize(ResizeAlgorithm.RESIZE_LINEAR, 480, 640)
# ! [ov:preprocess:resize_1]
...
ppp.input(input_name).tensor().set_color_format(ColorFormat.NV12_TWO_PLANES)
...
ppp.input("input").preprocess().custom(custom_abs)
# ! [ov:preprocess:custom]
...
ppp.output("result").tensor().set_layout(Layout("NHWC")).set_element_type(Type.u8)
...
ppp.output("result").postprocess().custom(custom_abs)
# ! [ov:preprocess:postprocess]

# ! [ov:preprocess:save_headers]
from openvino.preprocess import ColorFormat, PrePostProcessor, ResizeAlgorithm

from openvino import Core, Layout, Type, serialize, set_batch
...
ppp.input().tensor().set_element_type(Type.u8).set_spatial_dynamic_shape().set_layout(
    Layout("NHWC")
...
ppp.input().preprocess().convert_element_type().convert_color(ColorFormat.RGB).resize(
