...
ppp.input(input_name).model().set_layout(ov.Layout('NCHW'))
# ! [ov:preprocess:model]
# ! [ov:preprocess:steps]
import openvino as ov
...
# same for output
ppp.output() \
    .postprocess().convert_element_type(ov.Type.u8)
# ! [ov:preprocess:input_1]
...
ppp.input('input').tensor().set_element_type(ov.Type.u8)

# Then define preprocessing step
ppp.input('input').preprocess().convert_element_type(ov.Type.f32)
...
ppp.input('input').tensor().set_layout(ov.Layout('NHWC'))
...
ppp.input('input').model().set_layout(ov.Layout('??HW'))
ppp.input('input').preprocess()\
    .resize(ov.preprocess.ResizeAlgorithm.RESIZE_LINEAR, 480, 640)
# ! [ov:preprocess:resize_1]
...
ppp.output('result_image').model().set_layout(ov.Layout('NCHW'))
...
ov.serialize(model, '/path/to/some_model_saved.xml', '/path/to/some_model_saved.bin')

# Second method - using Manager and Serialize pass
manager = Manager()
manager.register_pass(ov.Serialize('/path/to/some_model_saved.xml', '/path/to/some_model_saved.bin'))
