...
int main() {
    ...
    ppp.input("input_image").preprocess().custom([](const ov::Output<ov::Node>& node) {
    ...
    ppp.output("result_image").tensor().set_layout("NHWC").set_element_type(ov::element::u8);

    // Also it is possible to insert some custom operations
    ppp.output("result_image").postprocess().custom([](const ov::Output<ov::Node>& node) {
