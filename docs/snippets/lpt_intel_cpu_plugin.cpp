...
int main() {
    ...
    //! [lpt_common]
    // check if the function is quantized to ignore LPT transformations for not quantized function to speed up model
    // loading
    ...
    if (useLpt) {
        // convert not supported cases FakeQuantize -> Convert -> Convert -> Subtract -> Multiply to a single
        // FakeQuantize
        pass_config->set_callback<ov::pass::ConvertQuantizeDequantize>(
            [&defaultPrecisions](const std::shared_ptr<const ov::Node>& node) -> bool {
        ...
        // convert not supported cases FakeQuantize -> Convert -> Convert -> Subtract -> Multiply to a single
        // FakeQuantize
        pass_config->set_callback<ov::pass::ConvertSubtract>(
            [&defaultPrecisions](const std::shared_ptr<const ov::Node>& node) -> bool {
                return ov::pass::low_precision::NetworkHelper::areQuantizeAndDequantizeSupportedForSubtract(
                    node,
                    defaultPrecisions);
            });
    }
    ...
    if (useLpt) {
        ...
        lptManager.get_pass_config()->set_callback<MarkupPrecisions>(
        ...
        lptManager.get_pass_config()->set_callback<ConvolutionBackpropDataTransformation>(
            [&defaultPrecisions](const std::shared_ptr<const ov::Node>& node) -> bool {
        ...
        lptManager.get_pass_config()->set_callback<MultiplyToGroupConvolutionTransformation>(
            [](const std::shared_ptr<const ov::Node>& node) -> bool {
