...
* Installer (recommended) - [Linux](@ref openvino_docs_install_guides_installing_openvino_linux) | [Windows](@ref openvino_docs_install_guides_installing_openvino_windows) | [macOS](@ref openvino_docs_install_guides_installing_openvino_macos)
...
To save disk space for your IR file, you can apply [weights compression to FP16](../MO_DG/prepare_model/FP16_Compression.md). To generate an IR with a specific precision, run Model Optimizer with the appropriate `--data_type` option.
