...
### Disabling Workflow's Run in Draft PRs

To reduce the number of unnecessary running workflows in PRs not yet ready for review, we strongly recommend including the following "if" condition to Smart CI job:
```yaml
jobs:
  Smart_CI:
    runs-on: ubuntu-latest
    ...
    ...
```
This will ensure that the workflow won't be executed automatically for PRs in draft status. You will still be able to execute selected workflows for draft PRs manually by re-building the corresponding Smart CI job. The easiest way to do that: 

From the main page of the draft PR, go to "Checks" tab -> in the left menu choose the workflow that you'd like to execute -> press "Re-run all jobs" button.
...
>**NOTE**: If you add the condition above, make sure to update the triggers section of the worfklow like that, to ensure that all checks will be re-executed once the PR is converted from the draft to "ready to review":
