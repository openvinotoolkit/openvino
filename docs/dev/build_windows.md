...
4. Build generated solution in Visual Studio or run `cmake --build . --config Release --verbose -j<number_of_jobs>` to build from the command line. View number of processing units available with `WMIC cpu get numberofLogicalProcessors`. Be aware that this process may take some time.
