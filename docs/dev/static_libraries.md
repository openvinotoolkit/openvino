...
GPU builds statically with the default OCL runtime and require no extra flags. Select another existing runtime
explicitly with `-DGPU_RUNTIME_TYPE=ZE` or `-DGPU_RUNTIME_TYPE=SYCL`.
