...
Beginning from 2020.4 release OpenVINO™ supports to read native ONNX models to nGraph representation.
...
OpenVINO™ doesn't provide a mechanism to specify pre-processing (like mean values subtraction, reverse input channels) for ONNX format.
If ONNX model contains dynamic shapes for input, please use `CNNNetwork::reshape` method for shape specialization.
