...
A program that uses the `register_operator` functionality requires `ngraph` and `onnx_ngraph_frontend` libraries in addition to the Inference Engine.
...
The `ngraph` package exposes two variables, `${NGRAPH_LIBRARIES}` and `${ONNX_FRONTEND_LIBRARIES}`, which reference the `ngraph` and `onnx_ngraph_frontend` libraries.
