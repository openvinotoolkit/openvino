...
* Linux* OS:
    - `libov_runtime.so`, which depends on `libtbb.so`, `libtbbmalloc.so` and `libngraph.so`
* Windows* OS:
    - `ov_runtime.dll`, which depends on `tbb.dll`, `tbbmalloc.dll` and `ngraph.dll`
* macOS*:
    - `libov_runtime.dylib`, which depends on `libtbb.dylib`, `libtbbmalloc.dylib` and `libngraph.dylib`
