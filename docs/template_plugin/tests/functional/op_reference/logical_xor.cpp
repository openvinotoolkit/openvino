...
struct LogicalXorParams {
    template <class IT, class OT>
    LogicalXorParams(const ngraph::PartialShape& input_shape1, const ngraph::PartialShape& input_shape2, const std::vector<IT>& iValues1,
...
class ReferenceLogicalXorLayerTest : public testing::TestWithParam<LogicalXorParams>, public CommonReferenceTest {
public:
    void SetUp() override {
    ...
    static std::string getTestCaseName(const testing::TestParamInfo<LogicalXorParams>& obj) {
...
private:
    static std::shared_ptr<Function> CreateFunction(const PartialShape& input_shape1, const PartialShape& input_shape2, const element::Type& input_type) {
        ...
        const auto logical_xor = std::make_shared<op::v1::LogicalXor>(in, in2);
        return std::make_shared<Function>(NodeVector {logical_xor}, ParameterVector {in, in2});
    }
};

TEST_P(ReferenceLogicalXorLayerTest, CompareWithHardcodedRefs) {
    Exec();
}

INSTANTIATE_TEST_SUITE_P(
    smoke_LogicalXor_With_Hardcoded_Refs, ReferenceLogicalXorLayerTest,
    ::testing::Values(LogicalXorParams(ngraph::PartialShape {2, 2}, ngraph::PartialShape {2, 2}, std::vector<char> {true, false, true, false},
                                      std::vector<char> {false, true, true, false}, std::vector<char> {true, true, false, false}),
                      LogicalXorParams(ngraph::PartialShape {2, 1, 2, 1}, ngraph::PartialShape {1, 1, 2, 1}, std::vector<char> {true, false, true, false},
                                      std::vector<char> {true, false}, std::vector<char> {false, false, false, false}),
                      LogicalXorParams(ngraph::PartialShape {3, 4}, ngraph::PartialShape {3, 4},
    ...
    ReferenceLogicalXorLayerTest::getTestCaseName);
