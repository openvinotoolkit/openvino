...
struct HSwishParams {
    template <class IT>
    HSwishParams(const PartialShape& shape, const element::Type& iType, const std::vector<IT>& iValues, const std::vector<IT>& oValues)
        : pshape(shape),
          inType(iType),
          outType(iType),
...
private:
    static std::shared_ptr<Function> CreateFunction(const PartialShape& input_shape, const element::Type& input_type,
        ...
        return std::make_shared<Function>(NodeVector {HSwish}, ParameterVector {in});
    }
};
