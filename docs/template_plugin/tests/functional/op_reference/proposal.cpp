...
private:
    static std::shared_ptr<Model> CreateFunction(const ProposalV1Params& params) {
        const auto class_probs_param = std::make_shared<op::v1::Parameter>(params.inType, params.clsScoreShape);
        const auto bbox_deltas_param = std::make_shared<op::v1::Parameter>(params.inType, params.bboxPredShape);
        const auto image_shape_param = std::make_shared<op::v1::Parameter>(params.inType, params.imageShapeShape);
        const auto Proposal = std::make_shared<op::v1::Proposal>(class_probs_param, bbox_deltas_param, image_shape_param, params.attrs);
