...
private:
    static std::shared_ptr<Model> CreateFunction(const ScatterElementsUpdateParams& params) {
        const auto A = std::make_shared<op::v1::Parameter>(params.input.type, params.input.shape);
        const auto B = std::make_shared<op::v1::Parameter>(params.indices.type, params.indices.shape);
        const auto C = std::make_shared<op::v1::Parameter>(params.updates.type, params.updates.shape);
        const auto D = std::make_shared<op::v1::Parameter>(params.axis.type, params.axis.shape);
