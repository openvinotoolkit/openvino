...
private:
    static std::shared_ptr<Model> CreateFunction(const PartialShape& data_shape, const element::Type& data_type, const PartialShape& axis_shape,
        ...
        const auto data_param = std::make_shared<op::v1::Parameter>(data_type, data_shape);
        const auto axis_param = std::make_shared<op::v1::Parameter>(axis_type, axis_shape);
        const auto cum_sum = std::make_shared<op::v3::CumSum>(data_param, axis_param, execlusive, reverse);
    ...
    static std::shared_ptr<Model> CreateFunction(const PartialShape& data_shape, const element::Type& data_type) {
        const auto data_param = std::make_shared<op::v1::Parameter>(data_type, data_shape);
        const auto cum_sum = std::make_shared<op::v3::CumSum>(data_param);
