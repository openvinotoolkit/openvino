...
// Hard Sigmoid activation function is unsupprted with v3::AUGRUCell
class ReferenceAUGRUCellTestHardsigmoidActivationFunction : public ReferenceAUGRUCellTest {
public:
    void SetUp() override {
        threshold = 1e-1f;
        auto params = GetParam();
...
TEST_P(ReferenceAUGRUCellTestHardsigmoidActivationFunction, CompareWithRefs) {
    Exec();
}
...
std::vector<AUGRUCellParams> generateParams() {
    ...
    std::vector<AUGRUCellParams> params {
        ...
        AUGRUCellParams(
            ...
            "augru_cell_linear_before_reset"),
    };
    return params;
}
...
template <element::Type_t ET>
std::vector<AUGRUCellParams> generateParamsHardsigmoidActivationFunction() {
    using T = typename element_type_traits<ET>::value_type;
    std::vector<AUGRUCellParams> params {
        AUGRUCellParams(
            2, 3, 3, 3, true,
            ...
            reference_tests::Tensor(ET, {2, 3}, std::vector<T>{
                0.8598948f, 0.41189128f, 0.72824323f, 0.53940123f, 0.31485787f, 0.04053852f}),
            "augru_cell_hardsigmoid_activation_function"),
    };
    return params;
}

std::vector<AUGRUCellParams> generateCombinedParamsHardsigmoidActivationFunction() {
    const std::vector<std::vector<AUGRUCellParams>> generatedParams {
...
INSTANTIATE_TEST_SUITE_P(smoke_AUGRUCell_With_Hardcoded_Refs, ReferenceAUGRUCellTestHardsigmoidActivationFunction,
    testing::ValuesIn(generateCombinedParamsHardsigmoidActivationFunction()), ReferenceAUGRUCellTest::getTestCaseName);
} // namespace
