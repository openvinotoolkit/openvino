...
struct LogicalAndParams {
    ...
    LogicalAndParams(const ngraph::PartialShape& input_shape1, const ngraph::PartialShape& input_shape2, const std::vector<IT>& iValues1,
...
private:
    static std::shared_ptr<Function> CreateFunction(const PartialShape& input_shape1, const PartialShape& input_shape2, const element::Type& input_type) {
        ...
        const auto logical_and = std::make_shared<op::v1::LogicalAnd>(in1, in2);
        return std::make_shared<Function>(NodeVector {logical_and}, ParameterVector {in1, in2});
    }
};
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(LogicalAndParams(ngraph::PartialShape {2, 2}, ngraph::PartialShape {2, 2}, std::vector<char> {true, false, true, false},
                                       std::vector<char> {false, true, true, false}, std::vector<char> {false, false, true, false}),
                      LogicalAndParams(ngraph::PartialShape {2, 1, 2, 1}, ngraph::PartialShape {1, 1, 2, 1}, std::vector<char> {true, false, true, false},
