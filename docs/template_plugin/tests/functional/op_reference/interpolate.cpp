...
private:
    static std::shared_ptr<Function> CreateFunction(const Shape& input_shape,
        ...
        return std::make_shared<Function>(NodeVector{interpolate}, ParameterVector{node_input});
    }
};
...
std::vector<InterpolateParams> generateParamsForInterpolate() {
    ...
    std::vector<InterpolateParams> params{
        InterpolateParams(ov::Shape{1, 1, 2, 4},
                          ...
                          std::vector<T>{1.0f, 3.0f},
                          {4},
                          element::i64,
                          {1.0},
                          op::v4::Interpolate::InterpolateAttrs())
    };
    return params;
}
...
INSTANTIATE_TEST_SUITE_P(
    smoke_Interpolate_With_Hardcoded_Refs,
