...
class FileHelper:
    def __init__(self, filetype, file, label, input_dir, output_dir, convert, file_to_label_mapping):
    ...
    def md_rst_convert(self):
        """
        Perform md->rst conversion
        """
        output = pypandoc.convert_text(self.content, 'rst', format='md')
        self.content = output
...
def process(filetype, input_dir, output_dir, exclude_dirs, convert):
    """
...
def main():
    ...
    parser.add_argument('--convert', action='store_true', help="Convert md input files to rst.")
    ...
    process(filetype, input_dir, output_dir, exclude_dirs, convert)
