...
def pytest_generate_tests(metafunc):
    ...
    for link in exclude_links:
        ...
        sphinx_ref_pattern = (
            "toctree contains reference to nonexisting document '{}".format(link)
        )
    ...
    xfail_list = [
        xfail.lower() for xfail in read_lists(metafunc.config.getoption("xfail"))
    ]
    ...
    if metafunc.config.getoption("doxygen"):
        doxy_parser = LogParser(
            metafunc.config.getoption("doxygen"),
    ...
    # read sphinx log
    sphinx_parser = LogParser(
        metafunc.config.getoption("sphinx"),
    ...
    if "errors" in metafunc.fixturenames:
        metafunc.parametrize(
            "errors",
            [
                pytest.param(errors, marks=marks[file]) if file in marks else errors
