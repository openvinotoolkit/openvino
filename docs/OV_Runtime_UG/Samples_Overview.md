...
The OpenVINO samples are simple console applications that shows how to:

* Operate specific OpenVINO API capabilities within an application, 
* Assist developers in executing tasks like loading a model, running inference, querying specific device capabilities and etc.

After Intel® Distribution of OpenVINO™ toolkit installation С, C++ and Python* sample applications are available in the following directories, respectively:
...
> **NOTE**: All C++ samples support input paths containing only ASCII characters, except the Hello Classification Sample which supports Unicode.
