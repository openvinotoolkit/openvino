...
The Multi-Device plugin automatically assigns inference requests to available computational devices to parallely execute the requests. By contrast, the Heterogeneous plugin can run different layers on different devices but not in parallel. The potential gains of using Multi-Device plugin are:

* Improved throughput from using multiple devices (compared to single-device execution)
* More consistent performance thanks to the devices sharing of the inference burden (if one device is too busy, another can take more of the load)

The logic within the Multi-Device application is left unchanged. There is no need to explicitly compile the model on every device, create and balance the inference requests and so on. From the application point of view, this is just another device that handles the actual machinery. The only thing that is required to leverage performance is to provide the multi-device (and hence the underlying devices) with enough inference requests to process. For example, if you were processing 4 cameras on the CPU (with 4 inference requests), it might be desirable to process more cameras (with more requests in flight) to keep CPU and GPU busy via Multi-Device.

The setup of Multi-Device can be described in three major steps:

1. Prepare configuration for each device. 
2. Compile the model on the Multi-Device plugin created on top of a (prioritized) list of the configured devices with the configuration prepared in previous step.
3. As with any other CompiledModel call (resulting from `compile_model`), you create as many requests as required to saturate the devices.

Below You may check more detailed descriptions of the above steps.
...
You may set the priorities directly as a string.
...
The priorities of the devices can be changed in real time for the compiled model:
...
Beyond the trivial "CPU", "GPU", "HDDL" and so on, when multiple instances of a device are available the names are more advanced. Below is presented an example how two Intel® Movidius™ Myriad™ X sticks are listed with the hello_query_sample:
```
...
...
So the explicit configuration to use both would be "MULTI:MYRIAD.1.2-ma2480,MYRIAD.1.4-ma2480". Accordingly, the code that loops over all available devices of "MYRIAD" type only is:
...
As an alternative, You can combine all the individual device settings into a single config file and load that. This will allow the Multi-Device plugin to parse and apply settings to the right devices. Check the code example in the next section.

> **NOTE**: While the performance of accelerators combines really well with Multi-Device, the CPU+GPU execution poses some performance caveats, as these devices share the power, bandwidth and other resources. For example it is recommended to enable the GPU throttling hint (which save another CPU thread for the CPU inference). Check the [Using the Multi-Device with OpenVINO samples and benchmarking the performance](#using-the-multi-device-with-openvino-samples-and-benchmarking-the-performance) section below for more details.
...
Refer to the code of the `benchmark_app`, which exists in both  [C++](../../samples/cpp/benchmark_app/README.md) and [Python](../../tools/benchmark_tool/README.md), for more details.    
-   CPU+GPU execution performs better with certain knobs 
    which you can find in the code of the same [Benchmark App](../../samples/cpp/benchmark_app/README.md) sample.
    One specific example is disabling GPU driver polling. In turn it requires multiple GPU streams to amortize slower 
...
Logic within the Multi-Device application is left unchanged. There is no need to explicitly compile the model on every device, create and balance the inference requests and so on. From the application point of view, this is just another device that handles the actual machinery. The only requirement to leverage performance is to provide the multi-device (and hence the underlying devices) with enough inference requests to process. For example, processing 4 cameras on the CPU (with 4 inference requests), it might be desirable to process more cameras (with more requests in flight) to keep CPU and GPU busy via Multi-Device.
...
* Option 3 - Use Explicit Hints for Controlling Request Numbers Executed by Devices
   There is an option to specify the number of requests that Multi-Device will internally keep for each device. If the original app was running 4 cameras with 4 inference requests, it might be best to share these 4 requests between 2 devices used in the MULTI. The easiest option is to specify a number of requests for each device using parentheses: “MULTI:CPU(2),GPU(2)” and use the same 4 requests in the app. However, such an explicit configuration is not performance-portable and not recommended. The better option is to configure the individual devices and query the resulting number of requests to be used at the application level. Check [Configuring the Individual Devices and Creating the Multi-Device On Top](#configuring-the-individual-devices-and-creating-the-multi-device-on-top) for more details..


### Enumerating Available Devices
The OpenVINO Runtime API features a dedicated methods to count devices and their capabilities. Check the [Hello Query Device Python Sample](../../samples/python/hello_query_device/README.md) for more details. Below is an output from the sample (truncated to device names only):
...
A simple programmatic way to count the devices and use with the multi-device is following:
...
Beyond the standars like: "CPU", "GPU", or "HDDL", when multiple instances of a device are available the names are more advanced. For example, this is how two Intel® Movidius™ Myriad™ X sticks are listed with the hello_query_sample:
...
So the explicit configuration to use both would be "MULTI:MYRIAD.1.2-ma2480,MYRIAD.1.4-ma2480". Accordingly, the code that olny loops over all available devices of "MYRIAD" type is below:
...
Alternative option is to combine all the individual device settings into a single config file and load that, allowing the Multi-Device plugin to parse and apply settings to the right devices. Check the code example in the next section.

> **NOTE**: While the performance of accelerators works well with Multi-Device, the CPU+GPU execution poses some performance caveats, as these devices share power, bandwidth and other resources. For example it is recommended to enable the GPU throttling hint (which saves another CPU thread for CPU inferencing). More details are presented in the section below.
