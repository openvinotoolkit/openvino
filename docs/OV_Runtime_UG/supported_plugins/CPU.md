...
## Device Name
For the CPU plugin `"CPU"` device name is used. Even though there may be more than one socket on a platform, from the plugin's point of view, there is only one `"CPU"` device.
...
To check if the CPU device supports the bfloat16 data type use the [query device properties interface](./config_properties.md) to query ov::device::capabilities property, which should contain `BF16` in the list of CPU capabilities:
...
In case the model has been converted to bf16, `ov::hint::inference_precision` is set to `ov::element::bf16` and it can be checked via `ov::CompiledModel::get_property` call. The code below demonstrates how to get the element type:
...
To enable the simulation, one has to explicitly set `ov::hint::inference_precision` to `ov::element::bf16`.
...
> In that case, it is better to use `ov::hint::PerformanceMode::LATENCY` performance hint (see the [performance hints overview](@ref openvino_docs_OV_UG_Performance_Hints) for details).
...
The most flexible configuration is the fully undefined shape, when you do not apply any constraints to the shape dimensions. It is also the most convenient approach. Reducing the level of uncertainty, however, will bring performance gains.
You may reduce memory consumption through memory reuse. As a result, you achieve better cache locality, which in turn leads to better inference performance, if you explicitly set dynamic shapes with defined upper bounds.
...
See the [model caching overview](@ref openvino_docs_OV_UG_Model_caching_overview) for more details.
...
It means that [OpenVINO™ Extensibility Mechanism](@ref openvino_docs_Extensibility_UG_Intro) can be used for the plugin extension as well.
To enable fallback on a custom operation implementation, one has to override `ov::Op::evaluate` method in the derived operation class (see the [custom OpenVINO™ operations](@ref openvino_docs_Extensibility_UG_add_openvino_ops) for details).
...
See the [stateful models guide](@ref openvino_docs_OV_UG_network_state_intro) for details.
