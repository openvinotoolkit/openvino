...
See the [GPU configuration page](@ref openvino_docs_install_guides_configurations_for_intel_gpu) for more details on how to configure machine to use GPU plugin.
...
* If the system does not have an integrated GPU, then devices are enumerated starting from 0.
...
Then, the device name can be passed to `ov::Core::compile_model()` method:
...
See the [Multi-device execution page](../multi_device.md) for more details.
...
Consequently, automatic batching is automatically enabled when `ov::optimal_batch_size` is > 1 and `ov::hint::performance_mode(ov::hint::PerformanceMode::THROUGHPUT)` is set.
...
`log2(N)` (`N` - is an upper bound for batch dimension here) low-level execution graphs for batch sizes equal to powers of 2 to emulate dynamic behavior. Therefore, incoming infer request with specific batch size is executed via minimal combination of internal networks.
...
See the [dynamic shapes guide](../ov_dynamic_shapes.md) for more details.
...
Refer to the [RemoteTensor API](./GPU_RemoteTensor_API.md) for usage examples.

See the [preprocessing API guide](../preprocessing_overview.md) for more details.
...
See the [Model caching overview page](../Model_caching_overview.md) for more details.
...
## Limitations
In some cases GPU plugin may implicitly execute several primitives on CPU, using internal implementations, which may lead to an increase of CPU utilization.
...
Since the OpenVINO relies on the OpenCL&trade; kernels for the GPU implementation, many general OpenCL tips apply:
-	Prefer `FP16` inference precision over `FP32`, as the Model Optimizer can generate both variants, and the `FP32` is default. Also, consider the [int8 inference](@ref openvino_docs_model_optimization_guide).
- 	Try to group individual infer jobs by using [automatic batching](../automatic_batching.md).
-	Consider the [caching](../Model_caching_overview.md) to minimize model load time.
-	If your application is simultaneously using the inference on the CPU or otherwise loads the host heavily, make sure that the OpenCL driver threads do not starve. You may use the [CPU configuration options](./CPU.md) to limit number of inference threads for the CPU plugin.
-	Even in the GPU-only scenario, a GPU driver might occupy a CPU core with spin-looped polling for completion. If the _CPU_ utilization is a concern, consider the dedicated reference in this document. Notice that this option might increase the inference latency. Therefore, try combining with multiple GPU streams or the [throughput performance hints](../performance_hints.md).
- When operating media inputs consider the [remote tensors API of the GPU Plugin](./GPU_RemoteTensor_API.md).
