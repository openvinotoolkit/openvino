...
The `ov::set_batch` method is a high level API of `ov::Model::reshape` functionality, so all information about `ov::Model::reshape` method implications is applicable for `ov::set_batch` too, including the troubleshooting section.
...
It is recommended to always set static shapes in case the shape of data is not going to change from one inference to another.
...
1) To change input shape of the model with single input, you may pass new shape into the method. See the example of adjusting spatial dimensions to the input image:
...
You will find usage scenarios of `reshape` feature in the [samples](Samples_Overview.md) starting with [Hello Reshape Sample](../../samples/cpp/hello_reshape_ssd/README.md).
...
Some operators which prevent normal shape propagation can be fixed. To do so, you may:
...
The [set_batch](api/ie_python_api/_autosummary/openvino.runtime.set_batch.html) method is a high level API of [Model.reshape](api/ie_python_api/_autosummary/openvino.runtime.Model.html#openvino.runtime.Model.reshape) functionality, so all information about [Model.reshape](api/ie_python_api/_autosummary/openvino.runtime.Model.html#openvino.runtime.Model.reshape) method implications is applicable for [set_batch](api/ie_python_api/_autosummary/openvino.runtime.set_batch.html) too, including the troubleshooting section.
...
1) To change input shape of model with single input you may pass new shape into the method. See the example of adjusting spatial dimensions to the input image:
...
You will find usage scenarios of `reshape` feature in the [samples](Samples_Overview.md), starting with [Hello Reshape Sample](../../samples/python/hello_reshape_ssd/README.md).
