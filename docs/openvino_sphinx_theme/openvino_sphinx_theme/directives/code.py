...
class DoxygenSnippet(LiteralInclude):
    ...
    option_spec = dict(
        {"fragment": directives.unchanged_required}, **LiteralInclude.option_spec
    )
...
def visit_scrollbox(self, node):
    ...
    attrs["style"] = (
        (
            ...
            if "height" in node is not None
            else ""
        )
        + (
            ("width:" + "".join(c for c in str(node["width"]) if c.isdigit()))
            if "width" in node is not None
            else ""
        )
        + (
        ...
        + (
            (
                "border-left:solid "
                + "".join(c for c in str(node["delimiter"]) if c.isdigit())
                + "px "
                + (
                    ("".join(str(node["delimiter-color"])))
                    if "delimiter-color" in node is not None
                    else "#dee2e6"
                )
                + "; "
            )
            if "delimiter" in node is not None
            else ""
        )
    )
...
def visit_showcase(self, node):
    attrs = {}
    notebook_file = (
        ("../../notebooks/" + node["title"] + "-with-output.html")
        if "title" in node is not None
        else ""
    )
    link_title = (
        (node["title"])
        if "title" in node is not None
        else "OpenVINO Interactive Tutorial"
    )
    ...
    self.body.append(
        "<img "
        + (
            " class='" + (node["img-class"] + " showcase-img' ")
            if "img-class" in node is not None
            else " class='showcase-img'"
        )
    )
    (
        self.body.append(
            ...
            + node["img"]
            + "' alt='"
            + os.path.basename(node["img"])
            + "' /></a></div>"
        )
        if "img" in node is not None
        else ""
    )

    self.body.append(
        "<div class='showcase-content'><div class='showcase-content-container'>"
    )
...
def depart_showcase(self, node):
    ...
    if not os.path.exists(openvino_notebooks_repo_listing):
        ...
        ipynb_list = [
            x for x in paths_list if re.match("notebooks/[0-9]{3}.*\.ipynb$", x)
        ]
    ...
    self.body.append("<a href='" + notebook_file + "' title='" + link_title + "'>")
    (
        self.body.append("<h2 class='showcase-title'>" + node["title"] + "</h2></a>")
        if "title" in node is not None
        else ""
    )
    ...
    self.body.append(
        "</div><button class='showcase-button' type='button' title='"
        + link_title
