...
The `--compress_to_fp16` compression parameter in Model Optimizer allows generating IR with constants (e.g. weights for convolutions and matrix multiplications) compressed to `FP16` data type. For more details, refer to the [Compression of a Model to FP16](prepare_model/FP16_Compression.md) guide.
