...
base class and works when the following steps are performed.
...
After running Model Optimizer with additional parameter *`--tensorflow_custom_operations_config_update`* pointing to
...
list is important. This order defines the order in which Model Optimizer attaches input tensors to a new generated
...
*`mo.front.common.replacement.FrontReplacementFromConfigFileGeneral`*. Model Optimizer executes the method
