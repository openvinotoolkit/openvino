...
Most likely, your model uses an operator that is not currently supported by OpenVINO. This error might be resolvable by [upgrading to the latest release version of OpenVINO](../../install_guides/installing-openvino-runtime.md). You can check which operators and layers are supported in the latest release of OpenVINO on the [Supported Framekwork Layers](Supported_Frameworks_Layers.md) page. If it isn't supported in the latest official release, you can check the [nightly documentation](https://docs.openvino.ai/nightly/openvino_docs_MO_DG_prepare_model_Supported_Frameworks_Layers.html) to see if it is supported in the nightly release. To install the nightly release (or other pre-release versions), check [Build OpenVINO From Source](https://github.com/openvinotoolkit/openvino/wiki/BuildingCode) for instructions on how to build the nightly release or the [Releases page on GitHub](https://github.com/openvinotoolkit/openvino/releases) to install pre-release versions of OpenVINO. If you tried to extend Model Optimizer with a new primitive for this model, you may have incorrectly specified the infer function: see the [OpenVINO™ Extensibility Mechanism](../../Extensibility_UG/Intro.md) page for more information.
