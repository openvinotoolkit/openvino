...
Input shape was not provided. That is mandatory for converting an MXNet model to the Intermediate Representation, because MXNet models do not contain information about input shapes. Use the `--input_shape` flag to specify it. For more information about using the `--input_shape`, refer to the FAQ [#56](#question-56).
...
This means that the layer `{layer_name}` is not supported in Model Optimizer. You will find a list of all unsupported layers in the corresponding section. You should implement the extensions for this layer ([OpenVINO&trade; Extensibility Mechanism](../../Extensibility_UG/Intro.md)).
...
Most likely, you have not specified inputs using `--mean_values`. Specify inputs with the `--input` flag. For usage examples, refer to the FAQ [#63](#question-63).

#### 61. What does the message "You should specify input for each scale value" mean? <a name="question-61"></a>

Most likely, you have not specified inputs using `--scale_values`. Specify inputs with the `--input` flag. For usage examples, refer to the FAQ [#64](#question-64).
...
The custom replacement configuration file provided with the `--tensorflow_custom_operations_config_update` cannot be parsed. Make sure that the file is correct and refer to the FAQs [#68](#question-68), [#69](#question-69), [#70](#question-70), and [#71](#question-71).
