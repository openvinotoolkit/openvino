...
For ONNX models are available two paths of IR conversion. 
The old one is handled by old Python implementation while the new one is processed via new C++ frontends. 
Since version 2022.1 the default path of IR conversion for ONNX models is handled via new ONNX frontend. 
Some features like `--extensions` and `--transformations_config` are unsupported yet on the new frontends. 
If a user does not choose expected path of conversion explicitly (by `--use_new_frontend` or `--use_legacy_frontend` MO arguments) and some unsupported pre-defined scenario is detected on the new frontend path, the IR conversion falls back to the old path.