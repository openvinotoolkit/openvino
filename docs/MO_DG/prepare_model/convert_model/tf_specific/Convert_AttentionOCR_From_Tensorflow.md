...
The original AOCR model contains data preprocessing, done in the following steps:
...
After that, the resized image is sent to the convolution neural network (CNN). The Model Optimizer does not support image decoding so you should cut off preprocessing part of the model using `--input` command line parameter.
```sh
mo \
