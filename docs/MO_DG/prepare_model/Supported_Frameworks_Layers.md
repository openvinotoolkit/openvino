...
Some of TensorFlow operations do not match to any OpenVINO operation. Yet, they are still supported by the Model Optimizer and can be used on constant propagation path. These layers are labeled *`Constant propagation`* in the table below:
