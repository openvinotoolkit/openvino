...
var languages = [ 'py', 'cpp', 'c' ]
    ...
    function ready() {
      ...
      const preselect = [] let changeLanguage = false
...
function getTabsState(currentPageHash) {
  allPagesState =
      JSON.parse(window.localStorage.getItem('sphinx-design-tabs-state')) || {}
  if (currentPageHash)
...
function saveCurrentTabsState(tabsState) {
  const checkedElements = [] for (label in sd_labels_by_text) {
  ...
  window.localStorage.setItem(
      'sphinx-design-tabs-state', JSON.stringify(tabsState))
}
