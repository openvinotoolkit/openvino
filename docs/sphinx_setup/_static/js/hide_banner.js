...
window.onload = (e) => {
  fetch('../_static/html/banner_data.json').then((data) => {
    data.json().then((item) => {
      if (item != null && item.p != "" && item.version != "" &&
          item.style != "") {
        cookieVersion = item.version;
        var transitionBanner = document.getElementById("info-banner");
        if (document.cookie.split(';')
                .filter(function(find_cookie_name) {
                  return find_cookie_name.trim().indexOf(
                             'TransitionBannerIsHiddenX' + cookieVersion +
                             '=') === 0;
                })
                .length) {
        ...
        button.addEventListener("click", function() {
          var cookieContent =
              'TransitionBannerIsHiddenX' + cookieVersion + '=true;';
          var expiry = 'expires=';
          var date = new Date();
