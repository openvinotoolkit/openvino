...
function search(query, selectedVersion, page, key) {
  ...
  var path = 'https://www.googleapis.com/customsearch/v1?cx=' + cx +
      '&q=' + query + '&siteSearch=' + siteSearch + '&start=' + start;
  ...
  return async function() {
    await gapi.client
        ...
        .then(function() {
          return gapi.client.request({'path' : path})
        })
        .then(
            function(response) {
...
function handleNoResults() {
  var $noResults =
      $('<div>', {'class' : 'gs-no-results', 'text' : 'No Results'});
...
function addVersionTabs(selectedVersion, query) {
  ...
  for (var i = 0; i < tab_versions.length; i++) {
    ...
    var $elem = $('<a>', {
      'class' : 'gs-tab',
      'href' :
          'search.html?version=' + tab_versions[i].version + '&query=' + query,
      'text' : tab_versions[i].version
    });
    if (tab_versions[i].version === 'ALL') {
      href = 'search.html?query=' + query;
    } else {
      href =
          'search.html?query=' + query + '&version=' + tab_versions[i].version
    }
    var $elem = $(
        '<a>',
        {'class' : 'gs-tab', 'href' : href, 'text' : tab_versions[i].version});
...
function renderResults(items) {
  ...
  $.each(items, function(i, item) {
    ...
    var $snippet = $('<div>', {
      'class' : 'gs-snippet',
      'html' : item['htmlSnippet'].replace(/\<br\>/g, '')
    });
    ...
    if (item.pagemap.hasOwnProperty('cse_thumbnail')) {
      ...
      var $image =
          $('<img>', {'src' : cse_thumbnail.src, 'alt' : 'Thumbnail image'});
      $link.append($image);
      $media.append($link);
    ...
    if (item.hasOwnProperty('fileFormat')) {
      ...
      var $fileFormat =
          $('<span>', {'class' : 'gs-file-format', 'text' : 'File format: '});
      var $type =
          $('<span>', {'class' : 'gs-file-type', 'text' : item['fileFormat']});
...
function addPagination(
  ...
  if (previousPage) {
    var $previous = $('<a>', {
      ...
      'href' : 'search.html?page=' + previousPage + '&query=' + query +
  ...
  for (var i = 0; i < pageList.length; i++) {
    $page = $('<a>', {
      ...
      'href' : 'search.html?page=' + pageList[i] + '&query=' + query +
          '&version=' + selectedVersion,
      'text' : pageList[i]
    });
  ...
  if (nextPage) {
    var $next = $('<a>', {
      ...
      'href' : 'search.html?page=' + nextPage + '&query=' + query +
          '&version=' + selectedVersion
    });
    $cursor.append($next);
  }
...
function getCurrentPath() {
  ...
  var currentPath =
      encodeURI(windowLocationArr[windowLocationArr.length - 1].replace(
