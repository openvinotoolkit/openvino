...
const newsletterModalPathVersion = ([ 'cn', 'jp' ].includes(currentPath[0]))
    ...
    : `/${currentPath[0]}`;
const newsletterModalPath =
    newsletterModalPathVersion + '/_static/html/newsletter.html';
...
$(document).ready(function() {
  ...
  waitForElement('#newsletterTrigger')
      .then((trigger) => { $(trigger).on('click', showForm); })

  // trigger with iframe
  // $('iframe').on('load', function() {
  //     $('iframe').contents().find('#newsletterTrigger').on('click',
  //     showForm);
  // });

  function showForm() {
    fetch(newsletterModalPath)
        ...
        .then((text) => {
          ...
          $("#newsletterForm").submit(function(event) {
            ...
            $(this).animate({opacity : 0}, 200, 'linear', () => {
              ...
              $(this).append(
                  `<input type="hidden" name="newsletter-pageSource" value="${
                      currentUrl}">`);
              ...
              for (var entry of rawFormData) {
                if (entry['name'].startsWith(newsletterFieldPrefix)) {
                  entry['name'] =
                      entry['name'].replace(newsletterFieldPrefix, '');
  ...
  function displayMessage(boxHeight, status, errorCode) {
    ...
    const iconBackground =
        $('<i class="fas fa-square fa-stack-2x newsletter-icon-background">');
