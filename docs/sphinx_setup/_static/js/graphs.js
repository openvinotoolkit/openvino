...
const chartDisclaimers = {
  Value :
      'Value: Performance/(No_of_sockets * Price_of_CPU_dGPU), where prices are in USD as of November 2023.',
  Efficiency :
      'Efficiency: Performance/(No_of_sockets * TDP_of_CPU_dGPU), where total power dissipation (TDP) is in Watt as of November 2023.'
}
...
const OVMSdefaultSelections = {
  platforms : {
    name : 'platform',
    data :
        [ 'Intel®  Xeon® 8260M CPU-only', 'Intel®  Xeon® Gold 6238M CPU-only' ]
  },
  models : {
    name : 'networkmodel',
    data : [
...
class Filter {
  ...
  static FilterByNetworkModel(graphDataArr, networkModels) {
    ...
    // Some of them might return dupes, so convert them to a map, and get unique
    // objects based on names
    const optionMap = new Map();
    networkModels
        .map(
            (model) => graphDataArr.filter(
                (graphData => graphData.networkModel === model)))
        .flat(1)
  ...
  static FilterByClientPlatforms(graphDataArr, platformsArr) {
    return graphDataArr.filter(
        (data) => platformsArr.includes(data.platformName));
  }
  ...
  // param: GraphData[] (of one networkModel), key (throughput, latency,
  // efficiency, value)
  static getKpiData(graphDataArr, key) {
    return graphDataArr.map((data) => { return data[key]; });
  }
}
    ...
    switch (kpi) {
      case 'throughput':
        return {
          ...
          datasets : precisions.map(
              (precision) => this.getPrecisionThroughputConfig(
                  precision, units.throughputUnit)),
        };
      case 'latency':
        return {
          chartTitle : 'Latency',
          ...
          datasets : precisions.map(
              (precision) =>
                  this.getPrecisionLatencyConfig(precision, units.latencyUnit)),
        };
      case 'value':
        return {
          chartTitle : 'Value',
    ...
    switch (precision) {
      case 'ovmsint8':
        return {
          data : null,
          color : '#FF8F51',
          label : `${unit} (OV Ref. INT8)`
        };
      case 'ovmsfp32':
        return {
          data : null,
          color : '#B24501',
          label : `${unit} (OV Ref. FP32)`
        };
      case 'int4':
...
$(document).ready(function() {
  ...
  function clickBuildGraphs(
      graph, networkModels, ietype, platforms, kpis, precisions) {
    ...
    $('.graph-chart-title-header').off('click').on('click', (event) => {
      ...
      if ($(parent)
              .children('.chart-wrap,.empty-chart-container')
              .is(":visible")) {
  ...
  function showModal(version) {
    ...
    Papa.parse(
        dataPath,
        {download : true, complete : (result) => renderModal(result, version)});
  }

  function getSelectedNetworkModels() {
    return $('.models-column-one input:checked, .models-column-two input:checked')
        .not('[data-networkmodel="Select All"]')
        .map(function() {
  ...
  function getSelectedIeType() {
    return $('.ietype-column input:checked')
        .map(function() {
  ...
  function getSelectedCoreTypes() {
    return $('.client-platform-column .selected')
        .map(function() {
  ...
  function getSelectedClientPlatforms() {
    return $('.client-platform-column input:checked')
        .map(function() {
  ...
  function getSelectedKpis() {
    return $('.kpi-column input:checked')
        .map(function() {
  ...
  function getSelectedPrecisions() {
    return $('.precisions-column input:checked')
        .map(function() {
  ...
  function validateSelections() {
    if (getSelectedNetworkModels().length > 0 && getSelectedIeType() &&
  ...
  function renderModal(result, version) {
    ...
    fetch('../_static/html/modal.html')
        ...
        .then((text) => {
          ...
          const models = networkModels.map(
              (networkModel) => createCheckMark(networkModel, 'networkmodel'));
          const selectAllModelsButton =
              createCheckMark('Select All', 'networkmodel')
          modal.find('.models-column-one')
              .append(selectAllModelsButton)
              .append(models.slice(0, models.length / 2));
          modal.find('.models-column-two')
              .append(models.slice(models.length / 2));

          const precisions = Modal.getPrecisionsLabels(version).map(
              (precision) => createCheckMark(precision, 'precision'));
          ...
          const kpiLabels = Modal.getKpisLabels(version).map(
              (kpi) => createCheckMark(kpi, 'kpi'));
          ...
          $('#build-graphs-btn').on('click', () => {
            ...
            clickBuildGraphs(
                graph,
                getSelectedNetworkModels(),
                getSelectedIeType(),
                getSelectedClientPlatforms(),
                getSelectedKpis(),
                Modal.getPrecisions(getSelectedPrecisions()));
          });
          $('.modal-close').on('click', hideModal);
          $('.close-btn').on('click', hideModal);
          modal.find('.models-column-one input[data-networkmodel="Select All"]')
              .on('click', function() {
                if ($(this).prop('checked'))
                  selectAllCheckboxes(models);
                else
                  deSelectAllCheckboxes(models);
              });
          modal.find('.ietype-column input')
              .on('click',
                  () =>
                      renderClientPlatforms(graph.data, modal, version, true));
          modal.find('.kpi-column input')
              .on('click', validateThroughputSelection);
  ...
  function validateThroughputSelection() {
    ...
    if (getSelectedKpis().includes('Throughput') ||
        getSelectedKpis().includes('Latency')) {
  ...
  function clearAll() {
    $('.modal-content-grid-container input:checkbox')
        .each((index, object) => $(object).prop('checked', false));
    // Uncomment if you want the Clear All button to reset the Platform Type
    // column as well modal.find('.ietype-column input').first().prop('checked',
    // true);
  ...
  function preselectDefaultSettings(data, modal, version) {
    const defaultSelections =
        (version == 'ov') ? OVdefaultSelections : OVMSdefaultSelections;
    ...
    if (defaultSelections.platformFilters) {
      const filters =
          modal.find('.selectable-box-container').children('.selectable-box');
      filters.removeClass('selected');
      defaultSelections.platformFilters.data.forEach(selection => {
        filters
            .filter(`[data-${defaultSelections.platformFilters.name}="${
                selection}"]`)
            .addClass('selected');
      });
      renderClientPlatforms(data, modal, version);
  ...
  function showCoreSelectorTypes(coreTypes, graphDataArr, modal) {
    ...
    $('.client-platform-column .selectable-box').on('click', function() {
      ...
      var fPlatforms = filterClientPlatforms(
          graphDataArr,
          getSelectedNetworkModels(),
          getSelectedIeType(),
          Modal.getCoreTypes(getSelectedCoreTypes()));
      renderClientPlatformsItems(
          modal, Graph.getPlatformNames(fPlatforms), true);
      validateSelections();
    });
  }
  ...
  function filterClientPlatforms(data, networkModels, ietype, coreTypes) {
    // No longer filtering on the network type, if at some point we want the
    // network type as a filter, uncomment this var first =
    // Filter.FilterByNetworkModel(data, networkModels);
  ...
  function renderClientPlatforms(data, modal, version, preselectEveryItem) {
    ...
    var fPlatforms = filterClientPlatforms(
        ...
        Modal.getCoreTypes(getSelectedCoreTypes()));
    renderClientPlatformsItems(
        modal, Graph.getPlatformNames(fPlatforms), preselectEveryItem);
  }

  function renderClientPlatformsItems(
      modal, platformNames, preselectEveryItem) {
    $('.client-platform-column .checkmark-container').remove();
    const clientPlatforms =
        platformNames.map((platform) => createCheckMark(platform, 'platform'));
  ...
  function selectAllCheckboxes(items) {
    items.forEach((item) => { item.find(':input').prop('checked', true); });
  }

  function enableAllCheckboxes(items) {
    items.forEach((item) => { item.find(':input').prop('disabled', false); })
  }

  function disableAllCheckboxes(items) {
    items.forEach((item) => { item.find(':input').prop('disabled', true); })
  }

  function deSelectAllCheckboxes(items) {
    items.forEach((item) => { item.find(':input').prop('checked', false); });
  }
  ...
  const htmlLegendPlugin = {
    ...
    afterUpdate(chart, args, options) {
      const ul = getOrCreateLegendList(
          chart, chart.options.plugins.htmlLegend.containerID);
  ...
  function getChartOptions(title, containerId) {
    return {
      responsive: true, indexAxis: 'y', maintainAspectRatio: false,
          title: {display: false, text: title}, scales: {
  ...
  function getChartDataNew(labels, datasets) {
    return {
      labels: labels, datasets: datasets.map((item) => {
        return {
          label: item.label, data: item.data, backgroundColor: item.color,
              borderColor: 'rgba(170,170,170,0)', barThickness: 10
        }
      })
    }
  }
  ...
  function renderData(
    ...
    $('.modal-footer').empty();
    const display =
        new ChartDisplay(getChartsDisplayMode(kpis.length), kpis.length);
    ...
    networkModels.forEach((networkModel) => {
      ...
      var chevronRight =
          '<span style="display:none" class="chevron-right-btn"></span>';
      $(chevronRight).hide();

      var chartContainerHeader =
          $(chevronDown + chevronRight + '<span class="graph-chart-title">' +
            networkModel + '</span>');
      ...
      var filteredNetworkModels =
          Filter.FilterByNetworkModel(graph.data, [ networkModel ]);
      var filteredIeTypes =
          Filter.FilterByIeType(filteredNetworkModels, ietype);
      var filteredGraphData =
          Filter.FilterByClientPlatforms(filteredIeTypes, platforms);

      $('.chart-placeholder').append(chartContainer);
      if (filteredGraphData.length > 0) {
        createChartWithNewData(
            filteredGraphData,
            chartContainer,
            kpis,
            ietype,
    ...
    $('.modal-footer')
        .append($(
            '<div class="modal-footer-content"><div class="modal-disclaimer-box"></div></div>'))
    for (let kpi of kpis) {
  ...
  function createEmptyChartContainer(chartContainer) {
    chartContainer.append($('<div>')
                              .addClass('empty-chart-container')
  ...
  function createChartWithNewData(
      model, chartContainer, kpis, ietype, precisions, display) {
    var chartWrap = $('<div>');
    ...
    graphConfigs.forEach((graphConfig, index) => {
      ...
      columnHeader.append(
          $('<div class="title">' + graphConfig.chartTitle + '</div>'));
      columnHeader.append(
          $('<div class="title">' + Graph.getGraphPlatformText(ietype) +
            '</div>'));
      columnHeader.append(
          $('<div class="subtitle">' + graphConfig.unit + ' ' +
            Modal.getUnitDescription(graphConfig.unit) + '</div>'));
  ...
  function removeEmptyLabel(config, indexes) {
    ...
    sorted.forEach((index) => { config.datasets.splice(index, 1); })
    return config;
  }

  function processMetricNew(
      labels, datasets, chartTitle, container, widthClass, id) {
  ...
  function resetChartsDisplay(currentDisplay) {
    const newDisplayMode =
        getChartsDisplayMode(currentDisplay.numberOfChartsInRow);
  ...
  function adjustLabels(displayMode) {
    const firstLabels =
        $('.chart-labels-container').find('.chart-labels-item:first-child');
  ...
  function adjustHeaderIcons(displayMode) {
    ...
    if (displayMode == 'rowCompact')
      ...
      else icons.css('flex-direction', 'row')
  }
