...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function showMenuToggle() {
    const ddMs = document.querySelectorAll('.dropdown-menu');
    ddMs.forEach((dm) => {
      dm.parentElement.classList.contains('show') ? dm.classList.add('show')
                                                  : dm.classList.remove('show');
    });
  }
});
...
(function() {
var host = (window.document.location.protocol == 'http:')
    ? "http://www.intel.com"
    : "https://www.intel.com";
...
function createSphinxTabSets() {
  ...
  sphinxTabSets.each(function() {
    ...
    tabSet.find('> .sphinxtab').each(function() {
      ...
      var input = $(`<input ${checked} class="tab-input" id="tab-set--${
          tabSetCount}-input--${inputCount}" name="tab-set--${
          tabSetCount}" type="radio">`);
      input.insertBefore(tab);
      var label = $(`<label class="tab-label" for="tab-set--${
          tabSetCount}-input--${inputCount}">${tabValue}</label>`);
...
function updateTitleTag() {
  ...
  var newTitle = (title.text() + ' — Version(' + currentVersion + ')')
                     .replace(/\s+/g, ' ')
                     .trim();
  title.text(newTitle);
}
...
function getCurrentVersion() {
  ...
  if ([ "index.html", "404.html", "", "latest" ].indexOf(wordAfterDomain) >=
      0) {
    /*
  ...
  versions.forEach(item => {
    var link =
        $('<a class="dropdown-item" href="' +
          getPageUrlWithVersion(item.version) + '">' + item.version + '</a>');
...
function handleSwitcherParam() {
  ...
  $('.reference.internal.nav-link[href="#' + section_id + '"]')
      .parent('li')
      .css('display', 'block');
...
// initBenchmarkPickers and initCollapsibleHeaders included with the new
// benchmarks page
...
function initCollapsibleHeaders() {
  $('#performance-information-frequently-asked-questions section')
      .on('click', function() {
  ...
  function resetCollapsibleHeaders() {
    ...
    $('#performance-information-frequently-asked-questions section p, #performance-information-frequently-asked-questions section table')
        .hide();
  }
}
...
function addViewTypeListeners() {
  const resultViewTypeFromLs =
      window.localStorage.getItem('atomicResultViewType');
  ...
  selectResultViewType(
      resultViewTypeFromLs || "grid", viewSelectorGrid, viewSelectorList);
}
    ...
    function() {
      (async () => {
        ...
        if (searchInterfaceSa) {
          ...
          if (ver) {
            searchInterfaceSa.innerHTML = searchInterfaceSa.innerHTML.replace(
                'search.html', '/' + ver + '/search.html#f-ovversion=' + ver);
          }
