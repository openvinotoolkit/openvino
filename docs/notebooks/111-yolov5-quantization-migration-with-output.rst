...
### Table of content: - `Preparation <#1>`__ - `Download the YOLOv5
model <#2>`__ - `Conversion of the YOLOv5 model to OpenVINO <#3>`__ -
`Imports <#4>`__ - `Prepare dataset for quantization <#5>`__ - `Create
YOLOv5 DataLoader class for POT <#6>`__ - `Create NNCF Dataset <#7>`__ -
`Configure quantization pipeline <#8>`__ - `Prepare config and pipeline
for POT <#9>`__ - `Prepare configuration parameters for NNCF <#10>`__ -
`Perform model optimization <#11>`__ - `Run quantization using
POT <#12>`__ - `Run quantization using NNCF <#13>`__ - `Compare accuracy
FP32 and INT8 models <#14>`__ - `Inference Demo Performance
Comparison <#15>`__ - `Benchmark <#16>`__ - `References <#17>`__

## Preparation `⇑ <#0>`__

### Download the YOLOv5 model `⇑ <#0>`__
...
### Conversion of the YOLOv5 model to OpenVINO `⇑ <#0>`__ There are
...
## Prepare dataset for quantization `⇑ <#0>`__
...
### Create YOLOv5 DataLoader class for POT `⇑ <#0>`__
...
## Configure quantization pipeline `⇑ <#0>`__

Next, we should define quantization algorithm parameters.

### Prepare config and pipeline for POT `⇑ <#0>`__
...
### Prepare configuration parameters for NNCF `⇑ <#0>`__
...
## Perform model optimization `⇑ <#0>`__

### Run quantization using POT `⇑ <#0>`__
...
### Run quantization using NNCF `⇑ <#0>`__
...
## Compare accuracy FP32 and INT8 models `⇑ <#0>`__
...
## Inference Demo Performance Comparison `⇑ <#0>`__
