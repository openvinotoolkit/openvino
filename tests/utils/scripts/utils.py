...
def filter_result(stats: dict):
    """Identify and remove outliers from statistical data."""
    filtered_stats = {}
    for step_name, results in stats.items():
        iqr, q1, q3 = calculate_iqr(results)
        cut_off = iqr * IQR_CUTOFF
        upd_time_results = [x for x in results if (q1 - cut_off < x < q3 + cut_off)]
        filtered_stats.update({step_name: upd_time_results if upd_time_results else results})
