  ...
  if (dir) {
    ...
    while (NULL != (entry = readdir(dir))) {
      if (DT_REG == entry->d_type)
        res.push_back(path + "/" + std::string(entry->d_name));
    }
    closedir(dir);
    dir = NULL;
  }
...
extern "C" int main(int argc, char* argv[]) {
  ...
  if (1 != positional.size()) {
    std::cerr << program_name
              << ": error: wrong number of positional arguments." << std::endl;
  ...
  for (auto const& path : input_files) {
    ...
    if (!test_file) {
      std::cerr << program_name << ": error: failed to open \"" << path << "\""
                << std::endl;
      return -2;
    }
    ...
    int fuzzer_res;
    if (0 !=
        (fuzzer_res = LLVMFuzzerTestOneInput(
             (const uint8_t*)data.str().c_str(), data.str().size()))) {
      std::cerr << program_name << ": error: testing \"" << path << "\" fails"
                << std::endl;
      return fuzzer_res;
    }
    time_total += time(nullptr) - time_start;
  }
  std::cout << "Executed " << input_files.size() << " item(s) in " << time_total
            << " seconds" << std::endl;
  return 0;
}
