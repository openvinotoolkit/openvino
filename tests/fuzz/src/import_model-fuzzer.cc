...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  size_t split[1] = {0};
  if (size < kSplitSequenceSize)
    return 0; // we at least expect one separator
  for (size_t i = 0;
       i < size - kSplitSequenceSize && split_counter < COUNT_OF(split);
       i++)
    if (0 == memcmp(data + i, kSplitSequence, kSplitSequenceSize)) {
      split[split_counter++] = i;
    }
  if (COUNT_OF(split) != split_counter)
    return 0; // not enough splits
  ...
  string device_name(
      reinterpret_cast<const char*>(dev_name_data),
      size - (_size + kSplitSequenceSize));

  struct ov::pass::StreamSerialize::DataHeader* dh =
      (struct ov::pass::StreamSerialize::DataHeader*)data;
  size_t total = dh->custom_data_size + dh->consts_size + dh->model_size;

  if (total != _size || dh->custom_data_offset + dh->custom_data_size > total ||
      dh->consts_offset + dh->consts_size > total ||
      dh->model_offset + dh->model_size > total)
    return 0;
