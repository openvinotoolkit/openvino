...
class ScoreTensorFlow(ClassProvider):
    ...
    def get_refs(self):
        ...
        input_data_constants = {
            name: tf.constant(val) for name, val in self.inputs.items()
        }
...
class ScoreTensorFlowV2ByV1(ScoreTensorFlowBase):
    ...
    def load_graph(self):
        ...
        assert LooseVersion(tf.__version__) >= LooseVersion(
            "2"
        ), "This collector can't be used with TF 1.* version"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            frozen_func = convert_variables_to_constants_v2(
                imported.signatures["serving_default"], lower_control_flow=False
            )
            graph_def = frozen_func.graph.as_graph_def(add_shapes=True)
            tf_v1.io.write_graph(
                graph_def,
                str(tmp_model_path.parent),
                tmp_model_path.name,
                as_text=False,
            )
