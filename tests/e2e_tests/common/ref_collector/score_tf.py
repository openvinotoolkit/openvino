...
from e2e_tests.utils.path_utils import resolve_dir_path, resolve_file_path
...
def trace_loop(enter, control_flow_map):
    ...
    while len(for_examination):
        ...
        if candidate.type == "Enter":
            ...
            nested_collected, nested_exits = trace_loop(candidate, control_flow_map)
...
def summarize_graph(graph_def):
    ...
    for node in graph.as_graph_def().node:  # pylint: disable=no-member
        if node.op == "Placeholder":
            ...
            node_dict["shape"] = (
                str(tf.TensorShape(node.attr["shape"].shape))
                .replace(" ", "")
                .replace("?", "-1")
            )
...
def get_output_node_names_list(graph_def, user_defined_output_node_names_list: list):
    return (
        ...
        if len(user_defined_output_node_names_list) == 0
...
class ScoreTensorFlowBase(ClassProvider):
    ...
    def load_graph(self):
        ...
        raise NotImplementedError(
            "{}\nDo not use {} class directly!".format(
                self.load_graph().__doc__, self.__class__.__name__
            )
        )
    ...
    def get_refs(self):
        ...
        input_layers, output_layers = (
            list(summary_info["inputs"].keys()),
            summary_info["outputs"],
        )
        ...
        if sorted(input_layers) != sorted(data_keys):
            raise ValueError(
                "input data keys: {data_keys} do not match input "
                "layers of network: {input_layers}".format(
                    data_keys=data_keys, input_layers=input_layers
                )
            )
    ...
    def load_graph(self):
        ...
        with tf.compat.v1.Session() as sess:
            meta_graph_def = tf.compat.v1.saved_model.loader.load(
                sess, tags, self.model
            )
            outputs = get_output_node_names_list(
                meta_graph_def.graph_def, self.user_output_node_names_list
            )
            graph_def = tf.compat.v1.graph_util.convert_variables_to_constants(
