...
class ONNXRuntimeRunner(ClassProvider):
    ...
    def __init__(self, config):
        ...
        self.ep = (
            config["onnx_rt_ep"]
            if isinstance(config["onnx_rt_ep"], list)
            else [config["onnx_rt_ep"]]
        )
    ...
    def run_rt(self, input_data):
        ...
        if len(input_data) > 1:
            log.warning(
                "ONNX Runtime runner is not properly tested to work with multi-input topologies. "
                "Please, contact QA."
            )
        for layer in sess.get_inputs():
            model_shape_to_compare = tuple(
                [
                    layer.shape[dim]
                    for dim in range(len(layer.shape))
                    if not (
                        (layer.shape[dim] is None)
                        or (isinstance(layer.shape[dim], str))
                    )
                ]
            )
            data_shape_to_compare = tuple(
                [
                    input_data[layer.name].shape[dim]
                    for dim in range(len(layer.shape))
                    if not (
                        (layer.shape[dim] is None)
                        or (isinstance(layer.shape[dim], str))
            ...
            if model_shape_to_compare != data_shape_to_compare:
                raise ValueError(
                    f"Shapes of input data {list(input_data.values())[0].shape} and "
                    f"input blob {sess.get_inputs()[0].shape} are not equal for layer {layer.name}"
                )
        output_names = [output.name for output in sess.get_outputs()]
        if len(output_names) > 1:
            log.warning(
                "ONNX Runtime runner is not properly tested to work with multi-output topologies. "
                "Please, contact QA."
            )
    ...
    def get_refs(self):
        self.res = multiprocessing_run(
            self.run_rt, [self.inputs], "ONNX Runtime Inference", timeout=200
        )
        return self.res
