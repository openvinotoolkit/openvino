    ...
    def apply(self, data):
        ...
        for batch in range(len(do_data)):
            for cur_bounding_box in range(len(do_data[batch])):
                ...
                x1 = int(
                    min(
                        max(0, do_data[batch][cur_bounding_box]["xmin"] * self.w),
                        self.w,
                    )
                )
                y1 = int(
                    min(
                        max(0, do_data[batch][cur_bounding_box]["ymin"] * self.h),
                        self.h,
                    )
                )
                x2 = int(
                    min(
                        max(0, do_data[batch][cur_bounding_box]["xmax"] * self.w),
                        self.w,
                    )
                )
                y2 = int(
                    min(
                        max(0, do_data[batch][cur_bounding_box]["ymax"] * self.h),
                        self.h,
                    )
                )
                num_detected_masks_per_image = int(masks_data.shape[0] / len(do_data))
                current_mask_index = (
                    batch * num_detected_masks_per_image + cur_bounding_box
                )
                # Shape of TF masks output blob is 3, IE - 4
                mask = (
                    masks_data[current_mask_index][label]
                    if len(masks_data.shape) > 3
                    else masks_data[current_mask_index]
                )
