...
class ParseBeforeODParser(ClassProvider):
    ...
    def apply(self, data):
        ...
        for layer in target_layers:
            ...
            assert (
                layer_data.shape[-1] == 7
            ), "Wrong data for postprocessing! Last dimension must be equal 7."
...
class ParseObjectDetection(ClassProvider):
    ...
    def apply(self, data):
        ...
        for layer in target_layers:
            ...
            assert (
                len(layer_data.shape) <= 2
            ), "Wrong data for postprocessing! Data length must be equal 2."
            for obj in layer_data:
                ...
                assert (
                    len(obj) == 7
                ), "Wrong data for postprocessing! Data length for one detection must be equal 7."
        ...
        for layer in apply_to:
            ...
            container = np.zeros(
                shape=(
                    1,
                    1,
                    data[layer].shape[2] * self.batch + 1,
                    data[layer].shape[3],
                )
            )
            ...
            for b in range(self.batch):
                ...
                container[0][0][detections_counter] = [
                    -1,
                    0,
                    0,
                    0,
        ...
        for layer in apply_to:
            for b in range(len(data[layer])):
                for i, box in enumerate(data[layer][b]):
                    data[layer][b][i].update(
                        {
                            "xmax": (
                                min(box["xmax"], self.max_w) if box["xmax"] > 0 else 0
                            ),
                            "xmin": max(box["xmin"], 0),
                            "ymax": (
                                min(box["ymax"], self.max_h) if box["ymax"] > 0 else 0
                            ),
                            "ymin": max(box["ymin"], 0),
                        }
                    )
