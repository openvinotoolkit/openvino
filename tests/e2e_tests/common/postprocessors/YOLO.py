    ...
    def apply(self, prediction):
        ...
        for layer, layer_data in prediction.items():
            ...
            for b in range(layer_data.shape[0]):
                ...
                data = layer_data[b]
                assert (
                    probability_size + confidence_size + boxes_size == data.shape[0]
                ), "Wrong input data shape"

                prob, scale, boxes = np.split(
                    data, [probability_size, probability_size + confidence_size]
                )
                ...
                probabilities = np.zeros(
                    (cells_y, cells_x, objects_per_cell, classes + 4)
                )
                for cls in range(classes):
                    probabilities[:, :, 0, cls] = np.multiply(
                        prob[:, :, cls], scale[:, :, 0]
                    )
                    probabilities[:, :, 1, cls] = np.multiply(
                        prob[:, :, cls], scale[:, :, 1]
                    )
                ...
                for i, j, k in np.ndindex((cells_x, cells_y, objects_per_cell)):
                    box = boxes[j, i, k]
                    box = [
                        (box[0] + i) / float(cells_x),
                        (box[1] + j) / float(cells_y),
                        box[2] ** 2,
                        box[3] ** 2,
                    ]
...
class YOLOV2Parser(ClassProvider):
    ...
    def _parse_yolo_v2_results(self, predictions):
        ...
        for y, x, n in np.ndindex((cells_y, cells_x, self.num)):
            ...
            box_index = self._entry_index(
                cells_x, cells_y, self.coords, self.classes, index, 0
            )
            obj_index = self._entry_index(
                cells_x, cells_y, self.coords, self.classes, index, self.coords
            )
            ...
            box = [
                ...
                np.exp(predictions[box_index + 2 * (cells_y * cells_x)])
                * self.anchors[2 * n + 0]
                / cells_x,
                np.exp(predictions[box_index + 3 * (cells_y * cells_x)])
                * self.anchors[2 * n + 1]
                / cells_y,
            ]
...
class YOLOV3Parser(ClassProvider):
    ...
    def _parse_yolo_v3_results(self, prediction):
        ...
        assert (
            cells_y == cells_x
        ), "Incorrect YOLO Region! Grid size sides are not equal"
        side = cells_x
        ...
        for i in range(side_square):
            ...
            for n in range(self.masks_length):
                obj_index = self._entry_index(
                    side, self.coords, self.classes, n * side_square + i, self.coords
                )
                scale = predictions[obj_index]
                if scale < self.scale_threshold:
                    continue
                box_index = self._entry_index(
                    side, self.coords, self.classes, n * side_square + i, 0
                )
                ...
                w = (
                    w_exp
                    * self.anchors[self.get_anchors_offset(side) + 2 * n]
                    / self.input_w
                )
                h = (
                    h_exp
                    * self.anchors[self.get_anchors_offset(side) + 2 * n + 1]
                    / self.input_h
                )

                for cls_id in range(self.classes):
                    class_index = self._entry_index(
                        side,
                        self.coords,
                        self.classes,
                        n * side_square + i,
...
class YOLORegion(ClassProvider):
    ...
    def _entry_index(width, height, coords, classes, outputs, batch, location, entry):
        ...
        return (
            batch * outputs
            + n * width * height * (coords + classes + 1)
            + entry * width * height
            + loc
        )
    ...
    def _softmax(data, B, C, H, W):
        ...
        for b in range(B):
            for i in range(H * W):
                ...
                exp_sum = 0
                for c in range(C):
                    dest_data[b * C * H * W + c * H * W + i] = math.exp(
                        data[b * C * H * W + c * H * W + i] - max_val
                    )
                    exp_sum += dest_data[b * C * H * W + c * H * W + i]
                for c in range(C):
                    dest_data[b * C * H * W + c * H * W + i] = (
                        dest_data[b * C * H * W + c * H * W + i] / exp_sum
                    )
        return dest_data
    ...
    def apply(self, data):
        for layer, layer_data in data.items():
            ...
            assert (
                IH == IW
            ), "Incorrect data layout! Input data should be in 'NCHW' format"
            ...
            if self.do_softmax:
                ...
                for b in range(B * self.num):
                    dst_data[index + b * batch_offset :] = self._softmax(
                        data=dst_data[index + b * batch_offset :],
                        B=1,
                        C=self.classes,
                        H=IH,
                        W=IW,
