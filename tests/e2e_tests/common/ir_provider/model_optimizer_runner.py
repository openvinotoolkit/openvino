...
class OVCMORunner(ClassProvider):
    ...
    def _build_arguments(self):
        ...
        if "additional_args" in self._config:
            if "tensorflow_object" in self._config["additional_args"]:
                ...
                ] = self._config["additional_args"].pop("tensorflow_object")
    ...
    def get_ir(self, passthrough_data):
        ...
        if "input" in self.args and "input_shape" not in self.args:
            ...
            for input_value in split_inputs(self.args["input"]):
                ...
                inputs.append(
                    [
                        attr
                        for attr in [node_name, shape, value, data_type]
                        if attr is not None
                    ]
                )
            ...
            input_shape_to_input_cut_info(
                self.args["input_shape"], filtered_args["input"]
            )
            for idx in range(len(filtered_args["input"])):
                if filtered_args["input"][idx].type:
                    filtered_args["input"][idx] = (
                        filtered_args["input"][idx].name,
                        filtered_args["input"][idx].shape,
        ...
        log.info(
            f"XML file with compress_to_fp16={compress_to_fp16} was saved to: {self.xml}"
        )
        log.info(
            f"BIN file with compress_to_fp16={compress_to_fp16} was saved to: {self.bin}"
        )
