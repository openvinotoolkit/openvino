...
def pytest_addoption(parser):
    ...
    if getattr(parser, "after_preparse", False):
        return
    parser.addoption(
        "--modules", nargs="+", help="Path to test modules", default=["pipelines"]
    )
    parser.addoption(
        "--env_conf",
        action="store",
...
def gpu_throughput_mode(request):
    """Fixture function for command-line option."""
    if request.config.getoption("gpu_throughput_mode") and request.config.getoption(
        "cpu_throughput_mode"
    ):
        raise ValueError(
            "gpu_throughput_mode and cpu_throughput_mode options can't be specified simultaneously"
        )
...
def pytest_collection_finish(session):
    ...
    if session.config.getoption("collect_output"):
        ...
        collect_output = session.config.getoption(
            "collect_output"
        ) or Environment.abs_path("logs_dir", "dry_run.csv")
        ...
        for item in session.items:
            ...
            if match and match.group(1):
                ...
                # Write csv
                if collect_only:
                    session_items_params = item._fixtureinfo.name2fixturedefs.get(
                        "instance"
                    )[0].params
                    write_csv({"test_filter": test_name}, collect_output, ",")
            else:
                log.error(
                    'Unable to extract test name from string "{}"'.format(item.name)
                )
...
def prepare_test_info(request, instance):
    ...
    upload_db(
        data=request.node._request.test_info, url=request.config.getoption("db_url")
    )
...
def upload_db(data, url):
    ...
    if resp.status_code == 200:
        ...
        log.error(
            f"Upload data failed. DB return: code - {resp.status_code}\n"
            f"Message - {resp.text}"
        )
...
def get_model_path(test_log):
    ...
    return (
        model_path.group(1)
        if model_path
        else "Model was not found! Please contact with QA team"
    )


def set_path_for_pytorch_files(instance, final_path):
    instance.ie_pipeline["prepare_model"]["prepare_model_for_mo"][
        "torch_model_zoo_path"
    ] = final_path
    # if pytorch weights is required for tests we should use new path also for them
    weights_path = instance.ie_pipeline["prepare_model"]["prepare_model_for_mo"].get(
        "weights"
    )
    if weights_path:
        weights_path = Path(weights_path)
        copied_weights_path = os.path.join(
            final_path,
            weights_path.parents[1].name,
            weights_path.parents[0].name,
            weights_path.name,
        )
        instance.ie_pipeline["prepare_model"]["prepare_model_for_mo"][
            "weights"
