...
def set_env(metafunc):
    ...
    with open(metafunc.config.getoption("env_conf"), "r") as f:
        Environment.env = fix_env_conf(
            yaml.load(f, Loader=yaml.FullLoader), root_path=str(metafunc.config.rootdir)
        )
...
def pytest_generate_tests(metafunc):
    ...
    test_classes, broken_modules = base.find_tests(
        metafunc.config.getoption("modules"),
    ...
    for test in test_classes:
        ...
        try:
            test_case = test(
                ...
                sequence_length=1,
                qb=8,
                device_mode="GNA_AUTO",
                skip_ir_generation=skip_ir_generation,
            ).ref_collection
        except Exception as e:
            log.warning(
                f"Test with name {name} failed to add in row with exception {e}"
            )
