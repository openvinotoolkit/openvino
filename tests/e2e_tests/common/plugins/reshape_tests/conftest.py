...
from e2e_tests.test_utils.reshape_tests_utils import (
    batch_was_changed,
    get_reshape_configurations,
    get_reshape_pipeline_pairs,
    should_run_reshape,
)
...
def set_env_for_reshape(metafunc):
    ...
    with open(metafunc.config.getoption("env_conf"), "r") as env_conf:
        Environment.env = fix_env_conf(
            yaml.load(env_conf, Loader=yaml.FullLoader),
    ...
    with open(
        metafunc.config.getoption("reshape_rules_conf"), "r"
    ) as reshape_rules_conf:
        Environment.reshape_rules = unwrap_rules(
            yaml.load(reshape_rules_conf, Loader=yaml.FullLoader)
        )

    with open(
        metafunc.config.getoption("dynamism_rules_conf"), "r"
    ) as dynamism_rules_conf:
        Environment.dynamism_rules = unwrap_rules(
            yaml.load(dynamism_rules_conf, Loader=yaml.FullLoader)
        )
...
def pytest_generate_tests(metafunc):
    ...
    reshape_test_classes, broken_modules = base.find_tests(
        metafunc.config.getoption("modules"), attributes=["__is_test_config__"]
    )
    for module in broken_modules:
        log.error(
            "Broken module: {}. Import failed with error: {}".format(
                module[0], module[1]
            )
        )
    ...
    if len(Environment.tconf["batch"]) > 1 or Environment.tconf["batch"][0] != 1:
        ...
        log.warning(
            "batch was set explicitly to '1' because reshape and dynamism tests do not use this parameter,"
            " but it is required to be in e2e"
        )

    for reshape_test in reshape_test_classes:
        required_args = [
            arg for arg in inspect.signature(reshape_test.__init__).parameters.keys()
        ]
        required_args.extend(getattr(metafunc, "test_add_args_to_parametrize", []))
        rules = (
            Environment.dynamism_rules
        ...
        params_list, addit_params_dict = read_reshape_test_config(
            required_args, Environment.tconf, rules
        )
        ...
        for _params in params_list:
            ...
            try:
                reshape_test_case = reshape_test(
                    **params,
                    **addit_params_dict,
                ...
                configurations = get_reshape_configurations(
                    reshape_test_case, dynamism_type
                )

            except Exception as e:
                configurations = [
                    SimpleNamespace(
                        shapes={}, changed_dims={}, layout={}, default_shapes={}
                    )
                ]
                tb = traceback.format_exc()
                broken_test = class_factory(
                    cls_name=name,
                    cls_kwargs={
                        "__name__": name,
                        **params,
            ...
            if not getattr(reshape_test_case, "__do_not_run__", False):
                if configurations:
                    for configuration in configurations:
                        ...
                        if (
                            dynamism_type == "negative_ones"
                            or dynamism_type == "range_values"
                        ):
                            reshape_test_id = test_id + "_".join(
                                "_{}_{}".format(k, v[0:])
                                for (k, v) in configuration.shapes.items()
                            )
                            if satisfies_all_rules(
                                params_for_satisfaction,
                                rules,
                                can_partially_match=False,
                            ):
                            ...
                            requested_reshape_pairs = get_reshape_pipeline_pairs(
                                reshape_test_case
                            )
                            for reshape_pair in requested_reshape_pairs:
                                ...
                                if "IE_SBS" in reshape_pair:
                                    batch = batch_was_changed(
                                        configuration.shapes,
                                        configuration.changed_dims,
                                        configuration.layout,
                                        configuration.default_shapes,
                                    )
                                    if not batch:
                                        continue

                                configuration.reshape_pair = reshape_pair
                                reshape_test_id = reshape_test_id + "{}".format(
                                    reshape_pair
                                )
                                if satisfies_all_rules(
                                    params_for_satisfaction,
    ...
    if reshape_test_cases:
        pairs_of_shape_and_test_case = list(
            zip(reshape_test_cases, reshape_configurations_list)
        )
        metafunc.parametrize(
            argnames="instance,configuration",
...
def pytest_collection_modifyitems(items):
    ...
    items.sort(
        key=lambda item: (
            item.callspec.params["instance"].batch,
    ...
    for num, test in enumerate(items):
        ...
        try:
            if pytorch_original_tests[num]:
                ...
                if (
                    pytorch_group_marked % bucket_size == 0
                    and pytorch_group_marked < bucket_size * len(group_names)
                ):
                    current_group_idx += 1

            base.set_pytest_marks(
                _test=test, _object=instance, _runner=target_test_runner, log=log
            )
