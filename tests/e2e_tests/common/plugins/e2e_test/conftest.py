...
from e2e_tests.common.marks import MarkGeneral, MarkRunType
...
def set_env(metafunc):
    ...
    with open(metafunc.config.getoption("base_rules_conf"), "r") as base_rules_conf:
        Environment.base_rules = unwrap_rules(
            yaml.load(base_rules_conf, Loader=yaml.FullLoader)
        )
...
def satisfies_rules(parameter_set, rules, filters, can_partially_match=False):
    ...
    for key in filters:
        applicable_rules = list(
            filter(lambda rule: match(rule[key], parameter_set[key]), applicable_rules)
        )
...
def satisfies_all_rules(values_set, rules_config, can_partially_match=False):
    ...
    for rules_dict in rules_config:
        ...
        if any(key not in values_set for key in filters):
            continue
        satisfies &= satisfies_rules(values_set, rules, filters, can_partially_match)
    return satisfies
...
def pytest_generate_tests(metafunc):
    ...
    test_classes, broken_modules = base.find_tests(
        modules, attributes=["__is_test_config__"]
    )
    ...
    for test in test_classes:
        setattr(
            ...
            metafunc.config.getoption("convert_pytorch_to_onnx"),
        )
        ...
        params_list, addit_params_dict = read_test_config(
            required_args, Environment.tconf, Environment.base_rules
        )
        for _params in params_list:
            ...
            test_id = "{}_{}".format(
                name,
                "_".join(
                    "{}_{}".format(key, val) for (key, val) in sorted(params.items())
                ),
            )
...
def pytest_collection_modifyitems(session, config, items):
    ...
    * Inference step parameters (inference type, batch, device)
    ...
    logger.info(
        "Preparing tests for test session in the following folder: {}".format(
            session.startdir
        )
    )
    ...
    for num, test in enumerate(items):
        ...
        if hook_utils.deselect(test, test_type, required_marker_ids):
            deselected.append(test)
            continue
        hook_utils.update_markers(
            test, test_type, all_components, MarkGeneral.COMPONENTS.mark
        )
        hook_utils.update_markers(
            test, test_type, all_requirements, MarkGeneral.REQIDS.mark
        )
    ...
    items[:] = sorted(
        items, key=lambda element: MarkRunType.test_type_mark_to_int(element)
    )
...
def call_and_report(item, when, log=True, **kwds):
    ...
    lg.basicConfig(
        format="[ %(levelname)s ] %(message)s", level=lg.DEBUG, stream=sys.stdout
    )
    ...
    if when == "call" and hasattr(report, "wasxfail"):
        ...
        for m in regexp_marks:
            matches = []
            xfail_reason = m.kwargs.get(
                "reason", "UNDEFINED"
            )  # TODO: update for non-xfail marks
            for pattern in m.regexps:
                regexp = re.compile(pattern)
                matches.append(
                    regexp.search(report.caplog) is not None
                    or regexp.search(report.longreprtext) is not None
                )

            if (m.match_mode == "all" and not all(matches)) or (
                m.match_mode == "any" and not any(matches)
            ):
                failed_msgs[xfail_reason] = (
                    "Some of regexps '{}' for xfail mark with reason '{}' doesn't match the test log! "
                    "Test will be forced to fail!".format(
                        ", ".join(m.regexps), xfail_reason
                    )
                )
            elif (m.match_mode == "all" and all(matches)) or (
                m.match_mode == "any" and any(matches)
            ):
                jira_link = "https://jira.devtools.intel.com/browse/{}".format(
                    xfail_reason
                )
                ...
                if getattr(item._request, "test_info", None):
                    item._request.test_info.update(
                        {"links2JiraTickets": [xfail_reason]}
                    )
                break
        else:
            jira_links = []
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if session.config.option.pregen_irs:
        path = (
            Path(Environment.env["pregen_irs_path"]) / session.config.option.pregen_irs
        ).with_suffix(".lock")
        if path.exists():
            os.remove(path)
