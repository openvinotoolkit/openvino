...
class CommonConfig:
    ...
    def __new__(cls, test_id, *args, **kwargs):
        ...
        subpath = re.sub(
            r"[^\w\-_\. ]", "_", test_id
        )  # filter all symbols not supported in a file systems
        ...
        for env_key in ["mo_out", "pytorch_to_onnx_dump_path", "pregen_irs_path"]:
            instance.environment[env_key] = str(
                Path(instance.environment[env_key]) / tmpdir_subpath
            )
        return instance
    ...
    def align_results(self, ref_res, optim_model_res, xml=None):
        ...
        if len(ref_res) == 1 and len(optim_model_res) == 1:
            ...
            if (
                isinstance(ref_res_vals, np.ndarray)
                and isinstance(ie_res_vals, np.ndarray)
            ) and ref_res_vals.shape == ie_res_vals.shape:
        ...
        for fw_layer_name in ref_res.keys():
            if fw_layer_name not in optim_model_res.keys():
                ...
                if isinstance(ir_layer_name, list):
                    ...
                    if isinstance(ir_layer_name, list):
                        raise Exception(
                            f"Output tensor names in references and in ov model are different\nRef names: "
        ...
        if missed_fw_layer_names:
            not_contain_layers_in_mapping_err_msg = (
                "mapping file does not contain {fw_layer}. Mapping: {mapping}".format(
                    fw_layer=missed_fw_layer_names, mapping=self.mapping
                )
            )
        if missed_ir_layer_names:
            not_found_layers_in_inference_err_msg = (
                "found IR layer {ir_layer} is not found in inference result. "
                "available layers: {avail_layers}. Mapping: {mapping}".format(
                    ir_layer=missed_ir_layer_names,
        ...
        if (
            not_contain_layers_in_mapping_err_msg
            or not_found_layers_in_inference_err_msg
        ):
            raise ValueError(
                "{}\n{}".format(
                    not_contain_layers_in_mapping_err_msg,
    ...
    def _add_defect(self, name, condition, params, test_name=None):
        self.__pytest_marks__ += tuple(
            [pytest.mark.bugs(name, condition, params, test_name)]
        )
    ...
    def _set_test_group(self, name, condition=True, params=None, test_name=None):
        ...
        self.__pytest_marks__ = (
            mark,
        ) + self.__pytest_marks__  # add mark as first element in tuple.
