...
class PassThroughData(dict):
    ...
    def strict_get(self, key, step):
        assert (
            key in self
        ), "Step `{}` requires `{}` key to be defined by previous steps".format(
            step.__step_name__, key
        )
        return self.get(key)
...
class Pipeline:
    ...
    def __init__(self, config, passthrough_data=None):
        ...
        self.passthrough_data = (
            ...
            else PassThroughData(passthrough_data)
        )
    ...
    def fetch_test_info(self):
        ...
        for step in self.steps:
            ...
            assert (
                len(set(test_info.keys()).intersection(info_from_step.keys())) == 0
            ), "Some keys have been overwritten: {}".format(
                set(test_info.keys()).intersection(info_from_step.keys())
            )
