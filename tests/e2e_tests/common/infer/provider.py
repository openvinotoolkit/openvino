    ...
    def validate(cls):
        ...
        if "infer" not in methods:
            raise AttributeError(
                "Requested class {} registred as '{}' doesn't provide required method infer".format(
...
class StepProvider(BaseStepProvider):
    ...
    def execute(self, passthrough_data=None):
        ...
        self.executor.xml, self.executor.bin = passthrough_data.get(
            "xml"
        ), passthrough_data.get("bin")
