...
class Resize(ClassProvider):
    ...
    def __init__(self, config):
        ...
        self.target_layers = config.get("target_layers", None)
        self.interpolation = Resize.resize_interp_map[config.get("mode", "linear")]

    def apply(self, data):
        """Resize data with opencv resize."""
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Resize input data for layers {} to size ({}, {}) ...".format(
                ", ".join('"{}"'.format(l) for l in apply_to), self.width, self.height
            )
        )
        for layer in apply_to:
            data[layer] = cv2.resize(
                data[layer], (self.width, self.height), interpolation=self.interpolation
            )
        return data
...
class PermuteShape(ClassProvider):
    ...
    def apply(self, data):
        ...
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Permute input data for layers {} to order {} ...".format(
                ", ".join('"{}"'.format(l) for l in apply_to), self.order
            )
        )
        for layer in apply_to:
...
class AlignWithBatch(ClassProvider):
    ...
    def apply(self, data):
        ...
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Align batch data for layers {} to batch {} ...".format(
                ", ".join('"{}"'.format(l) for l in apply_to), self.batch
            )
        )
        for layer in apply_to:
            if self.expand_dims:
...
class SubtractMeanValues(ClassProvider):
    ...
    def apply(self, data):
        ...
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Subtract mean values {} from input data for layers {} ...".format(
                self.mean_values, ", ".join('"{}"'.format(l) for l in apply_to)
            )
        )
        for layer in apply_to:
...
class SubtractMeanValuesFile(ClassProvider):
    ...
    def apply(self, data):
        ...
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Subtract mean file {} from input data for layers {} ...".format(
                self.mean_file, ", ".join('"{}"'.format(l) for l in apply_to)
            )
        )
        for layer in apply_to:
            ...
            if len(data_shape) != 3:
                raise ValueError(
                    "data layer {l} has unexpected shape {s}, "
...
class Normalize(ClassProvider):
    ...
    def apply(self, data):
        ...
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Normalize input data for layers {} with normalization factor {}...".format(
                ", ".join('"{}"'.format(l) for l in apply_to), self.factor
            )
        )
        for layer in apply_to:
...
class ExpandDims(ClassProvider):
    ...
    def apply(self, data):
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Expanding dims for layers {}...".format(
...
class Scale(ClassProvider):
    ...
    def apply(self, data):
        ...
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Scale input data for layers {} with scaling factor {} ...".format(
                ", ".join('"{}"'.format(l) for l in apply_to), self.factor
            )
        )
        for layer in apply_to:
...
class ReverseChannels(ClassProvider):
    ...
    def apply(self, data):
        ...
        def convert(data):
            ...
            return cv2.cvtColor(data.astype(np.float32), cv2.COLOR_RGB2BGR).astype(
                orig_type
            )

        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Convert colors from RGB to BGR for input data for layers {} ...".format(
                ", ".join('"{}"'.format(l) for l in apply_to)
            )
        )
        for layer in apply_to:
            if len(data[layer].shape) != 3:
                raise ValueError(
                    "data layer {l} has unexpected shape {s}, "
                    "expected shape of length 3.".format(l=layer, s=data[layer].shape)
                )
...
class RenameInputs(ClassProvider):
    ...
    def apply(self, data):
        ...
        if self.input_pairs:
            log.info(
                "Rename input data keys according to pairs {}...".format(
                    self.input_pairs
                )
            )
...
class CopyDataFromLayer(ClassProvider):
    ...
    def apply(self, data):
        ...
        for source, target in self.source_target_map.items():
            data = AddLayersToInputData({"layer_data": {target: data[source]}}).apply(
                data
            )
        return data
...
class CastDataType(ClassProvider):
    ...
    def apply(self, data):
        ...
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Converting layers {} data to type {}...".format(
...
class Grayscale(ClassProvider):
    ...
    def apply(self, data):
        ...
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Converting layers {} to grayscale ...".format(
                ", ".join('"{}"'.format(l) for l in apply_to)
            )
        )
        for layer in apply_to:
    ...
    def apply(self, data):
        if self.layers_to_expand:
            data_preproc = AlignWithBatch(
                {"batch": self.batch, "target_layers": self.layers_to_expand}
            )
            data = data_preproc.apply(data)
        else:
            log.warning(
                "No layers specified to be aligned with batch using dimension expanding."
            )
            ...
            data_preproc = AlignWithBatch(
                {
                    "batch": self.batch,
                    "expand_dims": False,
                    "target_layers": self.layers_not_to_expand,
                }
            )
            data = data_preproc.apply(data)
        else:
            log.warning(
                "No layers specified to be aligned with batch using no dimension expanding."
            )
        return data
...
class ConvertToTorchTensor(ClassProvider):
    ...
    def apply(self, data):
        ...
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Converting layers {} to torch.Tensor format ...".format(
                ", ".join('"{}"'.format(l) for l in apply_to)
            )
        )
        for layer in apply_to:
...
class ConvertNamesToIndices(ClassProvider):
    ...
    def apply(self, data):
        ...
        apply_to = self.target_layers if self.target_layers is not None else data.keys()
        log.info(
            "Converting names {} to indices ...".format(
                ", ".join('"{}"'.format(l) for l in apply_to)
            )
        )
