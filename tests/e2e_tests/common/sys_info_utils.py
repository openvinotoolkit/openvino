...
def get_cpu_count():
    ...
    if os_type_is_linux():
        ...
        with contextlib.suppress(OSError, ValueError):
            ...
            cfs_quota = int(
                pathlib.Path("/sys/fs/cgroup/cpu/cpu.cfs_quota_us").read_text(
                    errors="strict"
                )
            )
            # Real world time length of a period (in microseconds)
            cfs_period = int(
                pathlib.Path("/sys/fs/cgroup/cpu/cpu.cfs_period_us").read_text(
                    errors="strict"
                )
            )
...
class CoreInfo:
    ...
    def get_cpu_cores(self):
        ...
        if os_type_is_linux():
            return self._get_lscpu_info(
                cpu_property_name="Core(s) per socket", regex=r"(\d+)$"
            )

        if os_type_is_windows():
            return self._get_coreinfo_info(
                cpu_property_opt="-c", regex=r"Physical Processor (\d+)"
            )
    ...
    def get_cpu_numa_nodes(self):
        ...
        if os_type_is_linux():
            return self._get_lscpu_info(
                cpu_property_name="NUMA node(s)", regex=r"(\d+)$"
            )

        if os_type_is_windows():
            return self._get_coreinfo_info(
                cpu_property_opt="-n", regex=r"NUMA Node (\d+)"
            )
...
def path_to_url(artifact_path, test_folder):
    ...
    if is_running_under_jenkins():
        work_dir = os.path.join(
            os.environ["WORKSPACE"], os.path.join("tests", test_folder)
        )
        return os.path.join(
            ...
            "tests",
            test_folder,
            os.path.relpath(artifact_path, work_dir),
        ).replace("\\", "/")
    return None
