...
class EltwiseComparator(ClassProvider):
    ...
    def __init__(self, config, infer_result, reference):
        ...
        self.mean_r_eps = (
            config.get("mean_r_eps")
            if config.get("mean_r_eps")
            else default_thresholds[2]
        )
        self._config = config
    ...
    def compare(self):
        log.info(
            "Running Element-Wise comparator with following parameters:\n"
        ...
        for layer in layers:
            ...
            if data.shape != ref.shape:
                log.error(
                    "Shape of IE output {} isn't equal with shape of FW output {} for layer '{}'. "
                    "Run Dummy comparator to get statistics.".format(
                        data.shape, ref.shape, layer
                    )
                )
            ...
            if not np.any(data) and not np.any(ref):
                ...
                if (np.isnan(data) == np.isnan(ref)).all() and (
                    np.isinf(data) == np.isinf(ref)
                ).all():
                    log.info(
                        "All output values were 'nan'/'inf' have converted to numbers"
                    )
                ...
                with np.errstate(invalid="ignore"):
                    rel_diff = np.array(
                        abs_diff / np.maximum(np.absolute(data), np.absolute(ref))
                    )
                ...
                if common_type == "mixed":
                    log.error(
                        "Type of IE output {} isn't equal with type of FW output {} for layer '{}'".format(
                            data_type, ref_type, layer
                        )
                    )
                    status = False
                ...
                max_abs_diff_ind = np.unravel_index(np.argmax(abs_diff), abs_diff.shape)
                max_rel_diff = np.amax(rel_diff)
                max_rel_diff_ind = np.unravel_index(np.argmax(rel_diff), rel_diff.shape)
                ...
                if np.isnan(rel_diff).all():
                    log.warning(
                        "Output data for layer {} consists only of zeros in both "
                        "inference and reference results.".format(layer)
                    )

        log.info(
            "Element-Wise comparison statistic:\n{}".format(
