    ...
    def __init__(self, config, infer_result, reference, result_aligner=None, xml=None):
        ...
        if result_aligner:
            if type(reference) is list:
                reference = [
                    cur_reference
                    for cur_reference, cur_infer_result in map(
                        result_aligner, reference, infer_result, xml
                    )
                ]
                infer_result = [
                    cur_infer_result
        ...
        for name, comparator in config.items():
            self.comparators[name] = ClassProvider.provide(
                name, config=comparator, infer_result=infer_result, reference=reference
            )
    ...
    def report_statuses(self):
        ...
        for name, comparator in self.comparators.items():
            if getattr(comparator, "ignore_results", False):
                log.info("Results comparison in comparator '{}' ignored!".format(name))
                continue
            if comparator.status:
                log.info("Results comparison in comparator '{}' passed!".format(name))
            else:
                log.error("Results comparison in comparator '{}' failed!".format(name))
            statuses.append(comparator.status)
        if len(statuses) == 0:
            log.warning("Statuses of all comparators are ignored! Test will be failed")
            return False
        else:
            return all(statuses)
