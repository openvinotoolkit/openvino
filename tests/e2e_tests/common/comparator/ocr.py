    ...
    def compare(self):
        log.info(
            ...
            "\t\t Relative difference threshold: {}".format(
                self.top_paths, self.a_eps, self.r_eps
            )
        )
        ...
        for batch in range(len(data["predictions"])):
            ...
            for ref_predict, ref_prob in zip(ref_predicts, ref_probs):
                if ref_predict in ie_predicts:
                    ...
                    rel_diff = (
                        0
                        if max(ie_probs[ie_predicts.index(ref_predict)], ref_prob) == 0
                        else abs_diff
                        / max(ie_probs[ie_predicts.index(ref_predict)], ref_prob)
                    )
                    ...
                    table_rows.append(
                        [
                            ref_predict,
                            ref_prob,
                            ie_probs[ie_predicts.index(ref_predict)],
                            abs_diff,
                            rel_diff,
                            status,
                        ]
                    ...
                    status = False
                    statuses.append(status)
                    table_rows.append([ref_predict, ref_prob, None, None, None, status])
