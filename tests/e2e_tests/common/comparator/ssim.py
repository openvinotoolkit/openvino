...
class SSIMComparator(ClassProvider):
    ...
    def compare(self):
        ...
        assert (
            layers
        ), f"No layers for comparison specified for comparator '{self.__action_name__}'"
        ...
        for layer in layers:
            for batch_num in range(len(self.infer_result[layer])):
                ...
                if (
                    ...
                    log.info(
                        f"Data or reference for layer {layer} contains np.nan or np.inf values. "
                        f"Lets compare their positions before filtering"
                    )
                    if (np.isnan(data) == np.isnan(ref)).all():
                        log.info(
                            f"Data and reference for layer {layer} contains np.nan values at the same positions. "
                            f"Filtering them"
                        )
                        data, ref = np.nan_to_num(data), np.nan_to_num(ref)
                    else:
                        log.info(
                            f"Data and reference for layer {layer} contains np.nan values but not at "
                            f"the same positions. Proceed further"
                        )

                    if (np.isinf(data) == np.isinf(ref)).all():
                        log.info(
                            f"Data and reference for layer {layer} contains np.inf values at the same positions. "
                            f"Filtering them"
                        )
                        data, ref = np.nan_to_num(data), np.nan_to_num(ref)
                    else:
                        log.info(
                            f"Data and reference for layer {layer} contains np.inf values but not at "
                            f"the same positions. Proceed further"
                        )
