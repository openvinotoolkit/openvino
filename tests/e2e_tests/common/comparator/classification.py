    ...
    def compare(self):
        ...
        for layer in layers:
            ...
            for b in range(len(data)):
                table_rows = []
                log.info(
                    "Comparing results for layer '{}' and batch {}".format(layer, b + 1)
                )
                infer = data[b]
                ref = self.reference[layer][b]
                ntop_classes_ref = list(self.reference[layer][b].keys())[: self.ntop]
                for class_id in ntop_classes_ref:
                    abs_diff = abs(infer[class_id] - ref[class_id])
                    rel_diff = (
                        0
                        if max(infer[class_id], ref[class_id]) == 0
                        else abs_diff / max(infer[class_id], ref[class_id])
                    )
