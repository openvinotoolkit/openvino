...
class SemanticSegmentationComparator(ClassProvider):
    ...
    def compare(self):
        compared = False
        log.info(
            "Running Semantic Segmentation comparator with threshold: {}\n".format(
                self.thr
            )
        )
        table_header = [
            ...
            if self.target_layers and (layer in self.target_layers):
                ...
                for batch_num in range(len(self.reference[layer])):
                    ...
                    for pixel_class in np.unique(ref_batch):
                        intersect = np.sum(
                            np.logical_and(
                                ref_batch == pixel_class, pred_batch == pixel_class
                            )
                        )
                        union = np.sum(
                            np.logical_or(
                        ...
                        union_sum += union
                        iou = intersect / union
                        class_part_intersect = intersect / (
                            pred_batch.shape[0] * pred_batch.shape[1]
                        )
                        class_part_union = union / (
                            pred_batch.shape[0] * pred_batch.shape[1]
                        )
                        table_rows.append(
                            [
                                layer,
                                pixel_class,
                                class_part_intersect,
                                class_part_union,
                                iou,
                            ]
                        )
                    log.info(
                        "Semantic Segmentation comparison statistic:\n{}".format(
                    ...
                    log.info(
                        "IoU between segmentations with the same class form reference and inference: {}".format(
                            mean_iou
                        )
                    )
                    log.info(
                        "Batch {0} status: {1}".format(
                            str(batch_num), str(statuses[-1])
                        )
                    )

        if compared == False:
            log.info(
                "Comparator {} has nothing to compare".format(str(self.__action_name__))
            )
