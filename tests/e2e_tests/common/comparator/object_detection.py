...
from .threshold_utils import get_default_iou_threshold, get_default_thresholds
...
class ObjectDetectionComparator(ClassProvider):
    ...
    def __init__(self, config, infer_result, reference):
        ...
        self.a_eps = (
            config.get("a_eps") if config.get("a_eps") else default_thresholds[0]
        )
        self.r_eps = (
            config.get("r_eps") if config.get("r_eps") else default_thresholds[1]
        )
        self.p_thr = config["p_thr"]
        self.iou_thr = (
            config.get("iou_thr")
            if config.get("iou_thr")
            else get_default_iou_threshold(
                config.get("precision", "FP32"), config.get("device", "CPU")
            )
        )
        ...
        self.target_layers = (
            ...
            if config.get("target_layers")
            else self.infer_result.keys()
        )
    ...
    def intersection_over_union(self, pred_coord, ref_coord):
        ...
        """
        if (
            (pred_coord["xmax"] < ref_coord["xmin"])
            or (ref_coord["xmax"] < pred_coord["xmin"])
            or (ref_coord["ymax"] < pred_coord["ymin"])
            or (pred_coord["ymax"] < ref_coord["ymin"])
        ):
            iou = 0
        else:
            intersection_coord = {}
            intersection_coord["xmin"] = max(pred_coord["xmin"], ref_coord["xmin"])
            intersection_coord["xmax"] = min(pred_coord["xmax"], ref_coord["xmax"])
            intersection_coord["ymin"] = max(pred_coord["ymin"], ref_coord["ymin"])
            intersection_coord["ymax"] = min(pred_coord["ymax"], ref_coord["ymax"])
            intersection_square = (
                intersection_coord["xmax"] - intersection_coord["xmin"]
            ) * (intersection_coord["ymax"] - intersection_coord["ymin"])
            union_square = (
                (pred_coord["xmax"] - pred_coord["xmin"])
                * (pred_coord["ymax"] - pred_coord["ymin"])
                + (ref_coord["xmax"] - ref_coord["xmin"])
                * (ref_coord["ymax"] - ref_coord["ymin"])
                - intersection_square
            )
            if union_square == 0:
                iou = 1
        ...
        for layer in data.keys():
            if layer in self.target_layers:
                ...
                for batch_num in range(len(data[layer])):
                    batch_filtered = [
                        bbox
                        for bbox in data[layer][batch_num]
                        if bbox["prob"] >= threshold
                    ]
                    deleted_bound_boxes += len(data[layer][batch_num]) - len(
                        batch_filtered
                    )
                    if batch_filtered:
    ...
    def find_matches(self, prediction, reference):
        ...
        for layer in layers:
            for batch_num in range(len(prediction[layer])):
                force_fail = False
                log.info(
                    "Comparing results for layer '{}' and batch {}".format(
                        layer, batch_num
                    )
                )
                matrix = {}
                ...
                if len(ref_detections) != len(pred_detections):
                    ...
                    if len(ref_detections) == 0:
                        log.error(
                            "Reference doesn't contain detections in batch {} for layer '{}'".format(
                                batch_num, layer
                            )
                        )
                        force_fail = True
                    ...
                    if len(pred_detections) == 0:
                        log.error(
                            "Prediction doesn't contain detections in batch {} for layer '{}'".format(
                                batch_num, layer
                            )
                        )
                        force_fail = True
                    if len(ref_detections) == 0 and len(pred_detections) == 0:
                        force_fail = False
                        log.error(
                            "Both reference and prediction results doesn't contain "
                ...
                if detected_classes != set([bbox["class"] for bbox in pred_detections]):
                    log.error(
                        "Classes of detected objects are different in batch {} for layer '{}'".format(
                            batch_num, layer
                        )
                    )
                    force_fail = True
                ...
                for class_num in detected_classes:
                    matrix[class_num] = np.zeros(
                        (len(ref_detections), len(pred_detections))
                    )
                for i, ref_bbox in enumerate(ref_detections):
                    for j, pred_bbox in enumerate(pred_detections):
                        if ref_bbox["class"] == pred_bbox["class"]:
                            matrix[ref_bbox["class"]][i][j] = (
                                self.intersection_over_union(ref_bbox, pred_bbox)
                            )
                ...
                for class_num in detected_classes:
                    if np.max(matrix[class_num]) == 0:
                        log.warning(
                            "There is no pair of detections which has IOU > 0 for class {}".format(
                                class_num
                            )
                        )
                        no_detections = True
                    else:
                        required_pairs_len += len(
                            [1 for bbox in ref_detections if bbox["class"] == class_num]
                        )
                        while len(pairs) != required_pairs_len:
                            # Search pair of detected objects with max IoU
                            i, j = np.unravel_index(
                                np.argmax(matrix[class_num], axis=None),
                                matrix[class_num].shape,
                            )
                            ...
                            pairs.append(
                                OrderedDict(
                                    [
                                        ...
                                        (
                                            "abs_diff",
                                            abs(ref_bbox["prob"] - pred_bbox["prob"]),
                                        ),
                                        (
                                            "rel_diff",
                                            abs(ref_bbox["prob"] - pred_bbox["prob"])
                                            / max(ref_bbox["prob"], pred_bbox["prob"]),
                                        ),
                                        (
                                            "ref_coord",
                                            (
                                                (
                                                    round(ref_bbox["xmin"], 3),
                                                    round(ref_bbox["ymin"], 3),
                                                ),
                                                (
                                                    round(ref_bbox["xmax"], 3),
                                                    round(ref_bbox["ymax"], 3),
                                                ),
                                            ),
                                        ),
                                        (
                                            ...
                                            (
                                                (
                                                    round(pred_bbox["xmin"], 3),
                                                    round(pred_bbox["ymin"], 3),
                                                ),
                                                (
                                                    round(pred_bbox["xmax"], 3),
                                                    round(pred_bbox["ymax"], 3),
                                                ),
                                            ),
                                        ),
                                    ]
                            ...
                            matrix[class_num][:, j] = np.zeros(
                                matrix[class_num].shape[0]
                            )
                ...
                if pairs:
                    ...
                    status.append(False)
                    log.warning(
                        "No detection pairs have IOU > 0 for batch {}".format(batch_num)
                    )
    ...
    def logs_prereq(self):
        log.info(
            ...
            "\t\t IOU threshold: {}".format(
                self.p_thr, self.a_eps, self.r_eps, self.iou_thr
            )
        )
        if self.mean_iou_only:
            log.info(
                "For comparison will be used mean IoU of all boxes' pairs instead IoU of every pair"
            )
        if sorted(self.infer_result.keys()) != sorted(self.reference.keys()):
            log.error(
                ...
                "Output layers in reference: {}".format(
                    sorted(self.infer_result.keys()), sorted(self.reference.keys())
                )
            )
    ...
    def compare(self):
        ...
        infer_result_filtered, infer_num_deleted = self.prob_threshold_filter(
            self.p_thr, self.infer_result
        )
        reference_filtered, reference_num_deleted = self.prob_threshold_filter(
            self.p_thr, self.reference
        )
        log.info(
            "{} predictions were deleted from IE predictions set after comparing with probability threshold!".format(
                str(infer_num_deleted)
            )
        )
        log.info(
            "{} predictions were deleted from reference set after comparing with probability threshold!".format(
                str(reference_num_deleted)
            )
        )
