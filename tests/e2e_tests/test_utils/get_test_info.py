...
class TestInfo:
    ...
    def fill_mo_args(self, **mo_params):
        ...
        for mo_key in mo_params.keys():
            if mo_key == "additional_args":
                ...
                self.extra_info["model_optimizer_args"].update(
                    {mo_key: str(mo_params[mo_key])}
                )
    ...
    def fill_extra_info(self, op_type, name, op, shape, dtype):
        ...
        if op_type == "Const":
            self.extra_info["Constants"].append(
                {"name": name, "op": op, "shape": shape, "dtype": dtype}
            )

        if op_type == "FakeConst":
            self.extra_info["FakeConstants"].append(
                {"name": name, "op": op, "shape": shape, "dtype": dtype}
            )

        if op_type == "Variable":
            self.extra_info["Variables"].append(
                {"name": name, "op": op, "shape": shape, "dtype": dtype}
            )

        if op_type == "Input":
            self.extra_info["Inputs"].append(
                {"name": name, "op": op, "shape": shape, "dtype": dtype}
            )
        if op_type == "Intermediate":
            self.extra_info["Intermediates"].append(
                {"name": name, "op": op, "shape": shape, "dtype": dtype}
            )

        if op_type == "Output":
            self.extra_info["Outputs"].append(
                {"name": name, "op": op, "shape": shape, "dtype": dtype}
            )
