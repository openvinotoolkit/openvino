...
from e2e_tests.pipelines.pipeline_templates.comparators_template import (
    dummy_comparators,
    eltwise_comparators,
)
...
def should_run_reshape(instance) -> bool:
    ...
    if all(
        [v.get("changeable_dims") is None for v in instance.input_descriptor.values()]
    ):
...
def get_reshape_pipeline_pairs(instance) -> list:
    ...
    if len(types) == 1 or isinstance(types, str):
        log.info(
            f"Only {types} reshape pipeline was set for {instance.__class__.__name__}"
        )
        return [types]
    else:
        pipelines_pairs = []
    ...
    for input_layer, layer_value in changeable_dims.items():
        ...
        if layer_value is not None:
            for dimension in layer_value:
                assert (
                    dimension in layout[input_layer]
                ), "Dimension '{}' wasn't found in input '{}'" " layout: {}".format(
                    dimension, input_layer, layout[input_layer]
                )
                ...
                for data in layer_value[dimension]:
                    assert len(data) == len(dimension), (
                        ...
                        " as length of changeable dimension".format(
                            data, dimension, input_layer
                        )
                    )
    ...
    for input_layer, input_dimension in dims_to_change[test_number]:
        if input_dimension is None:
            ...
            for value_index, value in enumerate(
                values_to_change[input_layer][test_number]
            ):
                if value is None:
                    continue
                else:
...
def get_reshape_configurations(reshape_test_case, dynamism_type) -> list:
    ...
    default_shapes = {
        k: v["default_shape"]
        for k, v in input_descriptor.items()
        if not v.get("frozen_input")
    }
    layout = {
        k: v["layout"] for k, v in input_descriptor.items() if not v.get("frozen_input")
    }
    changeable_dims = {
        k: v["changeable_dims"]
        for k, v in input_descriptor.items()
    ...
    for test in range(number_of_tests):
        reshape_config = construct_new_shapes(
            test,
            default_shapes,
            layout,
            dims_to_change,
        ...
        reshape_configurations.append(
            SimpleNamespace(
                shapes=reshape_config,
                changed_dims=dict(dims_to_change[test]),
                layout=layout,
...
def get_input_data(shapes):
    return {
        "dynamism_preproc": {
            "execution_function": lambda data: replicator(data, shapes)
        }
    }
...
def compare(instance, ref_results, cur_results):
    assert (
        len(instance.comparators) == 1 or "dummy" not in instance.comparators
    ...
    if not ref_results:
        ...
        instance.comparators = eltwise_comparators(
...
def reorder_shapes_to_old_api(shapes):
    ...
    for k, v in shapes.items():
        if len(v) in [4, 5]:
            reorder_shapes[k] = tuple(
                np.array(v).take((0, len(v) - 1, *list(range(1, len(v) - 1))))
            )
...
def prepare_data_consecutive_inferences(
    default_shapes, changed_values, layout, dims_to_change
):
    def construct_input_data(data):
        input_data = copy.deepcopy(data)
        consecutive_infer_input_data = [data]

        changed_data_shapes = get_static_shape(
            default_shapes, changed_values, layout, dims_to_change
        )
    ...
    return {
        "dynamism_preproc": {
            "execution_function": lambda data: construct_input_data(data)
        }
    }
...
def replicator(data, shapes):
    for name, shape in shapes.items():
        ...
        if np.all((new_shape % old_shape) == 0):
            assert np.all(
                old_shape <= new_shape
            ), "Reshaping to shape that is less than original network shape"
            ...
            log.info(
                "New shape is not evenly divided by original network shape data_shape={}, net_shape={}"
                "".format(data[name].shape, new_shape)
            )
            assert (
                len(new_shape) == 4
            ), "Unsupported by tests reshape: Non 4D input {}, original shape {}".format(
                new_shape, old_shape
            )

            multiplier = tuple(
                np.array(new_shape // old_shape + np.ones(new_shape.size), dtype=np.int)
            )
            replicated_data = np.tile(data[name], multiplier)
            data[name] = replicated_data[
                0 : new_shape[0], 0 : new_shape[1], 0 : new_shape[2], 0 : new_shape[3]
            ]

        assert np.array_equal(
            data[name].shape, new_shape
        ), err_msg + "data_shape={}, net_shape={}".format(data[name].shape, new_shape)

    log.info(
        "Input data was aligned with shapes=`{}`, new_data_shapes=`{}`"
