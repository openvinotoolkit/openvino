...
from e2e_tests.test_utils.reshape_tests_utils import (
    get_input_data,
    get_mo_input_with_frozen_values,
    reorder_shapes_to_old_api,
)
...
def mo_reshape_config(pipeline, shapes, instance_class_name):
    ...
    mo_arg_input = (
        pipeline["get_ir"]["get_ovc_model"].get("additional_args").get("input")
    )
    ...
    if mo_arg_input is not None and "->" in mo_arg_input:
        ...
        mo_config["get_ir"]["get_ovc_model"]["additional_args"].update(
            {"input": ",".join(list(map(str, cmd_mo_input)))}
        )
    else:
        mo_config["get_ir"]["get_ovc_model"]["additional_args"].update(
            {
    ...
    mo_config = update_pre_post_process_reshape_config(
        mo_config, shapes, instance_class_name
    )
...
def ie_reshape_config(pipeline, shapes, test_name):
    ...
    ie_config["infer"][list(ie_config["infer"].keys())[0]]["network_modifiers"] = {
        "reshape": {"shapes": ie_shapes}
    }
...
def update_pre_post_process_reshape_config(
    ...
    if (
        instance_class_name.lower().startswith("tf")
        and "ie_sync" in instance_ie_pipeline["infer"]
    ):
    ...
    if not consecutive_infer:
        ...
        config["preprocess"].update(
            prepare_data_consecutive_inferences(
                default_shapes, changed_values, layout, changed_dims
            )
        )
...
def dynamism_config(
    ...
    if dynamic_config["infer"][list(dynamic_config["infer"].keys())[0]].get(
        "network_modifiers"
    ):
        for item in dynamic_config["infer"][list(dynamic_config["infer"].keys())[0]][
            "network_modifiers"
        ]:
            if item not in reshape_action_list:
                infer_network_modifiers[item] = dynamic_config["infer"][
                    list(dynamic_config["infer"].keys())[0]
                ]["network_modifiers"][item]

    dynamic_config["infer"][list(dynamic_config["infer"].keys())[0]][
        "network_modifiers"
    ] = {"reshape": {"shapes": shapes}}
    dynamic_config["infer"][list(dynamic_config["infer"].keys())[0]][
        "network_modifiers"
    ].update(infer_network_modifiers)
    if consecutive_infer_num:
        dynamic_config = update_pre_post_process_reshape_config(
            dynamic_config,
            shapes,
            test_name,
            default_shapes,
