...
from e2e_tests.test_utils.test_utils import (
    check_mo_precision,
    get_ir_tag,
    log_timestamp,
    read_irs_mapping_file,
    remove_mo_args_oob,
    set_infer_precision_hint,
    store_data_to_csv,
    timestamp,
)
...
def _test_run(
    instance, pregen_irs, record_property, prepare_test_info, inference_precision_hint
):
    """Parameterized test.
    ...
    ref_pipeline = Pipeline(
        instance_ref_pipeline, {"model_obj": model, "feed_dict": feed_dict}
    )
    ...
    if ref_pipeline.steps:
        ...
        log.warning(
            "Reference pipeline is empty, no results comparison will be performed"
        )
    ...
    if pregen_irs and "get_ir" in instance_ie_pipeline:
        try:
            ...
            if ir_tag not in irs_mapping:
                log.warning(
                    "IR with tag '{}' not found in IRs mapping. "
                    "IR will be generated in runtime ...".format(ir_tag)
                )
            else:
                log.info(
                    "Found pre-generated IR entry in IRs mapping: {}.\nTrying to reuse it ...".format(
                        {ir_tag: irs_mapping[ir_tag]}
                    )
                )
                pregen_ir_status, mo_log, xml, bin = irs_mapping[ir_tag]
                if not pregen_ir_status:
                    log.error(
                        "IR pre-generation failed. IR will be generated in runtime ..."
                    )
                else:
                    if not mo_log:
                        log.warning(
                            "IR was collected successfully, but MO log was not saved."
                        )
                    else:
                    ...
                    if not (Path(xml).exists() and Path(bin).exists()):
                        log.error(
                            "One of IR's .xml or .bin files not found. IR will be generated in runtime ..."
                        )
                    else:
                        ir_http_path = "http://{}".format(
                            re.sub(r"^[/|\\]+", "", str(xml)).replace("\\", "/")
                        )
                        record_property("ir_link", ir_http_path)
                        instance_ie_pipeline["get_ir"] = {
                            "pregenerated": {"xml": xml, "bin": bin}
                        }
        except Exception as ex:
            log.error(
                "Search of pre-generated IR failed with error: {err}."
    ...
    if instance_ie_pipeline.get("infer"):
        instance_ie_pipeline = set_infer_precision_hint(
            instance, instance_ie_pipeline, inference_precision_hint
        )
    ie_pipeline = Pipeline(
        instance_ie_pipeline, {"model_obj": model, "feed_dict": feed_dict}
    )
    log.debug(
...
def empty_dirs(env_conf):
    ...
    for env_clean_dir_flag, test_cfg_dir_to_clean in [
        ("TT_CLEAN_MO_OUT_DIR", "mo_out"),
        ("TT_CLEAN_PREGEN_IRS_DIR", "pregen_irs_path"),
        ("TT_CLEAN_INPUT_MODEL_DIR", "input_model_dir"),
    ]:
        clean_flag = (
            True if os.environ.get(env_clean_dir_flag, "False") == "True" else False
        )
        if clean_flag:
