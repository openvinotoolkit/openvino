...
from e2e_tests.pipelines.pipeline_templates.postproc_template import (
    assemble_postproc_tf,
)
from e2e_tests.test_utils.path_utils import ref_from_model


def get_refs_onnx_runtime(
    model, onnx_rt_ep, inputs, cast_input_data=True, cast_type="float32"
):
    """
...
def get_refs_tf(
    inputs,
    model=None,
    output_nodes_for_freeze=None,
    additional_outputs=[],
    additional_inputs=[],
    override_default_outputs=False,
    override_default_inputs=False,
    saved_model_dir=None,
    user_output_node_names_list=[],
    score_class_name="score_tf",
):
    """
...
def read_refs_pipeline(ref_file, batch):
    ...
    """
    return [
        ("get_refs", {"precollected": {"path": ref_file}}),
...
def read_pytorch_refs_pipeline(ref_file, batch):
    ...
    """
    return [
        ("get_refs", {"torch_precollected": {"path": ref_file}}),
        ("postprocessor", {"align_with_batch": {"batch": batch}}),
    ]


@wrap_ord_dict
def read_tf_refs_pipeline(
    ref_file, batch=None, align_with_batch_od=False, postprocessors=None
):
    """
    Construct Read pre-collected references pipeline
    ...
    return [
        (
            "get_refs",
            {"precollected": {"path": ref_from_model(ref_file, framework="tf")}},
        ),
        assemble_postproc_tf(
            batch=batch, align_with_batch_od=align_with_batch_od, **postprocessors
        ),
    ]
...
def collect_paddlepaddle_refs(model, inputs, params_filename=None, ref_name=None):
    ...
    return {
        "pipeline": OrderedDict(
            [
                get_refs_paddlepaddle(
                    model=model, inputs=inputs, params_filename=params_filename
                )
            ]
        ),
        "store_path": ref_from_model(ref_name, framework="paddlepaddle"),
        "store_path_for_ref_save": ref_from_model(
            ref_name, framework="paddlepaddle", check_empty_ref_path=False
        ),
    }
...
def collect_tf_refs_pipeline(model, inputs, saved_model_dir=None, ref_name=None):
    ...
    return {
        "pipeline": OrderedDict(
            [
                (
                    get_refs_tf(inputs=inputs, model=model)
        ...
        "store_path_for_ref_save": ref_from_model(
            ref_name, framework="tf", check_empty_ref_path=False
        ),
    }


def collect_onnx_refs_pipeline(
    model,
    inputs,
    onnx_rt_ep,
    framework,
    ...
    return {
        ...
        "store_path_for_ref_save": ref_from_model(
            ref_name, framework=framework, check_empty_ref_path=False
        ),
    }
