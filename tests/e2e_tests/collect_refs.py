...
def test_collect_reference(reference, dry_run):
    ...
    if not dry_run:
        save_reference(
            refs, reference["store_path"], reference.get("use_torch_to_save", False)
        )
        # Always save to `store_path_for_ref_save` (it points to share in automatics)
        if (
            "store_path_for_ref_save" in reference
            and reference["store_path"] != reference["store_path_for_ref_save"]
        ):
            save_reference(
                refs,
                reference["store_path_for_ref_save"],
                reference.get("use_torch_to_save", False),
            )
    else:
