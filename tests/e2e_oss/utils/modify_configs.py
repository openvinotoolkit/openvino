...
def mo_reshape_config(pipeline, shapes, instance_class_name):
    ...
    mo_arg_input = pipeline['get_ir']['mo'].get('additional_args').get('input')

    if mo_arg_input is not None and "->" in mo_arg_input:
        cmd_mo_input = get_mo_input_with_frozen_values(mo_arg_input, shapes)
        mo_config['get_ir']['mo']['additional_args'].update({'input': ','.join(list(map(str, cmd_mo_input)))})
    else:
        mo_config['get_ir']['mo']['additional_args'].update({
    ...
    for attribute in ['batch']:
        if attribute in mo_config['get_ir']['mo']['additional_args']:
            del mo_config['get_ir']['mo']['additional_args'][attribute]
...
def ie_reshape_config(pipeline, shapes, test_name):
    ...
    ie_api = next(iter(pipeline['infer']))
    if test_name.lower().startswith('tf') and 'ie_sync' in pipeline['infer']:
        ie_shapes = reorder_shapes_to_old_api(shapes)
    reshape_api = 'reshape_api_2' if ie_api == 'ie_sync_api_2' else 'reshape'

    # we force model optimizer to generate reshapable IR
    ie_config['infer'][list(ie_config['infer'].keys())[0]]['network_modifiers'] = {reshape_api: {'shapes': ie_shapes}}
    ie_config = update_pre_post_process_reshape_config(ie_config, shapes, test_name)

    return ie_config


def ie_sbs_reshape_config(pipeline, shapes, test_name):
    ie_sbs_config = deepcopy(pipeline)
    shape = iter(shapes.values()).__next__()
    batch = shape[0]

    ie_api = next(iter(pipeline['infer']))
    set_batch_api = 'set_batch_using_reshape_api_2' if ie_api == 'ie_sync_api_2' else 'set_batch'

    # we force model optimizer to generate reshapable IR
    ie_sbs_config['infer'][list(ie_sbs_config['infer'].keys())[0]]['network_modifiers'] = {
        set_batch_api: {'batch': batch}}
    ie_sbs_config = update_pre_post_process_reshape_config(ie_sbs_config, shapes, test_name)

    return ie_sbs_config
...
def dynamism_config(instance_ie_pipeline, shapes, test_name, default_shapes, changed_values, layout, changed_dims,
    ...
    reshape_action_list = ['set_batch_using_reshape_api_2', 'reshape_api_2']
...
def get_original_model_importer_pipeline_config(instance_ie_pipeline):
    ...
    model_path = ie_config["get_ir"]["mo"]["model"]
