...
def _test_run(instance, ir_gen_time_csv_name, load_net_to_plug_time_csv_name, mem_usage_mo_csv_name,
    ...
    if use_mo_legacy_frontend:
        ...
        if 'ONNX' or 'Precollected' or 'Caffe2' or 'WinML' in model_framework_name:
            prepare_test_info['pytestEntrypoint'] = 'E2E legacy Frontend: ONNX'
        if 'TF' in model_framework_name:
            prepare_test_info['pytestEntrypoint'] = 'E2E legacy Frontend: TF'
        elif not api_2:
    ...
    if instance_ie_pipeline.get('prepare_model') and instance.convert_pytorch_to_onnx:
        instance_ie_pipeline["prepare_model"]["prepare_model_for_mo"].update(
            {"convert_pytorch_to_onnx": instance.convert_pytorch_to_onnx})
