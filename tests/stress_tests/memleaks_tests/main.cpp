...
bool parseAndCheckCommandLine(int argc, char** argv) {
  ...
  if (!result) {
    log_err(
        "Exception while reading test config \""
        << FLAGS_test_conf << "\": " << result.description());
    return false;
  }
  return true;
}
...
int main(int argc, char** argv) {
  log_warn(
      "Use of attribute \"processes\" from config isn't implemented yet. It will be ignored.");
  log_warn(
      "Use of attribute \"threads\" from config greater than 1 is risky because of "
