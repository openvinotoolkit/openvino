...
class MemLeaksTestSuiteNoModel
    : public ::testing::TestWithParam<MemLeaksTestCase> {};

class MemLeaksTestSuiteNoDevice
    : public ::testing::TestWithParam<MemLeaksTestCase> {};

class MemLeaksTestSuite : public ::testing::TestWithParam<MemLeaksTestCase> {};

inline void test_runner(
    int numthreads,
  ...
  for (int i = 0; i < numthreads; i++) {
    ...
    EXPECT_EQ(results[i].first, TestStatus::TEST_OK)
        << "[Thread " << i << "] " << results[i].second;
  }
}
...
TEST_P(MemLeaksTestSuiteNoModel, load_unload_plugin) {
  ...
  std::vector<std::function<void()>> pipeline = {
      load_unload_plugin(test_params.device)};
  auto test = [&] {
    log_info(
        "Load/unload plugin for \"" << test_params.device << "\" device"
                                    << " for " << test_params.numiters
                                    << " times");
...
TEST_P(MemLeaksTestSuiteNoDevice, read_network) {
  ...
  auto test = [&] {
    log_info(
        "Read networks: " << test_params.model_name << " for "
...
TEST_P(MemLeaksTestSuiteNoDevice, cnnnetwork_reshape_batch_x2) {
  ...
  for (int i = 0; i < test_params.models.size(); i++) {
    pipeline.push_back(
        cnnnetwork_reshape_batch_x2(test_params.models[i]["full_path"], i));
  }
  auto test = [&] {
    log_info(
        "Reshape to batch*=2 of CNNNetworks created from networks: "
        << test_params.model_name << " for " << test_params.numiters
        << " times");
    return common_test_pipeline(pipeline, test_params.numiters);
  };
  test_runner(test_params.numthreads, test);
...
TEST_P(MemLeaksTestSuiteNoDevice, set_input_params) {
  ...
  auto test = [&] {
    log_info(
        "Apply preprocessing for CNNNetworks from networks: "
        << test_params.model_name << " for " << test_params.numiters
        << " times");
    return common_test_pipeline(pipeline, test_params.numiters);
  };
...
TEST_P(MemLeaksTestSuite, recreate_compiled_model) {
  ...
  auto test = [&] {
    log_info(
        "Recreate CompiledModels within existing ov::Core from networks: "
        << test_params.model_name << " for \"" << test_params.device
...
TEST_P(MemLeaksTestSuite, recreate_infer_request) {
  ...
  auto test = [&] {
    log_info(
        "Create InferRequests from networks: "
        << test_params.model_name << " for \"" << test_params.device
        << "\" device for " << test_params.numiters << " times");
    return common_test_pipeline(pipeline, test_params.numiters);
  };
...
TEST_P(MemLeaksTestSuite, reinfer_request_inference) {
  ...
  auto test = [&] {
    log_info(
        "Inference of InferRequests from networks: "
        << test_params.model_name << " for \"" << test_params.device
        << "\" device for " << test_params.numiters << " times");
    return common_test_pipeline(pipeline, test_params.numiters);
  };
...
TEST_P(MemLeaksTestSuite, infer_request_inference) {
  ...
  pipeline.reserve(test_params.models.size());
  for (int i = 0; i < test_params.models.size(); i++) {
    pipeline.push_back(infer_request_inference(
        test_params.models[i]["full_path"], test_params.device));
  }
  auto test = [&] {
    log_info(
        "Inference of InferRequests from networks: "
...
TEST_P(MemLeaksTestSuite, inference_with_streams) {
  ...
  pipeline.reserve(test_params.models.size());
  for (int i = 0; i < test_params.models.size(); i++) {
    pipeline.push_back(inference_with_streams(
        test_params.models[i]["full_path"], test_params.device, nstreams));
  }
  auto test = [&] {
    log_info(
        "Inference of InferRequests from networks: "
        << test_params.model_name << " for \"" << test_params.device
        << "\" device with " << nstreams << " streams for "
        << test_params.numiters << " times");
    return common_test_pipeline(pipeline, test_params.numiters);
  };
  test_runner(test_params.numthreads, test);
...
TEST_P(MemLeaksTestSuite, recreate_and_infer_in_thread) {
  ...
  auto test = [&] {
    log_info(
        "Inference in separate thread of InferRequests from networks: "
        << test_params.model_name << " for \"" << test_params.device
        << "\" device for " << test_params.numiters << " times");
