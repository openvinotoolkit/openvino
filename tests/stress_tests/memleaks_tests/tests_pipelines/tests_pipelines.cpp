...
// Number of pipeline runs before it starts measuring. Real number will be
// aliquot to AVERAGE_NUM
...
void transform(const In1& in1, const In2& in2, Out& out, const Func& func) {
  std::transform(
      std::begin(in1), std::end(in1), std::begin(in2), std::begin(out), func);
}
} // namespace util

TestResult common_test_pipeline(
    const std::vector<std::function<void()>>& test_pipeline,
    const int& n,
  ...
  if (AVERAGE_NUM > n)
    return TestResult(
        ...
        "Test failed: number of iterations less than defined AVERAGE_NUM");
  ...
  std::array<long, MeasureValueMax>
      diff{}; // difference between current and reference
  std::array<bool, MeasureValueMax>
      outlier{}; // flag if current does not fit upper_bound
  std::array<int, MeasureValueMax>
      outlier_count{}; // counter for how many times current does not fit
                       // upper_bound
  std::array<float, MeasureValueMax> upper_bound{}; // ref * threshold
  std::vector<std::array<long, MeasureValueMax>> past; // past measures
  std::array<long, MeasureValueMax>
      sliding_avg{}; // sliding average computed as avg of past AVERAGE_NUM
                     // values
  ...
  for (size_t iteration = 1, measure_count = n / AVERAGE_NUM;; iteration++) {
    ...
    for (const auto& step : test_pipeline)
      step();
    ...
    if (iteration % AVERAGE_NUM == 0) {
      ...
      util::transform(
          sliding_avg, sliding_avg, [](long sliding_avg_val) -> float {
            return sliding_avg_val / AVERAGE_NUM;
          });

      progress_str = std::to_string(iteration) + "\t" +
          std::to_string(sliding_avg[VMRSS]) + "\t" +
          std::to_string(sliding_avg[VMHWM]) + "\t" +
          std::to_string(sliding_avg[VMSIZE]) + "\t" +
          std::to_string(sliding_avg[VMPEAK]) + "\t" +
          std::to_string(sliding_avg[THREADS]);
      ...
      if (iteration >= WARMUP_STEPS) {
        ...
        if ((ref == std::array<long, MeasureValueMax>{-1}) ||
            (retry_count <= MAX_RETRY &&
             (outlier_count[VMRSS] > MAX_OUTLIERS ||
              outlier_count[VMHWM] > MAX_OUTLIERS))) {
          if (0 != retry_count)
            log_info("Retrying " << retry_count << " of " << MAX_RETRY);
          ...
          util::transform(ref, upper_bound, [&](long ref_val) -> float {
            return threshold * ref_val;
          });
          log_info(
              "Setting upper bounds:"
              << " VMRSS=" << ref[VMRSS] << "(+-"
              << static_cast<int>(upper_bound[VMRSS]) << "),"
              << " VMHWM=" << ref[VMHWM] << "(+-"
              << static_cast<int>(upper_bound[VMHWM]) << ")");
        ...
        util::transform(
            ...
            ref,
            diff,
            [](long sliding_avg_val, long ref_val) -> long {
        ...
        util::transform(
            ...
            upper_bound,
            outlier,
            [](long diff_val, float upper_bound_val) -> bool {
              return diff_val > upper_bound_val;
            });
  ...
  if (outlier_count[VMRSS] > MAX_OUTLIERS)
    return TestResult(
        TestStatus::TEST_FAILED,
        "Test failed: RSS virtual memory consumption grown too much.");

  if (outlier_count[VMHWM] > MAX_OUTLIERS)
    return TestResult(
        TestStatus::TEST_FAILED,
        "Test failed: HWM virtual memory consumption grown too much.");
