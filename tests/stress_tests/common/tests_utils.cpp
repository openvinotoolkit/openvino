...
std::vector<TestCase> generateTestsParams(
  ...
  for (const auto& field : fields) {
    if (field == "processes") {
      ...
      for (pugi::xml_node val = values.first_child(); val;
      ...
      for (pugi::xml_node val = values.first_child(); val;
        ...
        if (full_path.empty() || path.empty())
          throw std::logic_error(
              "One of the 'model' records from test config doesn't contain 'full_path' or "
              "'path' attributes");
        else {
  ...
  precisions =
      !precisions.empty() ? precisions : std::vector<std::string>{"NULL"};
  models_names =
      !models_names.empty() ? models_names : std::vector<std::string>{"NULL"};
...
std::vector<MemLeaksTestCase> generateTestsParamsMemLeaks() {
  ...
  for (pugi::xml_node device = cases.first_child(); device;
    ...
    for (pugi::xml_node model = device.first_child(); model;
    ...
    tests_cases.emplace_back(
        numprocesses, numthreads, numiterations, device_name, models);
  }
...
std::string getTestCaseNameMemLeaks(
    const testing::TestParamInfo<MemLeaksTestCase>& obj) {
  return obj.param.test_case_name;
}

void test_wrapper(
    const std::function<void(std::string, std::string, int)>& tests_pipeline,
...
void runTest(
  ...
  tests_pipeline(params.model, params.device, params.numiters);
#else
  int status = run_in_processes(
      params.numprocesses, [&]() { _runTest(tests_pipeline, params); });
  ASSERT_EQ(status, 0) << "Test failed with exitcode "
