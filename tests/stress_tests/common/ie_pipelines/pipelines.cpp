...
std::function<void()> cnnnetwork_reshape_batch_x2(
...
std::function<void()> create_compiled_model(
    const std::string& model,
...
std::function<void()> recreate_compiled_model(
    std::shared_ptr<InferApiBase>& ie_wrapper,
    const std::string& target_device) {
  return [=] { ie_wrapper->load_network(target_device); };
}

std::function<void()> create_infer_request(
    const std::string& model,
    const std::string& target_device) {
  return [&] {
    auto ie_api_wrapper = create_infer_api_wrapper();
...
std::function<void()> recreate_infer_request(
    std::shared_ptr<InferApiBase>& ie_wrapper) {
  return [=] { ie_wrapper->create_infer_request(); };
}

std::function<void()> infer_request_inference(
    const std::string& model,
    const std::string& target_device) {
  return [&] {
    auto ie_api_wrapper = create_infer_api_wrapper();
...
std::function<void()> reinfer_request_inference(
    std::shared_ptr<InferApiBase>& ie_wrapper) {
  return [=] { ie_wrapper->infer(); };
}

std::function<void()> recreate_and_infer_in_thread(
    std::shared_ptr<InferApiBase>& ie_wrapper) {
  return [=] {
    auto func = [=] {
...
std::function<void()> inference_with_streams(
    const std::string& model,
    const std::string& target_device,
    const int& nstreams) {
  return [&] {
    ...
    auto ie_api_wrapper = create_infer_api_wrapper();
    ie_api_wrapper->load_plugin(target_device);
    ie_api_wrapper->set_config(
        target_device, ov::AnyMap{ov::num_streams(nstreams)});
    ie_api_wrapper->read_network(model);
    ie_api_wrapper->load_network(target_device);
    try {
