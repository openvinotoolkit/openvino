...
void InferAPI2::change_batch_size(int multiplier, int cur_iter) {
  int new_batch_size = ((cur_iter % 2) == 0) ? original_batch_size * multiplier
                                             : original_batch_size;
  ...
  for (size_t i = 0; i < inputs.size(); ++i) {
    ...
    if (inputs[i].get_shape().size() == 4) {
      ...
      throw std::logic_error(
          "Setting of input parameters wasn't applied for a model.");
    }
  }
  network = ppp.build();
  inputs = network->inputs();
