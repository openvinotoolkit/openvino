...
// delimiter used for measurements print. Should be compatible with script
// parses tests logs
...
std::string MemCheckPipeline::get_reference_record_for_test(
  ...
  std::array<long, MeasureValueMax> measures = measure();
  std::stringstream ss;
  ss << "Record to update reference config: " << "<model path=\"" << model_name
     << "\" precision=\"" << precision << "\" test=\"" << test_name
     << "\" device=\"" << target_device << "\" vmsize=\""
     << (int)(measures[VMSIZE] * REPORTING_THRESHOLD) << "\" vmpeak=\""
     << (int)(measures[VMPEAK] * REPORTING_THRESHOLD) << "\" vmrss=\""
     << (int)(measures[VMRSS] * REPORTING_THRESHOLD) << "\" vmhwm=\""
     << (int)(measures[VMHWM] * REPORTING_THRESHOLD) << "\" />";
  return ss.str();
}
...
TestResult common_test_pipeline(
  ...
  if (measures[VMRSS] > references[VMRSS])
    return TestResult(
        ...
        "Reference RSS memory consumption: " +
            std::to_string(references[VMRSS]) + " KB.\n" +
            "Current RSS memory consumption: " +
            std::to_string(measures[VMRSS]) + " KB.\n");

  if (measures[VMHWM] > references[VMHWM])
    return TestResult(
        TestStatus::TEST_FAILED,
        "Test failed: HWM (peak of RSS) virtual memory consumption is greater than reference.\n"
        "Reference HWM of memory consumption: " +
            std::to_string(references[VMHWM]) + " KB.\n" +
            "Current HWM of memory consumption: " +
            std::to_string(measures[VMHWM]) + " KB.\n");
