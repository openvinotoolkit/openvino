...
class MemCheckTestSuite : public ::testing::TestWithParam<TestCase> {
 public:
  ...
  void SetUp() override {
    ...
    test_name = std::string(test_info->name())
                    .substr(0, std::string(test_info->name()).find('/'));
    ...
    EXPECT_GT(test_refs.references[VMSIZE], 0)
        << "Reference value of VmSize is less than 0. Value: "
        << test_refs.references[VMSIZE];
    EXPECT_GT(test_refs.references[VMPEAK], 0)
        << "Reference value of VmPeak is less than 0. Value: "
        << test_refs.references[VMPEAK];
    EXPECT_GT(test_refs.references[VMRSS], 0)
        << "Reference value of VmRSS is less than 0. Value: "
        << test_refs.references[VMRSS];
    EXPECT_GT(test_refs.references[VMHWM], 0)
        << "Reference value of VmHWM is less than 0. Value: "
...
TEST_P(MemCheckTestSuite, create_exenetwork) {
  log_info(
      "Create ExecutableNetwork from network: \""
      << model << "\" with precision: \"" << precision << "\" for device: \""
      << device << "\"");
  ...
  auto test_pipeline = [&] {
    ...
    log_debug(memCheckPipeline.get_reference_record_for_test(
        test_name, model_name, precision, device));
...
TEST_P(MemCheckTestSuite, infer_request_inference) {
  log_info(
      "Inference of InferRequest from network: \""
      << model << "\" with precision: \"" << precision << "\" for device: \""
      << device << "\"");
  auto test_params = GetParam();
  MemCheckPipeline memCheckPipeline;
...
INSTANTIATE_TEST_SUITE_P(
    MemCheckTests,
    MemCheckTestSuite,
    ::testing::ValuesIn(generateTestsParams({"devices", "models"})),
    getTestCaseName);
...
TEST_P(MemCheckTestSuite, inference_with_streams) {
  ...
  auto test_pipeline = [&] {
    ...
    ie_api_wrapper->load_network(device);
    try {
      nireq = ie_api_wrapper->get_property(
    ...
    log_info(
        "Memory consumption after Inference with streams: \""
        << nstreams << "\" with number of infer request: " << nireq);
    memCheckPipeline.record_measures(test_name);
