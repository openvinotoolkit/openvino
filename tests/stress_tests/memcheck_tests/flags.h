...
DEFINE_string(
    test_conf,
    OS_PATH_JOIN({"stress_tests_configs", "memcheck_tests", "test_config.xml"}),
    test_conf_message);

/// @brief message for refs_config argument
static const char refs_conf_message[] =
    "Optional. Path to a references config with values of memory consumption per test.";
...
DEFINE_string(
    refs_conf,
    OS_PATH_JOIN(
        {"stress_tests_configs", "memcheck_tests", "references_config.xml"}),
    refs_conf_message);
