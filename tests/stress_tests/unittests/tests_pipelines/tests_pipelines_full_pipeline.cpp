...
#define setInputParameters()                                               \
  ...
  if (input.second->getInputData()->getTensorDesc().getDims().size() == 4) \
    input.second->setLayout(Layout::NCHW);                                 \
      ...
      input.second->getInputData()->getTensorDesc().getDims().size() == 2) \
...
void test_load_unload_plugin_full_pipeline(
...
void test_read_network_full_pipeline(
    const std::string& model,
    const std::string& target_device,
    const int& n) {
  auto ie_api_wrapper = create_infer_api_wrapper();
...
void test_set_input_params_full_pipeline(
    const std::string& model,
    const std::string& target_device,
    const int& n) {
  auto ie_api_wrapper = create_infer_api_wrapper();
...
void test_create_exenetwork_full_pipeline(
    const std::string& model,
    const std::string& target_device,
    const int& n) {
  auto ie_api_wrapper = create_infer_api_wrapper();
...
void test_create_infer_request_full_pipeline(
    const std::string& model,
    const std::string& target_device,
    const int& n) {
  auto ie_api_wrapper = create_infer_api_wrapper();
...
void test_infer_request_inference_full_pipeline(
    const std::string& model,
    const std::string& target_device,
    const int& n) {
  auto ie_api_wrapper = create_infer_api_wrapper();
