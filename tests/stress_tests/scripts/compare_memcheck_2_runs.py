...
# Database arguments
from memcheck_upload import DATABASE, DB_COLLECTIONS, create_memcheck_records
...
def compare_memcheck_2_runs(cur_values, references, output_file=None):
    ...
    ops = OrderedDict(
        [
            ...
            (
                "cur-ref",
                lambda x, y: y - x if (x is not None and y is not None) else None,
            ),
            (
                "ref/cur",
    ...
    comparison_ops = {
        ...
        metric_name_template.format("vmrss", "ref/cur"): lambda x: (
            gmean(x) > GEOMEAN_THRESHOLD,
            "geomean={} is less than threshold={}".format(gmean(x), GEOMEAN_THRESHOLD),
        ),
        metric_name_template.format("vmhwm", "ref/cur"): lambda x: (
            gmean(x) > GEOMEAN_THRESHOLD,
            "geomean={} is less than threshold={}".format(gmean(x), GEOMEAN_THRESHOLD),
        ),
    }
    ...
    for record in references:
        filtered_rec = {
            key: val for key, val in record.items() if key in required_fields
        }
        filtered_rec_metrics = {
            key: val
    ...
    for device in devices:
        ...
        for field, comp_op in comparison_ops.items():
            ...
            if not status:
                log.error(
                    'Comparison for field="%s" for device="%s" failed: %s',
                    field,
                    device,
                    msg,
                )
...
def cli_parser():
    ...
    parser.add_argument(
        "--db_url", help='MongoDB URL in a for "mongodb://server:port".'
    )
    parser.add_argument(
        "--db_collection",
    ...
    missed_args = []
    if (
        not (os.path.isdir(args.cur_source) and os.path.isdir(args.ref_source))
        and not args.db_url
    ):
    ...
    if missed_args:
        raise argparse.ArgumentError(
            "Arguments {} are required".format(",".join(missed_args))
        )
...
if __name__ == "__main__":
    args = cli_parser()
    references = get_memcheck_records(
        args.ref_source, args.ref_db_collection, DATABASE, args.db_url
    )
    cur_values = get_memcheck_records(
        args.cur_source, args.db_collection, DATABASE, args.db_url
    )
    exit_code = compare_memcheck_2_runs(
        cur_values, references, output_file=args.output_file
    )
    sys.exit(exit_code)
