...
from compare_memcheck_2_runs import (
    compare_memcheck_2_runs,
    get_db_memcheck_records,
    get_memcheck_records,
)

# Database arguments
from memcheck_upload import (
    DATABASE,
    DB_COLLECTIONS,
    create_memcheck_records,
...
def main():
    ...
    init_parser.add_argument(
        "--timeline_report", help=f"create timeline HTML report file name"
    )
    init_parser.add_argument(
        "--upload", action="store_true", help=f"upload results to database"
    )
    init_parser.add_argument(
        "--compare",
        metavar="REFERENCE",
    ...
    parser.add_argument(
        "--timeout", help="timeout for tests run within gtest-parallel", default=""
    )
    parser.add_argument(
        "-d",
        "--output_dir",
    ...
    parser.add_argument(
        "-w", "--workers", help="number of gtest-parallel workers to spawn", default=""
    )
    parser.add_argument(
        "--db_url",
        required=args.timeline_report
        or args.upload
        or (args.compare and not os.path.isdir(args.compare)),
    ...
    parser.add_argument(
        "--manifest", help=f"extract commit information from build manifest"
    )
    parser.add_argument(
        "--metadata", help=f"add extra commit information, json formated"
    )
    parser.add_argument(
        "--strip_log_path",
    ...
    parser.add_argument(
        "--gtest_filter", default="", help="gtest filter passed to gtest"
    )
    ...
    if args.upload or args.timeline_report or args.compare:
        ...
        if args.upload:
            if records:
                ...
                logging.info(
                    "Uploaded to %s/%s.%s", args.db_url, DATABASE, args.db_collection
                )
        ...
        if args.timeline_report:
            try:
                create_memcheck_report(
                    records, args.db_url, args.db_collection, args.timeline_report
                )
                logging.info(
                    "Created memcheck timeline report %s", args.timeline_report
                )
        ...
        if args.compare:
            if os.path.isdir(args.compare):
                ...
                references = get_db_memcheck_records(
                    query=query,
                    db_collection=args.ref_db_collection,
                    db_name=DATABASE,
                    db_url=args.db_url,
                )
            compare_retcode = compare_memcheck_2_runs(
                cur_values=records,
