...
def metadata_from_manifest(manifest):
    ...
    repo_trigger = next(
        repo
        for repo in manifest["components"][PRODUCT_NAME]["repository"]
        if repo["trigger"]
    )
    ...
    return {
        ...
        "target_branch": (
            repo_trigger["target_branch"]
            if repo_trigger["target_branch"]
            else repo_trigger["branch"]
        ),
...
def parse_memcheck_log(log_path):
    ...
    for index, line in enumerate(log_lines):
        if RE_GTEST_REF_MEASURE.search(line):
            heading = [
                name.lower() for name in log_lines[index + 1][len(GTEST_INFO) :].split()
            ]
            values = [
                int(val) for val in log_lines[index + 2][len(GTEST_INFO) :].split()
            ]
    ...
    for index in reversed(range(len(log_lines))):
        if RE_GTEST_CUR_MEASURE.search(log_lines[index]):
            ...
            entry = SimpleNamespace(
                ...
                precision=next(
                    pr for pr in PRECISSIONS if pr.upper() in model["precision"].upper()
                ),
                model=model["path"],
                ...
                status=(
                    "passed"
                    if passed_match
                    else "failed" if failed_match else "started"
                ),
            )
            if ref_metrics:
...
def create_memcheck_records(logs, build_url, artifact_root, append=None):
    ...
    for log in logs:
        ...
        data["_id"] = hashlib.sha256(
            "".join([str(data[key]) for key in KEY_FIELDS]).encode()
        ).hexdigest()
        records += [data]
    return records
...
def modify_data_for_push_to_new_db(records):
    ...
    for record in new_records:
        ...
        if "os_name" in record and "os_version" in record:
            record["os"] = "{} {}.{}".format(
                record["os_name"], record["os_version"][0], record["os_version"][1]
            )
            del record["os_name"]
...
def push_to_db_facade(records, db_api_handler):
    ...
    if uploaded and not errors:
        ...
        logging.info(
            "Failed to upload records by API url {} due to errors {}".format(
                db_api_handler, errors
            )
        )
...
def query_timeline(
    records, db_url, db_collection, max_items=20, similarity=TIMELINE_SIMILARITY
):
    ...
    def timeline_key(item):
        ...
        if len(item["metrics"]["vmhwm"]) <= 1:
            return 1
        order = (
            item["metrics"]["vmhwm"][-1]
            - item["metrics"]["vmhwm"][-2]
            + item["metrics"]["vmrss"][-1]
            - item["metrics"]["vmrss"][-2]
        )
        if not item["status"]:
    ...
    for record in records:
        ...
        timeline["status"] = bool(
            timeline["metrics"]["vmrss"][-1] < timeline["ref_metrics"]["vmrss"][-1]
            and timeline["metrics"]["vmhwm"][-1] < timeline["ref_metrics"]["vmhwm"][-1]
        )
        result += [timeline]
...
def main():
    ...
    parser.add_argument(
        "--dryrun", action="store_true", help="Parse logs, not modify database."
    )
    is_dryrun = parser.parse_known_args()[0].dryrun
    parser.add_argument(
        "build_url", help="A place where memcheck execution logs can be found."
    )
    parser.add_argument(
        "log", nargs="+", help="Local path to log. Extended wildcards supported."
    )
    parser.add_argument(
        "--db_url",
        required=not is_dryrun,
    ...
    logging.basicConfig(
        format="{file}: [ %(levelname)s ] %(message)s".format(
