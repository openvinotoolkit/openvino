...
class VirtualEnv:
    ...
    def create(self):
        ...
        cmd = '"{executable}" -m venv {venv}'.format(
            executable=sys.executable, venv=self.get_venv_dir()
        )
    ...
    def install_requirements(self, *requirements):
        ...
        if not self.is_created:
            self.create()
        cmd = '"{executable}" -m pip install --upgrade pip'.format(
            executable=self.get_venv_executable()
        )
        for req in requirements:
            # Don't install requirements via one `pip install` call to prevent "ERROR: Double requirement given"
            cmd += ' && "{executable}" -m pip install -r {req}'.format(
                executable=self.get_venv_executable(), req=req
            )
        run_in_subprocess(cmd)
...
def main():
    ...
    if args.omz_repo:
        ...
        cmd = (
            ...
            " https://github.com/openvinotoolkit/open_model_zoo {omz_path}".format(
                omz_path=omz_path
            )
        )
        run_in_subprocess(cmd)
    ...
    for model_rec in model_recs:
        ...
        cmd = '"{executable}" "{info_dumper_path}" --name {model_name}'.format(
        ...
        if precision not in model_info["precisions"]:
            log.warning(
                "Please specify precision for the model "
        ...
        model_rec.attrib["path"] = str(
            Path(model_rec.attrib["subdirectory"])
            / precision
            / (model_rec.attrib["name"] + ".xml")
        )
        model_rec.attrib["full_path"] = str(
            args.omz_irs_out_dir
            / model_rec.attrib["subdirectory"]
            / precision
            / (model_rec.attrib["name"] + ".xml")
        )
        ...
        cmd = (
            '"{executable}" {downloader_path} --name {model_name}'
            " --precisions={precision}"
            " --num_attempts {num_attempts}"
            " --output_dir {models_dir}"
            " --cache_dir {cache_dir}".format(
                executable=python_executable,
        ...
        cmd = (
            f'"{python_executable}" {converter_path} --name {model_name}'
    ...
    for model_rec in model_recs:
        if model_rec.attrib.get("full_path") is None:
            log.warning(
                f"Model {model_rec.attrib['name']} does not have 'full_path' attribute! "
    ...
    for ir_src_path in args.omz_models_out_dir.rglob("*.xml"):
        ir_dst_path = args.omz_irs_out_dir / os.path.relpath(
            ir_src_path, args.omz_models_out_dir
        )
