...
infer_tool = str(
    (Path(getsourcefile(lambda: 0)) / ".." / "tools" / "infer_tool.py").resolve()
)
...
def read_session_info(
    path: Path = Path(getsourcefile(lambda: 0)).parent / SESSION_INFO_FILE,
):
...
def make_build(
    ...
    build_dir,
    install_dir,
    build_target: dict = None,
    cmake_additional_args=None,
    log=None,
):
    """Parametrized build and install OpenVINO package."""
    additional_args_line = (
        " ".join(cmake_additional_args) + " " if cmake_additional_args else ""
    )
    build_target_arg_line = (
        [
            f"cmake --build {build_target[target]} --target {target} && "
            for target in build_target.keys()
        ]
        if build_target
        else ""
    )
