...
def run_timetest(args: dict, log=None):
    ...
    for run_iter in range(args["niter"]):
        ...
        retcode, msg = cmd_exec(
            cmd_common + ["-s", str(tmp_stats_path)], log=log, env=ov_env
        )
        ...
        if retcode != 0:
            log.error(
                f"Run of executable '{args['executable']}' failed with return code '{retcode}'. Error: {msg}\n"
        ...
        stats = dict(
            (step_name, stats.get(step_name, []) + [duration])
...
def cli_parser():
    ...
    parser.add_argument("executable", type=Path, help="Binary to execute")
    parser.add_argument(
        "-m",
        required=True,
        dest="model",
    ...
    parser.add_argument(
        "-d", required=True, dest="device", type=str, help="Target device to infer on"
    )
    parser.add_argument(
        "-niter",
        default=10,
    ...
    parser.add_argument(
        "-c", dest="model_cache", action="store_true", help="Enable model cache usage"
    )
    parser.add_argument(
        "-ip",
        default="",
...
if __name__ == "__main__":
    ...
    exit_code, _, aggr_stats, _, _ = run_timetest(
        dict(args._get_kwargs()), log=logging
    ...
    if args.stats_path:
        ...
        logging.info(
            f"Aggregated statistics saved to a file: '{args.stats_path.resolve()}'"
        )
    else:
...
def test_timetest_parser():
    ...
    assert (
        flatten_dict == expected_result
    ), "Statistics parsing is performed incorrectly!"
