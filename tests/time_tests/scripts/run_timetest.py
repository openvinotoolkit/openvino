...
def aggregate_stats(stats: dict, args:dict):
    ...
    if args["model_cache"]: # FIL-First Inference Latency
        agg_stats['time_to_first_inference_result']={'avg': str((agg_stats['time_to_inference']['avg']+agg_stats['first_inference']['avg']) / 10 **3)+'ms'}
    else: #FEIL-First Ever Inference Latency
        agg_stats['time_to_first_inference_result']={'avg': str((agg_stats['time_to_inference']['avg']+agg_stats['first_inference']['avg']) / 10 **6)+'s'}
