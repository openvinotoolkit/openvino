...
void setPerformanceConfig(ov::Core ie, const std::string& device) {
  auto supported_config_keys =
      ie.get_property(device, ov::supported_properties);

  if (std::find(
          supported_config_keys.begin(),
          supported_config_keys.end(),
          ov::hint::performance_mode) == supported_config_keys.end()) {
    std::cerr << "Device " << device << " doesn't support "
              << ov::hint::performance_mode.name() << " property!\n"
              << "Performance config was not set.";
  } else
    ie.set_property(
        device, ov::hint::performance_mode(ov::hint::PerformanceMode::LATENCY));
}
} // namespace TimeTest
