...
int runPipeline(
    ...
    const bool isCacheEnabled,
    const std::string& inputPrecision,
        ...
        }
        {
          ...
          if (!isCacheEnabled) {
            if (TimeTest::fileExt(model) == "blob") {
              ...
              if (ip || op) {
                auto preprocessor =
                    ov::preprocess::PrePostProcessor(cnnNetwork);
                if (ip) {
                  const auto inputs = cnnNetwork->inputs();
                  for (size_t i = 0; i < inputs.size(); i++) {
                    preprocessor.input(i).tensor().set_element_type(
                ...
                if (op) {
                  ...
                  for (size_t i = 0; i < outputs.size(); i++) {
                    preprocessor.output(i).tensor().set_element_type(
