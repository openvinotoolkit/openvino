...
def get_cmd_output(*cmd):
    try:
        ...
        if isinstance(error, subprocess.CalledProcessError):
            ...
            print(
                f"'{' '.join(map(str, cmd))}' timed out after {error.timeout} seconds. Output:"
            )
        print(error.output)
        raise
    return output
                ...
                if not file_path.exists():
                    if test_data_dir / "bvlcalexnet-12.onnx" == file_path:
                        response = requests.get(
                            "https://github.com/onnx/models/raw/main/validated/vision/classification/alexnet/model/bvlcalexnet-12.onnx?download="
                        )
                        with file_path.open("wb") as nfnet:
                            nfnet.write(response.content)
                    elif test_data_dir / "efficientnet-lite4-11-qdq.onnx" == file_path:
                        response = requests.get(
                            "https://github.com/onnx/models/raw/main/validated/vision/classification/efficientnet-lite4/model/efficientnet-lite4-11-qdq.onnx?download="
                        )
                        with file_path.open("wb") as nfnet:
                            nfnet.write(response.content)
                    else:
                        response = requests.get(
                            "https://storage.openvinotoolkit.org/repositories/openvino/ci_dependencies/test/2021.4/samples_smoke_tests_data_2021.4.zip"
                        )
                        with zipfile.ZipFile(io.BytesIO(response.content)) as zfile:
                            zfile.extractall(test_data_dir)
                        cv2.imwrite(
                            str(test_data_dir / "dog-224x224.bmp"),
                            cv2.resize(
                                cv2.imread(
                                    str(
                                        test_data_dir
                                        / "samples_smoke_tests_data_2021.4/validation_set/227x227/dog.bmp"
                                    )
                                ),
                                (224, 224),
                            ),
    ...
    def join_env_path(param, cache, executable_path, complete_path=True):
        ...
        for k in param.keys():
            if ("i" == k) and complete_path:
                param["i"] = [
                    str(download(test_data_dir, test_data_dir / e)) for e in param["i"]
                ]
                param["i"] = " ".join(map(str, param["i"]))
            elif "m" == k and not param["m"].endswith(
                '/samples/cpp/model_creation_sample/lenet.bin"'
            ):
    ...
    def _test(
        ...
        if "python" in sample_type.lower():
            executable_path = self.made_executable_path(
                os.environ["IE_APP_PYTHON_PATH"],
                self.sample_name,
                sample_type=sample_type,
            )
        else:
            executable_path = self.made_executable_path(
                os.environ["IE_APP_PATH"], self.sample_name, sample_type=sample_type
            )
        ...
        cmd_line = get_cmd_func(
            param_cp, use_preffix=use_preffix, long_hyphen=long_hyphen
        )
