...
import pytest
from common.samples_common_test_class import get_cmd_output, get_devices, prepend
...
def get_executable(sample_language):
    ...
    if sample_language == "C++":
        executable = pathlib.Path(
            os.environ["IE_APP_PATH"], "benchmark_app"
    ...
    if tmp_path:
        ...
        if "CPU" == device:
            ...
            assert (
                not pin
                or pin == "YES"
                and config_json["CPU"]["AFFINITY"] == "CORE"
                or pin == "NO"
                and config_json["CPU"]["AFFINITY"] == "NONE"
...
def test_pin(sample_language, pin, cache, tmp_path):
    verify(
        sample_language, "CPU", pin=pin, nstreams="2", cache=cache, tmp_path=tmp_path
    )


@pytest.mark.parametrize("sample_language", ["C++", "Python"])
@pytest.mark.parametrize(
    "device", sorted({"CPU", "GPU"} & set(get_devices()))
)  # Determenisitic order is required for --numprocesses
...
def test_dynamic_shape(sample_language, device, cache):
    verify(
        sample_language,
        device,
        model="efficientnet-lite4-11-qdq.onnx",
        shape="[?,224,224,3]",
        data_shape="[1,224,224,3][2,224,224,3]",
        layout="[NHWC]",
        cache=cache,
    )
