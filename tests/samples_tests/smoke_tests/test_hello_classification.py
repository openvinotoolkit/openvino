...
from common.samples_common_test_class import (
...
def get_executable(sample_language):
    return pathlib.Path(
        os.environ["IE_APP_PATH"],
        "hello_classification" + ("" if sample_language == "C++" else "_c"),
    ).with_suffix(".exe" if os.name == "nt" else "")


def prepend(cache, model, inp):
    test_data_dir = cache.mkdir("test_data")
    return download(test_data_dir, test_data_dir / model), download(
        test_data_dir, test_data_dir / inp
    )
...
class Test_hello_classification(SamplesCommonTestClass):
    ...
    def test_hello_classification_fp32(self, param, cache):
        ...
        output = get_cmd_output(
            get_executable(param["sample_type"]),
            *prepend(cache, param["m"], param["i"]),
            param["d"],
        )
        ...
        for line in range(len(output)):
            if (
                re.match(
                    "\\d+ +\\d+.\\d+$", output[line].replace("[ INFO ]", "").strip()
                )
                is not None
            ):
    ...
    def test_hello_classification_check_unicode_path_support(
        self, param, cache, tmp_path
    ):
        """
        ...
        executable_path = self.made_executable_path(
            os.environ.get("IE_APP_PATH"),
            self.sample_name,
        ...
        for image_name in [encoded_words[-1]]:
            for model_name in [encoded_words[-1]]:
                ...
                stdout = get_cmd_output(
                    executable_path, new_model_path, image_path, param["d"]
                )
                probs = []
