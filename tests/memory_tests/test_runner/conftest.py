...
from utils import DATABASES, DB_COLLECTIONS, metadata_from_manifest, upload_data

MEMORY_TESTS_DIR = os.path.dirname(os.path.dirname(__file__))
sys.path.append(MEMORY_TESTS_DIR)

from test_runner.utils import REFS_FACTOR, query_memory_timeline
    ...
    if omz_path:
        ...
        if instance["instance"]["model"]["source"] == "omz":
            ...
            if model_precision not in model_info["precisions"]:
                logging.error(
                    f"Please specify precision for the model "
                    f"{model_name} from the list: {model_info['precisions']}"
                )

            sub_model_path = str(
                Path(model_info["subdirectory"])
                / model_precision
                / (model_name + ".xml")
            )
...
def prepare_db_info(request, instance, executable, niter, manifest_metadata):
    ...
    info = {
        ...
        **instance[
            "orig_instance"
        ],  # TODO: think about use `instance` instead of `orig_instance`
        "run_id": run_id,
        ...
        "results": {},
        "raw_results": {},
        "references": instance["instance"].get(
            "references", {}
        ),  # upload actual references that were used
    ...
    if report_path:
        ...
        env = jinja2.Environment(
            loader=jinja2.FileSystemLoader(
                searchpath=os.path.join(abs_path("."), "memory_template")
            ),
            autoescape=False,
        )
...
def prepare_tconf_with_refs(pytestconfig):
    ...
    if new_tconf_path:
        logging.info(
            f"Save new test config with test results as references to {new_tconf_path}"
        )
...
def pytest_runtest_makereport(item, call):
    ...
    if call.when in ["setup", "call"]:
        ...
        logging.info(
            f"Upload data to {db_url}/{db_name}.{db_collection}. "
            f"Data: {instance['db']}"
        )
