...
def query_memory_timeline(
    records,
    db_url,
    db_name,
    db_collection,
    max_items=20,
    similarity=TIMELINE_SIMILARITY,
):
    ...
    def timeline_key(item):
        ...
        for step_name, _ in item["results"].items():
            if len(item["results"][step_name]["vmhwm"]) <= 1:
                return 1
            order = (
                item["results"][step_name]["vmhwm"]["avg"][-1]
                - item["results"][step_name]["vmhwm"]["avg"][-2]
                + item["results"][step_name]["vmrss"]["avg"][-1]
                - item["results"][step_name]["vmrss"]["avg"][-2]
            )
            if not item["status"]:
    ...
    for record in records:
        ...
        try:
            ...
            pipeline = [
                {"$match": query},
                {
                    "$addFields": {
                        "commit_date": {
                            "$dateFromString": {"dateString": "$commit_date"}
                        }
                    }
                },
                {"$sort": {"commit_date": -1}},
        ...
        items += [record]
        for item in items:
            item["status"] = {"passed": True, "failed": False, "not_finished": False}[
                item["status"]
            ]
        ...
        for vm_metric, stat_metrics in vm_records.items():
            ...
            for stat_metric_name, reference_val in stat_metrics.items():
                if stat_metric_name not in stat_metrics_to_compare:
                    continue
                if (
                    aggr_stats[step_name][vm_metric][stat_metric_name]
                    > reference_val * REFS_FACTOR
                ):
                    logging.error(
                        f"Comparison failed for '{step_name}' step for '{vm_metric}' for"
                    ...
                    logging.info(
                        f"Comparison passed for '{step_name}' step for '{vm_metric}' for"
                        f" '{stat_metric_name}' metric. Reference: {reference_val}."
                        f" Current values: {aggr_stats[step_name][vm_metric][stat_metric_name]}"
                    )
    return status
