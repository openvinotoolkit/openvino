    ...
    return {
        step_name: {
            vm_metric: {
                ...
                "stdev": (
                    statistics.stdev(vm_values_list) if len(vm_values_list) > 1 else 0
                ),
            }
...
def run_memorytest(args: dict, log=None):
    ...
    for run_iter in range(args["niter"]):
        ...
        if retcode != 0:
            log.error(
                "Run of executable '{}' failed with return code '{}'. Error: {}\n"
                "Statistics aggregation is skipped.".format(
                    args["executable"], retcode, msg
                )
            )
            return retcode, msg, {}, {}
        ...
        log.debug(
            "Statistics after run of executable #{}: {}".format(run_iter, parsed_data)
        )
...
if __name__ == "__main__":
    ...
    exit_code, _, aggr_stats, _ = run_memorytest(
    ...
    if args.stats_path:
        ...
        logging.info(
            "Aggregated statistics saved to a file: '{}'".format(
