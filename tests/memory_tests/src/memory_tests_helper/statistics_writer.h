...
class StatisticsWriter {
 private:
  ...
  std::map<std::string, std::vector<size_t>>
      mem_structure; // mem_counter_name, memory measurements
 ...
 public:
   ...
   */
  void addMemCounterToStructure(
      const std::pair<std::string, std::vector<size_t>>& record) {
  ...
  void write() {
    ...
    for (auto& mem_counter : mem_struct_order) {
      statistics_file << mem_counter << ":" << '\n'
                      ...
                      << '\n'
                      << SEPARATOR
                      << "vmsize: " << mem_structure[mem_counter][2] << '\n'
                      << SEPARATOR
                      << "vmpeak: " << mem_structure[mem_counter][3] << '\n'
                      << SEPARATOR
                      << "threads: " << mem_structure[mem_counter][4] << '\n';
    }
    statistics_file << "---" << '\n' << "measurement_unit: Kb";
  }
};
