...
def get_element_wise_params(ie_device=None, precision=None, operation=None):
    """
    :param ie_device: list of devices
    :param precision: list of precisions
    :param operation: list of operation values
    """
    if ie_device:
...
def pytest_generate_tests(metafunc):
    generate_tests(metafunc, get_element_wise_params)


class TestEltwise(object):
    @pytest.mark.precommit
    def test_eltwise_power_precommit(self, ie_device, precision, operation):
        self.eltwise_power(ie_device, precision, operation)

    @pytest.mark.nightly
    def test_eltwise_power_nightly(self, ie_device, precision, operation):
        self.eltwise_power(ie_device, precision, operation)

    def eltwise_power(self, ie_device, precision, operation):
        ...
        elt = network.add_layer(layer_type='Eltwise',
                                inputs=[output, relu],
                                operation=operation,
                                get_out_shape_def=calc_same_out_shape,
                                framework_representation_def=eltwise_to_proto)
        ...
        ie_results = score_model(model_path=os.path.join(ir_path, network.name + '.xml'), device=ie_device,
                                 image_path=img_path, out_blob_name=elt.name)

        assert compare_infer_results_with_caffe(ie_results, elt.name), "Comparing with Caffe failed."
