...
class TestSoftMax(object):
    ...
    def test_soft_max_precommit(self, ie_device, precision, axis):
        self.soft_max(ie_device, precision, axis)

    @pytest.mark.nightly
    def test_soft_max_nightly(self, ie_device, precision, axis):
        self.soft_max(ie_device, precision, axis)

    def soft_max(self, ie_device, precision, axis):
        ...
        softmax = network.add_layer(layer_type='SoftMax',
                                    ...
                                    framework_representation_def=soft_max_to_proto)
        ...
        assert compare_infer_results_with_caffe(ie_results, softmax.name), "Comparing with Caffe failed."
