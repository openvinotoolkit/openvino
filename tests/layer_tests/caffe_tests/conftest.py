...
def pytest_generate_tests(metafunc):
    ir_version = int(metafunc.config.getoption("ir_version"))
    if ir_version < 10:
        pytest.skip("Caffe tests don't support 'ir_version' less than 10")


@pytest.fixture(scope="class", autouse=True)
def setup_teardown_test_class():
    init_logger('DEBUG')
    if not os.path.exists(constants.caffe_models_path):
        os.makedirs(constants.caffe_models_path)
    if not os.path.exists(constants.ir_path):
        os.makedirs(constants.ir_path)

    yield

    shutil.rmtree(constants.caffe_models_path, True)
