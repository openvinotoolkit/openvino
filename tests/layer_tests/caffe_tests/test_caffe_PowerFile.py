...
class TestPowerFile(object):
    @pytest.mark.precommit
    def test_power_file_precommit(self, ie_device, precision):
        self.power_file(ie_device, precision)

    @pytest.mark.nightly
    def test_power_file_nightly(self, ie_device, precision):
        self.power_file(ie_device, precision)

    def power_file(self, ie_device, precision):
        network = Net(precision=precision)
        output = network.add_layer(layer_type='Input',
                                   get_out_shape_def=calc_out_shape_input_layer,
                                   framework_representation_def=input_to_proto)
        pf = network.add_layer(layer_type='PowerFile',
                               inputs=[output],
                               get_out_shape_def=calc_same_out_shape,
                               framework_representation_def=power_file_to_proto)
        ...
        ie_results = score_model(model_path=os.path.join(ir_path, network.name + '.xml'), device=ie_device,
                                 image_path=img_path, out_blob_name=pf.name)

        assert compare_infer_results_with_caffe(ie_results, pf.name), "Comparing with Caffe failed."
