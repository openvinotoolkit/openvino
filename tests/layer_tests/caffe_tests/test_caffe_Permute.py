...
def get_permute_params(ie_device=None, precision=None, order=None):
    """
    ...
    for element in itertools.product(ie_device_params, precision_params, order_params):
        ...
        if element[0] == 'MYRIAD':
            if element[1] == 'FP32':
                continue
            if element[2][0] == 0 or element[2][1] == 0:
                continue
...
def pytest_generate_tests(metafunc):
    generate_tests(metafunc, get_permute_params)
...
class TestPermute(object):
    ...
    def permute(self, ie_device, precision, order):
        ...
        permute = network.add_layer(layer_type='Permute',
                                    inputs=[output],
                                    order=order,
                                    get_out_shape_def=caffe_calc_out_shape_permute_layer,
                                    framework_representation_def=permute_to_proto)
        ...
        ie_results = score_model(model_path=os.path.join(ir_path, network.name + '.xml'), device=ie_device,
                                 image_path=img_path, out_blob_name=permute.name)

        assert compare_infer_results_with_caffe(ie_results, permute.name), "Comparing with Caffe failed."
