...
class TestGatherNd(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        inputs_data = {}
        inputs_data["params:0"] = np.random.randint(-50, 50, params_shape).astype(
            self.params_type
        )
        # generate indices for each slice and concatenate
        indices_slices = []
        for idx in range(self.index_length):
            indices_slice = np.random.randint(
                0, self.max_indices[idx], indices_shape[:-1] + [1]
            ).astype(self.indices_type)
            indices_slices.append(indices_slice)
        indices_slice_rank = len(indices_slices[0].shape)
        inputs_data["indices:0"] = np.concatenate(
            indices_slices, axis=indices_slice_rank - 1
        )
        return inputs_data

    def create_gather_nd_net(
        self, params_shape, params_type, indices_shape, indices_type
    ):
        self.params_type = params_type
        self.indices_type = indices_type
        assert (
            len(indices_shape) > 0
        ), "Incorrect test case: shape of `indices` must be at least rank 1"
        self.index_length = indices_shape[-1]
        assert self.index_length > 0, "The length of indices must be at least 1"
        assert (
            len(params_shape) >= self.index_length
    ...
    test_data_precommit = [
        ...
        dict(
            params_shape=[2, 4, 5, 3],
            params_type=np.int32,
            indices_shape=[1, 4, 3],
