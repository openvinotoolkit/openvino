...
class TestTensorListLength(CommonTFLayerTest):
    ...
    def create_tensor_list_length(self, input_shape, input_type):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            tensor_list = tf.raw_ops.TensorListFromTensor(
                tensor=x, element_shape=tf.constant(input_shape[1:], dtype=tf.int32)
            )
...
class TestTensorListLengthEmptyList(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        inputs_data = {}
        inputs_data["tensor_list_size:0"] = np.array(
            [self.tensor_list_size], dtype=np.int32
        )
        return inputs_data
    ...
    def create_tensor_list_length_empty_list(self, tensor_list_size, element_shape):
        ...
        # Create the graph and model
        with tf.compat.v1.Session() as sess:
            tensor_list_size = tf.compat.v1.placeholder(
                tf.int32, [1], "tensor_list_size"
            )
            ...
            tensor_list = tf.raw_ops.TensorListFromTensor(
                tensor=tensor, element_shape=tf_element_shape
            )
            tf.raw_ops.TensorListLength(input_handle=tensor_list)
            tf.compat.v1.global_variables_initializer()
            tf_net = sess.graph_def
    ...
    @pytest.mark.nightly
    @pytest.mark.skipif(platform == "darwin", reason="Ticket - 122182")
    def test_tensor_list_length_empty_list(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_tensor_list_length_empty_list(**params),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
