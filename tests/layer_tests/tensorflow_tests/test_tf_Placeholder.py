...
class TestPlaceholder(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        if self.input_type == str or self.input_type == np.str_:
            ...
            inputs_data[tensor_name] = rng.uniform(-5.0, 5.0, x_shape).astype(
                self.input_type
            )
        elif np.issubdtype(self.input_type, np.signedinteger):
            inputs_data[tensor_name] = rng.integers(-8, 8, x_shape).astype(
                self.input_type
            )
        else:
            inputs_data[tensor_name] = rng.integers(0, 8, x_shape).astype(
                self.input_type
            )
        return inputs_data
    ...
    def test_placeholder(
        ...
        self._test(
            *self.create_placeholder_net(input_shape, input_type),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
