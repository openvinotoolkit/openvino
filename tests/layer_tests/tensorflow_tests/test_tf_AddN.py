...
class TestAddN(CommonTFLayerTest):
    ...
    def create_addn_placeholder_const_net(
        self, input_shapes, ir_version, use_legacy_frontend
    ):
        """
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for idx, input_shape in enumerate(input_shapes):
                tf_inputs.append(
                    tf.compat.v1.placeholder(tf.float32, input_shape, f"Input_{idx}")
                )
    ...
    test_data = [
        ...
        dict(
            input_shapes=[[3, 4, 5], [3, 4, 5], [3, 4, 5]]
        ),  # Tests sum of shapes which may trigger nchw/nhcw transformation
    ]

    @pytest.mark.parametrize("params", test_data)
    @pytest.mark.nightly
    def test_addn_placeholder_const(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_addn_placeholder_const_net(
...
class TestComplexAddN(CommonTFLayerTest):
    ...
    def create_complex_addn_net(self, input_shapes):
        ...
        with tf.compat.v1.Session() as sess:
            complex_tensors = []
            for idx, input_shape in enumerate(input_shapes):
                real = tf.compat.v1.placeholder(
                    np.float32, input_shape, f"param_real_{idx+1}"
                )
                imag = tf.compat.v1.placeholder(
                    np.float32, input_shape, f"param_imag_{idx+1}"
                )
    ...
    @pytest.mark.nightly
    def test_complex_addn(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_complex_addn_net(**params),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
