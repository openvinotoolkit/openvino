...
class TestBiasAdd(CommonTFLayerTest):
    def create_bias_add_placeholder_const_net(
        self, shape, ir_version, use_legacy_frontend, output_type=tf.float32
    ):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            constant_value = np.random.randint(0, 1, tf_y_shape).astype(
    ...
    def create_bias_add_2_consts_net(
        self, shape, ir_version, use_legacy_frontend, output_type=tf.float32
    ):
        tf.compat.v1.reset_default_graph()
        tf_concat_axis = -1
        ...
        with tf.compat.v1.Session() as sess:
            ...
            constant_value_x = np.random.randint(-256, 256, tf_x_shape).astype(
                output_type.as_numpy_dtype()
            )
            x = tf.constant(constant_value_x)
            constant_value_y = np.random.randint(-256, 256, tf_y_shape).astype(
                output_type.as_numpy_dtype()
            )
            ...
            placeholder = tf.compat.v1.placeholder(
                output_type, tf_x_shape, "Input"
            )  # Input_1 in graph_def

            concat = tf.concat(
                [placeholder, add], axis=tf_concat_axis, name="Operation"
            )
    ...
    test_data_2D = [dict(shape=[1, 1]), dict(shape=[1, 224])]

    @pytest.mark.parametrize("params", test_data_2D)
    @pytest.mark.nightly
    def test_bias_add_placeholder_const_2D(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_bias_add_placeholder_const_net(
    ...
    @pytest.mark.parametrize("params", test_data_2D)
    @pytest.mark.nightly
    def test_bias_add_2_consts_2D(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_bias_add_2_consts_net(
                **params, ir_version=ir_version, use_legacy_frontend=use_legacy_frontend
            ),
            ie_device,
            precision,
    ...
    test_data_3D = [
        pytest.param(
            dict(shape=[1, 1, 224]), marks=pytest.mark.xfail(reason="*-19053")
        ),
        pytest.param(
            dict(shape=[1, 3, 224]), marks=pytest.mark.xfail(reason="*-19053")
        ),
    ]

    @pytest.mark.parametrize("params", test_data_3D)
    @pytest.mark.nightly
    def test_bias_add_placeholder_const_3D(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_bias_add_placeholder_const_net(
    ...
    test_data_4D = [
        ...
        pytest.param(
            dict(shape=[1, 3, 100, 224], output_type=tf.float16),
    ...
    def test_bias_add_placeholder_const_4D(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_bias_add_placeholder_const_net(
    ...
    test_data_5D = [dict(shape=[1, 1, 50, 100, 224]), dict(shape=[1, 3, 220, 222, 224])]

    @pytest.mark.parametrize("params", test_data_5D)
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_bias_add_placeholder_const_5D(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_bias_add_placeholder_const_net(
...
class TestComplexBiasAdd(CommonTFLayerTest):
    ...
    def create_complex_bias_add_net(
        self,
        input_shape,
        bias_shape,
        data_format,
    ...
    @pytest.mark.precommit
    @pytest.mark.nightly
    def test_complex_bias_add(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_complex_bias_add_net(
                **params, ir_version=ir_version, use_legacy_frontend=use_legacy_frontend
            ),
            ie_device,
            precision,
