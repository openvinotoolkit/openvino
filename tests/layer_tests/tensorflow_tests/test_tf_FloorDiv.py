...
class TestFloorDiv(CommonTFLayerTest):
    def create_add_placeholder_const_net(
        self, x_shape, dtype, ir_version, use_legacy_frontend
    ):
    ...
    def test_add_placeholder_const_1D(
        ...
        if (
            ...
            and platform.machine() in list_arm_platforms()
            and np.issubdtype(params["dtype"], np.signedinteger)
        ):
            pytest.xfail(
                reason="Ticket CVS-132377 - Divide inconsistent behavior on different systems"
            )
        elif (
            platform.system() == "Darwin" and platform.machine() in list_arm_platforms()
        ):
...
class TestFloorDivStaticInput(CommonTFLayerTest):
    ...
    def create_flordiv_tf_net(
        self, min, max, step, y, dtype, ir_version, use_legacy_frontend
    ):
        import tensorflow as tf
    ...
    def _prepare_input(self, inputs_dict):
        for input in inputs_dict.keys():
            inputs_dict[input] = np.arange(
                self.min, self.max, self.step, dtype=self.dtype
            )
        return inputs_dict
