...
class TestBroadcastTo(CommonTFLayerTest):
    ...
    def create_broadcastto_placeholder_const_net(
        self, input_shape, broadcast_shape, ir_version, use_legacy_frontend
    ):
        """
    ...
    @pytest.mark.nightly
    def test_broadcastto_placeholder_const(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_broadcastto_placeholder_const_net(
