...
class TestAssignOps(CommonTFLayerTest):
    ...
    def test_assign(
        ...
        self._test(
            *self.create_assign_net(const_shape, input_type),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
    ...
    @pytest.mark.parametrize("const_shape", [[], [2], [3, 4], [3, 2, 1, 4]])
    @pytest.mark.parametrize(
        "assign_op", ["tf.raw_ops.AssignAdd", "tf.raw_ops.AssignSub"]
    )
    @pytest.mark.precommit
    @pytest.mark.nightly
    def test_assign_ops(
        ...
        self._test(
            *self.create_assign_op_net(const_shape, OPS[assign_op]),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
    ...
    def test_assign_ops2(
        ...
        self._test(
            *self.create_assign_op_net2(const_shape, OPS[assign_op]),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
