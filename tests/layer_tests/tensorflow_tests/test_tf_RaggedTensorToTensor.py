...
class TestRaggedTensorToTensor(CommonTFLayerTest):
    ...
    def create_ragged_tensor_to_tensor_net(self, shape, values_shape, values_type, default_value,
    ...
    @pytest.mark.parametrize('shape', [[4, 8], [-1, 64]])
    ...
    def test_ragged_tensor_to_tensor(self, shape, values_shape, values_type, default_value,
                                     row_partition_tensors, row_partition_types,
                                     ie_device, precision, ir_version, temp_dir, use_legacy_frontend):
        self._test(*self.create_ragged_tensor_to_tensor_net(shape=shape, values_shape=values_shape,
                                                            values_type=values_type, default_value=default_value,
                                                            row_partition_tensors=row_partition_tensors,
                                                            row_partition_types=row_partition_types),
