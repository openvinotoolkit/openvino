...
class TestRaggedTensorToTensor(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        assert (
            "values:0" in inputs_info
        ), "Test error: inputs_info must contain `values`"
        values_shape = inputs_info["values:0"]
        inputs_data = {}
        if np.issubdtype(self.values_type, np.floating):
            inputs_data["values:0"] = rng.uniform(-5.0, 5.0, values_shape).astype(
                self.values_type
            )
        elif np.issubdtype(self.values_type, np.signedinteger):
            inputs_data["values:0"] = rng.integers(-8, 8, values_shape).astype(
                self.values_type
            )
        else:
            inputs_data["values:0"] = rng.integers(0, 8, values_shape).astype(
                self.values_type
            )
        return inputs_data

    def create_ragged_tensor_to_tensor_net(
        self,
        shape_type,
        shape_value,
        values_shape,
    ...
    def test_ragged_tensor_to_tensor(
        self,
        shape_type,
        shape_value,
        values_shape,
        ...
        self._test(
            *self.create_ragged_tensor_to_tensor_net(
...
class TestRaggedTensorToTensorRowIds(CommonTFLayerTest):
    ...
    @pytest.mark.parametrize(
        "row_partition_tensors", [[20, [1, 2, 3, 3, 4, 8, 8, 9, 9, 9]]]
    )
    @pytest.mark.parametrize(
