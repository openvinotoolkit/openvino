...
class TestOnesLike(CommonTFLayerTest):
    ...
    def test_ones_like(
        ...
        self._test(
            *self.create_ones_like_net(**params),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
...
class TestComplexOnesLike(CommonTFLayerTest):
    ...
    def create_complex_ones_like_net(self, x_shape, x_type):
        ...
        with tf.compat.v1.Session() as sess:
            x_real = tf.compat.v1.placeholder(
                tf.dtypes.as_dtype(x_type), x_shape, "x_real"
            )
            x_imag = tf.compat.v1.placeholder(
                tf.dtypes.as_dtype(x_type), x_shape, "x_imag"
            )
    ...
    @pytest.mark.precommit
    @pytest.mark.nightly
    def test_complex_ones_like(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_complex_ones_like_net(**params),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
