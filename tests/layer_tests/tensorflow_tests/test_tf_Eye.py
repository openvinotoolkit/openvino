...
class TestTFEye(CommonTFLayerTest):
    ...
    def _prepare_input(self, inputs_dict):
        for input in inputs_dict.keys():
            inputs_dict[input] = np.zeros(inputs_dict[input]).astype(
                self.eye_output_type_param
            )
        return inputs_dict
    ...
    def create_tf_eye_net(self, num_rows, num_columns, batch_shape, output_type):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            input_zero = tf.compat.v1.placeholder(
                tf.as_dtype(self.eye_output_type_param), [1], "Input"
            )
    ...
    test_data = [
        dict(num_rows=5, num_columns=None, batch_shape=None, output_type=None),
