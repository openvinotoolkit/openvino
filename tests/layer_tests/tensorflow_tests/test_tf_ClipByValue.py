...
class TestClipByValue(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        inputs_data["clip_value_min:0"] = np.random.randint(
            -10, 10, clip_value_min_shape
        ).astype(np.float32)
        inputs_data["clip_value_max:0"] = np.random.randint(
            -10, 10, clip_value_max_shape
        ).astype(np.float32)
        return inputs_data
    ...
    def create_clip_by_value_net(self, t_shape):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            tf.raw_ops.ClipByValue(
                t=t, clip_value_min=clip_value_min, clip_value_max=clip_value_max
            )
