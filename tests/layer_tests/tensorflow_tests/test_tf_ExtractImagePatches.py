...
class TestExtractImagePatches(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        assert (
            "images:0" in inputs_info
        ), "Test error: inputs_info must contain `images`"
        images_shape = inputs_info["images:0"]
        inputs_data = {}
        inputs_data["images:0"] = np.random.randint(-10, 10, images_shape).astype(
            np.float32
        )
        return inputs_data

    def create_extract_image_patches_net(
        self, images_shape, ksizes, strides, rates, padding
    ):
    ...
    test_basic = [
        ...
        dict(
            images_shape=[2, 110, 50, 4],
            ksizes=[1, 20, 30, 1],
            strides=[1, 5, 5, 1],
            rates=[1, 1, 1, 1],
        ),
        dict(
            images_shape=[3, 30, 40, 3],
            ksizes=[1, 5, 10, 1],
            strides=[1, 3, 1, 1],
            rates=[1, 4, 3, 1],
        ),
    ...
    def test_extract_image_patches_basic(
        self,
        params,
        padding,
        ie_device,
        ...
        self._test(
            *self.create_extract_image_patches_net(**params, padding=padding),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
