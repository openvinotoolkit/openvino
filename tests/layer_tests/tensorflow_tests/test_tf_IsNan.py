...
class TestIsNan(CommonTFLayerTest):
    ...
    def create_is_nan_net(self, x_shape, x_type):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            tf.raw_ops.IsNan(x=x, name="is_nan")
    ...
    test_data_basic = [
        dict(x_shape=[4], x_type=tf.float32),
        dict(x_shape=[2, 5], x_type=tf.float32),
        dict(x_shape=[3, 2, 4], x_type=tf.float32),
    ]
