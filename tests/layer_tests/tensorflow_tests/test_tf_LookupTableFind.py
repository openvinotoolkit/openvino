...
class TestLookupTableFindOps(CommonTFLayerTest):
    ...
    def create_lookup_table_find_net(
        self,
        hash_table_type,
        keys_shape,
        keys_type,
        ...
        hash_table_op = (
            tf.raw_ops.HashTable if hash_table_type == 0 else tf.raw_ops.HashTableV2
        )
        import_table_op = (
            tf.raw_ops.LookupTableImport
            if hash_table_type == 0
            else tf.raw_ops.LookupTableImportV2
        )
        lookup_table_op = (
            tf.raw_ops.LookupTableFind
            if hash_table_type == 0
            else tf.raw_ops.LookupTableFindV2
        )
        ...
        with tf.compat.v1.Session() as sess:
            ...
            import_hash_table = import_table_op(
                table_handle=hash_table, keys=all_keys, values=all_values
            )
            with tf.control_dependencies([import_hash_table]):
                lookup_table_op(
                    table_handle=hash_table,
                    keys=keys,
    ...
    test_data = [
        dict(
            keys_type=np.int32,
            values_type=np.float32,
            all_keys=[0, 1, 2, 3, 4, 5],
        ...
        dict(
            keys_type=np.int32,
            values_type=tf.string,
            all_keys=[20, 10, 33, -22, 44, 11],
            all_values=[
                "PyTorch",
                "TensorFlow",
                "JAX",
    ...
    def test_lookup_table_find(
        ...
        if ie_device == "GPU" or run_in_jenkins():
            pytest.skip(
                "operation extesion is not supported on GPU or "
                "No layout format available for gather:LookupTableFind issue"
            )
        if params["keys_type"] == str:
            if platform.system() in ("Darwin") or platform.machine() in [
                "arm",
                "armv7l",
                "aarch64",
                "arm64",
        ...
        self._test(
            *self.create_lookup_table_find_net(
                hash_table_type=hash_table_type, keys_shape=keys_shape, **params
            ),
            ie_device,
            precision,
            ir_version,
