...
class TestDynamicPartition(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        assert (
            "partitions:0" in inputs_info
        ), "Test error: inputs_info must contain `partitions`"
    ...
    def create_dynamic_partition_net(
        self, data_shape, partitions_shape, num_partitions, data_type
    ):
        tf.compat.v1.reset_default_graph()
        # Create the graph and model
        with tf.compat.v1.Session() as sess:
            data = tf.compat.v1.placeholder(data_type, data_shape, "data")
            partitions = tf.compat.v1.placeholder(
                tf.int32, partitions_shape, "partitions"
            )
            dynamic_partition = tf.raw_ops.DynamicPartition(
                data=data, partitions=partitions, num_partitions=num_partitions
            )
            for ind in range(num_partitions):
                tf.identity(
                    dynamic_partition[ind], name="dynamic_partition_{}".format(ind)
                )
    ...
    @pytest.mark.nightly
    def test_dynamic_partition_basic(
        ...
        if not use_legacy_frontend:
            pytest.skip(
                "DynamicPartition operation is not supported via legacy frontend."
            )
        self._test(
            *self.create_dynamic_partition_net(**params),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
    ...
    @pytest.mark.nightly
    def test_dynamic_partition_other_types(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        if ie_device == "GPU":
            pytest.xfail("104855")
