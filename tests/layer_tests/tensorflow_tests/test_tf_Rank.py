...
class TestComplexRank(CommonTFLayerTest):
    ...
    def create_rank_net(self, input_shape):
        ...
        # Create the graph and model
        with tf.compat.v1.Session() as sess:
            input_real = tf.compat.v1.placeholder(np.float32, input_shape, 'param_real')
            input_imag = tf.compat.v1.placeholder(np.float32, input_shape, 'param_imag')
            input = tf.raw_ops.Complex(real=input_real, imag=input_imag)
            rank = tf.raw_ops.Rank(input=input)
            # real = tf.raw_ops.Real(input=rank)
            # imag = tf.raw_ops.Imag(input=rank)
            tf.compat.v1.global_variables_initializer()
            tf_net = sess.graph_def
