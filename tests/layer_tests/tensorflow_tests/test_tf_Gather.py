...
class TestGather(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        if self.params_type == str or self.params_type == np.str_:
            ...
            inputs_data["params:0"] = rng.integers(-50, 50, params_shape).astype(
                self.params_type
            )
        inputs_data["indices:0"] = rng.integers(
            0, self.max_index, indices_shape
        ).astype(self.indices_type)
        return inputs_data

    def create_gather_net(
        self,
        params_shape,
        params_type,
        indices_shape,
        ...
        assert (
            0 <= axis_norm < len(params_shape)
        ), "Incorrect `axis` value for the test case"
        ...
        with tf.compat.v1.Session() as sess:
            ...
            if operation_type == "Gather":
                ...
                tf.raw_ops.GatherV2(
                    params=params, indices=indices, axis=axis, batch_dims=batch_dims
                )
            else:
    ...
    test_data_precommit = [
        ...
        dict(
            params_shape=[3, 2, 6, 4],
            indices_shape=[3, 2, 1, 3],
            axis_value=-1,
            batch_dims=-2,
            operation_type="GatherV2",
        ),
    ...
    def test_gather(
        ...
        self._test(
            *self.create_gather_net(
                **params, params_type=params_type, indices_type=indices_type
            ),
            ie_device,
            precision,
            ir_version,
