...
class TestMatMul(CommonTFLayerTest):
    ...
    def create_net_with_matmul_op(
        self, x_shape, y_shape, x_bool, y_bool, op_type, ir_version, use_legacy_frontend
    ):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            if op_type == "MatMul":
                ...
                op_type_to_tf[op_type](
                    x=tf_x, y=tf_y, adj_x=x_bool, adj_y=y_bool, name="Operation"
                )
    ...
    def test_matmul_op_precommit(
        ...
        self._test(
            *self.create_net_with_matmul_op(
                **params,
    ...
    @pytest.mark.parametrize("x_bool", [False, True])
    @pytest.mark.parametrize("y_bool", [False, True])
    @pytest.mark.nightly
    def test_matmul_op_nightly(
        self,
        params,
        ie_device,
        precision,
