...
class TestSwitchMerge(CommonTFLayerTest):
    ...
    @pytest.mark.parametrize("x_type", [np.float32, np.int32])
    @pytest.mark.parametrize("params", test_data_basic)
    @pytest.mark.precommit
    @pytest.mark.nightly
    def test_merge_eliminating_several_cond_flows(
        self,
        params,
        cond_value,
        x_type,
        ...
        self._test(
            *self.merge_eliminating_several_cond_flows_net(
                **params, cond_value=cond_value, x_type=x_type
            ),
            ie_device,
            precision,
            ir_version,
