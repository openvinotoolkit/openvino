...
class TestResamplePattern(CommonTFLayerTest):
    def _prepare_input(self, inputs_dict):
        for input in inputs_dict.keys():
            inputs_dict[input] = np.random.randint(1, 256, inputs_dict[input]).astype(
                np.float32
            )
        return inputs_dict
    ...
    test_data = [
        pytest.param(
            dict(shape=[1, 1, 100, 200], factor=2), marks=pytest.mark.precommit
        ),
    ...
    def test_resample(
        ...
        self._test(
            *self.create_resample_net(
                params["shape"], params["factor"], use_legacy_frontend
            ),
            ie_device,
            precision,
            ir_version,
