...
#     def create_sub_placeholder_const_net(
#         self, x_shape, y_shape, ir_version, use_legacy_frontend
#     ):
...
#     test_data_1D = [dict(x_shape=[1], y_shape=[1]), dict(x_shape=[3], y_shape=[3])]

#     @pytest.mark.parametrize("params", test_data_1D)
#     @pytest.mark.nightly
#     def test_sub_placeholder_const_1D(
#         self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
#     ):
#         self._test(
#             *self.create_sub_placeholder_const_net(
...
#     @pytest.mark.nightly
#     def test_sub_placeholder_const_2D(
#         self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
#     ):
#         self._test(
#             *self.create_sub_placeholder_const_net(
...
#         pytest.param(
#             dict(x_shape=[1, 3, 224], y_shape=[1, 3, 224]),
...
#     @pytest.mark.nightly
#     def test_sub_placeholder_const_3D(
#         self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
#     ):
#         self._test(
#             *self.create_sub_placeholder_const_net(
...
#     @pytest.mark.nightly
#     def test_sub_placeholder_const_4D(
#         self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
#     ):
#         self._test(
#             *self.create_sub_placeholder_const_net(
...
#     @pytest.mark.nightly
#     def test_sub_placeholder_const_5D(
#         self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
#     ):
#         self._test(
#             *self.create_sub_placeholder_const_net(
...
#     test_data_broadcast_1D = [dict(x_shape=[3], y_shape=[1])]

#     @pytest.mark.parametrize("params", test_data_broadcast_1D)
#     @pytest.mark.nightly
#     def test_sub_placeholder_const_broadcast_1D(
#         self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
#     ):
#         self._test(
#             *self.create_sub_placeholder_const_net(
...
#     @pytest.mark.nightly
#     def test_sub_placeholder_const_broadcast_2D(
#         self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
#     ):
#         self._test(
#             *self.create_sub_placeholder_const_net(
...
#     @pytest.mark.nightly
#     def test_sub_placeholder_const_broadcast_3D(
#         self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
#     ):
#         self._test(
#             *self.create_sub_placeholder_const_net(
...
#     @pytest.mark.nightly
#     def test_sub_placeholder_const_broadcast_4D(
#         self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
#     ):
#         self._test(
#             *self.create_sub_placeholder_const_net(
...
#     @pytest.mark.nightly
#     def test_sub_placeholder_const_broadcast_5D(
#         self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
#     ):
#         self._test(
#             *self.create_sub_placeholder_const_net(
...
class TestComplexSub(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        inputs_data = {}
        inputs_data["param_real_1:0"] = (
            4 * rng.random(param_real_shape_1).astype(np.float32) - 2
        )
        inputs_data["param_imag_1:0"] = (
            4 * rng.random(param_imag_shape_1).astype(np.float32) - 2
        )
        inputs_data["param_real_2:0"] = (
            4 * rng.random(param_real_shape_2).astype(np.float32) - 2
        )
        inputs_data["param_imag_2:0"] = (
            4 * rng.random(param_imag_shape_2).astype(np.float32) - 2
        )
        print("inputsData", inputs_data)
        return inputs_data

    def create_complex_sub_net(self, input_shape):
        import tensorflow as tf

        tf.compat.v1.reset_default_graph()
        # Create the graph and model
        with tf.compat.v1.Session() as sess:
            param_real1 = tf.compat.v1.placeholder(
                np.float32, input_shape, "param_real_1"
            )
            param_imag1 = tf.compat.v1.placeholder(
                np.float32, input_shape, "param_imag_1"
            )
            param_real2 = tf.compat.v1.placeholder(
                np.float32, input_shape, "param_real_2"
            )
            param_imag2 = tf.compat.v1.placeholder(
                np.float32, input_shape, "param_imag_2"
            )
            ...
            # sub = tf.raw_ops.Sub(x=complex1, y=complex2, name="complex_sub")
            # real = tf.raw_ops.Real(input=sub)
            # img = tf.raw_ops.Imag(input=sub)
            tf.compat.v1.global_variables_initializer()
            tf_net = sess.graph_def
        return tf_net, None
    ...
    def test_complex_sub(
        ...
        self._test(
            *self.create_complex_sub_net(**params),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
