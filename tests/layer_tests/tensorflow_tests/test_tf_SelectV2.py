...
class TestComplexSelectV2(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        inputs_data = {}
        inputs_data['cond:0'] = np.random.randint(0, 2, cond_shape).astype(bool)
        inputs_data['x_real:0'] = np.random.randint(-100, 100, x_shape).astype(np.float32)
        inputs_data['x_imag:0'] = np.random.randint(-100, 100, x_shape).astype(np.float32)
        inputs_data['y_real:0'] = np.random.randint(-100, 100, y_shape).astype(np.float32)
        inputs_data['y_imag:0'] = np.random.randint(-100, 100, y_shape).astype(np.float32)
        return inputs_data
    ...
    def create_select_v2_net(self, cond_shape, x_shape, y_shape):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            y_real = tf.compat.v1.placeholder(tf.float32, x_shape, 'y_real')
            y_imag = tf.compat.v1.placeholder(tf.float32, x_shape, 'y_imag')

            x = tf.raw_ops.Complex(real=x_real, imag=x_imag)
            y = tf.raw_ops.Complex(real=y_real, imag=y_imag)
