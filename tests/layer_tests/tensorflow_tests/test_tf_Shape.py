...
class TestShape(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        if self.input_type == str or self.input_type == np.str_:
            ...
            inputs_data["input:0"] = rng.integers(-10, 10, size=input_shape).astype(
                self.input_type
            )
        return inputs_data
    ...
    def test_shape_basic(
        ...
        if ie_device == "GPU" and (
            input_shape == [] or input_type == str or input_type == np.str_
        ):
            pytest.skip(
                "scalar shape is not supported or string type is not supported on GPU"
            )
        if input_shape == [] and out_type == tf.int64:
            pytest.skip("129100 - Hangs or segfault")
        self._test(
            *self.create_shape_net(
                input_shape=input_shape, input_type=input_type, out_type=out_type
            ),
            ie_device,
            precision,
            ir_version,
