...
class TestFloorDiv(CommonTFLayerTest):
    def create_add_placeholder_const_net(self, x_shape, ir_version, use_new_frontend):
        """
            Tensorflow net                  IR net
        ...
        with tf.compat.v1.Session() as sess:
            ...
            x = tf.compat.v1.placeholder(tf.int32, x_shape, 'Input')
            constant_value = np.array(-10).astype(np.int32)
            y = tf.constant(constant_value)
...
#             constant_value = np.array(-2).astype(np.float32)
#             const = tf.constant(constant_value)
#
#             res = x / const
#
#             tf.compat.v1.global_variables_initializer()
