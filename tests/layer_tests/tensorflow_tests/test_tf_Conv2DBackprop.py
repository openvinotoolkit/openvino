...
class TestConv2DBackprop(CommonTFLayerTest):
    ...
    def create_conv2dbackprop_placeholder_const_net(
        self,
        input_shape,
        input_filter,
        out_backprop,
        ...
        with tf.compat.v1.Session() as sess:
            ...
            tf_backprop = tf.compat.v1.placeholder(
                tf.float32, out_backprop, "InputBack"
            )

            tf.raw_ops.Conv2DBackpropInput(
                input_sizes=tf_input,
                filter=tf_filter,
                out_backprop=tf_backprop,
    ...
    test_data = [
        dict(
            ...
            input_filter=[1, 1, 1, 1],
            out_backprop=[1, 10, 10, 1],
            input_strides=[1, 1, 1, 1],
            input_padding="SAME",
            dilations=None,
        ),
        dict(
            input_shape=[1, 10, 10, 3],
            input_filter=[2, 2, 3, 3],
            out_backprop=[1, 10, 10, 3],
            input_strides=[1, 1, 1, 1],
            input_padding="SAME",
            dilations=None,
        ),
        dict(
            input_shape=[1, 10, 10, 3],
            input_filter=[2, 2, 3, 3],
            out_backprop=[1, 5, 5, 3],
            input_strides=[1, 2, 2, 1],
            input_padding="SAME",
            dilations=None,
        ),
        dict(
        ...
        pytest.param(
            dict(
                ...
                input_filter=[2, 3, 3, 3],
                out_backprop=[1, 28, 28, 3],
                input_strides=[1, 2, 2, 1],
                input_padding="SAME",
                dilations=None,
            ),
        ...
        pytest.param(
            dict(
                ...
                input_filter=[3, 2, 3, 3],
                out_backprop=[1, 31, 24, 3],
                input_strides=[1, 2, 2, 1],
                input_padding="VALID",
                dilations=None,
            ),
    ...
    @pytest.mark.nightly
    def test_conv2dbackprop_placeholder_const(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_conv2dbackprop_placeholder_const_net(
