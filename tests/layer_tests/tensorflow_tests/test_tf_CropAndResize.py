...
class TestCropAndResize(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        inputs_data["image:0"] = np.random.randint(-10, 10, image_shape).astype(
            np.float32
        )

        if "boxes:0" in inputs_info:
            boxes_shape = inputs_info["boxes:0"]
            inputs_data["boxes:0"] = np.random.randint(0, 1.2, boxes_shape).astype(
                np.float32
            )
        if "box_ind:0" in inputs_info:
            box_ind_shape = inputs_info["box_ind:0"]
            inputs_data["box_ind:0"] = np.random.randint(
                0, image_shape[0], box_ind_shape
            ).astype(np.int32)

        return inputs_data

    def create_crop_and_resize_net(
        self, image_shape, num_boxes, crop_size_value, method, extrapolation_value
    ):
    ...
    test_data_basic = [
        ...
        pytest.param(
            dict(
                image_shape=[1, 80, 40, 3],
                num_boxes=10,
                crop_size_value=[20, 15],
                method="bilinear",
        ...
        pytest.param(
            dict(
                image_shape=[20, 10, 50, 2],
                num_boxes=5,
                crop_size_value=[4, 50],
                method="nearest",
                extrapolation_value=0.0,
            ),
            marks=pytest.mark.xfail(reason="102603"),
