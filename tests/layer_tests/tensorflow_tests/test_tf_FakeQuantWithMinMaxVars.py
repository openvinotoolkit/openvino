...
class TestFakeQuantWithMinMaxVars(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        inputs_data = {}
        inputs_data["inputs:0"] = np.random.randint(-10, 10, inputs_shape).astype(
            np.float32
        )
        return inputs_data

    def create_fake_quant_with_min_max_vars_net(
        self, inputs_shape, min_value, max_value, num_bits, narrow_range, fake_quant_op
    ):
    ...
    @pytest.mark.parametrize(
        "inputs_shape, min_value, max_value, num_bits, narrow_range", test_basic
    )
    @pytest.mark.parametrize(
        "fake_quant_op",
    ...
    def test_fake_quant_with_min_max_vars_basic(
        ...
        self._test(
            *self.create_fake_quant_with_min_max_vars_net(
                **params, fake_quant_op=OPS[fake_quant_op]
            ),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.parametrize(
        "inputs_shape, min_value, max_value, num_bits, narrow_range, fake_quant_op",
        test_per_channel_basic,
    )
    @pytest.mark.precommit
    @pytest.mark.nightly
    @pytest.mark.xfail("104822")
    def test_fake_quant_with_min_max_vars_per_channel_basic(
        self,
        inputs_shape,
        min_value,
        max_value,
        ...
        params = dict(
            ...
            num_bits=num_bits,
            narrow_range=narrow_range,
            fake_quant_op=OPS[fake_quant_op],
        )
        self._test(
            *self.create_fake_quant_with_min_max_vars_net(**params),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
