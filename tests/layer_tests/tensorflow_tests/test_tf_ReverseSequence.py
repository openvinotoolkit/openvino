...
class TestReverseSequence(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        inputs_data["seq_lengths:0"] = np.random.randint(
            0, self.max_seq_length + 1, seq_lengths_shape
        ).astype(self.seq_lengths_type)
        return inputs_data

    def create_reverse_sequence_net(
        self, input_shape, input_type, seq_lengths_type, seq_dim, batch_dim
    ):
        self.input_type = input_type
        self.seq_lengths_type = seq_lengths_type
        assert 0 <= batch_dim and batch_dim < len(
            input_shape
        ), "Incorrect `batch_dim` in the test case"
        assert 0 <= seq_dim and seq_dim < len(
            input_shape
        ), "Incorrect `seq_dim` in the test case"
        ...
        with tf.compat.v1.Session() as sess:
            ...
            seq_lengths = tf.compat.v1.placeholder(
                seq_lengths_type, [batch_size], "seq_lengths"
            )
    ...
    def test_reverse_sequence_basic(
        ...
        self._test(
            *self.create_reverse_sequence_net(**params),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
