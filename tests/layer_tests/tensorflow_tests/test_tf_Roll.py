...
class TestTFRoll(CommonTFLayerTest):
    def create_tf_roll_net(
        self, shift, axis, x_shape, input_type, ir_version, use_legacy_frontend
    ):
    ...
    test_data = [
        dict(shift=[1], axis=[-1], x_shape=[4, 3], input_type=tf.float32),
        pytest.param(
            dict(
                shift=[1, 5, -7],
                axis=[0, 1, 1],
        ...
        pytest.param(
            dict(
                shift=[1, -2], axis=[0, 1], x_shape=[2, 4, 3, 5], input_type=tf.float32
            ),
            marks=pytest.mark.precommit,
        ),
    ]
