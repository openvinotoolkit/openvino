...
class TestFusedBatchNorm(CommonTFLayerTest):
    ...
    def create_fused_batch_norm_net(
        self,
        x_shape,
        epsilon,
        ...
        with tf.compat.v1.Session() as sess:
            ...
            if not is_training:
                ...
                variance = tf.raw_ops.AddV2(
                    x=variance, y=tf.constant(2.0, dtype=tf.float32)
                )
            fused_batch_norm = fbn_func(
                x=x,
                scale=scale,
                offset=offset,
                epsilon=epsilon,
