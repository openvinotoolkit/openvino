...
class TestUnaryOps(CommonTFLayerTest):
    ...
    def _prepare_input(self, inputs_dict):
        ...
        for input in inputs_dict.keys():
            if self.current_op_type in logical_type:
                inputs_dict[input] = np.random.randint(0, 1, inputs_dict[input]).astype(
                    bool
                )
            elif self.current_op_type in bitwise_type:
                inputs_dict[input] = np.random.randint(
                    lower, upper, inputs_dict[input]
                ).astype(np.int32)
            else:
                inputs_dict[input] = np.random.uniform(
                    lower, upper, inputs_dict[input]
                ).astype(np.float32)
    ...
    @pytest.mark.parametrize("params", test_data_precommit)
    @pytest.mark.nightly
    def test_unary_op_mish_precommit(
    ...
    test_data = [
        pytest.param(dict(shape=[10, 12]), marks=pytest.mark.precommit),
        dict(shape=[8, 10, 12]),
