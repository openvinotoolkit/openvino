...
class TestSpaceToBatch(CommonTFLayerTest):
    ...
    test_data_basic = [
        ...
        dict(
            in_shape=[2, 3, 6, 5],
            block_shape_value=[2, 3, 3],
            pads_value=[[1, 0], [0, 0], [2, 2]],
        ),
        dict(
            in_shape=[1, 2, 9, 1], block_shape_value=[4, 3], pads_value=[[1, 1], [2, 4]]
        ),
    ]
    ...
    def test_space_to_batch_basic(
        ...
        self._test(
            *self.create_space_to_batch_net(**params),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
    ...
    test_data_4D = [
        dict(
            in_shape=[1, 2, 2, 3], block_shape_value=[2, 2], pads_value=[[0, 0], [0, 0]]
        ),
        dict(
            in_shape=[1, 2, 1, 4],
            block_shape_value=[3, 2, 2],
            pads_value=[[1, 0], [0, 1], [1, 1]],
        ),
    ]

    @pytest.mark.parametrize("params", test_data_4D)
    @pytest.mark.nightly
    def test_space_to_batch_4D(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_space_to_batch_net(**params),
    ...
    test_data_5D = [
        dict(
            in_shape=[3, 3, 4, 5, 2],
            block_shape_value=[3, 4, 2],
            pads_value=[[1, 2], [0, 0], [3, 0]],
        ),
        dict(
            in_shape=[3, 3, 4, 5, 2],
            block_shape_value=[3, 4, 2, 2],
