...
class TestResize(CommonTFLayerTest):
    ...
    @pytest.mark.parametrize(
        "images_shape, images_type, size_value, align_corners, half_pixel_centers, resize_op",
        test_data_basic,
    )
    ...
    def test_resize_basic(
        ...
        if (
            ie_device == "GPU"
            and images_type == tf.int32
            and resize_op == "tf.raw_ops.ResizeNearestNeighbor"
        ):
            pytest.skip(
                "Couldn't find a suitable kernel for interpolate:Interpolate_8438725 issue on GPU"
            )
        params = dict(
            ...
            align_corners=align_corners,
            half_pixel_centers=half_pixel_centers,
            resize_op=OPS[resize_op],
        )
        self._test(
            *self.create_resize_net(**params),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
