...
class TestStridedSlice(CommonTFLayerTest):
    ...
    test_squeeze_data = [
        ...
        dict(
            input_shape=[1, 1, 5, 5, 3],
            begin_value=[0, 0, 0, 0, 0],
            end_value=[1, 1, 5, 5, 3],
    ...
    @pytest.mark.nightly
    def test_strided_slice_replace_with_unsqueeze(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_strided_slice_net(**params),
...
class TestComplexStridedSlice(CommonTFLayerTest):
    ...
    def create_complex_strided_slice_net(
        self,
        input_shape,
        begin_value,
        end_value,
        ...
        with tf.compat.v1.Session() as sess:
            ...
            strides = tf.constant(strides_value, dtype=tf.int32)
            strided_slice = tf.raw_ops.StridedSlice(
                input=complex,
                begin=begin,
                end=end,
                strides=strides,
    ...
    @pytest.mark.precommit
    @pytest.mark.nightly
    def test_complex_strided_slice(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        if ie_device == "GPU":
