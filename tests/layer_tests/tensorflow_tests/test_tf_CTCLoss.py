...
class TestCTCLoss(CommonTFLayerTest):
    def _prepare_input(self, inputs_dict):
        for input in inputs_dict.keys():
            inputs_dict[input] = np.random.randint(0, 5, inputs_dict[input]).astype(
                np.float32
            )
        return inputs_dict

    def create_ctcloss_placeholder_const_net(
        self, inputs, targets, preprocess_collapse_repeated, ctc_merge_repeated
    ):
    ...
    def test_ctcloss_placeholder_const(
        self,
        params,
        ...
        self._test(
            *self.create_ctcloss_placeholder_const_net(
                **params,
                preprocess_collapse_repeated=preprocess_collapse_repeated,
                ctc_merge_repeated=ctc_merge_repeated
            ),
