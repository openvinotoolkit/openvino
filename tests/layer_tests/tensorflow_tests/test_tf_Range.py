...
class TestRange(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        if self.negative_delta:
            inputs_data["start:0"] = np.random.randint(1, 10, []).astype(
                self.input_type
            )
            inputs_data["limit:0"] = np.random.randint(-10, 0, []).astype(
                self.input_type
            )
            inputs_data["delta:0"] = np.random.randint(-5, -1, []).astype(
                self.input_type
            )
        else:
            inputs_data["start:0"] = np.random.randint(1, 10, []).astype(
            ...
            inputs_data["limit:0"] = np.random.randint(10, 30, []).astype(
                self.input_type
            )
