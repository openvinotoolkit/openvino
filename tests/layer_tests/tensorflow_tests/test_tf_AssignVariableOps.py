...
class TestAssignVariableOps(CommonTFLayerTest):
    ...
    def create_assign_variable_ops_net(self, const_shape):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            assign_variable_op = tf.raw_ops.AssignVariableOp(
                resource=resource, value=const_value
            )
            with tf.control_dependencies([assign_variable_op]):
                const_value2 = rng.uniform(-2.0, 2.0, const_shape).astype(input_type)
                assign_add_variable_op = tf.raw_ops.AssignAddVariableOp(
                    resource=resource, value=const_value2
                )
            with tf.control_dependencies([assign_add_variable_op]):
                const_value3 = rng.uniform(-2.0, 2.0, const_shape).astype(input_type)
                assign_sub_variable_op = tf.raw_ops.AssignSubVariableOp(
                    resource=resource, value=const_value3
                )
            with tf.control_dependencies([assign_sub_variable_op]):
                resource_value = tf.raw_ops.ReadVariableOp(
                    resource=resource, dtype=tf.float32
                )
    ...
    def test_assign_variable_ops(
        ...
        self._test(
            *self.create_assign_variable_ops_net(const_shape),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
