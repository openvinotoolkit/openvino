...
class TestTFEqual(CommonTFLayerTest):
        ...
        for input in inputs_dict.keys():
            if isinstance(self.x_shape, np.ndarray) or isinstance(self.x_shape, list):
                if not self.x_value is None:
                    inputs_dict[input] = np.full(
                        inputs_dict[input], self.x_value, dtype=self.output_type
                    )
                else:
                    inputs_dict[input] = np.random.randint(
                        -3, 3, inputs_dict[input]
                    ).astype(self.output_type)
            else:
                if not self.x_value is None:
                    if not isinstance(self.x_value, list):
                        ...
                        inputs_dict[input] = np.ndarray(
                            self.x_value, dtype=self.output_type
                        )
                else:
                    raise RuntimeError(
                        "x_shape shouldn't be a scalar value, use x_value instead"
                    )
        return inputs_dict
        ...
        with tf.compat.v1.Session() as sess:
            ...
            if isinstance(self.y_shape, np.ndarray) or isinstance(self.y_shape, list):
                if not self.y_value is None:
                    constant_value = np.full(
                        self.y_shape, self.y_value, dtype=self.output_type
                    )
                else:
                    constant_value = np.random.randint(-3, 3, self.y_shape).astype(
                        self.output_type
                    )
            else:
                if not self.y_value is None:
                    if not isinstance(self.y_value, list):
                        ...
                        constant_value = np.ndarray(
                            self.y_value, dtype=self.output_type
                        )
                else:
                    raise RuntimeError(
                        "y_shape shouldn't be a scalar value, use y_value instead"
                    )
    ...
    test_data_int32 = [
        ...
        dict(
            x_shape=[2, 3], y_value=2
        ),  # Comparing shape with scalar value (verifies false and possible true)
        dict(
            x_shape=[2, 3],
            y_shape=[
                2,
                3,
            ],  # Comparing shapes with same values (verifies true statement)
            x_value=2,
            y_value=2,
        ),
        dict(
            x_shape=[2, 3],
            y_value=2,  # Comparing shape with scalar value (verifies true statement)
            x_value=2,
        ),
    ...
    x_corner = [1.0, 1.0, 1.0, np.nan, np.nan, np.nan, np.inf, np.inf, np.NINF]
    y_corner = [
        np.nan,
        np.inf,
        np.NINF,
    ...
    test_data_float16 = [
        ...
        pytest.param(
            dict(
                x_shape=[9],
                y_shape=[9],  # Comparing shapes which contains corner cases
                x_value=x_corner,
                y_value=y_corner,
            ),
...
class TestEqualStr(CommonTFLayerTest):
    ...
    def test_equal_str(
        ...
        self._test(
            *self.create_equal_net(x_shape=x_shape, y_shape=y_shape),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
