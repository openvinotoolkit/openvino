...
class TestFunctionalGroupedMM(PytorchLayerTest):
    """Tests torch.nn.functional.grouped_mm — 3D x 3D no-`offs` (batched matmul) case."""
    ...
    def create_model(self):
        ...
        class functional_grouped_mm(torch.nn.Module):
            def forward(self, a, b):
                return torch.nn.functional.grouped_mm(a, b)

        return functional_grouped_mm(), "aten::_grouped_mm"
    ...
    @pytest.mark.precommit
    @pytest.mark.precommit_torch_export
    def test_functional_grouped_mm(self, kwargs_to_prepare_input, ie_device, precision, ir_version):
        if ie_device.startswith("GPU"):
            pytest.skip("GPU requires constant weights for GroupedMatMul")
        if ie_device == "CPU":
            pytest.skip("CPU does not support GroupedMatMul with non-constant weights")
...
class TestFunctionalGroupedMMOffsets(PytorchLayerTest):
    """Tests torch.nn.functional.grouped_mm with `offs` (2D x 3D MoE case)."""
    ...
    def create_model(self):
        ...
        class functional_grouped_mm_offs(torch.nn.Module):
            def forward(self, a, b, offs):
                return torch.nn.functional.grouped_mm(a, b, offs=offs)

        return functional_grouped_mm_offs(), "aten::_grouped_mm"
    ...
    @pytest.mark.precommit
    @pytest.mark.precommit_torch_export
    def test_functional_grouped_mm_offs(self, kwargs_to_prepare_input, ie_device, precision, ir_version):
        if ie_device.startswith("GPU"):
            pytest.skip("GPU requires constant weights for GroupedMatMul")
        if ie_device == "CPU":
            pytest.skip("CPU does not support GroupedMatMul with non-constant weights and offsets")
