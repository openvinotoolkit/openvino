...
class aten_lstm(torch.nn.Module):
    def __init__(
        self, input_size, hidden_size, num_layers, has_bias, bidirectional, batch_first
    ):
...
class aten_lstm_packed(torch.nn.Module):
    ...
    def forward(self, seq, lengths):
        seq1 = torch.nn.utils.rnn.pack_padded_sequence(
            seq, lengths, batch_first=self.batch_first
        )
        seq2, hid2 = self.rnn(seq1)
        seq = torch.nn.utils.rnn.pad_packed_sequence(
            seq2, batch_first=self.batch_first
        )[0]
...
class TestLSTMPacked(PytorchLayerTest):
    def _prepare_input(self):
        batch = 15
        if self.batch_first:
            input = np.random.randn(batch, 50, self.input_size).astype(np.float32)
        else:
            input = np.random.randn(50, batch, self.input_size).astype(np.float32)
        lengths = np.array(
            list(
                sorted(
                    np.random.randint(1, 50, [batch - 1]).tolist() + [50], reverse=True
                )
            ),
            dtype=np.int32,
        )
