...
class TestLinalgCross(PytorchLayerTest):
    def _prepare_input(self, x_shape, y_shape, out, dtype):
        ...
        return (
            ...
            y,
            np.zeros(
                np.maximum(np.array(x_shape), np.array(y_shape)).tolist(), dtype=dtype
            ),
        )
    ...
    def test_linalg_cross(
        self, x_shape, y_shape, dim, out, dtype, ie_device, precision, ir_version
    ):
        self._test(
...
class TestCross(PytorchLayerTest):
    ...
    def test_linalg_cross(
        ...
        self._test(
            *self.create_model(dim, out, x_shape),
            ie_device,
            precision,
            ir_version,
            use_convert_model=True,
