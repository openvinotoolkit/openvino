...
class TestSortConstants(PytorchLayerTest):
    ...
    @pytest.mark.parametrize("stable", [False, None, True])
    @pytest.mark.nightly
    @pytest.mark.precommit
    @pytest.mark.precommit_fx_backend
    def test_sort(
        self, input_shape, descending, stable, ie_device, precision, ir_version
    ):
        self.input_tensor = []
        ...
        for dim in range(-dims, dims):
            self._test(
                *self.create_model(dim, descending, stable),
                ie_device,
                precision,
                ir_version
            )
