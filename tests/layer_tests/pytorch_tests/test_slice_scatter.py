...
class TestSliceScatter(PytorchLayerTest):
    ...
    def create_model(self, src, dim, start, end, step):
        ...
        return (
            aten_slice_scatter(src, dim, start, end, step),
            ref_net,
            "aten::slice_scatter",
        )
    ...
    @pytest.mark.parametrize(
        ("src", "dim", "start", "end", "step"),
        [
            (torch.ones(2), 1, 1, 2, 1),
        ],
    )
    def aten_slice_scatter(
        self, src, dim, start, end, step, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(src, dim, start, end, step),
            ie_device,
            precision,
            ir_version
        )
