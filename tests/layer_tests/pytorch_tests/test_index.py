...
class TestIndexRange(PytorchLayerTest):
    ...
    @pytest.mark.parametrize(
        ("input_shape", "idx"),
        (((1, 1), [0]), ([2, 3], [1, 2]), ([7, 8, 9], [1]), ([2, 2, 3, 4], [0])),
    )
    ...
    def test_index_range_free_dims(
        self, input_shape, idx, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model2(),
            ie_device,
...
class TestIndexMask(PytorchLayerTest):
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    @pytest.mark.parametrize(("input_shape"), ((1, 1), [2, 3], [7, 8, 9], [2, 2, 3, 4]))
...
class TestIndexNone(PytorchLayerTest):
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                (2, 3, 4, 5),
                (torch.unsqueeze(torch.randint(0, 2, [14], dtype=torch.int32), 1),),
            ),
            (
                (2, 3, 4, 5),
                (
                    torch.unsqueeze(torch.randint(0, 2, [14], dtype=torch.int32), 1),
                    torch.randint(0, 3, [14], dtype=torch.int32),
                ),
            ),
            (
                (2, 3, 4, 5),
