...
class TestAdd(PytorchLayerTest):
    ...
    def test_add(
        self, ie_device, precision, ir_version, alpha, input_shape_rhs, op_type
    ):
        self.input_rhs = np.random.randn(*input_shape_rhs).astype(np.float32)
        self._test(
            *self.create_model(alpha, op_type),
            ie_device,
            precision,
            ir_version,
...
class TestAddTypes(PytorchLayerTest):
    ...
    def _prepare_input(self):
        ...
        return (
            torch.randint(0, 10, self.lhs_shape).to(self.lhs_type).numpy(),
    ...
    def create_model(self, lhs_type, lhs_shape, rhs_type, rhs_shape):
        ...
        class aten_add(torch.nn.Module):
            ...
            def forward1(self, rhs):
                return torch.add(
                    torch.tensor(1).to(self.lhs_type), rhs.to(self.rhs_type), alpha=2
                )

            def forward2(self, lhs):
                return torch.add(
                    lhs.to(self.lhs_type), torch.tensor(1).to(self.rhs_type), alpha=2
                )
...
class TestAddBool(PytorchLayerTest):
    ...
    def test_add(self, lhs_type, rhs_type, ie_device, precision, ir_version):
        self._test(
            *self.create_model(lhs_type, rhs_type), ie_device, precision, ir_version
        )
