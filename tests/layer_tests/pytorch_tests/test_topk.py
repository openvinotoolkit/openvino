...
class TestTopK(PytorchLayerTest):
    ...
    def create_model(self, k, dim, largest, sort):
        ...
        class aten_topk(torch.nn.Module):
            ...
            def forward(self, input_tensor):
                if self.dim is None:
                    return torch.topk(
                        input_tensor, k=self.k, largest=self.largest, sorted=self.sort
                    )
                else:
                    return torch.topk(
                        input_tensor,
    ...
    @pytest.mark.parametrize(("input_shape"), [[7, 5, 5, 4], [5, 6, 6, 7, 8]])
    ...
    def test_topK(
        self, input_shape, k, dim, largest, sort, ie_device, precision, ir_version
    ):
        self.input_tensor = np.random.randn(*input_shape).astype(np.float32)
        self._test(
            *self.create_model(k, dim, largest, sort), ie_device, precision, ir_version
        )
