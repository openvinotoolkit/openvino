...
@pytest.mark.parametrize('p', [-2, -1, 1, 2, 2.5, float('inf'), float('-inf')])
...
class TestLinalgVectorNorm(PytorchLayerTest):
    ...
    def _prepare_input(self, out=False, out_dtype=None):
        ...
        y = np.random.randn(2, 3, 4, 5).astype(out_dtype if out_dtype is not None else np.float32)
        return (x, y)
    ...
    @pytest.mark.parametrize('dim', [0, [0, 1], [0, 1, 2], None])
    @pytest.mark.parametrize('keepdim', [True, False])
