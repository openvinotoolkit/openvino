...
class TestNorm(PytorchLayerTest):
    ...
    def test_norm(self, ie_device, precision, ir_version, p, dim, keepdim):
        self._test(
            *self.create_model(p, dim, keepdim), ie_device, precision, ir_version
        )
    ...
    def test_norm_tensor(self, ie_device, precision, ir_version, p, dim, keepdim):
        self._test(
            *self.create_model_tensor_norm(p, dim, keepdim),
            ie_device,
            precision,
            ir_version
        )
...
class TestFrobeniusNorm(PytorchLayerTest):
            ...
            def forward_out(self, input_data, out):
                return (
                    torch._VF.frobenius_norm(
                        input_data, self.dim, self.keepdim, out=out
                    ),
                    out,
                )
    ...
    def test_frobenius_norm(
        self, ie_device, precision, ir_version, dim, keepdim, out, dtype
    ):
        self._test(
            *self.create_model(dim, keepdim, out),
            ie_device,
            precision,
            ir_version,
...
class TestLinalgVectorNorm(PytorchLayerTest):
    ...
    def create_model(self, p, dim, keepdim, dtype_str, out, out_as_dtype):
        ...
        return (
            aten_linalg_vector_norm(p, dim, keepdim, dtype, out, out_as_dtype),
            ref_net,
    ...
    def test_linalg_vector_norm(
        self, p, dim, keepdim, dtype, out, prim_dtype, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(p, dim, keepdim, dtype, out, prim_dtype),
            ie_device,
            precision,
            ir_version,
...
class TestLinalgMatrixNorm(PytorchLayerTest):
    ...
    def create_model(self, p, dim, keepdim, dtype_str, out, out_as_dtype):
        ...
        return (
            aten_linalg_matrix_norm(p, dim, keepdim, dtype, out, out_as_dtype),
            ref_net,
