...
class TestPad(PytorchLayerTest):
    ...
    def test_pad4d(self, pads, mode, value, dtype, ie_device, precision, ir_version):
        self._test(
            *self.create_model(pads, mode, value),
            ie_device,
            precision,
            ir_version,
    ...
    def test_pad5d(self, pads, mode, value, dtype, ie_device, precision, ir_version):
        self._test(
            *self.create_model(pads, mode, value),
            ...
            precision,
            ir_version,
            kwargs_to_prepare_input={"ndim": 5, "dtype": dtype},
            trace_model=True
        )
    ...
    def test_pad2d(self, pads, mode, value, dtype, ie_device, precision, ir_version):
        self._test(
            *self.create_model(pads, mode, value),
            ...
            precision,
            ir_version,
            kwargs_to_prepare_input={"ndim": 2, "dtype": dtype},
            trace_model=True
        )
...
class TestPadListPaddingings(PytorchLayerTest):
    def _prepare_input(self, ndim=4, pad_w=0, pad_h=0, dtype="float32"):
        ...
        input_5d_shape = [1, 3, 14, 14, 18]
        return (
            np.random.randn(*input_5d_shape[:ndim]).astype(dtype),
            np.array(pad_w, dtype=np.int32),
            np.array(pad_h, dtype=np.int32),
        )
    ...
    @pytest.mark.precommit
    def test_pad4d(
        self, pad_w, pad_h, mode, value, dtype, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(mode, value),
            ie_device,
            precision,
            ir_version,
