...
class TestRsub(PytorchLayerTest):
    ...
    def create_model(self, second_type="float"):
        ...
        model_cls = {"float": aten_rsub_float, "int": aten_rsub_int}
    ...
    @pytest.mark.parametrize(
        "input_data", [[[2, 3, 4], np.array(5).astype(np.float32), [1]]]
    )
    @pytest.mark.nightly
    ...
    def test_rsub1(self, ie_device, precision, ir_version, input_data):
        ...
        self._test(
            *self.create_model(second_type="float"),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.parametrize("input_data", [[[2, 3, 4], np.array(5).astype(int), [1]]])
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_rsub2(self, ie_device, precision, ir_version, input_data):
        ...
        for input in input_data:
            if type(input) is list:
                ...
                self.input_data.append(input)
        self._test(
            *self.create_model(second_type="int"),
            ie_device,
            precision,
            ir_version,
            use_convert_model=True
...
class TestRsubTypes(PytorchLayerTest):
    ...
    def test_rsub_types(
        self, ie_device, precision, ir_version, lhs_type, lhs_shape, rhs_type
    ):
        self.lhs_type = lhs_type
