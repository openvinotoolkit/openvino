    ...
    def __init__(self, mask, need_weights, average_attn_weights) -> None:
        ...
        if mask == 0:
            self.mask = torch.from_numpy(
                np.random.randint(0, 2, (SEQ_LENGTH, SEQ_LENGTH)).astype("bool")
            )
            self.mask_type = 0
        elif mask == 1:
            self.mask = torch.from_numpy(
                np.random.randint(0, 2, (BATCH_SIZE, SEQ_LENGTH)).astype("bool")
            )
            self.mask_type = 1
        elif mask == 2:
            self.mask = torch.from_numpy(
                np.random.randint(
                    0, 2, (BATCH_SIZE, NUM_HEADS, SEQ_LENGTH, SEQ_LENGTH)
                ).astype("bool")
            )
            self.mask_type = 2
        else:
...
class TestNativeMultiHeadAttention(PytorchLayerTest):
    def _prepare_input(self):
        ...
        qkv_tensor = np.random.randn(BATCH_SIZE, SEQ_LENGTH, EMBED_DIM).astype(
            np.float32
        )
        return (qkv_tensor.copy(), qkv_tensor.copy(), qkv_tensor.copy())

    @pytest.mark.nightly
    @pytest.mark.precommit
    @pytest.mark.parametrize("mask", [NO_MASK, ATTN_MASK, KEY_PAD_MASK, MERGED_MASK])
    ...
    def test_native_multi_head_attention(
        self, ie_device, precision, ir_version, mask, need_weights, average_attn_weights
    ):
        self._test(
            aten_native_multi_head_attention(mask, need_weights, average_attn_weights),
            None,
