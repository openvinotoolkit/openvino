...
class TestMul(PytorchLayerTest):
    def _prepare_input(self):
        return (
            self.input_array.astype(self.input_type),
            self.other_array.astype(self.other_type),
        )
    ...
    def test_mul_pt_spec(
        self, input_array, other_array, ie_device, precision, ir_version
    ):
...
class TestMulBool(PytorchLayerTest):
    ...
    def create_model(self, lhs_type, rhs_type):
        class aten_mul(torch.nn.Module):
            ...
            def forward(self, input_tensor, other_tensor):
                return torch.mul(
                    input_tensor.to(self.lhs_type), other_tensor.to(self.rhs_type)
                )
    ...
    def test_mul_bool(self, lhs_type, rhs_type, ie_device, precision, ir_version):
        self._test(
            *self.create_model(lhs_type, rhs_type),
            ie_device,
            precision,
            ir_version,
            use_convert_model=True
