...
class TestScatter(PytorchLayerTest):
        ...
        class aten_scatter(torch.nn.Module):
            ...
            def _forward_with_out_reduce(self, x: torch.Tensor, out: torch.Tensor):
                ...
                return torch.scatter(
                    x, self.dim, index, self.src, reduce=self.reduce, out=out
                )
    ...
    @pytest.mark.parametrize(
        ["inplace", "has_out"], [(True, False), (False, True), (False, False)]
    )
...
class TestScatterReduce(PytorchLayerTest):
    ...
    def create_model(self, dim, index, src, inplace, reduce, include_self, has_out):
        class aten_scatter_reduce(torch.nn.Module):
            def __init__(
                self, dim, index, src, inplace, reduce, include_self, has_out=False
            ):
        ...
        return (
            aten_scatter_reduce(
                dim, index, src, inplace, reduce, include_self, has_out
            ),
            ref_net,
            op_name,
        )
...
class TestScatterAdd(PytorchLayerTest):
    ...
    def test_scatter_add(
        self, dim, index, src, dtype, inplace, ie_device, precision, ir_version
    ):
        if isinstance(src, torch.Tensor):
            src = src.to(getattr(torch, dtype))
        if index is None:
