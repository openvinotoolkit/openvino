...
class TestScaledDotProductAttention(PytorchLayerTest):
    ...
    def _prepare_input(self, dtype):
        return (
            np.random.randn(1, 2, 8, 4).astype(dtype),
            np.random.randn(1, 2, 8, 4).astype(dtype),
            np.random.randn(1, 2, 8, 4).astype(dtype),
        )
        ...
        class aten_scaled_dot_product_atten(torch.nn.Module):
            ...
            def __init__(self, mask=False, is_causal=False, dtype=np.float32) -> None:
                ...
                self.mask = (
                    None
                    if not mask
                    else torch.from_numpy(np.random.randint(0, 2, (8, 8)).astype(dtype))
                )
            ...
            def forward(self, query, key, value):
                ...
                a = F.scaled_dot_product_attention(
                    query, key, value, attn_mask=self.mask, is_causal=self.is_causal
                )
                b = F.scaled_dot_product_attention(
                    ...
                    scale=torch.tensor(5, dtype=torch.float),
                )
                return a, b

        return (
            aten_scaled_dot_product_atten(mask, is_causal, dtype),
            None,
    ...
    def test_scaled_dot_product_atten(
        self, ie_device, precision, ir_version, mask, is_causal, dtype
    ):
        if PytorchLayerTest.use_torch_export() and not mask and is_causal:
            pytest.xfail(reason="Unsupported case for torch.export")
        self._test(
            *self.create_model(mask, is_causal, dtype),
            ie_device,
            precision,
            ir_version,
