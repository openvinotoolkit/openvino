...
class TestArgSort(PytorchLayerTest):
    ...
    def test_argsort(self, tensor_stable_pair, descending, ie_device, precision, ir_version, use_ts_backend):
        ...
        for dim in range(-dims, dims):
            ...
            for stable_value in stable_values:
                self._test(*self.create_model(dim, descending, stable_value), ie_device, precision, ir_version, use_ts_backend=use_ts_backend)
