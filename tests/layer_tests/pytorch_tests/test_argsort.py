...
class TestArgSort(PytorchLayerTest):
        ...
        class aten_argsort(torch.nn.Module):
            ...
            def forward(self, input_tensor):
                if self.stable is not None:
                    ...
                    return torch.argsort(
                        input_tensor, dim=self.dim, descending=self.descending
                    )
    ...
    def test_argsort(
        self, tensor_stable_pair, descending, ie_device, precision, ir_version
    ):
        ...
        for dim in range(-dims, dims):
            ...
            for stable_value in stable_values:
                self._test(
                    *self.create_model(dim, descending, stable_value),
                    ie_device,
                    precision,
                    ir_version
                )
