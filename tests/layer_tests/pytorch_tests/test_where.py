...
class Testwhere(PytorchLayerTest):
    ...
    def create_model(self, as_non_zero, dtypes=None):
        ...
        dtype_map = {"float32": torch.float32, "int32": torch.int32}
    ...
    def test_where(
        self, mask_fill, mask_dtype, x_dtype, y_dtype, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(False, dtypes=(x_dtype, y_dtype)),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_where_as_nonzero(
        self, mask_fill, mask_dtype, x_dtype, ie_device, precision, ir_version
    ):
        self._test(
