...
class TestAminMax(PytorchLayerTest):
    ...
    def create_model(self, dtype=None, dim=None, keepdim=False):
        ...
        class aten_aminmax(torch.nn.Module):
            ...
            def forward(self, x):
                return torch.aminmax(
                    x.to(self.dtype), dim=self.dim, keepdim=self.keepdim, out=None
                )
    ...
    @pytest.mark.parametrize(
        "inputs", [[0, 1, 2, 3, 4, -1], [-2, -1, 0, 1, 2, 3], [1, 2, 3, 4, 5, 6]]
    )
    @pytest.mark.parametrize(
        "dim,keepdim",
        [
            (None, False),  # Test with default arguments
            (0, False),  # Test with dim provided and keepdim=False
            (0, True),  # Test with dim provided and keepdim=True
            (None, True),
        ],
    )  # Test with keepdim=True and dim not provided
    def test_aminmax(
        self, dtype, inputs, ie_device, precision, ir_version, dim, keepdim
    ):
        self._test(
