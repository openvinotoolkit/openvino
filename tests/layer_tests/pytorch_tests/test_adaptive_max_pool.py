...
class TestAdaptiveMaxPool3D(PytorchLayerTest):
            ...
            def forward(self, input_tensor):
                if self.return_indices:
                    output, indices = F.adaptive_max_pool3d(
                        input_tensor, self.output_size, True
                    )
                    return output, indices
                return F.adaptive_max_pool3d(
                    input_tensor, self.output_size, False
                ), input_tensor.to(torch.int64)

        ref_net = None

        return (
            aten_adaptive_max_pool3d(output_size, return_indices),
            ref_net,
            "aten::adaptive_max_pool3d",
        )

    @pytest.mark.parametrize(
        "input_shape", [[2, 1, 1, 4, 4], [4, 1, 3, 32, 32], [1, 3, 32, 32]]
    )
    @pytest.mark.parametrize(
        "output_size",
        (
    ...
    def test_adaptive_max_pool3d(
        ...
        self._test(
            *self.create_model(output_size, return_indices),
            ie_device,
            precision,
            ir_version
        )
...
class TestAdaptiveMaxPool2D(PytorchLayerTest):
    ...
    def create_model(self, output_size=None, return_indices=False):
        ...
        return (
            aten_adaptive_max_pool2d(output_size, return_indices),
            ref_net,
            "aten::adaptive_max_pool2d",
        )

    @pytest.mark.parametrize("input_shape", [[2, 1, 4, 4], [1, 3, 32, 32], [3, 32, 32]])
    @pytest.mark.parametrize(
        "output_size",
        (
            [
...
class TestAdaptiveMaxPool1D(PytorchLayerTest):
    ...
    def create_model(self, output_size=None, return_indices=False):
        ...
        return (
            aten_adaptive_max_pool1d(output_size, return_indices),
            ref_net,
            "aten::adaptive_max_pool1d",
        )

    @pytest.mark.parametrize("input_shape", [[1, 4, 4], [3, 32, 32], [16, 8]])
    @pytest.mark.parametrize(
        "output_size",
        (
            [
