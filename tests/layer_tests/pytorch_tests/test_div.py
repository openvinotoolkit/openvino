...
class TestDiv(PytorchLayerTest):
    ...
    def create_model(self, rounding_mode):
        ...
        class aten_div(torch.nn.Module):
            ...
            def forward(self, input_tensor, other_tensor):
                return torch.div(
                    input_tensor, other_tensor, rounding_mode=self.rounding_mode
                )
    ...
    def test_div_pt_spec(
        self, input_array, other_array, rounding_mode, ie_device, precision, ir_version
    ):
        ...
        self._test(
            *self.create_model(rounding_mode),
            ie_device,
            precision,
            ir_version,
...
class TestDivTypes(PytorchLayerTest):
    ...
    def create_model(self, lhs_type, lhs_shape, rhs_type, rhs_shape, rounding_mode):
        ...
        class aten_div(torch.nn.Module):
            ...
            def forward1(self, rhs):
                return torch.div(
                    torch.tensor(3).to(self.lhs_type),
                    rhs.to(self.rhs_type),
                    rounding_mode=self.rm,
                )

            def forward2(self, lhs):
                return torch.div(
                    lhs.to(self.lhs_type),
                    torch.tensor(3).to(self.rhs_type),
                    rounding_mode=self.rm,
                )

            def forward3(self, lhs, rhs):
                return torch.div(
                    lhs.to(self.lhs_type), rhs.to(self.rhs_type), rounding_mode=self.rm
                )

        ref_net = None

        return (
            aten_div(lhs_type, lhs_shape, rhs_type, rhs_shape, rounding_mode),
            ref_net,
            "aten::div",
        )
    ...
    def test_div_types(
        ...
        if (
            rounding_mode == "floor"
            and not lhs_type.is_floating_point
            and not rhs_type.is_floating_point
        ):
            pytest.skip("Floor rounding mode and int inputs produce wrong results")
        self._test(
            *self.create_model(lhs_type, lhs_shape, rhs_type, rhs_shape, rounding_mode),
            ie_device,
            precision,
            ir_version
        )
