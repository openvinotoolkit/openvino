...
class TestReflectionPad(PytorchLayerTest):
    ...
    def create_model(self, pad, n):
        ...
        class aten_reflection_pad(torch.nn.Module):
            def __init__(self, pad=None):
                super().__init__()
                self.pad = pad

            def forward_1d(self, x):
                return torch.ops.aten.reflection_pad1d(x, self.pad)

            def forward_2d(self, x):
                return torch.ops.aten.reflection_pad2d(x, self.pad)

            def forward_3d(self, x):
                return torch.ops.aten.reflection_pad3d(x, self.pad)
    ...
    def test_reflection_pad(self, pad, n, ie_device, precision, ir_version):
