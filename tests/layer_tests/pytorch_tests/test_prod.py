...
class aten_prod_dim_dtype(torch.nn.Module):
    ...
    def forward(self, x):
        return torch.prod(
            x.to(self.in_dtype), self.dim, self.keepdims, dtype=self.dtype
        )
...
class TestProd(PytorchLayerTest):
    ...
    def test_prod(
        ...
        if dtype is not None:
            if has_dim:
                m = aten_prod_dim_dtype(
                    random.randint(0, len(shape) - 1), keepdims, dtype, in_dtype
                )
            else:
                m = aten_prod_dtype(dtype, in_dtype)
        else:
            if has_dim:
                m = aten_prod_dim(random.randint(0, len(shape) - 1), keepdims, in_dtype)
            else:
