...
class TestSum(PytorchLayerTest):
    def _prepare_input(self, out=False, input_dtype="float32", out_dtype="float32"):
        ...
        input = np.random.randint(min_value, max_value, (1, 3, 5, 5)).astype(
            input_dtype
        )
        if not out:
            return (input,)
    ...
    def create_model(self, axes, keep_dims, out, dtype, input_dtype):
        ...
        class aten_sum(torch.nn.Module):
            def __init__(
                self, input_dtype, axes=None, keep_dims=None, dtype=None, out=None
            ):
            ...
            def forward_out(self, x, out):
                ...
                if self.dtype is not None:
                    return (
                        torch.sum(
                            x, self.axes, self.keep_dims, dtype=self.dtype, out=out
                        ),
                        out,
                    )
                else:
        ...
        return (
            aten_sum(input_torch_dtype, axes, keep_dims, torch_dtype, out),
            ref_net,
            "aten::sum",
        )
    ...
    def test_sum(
        self, axes, keep_dims, out, dtype, input_dtype, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(axes, keep_dims, out, dtype, input_dtype),
            ie_device,
            precision,
            ir_version,
