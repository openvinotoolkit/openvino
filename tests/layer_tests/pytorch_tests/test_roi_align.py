...
class TestROIAlign(PytorchLayerTest):
    ...
    def create_model(self, output_size, spatial_scale, sampling_ratio, aligned):
        ...
        return (
            torchvision_roi_align(output_size, spatial_scale, sampling_ratio, aligned),
            ref_net,
    ...
    @pytest.mark.parametrize(
        "boxes",
        (
            np.array([[1, 2, 2, 3, 3]]).astype(np.float32),
            np.array([[0, 1, 2, 5, 4], [2, 1, 2, 5, 4], [3, 1, 2, 5, 4]]).astype(
                np.float32
            ),
        ),
    )
    ...
    def test_roi_align(
        ...
        self.boxes = boxes
        self._test(
            *self.create_model(output_size, spatial_scale, sampling_ratio, aligned),
            ie_device,
            precision,
            ir_version,
            trace_model=True
