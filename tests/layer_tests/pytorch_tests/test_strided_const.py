...
class TestStrides(PytorchLayerTest):
    def _prepare_input(self):
        import numpy as np
        x = self.random.randint(0, 10, [1, 3, 2, 2], dtype=np.float32)
        return (x,)
    ...
    def create_model(self):
        ...
        class strided_const(torch.nn.Module):
            def __init__(self, rng):
                super().__init__()
                self.const = rng.torch_randint(0, 10, [1, 3, 2, 2], dtype=torch.float32)
                self.const = self.const.to(memory_format=torch.channels_last)

            def forward(self, x):
                return x + self.const

        return strided_const(self.random), "aten::add"
