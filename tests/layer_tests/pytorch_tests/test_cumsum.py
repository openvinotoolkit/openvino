...
class TestCumSum(PytorchLayerTest):
            ...
            def __init__(self, axis, dtype, out=False, dtype_from_input=False):
                ...
                if self.dtype is not None:
                    if not dtype_from_input:
                        self.forward = (
                            self.forward_dtype if not out else self.forward_out_dtype
                        )
    ...
    @pytest.mark.parametrize(
        "out,dtype_from_input", [(False, False), (True, False), (True, True)]
    )
    @pytest.mark.nightly
    ...
    def test_cumsum(
        self, axis, dtype, out, dtype_from_input, ie_device, precision, ir_version
    ):
        ...
        self._test(
            *self.create_model(axis, dtype, out, dtype_from_input),
            ie_device,
            precision,
            ir_version,
            kwargs_to_prepare_input={"out": out, "out_dtype": dtype}
        )
