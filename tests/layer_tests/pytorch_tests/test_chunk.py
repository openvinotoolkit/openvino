...
class aten_chunk_2(torch.nn.Module):
    ...
    def forward(self, input_tensor):
        a, b = torch.chunk(input_tensor, chunks=2, dim=self.dim)
        return a, b
...
class aten_chunk_3(torch.nn.Module):
    ...
    def forward(self, input_tensor):
        a, b, c = torch.chunk(input_tensor, chunks=3, dim=self.dim)
        return a, b, c
...
class aten_chunk_4(torch.nn.Module):
    ...
    def forward(self, input_tensor):
        a, b, c, d = torch.chunk(input_tensor, chunks=4, dim=self.dim)
        return a, b, c, d
...
class aten_chunk_getitem(torch.nn.Module):
    ...
    def forward(self, input_tensor):
        return torch.chunk(input_tensor, chunks=self.chunks, dim=self.dim)[self.idx]
    ...
    def test_chunk_getitem(self, input_shape, chunks, ie_device, precision, ir_version):
        ...
        for dim, dim_shape in enumerate(input_shape):
            ...
            for idx in [0, 1, output_chunks - 1]:
                self._test(
                    aten_chunk_getitem(chunks, dim, idx),
                    None,
                    "aten::chunk",
                    ie_device,
                    precision,
...
class aten_chunk_loop_getitem(torch.nn.Module):
    ...
    def forward(self, input_tensor):
        chunks = torch.chunk(torch.arange(input_tensor.shape[0]), self.num_chunks)
...
class TestChunkLoopGetitem(PytorchLayerTest):
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_chunk_loop_getitem(
        self, input_shape, chunks, ie_device, precision, ir_version
    ):
        self.input_shape = input_shape

        self._test(
            aten_chunk_loop_getitem(chunks),
            None,
            ["aten::chunk", "prim::Loop", "aten::__getitem__"],
            ie_device,
            precision,
            ir_version,
        )
