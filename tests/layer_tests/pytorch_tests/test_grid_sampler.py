...
class TestGridSampler(PytorchLayerTest):
    def _prepare_input(self, h_in, w_in, h_out, w_out):
        ...
        return (
            np.random.randn(1, 3, h_in, w_in).astype(np.float32),
            np.random.randn(1, h_out, w_out, 2).astype(np.float32),
        )
    ...
    def create_model(self, mode, padding_mode, align_corners):
        ...
        return (
            aten_grid_sampler(mode, padding_mode, align_corners),
            ref_net,
            "aten::grid_sampler",
        )

    @pytest.mark.parametrize(
        ["h_in", "w_in", "h_out", "w_out"], ([10, 10, 5, 5], [10, 15, 3, 5])
    )
    ...
    def test_grid_sampler(
        ...
        self._test(
            *self.create_model(mode, padding_mode, align_corners),
            ie_device,
            precision,
            ir_version,
