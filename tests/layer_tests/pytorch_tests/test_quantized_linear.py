...
class TestQuantizedLinear(PytorchLayerTest):
    ...
    def create_model(self, weight_shape, is_bias, scale, zero_point):
        ...
        return (
            aten_quantized_linear(weight_shape, is_bias, scale, zero_point),
            ref_net,
            "quantized::linear",
        )
    ...
    def create_hardtanh_model(self, weight_shape, is_bias, scale, zero_point, inplace):
        ...
        return (
            aten_quantized_linear(weight_shape, is_bias, scale, zero_point, inplace),
            ref_net,
            ["quantized::linear", "aten::hardtanh_" if inplace else "aten::hardtanh"],
        )
    ...
    def test_quantized_linear(
        self, params, scale, zero_point, trace, ie_device, precision, ir_version
    ):
        input_shape = params.get("input_shape")
        weight_shape = params.get("weight_shape")
        bias = params.get("bias", False)
        self._test(
            *self.create_model(weight_shape, bias, scale, zero_point),
            ie_device,
            precision,
            ir_version,
    ...
    def test_quantized_hardtanh_linear(
        self, trace, inplace, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_hardtanh_model([10, 9], True, 1, 0.3, inplace),
            ie_device,
            precision,
            ir_version,
            kwargs_to_prepare_input={"input_shape": [2, 3, 9]},
            trace_model=trace,
            freeze_model=False,
            quantized_ops=True,
            quant_size=0.3
