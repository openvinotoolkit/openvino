...
class TestQuantizedLinear(PytorchLayerTest):
    ...
    def create_linear_relu_model(self, weight_shape, is_bias, scale, zero_point):
        
        class aten_quantized_linear_relu(torch.nn.Module):
            def __init__(self, weight_shape, is_bias, scale, zero_point):
                super(aten_quantized_linear_relu, self).__init__()
                if is_bias:
                    self.linear_relu = torch.ao.nn.quantized.Linear(
                        weight_shape[-1], weight_shape[0], True)
            ...
            def forward(self, inp):
                inp_q = torch.quantize_per_tensor(inp, 1.0, 0, torch.quint8)
                return torch.dequantize(torch.ops.quantized.linear_relu(
                    inp_q, self.linear_relu._packed_params._packed_params, self.linear_relu.scale, self.linear_relu.zero_point))
        
        ref_net = None
        
        return aten_quantized_linear_relu(weight_shape, is_bias, scale, zero_point), ref_net, "quantized::linear_relu"
    ...
    def test_quantized_linear_relu(self, params, scale, zero_point, trace, ie_device, precision, ir_version):
        ...
        weight_shape = params.get("weight_shape")
        bias = params.get("bias", False)
        self._test(*self.create_linear_relu_model(weight_shape, bias, scale, zero_point),
                ie_device, precision, ir_version,
                kwargs_to_prepare_input={"input_shape": input_shape},
                trace_model=trace, freeze_model=False, quantized_ops=True, quant_size=scale)
