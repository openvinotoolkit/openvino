...
class TestIndexPut_SingleIndices(PytorchLayerTest):
    ...
    @pytest.mark.parametrize(
        "input_data",
        (
            {"input_shape": [5], "values": np.array(11).astype(np.float32)},
            {
                "input_shape": [3, 3],
    ...
    def test_index_put_single_indices(
        self, ie_device, precision, ir_version, input_data, indices, accumulate
    ):
        self.input_tensor = np.random.randn(*input_data["input_shape"]).astype(
            np.float32
        )
        self.values = input_data["values"]
        self._test(
            *self.create_model(indices, accumulate), ie_device, precision, ir_version
        )
...
class TestIndexPut_ManyIndices(PytorchLayerTest):
    ...
    def create_model(self, indices, accumulate):
        class aten_index_put_(torch.nn.Module):
            ...
            def forward(self, input_tensor, values):
                input_tensor.index_put_(
                    (self.indices_first, self.indices_second), values, self.accumulate
                )
                return input_tensor
    ...
    @pytest.mark.parametrize(
        "input_data",
        (
            {"input_shape": [3, 3], "values": np.array(12).astype(np.float32)},
            {
                "input_shape": [3, 3, 3],
                "values": np.array([10, 11, 12]).astype(np.float32),
            },
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_index_put_many_indices(
        self, ie_device, precision, ir_version, input_data, indices, accumulate
    ):
        self.input_tensor = np.random.randn(*input_data["input_shape"]).astype(
            np.float32
...
class TestMaskKosmos_IndexPut(PytorchLayerTest):
    def _prepare_input(self):
        ...
        num = mask.sum()
        return (
            np.random.randn(1, 30, 50).astype(np.float32),
            mask.astype(np.int32),
            np.random.randn(num, 50).astype(np.float32),
        )
