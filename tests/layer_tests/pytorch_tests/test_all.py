...
class aten_all(torch.nn.Module):
    ...
    def forward(self, input_tensor):
        return (
            torch.all(input_tensor, dim=self.dim)
            if self.keepdim is None
            else torch.all(input_tensor, dim=self.dim, keepdim=self.keepdim)
        )
...
class aten_all_out(torch.nn.Module):
    ...
    def forward(self, input_tensor, out):
        return (
            torch.all(input_tensor, dim=self.dim, out=out)
            if self.keepdim is None
            else torch.all(input_tensor, dim=self.dim, keepdim=self.keepdim, out=out)
        ), out
...
class TestAll(PytorchLayerTest):
    def _prepare_input(self, out=False):
        ...
        return (
            ...
            np.zeros_like(
                self.input_tensor,
                dtype=bool if self.input_tensor.dtype != np.uint8 else np.uint8,
            ),
        )
    ...
    def test_all_noparams(
        self, input_shape, d_type, out, ie_device, precision, ir_version
    ):
        ...
        self._test(
            aten_all_noparam() if not out else aten_all_noparam_out(),
            None,
            "aten::all",
            ie_device,
            precision,
    ...
    def test_all(
        self, input_shape, d_type, keepdim, out, ie_device, precision, ir_version
    ):
        if type(input_shape) is list:
            self.input_tensor = np.random.randint(0, 2, input_shape, dtype=d_type)
        else:
        ...
        for dim in range(len(self.input_tensor.shape)):
            self._test(
                aten_all(dim, keepdim) if not out else aten_all_out(dim, keepdim),
                None,
                "aten::all",
                ie_device,
                precision,
