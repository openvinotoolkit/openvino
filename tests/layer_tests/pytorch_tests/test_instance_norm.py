...
class TestInstanceNorm(PytorchLayerTest):
    ...
    def create_model(self, weights=False, bias=False, mean_var=False, eps=1e-05):
        ...
        return (
            aten_instance_norm(weights, bias, mean_var, eps),
            ref_net,
            "aten::instance_norm",
        )
    ...
    def test_group_norm(
        self, params, ie_device, precision, ir_version, kwargs_to_prepare_input
    ):
        self._test(
            *self.create_model(**params),
            ...
            dynamic_shapes=not params.get("mean_var", False),
