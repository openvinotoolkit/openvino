    ...
    def create_model(self, dtype):
        class aten_exp(torch.nn.Module):
            def __init__(self, dtype):
                super(aten_exp, self).__init__()
                self.dtype = dtype

            def forward(self, x):
                return torch.exp(x.to(self.dtype))

        ref_net = None

        return aten_exp(dtype), ref_net, "aten::exp"

    @pytest.mark.nightly
    @pytest.mark.precommit
    @pytest.mark.parametrize("dtype", [torch.float32, torch.float64, torch.int32, torch.int64])
    def test_exp(self, dtype, ie_device, precision, ir_version):
        self.dtype = dtype
