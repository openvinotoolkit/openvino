...
class TestAddMM(PytorchLayerTest):
    def _prepare_input(
        self, input_shape=(2, 2), matrix1_shape=(2, 2), matrix2_shape=(2, 2)
    ):
        import numpy as np
    ...
    def test_addmm(
        self, kwargs_to_prepare_input, alpha, beta, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(alpha, beta),
            ie_device,
            precision,
            ir_version,
...
class TestBAddBMM(PytorchLayerTest):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {
                "input_shape": (1, 10, 1),
                "matrix1_shape": (1, 10, 5),
                "matrix2_shape": (1, 5, 1),
            },
            {
                "input_shape": (5, 1, 2),
                "matrix1_shape": (5, 1, 10),
                "matrix2_shape": (5, 10, 2),
            },
            {
                "input_shape": (1, 1, 1),
                "matrix1_shape": (1, 1, 10),
                "matrix2_shape": (1, 10, 1),
            },
        ],
    )
    @pytest.mark.parametrize(
