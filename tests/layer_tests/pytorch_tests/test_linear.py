...
class TestMatMul(PytorchLayerTest):
    def _prepare_input(self, m1_shape=(2, 2), m2_shape=(2, 2), bias_shape=None):
        ...
        if bias_shape is None:
            return (
                np.random.randn(*m1_shape).astype(np.float32),
                np.random.randn(*m2_shape).astype(np.float32),
            )
        else:
            return (
                np.random.randn(*m1_shape).astype(np.float32),
                np.random.randn(*m2_shape).astype(np.float32),
                np.random.randn(*bias_shape).astype(np.float32),
            )
    ...
    def test_matmul(self, kwargs_to_prepare_input, ie_device, precision, ir_version):
        self._test(
            *self.create_model(len(kwargs_to_prepare_input) == 3),
            ie_device,
            precision,
            ir_version,
...
class TestLinearBiasList(PytorchLayerTest):
    def _prepare_input(self):
        ...
        return (
            np.random.randn(1, 15, 10).astype(np.float32),
            np.random.randn(66, 10).astype(np.float32),
        )
    ...
    def create_model(self):
        ...
        class aten_mm(torch.nn.Module):
            def __init__(self):
                super(aten_mm, self).__init__()
                self.bias = [torch.randn(22), torch.randn(22), torch.randn(22)]
