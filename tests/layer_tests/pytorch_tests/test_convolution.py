...
class TestConvolution(PytorchLayerTest):
    ...
    def test_convolution1d(
        self, params, bias, underscore, ie_device, precision, ir_version
    ):
        if ie_device == "GPU" and params["dilations"] != [1]:
            pytest.xfail(reason="Unsupported dilations of Convolution on GPU")
        self._test(
            *self.create_model(**params, bias=bias, underscore=underscore),
            ie_device,
            precision,
            ir_version,
            dynamic_shapes=params["groups"] == 1,
