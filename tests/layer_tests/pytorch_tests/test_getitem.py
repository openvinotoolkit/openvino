...
class TestGetItem(PytorchLayerTest):
    ...
    def create_model(self, idx, case="size_with_getitem"):
        ...
        op_cls = {
            ...
            "getitem_with_if": (
                aten_size_get_item_with_if,
                ["aten::size", "aten::__getitem__", "prim::If"],
            ),
        }
...
class TestAddGetItem(PytorchLayerTest):
    ...
    def test_add_cat(self, ie_device, precision, ir_version, idx):
        self._test(
            aten_add_getitem(idx),
            None,
            ["aten::__getitem__", "aten::add", "prim::ListConstruct"],
            ie_device,
            precision,
            ir_version,
            freeze_model=False,
