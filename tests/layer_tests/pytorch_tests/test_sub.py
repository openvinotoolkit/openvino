...
class TestSub(PytorchLayerTest):
    ...
    @pytest.mark.parametrize(
        "input_shapes", [[[2, 3, 4], [2, 3, 4], [1]], [[4, 2, 3], [1, 2, 3], [1]]]
    )
...
class TestSubTypes(PytorchLayerTest):
    ...
    def _prepare_input(self):
        ...
        return (
            torch.randn(self.lhs_shape).to(self.lhs_type).numpy(),
    ...
    def create_model(self, lhs_type, lhs_shape, rhs_type, rhs_shape):
        ...
        class aten_sub(torch.nn.Module):
            ...
            def forward1(self, rhs):
                return torch.sub(
                    torch.tensor(3).to(self.lhs_type), rhs.to(self.rhs_type), alpha=2
                )

            def forward2(self, lhs):
                return torch.sub(
                    lhs.to(self.lhs_type), torch.tensor(3).to(self.rhs_type), alpha=2
                )
