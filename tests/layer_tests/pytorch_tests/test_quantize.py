...
class aten_quantize_per_tensor_aten_dequantize(torch.nn.Module):
    ...
    def forward(self, input_tensor):
        quantized_tensor = torch.quantize_per_tensor(
            input_tensor, self.scale, self.zero_point, self.dtype
        )
...
class aten_quantize_per_channel_aten_dequantize(torch.nn.Module):
    ...
    def forward(self, input_tensor):
        quantized_tensor = torch.quantize_per_channel(
            input_tensor, self.scales, self.zero_points, self.axis, self.dtype
        )
        dequantized_tensor = torch.dequantize(quantized_tensor)
        return dequantized_tensor
...
class TestQuantizePerTensorDequantize(PytorchLayerTest):
    ...
    @pytest.mark.parametrize("scale", [1.0, 0.21, 0.62])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                torch.qint32,
                marks=pytest.mark.skip(reason="Not supported with FakeQuantize."),
            ),
        ],
    )
    @pytest.mark.nightly
    ...
    def test_quantize_per_tensor_dequantize(
        self, scale, zero_point, dtype, ie_device, precision, ir_version
    ):
        if dtype == torch.quint8:
            zero_point = abs(zero_point)
        self._test(
            aten_quantize_per_tensor_aten_dequantize(scale, zero_point, dtype),
            None,
...
class TestQuantizePerChannelDequantize(PytorchLayerTest):
        ...
        [
            ...
            pytest.param(
                ...
                marks=pytest.mark.skip(
                    reason="Axis = 2 not supported in FakeQuantize."
                ),
            ),
            [
                np.array(
                    [1.0, 0.21, 0.62, 0.5, 0.74, 0.11, 0.89, 0.32], dtype=np.float32
                ),
    ...
    def test_quantize_per_channel_dequantize(
        self, scale, zero_point, dtype, axis, ie_device, precision, ir_version
    ):
        np.random.shuffle(scale), np.random.shuffle(zero_point)
        if dtype == torch.quint8:
            zero_point = abs(zero_point)
        self._test(
            aten_quantize_per_channel_aten_dequantize(scale, zero_point, dtype, axis),
            None,
            ["aten::quantize_per_channel", "aten::dequantize"],
            ie_device,
            precision,
            ir_version,
            quantized_ops=True,
