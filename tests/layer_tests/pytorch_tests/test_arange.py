...
class TestArange(PytorchLayerTest):
    ...
    def create_model(self, dtype=None, num_inputs=1, use_out=False, ref_dtype=False):
        ...
        model_classes = {
            ...
            2: (
                ...
                aten_arange_start_end_out,
                aten_arange_start_end_prim_dtype,
            ),
            3: (
                aten_arange_start_end_step_dtype,
                aten_arange_start_end_step_out,
                aten_arange_start_end_step_prim_dtype,
            ),
        }
    ...
    def test_arange_end_only(
        self, dtype, end, use_out, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(dtype, 1, use_out),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.precommit_fx_backend
    @pytest.mark.parametrize(
        "dtype", [None, "float32", "float64", "int32", "int64", "int8"]
    )
    @pytest.mark.parametrize("start,end", [(0, 1), (-1, 1), (1, 5), (0.5, 2.5)])
    def test_arange_start_end(
        self, dtype, end, start, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(dtype, 2),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.parametrize(
        "start,end,step",
        [(0, 1, 1), (-2, 1, 1.25), (1, -5, -1), (1, 10, 2), (-1, -5, -2)],
    )
    def test_arange_start_end_step(
        self, dtype, end, start, step, ie_device, precision, ir_version
    ):
        self._test(
    ...
    @pytest.mark.precommit_fx_backend
    @pytest.mark.parametrize(
        "dtype", ["float32", "float64", "int32", "int64", "int8", "uint8"]
    )
    @pytest.mark.parametrize("end", [1, 2, 3])
    def test_arange_end_only_with_prim_dtype(
        self, dtype, end, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(dtype, 1, False, True),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.parametrize("start,end", [(0, 1), (-1, 1), (1, 5), (0.5, 2.5)])
    def test_arange_start_end_with_prim_dtype(
        self, dtype, end, start, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(dtype, 2, ref_dtype=True),
            ie_device,
            precision,
            ir_version,
    ...
    def test_arange_start_end_step_with_prim_dtype(
        self, dtype, end, start, step, ie_device, precision, ir_version
    ):
        self._test(
