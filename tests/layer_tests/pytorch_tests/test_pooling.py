...
class TestPooling(PytorchLayerTest):
    ...
    def test_avg_pool1d(
        self, params, ceil_mode, count_include_pad, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(
                "avg_pool1d",
        ...
        + [
            pytest.param(
                ...
                marks=pytest.mark.xfail(
                    reason="Sliding windows that would start in the right padded are ignored."
                ),
            )
        ],
    )
    ...
    def test_max_pool1d(
        self, params, ceil_mode, dilation, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(
                "max_pool1d", **params, ceil_mode=ceil_mode, dilation=dilation
            ),
            ie_device,
            precision,
            ir_version,
    ...
    def test_max_pool2d(
        self, params, ceil_mode, dilation, dtype, ie_device, precision, ir_version
    ):
        to_trace = False
        if params["stride"] == []:
            to_trace = True
    ...
    def test_max_pool3d(
        ...
        self._test(
            *self.create_model(
                "max_pool3d", **params, ceil_mode=ceil_mode, dilation=dilation
            ),
            ie_device,
            precision,
            ir_version,
