...
class TestVarMean(PytorchLayerTest):
    ...
    def create_model(
        self, unbiased, dim=None, keepdim=True, two_args_case=True, op_type="var"
    ):
        import torch
        ...
        class aten_var(torch.nn.Module):
            ...
            def forward(self, x):
                return self.op(
                    x, self.dim, unbiased=self.unbiased, keepdim=self.keepdim
                )
    ...
    def test_op2args(self, unbiased, op_type, ie_device, precision, ir_version):
        self._test(
            *self.create_model(unbiased, op_type=op_type),
            ie_device,
            precision,
            ir_version,
        )
    ...
    @pytest.mark.parametrize(
        "dim", [None, 0, 1, 2, 3, -1, -2, (0, 1), (-1, -2), (0, 1, -1), (0, 1, 2, 3)]
    )
    ...
    def test_op(
        self, unbiased, dim, keepdim, op_type, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(
                unbiased, dim, keepdim, two_args_case=False, op_type=op_type
            ),
            ie_device,
            precision,
            ir_version,
