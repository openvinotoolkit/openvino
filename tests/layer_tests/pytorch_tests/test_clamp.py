...
class TestClamp(PytorchLayerTest):
    ...
    def test_clamp(
        self, minimum, maximum, as_tensors, op_type, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(minimum, maximum, as_tensors, op_type),
            ie_device,
            precision,
            ir_version,
        )
...
class TestClampMax(PytorchLayerTest):
    ...
    @pytest.mark.parametrize("maximum", [0.0, 1.0, -1.0, 0.5, 2])
    @pytest.mark.parametrize("as_tensor", [True, False])
    @pytest.mark.nightly
    @pytest.mark.precommit
    @pytest.mark.precommit_torch_export
    ...
    def test_clamp(self, maximum, as_tensor, ie_device, precision, ir_version):
        self._test(
            *self.create_model(maximum, as_tensor),
            ie_device,
            precision,
            ir_version,
            use_convert_model=True,
