...
class quantized_add_relu(torch.nn.Module):
    ...
    def forward(self, input_tensor1, input_tensor2):
        ...
        quantized_add_relu = torch.ops.quantized.add_relu(
            quantized_tensor1, quantized_tensor2, self.scale, self.zero_point
        )
...
class TestQuantizedAddReLU(PytorchLayerTest):
    def _prepare_input(self):
        return (
            np.round(
                np.array(5.00 * np.random.rand(10, 10) - 2.50, dtype=np.float32), 4
            ),
            np.round(
                np.array(5.00 * np.random.rand(10, 10) - 2.50, dtype=np.float32), 4
            ),
    ...
    def test_quantized_add_relu(
        ...
        self._test(
            quantized_add_relu(scale, zero_point, dtype),
            None,
            ["quantized::add_relu"],
            ie_device,
            precision,
