...
class TestMaskedScatter(PytorchLayerTest):
    ...
    def create_model(self, out=False, inplace=False):
        ...
        return (
            aten_masked_scatter(out, inplace),
            ref_net,
            "aten::masked_scatter" if not inplace else "aten::masked_scatter_",
        )

    @pytest.mark.nightly
    @pytest.mark.precommit
    @pytest.mark.parametrize(
        "shape", [[2, 5], [10, 10], [2, 3, 4], [10, 5, 10, 3], [2, 6, 4, 1]]
    )
    @pytest.mark.parametrize(
        "input_dtype", ["float32", "int32", "float", "int", "uint8"]
    )
    @pytest.mark.parametrize("mask_dtype", ["bool"])
    @pytest.mark.parametrize("out", [True, False])
    def test_masked_scatter(
        self, shape, input_dtype, mask_dtype, out, ie_device, precision, ir_version
    ):
        self._test(
    ...
    def test_masked_scatter_inplace(
        self, shape, input_dtype, mask_dtype, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(inplace=True),
            ie_device,
            precision,
            ir_version,
            kwargs_to_prepare_input={
    ...
    @pytest.mark.parametrize("mask_dtype", ["uint8"])
    def test_masked_scatter_inplace_u8(
        self, shape, input_dtype, mask_dtype, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(inplace=True),
