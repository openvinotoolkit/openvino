...
class TestTriuTril(PytorchLayerTest):
    ...
    def create_model(self, op, diagonal):
        ...
        op_map = {"tril": torch.tril, "triu": torch.triu}
    ...
    @pytest.mark.parametrize(
        "dtype", ["float32", "float64", "int32", "int64", "int8", "uint8", "bool"]
    )
    ...
    def test_trilu(
        self, input_shape, dtype, diagonal, op, ie_device, precision, ir_version
    ):
        self._test(
            *self.create_model(op, diagonal),
            ie_device,
            precision,
            ir_version,
