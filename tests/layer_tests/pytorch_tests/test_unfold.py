...
@pytest.mark.parametrize("input_shape", [[2, 2, 5], [3, 3, 3, 3], [2, 3, 4, 5]])
class TestUnfold(PytorchLayerTest):
    ...
    def create_model(self, dimension, size, step):
        class aten_unfold(torch.nn.Module):
            ...
            def forward(self, input_tensor):
                return input_tensor.unfold(
                    dimension=self.dimension, size=self.size, step=self.step
                )
    ...
    def test_unfold(
        self, ie_device, precision, ir_version, dimension, size, step, input_shape
    ):
        ...
        if ie_device == "GPU" and size == 1 and step == 1:
            dyn_shape = False
        self._test(
            *self.create_model(dimension, size, step),
            ie_device,
            precision,
            ir_version,
            dynamic_shapes=dyn_shape
        )
