...
def generate_ir_ovc(coverage=False, **kwargs):
    ...
    if coverage:
        params = [
            ...
            "run",
            "-p",
            "--source={}".format(ovc_runner.parent),
            "--omit=*_test.py",
            ovc_runner,
        ]
    else:
...
class TestOVCTool(unittest.TestCase):
    def setUp(self):
        ...
        self.tmp_dir = tempfile.TemporaryDirectory(
            dir=constants.out_path, prefix=f"{test_name}"
        ).name
    ...
    def test_ovc_tool(self):
        ...
        exit_code, stderr = generate_ir_ovc(
            coverage=False,
            **{
                "input_model": model_path,
                "output_model": self.tmp_dir + os.sep + "model1",
            },
        )
    ...
    def test_ovc_tool_output_dir(self):
        ...
        # tests for MO cli tool
        exit_code, stderr = generate_ir_ovc(
            coverage=False, **{"input_model": model_path, "output_model": self.tmp_dir}
        )
        assert not exit_code
    ...
    def test_ovc_tool_saved_model_dir(self):
        ...
        exit_code, stderr = generate_ir_ovc(
            coverage=False, **{"input_model": model_dir, "output_model": self.tmp_dir}
        )
        assert not exit_code
    ...
    def test_ovc_tool_saved_model_dir_with_sep_at_path_end(self):
        ...
        exit_code, stderr = generate_ir_ovc(
            coverage=False,
            **{"input_model": model_dir + os.sep, "output_model": self.tmp_dir},
        )
        assert not exit_code
    ...
    def test_ovc_tool_non_existng_output_dir(self):
        ...
        exit_code, stderr = generate_ir_ovc(
            coverage=False,
            **{
                "input_model": model_dir + os.sep,
                "output_model": self.tmp_dir + os.sep + "dir" + os.sep,
            },
        )
        assert not exit_code
    ...
    def test_ovc_tool_verbose(self):
        ...
        exit_code, stderr = generate_ir_ovc(
            coverage=False,
            **{"input_model": model_dir, "output_model": self.tmp_dir, "verbose": ""},
        )
        assert not exit_code
