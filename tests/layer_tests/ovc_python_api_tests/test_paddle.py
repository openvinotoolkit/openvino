    ...
    class NeuralNetwork(paddle.nn.Layer):
        def __init__(self):
            ...
            self.relu_sigmoid_stack = paddle.nn.Sequential(
                paddle.nn.ReLU(), paddle.nn.Sigmoid()
            )
...
def make_pd_hapi_graph_model(shape):
    ...
    net = paddle.nn.Sequential(paddle.nn.ReLU(), paddle.nn.Sigmoid())
...
class TestMoConvertPaddle(CommonMOConvertTest):
    ...
    def test_mo_import_from_memory_paddle_fe(
...
class TestPaddleConversionParams(CommonMOConvertTest):
    ...
    test_data = (
        [
            {
                "params_test": {
                    "input": paddle.shape(paddle.to_tensor(np.random.rand(2, 3, 4)))
                },
            ...
            },
            {
                "params_test": {
                    "input": (
                        paddle.to_tensor(np.random.rand(4, 2, 7)).shape,
                        paddle.int32,
                    )
                },
