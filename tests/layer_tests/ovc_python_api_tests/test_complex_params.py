...
class TestComplexParams(CommonMOConvertTest):
    ...
    test_data = [
        ...
        {
            "params_test": {
                "input": [([1, 3, -1, -1], ov.Type.i32), ov.Type.i32, ov.Type.i32]
            },
            "params_ref": {
                "input": "Input1:0[1,3,?,?]{i32},Input2:0{i32},Input3:0{i32}"
            },
        },
        {
            "params_test": {
    ...
    test_data = [
        ...
        {"params_test": {"input": (3, 2)}, "params_ref": {"input": "Input:0[3,2]"}},
        {
        ...
        {"params_test": {"input": (-1, 10)}, "params_ref": {"input": "Input:0[?,10]"}},
        {
            "params_test": {"input": PartialShape([-1, 10])},
            "params_ref": {"input": "Input:0[?,10]"},
        },
        {"params_test": {"input": np.int32}, "params_ref": {"input": "Input:0{i32}"}},
        {
    ...
    def test_mo_convert_tf_model_single_input_output(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
    ...
    def create_onnx_model_with_comma_in_names(temp_dir):
        ...
        input_1 = helper.make_tensor_value_info(
            "input_1", TensorProto.FLOAT, [1, 3, 2, 2]
        )
        input_2 = helper.make_tensor_value_info(
            "input_2", TensorProto.FLOAT, [1, 3, 2, 2]
        )
        output = helper.make_tensor_value_info(
            "relu_1,relu_2", TensorProto.FLOAT, [1, 3, 4, 2]
        )

        node_def_1 = onnx.helper.make_node(
            "Relu", inputs=["input_1"], outputs=["Relu_1_data"], name="relu_1"
        )
        node_def_2 = onnx.helper.make_node(
            "Relu", inputs=["input_2"], outputs=["Relu_2_data"], name="relu_2"
        )
    ...
    def create_ref_graph_with_comma_in_names():
        ...
        from openvino.runtime.opset12 import concat, relu
    ...
    def create_onnx_model_with_several_outputs(temp_dir):
        ...
        concat_output = helper.make_tensor_value_info(
            "concat", TensorProto.FLOAT, shape
        )
        relu_output = helper.make_tensor_value_info(
            "Relu_1_data", TensorProto.FLOAT, shape
        )
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_ovc_convert_model_with_comma_in_names(
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_ovc_convert_model_with_several_output(
        self, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        onnx_net_path = self.create_onnx_model_with_several_outputs(temp_dir)
        convert_model_params = {
            "input_model": onnx_net_path,
            "output": ["Relu_1_data", "concat"],
        }
    ...
    def test_non_numpy_types(
        ...
        self._test_by_ref_graph(
            temp_dir,
            {"input_model": model, "input": [ov.Type.bf16, tf.bfloat16]},
            bf16_ref,
            compare_tensor_names=False,
        )
        self._test_by_ref_graph(
            temp_dir,
            {"input_model": model, "input": {"a": ov.Type.bf16, "b": tf.bfloat16}},
            bf16_ref,
            compare_tensor_names=False,
        )
        self._test_by_ref_graph(
            temp_dir,
            {"input_model": model, "input": [ov.Type.string, tf.string]},
            string_ref,
            compare_tensor_names=False,
        )
        self._test_by_ref_graph(
            temp_dir,
            {"input_model": model, "input": {"a": ov.Type.string, "b": tf.string}},
            string_ref,
            compare_tensor_names=False,
        )
...
class NegativeCases(unittest.TestCase):
    ...
    def test_input_output_cut_exceptions(self):
        ...
        with tempfile.TemporaryDirectory(dir=self.test_directory) as temp_dir:
            tf_net_path = TestComplexParams.create_tf_model_single_input_output(
                temp_dir
            )

            with self.assertRaisesRegex(
                Exception, ".*Name Relu is not found among model inputs.*"
            ):
                convert_model(tf_net_path, input="Relu")
            with self.assertRaisesRegex(
                Exception, ".*Name Relu is not found among model outputs.*"
            ):
            ...
            with self.assertRaisesRegex(
                Exception, ".*Name Relu2 is not found among model inputs.*"
            ):
                convert_model(tf_net_path, input="Relu2")
            with self.assertRaisesRegex(
                Exception, ".*Name Relu2 is not found among model outputs.*"
            ):
