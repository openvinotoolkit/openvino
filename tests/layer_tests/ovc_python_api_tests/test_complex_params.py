...
class TestComplexParams(CommonMOConvertTest):
    ...
    test_data = [
        {'params_test': {'output': ["Sigmoid_0:0", "Sigmoid_2:0"]},
         'params_ref': {'output': "Sigmoid_0,Sigmoid_2"}, 'id':0},
        {'params_test': {'output': ["Sigmoid_0:0"]},
         'params_ref': {'output': "Sigmoid_0"},'id':1},
        {'params_test': {'input': [PartialShape([2, 3, 4]), [2, 3, 4], [Dimension(2), Dimension(3), Dimension(4)]]},
         'params_ref': {'input_shape': "[2,3,4],[2,3,4],[2,3,4]", 'input': 'Input1:0,Input2:0,Input3:0'},'id':2},
        {'params_test': {'input': [PartialShape([1, 3, -1, -1]), [1, 3, -1, -1]]},
         'params_ref': {'input_shape': "[1,3,?,?],[1,3,?,?]", 'input': 'Input1:0,Input2:0'} , 'id':3 },
        {'params_test': {'input': [(2, 3, 4), [2, 3, 4], (Dimension(2), Dimension(3), Dimension(4))]},
         'params_ref': {'input_shape': "[2,3,4],[2,3,4],[2,3,4]", 'input': 'Input1:0,Input2:0,Input3:0'}, 'id':4},
        {'params_test': {'input': {"Input1:0": PartialShape([2, 3, 4]), "Input2:0": [2, 3, 4],
                                   "Input3:0": [Dimension(2), Dimension(3), Dimension(4)]}},
         'params_ref': {'input_shape': "[2,3,4],[2,3,4],[2,3,4]", 'input': 'Input1:0,Input2:0,Input3:0'},'id':5},
        {'params_test': {'input': {"Input2:0": [1, -1, -1, -1],
                                   "Input3:0": [Dimension(1), Dimension(-1), Dimension(-1), Dimension(-1)]}},
         'params_ref': {'input_shape': "[1,?,?,?],[1,?,?,?]", 'input': 'Input2:0,Input3:0'},'id':6},
        {'params_test': {'input': [np.int32, Type(np.int32), np.int32]},
         'params_ref': {'input': 'Input1:0{i32},Input2:0{i32},Input3:0{i32}'}, 'id':7},
        {'params_test': {'input': [ov.Type.f32, ov.Type.f32]},
         'params_ref': {'input': 'Input1:0{f32},Input2:0{f32}'},'id':8},
        {'params_test': {'input': [([1, 3, -1, -1], ov.Type.i32), ov.Type.i32, ov.Type.i32]},
         'params_ref': {'input': 'Input1:0[1,3,?,?]{i32},Input2:0{i32},Input3:0{i32}'},'id':9},
        {'params_test': {'input': (PartialShape([2, 3, 4]), [2, 3, 4], [Dimension(2), Dimension(3), Dimension(4)])},
         'params_ref': {'input_shape': "[2,3,4],[2,3,4],[2,3,4]", 'input': 'Input1:0,Input2:0,Input3:0'},'id':10},
        {'params_test': {'input': (PartialShape([1, 3, -1, -1]), [1, 3, -1, -1])},
         'params_ref': {'input_shape': "[1,3,?,?],[1,3,?,?]", 'input': 'Input1:0,Input2:0'},'id':11},
        {'params_test': {'input': ((2, 3, 4), [2, 3, 4], (Dimension(2), Dimension(3), Dimension(4)))},
         'params_ref': {'input_shape': "[2,3,4],[2,3,4],[2,3,4]", 'input': 'Input1:0,Input2:0,Input3:0'},'id':12},
        {'params_test': {'input': (np.int32, Type(np.int32), np.int32)},
         'params_ref': {'input': 'Input1:0{i32},Input2:0{i32},Input3:0{i32}'},'id':13},
        {'params_test': {'input': (ov.Type.f32, ov.Type.f32)},
         'params_ref': {'input': 'Input1:0{f32},Input2:0{f32}'},'id':14},
        {'params_test': {'input': (([1, 3, -1, -1], ov.Type.i32), ov.Type.i32, ov.Type.i32)},
         'params_ref': {'input': 'Input1:0[1,3,?,?]{i32},Input2:0{i32},Input3:0{i32}'},'id':15}
    ]

    @pytest.mark.parametrize("params", sorted(test_data,key=lambda x: x['id']))
    @pytest.mark.nightly
    ...
    test_data = [
        {'params_test': {'input': {"Input:0": ([3, 2], ov.Type.i32)}},
         'params_ref': {'input': "Input:0[3,2]{i32}"},'id':0},
        {'params_test': {'input': {"Input:0": ov.Type.i32}},
         'params_ref': {'input': "Input:0{i32}"},'id':1},
        {'params_test': {'input': {"Input:0": [3, 2]}},
         'params_ref': {'input': "Input:0[3,2]"},'id':2},
        {'params_test': {'input': (3, 2)},
         'params_ref': {'input': "Input:0[3,2]"},'id':3},
        {'params_test': {'input': (3, Dimension(2))},
         'params_ref': {'input': "Input:0[3,2]"},'id':4},
        {'params_test': {'input': [3, 2]},
         'params_ref': {'input': "Input:0[3 2]"},'id':5},
        {'params_test': {'input': [Dimension(3, 10), 2]},
         'params_ref': {'input': "Input:0[3..10 2]"},'id':6},
        {'params_test': {'input': (-1, 10)},
         'params_ref': {'input': "Input:0[?,10]"},'id':7},
        {'params_test': {'input': PartialShape([-1, 10])},
         'params_ref': {'input': "Input:0[?,10]"},'id':8},
        {'params_test': {'input': np.int32},
         'params_ref': {'input': "Input:0{i32}"},'id':9},
        {'params_test': {'input': (np.int32, [1, 2, 3])},
         'params_ref': {'input': "Input:0[1,2,3]{i32}"},'id':10},
        {'params_test': {'input': [Dimension(3, 10), 10, -1]},
         'params_ref': {'input': 'Input:0[3..10,10,?]'},'id':11},
    ]
