...
from openvino.runtime import Dimension, Model, PartialShape, Type
...
def create_pytorch_nn_module_with_scalar_input(tmp_dir):
    ...
    return (
        ...
        ref_model,
        {
            "input": [PartialShape("[]"), PartialShape([-1, 3, -1, -1])],
            "example_input": sample_input,
        },
    )
...
def create_pytorch_nn_module_case3(tmp_dir):
    ...
    return (
        pt_model,
        ref_model,
        {"input": [[-1, 3, -1, -1], [-1, 3, -1, -1]], "example_input": sample_input},
    )
...
def create_pytorch_module_with_nested_list_and_single_input(tmp_dir):
    ...
    return (
        net,
        ref_model,
        {"example_input": [torch.ones((1, 11))], "compress_to_fp16": False},
    )
...
class ConvertRaises(unittest.TestCase):
    def test_example_inputs(self):
        ...
        with self.assertRaisesRegex(
            TypeError, ".*got an unexpected keyword argument 'example_inputs'.*"
        ):
    ...
    def test_incorrect_inputs_3(self):
        ...
        # check that it accepts specified names as is, without parsing into 2 different inputs
        with self.assertRaisesRegex(
            Exception, "No node with name input1\[1, 10\],input2\[2, 100\]"
        ):
...
def make_ref_model_three_inputs(shape, dtype=np.float32):
    x = ov.opset8.parameter(PartialShape(shape), name="x", dtype=dtype)
