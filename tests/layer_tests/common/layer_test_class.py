...
class CommonLayerTest:
    ...
    def produce_model_path(self, framework_model, save_path):
        raise RuntimeError(
            "This is base class, please implement produce_model_path function for"
            " the specific framework"
        )

    def get_framework_results(self, inputs_dict, model_path):
        raise RuntimeError(
            "This is base class, please implement get_framework_results function for"
            " the specific framework"
        )
    ...
    def _test(
        ...
        **kwargs
        ...
        """
        model_path = self.produce_model_path(
            framework_model=framework_model, save_path=temp_dir
        )
        ...
        assert not exit_code, "IR generation failed with {} exit code: {}".format(
            exit_code, stderr
        )
        ...
        if "kwargs_to_prepare_input" in kwargs and kwargs["kwargs_to_prepare_input"]:
            inputs_dict = self._prepare_input(
                ie_engine.get_inputs_info(precision), kwargs["kwargs_to_prepare_input"]
            )
        else:
            inputs_dict = self._prepare_input(ie_engine.get_inputs_info(precision))

        # OV infer:
        infer_res = ie_engine.infer(
            input_data=inputs_dict, infer_timeout=infer_timeout, config=config
        )
        ...
        # Framework infer:
        fw_res = self.get_framework_results(
            inputs_dict=inputs_dict, model_path=model_path
        )
        ...
        assert self.compare_ie_results_with_framework(
            infer_res=infer_res, framework_res=fw_res, framework_eps=custom_eps
        ), "Comparing with Framework failed: ie_res={}; framework_res={}.".format(
            infer_res, fw_res
        )
            ...
            if "input_names" in kwargs:
                ...
                for i, input_name in enumerate(input_names):
                    assert inputs_ie[i].attrib["name"] == input_name, (
                        ...
                        "but expected {}".format(
                            i, inputs_ie[i].attrib["name"], input_name
                        )
                    )
    ...
    def _prepare_input(self, inputs_dict):
        for input in inputs_dict.keys():
            inputs_dict[input] = np.random.randint(-10, 10, inputs_dict[input]).astype(
                np.float32
            )
        return inputs_dict

    def compare_ie_results_with_framework(
        self, infer_res, framework_res, framework_eps
    ):
        is_ok = True
        ...
        for framework_out_name in framework_res:
            ...
            if not allclose(
                ie_res,
                framework_res[framework_out_name],
                atol=framework_eps,
                rtol=framework_eps,
            ):
                is_ok = False
                ...
                print(
                    "absolute eps: {}, relative eps: {}".format(
                        framework_eps, framework_eps
                    )
                )
        return is_ok
...
def check_ir_version(left, right, ir_version):
    try:
        ...
        raise RuntimeError(
            "Wrong ir version type: {}, must be an integer".format(ir_version)
        )
