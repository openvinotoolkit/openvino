...
class CommonLayerTest:
    ...
    def _test(self, framework_model, ref_net, ie_device, precision, ir_version, temp_dir, timeout=60,
        ...
        ie_engine = IEInfer(model=path_to_xml,
                            weights=path_to_bin,
                            device=ie_device)
        infer_res = ie_engine.infer(input_data=inputs_dict, timeout=timeout)
