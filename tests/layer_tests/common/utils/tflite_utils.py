...
def make_positive_array(inputs_dict):
    for input in inputs_dict.keys():
        inputs_dict[input] = np.random.randint(1, 10, inputs_dict[input]).astype(
            np.float32
        )
    return inputs_dict
...
def short_range(inputs_dict):
    for input in inputs_dict.keys():
        inputs_dict[input] = np.random.randint(-1, 1, inputs_dict[input]).astype(
            np.float32
        )
    return inputs_dict
...
additional_test_params = [
    ...
    [
        ...
        {
            "activation": lambda x, name: tf.math.minimum(
                tf.math.maximum(-1.0, x), 1.0, name=name
            )
        },
    ],
]
...
def get_tflite_results(use_legacy_frontend, inputs_dict, model_path):
    ...
    for layer, data in inputs_dict.items():
        ...
        if list(input_details[tensor_id]["shape"]) != list(data.shape):
            warnings.warn(
                f"Model and data have different shapes:\nModel {tensor_id} "
                f'input shape{input_details[tensor_id]["shape"]}\nInput data shape: {data.shape}'
            )
            interpreter.resize_tensor_input(
                input_details[tensor_id]["index"], data.shape
            )
            interpreter.allocate_tensors()
