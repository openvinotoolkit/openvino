...
def load_graph(model_file, output_nodes_for_freeze=None):
    ...
    nodes_to_clear_device = (
        graph_def.node
        if isinstance(graph_def, tf.compat.v1.GraphDef)
    ...
    if is_meta:
        with tf.compat.v1.Session() as sess:
            ...
            graph_def = tf.compat.v1.graph_util.convert_variables_to_constants(
                sess, graph_def.graph_def, output_nodes_for_freeze
            )
...
def collect_tf_references(
    model_path, feed_dict, out_layer, output_nodes_for_freeze=None
):
    _feed_dict = dict()
    ...
    for input in feed_dict:
        input_node = [node for node in graph.as_graph_def().node if node.name == input][
            0
        ]
...
def summarize_graph(model_path, output_nodes_for_freeze=None, reshape_net=None):
    ...
    for node in graph.as_graph_def().node:
        if node.op == "Placeholder":
            ...
            node_dict["shape"] = tuple(
                map(lambda x: int(x) if x else 0, node_dict["shape"].split(","))
            )
            placeholders[node.name] = node_dict
        if node.op == "Variable" or node.op == "VariableV2":
            variables.append(node.name)
        if (
            len(children(node.name, graph)) == 0
            and node.name not in control_dependents_map
        ):
            if (
                node.op not in unlikely_output_types
                and node.name.split("/")[-1] not in unlikely_output_types
            ):
    ...
    if reshape_net:
        ...
        scoring_res = collect_tf_references(
            model_path=model_path, feed_dict=feed_dict, out_layer=out_layer
        )
...
def save_to_pb(tf_model, path_to_saved_tf_model, model_name="model.pb"):
    ...
    assert os.path.isfile(
        os.path.join(path_to_saved_tf_model, model_name)
    ), "model.pb haven't been saved " "here: {}".format(path_to_saved_tf_model)
