...
class TestConvolution(CommonMXNetLayerTest):
    def create_net(self, shape, output_shape, conv_params, ir_version):
        ...
        layer_ptr = mx.symbol.Convolution(data=data, name=layer_name, **conv_params)
        ...
        if check_ir_version(10, None, ir_version):
            ...
            if not conv_params['no_bias']:
                ...
                edges.extend([('conv_b', 'conv_b_op'), ('conv_b_op', 'conv_b_op_data'),
                              ('conv_op_data', 'add'), ('conv_b_op_data', 'add'), ('add', 'add_data'),
                              ('add_data', 'result')])
            else:
    ...
    test_data = [dict(shape=[1, 3, 120, 120], output_shape=[1, 32, 120, 120],
                      conv_params={'kernel': [3, 3], 'stride': [1, 1], 'pad': [0, 0],
                                   'dilate': [0, 0], 'num_filter': 32, 'no_bias': False}),
                 dict(shape=[1, 3, 120, 120], output_shape=[1, 32, 122, 124],
                      conv_params={'kernel': [3, 3], 'stride': [1, 1], 'pad': [1, 2],
                                   'dilate': [0, 0], 'num_filter': 32, 'no_bias': False}),
                 dict(shape=[1, 3, 120, 120], output_shape=[1, 32, 122, 124],
                      conv_params={'kernel': [3, 3], 'stride': [1, 1], 'pad': [1, 2],
                                   'dilate': [0, 0], 'num_filter': 32, 'no_bias': True}),
                 dict(shape=[1, 96, 27, 27], output_shape=[1, 256, 27, 27],
    ...
    def test_conv(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_net(**params, ir_version=ir_version), ie_device, precision,
                   input_shapes=[params['shape']], input_names=['data'],
