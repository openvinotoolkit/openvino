...
class TestPad(CommonMXNetLayerTest):
    def create_net(self, shape, output_shape, ir_version, **kwargs):
        ...
        layer_ptr = mx.symbol.pad(data=data, mode=kwargs['mode'], pad_width=kwargs['pad_width'])
        ...
        if check_ir_version(10, None, ir_version):
            nodes_attributes = {
                ...
                'node': {'kind': 'op', 'type': 'Pad', 'pad_mode': kwargs['mode']},
    ...
    test_data = [dict(shape=[1, 512, 38, 38], output_shape=[1, 512, 40, 40], pad_width=(0, 0, 0, 0, 1, 1, 1, 1),
                      mode='edge', pads_begin=[0, 0, 1, 1], pads_end=[0, 0, 1, 1], pad_value=0)]

    @pytest.mark.parametrize("params", test_data)
    @pytest.mark.nightly
    def test_pad(self, params, ie_device, precision, ir_version, temp_dir):
