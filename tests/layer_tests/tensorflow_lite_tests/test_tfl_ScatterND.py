...
class TestTFLiteScatterNDLayerTest(TFLiteLayerTest):
    ...
    def _prepare_input(self, inputs_dict, generator=None):
        inputs_dict["Indices"] = np.array(self.indices_value, dtype=np.int32).reshape(
            self.indices_shape
        )

        if self.updates_dtype in [tf.int32, tf.int64]:
            inputs_dict["Updates"] = np.random.randint(
                -100, 100 + 1, self.updates_shape
            )
        if self.updates_dtype == tf.float32:
            inputs_dict["Updates"] = (
                100 - (-100) * np.random.random_sample(self.updates_shape) + (-100)
            )
        if self.updates_dtype == tf.bool:
            inputs_dict["Updates"] = np.random.choice(
                [True, False], size=self.updates_shape
            )
    ...
    def make_model(self, params):
        ...
        with tf.compat.v1.Session() as sess:
            indices = tf.compat.v1.placeholder(
                tf.int32, self.indices_shape, name=self.inputs[0]
            )
            updates = tf.compat.v1.placeholder(
                self.updates_dtype, self.updates_shape, name=self.inputs[1]
            )
            shape = tf.compat.v1.placeholder(
                tf.int32, params["shape_shape"], name=self.inputs[2]
            )
