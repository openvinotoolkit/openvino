...
class TestTFLiteFullyConnectedLayerTest(TFLiteLayerTest):
    ...
    def make_model(self, params):
        assert (
            len(set(params.keys()).intersection({"shape_x", "shape_y"})) == 2
        ), "Unexpected parameters for test: " + ",".join(params.keys())

        tf.compat.v1.reset_default_graph()
        with tf.compat.v1.Session() as sess:
            x = tf.compat.v1.placeholder(
                params.get("dtype", tf.float32), params["shape_x"], name=self.inputs[0]
            )
            y = tf.compat.v1.placeholder(
                params.get("dtype", tf.float32), params["shape_y"], name=self.inputs[1]
            )
