...
class TestTFLiteSliceLayerTest(TFLiteLayerTest):
    ...
    def _prepare_input(self, inputs_dict, generator=None):
        if self.input_dtype == tf.string:
            ...
            inputs_dict["Input"] = np.random.choice(letters, size=self.shape).astype(
                self.input_dtype
            )
        else:
            inputs_dict["Input"] = np.random.randint(-1, 2, self.shape).astype(
                self.input_dtype
            )
    ...
    def make_model(self, params):
        assert (
            len(set(params.keys()).intersection({"shape", "begin", "size", "dtype"}))
            == 4
        ...
        with tf.compat.v1.Session() as sess:
            placeholder = tf.compat.v1.placeholder(
                params["dtype"], params["shape"], self.inputs[0]
            )
            begin = tf.compat.v1.placeholder(
                tf.int32, len(params["shape"]), name=self.inputs[1]
            )
            size = tf.compat.v1.placeholder(
                tf.int32, len(params["shape"]), name=self.inputs[2]
            )
