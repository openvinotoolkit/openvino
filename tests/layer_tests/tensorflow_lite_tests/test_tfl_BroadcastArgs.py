...
class TestTFLiteBroadcastArgsLayerTest(TFLiteLayerTest):
    ...
    def make_model(self, params):
        ...
        with tf.compat.v1.Session() as sess:
            s0 = tf.compat.v1.placeholder(
                params.get("dtype", tf.int32), [len(self.shape)], name=self.inputs[0]
            )
            s1 = tf.compat.v1.placeholder(
                params.get("dtype", tf.int32), [len(self.shape)], name=self.inputs[1]
            )
