...
class TestTFLiteStableHLOCompositeRmsNorm:
    ...
    The frontend emits the canonical RMSNorm decomposition (Power/ReduceMean/
    Add/Sqrt/Multiply) via ov::decomposition::rms_norm; ov::pass::RMSFusion
    fuses it back into ov::op::internal::RMS during plugin compilation, so
    the post-read_model graph still contains the raw primitives.
    """
    ...
    def test_stablehlo_composite_rms_norm(self, params, ie_device, precision, temp_dir):
        ...
        # The frontend emits the math-only decomposition; RMSFusion runs at
        # plugin compilation time, so the post-read_model graph must contain
        # the raw primitives, not an RMS op.
        op_types = [op.get_type_name() for op in ov_model.get_ordered_ops()]
        for required in ("ReduceMean", "Sqrt", "Multiply"):
