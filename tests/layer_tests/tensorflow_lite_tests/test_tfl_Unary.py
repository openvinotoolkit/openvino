...
test_ops = [
    ...
    {
        "op_name": "RSQRT",
        "op_func": tf.math.rsqrt,
        "kwargs_to_prepare_input": "positive",
    },
    {"op_name": "SIN", "op_func": tf.math.sin},
    {
        "op_name": "SOFTMAX",
        "op_func": partial(tf.nn.softmax, axis=-1),
    },  # additionally test with alpha
...
class TestTFLiteUnaryLayerTest(TFLiteLayerTest):
    ...
    def make_model(self, params):
        assert (
            len(set(params.keys()).intersection({"op_name", "op_func", "shape"})) == 3
