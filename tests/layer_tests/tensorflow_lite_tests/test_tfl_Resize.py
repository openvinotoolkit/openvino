...
test_ops = [
    ...
    {
        "op_name": ["RESIZE_NEAREST_NEIGHBOR"],
        "op_func": tf.compat.v1.image.resize_nearest_neighbor,
    },
]
...
class TestTFLiteResizeLayerTest(TFLiteLayerTest):
    ...
    def test_resize_resize(self, params, ie_device, precision, temp_dir):
        if (
            not params["align_corners"]
            and params["half_pixel_centers"]
            and params["op_name"] == ["RESIZE_NEAREST_NEIGHBOR"]
        ):
