...
test_ops = [
    ...
    {
        "op_name": "DIV",
        "op_func": tf.math.divide,
        "kwargs_to_prepare_input": "positive",
    },
    {"op_name": "MUL", "op_func": tf.math.multiply},
    {"op_name": "SUB", "op_func": tf.math.subtract},
]

test_params = [{"shape": [2, 10, 10, 3]}, {"shape": [2, 10]}]
...
class TestTFLiteBinaryWithActivationLayerTest(TFLiteLayerTest):
    ...
    def make_model(self, params):
        assert (
            len(
                set(params.keys()).intersection(
                    {"op_name", "op_func", "shape", "activation"}
                )
            )
            == 4
        ...
        with tf.compat.v1.Session() as sess:
            in0 = tf.compat.v1.placeholder(
                params.get("dtype", tf.float32), params["shape"], name=self.inputs[0]
            )
            in1 = tf.compat.v1.placeholder(
                params.get("dtype", tf.float32), params["shape"], name=self.inputs[1]
            )
            bin_op_name = (
                self.outputs[0] if not params["activation"] else self.outputs[0] + "/op"
            )
