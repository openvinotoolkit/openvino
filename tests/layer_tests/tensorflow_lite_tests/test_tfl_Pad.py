...
class TestTFLitePadLayerTest(TFLiteLayerTest):
    ...
    def make_model(self, params):
        assert (
            len(set(params.keys()).intersection({"op_name", "shape", "paddings"})) == 3
        ...
        with tf.compat.v1.Session() as sess:
            place_holder = tf.compat.v1.placeholder(
                tf.float32, params["shape"], name=self.inputs[0]
            )
            if params["op_name"] == "PADV2":
                ...
                paddings = tf.compat.v1.placeholder(
                    dtype=tf.int32, name=self.inputs[1], shape=shape
                )
