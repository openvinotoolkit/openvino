...
test_params = [
    {
        "shapes": ((None, 4, 5), (None, 5, 6), (3, 4, 5), (3, 5, 6)),
        "adjoint_a": True,
        "adjoint_b": True,
    },
    {
        "shapes": ((None, 1, 3, 4), (None, 4, 2), (2, 1, 3, 4), (5, 4, 2)),
        "adjoint_a": False,
        "adjoint_b": False,
    },
    {
...
class TestTFLiteBatchMatmulLayerTest(TFLiteLayerTest):
    ...
    def _prepare_input(self, inputs_dict, generator=None):
        ...
        inputs_dict["Input"] = np.float32(
            (1.0 - (-1.0)) * np.random.random_sample(input0_shape) + (-1.0)
        )

        input1_shape = (
            self.shapes[3] if not adj_b else self._swap_last_two_dims(*self.shapes[3])
        )
        inputs_dict["Input1"] = np.float32(
            (1.0 - (-1.0)) * np.random.random_sample(input1_shape) + (-1.0)
        )
    ...
    def make_model(self, params):
        assert (
            len(set(params.keys()).intersection({"shapes", "adjoint_a", "adjoint_b"}))
            == 3
        ...
        with tf.compat.v1.Session() as sess:
            ...
            input0_tensor = tf.compat.v1.placeholder(
                dtype=tf.float32, shape=placeholder0_shape, name=self.inputs[0]
            )
            if adj_b:
            ...
            input1_tensor = tf.compat.v1.placeholder(
                dtype=tf.float32, shape=placeholder1_shape, name=self.inputs[1]
            )
