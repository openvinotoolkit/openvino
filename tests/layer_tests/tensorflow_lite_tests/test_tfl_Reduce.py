...
from common.utils.tflite_utils import additional_test_params, parametrize_tests

test_ops = [
    {"op_name": "MEAN", "op_func": tf.math.reduce_mean},
    {
        "op_name": "REDUCE_ALL",
        "op_func": tf.math.reduce_all,
        "kwargs_to_prepare_input": "boolean",
        "dtype": tf.bool,
    },
    {
        "op_name": "REDUCE_ANY",
        "op_func": tf.math.reduce_any,
        "kwargs_to_prepare_input": "boolean",
        "dtype": tf.bool,
    },
    ...
    {
        "op_name": "REDUCE_PROD",
        "op_func": tf.math.reduce_prod,
        "kwargs_to_prepare_input": "short_range",
    },
...
class TestTFLiteReduceLayerTest(TFLiteLayerTest):
    ...
    def make_model(self, params):
        assert (
            len(
                set(params.keys()).intersection({"op_name", "op_func", "shape", "axis"})
            )
            == 4
