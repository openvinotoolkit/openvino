...
class LegacyExtTest(unittest.TestCase):
    ...
    def test_legacy_extensions(self):
        ...
        with tempfile.TemporaryDirectory(dir=self.test_directory) as tmpdir:
            ext_path1 = os.path.join(
                os.path.dirname(__file__), "test_legacy_exts/test_exts_dir1"
            )
            ext_path2 = os.path.join(
                os.path.dirname(__file__), "test_legacy_exts/test_exts_dir2"
            )
            ...
            exit_code, stderr = generate_ir(
                coverage=False,
                **{"input_model": model, "extensions": ext_path1, "output_dir": tmpdir}
            )
