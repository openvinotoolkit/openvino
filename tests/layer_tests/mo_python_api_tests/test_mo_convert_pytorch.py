...
from openvino.runtime import Core, Dimension, Model, PartialShape, Type, save_model
...
def make_ref_pt_model_two_inputs(shape, dtype=np.float32):
    if len(shape) == 2:
        param1 = ov.opset8.parameter(
            PartialShape(shape[0]), name="input_0", dtype=dtype
        )
        param2 = ov.opset8.parameter(
            PartialShape(shape[1]), name="input_1", dtype=dtype
        )
    else:
        shape = PartialShape(shape)
        param1 = ov.opset8.parameter(shape, name="input_0", dtype=dtype)
...
def make_ref_pt_model_with_optional_inputs(shape, dtype=np.float32, z_exist=False):
    ...
    op = (
        ov.opset8.multiply(param1, param2)
        if not z_exist
        else ov.opset8.add(param1, param2)
    )
...
def create_pytorch_nn_module_case2(tmp_dir):
    ...
    return (
        ...
        ref_model,
        {
            "input": [PartialShape("[?,3,?,?]"), PartialShape([-1, 3, -1, -1])],
...
def create_pytorch_nn_module_with_scalar_input(tmp_dir):
    ...
    return (
        ...
        ref_model,
        {
            "input_shape": [PartialShape("[]"), PartialShape([-1, 3, -1, -1])],
            "example_input": sample_input,
        },
    )
...
def create_pytorch_nn_module_case3(tmp_dir):
    ...
    return (
        pt_model,
        ref_model,
        {"input_shape": "[?,3,?,?],[?,3,?,?]", "example_input": sample_input},
    )
...
def create_pytorch_nn_module_case5(tmp_dir):
    ...
    return (
        pt_model,
        ref_model,
        {"example_input": sample_input, "input": (inp_shape, np.float32)},
    )
...
def create_pytorch_nn_module_case7(tmp_dir):
    ...
    return (
        pt_model,
        ref_model,
        {"example_input": sample_input, "input": ([1, 3, 20, 20], np.int32)},
    )
...
def create_pytorch_nn_module_sample_input_int32(tmp_dir):
    ...
    return (
        pt_model,
        ref_model,
        {"example_input": sample_input, "input": (shape, np.int32)},
    )
...
def create_pytorch_nn_module_sample_input_int32_two_inputs(tmp_dir):
    ...
    return (
        ...
        ref_model,
        {
            "input": [(np.int32, inp_shapes[0]), (np.int32, inp_shapes[1])],
            "example_input": sample_input,
        },
    )
...
def create_pytorch_jit_script_module(tmp_dir):
    ...
    return (
        scripted_model,
        model_ref,
        {"input": [([1, 3, 5, 5], np.float32), ([1, 3, 5, 5], np.float32)]},
    )
...
def create_pytorch_jit_script_function(tmp_dir):
    ...
    return (
        scripted_fn,
        ref_model,
        {"input": [(inp_shape, Type.f32), (inp_shape, Type.f32)]},
    )
...
def create_pytorch_nn_module_layout_list(tmp_dir):
    ...
    return (
        ...
        ref_model,
        {
            "input": [(shape, np.float32), (shape, np.float32)],
...
def create_pytorch_nn_module_shapes_list_static(tmp_dir):
    ...
    return (
        pt_model,
        ref_model,
        {"input": [([1, 3, 20, 20], Type.f32), ([1, 3, 20, 20], Type.f32)]},
    )
...
def create_pytorch_nn_module_shapes_list_static_via_input(tmp_dir):
    ...
    return (
        pt_model,
        ref_model,
        {"input": [([1, 3, 20, 20], np.float32), ([1, 3, 20, 20], np.float32)]},
    )
...
def create_pytorch_nn_module_shapes_list_dynamic(tmp_dir):
    pt_model = make_pt_model_two_inputs()
    inp_shapes = [
        [Dimension(-1), 3, 20, Dimension(20, -1)],
        [-1, 3, 20, Dimension(-1, 20)],
    ]

    param1 = ov.opset8.parameter(PartialShape(inp_shapes[0]), name="x", dtype=Type.f32)
    param2 = ov.opset8.parameter(PartialShape(inp_shapes[1]), name="y", dtype=Type.f32)
    mul = ov.opset8.multiply(param1, param2)
    relu = ov.opset8.relu(mul)
    sigm = ov.opset8.sigmoid(relu)
    ...
    return (
        pt_model,
        ref_model,
        {"input": [(inp_shapes[0], Type.f32), (inp_shapes[1], Type.f32)]},
    )
...
def create_pytorch_nn_module_shapes_list_dynamic_via_input(tmp_dir):
    ...
    param1 = ov.opset8.parameter(
        PartialShape(inp_shapes[0]), name="x", dtype=np.float32
    )
    param2 = ov.opset8.parameter(
        PartialShape(inp_shapes[1]), name="y", dtype=np.float32
    )
    mul = ov.opset8.multiply(param1, param2)
    relu = ov.opset8.relu(mul)
    sigm = ov.opset8.sigmoid(relu)
...
def create_pytorch_nn_module_shapes_list_dynamic_single_input_via_input(tmp_dir):
    ...
    return (
        pt_model,
        ref_model,
        {"input": InputCutInfo(shape=inp_shapes, type=np.float32)},
    )
...
def create_pytorch_nn_module_convert_pytorch_frontend3(tmp_dir):
    ...
    return (
        ...
        ref_model,
        {
            ...
            "input": [
                InputCutInfo(shape=[-1, -1, -1, -1], type="f32"),
                InputCutInfo(shape=[-1, -1, -1, -1], type="f32"),
            ],
        },
    )
...
def create_pytorch_module_with_optional_inputs_case3(tmp_dir):
    ...
    return (
        net,
        ref_model,
        {"example_input": example_input, "input": [[3, 3, 3, 3], [3, 3, 3, 3]]},
    )


def create_pytorch_module_with_compressed_int8_constant_compress_to_fp16_default(
    tmp_dir,
):
    import torch
    import torch.nn.functional as F
    ...
    class Int8Model(torch.nn.Module):
        def __init__(self):
            ...
            self.weights = torch.randint(-127, 128, [1, 3, 3, 3], dtype=torch.int8)
...
def create_pytorch_module_with_compressed_int8_constant(tmp_dir):
    ...
    return (
        traced_model,
        ref_model,
        {"example_input": example_input, "compress_to_fp16": False},
    )
...
def create_pytorch_module_with_nested_inputs(tmp_dir):
    ...
    return (
        ...
        ref_model,
        {
            "example_input": {"z": (torch.zeros((1, 10)), torch.ones((1, 5, 2)))},
            "compress_to_fp16": False,
        },
    )
...
def create_pytorch_module_with_nested_inputs2(tmp_dir):
    ...
    return (
        ...
        ref_model,
        {
            "example_input": {
                "x": torch.ones((1, 10)),
                "z": (torch.zeros((1, 9)), torch.ones((1, 5, 5))),
            },
            "compress_to_fp16": False,
        },
    )
...
def create_pytorch_module_with_nested_inputs3(tmp_dir):
    ...
    return (
        ...
        ref_model,
        {
            "example_input": {
                "x": torch.ones((1, 10)),
                "z": (torch.zeros((1, 9)), torch.ones((1, 5, 3))),
            },
            "compress_to_fp16": False,
        },
    )
...
def create_pytorch_module_with_nested_inputs4(tmp_dir):
    class PTModel(torch.nn.Module):
        ...
        def forward(
            self, x: torch.Tensor, z: Tuple[torch.Tensor, torch.Tensor], y: torch.Tensor
        ):
            z1, z2 = z
            zeros1 = torch.zeros((1, 1))
            zeros2 = torch.zeros((1, 5, 1))
    ...
    return (
        ...
        ref_model,
        {
            "example_input": {
                "x": torch.ones((1, 10)),
                "z": (torch.zeros((1, 9)), torch.ones((1, 5, 10))),
                "y": torch.ones((1,)),
            },
            "compress_to_fp16": False,
        },
...
def create_pytorch_module_with_nested_inputs5(tmp_dir):
    ...
    return (
        ...
        ref_model,
        {
            "example_input": [
                torch.ones((1, 10)),
                (torch.zeros((1, 9)), torch.ones((1, 5, 10))),
                torch.ones((1,)),
            ],
            "compress_to_fp16": False,
        },
...
def create_pytorch_module_with_nested_inputs6(tmp_dir):
    class PTModel(torch.nn.Module):
        ...
        def forward(
            ...
            x: torch.Tensor,
            y: torch.Tensor = None,
            z: Tuple[torch.Tensor, torch.Tensor] = None,
        ):
            z1, z2 = z
            zeros1 = torch.zeros((1, 1))
            zeros2 = torch.zeros((1, 5, 1))
    ...
    return (
        ...
        ref_model,
        {
            "example_input": {
                "x": torch.ones((1, 11)),
                "z": (torch.zeros((1, 10)), torch.ones((1, 5, 10))),
            },
            "compress_to_fp16": False,
        },
    )
...
class TestMoConvertPyTorch(CommonMOConvertTest):
    ...
    def test_mo_import_from_memory(
        self,
        create_model,
        ie_device,
        precision,
    ...
    @pytest.mark.precommit
    def test_sharing_memory_switched_off(
        self, ie_device, precision, ir_version, temp_dir
    ):
    ...
    @pytest.mark.precommit
    def test_sharing_memory_switched_on(
        self, ie_device, precision, ir_version, temp_dir
    ):
...
class ConvertRaises(unittest.TestCase):
    def test_example_inputs(self):
        ...
        with self.assertRaisesRegex(
            AssertionError, ".*'example_inputs' argument is not recognized.*"
        ):
    ...
    def test_failed_extension(self):
        ...
        # Check that mo raises error message of wrong argument.
        with self.assertRaisesRegex(
            Exception, ".*Conversion is failed for: aten::relu.*"
        ):
            convert_model(
                pt_model,
                input=(inp_shapes, np.float32),
                extensions=[ConversionExtension("aten::relu", relu_bad)],
            )
    ...
    def test_failed_extension(self):
        ...
        with self.assertRaisesRegex(
            Exception, ".*PyTorch Frontend doesn't support provided model type.*"
        ):
...
def create_pytorch_precision_sensitive_two_inp_1(tmp_dir):
    ...
    test_params = {
        "example_input": (
            10000 + np.ones((2, 10), dtype=np.float32),
...
def create_pytorch_not_precision_sensitive(tmp_dir):
    ...
    test_params = (
        10000.0 + np.zeros((2, 20), dtype=np.float32),
    )  # 10 000 * 20 = 200 000 > 65504 (fp16_max)
    ...
    def test_precision_sensitive(
        ...
        if ref_model is not None:
            flag, msg = compare_functions(
                ir_test, ref_model, compare_tensor_names=False
            )
            assert flag, msg
