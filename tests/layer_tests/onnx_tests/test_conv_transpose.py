...
class TestConvTranspose(OnnxRuntimeLayerTest):
    ...
    common_tests_4D_precommit = [
        ...
        pytest.param(
            dict(
                input_shape=[1, 2, 20, 20],
                output_shape=[1, 2, 85, 85],
                kernel_shape=[2, 1, 8, 8],
                strides=[4, 4],
                group=2,
                output_padding=[1, 1],
            ),
    ...
    common_tests_4D = [
        dict(
            input_shape=[1, 3, 10, 10],
            output_shape=[1, 3, 11, 11],
            kernel_shape=[3, 3, 2, 2],
            strides=[1, 1],
        ),
        dict(
            input_shape=[1, 3, 10, 10],
            output_shape=[1, 3, 20, 20],
            kernel_shape=[3, 3, 2, 2],
            strides=[2, 2],
        ),
        dict(
    ...
    def test_conv_transpose_4D_precommit(
        self, params, bias, ie_device, precision, ir_version, auto_pad, temp_dir
    ):
        if ie_device == "GPU" and "dilations" in params:
            pytest.xfail("dilations are not supported on GPU")
        self._test(
            *self.create_conv_transpose(
                **params, ir_version=ir_version, bias=bias, auto_pad=auto_pad
            ),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.nightly
    @pytest.mark.skip(reason="GREEN_SUITE")
    def test_conv_transpose_valid_auto_pad_4D(
        self, params, bias, ie_device, precision, ir_version, auto_pad, temp_dir
    ):
        if ie_device == "GPU" and "dilations" in params:
            pytest.xfail("dilations are not supported on GPU")
    ...
    @pytest.mark.nightly
    @pytest.mark.skip(reason="GREEN_SUITE")
    def test_conv_transpose_same_auto_pad_4D(
        self, params, bias, ie_device, precision, ir_version, auto_pad, temp_dir
    ):
        if ie_device == "GPU" and "dilations" in params:
            pytest.xfail("dilations are not supported on GPU")
