...
class TestReduce(OnnxRuntimeLayerTest):
    def create_reduce(self, shape, reshapped_shape, op, axes, keep_dims, ir_version):
        ...
        if op not in [
            ...
            raise ValueError(
                "Operation has to be either Reduce(Min or Max or Mean or Sum or Prod"
            )
        ...
        node_def = onnx.helper.make_node(
            op, inputs=["input"], outputs=["output"], axes=axes, keepdims=keep_dims
        )
        ...
        onnx_net = onnx_make_model(
            graph_def, producer_name="test_model", opset_imports=[onnx_opset]
        )
    ...
    test_data = [
        ...
        dict(shape=[2, 4, 6, 8], reshapped_shape=[2, 1, 4 * 6 * 8, 1], axes=[1, 2, 3]),
        dict(
            shape=[2, 4, 6, 8, 10], reshapped_shape=[1, 1, 2, 4 * 6 * 8 * 10], axes=[0]
        ),
        ...
        dict(
            shape=[2, 4, 6, 8, 10],
            reshapped_shape=[1, 1, 2 * 4, 6 * 8 * 10],
            axes=[0, 1],
        ),
        dict(
            shape=[2, 4, 6, 8, 10], reshapped_shape=[2, 1, 4 * 6, 8 * 10], axes=[1, 2]
        ),
        dict(shape=[2, 4, 6, 8, 10], reshapped_shape=[2, 4, 6 * 8, 10], axes=[2, 3]),
        dict(
            shape=[2, 4, 6, 8, 10], reshapped_shape=[2, 4 * 6, 8 * 10, 1], axes=[3, 4]
        ),
        dict(
            shape=[2, 4, 6, 8, 10],
            reshapped_shape=[1, 1, 2 * 4 * 6, 8 * 10],
    ...
    def test_reduce_max_precommit(
        self, params, keep_dims, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_reduce(
                **params, op="ReduceMax", keep_dims=keep_dims, ir_version=ir_version
            ),
            ie_device,
            precision,
            ir_version,
    ...
    def test_reduce_sum(
        ...
        self._test(
            *self.create_reduce(
                **params, op="ReduceSum", keep_dims=keep_dims, ir_version=ir_version
            ),
            ie_device,
            precision,
            ir_version,
    ...
    def test_reduce_prod(
        ...
        self._test(
            *self.create_reduce(
                **params, op="ReduceProd", keep_dims=keep_dims, ir_version=ir_version
            ),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.parametrize("keep_dims", [True, False])
    @pytest.mark.precommit
    def test_reduce_mean_precommit(
        self, params, keep_dims, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_reduce(
                **params, op="ReduceMean", keep_dims=keep_dims, ir_version=ir_version
            ),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.parametrize("keep_dims", [True, False])
    @pytest.mark.precommit
    def test_reduce_min_precommit(
        self, params, keep_dims, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_reduce(
                **params, op="ReduceMin", keep_dims=keep_dims, ir_version=ir_version
            ),
            ie_device,
            precision,
            ir_version,
