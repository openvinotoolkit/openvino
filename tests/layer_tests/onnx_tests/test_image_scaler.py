...
class TestImageScaler(OnnxRuntimeLayerTest):
    def create_net(self, shape, scale, ir_version):
        ...
        node_def = onnx.helper.make_node(
            "ImageScaler", inputs=["input"], outputs=["output"], bias=bias, scale=scale
        )
    ...
    def create_net_const(self, shape, scale, precision, ir_version):
        ...
        node_def = onnx.helper.make_node(
            "ImageScaler", inputs=["const1"], outputs=["scale"], bias=bias, scale=scale
        )

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "scale"], outputs=["output"], axis=concat_axis
        )
    ...
    def test_image_scaler_precommit(
    ...
    @pytest.mark.parametrize("params", test_data_precommit)
    def test_image_scaler_const_precommit(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
