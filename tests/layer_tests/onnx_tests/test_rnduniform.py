...
class TestScale(OnnxRuntimeLayerTest):
    def create_net(self, dtype, shape, low, high, seed, ir_version):
        """
            ONNX net                     IR net

            RandomUniform->Output   =>   RandomUniform->Output
    ...
    from onnx import TensorProto
    test_data = [dict(dtype=TensorProto.FLOAT, shape=[10, 12], low=-100.0, high=100.0, seed=284.57),]
