...
class TestLog(OnnxRuntimeLayerTest):
    def _prepare_input(self, inputs_dict):
        for input in inputs_dict.keys():
            inputs_dict[input] = (
                np.random.rand(*(inputs_dict[input])).astype(np.float32) * 255 + 0.5
            )
        return inputs_dict
    ...
    def create_net(self, shape, ir_version):
        ...
        node_def = onnx.helper.make_node("Log", inputs=["input"], outputs=["output"])
    ...
    def create_net_const(self, shape, precision, ir_version):
        ...
        node_def = onnx.helper.make_node("Log", inputs=["const"], outputs=["log"])

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "log"], outputs=["output"], axis=concat_axis
        )
    ...
    test_data_precommit = [dict(shape=[1, 2]), dict(shape=[2, 3, 4, 5, 6])]
    ...
    def test_log_precommit(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    @pytest.mark.parametrize("params", test_data_precommit)
    @pytest.mark.nightly
    def test_log_const_precommit(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_net_const(
                **params, precision=precision, ir_version=ir_version
            ),
            ie_device,
            precision,
            ir_version,
