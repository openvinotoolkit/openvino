...
class TestGemm(OnnxRuntimeLayerTest):
    ...
    def create_net(
        ...
        node_def = onnx.helper.make_node(
            "Gemm", inputs=inputs, outputs=["output"], **attrs
        )
    ...
    def create_net_double(
        ...
        if trans_b:
            attrs["transB"] = trans_b
        node_def = onnx.helper.make_node(
            "Gemm", inputs=["input1", "input2", "const"], outputs=["output"], **attrs
        )
    ...
    test_data = [dict(shapeA=[3, 6], shapeB=[6, 4], shapeC=[3, 4])]
...
class TestPytorchMM(PytorchLayerTest):
    ...
    def test_pytorch_mm(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_net(precision, **params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
