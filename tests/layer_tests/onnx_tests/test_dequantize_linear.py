...
class TestDequantizeLinear(OnnxRuntimeLayerTest):
    ...
    def create_dequanize_linear(
        ...
        node_def = onnx.helper.make_node(
            "DequantizeLinear", inputs=inputs, outputs=["output"], **args
        )
    ...
    test_data = [
        dict(
            shape=[8],
            y_scale=np.array(2, dtype=float),
    ...
    test_data_axis = [
        ...
        dict(
            shape=[2, 4],
            y_scale=np.array([2, 2.5, 3, 2.3], dtype=float),
            y_zero_point=np.array([128, 128, 128, 128], dtype=np.uint8),
            axis=1,
        ),
        dict(
            shape=[2, 4],
            y_scale=np.array([2, 2.5, 3, 2.3], dtype=float),
            y_zero_point=np.array([1, 1, 1, 1], dtype=np.int8),
            axis=1,
        ),
        ...
        dict(
            shape=[2, 4, 6, 8], y_scale=np.array([2, 2.5, 3, 2.3], dtype=float), axis=1
        ),
        dict(
            shape=[2, 4, 6, 8],
            y_scale=np.array([2, 2.5, 3, 2.3], dtype=float),
    ...
    @pytest.mark.nightly
    def test_quantize_linear_def_zerop_opset10(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_dequanize_linear(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    @pytest.mark.nightly
    def test_quantize_linear_opset10(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_dequanize_linear(**params, ir_version=ir_version),
    ...
    def test_quantize_linear_opset13(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_dequanize_linear(**params, opset=13, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    @pytest.mark.nightly
    @pytest.mark.skip(reason="DequantizeLinear-13 is unsupported in MO")
    def test_quantize_linear_axis_opset13(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_dequanize_linear(**params, opset=13, ir_version=ir_version),
