...
class TestSign(OnnxRuntimeLayerTest):
    def create_net(self, shape, ir_version):
        ...
        node_def = onnx.helper.make_node("Sign", inputs=["input"], outputs=["output"])
    ...
    def create_net_const(self, shape, precision, ir_version):
        ...
        Input->Concat(+sign const)->Output   =>    Input->Concat(+const)
        ...
        node_def = onnx.helper.make_node("Sign", inputs=["const1"], outputs=["sign"])

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "sign"], outputs=["output"], axis=concat_axis
        )
