...
class TestTranspose(OnnxRuntimeLayerTest):
    def create_net(self, shape, perm, ir_version):
        ...
        node_def = helper.make_node(
            "Transpose", inputs=["input"], outputs=["transpose"], **args
        )

        sigmoid_def = helper.make_node(
            "Sigmoid", inputs=["transpose"], outputs=["output"]
        )
    ...
    def create_net_const(self, shape, perm, ir_version):
        ...
        if perm:
            args["perm"] = perm
        node_def = helper.make_node(
            "Transpose", inputs=["const1"], outputs=["transpose"], **args
        )
    ...
    @pytest.mark.parametrize("params", test_data_precommit)
    @pytest.mark.nightly
    def test_transpose_const_precommit(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_net_const(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
