...
class TestGather(OnnxRuntimeLayerTest):
    def create_net(self, shape, axis, indices, output_shape, ir_version):
        ...
        node_def = onnx.helper.make_node(
            "Gather", inputs=["input", "indices"], outputs=["output"], **args
        )

        # Create the graph (GraphProto)
        graph_def = helper.make_graph(
            [node_indices_def, node_def], "test_model", [input], [output]
        )
    ...
    def create_net_const(self, shape, axis, indices, output_shape, ir_version):
        ...
        if axis:
            args["axis"] = axis
        node_def = onnx.helper.make_node(
            "Gather", inputs=["const1", "indices"], outputs=["gather"], **args
        )

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "gather"], outputs=["output"], axis=concat_axis
        )
    ...
    test_data_precommit = [
        ...
        dict(
            shape=[6, 8, 10, 12],
            axis=-1,
            indices=[[[2, -1], [3, 2]], [[5, -1], [3, -2]]],
            output_shape=[6, 8, 10, 2, 2, 2],
        ),
    ]

    test_data = [
        dict(shape=[10, 12], axis=0, indices=[3, 6], output_shape=[2, 12]),
    ...
    @pytest.mark.nightly
    def test_gather_nightly_negative_indices(
