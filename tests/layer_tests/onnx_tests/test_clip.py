...
class TestClip(OnnxRuntimeLayerTest):
    def create_net(self, shape, ir_version, opset, min=None, max=None):
        ...
        if opset < 11:
            ...
            node_def = onnx.helper.make_node(
                "Clip", inputs=["input"], outputs=["output"], **args
            )
            ...
            node_def = onnx.helper.make_node(
                "Clip", inputs=clip_inputs, outputs=["output"]
            )
            nodes.append(node_def)
            ...
            if opset < 11 or min is not None and max is not None:
                nodes_attributes = {
                    ...
                    "node": {
                        "kind": "op",
                        "type": "Clamp",
                        "min": min if min is not None else -3.4028235e38,
                        "max": max if max is not None else 3.4028235e38,
                    },
    ...
    def test_clip_opset6(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_net(**params, ir_version=ir_version, opset=6),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    def test_clip_opset11(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_net(**params, ir_version=ir_version, opset=11),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
