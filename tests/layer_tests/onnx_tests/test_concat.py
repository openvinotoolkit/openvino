...
class TestConcat(OnnxRuntimeLayerTest):
    ...
    def create_concat_net_const(self, input_shape, output_shape, axis, ir_version):
        ...
        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["const1", "const2"], outputs=["output_concat"], axis=axis
        )
        ...
        constant_reshape = np.concatenate(
            [constant_reshape, constant_reshape], axis=axis
        )
    ...
    def create_concat_net(
        self, input_shape, output_shape, axis, input_names, ir_version
    ):
        """
        ONNX net                                                 IR net
        ...
        node = onnx.helper.make_node(
            "Concat", inputs=input_names, outputs=["output"], axis=axis
        )
    ...
    test_data_3D = [
        dict(input_shape=[1, 50, 50], output_shape=[2, 50, 50], axis=0),
        dict(input_shape=[2, 50, 50], output_shape=[2, 100, 50], axis=1),
        dict(input_shape=[4, 50, 50], output_shape=[4, 50, 100], axis=2),
    ]

    test_data_4D_precommit = [
        dict(input_shape=[1, 32, 800, 800], output_shape=[2, 32, 800, 800], axis=0)
    ]

    test_data_4D = [
        dict(input_shape=[1, 32, 800, 800], output_shape=[2, 32, 800, 800], axis=0),
        dict(input_shape=[4, 32, 80, 80], output_shape=[4, 64, 80, 80], axis=1),
        dict(input_shape=[2, 21, 80, 80], output_shape=[2, 21, 160, 80], axis=2),
        dict(input_shape=[3, 21, 80, 80], output_shape=[3, 21, 80, 160], axis=3),
    ]

    test_data_5D_precommit = [
        dict(input_shape=[1, 50, 50, 80, 60], output_shape=[2, 50, 50, 80, 60], axis=0),
        dict(
            input_shape=[1, 50, 50, 80, 60], output_shape=[1, 50, 50, 80, 120], axis=4
        ),
    ]
    ...
    test_data_5D = [
        dict(input_shape=[1, 50, 50, 80, 60], output_shape=[2, 50, 50, 80, 60], axis=0),
        dict(
            input_shape=[1, 50, 50, 80, 60], output_shape=[1, 100, 50, 80, 60], axis=1
        ),
        dict(
            input_shape=[1, 50, 50, 80, 60], output_shape=[1, 50, 100, 80, 60], axis=2
        ),
        dict(
            input_shape=[1, 50, 50, 80, 60], output_shape=[1, 50, 50, 160, 60], axis=3
        ),
        dict(
            input_shape=[1, 50, 50, 80, 60], output_shape=[1, 50, 50, 80, 120], axis=4
        ),
    ...
    def test_concat_3D_const(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_concat_net_const(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    def test_concat_4D_const_precommit(
    ...
    @pytest.mark.nightly
    def test_concat_5D_const_precommit(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_concat_net_const(**params, ir_version=ir_version),
    ...
    @pytest.mark.nightly
    def test_concat_inputs_order(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_concat_net(**params, ir_version=ir_version),
            ie_device=ie_device,
            precision=precision,
