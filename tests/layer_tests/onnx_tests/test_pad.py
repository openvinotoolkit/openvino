...
class TestPad(OnnxRuntimeLayerTest):
    def create_net(self, shape, mode, pads, value, ir_version, opset=None):
        ...
        if opset is not None and opset < 11:
            ...
            node_def = onnx.helper.make_node(
                "Pad", inputs=["input"], outputs=["pad"], **args
            )
            ...
            if mode:
                args["mode"] = mode
            node_def = onnx.helper.make_node(
                "Pad", inputs=inputs, outputs=["pad"], **args
            )
            nodes.extend([node_pads_def, node_def])

        sigmoid_def = onnx.helper.make_node("Elu", inputs=["pad"], outputs=["output"])
        ...
        if check_ir_version(10, None, ir_version):
            ...
            if mode in (None, "constant"):
                nodes_attributes.update(
                    {
                        "const_node_indata": {"value": value, "kind": "data"},
                        "const_node": {"kind": "op", "type": "Const"},
                        "const_node_data": {"shape": None, "kind": "data"},
                    }
                )
                edges += [
                    ("const_node_indata", "const_node"),
    ...
    @pytest.mark.nightly
    def test_pad_opset_9(
        self, params, mode_value, ie_device, precision, ir_version, temp_dir
    ):
        mode, value = mode_value
        self._test(
            *self.create_net(
                **params, mode=mode, value=value, ir_version=ir_version, opset=9
            ),
            ie_device,
            precision,
            ir_version,
    ...
    def test_pad_opset_latest_precommit(
        self, params, mode_value, ie_device, precision, ir_version, temp_dir
    ):
        mode, value = mode_value
        self._test(
            *self.create_net(**params, mode=mode, value=value, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
