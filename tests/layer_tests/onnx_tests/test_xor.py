...
class TestXor(OnnxRuntimeLayerTest):
    ...
    def create_net(self, shape1, shape2, ir_version):
        ...
        Input->Xor with 2nd input->Output   =>    Input->LogicalXor
        """
        ...
        node_def = helper.make_node(
            "Xor", inputs=["input1", "input2"], outputs=["output"]
        )
    ...
    def create_net_one_const(self, shape1, shape2, ir_version):
        ...
        node_def = helper.make_node(
            "Xor", inputs=["input", "const"], outputs=["output"]
        )
    ...
    def create_net_const(self, shape1, shape2, ir_version):
        ...
        node_def = helper.make_node(
            "Xor", inputs=["const1", "const2"], outputs=["node_out"]
        )
        ...
        if check_ir_version(10, None, ir_version):
            nodes_attributes = {
                ...
                "input_const_data": {
                    "kind": "data",
                    "value": constant_calculated.flatten(),
                },
                "const": {"kind": "op", "type": "Const"},
    ...
    def test_xor_one_const(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_net_one_const(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
