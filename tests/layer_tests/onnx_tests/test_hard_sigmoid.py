...
class TestHardSigmoid(OnnxRuntimeLayerTest):
    ...
    def create_net(self, shape, alpha, beta, ir_version):
        ...
        node_def = onnx.helper.make_node(
            "HardSigmoid", inputs=["input"], outputs=["output"], **args
        )
        ...
        if check_ir_version(10, None, ir_version):
            nodes_attributes = {
                ...
                "input_beta_data": {
                    "kind": "data",
                    "value": [beta if beta is not None else 0.5],
                },
    ...
    def create_net_const(self, shape, alpha, beta, precision, ir_version):
        ...
        if beta is not None:
            args["beta"] = beta
        node_def = onnx.helper.make_node(
            "HardSigmoid", inputs=["const1"], outputs=["sigmoid1"], **args
        )
        ...
        #
        constant = np.clip(
            constant * (alpha if alpha is not None else 0.2)
            + (beta if beta is not None else 0.5),
            0,
            1,
        )
        if precision == "FP16":
    ...
    @pytest.mark.nightly
    def test_hard_sigmoid_const_precommit(
