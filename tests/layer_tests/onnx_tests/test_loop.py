...
class TestLoop(OnnxRuntimeLayerTest):
    ...
    def create_body_graph(
        input_nodes, output_nodes, input_names, output_names, input_shape, graph_name
    ):
        ...
        one_int = TestLoop.create_const(
            "one_int_" + graph_name, TensorProto.INT64, np.ones([1])
        )
        ...
        for i in range(2, len(input_names)):
            add_one_nodes.append(
                helper.make_node(
                    "Add",
                    inputs=[input_names[i], "one_" + graph_name],
        ...
        for i in range(1, len(input_nodes)):
            identity_nodes.append(
                helper.make_node(
                    "Identity", inputs=[input_names[i]], outputs=[output_names[i - 1]]
                )
            )
        ...
        body_graph = helper.make_graph(
            body_nodes, graph_name, input_nodes, output_nodes
        )
    ...
    def create_loop(self):
        ...
        in_1_int = helper.make_tensor_value_info(
            "in_1_int", TensorProto.FLOAT, input_shape
        )
        in_1_int_out = helper.make_tensor_value_info(
            "in_1_int_out", TensorProto.FLOAT, input_shape
        )
        ...
        body_graph_1 = self.create_body_graph(
        ...
        graph_def = helper.make_graph(
            [M_1, cond, node_loop_1, res_node], "graph", [in_1], [res]
        )
    ...
    def create_loop_in_loop(self):
        ...
        in_2_int = helper.make_tensor_value_info(
            "in_2_int", TensorProto.FLOAT, input_shape
        )
        in_2_int_out = helper.make_tensor_value_info(
            "in_2_int_out", TensorProto.FLOAT, input_shape
        )
        ...
        add_one_node = helper.make_node(
            "Add", inputs=["in_1_int", "one"], outputs=["in_1_loop_1"]
        )

        add_one_to_m_node = helper.make_node(
            "Add", inputs=["m_1", "one_int"], outputs=["m_1_loop_1"]
        )

        cond_2 = self.create_const("cond_2", TensorProto.BOOL, cond_value)

        # create body for internal loop
        body_graph_2 = self.create_body_graph(
    ...
    @pytest.mark.precommit
    @pytest.mark.timeout(250)
    def test_loop_in_loop_simple_precommit(
        self, ie_device, precision, ir_version, temp_dir
    ):
        if ie_device == "GPU":
            pytest.xfail(
                "Program doesn't contain primitive: constant:res/10/M_2 that is input to: loop"
            )
        self._test(
            *self.create_loop_in_loop(),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
