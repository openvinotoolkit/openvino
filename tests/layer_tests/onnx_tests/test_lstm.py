...
class TestLSTM(OnnxRuntimeLayerTest):
    ...
    def create_lstm(self, direction: str, cell_type: str, hidden_size=128):
        ...
        onnx_net = onnx_make_model(
            graph_def, producer_name="test_{}_model".format(cell_type)
        )
    ...
    def test_lstm_simple_precommit(
        self, direction, cell_type, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_lstm(direction, cell_type),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
    ...
    @pytest.mark.parametrize("direction", ["forward", "bidirectional", "reverse"])
    @pytest.mark.parametrize("cell_type", ["LSTM", "GRU", "RNN"])
    def test_lstm_sequence_generate(
        self, direction, cell_type, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_lstm(direction, cell_type),
