...
class TestROIAlign(OnnxRuntimeLayerTest):
    ...
    def create_net(
        ...
        onnx_net = onnx_make_model(
            graph_def, producer_name="test_model", opset_imports=[operatorsetid]
        )
    ...
    @pytest.mark.xfail(
        condition=platform.system() == "Windows", reason="Ticket - 122731"
    )
    @pytest.mark.xfail(
    ...
    def test_roi_alignv10(self, params, ie_device, precision, ir_version, temp_dir):
        ...
        if ie_device != "GPU":
            self._test(
                *self.create_net(**params, ir_version=ir_version, onnx_version=10),
                ie_device,
                precision,
                ir_version,
                temp_dir=temp_dir,
