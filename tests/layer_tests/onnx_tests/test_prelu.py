...
class TestPRelu(OnnxRuntimeLayerTest):
    def create_net(self, shape, slope_shape, precision, ir_version, opset=None):
        ...
        node_def = onnx.helper.make_node(
            "PRelu", inputs=["input", "slope"], outputs=["output"]
        )
    ...
    @pytest.mark.nightly
    def test_prelu_opset7_shared_channels(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_net(
    ...
    def test_prelu_shared_channels_precommit(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
    ...
    @pytest.mark.precommit
    def test_prelu_scalar_precommit(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_net(**params, precision=precision, ir_version=ir_version),
    ...
    @pytest.mark.parametrize("params", test_data_shared_channels)
    @pytest.mark.nightly
    def test_prelu_shared_channels(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_net(**params, precision=precision, ir_version=ir_version),
