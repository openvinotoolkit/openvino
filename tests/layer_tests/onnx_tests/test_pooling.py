...
class TestPooling(OnnxRuntimeLayerTest):
    def create_net(
        ...
        if auto_pad in ["SAME_UPPER", "SAME_LOWER"]:
            ...
            out_spacial_shape = rounding(
                (
                    float_array(shape[2:])
                    + np.add(_pads[:, 0], _pads[:, 1])
                    - float_array(kernel_shape)
                )
                / strides
                + 1
            )
        ...
        output = helper.make_tensor_value_info(
            "output", TensorProto.FLOAT, out_concat_shape
        )

        constant = np.random.randint(-127, 127, out_shape).astype(float)

        node_def = onnx.helper.make_node(
            op, inputs=["input"], outputs=["pool"], **node_args
        )
        ...
        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["pool", "const1"], outputs=["output"], axis=concat_axis
        )
        ...
        if check_ir_version(10, None, ir_version):
            nodes_attributes = {
                ...
                "node": {
                    ...
                    "kernel": (
                        kernel_shape[0] if len(kernel_shape) == 1 else kernel_shape
                    ),
                    "rounding_type": (
            ...
            if op == "AveragePool":
                ...
                nodes_attributes["node"]["exclude-pad"] = (
            ...
            ref_net = build_graph(nodes_attributes, edges, nodes_with_edges_only=True)
    ...
    def create_global_net(self, shape, op, ir_version):
        ...
        node_def = onnx.helper.make_node(op, inputs=["input"], outputs=["output"])
    ...
    test_data = [
        ...
        dict(
            shape=[2, 3, 28, 28], kernel_shape=[3, 3], pads=[2, 2, 2, 2], strides=None
        ),
        dict(
            shape=[2, 3, 28, 28], kernel_shape=[5, 5], pads=[0, 2, 0, 4], strides=None
        ),
        dict(
            shape=[2, 3, 28, 28], kernel_shape=[5, 5], pads=[2, 0, 4, 0], strides=None
        ),
    ...
    test_data_autopad = [
        ...
        dict(
            shape=[2, 3, 10], auto_pad="VALID", kernel_shape=[2], pads=None, strides=[3]
        ),
        dict(
            shape=[2, 3, 30, 30],
            auto_pad="SAME_UPPER",
    ...
    @pytest.mark.nightly
    def test_avgpool_opset7(
        self, params, incl_pad, ie_device, precision, ir_version, temp_dir
    ):
    ...
    @pytest.mark.nightly
    def test_avgpool_opset7_autopad(
        ...
        self._test(
            *self.create_net(
                **params, op="AveragePool", ir_version=ir_version, opset=7
            ),
            ie_device,
            precision,
            ir_version,
    ...
    def test_avgpool_opset10(
        self, params, incl_pad, ceil, ie_device, precision, ir_version, temp_dir
    ):
        if not len(params["shape"]) in [4, 5]:
            pytest.skip("Pooling layer support only 4D and 5D input tensors")
        self._test(
    ...
    @pytest.mark.parametrize("params", test_data_autopad)
    @pytest.mark.nightly
    def test_avgpool_opset10_autopad(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        if not len(params["shape"]) in [4, 5]:
            pytest.skip("Pooling layer support only 4D and 5D input tensors")
        self._test(
            *self.create_net(
                **params, op="AveragePool", ir_version=ir_version, opset=10
            ),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.nightly
    def test_maxpool_opset8(
        self, params, st_order, ie_device, precision, ir_version, temp_dir
    ):
        if not len(params["shape"]) in [4, 5]:
            pytest.skip("Pooling layer support only 4D and 5D input tensors")
        self._test(
    ...
    @pytest.mark.parametrize("params", test_data_autopad)
    @pytest.mark.nightly
    def test_maxpool_opset8_autopad(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        if not len(params["shape"]) in [4, 5]:
            pytest.skip("Pooling layer support only 4D and 5D input tensors")
        self._test(
            *self.create_net(**params, op="MaxPool", ir_version=ir_version, opset=8),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    @pytest.mark.parametrize("ceil", [True, False])
    @pytest.mark.nightly
    def test_maxpool_opset10(
        self, params, st_order, ceil, ie_device, precision, ir_version, temp_dir
    ):
        if not len(params["shape"]) in [4, 5]:
            pytest.skip("Pooling layer support only 4D and 5D input tensors")
        self._test(
    ...
    def test_maxpool_opset10_autopad(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        if not len(params["shape"]) in [4, 5]:
            pytest.skip("Pooling layer support only 4D and 5D input tensors")
        self._test(
            *self.create_net(**params, op="MaxPool", ir_version=ir_version, opset=10),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    def test_global_avgpool(self, params, ie_device, precision, ir_version, temp_dir):
        if not len(params["shape"]) in [4, 5]:
            pytest.skip("Pooling layer support only 4D and 5D input tensors")
        self._test(
            *self.create_global_net(
                **params, op="GlobalAveragePool", ir_version=ir_version
            ),
            ie_device,
            precision,
            ir_version,
    ...
    def test_global_maxpool(self, params, ie_device, precision, ir_version, temp_dir):
        ...
        self._test(
            *self.create_global_net(
                **params, op="GlobalMaxPool", ir_version=ir_version
            ),
            ie_device,
            precision,
            ir_version,
