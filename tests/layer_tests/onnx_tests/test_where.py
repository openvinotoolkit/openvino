...
class TestWhere(OnnxRuntimeLayerTest):
    def _prepare_input(self, inputs_dict):
        for input in inputs_dict.keys():
            inputs_dict[input] = np.random.randint(0, 2, inputs_dict[input]).astype(
                bool
            )
        return inputs_dict
    ...
    def create_net(self, condition_shape, shape_than, else_shape, ir_version):
        ...
        input_cond = helper.make_tensor_value_info(
            "input_cond", TensorProto.BOOL, condition_shape
        )
        input_than = helper.make_tensor_value_info(
            "input_than", TensorProto.BOOL, shape_than
        )
        input_else = helper.make_tensor_value_info(
            "input_else", TensorProto.BOOL, else_shape
        )
        output = helper.make_tensor_value_info(
            "output", TensorProto.BOOL, condition_shape
        )
    ...
    test_data = [
        dict(condition_shape=[4, 6], shape_than=[4, 6], else_shape=[4, 6]),
