...
class TestLeakyRelu(OnnxRuntimeLayerTest):
    def create_net(self, shape, alpha, ir_version):
        ...
        node_def = onnx.helper.make_node(
            "LeakyRelu", inputs=["input"], outputs=["output"], alpha=alpha
        )
    ...
    def create_net_const(self, shape, alpha, precision, ir_version):
        ...
        node_def = onnx.helper.make_node(
            "LeakyRelu", inputs=["const1"], outputs=["lrelu1"], alpha=alpha
        )

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "lrelu1"], outputs=["output"], axis=concat_axis
        )
    ...
    @pytest.mark.parametrize("params", test_data_precommit)
    @pytest.mark.precommit
    def test_leaky_relu_const_precommit(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
