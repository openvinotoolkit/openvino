...
class TestFlatten(OnnxRuntimeLayerTest):
    ...
    def test_flatten_3D(
        ...
        self._test(
            *self.create_flatten_net(**params, ir_version=ir_version, opset=opset),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    def test_flatten_3D_const(
        ...
        if params["axis"] < 0:
            ...
            self.skip_framework = False
        self._test(
            *self.create_flatten_net_const(
                **params, ir_version=ir_version, opset=opset
            ),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.parametrize("opset", [6, 9])
    @pytest.mark.nightly
    def test_flatten_4D_const_precommit(
        self, params, opset, ie_device, precision, ir_version, temp_dir
    ):
        # negative axis not allowed by onnx spec for flatten-1 and flatten-9
        if params["axis"] < 0:
    ...
    @pytest.mark.parametrize("opset", [6, 9])
    @pytest.mark.nightly
    def test_flatten_5D_const_precommit(
        self, params, opset, ie_device, precision, ir_version, temp_dir
    ):
        # negative axis not allowed by onnx spec for flatten-1 and flatten-9
        if params["axis"] < 0:
