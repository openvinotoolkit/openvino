...
class TestCeil(OnnxRuntimeLayerTest):
    def create_net(self, shape, ir_version):
        ...
        Input->Ceil->Output   =>    Input->Ceiling
        ...
        node_def = onnx.helper.make_node("Ceil", inputs=["input"], outputs=["output"])
    ...
    def create_net_const(self, shape, precision, ir_version):
        ...
        Input->Concat(+ceil const)->Output   =>    Input->Concat(+const)
        ...
        node_def = onnx.helper.make_node("Ceil", inputs=["const1"], outputs=["floor"])

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "floor"], outputs=["output"], axis=concat_axis
        )
