...
class TestReciprocal(OnnxRuntimeLayerTest):
    def create_net(self, shape, ir_version):
        ...
        node_add_def = helper.make_node(
            "Add", inputs=["input", "const"], outputs=["add"]
        )

        node_def = helper.make_node("Reciprocal", inputs=["add"], outputs=["output"])
    ...
    def create_net_const(self, shape, precision, ir_version):
        ...
        node_def = helper.make_node(
            "Reciprocal", inputs=["const"], outputs=["node_out"]
        )

        node_concat_def = helper.make_node(
            "Concat", inputs=["input", "node_out"], outputs=["output"], axis=concat_axis
        )
    ...
    test_data_precommit = [dict(shape=[2, 4]), dict(shape=[2, 4, 6, 8])]
    ...
    @pytest.mark.parametrize("params", test_data_precommit)
    @pytest.mark.precommit
    def test_reciprocal_const_precommit(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
