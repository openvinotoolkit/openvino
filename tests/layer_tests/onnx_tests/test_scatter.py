...
class TestScatters(OnnxRuntimeLayerTest):
    ...
    def create_net(
        self, input_shape, indices_shape, updates_shape, output_shape, axis, ir_version
    ):
        """
        ONNX net                    IR net
        ...
        updates = helper.make_tensor_value_info(
            "updates", TensorProto.FLOAT, indices_shape
        )
...
test_data = [
    ...
    dict(
        ...
        indices_shape=[1, 256, 200, 272],
        updates_shape=[1, 256, 200, 272],
        axis=None,
        output_shape=[1, 256, 200, 272],
    ),
]
