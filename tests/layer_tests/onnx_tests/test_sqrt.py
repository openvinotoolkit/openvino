...
class TestSqrt(OnnxRuntimeLayerTest):
    def _prepare_input(self, inputs_dict):
        for input in inputs_dict.keys():
            inputs_dict[input] = np.random.randint(0, 255, inputs_dict[input]).astype(
                np.float32
            )
        return inputs_dict
    ...
    def create_net(self, shape, ir_version):
        ...
        node_def = onnx.helper.make_node("Sqrt", inputs=["input"], outputs=["output"])
    ...
    def create_net_const(self, shape, precision, ir_version):
        ...
        node_def = onnx.helper.make_node("Sqrt", inputs=["const"], outputs=["sqrt"])

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "sqrt"], outputs=["output"], axis=concat_axis
        )
