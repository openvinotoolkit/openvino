...
class TestGenerateProposals(Caffe2OnnxLayerTest):
    ...
    def _prepare_input(self, inputs_dict):
        inputs_dict["scores"] = np.random.rand(1, 6, 10, 8).astype('float32')
        inputs_dict["bbox_deltas"] = np.random.rand(1, 24, 10, 8).astype('float32')
        inputs_dict["im_info"] = np.array([[1000, 1000, 1]]).astype('float32')
        #inputs_dict["anchors"] = np.reshape(np.arange(10 * 8 * 6 * 4),
        #                                    [10, 8, 6, 4]).astype('float32')
        inputs_dict["anchors"] = np.reshape(np.arange(6 * 4),
                                            [6, 4]).astype('float32')
