...
class TestUnsqueeze(OnnxRuntimeLayerTest):
    def create_unsqueeze_net(self, axes, input_shape, output_shape, ir_version):
        ...
        node_squeeze_def = onnx.helper.make_node(
            "Unsqueeze", inputs=["input"], outputs=["output"], axes=axes
        )
    ...
    def create_unsqueeze_net_const(self, axes, input_shape, output_shape, ir_version):
        ...
        node_squeeze_def = onnx.helper.make_node(
            "Unsqueeze", inputs=["const1"], outputs=["unsqueeze1"], axes=axes
        )
    ...
    def test_unsqueeze_5D(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_unsqueeze_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    @pytest.mark.nightly
    @pytest.mark.skip(reason="GREEN_SUITE")
    def test_unsqueeze_const_5D(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_unsqueeze_net_const(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
