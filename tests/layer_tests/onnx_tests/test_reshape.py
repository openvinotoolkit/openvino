...
class TestReshape(OnnxRuntimeLayerTest):
    def create_reshape_net(self, input_shape, output_shape, ir_version):
        ...
        node_reshape_def = onnx.helper.make_node(
            "Reshape", inputs=["input", "shape"], outputs=["output"]
        )
    ...
    def create_reshape_net_const(self, input_shape, output_shape, ir_version):
        ...
        node_reshape_def = onnx.helper.make_node(
            "Reshape", inputs=["const1", "shape"], outputs=["reshape1"]
        )

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "reshape1"], outputs=["output"], axis=concat_axis
        )
    ...
    test_data_5D = [
        dict(input_shape=[4, 6, 8, 10, 12], output_shape=[4, 6, 8, 120]),
    ...
    test_data_5D_precommit = [
        dict(input_shape=[2, 4, 6, 8, 10], output_shape=[8, 6, 8, 10])
    ]

    test_data_4D = [
        dict(input_shape=[4, 6, 8, 10], output_shape=[24, 8, 10]),
    ...
    test_data_3D = [
        dict(input_shape=[4, 6, 8], output_shape=[24, 8]),
    ...
    def test_reshape_5D_precommit(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_reshape_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    def test_reshape_const_5D(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_reshape_net_const(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
