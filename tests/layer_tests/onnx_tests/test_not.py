...
class TestNot(OnnxRuntimeLayerTest):
    ...
    def create_net(self, shape, ir_version):
        ...
        node_def = onnx.helper.make_node("Not", inputs=["input"], outputs=["output"])
    ...
    def create_net_const(self, shape, ir_version):
        ...
        node_def = onnx.helper.make_node("Not", inputs=["const1"], outputs=["not"])

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "not"], outputs=["output"], axis=concat_axis
        )
    ...
    test_data_precommit = [dict(shape=[2, 3, 4]), dict(shape=[2, 4, 6, 8, 10])]
