...
class TestPytorchEmbeddingBag(PytorchLayerTest):
    def _prepare_input(self, inputs_dict):
        assert (
            "input" in inputs_dict and "offsets" in inputs_dict
        ), "input and offsets should be in inputs_dict"
    ...
    def create_net(
        self, n, m, emb_batch_size, ir_version, per_sample_weights=False, offsets=None
    ):
        """
        Pytorch net                         IR net
        ...
        EE = EmbeddingBagModel(
            n, m, indices_shape=[emb_batch_size], per_sample_weights=per_sample_weights
        )
                ...
                nodes_attributes.update(
                    {
                        ...
                        "input_offsets_data": {
                            "kind": "data",
                            "value": np.arange(0, 128, 2),
                        },
                        "offsets": {"kind": "op", "type": "Const"},
                        "offsets_data": {
                            "shape": [int(emb_batch_size / 2)],
                            "kind": "data",
                        },
                        "node_data": {
                            "shape": [int(emb_batch_size / 2), m],
                            "kind": "data",
                        },
                    }
                )
        ...
        if offsets is not None:
            self.var = (
                torch.from_numpy(np.random.choice(n, emb_batch_size)).long(),
                torch.from_numpy(np.array(offsets)).long(),
            )
        else:
            self.var = (
                torch.from_numpy(
                    np.random.choice(n, emb_batch_size).reshape(
                        int(emb_batch_size / 2), 2
                    )
                ).long(),
            )
        return EE, ref_net
    ...
    test_data = [
        ...
        dict(
            n=1460, m=16, emb_batch_size=128, offsets=[0, 2, 6, 20, 20, 80]
        ),  # empty bag case
    ]
    ...
    def test_pytorch_embedding_bag(
