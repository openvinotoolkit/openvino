...
class TestArgMax(OnnxRuntimeLayerTest):
    def create_net(self, shape, axis, keepdims, ir_version):
        ...
        output = helper.make_tensor_value_info(
            "output", TensorProto.INT64, output_shape_squeeze
        )
        ...
        if keepdims is None or keepdims == 1:
            node_flatten_def = onnx.helper.make_node(
                "Flatten", inputs=["argmax"], outputs=["output"]
            )
        ...
        if check_ir_version(10, None, ir_version):
            ...
            if keepdims == 0:
                nodes_attributes.update(
                    {
                        "squeeze_const_indata": {"shape": [1], "kind": "data"},
                ...
                edges.extend(
                    [
                        ("squeeze_const_indata", "squeeze_const"),
                ...
                nodes_attributes.update(
                    {
                        ...
                        "flatten_data": {
                            "shape": [
                                output_shape_squeeze[0],
                                np.prod(output_shape_squeeze[1:]),
                            ],
                            "kind": "data",
                        },
                    }
    ...
    def test_argmax(self, params, keepdims, ie_device, precision, ir_version, temp_dir):
        ...
        self._test(
            *self.create_net(**params, ir_version=ir_version, keepdims=keepdims),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
