...
class TestSoftmax(OnnxRuntimeLayerTest):
    ...
    def create_net(self, shape, softmax_axis, ir_version):
        ...
        node_def = onnx.helper.make_node(
            "Softmax", inputs=["input"], outputs=["output"], axis=softmax_axis
        )
        ...
        if check_ir_version(10, None, ir_version):
            if len(shape) == 2 and shape == flatten_shape:
                ref_nodes_attributes = {
                    ...
                    "flatten_shape_val": {
                        "shape": int64_array(reshape_data_val).shape,
                        "kind": "data",
    ...
    test_data = [
        dict(shape=[10, 12], softmax_axis=0),
