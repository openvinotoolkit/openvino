...
class TestReduceL1L2(OnnxRuntimeLayerTest):
    def create_reduce_lp(self, shape, axes, keep_dims, reduce_p, ir_version):
        ...
        node_def = onnx.helper.make_node(
            "ReduceL" + str(reduce_p), inputs=["input"], outputs=["output"], **args
        )
    ...
    def create_reduce_lp_const(self, shape, axes, keep_dims, reduce_p, ir_version):
        ...
        if axes:
            args["axes"] = axes
        node_def = onnx.helper.make_node(
            "ReduceL" + str(reduce_p), inputs=["const1"], outputs=["reduce"], **args
        )

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "reduce"], outputs=["output"], axis=concat_axis
        )
        ...
        constant = np.power(
            np.sum(
                a=np.abs(np.power(constant, reduce_p)),
                axis=tuple(_axes),
                keepdims=keep_dims,
            ),
            1 / reduce_p,
    ...
    def test_reduce_lp_precommit(
        self, params, keep_dims, reduce_p, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_reduce_lp(
                **params, keep_dims=keep_dims, reduce_p=reduce_p, ir_version=ir_version
            ),
            ie_device,
            precision,
            ir_version,
    ...
    @pytest.mark.parametrize("reduce_p", [1, 2])
    @pytest.mark.precommit
    def test_reduce_lp_const_precommit(
        self, params, keep_dims, reduce_p, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_reduce_lp_const(
                **params, keep_dims=keep_dims, reduce_p=reduce_p, ir_version=ir_version
            ),
            ie_device,
            precision,
