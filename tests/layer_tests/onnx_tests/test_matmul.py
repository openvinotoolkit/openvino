...
class TestMatMul(OnnxRuntimeLayerTest):
    ...
    def create_net(self, shape1, shape2, precision, ir_version):
        ...
        extended_shape1 = np.concatenate(
            [np.ones(max_len - len(shape1)), shape1], axis=0
        )
        extended_shape2 = np.concatenate(
            [np.ones(max_len - len(shape2)), shape2], axis=0
        )
        output_shape = (
            np.concatenate(
                [
        ...
        node_def = onnx.helper.make_node(
            "MatMul", inputs=["input", "const"], outputs=["mm_output"]
        )

        # to avoid mapping problems
        node_elu_def = onnx.helper.make_node(
            "Elu", inputs=["mm_output"], outputs=["output"]
        )
    ...
    def test_dual_matmul(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_dual_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
