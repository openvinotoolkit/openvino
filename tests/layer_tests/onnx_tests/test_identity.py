...
class TestIdentity(OnnxRuntimeLayerTest):
    def create_net(self, shape, ir_version):
        ...
        node_def = helper.make_node("Identity", inputs=["input"], outputs=["identity"])

        sigmoid_def = helper.make_node(
            "Sigmoid", inputs=["identity"], outputs=["output"]
        )
    ...
    def create_net_const(self, shape, precision, ir_version):
        ...
        node_def = helper.make_node("Identity", inputs=["const1"], outputs=["identity"])

        node_concat_def = helper.make_node(
            "Concat", inputs=["input", "identity"], outputs=["output"], axis=concat_axis
        )
