...
test_data_3D = [
    ...
    dict(
        input_shape=[2, 50, 50],
        output_shapes=[[2, 20, 50], [2, 15, 50], [2, 15, 50]],
        axis=1,
    ),
    dict(
        input_shape=[4, 50, 50],
        output_shapes=[[1, 50, 50], [1, 50, 50], [1, 50, 50], [1, 50, 50]],
        axis=0,
    ),
]
...
test_data_4D = [
    dict(
        input_shape=[1, 32, 800, 800],
        output_shapes=[[1, 16, 800, 800], [1, 16, 800, 800]],
        axis=1,
    ),
    dict(
        input_shape=[4, 32, 80, 80],
        output_shapes=[[4, 8, 80, 80], [4, 8, 80, 80], [4, 8, 80, 80], [4, 8, 80, 80]],
        axis=1,
    ),
    dict(
        input_shape=[2, 21, 80, 80],
        output_shapes=[[2, 7, 80, 80], [2, 7, 80, 80], [2, 7, 80, 80]],
        axis=1,
    ),
    dict(
        input_shape=[3, 21, 80, 80],
        output_shapes=[[3, 14, 80, 80], [3, 5, 80, 80], [3, 2, 80, 80]],
        axis=1,
    ),
    dict(
        input_shape=[3, 21, 80, 80],
        output_shapes=[[1, 21, 80, 80], [1, 21, 80, 80], [1, 21, 80, 80]],
        axis=0,
    ),
    dict(
        input_shape=[3, 21, 80, 80],
        output_shapes=[[3, 21, 20, 80], [3, 21, 35, 80], [3, 21, 25, 80]],
        axis=2,
    ),
    dict(
        input_shape=[3, 21, 80, 80],
        output_shapes=[[3, 21, 80, 40], [3, 21, 80, 10], [3, 21, 80, 30]],
        axis=3,
    ),
]
...
test_data_5D = [
    ...
    dict(
        input_shape=[1, 50, 50, 80, 60],
        output_shapes=[[1, 25, 50, 80, 60], [1, 25, 50, 80, 60]],
        axis=1,
    ),
]
...
test_multiple_out = [
    dict(
        input_shape=[3, 10, 10],
        output_shapes=[[1, 10, 10], [1, 10, 10], [1, 10, 10]],
        axis=0,
        output_names=["h", "b", "l"],
    ),
    dict(
    ...
    dict(
        input_shape=[1, 4, 3],
        output_shapes=[[1, 1, 3], [1, 1, 3], [1, 1, 3], [1, 1, 3], [1, 1, 3]],
        axis=1,
...
class TestSplitConcat(OnnxRuntimeLayerTest):
    ...
    def create_split_concat_net(self, input_shape, output_shapes, axis, ir_version):
        ...
        for id, output_shape in enumerate(output_shapes):
            helper.make_tensor_value_info(
                "output_{}".format(id), TensorProto.FLOAT, output_shape
            )
            outputs.append("output_{}".format(id))
            split.append(output_shape[axis])

        # Output for concat
        output_concat = helper.make_tensor_value_info(
            "output_concat", TensorProto.FLOAT, input_shape
        )

        node_split_def = onnx.helper.make_node(
            "Split", inputs=["input"], outputs=outputs, axis=axis, split=split
        )

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=outputs, outputs=["output_concat"], axis=axis
        )
    ...
    # TODO Add test with default values (axis=0)
    def create_split_concat_net_const(
        self, input_shape, output_shapes, axis, ir_version
    ):
        """
        ONNX net                                               IR net
        ...
        # Output for concat
        output_concat = helper.make_tensor_value_info(
            "output_dyn_concat", TensorProto.FLOAT, concat_output_shape
        )
        ...
        node_split_def = onnx.helper.make_node(
            "Split", inputs=["const1"], outputs=outputs, axis=axis, split=split
        )
    ...
    def test_split_3D(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_split_concat_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    def test_split_3D_const(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_split_concat_net_const(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
...
class TestSplit(OnnxRuntimeLayerTest):
        ...
        for x in outputs:
            nodes.append(
                onnx.helper.make_node(
                    "Elu", inputs=["node_{}".format(x[1])], outputs=[x[1]]
                )
            )
    ...
    def create_split_net_ordered_outputs(
        self, input_shape, output_shapes, axis, output_names, ir_version
    ):
        """
        ONNX net                             IR net
        ...
        for i, output_name in enumerate(output_names):
            output_list.append(
                helper.make_tensor_value_info(
                    output_name, TensorProto.FLOAT, output_shapes[i]
                )
            )

        node = onnx.helper.make_node(
            "Split", inputs=["input"], outputs=output_names, axis=axis
        )
        ...
        onnx_net = onnx_make_model(
            graph_def, producer_name="test_split_model_outputs_order"
        )
    ...
    def create_split_net_ordered_outputs_with_add(
        self, input_shape, output_shapes, axis, output_names, ir_version
    ):
        """
        ...
        node = onnx.helper.make_node(
            "Split", inputs=["input"], outputs=outputs_without_add, axis=axis
        )
        ...
        output_list = (
            ...
            + [
                helper.make_tensor_value_info(
                    add_output_name1, TensorProto.FLOAT, output_shapes[0]
                )
            ]
            + [
                helper.make_tensor_value_info(
                    add_output_name2, TensorProto.FLOAT, output_shapes[0]
                )
            ]
        )
    ...
    def create_split_net_ordered_outputs_multiple_tensor_names(
        self,
        input_shape,
        output_shapes,
        axis,
        ...
        output_list.append(
            helper.make_tensor_value_info(
                identity_names[2], TensorProto.FLOAT, output_shapes[i]
            )
        )

        node = onnx.helper.make_node(
            "Split", inputs=["input"], outputs=split_out_names, axis=axis
        )
        identity1 = onnx.helper.make_node(
            "Identity", inputs=[split_out_names[0]], outputs=[identity_names[0]]
        )
        identity2 = onnx.helper.make_node(
            "Identity", inputs=[identity_names[0]], outputs=[identity_names[1]]
        )
        identity3 = onnx.helper.make_node(
            "Identity", inputs=[identity_names[1]], outputs=[identity_names[2]]
        )
    ...
    def test_split_3D(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_split_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    def test_split_outputs_order(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_split_net_ordered_outputs(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir,
    ...
    def test_split_outputs_order_multiple_connection_before_result_case(
        self, params, ie_device, precision, ir_version, temp_dir
    ):
        self._test(
            *self.create_split_net_ordered_outputs_with_add(
    ...
    def test_split_outputs_order_multiple_tensors_before_result_case(
        ...
        self._test(
            *self.create_split_net_ordered_outputs_multiple_tensor_names(
                **params, ir_version=ir_version
            ),
            ie_device,
            precision,
