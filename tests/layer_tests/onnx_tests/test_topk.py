...
class TestTopK(OnnxRuntimeLayerTest):
    def create_net(
        self, shape, k, axis, ir_version, largest=None, sorted=None, opset=None
    ):
        """
        ONNX net                    IR net
        ...
        indices = helper.make_tensor_value_info(
            "cindices", TensorProto.INT64, output_shape
        )
        ...
        node_def = onnx.helper.make_node(
            "TopK", inputs=inputs, outputs=["values", "indices"], **args
        )
        ...
        node_add1_def = onnx.helper.make_node(
            "Add", inputs=["indices", "const1"], outputs=["cindices"]
        )
        ...
        node_add2_def = onnx.helper.make_node(
            "Add", inputs=["values", "const2"], outputs=["cvalues"]
        )

        nodes.extend(
            [node_def, node_const1_def, node_add1_def, node_const2_def, node_add2_def]
        )
    ...
    def test_topk_opset11(
        self, params, ie_device, precision, ir_version, largest, sorted, temp_dir
    ):
        self._test(
            *self.create_net(
                **params,
