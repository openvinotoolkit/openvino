...
class TestSqueeze(OnnxRuntimeLayerTest):
    def create_squeeze_net(self, axes, input_shape, output_shape, ir_version):
        ...
        node_squeeze_def = onnx.helper.make_node(
            "Squeeze", inputs=["input"], outputs=["output"], axes=axes
        )
    ...
    def create_squeeze_net_const(self, axes, input_shape, output_shape, ir_version):
        ...
        node_squeeze_def = onnx.helper.make_node(
            "Squeeze", inputs=["const1"], outputs=["squeeze1"], axes=axes
        )

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "squeeze1"], outputs=["output"], axis=concat_axis
        )
    ...
    def test_squeeze_5D(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_squeeze_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
    ...
    def test_squeeze_const_5D(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(
            *self.create_squeeze_net_const(**params, ir_version=ir_version),
            ie_device,
            precision,
            ir_version,
            temp_dir=temp_dir
