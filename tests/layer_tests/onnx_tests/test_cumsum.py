...
class TestCumSum(OnnxRuntimeLayerTest):
    def create_net(self, shape, ir_version, axis=None, reverse=None, exclusive=None):
        ...
        node_def = onnx.helper.make_node(
            "CumSum", inputs=inputs, outputs=["output"], **args
        )
    ...
    def create_net_const(
        self, shape, precision, ir_version, axis=None, reverse=None, exclusive=None
    ):
        """
        ONNX net                                     IR net
        ...
        if reverse is not None:
            args["reverse"] = reverse
        node_def = onnx.helper.make_node(
            "CumSum", inputs=inputs, outputs=["cumsum"], **args
        )

        node_concat_def = onnx.helper.make_node(
            "Concat", inputs=["input", "cumsum"], outputs=["output"], axis=concat_axis
        )
    ...
    @pytest.mark.nightly
    def test_cumsum(
        self, params, reverse, exclusive, ie_device, precision, ir_version, temp_dir
    ):
        if "axis" not in params:
            pytest.skip("No axis cases fail in ONNX")
        elif "axis" in params and params["axis"] == -2 and exclusive == 1:
            pytest.skip(
                "Disabled due to an exception thrown by ONNXRuntime for this use case"
            )
        self._test(
            *self.create_net(
                **params, exclusive=exclusive, reverse=reverse, ir_version=ir_version
            ),
            ie_device,
            precision,
            ir_version,
