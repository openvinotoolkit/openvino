...
class TestResize(OnnxRuntimeLayerTest):
    def create_resize_net(
        ...
        onnx_scales = scales
        if scales is None:
            onnx_scales = np.array(output_shape).astype(float) / np.array(
                input_shape
            ).astype(float)
        ...
        onnx_coordinate_transformation_mode = (
            coordinate_transformation_mode or "half_pixel"
        )
        ...
        if check_ir_version(10, None, ir_version):
            ...
            if sizes is not None and scales is not None:
                ...
                scales_value = np.array(scales).astype(float)
                sizes_value = np.floor(
                    input_shape_as_array * scales_value + 1e-5
                ).astype(np.int64)
            ...
            if shape_calculation_mode == "scales":
                ref_net = create_ref_net_in_scales_mode(
                    precision,
                    input_shape_as_array,
                    output_shape,
                    sizes_value,
                    scales_value,
                    interp_attrs,
                ...
                ref_net = create_ref_net_in_sizes_mode(
                    precision,
                    input_shape_as_array,
                    output_shape,
                    sizes_value,
    ...
    test_data_cubic = [
        dict(
            input_shape=[1, 3, 100, 200],
            output_shape=[1, 3, 350, 150],
            scales=[1.0, 1.0, 3.5, 150 / 200],
            sizes=None,
        ),
        dict(
            input_shape=[16, 7, 190, 400],
            output_shape=[16, 7, 390, 600],
            scales=[1.0, 1.0, 390 / 190, 600 / 400],
            sizes=None,
        ),
        dict(
            input_shape=[4, 33, 1024, 800],
            output_shape=[4, 33, 512, 800],
            scales=[1.0, 1.0, 0.5, 1.0],
            sizes=None,
        ),
        dict(
            input_shape=[4, 33, 3, 800],
            output_shape=[4, 33, 1, 800],
            scales=[1.0, 1.0, 0.3333334, 1.0],
            sizes=None,
        ),
        dict(
            input_shape=[100, 200],
            output_shape=[350, 150],
            scales=[3.5, 150 / 200],
            sizes=None,
        ),
        dict(
            input_shape=[190, 400],
            output_shape=[390, 600],
            scales=[390 / 190, 600 / 400],
            sizes=None,
        ),
        dict(
            input_shape=[1024, 800],
            output_shape=[512, 800],
            scales=[0.5, 1.0],
            sizes=None,
        ),
        dict(
    ...
    def test_resize_combined_cubic(
        self,
        params,
        coordinate_transformation_mode,
        cubic_coeff_a,
    ...
    test_data_nearest = [
        ...
        dict(
            input_shape=[4, 33, 600, 800],
            output_shape=[4, 33, 300, 800],
            scales=[1.0, 1.0, 0.5, 1.0],
            sizes=None,
        ),
        dict(
    ...
    def test_resize_combined_nearest(
        self,
        params,
        coordinate_transformation_mode,
        cubic_coeff_a,
    ...
    @pytest.mark.parametrize("nearest_mode", ["round_prefer_floor"])
    def test_resize_combined_linear(
        self,
        params,
        coordinate_transformation_mode,
        cubic_coeff_a,
    ...
    test_data_sizes_cubic = [
        ...
        dict(
            input_shape=[4, 15, 700, 800],
            output_shape=[4, 15, 350, 800],
            scales=None,
            sizes=[4, 15, 350, 800],
        ),
        dict(
            input_shape=[4, 15, 3, 200],
            output_shape=[4, 15, 1, 200],
            scales=None,
            sizes=[4, 15, 1, 200],
        ),
        dict(
        ...
        dict(
            input_shape=[700, 800],
            output_shape=[350, 800],
            scales=None,
            sizes=[350, 800],
        ),
        dict(input_shape=[3, 200], output_shape=[1, 200], scales=None, sizes=[1, 200]),
    ]
    ...
    @pytest.mark.parametrize("mode", ["cubic"])
    @pytest.mark.parametrize("nearest_mode", ["round_prefer_floor"])
    def test_resize_combined_sizes_cubic(
        self,
        params,
        coordinate_transformation_mode,
        cubic_coeff_a,
    ...
    test_data_sizes_nearest = [
        ...
        dict(
            input_shape=[3, 100, 200],
            output_shape=[3, 350, 150],
            scales=None,
            sizes=[3, 350, 150],
        ),
        dict(
            input_shape=[7, 190, 400],
            output_shape=[7, 390, 600],
            scales=None,
            sizes=[7, 390, 600],
        ),
        dict(
            input_shape=[33, 600, 800],
            output_shape=[33, 300, 800],
            scales=None,
            sizes=[33, 300, 800],
        ),
        dict(
            input_shape=[33, 3, 800],
            output_shape=[33, 1, 800],
            scales=None,
            sizes=[33, 1, 800],
        ),
        dict(
        ...
        dict(input_shape=[3, 800], output_shape=[1, 800], scales=None, sizes=[1, 800]),
        dict(input_shape=[100], output_shape=[350], scales=None, sizes=[350]),
        dict(input_shape=[190], output_shape=[390], scales=None, sizes=[390]),
        dict(input_shape=[600], output_shape=[300], scales=None, sizes=[300]),
        dict(input_shape=[3], output_shape=[1], scales=None, sizes=[1]),
    ]
    ...
    def test_resize_combined_sizes_nearest(
        self,
        params,
        coordinate_transformation_mode,
        cubic_coeff_a,
    ...
    @pytest.mark.parametrize("mode", ["linear"])
    @pytest.mark.parametrize("nearest_mode", ["round_prefer_floor"])
    def test_resize_combined_sizes_linear(
        self,
        params,
        coordinate_transformation_mode,
        cubic_coeff_a,
...
def create_ref_net_in_sizes_mode(
    precision, input_shape, output_shape, sizes_value, scales_value, attrs
):
    ...
    nodes_attrs = {
        ...
        "shape_to_float": {
            "kind": "op",
            "type": "Convert",
            "destination_type": input_data_type,
        },
        ...
        "ss_scales_data": {
            "shape": int64_array([len(spatial_sizes_value)]),
            "kind": "data",
        },
        "ss_scales_begin_const_data": {
            "kind": "data",
            "value": int64_array([begin_dim]),
        },
...
def create_ref_net_in_scales_mode(
    ...
    nodes_attrs = {
        ...
        "strided_slice_data": {
            "shape": int64_array([len(spatial_scales_value)]),
            "kind": "data",
        },
