...
class TestFusedGemm(OnnxRuntimeLayerTest):
    ...
    def create_net(
        ...
        extended_shape1 = np.concatenate(
            [np.ones(max_len - len(shapeA)), shapeA], axis=0
        )
        extended_shape2 = np.concatenate(
            [np.ones(max_len - len(shapeB)), shapeB], axis=0
        )
        output_shape = (
            np.concatenate(
                [
                    np.maximum(*[extended_shape1[0:-2], extended_shape2[0:-2]]),
                    [shapeA[-2], shapeB[-1]],
                ],
                axis=0,
            )
    ...
    def test_fusedgemm(
        ...
        self._test(
            *self.create_net(
                ...
                trans_b,
                params["activation"],
                params["activation_alpha"],
                params["activation_beta"],
                params["activation_gamma"],
                opset,
                ir_version,
            ),
            ie_device,
