...
class TestPlus(object):
    ...
    def test_plus_precommit(self, ie_device, precision):
        self.plus(ie_device, precision)

    @pytest.mark.nightly
    def test_plus_nightly(self, ie_device, precision):
        self.plus(ie_device, precision)

    def plus(self, ie_device, precision):
        ...
        plus = network.add_layer(layer_type='Eltwise',
                                 layer_name='_plus0',
                                 inputs=[inputl, relu],
                                 operation='sum',
                                 get_out_shape_def=calc_same_out_shape,
                                 framework_representation_def=plus_to_symbol)
        ...
        ie_results = score_model(model_path=os.path.join(ir_path, network.name + '.xml'), device=ie_device, batch=1,
                                 image_path=img_path, out_blob_name=plus.name)

        assert compare_infer_results_with_mxnet(
            ie_results, network.name, plus.name, input_shape), "Comparing with MxNet failed."
