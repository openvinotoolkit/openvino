...
def get_activation_params(ie_device=None, precision=None):
    """
...
class TestActivation(object):
    ...
    def test_activation_relu_precommit(self, ie_device, precision):
        self.activation_relu(ie_device, precision)

    @pytest.mark.nightly
    def test_activation_relu_nightly(self, ie_device, precision):
        self.activation_relu(ie_device, precision)

    def activation_relu(self, ie_device, precision):
        ...
        relu = network.add_layer(layer_type='ReLU',
                                 ...
                                 framework_representation_def=activation_to_symbol)
        ...
        assert network.compare(network_cur, ignore_attributes={'ReLU': ['act_type']}), "Comparing of networks failed."
        ...
        assert compare_infer_results_with_mxnet(
            ie_results, network.name, relu.name, input_shape), "Comparing with MxNet failed."
        lg.info("[ TEST ] Comparing of scoring results passed")

    @pytest.mark.skip("Will be added after MR !456")
    @pytest.mark.precommit
    def test_activation_sigmoid_precommit(self, ie_device, precision):
        self.activation_relu(ie_device, precision)

    @pytest.mark.skip("Will be added after MR !456")
    @pytest.mark.nightly
    def test_activation_sigmoid_nightly(self, ie_device, precision):
        self.activation_relu(ie_device, precision)

    def activation_sigmoid(self, ie_device, precision):
        network = Net(precision=precision)
        inputl = network.add_layer(layer_type='Input',
                                   layer_name="data",
                                   get_out_shape_def=calc_out_shape_input_layer,
        ...
        assert network.compare(network_cur,
                               ignore_attributes={'Sigmoid': ['act_type']}), "Comparing of networks failed."
        lg.info("[ TEST ] Comparing of networks passed")

        ie_results = score_model(model_path=os.path.join(ir_path, network.name + '.xml'), device=ie_device,
                                 image_path=img_path, out_blob_name=sigmoid.name)

        assert compare_infer_results_with_mxnet(ie_results, network.name, sigmoid.name,
