...
class TestKerasRNN(CommonTF2LayerTest):
    def create_keras_rnn_net(self, input_names, input_shapes,
        ...
        y = tf.keras.layers.RNN(cell=cell, return_sequences=return_sequences, return_state=return_state,
                                go_backwards=go_backwards, stateful=stateful, unroll=unroll, time_major=time_major
                                )(x1)
