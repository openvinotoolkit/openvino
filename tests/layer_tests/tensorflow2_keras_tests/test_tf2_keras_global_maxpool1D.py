...
class TestKerasGlobalMaxPool1D(CommonTF2LayerTest):
    def create_keras_global_max_pooling1D_net(
        self, input_names, input_shapes, input_type, dataformat, ir_version
    ):
        """
    ...
    def test_keras_global_max_pooling1D_float32(
        self, params, ie_device, precision, temp_dir, ir_version, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_global_max_pooling1D_net(
                **params, ir_version=ir_version
            ),
            ie_device,
            precision,
            temp_dir=temp_dir,
