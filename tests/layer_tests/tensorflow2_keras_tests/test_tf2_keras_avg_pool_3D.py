...
class TestKerasAvgPool3D(CommonTF2LayerTest):
    def create_keras_avg_pool_3D_net(self, pool_size, strides, padding, data_format, input_names, input_shapes,
    ...
    test_data_float32 = [
        dict(pool_size=(3, 3, 3), strides=(2, 2, 2), padding='valid', data_format='channels_first', input_names=["x1"],
    ...
    def test_keras_avg_pool_3D_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_avg_pool_3D_net(**params, ir_version=ir_version), ie_device, precision,
                   temp_dir=temp_dir, ir_version=ir_version, **params)

    test_data_extended_float32 = [
        dict(pool_size=(3, 3, 3), strides=None, padding='same', data_format='channels_last', input_names=["x1"],
             input_shapes=[[3, 4, 5, 6, 7]], input_type=tf.float32),
        dict(pool_size=(5, 5, 5), strides=None, padding='same', data_format='channels_first', input_names=["x1"],
             input_shapes=[[3, 4, 5, 6, 7]], input_type=tf.float32),
        dict(pool_size=(5, 5, 5), strides=(3, 3, 3), padding='valid', data_format='channels_last', input_names=["x1"],
             input_shapes=[[3, 8, 7, 6, 5]], input_type=tf.float32)]

    @pytest.mark.parametrize("params", test_data_extended_float32)
    @pytest.mark.nightly
    def test_keras_avg_pool_3D_extended_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_avg_pool_3D_net(**params, ir_version=ir_version), ie_device, precision,
                   temp_dir=temp_dir, ir_version=ir_version, **params)
