...
class TestKerasAttention(CommonTF2LayerTest):
    def create_keras_attention_net(self, causal, dropout, use_scale, input_names, input_shapes, input_type, ir_version):
        ...
        y = tf.keras.layers.Attention(causal=causal, dropout=dropout, use_scale=use_scale)(inputs, training=False)
    ...
    test_data_float32 = [dict(use_scale=True, causal=True, dropout=0.5, input_names=["query", "value"],
    ...
    def test_keras_attention_float32_case1(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_attention_net(**params, ir_version=ir_version), ie_device, precision,
                   temp_dir=temp_dir, ir_version=ir_version, **params)

    test_data_extended_float32 = [
        dict(use_scale=False, causal=True, dropout=0.0, input_names=["input1_query", "input2_value"],
