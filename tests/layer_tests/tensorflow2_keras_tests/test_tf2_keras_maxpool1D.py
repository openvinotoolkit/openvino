...
class TestKerasMaxPool1D(CommonTF2LayerTest):
    def create_keras_maxpool1D_net(self, input_names, input_shapes, input_type, pool_size, strides, padding, dataformat,
                                   ir_version):
        """
        ...
        y = tf.keras.layers.MaxPool1D(pool_size=pool_size, strides=strides, padding=padding, data_format=dataformat)(x)
    ...
    def test_keras_maxpool1D_pool_strides_float32(self, params, ie_device, precision, temp_dir, ir_version):
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_keras_maxpool1D_padding_and_data_format(self, params, ie_device, precision, temp_dir, ir_version):
        self._test(*self.create_keras_maxpool1D_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version, **params)
