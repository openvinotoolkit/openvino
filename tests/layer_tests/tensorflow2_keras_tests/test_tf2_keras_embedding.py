...
class TestKerasEmbedding(CommonTF2LayerTest):
    def create_keras_emb_net(self, input_names, input_shapes, input_type, input_dim, output_dim, mask_zero,
        ...
        y = tf.keras.layers.Embedding(input_dim, output_dim, mask_zero=mask_zero, input_length=input_length)(layer)
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_keras_emb_without_zero_mask_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_emb_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version, **params)
