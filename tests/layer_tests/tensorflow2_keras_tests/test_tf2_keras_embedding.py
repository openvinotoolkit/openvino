...
class TestKerasEmbedding(CommonTF2LayerTest):
    def create_keras_emb_net(
        ...
        y = tf.keras.layers.Embedding(
            input_dim, output_dim, mask_zero=mask_zero, input_length=input_length
        )(layer)
    ...
    def test_keras_emb_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_emb_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_keras_emb_without_zero_mask_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_emb_net(**params, ir_version=ir_version),
