...
class TestKerasCropping2D(CommonTF2LayerTest):
    def create_keras_cropping_2d_net(self, cropping, input_names, input_shapes, input_type, ir_version):
    ...
    test_data_float32 = [
        pytest.param(dict(cropping=2, input_names=["x"], input_shapes=[[3, 5, 7, 5]], input_type=tf.float32),
                     marks=pytest.mark.precommit),

        dict(cropping=(1, 2), input_names=["x"], input_shapes=[[2, 3, 7, 5]], input_type=tf.float32),
    ...
    def test_keras_cropping_2d_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_cropping_2d_net(**params, ir_version=ir_version), ie_device, precision,
