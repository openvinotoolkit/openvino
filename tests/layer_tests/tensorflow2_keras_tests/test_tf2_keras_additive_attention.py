...
class TestKerasAdditiveAttention(CommonTF2LayerTest):
    def create_keras_additive_attention_net(self, causal, dropout, use_scale, input_names, input_shapes, input_type,
                                            ir_version):
        ...
        y = tf.keras.layers.AdditiveAttention(causal=causal, dropout=dropout, use_scale=use_scale)(inputs,
                                                                                                   training=False)
    ...
    test_data_float32 = [dict(use_scale=True, causal=True, dropout=0.0, input_names=["input1_query", "input2_value"],
                              input_shapes=[[5, 4, 3], [5, 4, 3]], input_type=tf.float32)]

    @pytest.mark.parametrize("params", test_data_float32)
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_keras_additive_attention_float32_case1(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_additive_attention_net(**params, ir_version=ir_version), ie_device, precision,
                   temp_dir=temp_dir, ir_version=ir_version,
                   **params)

    test_data_extended_float32 = [
        dict(use_scale=False, causal=True, dropout=0.5, input_names=["input1_query", "input2_value"],
             input_shapes=[[1, 5, 4], [1, 5, 4]], input_type=tf.float32),
        dict(use_scale=True, causal=False, dropout=0.5, input_names=["input1_query", "input2_value"],
             input_shapes=[[2, 1, 4], [2, 1, 4]], input_type=tf.float32),
        dict(use_scale=False, causal=False, dropout=0.8, input_names=["input1_query", "input2_value"],
    ...
    @pytest.mark.nightly
    def test_keras_additive_attention_float32_case2(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_additive_attention_net(**params, ir_version=ir_version), ie_device, precision,
