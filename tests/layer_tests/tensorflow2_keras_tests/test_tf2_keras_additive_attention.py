...
class TestKerasAdditiveAttention(CommonTF2LayerTest):
    def create_keras_additive_attention_net(
        self,
        causal,
        dropout,
        use_scale,
        ...
        y = tf.keras.layers.AdditiveAttention(
            causal=causal, dropout=dropout, use_scale=use_scale
    ...
    def test_keras_additive_attention_float32_case1(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_additive_attention_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
    ...
    test_data_extended_float32 = [
        ...
        dict(
            ...
            input_shapes=[[2, 4, 3], [2, 4, 3], [2, 4, 3]],
            input_type=tf.float32,
        ),
        dict(
            use_scale=False,
    ...
    @pytest.mark.nightly
    def test_keras_additive_attention_float32_case2(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_additive_attention_net(**params, ir_version=ir_version),
