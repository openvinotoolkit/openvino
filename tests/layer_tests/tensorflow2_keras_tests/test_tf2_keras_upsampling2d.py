...
class TestKerasUpSampling2D(CommonTF2LayerTest):
    ...
    def create_keras_upsampling2d_net(
        self, input_shapes, input_type, size, data_format, interpolation, ir_version
    ):
        ...
        y = tf.keras.layers.UpSampling2D(
            size=size, data_format=data_format, interpolation=interpolation
        )(x1)
    ...
    @pytest.mark.nightly
    def test_keras_upsampling2d_nearest(
        self,
        params,
        input_type,
        data_format,
        ...
        self._test(
            *self.create_keras_upsampling2d_net(
                **params,
