...
class TestKerasUpSampling2D(CommonTF2LayerTest):
    def create_keras_upsampling2d_net(self, input_names, input_shapes, input_type, size, data_format, interpolation,
                                      ir_version):
        ...
        y = tf.keras.layers.UpSampling2D(size=size, data_format=data_format, interpolation=interpolation)(x1)
    ...
    @pytest.mark.nightly
    def test_keras_upsampling2d_channels_first(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_upsampling2d_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version, **params)
