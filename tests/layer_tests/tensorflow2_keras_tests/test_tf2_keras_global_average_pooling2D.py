...
class TestKerasGlobalAvgPool2D(CommonTF2LayerTest):
    def create_keras_global_avg_pooling2D_net(
    ...
    test_data_float32 = [
        dict(
            input_names=["x"],
            input_shapes=[[5, 9, 4, 2]],
            input_type=tf.float32,
            dataformat="channels_last",
        ),
        dict(
            input_names=["x"],
            input_shapes=[[5, 3, 8, 8]],
            input_type=tf.float32,
    ...
    def test_keras_global_avg_pooling2D_float32(
        self, params, ie_device, precision, temp_dir, ir_version, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_global_avg_pooling2D_net(
