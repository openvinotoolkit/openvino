...
class TestKerasDropout(CommonTF2LayerTest):
    def create_keras_dropout_net(
        self, input_names, input_shapes, input_type, rate, noise_shape, ir_version
    ):
        """
        ...
        y = tf.keras.layers.Dropout(rate=rate, noise_shape=noise_shape, seed=4)(
            x, training=False
        )
    ...
    def test_keras_dropout_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_dropout_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
