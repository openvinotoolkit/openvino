...
class TestKerasGlobalMaxPool3D(CommonTF2LayerTest):
    def create_keras_global_maxpool3D_net(
    ...
    test_data_float32 = [
        dict(
            input_names=["x"],
            input_shapes=[[5, 2, 3, 4, 5]],
            input_type=tf.float32,
            dataformat="channels_last",
        ),
        dict(
            input_names=["x"],
            input_shapes=[[5, 12, 2, 5, 3]],
            input_type=tf.float32,
    ...
    def test_keras_global_maxpool3D_float32(
        self, params, ie_device, precision, temp_dir, ir_version, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_global_maxpool3D_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
