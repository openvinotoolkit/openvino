...
class TestKerasMaxPool2D(CommonTF2LayerTest):
    def create_keras_maxpool2D_net(self, input_names, input_shapes, input_type, pool_size, strides, padding, dataformat,
                                   ir_version):
        """
        ...
        y = tf.keras.layers.MaxPool2D(pool_size=pool_size, strides=strides, padding=padding, data_format=dataformat)(x)
    ...
    test_data_float32 = [
        dict(input_names=["x"], input_shapes=[[5, 4, 2, 6]], input_type=tf.float32, pool_size=(1, 1),
             strides=None, padding='valid', dataformat='channels_last'),
        dict(input_names=["x"], input_shapes=[[5, 8, 4, 8]], input_type=tf.float32, pool_size=(3, 4),
             strides=None, padding='valid', dataformat='channels_last'),
        dict(input_names=["x"], input_shapes=[[5, 3, 6, 4]], input_type=tf.float32, pool_size=(2, 2),
             strides=None, padding='valid', dataformat='channels_last'),
        ...
        dict(input_names=["x"], input_shapes=[[5, 4, 6, 6]], input_type=tf.float32, pool_size=(2, 3),
    ...
    def test_keras_maxpool2D_pool_strides_float32(self, params, ie_device, precision, temp_dir, ir_version):
