...
class TestKerasFlatten(CommonTF2LayerTest):
    def create_keras_flatten_net(
        self, input_names, input_shapes, input_type, data_format, ir_version
    ):
        """
    ...
    def test_keras_flatten_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_flatten_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
