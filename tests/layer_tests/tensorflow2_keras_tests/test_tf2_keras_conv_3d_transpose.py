...
class TestKerasConv3DTranspose(CommonTF2LayerTest):
    def create_keras_conv_3d_transpose_net(
        self, conv_params, input_names, input_shapes, input_type, ir_version
    ):
        ...
        if "activation" in conv_params:
            conv_params["activation"] = activation_func_structure[
        ...
        x = tf.keras.Input(
            shape=input_shapes[0][1:], dtype=input_type, name=input_names[0]
        )  # Variable-length sequence of ints

        y = tf.keras.layers.Conv3DTranspose(
            **conv_params, input_shape=input_shapes[0][1:]
        )(x)
    ...
    test_data_float32 = [
        dict(
            conv_params=dict(
                filters=27, kernel_size=3, padding="valid", strides=(1, 1, 2)
            ),
            input_names=["x"],
            input_shapes=[[5, 3, 5, 7, 6]],
        ...
        dict(
            conv_params=dict(
                filters=27, kernel_size=3, padding="valid", dilation_rate=3
            ),
            input_names=["x"],
            input_shapes=[[5, 8, 9, 7, 6]],
            input_type=tf.float32,
        ),
        dict(
            conv_params=dict(
                filters=10, kernel_size=3, padding="same", dilation_rate=4
            ),
            input_names=["x"],
            input_shapes=[[5, 3, 9, 7, 8]],
            input_type=tf.float32,
        ),
        dict(
            conv_params=dict(
    ...
    @pytest.mark.nightly
    def test_keras_conv_3D_transpose_float32(
        ...
        self._test(
            *self.create_keras_conv_3d_transpose_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
