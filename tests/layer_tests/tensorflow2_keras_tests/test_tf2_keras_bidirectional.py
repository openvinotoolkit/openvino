...
class TestKerasBidirectional(CommonTF2LayerTest):
    def create_keras_bidirectional_net(self, n_units, RNN_layer, input_names, input_shapes,
        ...
        backward_layer = RNN_layer(n_units, activation='relu', return_sequences=False, go_backwards=True)
    ...
    test_data_float32 = [dict(n_units=3, RNN_layer="LSTM", input_names=["x"], input_shapes=[[3, 4, 5]],
    ...
    def test_keras_bidirectional_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_bidirectional_net(**params, ir_version=ir_version), ie_device, precision,
