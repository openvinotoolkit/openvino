...
class TestKerasBidirectional(CommonTF2LayerTest):
    def create_keras_bidirectional_net(
        self, n_units, RNN_layer, input_names, input_shapes, input_type, ir_version
    ):
        RNN_layer_structure = {
        ...
        backward_layer = RNN_layer(
            n_units, activation="relu", return_sequences=False, go_backwards=True
        )
    ...
    @pytest.mark.nightly
    def test_keras_bidirectional_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_bidirectional_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
