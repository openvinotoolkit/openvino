...
class TestKerasMaxPool3D(CommonTF2LayerTest):
    def create_keras_maxpool3D_net(self, input_names, input_shapes, input_type, pool_size, strides, padding, dataformat,
                                   ir_version):
        """
        ...
        y = tf.keras.layers.MaxPool3D(pool_size=pool_size, strides=strides, padding=padding, data_format=dataformat)(x)
    ...
    test_data_float32 = [
        ...
        dict(input_names=["x"], input_shapes=[[2, 3, 4, 5, 6]], input_type=tf.float32, pool_size=2,
             strides=None, padding='valid', dataformat='channels_last'),
        dict(input_names=["x"], input_shapes=[[2, 3, 4, 5, 6]], input_type=tf.float32, pool_size=(2, 3, 4),
             strides=None, padding='valid', dataformat='channels_last'),
        ...
        dict(input_names=["x"], input_shapes=[[2, 6, 8, 4, 12]], input_type=tf.float32, pool_size=(3, 2, 4),
    ...
    def test_keras_maxpool3D_pool_strides_float32(self, params, ie_device, precision, temp_dir, ir_version):
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_keras_maxpool3D_padding_and_data_format(self, params, ie_device, precision, temp_dir, ir_version):
        self._test(*self.create_keras_maxpool3D_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version, **params)
