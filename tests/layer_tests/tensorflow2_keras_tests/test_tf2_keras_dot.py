...
class TestKerasDot(CommonTF2LayerTest):
    def create_keras_dot_net(
        self, input_names, input_shapes, input_type, axes, normalize, ir_version
    ):
        """
        ...
        inputs = [
            tf.keras.Input(shape=input_shapes[0][1:], name=input_names[0]),
    ...
    def test_keras_dot_normalize_float32(
        self, params, ie_device, precision, temp_dir, ir_version, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_dot_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
    ...
    test_data_difficult_axes_float32 = [
        ...
        dict(
            input_names=["x", "y"],
            input_shapes=[[5, 1, 64, 2], [5, 1, 16, 64]],
            input_type=tf.float32,
            axes=(2, 3),
            normalize=False,
        ),
        ...
        dict(
            input_names=["x", "y"],
            input_shapes=[[5, 1, 1024, 2], [5, 1, 8, 1024]],
            input_type=tf.float32,
            axes=(-2, -1),
            normalize=False,
        ),
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_keras_dot_difficult_axes_float32(
        self, params, ie_device, precision, temp_dir, ir_version, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_dot_net(**params, ir_version=ir_version),
    ...
    test_data_normalize_higher_rank = [
        ...
        dict(
            input_names=["x", "y"],
            input_shapes=[[5, 1, 1024, 2], [5, 1, 4, 1024]],
            input_type=tf.float32,
            axes=(-2, -1),
            normalize=True,
        ),
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_keras_dot_normalize_higher_rank(
        self, params, ie_device, precision, temp_dir, ir_version, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_dot_net(**params, ir_version=ir_version),
