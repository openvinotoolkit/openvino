...
class TestKerasConvLSTM2D(CommonTF2LayerTest):
    def _prepare_input(self, inputs_info):
        ...
        inputs_data = {}
        inputs_data[input_names[0]] = np.random.uniform(-1, 1, x_shape).astype(
            np.float32
        )
    ...
    def create_keras_conv_lstm_2d_net(self, params, input_shapes):
        ...
        if recurrent_activation is not None:
            params["recurrent_activation"] = tf.keras.activations.get(
    ...
    test_data_basic = [
        ...
        dict(
            params=dict(
                filters=3, kernel_size=(3, 3), padding="valid", return_sequences=False
            ),
    ...
    @pytest.mark.nightly
    def test_keras_conv_lstm_2d_basic(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_conv_lstm_2d_net(**params),
            ie_device,
            precision,
            temp_dir=temp_dir,
