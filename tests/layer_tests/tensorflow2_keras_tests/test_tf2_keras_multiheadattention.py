...
class TestKerasMultiHeadAttention(CommonTF2LayerTest):
    def create_keras_multiheadattention_net(
        self,
        input_names,
        input_shapes,
        input_types,
        ...
        query = tf.keras.Input(
            shape=input_shapes[0][1:], name=input_names[0], dtype=input_types[0]
        )
        value = tf.keras.Input(
            shape=input_shapes[1][1:], name=input_names[1], dtype=input_types[1]
        )
        inputs = [query, value]
        key = None
        if len(input_shapes) > 2:
            key = tf.keras.Input(
                shape=input_shapes[2][1:], name=input_names[2], dtype=input_types[2]
            )
            inputs.append(key)
        attention_mask = None
    ...
    def test_keras_multiheadattention(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_multiheadattention_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
    ...
    @pytest.mark.nightly
    def test_keras_multiheadattention_no_bias(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_multiheadattention_net(**params, ir_version=ir_version),
