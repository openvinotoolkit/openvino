...
class TestKerasMultiHeadAttention(CommonTF2LayerTest):
    def create_keras_multiheadattention_net(self,
        ...
        if len(input_shapes) > 2:
            key = tf.keras.Input(shape=input_shapes[2][1:], name=input_names[2], dtype=input_types[2])
            inputs.append(key)
        attention_mask = None
        ...
        y = tf.keras.layers.MultiHeadAttention(num_heads=num_heads, key_dim=key_dim, value_dim=value_dim,
                                               dropout=dropout, use_bias=use_bias,
                                               output_shape=output_shape,
                                               attention_axes=attention_axes)(query=query, value=value,
                                                                              key=key, attention_mask=attention_axes,
    ...
    test_data = [
        ...
        pytest.param(
            dict(input_names=["query", "value", "key"], input_shapes=[[3, 8, 16], [3, 5, 16], [3, 5, 16]],
            ...
            marks=pytest.mark.xfail(reason="45432")),
        pytest.param(
            dict(input_names=["query", "value", "key"], input_shapes=[[1, 8, 16], [1, 2, 4], [1, 2, 4]],
                 input_types=[tf.float32, tf.float32, tf.float32], attention_mask_value=None,
    ...
    test_data_no_bias = [
        pytest.param(
            ...
            marks=pytest.mark.xfail(reason="45432")),
        pytest.param(
            dict(input_names=["query", "value", "key"], input_shapes=[[2, 8, 16], [2, 4, 16], [2, 4, 16]],
                 input_types=[tf.float32, tf.float32, tf.float32], attention_mask_value=None,
    ...
    @pytest.mark.nightly
    def test_keras_multiheadattention_no_bias(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_multiheadattention_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version,
                   **params)
