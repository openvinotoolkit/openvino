...
class TestKerasGru(CommonTF2LayerTest):
    def create_keras_gru_net(self, input_names, input_shapes, input_type, units, activation, recurrent_activation,
        ...
        y = tf.keras.layers.GRU(units=units, activation=activation, recurrent_activation=recurrent_activation,
                                use_bias=use_bias, dropout=dropout, recurrent_dropout=recurrent_dropout,
                                return_sequences=False, return_state=False, go_backwards=go_backwards,
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_keras_gru_without_bias_float32(self, params, ie_device, precision, temp_dir, ir_version):
        self._test(*self.create_keras_gru_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version, **params)
    ...
    def test_keras_gru_flags_zero_recurrent_dropout_float32(self, params, ie_device, precision, temp_dir, ir_version):
        self._test(*self.create_keras_gru_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version, **params)
