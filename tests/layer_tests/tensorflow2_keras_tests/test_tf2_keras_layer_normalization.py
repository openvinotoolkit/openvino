...
class TestKerasLayerNormalization(CommonTF2LayerTest):
    def create_keras_lnorm_net(self, input_names, input_shapes, input_type, axis, epsilon, center, scale, ir_version):
        """
        ...
        y = tf.keras.layers.LayerNormalization(axis=axis, epsilon=epsilon, center=center, scale=scale,
                                               trainable=False)(x1)
    ...
    test_data_float32_precommit = [dict(input_names=["x"], input_shapes=[[2, 2, 3, 5]], input_type=tf.float32,
    ...
    test_data_float32 = [
        ...
        dict(input_names=["x"], input_shapes=[[3, 3, 5]], input_type=tf.float32, axis=1, epsilon=1e-7,
             center=True, scale=False),
        dict(input_names=["x"], input_shapes=[[2, 3, 8]], input_type=tf.float32, axis=2, epsilon=1e-6,
             center=False, scale=True),
        dict(input_names=["x"], input_shapes=[[2, 2, 3, 5]], input_type=tf.float32, axis=(1, 2, 3), epsilon=1e-5,
