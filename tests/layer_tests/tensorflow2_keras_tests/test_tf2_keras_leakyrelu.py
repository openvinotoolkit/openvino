...
class TestKerasLeakyRelu(CommonTF2LayerTest):
    def create_keras_leaky_relu_net(
    ...
    test_data_float32 = [
        ...
        dict(
            input_names=["x"],
            input_shapes=[[5, 2, 3, 4]],
            input_type=tf.float32,
            alpha=-1.0,
        ),
        dict(
            input_names=["x"],
            input_shapes=[[5, 2, 3, 5, 3]],
            input_type=tf.float32,
            alpha=-1.7,
        ),
    ]
    ...
    def test_keras_leaky_relu_float32(
        self, params, ie_device, precision, temp_dir, ir_version, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_leaky_relu_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
