...
class TestKerasGRUCell(CommonTF2LayerTest):
    def create_keras_grucell_net(self, input_names, input_shapes, input_type, units, activation, recurrent_activation,
        ...
        cell = tf.keras.layers.GRUCell(units=units, activation=activation, recurrent_activation=recurrent_activation,
    ...
    test_data_simple = [
        dict(input_names=["x"], input_shapes=[[2, 3, 4]], input_type=tf.float32, units=1,
             activation='relu', recurrent_activation='sigmoid', dropouts=(.0, .0), use_bias=True, reset_after=True),
        dict(input_names=["x"], input_shapes=[[5, 4, 6]], input_type=tf.float32, units=4,
             activation='elu', recurrent_activation='tanh', dropouts=(.2, .1), use_bias=False, reset_after=True),
        dict(input_names=["x"], input_shapes=[[3, 7, 5]], input_type=tf.float32, units=5,
             activation='linear', recurrent_activation='tanh', dropouts=(.9, .1), use_bias=True, reset_after=False),
        dict(input_names=["x"], input_shapes=[[5, 3, 3]], input_type=tf.float32, units=4,
             activation='softmax', recurrent_activation='sigmoid', dropouts=(.4, .2), use_bias=False, reset_after=False)
    ]
