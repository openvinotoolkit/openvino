...
class TestKerasPReLU(CommonTF2LayerTest):
    def create_keras_prelu_net(self, input_names, input_shapes, input_type, shared_axes, ir_version):
    ...
    test_data_float32_precommit = [
        dict(input_names=["x1"], input_shapes=[[5, 4, 8, 3, 2]], input_type=tf.float32, shared_axes=None)]
    ...
    test_data_float32 = [
        ...
        dict(input_names=["x1"], input_shapes=[[5, 4, 8, 3]], input_type=tf.float32, shared_axes=None),
        dict(input_names=["x1"], input_shapes=[[5, 4, 8, 3, 2]], input_type=tf.float32, shared_axes=None)]
    ...
    test_data_float32_shared_axes = [
        ...
        dict(input_names=["x1"], input_shapes=[[5, 4, 8, 3]], input_type=tf.float32, shared_axes=[1, 2]),
        dict(input_names=["x1"], input_shapes=[[5, 4, 8, 3, 2]], input_type=tf.float32, shared_axes=[1, 2, 3])]

    @pytest.mark.parametrize("params", test_data_float32_shared_axes)
    @pytest.mark.nightly
    def test_keras_prelu_float32_shared_axes(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_prelu_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version, **params)
