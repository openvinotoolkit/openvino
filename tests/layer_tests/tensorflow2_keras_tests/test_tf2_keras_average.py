...
class TestKerasAverage(CommonTF2LayerTest):
    ...
    def test_keras_average_case1_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_average_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
    ...
    test_data_extended_float32 = [
        dict(input_names=["x1", "x2"], input_shapes=[[1], [1]], input_type=tf.float32),
        dict(input_names=["x1", "x2"], input_shapes=[[5], [5]], input_type=tf.float32),
        dict(
            input_names=["x1", "x2"],
    ...
    @pytest.mark.nightly
    def test_keras_average_case2_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_average_net(**params, ir_version=ir_version),
    ...
    @pytest.mark.nightly
    def test_keras_average_float32_several_inputs(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_average_net(**params, ir_version=ir_version),
