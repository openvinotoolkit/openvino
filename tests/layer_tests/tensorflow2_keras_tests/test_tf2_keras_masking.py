...
class TestKerasMasking(CommonTF2LayerTest):
    def create_keras_masking_net(
        self, input_names, input_shapes, input_type, mask_value, ir_version
    ):
        """
    ...
    test_data_float32 = [
        ...
        dict(
            input_names=["x"],
            input_shapes=[[5, 4, 8, 16]],
            input_type=tf.float32,
            mask_value=1.0,
        ),
        dict(
            input_names=["x"],
            input_shapes=[[9, 4, 8, 47]],
            input_type=tf.float32,
            mask_value=0.3,
        ),
        dict(
    ...
    def test_keras_masking_float32(
        self, params, ie_device, precision, temp_dir, ir_version, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_masking_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
