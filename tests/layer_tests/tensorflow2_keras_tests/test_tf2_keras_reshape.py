...
class TestKerasReshape(CommonTF2LayerTest):
    def create_keras_reshape_net(
        self, input_names, input_shapes, input_type, target_shape, ir_version
    ):
    ...
    def test_keras_reshape(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_reshape_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
