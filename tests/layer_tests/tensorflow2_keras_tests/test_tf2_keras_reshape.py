...
class TestKerasReshape(CommonTF2LayerTest):
    def create_keras_reshape_net(self, input_names, input_shapes, input_type, target_shape, ir_version):
    ...
    test_data = [
        ...
        dict(input_names=["x1"], input_shapes=[[5, 4, 8]], input_type=tf.float32, target_shape=(2, 2, 8)),
        dict(input_names=["x1"], input_shapes=[[5, 4, 8, 3]], input_type=tf.float32, target_shape=(4, 24)),
        dict(input_names=["x1"], input_shapes=[[5, 4, 8, 3, 2]], input_type=tf.float32, target_shape=(32, 3, 2))]
