...
class TestKerasStackedRNNCells(CommonTF2LayerTest):
    def create_keras_stackedrnncells_net(
        self, input_names, input_shapes, input_type, rnn_cells, ir_version
    ):
        cells_structure = {
    ...
    def test_keras_stackedrnncells(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_stackedrnncells_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
