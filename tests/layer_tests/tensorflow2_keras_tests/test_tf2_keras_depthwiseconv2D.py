...
class TestKerasDepthwiseConv2D(CommonTF2LayerTest):
    def create_keras_dconv2D_net(self, input_names, input_shapes, input_type, kernel_size, strides, padding,
                                 depth_multiplier, data_format, dilation_rate, activation, use_bias, ir_version):
        """
        ...
        y = tf.keras.layers.DepthwiseConv2D(kernel_size=kernel_size, strides=strides, padding=padding,
    ...
    test_data_format_padding = [
        ...
        dict(input_names=["x"], input_shapes=[[5, 8, 16, 4]], input_type=tf.float32,
             kernel_size=(2, 2), strides=1, padding='same', depth_multiplier=2, data_format='channels_last',
             dilation_rate=(2, 2), activation=None, use_bias=False),
        dict(input_names=["x"], input_shapes=[[5, 16, 8, 4]], input_type=tf.float32,
             kernel_size=(2, 2), strides=(3, 3), padding='same', depth_multiplier=54, data_format='channels_first',
