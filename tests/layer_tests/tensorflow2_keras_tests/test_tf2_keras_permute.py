...
class TestKerasPermute(CommonTF2LayerTest):
    def create_keras_permute_net(
        self, input_names, input_shapes, input_type, dims, ir_version
    ):
    ...
    test_data_float32 = [
        ...
        dict(
            input_names=["x1"],
            input_shapes=[[2, 3, 8, 2]],
            input_type=tf.float32,
            dims=(3, 2, 1),
        ),
        dict(
            input_names=["x1"],
            input_shapes=[[1, 4, 1, 4, 2]],
            input_type=tf.float32,
            dims=(1, 2, 3, 4),
        ),
    ]
    ...
    def test_keras_permute_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_permute_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
