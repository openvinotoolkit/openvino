...
class TestKerasUpSampling3D(CommonTF2LayerTest):
    def create_keras_upsampling3d_net(
        self, input_names, input_shapes, input_type, size, data_format, ir_version
    ):
    ...
    def test_keras_upsampling3(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_upsampling3d_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
    ...
    @pytest.mark.nightly
    def test_keras_upsampling2d_channels_first(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_upsampling3d_net(**params, ir_version=ir_version),
