...
class TestKerasConv1DTranspose(CommonTF2LayerTest):
    def create_keras_conv1d_transpose_net(
        self, params, input_names, input_shapes, input_type, ir_version
    ):
        ...
        y = tf.keras.layers.Conv1DTranspose(**params, input_shape=input_shapes[0][1:])(
            x
        )
    ...
    test_data_float32_set1 = [
        dict(
            params=dict(filters=27, kernel_size=3, padding="valid", strides=2),
            input_names=["x"],
            input_shapes=[[5, 7, 6]],
        ...
        dict(
            params=dict(filters=27, kernel_size=3, padding="valid", dilation_rate=3),
            input_names=["x"],
            input_shapes=[[5, 7, 6]],
            input_type=tf.float32,
        ),
        dict(
            params=dict(
                filters=20, kernel_size=7, padding="valid", data_format="channels_first"
            ),
            input_names=["x"],
            input_shapes=[[5, 7, 8]],
            input_type=tf.float32,
        ...
        dict(
            params=dict(filters=10, kernel_size=5, padding="same", strides=3),
            input_names=["x"],
            input_shapes=[[5, 7, 8]],
            input_type=tf.float32,
        ),
        dict(
            params=dict(filters=20, kernel_size=7, padding="valid", strides=4),
            input_names=["x"],
            input_shapes=[[5, 7, 8]],
            input_type=tf.float32,
        ),
    ...
    def test_keras_conv_1d_case1_transpose_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_conv1d_transpose_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
