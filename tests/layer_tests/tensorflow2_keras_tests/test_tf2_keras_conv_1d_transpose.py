...
class TestKerasConv1DTranspose(CommonTF2LayerTest):
    def create_keras_conv1d_transpose_net(self, params, input_names, input_shapes, input_type, ir_version):
    ...
    test_data_float32_set1 = [
        pytest.param(dict(params=dict(filters=27, kernel_size=3, padding="valid", strides=2), input_names=["x"],
                          input_shapes=[[5, 7, 6]], input_type=tf.float32),
                     marks=pytest.mark.precommit),
        pytest.param(dict(params=dict(filters=10, kernel_size=5, padding="same", activation="relu", use_bias=True),
                          input_names=["x"], input_shapes=[[5, 7, 8]], input_type=tf.float32),
                     marks=pytest.mark.precommit),

        pytest.param(dict(params=dict(filters=27, kernel_size=3, padding="valid", dilation_rate=3), input_names=["x"],
                          input_shapes=[[5, 7, 6]], input_type=tf.float32),
                     marks=pytest.mark.xfail(reason="49505")),
        pytest.param(dict(params=dict(filters=20, kernel_size=7, padding="valid", data_format="channels_first"),
                          input_names=["x"], input_shapes=[[5, 7, 8]], input_type=tf.float32),
                     marks=pytest.mark.xfail(reason="49505")),
        ...
        dict(params=dict(filters=20, kernel_size=7, padding="valid", strides=4), input_names=["x"],
             input_shapes=[[5, 7, 8]], input_type=tf.float32),
        dict(params=dict(filters=27, kernel_size=3, padding="valid", dilation_rate=3), input_names=["x"],
             input_shapes=[[5, 7, 6]], input_type=tf.float32),
        dict(params=dict(filters=20, kernel_size=7, padding="valid", data_format="channels_first"), input_names=["x"],
             input_shapes=[[5, 7, 8]], input_type=tf.float32),
    ]
    ...
    def test_keras_conv_1d_case1_transpose_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_conv1d_transpose_net(**params, ir_version=ir_version), ie_device, precision,
