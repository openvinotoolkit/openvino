...
class TestKerasLocallyConnected2D(CommonTF2LayerTest):
    def create_keras_locally_connected2D_net(
        self,
        input_names,
        input_shapes,
        input_type,
        ...
        y = tf.keras.layers.LocallyConnected2D(
            filters,
            kernel_size,
            strides=strides,
            padding=padding,
    ...
    def test_keras_locally_connected2D_float32(
        self, params, ie_device, precision, temp_dir, ir_version, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_locally_connected2D_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_keras_locally_connected2D_channels_first_float32(
        self, params, ie_device, precision, temp_dir, ir_version, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_locally_connected2D_net(**params, ir_version=ir_version),
