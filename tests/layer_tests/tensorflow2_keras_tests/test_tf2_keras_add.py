...
class TestKerasAdd(CommonTF2LayerTest):
    ...
    test_data_float32_precommit = [
        dict(input_names=["x1", "x2"], input_shapes=[[5, 4, 8, 2, 3], [5, 4, 8, 2, 3]], input_type=tf.float32)]
    ...
    def test_keras_add_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_add_net(**params, ir_version=ir_version),
                   ie_device, precision, ir_version=ir_version, temp_dir=temp_dir, **params)
    ...
    @pytest.mark.precommit
    def test_keras_add_float32_several_inputs_precommit(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_add_net(**params, ir_version=ir_version),
                   ie_device, precision, ir_version=ir_version, temp_dir=temp_dir, **params)
    ...
    @pytest.mark.nightly
    def test_keras_add_float32_several_inputs(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_add_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version, **params)
