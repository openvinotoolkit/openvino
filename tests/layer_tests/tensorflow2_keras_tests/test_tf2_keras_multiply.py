...
class TestKerasMultiply(CommonTF2LayerTest):
    ...
    test_data_float32_precommit = [dict(input_names=["x1", "x2"], input_shapes=[[5, 4, 8, 2, 3], [5, 4, 8, 2, 3]],
    ...
    test_data_float32_several_inputs_precommit = [
        dict(input_names=["x1", "x2", "x3"], input_shapes=[[5, 4, 8, 2, 3], [5, 4, 8, 2, 3], [5, 4, 8, 2, 3]],
             input_type=tf.float32)]

    @pytest.mark.parametrize("params", test_data_float32_several_inputs_precommit)
    @pytest.mark.precommit
    def test_keras_multiply_float32_several_inputs(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_multiply_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version, **params)
