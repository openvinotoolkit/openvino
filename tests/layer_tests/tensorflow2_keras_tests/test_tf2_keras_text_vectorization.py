...
class TestTextVectorization(CommonTF2LayerTest):
    ...
    def create_text_vectorization_net(
        self,
        input_shapes,
        vocabulary,
        output_mode,
        ...
        text_input = tf.keras.Input(
            shape=input_shapes[0][1:], name="text_input", dtype=tf.string
        )
        vectorized_text = TextVectorization(
    ...
    @pytest.mark.parametrize(
        ...
        [
            [
                "hi OpenVINO here  ",
                "  hello OpenVINO there",
                "hello PyTorch here  ",
                "  hi TensorFlow here",
                "  hi JAX here \t",
            ],
            [
                "привет ОПЕНВИНО здесь  ",
                "  привет ОпенВИНО там",
                "привет Пайторч здесь  ",
                "  привет ТензорФлоу здесь",
                "  привет ДЖАКС там \t",
            ],
            [
                "這裡你好 OpenVINO ",
    ...
    def test_text_vectorization(
        ...
        self._test(
            *self.create_text_vectorization_net(**params),
            ie_device,
            precision,
            temp_dir=temp_dir,
