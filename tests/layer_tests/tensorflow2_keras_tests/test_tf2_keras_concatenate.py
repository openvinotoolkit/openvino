...
class TestKerasConcatenate(CommonTF2LayerTest):
    ...
    test_data_float32 = [dict(axis=2, input_names=["x1", "x2"], input_shapes=[[5, 4, 8, 2], [5, 4, 8, 2]],
    ...
    def test_keras_concatenate_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_concatenate_net(**params, ir_version=ir_version), ie_device, precision,
                   temp_dir=temp_dir, ir_version=ir_version, **params)

    test_data_extended_float32 = [
        dict(axis=-1, input_names=["x1", "x2"], input_shapes=[[5, 4], [5, 4]], input_type=tf.float32),
        dict(axis=2, input_names=["x1", "x2"], input_shapes=[[5, 4, 8], [5, 4, 8]], input_type=tf.float32),
        ...
        dict(axis=2, input_names=["x1", "x2", "x3"],
             input_shapes=[[5, 4, 8, 2, 3], [5, 4, 8, 2, 3], [5, 4, 8, 2, 3]], input_type=tf.float32)
    ]

    @pytest.mark.parametrize("params", test_data_extended_float32)
    @pytest.mark.nightly
    def test_keras_concatenate_extended_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_concatenate_net(**params, ir_version=ir_version), ie_device, precision,
                   temp_dir=temp_dir, ir_version=ir_version, **params)
