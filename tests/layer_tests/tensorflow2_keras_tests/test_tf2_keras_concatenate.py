...
class TestKerasConcatenate(CommonTF2LayerTest):
    def create_keras_concatenate_net(
        self, axis, input_names, input_shapes, input_type, ir_version
    ):
        ...
        for ind in range(len(input_names)):
            inputs.append(
                tf.keras.Input(shape=input_shapes[ind][1:], name=input_names[ind])
            )
    ...
    def test_keras_concatenate_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_concatenate_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
    ...
    test_data_extended_float32 = [
        dict(
            axis=-1,
            input_names=["x1", "x2"],
            input_shapes=[[5, 4], [5, 4]],
            input_type=tf.float32,
        ),
        dict(
            axis=2,
            input_names=["x1", "x2"],
            input_shapes=[[5, 4, 8], [5, 4, 8]],
            input_type=tf.float32,
        ),
        dict(
            axis=3,
    ...
    @pytest.mark.nightly
    def test_keras_concatenate_extended_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_concatenate_net(**params, ir_version=ir_version),
