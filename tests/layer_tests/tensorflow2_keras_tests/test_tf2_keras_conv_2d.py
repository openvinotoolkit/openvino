...
class TestKerasConv2D(CommonTF2LayerTest):
    ...
    test_data_float32 = [
        dict(
            conv_params=dict(
                filters=27, kernel_size=3, padding="valid", strides=(2, 2)
            ),
            input_names=["x"],
            input_shapes=[[3, 5, 7, 6]],
        ...
        dict(
            ...
            input_names=["x"],
            input_shapes=[[3, 5, 7, 8]],
            input_type=tf.float32,
        ),
        dict(
            conv_params=dict(filters=10, kernel_size=5, padding="same", strides=(3, 3)),
            input_names=["x"],
            input_shapes=[[3, 5, 7, 8]],
            input_type=tf.float32,
        ),
        dict(
            ...
            input_names=["x"],
            input_shapes=[[3, 9, 7, 6]],
            input_type=tf.float32,
        ),
        dict(
            conv_params=dict(
                filters=10, kernel_size=5, padding="same", dilation_rate=4
            ),
            input_names=["x"],
            input_shapes=[[3, 9, 7, 8]],
            input_type=tf.float32,
        ),
        dict(
            conv_params=dict(
    ...
    @pytest.mark.nightly
    def test_keras_conv_2d_float32(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_conv2d_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
