...
class TestKerasSoftmax(CommonTF2LayerTest):
    def create_keras_softmax_net(self, input_names, input_shapes, input_type, ir_version):
        ...
        x1 = tf.keras.Input(shape=input_shapes[0][1:], dtype=input_type, name=input_names[0])  # Variable-length sequence of ints
    ...
    test_data_float32_precommit = [dict(input_names=["x1"], input_shapes=[[5, 4, 8, 3, 2]], input_type=tf.float32)]
