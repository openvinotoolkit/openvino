...
class TestKerasTimeDistributed(CommonTF2LayerTest):
    def create_keras_timedistributed_net(
        self, input_names, input_shapes, input_type, conv_2d_layer, ir_version
    ):
    ...
    def test_keras_timedistributed(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_timedistributed_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
