...
class TestKerasSpatialDropout2D(CommonTF2LayerTest):
    def create_keras_spatialdropout2d_net(
        self, input_names, input_shapes, input_type, rate, data_format, ir_version
    ):
        # create TensorFlow 2 model with Keras SpatialDropout2D operation
        tf.keras.backend.clear_session()
        x1 = tf.keras.Input(
            shape=input_shapes[0][1:], name=input_names[0], dtype=input_type
        )
        y = tf.keras.layers.SpatialDropout2D(rate=rate, data_format=data_format)(
            x1, False
        )
    ...
    def test_keras_spatialdropout2d(
        ...
        self._test(
            *self.create_keras_spatialdropout2d_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
    ...
    @pytest.mark.nightly
    def test_keras_spatialdropout2d_channels_first(
        self, params, ie_device, precision, ir_version, temp_dir, use_legacy_frontend
    ):
        self._test(
            *self.create_keras_spatialdropout2d_net(**params, ir_version=ir_version),
