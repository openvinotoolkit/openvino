...
class TestKerasCropping1D(CommonTF2LayerTest):
    def create_keras_cropping_1d_net(self, cropping, input_names, input_shapes, input_type, ir_version):
    ...
    test_data_float32 = [
        pytest.param(dict(cropping=2, input_names=["x"], input_shapes=[[3, 5, 4]], input_type=tf.float32),
    ...
    def test_keras_cropping_1d_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_cropping_1d_net(**params, ir_version=ir_version), ie_device, precision,
