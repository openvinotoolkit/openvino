...
class TestKerasLSTM(CommonTF2LayerTest):
    def create_keras_lstm_net(self, input_names, input_shapes, input_type, units, activation, recurrent_activation,
        ...
        y = tf.keras.layers.LSTM(units=units, activation=activation, recurrent_activation=recurrent_activation,
    ...
    test_data_without_bias = [
        dict(input_names=["x"], input_shapes=[[2, 2, 7]], input_type=tf.float32, units=1, activation='tanh',
             recurrent_activation='sigmoid', dropouts=(.0, .3), use_bias=False, flags=(False, False)),
        dict(input_names=["x"], input_shapes=[[3, 8, 3]], input_type=tf.float32, units=4, activation='relu',
             recurrent_activation='sigmoid', dropouts=(.7, .4), use_bias=False, flags=(False, False)),
        dict(input_names=["x"], input_shapes=[[4, 2, 2]], input_type=tf.float32, units=2, activation='elu',
    ...
    @pytest.mark.nightly
    @pytest.mark.precommit
    def test_keras_lstm_without_bias_float32(self, params, ie_device, precision, temp_dir, ir_version):
        self._test(*self.create_keras_lstm_net(**params, ir_version=ir_version),
                   ie_device, precision, temp_dir=temp_dir, ir_version=ir_version, **params)

    test_data_different_flags = [
        dict(input_names=["x"], input_shapes=[[2, 3, 2]], input_type=tf.float32, units=1, activation='elu',
             recurrent_activation='sigmoid', dropouts=(.0, .3), use_bias=True, flags=(False, True)),
        dict(input_names=["x"], input_shapes=[[4, 8, 3]], input_type=tf.float32, dropouts=(.1, .3), units=3,
             activation='relu', use_bias=False, recurrent_activation='tanh', flags=(True, False)),
        dict(input_names=["x"], input_shapes=[[4, 2, 7]], input_type=tf.float32, units=5, activation='relu',
