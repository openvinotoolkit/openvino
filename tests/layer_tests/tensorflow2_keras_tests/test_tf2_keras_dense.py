...
class TestKerasDense(CommonTF2LayerTest):
    ...
    def test_keras_dense_float32(
        ...
        self._test(
            *self.create_keras_dense_net(**params, ir_version=ir_version),
            ie_device,
            precision,
            temp_dir=temp_dir,
    ...
    test_data_float32_activation = [
        ...
        pytest.param(
            ...
            marks=pytest.mark.skip(reason="110006"),
        ),
        dict(
            input_names=["x"],
            input_shapes=[[5, 4, 8, 8]],
            input_type=tf.float32,
            units=5,
            activation="linear",
            use_bias=True,
