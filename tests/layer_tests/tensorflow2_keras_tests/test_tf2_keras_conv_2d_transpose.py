...
class TestKerasConv2DTranspose(CommonTF2LayerTest):
    def create_keras_conv_2d_transpose_net(self, conv_params, input_names, input_shapes, input_type, ir_version):
    ...
    test_data_float32 = [
        pytest.param(dict(conv_params=dict(filters=27, kernel_size=3, padding="valid", strides=(2, 2),
                                           data_format="channels_first"), input_names=["x"],
                          input_shapes=[[3, 5, 7, 6]], input_type=tf.float32),
                     marks=pytest.mark.precommit),
        pytest.param(dict(conv_params=dict(filters=10, kernel_size=5, padding="same", strides=(7, 7),
                                           activation="relu", use_bias=True, output_padding=(3, 3)),
                          input_names=["x"], input_shapes=[[3, 5, 7, 8]], input_type=tf.float32),
                     marks=pytest.mark.precommit),
        dict(conv_params=dict(filters=10, kernel_size=5, padding="same", strides=(7, 7), output_padding=(5, 5)),
             input_names=["x"], input_shapes=[[3, 5, 7, 8]], input_type=tf.float32),
        ...
        dict(conv_params=dict(filters=27, kernel_size=3, padding="valid", dilation_rate=3, activation="sigmoid",
                              use_bias=False), input_names=["x"], input_shapes=[[3, 9, 7, 6]], input_type=tf.float32),
    ...
    @pytest.mark.nightly
    def test_keras_conv_2d_transpose_float32(self, params, ie_device, precision, ir_version, temp_dir):
        self._test(*self.create_keras_conv_2d_transpose_net(**params, ir_version=ir_version), ie_device, precision,
