...
import pytest
import requests
from models_hub_common.utils import get_params, round_num
...
def pytest_html_results_table_row(report, cells):
    ...
    cells.insert(
        4, html.td(round_num(report._results.converted_model_results.infer_mean_time))
    )
    cells.insert(
        5, html.td(round_num(report._results.read_model_results.infer_mean_time))
    )
