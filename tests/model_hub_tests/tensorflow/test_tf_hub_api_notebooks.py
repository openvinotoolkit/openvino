...
class TestTFHubApiNotebooks(TestConvertModel):
    def load_model(self, model_name, model_link):
        if model_name == "mobilenet_v2_100_224_dict":
            image = tf.keras.layers.Input(
                shape=(224, 224, 3), dtype=tf.float32, name="image"
            )
            ...
            classification_model = tf.keras.Model(
                inputs={"image": image}, outputs={"softmax": softmax}
            )
            ...
            film_layer = hub.KerasLayer(
                "https://www.kaggle.com/models/google/film/frameworks/tensorFlow2/variations/film/versions/1"
            )(inputs)
            film_model = tf.keras.Model(
                inputs=inputs, outputs=list(film_layer.values())[0]
            )
            return film_model
        else:
    ...
    @pytest.mark.parametrize(
        "model_name", ["mobilenet_v2_100_224_dict", "mobilenet_v2_100_224_list", "film"]
    )
