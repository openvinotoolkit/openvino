...
def write_to_log(model_name, model_link, ie_device, converted_time, real_time, diff):
    """
    It would be better to append this info to resulted html. Html is generated by pytest_html module.
    But unfortunately pytest creates separate processes for running tests. The only thing we can do
    is just to add output process log into resulted html. We could create a separate channel between
    child and parent testing processes to send this data. Writing the separate log is much simpler.
    File is cleaned each tests start in pytest_sessionstart() conftest.py.
    """
    if not performance_results_path:
        return
    with open(performance_results_path, 'a') as f:
        f.write('model_name={} model_link={} ie_device={} converted_time={} real_time={} diff={}\n'.format(model_name, model_link, ie_device, converted_time, real_time, diff))
...
class TestPerformanceModel:
    ...
    def _run(self, model_name, model_link, ie_device):
        try:
            ...
            diff = abs((converted_model_time - read_model_time)/read_model_time)
            write_to_log(model_name, model_link, ie_device, converted_model_time, read_model_time, diff)
