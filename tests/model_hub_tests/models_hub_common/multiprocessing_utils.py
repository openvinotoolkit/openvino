...
import sys
import traceback
from multiprocessing import Process, ProcessError, Queue, TimeoutError
from queue import Empty as QueueEmpty
from typing import Callable, Union


def _mp_wrapped_func(
    func: Callable, func_args: list, queue: Queue, logger_queue: Queue
):
    """
...
def multiprocessing_run(
    ...
    process = Process(
        target=_mp_wrapped_func, args=(func, func_args, queue, logger_queue)
    )
    process.start()
    try:
    ...
    ret_args = (
        ret_args[0] if len(ret_args) == 1 else ret_args
    )  # unwrap from list if only 1 item is returned
    return ret_args
...
def multiprocessing_exitcode_to_signal(exitcode):
    ...
    if exit_code > 0:
        code_map = {
            int(getattr(signal, sig)): str(getattr(signal, sig))
