...
import pytest
import torch
from torch_utils import TestTorchConvertModel, get_models_list, process_pytest_marks
...
class TestTorchbenchmarkConvertModel(TestTorchConvertModel):
    _model_list_path = os.path.join(os.path.dirname(__file__), "torchbench_models")
    ...
    def setup_class(self):
        ...
        subprocess.check_call(
            ["git", "checkout", "dbc109791dbb0dfb58775a5dc284fc2c3996cb30"],
            cwd=self.repo_dir.name,
        )

    def load_model(self, model_name, model_link):
        subprocess.check_call(
            [sys.executable, "install.py"] + [model_name], cwd=self.repo_dir.name
        )
        sys.path.append(self.repo_dir.name)
        from torchbenchmark import load_model_by_name

        try:
            model_cls = load_model_by_name(model_name)("eval", "cpu", jit=False)
        except:
