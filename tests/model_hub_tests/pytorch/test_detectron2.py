...
import pytest
import torch
from models_hub_common.utils import compare_two_tensors, get_models_list
...
class TestDetectron2ConvertModel(TestTorchConvertModel):
    ...
    def load_model(self, model_name, model_link):
        ...
        from detectron2 import export, model_zoo
        from detectron2.checkpoint import DetectionCheckpointer
        from detectron2.config import CfgNode
        from detectron2.modeling import PanopticFPN, build_model
    ...
    def teardown_class(self):
        subprocess.run([sys.executable, "-m", "pip", "uninstall", "-y", "detectron2"])

    @pytest.mark.parametrize(
        "name,type,mark,reason",
        get_models_list(
    ...
    @pytest.mark.parametrize(
        ...
        process_pytest_marks(
            os.path.join(os.path.dirname(__file__), "detectron2_models")
        ),
    )
    @pytest.mark.nightly
