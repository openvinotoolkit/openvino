...
class TestTransformersModel(TestTorchConvertModel):
    ...
    def load_model(self, name, type):
        ...
        if "clip_vision_model" in mi.tags:
            ...
            example = {
                "pixel_values": torch.randn(*(16, 3, 224, 224), dtype=torch.float32)
            }
        elif "audio-spectrogram-transformer" in mi.tags:
            example = {
                "input_values": torch.randn(*(1, 1024, 128), dtype=torch.float32)
            }
            ...
            example = dict(
                processor(
                    images=self.image, task_inputs=["semantic"], return_tensors="pt"
                )
            )
            ...
            example_inputs_map = {
                ...
                "audio_projection": {
                    "hidden_states": torch.randn([1, 768], dtype=torch.float32)
                },
            }
            ...
            example_inputs_map = {
                ...
                "qformer": {
                    "query_embeds": torch.randn([1, 32, 768], dtype=torch.float32),
                    "encoder_hidden_states": torch.randn(
                        [1, 257, 1408], dtype=torch.float32
                    ),
                    "encoder_attention_mask": torch.ones([1, 257], dtype=torch.int64),
                },
                "language_projection": {
                    "input": torch.randn([1, 32, 768], dtype=torch.float32)
                },
            }
            example = example_inputs_map[name_suffix]
        elif "t5" in mi.tags:
            ...
            model = VisionEncoderDecoderModel.from_pretrained(name, **model_kwargs)
            ...
            input_values = processor(
                torch.randn(16000).numpy(), sampling_rate=16_000, return_tensors="pt"
            )
            ...
            example["decoder_input_ids"] = (
                torch.ones(
                    (inputs.input_ids.shape[0] * model.decoder.num_codebooks, 1),
                    dtype=torch.long,
                )
                * pad_token_id
            )
            ...
            try:
                if auto_model == "AutoModelForCausalLM":
                    ...
                    processor = AutoProcessor.from_pretrained(name)
                    input_values = processor(
                        torch.randn(1000).numpy(), return_tensors="pt"
                    )
    ...
    def load_model_with_default_class(name, **kwargs):
        try:
            mi = model_info(name)
            assert (
                len({"owlv2", "owlvit", "vit_mae"}.intersection(mi.tags)) == 0
            ), "TBD: support default classes of these models"
    ...
    @pytest.mark.parametrize(
        ...
        process_pytest_marks(
            os.path.join(os.path.dirname(__file__), "hf_transformers_models")
        ),
    )
    @pytest.mark.nightly
