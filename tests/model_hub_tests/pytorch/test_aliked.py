...
from openvino import Model, PartialShape, Type, convert_model
...
class TestAlikedConvertModel(TestTorchConvertModel):
    def setup_class(self):
        ...
        subprocess.check_call(
            ["git", "checkout", "6008af43942925eec7e32006814ef41fbd0858d8"],
        ...
        subprocess.check_call(
            [
                ...
                "install",
                "-r",
                os.path.join(self.repo_dir.name, "requirements.txt"),
            ]
        )
        subprocess.check_call(
            ["sh", "build.sh"], cwd=os.path.join(self.repo_dir.name, "custom_ops")
        )
    ...
    def load_model(self, model_name, model_link):
        ...
        img_tensor = read_image(
            os.path.join(self.repo_dir.name, "assets", "st_pauls_cathedral"), 1
        )
        self.example = (img_tensor,)
        img_tensor2 = read_image(
            os.path.join(self.repo_dir.name, "assets", "st_pauls_cathedral"), 2
        )
