...
```bash
python scripts/classify_ov_component.py "$ERROR_CONTEXT" > ov_classify.json
```
...
1. Run quick export check (without full compilation):
   ```bash
   python scripts/quick_inference_check.py --model-id "$MODEL_ID" --patches-dir agent-results/openvino-orchestrator/patches/ --skip-compile
   ```
...
gh pr create \
  ...
  --body "Auto-generated by MEAT pipeline. Model: $MODEL_ID. Ops: $OP_TITLE." \
  --draft
```
...
Consult `scripts/classify_ov_component.py` output and search existing transformations:
```bash
grep -r "class.*Fusion" /tmp/openvino/src/common/transformations/include/ | head -20
```
