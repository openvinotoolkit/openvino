  ...
  for (i = 0; i < n; ++i) {
    for (j = i + 1; j < n; ++j) {
      if (res[i].probability < res[j].probability) {
        ...
        res[i] = res[j];
        res[j] = temp;
      } else if (res[i].probability == res[j].probability &&
                 res[i].class_id > res[j].class_id) {
        struct classify_res temp = res[i];
        res[i] = res[j];
        res[j] = temp;
      }
...
struct classify_res *output_blob_to_classify_res(ie_blob_t *blob, size_t *n) {
  ...
  *n = output_dim.dims[1];
  ...
  struct classify_res *cls =
      (struct classify_res *)malloc(sizeof(struct classify_res) * (*n));
  if (!cls) {
    return NULL;
  }
 ...
 * @return none
 */
void print_classify_res(struct classify_res *cls, size_t n,
                        const char *img_path) {
  printf("\nImage %s\n", img_path);
  printf("\nclassid probability\n");
  printf("------- -----------\n");
...
int main(int argc, char **argv) {
   ...
   * inference */
  status |= ie_network_set_input_resize_algorithm(network, input_name,
                                                  RESIZE_BILINEAR);
  ...
  if (status != OK) {
    fprintf(stderr,
            "ERROR ie_exec_network_create_infer_request status %d, line %d\n",
            status, __LINE__);
    goto err;
  }
  // -----------------------------------------------------------------------------------------------------
  ...
  dimensions_t dimens = {4,
                         {1, (size_t)img.mat_channels, (size_t)img.mat_height,
                          (size_t)img.mat_width}};
  ...
  // memory
  status = ie_blob_make_memory_from_preallocated(&tensorDesc, img.mat_data,
                                                 size, &imgBlob);
  if (status != OK) {
    fprintf(stderr,
            "ERROR ie_blob_make_memory_from_preallocated status %d, line %d\n",
            status, __LINE__);
    image_free(&img);
    goto err;
  }
  ...
  size_t class_num;
  struct classify_res *cls =
      output_blob_to_classify_res(output_blob, &class_num);
