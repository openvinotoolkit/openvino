...
class DataQueue:
    def __init__(self, input_data: dict, sizes: int):
    ...
    def get_first_input(self):
        ...
        for input_name, input_tensors in self.input_data.items():
            data[input_name] = input_tensors[0][0]
        return data, 0

    def get_next_input(self):
        data = {}
        for input_name, input_tensors in self.input_data.items():
            data[input_name] = input_tensors[self.current_id][0]
        self.current_id += 1
        return data, self.current_id-1

    def has_next_input(self):
        if self.current_id < self.size:
            return True
        return False
...
def get_group_batch_sizes(app_input_info):
    ...
    for i in range(niter):
        ...
        for info in app_input_info:
            batch_index = info.layout.get_index_by_name(
                'N') if info.layout.has_name('N') else -1
            logger.info(
                f"info={info.name}  batch_index={batch_index}, niter={niter}")
            if batch_index != -1:
    ...
    for info in app_input_info:
        if info.layout.has_name('N'):
            if info.is_dynamic:
                ...
                batch_sizes_map[info.name] = [
                    len(info.getDimensionByLayout('N'))]
        else:
...
def verify_objects_to_be_used(objects_to_be_used_map: Dict[str, List[str]], info: AppInputInfo, total_frames: int, input_type_name: str):
    if objects_to_be_used_map[info.name] > total_frames and objects_to_be_used_map[info.name] % total_frames != 0:
        objects_to_be_used_map[info.name] = objects_to_be_used_map[info.name] - \
            objects_to_be_used_map[info.name] % total_frames
...
def fill_tensors_with_randoms(info, count):
    data = []
    for i in range(count):
        data.append(fill_tensors_with_random(info))
    return data


def init_random_input_data(app_input_info, count):
    for info in app_input_info:
        if info.shapes:
            if not (info.is_image_info):
                logger.warning(
                    f"No input files were given for input '{info.name}'!. This input will be filled with random values!")
        else:
            raise Exception(
                f"Input {info.name} is dynamic. Provide data shapes!")
    ...
    return DataQueue(data, count)


def get_input_data(paths_to_input, app_input_info, count):
    image_mapping, numpy_mapping, binary_mapping = get_input_file_mappings(
        paths_to_input, app_input_info)
    ...
    for info in app_input_info:
        if info.shapes:
            ...
            if info.name in image_mapping:
                verify_objects_to_be_used(
                    images_to_be_used_map, info, total_frames, "images")
            elif info.name in numpy_mapping:
                verify_objects_to_be_used(
                    numpys_to_be_used_map, info, total_frames, "numpy arrays")
            elif info.name in binary_mapping:
                verify_objects_to_be_used(
                    binaries_to_be_used_map, info, total_frames, "binaries")
            else:
                if not (info.is_image_info and len(image_sizes) == 1):
                    logger.warning(
                        f"No input files were given for input '{info.name}'!. This input will be filled with random values! image_sizes={image_sizes}")
        else:
            if info.name in image_mapping:
                logger.info(
                    f"Images given for input '{info.name}' will be processed with original shapes.")
            elif info.name in numpy_mapping:
                logger.info(
                    f"Numpy arrays given for input '{info.name}' will be processed with original shapes.")
            else:
                raise Exception(
                    f"Input {info.name} is dynamic. Provide data shapes!")
            ...
            if extension == "npy":
                ...
                if process_with_original_shapes:
                    ...
                    try:
                        if info.layout.has_name("N"):
                            numpy_arrays[[
                                None] * info.layout.get_index_by_name("N") + [b]] = numpy_arr
                        else:
    ...
    for i in range(niter):
        ...
        for b in range(current_batch_size):
            ...
            if extension == "bin":
                ...
                from_file = np.reshape(np.fromfile(
                    binary_filename, dtype), shape)
                if info.layout.has_name("N"):
                    binaries[[None] *
                             info.layout.get_index_by_name("N") + [b]] = from_file
                else:
                    binaries = from_file
            else:
                raise Exception(
...
def fill_tensors_with_random(layer):
    ...
    # if dtype == bool else (np.iinfo(np.uint8).min, np.iinfo(np.uint8).max)
    ...
    for shape in layer.shapes:
        if shape:
            ...
            if np.float32 == dtype:
                data = Tensor(
                    (rs.randint(-127, 127, list(shape))/127.0).astype(dtype))
            elif np.int32 == dtype:
                data = Tensor(
                    (rs.randint(900, 1000, list(shape))).astype(dtype))
            else:
                data = Tensor(
                    (rs.randint(rand_min, rand_max, list(shape))).astype(dtype))
            input_tensors.append(data)
        else:
            scalar = rs.uniform(rand_min, rand_max)
            input_tensors.append(
                Tensor(np.ndarray([], dtype, np.array(scalar).astype(dtype))))
...
def get_input_file_mappings(paths_to_inputs, app_input_info):
    ...
    if paths_to_inputs is not None:
        for path in paths_to_inputs:
            image_dict, numpy_dict, binary_dict = parse_path(
                path, app_input_info)
...
def parse_path(path, app_input_info):
    ...
    wrong_names = list(
        name for name in parsed_names if name not in input_names + input_node_names)
    if wrong_names:
        raise Exception(
    ...
    tensor_names = [parsed_name if parsed_name in input_names else input_names[input_node_names.index(
        parsed_name)] for parsed_name in parsed_names]
    input_pathes = [path for path in re.split(
        r"(?=[^,])(?![a-zA-Z]:\\)[\w\.]+:", path) if path]
    input_path_mapping = defaultdict(list)
    # input mapping is used
    if tensor_names:
        input_path_mapping = {input_: files.strip(",").split(
            ",") for input_, files in zip(tensor_names, input_pathes)}
    else:
        input_files = list()
        ...
        for _input_path in _input_pathes:
            ...
            if input_path.exists():
                ...
                raise Exception(
                    f"Path '{str(input_path)}' doesn't exist \n {str(input_path)}")
        num_files, num_inputs = len(input_files), len(app_input_info)
        if num_inputs > 1:
            logger.warning(
                f"Model has {num_inputs} inputs. It's recommended to use name mapping to specify parameters for each input.")
        ...
        inputs_to_fill = list(
            info.name for info in app_input_info if not info.is_image_info)
        for i in range(num_files):
            input_path_mapping[inputs_to_fill[i %
                                              len(inputs_to_fill)]].append(input_files[i])
    ...
    for input_name, _input_pathes in input_path_mapping.items():
        for _input_path in _input_pathes:
            ...
            if input_path.exists():
                ...
                raise Exception(
                    f"Path for input '{input_name}' doesn't exist \n {str(input_path)}")
