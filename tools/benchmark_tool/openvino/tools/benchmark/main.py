...
from openvino.tools.benchmark.utils.constants import (
    AUTO_DEVICE_NAME,
    BLOB_EXTENSION,
    CPU_DEVICE_NAME,
    GPU_DEVICE_NAME,
...
from openvino.tools.benchmark.utils.statistics_report import (
    CsvStatisticsReport,
...
from openvino.tools.benchmark.utils.utils import (
    can_measure_as_static,
    check_for_static,
    dump_config,
    dump_exec_graph,
    ...
    get_inputs_info,
    get_latency_groups,
    get_network_batch_size,
    get_number_iterations,
    is_virtual_device,
    is_virtual_device_found,
    load_config,
    next_step,
    parse_devices,
    parse_value_for_virtual_device,
    parse_value_per_device,
    pre_post_processing,
    print_inputs_and_outputs_info,
    print_perf_counters,
    print_perf_counters_sort,
...
def parse_and_check_command_line():
    ...
    if args.latency_percentile < 1 or args.latency_percentile > 100:
        parser.print_help()
        raise RuntimeError(
            "The percentile value is incorrect. The applicable values range is [1, 100]."
        )

    if not args.perf_hint == "none" and (
        arg_not_empty(args.number_streams, "")
        or arg_not_empty(args.number_threads, 0)
        or arg_not_empty(args.infer_threads_pinning, "")
    ):
        raise Exception(
            "-nstreams, -nthreads and -pin options are fine tune options. To use them you "
    ...
    if (
        args.report_type == "average_counters"
        and MULTI_DEVICE_NAME in args.target_device
    ):
        raise Exception(
            "only detailed_counters report type is supported for MULTI device"
        )

    _, ext = os.path.splitext(args.path_to_model)
    is_network_compiled = True if ext == BLOB_EXTENSION else False
    is_precisiton_set = not (
        args.input_precision == ""
        and args.output_precision == ""
        and args.input_output_precision == ""
    )

    if is_network_compiled and is_precisiton_set:
        raise Exception(
            "Cannot set precision for a compiled model. "
...
def main():
    ...
    try:
        ...
        if args.report_type:
            _statistics_class = (
                JsonStatisticsReport if args.json_stats else CsvStatisticsReport
            )
            statistics = _statistics_class(
        ...
        device_number_streams = parse_value_per_device(
            devices, args.number_streams, "nstreams"
        )
        ...
        if (
            GPU_DEVICE_NAME in config.keys()
        ...
        def set_performance_hint(device):
            ...
            supported_properties = benchmark.core.get_property(
                device, properties.supported_properties()
            )
            if properties.hint.performance_mode() in supported_properties:
                if is_flag_set_in_command_line("hint"):
                    if args.perf_hint == "throughput" or args.perf_hint == "tput":
                        ...
                        args.perf_hint == "cumulative_throughput"
                        or args.perf_hint == "ctput"
                    ):
                        perf_hint = (
                            properties.hint.PerformanceMode.CUMULATIVE_THROUGHPUT
                        )
                    ...
                    perf_hint = (
                        ...
                        else properties.hint.PerformanceMode.THROUGHPUT
                    )
                    logger.warning(
                        f"Performance hint was not explicitly specified in command line. "
                        + f"Device({device}) performance hint will be set to {perf_hint}."
                    )
                config[device][properties.hint.performance_mode()] = perf_hint
            else:
                logger.warning(
                    f"Device {device} does not support performance hint property(-hint)."
                )
        ...
        for device in devices:
            ...
            if is_flag_set_in_command_line("nireq"):
                config[device][properties.hint.num_requests()] = str(
                    args.number_infer_requests
                )
            ...
            if is_flag_set_in_command_line("pc"):
                ...
                config[device][properties.enable_profiling()] = (
                    True if args.perf_counts else False
                )
            elif (
                properties.enable_profiling() in config[device].keys()
                and config[device][properties.enable_profiling()] == True
            ):
                logger.warning(
                ...
                config[device][properties.enable_profiling()] = (
                    True if args.perf_counts_sort else False
                )
            else:
                ## set to default value
            ...
            perf_counts = (
                True
                if config[device][properties.enable_profiling()] == True
                else perf_counts
            )

            ## insert or append property into hw device properties list
            def update_configs(hw_device, property_name, property_value):
                (key, value) = properties.device.properties(
                    {hw_device: {property_name: property_value}}
                )
            ...
            def update_device_config_for_virtual_device(value, config, key):
                ...
                if len(value.split(":")) == 1:
                    ...
                    device_properties = {
                        value_vec[i].split(":")[0]: value_vec[i].split(":")[1]
                ...
                if device in device_infer_precision.keys():
                    ...
                    if key in supported_properties:
                        ...
                        update_device_config_for_virtual_device(
                            device_infer_precision[device], config, key
                        )
                    else:
                        raise Exception(
                            f"Device {device} doesn't support config key INFERENCE_PRECISION_HINT!"
            ...
            def set_throughput_streams():
                ...
                if device in device_number_streams.keys():
                    ...
                    if key in supported_properties:
                        ...
                        key = properties.streams.num()
                        update_device_config_for_virtual_device(
                            device_number_streams[device], config, key
                        )
                    else:
                        raise Exception(
                    ...
                    key not in config[device].keys()
                    and args.api_type == "async"
                    and key not in config[device].keys()
                    and "PERFORMANCE_HINT" in config[device].keys()
                    and config[device]["PERFORMANCE_HINT"] == ""
                ):
                    ...
                    if key in supported_properties:
                        config[device][key] = (
                            get_device_type_from_name(device) + "_THROUGHPUT_AUTO"
                        )
                    elif properties.streams.Num() in supported_properties:
                        key = properties.streams.Num()
                        ...
                        for hw_device in hw_devices_list:
                            hw_supported_properties = benchmark.core.get_property(
                                hw_device, properties.supported_properties()
                            )
                            key = (
                                get_device_type_from_name(hw_device)
                                + "_THROUGHPUT_STREAMS"
                            )
                            value = (
                                get_device_type_from_name(hw_device)
            ...
            def set_nthreads_pin(property_name, property_value):
                ...
                if (
                    property_name in supported_properties
                    or device_name == AUTO_DEVICE_NAME
                ):
            ...
            if args.number_threads and is_flag_set_in_command_line("nthreads"):
                ...
                set_nthreads_pin(
                    properties.inference_num_threads(), str(args.number_threads)
                )
        ...
        load_from_file_enabled = is_flag_set_in_command_line(
            "load_from_file"
        ) or is_flag_set_in_command_line("lfile")
        if load_from_file_enabled and not is_network_compiled:
            ...
            compiled_model = benchmark.core.compile_model(
                args.path_to_model, benchmark.device, device_config
            )
            duration_ms = (
                f"{(datetime.utcnow() - start_time).total_seconds() * 1000:.2f}"
            )
            ...
            app_inputs_info, _ = get_inputs_info(
                args.shape,
                args.data_shape,
                args.layout,
                args.batch_size,
            ...
            app_inputs_info, reshape = get_inputs_info(
                args.shape,
                args.data_shape,
                args.layout,
                args.batch_size,
            ...
            if reshape:
                ...
                logger.info(
                    "Reshaping model: {}".format(
                        ", ".join(
                            "'{}': {}".format(k, str(v)) for k, v in shapes.items()
                        )
                    )
                )
            ...
            start_time = datetime.utcnow()
            compiled_model = benchmark.core.compile_model(
                model, benchmark.device, device_config
            )
            ...
            start_time = datetime.utcnow()
            compiled_model = benchmark.core.import_model(
                args.path_to_model, benchmark.device, device_config
            )
            duration_ms = (
                f"{(datetime.utcnow() - start_time).total_seconds() * 1000:.2f}"
        ...
        if not allow_inference_only_or_sync and benchmark.api_type == "sync":
            raise Exception(
                "Benchmarking of the model with dynamic shapes is available for async API only. "
        ...
        if benchmark.inference_only == None:
            ...
            raise Exception(
                "Benchmarking dynamic model available with input filling in measurement loop only!"
            )
        ...
        # Iteration limit
        benchmark.niter = get_number_iterations(
            benchmark.niter,
            benchmark.nireq,
            max(len(info.shapes) for info in app_inputs_info),
            benchmark.api_type,
        )
        ...
        if statistics:
            statistics.add_parameters(
                ...
                [
                    ...
                    (
                        "duration (ms)",
                        str(get_duration_in_milliseconds(benchmark.duration_seconds)),
                    ),
                ],
            )
        ...
        if benchmark.inference_only:
            ...
            logger.info(
                "Benchmarking in full mode (inputs filling are included in measurement loop)."
            )
        ...
        (
            ...
            requests, data_queue, batch_size, args.latency_percentile, pcseq
        )
        ...
        if args.dump_config:
            ...
            logger.info(
                f"OpenVINO configuration settings were dumped to {args.dump_config}"
            )
        ...
        if perf_counts:
            ...
            if args.perf_counts_sort:
                total_sorted_list = print_perf_counters_sort(
                    perfs_count_list, sort_flag=args.perf_counts_sort
                )
                if statistics:
        ...
        if statistics:
            ...
            if MULTI_DEVICE_NAME not in device_name:
                ...
                if args.latency_percentile == 50:
                    ...
                    latency_prefix = (
                        "latency (" + str(args.latency_percentile) + " percentile) (ms)"
                    )
                if latency_prefix:
                    statistics.add_parameters(
                        StatisticsReport.Category.EXECUTION_RESULTS,
        ...
        if MULTI_DEVICE_NAME not in device_name:
            ...
            if args.latency_percentile == 50:
                ...
                logger.info(
                    f"   {args.latency_percentile} percentile:     {median_latency_ms:.2f} ms"
                )
            ...
            if pcseq:
                ...
                for idx, group in enumerate(benchmark.latency_groups):
                    ...
                    if args.latency_percentile == 50:
                        ...
                        logger.info(
                            f"   {args.latency_percentile} percentile:     {group.median:.2f} ms"
                        )
        ...
        if statistics:
            statistics.add_parameters(
                StatisticsReport.Category.EXECUTION_RESULTS, [("error", str(e))]
            )
            statistics.dump()
        sys.exit(1)
