    ...
    try:
        ...
        if not args.perf_hint:
            for device in devices:
                supported_config_keys = benchmark.core.get_property(device, 'SUPPORTED_CONFIG_KEYS').value
        ...
        for device in devices:
            ...
            for k in keys.value:
                try:
                    logger.info(f'  {k}  , {benchmark.core.get_property(device, k).value}')
                except:
                    pass
        ...
        for device in device_number_streams.keys():
            key = get_device_type_from_name(device) + '_THROUGHPUT_STREAMS'
            device_number_streams[device] = benchmark.core.get_property(device, key).value
