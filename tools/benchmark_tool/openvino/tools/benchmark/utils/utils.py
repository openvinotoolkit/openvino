...
def next_step(additional_info="", step_id=0):
    ...
    if next_step.step_id not in step_names.keys():
        raise Exception(
            f"Step ID {next_step.step_id} is out of total steps number {str(len(step_names))}"
        )
    ...
    for input_info in app_inputs_info:
        if input_info.mean.size:
            if not warned:
                ...
                warned = True
            preproc.input(input_info.name).preprocess().convert_element_type(
                Type.f32
        ...
        for name, element_type in user_precision_map.items():
            if name in input_names or name in input_node_names:
                input_index = (
                    ...
                    else input_node_names.index(name)
                )
                app_inputs_info[input_index].element_type = element_type
                pre_post_processor.input(input_index).tensor().set_element_type(
                    element_type
                )
                ...
                if name in output_names:
                    ...
                    pre_post_processor.output(
                        output_node_names.index(name)
...
def print_inputs_and_outputs_info(model: Model):
    ...
    for input in inputs:
        ...
        logger.info(
            f"    {in_name} (node: {node_name}) : {input.element_type.get_type_name()} / "
    ...
    for output in outputs:
        ...
        logger.info(
            f"    {out_name} (node: {node_name}) : {output.element_type.get_type_name()} / "
            f"{str(output.node.layout)} / {output.partial_shape}"
        )


def get_number_iterations(
    number_iterations: int, nireq: int, num_shapes: int, api_type: str
):
    ...
    if api_type == "async" and niter:
        if num_shapes > nireq:
            ...
            if number_iterations != niter:
                logger.warning(
                    "Number of iterations was aligned by number of input shapes "
...
def get_duration_in_secs(target_device):
    ...
    if duration == 0:
        ...
        logger.warning(
            f"Default duration {duration} seconds is used for unknown device {target_device}"
        )
    ...
    for device_value_string in device_value_strings:
        ...
        if len(device_value_vec) == 2:
            ...
            if device_name in devices:
                ...
                raise Exception(
                    f"Failed to set property to '{device_name}' "
...
def print_perf_counters_sort(perf_counts_list, sort_flag="sort"):
    ...
    for ni in range(len(perf_counts_list)):
        ...
        if sort_flag == "sort":
            total_detail_data = sorted(
                total_detail_data, key=lambda tmp_data: tmp_data[-4], reverse=True
            )
            ...
            total_detail_data = [
                tmp_data
                for tmp_data in total_detail_data
                if str(tmp_data[1]) != "Status.NOT_RUN"
            ]
        ...
        print(
            f'Total proportion: {"%.2f"%(round(total_real_time_proportion)*100)} % \n'
        )
...
def print_detail_result(result_list):
    ...
    for tmp_result in result_list:
        ...
        print(
            f"{node_name[:max_print_length - 4] + '...' if (len(node_name) >= max_print_length) else node_name:<20} "
...
def print_perf_counters(perf_counts_list):
    ...
    for ni in range(len(perf_counts_list)):
        ...
        for pi in perf_counts:
            print(
                f"{pi.node_name[:max_print_length - 4] + '...' if (len(pi.node_name) >= max_print_length) else pi.node_name:<20} "
                f"{str(pi.status):<20} "
        ...
        print(
            f"Total CPU time: {total_time_cpu / timedelta(milliseconds=1)} milliseconds\n"
        )
...
class AppInputInfo:
    ...
    def is_image_info(self):
        ...
        return (
            len(self.channels) >= 2
            if self.channels.is_static
            else self.channels.relaxes(Dimension(2))
        )
...
def get_inputs_info(
    ...
    for i in range(len(inputs)):
        ...
        if info.name in layout_map:
            ...
            if num_dims == 4:
                if (
                    shape[1].get_max_length() <= image_colors_dim_max
                    and shape[3].get_max_length() > image_colors_dim_max
                ):
                    info.layout = Layout("NCHW")
                elif (
                    shape[3].get_max_length() <= image_colors_dim_max
                    and shape[1].get_max_length() > image_colors_dim_max
                ):
                    info.layout = Layout("NHWC")
            elif num_dims == 3:
                if (
                    shape[0].get_max_length() <= image_colors_dim_max
                    and shape[2].get_max_length() > image_colors_dim_max
                ):
                    info.layout = Layout("CHW")
                elif (
                    shape[2].get_max_length() <= image_colors_dim_max
                    and shape[0].get_max_length() > image_colors_dim_max
                ):
        ...
        if batch_size != 0:
            if batch_size.is_static and data_shape_map:
                logger.warning(
                    f"Batch size will be ignored. Provide batch deminsion in data_shape parameter."
                )
            else:
                batch_index = -1
                if info.layout.has_name("N"):
                    ...
                    if supposed_batch.is_dynamic or supposed_batch in [0, 1]:
                        logger.warning(
                            f"Batch dimension is not specified for input '{info.name}'. "
        ...
        # Data shape
        if (
            info.name in data_shape_map or info.node_name in data_shape_map
        ) and info.is_dynamic:
            ...
            for p_shape in data_shape_map[used_name]:
                if p_shape.is_dynamic:
                    raise Exception(
                        f"Data shape always should be static, {str(p_shape)} is dynamic."
                    )
            ...
            logger.warning(
                f"Input '{info.name}' has static shape. Provided data shapes for this input will be ignored."
            )
...
def string_to_device_properties(device_properties_str):
    ...
    if not device_properties_str.startswith("{") or not device_properties_str.endswith(
        ...
        raise Exception(
            ...
            "They are actually {} and {}".format(
                device_properties_str[0], device_properties_str[-1]
            )
        )
    pattern = r"(\w+):({.+?}|[^,}]+)"
    ...
    for device_name, device_config in config.items():
        ...
        for key, value in device_config.items():
            if isinstance(value, OVAny) and (isinstance(value.value, dict)):
                ...
                value, (properties.hint.PerformanceMode, properties.Affinity)
            ):
