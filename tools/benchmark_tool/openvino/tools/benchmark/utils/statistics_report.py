...
class CsvStatisticsReport(StatisticsReport):
    ...
    def dump(self):
        ...
        with open(
            os.path.join(self.config.report_folder, "benchmark_report.csv"), "w"
        ) as f:
            if self.Category.COMMAND_LINE_PARAMETERS in self.parameters.keys():
                f.write("Command line parameters\n")
                dump_parameters(
                    f, self.parameters[self.Category.COMMAND_LINE_PARAMETERS]
                )
                f.write("\n")
    ...
    def dump_performance_counters(self, prof_info_list):
        ...
        if self.config.report_type == "" or self.config.report_type == noCntReport:
            logger.info(
                "Statistics collecting for performance counters was not requested. No reports are dumped."
            )
            return
        ...
        filename = os.path.join(
            self.config.report_folder, f"benchmark_{self.config.report_type}_report.csv"
        )
        with open(filename, "w") as f:
            if self.config.report_type == detailedCntReport:
                    ...
                    for prof_info in prof_info_list:
                        for pi in prof_info:
                            item = next(
                                (
                                    x
                                    for x in performance_counters_avg
                                    if x.node_name == pi.node_name
                                ),
                                None,
                            )
                            if item:
                ...
                dump_performance_counters_request(
                    f, get_average_performance_counters(prof_info_list)
                )
            else:
                raise Exception(
    ...
    def dump_performance_counters_sorted(self, prof_sorted_info):
        ...
        filename = os.path.join(
            self.config.report_folder, f"benchmark_sorted_report.csv"
        )
        total = 0
        total_cpu = 0
        with open(filename, "w") as f:
...
class JsonStatisticsReport(StatisticsReport):
    ...
    def dump(self):
        ...
        with open(filename, "w") as file:
            json_statistics = {}
            if self.Category.COMMAND_LINE_PARAMETERS in self.parameters.keys():
                json_statistics["cmd_options"] = list_to_dict(
                    self.parameters[self.Category.COMMAND_LINE_PARAMETERS]
                )

            if self.Category.RUNTIME_CONFIG in self.parameters.keys():
                json_statistics["configuration_setup"] = list_to_dict(
                    self.parameters[self.Category.RUNTIME_CONFIG]
                )

            if self.Category.EXECUTION_RESULTS in self.parameters.keys():
                json_statistics["execution_results"] = list_to_dict(
                    self.parameters[self.Category.EXECUTION_RESULTS]
                )

            json.dump(json_statistics, file)
            logger.info(f"Statistics report is stored to {file.name}")

    def dump_performance_counters(
        self, prof_info_list: List[List[Any]]
    ):  # ProfilingInfo
        ...
        filename = os.path.join(
            ...
            f"benchmark_{self.config.report_type}_report.json",
        )
        with open(filename, "w") as file:
            if self.config.report_type == detailedCntReport:
    ...
    def dump_performance_counters_sorted(self, prof_sorted_info) -> None:
        def profiling_info_to_dict_list(
            prof_info_matrix: np.ndarray,
        ) -> List[Dict[str, str]]:
            total, total_cpu = 0, 0
        ...
        filename = os.path.join(
            self.config.report_folder, f"benchmark_sorted_report.json"
        )
        with open(filename, "w") as file:
