...
from .constants import BINARY_EXTENSIONS, IMAGE_EXTENSIONS, NUMPY_EXTENSIONS
...
if find_spec("cv2") is not None:
    try:
        ...
        raise Exception(
            "Failed to import opencv module. "
            "Please try to uninstall opencv-python "
            "and install opencv-python-headless instead."
        ) from ex
...
class DataQueue:
    ...
    def get_next_input(self):
        ...
        for input_name, input_tensors in self.input_data.items():
            ...
            self.index_map[input_name] = (
                self.index_map[input_name] + 1
            ) % self.sizes_map[input_name]
...
def get_group_batch_sizes(app_input_info):
    ...
    for i in range(niter):
        ...
        for info in app_input_info:
            batch_index = (
                info.layout.get_index_by_name("N") if info.layout.has_name("N") else -1
            )
            if batch_index != -1:
...
def verify_objects_to_be_used(
    objects_to_be_used_map: Dict[str, List[str]],
    info: AppInputInfo,
    total_frames: int,
    input_type_name: str,
):
    if (
        ...
        and objects_to_be_used_map[info.name] % total_frames != 0
    ):
        objects_to_be_used_map[info.name] = (
            objects_to_be_used_map[info.name]
            - objects_to_be_used_map[info.name] % total_frames
        )
        logger.warning(
            f"Number of provided {input_type_name} for input '{info.name}' is not a multiple of the number of "
        ...
        logger.warning(
            f"Some {input_type_name} will be dublicated: {total_frames} is required, "
...
def get_input_data(paths_to_input, app_input_info):
    ...
    images_to_be_used_map = {
        input_name: len(images) for input_name, images in image_mapping.items()
    }
    numpys_to_be_used_map = {
        input_name: len(images) for input_name, images in numpy_mapping.items()
    }
    binaries_to_be_used_map = {
        input_name: len(binaries) for input_name, binaries in binary_mapping.items()
    }
    ...
    for port, info in enumerate(app_input_info):
        if info.name in image_mapping:
            data[port] = get_image_tensors(
                image_mapping[info.name][: images_to_be_used_map[info.name]],
                info,
            ...
            data[port] = get_numpy_tensors(
                numpy_mapping[info.name][: numpys_to_be_used_map[info.name]],
                info,
                batch_sizes_map[info.name],
            )
            ...
            data[port] = get_binary_tensors(
                binary_mapping[info.name][: binaries_to_be_used_map[info.name]],
                info,
                batch_sizes_map[info.name],
            )
...
def get_image_tensors(
    image_paths: List[str], info: AppInputInfo, batch_sizes: List[int]
) -> List[Tensor]:
    if "cv2" not in sys.modules:
        logger.error(
            "Loading images requires the opencv-python or opencv-python-headless package. "
    ...
    for i in range(niter):
        ...
        current_batch_size = (
            1 if process_with_original_shapes else batch_sizes[i % num_shapes]
        )
        for b in range(current_batch_size):
            ...
            if model_channel == image_channel and str(info.layout) in [
                "[N,C,H,W]",
                "[C,H,W]",
            ]:
...
def get_numpy_tensors(
    numpy_paths: List[str], info: AppInputInfo, batch_sizes: List[int]
) -> List[Tensor]:
            ...
            if extension == "npy":
                ...
                if process_with_original_shapes:
                    ...
                    try:
                        if info.layout.has_name("N"):
                            numpy_arrays[
                                [None] * info.layout.get_index_by_name("N") + [b]
                            ] = numpy_arr
                        else:
                            numpy_arrays = numpy_arr
                    except ValueError:
                        raise Exception(
                            f"Numpy array shape {numpy_arr.shape} is not compatible with input shape {shape}! "
                            f"Make sure -i parameter is valid."
                        )
            else:
                raise Exception(f"Unsupported numpy file type: {extension}")
            numpy_index += 1
...
def get_binary_tensors(
    binary_paths: List[str], info: AppInputInfo, batch_sizes: List[int]
) -> List[Tensor]:
    num_shapes = len(info.shapes)
    ...
    for i in range(niter):
        ...
        for b in range(current_batch_size):
            ...
            if extension == "bin":
                ...
                if info.layout.has_name("N"):
                    binaries[[None] * info.layout.get_index_by_name("N") + [b]] = (
                        from_file
                    )
                else:
                    binaries = from_file
            else:
                raise Exception(f"Unsupported binary file type: {extension}")
...
def get_input_file_mappings(paths_to_inputs, app_input_info):
    ...
    return (
        remove_empty_items(merge_dicts(image_dicts_list)),
...
def parse_path(path, app_input_info):
    ...
    tensor_names = [
        (
            parsed_name
            if parsed_name in input_names
            else input_names[input_node_names.index(parsed_name)]
        )
        for parsed_name in parsed_names
    ]
    input_pathes = [
        path for path in re.split(r"(?=[^,])(?![a-zA-Z]:\\)[\w\.]+:", path) if path
    ]
    input_path_mapping = defaultdict(list)
    # input mapping is used
    if tensor_names:
        input_path_mapping = {
            input_: files.strip(",").split(",")
        ...
        for _input_path in _input_pathes:
            ...
            if input_path.exists():
                if input_path.is_dir():
                    input_files += list(
                        str(file_path) for file_path in input_path.iterdir()
                    )
        ...
        if num_files > num_inputs and num_files % num_inputs != 0:
            ...
            logger.warning(
                f"Number of provided input files '{num_files}' is not a multiple of the number of "
        ...
        for i in range(num_files):
            input_path_mapping[inputs_to_fill[i % len(inputs_to_fill)]].append(
                input_files[i]
            )
    ...
    if unsupported_files:
        logger.warning(
            f"This files has unsupported extensions and will "
