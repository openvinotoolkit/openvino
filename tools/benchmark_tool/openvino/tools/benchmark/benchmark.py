...
from .utils.constants import BIN_EXTENSION, GPU_DEVICE_NAME, XML_EXTENSION
...
class Benchmark:
    ...
    def main_loop(self, requests, data_queue, batch_size, latency_percentile, pcseq):
        if self.api_type == "sync":
            ...
            times, total_duration_sec, processed_frames, iteration = (
                self.async_inference_full_mode(requests, data_queue, pcseq)
            )
