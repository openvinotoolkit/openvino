...
def get_inputs(paths_to_input, batch_size, app_input_info, requests):
        ...
        if binary_to_be_used > 0 and len(binary_files) == 0:
            ...
            logger.warning(
                f"Some binary input files will be duplicated: \
                                        {binary_to_be_used} files are required, but only {len(binary_files)} were provided")
        elif binary_to_be_used < len(binary_files):
            logger.warning(
                f"Some binary input files will be ignored: only {binary_to_be_used} files are required from {len(binary_files)}")

        images_to_be_used = images_count * batch_size * len(requests)
        if images_to_be_used > 0 and len(image_files) == 0:
            logger.warning(f"No supported image inputs found! Please check your file extensions: {','.join(IMAGE_EXTENSIONS)}")
    ...
    for request_id in range(0, len(requests)):
        ...
        for key in keys:
            ...
            logger.info(f"Fill input '{key}' with random values ({'image' if info.is_image else 'some binary data'} is expected)")
