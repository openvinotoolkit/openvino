...
## Cyberspore utilities

The `tools/cyberspore/spore_inject.py` helper injects CybersporeTSSN branches into Gemma/Gemma3 IR graphs and prunes MatFormer shells inside the `.bin` file. Example usage:

```bash
python3 tools/cyberspore/spore_inject.py inject --model path/to/model.xml --output-dir build/chimera
python3 tools/cyberspore/spore_inject.py prune --model path/to/model.xml --weights path/to/model.bin
```

Tests for the tool live in `tools/cyberspore/tests`. Run them via:

```bash
pytest tools/cyberspore/tests
```
