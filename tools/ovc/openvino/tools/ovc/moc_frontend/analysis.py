...
def json_model_analysis_dump(framework_model: Model):
    ...
    for op in filter(
        lambda node: node.type_info.name != "NullNode",
        ...
        for out_idx in range(op.get_output_size()):
            ...
            json_dump["intermediate"][tensor_name]["shape"] = dump_partial_shape(
                output.get_partial_shape()
            )
            json_dump["intermediate"][tensor_name]["data_type"] = dump_element_type(
                output.get_element_type()
            )
            json_dump["intermediate"][tensor_name][
                "value"
            ] = "None"  # not supported in 22.1
