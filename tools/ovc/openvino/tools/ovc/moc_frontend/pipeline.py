...
from openvino.tools.ovc.moc_frontend.extractor import (
    convert_params_lists_to_dicts,
    fe_output_user_data_repack,
    fe_user_data_repack,
)
from openvino.tools.ovc.utils import mo_array, np_map_cast
...
def raise_exception_for_input_output_cut(
    model_inputs_or_outputs: List[Place], new_nodes: List[dict], is_input: bool
):
    for new_node in new_nodes:
        ...
        if not any([item.is_equal(node) for item in model_inputs_or_outputs]):
            if is_input:
                raise Exception(
                    "Name {} is not found among model inputs.".format(
                        new_node["input_name"]
                    )
                )
            else:
                raise Exception(
                    "Name {} is not found among model outputs.".format(
                        new_node["output_name"]
                    )
                )
...
def moc_pipeline(argv: argparse.Namespace, moc_front_end: FrontEnd):
    ...
    share_weights = getattr(
        argv, "share_weights", True
    )  # FIXME: Should be controlled by default value
    if isinstance(argv.input_model, (tuple, list)) and len(argv.input_model) == 2:
        ...
        input_model = moc_front_end.load(
            [part for part in argv.input_model], share_weights
        )
    else:
