...
from openvino.tools.ovc.convert_impl import (
    args_to_argv,
    arguments_post_parsing,
...
class TestShapesParsing(UnitTestWithMockedTelemetry):
    def test_get_shapes_several_inputs_several_shapes2(self):
        ...
        inputs_ref = [
            _InputCutInfo(name="inp1", shape=PartialShape([1, 22, 333, 123])),
    ...
    def test_get_shapes_complex_input(self):
        ...
        inputs = input_to_input_cut_info(argv_input)
        inputs_ref = [
            _InputCutInfo(shape=PartialShape([10, -1, 100])),
    ...
    def test_get_shapes_and_freezing_with_scalar_and_without_shapes_in_input(self):
        ...
        argv_input = parse_inputs(argv_input)
        inputs = input_to_input_cut_info(argv_input)
        inputs_ref = [_InputCutInfo(name="inp1"), _InputCutInfo(name="inp2")]
        self.assertEqual(inputs, inputs_ref)
    ...
    def test_get_shapes_several_inputs_several_shapes3(self):
        ...
        inputs = input_to_input_cut_info(argv_input)
        inputs_ref = [
            _InputCutInfo(name="inp1", shape=PartialShape([3, 1])),
    ...
    def test_get_shapes_and_data_types_shape_only(self):
        ...
        inputs = input_to_input_cut_info(argv_input)
        inputs_ref = [
            _InputCutInfo(name="placeholder1", shape=PartialShape([3, 1])),
    ...
    def test_get_shapes_and_data_types_shape_with_ports_only(self):
        ...
        inputs = input_to_input_cut_info(argv_input)
        inputs_ref = [
            _InputCutInfo(name="placeholder1:4", shape=PartialShape([3, 1])),
            _InputCutInfo(name="placeholder2"),
    ...
    def test_get_shapes_two_input_shapes_with_scalar(self):
        ...
        inputs = input_to_input_cut_info(argv_input)
        inputs_ref = [
            _InputCutInfo(shape=PartialShape([12, 4, 1])),
    ...
    def test_get_shapes_several_inputs_several_partial_shapes2(self):
        ...
        inputs = input_to_input_cut_info(argv_input)
        inputs_ref = [
            _InputCutInfo(name="inp1", shape=PartialShape("[1,?,50..100,123]")),
            _InputCutInfo(name="inp2", shape=PartialShape("[-1,45..,..7,1]")),
        ]
        self.assertEqual(inputs, inputs_ref)
    ...
    def test_not_supported_arrow(self):
        with self.assertRaisesRegex(Exception, "Incorrect format of input."):
...
class PathCheckerFunctions(unittest.TestCase):
    ...
    NOT_WRITABLE_SUB_DIR = os.path.join(
        tempfile.gettempdir(), "another_not_writable_dir", "not_existing_dir"
    )
    ...
    def tearDownClass(cls):
        ...
        if os.path.exists(os.path.dirname(__class__.NOT_WRITABLE_SUB_DIR)):
            shutil.rmtree(
                os.path.dirname(__class__.NOT_WRITABLE_SUB_DIR), ignore_errors=True
            )
...
class TestPackParamsToArgsNamespace(unittest.TestCase):
    def test_mo_convert_params(self):
        ...
        args = {
            "input_model": os.path.dirname(__file__),
    ...
    def test_output_post_parsing_5(self):
        ...
        assert argv.output == [
            "a,b"
        ]  # post parsing should decorate single string into a list
    ...
    def test_raises_output_post_parsing_2(self):
        ...
        with self.assertRaisesRegex(
            AssertionError, ".*output names should not be empty or contain spaces"
        ):
    ...
    def test_not_existing_dir(self):
        ...
        with self.assertRaisesRegex(
            Error,
            'The value for parameter "input_model" must be existing file/directory, '
            'but "abc" does not exist.',
        ):
            pack_params_to_args_namespace(args, cli_parser, True)

    def test_unknown_params(self):
        args = {"input_model": os.path.dirname(__file__), "a": "b"}
        cli_parser = get_all_cli_parser()
...
class GetModelNameTest(unittest.TestCase):
    ...
    def test_case4(self):
        current_dir = os.getcwd()
        dir = os.path.basename(current_dir)
        argv = argparse.Namespace(input_model="." + os.sep, output_model="." + os.sep)
        assert get_model_name_from_args(argv) == "." + os.sep + dir + ".xml"

    def test_case5(self):
        argv = argparse.Namespace(
            input_model="test_model.pb", output_model="." + os.sep
        )
        assert get_model_name_from_args(argv) == "." + os.sep + "test_model.xml"

    def test_case6(self):
        argv = argparse.Namespace(input_model="test_model", output_model="." + os.sep)
        assert get_model_name_from_args(argv) == "." + os.sep + "test_model.xml"

    def test_case7(self):
        argv = argparse.Namespace(
            input_model="test_dir" + os.sep, output_model="." + os.sep
        )
        assert get_model_name_from_args(argv) == "." + os.sep + "test_dir.xml"

    def test_case8(self):
        argv = argparse.Namespace(input_model="test_model.pb", output_model="new_model")
        assert get_model_name_from_args(argv) == "new_model.xml"

    def test_case9(self):
        argv = argparse.Namespace(
            input_model="test_model", output_model="new_dir" + os.sep
        )
        assert get_model_name_from_args(argv) == "new_dir" + os.sep + "test_model.xml"

    def test_case10(self):
        argv = argparse.Namespace(
            input_model="test_dir" + os.sep, output_model="new_model.xml"
        )
        assert get_model_name_from_args(argv) == "new_model.xml"

    def test_case11(self):
        argv = argparse.Namespace(input_model="/", output_model="new_model")
        assert get_model_name_from_args(argv) == "new_model.xml"
