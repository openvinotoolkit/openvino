...
def setup_env():
    ...
    def is_mo_imported():
        try:
            status = subprocess.run([sys.executable, os.path.join(mo_root_path, 'openvino/tools/mo_lite/utils/check_mo_import.py')],
                                    env=os.environ)
    ...
    ie_found = True
    try:
        from openvino.tools.mo_lite.utils.find_ie_version import find_ie_version  # pylint: disable=no-name-in-module
