...
import openvino
from openvino.tools.mo_lite.utils.clean_utils import destination_type_to_np_data_type, np_data_type_to_destination_type, split_node_in_port, default_path
from openvino.tools.mo_lite.utils.error import Error
from openvino.tools.mo_lite.utils.utils import refer_to_faq_msg, get_mo_root_dir
...
def single_input_to_str(input):
    if isinstance(input, str):
        return input
    if isinstance(input, openvino.tools.mo_lite.InputCutInfo):
    ...
    raise Exception("Unexpected object provided for input. Expected openvino.tools.mo_lite.InputCutInfo "
...
def layoutmap_to_str(value):
    if isinstance(value, str):
        return value
    if isinstance(value, openvino.tools.mo_lite.LayoutMap):
...
def transform_to_str(value):
    from openvino.tools.mo_lite.utils.offline_transformations import get_available_transformations
