...
from openvino.tools.mo_lite.moc_frontend.pipeline import moc_pipeline
from openvino.tools.mo_lite.moc_frontend.serialize import moc_emit_ir
from openvino.tools.mo_lite.utils.cli_parser import check_available_transforms, \
...
from openvino.tools.mo_lite.utils.find_ie_version import find_ie_version
from openvino.tools.mo_lite.utils.guess_framework import deduce_legacy_frontend_by_namespace
from openvino.tools.mo_lite.utils.logger import init_logger, progress_printer
from openvino.tools.mo_lite.utils.utils import refer_to_faq_msg
from openvino.tools.mo_lite.utils.telemetry_utils import send_params_info, send_framework_info
from openvino.tools.mo_lite.utils.version import get_simplified_mo_version, get_simplified_ie_version, get_version
from openvino.tools.mo_lite.utils.versions_checker import check_requirements  # pylint: disable=no-name-in-module
from openvino.tools.mo_lite.utils.telemetry_utils import get_tid
from openvino.tools.mo_lite.moc_frontend.check_config import legacy_extensions_used
...
def load_extensions(argv: argparse.Namespace, is_tf: bool, is_caffe: bool, is_mxnet: bool, is_kaldi: bool,
                    is_onnx: bool):
    # FIXME: 100% mo-dev path, make sure we have necessary functionality and in case we dont -- lets raise exception!
...
def prepare_ir(argv: argparse.Namespace):
    ...
    if not argv.use_legacy_frontend and is_tf:
        from openvino.tools.mo.front.tf.loader import convert_to_pb # FIXME: No conversion to pb would be available in the mo-lite
    ...
    # FIXME: this is 100% mo-dev, we need special checks if it is available here
...
def emit_ir(graph, argv: argparse.Namespace, non_default_params: dict):
    ...
    if not (argv.framework == 'tf' and argv.tensorflow_custom_operations_config_update):
        try:
            from openvino.tools.mo_lite.utils.offline_transformations import apply_offline_transformations
