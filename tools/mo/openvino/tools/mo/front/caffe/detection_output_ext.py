...
class DetectionOutputFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        visualize_threshold = (
            param.visualize_threshold if param.visualize_threshold else 0.6
        )
        ...
        if (
            param.save_output_param.resize_param.resize_mode < 1
            or param.save_output_param.resize_param.resize_mode > 3
        ):
        ...
        if (
            param.save_output_param.resize_param.pad_mode < 1
            or param.save_output_param.resize_param.pad_mode > 3
        ):
        ...
        attrs = {
            ...
            "pad_mode": pad_mode,
            "pad_value": ",".join(
                str(x) for x in param.save_output_param.resize_param.pad_value
            ),
