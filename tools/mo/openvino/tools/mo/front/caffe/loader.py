...
def load_caffe_proto_model(caffe_pb2, proto_path: str, model_path: [str, None] = None):
    ...
    if api_implementation._implementation_type == "python":
        message = (
            "Please expect that Model Optimizer conversion might be slow. "
        ...
        try:
            ...
            if (
                os.name == "nt"
                and os.environ.get("PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION", default="")
                != "cpp"
            ):
                # 2. cpp implementation is available but not used
                message += (
                    "However, cpp implementation is available, you can boost "
    ...
    try:
        ...
        raise FrameworkError(
            "Model Optimizer is not able to parse {}".format(proto_path)
        ) from e
    ...
    try:
        ...
        if api_implementation._implementation_type == "python":
            third_point = (
                "      3. Python protobuf implementation was used. Some models can't be converted "
        ...
        raise FrameworkError(
            "Model Optimizer is not able to parse {}".format(model_path)
        ) from e
...
def get_layers(proto):
    if len(proto.layer):
        ...
        raise Error(
            'Invalid proto file: there is neither "layer" nor "layers" top-level messages. '
            + refer_to_faq_msg(7)
        )
...
def caffe_pb_to_nx(graph, proto, model):
    ...
    if len(proto.input_dim) > 0 and len(list(proto.input)) > 1:
        ...
        # input_dim: 3
        raise Error(
            'Old-style inputs (via "input_dims") are not supported. '
    ...
    for i, layer in enumerate(proto_layers):
        ...
        if layer.type == "Input":
            if hasattr(layer, "input_param"):
                ...
                raise Error("Input layer has no input dims. " + refer_to_faq_msg(8))
            if hasattr(input_param, "shape"):
                ...
                dims = map(
                    int,
                    list(filter(None, str(list(input_param.shape)[0]).split("dim:"))),
                )
        ...
        graph.add_node(
            node_id, pb=layer, model_pb=model_layer, kind="op", type="Parameter"
        )
    ...
    add_outputs_identity(
        graph,
        all_blobs - used_blobs,
        add_edge_caffe,
        {"blob_producers": blob_producers, "dst_port": 0},
    )

    if len(input_names) <= 0:
        raise Error('The topology contains no "input" layers. ' + refer_to_faq_msg(79))
    return {
        fake_node_name: shape
        for (fake_node_name, shape) in zip(input_names, input_dims)
    }


def add_edge_caffe(
    graph: Graph, bottom: str, dst_layer: str, blob_producers: dict, dst_port: int
):
    """
