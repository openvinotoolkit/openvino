...
def append_unique(attrs, new_attr, value):
    if new_attr in attrs:
        log.error(
            "The parameter {} overwrites already existing value. ".format(new_attr)
...
def append_unique_enum(attrs: dict, descriptor, value):
    enum_name = "{}.{}".format(
        descriptor.enum_type.full_name.rsplit(".", 1)[
            0
        ],  # remove enum name Z from X.Y.Z name
...
def collect_optional_attributes(
    obj,
    prefix: str = "",
    ...
    for descriptor in obj.DESCRIPTOR.fields:
        ...
        if descriptor.label != descriptor.LABEL_OPTIONAL:
            continue
        if (
            descriptor.has_default_value or disable_omitting_optional
        ) or descriptor.name in fields:
            if descriptor.type == descriptor.TYPE_MESSAGE:
                attrs.update(
                    collect_optional_attributes(
                        value,
                        prefix=name,
                ...
                attrs.update(
                    append_unique(attrs, name, cast_to_string(descriptor, value))
                )
    return attrs
...
def collect_attributes(
    ...
    Args:
        ...
        enable_flattening_nested_params: disable flattening optional params flag
    """
    attrs = collect_optional_attributes(
        obj, prefix, disable_omitting_optional, enable_flattening_nested_params
    )
    fields = [field[0].name for field in obj.ListFields()]
    for descriptor in obj.DESCRIPTOR.fields:
        value = getattr(obj, descriptor.name)
        ...
        if descriptor.label == descriptor.LABEL_REPEATED:
            ...
            if descriptor.type == descriptor.TYPE_MESSAGE:
                ...
                attrs.update(
                    append_unique(attrs, name, ",".join([str(v) for v in value]))
                )
