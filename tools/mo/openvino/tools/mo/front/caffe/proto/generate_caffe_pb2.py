...
def shell(cmd, env=None, cwd=None):
    kwargs = dict(
        cwd=cwd, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False
    )
...
def get_cli_parser():
    ...
    parser.add_argument(
        "--output",
        help="Directory where output file are generated",
...
def build_proto(proto_file_path, python_path):
    ...
    if retcode:
        ...
        shutil.move(
            os.path.join(python_path, python_file),
            os.path.join(python_path, "caffe_pb2.py"),
        )
