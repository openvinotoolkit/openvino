...
class BinarizationToQuantize(FrontReplacementOp):
    ...
    def replace_op(self, graph: Graph, node: Node):
        ...
        threshold = Const(
            graph, {"name": node.id + "/Input_1", "value": broadcast(0)}
        ).create_node()
        in_1 = threshold
        in_2 = threshold
        in_3 = Const(
            graph, {"name": node.id + "/Input_3", "value": broadcast(-1)}
        ).create_node()
        in_4 = Const(
            graph, {"name": node.id + "/Input_4", "value": broadcast(+1)}
        ).create_node()
        quant = FakeQuantize(
            graph, {"name": node.id + "/FakeQuantize_", "levels": 2}
