...
def common_onnx_pool_extractor(node):
    kernel_shape = onnx_attr(
        node, "kernel_shape", "ints", default=None, dst_type=lambda x: int64_array(x)
    )
    final_kernel_shape = (
        int64_array([1, 1, *[x for x in kernel_shape]])
    ...
    if (
        ...
        and kernel_shape.size * 2 != pads.size
    ):
        log.warning(
            "Node {} has pad = {} which is ill-formed -- it should have even amount of elements.".format(
    ...
    final_strides = (
        int64_array([1, 1, *[x for x in strides]]) if strides is not None else None
    )
    ...
    final_dilation = (
        int64_array([1, 1, *[x for x in dilation]]) if dilation is not None else None
    )
