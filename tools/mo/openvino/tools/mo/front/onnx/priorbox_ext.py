...
class PriorBoxFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        variance = onnx_attr(
            node, "variance", "floats", default=[], dst_type=lambda x: float32_array(x)
        )
        if len(variance) == 0:
            variance = [0.1]

        update_attrs = {
            "aspect_ratio": onnx_attr(
                node, "aspect_ratio", "floats", dst_type=lambda x: float32_array(x)
            ),
            "min_size": onnx_attr(
                node, "min_size", "floats", dst_type=lambda x: float32_array(x)
            ),
            "max_size": onnx_attr(
                node, "max_size", "floats", dst_type=lambda x: float32_array(x)
            ),
