...
class ConvTransposeFrontExtractor(FrontExtractorOp):
    ...
    def get_pad(node, input_shape, kernel_shape):
        ...
        if node.has_valid("auto_pad") and node.auto_pad != "same_upper":
            ...
            pad[node.spatial_dims] = int64_array(
                [
                    [total_padding[x] - (total_padding[x] // 2), total_padding[x] / 2]
                    for x in range(len(node.spatial_dims))
                ]
            )
        return pad
    ...
    def extract(cls, node):
        ...
        final_output_padding = (
            int64_array([0, 0, *output_padding]) if output_padding is not None else None
        )

        output_shape = onnx_attr(
            node, "output_shape", "ints", default=None, dst_type=int64_array
        )
