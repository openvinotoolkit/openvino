...
class FlattenONNXToReshape(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        if axis == 0:
            dim = Const(
                graph,
                {"value": int64_array([1, -1]), "name": reshape_node.name + "/shape"},
            ).create_node()
        elif axis == 1:
            dim = Const(
                graph,
                {"value": int64_array([0, -1]), "name": reshape_node.name + "/shape"},
            ).create_node()
        else:
            ...
            second_dims = Const(
                graph, {"value": int64_array([-1]), "name": name + "/second_dims"}
            ).create_node()
            ...
            order_of_dims = (
                [first_dims, second_dims] if axis > 0 else [second_dims, first_dims]
            )
