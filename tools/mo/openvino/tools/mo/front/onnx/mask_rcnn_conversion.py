...
from openvino.tools.mo.ops.detection_output_onnx import (
...
class ONNXMaskRCNNTransformation(FrontReplacementFromConfigFileGeneral):
    ...
    def transform_graph(self, graph: Graph, replacement_descriptions: dict):
        insert_ExperimentalDetectronROIFeatureExtractor2(
            graph, replacement_descriptions
        )
        insert_do(graph, replacement_descriptions)
        insert_ExperimentalDetectronROIFeatureExtractor1(
            graph, replacement_descriptions
        )
...
def insert_do(graph: Graph, replacement_descriptions: dict):
    ...
    im_info_node = Parameter(
        graph, {"name": "im_info", "shape": int64_array([1, 3])}
    ).create_node()
    ...
    do_node = ExperimentalDetectronDetectionOutput(
        graph,
        {
            ...
            "max_detections_per_image": replacement_descriptions[
...
def insert_ExperimentalDetectronROIFeatureExtractor2(
    ...
    fpn_roi_align.in_port(0).connect(
        Node(graph, replacement_descriptions["ROIFeatureExtractor2_input"]).out_port(0)
    )
    for ind, fpn_node in enumerate(input_fpn_head_nodes):
        fpn_roi_align.in_port(ind + 1).connect(fpn_node.out_port(0))
