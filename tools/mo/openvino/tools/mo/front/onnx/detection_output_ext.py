...
class DetectionOutputFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        code_type = onnx_attr(
            node, "code_type", "s", default=code_type_values[b"CORNER"]
        )
        try:
        ...
        try:
            ...
            raise Error(
                "Incorrect value of resize_mode parameter {}".format(resize_mode)
            )
        ...
        try:
            ...
            raise Error(
                "Incorrect value of interp_mode parameter {}".format(interp_mode)
            )

        attrs = {
            "num_classes": onnx_attr(node, "num_classes", "i", default=0),
            "share_location": onnx_attr(node, "share_location", "i", default=0),
            "background_label_id": onnx_attr(
                node, "background_label_id", "i", default=0
            ),
            "code_type": code_type,
            "variance_encoded_in_target": onnx_attr(
                node, "variance_encoded_in_target", "i", default=0
            ),
            "keep_top_k": onnx_attr(node, "keep_top_k", "i", default=0),
            "confidence_threshold": onnx_attr(
                node, "confidence_threshold", "f", default=0
            ),
            "visualize_threshold": onnx_attr(
                node, "visualize_threshold", "f", default=0.6
            ),
            # nms_param
