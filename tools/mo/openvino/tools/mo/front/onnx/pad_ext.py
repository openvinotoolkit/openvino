...
class PadFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        mode = onnx_attr(
            node, "mode", "s", default="constant", dst_type=lambda x: x.decode()
        )
        ...
        if get_onnx_opset_version(node) < 11 or pads is not None:
            value = onnx_attr(node, "value", "f", default=0.0)
            ...
            AttributedPad.update_node_stat(
                node, {"mode": mode, "pads": pads, "fill_value": value}
            )
        else:
