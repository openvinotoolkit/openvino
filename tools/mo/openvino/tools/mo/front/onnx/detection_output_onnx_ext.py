...
class ExperimentalDetectronDetectionOutputFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        attrs = dict(
            class_agnostic_box_regression=onnx_attr(
            ...
            max_delta_log_wh=onnx_attr(
                node, "max_delta_log_wh", "f", log(1000.0 / 16.0)
            ),
            deltas_weights=float32_array(
                onnx_attr(node, "deltas_weights", "floats", [10.0, 10.0, 5.0, 5.0])
            ),
        )
