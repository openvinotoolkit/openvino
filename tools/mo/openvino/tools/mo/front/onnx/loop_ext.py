...
from openvino.tools.mo.front.onnx.loader import (
...
def create_edge_with_attrs(
    graph, src_name, src_internal_id, src_port, dst_id, dst_port
):
...
def create_parameter_with_empty_attrs(graph, param_name):
    graph.add_node(
        param_name, kind="op", op="Parameter", name=param_name, pb=None, shape=None
    )
...
def create_cross_body_edge(
    body_graph, external_edges, additional_params, src_internal_id, dst_id, dst_port
):
    ...
    while not is_finished and "parent_node" in cur_graph.graph:
        ...
        if src_internal_id in parent_graph.graph["tensor_mapping"]:
            log.debug(
                "The edge between outer and inner graphs detected: {} -> {}".format(
                    src_internal_id, dst_id
                )
            )
            # if parameter in inner graph already created, use it. Otherwise - create new one
            if (
                parent_graph.graph["tensor_mapping"][src_internal_id]
                not in additional_params[counter - 1]
            ):
                ...
                additional_params[counter][
                    parent_graph.graph["tensor_mapping"][src_internal_id][0]
                ] = parameter_node
            else:
                src_id, src_port = (
                    additional_params[counter - 1][
                        parent_graph.graph["tensor_mapping"][src_internal_id][0]
                    ].id,
                    0,
                )
            ...
            parent_parameter_node = create_parameter_with_empty_attrs(
                parent_graph, parent_param_id
            )

            external_edges[counter].append(
                ((parent_param_id, 0), parameter_node, parent_param_id)
            )
            src_id, src_port = param_id, 0
        ...
        if cur_graph.has_node(dst_id):
            create_edge_with_attrs(
                cur_graph, src_internal_id, src_id, src_port, dst_id, dst_port
            )
...
class LoopExtractor(FrontExtractorOp):
    ...
    def extract(cls, loop_node):
        ...
        data_nodes_map = {}
        body_graph.graph["tensor_mapping"] = (
            data_nodes_map  # save mapping for possible Loop inside the Loop
        )
            ...
            for dst_port, inp in enumerate(pb_node.input):
                ...
                if inp not in data_nodes_map:
                    if inp == "":
                        ...
                        is_finished = create_cross_body_edge(
                            body_graph,
                            external_edges,
                            additional_params,
                            inp,
                    ...
                    create_edge_with_attrs(
                        body_graph, inp, src_id, src_port, id, dst_port
                    )
        ...
        for output in body_graph_proto.output:
            ...
            assert body_graph.has_node(
                node_name
            ), 'The body graph does not contain output with name "{}"'.format(node_name)
            body_results.append(
                Node(
                    body_graph, add_opoutput(body_graph, node_name, output_port, False)
                )
            )
        ...
        scan_outputs_count = (
            len(body_graph_proto.output) - 1 - loop_carried_dependencies_count
        )
        ...
        for external_edges_subg in external_edges:
            ...
            for (src_node, src_port), body_node, tensor_name in external_edges_subg:
                create_edge_with_attrs(
                    ...
                    src_port,
                    cur_loop_node.id,
                    next_loop_input_port_idx,
                )

                Loop.connect_body_input(
                    cur_loop_node, next_loop_input_port_idx, body_node
                )
        ...
        for idx in range(loop_carried_dependencies_count):
            Loop.add_back_edge(
                loop_node, body_parameters[idx + 2], body_results[idx + 1]
            )
        ...
        for idx in range(
            loop_carried_dependencies_count,
