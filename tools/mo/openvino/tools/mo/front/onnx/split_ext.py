...
from openvino.tools.mo.ops.split import AttributedSplit, AttributedVariadicSplit
...
class SplitFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        size_splits = onnx_attr(
            node, "split", "ints", default=None, dst_type=int64_array
        )
        if size_splits is None:
            AttributedSplit.update_node_stat(
                node,
                {
                    "axis": axis,
            ...
            AttributedVariadicSplit.update_node_stat(
                node,
                {
                    "axis": axis,
