...
from openvino.tools.mo.front.onnx.extractors.utils import get_onnx_autopad, onnx_attr
...
class DeformableConvExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        pads = onnx_attr(
            node, "pads", "ints", default=None, dst_type=lambda x: int64_array(x)
        )
        ...
        dilations = onnx_attr(
            node, "dilations", "ints", default=None, dst_type=lambda x: int64_array(x)
        )
        final_dilations = (
            int64_array([1, 1, *dilations]) if dilations is not None else None
        )

        # Extract dilations attribute
        # In case if dilations is not specified it will be set in default (1) in infer function
        strides = onnx_attr(
            node, "strides", "ints", default=None, dst_type=lambda x: int64_array(x)
        )
        ...
        auto_pad = onnx_attr(
            node, "auto_pad", "s", default=None, dst_type=get_onnx_autopad
        )
        group = onnx_attr(
            node, "group", "i", default=1, dst_type=lambda x: int64_array(x)
        )
