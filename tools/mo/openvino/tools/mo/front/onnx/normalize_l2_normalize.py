    ...
    def find_and_replace_pattern(self, graph: Graph):
        for normalize_l2 in graph.get_op_nodes(op="NormalizeL2"):
            if normalize_l2.in_port(1).disconnected():
                assert normalize_l2.has_valid(
                    "axis"
                ), 'The NormalizeL2 node "{}" misses "axis" attribute.' "".format(
                    normalize_l2.name
                )
                axis_node = Const(
                    graph,
                    {
                        "name": normalize_l2.id + "/Axis",
                        "value": int64_array([normalize_l2.axis]),
                    },
                ).create_node()
                ...
                log.debug(
                    'The NormalizeL2 node input "{}" is already normalized'.format(
                        normalize_l2.name
                    )
                )
