...
class UpsampleFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        if onnx_opset_version is not None and onnx_opset_version >= 9:
            mode = onnx_attr(
                node, "mode", "s", default="nearest", dst_type=lambda x: x.decode()
            )
            ...
            scales = onnx_attr(
                node, "scales", "floats", dst_type=lambda x: float32_array(x)
            )
            ...
            if (width_scale is None or height_scale is None) and len(
                node.in_nodes()
            ) != 2:
                raise Error(
