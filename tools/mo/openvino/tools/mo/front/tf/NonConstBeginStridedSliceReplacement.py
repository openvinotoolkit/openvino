...
class NonConstBeginStridedSliceReplacement(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        if not np.array_equal(
            np.argwhere(begin_mask == 1), np.argwhere(end_mask == 1)
        ) or not np.array_equal(
            np.argwhere(begin_mask == 1), np.argwhere(shrink_mask == 1)
        ):
            return
        ...
        gather_node = create_op_with_const_inputs(
            graph,
            Gather,
            {2: int64_array(sliced_axis)},
            {"name": strided_slice_name + "/Gather"},
        )
        strided_slice_node.in_port(0).get_connection().set_destination(
        ...
        squeeze_node = create_op_with_const_inputs(
            graph,
            Squeeze,
            {1: int64_array(sliced_axis)},
            {"name": strided_slice_name + "/Squeeze"},
        )
        squeeze_node.in_port(0).connect(gather_node.out_port(0))
        rename_nodes(
            [
                (strided_slice_node, strided_slice_name + "/AbandonedName"),
                (squeeze_node, strided_slice_name),
            ]
        )

        # preserve a name of original StridedSlice node
        strided_slice_node.out_port(0).get_connection().set_source(
