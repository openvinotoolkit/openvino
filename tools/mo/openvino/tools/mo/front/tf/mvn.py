...
class MVNReplacer(FrontReplacementSubgraph):
        ...
        return dict(
            nodes=[
                ...
                (
                    ...
                    dict(
                        op=lambda op: op
                        in ["FusedBatchNorm", "FusedBatchNormV2", "FusedBatchNormV3"]
                    ),
                ),
            ],
            edges=[
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        input = fbn.in_node(0)
        log.debug(
            "Found potential MVN pattern after {} with name {}".format(
                input.op, input.name
            )
        )
        if (
            input.id != match["mean"].in_node(0).id
            or input.id != match["sqdiff"].in_node(0).id
        ):
            return

        log.debug(
            "Confirmed MVN pattern after {} with name {}".format(input.op, input.name)
        )
    ...
    def infer(node: Node):
        ...
        if axes_1_value is None or axes_2_value is None:
            log.warning(
                "Reduction indices for mean and variance for MVN node {} are not constants".format(
                    node.name
                )
            )
            return
        ...
        if not (all(axes_1_value == axes_2_value)):
            log.warning(
                "Reduction indices for mean {} and variance {} do not match".format(
                    axes_1_value, axes_2_value
                )
            )
            return
