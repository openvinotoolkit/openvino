...
class BatchToSpaceNormalizer(FrontReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in graph.get_op_nodes(op="SpaceToBatch") + graph.get_op_nodes(
            ...
            block_shape = Pad(
                graph, {"name": node.name + "/aligned_block_shape", "mode": "constant"}
            ).create_node()
            ...
            begin = Pad(
                graph, {"name": node.name + new_begin_name, "mode": "constant"}
            ).create_node()
            end = Pad(
                graph, {"name": node.name + new_end_name, "mode": "constant"}
            ).create_node()
            ...
            for idx, input_to_node in enumerate(inputs_array):
                ...
                convert = Cast(
                    graph,
                    {"name": name_of_input_to_node + "/i64", "dst_type": np.int64},
                ).create_node()
