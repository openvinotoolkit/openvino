...
from openvino.tools.mo.front.tf.extractors.utils import tf_dtype_extractor, tf_int_list
        ...
        Shape.update_node_stat(
            node,
            {
                "output_type": tf_dtype_extractor(
                    node.pb.attr["out_type"].type, np.int32
                )
            },
        )
        return cls.enabled
...
class SqueezeExtractor(FrontExtractorOp):
    ...
    def extract(cls, node: Node):
        Squeeze.update_node_stat(
            node, {"squeeze_dims": tf_int_list(node.pb.attr["squeeze_dims"].list)}
        )
        return cls.enabled
