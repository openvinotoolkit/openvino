...
from openvino.tools.mo.front.tf.replacement import (
    FrontReplacementFromConfigFileSubGraph,
)
...
from openvino.tools.mo.ops.elementwise import Mul, Pow, Sub
...
class RetinaNetFilteredDetectionsReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def output_edges_match(
        self, graph: Graph, match: SubgraphMatch, new_sub_graph: dict
    ):
    ...
    def append_variances(priors_scale_node: Node, variance: list):
        ...
        shape_concat = create_op_node_with_second_input(
            graph,
            Concat,
            int64_array([4]),
            {
        ...
        variance = Const(
            graph, {"name": name + "/variance", "value": float32_array(variance)}
        ).create_node()
        ...
        output_node = Reshape(
            graph, {"name": name + "/3D_priors_wth_variances"}
        ).create_node()
    ...
    def placeholder_scales(self, placeholder: Node):
        ...
        ), "[ {} replacer ] Placeholder `{}` should have shape attribute".format(
            self.replacement_id, name
        )
        assert isinstance(
            shape_value, np.ndarray
        ), "[ {} replacer ] Placeholder `{}` shape attribute should be np.ndarray".format(
            self.replacement_id, name
        )
        assert (
            shape_value.size == 4
        ), "[ {} replacer ] Placeholder `{}` should be 4D. Shape: {}".format(
            self.replacement_id, name, shape_value
        )
        ...
        power = Const(graph, {"value": float32_array([-1.0])}).create_node()
        ...
        priors_scale_node = Concat(
            graph, {"axis": 0, "in_ports_count": 2}
        ).create_node()
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        begin = Const(
            graph, {"value": mo_array([0, 0, 0], dtype=np.int32)}
        ).create_node()
        end = Const(graph, {"value": mo_array([1, 0, 0], dtype=np.int32)}).create_node()
        stride = Const(
            graph, {"value": mo_array([1, 1, 1], dtype=np.int32)}
        ).create_node()
        ...
        assert len(placeholders) == 1, (
            "{} replacer requires model to have one Placeholder, but current model has "
        ...
        split_node = create_op_with_const_inputs(
            graph,
            VariadicSplit,
            {1: int64_array(2), 2: int64_array([1, 1, 1, 1])},
            {"out_ports_count": 4},
            priors_scale_node,
        )

        priors_width_node = Sub(
            graph, dict(name=split_node.name + "/sub_2-0_")
        ).create_node([(split_node, 2), (split_node, 0)])
        priors_height_node = Sub(
            graph, dict(name=split_node.name + "/sub_3-1_")
        ...
        concat_1 = Concat(
            graph, {"name": "concat_width_height", "axis": -1, "in_ports_count": 2}
        ).create_node([priors_width_node, priors_height_node])
        concat_2 = Concat(
            graph,
            {"name": "concat_width_height_width", "axis": -1, "in_ports_count": 2},
        ).create_node([concat_1, priors_width_node])
        concat_width_height_node = Concat(
            graph,
            {"name": "concat_priors_width_height", "axis": -1, "in_ports_count": 2},
        ).create_node([concat_2, priors_height_node])

        applied_width_height_regressions_node = Mul(
            graph, {"name": "final_regressions"}
        ...
        if iou_threshold is None:
            raise Error(
                "During {} `iou_threshold` was not retrieved from RetinaNet graph".format(
                    self.replacement_id
                )
            )
