    ...
    def find_and_replace_pattern(self, graph: Graph):
        for embedding_segments_mean in graph.get_op_nodes(op="EmbeddingSegmentsMean"):
            ...
            non_norm_range_1_to_num_segments = create_op_with_const_inputs(
                graph,
                Range,
                {0: int64_array(0), 2: int64_array(1)},
                {
                    "name": embedding_segments_mean_name + "/Range1ToNumSegments",
            ...
            range_1_to_num_segments = ConvertLike(
                graph,
                {"name": embedding_segments_mean_name + "/Range1ToNumSegmentsNorm"},
            ).create_node()
            ...
            unsqueeze_segment_ids = create_op_with_const_inputs(
                ...
                Unsqueeze,
                {1: int64_array(0)},
                {"name": embedding_segments_mean_name + "/SegmentIdsUnsqueeze"},
            )
            segment_ids_input.get_connection().add_destination(
                unsqueeze_segment_ids.in_port(0)
            )
            boolean_membership_matrix = Equal(
                graph,
                {"name": embedding_segments_mean_name + "/BooleanMembershipMatrix"},
            ).create_node()
            ...
            shape_of_membership_matrix = Shape(
                graph,
                {"name": embedding_segments_mean_name + "/ShapeOfMembershipMatrix"},
            ...
            one_constant = Broadcast(
                graph, {"name": embedding_segments_mean_name + "/One"}
            ...
            ).create_node([boolean_membership_matrix, one_constant, zero_constant])
            ...
            num_indices_per_object = create_op_with_const_inputs(
                ...
                ReduceSum,
                {1: int64_array(1)},
                {"name": embedding_segments_mean_name + "/NumIndicesPerObject"},
            )
            ...
            where_zero_number = Equal(
                graph,
                {"name": embedding_segments_mean_name + "/WhereZeroIndicesNumber"},
            ).create_node([num_indices_per_object, zero_constant])
            normalized_num_indices_per_object = Select(
                graph,
                {
                    "name": embedding_segments_mean_name + "/NormNumIndicesPerObject",
                    "auto_broadcast": "numpy",
                },
            ).create_node(
                [where_zero_number, one_scalar_constant, num_indices_per_object]
            )

            # 4. cast normalized_num_indices_per_object to the same type as embedding vector table
            norm_coefficients = ConvertLike(
                graph, {"name": embedding_segments_mean_name + "/NormCoefficients"}
            ).create_node()
            ...
            embedding_segments_sum = EmbeddingSegmentsSum(
                graph, {"name": embedding_segments_mean_name + "/EmbeddingSegmentsSum"}
            ).create_node()
            ...
            result_node = Div(
                graph, {"name": embedding_segments_mean_name + "/Div"}
