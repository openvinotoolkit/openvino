...
from openvino.tools.mo.front.tf.nearest_neighbor_upsampling import (
...
class FlattenToReshapeableReshape(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(graph: Graph, match: dict):
        ...
        if not const_node.has_valid("value") or not is_value_is_constant(
            ...
            log.debug(
                "The pattern does not correspond to flatten. The second reshape dimension is not -1. It is {}".format(
                    const_node.soft_get("value")
                )
            )
            return
        ...
        for ind in range(3):
            if not strided_slice_node.in_node(ind + 1).has_valid(
                "value"
            ) or not is_value_is_constant(
                strided_slice_node.in_node(ind + 1).value, expected_values[ind]
            ):
                log.debug(
        ...
        reshape_const_node = Const(
            graph,
            {
                "value": int64_array([0, -1]),
                "name": reshape_node.soft_get("name", reshape_node.id) + "/shape",
            },
        ).create_node()
        reshape_node.in_port(1).connect(reshape_const_node.out_port(0))
        reshape_node["special_zero"] = True
        log.debug(
            'The node "{}" is actually a Flatten node'.format(
