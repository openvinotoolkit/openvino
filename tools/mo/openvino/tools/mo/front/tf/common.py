...
from tensorflow.core.framework import (
    types_pb2 as tf_types,  # pylint: disable=no-name-in-module,import-error
)
...
_tf_np_mapping = [
    ("DT_BOOL", bool, lambda pb: pb.bool_val, lambda x: bool_cast(x)),
    ...
    (
        "DT_HALF",
        np.float16,
        lambda pb: np.uint16(pb.half_val).view(np.float16),
        lambda x: np.float16(x),
    ),
...
tf_data_type_decode = {
    getattr(tf_types, tf_dt): (np_type, func)
    for tf_dt, np_type, func, _ in _tf_np_mapping
    if hasattr(tf_types, tf_dt)
}

tf_data_type_cast = {
    np_type: cast
    for tf_dt, np_type, _, cast in _tf_np_mapping
    if hasattr(tf_types, tf_dt)
}
