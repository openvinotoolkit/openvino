...
class RFFTRealImagToRDFTSplit(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: [dict, SubgraphMatch]):
        ...
        split_node = create_op_with_const_inputs(
            ...
            Split,
            {1: int64_array(-1)},
            {"name": rfft_name + "/split", "num_splits": 2, "out_ports_count": 2},
        )
        squeeze_real = create_op_with_const_inputs(
            graph, Squeeze, {1: int64_array(-1)}, {"name": rfft_name + "/squeeze_real"}
        )
        squeeze_imag = create_op_with_const_inputs(
            graph, Squeeze, {1: int64_array(-1)}, {"name": rfft_name + "/squeeze_imag"}
        )
        ...
        rename_nodes(
            [(real_node, real_name + "/to_be_removed"), (squeeze_real, real_name)]
        )
        rename_nodes(
            [(imag_node, imag_name + "/to_be_removed"), (squeeze_imag, imag_name)]
        )
