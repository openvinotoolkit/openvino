...
class TFSliceToSliceReplacer(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        rename_nodes(
            [(tf_slice_node, slice_name + "/to_be_removed"), (slice_node, slice_name)]
        )
        ...
        ends_casted_to_i64 = Cast(
            graph, {"name": slice_name + "/CastToI64", "dst_type": np.int64}
        ).create_node([ends_node])
        # if size[i] == 1 then take max_ends values
        correct_ends = Select(graph, {"name": slice_name + "/chosen_ends"}).create_node(
