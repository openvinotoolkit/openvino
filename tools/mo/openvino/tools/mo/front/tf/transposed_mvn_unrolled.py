...
def check_applicability(match: dict) -> bool:
    ...
    eps_port_num = (
        0 if add.in_port(0).get_connection().get_source().node.id != variance.id else 1
    )
    ...
    if not (
        mean_reduction.has_valid("value") and variance_reduction.has_valid("value")
    ):
        log.debug(
    ...
    if not (all(mean_reduction.value == variance_reduction.value)):
        log.debug(
            "Reduction indices for mean {} and variance {} do not match.".format(
                mean_reduction.value, variance_reduction.value
            )
        )
        return False
    ...
    if pow2 != 0.5:
        log.debug(
            "Power for MVN node {} ({}) is not equal to 0.5".format(new_name, pow2)
        )
        return False
...
class TransposedMVNUnrolled(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        first_permute = create_op_node_with_second_input(
            ...
            Transpose,
            permute_order,
            dict(name=div_name + "/first_permute/MVN_T_"),
            new_reshape,
        )
        ...
        eps_port_num = (
            0
            if add.in_port(0).get_connection().get_source().node.id != variance.id
            else 1
        )
        eps = add.in_port(eps_port_num).get_connection().get_source().node
        ...
        second_permute = create_op_node_with_second_input(
            ...
            Transpose,
            permute_order,
            dict(name=div_name + "/second_permute/MVN_T_"),
            mvn_node,
        )
        new_reshape2 = Reshape(
            graph, dict(name=div_name + "/second_reshape/MVN_T_")
        ).create_node()
        second_permute.out_port(0).connect(new_reshape2.in_port(0))
        gamma_val = np.reshape(
            match["gamma_identity"].in_port(0).get_connection().get_source().node.value,
            int64_array([1, 1, 1, c]),
        )
        new_mul = create_op_node_with_second_input(
            ...
            Mul,
            gamma_val,
            dict(name=match["mul"].name + "/MVN_T_"),
            new_reshape2,
        )
        beta_val = np.reshape(
            match["beta_identity"].in_port(0).get_connection().get_source().node.value,
            int64_array([1, 1, 1, c]),
        )
        new_add2 = create_op_node_with_second_input(
            graph, Add, beta_val, dict(name=match["add2"].name + "/MVN_T_"), new_mul
        )
