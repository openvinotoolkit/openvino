...
class GNMT_sequence_lengths(FrontReplacementPattern):
    ...
    def pattern():
        log.debug(
            "+++++++++++++++ GNMT Sequence Lengths ConditionMatching ++++++++++++++++"
        )
        return dict(
            nodes=[
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        check_seq_len = looking_for_op_in_list(
            [
                port.node
                for port in encoder_sequence_lengths.out_port(0).get_destinations()
            ],
            "Identity",
        )
        ...
        tensor_seq_len = looking_for_op_in_list(
            [minimum.in_port(port).get_source().node for port in minimum.in_ports()],
            "StridedSlice",
        )

        # Create node for multiplying seq_len by 2
        const = Const(
            graph, {"name": "FakeSeqLenMultiplyer", "value": mo_array(2)}
        ).create_node()
