...
class MapFNInputSlicing(FrontReplacementSubgraph):
    ...
    def get_body_pattern():
        return dict(
            ...
            edges=[
                ...
                (
                    "increment_iteration_identity",
                    "increment_iteration_result",
                    {"in": 0},
                ),
            ],
        )
    ...
    def transform_map_fn_input_slicing(external_match: dict, internal_match: dict):
        ...
        Loop.update_port_map_value_ext(
            loop_node.input_port_map,
            "internal_layer_id",
            unstack_placeholder_layer_id,
            "axis",
            0,
        )
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for loop_node in graph.get_op_nodes(op="Loop"):
            ...
            internal_matches += find_subgraph_match_to_pattern(
                body_graph, body_pattern_without_identity
            )
            ...
            for internal_match in internal_matches:
                ...
                unstack_node = (
                    ...
                    if (
                        len(unstack_node) == 1
                        and unstack_node[0].op == "TensorListFromTensor"
                    )
                    else None
                )
...
class MapFNOutputConcatenation(FrontReplacementSubgraph):
    ...
    @staticmethod
    def transform_map_fn_output_concatenation(
        external_match: dict, internal_match: dict
    ):
        """
        ...
        Loop.update_port_map_value_ext(
            loop_node.output_port_map,
            "internal_layer_id",
            list_result_node_layer_id,
            "axis",
            0,
        )
        ...
        for record in loop_node.output_port_map:
            if "purpose" in record and record["purpose"] == "execution_condition":
                ...
                const_true = Const(
                    body_graph, {"value": mo_array(True, dtype=bool)}
                ).create_node()
        ...
        for record in loop_node.back_edges:
            if (
                "from_layer" in record
                and record["from_layer"] == list_result_node_layer_id
            ):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for loop_node in graph.get_op_nodes(op="Loop"):
            ...
            body_pattern_without_identity = (
                MapFNOutputConcatenation.get_body_pattern_without_identity()
            )
            internal_matches = find_subgraph_match_to_pattern(body_graph, body_pattern)
            internal_matches += find_subgraph_match_to_pattern(
                body_graph, body_pattern_without_identity
            ...
            for internal_match in internal_matches:
                ...
                reserve_node = (
                    ...
                    if (
                        len(reserve_node) == 1
                        and reserve_node[0].op == "TensorListReserve"
                    )
                    else None
                )
                if reserve_node is None:
                    log.info(
                        "A sub-graph around the loop node {} does not match "
                        "TensorFlow 2 MapFN pattern for intermediate outputs concatenation".format(
                            loop_name
                        )
                    )
                    continue
...
class TensorListOutputConcatenation(FrontReplacementSubgraph):
    ...
    @staticmethod
    def transform_tensor_list_output_concatenation(
        external_match: dict, internal_match: dict
    ):
        """
        Transforms TensorFlow 2 output concatenation into use of axis attribute for output port of Loop node
        ...
        unsqueeze_list_element = create_op_with_const_inputs(
            ...
            Unsqueeze,
            {1: int64_array(0)},
            {"name": tensor_list_push_back_node_name + "/TensorListPushBackUnsqueeze"},
        )
        ...
        for loop_node in graph.get_op_nodes(op="Loop"):
            ...
            for internal_match in internal_matches:
                ...
                reserve_node = (
                    ...
                    if (
                        len(reserve_node) == 1
                        and reserve_node[0].op == "EmptyTensorList"
                    )
                    else None
                )
                if reserve_node is None:
