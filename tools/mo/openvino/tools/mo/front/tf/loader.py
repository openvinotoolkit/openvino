...
from openvino.tools.mo.utils.environment_setup_utils import (  # pylint: disable=no-name-in-module,import-error
...
from openvino.tools.mo.graph.graph import Graph, fill_graph_with_nodes
from openvino.tools.mo.utils.summarize_graph import summarize_graph


def freeze_checkpoints(
    graph_def: tf_v1.GraphDef, checkpoint_dir: str, output_node_names: list
):
    """
    ...
    if len(checkpoint_files) == 0:
        raise Error(
            "There are no checkpoint files in directory: {}".format(checkpoint_dir)
        )
...
def freeze_checkpoint(graph_def, checkpoint, output_node_names):
    ...
    with tf_v1.Session() as sess:
        var_list = {}
        var_to_shape_map = tf_v1.train.NewCheckpointReader(
            checkpoint
        ).get_variable_to_shape_map()
        for key in var_to_shape_map:
            try:
...
def read_file_to_graph_def(
    graph_def: [tf_v1.GraphDef, tf_v1.MetaGraphDef],
    graph_file_name: str = "",
    is_binary: bool = True,
):
    """
    ...
    try:
        ...
        nodes_to_clear_device = (
            graph_def.node
            if isinstance(graph_def, tf_v1.GraphDef)
            else graph_def.graph_def.node
        )
...
def get_output_node_names_list(graph_def, user_defined_output_node_names_list: list):
    return (
        summarize_graph(graph_def)["outputs"]
        if user_defined_output_node_names_list is None
        or len(user_defined_output_node_names_list) == 0
...
def prepare_graph_def(model):
    ...
    if isinstance(model, tf.keras.Model):  # pylint: disable=no-member
        ...
        assert (
            hasattr(model, "inputs") and model.inputs is not None
        ), "Model inputs specification is required."
    ...
    if Version(env_setup["tensorflow"]) >= parse("2.6.0") and isinstance(
        model, tf.types.experimental.GenericFunction
    ):

        assert (
            hasattr(model, "input_signature") and model.input_signature is not None
...
def load_tf_graph_def(
    ...
    if isinstance(graph_file_name, str) and (
        re.match(r".*\.(ckpt|meta)$", graph_file_name)
    ):
        print(
    ...
    try:
        if graph_file_name and not meta_graph_file and not checkpoint:
            # frozen graph
            return (
                read_file_to_graph_def(graph_def, graph_file_name, is_binary),
                variables_values,
                "tf",
                None,
            )
        ...
        if not graph_file_name and meta_graph_file:
            ...
            with tf_v1.Session() as sess:
                ...
                outputs = get_output_node_names_list(
                    input_meta_graph_def.graph_def, user_output_node_names_list
                )
        ...
        if model_dir:
            ...
            try:
                ...
                tags = (
                    saved_model_tags
                    if saved_model_tags is not None
                    else [tf_v1.saved_model.tag_constants.SERVING]
                )
                with tf_v1.Session() as sess:
...
def protobuf2nx(graph, pb: tf_v1.GraphDef):
    fill_graph_with_nodes(
        graph, pb.node, get_id=lambda pb: pb.name, get_attrs=protobuf_attrs
    )
    ...
    if hasattr(pb, "library") and hasattr(pb.library, "function"):
        ...
        for library_function in pb.library.function:
            ...
            graph.graph["library"][function_name][
                "input_arg"
            ] = library_function.signature.input_arg
            graph.graph["library"][function_name][
                "output_arg"
            ] = library_function.signature.output_arg
            graph.graph["library"][function_name][
                "node_def"
            ] = library_function.node_def
    ...
    for _, attrs in list(graph.nodes(data=True)):
        ...
        if "_class" in pb.attr:
            ...
            while index < len(pb.attr["_class"].list.s):
                if re.match(
                    "^loc:@.*", pb.attr["_class"].list.s[index].decode("utf-8")
                ):
...
def variables_to_constants(graph: Graph, variables_values: dict):
    ...
    for node in graph.get_op_nodes(op="FakeConst"):
        ...
        if node_name not in variables_values:
            log.debug(
                "There is no value for '{}': {} in checkpoint variable values".format(
                    node.op, node_name
                )
            )
            continue
