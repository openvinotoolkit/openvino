    ...
    def extract(cls, node):
        ...
        def get_num_groups(node):
            if "group" in node:
                ...
                node.in_node(0).shape is not None
                and node.kernel_shape is not None
                and node.in_node(0).shape[node.channel_dims[0]] is not dynamic_dimension
                and node.kernel_shape[node.input_feature_channel]
                ...
                return (
                    node.in_node(0).shape[node.channel_dims]
                    // node.kernel_shape[node.input_feature_channel]
                )
            else:
                return 1
        ...
        attrs.update(
            {
                ...
                "get_output_feature_dim": lambda node: node.kernel_shape[
                    node.output_feature_channel
                ],
        ...
        attrs.update(
            {
                ...
                "get_group": lambda node: node.kernel_shape[
                    node.output_feature_channel
                ],
                "get_output_feature_dim": lambda node: node.kernel_shape[-1]
                * node.kernel_shape[-2],
                "get_weights_permute": PermuteAttrs.Permutation(
                    perm=int64_array([2, 3, 0, 1]), inv=int64_array([2, 3, 0, 1])
                ),
            }
        )
        ...
        attrs.update(
            {
                ...
                "get_weights_permute": PermuteAttrs.Permutation(
                    perm=int64_array([4, 3, 0, 1, 2]), inv=int64_array([2, 3, 4, 1, 0])
                ),
            }
        )
