    ...
    def replace_sub_graph(graph: Graph, match: dict, **kwargs):
        ...
        if transpose_order is None or not np.all(
            ...
            log.debug(
                ...
                "BatchToSpaceToUpsample transformation.".format(
        ...
        if expand_axis_value != 0:
            log.debug(
                "The expand axis {} for node {} is not equal to 0. Cannot apply BatchToSpaceToUpsample "
        ...
        if tile_value is None:
            log.debug(
                "The tile value is not defined for node {}. Cannot apply BatchToSpaceToUpsample "
        ...
        if len(np.where(tile_value != 1)) != 1:
            log.debug(
                "The number of tiles not equal to 1 not equal to 1. Cannot apply BatchToSpaceToUpsample "
                "transformation."
            )
            return
        ...
        if len(block_shape) != 2:
            log.debug(
                "The block shape len is not equal to 2 for node {}. Cannot apply BatchToSpaceToUpsample "
        ...
        if crops is None or np.count_nonzero(crops) != 0:
            log.debug(
                "Crops for node {} are non zero. Cannot apply BatchToSpaceToUpsample "
                "transformation.".format(batch_to_space_nd.name)
            )
            return
