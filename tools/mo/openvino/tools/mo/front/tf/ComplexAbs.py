...
class ComplexAbs(FrontReplacementSubgraph):
    ...
    def run_after(self):
        from openvino.tools.mo.front.tf.ComplexAbsAfterComplex import (
            ComplexAbsAfterComplex,
        )
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for complex_abs in graph.get_op_nodes(op="ComplexAbs"):
            ...
            sqrt = create_op_with_const_inputs(
                graph, Pow, {1: power_type(0.5)}, {}, sum
            )
            ...
            rename_nodes(
                [
                    (complex_abs, complex_abs_name + "/to_be_removed"),
