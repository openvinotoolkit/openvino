...
class WhereDecomposition(FrontReplacementOp):
    ...
    def run_after(self):
        ...
        from openvino.tools.mo.front.TransposeOrderNormalizer import (
    ...
    def replace_op(self, graph: Graph, node: Node):
        ...
        non_zero_node = NonZero(
            graph, {"name": node_name + "/NonZero_", "output_type": np.int64}
        ).create_node()
        transpose_node = create_op_node_with_second_input(
            graph, Transpose, int64_array([1, 0]), op_attrs={}
        )
