...
def update_body_graph(
    ...
    for output in subgraph_proto["output_arg"]:
        ...
        src_id = map_original_name[orig_src_id]
        src_port = (
            0 if len(output_name.split(":")) == 1 else int(output_name.split(":")[-1])
        )
        assert body_graph.has_node(
            src_id
        ), 'The body graph does not contain output with name "{}"'.format(src_id)
        body_results.append(
            Node(body_graph, add_opoutput(body_graph, src_id, src_port, False))
        )
...
def get_graph_proto(external_graph: Graph, graph_id: str, node_with_graph: Node):
    ...
    assert "library" in external_graph.graph, (
        "The graph does not contain a library that is required "
        'by node with name "{}".'.format(node_name)
    )

    library_graph = external_graph.graph["library"]

    assert graph_name in library_graph, (
        'The library does not contain a function with name "{}" '
...
def convert_graph_inputs_to_parameters(internal_graph, internal_graph_proto):
    ...
    for idx, pb_node in enumerate(internal_graph_proto["input_arg"]):
        ...
        internal_graph.add_node(
            param_id, name=param_id, kind="op", op="Parameter", pb=None, shape=None
        )
