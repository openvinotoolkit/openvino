...
class GRUBlockCellToGRUCell(FrontReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for tf_gru_block_cell in graph.get_op_nodes(op="GRUBlockCell"):
            ...
            rename_nodes(
                [
                    (tf_gru_block_cell, original_name + "/to_be_removed"),
                    (new_gru_cell, original_name),
                ]
            )

            # Connect X data port
            tf_gru_block_cell.in_port(0).get_connection().set_destination(
                new_gru_cell.in_port(0)
            )
            # Connect hidden state port
            tf_gru_block_cell.in_port(1).get_connection().set_destination(
                new_gru_cell.in_port(1)
            )

            # W (Weights)
            # z - update, r - reset, h - hidden
            # Convert gate order W_rz, W_h -> W_zrh
            split_rz_w = AttributedSplit(
                graph,
                {"name": original_name + "/Split_W_rz", "axis": 1, "num_splits": 2},
            ).create_node()

            # Split W_rz to W_r and W_z
            tf_gru_block_cell.in_port(2).get_connection().set_destination(
                split_rz_w.in_port(0)
            )

            concat_zrh_w = Concat(
                graph,
                {
                    "name": original_name + "/Concat_W_zrh",
                    "in_ports_count": 3,
                    "axis": 1,
                },
            ).create_node()
            ...
            tf_gru_block_cell.in_port(3).get_connection().set_destination(
                concat_zrh_w.in_port(2)
            )

            # B (Bias)
            # z - update, r - reset, h - hidden
            # Convert gate order B_rz, B_h -> B_zrh
            split_rz_b = AttributedSplit(
                graph,
                {"name": original_name + "/Split_B_rz", "axis": 0, "num_splits": 2},
            ).create_node()

            # Split B_rz to B_r and B_z
            tf_gru_block_cell.in_port(4).get_connection().set_destination(
                split_rz_b.in_port(0)
            )

            concat_zrh_b = Concat(
                graph,
                {
                    "name": original_name + "/Concat_B_zrh",
                    "in_ports_count": 3,
                    "axis": 0,
                },
            ).create_node()
            ...
            tf_gru_block_cell.in_port(5).get_connection().set_destination(
                concat_zrh_b.in_port(2)
            )
            ...
            transpose_w = create_op_node_with_second_input(
                graph,
                Transpose,
                permute_order,
                dict(name=original_name + "/Transpose_W"),
                concat_zrh_w,
            )
            ...
            tf_gru_block_cell.out_port(3).get_connection().set_source(
                new_gru_cell.out_port(0)
            )
