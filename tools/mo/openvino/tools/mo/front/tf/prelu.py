...
class PReLUPatternFuse(FrontReplacementSubgraph):
    ...
    def pattern(self):
        return dict(
            nodes=[
                ...
                (
                    "neg",
                    dict(kind="op", op="AttributedPower", scale=-1, power=1, shift=0),
                ),
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        gamma = (
            match["mul"].in_node(0)
            if match["mul"].in_node(1).id == match["neg_1"].id
            else match["mul"].in_node(1)
        )
        prelu_node = PReLU(
            graph, {"name": "{}/PReLU".format(match["add"].id)}
...
class PReLUWithAbsPatternFuse(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        consumers = [
            n
            for n in match
            if n not in ["mul", "mul_1", "op", "add", "abs", "sub"]
            and not check_node_usages_out_of_match(match, n)
        ]
        if consumers:
            log.warning(
                'PReLUWithAbs pattern was detected. Non pattern consumers of nodes: "{}" were found. Won\'t '
                "replace ".format(", ".join([match[n].id for n in consumers]))
            )
            return
        gamma = (
            match["mul"].in_node(0)
            if match["mul"].in_node(1).id == match["sub"].id
            else match["mul"].in_node(1)
        )
        prelu_node = PReLU(
            graph, {"name": "{}/PReLU".format(match["add"].id)}
        ...
        match["add"].replace_node(prelu_node)
        log.debug(
            'PReLUWithAbs pattern starting from "{}" was collapsed to "{}"'.format(
                match["op"].id, prelu_node.id
            )
        )
