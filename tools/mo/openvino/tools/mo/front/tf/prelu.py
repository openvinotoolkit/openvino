...
class PReLUWithSubPatternFuse(FrontReplacementSubgraph):
    enabled = True
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        consumers = [n for n in match if n not in ['mul', 'op', 'sub'] and not check_node_usages_out_of_match(match, n)]
        if consumers:
            log.warning('PReLU pattern was detected. Non pattern consumers of nodes: "{}" were found. Won\'t replace'
                        ''.format(', '.join([match[n].id for n in consumers])))
            return
        gamma = match['mul'].in_node(0) if match['mul'].in_node(1).id == match['neg_relu'].id else match['mul'].in_node(1)
        prelu_node = PReLU(graph, {'name': '{}/PReLU'.format(match['sub'].id)}).create_node([match['op'], gamma])
        match['sub'].replace_node(prelu_node)
        log.debug('PReLU pattern starting from "{}" was collapsed to "{}"'.format(match['op'].id, prelu_node.id))