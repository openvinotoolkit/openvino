...
from openvino.tools.mo.front.common.partial_infer.utils import (
    convert_deconv_tf_padding_to_str,
    dynamic_dimension,
    int64_array,
)
...
def tf_create_attrs(node, input_feature_channel, output_feature_channel):
    ...
    return {
        "auto_pad": convert_deconv_tf_padding_to_str(
            node.pb.attr["padding"].s.decode()
        ),
        "bias_addable": True,
        "bias_term": False,
...
def get_conv_backprop_groups(node):
    ...
    if node.has_and_set("group"):
        ...
        return (
            output_shape[node.channel_dims] // kernel_shape[node.output_feature_channel]
        )
    else:
        return 1
