...
class CropAndResizeFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        if method != "bilinear":
            log.warning(
                'The crop and resize method "{}" for node "{}" is not supported.'.format(
                    method, node.soft_get("name")
                )
            )
            return False
        ROIPooling.update_node_stat(
            node, {"spatial_scale": 1, "op": "CropAndResize", "method": method}
        )
        return cls.enabled
