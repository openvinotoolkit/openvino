...
class UnpackPackReverseInputChannels(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        if unpack.soft_get("axis", None) is None or unpack.axis != pack.soft_get(
            "axis", None
        ):
        ...
        connected_unpack_ports_count = len(
            [port for port in unpack.out_ports().values() if not port.disconnected()]
        )
        connected_pack_ports_count = len(
            [port for port in pack.in_ports().values() if not port.disconnected()]
        )
        if (
            connected_pack_ports_count != connected_unpack_ports_count
            or connected_unpack_ports_count != 3
        ):
        ...
        reverse_channels = ReverseChannels(
            graph,
            {
