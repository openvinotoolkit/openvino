...
class EfficientDet(FrontReplacementFromConfigFileGeneral):
        ...
        def get(self, layer_id):
            ...
            for s in self.scales:
                for a in self.aspect_ratios:
                    base_anchor_size = (
                        2 ** (self.min_level + layer_id) * self.anchor_scale
                    )
    ...
    def transform_graph(self, graph: Graph, replacement_descriptions: dict):
        ...
        graph.remove_nodes_from(
            [node.id for node in graph.get_op_nodes(op="Result")] + ["detections"]
        )
        ...
        assert preprocessing_input_node_id in graph.nodes, (
            'The node with name "{}" is not found in the graph. This '
        ...
        consumer_node = (
            preprocessing_input_node.out_port(0).get_connection().get_destination().node
        )
        ...
        assert preprocessing_output_node_id in graph.nodes, (
            'The node with name "{}" is not found in the graph. This '
        ...
        for i in range(100):
            ...
            prior_boxes.append(
                prior_box_op.create_node([Node(graph, inp_name), parameter_node])
            )
