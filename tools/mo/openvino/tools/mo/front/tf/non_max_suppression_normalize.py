...
class TFNonMaxSuppressionNormalize(FrontReplacementSubgraph):
    ...
    def run_after(self):
        from openvino.tools.mo.front.non_max_suppression_normalize import (
            NonMaxSuppressionNormalize,
        )
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for nms in graph.get_op_nodes(op="NonMaxSuppression"):
            ...
            num_of_outputs = len(
                [port for port in nms.out_ports().values() if not port.disconnected()]
            )
