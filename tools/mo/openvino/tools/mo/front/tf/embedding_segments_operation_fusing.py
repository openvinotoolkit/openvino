...
class EmbeddingSegmentsOperationSingleFeatureFusing(FrontReplacementSubgraph):
    ...
    def pattern(self):
        return dict(
            nodes=[
                ...
                (
                    "sparse_segment_op",
                    dict(op=lambda op: op in ["SparseSegmentSum", "SparseSegmentMean"]),
                ),
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        log.debug(
            "Found EmbeddingSparseSegmentsSingleFeature pattern after {} with name {}".format(
                sparse_fill_empty_rows.op, sparse_fill_empty_rows.name
            )
        )

        split_for_indices = create_op_with_const_inputs(
            graph, Split, {1: int64_array(1)}, {"num_splits": 2}
        )
        ...
        cast_indices = Cast(
            graph, {"name": output_node_name + "/CastIndices", "dst_type": np.int32}
        ).create_node()
        cast_segment_ids = Cast(
            graph, {"name": output_node_name + "/CastSegmentIds", "dst_type": np.int32}
        ).create_node()
        cast_default_value = Cast(
            graph,
            {"name": output_node_name + "/CastDefaultValue", "dst_type": np.int32},
        ).create_node()
        cast_num_segments = Cast(
            graph,
            {"name": output_node_name + "/CastSegmentsNumber", "dst_type": np.int32},
        ).create_node()
        if sparse_segment_op.op == "SparseSegmentSum":
            embedding_segments_op = EmbeddingSegmentsSum(
                graph, {"name": output_node_name}
            ).create_node()
        else:
            embedding_segments_op = EmbeddingSegmentsMean(
                graph, {"name": output_node_name}
            ).create_node()
        rename_nodes(
            [
                (select, output_node_name + "/AbandonedName"),
                (embedding_segments_op, output_node_name),
            ]
        )

        # connect parameters table
        gather.in_port(0).get_connection().set_destination(
            embedding_segments_op.in_port(0)
        )
        # connect indices values
        greaterequal0.in_port(0).get_connection().set_destination(
            cast_indices.in_port(0)
        )
        ...
        select.out_port(0).get_connection().set_source(
            embedding_segments_op.out_port(0)
        )
...
class EmbeddingSegmentsOperationMultipleFeaturesFusing(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        log.debug(
            "Found EmbeddingSparseSegmentsMultipleFeatures pattern after {} with name {}".format(
                sparse_fill_empty_rows.op, sparse_fill_empty_rows.name
            )
        )
        ...
        split_for_indices = create_op_with_const_inputs(
            ...
            Split,
            {1: int64_array(1)},
            {"num_splits": 2, "name": output_node_name + "/SplitForIndices"},
        )
        squeeze_for_indices = create_op_with_const_inputs(
            graph, Squeeze, {1: int64_array([1])}
        )
        split_for_dense_shape = create_op_with_const_inputs(
            ...
            Split,
            {1: int64_array(0)},
            {"num_splits": 2, "name": output_node_name + "/SplitForDenseShape"},
        )
        squeeze_to_scalar = create_op_with_const_inputs(
            graph, Squeeze, {1: int64_array([0])}
        )
