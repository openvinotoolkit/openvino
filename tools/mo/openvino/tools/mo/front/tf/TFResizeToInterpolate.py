...
def replace_tf_resize(graph: Graph, resize: Node, interpolation_mode: str):
    ...
    assert (
        num_of_inputs == 2
    ), "Number of inputs of {} (with name {}) should be equal to 2".format(
        resize.op, resize_name
    )
    ...
    assert not resize.half_pixel_centers or (
        resize.half_pixel_centers and not resize.align_corners
    ), attrs_msg.format(resize_name, resize.op)

    if resize.has_valid("data_type") and not np.issubdtype(
        resize.data_type, np.floating
    ):
        input_cast = Cast(
            graph, {"name": resize_name + "/to_f32", "dst_type": np.float32}
        ).create_node()
    ...
    ss = create_op_with_const_inputs(
        graph,
        StridedSlice,
        {1: int64_array([1]), 2: int64_array([3]), 3: int64_array([1])},
        {
    ...
    if align_corners:
        ...
        coordinate_transformation_mode = (
            "tf_half_pixel_for_nn" if interpolation_mode == "nearest" else "half_pixel"
        )
    else:
