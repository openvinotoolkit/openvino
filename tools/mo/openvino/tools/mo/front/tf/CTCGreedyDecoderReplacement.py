...
def replace_ctc_greedy_decoder(graph: Graph, match: dict):
    ...
    assert ctc_greedy_decoder_tf.has_valid(
        "merge_repeated"
    ...
    rename_nodes(
        [
            (sparse_to_dense, sparse_to_dense_name + "/AbandonedName"),
    ...
    def pattern(**kwargs):
        return dict(
            nodes=[
                (
                    "decoder",
                    dict(op="CTCGreedyDecoderSeqLen", output_sparse_format=True),
                ),
                ("cast", dict(op="Cast")),
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for ctc_greedy_decoder_tf in graph.get_op_nodes(
            ...
            for port_num in ctc_greedy_decoder_tf.out_ports():
                if (
                    not ctc_greedy_decoder_tf.out_port(port_num).disconnected()
                    and ctc_greedy_decoder_tf.out_port(port_num)
                    .get_destination()
                    .node.soft_get("op")
                    != "Result"
                ):
