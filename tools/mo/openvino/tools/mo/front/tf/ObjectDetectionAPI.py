...
from openvino.tools.mo.front.extractor import add_output_ops, output_user_data_repack
...
from openvino.tools.mo.front.tf.graph_utils import (
    add_activation_function_after_node,
    add_convolution_to_swap_xy_coordinates,
    add_fake_background_loc,
    create_op_node_with_second_input,
    create_op_with_const_inputs,
)
from openvino.tools.mo.front.tf.MapFNTransformation import (
    MapFNInputSlicing,
    MapFNOutputConcatenation,
...
from openvino.tools.mo.ops.elementwise import Add, Div, Mul, Sub
...
missing_param_error = (
    "To convert the model specify path to the pipeline configuration file which was used to "
...
def _value_or_raise(match: SubgraphMatch, pipeline_config: PipelineConfig, key: str):
    ...
    if value is None:
        raise Error(
            'The sub-graph replacer "[REPLACEMENT_ID]" was not able to find the value for key "{}" in the '
            ...
            'value to the "custom_attributes" dictionary of the "[REPLACEMENT_ID]" replacer.'.format(
                key, key
            )
        )
    return value
...
def _find_ssd_head_node(graph: Graph, ssd_head_index: int, head_type: str):
    ...
    if head_type == "box":
        possible_node_names = [
            ...
            (
                "WeightSharedConvolutionalBoxPredictor/BoxPredictor/BiasAdd"
                if ssd_head_index == 0
                else "WeightSharedConvolutionalBoxPredictor_%d/BoxPredictor/BiasAdd"
                % ssd_head_index
            ),
        ]
        ...
        possible_node_names = [
            ...
            (
                "WeightSharedConvolutionalBoxPredictor/ClassPredictor/BiasAdd"
                if ssd_head_index == 0
                else "WeightSharedConvolutionalBoxPredictor_%d/ClassPredictor/BiasAdd"
                % ssd_head_index
            ),
        ]
    else:
    ...
    for head_node_name in possible_node_names:
        if head_node_name in graph.nodes():
            assert (
                head_node is None
            )  # only one of the possible node names should exist in the graph
...
def _variance_from_pipeline_config(pipeline_config: PipelineConfig):
    ...
    return 1.0 / mo_array(
        [
            pipeline_config.get_param("frcnn_variance_x"),
...
def _relax_reshape_nodes(graph: Graph, pipeline_config: PipelineConfig):
    ...
    if num_layers is None:
        num_layers = (
            pipeline_config.get_param("multiscale_anchor_generator_max_level")
            - pipeline_config.get_param("multiscale_anchor_generator_min_level")
            + 1
        )
    for ssd_head_ind in range(num_layers):
        input_node = _find_ssd_head_node(graph, ssd_head_ind, "box")
        assert input_node is not None
        old_reshape_node = _skip_node_of_type(
            input_node.out_node(),
            ["Identity", "FakeQuantWithMinMaxVars", "FakeQuantize"],
        )
        assert old_reshape_node.op == "Reshape"
        reshape_size_node = Const(
            graph, {"value": int64_array([0, -1, 1, 4])}
        ).create_node([])
        ...
        assert old_reshape_node.op == "Reshape"
        reshape_size_node_2 = Const(
            graph, {"value": int64_array([0, -1, num_classes + 1])}
        ).create_node([])
        new_reshape_op_2 = Reshape(graph, {"name": input_node.id + "/Reshape"})
        new_reshape_node_2 = new_reshape_op_2.create_node(
            [input_node, reshape_size_node_2]
        )
...
def _create_prior_boxes_node(graph: Graph, pipeline_config: PipelineConfig):
    ...
    _base_anchor_height = pipeline_config.get_param(
        "ssd_anchor_generator_base_anchor_height"
    )
    _base_anchor_width = pipeline_config.get_param(
        "ssd_anchor_generator_base_anchor_width"
    )
    base_anchor_size = [
        min_im_shape / image_height * _base_anchor_height,
    ...
    if pipeline_config.get_param("ssd_anchor_generator_reduce_lowest") is not None:
        reduce_boxes_in_lowest_layer = pipeline_config.get_param(
            "ssd_anchor_generator_reduce_lowest"
        )
    ...
    for ssd_head_ind in range(num_layers):
        ...
        if ssd_head_ind == 0 and reduce_boxes_in_lowest_layer:
            ...
            interpolated_scale_ar = pipeline_config.get_param(
                "ssd_anchor_generator_interpolated_scale_aspect_ratio"
            )
            if interpolated_scale_ar > 0.0:
                widths += [
                    sqrt(scales[ssd_head_ind] * scales[ssd_head_ind + 1])
                    * interpolated_scale_ar
                ]
                heights += [
                    sqrt(scales[ssd_head_ind] * scales[ssd_head_ind + 1])
                    / interpolated_scale_ar
        ...
        # Placeholder
        input_node_name = (
            "image_tensor" if "image_tensor" in graph.nodes else "input_tensor"
        )
        prior_box_node = prior_box_op.create_node(
            [ssd_head_node, Node(graph, input_node_name).out_node(0)],
    ...
    if len(prior_box_nodes) == 1:
        ...
        concat_prior_boxes_op = Concat(
            graph, {"axis": -1, "in_ports_count": len(prior_box_nodes)}
        )
        return concat_prior_boxes_op.create_node(
            prior_box_nodes, {"name": "ConcatPriorBoxesClustered"}
        )
...
def _create_multiscale_prior_boxes_node(graph: Graph, pipeline_config: PipelineConfig):
    ...
    aspect_ratios = pipeline_config.get_param(
        "multiscale_anchor_generator_aspect_ratios"
    )
    scales_per_octave = pipeline_config.get_param(
        "multiscale_anchor_generator_scales_per_octave"
    )

    prior_box_nodes = []
    scales = [
        2 ** (float(scale) / scales_per_octave) for scale in range(scales_per_octave)
    ]
    for level in range(min_level, max_level + 1):
        ...
        widths = [
            base_anchor_size * scale * sqrt(ar)
        ...
        # Placeholder
        prior_box_node = prior_box_op.create_node(
            [ssd_head_node, Node(graph, "image_tensor").out_node(0)],
            {"name": "PriorBoxClustered_{}".format(ssd_head_ind)},
        )
        prior_box_nodes.append(prior_box_node)
    if len(prior_box_nodes) == 1:
...
def insert_weights_swap_xy_sub_graph(graph: Graph, connection):
    ...
    origin_shape = Shape(
        graph, {"name": name + "/OriginShape", "force_dead_node": True}
    ).create_node()
    origin_shape.in_port(0).connect(weights_producer)

    reshaped = create_op_node_with_second_input(
        graph, Reshape, int64_array([-1, 2]), {"name": name + "/Reshape2D"}
    )
    reshaped.in_port(0).connect(weights_producer)

    swapped_weight = Gather(graph, {"name": name + "/SwappedWeights"}).create_node()
    gather_indices = Const(
        graph, {"name": swapped_weight.name + "/Indices", "value": int64_array([1, 0])}
    ).create_node()
    gather_axis = Const(
        graph, {"name": swapped_weight.name + "/Axis", "value": int64_array(1)}
    ).create_node()
...
def calculate_shape_keeping_aspect_ratio(
...
def calculate_placeholder_spatial_shape(
    ...
    if match and (
        "preprocessed_image_height" in match.custom_replacement_desc.custom_attributes
        or "preprocessed_image_width" in match.custom_replacement_desc.custom_attributes
    ):
        log.error(
            'The "preprocessed_image_height" or "preprocessed_image_width" is specified in the sub-graph '
            "replacement configuration file but they are ignored. Please, specify desired input shape using the "
            '"--input_shape" command line parameter.',
    ...
    if user_shapes and input_name in user_shapes and user_shapes[input_name]:
        ...
        if user_defined_shape is not None:
            user_defined_height = (
                user_defined_shape[1].get_min_length()
                if user_defined_shape[1].is_static
                else dynamic_dimension_value
            )
            user_defined_width = (
                user_defined_shape[2].get_min_length()
                if user_defined_shape[2].is_static
                else dynamic_dimension_value
            )
    ...
    if resizer_height and resizer_width:
        log.debug(
            "The model resizes image to a fixed shape: ({}, {})".format(
                resizer_height, resizer_width
            )
        )
        if user_defined_height and user_defined_width:
            if (
                user_defined_width != resizer_width
                or user_defined_width != resizer_width
            ):
                log.error(
                    "The model expects that the input image is resized to a fixed shape ({}, {}), but the shape "
    ...
    if resizer_min_dimension and resizer_max_dimension:
        log.debug(
            "The model resizes image using keep aspect ratio with minimum size {}, maximum size {}, pad {}."
            "".format(
                resizer_min_dimension, resizer_max_dimension, pad_to_max_dimension
            )
        )
        if pad_to_max_dimension:
            if user_defined_height and user_defined_width:
                log.error(
                    "The model contains pre-processing block which resizes image keeping aspect ratio with a "
                    ...
                    "".format(resizer_max_dimension, resizer_max_dimension),
                    extra={"is_warning": True},
                )
            height = width = resizer_max_dimension
        else:
            log.error(
                "Model Optimizer removes pre-processing block of the model which resizes image keeping aspect "
            ...
            if user_defined_height and user_defined_width:
                scaled_height, scaled_width = calculate_shape_keeping_aspect_ratio(
                    user_defined_height,
                    user_defined_width,
                ...
                if (
                    scaled_height != user_defined_height
                    or scaled_width != user_defined_width
                ):
                    log.error(
                        "The model resizes the input image keeping aspect ratio with min dimension {}, max "
                ...
                log.error(
                    'Specify the "--input_shape" command line parameter to override the default shape which is '
                    "equal to ({}, {}).".format(height, width),
                    extra={"is_warning": True},
                )

    if height is None or width is None:
        raise Error(
            "Failed to determine the placeholder shape. Unsupported image resizer from the pipeline.config was "
...
def update_parameter_shape(graph: Graph, match: [SubgraphMatch, None]):
    ...
    initial_input_node_name = (
        "input_tensor" if "input_tensor" in graph.nodes else "image_tensor"
    )
...
def mark_squeeze_reshape_concat_before_detection_output(start_nodes: list):
    ...
    while len(q) != 0:
        ...
        if cur_node.has_valid("type"):
            if (
                cur_node.soft_get("type") == "DetectionOutput"
            ):  # do not go beyond the DetectionOutput node
                continue
        ...
        if cur_node.has_port("out", 0):
            [
                q.append(port.node)
                for port in cur_node.out_port(0).get_destinations()
                if port.node.id not in visited
            ]
...
def get_specific_ops_with_const_inputs(
    ...
    """
    node = (
        first_node.out_port(0).get_destination().node
        if forward
        else first_node.in_port(0).get_source().node
    )
    ...
    while node.soft_get("op") in allowed_ops:
        ...
        assert (
            num_in_ports == 2
        ), 'The node "{}" should have exactly 2 inputs, but it has only {}.' "".format(
            node.soft_get("name", node.id), num_in_ports
        )
        for port in (0, 1):
            if (
                node.in_port(port).get_source().node.has_valid("value")
            ):  # this is a constant input to the node
                result.append(
                    (node, port, node.in_port(port).get_source().node.value.copy())
                )
                node = (
                    node.out_port(0).get_destination().node
                    if forward
                    else node.in_port(1 - port).get_source().node
                )
                break
    return result


def get_preprocessing_ops(
    graph: Graph, start_node_id_suffix: str, end_node_id_suffix: str
):
    """
    ...
    preprocessing_nodes = get_specific_ops_with_const_inputs(
        start_node, allowed_ops, False
    )
...
class ObjectDetectionAPIPreprocessorReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def is_preprocessing_applied_before_resize(
        self, to_float: Node, mul: Node, sub: Node
    ):
        """
        ...
        assert (
            sub is not None
        ), "The Sub node should not be None. Check the caller function."
        if mul is not None:
            return any(
                [
                    port.node.id == mul.id
                    for port in to_float.out_port(0).get_destinations()
                ]
            )
        else:
            return any(
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        if sub_node.soft_get("op") != "Sub":
            raise Error(
                'The output op of the Preprocessor sub-graph is not of type "Sub". Looks like the topology is '
        ...
        if to_float_node.soft_get("op") != "Cast":
            raise Error(
                'The output of the node "{}" is not Cast operation. Cannot apply transformation.'.format(
                    initial_input_node_name
                )
            )

        if self.is_preprocessing_applied_before_resize(
            to_float_node, mul_node, sub_node
        ):
            # connect sub node directly to nodes which consume resized image
            resize_output_node_id = (
                "Preprocessor/map/TensorArrayStack/TensorArrayGatherV3"
            )
            if resize_output_node_id not in graph.nodes:
                raise Error(
                    'There is no expected node "{}" in the graph.'.format(
                        resize_output_node_id
                    )
                )
        ...
        log.error(
            "The Preprocessor block has been removed. Only nodes performing mean value subtraction and scaling "
            "(if applicable) are kept.",
            extra={"is_warning": True},
        )
...
class ObjectDetectionAPIPreprocessor2Replacement(FrontReplacementFromConfigFileGeneral):
    ...
    def transform_graph(self, graph: Graph, replacement_descriptions: dict):
        ...
        sub_graph_node_ids = sub_graph_between_nodes(
            graph,
            start_nodes,
            end_nodes,
        ...
        loop_nodes_ids = [
            ...
            if graph.nodes[node_id].get("op") == "Loop"
        ]
        if len(loop_nodes_ids):
            assert (
                len(loop_nodes_ids) == 1
            ), "There should be exactly one Loop node in the pre-processor block."
            ...
            # them
            pre_processing_ops, trailing, casting = get_preprocessing_ops(
                body_graph,
            ...
            pre_processing_ops, trailing, casting = get_preprocessing_ops(
                graph, start_node.id, end_node.id
            )
        ...
        if len(pre_processing_ops):
            ...
            if pre_processing_in_loop:  # case 4 and 5
                ...
                for idx in range(len(pre_processing_ops)):
                    ...
                    new_node = create_op_with_const_inputs(
                        graph, ops_mapping[origin_node.op], {const_port_ind: value}
                    )
                    if len(new_preprocessing_ops):
                        new_node.in_port(1 - const_port_ind).connect(
                            new_preprocessing_ops[-1].out_port(0)
                        )
                ...
                end_node.out_port(0).get_connection().set_source(
                    new_preprocessing_ops[-1].out_port(0)
                )
                start_node.in_port(0).get_connection().set_destination(
                    new_preprocessing_ops[0].in_port(
                        int(new_preprocessing_ops[0].is_in_port_connected(0))
                    )
                )
            else:
                if trailing:  # case 2
                    ...
                    if pad_to_max_dimension:
                        ...
                        consumer_port = first_pre_processing_node.in_port(
                            int(not pre_processing_ops[0][1])
                        )
                        end_node.out_port(0).get_connection().set_source(
                            consumer_port.get_connection().get_source()
                        )
                    else:
                        # change output of the end_node to be produced with the last preprocessing op
                        end_node.out_port(0).get_connection().set_source(
                            pre_processing_ops[-1][0].out_port(0)
                        )
            ...
            end_node.out_port(0).get_connection().set_source(
                start_node.in_port(0).get_source()
            )

        if mean_scale_kept:
            log.error(
                "The pre-processing block has been removed. Only nodes performing mean value subtraction and "
            ...
            log.error(
                "The Preprocessor block has been removed including mean value subtraction and scaling (if "
                "applicable). It is necessary to resize, scale and pad an input image using the same algorithm "
                "as in the original model before feeding it to the OpenVINO.",
                extra={"is_warning": True},
            )


class ObjectDetectionAPIDetectionOutputReplacement(
    FrontReplacementFromConfigFileSubGraph
):
    """
    ...
    def nodes_to_remove(self, graph: Graph, match: SubgraphMatch):
        ...
        for output in outputs:
            if output in graph.nodes:
                ...
                if len(children) != 1:
                    log.warning(
                        "Output {} has {} children. It should have only one output: with op==`Result`"
    ...
    def output_edges_match(
        ...
        if match.outputs_count() >= 1:
            return {
                match.output_node(0)[0].id: new_sub_graph["detection_output_node"].id
            }
        else:
            return {
                list(graph.graph.get("packed_outputs").keys())[0]: new_sub_graph[
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        max_proposals = _value_or_raise(
            match, pipeline_config, "first_stage_max_proposals"
        )
        activation_function = _value_or_raise(
            match, pipeline_config, "postprocessing_score_converter"
        )

        activation_conf_node = add_activation_function_after_node(
            graph, match.single_input_node(1)[0].in_node(0), activation_function
        )
        ...
        reshape_conf_node = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([-1, (num_classes + 1) * max_proposals]),
            dict(name="do_reshape_conf"),
            activation_conf_node,
        )
        ...
        current_node = skip_nodes_by_condition(
            match.single_input_node(0)[0].in_node(0),
        ...
        share_box_across_classes = _value_or_raise(
        ...
        reshape_offsets = create_op_node_with_second_input(
            graph,
            Reshape,
            reshape_offsets_shape,
        ...
        reshape_offsets_2d = create_op_node_with_second_input(
            graph, Reshape, int64_array([-1, 4]), dict(name="reshape_locs_2d"), offsets
        )
        ...
        variances = Const(
            graph, dict(value=_variance_from_pipeline_config(pipeline_config))
        ).create_node([])
        scaled_offsets = Mul(graph, dict()).create_node(
            [reshape_offsets_2d, variances], dict(name="scale_locs")
        )
        ...
        if "coordinates_swap_method" in custom_attributes:
            log.error(
                'The "coordinates_swap_method" parameter is not needed anymore. Consider removing it from the '
                '"ObjectDetectionAPIDetectionOutputReplacement" transformation custom attributes.',
                extra={"is_warning": True},
            )
        matmul_or_conv_nodes = backward_bfs_for_operation(
            scaled_offsets, ["MatMul", "Conv2D"], ["ShapeOf", "CropAndResize"]
        )
        if len(matmul_or_conv_nodes) == 0:
            swapped_offsets = add_convolution_to_swap_xy_coordinates(
                graph, scaled_offsets, 4
            )
            flattened_offsets = Reshape(
                graph, dict(name="do_reshape_locs")
            ).create_node([swapped_offsets])
        else:
            swap_weights_xy(graph, matmul_or_conv_nodes)
            flattened_offsets = Reshape(
                graph, dict(name="do_reshape_locs")
            ).create_node([scaled_offsets])
        ...
        Const(
            graph, {"value": reshape_shape, "name": flattened_offsets.name + "/Dim"}
        ).create_node().out_port(0).connect(flattened_offsets.in_port(1))
        ...
        reshape_priors = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([-1, 1, max_proposals * 4]),
        ...
        for key in ("clip_before_nms", "clip_after_nms"):
            if key in match.custom_replacement_desc.custom_attributes:
                detection_output_op.attrs[key] = int(
                    match.custom_replacement_desc.custom_attributes[key]
                )

        detection_output = detection_output_op.create_node(
            [flattened_offsets, reshape_conf_node, reshape_priors],
            dict(
                ...
                background_label_id=int(
                    custom_attributes.get("background_label_id", 0)
                ),
                code_type="caffe.PriorBoxParameter.CENTER_SIZE",
                pad_mode="caffe.ResizeParameter.CONSTANT",
                resize_mode="caffe.ResizeParameter.WARP",
                confidence_threshold=_value_or_raise(
                    match, pipeline_config, "postprocessing_score_threshold"
                ),
                top_k=_value_or_raise(
                    match, pipeline_config, "postprocessing_max_detections_per_class"
                ),
                keep_top_k=_value_or_raise(
                    match, pipeline_config, "postprocessing_max_total_detections"
                ),
                nms_threshold=_value_or_raise(
                    match, pipeline_config, "postprocessing_iou_threshold"
                ),
            ),
        )
        ...
        if pipeline_config.get_param("use_matmul_crop_and_resize"):
            insert_weights_swap_xy_sub_graph(
                graph, detection_output.in_port(2).get_connection()
            )
        ...
        log.error(
            'The graph output nodes have been replaced with a single layer of type "DetectionOutput". Refer to '
...
class ObjectDetectionAPIMaskRCNNROIPoolingSecondReplacement(
    FrontReplacementFromConfigFileSubGraph
):
    """
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        if len(detection_output_nodes_ids) != 1:
            raise Error(
                "Found the following nodes '{}' with name 'detection_output' but there should be exactly 1.".format(
                    detection_output_nodes_ids
                )
            )
        ...
        do_outputs = [
            port.node
            for port in detection_output.out_port(0).get_destinations()
            if port.node.op == "Result"
        ]
        if len(do_outputs) == 1:
        ...
        flatten_do = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([-1, 7]),
        ...
        do_result = Result(graph, dict(name="do_reshaped_OutputOp")).create_node(
            [flatten_do]
        )
        ...
        batch_ids = Crop(
            graph,
            dict(axis=int64_array([1]), offset=int64_array([0]), dim=int64_array([1])),
        ).create_node([flatten_do], dict(name="crop_do_batch_ids"))
        coords = Crop(
            graph,
            dict(axis=int64_array([1]), offset=int64_array([3]), dim=int64_array([4])),
        ).create_node([flatten_do], dict(name="crop_do_coords"))
        batch_and_coords = Concat(graph, dict(axis=1)).create_node(
            [batch_ids, coords], dict(name="batch_and_coords")
        )
        ...
        roi_pooling = ROIPooling(
            ...
            dict(
                method="bilinear",
                spatial_scale=1,
                pooled_h=roi_pool_size,
                pooled_w=roi_pool_size,
            ),
        ).create_node(
            [match.single_input_node(0)[0].in_node(), batch_and_coords],
            dict(name="ROI_pooling_2"),
        )
        return {"roi_pooling_node": roi_pooling}


class ObjectDetectionAPIMaskRCNNSigmoidReplacement(
    FrontReplacementFromConfigFileGeneral
):
    """
    ...
    def transform_graph(self, graph: Graph, replacement_descriptions):
        ...
        # name prefix
        masks_node_prefix_name = replacement_descriptions.get(
        ...
        for op_output in op_outputs:
            ...
            if last_node.name.startswith(masks_node_prefix_name):
                ...
                op_output.in_port(0).get_connection().get_source().add_tensor_names(
                    [sigmoid_node["name"]]
                )

        log.error(
            'The predicted masks are produced by the "masks" layer for each bounding box generated with a '
...
class ObjectDetectionAPIProposalReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        if (
            match.custom_replacement_desc.custom_attributes.get(
                "operation_to_add", "Proposal"
            )
            == "DetectionOutput"
        ):
            self.matched_input_nodes_to_keep = (
                3  # keep the third input with prior boxes (anchors)
            )
            return self.insert_detection_output_instead_of_proposal(
                graph, match, pipeline_config
            )

        max_proposals = _value_or_raise(
            match, pipeline_config, "first_stage_max_proposals"
        )
        proposal_ratios = _value_or_raise(
            match, pipeline_config, "anchor_generator_aspect_ratios"
        )
        proposal_scales = _value_or_raise(
            match, pipeline_config, "anchor_generator_scales"
        )
        anchors_count = len(proposal_ratios) * len(proposal_scales)

        # Find Convolution/MatMul node that produces classes confidence
        class_conf = backward_bfs_for_operation(match.single_input_node(1)[0], ["Add"])[
            0
        ]
        ...
        reshape_class_conf = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([0, anchors_count, 2, -1]),
        ...
        softmax_conf_op = Softmax(
            graph,
            dict(axis=2, nchw_layout=True, name=reshape_class_conf.id + "/Softmax"),
        )
        ...
        order_const = Const(
            ...
            dict(
                ...
                name=softmax_conf.name + "/TransposeOrder",
            ),
        ).create_node()
        permute_reshape_softmax_op = Transpose(graph, dict())
        permute_reshape_softmax = permute_reshape_softmax_op.create_node(
            [softmax_conf, order_const], dict(name=softmax_conf.name + "/Transpose")
        )
        mark_input_as_in_correct_layout(permute_reshape_softmax, 1)
        mark_output_as_in_correct_layout(permute_reshape_softmax, 0)

        initial_shape = Shape(graph, dict(name=class_conf.id + "/Shape")).create_node(
            [class_conf]
        )

        reshape_conf_initial = Reshape(
            graph, dict(name="Reshape_Transpose_Class")
        ).create_node([permute_reshape_softmax, initial_shape])
        ...
        anchor_generator_height_stride = pipeline_config.get_param(
            "anchor_generator_height_stride"
        )
        anchor_generator_width_stride = pipeline_config.get_param(
            "anchor_generator_width_stride"
        )
        ...
        if variance_height != variance_width:
            log.error(
                'The values for variance for height "{}" is not equal to variance for width "{}". The detection '
                "results will be inaccurate.".format(variance_height, variance_width)
            )
        if variance_x != variance_y:
            log.error(
                'The values for variance for x "{}" is not equal to variance for y "{}". The detection '
                "results will be inaccurate.".format(variance_x, variance_y)
            )
        if anchor_generator_height_stride != anchor_generator_width_stride:
            log.error(
                'The values for the anchor generator height stride "{}" is not equal to the anchor generator '
        ...
        if anchor_generator_height != anchor_generator_width:
            log.error(
                'The values for the anchor generator height "{}" is not equal to the anchor generator width '
                'stride "{}". The detection results will be inaccurate.'.format(
                    anchor_generator_height, anchor_generator_width
                )
            )

        proposal_op = ProposalOp(
            graph,
            dict(
                min_size=1,
                ...
                nms_thresh=_value_or_raise(
                    match, pipeline_config, "first_stage_nms_iou_threshold"
                ),
            ),
        )
        for key in ("clip_before_nms", "clip_after_nms"):
            if key in match.custom_replacement_desc.custom_attributes:
                proposal_op.attrs[key] = int(
                    match.custom_replacement_desc.custom_attributes[key]
                )

        bboxes_offsets = backward_bfs_for_operation(
            match.single_input_node(0)[0], ["Add"]
        )[0]
        ...
        # size only as input
        im_info = Parameter(
            graph, dict(shape=int64_array([1, 3]), fixed_batch=True)
        ).create_node([], dict(name="image_info"))

        proposal = proposal_op.create_node(
            [reshape_conf_initial, bboxes_offsets, im_info], dict(name="proposals")
        )
        return {
            "proposal_node": ObjectDetectionAPIProposalReplacement.ie_to_tf_proposals(
                graph, proposal, match, pipeline_config, max_proposals
            )
        }

    @staticmethod
    def insert_detection_output_instead_of_proposal(
        graph: Graph, match: SubgraphMatch, pipeline_config: PipelineConfig
    ):
        """
        ...
        class_conf_nodes = backward_bfs_for_operation(
            match.single_input_node(1)[0], ["Add"]
        )
        assert len(class_conf_nodes) >= 1, (
        ...
        reshape_conf = create_op_node_with_second_input(
            graph, Reshape, int64_array([0, -1, 2]), dict(name="predictions/Reshape")
        )
        ...
        softmax_conf = Softmax(
            graph, dict(axis=2, name=reshape_conf.id + "/Softmax")
        ).create_node([reshape_conf])
        flattened_conf = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([0, -1]),
            dict(name=softmax_conf.name + "/Flatten"),
            softmax_conf,
        )
        # prepare input with bounding boxes shape offsets
        offsets = backward_bfs_for_operation(match.single_input_node(0)[0], ["Add"])[0]
        flatten_offsets = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([0, -1]),
        ...
        input_shape = Shape(graph, {"name": parameter_node.name}).create_node(
            [parameter_node]
        )
        input_image_hw = node_to_get_shape_value_of_indices(
            input_shape, [1, 2]
        )  # NHWC layout
        hwhw = create_op_with_const_inputs(
            graph, Tile, {1: int64_array([2])}, {"name": "image_hwhw"}, input_image_hw
        )

        hwhw_float = Cast(graph, {"dst_type": np.float32}).create_node([hwhw])
        scaled_anchors = Div(graph, {"name": "scaled_anchors"}).create_node(
            [yxyx_anchors, hwhw_float]
        )

        flattened_anchors = create_op_with_const_inputs(
            graph,
            Reshape,
            {1: int64_array([1, 1, -1])},
            {"name": "flattened_anchors"},
            scaled_anchors,
        )
        cropped_anchors = AttributedClamp(
            graph, {"min": 0.0, "max": 1.0, "name": "clamped_yxyx", "nchw_layout": True}
        ).create_node([flattened_anchors])
        ...
        variances = Const(
            graph, {"value": _variance_from_pipeline_config(pipeline_config)}
        ).create_node()

        anchors_shape = Shape(graph, {"name": "anchors_shape"}).create_node(
            [yxyx_anchors]
        )
        anchors_count = node_to_get_shape_value_of_indices(anchors_shape, [0])
        tiled_variances = Tile(graph, {"name": "tiled_variances"}).create_node(
            [variances, anchors_count]
        )
        reshaped_tiled_variances = create_op_with_const_inputs(
            graph,
            Reshape,
            {1: int64_array([1, 1, -1])},
            {"name": "flattened_variances"},
            tiled_variances,
        )
        ...
        duplicate_anchors = Concat(
            graph, {"axis": 1, "name": "anchors_with_variances"}
        ).create_node([cropped_anchors, reshaped_tiled_variances])
        ...
        do_3d = create_op_with_const_inputs(
            graph, Squeeze, {1: int64_array(0)}, {"name": do.name + "/SqueezeDO"}, do
        )
        ...
        do_split = create_op_node_with_second_input(
            graph,
            Split,
            int64_array(2),
            {"num_splits": 7, "name": do.name + "/Split"},
            do_3d,
        )

        coords = Concat(
            graph, {"axis": -1, "in_ports_count": 4, "name": do_split.name + "/coords"}
        ).create_node()
        ...
        clamped_coords = AttributedClamp(
            graph, {"min": 0.0, "max": 1.0, "name": "clamped_xyxy"}
        ).create_node([coords])

        # prepare final proposal boxes [batch_id, x1, y1, x2, y2]
        proposal_node = Concat(
            graph, {"axis": -1, "in_ports_count": 2, "name": "proposals"}
        ).create_node()
        do_split.out_port(0).connect(proposal_node.in_port(0))
        clamped_coords.out_port(0).connect(proposal_node.in_port(1))
        return {
            "proposal_node": ObjectDetectionAPIProposalReplacement.ie_to_tf_proposals(
                graph, proposal_node, match, pipeline_config, max_proposals
            )
        }

    @staticmethod
    def ie_to_tf_proposals(
        graph: Graph,
        proposal: Node,
        match: SubgraphMatch,
        pipeline_config: PipelineConfig,
        ...
        swap_proposals = not match.custom_replacement_desc.custom_attributes.get(
            "do_not_swap_proposals", False
        ) and not pipeline_config.get_param("use_matmul_crop_and_resize")
        if swap_proposals:
        ...
        proposal_reshape_2d = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([-1, 5]),
        ...
        crop_and_resize_nodes_ids = [
            node.id
            for node in graph.pseudo_topological_sort_with_start_node(start_node)
            if graph.nodes[node.id]["op"] == "CropAndResize"
        ]
        ...
        tf_proposal_reshape_4d = create_op_node_with_second_input(
            graph,
            Reshape,
        ...
        tf_proposals_crop_reshape_3d_node = create_op_node_with_second_input(
            graph, Reshape, int64_array([0, -1, 4]), dict(name="reshape_crop_3d"), crop
        )
...
class ObjectDetectionAPISSDPostprocessorReplacement(
    FrontReplacementFromConfigFileSubGraph
):
    """
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        has_background_class = _value_or_raise(
            match, pipeline_config, "add_background_class"
        )
        num_classes = (
        ...
        reshape_conf_before_ac = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([0, 1, -1, num_classes]),
            {"name": "do_ExpandDims_conf"},
        )
        reshape_conf_before_ac.in_port(0).connect(
            match.input_nodes(1)[0][0].in_node(0).out_port(0)
        )
        ...
        if not match.custom_replacement_desc.custom_attributes.get(
            "disable_prior_boxes_layers_generator", False
        ) and (
            pipeline_config.get_param("ssd_anchor_generator_num_layers") is not None
            or pipeline_config.get_param("multiscale_anchor_generator_min_level")
            is not None
        ):
            ...
            if pipeline_config.get_param("ssd_anchor_generator_num_layers") is not None:
                ...
                priors_node = _create_multiscale_prior_boxes_node(
                    graph, pipeline_config
                )
        else:
            log.info(
                "The anchor generator is not known. Save constant with prior-boxes to IR."
            )
            ...
            flattened_priors = create_op_with_const_inputs(
                graph,
                Reshape,
                {1: int64_array([1, 1, -1])},
                {"name": "flattened_priors"},
                tf_priors_node,
            )
            ...
            priors_shape = Shape(graph, {"name": "priors_shape"}).create_node(
                [tf_priors_node]
            )
            ...
            tiled_variances = Tile(graph, {"name": "tiled_variances"}).create_node(
                [variances, priors_count]
            )
            flattened_tiled_variances = create_op_with_const_inputs(
                graph,
                Reshape,
                {1: int64_array([1, 1, -1])},
            ...
            priors_node = Concat(
                graph, {"axis": 1, "name": "priors_with_variances"}
            ).create_node([flattened_priors, flattened_tiled_variances])
        ...
        conv_nodes = backward_bfs_for_operation(
            detection_output_node.in_node(0), ["Conv2D"], ["ShapeOf"]
        )
        ...
        if need_swap_priors:
            insert_weights_swap_xy_sub_graph(
                graph, detection_output_node.in_port(2).get_connection()
            )
        ...
        clear_tensor_names_info(
            [match.output_node(out)[0] for out in range(match.outputs_count())]
        )
...
class ObjectDetectionAPIOutputReplacement(FrontReplacementFromConfigFileGeneral):
    ...
    def transform_graph(self, graph: Graph, replacement_descriptions: dict):
        if graph.graph["cmd_params"].output is not None:
            log.warning(
                "User defined output nodes are specified. Skip the graph cut-off by the "
        ...
        for alternatives in outputs_string.split(","):
            for out_node_name in alternatives.split("|"):
                if graph.has_node(out_node_name):
                    ...
                    log.debug(
                        'A node "{}" does not exist in the graph. Do not add it as output'.format(
                            out_node_name
                        )
                    )
...
class ObjectDetectionAPIPSROIPoolingReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        if crop_height != crop_width:
            raise Error(
                'Different "crop_height" and "crop_width" parameters from the pipeline config are not '
        ...
        if len(proposal_nodes) != 1:
            raise Error(
                "Found the following nodes '{}' with name 'reshape_swap_proposals_2d' but there should be "
        ...
        psroipooling_node = PSROIPoolingOp(
            graph,
            {
                "name": input_node.soft_get("name") + "/PSROIPooling",
        ...
        reduce_node = create_op_node_with_second_input(
            graph,
            ReduceMean,
            int64_array([1, 2]),
            {"name": "mean", "keep_dims": True},
            psroipooling_node,
        )

        output_node = match.output_node(0)[0].out_node()
        if (
            len(output_node.in_ports()) == 2
            and not output_node.in_port(1).disconnected()
        ):
            output_node.in_port(
                1
            ).disconnect()  # disconnect the second input to make "erase_node" function work
