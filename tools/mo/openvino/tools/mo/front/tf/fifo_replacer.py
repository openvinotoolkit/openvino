...
class FIFOQueue(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(graph: Graph, match: dict, **kwargs):
        ...
        if (
            true_placeholder_shape is not None
            and true_placeholder_shape.ndim == 1
            and len(true_placeholder_shape) > 1
        ):
            log.warning(
                ...
                "".format(
                    match["placeholder"].id, true_placeholder_shape, placeholder_shape
                )
            )
        ...
        for node in graph.get_op_nodes():
            ...
            for port_idx, port in node.out_ports().items():
                if port.disconnected():
                    continue
                if (
                    not np_data_type_to_precision(node.types[port_idx])
                    in SUPPORTED_DATA_TYPES
                ):
                    raise Error(
                        "Data type {} is not supported for the"
