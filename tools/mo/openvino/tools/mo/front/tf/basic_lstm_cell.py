...
class BasicLSTMCell(FrontReplacementSubgraph):
        ...
        return dict(
            ...
            edges=[
                ...
                (
                    ...
                    "shift",
                    {"out": 2},
                ),  # f (this is unbiased f, there is an extra addition here)
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        match["input_cell_state"] = (
            match["mul_0"].in_node(0)
            if match["mul_0"].in_node(0).id != match["sigmoid_0"].id
            else match["mul_0"].in_node(1)
        )

        pattern_edges = self.pattern()["edges"]
        pattern_edges.extend(
            [
                ("input_op", "concat"),
        ...
        for i in [0, 1]:
            if i not in lstm_node.out_nodes():
                ...
                fake_output_node.create_node(
                    inputs=[lstm_node], edge_attrs={"out": i, "in": 0}
                )

        lstm_node["tf"] = True
        lstm_node["extra_inputs"] = {
            name: match[name].id for name in __class__.extra_inputs
        }
    ...
    def infer(node: Node):
        ...
        shift_const = node.in_node(
            __class__.extra_inputs.index("shift_const") + len(__class__.inputs)
        )
