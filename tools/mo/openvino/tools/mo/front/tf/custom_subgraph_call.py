...
def find_input_port(
    node: Node, input_desc: list, search_node_name: str, search_node_port: int
):
    ...
    for in_port, tensor_desc in enumerate(input_desc):
        for node_pattern, node_port in tensor_desc:
            if (
                findall(node_pattern, search_node_name)
                and node_port == search_node_port
            ):
                return in_port
    raise Exception(
...
def find_output_port(
    node: Node, output_desc: list, search_node_name: str, search_node_port: int
):
    ...
    for out_port, (node_pattern, node_port) in enumerate(output_desc):
        if findall(node_pattern, search_node_name) and node_port == search_node_port:
            return out_port
    raise Exception(
        'Did not find output port of the node "{}" with port "{}"'.format(
            search_node_name, search_node_port
        )
    )
...
def merge_nodes(
    ...
    log.info(
        "Create new node with name '{}' for nodes '{}'".format(
    ...
    for node_name in nodes_to_merge_names:
                ...
                if input_tensor_name not in added_input_tensors_names:
                    ...
                    graph.add_edge(
                        ...
                        **merge_edge_props(
                            {
                                "in": find_input_port(
                                    new_node, inputs_desc, node_name, edge_attrs["in"]
                                ),
                                "out": edge_attrs["out"],
                    ...
                    log.debug(
                        "Creating edge from outside of sub-graph to inside sub-graph: {} -> {}".format(
        ...
        for out_node_name, edge_attrs in Node(graph, node_name).get_outputs():
            if out_node_name not in nodes_to_merge_names:
                log.debug(
                    "Creating edge from inside of sub-graph to outside sub-graph: {} -> {}".format(
...
def skip_nodes_by_condition(
    current_node: Node, condition: callable, forward: bool = False
):
    if forward:
