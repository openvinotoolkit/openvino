...
from openvino.tools.mo.front.subgraph_matcher import SubgraphMatch, SubgraphMatcher
...
class FrontReplacementFromConfigFileGeneral(FrontReplacementPattern):
    ...
    def transform_graph(self, graph, replacement_descriptions):
        raise Exception(
            'Function "transform_graph" must be overridden in the sub-class'
        )

    def find_and_replace_pattern(self, graph: Graph):
        replacement_descriptions = (
            CustomReplacementRegistry().get_custom_replacement_description(
                self.replacement_id
            )
        )
        if replacement_descriptions is None or len(replacement_descriptions) < 1:
            log.info(
                "Failed to find custom replacement description with id '{}'".format(
                    self.replacement_id
                )
            )
            return
        for desc in replacement_descriptions:
            if "custom_attributes" in desc._replacement_desc:
                ...
                log.info(
                    "Failed to find 'custom_attributes' in replacement description with id '{}'".format(
                        self.replacement_id
                    )
                )
        ...
        for replacement_description in replacement_descriptions:
            ...
            for match in matched_instances:
                if not is_connected_component(graph, match.matched_nodes_names()):
                    log.warning(
                        "The following nodes don't form connected sub-graph: {}".format(
...
class FrontReplacementFromConfigFileOp(FrontReplacementFromConfigFileSubGraph):
    ...
    def input_edges_match(
        ...
        for sub_graph_input_port in range(inputs_count):
            ...
            input_edges_match[(input_node.id, input_port)] = (
                new_sub_graph["new_node"].id,
                sub_graph_input_port,
            )
    ...
    def output_edges_match(
        ...
        for sub_graph_output_port in range(outputs_count):
            ...
            output_edges_match[(output_node.id, output_port)] = (
                new_sub_graph["new_node"].id,
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        if "infer" not in op.attrs:
            ...
            self.input_edges_match = (
                lambda gr, ma, new_sub_graph: dict()
            )  # pylint: disable=method-hidden
            self.output_edges_match = (
                lambda gr, ma, new_sub_graph: dict()
            )  # pylint: disable=method-hidden
        else:
