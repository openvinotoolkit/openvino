...
class MVNUnrolled(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(graph: Graph, match: dict):
        ...
        new_subgraph = mvn.create_node(
            [match["mean"].in_node(0), mean_reduction, variance_reduction, pow2, eps]
        )
    ...
    def infer(node: Node):
        ...
        if power_value is None or eps_value is None:
            log.warning(
                "Power or/and epsilon values for MVN node {} are not constants".format(
                    node.name
                )
            )
            return
        ...
        if power_value != 0.5:
            log.warning(
                "Power for MVN node {} ({}) is not equal to 0.5".format(
                    node.name, power_value
                )
            )
            return
