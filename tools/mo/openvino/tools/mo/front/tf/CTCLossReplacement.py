...
class CTCLossReplacement(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        ctc_greedy_decoder = CTCGreedyDecoderSeqLenOp(
            graph,
            {"name": output_sparse_to_dense_name, "merge_repeated": merge_repeated_tf},
        ).create_node()
        rename_nodes(
            [
                (sparse_to_dense_tf, output_sparse_to_dense_name + "/AbandonedName"),
        ...
        assert ctc_loss_tf.has_valid(
            "preprocess_collapse_repeated"
        ), 'The CTCLoss node "{}" misses "preprocess_collapse_repeated" attribute'.format(
            output_ctc_loss_name
        )
        assert ctc_loss_tf.has_valid(
            "ctc_merge_repeated"
        ), 'The CTCLoss node "{}" misses "ctc_merge_repeated" attribute'.format(
            output_ctc_loss_name
        )
        assert ctc_loss_tf.has_valid(
            "unique"
        ), 'The CTCLoss node "{}" misses "unique" attribute'.format(
            output_ctc_loss_name
        )
        ...
        rename_nodes(
            [
                (ctc_loss_tf, output_ctc_loss_name + "/AbandonedName"),
