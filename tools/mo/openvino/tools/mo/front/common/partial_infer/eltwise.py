...
def eltwise_infer(node: Node, op=None, **kwargs):
    def broadcast_dims(dim1, dim2):
        if dim1 is not dynamic_dimension and dim2 is not dynamic_dimension:
            ...
            if mind == 1:
                ...
                raise Error(
                    "Input shapes mismatch for node {}: {}".format(node_name, shapes)
                )
            return mind
    ...
    raw_inputs = [
        (inp, attr)
        for inp, attr in node.get_sorted_inputs()
...
def eltwise_reverse_infer(node: Node):
    ...
    if node["auto_broadcast"] is "none":
        ...
        if output_shape is not None:
            ...
            if (
                input_1_shape is not None
                and input_2_shape is None
                and out_rank > len(input_1_shape)
            ):
                in_port_to_update = 1
                defined_in_shape = input_1_shape
            elif (
                input_2_shape is not None
                and input_1_shape is None
                and out_rank > len(input_2_shape)
            ):
                in_port_to_update = 0
            ...
            for i in range(-1, -defined_in_rank - 1, -1):
                assert (
                    defined_in_shape[i] == 1
                    or np.ma.is_masked(defined_in_shape[i])
                    or compatible_dims(defined_in_shape[i], output_shape[i])
                ), "Shapes of Elementwise node '{}' are not compatible for reverse_infer.".format(
                    node_name
                )
...
def bias_add_infer(node, op):
    if (
        node.in_port(0).data.get_value() is not None
        and node.in_port(1).data.get_value() is not None
        and op is not None
    ):
        node.out_port(0).data.set_value(
            op(node.in_port(0).data.get_value(), node.in_port(1).data.get_value())
        )
    else:
