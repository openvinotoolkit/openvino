...
class CustomReplacementRegistry(object):
    ...
    def __init__(self):
        if not CustomReplacementRegistry.instance:
            CustomReplacementRegistry.instance = (
                CustomReplacementRegistry.__CustomReplacementRegistry()
            )
        else:
            pass
    ...
    def add_custom_replacement_description_from_config(self, file_name: str):
        if not os.path.exists(file_name):
            raise Error(
                "Custom replacement configuration file '{}' doesn't exist. ".format(
                    file_name
                )
                + refer_to_faq_msg(46)
            )
    ...
    def get_custom_replacement_description(self, replacement_id: str):
        if replacement_id in self.registry:
            ...
            log.warning(
                "Configuration file for custom replacement with id '{}' doesn't exist".format(
                    replacement_id
                )
            )
            return None
