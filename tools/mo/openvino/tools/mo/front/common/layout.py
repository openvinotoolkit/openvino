...
# for this purpose.
indices_mapping = {
    4: {"NCHW": "NCHW", "NHWC": "NHWC"},
    5: {"NCHW": "NCDHW", "NHWC": "NDHWC"},
}
...
def shape_for_layout(layout: str, **kwargs):
    ...
    output_shape = np.ma.ones(
        shape=[shape_len], dtype=np.int64, fill_value=dynamic_dimension_value
    )
...
def get_dim_from_layout(node: Node, dim: str):
    ...
    graph = node.graph
    if (
        "layout_values" in graph.graph["cmd_params"]
        and graph.graph["cmd_params"].layout_values
    ):
