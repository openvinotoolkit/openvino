...
def check_for_duplicates(extractors_collection: dict):
    ...
    if len(duplicates) > 0:
        raise Error(
            "Extractors collection have case insensitive duplicates {}. "
            + refer_to_faq_msg(47),
            duplicates,
        )
...
def add_or_override_extractor(
    extractors: dict, keys: dict, name, extractor, extractor_desc
):
    ...
    if name_lower in keys:
        ...
        log.debug(
            "Overridden extractor entry {} by {}.".format(old_name, extractor_desc)
        )
        if old_name != name:
            log.debug("Extractor entry {} was changed to {}.".format(old_name, name))
    else:
        log.debug(
            "Added a new entry {} to extractors with {}.".format(name, extractor_desc)
        )
...
def update_extractors_with_extensions(
    extractors_collection: dict = None,
    disable_omitting_optional: bool = False,
    enable_flattening_optional_params: bool = False,
):
    """
    ...
    for op, op_cls in Op.registered_ops.items():
        ...
        if op_lower not in keys:
            extractors_collection[op] = (
                lambda c: lambda node: extension_op_extractor(node, c)
            )(op_cls)
            log.debug(
                "Added a new entry {} to extractors with custom op class {}.".format(
                    op, op_cls
                )
            )
            keys[op_lower] = op
