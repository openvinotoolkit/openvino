        ...
        if node.kind == "op":
            if node.has_valid("type") or (node.has_valid("op") and node.op == "Result"):
                for out_data_node in node.out_nodes().values():
                    if (
                        out_data_node.has_valid("data_type")
                        and out_data_node.data_type != np.float32
                    ):
                        log.info(
                            'Node "{}" produces output as non FP32. Consider it unsupported'.format(
                                node_name
                            )
                        )
                ...
                log.info(
                    'Node "{}" does not have type. Consider it unsupported'.format(
                        node_name
                    )
                )
                unsupported.append(node.id)
