...
def save_params_file(
    model_name: str, args: dict, auxs: dict, iteration_number: int = 0
):
    pretrained = {}
    for key in args:
...
def add_pretrained_model(
    ...
    if pretrained_params:
        symbol, arg_params, aux_params = mx.model.load_checkpoint(
            pretrained_model, iteration_number
        )
...
def build_params_file(
    nd_prefix_name: str = "", pretrained_model: str = "", input_names: str = ""
):
    ...
    nd_args = (
        mx.nd.load(os.path.join(files_dir, "%s_args.nd" % nd_prefix_name))
        if nd_prefix_name
        else None
    )
    nd_auxs = (
        mx.nd.load(os.path.join(files_dir, "%s_auxs.nd" % nd_prefix_name))
        if nd_prefix_name
        else None
    )
    nd_args = add_pretrained_model(
        pretrained_params, nd_args, pretrained_model_name, iteration_number, input_names
    )
