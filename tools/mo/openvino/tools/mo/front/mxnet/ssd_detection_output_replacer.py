...
class SsdPatternDetectionOutputReplacer(FrontReplacementSubgraph):
    ...
    force_clean_up = True
    graph_condition = [
        lambda graph: graph.graph["fw"] == "mxnet"
        and graph.graph["cmd_params"].enable_ssd_gluoncv
    ]
    ...
    def pattern(self):
        return dict(nodes=[("box_nms", dict(op="_contrib_box_nms"))], edges=[])
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        for i, concat_match in enumerate(self.concats_pattern):
            for matches in find_pattern_matches(
                graph, concat_match["nodes"], concat_match["edges"], None, None
            ):
        ...
        detection_output_node = DetectionOutput(
            graph,
            dict(
                name=graph.unique_id() + "/DetectionOutput_",
                top_k=top_k,
                keep_top_k=top_k,
        ...
        ssd_concats["ssd_concat1"].out_port(0).get_connection().set_destination(
            detection_output_node.in_port(0)
        )
        ssd_concats["ssd_concat2"].out_port(0).get_connection().set_destination(
            detection_output_node.in_port(2)
        )

        Result(graph, {"name": detection_output_node.id + "/Result"}).create_node(
