...
from openvino.tools.mo.ops.elementwise import Add, Div, Sub
...
def calculate_prior_box_value(value: Node, value_to_div: Port, value_to_add: Port):
    ...
    div = create_op_node_with_second_input(
        graph, Div, mo_array([2], dtype=dtype), op_attrs=dict(name=value.name + "/Div")
    )
...
class SsdAnchorsReplacer(FrontReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: Dict[str, Node]):
        ...
        value = create_op_node_with_second_input(
            ...
            Split,
            int64_array(1),
            op_attrs=dict(name=split_node_reshape.name + "/Split", num_splits=4),
            input_node=split_node_reshape,
        )

        xmin, xmax = calculate_prior_box_value(
            value, value_to_div=value.out_port(2), value_to_add=value.out_port(0)
        )
        ymin, ymax = calculate_prior_box_value(
            value, value_to_div=value.out_port(3), value_to_add=value.out_port(1)
        )

        concat_slice_value = Concat(
            graph, dict(name=value.name + "/Concat", in_ports_count=4, axis=1)
        ).create_node()
        ...
        concat = Concat(
            graph,
            dict(name=reshape_concat_values.name + "/Concat", in_ports_count=2, axis=1),
        ).create_node()
