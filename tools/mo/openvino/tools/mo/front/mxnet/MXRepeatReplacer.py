...
from openvino.tools.mo.ops.elementwise import Add, Mul, Sub
...
class MXRepeatReplacer(FrontReplacementPattern):
    ...
    def mxrepeat_decomposition(node: Node):
        ...
        one = Const(
            graph, {"name": name + "/Broadcast/One", "value": int64_array([1])}
        ).create_node()
        first_ones = Broadcast(
            graph, {"name": name + "/Broadcast/Ones_first_part"}
        ).create_node()
        first_ones.in_port(0).connect(one.out_port(0))
        first_ones.in_port(1).connect(unsqueeze_axis.out_port(0))

        repeats = Const(
            graph, {"name": name + "/repeats", "value": int64_array([node.repeats])}
        ).create_node()

        second_ones = Broadcast(
            graph, {"name": name + "/Broadcast/Ones_second_part"}
        ).create_node()
        second_part_broadcast_shape = Sub(
            graph, {"name": name + "/Broadcast/Shape/second_part"}
        ).create_node()
