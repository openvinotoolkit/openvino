...
class MXReshapeReverse(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        forward_reverse_node = Reverse(
            graph, dict(name=mxreshape.id + "/ForwardReverse", axis=1)
        ).create_node()
        ...
        reshape_node = Reshape(
            graph, dict(name=mxreshape.id + "/Reshape")
        ).create_node()
        ...
        reshape_shape_node = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array(np.flip(mxreshape.dim, 0)),
        ...
        if np.sum(np.in1d([-2, -3, -4], mxreshape.dim), axis=0):
            reshape_shape_node = MXReshape(
                graph,
                dict(
                    name=mxreshape.id + "/Reshape",
                    dim=int64_array(np.flip(mxreshape.dim, 0)),
                ),
            ).create_node()

        reshape_shape_node.in_port(0).connect(reshape_node.out_port(0))

        backward_shape_node = Shape(
            graph, dict(name=mxreshape.id + "/BackwardShape")
        ).create_node()
        ...
        backward_reverse_node = Reverse(
            graph, dict(name=mxreshape.id + "/BackwardReverse", axis=1)
        ).create_node()
        ...
        backward_reshape_node = Reshape(
            graph, dict(name=mxreshape.id + "/BackwardReshape")
        ).create_node()
