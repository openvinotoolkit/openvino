...
class RNNFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        if mode == "rnn_tanh":
            ...
            node_attrs["activations"] = (
                ["tanh"] if not bidirectional else ["tanh", "tanh"]
            )
            RNN.update_node_stat(node, node_attrs)
        elif mode == "rnn_relu":
            node_attrs["gate_order"] = [0]
            node_attrs["activations"] = (
                ["relu"] if not bidirectional else ["relu", "relu"]
            )
            RNN.update_node_stat(node, node_attrs)
        elif mode == "gru":
            ...
            raise Error(
                "Operation RNN with mode '{}' not supported." + refer_to_faq_msg(86),
                mode,
            )
        return cls.enabled
