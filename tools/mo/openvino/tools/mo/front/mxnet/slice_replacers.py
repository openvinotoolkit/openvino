    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        strided_slice_node = StridedSlice(
            ...
            dict(
                ...
                end_mask=np.ones(len(node.crop_end), dtype=np.int64),
            ),
        ).create_node()
        ...
        crop_begin_node = Const(
            graph,
            dict(
                value=node.crop_begin,
                symbol_dict={"name": node.id + "/crop_begin_const"},
            ),
        ).create_node()
        crop_end_node = Const(
            graph,
            dict(
                value=node.crop_end, symbol_dict={"name": node.id + "/crop_end_const"}
            ),
        ).create_node()
        ...
        if len(node.step) > 0:
            stride_node = Const(
                graph,
                dict(value=node.step, symbol_dict={"name": node.id + "/steps_const"}),
            ).create_node()
