...
from openvino.tools.mo.front.mxnet.eltwise_scalar_replacers import (
    MulScalarFrontReplacer,
)
from openvino.tools.mo.front.mxnet.ssd_detection_output_replacer import (
...
class SsdPatternAnchorReshape(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        variants = mo_array(
            [
                variants_dict["mul_scalar1x"],
                variants_dict["mul_scalar1y"],
                variants_dict["mul_scalar2x"],
                variants_dict["mul_scalar2y"],
            ]
            * int(const.value.size / 4)
        ).reshape(const.value.shape)
        priorbox_variants = Const(
            graph, dict(value=variants, name=const.id + "/priorbox_variants")
        ).create_node()
        variants_slice_like = SliceLike(
            graph,
            dict(axes=slice_like.axes, name=slice_like.id + "/variants_slice_like"),
        ).create_node()
