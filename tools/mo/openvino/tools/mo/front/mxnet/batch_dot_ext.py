    ...
    def extract(cls, node: Node):
        ...
        if forward_stype is not None:
            log.error(
                ...
                "Model Optimizer conversion assumes default value = None".format(
                    node.soft_get("name", node.id), forward_stype
                ),
                extra={"is_warning": True},
            )

        MatMul.update_node_stat(
            node, {"transpose_a": transpose_a, "transpose_b": transpose_b}
        )
        return cls.enabled
