...
class DivSqrtDim(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        pow_node = AttributedPower(
            graph, dict(name=div_sqrt_name + "/Sqrt", power=mo_array(0.5))
        ).create_node()

        # Due to specification, Power must have inputs with the same data type.
        convert_pow_input = Cast(
            graph,
            dict(dst_type=np.float32, name=shape_values_node.name + "/ConvertToFP32"),
        ).create_node()
        ...
        rename_nodes(
            [(div_sqrt, div_sqrt_name + "/ShouldBeDeleted"), (div_node, div_sqrt_name)]
        )
