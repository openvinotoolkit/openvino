...
def load_symbol_nodes(
    model_name, input_symbol: str = None, legacy_mxnet_model: bool = False
):
    if input_symbol:
    ...
    if legacy_mxnet_model and (input_symbol == json_name):
        ...
        if os.path.isfile(json_name):
            ...
            raise Error(
                "Specified input json {} does not exist. " + refer_to_faq_msg(84),
                json_name,
            )
    ...
    if not nd_prefix_name and not pretrained_model_name:
        ...
        if input_names:
            model_params = load_params(
                input_model_name, data_names=input_names.split(",")
            )
        else:
        ...
        model_params = build_params_file(
            nd_prefix_name, pretrained_model_name, input_names
        )
    else:
        raise Error(
...
def symbol2nx(graph, model_nodes, model_params, input_names: str = ""):
    ...
    for i, node in enumerate(model_nodes):
        if node["name"] in model_params._arg_params and node["name"] not in input_names:
            node["value"] = mo_array(
                model_params._arg_params[node["name"]].asnumpy(), dtype=np.float32
            )
        elif (
            node["name"] in model_params._aux_params and node["name"] not in input_names
        ):
            node["value"] = mo_array(
                model_params._aux_params[node["name"]].asnumpy(), dtype=np.float32
            )
    ...
    for i, attrs in enumerate(model_nodes):
        node = attrs
        edges, used_indices = get_mxnet_node_edges(
            node, i, list(model_nodes), index_node_keys
        )
        if len(edges) > 0:
