...
class ArangeReplacer(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        node = match["op"]
        if (
            not node.has_valid("start")
            or not node.has_valid("stop")
            or not node.has_valid("step")
        ):
            return

        start_value = Const(
            graph,
            dict(value=node.start, symbol_dict={"name": node.id + "/const_start"}),
        ).create_node()
        limit_value = Const(
            graph, dict(value=node.stop, symbol_dict={"name": node.id + "/const_limit"})
        ).create_node()
        delta_value = Const(
            graph, dict(value=node.step, symbol_dict={"name": node.id + "/const_delta"})
        ).create_node()
        ...
        if node.has_valid("repeat") and node.repeat > 1:
            rep = MXRepeat(
                graph, dict(name=node.id + "/mxrepeat", axis=0, repeats=node.repeat)
            ).create_node()
