...
from openvino.tools.mo.front.mxnet.extractors.l2_normalization import (
    l2_normalization_ext,
)
...
def mxnet_op_extractor(node: Node):
    ...
    if result_attr is None:
        raise Error(
            'Model Optimizer does not support layer "{}". Please, implement extension. '.format(
                node.name
            )
            + refer_to_faq_msg(45)
        )
