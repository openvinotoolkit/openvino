...
class MXReshapeToReshape(FrontReplacementSubgraph):
        ...
        return dict(
            nodes=[
                (
                    ...
                    dict(
                        op="MXReshape",
                        dim=lambda node_dim: node_dim is not None
                        and len(set(node_dim).intersection({-2, -3, -4})) != 0,
                        reverse=False,
                    ),
                )
            ],
    ...
    def resolve_minus2(self, shape_node, input_index, reshape_index, dims):
        rank_node = Shape(
            shape_node.graph, dict(name=shape_node.id + "/RankShapeMXReshapeMinus2")
        ).create_node()
        rank_node.in_port(0).connect(shape_node.out_port(0))
        shape_values_node = get_shape_values_by_range_idxs(
            shape=shape_node,
            rank=rank_node,
            begin=input_index,
            end=-1,
    ...
    def resolve_minus3(self, shape_node, input_index, reshape_index, dims):
        shape_indexes_node1 = Const(
            shape_node.graph,
            dict(
                name=shape_node.id + "/ShapeMinus3_index_const1_" + str(input_index),
                value=int64_array([input_index]),
            ),
        ).create_node()
        ...
        shape_indexes_node2 = Const(
            shape_node.graph,
            dict(
                name=shape_node.id + "/ShapeMinus3_index_const2_" + str(input_index),
                value=int64_array([input_index + 1]),
            ),
        ).create_node()
        ...
        mul_node = Mul(
            shape_node.graph,
            dict(name=shape_node.id + "/MulMinus3_" + str(input_index)),
        ).create_node()
    ...
    def resolve_minus4(self, shape_node, input_index, reshape_index, dims):
        shape_const_node = Const(
            shape_node.graph,
            dict(
                name=shape_node.id + "/ShapeMinus4_index_const_" + str(input_index),
                value=np.take(dims, [reshape_index + 1, reshape_index + 2]),
            ),
        ).create_node()
        input_index = input_index + 2
    ...
    def resolve_const_shape(self, shape_node, input_index, reshape_index, dims):
        dim_const_node = Const(
            shape_node.graph,
            dict(
                name=shape_node.id + "/DimConst_" + str(reshape_index),
                value=[dims[reshape_index]],
            ),
        ).create_node()
    ...
    def resolve(
        self, input_index, reshape_index, dims, input_shape_node, output_dims_nodes
    ):

        resolve_handler = self.resolve_handlers(dims[reshape_index])
        input_index, reshape_index, dims, dims_node = resolve_handler(
            input_shape_node, input_index, reshape_index, dims
        )
        ...
        for d in node.dim:
            if reshape_index < len(node.dim):
                input_index, reshape_index, output_dims_nodes = self.resolve(
                    input_index, reshape_index, node.dim, shape_node, output_dims_nodes
                )

        concat_node = Concat(
            shape_node.graph,
            dict(
                name=shape_node.id + "/ConcatMXReshape_",
                axis=0,
                in_ports_count=len(output_dims_nodes),
            ),
        ).create_node()
