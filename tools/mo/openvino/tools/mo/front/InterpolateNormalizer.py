...
class InterpolateNormalizer(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        if 1 not in node.in_ports() or node.in_port(1).disconnected():
            ...
            if (
                node.has_valid("factor")
                and not node.has_valid("width")
                and not node.has_valid("height")
            ):
                ...
                ss = StridedSlice(
                    graph,
                    {
                        ...
                        "begin_mask": mo_array([1]),
                        "end_mask": mo_array([0]),
                        "new_axis_mask": mo_array([0]),
                        "shrink_axis_mask": mo_array([0]),
                        "ellipsis_mask": mo_array([0]),
                    },
                ).create_node()
                ...
                if (
                    node.soft_get("shrink_factor") != 1
                    and node.soft_get("zoom_factor") == 1
                ):
                    ...
                    const = Const(
                        graph,
                        {
                            "name": node.name + "/pre_shrink_sub_const",
                            "value": mo_array(-1),
                        },
                    ).create_node()
                    sub = Add(
                        graph, {"name": node.name + "/pre_shrink_sub"}
                    ).create_node()
                    ...
                    const = Const(
                        graph,
                        {
                            "value": mo_array(1 / shrink_factor),
                            "name": node.name + "shrink_factor_div_const",
                        },
                    ).create_node()
                    div = Mul(
                        graph, {"name": node.name + "shrink_factor_div"}
                    ).create_node()
                    ...
                    const = Const(
                        graph,
                        {
                            "name": node.name + "/shrink_factor_add_one_const",
                            "value": mo_array(1),
                        },
                    ).create_node()
                    add = Add(
                        graph, {"name": node.name + "/shrink_factor_add_one"}
                    ).create_node()
                    ...
                    if zoom_factor < 1:
                        log.error(
                            "Zoom factor should be positive in node {}".format(node.id)
                        )
                        return None

                    node["debug_message"] = (
                        "Interpolate layer replacer may be wrong, please, try to update it in the"
                        " file (openvino/tools/mo/front/InterpolateNormalizer.py at the line {})."
                        "".format(inspect.currentframe().f_lineno)
                        + refer_to_faq_msg(100)
                    )
                    ...
                    const = Const(
                        graph,
                        {
                            "value": mo_array(zoom_factor),
                            "name": node.name + "/zoom_factor_mul_const",
                        },
                    ).create_node()
                    mul = Mul(
                        graph, {"name": node.name + "/zoom_factor_mul"}
                    ).create_node()
                    ...
                    const = Const(
                        graph, {"value": mo_array([node.height, node.width])}
                    ).create_node()
                    node.add_input_port(1, skip_if_exist=True)
                    assert node.in_port(1).disconnected()
                    ...
                    sub = Add(
                        graph, {"name": node.name + "/shrink_zoom_factor_sub"}
                    ).create_node()
                    add.out_port(0).connect(sub.in_port(0))
                    const.out_port(0).connect(sub.in_port(1))

                    const = Const(
                        graph, {"value": mo_array(1 / (shrink_factor + 1))}
                    ).create_node()
                    div = Mul(
                        graph, {"name": node.name + "/shrink_factor_div"}
                    ).create_node()
                    ...
                    const = Const(
                        ...
                        {
                            "value": mo_array(-1),
                            "name": node.name + "shrink_zoom_factor_sum_const",
                        },
                    ).create_node()
                    sum = Add(
                        graph, {"name": node.name + "/shrink_zoom_factor_sum"}
                    ).create_node()
                    div.out_port(0).connect(sum.in_port(0))
                    const.out_port(0).connect(sum.in_port(1))

                    const = Const(
                        graph, {"value": mo_array(zoom_factor - 1)}
                    ).create_node()
                    mul = Mul(
                        graph, {"name": node.name + "/zoom_factor_mul"}
                    ).create_node()
                    ...
                    sum = Add(
                        graph, {"name": node.name + "/final_shrink_zoom_factor_sum"}
                    ).create_node()
                    div.out_port(0).connect(sum.in_port(0))
                    mul.out_port(0).connect(sum.in_port(1))
