...
class RankDecomposer(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        assert node.has_valid(
            "output_type"
        ), "Rank node should have `output_type` attribute, but it`s not for node {}".format(
            name
        )

        shape_of = Shape(
            graph, {"name": name + "/shape_of", "output_type": node.output_type}
        ).create_node()
        rank_1d = Shape(
            graph, {"name": name + "/rank_of", "output_type": node.output_type}
        ).create_node()
