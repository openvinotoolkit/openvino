...
class AtenToEmbeddingBag(FrontReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in graph.get_op_nodes(op="ATen", operator="embedding_bag"):
            assert node.soft_get("mode") == 0, (
                'ATen::embedding_bag has unsupported mode, only "sum" '
            ...
            if len(node.in_ports()) < 3 or node.in_port(2).disconnected():
                is_packed = True
                embedding_bag = EmbeddingBagPackedSum(
                    graph, {"name": node_name}
                ).create_node()
            else:
                embedding_bag = EmbeddingBagOffsetsSum(
                    graph, {"name": node_name}
                ).create_node()
            ...
            if len(node.in_ports()) == 4 and not node.in_port(3).disconnected():
                if is_packed:
                    ...
                    weights_shape_node = Shape(
                        graph, {"name": node_name + "/WeightsShape"}
                    ).create_node()

                    weights_rank_node = Rank(
                        graph, {"name": node_name + "/WeightsRank"}
                    ).create_node()
                    ...
                    weights_concat = Concat(
                        graph,
                        {"axis": 0, "in_ports_count": 2, "name": node_name + "/Concat"},
                    ).create_node()
