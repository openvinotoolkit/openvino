        ...
        for u, v, d in edges:
            ...
            if len(undefined):
                raise Error(
                    ...
                    u,
                    v,
                    " and ".join(undefined) + refer_to_faq_msg(25),
                )
            used_tensors.add(u)
...
def add_attrs_props(attrs: dict):
    update_attrs(
        attrs, "dim_attrs", ["spatial_dims", "channel_dims", "batch_dims", "axis"]
    )
    update_attrs(
        attrs, "shape_attrs", ["shape", "pad", "window", "stride", "output_shape"]
    )
    return attrs


def spatial_attr_getter(
    node: Node, field: str = None, dim: int = None, post: callable = None
):
    """
...
def bool_to_str(node: Node, attr: str):
    ...
    if isinstance(attribute_name, bool):
        ...
        raise Error(
            "Wrong value {} for boolean attribute {} in node {}".format(
...
def kernel_getter(node: Node, dim: int):
    if node.kind == "op" and node.op in ["Conv2D", "DepthwiseConv2dNative", "Deconv2D"]:
        if node.has("kernel_spatial"):
            return node.kernel_spatial[
                dim
            ]  # TODO check if order of dimension matches expectations
...
def node_defs_to_str(node: Node):
    node_name_to_pb_mapping = {
        node_name: node_def for node_name, node_def in node["pbs"].items()
    }
    result = ""
                ...
                [
                    (
                        ...
                        [
                            ...
                            (
                                "aspect_ratio",
                                lambda node: attr_getter(node, "aspect_ratio"),
                            ),
                            "decrease_label_id",
                            "normalized",
                            "scale_all_sizes",
                            ...
                            (
                                "output",
                                lambda node: (
                                    node.output_shape[node.channel_dims][0]
                                    if node.has("output_shape")
                                    and node.has("channel_dims")
                                    else None
                                ),
                            ),
                            (
                                "input_nodes_names",
                                lambda node: (
                                    " ".join(node["input_nodes_names"])
                                    if node.has("input_nodes_names")
                                    else None
                                ),
                            ),
                            (
                                ...
                                lambda node: (
                                    " ".join(node["output_tensors_names"])
                                    if node.has("output_tensors_names")
                                    else None
                                ),
                            ),
                            (
                                ...
                                lambda node: (
                                    ";".join(
                                        [
                                            " ".join(map(str, shape))
                                            for shape in node["real_input_dims"]
                                        ]
                                    )
                                    if node.has("real_input_dims")
                                    else None
                                ),
                            ),
                            (
                                "protobuf",
                                lambda node: (
                                    node_defs_to_str(node) if node.has("pbs") else None
                                ),
                            ),
                            ...
                            (
                                ...
                                lambda node: (
                                    ",".join(
                                        map(str, node["stride"][node.spatial_dims])
                                    )
                                    if node.has_valid("stride")
                                    else None
                                ),
                            ),
                            (
                                "kernel",
                                lambda node: (
                                    ",".join(map(str, node["kernel_spatial"]))
...
def create_tensor_nodes(graph: Graph):
    ...
    for node in list(graph.nodes()):
        ...
        node_name = (
            str(smart_node.name) if smart_node.has_valid("name") else str(smart_node.id)
        )

        # assign to each output port a tensor unique id in the graph
        out_tensor_dict = {
            port: graph.unique_id("{}/Output_{}/Data_".format(node_name, port))
        ...
        graph.add_nodes_from(
            [
                ...
                for port, uid in out_tensor_dict.items()
            ]
        )
            ...
            if port not in added_out_ports:
                graph.add_edges_from(
                    [
                        (
                            node,
                            out_tensor_dict[port],
                            get_specific_edge_attrs(attrs, "out_attrs"),
                        )
                    ]
                )
                # merge additional data node attributes from original edge
                graph.node[out_tensor_dict[port]].update(
                    get_specific_edge_attrs(attrs, "data_attrs")
                )
        ...
        for u, v, d in out_edges:
            graph.add_edges_from(
                [(out_tensor_dict[d["out"]], v, get_specific_edge_attrs(d, "in_attrs"))]
            )
...
def extract_node_attrs(graph: Graph, extractor: callable):
    ...
    unsupported.report(
        log.warning, "Instructions/layers that do not have attribute extractors:"
    )
    ...
    for name in node_names:
        ...
        if match and match.group() == node_name:
            ...
            if match.group(1) and match.group(3):
                log.warning(
                    "Skipping the case with both in and out port specified, only one port can be specified"
                )
                continue
            node = Node(graph, graph.get_node_id_by_name(name))
            if match.group(1):
                in_port = int(match.group(1).replace(":", ""))
                if skip_if_no_port and in_port not in [
                    e["in"] for e in node.in_edges().values()
                ]:
                    # skip found node if it doesn't have such port number
                    continue
            if match.group(3):
                out_port = int(match.group(3).replace(":", ""))
                if skip_if_no_port and out_port not in [
                    e["out"] for e in node.out_edges().values()
                ]:
                    # skip found node if it doesn't have such port number
                    continue
...
def input_user_data_repack(
    ...
    if freeze_placeholder is None:
        ...
        if isinstance(freeze_placeholder, list):
            raise Error(
                "Unnamed inputs with values are not supported for legacy frontend. Please provide input names."
            )
        for placeholder_name, value in freeze_placeholder.items():
            placeholder_id, direction, port = get_node_id_with_ports(
                graph, placeholder_name
            )
            if port is None and placeholder_id in placeholders_ids:
                ...
                new_placeholder_id = get_new_placeholder_name(
                    placeholder_id, is_out_port, port
                )
    ...
    if isinstance(input_user_shapes, list):
        if len(input_user_shapes) == 1 and isinstance(
            input_user_shapes[0], PartialShape
        ):
    ...
    if input_user_shapes is None:
        ...
        raise Error(
            "Please provide input layer names for input layer shapes. "
            + refer_to_faq_msg(58)
        )
        ...
        for input_name in input_user_shapes:
            ...
            if (
                input_name in input_user_data_types
                and input_user_data_types[input_name] is not None
            ):
                data_type = input_user_data_types[input_name]
                _input_shapes[node_id].append(
                    {"shape": shape, direction: port, "data_type": data_type}
                )
            else:
        ...
        if _freeze_placeholder is not None:
            ...
            [
                _input_shapes[ph_id].append({"shape": None, "port": None})
                for ph_id in _freeze_placeholder
        ...
        if len(placeholders_ids) == 1:
            ...
            _input_shapes[placeholders_ids[0]].append(
                {"shape": input_user_shapes, "port": None}
            )
            ...
            if len(placeholders_ids) != 1:
                raise Error(
                    "Original placeholders: '{}'. Freezing was requested for '{}'. --input_shape was "
                    "provided without --input. Can not deduce which node shape to override"
                    "".format(
                        ", ".join(original_phs), ", ".join(_freeze_placeholder.keys())
                    )
                )
            _input_shapes[placeholders_ids[0]].append(
                {"shape": input_user_shapes, "port": None}
            ...
            [
                _input_shapes[node_id].append({"shape": None, "port": None})
                for node_id in _freeze_placeholder
            ]
        else:
            ...
            raise Error(
                "No or multiple placeholders in the model, but only one shape is provided, cannot set it. "
                + refer_to_faq_msg(32)
            )
    ...
    for node_id in _freeze_new_placeholder:
        ...
        if node_id not in _input_shapes:
            raise Error(
                "Shape is not specified for the placeholder with name {} through --input_shape option."
        ...
        for new_ph in new_phs:
            ...
            if any(
                [
                    _in["shape"] is not None
                    and direction in _in
                    and _in[direction] == port
                    for _in in _ins
                ]
            ):
                _freeze_placeholder[placeholder_id] = value
...
def user_data_repack(
    ...
    """
    _input_shapes, _freeze_placeholder = input_user_data_repack(
        graph,
        input_user_shapes,
        freeze_placeholder,
...
def add_output_ops(graph: Graph, user_defined_outputs: dict, inputs: dict = None):
    ...
    assert not (
        isinstance(user_defined_outputs, list) and not len(user_defined_outputs)
    )
    ...
    graph.remove_nodes_from(
        [
            ...
            if "op" in graph.node[node_name] and graph.node[node_name]["op"] == "Result"
        ]
    )

    if user_defined_outputs is None:
        inputs = (
            graph.get_nodes_with_attributes(op="Parameter")
            if inputs is None
            else list(inputs.keys())
        )
        ...
        for node_name in list(graph.nodes()):
            if len(list(graph.out_edges(node_name))) == 0:
                if node_name in input_reachable:
                    out_ports_count = (
                        Node(graph, node_name).out_ports_count
                        if Node(graph, node_name).has_valid("out_ports_count")
                        else 1
                    )
        ...
        if len(dead_outputs):
            log.info(
                "Possible outputs: '{!s}' are not input reachable. True outputs are {!s}"
                "".format(
                    ", ".join([str(d_o) for d_o in dead_outputs]),
        ...
        for node, values in user_defined_outputs.items():
            if node not in graph.nodes():
                raise Error(
                    'Node "{}" does not exist in the graph. ' + refer_to_faq_msg(26),
                    node,
                )
            for value in values:
                if "in" in value:
                    ...
                    if len(in_edges) - 1 < value["in"]:
                        raise Error(
                            'Port index {} is out of number of available input ports for node "{}". '
                            + refer_to_faq_msg(29),
                            value["in"],
                            node,
                        )
                    for u, v, attrs in in_edges:
                        if "in" in attrs and attrs["in"] == value["in"]:
                            sinks.append(
                                add_opoutput(
                                    graph, u, attrs["out"], user_defined_name=node
                                )
                            )
                elif "out" in value:
                    out_edges = list(graph.out_edges(node, data=True))
                    if len(out_edges) - 1 < value["out"]:
                        raise Error(
                            'Port index {} is out of number of available output ports for node "{}". '
                            + refer_to_faq_msg(29),
                            value["out"],
                            node,
                        )
                    for u, v, attrs in out_edges:
                        if "out" in attrs and attrs["out"] == value["out"]:
                            sinks.append(
                                add_opoutput(
                                    graph, node, attrs["out"], user_defined_name=node
                                )
                            )
                else:
...
def add_outputs_identity(
    graph: Graph, outputs: list, add_edge: callable, params: dict = {}
):
    """
...
def check_input(graph: Graph, node_name: str):
    ...
    if (
        node["kind"] == "op"
        and node["op"] == "Parameter"
        and not len(graph.in_edges(node_name))
        and not node["is_input"]
    ):
        raise Error(
            "--input parameter was provided. Other inputs are needed for output computation. "
            "Provide more inputs or choose another place to cut the net. "
            + refer_to_faq_msg(27)
        )
        ...
        if len(parts) > 1:
            if parts[0].isdigit():
                ...
                try:
                    ...
                    log.warning(
                        'Didn\'t recognize port:node format for "{}" because port is not an integer.'.format(
                            node_id
                        )
                    )
            else:
                ...
                try:
                    ...
                    log.warning(
                        'Didn\'t recognize node:port format for "{}" because port is not an integer.'.format(
                            node_id
                        )
                    )
...
def add_input_op_input_port_without_data(
    graph: Graph, node_id: str, input_op, edge_attrs: dict
):
...
def add_input_op_input_port_with_data(
    graph: Graph, node_id: str, input_op, edge_attrs: dict
):
    assert graph.stage == "middle", (
        "add_input_op_input_port_with_data() function can be used only for graph after "
        "shape inference!"
    )
...
def add_input_op_output_port_without_data(
    graph: Graph, node_id: str, input_op, port: int, fw_info: list
):
    input_node = input_op.create_node()
    ...
    for _, out_node, attrs in graph.out_edges(node_id, data=True):
        if attrs["out"] == port:
            ...
            log.debug(
                "Input: {} for node {} output port {}".format(
                    input_node.id, node_id, port
                )
            )
...
def add_input_op_output_port_with_data(
    graph: Graph, node_id: str, input_op, port: int, fw_info: list
):
    ...
    input_node = input_op.create_node()
    Node(graph, node_id).out_port(port).get_connection().set_source(
        input_node.out_port(0)
    )
    log.debug("Input: {} for node {}".format(input_node.id, node_id))
    log.debug("Add edge from {} to {}".format(input_node.id, node_id))
    return input_node.id
...
def add_input_op(
    ...
    if not is_out_port and port == 0:
        ...
        if graph.has_tensor_name(tensor_name_no_port):
            log.warning(
                "Could not add user defined input name {} to tensor names list of as "
    ...
    if not data:
        if is_out_port:
            new_input_id = add_input_op_output_port_without_data(
                graph=graph,
                node_id=node_id,
                input_op=input_op,
                port=port,
                fw_info=edge_attrs["fw_tensor_debug_info"],
            )
        else:
            new_input_id = add_input_op_input_port_without_data(
                graph=graph, node_id=node_id, input_op=input_op, edge_attrs=edge_attrs
            )
    else:
        if is_out_port:
            new_input_id = add_input_op_output_port_with_data(
                graph=graph,
                node_id=node_id,
                input_op=input_op,
                port=port,
            ...
            new_input_id = add_input_op_input_port_with_data(
                graph=graph, node_id=node_id, input_op=input_op, edge_attrs=edge_attrs
            )
    return new_input_id


def add_input_ops_helper_before_infer_input_port(
    graph: Graph,
    smart_node: Node,
    port: int,
    node_id: str,
...
def add_input_ops_helper_after_infer_input_port(
    graph: Graph,
    smart_node: Node,
    port: int,
    node_id: str,
    ...
    if shape is None:
        raise Error(
            'Shape for tensor "{}" is not defined. Can not proceed.'
            + refer_to_faq_msg(41),
...
def add_input_ops_helper_before_infer_output_port(
    graph: Graph,
    port: int,
    node_id: str,
    shape: np.array,
...
def add_input_ops_helper_after_infer_output_port(
    graph: Graph,
    smart_node: Node,
    port: int,
    node_id: str,
    ...
    if user_defined_inputs is None:
        ...
        for node_id in user_defined_inputs:
            for port_and_shape_info in user_defined_inputs[node_id]:
                ...
                is_out_port = (
                    "out" in port_and_shape_info
                )  # by default we assume input port or input node without port
                shape = (
                    port_and_shape_info["shape"]
                    if "shape" in port_and_shape_info
                    else None
                )
                user_shape = None
                ...
                data_type = (
                    port_and_shape_info["data_type"]
                    if "data_type" in port_and_shape_info
                    else None
                )
                ...
                if is_out_port:
                    port = port_and_shape_info[
                        "out"
                    ]  # we check that 'out' in port_and_shape_info earlier
                    if port is None:
                        raise Error(
                            "Output port for input node {} should be specified, it cannot be None!".format(
                                node_id
                            )
                        )
                    ...
                    port = (
                        port_and_shape_info["in"]
                        if "in" in port_and_shape_info
                        else None
                    )
                ...
                if before_infer:
                    ...
                    if is_out_port:
                        add_input_ops_helper_after_infer_output_port(
                            graph, smart_node, port, node_id, inputs, edges_to_remove
                        )
                    else:
    ...
    def check_param(op_cls, attrs):
        for a in attrs:
            if a not in op_cls.supported_attrs(op_cls):
                log.error(
                    "Parameter {} is not recognised, please check correctness.\n List of supported parameters "
                    "is: {}".format(a, op_cls.supported_attrs(op_cls)),
