...
class GeLUMergerTanh(FrontReplacementSubgraph):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        inp = inp_port.node
        log.debug(
            "Found potential TanH-based GeLU pattern after {} with name {}".format(
                inp.op, inp.name
            )
        )
        ...
        if (
            mul0_param.value.size == 1
            and mul_param.value.size == 1
            and mul1_param.value.size == 1
        ):
            ...
            if (
                fabs(mul0_param - sqrt2pi) < 1e-06
                and fabs(mul_param - 0.044715) < 1e-06
                and mul1_param == 0.5
            ):
                log.debug(
                    "Confirmed TanH-based GELU pattern after {} with name {}".format(
                        inp.op, inp.name
                    )
                )
                gelu = GeLUOP(
                    graph, dict(name=inp.name + "/GELU_", approximation_mode="tanh")
                ).create_node()
