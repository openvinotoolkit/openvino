...
class FreezePlaceholderValue(FrontReplacementSubgraph):
    ...
    @staticmethod
    def pattern():
        return dict(nodes=[("placeholder", dict(kind="op", op="Parameter"))], edges=[])
        ...
        if ph.name in graph.graph["freeze_placeholder"]:
            ...
            try:
                ...
                # TODO: investigate why boolean type is allowed only for TensorFlow
                    ...
                    raise Error(
                        "Cannot cast value {} to {} data_type".format(
            ...
            try:
                ...
                raise Error(
                    "Can not reshape value {} to shape {}".format(value, ph.shape)
                )
            ...
            log.info(
                'Placeholder node "{}" was replaced with Const node "{}" with value "{}"'.format(
