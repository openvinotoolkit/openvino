...
class FullyConnectedDecomposer(FrontReplacementSubgraph):
    ...
    def pattern(self):
        return dict(nodes=[("op", dict(kind="op", type="FullyConnected"))], edges=[])
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in graph.get_op_nodes(op="Gemm"):
            ...
            if node.is_in_port_connected(2):
                # biases normalization
                bias_node = Add(
                    graph, {"name": name + "/Bias_", "can_be_scaleshift": False}
                ).create_node()
                ...
                if node.has_valid("beta") and not math.isclose(node.beta, 1):
                    bias_node.insert_op_on_input_port(
                        in_port_idx=1,
                        new_op_class=Mul,
                        value=mo_array(node.beta),
                        new_op_attrs={
                            "name": name + "/Beta",
