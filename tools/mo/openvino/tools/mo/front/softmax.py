...
class SoftmaxFromKeras(FrontReplacementSubgraph):
    ...
    def pattern(self):
        return dict(
            nodes=[
                ...
                (
                    "reduce_indices_max",
                    dict(op="Const", value=lambda x: x is not None and x.size != 0),
                ),
                ("sub", dict(op="Sub")),
                ("exp", dict(op="Exp")),
    ...
    def replace_sub_graph(self, graph: Graph, match: [dict, SubgraphMatch]):
        ...
        if len(reduce_max_axis) != 1:
            log.info(
                "The reductions indices contain more than 1 element. Cannot convert to Softmax."
            )
            return

        if not np.array_equal(reduce_max_axis, reduce_sum_axis):
            log.info(
                "The reduce indices are not equal: {} vs {}. Cannot convert to Softmax"
        ...
        softmax = Softmax(
            graph,
            {"name": match["input"].name + "/Softmax", "axis": reduce_sum_axis[0]},
        ).create_node()
