...
class AttributedPadToPad(FrontReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for attr_pad in graph.get_op_nodes(op="AttributedPad"):
            ...
            rename_nodes(
                [(attr_pad, original_name + "/to_be_removed"), (new_pad, original_name)]
            )

            attr_pad.in_port(0).get_connection().set_destination(new_pad.in_port(0))
            new_pad.in_port(1).connect(
                Const(graph, {"value": attr_pad.pads[:, 0]}).create_node().out_port(0)
            )
            new_pad.in_port(2).connect(
                Const(graph, {"value": attr_pad.pads[:, 1]}).create_node().out_port(0)
            )
