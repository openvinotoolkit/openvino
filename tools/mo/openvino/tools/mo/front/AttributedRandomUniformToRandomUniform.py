...
class AttributedRandomUniformToRandomUniform(FrontReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for attr_random_uniform in graph.get_op_nodes(op="AttributedRandomUniform"):
            ...
            if (
                not attr_random_uniform.has_port("in", 0)
                or attr_random_uniform.in_port(0).disconnected()
            ):
            ...
            new_random_uniform = create_op_with_const_inputs(
                graph, op=RandomUniform, port_value_dict=port_value_dict, op_attrs=attrs
            )
            rename_nodes(
                [
                    (attr_random_uniform, original_name + "/to_be_removed"),
            ...
            if new_random_uniform.in_port(0).disconnected():
                if attr_random_uniform.in_port(0).disconnected():
                    ...
                    new_random_uniform.in_port(0).connect(
                        attr_random_uniform.in_port(0).get_connection().get_source()
                    )
