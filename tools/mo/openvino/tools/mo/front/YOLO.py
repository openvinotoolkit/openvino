...
class YoloRegionAddon(FrontReplacementFromConfigFileGeneral):
    ...
    def transform_graph(self, graph: Graph, replacement_descriptions):
        op_outputs = [
            n for n, d in graph.nodes(data=True) if "op" in d and d["op"] == "Result"
        ]
        for op_output in op_outputs:
            last_node = Node(graph, op_output).in_node(0)
            op_params = dict(
                name=last_node.id + "/YoloRegion", axis=1, end_axis=-1, nchw_layout=True
            )
