...
class ExpandRangeConstant(FrontReplacementSubgraph):
    ...
    def replace(node: Node, const: Node):
        ...
        gather = create_op_with_const_inputs(
            graph,
            Gather,
            {1: int64_array(negative_idx), 2: int64_array(0)},
        ...
        equal_node = create_op_with_const_inputs(
            graph, Equal, {1: int64_array(1)}, {"name": node_name + "/ConstOne"}
        )
        gather.out_port(0).connect(equal_node.in_port(0))

        select_node = Select(
            graph, {"name": node_name + "/Select", "auto_broadcast": "numpy"}
        ...
        range_node = create_op_with_const_inputs(
            graph,
            Range,
            {0: mo_array(0, dtype=value.dtype), 2: mo_array(1, dtype=value.dtype)},
        ...
        if one_dims.size:
            unsqueeze = create_op_node_with_second_input(
                graph, Unsqueeze, one_dims, {"name": const_name + "/KeepShape"}
            )
            range_node.out_port(0).get_connection().insert_node(unsqueeze)
            rename_nodes(
                [(const, const_name + "/ToBeDeleted"), (unsqueeze, const_name)]
            )
        else:
            rename_nodes(
                [(const, const_name + "/ToBeDeleted"), (range_node, const_name)]
            )
