    ...
    def find_and_replace_pattern(self, graph: Graph):
        for expand_dims_node in graph.get_op_nodes(op="ExpandDims"):
            if len(expand_dims_node.in_nodes()) == 1:
                ...
                unsqueeze_dims_node = Const(
                    graph, {"name": expand_dims_node.id + "/Dims", "value": expand_axis}
                ).create_node()
                ...
                unsqueeze_node = Unsqueeze(
                    graph, {"name": expand_dims_name + "/Unsqueeze"}
                ).create_node()
                rename_nodes(
                    [
                        (expand_dims_node, expand_dims_name + "/TBR"),
