...
class SizeFrontReplacer(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        assert node.has_valid(
            "output_type"
        ), "Size node should have `output_type` attribute, but it`s not for node {}".format(
            name
        )

        shape = Shape(
            graph, {"name": name + "/Shape/", "output_type": node.output_type}
        ).create_node()
        ...
        reduce_prod = create_op_node_with_second_input(
            ...
            ReduceProd,
            int64_array([0]),
            {"name": shape.name + "ReduceProd/", "keep_dims": False},
            shape,
        )
