...
class GlobalPoolingToReduce(FrontReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        assert (
            layout != "NHWC"
        ), "Global pooling transformation depends on layout (NHWC not enabled)"
        ...
        for pooling in global_poolings:
            ...
            assert (
                method in self.pool_method_to_reduce_type
            ), "Unexpected Global Pooling method `{}` for node `{}`".format(
                method, name
            )
            reduce_op_class = self.pool_method_to_reduce_type[method]

            reduce = reduce_op_class(
                graph, {"name": name + "/reduce", "keep_dims": True}
            ).create_node()
            ...
            axis = Range(
                graph, {"name": name + "/global_pooling_reduce_axis"}
            ).create_node()
            ...
            log.debug(
                "Global {} pooling was converted to reduce: `{}`".format(method, name)
            )
