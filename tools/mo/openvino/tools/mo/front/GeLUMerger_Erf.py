...
class GeLUMergerErf(FrontReplacementPattern):
    ...
    def replace_gelu(self, graph: Graph, match: dict):
        ...
        log.debug(
            "Found potential Erf-based GeLU pattern after {} with name {}".format(
                inp_node.op, inp_name
            )
        )
        ...
        if (
            add_param.value.size == 1
            and mul_param.value.size == 1
            and div_param.value.size == 1
        ):
            ...
            if (
                fabs(div_param - sqrt2) < 1e-06
                and mul_param == 0.5
                and add_param == 1.0
            ):
                log.debug(
                    "Confirmed Erf-based GELU pattern after {} with name {}".format(
                        inp_node.op, inp_name
                    )
                )
                gelu = GeLUOP(
                    graph, dict(name=inp_name + "/GELU_", approximation_mode="erf")
                ).create_node()
