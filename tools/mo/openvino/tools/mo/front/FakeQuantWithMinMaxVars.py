...
from openvino.tools.mo.ops.elementwise import Add, Div, Greater, Less, Mul, Round, Sub
...
class FakeQuantWithMinMaxVarsToQuantize(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: Dict[str, Node]):
        ...
        if min_port_tuple[0].has_and_set("value") and max_port_tuple[0].has_and_set(
            "value"
        ):
            assert (
                min_port_tuple[0]["value"].dtype == max_port_tuple[0]["value"].dtype
        ...
        # make sure min < max
        min_less_max = Less(graph, {"name": name + "/if_min_less_max"}).create_node(
            [min_port_tuple, max_port_tuple]
        )
        minimum = Select(graph, {"name": name + "/minimum"}).create_node(
            [min_less_max, min_port_tuple, max_port_tuple]
        )
        maximum = Select(graph, {"name": name + "/maximum"}).create_node(
            [min_less_max, max_port_tuple, min_port_tuple]
        )

        # to create zero of limits data type, we multiply it by integer zero
        zero = create_op_node_with_second_input(
            graph,
            Mul,
            mo_array(0, dtype=dtype),
            {"name": name + "/zero"},
            input_node=minimum,
        )

        # if 0 < min < max: min_adj = 0 and max_adj = max - min
        min_greater_zero = Greater(
            graph, {"name": name + "/if_minimum_greater_zero"}
        ).create_node([minimum, zero])
        max_minus_min = Sub(graph, {"name": name + "/max_minus_min"}).create_node(
            [maximum, minimum]
        )
        minimum = Select(graph, {"name": name + "/first_adj_min"}).create_node(
            [min_greater_zero, zero, minimum]
        )
        maximum = Select(graph, {"name": name + "/first_adj_max"}).create_node(
            [min_greater_zero, max_minus_min, maximum]
        )

        # if min < max < 0: min_adj = min - max and max_adj = 0
        max_less_zero = Less(graph, {"name": name + "/if_max_less_zero"}).create_node(
            [maximum, zero]
        )
        min_minus_max = Sub(graph, {"name": name + "/min_minus_max"}).create_node(
            [minimum, maximum]
        )
        minimum = Select(graph, {"name": name + "/second_adj_min"}).create_node(
            [max_less_zero, min_minus_max, minimum]
        )
        maximum = Select(graph, {"name": name + "/second_adj_max"}).create_node(
            [max_less_zero, zero, maximum]
        )

        # scale = (max - min) / (2 ^ num_bits - 1),
        float_range = Sub(graph, {"name": name + "/float_range"}).create_node(
            [maximum, minimum]
        )
        ...
        int_range = Const(
            graph, dict(name=name + "/int_range", value=int_range_value)
        ).create_node()
        ...
        rounded_descaled_min = Round(
            graph, {"name": name + "/rounded_descaled_min"}
        ).create_node([descaled_min])
        min_adj = Mul(graph, {"name": name + "/min_adj"}).create_node(
            [scale, rounded_descaled_min]
        )
        # max_adj = max + min_adj - min.
        adjustment = Sub(graph, {"name": name + "/limits_adjustment"}).create_node(
            [min_adj, minimum]
        )
        max_adj = Add(graph, {"name": name + "/max_adj"}).create_node(
            [maximum, adjustment]
        )
