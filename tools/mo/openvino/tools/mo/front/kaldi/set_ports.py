...
class SetPortsPattern(FrontReplacementSubgraph):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for node_id in graph.nodes(data=False):
            ...
            in_ports_count = (
                node.in_ports_count if node.has_valid("in_ports_count") else len(inputs)
            )
            out_ports_count = (
                node.out_ports_count
            ...
            if len(outputs) > out_ports_count > 1:
                raise Error(
                    "Node {} has more children than it should: "
            ...
            for in_node_id, edge_attrs in inputs:
                ...
                in_node.out_edge(edge_attrs["out"])["fw_tensor_debug_info"] = (
                    edge_attrs["fw_tensor_debug_info"]
                )
