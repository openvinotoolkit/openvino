...
from openvino.tools.mo.ops.split import AttributedVariadicSplit, Split
...
class ReplaceLstmNonLinearityPattern(FrontReplacementOp):
    ...
    def replace_op(self, graph: Graph, node: Node):
        ...
        split_node = create_op_with_const_inputs(
            ...
            Split,
            {1: np.int64(1)},
            {"name": node_name + "/split_lstm_input", "num_splits": 5},
        )
        ...
        i_scale_attrs = {"name": node_name + "/i_scaleshift", "bias_term": False}
        ...
        if node["use_dropout"]:
            mul_dropout_i = Mul(
                graph, {"name": split_node.soft_get("name", split_node.id) + "/mul_i"}
            ).create_node()
        ...
        f_scale_attrs = {"name": node_name + "/f_scaleshift", "bias_term": False}
        ...
        if node["use_dropout"]:
            mul_dropout_f = Mul(
                graph, {"name": split_node.soft_get("name", split_node.id) + "/mul_f"}
            ).create_node()
        ...
        prod_i_c_tanh = Mul(
            graph, {"name": node_name + "/prod_i_c_tanh_"}
        ).create_node()
        ...
        o_scale_attrs = {"name": node_name + "/o_scaleshift", "bias_term": False}
        ...
        if node["use_dropout"]:
            mul_dropout_o = Mul(
                graph, {"name": split_node.soft_get("name", split_node.id) + "/mul_o"}
            ).create_node()
        ...
        prod_o_c_t_tanh = Mul(
            graph, {"name": node_name + "/prod_o_c_t_tanh_"}
        ).create_node()
