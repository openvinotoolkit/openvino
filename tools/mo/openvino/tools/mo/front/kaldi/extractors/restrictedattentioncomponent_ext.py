...
from openvino.tools.mo.ops.restrictedattentioncomponent import (
...
class RestrictedAttentionComponentFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        attrs["entropy_stats"] = (
            mo_array(entropy_stats) if len(entropy_stats) != 0 else None
        )

        collect_until_token(params, b"<PosteriorStats>")
        posterior_stats, posterior_stats_shape = read_binary_matrix(params)
        attrs["posterior_stats"] = np.reshape(posterior_stats, posterior_stats_shape)
