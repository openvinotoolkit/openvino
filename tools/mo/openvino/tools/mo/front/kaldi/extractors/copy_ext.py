...
class CopyFrontExtractor(FrontReplacementOp):
    ...
    def replace_op(self, graph: Graph, node: Node):
        ...
        perm_in_1 = Const(
            graph, {"value": int64_array([1, 0]), "name": node_name + "/order"}
        ).create_node()
        perm1_node = Transpose(
            graph, {"name": node_name + "/input_permute"}
        ...
        gather_node = create_op_with_const_inputs(
            graph, Gather, {2: int64_array(0)}, {"name": node_name + "/gather"}
        )
        gather_node.in_port(0).connect(perm1_node.out_port(0))
        gather_node.in_port(1).connect(indexes_node.out_port(0))

        perm2_node = Transpose(
            graph, {"name": node_name + "/output_permute"}
        ).create_node()
