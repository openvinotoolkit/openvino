...
class ReplaceEltwiseNin1NodePattern(FrontReplacementOp):
    ...
    def replace_op(self, graph: Graph, node: Node):
        ss_node = create_op_with_const_inputs(
            ...
            Split,
            {1: int64_array(1)},
            {"name": "Split_eltwise_" + node.name, "num_splits": node["num_inputs"]},
        )
        ...
        if ss_node.num_splits == 2:
            if node["operation"] == "mul":
                eltwise_node = Mul(
                    graph, attrs={"name": "Eltwise_" + node.name}
                ).create_node()
                ...
                raise Error(
                    "Error on replacing Kaldi eltwise: unknown type "
                    + node["operation"]
                )
        elif ss_node.num_splits > 2:
            eltwise_node = EltwiseN(
                graph,
                attrs={"name": "Eltwise_" + node.name, "operation": node["operation"]},
            ).create_node()
        else:
