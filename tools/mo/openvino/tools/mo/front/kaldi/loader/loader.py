...
def load_parallel_component(file_descr, graph: Graph, prev_layer_id):
    ...
    log.debug(
        "Model contains parallel component with {} nested networks".format(nnet_count)
    )
    ...
    for i in range(nnet_count):
        ...
        mapping = {
            node: graph.unique_id(node) for node in g.nodes(data=False) if node in graph
        }
    ...
    graph.create_edge(
        ...
        0,
        0,
        create_edge_attrs(prev_layer_id, variadic_split_node.id, prev_layer_id),
    )
    ...
    for i, (input_node, output_node) in enumerate(zip(inputs, outputs)):
        ...
        graph.create_edge(
            ...
            0,
            create_edge_attrs(
                variadic_split_node.id, input_node.id, variadic_split_node.id, 0, i
            ),
        )
    return concat_id
...
def load_kalid_nnet1_model(graph, file_descr, name):
    ...
    graph.add_node(
        prev_layer_id, name=prev_layer_id, kind="op", op="Parameter", parameters=None
    )
    ...
    add_outputs_identity(
        graph,
        [output_layer],
        lambda g, output, fake_output: g.create_edge(
            Node(g, output),
            Node(g, fake_output),
            0,
            0,
...
def load_components(file_descr, graph, component_layer_map=None):
    ...
    for _ in range(num_components):
        if is_nnet3:
            name = collect_until_token_and_read(
                file_descr, b"<ComponentName>", np.string_
            )
        ...
        for dim_word in dim_words:
            try:
                collect_until_token(
                    file_descr, dim_word, size_search_zone=end_index - start_index
                )
...
def read_node(file_descr, graph, component_layer_map, layer_node_map):
    ...
    if tokens[0] == b"input-node":
        ...
        in_shape = mo_array(
            [1, s[s.find(b"dim=") + len(b"dim=") :].split(b" ")[0]], dtype=int
        )
        ...
        in_node_id = parse_input_for_node(
            s[s.find(b"input=") + 6 :], graph, layer_node_map
        )
        ...
        if in_node_id != node_name:
            ...
            graph.add_edge(
                ...
                node_name,
                **create_edge_attrs(
                    in_node_id, node_name, in_node_id, in_port, out_port
                )
            )
        ...
        graph.add_edge(
            node_name, out_name, **create_edge_attrs(node_name, out_name, node_name)
        )
        ...
        in_node_id = parse_input_for_node(
            s[s.find(b"input-node=") + len(b"input-node=") :], graph, layer_node_map
        )
        out_port = len(Node(graph, in_node_id).out_nodes())
        in_port = len(Node(graph, node_name).in_nodes())
        ...
        graph.create_edge(
            ...
            out_port,
            in_port,
            create_edge_attrs(in_node_id, node_name, in_node_id, in_port, out_port),
        )
...
def parse_specifier(string, graph, layer_node_map):
    ...
    if pos == -1:
        # node name
        input_name = (
            str(string.split(b" ")[0]).strip("b").replace("'", "").replace("\\n", "")
        )
    ...
    if spec == b"Append":
        ...
        if layer_name not in layer_node_map:
            ...
            graph.add_node(concat_name, parameters=None, op="concat", kind="op")
        ...
        if layer_name not in layer_node_map:
            ...
            const_node = Const(
                graph, {"name": scale_const_name, "value": float_array([scale_value])}
            ).create_node()
            ...
            graph.create_edge(
                ...
                out_port,
                1,
                create_edge_attrs(node_name, scale_node.id, node_name, 1, out_port),
            )
        else:
