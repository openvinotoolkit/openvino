...
def set_time_dim(graph):
    ...
    for node in sorted_nodes:
        ...
        if node.op == "MemoryOffset":
            ...
            node.time_dim = (
                node.in_port(0).get_source().node.time_dim + node.t
                if not node.in_port(0).disconnected()
                else node.t
            )
        elif node.op == "Splice":
            node.time_dim = (
                node.in_port(0).get_source().node.time_dim + len(node.context) - 1
            )
    ...
    for param_node in params:
        for dest in param_node.out_port(0).get_destinations():
            if dest.node.op == "Convolution":
                conv_node = dest.node
                assert (
                    param_node.time_dim is None
                    or param_node.time_dim == conv_node.soft_get("kernel")[1] - 1
...
class AddReshapeTransposeAroundConvPool(FrontReplacementPattern):
    ...
    def run_after(self):
        ...
        from openvino.tools.mo.front.kaldi.split_recurrent_memoryoffset import (
    ...
    def find_and_replace_pattern(graph: Graph):
        ...
        for node in conv_pool_nodes:
            ...
            N, H = node_to_get_shape_value_of_indices(
                i_shape, [0]
            ), node_to_get_shape_value_of_indices(i_shape, [1])

            # H / (patch_stride * t)
            H_div_stride_t = create_op_with_const_inputs(
                graph,
                Div,
                {1: int64_array([frame_height * time_dim])},
            ...
            concat_dims = create_op_with_const_inputs(
                graph,
                Concat,
                {index_const: int64_array([frame_height]), 1: int64_array([time_dim])},
                {
            ...
            reshape_in = Reshape(
                graph, {"name": node_name + "/reshape_in", "time_dim": time_dim - 1}
            ).create_node()
            ...
            direct_transpose = create_op_node_with_second_input(
                ...
                Transpose,
                int64_array([0, 3, 1, 2]),
                {"name": node_name + "/Transpose", "time_dime": time_dim - 1},
                reshape_in,
            )
            ...
            inverse_transpose = create_op_node_with_second_input(
                ...
                Transpose,
                int64_array([0, 2, 3, 1]),
                {"name": node_name + "/Transpose_back", "time_dim": 0},
            )
