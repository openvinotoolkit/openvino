...
def apply_biases_to_last_layer(graph, counts):
    ...
    sub_node = create_op_node_with_second_input(
        graph, Add, -counts, {"name": "sub_counts"}
    )
...
def read_counts_file(file_path):
    ...
    try:
        ...
        raise Error(
            "Expect counts file to contain list of floats." + refer_to_faq_msg(90)
        )
...
class ApplyCountsFilePattern(FrontReplacementSubgraph):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        if graph.graph["cmd_params"].counts == "":
            assert (
                isinstance(graph.graph["priors"], (list, np.ndarray))
                and len(graph.graph["priors"]) != 0
            ...
            try:
                ...
                raise Error(
                    "Model Optimizer is not able to read counts file {}".format(
                        graph.graph["cmd_params"].counts
                    )
                    + refer_to_faq_msg(92)
                ) from e
