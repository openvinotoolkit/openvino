...
class ReplaceLSTMNodePattern(FrontReplacementOp):
    ...
    def run_before(self):
        ...
        from openvino.tools.mo.front.MoveEmbeddedInputsToInputs import (
            MoveEmbeddedInputsToInputs,
        )

        return [FullyConnectedDecomposer, MoveEmbeddedInputsToInputs]

    def pattern(self):
        return dict(
            nodes=[("op", dict(op=self.__class__.op, format="kaldi"))], edges=[]
        )
    ...
    def replace_op(self, graph: Graph, node: Node):
        ...
        fc_layer_after_input = FullyConnected(
            graph, fc_layer_after_input_attrs
        ).create_node()
        ...
        #                           *Eltwise(sum)
        # Input -> FullyConnected   /
        join_input_prev_state_sum = Add(
            graph, {"name": "join_input_eltwise"}
        ).create_node()
        ...
        split_joined_input = Split(
            graph, {"name": "join_input_split", "num_splits": 4, "out_ports_count": 4}
        ).create_node()
        ...
        state_input_scaleshift_attrs = {"name": "input_scaleshift", "bias_term": False}
        state_input_scaleshift = ScaleShiftOp(
            graph, state_input_scaleshift_attrs
        ).create_node()
        ...
        state_forget_scaleshift = ScaleShiftOp(
            graph, state_forget_scaleshift_attrs
        ).create_node()
        ...
        join_prev_lstm_input_joined_input_sum = Add(
            graph, {"name": "join_prev_lstm_input_joined_input_eltwise"}
        ).create_node()
        ...
        #                                       (6)Eltwise(mul)
        # Split -> (3)Eltwise(sum) -> **Sigmoid /
        join_forget_prev_state_mul = Mul(
            graph, {"name": "join_forget_prev_state_mul"}
        ).create_node()
        ...
        join_remember_candidates_mul = Mul(
            graph, {"name": "join_remember_candidates_mul"}
        ).create_node()
        ...
        #               (7)Eltwise(sum)
        # (6)Eltwise(mul)   /
        join_forget_remember_sum = Add(
            graph, {"name": "join_forget_remember_sum"}
        ).create_node()
        ...
        join_forget_clamp = create_op_with_const_inputs(
            graph,
            Clamp,
            {1: float32_array(-node.clip_value), 2: float32_array(node.clip_value)},
            {"name": "join_forget_clamp"},
            join_forget_remember_sum,
        )
        #
        # Clamp -> (2)Memory(state)
        next_lstm_state = Assign(
            graph, {"name": "next_lstm_state", "variable_id": memory_pair_output}
        ).create_node()
        ...
        # Clamp -> (2)ScaleShift
        clamp_scaleshift_attrs = {"name": "clamp_scaleshift", "bias_term": False}
        ...
        input_as_const(
            fc_output, fc_output_attrs, 1, "weights", node.projection_weights
        )

        #                   / (2)Memory(output)
        # (3)FullyConnected
        #                   \ Output (any next node) (edge created automatically after replacement)
        next_lstm_output = Assign(
            graph, {"name": "next_lstm_output", "variable_id": memory_pair_input}
        ).create_node()
        next_lstm_output.in_port(0).connect(fc_output.out_port(0))

        res_node_lstm_output = Result(
            graph, {"name": "next_lstm_output_out"}
        ).create_node()
