...
class ReplaceDropoutMaskPattern(FrontReplacementPattern):
    ...
    def run_before(self):
        from openvino.tools.mo.front.kaldi.replace_lstm_nonlinearity import (
            ReplaceLstmNonLinearityPattern,
        )
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for dropout_node in replace_nodes:
            assert dropout_node.has_and_set(
                "size"
            ), "DropoutMaskComponent has not set size attribute"
            assert (
                dropout_node.size > 0
            ), "DropoutMaskComponent has negative or zero size attribute"
            assert dropout_node.has_and_set(
                "dropout_proportion"
            ), "DropoutMaskComponent has not set dropout_proportion attribute"
            assert (
                dropout_node.dropout_proportion > 0
            ), "DropoutMaskComponent has negative or zero dropout_proportion attribute"
            dp_const_node = create_const_with_batch_from_input(
                batch_port, dropout_node.size, dropout_node.dropout_proportion
            )
            dropout_node.out_port(0).get_connection().set_source(
                dp_const_node.out_port(0)
            )
