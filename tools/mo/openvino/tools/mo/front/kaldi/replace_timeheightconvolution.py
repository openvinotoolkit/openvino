...
class ReplaceTimeHeightConvolutionPattern(FrontReplacementPattern):
    ...
    def run_before(self):
        from openvino.tools.mo.front.kaldi.add_reshape_transpose_around_conv_pool import (
            AddReshapeTransposeAroundConvPool,
        )
    ...
    def replace_timeheightconv(self, graph: Graph, node: Node):
        ...
        for t in all_time_offsets:
            ...
            memoff = MemoryOffset(
                graph,
                attrs={
                    "name": in_name + "/MemoryOffset_" + str(i),
                    "t": t,
                    "has_default": has_default,
                    "splitted": False,
                    "pair_name": in_name + "/MemoryOffset_pair_" + str(i),
                },
            ).create_node()
        ...
        pad_h[1] = stride * node.height_out - (
            node.height_in - max([max(offsets[:, 1]), 0])
        )

        dilation_t = (
            (max(offsets[:, 0]) - min(offsets[:, 0])) / (kernel[0] - 1)
            if kernel[0] > 1
            else 1
        )
        dilation_h = (
            (max(offsets[:, 1]) - min(offsets[:, 1])) / (kernel[1] - 1)
            if kernel[0] > 1
            else 1
        )
