...
class RestrictedAttentionComponentReplacer(FrontReplacementPattern):
    ...
    *MemoryOffsets are described in the create_memory_offsets_subgraph method.
    ...
    def create_memory_offsets_subgraph(
        self, graph: Graph, input_node: Node, out_port, mem_offset_idx
    ):
        r"""
        ...
        """
        concat_node = Concat(
            graph, attrs={"name": self.in_name + f"/Concat_tmp_{mem_offset_idx}"}
        ).create_node()

        for idx, t in enumerate(
            list(
                range(
                    -self.time_stride * self.num_left_inputs,
    ...
    def replace_restrictedattention(self, graph: Graph, restrictedattention_node: Node):
        ...
        self.in_name = restrictedattention_node.soft_get(
            "name", restrictedattention_node.id
        )

        reshape_1_node = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([batch_num * num_heads, -1]),
            {"name": self.in_name + "/Reshape_1"},
        )
        restrictedattention_node.in_port(0).get_source().connect(
            reshape_1_node.in_port(0)
        )

        split_1_node = create_op_with_const_inputs(
            graph,
            VariadicSplit,
            {
                1: int64_array(1),
                2: int64_array([key_dim, value_dim, key_dim + context_dim]),
            },
            {"name": self.in_name + "/VariadicSplit_1", "out_ports_count": 3},
        )
        ...
        split_2_node = create_op_with_const_inputs(
            graph,
            VariadicSplit,
            {1: int64_array(1), 2: int64_array([key_dim, context_dim])},
            {"name": self.in_name + "/VariadicSplit_2", "out_ports_count": 2},
        )
        split_1_node.out_port(2).connect(split_2_node.in_port(0))

        einsum_1_node = Einsum(
            graph,
            {
                "name": self.in_name + "/Einsum_1",
        ...
        reshape_helper_1_node = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([num_heads, 1]),
            {"name": self.in_name + "/Reshape_helper_1"},
        )
        ...
        mul_node = create_op_with_const_inputs(
            graph,
            Mul,
            {1: mo_array(key_scale, dtype=float)},
        ...
        softmax_node = Softmax(
            graph, {"axis": 1, "name": self.in_name + "/Softmax"}
        ).create_node()
        ...
        reshape_helper_2_node = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([num_heads, value_dim, context_dim]),
            {"name": self.in_name + "/Reshape_helper_2"},
        )
        concat_2_node.out_port(0).connect(reshape_helper_2_node.in_port(0))

        reshape_helper_3_node = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([num_heads, 1, context_dim]),
            {"name": self.in_name + "/Reshape_helper_3"},
        )

        einsum_2_node = Einsum(
            graph,
            {
                "name": self.in_name + "/Einsum_2",
                "in_ports_count": 2,
                "equation": "ijk,ilk->ij",
            },
        ).create_node()
        ...
        concat_3_node = Concat(
            graph, {"name": self.in_name + "/Concat_2", "in_ports_count": 2}
        ).create_node()
        einsum_2_node.out_port(0).connect(concat_3_node.in_port(0))
        softmax_node.out_port(0).connect(concat_3_node.in_port(1))

        reshape_2_node = create_op_node_with_second_input(
            graph,
            Reshape,
        ...
        restrictedattention_node.out_port(0).get_connection().set_source(
            reshape_2_node.out_port(0)
        )
