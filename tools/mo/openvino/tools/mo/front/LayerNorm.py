...
class LayerNorm(FrontReplacementPattern):
    ...
    def replace_layer_norm(self, graph: Graph, match: dict):
        ...
        if (
            ...
            and pow_param.value.size == 1
            and add_param.value.item() <= 1e-05
            and pow_param.value.item() == 0.5
            and match["pool0_param"].value == match["pool1_param"].value
        ):
            log.debug(
                "Found LayerNorm pattern after {} with name {}".format(
