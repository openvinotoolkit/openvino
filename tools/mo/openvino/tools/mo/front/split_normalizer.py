...
class AttributedSplitToSplit(FrontReplacementSubgraph):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in graph.get_op_nodes(op="AttributedSplit"):
            ...
            assert (
                axis is not None
            ), "AttributedSplit should have `axis` parameter set, but it`s not for node {}".format(
                name
            )
            ...
            ), "AttributedSplit should have `num_splits` parameter set, but it`s not for node {}".format(
                name
            )
...
class AttributedVariadicSplitToVariadicSplit(FrontReplacementSubgraph):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in graph.get_op_nodes(op="AttributedVariadicSplit"):
            ...
            assert (
                axis is not None
            ), "AttributedVariadicSplit should have `axis` parameter set, but it`s not for node {}".format(
                name
            )

            size_splits = node.soft_get("size_splits", None)
            assert (
                size_splits is not None
            ), "AttributedVariadicSplit should have `size_splits` parameter set, but it`s not for node {}".format(
                name
            )
...
class VariadicSplitInputsSwap(FrontReplacementSubgraph):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in graph.get_op_nodes(
            op="VariadicSplit", swap_axis_and_split_size_inputs=True
        ):
