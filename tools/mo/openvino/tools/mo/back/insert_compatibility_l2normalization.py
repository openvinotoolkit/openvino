...
class CompatibilityL2NormalizationPattern(BackReplacementPattern):
    ...
    def pattern(self):
        return dict(nodes=[("l2_normalization", dict(op="Normalize"))], edges=[])
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if len(l2_normalization_node.in_nodes()) < 2:
            value = np.full(
                [l2_normalization_node.in_node(0).shape[1]], 1.0, dtype=np.float32
            )
            weights_node = Const(
                graph,
                dict(name=l2_normalization_node["name"] + "_weights", value=value),
            ).create_node()
