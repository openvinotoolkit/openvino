...
class ShuffleChannelPatternOptimization(BackReplacementPattern):
            ...
            nodes=[
                ...
                (
                    "t_start_order_d",
                    {
                        ...
                        and np.all(np.array_equal(v, [0, 2, 3, 1]))
                    },
                ),
                ...
                (
                    ...
                    {
                        "value": lambda v: v is not None
                        and np.all(np.array_equal(v, [0, 1, 2, 4, 3]))
                    },
                ),
                ("t_5d", {"type": "Transpose"}),
                ("t_5d_d", {}),
                ...
                (
                    ...
                    {
                        "value": lambda v: v is not None
                        and np.all(np.array_equal(v, [0, 3, 1, 2]))
                    },
                ),
    ...
    def feature_dim_splitted(short_shape, long_shape):
        return (
            all([short_shape[i] == long_shape[i] for i in range(len(short_shape) - 1)])
            and short_shape[-1] == long_shape[-1] * long_shape[-2]
        )
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if not ShuffleChannelPatternOptimization.feature_dim_splitted(
            short_shape=reshape_5d.in_port(0).data.get_shape(),
        ...
        if not ShuffleChannelPatternOptimization.feature_dim_splitted(
            short_shape=reshape_4d.out_port(0).data.get_shape(),
        ...
        match["reshape_dim"]["value"] = int64_array(
            np.take(new_start.in_port(1).data.get_value(), [0, 3, 4, 1, 2])
        )
        ...
        match["reshape_1_dim"]["value"] = int64_array(
            np.take(new_end.in_port(1).data.get_value(), [0, 3, 1, 2])
        )
...
class ShuffleChannelFusion(BackReplacementPattern):
        ...
        return dict(
            nodes=[
                ...
                (
                    "reshape_0_pattern_d",
                    dict(
                        value=lambda v: v is not None and v.size == 5 and np.all(v > 0)
                    ),
                ),
                ...
                (
                    ...
                    dict(
                        value=lambda v: v is not None
                        and np.array_equal([0, 2, 1, 3, 4], v)
                    ),
                ),
                ...
                (
                    "reshape_1_pattern_d",
                    dict(
                        value=lambda v: v is not None and v.size == 4 and np.all(v > 0)
                    ),
                ),
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if not all(
            [
                initial_shape[i] == channel_splitted_out_shape[j]
                for i, j in {0: 0, 2: 3, 3: 4}.items()
            ]
        ):
            return
        ...
        shuffle_channel = ShuffleChannels(
            graph, {"name": name, "group": group}
        ).create_node()
        channel_concating_reshape.out_port(0).get_connection().set_source(
            shuffle_channel.out_port(0)
        )
        shuffle_channel.in_port(0).connect(
        ...
        return dict(
            nodes=[
                ...
                (
                    "reshape_0_pattern_d",
                    dict(
                        value=lambda v: v is not None and v.size == 6 and np.all(v > 0)
                    ),
                ),
                ("reshape_0", dict(type="Reshape")),
                ("reshape_0_d", dict()),
                ...
                (
                    ...
                    dict(
                        value=lambda v: v is not None
                        and np.array_equal([0, 1, 4, 2, 5, 3], v)
                    ),
                ),
                ("transpose", dict(type="Transpose")),
                ("transpose_d", {}),
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if (
            not all(
                [
                    initial_shape[i] == channel_splitted_out_shape[j]
                    for i, j in {0: 0, 2: 4, 3: 5}.items()
                ]
            )
            or channel_splitted_out_shape[1] != channel_splitted_out_shape[2]
        ):
            return
        block_size = channel_splitted_out_shape[2]
        expected_output_shape = [
            initial_shape[0],
            initial_shape[1] // (block_size * block_size),
            initial_shape[2] * block_size,
            initial_shape[3] * block_size,
        ]
        if not np.array_equal(expected_output_shape, resulting_shape):
            return

        name = channel_concating_reshape.soft_get("name", channel_concating_reshape.id)
        depth_to_space = DepthToSpaceOp(
            graph, {"name": name, "block_size": block_size, "mode": "depth_first"}
        ).create_node()
        channel_concating_reshape.out_port(0).get_connection().set_source(
