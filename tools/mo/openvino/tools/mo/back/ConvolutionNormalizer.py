...
def resolve_convolution_with_group(node: Node, group: int, ir_version: str):
    ...
    if ir_version == "V7":
        ...
        wshape = Shape(
            node.graph, {"name": node_name + "/WeightsShape", "allow_fold": True}
        ).create_node()
        ...
        GOI = Const(
            node.graph,
            {
                "value": int64_array([group, node.output // group, -1]),
                "name": node_name + "/GOI_weights_part",
            },
        ).create_node()
        XY = create_op_with_const_inputs(
            node.graph,
            Gather,
            port_value_dict={
                1: int64_array(list(range(2, len(weights_shape)))),
                2: int64_array(0),
            },
        ...
        new_shape_node = Concat(
            node.graph,
            {"axis": 0, "in_ports_count": 2, "name": node_name + "/weights_shape"},
        ).create_node()
        new_shape_node.in_port(0).connect(GOI.out_port(0))
        new_shape_node.in_port(1).connect(XY.out_port(0))
        reshape = Reshape(
            node.graph, {"override_output_shape": True, "special_zero": True}
        ).create_node()
...
class ConvolutionNormalizer(BackReplacementPattern):
    ...
    def pattern(self):
        return dict(nodes=[("node", dict(kind="op", type="Convolution"))], edges=[])
...
class ConvolutionWithGroupsResolver(BackReplacementPattern):
    ...
    def run_before(self):
        ...
        from openvino.tools.mo.back.StridedSliceMasksNormalizer import (
...
class PullReshapeThroughFQ(BackReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        rank_reshape = (
            reshape.in_port(0).data.get_shape().size
            != reshape.out_port(0).data.get_shape().size
        )
...
class DeconvolutionNormalizer(BackReplacementPattern):
    ...
    @staticmethod
    def pattern():
        return dict(nodes=[("node", dict(type="Deconvolution"))], edges=[])
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if group != 1:
            ...
            assert (
                not is_fully_defined(new_shape)
                or not is_fully_defined(weights_shape)
                or np.prod(weights_shape) == np.prod(new_shape)
            ), "Initial weights shape {}, grouped weights shape {}" "".format(
                weights_shape, new_shape
            )
            reshape = create_op_node_with_second_input(
                graph,
                Reshape,
