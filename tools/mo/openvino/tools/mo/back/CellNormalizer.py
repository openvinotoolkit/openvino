...
class CellNormalizer(BackReplacementPattern):
    ...
    def pattern(self):
        return dict(
            nodes=[
                (
                    "cell",
                    dict(type=lambda type: type in ["LSTMCell", "GRUCell", "RNNCell"]),
                )
            ],
            edges=[],
        )
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        assert (
            WR_shape is not None
        ), "Undefined 'WR' input shape for Cell node '{}'".format(cell_name)
        assert is_fully_defined(
            WR_shape
        ), 'Not fully defined shape for WR for Cell node "{}"'.format(cell_name)

        num_elements_in_WR = np.prod(WR_shape)
        input_size = (
            num_elements_in_WR / (hidden_size_coef * hidden_size)
        ) - hidden_size
        ...
        reshape = create_op_node_with_second_input(
            graph,
            Reshape,
            int64_array([hidden_size_coef * hidden_size, hidden_size + input_size]),
            {"name": cell_name + "/Dims"},
        )
        ...
        split = create_op_with_const_inputs(
            graph,
            VariadicSplit,
            {1: int64_array(1), 2: int64_array([input_size, hidden_size])},
