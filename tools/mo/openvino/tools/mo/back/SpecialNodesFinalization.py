...
class CreateConstNodesReplacement(BackReplacementPattern):
    ...
    @staticmethod
    def pattern():
        return dict(nodes=[("data", dict(kind="data"))], edges=[])
    ...
    def _check_that_node_from_body(node):
        ...
        internal_port_in_out_ports = [
            "internal_port_id" in edge for edge in node.out_edges()
        ]
        ...
        if self._check_bin_attrs(node):
            if node.has_valid("value"):
                ...
                graph.add_edges_from(
                    [(node_copy.id, const_node_name, {"in": 0, "bin": "custom"})]
                )
...
class NormalizeTI(BackReplacementPattern):
    ...
    def maps_uniqueization(ti):
        ...
        ti.input_port_map = [
            dict(unique_r)
            for unique_r in set([tuple(rec.items()) for rec in ti.input_port_map])
        ]
        ti.output_port_map = [
            dict(unique_r)
            for unique_r in set([tuple(rec.items()) for rec in ti.output_port_map])
        ]
        ti.back_edges = [
            dict(unique_rec)
            for unique_rec in set([tuple(rec.items()) for rec in ti.back_edges])
        ]
    ...
    def external_nodes_normalization(ti):
        ...
        def update_external_port_id(
            ti, port_type, old_external_port_id, new_external_port_id, internal_layer_id
        ):
            ...
            for record in port_map:
                if (
                    record["external_port_id"] == old_external_port_id
                    and record["internal_layer_id"] == internal_layer_id
                ):
        ...
        for ti in graph.get_op_nodes(type="TensorIterator"):
            ...
            if (
                ...
                for record in ti.output_port_map:
                    if record.get("axis") is not None:
                        record["start"] = 0
                        real_output_port = TensorIterator.special_port_to_real_port(
                            ti, record["external_port_id"], "out"
                        )
