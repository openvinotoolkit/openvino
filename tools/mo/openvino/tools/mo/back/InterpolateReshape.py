...
class InterpolateConcat(BackReplacementPattern):
    ...
    def make_interpolate_reshapeable(interpolate, concat):
        ...
        concat_srcs = [
            port.get_source()
            for port in concat.in_ports().values()
            if not port.disconnected()
        ]
        non_interp_concat_srcs = [
            src for src in concat_srcs if src.node.soft_get("type") != "Interpolate"
        ]
...
class InterpolateReshapeWA(BackReplacementPattern):
    ...
    def make_interpolate_reshapeable(interpolate):
        ...
        output_shape = interpolate.out_port(0).data.get_shape()
        if not np.all(np.remainder(output_shape, input_shape) == 0) and not np.all(
            np.remainder(input_shape, output_shape) == 0
        ):
            return
        graph = interpolate.graph
        ...
        gather = create_op_with_const_inputs(
            graph,
            Gather,
            {1: mo_array(axes, dtype=np.int32), 2: int64_array(0)},
            {"name": shape.name + "/Gathered"},
            shape,
        )
        multipliers = output_shape[axes] / input_shape[axes]
        mul = create_op_node_with_second_input(
            graph, Mul, multipliers, {"name": gather.name + "/Multiplied"}, gather
        )
