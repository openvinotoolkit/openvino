...
class CropToStridedSlice(BackReplacementPattern):
    ...
    @staticmethod
    def pattern():
        return dict(nodes=[("crop", dict(op="Crop"))], edges=[])
    ...
    def replace_pattern(self, graph: Graph, match: [str, Node]):
        ...
        if len(node.in_nodes()) == 2 and node.has_valid("offset"):
            ...
            begin = Const(
                graph,
                {
                    "value": self.mask_normalizer(shape_rank, node_axis, node.offset),
                    "name": ss.name + "/begin",
                },
            ).create_node()
            ...
            begin = Const(
                graph,
                {
                    "value": self.mask_normalizer(shape_rank, node_axis, node_offset),
                    "name": ss.name + "/begin",
                },
            ).create_node()
            end_values = mo_array(
                ...
                graph,
                {
                    "value": self.mask_normalizer(shape_rank, node_axis, end_values),
                    "name": ss.name + "/end",
                },
            ).create_node()
            ...
            begin = Const(
                graph,
                {
                    "value": self.mask_normalizer(
                        shape_rank, node_axis, node_crop_begin
                    ),
                    "name": ss.name + "/begin",
                },
            ).create_node()
        ...
        stride = Const(
            graph,
            {"value": np.ones(shape_rank, dtype=np.int64), "name": ss.name + "/stride"},
        ).create_node()
