...
def serialize_constants_recursively(graph: Graph, bin_file, data_type, bin_hashes):
    ...
    for node in nodes:
        ...
        if (
            ...
            and any("bin" in d for u, v, d in graph.out_edges(node.node, data=True))
        ):
            ...
            assert is_fully_defined(
                blob
            ), "The constant value cannot contain dynamic values"
            if isinstance(blob, np.ma.masked_array):
                blob = np.ma.getdata(blob)
            blob_hash = hashlib.sha512(
                np.ascontiguousarray(blob).view(np.uint8)
            ).hexdigest()
            ...
            if blob_hash in bin_hashes and np.array_equal(
                ...
                graph.node[node.node]["blob_precision"] = np_data_type_to_precision(
                    blob.dtype
                )
                ...
                assert (
                    blob.dtype.itemsize * np.prod(node.shape) == end - start
                ) or node.has_valid("force_shape"), node.attrs()
        ...
        if node.has_valid("sub_graphs"):
            for sub_graph_attr_name in node.sub_graphs:
                ...
                serialize_constants_recursively(
                    sub_graph, bin_file, data_type, bin_hashes
                )
...
def xml_shape(shape: np.ndarray, element: Element):
    for d in unmask_shape(shape):
        ...
        if not isinstance(d, np.int64):
            log.warning(
                'The element of shape is not np.int64 value. Converting the value "{}" to integer'.format(
                    d
                )
            )
            d = int(d)
...
def xml_ports(node: Node, element: Element, edges: Element):
    ...
    for u, d in node.get_sorted_inputs():
        if "bin" not in d and ("xml_skip" not in d or not d["xml_skip"]):
            ...
            assert (
                node.graph.node[u]["shape"] is not None
            ), "Input shape is not calculated properly for node {}".format(node.id)
            xml_shape(node.graph.node[u]["shape"], port)

            # support saving rt_info passed from IR Reader
            port_id = d["in"]
            if (
                node.has("restored_input_ports")
                and port_id in node.restored_input_ports
            ):
                ...
                if port_rt_info_value != {}:
                    ...
                    for (name, version), info_elem in port_rt_info_value.items():
                        ...
                        params = (
                            info_elem.serialize(node)
                            if not isinstance(info_elem, dict)
                            else info_elem
                        )
    ...
    for v, d in node.get_sorted_outputs():
        if "xml_skip" not in d or not d["xml_skip"]:
            ...
            port_id = (
                d["out"] - len(node.in_nodes()) if node.type != "Const" else d["out"]
            )
            data_type = node.out_port(port_id).get_data_type()
            assert data_type is not None, (
                "The precision is not defined for the output port {} of node {}"
                "".format(port_id, node.soft_get("name"))
            )

            port.set(
                "precision",
                node.soft_get("force_type", np_data_type_to_precision(data_type)),
            )
            assert (
                node.graph.node[v]["shape"] is not None
            ), "Output shape is not calculated properly for node {}" "".format(node.id)
    ...
    for u, d in node.get_sorted_inputs():
        if "bin" in d and (node.type != "Const"):
            ...
            try:
                ...
                raise Error(
                    'Unable to access binary attributes ("offset" and/or "size") for blobs for node {}. '
...
def soft_get(node, attr):
    ...
    return (
        node.soft_get(attr)
        if hasattr(node, "soft_get") and callable(node.soft_get)
        else "<SUB-ELEMENT>"
    )
    ...
    for attr in attrs:
        if isinstance(attr, tuple):
            ...
            try:
                ...
                raise Error(
                    "Unable to extract {} from layer {}", key, soft_get(node, "name")
                ) from e
            except Exception as e:
                raise Error(
...
def serialize_runtime_info(node, parent_element: Element):
    ...
    for (name, version), info_elem in node.rt_info.info.items():
        ...
        attribute.set("version", str(version))
        params = (
            info_elem.serialize(node) if not isinstance(info_elem, dict) else info_elem
        )
        for key, value in params.items():
            attribute.set(key, value)
...
def serialize_node_attributes(
    ...
    if (
        ...
        and node.soft_get("type") == "Result"
        and node.has_and_set("keep_output_port")
    ):
        return
    try:
        for s in schema:
            if not isinstance(s, tuple):
                if s == "@ports":
                    try:
                        ...
                        raise Error(
                            (
                                "Unable to create ports for node with id {}. "
                                + refer_to_faq_msg(3)
                            ).format(node.id)
                        ) from e
                elif s == "@consts":
...
def create_pre_process_block_for_image(
    net: Element, ref_layer_names: list, mean_offset: tuple, mean_size: tuple
):
...
def serialize_node(
    ...
    try:
        ...
        raise Error(
            str(e).replace(
                "<SUB-ELEMENT>", "{} (id = {})".format(node.soft_get("name"), node.id)
            )
        ) from e
...
def get_tensor_names_of_result_node(graph):
    ...
    for res_node in result_nodes:
        ...
        assert (
            len(res_node.in_nodes()) > 0
        ), "Result node with name {} has no input node.".format(
            res_node.soft_get("name")
        )
        res_data_node = res_node.in_node(0)
        assert (
            len(res_data_node.in_nodes()) > 0
        ), "Data node of Result with name {} has no input node.".format(
            res_node.soft_get("name")
        )
        ...
        for v, d in res_in_node.get_sorted_outputs():
            port_id = (
                d["out"] - len(res_in_node.in_nodes())
                if res_in_node.type != "Const"
                else d["out"]
            )
            tensor_names = res_in_node.out_port(port_id).get_tensor_names(
                port_renumber=True
            )
...
def check_and_add_result_name(result_name: str, ordered_results: list):
    if result_name in ordered_results:
        log.warning(
            "Result node with name {} has at least two tensor names corresponding "
...
def serialize_network(graph, net_element, unsupported):
    ...
    for output_name in graph.outputs_order:
        ...
        if len(node) == 0:
            ...
            found_result_name = find_result_node_by_name(
                output_name, result_nodes, result_names_to_tensor_names
            )
            ...
            if found_result_name is not None:
                ...
                log.warning(
                    "Output node with name {} is not found in graph.".format(
                        output_name
                    )
                )
            continue
        ...
        assert len(node.out_nodes()) >= 1, (
            "Incorrect graph. Non-Result node with name {} "
        ...
        if not found_result:
            log.warning(
                "Node that expected to be output with name {} is not connected with Result node.".format(
                    output_name
                )
            )
    ...
    for input_name in graph.inputs_order:
        ...
        if not found_tensor_name:
            log.warning(
                "Input node with name {} is not found in graph.".format(param_name)
            )
    ...
    for output_name in ordered_results:
        node = graph.get_op_nodes(name=output_name)
        assert len(node) == 1, "Output node with name {} is not found in graph.".format(
            output_name
        )
        serialize_node(graph, node[0], layers, edges, unsupported)
...
def generate_ie_ir(
    ...
    if len(unsupported.unsupported):
        ...
        unsupported.report(
            log.error, "List of operations that cannot be converted to OpenVINO IR:"
        )
        raise Error(
            "Part of the nodes was not converted to IR. Stopped. "
            + refer_to_faq_msg(24)
        )
