...
class ReverseChannelsPropagationDown(BackReplacementPattern):
    ...
    propagation_rules = {
        "Convolution": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_conv(
            node, rc
        ),
        "ScaleShift": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_eltwise(
            node, rc
        ),
        "Power": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_eltwise(
            node, rc
        ),
        "BatchNormalization": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_eltwise(
            node, rc
        ),
        "FakeQuantize": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_eltwise(
            node, rc
        ),
        "Multiply": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_eltwise(
            node, rc
        ),
        "Divide": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_eltwise(
            node, rc
        ),
        "Add": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_eltwise(
            node, rc
        ),
        "Subtract": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_eltwise(
            node, rc
        ),
        "Pow": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_eltwise(
            node, rc
        ),
        "Convert": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_eltwise(
            node, rc
        ),
        "Shape": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_shape(
            node, rc
        ),
        "ShapeOf": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_shape(
            node, rc
        ),
        "Pad": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_zero_port_only(
            node, rc
        ),
        "Transpose": lambda node, rc: ReverseChannelsPropagationDown.pass_rc_through_transpose(
            node, rc
        ),
    }
    ...
    def pass_rc_through_transpose(node: Node, reverse_channels: Node):
        ...
        assert (
            0 < reverse_axis < data_rank
        ), "Incorrect ReverseChannels axis in node {}.".format(reverse_channels)

        if order is None:
            return False
        new_axis = list(order).index(reverse_axis)
        reverse_channels.axis = int64_array(new_axis)
        return ReverseChannelsPropagationDown.pass_rc_through_zero_port_only(
    ...
    def pass_rc_through_zero_port_only(node: Node, reverse_channels: Node):
        ...
        if (
            reverse_channels.is_in_port_connected(0)
            and reverse_channels.is_out_port_connected(0)
    ...
    def pass_rc_through_conv(node, reverse_channels):
        ...
        if (
            weights_shape is None
            or weights_shape[channel_idx] != reverse_channels.order.size
        ):
        ...
        ric_to_move_to_weights = (
            reverse_channels if group == 1 else reverse_channels.copy_node()
        )
        ...
        if group != 1 and group == reverse_channels.order.size:
            ...
            new_order = np.take(
                np.arange(bottom_channels).reshape((group, multiplier)),
                indices=reverse_channels.order,
                axis=0,
            ).flatten()
    ...
    def pass_rc_through_eltwise(node, reverse_channels):
        ...
        for idx, port in node.in_ports().items():
            ...
            if (
                len(non_one_dims) == 1
                and shape[non_one_dims.item()] == reverse_channels.order.size
            ):
        ...
        for port, axis in port_axis:
            ric_copy = reverse_channels.copy_node(
                {"axis": mo_array(axis), "order": mo_array(reverse_channels.order)}
            )
    ...
    def get_non_shape_taking_dst(dsts):
        return [
            dst for dst in dsts if dst.node.soft_get("type") not in ["Shape", "ShapeOf"]
        ]

    def check_if_we_propagate_down(self, reverse_channels):
        dsts = self.get_non_shape_taking_dst(
            reverse_channels.out_port(0).get_destinations()
        )
        return (
            len(dsts) == 1 and dsts[0].node.soft_get("type") in self.propagation_rules
        )

    def find_and_replace_pattern(self, graph: Graph):
        for reverse_channels in graph.get_op_nodes(op="ReverseChannels"):
            keep_moving_down = True
            while keep_moving_down and self.check_if_we_propagate_down(
                reverse_channels
            ):
                next_node = self.get_non_shape_taking_dst(
                    reverse_channels.out_port(0).get_destinations()
                )[0].node
                keep_moving_down = self.propagation_rules[next_node.type](
                    next_node, reverse_channels
                )
...
class ReverseChannelsPropagationUp(BackReplacementPattern):
    ...
    propagation_rules = {
        "ScaleShift": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(
            node, rc
        ),
        "Power": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(
            node, rc
        ),
        "BatchNormalization": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(
            node, rc
        ),
        "FakeQuantize": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(
            node, rc
        ),
        "Multiply": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(
            node, rc
        ),
        "Divide": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(
            node, rc
        ),
        "Add": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(
            node, rc
        ),
        "Subtract": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(
            node, rc
        ),
        "Pow": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(
            node, rc
        ),
        "Convert": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_eltwise(
            node, rc
        ),
        "Pad": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_zero_port_only(
            node, rc
        ),
        "Transpose": lambda node, rc: ReverseChannelsPropagationUp.lift_up_through_transpose(
            node, rc
        ),
    }
    ...
    def lift_up_through_transpose(node: Node, reverse_channels: Node):
        ...
        reverse_channels.axis = int64_array(new_axis)
        return ReverseChannelsPropagationUp.lift_up_through_zero_port_only(
            node, reverse_channels
        )
    ...
    def lift_up_through_zero_port_only(node: Node, reverse_channels: Node):
        ...
        if node.is_in_port_connected(0):
            ...
            if src.node.soft_get("type") == "Parameter":
                ...
                node_input_port_0.get_connection().set_source(
                    reverse_channels.out_port(0), attributes_save_mode="source"
                )
            else:
                node_input_port_0.get_connection().set_source(
            ...
            for reverse_channels_destination in reverse_channels_out_nodes:
                node.out_port(0).get_connection().add_destination(
                    reverse_channels_destination
                )
        ...
        for port, axis in port_axis:
            ...
            if src.node.soft_get("type") == "Parameter":
                ...
                port.get_connection().set_source(
                    reverse_channels_copy.out_port(0), attributes_save_mode="source"
                )
            else:
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        while len(reverse_channels):
            ...
            while keep_moving_up:
                ...
                if (
                    curr_reverse_channels.in_port(0).get_source().node.soft_get("type")
                    not in self.propagation_rules
                ):
                    break
                next_op = curr_reverse_channels.in_port(0).get_source().node
                keep_moving_up, new_reverses = self.propagation_rules[next_op.type](
                    next_op, curr_reverse_channels
                )
...
class DecomposeReverseChannels(BackReplacementPattern):
    ...
    def replace_with_gather(node):
        ...
        gather = create_op_with_const_inputs(
            graph, Gather, {1: order, 2: int64_array(axis)}, {"name": name}
        )
    ...
    def replace_with_split_concat(node):
        ...
        concat = Concat(
            graph,
            {"name": name + "/Concat", "axis": axis, "in_ports_count": order.size},
        ).create_node()
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for reverse_channels in graph.get_op_nodes(op="ReverseChannels"):
            if (
                reverse_channels.in_port(0).disconnected()
                or reverse_channels.out_port(0).disconnected()
            ):
