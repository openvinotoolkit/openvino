...
from openvino.tools.mo.back.InterpolateReshape import (
    InterpolateConcat,
...
class LinearToLinearONNXReplacer(BackReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for interpolate_node in graph.get_op_nodes(
            type="Interpolate", version="opset4", mode="linear"
        ):
            ...
            ), "Shape of interpolated data for node {} must not be None".format(
                interpolate_name
            )
