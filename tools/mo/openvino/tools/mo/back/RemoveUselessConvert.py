...
class RemoveUselessConvert(BackReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for cast_node in graph.get_op_nodes(op="Cast"):
            if (
                cast_node.in_port(0).get_data_type()
                == cast_node.out_port(0).get_data_type()
            ):
                log.debug(
                    "Convert node {} do not change the data type of the input data.".format(
                        cast_node.name
                    )
                )
                cast_node.out_port(0).get_connection().set_source(
                    cast_node.in_port(0).get_connection().get_source()
                )
