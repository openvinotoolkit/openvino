...
class RemoveLastSoftMaxPattern(BackReplacementPattern):
    ...
    graph_condition = [
        ...
        and graph.graph["cmd_params"].remove_output_softmax
    ]
    ...
    def pattern():
        return dict(
            ...
            edges=[("softmax_node", "softmax_data"), ("softmax_data", "op_output")],
        )
...
class RemoveLastLogSoftMaxPattern(BackReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        second_input_of_reduce_max = (
            reduce_max_node.in_port(1).get_connection().get_source().node
        )
        if (
            not second_input_of_reduce_max.has_valid("value")
            or len(second_input_of_reduce_max.value) != 1
        ):
            return

        reduce_sum_node = match["reduce_sum_node"]
        second_input_of_reduce_sum = (
            reduce_sum_node.in_port(1).get_connection().get_source().node
        )
        if (
            not second_input_of_reduce_sum.has_valid("value")
            or len(second_input_of_reduce_sum.value) != 1
        ):
            return
        ...
        for (
            name,
            number,
        ) in RemoveLastLogSoftMaxPattern.expected_number_of_outputs.items():
            if len(match[name].out_port(0).get_destinations()) != number:
                return

        match["op_output"].in_port(0).get_connection().set_source(
            match["sub_node"].in_port(0).get_source()
        )
