        ...
        for node in graph.get_op_nodes():
            if node.op in operations_with_data_type_attributes:
                ...
                assert node.has_valid(
                    dst_type
                ), "{} attribute is missing for node {}".format(dst_type, node_name)
                ...
                if (
                    node[dst_type] in [np.float32, np.float64]
                    and ir_data_type == np.float16
                    and not node.has_and_set("returns_shape_value")
                ):
                    final_type = np.float16
                elif (
                    node.has_and_set("returns_shape_value")
...
def assert_that_is_castable_to_fp16(node: Node):
    ...
    for i in operations_with_data_type_attributes[op_name]["in_ports_to_check"]:
        ...
        if np.any(val > np.finfo(np.float16).max) or np.any(
            ...
            raise Error(
                ...
                "'{}' node value {} exceeds FP16 allowed limits: [{}, {}]".format(
                    node_name, val, np.finfo(np.float16).min, np.finfo(np.float16).max
                )
            )
    ...
    original_output_len = (
        original_output.size if hasattr(original_output, "size") else None
    )
    ...
    if original_output_len != casted_output_len:
        raise Error(
            "Try to convert with --data_type=FP32 argument. "
            "This model can not be converted to FP16 precision, since "
            "after conversion of '{}' node to FP16 output shape {} differs from the original {}.".format(
                node_name, casted_output_len, original_output_len
            )
        )

    diff_count = np.count_nonzero(np.subtract(original_output, casted_output) > 1.0e-4)
    if diff_count > 0:
        log.warning(
