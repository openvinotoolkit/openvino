...
class ShapeOfConstFolding(BackReplacementPattern):
    ...
    def run_after(self):
        from openvino.tools.mo.back.MatMulNormalizer import (
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for shapeof_node in graph.get_op_nodes(op="ShapeOf"):
            ...
            if in_node.op == "Const" or (
                shapeof_node.has_and_set("allow_fold")
                and is_fully_defined(shapeof_node.in_port(0).data.get_shape())
            ):
                ...
                shape_const_node = Const(
                    graph,
                    {"name": shape_name + "/ExecutionConstValue", "value": shape_value},
                ).create_node()
                shapeof_node.out_port(0).get_connection().set_source(
                    shape_const_node.out_port(0)
                )
                rename_nodes(
                    [
                        (shapeof_node, shape_name + "/TBD"),
                        (shape_const_node, shape_name),
                    ]
                )
