...
class UselessConcatRemoval(BackReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        connected_ports = [
            port for port in concat_node.in_ports().values() if not port.disconnected()
        ]
        if len(connected_ports) == 1:
            log.debug(
                "Concat node {} has just one input. Removing it.".format(
                    concat_node.name
                )
            )
            concat_node.out_port(0).get_connection().set_source(
                connected_ports[0].get_connection().get_source()
            )
