...
class MatMulConstTransposesExtraction(BackReplacementPattern):
    ...
    @staticmethod
    def pattern():
        return dict(nodes=[("matmul", dict(kind="op", op="MatMul"))], edges=[])
    ...
    def insert_transpose(node, in_port_idx):
        ...
        assert (
            in_port_idx in node.in_ports()
            and not node.in_port(in_port_idx).disconnected()
        ), "Input port with index {} should be connected for node {}".format(
            in_port_idx, name
        )
        ...
        assert (
            port_shape is not None
        ), "Shape is unknown for input port with index {} for node {}".format(
            in_port_idx, name
        )

        transpose_order = list(range(port_shape.size))
        transpose_order[-1], transpose_order[-2] = (
            transpose_order[-2],
            transpose_order[-1],
        )
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if not node.has_and_set("transpose_b"):
            ...
            FQ_on_weights = (
                node.in_port(1).get_source().node.has_and_set("stop_value_propagation")
            )
            if (B_value is not None or FQ_on_weights) and B_shape[
                B_shape != 1
            ].size <= 2:
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        for idx, port in fq.in_ports().items():
            ...
            if idxs.size != 0:
                ...
                unsqueeze = Unsqueeze(
                    graph, {"name": name + "/in_{}_unsqueeze".format(idx)}
                ).create_node()
...
class SmartReshape_HC_Reshape_MatMul(BackReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        other_input_port_idx = (
            0
            if match["matmul"].in_port(0).get_source().node.id
            == match["other_input"].id
            else 1
        )
        ...
        N = Const(
            graph, {"name": shape_name + "/MinusOne", "value": int64_array([-1])}
        ).create_node()
