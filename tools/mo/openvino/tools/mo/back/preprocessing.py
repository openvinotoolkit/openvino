...
from openvino.runtime import Layout, Model, PartialShape, layout_helpers
    ...
    if not isinstance(mean_scale_val, dict):
        ...
        for idx, node in enumerate(input_nodes):
            ...
            mean_scale_val.update(
                {node_name: {"mean": data[idx][0], "scale": data[idx][1]}}
            )
            ...
            mean_scale_val.update(
                {
                    node_name: {
                        "mean": (
                            old_val["mean"] if old_val and "mean" in old_val else None
                        ),
                        "scale": scale,
                    }
                }
...
def guess_source_layouts_by_mean_scale(
    ov_function: Model, layout_values, mean_scale_values: dict
):
    """
    ...
    for ms_name, mean_scale in mean_scale_values.items():
        num_channels_mean = (
            len(mean_scale["mean"]) if mean_scale["mean"] is not None else 0
        )
        num_channels_scale = (
            len(mean_scale["scale"]) if hasattr(mean_scale["scale"], "__len__") else 0
        )

        if (
            num_channels_mean > 1
            and num_channels_scale > 1
            and num_channels_mean is not num_channels_scale
        ):
            raise Error(
        ...
        if (
            not need_guess_channels
        ):  # Mean/scale is complex and needs 'channels' specified in layout
            continue

        num_channels = (
            num_channels_mean if num_channels_mean > 1 else num_channels_scale
        )
...
def check_suitable_for_reverse(layout: Layout, ov_input):
    ...
    if c_idx >= rank:
        raise Error(
            "Layout {} for input {} is inconsistent with shape {}".format(
                layout,
                ov_input.get_tensor().get_any_name(),
                ov_input.get_partial_shape(),
            )
        )
...
def guess_source_layouts_for_reverse_channels(ov_function: Model, layout_values):
    ...
    for i in range(0, len(ov_function.inputs)):
        ...
        if not ov_function.get_parameters()[i].layout.empty:
            if check_suitable_for_reverse(
                ov_function.get_parameters()[i].layout, ov_input
            ):
        ...
        if layout_item is not None:
            ...
            if layout_item.get("source_layout"):
                if check_suitable_for_reverse(
                    Layout(layout_item["source_layout"]), ov_input
                ):
                    suitable_params.append(param_info)
            continue
    ...
    if not len(suitable_params):
        raise Error(
            "Network has {} inputs overall, but none of them are suitable for input channels reversing.\n"
        ...
        log.error(
            "Network has {} inputs overall, but only {} of them are suitable for input channels reversing.\n"
            "Suitable for input channel reversing inputs are 4-dimensional with 3 channels (in case of dynamic "
            "dimensions C channel must be provided in a layout for this input)\nAll inputs: {}\n"
            "Suitable inputs {}".format(
                len(all_params), len(suitable_params), all_params, suitable_params
            ),
...
def apply_preprocessing(ov_function: Model, argv: argparse.Namespace):
    ...
    mean_scale_values = update_tensor_names_to_first_in_sorted_list(
        mean_scale_values, ov_function
    )
    ...
    layout_values = guess_source_layouts_by_mean_scale(
        ov_function, layout_values, mean_scale_values
    )
    ...
    if need_reverse:
        for name, _ in suitable_params_ric:
            ...
            log.debug(
                "reverse_input_channels pre-processing applied to {}".format(name)
            )
