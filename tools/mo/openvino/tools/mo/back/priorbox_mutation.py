...
class PriorboxMutation(BackReplacementPattern):
        ...
        return dict(
            nodes=[
                (
                    "pb",
                    {
                        "type": lambda node_type: node_type
                        in ["PriorBox", "PriorBoxClustered"]
                    },
                )
            ],
            edges=[],
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        begin = Const(
            graph, {"value": mo_array([2], dtype=np.int32), "name": name + "/ss_begin"}
        ).create_node()
        end = Const(
            graph, {"value": mo_array([4], dtype=np.int32), "name": name + "/ss_end"}
        ).create_node()
        stride = Const(
            graph, {"value": mo_array([1], dtype=np.int32), "name": name + "/ss_stride"}
        ).create_node()
