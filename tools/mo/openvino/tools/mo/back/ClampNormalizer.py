...
from openvino.tools.mo.ops.elementwise import Maximum, Minimum
...
class ClampNormalizer(BackReplacementPattern):
    ...
    def pattern(self):
        return dict(nodes=[("clamp", dict(op="Clamp"))], edges=[])
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if (
            port_1_exist
            and clamp.in_port(1).get_source().node.soft_get("type") == "Const"
        ):
            min_value = clamp.in_port(1).data.get_value()
        if (
            port_2_exist
            and clamp.in_port(2).get_source().node.soft_get("type") == "Const"
        ):
        ...
        if min_value is None or max_value is None:
            ...
            assert (
                min_node is not None or max_node is not None
            ), "Clamp node should have either min or max input used"
            rename_node(min_node if min_node is not None else max_node, name)
        else:
            a_clamp = AttributedClamp(
                graph, {"name": name, "min": min_value, "max": max_value}
            ).create_node()
