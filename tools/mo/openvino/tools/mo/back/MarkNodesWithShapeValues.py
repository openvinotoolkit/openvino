...
from openvino.tools.mo.middle.MarkSubgraphsWithCorrectLayout import (
...
class MarkNodesWithShapeValues(BackReplacementPattern):
    ...
    @staticmethod
    def get_shape_returning_start_nodes(
        nodes_with_shape_inputs: List[Node],
    ) -> List[Node]:
        sources = []
        ...
        for node in shape_returning_nodes:
            ...
            if node.soft_get("type") == "Const":
                if node.value.dtype == np.float32:
                    ...
                    log.error(
                        "Const node '{}' returns shape values of '{}' type but it must be integer or float32. "
                        "During Elementwise type inference will attempt to cast to float32".format(
                            node.soft_get("name", node.id), node.value.dtype
                        ),
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        shape_returning_start_nodes = self.get_shape_returning_start_nodes(
            shape_accepting_nodes
        )
        shape_returning_nodes = MarkSubGraphsWithCorrectLayout.bfs(
            shape_returning_start_nodes, set(), condition, forward=False
        )
