...
class CutMemoryInput(BackReplacementPattern):
    ...
    graph_condition = [
        ...
        and graph.graph["cmd_params"].remove_memory
    ]
    force_clean_up = True

    @staticmethod
    def pattern():
        return dict(nodes=[("op", dict(kind="op", op="ReadValue"))], edges=[])
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        for dest in node.out_port(0).get_destinations():
            ...
            log.error(
                "Add input/output mapped {} -> {} ".format(
...
class CutMemoryOutput(BackReplacementPattern):
    ...
    @staticmethod
    def pattern():
        return dict(nodes=[("op", dict(kind="op", op="Assign"))], edges=[])
