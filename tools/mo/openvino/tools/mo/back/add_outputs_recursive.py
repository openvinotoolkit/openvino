...
def get_iterations_count_from_output_record(output_rec):
    ...
    if (
        check_field(output_rec, "end")
        and check_field(output_rec, "start")
        and (
            (output_rec["start"] >= 0 and output_rec["end"] >= 0)
            or (output_rec["start"] < 0 and output_rec["end"] < 0)
        )
    ):
...
def ti_infer(step_node, port_num):
    ...
    for record in out_port_map:
        if record["external_port_id"] == port_num:
            ...
            break
    assert (
        found_rec is not None
    ), "External port {} is not connected with body in node {}".format(
        port_num, step_node.soft_get("name", step_node.id)
    )
    ...
    ti_set_output_port_shape(
        step_node,
        found_rec["internal_layer_id"],
        port_num,
        iterations_count,
        found_rec["axis"],
    )
...
def max_internal_layer_id(graph):
    ...
    for n in graph.get_op_nodes():
        if (
            n.has_and_set("internal_layer_id")
            and n.internal_layer_id > max_int_layer_id
        ):
...
def add_output_in_body(
    node, port_num, cur_graph, cur_max_layer_id, tracks, track_index, add_unsqueeze=True
):
    ...
    if add_unsqueeze:
        ...
        unsq_node = create_op_node_with_second_input(
            cur_graph, Unsqueeze, int64_array([0]), {"name": unsq_name}
        )
...
class AddOutputRecursive(BackReplacementPattern):
    ...
    def add_output_for_path(graphs_nodes_path):
        ...
        for i in range(len(graphs_nodes_path) - 1, 0, -1):
            ...
            if cur_loop_node.op is not "If":
                ...
                for p_num in ports_to_add_nodes:
                    res_node, graphs_nodes_path, cur_max_layer_id = add_output_in_body(
                        step_node,
                        p_num,
                        cur_graph,
                        cur_max_layer_id,
                ...
                for p_num in ports_to_add_nodes:
                    ...
                    if cur_loop_node.then_graph == cur_graph:
                        ...
                        res_node["output_id"] = list(cur_loop_node.out_ports().keys())[
                            -1
                        ]
        ...
        for p_num in ports_to_add_nodes:
            ...
            if step_node.op == "TensorIterator":
                step_node.out_edges()[len(step_node.out_edges()) - 1][
                    "external_port_id"
                ] = p_num + len(step_node.in_ports())
            ...
            for p_num in step_node.out_ports():
                if not step_node.out_port(p_num).disconnected():
                    if step_node.op == "TensorIterator":
                        ...
                        out_shape = list(
                            step_node.in_port(0).get_source().data.get_shape()
                        )
        ...
        while len(lists_stack) != 0:
            ...
            while list_idx < len(cur_list):
                ...
                if isinstance(el, (list, np.ndarray)):
                    ...
                    paths_nodes_graphs[track_idx].append(
                        {"node": step_node, "graph": cur_graph}
                    )
                    ...
                    if list_idx != len(cur_list) - 1:
                        ...
                        if (
                            len(lists_stack) != 0
                            and lists_stack[-1]["pos"] != 0
                            and isinstance(
                                lists_stack[-1]["list"][lists_stack[-1]["pos"] - 1], str
                            )
                        ):
                            ...
                            assert step_node.has_and_set(
                                "sub_graphs"
                            ), "Node without sub-graphs is not last in path"
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for i in range(len(paths_nodes_graphs)):
            ...
            k = 0
            while (
                paths_nodes_graphs_old[i][0]["node"] != paths_nodes_graphs[i][k]["node"]
            ):
