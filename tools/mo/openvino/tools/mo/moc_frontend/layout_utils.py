...
def update_layout_to_dict(inputs: list, layout: [list, dict], get_names_func: Callable):
        ...
        if "" in layout:
            ...
            if len(input_names) > 1:
                raise Error(
                    ...
                    'Please specify explicitly input/output name for "layout" option'.format(
    ...
    if isinstance(layout, list):
        if len(layout) != len(inputs):
            raise Error(
                "Numbers of inputs and layout values do not match. "
                + refer_to_faq_msg(61)
            )
        layout_dict = {}
