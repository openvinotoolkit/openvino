...
from openvino.runtime.utils.types import (  # pylint: disable=no-name-in-module,import-error
    get_element_type,
    get_numpy_ctype,
)
...
from openvino.tools.mo.moc_frontend.extractor import (
    convert_params_lists_to_dicts,
    fe_output_user_data_repack,
    fe_user_data_repack,
)
from openvino.tools.mo.moc_frontend.layout_utils import (
    get_dimension_index_by_label,
    update_layout_to_dict,
)
...
def moc_pipeline(argv: argparse.Namespace, moc_front_end: FrontEnd):
    ...
    if argv.input_model and input_checkpoint:
        ...
        input_model = moc_front_end.load(
            [argv.input_model, argv.input_checkpoint], share_weights
        )
        ...
        if argv.saved_model_tags:
            input_model = moc_front_end.load(
                [argv.saved_model_dir, argv.saved_model_tags], share_weights
            )
        else:
        ...
        if argv.output:
            ...
            outputs = fe_output_user_data_repack(
                input_model, argv.output, moc_front_end.get_name()
            )
    ...
    def add_names_to_tensors(model: InputModel, places: List[Place]):
        ...
        for new_input in places:
            ...
            try:
                ...
                log.warning(
                    "Could not add an additional name to a tensor pointed to by '{}'. Details: {}".format(
    ...
    log.debug(
        "Inputs are same: {}, outputs are same: {}".format(inputs_equal, outputs_equal)
    )

    def create_target_input_shapes(new_input_places):
        if (
            isinstance(new_input_places, list)
            and len(new_input_places) > 1
    ...
    if user_shapes:
        for user_shape in user_shapes:
            if user_shape.get("shape") is not None:
                input_model.set_partial_shape(user_shape["node"], user_shape["shape"])
    ...
    if freeze_placeholder:
        for name, value in freeze_placeholder.items():
            ...
            for node_cur in user_shapes:
                if (
                    node_cur.get("input_name") == name
                    or node_cur.get("input_name") == name + ":0"
                ):
                    node = node_cur
                    break
            if node is None:
                raise Error(
                    "Please check correctness of the command-line. "
                    "Place (operation or tensor) with name {} is not found.".format(
                        name
                    )
                )
            place = node.get("node")
            ...
            if node.get("data_type"):
                ...
                try:
                    ...
                    raise Error(
                        "Please specify type for value freezing {} node explicitly "
                        "because the frontend does not support automatic type detection.".format(
                            name
                        )
                    )
    ...
    if argv.batch is not None and argv.batch > 0:
        ...
        frozen_input_names = (
            list(freeze_placeholder.keys()) if freeze_placeholder else []
        )
        for place in model_inputs:
            ...
            is_frozen_input = (
                len([name for name in input_names if name in frozen_input_names]) > 0
            )
            if is_frozen_input:
            ...
            batch_dim, is_default_index = get_dimension_index_by_label(
                input_partial_shape, place.get_names(), layout_values, "N", 0
            )
            ...
            if is_default_index:
                ...
                validate_batch_in_shape(
                    shape_to_array(input_partial_shape), joined_name
                )
            ...
            log.debug(
                "Input: {}, Old shape: {}, New shape: {}".format(
    ...
    if argv.batch is not None and argv.batch > 0 and len(deferred_batch_names) > 0:
        ...
        for model_input in ov_model.inputs:
            ...
            if (
                input_name in deferred_batch_names
                and input_partial_shape.rank.is_static
            ):
                # update input shape with the specified batch for input that originally has dynamic rank
                batch_dim, is_default_index = get_dimension_index_by_label(
                    input_partial_shape, model_input.get_names(), layout_values, "N", 0
                )
                if batch_dim is None:
                    continue
                ...
                if is_default_index:
                    # if the batch index is chosen by default, we need to ensure that its size equals -1, 0 or 1
                    validate_batch_in_shape(
                        shape_to_array(input_partial_shape), input_name
                    )
