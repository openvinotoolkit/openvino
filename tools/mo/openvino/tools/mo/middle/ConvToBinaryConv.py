...
class ConvToBinaryConv(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        weights_consistent = np.all(np.isclose(weights, weights_rounded)) and set(
            np.unique(weights_rounded)
        ).issubset({-1, 1})

        if (
            weights_consistent
            and np.all(np.isclose(output_low, 0))
            and np.all(np.isclose(output_high, 1))
        ):
            ...
            weights_reduced = weights_reduced.reshape(
                [len(weights_reduced), 1, 1]
            )  # FIXME: works for NCHW only

            operator_name = operator.soft_get("name", operator.id)
            add = create_op_node_with_second_input(
                graph, Add, weights_reduced, {"name": operator_name + "/Add_"}
            )
            mul = create_op_node_with_second_input(
                graph, Mul, mo_array(0.5), {"name": operator_name + "/Mul_"}
            )
            ...
            weights_consistent
            and np.all(np.isclose(output_low, -1))
            and np.all(np.isclose(output_high, +1))
        ):
            pass
        else:
