...
class QuantizeLinearResolver(MiddleReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for quantize_node in graph.get_op_nodes(op="QuantizeLinear"):
            if quantize_node.has_and_set(
                "isolated"
            ):  # node is detached and will be eliminated
                # during the next clean up
                continue
            QuantizeLinearResolver.quantize_to_fakequantize(graph, quantize_node)

    @staticmethod
    def quantize_to_fakequantize(
        graph: Graph, quantize_node: Node, set_stop_value_propagation=False
    ):
        ...
        assert (
            zerop.soft_get("type") == "Const"
        ), "only constant for zero_point is supported for QuantizeLinear"
        ...
        fake_quantize = create_op_with_const_inputs(
            graph,
            FakeQuantize,
            {3: float_array(output_low_value), 4: float_array(output_high_value)},
        ...
        mul_low = create_op_with_const_inputs(
            graph,
            Mul,
            {1: float_array(output_low_value - zerop.value)},
        ...
        mul_high = create_op_with_const_inputs(
            graph,
            Mul,
            {1: float_array(output_high_value - zerop.value)},
        ...
        cast = Cast(
            graph, {"dst_type": zero_point_type, "name": node_name + "/Cast"}
        ).create_node()
        ...
        assert (
            scale_y_shape is not None
        ), "{0} contains scale(input with port 1) with shape None".format(
            quantize_node.soft_get("name", quantize_node.id)
        )
