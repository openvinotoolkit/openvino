...
def create_const_with_batch_from_input(
    ...
    for dest in shape_of_input.out_port(0).get_destinations():
        if dest.node.soft_get("op") == "Crop" and dest.node.in_port(
            1
        ).get_source().node.soft_get("value", []) == int64_array([1]):
            get_batch = dest.node
            break

    if get_batch is None:
        get_batch = create_op_node_with_second_input(
            graph,
            Crop,
            int64_array([1]),
            {
    ...
    for dest in get_batch.out_port(0).get_destinations():
        if dest.node.soft_get("op") == "Concat" and dest.node.in_port(
            1
        ).get_source().node.soft_get("value", []) == int64_array([second_dim]):
            mem_shape = dest.node
            break

    if mem_shape is None:
        mem_shape = create_op_node_with_second_input(
            graph,
            Concat,
            int64_array([second_dim]),
            {"name": get_batch.name + "/Concat", "axis": 0, "in_ports_count": 2},
            get_batch,
        )

    init_value_prev_lstm_output = None
    for dest in mem_shape.out_port(0).get_destinations():
        if dest.node.soft_get("op") == "Broadcast" and dest.node.in_port(
            1
        ).get_source().node.soft_get("value", []) == mo_array([value], dtype=precision):
            init_value_prev_lstm_output = dest.node
            break

    if init_value_prev_lstm_output is None:
        init_value_prev_lstm_output = create_op_with_const_inputs(
            graph,
            Broadcast,
...
class MakeKaldiConstReshapable(MiddleReplacementPattern):
    ...
    def run_after(self):
        from openvino.tools.mo.middle.InsertSelect import (
            AddSelectBeforeMemoryNodePattern,
        )
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for p in params:
            assert (
                p.shape[0] == batch
            ), "Parameter {} has batch different from the {}".format(
