...
def replace_resize(graph: Graph, resize: Node):
    log.debug(
        "Converting of ONNX Resize-11 to Interpolate-4 "
    ...
    if input_rank not in {4, 5}:
        log.warning(
            "The input shape is not 4D or 5D for op with name {}".format(resize_name)
        )
        return

    assert resize.is_in_port_connected(0) and (
        resize.is_in_port_connected(2) or resize.is_in_port_connected(3)
    ), "Scales or sizes inputs must be connected to Node {} with op {}.".format(
        resize.soft_get("name", resize.id), resize.op
    )

    assert (
        resize.soft_get("coordinate_transformation_mode") != "tf_crop_and_resize"
    ), "Mode tf_crop_and_resize is not supported for op {} with name {}".format(
        resize.op, resize.soft_get("name", resize.id)
    )
    ...
    sizes_ss = create_op_with_const_inputs(
        graph,
        StridedSlice,
        {1: int64_array([begin_dim]), 2: int64_array([end_dim]), 3: int64_array([1])},
        {
    ...
    add_node = create_op_with_const_inputs(
        graph, Add, {1: float_array([1.0e-5])}, {"name": resize_name + "/Add"}
    )
