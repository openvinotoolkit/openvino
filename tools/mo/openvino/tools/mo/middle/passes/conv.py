...
from openvino.tools.mo.middle.passes.fusing.helpers import get_tensor_id, get_value_id
...
def pad_op_transform(graph: Graph, match: dict):
    ...
    if pad_op.mode == "constant":
        ...
        if fill_value is None or fill_value != 0.0:
            log.info(
                'The pad node "{}" with non-zero fill value cannot be fused.'.format(
    ...
    for in_port in [1, 2]:
        ...
        if (
            pads[get_features_dim(op.graph.graph["layout"], input_tensor_dims)] != 0
            or pads[get_batch_dim(op.graph.graph["layout"], input_tensor_dims)] != 0
        ):
            log.info(
                'The pad node "{}" with padding over feature/batch dimension cannot be fused.'.format(
                    pad_op.soft_get("name")
                )
            )
            return
...
def fuse_pad(graph: Graph):
    for op_type in ["Convolution", "Pooling", "Deconvolution"]:
        apply_pattern(
            ...
            edges=[("pad_op", "pad_output"), ("pad_output", "op", {"in": 0})],
...
def muladd_to_scaleshift_action(graph: Graph, match: dict):
    ...
    if (
        mul.soft_get("can_be_scaleshift") is False
        or add.soft_get("can_be_scaleshift") is False
    ):
        return
    ...
    if weights.value.ndim == 0 and bias.value.ndim == 1:
        weights.value = np.full(
            bias.shape, weights.value.item(), dtype=weights.value.dtype
        )
        weights.shape = int64_array(weights.value.shape)
...
def batch_norm_fuse_action(graph: Graph, match: dict):
    ...
    if match["norm"].value is None or match["kernel"].value is None:
        ...
        return
    if (
        len(graph.out_edges(match["conv_output"].node)) > 1
        or len(graph.out_edges(match["kernel"].node)) > 1
    ):
