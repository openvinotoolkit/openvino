...
def convert_blob(blob: np.ndarray, dst_type: type):
    ...
    if dst_type in (np.int32, np.int64, np.uint8, np.int8) and not np.array_equal(
        blob, converted_blob
    ):
        raise Error(
            'The conversion of blob with value "{}" to dst_type "{}" results in rounding'.format(
                blob, dst_type
            )
        )
...
def convert_node_blobs(graph: Graph, node: Node, data_type: type):
    ...
    if any("bin" in d for _, __, d in out_edges):
        ...
        if blob.dtype != data_type:
            ...
            consumers = [
                x.name if x.has_valid("name") else "<NO NAME>" for x in node.out_nodes()
            ]
            log.debug(
            ...
            if finite_match_count:
                log.error(
                    (
                        ...
                        + refer_to_faq_msg(76)
                    ).format(finite_match_count, blob.size, consumers, data_type)
                )
            if zero_match_count:
                log.warning(
                    (
                        ...
                        + refer_to_faq_msg(77)
                    ).format(zero_match_count, blob.size, consumers, data_type)
                )
...
def convert_parameters_data_type(graph: Graph, data_type_str: str):
    ...
    user_defined_data_types = (
        graph.graph["user_shapes"] if "user_shapes" in graph.graph else None
    )
    for input in inputs:
        ...
        if user_defined_type is not None:
            log.info(
                "Overriding Parameter node {} data type to {}".format(
            ...
            log.info(
                "Do not change data type for node {}".format(input.soft_get("name"))
            )
    ...
    for node in graph.get_data_nodes():
        if node.value is not None:
            try:
                ...
                raise Error(
                    "Coudn't convert blob {}, details: {}", node.soft_get("name"), e
                ) from e
