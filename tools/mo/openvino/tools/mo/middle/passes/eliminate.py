...
def mark_output_reachable_nodes(graph):
    ...
    log.debug(
        "The following nodes are seeded as output reachable:\n{}".format(
            "\n".join(sorted(map(str, outputs)))
        )
    )
    nx.set_node_attributes(
        G=graph, name="is_output_reachable", values={n: True for n in outputs}
    )
    visited = set()
    for output_name in outputs:
        reverse_dfs(
            graph,
            output_name,
            lambda graph, node_name: mark_input_nodes(
                graph, node_name, "is_output_reachable", True
            ),
            visited,
        )
...
def mark_undead_nodes(graph, undead_types: list):
    ...
    nx.set_node_attributes(
        G=graph, name="is_undead", values={n: True for n in undead_nodes}
    )
        ...
        if graph.node[node_name]["is_undead"]:
            for _, dst_node_name in graph.out_edges(node_name):
                ...
                if "kind" in node_attrs and (
                    node_attrs["kind"] == "data"
                    and node_attrs["value"] is not None
...
def mark_const_producer_nodes(graph):
    ...
    for node in graph.pseudo_topological_sort():
        ...
        if (
            ...
            or not is_fully_defined(node.value)
        ):
    ...
    for node_name, node_attrs in graph.nodes(data=True):
        ...
        if node_attrs.get("type", None) == "Const":
            ...
            if np.all(node_attrs.get("force_shape", False)):
                Node(graph, node_name).out_node()["force_shape"] = node_attrs[
                    "force_shape"
                ]
            if node_attrs.get("force_type", False):
                Node(graph, node_name).out_node()["force_type"] = node_attrs[
                    "force_type"
                ]
        ...
        if not node_attrs["is_output_reachable"] or (
            ...
            and (
                not node_attrs["is_undead"] or node_attrs.get("force_dead_node", False)
            )
        ):
            nodes_to_remove.add(node_name)
    log.debug(
        "Removing the following dead nodes: {}".format(
...
def remove_op_node_with_data_node(graph, node_to_remove, input_data_node=None):
    ...
    assert (
        len(output_node) == 1
    ), "Cannot remove node producing two or more output tensors"
...
def remove_edges_for_nodes(graph, node_attrs: dict, edge_attrs: dict):
    ...
    for node in graph.nodes():
        ...
        if all(
            [node.has(attr) and node[attr] == node_attrs[attr] for attr in node_attrs]
        ):
