...
def _check_lin_op(node: Node, layout: str):
    ...
    if node.soft_get("op") in lin_ops:
        ...
        if weights_id is None:
            ...
            log.info(
                "[ FUSING ] Node {} wasn't marked as fusable (no weights, probably this is element-wise operation"
    ...
    for node in nodes:
        ...
        for mask in regex_masks:
            ...
            if res and len(res):
                ...
                log.info(
                    "[ FUSING ] Node {} wasn't marked as fusable (user decision {})".format(
                        node.id, mask
                    )
                )
                disabled = True
...
def mark_shape_of_sugraph_as_unfusable(graph: Graph):
    ...
    shapeof_subgraph_nodes = MarkSubGraphsWithCorrectLayout.bfs(
        starting_nodes, set(), condition_to_continue
    )
