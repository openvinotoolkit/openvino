...
def _fuse_linear_sequence(graph: Graph, start_node: Node):
    ...
    while True:
        ...
        if (
            dst_node.soft_get("op") in ["Mul", "Add"]
            and get_value_in_port(dst_node) is not None
            and dst_node.soft_get("can_be_fused") is True
        ):
    ...
    if len(fnodes) == 1 or (
        len(fnodes) == 2 and fnodes[0].op == "Mul" and fnodes[1].op == "Add"
    ):
        return False
    ...
    if any([x != 0 for x in np.nditer(add)]) and any([x != 1 for x in np.nditer(mul)]):
        ...
        #  ----->Mul------>Add-->
        mul_const = Const(
            graph, dict(name="data_mul_", value=mo_array(mul))
        ).create_node()
        add_const = Const(
            graph, dict(name="data_add_", value=mo_array(add))
        ).create_node()
