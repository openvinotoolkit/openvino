...
def _fuse_mul(graph: Graph, node: Node, fuse_nodes: list, backward: bool = True):
    ...
    if const_port is None or tensor_port is None:
        log.warning(
            "Cannot do fuse_mul for node {} because this node has wrong inputs".format(
                node.id
            )
        )
        return False
    ...
    for fuse_node in fuse_nodes:
        if fuse_node.soft_get("can_be_fused") is False:
            log.warning(
                "Node {} can't be used in fusing because attr can_be_fused = False".format(
                    fuse_node.name
                )
            )
            return False
        ...
        if not weights_port.data.has_valid(
            "output_channel_dim"
        ) or not weights_port.data.has_valid("input_channel_dim"):
            log.warning(
                "Cannot do fuse_mul for node {} because there is no field "
                + "output_channel_dim and/or input_channel_dim in weights.".format(
                    fuse_node.soft_get("name")
                )
            )
            return False
    ...
    for fuse_node in fuse_nodes:
        ...
        try:
            ...
            log.error(
                "Cannot fuse const from {} to {}. Reshape failed. Skipping.".format(
        ...
        mul_const = Const(
            graph, {"value": value, "name": mul_name + "/const"}
        ).create_node()
        ...
        if (
            fuse_node_in_data.soft_get("name") == w_const_out_data.soft_get("name")
            and fuse_node_in_data.soft_get("name", None) is not None
        ):
        ...
        if (
            backward
            and len(fuse_node.in_ports()) == 3
            and not fuse_node.in_port(2).disconnected()
            and not fuse_node.has_and_set("shape_input")
        ):
...
def fuse_linear_ops(graph: Graph):
    ...
    for node in nodes:
        ...
        if (
            node.soft_get("op") == "Mul"
            and get_value_in_port(node) is not None
            and node.has_and_set("can_be_fused")
        ):
