...
def concat_convolutions(graph: Graph, start_node: Node, last_node: Node):
    ...
    for attr in conv_attrs:
        for id in range(len(conv_nodes)):
            ...
            if not np.array_equal(gconv[attr], conv[attr]):
                log.debug(
                    "Grouped convolutions fusion : attrs {} doesn't match".format(attr)
                )
                return False
    ...
    if has_biases:
        for id in range(len(conv_nodes)):
            ...
            if conv.in_node(2).shape != gconv.in_node(2).shape:
                log.debug(
                    "Group convolutions fusion : convolutions have different biases shape {} and {}".format(
    ...
    graph.add_edges_from(
        [(input.id, gconv.id, {"in": 0}), (gconv.id, output.id, {"out": 0})]
    )
    ...
    for conv in conv_nodes[1:]:
        weights_value = np.concatenate(
            (weights_value, conv.in_node(1).value), axis=feature_dim
        )
        if has_biases:
            bias_value = np.concatenate(
                (bias_value, conv.in_node(2).value), axis=-1
            )  # Not validated
    ...
    log.debug(
        "Start node : {} Last node : {}  Nodes inside : {}".format(
        ...
        for node in graph.pseudo_topological_sort():
            if node.kind == "op" and len(node.out_nodes()) > 1:
                ...
                if len(next_nodes) > 1 and all(
                    _node.soft_get("type") in ["Convolution", "Deconvolution"]
                    ...
                    if is_valid_convolutions:
                        is_fused = concat_convolutions(
                            graph, node, Node(graph, last_layer)
                        )
                        if is_fused:
                            break
