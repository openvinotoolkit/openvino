...
def log_debug_dict(nodes_per_port: dict, direction_name: str):
    for port, node in nodes_per_port.items():
        ...
        log.debug(
            "{}[{}]: shape = {}, value = {}".format(
                direction_name, port, node.soft_get("shape"), value
            )
        )
...
def control_flow_infer(graph: Graph, node_name: str):
    ...
    in_df_edges_with_data = [
        (u, v, attrs)
        for u, v, attrs in in_edges_with_data
        if "control_flow_edge" not in attrs or not attrs["control_flow_edge"]
    ]
    in_cf_edges_with_data = [
        (u, v, attrs)
        for u, v, attrs in in_edges_with_data
        if "control_flow_edge" in attrs and attrs["control_flow_edge"]
    ]
    is_executable_df = all(
        [graph.node[u]["executable"] for u, _, attrs in in_df_edges_with_data]
        if len(in_df_edges_with_data)
        else [True]
    )
    is_executable_cf = all(
        [graph.node[u]["executable"] for u, _, attrs in in_cf_edges_with_data]
        if len(in_cf_edges_with_data)
        else [True]
    )
...
def partial_infer(graph: Graph, start_node: str = None):
    ...
    if start_node is not None:
        ...
        nx.set_node_attributes(
            G=graph.subgraph(nodes[start_index:]),
...
def infer_nodes(graph: Graph, nodes: List[Node], constant_subgraph_only: bool = False):
    ...
    for n in nodes:
        ...
        try:
            if node.has("is_partial_inferred") and not node.is_partial_inferred:
                if node.has("infer") and not node.infer is None:
                    ...
                    if constant_subgraph_only:
                        in_values = [
                            port.data.get_value() for port in node.in_ports().values()
                        ]
                        if (
                            node.soft_get("op") == "Parameter"
                            or any(value is None for value in in_values)
                            or (
                                node.soft_get("op") == "ShapeOf"
                                and node.in_port(0).data.get_shape() is None
                            )
                        ):
                    ...
                    if not constant_subgraph_only:
                        ...
                        for out_port, out_node in out_nodes.items():
                            ...
                            if not out_node.has_valid("shape"):
                                log.error(
                                    'Shape is not defined for output {} of "{}".'.format(
                                        out_port, node_name
                                    )
                                )
            ...
            log.error(
                'Cannot infer shapes or values for node "{}".'.format(
                    node.soft_get("name")
                )
            )
            log.error(str(err))
            log.error("")
            log.error(
                "It can happen due to bug in custom shape infer function {}.".format(
                    node.soft_get("infer")
                )
            )
            ...
            raise Error(
                'Stopped shape/value propagation at "{}" node. '.format(
                    node.soft_get("name")
                )
                + refer_to_faq_msg(38)
            ) from err
            ...
            if not node.soft_get("fixed_batch", False):
                ...
                if has_layout:
                    if idx is not None:
                        ...
                        log.warning(
                            "Layout for input {} doesn't have batch dimension. Skipping this input.".format(
                                name
                            )
                        )
                else:
...
def override_placeholder_shapes(graph: Graph, user_shapes: dict, batch=None):
    ...
    for node_id in placeholders:
        ...
        if (
            batch is not None
            and node_attrs["shape"] is not None
            and len(node_attrs["shape"]) > 0
        ):
...
def copy_type_infer(node):
    for out_port in node.out_ports().values():
        ...
        if len(connected_in_ports) != 0:
            ...
            raise Error(
                "No input ports of node {} to determine data type".format(
                    node.soft_get("name")
                )
            )
...
def reverse_infer(graph: Graph, nodes: list):
    ...
    for n in nodes:
        ...
        if node.has_and_set("reverse_infer"):
            log.debug(
                "Executed reverse infer for node '{}'".format(
    ...
    if len(parameters_with_no_shape) > 0:
        raise Error(
            ...
            "Please use cli options --input or --input_shape to set model input shape.".format(
                parameters_names
            )
        )
