...
class Deconvolution3rdInputNormalization(MiddleReplacementPattern):
    ...
    @staticmethod
    def pattern():
        return dict(nodes=[("op", dict(kind="op", type="Deconvolution"))], edges=[])

    @staticmethod
    def replace_pattern(graph: Graph, match: dict):
        node = match["op"]
        if (
            not node.has_port("in", 2)
            or node.in_port(2).disconnected()
            or not node.has_and_set("shape_input")
        ):
            return

        if (
            node.has_valid("layout")
            and not node.layout.startswith("NC")
            and graph.graph["layout"] == "NCHW"
        ):
            ...
            axis_const = Const(
                graph, {"value": int64_array(0), "name": name + "/Axis"}
            ).create_node_with_data()
            gather = Gather(
                graph, {"name": name, "need_shape_inference": True}
