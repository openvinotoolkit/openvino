...
from openvino.tools.mo.utils.shape import (
    ...
    node_to_get_batch_value,
    node_to_get_features_dimension_value,
    node_to_get_spatial_dimensions_value,
)
...
class GroupNormToMVN(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: Dict[str, Node]):
        ...
        initial_shape_op_node = Shape(
            graph, {"name": group_norm_node.name + "/Shape"}
        ).create_node()
        ...
        initial_shape_op_node_float = Cast(
            graph,
            {
                "name": initial_shape_op_node.name + "/to_float",
        ...
        initial_spatial_dims_node = Cast(
            graph,
            {
                "name": initial_spatial_dims_node_int.name + "/to_float",
                "dst_type": data_type_str_to_np(graph.graph["cmd_params"].data_type),
            },
        ).create_node()
        ...
        reciprocal_group_size_node = Const(
            graph,
            {
                "value": mo_array([1.0 / group_norm_node.num_groups]),
        ...
        new_shape_node = Cast(
            graph,
            {"name": new_shape_node_float.name + "/to_int64", "dst_type": np.int64},
        ).create_node()
        ...
        _, rank = get_shape_and_rank_nodes_by_port(
            mvn_node.in_port(0).get_connection().get_source(), return_as_a_scalar=True
        )
        rng = create_op_with_const_inputs(
            graph,
            Range,
            {0: int64_array(1), 2: int64_array(1)},
