...
class SmartInputMatcher(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        assert (
            match["stack_data"]["value"][0] == 0
            and match["stack_1_data"]["value"][0] == 1
            and match["stack_2_data"]["value"][0] == 1
        )
        ...
        if 0 in ta_size.in_nodes():
            ...
            if shape["kind"] == "op" and shape["op"] == "Const":
                ...
                log.warning(
                    "Your network cannot be reshaped since shapes of placeholders are constants. "
...
class SimpleInputMatcher(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        input_node.create_node_with_data(
            inputs=[match["Enter"].in_node()], data_nodes=[match["Enter"].out_node()]
        )
...
class BackEdgeSimpleInputMatcher(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if len(init_input.in_nodes()) == 0 or (
            len(init_input.in_nodes()) == 1
            and init_input.has_valid("value")
            and init_input.in_node(0).soft_get("op") != "TensorIteratorInput"
        ):
            ...
            graph.add_edges_from(
                [(input_data_node.id, match["BackEdge"].id, {"in": 0, "out": 0})]
            )
...
class SmartMatcherInputSlicingWithGather(MiddleReplacementPattern):
    ...
    def run_before(self):
        ...
        from openvino.tools.mo.middle.TensorIteratorCondition import (
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        resulted_slice_node_name = resulted_slice_node.soft_get(
            "name", resulted_slice_node.id
        )

        # create TensorIteratorInput node that reflects slicing of input for each time step along axis
        ti_input_node = TensorIteratorInput(
            graph,
            dict(
                axis=axis,
                start=init_time,
        ...
        initial_input_node.in_port(0).get_connection().set_destination(
            ti_input_node.in_port(1)
        )
        current_index_node.out_port(0).connect(ti_input_node.in_port(2))
        resulted_slice_node.out_port(0).get_connection().set_source(
            ti_input_node.out_port(0)
        )

        # delete no longer needed nodes responsible for slicing of input in the original graph
        node_names_for_remove = [
            "EnterInput",
            "MergeInput",
            "SwitchInput",
            "IdentityInput",
        ...
        graph.remove_nodes_from(
            [match[node_name].id for node_name in node_names_for_remove]
        )
