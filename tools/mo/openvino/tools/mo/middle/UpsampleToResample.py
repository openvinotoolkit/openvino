...
class UpsampleToResample(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: Dict[str, Node]):
        ...
        if input_shape_rank not in [4, 5]:
            log.warning(
                "The input shape is not 4D or 5D for op {}".format(
        ...
        if len(upsample.in_nodes()) == 2:
            ...
            if not (
                math.isclose(scales[0], 1, rel_tol=1e-5)
        ...
        axes_node = Const(
            graph, {"name": upsample_name + "/axis", "value": axes}
        ).create_node()
        ...
        scales_node = Const(
            graph, {"name": upsample_name + "/scales", "value": factor_value}
        ).create_node()
        ...
        rename_nodes(
            [(upsample, upsample_name + "/delete"), (interpolate, upsample_name)]
        )
