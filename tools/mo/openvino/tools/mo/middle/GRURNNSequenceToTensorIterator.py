    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        inputs = [
            Op._create_data_node(
                body,
                rnn_layer.name + "/inport/" + str(inp),
                {
                    "shape": rnn_layer.in_node(inp).shape.copy(),
                    "value": (
                        rnn_layer.in_node(inp).value.copy()
                        if rnn_layer.in_node(inp).value is not None and inp in [1, 2]
                        else None
                    ),
                },
            )
        ...
        input_squeeze = Squeeze(
            body, dict(name=rnn_layer.name + "/input_squeeze", internal_layer_id=0)
        )
        input_squeeze_dim = Const(
            body,
            dict(
                name=rnn_layer.name + "/input_squeeze_dim", value=rnn_layer.sequence_dim
            ),
        ).create_node_with_data()
        inputs[0] = input_squeeze.create_node_with_data(
            [inputs[0], input_squeeze_dim], edge_attrs=[{"internal_port_id": 0}]
        )
        ...
        outputs = [
            Op._create_data_node(
                body,
                rnn_layer.name + "/outport/" + str(out),
                {
                    "shape": (
                        rnn_layer.out_node(out).shape.copy()
                        if out in rnn_layer.out_nodes()
                        else None
                    )
                },
            )
        ...
        output_unsqueeze_dim = Const(
            body,
            dict(
                name=rnn_layer.name + "/output_unsqueeze_dim",
                value=rnn_layer.sequence_dim,
            ),
        ).create_node_with_data()
        output_unsqueeze = Unsqueeze(
            body, dict(name=rnn_layer.name + "/output_unsqueeze/", internal_layer_id=2)
        )

        additional_attrs = dict(
            activations=rnn_layer.activations,
        ...
        gru_cell = rnn_cell_op.create_node_with_data(
            inputs,
            data_nodes=outputs,
            edge_attrs=[
                {},
        ...
        assert sorted(rnn_layer.out_nodes().keys()) == list(
            range(len(rnn_layer.out_nodes()))
        ), "There are gaps in output ports of GRUSequence operation. Node {}".format(
            rnn_layer.id
        )
        ...
        outs = ti_op.create_node_with_data(
            ...
            data_nodes=[
                rnn_layer.out_node(i) for i in range(len(rnn_layer.out_nodes()))
            ],
