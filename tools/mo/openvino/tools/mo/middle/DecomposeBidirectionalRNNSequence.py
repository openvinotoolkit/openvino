...
class DecomposeBidirectionalRNNSequence(MiddleReplacementPattern):
    ...
    def run_after(self):
        from openvino.tools.mo.middle.MXNetRNNSequenceNormalize import (
            MXNetRNNSequenceNormalize,
        )
        from openvino.tools.mo.middle.ONNXRNNSequenceNormalize import (
    ...
    def pattern(self):
        return dict(
            nodes=[
                (
                    "lstm",
                    dict(kind="op", type="RNNSequence", direction="bidirectional"),
                ),
    ...
    def split_data(self, data: Node):
        ...
        output_data = [
            Op._create_data_node(
                data.graph,
                name=data.name + "/SplittedBiLSTM/{}".format(["forward", "reverse"][i]),
            )
            for i in [0, 1]
        ]
        split_op = Split(
            data.graph, dict(name=data.name + "/DecomposedBiLSTM_0", num_splits=2)
        )
        axis_const = Const(
            data.graph,
            {
                "name": data.name + "/DecomposedBiLSTM_0" + "/Split_axis",
                "value": np.int64(0),
            },
        ).create_node_with_data()
        return split_op.create_node_with_data(
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        new_init_cells = (
            self.split_data(bidirectional_cell.in_node(6))
            if 6 in bidirectional_cell.in_nodes()
            else (None, None)
        )
        ...
        self.concat_outputs(
            bidirectional_cell, outputs[0], outputs[1], bidirectional_cell.out_nodes()
        )
    ...
    def get_new_cell(bidirectional_cell: Node, direction: str):
        ...
        if attrs["activations"] is not None and len(attrs["activations"]) > 1:
            assert (
                len(attrs["activations"]) == 2
            ), "Bidirectional RNN should have 2 activations"
    ...
    def concat_outputs(bi_rnn, forward_outputs, reverse_outputs, final_outputs):
        ...
        for i in final_outputs:
            concat_ops[i].create_node_with_data(
                [forward_outputs[i], reverse_outputs[i]], data_nodes=[final_outputs[i]]
            )
