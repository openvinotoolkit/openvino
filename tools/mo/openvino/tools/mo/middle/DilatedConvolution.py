...
from openvino.tools.mo.front.common.partial_infer.utils import mo_array, shape_insert
...
class DilatedConvolutionConverter(MiddleReplacementPattern):
            ...
            nodes=[
                (
                    ...
                    dict(
                        ...
                        in ["Conv2D", "DepthwiseConv2dNative", "Conv3D"],
                    ),
                ),
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        conv.pad[conv.spatial_dims] = [
            [pad_begin[x], pad_end[x]] for x in conv.spatial_dims
        ]
        conv["auto_pad"] = None
...
class DilatedConvolution1DConverter(MiddleReplacementPattern):
        ...
        return dict(
            nodes=[
                (
                    ...
                    dict(
                        kind="op",
                        op=lambda value: value in ["Conv2D", "DepthwiseConv2dNative"],
                    ),
                ),
                ("space_to_batch", dict(kind="op", op="SpaceToBatch")),
    ...
    def swap_pad_and_unsqueeze(self, pad: Node, unsqueeze: Node):
        ...
        for port_id in [1, 2]:
            ...
            new_value_node = Const(
                pad.graph,
                {
                    "name": pad.soft_get("name", pad.id) + "/value_{}".format(port_id),
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if len(conv.in_port(0).data.get_shape()) != 4:
            log.debug(
                'The convolution node "{}" input is not 4D'.format(
