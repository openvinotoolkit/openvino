...
class RemoveRedundantReshapes(MiddleReplacementPattern):
    ...
    force_clean_up = True
    run_not_recursively = (
        True  # non-unified data nodes view in TI body (no Const ops, bare data node)
    )
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for reshape_node in graph.get_op_nodes(type="Reshape"):
            in_ports = [
                port
                for port in reshape_node.in_ports().values()
            ...
            if np.array_equal(dim, in_shape) and len(reshape_node.out_nodes()):
                log.debug(
                    "Useless reshape with dim {} was deleted: {}".format(
