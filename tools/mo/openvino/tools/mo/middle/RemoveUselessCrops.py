...
class RemoveUselessCropsPattern(MiddleReplacementPattern):
    ...
    def run_after(self):
        from openvino.tools.mo.middle.RemoveDuplicationMemory import (
    ...
    def pattern():
        return dict(
            ...
            edges=[("crop", "data"), ("data", "concat", {"in": 0})],
        )
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        for in_port in outs:
            ...
            if (
                out["op"] == "Crop"
                and out["axis"] == axis
                and len(out.out_port(0).get_destinations()) == 1
                and out.out_port(0).get_destination().node == concat_node
            ):
                # crop type 1
                if "dim" in out:
                ...
                # crop type 3
                    ...
                    offsets_dims.append(
                        (out["crop_begin"], out["crop_end"] - out["crop_begin"])
                    )
                # crop type 2 with const dim
                elif (
                    not out.in_port(1).disconnected()
                    and out.in_port(1).data.get_value() is not None
                ):
                    offsets_dims.append(
                        (out["offset"], out.in_port(1).data.get_value())
                    )
