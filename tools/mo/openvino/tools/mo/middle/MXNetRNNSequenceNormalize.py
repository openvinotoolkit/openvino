...
class MXNetRNNSequenceNormalize(MiddleReplacementPattern):
    ...
    def run_after(self):
        from openvino.tools.mo.middle.MXNetSplitMultiLayers import (
    ...
    def repack_weights(graph: Graph, match: dict):
        ...
        W, R = mo_array(
            W[:, 0 : rnn_layer.hidden_size * rnn_layer.multiplier * input_size]
        ), mo_array(W[:, rnn_layer.hidden_size * rnn_layer.multiplier * input_size :])
    ...
    def add_output_reshape(graph: Graph, match: dict):
        ...
        new_data = Op._create_data_node(
            graph, name=lstm_name + "/Data/Reshape_mxnet/", attrs={"shape": mo_shape}
        )
        ...
        permute_data = Transpose(
            graph, {"name": lstm_name + "/Transpose_mxnet/"}
        ...
        reshape.create_node_with_data(
            [permute_data, reshape_dim_data], dict(), data_nodes=[old_data_node]
        )
