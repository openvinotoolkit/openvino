...
from openvino.tools.mo.graph.graph import Graph, dict_includes
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if (
            len(
                [
                    dst
                    for dst in direct_permute_dsts
            ...
            return
        for shape_of_dst in [
            dst for dst in direct_permute_dsts if dst.node.soft_get("type") == "ShapeOf"
        ]:
            name = (
                shape_of_dst.node.soft_get("name", shape_of_dst.node.id)
                + "/FusedToTITranspose"
            )
        ...
        data_input_port = find_ports(
            ti.input_port_map, {"axis": lambda attr: attr in [0, 1]}
        )
        data_output_port = find_ports(
            ti.output_port_map, {"axis": lambda attr: attr in [0, 1]}
        )
        ...
        assert (
            ti.in_edge(0)["external_port_id"]
            == ti.input_port_map[data_input_port]["external_port_id"]
        )
        assert (
            ti.out_edge(0)["external_port_id"]
            == ti.output_port_map[data_output_port]["external_port_id"]
        )
        ...
        ti.input_port_map[data_input_port]["axis"] = (
            1 - ti.input_port_map[data_input_port]["axis"]
        )
        ti.output_port_map[data_output_port]["axis"] = (
            1 - ti.output_port_map[data_output_port]["axis"]
        )
        ...
        isomorphism["unsqueeze_dim"].value = ti.output_port_map[data_output_port][
            "axis"
        ]
