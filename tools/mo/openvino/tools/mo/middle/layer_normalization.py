    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in graph.get_op_nodes(op="LayerNorm"):
            ...
            if node.output_mean_var is True:
                if (
                    not node.out_port(1).disconnected()
                    or not node.out_port(2).disconnected()
                ):
                    raise Error(
                        "Node {} is supported with only one output".format(node_name)
                    )
                log.error(
                    'LayerNorm node {} with attribute "output_mean_var" = True is not supported.'
                    "But since the node has one output, the conversion will continue.".format(
                        node_name
                    ),
            ...
            assert node.has_valid(
                "axis"
            ), "Incorrect axis value for the node {}".format(node_name)
            axis = node.axis

            mvn = create_op_node_with_second_input(
                graph,
                MVN,
                int64_array([axis]),
                dict(
                    eps=node.epsilon,
                    name=node_name + "/LayerNorm/MVN_",
                    across_channels=1,
            ...
            mul_const_unsqueeze = create_op_node_with_second_input(
                graph,
                Unsqueeze,
                int64_array(unsqueeze_value),
                dict(name=mul.name + "/Unsqueeze", override_output_shape=True),
            )
            add_const_unsqueeze = create_op_node_with_second_input(
                graph,
                Unsqueeze,
                int64_array(unsqueeze_value),
                dict(name=add.name + "/Unsqueeze", override_output_shape=True),
            )
