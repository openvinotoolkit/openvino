    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        inputs = [
            Op._create_data_node(
                body,
                lstm.name + "/inport/" + str(inp),
                {
                    "shape": lstm.in_node(inp).shape.copy(),
                    "value": (
                        lstm.in_node(inp).value.copy()
                        if lstm.in_node(inp).value is not None and inp in [1, 2]
                        else None
                    ),
                },
            )
        ...
        squeeze_dim_data = Const(
            body,
            {"name": lstm.name + "/input_squeeze_dim", "value": [lstm.sequence_dim]},
        ).create_node_with_data()
        inputs[0] = input_squeeze.create_node_with_data(
            [inputs[0], squeeze_dim_data], edge_attrs=[{"internal_port_id": 0}]
        )
        ...
        outputs = [
            Op._create_data_node(
                body,
                lstm.name + "/outport/" + str(out),
                {
                    "shape": (
                        lstm.out_node(out).shape.copy()
                        if out in lstm.out_nodes()
        ...
        output_unsqueeze = Unsqueeze(
            body, dict(name=lstm.name + "output_unsqueeze", internal_layer_id=2)
        )
        unsqueeze_dim_data = Const(
            body,
            {"name": lstm.name + "/output_unsqueeze_dim", "value": [lstm.sequence_dim]},
        ).create_node_with_data()
        ...
        assert sorted(lstm.out_nodes().keys()) == list(
            range(len(lstm.out_nodes()))
        ), "There are gaps in output ports of LSTMSequence operation. Node {}".format(
            lstm.id
        )
