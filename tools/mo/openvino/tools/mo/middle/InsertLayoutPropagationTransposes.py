...
class InsertLayoutPropagationTranspose(MiddleReplacementPattern):
    ...
    enabled = True
    force_clean_up = (
        True  # need to run clean up after the transformation to update shapes
    )
    ...
    def is_nchw_to_nhwc_transpose_needed(node: Node):
        ...
        """
        return (
            node.has_and_set("reinterp_shape")
            and not is_input_data_in_correct_layout(node, 0)
            and len(node.in_port(0).data.get_shape()) >= 4
            and all(
                [
                    port.data.get_value() is None
    ...
    def is_nhwc_to_nchw_transpose_needed(node: Node):
        ...
        return (
            node.has_and_set("reinterp_shape")
            and not is_output_data_in_correct_layout(node, 0)
            and len(node.out_port(0).data.get_shape()) >= 4
            and all(
                [
                    port.data.get_value() is None
                    for port in node.out_ports().values()
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for reinterp_shape_node_id in graph.get_nodes_with_attributes(
            ...
            assert (
                0 in reinterp_shape_node.in_nodes()
            ...
            if self.is_nchw_to_nhwc_transpose_needed(reinterp_shape_node):
                permute_node = create_op_node_with_second_input(
                    graph,
                    Transpose,
                    PermuteAttrs().get_nchw_to_nhwc_permutation(len(input_shape)).perm,
                    {
                        "name": reinterp_shape_node.in_port(0).get_source().node.name
                        + "/Transpose"
                    },
                )
        ...
        for reinterp_shape_node_id in graph.get_nodes_with_attributes(
            ...
            reinterp_shape_node = Node(graph, reinterp_shape_node_id)
            assert (
                0 in reinterp_shape_node.out_nodes()
            ...
            if self.is_nhwc_to_nchw_transpose_needed(reinterp_shape_node):
                permute_node = create_op_node_with_second_input(
                    graph,
                    Transpose,
                    PermuteAttrs().get_nhwc_to_nchw_permutation(len(output_shape)).perm,
...
def is_input_data_in_correct_layout(node: Node, port_ind: int):
    ...
    return (
        "correct_in_data_layout" in node.attrs()
        and port_ind in node.attrs()["correct_in_data_layout"]
    )
...
def is_output_data_in_correct_layout(node: Node, port_ind: int):
    assert node.soft_get("kind") == "op", "The function work with operation nodes only"
    return (
        "correct_out_data_layout" in node.attrs()
...
def insert_transpose(graph: Graph, input_port: Port, before_input=True):
    ...
    if input_rank > 3:
        if before_input:
            ...
            transpose_name = (
                source_node.soft_get("name", source_node.id) + "/TransposeToNHWC"
            )
        else:
            axis_order = np.concatenate(
                (
