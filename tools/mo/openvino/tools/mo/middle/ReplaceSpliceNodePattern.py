...
class ReplaceSpliceNodePattern(MiddleReplacementPattern):
   ...
   """
    ...
    def run_after(self):
        ...
        return [MergeNeighborSplicePattern, RemoveMemoryDuplicationPattern]

    @staticmethod
    def pattern():
        return dict(nodes=[("op", dict(op="Splice"))], edges=[])
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        # Input  /
        concat_node = Concat(
            graph, {"name": "Splice_Concat", "in_ports_count": 2, "axis": 1}
        ).create_node()
        concat_node.in_port(0).connect(crop.out_port(0))

        # Concat -> Memory(out)
        mem_out = Assign(
            graph, {"name": "out_splice_memory", "variable_id": memory_pair_id}
        ).create_node()
        ...
        if node.const_dim != 0:
            ...
            init_value_input_memory_const_dim = Const(
                graph,
                {
                    "name": "init_value_const_dim_in_memory",
                    "value": np.zeros(
            ...
            input_memory_const_dim = ReadValue(
                graph,
                {
            ...
            concat_node_const_dim = Concat(
                graph, {"name": "const_dim_concat", "in_ports_count": 2, "axis": 1}
            ).create_node()
            concat_node_const_dim.in_port(0).connect(crop_const_dim.out_port(0))

            mem_out_const_dim = Assign(
                graph, {"name": "const_dim_out_memory", "variable_id": memory_pair_id}
            ).create_node()
            ...
            concat_const = Concat(
                graph,
                {"name": node.id + "_concat_const", "axis": 1, "in_ports_count": 2},
            ).create_node()
            ...
            init_value_input_memory = Const(
                graph,
                {
                    ...
                    "value": np.zeros(
                        int64_array([in_shape[0], memory_size]), dtype=np.float32
                    ),
