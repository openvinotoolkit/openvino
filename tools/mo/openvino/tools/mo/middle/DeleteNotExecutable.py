...
class DeleteNotExecutable(MiddleReplacementPattern):
    ...
    def run_after(self):
        from openvino.tools.mo.middle.TensorIteratorConditionChecker import (
            ConditionChecks,
        )
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for node_name, node_attrs in list(graph.nodes(data=True)):
            if (
                ...
                and "executable" in node_attrs
                and not node_attrs["executable"]
            ):
                [nodes_to_remove.add(op) for op, _ in graph.in_edges(node_name)]
                nodes_to_remove.add(node_name)
        log.debug(
            "Removing the following not executable nodes: {}"
