...
class FusedBatchNormTraining(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        assert (
            shape is not None
        ), "The shape of scale input of the BatchNorm node {} is not defined".format(
            node.name
        )
        ...
        reshape_back = Reshape(
            graph, {"name": node_name + "/restore_shape", "override_output_shape": True}
        ).create_node()
