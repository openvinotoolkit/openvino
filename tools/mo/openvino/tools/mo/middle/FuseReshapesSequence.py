...
from openvino.tools.mo.middle.pass_separator import MiddleFinish, PostMiddleStart
        ...
        for node in reshape_nodes:
            ...
            if len(node.out_port(0).get_destinations()) == 1:
                ...
                if next_op.has_valid("type") and next_op.type == "Reshape":
                    ...
                    log.debug(
                        "Second phase for Reshape: {}".format(node.soft_get("name"))
                    )
    ...
    * tr = t and wr = w - that constants used in the second reshape is the same in shape before the first Reshape
    ...
    def pattern(self):
        return dict(
            ...
            edges=[
                ("reshape_in_dims", "reshape_in_dims_d"),
                ("reshape_in_dims_d", "reshape_in", {"in": 1}),
                ("reshape_in", "reshape_in_d"),
                ("reshape_in_d", "reshape_out", {"in": 0}),
                ("reshape_in_d", "shape"),
                ("shape", "shape_d"),
                ("shape_d", "gather_batch", {"in": 0}),
                ("gather_in_1", "gather_in_1_d"),
                ("gather_in_1_d", "gather_batch", {"in": 1}),
                ("gather_in_2", "gather_in_2_d"),
                ("gather_in_2_d", "gather_batch", {"in": 2}),
                ("gather_batch", "gather_batch_d"),
                ("gather_batch_d", "concat_dims", {"in": 0}),
                ("time_dim", "time_dim_d"),
                ("time_dim_d", "concat_dims", {"in": 1}),
                ("concat_dims", "concat_dims_d"),
                ("concat_dims_d", "reshape_out", {"in": 1}),
            ],
        )
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        log.debug(
            "second child_node: id={}, type={}".format(
                reshape_out.soft_get("id"), reshape_out.soft_get("type")
            )
        )
        ...
        if (
            w_node.op != "Const"
            or not is_fully_defined(t_node.out_port(0).data.get_value())
            or not is_fully_defined(w_node.out_port(0).data.get_value())
            or not np.all(t_node.out_port(0).data.get_value() == [in_shape[1]])
            or not np.all(
                w_node.out_port(0).data.get_value() == [in_shape[const_dim_2]]
            )
        ):
            return
