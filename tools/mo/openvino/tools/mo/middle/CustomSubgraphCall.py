...
class CustomSubgraphCall(MiddleReplacementPattern):
    ...
    def update_placeholder_shape_and_add_transpose(node: Node):
        ...
        from openvino.tools.mo.front.common.layout import (
            convert_shape,
            nchw_to_nhwc_permute,
            nhwc_to_nchw_permute,
        )
        from openvino.tools.mo.front.tf.extractors.utils import tf_tensor_shape
        from openvino.tools.mo.front.tf.partial_infer.tf import (
            add_node_def_to_subgraph,
            update_input_in_pbs,
        )
        ...
        nchw_to_nhwc_constant = tf_v1.constant(
            nchw_to_nhwc_permute, dtype=tf_v1.int32, name=nchw_to_nhwc_constant_name
        )
        nhwc_to_nchw_constant = tf_v1.constant(
            nhwc_to_nchw_permute, dtype=tf_v1.int32, name=nhwc_to_nchw_constant_name
        )
        ...
        for placeholder_name in node["input_nodes_names"]:
            ...
            if (
                len(cur_shape) == 4
            ):  # TODO think about better check that transpose is required
                ...
                transpose = tf_v1.transpose(
                    dummy_node, nchw_to_nhwc_constant, transpose_name
                )  # NCHW -> NHWC

                # add transpose operations to GraphDef after placeholders
                add_node_def_to_subgraph(
                    node,
                    transpose.op.node_def,
                    transpose_name,
    ...
    @staticmethod
    def add_reshape_before_op_node(
        graph: Graph, data_node_name: str, op_node_name: str, edge_attrs: dict
    ):
        """
    ...
    def add_reshape_after_data_node(graph: Graph, data_node_name: str):
        ...
        is_out_node = False
        if (
            len(data_node.out_nodes()) == 1
            and data_node.out_node().has("op")
            and data_node.out_node().op == "Result"
        ):
            is_out_node = True
        ...
        for out_node_name, edge_attrs in old_consumer_nodes_with_attrs:
            graph.add_edges_from([(reshaped_data_node_name, out_node_name, edge_attrs)])
    ...
    def add_reshapes_for_tf_subgraph_calls(graph: Graph):
        ...
        for src_node_name, dst_node_name, edge_attrs in list(graph.edges(data=True)):
            ...
            if (
                dst_node.kind == "op"
                and dst_node.has_valid("type")
                and dst_node.type == "TFCustomSubgraphCall"
                and src_node.has_valid("shape")
                and len(src_node.shape) != 4
            ):
                log.info(
                    "There is an data tensor of shape '{}' which goes into '{}' node".format(
    ...
    def add_sub_graph_call_output_tensors_transposes(node: Node):
        ...
        # dummy node which we can refer to as input in the transpose for the output node
        dummy_node = tf_v1.constant(
            value=[[[[1]]]], dtype=tf_v1.float32, name="random_dummy_name"
        )
        ...
        for out_tensor_name in node["output_tensors_names"]:
            ...
            if (
                len(output_tensors[int(out_port)].shape) == 4
            ):  # TODO think about better check whether transpose is required
                out_transpose_name = out_name + "_port_" + out_port + "_transpose"
                transpose = tf_v1.transpose(
                    dummy_node, nhwc_to_nchw_constant, name=out_transpose_name
                )
                ...
                add_node_def_to_subgraph(
                    node, new_node_def, position=len(node["nodes_order"])
                )
