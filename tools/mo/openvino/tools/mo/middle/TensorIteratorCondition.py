...
class LoopConditionMatcher(MiddleReplacementPattern):
    ...
    def check_dynamic_seq_len(graph: Graph, match: dict):
        ...
        if "Enter_2_less_data" in match:
            dynamic_seq_len = (
                dynamic_seq_len
                or match["Enter_2_less_data"].value is None
                or not np.array_equal(
                    match["Enter_1_less_data"].value, match["Enter_2_less_data"].value
                )
            )
    ...
    def find_and_replace_pattern(self, graph: Graph):
        apply_pattern(
            graph, **self.pattern(1), action=self.replace_pattern
        )  # pylint: disable=no-member
        apply_pattern(
            graph, **self.pattern(2), action=self.replace_pattern
        )  # pylint: disable=no-member
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        condition_attrs = dict(
            time=dict(init=init_2, step=step_2),
        ...
        condition_data = condition.create_node_with_data(
            inputs=condition_inp, data_nodes=[loop_condition, iterator_data]
        )
        ...
        if "GreaterEqual" in identity_ops:
            greater_equal_id = [
                n.id for n in iterator_data.out_nodes() if n.op == "GreaterEqual"
            ][0]
            ...
            if dynamic_seq_len:
                ...
                backedge_data = backedge.create_node_with_data(
                    inputs=[
                        match["init_2_data"],
                        match["add_2_data"],
                        condition_data[0],
...
class SimpleConditionMatcher(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        for time_consumer in time_dsts:
            if (
                time_consumer.node.soft_get("op")
                not in ["TensorIteratorInput", "TensorIteratorOutput"]
                and time_consumer.node.id != match["add_1"].id
            ):
        ...
        if other_time_consumers:
            ...
            new_identity_node = Identity(
                graph, dict(name=identity_node_name)
            ).create_node()
