...
class QuantizeDequantizeLinearResolver(MiddleReplacementPattern):
    ...
    def pattern(self):
        return dict(
            nodes=[
                ("const_input", dict(kind="op", op="Const")),
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        scale_zerop_is_exist = (
            quantize_node.is_in_port_connected(1)
            and quantize_node.is_in_port_connected(2)
            and dequantize_node.is_in_port_connected(1)
            and dequantize_node.is_in_port_connected(2)
        )
        ...
        scales_and_zerop_is_const = (
            q_scale.soft_get("type") == "Const"
            and dq_scale.soft_get("type") == "Const"
            and q_zerop.soft_get("type") == "Const"
            and dq_zerop.soft_get("type") == "Const"
        )
        scales_and_zerop_equals = np.array_equal(
            q_scale.value, dq_scale.value
        ) and np.array_equal(q_zerop.value, dq_zerop.value)
