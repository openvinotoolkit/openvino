...
class ConvertLayoutDependentOperations(MiddleReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in list(graph.nodes()):
            ...
            if (
                node.kind == "op"
                and node.has_valid("layout")
                and node.layout
                != indices_mapping[len(node.layout)][graph.graph["layout"]]
            ):
                input = node.in_node()
                ...
                input_order_const = Const(
                    graph,
                    {"name": input_permute_name + "/order", "value": permutation.perm},
                ...
                input_data_node = Op.create_data_node(
                    graph, node, {"shape": output.shape[permutation.perm]}, edge_attrs
                )

                output_permute_name = node_name + "/output_transpose"
                output_order_const = Const(
                    graph,
                    {"name": output_permute_name + "/order", "value": permutation.inv},
                ).create_node_with_data()
                output_permute_op = Transpose(
                    graph, {"name": output_permute_name}
                ).create_node_with_data(
                    [input_data_node, output_order_const], data_nodes=output
                )
