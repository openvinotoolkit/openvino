...
class CutInputHavingZeroDimFromConcat(MiddleReplacementPattern):
    ...
    def pattern(self):
        return dict(nodes=[("concat", dict(type="Concat"))], edges=[])
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        for port_num, node in enumerate(input_nodes):
            if (
                node.soft_get("type") == "Const"
                and len(node.shape) > 1
                and any(i == 0 for i in node.shape)
            ):
        ...
        if len(saved_ports) == 1:
            before_concat = (
                concat_node.in_port(saved_ports[0]).get_connection().get_source()
            )
