...
class ReverseTensorIteratorLSTM(MiddleReplacementPattern):
    ...
    def is_fusable_reverse_sequence(node: Node):
        ...
        if (
            ...
            and is_fully_defined(sequence_lengths)
            and is_fully_defined(seq_len)
        ):
            ...
            if gather_node.op != "Gather" or (
                np.all(gather_node.in_port(2).data.get_value() != [0])
                or np.all(gather_node.in_port(1).data.get_value() != [node.seq_axis])
            ):
                return False
            gather_node_2 = broadcast_node.in_port(1).get_source().node
            if gather_node_2.op != "Gather" or (
                np.all(gather_node_2.in_port(2).data.get_value() != [0])
                or np.all(
                    gather_node_2.in_port(1).data.get_value() != [node.batch_axis]
                )
            ):
                return False
            ...
            if shape_node.op != "ShapeOf":
                return False
            if (
                shape_node.in_port(0).get_source().node
                != node.in_port(0).get_source().node
            ):
                return False
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        assert (
            direct_reverse.batch_axis is None
            and inverse_reverse.batch_axis is None
            or direct_reverse.batch_axis == inverse_reverse.batch_axis
        )

        if not self.is_fusable_reverse_sequence(
            direct_reverse
        ) or not self.is_fusable_reverse_sequence(inverse_reverse):
        ...
        for port_map in [ti.input_port_map, ti.output_port_map]:
            for port in port_map:
                if (
                    "axis" in port
                    and port["axis"] is not None
                    and "external_port_id" in port
                ):
                    assert (
                        port["axis"] == direct_reverse.seq_axis
                    ), "axis == {} != {} == direct_reverse.seq_dim".format(
                        port["axis"], direct_reverse.seq_axis
                    )
