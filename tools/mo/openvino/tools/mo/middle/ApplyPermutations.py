...
class ApplyPermutation(MiddleReplacementPattern):
    ...
    def permute_data_nodes_attrs(graph: Graph):
        ...
        for node in graph.get_data_nodes():
            if not node.has_valid("permutation") or all(
                [
                    attrs.get("input_permutation", False)
                    for u, v, attrs in graph.out_edges(node.id, data=True)
                ]
            ):
                continue
            ...
            if (
                len(node.in_nodes()) != 0
            ):  # there are data nodes without input operation node inside the TensorIterator
                ...
                if is_output_data_in_correct_layout(node.in_node(0), edge_attrs["out"]):
                    log.debug(
                        'Do not permute data node attrs for node "{}" output port "{}"'.format(
                            node.in_node(0).id, edge_attrs["out"]
                        )
                    )
                    continue
        ...
        for node in graph.get_op_nodes():
            if node.has_valid("permute_attrs") and not node.has_and_set("nchw_layout"):
                try:
                    ...
                    raise Error(
                        "Can't permute attrs for node {}. Error message: {}".format(
                            node.id, e
                        )
                    )
    ...
    def permute_input_data(graph: Graph):
        for node in graph.get_op_nodes():
            input_permutations = [
                (in_port, edge_attrs["input_permutation"])
                for in_port, edge_attrs in node.in_edges().items()
                if edge_attrs.get("input_permutation") is not None
            ]
            for in_port, input_perm in input_permutations:
                ...
                port = int(port)
                port_to_check = (
                    node.in_port(port) if direction == "input" else node.out_port(port)
                )
                permutation_data_node = get_node_with_permutation(node, port_info)

                if (
                    permutation_data_node.has_and_set("permutation")
                    and not is_input_data_in_correct_layout(node, in_port)
                    and check_shape(port_to_check)
                ):
