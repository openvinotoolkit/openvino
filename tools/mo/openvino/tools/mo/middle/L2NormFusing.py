...
class L2NormToNorm(MiddleReplacementPattern):
        ...
        return dict(
            nodes=[
                ...
                (
                    ...
                    dict(
                        kind="data",
                        value=lambda x: np.all(x == -0.5) if x is not None else False,
                    ),
                ),
                ...
                (
                    ...
                    dict(
                        kind="data",
                        value=lambda x: np.all(x == 2) if x is not None else False,
                    ),
                ),
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if len(axes) == 1 and (
            (
                input_rank == 4
                and get_features_dim(graph.graph["layout"], input_rank) == axes[0]
            )
            or (input_rank != 4 and axes[0] == 1)
        ):
            transformation_applicable = True

        if not transformation_applicable:
            log.debug(
                "OV doesn't support l2 normalization with reduction along axes {}.".format(
                    axes
                )
            )
            return
        ...
        normalize_node = create_op_node_with_second_input(
            ...
            NormalizeL2Op,
            axes,
            {"name": output_name, "eps_mode": "max", "eps": y},
        )
        ...
        rename_nodes(
            [
                (match["l2_normalize"], output_name + "/TBR"),
