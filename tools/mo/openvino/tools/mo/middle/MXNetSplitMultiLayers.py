...
class MXNetSplitLayersToRNNSequence(MiddleReplacementPattern):
        ...
        return dict(
            nodes=[
                (
                    "rnn_layer",
                    dict(
                        kind="op", type="RNNSequence", format="mxnet", multilayers=True
                    ),
                ),
    ...
    def split_multilayer_cell(self, graph: Graph, match: dict):
        ...
        bsize = (
            2 * rnn_layer.hidden_size * direction * num_layers
        ...
        other_layer_params_size = (
            rnn_layer.hidden_size * direction + rnn_layer.hidden_size + 2
        ) * size
        assert params.size == (
            first_layer_params_size + (num_layers - 1) * other_layer_params_size
        )
        ...
        for l in range(num_layers):
            params_layer_size = (
                first_layer_params_size if l == 0 else other_layer_params_size
            )

            layer_params_w = param_w[
                params_layer_size_count : params_layer_size_count
                + (params_layer_size - layer_bsize)
            ].copy()
            layer_params_b = param_b[
                layer_bsize * l : layer_bsize * l + layer_bsize
            ].copy()
            layer_params = np.concatenate((layer_params_w, layer_params_b), axis=0)
            params_layer_size_count = (
                params_layer_size_count + params_layer_size - layer_bsize
            )
            ...
            params_value_node = Const(
                rnn_layer.graph,
                dict(
                    name=name + "/LayerSplittedParamsLSTM/{}/".format(l),
                    value=layer_params,
                ),
            ).create_node_with_data()

            if have_hidden:
                layer_hidden_state = hidden_state_value[
                    l * direction : l * direction + direction
                ]
                hidden_state_value_node = Const(
                    rnn_layer.graph,
                    dict(
                        name=name + "/LayerSplittedHiddenState/{}/".format(l),
                        value=layer_hidden_state,
                    ),
                ).create_node_with_data()
            else:
            ...
            if have_cell:
                layer_cell_state = cell_state_value[
                    l * direction : l * direction + direction
                ]
                cell_state_value_node = Const(
                    rnn_layer.graph,
                    dict(
                        name=name + "/LayerSplittedCellState/{}/".format(l),
                        value=layer_cell_state,
                    ),
                ).create_node_with_data()
            else:
