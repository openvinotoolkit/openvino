...
def get_concat_after_split(split: Node) -> Optional[Node]:
    ...
    split_outputs = [
        d.node
        for _, p in split.out_ports().items()
        for d in p.get_connection().get_destinations()
    ]
    ...
    groups_of_inputs = [
        [d.idx for d in p.get_connection().get_destinations()]
        for _, p in split.out_ports().items()
    ]
    ...
    dest_inputs = [
        p.get_source().node for p in dest.in_ports().values() if not p.disconnected()
    ]
...
def get_split_scale(split: Node) -> int:
    ...
    fractional_part = (
        num_of_split_dests / num_of_split_out_ports
        - num_of_split_dests // num_of_split_out_ports
    )
    assert (
        fractional_part == 0
    ), "Number of output ports of Split must be multiple of number of inputs of Concat"
...
def replace_interpolate_pattern(graph: Graph, match: dict):
    ...
    axis_node = Const(
        graph, {"name": split_node_name + "/axis", "value": int64_array([axis])}
    ).create_node()

    shape_node = Shape(graph, dict(name=split_node_name + "/Shape")).create_node()
    scales_node = Const(
        graph, dict(name=split_node_name + "/scales", value=scale)
    ).create_node()
    ...
    interp_node = Interpolate(
        graph,
        dict(
            name=split_node_name + "/Interpolate",
            mode="nearest",
            antialias=0,
