...
from openvino.tools.mo.front.tf.graph_utils import (
    add_constant_to_negative_values,
...
class SSliceComplex(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: Dict[str, Node]):
        ...
        if real_input.id != imag_input.id:
            log.debug(
                "The pattern does not correspond to operation for complex tensor. Different inputs."
            )
            return
        ...
        for dst in complex_node.out_port(0).get_connection().get_destinations():
            ...
            if after_complex_node.type == "Roll":
                add_constant_to_negative_values(
                    after_complex_node, 2, int64_array(emulated_complex_tensor_rank)
                )

        input_slices_have_ellipsis = (
            len(np.argwhere(real_slices == Ellipsis).flatten()) != 0
        )
        ...
        if (
            slice_dim_for_real_part == emulated_complex_tensor_rank - 1
            or input_slices_have_ellipsis
        ):
            complex_node.out_port(0).get_connection().set_source(
                strided_slice_real.in_port(0).get_source()
            )
        else:
            complex_node_name = complex_node.soft_get("name", complex_node.id)
            perm = int64_array(
                [
                    *range(0, slice_dim_for_real_part),
                    *range(slice_dim_for_real_part + 1, emulated_complex_tensor_rank),
                    slice_dim_for_real_part,
                ]
            )
            transpose = create_op_with_const_inputs(
                graph, Transpose, {1: perm}, {"name": complex_node_name + "/cmplx"}
            )
            ...
            rename_nodes(
                [
                    (complex_node, complex_node_name + "/to_be_removed"),
                    (transpose, complex_node_name),
                ]
            )
