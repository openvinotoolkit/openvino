...
class CanBeFused:
    ...
    def _compare_attributes_of_interpolate4(self, first: Node, second: Node) -> bool:
        ...
        for attr in self.default_values_for_opset4.keys():
            ...
            if first.soft_get(attr, default=default_value) != second.soft_get(
        ...
        for attr in ["pads_begin", "pads_end"]:
            if not np.array_equal(
                first.soft_get(attr, default=self.default_pads),
    ...
    def _compare_attributes(self, first: Node, second: Node) -> bool:
        ...
        if fst_opset not in ["opset1", "opset4"]:
            ...
            raise Error(
                "Unsupported opset {} for nodes with names {} and {}".format(
                    fst_opset, fst_name, snd_name
                )
            )
...
def get_interpolate_attributes(node: Node) -> dict:
    ...
    if opset in opset_to_default_values:
        ...
        raise Error(
            "Unsupported opset {} for node with name {}.".format(
...
def replace_sequence(seq: List[Node], graph: Graph):
    ...
    if opset == "opset1":
        ...
        interp_node = create_op_with_const_inputs(
            graph, Interpolate, {1: sizes}, attributes
        )
        ...
        interp_node = create_op_with_const_inputs(
            graph, Interpolate, {1: sizes, 2: scales, 3: axes_of_node}, attributes
        )
    ...
    rename_nodes(
        [
            (last_interp_node, last_interp_node_name + "/delete"),
...
class InterpolateSequenceToInterpolate(MiddleReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        log.debug(
            "Enabled replacement of a sequence of Interpolate layers with one Interpolate layer."
        )
        interps = [
            n
            for n in graph.pseudo_topological_sort()
            if n.kind == "op" and n.op == "Interpolate"
        ]
        fuser = CanBeFused()
