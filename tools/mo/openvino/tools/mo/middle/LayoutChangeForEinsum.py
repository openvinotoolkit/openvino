...
from openvino.tools.mo.middle.InsertLayoutPropagationTransposes import (
    is_input_data_in_correct_layout,
...
class LayoutChangeForEinsum(MiddleReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for einsum in graph.get_op_nodes(type="Einsum"):
            ...
            assert einsum.has_valid(
                "equation"
            ), "Equation attribute is mandatory" " for Einsum node {}".format(
                einsum_name
            )
            equation = einsum.equation
            connected_in_ports = [
                port for port in einsum.in_ports().values() if not port.disconnected()
            ]
            ...
            permuted_equation, are_inputs_adjusted, is_output_adjusted = (
            ...
            if not is_output_adjusted:
                ...
                InsertTransposes.insert_transpose(
                    graph, einsum.out_port(0), before_input=False
                )
