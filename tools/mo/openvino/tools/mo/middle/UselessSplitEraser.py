...
class UselessSplitEraser(MiddleReplacementPattern):
    ...
    def pattern(self):
        return dict(nodes=[("split", {"op": "Split", "num_splits": 1})], edges=[])
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        assert (
            node.soft_get("input_port", 0) == 0
        ), "Internal attribute `input_port` was not resolved on front phase, broken Split {}".format(
            name
        )
        assert len(node.out_ports()) == 1

        node.out_port(0).get_connection().set_source(
            node.in_port(0).get_connection().get_source()
        )
