...
from openvino.tools.mo.middle.MakeKaldiConstReshapable import (
...
def check_inputs(graph: Graph):
    ...
    if len(inputs) == 1:
        ...
        if inputs[0].name == "ivector":
            ...
            raise Error(
                "There are 2 inputs for Kaldi model but we can't find out which one is ivector. "
                + "Use name 'ivector' for the corresponding input"
            )
    else:
        raise Error(
            "There are {} inputs for Kaldi model but we expect only 1 or 2".format(
                len(inputs)
            )
        )
...
class AddSelectBeforeMemoryNodePattern(MiddleReplacementPattern):
    ...
    def run_after(self):
        ...
        from openvino.tools.mo.middle.ReplaceMemoryOffsetWithSplice import (
            ReplaceMemoryOffsetWithMemoryNodePattern,
        )

        return [ReplaceMemoryOffsetWithMemoryNodePattern, MergeNeighborSplicePattern]
            ...
            if node.frame_time < 0:
                ...
                if node.op == "Splice":
                    if node.in_port(0).get_source().node.frame_time == -1:
                        continue
                    node.frame_time = (
                        node.in_port(0).get_source().node.frame_time
                        + len(node.context)
                        - 1
                    )
                # crop often used to get concrete time frame, set frame_time correctly for this case
                    ...
                    if node.in_port(0).get_source().node.frame_time == -1:
                        continue
                    if (
                        node.in_port(0).get_connection().get_source().node.op
                        == "Splice"
                    ):
                        ...
                        new_delay = (
                            splice_node.context[node.offset[0] // node.dim[0]]
                            - splice_node.context[0]
                        )
                        node.frame_time = (
                            splice_node.in_port(0).get_source().node.frame_time
                            + new_delay
                        )
                    else:
                ...
                # for node with several inputs frame_time = maximum of delays from branches
                    ...
                    for inp in node.in_ports():
                        ...
                        if (
                            in_node.frame_time > node.frame_time
                            and in_node.frame_time != -1
                        ):
    ...
    def insert_select(graph: Graph, node: Node):
        ...
        existing_counters = find_pattern_matches(
            ...
            edges=[
                ...
                ("concat", "concat_data"),
                ("concat_data", "mem_out"),
                ("concat_data", "crop_out"),
                ("crop_out", "crop_out_data"),
        ...
        if counter_match is not None:
            ...
            input_port = Node(graph, inverse_dict(counter_match)["crop_out"]).out_port(
                0
            )
        else:
            ...
            concat = Concat(
                graph, {"name": "concat_ones", "in_ports_count": 2, "axis": 1}
            ).create_node()
        ...
        cast_in = Equal(
            graph, {"name": input_port.node.name + "/cast_to_bool"}
        ).create_node()
    ...
    def find_and_replace_pattern(self, graph: Graph):
        if np.all(
            [
                node.soft_get("name", node.id) == "iteration_number_out"
