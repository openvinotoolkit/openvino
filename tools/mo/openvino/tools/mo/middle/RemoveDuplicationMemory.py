    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        for child_port in mem_parent.get_destinations():
            ...
            if (
                child["op"] == "Splice"
                and child.id != mem.id
                and set(child["context"]).issubset(set(context))
            ):
                ...
                for child_of_child in child.out_port(0).get_destinations():
                    ...
                    if out_transfer["op"] == "Crop":
                        ...
                        crop_node = Crop(
                            graph,
                            {
                                "name": graph.unique_id(prefix="Splice_crop_"),
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        for child_port in mem_parent.get_destinations():
            ...
            if (
                ...
                and child.id != mem.id
                and (
                    child["context"][0] == context[-1]
                    or child["context"][0] == context[-1]
                )
            ):
                ...
                for out_port_rem in rem_node.out_port(0).get_destinations():
                    ...
                    if out_transfer["op"] == "Crop":
                        ...
                        out_transfer["offset"] = (
                            out_transfer["offset"]
                            + (len(new_context) - len(rem_node.context)) * mem_shape[-1]
                        )
                        ...
                        crop_node = Crop(
                            ...
                            {
                                "name": graph.unique_id(prefix="Splice_crop_"),
                                "offset": (len(new_context) - len(rem_node.context))
                                * mem_shape[-1],
                                "dim": mo_array(
                ...
                new_shape[1] += (
                    rem_node.out_port(0).data.get_shape()[1]
                    - rem_node.in_port(0).data.get_shape()[1]
                )
