...
class BlockLSTMtoLSTMSequenceSingleFirstOutput(MiddleReplacementPattern):
    ...
    def run_before(self):
        from openvino.tools.mo.middle.LSTMRNNSequenceToTensorIterator import (
    ...
    def run_after(self):
        from openvino.tools.mo.middle.RNNSequenceNormalizeToIE import (
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        connected_out_ports = [
            port_idx
            for port_idx, port in block_lstm.out_ports().items()
        ...
        assert (
            hidden_size is not dynamic_dimension
        ), "OpenVINO does not support BlockLSTM with dynamic hidden_size."
        ...
        assert (
            weights is not None and biases is not None
...
class BlockLSTMtoLSTMSequence(MiddleReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        attrs = dict(
            graph.get_edge_data(match["gather_1"].id, match["gather_1_data"].id)[0]
        )
        attrs.update({"out": 2})
        graph.remove_edge(
            match["BlockLSTM"].id, match["concatenated_cell_states_data"].id
        )
        ...
        data_to_mul = [
            n
            for n in match["mul"].in_nodes().values()
        ...
        attrs = dict(
            graph.get_edge_data(
                match["BlockLSTM"].id, match["concatenated_hidden_states"].id
            )[0]
        )
        ...
        conacenated_child_node_id = (
            list_of_concatenated_hidden_states_children_node_ids[0]
        )
        ...
        attrs = dict(
            graph.get_edge_data(match["gather_0"].id, match["gather_0_data"].id)[0]
        )
        attrs.update({"out": 1})
