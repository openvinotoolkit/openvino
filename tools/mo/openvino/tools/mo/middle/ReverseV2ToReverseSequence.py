...
class ReverseToReverseSequence(MiddleReplacementPattern):
    ...
    def run_before(self):
        from openvino.tools.mo.middle.reverse_tensor_iterator import (
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for reverse in reverse_nodes:
            ...
            broadcast_node = Broadcast(
                graph, {"name": reverse_name + "/Broadcast"}
            ).create_node()
            ...
            reverse_sequence = ReverseSequence(
                graph,
                {"name": reverse_name, "seq_axis": seq_axis, "batch_axis": batch_axis},
            ).create_node()
