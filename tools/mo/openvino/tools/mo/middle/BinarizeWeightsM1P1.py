...
class BinarizeWeightsM1P1(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        applicable = [
            pair
            for pair in match["operator"].multiplication_transparent_ports
            if pair[0] == port
        ]
        if len(applicable) == 0:
            return
        ...
        mult_term = (
            quantize.in_node(3) if np.all(output_high == 0) else quantize.in_node(4)
        )
        ...
        reshape = Reshape(
            graph, {"name": quantize_name + "/Reshape"}
        ...
        if len(match["quantized"].out_nodes()) > 1:
            ...
            return
        power_of_exponent = Const(
            graph,
            {"name": quantize_name + "/DivNormalize/Power", "value": mo_array(-1.0)},
        ).create_node_with_data()
