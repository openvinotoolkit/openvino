...
def create_ss_interval_border(
    ...
    clamp = create_op_with_const_inputs(
        graph,
        Clamp,
        port_value_dict={1: np.iinfo(np.int32).min, 2: np.iinfo(np.int32).max},
    ...
    cast = Cast(
        graph, dict(name=node_name + "/CastToI64", dst_type=np.int64)
    ).create_node()
    ...
    for value_idx, port_idx in enumerate(axes):
        ...
        value = create_op_with_const_inputs(
            graph,
            Gather,
            port_value_dict={1: int64_array([value_idx]), 2: int64_array(0)},
    ...
    for port_idx in range(len(shape)):
        if not concat.is_in_port_connected(port_idx):
            ...
            const = Const(
                graph, dict(name=node_name + "/Const", value=int64_array([0]))
            ).create_node()
...
class ConvertSlice(MiddleReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in graph.get_op_nodes(op="Slice"):
            ...
            ss_begin = create_ss_interval_border(
                graph, node.in_port(1).get_source(), input_shape, axes, node_name
            )
            ss_end = create_ss_interval_border(
                graph, node.in_port(2).get_source(), input_shape, axes, node_name
            )
            node.in_port(1).disconnect()
            node.in_port(2).disconnect()
            rename_nodes(
                [(ss_begin, node_name + "/Begin"), (ss_end, node_name + "/End")]
            )
            ...
            if node.is_in_port_connected(4):
                ...
                assert (
                    steps is not None
                ), "The input with steps is not constant for node {}".format(node_name)
            else:
            ...
            ss_strides = Const(
                graph, dict(name=node_name + "/Strides", value=ss_step)
            ).create_node()
