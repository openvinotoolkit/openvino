...
from openvino.tools.mo.ops.elementwise import Add, Mul, Pow
...
class FusedBatchNormNonConstant(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        node = match["op"]
        if (
            node.data_format != b"NHWC"
            or len(node.in_nodes()) != 5
            or node.in_node(0).value is not None  # input
            or node.in_node(1).value is None  # scale
            or node.in_node(2).value is None  # offset
            or node.in_node(3).value is not None  # mean
            or node.in_node(4).value is not None  # variance
            or node.in_node(1).value.ndim != 1
            or node.in_node(2).value.ndim != 1
        ):
            return
        ...
        neg_const = Const(
            graph, dict(value=mo_array(-1), name=node.name + "/mean_negate_")
        )
        ...
        mean_arg = mean_add.create_node_with_data(
            [
                ...
                mean_negate.create_node_with_data(
                    [node.in_node(3), neg_const.create_node_with_data()]
                ),
            ]
        )

        shift_const = Const(
            graph, dict(value=node.eps, name=node.name + "/variance_denom_shift_const_")
        )
        power_const = Const(
            graph, dict(value=-0.5, name=node.name + "/variance_denom_power_const_")
        )
                ...
                variance_denom_power.create_node_with_data(
                    [
                        variance_denom_shift.create_node_with_data(
                            [node.in_node(4), shift_const.create_node_with_data()]
                        ),
        ...
        shift_add.create_node_with_data(
            [
                scale_mul.create_node_with_data([variance_arg, node.in_node(1)]),
                node.in_node(2),
            ],
