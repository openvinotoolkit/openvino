...
from openvino.tools.mo.middle.passes.fusing.helpers import (
    get_tensor_in_port,
    get_value_in_port,
)
...
class EltwiseChecker(MiddleReplacementPattern):
    ...
    def mark_eltwise_node(self, node, feature_channel=None):
        ...
        assert len(value_shape) <= len(tensor_shape), (
            ...
            "But constant input rank is larger than tensor input rank, that is inconsistent".format(
                node.name
            )
        )
        ...
        feature_dim = (
            min(1, tensor_shape.size - 1)
            if node.graph.graph["layout"] == "NCHW"
            else -1
        )
        ...
        if not np.array_equal(broadcasted_value_shape, ones) and not np.array_equal(
            broadcasted_value_shape, possible_shape
        ):
