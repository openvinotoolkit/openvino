...
class UnsqueezeTileReshapeBlockToInterpolate(MiddleReplacementPattern):
    ...
    def pattern(self):
        log.debug(
            "Enabled replacement of a sequence of Unsqueeze, Tile, Reshape with Interpolate."
        )
        return dict(
            nodes=[
    ...
    def is_applicable(match: dict) -> bool:
        ...
        second_input_of_unsqueeze = (
            unsqueeze_node.in_port(1).get_connection().get_source().node
        )
        if (
            not second_input_of_unsqueeze.has_valid("value")
            or len(second_input_of_unsqueeze.value) != 1
        ):
            return False
        ...
        second_input_of_tile = (
            match["tile"].in_port(1).get_connection().get_source().node
        )
        ...
        if not np.array_equal(
            expected_tile_constant, float32_array(second_input_of_tile.value)
        ):
            return False
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        interp_node = create_op_with_const_inputs(
            graph,
            Interpolate,
            {2: float_scale, 3: int64_array([axis])},
            {
                "mode": "nearest",
                "antialias": 0,
        ...
        rename_nodes(
            [(reshape_node, reshape_name + "/delete"), (interp_node, reshape_name)]
        )
