...
class LayoutChangeForConstantShapePaths(MiddleReplacementPattern):
    ...
    def find_shape_subgraph_endpoints(
        self, out_ports: List[Port], visited: set = None, action: callable = None
    ) -> Set[Port]:
        """
    ...
    def find_and_replace_pattern(self, graph: Graph):
        ...
        for in_port in shape_sub_graph_end_points:
            ...
            should_insert_gather = (
                should_switch_layout and len(shape) == 1 and shape.item(0) in [4, 5]
            )
            ...
            if should_insert_gather:
                ...
                index = int64_array(
                    [0, shape.item(0) - 1, *list(range(1, shape.item(0) - 1))]
                )
                gather = create_op_with_const_inputs(
                    graph,
                    op=Gather,
