...
class PreserveRuntimeInfo(MiddleReplacementPattern):
    ...
    def add_old_api_map_order_into_rt_info(op: Node):
        ...
        assert op.has(
            "rt_info"
        ), "Unable to preserve runtime information for node with name={}".format(op)
    ...
    def preserve_rt_info(graph: Graph):
        for op in graph.get_op_nodes(type="Parameter"):
            ...
            if (
                "auto_disable_nhwc_to_nchw" in graph.graph["cmd_params"]
                and graph.graph["cmd_params"].auto_disable_nhwc_to_nchw
            ):
                ...
                op.has_valid("permute_attrs")
                and not op.has_and_set("nchw_layout")
                and op.out_node(0).has_valid("permutation")
            ):
