...
class MarkSubGraphsWithCorrectLayout(MiddleReplacementPattern):
    ...
    op_conditions = [
        lambda n: n.soft_get("op") == "MatMul"
        and any(
            [len(port.data.get_shape()) in (4, 5) for port in n.in_ports().values()]
        ),
    ]
    ...
    def get_input_nodes(node: Node):
        return [
            src_port.get_source().node
            for src_port in node.in_ports().values()
            if not src_port.disconnected()
        ]
        ...
        while len(d) != 0:
            ...
            for next_node in next_nodes:
                if next_node not in visited and (
                    condition is None or condition(next_node)
                ):
                    d.append(next_node)
        return result
        ...
        for node_condition in self.op_conditions:
            for node in graph.get_op_nodes():
                if node_condition(node):
                    ...
                    forward_visited_nodes = self.bfs(
                        [node], visited, condition_forward, True
                    )
                    backward_visited_nodes = self.bfs(
                        [node], visited, condition_backward, False
                    )
                    ...
                    for back_node in backward_visited_nodes:
                        for input_node in self.get_input_nodes(back_node):
                            if (
                                input_node not in backward_visited_nodes
                                and not condition_forward(input_node)
                            ):
                    ...
                    for forward_node in forward_visited_nodes:
                        for output_node in self.get_output_nodes(forward_node):
                            if (
                                output_node not in forward_visited_nodes
                                and not condition_backward(output_node)
                            ):
    ...
    def get_weighted_layer_type_to_in_weights_port():
        get_weights_port_index = lambda node: (
            node.weights_index if node.has_valid("weights_index") else 1
        )
    ...
    def insert_permute_inputs_before_dynamic_weights_subgraph(
        dynamic_subgraphs: Set[Node] = None,
    ):
        """
        ...
        for node in dynamic_subgraphs:
            ...
            if node_type not in ["Const", "Parameter", "ShapeOf"]:
                ...
                for idx in [
                    ...
                    and port.get_source().node not in dynamic_subgraphs
                ]:
    ...
    @staticmethod
    def walk_up_from_in_ports_to_out_ports(
        in_ports: Set[Port], out_ports: Set[Port], port_condition=None
    ):
        r""" "
        ...
        while len(deque_of_in_ports):
            ...
            if (
                in_port in visited_ports
            ):  # do not check visited_nodes as search is based on ports
                continue
            visited_ports.update({in_port, in_port.get_source()})
            if (
                in_port.get_source() in out_ports
            ):  # reached source marked to stop the search
                if not len(
                    in_port.get_source().node.in_ports()
                ):  # for Constants and Parameters to be visited
                    visited_nodes.add(in_port.get_source().node)
                continue
            for idx, port in source_node.in_ports().items():
                if not port.disconnected() and (
                    not port_condition or port_condition(source_node, idx)
                ):
    ...
    def is_not_weight_port(node: Node, idx: int):
        w_types_to_in_port_dict = (
            MarkSubGraphsWithCorrectLayout.get_weighted_layer_type_to_in_weights_port()
        )
        node_type = node.soft_get("type")
        return (
            node_type in w_types_to_in_port_dict.keys()
            and idx != w_types_to_in_port_dict[node_type](node)
        )
    ...
    def get_ports_and_nodes_on_weights(graph):
        ...
        for node in nodes:
            ...
            assert node.is_in_port_connected(
                weight_port_idx
            ), "Unexpected port configuration of {} node with name=`{}`".format(
                node_type, node.soft_get("name", node.id)
            )
        ...
        # weights
        ports_w, nodes_w = (
            MarkSubGraphsWithCorrectLayout.walk_up_from_in_ports_to_out_ports(
                weight_ports, start_ports
            )
        )
        ...
        ports_d, nodes_d = (
            MarkSubGraphsWithCorrectLayout.walk_up_from_in_ports_to_out_ports(
                result_ports,
                start_ports,
                MarkSubGraphsWithCorrectLayout.is_not_weight_port,
            )
        )
        nodes_dif = nodes_w.difference(nodes_d)
        nodes_in_w = MarkSubGraphsWithCorrectLayout.insert_permute_inputs_before_dynamic_weights_subgraph(
            nodes_dif
        )
    ...
    def get_ports_and_nodes_on_shape_subgraphs(graph):
        shape_sources = {
            shape_of.out_port(0) for shape_of in graph.get_op_nodes(type="ShapeOf")
        }
