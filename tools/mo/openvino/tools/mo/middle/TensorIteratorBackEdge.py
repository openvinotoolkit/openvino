...
from openvino.tools.mo.ops.TensorIterator_ops import (
    TensorIteratorBackEdge,
...
class BackEdgesMatching(MiddleReplacementPattern):
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        if 0 in match["Switch_1"].out_nodes():
            Exit = (
                match["Switch_1"].out_node(0).out_node(0)
            )  # Switch -> Switch_data -> Exit
        ...
        backedge = TensorIteratorBackEdge(
            graph, dict(name=match["Identity_1"].name + "/TensorIteratorBackEdge_")
        )
        backedge.create_node_with_data(
            inputs=[
                match["Enter_1_data"],
                from_body_data,
                match["condition_cond_data"],
            ],
