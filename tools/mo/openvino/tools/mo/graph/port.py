...
class Port:
    ...
    def __eq__(self, other):
        return (
            ...
            and self.node.graph == other.node.graph
            and self.node.id == other.node.id
    ...
    def __setattr__(self, key, value):
        edge = (
            self.node.in_edge(self.idx, control_flow=self.control_flow)
            if self.type == "in"
            else self.node.out_edge(self.idx, control_flow=self.control_flow)
        )
        edge[key] = value
    ...
    def __getattr__(self, item):
        ...
        if edge.get(item) is None:
            raise Error(
                "Edge from {}_port {} at node {} has no attribute {}".format(
                    self.type, self.idx, self.node.name, item
                )
            )
        return edge[item]
    ...
    def _create_data_if_necessary(self):
        if self.node.graph.stage == "front":
            raise Error(
                "_create_data_if_necessary method is not applicable for front Graph phase!"
            )
        if self.type == "in":
            raise Error(
                "_create_data_if_necessary method is not applicable for 'in' Port type!"
            )
    ...
    def _set_shape(self, shape):
        if self.node.graph.stage == "front":
            ...
            if self.type == "in":
                assert (
                    self.node.in_node(self.idx, control_flow=self.control_flow).value
                    is None
                )
                self.node.in_node(self.idx, control_flow=self.control_flow).shape = (
                    shape_array(shape)
                )
            else:
                data_node = self.node.out_node(self.idx, control_flow=self.control_flow)
                assert (
                    data_node.value is None
                    or self.node.has_and_set("override_output_shape")
                    or strict_compare_tensors(
                        data_node.soft_get("force_shape", data_node.shape),
                        shape_array(shape),
                    )
                )
                self.node.out_node(self.idx, control_flow=self.control_flow).shape = (
                    shape_array(shape)
                )
    ...
    def _get_value(self):
        if self.node.graph.stage == "front":
            ...
            if self.type == "in":
                if self.idx in self.node.in_nodes(
                    control_flow=self.control_flow
                ) and self.node.in_node(
                    self.idx, control_flow=self.control_flow
                ).has_valid(
                    "value"
                ):
                    return self.node.in_node(
                        self.idx, control_flow=self.control_flow
                    ).value
            else:
                if self.idx in self.node.out_nodes(
                    control_flow=self.control_flow
                ) and self.node.out_node(
                    self.idx, control_flow=self.control_flow
                ).has_valid(
                    "value"
                ):
                    return self.node.out_node(
                        self.idx, control_flow=self.control_flow
                    ).value
        return None
    ...
    def _set_value(self, value):
        if self.node.graph.stage == "front":
            ...
            data_node_caller = (
                self.node.in_node if self.type == "in" else self.node.out_node
            )
            data_node = data_node_caller(self.idx, control_flow=self.control_flow)
            const_node = (
                data_node.in_node(control_flow=self.control_flow)
                if self.type == "in"
                else self.node
            )

            force_shape = data_node.soft_get(
                "force_shape", const_node.soft_get("force_shape", None)
            )
        ...
        if self.node.graph.stage == "front":
            ...
            if self.type == "in":
                if self.idx in self.node.in_nodes(
                    ...
                    return self.node.in_node(self.idx, control_flow=self.control_flow)[
                        item
                    ]
            else:
                if self.idx in self.node.out_nodes(
    ...
    def get_source(self):
        ...
        assert self.type != "out", "Can't get source for output port at {} node".format(
            self.node.name
        )
        ...
        if self.node.graph.stage == "front":
            for n, d in self.node.get_inputs(control_flow=self.control_flow):
                if d["in"] == self.idx:
                    ...
                    producer_ports.append(
                        node.out_port(d["out"], control_flow=self.control_flow)
                    )
                    has_producer = True
            if not has_producer:
                return None
        ...
        if len(producer_ports) != 1:
            if self.node.graph.strict_mode:
                raise Error(
                    'Something bad has happened with graph! Data node "{}" has {} producers'.format(
    ...
    def get_destination(self):
        ...
        if len(consumer_ports) > 1:
            raise Error(
                "The number of destinations for {} node at {} port is {}".format(
                    self.node.name, self.idx, len(consumer_ports)
                )
            )
    ...
    def get_destinations(self):
        assert (
            self.type != "in"
        ), "Can't get destinations for input port at {} node".format(self.node.name)
        ...
        for n, d in producer_node.get_outputs(
            edge_attr={"out": self.idx} if self.node.graph.stage == "front" else None,
            control_flow=self.control_flow,
        ):
            node = Node(self.node.graph, n)
    ...
    def add_tensor_names(self, tensor_names: list, port_renumber: bool = False):
        ...
        for tensor_name in tensor_names:
            assert isinstance(
                tensor_name, str
            ), "Tensor names elements should be strings."
    ...
    def get_tensor_debug_info(self, port_renumber: bool = False):
        ...
        assert (
            self.type != "in"
        ), "Can't get tensor debug info for input port at {} node".format(
            self.node.name
        )
    ...
    def remove_debug_info(self, port_renumber: bool = False):
        ...
        assert (
            self.type != "in"
        ), "Tensor debug info is not defined for input port at {} node".format(
            self.node.name
        )
        ...
        if self.type == "out":
            ...
            if self.node.graph.stage == "front":
                ...
                for port in consumer_ports:
                    ...
                    for key, val in self.node.graph.get_edge_data(
                        src_node, dst_node
                    ).items():
                        if val["in"] == port.idx:
    ...
    def _get_data_type(self):
        ...
        if self.type == "out":
            if (
                node.has_valid("_out_port_data_type")
                and self.idx in node._out_port_data_type
            ):
            ...
            if value_data_type is not None:
                value_data_type = value.dtype if value is not None else None
                log.debug(
                    "The precision of the output port {} of node {} is determined from the data node as {}"
            ...
            if value_data_type is not None:
                log.debug(
                    "The precision of the input port {} of node {} is determined from the data node as {}"
                    "".format(self.idx, self.node.name, value_data_type)
                )
            ...
            if (
                value_data_type is not None
                and source_port_data_type is not None
                and value_data_type != source_port_data_type
            ):
                log.warning(
                    "Inconsistent data type of the data node and port attribute for port {} of node {}: {} vs "
                    "{}. Return data type of the data node.".format(
                        self.idx, self.node.name, value_data_type, source_port_data_type
                    )
                )
    ...
    def get_data_type(self):
        ...
        if data_type is None:
            raise Error(
                "The data type for {} port {} of node {} is not defined".format(
                    self.type, self.idx, self.node.name
                )
            )
        return data_type
    ...
    def set_data_type(self, data_type, override=False):
        ...
        if (
            self.idx in node._out_port_data_type
            and data_type != node._out_port_data_type[self.idx]
            and not override
        ):
            raise Error(
                "Trying to override data type for output port {} of operation {}: from {} to {}".format(
