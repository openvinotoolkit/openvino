...
class Connection:
    ...
    def _get_value(self):
        if self.graph.stage == "front":
            return None
        return self.source.node.out_node(
            self.source.idx, control_flow=self.control_flow
        ).value
    ...
    @staticmethod
    def _get_new_tensor_debug_info(
        attributes_save_mode: str, source_attrs: dict, dest_attrs: dict
    ):
        source_debug_info = []
    ...
    def get_destination(self):
        if self.destinations and len(self.destinations) > 1:
            raise Error(
                "Connection has more than one destination: {}".format(
    ...
    def set_source(self, port, attributes_save_mode=None):
        ...
        if port.type == "in":
            raise Error(
                "Wrong port type in set_source method. Should be 'out' but given 'in'"
            )
        ...
        if self.graph.stage == "front":
            ...
            for dst_port in self.destinations:
                ...
                if u is not None:
                    ...
                    new_tensor_info = self._get_new_tensor_debug_info(
                        attributes_save_mode, attrs, edge_attrs
                    )
            ...
            if (
                ...
                and self.source.idx in self.source.node.out_nodes()
            ):
            ...
            new_tensor_info = self._get_new_tensor_debug_info(
                attributes_save_mode, port_out_data.attrs(), attrs
            )
            ...
            for dst_port in self.destinations:
                ...
                if u is not None:
                    ...
                    self.graph.add_edge(
                        port_out_data.id, dst_port.node.id, **{"in": dst_port.idx}
                    )
    ...
    def set_destination(self, port, attributes_save_mode=None):
        ...
        if self.destinations and len(self.destinations) > 1:
            raise Error(
                "set_destination applicable only for connections that has exactly one destination or "
                "when there is no destinations"
            )

        if port.type == "out":
            raise Error(
                "Wrong port type in set_destination method. Should be 'in' but given 'out'"
            )
        ...
        if self.graph.stage == "front":
            if self.source is not None:
                ...
                edge_attrs = {}
                new_tensor_info = self._get_new_tensor_debug_info(
                    attributes_save_mode, source_attrs, dest_attrs
                )
                self._update_tensor_debug_info(edge_attrs, new_tensor_info)
            ...
            if self.source is not None:
                ...
                new_tensor_info = self._get_new_tensor_debug_info(
                    attributes_save_mode, data_node.attrs(), dest_attrs
                )
    ...
    def add_destination(self, port):
        ...
        if self.graph.stage == "front":
            node = self.source.node
            self.graph.create_edge(
                node, port.node, out_port=self.source.idx, in_port=port.idx
            )
        else:
            data_node = self.source._create_data_if_necessary()
    ...
    def insert_node(self, new_node, attributes_save_mode: str = "merge"):
        assert (
            len(new_node.out_ports()) == 1
        ), "The node {} has several output ports".format(new_node.soft_get("name"))
