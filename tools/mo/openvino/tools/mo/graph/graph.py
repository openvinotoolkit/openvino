...
from openvino.tools.mo.utils.utils import (
    deprecated_api,
    refer_to_faq_msg,
    shrink_str_value,
)
...
class Node:
    ...
    def __str__(self, max_length: int = 100):
        ...
        print_dict = {
            k: v if k != "value" else shrink_str_value(v, max_symbols=max_length)
    ...
    def __setattr__(self, k, v):
        ...
        if k == "version" and attrs.get(k, v) != v:
            raise AttributeError(
                "Attribute 'version' cannot be updated in {} node".format(self.name)
            )
    ...
    def add_input_port(self, idx, skip_if_exist=False, **kwargs):
        ...
        control_flow = (
            kwargs["control_flow"] if kwargs.get("control_flow") is not None else False
        )
        if skip_if_exist is False and idx in self.in_ports(control_flow=control_flow):
            raise Error(
                "Input port with {} index already exists for {} node.".format(
                    idx, self.name
                )
            )
    ...
    def delete_input_port(self, idx, skip_if_absent=False):
        ...
        if not skip_if_absent and idx not in self.in_ports(control_flow=control_flow):
            raise Error(
                "Input port with index {} doesn't exist in node {}.".format(
                    idx, self.soft_get("name")
                )
            )
    ...
    def delete_output_port(self, idx, skip_if_absent=False):
        ...
        if not skip_if_absent and idx not in self.out_ports(control_flow=control_flow):
            raise Error(
                "Output port with index {} doesn't exist in node {}.".format(
                    idx, self.soft_get("name")
                )
            )
    ...
    def add_output_port(self, idx, skip_if_exist=False, **kwargs):
        ...
        if skip_if_exist is False and idx in self.out_ports(control_flow=control_flow):
            raise Error(
                "Output port with {} index already exists for {} node.".format(
                    idx, self.name
                )
            )
    ...
    def in_port(self, idx=None, control_flow=False) -> Port:
        if not self.has_valid("_in_ports"):
            raise Error(
                "Operation {} {} has no _in_ports attribute", self.op, self.name
            )
        if idx not in self._in_ports:
            raise Error(
                "Input port with index {} is not in node {}".format(idx, self.name)
            )
        if (
            not control_flow
            and "control_flow" in self._in_ports[idx]
            and self._in_ports[idx]["control_flow"]
        ):
            raise Error(
                "Attempt to access control flow port when it's prohibited for node {}".format(
                    self.name
                )
            )
    ...
    def in_ports(self, control_flow=False):
        ...
        for idx in self._in_ports:
            if (
                control_flow
                or "control_flow" not in self._in_ports[idx]
                or not self._in_ports[idx]["control_flow"]
            ):
                ports.update({idx: self.in_port(idx, control_flow=control_flow)})
        return dict_to_ordered_dict(
            ports, func=lambda t: int(str(t).replace("control_flow_", ""))
        )

    def out_port(self, idx=None, control_flow=False) -> Port:
        if not self.has_valid("_out_ports"):
            raise Error(
                "Operation {} {} has no _out_ports attribute", self.op, self.name
            )
        if idx not in self._out_ports:
            raise Error(
                "Output port with index {} is not in node {}".format(idx, self.name)
            )
        if (
            not control_flow
            and "control_flow" in self._out_ports[idx]
            and self._out_ports[idx]["control_flow"]
        ):
            raise Error(
                "Attempt to access control flow port when it's prohibited for node {}".format(
                    self.name
    ...
    def out_ports(self, control_flow=False):
        ...
        for idx in self._out_ports:
            if (
                control_flow
                or "control_flow" not in self._out_ports[idx]
                or not self._out_ports[idx]["control_flow"]
            ):
    ...
    def is_in_port_connected(self, idx, control_flow=False):
        return (
            self.has_port("in", idx, control_flow)
            and not self.in_port(idx, control_flow).disconnected()
        )

    def is_out_port_connected(self, idx, control_flow=False):
        return (
            self.has_port("out", idx, control_flow)
            and not self.out_port(idx, control_flow).disconnected()
        )
    ...
    def in_nodes_edges(self, control_flow: bool = False):
        return dict_to_ordered_dict(
            {
                x[1]["in"]: (Node(self.graph, x[0]), x[1])
                for x in self.get_inputs(control_flow=control_flow)
            },
    ...
    def in_nodes(self, control_flow: bool = False):
        if self.kind == "op":
            ...
            return [
                Node(self.graph, n)
                for n, d in self.get_inputs(control_flow=control_flow)
            ]
    ...
    def in_edges(self, control_flow: bool = False):
        ...
        if self.kind == "op":
            return dict_to_ordered_dict(
                {x[1]["in"]: x[1] for x in self.get_inputs(control_flow=control_flow)},
                func=lambda t: int(str(t).replace("control_flow_", "")),
            )
        elif self.kind == "data":
    ...
    def out_nodes_edges(self, control_flow: bool = False):
        return dict_to_ordered_dict(
            {
                x[1]["out"]: (Node(self.graph, x[0]), x[1])
                for x in self.get_outputs(control_flow=control_flow)
            },
            func=lambda t: int(str(t).replace("control_flow_", "")),
        )
    ...
    def out_nodes(self, control_flow: bool = False):
        ...
        if self.kind == "op":
            ...
            return [
                Node(self.graph, n)
                for n, d in self.get_outputs(control_flow=control_flow)
            ]
    ...
    def get_inputs(self, edge_attr: dict = None, control_flow: bool = False):
        ...
        if not control_flow:
            in_edges = [
                ...
                if "control_flow_edge" not in d or not d["control_flow_edge"]
            ]
        return [
            (u, d)
            for u, v, d in in_edges
            if all([attr in d and d[attr] == edge_attr[attr] for attr in edge_attr])
        ]
    ...
    def get_sorted_inputs(self, control_flow: bool = False):
        return sorted(
            [x for x in self.get_inputs(control_flow=control_flow) if "in" in x[1]],
            key=lambda x: x[1]["in"],
        )

    def get_sorted_outputs(self, control_flow: bool = False):
        return sorted(
            [x for x in self.get_outputs(control_flow=control_flow) if "out" in x[1]],
    ...
    def edges(self, attrs: dict = None):
        ...
        """
        edges = list(self.graph.in_edges([self.id], data=True)) + list(
            self.graph.out_edges([self.id], data=True)
        )
    ...
    def edge(self, attrs: dict = None):
        ...
        assert len(edges) == 1, "edges: {}, required attributes: {}".format(
            edges, attrs
        )
        return edges[0]
    ...
    def copy_node(self, new_attrs: dict = None, dst_graph=None):
        ...
        new_id = (
            dst_graph.unique_id(attrs["name"])
            if "name" in attrs
            else dst_graph.unique_id()
        )
        attrs["name"] = new_id
    ...
    def insert_node_with_data_before(
        self,
        inp,
        new_op_class: callable,
        op_before_params: dict = None,
        ...
        new_inp = new_op_before.create_node_with_data(
            inputs, {"name": node.name + cls_name + "/Before"}
        )
        graph.add_edge(new_inp.id, node.id, **edge_attrs)
        if infer_current:
            node.infer(node)

    def insert_node_with_data_after(
        self,
        out,
        new_op_class: callable,
    ...
    def bracket_with_different_nodes_with_data(
        self,
        inp,
        out,
        new_op_class_before: callable,
    ...
    def bracket_op_with_another_op(
        self,
        inp,
        out,
        new_op_class: callable,
        ...
        [...] -> inp -> New_Op_bef -> New_Data_bef -> Cur_Op -> Cur_Data -> New_Op_aft -> New_Data_aft(==out) -> [...]
                    [op_before_params]                                  [op_after_params]
        """
        self.bracket_with_different_nodes_with_data(
            inp=inp,
            out=out,
            new_op_class_before=new_op_class,
            new_op_class_after=new_op_class,
            op_before_params=op_before_params,
            op_after_params=op_after_params,
        )
    ...
    def insert_node_after(self, new_node, node_out_port: int = 0):
        ...
        assert (
            len(
                [
                    ...
                    if Node(self.graph, name).soft_get("kind") == "data"
                ]
            )
            == 0
        )
        ...
        for _, dst_name, edge_key, edge_attrs in old_edges:
            if edge_attrs["out"] == node_out_port:
                log.debug(
                    'Create edge from "{}" to "{}"'.format(new_node.name, dst_name)
                )
    ...
    def insert_op_on_input_port(
        ...
        assert self.has_port(
            "in", in_port_idx
        ), "Node `{}` should have input port with idx `{}` but it does not".format(
            name, in_port_idx
        )
        ...
        if value is not None:
            ...
            constant = Const(
                graph, {"value": value, "name": op_node.name + "/value"}
            ).create_node()
    ...
    def replace_node(self, new_node, new_node_out_port: int = None):
        ...
        if (
            len(self.out_nodes()) == 1
            and self.out_node().has("op")
            and self.out_node().op == "Result"
        ):
    ...
    def update_node(self):
        ...
        """
        in_ports_count = (
            self.in_ports_count if self.has_valid("in_ports_count") else None
        )
        out_ports_count = (
            self.out_ports_count if self.has_valid("out_ports_count") else None
        )
...
class Graph(nx.MultiDiGraph):
    ...
    def add_edge(self, u_for_edge, v_for_edge, key=None, **attr):
        ...
        if self.strict_mode:
            ...
            if self.stage == "front":
                assert unode.kind == "op" and vnode.kind == "op", (
                    "{} Wrong add_adge usage! You can connect only two "
                    "operations in front phase".format(message)
                )
                assert "in" in attr and "out" in attr, (
                    "Missing necessary attribute in or out when adding edge "
                    "between {} and {}".format(u_for_edge, v_for_edge)
                )
                is_control_flow = (
                    "control_flow_edge" in attr and attr["control_flow_edge"] is True
                )
                in_port = (
                    "control_flow_{}".format(attr["in"])
                    if is_control_flow
                    else attr["in"]
                )
                out_port = (
                    "control_flow_{}".format(attr["out"])
                    if is_control_flow
                    else attr["out"]
                )
                assert unode.has_port(
                    "out", out_port, control_flow=is_control_flow
                ), "{} Missing out port ({}) in {} node".format(
                    message, out_port, unode.soft_get("name", unode.id)
                )
                assert vnode.has_port(
                    "in", in_port, control_flow=is_control_flow
                ), "{} Missing in port ({}) in {} node".format(
                    message, in_port, vnode.soft_get("name", vnode.id)
                )
            elif self.stage in ["middle", "back"]:
                assert (unode.kind == "data" and vnode.kind == "op") or (
                    unode.kind == "op" and vnode.kind == "data"
                )
                if unode.kind == "data" and vnode.kind == "op":
                    assert (
                        "in" in attr
                    ), "Attribute in is missing when adding edge to {}".format(
                        v_for_edge
                    )
                    assert vnode.has_port(
                        "in", attr["in"]
                    ), "{} Node {} has no in port ({})" "".format(
                        message, vnode.name, attr["in"]
                    )
                if unode.kind == "op" and vnode.kind == "data":
                    assert (
                        "out" in attr
                    ), "Attribute out is missing when adding edge from {}".format(
                        u_for_edge
                    )
                    assert unode.has_port(
                        "out", attr["out"]
                    ), "{} Node {} has no out port ({})" "".format(
                        message, unode.name, attr["out"]
                    )
    ...
    def erase_node(self, node: Node):
        ...
        assert node.kind == "op" and (
            len(node.out_nodes()) == 0
            or list(node.out_nodes().values())[0].kind != "data"
        ), "The function must be used before the partial infer when graph doesn't contain data nodes."
        assert (
            len(node.out_nodes()) <= 1
        ), "The node {} must produce just one output tensor".format(
            node.soft_get("name")
        )
        assert len(inputs) <= 1, "The node {} must have just one input".format(
            node.soft_get("name")
        )
        ...
        if len(outputs) == 0 and len(inputs) != 0:
            ...
            input_ids = {
                input_node_id: {"port": {"out": [attrs["out"]]}}
                for input_node_id, _, attrs in inputs
            }
        ...
        for name in input_names_in_pattern:
            ...
            for edge in pattern_edges:
                if edge[0] == name:
                    assert (
                        edge[1] in match
                    ), "name from pattern_edges {} not in match {}".format(
                        edge[1], match
                    )
                    dst.append(match[edge[1]])
            if len(dst) != 1:
                raise Error(
                    "Multiple output ports detected for node {} as {} in pattern".format(
                        match[name].id, name
                    )
                )
            dst = dst[0]
    ...
    def get_data_nodes(self, has_value=None):
        ...
        data_nodes = [
            ...
            if Node(self, node).soft_get("kind") == "data"
        ]
        return [
            node
            for node in data_nodes
            if has_value is None or node.has_valid("value") == has_value
        ]
    ...
    def check_shapes_consistency(self):
        ...
        if len(data_nodes_with_wrong_shapes) > 0:
            raise Error(
                "Graph contains data nodes ({}) with inconsistent shapes: {}".format(
                    len(data_nodes_with_wrong_shapes), data_nodes_with_wrong_shapes
                )
            )
        ...
        for node in op_nodes:
            for idx in range(len(node.in_ports())):
                if idx not in node.in_ports():
                    raise Error(
                        "Node {} has not consecutive in ports indexes: {}".format(
                            node.name, list(node.in_ports().keys())
                        )
                    )
            for idx in range(len(node.out_ports())):
                if idx not in node.out_ports():
                    raise Error(
                        "Node {} has not consecutive out ports indexes: {}".format(
                            node.name, list(node.out_ports().keys())
                        )
                    )
    ...
    def dump_graph_for_graphviz(
        ...
        from openvino.tools.mo.ops.tensor_iterator import (
            _get_internal_input_node_id,
            _get_internal_output_node_id,
        )

        fill_color = {"op": "lightblue", "data": "whitesmoke", "highlight": "firebrick"}
        fill_color_by_type = {
            "Const": "lightpink",
            "Parameter": "yellowgreen",
            "TensorIterator": "lemonchiffon",
        }
        ...
        def _subgraph_label(node_id, node_attrs: dict, attrs_to_print: list):
            ...
            label += 'color={}; \nstyle="filled,rounded";\n'.format(
                fill_color_by_type[node_attrs["op"]]
            )
        ...
        def _node_label(node_id, node_attrs: dict, attrs_to_print: list):
            label = (
                ...
                + "\\n".join(
                    [
                        str(key) + "=" + str(node_attrs.get(key, "None"))
                        for key in attrs_to_print
                ...
                if "value" not in attrs_to_print and "value" in node_attrs:
                    if node_attrs["value"] is not None:
                        label += (
                            '\\nvalue=\\"'
                            + ",".join(
                                [str(val) for val in node_attrs["value"].flatten()]
                            )[:40]
                            + '\\"'
                        )
                    else:
        ...
        def _dump_nodes_attrs():
            ...
            for node_id in nodes_to_dump:
                ...
                color = fill_color_by_type.get(
                    attrs.get("type", ""), fill_color[attrs["kind"]]
                )

                if (
                    node_id in highlight_nodes
                    or "highlight" in node_attrs
                    and node_attrs["highlight"]
                ):
                ...
                if attrs.get("op") == "TensorIterator":
                    ...
                    string += (
                        '"{}" [fillcolor={} style="{}" shape=box label="{}"];\n'.format(
                            node_id,
                            color,
                            style[attrs["kind"]],
                            _node_label(node_id, attrs, node_attrs),
                        )
                    )
            return string
        ...
        def _dump_edges_attrs():
            ...
            for src_node_id, dst_node_id, attrs in self.edges(data=True):
                ...
                if src_node_id in subgraphs:
                    ...
                    src_node_id = _get_internal_output_node_id(
                        self, src_node_id, attrs["external_port_id"]
                    )
                elif dst_node_id in subgraphs:
                    edge_label = subgraphs[dst_node_id]
                    edge_label_name = "lhead"
                    dst_node_id = _get_internal_input_node_id(
                        self, dst_node_id, attrs["external_port_id"]
                    )
                else:
                    edge_label = " ".join(
                        [
                ...
                string += '"{}" -> "{}" [{} = "{}"];\n'.format(
                    src_node_id, dst_node_id, edge_label_name, edge_label
                )
            return string
    ...
    def print_graph_stat(self):
        ...
        for _node in self.nodes():
            ...
            if node.has("shape") and np.any(node.shape == 0):
                log.error(
                    "Found bad shape: '{}' for node '{}'".format(node.shape, node.node)
                )
    ...
    def pseudo_topological_sort_with_start_node(
        self, start_node: Node, reverse: bool = False
    ):
...
def dict_includes(big: dict, sub_dict: dict, skip_attr_names=[]):
    ...
    return all(
        ...
        if attr not in skip_attr_names
    )
...
def add_opoutput(
    ...
    if cut and len(node.out_edges()) != 0:
        opoutput_node = Result(graph).create_node_on_port(
            node,
            port,
            {
                "name": node_name + "/sink_port_" + str(port),
                "keep_output_port": keep_output_port,
            },
        )
    else:
        opoutput_node = Result(graph).create_node(
            [(node, port)],
            {
                "name": node_name + "/sink_port_" + str(port),
                "keep_output_port": keep_output_port,
    ...
    if user_defined_name is not None and (
        graph.stage == "front" or graph.stage is None
    ):
        ...
        if user_defined_name not in prev_op_tensor_names:
            if graph.has_tensor_name(user_defined_name):
                log.warning(
                    "Could not add user defined output name {} to tensor names list of {} node as "
                    "graph contains tensor name with same name.".format(
                        user_defined_name, opoutput_node.soft_get("name")
                    )
                )
            else:
                if "fw_tensor_debug_info" not in in_edge_attrs:
                    in_edge_attrs["fw_tensor_debug_info"] = []
                in_edge_attrs["fw_tensor_debug_info"].append(
    ...
    for key, value in additional_attrs.items():
        if key not in ["in", "out"]:
            if type(additional_attrs[key]) is list:
                ...
                result[key] = list(
                    set(result[key])
                )  # silly solution to find unique elements
            else:
                result[key] = value
    return result
...
def set_edge_attribute_between_nodes(
    node1: Node, node2: Node, attr_name: str, new_value
):
    """
...
def get_inputs_with_ports(graph: Graph, match, pattern_edges, input_names_in_pattern):
    ...
    :return: list of tuples of node and output port
    """
    return graph.get_inputs_with_ports(
        match=match,
...
def dump_graph_for_graphviz(
    ...
    return graph.dump_graph_for_graphviz(
...
@deprecated_api(Node)
def copy_node(
    src_node: Node, new_attrs: dict = None, dst_graph: nx.MultiDiGraph = None
):
    """Copies node with all attributes (optionally updated) within the same graph or to different graph."""
...
@deprecated_api(Node)
def get_inputs(
    graph: Graph, node: str, edge_attr: dict = None, control_flow: bool = False
):
