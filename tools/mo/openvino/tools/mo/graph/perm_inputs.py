...
def axis(op_node: Node, port_info: str, input_port: int):
    ...
    assert permutation_data_node.has_and_set("permutation"), (
        'Data node "{}" does not have permutation for node {}, '
        'port_info "{}".'.format(permutation_data_node.id, op_node.id, port_info)
    )
    ...
    axis_const = Const(
        graph, {"value": int64_array(0), "name": gather_name + "/axis"}
    ).create_node_with_data()
    gather = Gather(
        graph, {"name": gather_name, "need_shape_inference": True}
...
def order(op_node: Node, port_info: str, input_port: int):
    ...
    axis_const_1 = Const(
        graph, {"value": int64_array(0), "name": gather_1_name + "/axis"}
    ).create_node_with_data()
    gather_1 = Gather(
        graph, {"name": gather_1_name, "need_shape_inference": True}
...
def strided_slice(op_node: Node, port_info: str, input_port: int):
    ...
    slice_rank = op_node.in_port(input_port).data.get_shape()[
        0
    ]  # length of begin, end or strides
...
def reorder_inputs_for_shape_or_slice(
    op_node: Node, input_port: int, permute_indices_for_gather: list
):
    """
...
def transpose(op_node: Node, port_info: str, input_port: int):
    graph = op_node.graph
    permutation_data_node = get_node_with_permutation(op_node, port_info)
    assert permutation_data_node.has_and_set(
        "permutation"
    ...
    from openvino.tools.mo.front.tf.graph_utils import (  # avoiding recursive imports
...
def transpose_nchw_to_nhwc(op_node: Node, port_info: str, input_port: int):
    ...
    assert (
        rank >= 4
    ), "Rank must be 4D or higher for HCHW to HHWC permutation on node {}.".format(
        op_node.id
    )
...
class PermuteInputs:
    input_permutes = {
        ...
        "slice": lambda node, port_info, input_port: strided_slice(
            node, port_info, input_port
        ),
        ...
        "transpose": lambda node, port_info, input_port: transpose(
            node, port_info, input_port
        ),
        "transpose_nchw_to_nhwc": lambda node, port_info, input_port: transpose_nchw_to_nhwc(
            node, port_info, input_port
        ),
    }
    ...
    shape_check_rules = {
        ...
        "dim_size": lambda port: bool(
            port.data.get_shape()[0] >= 4
        ),  # if input 'dim_size' >= 4 need to permute
    }

    def set_input_permutation(
        self,
        node1: Node,
        node2: Node,
        port_info: str,
        ...
        """
        assert (
            permutation_rule in self.input_permutes
        ), "No `{}` permutation rule in {}".format(permutation_rule, __class__.__name__)
        assert (
            shape_check_rule in self.shape_check_rules
        ), "No `{}` permutation shape check rule " "in {}".format(
            shape_check_rule, __class__.__name__
        )
        nx.set_edge_attributes(
            G=node1.graph,
            values={
                (node1.id, node2.id, 0): (
                    self.input_permutes[permutation_rule],
                    port_info,
