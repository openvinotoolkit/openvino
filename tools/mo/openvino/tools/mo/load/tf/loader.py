...
from openvino.tools.mo.front.extractor import (
    ...
    extract_node_attrs,
    remove_control_dependency_inputs,
    restore_edges,
)
...
class TFLoader(Loader):
    ...
    def load(self, graph: Graph):
        ...
        graph_def, variables_values, framework, inputs_outputs_order = (
            load_tf_graph_def(
        ...
        log.debug(
            "Number of nodes in graph_def: {}".format(len(graph_def.node))
...
def is_node_layout_nhwc(node: Node):
    ...
    if node.soft_get("op") in [
        ...
        if node.soft_get("layout") in ["NHWC", "NDHWC"]:
            log.debug(
                'Detected convolution node with NHWC layout: "{}"'.format(
