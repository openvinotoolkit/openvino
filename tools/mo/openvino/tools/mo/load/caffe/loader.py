...
from openvino.tools.mo.front.caffe.extractor import (
    caffe_extractor,
...
class CaffeLoader(Loader):
    ...
    def load(self, graph: Graph):
        ...
        if argv.caffe_parser_path is None:
            argv.caffe_parser_path = os.path.join(
        ...
        proto, model = loader.load_caffe_proto_model(
            caffe_pb2, argv.input_proto, argv.input_model
        )
        ...
        update_extractors_with_extensions(
            ...
            (
                ...
                if hasattr(argv, "disable_omitting_optional")
                else False
            ),
            (
                argv.disable_flattening_optional_params
                if hasattr(argv, "disable_flattening_optional_params")
                else False
            ),
        )
        ...
        try:
            ...
            raise Error(
                "Invalid prototxt file: value error {}. " + refer_to_faq_msg(11), str(e)
            ) from e
        ...
        custom_layers_mapping.update_extractors(
            ...
            (
                ...
                if hasattr(argv, "enable_flattening_nested_params")
                else False
            ),
        )
        extract_node_attrs(
            graph,
            lambda node: caffe_extractor(
