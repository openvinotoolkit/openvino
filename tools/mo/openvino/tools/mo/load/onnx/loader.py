...
class ONNXLoader(Loader):
    ...
    def load(self, graph: Graph):
        ...
        log.debug(
            "Number of initializers in graph_def: {}".format(
                len(model_graph.initializer)
            )
        )
        log.debug(
            "Number of real inputs in graph_def: {}".format(
        ...
        graph.__setattr__(
            "name", argv.model_name if argv.model_name else model_proto.graph.name
