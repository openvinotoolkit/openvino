...
try:
    ...
    raise Error(
        "Module mxnet was not found. Please install appropriate version of mxnet with requirements via \n"
...
from openvino.tools.mo.front.mxnet.extractor import (
    mxnet_op_extractor,
    mxnet_op_extractors,
)
from openvino.tools.mo.front.mxnet.loader import load_symbol_def, symbol2nx
...
class MxNetLoader(Loader):
    ...
    def load(self, graph: Graph):
        ...
        if (
            argv.nd_prefix_name
            and argv.pretrained_model_name
            and argv.save_params_from_nd
        ):
            save_params_file(
                model_name,
                model_params._arg_params,
                model_params._aux_params,
                iteration_number,
            )
