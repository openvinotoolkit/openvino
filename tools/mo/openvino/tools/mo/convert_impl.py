...
from openvino.tools.mo.middle.passes.convert_data_type import (
    destination_type_to_np_data_type,
)
from openvino.tools.mo.middle.pattern_match import (
    for_graph_and_each_sub_graph_recursively,
)
from openvino.tools.mo.moc_frontend.check_config import (  # pylint: disable=no-name-in-module,import-error
    legacy_extensions_used,
    legacy_transformations_config_used,
    new_extensions_used,
    tensorflow_custom_operations_config_update_used,
)
from openvino.tools.mo.moc_frontend.moc_emit_ir import (  # pylint: disable=no-name-in-module,import-error
    moc_emit_ir,
)
from openvino.tools.mo.moc_frontend.paddle_frontend_utils import (  # pylint: disable=no-name-in-module,import-error
    paddle_frontend_converter,
)
from openvino.tools.mo.moc_frontend.pipeline import (  # pylint: disable=no-name-in-module,import-error
    moc_pipeline,
)
from openvino.tools.mo.moc_frontend.pytorch_frontend_utils import (  # pylint: disable=no-name-in-module,import-error
    extract_input_info_from_example,
    get_pytorch_decoder,
)
from openvino.tools.mo.moc_frontend.shape_utils import (  # pylint: disable=no-name-in-module,import-error
    parse_input_shapes,
)
...
from openvino.tools.mo.utils.cli_parser import (
    ...
    get_advanced_cli_options,
    get_available_front_ends,
    get_caffe_cli_options,
    get_common_cli_options,
    get_freeze_placeholder_values,
    get_kaldi_cli_options,
    get_layout_values,
    get_mean_scale_dictionary,
    get_mo_convert_params,
    get_model_name_from_args,
    get_mxnet_cli_options,
    get_onnx_cli_options,
    get_placeholder_shapes,
    get_tf_cli_options,
    input_shape_to_input_cut_info,
    input_to_input_cut_info,
    parse_transform,
    parse_tuple_pairs,
)
from openvino.tools.mo.utils.error import Error, FrameworkError
from openvino.tools.mo.utils.get_ov_update_message import (  # pylint: disable=no-name-in-module,import-error
...
from openvino.tools.mo.utils.telemetry_utils import (
    init_mo_telemetry,
    send_conversion_result,
    send_framework_info,
    send_params_info,
)
from openvino.tools.mo.utils.utils import check_values_equal, refer_to_faq_msg
...
def arguments_post_parsing(argv: argparse.Namespace):
    ...
    if use_new_frontend and use_legacy_frontend:
        raise Error(
            "Options --use_new_frontend and --use_legacy_frontend must not be used simultaneously "
    ...
    if not moc_front_end and use_new_frontend:
        raise Error(
            "Option --use_new_frontend is specified but the Model Optimizer is unable to find new frontend. "
    ...
    is_tf, is_caffe, is_mxnet, is_kaldi, is_onnx = (
        deduce_legacy_frontend_by_namespace(argv)
    ...
    if not is_legacy_frontend and use_legacy_frontend:
        raise Error(
            "Option --use_legacy_frontend is specified but Model Optimizer does not have legacy frontend "
    ...
    if not is_legacy_frontend and not moc_front_end:
        ...
        if not argv.framework:
            ...
            if argv.framework == "ir":
                raise Error(
                    "OpenVINO IR is passed as input_model in convert_model/mo, the IR doesn't need "
    ...
    if (
        is_tf
        and not argv.input_model
        and not argv.saved_model_dir
        and not argv.input_meta_graph
    ):
        raise Error(
        ...
        is_mxnet
        and not argv.input_model
        and not argv.input_symbol
        and not argv.pretrained_model_name
    ):
        raise Error(
    ...
    if argv.input_model and (is_tf and argv.saved_model_dir):
        raise Error(
            "Both --input_model and --saved_model_dir are defined. "
    ...
    if hasattr(argv, "is_python_api_used") and argv.is_python_api_used:
        ...
        argv.inputs_list, argv.placeholder_shapes, argv.placeholder_data_types = (
            get_placeholder_shapes(argv.input, argv.input_shape, argv.batch)
        )
    ...
    argv.layout_values = get_layout_values(
        argv.layout, argv.source_layout, argv.target_layout
    )
...
def check_fallback(argv: argparse.Namespace):
    ...
    fallback_reasons["tensorflow_custom_operations_config_update"] = (
        tensorflow_custom_operations_config_update_used
    )

    reasons = [
        reason
        for reason, is_applicable in fallback_reasons.items()
        if is_applicable(argv)
    ]
    return reasons


def update_fallback_with_conversion_error(
    use_new_frontend: bool, is_tf: bool, ex_msg: str, fallback_reasons: list
):
    import re
    ...
    fallback_operations = [
        ...
        if operation in all_fallback_operations
    ]
    ...
    fallback_reasons.append(
        "Fallback to the legacy TF FE due to operation(s): "
        + ", ".join(fallback_operations)
    )
    return True
...
def get_default_frontends():
    ...
    default_frontends = {"onnx": "new", "tf": "new"}
...
def get_moc_frontends(argv: argparse.Namespace):
    ...
    if (
        default_frontends.get(moc_front_end.get_name()) == "legacy"
        and not use_new_frontend
    ):
        return None, available_moc_front_ends
...
def prepare_ir(argv: argparse.Namespace):
    ...
    if moc_front_end:
        ...
        if len(fallback_reasons) == 0:
            if (
                is_tf
                and tf_frontend_with_python_bindings_installed
            ...
            try:
                t.send_event(
                    "mo", "conversion_method", moc_front_end.get_name() + "_frontend"
                )
                moc_front_end.add_extension(
                    TelemetryExtension(
                        "mo", t.send_event, t.send_error, t.send_stack_trace
                    )
                )
                moc_front_end.add_extension(
                ...
                if legacy_transformations_config_used(argv):
                    raise Error(
                        "Legacy extensions are not supported for the new frontend"
                    )
                if legacy_extensions_used(argv):
                    raise Error(
                        "Legacy transformations configuration is not supported for the new frontend"
                    )
                if tensorflow_custom_operations_config_update_used(argv) and is_tf:
                    raise Error(
                        "TensorFlow custom operation config is not supported for the new frontend"
                    )
                if new_extensions_used(argv):
                ...
                if not update_fallback_with_conversion_error(
                    argv.use_new_frontend, is_tf, str(ex), fallback_reasons
                ):
    ...
    if len(fallback_reasons) > 0:
        ...
        assert not hasattr(
            argv, "is_fallback"
        ), "`is_fallback` argument must not exist."
...
def emit_ir(graph: Graph, argv: argparse.Namespace, non_default_params: dict):
    ...
    for_graph_and_each_sub_graph_recursively(
        graph, RemoveConstOps().find_and_replace_pattern
    )
    for_graph_and_each_sub_graph_recursively(
        graph, CreateConstNodesReplacement().find_and_replace_pattern
    )
    ...
    input_names = (
        deepcopy(graph.graph["input_names"]) if "input_names" in graph.graph else []
    )
    ...
    try:
        ...
        raise Error(
            "Exception occurred while serialization or reading of the temporary IR: {}".format(
                str(err),
            )
        ) from err
    finally:
    ...
    if not (argv.framework == "tf" and argv.tensorflow_custom_operations_config_update):
        try:
            from openvino.tools.mo.back.offline_transformations import (  # pylint: disable=no-name-in-module,import-error
                apply_offline_transformations,
            )
...
def args_dict_to_list(cli_parser, **kwargs):
    ...
    for key, value in kwargs.items():
        if value is None:
            continue
        if key in signature.parameters and check_values_equal(
            signature.parameters[key].default, value
        ):
            continue
        ...
        for inp in inputs:
            ...
            if inp.type is not None:
                ...
                if isinstance(inp.type, str):
                    data_type_dict[inp.name] = destination_type_to_np_data_type(
                        inp.type
                    )
...
def pack_params_to_args_namespace(args: dict, cli_parser: argparse.ArgumentParser):
    if len(args) > 0:
        ...
        for key, value in args_string.items():
            ...
            if value is not None and not check_values_equal(
                getattr(argv, key, None), value
            ):
...
def update_args_for_saved_model_dir(args: dict):
    ...
    """
    if (
        "saved_model_dir" in args
        and args["saved_model_dir"] is not None
        and "input_model" in args
        and args["input_model"] is not None
    ):
        raise Error(
            "Both --input_model and --saved_model_dir are defined. "
    ...
    if (
        "input_model" in args
        and isinstance(args["input_model"], (str, Path))
        and os.path.isdir(args["input_model"])
    ):
    ...
    try:
        ...
        if inp_model_is_object:
            ...
            if model_framework == "pytorch":
                ...
                decoder = get_pytorch_decoder(
                    args["input_model"], parse_input_shapes(args), example_inputs, args
                )
        ...
        frameworks = list(
            set(
                ...
                + (get_available_front_ends(argv.feManager) if argv.feManager else [])
            )
        )
        framework = (
            argv.framework
            if hasattr(argv, "framework") and argv.framework is not None
            else framework
        )
        if framework is not None:
            assert framework in frameworks, (
                "error: argument --framework: invalid choice: '{}'. "
        ...
        if model_framework is not None:
            if argv.framework is not None:
                if argv.framework != model_framework:
                    raise Error(
                        "Provided model does not correspond to provided framework. The provided "
                        "framework is {}, the model type is {} which is expected to be {} framework.".format(
                            argv.framework, type(argv.input_model), model_framework
                        )
                    )
            else:
        ...
        if silent_is_false(argv) or not python_api_used:
            ...
            _, is_caffe, is_mxnet, is_kaldi, _ = deduce_legacy_frontend_by_namespace(
                argv
            )
            if ov_update_message is not None:
                print(ov_update_message)
            if (
                ovc_message is not None
                and not is_caffe
                and not is_mxnet
                and not is_kaldi
            ):
        ...
        if silent_is_false(argv) or not python_api_used:
            if isinstance(e, (FileNotFoundError, NotADirectoryError)):
                ...
                log.error("-------------------------------------------------")
                is_fallback = (
                    getattr(argv, "is_fallback", False) if argv is not None else False
                )
                if (
                    not argv.use_legacy_frontend
                    and framework_is_tf(args, argv)
                    and not is_fallback
                ):
