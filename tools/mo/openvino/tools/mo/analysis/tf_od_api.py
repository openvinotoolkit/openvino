...
class TensorFlowObjectDetectionAPIAnalysis(AnalyzeAction):
    ...
    file_patterns = {
        "MaskRCNN": "mask_rcnn_support.*\\.json",
    ...
    def analyze(self, graph: Graph):
        ...
        if not tf_1_cond and not tf_2_cond:
            log.debug(
                "The model does not contain nodes that must exist in the TF OD API models"
            )
            return None, None
            ...
            for scopes in scopes_tuple:
                if all([graph_contains_scope(graph, scope) for scope in scopes]):
                    ...
                    result["mandatory_parameters"] = {
                        "transformations_config": files_by_pattern(
                            get_mo_root_dir() + "/openvino/tools/mo/front/tf",
                    ...
                    message = (
                        "Your model looks like TensorFlow Object Detection API Model.\n"
