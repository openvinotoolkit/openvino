...
class InputsAnalysis(AnalyzeAction):
    ...
    def fifo_queue_analysis(cls, graph: Graph, inputs_desc: dict):
        ...
        for fifo_queue in graph.get_op_nodes(op="FIFOQueueV2"):
            if len(fifo_queue.get_outputs({"out": 0})) != 1:
                log.debug(
                    'The FIFOQueue operation "{}" has more than 1 consumers'.format(
                        fifo_queue.id
                    )
                )
                continue
    ...
    def ignore_mxnet_softmax_inputs(cls, graph: Graph):
        ...
        softmax_nodes = []
        [
            softmax_nodes.extend(graph.get_op_nodes(op=op))
            for op in ("SoftMax", "SoftmaxActivation", "SoftmaxOutput")
        ]
        for softmax_node in softmax_nodes:
            for i in range(1, len(softmax_node.in_nodes())):
                if (
                    softmax_node.in_node(i).has_valid("op")
                    and softmax_node.in_node(i).op == "Parameter"
                ):
        ...
        for iter_get_next in op_nodes:
            for port in iter_get_next.out_nodes().keys():
                ...
                params = params + "{}:{}{}".format(
                    iter_get_next.soft_get("name", iter_get_next.id), port, shape
                )
    ...
    def analyze(self, graph: Graph):
        ...
        for input in inputs:
            inputs_desc[input.name] = {
                "shape": input.soft_get("shape", None),
        ...
        for input in placeholders_with_default:
            inputs_desc[input.name] = {
                ...
                "data_type": input.soft_get("data_type", None),
                "value": (
                    input.in_node(0).value
                    if 0 in input.in_nodes() and input.in_node(0).has_valid("value")
                    else None
                ),
            }
