...
class CustomReplacementDescriptor(object):
    ...
    def __init__(self, replacement_id: str, attrs: dict = None):
        ...
        :param attrs:
        """
        super(CustomReplacementDescriptor, self).__setattr__(
            "replacement_id", replacement_id
        )
        if attrs is not None:
            super(CustomReplacementDescriptor, self).__setattr__(
                "custom_attributes", attrs.setdefault("custom_attributes", {})
            )
            super(CustomReplacementDescriptor, self).__setattr__(
                "_replacement_desc", attrs.copy()
            )
    ...
    def register_type(cls, match_kind: str, class_type: object):
        if match_kind in cls.registered_types:
            log.warning(
                'Class for match kind "{}" is already registered'.format(match_kind)
            )
        else:
    ...
    def create_instance(cls, match_kind: str, replacement_id: str, attrs: dict = None):
        ...
        if match_kind in cls.registered_types:
            ...
            raise Error(
                'No class registered for match kind "{}". Supported match kinds are "{}". '.format(
    ...
    def sub_graph_instances(self):
        raise Exception(
            "The function 'get_sub_graph_instances' must be implemented in the sub-class."
        )
    ...
    def get_inputs_description(self):
        ...
        if "inputs" not in self._replacement_desc:
            log.error(
                "Information about inputs of layer with id '{}' is not available".format(
                    self.replacement_id
                )
            )
            return None
    ...
    def get_outputs_description(self):
        ...
        if "outputs" not in self._replacement_desc:
            log.error(
                "Information about outputs of layer with id '{}' is not available"
            )
            return None
    ...
    def update_custom_replacement_attributes(self, graph: Graph):
        ...
        """
        raise Exception(
            "The function 'update_custom_layer_attributes' must be implemented in the sub-class."
        )
    ...
    def validate_data(self):
        ...
        if not self.has("id"):
            errors.append(
                "Replacement id is not specified for custom replacement '{}'".format(
                    self.replacement_id
                )
            )
        if not self.has("instances") or self.instances == "":
            errors.append(
                "Attribute 'instances' is not specified for replacement '{}'".format(
                    self.replacement_id
                )
            )
        if not self.has("match_kind"):
            errors.append(
                "Replacement match type is not specified for replacement '{}'".format(
                    self.replacement_id
                )
            )
        return errors
...
class CustomReplacementDescriptorPoints(CustomReplacementDescriptor):
    ...
    def __init__(self, replacement_id: str, attrs: dict = None):
        ...
        if not self.has("include_inputs_to_sub_graph"):
            super(CustomReplacementDescriptorPoints, self).__setattr__(
    ...
    def get_inputs_description(self):
        return [
            [("^" + node_name + "$", 0)] for node_name in self.instances["start_points"]
        ]

    def get_outputs_description(self):
        return [
            ("^" + node_name + "$", 0) for node_name in self.instances["end_points"]
        ]
    ...
    def get_internal_input_nodes(self, graph: Graph):
        ...
        if not self.include_inputs_to_sub_graph:
            log.debug(
                "Do not include inputs to sub-graph for replacement with id {}".format(
                    self.replacement_id
                )
            )
            new_start_nodes = set()
    ...
    def get_internal_output_nodes(self, graph: Graph):
        ...
        if not self.include_outputs_to_sub_graph:
            log.debug(
                "Do not include outputs of sub-graph for replacement with id {}".format(
                    self.replacement_id
                )
            )
            new_end_nodes = set()
    ...
    def update_custom_replacement_attributes(self, graph: Graph):
        if not self.has("instances"):
            raise Error(
                "No instance(s) is(are) defined for the custom replacement '{}'. ".format(
                    self.replacement_id
                )
                + refer_to_faq_msg(66)
            )
        if not isinstance(self.instances, dict):
            raise Error(
                "The instance must be a single dictionary for the custom replacement with id '{}'. ".format(
                    self.replacement_id
                )
                + refer_to_faq_msg(67)
            )
        ...
        for src_node_name, dst_node_name, edge_attrs in graph.edges(data=True):
            ...
            if (src_node_name not in matched_nodes) and (
                dst_node_name in matched_nodes
            ):
            ...
            if (src_node_name in matched_nodes) and (
                dst_node_name not in matched_nodes
            ):
                output_tensors.add(
                    (
        ...
        for node_name in graph.nodes():
            ...
            if (
                node_name in matched_nodes
                and len(node.out_nodes()) == 0
                and node["pb"].op != "Const"
            ):
                log.debug(
                    "Node {} doesn't have output edges. Consider it output".format(
                        node_name
                    )
                )
        ...
        if not self.has("inputs"):
            self._replacement_desc["inputs"] = [
                [{"node": desc[0], "port": desc[1]} for desc in inp]
                for inp in sorted(input_nodes_mapping.values())
            ]
            log.debug(
                'Updated inputs of sub-graph for instance "{}"'.format(self.instances)
            )

        if not self.has("outputs"):
            self._replacement_desc["outputs"] = [
                {"node": node, "port": port} for node, port in sorted(output_tensors)
            ]
            log.debug(
                'Updated outputs of sub-graph for instance "{}"'.format(self.instances)
            )
...
class CustomReplacementDescriptorScope(CustomReplacementDescriptor):
    ...
    def update_custom_replacement_attributes(self, graph: Graph):
        if not self.has("instances") or len(self.instances) == 0:
            raise Error(
                "No instances are defined for replacement with id '{}'. ".format(
                    self.replacement_id
                )
                + refer_to_faq_msg(68)
            )
        ...
        pattern = self.instances[
            0
        ]  # use the first instance pattern to find input/output nodes patterns
...
def parse_custom_replacement_config_file(file_name: str):
    ...
    if not os.path.exists(file_name):
        raise Error(
            "Custom replacements configuration file '{}' does not exist. ".format(
                file_name
            )
            + refer_to_faq_msg(69)
        )
    ...
    for attrs in data:
        if "id" not in attrs:
            raise Error(
                'One of the custom replacements in the configuration file "{}" does not contain attribute '
                '"id". '.format(file_name) + refer_to_faq_msg(71)
            )
        if "match_kind" not in attrs:
            raise Error(
                'One of the custom replacements in the configuration file "{}" does not contain attribute '
                '"match_kind". Possible values are "points", "scope" and "general". '.format(
                    file_name
                )
                + refer_to_faq_msg(71)
            )
        desc = CustomReplacementDescriptor.create_instance(
            attrs["match_kind"], attrs["id"], attrs
        )
...
def generate_pattern_for_node(graph: Graph, sub_graph_pattern: str, node_name: str):
    ...
    if matched_index is None:
        raise RuntimeError(
            'Node name "{}" does not match pattern "{}"'.format(
    ...
    raise RuntimeError(
        'The pattern that uniquely identifies node "{}" using sub-graph pattern "{}" has not been found'.format(
            node_name, sub_graph_pattern
        )
    )
...
def load_and_validate_json_config(config_file_name: str):
    ...
    try:
        ...
        raise Error(
            "Failed to parse custom replacements configuration file '{}': {}. ".format(
                config_file_name, e
            )
            + refer_to_faq_msg(70)
        ) from e
