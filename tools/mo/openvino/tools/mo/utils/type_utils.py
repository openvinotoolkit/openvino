...
def bool_cast(x):
    if isinstance(x, str):
        return (
            ...
            else True if x.lower() in ["true", "1"] else "unknown_boolean_cast"
        )
    else:
        return bool(x)
...
def override_data_type_of_constant(node: Node, lhs_idx: int = 0, rhs_idx: int = 1):
    ...
    if in_type_0 != in_type_1:
        ...
        if in_node_0.op == "Const":
            ...
            raise Error(
                "{} operation '{}' has inputs of different data types: '{}' and '{}' "
                "that cannot be aligned".format(
                    node.soft_get("op"), node.soft_get("name"), in_type_0, in_type_1
                )
            )
        log.error(
            "Changing Const node '{}' data type from {} to {} for {} operation".format(
                node_to_convert.soft_get("name", node_to_convert.id),
                src_type,
                dst_type,
                node.soft_get("op"),
            ),
