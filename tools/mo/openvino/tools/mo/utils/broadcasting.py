...
def uni_directional_shape_broadcasting(input_shape: np.array, target_shape: np.array):
    ...
    if len(input_shape) > len(target_shape):
        log.debug(
            'The shape "{}" cannot be broadcasted to "{}"'.format(
...
def explicit_shape_broadcasting(
    input_shape: np.array, target_shape: np.array, axes_mapping: np.array
) -> [np.array, np.array]:
    """
    ...
    assert len(axes_mapping) == len(
        input_shape
    ), "size of axes_mapping does not match to rank of input"
    axes_mapping = mo_array(
        list(
            map(
                lambda axis: axis + len(target_shape) if axis < 0 else axis,
                axes_mapping,
            )
        )
    )
    ...
    for i, axis in enumerate(axes_mapping):
        assert (
            0 <= axis < len(res)
        ), "axis value from axes_mapping exceeds rank of target_shape"
        assert (
            res[axis] == input_shape[i]
        ), "specified mapping axis in target_shape differs from axis in input_shape"
...
def uni_directional_broadcasting(input_value: np.array, target_shape: np.array):
    ...
    assert (
        uni_directional_shape_broadcasting(shape_array(input_value.shape), target_shape)
        is not None
    ), 'The tensor of shape "{}" cannot be uni-directionally broadcasted to shape "{}"'.format(
        input_value.shape, target_shape
    )
...
def bi_directional_broadcasting(input_value: np.array, second_shape: np.array):
    ...
    assert output_shape is not None, (
        'The tensor of shape "{}" cannot be bi-directionally broadcasted to shape "{}"'
...
def explicit_broadcasting(
    input_value: np.array, target_shape: np.array, axes_mapping: np.array
) -> np.array:
    """
    ...
    :return: broadcasted value
    """
    res_shape, normalized_axes_mapping = explicit_shape_broadcasting(
        input_value.shape, target_shape, axes_mapping
    )
