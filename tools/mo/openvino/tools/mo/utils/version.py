...
def get_version_file_path():
    return os.path.join(
        os.path.dirname(os.path.realpath(__file__)), os.pardir, "version.txt"
    )
    ...
    try:
        ...
        branch_name = (
            subprocess.check_output(
                ["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=mo_dir
            )
            .strip()
            .decode()
        )
        commit_hash = (
            subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=mo_dir)
            .strip()
            .decode()
        )
        ...
        try:
            version = (
                subprocess.check_output(
                    [
                        sys.executable,
                        os.path.join(os.path.dirname(__file__), "ie_version.py"),
                    ],
                    timeout=2,
                    env=env,
                )
