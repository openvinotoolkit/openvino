...
from openvino.tools.mo.back.ConvolutionNormalizer import (
...
from openvino.tools.mo.utils.ir_reader.layer_to_class import (
    collect_extenders,
    collect_ops,
    copy_graph_with_ops,
)
...
def restore_graph_from_ir(path_to_xml: str, path_to_bin: str = None) -> (Graph, dict):
    ...
    assert ir.graph.graph.get("ir_version") >= 10, (
        "IR version {} is not supported, "
        "please generate actual IR for your model and use it.".format(
            ir.graph.graph.get("ir_version")
        )
    )
...
def save_restored_graph(
    graph: Graph, path: str, meta_data, name=None, rename_results=True
):
    """
    ...
    assert data_type in [
        "FP16",
        "FP32",
    ], "`data_type` value {} is not supported by MO," " cannot save graph".format(
        data_type
    )
    ...
    prepare_emit_ir(
        graph, data_type, path, name, meta_info=meta_data, rename_results=rename_results
    )
