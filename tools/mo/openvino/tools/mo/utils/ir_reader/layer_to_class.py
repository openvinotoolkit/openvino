...
def collect_extenders(path: str):
    ...
    """
    import_by_path(
        os.path.join(path, "mo", "utils", "ir_reader", "extenders"),
...
def restore_correct_ports(graph: Graph):
    ...
    for u, v, k, d in graph.edges(data=True, keys=True):
        ...
        if "in" in d:
            in_port_id = (
                d["in"] if not is_control_flow else "control_flow_" + str(d["in"])
            )
            to_node_attrs["_in_ports"].update(
                {in_port_id: {"control_flow": is_control_flow}}
            )
        if "out" in d:
            node = Node(graph, u)
            ...
            out_port_id = (
                ...
                else "control_flow_" + str(decremented_number)
            )
            from_node_attrs["_out_ports"].update(
                {out_port_id: {"control_flow": is_control_flow}}
            )
...
def propagate_const_values(op: Node):
    ...
    """
    assert (
        op.soft_get("type") == "Const"
    ), "Wrong operation type, {} instead of Const!" "".format(op.soft_get("type"))
    assert 0 in op.in_nodes(), (
        "Can't propagate restored value to Const operation with name: {}, check input ports"
        "".format(op.soft_get("name"))
    )
    assert 0 in op.out_nodes(), (
        "Can't propagate restored value to Const operation with name: {}, check output ports"
        "".format(op.soft_get("name"))
    )
    ...
    assert (
        len(op.out_node(0).out_nodes()) > 0
    ), "Const node {} have no consumers.".format(op.soft_get("name"))
    if op.out_node(0).out_node(0).type == "BinaryConvolution":
        ...
        weights_rounded = weights_rounded.reshape(
            [len(weights_rounded) // 8, 8]
...
def groupconv_to_conv(op: Node):
    ...
    """
    assert (
        op.soft_get("type") == "GroupConvolution"
    ), "Wrong operation type, {} instead of GroupConvolution!".format(
        op.soft_get("type")
    )
    ...
    if weights_node.type == "Const":
        ...
        assert strict_compare_tensors(
            weights_node.in_port(0).get_source().data.get_shape(), new_shape
        ), "Weight shape and calculated shape mismatch in GroupConv node {}.".format(
            op.name
        )
        op.in_port(1).disconnect()
        # We use add_destination method here to support case with multiple destinations of source port
        weights_node.in_port(0).get_source().get_connection().add_destination(
            op.in_port(1)
        )
        weights_node.in_port(0).disconnect()
        op.graph.remove_node(weights_node.id)
    elif (
        weights_node.type == "Convert"
        and weights_node.destination_type == "f32"
...
def ti_add_edge_attrs(op: Node):
    ...
    """
    assert (
        op.soft_get("type") == "TensorIterator"
    ), "Wrong operation type, {} instead of TensorIterator!" "".format(
        op.soft_get("type")
    )
...
def restore_tensor_names(op: Node):
    for out_port in op.ports:
        ...
        if op.soft_get("type") == "Const":
            assert len(op.ports) == 1, (
                "Something wrong with Constant node: {}, wrong number "
        ...
        if out_tensor_names is not None:
            ...
            if out_tensor_names.find(",") >= 0:
                ...
                out_tensor_names = (
                    out_tensor_names.replace("\\,", str_to_replace)
                ).split(",")
...
def copy_graph_with_ops(graph: Graph) -> Graph:
    ...
    for op in graph.get_op_nodes():
        if (
            op.soft_get("type") != "Const"
            and op.soft_get("type") in preprocessing_op_nodes
        ):
            preprocessing_op_nodes[op.type](op)
    ...
    for op in graph.get_op_nodes():
        ...
        if op.type in Extender.registered_ops:
            ...
            log.debug(
                "Extender for node {} with type={} not found, please note.".format(
                    op.name, op.type
                )
            )
                ...
                if "ir_data_attrs" in op:
                    node["IE"] = [
                        (
                            ...
                            [
                                ("data", list(op.ir_data_attrs.keys()), []),
                                "@ports",
                                "@consts",
                            ],
                        )
    ...
    for op in new_graph.get_op_nodes():
        ...
        if op.soft_get("type") == "Const":
            assert (
                len(op.out_nodes()) == 1 and op.out_node(0).soft_get("kind") == "data"
            ), "Const node {} not properly corrected to appropriate data node".format(
                op.soft_get("name")
            )
