...
class Extender(object):
    ...
    def attr_to_list(node: Node, attribute: str):
        if not node.has_valid(attribute):
            log.warning(
                "Attribute {} missed in node {} with type {}!".format(
    ...
    def use_shapes_from_ir(node: Node):
        ...
        assert len(node.new_input_shapes) == len(
        ...
        for new_input_shape, old_input_shape in zip(
            node.new_input_shapes, node.old_input_shapes
        ):
            assert np.array_equal(
                new_input_shape, old_input_shape
            ), "Something wrong happened while {} node with type {} copy shape inference! {} != {}".format(
        ...
        for num in node.out_ports():
            if i in node.ports:
                ...
                assert (
                    node.out_port(num).data.get_shape() is not None
                ), "Newly added port does not have set shape"
            i += 1
