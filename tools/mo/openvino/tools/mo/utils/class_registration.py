...
from openvino.tools.mo.front.common.custom_replacement_registry import (
    CustomReplacementRegistry,
)
...
from openvino.tools.mo.utils.error import Error, FrameworkError, InternalError
            ...
            for replacer_cls in replacers:
                if hasattr(replacer_cls, "id"):
                    ...
                    if id_cls in unique_idxs:
                        raise Error(
                            "Found replacer {} with not unique id!".format(replacer_cls)
                        )
...
def get_enabled_and_disabled_transforms():
    ...
    """
    disabled_transforms = (
        os.environ["MO_DISABLED_TRANSFORMS"]
        if "MO_DISABLED_TRANSFORMS" in os.environ
        else ""
    )
    enabled_transforms = (
        os.environ["MO_ENABLED_TRANSFORMS"]
        if "MO_ENABLED_TRANSFORMS" in os.environ
        else ""
    )
...
def _update(
    ...
    for c in cls.__subclasses__():
        ...
        if (
            hasattr(c, "id")
            and c.id in enabled_transforms
            or ".".join([c.__module__, c.__name__]) in enabled_transforms
        ):
        ...
        if (
            hasattr(c, "id")
            and c.id in disabled_transforms
            or ".".join([c.__module__, c.__name__]) in disabled_transforms
        ):
        ...
        if c not in registered_list:
            ...
            log.warning(
                "Skipped {} registration because it was already registered or it was disabled. ".format(
                    c
                )
            )
...
class DependencyGraph(Graph):
    ...
    def dump_graph_for_graphviz(
        ...
        for src_node_name, dst_node_name, attrs in self.edges(data=True):
            ...
            edge_string = " ".join(
                [
                    str(key) + "=" + str(attrs.get(key, "None"))
    ...
    def cycle_check(self):
        try:
            ...
            raise Error(
                ...
                " -> ".join([str(node) for node in list(cycles)[0]]),
            ) from exception
    ...
    def repeated_cls_names_check(self):
        ...
        for transform_class in self.node:
            ...
            assert (
                transform_name not in name_to_class_map
            ), "Transform name `{}` is not unique: at least {} and {} exist" "".format(
                transform_name, transform_class, name_to_class_map[transform_name]
            )
    ...
    def sort_util(self, v, visited, stack):
        visited.append(v)
        for i in sorted(
            [child for _, child in self.out_edges(v)], key=lambda x: x.__name__
        ):
            if i not in visited:
    ...
    def determined_sort(self):
        ...
        transforms = sorted(
            [cls for cls in self.nodes() if len(self.in_edges(cls)) == 0],
        ...
        try:
            ...
            raise InternalError(
                "Internal DependencyGraph determined_sort function behaves unexpectedly: cycle found"
            ) from e
        assert (
            nx_order == order
...
def get_replacers_order(transform_types: list):
    ...
    dependency_graph = DependencyGraph(
        name="UnifiedPipeline" if len(transform_types) != 1 else transform_types[0].name
    )
    ...
    for class_type, classes_set in _registered_classes_dict.items():
        if class_type in transform_types:
            for cls in classes_set:
                cur_cls_replacers = [
                    c for c in cls.registered_cls if not hasattr(c, "op")
                ] + [c for op, c in cls.registered_ops.items() if c]
                replacers.extend(
                    [
                        replacer
                        for replacer in cur_cls_replacers
    ...
    for i, replacer_cls in enumerate(replacers_order):
        debug_msg_list.append(
            "|{:5} |{:^9}| {}".format(
                i, str(getattr(replacer_cls, "enabled", None)), replacer_cls
            )
        )
...
def apply_transform(graph: Graph, replacer_cls, **kwargs):
    ...
    if hasattr(replacer, "graph_condition") and not all(
        [condition(graph) for condition in replacer.graph_condition]
    ):
        log.info(
            "Skip replacer {} (graph_condition not satisfied)".format(replacer_cls)
        )
        return
    ...
    try:
        ...
        if (
            hasattr(replacer, "force_shape_inference")
