...
def get_canonical_axis_index_node(rank: Node, axis: int) -> Node:
    ...
    if axis < 0:
        axis = Const(
            graph, {"name": name + "/negative_axis", "value": int64_array(axis)}
        ).create_node()
        ...
        return Const(
            graph, {"name": name + "/positive_axis", "value": int64_array(axis)}
        ).create_node()


def get_range_node_of_idxs(
    rank: Node,
    begin: int,
    end: int,
    ...
    if not include_begin:
        const = Const(
            graph, {"value": int64_array(1), "name": name + "/exclude_begin/value"}
        ).create_node()
    ...
    if include_end:
        const = Const(
            graph, {"value": int64_array(1), "name": name + "/including_end/value"}
        ).create_node()
    ...
    delta = Const(
        graph, {"name": name + "/delta", "value": int64_array(1)}
    ).create_node()
...
def get_shape_values_by_indices_node(shape_node: Node, indices_node: Node) -> Node:
    ...
    axis = Const(
        graph, {"value": int64_array(0), "name": shape_node.name + "/Axis"}
    ).create_node()
...
def node_to_get_shape_value_of_indices(shape_node: Node, indices: list) -> Node:
    ...
    graph = shape_node.graph
    indices_node = Const(
        graph, {"value": int64_array(indices), "name": shape_node.name + "/Indices"}
    ).create_node()
...
def get_shape_values_by_range_idxs(
    shape: Node,
    rank: Node,
    begin: int,
    end: int,
    ...
    """
    range_node = get_range_node_of_idxs(
        rank, begin, end, include_begin=include_begin, include_end=include_end
    )
...
def node_to_get_spatial_dimensions_value(shape_node: Node) -> Node:
    ...
    if layout == "NCHW":
        return node_to_get_shape_value_of_indices(
            shape_node, list(range(2, len(shape)))
        )
    elif layout == "NHWC":
        return node_to_get_shape_value_of_indices(
            shape_node, list(range(1, len(shape) - 1))
        )
    else:
        assert 'Unsupported layout "{}"'.format(layout)
...
def new_shape_node_from_shape_nodes(input_shape_nodes: list):
    ...
    """
    assert (
        len(input_shape_nodes) > 0
    ), "The list of input shape nodes should be non-empty"
    new_shape_node = Concat(
        input_shape_nodes[0].graph,
        {
            "axis": 0,
            "name": input_shape_nodes[0].soft_get("name", input_shape_nodes[0].id)
            + "/shapes_concat",
        },
    ).create_node()
