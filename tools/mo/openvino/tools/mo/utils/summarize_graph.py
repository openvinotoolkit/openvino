...
def summarize_graph(graph_def):
    ...
    for node in graph.as_graph_def().node:  # pylint: disable=no-member
        if node.op == "Placeholder":
            ...
            node_dict["shape"] = (
                str(tf_v1.TensorShape(node.attr["shape"].shape))
                .replace(" ", "")
                .replace("?", "-1")
            )
...
def main():
    ...
    parser.add_argument(
        "--input_model", type=str, help="Path to tensorflow model", default=""
    )
    parser.add_argument(
        "--input_model_is_text",
        dest="text",
    ...
    parser.add_argument(
        "--input_meta",
        action="store_true",
        help="TensorFlow*: treat the input model file as a meta graph def format",
        default=False,
    )
    parser.add_argument(
        "--input_checkpoint",
    ...
    parser.add_argument(
        "--saved_model_dir", type=str, default="", help="TensorFlow saved_model_dir"
    )
    parser.add_argument(
        "--saved_model_tags",
        type=str,
    ...
    if not argv.input_model and not argv.saved_model_dir:
        print(
            "[ ERROR ] Please, provide --input_model and --input_model_is_text if needed or --input_dir for saved "
            "model directory"
        )
        sys.exit(1)
    if argv.input_model and argv.saved_model_dir:
        print(
            "[ ERROR ] Both keys were provided --input_model and --input_dir. Please, provide only one of them"
        )
        sys.exit(1)
    tags = argv.saved_model_tags.split(",")
    graph_def, _, _, _ = load_tf_graph_def(
        graph_file_name=argv.input_model,
        is_binary=not argv.text,
