...
try:
    ...
    mo_root_path = os.path.normpath(
        os.path.join(os.path.dirname(__file__), os.pardir, os.pardir)
    )
    sys.path.insert(0, mo_root_path)
    execution_type = "install_prerequisites.{}".format(
...
def import_core_modules(silent: bool, path_to_module: str):
    ...
    try:
        ...
        from openvino.runtime.op import (  # pylint: disable=import-error,no-name-in-module
            Parameter,
        )
        ...
        print(
            "{}: \t{}".format(
                "OpenVINO runtime found in", os.path.dirname(openvino.__file__)
            )
        )
        ...
        if "No module named 'openvino" not in str(e):
            print(
                "[ WARNING ] Failed to import OpenVINO Python API in: {}".format(
                    path_to_module
                )
            )
