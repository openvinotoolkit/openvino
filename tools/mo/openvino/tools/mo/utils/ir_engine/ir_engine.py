...
class IREngine(object):
    def __init__(
        self, path_to_xml: str, path_to_bin=None, precision="FP32", xml_tree=None
    ):
    ...
    def __load_xml(self):
        ...
        self.graph.graph["ir_version"] = (
            int(xml_root.attrib["version"])
            if xml_root.attrib.get("version") is not None
            else None
        )
        ...
        self.graph.name = (
            xml_root.attrib["name"] if xml_root.attrib.get("name") is not None else None
        )
            ...
            # TODO: Remove this part when POT updates to using of rt_info
                ...
                for elem in child:
                    if elem.tag == "config":
                        ...
                        self.meta_data["quantization_parameters"][elem.tag] = (
                            elem.attrib["value"]
                        )

        self.graph.graph["cmd_params"] = Namespace(
            **self.meta_data
        )  # TODO check what we need all this attrs
        ...
        for node in nodes:
            ...
            for out_node, edge_attrs in out_edges:
                ...
                self.graph.add_edges_from(
                    [(data, out_node, {"in": edge_attrs["to_port"]})]
                )
            ...
            for w in ["weights", "biases", "custom"]:
                if w in graph.node[node]:
                    ...
                    if Node(graph, node).soft_get("type") == "BinaryConvolution":
                        precision = np.uint8
                    value = np.frombuffer(
                        buffer=bin_buff, dtype=precision, count=size, offset=offset
                    )
                    hashes[graph.node[node]["name"]][w] = hashlib.sha512(
                        value.tobytes()
                    ).hexdigest()
                    graph.add_node(
                        data, **{"kind": "data", "value": value, "shape": value.shape}
                    )
    ...
    def __load_bin_hashes(self):
        ...
        bin_hash_map = {
            ...
            for name, blob_map in dict(
                np.load(self.path_to_bin, allow_pickle=True)
            ).items()
        }
        ...
        for attr in layer:
            if attr.tag == "data":
                ...
                if layer.attrib["type"] == "Const":
                    assert (
                        "offset" in new_attrs and "size" in new_attrs
                    ), "Incorrect attributes for Const layer, {} instead of {}!".format(
                        new_attrs.keys(), ["offset", "size"]
                    )
                    precision = ""
                    for item in layer:
                        if item.tag == "output":
                ...
                for port in input:
                    ...
                    input_shape = shape_array(
                        [d if d != -1 else dynamic_dimension_value for d in input_shape]
                    )
                ...
                assert layer.attrib["type"] == "If", (
                    "Incorrect IR! The operation {0}" " has sub-graphs for If operation"
                )
    ...
    def __find_input(graph):
        ...
        if len(inputs) < 1:
            raise RuntimeError(
                "Graph {} has less than one input node".format(graph.name)
            )
    ...
    def generate_bin_hashes_file(self, path_for_file=None):
        ...
        assert (
            "hashes" in graph.graph
        ), "Loaded IR graph doesn't contain `hashes`: {}".format(self.path_to_xml)
    ...
    def get_inputs(self):
        ...
        return {
            input_node.name: input_node.out_node().shape for input_node in input_nodes
        }
    ...
    def __read_subgraph(self, layer, layer_attrs, body_child, port_map_name):
        ...
        assert (
            not len(xml_port_map) != 1
        ), "If then_body won't be compared due to missing {1} section in node {0}! ".format(
            layer_attrs["name"], port_map_name
        )
    ...
    def __read_if(self, layer, layer_attrs):
        ...
        assert (
            len(xml_then_body_child) == 1 and len(xml_else_body_child) == 1
        ), "If operation has only one subgraph"

        then_body_ir, then_input_port_map, then_output_port_map, _ = (
            self.__read_subgraph(
                layer, layer_attrs, xml_then_body_child, "then_port_map"
            )
        )
        ...
        else_body_ir, else_input_port_map, else_output_port_map, _ = (
            self.__read_subgraph(
                layer, layer_attrs, xml_else_body_child, "else_port_map"
            )
        )
    ...
    def __read_rt_info(self, layer, layer_attrs):
        ...
        for attr in xml_rt_info:
            ...
            if attr_name == "old_api_map_order":
                ...
                rt_info.info.update(
                    self.__read_old_api_map_element_type(attr, layer.attrib["type"])
                )
            else:
    ...
    def __read_old_api_map_order(attr, layer_type):
        ...
        if layer_type == "Parameter":
            ...
            raise AttributeError(
                "Cannot read old_api_map for layer of type: {}".format(layer_type)
            )
