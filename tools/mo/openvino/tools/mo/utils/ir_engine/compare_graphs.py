...
def compare_node(node_ref, node, ref_attr_value, attr_value, attr, errors_list: list):
    ...
    def err_format_string():
        return (
            'Current node "{}" with type "{}" and reference node "{}" with type "{}" have different attr "{}" : '
            "{} and {}".format(
                node.id,
                node.soft_get("type", None),
                node_ref.id,
                node_ref.soft_get("type", None),
                attr,
                attr_value,
                ref_attr_value,
            )
...
def compare_graphs(
    ...
    if "statistics" in graph.graph and "statistics" in graph_ref.graph:
        assert (
            graph.graph["statistics"] == graph_ref.graph["statistics"]
        ), "int8 statistics comparison failed"
    ...
    while len(q_ref) != 0:
        ...
        if len(node_ref.out_nodes()) != len(node.out_nodes()):
            stderr.append(
                'Current node "{}" and reference node "{}" have different amount of output nodes: {} vs {}'.format(
                    node.id,
                    node_ref.id,
                    len(node.out_nodes()),
                    len(node_ref.out_nodes()),
                )
            )
            continue
        ...
        if len(node_ref.in_nodes()) != len(node.in_nodes()):
            stderr.append(
                'Current node "{}" and reference node "{}" have different amount of input nodes: {} vs {}'.format(
                    node.id, node_ref.id, len(node.in_nodes()), len(node_ref.in_nodes())
                )
            )
            continue
        ...
        if node_ref.kind != node.kind:
            stderr.append(
                'Current node "{}" and reference node "{}" have different kind parameter'.format(
                    node.id, node_ref.id
                )
            )
            return (False, stderr)
        ...
        if node_ref.has_valid("can_be_fused"):
            if node_ref.soft_get("can_be_fused") != node.soft_get("can_be_fused"):
                stderr.append(
                    'Current node "{}" and reference node "{}" have different "can_be_fused" parameter '
                    "{} and {}".format(
                        node.id,
                        node_ref.id,
                        node.soft_get("can_be_fused"),
        ...
        if node_ref.kind == "op":
            ...
            if check_op_attrs:
                ...
                for attr in graph_ref.node[node_ref.id]:
                    ...
                    def align_strided_slice_masks(curr_node: Node, rank: int):
                        ...
                        for mask_name in StridedSlice.get_mask_names():
                            if isinstance(curr_node[mask_name], int):
                                ...
                                curr_node[mask_name], str
                            ):  # if mask is an empty string ''
                                assert len(curr_node[mask_name]) == 0
                                curr_node[mask_name] = []

                            num_insertions = rank - len(curr_node[mask_name])
                            curr_node[mask_name] = np.append(
                                curr_node[mask_name], [0] * num_insertions
                            ).astype(int)
                    ...
                    if (
                        cur_node_type == "StridedSlice"
                        and node.in_node(1).has("shape")
            ...
            if (
                node_ref.has_valid("shape")
                and node_ref.shape is not None
                and not np.array_equal(node_ref.shape, node.shape)
            ):
                stderr.append(
                    'Current node "{}" and reference node "{}" have different shapes {} and {}'.format(
                        node.id, node_ref.id, node.shape, node_ref.shape
                    )
                )
                continue

            if (
                node_ref.has_valid("value")
                and node_ref.value is not None
                and not values_are_equal(node.value, node_ref.value)
            ):
                stderr.append(
                    'Current node "{}" and reference node "{}" have different values \n{} \nand \n{}'.format(
                        node.id, node_ref.id, node.value, node_ref.value
                    )
                )
        ports = sorted(node.in_nodes().keys()) if node.kind == "op" else None
        in_nodes = (
            [node.in_node(k) for k in ports] if node.kind == "op" else node.in_nodes()
        )
        ...
        ports_ref = (
            sorted(node_ref.in_nodes().keys()) if node_ref.kind == "op" else None
        )
        if ports != ports_ref:
            stderr.append(
                'Current node "{}" and reference node "{}" have different ports'.format(
                    node.id, node_ref.id
                )
            )
            return (False, stderr)
        ...
        in_nodes = (
            [node_ref.in_node(k) for k in ports]
            if node_ref.kind == "op"
            else node_ref.in_nodes()
        )
        for in_node in in_nodes:
