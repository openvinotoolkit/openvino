...
import numpy as np
from openvino.runtime import Dimension, Layout, PartialShape, Shape, Type
...
from openvino.tools.mo.utils.help import (
    get_convert_model_help_specifics,
    get_to_string_methods_for_params,
)
...
def path_to_str(path):
    ...
    if isinstance(path, str):
        ...
        raise Exception(
            "Incorrect type of {} expected str or Path, got {}".format(path, type(path))
        )
    ...
    if isinstance(values, str):
        ...
        for value in values:
            if not isinstance(value, str):
                raise Error(
                    "Incorrect argument. {} expected to string, got type {}.".format(
                        value, type(value)
                    )
                )
        return ",".join(values)
        ...
        raise Error(
            "Incorrect argument. {} expected to string or list of strings, got type {}.".format(
                values, type(values)
            )
        )
...
def value_to_str(value, separator):
    ...
    if isinstance(value, list):
        ...
        for x in value:
            if not isinstance(x, numbers.Number):
                raise Exception(
                    "Incorrect value type. Expected numeric value, got {}".format(
                        type(x)
                    )
                )
            values.append(str(x))
    ...
    raise Exception(
        "Incorrect value type. Expected np.ndarray or list, got {}".format(type(value))
    )
...
def single_input_to_input_cut_info(input: [str, tuple, list, PartialShape, Type, type]):
    ...
    if isinstance(input, str):
        ...
        return openvino.tools.mo.InputCutInfo(
            node_name,
    ...
    raise Exception(
        "Unexpected object provided for input. Expected openvino.toos.mo.InputCutInfo "
...
def input_to_input_cut_info(input: [str, tuple, list]):
    ...
    if isinstance(input, str):
        ...
        for input_value in split_inputs(input):
            ...
            inputs.append(
                openvino.tools.mo.InputCutInfo(
                    node_name,
                    PartialShape(shape) if shape is not None else None,
                    data_type,
                    value,
...
def input_shape_to_input_cut_info(
    input_shape: [str, Shape, PartialShape, list, tuple], inputs: list
):
    """
    ...
    if isinstance(input_shape, (list, tuple)):
        ...
        if len(inputs) > 0 and len(input_shape) > 0:
            assert len(inputs) == len(input_shape), (
                'Different numbers of inputs were specified in "input" parameter '
                'and "input_shapes". "input" has {} items, "input_shape" has {} item.'.format(
                    len(inputs), len(input_shape)
                )
            )
        ...
        if len(inputs) > 0:
            for idx, shape in enumerate(input_shape):
                ...
                assert inputs[idx].shape is None, (
                    'Shape was set in both "input" and in "input_shape" parameter.'
                    'Please use either "input" or "input_shape" for shape setting.'
                )
                inputs[idx] = openvino.tools.mo.InputCutInfo(
                    inputs[idx].name, shape, inputs[idx].type, inputs[idx].value
                )
            ...
            for shape in input_shape:
                inputs.append(
                    openvino.tools.mo.InputCutInfo(
                        None, PartialShape(shape), None, None
                    )
                )
        return

    raise Exception(
        "Unexpected object provided for input_shape. Expected PartialShape, Shape, tuple, list or str. "
        "Got {}".format(type(input_shape))
    )


def freeze_placeholder_to_input_cut_info(
    argv_freeze_placeholder_with_value: str, inputs: list
):
    """
    ...
    placeholder_values = parse_freeze_placeholder_values(
    ...
    if inputs is not None and len(inputs) > 0:
        for input in inputs:
            ...
            if (
                node_name in placeholder_values
                and placeholder_values[node_name] != value
            ):
                raise Error(
...
def layout_to_str(layout):
    ...
    raise Exception(
        "Incorrect layout type. Expected Layout or string or dictionary, "
        "where key is operation name and value is layout or list of layouts, got {}".format(
            type(layout)
        )
    )
...
def layoutmap_to_str(value):
    ...
    if isinstance(value, openvino.tools.mo.LayoutMap):
        assert (
            value.source_layout is not None
        ), "Incorrect layout map. 'source_layout' should be set."
...
def transform_param_value_to_str(value):
    ...
    if isinstance(value, dict):
        ...
        for input_name, output_name in value.items():
            assert isinstance(
                input_name, str
            ), "Incorrect input name. " "Expected string, got {}".format(
                type(input_name)
            )
            assert isinstance(
                output_name, str
            ), "Incorrect output name. " "Expected string, got {}".format(
                type(output_name)
            )
...
def transform_to_str(value):
    ...
    if isinstance(value, tuple):
        assert 1 <= len(value) <= 2, (
            "Incorrect definition of transformation in transform argument: "
            "expected two elements in tuple, provided {}. "
            "Supported transforms are: {}".format(
                len(value), list(get_available_transformations().keys())
            )
        )
        transform_name = value[0]
        assert isinstance(
            transform_name, str
        ), "Incorrect transform name type. " "Expected string, got {}".format(
            type(transform_name)
        )
        if len(value) == 2:
            params = value[1]
            assert isinstance(params, dict), (
                "Incorrect transform params type. "
                "Expected dictionary, got {}".format(type(params))
            )
            params_str_list = []
            for param_name, val in params.items():
                assert isinstance(param_name, str), (
                    "Incorrect transform parameter name type. "
    ...
    raise Exception(
        "Incorrect transform type. Expected tuple with transform name and "
        "dictionary with transform parameters. Got object of type {}".format(
            type(value)
        )
    )
...
ParamDescription = namedtuple(
    "ParamData", ["description", "cli_tool_description", "to_string"]
)
...
def get_mo_convert_params():
    ...
    while len(mo_convert_docs) > 0:
        ...
        mo_convert_params[group][param_name] = ParamDescription(
    ...
    for group_name, param_group in mo_convert_params.items():
        for param_name, d in param_group.items():
            ...
            desc = ParamDescription(d.description, cli_tool_description, to_str_method)
...
class IgnoredAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        dep_msg = (
            "Use of removed cli option '{}' detected. The option is ignored. ".format(
                option_string
            )
        )
        log.error(dep_msg, extra={"is_warning": True})
...
def canonicalize_and_check_paths(
    values: Union[str, List[str]], param_name, try_mo_root=False, check_existence=True
) -> List[str]:
    if values is not None:
        ...
        if isinstance(values, str):
            ...
            raise Error(
                'Unsupported type of command line parameter "{}" value'.format(
                    param_name
                )
            )
        ...
        for idx, val in enumerate(list_of_values):
            ...
            error_msg = (
                'The value for command line parameter "{}" must be existing file/directory, '
...
class CanonicalizePathAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        list_of_paths = canonicalize_and_check_paths(
            values, param_name=option_string, try_mo_root=False, check_existence=False
        )
...
class CanonicalizeTransformationPathCheckExistenceAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        list_of_paths = canonicalize_and_check_paths(
            values, param_name=option_string, try_mo_root=True, check_existence=True
        )
        setattr(namespace, self.dest, ",".join(list_of_paths))
...
class CanonicalizePathCheckExistenceIfNeededAction(
...
class DeprecatedCanonicalizePathCheckExistenceAction(
    CanonicalizePathCheckExistenceAction
):
    def __call__(self, parser, namespace, values, option_string=None):
        dep_msg = "Use of deprecated cli option {} detected. Option use in the following releases will be fatal. ".format(
...
def add_args_by_description(args_group, params_description):
    ...
    for param_name, param_description in params_description.items():
        ...
        if cli_param_name not in args_group._option_string_actions:
            ...
            param_alias = (
                param_specifics["aliases"] if "aliases" in param_specifics else {}
            )
            param_version = (
...
def get_common_cli_options(model_name):
    ...
    d["output_dir"] = [
        ...
        lambda x: x if x != "." else os.getcwd(),
    ]
...
def get_advanced_cli_options():
    d = OrderedDict()
    d["use_legacy_frontend"] = (
        "- Force the usage of legacy Frontend for model conversion into IR"
    )
    d["use_new_frontend"] = (
        "- Force the usage of new Frontend for model conversion into IR"
    )
    return d
...
def get_data_type_from_input_value(input_value: str):
    ...
    return (
        destination_type_to_np_data_type(data_type_match.group(1))
...
def remove_shape_from_input_value(input_value: str):
    ...
    assert (
        "->" not in input_value
    ), "The function should not be called for input_value with constant value specified"
...
def get_shape_from_input_value(input_value: str):
    ...
    if len(shape) == 0:
        ...
        raise Error(
            'Wrong syntax to specify shape. Use "input" ' '"node_name[shape]->value"'
        )
    return shape
...
def get_node_name_with_port_from_input_value(input_value: str):
    ...
    """
    return remove_shape_from_input_value(
        remove_data_type_from_input_value(input_value.split("->")[0])
    )
...
def get_value_from_input_value(input_value: str):
    ...
    if len(parts) == 2:
        ...
        if value[0] == "[" and value[-1] != "]" or value[0] != "[" and value[-1] == "]":
            raise Error(
                'Wrong syntax to specify value. Use "input"="node_name[shape]->value"'
            )
        if "[" in value.strip(" "):
...
def partial_shape_prod(shape: [PartialShape, tuple]):
    assert not (
        isinstance(shape, PartialShape) and shape.is_dynamic
...
def parse_input_value(input_value: str):
    ...
    if (
        ...
        and partial_shape_prod(shape) != value_size
    ):
        raise Error(
            "The shape '{}' of the input node '{}' does not correspond to the number of elements '{}' in the "
...
def write_found_layout_list(
    idx: int, found_layout: str, parsed: list, dest: str = None
):
    """
    Writes found layout data to the 'parsed' dict.
...
def parse_layouts_by_destination(
    s: str, parsed: dict, parsed_list: list, dest: str = None
) -> None:
    """
    ...
    if len(list_s) == 1 and (
        list_s[0][-1] not in ")]" or (list_s[0][0] == "[" and list_s[0][-1] == "]")
    ):
        # single layout case
...
def get_layout_values(
    argv_layout: str = "", argv_source_layout: str = "", argv_target_layout: str = ""
):
    """
    ...
    if argv_layout and (argv_source_layout or argv_target_layout):
        raise Error(
            '"layout" is used as well as "source_layout" and/or "target_layout" which is not allowed, please '
            "use one of them."
        )
    res = {}
    res_list = []
    ...
    if len(res) > 0 and len(res_list) > 0:
        raise Error(
            "Some layout values are provided with names, and some without names. "
    ...
    if argv_freeze_placeholder_with_value is not None:
        for plh_with_value in argv_freeze_placeholder_with_value.split(","):
            ...
            if len(plh_with_value) != 2:
                raise Error(
                    'Wrong replacement syntax. Use "freeze_placeholder_with_value" '
...
def get_freeze_placeholder_values(
    argv_input: str, argv_freeze_placeholder_with_value: str
):
    """
...
def split_shapes(argv_input_shape: str):
    ...
    tuple_reg = r"((\({}{}\))|(\[{}{}\]))".format(
        first_digit_reg, next_digits_reg, first_digit_reg, next_digits_reg
    )

    full_reg = r"^{}(\s*,\s*{})*$|^$".format(tuple_reg, tuple_reg)
    if not re.match(full_reg, argv_input_shape):
        raise Error(
            'Input shape "{}" cannot be parsed. ' + refer_to_faq_msg(57),
            argv_input_shape,
        )
...
def get_placeholder_shapes(argv_input: str, argv_input_shape: str, argv_batch=None):
    ...
    if argv_input_shape and argv_batch:
        raise Error(
            'Both "input_shape" and "batch" were provided. Please provide only one of them. '
            + refer_to_faq_msg(56)
        )
    ...
    if argv_input_shape and are_shapes_specified_through_input:
        raise Error(
            'Shapes are specified using both "input" and "input_shape" command-line parameters, but only one '
            "parameter is allowed."
        )

    if argv_batch and are_shapes_specified_through_input:
        raise Error(
            'Shapes are specified using both "input" and "batch" command-line parameters, but only one '
            "parameter is allowed."
        )
        ...
        for inp in inputs:
            ...
            for dim in shape:
                if isinstance(dim, Dimension) and not dim.is_static:
                    raise Error(
                        "Cannot freeze input with dynamic shape: {}".format(shape)
                    )

    elif argv_input:
        raise Error(
            "Please provide each input layers with an input layer shape. "
            + refer_to_faq_msg(58)
        )
...
def parse_tuple_pairs(argv_values: str):
    ...
    matches = [
        m for m in re.finditer(r"[(\[]([0-9., -]+)[)\]]", argv_values, re.IGNORECASE)
    ]

    error_msg = (
        "Mean/scale values should consist of name and values specified in round or square brackets "
        "separated by comma, e.g. data(1,2,3),info[2,3,4],egg[255] or data(1,2,3). Or just plain set of "
        "values without names: (1,2,3),(2,3,4) or [1,2,3],[2,3,4]."
...
def get_tuple_values(
    argv_values: str or tuple, num_exp_values: int = 3, t=float or int
):
    """
    ...
    content = r"{0}\s*,{1}\s*{0}".format(
        digit_reg, (digit_reg + ",") * (num_exp_values - 2)
    )
    ...
    if not len(argv_values) or not re.match(tuple_reg, argv_values):
        raise Error(
            'Values "{}" cannot be parsed. ' + refer_to_faq_msg(59), argv_values
        )
    ...
    for mean in mean_values_matches:
        if len(mean.split(",")) != num_exp_values:
            raise Error(
                "{} channels are expected for given values. " + refer_to_faq_msg(60),
                num_exp_values,
            )
...
def get_mean_scale_dictionary(mean_values, scale_values, argv_input: list):
    ...
    if argv_input:
        inputs = [
            get_node_name_with_port_from_input_value(input_value)
            for input_value in split_inputs(argv_input)
        ]
    else:
        inputs = []
    ...
    if type(mean_values) is dict and type(scale_values) is dict:
        ...
        for inp_scale in scale_values.keys():
            if inp_scale not in inputs:
                raise Error(
                    "Specified scale_values name '{}' do not match to any of inputs: {}. "
                    "Please set 'scale_values' that correspond to values from input.".format(
                        inp_scale, inputs
                    )
                )
        ...
        for inp_mean in mean_values.keys():
            if inp_mean not in inputs:
                raise Error(
                    "Specified mean_values name '{}' do not match to any of inputs: {}. "
                    "Please set 'mean_values' that correspond to values from input.".format(
                        inp_mean, inputs
                    )
                )
        ...
        for inp in inputs:
            ...
            if inp in mean_values or inp in scale_values:
                res.update(
                    {
                        inp: {
                            "mean": mean_values[inp] if inp in mean_values else None,
    ...
    if inputs:
        if mean_values and scale_values:
            if len(inputs) != len(mean_values):
                raise Error(
                    "Numbers of inputs and mean values do not match. "
                    + refer_to_faq_msg(61)
                )
            if len(inputs) != len(scale_values):
                raise Error(
                    "Numbers of inputs and scale values do not match. "
                    + refer_to_faq_msg(62)
                )
        ...
        if mean_values:
            ...
            for i in range(len(data)):
                res.update({inputs[i]: {"mean": data[i], "scale": None}})
            return res
        ...
        if scale_values:
            ...
            for i in range(len(data)):
                res.update({inputs[i]: {"mean": None, "scale": data[i]}})
            return res
    ...
    for transform in all_transforms:
        ...
        if len(args) != 0:
            for arg in args.split(";"):
                ...
                if not m:
                    raise Error(
                        "Unrecognized attributes for transform key: {}".format(
                            transform
                        )
                    )
...
def check_available_transforms(transforms: list):
    ...
    if len(missing_transformations) != 0:
        raise Error(
            "Following transformations ({}) are not available. "
            "List with available transformations ({})".format(
                ",".join(missing_transformations), ",".join(available_transforms.keys())
            )
        )
    return True
