...
from openvino.tools.mo.middle.passes import convert_data_type, tensor_names
...
def get_fw_tensor_debug_info(node: Node):
    while (
        not node.has_valid("fw_tensor_debug_info")
        and not node.has_valid("output_sort_order")
        ...
        try:
            ...
            log.warning(
                "Was not able to determine tensor debug info for node {}".format(
                    node.name
                )
            )
...
def get_sorted_outputs(graph: Graph):
    ...
    for node in outputs:
        ...
        if isinstance(debug_info, str):
            ...
            raise Error(
                "Unsupported type of the variable with debug information used to sort output nodes"
            )
    if len(outputs_for_sort) != len(set(outputs_for_sort.values())):
        log.warning(
            "There are at least two output nodes with the same key used to sort the outputs. This means that "
    ...
    return [
        Node(graph, key)
        for key, value in sorted(outputs_for_sort.items(), key=itemgetter(1))
    ]
...
def relabel_nodes_inplace_safe(graph: Graph, new_labels: dict):
    ...
    intermediate_map = {
        node: graph.unique_id("__relabel__{}__".format(str(i)))
        for i, node in enumerate(graph.nodes())
    }
    final_map = {dst: new_labels[src] for src, dst in intermediate_map.items()}
    assert (
        len(set(intermediate_map.keys()).intersection(set(intermediate_map.values())))
        == 0
    )
...
def convert_const_node_value_type(const_node: Node, np_data_type):
    ...
    log.warning(
        'Converting type of Const node "{}" to "{}"'.format(
        ...
        if node.soft_get("version") != "opset11":
            ...
            if node.soft_get("type") in type_port:
                ...
                for port_id, precision in ports_to_update.items():
                    if (
                        port_id in node.in_ports()
                        and not node.in_port(port_id).disconnected()
                    ):
                        log.debug(
                            'Converting value for the input port "{}" of op "{}" to "{}".'
                            "".format(
                                port_id, node.soft_get("name", node.id), precision
                            )
                        )
                        ...
                        if in_node.type == "Const":
                            if np.issubdtype(in_type, np.integer) and np.issubdtype(
                                np_type, np.integer
                            ):
...
def prepare_emit_ir(
    ...
    for_graph_and_each_sub_graph_recursively(
        graph, OpVersioning().find_and_replace_pattern
    )
    ...
    for_graph_and_each_sub_graph_recursively(
        graph, RemoveUselessConvert().find_and_replace_pattern
    )
    ...
    bin_file = os.path.join(
        output_dir, "{}{}.bin".format(output_model_name, ir_path_suffix)
    )
    ...
    generate_ie_ir(
        ...
        file_name=os.path.join(
            output_dir, "{}{}.xml".format(output_model_name, ir_path_suffix)
        ),
    ...
    tensor_names.output_tensor_names_map(
        graph,
        os.path.join(
            output_dir, "{}{}.mapping".format(output_model_name, ir_path_suffix)
        ),
    )
