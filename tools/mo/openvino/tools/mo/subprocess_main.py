...
def log_ie_not_found():
    log.error(
        "Could not find the OpenVINO or Python API.\n"
...
def setup_env():
        ...
        try:
            status = subprocess.run(
                [
                    sys.executable,
                    os.path.join(
                        mo_root_path, "openvino/tools/mo/utils/check_mo_import.py"
                    ),
                ],
                env=os.environ,
            )
    ...
    ie_found = True
    try:
        from openvino.tools.mo.utils.find_ie_version import (  # pylint: disable=no-name-in-module
            find_ie_version,
        )
