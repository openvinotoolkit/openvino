...
class If(Op):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        base_attrs = {
            ...
            "sub_graphs": [
                "then_graph",
                "else_graph",
            ],  # built-in attribute with all sub-graphs
            "version": "opset8",
            "infer": self.infer,
    ...
    def port_map_attrs(self):
        return ["external_port_id", "internal_layer_id"]

    @staticmethod
    def connect_body_input(
        if_node: Node, condition: bool, if_input_port_idx: int, body_parameter: Node
    ):
        """
    ...
    def connect_body_output(
        if_node: Node, condition: bool, if_output_port_idx: int, internal_result: Node
    ):
        """
    ...
    def update_body_parameters_type(if_node: Node, condition: bool):
        ...
        for node in subgraph.get_op_nodes():
            if node.has("input_id"):
                ...
                log.debug(
                    'Updated data type for the body node with name "{}" with value {}'.format(
    ...
    def update_body_parameters_shape(if_node: Node, condition: bool):
        ...
        for node in subgraph.get_op_nodes():
            if node.has("input_id"):
                ...
                log.debug(
                    'Updated shape for the body node with name "{}" with value {}'.format(
                        node.soft_get("name", node.soft_get("id")), node.shape
                    )
                )

    @staticmethod
    def results_mapping_and_finding_fake_outputs(
        output_nodes_in_subgraph, branch_name, outputs_mapping
    ):
        """
        ...
        for output_node in output_nodes_in_subgraph:
            ...
            assert port_id in outputs_mapping.keys(), (
                "Incorrect mapping then_graph outputs with {0} outputs! "
                "Can't find port with ID {1} in If operation.".format(
                    output_node.name, port_id
                )
            )
            outputs_mapping[port_id][branch_name] = output_node
            out_node_shape = output_node.in_port(0).data.get_shape()
            graph_contain_fake_outputs = graph_contain_fake_outputs and np.any(
                unmask_shape(out_node_shape) == 0
            )
    ...
    def update_if_output_ports_shape(if_node: Node):
        ...
        then_outputs = [
            node for node in if_node.then_graph.get_op_nodes() if node.has("output_id")
        ]
        else_outputs = [
            node for node in if_node.else_graph.get_op_nodes() if node.has("output_id")
        ]
        outputs_mapping = {}
        ...
        then_contains_fake_outputs = If.results_mapping_and_finding_fake_outputs(
            then_outputs, "then_graph", outputs_mapping
        )
        else_contains_fake_outputs = If.results_mapping_and_finding_fake_outputs(
            else_outputs, "else_graph", outputs_mapping
        )
        ...
        for port_id in outputs_mapping:
            ...
            assert "then_graph" in then_else_nodes.keys(), (
                "then_graph does not connect with If.out_port[{0}] "
                "in {1} node!".format(port_id, node_name)
            )
            assert "else_graph" in then_else_nodes.keys(), (
            ...
            if is_fully_defined(cond_value):
                ...
                if then_contains_fake_outputs ^ else_contains_fake_outputs:
                    ...
                    assert len(then_shape) == len(else_shape), (
                        'Ranks of "then" and "else" output tensors are '
                        "different for node {} for port {}".format(node_name, port_id)
                    )
                    output_shape = [
                        (
                            d1
                            if is_fully_defined(d1)
                            and is_fully_defined(d2)
                            and d1 == d2
    ...
    def update_if_output_ports_type(if_node: Node):
        ...
        outputs_mapping = {}
        outputs_number = len(if_node.out_ports())
        assert outputs_number == len(then_outputs), (
            "Incorrect number outputs in then_graph of If with "
            "name {0}! then_graph must has {1} outputs".format(
                if_node.name, outputs_number
            )
        )
        assert outputs_number == len(else_outputs), (
            "Incorrect number outputs in else_graph of If with "
            "name {0}! else_graph must has {1} outputs".format(
                if_node.name, outputs_number
            )
        )
        for port_id in if_node.out_ports().keys():
        ...
        for port_id in outputs_mapping:
            ...
            assert then_type == else_type, (
                "Cannot get type for if.out_port[{0}]! "
                ...
                "IE": [
                    (
                        ...
                        [
                            (
                                "id",
                                lambda node: self.re_numerate_internal_id_and_get_if_id(
                                    node
                                ),
                            ),
                            "name",
    ...
    def generate_port_map(if_node: Node, condition: bool, dir: str):
        ...
        for internal_node in subgraph.get_op_nodes():
            if internal_node.has(name_of_connection):
                port_map.append(
                    {
                        "external_port_id": internal_node[name_of_connection],
