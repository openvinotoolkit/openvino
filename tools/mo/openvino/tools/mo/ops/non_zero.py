...
class NonZero(Op):
    ...
    def infer(node: Node):
        ...
        assert input_shape is not None, 'The input shape for node "{}" is None'.format(
            node_name
        )
        assert node.has_valid(
            "output_type"
        ), "`output_type` attribute is not set for NonZero node `{}`".format(node_name)
        assert node.output_type in [
            np.int64,
            np.int32,
        ], "NonZero `output_type` attribute must be int32 or int64, `{}` found".format(
            np.dtype(node.output_type).name
        )

        input_value = node.in_port(0).data.get_value()
        if is_fully_defined(input_value):
            node.out_port(0).data.set_value(
                mo_array(np.nonzero(input_value), dtype=node.output_type)
            )
        else:
            if is_fully_defined(input_shape):
                # output shape of NonZero is still static (upper bound)
                node.out_port(0).data.set_shape(
                    [len(input_shape), np.prod(input_shape)]
                )
            else:
                node.out_port(0).data.set_shape(
                    [len(input_shape), dynamic_dimension_value]
                )
