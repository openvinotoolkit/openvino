...
class CumSum(Op):
    ...
    def supported_attrs(self):
        return [
            ("exclusive", lambda node: bool_to_str(node, "exclusive")),
    ...
    def infer(node: Node):
        ...
        assert input_shape is not None, 'Input shape is None for node "{}"'.format(
            node_name
        )
        if not node.in_port(1).disconnected():
            assert (
                len(node.in_port(1).data.get_shape()) == 0
            ), "Axis is not scalar for node: {}".format(node_name)
        ...
        if input_value is not None:
            axis = (
                ...
                else node.in_port(1).data.get_value()
            )
            ...
            node.out_port(0).data.set_value(
                cumsum(input_value, axis=axis, reverse=reverse, exclusive=exclusive)
            )
