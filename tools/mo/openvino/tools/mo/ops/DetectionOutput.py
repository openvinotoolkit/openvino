...
class DetectionOutput(Op):
    ...
    def supported_attrs(self):
        supported_attrs = [
            ...
            (
                ...
                lambda node: bool_to_str(node, "variance_encoded_in_target"),
            ),
            "objectness_score",
        ]
    ...
    def infer(node: Node):
        ...
        if (
            is_fully_defined(prior_boxes_shape[-1])
        ...
        if (
            is_fully_defined(conf_shape[-1])
            and is_fully_defined(num_priors)
            and conf_shape[-1] % num_priors != 0
        ):
            raise Error(
