    ...
    def infer(node: Node):
        ...
        assert len(input_shape) == len(indices_shape), (
            ...
            "same rank. Instead got {} and {}".format(
                node_name, len(input_shape), len(indices_shape)
            )
        )
        ...
        is_opset12_reduction = opset == "opset12" and (
            node.soft_get("reduction") != "none" or not node.soft_get("use_init_val")
        )
        if (
            ...
            and not is_opset12_reduction
        ):
            assert (
                axis.size == 1
            ), "The node {} has axis input value size equal to {} but it should be exactly 1.".format(
                node_name, axis.size
            )
            axis = axis.item()
