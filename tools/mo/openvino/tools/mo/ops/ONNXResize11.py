...
class ONNXResize11Op(Op):
    ...
    def onnx_resize_infer(node: Node):
        ...
        assert node.is_in_port_connected(0) and (
            node.is_in_port_connected(2) or node.is_in_port_connected(3)
        ), (
        ...
        assert (
            node.coordinate_transformation_mode != "tf_crop_and_resize"
        ...
        if not node.is_in_port_connected(3):
            ...
            assert (
                input2_value is not None
            ), "Node {} with op {} has no value in input port 2".format(
            ...
            assert (
                sizes is not None
            ), "Node {} with op {} has no value in input port 3".format(
                node.soft_get("name", node.id), node.op
            )
