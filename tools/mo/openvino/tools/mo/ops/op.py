...
class Op(object):
    ...
    def __init__(self, graph: Graph, attrs1: dict = None, attrs2: dict = None):
        ...
        self.attrs = {"kind": "op", "rt_info": RTInfo()}
    ...
    def substitute_ie_attrs(self, new_attrs: dict):
        ...
        if self.ir_version not in backend_attrs_mapping.keys():
            raise Error(
                "Unrecognized IR version was specified: {}".format(self.ir_version)
            )
                ...
                "IE": [
                    (
                        ...
                        [
                            (
                                "data",
                                backend_attrs_mapping[self.ir_version]()
                                + self.default_backend_attrs,
                                [],
                            ),
                            "@runtime_info",
    ...
    def extract_port(node_port):
        ...
        if len(set(out_ids)) > 1 and not isinstance(node_port, tuple):
            raise Error(
                "Node {} has more than one outputs. Provide output port explicitly. ".format(
                    node.name
                )
            )
        return node, port

    def create_node_on_port(
        self, node: Node, out_port: int, attrs: dict = None, edge_attrs: dict = None
    ):
        """
    ...
    def create_node(
        self, inputs: list = None, attrs: dict = None, edge_attrs: dict = None
    ):
        ...
        for i, inp in enumerate(inputs):
            edge_attr = (
                ...
                if not inp[0].has_valid("kind") or inp[0].kind == "op"
    ...
    def create_node_with_data(
        ...
        if data_nodes is None:
            ...
            old_data_value = [
                data_node.value.copy() if data_node.has_valid("value") else None
                for data_node in data_nodes
            ]
            old_data_shape = [
                data_node.shape.copy() if data_node.has_valid("shape") else None
                for data_node in data_nodes
            ]
    ...
    def create_data_node(
        ...
        if edge_attrs is not None:
            graph.add_edges_from(
                [(op_node.id, data_node.id, {"out": out_port, **edge_attrs})]
            )
        else:
    ...
    def create_input_data_node(
        graph: Graph, name: str, value: np.array, attrs: dict = None
    ):
        if attrs is None:
            attrs = {}
        data_node = graph.unique_id(name)
    ...
    @staticmethod
    def create_and_connect_input_data_node(
        graph: Graph, op_node: Node, attrs: dict = None, edge_attrs: dict = None
    ):
        assert op_node is not None and op_node.kind == "op"
        if attrs is None:
            attrs = {}
    ...
    def normalize_outputs(node: Node):
        if (
            node.has_valid("out_ports_count")
            and len(node.out_edges()) < node.out_ports_count
        ):
            from openvino.tools.mo.ops.result import (  # Import is here to avoid circular import error
                Result,
            )
...
class PermuteAttrs:
    ...
    slice_permutation = lambda node, permutation, attr: node[
        attr
    ][  # doesn't depend from permutation variable
    ...
    common_attrs_permutation = {
        ...
        "order": lambda node, permutation, attr: permutation.inv[
            node[attr][permutation.perm]
        ],
    ...
    def __attr(name, port, func=None):
        if func is None:
            if name in PermuteAttrs.common_attrs_permutation:
                ...
                raise Error(
                    "Attr {} is missing in PermuteAttrs.common_attrs_permutation. Please update "
                    "common_attrs_permutation with permutation for your attribute!".format(
                        name
                    )
                )
        ...
        if len(port.split(":")) != 2 or port.split(":")[0] not in ["input", "output"]:
            raise Error(
                "Attribute port {} for {} wasn't set correctly!".format(port, name)
            )
    ...
    def update_attrs(self, attrs):
        for attr in attrs:
            if not isinstance(attr, tuple) or len(attr) not in [2, 3]:
                raise Error(
                    "attr object must be a tuple: (attribute_name, port) or (attribute_name, port, func)"
                )
    ...
    def permute_attrs(self, node):
        ...
        for attr in self.attrs.keys():
            ...
            port = int(port)
            node_with_permutation = (
                node.in_node(port) if node_type == "input" else node.out_node(port)
            )
    ...
    def set_permutation(node1, node2, permutation, override=False):
        ...
        if "permutation" not in edge_attrs or override:
            ...
            if (
                edge_attrs["permutation"] is None and permutation is not None
            ) or not np.array_equal(edge_attrs["permutation"], permutation):
                raise Error(
                    "Permutation already exists in edge between {} and {}".format(
                        node1.id, node2.id
                    )
                )
    ...
    def get_nhwc_to_nchw_permutation(dims_number: int):
        ...
        if dims_number != 3:
            perm = (
                [0, dims_number - 1, *[x for x in range(1, dims_number - 1)]]
                if dims_number > 1
                else [x for x in range(dims_number)]
            )
        else:
    ...
    def get_nchw_to_nhwc_permutation(dims_number: int):
        ...
        if dims_number != 3:
            perm = (
                [0, *[x for x in range(2, dims_number)], 1]
                if dims_number > 1
                else [x for x in range(dims_number)]
            )
        else:
