...
class Unique(Op):
    ...
    def infer(node: Node):
        ...
        assert node.has("sorted") and node.sorted in [
            "true",
            "false",
        ], "Unique does not have valid sorted attribute"
        assert node.has("return_inverse") and node.return_inverse in [
            "true",
            "false",
        ], "Unique does not have valid return_inverse attribute"
        assert node.has("return_counts") and node.return_counts in [
            "true",
            "false",
        ...
        assert (
            len(node.out_nodes()) <= max_num_outputs
        ...
        if (
            len(node.out_nodes()) == 2
            and 1 in node.out_nodes()
            and node.return_inverse == "true"
            and node.return_counts == "true"
        ):
            node.return_counts = "false"
        if (
            len(node.out_nodes()) == 2
            and 2 in node.out_nodes()
            and node.return_inverse == "true"
            and node.return_counts == "true"
        ):
            node.return_inverse = "false"
        ...
        assert (
            input_shape is not None and input_shape.size == 1
        ...
        if is_sorted:
            ...
            sorted_uniques, sorted_index, sorted_inverse, sorted_counts = np.unique(
                input_value, return_index=True, return_inverse=True, return_counts=True
            )
            ...
            for ind in np.sort(sorted_index):
                ...
                new_ind += 1
            inverse = [
                new_ind_by_elem[input_value[ind]] for ind in range(len(input_value))
            ]
        ...
        for out_node_ind in node.out_nodes():
            ...
            node.out_node(out_node_ind).shape = int64_array(
                node.out_node(out_node_ind).value.shape
            )
            j += 1
