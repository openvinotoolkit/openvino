...
class NormalizeOp(Op):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        ...
        if "across_spatial" in self.attrs and isinstance(
            self.attrs["across_spatial"], str
        ):
            self.attrs["across_spatial"] = int(self.attrs["across_spatial"])

        if "channel_shared" in self.attrs and isinstance(
            self.attrs["channel_shared"], str
        ):
    ...
    def supported_attrs(self):
        return [
            ...
            (
                "across_spatial",
                lambda node: (
                    bool(node.across_spatial)
                    if node.has_valid("across_spatial")
                    else None
                ),
            ),
            (
                "channel_shared",
                lambda node: (
                    bool(node.channel_shared)
                    if node.has_valid("channel_shared")
                    else None
                ),
            ),
        ]
