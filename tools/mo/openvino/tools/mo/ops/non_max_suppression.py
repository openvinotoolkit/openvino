...
class NonMaxSuppression(Op):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        mandatory_props = {
            ...
            "force_precision_in_ports": {2: "int64"},
    ...
    def infer(node: Node):
        ...
        input_msg_fmt = (
            "NonMaxSuppression node {} from {} must have from 2 to {} inputs"
        )
        ...
        ), "The shape of tensor with scores is not defined"
        assert len(boxes_shape) == 3, "Length of tensors with boxes must be equal to 3"
        assert (
            len(scores_shape) == 3
        ), "Length of tensors with scores must be equal to 3"
        ...
        if not max_output_boxes_per_class:
            log.info(
                'Set default "max_output_boxes_per_class" for node {} to number of boxes'.format(
                    node.name
                )
            )
        ...
        assert (
            scores_shape[2] is dynamic_dimension
            or scores_shape[2] == num_input_boxes
            or scores_shape[2] is None
            or num_input_boxes is None
        ), "Number of boxes mismatch for operation {}".format(node_name)

        if node.get_opset() in ["opset4", "opset5", "opset9"]:
            max_number_of_boxes = (
                min(num_input_boxes, max_output_boxes_per_class)
                * boxes_shape[0]
                * num_classes
            )
        else:
