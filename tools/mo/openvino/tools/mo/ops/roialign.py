...
class ROIAlign(Op):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        assert (
            "mode" in attrs
        ), "`mode` attribute is not set for ROIAlign during creation"
        assert (
            "pooled_h" in attrs
        ), "`pooled_h` attribute is not set for ROIAlign during creation"
        assert (
            "pooled_w" in attrs
        ), "`pooled_w` attribute is not set for ROIAlign during creation"
        assert (
            "sampling_ratio" in attrs
        ), "`sampling_ratio` attribute is not set for ROIAlign during creation"
        assert (
            "spatial_scale" in attrs
        ), "`spatial_scale` attribute is not set for ROIAlign during creation"
        super().__init__(
            graph,
            {
                "op": self.op,
    ...
    def infer(node):
        ...
        assert node.has_valid(
            "mode"
        ), '"mode" attribute is not set for node "{}"'.format(node_name)
        assert node.mode in [
            "avg",
            "max",
        ], '"mode" attribute range of values is ["avg", "max"], got {} for node "{}"'.format(
            node.mode, node_name
        )
        if node.get_opset() == "opset9":
            assert node.aligned_mode in [
                "asymmetric",
                "half_pixel_for_nn",
                "half_pixel",
        ...
        assert (
            ...
            and rois_shape is not None
            and indices_shape is not None
        ), 'The node "{}" input shape is None'.format(node_name)
        assert compatible_dims(rois_shape[0], indices_shape[0]), (
            "The number of batch indices does not correspond "
            'to number of ROIs for node "{}"'.format(node_name)
        )
        assert compatible_dims(
            rois_shape[1], 4
        ), 'The size of ROI element must be 4 for node "{}"'.format(node_name)
        assert (
            len(input_shape) == 4
        ), 'The rank of port 0 input tensor of node "{}" must be 4.'.format(node_name)
