...
class ExtractImagePatches(Op):
    ...
    def backend_attrs(self):
        return [
            (
                "sizes",
                lambda node: ",".join(map(str, node["sizes"][node.spatial_dims])),
            ),
            (
                "strides",
                lambda node: ",".join(map(str, node["strides"][node.spatial_dims])),
            ),
            (
                "rates",
                lambda node: ",".join(map(str, node["rates"][node.spatial_dims])),
            ),
            "auto_pad",
        ]
    ...
    def infer(node: Node):
        assert [
            port.idx for port in node.in_ports().values() if not port.disconnected()
        ] == [
            0
        ], "Wrong input nodes number for node {} with type ExtractImagePatches".format(
        ...
        ), "Input shape is not set for node {} with type ExtractImagePatches".format(
            name
        )

        assert (
            len(input_shape) == 4
        ), "ExtractImagePatches operation supports only 4D tensors"
        ...
        pad_spatial_shape, output_spatial_shape = tf_window_op_pad_infer(
            input_spatial_shape, size_extent, stride_spatial_shape, node.auto_pad, False
        )
