...
class ScatterNDBase(Op):
    ...
    def infer(node: Node):
        ...
        assert (
            len(input_shape) >= 1 and len(indices_shape) >= 1
        ...
        assert not is_fully_defined(indices_shape[-1]) or indices_shape[-1] <= len(
            input_shape
        ), 'The last dimension of indices shape must be at most a rank of input for the node "{}"'.format(
            node_name
        )
        ...
        # [1], [[1]], etc.)
        expected_updates_shape = np.ma.concatenate(
            (indices_shape[:-1], input_shape[indices_shape[-1] :]), axis=0
        )
        assert compatible_shapes(updates_shape, expected_updates_shape) or (
            strict_compare_tensors(expected_updates_shape, [])
            and strict_compare_tensors(
                updates_shape, np.ones(len(updates_shape), dtype=np.int64)
            )
        ), (
    ...
    def type_infer(node: Node):
        assert (
            node.in_port(0).get_source().get_data_type()
            == node.in_port(2).get_source().get_data_type()
        ), "The data type of the first and the third inputs must be equal for the node {}".format(
            node.name
        )
