...
def infer_for_opsetX(node: Node, opset: str):
    ...
    assert len([p for p in node.in_ports().values() if not p.disconnected()]) in [
        min_inputs_num,
        min_inputs_num + 1,
    ], "Interpolate node {} must have at least {} inputs".format(
        node.soft_get(node.name, node.id), axes_port
    )
    ...
    if len(node.in_ports()) == axes_port:
        ...
        assert (
            axes is not None
        ), "Interpolate node with name {} has None as 'axes' input".format(
    ...
    if node.shape_calculation_mode == "sizes":
        ...
        for i, axis in enumerate(axes):
            if (
                output_shape[axis] is not dynamic_dimension
    ...
    if node.is_in_port_connected(axes_port):
        PermuteInputs().set_input_permutation(
            node.in_node(axes_port), node, "input:0", "axis"
        )
...
class Interpolate(Op):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        ...
        if (
            self.attrs["version"] == "opset11"
            and self.attrs["shape_calculation_mode"] != "sizes"
        ):
