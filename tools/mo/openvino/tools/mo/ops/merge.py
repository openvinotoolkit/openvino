    ...
    def merge_infer(node: Node):
        ...
        if len(inferred_nodes) < len(node.in_nodes()):
            ...
            inferred_and_executable = [
                n
                for n in node.in_nodes().values()
                if n["is_partial_inferred"] and "executable" in n and n["executable"]
            ]
