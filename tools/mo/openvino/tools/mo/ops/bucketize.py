...
class Bucketize(Op):
    ...
    def infer(node: Node):
        ...
        assert (
            node.with_right_bound is not None
        ...
        if node.get_opset() != "extension":
            assert node.has_valid(
                "output_type"
            ), "`output_type` attribute is not set for Bucketize node `{}`".format(
                node_name
            )
            assert node.output_type in [
                np.int64,
                np.int32,
            ], "Bucketize `output_type` attribute must be int32 or int64, `{}` found".format(
        ...
        if input_value is not None and buckets_value is not None:
            node.out_port(0).data.set_value(
                mo_array(
                    np.digitize(
                        input_value, buckets_value, right=node.with_right_bound
                    ),
