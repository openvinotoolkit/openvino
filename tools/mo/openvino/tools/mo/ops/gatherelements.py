...
class GatherElements(Op):
    ...
    def infer(node: Node):
        ...
        assert data_rank == len(indices_shape), (
            "data and indices inputs for node {} must be of the "
            "same rank. Instead got {} and {}".format(
                node.name, data_rank, len(indices_shape)
            )
        )
        assert -data_rank <= axis < data_rank, (
            "axis for node {0} must be within interval "
            "[-{1},  {1} - 1]. Instead got: axis={2}".format(node.name, data_rank, axis)
        )
        if axis < 0:
            axis += data_rank
        ...
        for idx, (data_sz, ind_sz) in enumerate(zip(data_shape, indices_shape)):
            out_shape[idx] = (
                ind_sz if ind_sz is not dynamic_dimension or idx == axis else data_sz
            )
