...
class Tile(Op):
    ...
    def infer(node: Node):
        ...
        assert (
            tile_array is not None
        ), "Undefined `repeats` (1st port input value) of Tile node '{}'".format(name)
        ...
        if shape.size < tile_array.size:
            ...
            tile_array = shape_insert(
                tile_array, 0, [1] * (shape.size - tile_array.size)
            )

        input_value = node.in_port(0).data.get_value()
        if (
            input_value is not None
            and is_fully_defined(shape)
            and is_fully_defined(tile_array)
        ):
...
class AttributedTile(Op):
    ...
    def infer(node):
        ...
        assert (
            shape is not None
        ), "Undefined input shape for AttributedTile node '{}'.".format(name)
        axis = node.soft_get("axis", None)
        assert axis is not None
        tiles = node.soft_get("tiles", None)
        assert (
            tiles is not None
        ), "Undefined `tiles` attribute of Tile node '{}'".format(name)
