...
class CTCGreedyDecoderOp(Op):
    ...
    def infer(node: Node):
        ...
        assert len(logits_shape) == 3, "Incorrect rank of logits for {} node".format(
            node_name
        )
        assert (
            len(sequence_mask_shape) == 2
        ), "Incorrect rank of sequence length tensor for {} node".format(node_name)
        assert compatible_dims(
            logits_shape[1], sequence_mask_shape[1]
        ), "Batch dimensions of input tensors must be the same for {} node".format(
            node_name
        )
        assert compatible_dims(
            logits_shape[0], sequence_mask_shape[0]
