...
class Squeeze(Op):
    ...
    def infer(node: Node):
        ...
        assert (
            len(node.in_nodes()) == 2
        ), "The Squeeze node {} must have 2 inputs".format(node_name)
        ...
        for dim in squeeze_dims:
            if output_shape[dim] == 1 or output_shape[dim] is dynamic_dimension:
                real_squeeze_dims = np.ma.append(
                    real_squeeze_dims, get_canonical_axis_index(output_shape, dim)
                )
            else:
                raise Error(
                    'Trying to squeeze dimension not equal to 1 for node "{}"'.format(
                        node_name
                    )
                )
        ...
        if squeeze_dims.size == 0:
            for i in range(output_shape.size):
                if output_shape[i] == 1:
                    real_squeeze_dims = np.ma.append(
                        real_squeeze_dims, get_canonical_axis_index(output_shape, i)
                    )
    ...
    def reverse_infer(node: Node):
        ...
        if (
            ...
            and squeeze_dims is not None
        ):
            num_squeeze_dims = (
                1 if int64_array(squeeze_dims).ndim == 0 else len(squeeze_dims)
            )
