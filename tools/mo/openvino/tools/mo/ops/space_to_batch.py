...
class SpaceToBatch(Op):
    ...
    def infer(node):
        ...
        assert len(node.in_nodes()) == 4, (
            "Some inputs are not connected for the operation SpaceToBatch with name {}"
            "".format(node_name)
        )
        ...
        assert (
            block_size is not None and pads_begin is not None and pads_end is not None
        ), "Some inputs are not defined for SpaceToBatch operation with name {}".format(
            node_name
        )
