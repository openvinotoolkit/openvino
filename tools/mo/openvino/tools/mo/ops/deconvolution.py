...
class Deconvolution(Op):
    ...
    def backend_attrs(self):
        return [
            (
                "dilations",
                lambda node: ",".join(map(str, node["dilation"][node.spatial_dims])),
            ),
            (
                "strides",
            ...
            (
                "pads_begin",
                lambda node: (
                    ",".join(map(str, get_backend_pad(node.pad, node.spatial_dims, 0)))
            ...
            (
                "pads_end",
                lambda node: (
                    ",".join(map(str, get_backend_pad(node.pad, node.spatial_dims, 1)))
                    if node.has_valid("pad")
                    else None
                ),
            ),
    ...
    def infer(node: Node):
        ...
        if (
            ...
            or kernel_shape is None
            or node.spatial_dims is None
            or node.stride is None
        ):
            return
        ...
        PermuteInputs().set_input_permutation(
            node.in_node(1), node, "input:1", "transpose"
        )
