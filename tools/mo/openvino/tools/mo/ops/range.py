...
class Range(Op):
    ...
    def infer(node: Node):
        ...
        assert len(connected_input_ports) == 3 and [0, 1, 2] == sorted(
            connected_input_ports
        ), "Range operation should have 3 inputs, {} found for {}".format(
            len(connected_input_ports), name
        )
        ...
        if (
            not is_fully_defined(start)
            or not is_fully_defined(limit)
            or not is_fully_defined(delta)
        ):
