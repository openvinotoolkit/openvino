...
class ExperimentalDetectronDetectionOutput(Op):
    ...
    def backend_attrs(self):
        return [
            (
                ...
                lambda node: str(bool(node["class_agnostic_box_regression"])).lower(),
            ),
    ...
    def type_infer(node):
        ...
        node.out_port(1).set_data_type(
            np.int32
        )  # the second output contains class indices
        node.out_port(2).set_data_type(in_data_type)
        if node.is_out_port_connected(3):
            node.out_port(3).set_data_type(
                np.int32
            )  # the fourth output contains batch indices
