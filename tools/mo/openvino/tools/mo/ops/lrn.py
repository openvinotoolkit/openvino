...
class LRN(Op):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        assert (
            "alpha" in attrs
        ), "LRN operation should have `alpha` parameter set while creation"
        assert (
            "beta" in attrs
        ), "LRN operation should have `beta` parameter set while creation"
        assert (
            "bias" in attrs
        ), "LRN operation should have `bias` parameter set while creation"
        assert (
            "size" in attrs
        ), "LRN operation should have `size` parameter set while creation"
    ...
    def infer(node):
        ...
        assert (
            len(connected_inputs) == 2
            and 0 in connected_inputs
            and 1 in connected_inputs
        ), (
...
class AttributedLRN(Op):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        assert (
            "alpha" in attrs
        ), "AttributedLRN operation should have `alpha` parameter set while creation"
        assert (
            "beta" in attrs
        ), "AttributedLRN operation should have `beta` parameter set while creation"
        assert (
            "local_size" in attrs
        ), "AttributedLRN operation should have `local_size` parameter set while creation"
        ...
        assert (
            "region" in self.attrs
        ), "AttributedLRN operation should have `region` parameter set while creation"
