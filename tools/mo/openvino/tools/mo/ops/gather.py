...
class Gather(Op):
    ...
    def infer(node: Node):
        ...
        assert (
            len(connected_in_ports) == 3
            and 0 in connected_in_ports
            and 1 in connected_in_ports
            and 2 in connected_in_ports
        ), (
            "Gather should have 3 connected input port, but it doesn't for "
        ...
        assert (
            -len(data_shape) <= axis < len(data_shape)
        ), "axis must be within interval [-data_rank, data_rank). Instead got axis = {}, data_rank = {} ".format(
            axis, len(data_shape)
        )
        ...
        assert np.ma.allequal(
            data_shape[:batch_dims], indices_shape[:batch_dims]
        ), "data and indices inputs must have equal first dimensions until batch_dims"

        assert (
            batch_dims <= axis
        ), "normalized batch_dims must be <= axis. Instead got batch_dims = {}, axis = {}".format(
            axis, batch_dims
        )
        ...
        out_shape = np.concatenate(
            (data_shape[:axis], indices_shape[batch_dims:], data_shape[axis + 1 :])
        )
        ...
        if (
            ...
            and indices_value is not None
            and is_fully_defined(indices_value)
        ):
            ...
            if batch_dims == 0:
                ...
                for batch_idx in np.ndindex(tuple(batch_dims_range)):
                    out_value[batch_idx] = np.ma.take(
                        data_value[batch_idx],
...
class AttributedGather(Op):
    ...
    def infer(node: Node):
        ...
        if data_value is not None and indices_value is not None:
            node.out_port(0).data.set_value(
                mo_array(
                    np.take(data_value, indices_value, axis), dtype=data_value.dtype
                )
            )
            return
