...
from openvino.tools.mo.graph.graph import Error, Graph, Node
from openvino.tools.mo.ops.op import Op
from openvino.tools.mo.utils.broadcasting import (
...
class Select(Op):
    ...
    def infer(node: Node):
        ...
        assert (
            len([port for port in node.in_ports().values() if not port.disconnected()])
            == 3
        ), "Select operation must have 3 inputs: 'condition', 'then' and 'else' tensors for node {}".format(
            node_name
        )
        ...
        if broadcast_rule == "numpy":
            msg = (
                "In Select node '{}' condition and then/else shapes must be broadcastable. "
            ...
            if (
                node.has_valid("format")
                and node["format"] == "tf"
                and len(condition_shape) == 1
            ):
                # https://github.com/tensorflow/tensorflow/blob/master/tensorflow/python/ops/array_ops.py#L4596-L4598
                msg_tf = (
                    "In Select node '{}' if 'condition' is a 1D tensor then it's size "
                ...
                if (
                    is_fully_defined(condition_shape[0])
                    and not output_is_scalar
                    and is_fully_defined(output_shape[0])
                ):
                    assert condition_shape[0] == output_shape[0], msg_tf
                ones_shape = (
                    len(output_shape) if output_is_scalar else len(output_shape) - 1
                )
            ...
            assert compatible_shapes(a_shape, b_shape) and compatible_shapes(
                condition_shape, a_shape
            ), (
        ...
        if condition_value is not None:
            if is_fully_defined(condition_value) and np.all(
                ...
                output_value = np.ma.where(
                    condition_value, resulting_tensors[0], resulting_tensors[1]
                )
                node.out_port(0).data.set_value(output_value)
    ...
    def type_infer(node: Node):
        assert (
            ...
            == node.in_port(2).get_source().get_data_type()
        ), "The data type of the second and the third inputs must be equal for the node {}".format(
            node.name
        )
