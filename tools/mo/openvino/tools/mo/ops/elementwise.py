...
class Div(Elementwise):
    ...
    operation = staticmethod(
        lambda a, b: a // b if both_types_are_integer(a, b) else a / b
    )
...
class Round(UnaryElementwise):
    ...
    def __init__(self, graph: Graph, attrs):
        round_attrs = {"mode": "half_to_even", "infer": self.infer}
    ...
    def infer(cls, node: Node):
        ...
        if a is not None:
            assert node.soft_get("mode") in [
                "half_to_even",
                "half_away_from_zero",
            ], 'Round node {} has unsupported "mode" attribute value: {}'.format(
                node.soft_get("name", node.id), node.soft_get("mode")
            )
