...
class UpsampleOp(Op):
    ...
    def upsample_infer(node: Node):
        ...
        assert (
            len(layout) == 4
        ), 'Input tensor rank must be equal to 4 for node "{}"'.format(node_name)
        ...
        if len(node.in_nodes()) == 1:
            ...
            assert (
                node.has("width_scale") is not None
                and node.has("height_scale") is not None
            )
            ...
            assert (
                scales is not None
            ), 'The input with scales for node "{}" is not constant'.format(node_name)
