...
class Unsqueeze(Op):
    ...
    def infer(node):
        ...
        if unsqueeze_dims is None:
            raise Error(
                "The dimensions to unsqueeze are not defined for the node {}".format(
                    node.soft_get("name")
                )
            )
    ...
    def reverse_infer(node: Node):
        ...
        if (
            ...
            and unsqueeze_dims is not None
        ):
            num_unsqueeze_dims = (
                1 if int64_array(unsqueeze_dims).ndim == 0 else len(unsqueeze_dims)
            )
