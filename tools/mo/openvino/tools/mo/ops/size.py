...
class Size(Op):
    ...
    def infer(node: Node):
        ...
        ), "Size operation should have exact one input node, but it has {}".format(
        ...
        ), "Input shape is undefined for Size node `{}`".format(
        ...
        assert node.output_type in [
            np.int64,
            np.int32,
        ], "Size `output_type` attribute must be int32 or int64, `{}` found".format(
            np.dtype(node.output_type).name
        )

        if is_fully_defined(input_shape):
            node.out_port(0).data.set_value(
                mo_array(np.prod(input_shape), dtype=node.output_type)
            )
        else:
