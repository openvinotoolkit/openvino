...
class PoolingV2(Op):
    ...
    def infer(node: Node):
        assert len(node.in_nodes()) == 3, (
            "MaxPoolV2 node {} from must have only 3 inputs: input, window size, and "
            "strides but instead got {} inputs".format(
                node.soft_get("name", node.id), len(node.in_nodes())
            )
        )
...
class Pooling(Op):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        super().__init__(
            graph,
            {
                ...
                "out_ports_count": (
                    1
                    if attrs.get("version") == "opset1"
                    else poolings_map[attrs.get("pool_method")]["out_ports_count"]
                ),
            },
            attrs,
        )
    ...
    def backend_attrs(self):
        ...
        if self.attrs.get("pool_method") == "avg":
            ...
            return backend_attrs_list + [
                ...
                (
                    "index_element_type",
                    lambda node: np_data_type_to_destination_type(
    ...
    def infer(node: Node):
        assert len(node.in_nodes()) == 1, (
            "MaxPool node {} from must have only one input but instead got "
    ...
    def pool_infer(node: Node):
        ...
        if node.has_valid("auto_pad") and node.auto_pad != "explicit":
            ...
            rounding = np.floor
            if (
                node.soft_get("pooling_convention") == "full"
                or node.soft_get("rounding_type") == "ceil"
            ):
                rounding = np.ceil
            ...
            output_spatial_shape = shape_array(
                [dynamic_dimension_value for _ in range(len(padded_spatial_shape))]
            )
            for idx in range(len(padded_spatial_shape)):
                if (
                    padded_spatial_shape[idx] is not dynamic_dimension
                    and stride_spatial[idx] is not dynamic_dimension
                ):
                    output_spatial_shape[idx] = (
                        int(rounding(padded_spatial_shape[idx] / stride_spatial[idx]))
                        + 1
                    )
            ...
            for i in range(len(input_spatial_shape)):
                if (
                    original_pads[i]
                    and (output_spatial_shape[i] - 1) * stride_spatial[i]
