    ...
    def infer(node: Node):
        ...
        for batch_dim in range(batch_dims):
            assert compatible_dims(
                data_shape[batch_dim], indices_shape[batch_dim]
            ), "The dimension {} for data and indices tensors must be the same".format(
                batch_dim
            )
        ...
        assert (batch_dims + indices_shape[-1]) <= len(
            data_shape
        ), "Length of a tuple with indices must not exceed a rank of data tensor excluding batch dimensions"
        assert node["version"] in ["opset5", "opset8"], (
            "Unsupported version of GatherND operation: {}, operation "
        ...
        if batch_dims > 0:
            if (
                node["version"] == "opset5"
            ):  # Support old version of gatherND shape inference
                ...
                for dim in range(batch_dims):
                    assert compatible_dims(
                        indices_shape[dim], data_shape[dim]
        ...
        if is_fully_defined(indices_value) and data_value is not None:
            ...
            reshaped_indices = indices_value.reshape(
                batch_dims_size, -1, indices_shape[-1]
            )

            reshaped_data = data_value.reshape(
                (batch_dims_size,) + tuple((data_shape[batch_dims:]))
            )
            ...
            output_value = np.asarray(output_data, dtype=data_value.dtype).reshape(
                output_shape
            )
