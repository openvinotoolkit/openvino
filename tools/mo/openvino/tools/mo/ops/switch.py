...
class Switch(Op):
    ...
    def control_flow_infer(
        ...
        switch_data_0_port_node_id = (
            [out_data_nodes[0].id] if 0 in out_data_nodes else []
        )
        switch_data_1_port_node_id = (
            [out_data_nodes[1].id] if 1 in out_data_nodes else []
        )
        assert (
            1 <= len(switch_data_0_port_node_id) + len(switch_data_1_port_node_id) <= 2
        )

        if not node_with_switch_value.has_valid("value") or not is_fully_defined(
            node_with_switch_value.value
        ):
