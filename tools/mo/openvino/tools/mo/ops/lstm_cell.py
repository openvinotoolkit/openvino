    ...
    def backend_attrs(self):
        return [
            ...
            (
                "activations",
                lambda node: (
                    ",".join(node["activations"])
                    if node.has_and_set("activations")
                    else None
                ),
            ),
            (
                "activations_alpha",
                lambda node: (
                    ",".join(map(str, node["activations_alpha"]))
                    if node.has_and_set("activations_alpha")
                    else None
                ),
            ),
            (
                "activations_beta",
                lambda node: (
                    ",".join(map(str, node["activations_beta"]))
                    if node.has_and_set("activations_beta")
                    else None
                ),
            ),
            "clip",
    ...
    def infer(node: Node):
        ...
        if node.has_valid("hidden_size"):
            if node.hidden_size != hidden_size:
                raise Error(
                    "Input shape {} for hidden size doesn't match pre-defined hidden_size in node {}".format(
        ...
        assert compatible_dims(hidden_shape[0], cell_shape[0]) and compatible_dims(
            cell_shape[0], input_shape[0]
        ), "States are not broadcast-able by batch for node {}" "".format(
