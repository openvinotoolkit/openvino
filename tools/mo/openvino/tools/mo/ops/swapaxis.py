...
class SwapAxis(Op):
    ...
    def infer(node: Node):
        ...
        node.order[node.dim2], node.order[node.dim1] = (
            node.order[node.dim1],
            node.order[node.dim2],
        )
        ...
        if node.in_port(0).data.get_value() is not None:
            node.out_port(0).data.set_value(
                np.transpose(node.in_port(0).data.get_value(), axes=node.order)
            )
    ...
    def reverse_infer(node: Node):
        ...
        if node.in_port(0).data.get_shape() is None and output_shape is not None:
            ...
            input_shape[node.dim2], input_shape[node.dim1] = (
                input_shape[node.dim1],
                input_shape[node.dim2],
            )
