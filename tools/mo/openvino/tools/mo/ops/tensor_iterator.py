...
from openvino.tools.mo.graph.graph import Graph, Node, dict_includes
...
class TensorIterator(Op):
    ...
    def cover_body_input_data_nodes_with_parameter_ops(ti: Node):
        ...
        for operation_node, in_port in op_port_map:
            ...
            assert data_node.has_valid(
                "shape"
            ), "Data node should have `shape` attribute set, but it`s not for node {}".format(
                data_node.id
            )
            shape = data_node["shape"].copy()
            parameter_data_node = Parameter(
                body, {"shape": shape_array(shape)}
            ).create_node_with_data()

            body.create_edge(
                src_node=parameter_data_node,
    ...
    def cover_body_constant_data_nodes_with_const_ops(ti: Node):
        ...
        for data_node in body.get_data_nodes():
            if len(data_node.in_nodes()) == 0 and len(data_node.out_nodes()) != 0:
                ...
                assert data_node.has_valid(
                    "value"
                ), "Data node should have `value` attribute set, but it`s not for node {}".format(
                    data_node.id
                )
                shape = data_node["shape"].copy()
                ...
                body.create_edge(
                    src_node=const_node, dst_node=data_node, out_port=0, in_port=0
                )

    @staticmethod
    def special_port_to_real_port(
        node: Node, special_port_id: int, direction: str = "in"
    ):
        assert node.kind == "op"
    ...
    def set_internal_layer_id_for_nodes(ti: Node, nodes: list):
        max_internal_layer_id_used = max(
            [n.soft_get("internal_layer_id", 0) for n in ti.body.get_op_nodes()]
        )

        for node in nodes:
            if not node.has_valid("internal_layer_id"):
                node["internal_layer_id"] = max_internal_layer_id_used = (
                    max_internal_layer_id_used + 1
                )

    @staticmethod
    def update_back_edge_map(
        ti, direction, old_layer_id, old_port_id, new_layer_id, new_port_id=None
    ):
        ...
        for record in ti.back_edges:
            if record[layer_attr_name] != old_layer_id:
                continue
            if (
                port_attr_name in record and record[port_attr_name] == old_port_id
    ...
    def validate_maps(ti):
        def check_by_attribute(
            port_map, appropriate_attribute, inappropriate_attribute, node_type
        ):
            for record in port_map:
                ...
                assert inappropriate_attribute not in record, record[
                    inappropriate_attribute
                ]

        check_by_attribute(
            ti.input_port_map, "internal_layer_id", "internal_port_id", "Parameter"
        )
        check_by_attribute(
            ti.output_port_map, "internal_layer_id", "internal_port_id", "Result"
        )
    ...
    def normalize_internal_ids(ti):
        ...
        TensorIterator.set_internal_layer_id_for_nodes(
            ti, body.get_op_nodes(type="Parameter")
        )
        TensorIterator.set_internal_layer_id_for_nodes(
            ti, body.get_op_nodes(type="Result")
        )

        node_map = {
            copy(node.internal_layer_id): node
            for node in body.get_op_nodes()
            if node.has_valid("internal_layer_id")
        }
        ...
        for record in ti.input_port_map:
            ...
            in_port = TensorIterator.special_port_to_real_port(
                internal_node, copy(record["internal_port_id"])
            )
            assert (
                in_port in internal_node.in_ports()
                and not internal_node.in_port(in_port).disconnected()
            )
        ...
        for record in ti.output_port_map:
            ...
            out_port = TensorIterator.special_port_to_real_port(
                internal_node, copy(record["internal_port_id"]), "out"
            )
            assert (
                out_port in internal_node.out_ports()
                and not internal_node.out_port(out_port).disconnected()
            )
            ...
            for dst in internal_node.out_port(out_port).get_destinations():
                ...
                if possible_output_node.soft_get("type") == "Result":
                    assert (
                        internal_output_node is None
                    ), "Several Result operations on the same output port of {}".format(
                        internal_node
                    )
        ...
        for record in ti.back_edges:
            ...
            if internal_node.soft_get("type") != "Result":
                ...
                out_port = TensorIterator.special_port_to_real_port(
                    internal_node, record["from_port"], "out"
                )
                assert (
                    out_port in internal_node.out_ports()
                    and not internal_node.out_port(out_port).disconnected()
                ...
                for dst in internal_node.out_port(out_port).get_destinations():
                    possible_output_node = dst.node
                    if possible_output_node.soft_get("type") == "Result":
                        assert internal_output_node is None, (
                            "Several Result operations on the same output port of {}"
                ...
                "IE": [
                    (
                        ...
                        [
                            (
                                "data",
                                self.backend_attrs() + self.default_backend_attrs,
                                [],
                            ),
                            "@ports",
                            (
                                ...
                                [
                                    (
                                        "@list",
                                        lambda node: self.generate_port_map(
                                            node, node.input_port_map, "in"
                                        ),
                            ...
                            (
                                ...
                                [
                                    (
                                        "@list",
                                        lambda node: self.generate_back_edges(node),
                                        ("edge", back_edges_attrs, []),
                                    ),
                                ],
                            ),
                            ("body", [], [("@network", "body")]),
    ...
    def find_internal_layer_id(graph: Graph, virtual_id):
        internal_nodes = list(
            filter(
                lambda d: dict_includes(d[1], {"internal_layer_id": virtual_id}),
    ...
    def generate_port_map(node: Node, src_port_map, dir: str):
        ...
        for map_item in src_port_map:
            ...
            result["external_port_id"] = __class__.find_port_id(
                node, result["external_port_id"], "external_port_id"
            )
            result["internal_layer_id"] = __class__.find_internal_layer_id(
                node.body, result["internal_layer_id"]
            )
    ...
    def generate_back_edges(node: Node):
        ...
        for back_edge in node.back_edges:
            ...
            result["from_layer"] = __class__.find_internal_layer_id(
                node.body, result["from_layer"]
            )
            result["to_layer"] = __class__.find_internal_layer_id(
                node.body, result["to_layer"]
            )
            result_list.append(result)
        return result_list
    ...
    def infer(node: Node):
        return
        raise Error(
            "TensorIterator.infer is not implemented. "
    ...
    def ti_type_infer(node):
        ...
        for record in node.input_port_map:
            internal_node = get_internal_node_by_layer_id(
                node, record["internal_layer_id"]
            )
            assert (
                internal_node.soft_get("type") == "Parameter"
            ), internal_node.soft_get("type")

            real_external_port_idx = TensorIterator.special_port_to_real_port(
                node, record["external_port_id"]
            )
            external_data_type = (
                node.in_port(real_external_port_idx)
                .get_connection()
                .get_source()
                .get_data_type()
            )
        ...
        for data_node in ti_graph.get_data_nodes(has_value=True):
            if len(data_node.in_nodes()) == 0:
                const_node = Const(
                    ti_graph, {"name": "const_", "value": data_node.value}
                ).create_node()
        ...
        for record in node.output_port_map:
            ...
            assert internal_node.soft_get("type") == "Result", internal_node.soft_get(
                "type"
            )

            internal_data_type = internal_node.in_port(0).get_data_type()
            real_external_port_idx = TensorIterator.special_port_to_real_port(
                node, record["external_port_id"], "out"
            )
    ...
    def find_iterations_count_for_output(ti_node):
        ...
        for in_rec in ti_node.input_port_map:
            ...
            ), "external_port_id not set for input of {} node".format(ti_node.id)
            in_shape = ti_node.in_port(in_rec["external_port_id"]).data.get_shape()
            if (
                check_field(in_rec, "end")
                and in_rec["end"] >= 0
                and check_field(in_rec, "start")
                and in_rec["start"] >= 0
            ):
                ...
                in_rec_end = in_rec["end"]
                in_rec_start = (
                    in_shape[in_rec["axis"]]
                    if not check_field(in_rec, "start")
                    else in_shape[in_rec["axis"]] + 1 + in_rec["start"]
                )
            elif check_field(in_rec, "start") and in_rec["start"] >= 0:
                in_rec_end = (
                    in_shape[in_rec["axis"]]
                    if not check_field(in_rec, "end")
                    else in_shape[in_rec["axis"]] + 1 + in_rec["end"]
                )
                in_rec_start = in_rec["start"]
            elif (
                check_field(in_rec, "end")
            ...
            if (
                iterations_count is not dynamic_dimension_value
                and ceil((in_rec_end - in_rec_start) / in_rec_stride)
                != iterations_count
            ):
                raise Error(
                    "TensorIterator node {} have inputs with different iterations count".format(
                        ti_node.id
                    )
                )
...
def get_internal_node_by_layer_id(ti, internal_layer_id):
    ...
    assert (
        len(suitable_nodes) == 1
    ), "Expected 1 node with `internal_layer_id`={}, {} found".format(
        internal_layer_id, len(suitable_nodes)
    )
