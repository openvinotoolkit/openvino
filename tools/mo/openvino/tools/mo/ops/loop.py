...
class Loop(TensorIterator):
    ...
    def generate_port_map(node: Node, src_port_map, dir: str):
        ...
        for record in src_port_map:
            ...
            if record["external_port_id"] != -1:
                if (
                    dir == "out"
                ):  # increase the output port id by the number of input ports
                    # update the port id for proper generation of a "ports" section
                    record["external_port_id"] += len(node.in_ports())
            record["internal_layer_id"] = TensorIterator.find_internal_layer_id(
    ...
    def get_body_node_by_internal_id(loop_node: Node, internal_id: int):
        ...
        ), "Expected 0 or 1 node with `internal_layer_id`={}, {} found".format(
            internal_id, len(suitable_nodes)
        )
        return suitable_nodes[0] if len(suitable_nodes) == 1 else None

    @staticmethod
    def get_external_nodes_by_internal_id(
        loop_node: Node, internal_layer_id: int
    ) -> list:
        """
        ...
        for map_item in loop_node.input_port_map:
            if map_item[
                "internal_layer_id"
            ] == internal_layer_id and loop_node.is_in_port_connected(
                map_item["external_port_id"]
            ):
                return [
                    loop_node.in_port(map_item["external_port_id"]).get_source().node
                ]
        for map_item in loop_node.output_port_map:
            if map_item[
                "internal_layer_id"
            ] == internal_layer_id and loop_node.is_out_port_connected(
                map_item["external_port_id"]
            ):
                return [
                    dest.node
                    for dest in loop_node.out_port(
                        map_item["external_port_id"]
                    ).get_destinations()
                ]
        return []
    ...
    def updated_body_parameters_shape(loop_node: Node):
        ...
        for record in loop_node.input_port_map:
            ...
            if body_node is not None:
                ...
                if loop_port_idx != -1:
                    input_shape = (
                        loop_node.in_port(loop_port_idx)
                        .get_connection()
                        .get_source()
                        .data.get_shape()
                    )
    ...
    def updated_loop_output_ports_shape_and_value(loop_node: Node):
        ...
        for record in loop_node.output_port_map:
            ...
            if (
                loop_port_idx != -1
            ):  # the id = -1 for execution condition output which is not connected anywhere
                ...
                if concat_axis is not None:
                    assert output_shape[concat_axis] == 1, (
                        "Dimension for concatenation is not equal to 1 for scan "
                        'output for Loop node "{}" for loop output port "{}"'.format(
                            loop_name, loop_port_idx
                        )
                    )
    ...
    def update_body_parameters_type(loop_node: Node):
        ...
        for record in loop_node.input_port_map:
            ...
            if body_node is not None:
                ...
                log.debug(
                    'Updated data type for the body node with internal_id "{}" with value {}'
    ...
    @staticmethod
    def mark_current_iteration_parameter_node(
        loop_node: Node, body_parameter_node: Node
    ):
    ...
    @staticmethod
    def external_port_id_to_body_node(
        loop_node: Node, external_port_id: int, port_map: dict
    ):
        """
        ...
        for record in port_map:
            if record["external_port_id"] == external_port_id:
                result_nodes.extend(
                    body_graph.get_op_nodes(
                        internal_layer_id=record["internal_layer_id"]
                    )
                )
        assert len(result_nodes) == 1, (
            'There should be just one body node for external port "{}", but there "{}"'
            "".format(external_port_id, len(result_nodes))
        )
        return result_nodes[0]

    @staticmethod
    def connect_body_input(
        loop_node: Node,
        loop_input_port_idx: int,
        body_parameter: Node,
        axis: [int, None] = None,
        start: [int, None] = None,
        end: [int, None] = None,
        ...
        loop_node.input_port_map.append(
            {
                ...
                "start": start,
                "end": end,
                "external_port_id": loop_input_port_idx,
                "internal_layer_id": body_parameter["internal_layer_id"],
            }
        )

    @staticmethod
    def connect_body_output(
        loop_node: Node,
        loop_output_port_idx: int,
        internal_result: Node,
        axis: [int, None] = None,
        start: [int, None] = None,
        end: [int, None] = None,
        stride: [int, None] = None,
        ...
        loop_node.output_port_map.append(
            {
                ...
                "start": start,
                "end": end,
                "external_port_id": loop_output_port_idx,
    ...
    def add_back_edge(loop_node: Node, internal_parameter: Node, internal_result: Node):
        ...
        loop_node.back_edges.append(
            {
                "from_layer": internal_result["internal_layer_id"],
    ...
    def parameter_unchanged_after_iteration(loop_node: Node, body_parameter: Node):
        ...
        if not any(
            [
                attr["to_layer"] == body_parameter.soft_get("internal_layer_id")
                for attr in loop_node.back_edges
            ]
        ):
            return True

        for back_edge_attrs in loop_node.back_edges:
            if back_edge_attrs["to_layer"] == body_parameter.soft_get(
                "internal_layer_id"
            ):
                result_internal_id = back_edge_attrs["from_layer"]
                result_nodes = loop_node.body.get_op_nodes(
                    internal_layer_id=result_internal_id
                )
                assert len(result_nodes) == 1, (
                    "There should be exactly one node with id {}, but there are {}"
                ...
                if any(
                    [
                        node.soft_get("internal_layer_id")
                        == body_parameter.internal_layer_id
        ...
        for port_idx, in_port in reversed(loop_node.in_ports().items()):
            if (
                ...
                new_const_node = Const(
                    loop_node.body, original_const_node.attrs()
                ).create_node()
    ...
    @staticmethod
    def update_port_map_value(
        port_map: dict, attr: str, original_value: int, new_value: int
    ):
        matched = 0
        for record in port_map:
            if record[attr] == original_value:
                ...
                matched += 1
        assert matched == 1, (
            'More than one record in the portmap for attr "{}" with original value "{}"'
            "".format(attr, original_value)
        )

    @staticmethod
    def update_port_map_value_ext(
        port_map: dict,
        layer_id_attr: str,
        layer_id_value: int,
        updated_attr: str,
    ...
    def back_edge_exists(back_edges_map: dict, from_layer: int, to_layer: int):
        ...
        for back_edge in back_edges_map:
            if (
                back_edge["from_layer"] == from_layer
                and back_edge["to_layer"] == to_layer
            ):
                return True
        return False

    @staticmethod
    def inter_edge_exists(
        port_map: dict, external_port_id: int, internal_layer_id: int
    ):
        """
        ...
        for i_port in port_map:
            if (
                i_port["external_port_id"] == external_port_id
                and i_port["internal_layer_id"] == internal_layer_id
            ):
                return True
        return False
    ...
    def re_numerate_input_ports(loop_node: Node):
        ...
        def re_number_input_port(loop_node: Node, old_port_id: int, new_port_id: int):
            ...
            Loop.update_port_map_value(
                loop_node.input_port_map, "external_port_id", old_port_id, new_port_id
            )
    ...
    def re_numerate_output_ports(loop_node: Node):
        ...
        def re_number_output_port(loop_node: Node, old_port_id: int, new_port_id: int):
            ...
            Loop.update_port_map_value(
                loop_node.output_port_map, "external_port_id", old_port_id, new_port_id
            )
    ...
    @staticmethod
    def remove_unused_ops_from_port_map(
        loop_node: Node, port_map: dict, port_map_attr: str, dir: [None, str] = None
    ):
        """
        ...
        for record_id, record in enumerate(port_map):
            if len(
                loop_node.body.get_op_nodes(internal_layer_id=record[port_map_attr])
            ) == 0 or (
                dir == "out"
                and record.get("purpose", "") != "execution_condition"
                and record["external_port_id"] not in loop_node.out_ports()
            ):
    ...
    def normalize_input_output_ports(loop_node: Node):
        ...
        :return: None
        """
        Loop.remove_unused_ops_from_port_map(
            loop_node, loop_node.input_port_map, "internal_layer_id", "in"
        )
        Loop.remove_unused_ops_from_port_map(
            loop_node, loop_node.output_port_map, "internal_layer_id", "out"
        )
        Loop.remove_unused_ops_from_port_map(
            loop_node, loop_node.back_edges, "to_layer"
        )
