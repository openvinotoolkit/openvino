...
class Crop(Op):
        ...
        return [
            ...
            (
                ...
                lambda node: (
                    ...
                    else ",".join(map(str, node.offset))
                ),
            ),
            (
                "dim",
                lambda node: (
                    None if not node.has_valid("dim") else ",".join(map(str, node.dim))
                ),
            ),
            (
                "crop_begin",
                lambda node: (
                    None
                    if not node.has_valid("crop_begin")
                    else ",".join(map(str, node.crop_begin))
                ),
            ),
            (
                "crop_end",
                lambda node: (
                    None
                    if not node.has_valid("crop_end")
                    else ",".join(map(str, node.crop_end))
                ),
            ),
        ]
    ...
    def infer(node: Node):
        ...
        if input_count == 2:
            ...
            log.error(
                "Wrong number of input tensors ({}) in {}".format(
                    input_count, node.name
                )
            )
            return
    ...
    def _one_input_infer(node: Node):
        ...
        if not node.has_valid("axis"):
            raise Error(
                "axis attribute is missing for {} node. should be set in crop extractor".format(
                    node_name
                )
            )
        ...
        if node.has_valid("dim"):
            if len(node.dim) != len(node.axis):
                raise Error(
                    'Number of axis "{}" should match number of dim "{}" for node "{}"'
            ...
            if len(node.crop_begin) != len(node.axis) or len(node.crop_end) != len(
                node.axis
            ):
                raise Error(
                    'number of crop_begin({})/crop_end({}) should match number of axis "{}" for node "{}"'
                    "".format(node.crop_begin, node.crop_end, node.axis, node_name)
                )
            if type(node.axis) in [list, tuple]:
                for i in range(len(node.axis)):
                    output_shape[node.axis[i]] = (
                        output_shape[node.axis[i]]
                        - node.crop_begin[i]
                        - node.crop_end[i]
                    )
                ...
                output_shape[node.axis] = (
                    output_shape[node.axis] - node.crop_begin - node.crop_end
                )
        else:
            raise Error(
                "Crop node {} should have either dim or crop_begin and crop_end attributes".format(
                    node_name
                )
            )
    ...
    def _two_inputs_infer(node: Node):
        ...
        if any(s is None for s in shapes):
            raise Error(
                "Not all input shapes were defined for {} node".format(node_name)
            )
        ...
        if not node.has_valid("offset"):
            raise Error(
                "offset attribute is missing for {} node. should be set in crop extractor".format(
                    node_name
                )
            )
        ...
        for i in input_dim:
            ...
            if input_shape[i] - crop_offset < reference_shape[i]:
                raise Error(
                    "The crop for dimension is out of bounds in node {}".format(
                        node_name
                    )
                )
        ...
        if node.in_node(0).has_valid("value") and not getattr(
            node.graph.graph["cmd_params"], "enable_ssd_gluoncv", False
        ):
