...
class AttributedPower(Op):
    ...
    def infer(node: Node):
        ...
        assert node.has_valid(
            "scale"
        ), "AttributedPower operation should have `scale` parameter set, but it doesn`t for node {}".format(
            name
        )
        assert node.has_valid(
            "shift"
        ), "AttributedPower operation should have `shift` parameter set, but it doesn`t for node {}".format(
            name
        )
        assert node.has_valid(
            "power"
        ), "AttributedPower operation should have `power` parameter set, but it doesn`t for node {}".format(
            name
        )
