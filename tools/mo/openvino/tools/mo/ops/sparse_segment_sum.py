...
class SparseSegmentSum(Op):
    ...
    def infer(node: Node):
        ...
        assert (
            indices_shape is not None and indices_shape.size == 1
        ), "SparseSegmentSum supports only 1D indices tensor"
        assert (
            segment_ids_shape is not None and segment_ids_shape.size == 1
        ), "SparseSegmentSum supports only 1D segment IDs tensor"
        assert compatible_shapes(
            segment_ids_shape, indices_shape
        ...
        for i in range(1, len(segment_ids_value)):
            assert (
                segment_ids_value[i - 1] <= segment_ids_value[i]
        ...
        assert np.all(indices_value >= 0) and np.all(
            indices_value < data_shape[0]
        ), "Some value in indices tensor is out of range"

        # infer
        output_value = np.zeros(
            [num_segments] + data_shape[1:].tolist(), dtype=np.float32
        )
