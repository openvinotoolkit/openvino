...
class Shape(Op):
    ...
    def infer(node):
        ...
        assert (
            len(connected_in_ports) == 1
        ), "ShapeOf operation should have exact one input node, but it has {}".format(
        ...
        ), "Input shape is undefined for ShapeOf node `{}`".format(
        ...
        assert node.output_type in [
            np.int64,
            np.int32,
        ], "ShapeOf `output_type` attribute must be int32 or int64, `{}` found".format(
        ...
        if node.has_and_set("stop_value_propagation"):
            ...
            node.out_port(0).data.set_value(
                shape_array(mo_array(input_shape, dtype=node.output_type))
            )
