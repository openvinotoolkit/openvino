...
reduce_map = {
    "ReduceSum": np.sum,
    "ReduceProd": np.prod,
    "ReduceL1": lambda x, axis, keepdims: np.sum(
        a=np.absolute(x), axis=axis, keepdims=keepdims
    ),
    "ReduceL2": lambda x, axis, keepdims: np.sqrt(
        np.sum(a=np.square(x), axis=axis, keepdims=keepdims)
    ),
    "ReduceMax": np.max,
    "ReduceMin": np.min,
    "ReduceMean": np.mean,
...
def reduce_infer(node: Node):
    ...
    ), "Can not infer {} node `{}`: shape of 0-input unknown".format(node.op, node.id)
...
class ReduceOp(Op):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        ...
        assert isinstance(self.attrs["keep_dims"], int) or isinstance(
            self.attrs["keep_dims"], bool
        )
