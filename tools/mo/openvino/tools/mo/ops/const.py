...
class Const(Op):
    ...
    def supported_attrs(self):
        return [
            ...
            (
                "element_type",
                lambda node: (
                    precision_to_destination_type(node.force_type)
                    if node.has_valid("force_type")
                    else np_data_type_to_destination_type(node.value.dtype)
                ),
            ),
        ]
    ...
    def infer(node):
        ...
        output_value = (
            node.value
            if isinstance(node.value, np.ndarray) or len(node.shape) == 0
