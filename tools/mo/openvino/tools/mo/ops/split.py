...
class VariadicSplitBase(Op):
    ...
    def infer(node):
        ...
        assert op is not None and op in [
            "VariadicSplit",
            "AttributedVariadicSplit",
        ], "Unexpected `op`={} attribute for Split-like node {}".format(op, name)

        num_in_ports = (
            1
            if op == "AttributedVariadicSplit"
            else 3 if op == "VariadicSplit" else None
        )
        ...
        assert len(connected_inputs) == num_in_ports and all(
            [i in connected_inputs for i in range(num_in_ports)]
        ), (
        ...
        axis = (
            ...
            if op == "VariadicSplit"
            else node.soft_get("axis", None)
        )
        assert axis is not None, "{} `axis` is unknown for node {}".format(op, name)
        assert axis.ndim == 0 or (
            axis.ndim == 1 and axis.shape[0] == 1
        ), "{} `axis` should be scalar or tensor with shape [1], but it`s not for node {}".format(
            op, name
        )
        ...
        split_lengths = (
            ...
            if op == "VariadicSplit"
            else node.soft_get("split_lengths", None)
        )
        assert (
            split_lengths is not None
        ), "{} `split_lengths` is unknown for node {}".format(op, name)
        ...
        assert (
            undefined_elements.size != 0
            or input_elements is dynamic_dimension
            or input_elements == np.sum(split_lengths)
        ), (
            "The sum of split_lengths=`{}` must match data.shape[axis]="
            "`{}`. Node: {}".format(split_lengths, input_elements, name)
        )

        assert len(split_lengths) >= len(
            [port for i, port in node.out_ports().items() if not port.disconnected()]
        ), "Number of split_lengths=`{}` is less than connected output ports. Node: {}".format(
            split_lengths, name
        )
        ...
        for i in reversed(range(len(split_lengths))):
            if split_lengths[i] == 0:
                if node.out_port(i).disconnected():
                    ...
                    log.warning(
                        "Zero dimension on {} branch after Split node {}".format(
                            i, node.id
                        )
                    )
        ...
        for i, piece in enumerate(split_lengths):
            assert (
                piece >= -1
            ), "VariadicSplit split_lengths=`{}` should be non-negative".format(
                split_lengths
            )
            out_shape = input_shape.copy()

            split_length = (
                piece if piece > -1 else input_elements - (np.sum(split_lengths) + 1)
            )
        ...
        if input_value is not None:
            split = np.split(
                input_value, mo_array(idxs[:-1], dtype=split_lengths.dtype), axis
            )
    ...
    def reverse_infer(node: Node):
        ...
        axis = (
            node.in_port(1).data.get_value()
            if node.op == "VariadicSplit"
            else node.soft_get("axis", None)
        )
        assert axis is not None, "{} `axis` is unknown for node {}".format(
            node.op, node.soft_get("name", node.id)
        )
        split_lengths = (
            node.in_port(2).data.get_value()
            if node.op == "VariadicSplit"
        ...
        assert (
            split_lengths is not None
        ), "{} `split_lengths` is unknown for node {}".format(
            node.op, node.soft_get("name", node.id)
        )
...
class VariadicSplit(VariadicSplitBase):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        ...
        assert (
            "out_ports_count" in attrs
        ), "Please set `out_ports_count` attribute for VariadicSplit while creating"
...
class AttributedVariadicSplit(VariadicSplitBase):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        assert (
            "axis" in attrs
        ), "AttributedVariadicSplit operation should have `axis` parameter set while creation"
        assert (
            "size_splits" in attrs
...
class SplitBase(Op):
    ...
    def infer(node):
        ...
        assert (
            axis.ndim == 0
        ), "{} `axis` should be scalar, but it`s not for node {}".format(op, name)

        assert (
            not is_fully_defined(input_shape[axis])
            or input_shape[axis] % num_splits == 0
        ), (
        ...
        input_value = node.in_port(0).data.get_value()
        output_value = (
            np.split(input_value.copy(), axis=axis, indices_or_sections=num_splits)
            if input_value is not None
            else None
        )
...
class Split(SplitBase):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        assert (
            "num_splits" in attrs
        ), "Split operation should have `num_splits` while creation"
        if "out_ports_count" not in attrs:
...
class AttributedSplit(SplitBase):
    ...
    def __init__(self, graph: Graph, attrs: dict):
        assert (
            "num_splits" in attrs
        ), "AttributedSplit operation should have `num_splits` while creation"
        if "out_ports_count" not in attrs:
            attrs["out_ports_count"] = attrs["num_splits"]
        ...
        assert (
            "axis" in self.attrs
        ), "AttributedSplit operation should have `axis` parameter set while creation"
...
def split_reverse_infer(node: Node, num_splits: int, axis: int):
    ...
    for i in range(num_splits):
        shape = (
            node.out_port(i).data.get_shape()
            if not node.out_port(i).disconnected()
            else None
        )
