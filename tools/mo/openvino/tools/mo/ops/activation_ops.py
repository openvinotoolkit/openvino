...
class Floor(Activation):
    ...
    operation = staticmethod(
        lambda x: x if np.issubdtype(x.dtype, np.integer) else np.ma.floor(x)
    )
...
class HSwish(Activation):
    ...
    version = "opset4"
    operation = staticmethod(
        lambda x: x * np.ma.minimum(np.ma.maximum(x + 3.0, 0.0), 6.0) / 6.0
    )
...
class HSigmoid(Activation):
    ...
    version = "opset5"
    operation = staticmethod(
        lambda x: np.ma.minimum(np.ma.maximum(x + 3.0, 0.0), 6.0) / 6.0
    )
    ...
    def infer(node: Node):
        ...
        if node.is_in_port_connected(1):
            ...
            if beta is not None:
                assert (
                    beta.ndim == 0
                ), 'The "beta" value for node {} must be a scalar'.format(node_name)
                beta = beta.item()

        input_value = node.in_port(0).data.get_value()
        if input_value is not None and beta is not None:
            node.out_port(0).data.set_value(
                input_value / (1.0 + np.exp(-input_value * beta))
            )
