    ...
    def interp_infer(node: Node):
        ...
        if len(node.in_nodes()) == 2:
            ...
            if node.shrink_factor != 1 and node.zoom_factor == 1:
                ...
                if shrink_factor < 1:
                    log.error(
                        "Shrink factor should be positive in node {}".format(node.id)
                    )
                    return None
                ...
                node["debug_message"] = (
                    "Interp layer shape inference function may be wrong, please, try to update "
                    "layer shape inference function in the file (openvino/tools/mo/ops/interp.op at the "
                    "line {}).".format(inspect.currentframe().f_lineno)
