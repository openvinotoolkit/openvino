...
class Cast(Op):
    ...
    def helper_value_propagation(node_name, value, dst_type):
        ...
        if finite_match_count:
            log.error(
                ...
                " ".format(finite_match_count, new_blob.size, node_name, dst_type)
                + refer_to_faq_msg(76)
            )
        if zero_match_count:
            log.warning(
                "{} elements of {} were clipped to zero while converting an input blob for node '{}' to {}."
                " ".format(zero_match_count, new_blob.size, node_name, dst_type)
                + refer_to_faq_msg(77)
            )
        return new_blob
    ...
    def custom_type_casting_and_packing(node: Node, value, dst_type):
        ...
        packed = np.packbits(
            bit_order_big_flattened, bitorder="little" if num_bits > 1 else "big"
        )
    ...
    def infer(node: Node):
        ...
        assert dst_type is not None, (
            'Destination type of "Cast" operation should be extracted earlier, but it`s not for node: '
            + node_name
        )
