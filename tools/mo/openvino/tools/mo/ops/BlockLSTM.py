...
class BlockLSTM(Op):
    ...
    def infer(node: Node):
        ...
        connected_in_ports = [
            port_idx
            for port_idx, port in node.in_ports().items()
        ...
        assert len(connected_in_ports) >= 5, (
            "Internal Model Optimizer Error or unsupported BlockLSTM node {}. "
            "MO expects five inputs for BlockLSTM".format(node_name)
        )
        assert len(connected_out_ports) <= 2, (
            "Internal Model Optimizer Error or unsupported BlockLSTM node {}. "
