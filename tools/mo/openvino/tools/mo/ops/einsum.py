...
class Einsum(Op):
    ...
    @staticmethod
    def is_label_elsewhere(
        input_subscripts: list, label_to_check: str, excluded_subscript_inds: list
    ) -> bool:
        """
    ...
    def parse_equation(node_name: str, equation: str) -> (list, str):
        ...
        assert (
            len(splitted_equation) <= 2
        ), "Einsum node {} has `equation` of incorrect format".format(node_name)
        ...
        for input_subscript in input_subscripts_list:
            assert (
                re.match(subscript_pattern, input_subscript) is not None
            ), "Einsum node {} has `equation` with incorrect input subscript: {}".format(
                node_name, input_subscript
            )
            is_ellipsis_met = is_ellipsis_met or re.search(
                ellipsis_pattern, input_subscript
            )

        if len(splitted_equation) == 2:
            output_subscript = splitted_equation[1]
            assert re.match(
                subscript_pattern, output_subscript
            ), "Einsum node {} has `equation` with incorrect output subscript: {}".format(
                node_name, output_subscript
            )
            # if ellipsis is met, the output subscript must contain it as well
            if is_ellipsis_met:
                assert re.search(
                    ellipsis_pattern, output_subscript
                ), "The output subscript of Einsum node {} must contain ellipsis".format(
                    node_name
                )
            ...
            for ind, input_subscript in enumerate(input_subscripts_list):
                ...
                for label in labels:
                    if (
                        Einsum.is_label_elsewhere(input_subscripts_list, label, [ind])
                        is False
                    ):
            ...
            assert False, (
                "Einsum node {} equation has incorrect format. "
    ...
    def normalize_equation(node_name: str, equation: str) -> str:
        ...
        """
        input_subscripts_list, output_subscript = Einsum.parse_equation(
            node_name, equation
        )
    ...
    def extract_subscript_labels(node_name: str, subscript: str) -> list:
        ...
        while label_ind < len_subscript:
            if subscript[label_ind].isalpha():
                ...
                len_subscript - label_ind > 2
                and subscript[label_ind : label_ind + 3] == "..."
            ):
                labels.append("...")
                label_ind += 3
            else:
                assert (
                    False
                ), "Einsum node {} has `equation` with incorrect subscript: {}".format(
                    node_name, subscript
                )
        return labels

    @staticmethod
    def adjust_equation_with_NCHW_layout(
        node_name: str,
        equation: str,
        input_ranks: list,
        output_rank: int,
        input_correct_layout_mask: list,
        output_correct_layout_mask: bool,
    ) -> (str, list, bool):
        """
        ...
        assert len(input_subscripts) == num_inputs, (
            "The number of inputs must match a number " "of input subscripts"
        )
        assert len(input_correct_layout_mask) == num_inputs, (
        ...
        for input_ind in range(num_inputs):
            ...
            elif input_rank > 3 and (
                labels[-1] != "..." or labels[-1] == "..." and num_broadcasted_dims == 1
            ):
                is_inputs_adjusted.append(True)
    ...
    def infer(node: Node):
        ...
        assert len(input_subscripts) == num_inputs, (
            "The number of input operands of Einsum node {} "
                ...
                if label == "...":
                    ...
                    if label in label_to_shape.keys():
                        ...
                        assert common_shape is not None, (
                            "The dimensions labeled of ellipsis must be broadcastable "
                    ...
                    assert label not in label_to_shape.keys() or np.array_equal(
                        label_to_shape[label], sub_shape
                    ), (
        ...
        labels = Einsum.extract_subscript_labels(node_name, output_subscript)
        for label in labels:
            assert label in label_to_shape.keys(), (
                "The label in the output subscript must appear"
