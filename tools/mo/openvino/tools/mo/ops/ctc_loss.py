...
class CTCLoss(Op):
    ...
    def backend_attrs(self):
        return [
            (
                ...
                lambda node: bool_to_str(node, "preprocess_collapse_repeated"),
            ),
            (
                "ctc_merge_repeated",
                lambda node: bool_to_str(node, "ctc_merge_repeated"),
            ),
    ...
    def type_infer(node):
        ...
        assert logit_length_type == label_length_type and logit_length_type in [
            np.int64,
            np.int32,
        ...
        assert labels_type == blank_index_type and labels_type in [
            np.int64,
            np.int32,
    ...
    def infer(node: Node):
        ...
        assert len(connected_in_ports) in [
            4,
            5,
        ...
        assert (
            len(logits_shape) == 3
            and len(logit_length_shape) == 1
            and len(labels_shape) == 2
            and len(label_length_shape) == 1
            and len(blank_index_shape) == 0
        ), "Incorrect rank of some input tensor for {} node".format(node_name)
        assert (
            compatible_dims(logits_shape[0], logit_length_shape[0])
            and compatible_dims(logits_shape[0], labels_shape[0])
            and compatible_dims(logits_shape[0], label_length_shape[0])
        ), "Batch dimensions of input tensors must be the same for {} node".format(
            node_name
        )
        assert compatible_dims(
            logits_shape[1], labels_shape[1]
        ), "Time dimensions of input tensors must be the same for {} node".format(
            node_name
        )
