...
class MatMul(Op):
    ...
    def shape_alignment(node: Node):
        ...
        for i, shape in enumerate(input_shapes):
            ...
            ), "MatMul has shape=`None` for {} input of `{}` node".format(i, node_name)
            assert input_shape.ndim == 1, (
                "MatMul doesn't support scalar inputs. {} input of `{}` node has shape {}"
                "".format(i, node_name, input_shape)
            )
            assert input_shape.size >= 1, (
                "MatMul doesn't support inputs with rank lower than 1. {} input of `{}` "
            ...
            input_shape = shape_insert(
                input_shape, 0, [1] * (max_shape_length - input_shape.size)
            )
        ...
        ), "Shapes were not aligned by length for MatMul `{}`. Shapes: `{}`".format(
            node_name, transformed_shapes
        )
    ...
    def infer(node: Node):
        ...
        assert (
            ...
            and 0 in connected_in_ports
            and 1 in connected_in_ports
        ), (
        ...
        assign_dims_to_weights(
            node.in_node(1), None, in_ch, out_ch, node.in_port(1).data.get_shape().size
        )
...
def transpose(value):
    ...
    if num_of_dims == 1:
        ...
        return np.transpose(
            value, [*range(0, num_of_dims - 2), num_of_dims - 1, num_of_dims - 2]
        )
...
class FullyConnected(Op):
    ...
    def infer(node: Node):
        ...
        assert (
            input_shape is not None and weights_shape is not None
        ), "Incorrect FullyConnected input shapes. Node: {}. Shapes: {}".format(
            name, [input_shape, weights_shape]
        )
        assert weights_shape.size == 2
        out_size = node.soft_get("out-size")
        assert compatible_dims(
            weights_shape[0], out_size
        ...
        if 2 in connected_in_ports:
            ...
            assert (
                bias_shape is not None
            ), "Shape was not inferred for biases of FullyConnected {}".format(name)
            assert (
                bias_value is not None
            ), "Value was not inferred for biases of FullyConnected {}".format(name)
            assert compatible_shapes(bias_shape, [out_size]) or compatible_shapes(
                bias_shape, [1, out_size]
            ), "Incorrect FullyConnected bias shape `{}` for node {}. `out-size`={}".format(
                bias_shape, node, out_size
            )
