...
class Convolution(Op):
    ...
    def backend_attrs(self):
        def pad_attribute_helper(node: Node, pad_type: str = "begin"):
            ...
            if not node.has_valid("pad"):
                return None
            pad = get_backend_pad(
                node.pad, node.spatial_dims, 0 if pad_type == "begin" else 1
            )
        ...
        return [
            (
                "auto_pad",
                lambda node: (
                    node.auto_pad if node.has_valid("auto_pad") else "explicit"
                ),
            ),
            (
                "strides",
            ...
            (
                "output_padding",
                lambda node: (
                    ",".join(map(str, node.output_padding[node.spatial_dims]))
                    if node.has_valid("output_padding")
                    and node.type
                    in ("GroupConvolutionBackpropData", "ConvolutionBackpropData")
                    else None
                ),
            ),
    ...
    def calc_convolution(
        input_spatial_shape, stride_spatial_shape, pad_spatial_shape, kernel_extent
    ):
        """
    ...
    def calc_deconvolution(node, input_spatial_shape, pad_spatial_shape, kernel_extent):
        ...
        """
        return (
            node.stride[node.spatial_dims] * (input_spatial_shape - 1)
            + kernel_extent
            - pad_spatial_shape
        )
    ...
    def infer(node: Node):
        ...
        if input_shape is None:
            raise Error(
                "Input data shape is None for node {}".format(
        ...
        if node.has_valid("reshape_kernel") and node.reshape_kernel:
            if not (
                node.has_valid("output")
                and node.has_valid("channel_dims")
                and node.has_valid("group")
                and node.has_valid("kernel_spatial")
            ):
                log.error(
                    "Cannot reshape kernel due to not all required attrs was set to {} node".format(
                        node.id
                    )
                )
                return
            ...
            if is_fully_defined(kernel_shape) and np.prod(kernel_shape) != np.prod(
                node.in_node(weights_index).value.shape
            ):
                log.error(
                    "Size of weights {} does not match kernel shape: {}\n"
                    "".format(
                        np.prod(node.in_node(weights_index).value.shape), kernel_shape
                    )
            ...
            if not is_fully_defined(kernel_shape):
                num_undefined = np.count_nonzero(
                    kernel_shape.mask is True
                )  # pylint: disable=no-member
                if num_undefined > 1:
                    raise Error(
                        "Too many undefined dimensions of the kernel shape for node {}. Use --input_shape "
                        "command line parameter to specify model input shapes".format(
                            node.soft_get("name", node.id)
                        )
                    )
                ...
                # dimensions
                kernel_shape[np.where(kernel_shape == np.ma.masked)[0][0]] = (
        ...
        if (
            node.has_valid("auto_pad")
            and node.auto_pad != "explicit"
            and not node.has_valid("output_spatial_shape")
        ):
            node["pad_spatial_shape"], node["output_spatial_shape"] = (
                tf_window_op_pad_infer(
                    input_spatial_shape,
                    kernel_extent,
            ...
            if node.type in ("Convolution", "BinaryConvolution"):
                ...
                if node.has_valid("output_spatial_shape"):
                    if node.has_valid("get_pad"):
                        ...
                        log.debug(
                            "Can't calculate paddings due to missing lambda get_pad in {} node".format(
                                node.id
                            )
                        )
                        return
                    ...
                    output_padding = (
                        node.output_padding[node.spatial_dims]
                        if node.has_valid("output_padding")
                        else None
                    )
                    ...
                    float_spatial = Convolution.calc_deconvolution(
                        node, input_spatial_shape, pad_spatial_shape, kernel_extent
                    )
                    node["output_spatial_shape"] = shape_array(float_spatial)
        ...
        output_shape[node.batch_dims] = input_shape[
            node.batch_dims
        ]  # pylint: disable=unsupported-assignment-operation
        output_shape[node.spatial_dims] = (
            node.output_spatial_shape
        )  # pylint: disable=unsupported-assignment-operation
        ...
        output_shape[node.channel_dims] = (
            node.output
        )  # pylint: disable=unsupported-assignment-operation
    ...
    def type_infer(node):
        ...
        # input at index 0.
        if (
            in_type_1 in [np.float16, np.float32, np.float64]
            and in_type_0 != in_type_1
            and in_node_1.op == "Const"
        ):
            in_node_1 = node.in_port(1).get_source().node
            log.error(
                "Changing Const node '{}' data type from {} to {} for Convolution operation".format(
