...
class EmbeddingBagBase(Op):
    ...
    def infer(node: Node):
        raise NotImplementedError(
            "Please use specialized EmbeddingBag operation class, EmbeddingBagBase is base class"
        )
...
class EmbeddingBagOffsetsSum(EmbeddingBagBase):
    ...
    def infer(node: Node):
        ...
        assert len(connected_in_ports) >= 3 and all(
            p in connected_in_ports for p in [0, 1, 2]
        ), (
        ...
        assert (
            len(weights_shape) >= 2
        ), "EmbeddingBagOffsetsSum should have at least 2D weights for node: `{}`".format(
            name
        )
        offsets_shape = node.in_port(2).data.get_shape()
        assert (
            offsets_shape is not None and len(offsets_shape) == 1
        ), "Rank of the offsets in EmbeddingBagOffsetsSum should be equal to 1 for node: `{}`".format(
            name
        )

        node.out_port(0).data.set_shape(
            np.ma.concatenate((offsets_shape[:1], weights_shape[1:]))
        )
...
class EmbeddingBagPackedSum(EmbeddingBagBase):
    ...
    def infer(node: Node):
        ...
        assert len(connected_in_ports) >= 2 and all(
            p in connected_in_ports for p in [0, 1]
        ), (
        ...
        assert (
            len(weights_shape) >= 2
        ), "EmbeddingBagPackedSum should have at least 2D weights for node: `{}`".format(
            name
        )
        input_shape = node.in_port(1).data.get_shape()

        node.out_port(0).data.set_shape(
            np.ma.concatenate((input_shape[:1], weights_shape[1:]))
        )
...
class EmbeddingSegmentsSum(EmbeddingBagBase):
    ...
    def infer(node: Node):
        ...
        assert len(connected_in_ports) >= 4 and all(
            p in connected_in_ports for p in [0, 1, 2, 3]
        ), (
        ...
        assert (
            ...
            and indices_shape == segment_ids
        ), "Both indices and segment_ids should have the same shape for node: `{}`".format(
            name
        )
        num_segments = node.in_port(3).data.get_value()
        assert num_segments is not None, (
            "{} should have a constant num_segments provided, but it "
