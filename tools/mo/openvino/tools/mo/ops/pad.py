...
class Pad(Op):
    ...
    def infer(node):
        ...
        assert len(node.in_nodes()) in [
            3,
            4,
        ], "The node {} must have 3 or 4 inputs".format(pad_node_name)
        ...
        assert (
            pad_beg is not None
        ), "The padding begin value is None for node {}".format(pad_node_name)
        assert pad_end is not None, "The padding end value is None for node {}".format(
            pad_node_name
        )
        assert input_shape is not None, "The input shape is None for node {}".format(
            pad_node_name
        )
        ...
        assert (
            not node.is_in_port_connected(3)
            or node.in_port(3).data.get_shape().size == 0
        ), (
        ...
        if (
            ...
            and is_fully_defined(pad_beg)
            and is_fully_defined(pad_end)
        ):
            ...
            if len(node.in_nodes()) == 4:
                pad_val = (
                    ...
                    if node.in_port(3).data is not None
                    else 0
                )
            if is_fully_defined(input_value):
                node.out_port(0).data.set_value(
                    np.pad(input_value, pads, constant_values=pad_val, mode="constant")
                )
            else:
                node.out_port(0).data.set_value(
                    shape_array(
                        np.pad(
                            input_value, pads, constant_values=pad_val, mode="constant"
                        )
                    )
                )
    ...
    def reverse_infer(node: Node):
        ...
        if (
            input_shape is None
            and node.is_in_port_connected(2)
            and node.in_port(2).data.get_shape() is not None
        ):
