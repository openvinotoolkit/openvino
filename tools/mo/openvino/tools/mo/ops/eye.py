...
class Eye(Op):
    ...
    def infer(node: Node):
        ...
        assert (
            len(num_columns_shape) <= 1
        ), '"num_columns" should be 1D tensor or scalar. Got: '.format(
            len(num_columns_shape)
        )
        ...
        assert (
            len(diagonal_index_shape) <= 1
        ), '"diagonal_index" should be 1D tensor or scalar. Got: '.format(
            len(diagonal_index_shape)
        )
        ...
        if is_fully_defined(output_shape) and diagonal_index is not None:
            ...
            one_matrix = np.eye(
                num_rows,
                M=num_columns,
                k=np.array(diagonal_index).item(),
