...
class Broadcast(Op):
    ...
    def infer(node: Node):
        ...
        if target_shape is None:
            ...
            assert len(target_shape_shape) == 1
            assert target_shape_shape.item(0) > 0
            new_shape = [-1] * target_shape_shape.item(0)
            node.out_port(0).data.set_shape(shape_array(new_shape))
            return
