...
from openvino.tools.mo.utils.broadcasting import (
    ...
    explicit_shape_broadcasting,
    uni_directional_broadcasting,
    uni_directional_shape_broadcasting,
)
...
class Broadcast(Op):
    ...
    def infer(node: Node):
        ...
        assert node.has_and_set(
            "mode"
        ), 'Broadcasting mode is not defined for node "{}"'.format(node_name)
        ...
        if target_shape is None:
            assert (
                len(target_shape_shape) == 1
            ), 'Shape of target_shape must be [1] for node "{}"'.format(node_name)
            assert is_fully_defined(
                target_shape_shape
            ), 'Output shape is not defined for node "{}"'.format(node_name)
            ...
            if node.mode == "explicit":
                assert node.is_in_port_connected(
                    2
                ), 'Axes mapping must be specified for Broadcast(mode="explicit"). Node: `{}`'.format(
                    node_name
                )
                PermuteInputs().set_input_permutation(
                    node.in_node(2), node, "output:0", "axis"
                )
            return

        if (
            input_value is not None
            and not node.has_and_set("stop_value_propagation")
            and is_fully_defined(target_shape)
        ):
            if node.mode == "numpy":
                ...
                assert axes_mapping is not None, (
                    'Broadcast(mode="explicit") with dynamic axes_mapping input '
                ...
                axes_mapping = node.in_port(2).data.get_value()
                node.out_port(0).data.set_value(
                    explicit_broadcasting(input_value, target_shape, axes_mapping)
                )
            else:
                raise Error(
                    'The node "{}" has unsupported mode "{}"'.format(
                        node_name, node.mode
                    )
                )
        else:
