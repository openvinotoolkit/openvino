...
class Reshape(Op):
    ...
    def infer(node: Node):
        ...
        assert len(connected_inputs) == 2 and all(
            [i in connected_inputs for i in range(2)]
        ), (
        ...
        ), "Dynamic Reshape second input is not supported. Node {}".format(name)
        ...
        if (
            num_of_output_elements is not dynamic_dimension
            and all_dynamic_dimension_are_copied
            and is_fully_defined(new_shape)
        ):
            ...
            if dynamic_indices.size == 1:
                for index, x in enumerate(new_shape):
                    if x == 0 and node.has_and_set("special_zero"):
                        ...
                        num_of_output_elements *= x
            assert (
                num_of_input_elements % num_of_output_elements == 0
            ), "Incorrect number of output elements deduced for node {}: ".format(name)
            output_shape[dynamic_indices[0][0]] = (
                num_of_input_elements // num_of_output_elements
            )
        ...
        PermuteInputs().set_input_permutation(
            node.in_node(1), node, "output:0", "shape"
        )
