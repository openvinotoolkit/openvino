...
class TestMoFreezePlaceholderTFFE(unittest.TestCase):
    ...
    def test_conversion_fake_pb_model(
        self, use_new_frontend, use_legacy_frontend, framework
    ):
    ...
    def test_conversion_empty_model(
        self, use_new_frontend, use_legacy_frontend, framework, exp_reg_exp
    ):
        with tempfile.NamedTemporaryFile(mode="w") as tmp, self.assertRaisesRegex(
            Exception, exp_reg_exp
        ):
            tmp.write("")
            convert_model(
                tmp.name,
    ...
    def test_conversion_fake_model_with_no_ext(
        self, use_new_frontend, use_legacy_frontend, framework, exp_reg_exp
    ):
        with tempfile.NamedTemporaryFile(mode="w") as tmp, self.assertRaisesRegex(
            Exception, exp_reg_exp
