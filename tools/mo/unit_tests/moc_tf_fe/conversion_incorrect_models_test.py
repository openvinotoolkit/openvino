...
class TestMoFreezePlaceholderTFFE(unittest.TestCase):
    def test_conversion_fake_pb_model(self):
        ...
        for idx, (use_new_frontend, use_legacy_frontend, framework) in enumerate(test_cases):
            with self.subTest(idx=idx, use_new_frontend=use_new_frontend,
                              use_legacy_frontend=use_legacy_frontend, framework=framework):
                with self.assertRaisesRegex(Exception, "Internal error or inconsistent input model: the frontend supports frozen formats (.pb and .pbtxt), SavedModel and MetaGraph (.meta), and v1 checkpoints."):
    ...
    def test_conversion_empty_model(self):
        test_cases = [
            ...
            (False, False, "tf",
             r"Internal error or inconsistent input model: the frontend supports frozen formats (.pb and .pbtxt), SavedModel and MetaGraph (.meta), and v1 checkpoints."),
