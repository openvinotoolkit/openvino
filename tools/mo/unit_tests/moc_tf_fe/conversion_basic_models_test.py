...
class TestMoFreezePlaceholderTFFE(unittest.TestCase):
    ...
    def test_fp32(self):
        ...
        for idx, (input_freezing_value, inputs, expected,
                  dtype) in enumerate(test_cases):
    ...
    def test_int32(self):
        ...
        for idx, (input_freezing_value, inputs, expected, dtype) in enumerate(test_cases):
            with self.subTest(idx=idx, input_freezing_value=input_freezing_value,
                              inputs=inputs, expected=expected, dtype=dtype):
    ...
    def test_bool2(self):
        ...
        for idx, (input_freezing_value, inputs, expected, dtype, freeze_placeholder_with_value,
                  input_shape, only_conversion) in enumerate(test_cases):
            with self.subTest(idx=idx, input_freezing_value=input_freezing_value,
                              inputs=inputs, expected=expected, dtype=dtype,
                              freeze_placeholder_with_value=freeze_placeholder_with_value,
                ...
                self.basic("model_bool2.pbtxt", input_freezing_value, inputs, dtype, expected,
                           freeze_placeholder_with_value, input_shape, only_conversion)
        ...
        for idx, (input_freezing_value, inputs, expected, dtype, freeze_placeholder_with_value,
            ...
            with self.subTest(idx=idx, input_freezing_value=input_freezing_value,
                ...
                self.basic("model_three_inputs.pbtxt", input_freezing_value, inputs, dtype, expected,
                           freeze_placeholder_with_value, input_shape, only_conversion, True)
    ...
    def test_placeholder_with_default(self):
        ...
        for idx, (inputs, inputs_data, expected, dtype, freeze_placeholder_with_value,
                  input_shape, only_conversion) in enumerate(test_cases):
            with self.subTest(idx=idx, inputs=inputs, inputs_data=inputs_data, expected=expected, dtype=dtype,
                              freeze_placeholder_with_value=freeze_placeholder_with_value,
                              input_shape=input_shape, only_conversion=only_conversion):
    ...
    def test_conversion_pbtxt_model_with_inference(self):
        ...
        for idx, (inputs, expected, dtype) in enumerate(test_cases):
            with self.subTest(idx=idx, inputs=inputs, expected=expected, dtype=dtype):
    ...
    def test_conversion_model_with_undefined_constant(self):
        ...
        for idx, (model_name, argv_input, inputs, expected, dtype, use_new_frontend,
                  use_legacy_frontend) in enumerate(test_cases):
            with self.subTest(idx=idx, model_name=model_name, argv_input=argv_input, inputs=inputs,
                              expected=expected, dtype=dtype, use_new_frontend=use_new_frontend,
