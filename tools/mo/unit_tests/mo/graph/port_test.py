...
nodes = {
    ...
    **regular_op("Op3", {"type": "Op3", "kind": "op", "op": "Op3"}),
    "input_data": {
        "kind": "data",
        "fw_tensor_debug_info": [("input", "input"), ("Op1", "Op1,Op2")],
    },
...
class TestsGetTensorNames(unittest.TestCase):
    def test_front(self):
        ...
        self.assertTrue(
            input_node.out_port(0).get_tensor_names() == ["Op1\\,Op2", "input"]
        )
    ...
    def test_middle(self):
        graph = build_graph(
            nodes,
            [("input", "input_data"), ("input_data", "Op1"), ("input_data", "Op2")],
        )
    ...
    def test_port_renumber(self):
        ...
        input_node = Node(graph, "input")
        self.assertTrue(
            input_node.out_port(0).get_tensor_names(port_renumber=True)
            == ["Op1\\,Op2", "input"]
        )
        ...
        self.assertTrue(
            op1_node.out_port(0).get_tensor_names(port_renumber=True) == ["Op1\\,Op2"]
        )
    ...
    def test_reconnect_middle_case1(self):
        graph = build_graph(
            nodes, [("input", "input_data"), ("input_data", "Op1"), ("Op3", "Op3_data")]
        )
        input_node = Node(graph, "input")
        ...
        self.assertTrue(
            op3_node.out_port(0).get_tensor_names() == ["Op1\\,Op2", "Op3", "input"]
        )
        ...
        graph = build_graph(
            ...
            [
                ...
                (
                    "Op3",
                    "Op2",
                    {"in": 0, "out": 0, "fw_tensor_debug_info": [("Op3", "Op3")]},
                ),
            ],
        )
        graph.stage = "front"
    ...
    def test_reconnect_middle_case2(self):
        graph = build_graph(
            ...
            [
                ("input", "input_data"),
                ("input_data", "Op1", {"out": 0}),
                ("input_data", "Op1", {"out": 1}),
                ("Op3", "Op3_data"),
            ],
        )
        input_node = Node(graph, "input")
...
class TestForceShape(unittest.TestCase):
    def test_set_value_and_shape_with_force_shape_attribute_in_op(self):
        ...
        graph = build_graph(
            {**valued_const_with_data("const", np.array([1, 2, 3])), **result()},
        ...
        self.assertTrue(
            np.array_equal(
                node.out_port(0).data.get_shape(), np.array([2, 5, 7], dtype=np.int64)
            ),
