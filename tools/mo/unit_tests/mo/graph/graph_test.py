...
edges = {("0", "2"), ("2", "3"), ("4", "6"), ("1", "5"), ("5", "6"), ("input_3", "6")}
...
class TestReplaceNode(unittest.TestCase):
    def test_replace_node_one_consumer(self):
        ...
        new_node = Const(graph, {"name": "new"}).create_node(
            [Node(graph, "input_1"), Node(graph, "input_2")]
        )
    ...
    def test_replace_node_several_consumers(self):
        ...
        self.assertListEqual(
            sorted(graph.out_edges("new")),
            [("new", "output_1"), ("new", "output_2"), ("new", "output_3")],
        )
        expected_result = [
            ("new", "output_1", {"in": 0, "out": 2, "name": "old"}),
...
class GetNodesWithPorts(unittest.TestCase):
    def test_get_nodes_with_ports(self):
        ...
        result = graph.get_inputs_with_ports(
            match=match,
            pattern_edges=edges,
...
class TestGraphShapeChecker(unittest.TestCase):
    ...
    def test_check_shape_consistency_1(self):
        ...
        with self.assertRaisesRegex(
            Error, r"Graph contains data nodes \(1\) with inconsistent shapes:.*"
        ):
    ...
    def test_check_shape_consistency_2(self):
        ...
        with self.assertRaisesRegex(
            Error, r"Graph contains data nodes \(2\) with inconsistent shapes:.*"
        ):
            graph.check_shapes_consistency()
...
class TestGraphPortsChecker:
    ...
    @pytest.mark.parametrize(
        "node_id, port_type, port_idx",
        [("0", "in", 1), ("0", "out", 2), ("1", "in", 2), ("3", "out", 2)],
    )
    def test_check_shape_consistency_1(
        self, node_id: str, port_type: str, port_idx: int
    ):
        #
        #               ,->2-->2_data---,->3-->3_data
        #   0-->0_data-/-->1-->1_data--/
        ...
        with pytest.raises(
            Error,
            match="Node {} has not consecutive {} ports indexes:.*".format(
                node_id, port_type
            ),
        ):
...
class TestNewGraphAPIMiddle(unittest.TestCase):
    ...
    for idx in range(11):
        ...
        nodes_10_in_10_out.update(
            {"in_{}_data".format(idx): {"value": None, "shape": None, "kind": "data"}}
        )
        nodes_10_in_10_out.update(
            {
                "out_{}".format(idx): {
    ...
    def test_connection_set_source_1(self):
        ...
        for port in destinations:
            self.assertTrue(
                port in [node_1.in_port(0), node_2.in_port(0), node_4.in_port(0)]
            )
    ...
    def test_connection_data_1(self):
        ...
        self.assertTrue(
            np.array_equal(
                node_0.out_port(0).get_connection().data.get_shape(), (1, 3, 64, 64)
            )
        )
        self.assertTrue(
            np.array_equal(
                node_0.out_port(0).get_connection().data.get_value(),
                np.ones((1, 3, 64, 64)),
            )
        )
    ...
    def test_node_in_ports_order_10_inputs(self):
        ...
        l1 = [
            node_concat.in_port(idx).get_source().node.name
            for idx in node_concat.in_ports()
        ]
        l2 = [
            node_concat.in_node(idx).in_node(0).name for idx in node_concat.in_nodes()
        ]

        self.assertEqual(l1, l2)

        l1 = [
            node_split.out_port(idx).get_destination().node.name
            for idx in node_split.out_ports()
        ]
        l2 = [
            node_split.out_node(idx).out_node(0).name for idx in node_split.out_nodes()
        ]
    ...
    def test_node_in_ports_order_10_inputs_control_flow(self):
        ...
        l1 = [
            node_concat.in_port(idx, control_flow=True).get_source().node.name
            for idx in node_concat.in_ports(control_flow=True)
        ]
        l2 = [
            node_concat.in_node(idx, control_flow=True)
            .in_node(0, control_flow=True)
            .name
        ...
        l1 = [
            node_split.out_port(idx, control_flow=True).get_destination().node.name
            for idx in node_split.out_ports(control_flow=True)
        ]
        l2 = [
            node_split.out_node(idx, control_flow=True)
            .out_node(0, control_flow=True)
            .name
            for idx in node_split.out_nodes(control_flow=True)
        ]
...
class TestNewGraphAPIFront(unittest.TestCase):
    ...
    def test_port_get_destination_3(self):
        ...
        self.assertTrue(
            (destinations[0] == node_2_in_port and destinations[1] == node_3_in_port)
            or (destinations[1] == node_2_in_port and destinations[0] == node_3_in_port)
        )
    ...
    def test_port_get_source_2(self):
        graph = build_graph(self.nodes, [("0", "1"), ("2", "1")])
        graph.__setattr__("stage", "front")
    ...
    def test_port_disconnect_1(self):
        ...
        #                                               2-->2_data
        graph = build_graph(
            self.nodes, [("0", "1", {"out": 0}), ("0", "2", {"out": 0})]
        )
        graph.__setattr__("stage", "front")
    ...
    def test_port_disconnect_3(self):
        ...
        #   0---->2    ==> 0-->2
        #
        graph = build_graph(self.nodes, [("0", "2"), ("1", "2")])
        graph.__setattr__("stage", "front")
    ...
    def test_port_disconnect_4(self):
        ...
        #
        graph = build_graph(
            self.nodes, [("0", "2", {"out": 0}), ("1", "2", {"out": 0})]
        )
        graph.__setattr__("stage", "front")
    ...
    def test_port_disconnected_1(self):
        graph = build_graph(self.nodes, [("0", "1"), ("1", "2")])
        graph.__setattr__("stage", "front")
    ...
    def test_connection_get_source_destinations_1(self):
        graph = build_graph(self.nodes, [("0", "1"), ("0", "2")])
        graph.__setattr__("stage", "front")
...
class TestGetSetAttributeBetweenNodes(unittest.TestCase):
    ...
    def test_get_attribute_between_nodes(self):
        ...
        f_node = Node(graph, "F")
        self.assertTrue(
            get_edge_attribute_between_nodes(a_node, d_node, "Attr") == "A-D"
        )
        self.assertTrue(
            get_edge_attribute_between_nodes(a_node, e_node, "Attr") == "A-E"
        )
        self.assertTrue(
            get_edge_attribute_between_nodes(a_node, f_node, "Attr") == "A-F"
        )
        self.assertTrue(
            get_edge_attribute_between_nodes(b_node, d_node, "Attr") == "B-D"
        )
        self.assertTrue(
            get_edge_attribute_between_nodes(b_node, f_node, "Attr") == "B-F"
        )
    ...
    def test_set_attribute_between_nodes(self):
        ...
        self.assertTrue(
            get_edge_attribute_between_nodes(a_node, d_node, "Attr") == "new_value_1"
        )
        self.assertTrue(
            get_edge_attribute_between_nodes(a_node, e_node, "Attr") == "new_value_2"
        )
        self.assertTrue(
            get_edge_attribute_between_nodes(a_node, f_node, "Attr") == "new_value_3"
        )
        self.assertTrue(
            get_edge_attribute_between_nodes(b_node, d_node, "Attr") == "new_value_4"
        )
        self.assertTrue(
            get_edge_attribute_between_nodes(b_node, f_node, "Attr") == "new_value_5"
        )
...
class TestTopologicalSort(unittest.TestCase):
    ...
    def test_sort_with_start_node(self):
        ...
        nodes_names = [
            node.name
            for node in graph.pseudo_topological_sort_with_start_node(
                start_node=stat_node
            )
        ]
        ...
        nodes_names = [
            node.name
            for node in graph.pseudo_topological_sort_with_start_node(
                start_node=stat_node, reverse=True
            )
        ]
