...
class TestsFront(unittest.TestCase):
        ...
        for node in graph_ref.get_op_nodes():
            if len(node.out_edges()) > 0:
                ...
                if "fw_tensor_debug_info" in out_edge_ref:
                    self.assertTrue(
                        out_edge["fw_tensor_debug_info"]
                        == out_edge_ref["fw_tensor_debug_info"]
                    )
                else:
        ...
        graph = build_graph(
            ...
            [
                (
                    "input",
                    "Op1",
                    {"in": 0, "out": 0, "fw_tensor_debug_info": [("input", "input")]},
                )
            ],
        )
    ...
    def test_case5_merge(self):
        graph = build_graph(
            ...
            [
                ...
                (
                    "Op1",
                    "Op2",
                    {"in": 0, "out": 0, "fw_tensor_debug_info": [("Op1", 0, "Op1")]},
                ),
            ],
        )
        graph_ref = build_graph(
        ...
        graph.stage = "front"
        op1_node.out_port(0).get_connection().set_source(
            op1_node.in_port(0).get_source(), "merge"
        )
    ...
    def test_case5_source(self):
        ...
        graph.stage = "front"
        op1_node.out_port(0).get_connection().set_source(
            op1_node.in_port(0).get_source(), "source"
        )
    ...
    def test_case5_dest(self):
        ...
        graph.stage = "front"
        op1_node.out_port(0).get_connection().set_source(
            op1_node.in_port(0).get_source(), "dest"
        )
    ...
    def test_case6_merge(self):
        ...
        graph.stage = "front"
        op1_node.in_port(0).get_connection().set_destination(
            op1_node.out_port(0).get_destination(), "merge"
        )
    ...
    def test_case6_source(self):
        ...
        graph.stage = "front"
        op1_node.in_port(0).get_connection().set_destination(
            op1_node.out_port(0).get_destination(), "source"
        )
...
class TestsMiddle(unittest.TestCase):
        ...
        for node in graph_ref.get_op_nodes():
            if len(node.out_nodes()) > 0:
                ...
                if "fw_tensor_debug_info" in data_node_ref:
                    self.assertTrue(
                        data_node_ref["fw_tensor_debug_info"]
                        == data_node["fw_tensor_debug_info"]
                    )
                else:
    ...
    def test_case1_merge(self):
        ...
        graph_ref = build_graph(
            nodes,
            [("input", "input_data"), ("input_data", "Op1"), ("input_data", "NewOp")],
        )
        input_node = Node(graph, "input")
        new_node = Node(graph, "NewOp")
        new_node.add_input_port(0)
    ...
    def test_case2_merge(self):
        graph = build_graph(nodes, [("input", "input_data"), ("input_data", "Op1")])
        graph_ref = build_graph(
            nodes, [("input", "input_data"), ("input_data", "NewOp")]
        )
        op1_node = Node(graph, "Op1")
        new_node = Node(graph, "NewOp")
        new_node.add_input_port(0)
    ...
    def test_case3_merge(self):
        ...
        graph_ref = build_graph(
            nodes,
            [("input", "input_data"), ("NewOp", "NewOp_data"), ("NewOp_data", "Op1")],
        )
    ...
    def test_case6_merge(self):
        ...
        graph_ref = build_graph(
            nodes, [("input", "input_data"), ("input_data", "Op2"), ("Op1", "Op1_data")]
        )
