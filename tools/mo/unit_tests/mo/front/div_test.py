...
nodes = {
    **regular_op_with_shaped_data(
        "placeholder_1", [1, 227, 227, 3], {"type": "Parameter"}
    ),
    **regular_op_with_shaped_data(
        "placeholder_2", [1, 227, 227, 3], {"type": "Parameter"}
    ),
    **regular_op_with_shaped_data(
        "div", None, {"op": "Div", "type": "Divide", "name": "my_div"}
    ),
    **regular_op_with_shaped_data("reciprocal", [1, 227, 227, 3], {"type": "Power"}),
    **valued_const_with_data("minus_one", np.array(-1.0)),
...
class TestDiv(unittest.TestCase):
    def test_div_test_1(self):
        ...
        self.assertTrue(
            graph.node[graph.get_nodes_with_attributes(type="Multiply")[0]]["name"]
            == "my_div"
        )
        ...
        graph = build_graph(
            {
                **regular_op_with_shaped_data(
                    "parameter",
                    [1, 227, 227, 3],
                    {"type": "Parameter", "data_type": np.int32},
                ),
                **valued_const_with_data("const", np.array([-1.0], dtype=np.int32)),
                **regular_op_with_shaped_data(
                    "div", None, {"op": "Div", "type": "Divide", "name": "my_div"}
                ),
                **result(),
