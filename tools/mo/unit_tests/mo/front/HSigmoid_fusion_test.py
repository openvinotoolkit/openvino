...
from openvino.tools.mo.front.HSigmoid_fusion import (
    HSigmoidWithClamp,
    HSigmoidWithMinMax,
    HSigmoidWithReluDiv,
    HSigmoidWithReluMul,
...
class HSigmoidWithReluDivTest(unittest.TestCase):
    ...
    def test_hsigmoid_with_relu_div(self):
        ...
        self.assertTrue(flag, resp)
        self.assertTrue(
            len(graph.get_op_nodes(name="final_div")) == 1
            and graph.get_op_nodes(name="final_div")[0].op == "HSigmoid"
        )
        self.assertTrue(
            graph.get_op_nodes(name="final_div")[0].out_nodes()[0].node == "result"
        )

    def test_hsigmoid_with_relu_div_wrong_constant(self):
        graph = build_graph_with_edge_attrs(
            self.nodes, self.edges, {"add_const": {"value": float_array([0.00001])}}
        )
...
class HSigmoidWithReluMulTest(unittest.TestCase):
    ...
    def test_hsigmoid_with_relu_mul(self):
        ...
        self.assertTrue(
            graph.get_op_nodes(name="final_mul")[0].out_nodes()[0].node == "result"
        )
