...
from openvino.tools.mo.front.mxnet.ssd_pattern_remove_transpose import (
    SsdPatternRemoveTranspose,
)
...
class TestSsdPatternRemoveTranspose(unittest.TestCase):
    def test_pattern_remove_transpose(self):
        graph = build_graph(
            {
                ...
                "node_softmax_activation": {
                    "type": "SoftMax",
                    "value": None,
                    "kind": "op",
                    "op": "SoftMax",
                },
                "node_multi_box_detection": {
        ...
        self.assertTrue(
            graph.has_edge(
                Node(graph, "node_1").id, Node(graph, "node_softmax_activation").id
            )
        )
