...
class TestLoader(UnitTestWithMockedTelemetry):
    ...
    def test_load_symbol_nodes(
        self, mock_symbol_load, mock_isfile, mock_json_loads, mock_json_load
    ):
        ...
        with patch("openvino.tools.mo.front.mxnet.loader.open") as mock_open:
            self.assertEqual(
                {"node1": 1}, load_symbol_nodes("model_name", legacy_mxnet_model=True)
            )
    ...
    @patch("os.path.isfile")
    @patch("mxnet.symbol.load")
    def test_load_symbol_with_custom_nodes(
        self, mock_symbol_load, mock_isfile, mock_json_loads, mock_json_load
    ):
        mock_isfile.return_value = True
        mock_json_load.return_value = {
            "nodes": [{"op": "custom_op"}, {"op": "custom_op"}]
        }
        mock_json_loads.return_value = {"nodes": {"node1": 1}}
        mock_symbol_load_obj = MockSymbolLoadObj()
        mock_symbol_load.return_value = mock_symbol_load_obj
    ...
    @patch("os.path.isfile")
    @patch("mxnet.symbol.load")
    def test_load_symbol_nodes_with_json_and_lagacy_mode(
        self, mock_symbol_load, mock_isfile, mock_json_loads, mock_json_load
    ):
        mock_isfile.return_value = True
        mock_json_load.return_value = {"nodes": ""}
    ...
    @patch("os.path.isfile")
    @patch("mxnet.symbol.load")
    def test_load_symbol_nodes_with_json(
        self, mock_symbol_load, mock_isfile, mock_json_loads, mock_json_load
    ):
        mock_isfile.return_value = True
        # json.load
