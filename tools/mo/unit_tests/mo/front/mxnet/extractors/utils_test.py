...
from openvino.tools.mo.front.mxnet.extractors.utils import AttrDictionary, load_params
...
class TestUtils(unittest.TestCase):
    ...
    def test_load_symbol_nodes_from_params(self, mock_nd_load):
        mock_nd_load.return_value = {
            "arg:conv0_weight": mx.nd.array([1, 2], dtype="float32"),
        ...
        self.assertEqual(
            [1.0, 2.0], model_params._arg_params["conv0_weight"].asnumpy().tolist()
        )
        self.assertEqual(
            [2.0, 3.0], model_params._arg_params["conv1_weight"].asnumpy().tolist()
        )
        self.assertEqual(
            [5.0, 6.0], model_params._aux_params["bn_data_mean"].asnumpy().tolist()
        )

    @patch("mxnet.nd.load")
    def test_load_symbol_nodes_from_args_nd(self, mock_nd_load):
        mock_nd_load.return_value = {
            "conv0_weight": mx.nd.array([1, 2], dtype="float32"),
