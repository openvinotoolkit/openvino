...
from openvino.tools.mo.front.mxnet.extractors.slice_axis import (
    mxnet_slice_axis_infer,
    slice_axis_ext,
)
...
class TestMXNetSliceAxisExtractorOp(unittest.TestCase):
    def test_extract_slice_axis_layer(self):
        graph = build_graph(
            ...
            {
                "slice_axis_node": {
                    "symbol_dict": {"attrs": {"axis": 0, "begin": 10, "end": 25}}
                },
            },
        )

        exp_attrs = {"op": "Crop", "axis": 0, "offset": 10, "dim": 25}
