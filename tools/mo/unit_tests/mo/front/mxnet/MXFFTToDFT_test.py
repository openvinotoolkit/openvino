...
class TestMXFFTToDFTTest:
    @pytest.mark.parametrize(
        "input_shape", [int64_array([3, 100, 100, 8]), int64_array([5, 60])]
    )
    def test_fft_replacement(self, input_shape):
        graph = build_graph(
            nodes_attrs=fft_graph_node_attrs,
            edges=fft_graph_edges,
            update_attributes={"placeholder": {"shape": input_shape}},
        )
        graph.stage = "front"
        MXFFTToDFT().find_and_replace_pattern(graph)
        ref_graph = build_graph(
            nodes_attrs=ref_converted_fft_graph_node_attrs,
