    ...
    def test_1(self):
        graph = build_graph(
            nodes_attrs={
                ...
                **regular_op_with_empty_data(
                    "div_sqrt_dim", {"op": "_contrib_div_sqrt_dim"}
                ),
                **result("result"),
            },
            edges=[
        ...
        ref_graph = build_graph(
            nodes_attrs={
                **shaped_parameter("input", int64_array([1, 3, 15, 15])),
                **regular_op_with_empty_data(
                    "div_sqrt_shape_of", {"op": "ShapeOf", "type": "ShapeOf"}
                ),
                **shaped_const_with_data("gather_axis", None),
                **shaped_const_with_data("gather_indices", None),
                **regular_op_with_empty_data(
                    "gather", {"op": "Gather", "type": "Gather"}
                ),
                **regular_op_with_empty_data(
                    "power", {"op": "AttributedPower", "power": 0.5, "type": "Power"}
                ),
                **regular_op_with_empty_data(
                    "cast", {"op": "Cast", "type": "Convert", "dst_type": np.float32}
                ),
        ...
        flag, resp = compare_graphs(
            graph, ref_graph, "result", "result", check_op_attrs=True
        )
