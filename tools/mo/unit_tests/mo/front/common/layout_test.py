...
class TestLayoutFunctions(unittest.TestCase):
    ...
    def test_shape_for_layout_NCHW(self):
        self.assertListEqual(
            [2, 3, 4, 5],
            list(shape_for_layout("NCHW", batch=2, features=3, height=4, width=5)),
        )

    def test_shape_for_layout_NHWC(self):
        self.assertListEqual(
            [2, 4, 5, 3],
            list(shape_for_layout("NHWC", batch=2, features=3, height=4, width=5)),
        )
    ...
    def test_shape_for_layout_unknown_parameter(self):
        with self.assertRaises(Error):
            shape_for_layout(
                "NHWC", batch=2, features=3, height=4, width=5, unknown_parameter=123
            )
