...
class IsFullyDefinedTest(unittest.TestCase):
    def test_is_fully_defined(self):
        test_cases=[(None, False),
        ...
        for idx, (data, result) in enumerate(test_cases):
...
class ShapeArrayTest(unittest.TestCase):
    def test_shape_array(self):
        test_cases= [([1], shape_array([1]), True),
        ...
        for idx, (data, ref, result) in enumerate(test_cases):
            with self.subTest(test_cases=idx):
...
class CompareShapesTest(unittest.TestCase):
    def test_compare_shapes(self):
        test_cases=[(gen_masked_array([1, 2, 3], []), gen_masked_array([1, 2, 3], []), True),
        ...
        for idx, (input1, input2, result) in enumerate(test_cases):
            with self.subTest(test_cases=idx):
                self.assertEqual(compatible_shapes(input1, input2), result)


class ShapeDeleteTest(unittest.TestCase):
    def test_shape_delete(self):
        test_cases= [(gen_masked_array([1, 2, 3], []), [], gen_masked_array([1, 2, 3], [])),
        ...
        for idx, (shape, indices, result) in enumerate(test_cases):
            with self.subTest(test_cases=idx):
...
class ShapeInsertTest(unittest.TestCase):
    def test_shape_insert(self):
        test_cases =[(gen_masked_array([1, 2, 3], []), 1, [5], gen_masked_array([1, 5, 2, 3], [])),
        ...
        for idx, (shape, pos, values, result) in enumerate(test_cases):
            with self.subTest(test_cases=idx):
...
class mo_array_test(unittest.TestCase):
    def test_mo_array_positive(self):
        test_cases=[(mo_array([2, 3, 5, 7]), np.array([2, 3, 5, 7])),
    ...
    def test_mo_array_negative(self):
        test_cases=[(mo_array([2., 3., 5., 7.]), np.array([2., 3., 5., 7.])),
                ]
        for idx, (data, result) in enumerate(test_cases):
            with self.subTest(test_cases=idx):
