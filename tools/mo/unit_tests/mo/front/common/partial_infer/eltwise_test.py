...
from openvino.tools.mo.front.common.partial_infer.eltwise import (
    eltwise_infer,
    eltwise_reverse_infer,
)
from openvino.tools.mo.front.common.partial_infer.utils import (
    dynamic_dimension_value,
    reverse_bypass_infer,
    shape_array,
...
class TestEltwiseInfer:
    @pytest.mark.parametrize(
        "value1, shape1, value2, shape2, shape_infer, exp_value, exp_shape",
        [
            (
                np.array(2),
                [],
            ...
            (
                ...
                [],
                lambda a, b: np.add(a, b),
                shape_array([dynamic_dimension_value, 8]),
                [2],
            ),
            (
                shape_array([dynamic_dimension_value, 5]),
                ...
                [],
                lambda a, b: np.add(a, b),
                shape_array([dynamic_dimension_value, 12]),
                [2],
            ),
        ],
    )
    def test_eltwise_infer(
        self, value1, shape1, value2, shape2, shape_infer, exp_value, exp_shape
    ):
        graph = build_graph(
            nodes_attributes,
            [
...
class TestElementwiseReverseInfer(unittest.TestCase):
    @staticmethod
    def build_and_test_reverse_inference(
        inp_shape_1, inp_shape_2, out_shape, ref_shape, auto_broadcast="numpy"
    ):
        ...
        nodes = {
            **shaped_parameter(
                "undefined_shape_data", None, {"reverse_infer": Parameter.reverse_infer}
            ),
            **shaped_parameter(
                "data",
                shape_array(defined_shape),
        ...
        edges = [
            *connect(
                "undefined_shape_data",
                "{}:elementwise".format(int(not in_port_with_defined_shape)),
            ),
        ...
        Node(graph, "elementwise").in_port(in_port_with_defined_shape).data.set_shape(
            defined_shape
        )
    ...
    def test_reverse_infer_1(self):
        self.build_and_test_reverse_inference(
            inp_shape_1=[dyn, dyn],
            inp_shape_2=None,
    ...
    def test_reverse_infer_6(self):
        ...
        with self.assertRaisesRegex(
            Error, "Model Optimizer is unable to deduce input shapes"
        ):
            self.build_and_test_reverse_inference(
                inp_shape_1=[dyn, dyn, dyn, dyn],
                inp_shape_2=None,
                out_shape=[dyn, dyn, 4, 1],
                ref_shape=None,
            )
    ...
    def test_reverse_infer_8(self):
        with self.assertRaisesRegex(
            AssertionError, "Shapes of Elementwise node '.*' are not compatible"
        ):
            self.build_and_test_reverse_inference(
                inp_shape_1=[4, dyn],
                inp_shape_2=None,
        ...
        nodes = {
            ...
            **regular_op_with_empty_data(
                "elementwise",
                {
                    ...
                    "type": "Sqrt",
                    "infer": eltwise_infer,
                    "reverse_infer": lambda node: reverse_bypass_infer(
                        node, in_ports=[0]
                    ),
                },
            ),
