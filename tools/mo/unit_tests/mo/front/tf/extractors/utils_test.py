...
class TensorContentParsing(unittest.TestCase):
    ...
    def test_list_type_no_shape(self):
        pb_tensor = PB(
            {"dtype": 3, "int_val": np.array([1, 2, 3, 4, 5], dtype=np.int32)}
        )
        tf_dtype = pb_tensor.dtype
        shape = np.array([5])
        ref = [1, 2, 3, 4, 5]
    ...
    def test_nd_not_type_no_shape(self):
        pb_tensor = PB(
            {
                "dtype": 3,
                "tensor_content": b"\x01\x00\x00\x00\x02\x00\x00\x00\x03\x00\x00\x00\x04\x00\x00\x00\x05\x00\x00\x00\x06\x00\x00\x00",
            }
        )
        tf_dtype = pb_tensor.dtype
        shape = np.array([2, 3])
    ...
    def test_str_decode(self):
        ...
        shape = int64_array([1])
        warning_message = (
            "ERROR:root:Failed to parse a tensor with Unicode characters. Note that OpenVINO "
    ...
    def test_empty_value(self):
        pb_tensor = PB({"dtype": 1, "float_val": []})
    ...
    def test_scalar_value(self):
        pb_tensor = PB({"dtype": 3, "int_val": 4})
