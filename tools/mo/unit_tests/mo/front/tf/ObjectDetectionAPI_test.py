...
from unit_tests.utils.graph import build_graph, connect_front, const, regular_op, result

from openvino.tools.mo.front.common.partial_infer.utils import float32_array
from openvino.tools.mo.front.subgraph_matcher import SubgraphMatch
from openvino.tools.mo.front.tf.ObjectDetectionAPI import (
    ObjectDetectionAPIPreprocessor2Replacement,
    calculate_placeholder_spatial_shape,
    calculate_shape_keeping_aspect_ratio,
)
...
class TestCalculateShape:
    ...
    def test_calculate_shape(self, h, w, th, tw):
        assert calculate_shape_keeping_aspect_ratio(
...
class TestCalculatePlaceholderSpatialShape(unittest.TestCase):
    ...
    def test_fixed_shape_resizer_overrided_by_user(self):
        ...
        self.graph.graph["user_shapes"] = {
            "image_tensor": [{"shape": PartialShape([1, 400, 500, 3])}]
        }
        self.assertTupleEqual(
            (400, 500),
            calculate_placeholder_spatial_shape(
    ...
    def test_keep_aspect_ratio_resizer_overrided_by_user(self):
        ...
        self.pipeline_config._model_params["resizer_max_dimension"] = 1024
        self.graph.graph["user_shapes"] = {
            "image_tensor": [{"shape": PartialShape([1, 400, 300, 3])}]
        }
        self.assertTupleEqual(
            (800, 600),
            calculate_placeholder_spatial_shape(
...
class TestObjectDetectionAPIPreprocessor2Replacement(unittest.TestCase):
    def setUp(self):
        ...
        self.replacement_desc = {
            "start_nodes": [self.start_node_name],
    ...
    def test_case_1_pad_to_max_dim(self, update_parameter_shape_mock):
        ...
        (flag, resp) = compare_graphs(
            graph, self.build_ref_graph(False), "result", check_op_attrs=True
        )
    ...
    def test_case_1_no_pad_to_max_dim(self, update_parameter_shape_mock):
        ...
        with unittest.mock.patch(
            "builtins.open",
            unittest.mock.mock_open(read_data=updated_pipeline_config_content),
        ):
            ObjectDetectionAPIPreprocessor2Replacement().transform_graph(
                graph, self.replacement_desc
            )
        ...
        (flag, resp) = compare_graphs(
            graph, self.build_ref_graph(True), "result", check_op_attrs=True
        )
        self.assertTrue(flag, resp)
    ...
    def build_main_graph(self, pre_processing: str):
        def build_body_graph(pre_processing: str):
            nodes = {
                ...
                **regular_op(
                    "mul", {"op": "Mul", "type": "Multiply", "name": "my_body_mul"}
                ),
                **regular_op(
                    "sub", {"op": "Sub", "type": "Subtract", "name": "my_body_sub"}
                ),
        ...
        graph = build_graph(
            self.nodes,
            edges,
            {"loop": {"body": build_body_graph(pre_processing)}},
...
class TestPipelineConfig(unittest.TestCase):
    def test_pipeline_config_loading(self):
        ...
        pipeline_config = PipelineConfig(
            os.path.join(os.path.dirname(__file__), "test_configs/config1.config")
        )
        ...
        pipeline_config = PipelineConfig(
            os.path.join(os.path.dirname(__file__), "test_configs/config2.config")
        )
