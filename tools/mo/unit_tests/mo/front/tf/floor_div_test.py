...
nodes = {
    **regular_op_with_empty_data(
        "placeholder_1", {"type": "Parameter", "op": "Parameter"}
    ),
    **regular_op_with_empty_data(
        "placeholder_2", {"type": "Parameter", "op": "Parameter"}
    ),
    **regular_op_with_empty_data(
        "floor_div", {"op": "FloorDiv", "type": None, "name": "my_floor_div"}
    ),
...
class TestFloorDiv(unittest.TestCase):
    def test_floor_div_test_1(self):
        ...
        self.assertTrue(
            graph.node[graph.get_nodes_with_attributes(type="Floor")[0]]["name"]
            == "my_floor_div"
        )
