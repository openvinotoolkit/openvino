...
class TestActivationOpsONNXExtractorTest:
    ...
    def _match(self, out, ref):
        for key in ref.keys():
            ...
            assert (
                status
            ), f"Mismatch for field {key}, observed: {out[key]}, expected: {ref[key]}"
...
class TestEluONNXExt:
    ...
    def _create_elu_node(alpha=1.0):
        pb = onnx.helper.make_node("Elu", inputs=["x"], outputs=["y"], alpha=alpha)
        node = PB({"pb": pb})
        return node
    ...
    def test_elu_ext(self, alpha):
        ...
        exp_res = {"type": "Elu", "alpha": alpha, "infer": Elu.infer}
