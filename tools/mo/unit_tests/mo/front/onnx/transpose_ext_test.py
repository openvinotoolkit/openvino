...
class TestTransposeONNXExt:
    ...
    def _create_transpose_node(order: list):
        if order is None:
            ...
            pb = onnx.helper.make_node(
                "Transpose", inputs=["data"], outputs=["transposed"], perm=order
            )
        node = PB({"pb": pb})
        return node
    ...
    @pytest.mark.parametrize(
        "order",
        [list(order) for order in list(itertools.permutations(np.arange(4)))]
        + [list(order) for order in list(itertools.permutations(np.arange(3)))]
        + [None],
    )
    def test_transpose_ext(self, order):
        ...
        exp_res = {"type": "Transpose", "order": order, "infer": Transpose.infer}
