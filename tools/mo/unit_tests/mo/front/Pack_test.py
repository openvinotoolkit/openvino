...
class TestPackTest:
    ...
    @pytest.mark.parametrize(
        "num_inputs, num_placeholders, axis",
        [(2, 2, 0), (3, 3, 0), (4, 4, 0), (4, 4, 1), (4, 1, 0), (4, 1, 1)],
    )
    def test_pack_test_all(self, num_inputs: int, num_placeholders: int, axis: list):
        ...
        for i in range(num_placeholders):
            update_graph_attributes["placeholder_{}".format(i)] = {
        ...
        for i in range(num_inputs - num_placeholders + 1):
            for j in range(num_placeholders):
                graph_ref_edges.append(
                    ("placeholder_{}".format(j), "Unsqueeze_{}".format(i + j))
                )
        ...
        for i in range(num_placeholders):
            update_graph_ref_attributes["placeholder_{}".format(i)] = {
                "shape": np.array([1, 227, 227, 3])
            }
        for i in range(num_inputs):
            graph_ref_edges.append(
                ("Unsqueeze_{}_axis".format(i), "Unsqueeze_{}".format(i))
            )
            update_graph_ref_attributes["Unsqueeze_{}_axis".format(i)] = {
                "shape": int64_array([1]),
