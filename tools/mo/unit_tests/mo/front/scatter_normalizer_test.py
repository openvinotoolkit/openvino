...
nodes = {
    ...
    **regular_op_with_empty_data(
        "node", {"op": "ScatterElementsUpdate", "is_scatter": True}
    ),
...
class TestDiv(unittest.TestCase):
    def test_ScatterElementsUpdate_has_axis_and_3_inputs(self):
        graph = build_graph(
            nodes, edges, {"node": {"axis": 1}}, nodes_with_edges_only=True
        )
    ...
    def test_ScatterElementsUpdate_has_axis_and_4_inputs(self):
        graph = build_graph(
            ...
            {"node": {"axis": 1}, "axis": {"value": np.int64(1)}},
            nodes_with_edges_only=True,
        )
        self.assertRaises(
            AssertionError, ScatterNormalizer().find_and_replace_pattern, graph
        )
