...
from openvino.tools.mo.front.caffe.extractors.utils import (
    embed_input,
    get_canonical_axis_index,
    weights_biases,
)
...
class TestEmbedInput(unittest.TestCase):
    def test_embed_input_no_bin_name_no_bias(self):
        ...
        exp_res = {"weights": blob, "embedded_inputs": [(1, name, {"bin": name})]}
    ...
    def test_embed_input_w_bin_name(self):
        ...
        exp_res = {
            "weights": blob,
            "embedded_inputs": [(1, name, {"bin": "special_name"})],
        }
        for key in exp_res.keys():
            if key == name:
                np.testing.assert_equal(attrs[key], exp_res[key])
