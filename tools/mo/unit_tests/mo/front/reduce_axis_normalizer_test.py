...
import numpy as np
from unit_tests.utils.graph import build_graph, connect_front, regular_op, result
...
class ReduceAxisNormalizerTest(unittest.TestCase):
    def test_reduce_axis_is_None(self):
        ...
        ref_nodes.update(
            {
                **regular_op("rank", {"op": "Rank", "type": None}),
    ...
    def test_reduce_axis_is_const(self):
        graph = build_graph(
            nodes, edges, {"reduce": {"axis": 1}}, nodes_with_edges_only=True
        )
        graph.stage = "front"
