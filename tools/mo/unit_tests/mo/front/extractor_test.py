...
from openvino.tools.mo.front.extractor import (
    ...
    input_user_data_repack,
    output_user_data_repack,
    spatial_attr_getter,
    update_ie_fields,
)
...
class TestExtractor(unittest.TestCase):
    def test_spatial_attr_getter(self):
        ...
        graph = build_graph(
            nodes_attributes,
            [("input", "pool_1"), ("pool_1", "output"), ("output", "op_output")],
            {
        ...
        for param in params.keys():
            if type(params[param]) is np.ndarray:
                ...
                self.assertEqual(
                    params[param][2],
                    spatial_attr_getter(
                        pool_1_node, field=param, dim=0, post=port_lambda
                    ),
                )
                self.assertEqual(
                    params[param][3],
                    spatial_attr_getter(
                        pool_1_node, field=param, dim=1, post=port_lambda
                    ),
                )
    ...
    def test_attr_getter(self):
        ...
        graph = build_graph(
            nodes,
            [("input", "reshape"), ("reshape", "output"), ("output", "op_output")],
            {
                "input": {"shape": input_shape},
        ...
        for param in params.keys():
            if type(params[param]) is list:
                self.assertEqual(expect_params[param], attr_getter(pool_1_node, param))
...
class TestAddInputOp(unittest.TestCase):
    ...
    def test_in_port_no_data(self):
        ...
        graph_ref = build_graph_with_attrs(
            ...
            new_nodes_with_attrs=[
                (
                    "input_node",
                    {"kind": "op", "op": "Parameter", "shape": new_input_shape},
                )
            ],
    ...
    def test_in_port_with_data(self):
        ...
        graph_ref = build_graph_with_attrs(
            ...
            new_edges_with_attrs=[
                ("input_node", "input_data", {"in": 0, "out": 0}),
    ...
    edges_out = [
        ("op_node", "future_input", {"in": 0, "out": 1}),
        ("op_node", "another_node", {"in": 0, "out": 0}),
    ]

    def test_out_port_no_data(self):
        graph = build_graph_with_attrs(
            nodes_with_attrs=self.nodes_out, edges_with_attrs=self.edges_out
        )
        new_input_shape = np.array([1, 2, 3, 4])
        graph_ref = build_graph_with_attrs(
            nodes_with_attrs=self.nodes_out,
        ...
        add_input_op(
            graph, "op_node", 1, data=False, shape=new_input_shape, is_out_port=True
        )
    ...
    def test_out_port_with_data(self):
        ...
        add_input_op(
            graph, "op_node", 1, data=True, shape=new_input_shape, is_out_port=True
        )
...
class TestInputAddition(UnitTestWithMockedTelemetry):
    ...
    def test_wrong_output_port_raise(self):
        ...
        with self.assertRaisesRegex(
            Error,
            "Output port index 5 is out of number of available output ports for node",
        ):
            add_input_ops(graph=graph, user_defined_inputs=inputs, before_infer=True)
    ...
    def test_one_input_one_shape(self):
        ...
        edges = [("old_input", "conv_1"), ("conv_1", "relu_1"), ("relu_1", "output")]
    ...
    def test_add_input_with_input_port_before_infer(self):
        ...
        graph_ref = build_graph(
            nodes, edges, update_attributes={"old_input": {"shape": shape}}
        )
    ...
    def test_add_input_with_output_port_before_infer(self):
        ...
        graph_ref = build_graph(
            nodes_attrs={
                "new_input": {"kind": "op", "op": "Parameter", "shape": shape},
                **nodes,
            },
            edges=[
                ("new_input", "relu_1"),
...
class TestOutputCut:
    ...
    @pytest.mark.parametrize(
        "output", [{"C": [{"port": None}]}, {"C": [{"out": 0}]}, {"C": [{"out": 1}]}]
    )
    def test_output_port_cut(self, output):
        nodes = {
            "A": {"type": "Identity", "kind": "op", "op": "Identity"},
...
class TestUserDataRepack(UnitTestWithMockedTelemetry):
    nodes = {
        "A": {"name": "Aa", "op": "Parameter", "kind": "op"},
    ...
    def test_input_user_data_repack_names_to_ids_list(self):
        ...
        self.assertDictEqual(
            input,
            {
                "A": [{"shape": None, "port": None}],
                "B": [{"shape": None, "port": None}],
            },
        )
        self.assertEqual(freeze_placeholder, None)
    ...
    def test_input_user_data_repack_names_ports_in_out(self):
        ...
        self.assertDictEqual(
            input, {"A": [{"shape": None, "out": 0}], "C": [{"shape": None, "in": 1}]}
        )
        self.assertEqual(freeze_placeholder, None)
    ...
    def test_input_user_data_repack_dict_with_shapes(self):
        ...
        input, freeze_placeholder = input_user_data_repack(
            graph, {"Aa": shape_1, "Bb": shape_2}, None
        )
        self.assertDictEqual(
            input,
            {
                "A": [{"shape": shape_1, "port": None}],
                "B": [{"shape": shape_2, "port": None}],
            },
        )
        self.assertEqual(freeze_placeholder, None)
    ...
    def test_input_user_data_repack_dict_with_shapes_and_ports(self):
        ...
        shape_2 = np.array([1, 127, 127, 3])
        input, freeze_placeholder = input_user_data_repack(
            graph, {"Aa:0": shape_1, "Bb:0": shape_2}, None
        )
        self.assertDictEqual(
            input,
            {"A": [{"shape": shape_1, "out": 0}], "B": [{"shape": shape_2, "out": 0}]},
        )
        self.assertEqual(freeze_placeholder, None)
    ...
    def test_freeze_placeholder_and_input(self):
        ...
        self.assertDictEqual(
            input,
            {"A": [{"shape": shape_1, "out": 0}], "B": [{"shape": None, "port": None}]},
        )
    ...
    def test_error(self):
        graph = build_graph(self.nodes, self.edges)
        self.assertRaises(
            Error, input_user_data_repack, graph, PartialShape([1, 227, 227, 3]), None
        )
    ...
    def test_freeze_new_placeholder_1(self):
        ...
        input, freeze_placeholder = input_user_data_repack(
            graph,
            {"Aa:0": shape_1, "Cc:0": shape_2},
            {"Bb": False, "Cc:0": [1.0, 1.0, 2.0, 3.0, 5.0]},
        )
        self.assertDictEqual(
            input,
            {
                "A": [{"shape": shape_1, "out": 0}],
                "B": [{"shape": None, "port": None}],
                "C": [{"shape": shape_2, "out": 0}],
            },
        )
        self.assertEqual(
            freeze_placeholder,
            {"B": False, "C/placeholder_out_port_0": [1.0, 1.0, 2.0, 3.0, 5.0]},
        )
    ...
    def test_freeze_new_placeholder_2(self):
        ...
        input, freeze_placeholder = input_user_data_repack(
            graph,
            {"Aa:0": shape_1, "Ee": shape_2},
            {"Bb": False, "Ee": [[1.0, 1.0], [2.0, 3.0]]},
        )
        self.assertDictEqual(
            input,
            {
                "A": [{"shape": shape_1, "out": 0}],
                "B": [{"shape": None, "port": None}],
                "E": [{"shape": shape_2, "port": None}],
            },
        )
        self.assertEqual(
            freeze_placeholder,
            {"B": False, "E/placeholder_port_None": [[1.0, 1.0], [2.0, 3.0]]},
        )
