...
class TestKaldiUtilsLoading(unittest.TestCase):
    ...
    def test_read_binary_integer64_token(self):
        stream = self.bytesio_from(
            self.pack_value(8, "B") + self.pack_value(64, self.uint64_fmt)
        )
    ...
    def test_find_next_tag(self):
        ...
        self.assertEqual(
            find_next_tag(self.bytesio_from(test_token)), test_token.decode("ascii")
        )
        fake_token = b"<FakeBegin" + test_token
        self.assertEqual(
            find_next_tag(self.bytesio_from(fake_token)), test_token.decode("ascii")
        )
    ...
    def test_find_next_component(self):
        ...
        test_file = (
            b"<Nnet>somefakeinfo<another>info"
            + component
            + b"<tag><!EndOfComponent></Nnet>"
        )
        self.assertEqual(
            find_next_component(self.bytesio_from(test_file)),
    ...
    def test_find_end_of_component(self):
        ...
        test_file = (
            b"somefakeinfo<another>info<tag>"
            + bytes(end_of_component_tag, "ascii")
            + b"</Nnet>"
        )
        end_tag, position = find_end_of_component(
            self.bytesio_from(test_file), component.lower()[1:-1]
        )
        self.assertEqual(end_tag, end_of_component_tag)
        self.assertEqual(
            position,
            test_file.decode("ascii").index(end_of_component_tag)
            + len(end_of_component_tag),
        )
    ...
    def test_get_pb(self):
        ...
        end_tag, end_position = find_end_of_component(
            self.bytesio_from(test_file), component[1:-1].lower()
        )
    ...
    def test_collect_until_token_and_read(self):
        tag = b"<InputDim>"
        test_file = (
            b"<ComponentName> opgru3.renorm <NormalizeComponent> <InputDim> "
            + self.pack_value(4, "B")
            + self.pack_value(256, "I")
            + b" <TargetRms> "
            + self.pack_value(4, "B")
            + self.pack_value(0.5, "f")
            + b" <AddLogStddev> F</NormalizeComponent>"
        )
