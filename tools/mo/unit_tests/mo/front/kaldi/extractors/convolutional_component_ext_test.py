...
from openvino.tools.mo.front.kaldi.extractors.convolutional_component_ext import (
    ConvolutionalComponentFrontExtractor,
)
...
class ConvolutionalComponentFrontExtractorTest(KaldiFrontExtractorTest):
    ...
    def test_assertion(self):
        self.assertRaises(
            AttributeError, ConvolutionalComponentFrontExtractor.extract, None
        )
    ...
    def test_convolution_blobs(self):
        self.assertTrue(
            np.array_equal(self.test_node.weights, [0, 1, 2, 3, 4, 5, 6, 7])
        )
