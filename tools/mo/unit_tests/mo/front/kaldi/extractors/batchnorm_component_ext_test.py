...
from openvino.tools.mo.front.kaldi.extractors.batchnorm_component_ext import (
    BatchNormComponentFrontExtractor,
)
...
class BatchNormComponentFrontExtractorTest(KaldiFrontExtractorTest):
    ...
    def create_pb_for_test_node(cls):
        ...
        pb += KaldiFrontExtractorTest.write_tag_with_value(
            "<Epsilon>", 0.00001, np.float32
        )
        pb += KaldiFrontExtractorTest.write_tag_with_value(
            "<TargetRms>", 0.5, np.float32
        )
        pb += KaldiFrontExtractorTest.write_tag_with_value(
            "<TestMode>", "F", np.string_
        )
