...
from openvino.tools.mo.pipeline.common import (
    determined_sort,
    get_fw_tensor_debug_info,
    get_sorted_outputs,
)
...
class TestOutputSort(unittest.TestCase):
    def test_get_sorted_outputs(self):
        ...
        self.assertListEqual(
            [node.id for node in get_sorted_outputs(graph)], ["Hd", "Ed", "Gd"]
        )
    ...
    def test_get_sorted_outputs_fine_situation(self):
        ...
        graph = build_graph_with_edge_attrs(nodes, edges)
        self.assertListEqual(
            [node.id for node in get_sorted_outputs(graph)], ["Ed", "Gd", "Hd"]
        )
