...
ngraph_needed = pytest.mark.skipif(
    not ngraph_available, reason="mock MO fe is not available"
)
...
class TestMainErrors(unittest.TestCase):
    ...
    @patch(
        "openvino.tools.mo.convert_impl.driver",
        side_effect=FrameworkError("FW ERROR MESSAGE"),
    )
    @ngraph_needed
