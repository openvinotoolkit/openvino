...
from unit_tests.utils.graph import build_graph_with_edge_attrs, regular_op, result
...
class TestGraphUtils(UnitTestWithMockedTelemetry):
    ...
    def test_sub_graph_between_nodes_branches_included(self):
        ...
        graph.add_edges_from(
            [(1, 2), (2, 3), (3, 4), (2, 5), (5, 6), (5, 7), (7, 8), (9, 5)]
        )
        self.assertListEqual(
            sorted(sub_graph_between_nodes(graph, [1], [4])), node_names
        )
        self.assertListEqual(
            sorted(sub_graph_between_nodes(graph, [1], [6])), node_names
        )
        self.assertListEqual(
            sorted(sub_graph_between_nodes(graph, [1], [8])), node_names
        )
        # all nodes except 4 because it is a child of end node
        self.assertListEqual(
            sorted(sub_graph_between_nodes(graph, [1], [3])),
        ...
        self.assertListEqual(
            sorted(sub_graph_between_nodes(graph, [2], [8])),
    ...
    def test_sub_graph_between_nodes_control_flow_included(self):
        ...
        graph.add_nodes_from(list(range(1, 7)))
        graph.add_edges_from(
            [(1, 2), (2, 3), (3, 4), (5, 2, {"control_flow_edge": True}), (6, 5)]
        )
        sub_graph_nodes = sub_graph_between_nodes(
            graph, [1], [4], include_control_flow=True
        )
        self.assertIsNotNone(sub_graph_nodes)
    ...
    def test_sub_graph_between_nodes_control_flow_not_included(self):
        ...
        sub_graph_nodes = sub_graph_between_nodes(
            graph, [1], [4], include_control_flow=False
        )
        self.assertIsNotNone(sub_graph_nodes)
    ...
    def test_sub_graph_between_nodes_control_flow_included_forward(self):
        ...
        graph.add_nodes_from(list(range(1, 7)))
        graph.add_edges_from(
            [(1, 2), (2, 3), (3, 4), (3, 5, {"control_flow_edge": True}), (5, 6)]
        )
        sub_graph_nodes = sub_graph_between_nodes(
            graph, [1], [4], include_control_flow=True
        )
    ...
    def test_backward_bfs_for_op_no_ops_detected(self):
        ...
        found_nodes = backward_bfs_for_operation(
            Node(graph, "result"), ["NonExistingOp"]
        )
    ...
    def test_backward_bfs_for_op_parallel_branch_op_detected(self):
        ...
        self.assertSetEqual(
            {found_nodes[0].id, found_nodes[1].id}, {"hsigmoid_2", "hsigmoid_4"}
        )
    ...
    def test_backward_bfs_for_op_parallel_branch_stop_op(self):
        ...
        found_nodes = backward_bfs_for_operation(
            Node(graph, "result"), ["HSigmoid"], ["ShapeOf"]
        )
        self.assertEqual(len(found_nodes), 1)
        self.assertEqual(found_nodes[0].id, "hsigmoid_2")
