...
pbtxt = (
    'node{name:"Placeholder"op:"Placeholder"attr{key:"dtype"value{type:DT_FLOAT}}attr{key:"shape"value{shape{dim'
...
class TestingSummarizeGraph(unittest.TestCase):
    def test_summarize_graph(self):
        with patch(
            "openvino.tools.mo.front.tf.loader.open", mock_open(read_data=pbtxt)
        ) as m:
            ...
            self.assertEqual(
                str(summary["inputs"]["Placeholder"]["shape"]), "(1,227,227,3)"
            )
