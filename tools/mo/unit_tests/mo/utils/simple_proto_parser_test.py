...
correct_proto_message_1 = (
    "model { faster_rcnn { num_classes: 90 image_resizer { keep_aspect_ratio_resizer {"
    " min_dimension: 600  max_dimension: 1024 }}}}"
)

correct_proto_message_2 = (
    "    first_stage_anchor_generator {grid_anchor_generator {height_stride: 16 width_stride:"
    " 16 scales: 0.25 scales: 0.5 scales: 1.0 scales: 2.0  aspect_ratios: 0.5 aspect_ratios:"
    " 1.0 aspect_ratios: 2.0}}"
)

correct_proto_message_3 = (
    "  initializer \n{variance_scaling_initializer \n{\nfactor: 1.0 uniform: true bla: false "
    "mode: FAN_AVG}}"
)

correct_proto_message_4 = (
...
correct_proto_message_9 = (
    "    first_stage_anchor_generator {grid_anchor_generator {height_stride: 16, width_stride:"
    " 16 scales: [ 0.25, 0.5, 1.0, 2.0], aspect_ratios: [] }}"
)

correct_proto_message_10 = (
    r'train_input_reader {label_map_path: "C:\mscoco_label_map.pbtxt"'
    ' tf_record_input_reader { input_path: "PATH_TO_BE_CONFIGURED/  mscoco_train.record" }}'
)
...
class TestingSimpleProtoParser(unittest.TestCase):
    ...
    def test_correct_proto_reader_from_string_with_special_characters_in_string(self):
        ...
        expected_result = {"model": {"path": r"C:\[{],}", "other_value": [1, 2, 3, 4]}}
        self.assertDictEqual(result, expected_result)
    ...
    @unittest.skip(
        "Temporary disabled since chmod() is temporary not working on Linux. (Windows do not support not writable dir at all)"
    )
