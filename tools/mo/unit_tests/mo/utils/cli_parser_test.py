...
import numpy
import numpy as np
from openvino.runtime import Dimension, Layout, PartialShape
...
class TestingMeanScaleGetter(UnitTestWithMockedTelemetry):
    ...
    def test_tuple_parser_no_inputs(self):
        ...
        result = parse_tuple_pairs(tuple_values)
        exp_res = [np.array([1.1, 22.22, 333.333]), np.array([2.2, 33.33, 444.444])]
    ...
    def test_mean_scale_no_input(self):
        ...
        result = get_mean_scale_dictionary(
            parse_tuple_pairs(mean_values), parse_tuple_pairs(scale_values), None
        )
        exp_res = {
            "data": {"mean": np.array([1.1, 22.22, 333.333]), "scale": None},
            "info": {"mean": None, "scale": np.array([1.1, 22.22, 333.333])},
        }
    ...
    def test_mean_only_input(self):
        mean_values = "data(1.1,22.22,333.333)"
        result = get_mean_scale_dictionary(
            parse_tuple_pairs(mean_values), parse_tuple_pairs(""), None
        )
        exp_res = {"data": {"mean": np.array([1.1, 22.22, 333.333]), "scale": None}}
        for input in exp_res.keys():
            for key in exp_res[input].keys():
                if type(exp_res[input][key]) is np.ndarray:
                    assert np.array_equal(exp_res[input][key], result[input][key])
    ...
    def test_scale_only_input(self):
        ...
        result = get_mean_scale_dictionary(
            parse_tuple_pairs(""), parse_tuple_pairs(scale_values), None
        )
        exp_res = {"data": {"mean": None, "scale": np.array([1.1, 22.22, 333.333])}}
        for input in exp_res.keys():
            for key in exp_res[input].keys():
                if type(exp_res[input][key]) is np.ndarray:
                    assert np.array_equal(exp_res[input][key], result[input][key])
    ...
    def test_2_scale_only_with_input(self):
        ...
        exp_res = {
            "data": {"mean": None, "scale": np.array([1.1, 22.22, 333.333])},
            "info": {"mean": None, "scale": np.array([1.2, 22.33, 333.444])},
        }
        for input in exp_res.keys():
            for key in exp_res[input].keys():
                if type(exp_res[input][key]) is np.ndarray:
    ...
    def test_mean_scale_diff_no_input(self):
        ...
        exp_res = [
            ...
            [None, np.array([1.1, 22.22, 333.333])],  # mean  # scale
        ]
        for i in range(len(exp_res)):
            for j in range(len(exp_res[i])):
                if type(exp_res[i][j]) is np.ndarray:
    ...
    def test_multi_mean_scale_input(self):
        ...
        result = get_mean_scale_dictionary(
            parse_tuple_pairs(mean_values), parse_tuple_pairs(scale_values), input_names
        )
        exp_res = {
            "data": {
                "mean": np.array([1.1, 22.22, 333.333]),
    ...
    def test_scale_do_not_match_input(self):
        ...
        self.assertRaises(
            Error, get_mean_scale_dictionary, mean_values, scale_values, "input1,input2"
        )
    ...
    def test_values_match_input_name(self):
        ...
        exp_res = {
            ...
            "multi-dotted.input.3.": np.array([255.0, 128.0, 64.0]),
        }
...
class TestShapesParsing(UnitTestWithMockedTelemetry):
    ...
    def test_get_shapes_several_inputs_several_shapes2(self):
        ...
        self.assertEqual(
            list(placeholder_values_res.keys()), list(placeholder_values_ref.keys())
        )
        self.assertEqual(inputs_list, ["inp1", "inp2"])
    ...
    def test_get_shapes_several_inputs_several_shapes3(self):
        # shapes and value for freezing specified using --input command line parameter
        argv_input = (
            "inp1[3 1]->[1.0 2.0 3.0],inp2[3 2 3],inp3[5]->[1.0 1.0 2.0 3.0 5.0]"
        )
    ...
    def test_get_shapes_several_inputs_several_shapes5(self):
        ...
        input_node_names_ref = "inp1,inp2,inp3"
        self.assertEqual(
            sorted(list(placeholder_values_res.keys())),
    ...
    def test_get_shapes_several_inputs_several_shapes6(self):
        ...
        inputs_list, result, _ = get_placeholder_shapes(argv_input, None)
        exp_res = {
            "inp1": PartialShape([3, 1]),
            "inp2": PartialShape([3, 2, 3]),
            "inp3": None,
        }
        self.assertEqual(list(exp_res.keys()), list(result.keys()))
        self.assertEqual(inputs_list, ["inp1", "inp2", "inp3"])
    ...
    def test_get_shapes_and_data_types1(self):
        ...
        input_list, result_shapes, result_data_types = get_placeholder_shapes(
            argv_input, ""
        )
        ref_result_shapes = {
            "inp1": np.array([3, 1]),
        ...
        self.assertEqual(
            list(ref_result_data_types.keys()), list(result_data_types.keys())
        )
        self.assertEqual(input_list, ["inp1", "inp2", "inp3"])
    ...
    def test_get_shapes_and_data_types_shape_with_ports_only(self):
        ...
        self.assertEqual(
            input_list, ["placeholder1:4", "placeholder2", "2:placeholder3"]
        )
        for i in ref_result_data_types.keys():
            np.testing.assert_equal(result_data_types[i], ref_result_data_types[i])
    ...
    def test_get_shapes_several_inputs_several_partial_shapes(self):
        ...
        inputs_list, result, _ = get_placeholder_shapes(argv_input, input_shapes)
        exp_res = {
            "inp1": PartialShape([1, Dimension(0, 22), Dimension(1, 100), -1]),
            "inp2": PartialShape([-1, Dimension(45, -1), 7, 1]),
        }
        self.assertEqual(list(exp_res.keys()), list(result.keys()))
        self.assertEqual(inputs_list, ["inp1", "inp2"])
        for i in exp_res.keys():
    ...
    def test_get_shapes_several_inputs_several_partial_shapes2(self):
        ...
        inputs_list, result, _ = get_placeholder_shapes(argv_input, None)
        exp_res = {
            "inp1": PartialShape([1, -1, (50, 100), 123]),
    ...
    def test_get_shapes_several_inputs_several_partial_shapes3(self):
        ...
        exp_res = {
            "inp1": (3, 1),
            "inp2": PartialShape(
                [Dimension(3, -1), Dimension(0, 2), Dimension(5, 10), -1, -1]
            ),
            "inp3": (5,),
        }
        self.assertEqual(list(exp_res.keys()), list(result.keys()))
    ...
    def test_get_shapes_several_inputs_several_partial_shapes5(self):
        ...
        exp_res = {
            "inp1": PartialShape([3, 1]),
            "inp2": PartialShape(
                [(3, np.iinfo(np.int64).max), (0, 2), (5, 10), -1, -1]
            ),
            "inp3": PartialShape([5]),
        }
        self.assertEqual(list(exp_res.keys()), list(result.keys()))
    ...
    def test_get_shapes_and_data_types_partial_shape_with_input_port(self):
        ...
        ref_result_shapes = {
            "inp1:1": PartialShape([3, 1]),
            "0:inp2": PartialShape(
                [Dimension(3, -1), Dimension(-1, 2), Dimension(5, 10), -1, -1]
            ),
            "inp3:4": np.array([5]),
        }
    ...
    def test_get_shapes_and_data_types_partial_shape_with_input_port_comma_separator(
        self,
    ):
        argv_input = "inp1:1[3,1]->[1.0 2.0 3.0],0:inp2[ 3.. ,..2, 5..10, ?,-1]{i32},inp3:4[5]{f32}->[1.0 1.0 2.0 3.0 5.0]"
        input_list, result_shapes, result_data_types = get_placeholder_shapes(
    ...
    def test_get_shapes_and_data_types_partial_shape_with_output_port_comma_separator(
        self,
    ):
        argv_input = "inp1:1[3,1]->[1.0 2.0 3.0],inp2:3[3..,..2,5..10,?,-1]{i32},inp3:4[5]{f32}->[1.0 1.0 2.0 3.0 5.0]"
        input_list, result_shapes, result_data_types = get_placeholder_shapes(
...
class PathCheckerFunctions(unittest.TestCase):
    ...
    @unittest.skipIf(
        ...
        "chmod() on Windows do nor support not writable dir",
    )
    @unittest.skipIf(
        sys.platform.startswith("lin") and os.geteuid() == 0,
        "root user does not support not writable dir",
    )
    ...
    def test_multiple_writable_dirs(self):
        dirs_str = ",".join(
            [__class__.WRITABLE_DIR, __class__.WRITABLE_NON_EXISTING_DIR]
        )
        self.assertEqual(dirs_str, writable_dir(dirs_str))

    def test_single_writable_non_existing_dir(self):
        self.assertEqual(
            __class__.WRITABLE_NON_EXISTING_DIR,
            writable_dir(__class__.WRITABLE_NON_EXISTING_DIR),
        )
    ...
    def test_readable_file(self):
        self.assertEqual(
            __class__.EXISTING_FILE, readable_file(__class__.EXISTING_FILE)
        )
...
class TransformChecker(unittest.TestCase):
    ...
    def test_single_pass_with_multiple_args(self):
        self.assertEqual(
            parse_transform("LowLatency2[use_const_initializer=True;dummy_attr=3.14]"),
    ...
    @patch(
        "openvino.tools.mo.back.offline_transformations.get_available_transformations"
    )
...
class TestLayoutParsing(unittest.TestCase):
    def test_get_layout_1(self):
        ...
        exp_res = {
            "name1": {"source_layout": "[n,h,w,c]", "target_layout": None},
    ...
    def test_get_layout_2(self):
        ...
        result = get_layout_values(argv_layout)
        exp_res = {
            "name1": {"source_layout": "nhwc", "target_layout": None},
    ...
    def test_get_layout_3(self):
        ...
        result = get_layout_values(argv_layout)
        exp_res = {
            "name1": {"source_layout": "n...c", "target_layout": None},
    ...
    def test_get_layout_8(self):
        ...
        result = get_layout_values(argv_layout)
        exp_res = {
            "name1-0": {"source_layout": "n...c", "target_layout": None},
    ...
    def test_get_layout_target_layout_3(self):
        ...
        result = get_layout_values(argv_target_layout=argv_target_layout)
        exp_res = {
            "name1": {"source_layout": None, "target_layout": "nhwc"},
    ...
    def test_get_layout_target_layout_4(self):
        ...
        result = get_layout_values(argv_target_layout=argv_target_layout)
        exp_res = {
            "name1": {"source_layout": None, "target_layout": "[n,h,w,c]"},
    ...
    def test_get_layout_source_target_layout_1(self):
        ...
        result = get_layout_values(
            argv_source_layout=argv_source_layout, argv_target_layout=argv_target_layout
        )
        exp_res = {"": {"source_layout": "[n,h,w,c]", "target_layout": "[n,c,h,w]"}}
        self.assertEqual(list(exp_res.keys()), list(result.keys()))
        for i in exp_res.keys():
    ...
    def test_get_layout_source_target_layout_3(self):
        ...
        exp_res = {
            "name1": {"source_layout": "nhwc", "target_layout": "nchw"},
            "name2": {"source_layout": "nhwc", "target_layout": "nchw"},
        }
        self.assertEqual(list(exp_res.keys()), list(result.keys()))
        for i in exp_res.keys():
    ...
    def test_get_layout_source_target_layout_4(self):
        ...
        exp_res = {
            "name1": {"source_layout": "[n,h,w,c]", "target_layout": "[n,c,h,w]"},
            "name2": {"source_layout": "[n,h,w,c]", "target_layout": "[n,c,h,w]"},
        }
        self.assertEqual(list(exp_res.keys()), list(result.keys()))
        for i in exp_res.keys():
    ...
    def test_get_layout_source_target_layout_6(self):
        ...
        exp_res = {
            "name1.0:a/b": {"source_layout": "nhwc", "target_layout": "nchw"},
    ...
    def test_get_layout_source_target_layout_7(self):
        ...
        exp_res = {
            "name1-0": {"source_layout": "[n,h,w,c]", "target_layout": "nchw"},
    ...
    def test_get_layout_raises_if_layout_and_source_layout_provided(self):
        ...
        with self.assertRaises(Error):
            get_layout_values(
                argv_layout=argv_layout, argv_source_layout=argv_source_layout
            )
    ...
    def test_get_layout_raises_if_layout_and_target_layout_provided(self):
        ...
        with self.assertRaises(Error):
            get_layout_values(
                argv_layout=argv_layout, argv_target_layout=argv_target_layout
            )
    ...
    def test_get_layout_raises_if_layout_with_source_and_target_layout_provided(self):
        ...
        with self.assertRaises(Error):
            get_layout_values(
                argv_layout=argv_layout,
                argv_source_layout=argv_source_layout,
...
class TestLayoutParsingEmptyNames(unittest.TestCase):
    def test_get_layout_1(self):
        ...
        result = get_layout_values(argv_layout)
        exp_res = [
            {"source_layout": "[n,h,w,c]", "target_layout": None},
    ...
    def test_get_layout_2(self):
        ...
        result = get_layout_values(argv_layout)
        exp_res = [
            {"source_layout": "nhwc", "target_layout": None},
    ...
    def test_get_layout_3(self):
        ...
        result = get_layout_values(argv_layout)
        exp_res = [
            {"source_layout": "n...c", "target_layout": None},
    ...
    def test_get_layout_target_layout_3(self):
        ...
        result = get_layout_values(argv_target_layout=argv_target_layout)
        exp_res = [
            {"source_layout": None, "target_layout": "nhwc"},
    ...
    def test_get_layout_target_layout_4(self):
        ...
        result = get_layout_values(argv_target_layout=argv_target_layout)
        exp_res = [
            {"source_layout": None, "target_layout": "[n,h,w,c]"},
...
class TestPackParamsToArgsNamespace(unittest.TestCase):
    def test_mo_convert_params(self):
        ...
        assert argv.input == [
            "name",
            InputCutInfo("a", [1, 2, 3], numpy.float32, [5, 6, 7]),
        ]
    ...
    def test_not_existing_dir(self):
        ...
        with self.assertRaisesRegex(
            Error, 'The "abc" is not existing file or directory'
        ):
