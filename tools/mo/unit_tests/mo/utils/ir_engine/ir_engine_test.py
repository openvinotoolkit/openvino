...
class TestFunction(unittest.TestCase):
    def setUp(self):
        ...
        self.assertTrue(
            os.path.exists(self.xml), "XML file not found: {}".format(self.xml)
        )
        self.assertTrue(
            os.path.exists(self.bin), "BIN file not found: {}".format(self.bin)
        )
        ...
        self.IR_negative = IREngine(
            path_to_xml=str(self.xml_negative), path_to_bin=str(self.bin)
        )
    ...
    def test_is_float(self, test_data, result):
        test_data = test_data
        self.assertEqual(
            IREngine._IREngine__isfloat(test_data),
            result,
            "Function __isfloat is not working with value: {}".format(test_data),
        )
        log.info(
            "Test for function __is_float passed with value: {}, expected result: {}".format(
                test_data, result
            )
        )
    ...
    def test_compare_negative(self):
        ...
        reference_msg = (
            'Current node "2" with type "Const" and reference node "2" with type "Input" have different '
    ...
    def test_get_inputs(self):
        ...
        self.assertTrue(
            strict_compare_tensors(ref_input_dict["data"], inputs_dict["data"]),
    ...
    def test_eq_function(self):
        self.assertTrue(
            self.IR == self.IR_ref, "Comparing false, test eq function failed"
        )
    ...
    def test_load_bin_hashes(self):
        ...
        for ti in ti_nodes:
            if not ti.has_valid("body"):
                log.error(
                    "TensorIterator doesn't have body attribute for node: {}".format(
                        ti.name
                    )
                )
            else:
                ...
                for node in const_ti_nodes:
                    if not node.has_valid("hashes"):
                        log.error(
                            "Constant node {} do not include hashes".format(node.name)
                        )
                        is_ok = False
