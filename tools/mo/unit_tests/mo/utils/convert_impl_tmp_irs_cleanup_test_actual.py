...
class TestConvertImplTmpIrsCleanup(unittest.TestCase):
    ...
    def test_tmp_irs_cleanup_convert_impl_1(self):
        with patch(
            "openvino.tools.mo.back.offline_transformations.apply_offline_transformations"
        ) as emit_ir_func:
            emit_ir_func.side_effect = Error("offline transformations step has failed")

            params = {
                "input_model": self.test_model_file,
            ...
            self.assertRaisesRegex(
                Error,
                "offline transformations step has failed",
                convert_model,
                **params
            )
    ...
    def test_tmp_irs_cleanup_convert_impl_2(self):
        with patch(
            "openvino.tools.mo.back.ie_ir_ver_2.emitter.add_net_rt_info"
        ) as emit_ir_func:
            ...
            self.assertRaisesRegex(
                Error, "emitting tmp IR has failed", convert_model, **params
            )
            self.assertFalse(self.are_tmp_files_left(self.test_model_file))
    ...
    def test_tmp_irs_cleanup_convert_impl_3(self):
        with patch("openvino.tools.mo.convert_impl.read_model") as emit_ir_func:
            ...
            self.assertRaisesRegex(
                Error, "FEM read_model has failed", convert_model, **params
            )
            self.assertFalse(self.are_tmp_files_left(self.test_model_file))
