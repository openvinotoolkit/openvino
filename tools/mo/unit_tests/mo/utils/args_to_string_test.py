...
import numpy as np
from openvino.runtime import Dimension, Layout
...
class TestConvertingConvertArgumentsToString(UnitTestWithMockedTelemetry):
    def test_mean_scale_value_to_str(self):
        ...
        values = {
            "input1": [0.5, 1.3, 0.67],
            "input2": [4.2, 6.7, 3.15],
            "input3": [0.757, 4.6, 7.3],
        }
        self.assertTrue(
            mean_scale_value_to_str(values)
        ...
        self.assertRaises(
            Exception,
            mean_scale_value_to_str,
            **{"value": {("a", "b"): [0.5, 1.3, 0.67]}}
        )
        self.assertRaises(
            Exception, mean_scale_value_to_str, **{"value": {"name": Dimension(1)}}
        )
    ...
    def test_transform_param_to_str(self):
        ...
        self.assertTrue(
            transform_param_to_str(transform1)
        ...
        self.assertTrue(
            transform_param_to_str(transform2)
        ...
        self.assertTrue(
            transform_param_to_str(transform)
            == "LowLatency2[use_const_initializer=False],"
        ...
        self.assertRaises(
            Exception,
            transform_param_to_str,
            **{"value": ("LowLatency2", {"use_const_initializer": False}, "param")}
        )
        self.assertRaises(
            Exception, transform_param_to_str, **{"value": (("a", "b"), {})}
        )
        self.assertRaises(
            Exception,
            transform_param_to_str,
            **{"value": ("LowLatency2", Dimension(1))}
        )
        self.assertRaises(
            Exception,
            transform_param_to_str,
            **{"value": ("LowLatency2", {("a", "b"): False})}
        )
    ...
    def test_source_target_layout_to_str(self):
        ...
        self.assertTrue(
            source_target_layout_to_str(layout)
            == "input1([N,H,W,C]),input2([N,?,?]),input3(nchw)"
        )

        self.assertRaises(
            Exception, source_target_layout_to_str, **{"value": {"op": Dimension(1)}}
        )
        self.assertRaises(
            Exception,
            source_target_layout_to_str,
            **{"value": {("a", "b"): Layout("nhwc")}}
        )
        self.assertRaises(
            Exception, source_target_layout_to_str, **{"value": Dimension(1)}
        )

    def test_layout_param_to_str_to_str(self):
        layout = {"input1": Layout("nhwc"), "input2": Layout("n??"), "input3": "nchw"}
        self.assertTrue(
            layout_param_to_str(layout)
            == "input1([N,H,W,C]),input2([N,?,?]),input3(nchw)"
        )
        ...
        self.assertTrue(
            layout_param_to_str(layout) == "input1([N,?,?]),input2([N,H,W,C]->nchw),"
            "input3(abc->cab),input4([N,H,W,C]),input5(n?)"
        )

        self.assertRaises(
            Exception, layout_param_to_str, **{"value": {"op": Dimension(1)}}
        )
        self.assertRaises(
            Exception, layout_param_to_str, **{"value": {("a", "b"): Layout("nhwc")}}
        )
