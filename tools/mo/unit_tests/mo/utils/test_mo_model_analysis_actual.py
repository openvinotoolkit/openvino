...
def base_args_config():
    ...
    args.frontend_defaults = {"onnx": "legacy", "tf": "legacy"}
    return args
...
class TestMoFallback(unittest.TestCase):
    def setUp(self):
        ...
        mul = onnx.helper.make_node(
            "Mul", inputs=["add_out", "in3"], outputs=["mul_out"]
        )
        graph_2 = make_graph(
            [add, mul], "test_graph_2", input_tensors_2, output_tensors_2
        )
        model_2 = make_model(
            graph_2,
            producer_name="MO tests",
        ...
        split_1 = onnx.helper.make_node(
            "Split", inputs=["add_out"], outputs=["out1", "out2"], axis=0
        )
        split_2 = onnx.helper.make_node(
            "Split", inputs=["mul_out"], outputs=["out3", "out4"], axis=0
        )
        output_tensors_3 = [
    ...
    def test_model(self, json_print):
        ...
        assert result["inputs"] == json.loads(
            '{"in1": {"shape": [1, 2], "data_type": "float32", "value": "None"}, \
                                                "in2": {"shape": [1, 2], "data_type": "float32", "value": "None"}}'
        )

        assert "intermediate" in result
        assert result["intermediate"] == json.loads(
            '{"in1": {"shape": [1, 2], "data_type": "float32", "value": "None"}, \
                                                      "in2": {"shape": [1, 2], "data_type": "float32", "value": "None"}, \
    ...
    def test_model_with_dyn_shapes(self, json_print):
        ...
        print(result["inputs"])
        assert result["inputs"] == json.loads(
            '{"in1": {"shape": [1, 0, 3], "data_type": "int64", "value": "None"}, \
