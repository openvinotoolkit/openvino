...
full_name_patterns_to_skip = [
    "^openvino/tools/mo/utils/convert.py$",
    "^openvino/tools/mo/front/caffe/CustomLayersMapping.xml$",
]
if platform.system() == "Windows":
    full_name_patterns_to_skip = [
        i.replace("/", "\\\\") for i in full_name_patterns_to_skip
    ]
...
class TestBOMFile(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with open(
            os.path.join(mo_path, "automation", "package_BOM.txt"), "r"
        ) as bom_file:
            if platform.system() == "Windows":
                cls.existing_files = [
                    name.rstrip().replace("/", "\\") for name in bom_file.readlines()
                ]
            else:
        ...
        cls.prefix = os.path.normpath(
            get_mo_root_dir() + "../../../"
        )  # prefix which is used in BOM file
        cls.expected_header = [
            re.compile(pattern)
            for pattern in [
    ...
    def test_bom_file(self):
        ...
        if len(missing_files) != 0:
            ...
            for f in missing_files:
                print(f.replace("\\", "/"))
        self.assertTrue(
            not len(missing_files), "{} files missed in BOM".format(len(missing_files))
        )
    ...
    def test_bom_does_not_contain_unittest_files(self):
        for file_name in self.existing_files:
            self.assertFalse(
                ...
                "BOM file contains test file {}".format(file_name),
            )
    ...
    def test_deleted_files_still_stored_in_bom(self):
        ...
        self.assertTrue(
            not len(deleted),
            "{} files deleted but still stored in BOM".format(len(deleted)),
        )

    def test_alphabetical_order_and_duplicates(self):
        sorted_bom = sorted(
            [x for x in self.existing_files if self.existing_files.count(x) == 1],
            key=str.lower,
        )
            ...
            for root, dirs, files in os.walk(src_dir):
                ...
                for f in files:
                    ...
                    with open(full_name, "r") as source_f:
                        ...
                        s = [
                            x.strip()
                            for x in islice(source_f, len(self.expected_header) + 2)
                        ]
                        ...
                        try:
                            ...
                            for str_ind in range(0, len(self.expected_header)):
                                if not re.match(
                                    self.expected_header[str_ind], s[str_ind]
                                ):
                                    missing_files.append(full_name)
                                    break
                        except:
