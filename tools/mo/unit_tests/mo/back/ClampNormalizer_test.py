...
class AttributedClampNormalizerTests(unittest.TestCase):
    ...
    def test_2_inputs(self):
        nodes = {
            **regular_op_with_shaped_data(
                "placeholder", [1, 3, 20, 20], {"type": "Parameter"}
            ),
            **regular_op_with_shaped_data(
                "a_clamp", [1, 3, 20, 20], {"type": None, "op": "Clamp"}
            ),
            **regular_op_with_shaped_data(
                "clamp",
                [1, 3, 20, 20],
        ...
        ref_graph = build_graph(
            nodes, [*connect("placeholder", "0:clamp"), *connect("clamp", "result")]
        )
    ...
    def test_all_dynamic_inputs(self):
        nodes = {
            ...
            **regular_op_with_shaped_data(
                "maximum", [1, 3, 20, 20], {"type": "Maximum", "op": "Maximum"}
            ),
            **regular_op_with_shaped_data(
                "minimum", [1, 3, 20, 20], {"type": "Minimum", "op": "Minimum"}
            ),
            **result("result"),
        }
        edges = [
