...
from openvino.tools.mo.back.ShuffleChannelPatternOptimization import (
    DepthToSpaceFusion,
    ShuffleChannelFusion,
)
...
class TestShuffleChannelFusionTest:
    ...
    def get_graphs(input_shape, reshape_0_pattern, order, reshape_1_pattern, group):
        nodes = {
            ...
            **valued_const_with_data(
                "reshape_0_pattern", int64_array(reshape_0_pattern)
            ),
            **regular_op_with_empty_data(
                "reshape_0", {"type": "Reshape", "infer": Reshape.infer}
            ),
            **valued_const_with_data("order", int64_array(order)),
            **regular_op_with_empty_data(
                "transpose", {"type": "Transpose", "infer": Transpose.infer}
            ),
            **valued_const_with_data(
                "reshape_1_pattern", int64_array(reshape_1_pattern)
            ),
            **regular_op_with_empty_data(
                "reshape_1",
                {"type": "Reshape", "infer": Reshape.infer, "name": "final_reshape"},
            ),
            **result(),
        }
        edges = [
    ...
    @pytest.mark.parametrize(
        "input_shape, reshape_0_pattern, order, reshape_1_pattern, group",
        [
    ...
    def test_fusion(
        self, input_shape, reshape_0_pattern, order, reshape_1_pattern, group
    ):
        graph, graph_ref = self.get_graphs(
            input_shape, reshape_0_pattern, order, reshape_1_pattern, group
        )
        ...
        assert flag, resp
        assert (
            len(graph.get_op_nodes(name="final_reshape")) == 1
            and graph.get_op_nodes(name="final_reshape")[0].op == "ShuffleChannels"
        )
...
class TestDepthToSpaceFusionTest:
    @staticmethod
    def get_graphs(
        input_shape, reshape_0_pattern, order, reshape_1_pattern, block_size
    ):
        nodes = {
            **regular_op_with_shaped_data(
                "input",
    ...
    def test_fusion(
        self, input_shape, reshape_0_pattern, order, reshape_1_pattern, block_size
    ):
        graph, graph_ref = self.get_graphs(
            input_shape, reshape_0_pattern, order, reshape_1_pattern, block_size
        )
        ...
        assert (
            len(graph.get_op_nodes(name="final_reshape")) == 1
            and graph.get_op_nodes(name="final_reshape")[0].op == "DepthToSpace"
        )
