...
from openvino.tools.mo.back.ChangeRandomUniformOutputType import (
    ChangeRandomUniformOutputType,
)
...
nodes = {
    ...
    **regular_op_with_shaped_data(
        "random_uniform", [3, 4, 5], {"type": "RandomUniform", "op": "RandomUniform"}
    ),
...
class TestChangeRandomUniformOutputType:
    ...
    def test_change_random_uniform_output_type(self, ir_type, out_type, dst_type):
        ...
        graph_ref = build_graph(
            nodes,
            edges if dst_type is None else edges_with_convert,
            {},
        ...
        if dst_type is not None:
            convert_node = (
                Node(graph, "random_uniform").out_port(0).get_destination().node
            )
