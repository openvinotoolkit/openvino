...
class FakeOutputResolverTest(unittest.TestCase):
    def test_one(self):
        nodes = {
            ...
            **regular_op_with_empty_data(
                "some_op", {"type": "SomeOp", "name": "some_op_name"}
            ),
            **regular_op_with_empty_data(
                "fake_output",
                {
            ...
            **regular_op_with_empty_data(
                "add",
                {"type": None, "kind": "op", "op": "Add", "name": "my_output_name"},
            ),
            **result("result"),
        }
        edges = [
    ...
    def test_multi(self):
        nodes = {
            ...
            **regular_op_with_empty_data(
                "add1",
                {"type": None, "kind": "op", "op": "Add", "name": "my_output_name1"},
            ),
            **regular_op_with_empty_data(
                "add2",
                {"type": None, "kind": "op", "op": "Add", "name": "my_output_name2"},
            ),
            **result("result1"),
            **result("result2"),
        }
