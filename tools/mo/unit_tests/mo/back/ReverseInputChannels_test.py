...
from openvino.tools.mo.back.ReverseInputChannels import (
    ReverseChannelsPropagationDown,
    ReverseChannelsPropagationUp,
)
...
nodes = {
    **regular_op_with_shaped_data(
        "placeholder1", [1, 3, 10, 10], {"type": "Parameter", "rt_info": RTInfo()}
    ),
...
def get_nodes(shape, axis=1):
    return {
        ...
        **regular_op_with_shaped_data(
            "placeholder2", [1, 1, 1, 1], {"type": "Parameter", "shape": [1, 1, 1, 1]}
        ),
...
class ReverseInputChannelsTest(unittest.TestCase):
    ...
    def test_lift_up_through_eltwise(self):
        graph = build_graph(
            nodes,
            [
                *connect("placeholder1", "0:mul"),
                *connect("placeholder2", "1:mul"),
    ...
    def test_lift_up_through_pad(self):
        ...
        keep_moving_up, new_reverses = (
            ReverseChannelsPropagationUp.lift_up_through_zero_port_only(
    ...
    def test_pass_rc_through(self):
        ...
        ReverseChannelsPropagationDown.pass_rc_through_zero_port_only(
            node, reverse_channels
        )
        self.check_graph_attrs(graph, ["placeholder"])
    ...
    def test_lift_up_through_transpose(self):
        ...
        keep_moving_up, new_reverses = (
            ReverseChannelsPropagationUp.lift_up_through_transpose(
                node, reverse_channels
            )
        )
        self.assertTrue(keep_moving_up is True)
