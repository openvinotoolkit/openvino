...
from openvino.tools.mo.back.compress_quantized_weights import (
...
def nodes_dict(
    ...
    data = (
        np.ones(shape, dtype=original)
        if data is None
        else np.array(data, dtype=original)
    )
    if int_data is None:
...
class CompressionQuantizeDequantizeSeparateTest(unittest.TestCase):
    def test_quantize(self):
        ...
        self.assertEqual(
            len(fq_nodes), 1, error_message.format("before", len(fq_nodes))
        )
        fake_quantize = fq_nodes[0]

        CompressQuantizeWeights.quantize_data(
            fake_quantize, original_type, np.int8, "signed"
        )
        graph.clean_up()
...
class TestCompressionDataTypeTest:
    @pytest.mark.parametrize(
        "original", [np.int64, np.int32, np.float64, np.float32, np.float16]
    )
    ...
    def test_fp16_fake_quantize(self):
        original_type = np.float16
        input_low = np.array(
            [-0.59033203125, -1.4833984375, -1.2900390625], dtype=np.float16
        )
        input_high = np.array(
            [0.59033203125, 1.4833984375, 1.2900390625], dtype=np.float16
        )
        output_low = np.array(
            [0.295166015625, 0.74169921875, 0.64501953125], dtype=np.float16
        )
        output_high = np.array(
            [-0.295166015625, -0.74169921875, -0.64501953125], dtype=np.float16
        )
        ...
        nodes = nodes_dict(
            ...
            transformed=np.int8,
            levels=255,
            data=np.array([0.2, 1.2, 1.2], dtype=np.float16),
            il=input_low,
            ih=input_high,
            ol=output_low,
            oh=output_high,
        ...
        error_message = "Unexpected number of {} nodes {} CompressQuantizeWeights.dequantize_data call `{}`"
        fq_nodes = graph.get_op_nodes(type="FakeQuantize")
        assert len(fq_nodes) == 1, error_message.format(
            "FakeQuantize", "before", len(fq_nodes)
        )
        ...
        fq_nodes = graph.get_op_nodes(type="FakeQuantize")
        assert len(fq_nodes) == 0, error_message.format(
            "FakeQuantize", "after", len(fq_nodes)
        )
...
class TestAccuracyCheckFP32Test:
    ...
    def test_accuracy(self, data, in_low, in_high, out_low, out_high, levels, add_cast):
        if not add_cast:
            nodes = nodes_dict(
                np.float32, None, levels, data, in_low, in_high, out_low, out_high
            )
            graph = build_graph(
                nodes,
                [
            ...
            nodes = nodes_dict(
                np.float16, None, levels, data, in_low, in_high, out_low, out_high
            )
            graph = build_graph(
                nodes,
                [
        ...
        const_result_graph = build_graph(
            {**shaped_const_with_data("weights", np.array(data).shape), **result()},
            [*connect("weights", "output")],
            nodes_with_edges_only=True,
        )
        (flag, resp) = compare_graphs(
            graph, const_result_graph, "output", check_op_attrs=True
        )
        assert flag, resp

        (flag, resp) = compare_graphs(
            graph_ref, const_result_graph, "output", check_op_attrs=True
        )
        assert flag, resp
...
class TestZeroPointOptimizerTestClass:
    @pytest.mark.parametrize(
        "weights, zero_point, adj_weights, adj_zero_point",
        [
            ([-10, 7], [-1], [-9, 8], [0]),
            ([-10, 7], [-0.99999999], [-9, 8], [0]),
        ],
    )
    def test_zero_point_optimization(
        self, weights, zero_point, adj_weights, adj_zero_point
    ):
        nodes = lambda w, zp: {
        ...
        graph = build_graph(
            nodes(weights, zero_point), edges, nodes_with_edges_only=True
        )
        ZeroPointOptimizer().find_and_replace_pattern(graph)
        graph.clean_up()

        graph_ref = build_graph(
            nodes(adj_weights, adj_zero_point),
            [
                *connect("weights:0", "0:cast"),
    ...
    def test_negative_zero_point_optimization(
        self, weights, zero_point, adj_weights, adj_zero_point
    ):
        nodes = lambda w, zp: {
            **valued_const_with_data("weights", np.array(w, dtype=np.int8)),
        ...
        graph_ref = build_graph(
            nodes(adj_weights, adj_zero_point), edges, nodes_with_edges_only=True
        )
        graph_ref.clean_up()
