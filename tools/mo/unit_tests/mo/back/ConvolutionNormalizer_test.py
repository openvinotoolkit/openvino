...
from openvino.tools.mo.back.ConvolutionNormalizer import (
    PullReshapeThroughFQ,
    V7ConvolutionWithGroupsResolver,
...
def graph_template(
    weights_initial_shape,
    new_reshape_shape,
    limits_initial_shape,
    limits_new_shape=None,
):
    limits_new_shape = (
        limits_initial_shape if limits_new_shape is None else limits_new_shape
    )
    ...
    core_nodes = lambda weights_shape, limit_shape, reshape_shape: {
        **regular_op_with_shaped_data(
            "input", None, {"type": "Parameter", "op": "Parameter"}
        ),
        ...
        **regular_op_with_shaped_data(
            "reshape",
            reshape_shape,
            {"type": "Reshape", "infer": Reshape.infer, "op": "Reshape"},
        ),
        ...
        **regular_op_with_shaped_data(
            "convolution", None, {"type": "Convolution", "op": "Convolution"}
        ),
        **result(),
    }

    nodes_before = core_nodes(
        weights_initial_shape, limits_initial_shape, new_reshape_shape
    )
    edges_before = [
    ...
    graph = build_graph(
        nodes_attrs=nodes_before, edges=edges_before, nodes_with_edges_only=True
    )
    ...
    graph_ref = build_graph(
        nodes_attrs=nodes_after, edges=edges_after, nodes_with_edges_only=True
    )
...
class TestPullReshapeThroughFQ(unittest.TestCase):
    ...
    def test_v7_weights_reshape(self):
        ...
        (flag, resp) = compare_graphs(
            graph, graph_ref, last_node="output", check_op_attrs=True
        )
...
class TestV7ConvolutionWithGroupsResolver(unittest.TestCase):
    def test_v7_group_convolution_resolver(self):
        nodes = {
            **regular_op_with_shaped_data(
                "input", [1, 3, 224, 224], {"type": "Parameter"}
            ),
            ...
            **regular_op_with_shaped_data(
                "convolution", None, {"type": "Convolution", "group": 3, "output": 24}
            ),
            **result(),
        }
        graph = build_graph(
    ...
    def test_v7_group_convolution_resolver_depthwise_conv2d(self):
        nodes = {
            **regular_op_with_shaped_data(
                "input", [1, 1, 224, 224], {"type": "Parameter"}
            ),
