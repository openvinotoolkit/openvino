...
def create_function2(
    shape1=[2, 2], shape2=[2, 2], dtype1=np.float32, dtype2=np.float32
):
    ...
    function = Model(
        results=[res1, res2], parameters=[input1, input2], name="TestFunction"
    )
    return function
...
class TestPreprocessingMOC(UnitTestWithMockedTelemetry):
    ...
    def test_scale_vector(self):
        argv = Namespace(
            mean_scale_values={"input1": {"scale": np.array([4.0]), "mean": None}},
            scale=None,
        )
        function = create_function2()
        process_function(ov_function=function, argv=argv)
        op_node = list(function.get_parameters()[0].output(0).get_target_inputs())[
            0
        ].get_node()
        self.assertTrue(
            op_node.get_type_name() == "Divide" or op_node.get_type_name() == "Multiply"
        )
        self.check_scale_constant(op_node, [4.0], shape=None)
        # Verify that input2 is not affected
        op_node = list(function.get_parameters()[1].output(0).get_target_inputs())[
            0
        ].get_node()
    ...
    def test_scale_vector3(self):
        argv = Namespace(
            mean_scale_values={
                "input1": {"scale": np.array([2.0, 4.0, 8.0]), "mean": None}
            },
            scale=None,
        )
        ...
        self.check_scale_constant(op_node, expected=[2.0, 4.0, 8.0], shape=[1, 3, 1, 1])
    ...
    def test_scale_vector4_layout(self):
        argv = Namespace(
            mean_scale_values={
                "input1": {"scale": np.array([2.0, 4.0, 8.0, 9.0]), "mean": None}
            },
        ...
        function = create_function2(
            shape1=[1, 3, 3, 4]
        )  # Use layout to determine channels dim
        ...
        self.check_scale_constant(
            op_node, expected=[2.0, 4.0, 8.0, 9.0], shape=[1, 1, 1, 4]
        )
    ...
    def test_mean_single(self):
        argv = Namespace(
            mean_scale_values={"input1": {"mean": np.array([4.0]), "scale": None}},
            scale=None,
        )
        function = create_function2()
        process_function(ov_function=function, argv=argv)
        ...
        self.assertTrue(
            op_node.get_type_name() == "Subtract" or op_node.get_type_name() == "Add"
        )
    ...
    def test_mean_vector3(self):
        argv = Namespace(
            mean_scale_values={
                "input2": {"mean": np.array([2.0, 4.0, 8.0]), "scale": None}
            },
            scale=None,
        )
        ...
        self.check_mean_constant(op_node, expected=[2.0, 4.0, 8.0], shape=[1, 3, 1, 1])
    ...
    def test_mean_scale(self):
        ...
        self.check_mean_constant(op_node, expected=[1.0, 2.0, 3.0], shape=[1, 3, 1, 1])
    ...
    def test_mean_scale_with_layout(self):
        argv = Namespace(
            mean_scale_values={
                "input2a": {
                    ...
                    "scale": np.array([2.0, 4.0, 8.0, 9.0]),
                }
            },
            scale=None,
        )
        ...
        self.check_mean_constant(
            op_node, expected=[1.0, 2.0, 3.0, 4.0], shape=[1, 1, 1, 4]
        )
    ...
    def test_no_param_name(self):
        ...
        self.check_scale_constant(op_node, expected=[2.0, 4.0, 6.0], shape=[1, 3, 1, 1])
        ...
        self.check_mean_constant(op_node, expected=[7.0, 8.0, 9.0], shape=[1, 1, 1, 3])
    ...
    def test_no_param_name_single_value(self):
        argv = Namespace(
            mean_scale_values=list(
                np.array(
                    [
                        (np.array([1.0]), None),
                        (np.array([2.0, 3.0, 4.0]), np.array([5.0])),
                    ],
                    dtype="object",
                )
            ),
        ...
        self.check_mean_constant(op_node, expected=[1.0], shape=None)
        ...
        self.check_mean_constant(op_node, expected=[2.0, 3.0, 4.0], shape=[1, 1, 1, 3])

        op_node = list(op_node.output(0).get_target_inputs())[0].get_node()
        self.assertTrue(
            op_node.get_type_name() == "Divide" or op_node.get_type_name() == "Multiply"
        )
        self.check_scale_constant(op_node, expected=[5.0], shape=None)

    # Two inputs, but 'mean_scale_value' has only one array
    def test_error_no_param_name_number_not_match(self):
        argv = Namespace(
            mean_scale_values=[(np.array([2.0, 3.0]), np.array([4.0]))], scale=None
        )
    ...
    def test_mean_scale_error_no_node_name_found(self):
        argv = Namespace(
            mean_scale_values={
                "not_found": {"scale": np.array([1.0]), "mean": np.array([1.0])}
            },
            scale=None,
        )
        function = create_function2(shape1=[1, 3, 224, 224], shape2=[1, 2, 224, 224])
    ...
    def test_layout_error_no_node_name_found(self):
        argv = Namespace(
            layout_values={"not_found": {"source_layout": "nhwc"}}, scale=None
        )
        function = create_function2(shape1=[1, 3, 224, 224], shape2=[1, 2, 224, 224])
        with self.assertRaisesRegex(Error, ".*question.*83.*"):
            process_function(ov_function=function, argv=argv)
    ...
    def test_error_dimension_mismatch(self):
        argv = Namespace(
            mean_scale_values={
                "input1": {"scale": np.array([1.0, 2.0, 3.0, 4.0]), "mean": None}
            },
            scale=None,
        )
        function = create_function2(shape1=[1, 3, 224, 224])
    ...
    def test_error_dimension_not_clear(self):
        argv = Namespace(
            mean_scale_values={
                "input1": {"scale": np.array([1.0, 2.0, 3.0]), "mean": None}
            },
            scale=None,
        )
        function = create_function2(
            shape1=[1, 3, 3, 3]
        )  # Not clear to which 3 should scale be applied
        with self.assertRaises(Exception):
            process_function(ov_function=function, argv=argv)
    ...
    def test_error_2_names_to_same_input(self):
        argv = Namespace(
            mean_scale_values={
                "input1": {"scale": np.array([1.0, 2.0, 3.0])},
                "input1a": {"scale": np.array([1.0, 2.0, 3.0])},
            },
            scale=None,
        )
        function = create_function2(shape1=[1, 3, 224, 224])
    ...
    def test_error_2_names_to_same_input_single_value(self):
        argv = Namespace(
            mean_scale_values={
                "input1": {"scale": np.array([2.0])},
                "input1a": {"scale": np.array([3.0])},
            },
            scale=None,
        )
        function = create_function2(shape1=[1, 3, 224, 224])
    ...
    def test_reverse_input_channels(self):
        argv = Namespace(
            reverse_input_channels=True, mean_scale_values=None, scale=None
        )
        ...
        op_node0 = list(function.get_parameters()[0].output(0).get_target_inputs())[
            0
        ].get_node()
        self.assertTrue(op_node0.get_type_name() != "Relu")
        op_node1 = list(function.get_parameters()[1].output(0).get_target_inputs())[
            0
        ].get_node()
    ...
    def test_reverse_input_channels_6d(self):
        ...
        function = create_function2(
            shape1=[4, 4, 4, 4, 4, 3], shape2=[4, 3, 4, 4, 4, 4]
        )
        # no suitable inputs
        with self.assertRaises(Exception):
            process_function(ov_function=function, argv=argv)
    ...
    def test_reverse_input_channels_layout_change(self):
        argv = Namespace(
            ...
            scale=None,
            layout_values={
                "input1a": {"source_layout": "nchw", "target_layout": "nhwc"},
    ...
    def test_scale_vector3_layout_empty_input_name(self):
        argv = Namespace(
            mean_scale_values=list(
                np.array([(None, np.array([2.0, 4.0, 8.0]))], dtype="object")
            ),
            layout_values={"": {"source_layout": "nchw"}},
            scale=None,
        )
        function = create_function1(
            shape1=[1, 3, 3, 3]
        )  # Use layout to determine channels dim
    ...
    def test_layout_output(self):
        argv = Namespace(
            mean_scale_values=None,
            layout_values={
                "res1": {"source_layout": "nchw", "target_layout": "nhwc"},
                "res2a": {"source_layout": "ncdhw"},
            },
            scale=None,
        )
    ...
    def test_reverse_channels_and_mean_scale(self):
        ...
        gather = list(function.get_parameters()[1].output(0).get_target_inputs())[
            0
        ].get_node()
    ...
    def test_friendly_name(self):
        argv = Namespace(
            mean_scale_values={
                "input1": {"mean": np.array([2.0, 4.0, 8.0]), "scale": None}
            },
    ...
    def test_sorting_tensor_names(self):
        argv = Namespace(
            mean_scale_values={
                "c_input": {"mean": np.array([2.0, 4.0, 8.0]), "scale": None}
            },
    ...
    def test_sorting_tensor_names_unnamed_layout_list(self):
        argv = Namespace(
            ...
            scale=None,
            layout_values=[
                {"source_layout": "nchw", "target_layout": "nhwc"},
