...
import numpy as np
from unit_tests.utils.graph import (
    build_graph,
    connect,
...
main_graph_nodes = {
    ...
    **regular_op_with_empty_data(
        "Loop",
        {
            ...
            "body": None,
            "input_port_map": [
                {"external_port_id": 1, "internal_layer_id": 2, "axis": None},
                {"external_port_id": 2, "internal_layer_id": 0, "axis": None},
                {"external_port_id": 3, "internal_layer_id": 1, "axis": None},
            ],
            "output_port_map": [
                {"external_port_id": 0, "internal_layer_id": 4, "axis": None},
                {
                    "external_port_id": -1,
                    "internal_layer_id": 5,
                    "axis": None,
...
sub_graph_1_nodes = {
    ...
    **regular_op_with_empty_data(
        "Loop_2",
        {
            ...
            "body": None,
            "input_port_map": [
                {"external_port_id": 1, "internal_layer_id": 0, "axis": None},
                {"external_port_id": 2, "internal_layer_id": 2, "axis": None},
            ],
            "output_port_map": [
                {"external_port_id": 0, "internal_layer_id": 7, "axis": None},
                {
                    "external_port_id": -1,
                    "internal_layer_id": 6,
                    "axis": None,
    ...
    **shaped_parameter(
        "in_1_int", int64_array([1, 4, 64, 54]), {"internal_layer_id": 1}
    ),
    **regular_op_with_empty_data(
        "in_1_int_out",
        {
...
sub_graph_2_nodes = {
    ...
    **shaped_const_with_data(
        "ones", int64_array([1, 4, 64, 54]), {"internal_layer_id": 9}
    ),
    **regular_op_with_shaped_data(
        "OUT_2", int64_array([1, 4, 64, 54]), {"op": "Add", "infer": copy_shape_infer}
    ),
    **regular_op_with_empty_data(
        "OUT_2_out",
        {
...
def ti_create_main_graph(body):
    main_graph = build_graph(
        nodes_attrs=ti_main_graph_nodes,
        edges=[
            *connect("M", "0:Loop"),
            *connect("cond", "1:Loop"),
            *connect("IN_2", "2:Loop"),
...
def if_create_main_graph():
    sub_graph_2 = build_graph(
        nodes_attrs=if_sub_graph_2_then_nodes,
        edges=[
    ...
    sub_graph_1 = build_graph(
        nodes_attrs=if_sub_graph_1_then_nodes,
        edges=[
...
class AddOutputRecursiveTest(unittest.TestCase):
    ...
    def test_add_output_1(self):
        ...
        loop_node_1["output_port_map"].append(
            {"external_port_id": 1, "internal_layer_id": 8, "axis": None}
        )
        ...
        loop_node = Node(main_graph, "Loop")
        self.assertEqual(
            len(loop_node.output_port_map), loop_node_output_port_map_len + 2
        )
        ...
        self.assertTrue(
            np.all(
                loop_node.out_port(1).data.get_shape()
                == int64_array([5, 10, 4, 64, 54])
            )
        )
        ...
        self.assertTrue(
            np.all(
                unsq_node.out_port(0).data.get_shape()
...
ti_main_graph_nodes = {
    ...
    **regular_op_with_empty_data(
        "Loop",
        {
            ...
            "output_port_map": [
                {"external_port_id": 4, "internal_layer_id": 4, "axis": None}
            ],
            "back_edges": [
                {"from_layer": 8, "to_layer": 7},
                {"from_layer": 10, "to_layer": 9},
...
ti_sub_graph_1_nodes = {
    ...
    **regular_op_with_empty_data(
        "Loop_2",
        {
            ...
            "output_port_map": [
                {"external_port_id": 2, "internal_layer_id": 7, "axis": None},
            ],
            "back_edges": [
                {"from_layer": 1, "to_layer": 0},
                {"from_layer": 8, "to_layer": 2},
...
class TI_AddOutputRecursiveTest(unittest.TestCase):
    ...
    def create_graph():
        sub_graph_2 = build_graph(
            nodes_attrs=ti_sub_graph_2_nodes,
            edges=[
                *connect("cond_2_int", "cond_2_int_out"),
                *connect("in_2_int", "OUT_2"),
                *connect("ones", "OUT_2"),
    ...
    def check_body_last_node(self, body, node_id, loop_2_node_out_ports_len):
        ...
        self.assertTrue(
            np.all(
                unsq_node.out_port(0).data.get_shape() == int64_array([1, 1, 4, 64, 54])
            )
        )
    ...
    def test_add_output_1(self):
        ...
        self.check_loop_node(
            main_graph, "Loop", loop_node_output_port_map_len, loop_node_out_ports_len
        )
        self.assertTrue(
            np.all(
                loop_node.out_port(1).data.get_shape() == int64_array([1, 1, 4, 64, 54])
            )
        )
    ...
    def test_add_output_several_iterations(self):
        ...
        self.assertTrue(
            np.all(
                loop_node.out_port(1).data.get_shape() == shape_array([4, 1, 4, 64, 54])
            )
        )
        self.assertTrue(
            np.all(
                loop_node.out_port(0).data.get_shape() == shape_array([1, 5, 64, 54])
            )
        )
        self.check_body_last_node(sub_graph_1, "Loop_2", loop_2_node_out_ports_len)
    ...
    def test_add_output_several_iterations_negative_end(self):
        ...
        self.assertTrue(
            np.all(
                loop_node.out_port(1).data.get_shape() == shape_array([2, 1, 4, 64, 54])
            )
        )
        self.assertTrue(
            np.all(
                loop_node.out_port(0).data.get_shape() == shape_array([1, 2, 64, 54])
            )
        )
        self.check_body_last_node(sub_graph_1, "Loop_2", loop_2_node_out_ports_len)
    ...
    def test_add_output_several_iterations_negative_start_end_output(self):
        ...
        self.assertTrue(
            np.all(
                loop_node.out_port(0).data.get_shape() == shape_array([1, 3, 64, 54])
            )
        )
        self.check_body_last_node(sub_graph_1, "Loop_2", loop_2_node_out_ports_len)
...
if_main_graph_nodes = {
    ...
    **regular_op_with_empty_data(
        "If",
        {
            "op": "If",
            "type": "If",
            "sub_graphs": ["then_graph", "else_graph"],
            "then_graph": None,
            "else_graph": None,
            "infer": If.infer,
        },
...
if_sub_graph_1_then_nodes = {
    ...
    **regular_op_with_empty_data(
        "If_2_out", {"op": "Result", "type": "Result", "infer": lambda x: None}
    ),
    **shaped_parameter("in_1_int", int64_array([1, 4, 64, 54]), {"input_id": 1}),
    **regular_op_with_empty_data(
        "in_1_int_out", {"op": "Result", "type": "Result", "output_id": 0}
    ),
}
...
if_sub_graph_2_then_nodes = {
    ...
    **regular_op_with_empty_data(
        "OUT_2_out", {"op": "Result", "type": "Result", "output_id": 0}
    ),
}
...
if_sub_graph_2_else_nodes = {
    ...
    **regular_op_with_shaped_data(
        "OUT_2_else", int64_array([1, 4, 64, 54]), {"op": "Sub"}
    ),
    **regular_op_with_empty_data(
        "OUT_2_out_else", {"op": "Result", "type": "Result", "output_id": 0}
    ),
}
...
class IF_AddOutputRecursiveTest(unittest.TestCase):
    def test_add_output_1(self):
        ...
        main_graph = build_graph(
            nodes_attrs=if_main_graph_nodes,
            edges=[
                *connect("cond", "0:If"),
                *connect("IN_1", "1:If"),
                *connect("IN_2", "2:If"),
        ...
        self.assertTrue(
            np.all(if_node.out_port(1).data.get_shape() == int64_array([1, 4, 64, 54]))
        )
        ...
        self.assertTrue(
            np.all(
                last_node.out_port(0).data.get_shape() == int64_array([1, 4, 64, 54])
            )
        )
...
class SplitUserPathTest(unittest.TestCase):
    ...
    def test_linear_graph_change(self):
        ...
        ref_path.append(
            {"node": Node(loop_node.body, "in_1_int"), "graph": loop_node.body}
        )
    ...
    def test_1_if_graph_change(self):
        ...
        ref_path[0].append(
            {"node": Node(if_node.then_graph, "OUT_2"), "graph": if_node.then_graph}
        )
        ref_path[1].append(
            {
