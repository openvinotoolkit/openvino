...
from openvino.tools.mo.back.ie_ir_ver_2.emitter import (
    port_renumber,
    serialize_network,
    serialize_runtime_info,
    soft_get,
    xml_shape,
)
...
from openvino.tools.mo.utils.runtime_info import (
...
class TestSerializeRTInfo(unittest.TestCase):
    def test_serialize_old_api_map_parameter(self):
        graph = build_graph(
            {
                **regular_op("placeholder", {"type": "Parameter", "rt_info": RTInfo()}),
                **result("result"),
            },
            [("placeholder", "result")],
            {},
        ...
        param_node.rt_info.info[("old_api_map_order", 0)].old_api_transpose_parameter(
            [0, 2, 3, 1]
        )
        param_node.rt_info.info[("old_api_map_element_type", 0)] = (
            OldAPIMapElementType()
        )
        param_node.rt_info.info[("old_api_map_element_type", 0)].set_legacy_type(
            np.float32
        )
    ...
    def test_serialize_old_api_map_result(self):
        ...
        result_node.rt_info.info[("old_api_map_order", 0)].old_api_transpose_result(
            [0, 3, 1, 2]
        )
...
class TestSerialize(unittest.TestCase):
    ...
    def build_graph_with_gather():
        nodes = {
            **shaped_parameter(
                "data",
                int64_array([3, 3]),
                {"data_type": np.float32, "type": Parameter.op},
            ),
            **shaped_parameter(
                "indices",
                int64_array([1, 2]),
    ...
    def test_maxpool_raises(self):
        ...
        with self.assertRaisesRegex(
            AssertionError, "Incorrect graph. Non-Result node.*"
        ):
            serialize_network(graph, net, unsupported)
