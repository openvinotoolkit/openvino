...
dft_graph_node_attrs = {
    **regular_op_with_shaped_data(
        "placeholder", [8, 2, 40, 56], {"type": "Parameter", "op": "Parameter"}
    ),
    **valued_const_with_data("axes", int64_array([-2, -1])),
    **regular_op_with_shaped_data(
        "dft", [8, 2, 40, 56], {"op": "DFT", "need_insert_transposes_for_dft": True}
    ),
...
class TransposeDFTTest(unittest.TestCase):
    def test_dft_transpose(self):
        ...
        ref_graph = build_graph(
            nodes_attrs=transposed_dft_graph_node_attrs,
            edges=transposed_dft_graph_edges,
        )
        graph.graph["fw"] = "tf"
    ...
    def test_dft_nontranspose(self):
        graph = build_graph(
            nodes_attrs=nontransposed_dft_graph_node_attrs,
            edges=nontransposed_dft_graph_edges,
        )
        ref_graph = build_graph(
            nodes_attrs=nontransposed_dft_graph_node_attrs,
            edges=nontransposed_dft_graph_edges,
