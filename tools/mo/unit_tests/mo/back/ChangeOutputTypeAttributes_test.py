...
class ChangeOutputTypeAttributesTests(unittest.TestCase):
    ...
    def test_range_correct_case(self):
        graph, graph_ref = build_range_test_graphs(
    ...
    def test_range_different_values(self):
        graph, graph_ref = build_range_test_graphs(
            start=0, limit=50000, delta=1, dst_type_str="FP16"
        )
        self.assertRaises(
            Error, ChangeOutputTypeAttributes().find_and_replace_pattern, graph
        )

    def test_range_out_of_fp16_max(self):
        graph, graph_ref = build_range_test_graphs(
            start=0, limit=100000, delta=1, dst_type_str="FP16"
        )
        self.assertRaises(
            Error, ChangeOutputTypeAttributes().find_and_replace_pattern, graph
        )
