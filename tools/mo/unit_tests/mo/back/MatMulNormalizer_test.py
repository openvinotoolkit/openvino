...
import numpy as np
import pytest
from unit_tests.utils.graph import build_graph, connect, connect_data
...
class TestSmartReshape_HC_Reshape_MatMulTest:
    ...
    def test_reshape_on_the_A_input(
        self,
        in1_shape,
        in2_shape,
        reshape_pattern,
        ...
        graph = build_graph(
            nodes_attrs=nodes, edges=edges, cli=Namespace(static_shape=True)
        )
        graph.clean_up()
        ...
        graph_ref = build_graph(
            ...
            edges=edges,
            update_attributes={
                "dim": {"value": int64_array(updated_pattern)},
                "dim_d": {"value": int64_array(updated_pattern)},
            },
        )
        graph_ref.clean_up()
    ...
    def test_reshape_on_the_B_input(
        self,
        in1_shape,
        in2_shape,
        reshape_pattern,
...
class FQTransposePullerTest(unittest.TestCase):
    def nodes(self, input_shape, transpose_shape, fq_shape, is_input_const):
        nodes = {
            ...
            **regular_op_with_shaped_data(
                "FQ",
                fq_shape,
                dict(type="FakeQuantize", op="FakeQuantize", infer=FakeQuantize.infer),
            ),
            ...
            **regular_op_with_shaped_data(
                "relu", fq_shape, dict(type="Relu", op="Relu")
            ),
            **result(),
        }
    ...
    def test_positive(self):
        ...
        graph_ref = build_graph(
            nodes_attrs=nodes, edges=edges, nodes_with_edges_only=True
        )
