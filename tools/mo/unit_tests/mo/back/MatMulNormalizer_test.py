...
class SmartReshape_HC_Reshape_MatMulTest(unittest.TestCase):
    def test_reshape_on_the_A_input(self):
        ...
        for idx, (in1_shape, in2_shape, reshape_pattern, transpose_a, transpose_b,
                  updated_pattern) in enumerate(test_cases):
            with self.subTest(test_case=idx):
                nodes = {
                    ...
                    **op_with_empty_data('reshape',
                                         dict(type='Reshape', op='Reshape', infer=Reshape.infer,
    ...
    def test_reshape_on_the_B_input(self):
        ...
        for idx ,(in1_shape, in2_shape, reshape_pattern, transpose_a,
                  transpose_b, updated_pattern) in enumerate(test_cases):
    ...
    def nodes(self, input_shape, transpose_shape, fq_shape, is_input_const):
        nodes = {
            ...
            **regular_op_with_shaped_data('FQ', fq_shape, dict(type='FakeQuantize',
                                            op='FakeQuantize', infer=FakeQuantize.infer)),
            **valued_const_with_data('order', int64_array([0, 2, 3, 1])),
            **regular_op_with_shaped_data('transpose', transpose_shape, dict(type='Transpose',
                                            op='Transpose', infer=Transpose.infer)),
