...
class TestBroadcastTest:
    @pytest.mark.parametrize(
        "data, target_shape, axes_mapping, mode, ref_out, test_raising",
        [
            ...
            (
                ...
                [2, 2, 1, 3],
                [1, 2, 3],
                "explicit",  # in_shape (2, 1, 3)
                [[[[9, 5, 7]], [[9, 5, 7]]], [[[9, 5, 7]], [[9, 5, 7]]]],
                False,
            ),  # ref_out_shape (2, 2, 1, 3)
            (
                [[[9, 5, 7]], [[3, 4, 8]]],
                [2, 1, 3, 3],
                [0, 1, 2],
    ...
    def test_broadcast(
        self, data, target_shape, axes_mapping, mode, ref_out, test_raising
    ):
        ...
        nodes = {
            ...
            **regular_op_with_empty_data(
                "broadcast", {"op": "Broadcast", "mode": mode}
            ),
        }
        ...
        if axes_mapping is not None:
            nodes.update(
                **valued_const_with_data("axes_mapping", int64_array(axes_mapping))
            )
        ...
        if ref_out is not None:
            ...
            assert np.array_equal(
                broadcast_node.out_node().shape, np.array(target_shape)
            )
    ...
    def test_broadcast_dynamic(
        self, data, target_shape_shape, axes_mapping, mode, ref_out_shape, test_raising
    ):
        nodes = {
