...
class TestUpsampleOp:
    @pytest.mark.parametrize(
        "scales, input_shape, expected_shape",
        [
            (
                np.array([1.0, 1.0, 2.0, 2.0]),
                shape_array([1, 3, 227, 227]),
                shape_array([1, 3, 454, 454]),
            ),
            (
                np.array([1.0, 1.0, 2.5, 1.5]),
                shape_array([1, 5, 227, 227]),
                shape_array([1, 5, 567, 340]),
            ),
            (
                np.array([1.0, 1.0, 1.3, 0.7]),
                shape_array([1, 14, 1023, 713]),
                shape_array([1, 14, 1329, 499]),
            ),
            (
                np.array([1.0, 1.0, 1.3, 0.7]),
                shape_array([1, 14, dynamic_dimension_value, 713]),
                shape_array([1, 14, dynamic_dimension_value, 499]),
            ),
            (
                np.array([1.0, 1.0, 1.3, 0.7]),
                shape_array([1, 14, 1023, dynamic_dimension_value]),
    ...
    def test_upsample_with_scales_infer(self, scales, input_shape, expected_shape):
        graph = build_graph(
            nodes_attributes,
            [("node_1", "upsample"), ("upsample", "node_3"), ("node_3", "op_output")],
            {
    ...
    def test_upsample_with_second_input_infer(
        self, scales, input_shape, expected_shape
    ):
