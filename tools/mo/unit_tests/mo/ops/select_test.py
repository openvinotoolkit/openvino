...
class TestSelect(unittest.TestCase):
    ...
    @staticmethod
    def build_select_graph_and_infer(
        condition_value,
        then_value,
        else_value,
        out_value,
        ...
        nodes = {
            ...
            **regular_op_with_empty_data(
                "select",
                {"op": "Select", "auto_broadcast": auto_broadcast, "format": fw_format},
            ),
            **result("out"),
        }
        edges = [
        ...
        if out_shape is not None:
            ...
            assert (
                select_out_node["value"] is None
            ), "if 'out_shape' is defined manually 'value' must be None"

        flag = strict_compare_tensors(actual_val, ref_val)
        msg = (
            ""
            if flag
            else "reference {} and actual {} {} do not match\n".format(
    ...
    def test_1(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.ones([5, 6], dtype=bool),
    ...
    def test_2(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.ones([15, 3, 5], dtype=bool),
    ...
    def test_select_infer_condition_true(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.array([True, True], dtype=bool),
    ...
    def test_select_infer_condition_false(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.array([False, False], dtype=bool),
            then_value=np.array([1, 1], dtype=np.int8),
            else_value=np.array([2, 2], dtype=np.int8),
    ...
    def test_select_infer_condition_true_2(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.array([True], dtype=bool),
            then_value=np.ones([15, 3, 5], dtype=float),
            else_value=np.zeros([15, 1, 5], dtype=float),
            out_value=np.ones([15, 3, 5], dtype=float),
        )
    ...
    def test_select_infer_condition_false_then_and_else_are_scalars(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.array([False], dtype=bool),
            then_value=np.array(3, dtype=float),
            else_value=np.array(1, dtype=float),
    ...
    def test_select_infer_None_then_branch_1(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.zeros([15, 3, 5], dtype=bool),
            then_value=None,
            then_shape=[15, 3, 5],
    ...
    def test_select_broadcast_1(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.ones([2, 3, 4, 5], dtype=bool),
    ...
    def test_select_broadcast_2(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.ones([2, 3, 4, 1], dtype=bool),
    ...
    def test_select_broadcast_3(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.ones([2, 3, 1, 1], dtype=bool),
    ...
    def test_select_infer_assert_shapes(self):
        with self.assertRaisesRegex(AssertionError, "must be broadcastable"):
            self.build_select_graph_and_infer(
                condition_value=None,
                condition_shape=[2, 2],
                then_value=None,
                then_shape=[2, 2],
    ...
    def test_select_infer_masked_1(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.ma.array([True, True], mask=[1, 1]),
    ...
    def test_select_infer_masked_2(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.ma.array([False, False], mask=[1, 1]),
            # condition_value = [dynamic_dimension, dynamic_dimension])
            then_value=None,
            then_shape=[2],
            else_value=np.zeros((2, 2), dtype=np.int64),
    ...
    def test_select_infer_masked_4(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.ma.array([True, False], mask=[0, 1]),
    ...
    def test_select_infer_masked_5(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.ma.array([False, True], mask=[0, 1]),
            #  condition_value = [True, dynamic_dimension])
            then_value=np.ones((2, 2), dtype=np.int64),
            else_value=np.zeros((2, 2), dtype=np.int64),
    ...
    def test_select_infer_masked_6(self):
        flag, msg = self.build_select_graph_and_infer(
            condition_value=np.ma.array([True, False], mask=[1, 0]),
            #  condition_value = [True, dynamic_dimension])
            then_value=np.ones((2, 2), dtype=np.int64),
            else_value=np.zeros((2, 2), dtype=np.int64),
    ...
    def test_select_infer_no_broadcast_dynamic_then_else_shapes_2(self):
        flag, msg = self.build_select_graph_and_infer(
            ...
            condition_shape=shape_array([100, 100]),
            then_value=None,
            then_shape=shape_array([dynamic_dimension_value, 100]),
            else_value=None,
            else_shape=shape_array([100, dynamic_dimension_value]),
            out_value=None,
            out_shape=shape_array([100, 100]),
            auto_broadcast="none",
        )
    ...
    def test_select_infer_tf_condition_assert_raises(self):
        with self.assertRaisesRegex(
            AssertionError, "if 'condition' is a 1D tensor then it's size"
        ):
            self.build_select_graph_and_infer(
                condition_value=None,
    ...
    def test_select_infer_assert_pdpd(self):
        with self.assertRaisesRegex(
            Error, "PDPD broadcasting rule is not implemented yet"
        ):
            self.build_select_graph_and_infer(
                condition_value=None,
                condition_shape=[2, 2],
