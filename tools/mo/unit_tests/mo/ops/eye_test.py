...
class TestComplexOp:
    ...
    def test_complex_op_shape_inference(
        self, input_shape, output_shape, num_rows, num_cols, batch_shape
    ):
        graph = build_graph_with_attrs(
            nodes_with_attrs=graph_node_attrs_sizes,
            edges_with_attrs=graph_edges_sizes,
            update_nodes_attributes=[
                (
                    "num_rows_data",
                    {"shape": int64_array(input_shape), "value": num_rows},
                ),
                (
                    "num_columns_data",
                    {"shape": int64_array(input_shape), "value": num_cols},
                ),
                ("diagonal_index_data", {"shape": int64_array(input_shape)}),
                (
                    "batch_shape_data",
                    {"shape": int64_array([len(batch_shape)]), "value": batch_shape},
                ),
        ...
        assert np.array_equal(
            graph.node["eye_op_data"]["shape"], output_shape
    ...
    def test_value_inference(self):
        graph_node_attrs_sizes = {
            ...
            **regular_op_with_empty_data(
                "eye_op", {"op": "Eye", "output_type": np.float32}
            ),
            **result("res"),
        }
        graph_edges_sizes = [
        ...
        graph = build_graph(graph_node_attrs_sizes, graph_edges_sizes)
        node = Node(graph, "eye_op")
        Eye.infer(node)
        output_value = np.eye(
            int64_array(128), M=int64_array(128), k=int64_array(0), dtype=np.float32
        )

        msg = "Eye operation infer failed for case: expected_value={}, actual_value={}"

        assert np.array_equal(
            graph.node["eye_op_d"]["value"], output_value
