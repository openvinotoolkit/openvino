...
from openvino.tools.mo.ops.elementwise import Elementwise, Round


def round_test_graph(nodes_attributes, value, mode: str):
    graph = build_graph(
        nodes_attributes,
        [("node_1", "elementwise_node"), ("elementwise_node", "node_3")],
        {
            "node_1": {"value": value},
            "elementwise_node": {
                "op": "Round",
...
class TestElementwiseOp(unittest.TestCase):
    nodes_attributes = {
        "node_1": {"shape": np.array([13]), "value": None},
        "elementwise_node": {"op": None, "kind": "op", "operation": None},
        "node_3": {"shape": None},
    }

    value = np.array(
        [-23.5, -22.5, -2.5, -1.5, -0.5, 0.5, 0.9, 1.5, 2.3, 2.5, 3.5, 22.5, 23.5]
    )
    ...
    def test_elementwise_round_away_infer(self):
        graph = round_test_graph(
            self.nodes_attributes, self.value, "half_away_from_zero"
        )
        ...
        res_value = graph.node["node_3"]["value"]
        exp_value = np.array(
            [-24.0, -23.0, -3.0, -2.0, -1.0, 1.0, 1.0, 2.0, 2.0, 3.0, 4.0, 23.0, 24.0]
        )
        for i, value in enumerate(exp_shape):
            self.assertEqual(res_shape[i], value)
        for i, value in enumerate(exp_value):
...
class TestElementwiseTypeAlignment(unittest.TestCase):
    ...
    @staticmethod
    def build_graph_to_test_type_alignment(
        edges, input_1_type=np.float32, input_2_type=np.float32, const_type=np.float32
    ):
        ...
        nodes = {
            ...
            **regular_op_with_empty_data(
                "add",
                {"op": "Add", "type": "Add", "type_infer": Elementwise.type_infer},
            ),
            **valued_const_with_data(
                "const", const_value, kwargs={"data_type": const_type}
            ),
            **result("result"),
        }
    ...
    def test_first_input_const(self):
        ...
        graph = self.build_graph_to_test_type_alignment(
            edges, const_type=np.float16, input_1_type=np.float32
        )
    ...
    def test_second_input_const(self):
        ...
        graph = self.build_graph_to_test_type_alignment(
            edges, input_1_type=np.float32, const_type=np.float16
        )
    ...
    def test_raises(self):
        ...
        graph = self.build_graph_to_test_type_alignment(
            edges, input_1_type=np.float32, input_2_type=np.float16
        )
    ...
    def test_not_raises(self):
        ...
        graph = self.build_graph_to_test_type_alignment(
            edges, input_1_type=np.float32, input_2_type=np.float32
        )
