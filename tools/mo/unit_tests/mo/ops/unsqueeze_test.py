...
class TestUnsqueezeOp:
    ...
    @pytest.mark.parametrize(
        "input_shape, unsq_dims, output_shape, ref_uns_dims, input_value, output_value",
        [
            (
                shape_array([1, 3, 64, 64]),
                int64_array([0, 4]),
                shape_array([1, 1, 3, 64, 1, 64]),
                int64_array([0, 4]),
                None,
                None,
            ),
            (
                shape_array([2, 3, 64, 64]),
                int64_array([-1]),
                shape_array([2, 3, 64, 64, 1]),
                int64_array([4]),
                None,
                None,
            ),
            (
                shape_array([2, 3, dynamic_dimension_value, 64]),
                int64_array([0]),
                shape_array([1, 2, 3, dynamic_dimension_value, 64]),
                int64_array([0]),
                None,
                None,
            ),
    ...
    def test_unsqueeze_infer(
        ...
        assert strict_compare_tensors(
            Node(graph, "data_2").shape, Node(graph_ref, "data_2").shape
        )
        if Node(graph_ref, "data_2").value is not None:
            assert strict_compare_tensors(
                Node(graph, "data_2").value, Node(graph_ref, "data_2").value
            )
