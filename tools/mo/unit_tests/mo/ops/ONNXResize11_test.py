...
class TestONNXResize11Op:
    @pytest.mark.parametrize(
        "input_shape, output_shape, sizes, scales",
        [
            (
                [1, 260, 100, 150],
                [1, 260, 200, 350],
    ...
    def test_onnx_resize11_using_sizes(self, input_shape, output_shape, sizes, scales):
        ...
        graph = build_graph(
            nodes_attrs=graph_node_attrs_sizes,
        ...
        assert np.array_equal(
            graph.node["onnx_resize11_data"]["shape"], int64_array(output_shape)
        ), msg.format(
            sizes, scales, output_shape, graph.node["onnx_resize11_data"]["shape"]
        )

    @pytest.mark.parametrize(
        "input_shape, output_shape, scales",
        [
            ...
            (
                [1, 3, 260, 100, 150],
                [1, 3, 780, 200, 350],
                [1.0, 1.0, 3.0, 2.0, 350 / 150],
            ),
            (
                [1, 3, 450, 100, 150],
                [1, 3, 260, 200, 350],
                [1.0, 1.0, 260 / 450, 2.0, 350 / 150],
            ),
            (
                [5, 14, 1000, 300, 40],
                [5, 14, 500, 140, 280],
                [1.0, 1.0, 0.5, 140 / 300, 7.0],
            ),
            ([4, 3, 180, 1340], [4, 3, 60, 804], [1.0, 1.0, 0.33333334, 0.6]),
            (
                [4, 3, 500, 180, 1340],
                [4, 3, 750, 60, 804],
                [1.0, 1.0, 1.5, 0.33333334, 0.6],
            ),
        ],
    )
    def test_onnx_resize_using_scales(self, input_shape, output_shape, scales):
        np_scales = np.array(scales)
        graph = build_graph(
            nodes_attrs=graph_node_attrs_scales,
    ...
    def test_onnx_resize11_using_sizes_without_roi_input(
        self, input_shape, output_shape, sizes, scales
    ):
        np_scales = np.array(scales)
        np_sizes = int64_array(sizes)
        graph = build_graph(
    ...
    def test_onnx_resize_using_scales_without_roi(
        self, input_shape, output_shape, scales
    ):
        np_scales = np.array(scales)
        graph = build_graph(
            nodes_attrs=graph_node_attrs_scales,
