...
class TestExpandDimsOp:
    ...
    def test_expand_dims_infer(self, axis, ref_out_shape):
        graph = build_graph(
            nodes_attributes,
            [("data_1", "expand_dims"), ("expand_dims", "data_2")],
        ...
        assert np.array_equal(
            expand_dims_node.out_node().shape, np.array(ref_out_shape)
        )
...
class TestExpandDimsOpDynamicDims:
    @pytest.mark.parametrize(
        "axis, ref_out_shape",
        [
            (0, [1, 2, 3, dynamic_dimension_value, 224]),
    ...
    def test_expand_dims_infer(self, axis, ref_out_shape):
        ...
        assert strict_compare_tensors(
            expand_dims_node.out_node().shape, shape_array(ref_out_shape)
        )


class TestExpandDimsOpValueInfer:
    @pytest.mark.parametrize(
        "axis, in_shape, ref_out_shape",
        [
            (0, [2, 3, 224, 224], [1, 2, 3, 224, 224]),
    ...
    def test_expand_dims_infer_value(self, axis, in_shape, ref_out_shape):
        ...
        graph = build_graph(
            nodes_attributes,
            [("data_1", "expand_dims"), ("expand_dims", "data_2")],
            {"data_1": {"value": in_value}, "expand_dims": {"expand_axis": axis}},
        )
        expand_dims_node = Node(graph, "expand_dims")
        ...
        assert np.array_equal(
            expand_dims_node.out_node().value, np.array(in_value.reshape(ref_out_shape))
        )
