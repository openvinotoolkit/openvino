...
class ExpandDimsOp(unittest.TestCase):
    def test_expand_dims_infer(self):
        test_cases=[(0, [1, 2, 3, 224, 224]),
                (1, [2, 1, 3, 224, 224]),
                (2, [2, 3, 1, 224, 224]),
                (3, [2, 3, 224, 1, 224]),
                (4, [2, 3, 224, 224, 1]),
                ]
        for idx, (axis, ref_out_shape) in enumerate(test_cases):
...
class ExpandDimsOpDynamicDims(unittest.TestCase):
    def test_expand_dims_infer(self):
        test_cases=[(0, [1, 2, 3, dynamic_dimension_value, 224]),
...
class ExpandDimsOpValueInfer(unittest.TestCase):
    def test_expand_dims_infer_value(self):
        test_cases=[(0, [2, 3, 224, 224], [1, 2, 3, 224, 224]),
                (1, [2, 3, 224, 224], [2, 1, 3, 224, 224]),
                (2, [2, 3, 224, 224], [2, 3, 1, 224, 224]),
                (3, [2, 3, 224, 224], [2, 3, 224, 1, 224]),
                (4, [2, 3, 224, 224], [2, 3, 224, 224, 1]),
                ]
        for idx, (axis, in_shape, ref_out_shape) in enumerate(test_cases):
            with self.subTest(test_cases=idx):
                ...
                self.assertTrue(np.array_equal(expand_dims_node.out_node().shape, np.array(ref_out_shape)))
                self.assertTrue(np.array_equal(expand_dims_node.out_node().value,
