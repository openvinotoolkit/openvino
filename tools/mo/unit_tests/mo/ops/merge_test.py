...
class TestMerge(unittest.TestCase):
    nodes = [
        ...
        (
            "merge_output",
            {"value": None, "kind": "data", "executable": True, "shape": None},
        ),
    ]
    edges = [
        ("first", "merge", {"in": 0}),
    ...
    def test_merge_infer_simple_case_one_executable(self):
        ...
        graph_ref = build_graph_with_attrs(
            ...
            update_nodes_attributes=[
                ("merge_output", {"shape": np.array([2, 2]), "value": np.ones((2, 2))}),
        ...
        (flag, resp) = compare_graphs(
            graph, graph_ref, "merge_output", check_op_attrs=True
        )
    ...
    def test_merge_infer_complex_case(self):
        ...
        graph = build_graph_with_attrs(
            ...
            edges_with_attrs=self.edges,
            update_nodes_attributes=[
                ("first", {"is_partial_inferred": False, "value": None}),
        ...
        graph_ref = build_graph_with_attrs(
            ...
            update_nodes_attributes=[
                ("second", {"executable": True}),
                ("first", {"is_partial_inferred": False, "value": None}),
                ("merge_output", {"shape": np.array([2, 2]), "value": None}),
        ...
        graph_ref = build_graph_with_attrs(
            ...
            update_nodes_attributes=[
                ("second", {"executable": True}),
                ("first", {"is_partial_inferred": True, "value": None}),
                ("merge_output", {"shape": np.array([2, 2]), "value": None}),
                ("merge", {"is_not_fully_inferred": False}),
            ],
        )
    ...
    def test_merge_infer_only_second_executable(self):
        ...
        ref_graph = build_graph_with_attrs(
            ...
            update_nodes_attributes=[
                ...
                (
                    "merge_output",
                    {"shape": int64_array([4, 4]), "value": np.zeros([4, 4])},
                ),
            ],
        )
        ...
        (flag, resp) = compare_graphs(
            graph, ref_graph, "merge_output", check_op_attrs=True
        )
        self.assertTrue(flag, resp)
