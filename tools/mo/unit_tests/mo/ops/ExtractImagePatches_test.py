...
class TestExtractImagePatchesPartialInfer:
    @pytest.mark.parametrize(
        "input_shape, sizes, strides, rates, auto_pad, layout, output_shape",
        [
            (
                [1, 10, 10, 3],
                [1, 3, 3, 1],
    ...
    def test_eip_infer(
        self, input_shape, sizes, strides, rates, auto_pad, layout, output_shape
    ):
        graph = build_graph(
            nodes_attrs=nodes,
            edges=edges,
            update_attributes={
                ...
                "EIP": {
                    "spatial_dims": (
                        int64_array([1, 2]) if layout == "NHWC" else int64_array([2, 3])
                    ),
                    "sizes": int64_array(sizes),
                    "strides": int64_array(strides),
