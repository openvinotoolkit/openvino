...
class TestMatMul:
    ...
    def test_positive_matmul_infer(
        self, A_shape, B_shape, C_shape, transpose_a, transpose_b
    ):
        ...
        msg = (
            "MatMul infer failed for case: A_shape={}, B_shape={}, transpose_a={}, transpose_b={} "
            "expected_shape={}, actual_shape={}"
        )

        assert np.array_equal(
            graph.node["mat_mul_d"]["shape"], shape_array(C_shape)
        ), msg.format(
            A_shape,
            B_shape,
            transpose_a,
