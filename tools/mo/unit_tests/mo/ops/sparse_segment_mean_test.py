...
edges1 = [
    ("input_data", "sparse_segment_mean_node", {"in": 0}),
...
inputs2 = {
    ...
    "input_segment_ids": {
        "shape": int64_array([3]),
        "value": np.array([0, 0, 1, 2, 2], dtype=float),
    },
}
...
class TestSparseSegmentMean(unittest.TestCase):
    ...
    def test_incorrect_shapes(self):
        ...
        sparse_segment_mean_node = Node(graph, "sparse_segment_mean_node")
        self.assertRaises(
            AssertionError, SparseSegmentMean.infer, sparse_segment_mean_node
        )
    ...
    def test_infer_constant_input_mean(self):
        ...
        ref_output_segments_value = np.array(
            [[3, 4, 5, 6], [-1, -2, -3, -4], [2, 2, 2, 2]], dtype=float
        )
