...
class TestBlockLSTM(unittest.TestCase):
    ...
    def run_test(
        ...
        num_inputs,
        hidden_size,
        ref_hidden_states_shape,
        ref_cell_states_shape,
    ):
        nodes = {
            **regular_op_with_shaped_data(
                "x", [time_len, batch_size, num_inputs], {"type": "Parameter"}
            ),
            **regular_op_with_shaped_data(
                "weights", [num_inputs, hidden_size * 4], {"type": "Parameter"}
            ),
            **regular_op_with_shaped_data(
                "bias", [hidden_size * 4], {"type": "Parameter"}
            ),
            **regular_op_with_shaped_data(
                "init_hidden_state", [hidden_size * 4], {"type": "Parameter"}
            ),
            **regular_op_with_shaped_data(
                "init_cell_state", [hidden_size * 4], {"type": "Parameter"}
            ),
        ...
        self.assertTrue(
            strict_compare_tensors(hidden_states_shape, ref_hidden_states_shape)
        )
        self.assertTrue(
            strict_compare_tensors(cell_states_shape, ref_cell_states_shape)
        )
    ...
    def test_block_lstm_basic_infer(self):
        self.run_test(
            ...
            num_inputs=40,
            hidden_size=60,
            ref_hidden_states_shape=shape_array([4, 2, 60]),
            ref_cell_states_shape=shape_array([4, 2, 60]),
        )
    ...
    def test_block_lstm_dynamic_infer(self):
        self.run_test(
            ...
            num_inputs=40,
            hidden_size=60,
            ref_hidden_states_shape=shape_array(
                [dynamic_dimension, dynamic_dimension, 60]
            ),
            ref_cell_states_shape=shape_array(
                [dynamic_dimension, dynamic_dimension, 60]
            ),
        )
    ...
    def test_failed_three_outputs(self):
        nodes = {
            ...
            **regular_op_with_shaped_data(
                "init_hidden_state", [120], {"type": "Parameter"}
            ),
            **regular_op_with_shaped_data(
                "init_cell_state", [120], {"type": "Parameter"}
            ),
            **regular_op_with_empty_data("block_lstm", {"op": "BlockLSTM"}),
            **result("hidden_states"),
            **result("cell_states"),
        ...
        with self.assertRaisesRegex(
            AssertionError, "Internal Model Optimizer Error or unsupported BlockLSTM*"
        ):
            BlockLSTM.infer(node)
