...
edges1 = [
    ("input_data", "sparse_segment_sum_node", {"in": 0}),
...
inputs2 = {
    ...
    "input_indices": {
        "shape": int64_array([3]),
        "value": np.array([0, 1, 2], dtype=float),
    },
    "input_segment_ids": {
        "shape": int64_array([3]),
        "value": np.array([0, 0, 1], dtype=float),
    },
}
...
class TestSparseSegmentSum(unittest.TestCase):
    ...
    def test_incorrect_shapes(self):
        ...
        sparse_segment_sum_node = Node(graph, "sparse_segment_sum_node")
        self.assertRaises(
            AssertionError, SparseSegmentSum.infer, sparse_segment_sum_node
        )
