...
nodes_attributes = {
    "proposal_input": {"kind": "data", "shape": None, "value": None},
...
class TestProposal(unittest.TestCase):
    def test_proposal_infer_one_output(self):
        ...
        self.assertListEqual(
            [1 * 2, 5], list(graph.node["proposal_out_data_1"]["shape"])
        )
    ...
    def test_proposal_infer_two_outputs(self):
        ...
        self.assertListEqual(
            list([1 * 2, 5]), list(graph.node["proposal_out_data_1"]["shape"])
        )
        self.assertListEqual(
            list([1 * 2]), list(graph.node["proposal_out_data_2"]["shape"])
        )
