...
class TestGatherPartialInfer(UnitTestWithMockedTelemetry):
    ...
    def build_and_test_value_inference(
        data, indices, axis, batch_dims, ref_value, negative_test_string=None
    ):
        nodes = {
            ...
            **regular_op_with_empty_data(
                "gather",
                {"op": "Gather", "batch_dims": batch_dims, "infer": Gather.infer},
            ),
            **result("res"),
        }
    ...
    def build_and_test_shape_inference(
        data_shape, indices_shape, axis, batch_dims, ref_shape
    ):
        nodes = {
    ...
    def test_axis_0_batch_dims_0(self):
        self.build_and_test_value_inference(
            axis=0,
            batch_dims=0,
            data=[1, 2, 3, 4, 5],
            indices=[0, 0, 4],
...
class TestElementwiseReverseInfer(UnitTestWithMockedTelemetry):
    ...
    def build_and_test_reverse_inference(
        data_shape, indices_shape, axis, batch_dims, out_shape, ref_shape
    ):
        ...
        nodes = {
            **shaped_parameter(
                "data", data_shape, {"reverse_infer": Parameter.reverse_infer}
            ),
            **shaped_parameter(
                "indices", indices_shape, {"reverse_infer": Parameter.reverse_infer}
            ),
            **valued_const_with_data("axis", int64_array(axis)),
            **regular_op_with_empty_data(
                "gather",
        ...
        Node(graph, "gather").in_port(in_port_with_defined_shape).data.set_shape(
            defined_shape
        )
        ...
        actual_shape = (
            Node(graph, "gather")
            .in_port(int(not in_port_with_defined_shape))
            .data.get_shape()
        )
