...
class TestCTCGreedyDecoder(unittest.TestCase):
    def test_infer1(self):
        ...
        self.assertTrue(
            np.array_equal(ref_output1_shape, res_output1_shape),
            "shapes do not match expected: {} and given: {}".format(
                ref_output1_shape, res_output1_shape
            ),
        )
    ...
    def test_infer_invalid1(self):
        ...
        ctcgreedydecoder_node = Node(graph, "ctcgreedydecoder_node")
        self.assertRaises(
            AssertionError, CTCGreedyDecoderSeqLenOp.infer, ctcgreedydecoder_node
        )
