...
class TestIf(unittest.TestCase):
    def test_simple_shape_inf(self):
        ...
        for idx, (cond, output_port_0_shape, output_port_1_shape) in enumerate(test_cases):
            with self.subTest(test_cases=idx):
                then_graph_nodes = {**regular_op_with_empty_data('param_1', {'type': 'Parameter',
                                        'kind': 'op', 'input_id': 1, 'shape': None, 'infer': Parameter.infer}),
                                    **regular_op_with_empty_data('param_2', {'type': 'Parameter',
                                        'kind': 'op', 'input_id': 2, 'shape': None, 'infer': Parameter.infer}),
                                    ...
                                    **regular_op_with_empty_data('mul', {'type': 'Mul', 'kind': 'op', 'op': 'Mul',
                                                                'infer': lambda node: eltwise_infer(node,
                                                                                                Mul.operation)}),
                                    **regular_op_with_empty_data('res1', {'kind': 'op', 'type': 'Result',
                                                            'op': 'Result', 'infer': lambda x: 0, 'output_id': 0}),
                                    **regular_op_with_empty_data('res2', {'kind': 'op', 'type': 'Result',
                                                            'op': 'Result', 'infer': lambda x: 0, 'output_id': 1})}
                ...
                else_graph_nodes = {**regular_op_with_empty_data('param_1', {'type': 'Parameter',
                                                                            'kind': 'op', 'input_id': 1,
                ...
                # shape of the "then" branch is [3] and shape of the "else" branch is [2],
                # so the output shape is "[dynamic]"
