...
class TestIf:
    @pytest.mark.parametrize(
        "cond, output_port_0_shape, output_port_1_shape",
        [
    ...
    def test_simple_shape_inf(self, cond, output_port_0_shape, output_port_1_shape):
        ...
        else_graph_nodes = {
            ...
            **regular_op_with_empty_data(
                "identity", {"kind": "op", "op": "Identity", "infer": Identity.infer}
            ),
            **regular_op_with_empty_data(
                "identity_1", {"kind": "op", "op": "Identity", "infer": Identity.infer}
            ),
            **regular_op_with_empty_data(
                "res1",
                {
    ...
    def test_fake_results(self):
        ...
        external_graph_nodes = {
            ...
            **valued_const_with_data(
                "input_1", int64_array([1, 2, 3, 3, 2, 3]).reshape((2, 3))
            ),
            **regular_op_with_empty_data(
                "if",
                {
        ...
        npt.assert_array_equal(
            Node(graph, "if").out_port(0).data.get_shape(), int64_array([2, 3])
        )
