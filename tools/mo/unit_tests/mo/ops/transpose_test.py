...
class TestTransposeOp:
    nodes_attributes = {
        "parameter": {"kind": "op", "op": "Parameter", "shape": input_shape},
        "data_1": {"kind": "data", "shape": input_shape, "value": None},
        "order_const": {"kind": "op", "op": "Const", "shape": np.array([4])},
        "order_data": {"kind": "data", "shape": np.array([4])},
        "transpose": {
            "type": "Transpose",
            "op": "Transpose",
            "reverse_order": False,
        ...
        "data_2": {"kind": "data", "shape": None, "value": None},
    }
    ...
    @pytest.mark.parametrize(
        "order", [list(order) for order in list(itertools.permutations(np.arange(4)))]
    )
...
class TestTransposeReverseInfer(unittest.TestCase):
    ...
    def build_and_test_reverse_inference(order, out_shape, ref_shape):
        nodes = {
            **shaped_parameter(
                "data", None, {"reverse_infer": Parameter.reverse_infer}
            ),
    ...
    def test_reverse_infer_2(self):
        self.build_and_test_reverse_inference(
            order=[0, 3, 1, 2], out_shape=[44, 32, 77, 1], ref_shape=[44, 77, 1, 32]
        )

    def test_reverse_infer_3(self):
        self.build_and_test_reverse_inference(
            order=[0, 2, 3, 1], out_shape=[44, 32, 77, 1], ref_shape=[44, 1, 32, 77]
        )
