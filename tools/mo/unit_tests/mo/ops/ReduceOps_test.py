...
nodes_attributes = {
    **regular_op_with_shaped_data(
        "data",
        [1, 3, 224, 224],
        {"type": "Parameter", "value": None, "_out_port_data_type": {0: np.float32}},
    ),
    **valued_const_with_data("axis", int64_array(0)),
    **regular_op_with_shaped_data(
        "reduce_lp", None, {"op": "ReduceLp", "type": None, "name": "my_reduce_lp"}
    ),
    **regular_op_with_shaped_data(
        "identity", None, {"op": "Identity", "name": "identity"}
    ),
    **result("output"),
}
...
class TestReduceLpTest:
    ...
    def test_reduce_lp(self, shape, axes, keepdims, p):
        ...
        reduced = np.power(
            np.sum(a=np.abs(np.power(data, p)), axis=tuple(axes), keepdims=keepdims),
            1 / p,
        )
    ...
    def test_reduce_dynamic(self, shape, axes, keepdims, p):
        ...
        # values
        fully_undefined = np.ma.masked_array(
            reduced_tensor, mask=np.ones(reduced_tensor.shape)
        )
        axis = int64_array(axes)
        p = int64_array(p)
        graph = build_graph(
