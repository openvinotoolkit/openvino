...
class TestSwitch(unittest.TestCase):
    def test_switch_infer_with_condition(self):
        nodes = [
            ...
            (
                "switch",
                {"type": "Switch", "kind": "op", "op": "Switch", "infer": Switch.infer},
            ),
            (
                "switch_data_0",
        ...
        graph_ref = build_graph_with_attrs(
            ...
            update_nodes_attributes=[
                (
                    "switch_data_0",
                    {"shape": np.array([3, 3]), "value": np.zeros((3, 3))},
                ),
                (
                    "switch_data_1",
                    {"shape": np.array([3, 3]), "value": np.zeros((3, 3))},
        ...
        (flag, resp) = compare_graphs(
            graph, graph_ref, "switch_data_0", check_op_attrs=True
        )
    ...
    def test_switch_infer_no_condition(self):
        ...
        graph_ref = build_graph_with_attrs(
            ...
            edges_with_attrs=edges,
            update_nodes_attributes=[
                ("switch_data_0", {"shape": np.array([1, 2, 1])}),
    ...
    def test_switch_cf_infer_no_condition(self):
        ...
        me_mock.assert_has_calls(
            [call("switch_data_0", True), call("switch_data_1", True)], any_order=True
        )
    ...
    def test_switch_cf_true_both_ports(self):
        ...
        node.control_flow_infer(node, True, me_mock)
        me_mock.assert_has_calls(
            [call("switch_data_0", False), call("switch_data_1", True)], any_order=True
        )
    ...
    def test_switch_cf_false_both_ports(self):
        ...
        node.control_flow_infer(node, True, me_mock)
        me_mock.assert_has_calls(
            [call("switch_data_0", True), call("switch_data_1", False)], any_order=True
        )
