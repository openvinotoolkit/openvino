...
from openvino.tools.mo.ops.pad import AttributedPad, Pad
...
class TestPadOps(unittest.TestCase):
    ...
    edge_attrs = [("data_in", "pad"), ("pad", "data_out")]
    ...
    def test_attribute_pad_no_infer(self):
        graph = build_graph(
            ...
            {
                "pad": {
                    "pads": np.array([[0, 0], [0, 0], [1, 3], [2, 4]], dtype=np.int64)
                }
            },
    ...
    def test_two_inputs_dynamic_value_infer(self):
        ...
        mask[0][0][1][2] = True
        ref_value = np.ma.masked_array(
            np.zeros(out_shape, dtype=np.int64), mask=mask, dtype=np.int64
        )
        ref_value[0][0][1][3] = 3
