...
class TestTopKInfer(unittest.TestCase):
    def setUp(self):
        nodes = {
            ...
            **regular_op_with_shaped_data(
                "topk", None, {"op": "TopK", "type": "TopK", "name": "topk", "axis": 1}
            ),
        ...
        nodes2 = {
            ...
            **regular_op_with_shaped_data(
                "topk", None, {"op": "TopK", "type": "TopK", "name": "topk", "axis": 2}
            ),
            "topk_d2": {"kind": "data", "shape": None, "value": None},
            **result("output_1"),
            **result("output_2"),
        }
        self.graph2 = build_graph(
            nodes2,
            [
                *connect("data", "0:topk"),
                *connect("k", "1:topk"),
    ...
    def test_topk_infer_opset1(self):
        ...
        self.assertTrue(
            np.array_equal(
                topk_node.out_port(0).data.get_shape(), int64_array([20, 10, 4])
            )
        )
        self.assertTrue(
            np.array_equal(
                topk_node.out_port(1).data.get_shape(), int64_array([20, 10, 4])
            )
        )
    ...
    def test_topk_infer_with_dynamic_k(self):
        ...
        self.assertTrue(
            np.array_equal(
                ...
                shape_array([4, 10, dynamic_dimension_value]),
            )
        )
        self.assertTrue(
            np.array_equal(
