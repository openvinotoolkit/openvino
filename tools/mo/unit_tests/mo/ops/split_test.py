...
from openvino.tools.mo.ops.split import (
    AttributedSplit,
...
class TestSplitOp(unittest.TestCase):
    ...
    def test_split_dynamic_shape_infer(self):
        ...
        self.assertTrue(
            strict_compare_tensors(
                Node(graph, "split_output_0_data").shape, shape_array(output_shape)
            )
        )
...
class TestAttributedVariadicSplitOp(unittest.TestCase):
    ...
    def test_splitv_zero(self):
        graph = build_graph(
            ...
            self.edges,
            {
                ...
                "split_op": {
                    "axis": np.array(2),
                    "split_lengths": np.array([2, 13, 10, 0]),
                    "out_ports_count": 4,
                },
            },
        )
    ...
    def test_splitv_dynamic_input(self):
        graph = build_graph(
            ...
            {
                "split_input_data": {
                    "shape": shape_array([2, 12, dynamic_dimension_value, 30])
                },
                "split_op": {
                    "axis": np.array(2),
                    "split_lengths": np.array([2, 13, 10]),
                    "out_ports_count": 4,
                },
            },
        )
    ...
    def test_splitv_zero_not_last(self):
        graph = build_graph(
            ...
            self.edges,
            {
                ...
                "split_op": {
                    "axis": np.array(2),
                    "split_lengths": np.array([2, 13, 0, 10]),
                    "out_ports_count": 4,
                },
            },
        )
    ...
    def test_splitv_2_zero_not_last(self):
        graph = build_graph(
            ...
            self.edges,
            {
                ...
                "split_op": {
                    "axis": np.array(2),
                    "split_lengths": np.array([2, 13, 0, 0, 10]),
                    "out_ports_count": 5,
                },
            },
        )
...
class TestVariadicSplitOp:
    ...
    @pytest.mark.parametrize("axis", [int64_array(2), int64_array([2])])
    def test_variadic_split_axis(self, axis):
        ...
        for out in range(ont_nodes_count):
            assert np.all(
                node.out_node(out).shape == int64_array([2, 12, lengths[out], 30])
            )
    ...
    @pytest.mark.parametrize("axis", [int64_array([[2], [2]]), int64_array([2, 2])])
...
class TestSplitReverseInfer(unittest.TestCase):
    ...
    def test_split_reverse_infer(self):
        ...
        graph = build_graph(
            ...
            TestSplitOp.edges,
            {
                "split_input_data": {"shape": None, "value": None},
                "split_op": {
                    "axis": np.array(axis),
                    "num_splits": np.array(num_splits),
                },
