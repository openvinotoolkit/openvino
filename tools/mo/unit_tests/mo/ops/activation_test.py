...
from openvino.tools.mo.ops.activation_ops import Elu, Mish, SoftPlus, SoftSign, Swish


class TestActivationOp(unittest.TestCase):
    nodes_attributes = {
        "node_1": {"shape": np.array([4]), "value": None},
        "activation_node": {"op": "Activation", "kind": "op", "operation": None},
        "node_3": {"shape": None},
    }
    ...
    def test_activation_elu_infer(self):
        graph = build_graph(
            ...
            [("node_1", "activation_node"), ("activation_node", "node_3")],
            {
                "node_1": {"value": np.array([6, -4, -2, -1])},
                "activation_node": {
                    "operation": "elu",
                    "alpha": 1.0,
                },
        ...
        exp_value = np.array([6.0, -0.98168436, -0.86466472, -0.63212056])
    ...
    def test_activation_softplus_infer(self):
        graph = build_graph(
            ...
            [("node_1", "activation_node"), ("activation_node", "node_3")],
            {
                "node_1": {"value": np.array([-1.0, 0.0, 1.0, 20.0])},
                "activation_node": {
                    "op": "SoftPlus",
    ...
    def test_activation_softsign_infer(self):
        graph = build_graph(
            self.nodes_attributes,
            edges=[("node_1", "activation_node"), ("activation_node", "node_3")],
            update_attributes={
                "node_1": {"value": np.array([1.0, -1.0, 3.5, -5.8])},
                "activation_node": {"op": "SoftSign", "operation": SoftSign.operation},
                "node_3": {"value": None},
            },
        )
        graph.graph["layout"] = "NCHW"
