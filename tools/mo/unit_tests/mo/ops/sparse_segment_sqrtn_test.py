...
# graph 1
nodes_attributes1 = {
    "input_data": {"shape": None, "value": None, "kind": "data"},
...
edges1 = [
    ("input_data", "sparse_segment_sqrtn_node", {"in": 0}),
...
inputs1 = {
    "input_data": {"shape": int64_array([20, 4, 5]), "value": None},
...
inputs2 = {
    "input_data": {
        "shape": int64_array([3, 4]),
        "value": np.array([[1, 2, 3, 4], [-1, -2, -3, -4], [5, 6, 7, 8]], dtype=float),
    },
    "input_indices": {
        "shape": int64_array([3]),
        "value": np.array([0, 2, 1, 1, 2], dtype=float),
    },
    "input_segment_ids": {
        "shape": int64_array([3]),
        "value": np.array([0, 0, 0, 0, 2], dtype=float),
    },
}
...
class TestSparseSegmentSqrtN(unittest.TestCase):
    def test_partial_infer(self):
        ...
        self.assertTrue(
            np.array_equal(ref_output_segments_shape, res_output_segments_shape),
    ...
    def test_incorrect_shapes(self):
        ...
        sparse_segment_sqrtn_node = Node(graph, "sparse_segment_sqrtn_node")
        self.assertRaises(
            AssertionError, SparseSegmentSqrtN.infer, sparse_segment_sqrtn_node
        )
    ...
    def test_infer_constant_input_sqrtn(self):
        ...
        ref_output_segments_value = np.array(
            [[2, 2, 2, 2], [0, 0, 0, 0], [5, 6, 7, 8]], dtype=float
        )
        ...
        self.assertTrue(
            np.array_equal(ref_output_segments_value, res_output_segments_value),
            "Shapes do not match expected: {} and given: {}".format(
