...
class ExitTest(unittest.TestCase):
    def test_exit_static(self):
        graph = build_graph(
            nodes_attrs=graph_nodes,
            edges=[*connect("input", "exit"), *connect("exit", "output")],
        ...
        self.assertTrue(
            np.ma.allequal(exit_node.out_port(0).data.get_shape(), in_node.shape)
        )
    ...
    def test_exit_dynamic(self):
        ...
        in_node.shape = np.ma.masked_array(
            shape, mask=shape == -1, fill_value=dynamic_dimension_value
        )
