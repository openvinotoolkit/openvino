...
from openvino.tools.mo.ops.embedding_bag import (
...
nodes = {
    ...
    **regular_op_with_shaped_data(
        "indices1d",
        [100],
        {"type": "Parameter", "value": None, "_out_port_data_type": {0: np.int32}},
    ),
    **regular_op_with_shaped_data(
        "indices2d",
        [30, 3],
...
class TestEmbeddingInfer(unittest.TestCase):
    def test_embedding_bag_offsets_sum(self):
        ...
        self.assertTrue(
            np.array_equal(eb_node.out_port(0).data.get_shape(), int64_array([30, 8]))
        )
