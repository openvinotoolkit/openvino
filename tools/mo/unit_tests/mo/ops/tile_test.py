...
from openvino.tools.mo.ops.tile import AttributedTile, Tile
...
class TestTileInfer(unittest.TestCase):
    def test_tile_infer_correct(self):
        graph = build_graph(
            nodes_attributes, edges, {"tile_values": {"value": np.array([7, 1, 1, 1])}}
        )
        tile_node = Node(graph, "tile")
        Tile.infer(tile_node)
        self.assertTrue(
            np.all(np.array([70, 20, 30, 40]) == graph.node["tile_out"]["shape"])
        )

    def test_tile_infer_correct_2(self):
        graph = build_graph(
            nodes_attributes, edges, {"tile_values": {"value": np.array([1, 7, 1, 1])}}
        )
        tile_node = Node(graph, "tile")
        Tile.infer(tile_node)
        self.assertTrue(
            np.all(np.array([10, 140, 30, 40]) == graph.node["tile_out"]["shape"])
        )
    ...
    def test_tile_infer_all_ones(self):
        graph = build_graph(
            nodes_attributes, edges, {"tile_values": {"value": np.array([1, 1, 1, 1])}}
        )
        tile_node = Node(graph, "tile")
        Tile.infer(tile_node)
        self.assertTrue(
            np.all(np.array([10, 20, 30, 40]) == graph.node["tile_out"]["shape"])
        )

    def test_tile_infer_two_non_one(self):
        graph = build_graph(
            nodes_attributes, edges, {"tile_values": {"value": np.array([2, 1, 1, 2])}}
        )
        tile_node = Node(graph, "tile")
        Tile.infer(tile_node)
        self.assertTrue(
            np.all(np.array([20, 20, 30, 80]) == graph.node["tile_out"]["shape"])
        )

    def test_tile_infer_three_non_one(self):
        graph = build_graph(
            nodes_attributes, edges, {"tile_values": {"value": np.array([2, 1, 5, 2])}}
        )
        tile_node = Node(graph, "tile")
        Tile.infer(tile_node)
        self.assertTrue(
            np.all(np.array([20, 20, 150, 80]) == graph.node["tile_out"]["shape"])
        )
    ...
    def test_tile_infer_none_input_shape(self):
        graph = build_graph(
            nodes_attributes,
            edges,
            {"data": {"shape": None}, "tile_values": {"value": np.array([1, 7, 1, 1])}},
        )
        tile_node = Node(graph, "tile")
    ...
    def test_tile_infer_values_test(self):
        ...
        Tile.infer(tile_node)
        self.assertTrue(
            np.all(np.tile(input_data, tile_values) == graph.node["tile_out"]["value"])
        )
    ...
    def test_tile_infer_undefined_tile_values(self):
        graph = build_graph(nodes_attributes, edges, {"tile_values": {"value": None}})
        tile_node = Node(graph, "tile")
        self.assertRaises(AssertionError, Tile.infer, tile_node)
    ...
    def test_tile_infer_shapes_alignment(self):
        ...
        Tile.infer(tile_node)
        self.assertTrue(
            np.all(np.array([10, 20, 60, 120]) == graph.node["tile_out"]["shape"])
        )

    def test_tile_infer_one_input_correct(self):
        graph = build_graph(
            nodes_attributes, attributed_edges, {"tile": {"axis": 1, "tiles": 7}}
        )
        tile_node = Node(graph, "tile")
    ...
    def test_tile_infer_one_input_correct_missing_axis(self):
        graph = build_graph(nodes_attributes, attributed_edges, {"tile": {"tiles": 7}})
        tile_node = Node(graph, "tile")
        self.assertRaises(AssertionError, AttributedTile.infer, tile_node)

    def test_tile_infer_one_input_correct_missing_tiles(self):
        graph = build_graph(nodes_attributes, attributed_edges, {"tile": {"axis": 1}})
        tile_node = Node(graph, "tile")
        self.assertRaises(AssertionError, AttributedTile.infer, tile_node)
