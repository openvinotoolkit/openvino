...
class TestNonMaxSuppressionInfer(unittest.TestCase):
    def setUp(self):
        nodes = {
            ...
            **regular_op_with_shaped_data(
                "scores", [10, 5, 100], {"type": "Parameter"}
            ),
            **valued_const_with_data("max_output_per_class", int64_array(7)),
            **regular_op(
                "nms",
                {"op": "NonMaxSuppression", "type": "NonMaxSuppression", "name": "nms"},
            ),
        ...
        self.graph = build_graph(
            ...
            [
                ...
                *connect(
                    "nms:0", "nms_data_0", front_phase=True
                ),  # Use this WA for correct creating operation
                *connect(
                    "nms_data_0", "output_0", front_phase=True
                ),  # with multiple outputs
            ],
            nodes_with_edges_only=True,
        )

        self.graph_nms_5_2_outs = build_graph(
            nodes,
            [
                *connect("boxes", "0:nms"),
                *connect("scores", "1:nms"),
        ...
        self.graph_nms_5_3_outs = build_graph(
            nodes,
            [
                *connect("boxes", "0:nms"),
                *connect("scores", "1:nms"),
    ...
    def test_nms_infer_i32_opset4(self):
        ...
        self.assertTrue(
            np.array_equal(nms_node.out_port(0).data.get_shape(), [10 * 5 * 7, 3])
        )
        self.assertTrue(nms_node.out_port(0).get_data_type() == np.int32)
    ...
    def test_nms_infer_i32_opset5_2_outs(self):
        ...
        self.assertTrue(
            np.array_equal(
                nms_node.out_port(1).data.get_shape(),
                shape_array([dynamic_dimension_value, 3]),
            )
        )
        self.assertTrue(nms_node.out_port(0).get_data_type() == np.int32)
