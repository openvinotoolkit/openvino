...
from openvino.tools.mo.middle.TensorIteratorInput import (
    BackEdgeSimpleInputMatcher,
    SimpleInputMatcher,
    SmartInputMatcher,
)
...
class SmartInputMatcherTests(unittest.TestCase):
    def test(self):
        ...
        graph = build_graph_with_attrs(
            ...
            update_edge_attrs={
                ("range_data", "TensorArrayScatter", 0): {"in": 1},
        ...
        graph_ref = build_graph_with_attrs(
            ...
            edges_with_attrs=[
                ("ta_size", "TensorIteratorInput", {"in": 0}),
        ...
        (flag, resp) = compare_graphs(
            graph, graph_ref, "TensorArrayRead_data", check_op_attrs=True
        )
...
class SimpleInputMatcherTest(unittest.TestCase):
    def test(self):
        ...
        graph_ref = build_graph_with_attrs(
            ...
            edges_with_attrs=[
                ("in_node", "TensorIteratorInput"),
                ("TensorIteratorInput", "Enter_data"),
            ],
        )
        (flag, resp) = compare_graphs(
            graph, graph_ref, "Enter_data", check_op_attrs=True
        )
        self.assertTrue(flag, resp)
    ...
    def test1(self):
        ...
        graph_ref = build_graph_with_attrs(
            ...
            new_nodes_with_attrs=[
                ...
                ("TensorIteratorInput", {"kind": "op", "op": "TensorIteratorInput"}),
                (
                    "TensorIteratorInput_data",
                    {"kind": "data", "shape": np.array([1, 3])},
                ),
            ],
            new_edges_with_attrs=[
                ("TensorIteratorInput_data", "TensorIteratorInput"),
