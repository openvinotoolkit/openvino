...
nodes = {
    ...
    **regular_op_with_shaped_data(
        ...
        [1, 2, 34, 56],
        {
            ...
            "op": "TensorIterator",
            "output_port_map": [
                {"axis": 2, "start": 0, "end": -1, "stride": 1, "external_port_id": 0}
            ],
            "input_port_map": [
                {"axis": 2, "start": -1, "end": 0, "stride": -1, "external_port_id": 0}
            ],
        },
    ),
    **valued_const_with_data("inverse_seq_len", np.array([34])),
    **regular_op_with_empty_data(
        "inverse_shapeof", {"type": "ShapeOf", "op": "ShapeOf"}
    ),
    **regular_op_with_empty_data(
        "inverse_gather_batch", {"type": "Gather", "op": "Gather"}
    ),
    **regular_op_with_empty_data(
        "inverse_gather_seq", {"type": "Gather", "op": "Gather"}
    ),
    **regular_op_with_empty_data(
        "inverse_broadcast", {"type": "Broadcast", "op": "Broadcast"}
    ),
    **regular_op_with_shaped_data(
        "inverse_reverse",
        [1, 2, 34, 56],
