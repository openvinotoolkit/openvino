...
from openvino.tools.mo.middle.quantize_dequantize_linear_resolver import (
    QuantizeDequantizeLinearResolver,
)
...
nodes_attributes = {
    ...
    **regular_op_with_shaped_data(
        "input", [1, 2, 2], {"op": "Parameter", "type": "Parameter"}
    ),
    ...
    **regular_op_with_empty_data(
        "const_cast", {"op": "Cast", "type": "Convert", "dst_type": np.uint8}
    ),
    ...
    **regular_op_with_empty_data(
        "non_const_cast", {"op": "Cast", "type": "Convert", "dst_type": np.uint8}
    ),
    **result("result"),
}
