...
from openvino.tools.mo.middle.ReluQuantizeFuse import (
...
class ReluQuantizeFuseTests(unittest.TestCase):
    def test_classic_i8_positive_case(self):
        graph = build_graph(
            ...
            i8_edges,
            {
                "const_1_d": {"value": np.zeros([1, 2, 3, 4])},
                "quantize": {"levels": 256},
            },
    ...
    def test_classic_i8_negative_case(self):
        graph = build_graph(
            ...
            i8_edges,
            {
                "const_1_d": {"value": np.full([1, 2, 3, 4], -1)},
                "quantize": {"levels": 256},
            },
            nodes_with_edges_only=True,
        )
    ...
    def test_classic_i1_negative_case(self):
        ...
        self.assertTrue(flag, resp)
        np.array_equal(
            np.full([1, 2, 3, 4], float("-inf"), dtype=np.float32),
