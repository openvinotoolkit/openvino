...
nodes = [
    ...
    (
        "maximum_y_const",
        dict(kind="op", op="Const", value=np.array(12.0e-13, dtype=np.float32)),
    ),
    ("maximum_y_data", dict(kind="data", value=np.array(12.0e-13, dtype=np.float32))),
    ("rsqrt_pow", dict(kind="data", value=-0.5)),
    ("rsqrt", dict(kind="op", op="Pow")),
    ("rsqrt_data", dict(kind="data")),
    ("square_pow", dict(kind="op", op="Const", value=2.0)),
    ("square_pow_data", dict(kind="data", value=2.0)),
...
class TestL2NormToNormTest:
    @pytest.mark.parametrize(
        "input_shape, axes, layout",
        [
            (
                int64_array([2, 3]),
                int64_array([1]),
                "NCHW",
            ),  # NC layout, normalize C dimension
            (
                int64_array([2, 3]),
                int64_array([1]),
                "NHWC",
            ...
            (
                ...
                int64_array([1]),
                "NCHW",
            ),  # NCH layout, normalize C dimension
            (
                int64_array([2, 3, 5]),
                int64_array([1]),
                "NHWC",
            ...
            (
                ...
                int64_array([-1, -2]),
                "NHWC",
            ),  # NCH layout, normalize CH dimensions
            (
                int64_array([2, 3, 5]),
                int64_array([-1, -2]),
                "NCHW",
            ...
            (
                ...
                int64_array([1]),
                "NCHW",
            ),  # NCHW layout, normalize C dimension
            (
                int64_array([2, 3, 5, 7]),
                int64_array([-1]),
                "NHWC",
            ),  # NHWC layout, normalize C dimension
            (
                int64_array([2, 3, 5, 7]),
                int64_array([3]),
                "NHWC",
            ...
            (
                ...
                int64_array([-1, 1, 2]),
                "NCHW",
            ),  # NCHW layout, normalize CHW dimensions
            (
                int64_array([2, 3, 5, 7]),
                int64_array([-3, -2, -1]),
                "NHWC",
            ),  # NCHW layout, normalize HWC dimensions
        ],
    )
    def test_positive(self, input_shape, axes, layout):
        ...
        assert (
            graph.node[graph.get_nodes_with_attributes(type="NormalizeL2")[0]]["name"]
            == "l2_norm_name"
        )
        assert flag, resp
