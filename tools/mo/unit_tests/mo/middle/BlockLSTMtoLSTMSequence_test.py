...
from openvino.tools.mo.middle.BlockLSTMtoLSTMSequence import (
    BlockLSTMtoLSTMSequenceSingleFirstOutput,
)
...
nodes_attributes = {
    ...
    **valued_const_with_data(
        "bias", np.array([2, 4, 6, 8, 10, 12, 14, 16], dtype=np.float32)
    ),
    **regular_op_with_empty_data(
        "h_init_state", {"op": "Parameter", "type": "Parameter"}
    ),
    **regular_op_with_empty_data(
        "c_init_state", {"op": "Parameter", "type": "Parameter"}
    ),
    **regular_op_with_empty_data(
        "block_lstm", {"op": "BlockLSTM", "type": None, "forget_bias": 1}
    ),
    **result("result"),
    **valued_const_with_data(
        "weights_normalized",
        np.array(
            [[5, 13], [6, 14], [1, 9], [2, 10], [3, 11], [4, 12], [7, 15], [8, 16]],
            dtype=np.float32,
        ),
    ),
    **valued_const_with_data(
        "bias_normalized", np.array([11, 13, 2, 4, 6, 8, 14, 16], dtype=np.float32)
    ),
}
