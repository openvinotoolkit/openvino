...
from openvino.tools.mo.middle.passes.eliminate import (
    mark_const_producer_nodes,
    mark_output_reachable_nodes,
)
...
class TestEliminatePass(unittest.TestCase):
    ...
    def test_remove_node_from_graph(self):
        ...
        graph = build_graph(
            nodes_attributes,
            [("placeholder_1", "node_1"), ("node_1", "node_2"), ("node_2", "node_3")],
            nodes_with_edges_only=True,
        )
        graph.erase_node(Node(graph, "node_2"))

        self.assertListEqual(
            sorted(["placeholder_1", "node_1", "node_3"]), sorted(graph.nodes())
        )
