...
class BFSTests(unittest.TestCase):
    def test_forward_bfs_simple(self):
        ...
        self.assertTrue(
            len(res) == 1 and res[0].id == "add_1", "Add operation was not found by bfs"
        )
        ...
        self.assertTrue(
            len(res) == 0,
            "No one node should be found! But bfs found {} nodes".format(len(res)),
        )

        res = forward_bfs(
            Node(graph, "placeholder_1_data"), ["ScaleShift"], ["Mul", "Add"]
        )
        self.assertTrue(
            len(res) == 1 and res[0].id == "mul_1",
            "BFS should find only one Mul operation",
        )
    ...
    def test_backward_bfs_simple(self):
        ...
        res = backward_bfs(
            Node(graph, "add_1_data"), ["Add", "ScaleShift", "Mul"], ["Parameter"]
        )
        self.assertTrue(
        ...
        res = backward_bfs(
            Node(graph, "add_1_data"), ["Add", "Mul"], ["Parameter", "ScaleShift"]
        )
        self.assertTrue(
    ...
    def test_forward_bfs_hard(self):
        ...
        res = forward_bfs(
            Node(graph, "placeholder_1"), ["ScaleShift", "Mul", "Add"], ["Concat"]
        )
        self.assertTrue(
        ...
        self.assertTrue(
            len(res) == 2
            and all([res[x].id in ["add_1", "add_2"] for x in range(len(res))]),
    ...
    def test_backward_bfs_hard(self):
        ...
        res = backward_bfs(
            Node(graph, "concat_1"), ["ScaleShift", "Mul", "Add"], ["Parameter"]
        )
    ...
    def test_backward_bfs_multi_consumer_data_nodes(self):
        ...
        graph = build_graph(
            {
                **regular_op_with_shaped_data("parameter", [1], {"op": "Parameter"}),
        ...
        self.assertTrue(
            len(res) == 1, 'The multi-consumer data node "mul_d" was not followed'
        )
