...
from openvino.tools.mo.middle.passes.fusing.fuse_grouped_conv import (
...
nodes = {
    **regular_op_with_shaped_data(
        "placeholder1", [1, 16, 10, 10], {"type": "Parameter"}
    ),
    ...
    **shaped_const_with_data(
        "split_2_in_const_weights", int64_array([3, 3, 4, 16]), {"type": "Const"}
    ),
    ...
    **regular_op_with_shaped_data(
        ...
        [1, 4, 8, 8],
        {
            ...
            "pad": np.array([2, 2]),
            "stride": np.array([2, 2]),
            "get_weights_permute": PermuteAttrs.Permutation(
                perm=int64_array([3, 2, 0, 1]), inv=int64_array([2, 3, 1, 0])
            ),
            "group": 1,
            "output": 4,
    ...
    **regular_op_with_shaped_data(
        "concat", [1, 16, 8, 8], {"type": "Concat", "axis": np.array(1)}
    ),
    **regular_op_with_shaped_data(
        "fused_group_conv",
        [1, 16, 8, 8],
    ...
    **shaped_const_with_data(
        "new_weights_const", int64_array([3, 3, 4, 16]), {"type": "Const"}
    ),
    **result("result"),
}
...
class FuseGroupedConvTest(unittest.TestCase):
    def test_fuse_grouped_conv(self):
        graph = build_graph(
            nodes,
            [
                *connect("placeholder1", "0:split_1"),
                *connect("split_1_axis", "1:split_1"),
        ...
        graph_ref = build_graph(
            nodes,
            [
                *connect("placeholder1", "0:fused_group_conv"),
