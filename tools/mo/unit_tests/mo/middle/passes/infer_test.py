...
from openvino.tools.mo.middle.passes.infer import (
...
class TestInferPass(UnitTestWithMockedTelemetry):
    def test_override_placeholder_shapes(self):
        ...
        graph = build_graph(
            nodes_attributes,
            [("node_1", "node_2"), ("node_2", "op_output")],
            {
                "node_2": {"shape": None},
    ...
    def test_override_placeholder_shapes_batch_is_not_set(self):
        ...
        self.assertTrue(
            np.array_equal(self.nodes["placeholder_1"]["shape"], res_shape_1)
        )
        self.assertTrue(
            np.array_equal(self.nodes["placeholder_2"]["shape"], res_shape_2)
        )
    ...
    def test_override_placeholder_shapes_real_inputs_and_batch(self):
        ...
        graph = build_graph(self.nodes, self.edges)
        shapes = {
            "placeholder_1": [{"shape": np.array([1, 2, 3, 4])}],
    ...
    def test_override_placeholder_shapes_real_inputs_and_batch_2(self):
        ...
        graph = build_graph(self.nodes, self.edges)
        shapes = {
            "placeholder_1": [{"shape": None}],
            "placeholder_2": [{"shape": None}],
        }
        batch = 4
        ...
        np.testing.assert_array_equal(
            graph.node["placeholder_1"]["shape"], np.array([4, 2, 3, 4])
        )
        np.testing.assert_array_equal(
            graph.node["placeholder_2"]["shape"], np.array([4, 5, 6, 7])
        )
