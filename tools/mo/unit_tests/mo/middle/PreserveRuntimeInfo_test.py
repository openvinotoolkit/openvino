...
edges = [
    *connect("placeholder1", "0:add"),
    *connect("placeholder2", "1:add"),
    *connect("add", "result"),
]
edges_with_transpose = [
    *connect("placeholder1", "0:transpose_parameter"),
...
class TestPreserveRuntimeInfoTest:
    @pytest.mark.parametrize(
        "nhwc_to_nchw_order, nchw_to_nhwc_order, add_permutation_attrs",
        [
    ...
    def test_transpose_insert(
        self, nhwc_to_nchw_order, nchw_to_nhwc_order, add_permutation_attrs
    ):
        graph_nodes = {
            **valued_const_with_data(
            ...
            **valued_const_with_data(
                "transpose_result_order", np.array(nchw_to_nhwc_order)
            ),
        }
        ...
        graph_nodes.update(
            {
                ...
                **regular_op_with_shaped_data(
                    "result",
                    shape,
                    {"type": "Result", "rt_info": RTInfo(), "shape": shape},
                ),
                **regular_op_with_shaped_data(
                    "add",
                    add_shape,
        ...
        graph_ref = build_graph(
            graph_nodes, edges_with_transpose if add_permutation_attrs else edges
        )
        ...
        if add_permutation_attrs:
            ...
            param_node["permute_attrs"] = PermuteAttrs().update_attrs(
                attrs=[("shape", "output:0")]
            )
            param_node.out_node(0)[
                "permutation"
            ] = PermuteAttrs().get_nhwc_to_nchw_permutation(shape_len)
            result_node.in_node(0)[
                "permutation"
            ] = PermuteAttrs().get_nhwc_to_nchw_permutation(shape_len)
    ...
    def test_auto_disable_nhwc_to_nchw(self):
        ...
        graph_nodes = {
            ...
            **regular_op_with_shaped_data(
                "result", shape, {"type": "Result", "rt_info": RTInfo(), "shape": shape}
            ),
            **regular_op_with_shaped_data(
                "add",
                add_shape,
    ...
    @pytest.mark.parametrize(
        "nhwc_to_nchw_order, nchw_to_nhwc_order,add_permutation_attrs, fft_kind",
        [
    ...
    def test_transpose_insert_with_two_result_nodes(
        self, nhwc_to_nchw_order, nchw_to_nhwc_order, add_permutation_attrs, fft_kind
    ):
        shape_len = len(nhwc_to_nchw_order) if add_permutation_attrs else 3
        shape = np.array(range(shape_len))
        add_shape = shape if nhwc_to_nchw_order is None else shape[nhwc_to_nchw_order]
        graph = build_graph(
            nodes_attrs=nodes_for_case_with_two_results,
        ...
        if add_permutation_attrs:
            ...
            graph_ref = build_graph(
                nodes_for_case_with_two_results, edges_for_case_with_two_results
            )
        ...
        if add_permutation_attrs:
            shape_len = len(nhwc_to_nchw_order)
            param1_node["permute_attrs"] = PermuteAttrs().update_attrs(
                attrs=[("shape", "output:0")]
            )
            param1_node.out_node(0)[
                "permutation"
            ] = PermuteAttrs().get_nhwc_to_nchw_permutation(shape_len)
            result1_node.in_node(0)[
                "permutation"
            ] = PermuteAttrs().get_nhwc_to_nchw_permutation(shape_len)
            result2_node.in_node(0)[
                "permutation"
            ] = PermuteAttrs().get_nhwc_to_nchw_permutation(shape_len)
