...
from openvino.tools.mo.middle.ConvertGroupedStridedSlice import (
...
nodes_attributes = {
    ...
    "placeholder_begin_data": {
        "value": None,
        "shape": None,
        "kind": "data",
        "data_type": None,
    },
    "placeholder_end_data": {
        "value": None,
        "shape": None,
        "kind": "data",
        "data_type": None,
    },
    "placeholder_stride_data": {
        "value": None,
        "shape": None,
        "kind": "data",
        "data_type": None,
    ...
    "sslice_1": {
        ...
        "new_axis_mask": int64_array([0, 0, 0, 0]),
...
class TestConvertGroupedStridedSliceTests:
    def test_1(self):
        ...
        (flag, resp) = compare_graphs(
            graph, graph_ref, "concat_1_data", check_op_attrs=True
        )
        assert flag, resp
    ...
    @pytest.mark.parametrize(
        "input_shape, slices, output_shape",
        [
            (
                np.array([1, 227, 227, 54]),
                np.array(
                    [
                        slice(0, 1, 1),
                        slice(0, 227, 1),
    ...
    def test_9(self, input_shape, slices, output_shape):
        graph = build_graph(
            nodes_attrs=one_strided_slice_case_node_attributes,
        ...
        pattern.find_and_replace_pattern(graph)
        (flag, resp) = compare_graphs(
            graph, graph_ref, "op_output", check_op_attrs=True
        )
        assert flag, resp
    ...
    def test_11(self):
        graph = build_graph(
            ...
            ],
            {
                ...
                "sslice_2_data": {"shape": np.array([1, 227, 227, 17])},
                "sslice_3": {
                    "slices": [slice(0, 1, 1), slice(0, 227, 1), 12, slice(0, 19, 1)]
                },
                "sslice_3_data": {
                    "shape": shape_array([1, 227, dynamic_dimension_value, 19])
                },
                "concat_1_data": {
                    "shape": shape_array([1, 227, dynamic_dimension_value, 54])
                },
            },
        )
        graph.graph["layout"] = "NHWC"
    ...
    def test_12(self):
        graph = build_graph(
            ...
            ],
            {
                ...
                "sslice_1": {
                    "slices": np.array([slice(0, 1, 1), slice(0, 1, 1)]),
        ...
        (flag, resp) = compare_graphs(
            graph, graph_ref, "sslice_1_data", check_op_attrs=True
        )
        assert flag, resp
        (flag, resp) = compare_graphs(
            graph, graph_ref, "sslice_2_data", check_op_attrs=True
        )
        assert flag, resp
    ...
    def test_ss_1_new(self):
        ...
        graph_ref = build_graph(
            ...
            ],
            {
                ...
                "sslice_2/Unsqueeze_new_data": {
                    "shape": np.array([1, 1, 227, 227, 54])
                },
            },
            nodes_with_edges_only=True,
        )
