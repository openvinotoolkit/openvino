...
nodes_attributes = {
    ...
    **regular_op(
        "topk",
        {"op": "TopK", "type": "TopK", "sort": "index", "index_element_type": np.int32},
    ),
    **empty_data("topk_out_0_data"),
    **empty_data("topk_out_1_data"),
    **regular_op_with_empty_data(
        "topk_scalar",
        {"op": "Const", "type": "Const", "value": int64_array([1]), "shape": []},
    ),
    **regular_op_with_empty_data(
        "concat", {"op": "Concat", "type": "Concat", "axis": 1}
    ),
}
...
class ArgOpsToTopKTest(unittest.TestCase):
    ...
    def test_onnx_argmin_to_topk(self):
        graph = build_graph(
            nodes_attrs=nodes_attributes,
            edges=[*connect("input", "argmin"), *connect("argmin", "result")],
            nodes_with_edges_only=True,
        )
        ArgOpsToTopK().find_and_replace_pattern(graph)
        ref_graph = build_graph(
    ...
    def test_caffe_argmax_to_topk(self):
        graph = build_graph(
            nodes_attrs=nodes_attributes,
            edges=[*connect("input", "argmax"), *connect("argmax", "result")],
            update_attributes={"argmax": {"out_max_val": 1}},
            nodes_with_edges_only=True,
        )
        ArgOpsToTopK().find_and_replace_pattern(graph)
        ref_graph = build_graph(
