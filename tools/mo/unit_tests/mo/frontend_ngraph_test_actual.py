...
try:
    ...
    print(
        "No mock frontend API available, "
...
mock_needed = pytest.mark.skipif(
    not mock_available, reason="mock MO fe is not available"
)


def replaceArgsHelper(
    log_level="DEBUG",
    silent=False,
    model_name="abc",
...
class TestMainFrontend(unittest.TestCase):
    ...
    def test_simple_convert(self, mock_argparse):
        ...
        xml_file = (
            re.search(r"\[ SUCCESS \] XML file: (.*)", out).group(1).replace("\r", "")
        )
        bin_file = (
            re.search(r"\[ SUCCESS \] BIN file: (.*)", out).group(1).replace("\r", "")
        )
    ...
    @patch(
        ...
        return_value=replaceArgsHelper(
            _input="newIn1,newIn2", output="newOut1,newOut2"
        ),
    )
    ...
    @patch(
        "argparse.ArgumentParser.parse_args",
        return_value=replaceArgsHelper(
            _input="mock_input2,mock_input1", output="new_output2,mock_output1"
        ),
    )
    ...
    @patch(
        "argparse.ArgumentParser.parse_args",
        return_value=replaceArgsHelper(
            _input="newInput1,mock_input2", output="mock_output2,mock_output1"
        ),
    )
    ...
    @patch(
        "argparse.ArgumentParser.parse_args",
        return_value=replaceArgsHelper(_input="newIn1", input_shape="[1,2,3,4]"),
    )
    ...
    @mock_needed
    @patch(
        "argparse.ArgumentParser.parse_args", return_value=replaceArgsHelper(batch=123)
    )
    @pytest.mark.skip(reason="Unskip as 8301 will be merged")
