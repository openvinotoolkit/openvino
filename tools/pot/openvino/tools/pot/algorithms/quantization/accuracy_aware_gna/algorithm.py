...
class AccuracyAwareGNA(AccuracyAwareCommon):
    ...
    def __init__(self, config, engine):
        ...
        self._config['bias_correction_after_modify'] = self._config.get('bias_correction_after_modify', False)
        self._config['change_only_weights'] = self._config.get(
            'change_only_weights', True if self._config['target_device'] == 'GNA' else False)
        ...
        if self._config.bias_correction_after_modify:
            use_fast_bias = self._config.get('use_fast_bias', True)
            self._bias_algo = FastBiasCorrection(config, engine) if use_fast_bias else BiasCorrection(config, engine)
    ...
    def _request_alt_statistics(self, model):
        ...
        acc_stats_layout = self._minmax_algo.create_stats_layout(self._acc_fq_config, quantized_model, True)
        self._acc_weights_stats = compute_weights_stats(quantized_model, acc_stats_layout)

        del quantized_model

    def _get_fq_to_modify(self, model, nodes_names):
        change_only_weights = self._config['change_only_weights']
        ...
        if unify_conv_activation_weight:
            for fq_name in nodes_names:
                ...
                if output_node.type != 'Convolution':
                    target_nodes.append((None, fq))
                    continue

                fq_activation = nu.get_node_input(output_node, 0)
            ...
            for fq_name in nodes_names:
                ...
                if change_only_weights and not is_fq_for_weights:
                ...
                if not is_fq_for_weights and fq_output.type != 'Convolution':
                    continue
    ...
    def _modify_model_in_scope(self, model, nodes_names):
        self._acc_stats = self._minmax_algo._stats_collector.get_statistics_for_algorithm(self._minmax_algo.name)
