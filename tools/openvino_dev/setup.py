...
PKG_INSTALL_CFG = {
    ...
    "omz_tools": {
        "src_dir": OPENVINO_DIR
        / "thirdparty"
        / "open_model_zoo"
        / "tools"
        / "model_tools",  # noqa:E501
        "black_list": [],
        "prefix": "omz_tools",
        "extract_requirements": True,
        "extract_entry_points": True,
    ...
    def run(self):
        ...
        for cmp, cmp_data in PKG_INSTALL_CFG.items():
            ...
            lib_dir = (
                ...
                else f"lib/{PYTHON_VERSION}/site-packages"
            )
            src = BUILD_BASE / cmp_data.get("prefix") / lib_dir

            egg_info = list(src.glob("**/*.egg-info"))
            if egg_info:
                distributions = pkg_resources.find_distributions(
                    str(Path(egg_info[0]).parent)
                )
                for dist in distributions:
                    ...
                    if cmp_data.get("extract_requirements"):
                        ...
                        self.announce(
                            f"Install requires: {install_requires}", level=log.INFO
                        )
                        self.distribution.install_requires.extend(install_requires)
                        # conditional requirements {':<condition>': [requirements]}
                        conditionals_req = dict(
                            filter(
                                lambda x: x[0] is not None and x[0].split(":")[0] == "",
                                dmap.items(),
                            )
                        )
                        self.announce(
                            f"Install requires with marker: {conditionals_req}",
                            level=log.INFO,
                        )
                        for extra, req in conditionals_req.items():
                            ...
                            self.distribution.extras_require[extra].extend(
                                sorted(map(str, req))
                            )
                    ...
                    if cmp_data.get("extract_entry_points"):
                        for console_scripts in dist.get_entry_map("console_scripts"):
                            self.announce(
                                f"Entry point: {console_scripts}", level=log.INFO
                            )
            ...
            exclude = ignore_patterns(
                "*ez_setup*", "*__pycache__*", "*.egg-info*", *black_list
            )
...
description_md = (
    SCRIPT_DIR.parents[1] / "docs" / "dev" / "pypi_publish" / "pypi-openvino-dev.md"
)
md_files = [
    description_md,
    SCRIPT_DIR.parents[1] / "docs" / "dev" / "pypi_publish" / "pre-release-note.md",
]
...
setup(
    ...
    download_url=os.getenv(
        "WHEEL_DOWNLOAD_URL", "https://github.com/openvinotoolkit/openvino/tags"
    ),
