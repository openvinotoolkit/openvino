...
generateVector(size_t vec_len, uint32_t range = 10, uint32_t start_from = 1) {
    ...
    std::uniform_int_distribution<unsigned long> dist(start_from, range);
...
std::vector<ngraph::float16> inline generateF16Vector(size_t vec_len, uint32_t range = 10, uint32_t start_from = 1) {
...
std::vector<ngraph::bfloat16> inline generateBF16Vector(size_t vec_len, uint32_t range = 10, uint32_t start_from = 1) {
