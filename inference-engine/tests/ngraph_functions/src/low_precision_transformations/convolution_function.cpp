...
std::shared_ptr<ngraph::Function> ConvolutionFunction::getReferenceWithIncorrectWeights(
    ...
    const auto convolution = std::make_shared<ngraph::op::TypeRelaxed<ngraph::opset1::Convolution>>(
        dequantizationBefore.empty() ? fqOnData : deqBefore,
        isCorrect ? weights : subtract,
        ngraph::Strides{ 1, 1 },
        ngraph::CoordinateDiff{ 0, 0 },
        ngraph::CoordinateDiff{ 0, 0 },
        ngraph::Strides{ 1, 1 });
    ...
    if (!dequantizationAfter.multiply.empty()) {
        multiply = std::make_shared<ngraph::opset1::Multiply>(convolution,
            std::make_shared<ngraph::opset1::Constant>(precision, Shape{ 1, 1, 1, 1 }, dequantizationAfter.multiply.values[0]));
    }
    ...
    if (!dequantizationBefore.multiply.empty()) {
        replace_node(
            ...
            std::make_shared<ngraph::opset1::Constant>(precision, Shape{ 1, 1, 1, 1 }, dequantizationBefore.multiply.values[0]));
    }
