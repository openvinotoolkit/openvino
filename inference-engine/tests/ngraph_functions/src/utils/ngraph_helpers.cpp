...
std::vector<std::vector<std::uint8_t>> interpreterFunction(const std::shared_ptr<Function> &function,
    ...
    for (const auto &result : results) {
        ...
        if (outConvertType != element::Type_t::undefined)
            output = convertOutputPrecision(output, result->get_element_type(), outConvertType, shape_size(result->get_shape()));
    }
    ...
    switch (fromPrecision) {
        ...
        case element::Type_t::f32: {
            ...
            case element::Type_t::boolean: {
                return convertPrecision<float, bool>(output, elementsCount, element::Type(toPrecision).size());
            }
            default:
                throw std::runtime_error("convertOutputPrecision can't convert from: " + element::Type(fromPrecision).get_type_name() + " to: " +
                                                                                                        element::Type(toPrecision).get_type_name());
        ...
        case element::Type_t::boolean: {
            ...
            case element::Type_t::bf16: {
                return convertPrecision<bool, ngraph::bfloat16>(output, elementsCount, element::Type(toPrecision).size());
            }
            case element::Type_t::u64: {
        ...
        case element::Type_t::bf16: {
            switch (toPrecision) {
                ...
                case element::Type_t::boolean: {
                    return convertPrecision<ngraph::bfloat16, bool>(output, elementsCount, element::Type(toPrecision).size());
                }
                default:
                    throw std::runtime_error("convertOutputPrecision can't convert from: " + element::Type(fromPrecision).get_type_name() + " to: " +
                                             element::Type(toPrecision).get_type_name());
