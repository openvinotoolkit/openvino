...
TEST_F(VpuScaleTest, IsScaleWorkCorrectly) {
        ...
        SKIP();
    #endif
    config.VPUScalesOption = "conv1:0.2; conv2:1.4";
    SetUp();
    ASSERT_NO_THROW(Compile());
    for (const auto& stage : m_testModel->getStages()) {
        auto scales = stage->attrs().getOrDefault<float>("scaleFactor");
        if (stage->name() == "conv1") {
            ASSERT_FLOAT_EQ(scales, 0.2);
            continue;
        }
        if (stage->name() == "conv2") {
            ASSERT_FLOAT_EQ(scales, 1.4);
        }
    }
}
...
TEST_F(VpuScaleTest, IsRegexInScaleWorksCorrectly) {
        ...
        SKIP();
    #endif
    config.VPUScalesOption = "conv1:0.2";
    SetUp();
    ASSERT_NO_THROW(Compile());
    for (const auto& stage : m_testModel->getStages()) {
        auto scales = stage->attrs().getOrDefault<float>("scaleFactor");
