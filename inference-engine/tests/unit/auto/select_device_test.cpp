...
public:
    ...
    void SetUp() override {
       ...
       ON_CALL(*plugin, SelectDevice).WillByDefault([this](const std::vector<DeviceInformation>& metaDevices,
               ...
               return plugin->MultiDeviceInferencePlugin::SelectDevice(metaDevices, netPrecision);
               });
    }
};
...
TEST_P(SelectDeviceTest, SelectDevice) {
    ...
    if (throwExcept) {
        ASSERT_THROW(plugin->SelectDevice(devices, netPrecision), InferenceEngine::Exception);
    } else {
        auto result =  plugin->SelectDevice(devices, netPrecision);
