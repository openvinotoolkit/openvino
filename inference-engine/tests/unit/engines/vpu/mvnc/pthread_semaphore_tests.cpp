...
TEST_F(PThreadSemaphoreTest, CanInitSemaWithMaxIntCounter) {
    ASSERT_EQ(0, pthread_sem_init(&sem, 0, std::numeric_limits<int>::max()));
    ASSERT_EQ(errno, 0);
}

TEST_F(PThreadSemaphoreTest, CanNotPostSemaWithMaxIntCounter) {
    ASSERT_EQ(0, pthread_sem_init(&sem, 0, std::numeric_limits<int>::max()));
    ASSERT_EQ(errno, 0);
