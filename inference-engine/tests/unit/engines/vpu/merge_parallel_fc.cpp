...
public:
    ...
    void CreateModelWithParallelFC(const Dimensions& inputDimensions, const std::vector<FCDescriptor>& descriptors) {
        ...
        for (std::size_t i = 0; i < descriptors.size(); ++i) {
            const auto& fcDescriptor = descriptors[i];
            const auto& weights = CreateConstDataObject("Weights#" + std::to_string(i), fcDescriptor.weights);
            const auto& biases  = CreateConstDataObject("Biases#"  + std::to_string(i), fcDescriptor.biases);
            const auto& scales  = CreateConstDataObject("Scales#"  + std::to_string(i), fcDescriptor.scales);
            const auto& output  = model->addNewData("Output#" + std::to_string(i), CreateDescriptor(fcDescriptor.output));
            ...
            stageBuilder->addReLUStage(
                ...
                0.0f,
                output,
                model->addOutputData("NetworkOutput#" + std::to_string(i), CreateDescriptor(fcDescriptor.output)));
        }
    }
...
protected:
    ...
    Data CreateConstDataObject(const std::string& name, const Dimensions& dimensions) {
        if (dimensions.size() == 0) {
            return model->addFakeData();
        }

        const auto& descriptor = CreateDescriptor(dimensions);
        const auto ieBlob = ie::make_shared_blob<fp16_t>(descriptor.toTensorDesc());
        ieBlob->allocate();
        return model->addConstData(name, descriptor, ieBlobContent(ieBlob));
    }
