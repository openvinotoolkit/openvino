...
TEST(StaticShapeInferenceTest, ScatterNDUpdateTest) {
    ...
    auto scatter_nd_update =
        std::make_shared<op::v3::ScatterNDUpdate>(data_shape, indices_shape, updates_shape);
    ...
    shape_infer(scatter_nd_update.get(), input_shapes, output_shapes);
