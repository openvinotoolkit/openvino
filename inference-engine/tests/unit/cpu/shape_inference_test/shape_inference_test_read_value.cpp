...
std::shared_ptr<op::v6::ReadValue> constructGraph() {
    auto input = std::make_shared<op::v0::Parameter>(element::f32, PartialShape{-1, -1, -1, -1});
    auto variable =
        std::make_shared<ov::op::util::Variable>(ov::op::util::VariableInfo{
            PartialShape::dynamic(), element::dynamic, "ID"});
...
void readValueTest() {
    ...
    shape_infer(readValue.get(), input_shapes, output_shapes);
    ASSERT_EQ(output_shapes[0], (PartialShape{1, 2, 64, 64}));
    //Test StaticShape
    std::vector<StaticShape> static_input_shapes = {StaticShape{1, 2, 64, 64}},
                             static_output_shapes = {StaticShape{}};
    shape_infer(readValue.get(), static_input_shapes, static_output_shapes);
