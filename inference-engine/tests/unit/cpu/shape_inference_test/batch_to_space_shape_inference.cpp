...
static std::shared_ptr<op::v1::BatchToSpace> make_batch_to_space(
    ...
    auto space_to_batch = std::make_shared<op::v1::BatchToSpace>(data, block, crops_begin, crops_end);
...
TEST(StaticShapeInferenceTest, BatchToSpaceWithMissingTensorData) {
    ...
    EXPECT_THROW(shape_inference(space_to_batch.get(), input_shapes, output_shapes, constant_data), NodeValidationFailure);
}