...
std::shared_ptr<ngraph::Node> makeDetectionOutput(const ngraph::OutputVector &inputs,
    ...
    if (inputs.size() == 3)
        return std::make_shared<ngraph::op::v0::DetectionOutput>(inputs[0], inputs[1], inputs[2], attrs);
    else if (inputs.size() == 5)
        return std::make_shared<ngraph::op::v0::DetectionOutput>(inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], attrs);
    else
