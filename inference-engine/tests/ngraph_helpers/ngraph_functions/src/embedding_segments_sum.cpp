...
std::shared_ptr<Node> makeEmbeddingSegmentsSum(
    ...
    auto segmentIdNode = std::make_shared<ngraph::op::v0::Constant>(indicesType, o_shape, segment_ids);
    std::vector<size_t> shape_0 = {};
    auto segmentNumNode = std::make_shared<ngraph::op::v0::Constant>(indicesType, shape_0, num_segments);

    std::shared_ptr<Node> embBag;
    if (with_default_index) {
        auto defIdxNode = std::make_shared<ngraph::op::v0::Constant>(indicesType, shape_0, default_index);
        if (with_weights) {
            auto weightsNode = makeConstant<float>(dataType, {indices.size()}, {}, true);

            embBag = std::make_shared<op::v3::EmbeddingSegmentsSum>(
