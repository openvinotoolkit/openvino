...
std::shared_ptr<ngraph::Node> makeSpaceToBatch(const ngraph::Output<Node> &in,
    ...
    auto blockShapeNode = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, constShape,
                                                                     blockShape.data());
    auto padsBeginNode = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, constShape,
                                                                    padsBegin.data());
    auto padsEndNode = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, constShape, padsEnd.data());
    auto stbNode = std::make_shared<ngraph::op::v1::SpaceToBatch>(in, blockShapeNode, padsBeginNode, padsEndNode);
    return stbNode;
}
