...
std::shared_ptr<Node> makeEmbeddingBagOffsetsSum(
    ...
    auto indicesNode = std::make_shared<ngraph::op::v0::Constant>(indicesType, i_shape, indices);
    std::vector<size_t> o_shape = {offsets.size()};
    auto offsetsNode = std::make_shared<ngraph::op::v0::Constant>(indicesType, o_shape, offsets);

    std::shared_ptr<Node> embBag;
    if (with_default_index) {
        std::vector<size_t> d_shape = {};
        auto defIdxNode = std::make_shared<ngraph::op::v0::Constant>(indicesType, d_shape, default_index);
        if (with_weights) {
            auto weightsNode = makeConstant<float>(dataType, {indices.size()}, {}, true);

            embBag = std::make_shared<op::v3::EmbeddingBagOffsetsSum>(
