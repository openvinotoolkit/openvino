...
std::shared_ptr<Function> ShuffleChannelsFunction::getOriginal(
    ...
    const auto input = std::make_shared<op::v0::Parameter>(inputPrecision, inputShape);
    const auto dequantization = makeDequantization(input, deqBefore);

    const auto shuffleChannels = std::make_shared<op::v0::ShuffleChannels>(dequantization, axis, group);
    shuffleChannels->set_friendly_name("output");

    const auto function = std::make_shared<ngraph::Function>(
        ResultVector{ std::make_shared<op::v0::Result>(shuffleChannels) },
...
std::shared_ptr<ngraph::Function> ShuffleChannelsFunction::getOriginal(
    ...
    const auto shuffleChannels = std::make_shared<op::v0::ShuffleChannels>(fakeQuantize, axis, group);
    shuffleChannels->set_friendly_name("output");
...
std::shared_ptr<ngraph::Function> ShuffleChannelsFunction::getReference(
    ...
    const auto shuffleChannels = std::make_shared<op::v0::ShuffleChannels>(dequantizationBefore, axis, group);
    ...
    const auto function = std::make_shared<ngraph::Function>(
        ResultVector{ std::make_shared<op::v0::Result>(dequantizationAfter) },
        ParameterVector{ input },
        "ShuffleChannelsFunction");
