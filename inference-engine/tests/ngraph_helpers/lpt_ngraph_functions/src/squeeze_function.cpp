...
std::shared_ptr<ngraph::Function> SqueezeFunction::getOriginal(
    ...
    const auto squeeze = std::make_shared<ngraph::op::v0::Squeeze>(
        dequantizationOp,
        std::make_shared<ngraph::op::v0::Constant>(element::i64, Shape{ axes.size() }, axes));

    squeeze->set_friendly_name("output");

    ngraph::ResultVector results{ std::make_shared<ngraph::op::v0::Result>(squeeze) };
...
std::shared_ptr<ngraph::Function> SqueezeFunction::getOriginal(
    ...
    const std::shared_ptr<ngraph::Node> squeeze = std::make_shared<ngraph::op::v0::Squeeze>(
...
std::shared_ptr<ngraph::Function> SqueezeFunction::getReference(
    ...
    const auto squeeze = std::make_shared<ngraph::op::TypeRelaxed<ngraph::op::v0::Squeeze>>(
        op::Squeeze(dequantizationOpBefore, std::make_shared<ngraph::op::v0::Constant>(element::i64, Shape{ axes.size() }, axes)),
