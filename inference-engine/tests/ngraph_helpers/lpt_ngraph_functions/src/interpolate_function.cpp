...
std::shared_ptr<ngraph::Function> InterpolateFunction::getOriginal(
    ...
    const auto outShape = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, ngraph::Shape{ outputShape.size() }, outputShape);
    const auto interpolate = std::make_shared<ngraph::op::v0::Interpolate>(dequantizationOp, outShape, interpAttrs);
    interpolate->set_friendly_name("output");

    ngraph::ResultVector results{ std::make_shared<ngraph::op::v0::Result>(interpolate) };
...
std::shared_ptr<ngraph::Function> InterpolateFunction::getOriginal(
    ...
    const auto outShape = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, ngraph::Shape{ outputShape.size() }, outputShape);
    const auto interpolate = std::make_shared<ngraph::op::v0::Interpolate>(fakeQuantizeOnActivations, outShape, interpAttrs);
...
std::shared_ptr<ngraph::Function> InterpolateFunction::getReference(
    ...
    const auto outShape = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, ngraph::Shape{ outputShape.size() }, outputShape);
    const auto interpolate = std::make_shared<ngraph::op::v0::Interpolate>(quantizationOpBefore, outShape, interpAttrs);
...
std::shared_ptr<ngraph::Function> InterpolateFunction::getOriginal(
    ...
    const auto dequantizationOp = makeDequantization(input, dequantization);
    const auto outShape = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, ngraph::Shape{ outputShape.size() }, outputShape);
    const auto scales = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::f32, ngraph::Shape{ scalesShape.size() }, scalesShape);
