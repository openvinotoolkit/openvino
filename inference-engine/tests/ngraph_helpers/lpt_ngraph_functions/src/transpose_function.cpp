...
std::shared_ptr<ngraph::Function> TransposeFunction::getOriginal(
    ...
    const std::shared_ptr<Node> transpose = std::make_shared<ngraph::op::v1::Transpose>(
        dequantizationOp,
        std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, ngraph::Shape{ transposeConstValues.size() }, transposeConstValues));
...
std::shared_ptr<ngraph::Function> TransposeFunction::getReference(
    ...
    const std::shared_ptr<ngraph::op::v0::Constant> transposeConstant = std::make_shared<ngraph::op::v0::Constant>(
        ngraph::element::i64,
        ngraph::Shape{ transposeConstValues.size() },
        transposeConstValues);
    const std::shared_ptr<ngraph::op::v1::Transpose> transpose = std::make_shared<ngraph::op::v1::Transpose>(quantizationOpBefore, transposeConstant);
