...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginal(
    ...
    const auto input2 = std::make_shared<ngraph::op::v0::Parameter>(precision, ngraph::Shape(inputShape2));
    input2->set_friendly_name("input2");
    const auto fakeQuantize2 = makeFakeQuantize(input2, precision, fqOnData2);
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalWithChildAndOutput(
    ...
    std::shared_ptr<ngraph::op::v0::Result> res1;
    const auto inputShape2 = inputShape;
    const auto input2 = std::make_shared<ngraph::op::v0::Parameter>(precision, inputShape2);
    input2->set_friendly_name("input2");
    const auto fakeQuantize2 = makeFakeQuantize(input2, precision, fqOnData2);

    const auto concat = std::make_shared<ngraph::op::v0::Concat>(
        ngraph::OutputVector{ fakeQuantize1->output(0), fakeQuantize2->output(0) }, 1);
    concat->set_friendly_name("110");
    auto& rtInfo = concat->get_rt_info();
    rtInfo["Variant::std::string"] = std::make_shared<VariantWrapper<std::string>>("concat");

    const auto clamp = std::make_shared<ngraph::op::v0::Clamp>(concat, 0.0, 6.0);
    clamp->set_friendly_name("111");

    ResultVector results{ std::make_shared<ngraph::op::v0::Result>(clamp), std::make_shared<ngraph::op::v0::Result>(concat) };
    std::shared_ptr<ngraph::Function> function = std::make_shared<ngraph::Function>(
        results,
        ngraph::ParameterVector{ input1, input2 },
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalWithNeighbors(
    ...
    if (additionalLayer == "convolution") {
        ...
        auto convolutionAddition = std::make_shared<ngraph::op::v1::Convolution>(
                concat1,
                std::make_shared<op::v1::Multiply>(
                        std::make_shared<op::v0::Convert>(op::v0::Constant::create(element::i8, convShape, {1}), element::f32),
        ...
        results.push_back(std::make_shared<ngraph::op::v0::Result>(convolutionAddition));
    }
    if (neighborType == "concat") {
        ...
        results.push_back(std::make_shared<ngraph::op::v0::Result>(concat1));
        results.push_back(std::make_shared<ngraph::op::v0::Result>(concat2));
    } else if (neighborType == "convolution") {
        ngraph::Shape convShape(4);
        convShape[0] = inputShape[1].get_length() * 2;
        convShape[1] = inputShape[1].get_length();
        convShape[2] = convShape[3] = 1;
        auto convolutionNeighbor = std::make_shared<ngraph::op::v1::Convolution>(
                fakeQuantize2,
                std::make_shared<op::v1::Multiply>(
                        std::make_shared<op::v0::Convert>(op::v0::Constant::create(element::i8, convShape, {1}), element::f32),
                        op::v0::Constant::create(element::f32, Shape{}, {1})),
        ...
        results.push_back(std::make_shared<ngraph::op::v0::Result>(convolutionNeighbor));
    }
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalWithIntermediate(
    ...
    const auto input1 = std::make_shared<ngraph::op::v0::Parameter>(precision, inputShape1);
    input1->set_friendly_name("input1");
    const auto fakeQuantize1 = makeFakeQuantize(input1, precision, fqOnData1);
    fakeQuantize1->set_friendly_name("fakeQuantize1");
    ...
    if (transparentIntermediate) {
        ...
        auto weights = ngraph::op::v0::Constant::create(
            precision,
            ngraph::Shape{ channels, channels, 1, 1 },
            std::vector<float>(channels * channels, 1));

        intermediateOp = std::make_shared<ngraph::op::v1::Convolution>(
    ...
    auto weights = ngraph::op::v0::Constant::create(precision, ngraph::Shape{ channels, channels, 1, 1 }, { 1 });
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalWithIntermediateAvgPool(
    ...
    std::shared_ptr<Node> parent2 = std::make_shared<ngraph::op::v1::AvgPool>(
        intermediateOp,
        Strides{ 1, 1 },
        Shape{ 1, 1 },
        Shape{ 0, 0 },
    ...
    ngraph::ResultVector results {
        std::make_shared<ngraph::op::v0::Result>(concat),
        std::make_shared<ngraph::op::v0::Result>(parent2)
    };
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalWithSplitedIntermediate(
    ...
    intermediateOp = std::make_shared<ngraph::op::v1::Split>(fakeQuantize2->output(0), constant, numSplit);
    ...
    ngraph::ResultVector results{
        std::make_shared<ngraph::op::v0::Result>(concat),
        std::make_shared<ngraph::op::v0::Result>(lastOutput),
    };
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalSelectionWithIntermediate(
    ...
    const auto input1 = std::make_shared<ngraph::op::v0::Parameter>(precision, ngraph::Shape(inputShape1));
    input1->set_friendly_name("input1");
    const auto fakeQuantize1 = makeFakeQuantize(input1, precision, fqOnData1);
    fakeQuantize1->set_friendly_name("fakeQuantize1");
    ...
    auto weights = ngraph::op::v0::Constant::create(precision, ngraph::Shape{ inputShape[1], inputShape[1], 1, 1 }, { 1 });
    auto convolution = std::make_shared<ngraph::op::v1::Convolution>(
        intermediateOp,
        weights,
        ngraph::Strides { 1, 1 },
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalWithStridedSlice(
    ...
    if (ssBeforeConcat) {
        ...
        parent1 = std::make_shared<ngraph::op::v1::StridedSlice>(parent1, beginParam, endParam, beginMask, endMask);
        parent1->set_friendly_name("StridedSlice_1");
    }

    const auto clamp = std::make_shared<ngraph::op::v0::Clamp>(fakeQuantize1, 0.0, 6.0);
    ...
    const auto concat = std::make_shared<ngraph::op::v0::Concat>(NodeVector{ parent1, fakeQuantize2 }, 1);
    concat->set_friendly_name("Concat");
    ...
    if (ssAfterConcat) {
        ...
        const auto stridedSlice = std::make_shared<ngraph::op::v1::StridedSlice>(concat, beginParam, endParam, beginMask, endMask);
        stridedSlice->set_friendly_name("StridedSlice_2");

        const auto result1 = std::make_shared<ngraph::op::v0::Result>(stridedSlice);
        result1->set_friendly_name("Result_1");
        results.push_back(result1);
    } else {
        const auto result1 = std::make_shared<ngraph::op::v0::Result>(concat);
        result1->set_friendly_name("Result_1");
        results.push_back(result1);
    }
    ...
    const auto result2 = std::make_shared<ngraph::op::v0::Result>(convolution);
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalWithDifferentPrecisionOnChildren(
    ...
    const auto avgPool = std::make_shared<ngraph::op::v1::AvgPool>(
        concat->output(0),
        stride,
        padBegin,
        padEnd,
    ...
    ngraph::ResultVector results;
    results.push_back(std::make_shared<ngraph::op::v0::Result>(avgPool));
    results.push_back(std::make_shared<ngraph::op::v0::Result>(maxPool));
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalWithIntermediateWithConstant(
    ...
    if (transparentIntermediate) {
        ...
        intermediateOp = std::make_shared<ngraph::op::v0::Interpolate>(pooling->output(0), outputShape, attributes);
        intermediateOp->set_friendly_name("intermediate");
    } else {
    ...
    ngraph::ResultVector results{
        std::make_shared<ngraph::op::v0::Result>(avgPool),
    };
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalWithReshapeAtTheEndTransformation(
    ...
    const std::shared_ptr<ngraph::op::v0::Concat> concat1 = std::make_shared<ngraph::op::v0::Concat>(
        ngraph::OutputVector{ fakeQuantize1->output(0), fakeQuantize2->output(0) }, 1);
    concat1->set_friendly_name("concat1");
    ...
    const std::shared_ptr<ngraph::op::v0::Concat> concat2 = std::make_shared<ngraph::op::v0::Concat>(ngraph::OutputVector{ fakeQuantize3, intermediate }, 1);
    concat2->set_friendly_name("concat2");
    ...
    const std::shared_ptr<Node> reshape = std::make_shared<ngraph::op::v1::Reshape>(
        maxPool,
        std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, ngraph::Shape{2ul}, std::vector<size_t>{0, 0}),
        true);
...
std::shared_ptr<ngraph::Function> ConcatFunction::getOriginalWithIntermediateReshape(
    ...
    const auto reshape1 = std::make_shared<op::v1::Reshape>(
            fakeQuantize1,
            op::v0::Constant::create(element::i64, Shape{reshapeOutputShape.size()}, reshapeOutputShape),
            true);
...
std::shared_ptr<ngraph::Function> ConcatFunction::getReference(
    ...
    const std::shared_ptr<ngraph::op::v0::Concat> concat = std::make_shared<ngraph::op::TypeRelaxed<ngraph::op::v0::Concat>>(
        ngraph::OutputVector{ fakeQuantize1->output(0), fakeQuantize2->output(0) }, 1);
    auto& rtInfo = concat->get_rt_info();
    rtInfo["Variant::std::string"] = std::make_shared<VariantWrapper<std::string>>("concat");
...
std::shared_ptr<ngraph::Function> ConcatFunction::get(
    ...
    const auto input1 = std::make_shared<ngraph::op::v0::Parameter>(inputPrecision, inputShape);
    input1->set_friendly_name("input1");
    ...
    const auto input2 = std::make_shared<ngraph::op::v0::Parameter>(inputPrecision, inputShape);
    input2->set_friendly_name("input2");
    ...
    const std::shared_ptr<ngraph::op::v0::Concat> concat = std::make_shared<ngraph::op::v0::Concat>(ngraph::OutputVector{ parent1, parent2 }, axis);
    concat->set_friendly_name("concat");
    ...
    if (addNotPrecisionPreservedOperation) {
        auto avgPool = std::make_shared<op::v1::AvgPool>(
            lastDequantization,
            Strides{1, 1},
            Shape{1, 1},
            Shape{1, 1},
...
std::shared_ptr<ngraph::Function> ConcatFunction::get(
    ...
    const auto createReshape = [](const std::shared_ptr<Node>& parent) -> std::shared_ptr<Node> {
        ...
        const auto reshape1 = std::make_shared<op::v1::Reshape>(
            parent,
            std::make_shared<op::v0::Constant>(element::i32, Shape{ intermediateShape.size() }, intermediateShape),
            true);
        ...
        const auto reshape2 = std::make_shared<op::v1::Reshape>(
            maxPool,
            std::make_shared<op::v0::Constant>(element::i32, Shape{ originalShape.size() }, originalShape),
            true);

        return reshape2;
    };

    const auto input1 = std::make_shared<ngraph::op::v0::Parameter>(inputPrecision, inputShape1);
    input1->set_friendly_name("input1");
    ...
    const auto input2 = std::make_shared<ngraph::op::v0::Parameter>(inputPrecision, inputShape2);
    input2->set_friendly_name("input2");
    ...
    parent = std::make_shared<ngraph::op::v0::Concat>(ngraph::OutputVector{ parent1, parent2 }, axis);
...
std::shared_ptr<ngraph::Function> ConcatFunction::getReferenceWithNeighbors(
    ...
    if (additionalLayer == "convolution") {
        ...
        convShape[0] = convShape[1] * 2;
        convShape[2] = convShape[3] = 1;
        auto convolutionAddition = std::make_shared<op::TypeRelaxed<op::v1::Convolution>>(
                ...
                op::TemporaryReplaceOutputType(op::v0::Constant::create(element::i8, convShape, {1}), element::f32).get(),
                ngraph::Strides{ 1, 1 },
                ngraph::CoordinateDiff{ 0, 0 },
                ngraph::CoordinateDiff{ 0, 0 },
                ngraph::Strides{ 1, 1 });
    ...
    if (neighborType == "concat") {
        ...
        convShape[1] = inputShape[1].get_length();
        convShape[2] = convShape[3] = 1;
        auto convolutionNeighbor = std::make_shared<op::TypeRelaxed<ngraph::op::v1::Convolution>>(
                element::TypeVector{ element::f32, element::f32 },
                element::TypeVector{ element::f32 },
...
std::shared_ptr<ngraph::Function> ConcatFunction::getReferenceWithIntermediateAvgPool(
    ...
    std::shared_ptr<Node> parent2 = std::make_shared<ngraph::op::TypeRelaxed<ngraph::op::v1::AvgPool>>(
    ...
    ngraph::ResultVector results {
        std::make_shared<ngraph::op::v0::Result>(parent1),
        std::make_shared<ngraph::op::v0::Result>(parent2)
    };
...
std::shared_ptr<ngraph::Function> ConcatFunction::getReferenceWithSplitedIntermediate(
    ...
    const auto constant = std::make_shared<ngraph::op::v0::Constant>(element::i64, Shape{ }, splitedAxis);
    intermediateOp = std::make_shared<ngraph::op::v1::Split>(deqBefore2, constant, numSplit);
    intermediateOp->set_friendly_name("intermediate");
...
std::shared_ptr<ngraph::Function> ConcatFunction::getReferenceWithStridedSlice(
    ...
    const auto clamp = std::make_shared<ngraph::op::v0::Clamp>(dequantizationBefore, 0.0, 6.0);
    clamp->set_friendly_name("Clamp");
    ...
    if (ssAfterConcat) {
        ...
        const auto result1 = std::make_shared<ngraph::op::v0::Result>(dequantizationAfter1);
        result1->set_friendly_name("Result_1");
        results.push_back(result1);
    } else {
...
std::shared_ptr<ngraph::Function> ConcatFunction::getReferenceWithDifferentPrecisionOnChildren(
    ...
    if (!dequantizationAfter2.empty()) {
        const std::shared_ptr<ngraph::op::v1::MaxPool> maxPool = std::make_shared<ngraph::op::v1::MaxPool>(
            concat->output(0),
            stride,
            padBegin,
            padEnd,
        ...
        results.push_back(std::make_shared<ngraph::op::v0::Result>(lastDequantization2));
    }
...
std::shared_ptr<ngraph::Function> ConcatFunction::getReferenceWithReshapeAtTheEndTransformation(
    ...
    ngraph::ResultVector results{std::make_shared<ngraph::op::v0::Result>(dequantization)};
