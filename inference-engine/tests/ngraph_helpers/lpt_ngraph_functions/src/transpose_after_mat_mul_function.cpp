...
namespace subgraph {
    std::shared_ptr<ngraph::Function> TransposeAfterMatMulFunction::getOriginal(
        ...
        const auto input2 = std::make_shared<ngraph::op::v0::Parameter>(precision, inputShape);
        ...
        const auto matMul = std::make_shared<ngraph::op::v0::MatMul>(fakeQuantize1, fakeQuantize2, false, false);
        input2->set_friendly_name("matMul");
        const auto transpose = std::make_shared<ngraph::op::v1::Transpose>(
            matMul,
            ngraph::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{ 4ul }, { 0, 2, 1, 3 }));
