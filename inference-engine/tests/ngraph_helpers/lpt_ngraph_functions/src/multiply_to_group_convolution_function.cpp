...
std::shared_ptr<ngraph::Function> MultiplyToGroupConvolutionFunction::getOriginal(
    ...
    if (haveMultiplyWithNoConstBeforeDequantization) {
        secondInput = std::make_shared<ngraph::op::v0::Parameter>(precisionBeforeDequantization, inputShape);
        parent = std::make_shared<ngraph::op::v1::Multiply>(input, secondInput);
    }
...
std::shared_ptr<ngraph::Function> MultiplyToGroupConvolutionFunction::getOriginal(
    ...
    const auto multiply = std::make_shared<ngraph::op::v1::Multiply>(
        maxPool,
        std::make_shared<ngraph::op::v0::Constant>(constant.outPrecision, constant.shape, constant.values));
...
std::shared_ptr<ngraph::Function> MultiplyToGroupConvolutionFunction::getReference(
    ...
    const auto gconv = std::make_shared<ngraph::op::TypeRelaxed<ngraph::op::v1::GroupConvolution>>(
    ...
    if (biases) {
        lastNode = std::make_shared<ngraph::op::v1::Add>(gconv, biases);
    }
