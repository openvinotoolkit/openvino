...
std::shared_ptr<ngraph::op::v1::Convolution> createConvolution(
    ...
    const std::shared_ptr<ngraph::op::v1::Convolution> convolution = typeRelaxed ?
        std::make_shared<ngraph::op::TypeRelaxed<ngraph::op::v1::Convolution>>(
...
std::shared_ptr<ngraph::Function> FakeQuantizeAndTwoOutputBranchesWithConvolutionFunction::getOriginal(
    ...
    const std::shared_ptr<ngraph::op::v1::Convolution> convolution1 = createConvolution(
        precision,
        inputShape,
    ...
    const std::shared_ptr<ngraph::op::v1::Convolution> convolution2 = createConvolution(
        precision,
        inputShape,
        fakeQuantizeOnActivations,
        fqOnWeights2,
    ...
    const std::shared_ptr<ngraph::op::v0::Concat> concat = std::make_shared<ngraph::op::v0::Concat>(NodeVector{ convolution1, convolution2 }, 1ul);
...
std::shared_ptr<ngraph::Function> FakeQuantizeAndTwoOutputBranchesWithConvolutionFunction::getReference(
    ...
    const std::shared_ptr<ngraph::op::v0::Concat> concat = std::make_shared<ngraph::op::v0::Concat>(NodeVector{ deqAfter1, deqAfter2 }, 1ul);
    ...
    if (params.updatePrecisions) {
        replace_node(
            ...
            ngraph::pass::low_precision::fold<ngraph::op::v0::Convert>(convolution1->get_input_node_shared_ptr(1), element::i8));

        replace_node(
            convolution2->get_input_node_shared_ptr(1),
            ngraph::pass::low_precision::fold<ngraph::op::v0::Convert>(convolution2->get_input_node_shared_ptr(1), element::i8));
    }
