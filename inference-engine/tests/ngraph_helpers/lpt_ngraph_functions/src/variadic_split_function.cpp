...
namespace subgraph {
    std::shared_ptr<ngraph::Function> VariadicSplitFunction::getOriginal(
        ...
        const auto constantAxis = std::make_shared<ngraph::op::v0::Constant>(element::i64, Shape{ }, splitedAxis);
        const auto constantLengths = std::make_shared<ngraph::op::v0::Constant>(element::i64, Shape{ splitLengths.size() }, splitLengths);
        const std::shared_ptr<Node> variadicSplit = std::make_shared<ngraph::op::v1::VariadicSplit>(dequantizationOp, constantAxis, constantLengths);

        ngraph::ResultVector results;
        for (size_t i = 0; i < splitLengths.size(); ++i) {
            results.push_back(std::make_shared<ngraph::op::v0::Result>(variadicSplit->output(i)));
        }
...
std::shared_ptr<ngraph::Function> VariadicSplitFunction::getOriginal(
    ...
    const std::shared_ptr<Node> variadicSplit =
        std::make_shared<ngraph::op::v1::VariadicSplit>(fakeQuantize.empty() ? input : fq, constantAxis, constantLengths);
...
std::shared_ptr<ngraph::Function> VariadicSplitFunction::getReference(
    ...
    const auto constantAxis = std::make_shared<ngraph::op::v0::Constant>(element::i64, Shape{ }, splitedAxis);
    const auto constantLengths = std::make_shared<ngraph::op::v0::Constant>(element::i64, Shape{ splitLengths.size() }, splitLengths);
    const auto variadicSplit = std::make_shared<ngraph::op::v1::VariadicSplit>(deqBefore, constantAxis, constantLengths);

    ngraph::ResultVector results;
    for (size_t i = 0; i < splitLengths.size(); ++i) {
        results.push_back(std::make_shared<ngraph::op::v0::Result>(
