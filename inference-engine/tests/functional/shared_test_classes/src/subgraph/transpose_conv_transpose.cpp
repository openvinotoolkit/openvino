...
void TransposeConvTest::SetUp() {
    ...
    auto reshape_pattern = std::make_shared<ngraph::opset8::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{conv_input_shape.size()}, conv_input_shape);
    auto reshape = std::make_shared<ngraph::opset8::Reshape>(params[0], reshape_pattern, false);

    const auto input_order1 = std::make_shared<ngraph::opset8::Constant>(ngraph::element::i64,
                                                                         ngraph::Shape({conv_input_shape.size()}),
                                                                         nchw_order);
    auto transpose1 = std::make_shared<ngraph::opset8::Transpose>(reshape, input_order1);
    ...
    auto conv = std::make_shared<ngraph::opset8::Convolution>(transpose1, filter_weights_node, strides, std::vector<ptrdiff_t>{ 0, 0 },
                                                              std::vector<ptrdiff_t>{ 0, 0 }, std::vector<size_t>{ 1, 1 },
                                                              ngraph::op::PadType::VALID);

    const auto input_order2 = std::make_shared<ngraph::opset8::Constant>(ngraph::element::i64,
                                                                         ngraph::Shape({conv_input_shape.size()}),
                                                                         nhwc_order);
    auto transpose2 = std::make_shared<ngraph::opset8::Transpose>(conv, input_order2);
