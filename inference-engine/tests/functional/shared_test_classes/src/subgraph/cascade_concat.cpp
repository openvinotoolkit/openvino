...
void CascadeConcat::SetUp() {
    ...
    auto relu1 = std::make_shared<ngraph::op::v0::Relu>(input[0]);
    auto relu2 = std::make_shared<ngraph::op::v0::Relu>(input[1]);
    auto relu3 = std::make_shared<ngraph::op::v0::Relu>(input[2]);
    auto concat = std::make_shared<ngraph::op::v0::Concat>(ngraph::OutputVector{relu1->output(0),
                                                                                relu2->output(0)},
                                                                                1);
    ...
    auto concat2 = std::make_shared<ngraph::op::v0::Concat>(ngraph::OutputVector{reshape2->output(0),
                                                                                 relu3->output(0)},
                                                                                 1);
    ...
    if (multioutput) {
        ...
        results = ngraph::ResultVector{std::make_shared<ngraph::op::v0::Result>(concat2),
                                       std::make_shared<ngraph::op::v0::Result>(mult)};
    } else {
        results = ngraph::ResultVector{std::make_shared<ngraph::op::v0::Result>(concat2)};
    }
