...
void EltwiseBeforeConvTest::SetUp() {
    ...
    auto mul = std::make_shared<ngraph::op::v1::Multiply>(params[0], mul_const);
    auto add_const = std::make_shared<ngraph::op::Constant>(ngPrc, inputShape, shift);
    auto add = std::make_shared<ngraph::op::v1::Add>(mul, add_const);

    std::vector<size_t> convInputShape = {1, inputChannels, 1, inputShape[0] * inputShape[1] / inputChannels};
    auto reshapePattern1 = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{ 4 }, convInputShape);
    auto reshape1 = std::make_shared<ngraph::op::v1::Reshape>(mul, reshapePattern1, false);
...
void EltwiseWithTwoConvsAsInputsTest::SetUp() {
    ...
    auto reshapePattern3 = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{ 4 }, convInputShape);
    auto reshape3 = std::make_shared<ngraph::op::v1::Reshape>(params[1], reshapePattern3, false);
    ...
    auto reshapePattern4 = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{ 2 }, outFormShapes);
    auto reshape4 = std::make_shared<ngraph::op::v1::Reshape>(conv2, reshapePattern4, false);

    auto add = std::make_shared<ngraph::op::v1::Add>(reshape2, reshape4);
