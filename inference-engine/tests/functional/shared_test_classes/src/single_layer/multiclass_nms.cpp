...
void MulticlassNmsLayerTest::Compare(
    ...
    for (int outputIndex = static_cast<int>(expectedOutputs.size()) - 1;
        ...
        if (_dims.size() == 1 && _dims[0] == numBatches) {
            ...
            auto memory =
                InferenceEngine::as<InferenceEngine::MemoryBlob>(actual);
            IE_ASSERT(memory);
            ...
            if (outputIndex == 2) {
                if (expected.second.size() != k * actual->byteSize())
                    throw std::runtime_error(
                        "Expected and actual size 3rd output have different "
                        "size");
            }
                ...
                switch (precision) {
                    case InferenceEngine::Precision::FP32: {
                        ...
                        for (size_t tailing = validNums * 6;
                            ...
                            ASSERT_TRUE(
                                std::abs(
                                    fBuffer[(actual_offset * 6 + tailing)] -
                                    -1.f) < 1e-5)
                    ...
                    case InferenceEngine::Precision::I32: {
                        ...
                        for (size_t tailing = validNums;
                             tailing < maxOutputBoxesPerBatch; tailing++) {
...
void MulticlassNmsLayerTest::SetUp() {
    ...
    auto nms = std::make_shared<opset8::MulticlassNms>(paramOuts[0],
                                                       paramOuts[1], attrs);
