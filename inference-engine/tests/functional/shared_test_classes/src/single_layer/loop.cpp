...
namespace LayerTestsDefinitions {
    ...
    void LoopTest::SetUp() {
        ...
        // Example:
        /*      auto X = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{32, 1, 10});
        auto Y = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{32, 1, 10});
        auto M = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{32, 1, 10});*/
        ...
        auto current_iteration = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::i64, ngraph::Shape{1});

        //Example:
/*      auto Xi = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::PartialShape::dynamic());
        auto Yi = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::PartialShape::dynamic());
        auto M_body = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::PartialShape::dynamic());*/

        ngraph::ParameterVector body_params;
        for (const auto &pshape : body_params_shapes) {
            auto paramNode = std::make_shared<ngraph::op::v0::Parameter>(ngPrc, pshape);
        ...
        if (is_body_condition_const) {
            if (body_condition) {
                body_condition_const = std::make_shared<ngraph::op::v0::Constant>(
        ...
        auto trip_count_const =
                std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, ngraph::Shape{1}, trip_count);

        std::shared_ptr<ngraph::Node> exec_condition;
        if (execute_first_iteration) {
            exec_condition = std::make_shared<ngraph::op::v0::Constant>(
                    ngraph::element::boolean, ngraph::Shape{1}, true);
        } else {
            exec_condition = std::make_shared<ngraph::op::v0::Constant>(
                    ngraph::element::boolean, ngraph::Shape{1}, false);
        ...
        auto loop = std::make_shared<ngraph::op::v5::Loop>(trip_count_const, exec_condition);
        loop->set_function(body);
        loop->set_special_body_ports(ngraph::op::v5::Loop::SpecialBodyPorts{-1, 0});
        ...
        auto result0 = std::make_shared<ngraph::op::v0::Result>(out0);
        auto result1 = std::make_shared<ngraph::op::v0::Result>(out1);
        auto result2 = std::make_shared<ngraph::op::v0::Result>(out2);
    ...
    void StaticShapeLoopTest::SetUp() {
        ...
        auto cond_input_create = [&params] (ngraph::element::Type prc, const ngraph::Shape &shape, int value = 0, bool is_static = false)
            ...
            if (is_static)
                return std::make_shared<ngraph::op::v0::Constant>(prc, shape, value);

            auto input = std::make_shared<ngraph::op::v0::Parameter>(prc, shape);
        ...
        //
        auto b_indx = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::i64, ngraph::Shape{});
        auto b_data = std::make_shared<ngraph::op::v0::Parameter>(prc, ngShape);
        auto b_indx_cast = std::make_shared<ngraph::opset5::Convert>(b_indx, prc);
        auto b_add  = std::make_shared<ngraph::op::v1::Add>(b_data, b_indx_cast);

        std::shared_ptr<ngraph::Node> b_cond;
        if (dynamic_exit == -1) {
            b_cond = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::boolean, ngraph::Shape{}, true);
        } else {
            auto b_exit_value = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, scalarShape, dynamic_exit);
        ...
        auto loop = std::make_shared<ngraph::op::v5::Loop>(count, skip);
        loop->set_function(body);
    ...
    void TrivialLoopTest::CreateSlicedLoop(size_t batch_size, size_t num_iteration, InferenceEngine::Precision iePrc,
        ...
        auto to_slice = std::make_shared<ngraph::op::v0::Parameter>(prc, to_slice_shape);
        auto start = std::make_shared<ngraph::op::v0::Constant>(prc, shape, 0);
        auto count = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, scalarShape, num_iteration);
        auto icond = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::boolean, scalarShape, true);

        // Loop body
        auto b_data = std::make_shared<ngraph::op::v0::Parameter>(prc, shape);
        auto b_recu = std::make_shared<ngraph::op::v0::Parameter>(prc, shape);
        auto b_add  = std::make_shared<ngraph::op::v1::Add>(b_data, b_recu);
        auto b_cond = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::boolean, scalarShape, true);

        auto body = std::make_shared<ngraph::Function>(
                ngraph::OutputVector    {b_cond, b_add},
                ngraph::ParameterVector {b_data, b_recu});

        auto loop = std::make_shared<ngraph::op::v5::Loop>(count, icond);
        loop->set_function(body);
    ...
    void TrivialLoopTest::CreateSlicedLoopDynCondition(size_t batch_size, size_t num_iteration, InferenceEngine::Precision iePrc,
        ...
        auto to_slice = std::make_shared<ngraph::op::v0::Parameter>(prc, to_slice_shape);
        auto start = std::make_shared<ngraph::op::v0::Constant>(prc, shape, 0);
        auto exit_on = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, scalarShape, num_iteration);
        auto count = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64, scalarShape, trip_count);
        auto icond = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::boolean, scalarShape, true);
        ...
        auto b_recu = std::make_shared<ngraph::op::v0::Parameter>(prc, shape);
        auto b_add  = std::make_shared<ngraph::op::v1::Add>(b_data, b_recu);
        auto b_iter = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::i64, scalarShape);
        auto b_exit_on = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::i64, scalarShape);
