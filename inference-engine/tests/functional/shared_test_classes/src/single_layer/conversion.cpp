...
void ConversionLayerTest::SetUp() {
    ...
    // Do multiple conversions as IE/CPU can convert outputs to 'float'.
    auto convert = ngraph::builder::makeConversion(params.front(), targetPrc, conversionOpType);
    auto convert2 = ngraph::builder::makeConversion(convert, ngPrc, conversionOpType);
    auto convert3 = ngraph::builder::makeConversion(convert2, targetPrc, conversionOpType);

    ngraph::ResultVector results{std::make_shared<ngraph::opset3::Result>(convert3)};
