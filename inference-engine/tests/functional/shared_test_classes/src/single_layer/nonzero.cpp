...
void NonZeroLayerTest::SetUp() {
    ...
    const auto& paramNode = std::make_shared<ngraph::op::v0::Parameter>(precision, ngraph::Shape(inputShape));

    auto nonZeroOp = std::make_shared<ngraph::op::v3::NonZero>(paramNode->output(0));

    ngraph::ResultVector results{std::make_shared<ngraph::op::v0::Result>(nonZeroOp)};
