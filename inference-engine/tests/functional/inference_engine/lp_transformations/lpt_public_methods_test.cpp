...
TEST(DISABLED_LPT, isQuantizedTransformation) {
    const auto input = std::make_shared<op::v0::Parameter>(element::f32, Shape{ 1, 3, 16, 16 });
    const auto mulConst = op::v0::Constant::create(element::f32, Shape{}, { 1.f });
    const auto mul = std::make_shared<ngraph::op::v1::Multiply>(input, mulConst);
    const auto shapeConst = op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{ 4 }, { 1, 3, 16, 16 });
    const auto layer = std::make_shared<op::v1::Reshape>(mul, shapeConst, true);
