...
public:
    void SetUp() override {
        ...
        transform.add<ngraph::pass::low_precision::FakeQuantizeTransformation, ngraph::op::v0::FakeQuantize>(testValues.params);
...
private:
    std::shared_ptr<ngraph::Function> get(
        ...
        const auto inputLow = inputLowConst ?
            std::dynamic_pointer_cast<ngraph::Node>(std::make_shared<op::v0::Constant>(constantPresition, constantShape, low)) :
            std::make_shared<ngraph::op::v0::Parameter>(constantPresition, constantShape);

        const auto inputHigh = inputLowConst ?
            std::dynamic_pointer_cast<ngraph::Node>(std::make_shared<op::v0::Constant>(constantPresition, constantShape, high)) :
            std::make_shared<ngraph::op::v0::Parameter>(constantPresition, constantShape);
        ...
        auto fakeQuantize = std::make_shared<ngraph::op::v0::FakeQuantize>(input, inputLow, inputHigh, outputLow, outputHigh, levels);
        fakeQuantize->set_friendly_name("fakeQuantize");

        ngraph::ResultVector results{ std::make_shared<ngraph::op::v0::Result>(fakeQuantize) };

        ngraph::ParameterVector inputs{ input };
        if (as_type_ptr<ngraph::op::v0::Parameter>(inputLow)) {
            inputs.push_back(as_type_ptr<ngraph::op::v0::Parameter>(inputLow));
        }
        if (as_type_ptr<ngraph::op::v0::Parameter>(inputHigh)) {
            inputs.push_back(as_type_ptr<ngraph::op::v0::Parameter>(inputHigh));
        }
        if (as_type_ptr<ngraph::op::v0::Parameter>(outputLow)) {
            inputs.push_back(as_type_ptr<ngraph::op::v0::Parameter>(outputLow));
        }
        if (as_type_ptr<ngraph::op::v0::Parameter>(outputHigh)) {
            inputs.push_back(as_type_ptr<ngraph::op::v0::Parameter>(outputHigh));
        }
