...
//    auto a0_1 = std::dynamic_pointer_cast<ngraph::VariantWrapper<PerTensorQuantizationAttribute>>(convolutions[0]->input(0).get_rt_info().begin()->second);
//    ASSERT_NE(nullptr, a0_1);
//    auto a0_2 = std::dynamic_pointer_cast<ngraph::VariantWrapper<std::shared_ptr<PrecisionsAttribute>>>(
//        (convolutions[0]->input(0).get_rt_info().begin()++)->second);
//    ASSERT_EQ(element::u8, *a0_2->get().get()->sharedValue->precisions.begin());
