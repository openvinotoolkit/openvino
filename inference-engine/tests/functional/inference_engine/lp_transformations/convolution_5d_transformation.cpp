...
class Convolution5DTransformation : public LayerTransformation, public testing::WithParamInterface<ConvolutionTransformationParams> {
public:
    void SetUp() override {
...
TEST_P(Convolution5DTransformation, CompareFunctions) {
...
const std::vector<ConvolutionTransformationTestValues> testValues = {
    // with zero point
    {
        ...
        {
            ngraph::element::u8,
            {{}, { { 128.f }, ngraph::element::f32, { 1, 3, 1, 1, 1 }, false }, {}},
    ...
    // with zero point, per-channel quantization with the same values
    {
        ...
        // ActualValues
        {
            ...
            {
                {ngraph::element::f32},
                { { 128.f }, ngraph::element::f32, { 1, 3, 1, 1, 1 } },
                { { 0.02f },  ngraph::element::f32, { 1, 3, 1, 1, 1 } }
            },
            op::Constant::create(ngraph::element::f32, ngraph::Shape{}, std::vector<float>{ 2.f }),
            { 255ul, Shape({ 1, 1, 1, 1 }), { 0.f }, { 254.f }, { -1.27f }, { 1.27f } }
        },
    ...
    // with zero point, per-channel quantization with different values
    {
        ...
        // ActualValues
        {
            ...
            {
                {ngraph::element::f32},
                {{ 128.f, 0.f, 128.f }, ngraph::element::f32, { 1, 3, 1, 1, 1 }},
                {{ 0.02f, 0.01f, 0.03f }, ngraph::element::f32, {1, 3, 1, 1, 1}}
            },
            op::Constant::create(ngraph::element::f32, ngraph::Shape{}, std::vector<float>{ 2.f }),
            { 255ul, Shape({ 1, 1, 1, 1 }), { 0.f }, { 254.f }, { -1.27f }, { 1.27f } }
        },
...
INSTANTIATE_TEST_CASE_P(
    LPT,
    Convolution5DTransformation,
    ::testing::Combine(
        ::testing::ValuesIn(shapes),
        ::testing::ValuesIn(testValues)),
    Convolution5DTransformation::getTestCaseName);
