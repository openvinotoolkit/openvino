...
public:
    void SetUp() override {
        ...
        const auto createActualFunction = [](
            ...
            const std::shared_ptr<ngraph::op::v0::Parameter> input = std::make_shared<ngraph::op::v0::Parameter>(precision, inputShape);
            const auto relu = std::make_shared<ngraph::op::v0::Relu>(input);
            const auto dequantizationsNode = ngraph::builder::subgraph::makeDequantization(relu, dequantizations);

            const std::shared_ptr<ngraph::Node> reshape1 = std::make_shared<ngraph::op::v1::Reshape>(
                dequantizationsNode,
                std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i32, ngraph::Shape{ 2 }, std::vector<double>({0, -1})),
                true);
            reshape1->set_friendly_name("reshape1");

            const std::shared_ptr<ngraph::Node> reshape2 = std::make_shared<ngraph::op::v1::Reshape>(
                dequantizationsNode,
                std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i32, ngraph::Shape{ 2 }, std::vector<double>({0, -1})),
                true);
            ...
            return std::make_shared<ngraph::Function>(
                ngraph::ResultVector{
                    std::make_shared<ngraph::op::v0::Result>(reshape1),
