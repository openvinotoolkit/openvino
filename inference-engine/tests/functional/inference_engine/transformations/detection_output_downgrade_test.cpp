...
namespace {
void create_attributes_vectors(
      ...
      for (bool variance_encoded_in_target : {true, false}) {
        for (bool share_location : {true, false}) {
          for (bool normalized : {true, false}) {
            ...
            attrs_v1.variance_encoded_in_target =
                attrs_v8.variance_encoded_in_target =
...
TEST(TransformationTests, DetectionOutput8ToDetectionOutput1) {
  ...
  for (size_t ind = 0;
    ...
    PartialShape proposals_shape = {
        priors_batch_size, attributes_v8.variance_encoded_in_target ? 1 : 2,
        num_prior_boxes * prior_box_size};

    {
      auto box_logits = std::make_shared<ngraph::opset1::Parameter>(
          ngraph::element::f32, box_logits_shape);
      auto class_preds = std::make_shared<ngraph::opset1::Parameter>(
          ngraph::element::f32, class_preds_shape);
      auto proposals = std::make_shared<ngraph::opset1::Parameter>(
          ngraph::element::f32, proposals_shape);

      auto detection_output_v8 =
          std::make_shared<ngraph::opset8::DetectionOutput>(
              box_logits, class_preds, proposals, attributes_v8);
      ...
      manager.register_pass<
          ngraph::pass::ConvertDetectionOutput8ToDetectionOutput1>();
...

    {
      ...
      auto detection_output_v1 =
          std::make_shared<ngraph::opset1::DetectionOutput>(
              box_logits, class_preds, proposals, attributes_v1);
...
TEST(TransformationTests, DetectionOutput8ToDetectionOutput1FiveArguments) {
  ...
  for (size_t ind = 0;
    ...
    PartialShape ad_box_preds_shape = {N,
...

    {
      ...
      auto proposals = std::make_shared<ngraph::opset1::Parameter>(
          ngraph::element::f32, proposals_shape);
      auto ad_class_preds = std::make_shared<ngraph::opset1::Parameter>(
          ngraph::element::f32, ad_class_preds_shape);
      auto ad_box_preds = std::make_shared<ngraph::opset1::Parameter>(
          ngraph::element::f32, ad_box_preds_shape);

      auto detection_output_v8 =
          std::make_shared<ngraph::opset8::DetectionOutput>(
              box_logits, class_preds, proposals, ad_class_preds, ad_box_preds,
              attributes_v8);

      f = std::make_shared<ngraph::Function>(
          ngraph::NodeVector{detection_output_v8},
          ngraph::ParameterVector{box_logits, class_preds, proposals,
                                  ad_class_preds, ad_box_preds});
