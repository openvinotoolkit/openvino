...
TEST(TransformationTests, BatchToSpaceDecomposition) {
    ...
    {
        auto data = std::make_shared<op::Parameter>(element::f32, Shape{100, 7, 13, 3});
        auto block_shape = std::make_shared<op::Constant>(element::i64, Shape{4}, std::vector<int64_t>{1, 10, 5, 1});
        auto crops_begin = std::make_shared<op::Constant>(element::i64, Shape{4}, std::vector<int64_t>{0, 3, 1, 0});
        auto crops_end = std::make_shared<op::Constant>(element::i64, Shape{4}, std::vector<int64_t>{0, 3, 0, 0});
        ...
        ngraph::pass::ConvertBatchToSpace().run_on_function(f);
...
TEST(TransformationTests, SpaceToBatchDecomposition) {
    ...
    {
        auto data = std::make_shared<op::Parameter>(element::f32, Shape{2, 64, 64, 3});
        auto block_shape = std::make_shared<op::Constant>(element::i64, Shape{4}, std::vector<int64_t>{1, 10, 5, 1});
        auto pads_begin = std::make_shared<op::Constant>(element::i64, Shape{4}, std::vector<int64_t>{0, 3, 1, 0});
        auto pads_end = std::make_shared<op::Constant>(element::i64, Shape{4}, std::vector<int64_t>{0, 3, 0, 0});
        auto batch_to_space = std::make_shared<ngraph::opset3::SpaceToBatch>(data, block_shape, pads_begin, pads_end);

        f = std::make_shared<ngraph::Function>(ngraph::NodeVector{batch_to_space}, ngraph::ParameterVector{data});
        ngraph::pass::InitNodeInfo().run_on_function(f);

        ngraph::pass::ConvertSpaceToBatch().run_on_function(f);
        ASSERT_NO_THROW(check_rt_info(f));
    }
