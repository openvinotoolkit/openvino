...
TEST(TransformationTests, LowLatencyLSTM) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        ...
        auto H_init = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 128});
        H_init->set_friendly_name("H_init");
        auto C_init = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 128});
        C_init->set_friendly_name("C_init");

        auto Xi = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 1, 16});
        Xi->set_friendly_name("Xi");
        auto H_t = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 128});
        H_t->set_friendly_name("H_t");
        auto C_t = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 128});
    ...
    }
    {
        ...
        auto read_value_H = std::make_shared<op::v6::ReadValue>(H_t, variable_H);
        auto read_value_C = std::make_shared<op::v6::ReadValue>(C_t, variable_C);
        // Body
        auto axis = ngraph::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{}, {0});
        auto squeeze = std::make_shared<opset6::Squeeze>(Xi, axis);
        ...
        auto assign_H = std::make_shared<op::v6::Assign>(lstm_cell->output(0), variable_H);
        auto assign_C = std::make_shared<op::v6::Assign>(lstm_cell->output(1), variable_C);
        auto unsqueeze = std::make_shared<opset6::Unsqueeze>(lstm_cell->output(0), axis);
        auto res_2 = std::make_shared<op::v0::Result>(unsqueeze);
        auto res_1 = std::make_shared<op::v0::Result>(lstm_cell->output(0));
...
TEST(TransformationTests, LowLatencyGRU) {
    ...
    }
    {
        ...
        auto assign_H = std::make_shared<op::v6::Assign>(rnn_cell->output(0), variable_H);
        auto res_1 = std::make_shared<op::v0::Result>(assign_H);
...
TEST(TransformationTests, LowLatencyLSTMReshape) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        ...
        auto H = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 128});
        auto C = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 128});
        ...
        auto new_X = std::make_shared<op::v0::Parameter>(element::f32, Shape{1, 1, 16});
...
TEST(TransformationTests, LowLatencyLSTM_Loop) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        ...
        auto res_ti_1 = std::make_shared<op::v0::Result>(loop->output(1));
        auto res_ti_2 = std::make_shared<op::v0::Result>(loop->output(0));
        f = std::make_shared<ngraph::Function>(ngraph::NodeVector{res_ti_1, res_ti_2},
                                               ngraph::ParameterVector{X, H_init, C_init});
