...
TEST(TransformationTests, ConvAddMulFusion) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);

    {
        auto input = std::make_shared<opset3::Parameter>(ngraph::element::f32, ngraph::Shape{ 1, 3, 16, 16 });

        auto weights = opset3::Constant::create(ngraph::element::f32, ngraph::Shape{ 3, 3, 1, 1 }, { 1.f });
        auto conv = std::make_shared<opset3::Convolution>(
            input, weights,
            ngraph::Strides{ 1, 1 },
        ...
        auto add_const = opset3::Constant::create(ngraph::element::f32, ngraph::Shape{ 1, 3, 1, 1 }, { 4 });
        auto add = std::make_shared<opset3::Add>(conv, add_const);
        auto mul_const = opset3::Constant::create(ngraph::element::f32, ngraph::Shape{ 1, 3, 1, 1 }, { 2 });
        auto mul = std::make_shared<opset3::Multiply>(add, mul_const);
