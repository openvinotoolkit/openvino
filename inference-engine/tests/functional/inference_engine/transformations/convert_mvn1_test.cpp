...
TEST(TransformationTests, Convert_MVN1_across_channels) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        auto data = std::make_shared<ngraph::opset2::Parameter>(ngraph::element::f32, ngraph::Shape{ 1, 2, 3, 4 });
        auto mvn = std::make_shared<ngraph::opset2::MVN>(data, true, true, 1e-5);
