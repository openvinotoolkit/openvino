...
TEST(TransformationTests, ConvertPadToConv) {
    std::shared_ptr<Function> f(nullptr), f_ref(nullptr);
    {
        ...
        manager.register_pass<pass::ConvertPadToConvolution>();
    ...
    {
        auto input = std::make_shared<opset4::Parameter>(element::f32, Shape{1, 3, 64, 64});
        auto weights = opset4::Constant::create(element::f32, Shape{3, 3, 1, 1}, {1});
        Strides stride{1, 1};
        CoordinateDiff pad_begin{1, 0}, pad_end{0, 1};
        auto conv = std::make_shared<opset4::Convolution>(input, weights, stride, pad_begin, pad_end, stride);
