...
TEST(TransformationTests, ScaleInputs_float) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        ...
        m.register_pass<ngraph::pass::ScaleInputsSingle>(2.0);
        m.run_passes(f);
    }
...
TEST(TransformationTests, ScaleInputs_float_dyn_shape) {
    ...
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        f = create_simple_function(shape);

        ngraph::pass::Manager m;
        m.register_pass<ngraph::pass::InitNodeInfo>();
        m.register_pass<ngraph::pass::ScaleInputsSingle>(4.0);
        m.run_passes(f);
    }
...
TEST(TransformationTests, ScaleInputs_map_scale1) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        f = create_function_2inputs(ngraph::Shape{1, 3, 5, 5});

        ngraph::pass::Manager m;
        m.register_pass<ngraph::pass::InitNodeInfo>();
        std::map<std::string, std::vector<float>> map;
        map.insert({"input1", {2.f, 4.f, 8.f}});
        m.register_pass<ngraph::pass::ScaleInputsVector>(map);
        m.run_passes(f);
    }
...
TEST(TransformationTests, ScaleInputs_map_scale2) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        f = create_function_2inputs(ngraph::Shape{1, 3, 5, 5});
        ngraph::pass::Manager m;
...

    {
        ...
        auto data2 = std::make_shared<ngraph::opset7::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 5, 5});
        data2->set_friendly_name("input2");
        auto mul_const2 = ngraph::opset7::Constant::create(
                ngraph::element::f32, ngraph::Shape{1, 1, 1, 1}, {0.2f});
        mul_const2->set_friendly_name("input2/scale/Fused_Mul_Factor");
        auto mul2 = std::make_shared<ngraph::opset7::Multiply>(data2, mul_const2);
        mul2->set_friendly_name("input2/scale/Fused_Mul");
        auto add = std::make_shared<ngraph::opset7::Add>(mul1, mul2);
...
TEST(TransformationTests, ScaleInputs_map_dim_idx) {
    std::shared_ptr<ngraph::Function> f = create_simple_function(ngraph::Shape{1, 3, 3, 3}), f_ref(nullptr);
    {
        ...
        std::map<std::string, std::vector<float>> map;
        map.insert({"input1", {2.f, 4.f, 8.f}});
        m.register_pass<ngraph::pass::ScaleInputsVector>(map, 2);
        m.run_passes(f);
    }
...
TEST(TransformationTests, ScaleInputs_map_dim_idx_out_of_range) {
    ...
    std::map<std::string, std::vector<float>> map;
    map.insert({"input1", {2.f, 4.f, 8.f}});
    m.register_pass<ngraph::pass::ScaleInputsVector>(map, 2000);
...
TEST(TransformationTests, ScaleInputs_map_dynamic_one_value) {
    ngraph::PartialShape shape = {ngraph::Dimension::dynamic(), 1, ngraph::Dimension::dynamic()};
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        f = create_simple_function(shape);
...

    {
        auto data1 = std::make_shared<ngraph::opset7::Parameter>(ngraph::element::f32, shape);
        data1->set_friendly_name("input1");
        auto mul_const1 = ngraph::opset7::Constant::create(ngraph::element::f32, ngraph::Shape{1, 1, 1}, {0.5});
        mul_const1->set_friendly_name("input1/scale/Fused_Mul_Factor");
        auto mul1 = std::make_shared<ngraph::opset7::Multiply>(data1, mul_const1);
        mul1->set_friendly_name("input1/scale/Fused_Mul");
        auto res = std::make_shared<ngraph::opset7::Result>(mul1);
...
TEST(TransformationTests, ScaleInputs_map_full_dynamic_one_value) {
