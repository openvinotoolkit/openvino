...
TEST(PDPD_Reader_Tests, ImportBasicModelToCore) {
    ...
    const auto relu = std::make_shared<ngraph::op::v0::Relu>(data->output(0));
    relu->set_friendly_name("relu_0.tmp_0");
    relu->output(0).get_tensor().add_names({ "relu_0.tmp_0" });
    const auto scale = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::f32, ngraph::Shape{ 1 }, std::vector<float>{1});
    const auto bias = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::f32, ngraph::Shape{ 1 }, std::vector<float>{0});
    const auto node_multiply = std::make_shared<ngraph::op::v1::Multiply>(relu->output(0), scale);
    const auto node_add = std::make_shared<ngraph::op::v1::Add>(node_multiply, bias);
    ...
    const auto result = std::make_shared<ngraph::op::v0::Result>(node_add->output(0));
