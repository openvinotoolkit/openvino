...
public:
    void SetUp() override {
        ...
        {
            auto input_A = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, test_case.A_shape);
            input_A->set_friendly_name("input_A");
            auto input_B = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, test_case.B_shape);
            ...
            auto reshape = std::make_shared<ngraph::op::v1::Reshape>(test_case.reshape_is_A_input ? input_A : input_B, reshape_pattern, true);
            reshape->set_friendly_name("reshape");

            auto mat_mul = std::make_shared<ngraph::op::v0::MatMul>(test_case.reshape_is_A_input ? reshape->output(0) : input_A->output(0),
...
TEST(SmartReshapeTransposeMatMulTests, TransposeAMatMulFuse) {
    ...
    {
        auto data_A = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 2});
        auto data_B = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 5});
        auto order = ngraph::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{3}, {0, 2, 1});
        auto transpose = std::make_shared<ngraph::op::v1::Transpose>(data_A, order);
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(transpose, data_B, false, false);
    ...
    }
    {
        auto data_A = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 2});
        auto data_B = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 5});
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(data_A, data_B, true, false);
...
TEST(SmartReshapeTransposeMatMulTests, TransposeBMatMulFuse) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        auto data_A = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 2, 3});
        auto data_B = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 5, 3});
        auto order = ngraph::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{3}, {0, 2, 1});
        auto transpose = std::make_shared<ngraph::op::v1::Transpose>(data_B, order);
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(data_A, transpose, false, false);
        f = std::make_shared<ngraph::Function>(ngraph::NodeVector{matmul}, ngraph::ParameterVector{data_A, data_B});
    ...
    }
    {
        auto data_A = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 2, 3});
        auto data_B = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 5, 3});
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(data_A, data_B, false, true);
        f_ref = std::make_shared<ngraph::Function>(ngraph::NodeVector{matmul}, ngraph::ParameterVector{data_A, data_B});
    }
...
TEST(SmartReshapeTransposeMatMulTests, TransposeAMatMulWithAttrFuse) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        ...
        auto order = ngraph::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{3}, {0, 2, 1});
        auto transpose = std::make_shared<ngraph::op::v1::Transpose>(data_A, order);
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(transpose, data_B, true, false);
        f = std::make_shared<ngraph::Function>(ngraph::NodeVector{matmul}, ngraph::ParameterVector{data_A, data_B});
    ...
    }
    {
        auto data_A = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 2, 3});
        auto data_B = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 5});
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(data_A, data_B, false, false);
        f_ref = std::make_shared<ngraph::Function>(ngraph::NodeVector{matmul}, ngraph::ParameterVector{data_A, data_B});
    }
...
TEST(SmartReshapeTransposeMatMulTests, TransposeBMatMulWithAttrFuse) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        ...
        auto order = ngraph::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{3}, {0, 2, 1});
        auto transpose = std::make_shared<ngraph::op::v1::Transpose>(data_B, order);
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(data_A, transpose, false, true);
        f = std::make_shared<ngraph::Function>(ngraph::NodeVector{matmul}, ngraph::ParameterVector{data_A, data_B});
...
TEST(SmartReshapeTransposeMatMulTests, TransposeAMatMulSideAttrFuse) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        ...
        auto order = ngraph::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{3}, {0, 2, 1});
        auto transpose = std::make_shared<ngraph::op::v1::Transpose>(data_A, order);
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(transpose, data_B, true, true);
        f = std::make_shared<ngraph::Function>(ngraph::NodeVector{matmul}, ngraph::ParameterVector{data_A, data_B});
...
TEST(SmartReshapeTransposeMatMulTests, TransposeBMatMulSideAttrFuse) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        ...
        auto order = ngraph::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{3}, {0, 2, 1});
        auto transpose = std::make_shared<ngraph::op::v1::Transpose>(data_B, order);
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(data_A, transpose, true, true);
        f = std::make_shared<ngraph::Function>(ngraph::NodeVector{matmul}, ngraph::ParameterVector{data_A, data_B});
...
TEST(SmartReshapeTransposeMatMulTests, TransposeBothMatMulFuse) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        ...
        auto data_B = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 5, 3});
        auto order = ngraph::op::v0::Constant::create(ngraph::element::i64, ngraph::Shape{3}, {0, 2, 1});
        auto transpose_A = std::make_shared<ngraph::op::v1::Transpose>(data_A, order);
        auto transpose_B = std::make_shared<ngraph::op::v1::Transpose>(data_B, order);
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(transpose_A, transpose_B, false, false);
        f = std::make_shared<ngraph::Function>(ngraph::NodeVector{matmul}, ngraph::ParameterVector{data_A, data_B});
    ...
    }
    {
        auto data_A = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 3, 2});
        auto data_B = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 5, 3});
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(data_A, data_B, true, true);
        f_ref = std::make_shared<ngraph::Function>(ngraph::NodeVector{matmul}, ngraph::ParameterVector{data_A, data_B});
    }
...
TEST(SmartReshapeTransposeMatMulTests, TransposeBothMatMulWithAttrFuse) {
    std::shared_ptr<ngraph::Function> f(nullptr), f_ref(nullptr);
    {
        ...
        auto transpose_A = std::make_shared<ngraph::op::v1::Transpose>(data_A, order);
        auto transpose_B = std::make_shared<ngraph::op::v1::Transpose>(data_B, order);
        auto matmul = std::make_shared<ngraph::op::v0::MatMul>(transpose_A, transpose_B, false, true);
        f = std::make_shared<ngraph::Function>(ngraph::NodeVector{matmul}, ngraph::ParameterVector{data_A, data_B});
