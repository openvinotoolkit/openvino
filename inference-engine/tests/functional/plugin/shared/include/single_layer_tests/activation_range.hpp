...
typedef std::tuple<
        ...
        std::pair<int32_t, uint32_t>,  // Range of input values
        std::string> activationRangeParams;

class ActivationLayerRangeTest : public testing::WithParamInterface<activationRangeParams>,
                            virtual public LayerTestsUtils::LayerTestsCommon {
public:
    ngraph::helpers::ActivationTypes activationType;
    static std::string getTestCaseName(const testing::TestParamInfo<activationRangeParams> &obj);
    InferenceEngine::Blob::Ptr GenerateInput(const InferenceEngine::InputInfo &info) const override;

protected:
    void SetUp() override;

    std::pair<int32_t, uint32_t> inputValuesRange;
};
