...
using CallbackTests = BehaviorTestsUtils::InferRequestTests;

TEST_P(CallbackTests, canCallAsyncWithCompletionCallback) {
...
TEST_P(CallbackTests, syncInferDoesNotCallCompletionCallback) {
    // Skip test according to plugin specific disabledTestPatterns() (if any)
    SKIP_IF_CURRENT_TEST_IS_DISABLED()
...
TEST_P(CallbackTests, ReturnResultNotReadyFromWaitInAsyncModeForTooSmallTimeout) {
    // Skip test according to plugin specific disabledTestPatterns() (if any)
    SKIP_IF_CURRENT_TEST_IS_DISABLED()
