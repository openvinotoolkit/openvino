...
TEST_P(IEClassNetworkTestP, SetAffinityWithConstantBranches) {
    ...
    try {
        ...
        auto modifier = [](ngraph::Node& node, std::vector<std::string>& attributes) {
            std::stringstream ss;
            ss << getFusedNames(node.shared_from_this());
            if (!ss.str().empty()) {
                attributes.push_back("label=\"FusedNames: " + ss.str() + "\"");
            }
        };

        ngraph::pass::Manager manager;
        manager.register_pass<ngraph::pass::VisualizeTree>("/tmp/test_0.svg", modifier);
        manager.register_pass<ngraph::pass::InitNodeInfo>();
        manager.register_pass<ngraph::pass::VisualizeTree>("/tmp/test_1.svg", modifier);
        manager.register_pass<ngraph::pass::ConstantFolding>();
        manager.register_pass<ngraph::pass::VisualizeTree>("/tmp/test_2.svg", modifier);
        manager.run_passes(func);

        CNNNetwork net(func);
        net.serialize("/tmp/test.xml", "/tmp/test.bin");
