...
void get_port_range(const std::shared_ptr<ngraph::op::Constant> &const_node, OPInfo::PortInfo &port_info) {
...
void get_port_range(const std::shared_ptr<ngraph::op::Constant> &constant_input, OPInfo::PortInfo &port_info) {
...
const std::shared_ptr<ngraph::Node> clone(const std::shared_ptr<ngraph::Node> &node, OPInfo &meta) {
    ...
    for (size_t i = 0; i < node->get_input_size(); ++i) {
        ...
        auto port_info = OPInfo::PortInfo();
...
std::shared_ptr<ngraph::Node> clone_weightable_node(const std::shared_ptr<ngraph::Node> &node,
                                                    const std::vector<size_t> &weight_ports, OPInfo &meta) {
    ngraph::OutputVector op_inputs;
    for (size_t i = 0; i < node->get_input_size(); ++i) {
        const auto input = node->input(i).get_source_output();
