...
const auto deformablePSROIParams = ::testing::Combine(
    ...
    ::testing::ValuesIn(std::vector<std::vector<size_t>>{{1, 1}, {2, 2}, {3, 3}}),         // spatial_bins_x_y
...
const auto deformablePSROICases_test_params_model = ::testing::Combine(
    deformablePSROIParams_model,
    ::testing::Values(InferenceEngine::Precision::FP32), // Net precision
    ::testing::Values(CommonTestUtils::DEVICE_CPU)       // Device name
);

INSTANTIATE_TEST_CASE_P(smoke_TestsDeformablePSROIPooling_model, DeformablePSROIPoolingLayerTest, deformablePSROICases_test_params_model,
                        DeformablePSROIPoolingLayerTest::getTestCaseName);
