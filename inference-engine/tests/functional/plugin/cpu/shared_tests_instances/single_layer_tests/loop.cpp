...
std::vector<std::vector<std::pair<std::vector<size_t>, LOOP_IN_TYPE>>> inputs = {
    {{{32, 1, 10}, LOOP_IN_TYPE::INVARIANT},
     {{32, 1, 10}, LOOP_IN_TYPE::INVARIANT},
     {{32, 1, 10}, LOOP_IN_TYPE::MERGED}},
};
...
INSTANTIATE_TEST_SUITE_P(smoke_StaticShapeLoop,
                         StaticShapeLoopTest,
                         Combine(ValuesIn(std::vector<bool>{true, false}),
                                 Values(true),
...
using namespace testing;
INSTANTIATE_TEST_SUITE_P(smoke_TrivialLoop,
                         TrivialLoopTest,
                         Combine(Values<InferenceEngine::Precision>(Precision::FP32, Precision::I32),
