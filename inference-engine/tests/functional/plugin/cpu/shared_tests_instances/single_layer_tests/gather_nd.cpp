...
INSTANTIATE_TEST_SUITE_P(smoke_Set1,
                         GatherNDLayerTest,
                         ::testing::Combine(gatherNDArgsSubset1,
...
const auto gatherNDArgsSubset2 = ::testing::Combine(
    ::testing::ValuesIn(std::vector<std::vector<size_t>>({{15, 12, 20, 15, 2}, {15, 12, 18, 7, 17}})),  // Data shape
    ::testing::ValuesIn(std::vector<std::vector<size_t>>({{15, 12, 2}, {15, 12, 5, 9, 1, 3}})),         // Indices shape
    ::testing::ValuesIn(std::vector<int>({0, 1, 2}))                                                    // Batch dims
);
INSTANTIATE_TEST_SUITE_P(smoke_Set2,
                         GatherNDLayerTest,
                         ::testing::Combine(gatherNDArgsSubset2,
                                            ::testing::ValuesIn(dPrecisions),
                                            ::testing::ValuesIn(iPrecisions),
                                            ::testing::Values(CommonTestUtils::DEVICE_CPU),
                                            ::testing::Values<Config>({})),
