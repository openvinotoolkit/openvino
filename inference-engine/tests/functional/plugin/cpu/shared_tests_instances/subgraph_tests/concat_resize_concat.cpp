...
typedef std::tuple<ngraph::NodeTypeInfo,  // Node type
                   int,                   // channels count
                   int                    // batch count
                   >
...
protected:
    void SetUp() override {
        ...
        auto outputShape =
            std::make_shared<ngraph::opset3::Constant>(ngraph::element::i64, ngraph::Shape{2}, shape.data());
        auto scalesShape =
            std::make_shared<ngraph::opset3::Constant>(ngraph::element::f32, ngraph::Shape{2}, scales.data());
        auto axes = std::make_shared<ngraph::opset3::Constant>(ngraph::element::i64,
                                                               ngraph::Shape{2},
                                                               std::vector<int64_t>{2, 3});
        std::shared_ptr<ngraph::Node> preresizeNode =
            std::make_shared<ngraph::opset4::Interpolate>(inputNode, outputShape, scalesShape, axes, attrs);
...
INSTANTIATE_TEST_CASE_P(smoke_ConcResizeConc,
                        ...
                        ::testing::Combine(::testing::Values(ngraph::opset4::Interpolate::type_info),
