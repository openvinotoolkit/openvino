...
std::string FusePermuteAndReorderTest::getTestCaseName(testing::TestParamInfo<FusePermuteAndReorderParams> obj) {
    ...
    Precision inPrec;
    std::tie(graphType, inputShape, inPrec) = obj.param;

    result << "GraphType=" << graphType << "_";
...
void FusePermuteAndReorderTest::createGraph0() {
...
void FusePermuteAndReorderTest::createGraph1() {
    auto ngPrc = FuncTestUtils::PrecisionUtils::convertIE2nGraphPrc(inPrec);
    auto params = ngraph::builder::makeParams(ngPrc, {inputShape});
...
void FusePermuteAndReorderTest::SetUp() {
    ...
    std::tie(graphType, inputShape, inPrec) = this->GetParam();
    if (graphType == graph0) {
        createGraph0();
    } else if (graphType == graph1) {
        createGraph1();
    } else {
        THROW_IE_EXCEPTION << "Unsupported TestGraphType for FusePermuteAndReorderTest";
    }
}
...
TEST_P(FusePermuteAndReorderTest, CompareWithRefs) {
    ...
    size_t expectedPermuteCount = graphType == graph0 ? 0 : 2;
    ASSERT_EQ(permuteCount, expectedPermuteCount);
}

const auto fusePermuteAndReorderParams = ::testing::Combine(
        ::testing::Values(graph0, graph1),
