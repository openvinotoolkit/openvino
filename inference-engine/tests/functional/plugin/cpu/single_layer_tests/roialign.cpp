...
typedef std::tuple<int,                             // bin's column count
                   int,                             // bin's row count
                   float,                           // scale for given region considering actual input size
                   int,                             // pooling ratio
                   std::pair<std::vector<float>,    // united proposal vector of coordinates and
...
typedef std::tuple<ROIAlignSpecificParams,
                   InferenceEngine::Precision,    // Net precision
                   LayerTestsUtils::TargetDevice  // Device name
                   >
    ROIAlignLayerTestParams;

typedef std::tuple<CPULayerTestsDefinitions::ROIAlignLayerTestParams, CPUSpecificParams> ROIAlignLayerCPUTestParamsSet;
...
public:
    static std::string getTestCaseName(testing::TestParamInfo<ROIAlignLayerCPUTestParamsSet> obj) {
        ...
        std::tie(pooledH, pooledW, spatialScale, samplingRatio, proposal, mode, inputShape) = roiPar;
...
protected:
    void SetUp() override {
        ...
        std::tie(pooledH, pooledW, spatialScale, samplingRatio, proposal, mode, inputShape) = roiAlignParams;
        ...
        auto roialign = std::make_shared<ngraph::opset3::ROIAlign>(params[0],
                                                                   coords,
                                                                   roisIdx,
                                                                   pooledH,
                                                                   pooledW,
...
const std::vector<std::vector<size_t>> inputShapeVector = {SizeVector({2, 18, 20, 20}),
