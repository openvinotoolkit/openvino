...
typedef std::tuple<std::vector<int>,    // pooled vector
                   std::vector<size_t>  // feature map shape
                   >
    AdaPoolSpecificParams;

typedef std::tuple<AdaPoolSpecificParams,
                   std::string,                   // mode
...
typedef std::tuple<CPULayerTestsDefinitions::AdaPoolLayerTestParams, CPUSpecificParams> AdaPoolLayerCPUTestParamsSet;
...
protected:
    void SetUp() override {
        ...
        auto pooledParam =
            ngraph::builder::makeConstant<int32_t>(ngraph::element::i32, coordsShape, pooledSpatialShape);
        auto params = ngraph::builder::makeParams(ngraph::element::f32, {inputShape});

        // we cannot create abstract Op to use polymorphism
        auto adapoolMax =
            std::make_shared<ngraph::opset8::AdaptiveMaxPool>(params[0], pooledParam, ngraph::element::i32);
