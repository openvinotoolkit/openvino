...
protected:
        ...
        for (int i = 0; i < functionParams.size(); ++i) {
            ...
            if (!inputDynamicShapes.empty()) {
                if (inputDynamicShapes[i].rank() != 0) {
                    ...
                    blob = FuncTestUtils::createAndFillBlob(infoNew.getTensorDesc(), 0, scalarVal);
                }
            }
            if (blob == nullptr) {
                blob = FuncTestUtils::createAndFillBlob((*info).getTensorDesc(), 0, scalarVal);
            }
    ...
    void SetUp() override {
        ...
        std::pair<std::vector<ngraph::PartialShape>, std::vector<std::vector<ngraph::Shape>>> shapes;
        std::tie(rangeParams, netPrecision, targetDevice) = basicParamsSet;
        std::tuple<float, float, float> rangeInputs;
        InferenceEngine::Precision outPr = Precision::FP32;

        std::tie(shapes, rangeInputs, outPr) = rangeParams;
        ...
        auto ngOutPr = FuncTestUtils::PrecisionUtils::convertIE2nGraphPrc(outPr);
...
const std::vector<std::tuple<float, float, float>> rangeInputValues = {
        {1.0, -5.0, -1.0}, {1.0, 10.0, 1.2},
        {1.1, 12.2, 1.1}, {1.1, -5.1, -1.1}
};
