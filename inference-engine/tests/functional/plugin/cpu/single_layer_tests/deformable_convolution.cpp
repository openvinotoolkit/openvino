        ...
        for (const auto &input : cnnNetwork.getInputsInfo()) {
            ...
            if (name == "a_data") {
                ...
                if (offsetType == OffsetType::NATURAL) {
                    ...
                    blob = FuncTestUtils::createAndFillBlobFloat(info->getTensorDesc(), 0, 0, 1);
                ...
                blob = FuncTestUtils::createAndFillBlobFloat(info->getTensorDesc(), 1, 0, 20);
            } else {
...
const auto defConvSpecificParams = ::testing::Combine(
        ::testing::ValuesIn({ true, false }),  // with_bilinear_interpolation_pad
        ::testing::ValuesIn({ true, false }),  // with_modulation
        ::testing::ValuesIn({
            OffsetType::NATURAL,
            OffsetType::ZERO,
...
std::vector<ngraph::op::PadType> padTypes = {ngraph::op::PadType::EXPLICIT,
...
const auto chParams = ::testing::Combine(
        ::testing::ValuesIn(std::vector<size_t> {2, 4}),  // gr. 1,2,4
        ::testing::ValuesIn(std::vector<size_t> {1, 2}),  // def. gr. 1,2
        ::testing::ValuesIn(std::vector<size_t> {1, 2}),  // in. ch. per gr. 1,2
        ::testing::ValuesIn(std::vector<size_t> {2, 4}));  // out. ch. per gr. 2,4

INSTANTIATE_TEST_SUITE_P(smoke_DefConvLayoutTest1, DefConvLayerCPUTest,
                         ::testing::Combine(
                                 ::testing::Combine(
                                         spParams1,
                                         chParams,
...
INSTANTIATE_TEST_SUITE_P(smoke_DefConvLayoutTest2, DefConvLayerCPUTest,
                         ::testing::Combine(
                                 ::testing::Combine(
                                         spParams2,
                                         chParams,
