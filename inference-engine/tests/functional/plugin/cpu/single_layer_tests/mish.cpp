...
#include <single_layer_tests/mish.hpp>
...
protected:
    void SetUp() {
        ...
        auto activationType = ngraph::helpers::ActivationTypes::Mish;
        auto ngPrc = FuncTestUtils::PrecisionUtils::convertIE2nGraphPrc(netPrecision);
        auto params = ngraph::builder::makeParams(ngPrc, {inputShape});
        auto paramOuts = ngraph::helpers::convert2OutputVector(
                ngraph::helpers::castOps2Nodes<ngraph::op::Parameter>(params));
        auto mish = std::dynamic_pointer_cast<ngraph::op::v4::Mish>(
                ngraph::builder::makeActivation(paramOuts[0], ngPrc, activationType, inputShape));
...
INSTANTIATE_TEST_CASE_P(Mish_2D_Planar_FP32, MishLayerCPUTest,
                        ::testing::Combine(
                                ::testing::Combine(
                                        ...
                                        ::testing::Values(std::vector<size_t >({2, 3, 5, 5})),
...
INSTANTIATE_TEST_CASE_P(Mish_3D_Planar_FP32, MishLayerCPUTest,
                        ::testing::Combine(
                                ::testing::Combine(
                                        ::testing::Values(Precision::FP32),
                                        ::testing::Values(std::vector<size_t >({2, 3, 5, 5, 5})),
                                        ::testing::Values(CommonTestUtils::DEVICE_CPU)),
...
INSTANTIATE_TEST_CASE_P(Mish_Blocked_FP32, MishLayerCPUTest,
                        ::testing::Combine(
                                ::testing::Combine(
                                        ::testing::Values(Precision::FP32),
                                        ::testing::Values(std::vector<size_t >({2, 16, 3, 3})),
                                        ::testing::Values(CommonTestUtils::DEVICE_CPU)),
...
INSTANTIATE_TEST_CASE_P(Mish_3D_Blocked_FP32, MishLayerCPUTest,
                        ::testing::Combine(
                                ::testing::Combine(
                                        ::testing::Values(Precision::FP32),
                                        ::testing::Values(std::vector<size_t >({2, 16, 3, 3, 3})),
                                        ::testing::Values(CommonTestUtils::DEVICE_CPU)),
