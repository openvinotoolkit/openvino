...
protected:
    std::shared_ptr<ngraph::Function> createGraph(InferenceEngine::Precision netPrecision) override {
        ...
        ngraph::Shape convFilterShape = {intermediateChannelsCount,
                                         channelsCount,
                                         3,
        ...
        std::shared_ptr<ngraph::Node> convNode2 =
            std::make_shared<ngraph::opset1::Convolution>(argmaxNode->output(0),
                                                          weightsNode2->output(0),
                                                          ngraph::Strides({1, 1}),         // strides
                                                          ngraph::CoordinateDiff({0, 0}),  // pad begin
                                                          ngraph::CoordinateDiff({0, 0}),  // pad end
                                                          ngraph::Strides({1, 1}),         // dilation
        ...
        return std::make_shared<ngraph::Function>(ngraph::OutputVector{convNode2->output(0), argmaxNode->output(1)},
