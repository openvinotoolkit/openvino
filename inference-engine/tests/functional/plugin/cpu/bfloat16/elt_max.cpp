...
protected:
    std::shared_ptr<ngraph::Function> createGraph(InferenceEngine::Precision netPrecision) override {
        ...
        if (netPrecision == Precision::FP32) {
            ...
            powerConst = opset1::Constant::create(
                ntype,
                Shape{1},
        ...
        ngraph::Shape convFilterShape0 = {conv0OutputChannels,
                                          channelsCount,
                                          3,
                                          3};  // out channel, /input channels, kernel h, kernel w
        ngraph::Shape convFilterShape1 = {1,
                                          conv0OutputChannels,
                                          3,
                                          3};  // out channel, /input channels, kernel h, kernel w
        if (netPrecision == Precision::FP32) {
            ...
            weightsNode0 =
                std::make_shared<ngraph::opset1::Constant>(ntype, convFilterShape0, weightValuesBF16_0.data());
            weightsNode1 =
                std::make_shared<ngraph::opset1::Constant>(ntype, convFilterShape1, weightValuesBF16_1.data());
        }

        std::shared_ptr<ngraph::Node> convNode0 =
            std::make_shared<ngraph::opset1::Convolution>(powerNode0,
                                                          weightsNode0,
        ...
        if (netPrecision == Precision::FP32) {
            maxConst =
                opset1::Constant::create(ntype, Shape{batchSize, conv0OutputChannels, heightSize, widthSize}, {2.0f});
        } else {
            maxConst = opset1::Constant::create(
                ntype,
                Shape{batchSize, conv0OutputChannels, heightSize, widthSize},
                {bfloat16::from_bits(FuncTestUtils::Bf16TestUtils::reducePrecisionBitwiseS(2.0f))});
        }
