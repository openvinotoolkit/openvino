...
inline std::shared_ptr<ngraph::Function> makeTestModel(std::vector<size_t> inputShape = {1, 1, 32, 32}) {
    ...
    auto in = std::make_shared<ngraph::op::v0::Parameter>(et, in_shape);
    auto gelu = std::make_shared<ngraph::op::v7::Gelu>(in);
    auto swish_const = ngraph::op::Constant::create(et, ngraph::Shape{}, {2.5f});
    auto swish = std::make_shared<ngraph::op::v4::Swish>(gelu, swish_const);
    ...
    auto reshape2 = std::make_shared<ngraph::op::v1::Reshape>(swish, const2, false);
