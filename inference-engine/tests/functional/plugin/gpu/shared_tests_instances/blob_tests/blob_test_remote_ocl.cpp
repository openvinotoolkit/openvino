...
namespace {
    ...
    InferenceEngine::Blob::Ptr MakeTestBlobOCL(InferenceEngine::Blob::Ptr inputBlob, InferenceEngine::ExecutableNetwork& execNet) {
        ...
        auto shared_blob_test = std::shared_ptr<Blob>(nullptr,
            [](Blob *pi) { delete pi; }); // deleter ))
        auto deleter = std::get_deleter<void(*)(Blob*)>(shared_blob);
        shared_blob_test.swap(shared_blob);
        bufferMap[&(*shared_blob_test)] = shared_buffer;
        return shared_blob_test;
    }
} // namespace
...
INSTANTIATE_TEST_CASE_P(smoke_Remote_OCL, BlobTestBase,
    ::testing::Combine(
        ...
        ::testing::ValuesIn(funtions),
        ::testing::Values(MakeTestBlobOCL),
        ::testing::Values(Teardown),
        ::testing::ValuesIn(inferCounts),
        ::testing::ValuesIn(batchSizes),
        ::testing::ValuesIn(asyncExec),
...
INSTANTIATE_TEST_CASE_P(smoke_Remote_OCL_dynamic_batch, BlobTestBase,
    ::testing::Combine(
        ...
        ::testing::ValuesIn(asyncExec),
        ::testing::Values(CommonTestUtils::DEVICE_GPU),
        ::testing::Values(dyn_batch_config)),
    BlobTestBase::getTestCaseName);
