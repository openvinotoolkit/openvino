...
public:
    ...
    IE_SUPPRESS_DEPRECATED_START
    /**
     * @deprecated Use InferenceEngine::ExecutableNetwork::GetExecGraphInfo
     * instead
     ...
     * dymanically, e.g. DEVICE_ID cannot changed if an executable network has
     ...
     *
     * @param pContext Reference to a pointer that will receive resulting shared
     * context object ptr
     * @param resp Pointer to the response message that holds a description of an
     * error if any occurred
     * @return code of the operation. InferenceEngine::OK if succeeded
