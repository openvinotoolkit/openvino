 ...
 * This option should be used with values: CONFIG_VALUE(YES) or CONFIG_VALUE(NO)
 * (default)
 */
 ...
 * a tensor. This option should be used with values: CONFIG_VALUE(YES) or
 * CONFIG_VALUE(NO) (default)
 */
...
/**
 * @deprecated Use InputInfo::setLayout on input data from
 * CNNNetwork::getInputsInfo() or Data::setLayout on output data from
 * CNNNetwork::getOutputsInfo()
 * @brief This option allows to to specify input output layouts for network
 * layers. By default, this value set to VPU_CONFIG_VALUE(AUTO) value. Supported
 * values: VPU_CONFIG_VALUE(AUTO) executable network configured to use optimal
 * layer layout depending on available HW VPU_CONFIG_VALUE(NCHW) executable
 * network forced to use NCHW input/output layouts VPU_CONFIG_VALUE(NHWC)
 * executable network forced to use NHWC input/output layouts
 */
...
/**
 * @deprecated Use InferenceEngine::MYRIAD_PCIE or InferenceEngine::MYRIAD_USB
 * instead.
