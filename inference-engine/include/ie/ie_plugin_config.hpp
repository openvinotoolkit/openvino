...
/**
 * @brief A header for advanced hardware related properties for Inference Engine
 * plugins To use in SetConfig, LoadNetwork, ImportNetwork methods of plugins
 *
 ...
 * @brief shortcut for defining common Inference Engine ExecutableNetwork
 * metrics
 */
 ...
 * String value is "AVAILABLE_DEVICES"
 */
DECLARE_METRIC_KEY(AVAILABLE_DEVICES, std::vector<std::string>);

/**
 * @brief Metric to get a std::vector<std::string> of supported metrics. String
 * value is "SUPPORTED_METRICS"
 *
 * This can be used as an executable network metric as well.
 *
 * Each of the returned device metrics can be passed to Core::GetMetric,
 * executable network metrics can be passed to ExecutableNetwork::GetMetric.
 *
 */
 ...
 * String value is "SUPPORTED_CONFIG_KEYS"
 *
 * This can be used as an executable network metric as well.
 *
 * Each of the returned device configuration keys can be passed to
 * Core::SetConfig, Core::GetConfig, and Core::LoadNetwork, configuration keys
 * for executable networks can be passed to ExecutableNetwork::SetConfig and
 ...
 * String value is "FULL_DEVICE_NAME"
 */
DECLARE_METRIC_KEY(FULL_DEVICE_NAME, std::string);

/**
 * @brief Metric to get a std::vector<std::string> of optimization options per
 * device. String value is "OPTIMIZATION_CAPABILITIES"
 *
 * The possible values:
 ...
 *  - "WINOGRAD" - device can support models where convolution implemented via
...
/**
 * @brief Metric to provide information about a range for streams on platforms
 * where streams are supported.
 *
...
/**
 * @brief Metric to provide a hint for a range for number of async infer
 * requests. If device supports streams, the metric provides range for number of
 * IRs per stream.
 *
 * Metric returns a value of std::tuple<unsigned int, unsigned int, unsigned
 * int> type, where:
 *  - First value is bottom bound.
 *  - Second value is upper bound.
...
/**
 * @brief Metric to get an unsigned int value of number of waiting infer
 * request.
 *
 * String value is "NUMBER_OF_WAITNING_INFER_REQUESTS". This can be used as an
 * executable network metric as well
 */
DECLARE_METRIC_KEY(NUMBER_OF_WAITING_INFER_REQUESTS, unsigned int);

/**
 * @brief Metric to get an unsigned int value of number of infer request in
 * execution stage.
 *
 * String value is "NUMBER_OF_EXEC_INFER_REQUESTS". This can be used as an
 * executable network metric as well
 */
...
/**
 * @brief Metric to get a type of device. See DeviceType enum definition for
 * possible return values
 */
DECLARE_METRIC_KEY(DEVICE_TYPE, DeviceType);

/**
 * @brief Metric which defines Giga OPS per second count (GFLOPS or GIOPS) for a
 * set of precisions supported by specified device
 */
...
/**
 * @brief  Metric to get a float of device thermal. String value is
 * "DEVICE_THERMAL"
 */
DECLARE_METRIC_KEY(DEVICE_THERMAL, float);

/**
 * @brief Metric to get an unsigned integer value of optimal number of
 * executable network infer requests.
 */
...
/**
 * @brief Limit `#threads` that are used by Inference Engine for inference on
 * the CPU.
 */
 ...
 * PluginConfigParams::YES, which is default on the conventional CPUs (pinning
 * threads to cores, best for static benchmarks),
 *
 * the following options are implemented only for the TBB as a threading option
 * PluginConfigParams::NUMA (pinning threads to NUMA nodes, best for real-life,
 * contented cases) on the Windows and MacOS* this option behaves as YES
 * PluginConfigParams::HYBRID_AWARE (let the runtime to do pinning to the cores
 * types, e.g. prefer the "big" cores for latency tasks) on the hybrid CPUs this
 * option is default
 *
 * Also, the settings are ignored, if the OpenVINO compiled with OpenMP and any
 * affinity-related OpenMP's environment variable is set (as affinity is
 ...
 *
 * It is passed to Core::SetConfig(), this option should be used with values:
 * - KEY_CPU_THROUGHPUT_NUMA creates as many streams as needed to accommodate
 * NUMA and avoid associated penalties
 * - KEY_CPU_THROUGHPUT_AUTO creates bare minimum of streams to improve the
 * performance, this is the most portable option if you have no insights into
 * how many cores you target machine will have (and what is the optimal number
 * of streams)
 * - finally, specifying the positive integer value creates the requested number
 * of streams
 */
 ...
 *
 * Specified value is applied to all following Infer() calls. Inference Engine
 * processes min(batch_limit, original_batch_size) first pictures from input
 * blob. For example, if input blob has sizes 32x3x224x224 after applying
 * plugin.SetConfig({KEY_DYN_BATCH_LIMIT, 10}) Inference Engine primitives
 * processes only beginner subblobs with size 10x3x224x224. This value can be
 * changed before any Infer() call to specify a new batch limit.
 *
 * The paired parameter value should be convertible to integer number.
 * Acceptable values: -1 - Do not limit batch processing >0 - Direct value of
 * limit. Batch size to process is min(new batch_limit, original_batch)
 */
 ...
 *
 * This option should be used with values: PluginConfigParams::LOG_NONE
 * (default), PluginConfigParams::LOG_ERROR, PluginConfigParams::LOG_WARNING,
 * PluginConfigParams::LOG_INFO, PluginConfigParams::LOG_DEBUG,
 * PluginConfigParams::LOG_TRACE
 */
...
/**
 * @brief the key for enabling exclusive mode for async requests of different
 * executable networks and the same plugin.
 *
 * Sometimes it is necessary to avoid oversubscription requests that are sharing
 * the same device in parallel. E.g. There 2 task executors for CPU device: one
 * - in the Hetero plugin, another - in pure CPU plugin. Parallel execution both
 * of them might lead to oversubscription and not optimal CPU usage. More
 * efficient to run the corresponding tasks one by one via single executor. By
 * default, the option is set to YES for hetero cases, and to NO for
 * conventional (single-plugin) cases Notice that setting YES disables the CPU
 * streams feature (see another config key in this file)
 */
DECLARE_CONFIG_KEY(EXCLUSIVE_ASYNC_REQUESTS);

/**
 * @deprecated Use
 * InferenceEngine::ExecutableNetwork::GetExecGraphInfo::serialize method
 * @brief This key enables dumping of the internal primitive graph.
 *
 * Should be passed into LoadNetwork method to enable dumping of internal graph
 * of primitives and corresponding configuration information. Value is a name of
 * output dot file without extension. Files `<dot_file_name>_init.dot` and
 * `<dot_file_name>_perf.dot` will be produced.
 */
INFERENCE_ENGINE_DEPRECATED("Use InferenceEngine::ExecutableNetwork::GetExecGraphInfo::serialize "
                            "method")
DECLARE_CONFIG_KEY(DUMP_EXEC_GRAPH_AS_DOT);

/**
 * @brief The name for setting to execute in bfloat16 precision whenever it is
 * possible
 *
 * This option let plugin know to downscale the precision where it see
 * performance benefits from bfloat16 execution Such option do not guarantee
 * accuracy of the network, the accuracy in this mode should be verified
 * separately by the user and basing on performance and accuracy results it
 * should be user's decision to use this option or not to use
 */
DECLARE_CONFIG_KEY(ENFORCE_BF16);

/**
 * @brief This key defines the directory which will be used to store any data
 * cached by plugins.
 *
 * The underlying cache structure is not defined and might differ between
 * OpenVINO releases Cached data might be platform / device specific and might
 * be invalid after OpenVINO version change If this key is not specified or
 * value is empty string, then caching is disabled. The key might enable caching
 * for the plugin using the following code:
 *
 * @code
 * ie.SetConfig({{CONFIG_KEY(CACHE_DIR), "cache/"}}, "GPU"); // enables cache
 * for GPU plugin
 * @endcode
 *
 * The following code enables caching of compiled network blobs for devices
 * where import/export is supported
 *
 * @code
 ...
 * @brief A macro which provides an AUTO-mangled name for configuration key with
 * name `name`
 */
