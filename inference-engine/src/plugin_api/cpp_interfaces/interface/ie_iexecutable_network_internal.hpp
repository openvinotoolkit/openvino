...
public:
    ...
    /**
     * @brief Gets the Executable network output Data node information. The
     * received info is stored in the given Data node.
     * @return out Reference to the ConstOutputsDataMap object
     */
    virtual ConstOutputsDataMap GetOutputsInfo() const;

    /**
     * @brief Gets the Executable network input Data node information. The
     * received info is stored in the given InputsDataMap object.
     ...
     *  Note: the returned request will have allocated input and output blobs
    ...
    /**
     * @deprecated Use IExecutableNetworkInternal::Export(std::ostream&
     * networkModel)
     * @brief Export the current created executable network so it can be used
     * later in the Import() main API
...
protected:
     ...
     * @note       The method is called by
     * IExecutableNetworkInternal::CreateInferRequest as plugin-specific
     * implementation.
