 ...
 *             Copies count bytes from src to dest. If the source and
 * destination overlap, the behavior is undefined.
 * @ingroup    ie_dev_api_memory
 *
 * @param dest A Pointer to the object to copy to
 * @param destsz A max number of bytes to modify in the destination (typically
...
ie_memcpy(void* dest, size_t destsz, void const* src, size_t count);
