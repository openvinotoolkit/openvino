...
public:
    ...
    ExecutableNetwork LoadNetworkFromFile(const std::string& modelPath, const std::map<std::string, std::string>& config) {
        PLUGIN_CALL_STATEMENT(return actual->LoadNetworkFromFile(modelPath, config));
    }
