...
InferenceEngine::Blob::Ptr CNNStatisticHelper::calculateScaleFactor(
    ...
    std::shared_ptr<Data> iScaleData = std::shared_ptr<Data>(new Data("scale", { Precision::FP32, { channels }, Layout::C }));
...
CNNLayer::Ptr CNNNetworkInt8Normalizer::createDWConvolutionForScale(const std::string &layerName, size_t channels, float *ssWValues, float *ssSValues) {
    ...
    if (pConv->_out_depth % 16 == 0) {
        ...
        std::shared_ptr<Data> wData = std::shared_ptr<Data>(new Data("weights", { Precision::FP32, { pConv->_out_depth * 16 }, Layout::C }));
        ...
        Blob::Ptr weights = nullptr;
        std::shared_ptr<Data> wData = std::shared_ptr<Data>(new Data("weights", { Precision::FP32, { pConv->_out_depth * pConv->_out_depth }, Layout::C }));
        weights = CreateBlobFromData(wData);
        weights->allocate();
        float *buffer = weights->buffer().as<float *>();
    ...
    std::shared_ptr<Data> bData = std::shared_ptr<Data>(new Data("biases", { Precision::FP32, { pConv->_out_depth }, Layout::C }));
    ...
    for (auto iter : sortedLayers) {
        if (iter->type == "Concat" && iter->precision == Precision::I8) {
            ...
            if (all_inputs_are_int8) {
                ...
                std::shared_ptr<Data> oScaleData = std::shared_ptr<Data>(new Data("o-scale", { Precision::FP32, { outputChannels }, Layout::C }));
