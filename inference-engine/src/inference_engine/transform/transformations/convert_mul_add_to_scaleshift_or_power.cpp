...
std::shared_ptr<ngraph::Node> normalize_constant(const std::shared_ptr<ngraph::op::Constant> & constant,
    ...
    auto reshape = std::make_shared<ngraph::op::Reshape>(constant->output(0), order, const_shape);
...
void ngraph::pass::ConvertMulAddToScaleShiftOrPower::convert_mul_add_to_scaleshift_or_power() {
    ...
    ngraph::graph_rewrite_callback callback = [](pattern::Matcher& m) {
        ...
        auto mul_node = std::dynamic_pointer_cast<ngraph::op::Multiply> (add_input_0);
        auto broadcast_bias_node = std::dynamic_pointer_cast<ngraph::op::DynBroadcast> (add_input_1);
        if (!mul_node) {
            mul_node = std::dynamic_pointer_cast<ngraph::op::Multiply> (add_input_1);
            broadcast_bias_node = std::dynamic_pointer_cast<ngraph::op::DynBroadcast> (add_input_0);
        }
        ...
        auto broadcast_weights_node = std::dynamic_pointer_cast<ngraph::op::DynBroadcast> (mul_input_1);
        if (!broadcast_weights_node) {
            data_node = mul_node->input(1).get_source_output();
            broadcast_weights_node = std::dynamic_pointer_cast<ngraph::op::DynBroadcast> (mul_input_0);
        }
        ...
        auto weights_node = std::dynamic_pointer_cast<ngraph::op::Constant>(broadcast_weights_node->input(0).get_source_output().get_node_shared_ptr());
        auto bias_node = std::dynamic_pointer_cast<ngraph::op::Constant> (broadcast_bias_node->input(0).get_source_output().get_node_shared_ptr());
