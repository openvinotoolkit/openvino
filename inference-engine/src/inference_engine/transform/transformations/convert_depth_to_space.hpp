...
private:
    void convert() {
        ...
        auto dts_node = std::make_shared<ngraph::op::DepthToSpace>(input0, ngraph::op::DepthToSpace::DepthToSpaceMode::DEPTH_FIRST);

        ngraph::graph_rewrite_callback callback = [](pattern::Matcher& m) {
            auto dts_node = std::dynamic_pointer_cast<ngraph::op::DepthToSpace> (m.get_match_root());
            if (!dts_node) {
                return false;
            }
