...
TensorDesc verifyNV12BlobInput(const Blob::Ptr& y, const Blob::Ptr& uv) {
    ...
    if (yMemoryBlob->element_size() != uvMemoryBlob->element_size()) {
        IE_THROW() << "Y and UV planes have different element sizes: " << yMemoryBlob->element_size() << " != " << uvMemoryBlob->element_size();
    }
    ...
    if (yDims.size() != 4 || uvDims.size() != 4) {
        IE_THROW() << "Y and UV planes dimension sizes must be 4, actual: " << yDims.size() << "(Y plane) and " << uvDims.size() << "(UV plane)";
    }
    ...
    if (yDims[2] != 2 * uvDims[2]) {
        IE_THROW() << "The height of the Y plane must be equal to (2 * the height of the UV plane), actual: " << yDims[2] << "(Y plane) and " << uvDims[2]
                   << "(UV plane)";
    }

    // check width
    if (yDims[3] != 2 * uvDims[3]) {
        IE_THROW() << "The width of the Y plane must be equal to (2 * the width of the UV plane), actual: " << yDims[3] << "(Y plane) and " << uvDims[3]
                   << "(UV plane)";
    }
...
TensorDesc verifyI420BlobInput(const Blob::Ptr& y, const Blob::Ptr& u, const Blob::Ptr& v) {
    ...
    if (yMemoryBlob->element_size() != uMemoryBlob->element_size() || yMemoryBlob->element_size() != vMemoryBlob->element_size()) {
        IE_THROW() << "Y and UV planes have different element sizes: " << yMemoryBlob->element_size() << " != " << uMemoryBlob->element_size()
    ...
    if (yDims.size() != 4 || uDims.size() != 4 || vDims.size() != 4) {
        IE_THROW() << "Y,U and V planes dimension sizes must be 4, actual: " << yDims.size() << "(Y plane) and " << uDims.size() << "(U plane) " << vDims.size()
                   << "(V plane)";
    }
    ...
    if (yDims[2] != 2 * uDims[2]) {
        IE_THROW() << "The height of the Y plane must be equal to (2 * the height of the U plane), actual: " << yDims[2] << "(Y plane) and " << uDims[2]
                   << "(U plane)";
    }

    if (yDims[2] != 2 * vDims[2]) {
        IE_THROW() << "The height of the Y plane must be equal to (2 * the height of the UV plane), actual: " << yDims[2] << "(Y plane) and " << vDims[2]
                   << "(V plane)";
    }

    // check width
    if (yDims[3] != 2 * uDims[3]) {
        IE_THROW() << "The width of the Y plane must be equal to (2 * the width of the UV plane), actual: " << yDims[3] << "(Y plane) and " << uDims[3]
                   << "(U plane)";
    }
    if (yDims[3] != 2 * vDims[3]) {
        IE_THROW() << "The width of the Y plane must be equal to (2 * the width of the UV plane), actual: " << yDims[3] << "(Y plane) and " << vDims[3]
                   << "(V plane)";
    }
...
I420Blob::I420Blob(const Blob::Ptr& y, const Blob::Ptr& u, const Blob::Ptr& v): CompoundBlob(verifyI420BlobInput(y, u, v)) {
    this->_blobs = {y, u, v};
}
