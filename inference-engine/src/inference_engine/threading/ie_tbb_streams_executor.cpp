...
struct TBBStreamsExecutor::Impl {
    ...
    struct Stream {
        ...
        explicit Stream(Impl* impl, const bool externStream = false) :
            _impl{impl} {
            {
            ...
            if (ThreadBindingType::HYBRID_AWARE == _impl->_config._threadBindingType) {
                if (Config::PreferredCoreType::ROUND_ROBIN != _impl->_config._threadPreferredCoreType) {
                   if (Config::PreferredCoreType::ANY == _impl->_config._threadPreferredCoreType) {
                    ...
                    const auto& selected_core_type = std::find_if(_impl->_totalSreamsOnCoreTypes.cbegin(), _impl->_totalSreamsOnCoreTypes.cend(),
                        [streamId_wrapped](const decltype(_impl->_totalSreamsOnCoreTypes)::value_type & p) { return p.second > streamId_wrapped; })->first;
                    _arena.initialize(custom::task_arena::constraints{}
                        .set_core_type(selected_core_type)
                        .set_max_concurrency(concurrency));
                }
            ...
            _observer.reset(new Observer{_arena, this, &(_impl->_localStream),
    ...
    explicit Impl(const Config& config) :
        ...
        if (_config._streams*_config._threadsPerStream >= static_cast<int>(std::thread::hardware_concurrency())) {
            _maxTbbThreads.reset(new tbb::global_control{tbb::global_control::max_allowed_parallelism,
...
TBBStreamsExecutor::TBBStreamsExecutor(const Config& config)
    : _impl{new TBBStreamsExecutor::Impl{config}} {
}
