...
template <typename T>
std::string
create_atribute_list(ngraph::ValueAccessor<std::vector<T>> &adapter) {
...
template <>
std::string
create_atribute_list(ngraph::ValueAccessor<std::vector<std::string>> &adapter) {
  std::string comma_list = "";
  for (auto v : adapter.get()) {
    comma_list += v;
    comma_list += ",";
...
public:
  ...
  void
  on_adapter(const std::string &name,
    ...
    if (node->get_output_size() > 0) {
      ...
      for (auto o : node->outputs()) {
        ...
        port.append_attribute("precision")
            .set_value(details::convertPrecision(o.get_element_type()).name());
