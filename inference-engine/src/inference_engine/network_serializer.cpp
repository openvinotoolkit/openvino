...
std::size_t updatePreProcInfo(const InferenceEngine::ICNNNetwork& network, pugi::xml_node &netXml, const std::size_t weightsDataOffset) {
        ...
        if (nInChannels) {
            ...
            for (size_t ch = 0; ch < nInChannels; ch++) {
                ...
                if (1.f != preProcessChannel->stdScale) {
                    channel.append_child("scale").append_attribute("value").set_value(std::to_string(preProcessChannel->stdScale).c_str());
                }
            }
        }
    }
...
std::size_t NetworkSerializer::fillXmlDoc(const InferenceEngine::ICNNNetwork&  network,
