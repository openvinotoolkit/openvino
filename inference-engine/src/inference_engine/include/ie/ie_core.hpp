...
public:
    /** @brief Constructs Inference Engine Core instance using XML configuration
     * file with plugins description.
     *
     * See RegisterPlugins for more details.
     *
     * @param xmlConfigFile A path to .xml file with plugins to load from. If XML
     * configuration file is not specified, then default Inference Engine plugins
     * are loaded from the default plugin.xml file.
     */
     ...
     * For IR format (*.bin):
     *  * if path is empty, will try to read bin file with the same name as xml
     * and
     *  * if bin file with the same name was not found, will load IR without
     * weights. For ONNX format (*.onnx or *.prototxt):
     ...
     * `InferenceEngine::Core::ReadNetwork(const std::string& model, const
     * Blob::CPtr& weights) const` function overload which takes a filesystem path
     * to the model. For ONNX case the second parameter should contain empty blob.
     * @note Created InferenceEngine::CNNNetwork object shares the weights with
     * `weights` object. So, do not create `weights` on temporary data which can
     * be later freed, since the network constant datas become to point to invalid
     * memory.
     * @return CNNNetwork
     */
     ...
     *
     * This can be more efficient than using ReadNetwork + LoadNetwork(CNNNetwork)
     * flow especially for cases when caching is enabled and cached model is
     * available
     *
     * @param modelPath path to model
     * @param deviceName Name of device to load network to
    ...
    /**
     * @brief Creates an executable network from a network object within a
     * specified remote context.
     * @param network CNNNetwork object acquired from Core::ReadNetwork
     ...
     *
     * @param extension Pointer to already loaded extension
     * @param deviceName Device name to identify plugin to add an executable
     * extension
     */
    ...
    /**
     * @brief Creates an executable network from a previously exported network
     * within a specified remote context.
     *
     * @param networkModel Network model stream
     * @param context Pointer to RemoteContext object
     * @param config Optional map of pairs: (config parameter name, config
     ...
     *
     * @param deviceName An optional name of a device. If device name is not
     * specified, the config is set for all the registered devices.
     *
     ...
     *
     * The method is targeted to extract information which can be set via
     * SetConfig method.
     *
    ...
    /**
     * @brief Register new device and plugin which implement this device inside
     * Inference Engine.
     *
     * @param pluginName A name of plugin. Depending on platform pluginName is
     * wrapped with shared library suffix and prefix to identify library full name
     *
     * @param deviceName A device name to register plugin for. If device name is
     * not specified, then it's taken from plugin itself.
     */
    void RegisterPlugin(const std::string& pluginName, const std::string& deviceName);

    /**
     * @brief Unloads previously loaded plugin with a specified name from
     * Inference Engine The method is needed to remove plugin instance and free
     * its resources. If plugin for a specified device has not been created
     * before, the method throws an exception.
     *
     * @param deviceName Device name identifying plugin to remove from Inference
     * Engine
     */
    void UnregisterPlugin(const std::string& deviceName);

    /** @brief Registers plugin to Inference Engine Core instance using XML
     * configuration file with plugins description.
     *
     ...
     * - `location` specifies absolute path to dynamic library with plugin. A path
     * can also be relative to inference engine shared library. It allows to have
     * common config for different systems with different configurations.
     ...
     * using specified plugin-specific low level device API parameters (device
     * handle, pointer, etc.)
    ...
    /**
     * @brief Get a pointer to default(plugin-supplied) shared context object for
     * specified accelerator device.
