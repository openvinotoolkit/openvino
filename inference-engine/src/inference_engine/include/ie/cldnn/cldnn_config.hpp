...
/**
 * @brief This key instructs the clDNN plugin to use the OpenCL queue priority
 * hint as defined in
...
/**
 * @brief This key instructs the clDNN plugin to use throttle hints the OpenCL
...
/**
 * @brief This key defines the directory name to which clDNN graph visualization
 * will be dumped.
 */
INFERENCE_ENGINE_DEPRECATED("The config key will be removed")
...
/**
 * @brief This key defines the directory name to which full program sources will
 * be dumped.
 */
INFERENCE_ENGINE_DEPRECATED("The config key will be removed")
 ...
 * By default the model is converted to FP32 precision before running LPT. If
 * this key is enabled (default), then non-quantized layers will be converted
 * back to FP16 after LPT, which might imrpove the performance if a model has a
 * lot of compute operations in non-quantized path. This key has no effect if
 * current device doesn't have INT8 optimization capabilities.
 */
...
/**
 * @brief This key enables dumping of the kernels used by the plugin for custom
 * layers.
 *
 * This option should be used with values: PluginConfigParams::YES or
 * PluginConfigParams::NO (default)
 */
INFERENCE_ENGINE_DEPRECATED("The config key will be removed")
 ...
 * PluginConfigParams::TUNING_CREATE - create tuning data for parameters not
 * present in tuning file PluginConfigParams::TUNING_UPDATE - perform non-tuning
 * updates like removal of invalid/deprecated data
 * PluginConfigParams::TUNING_RETUNE - create tuning data for all parameters,
 * even if already present
 *
 * For values TUNING_CREATE and TUNING_RETUNE the file will be created if it
 * does not exist.
 */
INFERENCE_ENGINE_DEPRECATED("The config key will be removed")
