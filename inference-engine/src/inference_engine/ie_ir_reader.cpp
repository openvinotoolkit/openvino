...
ICNNNetwork::Ptr InferenceEngine::convertFunctionToICNNNetwork(const std::shared_ptr<ngraph::Function>& nGraph) {
    const auto createCNNLayer = [](const std::shared_ptr<ngraph::Node>& node, const Precision& netPrc) -> CNNLayerPtr {
        static std::vector<std::shared_ptr<Builder::INodeConverter>> convertors = {
            ...
            // std::make_shared<Builder::NodeConverter<ngraph::op::DynSlice>>(), // StridedSlice, no NodeConverter
            ...
            for (const auto& input : output.get_inputs()) {
                ...
                internalLayer = internalLayer && (input->get_index() ||
                    ...
                    (std::dynamic_pointer_cast<ngraph::op::GroupConvolution>(inputLayer) != nullptr ||
                     ...
                     std::dynamic_pointer_cast<ngraph::op::NormalizeIE>(inputLayer) != nullptr ||
    ...
    for (const auto& layer : graph->get_ops()) {
        ...
        if (layer->get_placement() != ngraph::Placement::DEFAULT) {
            switch (layer->get_placement()) {
            case ngraph::Placement::CPU:
                cnnLayer->affinity = "CPU";
                break;
            case ngraph::Placement::GPU:
                cnnLayer->affinity = "GPU";
                break;
            default:
                THROW_IE_EXCEPTION << "Cannot convert nGraph operation to cnnLayer. Unsupported affinity!";
            }
        }
        size_t inputCount(0);
        ...
        for (size_t i = 0; i < layer->get_output_size(); i++) {
            ...
            if (!ptr) {
                ...
                ptr.reset(new Data(outName, { details::ngraph::convertPrecision(layer->get_output_element_type(i)),
