...
void IInferRequestInternal::SetBlob(const std::string& name, const Blob::Ptr& data) {
    OV_ITT_SCOPED_TASK(itt::domains::Plugin, "SetBlob");
    if (name.empty()) {
        IE_THROW(NotFound) << "Failed to set blob with empty name";
    }
    if (!data) IE_THROW(NotAllocated) << "Failed to set empty blob with name: \'" << name << "\'";
    ...
    if (!compoundBlobPassed && !remoteBlobPassed && data->buffer() == nullptr)
        IE_THROW() << "Input data was not allocated. Input name: \'" << name << "\'";
    if (data->size() == 0) {
...
void IInferRequestInternal::checkBlob(const Blob::Ptr& blob, const std::string& name, bool isInput, const SizeVector& refDims) const {
    ...
    if (refDims.empty()) {
        ...
        if (isInput) {
            ...
            if (foundInputPair == std::end(_networkInputs)) {
                IE_THROW(NotAllocated) << "Failed to find input with name: \'" << name << "\'";
            }
