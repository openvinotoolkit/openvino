...
const std::set<std::string> true_names{"true", "1"};
const std::set<std::string> false_names{"false", "0"};
...
public:
    ...
    void on_adapter(const std::string& name, ngraph::ValueAccessor<bool>& value) override {
        ...
        const bool is_true = true_names.find(val) != true_names.end();
        const bool is_false = false_names.find(val) != false_names.end();
...
std::shared_ptr<ngraph::Function> XmlDeserializer::parse_function(
    ...
    auto function = std::make_shared<ngraph::Function>(
        result_nodes, sink_nodes, parameter_nodes, GetStrAttr(root, "name", ""));
...
V10Parser::GenericLayerParams XmlDeserializer::parseGenericParams(const pugi::xml_node& node) {
