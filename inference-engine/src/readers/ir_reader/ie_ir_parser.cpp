...
std::shared_ptr<ICNNNetwork> V10Parser::parse(const pugi::xml_node& root, std::istream& binStream) {
    ...
    ngraph::NodeVector assign_nodes;
    ...
    function->set_leafs(assign_nodes);
    if (!result_nodes.empty()) {
        for (const auto& assign : assign_nodes) {
            assign->add_control_dependency(variable_id_to_read_value.at(std::dynamic_pointer_cast<ngraph::op::Assign>(assign)->get_variable_id()));
        }
    }
