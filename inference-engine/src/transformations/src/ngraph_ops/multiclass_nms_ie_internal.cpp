...
std::vector<int64_t> get_integers(const std::shared_ptr<HostTensor>& input,
    ...
    break;
    default:
        throw std::runtime_error("Unsupported data type in op multiclass-nms");
        break;
    }
...
InfoForNMS get_info_for_nms_eval(const op::internal::MulticlassNmsIEInternal* nms,
    ...
    result.boxes_data = prepare_boxes_data(
        inputs[boxes_port], result.boxes_shape, (ngraph::opset7::MulticlassNms::BoxEncodingType)nms->m_center_point_box);
