...
op::NonMaxSuppressionIE::NonMaxSuppressionIE(const Output<Node> &boxes,
        ...
        : Op({boxes, scores, max_output_boxes_per_class, iou_threshold, score_threshold}),
          m_center_point_box(center_point_box), m_sort_result_descending(sort_result_descending), m_converted_from_nms_version(0) {
...
void op::NonMaxSuppressionIE::validate_and_infer_types() {
    ...
    std::shared_ptr<op::Op> nms;
    /* second input is used for output calculation and only if it's Constant output shape won't be dynamic */
    if (m_converted_from_nms_version == 1) {
        nms = std::make_shared<opset1::NonMaxSuppression>(input_value(0),
                                                          input_value(1),
                                                          max_output_boxes_per_class ? opset1::Constant::create(
                                                                  element::i64, Shape{},
    ...
    if (m_converted_from_nms_version == 4) {
        nms = std::make_shared<opset4::NonMaxSuppression>(input_value(0),
                                                          input_value(1),
                                                          max_output_boxes_per_class ? opset1::Constant::create(
                                                                  element::i64, Shape{},
                                                                  max_output_boxes_per_class->cast_vector<int64_t>()) :
        ...
        throw ngraph_error("Unknown version of the NMS operation which NonMaxSuppressionIE was converted from");
    }
