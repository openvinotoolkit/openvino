...
void MaxPoolTransformation::registerMatcherIn(GraphRewrite &pass, TransformationContext &context) const {
    addPattern(
            pass,
            context,
            make_op_pattern<opset1::MaxPool>(
                    { make_op_label<op::MultiplyAdd>()}));
}

void MaxPoolTransformation::transform(TransformationContext& context, ngraph::pattern::Matcher &m) const {
    auto pooling = m.get_match_root();
    auto multiplyAdd = as_type_ptr<op::MultiplyAdd>(pooling->input_value(0).get_node_shared_ptr());
    auto convert = as_type_ptr<opset1::Convert>(multiplyAdd->input_value(0).get_node_shared_ptr());
    Output<Node> data;
    if (convert) {
        data = convert->input_value(0);
    } else {
        data = multiplyAdd->input_value(0);
    }

    auto scaleConstant = as_type_ptr<opset1::Constant>(multiplyAdd->input_value(1).get_node_shared_ptr());
    auto scales = scaleConstant->cast_vector<float>();
    if (std::all_of(scales.begin(), scales.end(), [](float value) {
                return value >= 0.0;
            })) {
        auto newPooling = pooling->copy_with_new_inputs({data});
        auto replacement = multiplyAdd->copy_with_new_inputs({
            convert ? convert->copy_with_new_inputs({newPooling}) : newPooling,
            multiplyAdd->input_value(1), multiplyAdd->input_value(2)});
