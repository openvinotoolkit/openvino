...
NGRAPH_RTTI_DEFINITION(ov::pass::LogSoftmaxDecomposition, "LogSoftmaxDecomposition", 0);
...
ov::pass::LogSoftmaxDecomposition::LogSoftmaxDecomposition() {
    ...
    auto log_softmax = ov::pattern::wrap_type<opset5::LogSoftmax>();

    ov::matcher_pass_callback callback = [=](ov::pattern::Matcher& m) {
        auto& pattern_to_output = m.get_pattern_value_map();
        auto log_softmax_node = std::dynamic_pointer_cast<ov::opset5::LogSoftmax>(pattern_to_output.at(log_softmax).get_node_shared_ptr());

        if (log_softmax_node == nullptr || transformation_callback(log_softmax_node)) {
            return false;
        }

        auto axis1 = ov::opset5::Constant::create(element::Type_t::i64, ov::Shape{1}, { log_softmax_node->get_axis() });
        auto axis2 = ov::opset5::Constant::create(element::Type_t::i64, ov::Shape{1}, { log_softmax_node->get_axis() });
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(log_softmax, matcher_name);
