...
namespace {
    ...
    /// \brief      Check if the dimension with a given label is reduced. The dimension is reduced iff. the corresponding label
    /// is met in neither the output subscript nor the input subscripts excluding ones specified by a vector excluded_indices
    ///
    ...
    bool is_dimension_reduced(const std::vector<std::string>& input_subscripts, const std::string& output_subscript,
        ...
        if (output_subscript.find(label_to_check) != std::string::npos) {
            return false;
        }
        return true;
    }

    /// \brief      Generate an input subscript that provides to group dimensions into the common, separate
    /// and reduced dimensions after transpose
    ///
    ...
    /// \param      is_separate_first        A boolean flag. It is true if the separate dimensions goes before the reduced dimensions
    ///
    ...
    std::string generate_grouping_subscript(const std::string& input_subscript, const std::vector<int64_t>& common_labels_inds,
        const std::vector<int64_t>& separate_labels_inds, const std::vector<int64_t>& reduced_labels_inds, bool& is_separate_first) {
        // transpose is not needed if common labels, reduced labels and separate labels indices go concurrently
        ...
        if (is_transpose_needed == false) {
            is_separate_first = false;
            return input_subscript;
        }

        // transpose is not needed if common labels, separate labels and reduced labels indices go concurrently
    ...
    /// \brief      Update a vector of input nodes and subscripts by removing items for operands with indices input_ind1 and input_ind2
    /// and inserted new input node and the corresponsing subscript in the tail
    ///
    ...
    ///
    void update_operands(ngraph::OutputVector& input_nodes, std::vector<std::string>& input_subscripts,
        size_t input_ind1, size_t input_ind2, const ngraph::Output<ngraph::Node>& new_node, const std::string& new_subscript) {
        NGRAPH_CHECK(input_ind2 < input_nodes.size());
        input_nodes.erase(input_nodes.begin() + input_ind2);
        NGRAPH_CHECK(input_ind1 < input_nodes.size());
        ...
        NGRAPH_CHECK(input_ind1 < input_subscripts.size());
    ...
    /// \param      is_product          A boolean flag that indicates if to compute a product of dimension sizes in the computed sub-shape
    ///
    ...
    ///
    ngraph::OutputVector compute_sub_shape(const ngraph::Output<ngraph::Node>& data_shape, size_t s_begin, size_t s_end,
        ngraph::NodeVector& subgraph_nodes, bool is_product = false) {
        ...
        auto sub_shape = std::make_shared<ngraph::opset7::StridedSlice>(data_shape,
            begin_const, end_const, begin_mask, end_mask);
    ...
    /// \brief      Unsqueeze input node by given dimensions if a vector of unsqueezing dimensions is not empty
    ///
    ...
    /// 
    /// \return     Unsqueezed input node if a vector of unsqueezing dimensions is not empty, otherwise, the original input node
    ///
    ngraph::Output<ngraph::Node> unsqueeze_input(const ngraph::Output<ngraph::Node>& input_node, const std::vector<int64_t>& unsqueeze_axes,
        ...
        auto unsqueeze_axes_const = ngraph::opset7::Constant::create(ngraph::element::Type_t::i64,
            ngraph::Shape{ unsqueeze_axes.size() }, unsqueeze_axes);
    ...
    /// \brief      Reshape input node to the new shape specified by sub-shapes of the common, separate and reduced dimensions
    /// so that the reshaped input has a format acceptable by MatMul
    ///
    ...
    /// \param      is_separate_first       true - the separate dimensions placed before reduced dimensions, otherwise, it is after them
    /// \param      subgraph_nodes          A vector of operation nodes that is included into a sub-graph decomposing Einsum
    /// that is needed for copy_runtime_info
    /// 
    ...
    ngraph::Output<ngraph::Node> reshape_input_for_matmul(const ngraph::Output<ngraph::Node>& input_node,
        const ngraph::OutputVector& common_sub_shape, const ngraph::OutputVector& separate_sub_shape,
        const ngraph::OutputVector& reduced_sub_shape_prod, bool is_separate_first, ngraph::NodeVector& subgraph_nodes) {
        ...
        if (common_sub_shape.size() > 0 && separate_sub_shape.size() == 0) {
            ...
            // since MatMul operation is not possible to do without separate dimensions if the common dimension presents
            ...
            // in this case compute a product of separate dimension sizes since they must be presented with just one dimension for MatMul
            auto reduce_axis_const = ngraph::opset7::Constant::create(ngraph::element::Type_t::i64, ngraph::Shape{ 1 }, { 0 });
        ...
        // concatenate parts of new shape
        auto new_shape_op = std::make_shared<ngraph::opset7::Concat>(new_shape_parts, 0);

        auto reshaped_input_op = std::make_shared<ngraph::opset7::Reshape>(input_node, new_shape_op->output(0), false);

        subgraph_nodes.insert(subgraph_nodes.end(), { new_shape_op, reshaped_input_op });
        return reshaped_input_op->output(0);
    }

    /// \brief      Transpose one of the Einsum inputs to layout specified through the required subscript
    ///
    ...
    void transpose_input(ngraph::OutputVector& input_nodes, std::vector<std::string>& input_subscripts,
        ...
        // find permutation that establishes bijection between the input subscript and the required one
    ...
    /// \param      output_subscript    The output subscript
    /// \param      input_ind           An index of the input node for which it will check dimensions to be reduced
    /// \param      subgraph_nodes      A vector of operation nodes that is included into a sub-graph decomposing Einsum
    /// that is needed for copy_runtime_info
    ///
    void reduce_input(ngraph::OutputVector& input_nodes, std::vector<std::string>& input_subscripts,
    ...
    /// The input nodes for these two operands are removed from input_nodes along with their input subscripts
    ///
    /// \param      input_nodes         A vector of input nodes to Einsum operation
    /// \param      input_subscripts    A vector of corresponding subscripts for the input nodes
    /// \param      output_subscript    The output subscript
    ...
    /// that is needed for copy_runtime_info
    ///
    void contract_two_inputs(ngraph::OutputVector& input_nodes, std::vector<std::string>& input_subscripts,
        ...
        // 1. dimension indices with the same labels (in both subscripts) that are NOT reduced - common labels (dimensions)
        // 2. dimension indices with labels that are met only in one of two subscripts - separate labels (dimensions)
        // 3. dimension indices with the same labels (in both subscripts) that are reduced - reduced labels (dimensions)
        // NOTE: dimension is reduced iff. the corresponding label are met in neither the output subscript
        // nor the input subscripts for other Einsum inputs excluding two given inputs
        ...
        if (reduced_labels_inds1.empty()) {
            ...
            // transpose the second operand in order to get the convenient layout for further unsqueezing
            ...
            // and the number of them is equal to the number of separate labels in the second subscript
        ...
        auto int_subscript1 = generate_grouping_subscript(input_subscript1, common_labels_inds1,
            separate_labels_inds1, reduced_labels_inds1, is_separate_first1);
        transpose_input(input_nodes, input_subscripts, int_subscript1, input_ind1, subgraph_nodes);
        bool is_separate_first2 = false;
        auto int_subscript2 = generate_grouping_subscript(input_subscript2, common_labels_inds2,
            separate_labels_inds2, reduced_labels_inds2, is_separate_first2);
        transpose_input(input_nodes, input_subscripts, int_subscript2, input_ind2, subgraph_nodes);

        // step 2. reshape both operands so that separate labels and reduced labels are represented with just one dimension
        // this is needed by MatMul operation requirement to operands format.
        // For example, the shape must be in a format [B1, ..., Bm, X, Y] or [B1, ..., Bm, Y, X],
        // where B1, ..., Bm are common dimensions, X is collapsed dimension for separate labels
        // and Y is collapsed dimension for reduced labels
        ...
        auto common_sub_shape = compute_sub_shape(data_shape1, common_dims_begin, common_dims_end, subgraph_nodes);
        ...
        int64_t separate1_dims_begin = (is_separate_first1 ? common_labels_inds1.size() :
        ...
        auto separate1_sub_shape = compute_sub_shape(data_shape1, separate1_dims_begin, separate1_dims_end, subgraph_nodes);

        int64_t separate2_dims_begin = (is_separate_first2 ? common_labels_inds2.size() :
        ...
        auto separate2_sub_shape = compute_sub_shape(data_shape2, separate2_dims_begin, separate2_dims_end, subgraph_nodes);

        auto reshaped_input1 = reshape_input_for_matmul(input_node1, common_sub_shape, separate1_sub_shape,
            reduced_sub_shape_prod, is_separate_first1, subgraph_nodes);
        auto reshaped_input2 = reshape_input_for_matmul(input_node2, common_sub_shape, separate2_sub_shape,
        ...
        auto matmul = std::make_shared<ngraph::opset7::MatMul>(reshaped_input1, reshaped_input2, transpose_a, transpose_b);
        ...
        std::string resultant_subscript = input_subscript1.substr(common_dims_begin, common_dims_end)
            + separate_part1 + separate_part2;

        if (new_shape.size() > 0) {
            auto result_shape_op = std::make_shared<ngraph::opset7::Concat>(new_shape, 0);
            auto result_op = std::make_shared<ngraph::opset7::Reshape>(matmul->output(0), result_shape_op->output(0), false);

            // update input operand and input subscript for Einsum operation
            update_operands(input_nodes, input_subscripts, input_ind1, input_ind2, result_op->output(0), resultant_subscript);

            // update a vector of nodes for copy_runtime_info
            subgraph_nodes.insert(subgraph_nodes.end(), { result_shape_op, result_op });
        }
        else {
            // this is a case when all dimensions are reduced and the result is scalar
            // update input operand and input subscript for Einsum operation
            update_operands(input_nodes, input_subscripts, input_ind1, input_ind2, matmul->output(0), resultant_subscript);
        }

        // update a vector of nodes for copy_runtime_info
        subgraph_nodes.insert(subgraph_nodes.end(), { data_shape1, data_shape2, matmul });
    }
}
...
ngraph::pass::EinsumDecomposition::EinsumDecomposition() {
    ...
    // but not applicable to a case with equation="aabc,bd->ad" due to repeated labels in one subscript.
    ...
    ngraph::matcher_pass_callback callback = [this](ngraph::pattern::Matcher& m) {
        ...
        for (auto const& inds_pair : einsum_path) {
            contract_two_inputs(input_nodes, input_subscripts, output_subscript,
