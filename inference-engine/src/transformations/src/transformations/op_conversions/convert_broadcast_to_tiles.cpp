...
NGRAPH_RTTI_DEFINITION(ov::pass::ConvertBroadcastToTiles, "ConvertBroadcastToTiles", 0);

ov::pass::ConvertBroadcastToTiles::ConvertBroadcastToTiles() {
    MATCHER_SCOPE(ConvertBroadcastToTiles);
    auto broadcast = ov::pattern::wrap_type<ov::opset1::Broadcast>();
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(broadcast, matcher_name);
