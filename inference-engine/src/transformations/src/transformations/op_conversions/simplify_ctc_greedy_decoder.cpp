...
ngraph::pass::SimplifyCTCGreedyDecoder::SimplifyCTCGreedyDecoder() {
    ...
    ngraph::matcher_pass_callback callback = [=](ngraph::pattern::Matcher& m) {
        ...
        decoder_v6->output(0).replace(output_i->output(0));
        decoder_v6->output(1).replace(output_seq_len->output(0));
        ...
        auto simplified_decoder = std::make_shared<ngraph::opset6::CTCGreedyDecoder>(transpose,
                                                                             ...
                                                                             decoder_v6->get_merge_repeated());
        simplified_decoder->set_friendly_name(decoder_v6->get_friendly_name());
        ngraph::copy_runtime_info(decoder_v6, {transpose, simplified_decoder, data_shape, T, N, Tplus, range1T, tT, tN, mask_shape,
                                               transpose_seq_mask, transpose_upper_bounds, upper_bounds, bool_seq_mask, seq_mask});
