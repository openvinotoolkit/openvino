...
ngraph::pass::SimplifyCTCGreedyDecoder::SimplifyCTCGreedyDecoder() {
    ...
    ngraph::matcher_pass_callback callback = [=](ngraph::pattern::Matcher& m) {
        ...
        auto axisT = ngraph::opset6::Constant::create(element::i32, Shape{}, {0});
        auto indexT = ngraph::opset6::Constant::create(element::i32, Shape{1}, {1});
        auto T = std::make_shared<ngraph::opset6::Gather>(data_shape, indexT, axisT);

        auto axisN = ngraph::opset6::Constant::create(element::i32, Shape{}, {0});
        auto indexN = ngraph::opset6::Constant::create(element::i32, Shape{1}, {0});
        ...
        auto const_plusT = ngraph::opset6::Constant::create(element::i32, Shape{1}, {0});
        ...
        auto mask_val_true = ngraph::opset6::Constant::create(element::i32, Shape{}, {1});
        auto mask_val_false = ngraph::opset6::Constant::create(element::i32, Shape{}, {0});
        ...
        ngraph::copy_runtime_info(decoder_v6, {simplified_decoder, data_shape, T, N, range1T, plusT, plusT_scalar, mask_shape, upper_bounds, bool_seq_mask, seq_mask});
        decoder_v6->output(0).replace(simplified_decoder->output(0));
        /*auto data_shape = std::make_shared<ngraph::opset6::ShapeOf>(decoder_v6->input_value(0));
        auto constT_0 = ngraph::opset6::Constant::create(element::i32, Shape{}, {-1});//TODO set axis 0
        auto constT_1 = ngraph::opset6::Constant::create(element::i32, Shape{}, {1});
        auto T = std::make_shared<ngraph::opset6::Gather>(data_shape, constT_1, constT_0);

        auto constN_0 = ngraph::opset6::Constant::create(element::i32, Shape{}, {0});
        auto constN_1 = ngraph::opset6::Constant::create(element::i32, Shape{}, {-1});
        auto N = std::make_shared<ngraph::opset6::Gather>(data_shape, constN_0, constN_0);

        auto start = opset6::Constant::create(seq_len_type, Shape{}, {1});
        auto step = opset6::Constant::create(seq_len_type, Shape{}, {1});
        auto plus1 = opset6::Constant::create(element::i64, Shape{}, {1});
        auto Tplus = std::make_shared<ngraph::opset6::Add>(T, plus1);
        auto range1T = std::make_shared<ngraph::opset6::Range>(start, Tplus, step, seq_len_type);

        auto constUnsqueeze1 = ngraph::opset6::Constant::create(element::i32, Shape{1}, {0});
        auto tT = std::make_shared<ngraph::opset6::Unsqueeze>(T, constUnsqueeze1);
        auto constUnsqueeze2 = ngraph::opset6::Constant::create(element::i32, Shape{1}, {0});
        ...
        auto bool_seq_mask = std::make_shared<ngraph::opset6::GreaterEqual>(transpose_upper_bounds->output(0),
                                                                            range1T->output(0));
        std::cout << upper_bounds->output(0).get_partial_shape() <<std::endl;
        std::cout << range1T->output(0).get_partial_shape() << std::endl;
        std::cout << bool_seq_mask->output(0).get_partial_shape() << std::endl;
        auto const_0f = ngraph::opset6::Constant::create(element::i32, Shape{}, {0});
        auto const_1f = ngraph::opset6::Constant::create(element::i32, Shape{}, {1});
        auto seq_mask = std::make_shared<ngraph::opset6::Select>(bool_seq_mask, const_1f, const_0f);
        std::cout << seq_mask->output(0).get_partial_shape() << std::endl;
