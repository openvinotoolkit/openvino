...
NGRAPH_RTTI_DEFINITION(ov::pass::MVN6Decomposition, "MVN6Decomposition", 0);
...
ov::pass::MVN6Decomposition::MVN6Decomposition() {
    ...
    auto mvn = ov::pattern::wrap_type<opset6::MVN>();
    ...
    ov::matcher_pass_callback callback = [=](ov::pattern::Matcher& m) {
        ...
        if (!mvn_node->get_normalize_variance()) {
            ...
            auto sqr_const = ov::opset6::Constant::create(data.get_element_type(), ov::Shape{ 1 }, { 2 });
            ...
            auto eps_node = ov::opset6::Constant::create(data.get_element_type(), ov::Shape{ 1 }, { eps });
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(mvn, matcher_name);
