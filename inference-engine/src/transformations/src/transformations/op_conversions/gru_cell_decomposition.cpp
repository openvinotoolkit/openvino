...
NGRAPH_RTTI_DEFINITION(ov::pass::GRUCellDecomposition, "GRUCellDecomposition", 0);

ov::pass::GRUCellDecomposition::GRUCellDecomposition() {
    MATCHER_SCOPE(GRUCellDecomposition);
    auto gru_cell = ov::pattern::wrap_type<opset4::GRUCell>();
    ov::matcher_pass_callback callback = [this](ov::pattern::Matcher& m) {
        ...
        ov::copy_runtime_info(gru_cell, {Xt_W, Ht_R, axis_0, Xt_W_zrh, R_zrh, Ht_R_zrh, biases_zrh,
                                             add_z_1, add_z_2, add_r_1, add_r_2, h_t, one, sub, mul_1, mul_2, out_H});
        ov::replace_node(gru_cell, out_H);
        return true;
    };

    auto m = std::make_shared<ov::pattern::Matcher>(gru_cell, matcher_name);
