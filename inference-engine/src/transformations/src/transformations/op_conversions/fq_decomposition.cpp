...
NGRAPH_RTTI_DEFINITION(ov::pass::FakeQuantizeDecomposition, "FakeQuantizeDecomposition", 0);
...
ov::pass::FakeQuantizeDecomposition::FakeQuantizeDecomposition() {
    ...
    auto data = ov::pattern::any_input();
    auto il = ov::pattern::wrap_type<opset1::Constant>();
    auto ih = ov::pattern::wrap_type<opset1::Constant>();
    auto ol = ov::pattern::wrap_type<opset1::Constant>();
    auto oh = ov::pattern::wrap_type<opset1::Constant>();
    auto fake_quantize = ov::pattern::wrap_type<ov::opset1::FakeQuantize>({data, il, ih, ol, oh});

    ov::matcher_pass_callback callback = [=](ov::pattern::Matcher &m) {
        auto &pattern_to_output = m.get_pattern_value_map();
        const auto fake_quantize_node = std::dynamic_pointer_cast<ov::opset1::FakeQuantize>(pattern_to_output.at(fake_quantize).get_node_shared_ptr());
        ...
        // (levels-1)
        const auto levels_minus_one = std::make_shared<ov::opset1::Constant>(input_type, Shape{}, fake_quantize_node->get_levels() - 1);
        ...
        const auto round = std::make_shared<ov::opset5::Round>(after_ish_apply, ov::opset5::Round::RoundMode::HALF_TO_EVEN);
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(fake_quantize, matcher_name);
