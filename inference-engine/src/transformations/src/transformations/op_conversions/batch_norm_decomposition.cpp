...
template <class T>
ngraph::graph_rewrite_callback get_callback(ngraph::pass::MatcherPass* matcher_pass, std::shared_ptr<T>& bn) {
    Shape shape{2, 2, 1, 1};
    ...
    bn = make_shared<T>(input, gamma, beta, mean, var, 0.001);

    ngraph::graph_rewrite_callback callback = [matcher_pass, input, gamma, beta, mean, var](ngraph::pattern::Matcher &m) {
        ...
        auto m_bn = dynamic_pointer_cast<T>(m.get_match_root());
        if (!m_bn) {
            return false;
        }
        ...
        auto input_sub_mean = matcher_pass->register_new_node<opset5::Subtract>(m_input, mean_aligned);
...
class BatchNormV0Decomposition: public ngraph::pass::MatcherPass {
public:
    NGRAPH_RTTI_DECLARATION;
    BatchNormV0Decomposition() {
        std::shared_ptr<opset1::BatchNormInference> bn;
        auto callback = get_callback<opset1::BatchNormInference>(this, bn);
        auto m = std::make_shared<ngraph::pattern::Matcher>(bn, "BatchNormDecomposition");
        this->register_matcher(m, callback);
    }
};
NGRAPH_RTTI_DEFINITION(BatchNormV0Decomposition, "BatchNormDecomposition", 0);

class BatchNormV5Decomposition: public ngraph::pass::MatcherPass {
public:
    NGRAPH_RTTI_DECLARATION;
    BatchNormV5Decomposition() {
        std::shared_ptr<opset5::BatchNormInference> bn;
        auto callback = get_callback<opset5::BatchNormInference>(this, bn);
        auto m = std::make_shared<ngraph::pattern::Matcher>(bn, "BatchNormDecomposition");
        this->register_matcher(m, callback);
    }
};
NGRAPH_RTTI_DEFINITION(BatchNormV5Decomposition, "BatchNormDecomposition", 5);

}  // namespace


ngraph::pass::BatchNormDecomposition::BatchNormDecomposition() {
    add_matcher<BatchNormV0Decomposition>();
    add_matcher<BatchNormV5Decomposition>();
}
