...
void ngraph::pass::ConvertTensorIteratorToSequence::convert_ti_to_sequence() {
    ...
    auto Xi     = std::make_shared<opset4::Parameter>(element::f32, Shape{32, 2, 10});
    auto Yi     = std::make_shared<opset4::Parameter>(element::f32, Shape{32, 2, 10});
    auto M_body = std::make_shared<opset4::Parameter>(element::f32, Shape{32, 2, 10});
    ...
    ngraph::graph_rewrite_callback callback = [this](pattern::Matcher &m) {
        ...
        auto squeeze = std::dynamic_pointer_cast<opset4::Squeeze>(ord_ops[0]);
        if (!squeeze)
            return false;
        ...
        auto unsqueeze = std::dynamic_pointer_cast<opset4::Unsqueeze>(ord_ops[2]);
        if (!unsqueeze)
            return false;

        if (std::dynamic_pointer_cast<ngraph::opset4::RNNCell>(cell)) {
            // convert to RNN Seq
        } else if (std::dynamic_pointer_cast<ngraph::opset4::LSTMCell>(cell)) {
            // convert to LSTM Seq
        } else if ((std::dynamic_pointer_cast<ngraph::opset4::GRUCell>(cell))) {
            // convert to GRU Seq
        } else {
            return false;
        }
