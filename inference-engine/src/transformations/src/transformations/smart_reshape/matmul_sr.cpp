...
bool relax_hc_reshape_followed_by_matmul(const ov::pattern::PatternValueMap & pattern_to_output,
    ...
    const auto & matmul = std::dynamic_pointer_cast<ov::opset4::MatMul>(pattern_to_output.at(matmul_label).get_node_shared_ptr());
    ...
    if (ov::is_type<ov::opset4::Transpose>(shape_source.get_node_shared_ptr()) ||
            ov::is_type<ov::opset4::Reshape>(shape_source.get_node_shared_ptr()))
    ...
    const auto & C = ov::op::util::node_to_get_shape_value_of_indices_from_shape_source(shape_source, idx);
    const auto & N = ov::opset4::Constant::create(ov::element::i64, {1}, {-1});
...
NGRAPH_RTTI_DEFINITION(ov::pass::ReshapeAMatMul, "ReshapeAMatMul", 0);
...
ov::pass::ReshapeAMatMul::ReshapeAMatMul() {
    ...
    auto reshape_label = ov::pattern::wrap_type<opset4::Reshape>({reshape_input_label, reshape_pattern_label});
    auto matmul_label = ov::pattern::wrap_type<opset4::MatMul>({reshape_label, other_input_label});
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(matmul_label, matcher_name);
    register_matcher(m, callback);
}

NGRAPH_RTTI_DEFINITION(ov::pass::ReshapeBMatMul, "ReshapeBMatMul", 0);
...
ov::pass::ReshapeBMatMul::ReshapeBMatMul() {
    ...
    auto reshape_pattern_label = pattern::any_input();
    auto reshape_label = ov::pattern::wrap_type<opset4::Reshape>({reshape_input_label, reshape_pattern_label});
    auto matmul_label = ov::pattern::wrap_type<opset4::MatMul>({other_input_label, reshape_label});
...
NGRAPH_RTTI_DEFINITION(ov::pass::TransposeMatMul, "TransposeMatMul", 0);

ov::pass::TransposeMatMul::TransposeMatMul() {
    MATCHER_SCOPE(TransposeMatMul);
    auto matmul_label = ov::pattern::wrap_type<opset4::MatMul>();

    matcher_pass_callback callback = [=](pattern::Matcher &m) -> bool {
        const auto & pattern_to_output = m.get_pattern_value_map();
        auto matmul = std::dynamic_pointer_cast<ov::opset4::MatMul>(pattern_to_output.at(matmul_label).get_node_shared_ptr());
        if (!matmul)
            return false;
