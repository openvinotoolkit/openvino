...
NGRAPH_RTTI_DEFINITION(ov::pass::StridedSliceOptimization, "StridedSliceOptimization", 0);

NGRAPH_RTTI_DEFINITION(ov::pass::UselessStridedSliceEraser, "UselessStridedSliceEraser", 0);

bool ov::pass::UselessStridedSliceEraser::run_on_function(std::shared_ptr<ov::Function> f) {
    ...
    for (auto & node : f->get_ordered_ops()) {
        ...
        auto ss = std::dynamic_pointer_cast<ov::opset1::StridedSlice>(node);
        ...
        auto stridesNode = std::dynamic_pointer_cast<ov::opset3::Constant>(ss->input_value(3).get_node_shared_ptr());
        if (stridesNode) {
...
ov::SlicePlan get_slice_plan(std::shared_ptr<ov::opset1::StridedSlice> slice) {
    ...
    auto begin = std::dynamic_pointer_cast<ov::opset1::Constant>(slice->input_value(1).get_node_shared_ptr());
    auto end = std::dynamic_pointer_cast<ov::opset1::Constant>(slice->input_value(2).get_node_shared_ptr());
    auto strides = std::dynamic_pointer_cast<ov::opset1::Constant>(slice->input_value(3).get_node_shared_ptr());
    ...
    ov::SlicePlan plan = ov::make_slice_plan(slice->input(0).get_shape(),
                                                     begin_vec,
                                                     end_vec,
                                                     strides_vec,
                                                     begin_mask,
...
bool strided_slices_perform_the_same(std::shared_ptr<ov::opset1::StridedSlice> lhs,
                                     std::shared_ptr<ov::opset1::StridedSlice> rhs) {
    auto lhs_plan = get_slice_plan(lhs);
    auto rhs_plan = get_slice_plan(rhs);

    auto empty_plan = ov::SlicePlan();
    if (lhs_plan == empty_plan || rhs_plan == empty_plan)
        return false;
    return lhs_plan == rhs_plan;
}

NGRAPH_RTTI_DEFINITION(ov::pass::SharedStridedSliceEraser, "SharedStridedSliceEraser", 0);

bool ov::pass::SharedStridedSliceEraser::run_on_function(std::shared_ptr<ov::Function> f) {
    RUN_ON_FUNCTION_SCOPE(SharedStridedSliceEraser);
    bool graph_rewritten = false;

    std::map<ov::Output<Node>, std::vector<std::shared_ptr<ov::opset1::StridedSlice>>> source_to_ss;
    for (const auto & node : f->get_ordered_ops()) {
        ...
        if (auto ss = std::dynamic_pointer_cast<ov::opset1::StridedSlice>(node)) {
...
NGRAPH_RTTI_DEFINITION(ov::pass::GroupedStridedSliceOptimizer, "GroupedStridedSliceOptimizer", 0);

bool ov::pass::GroupedStridedSliceOptimizer::run_on_function(std::shared_ptr<ov::Function> f) {
    RUN_ON_FUNCTION_SCOPE(GroupedStridedSliceOptimizer);
    bool graph_rewritten = false;
    using planned_slice = std::pair<std::shared_ptr<ov::opset1::StridedSlice>, ov::SlicePlan>;
    ...
    for (auto& pair : source_to_ss_with_plan) {
        ...
        auto axis_const = ov::opset1::Constant::create(ov::element::i64, ov::Shape{}, {axis});

        std::vector<int64_t> size_splits;
        for (const auto & item : output_to_size)
            size_splits.push_back(item.second);
        auto size_splits_const = ov::opset1::Constant::create(ov::element::i64, ov::Shape{size_splits.size()}, size_splits);
        auto variadic_split = std::make_shared<ov::opset1::VariadicSplit>(pair.first, axis_const, size_splits_const);
...
bool ov::pass::StridedSliceOptimization::run_on_function(std::shared_ptr<ov::Function> f) {
