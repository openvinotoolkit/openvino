...
NGRAPH_RTTI_DEFINITION(ov::pass::PullTransposeThroughFQUp, "PullTransposeThroughFQUp", 0);
...
ov::pass::PullTransposeThroughFQUp::PullTransposeThroughFQUp() {
    ...
    ov::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        for (size_t i = 0; i < fq->inputs().size(); ++i) {
            ...
            if (!unsqueeze_axes.empty()) {
                fq_input = std::make_shared<ov::opset1::Unsqueeze>(fq_input,
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(m_transpose, matcher_name);
