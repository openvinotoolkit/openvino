...
bool check_shapes(const ov::Shape& shape_input, const ov::Shape& shape_reshape_before,
                  const ov::AxisVector& transpose_constant_values, const ov::Shape& shape_reshape_after) {
    // x: [N, C, H, W]
    ...
    if (is_reshape_before_valid) {
        ...
        ov::Shape expected_reshape_before = { shape_input[0], group, shape_input[1] / group };
...
NGRAPH_RTTI_DEFINITION(ov::pass::ShuffleChannelsFusion, "ShuffleChannelsFusion", 0);
...
ov::pass::ShuffleChannelsFusion::ShuffleChannelsFusion(const bool reshape_constants_check) {
    ...
    auto input = ov::pattern::any_input(has_static_4d_shape);
    auto reshape_before_const_pattern = ov::pattern::wrap_type<ov::opset6::Constant>();
    auto transpose_const_pattern = ov::pattern::wrap_type<ov::opset6::Constant>();
    auto reshape_after_const_pattern = ov::pattern::wrap_type<ov::opset6::Constant>();
    ...
    auto reshape_before_pattern = ov::pattern::wrap_type<ov::opset6::Reshape>({input, reshape_before_const_pattern},
                                                                                      has_static_shape_and_single_consumer);
    auto transpose_pattern = ov::pattern::wrap_type<ov::opset6::Transpose>({reshape_before_pattern, transpose_const_pattern},
                                                                                   has_static_shape_and_single_consumer);
    auto reshape_after_pattern = ov::pattern::wrap_type<ov::opset6::Reshape>({transpose_pattern, reshape_after_const_pattern},
    ...
    ov::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        auto reshape_before = std::dynamic_pointer_cast<ov::opset6::Reshape>(pattern_map.at(reshape_before_pattern).get_node_shared_ptr());
        auto transpose = std::dynamic_pointer_cast<ov::opset6::Transpose>(pattern_map.at(transpose_pattern).get_node_shared_ptr());
        auto reshape_after = std::dynamic_pointer_cast<ov::opset6::Reshape>(pattern_map.at(reshape_after_pattern).get_node_shared_ptr());
        if (!reshape_after || !transpose || !reshape_after)
            return false;

        if (reshape_constants_check) {
            auto reshape_before_constant = std::dynamic_pointer_cast<ov::opset6::Constant>(
                pattern_map.at(reshape_before_const_pattern).get_node_shared_ptr());
            auto reshape_after_constant = std::dynamic_pointer_cast<ov::opset6::Constant>(
        ...
        auto transpose_constant = std::dynamic_pointer_cast<ov::opset6::Constant>(pattern_map.at(transpose_const_pattern).get_node_shared_ptr());
        ...
        auto shuffle_shannels = std::make_shared<ov::opset6::ShuffleChannels>(data, axis, group);
        shuffle_shannels->set_friendly_name(reshape_after->get_friendly_name());
        ov::copy_runtime_info({ reshape_before, transpose, reshape_after }, shuffle_shannels);
        ov::replace_node(reshape_after, shuffle_shannels);
        return true;
    };

    auto m = std::make_shared<ov::pattern::Matcher>(reshape_after_pattern, matcher_name);
