...
NGRAPH_RTTI_DEFINITION(ov::pass::SplitSqueezeConcatFusion, "SplitSqueezeConcatFusion", 0);
...
ov::pass::SplitSqueezeConcatFusion::SplitSqueezeConcatFusion() {
    ...
    auto concat_pattern = ov::pattern::wrap_type<ov::opset7::Concat>();

    ov::matcher_pass_callback callback = [=](ov::pattern::Matcher& m) {
        const auto& pattern_to_output = m.get_pattern_value_map();
        auto concat = std::dynamic_pointer_cast<ov::opset7::Concat>(pattern_to_output.at(concat_pattern).get_node_shared_ptr());
        if (!concat) return false;

        NodeVector nodes_to_delete{ concat };

        int64_t axis_value = 0;
        std::shared_ptr<ov::opset7::Split> split;

        const auto& concat_inputs = concat->input_values();
        if (concat_inputs.empty()) return false;
        for (size_t i = 0; i < concat_inputs.size(); i++) {
            auto squeeze = std::dynamic_pointer_cast<ov::opset7::Squeeze>(concat_inputs[i].get_node_shared_ptr());
            if (!squeeze) return false;

            nodes_to_delete.push_back(squeeze);

            auto split_to_check = std::dynamic_pointer_cast<ov::opset7::Split>(squeeze->input_value(0).get_node_shared_ptr());
            auto squeeze_axes = std::dynamic_pointer_cast<ov::opset7::Constant>(squeeze->input_value(1).get_node_shared_ptr());
        ...
        auto split_axis = std::dynamic_pointer_cast<ov::opset7::Constant>(split->input_value(1).get_node_shared_ptr());
        ...
        auto transpose_order = ov::opset7::Constant::create(element::i64, { (size_t)rank.get_length() }, order);
        auto transpose = register_new_node<ov::opset7::Transpose>(input, transpose_order);
        auto shape_after = ov::opset7::Constant::create(element::i64, { (size_t)rank.get_length() - 1 }, concat->get_output_shape(0));
        auto reshape = std::make_shared<ov::opset7::Reshape>(transpose, shape_after, false);
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(concat_pattern, matcher_name);
