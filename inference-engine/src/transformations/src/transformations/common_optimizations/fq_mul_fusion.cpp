...
namespace {
  ...
  get_adjusted_output_range(ngraph::Output<ngraph::Node> out_low,
                            ...
                            ngraph::Output<ngraph::Node> mul_constant) {
    const auto mul_out_low = std::make_shared<ngraph::opset4::Multiply>(out_low, mul_constant);
    const auto mul_out_high = std::make_shared<ngraph::opset4::Multiply>(out_high, mul_constant);

    ngraph::OutputVector folded_out_low(1), folded_out_high(1);

    if (!mul_out_low->constant_fold(folded_out_low, {out_low, mul_constant})) {
      throw ngraph::ngraph_error("Could not constant fold the output_low Multiply operation");
    }

    if (!mul_out_high->constant_fold(folded_out_high, {out_high, mul_constant})) {
