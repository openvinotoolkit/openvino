...
NGRAPH_RTTI_DEFINITION(ov::pass::FakeQuantizeMulFusion, "FakeQuantizeMulFusion", 0);

namespace {
std::pair<ov::Output<ov::Node>, ov::Output<ov::Node>>
    get_adjusted_output_range(ov::Output<ov::Node> out_low,
                              ov::Output<ov::Node> out_high,
                              ov::Output<ov::Node> multiplier) {
        const auto mul_out_low = std::make_shared<ov::opset4::Multiply>(out_low, multiplier);
        const auto mul_out_high = std::make_shared<ov::opset4::Multiply>(out_high, multiplier);
        ...
        ov::OutputVector new_out_low(1), new_out_high(1);
...
ov::pass::FakeQuantizeMulFusion::FakeQuantizeMulFusion() {
    ...
    const auto fq_output_low_p = ov::pattern::any_input();
    const auto fq_output_high_p = ov::pattern::any_input();

    const auto fq_node_p = ov::pattern::wrap_type<opset4::FakeQuantize>({ov::pattern::any_input(),
    ...
    const auto mul_constant_p = ov::pattern::wrap_type<opset4::Constant>();
    const auto mul_node_p = ov::pattern::wrap_type<opset4::Multiply>(
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(mul_node_p, matcher_name);
