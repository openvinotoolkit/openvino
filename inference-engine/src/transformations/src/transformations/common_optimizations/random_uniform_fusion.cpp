...
ngraph::pass::RandomUniformMaxValFusion::RandomUniformMaxValFusion() {
    ...
    const auto mul_pattern = ngraph::pattern::wrap_type<opset8::Multiply>({random_uniform_or_convert_pattern, mul_const_pattern});
    ...
    ngraph::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        const auto mul_const = pattern_map.at(mul_const_pattern);
...
ngraph::pass::RandomUniformMinValFusion::RandomUniformMinValFusion() {
    ...
    const auto add_pattern = ngraph::pattern::wrap_type<opset8::Add>({convert_or_random_uniform_pattern, add_const_pattern});

    ngraph::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        const auto& pattern_map = m.get_pattern_value_map();
        const auto data = pattern_map.at(data_pattern);
        const auto random_uniform = pattern_map.at(random_uniform_pattern);
        const auto add_const = pattern_map.at(add_const_pattern);
