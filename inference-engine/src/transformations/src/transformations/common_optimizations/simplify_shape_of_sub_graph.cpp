...
NGRAPH_RTTI_DEFINITION(ov::pass::SharedShapeOf, "SharedShapeOf", 0);

bool ov::pass::SharedShapeOf::run_on_function(std::shared_ptr<ov::Function> f) {
    RUN_ON_FUNCTION_SCOPE(SharedShapeOf);
    bool graph_rewritten = false;

    std::map<ov::Output<Node>, std::vector<std::shared_ptr<ov::Node>>> source_to_shape_of;
...
NGRAPH_RTTI_DEFINITION(ov::pass::GroupedGatherElimination, "GroupedGatherElimination", 0);

ov::pass::GroupedGatherElimination::GroupedGatherElimination() {
    MATCHER_SCOPE(GroupedGatherElimination);
    auto concat_label = ov::pattern::wrap_type<ov::opset1::Concat>(pattern::rank_equals(1));
    ...
    ov::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        while (inputs.size() > i + 1) {
            ...
            auto joint_indices = ov::op::util::make_try_fold<opset1::Concat>(OutputVector{curr->input_value(1), next->input_value(1)}, 0);
            std::shared_ptr<Node> new_gather;
            if (is_opset1)
                new_gather = register_new_node<ov::opset1::Gather>(
                    curr->input_value(0), joint_indices->output(0), ov::opset1::Constant::create(element::i64, {}, {0})->output(0));
            else
                new_gather = register_new_node<ov::opset7::Gather>(
                        curr->input_value(0), joint_indices->output(0), ov::opset1::Constant::create(element::i64, {}, {0})->output(0));
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(concat_label, matcher_name);
    this->register_matcher(m, callback);
}

NGRAPH_RTTI_DEFINITION(ov::pass::GatherNopElimination, "GatherNopElimination", 0);
...
ov::pass::GatherNopElimination::GatherNopElimination() {
    ...
    const auto gather_label = ov::pattern::wrap_type<ov::op::util::GatherBase>(
            {ov::pattern::any_input(pattern::has_static_shape()),
             ov::pattern::wrap_type<ov::op::Constant>(),
             ov::pattern::wrap_type<ov::op::Constant>()});
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(gather_label, matcher_name);
    this->register_matcher(m, callback);
}

NGRAPH_RTTI_DEFINITION(ov::pass::SimplifyGatherShapeOf, "SimplifyGatherShapeOf", 0);

ov::pass::SimplifyGatherShapeOf::SimplifyGatherShapeOf() {
    MATCHER_SCOPE(SimplifyGatherShapeOf);
    const auto gather_pattern = ov::pattern::wrap_type<op::util::GatherBase>();
    const auto shape_of_pattern = ov::pattern::wrap_type<opset2::ShapeOf, opset3::ShapeOf>({gather_pattern});
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(shape_of_pattern, matcher_name);
    this->register_matcher(m, callback);
}

NGRAPH_RTTI_DEFINITION(ov::pass::SimplifyShapeOfSubGraph, "SimplifyShapeOfSubGraph", 0);

bool ov::pass::SimplifyShapeOfSubGraph::run_on_function(std::shared_ptr<ov::Function> f) {
    RUN_ON_FUNCTION_SCOPE(SimplifyShapeOfSubGraph);
    ov::pass::Manager manager;
    manager.register_pass<ov::pass::EliminateGatherUnsqueeze>();
    manager.register_pass<ov::pass::SharedShapeOf>();
    manager.register_pass<ov::pass::GroupedGatherElimination>();
    manager.register_pass<ov::pass::GatherNopElimination>();
    manager.register_pass<ov::pass::SimplifyGatherShapeOf>();
