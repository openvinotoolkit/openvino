...
static bool can_be_fused(const std::shared_ptr<opset5::Pad>& pad, const std::shared_ptr<T>& node,
    ...
    if (!pads_begin || !is_vector(pads_begin->output_shape(0).to_shape()))
        return false;
    if (!pads_end || !is_vector(pads_end->output_shape(0).to_shape()))
        return false;
    if (node->get_pads_begin().size() != shape_size(pads_begin->output_shape(0).to_shape()) - 2)
        return false;
    if (node->get_pads_end().size() != shape_size(pads_end->output_shape(0).to_shape()) - 2)
        return false;
