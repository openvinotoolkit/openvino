...
static bool simplify_gather(std::shared_ptr<Node> node) {
    if (auto gather = as_type_ptr<opset7::Gather>(node)) {
...
static bool simplify_gather_shapeof(shared_ptr<Node> node) {
    auto gather = as_type_ptr<opset7::Gather>(node->input_value(0).get_node_shared_ptr());
    if (!gather) {
        return false;
    }
    ...
    if (indices_rank.get_length() == 0) {
        ...
        replace_op = make_shared<opset7::Gather>(new_shapeof, new_indices, zero_axis);
        ...
        if (axis > 0) {
            ...
            auto gather = make_shared<opset7::Gather>(new_shapeof, indices, zero_axis);
...
SIMPLE_MATCHER_PASS_DEFINITION(EliminateGather, opset7::Gather, simplify_gather);
