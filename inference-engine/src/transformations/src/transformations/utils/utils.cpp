...
bool is_seq_len_provided(const std::shared_ptr<const Node> &seq_len_input, int64_t max_seq_len) {
    if (const auto &seq_len_const = std::dynamic_pointer_cast<const ngraph::op::Constant>(seq_len_input)) {
