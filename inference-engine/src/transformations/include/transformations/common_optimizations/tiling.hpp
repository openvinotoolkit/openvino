...
public:
    ...
    TileConvolution() {
        ...
        ngraph::matcher_pass_callback callback = [=](ngraph::pattern::Matcher &m) {
            ...
            Tiles tiles(requested_tiles.get_tiles_sizes());
        ...
        ngraph::matcher_pass_callback callback = [=](ngraph::pattern::Matcher &m) {
            ...
            for (auto & tile : tiles) {
                ...
                new_tile.modifier = [pad_to_add_h_begin, pad_to_add_w_begin, pad_to_add_h_end, pad_to_add_w_end](std::shared_ptr<Node> node) {
                    ...
                    auto old_pad_begin = std::dynamic_pointer_cast<opset7::Constant>(pad->input_value(1).get_node_shared_ptr());
                    auto old_pad_end = std::dynamic_pointer_cast<opset7::Constant>(pad->input_value(2).get_node_shared_ptr());

                    if (!old_pad_begin || !old_pad_end) return;

                    auto pad_begin_value = old_pad_begin->cast_vector<int64_t>();
                    auto pad_end_value = old_pad_end->cast_vector<int64_t>();

                    // update spatial dims values to the tile values
                    pad_begin_value[2] = pad_to_add_h_begin;
                    pad_begin_value[3] = pad_to_add_w_begin;

                    pad_end_value[2] = pad_to_add_h_end;
                    pad_end_value[3] = pad_to_add_w_end;

                    auto new_pad_begin = opset7::Constant::create(element::i64, Shape{pad_begin_value.size()}, pad_begin_value);
                    auto new_pad_end = opset7::Constant::create(element::i64, Shape{pad_end_value.size()}, pad_end_value);

                    pad->input(1).replace_source_output(new_pad_begin);
                    pad->input(2).replace_source_output(new_pad_end);
                };
...
public:
    ...
    TileEltwise() {
        ...
        ngraph::matcher_pass_callback callback = [](ngraph::pattern::Matcher &m) {
            ...
            // TODO: propagate tile to the second input
...
public:
    ...
    TileUnary() {
        // MATCHER_SCOPE(TileConvolution);
        auto unary = pattern::wrap_type<ngraph::op::util::UnaryElementwiseArithmetic, ngraph::opset7::PRelu, ngraph::opset7::MVN>();
    ...
    TileUnsupported() {
        ...
        ngraph::matcher_pass_callback callback = [](ngraph::pattern::Matcher &m) {
            ...
            if (!std::dynamic_pointer_cast<opset7::Constant>(node) &&
                !std::dynamic_pointer_cast<opset7::Parameter>(node)) {
...
public:
    ...
    InitTiles() {
        ...
        ngraph::matcher_pass_callback callback = [=](ngraph::pattern::Matcher &m) {
            ...
            // Change this count to set tiling size
            const int h_count = 2;
            const int w_count = 2;
            ...
            for (int i = 0, oh_begin = 0; i < h_count; ++i, oh_begin += h_step) {
                for (int j = 0, ow_begin = 0; j < w_count; ++j, ow_begin += w_step) {
                    ...
                    tiles[tile_index++] = TileElement({oh_begin, oh_end, ow_begin, ow_end});
                }
            }
...
private:
    using Functions = std::vector<std::vector<std::shared_ptr<Function>>>;
    Functions & m_tiles;
    bool m_concat_outputs;

public:
    TileFunction() = delete;

    TileFunction(Functions & tiles, bool concat_outputs)
            : m_tiles(tiles), m_concat_outputs(concat_outputs) {}
    ...
    bool run_on_function(std::shared_ptr<Function> f) override {
        ...
        const auto & tiles_size = tiles.get_tiles_sizes();
        assert(tiles_size.size() == 2);

        std::vector<std::vector<std::shared_ptr<Function>>> & function_tiles = m_tiles;
        function_tiles.resize(tiles_size[0], std::vector<std::shared_ptr<Function>>(tiles_size[1]));

        size_t tile_index{0};
        for (size_t i = 0; i < function_tiles.size(); ++i) {
            for (size_t j = 0; j < function_tiles[i].size(); ++j) {
                auto f_copy = ngraph::clone_function(*f);

                for (auto &&node : f_copy->get_ordered_ops()) {
                    for (auto &&input : node->inputs()) {
                        if (!has_tiles(input)) continue;
                        auto input_tile = get_tiles(input).tiles[tile_index];
    ...
    bool tile_is_equal_to_input(const TileElement & tile, const Shape & input_shape) {
        return tile.size() == std::accumulate(input_shape.begin() + 2,
                                              input_shape.end(),
                                              1ull,
...
public:
    // NGRAPH_RTTI_DECLARATION;
    Tiling(Functions & tiles, bool concat_outputs)
        : m_tiles(tiles), m_concat_outputs(concat_outputs) {}
    ...
    bool run_on_function(std::shared_ptr<Function> f) override {
        ...
        m.register_pass<TileFunction>(m_tiles, m_concat_outputs);
        m.run_passes(f);
