...
ov::matcher_pass_callback ConvertReduceBase::convert_reduce_to_pooling() {
    return [&](ov::pattern::Matcher& m) {
        ...
        if (std::all_of(axes_vector.begin(), axes_vector.end(),
            ...
            auto reshape = std::make_shared<ov::opset1::Reshape>(input,
                    ov::opset1::Constant::create(ov::element::i64, ov::Shape{reshape_shape.size()}, reshape_shape), true);
        ...
        if (!shape_begin.empty() && shape_begin != input.get_shape()) {
            input = std::make_shared<ov::opset1::Reshape>(input,
                    ov::opset1::Constant::create(ov::element::i64, ov::Shape{shape_begin.size()}, shape_begin), true);
        ...
        if (std::is_same<T, ov::opset1::ReduceMean>()) {
            ...
            input = std::make_shared<ov::opset1::Multiply>(input,
        ...
        if (!shape_end.empty() && shape_end != input.get_shape()) {
            input = std::make_shared<ov::opset1::Reshape>(input,
                    ov::opset1::Constant::create(ov::element::i64, ov::Shape{shape_end.size()}, shape_end), true);
            new_ops.push_back(input.get_node_shared_ptr());
        }
