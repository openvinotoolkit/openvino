...
void AddConvolutionKernelPadPass::run() {
    int numOfadditionalConcatCropLayers = 0;
    ...
    for (auto& l : *pLayers) {
        ...
        if (inputs->getLayout() != Layout::NHWC &&
            ...
            THROW_GNA_LAYER_EXCEPTION(l) << "with layout " << inputs->getLayout() << " isn't currently supported on GNA";
        }

        auto inputDims = inputs->getTensorDesc().getDims();
        auto outputDims = outputs->getTensorDesc().getDims();

        auto getIROrder = [](InferenceEngine::Layout layout) {
        ...
        auto getIRDim = [](InferenceEngine::DataPtr mem, size_t idx) {
        ...
        auto in_batch = getIRDim(inputs, inputOrder[0]);
        auto in_channels = getIRDim(inputs, inputOrder[1]);
        auto in_height = getIRDim(inputs, inputOrder[2]);
        auto in_width = getIRDim(inputs, inputOrder[3]);

        auto out_batch = getIRDim(outputs, outputOrder[0]);
        auto out_channels = getIRDim(outputs, outputOrder[1]);
        auto out_height = getIRDim(outputs, outputOrder[2]);
        auto out_width = getIRDim(outputs, outputOrder[3]);
        ...
        CNNLayerPtr concatLayer = std::make_shared<GenericLayer>(LayerParams({ concatName, "Concat", inputs->getTensorDesc().getPrecision() }));
        auto concatLayerWithQuant = quantized ?
            InferenceEngine::injectData<QuantizedLayerParams>(concatLayer) :
            concatLayer;
        ...
        auto constName = l->name + std::string("/constant_") + std::to_string(numOfadditionalConcatCropLayers);
        CNNLayerPtr constLayer = std::make_shared<GenericLayer>(LayerParams({ constName, "const", inputs->getTensorDesc().getPrecision() }));
        ...
        getInputTo(constLayerWithQuant->outData.front())[concatLayerWithQuant->name] = concatLayerWithQuant;

        concatLayerWithQuant->insData.push_back(prev->outData.front());
        //ERROR: Data 1 inserted into layer Convolution_4/new/concat_0 is nullptr
        //concatLayerWithQuant->insData.push_back(constLayerWithQuant->outData.front());
        getInputTo(concatLayerWithQuant->outData.front())[l->name] = l;
        getInputTo(prev->outData.front()).erase(l->name);
        getInputTo(prev->outData.front())[concatLayerWithQuant->outData.front()->getName()] = concatLayerWithQuant;

        l->insData.clear();
        l->insData.push_back(concatLayerWithQuant->outData.front());

        l->outData.front()->setDims(SizeVector({ out_batch, out_channels, out_height, out_width + num_input_padding }));

        prev = l;
        auto next = getInputTo(l->outData.front()).size() != 0 ? getInputTo(l->outData.front()).begin()->second : nullptr;
        if (LayerInfo(next).isActivation()) {
            prev = next;
        ...
        auto cropLayer = std::make_shared<CropLayer>(LayerParams({ cropName, "Crop", outputs->getTensorDesc().getPrecision() }));
        cropLayer->axis = std::vector<int>{ static_cast<int>(inputOrder[3]) };
        cropLayer->dim = std::vector<int>{ static_cast<int>(out_width) };
        cropLayer->offset = std::vector<int>{ 0 };
        auto cropLayerWithQuant = quantized ?
            InferenceEngine::injectData<QuantizedLayerParams>(cropLayer) :
            cropLayer;

        auto cropOutData = std::make_shared<Data>(cropName,
            TensorDesc(inputs->getTensorDesc().getPrecision(),
                outputDims,
                inputs->getTensorDesc().getLayout()));
        getCreatorLayer(cropOutData) = cropLayerWithQuant;
