...
void GNAPlugin::LoadNetwork(CNNNetwork & _network) {
    ...
    if (_network.getFunction()) {
        ...
        if (config.gnaCompileTarget == InferenceEngine::GNAConfigParams::GNA_TARGET_2_0) {
            ...
            manager.register_pass<Decompose2DConvWithBias>();
            manager.register_pass<Decompose2DConvWithBiasAF>();
            manager.register_pass<Decompose2DConvWithBiasMaxPool>();
            manager.register_pass<Decompose2DConvWithBiasMaxPoolAF>();
