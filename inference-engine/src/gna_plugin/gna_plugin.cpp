...
InferenceEngine::ExecutableNetwork GNAPlugin::ImportNetwork(std::istream& networkModel) {
    ...
    for (auto && memory : mt) {
        ...
        std::string name;
        std::tie(memoryLayer.gna_ptr, memoryLayer.reserved_size, name) = memory;
...
void GNAPlugin::Export(std::ostream &outStream) {
    ...
    for (auto && memoryConnection : graphCompiler.memory_connection) {
        serial.AddState(memoryConnection.second.gna_ptr, memoryConnection.second.reserved_size, memoryConnection.first);
    }
