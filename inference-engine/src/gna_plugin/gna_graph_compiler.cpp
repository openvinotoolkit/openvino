...
void GNAGraphCompiler::ConvolutionPrimitive(InferenceEngine::CNNLayerPtr layer) {
    ...
    if (in_batch != out_batch) {
        THROW_GNA_LAYER_EXCEPTION(layer) << "I/O batch mismatch";
    }
    ...
    //if (convolution._stride_x > 1) {
    //    num_feature_map_rows = (in_channels * in_height * in_width) / convolution._stride_x;
    //    num_feature_map_columns = convolution._stride_x;
    //} else if (convolution._stride_y > 1) {
    //    num_feature_map_rows = (in_channels * in_height * in_width) / convolution._stride_y;
    //    num_feature_map_columns = convolution._stride_y;
    //} else {
    //    num_feature_map_rows = in_channels * in_height * in_width; // (in_height > 1) ? in_height : in_width;
    //    num_feature_map_columns = 1; // (in_channels > 1) ? in_channels : ((convolution._kernel_x > 1) ? convolution._kernel_x : convolution._kernel_y);
    //}
    ...
    if (num_conv_kernel_padding == 0) {
        gnamem->readonly().push_local_ptr(ptr_weights,
            doTransposeWeights ? transposedWeights.data() : convolution._weights->buffer(),
            convolution._weights->byteSize(),
            64);
    } else {
        ...
        auto initializer = [=](void* data, std::size_t size) {
            ...
            const uint8_t* srcPtr = doTransposeWeights ? transposedWeights.data() : convolution._weights->cbuffer().as<uint8_t*>();
            uint8_t* dstPtr = reinterpret_cast<uint8_t*>(data);
            for (int i = 0; i < convolution._out_depth; i++) {
                ie_memcpy(dstPtr + offset,
                    size - offset,
                    srcPtr + single_conv_kernel_size * i * convolution.precision.size(),
