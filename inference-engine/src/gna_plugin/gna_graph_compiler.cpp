...
void GNAGraphCompiler::PowerPrimitive(InferenceEngine::CNNLayerPtr layer) {
    ...
    uint32_t num_rows_in = 0;
    uint32_t num_columns_in = 0;
    uint32_t num_rows_out = 0;
    uint32_t num_padding = 0;

    num_rows_in = InferenceEngine::details::product(begin(input->getDims()), end(input->getDims()));
    for (size_t index_divide = 8;; index_divide--) {
