...
ngraph::pass::ConvertLSTMSequenceMatcher::ConvertLSTMSequenceMatcher() {
    ...
    ngraph::matcher_pass_callback callback = [](pattern::Matcher &m) {
        ...
        int64_t seq_axis = 1;
        Output<Node> in_0 = lstm_sequence->input(0).get_source_output();
        auto transpose_before = std::dynamic_pointer_cast<ngraph::opset5::Transpose>(lstm_sequence->input_value(0).get_node_shared_ptr());
        // todo check count of target inputs

        auto transpose_after = std::dynamic_pointer_cast<ngraph::opset5::Transpose>(lstm_sequence->output(0).get_target_inputs().begin()->get_node()->shared_from_this());
        bool flag = (transpose_after != nullptr) && (transpose_before != nullptr);
        if (flag) {
            auto order_before = std::dynamic_pointer_cast<ngraph::opset5::Constant>(transpose_before->input_value(1).get_node_shared_ptr());
            auto order_after = std::dynamic_pointer_cast<ngraph::opset5::Constant>(transpose_after->input_value(1).get_node_shared_ptr());
            if (order_before != nullptr && order_after != nullptr) {
                ...
                if (order_before_values == order_ref_before && order_after_values == order_ref_after) {
                    in_0 = transpose_before->input(0).get_source_output();
                    seq_axis = 0;
                }
            }
        }
