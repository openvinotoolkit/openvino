...
NGRAPH_RTTI_DEFINITION(ov::pass::ConvertNMSToNMSIEMatcher, "ConvertNMSToNMSIEMatcher", 0);

ov::pass::ConvertNMSToNMSIEMatcher::ConvertNMSToNMSIEMatcher() {
    auto nms = ov::pattern::wrap_type<opset3::NonMaxSuppression>();
    ...
    ov::matcher_pass_callback callback = [](pattern::Matcher &m) {
        ...
        if (max_output_boxes_per_class_rank.get_length() == 0) {
            ...
            if (auto new_max_per_class_const = std::dynamic_pointer_cast<opset1::Constant>(new_max_per_class.get_node_shared_ptr())) {
                ...
                new_max_per_class = std::make_shared<ov::op::v0::Unsqueeze>(
                        nms->input_value(2),
        ...
        if (iou_threshold_rank.get_length() == 0) {
            new_iou_threshold = std::make_shared<ov::op::v0::Unsqueeze>(
                    nms->input_value(3),
                    opset1::Constant::create(element::i64, Shape{1}, {0}));
        ...
        if (score_threshold_rank.get_length() == 0) {
            new_score_threshold = std::make_shared<ov::op::v0::Unsqueeze>(
                    nms->input_value(4),
                    opset1::Constant::create(element::i64, Shape{1}, {0}));
        ...
        auto new_nms = std::make_shared<ov::op::NonMaxSuppressionIE>(nms->input_value(0),
                                                                         nms->input_value(1),
                                                                         new_max_per_class,
                                                                         new_iou_threshold,
                                                                         new_score_threshold,
        ...
        ov::replace_node(nms, new_nms);
        return true;
    };

    auto m = std::make_shared<ov::pattern::Matcher>(nms, "ConvertNMSToNMSIE");
