...
CONVERSION_RESULT check_constant(const std::shared_ptr<ov::opset1::Constant>& constant,
                                 const ov::PartialShape& shape) {
    if (!constant || shape.rank().is_dynamic()) return CONVERSION_RESULT::NONE;

    auto const_shape = constant->get_shape();
    std::vector<ov::Dimension> input_shape(shape);
...
NGRAPH_RTTI_DEFINITION(ov::pass::ConvertMulAddToScaleShiftOrPower, "ConvertMulAddToScaleShiftOrPower", 0);

ov::pass::ConvertMulAddToScaleShiftOrPower::ConvertMulAddToScaleShiftOrPower() {
    auto data_batch = std::make_shared<pattern::op::Label>(element::f32, Shape {1});

    auto weights = std::make_shared<ov::opset1::Constant>(element::f32, Shape {1}, std::vector<float> {0});
    auto bias = std::make_shared<ov::opset1::Constant>(element::f32, Shape {1}, std::vector<float> {0});

    auto mul = std::make_shared<ov::opset1::Multiply>(data_batch, weights);
    auto add = std::make_shared<ov::opset1::Add>(mul, bias);

    ov::matcher_pass_callback callback = [](pattern::Matcher& m) {
        auto add_node = ov::as_type_ptr<ov::opset1::Add>(m.get_match_root());
        ...
        auto mul_node = ov::as_type_ptr<ov::opset1::Multiply>(add_input_0);
        auto const_bias_node = ov::as_type_ptr<ov::opset1::Constant>(add_input_1);
        if (!mul_node) {
            mul_node = ov::as_type_ptr<ov::opset1::Multiply>(add_input_1);
            const_bias_node = ov::as_type_ptr<ov::opset1::Constant>(add_input_0);
        }
        ...
        auto const_weights_node = ov::as_type_ptr<ov::opset1::Constant>(mul_input_1);
        if (!const_weights_node) {
            data_node = mul_node->input(1).get_source_output();
            const_weights_node = ov::as_type_ptr<ov::opset1::Constant>(mul_input_0);
        }
        ...
        if (ov::op::util::constantIsEqualTo(const_weights_node, 1) &&
            ...
            for (const auto & output : add_node->output(0).get_target_inputs()) {
                if (dynamic_cast<ov::opset1::Result*>(output.get_node())) {
            ...
            if (!has_result_output || consumers_count == 1) {
                if (!std::dynamic_pointer_cast<ov::opset1::Parameter>(parent)) {
                    parent->set_friendly_name(add_node->get_friendly_name());
                }
                // TODO: due to ov::replace_node function limitations we have to reconnect output port consumers to the new input
        ...
        if (res1 == CONVERSION_RESULT::SCALE_SHIFT || res2 == CONVERSION_RESULT::SCALE_SHIFT || is_dequantization) {
            ...
            auto scaleshift = std::make_shared<ov::op::ScaleShiftIE>(data_node, weights_in, biases_in, output_type);
            new_ops.push_back(scaleshift);

            scaleshift->set_friendly_name(add_node->get_friendly_name());
            ov::copy_runtime_info({mul_node, add_node}, new_ops);
            ...
            auto output_type = m.get_match_root()->get_output_element_type(0);
            auto power = std::make_shared<ov::op::PowerIE>(data_node, 1.0f, scale, shift, output_type);
            power->set_friendly_name(add_node->get_friendly_name());
            ov::copy_runtime_info({mul_node, add_node}, power);
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(add, "MulAddToScaleShiftOrPower");
