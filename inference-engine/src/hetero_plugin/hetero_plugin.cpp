...
InferenceEngine::InferencePlugin Engine::GetDevicePlugin(const std::string& device) const {
    ...
    if (nullptr == _core) {
        ...
        plugin = InferencePlugin{_core->GetPluginByName(device)};
    }
...
void Engine::AddExtension(InferenceEngine::IExtensionPtr extension) {
    ...
    try {
        for (auto&& plugin : _plugins) {
            plugin.second.AddExtension(std::move(extension));
        }
    } catch (InferenceEngine::details::InferenceEngineException &) {}
}
