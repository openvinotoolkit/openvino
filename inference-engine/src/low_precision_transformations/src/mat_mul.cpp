...
bool MatMulTransformation::canBeTransformed(const TransformationContext& context, std::shared_ptr<Node> layer) const {
    ...
    if (!dequantization2.empty()) {
        ...
        if (dequantization2.subtract) {
            std::shared_ptr<Node> roundedConst = NetworkHelper::round(dequantization2.subtractConstant, dequantization2.data.get_element_type());
            roundedConst = NetworkHelper::toScalarIfPossible(roundedConst);
