...
FoldConvertTransformation::FoldConvertTransformation(const Params& params) : LayerTransformation(params) {
    auto subtract = pattern::wrap_type<op::v1::Subtract>();
...
bool FoldConvertTransformation::transform(TransformationContext& context, ngraph::pattern::Matcher &m) {
    ...
    auto foldConvert = [&](const size_t branch) {
        ...
        if (!is_type<op::v0::Convert>(convert) || !is_type<op::Constant>(convert->get_input_node_shared_ptr(0))) {
            return;
        }
        ...
        assert(is_type<op::Constant>(resultConstant));
...
bool FoldConvertTransformation::canBeTransformed(const TransformationContext& context, std::shared_ptr<Node> operation) const {
    return
        (is_type<op::v0::Convert>(operation->get_input_node_ptr(1)) &&
        is_type<op::Constant>(operation->get_input_node_ptr(1)->get_input_node_ptr(0))) ||
        (is_type<op::v0::Convert>(operation->get_input_node_ptr(0)) &&
        is_type<op::Constant>(operation->get_input_node_ptr(0)->get_input_node_ptr(0)));
}
