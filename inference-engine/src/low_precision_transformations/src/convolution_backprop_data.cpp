...
bool ConvolutionBackpropDataTransformation::transform(TransformationContext &context, ngraph::pattern::Matcher &m) const {
    ...
    if (!WeightableLayerTransformation::canBeTransformed(context, convolutionBackpropData)) {
        return false;
    }

    FakeQuantizeDequantization dequantization = NetworkHelper::getDequantization(convolutionBackpropData);
    if (!canSubtractBeHandled(convolutionBackpropData, dequantization)) {
        return false;
    }

    if ((!supportAsymmetricQuantization) && getDataPrecisionOnWeights(convolutionBackpropData).hasZeroPoint) {
        return false;
    }
    ...
    dequantization = NetworkHelper::getDequantization(convolutionBackpropData);
