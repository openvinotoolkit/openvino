...
bool QuantizationDetails::outputLayoutIsSupported(std::shared_ptr<opset1::FakeQuantize> quantize) {
    if (!NetworkHelper::isConstantPath(quantize->get_input_node_shared_ptr(1)) ||
        !NetworkHelper::isConstantPath(quantize->get_input_node_shared_ptr(2)) ||
        !NetworkHelper::isConstantPath(quantize->get_input_node_shared_ptr(3)) ||
        !NetworkHelper::isConstantPath(quantize->get_input_node_shared_ptr(4))) {
        return false;
    }
