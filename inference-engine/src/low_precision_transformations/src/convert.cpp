...
NGRAPH_RTTI_DEFINITION(ov::pass::low_precision::ConvertTransformation, "ConvertTransformation", 0);
...
ConvertTransformation::ConvertTransformation(const Params& params) : LayerTransformation(params) {
    ...
    auto m = std::make_shared<ov::pattern::Matcher>(matcher, "ConvertTransformation");
    this->register_matcher(m, callback);
}

bool ConvertTransformation::transform(TransformationContext& context, ov::pattern::Matcher &m) {
