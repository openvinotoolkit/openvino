...
bool NetworkHelper::areQuantizeAndDequantizeSupportedForSubtract(const std::shared_ptr<Node>& node) {
...
bool NetworkHelper::areQuantizeAndDequantizeSupportedForMultiply(const std::shared_ptr<Node>& node) {
    if (!is_type<opset1::Multiply>(node)) {
        return false;
    }

    const auto dequantization = ngraph::pass::low_precision::NetworkHelper::getDequantization(node, 0, true);
    if (dequantization.empty()) {
        return false;
    }
