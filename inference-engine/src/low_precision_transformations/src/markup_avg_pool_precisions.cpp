...
bool ngraph::pass::low_precision::MarkupAvgPoolPrecisions::run_on_function(std::shared_ptr<ngraph::Function> f) {
    for (const std::shared_ptr<Node>& node : f->get_ordered_ops()) {
        ...
        if (ngraph::is_type<opset1::AvgPool>(node)) {
            ...
            const auto attribute = std::make_shared<::ngraph::VariantWrapper<ExpectedOperationAttribute>>(
                ExpectedOperationAttribute::create<opset1::FakeQuantize>(true));
            rtInfo[ngraph::VariantWrapper<ExpectedOperationAttribute>::type_info.name] = attribute;
            continue;
        }
        ...
        //}
        {
            ...
            auto tmpAttribute = std::dynamic_pointer_cast<ngraph::VariantWrapper<ExpectedOperationAttribute>>(it->second);
        ...
        std::shared_ptr<ngraph::VariantWrapper<ExpectedOperationAttribute>> firstExistingAttribute;
        ...
        if (firstExistingAttribute != nullptr) {
            ...
            if (!wasFound) {
                auto& rtInfo = node->get_rt_info();
                rtInfo[ngraph::VariantWrapper<ExpectedOperationAttribute>::type_info.name] = newAttribute;
            }
        }
    }
    return true;
