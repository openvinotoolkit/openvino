...
bool ReduceBaseTransformation::transform(TransformationContext& context, ov::pattern::Matcher& m) {
...
bool ReduceBaseTransformation::canBeTransformed(const TransformationContext& context, std::shared_ptr<Node> reduce) const {
    ...
    const auto axesConstant = as_type_ptr<ov::opset1::Constant>(reduce->get_input_node_shared_ptr(1));
