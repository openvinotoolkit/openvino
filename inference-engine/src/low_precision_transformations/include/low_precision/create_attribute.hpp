...
template <typename AttributeType, typename OperationType = ov::pattern::op::Label>
    ...
    CreateAttribute(const AttributeSource source = AttributeSource::Node) {
        ...
        ov::graph_rewrite_callback callback = [&](pattern::Matcher& m) {
            ...
            }
            {
                ...
                const auto attribute = ov::VariantWrapper<AttributeType>::create(op, params);
        ...
        auto matcher = std::make_shared<ov::pattern::Matcher>(operation, "CreateAttribute");
