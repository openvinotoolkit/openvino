...
bool MKLDNNEmbeddingBagPackedSumNode::isSupportedOperation(const std::shared_ptr<const ngraph::Node>& op,
...
MKLDNNEmbeddingBagPackedSumNode::MKLDNNEmbeddingBagPackedSumNode(const std::shared_ptr<ngraph::Node>& op,
                                                                 const mkldnn::engine& eng,
                                                                 MKLDNNWeightsSharing::Ptr& cache)
    : MKLDNNNode(op, eng, cache),
      MKLDNNEmbeddingBagSumNode(op, 2lu, 1lu, 2lu, 3lu) {
...
void MKLDNNEmbeddingBagPackedSumNode::initSupportedPrimitiveDescriptors() {
    ...
    std::vector<PortConfigurator> inDataConfigurators(
        {{LayoutType::ncsp, inDataPrecision}, {LayoutType::ncsp, Precision::I32}});
...
void MKLDNNEmbeddingBagPackedSumNode::getIndices(int embIndex,
                                                 const int*& indices,
                                                 size_t& size,
                                                 int& weightsIdx,
                                                 bool& withWeight) {
