...
void MKLDNNQuantizeNode::appendPostOps(mkldnn::post_ops& ops) {
    if (quantizeOpType == QuantizeOpType::Binarization) {
        size_t paddedSize = rnd_up(binarizationThresholds.size(), 16);
        binarizationThresholds.resize(paddedSize, 0);
        binarizationOutputMask.resize(paddedSize, 0);

        ops.append_binarization(binarization_depthwise, (const float*)&binarizationThresholds[0], (const float*)&binarizationOutputMask[0]);
    } else {
        ...
        mkldnn::algorithm alg =
                quantizeOpType == QuantizeOpType::FakeQuantization ? mkldnn::quantization_quantize_dequantize :
                quantizeOpType == QuantizeOpType::Quantization ? mkldnn::quantization_quantize
                                                               : mkldnn::binarization_depthwise;
