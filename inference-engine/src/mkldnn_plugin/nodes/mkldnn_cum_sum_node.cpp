...
bool MKLDNNCumSumNode::isSupportedOperation(const std::shared_ptr<const ngraph::Node>& op,
...
MKLDNNCumSumNode::MKLDNNCumSumNode(const std::shared_ptr<ngraph::Node>& op,
...
void MKLDNNCumSumNode::initSupportedPrimitiveDescriptors() {
    ...
    if (dataPrecision != Precision::I8 && dataPrecision != Precision::U8 && dataPrecision != Precision::I16 &&
        dataPrecision != Precision::I32 && dataPrecision != Precision::FP32 && dataPrecision != Precision::I64 &&
        dataPrecision != Precision::U64 && dataPrecision != Precision::BF16)
...
void MKLDNNCumSumNode::cumSum(const dataType* input, dataType* output, const VectorDims& strides) {
    ...
    size_t work_amount_dst =
        std::accumulate(iterationRange.begin(), iterationRange.end(), 1, std::multiplies<size_t>());
...
void MKLDNNCumSumNode::parallelItInit(size_t start,
...
inline size_t MKLDNNCumSumNode::getStartOffset(const std::vector<size_t>& forStartOffset,
