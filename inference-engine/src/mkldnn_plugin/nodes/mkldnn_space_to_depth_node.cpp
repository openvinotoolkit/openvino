...
#define THROW_ERROR                                                            \
  IE_THROW() << "SpaceToDepth layer with name '" << getName() << "' "
...
bool MKLDNNSpaceToDepthNode::isSupportedOperation(
    const std::shared_ptr<const ngraph::Node> &op,
    std::string &errorMessage) noexcept {
  try {
    const auto spaceToDepth =
        ov::as_type_ptr<const ngraph::opset1::SpaceToDepth>(op);
    if (!spaceToDepth) {
...
MKLDNNSpaceToDepthNode::MKLDNNSpaceToDepthNode(
  ...
  attrs.blockStep =
      static_cast<size_t>(std::pow(attrs.blockSize, nSpatialDims));
}
...
void MKLDNNSpaceToDepthNode::initSupportedPrimitiveDescriptors() {
  ...
  if (inputDataShape.getRank() > 2) {
    ...
    auto canUseBlocked = [=](const size_t block) {
      return srcDims[1] != Shape::UNDEFINED_DIM && srcDims[1] % block == 0 &&
             (attrs.mode == Mode::DEPTH_FIRST ? block % attrs.blockStep == 0
                                              : true);
    };
  ...
  auto range = BlockedDescCreator::makeFilteredRange(
      creators, inputDataShape.getRank(), supportedTypes);

  for (auto itr = range.first; itr != range.second; ++itr) {
    config.inConfs[0].desc =
        itr->second->createSharedDesc(precision, inputDataShape);
    config.outConfs[0].desc =
        itr->second->createSharedDesc(precision, outputDataShape);
...
void MKLDNNSpaceToDepthNode::createPrimitive() {
  ...
  attrs.layoutType = memoryDesc.hasLayoutType(LayoutType::nCsp16c)
                         ...
                         : memoryDesc.hasLayoutType(LayoutType::nCsp8c)
                               ? LayoutType::nCsp8c
                               : memoryDesc.hasLayoutType(LayoutType::nspc)
                                     ? LayoutType::nspc
                                     : LayoutType::ncsp;
...
void MKLDNNSpaceToDepthNode::prepareParams() {
  attrs.srcBlockedDims = getParentEdgeAt(0)
                             ->getMemoryPtr()
                             ->GetDescWithType<BlockedMemoryDesc>()
                             ->getBlockDims();
  attrs.dstBlockedDims = getChildEdgeAt(0)
                             ->getMemoryPtr()
                             ->GetDescWithType<BlockedMemoryDesc>()
                             ->getBlockDims();
...
MKLDNNSpaceToDepthNode::SpaceToDepthExecutor::SpaceToDepthExecutor(
    const SpaceToDepthAttrs &attrs) {
  if (!MKLDNNPlugin::one_of(attrs.layoutType, LayoutType::nCsp16c,
                            LayoutType::nCsp8c, LayoutType::nspc,
                            LayoutType::ncsp))
    IE_THROW() << "DepthToSpace executor supports only 'nCsp16c', 'nCsp8c', "
                  "'nspc' or 'ncsp' layouts.";

  const bool isBlocked = MKLDNNPlugin::one_of(
      attrs.layoutType, LayoutType::nCsp16c, LayoutType::nCsp8c);
  ...
  const size_t reshapedRank =
      nDims + attrs.nSpatialDims +
      static_cast<int>(isBlocked && attrs.mode == Mode::DEPTH_FIRST);
  ...
  auto reshapeAndSetPermOrder = [&](const size_t idx1, const size_t idx2,
                                    const size_t shift,
  ...
  if (isBlocked) {
    ...
    if (attrs.mode == Mode::BLOCKS_FIRST) {
      ...
      params.src_block_dims[params.order[attrs.nSpatialDims + 1]] =
          attrs.srcBlockedDims[1];
      params.src_block_dims[params.order[lastIdx]] =
          attrs.srcBlockedDims.back();
    } else {
    ...
    reshapeAndSetPermOrder(orderShiftForBlocks, orderShiftForDims,
                           firstSpatialOrder, attrs.dstBlockedDims);
  } else if (isChannelsFirst) {
    firstSpatialOrder = 1;

    size_t shift =
        static_cast<size_t>(attrs.mode == DEPTH_FIRST) + attrs.nSpatialDims + 1;
    params.order[attrs.mode == Mode::DEPTH_FIRST ? attrs.nSpatialDims + 1
                                                 : lastIdx] = lastIdx;
    params.src_block_dims[lastIdx] = attrs.srcBlockedDims.back();

    reshapeAndSetPermOrder(firstSpatialOrder, shift, firstSpatialOrder,
                           attrs.dstBlockedDims);
  } else {
    size_t shift = static_cast<size_t>(attrs.mode == DEPTH_FIRST) + 1;
    params.order[attrs.mode == Mode::DEPTH_FIRST ? 1 : attrs.nSpatialDims + 1] =
        1;
    params.src_block_dims[1] = attrs.srcBlockedDims[1];

    reshapeAndSetPermOrder(attrs.nSpatialDims + firstSpatialOrder, shift,
                           firstSpatialOrder, attrs.dstBlockedDims);
  }
...
void MKLDNNSpaceToDepthNode::SpaceToDepthExecutor::exec(
    MKLDNNMemoryPtr &srcMemPtr, MKLDNNMemoryPtr &dstMemPtr, const int MB) {
  if (!permuteKernel)
    IE_THROW()
        << "Could not execute. Kernel for Transpose node was not compiled.";

  const uint8_t *srcData =
      reinterpret_cast<const uint8_t *>(srcMemPtr->GetPtr());
...
void MKLDNNSpaceToDepthNode::execute(mkldnn::stream strm) {
  ...
  int MB = isDynamicNode()
               ? getParentEdgeAt(0)->getMemoryPtr()->getStaticDims()[0]
               : batchToProcess();
  execPtr->exec(getParentEdgeAt(0)->getMemoryPtr(),
                getChildEdgeAt(0)->getMemoryPtr(), MB);
}
