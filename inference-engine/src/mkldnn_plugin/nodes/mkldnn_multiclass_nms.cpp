...
MKLDNNMultiClassNmsNode::MKLDNNMultiClassNmsNode(
    ...
    if (boxes_dims.size() != 3)
        IE_THROW() << errorPrefix << "has unsupported 'boxes' input rank: "
                   << boxes_dims.size();
    if (boxes_dims[2] != 4)
    ...
    if (scores_dims.size() != 3)
        IE_THROW() << errorPrefix << "has unsupported 'scores' input rank: "
                   << scores_dims.size();
    ...
    const SizeVector& valid_outputs_dims =
        op->get_output_shape(NMS_SELECTEDNUM);
...
void MKLDNNMultiClassNmsNode::execute(mkldnn::stream strm) {
    ...
    if (outDims.size() > NMS_SELECTEDOUTPUTS)
        selected_outputs =
            reinterpret_cast<float*>(getChildEdgesAtPort(NMS_SELECTEDOUTPUTS)[0]
                                         ->getMemoryPtr()
                                         ->GetPtr());
    ...
    for (size_t i = 0; i < dims_boxes[0]; i++) {
        ...
        for (size_t j = 0; j < real_boxes; j++) {
            ...
            selected_base[3] =
                boxes[selected_indices[j + output_offset] * 4 + 1];
            selected_base[4] =
                boxes[selected_indices[j + output_offset] * 4 + 2];
            selected_base[5] =
                boxes[selected_indices[j + output_offset] * 4 + 3];
        }
...
void MKLDNNMultiClassNmsNode::nmsWithEta(
    ...
    parallel_for2d(num_batches, num_classes, [&](int batch_idx, int class_idx) {
        if (class_idx != background_class) {
            ...
            const float* scoresPtr = scores + batch_idx * scoresStrides[0] +
            ...
            for (int box_idx = 0; box_idx < num_boxes; box_idx++) {
                if (scoresPtr[box_idx] >= score_threshold)  // algin with ref
                    sorted_boxes.emplace(
                        boxInfo({scoresPtr[box_idx], box_idx, 0}));
            }
            fb.reserve(sorted_boxes.size());
            if (sorted_boxes.size() > 0) {
                auto adaptive_threshold = iou_threshold;
                int max_out_box =
                    (max_output_boxes_per_class > sorted_boxes.size())
                        ? sorted_boxes.size()
                ...
                while (max_out_box && !sorted_boxes.empty()) {
                    ...
                    for (int idx = static_cast<int>(fb.size()) - 1;
                        ...
                        float iou = intersectionOverUnion(
                            &boxesPtr[currBox.idx * 4],
                            &boxesPtr[fb[idx].box_index * 4], normalized);
                    ...
                    if (box_is_selected) {
                        ...
                        if (currBox.score == origScore) {
                            fb.push_back({currBox.score, batch_idx, class_idx,
                                          currBox.idx});
                            continue;
                        }
                        if (currBox.score > score_threshold) {
            ...
            size_t offset =
                batch_idx * num_classes * max_output_boxes_per_class +
        ...
        if (class_idx != background_class) {
            ...
            if (sorted_boxes.size() > 0) {
                ...
                for (size_t box_idx = 1; box_idx < max_out_box; box_idx++) {
                    ...
                    if (box_is_selected) {
                        filtBoxes[offset + io_selection_size] = filteredBoxes(
                            sorted_boxes[box_idx].first, batch_idx, class_idx,
