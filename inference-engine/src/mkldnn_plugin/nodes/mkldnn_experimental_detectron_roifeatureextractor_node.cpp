...
void reorder_rois(const float* rois,
                  const int* ids,
                  int* mapping,
                  const int rois_num,
                  float* reordered_rois,
...
bool MKLDNNExperimentalDetectronROIFeatureExtractorNode::isSupportedOperation(
    ...
    try {
        ...
        const auto roiFeatureExtractor =
            std::dynamic_pointer_cast<const ngraph::opset6::ExperimentalDetectronROIFeatureExtractor>(op);
...
void MKLDNNExperimentalDetectronROIFeatureExtractorNode::execute(mkldnn::stream strm) {
    ...
    auto* output_rois_features =
        reinterpret_cast<float*>(getChildEdgesAtPort(OUTPUT_ROI_FEATURES)[0]->getMemoryPtr()->GetPtr());
    ...
    for (int i = 0; i < levels_num; ++i) {
        ...
        if (level_rois_num > 0) {
            auto* featuremap =
                reinterpret_cast<const float*>(getParentEdgeAt(INPUT_FEATURES_START + i)->getMemoryPtr()->GetPtr());
