...
public:
        ...
        try {
            ...
            if (noTrans) {
                addConfig(layer, {DataConfigurator(ConfLayout::BLK16, supportedPrecision),
                          ...
                          {DataConfigurator(ConfLayout::BLK16, supportedPrecision)});

                addConfig(layer, {DataConfigurator(ConfLayout::BLK8, supportedPrecision),
                                  DataConfigurator(ConfLayout::PLN, Precision::FP32)},
                          {DataConfigurator(ConfLayout::BLK8, supportedPrecision)});

            } else {
                addConfig(layer, {DataConfigurator(ConfLayout::BLK16, supportedPrecision),
                                  DataConfigurator(ConfLayout::PLN, Precision::FP32),
                                  DataConfigurator(ConfLayout::PLN)}, {DataConfigurator(ConfLayout::BLK16, supportedPrecision)});

                addConfig(layer, {DataConfigurator(ConfLayout::BLK8, supportedPrecision),
                                  DataConfigurator(ConfLayout::PLN, Precision::FP32),
                                  DataConfigurator(ConfLayout::PLN)}, {DataConfigurator(ConfLayout::BLK8, supportedPrecision)});
            }
    ...
    StatusCode executeSpecified(std::vector<Blob::Ptr>& inputs, std::vector<Blob::Ptr>& outputs, ResponseDesc *resp) {
        ...
        const auto hInputStride = srcDesc.getBlockingDesc().getStrides()[hInStrIndex];
        const auto wInputStride = srcDesc.getBlockingDesc().getStrides()[wInStrIndex];
        int hOutputStride = dstDesc.getBlockingDesc().getStrides()[hOutStrIndex];
        int wOutputStride = dstDesc.getBlockingDesc().getStrides()[wOutStrIndex];
                ...
                for (int c = 0; c < nc; c++) {
                    for (int h = 0; h < nh; h++) {
                        for (int w = 0; w < nw; w++) {
                            ...
                            int binOffsetOutput;
                            int binOffsetInput;
                            int inputBlockResidual = 0;
                            int outputBlockResidual = 0;
                            if (outFmt == Layout::NHWC) {
                                inputBlockResidual = 0;
                                outputBlockResidual = 0;
                                binOffsetInput = roiBatchInd * channels * height * width + gc;
                                binOffsetOutput = n * nc * nh * nw + c;
                            } else {
                                inputBlockResidual = (inFmt == Layout::NCHW ? 0 : gc % inBlockSize);
                                outputBlockResidual = (outFmt == Layout::NCHW ? 0 : c % inBlockSize);
                                binOffsetInput = (roiBatchInd * inputChannelsPadding + inputBlockIdx) * height * width;
                                binOffsetOutput = (n * outputChannelsPadding + outputBlockIdx) * nh * nw;
                            }
                            ...
                            if (binArea) {
                                ...
                                for (int hh = hStart * hInputStride; hh < heightIndexBound; hh += hInputStride) {
                                    for (int ww = wStart * wInputStride; ww < weightIndexBound; ww += wInputStride) {
                                        outSum += srcData[binOffsetInput + hh + ww + inputBlockResidual];
                                    }
                                }
                ...
                for (int c = 0; c < nc; c++) {
                    for (int h = 0; h < nh; h++) {
                        for (int w = 0; w < nw; w++) {
                            ...
                            const int binOffsetOutput = (n * outputChannelsPadding + outputBlockIdx) * binCount;
                            ...
                            for (size_t binY = 0; binY < spatialBinsY; binY++) {
                                for (size_t binX = 0; binX < spatialBinsX; binX++) {
                                    ...
                                    const int blockResidual = gc % inBlockSize;
                                    size_t srcIdx = (roiBatchInd * channels + (gc / inBlockSize) * inBlockSize) * height * width;
                                    ...
                                    if (!(inY < 0 || inY > height - 1 || inX < 0 || inX > width - 1)) {
                                        ...
                                        auto topLeftIndex = (topYIndex * width + leftXIndex) * inBlockSize + blockResidual;
                                        auto topRightIndex = (topYIndex * width + rightXIndex) * inBlockSize + blockResidual;
                                        auto bottomLeftIndex = (bottomYIndex * width + leftXIndex) * inBlockSize + blockResidual;
                                        auto bottomRightIndex = (bottomYIndex * width + rightXIndex) * inBlockSize + blockResidual;
    ...
    inline float bilinearInterp(const inputType* data_, const float x_, const float y_, const int width_,
        ...
        int y2 = static_cast<int>(std::ceil(y_));
        float distX = x_ - x1;
        float distY = y_ - y1;
