...
struct jit_uni_bin_conv_kernel_f32 : public jit_uni_bin_conv_kernel, public jit_generator {
    ...
    explicit jit_uni_bin_conv_kernel_f32(jit_bin_conv_params jcp, jit_dw_conv_params jcp_dw_conv, const mkldnn_primitive_attr &attr) :
        ...
        L(main_loop_label); {
            ...
            add(reg_kernel_base, jcp.oc_block * jcp.nb_ic * jcp.kh * jcp.kw * MKLDNNPlugin::div_up(jcp.ic_block, nbits) * jcp.typesize_in);

            if (jcp.with_dw_conv) {
                add(reg_output_base, jcp.oc_block * jcp_dw_conv_.kh * jcp.ow * jcp.typesize_out);
            } else {
                if (jcp.with_binarization)
                    add(reg_output_base, MKLDNNPlugin::div_up(jcp.oc_block, nbits) * jcp.typesize_out);
                else
...
private:
    ...
    Vmm vmm_scale = Vmm(isa == cpu::avx512_common ? 30 : 14);
    Vmm vmm_shift = Vmm(0);
    Vmm vmm_sum = Vmm(isa == cpu::avx512_common ? 26 : 10);
    Vmm vmm_lookup = Vmm(isa == cpu::avx512_common ? 28 : 12);
    Vmm vmm_mask = Vmm(isa == cpu::avx512_common ? 29 : 13);
    Vmm vmm_one_u8 = Vmm(isa == cpu::avx512_common ? 30 : 14);
    Vmm vmm_one_s16 = Vmm(isa == cpu::avx512_common ? 31 : 15);
    Ymm ymm_tmp = Ymm(isa == cpu::avx512_common ? 26 : 10);
    Vmm vmm_tmp = Vmm(isa == cpu::avx512_common ? 26 : 10);
    Vmm vmm_tmp1 = Vmm(isa == cpu::avx512_common ? 27 : 11);
    Vmm vmm_src = Vmm(0);
    Vmm vmm_tmp2 = Vmm(isa == cpu::avx512_common ? 25 : 9);
    Vmm vmm_thr = Vmm(isa == cpu::avx512_common ? 26 : 10);
    Vmm vmm_out_mask = Vmm(isa == cpu::avx512_common ? 30 : 14);
    ...
    void apply_filter(int ur_w, int pad_l, int pad_r, int oc_blocks, int oc_step, int ic_blocks, bool last_icb, bool h_padded) {
        ...
        int repeats = isa == cpu::sse42 && oc_step > (oc_blk / 2) ? 2 : 1;
        int nbits = 8;
                ...
                for (int jj = _start; jj < _end; jj++) {
                    ...
                    for (int r = 0; r < repeats; r++) {
                        for (int ii = 0; ii < oc_blocks; ii++) {
                            int ker_off = (ifm2 * kw * MKLDNNPlugin::div_up(ic_blk, nbits) * oc_blk
    ...
    void oh_step_unroll_kw(int ur_w, int pad_l, int pad_r, int oc_blocks, int oc_step, bool h_padded) {
        ...
        L(icb_main_loop);
        {
            ...
            add(aux1_reg_kernel, kw * inp_mult * out_mult * jcp_.typesize_in);
            sub(reg_icb_iter, 1);
    ...
    void kh_loop(int ur_w, int pad_l, int pad_r, int oc_blocks, int oc_step) {
        ...
        if (!jcp_.exclude_pad) {
            ...
            L(t_overflow_label); {
                ...
                add(aux_reg_kernel, jcp_.typesize_in * kw * jcp_.oc_block * jcp_.nb_ic * MKLDNNPlugin::div_up(jcp_.ic_block, nbits));
                dec(reg_overflow);
                cmp(reg_overflow, 0);
    ...
    void width_blk_step(int ur_w, int pad_l, int pad_r, int oc_blocks, int oc_step) {
        ...
        if (isa == cpu::avx512_common && oc_step != jcp_.oc_block) {
        ...
        const auto &p = attr_.post_ops_;
        for (int r = 0; r < repeats; r++) {
            int tail_size = isa == cpu::sse42 ? nstl::min(jcp_.oc_block / 2, oc_step - r * jcp_.oc_block / 2) : oc_step;
            ...
            int end_idx = jcp_.with_dw_conv ? p.find(primitive_kind::convolution) : p.len_;
            for (int i = 0; i < end_idx; i++) {
            ...
            for (int ii = 0; ii < oc_blocks; ii++) {
                for (int jj = 0; jj < ur_w; jj++) {
                    for (int r = 0; r < repeats; r++) {
                        ...
                        if (r == repeats - 1) {
                            if (isa == cpu::avx512_common && oc_step > nbits) {
...
void MKLDNNBinaryConvolutionNode::initSupportedPrimitiveDescriptors() {
    ...
    if (implType != impl_desc_type::ref) {
        ...
        auto weiFormat = implType == impl_desc_type::jit_avx512 ? memory::format::OhIw16o32i : memory::format::OhIw8o32i;

        config.inConfs[0].desc = MKLDNNMemoryDesc(getParentEdgeAt(0)->getDims(), memory::bin, memory::format::nhwc);
        config.inConfs[1].desc = MKLDNNMemoryDesc(getParentEdgeAt(1)->getDims(), memory::bin, weiFormat);
        config.outConfs[0].desc = MKLDNNMemoryDesc(getChildEdgeAt(0)->getDims(), outputDataType, memory::format::nhwc);
        if (withSum) {
            config.inConfs.push_back(config.outConfs[0]);
            config.outConfs[0].inPlace = 2;
        }
        supportedPrimitiveDescriptors.push_back({config, implType, memory::format::nhwc});
    } else {
        ...
        config.inConfs[0].desc = MKLDNNMemoryDesc(getParentEdgeAt(0)->getDims(), memory::bin, memory::format::nhwc);
        config.inConfs[1].desc = MKLDNNMemoryDesc(getParentEdgeAt(1)->getDims(), memory::bin, weiFormat);
        config.outConfs[0].desc = MKLDNNMemoryDesc(getChildEdgeAt(0)->getDims(), memory::f32, memory::format::nhwc);
        supportedPrimitiveDescriptors.push_back({config, implType, memory::format::nhwc});
    }
}
...
void MKLDNNBinaryConvolutionNode::createPrimitive() {
    ...
    if (implType == impl_desc_type::jit_avx512) {
        bin_conv_kernel.reset(new jit_uni_bin_conv_kernel_f32<cpu::avx512_common>(jcp, jcp_dw_conv, *attr.get()));
    } else if (implType == impl_desc_type::jit_avx2) {
        bin_conv_kernel.reset(new jit_uni_bin_conv_kernel_f32<cpu::avx2>(jcp, jcp_dw_conv, *attr.get()));
    } else if (implType == impl_desc_type::sse42) {
        bin_conv_kernel.reset(new jit_uni_bin_conv_kernel_f32<cpu::sse42>(jcp, jcp_dw_conv, *attr.get()));
    }
}
...
void MKLDNNBinaryConvolutionNode::execute(mkldnn::stream strm) {
    ...
    auto weights = reinterpret_cast<const uint8_t *>(weightsMemory->GetData()) +
                   weightsMemory->GetDescriptor().data.layout_desc.blocking.offset_padding *
                   MKLDNNExtensionUtils::sizeOfDataType(mkldnn::memory::data_type(weightsMemory->GetDescriptor().data.data_type));
