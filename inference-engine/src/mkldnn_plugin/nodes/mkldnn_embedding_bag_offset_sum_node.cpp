...
bool MKLDNNEmbeddingBagOffsetSumNode::isSupportedOperation(const std::shared_ptr<const ngraph::Node>& op,
...
MKLDNNEmbeddingBagOffsetSumNode::MKLDNNEmbeddingBagOffsetSumNode(const std::shared_ptr<ngraph::Node>& op,
                                                                 const mkldnn::engine& eng,
                                                                 MKLDNNWeightsSharing::Ptr& cache)
    : MKLDNNNode(op, eng, cache),
      MKLDNNEmbeddingBagSumNode(op, 3lu, 1lu, 4lu, 3lu) {
...
void MKLDNNEmbeddingBagOffsetSumNode::initSupportedPrimitiveDescriptors() {
    ...
    std::vector<PortConfigurator> inDataConfigurators(
        {{LayoutType::ncsp, inDataPrecision}, {LayoutType::ncsp, Precision::I32}, {LayoutType::ncsp, Precision::I32}});
...
void MKLDNNEmbeddingBagOffsetSumNode::getIndices(int embIndex,
                                                 const int*& indices,
                                                 size_t& size,
                                                 int& weightsIdx,
                                                 bool& withWeight) {
