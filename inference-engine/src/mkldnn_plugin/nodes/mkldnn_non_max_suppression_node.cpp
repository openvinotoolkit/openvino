...
bool MKLDNNNonMaxSuppressionNode::isSupportedOperation(const std::shared_ptr<const ngraph::Node>& op,
    ...
    try {
        ...
        if (!one_of(op->get_type_info(),
                    NonMaxSuppressionV5::get_type_info_static(),
        ...
        if (const auto nms5 = std::dynamic_pointer_cast<const NonMaxSuppressionV5>(op)) {
            ...
            if (!one_of(boxEncoding,
                        NonMaxSuppressionV5::BoxEncodingType::CENTER,
                        NonMaxSuppressionV5::BoxEncodingType::CORNER)) {
...
MKLDNNNonMaxSuppressionNode::MKLDNNNonMaxSuppressionNode(const std::shared_ptr<ngraph::Node>& op,
    ...
    } else if (const auto nmsIe =
                   std::dynamic_pointer_cast<const ngraph::op::internal::NonMaxSuppressionIEInternal>(op)) {
    ...
    if (valid_outputs_shape.getDims()[0] != 1)
        IE_THROW() << errorPrefix
                   << "has unsupported 'valid_outputs' output 1st dimension size: " << valid_outputs_shape.getDims()[1];
}
...
void MKLDNNNonMaxSuppressionNode::initSupportedPrimitiveDescriptors() {
    ...
    checkPrecision(getOriginalOutputPrecisionAtPort(NMS_VALIDOUTPUTS),
                   supportedIntOutputPrecision,
                   "valid_outputs",
                   outType);

    const std::vector<Precision> supportedPrecision = {Precision::I16,
                                                       Precision::U8,
                                                       Precision::I8,
                                                       Precision::U16,
                                                       Precision::I32,
    ...
    if (inputShapes.size() > NMS_MAXOUTPUTBOXESPERCLASS)
        check1DInput(getInputShapeAtPort(NMS_MAXOUTPUTBOXESPERCLASS),
                     supportedPrecision,
    ...
    if (inputShapes.size() > NMS_SCORETHRESHOLD)
        check1DInput(getInputShapeAtPort(NMS_SCORETHRESHOLD),
    ...
    checkOutput(getOutputShapeAtPort(NMS_SELECTEDINDICES),
                supportedIntOutputPrecision,
                "selected_indices",
                NMS_SELECTEDINDICES);
    checkOutput(getOutputShapeAtPort(NMS_SELECTEDSCORES),
                supportedFloatPrecision,
                "selected_scores",
                NMS_SELECTEDSCORES);
...
void MKLDNNNonMaxSuppressionNode::execute(mkldnn::stream strm) {
    ...
    if (inputShapes.size() > NMS_MAXOUTPUTBOXESPERCLASS) {
        max_output_boxes_per_class =
            reinterpret_cast<int*>(getParentEdgeAt(NMS_MAXOUTPUTBOXESPERCLASS)->getMemoryPtr()->GetPtr())[0];
    }
...
void MKLDNNNonMaxSuppressionNode::nmsWithSoftSigma(const float* boxes,
                                                   const float* scores,
...
void MKLDNNNonMaxSuppressionNode::nmsWithoutSoftSigma(const float* boxes,
                                                      const float* scores,
                                                      const VectorDims& boxesStrides,
                                                      const VectorDims& scoresStrides,
                                                      std::vector<filteredBoxes>& filtBoxes) {
            ...
            for (size_t box_idx = 1; (box_idx < sorted_boxes.size()) && (io_selection_size < max_out_box); box_idx++) {
                ...
                if (box_is_selected) {
                    filtBoxes[offset + io_selection_size] =
                        filteredBoxes(sorted_boxes[box_idx].first, batch_idx, class_idx, sorted_boxes[box_idx].second);
                    io_selection_size++;
                }
            }
        }
...
void MKLDNNNonMaxSuppressionNode::checkPrecision(const Precision& prec,
...
void MKLDNNNonMaxSuppressionNode::check1DInput(const Shape& shape,
                                               const std::vector<Precision>& precList,
                                               const std::string& name,
                                               const size_t port) {
    ...
    if (shape.getRank() == 1)
        if (shape.getDims()[0] != 1)
            IE_THROW() << errorPrefix << "has unsupported '" << name
                       << "' input 1st dimension size: " << MemoryDescUtils::dim2str(shape.getDims()[0]);
}

void MKLDNNNonMaxSuppressionNode::checkOutput(const Shape& shape,
                                              const std::vector<Precision>& precList,
                                              const std::string& name,
                                              const size_t port) {
    ...
    if (shape.getDims()[1] != 3)
        IE_THROW() << errorPrefix << "has unsupported '" << name
                   << "' output 2nd dimension size: " << MemoryDescUtils::dim2str(shape.getDims()[1]);
}
