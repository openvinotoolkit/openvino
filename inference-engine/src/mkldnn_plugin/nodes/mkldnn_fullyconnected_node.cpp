...
bool MKLDNNFullyConnectedNode::isSupportedOperation(const std::shared_ptr<const ngraph::Node>& op,
    ...
    try {
        ...
        if (fc->get_input_size() == 3 && std::dynamic_pointer_cast<const ngraph::opset1::Constant>(
                                             fc->get_input_node_shared_ptr(BIAS_ID)) == nullptr) {
            errorMessage = "Only Constant operation on 'bias' input is supported";
            return false;
        }
        if (!one_of(fc->get_input_shape(DATA_ID).size(), 2, 3, 4)) {
            errorMessage =
                "Doesn't support 'data' input with rank: " + std::to_string(fc->get_input_shape(DATA_ID).size());
            return false;
        }
    } catch (...) {
        return false;
...
MKLDNNFullyConnectedNode::MKLDNNFullyConnectedNode(const std::shared_ptr<ngraph::Node>& op,
...
void MKLDNNFullyConnectedNode::getSupportedDescriptors() {
    ...
    for (auto format : getAvailableFormatsForDims(getInputShapeAtPort(0))) {
        auto in_candidate =
            mkldnn::memory::desc(MKLDNNExtensionUtils::convertToDnnlDims(inDims), inputDataType, format);
        auto out_candidate = mkldnn::memory::desc(MKLDNNExtensionUtils::convertToDnnlDims(outDims),
                                                  outputDataType,
...
void MKLDNNFullyConnectedNode::createPrimitive() {
    ...
    if (withBiases)
        primArgs = {{DNNL_ARG_SRC, src},
                    {DNNL_ARG_WEIGHTS, getParentEdgeAt(WEIGHTS_ID)->getMemory().GetPrimitive()},
                    {DNNL_ARG_BIAS, getParentEdgeAt(BIAS_ID)->getMemory().GetPrimitive()},
                    {DNNL_ARG_DST, dst}};
    else
        primArgs = {{DNNL_ARG_SRC, src},
                    {DNNL_ARG_WEIGHTS, getParentEdgeAt(WEIGHTS_ID)->getMemory().GetPrimitive()},
...
const std::vector<impl_desc_type>& MKLDNNFullyConnectedNode::getPrimitivesPriority() {
    std::vector<impl_desc_type> priorities = {
        impl_desc_type::unknown,       impl_desc_type::gemm_blas,      impl_desc_type::gemm_avx512,
        impl_desc_type::gemm_avx2,     impl_desc_type::gemm_avx,       impl_desc_type::gemm_sse42,
        impl_desc_type::gemm_any,      impl_desc_type::gemm,           impl_desc_type::jit_gemm,
...
void MKLDNNFullyConnectedNode::createDescriptorInternal(const mkldnn::memory::desc& inputDesc,
    ...
    } else if (in_candidate.data_type() == mkldnn::memory::data_type::u8 ||
               in_candidate.data_type() == mkldnn::memory::data_type::s8) {
    ...
    if (in_candidate.dims().size() == 3) {
        ...
        in_candidate = mkldnn::memory::desc(normalizedInDims,
                                            in_candidate.data_type(),
                                            MKLDNNExtensionUtils::GetPlainFormatByRank(normalizedInDims.size()));
        out_candidate = mkldnn::memory::desc(normalizedOutDims,
                                             out_candidate.data_type(),
                                             MKLDNNExtensionUtils::GetPlainFormatByRank(normalizedOutDims.size()));
    }

    mkldnn::memory::desc wgh_candidate(MKLDNNExtensionUtils::convertToDnnlDims(weightsDims),
                                       wdt,
                                       mkldnn::memory::format_tag::any);

    if (withBiases) {
        mkldnn::memory::desc bias_candidate(
            MKLDNNExtensionUtils::convertToDnnlDims(inputShapes[BIAS_ID].getStaticDims()),
            bdt,
            mkldnn::memory::format_tag::any);
        MKLDNNDescriptor desc(
            std::shared_ptr<inner_product_forward::desc>(new inner_product_forward::desc(prop_kind::forward_scoring,
                                                                                         in_candidate,
                                                                                         wgh_candidate,
                                                                                         bias_candidate,
                                                                                         out_candidate)));
        ...
        MKLDNNDescriptor desc(std::shared_ptr<inner_product_forward::desc>(
            new inner_product_forward::desc(prop_kind::forward_scoring, in_candidate, wgh_candidate, out_candidate)));
        descs.push_back(desc);
    }
}
...
std::shared_ptr<MemoryDesc> MKLDNNFullyConnectedNode::getSrcMemDesc(mkldnn::primitive_desc_iterator& primitive_desc_it,
                                                                    size_t idx) {
...
std::shared_ptr<MemoryDesc> MKLDNNFullyConnectedNode::getDstMemDesc(mkldnn::primitive_desc_iterator& primitive_desc_it,
                                                                    size_t idx) {
