...
const uint32_t jit_has_subnormals_base::mask_data[8] =
    {0xFF << 23, 0xFF << 23, 0xFF << 23, 0xFF << 23, 0xFF << 23, 0xFF << 23, 0xFF << 23, 0xFF << 23};
...
MKLDNNInputNode::MKLDNNInputNode(const std::shared_ptr<ngraph::Node>& op,
    ...
    if (!one_of(op->get_type_info(),
        ...
        IE_THROW(NotImplemented) << "CPU Input node doesn't support ngraph operation " << op->get_type_name()
...
void MKLDNNInputNode::cloneBlobIfRequired() {
    ...
    auto isWA = [&, this]() {
        ...
        for (auto const output : outputs) {
            ...
            if (!node || TypeFromName(node->get_type_name()) != Type::FullyConnected)
                continue;
...
MKLDNNInputNode::MKLDNNInputNode(const Shape& shape,
...
void MKLDNNInputNode::createPrimitive() {
    for (size_t i = 0; i < getChildEdges().size(); i++) {
        ...
        if (!dstMemPtr || !dstMemPtr->GetPrimitivePtr())
            IE_THROW() << "Destination memory didn't allocate for node " << getName() << " to node "
                       << getChildEdgeAt(i)->getChild()->getName() << ".";
    }
    for (size_t i = 0; i < getParentEdges().size(); i++) {
        auto& srcMemPtr = getParentEdgeAt(i)->getMemoryPtr();
        if (!srcMemPtr || !srcMemPtr->GetPrimitivePtr())
            IE_THROW() << "Destination memory didn't allocate for node " << getName() << " from node "
