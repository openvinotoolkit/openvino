...
static mkldnn::algorithm ie2dnnl(const std::shared_ptr<const ngraph::Node>& op) {
    if (one_of(op->get_type_info(), ngraph::op::v3::GRUCell::type_info, ngraph::op::v5::GRUSequence::type_info)) {
...
bool MKLDNNRNN::isSupportedOperation(const std::shared_ptr<const ngraph::Node>& op,
...
MKLDNNRNN::MKLDNNRNN(const std::shared_ptr<ngraph::Node>& op,
...
void MKLDNNRNN::initCell(const std::shared_ptr<ngraph::Node>& op) {
    ...
    if (in_data_dims != D_shape || in_h_state_dims != S_shape || out_h_state_dims != S_shape)
        IE_THROW() << "Incorrect shape of input/output ports for layer " << getName();
    ...
    if (S == 2) {
        ...
        if (in_c_state_dims != S_shape || out_c_state_dims != S_shape)
            IE_THROW() << "Incorrect shape of input/output ports for layer " << getName();
    }
}
...
void MKLDNNRNN::fillCellDesc() {
    ...
    w_data_d = std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{L, D, DC, G, SC}),
                                                       dataType,
                                                       memory::format_tag::ldigo);
    w_state_d = std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{L, D, SC, G, SC}),
                                                        dataType,
                                                        memory::format_tag::ldigo);

    // Add 5th input
    w_bias_d = std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{L, D, Gb, SC}),
    ...
    // Expected shapes
    Shape D_shape(VectorDims{N, DC}), S_shape(VectorDims{N, SC}), WShape(VectorDims{SC * G, DC}),
        RShape(VectorDims{SC * G, SC}), BShape(VectorDims{SC * Gb});
    ...
    if (haveCellState(cell_type)) {
        in_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(S_shape, memory::data_type::f32, memory::format_tag::nc));
        out_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(S_shape, memory::data_type::f32, memory::format_tag::nc));
    }
    if (one_of(cell_type,
               mkldnn::algorithm::vanilla_rnn,
               mkldnn::algorithm::vanilla_gru,
               mkldnn::algorithm::lbr_gru,
               mkldnn::algorithm::vanilla_lstm)) {
        in_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(WShape, memory::data_type::f32, memory::format_tag::nc));
        in_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(RShape, memory::data_type::f32, memory::format_tag::nc));
        in_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(BShape, memory::data_type::f32, memory::format_tag::x));
    }
...
void MKLDNNRNN::fillSeqDesc() {
    ...
    if (nativeOrder)
        in_candidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(inputShapes[RNNInOutKind::Layer],
                                                                          dataType,
                                                                          memory::format_tag::tnc));
    else if (N == 1)
        // WA to avoid reorder before sequence for some models
        in_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N, T, DC}), dataType, memory::format_tag::tnc));
    else
        in_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N, T, DC}), dataType, memory::format_tag::ntc));

    // initial hidden state
    // WA to avoid reorder before
    if (D == 1)
        in_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N, D, SC}), dataType, memory::format_tag::tnc));
    else
        in_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N, D, SC}), dataType, memory::format_tag::ntc));

    // initial cell state
    if (haveCellState(cell_type)) {
        if (D == 1)
            in_candidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N, D, SC}),
                                                                              memory::data_type::f32,
                                                                              memory::format_tag::tnc));
        else
            in_candidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N, D, SC}),
    ...
    in_candidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N}),
                                                                      memory::data_type::s32,
                                                                      memory::format_tag::x));  // sequence lengths
    in_candidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{D, G * SC, DC}),
                                                                      memory::data_type::f32,
                                                                      memory::format_tag::ntc));  // W
    in_candidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{D, G * SC, SC}),
                                                                      memory::data_type::f32,
                                                                      memory::format_tag::ntc));  // R
    in_candidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{D, Gb * SC}),
                                                                      memory::data_type::f32,
                                                                      memory::format_tag::nc));  // B
    ...
    if (nativeOrder) {
        ...
        out_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N, T, SC}), dataType, memory::format_tag::tnc));
    } else {
        out_candidate.emplace_back(
            std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N, T, SC}), dataType, memory::format_tag::ntc));
    }
    ...
    if (haveCellState(cell_type)) {
        if (D == 1)
            out_candidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N, D, SC}),
                                                                               memory::data_type::f32,
                                                                               memory::format_tag::tnc));
        else
            out_candidate.emplace_back(std::make_shared<DnnlBlockedMemoryDesc>(Shape(VectorDims{N, D, SC}),
...
void MKLDNNRNN::fillWeights(const int* gate_map, const size_t wIdx, const size_t rIdx) {
    ...
    if (!verifyWeightsPrecision(runtimePrecision, weightPrec) && runtimePrecision != Precision::BF16 &&
        weightPrec != Precision::FP32) {
        IE_THROW() << "Doesn't support combination of weights precision: " << weightPrec
                   << " and runtime precision: " << runtimePrecision;
    }
...
void MKLDNNRNN::createDescriptor(const std::vector<MemoryDescPtr>& inputDesc,
    ...
    case mkldnn::algorithm::vanilla_rnn: {
        MKLDNNDescriptor desc(std::shared_ptr<vanilla_rnn_forward::desc>(
            new vanilla_rnn_forward::desc(prop_kind::forward_scoring,
                                          cell_act,
                                          direction,
    ...
    case mkldnn::algorithm::vanilla_gru: {
        MKLDNNDescriptor desc(std::shared_ptr<gru_forward::desc>(
            new gru_forward::desc(prop_kind::forward_scoring,
                                  direction,
                                  /* In Data       */ in_data_d[RNNInOutKind::Layer].getDnnlDesc(),
                                  /* In State      */ in_data_d[RNNInOutKind::HiddenState].getDnnlDesc(),
                                  /* Weights data  */ w_data_d->getDnnlDesc(),
    ...
    case mkldnn::algorithm::lbr_gru: {
        MKLDNNDescriptor desc(std::shared_ptr<lbr_gru_forward::desc>(
            new lbr_gru_forward::desc(prop_kind::forward_scoring,
                                      direction,
                                      /* In Data       */ in_data_d[RNNInOutKind::Layer].getDnnlDesc(),
                                      /* In State      */ in_data_d[RNNInOutKind::HiddenState].getDnnlDesc(),
                                      /* Weights data  */ w_data_d->getDnnlDesc(),
    ...
    case mkldnn::algorithm::vanilla_lstm: {
        MKLDNNDescriptor desc(std::shared_ptr<lstm_forward::desc>(
            new lstm_forward::desc(prop_kind::forward_scoring,
                                   direction,
                                   /* In Data       */ in_data_d[RNNInOutKind::Layer].getDnnlDesc(),
                                   /* In State      */ in_data_d[RNNInOutKind::HiddenState].getDnnlDesc(),
