...
public:
    PortIteratorHelper(const MKLDNNMemoryPtr& from,
                       const MKLDNNMemoryPtr& to,
                       bool sliced_src,
...
int getNumIteration(const std::shared_ptr<const ngraph::Node>& op,
                    const std::vector<PortMap>& inputPortMap,
                    const std::vector<PortMap>& outputPortMap) {
...
bool MKLDNNTensorIteratorNode::isSupportedOperation(const std::shared_ptr<const ngraph::Node>& op,
    ...
    try {
        ...
        if (!one_of(op->get_type_info(), ngraph::op::v0::TensorIterator::type_info, ngraph::op::v5::Loop::type_info)) {
...
MKLDNNTensorIteratorNode::MKLDNNTensorIteratorNode(const std::shared_ptr<ngraph::Node>& op,
                                                   const mkldnn::engine& eng,
