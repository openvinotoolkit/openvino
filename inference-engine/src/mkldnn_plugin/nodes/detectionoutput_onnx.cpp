...
void refine_boxes(const float* boxes, const float* deltas, const float* weights, const float* scores,
    ...
    for (int roi_idx = 0; roi_idx < rois_num; ++roi_idx) {
        ...
        for (int class_idx = 1; class_idx < classes_num; ++class_idx) {
            ...
            x0_new = std::max<float>(0.0f, (std::min)(x0_new, img_W - coordinates_offset));
            y0_new = std::max<float>(0.0f, (std::min)(y0_new, img_H - coordinates_offset));
            x1_new = std::max<float>(0.0f, (std::min)(x1_new, img_W - coordinates_offset));
            y1_new = std::max<float>(0.0f, (std::min)(y1_new, img_H - coordinates_offset));
