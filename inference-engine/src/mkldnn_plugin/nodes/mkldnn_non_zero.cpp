...
bool MKLDNNNonZeroNode::isSupportedOperation(const std::shared_ptr<const ngraph::Node>& op, std::string& errorMessage) noexcept {
    try {
        if (std::dynamic_pointer_cast<const ngraph::opset3::NonZero>(op) == nullptr) {
            errorMessage = "Node is not an instance of NonZero form the operation set v3.";
            return false;
        }
    } catch (...) {
        return false;
...
void MKLDNNNonZeroNode::initSupportedPrimitiveDescriptors() {
    if (!supportedPrimitiveDescriptors.empty())
        return;
    std::vector<Precision> precs = { Precision::FP32, Precision::BF16,  Precision::I32,
