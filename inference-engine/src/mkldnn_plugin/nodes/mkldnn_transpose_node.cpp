...
bool MKLDNNTransposeNode::isSupportedOperation(const std::shared_ptr<const ngraph::Node>& op,
...
MKLDNNTransposeNode::MKLDNNTransposeNode(const std::shared_ptr<ngraph::Node>& op,
...
void MKLDNNTransposeNode::initSupportedPrimitiveDescriptors() {
    ...
    config.inConfs[1].desc =
        creatorsMap.at(LayoutType::ncsp)->createSharedDesc(getOriginalInputPrecisionAtPort(1), getInputShapeAtPort(1));
    ...
    if (getInputShapeAtPort(0).getRank() == 4 || getInputShapeAtPort(0).getRank() == 5) {
        ...
        if (srcDims[1] % 16 == 0) {
            config.inConfs[0].desc =
                creatorsMap.at(LayoutType::nCsp16c)->createSharedDesc(prec, getInputShapeAtPort(0));
            supportedPrimitiveDescriptors.push_back({config, impl_desc_type::unknown});
        }
