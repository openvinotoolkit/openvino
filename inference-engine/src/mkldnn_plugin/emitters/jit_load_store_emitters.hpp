...
struct load_emitter_context : public emitter_context {
    load_emitter_context()
        : src_prc_(Precision::FP32),
          dst_prc_(Precision::FP32),
          load_num_(8),
          offset_byte_(0),
          is_fill_(false),
          fill_value_("zero") {}

    load_emitter_context(Precision src_prc,
                         Precision dst_prc,
                         int load_num,
                         int offset_byte = 0,
                         bool is_fill = false,
...
struct store_emitter_context : public emitter_context {
    store_emitter_context() : src_prc_(Precision::FP32), dst_prc_(Precision::FP32), store_num_(8), offset_byte_(0) {}
...
public:
    jit_load_emitter(mkldnn::impl::cpu::x64::jit_generator* host,
     ...
     * Vmm[out_idxs[0]] as dst_prc. is_fill: when load_num can not fully fit in vector register, whether fill_value
     * should be filled as default values. fill_value: when load_num can not fully fit in vector register, what values
     * should be filled as default values. currently support "zero", "int_one", "float_one", "int32_min", "float_min",
     * "int32_max" and "float_max". supported src_prc and dst_prc pairs are as below(x indicate for support): FP32  I32
     * I16   U16   I8    U8    BF16  --> src_prc FP32   x     x     x     x     x    x     x I32    x     x     x     x
     * x    x     x I16                x U16                      x I8                             x U8 x BF16 x
     *  |
     * \|/
     * dst_prc
     */
...
private:
    ...
    void emit_isa(const Xbyak::Reg64& reg_src,
                  int offset_byte,
                  ...
                  int load_num,
                  bool is_fill = false,
                  std::string fill_value = "zero") const;

    template <typename Vmm>
    void load_bytes(const Vmm& vmm,
...
public:
    jit_store_emitter(mkldnn::impl::cpu::x64::jit_generator* host,
                      mkldnn::impl::cpu::x64::cpu_isa_t host_isa,
                      const MKLDNNNode* node,
                      InferenceEngine::Precision exec_prc = InferenceEngine::Precision::FP32,
...
private:
    template <mkldnn::impl::cpu::x64::cpu_isa_t isa>
    void emit_isa(const int in_vec_idx,
                  InferenceEngine::Precision src_prc,
    ...
    template <typename Vmm>
    void store_dword_to_byte_extension(const Vmm& vmm,
                                       const Xbyak::Reg64& reg,
                                       int offset,
                                       bool is_signed,
    ...
    template <typename Vmm>
    void store_dword_to_word_extension(const Vmm& vmm,
                                       const Xbyak::Reg64& reg,
                                       int offset,
                                       bool is_bf16,
                                       bool is_signed,
