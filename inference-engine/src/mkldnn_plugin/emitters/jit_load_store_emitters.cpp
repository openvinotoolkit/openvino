...
void jit_load_emitter::emit_impl(const std::vector<size_t>& in_idxs,
    ...
    if (host_isa_ == cpu::x64::sse41) {
        emit_isa<cpu::x64::sse41>(Reg64(in_idxs[0]),
                                  load_emitter_context->offset_byte_,
                                  load_emitter_context->src_prc_,
                                  static_cast<int>(out_idxs[0]),
                                  load_emitter_context->dst_prc_,
                                  load_emitter_context->load_num_,
                                  load_emitter_context->is_fill_,
                                  load_emitter_context->fill_value_);
    } else if (host_isa_ == cpu::x64::avx2) {
        emit_isa<cpu::x64::avx2>(Reg64(in_idxs[0]),
                                 load_emitter_context->offset_byte_,
                                 load_emitter_context->src_prc_,
                                 static_cast<int>(out_idxs[0]),
                                 load_emitter_context->dst_prc_,
        ...
        emit_isa<cpu::x64::avx512_common>(Reg64(in_idxs[0]),
                                          load_emitter_context->offset_byte_,
                                          load_emitter_context->src_prc_,
                                          static_cast<int>(out_idxs[0]),
                                          load_emitter_context->dst_prc_,
...
void jit_load_emitter::emit_isa(const Xbyak::Reg64& reg_src,
                                int offset_byte,
                                InferenceEngine::Precision src_prc,
                                const int out_vec_idx,
                                InferenceEngine::Precision dst_prc,
                                int load_num,
                                bool is_fill,
                                std::string fill_value) const {
    bool matched_prc = (dst_prc == src_prc) || (dst_prc == Precision::FP32) || (dst_prc == Precision::I32);
    if (!matched_prc) {
        IE_THROW() << "Load emitter in " << name
    ...
    if (src_prc == dst_prc) {
        ...
        case Precision::I32:
            ...
            break;
        case Precision::I8:
            load_bytes_to_dword_extension<Vmm>(Vmm(out_vec_idx),
                                               reg_src,
                                               offset_byte,
                                               true,
                                               load_num * src_prc.size(),
                                               is_fill,
                                               fill_value);
            break;
        case Precision::U8:
            ...
            break;
        case Precision::I16:
            load_words_to_dword_extension<Vmm>(Vmm(out_vec_idx),
                                               reg_src,
                                               offset_byte,
                                               false,
                                               true,
...
void jit_load_emitter::load_bytes(const Vmm& vmm,
    ...
    if (is_zmm && (load_size != 64) && mayiuse(cpu::x64::avx512_core)) {
        ...
        if (load_size == 64) {
            ...
            case 0:
                break;
            case 1:
                h->uni_vpinsrb(xmm, xmm, addr(start_bytes), 0);
                break;
            case 2:
...
/// STORE ///
jit_store_emitter::jit_store_emitter(jit_generator* host,
                                     cpu_isa_t host_isa,
                                     const MKLDNNNode* node,
                                     Precision exec_prc,
                                     emitter_in_out_map in_out_type)
...
void jit_store_emitter::emit_impl(const std::vector<size_t>& in_idxs,
                                  const std::vector<size_t>& out_idxs,
                                  const std::vector<size_t>& pool_vec_idxs,
                                  const std::vector<size_t>& pool_gpr_idxs,
                                  const emitter_context* emit_context) const {
    ...
    if (host_isa_ == cpu::x64::sse41) {
        emit_isa<cpu::x64::sse41>(static_cast<int>(in_idxs[0]),
                                  store_emitter_context->src_prc_,
                                  Reg64(out_idxs[0]),
                                  store_emitter_context->offset_byte_,
                                  store_emitter_context->dst_prc_,
                                  store_emitter_context->store_num_);
    } else if (host_isa_ == cpu::x64::avx2) {
        emit_isa<cpu::x64::avx2>(static_cast<int>(in_idxs[0]),
                                 store_emitter_context->src_prc_,
                                 Reg64(out_idxs[0]),
                                 store_emitter_context->offset_byte_,
                                 store_emitter_context->dst_prc_,
        ...
        emit_isa<cpu::x64::avx512_common>(static_cast<int>(in_idxs[0]),
                                          store_emitter_context->src_prc_,
                                          Reg64(out_idxs[0]),
                                          store_emitter_context->offset_byte_,
                                          store_emitter_context->dst_prc_,
...
template <mkldnn::impl::cpu::x64::cpu_isa_t isa>
void jit_store_emitter::emit_isa(const int in_vec_idx,
                                 InferenceEngine::Precision src_prc,
    ...
    if (!matched_prc) {
        IE_THROW() << "Store emitter in " << name
    ...
    if (is_zmm && (store_size != 64) && mayiuse(cpu::x64::avx512_core)) {
        ...
        if (store_size == 64) {
            ...
            case 0:
                break;
            case 1:
                h->uni_vpextrb(addr(start_bytes), xmm, 0);
                break;
            case 2:
 ...
 * 1. convert store_num (0 <= store_num <= 16) dwords in the Xmm/Ymm/Zmm to store_num bytes with singed or unsinged
 * saturation.
 * 2. store the packed byte into the memory referenced by ptr[reg + offset] address.
 */
template <typename Vmm>
void jit_store_emitter::store_dword_to_byte_extension(const Vmm& vmm,
                                                      const Xbyak::Reg64& reg,
                                                      int offset,
                                                      bool is_signed,
                                                      int store_num) const {
 ...
 */
template <typename Vmm>
void jit_store_emitter::store_dword_to_word_extension(const Vmm& vmm,
                                                      const Xbyak::Reg64& reg,
                                                      int offset,
                                                      bool is_bf16,
                                                      bool is_signed,
