...
void MKLDNNPlugin::MKLDNNInferRequest::pushInput(const std::string& inputName,
    ...
    if (needConvert) {
        iconv = make_blob_with_precision(inPrec,
                                         InferenceEngine::TensorDesc(inPrec,
        ...
        cpu_convert(srcData,
                    dstData,
                    inputBlob->getTensorDesc().getPrecision(),
                    iconv->getTensorDesc().getPrecision(),
                    iconv->size());
    }
...
void MKLDNNPlugin::MKLDNNInferRequest::PushInputData() {
    for (auto input : _inputs) {
        if (!_networkInputs[input.first]) {
            IE_THROW() << "Input blobs map contains not registered during IInferencePlugin::LoadNetwork blob with name "
                       << input.first;
        }
        ...
        // Currently IE doesn't specify behavior in such scenario, so we assume real layout is equal to the network
        // input.
...
MKLDNNPlugin::MKLDNNInferRequest::GetPerformanceCounts() const {
...
InferenceEngine::Blob::Ptr MKLDNNPlugin::MKLDNNInferRequest::GetBlob(const std::string& name) {
    ...
    if (graph->hasInputWithName(name)) {
        ...
        auto preProcessedInput = std::find_if(std::begin(_networkInputs),
        ...
        if (_outputs.find(name) == _outputs.end()) {
            if (_networkOutputs.find(name) != _networkOutputs.end()) {
                ...
                if (!data) {
                    ...
                    const auto& expectedTensorDesc =
                        isDynamic ? InferenceEngine::TensorDesc(
                                        desc.getPrecision(),
                    ...
                    if (expectedTensorDesc.getPrecision() != tensorDesc.getPrecision()) {
                        IE_THROW(ParameterMismatch)
                            ...
                            << " but expect blobs with different precision: " << tensorDesc.getPrecision()
                            << " for input and " << expectedTensorDesc.getPrecision() << " for output.";
                    }
                    ...
                    if (expectedTensorDesc.getDims() != tensorDesc.getDims()) {
                        IE_THROW(ParameterMismatch) << "Network input and output use the same name: " << name
                                                    << " but expect blobs with different shapes.";
                    }

                    if (tensorDesc.getLayout() != InferenceEngine::Layout::ANY &&
                        expectedTensorDesc.getLayout() != InferenceEngine::Layout::ANY) {
                        if (tensorDesc.getLayout() != expectedTensorDesc.getLayout() &&
                            !(tensorDesc.getLayout() == InferenceEngine::Layout::BLOCKED &&
                              InferenceEngine::TensorDesc(tensorDesc.getPrecision(),
                                                          tensorDesc.getDims(),
                                                          tensorDesc.getBlockingDesc())
                                      .getLayout() == expectedTensorDesc.getLayout())) {
                            IE_THROW(ParameterMismatch)
                                << "Network input and output use the same name: " << name << " but expect blobs"
                ...
                if (!isDynamic && !externalPtr.count(name) &&
                    data->getTensorDesc() == MemoryDescUtils::convertToTensorDesc(desc) &&
                    !graph->getProperty().batchLimit) {
...
void MKLDNNPlugin::MKLDNNInferRequest::SetBlob(const std::string& name, const InferenceEngine::Blob::Ptr& data) {
    ...
    if (foundInput) {
        if (foundInput->getPrecision() != blobDesc.getPrecision()) {
            IE_THROW(ParameterMismatch) << "Failed to set input blob with precision: " << blobDesc.getPrecision()
                                        << ", if CNNNetwork input blob precision is: " << foundInput->getPrecision();
        }
        ...
        if (preProcRequired) {
            ...
            if (blobDesc.getLayout() != InferenceEngine::Layout::ANY &&
                ...
                if (isDynamic &&
                    InferenceEngine::TensorDesc(foundInput->getPrecision(), blobDesc.getDims(), foundInput->getLayout())
                            .getBlockingDesc() != blobDesc.getBlockingDesc())
    ...
    if (foundOutput) {
        ...
        if (foundOutput->getPrecision() != blobDesc.getPrecision()) {
            IE_THROW(ParameterMismatch) << "Failed to set output blob with precision: " << blobDesc.getPrecision()
                                        << ", if CNNNetwork output blob precision is: " << foundOutput->getPrecision();
        }
        ...
        if (blobDesc.getLayout() != InferenceEngine::Layout::ANY &&
            foundOutput->getTensorDesc().getLayout() != InferenceEngine::Layout::ANY) {
            if (isDynamic &&
                InferenceEngine::TensorDesc(foundOutput->getPrecision(), blobDesc.getDims(), foundOutput->getLayout())
                        .getBlockingDesc() != blobDesc.getBlockingDesc())
                IE_THROW(ParameterMismatch) << "Failed to set input blob. Layouts mismatch.";
...
void MKLDNNPlugin::MKLDNNInferRequest::SetBatch(int new_batch) {
    ...
    if (new_batch < 1 || new_batch > graph->getProperty().batchLimit) {
        IE_THROW() << "Invalid dynamic batch size " << new_batch << " for this request.";
    }
