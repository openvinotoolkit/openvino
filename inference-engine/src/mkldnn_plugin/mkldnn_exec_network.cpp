...
using InferenceEngine::details::Quantizer;
...
MKLDNNExecNetwork::MKLDNNExecNetwork(const InferenceEngine::ICNNNetwork &network,
    ...
    } else if (cfg.quantizerMode != Config::QuantizerMode::None) {
        if (Quantizer::isNetworkSupported(*clonedNetwork)) {
            if (cfg.quantizerMode == Config::QuantizerMode::TransformQuantizeOnWeightsFp32) {
                Quantizer quantizer;
                quantizer.transformQuantizeOnWeightsPath(*clonedNetwork, cfg.dumpQuantizedGraphToIr);
            } else {
                const bool transformQuantizeOnDataPath =
                        (cfg.quantizerMode == Config::QuantizerMode::TransformQuantizeOnWeightsAndDataInt8) ||
                        (cfg.quantizerMode == Config::QuantizerMode::TransformQuantizeOnWeightsAndDataFp32);

                const Quantizer::QuantizationPrecision precision =
                        ((cfg.quantizerMode == Config::QuantizerMode::TransformQuantizeOnWeightsInt8) ||
                         (cfg.quantizerMode == Config::QuantizerMode::TransformQuantizeOnWeightsAndDataInt8)) ?
                        Quantizer::QuantizationPrecision::INT8 :
                        Quantizer::QuantizationPrecision::FP32;
                ...
                quantizer.quantize(
                        *clonedNetwork,
                        ...
                        cfg.dumpQuantizedGraphToDot,
                        cfg.dumpQuantizedGraphToIr);
            }
        }
    }
