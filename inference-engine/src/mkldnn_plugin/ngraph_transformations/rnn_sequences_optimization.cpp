...
int64_t getSeqAxis(const std::shared_ptr<ngraph::Node>& sequenceOp) {
    ...
    if (target_inputs.size() == 1) {
        const auto& transpose_before =
            std::dynamic_pointer_cast<ngraph::op::v1::Transpose>(sequenceOp->input_value(0).get_node_shared_ptr());
        const auto& transpose_after =
            std::dynamic_pointer_cast<ngraph::op::v1::Transpose>(target_inputs.begin()->get_node()->shared_from_this());
...
bool transform(const std::shared_ptr<ngraph::Node>& sequenceOp) {
    ...
    if (seqAxis == 0) {
        ngraph::Output<ngraph::Node> in_0 =
            sequenceOp->get_input_source_output(0).get_node_shared_ptr()->get_input_source_output(0);

        auto newInShape =
            ngraph::op::v0::Constant::create(ngraph::element::i32, ngraph::Shape{3}, sequenceOp->get_input_shape(0));
        ...
        auto newOutShape = ngraph::op::v0::Constant::create(ngraph::element::i32,
                                                            ngraph::Shape{4},
                                                            transposeAfter->get_output_shape(0));
...
MKLDNNPlugin::OptimizeLSTMSequenceTransposes::OptimizeLSTMSequenceTransposes() {
    ...
    const auto lstmSeqInputs =
        std::make_shared<ngraph::pattern::op::Or>(ngraph::OutputVector{lstmSequenceNgraph_0, lstmSequenceNgraph_5});
