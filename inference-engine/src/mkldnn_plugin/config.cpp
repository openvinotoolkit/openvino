...
void Config::readProperties(const std::map<std::string, std::string> &prop) {
    for (auto& kvp : prop) {
        ...
        if (key == PluginConfigParams::KEY_CPU_BIND_THREAD) {
            ...
            dumpToDot = val;
        } else if (key.compare(PluginConfigInternalParams::KEY_QUANTIZE_MODE) == 0) {
            if (val == PluginConfigInternalParams::QUANTIZE_MODE_NONE)
                quantizerMode = QuantizerMode::None;
            else if (val == PluginConfigInternalParams::QUANTIZE_MODE_TRANFORM_ON_WEIGHTS_FP32)
                quantizerMode = QuantizerMode::TransformQuantizeOnWeightsFp32;
            else if (val == PluginConfigInternalParams::QUANTIZE_MODE_TRANFORM_ON_WEIGHTS_INT8)
                quantizerMode = QuantizerMode::TransformQuantizeOnWeightsInt8;
            else if (val == PluginConfigInternalParams::QUANTIZE_MODE_TRANFORM_ON_WEIGHTS_AND_DATA_FP32)
                quantizerMode = QuantizerMode::TransformQuantizeOnWeightsAndDataFp32;
            else if (val == PluginConfigInternalParams::QUANTIZE_MODE_TRANFORM_ON_WEIGHTS_AND_DATA_INT8)
                quantizerMode = QuantizerMode::TransformQuantizeOnWeightsAndDataInt8;
            else
                THROW_IE_EXCEPTION << "Wrong value for property key " << PluginConfigInternalParams::KEY_QUANTIZE_MODE;
