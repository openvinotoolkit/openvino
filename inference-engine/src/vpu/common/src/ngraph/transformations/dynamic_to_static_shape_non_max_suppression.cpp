...
void dynamicToStaticNonMaxSuppression(std::shared_ptr<ngraph::Node> node) {
    ...
    dsrIndices->set_friendly_name(nms->output(0).get_node()->shared_from_this()->get_friendly_name() + ".0");
    dsrScores->set_friendly_name(nms->output(1).get_node()->shared_from_this()->get_friendly_name() + ".1");
    ...
    const auto gatherValidOutputs = std::make_shared<ngraph::opset5::Gather>(
            ...
            ngraph::opset5::Constant::create(staticShapeNMS->output(2).get_element_type(), ngraph::Shape{1}, {0}));
    gatherValidOutputs->set_friendly_name(nms->output(2).get_node()->shared_from_this()->get_friendly_name() + ".2");
