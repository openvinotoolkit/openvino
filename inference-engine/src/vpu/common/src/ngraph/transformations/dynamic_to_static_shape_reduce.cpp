...
void dynamicToStaticShapeReduce(std::shared_ptr<ngraph::Node> target) {
    ...
    const auto axes_const_node = ngraph::as_type_ptr<ngraph::opset3::Constant>(target->input_value(1).get_node()->shared_from_this());
    VPU_THROW_UNLESS(axes_const_node,
                     ...
                     target->input_value(1).get_node()->shared_from_this()->get_friendly_name(), target->input_value(1).get_node()->shared_from_this()->get_type_info());
