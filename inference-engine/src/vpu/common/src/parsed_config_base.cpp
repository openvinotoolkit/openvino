...
void ParsedConfigBase::update(
    ...
    for (const auto& entry : config) {
        const auto compileIt = compileOptions.find(entry.first);
        const auto runTimeIt = runTimeOptions.find(entry.first);

        if (compileIt == compileOptions.end() &&
            runTimeIt == runTimeOptions.end()) {
            THROW_IE_EXCEPTION
        ...
        if (mode == ConfigMode::RunTime) {
            if (runTimeIt == runTimeOptions.end()) {
