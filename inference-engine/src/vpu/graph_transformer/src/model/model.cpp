...
Stage getDataConnectionStage(
    ...
    VPU_THROW_UNLESS(child->usage() == DataUsage::Intermediate);

    //
    // Parent can't be Temp or Fake.
    //

    VPU_THROW_UNLESS(parent->usage() != DataUsage::Temp && parent->usage() != DataUsage::Fake);
...
SharedAllocation Model::connectDatasImpl(
    ...
    //
    // Get connections Stage.
    //

    auto connectionStage = getDataConnectionStage(
        parent, child,
        mode, order, offset,
        this);
...
void Model::disconnectDatas(const SharedAllocation& edge) {
    auto parent = edge->parent();
    auto child = edge->child();
...
void Model::runDFS(
    ...
    for (const auto& nextStage : stage->_nextStages | asRange() | reverse()) {
