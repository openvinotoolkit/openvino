...
void PassImpl::runForStage(const Model& model, const Stage& stage) {
    ...
    const auto mergedBlob = make_blob_with_precision(output->desc().toTensorDesc());
    mergedBlob->allocate();
    const auto mergedPtr = mergedBlob->buffer().as<uint8_t*>();
    ...
    const auto constOutput = model->addConstData(
        output->name(),
        output->desc(),
        ieBlobContent(mergedBlob));
