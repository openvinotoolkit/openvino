...
protected:
    size_t getTempBufSize(const SmallVector<DataContent::Ptr, 2>&) const override {
        auto countElem = desc().dim(Dim::W) * desc().dim(Dim::H) * desc().dim(Dim::C);
    ...
    void fillTempBuf(const SmallVector<DataContent::Ptr, 2>&, void* tempBuf) const override {
        ...
        auto imagePixels = desc().dim(Dim::W) * desc().dim(Dim::H);
        auto countElem = desc().dim(Dim::W) * desc().dim(Dim::H) * desc().dim(Dim::C);
...
protected:
    ...
    void fillTempBuf(const SmallVector<DataContent::Ptr, 2>&, void* tempBuf) const override {
        ...
        IE_ASSERT(desc().totalDimSize() == _info.getNumberOfChannels());
