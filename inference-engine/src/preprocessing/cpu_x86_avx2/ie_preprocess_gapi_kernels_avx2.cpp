...
static inline void verticalPass_8U(const uint8_t *src0[], const uint8_t *src1[], uint8_t tmp[],
                     v_int16& b0, v_int16& b1, v_int16& b2, v_int16& b3,
                     v_uint8& shuf_mask, int w, int half_nlanes) {
...
static inline void horizontalPass_U8C1(const short clone[], const short mapsx[], uint8_t tmp[], uint8_t *dst[], int x,
                         v_uint8& val_0, v_uint8& val_1, v_uint8& val_2, v_uint8& val_3,
                         v_uint8& shuf_mask, v_uint8& shuf_mask1, v_uint32& idxs) {
    ...
    val_0 = v_insert64< 0 >(val_0, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 0]]));
    val_0 = v_insert64< 1 >(val_0, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 1]]));
    val_0 = v_insert64< 2 >(val_0, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 2]]));
    val_0 = v_insert64< 3 >(val_0, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 3]]));

     val_1 = v_insert64< 0 >(val_1, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 4]]));
     val_1 = v_insert64< 1 >(val_1, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 5]]));
     val_1 = v_insert64< 2 >(val_1, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 6]]));
     val_1 = v_insert64< 3 >(val_1, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 7]]));

     val_2 = v_insert64< 0 >(val_2, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 8]]));
     val_2 = v_insert64< 1 >(val_2, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 9]]));
     val_2 = v_insert64< 2 >(val_2, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 10]]));
     val_2 = v_insert64< 3 >(val_2, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 11]]));

     val_3 = v_insert64< 0 >(val_3, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 12]]));
     val_3 = v_insert64< 1 >(val_3, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 13]]));
     val_3 = v_insert64< 2 >(val_3, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 14]]));
     val_3 = v_insert64< 3 >(val_3, *reinterpret_cast<int64_t*>(&tmp[4 * mapsx[x + 15]]));
     ...
     v_uint8 q2 = v_shuffle_s8(q0, shuf_mask);
     v_uint8 q3 = v_shuffle_s8(q1, shuf_mask);
     ...
     v_uint8 q8 = v_shuffle_s8(q6, shuf_mask1);
     v_uint8 q9 = v_shuffle_s8(q7, shuf_mask1);

     v_store_low(&dst[0][x], q8);
     v_store_high(&dst[1][x], q8);
     v_store_low(&dst[2][x], q9);
     v_store_high(&dst[3][x], q9);
}
        ...
        if (4 == lpi) {
            ...
            for (int w = 0; w < inSz.width; ) {
                for (; w <= inSz.width - half_nlanes; w += half_nlanes) {
                    verticalPass_8U(src0, src1, tmp, b0, b1, b2, b3, shuf_mask1, w, half_nlanes);
                }
            ...
            for (int x = 0; x < outSz.width; ) {
                for (; x <= outSz.width - half_nlanes; x += half_nlanes) {
                    horizontalPass_U8C1(clone, mapsx, tmp, dst, x,
                                        val_0, val_1, val_2, val_3,
                                        shuf_mask2, shuf_mask3, idxs);
                }
            ...
            for (int l = 0; l < lpi; ++l) {
                ...
                for (int x = 0; x < outSz.width; ) {
                    for (; x <= outSz.width - half_nlanes; x += half_nlanes) {
                        v_int16 a0 = vx_load(&alpha[x]);        // as signed Q1.1.14
                        v_int16 sx = vx_load(&mapsx[x]);        // as integer (int16)
                        v_uint8 t = v_gather_pairs(src, sx);  // 8 pairs of src0 pixels
                        v_int16 t0, t1;
                        v_deinterleave_expand(t, t0, t1);        // tmp pixels as int16
                        v_int16 d = v_mulhrs(t0 - t1, a0) + t1;
                        v_pack_u_store(&dst[l][x], d);
