...
class GNAEltwiseTest : public GNATest<>, public testing::WithParamInterface<GNAEltwiseTestParams>, public EltwiseTestParams {
 ...
 protected:
    ...
    std::shared_ptr<ngraph::Function> buildNgraphModel() {
        auto input1 = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 32});
        const_cast<std::string&>(input1->get_name()) = "input1";

        auto input2 = std::make_shared<ngraph::op::v0::Parameter>(ngraph::element::f32, ngraph::Shape{1, 32});
        const_cast<std::string&>(input2->get_name()) = "input2";

        auto weights = ngraph::op::v0::Constant::create(ngraph::element::f32, ngraph::Shape{ 32, 32 },  {1});
        auto biases = ngraph::op::v0::Constant::create(ngraph::element::f32, ngraph::Shape{32},  {0});
        ...
        if (has_reshape_before_eltwise) {
            auto reshape_pattern = std::make_shared<ngraph::op::v0::Constant>(ngraph::element::i64,
                                                                           ngraph::Shape{2},
        ...
        auto add = std::make_shared<ngraph::op::v0::Add>(FC1, FC2);
