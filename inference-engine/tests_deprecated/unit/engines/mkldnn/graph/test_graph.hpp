...
#define SEARCH_TYPE(_type)                                                                      \
    ...
    void PushInputData(const std::string& name, const InferenceEngine::Blob::Ptr &in, int batch) {
        ...
        if (input != inputNodes.end()) {
            ...
            mkldnn::memory::data_type inPrc = MKLDNNPlugin::MKLDNNExtensionUtils::IEPrecisionToDataType(in->getTensorDesc().getPrecision());
            if (ext_data_ptr != inter_data_ptr)
                input->second->getChildEdgeAt(0)->getMemory().SetData(MKLDNNPlugin::MKLDNNMemory::createMemDesc(outDims, inPrc,
                                                                      MKLDNNPlugin::MKLDNNMemory::GetPlainFormat(outDims)),
                                                                      ext_data_ptr, in->byteSize() / outDims[0] * batch, false);
