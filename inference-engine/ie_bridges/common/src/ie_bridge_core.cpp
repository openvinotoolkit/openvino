...
std::map<std::string, InferenceEngine::Version> InferenceEngineBridge::IECore::getVersions(const std::string &deviceName) {
    return actual.GetVersions(deviceName);
}

std::unique_ptr<InferenceEngineBridge::IEExecNetwork> InferenceEngineBridge::IECore::loadNetwork(IENetwork network,
