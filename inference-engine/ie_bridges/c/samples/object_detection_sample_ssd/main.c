  ...
  if (S_ISDIR(sb.st_mode)) {
    ...
    while (NULL != (ep = readdir(dp))) {
      ...
      if (strcmp(fileName, ".") == 0 || strcmp(fileName, "..") == 0)
        continue;
      char *file_path =
          (char *)calloc(strlen(arg) + strlen(ep->d_name) + 2, sizeof(char));
      memcpy(file_path, arg, strlen(arg));
      memcpy(file_path + strlen(arg), "/", strlen("/"));
      memcpy(file_path + strlen(arg) + strlen("/"), ep->d_name,
             strlen(ep->d_name) + 1);
      ...
      if (file_num == 0) {
        ...
        char **temp =
            (char **)realloc(file_paths, sizeof(char *) * (file_num + 1));
        if (temp) {
          file_paths = temp;
...
int main(int argc, char **argv) {
  ...
  char *input_weight = NULL, *imageInputName = NULL, *imInfoInputName = NULL,
       *output_name = NULL;
  ie_core_t *core = NULL;
  ...
  for (i = 0; i < ver.num_vers; ++i) {
    ...
    printf("         %s version ......... %zu.%zu\n",
           ver.versions[i].description, ver.versions[i].major,
  ...
  if (custom_plugin_cfg_msg &&
      (device_name == "GPU" || device_name == "MYRIAD" ||
       device_name == "HDDL")) {
  ...
  for (i = 0; i < file_num; ++i) {
    ...
    if ((input_width == img.mat_width) && (input_height == img.mat_height)) {
      ...
      printf("%sImage is resized from (%d, %d) to (%zu, %zu)\n", warn,
             img.mat_width, img.mat_height, input_width, input_height);

      if (image_resize(&img, &resized_img, (int)input_width,
                       (int)input_height) == -1) {
  ...
  rectangle_t **boxes =
      (rectangle_t **)calloc(image_num, sizeof(rectangle_t *));
  ...
  for (curProposal = 0; curProposal < maxProposalCount; curProposal++) {
    ...
    int xmin = (int)(detection[curProposal * objectSize + 3] *
                     originalImages[image_id].mat_width);
    int ymin = (int)(detection[curProposal * objectSize + 4] *
                     originalImages[image_id].mat_height);
    int xmax = (int)(detection[curProposal * objectSize + 5] *
                     originalImages[image_id].mat_width);
    int ymax = (int)(detection[curProposal * objectSize + 6] *
                     originalImages[image_id].mat_height);
  ...
  for (batch_id = 0; batch_id < batchSize; ++batch_id) {
    if (object_num[batch_id] > 0) {
      image_add_rectangles(&originalImages[batch_id], boxes[batch_id],
                           classes[batch_id], object_num[batch_id], 2);
    }
    const char *out = "out_";
    char str_num[16] = {0};
    int2str(str_num, batch_id);
    char *img_path = (char *)calloc(
        strlen(out) + strlen(str_num) + strlen(".bmp") + 1, sizeof(char));
    memcpy(img_path, out, strlen(out));
    memcpy(img_path + strlen(out), str_num, strlen(str_num));
    memcpy(img_path + strlen(out) + strlen(str_num), ".bmp",
           strlen(".bmp") + 1);
