...
namespace {
template <typename T>
auto make_vla(std::size_t size) noexcept {
    struct vla_deleter_t {
        void operator()(int *p) const noexcept { _freea(p); };
    };
    return std::unique_ptr<T[], vla_deleter_t>{ static_cast<T *>(
            _malloca(sizeof(T) * size)) };
}
}
#endif  // WIN32
