            ...
            for (int oc = 0; oc < c.oc / c.ng; oc++) {
                for (int oh = 0; oh < c.oh; oh++) {
                    for (int ow = 0; ow < c.ow; ow++) {
                        ...
                        switch (elt_alg) {
                            case eltwise_relu:         dst_data[didx] = relu_fwd(dst_data[didx], elt_alpha);             break;
                            case eltwise_tanh:         dst_data[didx] = tanh_fwd(dst_data[didx]);                        break;
                            case eltwise_elu:          dst_data[didx] = elu_fwd(dst_data[didx], elt_alpha);              break;
                            case eltwise_square:       dst_data[didx] = square_fwd(dst_data[didx]);                      break;
                            case eltwise_abs:          dst_data[didx] = abs_fwd(dst_data[didx]);                         break;
                            case eltwise_sqrt:         dst_data[didx] = sqrt_fwd(dst_data[didx]);                        break;
                            case eltwise_linear:       dst_data[didx] = linear_fwd(dst_data[didx], elt_alpha, elt_beta); break;
                            case eltwise_bounded_relu: dst_data[didx] = bounded_relu_fwd(dst_data[didx], elt_alpha);     break;
                            case eltwise_soft_relu:    dst_data[didx] = soft_relu_fwd(dst_data[didx]);                   break;
                            case eltwise_logistic:     dst_data[didx] = logistic_fwd(dst_data[didx]);                    break;
                            case eltwise_clamp:        dst_data[didx] = clamp_fwd(dst_data[didx], elt_alpha, elt_beta);  break;
