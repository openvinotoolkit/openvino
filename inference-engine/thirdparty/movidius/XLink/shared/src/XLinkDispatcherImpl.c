        ...
        case XLINK_READ_REQ:
        {
            ...
            if (packet){
                ...
                if(packet->packetType == PACKET_SIMPLE) {
                    event->packet = &packet->streamPacket;
                    event->header.size = packet->streamPacket.length;
                } else {
                    event->packet = &packet->inferPacket;
                    event->header.size = packet->inferPacket.streamPacket.length;
                }
...
int releasePacketFromStream(streamDesc_t* stream, uint32_t* releasedSize)
{
    ...
    switch (currPack->packetType) {
        case PACKET_SIMPLE:
        {
            packet = &currPack->streamPacket;
            break;
        }
        case PACKET_INFER:
        {
            inferPacketDesc_t* inferPacket = &currPack->inferPacket;
            packet = &inferPacket->streamPacket;
            break;
        }
        default:
            mvLog(MVLOG_ERROR,"Unexpected packet type %d\n", currPack->packetType);
...
int deserializeSimplePacket(xLinkEvent_t* event) {
    ...
    internalPacket.streamPacket.data = buffer;
    internalPacket.streamPacket.length = event->header.size;
...
int deserializeInferPacket(xLinkEvent_t* event) {
    ...
    void* buffer = NULL;
    streamPacketInternal_t internalPacket = {0};
    inferPacketDesc_t* inferPacket = &internalPacket.inferPacket;
