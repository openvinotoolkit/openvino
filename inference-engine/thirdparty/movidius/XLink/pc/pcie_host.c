...
int pcie_write(HANDLE fd, void * buf, size_t bufSize)
{
    ...
    BOOL ret = WriteFile(dev, buf, bufSize, &bytesWritten, XLINK_PCIE_DATA_TIMEOUT);
...
#else
pcieHostError_t pcie_write(void *fd, void * buf, size_t bufSize)
{
    fd_set wrfds;
    int ret;
...
int pcie_read(HANDLE fd, void * buf, size_t bufSize)
{
    int bytesRead;
    HANDLE dev = fd;
    BOOL ret = ReadFile(dev, buf, bufSize, &bytesRead, XLINK_PCIE_DATA_TIMEOUT);
...
#else
pcieHostError_t pcie_read(void *fd, void *buf, size_t bufSize)
{
    fd_set rdfds;
    int ret;
