...
protected:
    ...
    virtual kernel::kernel_arguments_data get_arguments(typed_primitive_inst<scale_grad_input>& instance, int32_t) const override
    {
        kernel::kernel_arguments_data args;
        args.inputs = { &instance.input_memory(), &instance.scale_input_memory() };
...
namespace {
    struct attach {
        attach() {
            ...
            implementation_map<scale_grad_input>::add(std::make_tuple(engine_types::ocl, data_types::f32, format::yxfb), val_fw);
            implementation_map<scale_grad_input>::add(std::make_tuple(engine_types::ocl, data_types::f16, format::yxfb), val_fw);
            implementation_map<scale_grad_input>::add(std::make_tuple(engine_types::ocl, data_types::f32, format::bfyx), val_fw);
            implementation_map<scale_grad_input>::add(std::make_tuple(engine_types::ocl, data_types::f16, format::bfyx), val_fw);
            implementation_map<scale_grad_input>::add(std::make_tuple(engine_types::ocl, data_types::f32, format::byxf), val_fw);
            implementation_map<scale_grad_input>::add(std::make_tuple(engine_types::ocl, data_types::f16, format::byxf), val_fw);
        }
        ~attach() {}
    };
    attach attach_impl;
