...
// It returns the number of big cores on Windows and Linux.
// For __APPLE__, it return of tbb::this_task_arena::max_concurrency
// The reference of this method is InferenceEngine::getNumberOfCPUCores(true)
int get_num_big_cores() {
    int phys_cores = tbb::this_task_arena::max_concurrency();
#ifndef __APPLE__
    auto core_types = cldnn::custom::info::core_types();
    if (core_types.size() > 1) /*Hybrid CPU*/ {
        phys_cores = cldnn::custom::info::default_concurrency(cldnn::custom::task_arena::constraints{}
