...
void prepare_primitive_fusing::fuse_simple_primitives(program_impl &p) {
    ...
    while (itr != p.get_processing_order().end()) {
        ...
        auto bfyx_f16_conv = [this](convolution_node& node) -> bool {
            // Since reorder inputs is called after this pass
            // we have to check that bfyx_f16 format can be used in the network and layer is optimized for it.
        ...
        auto fuse_activation_f = [&p, bfyx_f16_conv](activation_node& activation_node) {
            auto& input_data = activation_node.get_dependency(0);
            if (input_data.get_users().size() != 1)
                return;

            if ((!(input_data.is_type<convolution>() && bfyx_f16_conv(input_data.as<convolution>())) &&
        ...
        auto fuse_scale_f = [&p, bfyx_f16_conv](scale_node& scale_node) {
            ...
            bool fuse_to_conv = input_data.is_type<convolution>() && bfyx_f16_conv(input_data.as<convolution>());
        ...
        auto fuse_eltwise_f = [&p, &recalc_processing_order, bfyx_f16_conv](eltwise_node& node) {
            ...
            if (parent1->is_type<convolution>() && !bfyx_f16_conv(parent1->as<convolution>()))
                return;
            if (parent2->is_type<convolution>() && !bfyx_f16_conv(parent2->as<convolution>()))
                return;
