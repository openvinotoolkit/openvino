...
bool layout_optimizer::can_fuse_reorder(program_node& prev, program_node& next, format fmt_prev, format fmt_next) {
    ...
    if (next.is_type<convolution>() &&
        ...
        (prev_output_layout.size.feature[0] == 1 || prev_output_layout.size.feature[0] == 3 ||
        (prev_output_layout.size.feature[0] == 4 && (prev_dt == data_types::u8 || prev_dt == data_types::i8)))) ||
...
bool layout_optimizer::convolution_b_fs_yx_fsv16_opt(layout const &input_layout,
    ...
    if (!correct_in_feature && (input_layout.size.feature[0] == 1 || input_layout.size.feature[0] == 3) && weights_layout.size.batch[0] >= feature_block_size)
