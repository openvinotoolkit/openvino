...
inline format imad_case(convolution_node const& node) {
...
layout layout_optimizer::get_expected_layout(layout const& current_layout,
    ...
    } else if ((current_layout.data_type == data_types::u8 || current_layout.data_type == data_types::i8)
        && output_or_weights_layout.size.spatial[0] == output_or_weights_layout.size.spatial[1]
        && (output_or_weights_layout.size.spatial[0] == 1 || output_or_weights_layout.size.spatial[0] == 3 || output_or_weights_layout.size.spatial[0] == 7)
        && (prim->stride.spatial[0] == prim->stride.spatial[1])) {  // imad case
