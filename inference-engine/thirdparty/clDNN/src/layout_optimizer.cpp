...
bool layout_optimizer::data_types_are_suitable_for_onednn(program_node& node) {
...
impl_types layout_optimizer::get_preferred_impl_type(program_node& node, format preferred_format) {
    ...
    const bool use_onednn_impls = (node.get_program().get_engine().get_device_info().supports_immad &&
                                  node.get_program().get_engine().configuration().queue_type != queue_types::out_of_order);
    ...
    //} else if (node.is_type<convolution>() || node.is_type<pooling>() || node.is_type<deconvolution>()) {
    } else if (node.is_type<pooling>() && use_onednn_impls) {
