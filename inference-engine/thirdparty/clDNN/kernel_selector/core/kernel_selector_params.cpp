...
JitConstants base_params::fused_operation_desc::MakeOpJitConstants(const FusedOpsConfiguration& conf,
    ...
    if (activation.function != ActivationFunction::NONE) {
        ...
        std::string nl_m = std::to_string(activation.m);
        std::string nl_n = std::to_string(activation.n);
        ...
        if (typed_activation) {
            std::string params = Broadcast(nl_m, vec_size) + ","+ Broadcast(nl_n, vec_size);
...
std::string base_params::fused_operation_desc::Broadcast(std::string var, size_t vec_size) const {
    return "(" + GetOutputType(vec_size) + ")(" + var + ")";
}
