...
TEST(set_output_memory_gpu, basic_opt) {
    ...
    //  bo.set_option(build_option::graph_dumps_dir("cldnn_graph_dump"));
    network network(engine, topology, bo);

    network.set_input_data("input1", input1);
    network.set_input_data("input2", input2);
    network.set_output_memory(outputID, output_mem, true, true, true);
    // print out primitive memory info
    auto opt_names = network.get_output_optimized_chain_ids(outputID);
    if (!opt_names.empty()) {
        std::cout << "Output " << outputID << ", optimized layers:\n";
        for (auto& name : opt_names) {
            const auto opt_layout = network.get_output_memory(name)->get_layout();
            std::cout << " [" + name + "], " + opt_layout.format.traits(opt_layout.format.value).order;
            std::cout << opt_layout.size;
            std::cout << std::endl << std::flush;
        }

        auto real_names = network.get_output_memory_primitive_ids(outputID);
        if (!real_names.empty()) {
            std::cout << "real memory owners:\n";
