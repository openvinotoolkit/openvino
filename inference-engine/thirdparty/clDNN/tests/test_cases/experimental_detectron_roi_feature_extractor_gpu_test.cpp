...
TEST(experimental_detectron_roi_feature_extractor_gpu_fp32, one_level) {
    ...
    auto roi_input = engine.allocate_memory({data_types::f32, format::bfyx, tensor(batch(rois_num), feature(4))});
    auto level_1 = engine.allocate_memory({data_types::f32, format::bfyx, {1, 2, 3, 2}});
    auto second_output = engine.allocate_memory({ data_types::f32, format::bfyx, tensor(batch(rois_num), feature(4))});
    ...
    topology.add(
        experimental_detectron_roi_feature_extractor("edrfe",
