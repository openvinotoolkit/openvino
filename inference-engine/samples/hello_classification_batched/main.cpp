 ...
 */
std::vector<std::string> readInputFileNames(const std::string& path, size_t files_limit) {
    struct stat sb;
    ...
    if (S_ISDIR(sb.st_mode)) {
        ...
        for (struct dirent* ep = readdir(dp); ep != nullptr && file_counter < files_limit; ep = readdir(dp)) {
...
int main(int argc, char* argv[]) {
    try {
        ...
        // --------------------------- Reshape model
        // -------------------------------------------------
        bool is_batched_supported = isBatchedBlobSupported(ie, device_name);
        if (!is_batched_supported)
        {
            std::cerr << "Batched blob is not supported for device: " << device_name <<std::endl;
            std::vector<std::string> availableDevices = ie.GetAvailableDevices();
            std::cerr << "Try use another device type, available devices: ";
            std::copy(availableDevices.begin(), availableDevices.end(), std::ostream_iterator<std::string>(std::cerr, ","));
            std::cerr << std::endl;
            return -1;
        }
        
        size_t netInputSize =  image_names.size();
        ...
        std::cout << "Batch size: " << batchSize  << ", batched_blob_size_limit: " << batched_blob_size_limit<< std::endl;
        // -----------------------------------------------------------------------------------------------------
 ...
 * @return blob point to hold the NV12 input data
 */
std::vector<Blob::Ptr> generateInputBlobs(size_t netInputSize, size_t width, size_t height) {
    // read image with size converted to NV12 data size: height(NV12) = 3 / 2 *
    // logical height
    ...
    std::vector<Blob::Ptr> blobs;
    std::vector<UString> data, 
    for (size_t blob_counter = 0; blob_counter < netInputSize; blob_counter++) {
        // --------------------------- Create a blob to hold the NV12 input data
        // -------------------------------
        auto ptr = &buf[0];
...
int main(int argc, char* argv[]) {
    ...
    std::string model_xml_path = mobilenet-ssd;
    network = core.ReadNetwork(model_xml_path);

    std::string device_name{"CPU"};
    
    bool batch_enabled = false;
    const std::vector<std::string> supported_metrics = core.GetMetric(device_name, METRIC_KEY(SUPPORTED_METRICS));
    if (std::find(supported_metrics.begin(), supported_metrics.end(), METRIC_KEY(OPTIMIZATION_CAPABILITIES))
        != supported_metrics.end())
    {
        const std::vector<std::string> optimization_caps = core.GetMetric(device_name, METRIC_KEY(OPTIMIZATION_CAPABILITIES));
    
        if(std::find(optimization_caps.begin(), optimization_caps.end(), METRIC_VALUE(BATCHED_BLOB)) != optimization_caps.end())
        {
            batch_enabled = true;
        }
    }
    ...
    if (!batch_enabled)
    {
        std::cerr << "BATCHED_BLOB is not enabled, nothing to do" << std::endl;
        return -1;
    }
    ...
    if (batchSize != netInputSize)
    {
        std::cerr << "requested batch size and set batch size are differ, requested: " << netInputSize << std::endl;
        return -1;
    }
    ...
    // -----------------------------------------------------------------------------------------------------
        ...
        // --------------------------- Step 4. Loading a model to the device
        // ----------------------------------------
        InferenceEngine::ExecutableNetwork executable_network = core.LoadNetwork(network, device_name);
        // -----------------------------------------------------------------------------------------------------
        ...
        // --------------------------- Step 6. Prepare input
        // --
        auto inputs = generateInputBlobs(netInputSize, input_width, input_height);
