...
bool ParseAndCheckCommandLine(int argc, char* argv[])
{
    ...
    if (FLAGS_nt <= 0 || FLAGS_nt > 10)
    {
        throw std::logic_error(
            "Incorrect value for nt argument. It should be "
...
std::shared_ptr<Function> createNgraphFunction()
{
    ...
    auto paramNode = std::make_shared<op::Parameter>(element::Type_t::f32,
                                                     Shape(std::vector<size_t>{{64, 1, 28, 28}}));
    ...
    std::shared_ptr<Node> maxPoolingNodeFirst =
        std::make_shared<op::v1::MaxPool>(addNodeFirst->output(0),
    ...
    std::shared_ptr<Node> convolutionSecondConstantNode = std::make_shared<op::Constant>(
        element::Type_t::f32, convSecondShape, (weightsPtr->cbuffer().as<uint8_t*>() + offset));
    ...
    std::shared_ptr<Node> maxPoolingNodeSecond =
        std::make_shared<op::v1::MaxPool>(addNodeSecond->output(0),
                                          Strides{2, 2},
                                          padBeginShape,
                                          padEndShape,
                                          Shape{2, 2},
    ...
    std::shared_ptr<Node> reshapeFirstNode = std::make_shared<op::v1::Reshape>(
        maxPoolingNodeSecond->output(0), reshapeFirstConstantNode->output(0), true);
    ...
    std::shared_ptr<Node> matMulFirstNode = std::make_shared<op::MatMul>(
        reshapeFirstNode->output(0), matMulFirstConstantNode->output(0), false, true);
    ...
    std::shared_ptr<Node> matMulSecondNode = std::make_shared<op::MatMul>(
        reshapeSecondNode->output(0), matMulSecondConstantNode->output(0), false, true);
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        for (const auto& item : inputInfo)
        {
            ...
            size_t image_size =
                input->getTensorDesc().getDims()[2] * input->getTensorDesc().getDims()[3];
