...
bool ParseAndCheckCommandLine(int argc, char *argv[]) {
  ...
  if (FLAGS_m.empty()) {
    ...
    throw std::logic_error(
        "Path to a .bin file with weights for the trained model is required "
  ...
  if (FLAGS_i.empty()) {
    showUsage();
    throw std::logic_error(
        "Path to an image is required but not set. Please set -i option.");
  }
...
TBlob<uint8_t>::CPtr ReadWeights(std::string filepath) {
  ...
  TBlob<uint8_t>::Ptr weightsPtr(
      new TBlob<uint8_t>({Precision::FP32, {ulFileSize}, Layout::C}));
  weightsPtr->allocate();
...
std::shared_ptr<Function> createNgraphFunction() {
  ...
  std::shared_ptr<Node> convolutionFirstConstantNode =
      std::make_shared<op::Constant>(element::Type_t::f32, convFirstShape,
                                     weightsPtr->cbuffer().as<uint8_t *>());

  std::shared_ptr<Node> convolutionNodeFirst =
      std::make_shared<op::v1::Convolution>(
          paramNode->output(0), convolutionFirstConstantNode->output(0),
          Strides(SizeVector{1, 1}), CoordinateDiff(padBegin),
          CoordinateDiff(padEnd), Strides(SizeVector{1, 1}));
  ...
  std::shared_ptr<Node> addFirstConstantNode = std::make_shared<op::Constant>(
      element::Type_t::f32, addFirstShape,
      (weightsPtr->cbuffer().as<uint8_t *>() + offset));

  std::shared_ptr<Node> addNodeFirst = std::make_shared<op::v1::Add>(
  ...
  std::shared_ptr<Node> maxPoolingNodeFirst = std::make_shared<op::v1::MaxPool>(
      addNodeFirst->output(0), std::vector<size_t>{2, 2}, padBeginShape,
      padEndShape, std::vector<size_t>{2, 2}, op::RoundingType::CEIL,
      op::PadType::EXPLICIT);
  ...
  std::shared_ptr<Node> convolutionNodeSecond =
      std::make_shared<op::v1::Convolution>(
          maxPoolingNodeFirst->output(0),
          convolutionSecondConstantNode->output(0), Strides({1, 1}),
  ...
  std::shared_ptr<Node> addSecondConstantNode = std::make_shared<op::Constant>(
      element::Type_t::f32, addSecondShape,
      (weightsPtr->cbuffer().as<uint8_t *>() + offset));

  std::shared_ptr<Node> addNodeSecond = std::make_shared<op::v1::Add>(
      convolutionNodeSecond->output(0), addSecondConstantNode->output(0));

  // -------MAXPOOL 2--------
  std::shared_ptr<Node> maxPoolingNodeSecond =
      std::make_shared<op::v1::MaxPool>(
          addNodeSecond->output(0), Strides{2, 2}, padBeginShape, padEndShape,
  ...
  std::shared_ptr<Node> reshapeFirstConstantNode =
      std::make_shared<op::Constant>(
          element::Type_t::i64, reshapeFirstShape,
          (weightsPtr->cbuffer().as<uint8_t *>() + reshapeOffset));

  std::shared_ptr<Node> reshapeFirstNode = std::make_shared<op::v1::Reshape>(
      maxPoolingNodeSecond->output(0), reshapeFirstConstantNode->output(0),
      true);
  ...
  std::shared_ptr<Node> matMulFirstConstantNode =
      std::make_shared<op::Constant>(
          element::Type_t::f32, matMulFirstShape,
          (weightsPtr->cbuffer().as<uint8_t *>() + offset));

  std::shared_ptr<Node> matMulFirstNode = std::make_shared<op::MatMul>(
      reshapeFirstNode->output(0), matMulFirstConstantNode->output(0), false,
      true);
  ...
  std::shared_ptr<Node> addThirdConstantNode = std::make_shared<op::Constant>(
      element::Type_t::f32, addThirdShape,
      (weightsPtr->cbuffer().as<uint8_t *>() + offset));

  std::shared_ptr<Node> addThirdNode = std::make_shared<op::v1::Add>(
      matMulFirstNode->output(0), addThirdConstantNode->output(0));

  // -------Relu-------------
  std::shared_ptr<Node> reluNode =
      std::make_shared<op::Relu>(addThirdNode->output(0));

  // -------Reshape 2--------
  auto reshapeSecondShape = Shape{2};
  std::shared_ptr<Node> reshapeSecondConstantNode =
      std::make_shared<op::Constant>(
          element::Type_t::i64, reshapeSecondShape,
          (weightsPtr->cbuffer().as<uint8_t *>() + reshapeOffset));

  std::shared_ptr<Node> reshapeSecondNode = std::make_shared<op::v1::Reshape>(
      reluNode->output(0), reshapeSecondConstantNode->output(0), true);
  ...
  std::shared_ptr<Node> matMulSecondConstantNode =
      std::make_shared<op::Constant>(
          element::Type_t::f32, matMulSecondShape,
          (weightsPtr->cbuffer().as<uint8_t *>() + offset));

  std::shared_ptr<Node> matMulSecondNode = std::make_shared<op::MatMul>(
      reshapeSecondNode->output(0), matMulSecondConstantNode->output(0), false,
      true);
  ...
  std::shared_ptr<Node> add4ConstantNode = std::make_shared<op::Constant>(
      element::Type_t::f32, add4Shape,
      (weightsPtr->cbuffer().as<uint8_t *>() + offset));

  std::shared_ptr<Node> add4Node = std::make_shared<op::v1::Add>(
      matMulSecondNode->output(0), add4ConstantNode->output(0));

  // -------softMax----------
  std::shared_ptr<Node> softMaxNode =
      std::make_shared<op::v1::Softmax>(add4Node->output(0), 1);
...
int main(int argc, char *argv[]) {
  try {
    ...
    // -----------------------------------------------------
    slog::info << "Checking that the outputs are as the sample expects"
               << slog::endl;
    ...
    if (classCount > 10) {
      throw std::logic_error(
          "Incorrect number of output classes for LeNet network");
    }
    ...
    for (const auto &item : inputInfo) {
      ...
      size_t image_size = input->getTensorDesc().getDims()[2] *
    ...
    ClassificationResult classificationResult(outputBlob, images, batchSize,
                                              FLAGS_nt, labels);
