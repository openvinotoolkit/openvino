...
bool ParseAndCheckCommandLine(int argc, char *argv[]) {
  ...
  if (FLAGS_i.empty()) {
    showUsage();
    throw std::logic_error(
        "Input is required but not set. Please set -i option.");
  }
  ...
  try {
    ...
    for (const auto &item : inputInfo) {
      MemoryBlob::Ptr minput =
          as<MemoryBlob>(infer_request.GetBlob(item.first));
      if (!minput) {
      ...
      size_t image_size = minput->getTensorDesc().getDims()[3] *
                          minput->getTensorDesc().getDims()[2];
    ...
    const auto output_data =
        lmoHolder.as<const PrecisionTrait<Precision::FP32>::value_type *>();
    ...
    slog::info << "Output size [N,C,H,W]: " << num_images << ", "
               << num_channels << ", " << H << ", " << W << slog::endl;

    {
      ...
      for (size_t n = 0; n < num_images; n++) {
        ...
        std::string out_img_name =
            std::string("out" + std::to_string(n + 1) + ".bmp");
