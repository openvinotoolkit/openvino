  ...
  try {
    ...
    if (argc != 5) {
      std::cout << "Usage : " << argv[0]
                << " <path_to_model> <path_to_image> <device> <batch>"
                << std::endl;
      return EXIT_FAILURE;
    }
    ...
    if (inputs_info.size() != 1 || outputs_info.size() != 1)
      throw std::logic_error(
          "Sample supports clean SSD network with one input and one output");
    ...
    std::cout << "Resizing network to the image size = [" << image.rows << "x"
              << image.cols << "] "
      ...
      for (const auto &op : ngraphFunction->get_ops()) {
        if (op->get_type_info() == ngraph::op::DetectionOutput::type_info) {
          if (output_info->getName() != op->get_friendly_name()) {
            throw std::logic_error(
                "Detection output op does not produce a network output");
          }
          break;
        }
      }
    ...
    std::cout << "Resulting input shape = " << dumpVec(input_shape)
              << std::endl;
    std::cout << "Resulting output shape = " << dumpVec(output_shape)
              << std::endl;
    // -----------------------------------------------------------------------------------------------------
    ...
    for (size_t cur_proposal = 0; cur_proposal < max_proposal_count;
         cur_proposal++) {
      ...
      if (confidence > 0.5f) {
        ...
        cv::rectangle(image, cv::Point2f(xmin, ymin), cv::Point2f(xmax, ymax),
                      cv::Scalar(0, 0, 255));
        std::cout << "[" << cur_proposal << "," << label
                  << "] element, prob = " << confidence << ", bbox = (" << xmin
                  << "," << ymin << ")-(" << xmax << "," << ymax << ")"
