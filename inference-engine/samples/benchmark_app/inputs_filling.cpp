                   ...
                   const size_t& inputSize)
{
         ...
         i++, inputIndex += inputSize)
    {
        ...
        {
            slog::warn << "Image " << filePaths[inputIndex] << " cannot be read!" << slog::endl
                       << slog::endl;
            continue;
        }
            ...
            for (size_t h = 0; h < app_info.height(); ++h)
            {
                ...
                for (size_t ch = 0; ch < numChannels; ++ch)
                {
                    ...
                    inputBlobData[offset] =
                        vreader.at(imageId).get()[h * width * numChannels + w * numChannels + ch];
                }
            }
        }
    }
    ...
    else
    {
        ...
        else if (binaryToBeUsed > binaryFiles.size())
        {
            slog::warn << "Some binary input files will be duplicated: " << binaryToBeUsed
                       << " files are required but only " << binaryFiles.size() << " are provided"
                       << slog::endl;
        }
        else if (binaryToBeUsed < binaryFiles.size())
        {
        ...
        else if (imagesToBeUsed > imageFiles.size())
        {
            slog::warn << "Some image input files will be duplicated: " << imagesToBeUsed
                       << " files are required but only " << imageFiles.size() << " are provided"
                       << slog::endl;
        }
        else if (imagesToBeUsed < imageFiles.size())
        {
            ...
            else
            {
                ...
                if (app_info.isImageInfo() && (input_image_sizes.size() == 1))
                {
                    ...
                    slog::info << "Fill input '" << item.first << "' with image size "
                               << image_size.first << "x" << image_size.second << slog::endl;
                    if (precision == InferenceEngine::Precision::FP32)
                    {
