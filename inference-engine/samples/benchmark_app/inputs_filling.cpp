...
filterFilesByExtensions(const std::vector<std::string> &filePaths,
  ...
  auto getExtension = [](const std::string &name) {
    ...
    return extensionPosition == std::string::npos
               ? ""
               : name.substr(extensionPosition + 1, name.size() - 1);
  };
...
void fillBlobImage(Blob::Ptr &inputBlob,
                   ...
                   const size_t &requestId, const size_t &inputId,
                   const size_t &inputSize) {
  MemoryBlob::Ptr minput = as<MemoryBlob>(inputBlob);
  if (!minput) {
    IE_THROW()
        << "We expect inputBlob to be inherited from MemoryBlob in "
           "fillBlobImage, "
  ...
  for (size_t i = 0ULL,
              inputIndex = requestId * batchSize * inputSize + inputId;
       i < batchSize; i++, inputIndex += inputSize) {
    ...
    if (reader.get() == nullptr) {
      slog::warn << "Image " << filePaths[inputIndex] << " cannot be read!"
                 << slog::endl
                 << slog::endl;
      continue;
    }
    ...
    std::shared_ptr<uint8_t> imageData(
        reader->getData(app_info.width(), app_info.height()));
    if (imageData) {
    ...
    for (size_t w = 0; w < app_info.width(); ++w) {
      ...
      for (size_t h = 0; h < app_info.height(); ++h) {
        ...
        for (size_t ch = 0; ch < numChannels; ++ch) {
           ...
           * bytes            **/
          size_t offset =
              imageId * numChannels * width * height +
          ...
          inputBlobData[offset] =
              vreader.at(imageId)
                  .get()[h * width * numChannels + w * numChannels + ch];
        }
      }
    }
  }
...
void fillBlobBinary(Blob::Ptr &inputBlob,
                    const std::vector<std::string> &filePaths,
                    const size_t &batchSize, const size_t &requestId,
                    const size_t &inputId, const size_t &inputSize) {
  MemoryBlob::Ptr minput = as<MemoryBlob>(inputBlob);
  if (!minput) {
    IE_THROW()
        << "We expect inputBlob to be inherited from MemoryBlob in "
  ...
  for (size_t i = 0ULL,
    ...
    std::ifstream binaryFile(filePaths[inputIndex],
    ...
    if (fileSize != inputSize) {
      IE_THROW() << "File " << filePaths[inputIndex] << " contains "
...
template <typename T>
void fillBlobImInfo(Blob::Ptr &inputBlob, const size_t &batchSize,
...
void fillBlobs(const std::vector<std::string> &inputFiles,
  ...
  for (auto &item : app_inputs_info) {
    if (item.second.isImage()) {
      input_image_sizes.push_back(
          std::make_pair(item.second.width(), item.second.height()));
    }
    slog::info << "Network input '" << item.first << "' precision "
               << item.second.precision << ", dimensions ("
               << item.second.layout << "): ";
  ...
  if (inputFiles.empty()) {
    ...
    binaryFiles =
        filterFilesByExtensions(inputFiles, supported_binary_extensions);
    ...
    if (binaryToBeUsed > 0 && binaryFiles.empty()) {
      ...
      slog::warn << "Some binary input files will be duplicated: "
                 << binaryToBeUsed << " files are required but only "
                 << binaryFiles.size() << " are provided" << slog::endl;
    } else if (binaryToBeUsed < binaryFiles.size()) {
      slog::warn << "Some binary input files will be ignored: only "
                 << binaryToBeUsed << " are required from "
                 << binaryFiles.size() << slog::endl;
    }

    imageFiles =
        filterFilesByExtensions(inputFiles, supported_image_extensions);
    ...
    if (imagesToBeUsed > 0 && imageFiles.empty()) {
      ...
      slog::warn << "Some image input files will be duplicated: "
                 << imagesToBeUsed << " files are required but only "
                 << imageFiles.size() << " are provided" << slog::endl;
    } else if (imagesToBeUsed < imageFiles.size()) {
      slog::warn << "Some image input files will be ignored: only "
                 << imagesToBeUsed << " are required from " << imageFiles.size()
                 << slog::endl;
    }
  }
  ...
  for (size_t requestId = 0; requestId < requests.size(); requestId++) {
    ...
    for (auto &item : app_inputs_info) {
      ...
      if (app_info.isImage()) {
        if (!imageFiles.empty()) {
          // Fill with Images
          fillBlobImage(inputBlob, imageFiles, batchSize, app_info, requestId,
                        imageInputId++, imageInputCount);
          continue;
        }
      } else {
        if (!binaryFiles.empty()) {
          ...
          if (precision == InferenceEngine::Precision::FP32) {
            fillBlobBinary<float>(inputBlob, binaryFiles, batchSize, requestId,
                                  binaryInputId++, binaryInputCount);
          } else if (precision == InferenceEngine::Precision::FP16) {
            fillBlobBinary<short>(inputBlob, binaryFiles, batchSize, requestId,
                                  binaryInputId++, binaryInputCount);
          } else if (precision == InferenceEngine::Precision::I32) {
            fillBlobBinary<int32_t>(inputBlob, binaryFiles, batchSize,
                                    requestId, binaryInputId++,
                                    binaryInputCount);
          } else if (precision == InferenceEngine::Precision::I64) {
            fillBlobBinary<int64_t>(inputBlob, binaryFiles, batchSize,
                                    requestId, binaryInputId++,
                                    binaryInputCount);
          } else if ((precision == InferenceEngine::Precision::U8) ||
                     (precision == InferenceEngine::Precision::BOOL)) {
            fillBlobBinary<uint8_t>(inputBlob, binaryFiles, batchSize,
                                    requestId, binaryInputId++,
                                    binaryInputCount);
          } else {
        ...
        if (app_info.isImageInfo() && (input_image_sizes.size() == 1)) {
          ...
          slog::info << "Fill input '" << item.first << "' with image size "
      ...
      slog::info << "Fill input '" << item.first << "' with random values ("
                 << std::string(
                        (app_info.isImage() ? "image" : "some binary data"))
