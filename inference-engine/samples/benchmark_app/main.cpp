...
bool ParseAndCheckCommandLine(int argc, char *argv[]) {
  ...
  if (FLAGS_api != "async" && FLAGS_api != "sync") {
    throw std::logic_error(
        "Incorrect API. Please set -api option to `sync` or `async` value.");
  }

  if (!FLAGS_report_type.empty() && FLAGS_report_type != noCntReport &&
      FLAGS_report_type != averageCntReport &&
      FLAGS_report_type != detailedCntReport) {
    std::string err =
        "only " + std::string(noCntReport) + "/" +
        std::string(averageCntReport) + "/" + std::string(detailedCntReport) +
        " report types are supported (invalid -report_type option value)";
    throw std::logic_error(err);
  }

  if ((FLAGS_report_type == averageCntReport) &&
      ((FLAGS_d.find("MULTI") != std::string::npos))) {
    throw std::logic_error("only " + std::string(detailedCntReport) +
                           " report type is supported for MULTI device");
  }

  bool isNetworkCompiled = fileExt(FLAGS_m) == "blob";
  bool isPrecisionSet =
      !(FLAGS_ip.empty() && FLAGS_op.empty() && FLAGS_iop.empty());
  if (isNetworkCompiled && isPrecisionSet) {
    std::string err =
        std::string("Cannot set precision for a compiled network. ") +
...
static void next_step(const std::string additional_info = "") {
  ...
  if (step_names.count(step_id) == 0)
    IE_THROW() << "Step ID " << step_id << " is out of total steps number "
               << step_names.size();

  std::cout << "[Step " << step_id << "/" << step_names.size() << "] "
            << step_names.at(step_id)
            << (additional_info.empty() ? "" : " (" + additional_info + ")")
            << std::endl;
}

template <typename T> T getMedianValue(const std::vector<T> &vec) {
  std::vector<T> sortedVec(vec);
  std::sort(sortedVec.begin(), sortedVec.end());
  return (sortedVec.size() % 2 != 0)
             ? sortedVec[sortedVec.size() / 2ULL]
             : (sortedVec[sortedVec.size() / 2ULL] +
                sortedVec[sortedVec.size() / 2ULL - 1ULL]) /
                   static_cast<T>(2.0);
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    auto isFlagSetInCommandLine = [&command_line_arguments](
    ...
    std::map<std::string, std::string> device_nstreams =
        parseNStreamsValuePerDevice(devices, FLAGS_nstreams);
    ...
    if (FLAGS_d.find("CPU") != std::string::npos && !FLAGS_l.empty()) {
      ...
      const auto extension_ptr =
          std::make_shared<InferenceEngine::Extension>(FLAGS_l);
      ie.AddExtension(extension_ptr);
      slog::info << "CPU (MKLDNN) extensions is loaded " << FLAGS_l
                 << slog::endl;
    }
    ...
    if (config.count("GPU") &&
        config.at("GPU").count(CONFIG_KEY(CONFIG_FILE))) {
    ...
    for (auto &device : devices) {
      ...
      if (isFlagSetInCommandLine("pc")) {
        ...
        device_config[CONFIG_KEY(PERF_COUNT)] =
            FLAGS_pc ? CONFIG_VALUE(YES) : CONFIG_VALUE(NO);
        ...
        slog::warn << "Performance counters for " << device
                   << " device is turned on. To print results use -pc option."
                   << slog::endl;
      } else if (FLAGS_report_type == detailedCntReport ||
                 FLAGS_report_type == averageCntReport) {
        slog::warn << "Turn on performance counters for " << device
                   << " device since report type is " << FLAGS_report_type
                   << "." << slog::endl;
      ...
      perf_counts =
          (device_config.at(CONFIG_KEY(PERF_COUNT)) == CONFIG_VALUE(YES))
              ? true
              : perf_counts;
      ...
      auto setThroughputStreams = [&]() {
        ...
        if (device_nstreams.count(device)) {
          ...
          if (std::find(supported_config_keys.begin(),
                        ...
                        key) == supported_config_keys.end()) {
            throw std::logic_error(
                "Device " + device + " doesn't support config key '" + key +
                "'! " +
          ...
          slog::warn
              << "-nstreams default value is determined automatically for "
              << device
              << " device. "
              ...
              << slog::endl;
          if (std::string::npos ==
              device.find("MYRIAD")) // MYRIAD sets the default number of
      ...
      if (device ==
          "CPU") { // CPU supports few special performance-oriented keys
        // limit threading for CPU portion of inference
        if (isFlagSetInCommandLine("nthreads"))
          device_config[CONFIG_KEY(CPU_THREADS_NUM)] =
              std::to_string(FLAGS_nthreads);

        if (isFlagSetInCommandLine("enforcebf16"))
          device_config[CONFIG_KEY(ENFORCE_BF16)] =
              FLAGS_enforcebf16 ? CONFIG_VALUE(YES) : CONFIG_VALUE(NO);
        ...
        if (isFlagSetInCommandLine("pin")) {
          ...
          if ((device_name.find("MULTI") != std::string::npos) &&
            ...
            slog::warn
                << "Turn off threads pinning for " << device
        ...
        if (isFlagSetInCommandLine("nthreads"))
          device_config[GNA_CONFIG_KEY(LIB_N_THREADS)] =
              std::to_string(FLAGS_nthreads);
      } else {
        std::vector<std::string> supported_config_keys =
            ie.GetMetric(device, METRIC_KEY(SUPPORTED_CONFIG_KEYS));
        auto supported = [&](const std::string &key) {
          return std::find(std::begin(supported_config_keys),
                           std::end(supported_config_keys),
                           key) != std::end(supported_config_keys);
        };
        if (supported(CONFIG_KEY(CPU_THREADS_NUM)) &&
            isFlagSetInCommandLine("nthreads")) {
          device_config[CONFIG_KEY(CPU_THREADS_NUM)] =
              std::to_string(FLAGS_nthreads);
        }
        if (supported(CONFIG_KEY(CPU_THROUGHPUT_STREAMS)) &&
            isFlagSetInCommandLine("nstreams")) {
          device_config[CONFIG_KEY(CPU_THROUGHPUT_STREAMS)] = FLAGS_nstreams;
        }
        if (supported(CONFIG_KEY(CPU_BIND_THREAD)) &&
            isFlagSetInCommandLine("pin")) {
          device_config[CONFIG_KEY(CPU_BIND_THREAD)] = FLAGS_pin;
        }
      }
    }
    ...
    auto get_total_ms_time = [](Time::time_point &startTime) {
      return std::chrono::duration_cast<ns>(Time::now() - startTime).count() *
             0.000001;
    };
    ...
    if (!isNetworkCompiled) {
      ...
      bool reshape = false;
      app_inputs_info = getInputsInfo<InputInfo::Ptr>(
          FLAGS_shape, FLAGS_layout, FLAGS_b, inputInfo, reshape);
      if (reshape) {
        InferenceEngine::ICNNNetwork::InputShapes shapes = {};
        for (auto &item : app_inputs_info)
          shapes[item.first] = item.second.shape;
        slog::info << "Reshaping network: " << getShapesString(shapes)
                   << slog::endl;
        startTime = Time::now();
        cnnNetwork.reshape(shapes);
        auto duration_ms = double_to_string(get_total_ms_time(startTime));
        slog::info << "Reshape network took " << duration_ms << " ms"
                   << slog::endl;
        if (statistics)
          statistics->addParameters(
      ...
      batchSize = (!FLAGS_layout.empty()) ? getBatchSize(app_inputs_info)
                                          : cnnNetwork.getBatchSize();

      topology_name = cnnNetwork.getName();
      slog::info << (FLAGS_b != 0 ? "Network batch size was changed to: "
                                  : "Network batch size: ")
                 << batchSize << slog::endl;
      ...
      for (auto &item : cnnNetwork.getInputsInfo()) {
        ...
        if (!FLAGS_ip.empty() ||
            FLAGS_iop.find(item.first) != std::string::npos) {
          app_inputs_info.at(item.first).precision =
              item.second->getPrecision();
      ...
      auto duration_ms = double_to_string(get_total_ms_time(startTime));
      slog::info << "Import network took " << duration_ms << " ms"
                 << slog::endl;
      if (statistics)
        statistics->addParameters(StatisticsReport::Category::EXECUTION_RESULTS,
                                  {{"import network time (ms)", duration_ms}});
      app_inputs_info = getInputsInfo<InputInfo::CPtr>(
          FLAGS_shape, FLAGS_layout, FLAGS_b, exeNetwork.GetInputsInfo());
      if (batchSize == 0) {
        batchSize = 1;
      }
    }
    ...
    if (statistics) {
      statistics->addParameters(
          StatisticsReport::Category::RUNTIME_CONFIG,
          {
              ...
              {"duration (ms)",
               std::to_string(getDurationInMilliseconds(duration_seconds))},
          });
    ...
    fillBlobs(inputFiles, batchSize, app_inputs_info,
    ...
    while ((niter != 0LL && iteration < niter) ||
      ...
      if (niter > 0) {
        ...
        // skipped.
        auto progressIntervalTime =
            duration_nanoseconds / progressBarTotalCount;
    ...
    if (!FLAGS_dump_config.empty()) {
      ...
      slog::info << "Inference Engine configuration settings were dumped to "
    ...
    if (!FLAGS_exec_graph_path.empty()) {
      try {
        ...
        slog::info << "executable graph is stored to " << FLAGS_exec_graph_path
                   << slog::endl;
      } catch (const std::exception &ex) {
    ...
    if (perf_counts) {
      ...
      for (size_t ireq = 0; ireq < nireq; ireq++) {
        auto reqPerfCounts =
            inferRequestsQueue.requests[ireq]->getPerformanceCounts();
        if (FLAGS_pc) {
          slog::info << "Performance counts for " << ireq
                     << "-th infer request:" << slog::endl;
          printPerformanceCounts(reqPerfCounts, std::cout,
                                 getFullDeviceName(ie, FLAGS_d), false);
        }
    ...
    std::cout << "Duration:   " << double_to_string(totalDuration) << " ms"
              << std::endl;
    if (device_name.find("MULTI") == std::string::npos)
      std::cout << "Latency:    " << double_to_string(latency) << " ms"
                << std::endl;
