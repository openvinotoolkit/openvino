...
bool ParseAndCheckCommandLine(int argc, char* argv[])
{
    ...
    if (!FLAGS_report_type.empty() && FLAGS_report_type != noCntReport &&
        FLAGS_report_type != averageCntReport && FLAGS_report_type != detailedCntReport)
    {
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (!FLAGS_report_type.empty())
        {
            ...
            statistics->addParameters(StatisticsReport::Category::COMMAND_LINE_PARAMETERS,
        ...
        for (auto& device : devices)
        {
            ...
            perf_counts = (device_config.at(CONFIG_KEY(PERF_COUNT)) == CONFIG_VALUE(YES))
                              ? true
                              : perf_counts;
            ...
            auto setThroughputStreams = [&]() {
                ...
                {
                    slog::warn << "-nstreams default value is determined automatically for "
                               << device
                               << " device. "
        ...
        if (!isNetworkCompiled)
        {
            ...
            batchSize =
                (!FLAGS_layout.empty()) ? getBatchSize(app_inputs_info) : cnnNetwork.getBatchSize();
        ...
        if ((niter > 0) && (FLAGS_api == "async"))
        {
            ...
            if (FLAGS_niter != niter)
            {
                slog::warn << "Number of iterations was aligned by request number from "
                           << FLAGS_niter << " to " << niter << " using number of requests "
                           << nireq << slog::endl;
            }
        }
        ...
        if (perf_counts)
        {
            std::vector<std::map<std::string, InferenceEngine::InferenceEngineProfileInfo>>
                perfCounts;
            for (size_t ireq = 0; ireq < nireq; ireq++)
            {
                ...
                if (FLAGS_pc)
                {
                    ...
                    printPerformanceCounts(
                        reqPerfCounts, std::cout, getFullDeviceName(ie, FLAGS_d), false);
                }
