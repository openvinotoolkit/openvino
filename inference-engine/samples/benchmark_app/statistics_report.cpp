...
void StatisticsReport::addParameters(const Category& category, const Parameters& parameters)
{
    if (_parameters.count(category) == 0)
        ...
        _parameters[category].insert(
            _parameters[category].end(), parameters.begin(), parameters.end());
}
                                                      ...
                                                      const PerformaceCounters& perfCounts)
{
    ...
    for (const auto& layer : performanceMapSorted)
    {
        ...
        {
        case InferenceEngine::InferenceEngineProfileInfo::EXECUTED: dumper << "EXECUTED"; break;
        case InferenceEngine::InferenceEngineProfileInfo::NOT_RUN: dumper << "NOT_RUN"; break;
...
void StatisticsReport::dumpPerformanceCounters(const std::vector<PerformaceCounters>& perfCounts)
{
    ...
    CsvDumper dumper(true,
                     _config.report_folder + _separator + "benchmark_" + _config.report_type +
                         "_report.csv");
