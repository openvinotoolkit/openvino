...
void StatisticsReport::addParameters(const Category &category,
                                     const Parameters &parameters) {
  if (_parameters.count(category) == 0)
    _parameters[category] = parameters;
  else
    _parameters[category].insert(_parameters[category].end(),
                                 parameters.begin(), parameters.end());
}

void StatisticsReport::dump() {
  CsvDumper dumper(true,
                   _config.report_folder + _separator + "benchmark_report.csv");
  ...
  slog::info << "Statistics report is stored to " << dumper.getFilename()
             << slog::endl;
}

void StatisticsReport::dumpPerformanceCountersRequest(
    CsvDumper &dumper, const PerformaceCounters &perfCounts) {
  ...
  for (const auto &layer : performanceMapSorted) {
    ...
    dumper << std::to_string(layer.second.realTime_uSec / 1000.0)
           << std::to_string(layer.second.cpu_uSec / 1000.0);
...
void StatisticsReport::dumpPerformanceCounters(
    const std::vector<PerformaceCounters> &perfCounts) {
  ...
  if (perfCounts.empty()) {
    slog::info << "Performance counters are empty. No reports are dumped."
               << slog::endl;
    return;
  }
  CsvDumper dumper(true, _config.report_folder + _separator + "benchmark_" +
                             _config.report_type + "_report.csv");
  if (_config.report_type == detailedCntReport) {
      ...
      for (size_t i = 0; i < perfCounts.size(); i++) {
        ...
        for (const auto &pm : performanceMapSorted) {
          if (performanceCountersAvg.count(pm.first) == 0) {
            ...
            performanceCountersAvg[pm.first].realTime_uSec +=
                perfCounts.at(i).at(pm.first).realTime_uSec;
            performanceCountersAvg[pm.first].cpu_uSec +=
                perfCounts.at(i).at(pm.first).cpu_uSec;
          }
        }
      }
    ...
    throw std::logic_error(
        "PM data can only be collected for average or detailed report types");
  }
  slog::info << "Performance counters report is stored to "
             << dumper.getFilename() << slog::endl;
}
