...
uint32_t deviceDefaultDeviceDurationInSeconds(const std::string& device)
{
    ...
    if (duration == 0)
    {
        ...
        slog::warn << "Default duration " << duration << " seconds for unknown device '" << device
                   << "' is used" << slog::endl;
    }
    return duration;
}
        ...
        if (batch_index != std::string::npos)
        {
            if (batch_size == 0)
                ...
                throw std::logic_error(
                    "Can't deterimine batch size: batch is "
                 ...
                 const std::map<std::string, std::map<std::string, std::string>>& config)
{
    ...
    {
        fs << device_it->first << "{:";
        for (auto param_it = device_it->second.begin(); param_it != device_it->second.end();
             ++param_it)
