...
std::pair<size_t, size_t> parseImageSize(const std::string &size_string) {
  ...
  if (delimiter_pos == std::string::npos ||
      delimiter_pos >= size_string.size() - 1 || delimiter_pos == 0) {
  ...
  size_t width =
      static_cast<size_t>(std::stoull(size_string.substr(0, delimiter_pos)));
  ...
  if (width % 2 != 0 || height % 2 != 0) {
    throw std::runtime_error(
        "Unsupported image size, width and height must be even numbers");
  }
...
std::vector<std::string> readInputFileNames(const std::string &path) {
  ...
  if (files.size() < max_files) {
    ...
    slog::info << "Files were added: " << files.size()
               << ". Too many to display each of them." << slog::endl;
  }
 ...
 */
std::vector<UString>
readImagesDataFromFiles(const std::vector<std::string> &files, size_t size) {
  ...
  for (const auto &image_path : files) {
    ...
    if (file_size < size) {
      std::stringstream err;
      err << "Invalid read size provided. File size: " << file_size
          << ", to read: " << size;
      throw std::runtime_error(err.str());
    }
    file.seekg(0);
...
int main(int argc, char *argv[]) {
  try {
    ...
    // -------------------------------------------------
    size_t netInputSize =
        isBatchedBlobSupported(ie, device_name) ? image_names.size() : 1;
    ICNNNetwork::InputShapes inputShapes = network.getInputShapes();
    for (auto &shape : inputShapes) {
      auto &dims = shape.second;
      if (dims.empty()) {
        throw std::runtime_error(
            "Network's input shapes have empty dimensions");
      }
      dims[0] = netInputSize;
    }
    ...
    // --------------------------------------------------------
    auto image_bufs = readImagesDataFromFiles(
        image_names, input_width * (input_height * 3 / 2));
    ...
    for (size_t i = 0; i < inputs.size(); i++) {
      ...
      ClassificationResult classificationResult(output, names, netInputSize, 10,
                                                labels);
