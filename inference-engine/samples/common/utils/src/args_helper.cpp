...
namespace
{
    ...
    void setLayouts(const InferenceEngine::CNNNetwork& network, const std::string iol)
    {
        ...
        for (auto&& item : user_layouts_map)
        {
            ...
            if (input != inputs.end())
            {
                if (!isMatchLayoutToDims(user_layout,
                                         input->second->getTensorDesc().getDims().size()))
                {
                    throw std::logic_error(item.second + " layout is not applicable to " +
                                           layer_name);
                }
            ...
            {
                if (!isMatchLayoutToDims(user_layout,
                                         output->second->getTensorDesc().getDims().size()))
                {
                    throw std::logic_error(item.second + " layout is not applicable to " +
                                           layer_name);
                }
