...
#define TBLOB_TOP_RESULT(precision)                                                                \
    ...
    {                                                                                              \
        using myBlobType =                                                                         \
            InferenceEngine::PrecisionTrait<InferenceEngine::Precision::precision>::value_type;    \
        InferenceEngine::TBlob<myBlobType>& tblob =                                                \
            dynamic_cast<InferenceEngine::TBlob<myBlobType>&>(input);                              \
        ...
        default:
            IE_THROW() << "cannot locate blob for precision: "
                       << input.getTensorDesc().getPrecision();
        }
...
public:
        ...
        for (unsigned int image_id = 0; image_id < _batchSize; ++image_id)
        {
            ...
            for (size_t id = image_id * _nTop, cnt = 0; id < (image_id + 1) * _nTop; ++cnt, ++id)
            {
                ...
                InferenceEngine::MemoryBlob::CPtr moutput =
                    InferenceEngine::as<InferenceEngine::MemoryBlob>(_outBlob);
                if (!moutput)
                {
                    throw std::logic_error(
                        "We expect _outBlob to be inherited from MemoryBlob in "
                ...
                const auto result = moutputHolder.as<const InferenceEngine::PrecisionTrait<
                    InferenceEngine::Precision::FP32>::value_type*>()[_results[id] +
                                                                      image_id * (_outBlob->size() /
                                                                                  _batchSize)];

                std::cout << std::setw(static_cast<int>(_classidStr.length())) << std::left
                          << _results[id] << " ";
                std::cout << std::left << std::setw(static_cast<int>(_probabilityStr.length()))
                          << std::fixed << result;
     ...
     */
    std::vector<unsigned> getResults() { return _results; }
};
