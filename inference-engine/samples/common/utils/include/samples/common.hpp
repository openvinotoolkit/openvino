...
public:
    ...
    inline unsigned char red() { return _r; }

    inline unsigned char blue() { return _b; }

    inline unsigned char green() { return _g; }
};
 ...
 */
static UNUSED void
    writeOutputBmp(std::vector<std::vector<size_t>> data, size_t classesNum, std::ostream& outFile)
{
 ...
 */
static UNUSED bool
    writeOutputBmp(std::string name, unsigned char* data, size_t height, size_t width)
{
...
static UNUSED bool
    writeOutputBmp(unsigned char* data, size_t height, size_t width, std::ostream& outFile)
{
    unsigned char file[14] = {
        'B',
        'M', // magic
    ...
    bool bshowHeader = true)
{
    ...
    for (const auto& it : performanceMapSorted)
    {
        ...
        stream << std::setw(30) << std::left
               << "layerType: " + std::string(it.second.layer_type) + " ";
        stream << std::setw(20) << std::left
               << "realTime: " + std::to_string(it.second.realTime_uSec);
    ...
    for (std::string& deviceName : devices)
    {
        if (deviceName != "")
        {
            try
            {
                ...
                devicesMap.insert(
                    std::pair<std::string, std::string>(deviceName, p.as<std::string>()));
            }
...
inline std::string getFullDeviceName(std::map<std::string, std::string>& devicesMap,
                                     std::string device)
{
...
public:
    explicit AveragePrecisionCalculator(double _threshold)
    ...
    void consumeImage(const ImageDescription& detectedObjects,
                      const ImageDescription& desiredObjects)
    {
...
inline std::size_t getTensorChannels(const InferenceEngine::TensorDesc& desc)
{
        ...
        layout == InferenceEngine::Layout::NC || layout == InferenceEngine::Layout::CN)
    {
        ...
        default: IE_THROW() << "Tensor does not have channels dimension";
        }
    }
    else
    {
