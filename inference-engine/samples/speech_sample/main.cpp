...
#define MAX_SCORE_DIFFERENCE                                                   \
  0.0001f                     // max score difference for frame error threshold
 ...
 * @return none.
 */
void GetKaldiArkInfo(const char *fileName, uint32_t numArrayToFindSize,
                     uint32_t *ptrNumArrays, uint32_t *ptrNumMemoryBytes) {
  ...
  if (in_file.good()) {
    while (!in_file.eof()) {
      ...
      std::getline(in_file, line,
                   '\0'); // read variable length name followed by space and NUL
      std::getline(in_file, line,
                   '\4'); // read "BFM" followed by space and control-D
      if (line.compare("BFM ") != 0) {
        break;
      }
      in_file.read(reinterpret_cast<char *>(&numRows),
                   sizeof(uint32_t));    // read number of rows
      std::getline(in_file, line, '\4'); // read control-D
      in_file.read(reinterpret_cast<char *>(&numCols),
                   sizeof(uint32_t)); // read number of columns
 ...
 * @param ptrNumBytesPerElement pointer to number bytes per element (size of
 * float by default)
 * @return none.
 */
void LoadKaldiArkArray(const char *fileName, uint32_t arrayIndex,
                       std::string &ptrName, std::vector<uint8_t> &memory,
                       uint32_t *ptrNumRows, uint32_t *ptrNumColumns,
  ...
  if (in_file.good()) {
    ...
    while (i < arrayIndex) {
      ...
      in_file.read(reinterpret_cast<char *>(&numCols),
                   sizeof(uint32_t)); // read number of columns
      in_file.seekg(numRows * numCols * sizeof(float),
                    in_file.cur); // read data
      i++;
    }
    if (!in_file.eof()) {
      std::string line;
      std::getline(in_file, ptrName,
                   '\0'); // read variable length name followed by space and NUL
      std::getline(in_file, line,
                   '\4'); // read "BFM" followed by space and control-D
      if (line.compare("BFM ") != 0) {
        fprintf(
            stderr,
            "Cannot find array specifier in file %s in LoadKaldiArkArray()!\n",
            fileName);
        exit(-1);
      }
      in_file.read(reinterpret_cast<char *>(ptrNumRows),
                   sizeof(uint32_t));    // read number of rows
      std::getline(in_file, line, '\4'); // read control-D
      in_file.read(reinterpret_cast<char *>(ptrNumColumns),
                   sizeof(uint32_t)); // read number of columns
      in_file.read(reinterpret_cast<char *>(&memory.front()),
                   *ptrNumRows * *ptrNumColumns *
                       sizeof(float)); // read array data
    }
    in_file.close();
  } else {
 ...
 * @return none.
 */
void SaveKaldiArkArray(const char *fileName, bool shouldAppend,
                       std::string name, void *ptrMemory, uint32_t numRows,
                       uint32_t numColumns) {
  ...
  if (out_file.good()) {
    ...
    out_file.write(reinterpret_cast<char *>(ptrMemory),
                   numRows * numColumns * sizeof(float));
    out_file.close();
  } else {
    throw std::runtime_error(
        std::string("Failed to open %s for writing in SaveKaldiArkArray()!\n") +
        fileName);
  }
}
 ...
 * @return scale factor
 */
float ScaleFactorForQuantization(void *ptrFloatMemory, float targetMax,
 ...
 * @return none.
 */
void CompareScores(float *ptrScoreArray, void *ptrRefScoreArray,
                   score_error_t *scoreError, uint32_t numRows,
                   uint32_t numColumns) {
  ...
  scoreError->rmsError =
      sqrt(scoreError->sumSquaredError / (numRows * numColumns));
...
float StdDevError(score_error_t error) {
  return (sqrt(error.sumSquaredError / error.numScores -
               (error.sumError / error.numScores) *
                   (error.sumError / error.numScores)));
}
...
inline void native_cpuid(unsigned int *eax, unsigned int *ebx,
  ...
  size_t level = *eax;
#ifdef _WIN32
  int regs[4] = {static_cast<int>(*eax), static_cast<int>(*ebx),
...
void printReferenceCompareResults(score_error_t const &totalError,
                                  size_t framesNum, std::ostream &stream) {
  stream << "         max error: " << totalError.maxError << std::endl;
  stream << "         avg error: " << totalError.sumError / totalError.numScores
         << std::endl;
  stream << "     avg rms error: " << totalError.sumRmsError / framesNum
         << std::endl;
  stream << "       stdev error: " << StdDevError(totalError) << std::endl
         << std::endl;
  stream << std::endl;
}
...
void printPerformanceCounters(
    std::map<std::string, InferenceEngine::InferenceEngineProfileInfo> const
        &utterancePerfMap,
    size_t callsNum, std::ostream &stream, std::string fullDeviceName) {
#if !defined(__arm__) && !defined(_M_ARM) && !defined(__aarch64__) &&          \
    !defined(_M_ARM64)
  ...
  for (const auto &it : utterancePerfMap) {
    ...
    stream << std::setw(30) << std::left
           << counter_name.substr(4, counter_name.size() - 1);
 ...
 * @param totalPerfCounters reference to a map to save total performance
 * counters
 * @return none.
 */
void sumPerformanceCounters(
    std::map<std::string, InferenceEngine::InferenceEngineProfileInfo> const
        &perfCounters,
    std::map<std::string, InferenceEngine::InferenceEngineProfileInfo>
        &totalPerfCounters) {
 ...
 * @param str reference to user-specified input scale factor for quantization,
...
std::vector<std::string> ParseScaleFactors(const std::string &str) {
  ...
  if (!str.empty()) {
    ...
    while (getline(stream, outStr, ',')) {
      ...
      if (floatScaleFactor <= 0.0f) {
        throw std::logic_error(
            "Scale factor for input #" + std::to_string(i) +
    ...
    throw std::logic_error("Scale factor need to be specified via -sf option "
                           "if you are using -q user");
  }
  return scaleFactorInput;
}

/**
 * @brief Parse string of file names separated by comma to save it to vector of
 * file names
...
bool ParseAndCheckCommandLine(int argc, char *argv[]) {
  ...
  if (FLAGS_i.empty() && (!isDumpMode || FLAGS_q.compare("user") != 0)) {
    showUsage();
    if (isDumpMode) {
      throw std::logic_error("In model dump mode either static quantization is "
                             "used (-i) or user scale"
  ...
  if (FLAGS_m.empty() && FLAGS_rg.empty()) {
    showUsage();
    throw std::logic_error(
        "Either IR file (-m) or GNAModel file (-rg) need to be set.");
  }
  ...
  if (std::find(supportedDevices.begin(), supportedDevices.end(), FLAGS_d) ==
  ...
  if ((FLAGS_q.compare("static") != 0) && (FLAGS_q.compare("dynamic") != 0) &&
      (FLAGS_q.compare("user") != 0)) {
    throw std::logic_error(
        "Quantization mode not supported (static, dynamic, user).");
  }
  ...
  if (FLAGS_nthreads <= 0) {
    throw std::logic_error("Invalid value for 'nthreads' argument. It must be "
                           "greater that or equal to 0");
  }

  if (FLAGS_cw_r < 0) {
    throw std::logic_error("Invalid value for 'cw_r' argument. It must be "
                           "greater than or equal to 0");
  }

  if (FLAGS_cw_l < 0) {
    throw std::logic_error("Invalid value for 'cw_l' argument. It must be "
                           "greater than or equal to 0");
  }

  if (FLAGS_pwl_me < 0.0 || FLAGS_pwl_me > 100.0) {
    throw std::logic_error("Invalid value for 'pwl_me' argument. It must be "
                           "greater than 0.0 and less than 100.0");
  }
  ...
  try {
    ...
    if (!FLAGS_i.empty()) {
      ...
      while (getline(stream, outStr, ',')) {
        ...
        GetKaldiArkInfo(filename.c_str(), 0, &currentNumUtterances,
                        &currentNumBytesThisUtterance);
        if (numUtterances == 0) {
          ...
          throw std::logic_error("Incorrect input files. Number of utterance "
                                 "must be the same for all ark files");
        }
    ...
    std::string deviceStr = useHetero && useGna
                                ? "HETERO:GNA,CPU"
                                : FLAGS_d.substr(0, (FLAGS_d.find("_")));

    slog::info << "Device info: " << slog::endl;
    std::cout << ie.GetVersions(deviceStr) << std::endl;
    // -----------------------------------------------------------------------------------------------------

    // --------------------------- Step 2. Read a model in OpenVINO Intermediate
    // Representation (.xml and .bin files) or ONNX (.onnx file) format
    slog::info << "Loading network files:" << slog::endl
               << FLAGS_m << slog::endl;

    uint32_t batchSize =
        (FLAGS_cw_r > 0 || FLAGS_cw_l > 0) ? 1 : (uint32_t)FLAGS_bs;
    ...
    if (!FLAGS_m.empty()) {
      ...
      // --------------------------- Set batch size
      // ---------------------------------------------------
      /** Set batch size.  Unlike in imaging, batching in time (rather than
       * space) is done for speech recognition. **/
    ...
    if (useGna) {
      std::string gnaDevice =
          useHetero ? FLAGS_d.substr(FLAGS_d.find("GNA"),
                                     FLAGS_d.find(",") - FLAGS_d.find("GNA"))
                    : FLAGS_d;
    ...
    if (FLAGS_pc) {
      genericPluginConfig[PluginConfigParams::KEY_PERF_COUNT] =
          PluginConfigParams::YES;
    }

    if (FLAGS_q.compare("user") == 0) {
      if (!FLAGS_rg.empty()) {
        slog::warn << "Custom scale factor will be ignored - using scale "
                      "factor from provided imported gna model: "
                   << FLAGS_rg << slog::endl;
      } else {
        auto scaleFactorInput = ParseScaleFactors(FLAGS_sf);
        if (numInputArkFiles != scaleFactorInput.size()) {
          std::string errMessage(
              "Incorrect command line for multiple inputs: " +
              std::to_string(scaleFactorInput.size()) +
        ...
        for (size_t i = 0; i < scaleFactorInput.size(); ++i) {
          slog::info << "For input " << i << " using scale factor of "
                     << scaleFactorInput[i] << slog::endl;
      ...
      if (!FLAGS_rg.empty()) {
        slog::info << "Using scale factor from provided imported gna model: "
                   << FLAGS_rg << slog::endl;
      } else {
        for (size_t i = 0; i < numInputArkFiles; i++) {
          ...
          uint32_t numArrays(0), numBytes(0), numFrames(0), numFrameElements(0),
              numBytesPerElement(0);
          GetKaldiArkInfo(inputArkName, 0, &numArrays, &numBytes);
          ptrFeatures.resize(numBytes);
          LoadKaldiArkArray(inputArkName, 0, name, ptrFeatures, &numFrames,
                            &numFrameElements, &numBytesPerElement);
          auto floatScaleFactor =
              ScaleFactorForQuantization(ptrFeatures.data(), MAX_VAL_2B_FEAT,
                                         numFrames * numFrameElements);
          slog::info << "Using scale factor of " << floatScaleFactor
                     << " calculated from first utterance." << slog::endl;
          std::string scaleFactorConfigKey = GNA_CONFIG_KEY(SCALE_FACTOR) +
                                             std::string("_") +
                                             std::to_string(i);
          gnaPluginConfig[scaleFactorConfigKey] =
              std::to_string(floatScaleFactor);
        }
      }
    }
    ...
    gnaPluginConfig[GNA_CONFIG_KEY(PWL_MAX_ERROR_PERCENT)] =
        std::to_string(FLAGS_pwl_me);
    // -----------------------------------------------------------------------------------------------------
    ...
    if (!FLAGS_we.empty()) {
      ...
      gnaPluginConfig
          [GNAConfigParams::KEY_GNA_FIRMWARE_MODEL_IMAGE_GENERATION] =
              FLAGS_we_gen;
    }
    // -----------------------------------------------------------------------------------------------------
    ...
    // ------------------------------------------
    if (useGna) {
      genericPluginConfig.insert(std::begin(gnaPluginConfig),
                                 std::end(gnaPluginConfig));
    }
    auto t0 = Time::now();
    ...
    if (!FLAGS_oname.empty()) {
      ...
      for (const auto &outBlobName : output_names) {
        ...
        if (pos_layer == -1) {
          throw std::logic_error(std::string("Output ") +
    ...
    if (!FLAGS_m.empty()) {
      ...
      executableNet =
          ie.ImportNetwork(FLAGS_rg.c_str(), deviceStr, genericPluginConfig);
    }
    ms loadTime = std::chrono::duration_cast<ms>(Time::now() - t0);
    slog::info << "Model loading time " << loadTime.count() << " ms"
               << slog::endl;
    ...
    if (!FLAGS_wg.empty()) {
      ...
      slog::info << "Exporting time " << exportTime.count() << " ms"
                 << slog::endl;
      return 0;
    }

    if (!FLAGS_we.empty()) {
      slog::info << "Exported GNA embedded model to file " << FLAGS_we
                 << slog::endl;
      if (!FLAGS_we_gen.empty()) {
        slog::info << "GNA embedded model export done for GNA generation: "
                   << FLAGS_we_gen << slog::endl;
      }
      return 0;
    }
    // ---------------------------------------------------------------------------------------------------------
    ...
    // --------------------------------------------------
    std::vector<InferRequestStruct> inferRequests(
        (FLAGS_cw_r > 0 || FLAGS_cw_l > 0) ? 1 : FLAGS_nthreads);
    ...
    // -------------------------------------------------- This step executed
    // after creating infer request to check input/output layers mentioned via
    // -iname and -oname args
    // --------------------------- Prepare input blobs
    // -----------------------------------------------------
    ...
    if (!FLAGS_iname.empty()) {
      ...
      if (inputNameBlobs.size() != cInputInfo.size()) {
        std::string errMessage(std::string("Number of network inputs ( ") +
                               std::to_string(cInputInfo.size()) +
      ...
      for (const auto &input : cInputInfo) {
        ptrInputBlobs.push_back(
            inferRequests.begin()->inferRequest.GetBlob(input.first));
      }
    }
    ...
    for (auto &item : inputInfo) {
      Precision inputPrecision =
          Precision::FP32; // specify Precision::I16 to provide quantized inputs
    ...
    if (!outputs.empty()) {
      ...
      for (auto &output : cOutputInfo) {
        ptrOutputBlob.push_back(
            inferRequests.begin()->inferRequest.GetBlob(output.first));
      }
    }
    ...
    for (auto &item : outputInfo) {
      ...
      Precision outputPrecision = Precision::FP32; // specify Precision::I32 to
                                                   // retrieve quantized outputs
    ...
    if (!FLAGS_r.empty()) {
      ...
      count_file =
          reference_name_files.empty() ? 1 : reference_name_files.size();
    }
    // ---------------------------------------------------------------------
    // -----------------------------------------------------------------------------------------------------
    ...
    for (size_t next_output = 0; next_output < count_file; next_output++) {
      ...
      for (uint32_t utteranceIndex = 0; utteranceIndex < numUtterances;
           ++utteranceIndex) {
        std::map<std::string, InferenceEngine::InferenceEngineProfileInfo>
            utterancePerfMap;
        std::string uttName;
        ...
        auto dims = outputs.empty()
                        ? cOutputInfo.rbegin()->second->getDims()
        ...
        for (size_t i = 0; i < inputArkFiles.size(); i++) {
          ...
          uint32_t currentNumFrames(0), currentNumFrameElementsInput(0),
              currentNumBytesPerElementInput(0);
          GetKaldiArkInfo(inputArkFilename, utteranceIndex, &n,
                          &numBytesThisUtterance[i]);
          ptrUtterance.resize(numBytesThisUtterance[i]);
          LoadKaldiArkArray(inputArkFilename, utteranceIndex, uttName,
                            ptrUtterance, &currentNumFrames,
          ...
          if (numFrames == 0) {
            ...
            std::string errMessage(
                "Number of frames in ark files is different: " +
                std::to_string(numFrames) + " and " +
        ...
        for (auto &ptrInputBlob : ptrInputBlobs) {
          if (ptrInputBlob->size() != numFrameElementsInput[i++] * batchSize) {
            throw std::logic_error(
                "network input size(" + std::to_string(ptrInputBlob->size()) +
        ...
        if (!FLAGS_r.empty()) {
          ...
          GetKaldiArkInfo(reference_name_files[next_output].c_str(),
                          utteranceIndex, &n,
          ...
          LoadKaldiArkArray(
              ...
              refUtteranceName, ptrReferenceScores, &numFramesReference,
        ...
        while (frameIndex <= numFrames) {
          if (frameIndex == numFrames) {
            if (std::find_if(inferRequests.begin(), inferRequests.end(),
                             [&](InferRequestStruct x) {
          ...
          for (auto &inferRequest : inferRequests) {
            if (frameIndex == numFrames) {
              ...
              numFramesThisBatch = (numFrames - frameIndex < batchSize)
            ...
            if (inferRequest.frameIndex != -1) {
              ...
              if (inferRequest.frameIndex >= 0) {
                if (!FLAGS_o.empty()) {
                  ...
                  if (!outputs.empty()) {
                    newOutputInfo[outputs[next_output]] =
                        cOutputInfo[outputs[next_output]];
                  } else {
                    newOutputInfo = cOutputInfo;
                  }
                  Blob::Ptr outputBlob = inferRequest.inferRequest.GetBlob(
                      newOutputInfo.rbegin()->first);
                  MemoryBlob::CPtr moutput = as<MemoryBlob>(outputBlob);

                  if (!moutput) {
                    throw std::logic_error(
                        "We expect output to be inherited from MemoryBlob, "
                        "but in fact we were not able to cast output to "
                        "MemoryBlob");
                  }
                  ...
                  std::memcpy(outputFrame, moutputHolder.as<const void *>(),
                              byteSize);
                }
                if (!FLAGS_r.empty()) {
                  ...
                  CompareScores(
                      ...
                      &frameError, inferRequest.numFramesThisBatch,
                      numFrameElementsReference);
                  UpdateScoreError(&frameError, &totalError);
                }
                if (FLAGS_pc) {
                  // retrieve new counters
                  getPerformanceCounters(inferRequest.inferRequest,
                                         callPerfMap);
            ...
            if (FLAGS_iname.empty()) {
              for (auto &input : cInputInfo) {
                ptrInputBlobs.push_back(
                    inferRequest.inferRequest.GetBlob(input.first));
              }
            } else {
              std::vector<std::string> inputNameBlobs =
                  ConvertStrToVector(FLAGS_iname);
              for (const auto &input : inputNameBlobs) {
                Blob::Ptr blob =
                    inferRequests.begin()->inferRequest.GetBlob(input);
                if (!blob) {
                  std::string errMessage("No blob with name : " + input);
                  throw std::logic_error(errMessage);
                }
            ...
            for (size_t i = 0; i < numInputArkFiles; ++i) {
              ...
              if (!minput) {
                std::string errMessage("We expect ptrInputBlobs[" +
                                       std::to_string(i) +
                                       "] to be inherited from MemoryBlob, " +
                                       "but in fact we were not able to cast "
                                       "input blob to MemoryBlob");
                throw std::logic_error(errMessage);
              }
              // locked memory holder should be alive all time while access to
              // its buffer happens
              auto minputHolder = minput->wmap();

              std::memcpy(minputHolder.as<void *>(), inputFrame[i],
                          minput->byteSize());
            }
            // -----------------------------------------------------------------------------------------------------

            int index =
                static_cast<int>(frameIndex) - (FLAGS_cw_l + FLAGS_cw_r);
            ...
            for (size_t j = 0; j < inputArkFiles.size(); j++) {
              if (FLAGS_cw_l > 0 || FLAGS_cw_r > 0) {
                ...
                if (idx > 0 && idx < static_cast<int>(numFramesArkFile)) {
                  inputFrame[j] += sizeof(float) * numFrameElementsInput[j] *
                                   numFramesThisBatch;
                } else if (idx >= static_cast<int>(numFramesArkFile)) {
                  inputFrame[j] = &ptrUtterances[j].front() +
                                  (numFramesArkFile - 1) * sizeof(float) *
        ...
        if (!FLAGS_o.empty()) {
          ...
          SaveKaldiArkArray(output_name_files[next_output].c_str(),
        ...
        std::cout << "Frames in utterance:\t\t\t" << numFrames << " frames"
                  << std::endl;
        std::cout << "Average Infer time per frame:\t\t"
                  << totalTime / static_cast<double>(numFrames) << " ms"
                  << std::endl;
        if (FLAGS_pc) {
        ...
        std::cout << "End of Utterance " << utteranceIndex << std::endl
                  << std::endl;
        // -----------------------------------------------------------------------------------------------------
      }
    }
