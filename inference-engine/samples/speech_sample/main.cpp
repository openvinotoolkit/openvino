                     ...
                     uint32_t* ptrNumMemoryBytes)
{
    ...
    if (in_file.good())
    {
        while (!in_file.eof())
        {
            ...
            std::getline(
                in_file, line, '\0'); // read variable length name followed by space and NUL
 ...
 * @return none.
 */
void LoadKaldiArkArray(const char* fileName,
                       uint32_t arrayIndex,
                       std::string& ptrName,
                       ...
                       uint32_t* ptrNumBytesPerElement)
{
    ...
    if (in_file.good())
    {
        ...
        if (!in_file.eof())
        {
            std::string line;
            std::getline(
                in_file, ptrName, '\0'); // read variable length name followed by space and NUL
            std::getline(in_file, line, '\4'); // read "BFM" followed by space and control-D
            if (line.compare("BFM ") != 0)
            {
                fprintf(stderr,
...
float getGnaFrequencyMHz()
{
    ...
    if (family == sixth_family)
    {
        ...
        case ice_lake_model:
        case next_model: return 400;
        case gemini_lake_model: return 200;
        default: return 1;
        }
    }
    else
...
void sumPerformanceCounters(
    std::map<std::string, InferenceEngine::InferenceEngineProfileInfo> const& perfCounters,
 ...
 * @param str reference to user-specified input scale factor for quantization, can be separated by
 * comma
...
std::vector<std::string> ParseScaleFactors(const std::string& str)
{
    ...
    else
    {
        throw std::logic_error(
            "Scale factor need to be specified via -sf option if you are using -q user");
    }
...
bool ParseAndCheckCommandLine(int argc, char* argv[])
{
    ...
    if (FLAGS_i.empty() && (!isDumpMode || FLAGS_q.compare("user") != 0))
    {
        ...
        {
            throw std::logic_error(
                "In model dump mode either static quantization is used (-i) or user scale"
    ...
    {
        throw std::logic_error(
            "Invalid value for 'nthreads' argument. It must be greater that or equal to 0");
    }

    if (FLAGS_cw_r < 0)
    {
        throw std::logic_error(
            "Invalid value for 'cw_r' argument. It must be greater than or equal to 0");
    }

    if (FLAGS_cw_l < 0)
    {
        throw std::logic_error(
            "Invalid value for 'cw_l' argument. It must be greater than or equal to 0");
    }

    if (FLAGS_pwl_me < 0.0 || FLAGS_pwl_me > 100.0)
    {
        throw std::logic_error(
            "Invalid value for 'pwl_me' argument. It must be greater than 0.0 and less than 100.0");
    }
    ...
    try
    {
        ...
        if (!FLAGS_i.empty())
        {
            ...
            while (getline(stream, outStr, ','))
            {
                ...
                GetKaldiArkInfo(
                    filename.c_str(), 0, &currentNumUtterances, &currentNumBytesThisUtterance);
                if (numUtterances == 0)
                {
                ...
                {
                    throw std::logic_error(
                        "Incorrect input files. Number of utterance must be the same for all ark "
                        "files");
                }
        ...
        if (!FLAGS_l.empty())
        {
            // Custom CPU extension is loaded as a shared library and passed as a pointer to base
            // extension
        ...
        if (!FLAGS_m.empty())
        {
            ...
            // --------------------------- Set batch size
            // ---------------------------------------------------
            /** Set batch size.  Unlike in imaging, batching in time (rather than space) is done for
             * speech recognition. **/
        ...
        if (useGna)
        {
            std::string gnaDevice =
                useHetero
                    ? FLAGS_d.substr(FLAGS_d.find("GNA"), FLAGS_d.find(",") - FLAGS_d.find("GNA"))
                    : FLAGS_d;
        ...
        if (FLAGS_q.compare("user") == 0)
        {
            if (!FLAGS_rg.empty())
            {
                slog::warn << "Custom scale factor will be ignored - using scale factor from "
                              "provided imported gna model: "
                           << FLAGS_rg << slog::endl;
            }
            else
            {
        ...
        else
        {
            ...
            if (!FLAGS_rg.empty())
            {
                slog::info << "Using scale factor from provided imported gna model: " << FLAGS_rg
                           << slog::endl;
            }
            else
            {
                for (size_t i = 0; i < numInputArkFiles; i++)
                {
                    ...
                    LoadKaldiArkArray(inputArkName,
                                      ...
                                      &numFrameElements,
                                      &numBytesPerElement);
                    auto floatScaleFactor = ScaleFactorForQuantization(
                        ptrFeatures.data(), MAX_VAL_2B_FEAT, numFrames * numFrameElements);
        ...
        // --------------------------- Write model to file
        // -------------------------------------------------- Embedded GNA model dumping (for
        // Intel(R) Speech Enabling Developer Kit)
        if (!FLAGS_we.empty())
        {
            gnaPluginConfig[GNAConfigParams::KEY_GNA_FIRMWARE_MODEL_IMAGE] = FLAGS_we;
            gnaPluginConfig[GNAConfigParams::KEY_GNA_FIRMWARE_MODEL_IMAGE_GENERATION] =
                FLAGS_we_gen;
        }
        // -----------------------------------------------------------------------------------------------------
        ...
        if (!FLAGS_we.empty())
        {
            ...
            {
                slog::info << "GNA embedded model export done for GNA generation: " << FLAGS_we_gen
                           << slog::endl;
            }
            return 0;
        }
        ...
        // -------------------------------------------------- This step executed after creating
        // infer request to check input/output layers mentioned via -iname and -oname args
        // --------------------------- Prepare input blobs
        // -----------------------------------------------------
        ...
        if (!FLAGS_iname.empty())
        {
            ...
            if (inputNameBlobs.size() != cInputInfo.size())
            {
                std::string errMessage(
                    std::string("Number of network inputs ( ") + std::to_string(cInputInfo.size()) +
        ...
        for (auto& item : outputInfo)
        {
            ...
            Precision outputPrecision =
                Precision::FP32; // specify Precision::I32 to retrieve quantized outputs
        ...
        for (size_t next_output = 0; next_output < count_file; next_output++)
        {
            ...
            for (uint32_t utteranceIndex = 0; utteranceIndex < numUtterances; ++utteranceIndex)
            {
                ...
                uint32_t numFramesReference(0), numFrameElementsReference(0),
                    numBytesPerElementReference(0), numBytesReferenceScoreThisUtterance(0);
                auto dims = outputs.empty() ? cOutputInfo.rbegin()->second->getDims()
                                            : cOutputInfo[outputs[next_output]]->getDims();
                const auto numScoresPerFrame = std::accumulate(
                ...
                for (size_t i = 0; i < inputArkFiles.size(); i++)
                {
                    ...
                    GetKaldiArkInfo(
                        inputArkFilename, utteranceIndex, &n, &numBytesThisUtterance[i]);
                    ...
                    {
                        std::string errMessage("Number of frames in ark files is different: " +
                ...
                while (frameIndex <= numFrames)
                {
                    ...
                    for (auto& inferRequest : inferRequests)
                    {
                        ...
                        if (inferRequest.frameIndex != -1)
                        {
                            ...
                            if (inferRequest.frameIndex >= 0)
                            {
                                if (!FLAGS_o.empty())
                                {
                                    ...
                                    std::memcpy(
                                        outputFrame, moutputHolder.as<const void*>(), byteSize);
                                }
                                if (!FLAGS_r.empty())
                                {
                        ...
                        for (size_t i = 0; i < numInputArkFiles; ++i)
                        {
                            ...
                            if (!minput)
                            {
                                std::string errMessage("We expect ptrInputBlobs[" +
                                                       ...
                                                       "but in fact we were not able to cast input "
                                                       "blob to MemoryBlob");
                                throw std::logic_error(errMessage);
                            }
                            ...
                            std::memcpy(
                                minputHolder.as<void*>(), inputFrame[i], minput->byteSize());
                        }
                        // -----------------------------------------------------------------------------------------------------
                        ...
                        for (size_t j = 0; j < inputArkFiles.size(); j++)
                        {
                            ...
                            {
                                inputFrame[j] +=
                                    sizeof(float) * numFrameElementsInput[j] * numFramesThisBatch;
                            }
                        }
                ...
                std::cout << "Average Infer time per frame:\t\t"
                          << totalTime / static_cast<double>(numFrames) << " ms" << std::endl;
                if (FLAGS_pc)
                {
                    // print performance results
                    printPerformanceCounters(
                        utterancePerfMap, frameIndex, std::cout, getFullDeviceName(ie, FLAGS_d));
                }
                if (!FLAGS_r.empty())
                {
