...
bool ParseAndCheckCommandLine(int argc, char *argv[]) {
  ...
  if (FLAGS_nt <= 0) {
    throw std::logic_error(
        "Incorrect value for nt argument. It should be greater than 0.");
  }
...
int main(int argc, char *argv[]) {
  try {
    ...
    for (auto &item : inputInfo) {
      ...
      auto data =
          minputHolder.as<PrecisionTrait<Precision::U8>::value_type *>();
      if (data == nullptr)
    ...
    inferRequest.SetCompletionCallback([&] {
      curIteration++;
      slog::info << "Completed " << curIteration << " async request execution"
                 << slog::endl;
    ...
    slog::info << "Start inference (" << numIterations
               << " asynchronous executions)" << slog::endl;
    inferRequest.StartAsync();
    ...
    if (FLAGS_nt > resultsCnt || FLAGS_nt < 1) {
      slog::warn
          ...
          << resultsCnt << " will be used." << slog::endl;
      FLAGS_nt = resultsCnt;
    }
