...
def print_user_info(info, info_level=None):
    ...
    if not info:
        raise LdapApiException('ERROR: No info')
...
class LdapApi:
    ...
    _check_existing = ['intelExportCountryGroup', 'physicalDeliveryOfficeName']
    ...
    def get_user_emails(self, groups=None):
        ...
        def process_group_members(member):
            ...
            if 'OU=Workers' in member:
                if self.connection.response[0]['attributes']['mail']:
                    mails.add(self.connection.response[0]['attributes']['mail'].lower())
                    return
            ...
            if self.connection.response[0]['attributes']['member']:
                for group_member in self.connection.response[0]['attributes']['member']:
                    process_group_members(group_member)
            else:
                raise LdapApiException(f'ERROR: no member. LDAP member: {member}\n'
                                       f'{self.connection.entries}')
    ...
    def get_user_info_by_email(self, email):
        ...
        search_filter = f'(&(objectcategory=person)(objectclass=user)(intelflags=1)(mail={email}))'

        for domain in self._cfg.LDAP_DOMAINS:
            search_base = f'OU=Workers,DC={domain},DC=corp,DC=intel,DC=com'
            self.connection.search(search_base, search_filter, SUBTREE, attributes=['*'])

            if self.connection.response:
                if len(self.connection.response) > 1:
                    raise LdapApiException(f'ERROR: multiple responses for {email}: '
                                           f'{len(self.connection.response)}\n'
                                           f'{self.connection.entries}')
