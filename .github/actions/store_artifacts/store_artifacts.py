...
def main():
    ...
    storage = Path(storage_root) / 'dldt' / event_type / commit_hash / args.storage_dir
    ...
    if github_server:  # If running from GHA context
        ...
        job_link = f"{github_server}/{repository}/actions/runs/{run_id}/job/{job_id}"
        with open(storage / 'job_link.txt', 'w') as file:
            file.write(job_link)
