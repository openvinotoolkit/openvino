...
def main():
    ...
    storage_branch_dir = artifact_utils.get_storage_branch_dir(args.storage_root, args.branch_name)
    latest_artifacts_for_branch = storage_branch_dir / f"latest_{args.storage_dir}"
    # Overwrite path to "latest" built artifacts only if a given commit is the head of a given PR or branch
    if args.commit_sha == os.getenv('PR_HEAD_SHA') or args.commit_sha == os.getenv('GITHUB_SHA'):
