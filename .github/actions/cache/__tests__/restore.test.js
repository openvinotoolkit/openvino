...
describe('restore', () => {
  beforeEach(() => {
    ...
    tar.c(
        {gzip : true, file : cacheFiles[0], cwd : cacheTmpPath, sync : true},
        [ '.' ]);
    ...
    tar.c(
        {gzip : true, file : cacheFiles[1], cwd : cacheTmpPath, sync : true},
        [ '.' ]);
    ...
    tar.c(
        {gzip : true, file : cacheFiles[2], cwd : cacheTmpPath, sync : true},
        [ '.' ]);
  ...
  it('gets the cache file', async () => {
    ...
    expect(setOutputMock)
        .toHaveBeenNthCalledWith(1, 'cache-file', cacheFiles[2]);
  ...
  it('gets the updated cache file', async () => {
    ...
    // Verify that all of the core library functions were called correctly
    expect(setOutputMock)
        .toHaveBeenNthCalledWith(1, 'cache-file', cacheFiles[1]);
    expect(setOutputMock).toHaveBeenNthCalledWith(2, 'cache-hit', true);
    let id = 1;
