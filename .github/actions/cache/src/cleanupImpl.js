...
const {getSortedCacheFiles, humanReadableFileSize, calculateTotalSize} =
    require('./utils');
...
async function cleanUp() {
  try {
    ...
    const keysRestore = core.getInput('restore-keys', {required : false})
                            .split('\n')
    ...
    if (totalSize <= minCacheSizeInBytes) {
      core.info(`The cache storage size ${
          humanReadableFileSize(totalSize)} less then the allowed size ${
          humanReadableFileSize(minCacheSizeInBytes)}`);
      return;
    }

    core.info(`The cache storage size ${
        humanReadableFileSize(totalSize)} exceeds allowed size ${
        humanReadableFileSize(minCacheSizeInBytes)}`);
    for (const file of files.reverse()) {
      const filePath = path.join(cacheRemotePath, file);
      const fileStats = await fs.stat(filePath);

      // skipping recently used files if total cache size less then
      // maxCacheSizeInBytes
      if (!fileStats.isFile() ||
          (fileStats.atime >= minAccessDateAgo &&
           totalSize < maxCacheSizeInBytes)) {
