...
const {getSortedCacheFiles, humanReadableFileSize, checkFileExists} =
    require('./utils');
...
async function restore() {
  try {
    ...
    if (files.length) {
      ...
      core.info(`Found cache file: ${cacheFile}, size: ${
          humanReadableFileSize(cacheStat.size)}`);
      ...
      core.warning(`Could not found any suitable cache files in ${
          cacheRemotePath} with key ${key}`);
