...
class ComponentConfig:
    ...
    def __init__(self, config: dict, schema: dict, all_possible_components: set):
        ...
        self.all_defined_components = set(
            self.config.keys()
        )  # already defined in components.yml
        # can be added to components.yml (based on labeler.yml)
        self.all_possible_components = all_possible_components.union(
            self.all_defined_components
        )
    ...
    def validate(self, schema: dict) -> None:
        ...
        for component_name, data in self.config.items():
            ...
            for key in self.ScopeKeys:
                scope = data.get(key)
                dependent_components = dependent_components.union(
                    set(scope) if isinstance(scope, list) else set()
                )

            invalid_dependents = dependent_components.difference(
                self.all_possible_components
            )
            if invalid_dependents:
                error_msg = (
                    f"dependent components of {component_name} are invalid: "
    ...
    def get_affected_components(self, changed_components_names: set) -> dict:
        ...
        changed_not_defined_components = changed_components_names.difference(
            self.all_defined_components
        )
        if not changed_components_names or changed_not_defined_components:
            self.log.info(
                f"Changed components {changed_not_defined_components} are not defined in smart ci config, "
                "run full scope"
            )
        ...
        for name in changed_components_names:
            component_scopes = {
                k: v
                for k, v in self.config.get(name, dict()).items()
                if k in self.ScopeKeys
            }
            for key, dependents in component_scopes.items():
                if dependents == "all":
                    dependents = self.all_possible_components
                for dep_name in dependents:
                    affected_components[dep_name] = affected_components.get(
                        dep_name, set()
                    )
                    scope = self.FullScope if key == "revalidate" else {key}
                    affected_components[dep_name] = affected_components[dep_name].union(
                        scope
                    )

            if not component_scopes:
                self.log.info(
                    f"Changed component '{name}' doesn't have {self.ScopeKeys} keys in components config. "
        ...
        affected_components.update(
            {name: self.FullScope for name in changed_components_names}
        )
        ...
        affected_components.update(
            {
                name: self.FullScope
                for name in self.all_possible_components
    ...
    def get_static_data(
        self, components_names: set, data_key: str, default: str = None
    ) -> dict:
        """Returns requested generic static data defined for each component"""
        data = {
            name: self.config[name].get(data_key, default) for name in components_names
        }
        return data
...
def get_changed_component_names(
    pr, all_possible_components: set, component_pattern: str = None
) -> set:
...
def get_changeset(gh_api, pr, target_branch, commit_sha):
    ...
    if target_branch:
        ...
        # commit_sha below
        changed_files = gh_api.repos.compare_commits(
            f"{target_branch_head_commit}...{commit_sha}"
        ).get("files", [])
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Returns product components changed in a given PR or commit"
    )
    parser.add_argument(
        "--pr", type=int, required=False, help="PR number. If not set, --commit is used"
    )
    parser.add_argument(
        "-s",
        "--commit-sha",
        required=False,
        help="Commit SHA. If not set, --pr is used",
    )
    ...
    parser.add_argument(
        ...
        "--pattern",
        default=None,
        help="Pattern to extract component name from PR label. "
...
def set_github_output(
    name: str, value: str, github_output_var_name: str = "GITHUB_OUTPUT"
):
...
def main():
    ...
    merge_queue_target_branch = (
        re.findall(f"^{merge_queue_prefix}(.*)/", args.ref_name)[0]
        if is_merge_queue
        else None
    )
    ...
    if is_merge_queue:
        logger.info(
            f"The run is a merge-queue run, executing full validation scope for all components, if "
            f"not all queued changes match patterns in 'skip-when-only-listed-files-changed'"
        )
        run_full_scope = True
    elif is_postcommit:
        logger.info(
            f"The run is a post-commit run, executing full validation scope for all components"
        )
        run_full_scope = True
    else:
        ...
        if no_match_files_changed:
            logger.info(
                f"There are changed files that don't match any pattern in labeler config, "
    ...
    changed_component_names = (
        set(all_defined_components)
        if run_full_scope
        else get_changed_component_names(pr, all_possible_components, args.pattern)
    )
    ...
    if is_merge_queue or (args.pr and not run_full_scope):
        ...
        if not skip_workflow and args.skip_when_only_listed_files_changed:
            ...
            changed_files = get_changeset(
                gh_api, args.pr, merge_queue_target_branch, args.commit_sha
            )
            patterns = set(args.skip_when_only_listed_files_changed.split(","))

            matched_files_only = all(
                any(fnmatch(f.filename, pattern) for pattern in patterns)
    ...
    affected_components_output = {
        name: {s: True for s in scope} for name, scope in affected_components.items()
    }
