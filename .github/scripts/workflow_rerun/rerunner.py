...
if __name__ == "__main__":
    ...
    if is_dry_run:
        LOGGER.info(
            "RUNNING IN DRY RUN MODE. IF ERROR WILL BE FOUND, WILL NOT RETRIGGER"
        )
    ...
    LOGGER.info(
        f"CHECKING IF RERUN IS NEEDED FOR {run.html_url} RUN IN {repository_name}."
    )
    ...
    if run.run_attempt > 1:
        LOGGER.info(
            f"THERE ARE {run.run_attempt} ATTEMPTS ALREADY. NOT CHECKING LOGS AND NOT RETRIGGERING. EXITING"
        )
        sys.exit(0)
    ...
    log_analyzer = LogAnalyzer(
        path_to_log_archive=log_archive_path, path_to_errors_file=errors_file
    )
    ...
    if log_analyzer.found_matching_error:
        ...
        with open(file=os.environ["GITHUB_ENV"], mode="a") as fh:
