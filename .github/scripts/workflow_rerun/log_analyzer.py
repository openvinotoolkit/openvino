...
class LogAnalyzer:
    def __init__(self, path_to_log_archive: Path, path_to_errors_file: Path) -> None:
        ...
        all_txt_log_files_pretty = "\n".join(
            map(lambda item: str(item["path"]), self._log_files)
        )
    ...
    def _collect_errors_to_look_for(self) -> None:
        with open(
            file=self._path_to_errors_file, mode="r", encoding="utf-8"
        ) as errors_file:
            errors_data = json.load(errors_file)
            for error_data in errors_data:
                self._errors_to_look_for.append(
                    ErrorData(
                        error_text=error_data["error_text"], ticket=error_data["ticket"]
                    )
                )
    ...
    def _collect_log_files(self) -> None:
        ...
        with ZipFile(file=self._path_to_log_archive, mode="r") as zip_file:
            ...
            if _file.is_dir():
                for log_file in _file.iterdir():
                    self._log_files.append(
                        LogFile(file_name=log_file.name, path=log_file.resolve())
                    )

    def _is_error_in_log(self, error_to_look_for: str, log_file_path: Path) -> bool:
        """
    ...
    def analyze(self) -> None:
        ...
        for error in self._errors_to_look_for:
            ...
            for log_file in self._log_files:
                if self._is_error_in_log(
                    error_to_look_for=error["error_text"],
                    log_file_path=log_file["path"],
                ):
                    LOGGER.info(
                        f'FOUND "{error["error_text"]}" ERROR IN {log_file["path"]}. TICKET: {error["ticket"]}'
                    )
...
if __name__ == "__main__":
    ...
    log_analyzer = LogAnalyzer(
        path_to_log_archive=Path("/tmp/logs/log.zip"),
