...
class LogAnalyzer:
    def __init__(self, 
        ...
        self._collect_log_files_from_log_dir()
    ...
    def _collect_log_files_from_log_dir(self) -> None:
        """
        Collects the .txt log files from the log directory

        The GitHub Actions pipeline logs should have the following structure:
            > Job_name_0
                > step_name_0.txt
                > step_name_1.txt
                ...
        ...
        for _file in Path(self._log_dir).iterdir():
            if _file.is_dir():
                for log_file in _file.iterdir():
                    self._log_files.append(LogFile(file_name=log_file.name, path=log_file.resolve()))
    ...
    def analyze(self) -> None:
        """
        Iterates over all the known errors and tries to find them in the collected log files
        """
