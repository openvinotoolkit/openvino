...
def main():
    ...
    if run.status != "completed":
        logger.error(
            "Run %s is not completed! Only completed runs should be in the database",
            run_id,
        )
        raise SystemExit(1)
    ...
    workflow_query_args = (
        ...
        run.event,
        run.run_attempt,
        run.raw_data["repository"]["full_name"],
    ...
    logger.debug(
        "Workflow run query: %s", cur.mogrify(workflow_data_query, workflow_query_args)
    )
    ...
    for job in run.jobs():
        ...
        if job_created_at_date > job.started_at:
            logger.warning(
                "Skipping job %s of run %s - most likely a stub \
            job created after workflow restart",
                job.name,
                run_id,
            )
            continue
        ...
        job_query_args = (
            ...
            job.html_url,
            job.name,
            job.raw_data["created_at"],
            job.started_at,
            job.completed_at,
            queued_duration_seconds,
            duration_seconds,
            job.raw_data["runner_name"],
            job.status,
            job.conclusion,
            job.raw_data["head_branch"],
            job.raw_data["run_attempt"],
            job.raw_data["workflow_name"],
        )
        ...
        for step in job.steps:
            ...
            if step.status != "completed":
                logger.warning(
                    "Step %s isn't  completed. Setting duration to zero.", step.name
                )
                step_duration_seconds = 0
            else:
                step_duration_seconds_timedelta = step.completed_at - step.started_at
                step_duration_seconds = round(
                    step_duration_seconds_timedelta.total_seconds()
                )
            ...
            logger.debug(
                "Step query: %s", cur.mogrify(step_data_query, step_query_args)
            )
