#!/usr/bin/env python3
"""Pre-download failed CI logs and pre-locate error hints for the CI Doctor.

Used by the shared pre-agent step of the CI Doctor workflows
(.github/workflows/shared/agentic-workflows/download-failure-logs.md). Auto-
detects its mode from the environment:
  - run mode  (RUN_ID set):    analyse a single workflow run.
  - pr mode   (PR_NUMBER set):  analyse every failed run on a PR head commit.
