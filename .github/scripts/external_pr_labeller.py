...
if __name__ == "__main__":
    ...
    LOGGER.info(
        f"CONTEXT: PR #{pr_number}. USER: {pr.user.login}. ALL PR LABELS: {list(pr.get_labels())}"
    )
    ...
    if not gh_repo.has_in_collaborators(pr.user.login):
        ...
        for label in pr.get_labels():
            if label.name in (
                ExternalPRLabels.ExternalPR,
                ExternalPRLabels.ExternalIntelPR,
            ):
            ...
            label_to_add: str = (
                ExternalPRLabels.ExternalIntelPR.name
                if is_intel_user
                else ExternalPRLabels.ExternalPR.name
            )
