...
agent/<component>-<short-desc>      # general
```
...
Every agent-generated PR **must** open with the following notice block (exact
wording, first lines of the body before any other content):
...
After the notice, follow the upstream `pull_request_template.md` structure:
...
Write this body to `agent-results/<agent>/pr_body.md` before calling `gh pr create`.

### 5. Create branch, commit, and open draft PR
...
gh pr create --draft \
  ...
  --body-file agent-results/<agent>/pr_body.md
```
