...
image in the `NV12` color format to an OpenVINO Runtime plugin. Then, the sample creates an synchronous inference request object. When inference is done, the application outputs data to the standard output stream. You can place labels in the `.labels` file near the model to get pretty output.
...
>
> - By default, this sample expects that model input has the `BGR` channel order. If you trained your
...
4. Do inference of the `NV12` image, using the `alexnet` model on `CPU`, for example:
