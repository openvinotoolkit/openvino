...
int main(int argc, char *argv[]) {
  try {
    ...
    // ----------------------------------------------
    slog::info << "OpenVINO runtime: " << ov::get_openvino_version()
               << slog::endl;
    ...
    if (!FLAGS_i.empty()) {
      ...
      while (getline(stream, outStr, ',')) {
        ...
        file->get_file_info(filename.c_str(), 0, &currentNumUtterances,
    ...
    if (!FLAGS_oname.empty()) {
      std::vector<std::string> output_names =
          convert_str_to_vector(FLAGS_oname);
      for (const auto &output_name : output_names) {
        auto pos_layer = output_name.rfind(":");
        if (pos_layer == std::string::npos) {
          throw std::logic_error("Output " + output_name +
                                 " doesn't have a port");
        }
    ...
    if (FLAGS_q.compare("user") == 0) {
      if (!FLAGS_rg.empty()) {
        slog::warn
            << "Custom scale factor will be used for imported gna model: "
            << FLAGS_rg << slog::endl;
      }
      ...
      if (!FLAGS_rg.empty()) {
        ...
        for (size_t i = 0; i < numInputFiles; i++) {
          ...
          file->load_file(inputFileName, 0, name, ptrFeatures, &numFrames,
                          &numFrameElements, &numBytesPerElement);
          auto floatScaleFactor =
              scale_factor_for_quantization(ptrFeatures.data(), MAX_VAL_2B_FEAT,
    ...
    if (FLAGS_qb == 8) {
      gnaPluginConfig[InferenceEngine::GNAConfigParams::KEY_GNA_PRECISION] =
          "I8";
    } else {
      gnaPluginConfig[InferenceEngine::GNAConfigParams::KEY_GNA_PRECISION] =
          "I16";
    }
    gnaPluginConfig[InferenceEngine::GNAConfigParams::KEY_GNA_EXEC_TARGET] =
        FLAGS_exec_target;
    gnaPluginConfig[InferenceEngine::GNAConfigParams::KEY_GNA_COMPILE_TARGET] =
        FLAGS_compile_target;
    ...
    if (!FLAGS_we.empty()) {
      IE_SUPPRESS_DEPRECATED_START
      gnaPluginConfig
          [InferenceEngine::GNAConfigParams::KEY_GNA_FIRMWARE_MODEL_IMAGE] =
              FLAGS_we;
      IE_SUPPRESS_DEPRECATED_END
    }
    // -----------------------------------------------------------------------------------------------------
    ...
    if (!FLAGS_m.empty()) {
      ...
      std::ifstream streamrq(FLAGS_rg,
                             std::ios_base::binary | std::ios_base::in);
      if (!streamrq.is_open()) {
        throw std::runtime_error("Cannot open model file " + FLAGS_rg);
      }
      executableNet =
          core.import_model(streamrq, deviceStr, genericPluginConfig);
    }
    ...
    if (!FLAGS_iname.empty()) {
      std::vector<std::string> inputNameBlobs =
          convert_str_to_vector(FLAGS_iname);
      ...
      for (const auto &input : cInputInfo) {
        ptrInputBlobs.push_back(
            inferRequests.begin()->inferRequest.get_tensor(input));
      }
    }
    ...
    for (uint32_t utteranceIndex = 0; utteranceIndex < numUtterances;
      ...
      for (size_t i = 0; i < inputFiles.size(); i++) {
        ...
        file->get_file_info(inputFilename, utteranceIndex, &n,
                            &numBytesThisUtterance[i]);
        ptrUtterance.resize(numBytesThisUtterance[i]);
        file->load_file(inputFilename, utteranceIndex, uttName, ptrUtterance,
                        &currentNumFrames, &currentNumFrameElementsInput,
                        &currentNumBytesPerElementInput);
        if (numFrames == 0) {
          numFrames = currentNumFrames;
        } else if (numFrames != currentNumFrames) {
          std::string errMessage(
              "Number of frames in input files is different: " +
      ...
      for (auto &ptrInputBlob : ptrInputBlobs) {
        if (ptrInputBlob.get_size() != numFrameElementsInput[i++] * batchSize) {
          throw std::logic_error(
              "network input size(" + std::to_string(ptrInputBlob.get_size()) +
      ...
      if (!FLAGS_r.empty()) {
        ...
        for (size_t next_output = 0; next_output < count_file; next_output++) {
          if (fileReferenceScores != nullptr) {
            fileReferenceScores->get_file_info(
                reference_name_files[next_output].c_str(), utteranceIndex, &n,
                &numBytesReferenceScoreThisUtterance);
            ptrReferenceScores.resize(numBytesReferenceScoreThisUtterance);
            fileReferenceScores->load_file(
        ...
        for (auto &inferRequest : inferRequests) {
            ...
            if (inferRequest.frameIndex >= 0)
              for (size_t next_output = 0; next_output < count_file;
                ...
                if (FLAGS_pc) {
                  ...
                  get_performance_counters(inferRequest.inferRequest,
                                           callPerfMap);
                  // summarize retrieved counters with all previous
                  sum_performance_counters(callPerfMap, utterancePerfMap,
          ...
          for (int i = 0; i < executableNet.inputs().size(); i++) {
            inferRequest.inferRequest.set_input_tensor(
                i, ov::Tensor(ov::element::f32,
                              executableNet.inputs()[i].get_shape(),
                              inputFrame[i]));
          }
      ...
      if (FLAGS_pc) {
        ...
        print_performance_counters(utterancePerfMap, frameIndex, std::cout,
                                   getFullDeviceName(core, FLAGS_d),
                                   totalNumberOfRunsOnHw, FLAGS_d);
      }
      if (!FLAGS_r.empty()) {
