...
ov::Tensor create_tensor_from_numpy(const std::vector<std::string>& files,
    ...
    for (size_t b = 0; b < numpyBatchSize; ++b) {
        ...
        if (extension == "npy") {
            verifyNumpyFile<T>(numpyFile, files[inputIndex], inputSize);
        } else {
        ...
        if (inputInfo.layout != "CN") {
            ...
            for (int i = 0; i < inputInfo.channels(); i++) {
                numpyFile.read(&data[(i * numpyBatchSize + b) * sizeof(T)], sizeof(T));
            }
        }
...
ov::Tensor get_image_tensor(const std::vector<std::string>& files,
    ...
    if (type == ov::element::bf16) {
        ...
        return create_tensor_from_image<__int8_t>(files,
                                                 inputId,
                                                 batchSize,
                                                 inputInfo.second,
                                                 inputInfo.first,
        ...
        return create_tensor_from_image<__int16_t>(files,
                                                 inputId,
                                                 batchSize,
                                                 inputInfo.second,
                                                 inputInfo.first,
...
ov::Tensor get_numpy_tensor(const std::vector<std::string>& files,
    ...
    if (type == ov::element::bf16) {
        ...
        return create_tensor_from_numpy<__int8_t>(files,
                                                 inputId,
                                                 batchSize,
                                                 inputInfo.second,
                                                 inputInfo.first,
        ...
        return create_tensor_from_numpy<__int16_t>(files,
                                                 inputId,
                                                 batchSize,
                                                 inputInfo.second,
                                                 inputInfo.first,
...
ov::Tensor get_binary_tensor(const std::vector<std::string>& files,
    ...
    if (type == ov::element::bf16) {
        ...
        return create_tensor_from_binary<__int8_t>(files,
                                                 inputId,
                                                 batchSize,
                                                 inputInfo.second,
                                                 inputInfo.first,
        ...
        return create_tensor_from_binary<__int16_t>(files,
                                                 inputId,
                                                 batchSize,
                                                 inputInfo.second,
                                                 inputInfo.first,
...
std::map<std::string, ov::TensorVector> get_tensors(std::map<std::string, std::vector<std::string>> inputFiles,
    ...
    for (const auto& files : inputFiles) {
        ...
        while (n_shape < app_inputs_info.size() || m_file < filesNum) {
            ...
            } else if (std::find(supported_numpy_extensions.begin(), supported_numpy_extensions.end(), get_extension(files.second[0])) !=  supported_numpy_extensions.end()){
