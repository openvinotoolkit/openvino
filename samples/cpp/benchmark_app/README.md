...
This sample demonstrates how to use the Benchmark C++ Tool to estimate deep learning inference performance on supported devices.
...
Experimenting with individual parameters allows finding the optimal performance. However, the parameters are not very performance-portable,
...
Depending on the type, the report is stored in `benchmark_no_counters_report.csv`, `benchmark_average_counters_report.csv`,
...
The `benchmark_app` tool usually produces optimal performance for any device out of the box.
...
Options:
    ...
    -dopt                     Optional. Specify denormals is optimized by treating as zero.
...
To run the tool, use [public](@ref omz_models_group_public) or [Intel's](@ref omz_models_group_intel) pre-trained models from the Open Model Zoo. The models can be downloaded by using [Model Downloader](@ref omz_tools_downloader).
...
4. Run the tool, specifying the `dog.bmp` file as an input image, the OpenVINO IR of the `googlenet-v1` model and a device to do inference on. The following commands demonstrate running the Benchmark Tool in the asynchronous mode on CPU and GPU devices:
