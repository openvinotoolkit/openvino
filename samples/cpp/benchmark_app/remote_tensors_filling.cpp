...
std::map<std::string, ov::TensorVector> get_remote_input_tensors(
    ...
    const auto context_type = context_params.at(ov::intel_gpu::context_type.name()).as<ov::intel_gpu::ContextType>();
    switch (context_type) {
    case ov::intel_gpu::ContextType::OCL:
        break;
    case ov::intel_gpu::ContextType::ZE:
        OPENVINO_THROW("[GPU] OCL context is required for remote device memory. Driver needs to support LEO.");
        break;
    default:
