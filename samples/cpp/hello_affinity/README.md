...
Use `--niter <integer>` to run multiple synchronous inference iterations with the same generated
...
Use `--hint <performance_hint>` to set the device performance mode. Supported values are
...
Use `--shape <shapes>` and `--data-shape <shapes>` with single-shape-group strings such as
`input0[1,96],input1[1,96]`. `--shape` reshapes the model before affinity assignment; `--data-shape` provides concrete
