...
When inference is done, the application outputs data to the standard output stream. Labels may placed in the `.labels` file near the model to get pretty output.
...
4. Do inference of the `dog.bmp` image, using the `googlenet-v1` model on `GPU`, for example:
