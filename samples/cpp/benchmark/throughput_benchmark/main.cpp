...
int main(int argc, char* argv[]) {
    try {
        ...
        std::vector<ov::InferRequest> ireqs; // TODO: fix destructor
        ...
        for (uint32_t i = 0; i < nireq; ++i) {
            ...
            ireq.set_callback([&ireq, &time_point, &mutex, &cv, &latencies, &callback_exception, &niter, &nstarted](std::exception_ptr ex) {
        ...
        std::unique_lock<std::mutex> lock(mutex);
        while (!callback_exception && latencies.size() < niter) {
            cv.wait(lock);
        }
        if (callback_exception) {
