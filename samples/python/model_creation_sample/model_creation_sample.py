...
def create_model(model_path: str) -> ov.Model:
    ...
    conv_1_kernel = op.Constant(
        ov.Type.f32,
        ov.Shape(conv_1_kernel_shape),
        weights[0:conv_1_kernel_length].tolist(),
    )
    weights_offset += conv_1_kernel_length
    conv_1_node = opset8.convolution(
        param_node, conv_1_kernel, [1, 1], padding_begin, padding_end, [1, 1]
    )
    ...
    # maxpool 1
    maxpool_1_node = opset1.max_pool(
        add_1_node, [2, 2], padding_begin, padding_end, [2, 2], "ceil"
    )
    ...
    conv_2_kernel = op.Constant(
        ov.Type.f32,
        ov.Shape(conv_2_kernel_shape),
        weights[weights_offset : weights_offset + conv_2_kernel_length],
    )
    weights_offset += conv_2_kernel_length
    conv_2_node = opset8.convolution(
        maxpool_1_node, conv_2_kernel, [1, 1], padding_begin, padding_end, [1, 1]
    )
    ...
    # maxpool 2
    maxpool_2_node = opset1.max_pool(
        add_2_node, [2, 2], padding_begin, padding_end, [2, 2], "ceil"
    )
    ...
    reshape_1_kernel = op.Constant(
        ov.Type.i64, ov.Shape(list(dtype_weights.shape)), dtype_weights
    )
    ...
    matmul_1_kernel = op.Constant(
        ov.Type.f32,
        ov.Shape(matmul_1_kernel_shape),
    ...
    matmul_2_kernel = op.Constant(
        ov.Type.f32,
        ov.Shape(matmul_2_kernel_shape),
