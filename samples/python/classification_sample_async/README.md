...
In the beginning, the sample application reads command-line parameters, prepares input data, loads a specified model and image(s) to OpenVINO Runtime plugin, does synchronous inference, and processes output data, logging each step in a standard output stream.
...
4. Do inference of the `banana.jpg` and `car.bmp` images, using the `alexnet` model on `GPU`, for example:
