...
def output_comments(result, use_case, writer):
    ...
    comment_list.append("max_rss_mem: max rss memory consumption; the value in -1 iteration row is the maximum value of all available RSS memory numbers in iterations.")
    comment_list.append("max_shared_mem: max shared memory consumption; the value in -1 iteration row is the maximum value of all available shared memory numbers in iterations.")
...
def write_result(report_file, model, framework, device, use_case, iter_data_list, pretrain_time, model_precision):
    header = ["iteration", "model", "framework", "device", "pretrain_time(s)", "input_size", "infer_count", "generation_time(s)", "output_size", "latency(ms)", "1st_latency(ms)", "2nd_avg_latency(ms)", "precision", "max_rss_mem(MB)", "max_shared_mem(MB)", "prompt_idx", "result_md5"]
    ...
    if len(iter_data_list) > 0:
        with out_file.open('w+') as f:
            ...
            for i in range(total_iters):
                ...
                result['generation_time(s)'] = round(iter_data['generation_time'], 5) if iter_data['generation_time'] != "" else iter_data['generation_time']
                result['output_size'] = iter_data['output_size']
                result['latency(ms)'] = round(iter_data['latency'], 5) if iter_data['latency'] != "" else iter_data['latency']
                result['result_md5'] = iter_data['result_md5']
                result['1st_latency(ms)'] = round(iter_data['first_token_latency'], 5) if iter_data['first_token_latency'] != "" else iter_data['first_token_latency']
                result['2nd_avg_latency(ms)'] = round(iter_data['other_tokens_avg_latency'], 5) if iter_data['other_tokens_avg_latency'] != "" else iter_data['other_tokens_avg_latency']
                result['max_rss_mem(MB)'] = round(iter_data['max_rss_mem_consumption'], 5) if iter_data['max_rss_mem_consumption'] != "" else iter_data['max_rss_mem_consumption']
                result['max_shared_mem(MB)'] = round(iter_data['max_shared_mem_consumption'], 5) if iter_data['max_shared_mem_consumption'] != "" else iter_data['max_shared_mem_consumption']
