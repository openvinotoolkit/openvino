...
def print_metrics(iter_num, iter_data, tms=None, tms_infer=None, generated=None, warm_up=False, max_rss_mem=-1, max_shared_mem=-1):
        ...
        if len(tms) > 0:
            ...
            log.info(f"[{iter_str}] First token time: {iter_data['first_token_latency']:.2f} ms, other tokens average time: {iter_data['other_tokens_avg_latency']:.2f} ms/token, len of tokens: {len(tms)}")
            log.info(f"[{iter_str}] First token infer time: {iter_data['first_token_infer_latency']:.2f} ms, other tokens average infer time: {iter_data['other_tokens_infer_avg_latency']:.2f} ms/token, len of tokens: {len(tms)}")
