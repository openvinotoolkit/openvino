...
def print_metrics(iter_num, iter_data, tms=[], generated=None, warm_up=False, max_rss_mem=-1, max_shared_mem=-1):
        ...
        if iter_data['input_size'] != "":
            print(f"[{iter_str}] Input token size: {iter_data['input_size']}")
        if iter_data['output_size'] != "":
            print(f"[{iter_str}] Output size: {iter_data['output_size']}")
        if iter_data['infer_count'] != "":
            print(f"[{iter_str}] Infer count: {iter_data['infer_count']}")
        if iter_data['generation_time'] != "":
            print(f"[{iter_str}] Generation Time: {iter_data['generation_time']:.2f}s")
        if iter_data['latency'] != "":
            print(f"[{iter_str}] Latency: {iter_data['latency']:.2f} ms/token")
        if None != generated:
            print(f"[{iter_str}] Generated:\n{generated}")
        if iter_data['result_md5'] != "":
            print(f"[{iter_str}] Result MD5:{iter_data['result_md5']}")
        if len(tms) > 0:
            ...
            print(f"[{iter_str}] First token time: {iter_data['first_token_latency']:.2f} ms, other tokens average time: {iter_data['other_tokens_avg_latency']:.2f} ms/token")
        if max_rss_mem != "" and max_rss_mem > -1:
            print(f"[{iter_str}] max rss memory cost:\n{max_rss_mem}")
        if max_shared_mem != "" and max_shared_mem > -1:
            print(f"[{iter_str}] max shared memory cost:\n{max_shared_mem}")
...
def print_average(iter_data_list):
    ...
    if total_iters > 0:
        print("<<< Warm-up iteration is excluded. >>>")
        print(f"[Total] Iterations: {total_iters}")
        if total_num_tokens > 0:
            print(f"[Total] Output size: {total_num_tokens} tokens")
        if total_generation_time > 0:
            avg_per_iter_time = total_generation_time / total_iters
            print(f"[Average] Iteration time: {avg_per_iter_time:.2f}s")
            if total_num_tokens > 0:
                avg_per_token_time = total_generation_time * 1000 / total_num_tokens
                print(f"[Average] Latency: {avg_per_token_time:.2f} ms/token")