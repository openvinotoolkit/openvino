...
def get_prompts(args):
    prompts_list = []
    if args['prompt'] == None and args['prompt_file'] == None:
    ...
    elif args['prompt'] != None and args['prompt_file'] != None:
        ...
        if args['prompt'] != None:
            ...
            if input_prompt.endswith(".jsonl"):
                if os.path.exists(input_prompt):
                    print(f"Read prompts from {input_prompt}")
...
def analyze_args(args):
    model_args = {}
    model_args['prompt'] = args.text if (args.text != None and args.text.endswith(".jsonl") == False) else args.prompt
    model_args['prompt_file'] = args.text if (args.text != None and args.text.endswith(".jsonl")) else args.prompt_file
    ...
    if model_framework == "ov":
        ...
        print(f"ov_config={model_args['config']}")
    elif model_framework == "pt":
        print(f"pt_config={model_args['config']}")
        ...
        for case, model_ids in USE_CASES.items():
            for model_id in model_ids:
                if model_id in model_name:
                    print("==SUCCESS FOUND==: use_case: {}, model_name: {}".format(case, model_name))
...
def get_IR_conversion_frontend(modelName, model_name_list):
    IR_conversion_frontend = ""
    idx = 0
    for model_name in model_name_list:
        # idx+1 < len(model_name_list) to avoid out of bounds index of model_name_list
        if model_name == modelName and idx+1 < len(model_name_list):
