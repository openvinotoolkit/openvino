...
class OpenPoseDecoder {
    constructor(
        ...
        pafIndices = [12, 20, 14, 16, 22, 24, 0, 2, 4, 6, 8, 10, 28, 30, 34, 32, 36, 18, 26],
        maxPoints = 100,
        scoreThreshold = 0.1,
        ...
        this.grid = utils.range(this.pointsPerLimb, 0, 1, 'float32').reshape([1, -1, 1]);
    }
    ...
    async call(heatmaps, nmsHeatmaps, pafs) {
        ...
        const [poseEntries, keypointsArray] = await this.groupKeypoints(keypoints, pafs, this.numJoints + 2);
    ...
    async extractPoints(heatmaps, nmsHeatmaps) {
        ...
        for (let k = 0; k < this.numJoints; k++) {
            ...
            const x = utils.booleanMaskAsync(xs.slice([0, k], [1, -1]).reshape([-1]), mask);
            const y = utils.booleanMaskAsync(ys.slice([0, k], [1, -1]).reshape([-1]), mask);
            const score = utils.booleanMaskAsync(scores.slice([0, k], [1, -1]).reshape([-1]), mask);
            ...
            const [refinedX, refinedY] = await this.refine(heatmaps.slice([0, k], [1, -1, -1]).reshape([h, w]), x, y);
            const clippedX = refinedX.clipByValue(0, w - 1);
            const clippedY = refinedY.clipByValue(0, h - 1);

            const keypoints = utils.stack([clippedX, clippedY, score, utils.range(keypointId, keypointId + n, 1, 'float32')], 1);
            keypointId += n;
    ...
    async topK(heatmaps) {
        ...
        return [x.reshape([N, K, this.maxPoints]), y.reshape([N, K, this.maxPoints]), sortedScores];
    }

    static async refine(heatmap, x, y) {
        const [h, w] = heatmap.shape;
        const valid = x.greater(0).logicalAnd(x.less(w - 1)).logicalAnd(y.greater(0)).logicalAnd(y.less(h - 1));

        const validX = x.booleanMaskAsync(valid);
        const validY = y.booleanMaskAsync(valid);
        const dx = (heatmap.gather([validY, validX.add(1)]).sub(heatmap.gather([validY, validX.sub(1)]))).sign().mul(0.25);
        const dy = (heatmap.gather([validY.add(1), validX]).sub(heatmap.gather([validY.sub(1), validX]))).sign().mul(0.25);
    ...
    static isDisjoint(poseA, poseB) {
        ...
        return poseA.equal(poseB).logicalOr(poseA.less(0)).logicalOr(poseB.less(0)).all();
    }

    async updatePoses(kptAId, kptBId, allKeypoints, connections, poseEntries, poseEntrySize) {
        for (const connection of connections) {
            ...
            if (poseAIdx < 0 && poseBIdx < 0) {
                ...
                poseEntry[poseEntrySize - 2] = utils.sum(utils.gather(allKeypoints, [connection[0], connection[1]], 0).slice([0, 2], [-1, 1])).add(connection[2]);
                ...
                if (await OpenPoseDecoder.isDisjoint(poseA, poseB)) {
                    ...
                    poseA[poseEntrySize - 2] = poseA[poseEntrySize - 2].add(connection[2]);
                    poseEntries.splice(poseBIdx, 1);
                }
            } else if (poseAIdx >= 0) {
                const pose = poseEntries[poseAIdx];
                if (pose[kptBId] < 0) {
                    pose[poseEntrySize - 2] = pose[poseEntrySize - 2].add(allKeypoints[connection[1], 2]);
                }
                ...
                if (pose[kptAId] < 0) {
                    pose[poseEntrySize - 2] = pose[poseEntrySize - 2].add(allKeypoints[connection[0], 2]);
                }
    ...
    async groupKeypoints(allKeypointsByType, pafs, poseEntrySize = 20) {
        ...
        for (const [partId, pafChannel] of this.pafIndices.entries()) {
            ...
            const a = utils.tile(kptsA.slice([0, 0], [n, 2]).reshape([1, n, 2]), [m, 1, 1]);
            const b = kptsB.slice([0, 0], [m, 2]);
            const vecRaw = b.reshape([m, 1, 2]).sub(a).reshape([-1, 1, 2]);

            const steps = vecRaw.div(this.pointsPerLimb - 1);
            const points = steps.mul(this.grid).add(a.reshape([-1, 1, 2])).round().cast('int32');
            const x = points.slice([0, 0, 0], [-1, -1, 1]).reshape([-1]);
            const y = points.slice([0, 0, 1], [-1, -1, 1]).reshape([-1]);

            const partPafs = pafs.slice([0, 0, 0, pafChannel], [1, -1, -1, 2]).reshape([h, w, 2]);
            const field = utils.gather(partPafs, y, 0).reshape([-1, this.pointsPerLimb, 2]);
            const vecNorm = vecRaw.norm('euclidean', -1).reshape([-1, 1, 1]);
            const vec = vecRaw.div(vecNorm.add(1e-6));
            const affinityScores1 = utils.sum(field.mul(vec), -1).reshape([-1, this.pointsPerLimb]);
            const validAffinityScores = affinityScores1.greater(this.minPafAlignmentScore);
            const validNum = utils.sum(validAffinityScores, 1);
            const affinityScoresSum = utils.sum(affinityScores.mul(validAffinityScores), 1);
            const affinityScores2 = affinityScoresSum.div(validNum.add(1e-6));
            const successRatio = validNum.div(this.pointsPerLimb);

            const validLimbs = affinityScores2.greater(0).logicalAnd(successRatio.greater(0.8)).toBool().where().flatten();
            if (validLimbs.shape[0] === 0) continue;

            const validIdx = utils.div(validLimbs, n).reshape([-1]);
            const aIdx = validLimbs.mod(n).reshape([-1]);
            const affinityScores3 = utils.gather(affinityScores, validLimbs);

            const connections = await this.connectionsNms(aIdx, validIdx, affinityScores3);
            if (connections.length === 0) continue;

            poseEntries = await this.updatePoses(kptAId, kptBId, allKeypoints, connections, poseEntries, poseEntrySize);
        }

        poseEntries = utils.tensor(poseEntries, [poseEntries.length, poseEntrySize], 'float32');
        poseEntries = poseEntries.where(poseEntries.slice([0, poseEntrySize - 1], [-1, 1]).greaterEqual(3));
    ...
    convertToCocoFormat(poseEntries, allKeypoints) {
        ...
        for (const pose of poseEntries.arraySync()) {
            ...
            const reorderMap = [0, -1, 6, 8, 10, 5, 7, 9, 12, 14, 16, 11, 13, 15, 2, 1, 4, 3];
