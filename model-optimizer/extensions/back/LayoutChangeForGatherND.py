...
class LayoutChangeForGatherND(BackReplacementPattern):
    ...
    def insert_transpose(graph: Graph, input_port: Port, before_input=True):
        ...
        if input_rank > 3 and before_input:
            axis_order = np.concatenate((int64_array([0]),
                                         int64_array(list(range(2, input_rank))),
                                         int64_array([1])))
            transpose = create_op_with_const_inputs(graph, Transpose, {1: axis_order},
                                                    {'name': 'Transpose'})
            input_port.get_connection().set_destination(transpose.in_port(0))
            input_port.connect(transpose.out_port(0))
            ...
            transpose = create_op_with_const_inputs(graph, Transpose, {1: axis_order},
                                                    {'name': 'Transpose'})
            input_port.get_connection().set_source(transpose.out_port(0))
            input_port.connect(transpose.in_port(0))
