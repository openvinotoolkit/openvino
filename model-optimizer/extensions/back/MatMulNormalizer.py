...
class MatMulConstTransposesExtraction(BackReplacementPattern):
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if node.has_and_set('transpose_a') and node.in_port(0).data.get_value() is not None:
            MatMulConstTransposesExtraction.insert_transpose(node, 0)
            node['transpose_a'] = False

        if node.has_and_set('transpose_b') and node.in_port(1).data.get_value() is not None:
...
class MatMulToFullyConnected(BackReplacementPattern):
    ...
    def run_before(self):
        return [ReshapeMutation, BlobNormalizer, TransposeToPermute]
    ...
    def replace_pattern(graph: Graph, match: dict):
        ...
        if B_value is not None and B_shape[B_shape != 1].size <= 2:
...
class FullyConnectedFinalization(BackReplacementPattern):
    ...
    graph_condition = [
        ...
        lambda graph: graph.graph['fw'] not in ['kaldi'],
    ]
    force_clean_up = True

    def run_after(self):
        return [SSBiasAddonForFC, BiasAddonForFC]
