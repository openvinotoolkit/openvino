    ...
    def pattern():
        return dict(
            nodes=[
                ...
                ('mul_const_data', dict(value=lambda v: v is not None and np.allclose(v, 1.0, atol=1e-6))),
                ('add', dict(op='Add')),
                ('add_data', dict()),
                ('add_const', dict(op='Const')),
                ('add_const_data', dict(value=lambda v: v is not None and np.allclose(v, 0.0, atol=1e-6))),
                ('add2', dict(op='Add')),
                ('add2_data', dict()),
                ('add2_const', dict(op='Const')),
                ('add2_const_data', dict(value=lambda v: v is not None and np.allclose(v, 1.0, atol=1e-6))),
                ('floor', dict(op='Floor')),
            ],
            edges=[
                ('shape', 'shape_data'),
