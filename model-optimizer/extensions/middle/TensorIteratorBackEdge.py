...
class BackEdgesMatching(MiddleReplacementPattern):
    ...
    def run_after(self):
        ...
        return [DynamicDecoderConditionMatcher, SimpleConditionMatcher]
...
class BackEdgesMatching2(MiddleReplacementPattern):
    """
    """
    enabled = True
    graph_condition = [lambda graph: graph.graph['is_cyclic']]
    ...
    def run_after(self):
        from extensions.middle.TensorIteratorCondition import SimpleConditionMatcher
        return [DynamicDecoderConditionMatcher, SimpleConditionMatcher, BackEdgesMatching]
    ...
    def replace_pattern(self, graph: Graph, match: dict):
        ...
        # check that one output of Identity is NextIteration node that goes to the Merge node
        identity_data_node = match['Identity_1_data']
        next_iteration_node = None
        for node in identity_data_node.out_nodes():
            if node['kind'] == 'op' and node['op'] == 'NextIteration':
                next_iteration_node = node
                break
        from_body_data = next_iteration_node.in_node()
        ...
        enter_node = match['Enter_1_data'].in_node()
        graph.add_edges_from([(enter_node.id, from_body_data.id, {'in': 0, 'out': 0})])
        # Delete useless nodes
        safe_nodes = ['Identity_1_data', 'condition', 'condition_cond_data']
        for node in match.keys():
            if node not in safe_nodes:
                nodes_for_remove.append(match[node].id)
        nodes_for_remove.append(next_iteration_node.id)
        graph.remove_nodes_from(nodes_for_remove)
