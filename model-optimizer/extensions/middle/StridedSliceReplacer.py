...
class ReplaceStridedSliceWithSqueezeUnsqueeze(MiddleReplacementPattern):
    ...
    def find_and_replace_pattern(self, graph: Graph):
        for node in graph.get_op_nodes(op='StridedSlice'):
            ...
            shrink_axis_mask = node.soft_get('shrink_axis_mask', np.zeros(len(input_shape), dtype=np.float32))
            new_axis_mask = node.soft_get('new_axis_mask', np.zeros(len(input_shape), dtype=np.float32))
