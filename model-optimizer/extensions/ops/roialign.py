...
class ROIAlign(Op):
    ...
    def infer(node):
        ...
        assert compare_dimensions(rois_shape[0], indices_shape[0]), 'The number of batch indices does not correspond ' \
                                                                    'to number of ROIs for node "{}"'.format(node_name)
        assert compare_dimensions(rois_shape[1], 4), 'The size of ROI element must be 4 for node "{}"'.format(node_name)
