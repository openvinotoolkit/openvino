...
class CTCLoss(Op):
    ...
    def infer(node: Node):
        ...
        assert compare_dimensions(logits_shape[0], logit_length_shape[0]) and \
               compare_dimensions(logits_shape[0], labels_shape[0]) and \
               compare_dimensions(logits_shape[0], label_length_shape[0]), \
            'Batch dimensions of input tensors must be the same for {} node'.format(node_name)
        assert compare_dimensions(logits_shape[1], labels_shape[1]), \
