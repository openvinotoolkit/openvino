...
class RetinaNetFilteredDetectionsReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def append_variances(priors_scale_node: Node, variance: list):
        ...
        shape_part_for_tiling = StridedSlice(graph, {'name': name + '/get_-2_dim', 'begin_mask': mo_array([1]),
                                                     'end_mask': mo_array([1]), 'new_axis_mask': mo_array([0]),
        ...
        concat = Concat(graph,
                        {'name': name + '/priors_concat', 'axis': mo_array(0), 'in_ports_count': 2}).create_node()
    ...
    def placeholder_scales(self, placeholder: Node):
        ...
        stride = Const(graph, {'value': int64_array([1])}).create_node()
        spatial = StridedSlice(graph, {'name': name + '/get_h_w', 'begin_mask': mo_array([1]),
                                       'end_mask': mo_array([1]), 'new_axis_mask': mo_array([0]),
                                       'shrink_axis_mask': mo_array([0]), 'ellipsis_mask': mo_array([0])}).create_node()
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        priors_node = StridedSlice(graph, {'name': priors_name + '/0_batch_slice',
                                           'begin_mask': mo_array([1, 1, 1], dtype=np.int32),
                                           'end_mask': mo_array([1, 0, 0], dtype=np.int32),
