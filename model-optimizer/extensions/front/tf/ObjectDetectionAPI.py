...
class ObjectDetectionAPIProposalReplacement(FrontReplacementFromConfigFileSubGraph):
    ...
    def generate_sub_graph(self, graph: Graph, match: SubgraphMatch):
        ...
        image_info_shape = int64_array([3]) if graph.graph['cmd_params'].generate_experimental_IR_V10 else int64_array([1, 3])
        input_op_with_image_size = Parameter(graph, dict(shape=image_info_shape, fixed_batch=True))
