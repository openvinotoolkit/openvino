...
class EfficientDet(FrontReplacementFromConfigFileGeneral):
    ...
    def transform_graph(self, graph: Graph, replacement_descriptions: dict):
        # determine if the op which is a final result of mean value and scale applying to the input tensor
        ...
        for i in range(100):
            ...
            prior_box_op = PriorBoxClusteredOp(graph, {'width': np.array(widths),
                                                       ...
                                                       'offset': 0.5})
            prior_boxes.append(prior_box_op.create_node([Node(graph, inp_name), graph.get_op_nodes(op='Parameter')[0]]))
