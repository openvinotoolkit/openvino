...
class ConvFrontExtractor(FrontExtractorOp):
    ...
    def extract(cls, node):
        ...
        if pads is not None:
            ...
            final_pad = mo_array([[0, 0], [0, 0], *pads], dtype=np.int64)

        # Extract dilations attribute
        # In case if dilations is not specified it will be set in default (1) in infer function
        dilations = onnx_attr(node, 'dilations', 'ints', default=None, dst_type=lambda x: mo_array(x, dtype=np.int64))
        final_dilations = mo_array([1, 1, *dilations], dtype=np.int64) if dilations is not None else None

        # Extract dilations attribute
        # In case if dilations is not specified it will be set in default (1) in infer function
        strides = onnx_attr(node, 'strides', 'ints', default=None, dst_type=lambda x: mo_array(x, dtype=np.int64))
        final_strides = mo_array([1, 1, *strides], dtype=np.int64) if strides is not None else None
        ...
        group = onnx_attr(node, 'group', 'i', default=1, dst_type=lambda x: mo_array(x, dtype=np.int64))
        ...
        attrs = {
            ...
            'group': group,
            'output': None,
            'kernel_spatial': mo_array(kernel_shape, dtype=np.int64) if kernel_shape is not None else None,

            'input_feature_channel': 1,
            'output_feature_channel': 0,
            'kernel_spatial_idx': None,  # Will be calculated in infer function (mo_array([2, 3]))
