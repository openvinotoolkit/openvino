...
class DeconvFrontExtractor(FrontExtractorOp):
    ...
    def get_pad(node, input_shape, kernel_shape):
        ...
        return mo_array([[0, 0], [0, 0], *[[pad, pad] for pad in padding[2:]]], dtype=np.int64)
    ...
    def extract(cls, node):
        ...
        if target_shape:
            target_shape = mo_array(target_shape, dtype=np.int64)
        ...
        if target_shape is None and output_padding:
            node_attrs["output_padding"] = mo_array([0, 0, *[s for s in output_padding]], dtype=np.int64)
