...
class MXSliceToStridedSliceReplacer(FrontReplacementOp):
    ...
    def replace_sub_graph(self, graph: Graph, match: dict):
        ...
        strided_slice_node = StridedSlice(graph, dict(name=node.id + '/strided_slice_',
                                                      shrink_axis_mask=mo_array(np.zeros(len(node.crop_begin), dtype=np.int64)),
                                                      new_axis_mask=mo_array(np.zeros(len(node.crop_begin), dtype=np.int64)),
                                                      ellipsis_mask=mo_array(np.zeros(len(node.crop_begin), dtype=np.int64)),
                                                      begin_mask=mo_array(np.ones(len(node.crop_begin), dtype=np.int64)),
                                                      end_mask=mo_array(np.ones(len(node.crop_end), dtype=np.int64)))).create_node()
