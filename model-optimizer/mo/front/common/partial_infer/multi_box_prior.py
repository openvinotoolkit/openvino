...
def multi_box_prior_infer_mxnet(node: Node):
    ...
    if v10:
        node.out_node(0).shape = mo_array([2, data_H * data_W * num_priors * 4], dtype=np.int64)
    else:
        node.out_node(0).shape = mo_array([1, 2, data_H * data_W * num_priors * 4], dtype=np.int64)
