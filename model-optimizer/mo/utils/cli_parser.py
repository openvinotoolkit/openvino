...
def get_common_cli_parser(parser: argparse.ArgumentParser = None):
    ...
    common_group.add_argument('--generate_deprecated_IR_V2',
                              ...
                              action=DeprecatedStoreTrue)
    common_group.add_argument('--generate_experimental_IR_V10',
                              help='Force to generate experimental IR V10 with changed layer semantics that'
                                   ' aligned with nGraph/DLDT merged specification. Please use it for experimental'
                                   ' purposes only.',
                              action='store_true')
    ...
    common_group.add_argument('--keep_quantize_ops_in_IR',
                              help='[ Experimental feature ] Enables `Quantize` operation with all children keeping. ',
                              action='store_true', default=False)
    common_group.add_argument('--blobs_as_inputs',
                              help='[ Experimental feature ] Weights and biases of Convolution and FullyConnected '
                                   'will be moved to inputs',
                              action='store_true', default=False)
...
def append_exp_keys_to_namespace(argv: argparse.Namespace):
    # setattr(argv, 'some_arg', some_value)
    pass
