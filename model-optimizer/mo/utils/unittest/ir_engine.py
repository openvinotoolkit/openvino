...
class IREngine(object):
        ...
        for attr in layer:
            if attr.tag == 'data':
                ...
                if layer.attrib['type'] == 'Const':
                    assert len(new_attrs.keys()) == 2 and all(key in ['offset', 'size'] for key in new_attrs.keys()), \
                        'Incorrect attributes for Const layer, {} instead of {}!'.format(new_attrs.keys(), ['offset', 'size'])
                    new_attrs = self.__prepare_bin_attrs(layer, 0, 'custom', new_attrs['offset'], new_attrs['size'])
    ...
    @staticmethod
    def __prepare_bin_attrs(xml_layer, in_port, tag, offset, size):
        layer_attrs = dict()
