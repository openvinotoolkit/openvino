...
def moc_emit_ir(ov_function: Function, argv: argparse.Namespace):
    output_dir = argv.output_dir if argv.output_dir != '.' else os.getcwd()

    from mo.back.offline_transformations import apply_user_transformations, apply_moc_transformations
    apply_user_transformations(ov_function, parse_transform(argv.transform))
    apply_moc_transformations(ov_function)

    if argv.compress_fp16:
        from mo.back.offline_transformations import compress_model
        compress_model(ov_function)
    ...
    serialize(ov_function, (orig_model_name + ".xml").encode('utf-8'), (orig_model_name + ".bin").encode('utf-8'))
    del argv.feManager
