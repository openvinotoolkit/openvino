...
def check_scale_constant(node, expected, shape=None):
    const_node = node.input(1).get_source_output().get_node()
    assert const_node.get_type_name() == 'Constant'
    if node.get_type_name() == 'Divide':
        assert (const_node.get_vector() == expected).all()
    else:
        assert (const_node.get_vector() == 1. / expected).all()
    if shape:
        assert const_node.shape == PartialShape(shape)


def check_mean_constant(node, expected, shape=None):
    const_node = node.input(1).get_source_output().get_node()
    assert const_node.get_type_name() == 'Constant'
...
class TestMeanScaleMOC(unittest.TestCase):
    ...
    def test_scale_single_value(self):
        ...
        for param in function.get_parameters():
            ...
            assert op_node.get_type_name() == 'Divide' or op_node.get_type_name() == 'Multiply'
            check_scale_constant(op_node, [2.0])
    ...
    def test_scale_vector3(self):
        ...
        op_node = list(function.get_parameters()[0].output(0).get_target_inputs())[0].get_node()
        assert op_node.get_type_name() == 'Divide' or op_node.get_type_name() == 'Multiply'
        check_scale_constant(op_node, expected=[2., 4., 8.], shape=[1, 3, 1, 1])
    ...
    def test_mean_single(self):
        ...
        process_mean_scale(network=ie_network,
                           ...
                           argv=argv)
        op_node = list(function.get_parameters()[0].output(0).get_target_inputs())[0].get_node()
        assert op_node.get_type_name() == 'Subtract' or op_node.get_type_name() == 'Add'
        check_mean_constant(op_node, [4.0], shape=[1, 1])
        # Verify that input2 is not affected
        op_node = list(function.get_parameters()[1].output(0).get_target_inputs())[0].get_node()
        assert op_node.get_type_name() == 'Relu'
    ...
    def test_mean_vector3(self):
        ...
        op_node = list(function.get_parameters()[1].output(0).get_target_inputs())[0].get_node()
        assert op_node.get_type_name() == 'Subtract' or op_node.get_type_name() == 'Add'
        check_mean_constant(op_node, expected=[2., 4., 8.], shape=[1, 3, 1, 1])
    ...
    def test_mean_scale(self):
        ...
        op_node = list(function.get_parameters()[1].output(0).get_target_inputs())[0].get_node()
        assert op_node.get_type_name() == 'Subtract' or op_node.get_type_name() == 'Add'
        check_mean_constant(op_node, expected=[1., 2., 3.], shape=[1, 3, 1, 1])
    ...
    def test_no_param_name(self):
        argv = Namespace(mean_scale_values=[(np.array([1., 2., 3.]), np.array([2., 4., 6.])),
                                            (np.array([7., 8.]), None)], scale=None)
        ...
        op_node = list(op_node.output(0).get_target_inputs())[0].get_node()
        assert op_node.get_type_name() == 'Divide' or op_node.get_type_name() == 'Multiply'
        check_scale_constant(op_node, expected=[2., 4., 6.], shape=[1, 3, 1, 1])

        op_node = list(function.get_parameters()[1].output(0).get_target_inputs())[0].get_node()
        assert op_node.get_type_name() == 'Subtract' or op_node.get_type_name() == 'Add'
        check_mean_constant(op_node, expected=[7., 8.], shape=[1, 2, 1, 1])

    def test_no_param_name_single_value(self):
        argv = Namespace(mean_scale_values=[(np.array([1.]), None),
                                            (np.array([2., 3.]), np.array([4.]))], scale=None)
        function = create_function2(shape1=[1, 3, 224, 224], shape2=[1, 2, 224, 224])
        ie_network = function_to_cnn(function)
        process_mean_scale(network=ie_network,
                           ngraph_function=function,
        ...
        op_node = list(function.get_parameters()[0].output(0).get_target_inputs())[0].get_node()
        assert op_node.get_type_name() == 'Subtract' or op_node.get_type_name() == 'Add'
        check_mean_constant(op_node, expected=[1.], shape=[1, 1, 1, 1])

        op_node = list(function.get_parameters()[1].output(0).get_target_inputs())[0].get_node()
        assert op_node.get_type_name() == 'Subtract' or op_node.get_type_name() == 'Add'
        check_mean_constant(op_node, expected=[2., 3.], shape=[1, 2, 1, 1])

        op_node = list(op_node.output(0).get_target_inputs())[0].get_node()
        assert op_node.get_type_name() == 'Divide' or op_node.get_type_name() == 'Multiply'
        check_scale_constant(op_node, expected=[4.], shape=[1, 1, 1, 1])
