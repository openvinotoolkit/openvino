...
class TestMainFrontend(unittest.TestCase):
    ...
    def test_SimpleConvert(self, mock_argparse):
        ...
        xmlFile = re.search(r'\[ SUCCESS \] XML file: (.*)', out).\
            group(1).replace("\r", "")
        binFile = re.search(r'\[ SUCCESS \] BIN file: (.*)', out).\
            group(1).replace("\r", "")
        assert xmlFile and binFile
    ...
    @mock_needed
    @patch('argparse.ArgumentParser.parse_args',
           return_value=replaceArgsHelper(output='newOut1,newOut2'))
