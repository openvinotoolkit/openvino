...
if(CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
    ...
    # Optional alternate path to store in PDBs for debugging on a different machine (MSVC /PDBALTPATH).
    # Set OV_MSVC_PDB_ALT_PATH to a path string, or set OV_MSVC_PDB_ALT_PATH_FILENAME_ONLY=ON to use
    # /PDBALTPATH:%_PDB% so the debugger looks for the PDB next to the binary.
    if(OV_MSVC_PDB_ALT_PATH_FILENAME_ONLY)
        set(_pdbalt "/PDBALTPATH:%_PDB%")
        set(CMAKE_SHARED_LINKER_FLAGS_DEBUG "${CMAKE_SHARED_LINKER_FLAGS_DEBUG} ${_pdbalt}")
        set(CMAKE_MODULE_LINKER_FLAGS_DEBUG "${CMAKE_MODULE_LINKER_FLAGS_DEBUG} ${_pdbalt}")
        set(CMAKE_EXE_LINKER_FLAGS_DEBUG "${CMAKE_EXE_LINKER_FLAGS_DEBUG} ${_pdbalt}")
        set(CMAKE_SHARED_LINKER_FLAGS_RELWITHDEBINFO "${CMAKE_SHARED_LINKER_FLAGS_RELWITHDEBINFO} ${_pdbalt}")
        set(CMAKE_MODULE_LINKER_FLAGS_RELWITHDEBINFO "${CMAKE_MODULE_LINKER_FLAGS_RELWITHDEBINFO} ${_pdbalt}")
        set(CMAKE_EXE_LINKER_FLAGS_RELWITHDEBINFO "${CMAKE_EXE_LINKER_FLAGS_RELWITHDEBINFO} ${_pdbalt}")
    elseif(DEFINED OV_MSVC_PDB_ALT_PATH AND NOT OV_MSVC_PDB_ALT_PATH STREQUAL "")
        set(_pdbalt "/PDBALTPATH:${OV_MSVC_PDB_ALT_PATH}")
        set(CMAKE_SHARED_LINKER_FLAGS_DEBUG "${CMAKE_SHARED_LINKER_FLAGS_DEBUG} ${_pdbalt}")
        set(CMAKE_MODULE_LINKER_FLAGS_DEBUG "${CMAKE_MODULE_LINKER_FLAGS_DEBUG} ${_pdbalt}")
        set(CMAKE_EXE_LINKER_FLAGS_DEBUG "${CMAKE_EXE_LINKER_FLAGS_DEBUG} ${_pdbalt}")
        set(CMAKE_SHARED_LINKER_FLAGS_RELWITHDEBINFO "${CMAKE_SHARED_LINKER_FLAGS_RELWITHDEBINFO} ${_pdbalt}")
