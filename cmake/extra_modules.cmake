    ...
    foreach(module_path IN LISTS extra_modules)
        if(module_path)
            ...
            if(BUILD_${module_name})
                ...
                set(_OLD_LINKER_FLAGS ${CMAKE_EXE_LINKER_FLAGS})
                set(_OLD_WARNING_AS_ERROE ${CMAKE_COMPILE_WARNING_AS_ERROR})
                # temporarily remove CMAKE_COMPILE_WARNING_AS_ERROR for nvidia plugin and npu plugin
                if ((${module_name} STREQUAL "nvidia_plugin" OR ${module_name} STREQUAL "npu") AND
                    CMAKE_COMPILE_WARNING_AS_ERROR AND WIN32)
                    if(CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
                        if(CMAKE_VERSION VERSION_LESS 3.24)
                            set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} /WX:NO")
                            set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} /WX:NO")
                        endif()
                        set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} /WX:NO")
                    endif()
                    set(CMAKE_COMPILE_WARNING_AS_ERROR OFF)
                endif()
                add_subdirectory("${module_path}" "build-modules/${module_name}")
                # recover CMAKE_COMPILE_WARNING_AS_ERROR for nvidia plugin and npu plugin
                set(CMAKE_CXX_FLAGS "${_OLD_CXX_FLAGS}")
                set(CMAKE_C_FLAGS "${_OLD_C_FLAGS}")
                set(CMAKE_EXE_LINKER_FLAGS "${_OLD_LINKER_FLAGS}")
                set(CMAKE_COMPILE_WARNING_AS_ERROR ${_OLD_WARNING_AS_ERROE})
