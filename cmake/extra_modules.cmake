...
function(_ov_register_extra_modules)
    ...
    function(_ov_generate_fake_developer_package NS)
        ...
        # The developer target list can accumulate duplicates across repeated
        # configure/generate cycles. De-duplicate it before emitting aliases so
        # the generated fake package stays stable and easier to inspect.
        set(exported_targets ${_OPENVINO_DEVELOPER_PACKAGE_TARGETS})
        list(REMOVE_DUPLICATES exported_targets)

        file(REMOVE "${devconfig_file}")
        file(WRITE "${devconfig_file}" "\# !! AUTOGENERATED: DON'T EDIT !!\n\n")

        foreach(exported_target IN LISTS exported_targets)
            # Only emit aliases for real CMake targets. This protects the
            # generated config from malformed entries that would otherwise break
            # parsing during extra-module configure.
            if(NOT TARGET "${exported_target}")
                continue()
            endif()

            # Keep the generated alias name conservative to avoid writing
            # syntactically invalid CMake when an unexpected list entry appears.
            if(NOT exported_target MATCHES "^[A-Za-z0-9_+.-]+$")
                message(VERBOSE "Skipping invalid developer package target '${exported_target}'")
                continue()
            endif()
