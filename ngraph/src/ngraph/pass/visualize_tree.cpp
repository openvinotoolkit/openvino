...
ostream& visualize_constant_values(ostream& label, shared_ptr<Node> node)
{
    if (not is_type_ptr<op::Constant>(node) || ngraph::shape_size(node->get_shape()) > 7)
        return label;
    ...
    {
        case element::Type_t::undefined: label << "[ undefined value ]"; break;
        case element::Type_t::dynamic: label << "[ dynamic value ]"; break;
        case element::Type_t::u1: label << "[ u1 value ]"; break;
        case element::Type_t::bf16:
        case element::Type_t::f16:
        case element::Type_t::f32:
        case element::Type_t::f64:
            label << "[" << pretty_value(constant->cast_vector<double>()) << "]";
            break;
...
string pass::VisualizeTree::get_attributes(shared_ptr<Node> node)
{
    ...
    // Construct the label attribute
    {
        ...
        // visualization of short constant values (up to 7 elements)
        visualize_constant_values(label, node);
        ...
        if (nvtos || nvtot)
            for (const auto& output : node->outputs())
            {
                ...
                if (nvtot)
                    label << "{" << output.get_element_type().c_type_string() << "}";
                if (nvtos)
