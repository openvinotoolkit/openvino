...
void regclass_pyngraph_Shape(py::module m)
{
    ...
    shape.def("__len__", [](const ov::Shape& v) { return v.size(); });
    shape.def("__getitem__", [](const ov::Shape& v, int key) { return v[key]; });

    shape.def(
        "__iter__",
        [](ov::Shape& v) { return py::make_iterator(v.begin(), v.end()); },
