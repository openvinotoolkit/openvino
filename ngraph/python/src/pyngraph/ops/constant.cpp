...
template <typename T>
py::buffer_info _get_buffer_info(const ov::op::Constant& c)
{
...
template <>
py::buffer_info _get_buffer_info<ov::float16>(const ov::op::Constant& c)
{
    ov::Shape shape = c.get_shape();
    return py::buffer_info(
        const_cast<void*>(c.get_data_ptr()),               /* Pointer to buffer */
...
template <typename T>
py::array _cast_vector(const ov::op::Constant& self)
{
...
{
    py::class_<ov::op::Constant, std::shared_ptr<ov::op::Constant>, ov::Node> constant(
        m, "Constant", py::buffer_protocol());
    constant.doc() = "ngraph.impl.op.Constant wraps ov::op::Constant";
    constant.def(py::init<const ov::element::Type&, const ov::Shape&, const std::vector<char>&>());
    constant.def(
        py::init<const ov::element::Type&, const ov::Shape&, const std::vector<ov::float16>&>());
    constant.def(py::init<const ov::element::Type&, const ov::Shape&, const std::vector<float>&>());
    constant.def(
        py::init<const ov::element::Type&, const ov::Shape&, const std::vector<double>&>());
    constant.def(
        py::init<const ov::element::Type&, const ov::Shape&, const std::vector<int8_t>&>());
    constant.def(
        py::init<const ov::element::Type&, const ov::Shape&, const std::vector<int16_t>&>());
    constant.def(
        py::init<const ov::element::Type&, const ov::Shape&, const std::vector<int32_t>&>());
    constant.def(
        py::init<const ov::element::Type&, const ov::Shape&, const std::vector<int64_t>&>());
    constant.def(
        py::init<const ov::element::Type&, const ov::Shape&, const std::vector<uint8_t>&>());
    constant.def(
        py::init<const ov::element::Type&, const ov::Shape&, const std::vector<uint16_t>&>());
    constant.def(
        py::init<const ov::element::Type&, const ov::Shape&, const std::vector<uint32_t>&>());
    constant.def(
        py::init<const ov::element::Type&, const ov::Shape&, const std::vector<uint64_t>&>());

    constant.def("get_value_strings", &ov::op::Constant::get_value_strings);

    constant.def("get_vector", [](const ov::op::Constant& self) {
    ...
    constant.def_buffer([](const ov::op::Constant& self) -> py::buffer_info {
        auto element_type = self.get_element_type();
        if (element_type == ov::element::boolean)
        {
