...
void regclass_pyngraph_FrontEndManager(py::module m)
{
    ...
    fem.def(py::pickle([](const ov::frontend::FrontEndManager&) { return py::make_tuple(0); },
                       [](py::tuple t) { return ov::frontend::FrontEndManager(); }));
...
{
    static py::exception<ov::frontend::OpValidationFailure> exc(std::move(m),
                                                                "OpValidationFailure");
    py::register_exception_translator([](std::exception_ptr p) {
        try
        {
...
{
    static py::exception<ov::frontend::OpConversionFailure> exc(std::move(m),
                                                                "OpConversionFailure");
    py::register_exception_translator([](std::exception_ptr p) {
        try
        {
...
{
    static py::exception<ov::frontend::InitializationFailure> exc(std::move(m),
...
{
    static py::exception<ov::frontend::NotImplementedFailure> exc(std::move(m),
