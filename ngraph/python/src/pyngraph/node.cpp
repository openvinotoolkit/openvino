...
{
    py::class_<ov::Node, std::shared_ptr<ov::Node>> node(m, "Node", py::dynamic_attr());
    node.doc() = "ngraph.impl.Node wraps ov::Node";
    node.def(
        "__add__",
    ...
    node.def("_get_attributes", [](const std::shared_ptr<ov::Node>& self) {
        util::DictAttributeSerializer dict_serializer(self);
        return dict_serializer.get_attributes();
    });
    node.def("_set_attribute",
             [](std::shared_ptr<ov::Node>& self, const std::string& atr_name, py::object value) {
                 py::dict attr_dict;
