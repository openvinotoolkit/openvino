...
namespace {
    ...
    public:
        explicit XmlDeserializer(
                ...
                : m_node(node), m_weights(weights), m_opsets(opsets), m_variables(variables) {}

        void on_adapter(const std::string& name, ngraph::ValueAccessor<std::string>& value) override {
            std::string val;
            if (!getStrAttribute(m_node.child("data"), name, val)) return;
            value.set(val);
        }
        ...
        void on_adapter(
            ...
            if (!getParameters<int32_t>(m_node.child("data"), name, value)) return;
            adapter.set(value);
        }
        ...
        void on_adapter(
            ...
            if (!getParameters<int64_t>(m_node.child("data"), name, value)) return;
            adapter.set(value);
        }
        ...
        void on_adapter(
            ...
            if (!getParameters<float>(m_node.child("data"), name, value)) return;
            adapter.set(value);
        }
        ...
        void on_adapter(
            ...
            if (!getParameters<std::string>(m_node.child("data"), name, value)) return;
            adapter.set(value);
        }
    ...
    void XmlDeserializer::on_adapter(const std::string& name, ngraph::ValueAccessor<void>& adapter) {
        ...
        if (skip_names.count(name) && !getStrAttribute(m_node.child("data"), name, val)) return;
        if (auto a = ngraph::as_type<ngraph::AttributeAdapter<ngraph::element::Type>>(&adapter)) {
            static_cast<ngraph::element::Type&>(*a) = details::convertPrecision(val);
        } else if (auto a = ngraph::as_type<ngraph::AttributeAdapter<ngraph::PartialShape>>(&adapter)) {
            std::vector<int64_t> shape;
            std::vector<ngraph::Dimension> dims;
            if (!getParameters<int64_t>(m_node.child("data"), name, shape)) return;
            ...
            if (!getParameters<size_t>(m_node.child("data"), name, shape)) return;
            ...
            if (!getParameters<size_t>(m_node.child("data"), name, result)) return;
        ...
        a->set(result);
            ...
            if (!getParameters<size_t>(m_node.child("data"), name, axes)) return;
            ...
            if (!getStrAttribute(m_node.child("data"), name, variable_id)) return;
    ...
    std::shared_ptr<ngraph::Function> XmlDeserializer::parse_function(
        ...
        auto function = std::make_shared<ngraph::Function>(
                func_nodes.results, func_nodes.sinks, func_nodes.parameters, XMLParseUtils::GetStrAttr(root, "name", ""));
