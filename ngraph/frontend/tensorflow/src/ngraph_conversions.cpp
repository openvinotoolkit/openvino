...
void NHWCtoNCHW(const std::string& op_name, bool is_nhwc, ov::Output<ov::Node>& node) {
    if (is_nhwc) {
...
void NCHWtoNHWC(const std::string& op_name, bool is_nhwc, ov::Output<ov::Node>& node) {
    if (is_nhwc) {
        auto rank = node.get_shape().size();
        if (rank == 4) {
...
void TFDataTypeToNGraphElementType(DataType tf_dt, ov::element::Type* ng_et) {
    ...
    case DataType::DT_HALF:
        *ng_et = ov::element::f16;
        break;
    default:
        FRONT_END_THROW("Unsupported TensorFlow data type: " + DataType_Name(tf_dt));
    }
}
