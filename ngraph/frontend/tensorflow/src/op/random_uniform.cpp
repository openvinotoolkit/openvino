...
namespace tf {
namespace op {
OutputVector TranslateRandomUniformOp(const NodeContext& node) {
    ...
    auto ng_et = node.get_attribute<element::Type>("dtype");
    auto random_uniform = make_shared<RandomUniform>(shape, minval_const, maxval_const, ng_et, seed, seed2);
    random_uniform->set_friendly_name(node.get_name());
    return random_uniform->outputs();
}

OutputVector TranslateRandomUniformIntOp(const NodeContext& node) {
    auto shape = node.get_ng_input(0);
    ...
    auto random_uniform = make_shared<RandomUniform>(shape, minval, maxval, ng_et, seed, seed2);
    random_uniform->set_friendly_name(node.get_name());
    return random_uniform->outputs();
}
}  // namespace op
