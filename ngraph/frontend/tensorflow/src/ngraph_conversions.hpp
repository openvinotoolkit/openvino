...
void Transpose(ngraph::Output<ngraph::Node>& node) {
    ...
    NGRAPH_VLOG(3) << "transposing " << ngraph::join(s) << " to " << ngraph::join(reshaped_shape) << " axis-order "
                   << ngraph::join(transpose_order);
    auto input_order = std::make_shared<ngraph::opset8::Constant>(ngraph::element::u64,
