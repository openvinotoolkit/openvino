...
{
    NGRAPH_CHECK(variants.size() == 1, "Only one parameter to load function is expected. Got " + std::to_string(variants.size()));
    NGRAPH_CHECK(is_type<VariantWrapper<std::string>>(variants[0]), "Parameter to load function need to be a std::string");
