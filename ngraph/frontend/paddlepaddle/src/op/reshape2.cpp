...
NamedOutputs reshape2(const NodeContext& node) {
    ...
    if (!node.has_ng_input("Shape") && !node.has_ng_input("ShapeTensor")) {
        ...
        return node.default_single_output_mapping({std::make_shared<ngraph::opset6::Reshape>(data, shape_node, true)},
                                                  {"Out"});
    } else {
        ...
        for (auto& input_node : nodes) {
            auto cast = std::make_shared<ngraph::opset6::Convert>(input_node, element::i64);
