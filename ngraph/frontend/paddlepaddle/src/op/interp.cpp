            ...
            namespace op {
                ...
                calculate_output_shape_based_on_scales(const Output<ngraph::Node> &data,
                    ...
                    if (scale.size() == 1)
                        ...
                        scales = opset6::Constant::create<float>(element::f32, Shape{4},
                                                                 {1, scale[0], scale[1], scale[2]});
                    else
                        scales = opset6::Constant::create<float>(element::f32, Shape{scale.size()},
                ...
                interpolate(const NodeContext &node, const ngraph::opset6::Interpolate::InterpolateMode &mode) {
                    ...
                    if (node.has_ng_input("OutSize")) {
                        ...
                        target_spatial_shape = std::make_shared<opset6::Concat>(
                                OutputVector{nc_node, std::make_shared<opset6::Convert>(hw_shape, element::i64)}, 0);
