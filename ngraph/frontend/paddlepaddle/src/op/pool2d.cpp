...
NamedOutputs pool2d (const NodeContext& node) {
    ...
    if (pooling_type == "max" && !global_pooling) {
        ...
        return default_single_output_mapping(node, {std::make_shared<ngraph::opset6::MaxPool>(
                    data,
                                                   ...
                                                   [](int32_t s) { return s == 1; })))
    {
        ...
        return default_single_output_mapping(node, {std::make_shared<ngraph::opset6::ReduceMean>(data, axes, true)});
    } else {
