...
namespace pdpd
{
    namespace op
    {
        ...
        NamedOutputs yolo_box(const NodeContext& node_context)
        {
            ...
            std::cout << "downsample_ratio: " << downsample_ratio << " conf_thresh: " << conf_thresh
                      << std::endl;
            ...
            // x/y
            std::shared_ptr<Node> node_box_x_sigmoid = std::make_shared<Sigmoid>(node_box_x);
            std::shared_ptr<Node> node_box_y_sigmoid = std::make_shared<Sigmoid>(node_box_y);
            ...
            auto node_pred_box = std::make_shared<Concat>(OutputVector{node_box_x_new_shape,
            ...
            auto node_sqeeze_image_size =
                std::make_shared<Squeeze>(image_size, squeeze_image_size_axes); // input ImgSize
            ...
            if (clip_bbox)
            {
                ...
                auto node_pred_box_x2_res =
                    std::make_shared<Subtract>(node_pred_box_x2_decode, node_pred_box_x2_clip);
                auto node_pred_box_y2_res =
                    std::make_shared<Subtract>(node_pred_box_y2_decode, node_pred_box_y2_clip);

                node_pred_box_result = std::make_shared<Concat>(OutputVector{node_pred_box_x1_clip,
