            ...
            namespace op
            {
                NamedOutputs expand_v2(const NodeContext& node)
                {
                    ...
                    else
                    {
                        ...
                        shape_expected_node = ov::opset6::Constant::create(
                            ov::element::i32, {shape_expected.size()}, shape_expected);
                    }
                    // if -1 in shape we will copy the orginal value from input
                    auto zero_node = ov::opset6::Constant::create(ov::element::i32, {1}, {0});
                    auto mask_node =
                        std::make_shared<ov::opset6::Greater>(shape_expected_node, zero_node);
                    auto input_shape_node = std::make_shared<ov::opset6::ShapeOf>(x, element::i32);
                    auto fixed_shape_node = std::make_shared<ov::opset6::Select>(
                        mask_node, shape_expected_node, input_shape_node);
                    auto repeated_node = std::make_shared<ov::opset6::Divide>(
                        fixed_shape_node, input_shape_node, false);

                    return node.default_single_output_mapping(
                        {std::make_shared<ov::opset6::Tile>(
                            x, std::make_shared<ov::opset6::Convert>(repeated_node, element::i64))},
                        {"Out"});
                }
