                ...
                NamedOutputs argmax(const NodeContext& node)
                {
                    ...
                    if (!flatten)
                    {
                        ...
                        const auto axis_to_remove =
                            opset6::Constant::create(element::u64, Shape{}, {axis});
                        ...
                        return node.default_single_output_mapping(
                            {std::make_shared<opset6::Convert>(reshaped_indices, element::i64)},
                            {"Out"});
                    }
                    else
                    {
                        ...
                        auto node_reshape =
                            std::make_shared<opset6::Reshape>(data, reshape_flatten, true);
                        auto node_topk = std::make_shared<opset6::TopK>(
