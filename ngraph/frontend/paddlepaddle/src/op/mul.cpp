...
namespace pdpd
{
    namespace op
    {
        NamedOutputs mul(const NodeContext& node)
        {
            ...
            PDPD_NODE_VALIDATION_CHECK(ngraph::frontend::ErrorCode::OP_VALIDATION_FAILED, node, x.get_partial_shape().rank().is_static(), "matmul: X rank must be static!");
            ...
            PDPD_NODE_VALIDATION_CHECK(ngraph::frontend::ErrorCode::OP_VALIDATION_FAILED,
                                       node,
                                       y.get_partial_shape().rank().is_static() && y.get_partial_shape().rank().get_length() == 2, "matmul: Y rank must be static, and 2!");
            if (x_rank > 2)
            {
                ...
                auto split = std::make_shared<opset6::VariadicSplit>(shape, axis, split_lengths);
                ...
                auto second_dim =
                    std::make_shared<opset6::Reshape>(second_dim_reduce, s_dim_shape, false);
                auto out_shape =
            ...
            return node.default_single_output_mapping({std::make_shared<opset6::MatMul>(x, y)},
                                                      {"Out"});
        }
