...
NamedOutputs mul (const NodeContext& node) {
    ...
    PDPD_NODE_VALIDATION_CHECK(node, x.get_partial_shape().rank().is_static(), "matmul: X rank must be static!");
    int64_t x_rank = x.get_partial_shape().rank().get_length();
    PDPD_NODE_VALIDATION_CHECK(node, y.get_partial_shape().rank().is_static() &&
