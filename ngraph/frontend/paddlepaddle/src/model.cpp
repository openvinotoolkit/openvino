    ...
    for (int block_idx = 0; block_idx < cnt_of_blocks; block_idx++)
    {
        ...
        for (const auto& var : block.vars())
        {
            m_var_places[var.name()] =
                std::make_shared<TensorPlacePDPD>(m_input_model, std::make_shared<VarDesc>(var));
        }
        ...
        for (const auto& op : block.ops())
        {
            ...
            if (op.type() == "feed")
            {
                ...
                const auto& var_place =
                    std::dynamic_pointer_cast<TensorPlacePDPD>(place->getTargetTensorPDPD());
    ...
    , m_input_model(input_model)
{
        ...
        (0 == model_file.compare(model_file.length() - ext.length(), ext.length(), ext)))
    {
        ...
        weights_stream =
            std::unique_ptr<std::ifstream>(new std::ifstream(weights_file, std::ios::binary));
...
Place::Ptr
    InputModelPDPD::InputModelPDPDImpl::getPlaceByTensorName(const std::string& tensorName) const
{
...
void InputModelPDPD::InputModelPDPDImpl::setDefaultShape(Place::Ptr place, const Shape& shape)
{
...
void InputModelPDPD::InputModelPDPDImpl::setElementType(Place::Ptr place, const element::Type& type)
{
