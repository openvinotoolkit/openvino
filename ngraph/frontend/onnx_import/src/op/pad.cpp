        ...
        namespace op
        {
            namespace set_1
            {
                OutputVector pad(const Node& node)
                {
                    ...
                    return {std::make_shared<default_opset::Pad>(
                        ...
                        std::make_shared<default_opset::Constant>(
                            element::Type_t::i64, ngraph::Shape{padding_above.size()}, padding_above),
                        std::make_shared<default_opset::Constant>(
            ...
            namespace set_11
            {
                OutputVector pad(const Node& node)
                {
                    ...
                    {
                        auto axis =
                            default_opset::Constant::create(element::Type_t::i64, ngraph::Shape{}, {0});
                        OutputVector padding = builder::opset1::split(pads, 2, 0);

                        padding_begin =
                            std::make_shared<default_opset::Convert>(padding.at(0), element::Type_t::i64);
                        padding_end =
                            std::make_shared<default_opset::Convert>(padding.at(1), element::Type_t::i64);
                    }
