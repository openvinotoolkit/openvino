...
public:
    ...
    ONNXFrameworkNode(const std::shared_ptr<onnx_import::Graph>& graph, const onnx_import::Node& node) :
    ...
    ONNXFrameworkNode(const std::shared_ptr<onnx_import::Graph>& graph, const onnx_import::Node& node, const OutputVector& inputs) :
...
public:
    ...
    ONNXSubgraphFrameworkNode(const std::shared_ptr<onnx_import::Graph>& graph, const onnx_import::Node& node, const OutputVector& inputs) :
    ...
    std::shared_ptr<Function> get_subgraph_body() const
    {
        ...
        return std::make_shared<Function>(subgraph->get_ng_outputs(), subgraph->get_ng_parameters(), subgraph->get_name());
    }
};
