...
namespace
{
    void incorrect_init(const ngraph::element::Type& type,
                        ...
                        const Shape& shape1 = {1, 3, 6},
                        const Shape& shape2 = {1, 3, 6})
    {
        auto input1 = make_shared<op::Parameter>(type, shape1);
        auto input2 = make_shared<op::Parameter>(type, shape2);
        try
        {
            auto logical_xor = make_shared<op::v1::LogicalXor>(input1, input2);
        }
...
TEST(type_prop, logical_xor_incorrect_type_f32)
{
    incorrect_init(
        element::f32,
...
TEST(type_prop, logical_xor_incorrect_type_f64)
{
    incorrect_init(
        element::f64,
...
TEST(type_prop, logical_xor_incorrect_type_i32)
{
    incorrect_init(
        element::i32,
...
TEST(type_prop, logical_xor_incorrect_type_i64)
{
    incorrect_init(
        element::i64,
...
TEST(type_prop, logical_xor_incorrect_type_u32)
{
    incorrect_init(
        element::u32,
...
TEST(type_prop, logical_xor_incorrect_type_u64)
{
    incorrect_init(
        element::u64,
...
TEST(type_prop, logical_xor_incorrect_shape)
{
    incorrect_init(
        element::boolean, "Argument shapes are inconsistent", Shape{1, 3, 6}, Shape{1, 2, 3});
}

TEST(type_prop, logical_xor_broadcast)
{
    ...
    ASSERT_EQ(logical_xor->get_element_type(), element::boolean);
    ASSERT_EQ(logical_xor->get_shape(), (Shape{1, 3, 6}));
}
