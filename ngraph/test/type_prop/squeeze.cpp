...
{
    auto param = make_shared<op::v0::Parameter>(element::f32, Shape{1, 4, 1, 4, 1, 8});
    auto axes_node =
        make_shared<ngraph::op::v0::Constant>(element::u64, Shape{2}, vector<int64_t>{0, 2});
    auto squeeze = make_shared<op::v0::Squeeze>(param, axes_node);

    ASSERT_EQ(squeeze->get_element_type(), element::f32);
    ASSERT_EQ(squeeze->get_shape(), (Shape{4, 4, 1, 8}));

    axes_node = make_shared<ngraph::op::v0::Constant>(element::u64, Shape{0}, vector<int64_t>{});
    auto squeeze_default_axes = make_shared<op::v0::Squeeze>(param, axes_node);
...
{
    auto param = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic(6));
    auto axes_node =
        make_shared<ngraph::op::v0::Constant>(element::u64, Shape{2}, vector<int64_t>{0, 2});
    auto squeeze = make_shared<op::v0::Squeeze>(param, axes_node);
...
{
    auto param = make_shared<op::v0::Parameter>(element::f32, Shape{1, 2, 3, 4});
    auto axes_node =
        make_shared<ngraph::op::v0::Constant>(element::u64, Shape{2}, vector<int64_t>{0, 2});
