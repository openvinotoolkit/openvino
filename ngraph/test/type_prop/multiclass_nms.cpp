...
TEST(type_prop, multiclass_nms_scalar_inputs_check)
{
    ...
    try
    {
        make_shared<op::v8::MulticlassNms>(boxes, scores, non_0d_or_1d, scalar, scalar);
    }
    catch (const NodeValidationFailure& error)
    {
        EXPECT_HAS_SUBSTRING(error.what(),
    ...
    try
    {
        make_shared<op::v8::MulticlassNms>(boxes, scores, scalar, non_0d_or_1d, scalar);
    }
    catch (const NodeValidationFailure& error)
    {
        EXPECT_HAS_SUBSTRING(error.what(),
    ...
    try
    {
        make_shared<op::v8::MulticlassNms>(boxes, scores, scalar, scalar, non_0d_or_1d);
    }
    catch (const NodeValidationFailure& error)
    {
        EXPECT_HAS_SUBSTRING(error.what(),
    ...
    try
    {
        make_shared<op::v8::MulticlassNms>(boxes, scores, scalar, scalar, scalar, non_0d_or_1d);
    }
    catch (const NodeValidationFailure& error)
    {
        EXPECT_HAS_SUBSTRING(error.what(),
...
TEST(type_prop, multiclass_nms_output_shape)
{
...
TEST(type_prop, multiclass_nms_output_shape_2)
{
...
TEST(type_prop, multiclass_nms_output_shape_3)
{
    const auto boxes = make_shared<op::Parameter>(element::f32, Shape{2, 7, 4});
    const auto scores = make_shared<op::Parameter>(element::f32, Shape{2, 5, 7});
...
TEST(type_prop, multiclass_nms_output_shape_i32)
{
    ...
    const auto nms =
        make_shared<op::v8::MulticlassNms>(boxes,
                                               scores,
