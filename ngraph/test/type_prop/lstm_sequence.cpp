...
shared_ptr<op::v5::LSTMSequence> lstm_seq_tensor_initialization(const recurrent_sequence_parameters& param) {
    ...
    const auto W = make_shared<op::Parameter>(et, PartialShape{num_directions, hidden_size * 4, input_size});
    const auto R = make_shared<op::Parameter>(et, PartialShape{num_directions, hidden_size * 4, hidden_size});
...
TEST(type_prop, lstm_sequence_invalid_input_dynamic_rank) {
    ...
    auto lstm_sequence = lstm_seq_tensor_initialization(param);
    auto invalid_dynamic_tensor = make_shared<op::Parameter>(param.et, PartialShape::dynamic(Rank::dynamic()));
