...
{
    const auto data = make_shared<op::Parameter>(element::Type_t::f32, Shape{2, 3, 5, 5});
...
{
    const auto data = make_shared<op::Parameter>(element::Type_t::f32, PartialShape{10, 3, 5, 5});
    const auto rois =
        make_shared<op::Parameter>(element::Type_t::f32, PartialShape{Dimension{}, Dimension{}});
    const auto batch_indices = make_shared<op::Parameter>(element::Type_t::i32, Shape{9});
    const auto op = make_shared<op::v3::ROIAlign>(data, rois, batch_indices, 3, 4, 1, 1.0f, "avg");
...
{
    const auto data = make_shared<op::Parameter>(element::Type_t::f32, Shape{1, 10, 3, 5, 5});
    const auto rois = make_shared<op::Parameter>(element::Type_t::f32, Shape{1, 4});
    const auto batch_indices = make_shared<op::Parameter>(element::Type_t::i32, Shape{1});
...
{
    const auto data = make_shared<op::Parameter>(element::Type_t::f32, Shape{10, 3, 5, 5});
    const auto rois = make_shared<op::Parameter>(element::Type_t::f32, Shape{1, 5});
    const auto batch_indices = make_shared<op::Parameter>(element::Type_t::i32, Shape{1});
