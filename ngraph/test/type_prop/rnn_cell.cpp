...
TEST(type_prop, rnn_cell_invalid_input_rank0) {
    ...
    ASSERT_THROW(make_shared<op::v0::RNNCell>(X, H_t, W, R, hidden_size), ngraph::NodeValidationFailure)
    ...
    ASSERT_THROW(make_shared<op::v0::RNNCell>(X, H_t, W, R, B, hidden_size), ngraph::NodeValidationFailure)
        << "RNNCell node was created with invalid data.";
}
