...
TEST(type_prop, rnn_cell_invalid_input)
{
    ...
    auto R = make_shared<opset4::Parameter>(element::Type_t::f32, Shape{hidden_size, hidden_size});
    ...
    W = make_shared<opset4::Parameter>(element::Type_t::f32, Shape{hidden_size, input_size});
    R = make_shared<opset4::Parameter>(element::Type_t::f32, Shape{hidden_size, 1});
    try
    {
        const auto rnn_cell = make_shared<opset4::RNNCell>(X, H_t, W, R, hidden_size);
        FAIL() << "RNNCell node was created with invalid data.";
    ...
    R = make_shared<opset4::Parameter>(element::Type_t::f32, Shape{hidden_size, hidden_size});
    ...
    auto B = make_shared<opset4::Parameter>(element::Type_t::f32, Shape{2 * hidden_size});
    try
    {
...
TEST(type_prop, rnn_cell_invalid_input_rank0)
{
    ...
    auto W = make_shared<opset4::Parameter>(element::Type_t::f32, PartialShape{});
