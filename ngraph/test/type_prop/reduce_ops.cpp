...
TYPED_TEST_P(ReduceArithmeticTest, reduce_basic_shape_infer)
{
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_basic_shape_infer_keep_dims)
{
    PartialShape data_ps{3, 4, 5};
    element::Type data_et = element::f32;
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_basic_shape_infer_scalar_axis)
{
    PartialShape data_ps{3, 4, 5};
    element::Type data_et = element::f32;
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_basic_shape_infer_axes_as_param)
{
    PartialShape data_ps{3, 4, 5};
    element::Type data_et = element::f32;
    ...
    ASSERT_EQ(reduce_op->get_output_element_type(0), data_et);
    ASSERT_TRUE(reduce_op->get_output_partial_shape(0).same_scheme(out_ps));
}

TYPED_TEST_P(ReduceArithmeticTest, reduce_dynamic_shape_reduced_axes_static)
{
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_dynamic_shape_reduced_axes_static_keep_dims)
{
    PartialShape data_ps{3, 4, 5, Dimension::dynamic()};
    element::Type data_et = element::f32;
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_dynamic_shape_reduced_axes_not_static)
{
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_dynamic_shape_reduced_axes_not_static_keep_dims)
{
    PartialShape data_ps{Dimension::dynamic(), 4, 5, Dimension::dynamic()};
    element::Type data_et = element::f32;
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_dynamic_shape_data)
{
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_data_dynamic_element_type)
{
    PartialShape data_ps{3, 4, 5};
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_invalid_axis_out_of_range)
{
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_invalid_axes_not_unique)
{
    ...
    std::vector<ReduceParams> test_cases{
        ReduceParams{data_ps, data_et, axes_ps, axes, axes_et, false},
        ReduceParams{data_ps, data_et, axes_ps, axes, axes_et, true}
    };
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_invalid_axes_shape)
{
    PartialShape data_ps{1, 2, 3};
    element::Type data_et = element::f32;
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_invalid_axes_et)
{
    element::Type data_et = element::f32;
    PartialShape data_ps{1, 2, 3};
    ...
    std::vector<element::Type> invalid_axes_et{
        element::f32,
        element::f16,
        element::i16,
        element::u32};
    
    for (const auto& axes_et : invalid_axes_et)
    {
        const ReduceParams params{data_ps, data_et, axes_ps, axes, axes_et, keep_dims};
        try
        {
            auto reduce_op = makeReduceOp<TypeParam>(params, true);
            FAIL() << "Invalid element type of axes input not detected";
        }
        catch (const NodeValidationFailure& error)
        {
            EXPECT_HAS_SUBSTRING(error.what(), "Element type of axes input must be either i64 or i32.");
        }
        catch (...)
        {
...
TYPED_TEST_P(ReduceArithmeticTest, reduce_invalid_data_et)
{
...
INSTANTIATE_TYPED_TEST_CASE_P(type_prop, ReduceArithmeticTest, Types);