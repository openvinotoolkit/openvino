...
TEST(type_prop, group_conv_backprop_data)
{
    ...
    const auto weights = make_shared<op::v0::Parameter>(element::f32, Shape{2, 8, 2, 3, 3});
    // N x C_IN * GROUPS x H x W
    const auto data = make_shared<op::v0::Parameter>(element::f32, Shape{1, 16, 6, 6});
...
{
    // N x C_IN * GROUPS x H x W
    const auto data = make_shared<op::v0::Parameter>(element::f32, Shape{1, 16, 5, 5});
    // GROUPS x C_IN x C_OUT x kH x kW
    const auto weights = make_shared<op::v0::Parameter>(element::f32, Shape{1, 16, 2, 3, 3});
    const auto output_shape = op::v0::Constant::create(element::i64, Shape{2}, {3, 3});
...
{
    // GROUPS x C_IN x C_OUT x kH x kW
    auto weights = make_shared<op::v0::Parameter>(element::f32, Shape{21, 16, 20, 3, 3});
    // N x C_IN * GROUPS x H x W
    const auto data = make_shared<op::v0::Parameter>(element::f32, Shape{1, 16, 5, 5});
    ...
    // GROUPS x C_IN x C_OUT x kH x kW
    weights = make_shared<op::v0::Parameter>(element::f32, Shape{4, 16, 20, 3, 3});
    ...
    // GROUPS x C_IN x C_OUT x kH x kW
    weights = make_shared<op::v0::Parameter>(element::f32, Shape{4, 4, 20, 3, 3});
