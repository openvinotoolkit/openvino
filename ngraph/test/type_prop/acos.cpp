...
TEST(type_prop, acos_basic_shape_inference)
{
...
TEST(type_prop, acos_incompatible_input_type)
{
    Shape data_shape{3, 3};
    const auto param = make_shared<op::Parameter>(element::boolean, data_shape);
    ASSERT_THROW(make_shared<op::Acos>(param), ngraph::NodeValidationFailure);
}

TEST(type_prop, acos_dynamic_rank_input_2D)
{
...
TEST(type_prop, acos_dynamic_rank_input_3D)
{
