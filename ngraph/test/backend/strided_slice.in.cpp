...
void check_strided_slice_stride_optional_success(const element::Type& input_element_type,
    ...
    auto f = std::make_shared<Function>(NodeVector{strided_slice}, ParameterVector{arg, begin_op, end_op});
...
NGRAPH_TEST(${BACKEND_NAME}, strided_slice_0) {
    check_strided_slice_success<uint32_t>(element::u32,
                                          Shape{2, 3, 4},
...
NGRAPH_TEST(${BACKEND_NAME}, strided_slice_1) {
    check_strided_slice_success<uint32_t>(
        ...
        Shape{2, 4, 2, 2, 1, 2, 2},
        std::vector<uint32_t>{185,  187,  189,  191,  169,  171,  173,  175,  313,  315,  317,  319,  297,  299,  301,
