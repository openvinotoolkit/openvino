...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_float32_case1)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::f32, element::Type_t::f32, 150, 10),
        ParameterVector{});
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_float16_case1)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::f16, element::Type_t::f16, 150, 10),
        ParameterVector{});
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_double)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::f64, element::Type_t::f64, 150, 10),
        ParameterVector{});
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_double_min_max)
{
    ...
    test_case.add_expected_output(
        Shape{3, 2, 4},
        vector<double>{-37.4826139820484058873262256383895874023437500000000000,
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_int32)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::i32, element::Type_t::i32, 100, 350),
        ParameterVector{});
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_int64)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::i64, element::Type_t::i64, 755, 951),
        ParameterVector{});
...
{
    auto out_shape =
        make_shared<opset8::Constant>(element::i32, Shape{3}, vector<int32_t>{4, 3, 5});
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::f32, element::Type_t::f16, 1976, 3461),
        ParameterVector{});
    ...
    test_case.add_expected_output(
        Shape{4, 3, 5},
        vector<float16>{-7.0667686462, 0.7613515854,  15.2241306305, -7.0545663834, -8.1617279053,
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_f16_to_bf16)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::f16, element::Type_t::bf16, 1976, 3461),
        ParameterVector{});
    ...
    test_case.add_expected_output(
        Shape{4, 3, 5},
        bfloat16::from_float_vector(vector<float>{
            4.679687500,  0.664062500,  3.687500000,  -2.001953125, 4.703125000,  3.906250000,
            1.593750000,  -3.964843750, 2.453125000,  0.476562500,  -1.396484375, -3.085937500,
            -4.421875000, 0.175781250,  -4.929687500, 0.703125000,  -3.203125000, 3.367187500,
            3.421875000,  -2.949218750, -3.447265625, 3.164062500,  2.597656250,  0.945312500,
            -1.132812500, -3.740234375, -1.386718750, 2.167968750,  -1.933593750, -2.792968750,
            0.671875000,  -2.187500000, -1.533203125, 2.753906250,  4.531250000,  1.367187500,
            -2.578125000, 3.437500000,  1.257812500,  0.781250000,  -1.943359375, 2.695312500,
            -0.332031250, 0.437500000,  -1.152343750, -2.958984375, -4.054687500, -2.519531250,
            1.218750000,  1.777343750,  -0.429687500, -2.998046875, -2.792968750, 3.140625000,
            -0.820312500, 0.878906250,  4.304687500,  -1.357421875, -1.689453125, -4.968750000}));
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_f32_to_f64)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::f32, element::Type_t::f64, 1976, 3461),
        ParameterVector{});
    ...
    test_case.add_expected_output(
        Shape{4, 3, 5},
        vector<double>{-0.8044512271881103516, -0.2825765609741210938, 0.6816086769104003906,
                       -0.8036377429962158203, -0.8774485588073730469, -0.7119407653808593750,
                       0.5436179637908935547,  -0.0585684776306152344, -0.6792614459991455078,
                       -0.2134892940521240234, -0.8036229610443115234, 0.6006326675415039062,
                       0.4870746135711669922,  0.0565228462219238281,  0.4516618251800537109,
                       -0.9493236541748046875, -0.9460010528564453125, 0.1764738559722900391,
                       -0.2195210456848144531, -0.3695788383483886719, -0.0155870914459228516,
                       -0.3660116195678710938, -0.2305274009704589844, 0.0804674625396728516,
                       -0.7477083206176757812, 0.3774721622467041016,  0.2552151679992675781,
                       0.7318644523620605469,  0.2705826759338378906,  0.8384327888488769531,
                       -0.7147052288055419922, -0.6283493041992187500, 0.3572623729705810547,
                       -0.2493224143981933594, 0.0007209777832031250,  -0.0855379104614257812,
                       0.6502056121826171875,  0.2470321655273437500,  0.7848207950592041016,
                       -0.3062553405761718750, -0.5775620937347412109, -0.6536207199096679688,
                       -0.1444172859191894531, -0.3233535289764404297, 0.1734337806701660156,
                       -0.4118154048919677734, 0.1858141422271728516,  0.6226191520690917969,
                       -0.8440864086151123047, -0.2947564125061035156, 0.7598772048950195312,
                       -0.3812987804412841797, -0.8822703361511230469, -0.9739222526550292969,
                       0.2156782150268554688,  -0.2166533470153808594, 0.7985670566558837891,
                       -0.7147548198699951172, -0.4423019886016845703, -0.8056633472442626953});
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_f64_to_i32)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::f64, element::Type_t::i32, 1976, 3461),
        ParameterVector{});
    ...
    test_case.add_expected_output(
        Shape{4, 3, 5},
        vector<int32_t>{56,  45,  -2,  -65, -43, 57,  90,  61, -57, -76, 88,  -84, -98, -96, -84,
                        -72, -14, -99, 20,  -72, 38,  -16, 39, 49,  25,  -92, -6,  73,  -61, -54,
                        5,   -65, -80, -11, 71,  -35, 49,  36, 55,  74,  64,  -88, 3,   -46, -61,
                        27,  15,  -23, -4,  -10, 35,  17,  31, -82, -78, 46,  19,  -41, 2,   75});
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_f16_to_i64)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::f16, element::Type_t::i64, 1976, 3461),
        ParameterVector{});
    ...
    test_case.add_expected_output(
        Shape{4, 3, 5},
        vector<int64_t>{42,   -58,  17,   -125, 43,   23,   -35, -174, -14,  -63,  -110, -152,
                        -186, -71,  -198, -57,  -155, 9,    11,  -149, -161, 4,    -10,  -51,
                        -103, -169, -110, -21,  -123, -145, -58, -130, -113, -6,   38,   -41,
                        -140, 11,   -44,  -56,  -124, -8,   -83, -64,  -104, -149, -176, -138,
                        -45,  -31,  -86,  -150, -145, 4,    -96, -53,  33,   -109, -117, -199});
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_i32_to_i64)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::i32, element::Type_t::i64, 1976, 3770),
        ParameterVector{});
    ...
    test_case.add_expected_output(
        Shape{4, 3, 5},
        vector<int64_t>{-1269, -342, -164,  -1154, -799,  -820,  -762,  -288,  -1065, -1327,
                        -945,  -969, -609,  -1029, -1224, -1088, 0,     -727,  -808,  -101,
                        -579,  -284, -1000, -922,  -684,  -859,  -1068, -668,  -1079, -861,
                        -607,  -877, -197,  -889,  -441,  -1115, -1425, -308,  -1209, -1318,
                        -142,  32,   -511,  -1060, -1304, -1403, -93,   -676,  -675,  -1389,
                        -229,  -376, -1111, -1218, -38,   -447,  -1031, -1147, -181,  -1490});
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_i64_to_i32)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::i64, element::Type_t::i32, 1976, 3770),
        ParameterVector{});
    ...
    test_case.add_expected_output(
        Shape{4, 3, 5},
        vector<int32_t>{-1101, -1198, -869,  -1060, -807,  -619,  -1143, -72,   -442,  -854,
                        7,     -362,  -848,  -96,   -135,  -99,   -791,  -1381, -1493, -1287,
                        -1020, -921,  -792,  -889,  -169,  -1375, -789,  -600,  -1293, -1071,
                        -766,  -621,  -1064, -798,  -174,  -397,  -277,  -1153, -474,  -830,
                        -736,  -172,  -881,  -1464, 49,    -495,  -1355, -1280, -530,  -714,
                        -986,  -1277, -1077, -37,   -1110, -1018, -1165, -401,  -188,  -79});
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_i32_to_f64)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::i32, element::Type_t::f64, 1976, 3770),
        ParameterVector{});
    ...
    test_case.add_expected_output(
        Shape{4, 3, 5},
        vector<double>{-1169, -1342, -1264, -1204, -1349, -1370, -1112, -1338, -1065, -1077,
                       -1395, -1469, -1459, -1229, -1024, -1238, -1300, -1377, -1308, -1401,
                       -1179, -1384, -1250, -1022, -1434, -1209, -1118, -1118, -1179, -1311,
                       -1107, -1277, -1097, -1339, -1491, -1465, -1325, -1008, -1359, -1268,
                       -1192, -1118, -1411, -1210, -1004, -1353, -1493, -1376, -1175, -1339,
                       -1429, -1026, -1461, -1268, -1188, -1297, -1231, -1397, -1431, -1140});
...
NGRAPH_TEST(${BACKEND_NAME}, random_uniform_i32_to_bf16)
{
    ...
    auto f = make_shared<Function>(
        make_shared<opset8::RandomUniform>(
            out_shape, min_val, max_val, element::Type_t::i32, element::Type_t::bf16, 1976, 3770),
        ParameterVector{});
    ...
    test_case.add_expected_output(
        Shape{4, 3, 5},
        bfloat16::from_float_vector(vector<float>{
            231, 358, 436, 496, 251, 130, 488, 362, 135, 423, 305, 131, 341, 471, 376,
            462, 200, 223, 492, 499, 121, 316, 350, 378, 366, 191, 382, 282, 421, 389,
            393, 323, 403, 361, 109, 435, 175, 392, 141, 132, 408, 282, 389, 390, 496,
            347, 307, 424, 125, 261, 471, 474, 139, 132, 112, 303, 369, 403, 169, 360}));
