...
NGRAPH_TEST(${BACKEND_NAME}, mish_f32)
{
    ...
    make_mish_test<element::f32>(shape1).test({test::rand_normal_tag(), 0, 20, shape1}, f_mish, 1e-5f);
    make_mish_test<element::f32>(shape2).test({test::rand_normal_tag(), 0, 20, shape2}, f_mish, 1e-5f);
}
...
NGRAPH_TEST(${BACKEND_NAME}, mish_f16)
{
    ...
    make_mish_test<element::f16>(shape1).test({test::rand_normal_tag(), 0, 20, shape1}, f_mish);
    make_mish_test<element::f16>(shape2).test({test::rand_normal_tag(), 0, 20, shape2}, f_mish);
}

NGRAPH_TEST(${BACKEND_NAME}, mish_dynamic)
{
    make_mish_test<element::f32>(PartialShape::dynamic())
        .test({test::rand_normal_tag(), 0, 20, Shape{{2, 3, 4, 5}}}, f_mish);
