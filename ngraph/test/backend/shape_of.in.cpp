...
NGRAPH_TEST(${BACKEND_NAME}, shape_of_scalar_v0)
{
    ...
    auto A = std::make_shared<op::Parameter>(element::Type_t::f32, input_shape);
    auto f = std::make_shared<Function>(std::make_shared<op::v0::ShapeOf>(A), ParameterVector{A});

    auto backend = runtime::Backend::create("${BACKEND_NAME}");

    auto a = backend->create_tensor(element::Type_t::f32, input_shape);
    copy_data(a, vector<float>{0});
    auto result = backend->create_tensor(element::Type_t::i64, output_shape);
...
NGRAPH_TEST(${BACKEND_NAME}, shape_of_scalar_v3)
{
    ...
    auto f = std::make_shared<Function>(
        OutputVector{std::make_shared<op::v3::ShapeOf>(A),
                     std::make_shared<op::v3::ShapeOf>(A, element::Type_t::i32)},
        ParameterVector{A});
    ...
    auto a = backend->create_tensor(element::Type_t::f32, input_shape);
    copy_data(a, vector<float>{0});
    auto result64 = backend->create_tensor(element::Type_t::i64, output_shape);
    auto result32 = backend->create_tensor(element::Type_t::i32, output_shape);
