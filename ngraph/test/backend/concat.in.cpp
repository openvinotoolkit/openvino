...
NGRAPH_TEST(${BACKEND_NAME}, concat_negative_axis) {
    ...
    auto f = make_shared<Function>(make_shared<op::Concat>(NodeVector{A, B, C}, -1), ParameterVector{A, B, C});
    ...
    EXPECT_TRUE(test::all_close_f((vector<float>{2, 4, 1, 2, 4, 2, 3, 5, 8, 16, 8, 16, 32, 7, 11, 13}),
...
NGRAPH_TEST(${BACKEND_NAME}, concat_matrix_rowwise) {
    ...
    auto C = make_shared<op::Parameter>(element::f32, shape_c);
    Shape shape_r{8, 2};
    auto f = make_shared<Function>(make_shared<op::Concat>(NodeVector{A, B, C}, 0), ParameterVector{A, B, C});
    ...
    auto handle = backend->compile(f);
    handle->call_with_validate({result}, {a, b, c});
    EXPECT_TRUE(test::all_close_f((vector<float>{2, 4, 8, 16, 1, 2, 4, 8, 16, 32, 2, 3, 5, 7, 11, 13}),
                                  read_vector<float>(result),
                                  MIN_FLOAT_TOLERANCE_BITS));
}
...
NGRAPH_TEST(${BACKEND_NAME}, concat_matrix_int64) {
    ...
    auto handle = backend->compile(f);
    handle->call_with_validate({result}, {a, b, c});
    EXPECT_EQ((vector<int64_t>{2, 4, 8, 16, 1, 2, 4, 8, 16, 32, 2, 3, 5, 7, 11, 13}), read_vector<int64_t>(result));
}
...
NGRAPH_TEST_P(${BACKEND_NAME}, concat_vector_params, concat_vector_large) {
    ...
    EXPECT_TRUE(test::all_close_f(ref_result, read_vector<float>(result), MIN_FLOAT_TOLERANCE_BITS));
}
...
NGRAPH_INSTANTIATE_TEST_SUITE_P(${BACKEND_NAME}, input_sizes, concat_vector_params, testing::Values(100, 128, 999));
...
NGRAPH_TEST(${BACKEND_NAME}, concat_in_place_2d_tensor) {
    ...
    EXPECT_TRUE(test::all_close_f((vector<float>{3, 7, 2}), read_vector<float>(result), MIN_FLOAT_TOLERANCE_BITS));
}
...
NGRAPH_TEST(${BACKEND_NAME}, concat_in_place_propagate_2d_tensor) {
    ...
    Shape shape_r{3, 1};
    auto f =
        make_shared<Function>(make_shared<op::Concat>(NodeVector{concat1, subtract}, 0), ParameterVector{A, B, C, D});
...
NGRAPH_TEST(${BACKEND_NAME}, concat_zero_length_1d_last) {
    ...
    auto handle = backend->compile(f);
    handle->call_with_validate({result}, {a, b});
    EXPECT_TRUE(test::all_close_f((vector<float>{1, 2, 3, 4}), read_vector<float>(result), MIN_FLOAT_TOLERANCE_BITS));
}
...
NGRAPH_TEST(${BACKEND_NAME}, concat_zero_zero) {
    ...
    EXPECT_TRUE(test::all_close_f(vector<float>{}, read_vector<float>(result), MIN_FLOAT_TOLERANCE_BITS));
}
