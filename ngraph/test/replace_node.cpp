...
TEST(replace_node, replace_nodes)
{
    ...
    auto y_replacement = make_shared<op::Constant>(element::Type_t::f32, Shape{2}, vector<float>{3, 4});
    auto k_replacement = make_shared<op::Constant>(element::Type_t::f32, Shape{2}, vector<float>{5, 6});
