...
namespace referenceTest
{
    ...
    private:
        ...
        void GetConfidenceScores(const dataType* confData,
                            ...
                            std::vector<std::vector<std::vector<dataType>>>& priorVariances)
        {
            ...
            {
                priorData += attrs.variance_encoded_in_target ? n * numPriors * priorSize
            ...
            for (int i = 0; i < numImages; ++i)
            {
                ...
                for (int c = 0; c < numLocClasses; ++c)
                {
                    ...
                    const std::vector<NormalizedBBox>& labelLocPreds =
                        locPreds[i].find(label)->second;
                    DecodeBBoxes(
                        decodePriorBboxes, currPrVar, labelLocPreds, decodeBboxesImage[label]);
                }
            }
        }
                      ...
                      std::map<int, std::vector<int>>& indices)
        {
            ...
            while (scoreIndexPairs.size() != 0)
            {
                ...
                for (int i = 0; i < currInd.size(); i++)
                {
                    ...
                    auto currBbox =
                        attrs.share_location ? decodeBboxesImage.at(-1) : decodeBboxesImage.at(cls);
