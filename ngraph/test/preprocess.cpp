...
TEST(pre_post_process, tensor_data_shape_with_spatial) {
    auto f = create_simple_function(element::f32, PartialShape{Dimension::dynamic(), 3, 224, 224});
    EXPECT_THROW(f = PrePostProcessor()
                         .input(InputInfo()
                                    .tensor(InputTensorInfo()
                                                .set_layout("NCHW")
                                                .set_data_shape({1, 3, 224, 224})
    ...
    EXPECT_THROW(
        f = PrePostProcessor()
                .input(InputInfo()
                           .tensor(InputTensorInfo().set_layout("NCHW").set_spatial_dynamic_shape().set_data_shape(
                               {1, 3, 224, 224}))
                           .network(InputNetworkInfo().set_layout("NCHW")))
                .build(f),
        ov::AssertFailure);
...
TEST(pre_post_process, tensor_data_shape_with_spatial_static) {
    auto f = create_simple_function(element::f32, PartialShape{Dimension::dynamic(), 3, 224, 224});
    EXPECT_THROW(f = PrePostProcessor()
                         .input(InputInfo()
                                    .tensor(InputTensorInfo()
                                                .set_layout("NCHW")
                                                .set_data_shape({1, 3, 224, 224})
                                                .set_spatial_static_shape(640, 480))
                                    .network(InputNetworkInfo().set_layout("NCHW")))
                         .build(f),
                 ov::AssertFailure);
    ...
    EXPECT_THROW(
        f = PrePostProcessor()
                .input(
                    InputInfo()
                        .tensor(InputTensorInfo().set_layout("NCHW").set_spatial_static_shape(640, 480).set_data_shape(
                            {1, 3, 224, 224}))
                        .network(InputNetworkInfo().set_layout("NCHW")))
                .build(f),
        ov::AssertFailure);
