...
static std::shared_ptr<Function> create_simple_function2(element::Type type, const PartialShape& shape) {
    auto data1 = std::make_shared<ngraph::op::v0::Parameter>(element::dynamic, shape);
    data1->set_friendly_name("input1");
    auto convert = std::make_shared<ngraph::op::v0::Convert>(data1, type);
    auto res = std::make_shared<ngraph::op::v0::Result>(convert);
    res->set_friendly_name("Result");
    return std::make_shared<ngraph::Function>(ngraph::ResultVector{res}, ngraph::ParameterVector{data1});
}

TEST(pre_post_process, simple_scale123) {
    auto f = create_simple_function2(ngraph::element::i32, ngraph::Shape{1, 2});
    f->validate_nodes_and_infer_types();
    auto result = std::make_shared<HostTensor>();
    f->evaluate({result}, {make_host_tensor<ngraph::element::f32>(ngraph::Shape{1, 2}, {1.0f, 2.0f})});
    ngraph::pass::VisualizeTree("after123.png").run_on_function(f);  // Visualize the nGraph function to an image
    auto result_val = read_vector<int>(result);
    std::cout << result_val[0] << "  --   " << result_val[1] << "\n";
//    std::vector<int> expec{1, 2};
    ASSERT_NE(f, nullptr);
    ASSERT_EQ(f->get_output_element_type(0), element::i32);
}
...
TEST(pre_post_process, simple_mean_scale) {
    ...
    auto result = std::make_shared<HostTensor>();
    f->evaluate({result}, {make_host_tensor<ngraph::element::f32>(
...
TEST(pre_post_process, convert_element_type_and_scale) {
    ...
    auto result = std::make_shared<HostTensor>();
    f->evaluate({result}, {make_host_tensor<ngraph::element::i32>(
            ngraph::Shape{1, 3, 2, 2},
