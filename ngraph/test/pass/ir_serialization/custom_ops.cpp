...
TEST_F(CustomOpsSerializationTest, CustomOpTransformation) {
    ...
    std::map<std::string, ov::OpSet> opsets;
...
TEST_F(CustomOpsSerializationTest, CustomOpNoExtensions) {
    ...
    auto result = read(m_out_xml_path, m_out_bin_path, exts);

    bool success;
    std::string message;
    std::tie(success, message) = compare_functions(result, f, true, false, false, true, true);
