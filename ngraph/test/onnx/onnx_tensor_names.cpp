...
NGRAPH_TEST(onnx_tensor_names, simple_model)
{
    ...
    ASSERT_EQ(ops[4]->get_output_tensor(0).get_names(),
              std::unordered_set<std::string>{"final_output"});
...
NGRAPH_TEST(onnx_tensor_names, node_multiple_outputs)
{
    ...
    ASSERT_EQ(ops[3]->get_output_tensor(0).get_names(), std::unordered_set<std::string>{"indices"});
    ASSERT_EQ(ops[4]->get_friendly_name(), "values");
    ASSERT_EQ(ops[4]->get_output_tensor(0).get_names(), std::unordered_set<std::string>{"values"});
