...
TEST(graph_util, get_subgraph_outputs_trivial_tests) {
    ...
    outputs = ngraph::get_subgraph_outputs(NodeVector{B, abs_b, neg_b, abs_b_neg, add_b}, NodeVector{});
    ASSERT_EQ(outputs, (NodeVector{}));
...
TEST(util, enum_mask_construction) {
    enum class Type : uint32_t { a = 0x1, b = 1 << 1, c = 1 << 2, d = 1 << 3 };
    {
        EnumMask<Type> m;
        EXPECT_EQ(0, m.value());
    }
    ...
    }
    {
        ...
        EXPECT_EQ((static_cast<uint32_t>(Type::a) | static_cast<uint32_t>(Type::c) | static_cast<uint32_t>(Type::d)),
                  a.value());
    }
}
...
TEST(util, apply_permutation_pshape) {
    ASSERT_TRUE(apply_permutation(PartialShape{0, Dimension::dynamic(), 2, 3}, AxisVector{2, 1, 0, 3})
                    .same_scheme(PartialShape{2, Dimension::dynamic(), 0, 3}));
}

TEST(util, apply_permutation_pshape_rank_dynamic) {
    ASSERT_TRUE(
        apply_permutation(PartialShape::dynamic(), AxisVector{2, 1, 0, 3}).same_scheme(PartialShape::dynamic()));
}

TEST(util, apply_permutation_pshape_too_short_fails) {
    ASSERT_THROW(apply_permutation(PartialShape{0, Dimension::dynamic(), 2, 3}, AxisVector{0, 1, 2}), CheckFailure);
}

TEST(util, apply_permutation_pshape_too_long_fails) {
    ASSERT_THROW(apply_permutation(PartialShape{0, Dimension::dynamic(), 2, 3}, AxisVector{0, 1, 2, 3, 3}),
                 CheckFailure);
}

TEST(util, apply_permutation_pshape_oob_axis_fails) {
    ASSERT_THROW(apply_permutation(PartialShape{0, Dimension::dynamic(), 2, 3}, AxisVector{0, 1, 2, 4}), CheckFailure);
}

TEST(util, apply_permutation_pshape_repeated_axis_fails) {
    ASSERT_THROW(apply_permutation(PartialShape{0, Dimension::dynamic(), 2, 3}, AxisVector{0, 1, 2, 2}), CheckFailure);
}
...
TEST(util, clone_function_op_annotations) {
    ...
    auto C = make_shared<op::Parameter>(element::f32, shape);
    auto f =
        make_shared<Function>(make_shared<op::v1::Add>(make_shared<op::v1::Add>(A, B), C), ParameterVector{A, B, C});
...
TEST(util, topological_sort_replace) {
    ...
    f->set_topological_sort([&custom_sorter_used](const std::vector<std::shared_ptr<Node>>& root_nodes) {
...
TEST(util_host_tensor_2_vector, ht_boolean_2_vec_bool) {
    ...
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input,
                                                                                         output,
                                                                                         element::boolean);
}
...
TEST(util_host_tensor_2_vector, ht_i8_2_vec_int64) {
    vector<int8_t> input{0, 1, std::numeric_limits<int8_t>::min(), std::numeric_limits<int8_t>::max()};
    vector<int64_t> output{0, 1, std::numeric_limits<int8_t>::min(), std::numeric_limits<int8_t>::max()};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::i8);
}

TEST(util_host_tensor_2_vector, ht_i16_2_vec_int64) {
    vector<int16_t> input{0, 1, std::numeric_limits<int16_t>::min(), std::numeric_limits<int16_t>::max()};
    vector<int64_t> output{0, 1, std::numeric_limits<int16_t>::min(), std::numeric_limits<int16_t>::max()};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::i16);
}

TEST(util_host_tensor_2_vector, ht_i32_2_vec_int64) {
    vector<int32_t> input{0, 1, std::numeric_limits<int32_t>::min(), std::numeric_limits<int32_t>::max()};
    vector<int64_t> output{0, 1, std::numeric_limits<int32_t>::min(), std::numeric_limits<int32_t>::max()};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::i32);
}

TEST(util_host_tensor_2_vector, ht_i64_2_vec_int64) {
    vector<int64_t> input{0, 1, std::numeric_limits<int64_t>::min(), std::numeric_limits<int64_t>::max()};
    vector<int64_t> output{input};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::i64);
}

TEST(util_host_tensor_2_vector, ht_bf16_2_vec_double) {
    vector<bfloat16> input{0, 1, std::numeric_limits<bfloat16>::min(), std::numeric_limits<bfloat16>::max()};
    vector<double> output{0, 1, std::numeric_limits<bfloat16>::min(), std::numeric_limits<bfloat16>::max()};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::bf16);
}

TEST(util_host_tensor_2_vector, ht_f16_2_vec_double) {
    vector<float16> input{0, 1, std::numeric_limits<float16>::min(), std::numeric_limits<float16>::max()};
    vector<double> output{0, 1, std::numeric_limits<float16>::min(), std::numeric_limits<float16>::max()};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::f16);
}
...
TEST(util_host_tensor_2_vector, ht_f32_2_vec_double) {
    ...
    vector<double> output{0, 1, std::numeric_limits<float>::min(), std::numeric_limits<float>::max()};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::f32);
}

TEST(util_host_tensor_2_vector, ht_f64_2_vec_double) {
    vector<double> input{0, 1, std::numeric_limits<double>::min(), std::numeric_limits<double>::max()};
    vector<double> output{0, 1, std::numeric_limits<double>::min(), std::numeric_limits<double>::max()};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::f64);
}

TEST(util_host_tensor_2_vector, ht_u8_2_vec_uint64) {
    vector<uint8_t> input{0, 1, std::numeric_limits<uint8_t>::min(), std::numeric_limits<uint8_t>::max()};
    vector<uint64_t> output{0, 1, std::numeric_limits<uint8_t>::min(), std::numeric_limits<uint8_t>::max()};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::u8);
}

TEST(util_host_tensor_2_vector, ht_u16_2_vec_uint64) {
    vector<uint16_t> input{0, 1, std::numeric_limits<uint16_t>::min(), std::numeric_limits<uint16_t>::max()};
    vector<uint64_t> output{0, 1, std::numeric_limits<uint16_t>::min(), std::numeric_limits<uint16_t>::max()};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::u16);
}

TEST(util_host_tensor_2_vector, ht_u32_2_vec_uint64) {
    vector<uint32_t> input{0, 1, std::numeric_limits<uint32_t>::min(), std::numeric_limits<uint32_t>::max()};
    vector<uint64_t> output{0, 1, std::numeric_limits<uint32_t>::min(), std::numeric_limits<uint32_t>::max()};
    host_tensor_2_vector_test<decltype(input)::value_type, decltype(output)::value_type>(input, output, element::u32);
}
...
TEST(evaluate_subgraph, split) {
    ...
    auto len_0 = std::make_shared<opset8::Divide>(shape, op::Constant::create(element::i64, Shape{}, {2}));
