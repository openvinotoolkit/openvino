...
public:
    ...
    static bool equal_descriptions(const InputDescripton* lhs, const InputDescripton* rhs) {
        ...
        std::stringstream s;
        s << "Type is not supported: [" << lhs->get_type_info().name << "]";
        throw ov::Exception(s.str());
    }
    ...
    bool parameter_and_input_match(size_t num_iterations) const {
        ...
        std::stringstream s;
        s << "Type is not supported: [" << m_description->get_type_info().name << "]";
        throw ov::Exception(s.str());
    }
...
Comparator::Result compare_io(ngraph::op::util::SubGraphOp* sub_lhs, ngraph::op::util::SubGraphOp* sub_rhs) {
