...
namespace
{
    ONNX_NAMESPACE::ModelProto parse_from_file(const std::string& file_path)
    {
        ...
        if (!model_proto.ParseFromIstream(&file_stream))
        {
            ...
            throw ngraph_error("Could not import the model");
#else
            ...
            {
                throw std::runtime_error("Could not import the prototxt model");
            }
#endif
        }
    ...
    const std::pair<bool, std::string> COMPARISON_OK{true, {}};

    std::pair<bool, std::string> compare_nodes(const ONNX_NAMESPACE::GraphProto& graph,
                                               const ONNX_NAMESPACE::GraphProto& ref_graph)
    {
        if (graph.node_size() != ref_graph.node_size())
        {
            return {false, "The number of nodes in compared models doesn't match"};
        }
        else
        {
            for (int i = 0; i < graph.node_size(); ++i)
            {
                ...
                {
                    return {false,
                            "Operation types are different at index " + std::to_string(i) + ": " +
    ...
    std::pair<bool, std::string> compare_value_info(const ONNX_NAMESPACE::ValueInfoProto& lhs,
                                                    ...
                                                    const std::string& item_type)
    {
        ...
        {
            return {false,
                    item_type + " names in the graph don't match: " + lhs.name() + " vs " +
                        rhs.name()};
        }
        ...
        {
            return {false,
                    "Element types don't match for " + item_type + "  " + lhs.name() + ": " +
        ...
        {
            return {false,
                    "Tensor ranks don't match for " + item_type + " " + lhs.name() + ": " +
        ...
        else
        {
            for (int j = 0; j < lhs_shape.dim_size(); ++j)
            {
                ...
                {
                    return {false,
                            "Dynamic vs static dimension mismatch for " + item_type + " " +
                                lhs.name() + " at index: " + std::to_string(j)};
                }
                else if (lhs_dim.has_dim_value() && lhs_dim.dim_value() != rhs_dim.dim_value())
                {
                    return {false,
                            "Shape dimensions don't match for " + item_type + " " + lhs.name() +
    ...
    std::pair<bool, std::string> compare_inputs(const ONNX_NAMESPACE::GraphProto& graph,
                                                const ONNX_NAMESPACE::GraphProto& ref_graph)
    {
        if (graph.input_size() != ref_graph.input_size())
        {
            return {false, "The number of inputs in compared models doesn't match"};
        }
        else
        {
    ...
    std::pair<bool, std::string> compare_outputs(const ONNX_NAMESPACE::GraphProto& graph,
                                                 const ONNX_NAMESPACE::GraphProto& ref_graph)
    {
        if (graph.output_size() != ref_graph.output_size())
        {
            return {false, "The number of outputs in compared models doesn't match"};
        }
        else
        {
    ...
    std::pair<bool, std::string> compare_initializers(const ONNX_NAMESPACE::GraphProto& graph,
                                                      const ONNX_NAMESPACE::GraphProto& ref_graph)
    {
        if (graph.initializer_size() != ref_graph.initializer_size())
        {
            return {false, "The number of initializers in compared models doesn't match"};
        }
        else
        {
            for (int i = 0; i < graph.initializer_size(); ++i)
            {
                ...
                {
                    return {false,
                            "Initializer names in the graph don't match: " + lhs.name() + " vs " +
                                rhs.name()};
                }
                else if (lhs.data_type() != rhs.data_type())
                {
                    return {false,
                            "Initializer data types in the graph don't match: " +
                                std::to_string(lhs.data_type()) + " vs " +
                                std::to_string(rhs.data_type())};
                }
                else if (lhs.dims_size() != rhs.dims_size())
                {
                    return {false,
                            "Initializer ranks in the graph don't match: " +
...
namespace ngraph
{
    namespace test
    {
        ...
        {
            std::pair<bool, std::string> res = COMPARISON_OK;

            const auto ref_model = parse_from_file(reference_model_path);
            const auto& ref_graph = ref_model.graph();

            res = compare_inputs(graph, ref_graph);
            if (!res.first)
            {
                return res;
            }

            res = compare_outputs(graph, ref_graph);
            if (!res.first)
            {
                return res;
            }

            res = compare_initializers(graph, ref_graph);
            if (!res.first)
            {
                return res;
            }

            res = compare_nodes(graph, ref_graph);
