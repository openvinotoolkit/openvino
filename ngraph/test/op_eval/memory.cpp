...
shared_ptr<ov::Function> AssignReadGraph()
{
    ...
    return make_shared<Function>(
        OutputVector{assign}, ParameterVector{p}, VariableVector{variable});
}

shared_ptr<ov::Function> AssignReadAddGraph()
{
    auto p = make_shared<op::Parameter>(element::f32, Shape{3});
...
shared_ptr<ov::Function> AssignReadMultiVariableGraph()
{
    auto c = std::make_shared<Constant>(element::f32, Shape{3}, std::vector<float>({0, 0, 0}));
    ...
    auto variable =
        make_shared<Variable>(VariableInfo{PartialShape::dynamic(), element::dynamic, "var_1"});
    auto variable_2 =
        make_shared<Variable>(VariableInfo{PartialShape::dynamic(), element::dynamic, "var_2"});
    ...
    return make_shared<Function>(
        OutputVector{assign}, ParameterVector{}, VariableVector{variable, variable_2});
}
...
TEST(op_eval, assign_readvalue_evaluation_context)
{
    ...
    eval_context["VariableContext"] =
        std::make_shared<VariantWrapper<VariableContext>>(variable_context);

    const int COUNT_RUNS = 10;
    for (int i = 0; i < COUNT_RUNS; ++i)
    {
        ASSERT_TRUE(fun->evaluate(
            {result}, {make_host_tensor<element::Type_t::f32>(Shape{3}, inputs)}, eval_context));
        EXPECT_EQ(result->get_element_type(), element::f32);
        EXPECT_EQ(result->get_shape(), Shape{3});
...
TEST(op_eval, assign_readvalue_add)
{
    ...
    auto variable_value =
        make_shared<VariableValue>(make_host_tensor<element::Type_t::f32>(Shape{3}, inputs));
...
TEST(op_eval, assign_readvalue_add_modify)
{
    ...
    for (int i = 0; i < COUNT_RUNS; ++i)
    {
        ...
        std::vector<float> expected_result{
            1 + inputs[0] * cnt, 2 + inputs[1] * cnt, 3 + inputs[2] * cnt};
        ASSERT_TRUE(test::all_close_f(read_vector<float>(result), expected_result));
    }
}
...
TEST(op_eval, assign_readvalue_add_modify_multi_variables)
{
    ...
    EvaluationContext eval_context;
    auto variable_context = std::make_shared<VariantWrapper<VariableContext>>(VariableContext());
    auto variable_value_1 =
        make_shared<VariableValue>(make_host_tensor<element::Type_t::f32>(Shape{3}, inputs_1));
    auto variable_value_2 =
        make_shared<VariableValue>(make_host_tensor<element::Type_t::f32>(Shape{3}, inputs_2));
