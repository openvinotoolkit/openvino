...
TEST(attributes, deformable_convolution_default_attributes) {
    NodeBuilder::get_ops().register_factory<op::v8::DeformableConvolution>();
    ...
    auto convolution =
        make_shared<op::v8::DeformableConvolution>(data, offsets, filters, strides, pads_begin, pads_end, dilations);
    NodeBuilder builder(convolution);
    auto g_convolution = as_type_ptr<op::v8::DeformableConvolution>(builder.create());
...
TEST(attributes, deformable_convolution_attributes) {
    ...
    auto pads_end = CoordinateDiff{0, 0};
    auto dilations = Strides{1, 1};
    auto convolution = make_shared<op::v8::DeformableConvolution>(data,
                                                                  offsets,
                                                                  filters,
                                                                  mask,
                                                                  strides,
