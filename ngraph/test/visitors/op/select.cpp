...
TEST(attributes, select) {
    NodeBuilder::get_ops().register_factory<op::v1::Select>();
    ...
    auto select = std::make_shared<op::v1::Select>(in_cond, in_then, in_else, auto_broadcast);
    NodeBuilder builder(select);

    const auto expected_attr_count = 1;
    EXPECT_EQ(builder.get_value_map_size(), expected_attr_count);

    auto g_select = ov::as_type_ptr<op::v1::Select>(builder.create());
