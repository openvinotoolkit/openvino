...
TEST(attributes, convolution_backprop_op) {
    NodeBuilder::get_ops().register_factory<op::v1::ConvolutionBackpropData>();
    ...
    auto convolution = make_shared<op::v1::ConvolutionBackpropData>(data,
                                                                    filters,
                                                                    strides,
                                                                    pads_begin,
                                                                    pads_end,
...
TEST(attributes, convolution_backprop_output_shape_output_padding) {
    ...
    for (auto padType : allPadTypes) {
        const auto convolution = make_shared<op::v1::ConvolutionBackpropData>(data,
                                                                              filter,
                                                                              output_shape,
                                                                              strides,
                                                                              pads_begin,
