...
TEST(dyn_elimination, transpose)
{
    ...
    auto new_reshape = as_type_ptr<op::Reshape>(f->get_results().at(0)->input_value(0).get_node_shared_ptr());
...
TEST(dyn_elimination, transpose_dyn_shape)
{
    ...
    auto new_transpose = as_type_ptr<op::Transpose>(f->get_results().at(0)->input_value(0).get_node_shared_ptr());
...
TEST(dyn_elimination, range)
{
    ...
    auto replacement = as_type_ptr<op::Constant>(f->get_results().at(0)->input_value(0).get_node_shared_ptr());
