...
static Output<Node> get_sub_matrix(const Output<Node>& node, size_t idx)
{
    ...
    auto begin = op::Constant::create(
        element::i64, {lower_bounds.size()}, static_cast<std::vector<size_t>>(lower_bounds));
    auto end = op::Constant::create(
        element::i64, {upper_bounds.size()}, static_cast<std::vector<size_t>>(upper_bounds));
