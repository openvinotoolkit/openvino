...
template<class T>
void shape_infer(const ExperimentalDetectronDetectionOutput* op, const std::vector<T> &input_shapes, std::vector<T> &output_shapes) {
    ...
    if (deltas_shape_rank_is_static) {
        ...
        NODE_VALIDATION_CHECK(op,
                              deltas_shape[1].is_dynamic() || deltas_shape[1].get_length() == op->m_attrs.num_classes * 4,
    ...
    if (scores_shape_is_static) {
        ...
        NODE_VALIDATION_CHECK(op,
                              scores_shape[1].is_dynamic() || scores_shape[1].get_length() == op->m_attrs.num_classes,
