...
namespace
{
                                            ...
                                            bool& is_separate_first)
    {
        ...
        labels_inds.insert(
            labels_inds.end(), separate_labels_inds.begin(), separate_labels_inds.end());
                                      ...
                                      const Shape& separate_sub_shape2)
    {
        ...
        matmul_output_shape.insert(
            matmul_output_shape.end(), common_sub_shape.begin(), common_sub_shape.end());
        ...
        else if (separate_sub_shape_prod1.size() > 0)
        {
            ...
            size_t prod = 1;
            for (auto dim : separate_sub_shape_prod1)
            {
                prod *= dim;
            }
            separate_sub_shape_prod1.clear();
            separate_sub_shape_prod1.push_back(prod);
        }
        ...
        else if (separate_sub_shape_prod2.size() > 0)
        {
            ...
            // presented with just one dimension for MatMul
            size_t prod = 1;
            for (auto dim : separate_sub_shape_prod2)
            {
                prod *= dim;
            }
            separate_sub_shape_prod2.clear();
            separate_sub_shape_prod2.push_back(prod);
        }
    ...
    HostTensorPtr unsqueeze_input(const HostTensorPtr& input, std::vector<int64_t>& unsqueeze_axes)
    {
        ...
        HostTensorPtr output =
            std::shared_ptr<HostTensor>(new HostTensor(input->get_element_type(), output_shape));
                         ...
                         bool is_separate_first)
    {
        ...
        HostTensorPtr output =
            std::shared_ptr<HostTensor>(new HostTensor(input->get_element_type(), new_shape));

        std::vector<size_t> broadcast_axes(old_shape.size());
        std::iota(
            broadcast_axes.begin(), broadcast_axes.end(), new_shape.size() - old_shape.size());
                             ...
                             size_t input_ind2)
    {
        ...
        for (size_t label_ind = 0; label_ind < labels1.size(); ++label_ind)
        {
            ...
            if (iter != labels2.end())
            {
                bool is_dim_reduced = is_dimension_reduced(
                    input_subscripts, output_subscript, label, {input_ind1, input_ind2});
                common_part += label;
                if (is_dim_reduced)
                {
        ...
        if (reduced_labels_inds1.empty())
        {
            ...
            auto label_to_dim_map1 =
                compute_label_dim_map(input1->get_partial_shape().rank(), input_subscript1);
            auto label_to_dim_map2 =
                compute_label_dim_map(input2->get_partial_shape().rank(), input_subscript2);
            ...
            update_operands(
                inputs, input_subscripts, input_ind1, input_ind2, mul_output, resultant_subscript);
            return;
        }
        ...
        size_t common_dims_begin2, common_dims_end2, reduced_dims_begin2, reduced_dims_end2,
            separate2_dims_begin, separate2_dims_end;
        ...
        Shape matmul_output_shape =
            compute_matmul_output_shape(common_sub_shape, separate1_sub_shape, separate2_sub_shape);
    ...
    {
        // parse equation to extract input subscripts and output subscript
        std::vector<std::string> input_subscripts;
        ...
        {
            contract_two_inputs<T>(
                int_inputs, input_subscripts, output_subscript, inds_pair.first, inds_pair.second);
        }
        ...
        // write the result into output host tensor
