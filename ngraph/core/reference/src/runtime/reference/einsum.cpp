            ...
            namespace einsum_details
            {
                                          ...
                                          const std::vector<size_t>& excluded_indices)
                {
                    for (size_t input_ind = 0; input_ind < input_subscripts.size(); ++input_ind)
                    {
                        ...
                        // list
                        bool check_subscript = (std::find(excluded_indices.begin(),
                                                          excluded_indices.end(),
                                                          input_ind) == excluded_indices.end());
                        if (check_subscript &&
                            input_subscript.find(label_to_check) != std::string::npos)
                        {
                            return false;
                        }
                    }
                ...
                ///
                std::string
                    generate_grouping_subscript(const std::string& input_subscript,
                                                ...
                                                bool& is_separate_first)
                {
                    ...
                    labels_inds.insert(
                        labels_inds.end(), reduced_labels_inds.begin(), reduced_labels_inds.end());
                    labels_inds.insert(labels_inds.end(),
                                       separate_labels_inds.begin(),
                                       separate_labels_inds.end());
                ...
                std::unordered_map<std::string, std::vector<size_t>>
                    compute_label_dim_map(const Rank& input_rank,
                                    ...
                                    bool is_separated_first)
                {
                    ...
                    if (std::find(common_labels.begin(), common_labels.end(), ellipsis) !=
                        common_labels.end())
                    {
                    ...
                    if (std::find(sep_labels.begin(), sep_labels.end(), ellipsis) !=
                        sep_labels.end())
                    {
                        NGRAPH_CHECK(label_to_dim_map.find(ellipsis) != label_to_dim_map.end());
                                                  ...
                                                  const Shape& separate_sub_shape2)
                {
                    ...
                    matmul_output_shape.insert(matmul_output_shape.end(),
                                               common_sub_shape.begin(),
                                               common_sub_shape.end());
                ...
                template <typename T>
                HostTensorPtr unsqueeze_input(const HostTensorPtr& input,
                                              std::vector<int64_t>& unsqueeze_axes)
                {
                    ...
                    HostTensorPtr output = std::shared_ptr<HostTensor>(
                        new HostTensor(input->get_element_type(), output_shape));
                                  ...
                                  size_t input_ind)
                {
                    ...
                    NGRAPH_CHECK(num_inputs == input_subscripts.size(),
                                 "Each input must have own subscript.");
                    ...
                    for (const auto& label : labels)
                    {
                        ...
                        bool is_dim_reduced = is_dimension_reduced(
                            input_subscripts, output_subscript, label, {input_ind});
                                     ...
                                     bool is_separate_first)
                {
                    ...
                    Shape new_shape;
                    new_shape.insert(
                        new_shape.end(), new_common_shape.begin(), new_common_shape.end());
                    if (is_separate_first)
                    {
                        new_shape.insert(
                            new_shape.end(), separate_shape.begin(), separate_shape.end());
                        new_shape.insert(
                            new_shape.end(), reduced_shape.begin(), reduced_shape.end());
                    }
                    else
                    {
                        new_shape.insert(
                    ...
                    HostTensorPtr output = std::shared_ptr<HostTensor>(
                        new HostTensor(input->get_element_type(), new_shape));

                    std::vector<size_t> broadcast_axes(old_shape.size());
                    std::iota(broadcast_axes.begin(),
                              broadcast_axes.end(),
                                             ...
                                             const std::vector<size_t>& repeated_label_dims)
                {
                    ...
                    HostTensorPtr identity = std::shared_ptr<HostTensor>(
                        new HostTensor(input_ptr->get_element_type(), identity_shape));

                    T* identity_data_ptr = identity->get_data_ptr<T>();
                    size_t data_size =
                        shape_size(identity_shape) * identity->get_element_type().size();
                    ...
                    std::unordered_map<std::string, std::vector<size_t>>& label_dim_map)
                {
                    ...
                    HostTensorPtr multi_identity =
                        build_identity<T>(input_ptr, repeated_label_dims);

                    for (size_t label_ind = 1; label_ind < repeated_labels.size(); ++label_ind)
                    {
                        NGRAPH_CHECK(label_dim_map.find(repeated_labels[label_ind]) !=
                                     label_dim_map.end());
                ...
                ///
                template <typename T>
                void extract_diagonal(HostTensorVector& inputs,
                                      std::vector<std::string>& input_subscripts,
                                      size_t input_ind)
                {
                    // perform sanity check for arguments
                                                       ...
                                                       bool is_separate_first)
                {
                    Shape new_shape;
                    new_shape.insert(
                        new_shape.end(), common_sub_shape.begin(), common_sub_shape.end());
                    ...
                    if (is_separate_first)
                    {
                        new_shape.insert(new_shape.end(),
                                         separate_sub_shape_prod.begin(),
                                         separate_sub_shape_prod.end());
                        new_shape.insert(new_shape.end(),
                                         reduced_sub_shape_prod.begin(),
                                         reduced_sub_shape_prod.end());
                    }
                    else
                    {
                        new_shape.insert(new_shape.end(),
                                         ...
                                         size_t input_ind2)
                {
                    ...
                    NGRAPH_CHECK(num_inputs == input_subscripts.size(),
                                 "Each input must have own subscript.");
                    NGRAPH_CHECK(input_ind2 < num_inputs && input_ind1 != input_ind2,
                                 "Incorrect input index is specified.");
                    ...
                    std::vector<std::string> common_labels, sep_labels1, sep_labels2,
                        reduced_labels;
                    for (size_t label_ind = 0; label_ind < labels1.size(); ++label_ind)
                    {
                        ...
                        if (iter != labels2.end())
                        {
                            bool is_dim_reduced = is_dimension_reduced(input_subscripts,
                                                                       output_subscript,
                                                                       label,
                                                                       {input_ind1, input_ind2});
                            common_part += label;
                            if (is_dim_reduced)
                            {
                                reduced_labels_inds1.push_back(static_cast<int64_t>(label_ind));
                                reduced_labels_inds2.push_back(
                                    static_cast<int64_t>(iter - labels2.begin()));
                                reduced_labels.push_back(label);
                            }
                            else
                            {
                                common_labels_inds1.push_back(static_cast<int64_t>(label_ind));
                                common_labels_inds2.push_back(
                                    static_cast<int64_t>(iter - labels2.begin()));
                    ...
                    if (reduced_labels_inds1.empty())
                    {
                        ...
                        auto label_to_dim_map2 = compute_label_dim_map(
                            input2->get_partial_shape().rank(), input_subscript2);
                        ...
                        {
                            NGRAPH_CHECK(label_to_dim_map2.find(sep_label2) !=
                                         label_to_dim_map2.end());
                            ...
                            for (size_t dim_ind = 0; dim_ind < label_dims.size(); ++dim_ind)
                            {
                                unsqueeze_axis1.push_back(unsqueeze_dim +
                                                          static_cast<int64_t>(dim_ind));
                            }
                        }
                        for (const auto& sep_label1 : separate_labels1)
                        {
                            NGRAPH_CHECK(label_to_dim_map1.find(sep_label1) !=
                                         label_to_dim_map1.end());
                        ...
                        HostTensorPtr unsqueeze_output1 =
                            unsqueeze_input<T>(input1, unsqueeze_axis1);
                        HostTensorPtr unsqueeze_output2 =
                            unsqueeze_input<T>(input2, unsqueeze_axis2);
                    ...
                    size_t common_dims_begin, common_dims_end, reduced_dims_begin, reduced_dims_end,
                        separate1_dims_begin, separate1_dims_end;
                    ...
                    size_t common_dims_begin2, common_dims_end2, reduced_dims_begin2,
                        reduced_dims_end2, separate2_dims_begin, separate2_dims_end;
                    ...
                    PartialShape common_sub_shape1 =
                        compute_sub_shape(input_shape1, common_dims_begin, common_dims_end);
                    PartialShape common_sub_shape2 =
                        compute_sub_shape(input_shape2, common_dims_begin2, common_dims_end2);

                    Shape reduced_sub_shape_prod =
                        compute_sub_shape(input_shape1, reduced_dims_begin, reduced_dims_end, true);
                    Shape reduced_sub_shape =
                        compute_sub_shape(input_shape1, reduced_dims_begin, reduced_dims_end);
                    Shape separate1_sub_shape =
                        compute_sub_shape(input_shape1, separate1_dims_begin, separate1_dims_end);
                    Shape separate2_sub_shape =
                        compute_sub_shape(input_shape2, separate2_dims_begin, separate2_dims_end);
                    ...
                    HostTensorPtr matmul_operand1 =
                        reshape_input_for_matmul<T>(input1,
                                                    common_sub_shape,
                                                    separate1_sub_shape,
                                                    reduced_sub_shape_prod,
                                                    is_separate_first1);
                    HostTensorPtr matmul_operand2 =
                        reshape_input_for_matmul<T>(input2,
                                                    common_sub_shape,
                                                    separate2_sub_shape,
                                                    reduced_sub_shape_prod,
                                                    is_separate_first2);
                    ...
                    Shape matmul_output_shape = compute_matmul_output_shape(
                        common_sub_shape, separate1_sub_shape, separate2_sub_shape);
                    HostTensorPtr matmul_output = std::shared_ptr<HostTensor>(
                        new HostTensor(matmul_operand1->get_element_type(), matmul_output_shape));
                    ...
                    Shape back_shape;
                    back_shape.insert(
                        back_shape.end(), common_sub_shape.begin(), common_sub_shape.end());
                    back_shape.insert(
                        back_shape.end(), separate1_sub_shape.begin(), separate1_sub_shape.end());
                    back_shape.insert(
                        back_shape.end(), separate2_sub_shape.begin(), separate2_sub_shape.end());
                ...
                template <typename T>
                void einsum_impl(const HostTensorVector& inputs,
                                 ...
                                 const std::string& equation)
                {
                    ...
                    {
                        contract_two_inputs<T>(int_inputs,
                                               input_subscripts,
                                               output_subscript,
                                               inds_pair.first,
                                               inds_pair.second);
