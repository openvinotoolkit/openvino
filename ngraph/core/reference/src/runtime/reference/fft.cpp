    ...
    namespace runtime
    {
        namespace reference
        {
            ...
            namespace
            {
                ...
                // Helper function to get lengths except length with respect to given axes.
                std::vector<int64_t> lengths_without_excluded(const std::vector<int64_t>& shape,
                                                              int64_t excluded_axes)
                {
                ...
                std::vector<int64_t> permute_vector(const std::vector<int64_t>& v,
                                                    const std::vector<int64_t>& axes_order)
                {
                    int64_t num_of_elems = static_cast<int64_t>(v.size());
                    std::vector<int64_t> result(num_of_elems);
                    for (int64_t i = 0; i < num_of_elems; ++i)
                    {
                        result[i] = v[axes_order[i]];
                    }
                    return result;
                }
                ...
                class Transposed_view final
                {
                public:
                    Transposed_view() = default;
                    Transposed_view(const Transposed_view& orig) = default;
                    ~Transposed_view() = default;

                    Transposed_view(complex_type* data_ptr,
                                    const std::vector<int64_t>& input_shape,
                                    const std::vector<int64_t>& axes_order)
                        : ptr_to_data{data_ptr}
                        , strides{permute_vector(compute_strides(input_shape), axes_order)}
                        , output_shape{permute_vector(input_shape, axes_order)}
                    {
                    }

                    complex_type& operator()(const std::vector<int64_t>& coords)
                    {
                        return ptr_to_data[offset_from_coords_and_strides(coords, strides)];
                    }

                    complex_type& operator()(const std::vector<int64_t>& coords) const
                    {
                        return ptr_to_data[offset_from_coords_and_strides(coords, strides)];
                    }
                ...
                class Flatten_indexing final
                {
                public:
                    Flatten_indexing() = default;
                    Flatten_indexing(const Flatten_indexing& orig) = default;
                    ~Flatten_indexing() = default;

                    Flatten_indexing(const std::vector<int64_t>& shape_to_flatten)
                        : strides{compute_strides(shape_to_flatten)}
                    {
                    }

                    std::vector<int64_t> operator()(int64_t i) const
                    {
                ...
                class Padded_flatten_view final
                {
                public:
                    Padded_flatten_view() = default;
                    Padded_flatten_view(const Padded_flatten_view& orig) = default;
                    ~Padded_flatten_view() = default;

                    Padded_flatten_view(const complex_type* data_ptr,
                                        const std::vector<int64_t>& padded_shape,
                                        const std::vector<int64_t>& nonpadded_shape)
                        : ptr_to_data{data_ptr}
                        , padded_data_shape{padded_shape}
                        , nonpadded_data_shape{nonpadded_shape}
                        , padded_shape_strides{compute_strides(padded_shape)}
                        , nonpadded_shape_strides{compute_strides(nonpadded_shape)}
                        , rank{padded_shape.size()}
                    {
                    }

                    const complex_type& operator()(int64_t idx) const
                    {
                        int64_t offset = 0;
                        auto coords = coords_from_index(idx, padded_shape_strides);

                        for (size_t i = 0; i < rank; ++i)
                        {
                            int64_t coord = coords[i];
                            if (coord >= nonpadded_data_shape[i])
                            {
                                return default_value;
                            }
                            offset += coord * nonpadded_shape_strides[i];
                        }
                ...
                private:
                    ...
                    std::vector<int64_t> nonpadded_data_shape;
                    std::vector<int64_t> padded_shape_strides;
                    std::vector<int64_t> nonpadded_shape_strides;
                    size_t rank;
                                        ...
                                        const std::vector<int64_t>& axes_order)
                {
                    ...
                    int64_t output_size = output_indexing.size();

                    std::vector<complex_type> result(output_size);
                    for (int64_t i = 0; i < output_size; ++i)
                    {
                        result[i] = transposed_view(output_indexing(i));
                    }
                    return result;
                }
                ...
                void copy_data_from_input(complex_type* result,
                                          ...
                                          const std::vector<int64_t>& output_fft_lengths,
                                          const std::vector<int64_t>& input_fft_lengths)
                {
                    Padded_flatten_view padded_view{
                        input_data, output_fft_lengths, input_fft_lengths};
                    int64_t padded_size = padded_view.padded_size();
                    for (int64_t idx = 0; idx < padded_size; ++idx)
                    {
                        result[idx] = padded_view(idx);
                    }
                }
                ...
                std::vector<int64_t> build_inverse_permutation(const std::vector<int64_t>& p)
                {
                    int64_t num_of_elems = static_cast<int64_t>(p.size());
                    std::vector<int64_t> result(num_of_elems);
                    for (int64_t i = 0; i < num_of_elems; ++i)
                    {
                        result[p[i]] = i;
                ...
                void copy_transposed_results_to_output(complex_type* output,
                                                       const std::vector<complex_type>& results,
                                                       const std::vector<int64_t>& permuted_shape,
                                                       const std::vector<int64_t>& permutation)
                {
                    auto inverse_permutation = build_inverse_permutation(permutation);
                    Transposed_view transposed_view{const_cast<complex_type*>(results.data()),
                                                    permuted_shape,
                                                    inverse_permutation};
                    const auto output_shape = transposed_view.get_output_shape();
                    ...
                    for (int64_t i = 0; i < output_size; ++i)
                    {
                        output[i] = transposed_view(output_indexing(i));
                    }
                }
                ...
                {
                    std::vector<int64_t> reversed_input_shape;
                    std::vector<int64_t> reversed_output_shape;
                    std::vector<int64_t> axes_order;
                    std::vector<int64_t> permuted_input_shape;
                    std::vector<int64_t> permuted_output_shape;
                                                               ...
                                                               const Shape& output_shape)
                {
                    ...
                    std::vector<int64_t> axes_order(fft_axes.begin(), fft_axes.end());
                    axes_order.insert(axes_order.end(), outer_axes.begin(), outer_axes.end());
                    ...
                    std::vector<int64_t> outer_shape(permuted_output_shape.begin() + fft_rank,
                                                     permuted_output_shape.end());
                    const auto outer_strides = compute_strides(outer_shape);
                    const int64_t outer_size = outer_strides.back();

                    result.reversed_input_shape = reversed_input_shape;
                    result.reversed_output_shape = reversed_output_shape;
                    result.axes_order = axes_order;
                    result.permuted_input_shape = permuted_input_shape;
                    result.permuted_output_shape = permuted_output_shape;
                    result.permuted_fft_input_lengths = permuted_fft_input_lengths;
                    result.permuted_fft_output_lengths = permuted_fft_output_lengths;
            ...
            void fft(const float* input_data,
                     ...
                     FFTKind fft_kind)
            {
                std::cout << "Started fft calculation.\n";
                ...
                int64_t fft_size = info.fft_size;
                ...
                std::cout << "permuted_fft_output_lengths: [";
                for (auto x : permuted_fft_output_lengths)
                {
                    std::cout << x << ", ";
                }
                std::cout << "]\n";
                std::cout << "permuted_fft_input_lengths:  [";
                for (auto x : permuted_fft_input_lengths)
                {
                    std::cout << x << ", ";
                }
                std::cout << "]\n";
                ...
                std::cout << "transposed_data:             [";
                for (auto z : transposed_data)
                {
                    std::cout << std::real(z) << ", " << std::imag(z) << ", ";
                }
                std::cout << "]\n";
                ...
                complex_type* transposed_result_ptr = transposed_result.data();
                ...
                for (int64_t outer_idx = 0; outer_idx < outer_size; ++outer_idx)
                {
                    ...
                    copy_data_from_input(data.data(),
                                         transposed_data_ptr,
                                         permuted_fft_output_lengths,
                                         permuted_fft_input_lengths);
                    std::cout << "Got current part of input data: [";
                    for (auto z : data)
                    {
                        std::cout << std::real(z) << ", " << std::imag(z) << ", ";
                    }
                    ...
                    if (!blob_is_zero(data.data(), fft_size))
                    {
                        ...
                        for (int64_t axis_idx = 0; axis_idx < fft_rank; ++axis_idx)
                        {
                            ...
                            int64_t current_fft_length = permuted_fft_output_lengths[axis_idx];

                            auto outer_fft_lengths =
                                lengths_without_excluded(permuted_fft_output_lengths, axis_idx);
                            auto fft_strides_for_outer_fft_axes =
                                lengths_without_excluded(fft_strides, axis_idx);

                            Flatten_indexing outer_fft_indexing{outer_fft_lengths};
                            int64_t outer_fft_size = outer_fft_indexing.size();
                            // Loop along with all FFT axes, except the current one.
                            for (int64_t outer_fft_idx = 0; outer_fft_idx < outer_fft_size;
                                 ++outer_fft_idx)
                            {
                                const auto outer_fft_coords = outer_fft_indexing(outer_fft_idx);
                    ...
                    // Copy data to transposed results buffer
                    memcpy(
                        transposed_result_ptr, data.data(), info.fft_size * sizeof(complex_type));

                    transposed_data_ptr += fft_size;
                    transposed_result_ptr += fft_size;
                }
                copy_transposed_results_to_output(complex_output_ptr,
                                                  transposed_result,
                                                  info.permuted_output_shape,
                                                  info.axes_order);
            }
...
//                    ~Padded_flatten_view() = default;
//
//                    Padded_flatten_view(complex_type* data_ptr,
//                                        const std::vector<int64_t>& padded_shape,
//                                        const std::vector<int64_t>& nonpadded_shape)
//                        : ptr_to_data{data_ptr}
//                        , padded_data_shape{padded_shape}
...
//                    }
//
// //                     complex_type& operator()(int64_t idx)
// //                     {
// //                         int64_t offset = 0;
// //                         auto coords = coords_from_index(idx, padded_shape_strides);
// //
...
//                {
//                    Padded_flatten_view padded_view{
//                        const_cast<complex_type*>(input_data), output_fft_lengths, input_fft_lengths};
//                    int64_t padded_size = padded_view.padded_size();
//                    for (int64_t idx = 0; idx < padded_size; ++idx)
//                    {
//                        result[idx] = padded_view(idx);
...
//                    axes_order.insert(axes_order.end(), outer_axes.begin(), outer_axes.end());
//
//                    result.outer_shape = outer_lengths;
//                    result.axes_order = axes_order;
...
//                std::cout << " ]\n";
//                std::cout << "fft_size:  " << fft_size << "\n";
//                std::cout << "buffer_size: " << info.buffer_size << "\n";
//
//                if (fft_size <= 0)
//                {
//                    return;
...
//                }
//                std::cout << " ]\n";
//                const auto& outer_shape = info.outer_shape;
//                std::cout << "outer_shape: [";
//                for (auto a : outer_shape)
//                {
//                    std::cout << " " << a;
//                }
//                std::cout << " ]\n";
...
//                std::cout << " ]\n";
//
//                Flatten_indexing outer_indexing{outer_shape};
//                const int64_t outer_size = outer_indexing.size();
//                std::cout << "outer_size: " << outer_size << "\n";
//                const auto& axes_order = info.axes_order;
//                std::cout << "axes_order: [";
//                for (auto a : axes_order)
//                {
//                    std::cout << " " << a;
//                }
//                std::cout << " ]\n";
//                const auto transposed_data = get_transposed_data(complex_input_data_ptr,
//                                                                 reverse_shape(input_data_shape),
//                                                                 axes_order);
//                std::vector<complex_type> transposed_result(fft_size * outer_size);
//                std::cout << "fft_size * outer_size: " << transposed_result.size() << "\n";
//                std::cout << "transposed_data: [";
//                for (auto a : transposed_data)
//                {
//                    std::cout << " " << a;
//                }
//                std::cout << " ]\n";
...
//                complex_type* transposed_result_ptr = transposed_result.data();
//
//                std::vector<complex_type> t_data(fft_size);
//                std::vector<complex_type> t_buffer(info.buffer_size);
//
//                const auto permuted_input_shape = permute_vector(reverse_shape(input_data_shape),
//                                                                 axes_order);
//                std::vector<int64_t> permuted_fft_lengths(permuted_input_shape.begin(),
//                                                          permuted_input_shape.begin() + fft_rank);
//                const auto permuted_fft_strides = compute_strides(permuted_fft_lengths);
//                std::cout << "permuted_input_shape: [";
//                for (auto a : permuted_input_shape)
//                {
//                    std::cout << " " << a;
//                }
//                std::cout << " ]\n";
//                std::cout << "permuted_fft_lengths: [";
//                for (auto a : permuted_fft_lengths)
//                {
//                    std::cout << " " << a;
//                }
//                std::cout << " ]\n";
//                std::cout << "permuted_fft_strides: [";
//                for (auto a : permuted_fft_strides)
//                {
//                    std::cout << " " << a;
//                }
//                std::cout << " ]\n";
...
//                    const auto outer_coords = outer_indexing(outer_idx);
...
//
//                    // Copying current data to transform
//                    copy_data_from_input(t_data.data(),
//                                         transposed_data_ptr,
//                                         fft_lengths,
//                                         input_fft_lengths);
//                    std::cout << "Got input t_data: [";
//                    for (auto a : t_data)
//                    {
//                        std::cout << " " << a;
//                    }
//                    std::cout << " ]\n";
//
//                    bool t_data_is_zero = blob_is_zero(t_data.data(), fft_size);
//                    std::cout << "t_data " << (t_data_is_zero ? "is zero" : "is not zero") << "\n";
//
//                    if (!blob_is_zero(data.data(), fft_size))
//                    {
//                        std::cout << "Blob is not zero.\n";
