...
void random_uniform(const uint64_t* out_shape,
    ...
    for (size_t k = 0; k < elem_count; k += step) {
        ...
        case ngraph::element::Type_t::f32: {
            ...
            // [min_val, max_val)
            std::transform(res.data(), res.data() + 4, res_float, [&mn, &mx](uint32_t elem) {
        ...
        case ngraph::element::Type_t::f16: {
            ...
            memcpy(mn, min_val, elem_type.size());
            memcpy(mx, max_val, elem_type.size());
            std::transform(res.data(), res.data() + 4, res_float16, [&mn, &mx](uint32_t elem) {
                return uint32_to_float16(elem) * (mx[0] - mn[0]) + mn[0];
            });
            memcpy(out + k * elem_type.size(), res_float16, std::min((size_t)4, elem_count - k) * elem_type.size());
            break;
        }
        case ngraph::element::Type_t::bf16: {
            ...
            // [min_val, max_val)
            std::transform(res.data(), res.data() + 4, res_bfloat16, [&mn, &mx](uint32_t elem) {
                return uint32_to_bfloat16(elem) * (mx[0] - mn[0]) + mn[0];
            });
            memcpy(out + k * elem_type.size(), res_bfloat16, std::min((size_t)4, elem_count - k) * elem_type.size());
            break;
        }
        case ngraph::element::Type_t::f64: {
            ...
            memcpy(out + k * elem_type.size(), res_double, std::min((size_t)2, elem_count - k) * elem_type.size());
            break;
        }
        case ngraph::element::Type_t::i32: {
            ...
            // [min_val, max_val)
            std::transform(res.data(), res.data() + 4, res_int, [&mn, &mx](uint32_t elem) {
        ...
        case ngraph::element::Type_t::i64: {
            ...
            memcpy(out + k * elem_type.size(), res_int64, std::min((size_t)2, elem_count - k) * elem_type.size());
            break;
        }
        default:
