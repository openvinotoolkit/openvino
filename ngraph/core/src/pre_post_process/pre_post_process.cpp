...
struct PreProcessActionBase {
    virtual std::shared_ptr<ngraph::Node> apply(std::shared_ptr<ngraph::Node> node,
                                                InTensorData& state) = 0;
};
...
struct PreProcessConvertElementType: public PreProcessActionBase {
    ...
    std::shared_ptr<ngraph::Node> apply(std::shared_ptr<ngraph::Node> node,
                                        InTensorData& state) override {
...
struct PreProcessScale: public PreProcessActionBase {
    ...
    std::shared_ptr<ngraph::Node> apply(std::shared_ptr<ngraph::Node> node,
        ...
        if (m_values.size() == 1) {
            ...
            NGRAPH_CHECK(layouts::has_channels(state.m_layout),
                         "Can't apply scale for unknown channels layout");
        ...
        auto constant = ngraph::op::v0::Constant::create(ngraph::element::f32,
                                                         shape,
                                                         m_values);
...
std::shared_ptr<Function> PrePostProcessorBase::build() {
    ...
    for (auto in_context: m_impl->in_contexts) {
        if (in_context.is_default()) {
            ...
            auto new_param = std::make_shared<ngraph::op::v0::Parameter>(
                    new_param_type, new_param_shape);
...
InTensorContext::InTensorContext(std::unique_ptr<PrePostProcessorInternalData>&& impl): PrePostProcessorBase(std::move(impl)) {}
...
PreProcessContext::PreProcessContext(std::unique_ptr<PrePostProcessorInternalData>&& impl): PrePostProcessorBase(std::move(impl)) {}
PreProcessContext::PreProcessContext(PreProcessContext&&) = default;

PreProcessContext& PreProcessContext::scale(float value) {
    m_impl->last_in().m_preprocess_data.m_actions.push_back(std::make_shared<PreProcessScale>(std::vector<float>{value}));
    std::cout << "\nAdd scale: " << value;
    return *this;
}
...
InNetworkContext::InNetworkContext(std::unique_ptr<PrePostProcessorInternalData>&& impl): PrePostProcessorBase(std::move(impl)) {}
...
OutProcessor::OutProcessor(std::unique_ptr<PrePostProcessorInternalData>&& impl): PrePostProcessorBase(std::move(impl)) {}
