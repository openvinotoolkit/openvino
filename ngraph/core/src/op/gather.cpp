...
int64_t op::v8::Gather::get_axis() const
{
...
bool op::v8::Gather::evaluate(const HostTensorVector& outputs, const HostTensorVector& inputs) const
{
    NGRAPH_OP_SCOPE(v8_Gather_evaluate);
...
bool op::v8::Gather::evaluate_lower(const HostTensorVector& output_values) const
{
...
bool op::v8::Gather::evaluate_upper(const HostTensorVector& output_values) const
{
    if (!input_value(1).get_tensor().has_and_set_bound() ||
        !input_value(2).get_tensor().has_and_set_bound())
        return false;
...
bool op::v8::Gather::constant_fold(OutputVector& output_values, const OutputVector& input_values)
{
...
NGRAPH_RTTI_DEFINITION(op::v7::Gather, "Gather", 7, op::v8::Gather);

// todo: check constructors and virtual methods
op::v7::Gather::Gather(const Output<Node>& data,
                       const Output<Node>& indices,
                       const Output<Node>& axis,
                       const int64_t batch_dims)
    : op::v8::Gather(data, indices, axis, batch_dims)
{
    constructor_validate_and_infer_types();
}
...
NGRAPH_RTTI_DEFINITION(op::v1::Gather, "Gather", 1, op::v7::Gather);
...
int64_t ngraph::op::v1::Gather::get_axis() const
{
    ...
    return ngraph::op::v8::Gather::get_axis();
}
