        ...
        else
        {
            ...
            if (max_rank > 2)
            {
                ...
                if (arg0_rank != arg1_rank)
                {
                    ...
                    // resize low_size_matrix (with 0's) to have the same dimension as
                    // big_size_matrix
                    low_size_matrix.resize(low_size_matrix.size() + delta_rank, 0);
                    // move added 0's values to the beginning
...
void ngraph::op::v0::MatMul::validate_and_infer_types()
{
    ...
    {
        // Rank for matrices A and B shall be >= 1
