...
{
    NGRAPH_OP_SCOPE(v0_Multiply_evaluate) return multiplyop::evaluate_multiply(inputs[0], inputs[1], outputs[0], get_autob());
    return false;
}
...
bool op::v1::Multiply::evaluate(const HostTensorVector& outputs,
                                const HostTensorVector& inputs) const
{
    NGRAPH_OP_SCOPE(v1_Multiply_evaluate) return multiplyop::evaluate_multiply(inputs[0], inputs[1], outputs[0], get_autob());
    return false;
}
