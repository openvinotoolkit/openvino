...
{
    NGRAPH_OP_SCOPE(v0_TopK_validate_and_infer_types,
...
#define NGRAPH_CASE_GET_K(NAME, TYPE, ...)                                                         \
    ...
    size_t read_k_from_host_tensor(const HostTensorPtr& arg_k)
    {
        ...
        {
            NGRAPH_CASE_GET_K(read_k_from_host_tensor, i8, arg_k)
            NGRAPH_CASE_GET_K(read_k_from_host_tensor, i16, arg_k)
            NGRAPH_CASE_GET_K(read_k_from_host_tensor, i32, arg_k)
            NGRAPH_CASE_GET_K(read_k_from_host_tensor, i64, arg_k)
            NGRAPH_CASE_GET_K(read_k_from_host_tensor, u8, arg_k)
            NGRAPH_CASE_GET_K(read_k_from_host_tensor, u16, arg_k)
            NGRAPH_CASE_GET_K(read_k_from_host_tensor, u32, arg_k)
            NGRAPH_CASE_GET_K(read_k_from_host_tensor, u64, arg_k)
        default:
...
{
    NGRAPH_OP_SCOPE(v0_TopK_evaluate,
...
#if defined(__clang__) && !defined(OV_SELECTIVE_BUILD)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wswitch-enum"
#endif
    switch (static_cast<element::Type_t>(k_element_type))
    {
    NGRAPH_CASE(ngraph_op_v1_TopK_read_k_from_constant_node, i8,
        k = validate_and_get_k<int8_t>(k_constant); break;
    )
    NGRAPH_CASE(ngraph_op_v1_TopK_read_k_from_constant_node, i32,
        k = validate_and_get_k<int32_t>(k_constant); break;
    )
    NGRAPH_CASE(ngraph_op_v1_TopK_read_k_from_constant_node, i64,
        k = validate_and_get_k<int64_t>(k_constant); break;
    )
    default: break;
    }
#if defined(__clang__) && !defined(OV_SELECTIVE_BUILD)
...
void op::v3::TopK::validate_and_infer_types()
{
    NGRAPH_OP_SCOPE(v3_TopK_validate_and_infer_types,
        NODE_VALIDATION_CHECK(this,
                            ...
                            get_input_element_type(1));
        return op::v1::TopK::validate_and_infer_types();
    )
    NODE_VALIDATION_CHECK(this, false, "Function is not included into the selective build.");
}
...
bool op::v3::TopK::evaluate(const HostTensorVector& outputs, const HostTensorVector& inputs) const
{
    ...
    NGRAPH_OP_SCOPE(v3_TopK_evaluate,
        rc = op::v1::TopK::evaluate(outputs, inputs);
    )
    return rc;
}
