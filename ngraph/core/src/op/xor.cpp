...
{
    NGRAPH_OP_SCOPE(v1_LogicalXor_evaluate) return logxor::evaluate_logxor(inputs[0], inputs[1], outputs[0], get_autob());
    return false;
}
...
{
    NGRAPH_OP_SCOPE(v0_Xor_evaluate) return logxor::evaluate_logxor(inputs[0], inputs[1], outputs[0], get_autob());
    return false;
}
