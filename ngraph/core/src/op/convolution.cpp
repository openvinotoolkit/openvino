...
OPENVINO_RTTI_DEFINITION(op::v1::Convolution, "Convolution", "opset1", op::Op, 1);
...
void op::v1::Convolution::validate_and_infer_types() {
    ...
    ov::Shape result_shape = validate_and_infer_convolution_forward_output_shape(this,
                                                                                 result_ps_rank,
                                                                                 data_batch_pshape,
                                                                                 filters_pshape,
                                                                                 m_auto_pad,
...
OPENVINO_RTTI_DEFINITION(op::v1::ConvolutionBackpropData, "ConvolutionBackpropData", "opset1", op::Op, 1);
...
const ov::Shape op::v1::ConvolutionBackpropData::get_output_shape() const {
    ...
    if (data_pshape.rank().is_static()) {
        ...
        shape = ov::Shape{vector<Dimension>(filter_pshape.rank().get_length() - 2)};
    } else {
...
void op::v1::ConvolutionBackpropData::validate_and_infer_types() {
    ...
    ov::Shape output_spatial_pshape = get_output_shape();
