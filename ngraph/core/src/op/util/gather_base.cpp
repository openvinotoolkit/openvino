...
bool evaluate(const ngraph::HostTensorPtr& arg0,
    ...
    ov::StaticShape out_shape(params_shape.size() + indices_shape.size() - 1 - batch_dims);
    int64_t i = 0;
    for (; i < axis; i++) {
...
bool cf_gather_with_subgraph(ov::OutputVector& output_values,
    ...
    if (indices_shape.empty()) {
        // gathering a scalar
        const auto axis_const = ngraph::op::Constant::create(ov::element::i64, ov::StaticShape{1}, {0});
