...
{
    NGRAPH_OP_SCOPE(v0_Reshape_validate_and_infer_types,
...
{
    NGRAPH_OP_SCOPE(v0_Reshape_visit_attributes,
...
{
    bool rc = false;
    NGRAPH_OP_SCOPE(v0_Reshape_evaluate,
        rc = evaluate_reshape(inputs[0], outputs[0], get_input_order());
    )
    return rc;
}
