...
bool op::Quantize::evaluate(const HostTensorVector& output_values,
                            const HostTensorVector& input_values) const
{
    const auto& data = input_values[0];
    const auto& scale = input_values[1];
    const auto& zero_point = input_values[2];

    const auto& out = output_values[0];

    if (get_element_type() == element::i8)
    {
        runtime::reference::quantize<float, int8_t>(data->get_data_ptr<float>(),
                                                    scale->get_data_ptr<float>(),
                                                    zero_point->get_data_ptr<int8_t>(),
                                                    out->get_data_ptr<int8_t>(),
                                                    data->get_shape(),
                                                    scale->get_shape(),
                                                    get_axes(),
                                                    get_round_mode());
    }
    else if (get_element_type() == element::u8)
    {
        runtime::reference::quantize<float, uint8_t>(data->get_data_ptr<float>(),
                                                     scale->get_data_ptr<float>(),
                                                     zero_point->get_data_ptr<uint8_t>(),
                                                     out->get_data_ptr<uint8_t>(),
                                                     data->get_shape(),
                                                     scale->get_shape(),
                                                     get_axes(),
                                                     get_round_mode());
    }
    else if (get_element_type() == element::i32)
    {
        runtime::reference::quantize<float, int32_t>(data->get_data_ptr<float>(),
                                                     scale->get_data_ptr<float>(),
                                                     zero_point->get_data_ptr<int32_t>(),
                                                     out->get_data_ptr<int32_t>(),
                                                     data->get_shape(),
                                                     scale->get_shape(),
                                                     get_axes(),
                                                     get_round_mode());
