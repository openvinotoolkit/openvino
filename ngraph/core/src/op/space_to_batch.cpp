...
OPENVINO_RTTI_DEFINITION(op::v1::SpaceToBatch, "SpaceToBatch", "opset2", op::Op, 1);
...
void op::v1::SpaceToBatch::validate_and_infer_types() {
    ...
    if (block_const && pads_begin_const && pads_end_const && data_pshape.is_static()) {
        ...
        ov::StaticShape output_shape = {static_cast<size_t>(data_shape[0] * block_prod)};
...
bool ngraph::op::v1::SpaceToBatch::evaluate_space_to_batch(const HostTensorVector& outputs,
    ...
    ov::StaticShape padded_shape(data_shape.size());
    ...
    ov::StaticShape dispersed_shape(block_values_size + 1);
