...
#include "ngraph/runtime/reference/space_to_batch.hpp"
                                            ...
                                            const HostTensorVector& inputs) const
{
    ...
    if (!(inputs[0]->get_shape().size() == 4 || inputs[0]->get_shape().size() == 5))
    {
        return false;
    }
    ...
    runtime::reference::space_to_batch(inputs[0]->get_data_ptr<char>(),
                                       inputs[1]->get_data_ptr<const int64_t>(),
                                       inputs[2]->get_data_ptr<const int64_t>(),
                                       inputs[3]->get_data_ptr<const int64_t>(),
                                       pad_value,
