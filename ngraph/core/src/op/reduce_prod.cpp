...
namespace
{
                          ...
                          bool keep_dims)
    {
        ...
        {
            NGRAPH_TYPE_CASE(ReduceProd_evaluate_product, i32, arg, out, axes, keep_dims)
            NGRAPH_TYPE_CASE(ReduceProd_evaluate_product, i64, arg, out, axes, keep_dims)
            NGRAPH_TYPE_CASE(ReduceProd_evaluate_product, u32, arg, out, axes, keep_dims)
            NGRAPH_TYPE_CASE(ReduceProd_evaluate_product, u64, arg, out, axes, keep_dims)
            NGRAPH_TYPE_CASE(ReduceProd_evaluate_product, f16, arg, out, axes, keep_dims)
            NGRAPH_TYPE_CASE(ReduceProd_evaluate_product, f32, arg, out, axes, keep_dims)
                                  ...
                                  const HostTensorVector& inputs) const
{
    ...
    NGRAPH_OP_SCOPE(v1_ReduceProd_evaluate,
        rc = evaluate_product(inputs[0], outputs[0], get_reduction_axes(), get_keep_dims());
    )
    return rc;
}
