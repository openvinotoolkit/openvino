...
OPENVINO_RTTI_DEFINITION(ov::op::v0::Constant, "Constant", "opset1");
...
ov::op::v0::Constant::Constant(const element::Type& type, const ngraph::Shape& shape)
    : m_element_type(type),
      m_shape(shape) {
    allocate_buffer();
    constructor_validate_and_infer_types();
...
ov::op::v0::Constant::Constant(const element::Type& type, const ngraph::Shape& shape, const void* data)
    : Constant(type, shape) {
...
ov::op::v0::Constant::Constant(const Constant& other, const ngraph::Shape& new_shape) {
...
ngraph::Shape ov::op::v0::Constant::get_shape_val() const {
    NGRAPH_CHECK(m_element_type.is_integral_number());
    std::vector<int64_t> out_shape = cast_vector<int64_t>();
    ngraph::Shape output_shape(shape_size(m_shape));
...
void ov::op::v0::Constant::set_data_shape(const ngraph::Shape& shape) {
...
bool ov::op::v0::Constant::visit_attributes(AttributeVisitor& visitor) {
    ...
    ngraph::Shape prev_shape = m_shape;
