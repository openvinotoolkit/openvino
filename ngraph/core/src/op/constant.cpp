...
void op::Constant::need_alloc_buffer_on_visit_attrubutes(bool val)
{
    m_need_alloc_buffer_on_visit_attrubutes = val;
}
...
bool op::v0::Constant::visit_attributes(AttributeVisitor& visitor)
{
    ...
    element::Type prev_element_type = m_element_type;

    visitor.on_attribute("element_type", m_element_type);
    visitor.on_attribute("shape", m_shape);

    bool need_to_reallocate = (m_shape != prev_shape || m_element_type != prev_element_type);

    if (m_need_alloc_buffer_on_visit_attrubutes && need_to_reallocate)
    {
