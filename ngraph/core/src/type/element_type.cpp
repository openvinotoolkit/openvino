...
struct ElementTypes {
    struct TypeHash {
        size_t operator()(element::Type_t t) const {
...
element::Type::Type(size_t bitwidth, bool is_real, bool is_signed, bool is_quantized, const std::string& /* cname */) {
...
size_t ngraph::compiler_byte_size(element::Type_t et) {
    switch (et) {
#define ET_CASE(et)           \
    case element::Type_t::et: \
        return sizeof(element_type_traits<element::Type_t::et>::value_type);
        ET_CASE(boolean);
        ET_CASE(bf16);
        ET_CASE(f16);
        ET_CASE(f32);
