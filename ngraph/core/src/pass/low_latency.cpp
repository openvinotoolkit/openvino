...
bool pass::LowLatency_v2::run_on_function(shared_ptr<Function> f) {
    ...
    for (const auto& op : f->get_ops()) {
        if (const auto& sub_graph_op = dynamic_pointer_cast<op::util::SubGraphOp>(op)) {
            ...
            {
                const auto& trip_count = dynamic_pointer_cast<Constant>(loop->get_input_node_shared_ptr(0));
                const auto& num_iter = loop->get_num_iterations();
                if (trip_count && num_iter > 0 && trip_count->get_output_target_inputs(0).size() == 1)
                {
                    auto iterations = std::make_shared<Constant>(element::i64, Shape{}, m_iterations);
            ...
            for (const auto& in : sub_graph_op->get_input_descriptions()) {
                ...
                if (const auto& merged_in = dynamic_pointer_cast<op::util::SubGraphOp::MergedInputDescription>(in)) {
                    ...
                    const string &var_name = generate_variable_name(sub_graph_op->get_friendly_name(),
                                                                    param_name,
                                                                    variable_id);
                    ...
                    bool is_assign_found = std::any_of(out_desc.begin(), out_desc.end(),
