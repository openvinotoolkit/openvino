...
class XmlSerializer : public ngraph::AttributeVisitor {
    ...
    std::vector<std::string> map_type_from_body(const pugi::xml_node& xml_node, const std::string& map_type) {
    ...
    void input_descriptions_on_adapter(
        const std::vector<std::shared_ptr<ngraph::op::util::SubGraphOp::InputDescription>>& input_descriptions,
    ...
    void output_descriptions_on_adapter(
        const std::vector<std::shared_ptr<ngraph::op::util::SubGraphOp::OutputDescription>>& output_descriptions,
...
public:
    ...
    void on_adapter(const std::string& name, ngraph::ValueAccessor<std::shared_ptr<ov::Function>>& adapter) override {
        if (name == "body") {
...
std::string get_opset_name(const ngraph::Node* n, const std::map<std::string, ov::OpSet>& custom_opsets) {
    ...
    auto opsets = std::array<std::reference_wrapper<const ov::OpSet>, 8>{ngraph::get_opset1(),
                                                                         ngraph::get_opset2(),
                                                                         ngraph::get_opset3(),
                                                                         ngraph::get_opset4(),
                                                                         ngraph::get_opset5(),
...
bool resolve_dynamic_shapes(const ngraph::Function& f) {
    ...
    if (std::all_of(f_ops.begin(), f_ops.end(), [](std::shared_ptr<ov::Node> results) {
