...
bool ngraph::pass::FreezeNodes::run_on_function(std::shared_ptr<ngraph::Function> f) {
    NGRAPH_CHECK(m_nodes_to_freeze.size() == m_replacing_values.size(), "FreezeNodes transformation failed. "
    ...
    for (const auto &node : f->get_ops()) {
        ...
        if (node_to_freeze != m_nodes_to_freeze.end()) {
            ...
            NGRAPH_CHECK(node->outputs().size() == m_replacing_values[idx].size(), "FreezeNodes transformation failed."
                                                                                   "Values are not provided for all node outputs."
                                                                                   "Node", node->get_friendly_name(),
                                                                                   " has ", node->outputs().size(),
