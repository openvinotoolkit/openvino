...
ResultVector ngraph::as_result_vector(const OutputVector& values) {
    ...
    for (auto value : values) {
        ...
        result.push_back(ov::is_type<op::Result>(node) ? ov::as_type_ptr<op::Result>(node) : make_shared<op::Result>(value));
    }
    return result;
}
...
bool Node::constant_fold(OutputVector& output_values, const OutputVector& input_values) {
    ...
    for (const auto& input : input_values) {
        auto host_tensor = make_shared<runtime::HostTensor>(ov::as_type_ptr<op::v0::Constant>(input.get_node_shared_ptr()));
