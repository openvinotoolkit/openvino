...
void Function::validate_nodes_and_infer_types() const
{
    ...
    bool only_pairs = std::all_of(pair_checker.begin(), pair_checker.end(),
                                  [](const std::pair<Variable*, Counter>& val) {
        return val.second.cnt_assign == 1 && val.second.cnt_read_val == 1;
    });
    if (!only_pairs)
        throw ngraph_error("Function is incorrect. Assign and ReadValue operations must be in pairs on the network.");
}
