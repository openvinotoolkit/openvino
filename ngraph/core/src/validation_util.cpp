...
//
// Infers the output shape for reduction operations.
//
PartialShape ngraph::infer_reduction_output_shape(const Node* node,
                                                  ...
                                                  const bool keep_dims)
{
    ...
    const auto& axes = get_constant_from_source(node->input_value(1));
    if (data_rank.is_static() && axes)
    {
        ...
        for (auto axis : reduction_axes_val)
        {
            try
            {
                axis = normalize_axis(node, axis, data_rank);
            }
