...
<<<<<<< HEAD:runtime/bindings/python/src/compatibility/openvino/inference_engine/ie_api.py
            if np.isfortran(arr):
...
=======
            if arr is not None:
                ...
                arr = np.empty(0, dtype=precision_map[precision])
>>>>>>> 721515efd... fix setup.py building:runtime/bindings/python/src/openvino/ie_api.py
        else:
