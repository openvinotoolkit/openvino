...
void regclass_graph_FrontEndManager(py::module m) {
    ...
    fem.def(py::pickle(
        [](const ngraph::frontend::FrontEndManager&) {
            return py::make_tuple(0);
        },
        [](py::tuple t) {
            return ngraph::frontend::FrontEndManager();
        }));
    ...
    fem.def(
        "load_by_model",
        [](const std::shared_ptr<ngraph::frontend::FrontEndManager>& fem, const std::string& model_path) {
