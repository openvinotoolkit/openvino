...
void util::DictAttributeDeserializer::on_adapter(const std::string& name, ov::ValueAccessor<void>& adapter) {
    if (m_attributes.contains(name)) {
        if (const auto& a = ov::as_type<
                ...
                &adapter)) {
            std::vector<std::shared_ptr<ov::op::util::MultiSubGraphOp::InputDescription>> input_descs;
            if (name == "input_descriptions") {
                ...
                for (py::handle h : slice_input_desc) {
                    ...
                    auto slice_in = std::make_shared<ov::op::util::MultiSubGraphOp::SliceInputDescription>(
                ...
                for (py::handle h : merged_input_desc) {
                    const py::dict& desc = h.cast<py::dict>();
                    auto merged_in = std::make_shared<ov::op::util::MultiSubGraphOp::MergedInputDescription>(
                        desc["input_idx"].cast<int64_t>(),
                        desc["body_parameter_idx"].cast<int64_t>(),
                ...
                for (py::handle h : invariant_input_desc) {
                    const py::dict& desc = h.cast<py::dict>();
                    auto invariant_in = std::make_shared<ov::op::util::MultiSubGraphOp::InvariantInputDescription>(
                        desc["input_idx"].cast<int64_t>(),
                        desc["body_parameter_idx"].cast<int64_t>());
            ...
            if (name == "output_descriptions") {
                ...
                for (py::handle h : body_output_desc) {
                    const py::dict& desc = h.cast<py::dict>();
                    auto body_output = std::make_shared<ov::op::util::MultiSubGraphOp::BodyOutputDescription>(
                        desc["body_value_idx"].cast<int64_t>(),
                ...
                for (py::handle h : concat_output_desc) {
                    const py::dict& desc = h.cast<py::dict>();
                    auto concat_output = std::make_shared<ov::op::util::MultiSubGraphOp::ConcatOutputDescription>(
                        desc["body_value_idx"].cast<int64_t>(),
                        desc["output_idx"].cast<int64_t>(),
                        desc["start"].cast<int64_t>(),
                        desc["stride"].cast<int64_t>(),
