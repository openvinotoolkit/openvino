...
from openvino.descriptor import Tensor as TensorDescriptor
...
def test_evaluate():
    ...
    input1 = np.ndarray((2, 1), dtype=np.float32, buffer=np.array([2, 1]))
    input2 = np.ndarray((2, 1), dtype=np.float32, buffer=np.array([3, 7]))
